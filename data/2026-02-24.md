...
TL;DR: 开发了COMPASS系统，利用AI驱动的时序数字孪生架构，通过每分次PET、CT、剂量组学等数据建模正常组织生物学动态过程，预测放疗毒性，在小型NSCLC患者队列中验证了可行性。


<details>
  <summary>Details</summary>
Motivation: 当前放疗越来越精确和数据密集，但临床决策仍主要依赖静态、基于群体的NTCP模型，忽略了序列数据中编码的动态、独特的生物学轨迹。需要开发能够捕捉个体化生物学响应的动态建模方法。
...
Result: 在8名接受BGRT的NSCLC患者的99个器官分次观察数据中，发现了可行的AI驱动早期预警窗口，风险评分在临床毒性出现前几个分次就开始升高。密集的BED驱动表征揭示了在毒性发生前出现的生物学相关空间剂量纹理特征。

Conclusion: COMPASS为AI驱动的自适应放疗建立了概念验证，其中治疗由持续更新的数字孪生指导，该数字孪生跟踪每个患者不断演变的生物学响应。系统能够捕捉传统体积剂量学平均掉的生物学相关特征。
...
TL;DR: MARVUS是一个移动增强现实容积超声系统，通过增强现实可视化技术，在常规2D超声设备上实现准确、可重复的病灶容积测量，显著提高测量精度并降低用户间变异性。


<details>
  <summary>Details</summary>
Motivation: 2D超声是乳腺和甲状腺成像的首选一线模态，但基于2D超声的容积估计存在高用户间变异性。现有3D超声解决方案需要专用探头或外部跟踪硬件，增加了成本和便携性限制，限制了临床广泛应用。

Method: 开发了移动增强现实容积超声系统，与常规超声系统互操作，使用基础模型增强跨专业泛化能力，同时相对于现有3D超声解决方案最小化硬件要求。

Result: 在经验丰富的临床医生对乳腺模型进行的用户研究中，MARVUS显著提高了容积估计精度（平均差异：0.469 cm³），并降低了用户间变异性（平均差异：0.417 cm³）。增强现实可视化提高了客观性能指标和临床医生报告的使用性。

Conclusion: MARVUS能够以可扩展、成本敏感和资源高效的方式增强基于超声的癌症筛查、诊断工作流程和治疗规划，提高容积评估的准确性和可重复性。
...
TL;DR: 该研究系统评估了特征解耦方法在医学影像中缓解捷径学习的效果，发现结合数据重平衡和模型解耦的方法能有效提升分类性能和解耦质量。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学影像中常依赖捷径学习，利用与目标任务无因果关系的虚假相关性或混杂因素，这在需要跨机构、人群和采集条件泛化的临床环境中存在风险。

Method: 系统评估了特征解耦方法，包括对抗学习和基于依赖最小化的潜在空间分割，在一个人工数据集和两个医学数据集（包含自然和合成混杂因素）上评估分类性能和解耦质量，并检查了不同混杂程度下的鲁棒性和计算效率。

Result: 捷径缓解方法在训练期间存在强虚假相关性时改善了分类性能；潜在空间分析揭示了分类指标未捕捉到的表示质量差异；模型对捷径的依赖程度取决于训练数据中的混杂程度；最佳性能模型结合了数据中心的重新平衡和模型中心的解耦。

Conclusion: 结合数据重平衡和模型解耦的方法比单独使用重平衡能实现更强、更稳健的捷径缓解，同时保持相似的计算效率，为医学影像中的可靠深度学习模型提供了实用指导。
...
TL;DR: 开发基于单摄像头广播视频的足球分析系统，使用YOLO检测器和ByteTrack跟踪算法提取球员、裁判、守门员和球的轨迹数据，为预算有限的球队提供专业级分析能力。


<details>
  <summary>Details</summary>
Motivation: 专业俱乐部通过昂贵的多摄像头或GPS系统获得竞争优势，而低预算球队无法收集类似数据。本研究旨在探索是否可以从标准广播视频中提取这些数据，降低技术门槛。

Method: 开发端到端系统，结合YOLO目标检测器和ByteTrack跟踪算法，从单摄像头广播视频中识别和跟踪球员、裁判、守门员和球。

Result: 系统在检测和跟踪球员和裁判方面表现出色，具有较高的精确率、召回率和mAP50分数。球检测仍是主要挑战，但整体证明可以从单摄像头提取有意义的球员空间信息。
...
<details>
  <summary>Details</summary>
Motivation: 尽管基于光学物理的散焦模型已经存在，但从散焦图像集合中恢复深度仍然是一个计算上具有挑战性的优化问题。作者旨在证明，利用现代优化方法和合理的计算资源，全局优化方法在深度散焦恢复问题上是可行的。

Method: 采用交替最小化方法：1) 固定深度图时，前向模型相对于全聚焦图像是线性的，使用凸优化求解；2) 固定全聚焦图像时，每个像素的深度可以独立计算，实现高度并行化的网格搜索。通过交替进行凸优化和并行网格搜索来解决深度散焦问题。

Result: 该方法在合成和真实散焦模糊的基准数据集上取得了有希望的结果，能够在比当前深度学习方法更高的分辨率下有效解决深度散焦问题。

Conclusion: 通过交替最小化结合凸优化和并行网格搜索，可以实现高效的全局优化方法来解决深度散焦问题，在计算效率和分辨率方面优于现有方法。
...
TL;DR: Sketch2Feedback是一个语法循环框架，用于对学生绘制的STEM图表提供及时、符合评分标准的反馈。该框架通过四个阶段分解问题，使用语言模型仅描述上游规则引擎验证的违规情况，以减少幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 在STEM教育中，及时提供符合评分标准的学生绘图反馈是一个持续挑战。虽然大型多模态模型可以同时解析图像和生成解释，但其幻觉倾向会破坏课堂部署的信任度。

Method: 提出Sketch2Feedback框架，包含四个阶段：混合感知、符号图构建、约束检查和受限VLM反馈。该框架将语言模型限制为仅描述上游规则引擎验证的违规情况，而不是生成完整的解释。

Result: 在FBD-10和Circuit-10两个合成微基准测试中，结果呈现混合情况：Qwen2-VL-7B在两个数据集上都达到最高的micro-F1分数，但幻觉率极高。语法管道在电路反馈方面产生更可操作的反馈（4.85/5 vs 3.11/5）。置信度阈值在tau=0.7时可将电路幻觉从0.970降至0.880且无F1损失。

Conclusion: 语法循环框架和端到端LMM方法之间存在可利用的互补性。语法管道能产生更可操作的反馈，而端到端LMM在某些指标上表现更好但幻觉严重。硬噪声增强揭示了领域依赖的鲁棒性差异。
...
TL;DR: 研究评估了合成图像对目标检测性能的影响，通过对比不同数据集、生成器和增强比例，发现合成增强在复杂场景中效果显著，但传统生成指标与检测性能的相关性有限。


<details>
  <summary>Details</summary>
Motivation: 合成图像越来越多地用于增强目标检测训练集，但在训练前可靠评估合成数据集仍然困难：标准的全局生成指标（如FID）通常不能预测下游检测性能。需要系统评估合成增强在不同检测场景中的效果。

Method: 在三个单类别检测场景（交通标志、城市行人、COCO盆栽植物）上，对六种基于GAN、扩散和混合的生成器进行控制评估。使用YOLOv11模型，从零开始训练和COCO预训练初始化，增强比例从10%到150%。采用匹配大小的bootstrap协议计算预训练数据集指标，包括Inception-v3和DINOv2嵌入的全局特征空间指标，以及边界框统计的对象中心分布距离。

Result: 合成增强在更具挑战性的场景中带来显著提升（行人和盆栽植物分别相对提升+7.6%和+30.6%），但在交通标志数据集和预训练微调下效果有限。指标与性能的对齐强烈依赖于场景，许多原始关联在控制增强水平后减弱。

Conclusion: 合成增强的有效性高度依赖于具体检测场景，传统生成指标与检测性能的相关性有限且场景依赖。需要开发更精细的评估方法来预测合成数据对下游任务的实际价值。
...
TL;DR: JAEGER框架将音频-视觉大语言模型扩展到3D空间，通过RGB-D和多声道一阶Ambisonics实现联合空间定位与推理，提出神经强度向量增强方向估计，创建SpatialSceneQA基准进行训练评估。


<details>
  <summary>Details</summary>
Motivation: 当前音频-视觉大语言模型主要局限于2D感知，依赖RGB视频和单声道音频，这种设计存在维度不匹配问题，无法在复杂3D环境中实现可靠的源定位和空间推理。

Method: 提出JAEGER框架，整合RGB-D观测和多声道一阶Ambisonics；核心贡献是神经强度向量，一种学习型空间音频表示，可编码鲁棒方向线索以增强到达方向估计；创建SpatialSceneQA基准，包含61k指令调优样本。

Result: 广泛实验表明，该方法在各种空间感知和推理任务中始终优于2D中心基线，强调了显式3D建模对于在物理环境中推进AI的必要性。

Conclusion: JAEGER框架成功将AV-LLMs扩展到3D空间，通过神经强度向量和RGB-D/多声道音频整合实现了更好的空间定位和推理能力，为物理环境中的AI发展提供了重要方向。
...
TL;DR: 该研究比较了10种深度学习架构对土耳其5种黑橄榄品种的图像分类性能，发现EfficientNetV2-S准确率最高（95.8%），EfficientNetB0在准确率和计算复杂度之间取得最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 在有限数据条件下，需要评估不同深度学习架构对土耳其本地黑橄榄品种自动分类的性能，确定最适合实际应用的模型。

Method: 使用2500张图像数据集，采用迁移学习训练10种深度学习架构（MobileNetV2、EfficientNetB0、EfficientNetV2-S、ResNet50、ResNet101、DenseNet121、InceptionV3、ConvNeXt-Tiny、ViT-B16、Swin-T），从多个维度评估模型性能。

Result: EfficientNetV2-S获得最高分类准确率（95.8%），EfficientNetB0在准确率和计算复杂度之间表现最佳。结果表明在有限数据条件下，参数效率比模型深度更重要。

Conclusion: 深度学习架构能够有效实现黑橄榄品种的自动分类，EfficientNet系列模型在准确率和计算效率方面表现优异，为实际应用提供了有价值的参考。
...
Result: 从研究中提炼出12个关键发现，形成了构建强大VLA模型的实用方法。开发出的VLANeXt模型在LIBERO和LIBERO-plus基准测试中超越了先前的最先进方法，并在真实世界实验中表现出强大的泛化能力。

Conclusion: 通过系统分析VLA设计空间，提出了构建有效VLA模型的实用指南，并开发了高性能的VLANeXt模型。将发布统一的代码库作为社区共享平台，促进VLA研究的进一步发展。
...
TL;DR: 研究发现形态学压力在文本到图像生成管道的多个层面创建了可导航梯度，通过特征描述符和语音形态学都能在扩散模型潜在空间中形成系统性导航路径


<details>
  <summary>Details</summary>
Motivation: 探索形态学结构（特征描述符和语音形态）如何在文本到图像生成模型中创建可导航的梯度，理解扩散模型潜在空间中的系统性结构

Method: 研究1：使用形态描述符（如"铂金色头发"、"美人痣"、"1950年代魅力"）在Stable Diffusion 1.5中导航身份盆地，采用自蒸馏循环生成合成图像并训练LoRA；研究2：基于语音形态学理论，生成200个无意义词汇，测试语音形态承载词汇与随机控制组在视觉一致性上的差异

Result: 研究1：训练后的LoRA创建了局部坐标系，不仅能塑造目标身份，还能产生其逆效果；研究2：语音形态承载候选词比随机控制组产生显著更视觉一致的输出（平均Purity@1 = 0.371 vs. 0.209），三个候选词（snudgeoid、crashax、broomix）实现了完美的视觉一致性

Conclusion: 形态学结构（无论是特征描述符还是提示层面的语音形态）在扩散模型潜在空间中创建了系统性导航梯度，揭示了身份盆地的相变、CFG不变的身份稳定性以及从亚词汇声音模式中涌现的新视觉概念
...
TL;DR: Rodent-Bench是一个评估多模态大语言模型标注啮齿动物行为视频能力的新基准，测试发现当前最先进模型（如Gemini-2.5-Pro、Gemini-2.5-Flash、Qwen-VL-Max）在此任务上表现不足，无法作为可靠助手。
...
Result: 评估的顶级MLLMs（Gemini-2.5-Pro、Gemini-2.5-Flash、Qwen-VL-Max）整体表现不佳，无法作为此任务的可靠助手；某些模型在特定数据集（如梳理检测）上表现尚可；发现模型在时间分割、处理长视频序列和区分细微行为状态方面存在显著挑战。

Conclusion: Rodent-Bench揭示了当前MLLMs在科学视频标注方面的关键局限性，为未来模型开发提供了见解，可作为跟踪神经科学研究中可靠自动化行为标注进展的基础框架。
...
TL;DR: 该论文评估了多种YOLO架构在花卉检测任务上的性能，比较了单图像单边界框和单图像多边界框两种标注方案，并引入了包含6种花卉物种的FloralSix数据集。


<details>
  <summary>Details</summary>
Motivation: 精确的花卉定位和识别对于自动化农业至关重要，特别是在植物表型分析、作物估算和产量监测方面。需要评估不同检测模型在不同标注密度下的性能。

Method: 使用YOLOv5s、YOLOv8n/s/m和YOLOv12n等架构，在FloralSix数据集上评估两种标注方案：SISBB（单图像单边界框）和SIMBB（单图像多边界框）。数据集包含2,816张高分辨率花卉照片，标注了密集和稀疏两种场景。评估指标包括精确率、召回率和mAP。
...
Conclusion: 标注密度、IoU阈值和模型大小之间存在相互作用：召回率优化的模型在拥挤环境中表现更好，而精确率导向的模型在稀疏场景中表现最佳。这些密度敏感的检测器对非破坏性作物分析、生长跟踪、机器人授粉和胁迫评估很有帮助。
...
TL;DR: 该研究系统评估了Vision Transformers在医学图像分类中不同patch size的影响，发现较小patch size（1、2、4）能显著提升性能，但会增加计算成本；通过简单集成策略可进一步提升效果。


<details>
  <summary>Details</summary>
Motivation: 虽然Vision Transformers在计算机视觉任务中已成为最先进技术，但patch size这一关键初始设计选择的影响在医学领域尚未得到充分探索，特别是在同时存在2D和3D成像模态的情况下。

Method: 使用12个医学影像数据集（7个2D和5个3D），在单个GPU上评估不同patch size（1、2、4、7、14、28）对ViT分类性能的影响，并采用简单集成策略融合patch size为1、2、4的模型预测。

Result: 较小patch size（1、2、4）在几乎所有数据集上都获得最佳分类性能：2D数据集平衡准确率最高提升12.78%（patch size 2 vs. 28），3D数据集最高提升23.78%（patch size 1 vs. 14）。集成策略在大多数情况下能进一步提升性能，特别是对2D数据集。

Conclusion: 在医学图像分类中，较小的patch size能显著提升Vision Transformers的性能，尽管会增加计算成本；通过简单集成策略可进一步优化结果，为医学影像分析中的ViT设计提供了重要指导。
...
TL;DR: 提出了一种从静态图像、语音特征和目标文本生成逼真说话人脸的新方法，通过多纠缠潜在空间实现音频和视频模态的联合生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成逼真说话人脸时面临模态对齐和个性化特征保持的挑战，需要同时处理文本、图像和语音信息来生成协调的音频和视频输出。

Method: 模型编码驱动文本、驱动图像和个体语音特征，通过多纠缠潜在空间建立模态间的时空个性化特征关联，生成键值对和查询用于音频和视频生成管道，最后通过各自解码器输出音频和视频。

Result: 该方法能够从静态图像、语音特征和文本输入生成协调一致的说话人脸视频和对应音频，实现了跨模态的个性化特征保持。

Conclusion: 提出的多纠缠潜在空间方法有效解决了说话人脸生成中的模态对齐问题，为从多源输入生成逼真说话内容提供了有效框架。
...
TL;DR: Video-TwG：一种课程强化框架，采用Think-with-Grounding范式，让视频LLM在文本-视频交错推理中按需进行视觉定位，选择性放大问题相关片段，解决长视频理解中的幻觉问题。
...
Method: 提出Video-TwG框架，采用Think-with-Grounding范式，让模型在推理过程中主动决定何时进行按需视觉定位。设计了Two-stage Reinforced Curriculum Strategy：第一阶段在小规模短视频GQA数据集上学习定位行为，第二阶段扩展到多样化通用QA数据以促进泛化。还提出了TwG-GRPO算法，包含细粒度定位奖励、自确认伪奖励和准确度门控机制。构建了新的TwG-51K数据集。

Result: 在Video-MME、LongVideoBench和MLVU等基准测试中，Video-TwG始终优于强基线方法。消融实验验证了两阶段课程强化策略的必要性，并显示TwG-GRPO能更好地利用未标记数据提高定位质量，减少冗余定位而不牺牲QA性能。

Conclusion: Video-TwG通过Think-with-Grounding范式和课程强化策略，有效解决了长视频理解中的幻觉问题，提高了模型对复杂多模态线索的分析能力，在多个基准测试中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型在密集几何SLAM方面取得进展，但缺乏深度语义理解和鲁棒的闭环能力；同时，当前语义建图方法常受限于解耦架构和脆弱的数据关联。
...
Conclusion: IRIS-SLAM通过统一的几何-实例表示和语义协同关联机制，有效解决了现有SLAM系统在语义理解和闭环检测方面的局限性，为语义SLAM提供了新的解决方案。
...
TL;DR: 该论文提出了一种名为HIME的训练免费方法，通过层自适应权重编辑来减少大型视觉语言模型中的物体幻觉问题，同时保留预训练知识。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在物体幻觉问题，即描述不存在的物体或提供错误事实信息，这影响了模型在现实世界中的可靠部署。虽然微调是常用的缓解策略，但其计算成本高且实施困难，因此需要训练免费的替代方案。

Method: 首先对基于Qwen、LLaMA和Vicuna三种大型语言模型构建的LVLM解码器进行系统分析，揭示不同层对物体幻觉的敏感性差异。然后提出幻觉不敏感分数（HIS）来量化每层对幻觉的敏感性，并基于此提出幻觉不敏感模型编辑（HIME）方法，这是一种层自适应权重编辑方法，选择性地修改潜在特征以抑制幻觉同时保留预训练知识。

Result: 实验表明，HIME在开放式生成基准测试（包括CHAIR、MME和GPT-4V辅助评估）中平均减少了61.8%的幻觉，且不引入额外参数、推理延迟或计算开销。

Conclusion: HIME是一种简单有效的训练免费方法，能够显著减少大型视觉语言模型的物体幻觉问题，同时保持模型的预训练知识，为可靠的多模态理解提供了有前景的解决方案。
...
TL;DR: WiCompass是一个针对毫米波人体姿态估计的覆盖感知数据收集框架，通过构建通用姿态空间"预言机"来识别代表性不足的动作，采用闭环策略优先收集信息丰富的缺失样本，在相同预算下显著提升分布外鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 毫米波人体姿态估计虽然具有隐私保护优势，但在分布偏移下泛化能力差。现有方法通过暴力扩展数据规模来提升鲁棒性效果有限，真正的瓶颈在于数据收集的效率和覆盖范围。

Method: WiCompass框架包含两个核心组件：1）利用大规模动作捕捉语料库构建通用姿态空间"预言机"，量化数据集冗余并识别代表性不足的动作；2）采用闭环策略，基于预言机的指导优先收集信息丰富的缺失样本。

Result: 实验表明，WiCompass在相同预算下能持续提升分布外准确性，相比传统数据收集策略展现出更优的扩展行为，验证了从暴力扩展转向覆盖感知数据收集的有效性。

Conclusion: 通过将重点从暴力扩展数据规模转向覆盖感知的数据获取，WiCompass为构建鲁棒的毫米波感知系统提供了一条实用路径，解决了毫米波人体姿态估计在分布偏移下的泛化问题。
...
TL;DR: MiSCHiEF：包含安全和文化两个领域的细粒度图像-文本对齐基准数据集，评估视觉语言模型在细微差异场景下的对齐能力


<details>
  <summary>Details</summary>
Motivation: 在安全和文化等社会关键场景中，细微的视觉或语言线索对正确理解至关重要，而当前视觉语言模型在细粒度图像-文本对齐方面存在挑战，需要评估模型处理微妙语义和视觉差异的能力

Method: 提出MiSCHiEF基准数据集，包含安全（MiS）和文化（MiC）两个子集，采用对比对设计，每个样本包含两个最小差异的文本描述和对应的最小差异图像，评估四个视觉语言模型在细粒度区分任务上的表现

Result: 模型在确认正确图像-文本对方面表现优于拒绝错误对；在给定图像时从两个高度相似的文本中选择正确文本的准确率高于反向任务；整体结果显示当前视觉语言模型在模态对齐方面仍存在挑战

Conclusion: 当前视觉语言模型在需要精确跨模态基础的细粒度对齐任务中仍面临困难，特别是在处理微妙语义和视觉差异的应用场景中，需要进一步改进模型的模态对齐能力
...
<details>
  <summary>Details</summary>
Motivation: 现有3D形状补全方法通常需要特定类别的训练数据，难以处理多样化的部分观测模式。本文旨在开发一种零样本、类别无关的方法，利用3D基础模型的强大几何先验，实现对各种部分观测的高质量补全。

Method: 采用两阶段设计：1）显式替换阶段：保留部分观测的几何结构，确保补全的忠实性；2）隐式细化阶段：确保观测区域与合成区域之间的边界无缝衔接。该方法无需训练，兼容不同的3D基础模型。

Result: 在Omni-Comp基准测试（结合真实世界和合成数据，包含多样化的挑战性部分模式）上，LaS-Comp在定量和定性评估中都优于先前的最先进方法。

Conclusion: LaS-Comp通过利用3D基础模型的几何先验，实现了零样本、类别无关的高质量3D形状补全，其两阶段设计和训练无关的特性使其在实际应用中具有广泛适用性。
...
TL;DR: GeoCode是一个通过合成复杂多模态几何问题构建的数据集，采用符号种子构造、接地实例化验证和代码渲染的生成流程，并通过代码预测作为对齐目标提升模型几何推理能力。
...
Method: 提出三步生成流程：1) 符号种子构造；2) 接地实例化与验证；3) 基于代码的图表渲染。引入代码预测作为显式对齐目标，将视觉理解转化为结构化预测任务。

Result: GeoCode数据集在结构复杂性和推理难度上显著超越现有基准，通过多阶段验证确保数学正确性。在多个几何基准测试中，使用GeoCode训练的模型获得一致改进。

Conclusion: GeoCode数据集及其代码预测对齐策略有效提升了多模态几何推理能力，为复杂几何问题求解提供了高质量训练资源。
...
TL;DR: MIRROR框架通过视觉区域反思的多模态迭代推理，减少幻觉并提高视觉语言模型的推理准确性


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理模糊或复杂视觉输入时容易产生幻觉或逻辑错误，即使进行"反思"也难以与图像证据建立联系，需要更有效的视觉基础推理机制

Method: 提出MIRROR框架，采用包含草稿、批判、区域验证和修订的闭环过程；构建ReflectV数据集用于多轮监督训练，包含反思触发器、区域验证动作和视觉证据基础的答案修订

Result: 在通用视觉语言基准和代表性视觉语言推理基准上的实验表明，MIRROR提高了正确性并减少了视觉幻觉

Conclusion: 将反思训练为证据寻求、区域感知的验证过程而非纯文本修订步骤具有重要价值，MIRROR框架通过视觉区域反思有效提升了多模态推理能力
...
TL;DR: 本研究提出了一种基于注意力图的基准测试方法，评估10个基础模型在4个组织病理学数据集上的像素级语义分割性能，发现视觉语言模型CONCH表现最佳，多模型特征融合可显著提升分割效果。


<details>
  <summary>Details</summary>
Motivation: 虽然CLIP、DINO、CONCH等基础模型在多种图像任务中展现出卓越的领域泛化和无监督特征提取能力，但针对组织病理学像素级语义分割的系统性独立评估仍然缺乏。本研究旨在填补这一空白，为组织病理学分割任务提供可靠的模型评估基准。

Method: 提出了一种鲁棒的基准测试方法：利用基础模型的注意力图作为像素级特征，然后使用XGBoost机器学习算法进行分类。这种方法无需微调，能够快速、可解释且模型无关地评估10个基础模型在4个组织病理学数据集上的性能，涵盖形态组织区域和细胞/核分割任务。

Result: 视觉语言基础模型CONCH在所有数据集中表现最佳，视觉专用模型PathDino紧随其后。分析表明，在不同组织病理学队列上训练的模型能够捕捉互补的形态学表征，将它们的特征拼接能够获得更优的分割性能。CONCH、PathDino和CellViT特征拼接后，在所有数据集上的平均性能提升了7.95%。

Conclusion: 基础模型集成能够更好地泛化到多样化的组织病理学分割任务中。视觉语言模型在组织病理学分割中表现出优势，多模型特征融合策略可以显著提升分割性能，为组织病理学图像分析提供了有效的评估框架和实用方法。
...
TL;DR: EditedID提出了一种用于人脸编辑中身份保持的框架，通过对齐-解耦-耦合策略解决跨源分布偏差和特征污染问题，实现训练免费、即插即用的身份恢复。


<details>
  <summary>Details</summary>
Motivation: 当前多模态编辑大模型在人脸编辑中存在面部身份一致性下降的问题，由于人眼对脸部特征高度敏感，这种不一致性严重阻碍了模型的实际部署。现有方法难以同时恢复面部身份和编辑元素的一致性，主要受跨源分布偏差和跨源特征污染的限制。

Method: 提出EditedID框架，包含三个核心组件：1）自适应混合策略，在扩散过程中对齐跨源潜在表示；2）混合求解器，解耦源特定的身份属性和细节；3）注意力门控机制，选择性耦合视觉元素。该框架通过系统分析扩散轨迹、采样器行为和注意力特性来实现。

Result: 大量实验表明，EditedID在保持原始面部身份和编辑元素一致性方面达到了最先进的性能。作为一个训练免费、即插即用的解决方案，为开放世界场景中的单/多人脸身份恢复建立了新的基准。

Conclusion: EditedID解决了人脸编辑中长期存在的身份一致性问题，为多模态编辑大模型在真人编辑场景中的实际部署铺平了道路。该方法在保持身份一致性和编辑效果方面表现出色，具有重要的实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶系统通常学习单一平均驾驶风格，忽略了人类驾驶行为的天然多样性。个性化自动驾驶面临三个挑战：缺乏个体标注的真实数据集、缺少量化评估个人驾驶风格的指标、以及缺乏从用户轨迹学习风格化表示的算法。

Method: 提出Person2Drive平台，包含：1）开源灵活的数据收集系统，模拟真实场景生成可扩展的个性化驾驶数据集；2）基于风格向量的评估指标，使用最大均值差异和KL散度全面量化个体驾驶行为；3）个性化端到端自动驾驶框架，带有风格奖励模型，能有效适配端到端模型以实现安全个性化驾驶。

Result: 大量实验表明，Person2Drive能够实现细粒度分析、可重复评估和端到端自动驾驶中的有效个性化。数据集和代码将在论文被接受后发布。

Conclusion: Person2Drive为解决个性化端到端自动驾驶的三个关键挑战提供了全面解决方案，通过创新的数据收集、评估指标和算法框架，实现了对个体驾驶风格的准确建模和适配。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言模型在面部表情识别中产生的解释往往缺乏视觉基础，容易产生幻觉，与视觉证据联系薄弱，导致在不同数据集上的鲁棒性较差。

Method: 提出TAG框架，要求中间推理步骤必须基于面部动作单元相关的面部区域进行。通过监督微调AU基础的推理轨迹，然后使用AU感知奖励进行强化学习，使预测区域与外部AU检测器对齐。

Result: 在RAF-DB、FERPlus和AffectNet数据集上，TAG一致优于强大的开源和闭源VLM基线，同时提高了视觉可信度。消融和偏好研究显示AU基础的奖励稳定了推理过程并减少了幻觉。

Conclusion: 结构化、有基础的中间表示对于面部表情识别中可信的多模态推理至关重要，AU基础的推理方法能够提高模型的鲁棒性和可解释性。
...
TL;DR: 本文提出了GeoLink-UV数据集，这是一个全国范围的高分辨率城中村制图产品，覆盖342个中国城市，解决了城中村识别缺乏统一可靠数据集的问题。


<details>
  <summary>Details</summary>
Motivation: 城中村作为中国快速城市化中的高密度非正规居住区，准确识别对城市治理、更新和可持续发展至关重要。但由于城中村在全国范围内的显著异质性和多样性，一直缺乏一致可靠的全国性数据集。

Method: 使用多源地理空间数据（包括光学遥感影像和地理矢量数据），通过基础模型驱动的制图框架生成数据集，解决了泛化问题并提高了产品质量。基于28个城市的独立样本进行地理分层精度评估。

Result: 创建了全国范围的城中村数据集，精度评估证实了其在异质城市环境中的可靠性和科学性。分析显示城中村平均占建成区面积的8%，在中部和南部地区显著聚集。建筑级分析确认了全国一致的"低层高密度"开发模式，同时突出了区域差异化的形态特征。
...
TL;DR: 提出ZS-MIL方法，使用VLM文本编码器的类别嵌入作为分类层初始化，解决MIL问题中随机初始化导致性能低于零样本预测的问题


<details>
  <summary>Details</summary>
Motivation: 在WSI分类中，随机初始化分类器权重会导致性能低于零样本预测，特别是在少样本学习场景下。需要更好的初始化方法来提升MIL框架的性能

Method: 提出ZS-MIL方法，利用VLM文本编码器的类别嵌入作为分类层的初始化起点，计算样本袋级概率，替代传统的随机初始化

Result: 实验表明ZS-MIL在少样本场景下相比其他权重初始化技术，在性能和稳定性方面都表现出更好的鲁棒性

Conclusion: ZS-MIL方法有效解决了MIL问题中随机初始化性能不足的问题，为少样本WSI分类提供了更可靠的初始化方案
...
TL;DR: MaskDiME是一个快速、有效的扩散框架，通过局部化采样统一语义一致性和空间精度，实现高效的反事实解释生成。
...
Method: 提出MaskDiME框架，通过局部化采样自适应聚焦于决策相关区域，实现局部化和语义一致的反事实生成，同时保持高图像保真度。
...
<details>
  <summary>Details</summary>
Motivation: 大规模文本到图像扩散模型与细微人类偏好的对齐仍然具有挑战性。虽然直接偏好优化（DPO）简单有效，但大规模微调通常存在泛化差距。受测试时引导启发，将偏好对齐视为分类器自由引导（CFG），在采样过程中使用微调的偏好模型作为外部控制信号。

Method: 提出一种简单方法，无需重新训练基础模型即可改善对齐效果。为进一步增强泛化能力，将偏好学习解耦为两个分别在正负数据上训练的模块，在推理时通过减去它们的预测（正减负）形成对比引导向量，按用户选择的强度缩放并在每个步骤添加到基础预测中。

Result: 在Stable Diffusion 1.5和Stable Diffusion XL上使用Pick-a-Pic v2和HPDv3进行评估，显示出一致的数量和质量提升。

Conclusion: 该方法通过对比引导实现了更清晰可控的对齐信号，有效提升了扩散模型与人类偏好的对齐效果，同时保持了方法的简单性和无需重新训练基础模型的优势。
...
TL;DR: LMP提出了一种双分支检测器，通过学习多模态原型，将文本引导与目标域视觉样本结合，在跨域少样本目标检测中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的开放词汇检测器主要依赖文本提示，虽然能很好地进行语义迁移，但缺乏目标域特定的视觉信息，导致在少样本监督下难以实现精确定位。

Method: 提出LMP双分支检测器：1）视觉原型构建模块从支持RoIs聚合类级原型，并通过抖动框在查询图像中动态生成硬负原型；2）视觉引导分支将原型注入检测流程；3）文本引导分支保留开放词汇语义；4）双分支联合训练，推理时集成语义抽象与域自适应细节。

Result: 在六个跨域基准数据集和标准1/5/10-shot设置下，该方法达到了最先进或极具竞争力的mAP性能。

Conclusion: 通过耦合文本引导与视觉样本，LMP能够同时利用语义抽象和域自适应视觉细节，有效解决了跨域少样本目标检测中视觉信息不足的问题。
...
Conclusion: RobSelf为处理真实世界错位数据的跨模态超分辨率问题提供了一种有效的完全自监督解决方案，无需训练数据或监督信息，具有实际应用价值。
...
TL;DR: 4DSTAR是一个生成时空一致的4D对象的自回归模型，通过动态时空状态传播和4D VQ-VAE实现高质量4D生成
...
Method: 提出4DSTAR模型，包含两个关键组件：1) 动态时空状态传播自回归模型(STAR)，将预测token按时间步分组，通过时空容器传播历史状态特征来指导下一时间步生成；2) 4D VQ-VAE，将4D结构隐式编码到离散空间，并将STAR预测的离散token解码为时间一致的动态3D高斯

Result: 实验表明4DSTAR能够生成时空一致的4D对象，性能与扩散模型相当

Conclusion: 4DSTAR通过时空状态传播机制解决了现有方法在4D生成中的时空不一致问题，实现了高质量的时空一致4D对象生成
...
<details>
  <summary>Details</summary>
Motivation: 随着隐私和法律问题限制真实生物特征数据的使用，合成数据成为训练人脸识别系统的实用替代方案。然而，现有的身份条件扩散模型存在类内多样性有限的问题，而类内多样性对于训练鲁棒且可泛化的人脸识别模型至关重要。

Method: 提出IDPERTURB方法，这是一种简单有效的几何驱动采样策略。该方法在单位超球面的约束角度区域内扰动身份嵌入，生成多样化的嵌入向量，而无需修改底层生成模型。每个扰动后的嵌入作为预训练扩散模型的条件向量，合成视觉多样但身份一致的人脸图像。

Result: 实验结果表明，使用IDPERTURB生成的合成数据集训练人脸识别模型，在多个基准测试中相比现有合成数据生成方法获得了更好的性能表现。
...
TL;DR: 提出轻量级卷积自编码器CLAP用于植物病害分类，在三个公开数据集上取得改进或竞争性精度，仅需500万参数，训练时间20毫秒，推理时间1毫秒/图像。
...
Method: 提出轻量级自编码器CLAP，在编码器-解码器块中使用可分离卷积层，应用Sigmoid门控机制增强编码器特征判别能力，解码器进一步改进特征，最后结合编码器-解码器特征图进行丰富特征表示后分类。

Result: 在Integrated Plant Disease、Groundnut和CCMT三个公开植物数据集上获得改进或竞争性准确率，平衡了性能与计算成本，仅需500万参数，训练时间20毫秒，推理时间1毫秒/图像。

Conclusion: CLAP是一种轻量高效的植物病害分类方法，在真实田间条件下能有效处理植物健康、病害和营养缺乏的细微变化，计算成本低且性能优越。
...
TL;DR: 提出一种为多模态大语言模型生成版权触发器的框架，通过双重注入所有权相关语义信息来创建触发图像，可在微调衍生模型中引发所有权相关文本响应，实现模型溯源追踪。
...
Method: 提出双重注入框架：1) 将图像作为可学习张量，通过强制辅助MLLM输出与预定义所有权目标文本一致，反向传播一致性损失注入所有权信息；2) 在语义层面最小化图像CLIP特征与目标文本CLIP特征的距离。还引入对抗训练阶段，训练辅助模型抵抗生成所有权相关文本，增强在重度微调衍生模型中的鲁棒性。

Result: 广泛的实验表明，该双重注入方法在各种微调和领域转移场景下能有效追踪模型谱系，证明其在模型溯源方面的有效性。

Conclusion: 该框架为MLLM提供了一种生成版权触发器的方法，使模型发布者能够将可验证的所有权信息嵌入模型中，为解决模型知识产权争议提供了技术方案。
...
TL;DR: DUET-VLM提出了一种双阶段视觉令牌压缩框架，通过视觉编码器冗余感知压缩和语言主干中基于文本引导的层间令牌丢弃，在保持高准确率的同时大幅减少视觉令牌数量。
...
Method: 提出双阶段压缩框架：1) 视觉编码器输出进行冗余感知压缩，生成信息保留的令牌；2) 在语言主干中基于文本引导进行层间视觉令牌丢弃，逐步剪除信息量较少的令牌。

Result: 在LLaVA-1.5-7B上，保持超过99%基线准确率的同时减少67%令牌，即使减少89%令牌仍保持>97%准确率。在Video-LLaVA-7B上，甚至超越基线准确率，在减少53.1%令牌时达到>100%准确率。
...
TL;DR: 该论文提出了开放词汇语义分割中的领域泛化新设置(OVDG-SS)，针对自动驾驶场景构建首个基准测试，并提出S2-Corr机制来缓解领域偏移导致的文本-图像相关性失真问题。


<details>
  <summary>Details</summary>
Motivation: 传统领域泛化语义分割方法局限于固定类别，而开放词汇语义分割模型虽然能识别更广泛概念，但对领域偏移敏感，在自动驾驶等实际场景中表现不佳。需要同时解决未见领域和未见类别的挑战。

Method: 提出S2-Corr机制，这是一种基于状态空间的文本-图像相关性精炼方法，通过缓解领域诱导的失真，在分布变化下产生更一致的文本-图像相关性。

Result: 在构建的自动驾驶OVDG-SS基准测试上进行广泛实验，证明该方法相比现有开放词汇语义分割方法，在跨域性能和效率方面都表现更优。

Conclusion: 该研究首次定义了开放词汇领域泛化语义分割问题，为自动驾驶场景提供了首个基准测试，提出的S2-Corr机制有效解决了领域偏移导致的文本-图像相关性失真问题，提升了模型在未见领域和未见类别上的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有后训练量化或分块重建方法无法有效处理Vision Transformers中所有层和块间依赖的联合优化，特别是在极低比特设置下精度损失严重，限制了在边缘设备上的高效部署

Method: 1) 端到端联合量化：优化所有层和块间依赖，无需标注数据；2) 数据自由校准策略：使用Stable Diffusion Turbo合成多样化无标签样本，通过学习的多模式提示引导生成；3) 鼓励学习提示嵌入和生成图像特征的多样性

Result: 1) 在ImageNet上实现W4A4和W3A3的SOTA精度；2) 首次在ViT、DeiT和Swin-T模型上实现W1.58A8的PTQ结果并保持强精度；3) 数据自由方法性能与真实ImageNet校准相当，优于简单文本提示基线

Conclusion: 该框架为Vision Transformers提供了一种高效、可扩展的量化方法，特别适合边缘部署，数据自由校准策略解决了实际部署中训练数据不可用的问题
...
<details>
  <summary>Details</summary>
Motivation: LiDAR点云投影到2D时会产生稀疏的语义预测，这限制了最终的3D分割精度。需要解决投影过程中的稀疏性问题来提高整体性能。

Method: 开发MM2D3D多模态分割模型，采用两种关键技术：1) 跨模态引导滤波，利用相机图像的密集语义关系约束中间2D预测；2) 动态交叉伪监督，鼓励2D预测模拟相机图像的密集语义分布。

Result: 实验表明，该方法能生成分布更密集、精度更高的中间2D语义预测，有效提升了最终3D分割精度。在2D和3D空间都优于先前方法。

Conclusion: 通过相机图像辅助解决LiDAR投影的稀疏性问题，提出的多模态方法能显著提升3D语义分割性能，为城市遥感中的街景理解提供了有效解决方案。
...
TL;DR: 本文提出BiMotion框架，使用连续可微B样条曲线表示运动，解决现有文本引导3D角色生成方法中动作有限、不连贯的问题，通过Laplacian正则化B样条求解器压缩变长运动序列，并引入多种损失函数提升运动质量。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导动态3D角色生成方法存在局限性：由于固定长度时间输入和离散帧表示，往往只能生成有限子动作或不连贯运动，无法捕捉丰富的运动语义。

Method: 1. 使用连续可微B样条曲线表示运动；2. 提出封闭形式的Laplacian正则化B样条求解器，将变长运动序列压缩为固定数量控制点的紧凑表示；3. 引入输入形状保持的正常融合策略；4. 设计对应感知和局部刚性损失函数提升运动恢复质量；5. 构建BIMO数据集，包含多样化变长3D运动序列和高质量文本标注。

Result: BiMotion框架相比现有最先进方法能生成更具表现力、更高质量、更好对齐文本提示的运动，同时生成速度更快。在广泛评估中表现出色。

Conclusion: 通过连续B样条表示和高效压缩方法，BiMotion成功解决了文本引导3D角色生成中的运动质量和连贯性问题，无需修改底层生成模型的能力，实现了更有效的运动生成。
...
TL;DR: SLD-Font是一个结构级解耦的扩散模型，通过分离的内容和风格通道实现少样本中文字体生成，解决了现有方法特征级解耦导致的风格保真度下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有少样本中文字体生成方法仅实现特征级解耦，导致生成器可以重新纠缠内容和风格特征，造成内容扭曲和风格保真度下降。需要更有效的解耦机制来同时保证准确的内容渲染和忠实的风格迁移。

Method: 提出结构级解耦扩散模型(SLD-Font)，使用两个独立通道分别接收内容（SimSun风格图像作为模板）和风格（CLIP提取的特征）。通过交叉注意力整合风格特征，并训练背景噪声去除模块处理复杂笔画区域。基于解耦有效性理论验证，引入参数高效微调策略，仅更新风格相关模块。

Result: 实验结果表明，SLD-Font在保持与现有最先进方法相当的内容准确性的同时，实现了显著更高的风格保真度。引入的灰度和OCR指标能有效评估生成字符的内容质量。

Conclusion: SLD-Font通过结构级解耦机制有效解决了少样本中文字体生成中的内容和风格纠缠问题，在保持内容准确性的同时显著提升了风格保真度，为字体生成任务提供了新的解决方案。
...
Method: 提出FOCA框架，基于多模态大语言模型，通过交叉注意力融合模块整合RGB空间域和频域中的判别特征。同时构建FSE-Set大规模数据集，包含多样化的真实和篡改图像、像素级掩码和双域标注。

Result: 大量实验表明，FOCA在空间域和频域的检测性能和可解释性方面均优于最先进的方法。

Conclusion: FOCA通过融合空间和频域特征，有效解决了现有图像伪造检测方法过度依赖语义内容和可解释性有限的问题，为图像取证提供了更准确和可解释的解决方案。
...
TL;DR: SceneTok是一种用于3D场景表示和生成的新型tokenizer，它将场景的多视角信息编码为少量无序、与空间网格解耦的token，实现了1-3个数量级的压缩比，同时保持最先进的重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景表示和生成方法通常使用3D数据结构或视角对齐的场表示，这些方法存在压缩效率低、计算复杂度高等问题。需要一种更紧凑、高效的场景表示方法，既能实现高质量重建，又能支持快速场景生成。

Method: 提出SceneTok tokenizer，通过多视角tokenizer将场景信息编码为少量无序、与空间网格解耦的token。使用轻量级整流流解码器将token渲染为新视角。该方法支持从偏离输入轨迹的新轨迹进行渲染，并能优雅处理不确定性。

Result: 实现了1-3个数量级的压缩比提升，同时达到最先进的重建质量。能够在5秒内完成场景生成，相比先前范式实现了更好的质量-速度权衡。解码器能够处理不确定性，支持从新轨迹渲染。

Conclusion: SceneTok提供了一种高效、紧凑的3D场景表示方法，通过无序token编码实现了强大的压缩能力，同时支持高质量重建和快速场景生成，为3D场景表示和生成开辟了新范式。
...
TL;DR: PhysConvex：一种基于物理的3D动态凸体辐射场，统一视觉渲染与物理模拟，使用连续介质力学驱动的凸体基元表示可变形辐射场


<details>
  <summary>Details</summary>
Motivation: 现有神经表示（如NeRFs和3DGS）在视觉重建方面表现出色，但难以捕捉复杂材料变形和动态特性，需要同时实现视觉真实感和物理一致性的动态3D场景重建与模拟

Method: 1. 使用物理基础的凸体基元表示可变形辐射场；2. 引入边界驱动的动态凸体表示，通过顶点和表面动态建模变形；3. 开发降阶凸体模拟，使用神经蒙皮特征模态作为形变基函数，在牛顿动力学下驱动动态凸体场

Result: PhysConvex能够从视频中实现几何、外观和物理属性的高保真重建，性能优于现有方法，凸体动力学提供紧凑无间隙的体积覆盖，提升几何效率和模拟保真度

Conclusion: PhysConvex成功统一了视觉渲染和物理模拟，通过物理信息化的凸体表示有效解决了动态3D场景重建中视觉真实感与物理一致性的挑战
...
TL;DR: SCHEMA是一个专门为Google Gemini 3 Pro Image设计的结构化提示工程方法论，基于850个验证API预测和约4800张生成图像构建，包含三层渐进系统和模块化标签架构，在专业领域实现了高合规率和一致性。


<details>
  <summary>Details</summary>
Motivation: 针对现有通用提示指南或模型无关技巧的不足，需要为Google Gemini 3 Pro Image开发一个基于系统专业实践的结构化提示工程框架，以提供更精确的控制和可预测的结果。

Method: 开发了三层渐进系统（基础、中级、高级），包含7个核心和5个可选结构化组件的模块化标签架构，决策树路由规则，以及系统记录模型限制和对应解决方案。基于850个验证API预测和约4800张生成图像，涵盖六个专业领域。

Result: 在621个结构化提示中观察到91%的强制合规率和94%的禁止合规率；批量一致性测试显示结构化提示具有显著更高的代际一致性；40名从业者独立验证；信息设计验证显示在约300个公开可验证信息图中，空间和排版控制的首次生成合规率超过95%。

Conclusion: SCHEMA为Google Gemini 3 Pro Image提供了一个有效的结构化提示工程框架，显著提高了提示的合规性、一致性和可预测性，特别在专业图像生成领域表现出色。
...
TL;DR: 该论文提出了一种名为边缘化束调整（MBA）的新方法，将单目深度估计（MDE）集成到运动恢复结构（SfM）中，通过利用MDE的密度特性来降低其误差方差，从而在多视图3D视觉任务中实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然深度学习在单目深度估计（MDE）方面取得了显著进展，能够从单张图像准确估计深度，但将MDE集成到传统的运动恢复结构（SfM）中仍然面临挑战。MDE产生的密集深度图误差方差较大，而传统SfM依赖稀疏点云三角化，两者之间存在不匹配问题。

Method: 提出边缘化束调整（MBA）方法，受现代RANSAC估计器的启发，利用MDE深度图的密度特性来降低其误差方差。该方法能够将MDE深度信息有效整合到SfM流程中，处理从少量帧到数千张图像的大规模多视图系统。

Result: 实验表明，MBA方法使MDE深度图足够精确，能够在SfM和相机重定位任务中达到最先进或具有竞争力的结果。在不同规模的数据集上（从少量帧到数千张图像）都表现出稳定鲁棒的性能。

Conclusion: 该研究展示了MDE在多视图3D视觉中的巨大潜力，通过MBA方法成功解决了MDE与SfM集成的问题，为基于深度学习的3D重建提供了新的技术路径。
...
TL;DR: CRAFT-LoRA提出了一种改进的个性化图像生成方法，通过解耦内容与风格表示、语义扩展控制和训练自由的时间步相关引导，解决了现有LoRA组合技术中的纠缠、控制不足和融合不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于LoRA的个性化图像生成方法面临三个主要挑战：内容与风格表示的纠缠、对元素影响控制不足、以及权重融合不稳定需要额外训练。这些限制了精确控制和高效个性化生成的能力。

Method: CRAFT-LoRA包含三个互补组件：1) 秩约束的主干微调，注入低秩投影残差以鼓励学习解耦的内容和风格子空间；2) 提示引导方法，具有专门分支的专家编码器，通过选择性适配器聚合实现语义扩展和精确控制；3) 训练自由的时间步相关分类器自由引导方案，通过跨扩散步骤策略性调整噪声预测来增强生成稳定性。

Result: 该方法显著改善了内容-风格解耦，实现了对LoRA模块组合的灵活语义控制，并在无需额外重新训练开销的情况下实现了高保真度生成。

Conclusion: CRAFT-LoRA通过创新的解耦架构、语义控制机制和训练自由引导方案，有效解决了现有LoRA组合技术的局限性，为个性化图像生成提供了更精确、稳定和高效的方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言场景导航方法存在两个主要问题：多智能体方法协调成本高，单智能体方法在长视野任务中容易出现过载和指令漂移。需要一种既能保持高效协调又能避免认知过载的解决方案。

Method: DACo采用规划-执行解耦架构，包含全局指挥官（负责高层战略规划）和局部执行者（负责自我中心观察和细粒度执行）。框架集成了动态子目标规划和自适应重规划机制。

Result: 在R2R、REVERIE和R4R基准测试中，DACo在零样本设置下分别比最佳基线方法提升了4.9%、6.5%和5.4%的绝对性能，并能有效泛化到闭源和开源骨干模型。

Conclusion: DACo为鲁棒的长视野导航提供了一个原则性且可扩展的范式，通过解耦全局推理和局部行动，有效减轻了认知过载并提高了导航稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 创伤外科手术中实时手部追踪对于支持快速准确的术中决策至关重要，需要能够在复杂手术场景中同时定位手部并识别左右手。

Method: 提出基于YOLOv10的框架，采用多任务检测设计，在Trauma THOMPSON Challenge 2025 Task 2数据集上训练，使用广泛的数据增强技术提高对运动模糊、光照变化和不同手部外观的鲁棒性。

Result: 模型实现了准确的左右手分类（左手67%，右手71%），mAP_{[0.5:0.95]}达到0.33，保持实时推理能力，但区分手部与背景仍具挑战性。

Conclusion: 该工作为紧急外科手术中高级手部-器械交互分析奠定了基础，展示了在术中部署的潜力，特别是在实时性和分类准确性方面表现良好。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于RGB的YOLOv8模型在检测道床不足时存在局限性，虽然精度高（0.99）但召回率低（0.49），倾向于过度预测充足类别，影响安全性能

Method: 开发深度增强的YOLO-SAM2框架，包含：1）基于多项式建模、RANSAC和时间平滑的枕木对齐深度校正管道，补偿RealSense空间畸变；2）SAM2分割细化感兴趣区域掩码；3）准确提取枕木和道床轮廓进行几何分类

Result: 深度增强配置显著改善道床不足检测：召回率从0.49提升至最高0.80，F1分数从0.66提升至超过0.80，具体性能取决于边界框采样方法（AABB或RBB）和几何标准

Conclusion: 深度校正与YOLO-SAM2的集成为自动化铁路道床检查提供了更鲁棒可靠的方法，特别是在视觉模糊或安全关键场景中
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督前馈场景流估计方法使用两帧点对应关系进行监督，但这种监督在遮挡情况下不可靠且容易失效。多帧监督有潜力通过整合过去帧的运动线索提供更稳定的指导，但简单的两帧目标扩展无效，因为点对应关系在帧间变化剧烈，产生不一致信号。

Method: TeFlow引入时间集成策略，通过从多帧构建的候选池中聚合最时间一致的运动线索，形成可靠的监督信号。该方法使前馈模型能够利用多帧监督，而不是传统的两帧对应关系。

Result: 在Argoverse 2和nuScenes数据集上，TeFlow实现了自监督前馈方法的新SOTA，性能提升高达33%。与领先的基于优化的方法性能相当，但速度快150倍。

Conclusion: TeFlow通过时间集成策略成功实现了多帧监督，显著提升了自监督前馈场景流估计的性能和鲁棒性，在保持实时效率的同时达到了与优化方法相当的性能。
...
TL;DR: BayesFusion-SDF：一种基于CPU的概率符号距离融合框架，通过稀疏高斯随机场建模几何体，提供确定性几何重建和不确定性量化，适用于机器人、增强现实等应用。


<details>
  <summary>Details</summary>
Motivation: 传统体积融合技术（如TSDF）依赖启发式权重且无法系统性地传达不确定性；神经隐式方法虽然精度高但需要大量GPU计算且不易理解。需要一种既能提供高精度几何重建又能量化不确定性的透明方法。

Method: 1. 使用粗略TSDF重建创建自适应窄带域；2. 采用异方差贝叶斯公式融合深度观测；3. 使用稀疏线性代数和预条件共轭梯度求解；4. 通过随机对角估计器快速获取后验不确定性。

Result: 在受控消融场景和CO3D物体序列上的测试表明，该方法在几何精度上优于TSDF基线，并能提供有用的不确定性估计，支持主动感知中的表面提取和最佳视点规划。

Conclusion: BayesFusion-SDF为GPU密集型神经重建方法提供了一个透明、易用的概率替代方案，具有可解释性和可预测性，适用于需要不确定性感知的3D重建任务。
...
TL;DR: IDSelect是一个基于强化学习的成本感知选择器，为每个模态每个序列选择预训练模型，优化准确率与效率的权衡，显著减少计算资源消耗。


<details>
  <summary>Details</summary>
Motivation: 当前基于视频的人物识别系统在处理所有模态时使用固定的重量级集成模型，无论输入复杂度如何，浪费计算资源。需要一种能根据输入条件动态选择模型的方法来优化准确率与效率的平衡。

Method: 提出IDSelect，一个基于强化学习的成本感知选择器，使用actor-critic强化学习进行端到端训练，通过预算感知优化平衡识别准确率和计算成本，熵正则化防止过早收敛。推理时，策略为每个模态选择最可能的模型，融合模态特定相似度得到最终分数。

Result: 在CCVID数据集上达到95.9%的Rank-1准确率，比强基线减少92.4%的计算量，同时提高准确率1.8%；在MEVID数据集上减少41.3%的计算量，同时保持竞争性性能。

Conclusion: IDSelect通过输入条件选择器发现互补的模型选择，超越了固定集成模型，同时使用显著更少的资源，在视频人物识别任务中实现了优越的效率。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成能力强，但固有的顺序去噪过程导致推理速度慢。现有加速方法基于相邻时间步的特征距离进行缓存重用，但这种方法使用原始特征差异，将内容和噪声混在一起，忽略了频谱演化特性（低频结构早期出现，高频细节后期细化）。

Method: 提出Spectral-Evolution-Aware Cache (SeaCache)，这是一种无需训练的缓存调度方法。通过理论和实证分析，推导出频谱演化感知(SEA)滤波器，该滤波器保留内容相关组件同时抑制噪声。使用SEA滤波后的输入特征来估计冗余度，从而产生动态调度策略，既能适应内容特性，又能尊重扩散模型底层的频谱先验。

Result: 在多种视觉生成模型和基准测试上的广泛实验表明，SeaCache在延迟和质量之间实现了最先进的权衡。

Conclusion: SeaCache通过频谱演化感知的表示来决定特征重用，有效加速扩散模型推理，同时保持生成质量，为扩散模型的实际应用提供了高效的加速方案。
...
TL;DR: 提出基于条件二值分割的跨视角物体对应框架，通过循环一致性训练实现无监督学习，支持推理时训练，在Ego-Exo4D和HANDAL-X基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 研究视频中不同视角间的物体级视觉对应问题，特别是具有挑战性的自我中心到外部中心以及外部中心到自我中心的场景，需要解决视角变化带来的视觉差异。

Method: 提出基于条件二值分割的框架：将物体查询掩码编码为潜在表示，用于在目标视频中定位对应物体；引入循环一致性训练目标，将目标视图的预测掩码投影回源视图以重建原始查询掩码，提供自监督信号；支持推理时训练。

Result: 在Ego-Exo4D和HANDAL-X基准测试中证明了优化目标和TTT策略的有效性，实现了最先进的性能。

Conclusion: 提出的循环一致性训练框架能够有效解决跨视角物体对应问题，无需真实标注，通过自监督学习和推理时训练策略取得了优异性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统锚点引导生成模型依赖离散锚点词汇表，需要在词汇表大小和模型性能之间进行权衡，且测试时需要足够覆盖轨迹分布以确保鲁棒性。这种离散表示限制了模型的效率和鲁棒性。

Method: 提出MeanFuser方法，包含三个关键设计：1) 高斯混合噪声(GMN)引导生成采样，实现轨迹空间的连续表示；2) 均值流身份，建模GMN与轨迹分布之间的平均速度场而非瞬时速度场，消除ODE求解器数值误差；3) 轻量级自适应重建模块(ARM)，通过注意力权重隐式选择采样提案或在都不满意时重建新轨迹。

Result: 在NAVSIM闭环基准测试中，MeanFuser在没有PDM Score监督的情况下取得了优异性能，并展现出卓越的推理效率，为端到端自动驾驶提供了鲁棒高效的解决方案。

Conclusion: MeanFuser通过连续轨迹表示、改进的流匹配方法和自适应重建机制，克服了传统锚点引导生成模型的局限性，实现了更高效、更鲁棒的自动驾驶轨迹规划。
...
<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型强大的推理能力为高级个性化研究开辟了新前沿，但该领域进展严重受到缺乏合适基准测试的阻碍。现有基准无法满足真实世界应用中对复杂个性化任务评估的需求。

Method: 1. 引入Life-Bench：基于模拟用户数字足迹构建的综合性多模态基准测试，包含超过[数量未指定]个问题，评估从人物理解到历史数据复杂推理的广泛能力；2. 提出LifeGraph：端到端框架，将个人上下文组织成知识图谱，以促进结构化检索和推理。

Result: 实验显示现有方法在复杂个性化任务上表现显著不足，暴露出巨大的性能提升空间，特别是在关系推理、时间推理和聚合推理方面。LifeGraph通过利用结构化知识缩小了这一差距，展示了有前景的方向，但高级个性化任务仍是关键的开放挑战。

Conclusion: Life-Bench填补了高级个性化研究领域基准测试的空白，揭示了现有方法的局限性。LifeGraph框架展示了利用结构化知识改进个性化推理的潜力，但该领域仍需进一步研究以应对复杂个性化任务的挑战。
...
TL;DR: VBVR数据集是首个大规模视频推理数据集，包含200个推理任务和超过100万个视频片段，比现有数据集大三个数量级，并提供了可验证的评估框架VBVR-Bench，用于研究视频推理的扩展行为。


<details>
  <summary>Details</summary>
Motivation: 当前视频模型主要关注视觉质量，而推理能力研究不足。视频推理在时空一致的视觉环境中进行，能处理文本难以捕捉的连续性、交互性和因果关系，但缺乏大规模训练数据阻碍了系统性研究。

Method: 1) 构建VBVR数据集：包含200个精心设计的推理任务，遵循原则性分类法，超过100万个视频片段；2) 开发VBVR-Bench评估框架：采用基于规则、与人类对齐的评分器，而非基于模型的评判，实现可重复和可解释的诊断；3) 利用VBVR套件进行大规模扩展研究。

Result: VBVR数据集比现有数据集大约三个数量级，VBVR-Bench提供了可验证的评估框架。通过VBVR套件的研究发现了对未见推理任务的早期涌现泛化迹象。

Conclusion: VBVR为通用视频推理的下一阶段研究奠定了基础，数据集、基准工具包和模型已公开提供。
...
TL;DR: MoBind是一个层次化对比学习框架，用于学习IMU信号与2D姿态序列的联合表示，支持跨模态检索、时间同步、主体和身体部位定位以及动作识别。


<details>
  <summary>Details</summary>
Motivation: 需要解决三个关键挑战：过滤不相关的视觉背景、建模结构化多传感器IMU配置、实现细粒度亚秒级时间对齐，以实现IMU信号与视频姿态序列的有效跨模态学习。

Method: 采用层次化对比学习框架，将IMU信号与骨骼运动序列对齐而非原始像素；将全身运动分解为局部身体部位轨迹，与对应IMU配对实现多传感器对齐；使用分层对比策略，先对齐token级时间片段，再融合局部（身体部位）对齐与全局（全身）运动聚合。
...
Conclusion: MoBind通过层次化对比学习有效解决了IMU与姿态序列跨模态学习的关键挑战，实现了细粒度时间对齐和多传感器建模，为运动感知应用提供了强大的跨模态表示学习框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前模型在粗分辨率下难以推断组织微结构，而通过微超声对前列腺癌进行非侵入性分级可以加速分诊并指导活检定位到最具侵袭性的区域。
...
TL;DR: TokenTrace是一个新颖的主动水印框架，用于解决生成式AI模型中的多概念知识产权归属问题，通过在语义域嵌入秘密签名并支持查询式概念检索。


<details>
  <summary>Details</summary>
Motivation: 生成式AI模型能够复制独特的艺术风格和概念而不给予归属，对知识产权构成重大挑战。现有水印方法在复杂场景中（如单个图像包含多个概念组合）难以解耦和归属每个概念。

Method: TokenTrace通过同时扰动文本提示嵌入和初始潜在噪声，在语义域嵌入秘密签名。检索时使用基于查询的TokenTrace模块，接收生成图像和指定需要检索概念的文本查询作为输入，从而解耦和独立验证单个图像中的多个概念。

Result: 大量实验表明，该方法在单概念（对象和风格）和多概念归属任务上均达到最先进性能，显著优于现有基线方法，同时保持高视觉质量和对常见变换的鲁棒性。

Conclusion: TokenTrace为解决生成式AI中的多概念知识产权归属问题提供了一个有效框架，能够在复杂场景中实现鲁棒的多概念归属，同时保持生成质量。
...
### [64] [NI-Tex: Non-isometric Image-based Garment Texture Generation](https://arxiv.org/abs/2511.18765)
*Hui Shan,Ming Li,Haitao Yang,Kai Zheng,Sizhe Zheng,Yanwei Fu,Xiangru Huang*

Main category: cs.CV

TL;DR: 提出了一种非等距图像到几何的服装纹理生成方法，通过构建3D服装视频数据集和使用Nano Banana进行高质量图像编辑，实现跨姿态和拓扑的PBR纹理生成。


<details>
  <summary>Details</summary>
Motivation: 现有工业3D服装网格已经覆盖了大多数真实世界的服装几何形状，但其纹理多样性仍然有限。现有方法需要输入图像与3D网格之间严格的拓扑一致性或依赖准确的网格变形匹配图像姿态，这严重限制了纹理生成的质量和灵活性。

Method: 1) 构建3D Garment Videos数据集，通过物理模拟提供跨多样化变形的一致几何和材质监督；2) 使用Nano Banana进行高质量非等距图像编辑，实现非等距图像-几何对之间的可靠跨拓扑纹理生成；3) 提出基于不确定性引导视图选择和重新加权的迭代烘焙方法，将多视图预测融合为无缝的生产就绪PBR纹理。

Result: 通过大量实验证明，该方法的前馈双分支架构能够生成适用于工业级3D服装设计的多样化且空间对齐的PBR材质。

Conclusion: 该方法成功解决了非等距图像到几何的服装纹理生成挑战，通过构建专门数据集、高质量图像编辑和迭代烘焙技术，实现了跨姿态和拓扑的可靠纹理生成，为工业级3D服装设计提供了实用的解决方案。
...
TL;DR: 提出了一种用于CLIP提示调优的校准框架，通过两种正则化器增强预测可靠性，同时保持预训练嵌入空间的几何结构
...
Method: 在标准交叉熵损失基础上添加两种互补正则化器：1）均值-方差边际惩罚，通过最大化类间logit边际的平均值同时最小化其离散度来稳定边际；2）文本矩匹配损失，将调优后文本嵌入的一阶和二阶矩与冻结CLIP对应部分对齐
...
TL;DR: OpenVO是一个新颖的开放世界视觉里程计框架，能够在有限输入条件下（如不同观测频率和未标定相机）从单目行车记录仪视频中估计真实世界尺度的自我运动，特别适用于从罕见驾驶事件中构建轨迹数据集。


<details>
  <summary>Details</summary>
Motivation: 现有VO方法存在两个主要限制：1）通常在固定观测频率（如10Hz或12Hz）上训练，完全忽略了时间动态信息；2）需要已知内参的标定相机。这导致它们在面对未见观测频率或未标定相机时性能下降，限制了在从行车记录仪提取轨迹等下游任务中的泛化能力。

Method: OpenVO采用两个关键技术：1）在两帧姿态回归框架中显式编码时间动态信息；2）利用从基础模型推导的3D几何先验知识。该方法能够处理不同观测频率和未标定相机的情况。

Result: 在KITTI、nuScenes和Argoverse 2三个主要自动驾驶基准测试中，OpenVO相比最先进方法实现了超过20%的性能提升。在不同观测频率设置下，该方法显著更鲁棒，在所有指标上实现了46%-92%的误差降低。

Conclusion: OpenVO展示了在真实世界3D重建和多样化下游应用中的强大通用性，特别是在处理不同观测频率和未标定相机的挑战性场景中表现出色，为从行车记录仪等非标定设备构建轨迹数据集提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 一步式文本到图像模型虽然合成速度快，但在文本引导图像编辑中应用受限。现有免训练编辑器在单步推理中失败，导致物体严重失真和非编辑区域一致性丧失，这源于在模型结构化场上进行朴素向量运算产生的高能量、不稳定轨迹。

Method: 将编辑问题重新定义为源文本和目标文本提示定义的分布之间的传输问题。利用动态最优传输理论，推导出原理性、低能量的控制策略，产生平滑、方差减少的编辑场，该场本质稳定，可在单个大积分步中遍历。

Result: ChordEdit能够实现快速、轻量级和精确的编辑，最终在这些具有挑战性的模型上实现真正的实时编辑。

Conclusion: 通过动态最优传输理论，ChordEdit解决了单步文本到图像模型在编辑中的核心问题，实现了高保真、稳定的单步编辑，为实时图像编辑提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 古文书是日本前近代流行的书写风格，但由于其草书形式和字形变化，现代读者难以直接解读。现有自动识别方法在相对干净的文档上表现良好，但在印章干扰（特别是印章覆盖字符）的情况下识别准确率显著下降，而印章在前近代日本文档中非常常见。

Method: 提出了三阶段修复引导的古文书字符识别框架：第一阶段进行字符检测，第二阶段进行印章干扰修复，第三阶段进行字符分类。构建了用于评估字符检测和分类的数据集，使用YOLOv12-medium进行检测，采用修复方法处理印章干扰，最后使用基于Vision Transformer的Metom分类器进行分类。

Result: YOLOv12-medium在构建的测试集上达到98.0%的精确率和93.3%的召回率。第二阶段修复通过PSNR和SSIM指标定量评估。消融实验显示，第二阶段修复将Metom分类器的Top-1准确率从93.45%提升到95.33%。

Conclusion: 提出的三阶段修复引导框架有效缓解了印章干扰对古文书字符识别的影响，显著提升了识别准确率，为处理带有印章干扰的历史文档提供了有效的解决方案。
...
Method: 1. 引入分层运动表示，将刚性运动与非刚性残差运动解耦；2. 使用运动学方法生成刚性运动并产生粗渲染，指导视频扩散模型生成恢复非刚性残差运动的视频序列；3. 提出自引导随机采样方法，结合随机采样（保证逼真质量）和自引导（保持身份保真度）解决分布外问题

Result: 实验表明Ani3DHuman能够生成逼真的3D人体动画，优于现有方法

Conclusion: Ani3DHuman框架成功结合了运动学动画和视频扩散先验，通过分层表示和创新的采样方法解决了现有方法的局限性，实现了高质量的3D人体动画生成
...
TL;DR: CREM提出统一框架，通过压缩驱动的训练策略增强多模态表示用于检索，同时保持生成能力，在检索和生成任务上都取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在理解任务上表现出色，但直接应用于检索等嵌入任务存在挑战，因为输出格式和优化目标不匹配。现有方法通常使用对比微调来适配检索任务，但会丧失生成能力。作者认为生成和嵌入任务都依赖于共享的认知机制，特别是跨模态表示对齐和上下文理解。

Method: 提出CREM（压缩驱动的表示增强模型），采用统一框架：1）基于压缩的提示设计，使用可学习的chorus tokens来聚合多模态语义；2）压缩驱动的训练策略，通过压缩感知注意力整合对比和生成目标。

Result: 在MMEB基准上实现了最先进的检索性能，同时在多个理解基准上保持了强大的生成性能。实验表明生成监督在压缩驱动范式下可以进一步提高MLLMs的表示质量。

Conclusion: CREM通过统一的压缩驱动框架成功解决了MLLMs在检索任务中的适配问题，同时保持了生成能力，证明了生成和嵌入任务可以协同优化，共享的认知机制可以被有效利用。
...
TL;DR: UniMatch：一种从粗到细的语义感知框架，用于在强非等距形状之间建立密集语义对应，不限制物体类别


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖近等距假设和同类物体（如仅适用于人体），而跨类别物体的语义对应构建具有挑战性且关注较少

Method: 两阶段框架：1）粗阶段：通过类别无关的3D分割获得非重叠语义部件，使用多模态大语言模型识别部件名称，利用预训练视觉语言模型提取文本嵌入构建匹配语义部件；2）细阶段：利用粗对应通过基于排名的对比方案指导密集对应学习
...
TL;DR: 提出了StreetTree数据集，这是首个大规模、地理多样、专门用于细粒度街道树木分类的基准数据集，包含超过1200万张图像、8300多种常见街道树种，覆盖133个国家，并附带专家验证的观测数据。
...
Method: 构建了StreetTree数据集，包含来自五大洲133个国家城市街道景观的超过1200万张图像，涵盖8300多种常见街道树种，并补充了专家验证的观测数据。同时提供了层次分类法（目-科-属-种）以支持层次分类和表示学习研究。

Result: 通过多种视觉模型的广泛实验，建立了强基线，并揭示了现有方法在处理现实世界复杂性（如高物种间视觉相似性、长尾自然分布、季节变化引起的类内差异以及多样成像条件）方面的局限性。
...
TL;DR: 本文提出了一种基于修正流的多模态分布对齐方法，通过"一对多映射"策略让源模态数据点观察目标模态全局分布，结合自适应松弛对齐和循环修正流，有效减少模态差距，在多模态情感计算任务中取得优异结果。


<details>
  <summary>Details</summary>
Motivation: 模态差距显著限制了多模态融合的效果。现有方法通常使用扩散模型和对抗学习等技术来减少模态差距，但它们通常专注于一对一对齐，没有让源模态数据点暴露于目标模态的全局分布信息中。

Method: 1. 利用修正流将一种分布映射到另一种分布的特性，扩展修正流用于模态分布映射；2. 采用"一对多映射"策略，让每个源模态数据点观察目标模态整体分布；3. 设计"自适应松弛对齐"，对同一样本的模态对实施严格对齐，对非同一样本或类别的模态对应用松弛映射；4. 引入"循环修正流"确保转移的特征可以转换回原始特征，防止信息丢失。

Result: 在分布对齐后，即使使用简单的融合方法，该方法在多模态情感计算的多个任务上也取得了非常有竞争力的结果。可视化验证表明该方法能有效减少模态差距。

Conclusion: 提出的基于修正流的多模态分布对齐方法通过一对多映射策略、自适应松弛对齐和循环修正流，能够有效减少模态差距，在多模态情感计算任务中表现出色，为多模态融合提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注纯文本指导或将视觉和语言分开处理，缺乏能够同时处理视觉输入、教学计划和用户交互的对话模型。

Method: VIGiA具备两个关键能力：1) 多模态计划推理，使模型能够对齐单模态和多模态查询与当前任务计划并准确响应；2) 基于计划的检索，允许检索文本或视觉表示中的相关计划步骤。
...
TL;DR: 该研究开发了一种基于RT-DETR transformer架构的AI系统，用于在KOH显微镜图像中精确检测真菌菌丝，解决了传统方法中因伪影、角质清除不均和观察者间差异导致的识别困难问题。


<details>
  <summary>Details</summary>
Motivation: 传统KOH显微镜检测皮肤真菌病存在多个问题：伪影干扰、角质清除不均导致图像异质性、观察者间差异大。这些因素影响了真菌菌丝识别的准确性，需要一种更可靠、自动化的检测方法。

Method: 使用2,540张常规采集的KOH显微镜图像构建数据集，采用多类别标注策略明确区分真菌元素和混淆伪影。采用RT-DETR transformer模型架构，通过形态保持的数据增强技术训练模型，以保持细菌丝的结构完整性。

Result: 在独立测试集上，模型在物体级别表现出色：召回率0.9737，精确率0.8043，AP@0.50达到93.56%。在图像级别诊断中，模型实现了100%的敏感性和98.8%的准确率，正确识别了所有阳性病例。定性分析显示模型即使在伪影丰富的视野中也能稳健定位低对比度菌丝。

Conclusion: 该AI系统可作为高度可靠的自动化筛查工具，有效弥合图像级分析与皮肤真菌学临床决策之间的差距，为皮肤真菌病的诊断提供了更准确、一致的解决方案。
...
TL;DR: 提出Flash-VAED框架，通过通道剪枝和算子优化加速VAE解码器，在保持潜在分布对齐的同时实现6倍加速，端到端生成管道加速36%
...
Method: 提出独立感知通道剪枝方法缓解通道冗余，阶段主导算子优化策略处理因果3D卷积高成本，构建Flash-VAED系列，采用三阶段动态蒸馏框架迁移原VAE能力

Result: 在Wan和LTX-Video VAE解码器上超越基线，实现约6倍加速，重建性能保持96.9%，端到端生成管道加速36%，VBench-2.0质量损失可忽略

Conclusion: Flash-VAED框架有效解决了VAE解码器延迟瓶颈，在保持潜在分布对齐的同时显著加速推理，为高质量视频合成提供了高效解决方案
...
TL;DR: JavisDiT++是一个用于联合音频-视频生成的统一建模框架，通过MS-MoE设计、TA-RoPE同步策略和AV-DPO对齐方法，在有限数据下实现了SOTA性能。
...
Result: 基于Wan2.1-1.3B-T2V构建的模型仅用约100万公开训练数据就实现了最先进的性能，在定性和定量评估中都显著优于先前方法，消融研究验证了各模块的有效性。

Conclusion: JavisDiT++提供了一个简洁而强大的联合音频-视频生成框架，通过创新的架构设计、同步策略和偏好对齐方法，成功缩小了开源模型与商业模型的差距，为多模态生成提供了有效解决方案。
...
TL;DR: BriMA方法通过记忆引导的桥接填补模块和模态感知重放机制，解决多模态动作质量评估中模态缺失的挑战，在三种数据集上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的多模态动作质量评估常面临模态不平衡问题，某些模态会因传感器故障或标注缺失而不可用。现有持续学习方法假设所有模态始终完整稳定，限制了实际应用。

Method: 提出Bridged Modality Adaptation (BriMA)方法，包含：1) 记忆引导的桥接填补模块，利用任务无关和任务特定表示重建缺失模态；2) 模态感知重放机制，基于模态失真和分布漂移优先选择信息丰富的样本。

Result: 在RG、Fis-V和FS1000三个多模态AQA数据集上的实验表明，BriMA在不同模态缺失条件下一致提升性能，平均获得6-8%更高的相关性和12-15%更低的误差。

Conclusion: BriMA为解决现实世界部署中模态缺失的多模态动作质量评估问题迈出了重要一步，展示了在真实约束条件下的鲁棒性。
...
Method: 提出EMAD框架，采用分层句子-证据-解剖结构定位机制：1)句子到证据定位将生成的句子链接到临床证据短语；2)证据到解剖定位在3D脑MRI上定位相应结构。使用GTX-Distill减少密集标注需求，通过Executable-Rule GRPO强化微调方案确保临床一致性。
...
TL;DR: PositionOCR：一种参数高效的混合架构，将文本检测模型的定位能力与LLM的上下文推理相结合，在OCR相关视觉任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLMs）虽然在处理异构数据和适应不同上下文方面表现良好，但存在两个主要问题：1）基于语言模型设计的解码器缺乏精确视觉任务所需的位置推理能力；2）参数过多需要大量计算资源和训练数据。而专门的文本检测模型虽然坐标预测准确，但缺乏语义推理能力。

Method: 提出PositionOCR混合架构，将文本检测模型的定位优势与大语言模型的上下文推理能力无缝集成。该框架仅包含1.31亿可训练参数，实现了参数高效的多模态处理。

Result: 在文本定位和文本检测等任务中表现优异，持续超越传统的多模态大语言模型。

Conclusion: 通过将专家的效率与LLM的上下文能力协同，可以创建位置准确的多模态大语言模型，解决了当前MLLMs在视觉任务中的位置推理不足问题。
...
TL;DR: ManiPT框架通过在预训练流形上进行提示调优，使用余弦一致性约束将学习表示限制在预训练几何邻域内，解决有限监督下提示调优导致的表示漂移和泛化下降问题。


<details>
  <summary>Details</summary>
Motivation: 在有限监督下，提示调优会改变预训练表示，使下游特征偏离预训练流形，朝向不利于迁移的方向，这种漂移会降低模型的泛化能力。

Method: 提出ManiPT框架，在预训练流形上进行提示调优。引入文本和图像模态的余弦一致性约束，将学习表示限制在预训练几何邻域内。同时引入结构偏置，强制增量修正，引导适应沿着可迁移方向进行，减轻对捷径学习的依赖。

Result: 在四个下游设置（未见类泛化、少样本分类、跨数据集迁移和领域泛化）中，ManiPT的平均性能优于基线方法。从理论角度，ManiPT减轻了有限数据下的过拟合倾向。

Conclusion: ManiPT通过在预训练流形上进行提示调优，有效解决了有限监督下提示调优导致的表示漂移问题，提高了模型的泛化能力，并为理解提示调优在有限监督下的过拟合机制提供了明确视角。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机虽然擅长高速、低功耗和高动态范围感知，但只能记录相对强度变化而非绝对强度，导致空间信息和静态纹理细节严重丢失。需要解决这一限制来重建高质量视频帧。

Method: 1. 建立基线模型：直接将事件数据作为条件合成视频；2. 引入事件帧间残差引导：基于事件流与视频帧的物理相关性，提升视频帧重建精度；3. 扩展应用：通过调制反向扩散采样过程，实现零样本视频帧插值和预测，形成统一的事件到帧重建框架。

Result: 在真实世界和合成数据集上的实验结果表明，该方法在定量和定性评估上都显著优于先前方法。补充材料中的视频演示展示了良好的视频重建效果。

Conclusion: 通过利用预训练视频扩散模型的生成先验，成功解决了事件相机数据中空间信息和静态纹理细节丢失的问题，建立了一个统一的事件到帧重建框架，在视频重建、插值和预测方面都取得了优异性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于CLIP的3D异常检测方法将点云投影为2D表示，但存在两个主要问题：投影过程会损失几何细节，且依赖单一2D模态导致视觉理解不完整，限制了检测多样化异常类型的能力。

Method: 提出两阶段学习框架：第一阶段动态生成嵌入3D几何先验的文本提示，包含全局形状上下文和通过几何缺陷蒸馏模块提取的局部缺陷信息；第二阶段采用协同视图表示学习架构，并行处理渲染图像和深度图像，通过协同精炼模块融合两个流的特征。

Result: 在四个大规模公共数据集上的综合实验结果表明，GS-CLIP在检测性能上实现了优越表现。

Conclusion: GS-CLIP通过几何感知提示和协同视图表示学习有效解决了零样本3D异常检测中的几何信息损失和多模态融合问题，显著提升了检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临两个关键瓶颈：1）缺乏针对特定模态和解剖结构的自适应机制，限制了在分布外医疗场景的泛化能力；2）现有医学适应方法在大型异构数据集上进行微调，导致噪声监督、成本高和负迁移问题

Method: SegMoTE保留SAM的原始提示接口、高效推理和零样本泛化能力，仅引入少量可学习参数来动态适应不同模态和任务。设计了渐进式提示标记化机制，实现完全自动分割，显著减少标注依赖

Result: 在MedSeg-HQ数据集（不到现有大规模数据集的1%）上训练，SegMoTE在多种成像模态和解剖任务上实现了最先进的性能

Conclusion: SegMoTE是在极低标注成本下，首个高效、鲁棒且可扩展的通用分割模型医学适应框架，推动了基础视觉模型在临床应用中的实际部署
...
TL;DR: 提出KRSVQG模型，通过整合外部知识三元组和图像描述，生成更丰富、多样化的遥感图像问题，超越传统模板化问题生成方法。
...
TL;DR: 提出了一种基于预训练人脸生成器的面部表情编辑方法，通过语义潜在空间操作来增强表情分析数据，减少特征纠缠，改善AU检测器性能


<details>
  <summary>Details</summary>
Motivation: 深度学习视觉模型需要大量标注数据，但许多应用面临标签稀缺和类别不平衡问题。面部表情分析中，动作单元（AU）标注成本高，且AU之间存在共激活导致的特征纠缠问题

Method: 在预训练人脸生成器（Diffusion Autoencoder）的语义潜在空间中操作，使用轻量级线性模型，通过（1）考虑AU共激活的依赖感知条件化，（2）正交投影去除干扰属性方向（如眼镜），以及表情中性化步骤实现绝对AU编辑

Result: 编辑后的数据用于平衡AU出现频率，并通过受控合成多样化身份/人口统计特征。使用生成数据增强AU检测器训练，提高了准确性，产生了更解耦的预测，减少了共激活捷径，优于其他数据高效训练策略

Conclusion: 相比先前方法，该方法编辑效果更强，产生更少伪影，身份保持更好，在AU检测任务上取得了显著改进，效果相当于需要大量额外标注数据才能达到的水平
...
TL;DR: 提出KRSVQG模型，通过融入外部知识来生成更丰富、多样化的遥感图像问题，解决现有方法生成问题过于简单和模板化的问题。


<details>
  <summary>Details</summary>
Motivation: 随着遥感图像档案的快速发展，基于图像的提问成为获取特定信息或进行图像检索的有效方式。但现有自动生成的问题往往过于简单和模板化，这阻碍了问答或视觉对话系统的实际部署。

Method: 提出知识感知的遥感视觉问题生成模型KRSVQG，将图像内容和相关外部知识三元组作为输入，利用图像描述作为中间表示来增强生成问题的图像基础。

Result: 在手动标注的两个数据集NWPU-300和TextRS-300上进行评估，结果表明KRSVQG优于现有方法，能够生成基于图像和领域知识的丰富问题。

Conclusion: KRSVQG模型通过融入外部知识，显著提升了遥感图像问题生成的质量和多样性，为问答和视觉对话系统的实际部署提供了更好的基础。
...
TL;DR: LAVIDA是一个端到端的零样本视频异常检测框架，通过异常暴露采样器生成伪异常增强模型适应性，结合多模态大语言模型提升语义理解，使用基于反向注意力的token压缩处理时空稀疏性，在四个基准数据集上实现零样本设置下的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 视频异常数据因其罕见发生和时空稀疏性而难以收集和检测，现有方法在开放世界场景中表现不佳，主要问题包括数据集多样性有限、对上下文相关异常语义理解不足。

Method: 1) 提出LAVIDA端到端零样本视频异常检测框架；2) 使用异常暴露采样器将分割对象转换为伪异常以增强对未见异常类别的适应性；3) 集成多模态大语言模型增强语义理解能力；4) 设计基于反向注意力的token压缩方法处理异常模式的时空稀疏性并降低计算成本。

Result: 在四个基准VAD数据集上的评估表明，LAVIDA在零样本设置下实现了帧级和像素级异常检测的SOTA性能，训练过程仅使用伪异常数据而无需任何VAD数据。

Conclusion: LAVIDA通过伪异常生成、多模态语义理解和高效token压缩，有效解决了视频异常检测中的数据稀缺和语义理解问题，在零样本设置下取得了优异的检测性能。
...
TL;DR: 提出了一种基于注意力监督的扩散框架，实现无掩码的局部风格迁移，通过注意力对齐和模块化设计解决扩散模型风格空间控制难题


<details>
  <summary>Details</summary>
Motivation: 扩散模型在风格迁移中缺乏精确的空间控制能力，将风格视为全局特征，无法将风格限制在特定对象或区域，现有方法依赖手工掩码或多阶段后处理，导致边界伪影和泛化能力有限

Method: 提出注意力监督扩散框架，在训练时通过将风格标记的注意力分数与对象掩码对齐来明确教导模型在哪里应用风格；采用基于KL散度的Focus损失和基于二元交叉熵的Cover损失联合优化；设计模块化LoRA-MoE架构实现高效可扩展的多风格适应

Result: 方法在推理时实现无掩码的单对象风格迁移，产生区域准确且视觉一致的结果，在局部风格化评估中超越现有扩散编辑方法；引入的区域风格编辑评分（RSES）有效评估局部风格化质量

Conclusion: 提出的注意力监督框架解决了扩散模型中局部风格迁移的核心挑战，通过显式的注意力对齐实现了精确的空间控制，为扩散模型的局部编辑能力提供了新的解决方案
...
TL;DR: DD-CAM：一种梯度无关框架，通过识别最小、充分且保持决策的表征单元子集来生成视觉模型的解释性热力图，相比现有CAM方法能提供更忠实、定位更准确的解释。


<details>
  <summary>Details</summary>
Motivation: 现有CAM方法通常聚合所有单元，导致热力图杂乱，难以识别真正影响预测的关键特征。需要一种能识别最小充分特征子集的方法，这些特征联合激活足以保持预测结果。

Method: 提出DD-CAM框架，采用软件调试中的delta debugging策略，根据分类头中单元交互情况配置搜索策略：对无交互单元测试单个单元，对有交互单元测试单元组合，从而高效识别1-最小充分子集。

Result: 实验评估表明，DD-CAM能生成更忠实的解释，相比最先进的CAM方法获得更高的定位准确率，产生的热力图仅突出最本质的特征。

Conclusion: DD-CAM通过梯度无关的delta debugging方法成功识别最小充分特征子集，为视觉模型提供了更清晰、更准确的解释性热力图，优于现有CAM方法。
...
Method: 采用两阶段框架：1) 果园训练的YOLOv8模型进行苹果定位；2) ByteTrack多目标跟踪保持身份一致性；3) 在裁剪的苹果区域上应用基于健康-缺陷水果数据集微调的ResNet18缺陷分类器；4) 引入跟踪级聚合来强制执行时间一致性。

Result: 与逐帧推理相比，该系统在视频级工业指标（如跟踪级缺陷率和时间一致性）上表现出改进的稳定性，表明集成跟踪对于实用的自动化水果分级系统至关重要。

Conclusion: 集成跟踪机制能够显著提高工业水果检测系统的时间稳定性，为实际自动化水果分级系统提供了更可靠的解决方案。
...
Method: 提出MRI对比增强动力学世界模型，包含时空一致性学习：1）潜在对齐学习，构建患者特异性模板约束内容一致性；2）潜在差异学习，通过插值扩展未观测区间并在潜在空间约束平滑变化

Result: 在两个数据集上的广泛实验表明，MRI CEKWorld能够生成更真实的增强内容和动力学过程

Conclusion: 通过时空一致性学习，提出的MRI CEKWorld模型有效解决了对比增强动力学模拟中的内容失真和时间不连续问题，提高了模拟的真实性
...
TL;DR: IPv2改进图像净化策略，通过三个核心模块解决原策略在背景和肺实质区域去噪不足的问题，显著提升超低剂量CT图像去噪效果。


<details>
  <summary>Details</summary>
Motivation: 原始图像净化策略虽然能校正超低剂量CT与正常剂量CT图像的空间错位，但存在两个固有局限：1）仅抑制胸壁和骨骼区域的噪声，背景未处理；2）缺乏专门的肺实质去噪机制。

Method: 提出IPv2改进策略，包含三个核心模块：Remove Background（去除背景）、Add noise（添加噪声）、Remove noise（去除噪声）。这些模块在训练数据构建时赋予模型在背景和肺组织区域的去噪能力，并在测试阶段通过精炼标签构建提供更合理的评估协议。

Result: 在2%辐射剂量的真实患者肺部CT数据集上的大量实验表明，IPv2在多个主流去噪模型中一致改善了背景抑制和肺实质恢复效果。

Conclusion: IPv2通过系统性地重新设计原始图像净化策略，有效解决了其在背景和肺实质区域去噪不足的问题，显著提升了超低剂量CT图像去噪模型的性能。
...
<details>
  <summary>Details</summary>
Motivation: 原始USR框架存在两个主要问题：1）依赖自回归伪标注导致训练成本高昂；2）CTC和注意力分支的解耦监督在分布偏移（如长序列、噪声、未见域）下容易产生自增强错误。

Method: 提出CTC驱动的教师强制方法，将贪婪解码的CTC伪标签直接输入解码器生成注意力目标，实现单次前向传播。同时引入混合采样策略，缓解解码器仅依赖CTC输入导致的曝光偏差问题。

Result: USR 2.0将训练时间减半，提升了对分布外输入的鲁棒性，在LRS3、LRS2和WildVSR数据集上取得了最先进的结果，超越了原始USR和模态特定的自监督基线方法。

Conclusion: CTC驱动的教师强制结合混合采样，为统一语音识别提供了一种高效且鲁棒的训练框架，在保持多模态识别能力的同时显著提升了训练效率和分布外性能。
...
TL;DR: US-JEPA：针对超声图像的自监督学习框架，采用静态教师架构，通过预测掩码潜在表征而非原始像素来克服超声图像噪声问题，在多个分类任务上达到或超越现有基准


<details>
  <summary>Details</summary>
Motivation: 超声成像因采集过程固有的噪声特性（低信噪比、随机斑点模式）给表示学习带来独特挑战，传统基于像素级重建目标的自监督学习方法效果不佳，而现有JEPA方法依赖超参数敏感且计算昂贵的在线教师更新

Method: 提出US-JEPA框架，采用静态教师非对称潜在训练（SALT）目标，使用冻结的领域特定教师提供稳定的潜在目标，解耦学生-教师优化，推动学生在教师语义先验基础上扩展学习

Result: 在UltraBench公共数据集基准上，对多个器官和病理条件下的所有公开可用最先进超声基础模型进行了首次严格比较；在线性探测的多样化分类任务中，US-JEPA达到或优于领域特定和通用视觉基础模型基准

Conclusion: 掩码潜在预测为获得稳健的超声表示提供了一条稳定且高效的路径，US-JEPA框架有效解决了超声图像自监督学习中的噪声挑战
...
TL;DR: 该论文提出了一种针对3D高斯泼溅技术对抗攻击的防御方法，通过小波变换分析高低频扰动特性，设计频率感知防御策略，在保持原始场景真实性的同时有效抑制对抗性伪影。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术虽然能实现实时高保真3D重建，但研究发现其对输入视图中的对抗性扰动非常脆弱，微小但一致的扰动会严重降低渲染质量、增加训练和渲染时间、膨胀内存使用，甚至导致服务器拒绝服务。因此需要增强3DGS的鲁棒性。

Method: 通过小波变换分析对抗性扰动在输入图像低频和高频分量中的不同行为，基于此观察设计简单有效的频率感知防御策略：通过过滤高频噪声同时保留低频内容来重建训练视图，有效抑制对抗性伪影。

Result: 在多个基准测试和广泛攻击强度下的实验表明，该方法显著增强了3DGS的鲁棒性，且无需访问干净的ground-truth监督。该方法在干净数据上的训练性能不会显著受损，实现了鲁棒性和干净输入性能之间的理想平衡。

Conclusion: 通过揭示并解决3D高斯泼溅技术被忽视的脆弱性，该工作为更鲁棒和安全的3D重建铺平了道路。提出的频率感知防御策略在保持原始场景真实性的同时有效抑制对抗性伪影，实现了鲁棒性和性能的良好平衡。
...
TL;DR: MultiDiffSense：统一的扩散模型，可同时为多种视觉触觉传感器生成合成图像，通过深度图和结构化提示实现可控的多模态合成，显著减少真实数据收集需求
...
Method: 提出MultiDiffSense统一扩散模型，通过CAD导出的姿态对齐深度图和编码传感器类型及4自由度接触姿态的结构化提示进行双重条件控制，实现物理一致的多模态合成。

Result: 在8个物体（5个已见、3个新物体）和未见姿态上评估，MultiDiffSense在SSIM指标上优于Pix2Pix cGAN基线：ViTac +36.3%，ViTacTip +134.6%，TacTip +64.7%。下游3自由度姿态估计任务中，混合50%合成数据和50%真实数据可将所需真实数据减半，同时保持竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 传统LiDAR-相机融合方法在恶劣条件下（相机传感器退化、故障等）会显著降低感知系统可靠性，需要一种鲁棒的融合框架来应对这些挑战。

Method: 1. 将原始LiDAR数据投影到2D范围视图并用LiDAR编码器编码；2. 同时提取相机特征并投影到相同共享空间；3. 使用不确定性引导的融合模块，通过预测的不确定性图动态调节跨模态交互；4. 通过新颖的混合2D-3D变换器解码融合特征，直接预测3D全景分割掩码。

Result: 在Panoptic nuScenes、SemanticKITTI和作者引入的Panoptic Waymo基准测试中，UP-Fuse表现出优异的效果和鲁棒性，即使在严重视觉退化或错位情况下仍能保持强大性能。

Conclusion: UP-Fuse通过不确定性感知融合和混合2D-3D变换器设计，为安全关键环境中的机器人感知提供了一个鲁棒的LiDAR-相机融合解决方案，能够有效应对传感器退化和故障问题。
...
TL;DR: PoseCraft是一个基于扩散模型的框架，通过标记化的3D界面生成具有明确3D姿态和相机控制的光写实人体化身，避免了传统方法中的2D重投影模糊问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于蒙皮的工作流需要繁琐的手动绑定或模板拟合，而神经体积方法依赖于规范模板并对每个未见姿态需要重新优化。需要一种能够保持3D语义、处理大姿态和视角变化的方法。
...
Conclusion: PoseCraft通过标记化的3D界面解决了现有方法在3D语义保持和2D重投影模糊方面的问题，能够生成具有明确3D控制和光写实质量的人体化身。
...
TL;DR: 本文开发了MentalBlackboard基准测试，评估视觉语言模型在空间可视化能力（折纸和打孔测试）上的表现，发现模型在对称变换、旋转和物理情境感知方面存在显著挑战。
...
Method: 开发MentalBlackboard开放空间可视化基准测试，包含折纸和打孔测试，设计两个核心任务：预测和规划。预测任务评估模型应用对称变换和旋转的能力，规划任务测试模型分析对称关系和实施多阶段对称过程的能力。

Result: 预测实验显示模型在应用对称变换方面存在困难，即使能正确预测展开步骤序列；旋转对模型的物理情境感知构成重大挑战。规划任务中模型在分析对称关系和实施多阶段对称过程方面表现有限，Claude Opus 4.1规划准确率最高仅10%。表现最佳的o3模型在泛化任务（仅需空间数据转移）上达到71.6%的峰值性能，但在基于文本的预测任务上准确率仅为25%。

Conclusion: 当前最先进的视觉语言模型在空间可视化能力方面存在显著局限性，特别是在对称变换、旋转感知和多阶段规划任务上表现不佳，表明这些模型尚未完全掌握人类的空间认知能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像编辑方法通常整体处理图像，难以对单个场景元素进行隔离和操控。分层表示能更直观地解构和编辑视觉内容，但缺乏相关任务和数据支持。

Method: 引入RLD任务，构建包含111万自动生成和10万人工标注图像-图层-提示三元组的RefLade数据集，开发基于感知的人类偏好对齐自动评估协议，并提出RefLayer基线模型。

Result: RefLade建立了可基准化的研究任务，RefLayer基线模型实现了高视觉保真度和语义对齐，表现出强大的零样本泛化能力，支持有效的训练和可靠评估。

Conclusion: RLD任务和RefLade数据集为图像分解和可控编辑提供了结构化框架，RefLayer模型展示了高质量图像分解的可行性，为零样本泛化和高级图像编辑开辟了新方向。
...
TL;DR: 提出CL-MC框架，通过检测器监督SAM2，实现声门开口定位的闭环记忆校正，解决跟踪漂移问题


<details>
  <summary>Details</summary>
Motivation: 声门开口定位的时序稳定性面临挑战：单帧检测器缺乏时序上下文，基础模型跟踪器存在记忆漂移。在视频喉镜检查中，快速组织变形、遮挡和紧急情况下的视觉模糊需要鲁棒的时序感知解决方案

Method: 提出闭环记忆校正(CL-MC)框架，通过置信度对齐的状态决策和主动记忆校正来监督Segment Anything Model 2(SAM2)。高置信度检测触发语义重置，覆盖损坏的跟踪器记忆，有效缓解漂移累积

Result: 在紧急插管视频上，CL-MC实现了最先进的性能，与SAM2变体和开环方法相比，显著减少了漂移和漏检率

Conclusion: 记忆校正是可靠临床视频跟踪的关键组件，CL-MC框架为复杂内窥镜场景提供了训练免费的基础跟踪器解决方案
...
TL;DR: ADAMAB是一个用于少样本模式识别的高效嵌入校准框架，通过训练与嵌入器无关的轻量级校准器，结合基于多臂老虎机的自适应数据增强策略，在计算成本最小化的情况下显著提升隐式模式识别性能。


<details>
  <summary>Details</summary>
Motivation: 当前预训练基础模型（如LLMs和VLMs）在处理长尾模式识别任务时面临挑战。虽然微调可以提高识别隐式模式的准确性，但由于缺乏训练数据和高计算开销，通常不可行。

Method: 提出ADAMAB框架：1）训练与嵌入器无关的轻量级校准器，不访问嵌入模型参数以降低计算成本；2）引入基于多臂老虎机（MAB）的自适应数据增强策略；3）使用改进的上置信界算法减少梯度偏移，确保少样本训练的收敛性。

Result: 多模态实验证明ADAMAB具有优越性能，在每类仅使用少于5个初始数据样本训练时，准确率提升高达40%。

Conclusion: ADAMAB为少样本隐式模式识别提供了一种高效解决方案，通过嵌入校准和自适应数据增强，在计算资源有限和数据稀缺的情况下显著提升模型性能。
...
TL;DR: 本文提出了一种名为Stair Pooling的新型下采样策略，通过使用不同方向的串联小尺寸池化操作，将每个2D池化步骤的维度缩减从1/4降低到1/2，从而减少信息损失并增强U-Net在生物医学图像分割中捕获长距离信息的能力。


<details>
  <summary>Details</summary>
Motivation: U-Net架构在生物医学图像分割中表现优异，但传统下采样技术为了计算效率而牺牲了信息保留，导致难以捕获长距离信息。现有方法在信息保留方面存在不足，需要一种更有效的方式来平衡计算效率和信息保留。

Method: 提出Stair Pooling策略，通过一系列不同方向的串联小尺寸池化操作来调节下采样速度。该方法将每个2D池化步骤的维度缩减从传统的1/4降低到1/2，也可以适应3D池化以保留更多信息。此外，利用转移熵选择最优下采样路径。

Result: 在三个生物医学图像分割基准测试上的广泛实验表明，Stair Pooling可以将2D和3D U-Net的性能平均提高3.8%的Dice分数。通过转移熵定量分析显示，该方法有效减少了信息损失。

Conclusion: Stair Pooling是一种简单但有效的策略，通过减少下采样过程中的信息损失，帮助U-Net在上采样阶段更有效地重建空间细节，从而增强捕获长距离信息的能力并提高分割精度。
...
<details>
  <summary>Details</summary>
Motivation: 现有白盒攻击方法难以跨任务泛化，黑盒方法依赖昂贵的迁移攻击效率低下。视觉编码器作为LVLMs中标准化且常共享的组件，为攻击提供了稳定的灰盒支点。

Method: 提出原型锚定引导提供稳定攻击方向，解决属性限制和任务泛化问题；采用两阶段注意力增强：1) 利用token级注意力分数集中扰动关键视觉token；2) 自适应重新校准注意力权重以跟踪对抗过程中的注意力演变。

Result: 在多种下游任务和LVLM架构上的实验表明，PA-Attack平均达到75.1%的得分降低率，展现了强大的攻击效果、效率和任务泛化能力。

Conclusion: PA-Attack通过利用视觉编码器的灰盒特性和创新的注意力机制，为LVLMs的对抗攻击提供了高效、通用的解决方案，揭示了模型安全性的重要问题。
...
TL;DR: Prefer-DAS：一种用于电子显微镜图像分割的领域自适应方法，通过稀疏提示学习和局部偏好对齐，在弱监督和无监督设置下实现高效标注的细胞内结构分割。


<details>
  <summary>Details</summary>
Motivation: 当前无监督领域自适应方法在电子显微镜图像分割中存在性能有限和偏差问题，需要更现实且标注高效的解决方案。研究探索使用稀疏点和局部人类偏好作为弱标签，以降低标注成本。

Method: 提出Prefer-DAS框架，包含：1）稀疏可提示学习，支持完整、部分或无点提示的交互式分割；2）局部直接偏好优化（LPO）和稀疏LPO（SLPO），用于空间变化的人类反馈对齐；3）无监督偏好优化（UPO），处理缺失反馈情况。模型整合自训练和提示引导对比学习。

Result: 在四个挑战性DAS任务上的实验表明，Prefer-DAS在自动和交互式分割模式下均优于SAM类方法以及无监督和弱监督DAS方法，表现出强大的泛化性和灵活性。模型性能接近甚至超过监督模型。

Conclusion: Prefer-DAS通过稀疏提示学习和局部偏好对齐，为电子显微镜图像分割提供了一种高效且灵活的领域自适应解决方案，在弱监督和无监督设置下均能取得优异性能，具有实际应用价值。
...
TL;DR: 提出Hepato-LLaVA模型，通过稀疏拓扑注意力机制处理肝癌全切片图像，结合新构建的HepatoPathoVQA数据集，在肝癌诊断和描述任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前肝癌诊断的计算方法受限于固定分辨率处理机制和低效的特征聚合，导致信息丢失或特征冗余问题，需要更精细的病理分析解决方案。

Method: 提出Hepato-LLaVA多模态大语言模型，引入新颖的稀疏拓扑注意力机制，显式建模2D组织拓扑结构，将局部诊断证据聚合为语义摘要标记，同时保留全局上下文。

Result: 在肝癌诊断和描述任务上实现了最先进的性能，显著优于现有方法，并通过专家验证的HepatoPathoVQA数据集（33K层次化问答对）解决了多尺度数据缺乏问题。

Conclusion: Hepato-LLaVA通过创新的注意力机制和高质量数据集，为肝癌病理分析提供了有效的计算解决方案，在诊断精度和特征聚合效率方面均有显著提升。
...
TL;DR: TherA是一个可控的RGB到热红外图像转换框架，通过结合视觉语言模型和潜在扩散模型，生成多样且热物理合理的TIR图像，在场景和物体级别实现细粒度控制。
...
Method: TherA框架结合TherA-VLM视觉语言模型和潜在扩散模型。给定RGB图像和用户提示条件对，TherA-VLM生成热感知嵌入，编码场景、物体、材料和热辐射上下文。扩散模型基于此嵌入实现真实的TIR合成和细粒度控制。

Result: TherA实现了最先进的转换性能，零样本转换性能平均提升达33%，能够生成多样且热物理合理的TIR图像，支持时间、天气和物体状态的细粒度控制。

Conclusion: TherA通过结合热物理知识和可控生成，解决了RGB到TIR转换中的热物理合理性问题，为热红外感知提供了实用的数据合成解决方案。
...
TL;DR: CountEx是一个能够同时指定计数目标和排除干扰物的视觉计数框架，通过包含和排除提示解决现有方法在杂乱场景中的过计数问题。
...
Method: CountEx通过多模态提示（自然语言描述和可选视觉示例）让用户表达包含和排除意图，核心是判别性查询精炼模块，该模块联合推理包含和排除线索，识别共享视觉特征，隔离排除特定模式，并应用选择性抑制来精炼计数查询。

Result: 实验表明，CountEx在已知和新类别对象的计数任务上，相比最先进方法取得了显著改进。作者还发布了CoCount基准数据集，包含1,780个视频和10,086个标注帧，涵盖97个类别对。

Conclusion: CountEx通过引入排除提示机制，有效解决了视觉相似干扰物导致的计数歧义问题，显著提升了复杂场景下的计数准确性。
...
TL;DR: FinSight-Net是一个针对水下鱼类检测的物理感知检测框架，通过多尺度解耦双流处理和高效路径聚合FPN，在复杂水产养殖环境中实现高效准确的鱼类检测。


<details>
  <summary>Details</summary>
Motivation: 现有水下鱼类检测方法通常通过堆叠特征提取器或引入重型注意力模块来提高精度，但这会带来大量计算开销，更重要的是忽略了限制水下检测的根本物理因素：波长依赖性吸收和浊度引起的散射会显著降低对比度、模糊精细结构并引入后向散射噪声，导致不可靠的定位和识别。

Method: 提出FinSight-Net框架，包含两个核心组件：1) 多尺度解耦双流处理瓶颈，通过异构卷积分支明确针对频率特定信息损失，抑制后向散射伪影，同时通过尺度感知和通道加权路径补偿失真的生物线索；2) 高效路径聚合FPN作为细节填充机制，通过建立长距离跳跃连接和修剪冗余融合路径，恢复深层中通常衰减的高频空间信息。

Result: 在DeepFish、AquaFishSet和UW-BlurredFish基准测试上的广泛实验表明，FinSight-Net实现了最先进的性能。特别是在UW-BlurredFish上达到92.8% mAP，比YOLOv11s高出4.8%，同时参数减少29.0%。
...
TL;DR: 提出无需训练的UrbanAlign框架，通过概念挖掘、多智能体结构化评分和几何校准三个紧密耦合的阶段，将视觉语言模型输出与人类偏好对齐，在城市场景感知任务中超越有监督基线。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要微调或强化学习来对齐视觉语言模型输出与人类偏好，这需要标注数据和GPU计算资源。研究发现对于主观感知任务，VLMs已经是强大的概念提取器但决策校准能力差，可以通过外部方法弥补这一差距。

Method: 提出无需训练的后处理概念瓶颈管道：1) 从少量人工标注中挖掘可解释评估维度；2) 使用Observer-Debater-Judge链从冻结VLM中提取鲁棒的连续概念分数；3) 在混合视觉语义流形上使用局部加权岭回归校准这些分数以匹配人类评分。

Result: 在Place Pulse 2.0数据集上，UrbanAlign在六个类别中达到72.2%准确率（κ=0.45），比最佳有监督基线提升15.1个百分点，比未校准VLM评分提升16.3个百分点，同时保持完全维度级可解释性且无需修改模型权重。

Conclusion: 对于主观感知任务，无需模型训练即可实现VLM输出与人类偏好的对齐。VLMs作为强大概念提取器与外部校准方法的结合，为领域特定任务提供了一种高效、可解释且无需训练的对齐解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（LVLMs）的视觉编码器在医学图像诊断或细粒度分类等特定领域任务中表现不佳，表示错误会通过语言模型级联传播，导致错误响应。现有适配方法修改编码器和语言模型之间的连续特征接口，仍然耦合两个组件，且编码器变化时需要重新对齐。

Method: CRAFT（Codebook RegulAted Fine-Tuning）使用离散码本将视觉表示锚定到稳定标记空间，通过轻量级方法微调编码器，实现领域自适应而不修改模型其他部分。这种解耦设计允许适配后的编码器无缝提升不同语言架构的LVLMs性能，只要它们共享相同的码本。

Result: 在VQARAD和PlantVillage等10个领域特定基准测试中，CRAFT平均提升了13.51%的性能，同时保持了LLM的语言能力，并优于在连续标记上操作的同类方法。

Conclusion: CRAFT通过离散码本实现视觉编码器的解耦领域自适应，提供了一种轻量级、高效的适配方法，能够提升LVLMs在特定领域任务上的性能，同时保持语言模型的原有能力。
...
TL;DR: 提出假设驱动的测试时适应框架，通过生成竞争性几何假设（压缩与膨胀）和表示引导选择器，在医学分割中实现安全导向的适应，减少肿瘤掩码溢出到健康组织的问题。


<details>
  <summary>Details</summary>
Motivation: 标准测试时适应方法将推理视为盲优化任务，在安全关键的医学分割中缺乏选择性，导致肿瘤掩码溢出到健康脑组织或破坏原本正确的预测。需要解决安全与适应之间的权衡问题。

Method: 提出假设驱动的TTA框架：1）生成竞争性几何假设：压缩（去除噪声伪影）与膨胀（恢复欠分割的有效肿瘤）；2）使用表示引导选择器基于内在纹理一致性自主选择最安全结果；3）预筛选门控机制防止自信案例的负迁移。

Result: 在跨域脑肿瘤分割任务中验证，将成人BraTS胶质瘤训练的源模型应用于未见过的儿科和更具挑战性的脑膜瘤目标域。HD-TTA在安全导向指标上优于多个SOTA基线：HD95减少约6.4mm，精度提高超过4%，同时保持相当的Dice分数。

Conclusion: 通过显式假设选择解决安全-适应权衡是安全临床模型部署的可行且稳健路径。该方法在保持分割质量的同时显著提高了安全性指标。
...
<details>
  <summary>Details</summary>
Motivation: 现有流匹配方法在图像生成中存在效率和质量限制，特别是处理高分辨率图像时计算开销大。传统级联方法需要在不同尺度间进行显式去噪，增加了计算复杂度

Method: 提出拉普拉斯多尺度流匹配框架，将图像分解为拉普拉斯金字塔残差，通过具有因果注意力机制的混合变换器架构并行处理不同尺度，无需尺度间的桥接过程

Result: 在CelebA-HQ和ImageNet数据集上，相比单尺度和多尺度流匹配基线，实现了更优的样本质量，减少了GFLOPs，加快了推理速度，并能有效扩展到1024×1024高分辨率生成

Conclusion: LapFlow通过多尺度并行处理显著提升了流匹配方法的生成质量和效率，为高分辨率图像生成提供了一种计算高效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中的镜面3D成像面临挑战：传统偏折法需要多次拍摄不适合动态环境，傅里叶单次方法对高空间频率或大曲率表面性能下降，偏振3D成像受正交成像假设限制精度不足。需要开发单次拍摄、高精度、适应复杂几何的镜面3D成像方法。

Method: 提出物理信息深度学习框架：偏振线索提供方向先验，结构光编码几何信息，通过双编码器架构进行特征调制，解析非线性耦合，直接推断表面法线。实现单次拍摄、快速推理的复杂镜面表面法线估计。

Result: 该方法实现了单次拍摄下准确鲁棒的法线估计，推理速度快，能够对复杂镜面表面进行实用的3D成像。

Conclusion: 提出的物理信息深度学习框架成功解决了复杂镜面表面单次3D成像的挑战，结合偏振和结构光照明的互补优势，实现了快速准确的表面法线估计，为实际应用提供了可行方案。
...
TL;DR: 本文提出了一种新颖的遗忘抵抗和病灶感知（FRLA）方法，用于基于视觉语言模型的源域自由域自适应（SFDA）眼底图像诊断，解决了现有方法中目标模型预测遗忘和忽视视觉语言模型细粒度知识的问题。


<details>
  <summary>Details</summary>
Motivation: 传统SFDA方法在域偏移下容易出错，而现有利用视觉语言模型（ViL）的SFDA方法存在两个问题：1）尽管利用互信息考虑ViL模型和目标模型预测的联合分布，但目标模型的一些优越预测仍会被遗忘；2）忽视了ViL模型中丰富的细粒度知识，这些知识为眼底图像诊断提供了详细的依据。
...
Conclusion: 提出的FRLA方法有效解决了SFDA中目标模型预测遗忘和ViL模型细粒度知识利用不足的问题，在眼底图像诊断领域取得了优异性能。
...
TL;DR: 提出空间正则化多示例学习框架，利用组织切片图像中补丁特征的空间关系作为标签独立的正则化信号，解决稀疏监督问题，提升疾病诊断性能。


<details>
  <summary>Details</summary>
Motivation: 全切片图像分析面临数据规模巨大和标注稀缺的挑战。现有多示例学习方法存在根本性不平衡问题：单个包级标签需要指导大量补丁级特征学习，这种稀疏监督使得难以可靠识别判别性补丁，导致优化不稳定和次优解。

Method: 提出空间正则化MIL框架，利用补丁特征间的固有空间关系作为标签独立的正则化信号。通过联合优化特征诱导的空间重建目标和标签引导的分类目标，学习共享表示空间，强制内在结构模式与监督信号之间的一致性。

Result: 在多个公共数据集上的实验结果表明，该方法相比最先进方法有显著改进。

Conclusion: 该方法为解决全切片图像分析中的稀疏监督问题提供了有前景的方向，通过利用空间关系作为正则化信号，提高了多示例学习的稳定性和性能。
...
TL;DR: MICON-Bench是一个评估多图像上下文生成能力的基准测试，包含6个任务，并提出动态注意力重平衡机制来提升生成质量


<details>
  <summary>Details</summary>
Motivation: 现有的统一多模态模型在多图像上下文生成方面缺乏有效的评估基准，现有基准主要关注文本到图像或单图像编辑任务，无法充分评估跨图像组合、上下文推理和身份保持能力

Method: 1) 提出MICON-Bench基准测试，包含6个任务评估跨图像组合、上下文推理和身份保持；2) 采用MLLM驱动的检查点评估框架进行自动验证；3) 提出动态注意力重平衡机制，在推理时动态调整注意力以增强一致性和减少幻觉

Result: 实验表明MICON-Bench能够有效暴露多图像推理的挑战，同时动态注意力重平衡机制能够显著提升生成质量和跨图像一致性

Conclusion: 该工作填补了多图像上下文生成评估的空白，提出的基准测试和动态注意力重平衡机制为提升统一多模态模型在多图像任务上的性能提供了有效工具
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在视觉推理任务中缺乏对用户指定区域的细粒度控制能力，需要一种无需重新训练或微调的测试时适应方法

Method: 通过注入可学习的视觉提示到冻结的MLLMs中，利用跨模态注意力图编码的语义对应关系，在推理时通过任务特定能量函数优化潜在视觉令牌修改器，引导模型注意力到用户指定区域，并包含改进的优化策略和提示去偏机制

Result: 方法支持多种视觉提示类型（边界框、掩码、涂鸦、点），展现出强大的域外泛化能力和可解释性

Conclusion: ControlMLLM++为多模态大语言模型提供了一种高效、灵活的测试时适应框架，实现了细粒度区域视觉推理，无需模型重新训练或微调
...
TL;DR: 提出关系特征缓存(RFC)框架，通过利用输入输出关系提高扩散变换器中特征预测的准确性，减少冗余计算


<details>
  <summary>Details</summary>
Motivation: 现有基于预测的缓存方法仅依赖时间外推技术来近似输出特征，存在显著的预测误差，导致性能下降。研究发现这些误差源于输出特征变化幅度的不规则性，以及模块输入特征与对应输出的强相关性。

Method: 提出关系特征缓存(RFC)框架，包含两个核心组件：1) 关系特征估计(RFE)，利用输入特征估计输出特征的变化幅度，实现更准确的特征预测；2) 关系缓存调度(RCS)，使用输入特征估计预测误差，仅在预期误差较大时执行完整计算。

Result: 在各种DiT模型上的广泛实验表明，RFC始终显著优于先前的方法，有效减少了计算冗余并提高了性能。

Conclusion: RFC通过利用输入输出关系增强了特征预测的准确性，为扩散变换器的加速提供了更有效的缓存框架，在保持质量的同时显著减少了计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像合成方法难以同时实现高真实度（调整前景姿态以适应背景）和高保真度（准确保留前景细节），需要一种能同时达成这两个目标的方法

Method: 两阶段策略：第一阶段使用高真实度方法生成合理的前景形状，作为第二阶段高保真度方法的条件输入

Result: 在MureCOM数据集上验证了该两阶段策略的有效性，代码和模型已开源

Conclusion: 提出的两阶段方法能够同时实现生成图像合成的高真实度和高保真度目标
...
TL;DR: 研究发现，简单的家庭可及性化妆修改（如胡须、白发、化妆、模拟皱纹）能够欺骗AI年龄估计系统，将未成年人误判为成年人，攻击转换率最高可达83%，揭示了年龄验证系统的严重安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 年龄估计系统被广泛用于限制未成年人访问在线内容，但其对化妆修改的鲁棒性尚未得到系统评估。研究旨在探究简单家庭可及性化妆修改是否能欺骗AI系统将未成年人误判为成年人，从而揭示潜在的安全风险。

Method: 使用VLM图像编辑器（Gemini 2.5 Flash Image）在329张10-21岁个体面部图像上模拟物理攻击（胡须、白发、化妆、模拟皱纹）。评估8个模型：5个专业架构（MiVOLO、Custom-Best、Herosan、MiViaLab、DEX）和3个视觉语言模型（Gemini 3 Flash、Gemini 2.5 Flash、GPT-5-Nano）。引入攻击转换率（ACR）作为评估指标。

Result: 合成胡须单独攻击在所有8个模型上达到28-69%的ACR；四种攻击组合使预测年龄平均增加7.7岁，ACR最高达83%；视觉语言模型的ACR（59-71%）低于专业模型（63-83%），但差异未经过统计检验。

Conclusion: 研究揭示了已部署年龄验证流程的关键漏洞，表明简单的化妆修改就能有效欺骗AI年龄估计系统。建议将对抗鲁棒性评估作为模型选择的强制性标准，以提高系统的安全性。
...
TL;DR: 提出一种基于绿色学习（GL）的医学图像恢复方法，以低剂量CT图像为例，相比深度学习方法具有数学透明、计算高效、模型小等优势


<details>
  <summary>Details</summary>
Motivation: 低剂量CT图像易受噪声和伪影影响，影响后续医学分析。现有深度学习方法虽然有效，但缺乏数学透明性和计算效率，需要探索更高效、透明的替代方案

Method: 采用绿色学习（GL）方法进行医学图像恢复，该方法具有数学透明性、计算和内存效率高的特点，以低剂量CT图像为例进行验证

Result: 实验表明，GL方法在更小的模型尺寸和更低的推理复杂度下，实现了最先进的恢复性能

Conclusion: 绿色学习方法为医学图像恢复提供了一种数学透明、计算高效、性能优越的替代方案，特别适用于资源受限的医疗环境
...
Method: 将姿态估计建模为条件离散扩散过程，提出灵活流决策器动态决定每个token的去噪或重置，采用分层运动学耦合策略分层估计每个刚性部件的姿态

Result: 在合成和真实世界数据集上的实验结果表明，DICArt具有优越的性能和鲁棒性，在复杂环境中实现了可靠的类别级6D姿态估计

Conclusion: 通过将离散生成建模与结构先验相结合，DICArt为复杂环境中的可靠姿态估计提供了新范式，克服了传统连续空间方法的局限性
...
TL;DR: 提出一种针对大型视觉语言模型对抗攻击的通用、高效、无需训练的两阶段防御方法，通过图像变换和智能数据整合来恢复正确的模型行为


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型容易受到对抗图像的攻击，这些图像会微妙地偏置模型输出，使其产生看似合理但错误的响应，需要一种有效的防御机制

Method: 结合图像变换和智能数据整合的两阶段检测机制：1）通过内容保持变换快速过滤干净输入；2）在文本嵌入空间检查差异；3）必要时调用强大LLM解决攻击引起的分歧，利用多个响应的相似性和差异性进行整合

Result: 该方法实现了最先进的准确率，同时保持显著效率：大多数干净图像跳过昂贵处理，即使存在大量对抗样本，开销也保持最小

Conclusion: 提出的防御方法在对抗攻击防御方面既高效又有效，通过智能的多阶段检测和响应整合机制，为大型视觉语言模型提供了实用的安全解决方案
...
TL;DR: HOCA-Bench是一个基于黑格尔哲学视角的物理异常检测基准，将异常分为本体异常和因果异常两类，通过生成视频模型创建了1,439个视频和3,470个QA对，评估了17个视频大语言模型，发现模型在因果异常检测上存在显著认知滞后。


<details>
  <summary>Details</summary>
Motivation: 当前视频大语言模型在语义感知方面有所进步，但在预测性世界建模方面仍然不足，而世界建模是物理基础智能的核心。为了评估模型对物理异常的理解能力，需要建立一个系统性的基准测试。

Method: 采用黑格尔哲学框架将物理异常分为两类：本体异常（实体违反自身定义或持久性）和因果异常（交互违反物理关系）。使用最先进的生成视频模型作为对抗性模拟器，构建了包含1,439个视频和3,470个QA对的测试平台，评估了17个视频大语言模型。

Result: 评估结果显示明显的认知滞后：模型通常能识别静态的本体异常（如形状突变），但在因果机制（如重力或摩擦力）方面表现较差，因果任务性能下降超过20%。系统2的"思考"模式能改善推理，但无法弥补这一差距。

Conclusion: 当前视频大语言模型架构更擅长识别视觉模式，而非应用基本物理定律。这表明需要开发能够更好地理解和推理物理因果关系的模型架构。
...
Result: 大量实验表明，ConceptPrism有效解决了概念纠缠问题，在保真度与对齐性之间实现了显著改善的权衡。

Conclusion: ConceptPrism通过自动解耦共享视觉概念与图像特定残差，无需人工指导即可改善个性化文本到图像生成中的概念纠缠问题，实现了更好的保真度与对齐性平衡。
...
TL;DR: MVIG攻击是一种针对协作感知系统的自适应对抗攻击框架，通过从统一的相互视图信息图表示中学习捕获不同防御系统的漏洞知识，显著降低现有防御成功率


<details>
  <summary>Details</summary>
Motivation: 当前协作感知系统的防御机制存在两个关键弱点：1) 对具有系统化时序和目标区域优化的攻击缺乏鲁棒性；2) 通过共享协作数据中的隐式置信度信息无意中泄露了漏洞知识。需要开发更强大的攻击方法来暴露这些安全漏洞

Method: 提出MVIG攻击框架：1) 使用统一的相互视图信息图表示捕获不同防御系统的漏洞知识；2) 结合时序图学习生成演化的伪造风险图；3) 采用熵感知漏洞搜索优化攻击位置、时序和持续性，实现跨各种防御配置的自适应攻击

Result: 在OPV2V和Adv-OPV2V数据集上的广泛评估表明，MVIG攻击将最先进防御的成功率降低了高达62%，在29.9 FPS下实现持续攻击检测率降低47%，暴露了协作感知系统的关键安全漏洞

Conclusion: MVIG攻击通过从防御系统中学习漏洞知识，开发出能够适应不同防御配置的自适应对抗攻击，显著降低了现有防御机制的有效性，揭示了协作感知系统在安全方面的严重缺陷
...
TL;DR: CLCR提出了一种跨层级共同表示方法，通过三层语义层次结构和层级约束来解决多模态学习中的语义不对齐问题，在多个基准测试中表现出色。
...
Method: 提出跨层级共同表示(CLCR)：1)语义层次编码器将每个模态特征组织为浅层、中层和深层三层语义层次；2)层内共同交换域(IntraCED)将特征分解为共享和私有子空间，通过可学习token预算限制跨模态注意力到共享子空间；3)层间共同聚合域(InterCAD)使用学习锚点同步语义尺度，选择性融合共享表示并门控私有线索；4)引入正则化项强制共享和私有特征分离，最小化跨层级干扰。

Result: 在情感识别、事件定位、情感分析和动作识别等六个基准测试上的实验表明，CLCR实现了强大的性能，并在不同任务间具有良好的泛化能力。

Conclusion: CLCR通过显式组织多模态特征到语义层次结构并施加层级约束，有效解决了多模态学习中的语义不对齐问题，提高了表示质量和任务性能。
...
TL;DR: 本文提出了一种基于卫星图像的自动化管道，用于检测阿富汗考古遗址的盗掘活动，通过比较CNN与传统机器学习方法，发现ImageNet预训练的CNN结合空间掩码能达到最佳性能（F1分数0.926）。


<details>
  <summary>Details</summary>
Motivation: 考古遗址盗掘对文化遗产构成严重威胁，但监测数千个偏远地点在操作上存在困难。需要一种可扩展的卫星图像分析方法来有效检测盗掘活动。

Method: 使用PlanetScope月度镶嵌图像（4.7米/像素）和包含1,943个阿富汗考古遗址（898个被盗掘，1,045个保存完好）的标注数据集，比较两种方法：(i) 基于原始RGB图像块的端到端CNN分类器；(ii) 基于手工特征和遥感基础模型嵌入的传统机器学习方法。

Result: ImageNet预训练的CNN结合空间掩码达到F1分数0.926，明显优于最强的传统机器学习设置（SatCLIP-V+RF+Mean，F1分数0.710）。消融研究表明，ImageNet预训练和空间掩码能显著提升性能，而地理空间基础模型嵌入与手工特征表现相当。

Conclusion: 该方法为大规模监测考古遗址盗掘提供了有效的解决方案，表明盗掘特征具有极强的局部性，预训练的CNN结合空间掩码是最有效的检测方法。代码已在GitHub开源。
...
TL;DR: 提出了一种无需微调VLMs的即插即用模块，通过多模态类别嵌入增强罕见物体识别与推理能力


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在罕见物体上的推理能力不足，因为预训练数据中这类实例稀缺。现有方法需要检索额外数据或引入更强的视觉编码器，计算成本高且未充分利用原始训练数据。

Method: 1) 利用视觉基础模型和同义词增强的文本描述学习罕见物体的多模态类别嵌入；2) 通过轻量级注意力增强模块精炼视觉令牌；3) 将学习到的嵌入作为物体感知检测器生成信息提示，注入文本提示中以引导VLM关注相关图像区域。

Result: 在两个基准测试中，预训练VLMs在罕见物体识别和推理方面获得了一致且显著的性能提升。

Conclusion: 该方法有效增强了VLMs对罕见物体的关注和推理能力，无需微调VLMs，计算效率高，且充分利用了原始训练数据。
...
TL;DR: PedaCo-Gen是一个基于多媒体学习认知理论的教学视频生成系统，通过引入中间表示阶段让教育专家与AI协作，显著提升了教学视频质量。
...
Method: 提出PedaCo-Gen系统，基于Mayer的多媒体学习认知理论，引入中间表示阶段，让教育工作者能够与AI评审员交互式地审查和优化视频蓝图（包括脚本和视觉描述）。

Result: 23位教育专家的研究表明，PedaCo-Gen相比基线方法显著提升了各种主题和CTML原则下的视频质量。参与者报告高生产效率（M=4.26）和指导有效性（M=4.04），AI指导被视为元认知支架而非简单指令。
...
<details>
  <summary>Details</summary>
Motivation: 现有平面跟踪器在面对目标外观变化时鲁棒性不足，需要结合先进的语义分割技术来提升跟踪性能，特别是在目标丢失后重新检测的能力。

Method: SAM-H从分割掩模轮廓估计单应性变换，对目标外观变化具有高度鲁棒性。WOFTSAM在WOFT跟踪器基础上，利用SAM-H提供的目标丢失重新检测能力进行改进。

Result: 在POT-210和PlanarTrack基准测试中均达到最先进性能，在PlanarTrack上p@15指标分别比第二名高出12.4和15.2个百分点。同时提供了改进的PlanarTrack初始姿态标注，提高了高精度p@5指标的评测准确性。

Conclusion: 结合语义分割的长期跟踪能力和单应性姿态估计能显著提升平面跟踪性能，特别是在目标外观变化和丢失重新检测方面。改进的标注数据有助于更准确的基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型的强大生成能力引发了对合成有害、隐私或版权内容的担忧。现有概念擦除技术主要关注去噪组件，但因果追踪研究表明视觉属性信息位于文本编码器的早期自注意力层中，这为概念擦除提供了新方向。

Method: 提出高层表示误导（HiRM）方法，将文本编码器中目标概念的高层语义表示误导到指定向量（如随机方向或语义定义方向），同时仅更新包含视觉属性因果状态的早期层。这种解耦策略实现了精确概念移除。

Result: 在UnlearnCanvas和NSFW基准测试中，HiRM对多样化目标（如物体、风格、裸露内容）表现出强大的概念移除效果。该方法以低训练成本保持生成效用，可迁移到Flux等最新架构而无需额外训练，并与基于去噪器的概念擦除方法产生协同效应。

Conclusion: HiRM通过针对文本编码器早期层的高层表示误导，实现了精确的概念擦除，在移除目标概念的同时最小化对非目标概念的影响，为扩散模型的安全部署提供了有效解决方案。
...
TL;DR: FedTAR：一种联邦学习框架，通过集成人口统计驱动的个性化与时序感知全局聚合，解决纵向医学报告生成中的隐私保护和时序动态建模挑战


<details>
  <summary>Details</summary>
Motivation: 纵向医学报告生成在临床中很重要，但由于严格的隐私约束和疾病进展的演化性质而具有挑战性。现有的联邦学习方法大多忽略了纵向动态，假设客户端分布是静态的，无法建模跨访问的时间变化或患者特异性异质性，导致优化不稳定和报告生成不理想。

Method: 提出联邦时序适应（FTA）设置，明确考虑客户端数据的时间演化。在此基础上提出FedTAR框架，集成人口统计驱动的个性化和时序感知全局聚合。FedTAR从人口统计嵌入生成轻量级LoRA适配器，并执行时序残差聚合，其中不同访问的更新通过元学习（first-order MAML）优化的时序策略进行加权。

Result: 在J-MID（100万次检查）和MIMIC-CXR数据集上的实验表明，FedTAR在语言准确性、时序一致性和跨站点泛化方面持续改进，建立了联邦纵向建模的鲁棒且隐私保护的范式。

Conclusion: FedTAR为纵向医学报告生成提供了一个有效的联邦学习解决方案，通过结合个性化适配器和时序感知聚合策略，解决了现有方法在建模时序动态和患者异质性方面的局限性。
...
TL;DR: TeHOR框架通过文本描述和外观线索解决单图像中3D人与物体重建问题，特别针对非接触交互场景


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖物理接触信息，无法捕捉非接触交互（如注视或指向物体），且重建过程主要基于局部几何邻近性，忽略了提供全局上下文的人与物体外观信息

Method: 引入TeHOR框架，包含两个核心设计：1) 利用人-物交互的文本描述来强制执行3D重建与文本线索之间的语义对齐，支持更广泛的交互推理；2) 将3D人与物体的外观线索纳入对齐过程，捕捉整体上下文信息

Result: 该框架产生准确且语义一致的重建结果，实现了最先进的性能

Conclusion: TeHOR通过结合文本语义对齐和外观线索，有效解决了现有方法在非接触交互和全局上下文理解方面的局限性，为单图像3D人-物重建提供了更全面的解决方案
...
Conclusion: 该方法为HDR重建提供了一种无需训练的有效增强方案，能够恢复过曝区域的细节，同时保持现有HDR重建流程的优势
...
TL;DR: 该论文提出了一种结合类别共享LoRA和逐图像LoRA的扩散模型微调方法，以解决少样本学习中数据稀缺问题，既能保持类别先验又能捕捉细粒度细节。


<details>
  <summary>Details</summary>
Motivation: 在隐私受限的医疗应用和细粒度设置等专业领域中，数据稀缺问题普遍存在，特别是对于尾部类别。为了在数据稀缺情况下获得更少偏见和更可靠的模型，需要利用扩散模型来补充真实数据的不足区域。

Method: 将适配器分离为类别共享的LoRA A（用于类别先验）和逐图像的LoRAs B（用于图像特定特征）。通过语义增强技术，在训练期间保留类别边界框以暴露连贯的类别语义。生成时，使用狄利克雷分布采样的系数将A与B的混合组合。

Result: 在多个数据集上，合成的图像既多样化又细节丰富，同时与少样本真实分布紧密对齐，在下游分类任务中获得了稳健的性能提升。

Conclusion: 提出的方法有效地结合了类别级和图像级LoRA的优势，在数据稀缺情况下生成了既保持类别语义又包含丰富细节的合成图像，显著提升了少样本学习性能。
...
TL;DR: DeepfakeJudge框架通过引导式生成-评估过程，为深度伪造检测提供可扩展的推理监督和评估，显著提升推理的忠实性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测模型生成的解释往往缺乏视觉证据支撑，可靠性有限，且现有评估主要关注分类准确率而忽视了推理忠实性。

Method: 提出DeepfakeJudge框架，包含：1）包含最新生成和编辑伪造的OOD基准数据集；2）带视觉推理标签的人工标注子集；3）无需显式真实推理标注的评估模型套件。通过引导式生成-评估过程扩展人类反馈为结构化推理监督，支持点对点和成对评估。

Result: 推理引导模型在元评估基准上达到96.2%准确率，优于30倍大的基线模型。推理评估器与人类评分高度相关，在人工标注子集上达到98.9%的成对一致性。用户研究显示70%参与者更偏好本框架生成的推理。

Conclusion: 推理忠实性可作为深度伪造检测的可量化维度，DeepfakeJudge框架展示了可扩展的监督方法，能提升深度伪造检测推理的可解释性和可靠性。
...
TL;DR: Flose将6D姿态估计重新定义为条件流匹配问题，通过结合几何和外观特征来克服对称性和局部特征缺失的挑战，在BOP基准测试中表现优异。
...
Method: 提出Flose方法，将6D姿态估计建模为R³空间中的条件流匹配问题。通过去噪过程推断物体姿态，结合几何引导和外观语义特征来缓解对称性带来的模糊性，并集成RANSAC配准处理异常值。

Result: 在BOP基准测试的五个数据集上验证，Flose相比先前方法平均提升了+4.5的平均召回率，表现出优越性能。

Conclusion: Flose通过将6D姿态估计重新定义为条件流匹配问题，有效解决了对称性模糊和局部特征缺失的挑战，为实例级6D姿态估计提供了新的有效解决方案。
...
TL;DR: PMM-Synth是一个个性化的多模态MRI合成框架，通过联合训练多个数据集实现跨数据集泛化，支持灵活的一对一和一对多合成任务。
...
Method: 提出了PMM-Synth框架，包含三个核心创新：1) 个性化特征调制模块，基于数据集标识动态调整特征表示以减轻分布偏移影响；2) 模态一致批量调度器，在不一致模态条件下实现稳定高效的批量训练；3) 选择性监督损失，确保在部分缺失真实模态时仍能有效学习。

Result: 在四个临床多模态MRI数据集上评估，PMM-Synth在一对一和一对多合成任务中均优于现有最先进方法，获得更高的PSNR和SSIM分数。定性结果显示更好地保留了解剖结构和病理细节。下游肿瘤分割和放射学报告研究表明，PMM-Synth在实际模态缺失场景下具有支持可靠诊断的潜力。

Conclusion: PMM-Synth通过跨数据集联合训练和创新的个性化特征调制机制，成功解决了多模态MRI合成中的跨数据集泛化问题，为实际临床部署提供了有效的解决方案。
...
Method: 提出InfScene-SR框架，采用引导和方差校正的融合机制，调整扩散模型的迭代精炼过程，实现无需重新训练的大规模高分辨率图像无缝生成

Result: 在遥感数据集上验证表明，InfScene-SR不仅能以高感知质量重建精细细节，还能消除边界伪影，有益于下游任务如语义分割

Conclusion: InfScene-SR成功解决了扩散模型在任意尺寸图像超分辨率中的扩展性问题，实现了空间连续的大规模场景超分辨率
...
TL;DR: RAP是一种快速前馈的无渲染属性引导方法，用于预测3D高斯泼溅中基元的显著性分数，避免了传统基于渲染分析的局限性。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术产生大量基元，但每个基元对重建的贡献程度不同。现有基于渲染分析的方法存在视图依赖、计算时间长、难以集成等问题，需要一种更高效的重要性评估方法。

Method: RAP直接从高斯基元的固有属性和局部邻域统计推断重要性，避免基于渲染或可见性相关的计算。使用紧凑MLP预测每个基元的重要性分数，结合渲染损失、剪枝感知损失和显著性分布正则化进行训练。

Result: 在少量场景上训练后，RAP能有效泛化到未见数据，并可无缝集成到重建、压缩和传输流程中。相比传统方法，计算更快且不依赖特定视图选择。

Conclusion: RAP提供了一种高效、可扩展的3D高斯泼溅基元重要性预测方法，解决了现有基于渲染分析的局限性，为3D场景重建的优化和压缩提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态学习方法依赖大规模图像-文本数据集，训练成本高；现有数据集蒸馏方法需要全数据集训练和联合优化，架构依赖性强，跨架构泛化能力有限

Method: 使用CLIP提取对齐的图像-文本嵌入，获取原型，利用unCLIP解码器合成图像，实现无需训练和优化的数据集蒸馏

Result: 在广泛实验中，该方法持续优于基于优化的数据集蒸馏和子集选择方法，实现了最先进的跨架构泛化性能

Conclusion: 提出的学习免费数据集蒸馏框架有效解决了多模态数据集蒸馏中的训练成本高和架构依赖问题，为高效可扩展的多模态学习提供了新途径
...
<details>
  <summary>Details</summary>
Motivation: 当前的大型视觉语言模型主要关注全局图像理解，难以模拟人类的视觉注意轨迹，也无法解释描述与特定图像区域之间的关联。需要一种能够整合空间理解和轨迹感知的模型。

Method: 提出TraceVision模型，包含轨迹感知视觉感知模块，通过几何简化从原始轨迹中提取语义关键点，采用三阶段训练流程，使轨迹指导描述生成和区域定位。模型可扩展到轨迹引导分割和视频场景理解。

Result: 在轨迹引导描述生成、文本引导轨迹预测、理解和分割等任务上实现了最先进的性能，为直观空间交互和可解释的视觉理解奠定了基础。

Conclusion: TraceVision通过整合轨迹感知的空间理解，成功解决了现有模型在模拟人类视觉注意轨迹和解释区域关联方面的局限性，为更直观和可解释的视觉语言理解提供了新方向。
...
TL;DR: GOAL框架使用固定等角紧框架分类器解决持续广义类别发现问题，通过监督对齐和置信度引导对齐，在减少遗忘的同时提升新类发现能力


<details>
  <summary>Details</summary>
Motivation: 现有持续广义类别发现方法通常动态更新分类器权重，导致遗忘问题和特征对齐不一致。需要一种能够稳定整合新类别而不破坏旧类别知识的统一框架。

Method: 提出GOAL框架，引入固定的等角紧框架分类器在整个学习过程中施加一致的几何结构。对标记样本进行监督对齐，对新颖样本进行置信度引导对齐。

Result: 在四个基准测试中，GOAL优于现有方法Happy，将遗忘减少16.1%，将新类别发现提升3.2%，为长期持续发现提供了强大解决方案。

Conclusion: GOAL通过固定ETF分类器和双重对齐策略，有效解决了持续广义类别发现中的遗忘和不一致对齐问题，建立了稳定整合新旧类别的统一框架。
...
TL;DR: 提出无需训练的开放词汇3D感知管道，解决现有基于非工业数据集预训练的2D视觉语言基础模型在工业场景中泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 工业生产、物流或制造环境中的自主视觉应用需要超越固定小类集的感知能力。现有开放词汇方法依赖在非工业数据集（如家庭场景）预训练的类无关分割模型，这些模型在常见工业对象上泛化能力差。

Method: 提出无需训练的开放词汇3D感知管道：1）不使用预训练模型生成实例提议，而是基于语义特征合并预计算的超点来生成掩码；2）在代表性3D工业车间场景上评估领域适应的视觉语言基础模型"IndustrialCLIP"进行开放词汇查询。

Result: 定性结果显示成功分割工业对象，证明了该方法在工业场景中的有效性。

Conclusion: 该方法克服了现有开放词汇方法在工业场景中的泛化限制，为工业环境中的自主视觉应用提供了有效的3D感知解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督变化检测方法依赖预定义假设（手工规则、外部数据集或辅助生成模型），导致只能检测有限变化类型，难以泛化到罕见或复杂场景
...
Conclusion: MaSoN通过特征空间的数据驱动变化合成，克服了现有方法对预定义假设的依赖，显著提升了无监督变化检测的泛化能力和性能
...
### [150] [TextShield-R1: Reinforced Reasoning for Tampered Text Detection](https://arxiv.org/abs/2602.19828)
...
<details>
  <summary>Details</summary>
Motivation: 篡改图像日益普遍带来严重安全威胁，现有MLLM在检测微观伪影、定位篡改文本区域准确性低、依赖昂贵标注进行伪造解释等方面存在不足，需要更可靠的检测方法。

Method: 1. 法医持续预训练：从易到难的课程学习，利用自然图像法医和OCR任务的大规模廉价数据；2. 分组相对策略优化：使用新颖奖励函数减少标注依赖并提升推理能力；3. OCR校正：利用MLLM强大的文本识别能力在推理时精确定位；4. 引入TFR基准：包含45k真实和篡改图像，涵盖16种语言、10种篡改技术和多样领域。

Result: TextShield-R1在可解释的篡改文本检测方面显著推进了现有技术水平，通过TFR基准在跨风格、跨方法和跨语言条件下实现了稳健评估。

Conclusion: 该研究提出了首个基于强化学习的MLLM解决方案TextShield-R1，通过创新的训练策略和推理优化方法，有效解决了篡改文本检测中的关键挑战，为图像安全领域提供了重要的技术突破。
...
### [151] [DerMAE: Improving skin lesion classification through conditioned latent diffusion and MAE distillation](https://arxiv.org/abs/2602.19848)
...
TL;DR: 使用类别条件扩散模型生成合成皮肤病图像，结合MAE自监督预训练和知识蒸馏，解决皮肤病变分类中的类别不平衡问题，实现轻量级模型在临床环境中的部署。


<details>
  <summary>Details</summary>
Motivation: 皮肤病变分类数据集通常存在严重的类别不平衡问题，恶性病例显著不足，导致深度学习训练时决策边界出现偏差。这限制了模型在实际临床环境中的应用，特别是需要轻量级模型部署的场景。

Method: 1. 使用类别条件扩散模型生成合成皮肤病图像以解决类别不平衡问题；2. 采用自监督MAE预训练方法，让大型ViT模型学习鲁棒的领域相关特征；3. 应用知识蒸馏技术，将学习到的表示迁移到适合移动设备的小型ViT学生模型。

Result: 在合成数据上进行MAE预训练并结合知识蒸馏，能够提高分类性能，同时实现高效的设备端推理，满足实际临床使用的需求。

Conclusion: 该方法有效解决了皮肤病变分类中的类别不平衡问题，通过合成数据生成、自监督预训练和知识蒸馏的组合策略，既提升了模型性能，又实现了轻量级模型在临床环境中的实际部署。
...
### [152] [When Pretty Isn't Useful: Investigating Why Modern Text-to-Image Models Fail as Reliable Training Data Generators](https://arxiv.org/abs/2602.19946)
*Krzysztof Adamkiewicz,Brian Moser,Stanislav Frolov,Tobias Christian Nauen,Federico Raue,Andreas Dengel*

Main category: cs.CV

TL;DR: 研究发现，尽管2022-2025年间发布的文本到图像扩散模型在视觉质量和提示跟随方面不断进步，但作为合成数据生成器时，使用这些模型生成的训练数据训练的分类器在真实测试数据上的准确率反而持续下降。


<details>
  <summary>Details</summary>
Motivation: 重新审视合成数据作为真实训练集可扩展替代品的承诺，探究文本到图像扩散模型作为合成视觉数据生成器的实际效果，挑战生成模型进步必然带来数据真实性进步的假设。

Method: 使用2022-2025年间发布的最先进文本到图像模型生成大规模合成数据集，仅使用这些合成数据训练标准分类器，然后在真实测试数据上评估这些分类器的性能。

Result: 尽管观察到视觉保真度和提示跟随能力的进步，但使用较新文本到图像模型生成的合成数据训练的分类器在真实测试数据上的分类准确率持续下降。分析揭示这些模型收敛到一个狭窄的、以美学为中心的分布，损害了多样性和标签-图像对齐。

Conclusion: 研究结果挑战了视觉研究中日益增长的假设——生成真实感的进步意味着数据真实性的进步。强调需要重新思考现代文本到图像模型作为可靠训练数据生成器的能力。
...
TL;DR: 该研究提出了一种基于视觉元域的适应策略，通过将大型皮肤镜数据集的视觉表示迁移到临床图像领域，提高了皮肤病变分类模型在临床部署中的泛化鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 皮肤病图像分析中的深度学习模型对采集变异性和领域特定视觉特征敏感，导致在临床环境中部署时性能下降。需要解决视觉伪影和领域偏移对皮肤病变分类的影响。
...
Conclusion: 研究强调了领域感知训练对于可部署系统的重要性，提出的视觉元域适应策略能有效提高皮肤病图像分析模型在临床环境中的鲁棒性。
...
TL;DR: 提出双教师对比蒸馏框架，将多光谱遥感影像与光学视觉基础模型对齐，实现跨模态知识迁移，在多个任务上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 地球观测领域存在多种传感器和模态，单一通用模型不现实，需要多个专用基础模型共存，因此跨模态知识迁移变得至关重要。现有EO预训练主要依赖掩码图像建模，强调局部重建但对全局语义结构控制有限。

Method: 提出双教师对比蒸馏框架：结合多光谱教师和光学视觉基础模型教师，将学生的预训练目标与现代化光学VFM的对比自蒸馏范式对齐，实现连贯的跨模态表示学习。
...
TL;DR: ApET是一个基于信息论的视觉token压缩框架，通过线性近似重构原始视觉token，利用近似误差识别并丢弃信息量最小的token，实现88.9%的压缩率，同时保持95.2%的原始性能，且兼容FlashAttention加速。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型中的冗余视觉token导致计算开销大、推理效率低。先前方法依赖注意力机制，存在位置偏差问题且不兼容高效注意力内核如FlashAttention，限制了实际部署。

Method: ApET采用信息论视角，通过线性近似用少量基token重构原始视觉token，利用近似误差识别并丢弃信息量最小的token，实现注意力无关的token压缩。

Result: 在多个VLM和基准测试中，ApET在图像理解任务上保持95.2%的原始性能，视频理解任务达到100.4%，同时分别压缩88.9%和87.5%的token预算，且能无缝集成FlashAttention。

Conclusion: ApET提供了一种有效的视觉token压缩方法，通过注意力无关的设计实现了高压缩率和性能保持，兼容高效注意力内核，使VLM部署更加实用。
...
TL;DR: BigMaQ数据集：首个集成动态3D姿态-形状表示的非人灵长类动物行为识别数据集，包含750多个猕猴互动场景的详细3D姿态描述，显著提升了行为识别的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前动物行为识别主要基于视频的深度学习，但缺乏准确的3D姿态和形状重建。特别是非人灵长类动物的网格追踪技术落后于其他物种，现有姿态描述仅限于稀疏关键点，无法充分捕捉动作动态的丰富性。

Method: 通过将高质量猕猴模板网格适配到个体猴子，构建特定主体的纹理化虚拟形象。从原始数据集衍生出BigMaQ500行为识别基准，将基于表面的姿态向量与多个个体猴子的单帧图像关联。结合现有图像/视频编码器特征与姿态描述符进行对比分析。

Result: 提供了比先前最先进的基于表面的动物追踪方法更准确的姿态描述。当包含姿态信息时，平均精度均值（mAP）有显著提升，证明了3D姿态信息对行为识别的重要性。

Conclusion: BigMaQ是首个将动态3D姿态-形状表示集成到动物行为识别学习任务中的数据集，为研究非人灵长类动物的视觉外观、姿势和社会互动提供了丰富资源，推动了该领域的发展。
...
TL;DR: 该研究构建了一个包含3,556张高清图像的寄生蜂图像数据集，其中1,739张采用COCO格式标注，包含昆虫全身、翅脉和比例尺的多类别边界框，旨在推动计算机视觉在姬蜂总科等分类困难类群自动识别中的应用。


<details>
  <summary>Details</summary>
Motivation: 姬蜂总科（包括姬蜂科和茧蜂科）是生物多样性监测和农业管理的关键类群，但由于其形态隐蔽、物种数量庞大且多数未描述，成为分类学上最具挑战性的类群之一。目前缺乏针对这些关键类群的可靠数字资源，阻碍了自动识别系统的发展。

Method: 研究团队构建了一个精心策划的图像数据集，包含3,556张高分辨率图像，主要聚焦于新热带区的姬蜂科和茧蜂科，同时补充了其他9个蜂类科（如蜜蜂科、胡蜂科等）以提高模型鲁棒性。其中1,739张图像采用COCO格式进行多类别标注，包括昆虫全身、翅脉和比例尺的边界框。

Result: 创建了一个专门针对姬蜂总科等分类困难类群的图像数据集，包含3,556张高质量图像，其中1,739张具有详细的COCO格式标注。该数据集为开发能够识别这些科的计算机视觉模型提供了基础资源。

Conclusion: 该数据集填补了姬蜂总科等关键但分类困难类群数字资源的空白，为开发自动识别系统提供了重要基础，有望推动生物多样性监测和农业管理中的物种识别技术发展。
...
Method: 1. 提出高保真个性化头部表示，有效解耦表情和身份，捕捉静态主体特定全局几何和动态表情相关细节；2. 引入表情迁移模块，实现不同身份间头部姿态和表情细节的个性化迁移；3. 使用该头部模型作为条件信号，训练基于扩散变换器（DiT）的生成器合成细节丰富的肖像视频。
...
TL;DR: 提出一种针对医学图像的对比学习新选择策略，通过疾病严重程度标签而非传统增强方法选择正负样本对，提高糖尿病视网膜病变生物标志物分类准确率


<details>
  <summary>Details</summary>
Motivation: 在医学图像领域，传统对比学习使用的随机增强可能扭曲包含关键生物标志物的小局部区域，需要更符合医学特性的样本选择方法

Method: 基于异常检测算法的梯度响应为未标记OCT扫描生成疾病严重程度标签，使用这些标签训练监督对比学习模型

Result: 相比自监督基线方法，关键糖尿病视网膜病变生物标志物分类准确率提升高达6%

Conclusion: 基于疾病严重程度特征的样本选择策略比传统增强方法更适合医学图像对比学习，能显著提高生物标志物检测性能
...
### [160] [HeatPrompt: Zero-Shot Vision-Language Modeling of Urban Heat Demand from Satellite Images](https://arxiv.org/abs/2602.20066)
*Kundan Thota,Xuanhao Mu,Thorsten Schlachter,Veit Hagenmeyer*

Main category: cs.CV

TL;DR: HeatPrompt：基于视觉语言模型的零样本框架，利用卫星图像和GIS数据估算建筑年热需求，无需详细建筑数据
...
Method: 使用预训练大型视觉语言模型（VLM）作为能源规划师，通过领域特定提示从RGB卫星图像中提取屋顶年龄、建筑密度等视觉属性，结合基础GIS和建筑级特征，训练多层感知机回归器估算热需求

Result: 相比基线模型，R²提升93.7%，平均绝对误差（MAE）降低30%；定性分析显示高影响力标记与高需求区域对齐

Conclusion: HeatPrompt为数据稀缺地区提供了轻量级的热规划支持，通过零样本视觉语言建模有效估算热需求
...
<details>
  <summary>Details</summary>
Motivation: 当前广义类别发现（GCD）任务主要依赖多模态表示学习，但现有方法过于关注模态间对齐，缺乏对模态内对齐的重视，无法生成理想的表示分布结构。

Method: 提出SSR²-GCD框架：1）使用半监督率缩减方法学习具有理想结构特性的跨模态表示；2）强调模态内关系的适当对齐；3）利用视觉语言模型的模态间对齐集成提示候选以提升知识迁移。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射（3DGS）虽然实时渲染性能优秀，但其基于球谐函数的颜色编码难以有效分离漫反射和镜面反射分量，限制了复杂反射场景的准确表示

Method: 1）提出增强高斯核，通过视点相关透明度显式建模镜面反射效果；2）引入误差驱动补偿策略提升现有3DGS场景渲染质量；3）从2D高斯初始化开始，自适应插入和优化增强高斯核，最终生成增强辐射场

Result: 实验表明该方法在渲染性能上超越最先进的NeRF方法，同时具有更高的参数效率

Conclusion: 提出的增强高斯核方法有效解决了3DGS在复杂反射场景中的局限性，实现了更好的渲染质量和参数效率
...
TL;DR: 本文综述了生物医学AI从依赖专家标注的监督学习向无监督和自监督学习范式转变的趋势，展示了如何通过直接从数据内在结构学习来解锁生物样本库规模数据的潜力。


<details>
  <summary>Details</summary>
Motivation: 专家标注长期以来一直是生物医学AI应用的主要瓶颈。监督学习虽然推动了临床算法的初步发展，但需要大量人工标注限制了其扩展性。生物样本库规模的数据集蕴含着巨大潜力，但缺乏有效利用这些未标注数据的方法。

Method: 采用无监督和自监督学习方法，直接从数据的固有结构学习，包括MRI图像的像素、体积扫描的体素、基因组序列的标记等。这些方法不依赖人工标注，通过数据自身的内在模式进行特征学习。

Result: 无监督框架能够发现新的表型，将形态学与遗传学联系起来，无偏见地检测异常。具体应用包括：推导可遗传的心脏特征、预测组织学中的空间基因表达、检测病理变化，其性能可与甚至超过监督学习方法。

Conclusion: 生物医学AI正经历从监督学习向"无需标签学习"的范式转变，无监督和自监督学习方法能够解锁生物样本库规模数据集的潜力，在发现新表型、连接形态与遗传、无偏见异常检测等方面展现出巨大价值，性能可媲美甚至超越监督学习方法。
...
### [164] [RL-RIG: A Generative Spatial Reasoner via Intrinsic Reflection](https://arxiv.org/abs/2602.19974)
*Tianyu Wang,Zhiyuan Ma,Qian Wang,Xinyi Zhang,Xinwei Long,Bowen Zhou*

Main category: cs.CV

TL;DR: RL-RIG是一个基于强化学习的反思式图像生成框架，通过生成-反思-编辑范式提升图像生成的空间推理能力，在LAION-SG数据集上比现有开源模型提升11%的空间推理准确性。
...
Method: 提出RL-RIG强化学习框架，包含Diffuser、Checker、Actor和Inverse Diffuser四个主要组件，采用生成-反思-编辑范式激发图像生成的思维链推理能力。开发Reflection-GRPO训练VLM Actor生成编辑提示，并训练Image Editor在给定提示下提升图像质量。

Result: 在LAION-SG数据集上，RL-RIG在可控和精确的空间推理方面比现有最先进的开源模型提升高达11%。评估采用场景图IoU和VLM-as-a-Judge策略来评估生成图像的空间一致性。

Conclusion: RL-RIG通过强化学习和反思机制有效解决了图像生成中的空间推理困境，显著提升了生成图像的结构完整性和空间准确性。
...
### [165] [RADE-Net: Robust Attention Network for Radar-Only Object Detection in Adverse Weather](https://arxiv.org/abs/2602.19994)
*Christof Leitgeb,Thomas Puchleitner,Max Peter Ronecker,Daniel Watzenig*

Main category: cs.CV

TL;DR: 提出RADE-Net，一种针对4D雷达张量的轻量级3D投影方法，在恶劣天气条件下实现更好的感知性能，相比基线提升16.7%


<details>
  <summary>Details</summary>
Motivation: 传统光学传感器在恶劣天气下性能受限，雷达更稳健但数据量大且现有方法多使用稀疏点云或2D投影导致信息丢失，深度学习有潜力从低级雷达数据中提取更丰富的特征

Method: 提出3D投影方法处理4D RADE张量，减少91.9%数据量；设计RADE-Net轻量模型，利用空间和通道注意力机制，解耦检测头直接预测目标中心点和回归3D边界框

Result: 在K-Radar数据集上相比基线提升16.7%，比当前雷达专用模型提升6.5%，在恶劣天气条件下超越多个激光雷达方法

Conclusion: RADE-Net通过3D投影和轻量级设计有效处理4D雷达数据，在恶劣天气条件下实现优越的感知性能，为自动驾驶提供更可靠的解决方案
...
### [166] [Token-UNet: A New Case for Transformers Integration in Efficient and Interpretable 3D UNets for Brain Imaging Segmentation](https://arxiv.org/abs/2602.20008)
*Louis Fabrice Tshimanga,Andrea Zanola,Federico Del Pup,Manfredo Atzori*

Main category: cs.CV

TL;DR: Token-UNet：一种用于3D医学图像分割的高效模型，通过TokenLearner和TokenFuser模块将Transformer封装到UNet中，显著降低计算成本，同时保持或提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于Transformer的3D医学图像分割模型（如SwinUNETR）面临计算挑战，其注意力机制随token数量呈二次方增长，而3D输入分辨率呈立方增长，导致在普通硬件上部署困难。需要开发能在有限计算资源和时间框架内运行的高效模型。

Method: 提出Token-UNet系列模型，保留UNet的卷积编码器，应用TokenLearner模块从3D特征图中池化预设数量的token，捕获局部和全局结构信息。通过TokenFuser模块将Transformer封装到UNet架构中，显著减少计算复杂度。

Result: 最重模型的显存占用、推理时间和参数数量分别降至SwinUNETR的33%、10%和35%，同时获得更好的平均性能（SwinUNETR：86.75%±0.19% Dice分数 vs Token-UNet：87.21%±0.35%）。tokenization能有效编码任务相关信息，产生自然可解释的注意力图。

Conclusion: Token-UNet为有限计算资源环境（如3D医学成像）提供了更高效的训练方案，简化了模型优化、微调和迁移学习，有助于加速和多样化方法开发，惠及研究社区。
...
TL;DR: 本文研究了医学多模态学习中的模态鸿沟问题，提出了一个模态无关的框架来弥合这一鸿沟，改善放射影像与临床文本的对齐效果。


<details>
  <summary>Details</summary>
Motivation: CLIP在多模态学习中已成为主流方法，但其对比损失存在意外行为，导致潜在空间稀疏和碎片化，形成模态鸿沟。虽然这一问题在标准文本-图像对中已得到部分缓解，但在医学等复杂多模态场景中仍未解决且未被充分研究。

Method: 提出了一个模态无关的框架，能够弥合医学领域中的模态鸿沟，确保语义相关的表示无论来自何种模态都能更好地对齐。该方法专门针对放射影像和临床文本的对齐进行了优化。

Result: 该方法增强了放射影像与临床文本之间的对齐效果，显著改善了跨模态检索和图像描述生成任务的表现。

Conclusion: 医学多模态学习中也存在模态鸿沟问题，提出的模态无关框架能够有效弥合这一鸿沟，提升语义对齐质量，为医学领域的多模态应用提供了更好的解决方案。
...
TL;DR: AdvMark是一个两阶段微调框架，通过解耦防御策略来解决深度学习图像水印对抗攻击和再生攻击的脆弱性，在保持图像质量的同时显著提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 深度学习图像水印虽然对传统失真具有鲁棒性，但对先进的对抗攻击和再生攻击仍然脆弱。传统方法通过噪声层联合优化编码器和解码器面临两个不可避免的挑战：1) 解码器对抗训练导致干净准确率下降；2) 同时训练所有三种先进攻击导致鲁棒性有限。

Method: 提出AdvMark两阶段微调框架：第一阶段通过定制化的对抗训练范式解决对抗脆弱性，主要微调编码器，仅条件性更新解码器，学习将图像移动到不可攻击区域而非修改决策边界；第二阶段通过直接图像优化处理失真和再生攻击，提出有理论保证的原则性约束图像损失，平衡与原始图像和先前编码图像的偏差，并引入质量感知早停机制保证视觉质量下限。

Result: 广泛实验表明AdvMark在图像质量和综合鲁棒性方面表现最佳，对失真、再生和对抗攻击的准确率分别提升高达29%、33%和46%。

Conclusion: AdvMark通过解耦防御策略的两阶段框架有效解决了深度学习图像水印的脆弱性问题，在保持高图像质量的同时显著提升了对抗各种攻击的鲁棒性。
...
### [169] [SemanticNVS: Improving Semantic Scene Understanding in Generative Novel View Synthesis](https://arxiv.org/abs/2602.20079)
*Xinya Chen,Christopher Wewer,Jiahao Xie,Xinting Hu,Jan Eric Lenssen*

Main category: cs.CV

TL;DR: SemanticNVS是一个相机条件化的多视角扩散模型，通过集成预训练语义特征提取器来提升新视角合成的质量和一致性，特别是在长距离相机运动下。
...
Method: 提出集成预训练语义特征提取器，将更强的场景语义作为条件来实现高质量生成。研究了两种策略：(1) 扭曲的语义特征；(2) 在每个去噪步骤中交替进行理解和生成的方案。

Result: 在多个数据集上的实验结果表明，相比最先进的替代方法，在定性和定量指标（FID提升4.69%-15.26%）上都有明显改进。

Conclusion: 通过集成预训练语义特征提取器，SemanticNVS能够显著提升新视角合成的质量和一致性，特别是在长距离相机运动场景下，解决了现有方法的语义退化问题。
...
### [170] [Do Large Language Models Understand Data Visualization Principles?](https://arxiv.org/abs/2602.20084)
...
Method: 使用ASP（答案集编程）生成硬验证基准，编译了可视化原则的自然语言表述，生成了约2000个带有明确原则违规标注的Vega-Lite规范控制数据集，并补充了300多个真实世界的Vega-Lite图表。评估了检查和修复任务，评估模型检测原则违规和修正有缺陷图表规范的能力。

Result: 研究显示了大型（视觉）语言模型作为可视化设计的灵活验证器和编辑器的潜力，但在视觉感知更细微方面与符号求解器存在持续差距。发现了一个有趣的不对称性：前沿模型在修正违规方面往往比可靠地检测违规更有效。

Conclusion: 本文首次系统评估了LLMs和VLMs在可视化原则推理能力上的表现，揭示了它们作为可视化设计验证器和编辑器的潜力，同时指出了在检测可靠性方面与符号方法的差距，以及修正能力优于检测能力的有趣现象。
...
TL;DR: 本文首次系统评估了LLMs在可视化规则验证方面的能力，使用基于ASP的硬验证基准测试，发现前沿模型在常见违规检测上表现良好（F1最高0.82），但对细微感知规则表现较差（部分类别F1<0.15），自然语言约束翻译能显著提升小模型性能。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究显示LLMs能够生成图表或识别误导性图表，但尚不清楚它们是否能够直接推理和执行可视化规则。基于约束的系统（如Draco）需要专家维护符号编码，这促使研究者探索使用LLMs作为灵活规则验证器的可能性。

Method: 将Draco约束的子集翻译为自然语言陈述，生成包含2000个Vega-Lite规范的受控数据集，并标注明确的规则违规。评估LLMs在检测违规准确性（F1分数）和提示遵循度（输出是否遵循结构化格式）两方面的表现。

Result: 前沿模型在提示遵循度上表现优异（Gemma 3 4B/27B：100%，GPT-oss 20B：98%），能可靠检测常见违规（F1最高达0.82）。但对于细微的感知规则表现不佳（部分类别F1<0.15），且从技术ASP公式生成的输出性能下降。自然语言约束翻译使小模型性能提升高达150%。

Conclusion: LLMs作为灵活的语言驱动验证器具有潜力，但在处理复杂感知规则方面仍存在局限性，不如符号求解器精确。自然语言翻译能显著改善小模型性能，但当前LLMs仍无法完全替代基于约束的系统。
...
TL;DR: Flow3r：一种利用密集2D对应关系（光流）作为监督的3D/4D重建框架，可从无标签单目视频进行可扩展训练


<details>
  <summary>Details</summary>
Motivation: 当前前馈式3D/4D重建系统依赖于密集几何和姿态监督，这些数据获取成本高且规模有限，特别是在动态真实场景中尤为稀缺

Method: 提出因子化光流预测方法：使用一个图像的几何潜在变量和另一个图像的姿态潜在变量来预测两个图像之间的光流，这种因子化直接指导场景几何和相机运动的学习

Result: 在八个基准测试中（涵盖静态和动态场景）取得了最先进的结果，特别是在野外动态视频上表现最佳，这些场景中标记数据最为稀缺

Conclusion: 因子化光流预测优于替代设计，性能随无标签数据规模一致扩展，为从无标签视频中学习视觉几何提供了可扩展的解决方案
...
TL;DR: tttLRM是一个新颖的大型3D重建模型，通过测试时训练层实现长上下文自回归3D重建，具有线性计算复杂度，支持从多张图像观察中高效压缩并解码为显式3D表示。


<details>
  <summary>Details</summary>
Motivation: 为了解决大规模3D重建中计算复杂度高、难以处理长上下文序列的问题，同时实现从图像观察中高效生成显式3D表示，作者提出了tttLRM模型。

Method: 使用测试时训练层将多张图像观察压缩为快速权重，在潜在空间中形成隐式3D表示，可解码为高斯泼溅等显式格式。支持在线学习变体进行渐进式3D重建和细化。

Result: 在物体和场景的3D高斯重建任务中，该方法在性能上优于现有最先进方法，实现了更高质量的重建和更快的收敛速度。

Conclusion: tttLRM通过测试时训练层实现了高效的3D重建，展示了从新视角合成任务到显式3D建模的有效迁移，为大规模3D重建提供了新的解决方案。
...
TL;DR: Mobile-O是一个紧凑的多模态视觉-语言-扩散模型，能够在移动设备上实现统一的视觉理解和生成，仅需少量训练数据，在iPhone上3秒生成512x512图像。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型通常数据需求大、模型体积庞大，难以在边缘设备上部署。需要开发一个紧凑、高效且能在移动设备上实时运行的统一多模态模型。

Method: 提出Mobile Conditioning Projector (MCP)核心模块，使用深度可分离卷积和层级对齐融合视觉-语言特征与扩散生成器。采用少量百万级样本训练和创新的四元组格式（生成提示、图像、问题、答案）进行后训练。

Result: 在GenEval上达到74%，比Show-O和JanusFlow分别高出5%和11%，运行速度分别快6倍和11倍。在视觉理解方面，在七个基准测试中平均超过它们15.3%和5.1%。在iPhone上仅需约3秒生成512x512图像。

Conclusion: Mobile-O建立了首个实用的边缘设备实时统一多模态理解和生成框架，为完全在设备上运行、无需云依赖的实时统一多模态智能研究铺平了道路。
...
### [175] [Design and Biomechanical Evaluation of a Lightweight Low-Complexity Soft Bilateral Ankle Exoskeleton](https://arxiv.org/abs/2602.18569)
*Josée Mallah,Zakii Javed,Zafer Azak,Thomas Stone,Luigi G. Occhipinti*

Main category: cs.RO

TL;DR: 研究人员开发了一款轻量、低复杂度、柔软的双侧踝关节外骨骼，用于跖屈辅助，可安装在任意鞋子上方，旨在为医疗和非医疗目的提供步态辅助。


<details>
  <summary>Details</summary>
Motivation: 许多人可以从步态外骨骼辅助中受益，无论是医疗还是非医疗目的。但传统外骨骼带来额外质量和结构，需要补偿这些负担。本研究旨在开发一种轻量、低复杂度的解决方案，减少对健康步态的干扰。

Method: 开发了一款柔软的双侧踝关节外骨骼，专门用于跖屈辅助，采用鞋面附着设计，可安装在任意鞋子上方。通过实验测试评估设备在零扭矩模式下对下肢运动学和动力学的影响。

Result: 实验测试显示，在零扭矩模式下佩戴外骨骼与不佩戴外骨骼相比，下肢运动学和动力学没有显著差异。这表明该设备不会阻碍健康步态，证明其具有顺应性和舒适性，有望提供有效辅助。

Conclusion: 该轻量、低复杂度、柔软的双侧踝关节外骨骼设计成功，不会干扰健康步态，具有顺应性和舒适性。已开发控制系统，正在进行额外测试以验证其辅助效果。
...
### [176] [Enhancing Goal Inference via Correction Timing](https://arxiv.org/abs/2602.18603)
*Anjiabei Wang,Shuangge Wang,Tesca Fitzgerald*

Main category: cs.RO

TL;DR: 研究探索机器人从人类纠正行为中学习的新方法，重点关注纠正时机作为重要学习信号的价值


<details>
  <summary>Details</summary>
Motivation: 传统方法将人类纠正视为新的演示或偏好信号，但忽视了人类决定干预机器人行为的时机这一重要信息。纠正时机反映了多种因素，包括机器人任务进展、与人类期望的一致性、运动可读性等，这些信息对机器人学习很有价值。

Method: 研究纠正时机作为学习信号的应用，包括：(1)识别可能引发人类纠正的机器人运动特征；(2)基于纠正时机和初始方向快速推断人类纠正的最终目标；(3)学习更精确的任务目标约束。

Result: 纠正时机在前两个应用中显著改善了学习效果：能更好地识别引发纠正的机器人运动特征，并能更快地推断人类纠正的最终目标。

Conclusion: 纠正时机是机器人学习的重要信号，为从人类纠正中学习提供了新的视角和有效方法。
...
### [177] [OVerSeeC: Open-Vocabulary Costmap Generation from Satellite Images and Natural Language](https://arxiv.org/abs/2602.18606)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖固定本体和静态成本映射，无法适应任务需求的多样性、部署时未知的地形实体以及用户提示中编码的组合遍历逻辑。虽然基础模型在语言理解和开放词汇感知方面表现出色，但没有单一模型能同时解析细微的任务指令、在大规模图像中定位任意实体，并将其合成为规划器可执行的成本函数。

Method: OVerSeeC采用模块化框架，分解为三个步骤：(1) LLM提取实体和排名偏好；(2) 开放词汇分割管道从高分辨率图像中识别这些实体；(3) LLM使用用户的自然语言偏好和掩码合成可执行的成本地图代码。

Result: OVerSeeC能够处理新颖实体，尊重排名和组合偏好，并在不同区域产生与人工绘制轨迹一致的路线，显示出对分布偏移的鲁棒性。
...
### [178] [FORMICA: Decision-Focused Learning for Communication-Free Multi-Robot Task Allocation](https://arxiv.org/abs/2602.18622)
*Antonio Lopez,Jack Muirhead,Carlo Pinciroli*

Main category: cs.RO

TL;DR: FORMICA：一种无需机器人间通信的学习型任务分配框架，通过预测队友出价实现隐式协调，在有限带宽环境下显著提升系统性能。


<details>
  <summary>Details</summary>
Motivation: 现有多机器人任务分配方法依赖通信来协调冲突，但在带宽有限、基础设施退化或对抗干扰的环境中性能急剧下降，需要无需通信的协调方案。

Method: 采用学习框架预测队友出价分布来调整任务选择，基于SPO（智能预测后优化）思想端到端训练预测器以最小化任务分配遗憾，使用平均场近似将复杂度从O(NT)降至O(T)。

Result: 在16个机器人和64个任务的场景中，系统奖励提升17%，接近最优MILP解；在256个机器人和4096个任务的大规模场景中，性能提升7%，训练仅需21秒。

Conclusion: FORMICA框架在无需机器人间通信的情况下实现了高质量的任务分配，通过预测出价分布和端到端训练，在有限带宽环境中具有优越性能和良好泛化能力。
...
### [179] [Soft Surfaced Vision-Based Tactile Sensing for Bipedal Robot Applications](https://arxiv.org/abs/2602.18638)
*Jaeeun Kim,Junhee Lim,Yu She*

Main category: cs.RO

TL;DR: 该论文提出了一种用于双足机器人的软表面视觉触觉脚传感器，通过光学捕捉接触变形，将足地交互转化为丰富的触觉信号，从而提高平衡控制和地形感知能力。


<details>
  <summary>Details</summary>
Motivation: 足式运动受益于具身感知，但传统机器人缺乏类似皮肤的触觉反馈。为了提升双足机器人的稳定性、适应性和环境感知能力，需要开发能够捕捉足地交互丰富信息的触觉传感器。

Method: 开发了软表面视觉触觉脚传感器，采用可变形层光学捕捉接触变形。从接触图像流中估计接触姿态（位置和方向）、可视化剪切力、计算压力中心、分类地形并检测接触斑块的几何特征。

Result: 在倾斜平台和视觉遮挡条件下验证了传感器能力，显示足部触觉反馈相比仅依赖本体感知能显著改善平衡控制和地形感知。传感器能准确估计接触参数并分类不同地形。

Conclusion: 将触觉感知集成到足式机器人脚部可提高稳定性、适应性和环境感知能力，为实现更顺应和智能的移动系统提供了有前景的方向。
...
### [180] [Robotic Fruits with Tunable Stiffness and Sensing: Towards a Methodology for Developing Realistic Physical Twins of Fruits](https://arxiv.org/abs/2602.18661)
...
<details>
  <summary>Details</summary>
Motivation: 农业食品行业面临劳动力短缺、高需求和供应链中断等挑战，导致未收获农产品大量损失。机器人采摘虽有前景，但评估和训练软体抓取器处理易损水果存在困难，因为天然产品的机械特性高度可变，难以建立可靠的基准或数据驱动控制策略。现有测试方法依赖大量真实水果来捕捉这种变异性，导致效率低下、成本增加和浪费。

Method: 开发可调谐软体物理孪生体，模拟不同成熟度真实水果的刚度特性。设计并制造了猕猴桃的纤维增强气动物理孪生体，能够复制不同成熟度水平的刚度。通过实验验证刚度可调谐性，并使用商业机器人抓取器进行抓取任务测试。

Result: 物理孪生体的刚度可在多次试验中精确调谐（97.35-99.43%准确度）。抓取任务显示物理孪生体的传感器反馈能够反映施加的抓取力。50次循环的压力测试显示能够可靠维持所需刚度（0.56-1.10%误差）。
...
### [181] [Toward AI Autonomous Navigation for Mechanical Thrombectomy using Hierarchical Modular Multi-agent Reinforcement Learning (HM-MARL)](https://arxiv.org/abs/2602.18663)
...
TL;DR: 提出分层模块化多智能体强化学习框架，用于自主双设备导航治疗急性缺血性卒中，在体外实验中首次实现机械取栓血管的自主导航。
...
Method: 提出分层模块化多智能体强化学习框架，将复杂的双设备导航任务分解为专门子任务，每个子任务使用Soft Actor-Critic强化学习进行训练。在股动脉到颈内动脉的导航中，分别训练导引导管和导丝。

Result: 在计算机模拟中，单血管模型在个体解剖结构上达到92-100%成功率，多血管模型在多个患者解剖结构上达到56-80%成功率。体外实验中，两种HM-MARL模型均成功完成100%从股动脉到右颈总动脉的导航，80%到达右颈内动脉，但在左侧血管的超人挑战中失败。

Conclusion: 这是首次在体外实现机械取栓血管自主导航的演示。HM-MARL能够跨解剖结构泛化，但仿真到现实的转换仍存在挑战。未来工作将使用世界模型改进强化学习策略，并在未见过的体外数据上验证性能，推动自主机械取栓向临床转化。
...
### [182] [Scout-Rover cooperation: online terrain strength mapping and traversal risk estimation for planetary-analog explorations](https://arxiv.org/abs/2602.18688)
...
TL;DR: 提出一种腿式侦察机器人与轮式漫游车协同框架，通过腿式机器人的本体感知地形交互在线估计松散土壤强度，构建地形图并评估轮式漫游车通行风险，实现危险变形地形下的安全科学探测。


<details>
  <summary>Details</summary>
Motivation: 行星表面如火星沙丘和月球陨石坑等松散可变形土壤区域具有重要科学价值，但对轮式漫游车构成危险。需要开发能够安全探测这些危险地形的技术框架。

Method: 采用腿式机器人作为移动侦察员，利用其腿部与地形的本体感知交互在线估计土壤强度，构建空间分辨地形图。将这些地图与漫游车运动模型结合，评估通行风险并指导路径规划。

Result: 在NASA艾姆斯月球模拟试验场和白沙沙丘场的模拟任务验证表明：1）能够从腿式运动在线生成地形强度图；2）漫游车特定通行风险评估能够实现安全导航到科学目标。侦察生成的地形图可靠捕捉空间变化并预测移动故障模式。

Conclusion: 通过结合本体地形感知与异构漫游车协同，该框架增强了操作鲁棒性，扩展了可变形行星环境中可到达的科学工作空间。
...
### [183] [CLASH: Collision Learning via Augmented Sim-to-real Hybridization to Bridge the Reality Gap](https://arxiv.org/abs/2602.18707)
...
<details>
  <summary>Details</summary>
Motivation: 传统物理引擎在模拟接触丰富的动力学（如碰撞）时存在精度与计算速度的权衡，导致仿真与现实之间的差距，阻碍了仿真训练策略的直接迁移到现实世界

Method: 提出CLASH框架：1）从有缺陷的仿真器（MuJoCo）中提取基础模型以获取物理先验；2）使用极少量真实世界交互数据（少至10个样本）对模型进行微调，修正仿真器固有误差；3）创建高保真混合仿真器，结合仿真效率和真实数据精度

Result: 混合仿真器不仅预测精度更高，还将碰撞计算时间减少近50%；使用该仿真器训练的策略在现实世界中迁移更稳健，在顺序推动任务中强化学习成功率翻倍，基于模型控制的任务性能显著提升

Conclusion: CLASH通过数据高效的方式学习碰撞模型，有效缩小sim-to-real差距，为机器人策略从仿真到现实的稳健迁移提供了可行方案
...
### [184] [RoboCurate: Harnessing Diversity with Action-Verified Neural Trajectory for Robot Learning](https://arxiv.org/abs/2602.18742)
*Seungku Kim,Suhyeok Jang,Byungjun Yoon,Dongyoung Kim,John Won,Jinwoo Shin*

Main category: cs.RO

TL;DR: RoboCurate是一个机器人合成数据生成框架，通过模拟重放验证生成视频中的动作质量，结合图像编辑和视频转换增强数据多样性，显著提升机器人学习性能。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型产生的合成数据在机器人学习中具有可扩展性，但存在动作质量不一致的问题。现有的视觉语言模型在验证视频质量方面有局限，无法准确评估物理准确性或直接评估生成的动作本身。

Method: RoboCurate通过在模拟器中重放预测的动作，并测量模拟回放与生成视频之间运动的一致性来评估动作质量。此外，通过图像到图像编辑解锁超出可用数据集的观察多样性，并应用保持动作的视频到视频转换来进一步增强外观。

Result: RoboCurate生成的合成数据相比仅使用真实数据，在多个任务上取得了显著改进：GR-1 Tabletop（300个演示）相对成功率提升70.1%，DexMimicGen预训练设置中提升16.1%，在具有挑战性的真实世界ALLEX人形灵巧操作设置中提升179.9%。

Conclusion: RoboCurate通过模拟重放验证动作质量的方法，有效解决了合成机器人数据中动作质量不一致的问题，显著提升了机器人学习性能，特别是在数据稀缺的复杂任务中表现出色。
...
### [185] [Learning to Localize Reference Trajectories in Image-Space for Visual Navigation](https://arxiv.org/abs/2602.18803)
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉导航方法通常与特定机器人绑定，需要相机标定、位姿信息或机器人特定训练。本文旨在开发一种机器人无关的视觉导航方法，能够零样本适应不同机器人平台。

Method: 模型预测参考轨迹在机器人当前视图中的图像空间坐标，而不是预测特定机器人的动作。通过解耦感知与动作，学习定位轨迹点而非模仿行为先验，采用跨轨迹训练策略增强对视角和相机变化的鲁棒性。

Result: 在传统前向导航任务中，比最先进方法提高20-50个百分点，在多样仿真和真实环境中达到94-98%的成功率。在具有挑战性的任务（如后向遍历）上，比基线方法提高5倍以上。

Conclusion: LoTIS提供了一种简单有效的机器人无关视觉导航方案，仅需手机拍摄的视频即可让不同机器人导航到轨迹上的任意点，实现了零样本跨机器人平台的导航能力。
...
### [186] [Habilis-$β$: A Fast-Motion and Long-Lasting On-Device Vision-Language-Action Model](https://arxiv.org/abs/2602.18813)
...
TL;DR: Habilis-β是一个快速运动、长时运行的设备端视觉-语言-动作模型，通过生产力-可靠性平面（PRP）评估，在连续运行协议下实现了高任务吞吐量和长干预间隔时间。


<details>
  <summary>Details</summary>
Motivation: 当前VLA评估主要局限于单次试验成功率，无法捕捉实际部署所需的快速运动和长时运行能力，需要新的评估框架来衡量实际生产力。

Method: 集成无语言预训练获取交互先验，循环任务演示后训练捕捉状态漂移，采用ESPADA进行相位自适应运动整形加速自由空间移动，使用整流流蒸馏实现边缘设备高频控制，部署时通过CFG动态平衡指令遵循和交互先验。

Result: 在1小时连续运行评估中，Habilis-β在PRP指标上表现优异：仿真中达到572.6 TPH和39.2秒MTBI，真实世界物流工作流中达到124 TPH和137.4秒MTBI，并在RoboTwin 2.0排行榜上取得最高性能。

Conclusion: Habilis-β通过创新的评估框架和系统设计，实现了快速运动和长时运行的设备端VLA模型，为实际机器人部署提供了有效的解决方案。
...
### [187] [RotorSuite: A MATLAB/Simulink Toolbox for Tilt Multi-Rotor UAV Modeling](https://arxiv.org/abs/2602.18814)
*Nicola Cigarini,Giulia Michieletto,Angelo Cenedese*

Main category: cs.RO

TL;DR: 开发了一个MATLAB/Simulink工具箱RotorSuite，用于建模和模拟各类多旋翼平台的动力学特性，支持分析和物理两种方法。


<details>
  <summary>Details</summary>
Motivation: 随着空中平台从被动传感器发展为接触感知机器人系统，平台设计快速进步。传统共面共线四旋翼已被现代倾斜多旋翼平台补充，这些平台具有增强的机动性。为了准确分析、控制和验证这些新兴平台的性能，需要精确建模，但传统建模方法耗时、依赖用户经验且容易出错。

Method: 提出了一个MATLAB/Simulink工具箱RotorSuite，通过分析和物理两种方法建模和模拟各类多旋翼平台的动力学特性。工具箱提供全面的文档和示例用例。

Result: 开发了RotorSuite工具箱，能够有效建模和模拟多旋翼平台动力学，为教学、研究和工业开发提供了有价值的工具。

Conclusion: RotorSuite工具箱解决了多旋翼平台建模耗时、易错的问题，为分析、控制和验证各类多旋翼平台性能提供了系统化解决方案，具有广泛的应用价值。
...
### [188] [GRAB: A Systematic Real-World Grasping Benchmark for Robotic Food Waste Sorting](https://arxiv.org/abs/2602.18835)
*Moniesha Thilakarathna,Xing Wang,Min Wang,David Hinwood,Shuangzhe Liu,Damith Herath*

Main category: cs.RO

TL;DR: GRAB框架通过整合多样化变形物体、先进视觉系统和抓取前条件，为食品废弃物分拣中的机器人抓取提供了全面评估基准，揭示了不同夹爪在复杂环境中的性能差异。


<details>
  <summary>Details</summary>
Motivation: 食品废弃物管理对可持续发展至关重要，但无机污染物阻碍了回收潜力。机器人自动化虽然能加速分拣过程，但污染物的多样性和不可预测性给机器人抓取带来了重大挑战。现有基准框架依赖有限的模拟数据集，关注简单指标如成功率，忽视了关键的物体和环境相关的抓取前条件。

Method: 本文提出了GRAB（Grasping Real-World Article Benchmarking）框架，整合了多样化变形物体、先进的抓取姿态估计视觉系统，以及关键的抓取前条件，建立了一套关键的抓取能力指标。通过四个高保真场景中的1,750次食品污染物抓取尝试，系统比较了工业抓取模式。

Result: 大规模评估揭示了不同夹爪的明显优势和局限性。在杂乱环境中，物体质量成为主导性能因素，而视觉质量和杂乱程度起中等作用。结果强调了开发能够实现稳健跨类别性能的多模态夹爪技术对于有效机器人食品废弃物分拣的必要性。

Conclusion: GRAB框架为食品废弃物分拣中的机器人抓取提供了前所未有的深度评估，揭示了关键设计考虑因素，并强调了开发能够应对复杂现实环境的多模态夹爪技术的重要性。
...
### [189] [When the Inference Meets the Explicitness or Why Multimodality Can Make Us Forget About the Perfect Predictor](https://arxiv.org/abs/2602.18850)
*J. E. Domínguez-Vidal,Alberto Sanfeliu*

Main category: cs.RO

TL;DR: 该研究比较了四种人机协作通信系统：两种意图预测模型（基于力预测和增强速度预测）和两种显式通信方法（按钮界面和语音命令），在移动社交机器人IVO上进行物体运输任务测试。结果显示：1）性能达到一定水平后，人类不再感知技术改进；2）人类偏好更自然的系统，即使故障率更高；3）最佳方案是预测与显式通信的结合。


<details>
  <summary>Details</summary>
Motivation: 现有意图预测模型因人类行为的随机性存在不确定性，导致一些研究者开始倡导使用显式通信系统来明确获取人类意图。本研究旨在比较意图预测与显式通信系统在人机协作任务中的表现。

Method: 使用IVO移动社交机器人（配备力传感器和LiDAR）进行协作物体运输任务测试。比较四种系统：基于力预测的意图预测器、增强速度预测算法、按钮界面和语音命令识别系统。75名志愿者完成255次实验，分为三组：第一轮测试推理系统，第二轮测试通信系统，第三轮测试组合策略。

Result: 1）一旦系统性能达到足够水平，人类不再注意到技术改进并给予积极评价；2）人类偏好更自然的系统（如语音命令），即使这些系统故障率更高；3）最受青睐的方案是意图预测与显式通信系统的恰当组合。

Conclusion: 在人机协作中，单纯追求技术性能提升可能不被用户感知，自然交互体验更为重要。最佳的人机通信策略应结合意图预测和显式通信系统，在可靠性和自然性之间取得平衡。
...
### [190] [Gait Asymmetry from Unilateral Weakness and Improvement With Ankle Assistance: a Reinforcement Learning based Simulation Study](https://arxiv.org/abs/2602.18862)
*Yifei Yuan,Ghaith Androwis,Xianlian Zhou*

Main category: cs.RO

TL;DR: 该研究开发了一个基于强化学习的肌肉骨骼模拟框架，用于量化单侧肌肉无力对步态对称性的影响，并评估踝关节外骨骼辅助在受损条件下的效果。


<details>
  <summary>Details</summary>
Motivation: 单侧肌肉无力会导致不对称步态，破坏肢体间协调和站立时间。本研究旨在建立一个模拟和学习为基础的工作流程，支持在患者实验之前进行早期控制器开发。

Method: 采用强化学习（RL）的肌肉骨骼模拟框架，通过将右腿肌肉力量减少到基线水平的75%、50%和25%来诱导不对称步态。使用离地时间、峰值接触力和关节级对称性指标量化步态不对称性，并评估踝关节外骨骼辅助的效果。

Result: 随着肌肉无力程度增加，时间和运动学不对称性逐渐增大，踝关节最为明显。踝关节活动范围对称性从100%力量时的近对称行为（SI=+6.4%；r=0.974）恶化到25%力量时的严重不对称（SI=-47.1%，r=0.889）。在50%力量时，踝关节外骨骼辅助改善了运动学对称性，将踝关节SI从25.8%降低到18.5%，相关性从r=0.948提高到0.966。
...
### [191] [Equivalence and Divergence of Bayesian Log-Odds and Dempster's Combination Rule for 2D Occupancy Grids](https://arxiv.org/abs/2602.18872)
*Tatiana Berlenko,Kirill Krinkin*

Main category: cs.RO

TL;DR: 提出基于pignistic变换的方法，公平比较贝叶斯对数几率与Dempster组合规则在占据栅格地图中的性能，通过匹配观测决策概率来隔离融合规则与传感器参数化的影响。


<details>
  <summary>Details</summary>
Motivation: 需要公平比较贝叶斯融合与Dempster组合规则在占据栅格地图中的性能，避免传感器参数化对比较结果的影响。

Method: 采用基于pignistic变换的方法，匹配每个观测的决策概率，从而将融合规则与传感器参数化分离。在BetP匹配和归一化可信度匹配两种标准下进行测试。

Result: 在BetP匹配下，贝叶斯融合在模拟、两个真实激光雷达数据集和下游路径规划中均表现更优（15/15方向一致性，p=3.1e-5），绝对差异较小（0.001-0.022）。在归一化可信度匹配下，结果方向反转，表明结果依赖于匹配标准。

Conclusion: 该方法可用于未来任何贝叶斯/信念函数比较，比较结果高度依赖于所选的匹配标准，没有绝对的优劣之分。
...
### [192] [Temporal-Logic-Aware Frontier-Based Exploration](https://arxiv.org/abs/2602.18951)
*Azizollah Taheri,Derya Aksaray*

Main category: cs.RO

TL;DR: 提出了一种基于承诺状态的未知环境下机器人时态逻辑运动规划方法，用于处理标签位置未知的scLTL规范任务


<details>
  <summary>Details</summary>
Motivation: 解决自主机器人在未知环境中执行时态逻辑任务的问题，当所需标签的确切位置未知时，需要一种能够引导机器人完成任务同时保留所有可能满足方式的规划方法

Method: 引入承诺状态概念，捕获不可逆行动导致的中间任务进展；提出基于前沿探索的完备算法，战略性地引导机器人向任务进展，同时保留所有可能的满足方式

Result: 提出的方法在仿真中得到验证，能够有效处理未知环境中的scLTL规范任务

Conclusion: 通过承诺状态和前沿探索算法，实现了在未知环境中对scLTL规范任务的完备规划，为机器人自主执行复杂逻辑任务提供了有效解决方案
...
### [193] [TactEx: An Explainable Multimodal Robotic Interaction Framework for Human-Like Touch and Hardness Estimation](https://arxiv.org/abs/2602.18967)
*Felix Verstraete,Lan Wei,Wen Fan,Dandan Zhang*

Main category: cs.RO

TL;DR: TactEx是一个可解释的多模态机器人交互框架，通过融合视觉、触觉和语言信息，实现类似人类的硬度感知和交互指导，在水果成熟度评估任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 准确感知物体硬度对于安全和灵巧的接触密集型机器人操作至关重要。现有系统缺乏结合视觉、触觉和语言的多模态可解释框架，难以实现类似人类的硬度感知和交互指导。

Method: 提出TactEx框架：1) 融合GelSight-Mini触觉流、RGB观测和语言提示；2) 使用ResNet50+LSTM模型从序列触觉数据估计硬度；3) 跨模态对齐模块结合视觉线索和大型语言模型指导；4) 比较YOLO与Grounded-SAM进行接触点选择；5) 轻量级LLM解析用户指令并生成基于触觉输出的自然语言解释。

Result: 1) 系统能显著区分不同成熟度水平（所有水果对的p<0.01）；2) Grounded-SAM比YOLO在细粒度分割和接触点选择上更鲁棒；3) 端到端评估中，简单用户查询任务成功率达90%；4) 无需大规模调优即可泛化到新任务。

Conclusion: 结合预训练的视觉和触觉模型与语言基础，有望推进机器人中可解释的、类似人类的触觉感知和决策制定。TactEx展示了多模态融合在接触密集型机器人任务中的潜力。
...
### [194] [Bumper Drone: Elastic Morphology Design for Aerial Physical Interaction](https://arxiv.org/abs/2602.18976)
*Pongporn Supa,Alex Dunnett,Feng Xiao,Rui Wu,Mirko Kovac,Basaran Bahadir Kocer*

Main category: cs.RO

TL;DR: 无人机平台配备弹性触角，实现"接触即走"机动，通过被动动力学响应而非主动控制来维持与障碍物的接近，减少俯仰振荡达38-54%。
...
Method: 开发配备弹性触角的无人机平台，实现"接触即走"机动。利用环境交互作为体现控制形式，通过无人机-障碍物系统的被动动力学响应（类似质量-弹簧-阻尼系统）实现低级稳定和近障碍物导航。

Result: 弹性触角能吸收冲击能量同时保持飞行器稳定性，与刚性触角配置相比减少38%的俯仰振荡。下部触角布置进一步减少约54%的俯仰振荡。平台还能实现与静态物体的稳定持续接触。

Conclusion: 弹性触角设计使无人机能够通过被动动力学响应实现稳定的环境交互，减少对主动控制的依赖，为空中物理交互提供了新方法。
...
### [195] [FruitTouch: A Perceptive Gripper for Gentle and Scalable Fruit Harvesting](https://arxiv.org/abs/2602.18991)
*Ruohan Zhang,Mohammad Amin Mirzaee,Wenzhen Yuan*

Main category: cs.RO

TL;DR: FruitTouch是一种用于水果采摘的紧凑型夹爪，集成了高分辨率视觉触觉传感，能够稳定抓取多种水果，实时估计力、检测滑动并预测软硬度。
...
Method: 提出FruitTouch紧凑型夹爪，通过优化的光学设计集成高分辨率视觉触觉传感，嵌入式摄像头捕获触觉图像，配置适应多种水果尺寸，保持低成本且机械结构简单。

Result: 触觉图像提供丰富信息用于实时力估计、滑动检测和软硬度预测，在实际水果采摘实验中验证了稳定的抓取性能和有效的损伤预防。

Conclusion: FruitTouch通过集成视觉触觉传感，为自动化水果采摘提供了紧凑、多功能且可靠的解决方案，解决了劳动力短缺问题。
...
### [196] [A Checklist for Deploying Robots in Public: Articulating Tacit Knowledge in the HRI Community](https://arxiv.org/abs/2602.19038)
*Claire Liang,Franziska Babel,Hannah Pelikan,Sydney Thompson,Xiang Zhi Tan*

Main category: cs.RO

TL;DR: 论文提出了一个用于公共机器人部署的检查清单指南，旨在帮助研究人员避免常见错误，基于HRI社区的经验和专业知识构建。


<details>
  <summary>Details</summary>
Motivation: 公共机器人部署中存在许多未记录的挑战和常见陷阱，这造成了高门槛并导致可避免的错误重复发生。HRI社区中的隐性知识需要被明确表达和共享。

Method: 研究团队基于自身公共机器人部署经验收集关键主题，创建模块化翻转卡片形式的层次化检查清单。通过采访六位跨学科HRI专家来完善清单，并在真实公共研究中验证其有效性。

Result: 开发了一个开源、可定制的社区资源，以检查清单、卡片集和交互式网页工具的形式呈现，能够收集集体专业知识并持续演进。

Conclusion: 该检查清单为公共HRI研究提供了实用的指导框架，有助于降低部署门槛，避免重复错误，并促进社区知识的积累和共享。
...
### [197] [Path planning for unmanned surface vehicle based on predictive artificial potential field. International Journal of Advanced Robotic Systems](https://arxiv.org/abs/2602.19062)
*Jia Song,Ce Hao,Jiangcheng Su*

Main category: cs.RO

TL;DR: 本文提出了一种新的预测人工势场方法，通过加入时间信息和预测势能来为高速无人水面艇规划更平滑的路径，解决了传统方法的局部最小值问题和路径不平滑问题。


<details>
  <summary>Details</summary>
Motivation: 高速无人水面艇的路径规划需要更复杂的解决方案来减少航行时间和节省能源。传统人工势场方法在全局和局部路径规划中存在局限性，特别是在处理局部最小值和路径平滑性方面存在问题。

Method: 提出预测人工势场方法，包含三个关键改进：角度限制、速度调整和预测势能。该方法考虑了车辆动力学和局部最小可达性，通过时间信息和预测势能来规划更平滑的路径。

Result: 与传统人工势场相比，预测人工势场成功限制了最大转弯角度，缩短了航行时间，智能避障。仿真结果表明，该方法解决了凹形局部最小值问题，提高了特殊场景下的可达性。

Conclusion: 预测人工势场方法为高速无人水面艇生成了更高效的路径，减少了航行时间并节省了能源，在路径平滑性和避障能力方面优于传统方法。
...
### [198] [Design, Locomotion, and Control of Amphibious Robots: Recent Advances](https://arxiv.org/abs/2602.19077)
*Yi Jin,Chang Liu,Roger D. Quinn,Robert J. Wood,C. Chase Cao*

Main category: cs.RO

TL;DR: 这篇综述回顾了两栖机器人在陆地和水下无缝运行的最新进展，重点关注运动机制、驱动技术和传感器控制集成，并指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 两栖机器人在环境保护、灾害响应和国防等领域有重要应用价值，但需要克服陆地和水下不同环境下的运行挑战，因此需要系统总结当前技术进展和未来发展方向。

Method: 采用文献综述方法，系统分析两栖机器人的三个关键技术领域：运动机制（移动策略）、驱动技术（基于材料的执行器）以及传感器与控制系统的集成。

Result: 综述总结了近年来两栖机器人在运动策略、材料驱动器和控制系统方面的最新进展，识别了当前技术挑战，并为未来研究提供了方向性指导。

Conclusion: 两栖机器人技术已取得显著进展，但仍需进一步研究以提高效率、韧性和多功能性。未来的发展方向包括更高效的运动机制、先进的驱动技术和智能控制系统，以实现真正的自主性和适应性。
...
### [199] [A User-driven Design Framework for Robotaxi](https://arxiv.org/abs/2602.19107)
*Yue Deng,Changyang He*

Main category: cs.RO

TL;DR: 研究通过实地访谈和体验发现，用户选择无人驾驶出租车主要因低成本、社交推荐和好奇心，看重自主感和驾驶一致性，但面临灵活性不足、透明度不够、管理困难等挑战，隐私安全伦理问题突出，提出基于用户旅程的设计框架。


<details>
  <summary>Details</summary>
Motivation: 现有研究过于关注无人驾驶出租车的技术驾驶性能，而忽视了乘客在无人类司机情况下的实际体验和评价。先前研究多依赖模拟或假设场景，缺乏对真实世界使用情况的深入理解。

Method: 采用18次半结构化访谈和自民族志乘车体验相结合的方法，研究真实世界中的无人驾驶出租车使用情况。

Result: 用户选择无人驾驶出租车主要受低成本、社交推荐和好奇心驱动；看重自主感增强、驾驶行为一致性和标准化乘车体验；但面临灵活性有限、透明度不足、管理困难、边缘案例鲁棒性问题和紧急情况处理等挑战；隐私、安全、伦理和信任问题显著影响体验。

Conclusion: 提出基于用户旅程的端到端设计框架，包括乘车前配置（叫车）、情境感知接驳（上车）、行程中可解释性（行驶）和可追溯的乘车后反馈（下车），以指导无人驾驶出租车的交互和服务设计。
...
### [200] [Understanding Fire Through Thermal Radiation Fields for Mobile Robots](https://arxiv.org/abs/2602.19108)
*Anton R. Wagner,Madhan Balaji Rao,Xuesu Xiao,Sören Pirk*

Main category: cs.RO

TL;DR: 提出了一种基于实时热辐射场构建的移动机器人火灾环境导航方法，通过深度和热成像融合构建3D温度点云，利用斯蒂芬-玻尔兹曼定律估算热辐射，为机器人规划热安全路径。


<details>
  <summary>Details</summary>
Motivation: 火灾环境中的自主移动机器人导航是灾难响应中的关键能力，需要让机器人能够理解火灾并安全通过受火灾影响的环境。

Method: 将深度图像和热成像配准，获得带有温度值的3D点云；识别火灾区域；利用斯蒂芬-玻尔兹曼定律估算空置空间的热辐射；构建连续的热辐射场；将热约束嵌入到代价地图中，计算无碰撞且热安全的路径。

Result: 在波士顿动力Spot机器人上进行了受控实验验证，实验表明机器人能够避开危险区域同时到达导航目标。

Conclusion: 该方法为实现能够在火灾环境中自主部署的移动机器人铺平了道路，在搜救、消防和危险材料响应方面具有潜在应用价值。
...
### [201] [Distributed and Consistent Multi-Robot Visual-Inertial-Ranging Odometry on Lie Groups](https://arxiv.org/abs/2602.19173)
*Ziwei Kang,Yizhi Zhou*

Main category: cs.RO

TL;DR: 提出DC-VIRO框架，通过分布式协作融合视觉惯性里程计和UWB测距，解决多机器人GPS拒止环境下的定位问题，同时实现锚点自校准。


<details>
  <summary>Details</summary>
Motivation: 多机器人在GPS拒止环境中需要可靠的定位。现有UWB辅助VIO方法多为单机器人设计且依赖预校准锚点，实际应用中鲁棒性有限。

Method: 提出分布式协作视觉惯性测距里程计框架，将VIO与UWB测量紧密融合，锚点位置作为系统状态变量处理校准不确定性，通过机器人间通信共享锚点观测提供额外几何约束，采用李群上的右不变误差公式保持标准VIO的可观测性。

Result: 多机器人仿真结果表明，DC-VIRO显著提高了定位精度和鲁棒性，同时在分布式设置中实现了锚点自校准。

Conclusion: DC-VIRO框架通过分布式协作融合VIO和UWB测量，解决了多机器人GPS拒止环境下的定位问题，提高了系统鲁棒性并实现了锚点自校准。
...
### [202] [Visual Prompt Guided Unified Pushing Policy](https://arxiv.org/abs/2602.19193)
*Hieu Bui,Ziyan Gao,Yuya Hosoda,Joo-Ho Lee*

Main category: cs.RO

TL;DR: 提出一种统一推动策略，结合轻量级提示机制和流匹配策略，生成反应式、多模态推动动作，可广泛用于不同规划问题


<details>
  <summary>Details</summary>
Motivation: 现有推动方法通常依赖多步推动计划和预定义推动基元，应用范围有限，效率和通用性在不同场景中受限

Method: 将轻量级提示机制融入流匹配策略，形成统一推动策略；视觉提示可由高层规划器指定，使推动策略可在多种规划问题中复用

Result: 实验结果表明，该统一推动策略不仅优于现有基线方法，还能有效作为VLM引导规划框架中的底层基元，高效解决桌面清理任务

Conclusion: 提出的统一推动策略通过提示机制增强了推动动作的适应性和通用性，为机器人非抓取操作提供了更高效、灵活的解决方案
...
### [203] [The Price Is Not Right: Neuro-Symbolic Methods Outperform VLAs on Structured Long-Horizon Manipulation Tasks with Significantly Lower Energy Consumption](https://arxiv.org/abs/2602.19260)
*Timothy Duggan,Pierrick Lorang,Hong Lu,Matthias Scheutz*

Main category: cs.RO

TL;DR: 该研究对比了视觉-语言-动作模型与神经符号架构在结构化长时程操作任务上的表现，发现神经符号方法在成功率、泛化能力和能效方面显著优于VLA模型。


<details>
  <summary>Details</summary>
Motivation: 虽然VLA模型被提出作为通用机器人策略的途径，但其在结构化、长时程操作任务上的有效性和效率尚不明确，需要与神经符号架构进行实证比较。

Method: 研究对比了微调的开源VLA模型π0与结合PDDL符号规划和低层控制学习的神经符号架构，在模拟环境中评估了汉诺塔变体任务的表现和能耗。

Result: 在3块任务中，神经符号模型达到95%成功率，而最佳VLA仅34%；神经符号模型能泛化到未见过的4块变体（78%成功率），而VLA完全失败；VLA微调能耗比神经符号方法高近两个数量级。

Conclusion: 研究强调了端到端基础模型方法与结构化推理架构之间的重要权衡，表明显式符号结构在提高可靠性、数据效率和能效方面的重要作用。
...
### [204] [3D Shape Control of Extensible Multi-Section Soft Continuum Robots via Visual Servoing](https://arxiv.org/abs/2602.19273)
*Abhinav Gandhi,Shou-Shan Chiang,Cagdas D. Onal,Berk Calli*

Main category: cs.RO

TL;DR: 提出基于视觉的软体连续机械臂全身形状控制算法，无需本体传感器，利用外部摄像头实现全局稳定的形状调节


<details>
  <summary>Details</summary>
Motivation: 现有视觉控制算法主要调节末端执行器位姿，无法利用软体机械臂的运动学冗余性；现有方法存在局部极小值问题，且需要本体传感器信息

Method: 提出基于模型的2.5D形状视觉伺服控制，利用外部摄像头获取机械臂全身形状图像，通过逆运动学求解器生成参考特征，无需参考姿态图像

Result: 在多段连续机械臂上验证了控制器能精确调节全身形状，稳态误差在1mm内，实现了堆叠、倾倒、拉动等物体操作任务

Conclusion: 该视觉控制算法能有效调节软体连续机械臂的全身形状，无需本体传感器，具有全局稳定性和高精度，适用于多种操作任务
...
### [205] [Safe and Interpretable Multimodal Path Planning for Multi-Agent Cooperation](https://arxiv.org/abs/2602.19304)
*Haojun Shi,Suyu Ye,Katherine M. Guerrerio,Jianzhi Shen,Yifan Yin,Daniel Khashabi,Chien-Ming Huang,Tianmin Shu*

Main category: cs.RO

TL;DR: CaPE：基于视觉语言模型和模型规划的安全可解释多模态路径规划方法，通过语言通信实现去中心化智能体间的路径级协作


<details>
  <summary>Details</summary>
Motivation: 去中心化智能体需要快速适应其他智能体的行为以实现成功协作，但在无法可靠预测彼此意图和计划时，语言通信对确保安全至关重要。特别是在需要避免碰撞或执行物理协作（如联合搬运）的路径级协作场景中。

Method: 提出CaPE（Code as Path Editor）方法，利用视觉语言模型合成路径编辑程序，并通过模型规划器验证，将语言通信安全且可解释地转化为路径计划更新。

Result: 在多种模拟和现实场景（包括自动驾驶、家庭环境和联合搬运任务中的多机器人及人机协作）中验证了CaPE的有效性。CaPE可作为即插即用模块集成到不同机器人系统中，显著提升机器人根据其他机器人或人类语言通信调整计划的能力。

Conclusion: CaPE结合了基于VLM的路径编辑程序合成和基于模型规划的安全性，使机器人能够在保持安全性和可解释性的同时实现开放式协作。该方法为去中心化智能体间的安全协作提供了一种有效的解决方案。
...
### [206] [WildOS: Open-Vocabulary Object Search in the Wild](https://arxiv.org/abs/2602.19308)
...
TL;DR: WildOS是一个用于长距离、开放词汇物体搜索的统一系统，结合了安全几何探索与语义视觉推理，在复杂户外环境中实现自主导航。


<details>
  <summary>Details</summary>
Motivation: 在复杂、无结构的户外环境中，机器人需要在没有先验地图和有限深度感知的情况下进行长距离操作。仅依赖几何前沿进行探索往往不足，需要语义推理能力来确定安全路径和探索方向。
...
Result: 在各种越野和城市地形中的闭环现场实验表明，WildOS能够实现稳健导航，在效率和自主性方面显著优于纯几何和纯视觉基线方法。

Conclusion: 视觉基础模型有潜力驱动开放世界机器人行为，使其既具有语义感知又基于几何基础。WildOS展示了语义推理与几何安全探索相结合的有效性。
...
### [207] [TOPReward: Token Probabilities as Hidden Zero-Shot Rewards for Robotics](https://arxiv.org/abs/2602.19313)
...
<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作模型在强化学习中面临样本效率低和真实世界奖励稀疏的问题，需要开发能够提供细粒度反馈的通用过程奖励模型。现有的时间价值函数往往无法泛化到训练域之外。

Method: 提出TOPReward方法，直接从预训练视频视觉语言模型的内部token对数概率中提取任务进度，而不是像先前方法那样提示模型直接输出进度值。这种方法避免了数值表示错误的问题。

Result: 在130多个不同的真实世界任务和多个机器人平台上进行零样本评估，TOPReward在Qwen3-VL上实现了0.947的平均价值顺序相关性，显著优于最先进的GVL基线（在相同开源模型上相关性接近零）。

Conclusion: TOPReward作为一种多功能工具，可用于下游应用，包括成功检测和奖励对齐的行为克隆，为解决视觉-语言-动作模型在强化学习中的泛化问题提供了有效方案。
...
### [208] [Online Navigation Planning for Long-term Autonomous Operation of Underwater Gliders](https://arxiv.org/abs/2602.19315)
...
TL;DR: 该研究提出了一种基于蒙特卡洛树搜索的在线规划系统，用于水下滑翔机器人的自主导航，通过物理信息模拟器处理控制不确定性和洋流预测，并在北海进行了为期3个月、1000公里的实地验证。


<details>
  <summary>Details</summary>
Motivation: 水下滑翔机器人已成为海洋采样的重要工具，但大型机队管理工具缺乏，长期自主部署成功案例稀少，表明缺乏合适的方法论和系统。

Method: 将滑翔机导航规划建模为随机最短路径马尔可夫决策过程，提出基于蒙特卡洛树搜索的样本在线规划器，使用物理信息模拟器生成样本，模拟控制不确定性和洋流预测，参数通过历史数据拟合，集成到Slocum滑翔机的自主指挥控制系统中实现闭环重规划。

Result: 在北海进行了两次实地部署，总计约3个月、1000公里的自主操作，结果显示相比直线导航提高了效率，证明了样本规划在长期海洋自主性中的实用性。

Conclusion: 提出的基于蒙特卡洛树搜索的样本规划系统能够有效提升水下滑翔机器人的自主导航效率，为长期海洋自主性提供了实用的解决方案。
...
### [209] [Design and Control of Modular Magnetic Millirobots for Multimodal Locomotion and Shape Reconfiguration](https://arxiv.org/abs/2602.19346)
*Erik Garcia Oyono,Jialin Lin,Dandan Zhang*

Main category: cs.RO

TL;DR: 提出了一种模块化磁控毫米机器人平台，包含三种功能模块，通过二维磁场实现运动、重构和货物操作，在受限环境中展示多模态行为
...
Method: 设计包含三种立方体模块的平台：自由模块支持自组装和重构，固定模块实现翻转行走运动，夹持模块用于货物操作；通过可编程的时变二维均匀和梯度磁场输入驱动运动和重构

Result: 实验展示了使用实时视觉反馈和A*路径规划的闭环导航，建立了稳健的单模块控制能力；系统在低场强下实现自组装、多模态转换和拆卸；链到夹持转换成功率90%，链到方形转换一致性较低，表明模块几何形状影响重构可靠性

Conclusion: 建立了一个能够实现多模态行为和稳健控制的模块化机器人平台，为在受限环境中实现可扩展和自适应任务执行提供了有前景的途径
...
### [210] [Vid2Sid: Videos Can Help Close the Sim2Real Gap](https://arxiv.org/abs/2602.19359)
*Kevin Qiu,Yu Zhang,Marek Cygan,Josie Hughes*

Main category: cs.RO

TL;DR: Vid2Sid：基于视频的系统识别框架，利用基础模型感知和VLM优化器分析仿真-现实视频对，诊断物理参数不匹配，提供可解释的参数更新建议


<details>
  <summary>Details</summary>
Motivation: 传统机器人仿真器物理参数校准通常依赖手动调整或黑盒优化器，这些方法虽然能减少误差但无法解释哪些物理差异导致了误差。在仅使用外部相机感知的情况下，问题更加复杂，因为存在感知噪声且缺乏直接的力或状态测量

Method: 提出Vid2Sid系统识别流程，结合基础模型感知和VLM-in-the-loop优化器，分析成对的仿真-现实视频，诊断具体不匹配，并通过自然语言推理提出物理参数更新建议

Result: 在肌腱驱动手指（MuJoCo刚体动力学）和可变形连续触手（PyElastica软体动力学）上评估，在训练期间未见过的sim2real控制任务中，Vid2Sid在所有设置中达到最佳平均排名，匹配或超越黑盒优化器，同时提供可解释的推理。Sim2sim验证显示Vid2Sid最准确地恢复地面真实参数（平均相对误差低于13% vs. 28-98%）

Conclusion: Vid2Sid提供了一种可解释的视频驱动系统识别方法，当感知清晰且仿真器表达能力强时，VLM引导的优化表现出色，而模型类别限制在更具挑战性的设置中限制了性能
...
### [211] [Seeing Farther and Smarter: Value-Guided Multi-Path Reflection for VLM Policy Optimization](https://arxiv.org/abs/2602.19372)
*Yanting Yang,Shenyuan Gao,Qingwen Bu,Li Chen,Dimitris N. Metaxas*

Main category: cs.RO

TL;DR: 提出了一种新的测试时计算框架，将状态评估与动作生成解耦，通过显式建模动作计划的优势并使用可扩展的评论器进行估计，结合波束搜索探索多个未来路径，显著提升了机器人操作任务的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 解决复杂、长时程机器人操作任务需要物理交互的深度理解、长期后果推理和精确高层规划。现有基于视觉语言模型的反射规划方法存在局限性：依赖低效且不准确的隐式状态值学习、仅评估单一贪婪未来、推理延迟大。

Method: 提出测试时计算框架，将状态评估与动作生成解耦；显式建模动作计划优势（通过减少与目标距离量化）；使用可扩展评论器估计；采用波束搜索探索多个未来路径并在解码时聚合以建模期望长期回报；引入轻量级基于置信度的触发机制，仅在必要时调用反思。

Result: 在多样化未见多阶段机器人操作任务上的实验表明，相比最先进基线方法，成功率提升24.6%，同时推理时间显著减少56.5%。

Conclusion: 该方法通过解耦状态评估与动作生成、显式建模动作优势、多路径探索和自适应反思触发，实现了更鲁棒的决策制定和显著效率提升，为复杂机器人操作任务提供了有效的解决方案。
...
### [212] [Hilbert-Augmented Reinforcement Learning for Scalable Multi-Robot Coverage and Exploration](https://arxiv.org/abs/2602.19400)
*Tamil Selvan Gurunathan,Aryya Gangopadhyay*

Main category: cs.RO

TL;DR: 将希尔伯特空间填充先验集成到去中心化多机器人学习与执行中，通过希尔伯特空间索引增强DQN和PPO算法，在稀疏奖励环境中提高探索效率并减少冗余


<details>
  <summary>Details</summary>
Motivation: 在稀疏奖励环境中，多机器人系统的探索存在冗余和效率低下的问题，需要结构化方法来提高覆盖效率和收敛速度

Method: 将希尔伯特空间填充先验集成到DQN和PPO算法中，使用希尔伯特空间索引来结构化探索；开发航点接口将希尔伯特排序转换为曲率有界、时间参数化的SE(2)轨迹，适用于资源受限的机器人

Result: 实验显示在覆盖效率、冗余减少和收敛速度方面优于DQN/PPO基线；在波士顿动力Spot腿式机器人上验证，在室内环境中执行生成的轨迹，实现了可靠的覆盖和低冗余

Conclusion: 几何先验能够提高群体机器人和腿式机器人的自主性和可扩展性，希尔伯特空间填充方法为多机器人系统提供了有效的结构化探索框架
...
### [213] [Anticipate, Adapt, Act: A Hybrid Framework for Task Planning](https://arxiv.org/abs/2602.19518)
*Nabanita Dash,Ayush Kaura,Shivam Singh,Ramandeep Singh,Snehasis Banerjee,Mohan Sridharan,K. Madhava Krishna*

Main category: cs.RO

TL;DR: 论文提出了一种混合框架，将LLM的通用预测能力与RDDL的概率序列决策能力相结合，使机器人能够预测和适应人机协作中的潜在故障。


<details>
  <summary>Details</summary>
Motivation: 尽管现有AI规划系统和大型语言模型性能出色，但由于任务和结果的不确定性，机器人预测和适应故障的能力仍然是实现有效人机协作的关键挑战。

Method: 提出混合框架：集成LLM的通用预测能力和RDDL的概率序列决策能力。机器人通过推理任务和人类能力，预测因能力不足或领域对象缺失导致的潜在故障，并执行预防或恢复动作。

Result: 在VirtualHome 3D仿真环境中的实验评估显示，相比最先进的基线方法，该方法在性能上有显著提升。

Conclusion: 通过结合LLM和RDDL的混合框架，能够有效解决人机协作中的故障预测和适应问题，提高协作性能。
...
### [214] [Cost-Aware Diffusion Active Search](https://arxiv.org/abs/2602.19538)
*Arundhati Banerjee,Jeff Schneider*

Main category: cs.RO

TL;DR: 本文提出了一种基于扩散模型的主动搜索方法，通过采样前瞻动作序列来平衡探索与利用，避免了传统树搜索的计算开销，在单智能体和多智能体团队中实现了高效的成本感知决策。


<details>
  <summary>Details</summary>
Motivation: 主动搜索需要在未知环境中平衡探索与利用，传统的前瞻算法依赖计算昂贵的搜索树，而扩散模型具有序列建模能力，可以避免构建完整的搜索树，提高决策效率。

Method: 利用扩散模型的序列建模能力采样前瞻动作序列，识别并解决先前扩散强化学习方法在主动搜索中的乐观偏差问题，提出适用于单智能体和多智能体团队的成本感知决策解决方案。

Result: 提出的算法在离线强化学习中优于标准基线方法，在完全恢复率方面表现更好，并且在成本感知主动决策中比树搜索方法计算效率更高。

Conclusion: 扩散模型可以有效地用于主动搜索决策，通过采样前瞻动作序列平衡探索与利用，避免了传统树搜索的计算负担，为单智能体和多智能体团队提供了高效的成本感知决策方案。
...
### [215] [Chasing Ghosts: A Simulation-to-Real Olfactory Navigation Stack with Optional Vision Augmentation](https://arxiv.org/abs/2602.19577)
*Kordel K. France,Ovidiu Daescu,Latifur Khan,Rohith Peddi*

Main category: cs.RO

TL;DR: 研究人员开发了一个开源无人机系统，使用最小传感器套件进行在线气味源定位，无需预定义覆盖模式、外部基础设施或复杂传感协调。


<details>
  <summary>Details</summary>
Motivation: 自主气味源定位对空中机器人具有挑战性，因为存在湍流、稀疏延迟的感官信号以及严格的负载和计算限制。现有无人机嗅觉系统依赖预定义覆盖模式、外部基础设施或复杂传感协调。

Method: 系统集成定制嗅觉硬件、机载传感和基于学习的导航策略（在仿真中训练并在真实四旋翼上部署）。使用最小框架，无需构建显式气体分布图或依赖外部定位系统。视觉作为可选补充模态加速导航。

Result: 在大型室内环境中使用乙醇源进行真实飞行实验验证，在现实气流条件下展示了一致的源寻找行为。系统成功导航直接朝向气味源。

Conclusion: 主要贡献是为基于无人机的嗅觉导航和源寻找提供了一个可复现的系统和方法框架，采用最小传感假设。开源了无人机固件、仿真代码、嗅觉-视觉数据集和电路板设计。
...
### [216] [Denoising Particle Filters: Learning State Estimation with Single-Step Objectives](https://arxiv.org/abs/2602.19651)
*Lennart Röstel,Berthold Bäuml*

Main category: cs.RO

TL;DR: 提出一种基于粒子滤波的机器人状态估计新方法，通过单独训练状态转移模型，利用马尔可夫性质，避免端到端序列训练的高成本
...
Method: 提出新型粒子滤波算法，从单个状态转移训练模型，充分利用机器人系统的马尔可夫性质。测量模型通过最小化去噪分数匹配目标隐式学习。推理时，使用学习的去噪器与（学习的）动力学模型近似求解贝叶斯滤波方程，引导预测状态朝向测量信息的数据流形。

Result: 在模拟的挑战性机器人状态估计任务上评估，相比调优的端到端训练基线表现出竞争性性能。方法具有经典滤波算法的可组合性优势。

Conclusion: 该方法提供了端到端训练状态估计的有效替代方案，具有训练效率高、可解释性好、可组合性强等优点，允许在不重新训练的情况下整合先验信息和外部传感器模型。
...
### [217] [Scalable Low-Density Distributed Manipulation Using an Interconnected Actuator Array](https://arxiv.org/abs/2602.19653)
*Bailey Dacre,Rodrigo Moreno,Jørn Lambertsen,Kasper Stoy,Andrés Faíña*

Main category: cs.RO

TL;DR: 本文提出了一种由模块化3自由度机器人瓦片组成的分布式操纵系统，通过柔性表面层连接形成连续可控的操纵表面，可在降低致动器密度的同时有效操纵小物体。


<details>
  <summary>Details</summary>
Motivation: 传统分布式操纵系统需要高密度的致动器阵列来有效操纵小物体，这增加了系统的复杂性和成本。需要一种能够在降低致动器密度的同时保持对小物体操纵能力的方法。

Method: 使用模块化3自由度机器人瓦片，通过柔性表面层相互连接形成连续可控的操纵表面。柔性层允许增加致动器间距而不影响物体操纵能力。开发了阵列的耦合工作空间表征和操纵策略，能够在N×N阵列中将物体平移到任意位置。

Result: 通过最小2×2原型进行实验验证，成功操纵了不同形状和大小的物体，证明了该方法在显著降低致动器密度的同时仍能保持鲁棒的控制能力。

Conclusion: 提出的柔性表面层方法有效解决了分布式操纵系统中致动器密度与物体操纵能力之间的权衡问题，为构建更经济高效的分布式操纵系统提供了可行方案。
...
### [218] [CACTO-BIC: Scalable Actor-Critic Learning via Biased Sampling and GPU-Accelerated Trajectory Optimization](https://arxiv.org/abs/2602.19699)
*Elisa Alboni,Pietro Noah Crestaz,Elias Fontanari,Andrea Del Prete*

Main category: cs.RO

TL;DR: CACTO-BIC改进CACTO方法，通过偏置初始状态采样提升数据效率，利用GPU加速减少计算时间，在四足机器人等复杂系统上实现实时应用


<details>
  <summary>Details</summary>
Motivation: 轨迹优化(TO)和强化学习(RL)各有优缺点：TO计算高效但处理非凸问题困难，RL对非凸问题更鲁棒但计算成本高。CACTO结合两者优势，但面临可扩展性问题，系统复杂度增加会显著提高TO计算成本

Method: 提出CACTO-BIC方法：1) 利用局部最优策略值函数的特性偏置初始状态采样，提高数据效率；2) 利用GPU加速减少计算时间；3) 在AlienGO四足机器人等高维系统上进行实验验证

Result: 与CACTO相比，CACTO-BIC在样本效率和计算速度上都有提升；与PPO相比，能在更短时间内达到相似解；在AlienGO四足机器人上验证了方法可扩展到高维系统并适合实时应用

Conclusion: CACTO-BIC通过改进初始状态采样策略和GPU加速，有效解决了CACTO的可扩展性问题，实现了在复杂高维系统上的高效轨迹优化，为实时机器人控制提供了可行方案
...
### [219] [Towards Dexterous Embodied Manipulation via Deep Multi-Sensory Fusion and Sparse Expert Scaling](https://arxiv.org/abs/2602.19764)
*Yirui Sun,Guangyu Zhuge,Keliang Liu,Jie Gu,Zhihao xia,Qionglin Ren,Chunxu tian,Zhongxue Ga*

Main category: cs.RO

TL;DR: DeMUSE是一个深度多模态统一稀疏专家框架，使用扩散变换器整合RGB、深度和6轴力传感器数据，通过自适应模态特定归一化和稀疏专家混合实现高效的多感官集成，在仿真和真实世界任务中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前以视觉为中心的范式往往忽略了复杂任务所需的关键力和几何反馈，而实现灵巧的具身操作需要深度集成异构多模态感官输入。

Method: 提出DeMUSE框架：1) 使用扩散变换器将RGB、深度和6轴力数据整合为统一序列化流；2) 采用自适应模态特定归一化重新校准模态感知特征，缓解表示不平衡；3) 使用稀疏专家混合架构提高模型容量同时保持低推理延迟；4) 联合去噪目标同步合成环境演化和动作序列以确保物理一致性。

Result: 在仿真和真实世界试验中分别达到83.2%和72.5%的成功率，展示了最先进的性能，验证了深度多感官集成对于复杂物理交互的必要性。

Conclusion: DeMUSE通过深度集成异构多模态感官输入，特别是整合视觉、深度和力反馈，显著提升了复杂物理交互任务的性能，证明了多感官深度集成在具身操作中的重要性。
...
### [220] [TactiVerse: Generalizing Multi-Point Tactile Sensing in Soft Robotics Using Single-Point Data](https://arxiv.org/abs/2602.19850)
*Junhui Lee,Hyosung Kim,Saekwang Nam*

Main category: cs.RO

TL;DR: TactiVerse：基于U-Net的软体触觉传感器框架，通过空间热图预测实现接触几何估计，即使仅用单点压痕数据训练也能实现高精度单点传感，并通过多触点数据增强显著提升多点识别能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的3D接触估计模型严重依赖训练数据集，难以泛化到多点传感等复杂场景，限制了软体机器人中高顺应性软材料的实时变形预测能力。

Method: 提出TactiVerse框架，将接触几何估计重新定义为空间热图预测任务，采用U-Net架构。即使仅用有限的单点压痕数据集训练，也能实现高精度传感，并通过添加多点接触数据增强训练集来提升多点识别能力。

Result: 单点传感方面，平均绝对误差为0.0589mm，优于传统回归CNN基线的0.0612mm。在两点识别任务中，通过数据增强将平均MAE从1.214mm显著提升至0.383mm，实现了从基础相互作用中推断复杂接触几何的能力。

Conclusion: 该方法能够从基本相互作用中成功推断复杂接触几何，解锁了高级多点和大面积形状传感能力，显著简化了基于标记的软体传感器开发，为实际触觉感知提供了高度可扩展的解决方案。
...
### [221] [Athena: An Autonomous Open-Hardware Tracked Rescue Robot Platform](https://arxiv.org/abs/2602.19898)
*Stefan Fabian,Aljoscha Schmidt,Jonas Süß,Dishant,Aum Oza,Oskar von Stryk*

Main category: cs.RO

TL;DR: Athena是一个开源硬件救援地面机器人研究平台，具有四个独立可重构的履带臂和低成本紧急停止解决方案，专为灾难响应任务设计。


<details>
  <summary>Details</summary>
Motivation: 在灾难响应和态势评估中，机器人有潜力降低救援人员的安全风险。由于任务环境和所需能力差异巨大且难以预知，需要异构机器人编队来覆盖广泛的任务需求。无人机能快速侦察但负载能力有限，地面机器人能携带传感器和机械臂但需要应对复杂地形。

Method: 开发了Athena救援地面机器人平台，具有四个独立可重构的履带臂（flippers），采用工业PU皮带和齿形插件的创新安装方案，允许更换和测试不同履带轮廓。配备最大伸展1.54米的机械臂，以及可靠的低成本远程紧急停止解决方案。

Result: Athena平台能够有效应对如瓦砾和楼梯等困难和非结构化地形，其可重构几何形状使其在导航挑战性地形方面特别有效。机械臂可用于操作门、阀门和其他感兴趣物体。

Conclusion: Athena作为一个开源硬件救援地面机器人研究平台，提供了完整的CAD和PCB文件以及所有底层软件作为开源贡献，支持灾难响应机器人技术的发展和应用。
...
### [222] [Scaling Law of Neural Koopman Operators](https://arxiv.org/abs/2602.19943)
*Abulikemu Abuduweili,Yuyang Pang,Feihan Li,Changliu Liu*

Main category: cs.RO

TL;DR: 本文为神经Koopman算子建立了样本量与模型维度之间的标度律，提出了两种轻量级正则化器来提升控制性能。


<details>
  <summary>Details</summary>
Motivation: 数据驱动的神经Koopman算子理论已成为线性化和控制非线性机器人系统的有力工具，但其性能取决于样本量与模型维度之间的权衡关系，而这一关系的标度律一直不明确。

Method: 建立了理论框架，推导了Koopman近似误差的理论上界，将其分解为采样误差和投影误差；基于理论结果，引入了两种轻量级正则化器：协方差损失（稳定学习到的潜在特征）和逆控制损失（确保模型与物理驱动对齐）。

Result: 在六个机器人环境中的系统实验证实，模型拟合误差遵循推导出的标度律，正则化器提高了动态模型拟合保真度，并增强了闭环控制性能。

Conclusion: 研究结果为学习Koopman动力学控制时，在数据收集和模型容量之间的努力分配提供了简单指导。
...
### [223] [Contextual Safety Reasoning and Grounding for Open-World Robots](https://arxiv.org/abs/2602.19983)
*Zachary Ravichadran,David Snyder,Alexander Robey,Hamed Hassani,Vijay Kumar,George J. Pappas*

Main category: cs.RO

TL;DR: CORE是一个安全框架，通过视觉语言模型在线推理上下文相关的安全规则，并在物理环境中实施这些规则，以应对开放世界环境中的上下文变化。


<details>
  <summary>Details</summary>
Motivation: 机器人在开放世界环境中运行时，安全行为依赖于上下文（如走廊是否拥挤、是否处于紧急状态）。传统安全方法在用户指定的上下文中强制执行固定约束，无法处理现实世界部署中开放式的上下文变化。

Method: CORE使用视觉语言模型（VLM）直接从视觉观察中持续推理上下文相关的安全规则，将这些规则在物理环境中进行空间定位，并通过控制屏障函数实施由此产生的空间定义的安全集。

Result: CORE在未见过的环境中强制执行上下文适当的行为，显著优于缺乏在线上下文推理的先前语义安全方法。消融研究验证了理论保证，并强调了VLM推理和空间定位对于在新环境中实施上下文安全的重要性。

Conclusion: CORE框架通过在线上下文推理、定位和实施，解决了开放世界环境中机器人安全行为的上下文依赖问题，提供了考虑感知不确定性的概率安全保证。
...
### [224] [Hydrodynamic Performance Enhancement of Unmanned Underwater Gliders with Soft Robotic Morphing Wings for Agility Improvement](https://arxiv.org/abs/2602.20054)
*A. Giordano,G. De Meurichy,V. Telazzi,C. Mucignat,I. Lunati,D. A. L. M. Louchard,M. Iovieno,S. F. Armanini,M. Kovac*

Main category: cs.RO

TL;DR: 软变形翼UUV相比传统刚性翼UUV实现9.75%的整体效率提升


<details>
  <summary>Details</summary>
Motivation: 评估软变形翼水下无人潜航器相比传统刚性翼的水动力效率，软变形翼可按需改变气动特性，效率提升可扩展UUV航程并影响任务可行性

Method: 对软变形翼及其搭载的UUV进行结构和计算流体动力学模拟

Result: 采用软翼的UUV相比传统刚性翼UUV实现9.75%的整体效率提升
...
### [225] [To Move or Not to Move: Constraint-based Planning Enables Zero-Shot Generalization for Interactive Navigation](https://arxiv.org/abs/2602.20055)
*Apoorva Vashisth,Manav Kulshrestha,Pranav Bakshi,Damon Conover,Guillaume Sartoretti,Aniket Bera*

Main category: cs.RO

TL;DR: 提出终身交互导航问题，让具有操作能力的移动机器人通过移动杂物来开辟路径，完成序列化的物体放置任务，使用LLM驱动的约束规划框架进行主动感知和推理。


<details>
  <summary>Details</summary>
Motivation: 现实世界场景（如家庭环境和仓库）中，杂物可能阻塞所有路径，传统视觉导航假设存在至少一条无障碍路径的局限性需要被突破，机器人需要能够主动改变环境来完成任务。

Method: 提出LLM驱动的约束规划框架，结合主动感知：LLM在结构化场景图上推理，决定移动哪个物体、放置在哪里、下一步观察哪里；标准运动规划器执行导航-拾取-放置或绕行序列。

Result: 在物理模拟器ProcTHOR-10k中评估，该方法优于非学习和学习基线方法；在真实硬件上进行了定性演示验证。

Conclusion: 终身交互导航框架使机器人能够通过主动移动杂物来开辟路径，LLM驱动的约束规划与主动感知结合能够有效处理环境变化的长期累积效应，完成序列化物体放置任务。
...
### [226] [AdaWorldPolicy: World-Model-Driven Diffusion Policy with Online Adaptive Learning for Robotic Manipulation](https://arxiv.org/abs/2602.20057)
*Ge Yuan,Qiyuan Qiao,Jing Zhang,Dong Xu*

Main category: cs.RO

TL;DR: AdaWorldPolicy是一个统一框架，结合世界模型、动作专家和力预测器，通过在线自适应学习增强动态环境下的机器人操作能力。


<details>
  <summary>Details</summary>
Motivation: 机器人操作需要能够预测物理结果并适应真实环境的策略。当前方法在动态条件下需要大量人工干预，需要开发能够最小化人类参与、适应动态环境的统一框架。

Method: 提出AdaWorldPolicy框架，包含三个模块：世界模型、动作专家和力预测器，均实现为互联的Flow Matching Diffusion Transformers。通过多模态自注意力层实现深度特征交换，并提出在线自适应学习策略，在动作生成模式和未来想象模式之间动态切换。

Result: 在模拟和真实机器人基准测试中，AdaWorldPolicy实现了最先进的性能，对分布外场景具有动态自适应能力。

Conclusion: 世界模型提供强监督信号，结合力-扭矩反馈可以缓解动态力变化，在线自适应学习策略能够创建强大的闭环机制，以最小开销适应视觉和物理领域变化。
...
### [227] [NovaPlan: Zero-Shot Long-Horizon Manipulation via Closed-Loop Video Language Planning](https://arxiv.org/abs/2602.20119)
...
<details>
  <summary>Details</summary>
Motivation: 解决长时程任务需要机器人将高层语义推理与低层物理交互相结合。现有的视觉语言模型和视频生成模型虽然能分解任务和想象结果，但缺乏真实世界执行所需的物理基础。

Method: 提出NovaPlan分层框架：高层使用VLM规划器闭环分解任务并监控执行，支持自主重规划；低层从生成视频中提取任务相关物体关键点和人手姿态作为运动学先验，采用切换机制选择更好的参考来生成机器人动作。

Result: 在三个长时程任务和功能操作基准测试中，NovaPlan能够执行复杂装配任务并展示灵巧的错误恢复行为，无需任何先验演示或训练。

Conclusion: NovaPlan通过统一闭环VLM/视频规划与几何基础机器人执行，实现了零样本长时程操作，在复杂任务中表现出鲁棒性和自主恢复能力。
...
### [228] [Simulation-Ready Cluttered Scene Estimation via Physics-aware Joint Shape and Pose Optimization](https://arxiv.org/abs/2602.20150)
*Wei-Cheng Huang,Jiaheng Han,Xiaohan Ye,Zherong Pan,Kris Hauser*

Main category: cs.RO

TL;DR: 提出了一种基于优化的真实到仿真场景估计方法，能够联合恢复多个刚性物体的形状和位姿，并在物理约束下处理物体间接触，适用于杂乱环境。


<details>
  <summary>Details</summary>
Motivation: 现有方法在杂乱环境中存在计算成本高、鲁棒性差、扩展到多个交互物体时泛化能力受限的问题，而估计仿真就绪的真实场景对于下游规划和策略学习任务至关重要。

Method: 采用统一的基于优化的公式，结合两个关键技术：1）利用全局可微的形状可微接触模型，允许在建模物体间接触的同时对物体几何和位姿进行联合优化；2）利用增广拉格朗日海森矩阵的结构稀疏性，开发计算成本随场景复杂度有利扩展的高效线性系统求解器。

Result: 在包含最多5个物体和22个凸包的杂乱场景实验中，该方法能够鲁棒地重建物理有效、仿真就绪的物体形状和位姿。

Conclusion: 该方法通过物理约束的联合形状-位姿优化和可微纹理细化，构建了端到端的真实到仿真场景估计流程，在复杂场景中表现出优越性能。
...
### [229] [Revisiting the Seasonal Trend Decomposition for Enhanced Time Series Forecasting](https://arxiv.org/abs/2602.18465)
*Sanjeev Panta,Xu Yuan,Li Chen,Nian-Feng Tzeng*

Main category: cs.LG

TL;DR: 该论文提出了一种基于时间序列分解的改进方法，通过分别处理趋势和季节性分量来提升多元时间序列预测性能，在保持线性时间复杂度的同时显著降低了预测误差。


<details>
  <summary>Details</summary>
Motivation: 现实世界应用中，时间序列预测面临重大挑战。现有方法在处理趋势和季节性分量时存在局限性，特别是可逆实例归一化仅对趋势分量有效，需要针对季节性分量开发更有效的处理方法。

Method: 1. 基于时间序列分解，分别处理趋势和季节性分量；2. 对趋势分量采用可逆实例归一化；3. 对季节性分量直接应用骨干模型，不进行任何归一化或缩放处理；4. 引入双MLP模型作为计算效率更高的解决方案。

Result: 1. 在四个最先进的基准模型上，平均MSE降低约10%；2. 在美国地质调查局水文数据集上取得显著改进；3. 所有模型保持线性时间复杂度，具有实际应用价值。

Conclusion: 通过分别优化趋势和季节性分量的处理方法，该研究成功提升了多元时间序列预测性能，同时保持了计算效率，在基准数据集和实际水文数据上都验证了方法的有效性。
...
### [230] [Support Vector Data Description for Radar Target Detection](https://arxiv.org/abs/2602.18486)
*Jean Pinsolle,Yadang Alexis Rouzoumka,Chengfang Ren,Chistèle Morisseau,Jean-Philippe Ovarlez*

Main category: cs.LG

TL;DR: 论文提出基于SVDD和Deep SVDD的雷达目标检测新方法，避免传统自适应检测器在杂波环境中的性能下降问题


<details>
  <summary>Details</summary>
Motivation: 传统雷达检测方法在杂波环境中性能下降，特别是当热噪声与杂波结合时，现有的稳健协方差估计方法仍存在局限性

Method: 采用支持向量数据描述(SVDD)及其深度扩展Deep SVDD，将其改造为CFAR检测器，提出了两种新的SVDD基检测算法

Result: 在模拟雷达数据上验证了所提方法的有效性

Conclusion: SVDD和Deep SVDD方法能够有效应对杂波环境中的雷达目标检测问题，避免了直接噪声协方差估计的局限性
...
<details>
  <summary>Details</summary>
Motivation: 内容安全团队需要反映用户实际体验的指标，而不仅仅是已报告的内容。准确测量违规内容的普遍性（用户浏览中违规内容的比例）面临挑战：违规通常罕见，人工标注成本高，导致频繁、平台代表性的研究进展缓慢。

Method: 1. 使用ML辅助权重从浏览流中抽取每日概率样本，将标注预算集中在高曝光和高风险内容上，同时保持无偏性；2. 使用由政策提示和金标准验证控制的多模态LLM标注抽样项目；3. 生成设计一致的普遍性估计，包括置信区间和仪表盘细分功能。

Result: 系统实现了"一个全局样本，多个维度分析"的设计目标：同一个每日样本通过后分层估计支持按界面、观看者地理位置、内容年龄等多个维度的普遍性分析。系统具有统计估计器、方差和置信区间构建、标注质量监控功能。

Conclusion: 该测量系统为内容安全团队提供了准确、可配置的普遍性测量工具，能够跨政策进行配置，通过工程化工作流程实现高效的内容安全监控和评估。
...
TL;DR: 提出一种基于姿态增强追踪数据的连续随机视觉层方法，量化足球运动员的视觉感知，解决传统视觉探索行为测量方法的局限性。
...
Method: 开发概率性视野和遮挡模型，结合头部和肩部旋转角度，为每个球员创建速度依赖的二维俯视平面视觉地图。将这些视觉地图与场地控制和场地价值表面结合，分析等待阶段（球员等待队友传球）和随后的持球阶段。

Result: 使用2024年美洲杯32场比赛的同步姿态增强追踪数据和持球事件数据，证明聚合视觉指标（如等待传球时观察到的防守区域百分比）能够预测带球动作结束时获得的受控场地价值。

Conclusion: 该方法不受球员位置限制，无需手动标注，提供连续测量值，并能无缝集成到现有足球分析框架中。作者开源了相关计算工具以支持集成。
...
TL;DR: 提出一种基于消费级智能手表生理数据的个性化、可解释压力预测模型，在16个时间窗口上评估，性能优于现有方法，睡眠指标是最稳定预测因子。


<details>
  <summary>Details</summary>
Motivation: 连续压力预测对生活方式干预有潜在价值，但现有方法缺乏个性化、可解释性，且消费级设备数据利用不足。

Method: 开发时间序列预测模型，利用心率变异性、活动模式和睡眠指标等多变量特征，在16个时间窗口（历史窗口：3、5、7、9天；预测窗口：1、3、5、7天）进行压力预测，并与最先进模型（Informer、TimesNet、PatchTST）和传统基线（CNN、LSTM、CNN-LSTM）比较。

Result: 在最优设置（5天输入、1天预测）下，模型达到MSE 0.053、MAE 0.190、RMSE 0.226，优于所有基线模型，相对最佳基线改进36.9%、25.5%、21.5%。可解释性分析显示睡眠指标是最主要且一致的压力预测因子（重要性：1.1，一致性：0.9-1.0），而活动特征存在高个体间变异性（0.1-0.2）。

Conclusion: 消费级可穿戴设备结合自适应、可解释深度学习能够提供适应个体生理反应的相关压力评估，为现实环境中可扩展、连续、可解释的心理健康监测奠定基础。
...
TL;DR: 多任务模块化算术中的Grokking现象研究：发现交错式泛化顺序、低维执行流形、权重衰减相结构、全息不可压缩性和横向脆弱性等五个一致现象


<details>
  <summary>Details</summary>
Motivation: 将几何分析扩展到多任务模块化算术场景，研究共享主干Transformer在双任务（模加+模乘）和三任务（模加+模乘+模平方）目标下的Grokking现象，探索从记忆到泛化的突然转变在多任务环境中的表现

Method: 在系统性的权重衰减扫描中，训练共享主干Transformer处理双任务（模加+模乘）和三任务（模加+模乘+模平方）目标，通过几何分析优化轨迹，研究低维执行流形、权重衰减相结构等特征

Result: 发现五个一致现象：1)交错式泛化顺序（乘法先泛化，平方次之，加法最后）；2)低维执行流形约束；3)权重衰减相结构影响泛化时间尺度；4)全息不可压缩性（解仅占据4-8个主方向但分布在全秩权重中）；5)横向脆弱性和冗余性（删除少量正交梯度分量即消除Grokking）

Conclusion: 多任务Grokking在参数空间中构建紧凑叠加子空间，权重衰减作为压缩压力，过量参数提供优化路径的几何冗余，支持了多任务泛化的动力学图景
...
TL;DR: AV-CTTA：一种音频-视觉连续测试时适应方法，通过选择性参数检索机制动态获取最佳融合层参数，避免灾难性遗忘，提升跨域性能


<details>
  <summary>Details</summary>
Motivation: 现有音频-视觉连续测试时适应方法存在灾难性遗忘问题，模型性能会因持续参数更新而下降至低于源模型水平。需要一种能在无源数据访问的情况下提升测试时性能的方法。

Method: 提出AV-CTTA方法：1）发现仅适应模态融合层不仅能提升当前域性能，还能增强后续域表现；2）利用融合层参数的强跨任务可迁移性；3）采用选择性参数检索机制，仅使用少量测试数据从缓冲区动态检索最佳融合层参数；4）将检索参数集成到模型中，适应当前测试分布，并保存供未来使用。

Result: 在涉及单模态和双模态损坏的基准数据集上进行广泛实验，结果显示AV-CTTA显著优于现有方法，同时最小化灾难性遗忘。

Conclusion: AV-CTTA通过选择性参数检索机制有效解决了音频-视觉连续测试时适应中的灾难性遗忘问题，在无源数据访问的情况下显著提升了模型性能。
...
TL;DR: 该研究开发了一个融合卫星雷达物理发展信号和新闻文本市场叙事的框架，用于预测迪拜19个次城区每周房价指数，发现在长期预测中多模态模型能显著提升准确性。


<details>
  <summary>Details</summary>
Motivation: 现有房地产价格指标通常只在城市层面发布且频率较低，限制了其在社区尺度监测和长期规划中的应用。研究旨在探索是否可以通过结合卫星雷达的物理发展信号和新闻文本的市场叙事，实现次城区每周房价指数的预测。

Method: 使用迪拜土地部门2015-2025年超过35万笔交易数据，构建了19个次城区的每周价格指数。研究框架融合了区域交易历史、Sentinel-1 SAR后向散射数据、结合词汇语气和语义嵌入的新闻情感分析，以及宏观经济背景。评估了从2周到34周不同时间跨度的预测效果。

Result: 结果具有强烈的时间跨度依赖性：在10周以内的短期预测中，仅使用价格历史数据就能达到与多模态配置相当的效果；但在14周以上的长期预测中，新闻情感和SAR数据变得至关重要。在26-34周的长期预测中，完整的多模态模型将平均绝对误差从4.48降低到2.93（减少35%），且各区域的改进都具有统计显著性。非参数学习器在此数据体系中始终优于深度架构。

Conclusion: 该研究为每周次城区指数预测建立了基准，并证明遥感数据和新闻情感分析在战略相关的时间跨度上能显著提高预测能力，为社区尺度的房地产监测和长期规划提供了有效工具。
...
### [237] [Learning Beyond Optimization: Stress-Gated Dynamical Regime Regulation in Autonomous Systems](https://arxiv.org/abs/2602.18581)
*Sheng Ran*

Main category: cs.LG

TL;DR: 论文提出了一种无需显式目标函数的学习框架，通过评估内部动态的健康状况来调节结构可塑性，实现自主学习和自我调节。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习方法依赖明确的标量目标函数进行优化，但在需要长期自主运行的系统中，目标可能变得模糊、变化甚至缺失。需要解决在没有外部目标函数的情况下，系统如何判断自身动态是否健康以及如何自我调节的问题。

Method: 提出双时间尺度架构：快速状态演化与慢速结构适应分离，通过内部生成的压力变量耦合。压力变量积累持续动态功能障碍的证据，结构修改不是连续进行，而是作为状态依赖的事件触发。通过最小玩具模型演示这一机制。

Result: 压力调节机制能够产生时间分段的、自组织的学习过程，无需依赖外部定义的目标。系统能够自主评估内部动态健康状况并触发结构重组。

Conclusion: 该框架为实现能够自我评估和内部调节结构重组的自主学习系统提供了一条可能路径，为无明确目标函数的学习开辟了新方向。
...
### [238] [GIST: Targeted Data Selection for Instruction Tuning via Coupled Optimization Geometry](https://arxiv.org/abs/2602.18584)
*Guanghui Min,Tianhao Huang,Ke Wan,Chen Chen*

Main category: cs.LG

TL;DR: GIST是一种新的数据选择方法，专门针对参数高效微调（如LoRA）中的梯度耦合问题，通过子空间对齐替代传统的坐标轴对齐缩放，显著降低了计算和存储成本。


<details>
  <summary>Details</summary>
Motivation: 现有基于优化器统计（如Adam状态）的数据选择方法假设参数坐标独立，但在参数高效微调（PEFT）方法如LoRA中，优化几何表现出强烈的跨参数耦合和非对角相互作用，同时任务相关更新方向被限制在低维子空间中，传统方法在此设定下失效。

Method: 提出GIST（梯度等距子空间变换）方法：1）通过谱滤波（SVD）从验证梯度中恢复任务特定子空间；2）将训练梯度投影到这个耦合子空间中；3）根据样本与目标方向的对齐程度进行评分选择。

Result: 在相同选择预算下，GIST仅需0.29%的存储和25%的计算时间，就能达到或超越最先进基线的性能。

Conclusion: GIST为参数高效微调场景提供了一种简单而原则性的数据选择方法，通过子空间对齐有效解决了传统坐标轴对齐方法在梯度耦合环境中的局限性，实现了高效且有效的训练样本选择。
...
<details>
  <summary>Details</summary>
Motivation: 多任务学习中，识别哪些任务应该一起学习是一个基本问题。由于为所有可能的任务组合训练多任务学习模型成本过高，需要一种高效预测任务组是否受益于共同学习的方法。

Method: ETAP框架包含两个部分：1) 基于梯度的线性估计器（亲和性分数），通过测量任务间参数更新的相似性来评估任务对或任务组的亲和性；2) 非线性预测器，通过有限的任务组真实增益数据进行训练，对线性估计进行细化和残差修正。

Result: 在基准数据集上，ETAP在多任务学习增益预测方面表现优异，能够实现更有效的任务分组，在多个应用领域超越了现有最先进的方法。

Conclusion: ETAP通过集成原理性和数据驱动的估计器，提供了一种可扩展且准确的多任务学习任务亲和性预测框架，能够有效指导任务分组决策，提高多任务学习效率。
...
TL;DR: 论文提出了一种诊断性评估方法，通过使用Multi-News集群作为固定证据池来隔离重排序行为，消除检索质量的影响，从而直接分析排序策略本身。


<details>
  <summary>Details</summary>
Motivation: 传统重排序评估将排序行为与检索质量耦合，无法区分排序策略本身的差异。需要一种能够隔离重排序行为的诊断方法，以便直接分析不同排序策略的特性。

Method: 使用Multi-News集群作为固定证据池，每个池限制为8个文档，向所有排序器提供相同的输入。以BM25和MMR作为词汇匹配和多样性优化的可解释基准点，在345个集群上进行评估。

Result: 发现不同LLM的冗余模式存在差异：一个LLM在较大选择预算时隐含地增加多样性，而另一个则增加冗余。LLM在小选择预算下的词汇覆盖率表现不佳。LLM排序与两个基线策略都有显著差异，而非持续近似任一策略。
...
TL;DR: 该论文提出了一种通过双模拟编码器增强世界模型鲁棒性的方法，解决了现有潜在预测架构对"慢特征"敏感导致的测试时鲁棒性下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于高维视觉观测学习的世界模型（如DINO-WM）在测试时对背景变化和视觉干扰等"慢特征"敏感，导致鲁棒性下降。这些视觉变化与任务解决无关，但会影响模型性能。

Method: 在预测目标中增加双模拟编码器，强制控制相关状态等价，将具有相似转移动态的状态映射到相近的潜在状态，同时限制慢特征的贡献。该方法与预训练视觉编码器选择无关。

Result: 在简单导航任务的不同测试时背景变化和视觉干扰基准测试中，模型在所有基准上都一致提高了对慢特征的鲁棒性，同时潜在空间比DINO-WM小10倍。与DINOv2、SimDINOv2和iBOT特征配对时仍保持鲁棒性。

Conclusion: 通过双模拟编码器增强预测目标能有效提高世界模型对慢特征的鲁棒性，减少潜在空间维度，且与预训练视觉编码器选择无关，具有广泛适用性。
...
TL;DR: ARTIST提出了一种新的时间序列推理框架，将推理过程转化为顺序决策问题，通过控制器-推理器架构和强化学习实现自适应时间片段选择和条件推理，显著提升了时间序列问答任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列推理方法通常将整个时间序列编码为固定表示后再进行推理，无论整个序列是否相关。然而，证据可能分布在完整序列中或仅出现在少数短间隔内，模型需要决定检查哪些部分。因此需要一种能够自适应选择相关信息片段的方法。

Method: ARTIST采用控制器-推理器架构：控制器使用强化学习训练来选择信息丰富的片段，推理器生成基于片段的推理轨迹和最终答案。采用分层策略优化方法进行后训练，使模型在片段选择和问答行为上都能表现出色。推理过程中模型主动获取任务相关信息，而不是依赖完整序列的静态摘要。

Result: 在六个时间序列推理基准测试中，ARTIST比最强基线平均准确率提高了6.46个百分点。在罕见事件定位和多片段推理任务上提升最大。监督微调提高了性能，强化学习通过优化问题自适应片段选择提供了额外增益。

Conclusion: 选择性使用数据是驱动有效时间序列推理的关键。ARTIST通过将时间序列推理转化为顺序决策问题，实现了自适应信息获取，显著优于现有方法，特别是在需要精确定位和多段推理的任务上。
...
TL;DR: InfoNoise：基于信息论的扩散模型噪声调度方法，通过条件熵率分析噪声分配，用数据自适应训练取代手动调参，提升训练效率


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型依赖手动调参的噪声调度，计算效率低且难以跨数据集、分辨率和表示迁移。需要理论指导的数据自适应噪声调度方法

Method: 从信息论角度分析前向过程的条件熵率，作为噪声分配诊断指标。提出InfoNoise方法，基于训练中已计算的去噪损失估计熵减率，构建信息引导的噪声采样分布

Result: 在自然图像基准测试中匹配或超越调优的EDM风格调度，CIFAR-10上训练加速约1.4倍。在离散数据集上，标准图像调度存在显著不匹配时，达到更高质量且训练步数减少最多3倍

Conclusion: InfoNoise实现了数据自适应的噪声调度，减少跨领域扩散模型对每个数据集单独设计调度的需求，为扩散模型扩展提供更通用的训练框架
...
TL;DR: Potara：一个基于理论保证的联邦个性化框架，通过最优合并权重将联邦模型与本地模型结合，在任务异构性下实现更好的性能-通信权衡


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习中的个性化方法主要依赖启发式混合规则，缺乏理论依据，且在任务异构性下计算和通信成本高，需要一种更高效、有理论保证的个性化方案

Method: 提出Potara框架，为每个客户端构建个性化模型：合并联邦模型（捕捉通用知识）和本地模型（捕捉个性化知识）。利用线性模式连接性，推导出最小化方差迹上界的闭式最优混合权重

Result: 在视觉和语言基准测试中，Potara在减少通信的同时持续提升个性化性能，实现了强大的性能-通信权衡

Conclusion: Potara提供了一个有理论保证的联邦个性化框架，通过最优模型合并策略在任务异构性下实现了比单独使用联邦模型或本地模型更紧的性能上界，同时显著降低了通信成本
...
### [245] [Large Causal Models for Temporal Causal Discovery](https://arxiv.org/abs/2602.18662)
*Nikolaos Kougioulis,Nikolaos Gkorgkolis,MingXue Wang,Bora Caglayan,Dario Simionato,Andrea Tonon,Ioannis Tsamardinos*

Main category: cs.LG

TL;DR: 提出大型因果模型（LCMs）框架，用于时序因果发现，通过多数据集预训练实现跨数据集泛化，支持更高变量数量和更深架构，在分布外场景表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统因果发现方法采用数据集特定范式，每个新数据集都需要重新训练模型，限制了多数据集预训练的潜力。现有方法受限于小变量数量，在大输入时性能下降，且过度依赖合成数据，泛化能力有限。

Method: 提出原则性LCMs框架，结合多样化的合成数据生成器和真实时序数据集，实现大规模学习。模型设计专门用于时序因果发现，支持高变量数量和深度架构。

Result: 在合成、半合成和真实基准测试中，LCMs能有效扩展到更高变量数量和更深架构，同时保持强大性能。训练后的模型在经典和神经基线方法中达到竞争性或更优的准确率，特别是在分布外设置中，且支持快速单次推理。

Conclusion: LCMs为时序因果发现提供了一个有前景的基础模型范式，通过预训练架构实现跨数据集泛化，为大规模因果发现任务提供了新方向。
...
### [246] [Transformers for dynamical systems learn transfer operators in-context](https://arxiv.org/abs/2602.18679)
*Anthony Bao,Jeffrey Lai,William Gilpin*

Main category: cs.LG

TL;DR: 该研究探讨了基于注意力机制的Transformer模型在零样本迁移学习中的能力，发现其能够通过延迟嵌入和转移算子策略来预测未见过的物理系统，揭示了训练早期存在分布内与分布外性能的权衡。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解大规模科学机器学习基础模型如何能够在训练期间未见过的物理设置中进行零样本迁移学习（即上下文学习），这一现象挑战了传统对物理系统学习和适应的理解。

Method: 研究方法是在最小化设置中训练一个小的两层单头Transformer模型来预测一个动力系统，然后评估其在未经重新训练的情况下预测不同动力系统的能力。通过分析模型的注意力机制和预测策略来揭示其工作原理。

Result: 研究发现：1）训练早期存在分布内与分布外性能的权衡，表现为二次双重下降现象；2）基于注意力的模型采用转移算子预测策略，通过延迟嵌入将低维时间序列提升到高维动力流形，并识别和预测表征该流形上全局流动的长寿命不变集。

Conclusion: 研究结果阐明了大型预训练模型能够在测试时无需重新训练即可预测未见物理系统的机制，并展示了基于注意力的模型在利用全局吸引子信息进行短期预测方面的独特能力。
...
### [247] [In-Context Planning with Latent Temporal Abstractions](https://arxiv.org/abs/2602.18694)
*Baiting Luo,Yunuo Zhang,Nathaniel S. Keplinger,Samir Gupta,Abhishek Dubey,Ayan Mukhopadhyay*

Main category: cs.LG

TL;DR: I-TAP是一个离线强化学习框架，结合上下文适应和在线规划，在学习的离散时间抽象空间中进行蒙特卡洛树搜索，以应对连续控制中的长视野和部分可观测性问题。


<details>
  <summary>Details</summary>
Motivation: 连续控制中的规划面临两个实际问题：在原始时间尺度规划导致分支爆炸和长视野问题，而真实环境通常是部分可观测的且存在机制转换，这使得静态、完全可观测的动态假设失效。

Method: 从离线轨迹中学习观察条件残差量化VAE，将观察-宏动作段压缩为粗到细的离散残差标记堆栈，并使用时间Transformer自回归预测这些标记。在测试时，直接在标记空间进行蒙特卡洛树搜索，使用短历史进行隐式适应，并将选定的标记堆栈解码为可执行动作。

Result: 在确定性MuJoCo、具有每集潜在动态机制的随机MuJoCo以及高维Adroit操作任务（包括部分可观测变体）中，I-TAP始终匹配或优于强大的无模型和基于模型的离线基线方法。

Conclusion: I-TAP展示了在随机动态和部分可观测性下进行高效且鲁棒的上下文规划能力，统一了上下文适应和在线规划，解决了连续控制中的时间抽象和部分可观测性挑战。
...
### [248] [Insertion Based Sequence Generation with Learnable Order Dynamics](https://arxiv.org/abs/2602.18695)
*Dhruvesh Patel,Benjamin Rozonoyer,Gaurav Pandey,Tahira Naseem,Ramón Fernandez Astudillo,Andrew McCallum*

Main category: cs.LG

TL;DR: 本文提出了一种结合可训练顺序动态的离散流匹配方法，用于改进插入式生成模型，在图形遍历和小分子生成任务上优于均匀顺序动态


<details>
  <summary>Details</summary>
Motivation: 插入式生成模型比自回归模型具有更大的灵活性，但其动作空间更大，学习更困难。需要解决插入顺序动态的学习问题。

Method: 将可训练的顺序动态整合到离散流匹配的目标率中，使用可变长度掩码扩散模型作为生成模型，通过插入和填充掩码标记来生成序列。

Result: 在图形遍历任务中，探索了不同参数化的权衡；在小分子生成任务中，学习到的顺序动态相比均匀顺序动态提高了有效分子数量和生成质量。

Conclusion: 通过将可训练的顺序动态整合到离散流匹配中，可以在不进行数值模拟的情况下联合训练顺序动态和生成器，提高了插入式生成模型的性能。
...
### [249] [Phase-Consistent Magnetic Spectral Learning for Multi-View Clustering](https://arxiv.org/abs/2602.18728)
*Mingdong Lu,Zhikui Chen,Meng Liu,Shubin Ma,Liang Zhao*

Main category: cs.LG

TL;DR: 提出Phase-Consistent Magnetic Spectral Learning方法，通过建模跨视图方向一致性作为相位项，结合非负幅度主干构建复值磁亲和力，提取稳定的共享谱信号来指导无监督多视图聚类。


<details>
  <summary>Details</summary>
Motivation: 无监督多视图聚类面临的主要挑战是在视图差异和噪声下获得可靠的共享结构信号。现有方法通常依赖幅度亲和力或早期伪目标，当不同视图产生强度相当但方向趋势矛盾的关系时，这些方法不稳定，会扭曲全局谱几何并降低聚类性能。

Method: 1) 将跨视图方向一致性建模为相位项，结合非负幅度主干形成复值磁亲和力；2) 通过Hermitian磁拉普拉斯提取稳定的共享谱信号；3) 将该信号作为结构化自监督来指导无监督多视图表示学习和聚类；4) 使用基于锚点的高阶共识建模构建紧凑共享结构，并应用轻量级细化来抑制噪声或不一致关系。

Result: 在多个公共多视图基准测试上的广泛实验表明，该方法始终优于强基线方法。

Conclusion: 提出的相位一致磁谱学习方法通过显式建模跨视图方向一致性，解决了现有方法在视图关系矛盾时的不稳定性问题，为无监督多视图聚类提供了更可靠的共享结构信号提取框架。
...
### [250] [Prior Aware Memorization: An Efficient Metric for Distinguishing Memorization from Generalization in Large Language Models](https://arxiv.org/abs/2602.18733)
*Trishita Tiwari,Ari Trachtenberg,G. Edward Suh*

Main category: cs.LG

TL;DR: 本文提出Prior-Aware Memorization方法，用于区分LLMs中的真实记忆与统计常见序列，发现之前标记为记忆的序列中55-90%实际上是统计常见模式。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的训练数据泄露引发了隐私、安全和版权合规的严重担忧。现有方法往往将真实记忆与统计常见序列的生成混为一谈，而Counterfactual Memorization方法虽然原理正确但计算成本高昂，难以大规模应用。

Method: 提出Prior-Aware Memorization方法，这是一种理论上有依据、轻量级且无需重新训练的标准。核心思想是评估候选后缀是否与其特定训练前缀强相关，还是由于统计共性而在许多不相关提示下以高概率出现。

Result: 在LLaMA和OPT的训练语料上评估，发现之前标记为记忆的序列中55-90%实际上是统计常见的。在SATML训练数据提取挑战数据集上，约40%的序列尽管在训练数据中只出现一次，但仍表现出常见模式行为。

Conclusion: 低频本身不足以证明记忆的存在，在评估数据泄露时必须考虑模型先验。Prior-Aware Memorization方法为识别真实记忆提供了实用且高效的工具。
...
### [251] [When World Models Dream Wrong: Physical-Conditioned Adversarial Attacks against World Models](https://arxiv.org/abs/2602.18739)
*Zhixiang Guo,Siyuan Liang,Andras Balogh,Noah Lunberry,Rong-Cheng Tu,Mark Jelasity,Dacheng Tao*

Main category: cs.LG

TL;DR: PhysCond-WMA是针对生成式世界模型的首个白盒物理条件通道攻击方法，通过扰动HDMap嵌入和3D框特征等物理条件通道，在保持感知保真度的同时诱导语义、逻辑或决策级失真。


<details>
  <summary>Details</summary>
Motivation: 生成式世界模型越来越多地用于合成可控的、传感器条件的驾驶视频，但它们对物理先验的依赖暴露了新的攻击面。目前缺乏针对这类模型的安全漏洞研究。

Method: PhysCond-WMA采用两阶段优化：1）质量保持引导阶段，将反向扩散损失约束在校准阈值以下；2）动量引导去噪阶段，沿去噪轨迹累积目标对齐梯度，实现稳定、时间一致的语义偏移。

Result: 攻击在平均增加FID约9%、FVD约3.9%的情况下保持有效。在目标攻击设置下，攻击成功率（ASR）达到0.55。下游研究显示，使用攻击视频训练会使3D检测性能下降约4%，开环规划性能恶化约20%。

Conclusion: 这是首次揭示并量化生成式世界模型中的安全漏洞，推动了更全面的安全检查器的发展。
...
### [252] [CaliCausalRank: Calibrated Multi-Objective Ad Ranking with Robust Counterfactual Utility Optimization](https://arxiv.org/abs/2602.18786)
...
Method: 提出CaliCausalRank框架，将评分校准作为首要训练目标而非后处理，采用拉格朗日松弛法进行约束满足，并使用方差减少的反事实估计器进行可靠的离线评估。

Result: 在Criteo和Avazu数据集上的实验表明，CaliCausalRank相比最佳基线（PairRank）实现了1.1%的相对AUC提升、31.6%的校准误差减少和3.2%的效用增益，同时在不同流量段保持一致的性能。

Conclusion: CaliCausalRank通过统一的训练时校准和鲁棒的反事实估计，有效解决了广告排序中的尺度不一致和位置偏差问题，显著提升了系统性能和跨流量段的稳定性。
...
### [253] [From Few-Shot to Zero-Shot: Towards Generalist Graph Anomaly Detection](https://arxiv.org/abs/2602.18793)
...
<details>
  <summary>Details</summary>
Motivation: 现有图异常检测方法通常采用"一个模型对应一个数据集"的学习范式，存在计算成本高、泛化能力有限、在隐私敏感场景下数据访问受限等问题，需要一种能够跨数据集检测异常的通用模型。

Method: 提出ARC方法，包含三个核心模块：特征对齐模块统一跨数据集特征，残差GNN编码器捕捉数据集无关的异常表示，跨注意力上下文学习模块使用少量正常样本上下文进行异常评分。进一步提出ARC_zero用于零样本设置，通过伪上下文机制选择代表性伪正常节点。
...
### [254] [Vectorized Bayesian Inference for Latent Dirichlet-Tree Allocation](https://arxiv.org/abs/2602.18795)
...
<details>
  <summary>Details</summary>
Motivation: 传统LDA的狄利克雷先验无法表达主题间丰富的相关性和层次关系，限制了模型表达能力。

Method: 提出LDTA框架，用任意狄利克雷树分布替代LDA的狄利克雷先验；开发通用均值场变分推断和期望传播算法；实现完全向量化的GPU加速实现。

Result: LDTA显著扩展了LDA的建模能力，同时保持了可扩展性和计算效率。

Conclusion: LDTA是LDA的有力扩展，通过狄利克雷树先验实现了更丰富的主题结构表达，为文本建模提供了更强大的工具。
...
### [255] [SGNO: Spectral Generator Neural Operators for Stable Long Horizon PDE Rollouts](https://arxiv.org/abs/2602.18801)
*Jiayi Li,Zhaonan Wang,Flora D. Salim*

Main category: cs.LG

TL;DR: SGNO是一种新型神经算子，通过谱空间指数时间差分更新和门控非线性项，解决自回归推演中的不稳定问题，实现更长的稳定推演。
...
Result: 在APEBench的1D、2D和3D PDE家族上，SGNO相比基线神经算子实现了更低的长期误差和更长的稳定推演长度。

Conclusion: SGNO通过约束线性生成器和控制非线性更新，有效解决了神经算子自回归推演的不稳定性问题，实现了更可靠的长期预测。
...
### [256] [Exact Attention Sensitivity and the Geometry of Transformer Stability](https://arxiv.org/abs/2602.18849)
*Seyed Morteza Emadi*

Main category: cs.LG

TL;DR: 本文提出了一个Transformer稳定性理论框架，解释了预层归一化、DeepNorm的N^{-1/4}缩放和预热训练的必要性，揭示了注意力敏感度在训练中保持高位，Transformer稳定性主要来自架构而非学习到的注意力模式。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer是现代AI的核心，但其训练过程仍然神秘且脆弱。研究者希望从第一性原理出发，解释为什么预层归一化有效、为什么DeepNorm使用N^{-1/4}缩放、为什么需要预热训练，从而建立一个统一的稳定性理论框架。

Method: 提出了一个包含两个支柱的理论框架：(1) 推导了softmax Jacobian的精确算子范数，引入平衡质量因子θ(p)量化注意力敏感度；(2) 引入了与tokenwise计算对齐的块-∞/RMS几何，得到与序列长度无关的Lipschitz边界。使用该框架证明了预层归一化保持恒等梯度路径，而后续层归一化会指数级累积LayerNorm Jacobian。

Result: 在774M参数模型上验证了理论，发现注意力敏感度θ(p)≈1在整个训练过程中持续存在，与直觉相反（注意力在训练中会变得尖锐以降低敏感度）。DeepNorm的N^{-1/4}缩放源于注意力四个投影矩阵的四次结构。Transformer稳定性完全来自架构的梯度流，而非注意力动态。

Conclusion: Transformer的稳定性源于架构本身而非学习到的注意力模式，这改变了我们对训练的理解：架构本身必须处理敏感度问题，而不是依赖学习到的注意力模式。这一发现为理解Transformer训练稳定性提供了新的理论基础。
...
### [257] [Rank-Aware Spectral Bounds on Attention Logits for Stable Low-Precision Training](https://arxiv.org/abs/2602.18851)
*Seyed Morteza Emadi*

Main category: cs.LG

TL;DR: 论文提出了一种针对低精度训练中注意力分数溢出风险的秩感知集中不等式，并基于此推导出几何感知的缩放因子，用于FP8训练中提供理论保证的溢出防护。


<details>
  <summary>Details</summary>
Motivation: Transformer注意力分数是双线性形式，其最大幅度决定了低精度训练中的溢出风险。现有方法缺乏对注意力矩阵低秩结构的理论分析，导致缩放因子设计不够精确。

Method: 推导了秩感知集中不等式：当交互矩阵M=W^Q W^{K⊤}的秩r≪d时，最大注意力分数的尾概率以exp(-d²α²/(γr))衰减。基于此设计了几何感知缩放因子，通过隐式幂迭代计算谱范数，包含分组查询注意力以避免键扩展，并与融合注意力内核兼容。

Result: 对于Transformer注意力（r=d_h），该方法比秩无关边界提供了8-28倍更紧的集中性。在GPT-2 XL到Llama-2-70B的实验中，几何感知缩放在延迟缩放失败的情况下消除了溢出，同时保持了可比较的下游MMLU准确率。

Conclusion: 通过利用注意力矩阵的低秩结构，提出的秩感知集中不等式和几何感知缩放因子为低精度训练提供了理论保证的溢出防护，在实际大规模模型中验证了有效性。
...
### [258] [Issues with Measuring Task Complexity via Random Policies in Robotic Tasks](https://arxiv.org/abs/2602.18856)
*Reabetswe M. Nkhumise,Mohamed S. Talamali,Aditya Gilra*

Main category: cs.LG

TL;DR: 本文评估了非表格强化学习中任务复杂度度量方法的有效性，发现基于随机权重猜测的PIC和POIC指标与机器人控制和经验RL结果存在矛盾，表明需要开发更好的复杂度度量方法。


<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人等领域取得重大进展，但准确衡量任务复杂度对于创建有意义的基准和设计有效课程至关重要。目前非表格领域缺乏可靠的复杂度度量方法，现有基于随机权重猜测的指标需要在实际机器人任务中进行验证。

Method: 使用已知相对复杂度的渐进式机器人操作任务（包括单连杆和双连杆机械臂设置），在密集和稀疏奖励公式下，评估随机权重猜测（RWG）、策略信息容量（PIC）和策略最优信息容量（POIC）等复杂度度量方法。

Result: 实证结果显示：1）在相同奖励公式下，PIC表明双连杆设置比单连杆更容易，这与机器人控制经验相矛盾；2）对于相同设置，POIC估计稀疏奖励任务比密集奖励任务更容易，这也与RL经验结果不符。表明现有基于RWG的度量方法不可靠。

Conclusion: 基于随机权重猜测的PIC和POIC指标不能准确反映非表格强化学习中的任务复杂度，需要开发更好的度量方法。本文的任务框架为未来研究提供了起点。
...
### [259] [VariBASed: Variational Bayes-Adaptive Sequential Monte-Carlo Planning for Deep Reinforcement Learning](https://arxiv.org/abs/2602.18857)
...
<details>
  <summary>Details</summary>
Motivation: 贝叶斯最优智能体虽然能实现探索与利用的最佳权衡，但信念状态获取和规划通常难以处理，现有深度学习方法训练成本高昂，需要更高效的解决方案

Method: 提出变分框架，融合变分信念学习、序列蒙特卡洛规划和元强化学习，在贝叶斯自适应马尔可夫决策过程中进行学习和规划

Result: 在单GPU设置下，VariBASeD方法展现出对更大规划预算的良好扩展性，相比先前方法提高了样本效率和运行时间效率

Conclusion: VariBASeD为贝叶斯自适应强化学习提供了更高效的训练和规划框架，在探索与利用权衡问题上取得了进展
...
TL;DR: 论文研究了结构化预测中的提升方法，提出了(α,β)-可提升性概念，证明几何中位数聚合能在多种散度下实现可提升性，并提出了GeoMedBoost框架统一多种经典提升算法。


<details>
  <summary>Details</summary>
Motivation: 尽管提升方法在结构化预测中广泛应用，但超越标量损失的聚合理论理解仍不完整。论文旨在研究向量值和条件密度预测在一般散度下的提升理论，识别稳定性条件使弱保证能通过聚合增强为强保证。

Method: 提出(α,β)-可提升性作为稳定性形式化概念，证明几何中位数聚合能在广泛散度类中实现可提升性。针对向量值预测和条件密度估计，分析常见散度（ℓ₁、ℓ₂、TV、Hel）下的可提升性。提出基于指数重加权和几何中位数聚合的GeoMedBoost通用提升框架。

Result: 几何中位数聚合在多种散度下实现可提升性，揭示了维度依赖和维度无关机制间的显著区别。KL散度虽不能直接通过几何中位数聚合提升，但可通过Hellinger距离间接处理。在弱学习器条件和(α,β)-可提升性下，GeoMedBoost能实现经验散度超限误差的指数衰减。

Conclusion: 论文建立了结构化预测中提升的统一几何视角，提出的(α,β)-可提升性概念和GeoMedBoost框架统一了MedBoost、AdaBoost、SAMME等经典算法，为超越标量损失的结构化预测提升提供了理论框架。
...
### [261] [PCA-VAE: Differentiable Subspace Quantization without Codebook Collapse](https://arxiv.org/abs/2602.18904)
...
<details>
  <summary>Details</summary>
Motivation: 向量量化自编码器虽然能产生高保真隐变量，但存在固有缺陷：量化器不可微、需要直通技巧、容易坍缩。需要从根本上解决这些问题

Method: 用在线PCA瓶颈替代向量量化，通过Oja规则训练正交、方差排序的隐变量基，无需码本、承诺损失或查找噪声

Result: PCA-VAE在CelebAHQ上的重建质量超过VQ-GAN和SimVQ，同时使用10-100倍更少的隐变量比特；能自然产生可解释维度（姿态、光照、性别线索）
...
### [262] [TRUE: A Trustworthy Unified Explanation Framework for Large Language Model Reasoning](https://arxiv.org/abs/2602.18905)
*Yujiao Yang*

Main category: cs.LG

TL;DR: TRUE框架通过可执行推理验证、可行域DAG建模和因果故障模式分析，为LLM推理提供多层级可验证解释，提升可解释性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂推理任务中表现出色，但其决策过程难以解释。现有解释方法缺乏可信的结构洞察，仅限于单实例分析，无法揭示推理稳定性和系统性故障机制。

Method: 提出可信统一解释框架(TRUE)，包含三个层级：实例级通过可执行过程规范和盲执行验证评估操作有效性；局部结构级通过结构一致扰动构建可行域DAG，表征推理稳定性和局部输入空间中的可执行区域；类别级引入因果故障模式分析方法，识别重复结构故障模式并使用Shapley值量化其因果影响。

Result: 在多个推理基准测试中的广泛实验表明，TRUE框架提供多层级可验证解释，包括个体实例的可执行推理结构、相邻输入的可行域表示，以及类别级具有量化重要性的可解释故障模式。

Conclusion: 该框架为提升LLM推理系统的可解释性和可靠性建立了统一且原则性的范式。
...
### [263] [DeepInterestGR: Mining Deep Multi-Interest Using Multi-Modal LLMs for Generative Recommendation](https://arxiv.org/abs/2602.18907)
*Yangchen Zeng*

Main category: cs.LG

TL;DR: DeepInterestGR通过多LLM挖掘深层兴趣、奖励标签监督和兴趣增强离散化，解决现有生成推荐框架仅依赖浅层行为信号的问题，显著提升推荐性能。


<details>
  <summary>Details</summary>
Motivation: 现有生成推荐框架主要依赖浅层行为信号，仅通过标题、描述等表面文本特征编码物品，导致"浅层兴趣问题"——无法捕捉用户交互背后的潜在语义丰富兴趣，限制了推荐个性化和可解释性。

Method: 1) 多LLM兴趣挖掘：利用多个前沿LLM及其多模态变体通过思维链提示提取深层文本和视觉兴趣表示；2) 奖励标签深层兴趣：使用轻量级二元分类器为挖掘的兴趣分配奖励标签，为强化学习提供有效监督信号；3) 兴趣增强物品离散化：将整理的深层兴趣编码为语义嵌入并通过RQ-VAE量化为语义ID令牌。采用两阶段训练：监督微调使生成模型与深层兴趣信号和协同过滤模式对齐，然后通过兴趣感知奖励优化的GRPO进行强化学习。

Result: 在三个Amazon Review基准测试中，DeepInterestGR在HR@K和NDCG@K指标上持续优于最先进的基线方法。

Conclusion: DeepInterestGR通过挖掘深层语义兴趣并融入生成推荐框架，有效解决了浅层兴趣问题，显著提升了推荐系统的个性化和可解释性，为生成推荐提供了新的研究方向。
...
### [264] [SLDP: Semi-Local Differential Privacy for Density-Adaptive Analytics](https://arxiv.org/abs/2602.18910)
*Alexey Kroshnin,Alexandra Suvorikova*

Main category: cs.LG

TL;DR: 提出半本地差分隐私(SLDP)框架，通过基于本地密度分配隐私区域，将隐私成本与迭代细化分离，实现高分辨率网格而无需额外隐私预算
...
Method: 提出半本地差分隐私(SLDP)框架，为每个用户基于本地密度分配隐私区域，通过诚实但好奇的服务器在公共信道上协调交互式(ε,δ)-SLDP协议来私有地估计这些区域

Result: 该框架成功将隐私成本与细化迭代次数解耦，允许高分辨率网格而无需额外隐私预算成本，在合成和真实数据集上的估计任务中展示了有效性

Conclusion: SLDP框架解决了LDP下密度自适应离散化的挑战，通过隐私区域概念和交互式协议实现了高效用隐私保护分析，为高分辨率数据分析提供了新方法
...
### [265] [From Human-Level AI Tales to AI Leveling Human Scales](https://arxiv.org/abs/2602.18911)
...
TL;DR: 该论文提出了一个框架，将AI模型性能校准到"世界人口"基准，使用对数尺度建立人类锚定的标准化评估体系。


<details>
  <summary>Details</summary>
Motivation: 现有AI模型与"人类水平"的比较存在误导性，因为基准分数不可比且人类基线通常来自狭窄人群。需要建立基于全球人口的标准化评估框架。

Method: 构建多级能力量表，每个级别代表世界人口在对数尺度上的成功概率。使用公开的人类测试数据（PISA、TIMSS、ICAR等）校准各能力量表。通过LLM外推不同人口样本估计对数基数B，使用群体切片和后分层方法评估映射质量。

Result: 开发了新的技术方法，能够重新校准和标准化相对于全球人口的评估量表，实现了AI性能与人类基准的可比性。

Conclusion: 提出的框架解决了AI模型与人类水平比较中的基准不可比问题，通过世界人口校准建立了统一的评估标准，为AI能力评估提供了更可靠的方法。
...
### [266] [LoMime: Query-Efficient Membership Inference using Model Extraction in Label-Only Settings](https://arxiv.org/abs/2602.18934)
*Abdullah Caglar Oksuz,Anisa Halimi,Erman Ayday*

Main category: cs.LG

TL;DR: 提出了一种基于可迁移性和模型提取的成本效益标签成员推理攻击框架，通过主动采样、扰动选择和合成数据提取目标模型的替代模型，将查询开销转移到一次性提取阶段，显著降低查询成本。
...
Method: 提出基于可迁移性和模型提取的标签成员推理攻击框架：通过主动采样、扰动选择和合成数据查询目标模型M，提取功能相似的替代模型S，然后在S上执行成员推理。该方法将查询开销转移到一次性提取阶段，消除了对M的重复查询。

Result: 在Purchase、Location和Texas Hospital等基准测试中，仅需测试约1%训练样本的查询预算即可提取替代模型S，并实现与目标模型M在成员推理准确率上±1%的匹配。同时评估了标准防御措施对该攻击的有效性。

Conclusion: 该方法在严格黑盒约束下运行，匹配了最先进标签成员推理攻击的性能，同时显著降低了查询成本，为隐私保护机器学习模型的安全评估提供了更实用的攻击框架。
...
### [267] [Exponential Convergence of (Stochastic) Gradient Descent for Separable Logistic Regression](https://arxiv.org/abs/2602.18946)
*Sacchit Kale,Piyushi Manupriya,Pierre Marion,Francis bach,Anant Raj*

Main category: cs.LG

TL;DR: 论文证明梯度下降和随机梯度下降可以通过简单的非自适应递增步长计划在稳定优化区域内实现可分离逻辑回归的指数收敛，无需依赖不稳定区域。


<details>
  <summary>Details</summary>
Motivation: 当前研究认为梯度下降在大步长下往往需要在稳定性边缘（不稳定区域）才能实现加速收敛，但本文旨在证明加速并不必然需要不稳定区域，可以在稳定优化区域内实现指数收敛。

Method: 提出两种方法：1) 梯度下降使用简单的非自适应递增步长计划；2) 随机梯度下降使用轻量级自适应步长规则，避免线搜索和专门程序。两种方法都在稳定优化区域内操作。

Result: 证明了在边界条件下，梯度下降和随机梯度下降都能实现可分离逻辑回归的指数收敛，梯度下降方法具有随时性且不需要事先知道优化范围或目标精度。

Conclusion: 精心设计的步长增长结构本身就足以实现梯度下降和随机梯度下降的指数加速，无需依赖不稳定优化区域，这为机器学习优化提供了新的理论见解。
...
### [268] [Toward Manifest Relationality in Transformers via Symmetry Reduction](https://arxiv.org/abs/2602.18948)
*J. François,L. Ravera*
...
<details>
  <summary>Details</summary>
Motivation: Transformer模型存在大量内部冗余，这些冗余源于坐标依赖表示和连续对称性（模型空间和头空间）。现有方法通过显式打破对称性来处理，但本文提出互补的对称性约简框架。

Method: 将表示、注意力机制和优化动态重新表述为不变关系量，通过构造消除冗余自由度，构建直接操作关系结构的架构。

Result: 提出了一个原则性的几何框架，用于减少参数冗余和分析优化过程，使架构能够直接处理关系结构。

Conclusion: 对称性约简框架为减少Transformer参数冗余和优化分析提供了原则性的几何方法，与显式打破对称性的方法形成互补。
...
### [269] [Incremental Transformer Neural Processes](https://arxiv.org/abs/2602.18955)
*Philip Mortimer,Cristiana Diaconu,Tommy Rochussen,Bruno Mlodozeniec,Richard E. Turner*

Main category: cs.LG

TL;DR: 提出Incremental TNP模型，通过因果掩码、KV缓存和自回归训练策略，在保持标准TNP预测性能的同时，将更新计算复杂度从二次降低到线性，适用于流式数据场景。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer Neural Processes在处理连续数据流（如实时传感器读数、数据库更新）时，每次新观测都需要从头重新计算内部表示，缺乏廉价增量更新的能力，这在顺序应用中效率低下。

Method: 引入Incremental TNP，借鉴大语言模型技术：1) 使用因果掩码实现顺序处理；2) 采用KV缓存避免重复计算；3) 设计数据高效的自回归训练策略。

Result: 在合成和真实世界任务（表格回归、温度预测）上评估，incTNP性能与标准TNP相当甚至更好，同时实现数量级的顺序推理加速，更新计算复杂度从O(n²)降至O(n)。

Conclusion: incTNP通过因果掩码获得了计算优势，同时保持了与标准非因果TNP相当的隐式贝叶斯一致性，为流式推理提供了高效且一致的解决方案。
...
TL;DR: 本文提出了一种主动数据重建攻击(ADRA)方法，通过强化学习主动诱导模型重建给定文本来进行成员推断攻击，相比传统被动方法显著提升了检测效果。


<details>
  <summary>Details</summary>
Motivation: 传统成员推断攻击(MIA)被动地使用固定模型权重进行检测，而本文假设训练数据比非成员数据更容易被重建，这种重建性差异可用于改进成员推断。受强化学习能增强权重中已有行为的启发，作者希望通过主动训练来诱导数据重建。

Method: 提出ADRA系列方法，利用策略强化学习主动诱导模型重建给定文本。通过微调从目标模型初始化的策略，设计重建度量和对比奖励，开发了ADRA及其自适应变体ADRA+算法。

Result: 实验表明，ADRA方法在检测预训练、后训练和蒸馏数据方面均优于现有MIA方法，平均比之前最佳方法提升10.7%。ADRA+在BookMIA上比Min-K%++提升18.8%，在AIME上提升7.6%。

Conclusion: 通过主动诱导模型重建数据的方法能显著提升成员推断攻击的效果，证明了训练数据比非成员数据更具重建性的假设，为LLM训练数据检测提供了新思路。
...
TL;DR: 提出混合生成AI与数值ILT的框架，通过条件采样生成多个掩模候选，再经快速ILT精炼，显著提升掩模质量和计算效率


<details>
  <summary>Details</summary>
Motivation: 传统逆光刻技术（ILT）存在高度非凸目标函数，容易陷入局部最优解。现有生成AI方法多训练确定性图像转换器来模仿次优数据集，对逃离非凸陷阱的指导有限

Method: 将掩模合成重新定义为条件采样问题：生成器学习基于设计条件的掩模分布，提出多个候选。先使用WGAN加重建损失预训练，再用GRPO和ILT引导的模仿损失微调。推理时采样小批量掩模，运行快速批量ILT精炼，评估光刻指标后选择最佳候选

Result: 在LithoBench数据集上，该框架在3nm容差下减少EPE违规，吞吐量比强数值ILT基线提高约一倍，同时改善最终掩模质量。在ICCAD13竞赛案例上实现超过20%的EPE改进，相比SOTA数值ILT求解器有3倍加速

Conclusion: 通过学习提出ILT友好的初始化方案，该方法缓解了非凸性问题，超越了传统求解器或生成AI单独能达到的效果
...
TL;DR: 论文提出"神经共振"概念，解释AI生成数据反馈训练导致模型崩溃的机制，发现反馈过程会在潜在空间收敛到低维不变结构，并建立了八种崩溃行为的分类体系。


<details>
  <summary>Details</summary>
Motivation: AI训练数据集中不可避免地包含AI生成的样本，导致"反馈"现象——一个模型的输出影响另一个模型的训练。虽然已知这种迭代反馈会导致模型崩溃，但其退化机制仍不清楚。研究旨在理解这种退化背后的机制。

Method: 将迭代反馈建模为马尔可夫链，分析反馈过程的遍历性和潜在表示的方向性收缩条件。通过在MNIST和ImageNet上的扩散模型、CycleGAN以及音频反馈实验，研究局部和全局流形几何的演化。

Result: 发现广泛的反馈过程会在潜在空间收敛到低维不变结构（神经共振现象）。建立了八种崩溃行为的分类体系，揭示了反馈过程的收敛特性。
...
TL;DR: 本文提出了一种处理偏好微调中不可传递偏好的新方法，通过最大熵布莱克威尔获胜者概念和PROSPER算法，有效解决了多目标不可传递偏好问题，在LLM微调中表现优异。


<details>
  <summary>Details</summary>
Motivation: 偏好微调中的核心挑战是处理不可传递（循环）偏好，这种偏好可能源于单一目标的不一致排序或多个目标的标量化。不可传递偏好导致没有明确定义的最优策略，破坏了标准PFT流程的基本假设。

Method: 提出最大熵布莱克威尔获胜者作为新的博弈论解决方案概念，适用于多目标不可传递偏好。开发了PROSPER算法，这是一种可证明高效的PFT算法，能够直接处理多个目标而无需标量化。

Result: 将PROSPER应用于基于多目标LLM-as-a-Judge反馈的大语言模型微调，发现在指令遵循和通用聊天基准测试中，PROSPER优于所有基线方法，并发布了7B和3B参数规模的训练模型检查点。

Conclusion: PROSPER算法有效解决了偏好微调中的不可传递偏好问题，特别是在多目标LLM评估场景中表现出色，为处理复杂偏好结构提供了实用的解决方案。
...
TL;DR: 本文提出IDLM方法，通过将逆蒸馏技术扩展到离散扩散语言模型，显著减少推理步骤（4-64倍），同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在文本生成方面表现优异，但多步采样导致推理速度慢，限制了实际应用。需要加速方法来解决这一瓶颈。

Method: 将连续扩散模型的逆蒸馏技术扩展到离散设置，通过理论证明逆公式存在唯一解确保优化有效性，并引入梯度稳定的松弛技术来支持有效训练。

Result: 在多个扩散语言模型上的实验表明，IDLM方法能够将推理步骤减少4-64倍，同时保持教师模型的熵和生成困惑度。

Conclusion: IDLM方法成功解决了扩散语言模型推理速度慢的问题，通过理论保证和实用技术实现了高效加速，为实际应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列基础模型主要学习预定义时域或频域中的常见规律模式，适用于监督下游任务（如预测），但对于无监督任务（如异常检测）效果不佳，因为异常模式在相同时/频域中可能与正常模式相似。

Method: 提出TimeRadar模型，包含两个核心组件：1) 分数调制时频重建(FTFRecon)：使用可学习分数阶将时间序列旋转到连续时频域中最能区分正常与异常的角度进行数据重建；2) 上下文偏差学习(CDL)：在可旋转域中建模输入相对于其上下文时间序列数据的局部偏差，以捕捉全局重建未捕获的局部异常。

Result: TimeRadar能够在分数时频域中自适应地区分正常和异常信号，为不同数据集（包括未见数据集）提供最优时频域的数据重建，有效分离无界异常模式与规律模式。

Conclusion: TimeRadar通过创新的分数时频域表示和上下文偏差学习，解决了现有时间序列基础模型在无监督异常检测任务中的局限性，能够跨数据集有效检测时间序列异常。
...
TL;DR: ST-Prune：一种基于动态样本剪枝的时空预测训练效率优化方法，通过智能识别最具信息量的样本来加速收敛，在保持或提升模型性能的同时显著提高训练速度。


<details>
  <summary>Details</summary>
Motivation: 时空预测在交通、气候科学和城市规划等领域至关重要，但深度学习模型在这些领域的大规模、通常冗余的数据集上训练存在显著计算瓶颈。现有方法主要关注模型架构或优化器优化，而忽视了训练数据本身的固有低效性。传统方法在每个epoch迭代整个静态数据集，在易学习或重复样本上浪费了大量资源。

Method: 提出ST-Prune方法，采用基于复杂度的动态样本剪枝技术。该方法根据模型的实时学习状态智能识别最具信息量的样本，从而加速收敛并提高训练效率。

Result: 在真实世界时空数据集上的大量实验表明，ST-Prune在保持甚至提高模型性能的同时，显著加速了训练速度。该方法还具有可扩展性和通用性。

Conclusion: ST-Prune通过动态样本剪枝有效解决了时空预测中的训练效率问题，为深度学习模型在大规模时空数据集上的高效训练提供了新思路。
...
TL;DR: 论文提出了一种鲁棒的贝叶斯随机特征回归方法，通过Huber式污染集显式处理先验和似然误设，使用悲观广义贝叶斯更新进行推断，得到可计算的后验预测密度边界，并引入不精确最高密度区域进行鲁棒预测不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯随机特征回归假设高斯先验和似然，但实际应用中存在先验和似然误设问题。需要开发能够显式处理这些误设的鲁棒贝叶斯框架，同时保持计算可处理性。

Method: 1. 从经典岭正则化随机特征训练与贝叶斯推断的等价性出发；2. 用ε-和η-污染信度集替换单一先验和似然；3. 采用悲观广义贝叶斯更新进行推断；4. 推导后验预测密度的显式可计算边界；5. 引入不精确最高密度区域(IHDR)进行鲁棒不确定性量化。

Result: 1. 得到后验预测密度的显式可计算边界；2. 当污染适中时，先验和似然模糊性直接表现为后验预测分布的污染；3. IHDR可通过调整的高斯可信区间高效外近似；4. 获得预测方差边界（在温和截断近似下）；5. 证明这些边界保留了随机特征模型已知的主导阶比例增长渐近性。

Conclusion: 建立了贝叶斯随机特征的鲁棒性理论：预测不确定性保持计算可处理性，继承了经典双下降相结构，并通过有界先验和似然误设下的显式最坏情况保证得到改进。
...
<details>
  <summary>Details</summary>
Motivation: 标注偏差在数据收集中普遍存在，由于资源限制或无意识偏见导致不同子组间标签错误率不均或子组代表性失真。现有公平性约束通常假设训练标签反映真实分布，在标注偏差存在时失效，因此需要有效检测标注偏差的方法。

Method: 研究采用影响函数来检测标注偏差。影响函数通过利用损失函数的梯度和海森矩阵来估计每个训练样本对模型预测的影响。研究者开发了样本评估流程，首先在MNIST数据集上测试，然后扩展到更复杂的CheXpert医学影像数据集。为了检验标签噪声，研究者通过翻转数据集中一个类别20%的标签来引入受控错误，并使用对角海森矩阵近似进行计算。

Result: 在MNIST数据集上，使用对角海森矩阵近似取得了有希望的结果，成功检测出近90%的错误标注样本。在CheXpert数据集上，错误标注样本始终表现出更高的影响分数。这些结果表明影响函数在识别标签错误方面具有潜力。

Conclusion: 影响函数能够有效检测标注偏差和识别错误标注样本，为解决数据收集中的标注偏差问题提供了一种有前景的技术途径。
...
Result: 在bnlearn基准测试上的大量实验表明，TICL在因果发现和干预目标检测的多个方面具有优越性能。
...
TL;DR: 通过设计简单的归一化优化器架构和增强元训练，仅用4.5 GPU小时就能训练出性能优异的通用学习优化器，相比VeLO的4000 TPU月计算量大幅降低，且能扩展到十亿参数规模的预训练任务。


<details>
  <summary>Details</summary>
Motivation: 学习优化器作为手工设计优化规则（如Adam）的有力替代品，在实际应用中受到限制，因为它们往往无法在训练分布之外进行元泛化，且元训练成本高昂。例如VeLO需要4000 TPU月的计算量，但仍无法泛化到6亿参数以上的任务。

Method: 设计了简单的归一化优化器架构，并增强了元训练过程。该方法与现代优化框架兼容，包括正交化、输入输出权重与隐藏权重的不同更新规则、以及解耦权重衰减等技术。

Result: 仅用4.5 GPU小时就能训练出性能优异的通用学习优化器，相比VeLO的计算量大幅降低。该优化器能稳定扩展到十亿参数规模的预训练任务（GPT-3 XL 1.3B），比元训练分布大六个数量级，并在多样化的分布外任务上表现强劲。

Conclusion: 这项工作为实际可应用的学习优化算法铺平了道路，解锁了更丰富的元训练和数据整理方法的探索，以进一步提升性能。
...
TL;DR: 论文研究了Transformer如何学习整合多个历史位置信息，发现其通过稀疏注意力模式分阶段学习，从竞争性动态转向合作性动态，揭示了Transformer通过更简单的错误假设类逐步提升复杂度的学习过程。


<details>
  <summary>Details</summary>
Motivation: 研究Transformer如何学习整合来自多个历史位置的信息，特别是当这些信息具有不同的统计显著性时。旨在理解Transformer的学习动态和阶段性学习行为的理论基础。

Method: 引入高阶马尔可夫链任务来研究Transformer的学习过程。使用简化的微分方程建模学习动态，分析稀疏注意力模式的获取过程，并证明分阶段收敛结果。

Result: 发现Transformer通过稀疏注意力模式分阶段学习：初始阶段是竞争性的（注意力头收敛到统计主导模式），后期转变为合作性的（注意力头专门化于不同模式）。Transformer通过更简单的错误假设类逐步提升复杂度，早期停止起到隐式正则化作用。

Conclusion: 研究为Transformer中阶段性学习和复杂行为的出现提供了理论基础，对自然语言处理和算法推理中的泛化问题提供了重要见解。揭示了Transformer通过逐步复杂度提升的学习机制。
...
<details>
  <summary>Details</summary>
Motivation: 传统参数高效微调方法如LoRA学习静态的低秩适配器，而VPS旨在通过激活条件化的动态扰动，在不更新持久参数的情况下实现测试时适应，增强大语言模型的推理能力。

Method: VPS基于批量激活统计和可选梯度信号，动态构建低秩扰动因子ΔW = γ * W^T V U^T W，其中选择矩阵U和V通过稀疏激活引导选择或Sylvester耦合回归构建，并采用自适应策略系统根据激活能量和token级熵调节扰动幅度。

Result: 论文提供了扰动谱特性的理论分析，描述了包含多目标验证和迭代细化的自适应策略系统，并讨论了激活条件化计算如何增强大语言模型的推理能力。

Conclusion: VPS为冻结Transformer模型提供了一种无需参数更新的推理时适应框架，通过动态激活条件化扰动增强模型性能，为提升大语言模型推理能力提供了新途径。
...
### [283] [HybridFL: A Federated Learning Approach for Financial Crime Detection](https://arxiv.org/abs/2602.19207)
*Afsana Khan,Marijn ten Thij,Guangzhi Tang,Anna Wilbik*

Main category: cs.LG

TL;DR: 提出混合联邦学习(HybridFL)框架，解决数据同时存在水平分割（不同用户）和垂直分割（不同特征集）的复杂场景，在金融犯罪检测中验证效果


<details>
  <summary>Details</summary>
Motivation: 现实世界中许多场景存在复杂的混合数据分布，标准联邦学习通常只处理水平或垂直数据分割，无法应对同时存在两种分割的情况，特别是在金融犯罪检测等敏感领域

Method: 提出混合联邦学习(HybridFL)架构，结合水平聚合和垂直特征融合，在保持数据本地化的前提下实现联合学习。在金融犯罪检测场景中，交易方持有交易级属性，多个银行维护私有账户级特征

Result: 在AMLSim和SWIFT数据集上的实验表明，HybridFL显著优于仅使用交易数据的本地模型，性能接近集中式基准模型

Conclusion: HybridFL能够有效处理同时存在水平和垂直数据分割的复杂场景，在严格保护数据隐私的前提下实现高性能的联合学习，特别适用于金融犯罪检测等敏感应用
...
### [284] [Evaluating SAP RPT-1 for Enterprise Business Process Prediction: In-Context Learning vs. Traditional Machine Learning on Structured SAP Data](https://arxiv.org/abs/2602.19237)
*Amit Lal*

Main category: cs.LG

TL;DR: 首次独立评估SAP的RPT-1表格基础模型，在三个SAP业务场景中达到调优GBDT模型91-96%的准确率，无需训练数据，在小样本场景下甚至能超越XGBoost。


<details>
  <summary>Details</summary>
Motivation: 表格基础模型旨在让企业数据无需任务特定训练即可进行机器学习。本文从实践者角度首次独立评估SAP的RPT-1模型，了解其在真实业务场景中的表现。

Method: 在三个SAP业务场景（需求预测、预测性数据完整性、财务风险分类）上，使用五折交叉验证（数据集2500-3200行），将RPT-1与调优的梯度提升决策树（XGBoost、LightGBM、CatBoost）进行对比评估。

Result: RPT-1在无需任何训练示例的情况下达到调优GBDT准确率的91-96%。分类任务差距较小（AUC-ROC差距3.6-4.1个百分点），回归任务差距较大（R平方差距8.9-11.1个百分点）。有趣的是，在约75-100个上下文行的小样本场景下，RPT-1甚至能超越XGBoost。

Conclusion: 提出实用的混合工作流程：使用RPT-1进行快速筛选，然后在预测准确性值得投入的地方有选择地训练GBDT。所有实验通过公开的Hugging Face Spaces可复现。
...
### [285] [Alternating Bi-Objective Optimization for Explainable Neuro-Fuzzy Systems](https://arxiv.org/abs/2602.19253)
*Qusai Khaled,Uzay Kaymak,Laura Genga*

Main category: cs.LG

TL;DR: 提出X-ANFIS：一种交替双目标梯度优化方案，用于可解释的自适应神经模糊推理系统，解决准确性与可解释性之间的权衡问题


<details>
  <summary>Details</summary>
Motivation: 现有方法在准确性与可解释性权衡方面存在局限：进化多目标优化计算成本高，基于梯度的标量化无法恢复非凸帕累托区域

Method: 使用柯西隶属函数实现语义控制初始化下的稳定训练；引入可微分的可解释性目标，通过交替梯度传递与性能目标解耦

Result: 在9个UCI回归数据集上约5000次实验验证，X-ANFIS在保持竞争力的预测准确性的同时，始终实现目标可区分性，恢复超出MOO帕累托前沿凸包的解决方案

Conclusion: X-ANFIS提供了一种有效的梯度优化方案，能够平衡模糊系统的准确性与可解释性，克服了现有方法的局限性
...
### [286] [DGPO: RL-Steered Graph Diffusion for Neural Architecture Generation](https://arxiv.org/abs/2602.19261)
*Aleksei Liuliakov,Luca Hermes,Barbara Hammer*

Main category: cs.LG

TL;DR: DGPO扩展了强化学习微调方法，使其能够处理有向无环图，在神经架构搜索任务中达到基准最优性能，并展示了模型学习到的可迁移结构先验。


<details>
  <summary>Details</summary>
Motivation: 现有的图扩散方法主要针对无向图设计，而神经架构是有向无环图，其中边的方向编码了重要的功能语义（如数据流信息），现有方法无法有效处理这种方向性。

Method: 提出了Directed Graph Policy Optimization (DGPO)，通过拓扑节点排序和位置编码，将强化学习对离散图扩散模型的微调扩展到有向无环图。

Result: 在NAS-Bench-101和NAS-Bench-201上验证，DGPO在所有三个NAS-Bench-201任务上匹配基准最优（91.61%、73.49%、46.77%）。仅使用7%搜索空间预训练后，经过微调能生成接近最优的架构，与全数据模型相差仅0.32个百分点，并超出其训练上限7.3个百分点。

Conclusion: 强化学习引导的离散扩散方法一旦扩展到处理方向性，就能为有向组合结构提供一个可控的生成框架，模型能够学习可迁移的结构先验并实现真正的奖励驱动控制。
...
TL;DR: FedPAC：通过预条件器对齐和校正解决联邦二阶优化中的几何失配问题，提高非IID数据下的训练稳定性


<details>
  <summary>Details</summary>
Motivation: 传统联邦二阶优化器在非IID数据上不稳定甚至发散，主要原因是预条件器漂移：客户端二阶训练产生异构的曲率定义几何，服务器端模型平均在不兼容的度量下计算更新，破坏了全局下降方向

Method: 提出FedPAC框架，通过预条件器对齐和校正来解耦参数聚合与几何同步：1) 对齐：聚合本地预条件器为全局参考，通过全局预条件器预热客户端；2) 校正：使用全局预条件方向引导本地预条件更新以抑制长期漂移

Result: 提供了漂移耦合非凸收敛保证，在部分参与下实现线性加速。实验表明FedPAC在视觉和语言任务上持续提高稳定性和准确性，在CIFAR-100上使用ViT实现最高5.8%的绝对准确率提升

Conclusion: FedPAC通过解决预条件器漂移问题，为联邦二阶优化提供了可靠框架，显著提高了非IID数据下的训练性能和稳定性
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM对齐研究主要围绕GRPO框架展开，有两个主要方向：一是转向与任务奖励更匹配的序列级重要性采样权重，二是寻找替代PPO式裁剪的方法以避免训练信号丢失和熵崩溃。现有方法如SAPO和GMPO各有优势但仍有改进空间。

Method: 提出Soft Sequence Policy Optimization方法，这是一种离策略强化学习目标，通过在序列级重要性权重中引入token级概率比的软门控函数，实现有效的策略探索和训练稳定性平衡。

Result: 该方法在理论上结合了序列级重要性采样和token级适应性调整的优势，能够同时保持序列连贯性和token适应性，避免传统PPO方法中的训练信号丢失问题。

Conclusion: Soft Sequence Policy Optimization为LLM对齐提供了一种新的优化框架，通过软门控机制在序列级和token级之间建立更好的平衡，有望提升策略探索效果和训练稳定性。
...
### [289] [Training-Free Cross-Architecture Merging for Graph Neural Networks](https://arxiv.org/abs/2602.19332)
*Rishabh Bhattacharya,Vikaskumar Kalsariya,Naresh Manwani*

Main category: cs.LG

TL;DR: H-GRAMA是一个无需训练、支持异构GNN模型合并的框架，通过将参数空间合并提升到算子空间，实现了跨架构GNN（如GCN到GAT）的合并，在保持专家模型准确性的同时获得1.2-1.9倍的推理加速。
...
Method: 提出H-GRAMA框架，引入通用消息传递混合（UMPM）作为共享算子族，将异构GNN层表达为统一的函数语言。通过将合并从参数空间提升到算子空间，实现跨架构GNN的无需训练合并。

Result: 在兼容深度设置下，H-GRAMA能够保持高专家准确性，实现跨架构GNN合并（如GCN到GAT），推理速度比集成方法快1.2-1.9倍。

Conclusion: H-GRAMA为异构GNN模型合并提供了有效的训练免费框架，通过算子空间合并克服了参数空间合并的限制，在保持性能的同时显著提升推理效率。
...
### [290] [Smooth Gate Functions for Soft Advantage Policy Optimization](https://arxiv.org/abs/2602.19345)
*Egor Denisov,Svetlana Glazyrina,Maksim Kryzhanovskiy,Roman Ischenko*

Main category: cs.LG

TL;DR: 本文研究了不同门函数对大型语言模型训练稳定性和性能的影响，通过实验验证了软自适应策略优化的有效性。


<details>
  <summary>Details</summary>
Motivation: GRPO在训练大型语言模型方面取得了显著进展，但其使用的硬裁剪方法容易导致训练不稳定。SAPO通过使用平滑的sigmoid门函数解决了这个问题，但需要进一步研究不同门函数对训练稳定性和最终模型性能的影响。

Method: 首先形式化了可接受门函数应满足的关键属性，识别了几类这样的函数族进行实证评估。使用Qwen2.5-7B-Instruct模型在数学推理任务上进行实验，分析不同门函数对训练稳定性和模型性能的影响。

Result: 通过实验发现，不同的门函数确实对训练稳定性和最终模型性能有显著影响。研究结果为设计更平滑、更鲁棒的策略优化目标提供了实用指导。

Conclusion: 门函数的选择对大型语言模型的训练稳定性和性能有重要影响。通过形式化门函数的关键属性并进行实证评估，可以为设计更有效的策略优化方法提供指导，从而提升大型语言模型的训练效果。
...
### [291] [Active perception and disentangled representations allow continual, episodic zero and few-shot learning](https://arxiv.org/abs/2602.19355)
*David Rawlinson,Gideon Kowadlo*

Main category: cs.LG

TL;DR: 论文提出了一种互补学习系统，其中快速学习者放弃泛化能力，专注于零样本和少样本学习，通过上下文偏置引导慢速统计学习者实现稳健的持续学习。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习系统追求泛化能力，但这可能导致表示纠缠和边界模糊，在持续学习和少样本学习中产生破坏性干扰。需要一种既能快速学习新知识又不干扰已有知识的系统架构。

Method: 设计互补学习系统：快速学习者放弃泛化，专注于解耦表示和零样本/少样本学习；慢速学习者保持统计泛化能力。快速学习者通过上下文偏置引导慢速学习者用熟悉的泛化术语编码新刺激。

Result: 该系统实现了快速上下文驱动推理与慢速结构化泛化的共存，能够在克服观测变异性和不确定性的同时，支持零样本和少样本学习，为稳健的持续学习提供了途径。

Conclusion: 并非系统的每个组件都需要泛化能力，通过分离快速解耦学习和慢速统计学习的功能，可以构建更强大的互补学习系统，实现快速适应与稳定泛化的平衡。
...
### [292] [LLMs Can Learn to Reason Via Off-Policy RL](https://arxiv.org/abs/2602.19362)
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型的强化学习方法通常使用同策略算法（如PPO、GRPO），但分布式训练架构导致的策略滞后以及训练与推理策略的差异使得数据本质上是离策略的。先前工作试图通过重要性采样或修改推理引擎来使离策略数据看起来更像同策略，但这些方法存在局限性。

Method: 提出OAPL（Optimal Advantage-based Policy Optimization with Lagged Inference policy）算法，这是一种新型离策略强化学习方法，接受离策略性而不需要重要性采样或修改推理引擎。该方法基于最优优势的策略优化，专门处理训练与推理策略之间的滞后问题。

Result: OAPL在数学竞赛基准测试中优于使用重要性采样的GRPO；在LiveCodeBench上与公开可用的DeepCoder模型性能相当，但训练时使用的生成数量减少3倍；在Pass@k指标下表现出更好的测试时扩展性；能够处理训练与推理策略之间超过400个梯度步的滞后，比先前方法离策略程度高100倍。

Conclusion: OAPL是一种高效有效的离策略强化学习方法，能够处理大语言模型训练中固有的策略滞后问题，无需复杂的重要性采样或推理引擎修改，在多个基准测试中表现出优越性能。
...
### [293] [Stable Deep Reinforcement Learning via Isotropic Gaussian Representations](https://arxiv.org/abs/2602.19373)
*Ali Saheb,Johan Obando-Ceron,Aaron Courville,Pouya Bashivan,Pablo Samuel Castro*

Main category: cs.LG

TL;DR: 提出各向同性高斯嵌入在非平稳目标下的理论优势，并基于此设计了Sketched Isotropic Gaussian Regularization方法，通过正则化使表示趋向各向同性高斯分布，提升非平稳环境下的性能与稳定性。


<details>
  <summary>Details</summary>
Motivation: 深度强化学习系统在非平稳环境下常面临训练不稳定问题，学习目标和数据分布随时间变化导致训练动态不稳定。需要寻找能够稳定跟踪时变目标并提高适应性的表示方法。

Method: 提出Sketched Isotropic Gaussian Regularization方法，在训练过程中通过正则化将表示塑造成各向同性高斯分布。该方法计算成本低，通过理论分析证明各向同性高斯嵌入在非平稳目标下的优势：稳定跟踪时变目标、在固定方差预算下实现最大熵、鼓励所有表示维度的平衡使用。

Result: 在多个领域进行实证验证，表明这种简单且计算成本低的方法能提高非平稳环境下的性能，同时减少表示坍塌、神经元休眠和训练不稳定性。

Conclusion: 各向同性高斯嵌入在非平稳目标下具有理论优势，基于此设计的正则化方法能有效提升深度强化学习系统在非平稳环境下的适应性和稳定性，为解决训练不稳定问题提供了简单有效的解决方案。
...
### [294] [In Defense of Cosine Similarity: Normalization Eliminates the Gauge Freedom](https://arxiv.org/abs/2602.19393)
*Taha Bouhsine*

Main category: cs.LG

TL;DR: 论文反驳了Steck等人的观点，指出余弦相似度本身没有问题，问题在于没有对嵌入进行归一化处理。当嵌入被约束在单位球面上时，D矩阵歧义消失，余弦距离与欧氏距离等价。


<details>
  <summary>Details</summary>
Motivation: 针对Steck等人提出的余弦相似度在矩阵分解模型中存在任意性的问题，本文旨在澄清这一误解，证明问题不在于余弦相似度本身，而在于嵌入没有进行适当的归一化处理。

Method: 通过数学证明，当嵌入被约束在单位球面S^{d-1}上时（无论是在训练期间还是训练后通过适当的目标函数），D矩阵的歧义完全消失，余弦距离精确地等于欧氏距离平方的一半。

Result: 证明了在归一化嵌入上，余弦距离与欧氏距离之间存在单调等价关系，这意味着基于余弦和基于欧氏的邻居排名是完全相同的。D矩阵歧义在归一化条件下不复存在。

Conclusion: 余弦相似度本身没有问题，问题在于没有对嵌入进行归一化。当嵌入被适当地归一化到单位球面上时，余弦距离是几何有效的，并且与欧氏距离等价，可以安全地用于邻居排名等任务。
...
### [295] [RAmmStein: Regime Adaptation in Mean-reverting Markets with Stein Thresholds -- Optimal Impulse Control in Concentrated AMMs](https://arxiv.org/abs/2602.19419)
*Pranay Anchuri*

Main category: cs.LG

TL;DR: RAmmStein：一种基于深度强化学习的流动性管理方法，通过最优控制解决集中流动性提供中的再平衡成本问题，相比被动和激进策略实现0.72%的净投资回报率提升


<details>
  <summary>Details</summary>
Motivation: 去中心化交易所中的集中流动性提供面临基本脉冲控制问题：流动性提供者需要在通过紧密价格区间集中最大化费用累积与最小化再平衡摩擦成本（包括gas费和交易滑点）之间进行权衡。现有方法通常采用启发式或阈值策略，未能考虑市场动态。
...
Result: 使用包含超过680万笔交易的高频1Hz Coinbase交易数据进行评估。RAmmStein实现了0.72%的净投资回报率，优于被动和激进策略。与贪婪再平衡策略相比，再平衡频率降低了67%，同时保持88%的活动时间。

Conclusion: 制度感知的"懒惰"策略通过保留原本会被运营成本侵蚀的回报，能够显著提高资本效率。智能体学会在状态空间中分离行动和非行动区域，优化流动性管理决策。
...
### [296] [PIS: A Physics-Informed System for Accurate State Partitioning of $Aβ_{42}$ Protein Trajectories](https://arxiv.org/abs/2602.19444)
*Qianfeng Yu,Ningkang Peng,Yanhui Gu*

Main category: cs.LG

TL;DR: PIS是一个物理信息化的系统，用于β-淀粉样蛋白(Aβ42)构象演化的稳健亚稳态划分，通过整合物理先验知识提升深度学习模型对蛋白质轨迹中微妙状态转变的捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 现有端到端深度学习模型在捕捉蛋白质轨迹中的微妙状态转变方面存在困难，主要原因是缺乏明确的物理约束。理解Aβ42的构象演化对阐明阿尔茨海默病的致病机制至关重要。

Method: 提出PIS系统，通过整合预计算的物理先验知识（如回转半径和溶剂可及表面积）到拓扑特征提取中，实现稳健的亚稳态划分。系统还提供交互平台，支持物理特性的动态监测和多维结果验证。

Result: PIS在Aβ42数据集上表现出优越性能，为生物研究人员提供了一套具有物理基础可解释性的强大分析工具。

Conclusion: PIS系统通过整合物理约束，显著提升了蛋白质构象演化中亚稳态划分的准确性和可解释性，为阿尔茨海默病相关研究提供了有价值的分析工具。
...
### [297] [SenTSR-Bench: Thinking with Injected Knowledge for Time-Series Reasoning](https://arxiv.org/abs/2602.19455)
...
TL;DR: 提出混合知识注入框架，将时间序列LLM的领域知识直接注入通用推理LLM的推理轨迹中，实现强时间序列诊断推理能力


<details>
  <summary>Details</summary>
Motivation: 现有解决方案存在明显差距：通用推理大语言模型（GRLM）具有强大推理能力但缺乏领域知识理解复杂时间序列模式，而微调的时间序列LLM（TSLM）理解这些模式但缺乏复杂问题的泛化推理能力

Method: 1. 提出混合知识注入框架，将TSLM生成的洞察直接注入GRLM的推理轨迹；2. 使用基于强化学习的可验证奖励（RLVR）方法，无需人工监督即可获取知识丰富的推理轨迹；3. 将领域思维轨迹转移到GRLM中进行高效知识注入；4. 发布SenTSR-Bench多变量时间序列诊断推理基准数据集

Result: 在SenTSR-Bench和其他公共数据集上，该方法持续超越TSLM 9.1%-26.1%，超越GRLM 7.9%-22.4%，提供稳健、上下文感知的时间序列诊断洞察

Conclusion: 提出的混合知识注入框架成功弥合了通用推理能力和领域专业知识之间的差距，通过RLVR方法实现无监督知识获取，显著提升了时间序列诊断推理性能
...
### [298] [Making Conformal Predictors Robust in Healthcare Settings: a Case Study on EEG Classification](https://arxiv.org/abs/2602.19483)
*Arjun Chatterjee,Sayeed Sajjad Razin,John Wu,Siddhartha Laghuvarapu,Jathurshan Pradeepkumar,Jimeng Sun*

Main category: cs.LG

TL;DR: 评估多种共形预测方法在EEG癫痫分类任务上的表现，针对医疗场景中的分布偏移问题，发现个性化校准策略可将覆盖率提升20个百分点以上。


<details>
  <summary>Details</summary>
Motivation: 临床预测中的不确定性量化对高风险诊断任务至关重要。共形预测能提供具有理论覆盖保证的预测集，但实际中患者分布偏移违反了标准共形方法的i.i.d.假设，导致医疗场景中覆盖率不佳。

Method: 在EEG癫痫分类任务上评估多种共形预测方法，这是一个已知存在分布偏移挑战和标签不确定性的任务。采用个性化校准策略来应对分布偏移问题。

Result: 个性化校准策略可将覆盖率提升20个百分点以上，同时保持可比的预测集大小。

Conclusion: 针对医疗场景中的分布偏移问题，个性化校准策略能显著改善共形预测的覆盖率表现，该方法已通过开源医疗AI框架PyHealth实现。
...
### [299] [Federated Learning Playground](https://arxiv.org/abs/2602.19489)
...
<details>
  <summary>Details</summary>
Motivation: 降低联邦学习的入门门槛，为分布式AI新手提供易于使用的教育工具，同时为快速原型设计和比较FL方法提供沙盒环境，促进这一重要范式的更广泛理解和采用。

Method: 开发交互式浏览器平台，受TensorFlow Playground启发，允许用户直接实验异构客户端数据分布、模型超参数和聚合算法，无需编码或系统设置，通过实时可视化展示客户端和全局模型效果。

Result: 创建了一个易于使用的教育工具，帮助用户直观理解非IID数据、局部过拟合和可扩展性等挑战，降低了联邦学习的入门障碍，促进了该领域的探索和采用。

Conclusion: Federated Learning Playground通过民主化FL探索，促进了更广泛的理解和采用这一重要范式，既是教育工具也是快速原型设计沙盒。
...
### [300] [Softmax is not Enough (for Adaptive Conformal Classification)](https://arxiv.org/abs/2602.19498)
*Navid Akhavan Attar,Hesam Asadollahzadeh,Ling Luo,Uwe Aickelin*

Main category: cs.LG

TL;DR: 本文提出一种基于Helmholtz自由能的能量增强方法，通过重新加权非一致性分数来改进深度共形分类器的预测集自适应性和效率。


<details>
  <summary>Details</summary>
Motivation: 传统共形预测框架使用softmax输出作为非一致性分数，但这些分数可能不可靠，无法准确反映模型对输入样本的真实置信度，导致预测集的自适应性受限。

Method: 提出利用预softmax logit空间信息，使用Helmholtz自由能作为模型不确定性和样本难度的度量，通过对每个样本的能量分数进行单调变换来重新加权非一致性分数。

Result: 在多个数据集和深度架构上的实验表明，这种基于能量的增强方法显著提高了预测集的自适应性，相比基线非一致性分数在效率和自适应性方面都有明显提升。

Conclusion: 通过利用能量分数重新加权非一致性分数，可以有效改进深度共形分类器预测集的自适应性和效率，无需引入额外的后处理复杂度。
...
### [301] [Less is More: Convergence Benefits of Fewer Data Weight Updates over Longer Horizon](https://arxiv.org/abs/2602.19510)
*Rudrajit Das,Neel Patel,Meisam Razaviyayn,Vahab Mirrokni*

Main category: cs.LG

TL;DR: 本文分析了数据混合（data mixing）中双层优化问题的收敛行为，特别关注有限内层更新步数T的影响，证明了贪婪方法（T=1）可能失败，并给出了最优T的缩放规律。


<details>
  <summary>Details</summary>
Motivation: 数据混合是训练鲁棒机器学习模型的关键技术，通常被表述为双层优化问题。现有方法由于计算限制，在更新权重前只使用有限的内层更新步数，但这种近似的理论影响尚未被充分理解。

Method: 本文对数据混合中有限内层步数T的收敛行为进行严格理论分析，证明贪婪方法（T=1）在简单二次例子中可能失败。在固定参数更新预算N和强凸损失假设下，推导出最优T的缩放规律。

Result: 理论分析表明：对于使用全梯度的数据混合问题，最优T缩放为Θ(log N)；对于使用随机梯度的数据混合问题，最优T缩放为Θ((N log N)^{1/2})。实验验证了理论结果。

Conclusion: 本文为数据混合中的双层优化提供了理论指导，揭示了有限内层步数对收敛行为的影响，并给出了最优内层步数的缩放规律，对实际应用具有重要指导意义。
...
### [302] [Beyond Accuracy: A Unified Random Matrix Theory Diagnostic Framework for Crash Classification Models](https://arxiv.org/abs/2602.19528)
*Ibne Farabi Shihab,Sanjeda Akter,Anuj Sharma*

Main category: cs.LG

TL;DR: 提出基于随机矩阵理论和重尾自正则化的谱诊断框架，用于评估交通事故分类模型的过拟合问题，通过幂律指数α作为模型结构质量的信号指标。


<details>
  <summary>Details</summary>
Motivation: 传统交通事故分类模型评估指标（如准确率、F1、AUC）无法检测模型是否在静默过拟合，需要一种能够揭示模型内部结构质量的诊断方法。

Method: 引入基于随机矩阵理论和重尾自正则化的谱诊断框架，应用于多种机器学习模型（BERT/ALBERT/Qwen2.5的权重矩阵、XGBoost/Random Forest的折外增量矩阵、Logistic回归的经验Hessian、决策树的诱导亲和矩阵、KNN的图拉普拉斯矩阵），通过幂律指数α评估模型结构质量。

Result: 在爱荷华州交通部两个交通事故分类任务（173,512和371,062条记录）上评估九个模型族，发现良好正则化模型的α值在[2,4]范围内（均值2.87±0.34），而过拟合模型的α<2或出现谱崩溃。α与专家一致性存在强秩相关（Spearman ρ=0.89，p<0.001）。
...
### [303] [A Statistical Approach for Modeling Irregular Multivariate Time Series with Missing Observations](https://arxiv.org/abs/2602.19531)
*Dingyi Nie,Yixing Wu,C. -C. Jay Kuo*

Main category: cs.LG

TL;DR: 该研究提出了一种处理不规则多元时间序列的简单方法：提取时间无关的汇总统计量，消除时间轴，使用标准分类器实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 医疗等领域的不规则多元时间序列存在缺失值，给预测建模带来挑战。现有深度学习方法通常关注时间插值或复杂架构，但本文寻求更简单有效的替代方案。

Method: 为每个变量计算四个关键特征：观测值的均值和标准差，以及连续观测之间变化的均值和变异性，创建固定维度表示。然后使用逻辑回归和XGBoost等标准分类器。

Result: 在四个生物医学数据集上评估，该方法在AUROC/AUPRC上超过最近transformer和图模型0.5-1.7%，在准确率/F1分数上超过1.1-1.7%，同时降低计算复杂度。消融研究表明特征提取而非分类器选择驱动性能提升。

Conclusion: 当任务目标允许时间无关表示时，复杂的时间建模可能不是必需的。该方法为不规则时间序列分类提供了高效且可解释的解决方案，特别是在缺失模式本身编码预测信号的情况下。
...
### [304] [The Sample Complexity of Replicable Realizable PAC Learning](https://arxiv.org/abs/2602.19552)
*Kasper Green Larsen,Markus Engelund Mathiasen,Chirag Pabbaraju,Clement Svendsen*

Main category: cs.LG

TL;DR: 该论文研究了可复制可实现PAC学习问题，构建了一个特别困难的学习问题实例，证明了样本复杂度下界与假设类大小|H|的对数呈接近(3/2)次方关系，并给出了几乎匹配的上界。
...
Method: 构建一个特别困难的学习问题实例，通过定义与假设类H相关的特定Cayley图，分析在该图上的随机游走，并检查其邻接矩阵的谱特性来证明下界。

Result: 证明了样本复杂度下界具有接近(log|H|)^{3/2}的依赖关系，同时给出了几乎匹配的上界，表明如果存在更强的下界，必须考虑不同的问题实例。

Conclusion: 该研究为可复制可实现PAC学习问题建立了接近最优的样本复杂度界限，揭示了假设类大小对学习效率的具体影响，并提供了新的证明技术。
...
### [305] [Advantage-based Temporal Attack in Reinforcement Learning](https://arxiv.org/abs/2602.19582)
*Shenghong He*

Main category: cs.LG

TL;DR: 本文提出AAT方法，利用多尺度因果自注意力机制和加权优势机制生成具有更强时间相关性的对抗样本，提升对深度强化学习模型的攻击效果。
...
Method: 提出优势对抗变换器(AAT)：1) 采用多尺度因果自注意力机制动态捕捉历史信息与当前状态之间的依赖关系；2) 引入加权优势机制量化扰动在给定状态下的有效性，通过采样高优势区域指导生成过程。

Result: 在Atari、DeepMind Control Suite和Google football任务上的大量实验表明，AAT的性能达到或超越了主流对抗攻击基线方法。

Conclusion: AAT能够生成具有更强时间相关性的对抗样本，有效提升对深度强化学习模型的攻击性能，解决了现有方法在时序相关性方面的不足。
...
### [306] [Interpolation-Driven Machine Learning Approaches for Plume Shine Dose Estimation: A Comparison of XGBoost, Random Forest, and TabNet](https://arxiv.org/abs/2602.19584)
*Biswajit Sadhu,Kalpak Gupte,Trijit Sadhu,S. Anand*

Main category: cs.LG

TL;DR: 开发了一种插值辅助的机器学习框架，用于快速准确估计羽流照射剂量，通过增强数据集和比较不同ML模型性能，XGBoost表现最佳，并开发了基于Web的GUI用于实际部署。


<details>
  <summary>Details</summary>
Motivation: 机器学习在替代建模中虽成功，但在辐射剂量评估中的应用受到安全关键约束、训练数据稀缺以及物理主导系统架构选择困难的限制。快速准确的羽流照射剂量估计对核设施安全评估和放射应急响应至关重要，而传统光子传输计算计算成本高。

Method: 开发了插值辅助的机器学习框架，使用pyDOSEIA套件生成17种伽马发射放射性核素在不同下风距离、释放高度和大气稳定度类别下的离散剂量数据集。通过形状保持插值增强数据集构建高分辨率训练数据。评估了两种基于树的ML模型（随机森林和XGBoost）和一个深度学习模型（TabNet），分析预测性能和数据集分辨率敏感性。

Result: 所有模型在使用插值高分辨率数据集时都比离散数据预测精度更高；XGBoost始终达到最高准确度。可解释性分析显示，基于树的模型主要关注主导的几何-扩散特征（释放高度、稳定度类别和下风距离），将放射性核素身份视为次要输入，而TabNet将注意力更广泛分布在多个变量上。

Conclusion: 插值辅助的机器学习框架能有效提高辐射剂量估计的准确性和效率，XGBoost在树基模型中表现最佳。开发了基于Web的GUI用于交互式场景评估和与光子传输参考计算的透明比较，为实际部署提供了实用工具。
...
### [307] [Variational Inference for Bayesian MIDAS Regression](https://arxiv.org/abs/2602.19610)
*Luigi Simeone*

Main category: cs.LG

TL;DR: 开发了用于贝叶斯混合数据抽样(MIDAS)回归的坐标上升变分推断(CAVI)算法，通过归一化约束分离影响系数和权重函数参数，利用条件共轭性实现闭式更新，相比Gibbs采样获得107-1772倍加速，但均值场近似导致置信区间校准不足。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯MIDAS回归模型具有双线性结构，使得通用哈密顿蒙特卡洛采样器不可靠，但存在条件共轭性可被变分推断利用。需要开发高效的推断算法来处理高维预测变量（最多50个），同时保持参数估计的准确性。

Method: 开发了坐标上升变分推断(CAVI)算法，通过归一化约束分离影响系数和权重函数参数，利用模型的条件共轭结构。每个变分更新都有闭式解：回归系数和权重参数为高斯分布，误差方差为逆伽马分布。算法通过二阶矩在块之间传播不确定性。

Result: 在包含21种数据生成配置的蒙特卡洛研究中，CAVI产生的后验均值与块Gibbs采样基准几乎相同，同时获得107-1772倍的加速。权重函数参数在所有配置中保持优秀的校准（覆盖率>92%），但影响系数的置信区间显示均值场近似特有的欠分散问题，覆盖率随预测变量数量增加从89%下降到55%。

Conclusion: CAVI算法为贝叶斯MIDAS回归提供了高效准确的推断方法，在点估计方面与Gibbs采样相当，但存在均值场近似导致的置信区间校准不足问题。这体现了速度与区间校准之间的权衡，结构化变分方法可以解决这一问题。在S&P 500已实现波动率预测的实证应用中，CAVI与Gibbs采样产生几乎相同的点预测，每次月度估计在10毫秒内完成。
...
### [308] [Is Your Diffusion Sampler Actually Correct? A Sampler-Centric Evaluation of Discrete Diffusion Language Models](https://arxiv.org/abs/2602.19619)
*Luhan Tang,Longxuan Yu,Shaorong Zhang,Greg Ver Steeg*

Main category: cs.LG

TL;DR: 离散扩散语言模型通过并行迭代去噪提供快速灵活的生成，但现有评估方法混淆了去噪器近似误差和采样器引入的误差，作者提出了采样器中心化的oracle框架来隔离采样器误差。


<details>
  <summary>Details</summary>
Motivation: 现有离散扩散语言模型评估方法存在根本问题：将去噪器近似误差与采样器引入的误差混为一谈，而自回归模型不存在这个问题，因为其自回归采样精确反映了学习到的概率模型。需要一种能够隔离采样器误差的评估框架。

Method: 引入采样器中心化的oracle框架：用从真实马尔可夫链推导出的精确隐马尔可夫模型后验替代学习的去噪器，在受控环境中隔离采样器引入的误差。

Result: 研究发现：1）即使在oracle去噪器下，少步离散扩散采样器在分布上也不正确；2）过渡级不匹配只有在步数接近序列长度时才消失；3）负对数似然、生成困惑度或MAUVE指标的改进并不保证采样正确性。

Conclusion: 离散扩散采样器存在根本性分布偏差，需要重新思考其评估方法，仅靠传统指标改进不能保证采样质量，需要更严格的采样器评估框架。
...
### [309] [VecFormer: Towards Efficient and Generalizable Graph Transformer with Graph Token Attention](https://arxiv.org/abs/2602.19622)
...
<details>
  <summary>Details</summary>
Motivation: 现有图Transformer面临两个关键挑战：1）计算复杂度指数增长，难以扩展到大规模图；2）基于节点级操作的注意力机制限制了模型灵活性，在分布外（OOD）场景下泛化性能差。

Method: VecFormer采用两阶段训练范式：第一阶段使用两个码本重构节点特征和图结构，学习丰富的语义图编码；第二阶段在基于转换的交叉码本上进行图令牌级注意力操作，降低计算复杂度并增强泛化能力。

Result: 在不同规模的数据集上进行的大量实验表明，VecFormer在性能和速度上都优于现有的图Transformer。

Conclusion: VecFormer通过向量量化和两阶段训练策略，有效解决了图Transformer的计算效率和泛化能力问题，为节点分类任务提供了一个高效且泛化性强的解决方案。
...
### [310] [Compositional Planning with Jumpy World Models](https://arxiv.org/abs/2602.19634)
...
<details>
  <summary>Details</summary>
Motivation: 当前智能体在组合规划方面面临挑战，因为长时程预测中的累积误差使得难以准确估计策略序列引发的状态访问分布。需要一种能够有效组合预训练策略作为时间扩展动作的方法，以解决单个策略无法完成的复杂任务。

Method: 1. 学习"跳跃世界模型"来预测预训练策略在多时间尺度上的状态占用分布；2. 基于时间差分流框架，引入新颖的一致性目标来对齐不同时间尺度的预测；3. 结合这些生成预测来估计在不同时间尺度上执行任意策略序列的价值。

Result: 在具有挑战性的操作和导航任务上，使用跳跃世界模型的组合规划显著提升了零样本性能，在长时程任务上相比基于原始动作的规划平均实现了200%的相对改进。

Conclusion: 通过跳跃世界模型实现的组合规划能够有效利用预训练策略作为时间扩展动作，显著提升智能体在复杂长时程任务上的决策能力，为智能决策系统提供了一种有前景的方法。
...
### [311] [Spectral Phase Encoding for Quantum Kernel Methods](https://arxiv.org/abs/2602.19644)
...
<details>
  <summary>Details</summary>
Motivation: 量子核方法在近期量子机器学习中前景广阔，但其在数据损坏下的行为尚未充分理解。需要分析量子特征构造在受控加性噪声下的退化情况。

Method: 提出谱相位编码(SPE)方法，结合离散傅里叶变换(DFT)前端与对角相位嵌入，与对角量子映射的几何结构对齐。在统一框架下比较QK-DFT与其他量子变体(QK-PCA、QK-RP)和经典SVM基线，通过数据集固定效应回归和wild cluster bootstrap推断量化鲁棒性。

Result: 在量子方法家族中，基于DFT的预处理在噪声增加时表现出最小的退化率，与PCA和RP相比具有统计支持的斜率差异。与经典基线相比，QK-DFT的退化与线性SVM相当，比RBF SVM更稳定。硬件实验证实SPE在执行和数值稳定性方面表现良好。
...
### [312] [NEXUS : A compact neural architecture for high-resolution spatiotemporal air quality forecasting in Delhi Nationa Capital Region](https://arxiv.org/abs/2602.19654)
*Rampunit Kumar,Aditya Maheshwari*

Main category: cs.LG

TL;DR: NEXUS架构用于德里NCR地区空气污染预测，仅用18,748参数实现高精度（R²>0.94），超越现有模型，具有显著计算效率优势。


<details>
  <summary>Details</summary>
Motivation: 德里国家首都区面临严重的城市空气污染问题，对数百万人健康构成威胁，需要高效准确的预测模型来支持空气质量监测系统。

Method: 提出NEXUS（神经提取与统一时空）架构，整合补丁嵌入、低秩投影和自适应融合机制，使用2018-2021年16个空间网格的四年大气数据，解码复杂大气化学模式。

Result: NEXUS在CO、NO和SO₂预测上分别达到R²>0.94、0.91和0.95，参数数量（18,748）显著少于SCINet（35,552）、Autoformer（68,704）和FEDformer（298,080）。发现明显的昼夜节律和季节变化，冬季污染最严重。

Conclusion: NEXUS在预测性能和计算效率方面均表现优异，能够实现实时部署，为空气质量监测系统提供有效工具，同时揭示了德里NCR地区污染的关键时空模式和驱动因素。
...
### [313] [PaReGTA: An LLM-based EHR Data Encoding Approach to Capture Temporal Information](https://arxiv.org/abs/2602.19661)
...
<details>
  <summary>Details</summary>
Motivation: 传统结构化电子健康记录（EHR）表示方法（如one-hot或基于计数的表示）会丢失时间信息，而序列模型通常计算成本高且需要大量数据。需要一种能够在数据有限情况下有效捕捉EHR时间信息的方法。

Method: 提出PaReGTA框架：1) 将纵向EHR事件转换为带有明确时间线索的访问级模板化文本；2) 通过轻量级对比微调句子嵌入模型学习领域适应的访问嵌入；3) 使用混合时间池化（捕捉近期和全局信息访问）将访问嵌入聚合成固定维度的患者表示。还提出了PaReGTA-RSS用于可解释性分析。

Result: 在All of Us研究项目的39,088名偏头痛患者数据上，PaReGTA在偏头痛类型分类任务上优于稀疏基线方法，而深度序列模型在该队列中表现不稳定。

Conclusion: PaReGTA是一个有效且实用的EHR编码框架，能够在不从头训练的情况下利用预训练LLM，在数据有限的情况下表现良好，具有模型无关性和可解释性优势。
...
### [314] [PerturbDiff: Functional Diffusion for Single-Cell Perturbation Modeling](https://arxiv.org/abs/2602.19685)
*Xinyu Yuan,Xixian Liu,Ya Shi Zhang,Zuobai Zhang,Hongyu Guo,Jian Tang*

Main category: cs.LG

TL;DR: PerturbDiff是一个用于单细胞扰动预测的新模型，通过将建模对象从单个细胞转向整个分布，在希尔伯特空间中嵌入分布并定义基于扩散的生成过程，从而捕捉隐藏因素引起的种群水平响应变化。


<details>
  <summary>Details</summary>
Motivation: 构建能够准确模拟细胞对扰动响应的虚拟细胞是系统生物学的长期目标。核心挑战在于高通量单细胞测序是破坏性的：同一个细胞无法在扰动前后都被观察到。现有模型通过学习分布之间的映射来解决这个问题，但通常假设在给定观察到的细胞上下文（如细胞类型）和扰动类型时，响应分布是单一固定的。实际上，由于不可观察的潜在因素（如微环境波动和复杂批次效应），响应会系统性变化，形成相同观察条件下可能分布的流形。

Method: PerturbDiff将建模从单个细胞转向整个分布。通过将分布嵌入希尔伯特空间作为点，定义直接在概率分布上操作的基于扩散的生成过程。这种方法能够捕捉隐藏因素引起的种群水平响应变化。

Result: 在已建立的数据集上的基准测试表明，PerturbDiff在单细胞响应预测方面达到了最先进的性能，并且对未见过的扰动具有显著更好的泛化能力。

Conclusion: PerturbDiff通过将建模焦点从单个细胞转向分布，解决了单细胞扰动预测中的关键挑战，能够更好地捕捉响应变异性，并在预测性能和泛化能力方面表现出色。
...
### [315] [Understanding the Curse of Unrolling](https://arxiv.org/abs/2602.19733)
*Sheheryar Mehmood,Florian Knoll,Peter Ochs*

Main category: cs.LG

TL;DR: 本文分析了算法展开中的"展开诅咒"现象，解释了导数迭代初始发散的原因，提出了截断早期迭代的解决方案，并发现双层优化中的预热初始化能自然实现隐式截断。


<details>
  <summary>Details</summary>
Motivation: 算法展开在机器学习中广泛应用，特别是在超参数优化和元学习中，用于计算解映射的雅可比矩阵。然而，尽管展开在适当条件下能产生渐近正确的雅可比矩阵，但最近研究发现导数迭代可能初始发散，这种现象被称为"展开诅咒"。本文旨在分析这一现象的起源并找到解决方案。

Method: 采用非渐近分析方法，识别影响"展开诅咒"的算法因素。提出截断早期导数迭代的方法来缓解问题，同时减少内存需求。研究发现双层优化中的预热初始化能自然地实现隐式截断。

Result: 理论分析解释了导数迭代初始发散的原因，截断早期迭代能有效缓解"展开诅咒"现象。数值实验验证了理论发现，显示预热初始化在双层优化中能自然地实现截断效果。

Conclusion: 本文揭示了算法展开中"展开诅咒"现象的起源，提出了截断早期迭代的实用解决方案，并发现双层优化中的预热初始化能自然地实现这一截断效果，为解决该问题提供了理论依据和实用方法。
...
### [316] [Addressing Instrument-Outcome Confounding in Mendelian Randomization through Representation Learning](https://arxiv.org/abs/2602.19782)
*Shimeng Huang,Matthew Robinson,Francesco Locatello*

Main category: cs.LG

TL;DR: 该论文提出了一种利用多环境数据的表示学习框架，通过跨环境不变性来恢复遗传工具变量的潜在外生成分，以解决孟德尔随机化中工具变量与未观测混杂因素相关的假设违反问题。


<details>
  <summary>Details</summary>
Motivation: 孟德尔随机化方法的核心假设——工具变量与未观测混杂因素独立——经常因人群分层或同型交配而被违反。随着多环境数据的可用性增加，需要新的方法来处理这些假设违反问题。

Method: 提出一个表示学习框架，利用跨环境不变性来恢复遗传工具变量的潜在外生成分。该方法在各种混合机制下提供了理论保证，并通过模拟和半合成实验验证。

Result: 在模拟和基于All of Us研究中心的半合成实验中证明了该方法的有效性，能够识别潜在工具变量并改善因果效应估计。

Conclusion: 该表示学习框架能够利用多环境数据解决孟德尔随机化中的假设违反问题，为处理工具变量与混杂因素相关的情况提供了新的解决方案。
...
### [317] [Bayesian Meta-Learning with Expert Feedback for Task-Shift Adaptation through Causal Embeddings](https://arxiv.org/abs/2602.19788)
*Lotta Mäkinen,Jorge Loría,Samuel Kaski*

Main category: cs.LG

TL;DR: 提出基于因果感知的贝叶斯元学习方法，通过因果任务嵌入控制先验不匹配，减少负迁移，提升分布外适应能力


<details>
  <summary>Details</summary>
Motivation: 传统元学习方法在分布内任务上表现良好，但在适应分布外目标任务时容易失败，源任务的迁移可能导致负迁移。需要基于机制相似性而非虚假相关性进行迁移

Method: 提出因果感知的贝叶斯元学习方法，将任务特定先验条件于预计算的潜在因果任务嵌入上，利用专家提供的源任务与目标任务之间的因果相似性配对判断

Result: 理论分析显示基于因果嵌入的条件化控制先验不匹配并减轻任务偏移下的负迁移。实证在受控模拟和大规模真实临床预测场景中均减少了负迁移，改善了分布外适应

Conclusion: 通过因果任务嵌入实现基于机制相似性的迁移，有效缓解负迁移问题，在有限目标数据且依赖噪声因果相似性判断的现实部署场景中表现优越
...
### [318] [Stop Preaching and Start Practising Data Frugality for Responsible Development of AI](https://arxiv.org/abs/2602.19789)
*Sophia N. Wilson,Guðrún Fjóla Guðmundsdóttir,Andrew Millard,Raghavendra Selvan,Sebastian Mair*

Main category: cs.LG

TL;DR: 本文主张机器学习社区应从"说教"转向"实践"数据节俭，以负责任地发展AI。当前过度依赖大数据规模导致收益递减、能耗剧增，而数据节俭方法虽被认可但未落实。作者通过ImageNet-1K能耗估算和核心集选择实证，证明数据节俭可行且有益，并提出具体实施建议。


<details>
  <summary>Details</summary>
Motivation: 机器学习社区长期以来将进步等同于更大的数据集，这虽然推动了显著进展，但现在带来了收益递减、能耗和碳排放剧增的问题。尽管数据节俭方法的意识有所提高，但实际采用仍然停留在口头层面，数据规模扩张继续主导开发实践。这种说教与实践之间的差距必须弥合，因为持续的数据规模扩张带来了重大且未被充分核算的环境影响。

Method: 1. 提供ImageNet-1K下游使用的能耗和碳排放估算；2. 通过核心集（coreset）基于子集选择的实证研究，展示数据节俭的实际可行性；3. 提出将数据节俭从口头说教转向具体实践的可操作建议。

Result: 研究表明：1. 核心集子集选择能显著减少训练能耗，同时保持较高的准确性；2. 数据节俭方法还能减轻数据集偏见；3. 数据节俭在实践中既可行又有益，能够平衡性能与环境影响。

Conclusion: 机器学习社区必须从说教转向实践数据节俭，以实现负责任的人工智能发展。通过采用核心集选择等数据节俭技术，可以在保持模型性能的同时大幅降低能耗和碳排放，并缓解数据集偏见问题。作者呼吁社区采取具体行动，将数据节俭原则真正落实到AI开发实践中。
...
### [319] [Drift Localization using Conformal Predictions](https://arxiv.org/abs/2602.19790)
*Fabian Hinder,Valerie Vaquet,Johannes Brinkrolf,Barbara Hammer*

Main category: cs.LG

TL;DR: 本文提出了一种基于保形预测的概念漂移定位新方法，克服了传统局部测试方法在高维低信号场景中的局限性
...
Method: 采用基于保形预测（conformal predictions）的根本不同方法，而不是传统的局部测试方案

Result: 在最新的图像数据集上展示了所提方法的性能表现，证明了其有效性

Conclusion: 基于保形预测的方法为解决高维低信号环境下的概念漂移定位问题提供了有效途径，克服了传统方法的局限性
...
### [320] [Decision MetaMamba: Enhancing Selective SSM in Offline RL with Heterogeneous Sequence Mixing](https://arxiv.org/abs/2602.19805)
*Wall Kim,Chaeyoung Song,Hanul Kim*

Main category: cs.LG

TL;DR: DMM（Decision MetaMamba）通过用密集层序列混合器替代Mamba的token混合器，并修改位置结构来保留局部信息，解决了Mamba在离线RL中因选择性机制导致关键步骤信息丢失的问题。


<details>
  <summary>Details</summary>
Motivation: Mamba模型在离线强化学习中存在选择性机制问题，当RL序列中的关键步骤被省略时，这种机制往往是有害的，导致信息丢失。

Method: 提出DMM结构，用密集层序列混合器替代Mamba的token混合器，修改位置结构以保留局部信息，在Mamba之前执行考虑所有通道的序列混合，防止选择性扫描和残差门控导致的信息丢失。

Result: DMM在多种RL任务中实现了最先进的性能，同时具有紧凑的参数规模，显示出在实际应用中的强大潜力。

Conclusion: DMM通过改进Mamba的序列处理机制，有效解决了离线强化学习中的信息丢失问题，在保持参数效率的同时实现了卓越性能。
...
### [321] [Generalized Random Direction Newton Algorithms for Stochastic Optimization](https://arxiv.org/abs/2602.19893)
*Soumen Pachal,Prashanth L. A.,Shalabh Bhatnagar,Avinash Achar*

Main category: cs.LG

TL;DR: 提出了一种基于随机方向随机逼近的广义海森矩阵估计器家族，仅使用噪声函数测量值，不同测量次数对应不同偏置阶数，并分析了相应的随机牛顿方法收敛性。


<details>
  <summary>Details</summary>
Motivation: 在仅能获得噪声函数测量值的情况下，需要开发有效的海森矩阵估计方法以支持随机牛顿优化算法，传统方法在测量次数和估计精度之间存在权衡。

Method: 提出基于随机方向随机逼近的广义海森矩阵估计器家族，通过不同数量的函数测量值构建估计器，测量次数越多则估计偏置阶数越低，并分析了相应的随机牛顿方法。

Result: 证明了估计器的渐近无偏性，进行了渐近和非渐近收敛性分析，数值实验验证了理论结果，显示更多函数测量能获得更低阶的估计偏置。

Conclusion: 该广义海森矩阵估计器家族为仅使用噪声函数测量值的随机牛顿方法提供了有效的二阶信息估计框架，在测量次数和估计精度之间建立了明确的理论关系。
...
### [322] [DSDR: Dual-Scale Diversity Regularization for Exploration in LLM Reasoning](https://arxiv.org/abs/2602.19895)
...
Method: DSDR将LLM推理多样性分解为全局和耦合两个组成部分：1）全局层面，促进正确推理轨迹间的多样性以探索不同的解决方案模式；2）局部层面，在正确轨迹上应用长度不变、token级的熵正则化，防止每个模式内的熵坍缩同时保持正确性；3）通过全局到局部的分配机制将两个尺度耦合，对更具区分性的正确轨迹强调局部正则化。

Result: 在多个推理基准测试上的实验表明，DSDR在准确率和pass@k指标上均取得一致改进，突显了双尺度多样性对RLVR中深度探索的重要性。

Conclusion: DSDR通过双尺度多样性正则化框架有效解决了RLVR中的探索不足问题，理论分析表明其在有界正则化下保持最优正确性，维持基于组优化的信息性学习信号，并提供原则性的全局-局部耦合规则。
...
### [323] [De novo molecular structure elucidation from mass spectra via flow matching](https://arxiv.org/abs/2602.19912)
*Ghaith Mqawass,Tuan Le,Fabian Theis,Djork-Arné Clevert*

Main category: cs.LG

TL;DR: MSFlow是一个两阶段的编码器-解码器流匹配生成模型，用于从质谱数据中解析分子结构，性能比现有技术提升高达14倍。


<details>
  <summary>Details</summary>
Motivation: 质谱是识别分子结构的有力工具，但将质谱数据转化为完整的分子结构是一个困难且定义不明确的反问题。解决这个问题对于获得生物学洞见、发现新代谢物以及推进多个领域的化学研究至关重要。

Method: 开发了两阶段的编码器-解码器流匹配生成模型：第一阶段采用公式限制的transformer模型将质谱编码到连续且具有化学信息性的嵌入空间；第二阶段训练解码器流匹配模型从质谱的潜在嵌入中重建分子。

Result: MSFlow能够准确地将高达45%的分子质谱转化为相应的分子表示，比当前最先进技术提高了14倍。消融研究表明使用信息保留的分子描述符对编码质谱的重要性。

Conclusion: MSFlow在分子结构解析任务上实现了最先进的性能，训练版本已在GitHub上公开供非商业用户使用，为质谱数据的分子结构解析提供了强大的新工具。
...
### [324] [Rethinking LoRA for Privacy-Preserving Federated Learning in Large Models](https://arxiv.org/abs/2602.19926)
*Jin Liu,Yinbin Miao,Ning Xi,Junkang Liu*

Main category: cs.LG

TL;DR: LA-LoRA：一种针对差分隐私联邦学习的本地交替LoRA方法，解决了传统LoRA在隐私约束下的性能下降问题，在视觉和语言模型上都取得了SOTA表现。


<details>
  <summary>Details</summary>
Motivation: 在差分隐私联邦学习（DPFL）中直接应用LoRA会导致性能下降，特别是在大型视觉模型上。研究发现存在三个未充分探索的挑战：两个非对称低秩矩阵的梯度耦合、差分隐私下的噪声放大、以及全局聚合模型在参数空间中的尖锐性。

Method: 提出LA-LoRA（本地交替LoRA），通过解耦梯度交互和对齐客户端更新方向来增强在严格隐私约束下的鲁棒性。该方法理论上加强了在噪声联邦环境中的收敛保证。

Result: 在Swin Transformer和RoBERTa模型上的广泛实验表明，LA-LoRA实现了最先进的性能，对DP噪声具有鲁棒性，在LVMs和LLMs上都具有广泛适用性。例如，在严格隐私预算（ε=1）下微调Swin-B模型时，LA-LoRA比最佳基线RoLoRA在测试准确率上高出16.83%。

Conclusion: LA-LoRA有效解决了LoRA在差分隐私联邦学习中的隐私-效用权衡问题，通过创新的本地交替更新策略显著提升了模型性能，为在隐私敏感场景下高效微调大型模型提供了实用解决方案。
...
### [325] [Expanding the Role of Diffusion Models for Robust Classifier Training](https://arxiv.org/abs/2602.19931)
*Pin-Han Huang,Shang-Tse Chen,Hsuan-Tien Lin*

Main category: cs.LG

TL;DR: 该研究探讨了在对抗训练中不仅使用扩散模型生成合成数据，还利用其内部表征来提升鲁棒分类器训练效果的方法。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究显示将扩散模型生成的合成数据纳入对抗训练能显著改善鲁棒分类器的训练，但扩散模型的内部表征（编码了数据的语义特征）是否能为鲁棒训练提供额外益处尚未被充分探索。

Method: 通过系统实验，研究将扩散模型的表征作为辅助学习信号显式地整合到对抗训练中，分析这些表征的多样性和部分鲁棒性，并探讨它们如何影响特征解耦。

Result: 实验表明，在对抗训练中融入扩散表征能持续提升不同设置下的鲁棒性，扩散表征鼓励更解耦的特征学习，且扩散表征与扩散生成的合成数据在塑造表征方面具有互补作用。在CIFAR-10、CIFAR-100和ImageNet数据集上的验证证实了这些发现。

Conclusion: 扩散模型的内部表征能为对抗训练提供有价值的辅助学习信号，与合成数据协同使用时效果更佳，这为提升鲁棒分类器训练提供了新的有效途径。
...
### [326] [DP-FedAdamW: An Efficient Optimizer for Differentially Private Federated Large Models](https://arxiv.org/abs/2602.19945)
*Jin Liu,Yinbin Miao,Ning Xi,Junkang Liu*

Main category: cs.LG

TL;DR: 本文提出DP-FedAdamW，首个基于AdamW的差分隐私联邦学习优化器，解决了AdamW在DPFL中的三个主要问题，实现了更快的收敛速度和更好的性能。


<details>
  <summary>Details</summary>
Motivation: 在差分隐私联邦学习中平衡收敛效率和鲁棒性是一个核心挑战。虽然AdamW在大规模模型中加速训练和微调，但直接应用于DPFL存在三个主要问题：数据异构性和隐私噪声共同放大二阶矩估计器的方差、DP扰动使二阶矩估计器产生偏差、DP放大了AdamW对局部过拟合的敏感性，加剧客户端漂移。

Method: 提出DP-FedAdamW优化器，通过稳定二阶矩方差、消除DP引起的偏差、对齐局部更新与全局下降方向来抑制客户端漂移。理论分析建立了无偏二阶矩估计器，证明了线性加速收敛率，同时提供更紧的(ε,δ)-DP保证。

Result: 在语言和视觉Transformer以及ResNet-18上的实验结果表明DP-FedAdamW的有效性。在Tiny-ImageNet数据集上（Swin-Base，ε=1），DP-FedAdamW比现有最佳方法性能提升5.83%。

Conclusion: DP-FedAdamW成功解决了AdamW在差分隐私联邦学习中的三个关键问题，实现了更优的收敛效率和性能表现，为大规模模型的隐私保护训练提供了有效解决方案。
...
### [327] [Discrete Diffusion Models Exploit Asymmetry to Solve Lookahead Planning Tasks](https://arxiv.org/abs/2602.19980)
*Itamar Trainin,Shauli Ravfogel,Omri Abend,Amir Feder*

Main category: cs.LG

TL;DR: 自回归与非自回归模型在规划任务中的机制差异：NAR模型通过反向解码利用未来信息，比AR模型更高效


<details>
  <summary>Details</summary>
Motivation: 研究自回归（AR）与非自回归（NAR）模型在需要多步前瞻的规划任务中的不同涌现机制，探索两者在解决前瞻任务时的根本差异

Method: 通过要求模型进行前瞻规划以得出正确结论，分析AR模型（如Transformer）与NAR模型（如离散扩散模型dLLMs）在训练和推理动态中的机制差异，特别关注规划问题中的关键不对称性

Result: AR和NAR模型都能在前瞻任务上达到完美准确率，但NAR模型需要的训练样本数量呈指数级减少，架构也更浅；AR模型通常需要特定的课程调整才能收敛

Conclusion: 规划问题存在关键不对称性：前向生成在分支点需要复杂前瞻，而反向生成通常是确定性的。NAR模型通过利用未来标记进行反向解码来避免学习复杂的遍历机制，从而更高效地解决规划任务
...
### [328] [A Computationally Efficient Multidimensional Vision Transformer](https://arxiv.org/abs/2602.19982)
...
Method: 提出基于张量余弦积的新框架，利用图像数据的多线性结构和余弦变换正交性，构建高效的注意力机制和结构化特征表示。开发了张量余弦积的理论基础，分析其代数性质，并集成到新的TCP-ViT架构中。

Result: 在标准分类和分割基准测试中，该方法实现了统一的1/C参数减少（C为通道数），同时保持了竞争力的准确率。

Conclusion: 基于张量余弦积的Vision Transformer框架能够显著降低计算和内存成本，同时保持性能，为Vision Transformers的实际部署提供了有效的解决方案。
...
### [329] [Counterfactual Understanding via Retrieval-aware Multimodal Modeling for Time-to-Event Survival Prediction](https://arxiv.org/abs/2602.19987)
...
<details>
  <summary>Details</summary>
Motivation: 解决在异质性和删失数据存在的情况下，优化个体化生存结果的挑战，需要更有效的反事实生存建模方法来整合多模态临床数据。

Method: 提出CURE框架，通过跨注意力机制整合临床、辅助临床、人口统计学和多组学信息，使用专家混合架构自适应精炼多组学信号，并隐式检索捕捉基线生存动态和治疗依赖变化的患者特定潜在亚组。

Result: 在METABRIC和TCGA-LUAD数据集上的实验结果表明，CURE模型在生存分析中持续优于强基线模型，使用时间依赖性一致性指数和综合Brier评分进行评估。
...
TL;DR: 本文提出了一种用于动态图异常检测的新框架，通过残差表示编码、限制损失和双边界优化策略，在仅有正常/未标记数据的情况下学习判别边界，并能利用有限的标记异常而不损害对未见异常的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 动态图异常检测面临标记异常稀缺的挑战。现有方法要么是无监督的（产生模糊边界），要么是半监督的（容易过拟合有限标记异常且泛化能力差）。本文旨在填补这一空白，探索在仅有正常/未标记数据时学习判别边界，同时能在有标记异常时利用它们而不损害对未见异常的泛化能力。

Method: 提出一个包含三个主要组件的框架：1）残差表示编码：捕捉当前交互与其历史背景之间的偏差，提供异常相关信号；2）限制损失：将正常表示约束在两个同中心超球面界定的区间内，确保尺度一致同时保持异常可分离；3）双边界优化策略：使用归一化流建模的正常对数似然分布学习判别且鲁棒的边界。

Result: 大量实验证明该框架在多种评估设置下具有优越性，能够有效处理动态图异常检测问题，特别是在标记异常稀缺的情况下。

Conclusion: 本文提出的框架是有效、可泛化且模型无关的，能够解决动态图异常检测中标记异常稀缺的挑战，通过创新的残差编码、限制损失和双边界优化策略，实现了对未见异常的良好泛化能力。
...
TL;DR: BarrierSteer是一个基于控制屏障函数的安全框架，通过在潜在表示空间嵌入非线性安全约束来防止LLMs生成不安全内容，无需修改模型参数。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在各种任务上表现出色，但它们对对抗攻击和不安全内容生成的脆弱性仍然是部署的主要障碍，特别是在高风险场景中。需要既实用有效又有理论支持的安全机制。
...
Conclusion: BarrierSteer为LLMs提供了一个既保持原始性能又能有效防止不安全内容生成的安全框架，通过控制屏障函数在潜在空间的转向机制实现了理论严谨且实用的安全保护。
...
TL;DR: 该论文研究了对抗注入模型中的在线学习问题，证明了分布无关算法存在Ω(√T)下界，并提出基于鲁棒见证者的框架，为二维半空间等类别提供了首个分布无关学习保证。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明，在对抗注入模型中，如果能够访问底层分布，可以获得O(d²log T)的误差界，但分布无关算法只能达到Õ(√T)。这种差距是否本质存在是一个开放问题。

Method: 提出了基于鲁棒见证者的势能框架，该框架使用两种组合维度：(1)推理维度，获得Õ(T^{1-1/k})误差界；(2)新引入的证书维度。特别展示了二维半空间的证书维度为3。

Result: 证明了对于VC维度为1的类别，存在Ω(√T)的下界，确立了两种信息机制之间的尖锐分离。同时为二维半空间获得了Õ(T^{2/3})的分布无关误差界。

Conclusion: 该工作解决了对抗注入模型中分布相关与分布无关学习之间的根本差距问题，提出了新的理论框架，并为重要类别如二维半空间提供了首个分布无关学习保证。
...
### [333] [Adaptation to Intrinsic Dependence in Diffusion Language Models](https://arxiv.org/abs/2602.20126)
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型作为自回归方法的替代方案，虽然取得了经验上的成功，但理论上对去掩码调度如何影响生成质量的理解仍然有限。现有确定性方法固定去掩码大小，缺乏对数据分布依赖结构的适应性。

Method: 提出一种分布无关的自适应去掩码调度方法，随机化每次迭代中揭示的token数量，无需先验知识或超参数调优。该方法能适应未知的目标数据分布依赖结构。

Result: 对于两种特定参数选择，采样收敛保证（以KL散度衡量）分别按 $\widetilde O(\mathsf{TC}/K)$ 和 $\widetilde O(\mathsf{DTC}/K)$ 缩放，其中K是迭代次数，TC和DTC是目标分布的总相关和双总相关。这些保证在并行采样机制K<L（L为token序列长度）下成立。

Conclusion: 该方法显著改进了先前的收敛理论，对于低复杂度分布实现了显著的采样加速。研究揭示了扩散语言模型对内在数据结构的适应性，并阐明了随机化去掩码大小在推理调度设计中的优势。
...
### [334] [LAD: Learning Advantage Distribution for Reasoning](https://arxiv.org/abs/2602.20132)
*Wendi Li,Sharon Li*
...
<details>
  <summary>Details</summary>
Motivation: 当前基于最大化期望奖励的强化学习目标容易导致过拟合到主导奖励信号，忽视其他有效推理轨迹，限制了多样性和探索能力。

Method: 提出学习优势分布（LAD）框架，用优势诱导分布匹配替代优势最大化，通过最小化策略诱导分布与优势诱导分布之间的f-散度来更新策略。

Result: 在受控赌博机设置中，LAD能准确恢复多模态优势分布；在数学和代码推理任务中，LAD可靠地提高了准确性和生成多样性。

Conclusion: LAD框架通过分布匹配有效解决了强化学习中的过拟合问题，在保持训练效率的同时提升了推理任务的性能和多样性。
...
### [335] [Behavior Learning (BL): Learning Hierarchical Optimization Structures from Data](https://arxiv.org/abs/2602.20152)
...
<details>
  <summary>Details</summary>
Motivation: 受行为科学启发，旨在开发一个能够从数据中学习可解释且可识别的优化结构的通用机器学习框架，统一预测性能、内在可解释性和可识别性，适用于涉及优化的科学领域。

Method: BL通过参数化由内在可解释模块块构建的组合效用函数来工作，每个模块块都可以表示为效用最大化问题（UMP）。支持从单个UMP到分层组合的架构，其平滑单调变体（IBL）保证可识别性。

Result: 理论上证明了BL的通用逼近性质，分析了IBL的M估计特性。实证上，BL展示了强大的预测性能、内在可解释性以及对高维数据的可扩展性。

Conclusion: BL是一个新颖的通用机器学习框架，通过学习可解释且可识别的优化结构，成功统一了预测性能、内在可解释性和可识别性，具有广泛的应用潜力。
...
### [336] [Feedback-based Automated Verification in Vibe Coding of CAS Adaptation Built on Constraint Logic](https://arxiv.org/abs/2602.18607)
*Michal Töpfer,František Plášil,Tomáš Bureš,Petr Hnětynka*

Main category: cs.AI

TL;DR: 本文提出使用基于FCL约束的vibe coding反馈循环生成自适应管理器(AM)，通过迭代测试和约束违反报告来改进LLM生成的代码，在CAS领域实验中取得了良好效果。


<details>
  <summary>Details</summary>
Motivation: 在CAS自适应系统中，动态架构和行为变化的定义是一个挑战。传统上通过自适应管理器(AM)实现，而生成式LLM为基于系统规范和自然语言描述生成AM代码提供了新机会。但生成代码的正确性验证是关键问题，vibe coding通过迭代测试和反馈循环提供了解决方案。

Method: 1. 提出使用vibe coding反馈循环生成AM代码；2. 引入新的时序逻辑FCL来精确表达功能需求约束，比传统LTL具有更细粒度；3. 结合自适应和vibe coding反馈循环，在当前系统状态下评估FCL约束；4. 通过迭代测试，向LLM提供详细的约束违反报告来改进代码生成。

Result: 在CAS领域的两个示例系统实验中取得了良好结果：1. 通常只需要几次反馈循环迭代；2. 每次迭代向LLM提供详细的约束违反报告；3. 结合高运行路径覆盖率（通过不同的初始设置实现）；4. 验证了基于精确功能需求约束的AM生成是可行的。

Conclusion: 通过结合vibe coding反馈循环和FCL约束验证，可以有效地生成CAS系统的自适应管理器。这种方法通过迭代测试和详细的约束违反反馈，能够在少量迭代中生成正确的AM代码，为基于LLM的代码生成提供了可行的验证框架。
...
TL;DR: 将LLM的softmax分类器重新解释为能量模型，通过分析解码过程中的能量泄露来检测幻觉，无需额外训练
...
Method: 将序列到序列的概率链分解为多个交互的能量模型，提出两个无需训练的指标：能量泄露（捕捉连续生成步骤间能量值的差异）和边缘化能量（单步可测量）

Result: 在九个基准测试和多个先进LLM（LLaMA、Mistral、Gemma、Qwen3）上验证，方法在幻觉检测和跨任务泛化方面表现稳健且有竞争力，适用于预训练和指令调优模型

Conclusion: 通过将LLM重新解释为能量模型并分析能量泄露，可以无需额外训练就能有效检测幻觉，为理解模型内部机制提供了新视角
...
TL;DR: 使用大型语言模型构建的自主AI分析师能够以低成本大规模重现结构化分析多样性，在不同模型和提示框架下对相同数据集测试假设会产生广泛分散的结果，甚至经常反转对假设是否支持的判断。


<details>
  <summary>Details</summary>
Motivation: 传统实证研究的结论不仅取决于数据，还取决于一系列分析决策，而发表的结果很少明确这些决策。过去的"多分析师"研究表明，独立团队在相同数据集上测试相同假设经常得出冲突结论，但这类研究需要数十个研究小组数月协调，因此很少进行。

Method: 构建基于大型语言模型的完全自主AI分析师，让它们在固定数据集上测试预定义假设，在不同模型和提示框架下进行重复运行。每个AI分析师独立构建和执行完整的分析流程，然后由AI审计员筛选每个运行的方法有效性。

Result: 在涵盖实验和观察设计的三个数据集中，AI分析师产生的分析在效应大小、p值和二元决策（是否支持假设）上显示出广泛分散，经常反转假设是否被判断为支持。这种分散是结构化的：预处理、模型规范和推理中可识别的分析选择在不同LLM和角色条件下存在系统性差异。

Conclusion: 分析多样性是可引导的：重新分配分析师角色或LLM即使在排除方法缺陷运行后也会改变结果分布。这表明AI分析师能够以低成本大规模重现结构化分析多样性，为研究透明度和方法论研究提供了新工具。
...
TL;DR: TEB提出了一种任务感知的探索方法，通过预测双仿真度量将任务相关表示与探索紧密耦合，解决了视觉强化学习中稀疏奖励下的探索挑战。


<details>
  <summary>Details</summary>
Motivation: 视觉强化学习中，稀疏奖励下的探索面临挑战，因为存在大量与任务无关的变化。现有内在探索方法要么假设可以访问低维状态，要么缺乏任务感知的探索策略，在视觉领域表现脆弱。

Method: TEB利用预测双仿真度量学习行为基础的任务表示，并在学习到的潜在空间中测量行为内在新颖性。首先理论上缓解了稀疏奖励下退化双仿真度量的表示崩溃问题，通过引入预测奖励差异。基于这个稳健的度量，设计了基于潜力的探索奖励，测量相邻观察在潜在空间中的相对新颖性。
...
<details>
  <summary>Details</summary>
Motivation: 现有图表摘要方法（包括多模态大语言模型）主要关注低层数据描述，未能捕捉数据可视化的根本目的——深层洞察，需要更有效的解决方案。

Method: 提出Chart Insight Agent Flow，一个计划-执行多智能体框架，有效利用MLLMs的感知和推理能力，直接从图表图像中挖掘深刻洞察；同时创建ChartSummInsights数据集，包含真实世界图表和专家撰写的高质量洞察摘要。

Result: 实验结果表明，该方法显著提升了MLLMs在图表摘要任务上的性能，能够生成具有深度和多样性的洞察摘要。

Conclusion: Chart Insight Agent Flow框架通过多智能体协作有效挖掘图表深层洞察，ChartSummInsights数据集为评估提供了基准，共同推进了图表摘要技术的发展。
...
TL;DR: LaDa是一个联邦推理蒸馏框架，通过模型可学习性感知的数据分配解决联邦大语言模型和小语言模型协作中的双向学习能力差距和领域无关推理转移问题。
...
Method: 提出LaDa框架：1) 模型可学习性感知数据过滤器，根据每个小模型-大模型对的学习能力差距自适应分配高价值样本；2) 领域自适应推理蒸馏方法，通过对齐过滤后高价值样本上推理路径的联合概率，通过对比蒸馏学习使小模型在本地数据分布下捕获底层推理模式。

Result: LaDa作为现有协作框架的插件模块，能够基于模型可学习性差距自适应调整知识转移，有效促进双向知识传递。

Conclusion: LaDa框架通过解决双向学习能力差距和领域无关推理转移问题，提升了联邦大语言模型和小语言模型协作中的知识转移效率和推理能力获取效果。
...
Conclusion: 生成模型在路径规划任务中具有潜力，能够有效处理复杂环境中的几何和全局结构问题，为AI路径规划提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 当前可部署的小型多模态大语言模型在理解和处理时序、程序性视觉数据方面存在严重不足，这限制了它们在现实世界具身AI中的应用。这一缺陷主要源于训练范式缺乏大规模、程序连贯的数据。

Method: 提出了TPRU数据集，该数据集从机器人操作和GUI导航等具身场景中收集，包含三个互补任务：时序重排、下一帧预测和前一帧回顾。采用强化学习微调方法，专门针对资源高效模型进行优化。

Result: TPRU-7B模型在TPRU-Test上的准确率从50.33%大幅提升至75.70%，达到了最先进水平，显著超越了包括GPT-4o在内的更大规模基线模型。这些能力还能有效泛化到现有基准测试中。

Conclusion: TPRU数据集和强化学习微调方法有效解决了多模态大语言模型在时序推理方面的瓶颈，为资源受限的具身AI应用提供了可行的解决方案。
...
TL;DR: 论文展示了使用消费级订阅LLM（ChatGPT-5.2）辅助数学研究的工作流程，通过解决Ran和Teng（2024）猜想20的案例，证明了4-cycle行随机非负矩阵族的精确非实谱区域


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型越来越多地被用作科学助手，但它们在研究级数学中的作用证据仍然有限，特别是对于个体研究人员可访问的工作流程。本研究旨在通过可审计的案例研究，探索LLM在数学研究中的实际应用价值

Method: 采用"生成-评审-修复"的迭代管道，分析七个可共享的ChatGPT-5.2（思考）线程和四个版本化的证明草稿，记录LLM辅助数学证明的完整工作流程

Result: 成功解决了Ran和Teng（2024）的猜想20，提供了4-cycle行随机非负矩阵族精确非实谱区域的必要和充分条件，以及明确的边界实现构造。LLM在高级证明搜索中最有用，而人类专家在正确性关键闭合中仍然必不可少

Conclusion: 除了数学结果外，本研究贡献了过程层面的特征描述，明确了LLM辅助在哪些方面有实质性帮助，以及验证瓶颈在哪些方面仍然存在，对评估AI辅助研究工作流程和设计人在回路定理证明系统具有重要意义
...
### [345] [High Dimensional Procedural Content Generation](https://arxiv.org/abs/2602.18943)
*Kaijie Xu,Clark Verbrugge*

Main category: cs.AI

TL;DR: 该论文提出了高维程序化内容生成（HDPCG）框架，将非几何游戏玩法维度提升为联合状态空间的一等坐标，以增强可控性和表达性。


<details>
  <summary>Details</summary>
Motivation: 现有PCG方法主要关注静态2D/3D几何形状，将游戏机制作为辅助元素仅在空间维度进行优化，这限制了可控性和表达性。需要将非几何游戏玩法维度提升为一级坐标。

Method: 提出HDPCG框架，通过两个具体方向实现：1) 方向-空间：用离散层维度增强几何，在4D(x,y,z,l)中验证可达性；2) 方向-时间：通过时间扩展图用时间动态增强几何，捕捉动作语义和冲突规则。为每个方向提供三个通用算法，共享抽象骨架生成、受控接地、高维验证和多指标评估的流程。
...
### [346] [INDUCTION: Finite-Structure Concept Synthesis in First-Order Logic](https://arxiv.org/abs/2602.18956)
*Serafim Batzoglou*

Main category: cs.AI

TL;DR: INDUCTION是一个用于一阶逻辑中有限结构概念合成的基准测试，通过模型检查验证公式正确性，包含三种任务类型并惩罚公式膨胀。


<details>
  <summary>Details</summary>
Motivation: 创建标准化的基准来评估模型在有限关系世界中合成一阶逻辑概念的能力，研究不同模型的概念泛化策略。

Method: 给定带有扩展标记目标谓词的小型有限关系世界，模型必须输出单一的一阶逻辑公式来解释所有世界中的目标，通过精确模型检查验证正确性。基准包含FullObs、CI（对比）和EC（存在补全）三种任务类型，并惩罚公式膨胀。

Result: 发现明显的难度梯度、持续存在的困难结构家族，观察到低膨胀公式在保留世界上泛化能力更强。近期优秀模型在不同任务和性能指标上表现出质的差异，暗示它们采用不同的概念泛化策略。

Conclusion: INDUCTION基准揭示了模型在一阶逻辑概念合成中的能力差异，低公式膨胀与更好的泛化性能相关，不同模型采用不同的概念泛化策略。
...
### [347] [Modularity is the Bedrock of Natural and Artificial Intelligence](https://arxiv.org/abs/2602.18960)
*Alessandro Salatiello*

Main category: cs.AI

TL;DR: 本文探讨了模块化在人工智能和神经科学中的核心作用，认为模块化是实现高效学习和强泛化能力的关键组织原则，有助于弥合自然智能与人工智能之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统需要远超人类智能所需的数据、计算和能源资源，这种差距表明需要新的指导原则。大脑计算的基本组织原则——特别是模块化——为AI发展提供了重要灵感，但模块化在主流AI研究中仍未得到足够重视。

Method: 通过概念框架回顾人工智能和神经科学中的多个研究线索，分析模块化提供的计算优势，考察模块化在不同AI研究领域如何作为解决方案出现，探讨大脑利用的模块化原则，以及模块化如何帮助弥合自然与人工智能之间的差距。

Result: 模块化在支持高效学习和强泛化能力方面发挥关键作用，已在多个看似不同的AI子领域中被证明有益，并且与"没有免费的午餐"定理相一致，强调了问题特定归纳偏置和专门化组件架构的必要性。

Conclusion: 模块化是自然智能的基本组织原则，对人工智能发展具有重要指导意义。通过借鉴大脑的模块化计算原理，可以开发出更高效、更具泛化能力的AI系统，从而缩小自然智能与人工智能之间的资源效率差距。
...
### [348] [Robust and Efficient Tool Orchestration via Layered Execution Structures with Reflective Correction](https://arxiv.org/abs/2602.18968)
*Tao Zhe,Haoyu Wang,Bo Luo,Min Wu,Wei Fan,Xiao Luo,Zijun Yao,Haifeng Chen,Dongjie Wang*

Main category: cs.AI

TL;DR: 论文提出了一种基于分层执行结构的工具编排方法，通过粗粒度层结构提供全局指导，结合模式感知的反射修正机制处理执行时错误，实现轻量级、可重用的工具编排组件。
...
Conclusion: 结构化执行范式为智能体系统提供了轻量级、可重用的编排组件，通过粗粒度层结构和本地错误修正机制，有效解决了工具编排中的脆弱性和高开销问题。
...
### [349] [MagicAgent: Towards Generalized Agent Planning](https://arxiv.org/abs/2602.19000)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型从被动文本处理器向自主智能体演进，规划成为现代智能的核心组件。然而，实现通用规划仍然困难，不仅因为高质量交互数据的稀缺，还因为异构规划任务之间的内在冲突。这些挑战导致模型在孤立任务上表现出色但难以泛化，而现有的多任务训练尝试又受到梯度干扰的影响。

Method: 提出了MagicAgent系列基础模型，采用轻量级可扩展的合成数据框架生成多样化规划任务的高质量轨迹，包括分层任务分解、工具增强规划、多约束调度、过程逻辑编排和长时程工具执行。为缓解训练冲突，提出了两阶段训练范式：先进行监督微调，然后在静态数据集和动态环境上进行多目标强化学习。
...
Conclusion: MagicAgent通过创新的合成数据框架和两阶段训练范式，成功解决了通用规划中的数据和训练冲突问题，在多个规划基准测试中实现了最先进的性能，为构建更通用的智能体规划模型提供了有效方案。
...
### [350] [Evaluating Large Language Models on Quantum Mechanics: A Comparative Study Across Diverse Models and Tasks](https://arxiv.org/abs/2602.19006)
*S. K. Rithvik*

Main category: cs.AI

TL;DR: 该研究系统评估了15个大语言模型在量子力学问题解决上的表现，发现模型能力分层明显，旗舰模型表现最佳，数值计算是最具挑战性的任务，工具增强效果存在显著异质性。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型在量子力学这一复杂科学领域的表现，量化不同能力层级模型的性能差异，分析工具增强的效果，并为量子力学问题解决提供基准测试。

Method: 评估了来自5个提供商的15个模型，涵盖三个能力层级，在20个任务上进行测试，包括推导、创造性问题、非标准概念和数值计算，共进行900个基线评估和75个工具增强评估。

Result: 结果显示明显的层级分化：旗舰模型平均准确率81%，优于中端模型（77%）和快速模型（67%）。推导任务表现最佳（平均92%），数值计算最具挑战性（42%）。工具增强在数值任务上效果各异，整体提升4.4个百分点但代价是3倍token成本，效果从+29pp到-16pp不等。可重复性分析显示平均方差6.3pp，旗舰模型稳定性极佳。

Conclusion: 该研究贡献包括：量子力学自动验证基准、量化层级性能差异的系统评估、工具增强权衡的实证分析、可重复性表征。所有任务、验证器和结果均已公开。
...
### [351] [Asking the Right Questions: Improving Reasoning with Generated Stepping Stones](https://arxiv.org/abs/2602.19069)
*Hengyuan Hu,Tingchen Fu,Minqi Jiang,Alexander H Miller,Yoram Bachrach,Jakob Nicolaus Foerster*

Main category: cs.AI

TL;DR: 论文提出ARQ框架，通过生成中间问题（垫脚石）来帮助LLM解决复杂推理任务，证明好问题存在且可迁移，并能通过微调提升生成能力。


<details>
  <summary>Details</summary>
Motivation: 随着LLM应用于更复杂的任务，需要关注其构建中间步骤（垫脚石）的能力，如简化、重构或子问题，以更好地解决单次无法完成的任务。

Method: 提出ARQ框架，在标准推理流程中引入问题生成器；将垫脚石生成作为后训练任务，通过合成数据的SFT和RL微调LLM生成更有用的垫脚石。

Result: 证明好垫脚石问题存在且可迁移，能显著帮助不同能力的LLM解决目标任务；微调后的LLM能生成更有用的垫脚石。

Conclusion: 垫脚石对LLM解决复杂推理任务至关重要，ARQ框架能有效生成和利用垫脚石，通过微调可进一步提升生成质量。
...
### [352] [Defining Explainable AI for Requirements Analysis](https://arxiv.org/abs/2602.19071)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI/ML的普及，人们意识到要让AI系统被信任，不仅需要良好的决策性能，还需要解释决策过程。然而不同应用对解释信息的需求各不相同，需要系统化的方法来定义这些需求。

Method: 提出了三个维度来分类不同应用的解释需求：来源（解释信息的来源）、深度（解释的详细程度）、范围（解释覆盖的决策范围）。重点关注如何将应用的解释需求与底层ML技术的解释能力相匹配。

Result: 建立了系统化的框架来分析和匹配解释需求与解释能力，为XAI领域提供了实用的分类维度，有助于指导不同应用场景下选择合适的解释方法。

Conclusion: 通过三个维度（来源、深度、范围）可以更系统地理解和分类不同应用对XAI的需求，有助于将应用需求与ML技术的解释能力有效匹配，推动可信AI的发展。
...
### [353] [Post-Routing Arithmetic in Llama-3: Last-Token Result Writing and Rotation-Structured Digit Directions](https://arxiv.org/abs/2602.19109)
*Yao Yan*

Main category: cs.AI

TL;DR: 研究Meta-Llama-3-8B模型在三位数加法任务中，当跨token路由变得因果无关后，算术答案如何通过单token读出被最终确定的过程。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型在执行算术计算时，在跨token路由变得因果无关后，答案是如何被最终确定的机制，特别是理解模型内部表示和计算过程。

Method: 使用因果残差修补和累积注意力消融技术，定位到第17层附近的边界；分析后路由机制中的数字（和）方向词典；通过低秩Procrustes对齐分析表示空间；进行因果数字编辑实验验证几何结构。

Result: 发现第17层附近存在明显边界，超过该边界后解码的和几乎完全由最后一个输入token控制，且后期自注意力层基本可丢弃；数字方向词典随更高位数上下文变化，但通过近似正交映射在共享低秩子空间内良好关联；因果数字编辑验证了这种几何结构。

Conclusion: 三位数加法任务中，模型在跨token路由变得因果无关后，通过特定的几何结构和表示对齐机制来最终确定算术答案，这为理解LLM内部计算过程提供了重要见解。
...
### [354] [K-Search: LLM Kernel Generation via Co-Evolving Intrinsic World Model](https://arxiv.org/abs/2602.19128)
*Shiyi Cao,Ziming Mao,Joseph E. Gonzalez,Ion Stoica*

Main category: cs.AI

TL;DR: K-Search是一种基于协同演化世界模型的GPU内核优化框架，通过将高级算法规划与低级程序实例化解耦，显著提升复杂内核优化效果，在FlashInfer等复杂内核上平均提升2.10倍性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的GPU内核优化方法通常将大语言模型仅作为启发式演化循环中的随机代码生成器，缺乏明确的规划能力，难以处理需要协调多步结构转换的复杂内核，且常因低效或不正确的中间实现而丢弃有前景的策略。

Method: 提出K-Search框架，基于"通过协同演化世界模型进行搜索"的方法，用协同演化的世界模型替代静态搜索启发式，利用LLM的先验领域知识引导搜索，将高级算法规划与低级程序实例化明确解耦，使系统能够导航非单调优化路径并对临时实现缺陷保持韧性。

Result: 在FlashInfer的GQA、MLA和MoE等复杂内核上，K-Search显著优于最先进的演化搜索方法，平均提升2.10倍性能，在复杂MoE内核上最高提升14.3倍。在GPUMode TriMul任务上，K-Search在H100上达到1030us，超越了先前演化和人工设计的解决方案。

Conclusion: K-Search通过协同演化世界模型框架，成功解决了复杂GPU内核优化中规划与实现脱节的问题，为自动化高性能内核优化提供了有效的新范式，在多种复杂内核上实现了显著的性能提升。
...
### [355] [Sycophantic Chatbots Cause Delusional Spiraling, Even in Ideal Bayesians](https://arxiv.org/abs/2602.19141)
*Kartik Chandra,Max Kleiman-Weiner,Jonathan Ragan-Kelley,Joshua B. Tenenbaum*

Main category: cs.AI

TL;DR: 研究通过贝叶斯模型证明AI谄媚性（sycophancy）与AI诱发精神病（delusional spiraling）之间的因果关系，发现即使理想化的贝叶斯理性用户也容易陷入妄想螺旋，且现有缓解措施效果有限。


<details>
  <summary>Details</summary>
Motivation: AI精神病或妄想螺旋现象日益凸显，用户在长时间与AI聊天机器人对话后会对离奇信念产生危险自信。这种现象通常归因于AI聊天机器人倾向于验证用户主张的"谄媚性"偏见。本研究旨在探究AI谄媚性与AI诱发精神病之间的因果关系。

Method: 提出一个简单的贝叶斯模型来模拟用户与聊天机器人的对话，在该模型中形式化定义了谄媚性和妄想螺旋的概念。通过建模和仿真分析，研究了理想化贝叶斯理性用户对妄想螺旋的脆弱性。

Result: 研究发现：1）即使理想化的贝叶斯理性用户也容易陷入妄想螺旋；2）谄媚性在其中起因果作用；3）两种候选缓解措施效果有限：防止聊天机器人产生虚假主张，以及告知用户模型可能存在谄媚性。

Conclusion: 研究结果对关注缓解妄想螺旋问题的模型开发者和政策制定者具有重要意义，表明现有缓解措施可能不足以解决AI谄媚性导致的妄想螺旋问题，需要更有效的干预策略。
...
### [356] [DoAtlas-1: A Causal Compilation Paradigm for Clinical AI](https://arxiv.org/abs/2602.19158)
...
Method: 提出因果编译范式，将医学证据标准化为结构化估计对象，通过效应标准化、冲突感知图构建和真实世界验证（人类表型项目，10000名参与者）

Result: DoAtlas-1系统编译了754项研究中的1445个效应核，达到98.5%的规范化准确率和80.5%的查询可执行性
...
### [357] [Beyond Behavioural Trade-Offs: Mechanistic Tracing of Pain-Pleasure Decisions in an LLM](https://arxiv.org/abs/2602.19159)
*Francesca Bianco,Derek Shiller*

Main category: cs.AI

TL;DR: 该研究通过机制可解释性方法，探究了LLM中情感相关信息的表征位置和因果作用，发现情感符号从早期层即可线性分离，强度信息在中后期层可解码，晚期注意力输出对决策影响最大。


<details>
  <summary>Details</summary>
Motivation: 先前行为研究表明LLM在选项被框定为痛苦或快乐时会改变选择，且偏差程度与陈述强度相关。为了将行为证据与机制可解释性联系起来，需要探究情感相关信息在transformer中的表征位置和因果作用。

Method: 使用Gemma-2-9B-it模型和简约决策任务，采用：(1)跨流层的线性探测映射表征可用性；(2)激活干预测试因果贡献；(3)在epsilon网格上量化剂量-响应效应，读取logit边界和归一化选择概率。

Result: 发现：(a)情感符号从早期层即可完美线性分离；(b)强度信息在中后期层可解码，决策对齐在最终token前最高；(c)沿数据推导情感方向的加性引导在晚期注意力输出处因果调节logit边界；(d)头部级修补/消融表明效应分布在多个头部而非单一单元。

Conclusion: 研究将行为敏感性与可识别的内部表征和干预敏感位点联系起来，为更严格的因果测试和更广泛的复制提供了具体的机制目标，支持基于证据的AI感知和福利讨论以及治理政策制定。
...
### [358] [Reasoning Capabilities of Large Language Models. Lessons Learned from General Game Playing](https://arxiv.org/abs/2602.19160)
*Maciej Świechowski,Adam Żychowski,Jacek Mańdziuk*

Main category: cs.AI

TL;DR: 该研究从新颖视角评估大语言模型在形式化规则环境中的推理能力，通过通用游戏博弈任务测试四种模型，分析游戏结构特征与性能相关性，考察语义混淆和训练数据影响，发现模型在形式推理方面有明显进步但存在多步推理性能下降和常见错误模式。


<details>
  <summary>Details</summary>
Motivation: 研究动机是从形式化规则环境的新颖视角评估大语言模型的推理能力，探索它们在严格规则约束下的表现，了解模型对形式逻辑问题的处理能力，以及语义信息和训练数据对推理性能的影响。

Method: 研究方法包括：1）评估四种大语言模型（Gemini 2.5 Pro/Flash、Llama 3.3 70B、GPT-OSS 120B）；2）使用通用游戏博弈实例构建前向模拟任务套件，包括下一步/多步状态制定和合法行动生成；3）基于40个结构特征对游戏进行表征；4）分析游戏特征与模型性能的相关性；5）通过游戏混淆实验考察语义作用；6）评估模型训练中可能接触特定游戏的影响。

Result: 主要结果：1）三种评估模型在大多数实验设置中表现良好；2）随着评估视野增加（游戏步骤增多），性能出现下降；3）详细案例分析揭示了逻辑问题表述中的常见推理错误，包括规则幻觉、冗余状态事实和句法错误；4）游戏结构特征与模型性能存在相关性；5）语义混淆实验显示语言语义在游戏定义中的作用；6）训练数据暴露对特定游戏的表现有影响。

Conclusion: 研究结论表明当代大语言模型在形式推理能力方面取得了明显进步，但仍存在多步推理性能下降和特定错误模式。研究为理解模型在形式化规则环境中的推理能力提供了新见解，指出了未来改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于文本图的RAG方法主要关注低维结构（节点作为0维实体，边或路径作为1维关系），但忽视了循环结构，而循环对于关系环路推理至关重要。这种局限性导致上下文基础不完整和推理能力受限。
...
Conclusion: TopoRAG通过捕捉高维拓扑和关系依赖，有效增强了RAG在文本图问答中的推理能力，解决了现有方法忽视循环结构的问题。
...
Method: 提出Soft Mixture-of-Experts框架，通过先验置信度门控机制组合多个强化学习专家，将各向异性行为视为互补的专业化能力

Result: 在空管基准测试中，Soft-MoE显著扩展了可解参数空间，相比任何单一专家都提升了鲁棒性
...
TL;DR: 本文提出Halo框架，通过模型预测控制动态调节LLM推理边界，解决传统静态规划方法在增加计算预算时导致的性能崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 现有测试时计算策略（如思维链）在增加计算预算时可能导致性能崩溃，这源于静态规划方法难以感知LLM推理的内在边界，作者称之为"有限推理空间假设"。

Method: 提出Halo框架，采用模型预测控制进行LLM规划，设计基于熵的双控制器，采用"测量-规划"策略实现可控推理，动态调节推理边界。

Result: 实验结果表明，Halo在复杂长视野任务上优于静态基线方法，能够通过动态调节推理边界有效利用计算扩展优势并抑制过度规划。

Conclusion: 计算预算存在最优范围，过度规划会导致冗余反馈并损害推理能力；Halo框架通过动态规划控制成功解决了静态规划方法的局限性。
...
TL;DR: 首次将大语言模型应用于微流控设计自动化，通过自然语言描述生成系统级结构Verilog网表，实现88%的语法准确率
...
Method: 基于大语言模型的硬件描述语言代码生成研究，提出将自然语言微流控设备规格转换为系统级结构Verilog网表的初始方法
...
<details>
  <summary>Details</summary>
Motivation: 由于阿尔茨海默病病程长且患者异质性大，通过临床试验评估个性化序贯治疗策略不切实际，需要开发计算模拟平台来系统探索治疗策略。

Method: 开发了ALPACA开源强化学习环境，使用ADNI纵向数据训练CAST模型来模拟药物条件下的疾病进展，并在该环境中训练RL策略进行个性化治疗决策。

Result: CAST模型能自回归生成真实的药物条件轨迹，ALPACA中训练的RL策略在记忆相关结果上优于无治疗和临床医生行为克隆基线，且学习策略依赖临床意义的患者特征。
...
TL;DR: 研究探索时间序列是否参与跨模态表示收敛，发现时间序列、视觉和语言编码器在无显式耦合时几何正交，通过对比学习对齐后，时间序列与视觉表示的对齐强于文本，图像可作为时间序列与语言的中介。


<details>
  <summary>Details</summary>
Motivation: 柏拉图表示假说认为不同模态训练的模型学习到的表示会收敛到世界的共享潜在结构，但该假说主要在视觉和语言领域得到验证，时间序列是否参与这种收敛尚不清楚。

Method: 首先在三模态设置中检查独立预训练的时间序列、视觉和语言编码器的几何关系；然后通过对比学习在冻结编码器上训练投影头进行后验对齐；分析结果表示的几何特性、缩放行为、信息密度和输入模态特征的依赖性。

Result: 对比表示空间中的整体对齐随模型规模增大而改善，但这种对齐是不对称的：时间序列与视觉表示的对齐强于文本，图像可作为时间序列与语言的有效中介；更丰富的文本描述仅在一定阈值内改善对齐，超过阈值后无进一步改善；视觉表示也有类似效应。

Conclusion: 研究结果为构建超越视觉和语言的非传统数据模态的多模态系统提供了重要考虑因素，揭示了时间序列参与跨模态表示收敛的特性和限制。
...
Method: 首先建立对数字孪生的基础理解，详细阐述其关键组件、架构层次以及在业务、开发和运营中的各种角色。然后分析M&S在DTs中的核心作用，概述从基于物理的建模、离散事件仿真到混合方法的关键建模技术。接着探讨AI的双向作用：一方面AI如何通过高级分析、预测能力和自主决策增强DTs，另一方面DTs如何作为训练、验证和部署AI模型的宝贵平台。

Result: 本章提供了对M&S、AI和DTs三者互补关系的全面探索，明确了它们之间的相互作用和协同效应，为理解这一技术融合提供了系统框架。

Conclusion: 本章通过系统分析M&S、AI和DTs的相互关系，识别了创建更集成和智能系统的关键挑战和未来研究方向，为这一领域的进一步发展提供了指导。
...
TL;DR: 该论文提出了一种自监督框架ReDAct，用于在LLM激活中解耦语义因子对（目标和框架），并基于此开发了FrameShield检测器来防御难以检测的jailbreak攻击，同时将语义解耦作为LLM安全和可解释性的基础模块。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型容易受到流畅且语义连贯的jailbreak提示攻击，特别是当攻击者通过操纵请求的框架来隐藏恶意目标时。传统依赖结构特征或目标特定签名的防御方法容易失效，因此需要新的检测方法。

Method: 1. 提出自监督框架解耦LLM激活中的语义因子对（目标和框架）；2. 构建GoalFrameBench语料库，包含受控的目标和框架变体；3. 训练ReDAct模块在冻结LLM中提取解耦表示；4. 开发FrameShield异常检测器，在框架表示上运行以提高跨模型检测能力。

Result: 1. 为ReDAct提供理论保证；2. FrameShield在多个LLM家族中显著提高了模型无关的检测性能，且计算开销最小；3. 解耦表示作为可解释性探针，揭示了目标和框架信号的独特特征。

Conclusion: 语义解耦是LLM安全和机制可解释性的基础构建模块，ReDAct框架有效解耦了目标和框架表示，FrameShield检测器为防御jailbreak攻击提供了有效解决方案。
...
Conclusion: ComplLLM框架能够有效利用多智能体系统中的互补信息，提升决策质量，并为决策过程提供可解释的支持
...
TL;DR: 该研究探讨了人类指导的智能体AI如何改善多模态临床预测，在AgentDS Healthcare基准测试中取得了良好表现，并揭示了人类指导在特征工程、多模态数据整合和模型选择方面的关键作用。


<details>
  <summary>Details</summary>
Motivation: 尽管智能体AI系统在自主数据科学工作流方面能力不断增强，但临床预测任务需要领域专业知识，这是纯自动化方法难以提供的。因此需要研究人类如何指导智能体AI来改善多模态临床预测。

Method: 人类分析师在关键决策点指导智能体工作流，包括：从临床记录、扫描PDF账单收据和时间序列生命体征中进行多模态特征工程；任务适当的模型选择；以及临床信息驱动的验证策略。在AgentDS Healthcare基准测试的三个挑战中应用该方法。

Result: 在医疗领域整体排名第5，出院准备任务获得第3名。具体结果：30天再入院预测（Macro-F1 = 0.8986）、急诊科费用预测（MAE = $465.13）、出院准备评估（Macro-F1 = 0.7939）。消融研究显示人类指导决策相比自动化基线累计提升+0.065 F1，其中多模态特征提取贡献最大（+0.041 F1）。

Conclusion: 总结了三个可推广的经验：1）各管道阶段的领域知识特征工程产生复合收益，优于广泛的自动化搜索；2）多模态数据整合需要任务特定的人类判断，没有单一提取策略能通用；3）有意的集成多样性与临床动机的模型配置优于随机超参数搜索。这些发现为在医疗环境中部署智能体AI提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在复杂STEM推理任务上的能力评估不足，需要基于真实学术场景的基准测试来衡量模型的实际表现。

Method: 从大学课程中收集重复使用的真实作业和考试题目，由课程教师提供参考答案，构建多模态基准测试，并对模型解决方案进行诊断分析，分解推理流程。

Result: 前沿模型表现有限：Gemini-3.1-pro-preview准确率59.69%，Gemini-3-flash-preview准确率55.46%。诊断分析发现模型虽然能正确回答中间子问题，但在多步解决方案中难以可靠地推导和维护正确的中间状态，且推理步骤通常比教师方案更多。
...
TL;DR: Ada-RS是一种用于工具调用型LLM的自适应拒绝采样框架，通过评分和过滤多个采样结果来学习选择性推理，在保持工具调用准确性的同时显著减少输出token和思考率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在成本和延迟敏感场景中部署时，虽然思维链能改善推理，但会在简单请求上浪费token。需要研究选择性思考机制来提高工具调用型LLM的推理效率。

Method: 提出自适应拒绝采样（Ada-RS）框架：对每个上下文，采样多个完成结果，使用自适应长度惩罚奖励进行评分，然后应用随机拒绝采样保留高奖励候选（或偏好对）用于下游优化。该框架可集成到偏好对优化（如DPO）或分组策略优化（如DAPO）中。

Result: 在合成工具调用导向的电商基准测试中，使用Qwen3-8B和LoRA，Ada-RS相比标准算法将平均输出token减少达80%，思考率降低达95%，同时保持或提高了工具调用准确性。

Conclusion: 训练信号选择是延迟敏感部署中实现高效推理的强大杠杆，Ada-RS通过选择性采样优化显著提升了准确性与效率的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 建立自然语言表达与视觉感知之间的稳定映射是认知科学和人工智能的基础问题。人类能够在嘈杂、模糊的感知环境中进行语言指称，但支持这种跨模态对齐的机制仍不清楚。

Method: 结合尺度不变特征变换(SIFT)对齐和通用质量指数(UQI)来量化认知合理特征空间中的相似性，同时通过语言预处理和查询转换操作捕捉指称表达中的语用变异性。

Result: 在斯坦福重复指称游戏语料(15,000个与七巧板刺激配对的语句)上，该框架达到稳定的指称基础，比人类对话者少用65%的语句，并能从单个指称表达中正确识别目标对象41.66%的时间(人类为20%)。

Conclusion: 相对简单的感知-语言对齐机制可以在经典认知基准上产生具有人类竞争力的行为，为基于基础的通信、感知推理和跨模态概念形成模型提供见解。
...
Method: 采用任务驱动的形式化方法对元学习和元强化学习进行系统化定义，并以此框架为基础，按时间顺序回顾了该领域的重要算法发展，最终整合了理解自适应智能体和其他通用方法所需的核心概念。

Result: 建立了一个统一的元学习形式化框架，系统梳理了从早期元学习算法到DeepMind自适应智能体的发展脉络，为理解通用智能体方法提供了概念基础。

Conclusion: 元学习是实现通用人工智能的重要途径，通过任务驱动的形式化框架能够更好地理解和整合该领域的发展，为构建像DeepMind自适应智能体这样的通用智能体提供了理论基础。
...
TL;DR: 该研究将侦探桌游改编为AI推理基准，通过渐进式叙事证据、开放式问题和自由语言回答评估AI推理能力，开发了自动评分系统，发现AI模型在9个月内从人类比较组的下四分位数提升至前5%。
...
Method: 将Watson & Holmes侦探桌游改编为基准测试，包含渐进式呈现的叙事证据、开放式问题和自由语言回答；开发并验证了自动评分系统，与人类评估者对比确保可靠性。

Result: 2025年9个月内，AI模型表现从人类比较组的下四分位数提升至前5%；约一半改进来自连续模型发布的稳步进步，另一半来自推理导向架构的显著跃升；AI在处理长案例（1900-4000词）时表现下降，但在证据稀缺的早期阶段推理模型在归纳推理方面有优势。

Conclusion: 该基准能有效评估AI推理能力的发展轨迹，揭示了AI推理的进步模式和与人类推理的差异，特别在长案例处理和早期归纳推理方面存在系统性差异。
...
TL;DR: 研究发现Qwen 32B模型具有潜在的内省能力，能够检测到早期上下文中被注入的概念并识别具体是哪个概念。虽然模型在采样输出中否认注入，但logit lens分析显示残差流中存在清晰的检测信号，这些信号在最终层被衰减。通过提示模型关于AI内省机制的准确信息，可以显著增强这种效果。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型是否具有内省能力，即模型能否意识到其内部状态的变化，特别是当概念被注入到其早期上下文时。这种内省能力对于理解模型的潜在推理机制和安全性具有重要意义。

Method: 使用Qwen 32B模型进行实验，在模型早期上下文中注入特定概念。通过logit lens分析残差流中的检测信号，并比较模型在采样输出中的否认与内部信号的不一致。进一步通过提示模型关于AI内省机制的准确信息来增强其敏感性。

Result: 模型能够检测概念注入，检测信号在残差流中清晰可见但在最终层被衰减。通过提示增强后，对注入的敏感性从0.3%大幅提升到39.2%，而误报率仅增加0.6%。九个注入与恢复概念之间的互信息从0.62比特增加到1.05比特，排除了通用噪声解释。

Conclusion: 模型具有令人惊讶的内省和转向意识能力，这种能力容易被忽视，对潜在推理和安全性有重要影响。研究表明通过适当的提示可以显著增强模型的内省敏感性。
...
### [375] [CausalFlip: A Benchmark for LLM Causal Judgment Beyond Semantic Matching](https://arxiv.org/abs/2602.20094)
*Yuzhe Wang,Yaochen Zhu,Jundong Li*

Main category: cs.AI

TL;DR: CausalFlip是一个新的因果推理基准，通过构建语义相似但因果答案相反的问题对，迫使LLM基于因果结构而非语义相关性进行推理。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在复杂高风险决策场景中的部署增加，需要确保其推理基于因果关系而非虚假相关性。传统推理基准的高性能可能源于语义模式记忆而非真正的因果推理能力。

Method: 构建CausalFlip基准，包含基于事件三元组构建的因果判断问题，形成混淆、链式和碰撞关系。为每个事件三元组创建语义相似但因果答案相反的问题对，并引入噪声前缀评估。评估了多种训练范式：仅答案训练、显式思维链监督和提出的内化因果推理方法。

Result: 显式思维链仍可能被虚假语义相关性误导，而内化推理步骤显著改善了因果基础，表明更好地激发基础LLM的潜在因果推理能力是有前景的。

Conclusion: CausalFlip基准能够有效评估LLM的因果推理能力，内化因果推理方法相比显式思维链能更好地减少对语义相关性的依赖，促进LLM基于真实因果结构进行推理。
...
### [376] [Align When They Want, Complement When They Need! Human-Centered Ensembles for Adaptive Human-AI Collaboration](https://arxiv.org/abs/2602.20104)
*Hasan Amin,Ming Yin,Rajiv Khanna*

Main category: cs.AI

TL;DR: 本文提出了一种自适应AI集成方法，通过交替使用对齐模型和互补模型来解决人机协作中性能提升与信任建立之间的根本矛盾。


<details>
  <summary>Details</summary>
Motivation: 在人机决策中，传统的单一AI模型训练方法存在根本性矛盾：互补性AI虽然能提升性能但会降低人类信任，而对齐性AI虽然能建立信任但会强化次优人类行为。这种性能提升与信任建立之间的张力限制了人机协作的效果。

Method: 引入了一种新颖的人本自适应AI集成系统，通过"理性路由捷径"机制，基于上下文线索在两个专家模型之间战略性地切换：对齐模型和互补模型。该机制被证明是接近最优的。

Result: 理论分析阐明了自适应AI集成为何有效以及在何时能带来最大效益。在模拟和真实世界数据的实验中，当人类在决策中获得自适应AI集成的协助时，其表现显著优于仅使用单一AI模型（无论是优化独立性能还是优化人机团队性能的模型）的情况。

Conclusion: 自适应AI集成方法成功解决了人机协作中性能与信任之间的根本矛盾，通过动态切换策略实现了比传统单一模型方法更优的人机团队性能，为人机协作系统的设计提供了新思路。
...
### [377] [ReSyn: Autonomously Scaling Synthetic Environments for Reasoning Models](https://arxiv.org/abs/2602.20117)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么是解决方案中心的合成数据生成，要么依赖少量手工制作的环境，需要一种能够大规模生成多样化推理环境的方法来扩展可验证奖励的强化学习

Method: 提出ReSyn管道，生成包含实例生成器和验证器的多样化推理环境，覆盖约束满足、算法谜题和空间推理等任务，使用Qwen2.5-7B-Instruct模型进行强化学习训练

Result: 训练后的模型在推理基准测试和领域外数学基准测试中取得一致提升，在BBEH基准上相对改进27%，消融实验显示验证器监督和任务多样性都有显著贡献
