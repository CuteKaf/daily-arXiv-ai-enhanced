...
Motivation: 自主系统的可靠环境感知需要稳健的视觉标记，而现有的标记在全方位视角和抗遮挡方面存在局限。

Method: 基于PuzzleBoard模式的组合结构，开发圆柱形标记PuzzlePole，利用其独特结构实现360度视角的识别和姿态估计。

Result: PuzzlePole能够从任意方向可靠识别，提供高精度的定位和方向估计，同时对遮挡具有鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型的评估方法（如通用奖励函数或基于相似性的指标）无法捕捉个人视觉偏好的多样性和复杂性，需要开发能够评估个性化对齐的方法。

Method: 采用自引导策略，在有限参考数据上进行推理构建丰富的用户上下文，通过动态生成用户条件评估维度和CoT推理来评估图像，并提供个性化反馈以驱动用户特定的提示优化。

Result: PIGReward在准确性和可解释性方面优于现有方法，并建立了PIGBench个人偏好基准，展示了在个性化T2I评估和优化方面的有效性。

Conclusion: PIGReward为个性化文本到图像生成评估和优化提供了可扩展且基于推理的基础，是实现个体对齐T2I生成的重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 深度视觉模型中训练点对测试预测的影响估计面临计算成本高、训练非平稳性等挑战，现有方法存在离线计算滞后和置信度校准缺失问题。

Method: SG-OIF框架将算法稳定性作为实时控制器，使用随机Richardson和预条件Neumann维护轻量级锚点IHVPs，通过模块化曲率后端调节每个样本的影响分数。

Result: 在CIFAR-10(20%不对称噪声)上，前1%预测样本准确率达91.1%；在MNIST上AUPR得分达99.8%，在多个数据集的各种噪声场景下表现优异。

Conclusion: SG-OIF框架是实用的在线影响估计控制器，能有效识别关键训练样本，提升模型鲁棒性。
...
TL;DR: Pistachio是一个新的视频异常检测(VAD)和理解(VAU)基准，通过生成式流水线构建，提供场景多样性、平衡异常覆盖和时间复杂性，克服了现有基于互联网收集数据集的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有VAD基准缺乏场景多样性、平衡异常覆盖和时间复杂性，而VAU需要更深层次的语义和因果推理但难以进行基准测试。

Method: 利用视频生成模型，通过场景条件异常分配、多步骤故事情节生成和时间一致的长格式合成策略，构建41秒的连贯视频。

Result: Pistachio展示了规模、多样性和复杂性，为现有方法揭示了新挑战，并推动动态和多事件异常理解的未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用专门架构或模态特定参数处理跟踪和分割任务，限制了泛化性和可扩展性。不同模态间的分布差异和跨任务的特征表示差异阻碍了有效的知识共享，制约了通用模型的发展。
...
<details>
  <summary>Details</summary>
Motivation: 姿态估计是计算机视觉中的基础问题，现有的3D-3D对齐问题已有SVD和四元数特征系统等精确解法，但3D-2D正交投影(OnP)问题缺乏类似的封闭形式解，需要新的解决方案。

Method: 采用行列式比值矩阵(DRaM)方法求解无误差EnP和OnP问题的最小二乘系统，对于含噪声数据采用简单的旋转校正方案。将QR分解和Moore-Penrose伪逆变换等方法统一到DRaM框架下。

Result: 提出了EnP和OnP旋转估计问题的DRaM家族解决方案，比较了不同解决方案家族的行为特性，为这类问题提供了新的解决方法和深入理解。

Conclusion: DRaM方法不仅为3D和2D正交投影姿态估计提供了新解决方案，而且展示了该方法可追溯到高斯时代，并能推广到所有N维欧几里得姿态估计问题。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决工业任务理解中缺乏高质量多模态数据集的问题，特别是协作工作场景的数据需求，为工业AI应用提供基准测试资源。

Method: 收集了约197小时的第一人称视角和97小时第三人称视角视频，包含丰富的多模态数据（眼动追踪、语音叙述、声音、运动等），并提供详细标注和基准测试。

Result: 数据集包含丰富的多模态数据和标注，基准评估显示现有最先进的多模态模型在该数据集上的错误检测、问答和协作任务理解方面仍面临挑战。

Conclusion: IndEgo数据集为工业任务理解提供了有价值的基准资源，展示了当前多模态模型在复杂工业场景中的局限性，推动了该领域的发展。
...
TL;DR: LatentDreamer是一个从单张图像生成3D物体的新框架，使用预训练的变分自编码器将3D几何映射到潜在特征，通过序列化流程生成粗糙几何、精细几何和真实纹理，能在70秒内完成高保真度生成。


<details>
  <summary>Details</summary>
Motivation: 当前自动3D生成方法（如图像到3D）难以同时实现快速、详细和高保真度的生成，需要一种能同时满足这些要求的解决方案。

Method: 使用预训练变分自编码器将3D几何映射到潜在特征，通过序列化流程依次生成粗糙几何、精细几何和真实纹理。

Result: 生成的3D物体与输入图像具有高保真度，整个生成过程可在70秒内完成，少量训练即可达到与当代方法竞争的性能。

Conclusion: LatentDreamer通过潜在特征映射和序列化生成流程，成功实现了快速、高保真度的3D物体生成，在效率和效果上都具有竞争优势。
...
TL;DR: 本文研究了模型容量与保留图像语义所需最少视觉token数量之间的基本关系，提出了正交过滤方法来压缩冗余token，并揭示了token与模型规模之间的缩放规律。


<details>
  <summary>Details</summary>
Motivation: 受最小描述长度原理启发，重新解释图像token为视觉语义空间中的向量，定义图像内在语义复杂度为跨越该空间所需的最小基向量集合。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型规模不断增长，部署和效率面临挑战，现有压缩方法缺乏信息保留的理论保证。
...
TL;DR: 提出了一种基于3D面部标志的新颖眼睑角度(ELA)指标，用于检测驾驶员疲劳。ELA比传统的2D眼动指标更稳定，能抵抗相机角度变化，并用于构建眨眼检测框架和生成合成数据集。


<details>
  <summary>Details</summary>
Motivation: 可靠检测驾驶员疲劳对道路安全和高级驾驶辅助系统至关重要，但现有方法如眼动比(EAR)对相机角度变化敏感，且自然疲劳数据收集困难且危险。

Method: 使用3D面部标志计算眼睑角度(ELA)，设计眨眼检测框架提取时间特征，并利用ELA信号在Blender 3D中动画化虚拟角色生成合成数据集。
...
TL;DR: VideoChat-M1是一个创新的多智能体视频理解系统，采用协作策略规划范式，通过动态策略生成、执行和通信机制，结合多智能体强化学习，在多个基准测试中达到最先进性能。
...
Method: 提出协作策略规划范式，包含三个关键过程：策略生成（每个智能体生成独特的工具调用策略）、策略执行（顺序调用相关工具探索视频内容）、策略通信（智能体在中间阶段交互更新策略）。同时采用多智能体强化学习方法进行联合优化。

Result: 在8个基准测试的4个任务中达到最先进性能，在LongVideoBench上比SOTA模型Gemini 2.5 pro提升3.6%，比GPT-4o提升15.6%。

Conclusion: VideoChat-M1通过动态协作的多智能体框架有效提升了视频理解能力，证明了协作策略规划范式在多模态大语言模型工具增强系统中的有效性。
...
TL;DR: 提出了感知分类法，这是一种场景理解的结构化过程，通过识别对象及其空间配置，推断任务相关属性来支持目标导向推理。创建了包含5802张图像和28033个问题的基准测试，评估模型在物理基础视觉推理方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言基准缺乏对这种能力的全面评估，主要关注表面识别或图像文本对齐，而感知分类推理是人类认知的基础。

Method: 标注3173个对象的84个细粒度属性，构建包含对象描述、空间推理、属性匹配和分类推理四种类型问题的多选问答基准，包含合成和真实域图像。

Result: 领先的视觉语言模型在识别任务上表现良好，但在属性驱动问题上性能下降10-20%，特别是需要多步推理的问题。提供模拟场景的上下文推理示例能提高真实世界和专家策划问题的性能。

Conclusion: 当前模型在结构化视觉理解方面存在持续差距，主要依赖模式匹配。感知分类法引导的提示方法能有效提升推理性能。
...
TL;DR: Vidi2模型在视频理解方面取得重大进展，具备细粒度时空定位能力，可同时识别时间戳和边界框，并在VUE-STG和VUE-TR-V2基准测试中超越Gemini 3 Pro和GPT-5等专有系统。


<details>
  <summary>Details</summary>
Motivation: 满足互联网上对可扩展、高质量视频制作的强烈需求，推动下一代视频创作技术的发展。

Method: 开发Vidi2模型，具备端到端时空定位能力，引入新的VUE-STG基准测试，包含长视频、名词短语查询、高质量标注和改进的评估指标。

Result: Vidi2在VUE-TR-V2和VUE-STG基准测试中显著优于领先的专有系统，在视频问答基准测试中与类似规模的开源模型竞争激烈。

Conclusion: Vidi2模型在视频理解和时空定位方面表现出色，为复杂编辑场景提供了强大的技术支持，同时新的基准测试为实际应用中的全面评估奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 分布式光伏系统的快速扩展给电网管理带来挑战，许多安装未被记录。传统计算机视觉模型需要大量标注数据且难以跨区域泛化。

Method: 利用多模态大语言模型，通过结构化提示和微调，将检测、定位和量化集成到统一框架中。

Result: 跨区域评估显示，所提模型在未见区域性能下降最小，优于传统CV和transformer基线方法。

Conclusion: 多模态大语言模型在领域偏移下具有鲁棒性，具备可扩展、可迁移和可解释的全球光伏映射潜力。
...
TL;DR: 本研究提出了大规模分析制图遗产的方法和数据集，利用来自38个数字目录的771,561条地图记录和99,715张数字化图像，涵盖1492-1948年六个世纪。通过语义分割和对象检测技术分析地图内容和符号系统，揭示了制图的地理结构、政治动态影响以及符号系统的演变规律。


<details>
  <summary>Details</summary>
Motivation: 尽管全球遗产机构已数字化超过一百万张地图，自动化技术也能大规模识别地图内容，但这些方法很少与制图历史或地图作为文化对象的语义符号系统视角相结合。本研究旨在从文化角度大规模研究制图遗产。

Method: 收集并标准化来自38个数字目录的771,561条地图记录和99,715张数字化图像；使用语义分割技术和对象检测模型进行通用土地类别和制图符号识别；训练基于标注数据和合成图像的模型；分析236,925位贡献者的协作和传播模式。

Result: 揭示了制图地理结构与政治动态的联系（如大西洋海图绘制、三角贸易和殖民扩张）；记录了国家关注焦点和军事冲突对出版量的影响；分析了63M符号和25M片段，发现符号形成局部一致系统，如地形等高线取代阴影线；证明了地图是经过设计的图像，通过中心化和语义对称强调特征。

Conclusion: 制图遗产研究揭示了地图作为文化对象和语义符号系统的复杂性，展示了符号系统的演变规律、协作传播中合法性和主要参与者的作用，为大规模文化制图分析提供了方法论和数据基础。
...
TL;DR: 提出ThiFAN-VQA框架，通过两阶段推理机制解决灾难场景下视觉问答问题，结合思维链提示和上下文学习，在有限监督下实现可解释推理，并通过答案选择模块提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于AI的灾害评估方法存在数据标注成本高、数据集有限、传统分类框架灵活性不足、生成模型易产生幻觉或通用回答等问题，需要开发更灵活且准确的解决方案。

Method: 采用两阶段推理框架：第一阶段使用思维链提示和上下文学习生成结构化推理轨迹；第二阶段通过答案选择模块评估生成响应，选择最连贯和上下文准确的答案。集成定制信息检索系统、领域特定提示和推理引导的答案选择。

Result: 在FloodNet和RescueNet-VQA数据集上的实验表明，ThiFAN-VQA在准确性、可解释性和适应性方面优于现有方法，适用于真实世界的灾后损害评估任务。

Conclusion: ThiFAN-VQA成功填补了零样本和监督方法之间的差距，将灵活性与一致性相结合，为灾后损害评估提供了优越的解决方案。
...
TL;DR: HunyuanOCR是一个商业级、开源、轻量级的视觉语言模型，专门用于OCR任务。该模型在感知任务和语义任务上表现出色，在ICDAR 2025 DIMT挑战赛中获得第一名，并在OCRBench上达到SOTA水平。
...
Method: 采用原生视觉变换器和轻量级LLM架构，通过MLP适配器连接，使用纯端到端范式，结合高质量数据和强化学习策略。

Result: 在文本检测、解析、信息提取、文本图像翻译等任务上超越商业API和传统流程，在ICDAR 2025 DIMT挑战赛中获得第一名，在OCRBench上达到SOTA。
...
TL;DR: 本文提出了一种基于VAE的多尺度向量量化变分自编码器(MSVQ-VAE)，用于生成无线胶囊内窥镜(WCE)图像，特别是能够将异常无缝引入正常WCE图像中，支持条件生成多种异常类型。


<details>
  <summary>Details</summary>
Motivation: 由于隐私约束和标注成本，医疗数据稀缺限制了基于深度学习的临床决策支持系统发展。生成式机器学习为解决这一限制提供了可行方案，但现有方法在训练稳定性和视觉多样性方面存在挑战。

Method: 提出多尺度向量量化变分自编码器(MSVQ-VAE)，能够无缝将异常引入正常WCE图像，支持条件生成多种异常类型（息肉、血管和炎症等）。
...
TL;DR: SkillSight是一个用于从第一人称视角数据中进行节能技能评估的系统，通过结合注视数据和视频来预测技能水平，最终仅使用注视数据进行推理，大幅降低功耗。
...
Method: 采用两阶段框架：首先学习联合建模注视和自我中心视频来预测技能水平，然后蒸馏出一个仅使用注视数据的学生模型。在推理时，学生模型只需要注视输入，通过消除连续视频处理来大幅降低功耗。

Result: 在三个涵盖烹饪、音乐和体育的数据集上的实验首次确立了注视在不同现实世界环境中技能理解中的重要作用。SkillSight教师模型实现了最先进的性能，而仅使用注视的学生变体在比竞争方法功耗低73倍的情况下保持了高精度。

Conclusion: 这些结果为在野外进行AI支持的技能学习铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索视觉-语言基础模型能否通过提供超出传统先验的高级上下文信息来增强欠采样MRI重建。传统方法主要依赖低层图像特征，而高级语义信息可能提供更有效的重建指导。

Method: 提出了语义分布引导的重建框架，使用预训练的视觉-语言基础模型编码重建图像和辅助信息为高级语义特征。采用对比目标对齐重建表示与目标语义分布，确保与高级感知线索的一致性。该方法可与各种深度学习重建方法配合使用，并能灵活整合多模态语义先验。

Result: 在膝关节和脑部数据集上的实验表明，基于图像的语义先验能保留精细解剖结构，获得更好的感知质量（更低的LPIPS值、更高的Tenengrad分数和读者研究评分）。图像-语言信息进一步扩展了语义分布，实现了对重建属性的高级控制。对比目标在所有评估中都能一致地引导重建特征朝向期望的语义分布，同时保持数据保真度。

Conclusion: 研究表明视觉-语言基础模型通过语义空间优化可以改善欠采样MRI重建。该方法通过整合高级语义信息，在保持解剖结构细节和提升感知质量方面优于传统正则化方法。
...
TL;DR: GIANT框架让通用大模型能够像病理学家一样迭代导航全切片图像，在病理学图像解释任务中显著优于传统方法，接近甚至超越专门训练的专业模型。
...
Method: 开发GIANT框架，使大模型能够迭代导航全切片图像；发布MultiPathQA基准，包含934个WSI级问题和128个病理学家编写的问题。

Result: GIANT在病理学家编写的问题上达到62.5%准确率，显著优于TITAN(43.8%)和SlideChat(37.5%)等专业模型。
...
<details>
  <summary>Details</summary>
Motivation: 旨在通过结合影像和临床数据来提高乳腺癌诊断的准确性，减少观察者间差异，并为医疗资源匮乏地区提供可扩展的筛查解决方案。

Method: 使用基于注意力的编码器-解码器架构分割四个ROI区域，预测十项结构化临床特征，并开发两种晚期融合策略来整合多模态数据。

Result: 实现了最先进的分割精度，能够稳健预测临床特征，通过晚期融合策略提高了诊断精度，并开发了用户友好的Web应用程序。

Conclusion: OncoVision通过准确分割与临床直觉的结合，为基于AI的乳腺X光检查设立了新标准，提供了可扩展且公平的早期乳腺癌检测解决方案。
...
TL;DR: 提出一种基于高度的动态分块方法，通过自适应图像分块提升无人机搜救任务中的小目标检测性能，在保持检测准确性的同时显著提高推理速度。
...
Method: 采用高度感知的动态分块方法，结合高度相关的缩放因子和自适应分块策略，在SeaDronesSee数据集上使用YOLOv5和SAHI框架进行测试。

Result: 相比基线方法，小目标的平均精度提升38%，推理速度比静态分块方法提高两倍以上。

Conclusion: 该方法能够在多样化条件下实现更高效、准确的无人机搜救操作。
...
TL;DR: 提出Prune-Then-Plan框架，通过步骤级校准稳定探索，解决VLM在具身问答中的边界振荡问题，在OpenEQA和EXPRESS-Bench数据集上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在具身问答中存在边界振荡问题，即由于过度自信和校准不当导致的不稳定来回移动，降低了导航效率和答案质量。

Method: 采用Prune-Then-Plan框架：首先使用Holm-Bonferroni启发的剪枝程序剔除不可信的边界选择，然后将最终决策委托给基于覆盖率的规划器，将过度自信的预测转化为保守可解释的动作。

Result: 在3D-Mem EQA框架中集成该方法，在视觉基础SPL和LLM-Match指标上分别比基线相对提升49%和33%，在相同探索预算下获得更好的场景覆盖率。

Conclusion: 该方法通过分离剪枝和规划步骤，利用人类级判断校准VLM的步骤级行为，有效解决了边界振荡问题，提高了具身问答的效率和准确性。
...
### [26] [GigaWorld-0: World Models as Data Engine to Empower Embodied AI](https://arxiv.org/abs/2511.19861)
...
TL;DR: GigaWorld-0是一个统一的世界模型框架，通过视频生成和3D建模的协同优化，为视觉-语言-动作学习提供大规模合成数据，显著提升了物理机器人的泛化能力和任务成功率。


<details>
  <summary>Details</summary>
Motivation: 世界模型正在成为可扩展、数据高效的具身AI基础范式，需要开发能够生成多样化、高质量训练数据的统一框架来支持VLA学习。

Method: GigaWorld-0包含两个组件：GigaWorld-0-Video利用大规模视频生成创建纹理丰富、时间连贯的具身序列；GigaWorld-0-3D结合3D生成建模、高斯溅射重建、物理系统识别和运动规划确保几何一致性和物理真实性。通过GigaTrain框架实现高效训练。

Result: GigaWorld-0能够生成高质量、多样化且可控的数据，在GigaWorld-0生成数据上训练的VLA模型（如GigaBrain-0）在物理机器人上表现出强大的真实世界性能，显著提高了泛化能力和任务成功率。

Conclusion: GigaWorld-0作为数据引擎，能够有效支持VLA学习，在无需真实世界交互训练的情况下，显著提升物理机器人的实际性能，为具身AI的发展提供了重要支撑。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型微调方法要么过度约束适应，要么忽略不同组件的差异作用，容易破坏预训练表示并损害泛化能力

Method: MAPS通过系统分析发现邻近度约束的松弛顺序，并线性调度这种松弛，使视觉编码器保持接近预训练先验，而面向动作的语言层更自由地适应
...
### [28] [RADSeg: Unleashing Parameter and Compute Efficient Zero-Shot Open-Vocabulary Segmentation Using Agglomerative Models](https://arxiv.org/abs/2511.19704)
...
Method: 采用RADIO视觉基础模型，引入自相关递归注意力、自相关全局聚合和计算高效的掩码细化技术来增强零样本分割性能。

Result: RADSeg在基础ViT类别中mIoU提升6-30%，速度快3.95倍，参数减少2.5倍。RADSeg-base（105M）在mIoU上超越了之前850-1350M参数的巨型视觉模型组合。

Conclusion: RADSeg在准确性和效率方面都达到了最先进水平，以显著更低的计算和内存成本实现了最先进的精度。
...
### [29] [Reasoning-VLA: A Fast and General Vision-Language-Action Reasoning Model for Autonomous Driving](https://arxiv.org/abs/2511.19912)
...
Method: 使用高斯采样从训练数据真实轨迹初始化可学习动作查询，与推理增强的视觉语言特征交互，并行生成连续动作轨迹；整合8个公开数据集形成标准化、基于思维链推理的数据格式；结合监督学习和强化学习微调。

Result: 在多个基准测试中达到最先进性能，具有优越的泛化能力和迄今为止报道的最佳推理速度。

Conclusion: Reasoning-VLA框架在自动驾驶决策中实现了高效推理、强大泛化和快速动作生成，为VLA模型提供了有效的解决方案。
...
### [30] [Rethinking Vision Transformer Depth via Structural Reparameterization](https://arxiv.org/abs/2511.19718)
*Chengwei Zhou,Vipin Chaudhary,Gourav Datta*

Main category: cs.CV

TL;DR: 提出基于分支的结构重参数化技术，在训练阶段使用并行分支，然后合并为单路径模型，可将ViT-Tiny从12层压缩到3-6层，在ImageNet-1K上保持精度，推理速度提升达37%。


<details>
  <summary>Details</summary>
Motivation: 现有ViT加速方法主要关注算法级优化（如token剪枝和注意力加速），但忽略了减少堆叠层数的可能性。本文探索是否能在减少transformer层数的同时保持相当的表示能力。

Method: 使用基于分支的结构重参数化技术，在训练阶段引入并行分支，然后通过数学方法将分支合并到非线性组件的入口点，实现FFN和MHSA模块的精确重参数化，无推理时近似误差。
...
TL;DR: MAP-World是一个无先验的多模态规划框架，结合了掩码动作规划和路径加权世界模型，用于自动驾驶运动规划，无需手工锚点或强化学习，能生成多样化的轨迹模式。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶规划系统通常依赖手工锚点或强化学习来选择单一最佳轨迹模式，这会丢弃替代未来信息并使优化复杂化。

Method: 使用掩码动作规划模块将未来自车运动视为掩码序列补全，通过注入噪声生成多样化轨迹查询；轻量级世界模型基于候选轨迹推出未来BEV语义；训练时使用轨迹概率作为离散路径权重计算语义损失期望。

Result: 在NAVSIM基准测试中，该方法与基于锚点的方法性能相当，并在基于世界模型的方法中达到最先进性能，同时保持实时推理延迟。
...
<details>
  <summary>Details</summary>
Motivation: 虽然基于切片的传输方法能降低计算成本，但学习到的最优切片器能否在分布偏移下迁移到新的分布对仍是一个开放问题，这在数据演化或重复OT计算的场景中至关重要。

Method: 研究min-STP框架，理论上分析优化切片器在数据分布扰动下的稳定性，并引入小批量min-STP公式以提高可扩展性。

Result: 理论证明优化切片器在数据分布轻微扰动下保持接近，实验表明可迁移min-STP在点云对齐和基于流的生成建模中实现强的一次性匹配性能和摊销训练。

Conclusion: 优化切片器具有可迁移性，能够在相关任务间高效转移，小批量min-STP提供了统计保证，为分布演化场景中的OT应用提供了实用解决方案。
...
### [33] [Material-informed Gaussian Splatting for 3D World Reconstruction in a Digital Twin](https://arxiv.org/abs/2511.20348)
*João Malheiro Silva,Andy Huynh,Tong Duy Son,Holger Caesar*

Main category: cs.CV

TL;DR: 提出了一种仅使用相机的3D重建方法，通过多视角图像进行3D高斯泼溅重建，结合视觉模型提取语义材质信息，并将其转换为具有物理材质属性的网格模型，用于数字孪生中的传感器仿真。


<details>
  <summary>Details</summary>
Motivation: 传统LiDAR-相机融合方法需要复杂的校准过程，且在处理玻璃等特殊材质时效果不佳，而相机自然捕捉了语义和纹理信息但缺乏准确的几何数据。

Method: 使用多视角图像进行3D高斯泼溅重建，通过视觉模型提取语义材质掩码，将高斯表示转换为带材质标签的网格表面，并分配基于物理的材质属性。

Result: 该方法实现了与LiDAR-相机融合相当的传感器仿真保真度，同时消除了硬件复杂性和校准需求，在内部数据集上使用LiDAR作为地面真值进行验证。
...
TL;DR: VESSA是一个结合视觉语言模型和半监督学习的医学图像分割方法，通过两阶段框架在有限标注条件下显著提升分割精度。
...
Conclusion: VESSA成功将基础级视觉语义理解融入半监督学习框架，为医学图像分割提供了有效的解决方案，特别是在标注数据稀缺的情况下。
...
TL;DR: 本研究提出了一种新的相对角度特征，用于点云损伤重建，该特征在保持与法向量相似性能的同时，显著减少了数据存储和输入通道需求。


<details>
  <summary>Details</summary>
Motivation: 点云损伤重建方法虽然能有效避免基于图像方法易受背景噪声影响的问题，但由于3D数据量大，应用受到限制。需要开发更高效的特征表示方法。

Method: 提出相对角度特征，计算为点的法向量与其父点云平均法向量之间的夹角。通过基于熵的特征评估验证该特征的有效性，并使用PointNet++模型进行训练和测试。

Result: 基于相对角度的模型与基于法向量的模型性能相当，同时实现了27.6%的存储减少和83%的输入通道压缩。

Conclusion: 相对角度特征能够在资源受限的硬件上实现更大批量的执行，且无需对模型架构进行修改，具有重要的应用潜力。
...
TL;DR: 论文识别了在扩散变换器中使用旋转位置编码线性插值进行混合分辨率去噪时的核心失败模式，提出了无需训练的交叉分辨率相位对齐注意力机制来解决该问题。


<details>
  <summary>Details</summary>
Motivation: 当在不同空间网格的token混合时，线性坐标重映射会导致注意力机制崩溃，因为单个注意力头需要比较以不兼容速率采样的RoPE相位，产生相位混叠并破坏分数景观。

Method: 提出了交叉分辨率相位对齐注意力（CRPA），通过修改每个注意力调用的RoPE索引映射，将所有Q/K位置表达在查询的步长上，确保相等的物理距离始终产生相同的相位增量。

Result: CRPA能够稳定所有头和层，实现高保真和高效的混合分辨率生成，在图像和视频生成任务上优于先前的最先进方法。

Conclusion: CRPA是一个无需训练的即插即用解决方案，能够从根本上消除混合分辨率生成中的相位混叠问题，恢复扩散变换器所依赖的精确相位模式。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在需要细粒度图像理解任务（如场景文本识别、文档分析）中的感知限制和视觉碎片化问题。

Method: 使用强化学习训练CropVLM模型，无需人工标注边界框作为监督信号，也不需要昂贵的合成评估。模型训练一次后可与开源和专有视觉语言模型配对使用。

Result: 在需要高分辨率图像理解的任务上取得显著改进，特别是在目标视觉语言模型的域外基准测试中表现优异。

Conclusion: CropVLM能够在不修改或微调视觉语言模型的情况下提升性能，避免灾难性遗忘，为细粒度图像理解任务提供有效解决方案。
...
TL;DR: 本文提出潜在表示探测（LRP）方法，通过训练轻量级探测器分析VLM内部隐藏状态或注意力模式来检测不确定性，在STVQA任务中显著提升弃权准确性。


<details>
  <summary>Details</summary>
Motivation: VLM在安全关键应用中需要具备在不确定时弃权的能力，特别是在STVQA任务中OCR错误可能导致严重后果。现有弃权方法存在校准不当或语义一致性不适用的问题。

Method: 提出LRP方法：在隐藏状态或注意力模式上训练轻量级探测器，探索三种设计：跨层表示拼接、视觉标记注意力聚合、单层探测器多数投票集成。

Result: 在四个图像和视频基准测试中，LRP比最佳基线方法提升弃权准确性7.6%，探测器能泛化到各种不确定性来源和数据集，最佳信号来自中间层而非最终层。

Conclusion: 建立了一个基于内部状态而非不可靠输出来检测置信信号的原理性框架，为构建可部署AI系统提供了基础。
...
### [40] [CounterVQA: Evaluating and Improving Counterfactual Reasoning in Vision-Language Models for Video Understanding](https://arxiv.org/abs/2511.19923)
*Yuefei Chen,Jiang Liu,Xiaodong Lin,Ruixiang Tang*

Main category: cs.CV

TL;DR: 该论文提出了CounterVQA视频基准来评估视觉语言模型的因果推理能力，发现现有模型在复杂多跳因果链推理上表现不佳，并开发了CFGPT方法来提升模型的视觉因果推理能力。
...
Conclusion: 视觉语言模型在反事实推理方面存在显著能力差距，特别是在复杂因果推理任务上；CFGPT方法能有效提升模型的视觉反事实推理能力，为更稳健的视频理解系统提供了可行路径。
...
<details>
  <summary>Details</summary>
Motivation: 解决先前工作中RoPE的误用问题，改进输入视频和目标重拍之间的时空位置对齐，并处理多视角关系集成问题。

Method: 提出Rotary Camera Encoding (RoCE)，一种相机条件的RoPE相位偏移，用于捕获和集成输入与目标视频内部及之间的多视角关系。通过将相机条件集成到RoPE中，方法能够泛化到分布外的相机轨迹和视频长度。

Result: 实验表明，该方法在相机可控性、几何一致性和视频质量方面都有显著提升，能够改善动态对象定位和静态背景保持。

Conclusion: ReDirector通过改进的相机条件编码机制，能够有效处理变长视频重拍生成，在各种轨迹和长度下都表现出优越性能。
...
TL;DR: 本文提出Rectified SpaAttn方法，通过隐式全注意力参考来修正注意力分配，解决现有注意力稀疏化方法导致的系统性偏差问题，在保持生成质量的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在视频生成中占主导地位，但注意力计算的二次复杂度带来显著延迟。现有注意力稀疏化方法虽然能降低计算成本，但会导致严重的性能下降，主要原因是存在系统性偏差：过度关注关键token放大其注意力权重，完全忽略非关键token导致相关注意力权重丢失。

Method: 提出Rectified SpaAttn方法：1）针对关键token，提出孤立池化注意力重分配，通过重分配多模态池化权重计算准确的修正因子；2）针对非关键token，提出增益感知池化修正，确保修正后的增益始终超过引入的误差。使用Triton定制并集成Rectified SpaAttn内核。

Result: 在HunyuanVideo和Wan 2.1上分别实现了3.33倍和2.08倍的加速，同时保持高生成质量。

Conclusion: Rectified SpaAttn有效解决了注意力稀疏化中的系统性偏差问题，在显著提升计算效率的同时保持了生成质量，为视频生成提供了高效的解决方案。
...
TL;DR: 该论文提出了一种名为IMIC的梯度耦合交错训练方法，能够在单个嵌入空间中同时处理四个任务（物体识别、高/低质量人脸识别、全身图像人物识别），而不会产生严重的灾难性遗忘问题。
...
Method: 提出了IMIC方法的两个变体，这是一种梯度耦合的交错训练计划，同时对所有四个任务进行微调，支持DINOv3、CLIP和EVA-02三种基础模型。

Result: EVA-02和CLIP模型在所有四个任务上表现与领域专家相当，在跨人脸、身体和物体数据集的多任务处理中比人类更准确，且不损害分布外泛化能力。

Conclusion: IMIC方法成功实现了多任务统一嵌入空间，四个任务在嵌入空间中线性可分但特征高度共享，仅需少量主成分即可完成所有任务。
...
Method: 提出两个关键组件：(1) UV自适应软绑定框架，利用图像和几何先验在UV空间中学习每个高斯的特征偏移，支持动态重采样；(2) 时间自适应密度控制策略，通过聚类结构相似帧并引入融合感知误差作为克隆标准，联合捕捉几何和纹理差异。
...
TL;DR: TimeArtist是一个时间-视觉转换框架，通过语义级对齐实现从时间序列到高质量图像的生成，同时捕捉时间波动模式进行风格转换。
...
Result: 在图像生成指标上取得满意性能，同时在零样本时间任务中获得优越结果，能够直接从时间序列生成高质量、多样化的图像。
...
<details>
  <summary>Details</summary>
Motivation: 解决多时相遥感图像变化检测中的异质性和时空错位问题，现有方法在时间序列化过程中破坏局部结构一致性，导致变化定位不可靠。

Method: 提出ChessMamba框架，集成SpatialMamba编码器和轻量级跨源交互模块，采用棋盘交错与蛇形扫描顺序将多时相特征序列化，以及通过多扩张卷积进行结构感知融合。

Result: 在三个变化检测任务（二元变化检测、语义变化检测、多模态建筑损伤评估）上的综合评估表明，ChessMamba能有效融合异质特征，相比最先进方法实现显著精度提升。

Conclusion: ChessMamba通过结构感知的状态空间建模，在多时相遥感图像变化检测中实现了稳健的性能，为细粒度变化识别提供了有效解决方案。
...
TL;DR: 提出频率解耦跨模态知识蒸馏方法，通过频域特征解耦和平衡跨模态知识迁移，在多个基准数据集上显著优于传统知识蒸馏和现有跨模态知识蒸馏方法。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏在跨模态场景（如视觉到语言）中效果下降，因为不同模态间的表示不一致导致知识迁移困难。

Method: 利用频域特征，对低频特征实施强对齐损失，对高频特征引入宽松对齐，并提出尺度一致性损失解决模态间分布偏移，使用共享分类器统一特征空间。

Result: 在多个基准数据集上的广泛实验表明，该方法显著优于传统知识蒸馏和最先进的跨模态知识蒸馏方法。

Conclusion: 频率解耦跨模态知识蒸馏方法有效解决了跨模态知识迁移的挑战，通过频域特征解耦实现了更好的知识传递效果。
...
TL;DR: 本文构建了一个多任务肝脏数据集LiMT，用于肝脏和肿瘤分割、多标签病变分类和病变检测，旨在解决现有数据集仅支持单任务的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有计算机辅助诊断技术的数据集应用范围有限，通常只支持单任务，这限制了CAD技术的发展。

Method: 基于动脉期增强CT构建包含150个病例的多任务肝脏数据集，涵盖四种肝脏疾病和正常病例，由经验丰富的临床医生仔细标注和校准。

Result: 提供了相关基线实验结果，并回顾了现有肝脏相关任务的数据集和方法。数据集已公开可用。

Conclusion: 该公开多任务数据集可能成为医学影像研究社区的宝贵资源，能够探索任务间的相关性，无需担心训练过程中任务特定数据集间的异质性。
...
TL;DR: 本文提出了一个生成-验证框架来创建高质量的SVQA数据集VeriSciQA，包含20,351个QA对，涵盖20个科学领域和12种图表类型，显著提升了开源模型在科学视觉问答任务上的性能。
...
Method: 提出生成-验证框架：首先生成带有图表文本上下文的QA对，然后通过跨模态一致性检查和辅助过滤器消除错误对，最终构建VeriSciQA数据集。

Result: VeriSciQA成为开源模型的挑战性基准，领先开源模型准确率64%，专有模型82%。在VeriSciQA上微调的模型在SVQA基准上获得一致改进，性能随数据规模扩大而提升。

Conclusion: 通过可扩展的生成-验证框架持续扩展数据可以进一步推进开源社区在科学视觉问答领域的能力发展。
...
TL;DR: 本文提出了一种多模态连续手语识别方法，通过机器学习检测美国手语句子视频中手语的起止帧，然后识别分割后的手语。使用3D骨骼特征捕捉手语边界处的特征收敛，并整合3D手形信息进行边界检测。


<details>
  <summary>Details</summary>
Motivation: 提高连续手语识别的鲁棒性，通过结合手势动态特征和手形信息来更准确地检测手语边界，解决连续手语中手语边界检测的挑战。

Method: 1) 使用3D骨骼特征提取手语边界特征；2) 预训练87个语言学定义的手形分类器；3) 多模态融合模块整合视频分割框架和手形分类模型；4) 在大规模数据库上训练识别模型，包含孤立手语和连续手语片段。

Result: 在ASLLRP语料库上的评估显示，该方法相比先前工作有显著改进。

Conclusion: 多模态方法结合手势动态和手形信息能够有效提高连续手语边界检测和识别的准确性，为连续手语识别提供了更鲁棒的解决方案。
...
TL;DR: Motion Marionette是一个零样本框架，用于将单目源视频的刚体运动转移到单视图目标图像。该方法通过内部先验而非外部先验来指导运动转移过程，在统一3D表示空间中提取源视频的运动轨迹构建时空先验，然后与目标对象结合生成可控速度场，最终通过基于位置的动力学进行细化。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用几何、生成或模拟先验来指导运动转移，但这些外部先验引入了辅助约束，导致泛化性和时间一致性之间的权衡。为了解决这些限制，作者提出通过内部先验来指导运动转移过程。

Method: 首先将源视频和目标图像提升到统一的3D表示空间，从源视频中提取运动轨迹构建独立于对象几何和语义的时空先验，该先验与目标对象结合生成可控速度场，最后使用基于位置的动力学进行细化以消除伪影并增强视觉连贯性。

Result: 实证结果表明，Motion Marionette能够泛化到不同的对象，生成与源运动对齐的时间一致视频，并支持可控视频生成。

Conclusion: Motion Marionette通过内部时空先验实现了有效的刚体运动转移，在保持时间一致性的同时具有良好的泛化能力，为高效视频制作提供了灵活的工具。
...
TL;DR: 该论文提出了一个预测3D打印物体光化学转化的框架，通过C-PGA架构从3D视觉数据预测非视觉的体物理性质，解决了传统视觉模型无法处理光学物理与材料物理耦合作用的挑战。
...
Conclusion: C-PGA架构通过显式建模物理耦合，成功解决了从3D视觉数据预测密集非视觉体物理性质的挑战性任务。
...
TL;DR: 本文提出了LoTTS框架，首次实现无需训练的局部化测试时缩放，通过在图像缺陷区域进行自适应重采样，显著减少计算量并提升图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放方法在全图像级别操作，忽略了图像质量的空间异质性，导致对已满意区域进行不必要计算，对局部缺陷修正不足。
...
TL;DR: HybriDLA是一个新颖的文档布局分析框架，结合了扩散和自回归解码，通过多尺度特征融合编码器实现83.5% mAP的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统文档布局分析方法依赖于经验先验或固定数量的可学习查询，无法处理现代文档中元素数量多样且布局复杂的问题。
...
Conclusion: HybriDLA通过融合扩散和自回归解码，有效解决了现代文档布局分析的挑战，为复杂文档布局理解提供了强大解决方案。
...
Method: 提出两阶段流水线：首先使用YOLO检测器进行高效的组件级筛选，然后使用VLM作为召回增强单元，对检测器漏检的组件进行二次验证。

Result: 在车辆组件数据集上，DetVLM实现了94.82%的整体检索准确率，显著优于仅使用检测的基线方法。在零样本搜索中达到94.95%的准确率，状态搜索任务平均准确率超过90%。
...
TL;DR: CPDATrack是一种新型跟踪框架，通过概率驱动的注意力机制抑制背景和干扰物令牌的干扰，同时提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有单流Transformer跟踪器存在过多背景搜索令牌关注目标模板令牌的问题，削弱了跟踪器的判别能力，且现有令牌剪枝方法容易丢失目标附近的关键上下文信息。

Method: 1) 在指定编码器层间集成可学习模块估计每个搜索令牌与目标的关联概率；2) 基于概率估计剪除信息量少的背景令牌，保留目标周围上下文；3) 采用判别性选择性注意力机制，早期层完全阻断搜索到模板的注意力；4) 后续层从局部区域选择性提取高概率目标令牌关注模板令牌。

Result: 在多个基准测试中达到最先进性能，特别是在GOT-10k上获得75.1%的平均重叠率。

Conclusion: CPDATrack通过概率驱动的注意力机制有效抑制背景和干扰物干扰，在保持计算效率的同时显著提升了跟踪性能。
...
<details>
  <summary>Details</summary>
Motivation: 探索扩散模型自注意力图作为语义标签传播核的潜力，实现超越图像生成的识别和定位任务，特别是零样本视频对象跟踪。

Method: 将扩散模型自注意力图重新解释为语义标签传播核，构建时间传播核进行视频跟踪；采用DDIM反演、文本反演和自适应头权重等测试时优化策略；结合SAM进行掩码优化。

Result: 在标准视频对象分割基准上实现了最先进的零样本性能，证明了扩散特征在鲁棒和一致的标签传播中的有效性。

Conclusion: 扩散模型的自注意力机制能够提供强大的像素级语义对应关系，为视频对象跟踪等任务提供有效的零样本解决方案。
...
Method: 采用测试时优化框架，包括对高分辨率源图像进行分块优化，然后使用细粒度细节传输模块和新型同步策略来保持跨块一致性。

Result: 大量实验表明，该方法能产生高质量的高分辨率编辑结果，为高分辨率内容创建开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有的核实例分割方法大多依赖密集监督和计算昂贵的微调，而无需训练的方法研究不足。SPROUT旨在开发完全无需训练和标注的核实例分割框架，解决领域差距问题。

Method: SPROUT利用组织学先验构建切片特定参考原型，通过部分最优传输方案进行渐进式特征对齐，将前景和背景特征转换为正负点提示，使用SAM模型无需参数更新实现分割。

Result: 在多个组织病理学基准测试上的广泛实验表明，SPROUT在无监督和无需重新训练的情况下实现了具有竞争力的性能。

Conclusion: SPROUT为病理学中可扩展的、无需训练的核实例分割建立了一个新颖的范式，展示了在无监督情况下实现竞争性性能的潜力。
...
TL;DR: HiCoGen是一个分层组合生成框架，通过链式合成范式解决复杂提示下的图像生成问题，使用LLM分解提示并迭代合成，结合强化学习和衰减随机性调度来提升概念覆盖率和组合准确性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在处理涉及多个对象和分层结构的复杂提示时，难以准确遵循指令，导致概念遗漏、混淆和组合性差的问题。

Method: 提出分层组合生成框架HiCoGen，基于链式合成范式：首先用LLM将复杂提示分解为最小语义单元，然后迭代合成这些单元；引入强化学习框架，采用衰减随机性调度增强探索，并通过分层奖励机制评估图像。

Result: 实验表明，该方法在概念覆盖率和组合准确性方面显著优于现有方法。

Conclusion: HiCoGen通过分层分解和链式合成有效解决了复杂提示下的图像生成问题，结合强化学习和衰减随机性调度显著提升了生成质量。
...
TL;DR: VGGT4D是一个无需训练的方法，通过挖掘VGGT全局注意力层中的动态线索来实现4D场景重建，在动态物体分割、相机姿态估计和稠密重建方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有4D方法依赖外部先验、繁重的后优化或需要在4D数据集上微调，而3D基础模型VGGT在动态物体主导时性能显著下降，需要一种无需训练的方法来扩展VGGT用于4D场景重建。

Method: 通过挖掘VGGT全局注意力层中的动态线索，使用Gram相似性放大全局动态线索并跨时间窗口聚合，引入基于投影梯度的细化策略来锐化掩码边界，并将精确掩码集成到VGGT的早期推理阶段。

Result: 在六个数据集上，该方法在动态物体分割、相机姿态估计和稠密重建方面实现了优越性能，支持单次推理超过500帧的长序列。

Conclusion: VGGT4D证明了无需训练即可扩展3D基础模型进行稳健4D场景重建的可行性，通过挖掘模型内在的动态线索有效缓解了运动干扰。
...
TL;DR: 本文提出EmoFeedback2框架，通过生成-理解-反馈的强化范式，利用大视觉语言模型为连续情感图像生成提供奖励和文本反馈，解决现有方法缺乏情感反馈和情感保真度不足的问题。
...
TL;DR: 提出一种无需训练的图像修复方法，通过优化初始种子噪声来匹配未掩码区域，结合线性近似和频域优化实现高效修复。
...
Method: 优化初始种子噪声以近似匹配未掩码区域数据，采用线性近似避免计算成本高昂的反向传播，并在频域进行优化以稳定过程。

Result: 在各种修复任务中表现出色，优于现有最先进方法。

Conclusion: 通过优化初始种子噪声并采用线性近似和频域优化，实现了无需训练的高效图像修复方法，显著提升了现成文本到图像模型在修复任务中的表现。
...
TL;DR: 本文提出了一种多模态VR视线预测方法，结合时间视线模式、头部运动数据和视觉场景信息，通过门控融合机制和跨模态注意力自适应加权不同模态，在22个VR场景的530万视线样本数据集上验证了优于单一模态的预测准确性。


<details>
  <summary>Details</summary>
Motivation: 预测虚拟现实环境中的视线行为对于渲染优化和界面设计具有重要意义，但现有方法面临挑战。本文旨在通过多模态融合提升VR视线预测的准确性，为更高效的VR系统开发提供支持。

Method: 采用多模态方法，结合时间视线模式、头部运动数据和视觉场景信息，使用门控融合机制和跨模态注意力来自适应地加权不同模态的贡献，基于上下文相关性进行融合。

Result: 在22个VR场景的530万视线样本数据集上，多模态融合方法相比单一数据流显著提升了预测准确性，在1-3个未来帧的预测中表现优异，跨场景泛化测试显示93.1%的验证准确率和时间一致性。

Conclusion: 整合过去视线轨迹、头部朝向和场景内容能够有效提升VR视线预测准确性，该方法为理解虚拟环境中的注意力机制提供了新见解，在渲染优化、交互设计和用户体验评估方面具有应用潜力，是实现无需昂贵眼动追踪硬件的更高效VR系统的重要一步。
...
TL;DR: 提出了一种细节感知的参考引导图像生成框架，通过两阶段参考驱动修正来增强像素级一致性，解决了现有方法在保留细粒度视觉细节方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前基于VAE的潜在压缩方法会丢弃细微纹理信息，导致身份和属性特定线索丢失，而现有的后编辑方法在放大局部细节时往往与原始图像在光照、纹理或形状上不一致。

Method: 采用两阶段参考驱动修正：首先微调单图像扩散编辑器，使其同时处理草稿图像和参考图像以实现全局一致性细化；然后应用强化学习进一步强化局部编辑能力，明确优化细节准确性和语义一致性。

Result: 在具有挑战性的参考引导修复基准测试中，该方法显著提高了参考对齐和细粒度细节保留能力，产生的忠实且视觉连贯的编辑效果超越了开源和商业模型。

Conclusion: 该方法通过细节感知的细化框架有效解决了参考引导图像生成中的细节保留问题，实现了更好的像素级一致性和视觉连贯性。
...
TL;DR: 本文提出了首个类型特定的创造力奖励模型CREward，涵盖几何、材质和纹理三个创造力维度，通过人类基准评估和大型视觉语言模型(LVLM)生成标签来训练模型，并在创造力评估、可解释创造力和创意样本获取三个应用场景中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 创造力是一个复杂现象，将其视为单一未分化的数量显得过于简单。需要从图像形成流程的角度，通过不同类型（几何、材质、纹理）来更细致地表示和评估创造力。

Method: 1）进行人类基准评估，捕捉人类对不同类型创意图像的感知；2）分析人类判断与LVLM预测的相关性；3）利用LVLM生成的标签训练CREward模型；4）在三个应用场景中验证模型：创造力评估、可解释创造力和创意样本获取。

Result: 研究发现LVLM与人类感知具有强对齐性，基于此训练的CREward模型能够有效应用于创意图像的评估和生成，支持通过低秩自适应引导创意生成，并为人类设计提供灵感。

Conclusion: CREward作为首个类型特定的创造力奖励模型，成功地将创造力分解为多个维度，并通过LVLM与人类感知的对齐性实现了有效的创造力评估和生成引导，为创意计算领域提供了新的工具和方法。
...
TL;DR: 本文揭示了一种新型对抗攻击威胁：单个扰动可以劫持整个决策链，同时操纵模型产生多个预定义错误输出，而不仅仅是单个决策错误。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的模型通常在一系列决策中运行，孤立错误容易被纠正，但级联错误可能导致严重风险。传统对抗攻击只关注操纵单个决策，无法应对这种级联威胁。

Method: 提出了语义感知通用扰动（SAUPs），通过归一化空间中的扰动搜索和语义分离策略来克服优化挑战，开发了有效的算法。

Result: 在三个多模态大语言模型上的广泛实验表明，仅使用一个对抗框架就能在控制五个不同目标时达到70%的攻击成功率。

Conclusion: 多模态大语言模型对SAUPs攻击存在严重脆弱性，单个扰动就能劫持整个决策链，这揭示了现实应用中新的安全威胁。
...
TL;DR: 本文提出了一种注意力引导的跨模态交互Transformer（ACIT）用于预测行人过街意图，通过六种视觉和运动模态的三组交互对，结合双路径注意力机制和Transformer时序特征聚合，在JAAD数据集上取得了优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 有效提取和整合不同类型数据中的互补线索是预测行人过街意图的主要挑战，这对于自动驾驶车辆预防与行人相关的碰撞至关重要。

Method: ACIT方法利用六种视觉和运动模态，分为三组交互对：全局语义图和全局光流、局部RGB图像和局部光流、自车速度和行人边界框。采用双路径注意力机制增强主要模态的显著区域，并通过光流引导注意力与辅助模态深度交互。使用跨模态注意力建模跨模态动态，并引入Transformer时序特征聚合模块捕捉序列依赖。

Result: ACIT在JAADbeh和JAADall数据集上的准确率分别达到70%和89%，优于现有最先进方法。消融研究验证了不同模块的贡献。
...
Method: 提出VGA训练免费方法：首先利用视觉令牌的语义内容构建精确的视觉基础，然后使用该基础引导模型关注相关视觉区域。在图像描述任务中，VGA在生成过程中动态细化引导，抑制已描述区域。

Result: VGA仅引入4.36%的延迟开销，与FlashAttention等高效注意力实现完全兼容。在多个MLLM和幻觉基准测试上的广泛实验表明，VGA实现了最先进的去幻觉性能。

Conclusion: 明确的视觉引导在增强MLLM的视觉理解能力中起着关键作用，VGA方法有效提升了模型的视觉定位和推理能力。
...
TL;DR: COVec是一个基于明暗对比原理的感知向量化框架，首次在向量域引入内在图像分解，将图像分离为反照率、阴影和光照层，通过语义引导初始化和两阶段优化实现高保真度和可编辑性。


<details>
  <summary>Details</summary>
Motivation: 现有向量化方法难以表示复杂的真实世界图像，通常以牺牲语义简洁性为代价产生碎片化形状。

Method: 引入内在图像分解到向量域，将图像分离为反照率、阴影和光照层；采用语义引导初始化和两阶段优化，结合可微分渲染。

Result: 在多个数据集上的实验表明，COVec相比现有方法实现了更高的视觉保真度和显著改进的可编辑性。

Conclusion: COVec通过明暗感知的向量化方法，在保持视觉质量的同时显著提升了向量表示的可编辑能力。
...
Conclusion: MFM-Point框架显著提升了基于点的点云生成方法的性能，证明了多尺度方法在保持简单性和效率的同时能够实现高质量生成。
...
TL;DR: HACBSR是一个无监督的盲超分辨率框架，无需地面真实图像和外部核先验，通过对比核采样和历史增强对比学习来解决行星遥感图像中的未知退化问题。
...
Method: HACBSR包含两个组件：(1) 带核相似性控制的对比核采样机制，减轻高斯采样的分布偏差；(2) 历史增强对比学习，使用历史模型生成负样本，实现较少贪婪优化并诱导强凸性。

Result: 实验表明HACBSR在多个放大因子下与最先进的无监督方法相比具有竞争力，并引入了Ceres-50数据集用于行星应用评估。

Conclusion: HACBSR为行星遥感图像的盲超分辨率提供了一种有效的无监督解决方案，无需地面真实图像即可处理未知退化问题。
...
### [73] [FLaTEC: Frequency-Disentangled Latent Triplanes for Efficient Compression of LiDAR Point Clouds](https://arxiv.org/abs/2511.20065)
...
Conclusion: FLaTEC通过频率感知机制有效解耦点云的低频和高频组件，实现了高压缩比下的高质量重建，显著优于现有方法。
...
### [74] [PRADA: Probability-Ratio-Based Attribution and Detection of Autoregressive-Generated Images](https://arxiv.org/abs/2511.20068)
...
<details>
  <summary>Details</summary>
Motivation: 自回归图像生成器能够生成逼真的图像，但目前缺乏专门针对这类生成图像的检测方法，需要可靠的技术来识别和归因AR生成的图像。

Method: PRADA方法通过检查模型对自回归标记序列的条件概率与无条件概率比率，利用特定模型的分数函数进行阈值检测和归因。

Result: 实验评估显示PRADA对8个类别到图像模型和4个文本到图像模型具有高度有效性。
...
### [75] [Learning Procedural-aware Video Representations through State-Grounded Hierarchy Unfolding](https://arxiv.org/abs/2511.20073)
*Jinghan Zhao,Yifei Huang,Feng Lu*

Main category: cs.CV

TL;DR: 本文提出了Task-Step-State (TSS)框架，通过引入'状态'作为视觉基础语义层，将抽象程序与可观察细节联系起来，改进了程序感知视频表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过任务和步骤级别的文本-视觉对齐来注入程序语义，但由于任务和步骤描述过于抽象，难以与视觉数据中的具体细节形成稳健对齐。

Method: 提出TSS框架，将任务分解为步骤，步骤驱动可观察状态之间的转换。采用渐进式预训练策略，逐步展开TSS层次结构，强制模型在状态基础上构建表示。

Result: 在COIN和CrossTask数据集上的实验表明，该方法在任务识别、步骤识别和下一步预测等多个下游任务上优于基线模型。消融研究显示状态监督是性能提升的关键驱动因素。

Conclusion: 引入状态作为视觉基础语义层能有效改进程序感知视频表示学习，渐进式预训练策略比标准联合训练更有效地强制执行预期的层次结构。
...
### [76] [Blind Adaptive Local Denoising for CEST Imaging](https://arxiv.org/abs/2511.20081)
*Chu Chen,Aitor Artola,Yang Liu,Se Weon Park,Raymond H. Chan,Jean-Michel Morel,Kannie W. Y. Chan*

Main category: cs.CV

TL;DR: 提出了一种新的盲自适应局部去噪方法（BALD），用于解决CEST MRI中的异方差噪声问题，通过自适应方差稳定变换和两阶段去噪来分离分子信号与噪声。


<details>
  <summary>Details</summary>
Motivation: CEST MRI在临床转化中面临空间变化噪声和复杂成像协议导致的异方差性挑战，传统去噪方法无法处理这种复杂噪声且会改变关键生物医学信息。
...
TL;DR: 本文提出CONVAD方法，将概念瓶颈模型扩展到视觉异常检测领域，通过概念学习提供人类可理解的异常描述，同时保持与传统VAD方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉异常检测模型虽然能提供视觉解释（高亮异常区域），但缺乏直接且语义上有意义的解释，限制了用户理解和信任。
...
Conclusion: 概念瓶颈模型成功应用于视觉异常检测，通过概念学习提供语义解释，为VAD系统提供了更深入的洞察力和用户信任。
...
TL;DR: WPT提出了一种世界模型到策略的转移训练范式，通过在线蒸馏在端到端世界模型指导下，将教师策略的推理能力转移到轻量级学生策略中，实现高性能规划同时保持实时部署能力。
...
Method: 开发可训练奖励模型将世界知识注入教师策略，通过策略蒸馏和世界奖励蒸馏将教师推理能力转移到学生策略。
...
TL;DR: 该论文提出了一种用于早期森林火灾检测的视觉分析数据集，通过游戏模拟器合成烟雾和初始火灾图像，结合现有数据集，并比较了YOLOv7和检测变换器模型的性能。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏大规模数据集和专门针对早期火灾检测的模型，现有方法存在漏检问题，需要开发早期预警系统来检测森林火灾的初始阶段。

Method: 使用游戏模拟器（如Red Dead Redemption 2）合成烟雾羽流和初始火灾图像，构建专门用于早期火灾识别的数据集，并与现有图像结合形成更全面的数据集，然后比较YOLOv7和不同检测变换器模型的图像分类和定位方法。

Result: 创建了一个包含多个烟雾羽流和初始火灾实例的数据集，不同于现有的大范围火灾图像库，该数据集专注于火灾起始阶段的检测。

Conclusion: 提出的合成数据集和模型比较方法为早期森林火灾检测提供了有效的解决方案，有助于改善现有方法的漏检问题。
...
TL;DR: 本文提出了一种结合深度学习和注意力机制的集成方法，用于从X射线图像中自动检测心脏肥大。该方法使用Inception V3模型和多头注意力机制，在准确率、精确度等指标上表现优异。


<details>
  <summary>Details</summary>
Motivation: 医疗影像技术革命性地改变了心血管疾病诊断方式，特别是心脏肥大等结构异常的检测。传统方法存在效率低、主观性强等问题，需要开发自动化的精准检测系统。

Method: 采用集成方法，包括数据收集、预处理、CNN配置（基于Inception V3模型）和多层注意力机制。关键特点是多头注意力机制，能够自动学习特征并选择性聚焦输入图像的特定区域。

Result: 模型在各项指标上表现优异：准确率95.6%、精确度95.2%、召回率96.2%、灵敏度95.7%、特异性96.1%、AUC值96.0%。

Conclusion: 该方法能够敏感地识别心脏肥大，具有临床意义，严格的评估验证了模型的有效性和可靠性。
...
TL;DR: LungEvaty是一个基于transformer的框架，用于从单次低剂量CT扫描预测1-6年肺癌风险，无需像素级注释，直接处理全肺输入，在大规模筛查数据上训练。


<details>
  <summary>Details</summary>
Motivation: 随着更多国家引入基于低剂量CT的肺癌筛查项目，需要可扩展的方法来处理不断增长的影像数据量，现有方法要么过度依赖像素级注释，要么将肺部分析为片段，限制了性能和可扩展性。

Method: 开发了完全基于transformer的LungEvaty框架，处理全肺输入，从大规模筛查数据中学习，捕获与恶性肿瘤风险相关的全面解剖和病理线索，无需区域监督，可选解剖感知注意力引导损失。

Result: 在超过90,000个CT扫描上训练（28,000个用于微调，6,000个用于评估），仅使用影像数据且无区域监督的情况下，LungEvaty达到了最先进的性能水平。
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散变换器在训练长度外推时存在周期性内容重复和普遍质量下降两个失败模式，现有方法仅通过位置编码解决重复问题，忽视了质量下降。

Method: 从注意力图角度分析问题，发现两个失败模式都源于注意力分散。提出UltraViCo方法，通过恒定衰减因子抑制训练窗口外token的注意力。

Result: 在4倍外推时，动态度和图像质量分别比之前最佳方法提升233%和40.5%，外推极限从2倍提升到4倍。
...
TL;DR: 提出了PETRG-3D框架，用于自动化PET/CT报告生成，解决功能成像的特殊挑战，并在多中心淋巴瘤数据集上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: PET/CT在肿瘤学中至关重要，但扫描仪快速扩张导致专业医生短缺，需要自动化报告生成来减轻临床工作负担。功能PET成像相比结构成像面临代谢模式变化和全身3D上下文信息需求的独特挑战。

Method: 提出PETRG-3D端到端3D双分支框架，分别编码PET和CT体积，并引入风格自适应提示来缓解医院间报告实践的变异性。构建了PETRG-Lym多中心淋巴瘤数据集和AutoPET-RG-Lym公开基准。
...
TL;DR: Restora-Flow是一种无需训练的基于流匹配的图像修复方法，通过退化掩码引导采样并加入轨迹校正机制，在自然和医学图像修复任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于流匹配的修复方法存在处理时间长或结果过度平滑的问题，需要一种既能保持高质量生成又能快速处理的方法。

Method: 使用退化掩码引导流匹配采样过程，并引入轨迹校正机制来确保与退化输入的一致性，无需额外训练。

Result: 在自然和医学数据集上的修复任务（修复、超分辨率和去噪）中，相比基于扩散和流匹配的参考方法，具有更好的感知质量和更快的处理时间。

Conclusion: Restora-Flow通过简单的掩码引导和轨迹校正机制，实现了高效且高质量的图像修复，为流匹配在图像恢复任务中的应用提供了有效解决方案。
...
TL;DR: 该论文提出了R-TNAG框架，通过黎曼流形映射、时间感知神经常微分方程和注意力机制黎曼门控循环单元，解决了不规则采样纵向sMRI数据中阿尔茨海默病进展建模的挑战。


<details>
  <summary>Details</summary>
Motivation: 临床检查的不确定性导致纵向成像数据观察间隔不规则，现有基于欧几里得空间的疾病预测模型无法充分捕捉不规则采样纵向图像的内在连续性和非线性几何结构。

Method: 首先将高维sMRI特征投影到流形空间以保留疾病进展的内在几何结构；然后使用时间感知神经常微分方程建模观测间潜在状态的连续演化；最后通过注意力机制黎曼门控循环单元自适应整合历史和当前信息处理不规则间隔。

Result: 实验结果表明该方法在疾病状态预测和认知评分回归方面持续优于最先进模型，消融研究验证了各模块的贡献，模型在不同序列长度和缺失数据率下表现稳定，交叉数据集验证确认了其鲁棒性。

Conclusion: 提出的R-TNAG框架通过联合设计提高了时间一致性，在不规则采样下实现了稳健的AD轨迹预测，在多样化临床环境中具有强适用性。
...
TL;DR: 研究探索大型多模态模型在视频中定位物理交互事件的能力，发现虽然模型能描述对象和动作，但无法准确定位交互开始/结束的时刻和位置。


<details>
  <summary>Details</summary>
Motivation: 评估大型多模态模型是否真正基于视觉输入来理解语义，特别是在手与物体交互的动态场景中定位关键事件的能力。

Method: 创建包含2万多个标注交互事件的数据集，使用250名人工标注者标注接触和释放事件，然后让两个LMM模型在短视频中定位这些事件。
...
<details>
  <summary>Details</summary>
Motivation: 现有持续视觉指令调优研究主要关注未安全对齐的模型，忽略了真实世界多模态大语言模型需要安全机制来降低潜在风险。当对安全对齐模型进行持续调优时，不仅会出现任务遗忘，还会导致安全性退化。

Method: 提出HPA后训练框架，包括基于关注度的参数划分、和谐平衡的参数选择和正交参数调整。将参数按关注安全或任务性能分为两类，从平衡角度选择要保留的关注参数，并对参数更新施加正交约束以减轻灾难性遗忘。

Result: 在持续视觉指令调优基准和安全评估数据集上的大量实验表明，HPA比现有基线更好地保持高安全性并减轻遗忘。

Conclusion: HPA框架能够有效平衡安全对齐多模态大语言模型在持续调优过程中的安全性和任务性能，为解决这一关键挑战提供了有效解决方案。
...
TL;DR: OmniAlpha是首个统一的序列到序列RGBA图像生成和编辑框架，通过创新的MSRoPE-BiL方法和AlphaLayers数据集，在21个多样化任务上超越专用模型。


<details>
  <summary>Details</summary>
Motivation: 解决RGBA操作领域的分裂现状：专用模型缺乏通用性，而统一框架局限于RGB领域，需要桥接这一关键差距。

Method: 提出OmniAlpha框架，采用MSRoPE-BiL（一种具有双向可扩展层轴的新型RoPE方法）的Diffusion Transformer架构，能够并发处理多个输入和输出RGBA层。构建AlphaLayers数据集，包含1000个高质量多层三元组。

Result: 在21个任务上的联合训练显示，统一方法持续优于专用基线。在AIM-500上的无掩码抠图任务中SAD相对降低84.8%，在层条件完成任务中赢得超过90%的人类偏好。
...
TL;DR: ADNet是一个大规模多领域异常检测基准，包含380个类别、196,294张RGB图像，涵盖电子、工业、农业食品、基础设施和医疗等多个领域。实验显示现有方法在扩展到所有类别时性能显著下降，作者提出Dinomaly-m方法来解决可扩展性问题。
...
Method: 构建ADNet基准，整合49个公开数据集，包含380个类别、196,294张标准化图像，提供像素级注释和结构化文本描述。提出Dinomaly-m方法，基于上下文引导的专家混合机制扩展解码器容量而不增加推理成本。

Result: 现有最先进方法在单类别设置下达到90.6% I-AUROC，但在扩展到380个类别的多类别设置下下降到78.5%。Dinomaly-m方法达到83.2% I-AUROC和93.1% P-AUROC，优于现有方法。

Conclusion: ADNet作为标准化可扩展的基准，支持社区在不同领域扩展异常检测数据集，为未来异常检测基础模型提供可扩展基础。该方法有效解决了大规模异常检测的可扩展性挑战。
...
TL;DR: Exo2EgoSyn是基于WAN 2.2的扩展框架，实现了从第三人称视角到第一人称视角的跨视图视频生成，包含三个关键模块：视角对齐、多视图条件化和姿态感知潜在注入。


<details>
  <summary>Details</summary>
Motivation: 现有的基础视频生成模型如WAN 2.2仅限于同视图生成，无法实现跨视角的视频合成，限制了其在多视角场景中的应用。

Method: 提出三个核心模块：EgoExo-Align实现视角间潜在空间对齐，MultiExoCon聚合多视图视频作为条件信号，PoseInj注入相机姿态信息指导几何感知合成。

Result: 在ExoEgo4D数据集上的实验表明，Exo2EgoSyn显著提升了Ego2Exo合成质量，无需从头训练即可实现高保真度的第一人称视角视频生成。

Conclusion: 该工作为基础模型的可扩展跨视图视频生成开辟了新途径，相关代码和模型将公开发布。
...
TL;DR: SFA是一个无需训练的视频文本视觉问答框架，通过自适应扫描视频帧、选择性关注关键区域并直接放大它们，引导Video-LLM注意力到关键线索，在多个数据集上达到新的最先进水平。


<details>
  <summary>Details</summary>
Motivation: 视频文本视觉问答任务面临重大挑战，需要模型准确感知和理解场景文本，有效整合时间和语义上下文，同时识别问题相关文本线索并过滤冗余信息。

Method: 提出SFA训练免费框架，基于人类回答问题过程，通过自适应扫描视频帧、选择性关注关键区域并直接放大它们，引导Video-LLM注意力到关键线索。

Result: SFA在多个公共视频文本视觉问答数据集上取得新的最先进结果，大幅超越先前方法，证明其有效性和泛化能力。

Conclusion: SFA作为首个基于Video-LLM的视频文本视觉问答方法，通过注意力引导机制有效解决了该任务的挑战，展现了优越性能。
...
TL;DR: 本文介绍了参加2025年ASNR-MICCAI BraTS-Inpainting挑战赛的获胜方案，提出了一种基于U-Net架构的深度学习框架，用于在3D脑肿瘤MRI扫描中合成健康组织，通过随机掩码增强策略提升泛化能力。
...
Method: 采用U-Net架构训练模型来修复合成损坏区域，并引入随机掩码增强策略以提高泛化能力。
...
Conclusion: 该方法在脑肿瘤MRI图像修复任务中表现出色，超越了2023和2024年的获胜方案，证明了其有效性和优越性。
...
TL;DR: 本文提出CT-CIG方法，通过可控文本引导生成逻辑合理的伪装图像，利用大视觉语言模型标注高质量文本提示，并设计频率交互细化模块提升伪装效果。
...
Result: 通过CLIPScore评估和伪装效果测试，证明生成的文本提示语义对齐，能产生逼真的伪装图像。
...
Conclusion: 该方法为基于ViT的组织病理学分析建立了坚实的基线，未来工作将重点解决在未见验证数据上观察到的性能差距。
...
Method: 提出V-Attack方法，包含两个核心组件：自值增强模块和文本引导的值操作模块，通过操作注意力机制中的值特征来避免语义纠缠。

Result: 在LLaVA、InternVL、DeepseekVL和GPT-4o等多种LVLM上的实验表明，V-Attack相比最先进方法平均提升36%的攻击成功率。

Conclusion: V-Attack揭示了现代视觉语言理解系统中的关键漏洞，值特征作为更精确的操控手柄能够实现有效的语义控制。
...
TL;DR: BoxPromptIML是一个弱监督图像篡改定位框架，通过粗粒度区域标注策略平衡标注成本和定位精度，使用知识蒸馏和双引导特征融合模块实现高效部署和精确定位。
...
Method: 提出粗粒度区域标注策略降低标注成本；设计轻量级学生模型通过知识蒸馏从SAM教师模型学习；采用基于人类潜意识记忆机制的双引导特征融合模块，动态结合原型模式和实时观察线索。

Result: 在分布内和分布外数据集上的广泛实验表明，BoxPromptIML性能优于或媲美全监督模型，同时保持强泛化性、低标注成本和高效部署特性。
...
TL;DR: HistoSpeckle-Net是一种深度学习架构，用于从多模光纤散斑重建结构丰富的医学图像，通过分布感知学习策略和双损失函数，在有限数据下实现高保真重建。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在MMF成像中主要关注简单数据集，难以应用于复杂的真实世界成像任务，且通常需要大量数据。

Method: 提出HistoSpeckle-Net架构，包含直方图计算单元和三级特征精炼模块，采用基于直方图的互信息损失和多尺度SSIM损失。

Result: 在复杂OrganAMNIST数据集上，HistoSpeckle-Net比U-Net和Pix2Pix等基线模型具有更高的保真度，在有限训练样本和不同光纤弯曲条件下表现优异。
...
TL;DR: 本文研究了文本到图像生成模型在长提示词下的表现，发现长提示词会提高保真度但降低多样性，提出了LPD-Bench评估基准和PromptMoG方法来增强多样性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型在长提示词下表现出保真度与多样性的矛盾，长提示词编码丰富信息但往往抑制多样性，导致输出重复且缺乏创意。

Method: 提出了LPD-Bench评估基准，建立了理论框架通过提示词重构增加采样熵，并提出了无训练方法PromptMoG，在嵌入空间中使用高斯混合模型采样提示词嵌入来增强多样性。

Result: 在SD3.5-Large、Flux.1-Krea-Dev、CogView4和Qwen-Image四个最先进模型上的广泛实验表明，PromptMoG能持续提升长提示词生成的多样性而不产生语义漂移。
...
Method: 提出两阶段匹配策略，使用新的Bbox-Based距离替代传统马氏距离，将视觉跟踪整合到卡尔曼滤波和整体跟踪流程中。

Result: 在MOT17-val数据集上，1Hz低频检测条件下HOTA指标提升11.6%，在标准MOT17、MOT20和DanceTrack基准测试中与最佳方法保持相当性能。

Conclusion: StableTrack方法有效解决了低频检测条件下的多目标跟踪问题，在保持标准检测频率性能的同时显著提升了低频场景的跟踪质量。
...
TL;DR: Zoo3D是首个无需训练的3D物体检测框架，通过2D实例掩码的图聚类构建3D边界框，并使用开放词汇模块分配语义标签，在零样本和自监督两种模式下均达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现实环境需要能够识别多样化、未见过的物体的模型，而现有的开放词汇3D检测器仍然依赖训练场景，这是封闭式方法的主要限制。

Method: 通过2D实例掩码的图聚类构建3D边界框，使用具有最佳视图选择和视图一致性掩码生成的开放词汇模块分配语义标签，提供零样本和自监督两种模式。

Result: 在ScanNet200和ARKitScenes基准测试中，Zoo3D的零样本和自监督版本均实现了开放词汇3D物体检测的最先进结果，零样本版本甚至超过了所有现有的自监督方法。

Conclusion: 训练即用的方法在真实世界3D理解中具有强大的能力和适应性，无需训练即可实现高性能的3D物体检测。
...
TL;DR: Adv-GRPO是一个对抗性强化学习框架，通过迭代更新奖励模型和生成器来解决传统标量奖励模型在图像生成中的局限性，使用视觉基础模型提供丰富的视觉奖励信号，显著提升图像质量和美学效果。


<details>
  <summary>Details</summary>
Motivation: 当前RL图像生成方法依赖预训练偏好模型输出的标量奖励，但这些奖励往往无法准确捕捉人类感知，容易受到奖励攻击，导致高分并不对应更好的图像质量。

Method: 提出Adv-GRPO框架，采用对抗性奖励机制，使用参考图像作为正样本监督奖励模型，避免奖励攻击。利用视觉基础模型（如DINO）提供密集视觉奖励信号，而非单一标量奖励。结合参考样本和基础模型奖励实现分布迁移和风格定制。

Result: 在人类评估中，Adv-GRPO在图像质量和美学方面分别以70.0%和72.4%的胜率优于Flow-GRPO和SD3。方法在图像质量、美学和任务特定指标上均取得一致提升。

Conclusion: Adv-GRPO通过对抗性奖励机制和视觉基础模型的密集奖励信号，有效解决了传统标量奖励模型的局限性，显著提升了RL图像生成的性能和质量。
...
TL;DR: MBCD是一个统一协作蒸馏框架，通过自适应模态丢弃、梯度一致性约束和基于权重平均的跨模态蒸馏，解决多模态域泛化中权重平均导致的模态优化不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 权重平均在多模态域泛化中面临挑战：不同模态优化速度差异导致早期偏向快速收敛模态，抑制慢速但互补模态的贡献，阻碍有效模态融合并导致损失曲面偏向尖锐、泛化性差的最小值。
...
Conclusion: MBCD保留了权重平均的平坦性优势，同时克服了其在多模态环境中的缺点，通过协调优化和增强跨模态交互，引导收敛到更平坦的解决方案。
...
TL;DR: DiDiCM是一个新颖的扩散分类框架，通过建模类别标签的后验分布来提高图像分类在不确定性条件下的性能，在ImageNet数据集上优于标准分类器。


<details>
  <summary>Details</summary>
Motivation: 解决传统分类方法在输入图像损坏或训练数据有限等高不确定性条件下的性能不佳问题。

Method: 提出离散扩散分类建模(DiDiCM)，利用扩散过程建模类别标签的条件后验分布，支持在类别概率或离散类别标签上进行扩散预测。

Result: 在ImageNet数据集上，仅需少量扩散迭代就能获得比基线更高的分类准确率，且随着任务难度增加，准确率提升更明显。

Conclusion: DiDiCM框架在挑战性分类任务中表现出色，为高不确定性条件下的图像分类提供了有效的解决方案。
...
TL;DR: 提出了一种半监督深度强化学习框架，通过自适应采样、自动编码器和预测器的协同工作，在工业视觉检测中实现了对细微缺陷的准确检测和定位。
...
Method: 采用深度强化学习框架，包含神经批量采样器（平衡探索与利用）、自动编码器（生成损失分布图）和预测器（在损失分布空间进行分割）。

Result: 在MVTec AD数据集上，相比现有最优方法，F1_max平均提升0.15，AUC平均提升0.06，最佳情况下F1_max最大提升0.37。

Conclusion: 该方法能够在有限标注数据下有效学习正常和缺陷模式，实现更高的检测精度和更好的细微异常定位性能，同时保持低复杂度。
...
TL;DR: 本文提出了VKnowU基准来评估多模态大语言模型(MLLMs)的视觉知识能力，发现现有模型在理解物理和社会原理方面仍落后于人类。作者开发了VideoKnow+模型，通过结构化思维和强化学习显著提升了视觉知识理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs擅长物体识别，但缺乏对人类世界物理和社会原理的直观理解。这种视觉知识能力是连接感知和推理的关键，但在现有研究中尚未得到充分探索。

Method: 构建了VKnowU基准(1,680个问题，1,249个视频)，涵盖8类视觉知识。开发了VideoKnow+模型，采用See-Think-Answer结构化范式，并使用视觉知识奖励的强化学习。

Result: 评估23个SOTA MLLMs显示领先模型仍落后于人类，特别是在世界中心型知识方面。VideoKnow+在VKnowU上提升3.7%，在MVBench、Video-MME和MMVU上也有持续改进。

Conclusion: 视觉知识是开发更通用MLLMs的关键基石，使模型不仅能看，还能真正理解物理和社会世界。VideoKnow+的成功证明了显式整合视觉知识的有效性。
...
TL;DR: ScenarioCLIP提出了一种新的CLIP模型，专门用于处理具有丰富组合结构的真实场景图像，通过显式建模对象间关系和组合结构来改进场景分析。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP模型主要处理单对象分类或短描述检索，但真实场景图像包含多个对象和动作的复杂组合结构，现有方法缺乏对关系和组合结构的显式建模。

Method: ScenarioCLIP接受输入文本、接地关系、输入图像以及突出关系的聚焦区域，在精心策划的场景数据上进行预训练，并在特定下游任务上进行微调。构建了新的数据集扩展现有图像-文本对，使用语言模型管道来接地动作、对象和关系。
...
TL;DR: SelfMOTR是一种新颖的跟踪转换器，利用自生成的检测先验来解决端到端跟踪中检测性能差和检测与关联冲突的问题，在DanceTrack上实现了与最先进方法相竞争的性能。
...
Method: 受集成检测先验成功的启发，以及发现MOTR类模型实际上是强大的检测模型的关键洞察，作者提出了SelfMOTR——一种依赖自生成检测先验的新型跟踪转换器。通过广泛分析和消融研究，揭示并展示了MOTR类模型的隐藏检测能力，并提供了有效利用这些能力的实用工具集。

Result: 在DanceTrack数据集上，SelfMOTR实现了强大的性能，与最近最先进的端到端跟踪方法相竞争。

Conclusion: SelfMOTR通过利用自生成的检测先验，成功解决了端到端跟踪中的关键挑战，证明了MOTR类模型具有隐藏的强大检测能力，并提供了有效利用这些能力的方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型虽然在视觉质量上取得了显著进展，但在遵循真实世界物理原理方面仍存在不足，需要提升物理一致性。
...
TL;DR: 提出了CrossEarth-Gate方法，通过建立遥感模块工具箱和Fisher引导的自适应选择机制，解决遥感数据中多面域差距问题，在16个跨域基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有专用PEFT方法在处理大规模地球观测任务时往往失效，因为它们无法完全处理遥感数据中固有的多面性和不可预测的域差距（如空间、语义和频率偏移）。

Method: 建立包含空间、语义和频率模块的遥感模块工具箱，开发基于Fisher信息的自适应选择机制，动态激活最关键模块以最大化适应效果和效率。

Result: 在16个遥感语义分割跨域基准测试中达到最先进性能，验证了方法的有效性和泛化能力。

Conclusion: CrossEarth-Gate通过综合模块工具箱和自适应选择机制，成功解决了遥感数据中的多面域差距问题，为参数高效微调提供了有效解决方案。
...
TL;DR: TReFT是一种新方法，用于驯化整流流模型实现一步图像翻译，解决了在对抗训练下收敛问题，实现实时推理。


<details>
  <summary>Details</summary>
Motivation: 虽然整流流模型在图像和视频合成方面取得进展，但在图像翻译中仍依赖昂贵的多步去噪，阻碍实时应用。直接应用CycleGAN-Turbo到RF模型会导致严重收敛问题。

Method: TReFT直接使用预训练DiT或UNet预测的速度作为输出，这一简单设计解决了对抗训练下的收敛问题。引入内存高效的潜在循环一致性和身份损失，以及轻量级架构简化。
...
TL;DR: 提出IrisNet框架，通过元学习动态适应不同红外场景，使用图像到解码器的变换器生成自适应解码参数，集成高频组件增强感知能力，在多个数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决红外小目标检测中由于低信噪比、复杂背景和缺乏可识别特征导致的挑战，以及静态模式学习在不同场景（如昼夜变化、天空/海洋/地面领域）中模式漂移的问题。

Method: 建立红外图像特征与整个解码器参数之间的动态映射，通过图像到解码器的变换器实现；将参数化解码器表示为结构化2D张量以保持层次层相关性；通过自注意力建模层间依赖关系，通过交叉注意力生成自适应解码模式；集成高频组件补充目标位置和场景边缘信息。

Result: 在NUDT-SIRST、NUAA-SIRST和IRSTD-1K数据集上的实验表明，IrisNet实现了最先进的性能。

Conclusion: IrisNet通过动态适应不同红外图像状态的检测策略，有效解决了红外小目标检测中的模式漂移问题，提高了检测的鲁棒性。
...
TL;DR: AMB3R是一个用于密集3D重建的多视图前馈模型，能够在度量尺度上处理多样化的3D视觉任务。该模型使用稀疏但紧凑的体素场景表示作为后端，仅需多视图重建训练即可扩展到未标定视觉里程计和大规模运动恢复结构任务。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在3D重建任务中需要任务特定微调或测试时优化的问题，开发一个统一的模型来处理多个3D视觉任务。

Method: 利用稀疏紧凑的体素场景表示作为后端，实现几何推理和空间紧凑性。模型仅通过多视图重建训练，即可扩展到其他任务而无需额外调整。

Result: 在相机姿态、深度和度量尺度估计、3D重建方面达到最先进性能，甚至超过基于优化的SLAM和SfM方法，在常见基准测试中表现出色。

Conclusion: AMB3R证明了单一模型可以统一处理多个3D视觉任务，无需任务特定优化，在多个基准测试中超越了现有方法。
...
TL;DR: 本文提出了人形视觉搜索任务，让智能体在360°全景图像中主动旋转头部来搜索物体或路径，并构建了H* Bench基准测试来评估在复杂真实场景中的视觉搜索能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉搜索方法局限于静态图像，忽略了物理体现与3D世界的交互。为了开发与人类一样高效的具身视觉搜索智能体，同时规避真实世界硬件的限制。

Method: 提出人形视觉搜索框架，使用360°全景图像模拟沉浸式环境，通过后训练技术增强开源模型Qwen2.5-VL的性能。

Result: 实验显示顶级专有模型在物体和路径搜索中成功率仅约30%。通过后训练，Qwen2.5-VL的物体搜索成功率从14.83%提升至47.38%，路径搜索从6.44%提升至24.94%。

Conclusion: 路径搜索的较低成功率揭示了其固有的难度，需要复杂的空间常识。研究展示了构建可无缝融入日常生活的MLLM智能体的前进方向，但也量化了面临的巨大挑战。
...
TL;DR: GS-Checker是一种用于定位3D高斯泼溅模型中篡改区域的新方法，通过集成3D篡改属性、3D对比机制和循环优化策略来实现无需昂贵3D标签的篡改定位。
...
Method: 方法包括：1）在3D高斯参数中集成3D篡改属性来指示高斯是否被篡改；2）设计3D对比机制，通过比较3D高斯之间关键属性的相似性来寻找篡改线索；3）引入循环优化策略来细化3D篡改属性。
...
TL;DR: VGGTFace是一种从多视角图像自动重建拓扑一致面部几何的方法，利用3D基础模型VGGT实现快速高质量重建，在10秒内处理16个视图。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要繁琐的手动操作、缺乏对野外数据的泛化能力，或受限于3D形变模型的表达能力。本文旨在解决这些限制，实现自动化的拓扑一致面部重建。

Method: 结合VGGT和Pixel3DMM，通过像素对齐的UV值注入拓扑信息，将VGGT的点图转换为带拓扑的点云，并提出拓扑感知的束调整策略进行融合。

Result: 在单个NVIDIA RTX 4090上10秒内完成16个视图的高质量重建，在基准测试中达到最先进结果，对野外数据具有出色的泛化能力。

Conclusion: VGGTFace成功实现了自动化的拓扑一致面部几何重建，结合了3D基础模型的泛化能力和拓扑信息的注入，为数字头像创建提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: DiTs虽然生成质量优秀，但由于需要长序列去噪轨迹导致推理延迟高。现有的推测推理方法在DiTs上加速有限，因为验证阶段的草稿准确性不足。
...
Conclusion: FREE框架通过特征级自回归和不确定性引导策略，有效解决了DiTs推理延迟问题，实现了显著加速且保持生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 多ID定制相比单ID定制更加困难，面临两个主要挑战：1）由于模型训练时基于裁剪的人物区域重建图像，推理时容易出现复制粘贴问题导致质量下降；2）模型文本控制性较差，生成结果只是简单组合多个人物，与输入文本对齐度低。

Method: 提出MultiID方法，采用ID解耦交叉注意力机制将不同ID嵌入注入对应图像区域；引入三种关键策略增强控制性：局部提示、深度引导空间控制和扩展自注意力；构建IDBench基准进行评估。

Result: 广泛的定性和定量结果表明MultiID能有效解决上述两个挑战，其性能与基于训练的多ID定制方法相当甚至更好。

Conclusion: MultiID在无需训练的情况下成功实现了多ID定制，通过创新的注意力机制和控制策略显著提升了生成质量和文本对齐度。
...
TL;DR: TBCM是一种无需外部训练数据的轨迹反向一致性模型，通过直接从教师模型的生成轨迹中提取潜在表示，显著提高了扩散模型蒸馏的效率和简单性。
...
Method: 提出自包含的蒸馏范式，无需VAE编码和大规模数据集，直接从教师模型的生成轨迹中提取潜在表示来消除对外部训练数据的依赖。

Result: 在MJHQ-30k数据集上，一步生成达到6.52 FID和28.08 CLIP分数，训练时间比Sana-Sprint减少约40%，并节省大量GPU内存。

Conclusion: TBCM展示了在不牺牲质量的情况下实现卓越效率的潜力，并揭示了连续时间一致性蒸馏中的扩散-生成空间差异问题，为未来蒸馏研究提供了见解。
...
TL;DR: 提出了两种混合算法来解决多跳知识图谱问答中的效率和可验证性问题：LLM引导规划使用单次LLM调用预测关系序列，实现高准确率且答案可验证；嵌入引导神经搜索完全消除LLM调用，通过轻量级边评分器实现100倍加速。
...
Method: 1. LLM引导规划：单次LLM调用预测关系序列，通过广度优先搜索执行；2. 嵌入引导神经搜索：融合文本和图嵌入的轻量级边评分器；3. 知识蒸馏：将规划能力压缩到40亿参数模型中。

Result: LLM引导规划实现接近完美的准确率（micro-F1 > 0.90），嵌入引导神经搜索实现100倍加速且保持竞争力准确率，知识蒸馏模型在零API成本下匹配大模型性能。
...
TL;DR: 评估大型语言模型在波斯语情感分类任务中生成解释的忠实性，通过比较模型和人类标注者识别的重要词汇，发现模型解释与真实推理存在偏差。
...
Method: 通过比较模型识别的影响词汇与人类标注者的识别结果，使用基于token级对数概率的置信度评分来评估忠实性，并测试两种提示策略（先预测后解释 vs 先解释后预测）对解释忠实性的影响。
...
Conclusion: 研究结果挑战了大型语言模型必然产生更好领域特定嵌入的假设，为临床NLP系统开发提供了实用指导，所有模型和数据集均已公开以支持可重复研究。
...
<details>
  <summary>Details</summary>
Motivation: 探索语言理解的深层认知和神经机制，超越表面意义提取，研究大脑如何构建情境的心理模型。

Method: 回顾现有证据，利用认知神经科学的概念基础和方法直接测试语言理解需要信息从语言系统导出到其他大脑区域的假设。

Result: 论文提出了一个理论框架，认为深度语言理解需要大脑其他区域参与计算感知、运动表征、构建心理模型和存储知识记忆。

Conclusion: 认知神经科学的进展为揭示语言理解的认知和神经基础提供了新策略，支持语言理解是跨脑区协作过程的观点。
...
TL;DR: 评估大语言模型在情感心智理论中的性别偏见，并比较不同去偏见策略的效果。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在日常生活中的广泛应用，评估和确保其公平性变得尤为重要。本研究关注情感心智理论领域的性别偏见问题。

Method: 通过向模型描述人物及其环境，询问"这个人感觉如何？"来评估性别偏见，并测试多种去偏见策略，包括基于训练和基于提示的方法。

Result: 研究发现，实现有意义的偏见减少需要基于训练的方法，而仅依赖推理时的提示工程等方法是无效的。

Conclusion: 在情感心智理论领域，大语言模型存在性别偏见，有效的去偏见需要训练层面的干预措施。
...
TL;DR: 本文提出了NRC VAD词典v2版本，新增了1万个英语多词表达及其组成词的效价、唤醒度和支配度人工评分，并将单字词覆盖范围扩展到2.5万个词，显著扩展了2018年发布的v1版本。


<details>
  <summary>Details</summary>
Motivation: 现有词典如2018年发布的NRC VAD词典只包含单词的VAD关联评分，缺乏对多词表达的情感分析支持，且需要更新近年来更常见的词汇。

Method: 通过人工评分收集1万个多词表达及其组成词的效价、唤醒度和支配度数据，并扩展单字词覆盖范围，构建了包含多词表达和单词的增强版词典。

Result: 新词典包含1万个多词表达和2.5万个单词，评分具有高可靠性。研究发现多词表达表现出强烈的情感性，并探讨了多词表达的情感组合性程度。

Conclusion: NRC VAD词典v2为NLP、心理学、公共卫生、数字人文和社会科学等多个领域的研究提供了重要资源，支持更广泛的情感分析应用。
...
<details>
  <summary>Details</summary>
Motivation: 非洲语言作为低资源语言缺乏标注数据，手动标注耗时且昂贵，需要自动高效的标注流程来减少人工工作量。

Method: 利用包含情感信息的表情符号和词汇进行语言无关的自动情感标注。

Result: 在SAfriSenti多语言情感语料库的英语、Sepedi和Setswana推文上，标注准确率分别为66%、69%和63%。

Conclusion: 该方法能有效自动标注低资源语言的情感数据，显著减少人工标注工作量。
...
TL;DR: 提出了PersonaPulse框架，通过LLM内在人格知识迭代优化角色扮演提示，结合情境响应基准作为评分工具，实现更真实的人格表达。
...
Method: PersonaPulse框架利用LLM对人格特质的内在知识，迭代增强角色扮演提示，同时整合情境响应基准作为评分工具，确保更真实和情境化的评估。

Result: 定量评估显示PersonaPulse生成的提示优于基于心理学研究设计的先前工作；探索了模型大小与人格建模的关系；发现某些人格特质的激发程度可通过暂停优化过程部分控制。

Conclusion: 提示优化在塑造LLM中人格表达方面的重要性，为未来自适应AI交互研究提供了宝贵见解。
...
<details>
  <summary>Details</summary>
Motivation: 临床文档中存在可能影响患者安全的事实性、诊断性和管理性错误，需要探索LLMs在不同提示策略下检测和纠正这些错误的能力。

Method: 使用MEDEC数据集评估9个指令调优的LLM，采用零样本提示、静态随机示例提示和检索增强动态提示三种策略，测量准确性、召回率、假阳性率等指标。

Result: 检索增强动态提示在所有9个LLM中将假阳性率降低约15%，在错误句子检测中召回率提高5-10%，并生成更准确的修正。

Conclusion: 检索增强动态提示在多样化LLM中优于零样本和静态提示，使用检索示例可提高检测准确性、减少假阳性并增强医疗错误纠正的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 通用重排序模型在金融、法律等高风险领域缺乏领域特定知识，而简单微调会导致表面形式过拟合和灾难性遗忘，需要一种更有效的领域专业化方法。

Method: 提出R2R框架，结合动态专家路由和两阶段训练策略（EAG）。EAG通过掩码最具预测性的表面线索引入反捷径机制，迫使模型学习领域不变的相关性模式。使用轻量级潜在语义路由器从冻结的解码器中选择最优LoRA专家。

Result: 在多个重排序骨干模型和不同领域（法律、医疗、金融）上的广泛实验表明，R2R始终优于通用模型和单领域微调基线。

Conclusion: R2R是一种模型无关的模块化领域专业化方法，具有强大的跨领域鲁棒性。
...
TL;DR: 该论文提出了一种结合图像数据增强技术的网格标记方法，用于解决不连续命名实体识别中的分割和遗漏问题，在多个数据集上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统命名实体识别方法在处理跨句子的不连续实体时存在分割错误和遗漏问题，严重影响识别准确性，因此需要改进现有方法。
...
TL;DR: 论文通过构建完全合成的基准测试，揭示了因果Transformer在方向学习上存在固有偏差，即使在没有语言先验的情况下，逆向映射仍然比正向映射更难优化。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer架构中方向性失败的根本原因：是源于语言统计特性还是架构本身。通过构建无语义干扰的合成基准来隔离这一问题。

Method: 使用可调分支因子K的随机字符串映射构建基准，创建零条件熵的正向任务和具有确定熵下限的逆向任务，比较GPT-2和MLP在不同初始化条件下的性能差异。

Result: 发现scratch训练的GPT-2模型存在显著的方向优化差距（如K=5时1.16 nats），远大于MLP；预训练初始化改变优化行为但未消除差距；LoRA在高熵逆向映射中遇到容量瓶颈。

Conclusion: 因果Transformer训练中存在固有的方向摩擦，这种偏差在移除语言先验、词频和语料级时间不对称性后仍然存在，需要更深入的机制研究。
...
TL;DR: 提出了Online-PVLM框架，通过双曲表示实现个性化视觉语言模型的在线概念学习，无需训练即可在测试时生成概念嵌入，解决了现有方法需要为每个新概念学习单独嵌入且无法支持实时适应的问题。


<details>
  <summary>Details</summary>
Motivation: 现有个性化视觉语言模型方法需要为每个新概念学习单独的嵌入，无法在测试时进行实时适应，这在大规模场景中尤为突出，无法实现概念嵌入的高效检索。

Method: 提出Online-PVLM框架，利用双曲表示实现在线概念学习，采用免训练范式在测试时生成概念嵌入，使个性化视觉语言模型的使用既具有可扩展性又高效。

Result: 开发了OP-Eval基准测试，包含1,292个概念和超过30K高质量实例，涵盖多样化问题类型。大量实验证明了所提框架的最先进性能。

Conclusion: Online-PVLM框架通过双曲表示实现了高效、可扩展的在线概念学习，解决了现有方法的实时适应限制，在大规模场景中表现出色。
...
Method: MTA框架：1)构建共享元LoRA银行，预训练元个性化特征；2)自适应LoRA融合，动态合并相关锚点元LoRA；3)少样本个性化LoRA堆叠，在合并LoRA基础上添加超低秩轻量LoRA模块进行微调。

Result: 在LaMP基准测试上的广泛实验表明，该方法在多个任务上优于现有最先进方法。

Conclusion: MTA框架有效解决了PLLMs的可扩展性和稀疏数据个性化问题，通过动态融合和轻量级适应实现了高效且灵活的个性化。
...
TL;DR: BiasPrompting是一个新颖的推理框架，通过引导LLMs生成和批判性评估所有可能答案选项的推理来提升多选题任务性能。
...
TL;DR: SSA是一种统一的稀疏注意力训练框架，通过双向对齐机制解决现有稀疏注意力方法的梯度更新缺陷问题，在保持稀疏性的同时实现与全注意力相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏注意力方法存在一个关键悖论：虽然旨在近似全注意力，但产生的注意力稀疏度反而比全注意力模型更低，这限制了其有效性。作者将此归因于梯度更新缺陷——在稀疏训练中被排除的低秩键值对既没有前向贡献也没有反向梯度，因此无法学习适当的抑制。
...
Conclusion: 原生稀疏注意力训练通过减轻汇点区域注意力值的过度分配，意外地改善了长上下文外推能力，其中SSA表现出最强的外推能力。SSA框架有效解决了稀疏注意力训练中的梯度更新问题，实现了性能与稀疏性的平衡。
...
TL;DR: EM2LDL是一个多语言语音语料库，用于通过标签分布学习进行混合情感识别，包含英语、普通话和粤语的表达性话语，捕捉语码转换现象，并标注了32个情感类别的细粒度分布。


<details>
  <summary>Details</summary>
Motivation: 解决现有语料库主要局限于单语言和单标签情感识别的问题，这些语料库缺乏语言多样性、无法建模混合情感且缺乏生态效度。

Method: 构建包含英语、普通话和粤语的多语言语料库，整合来自在线平台的自发情感表达，使用自监督学习模型（如HuBERT-large-EN）进行实验基线评估。

Result: 实验基线显示在独立于说话者的性别、年龄和个性评估中表现稳健，HuBERT-large-EN模型取得了最佳结果。
...
TL;DR: 提出了一种基于检索技术的免训练框架，用于语音误读检测和诊断，无需音素级建模或额外训练，在L2-ARCTIC数据集上取得了69.60%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 传统的误读检测和诊断方法需要评分模型或音素级模型训练，过程复杂且资源消耗大，因此需要一种更简单有效的替代方案。

Method: 利用预训练的自动语音识别模型，结合检索技术构建免训练框架，避免了音素特定建模和任务特定训练。

Result: 在L2-ARCTIC数据集上的实验表明，该方法达到了69.60%的F1分数，优于传统方法，同时避免了模型训练的复杂性。

Conclusion: 所提出的免训练框架能够有效进行语音误读检测和诊断，在保持准确性的同时显著简化了实现过程。
...
<details>
  <summary>Details</summary>
Motivation: 印地语系语言的语法错误纠正面临资源有限、语言多样性和复杂形态学的挑战，而现有方法主要针对英语等高资源语言，需要探索适用于低资源语言的解决方案。

Method: 采用基于提示的方法，结合零样本和少样本策略，使用最先进的大型语言模型进行语法错误纠正，并通过精心设计的提示和轻量级适配来提升性能。
...
TL;DR: 开发了首个公开的吉尔吉斯语单语BERT模型KyrgyzBERT，包含35.9M参数和定制分词器，在情感分析任务上表现优于5倍大的mBERT模型。
...
Method: 构建KyrgyzBERT单语BERT模型，使用定制分词器适应吉尔吉斯语形态结构，并通过翻译和人工标注创建情感分析基准数据集kyrgyz-sst2。

Result: KyrgyzBERT在情感分析任务上达到0.8280的F1分数，与比其大5倍的mBERT模型性能相当。

Conclusion: KyrgyzBERT为吉尔吉斯语NLP研究提供了有效的基础工具，所有模型、数据和代码均已公开。
...
TL;DR: REFLEX是一种基于大语言模型的自动事实核查系统，通过角色扮演对话和对比激活对提取，实现高效准确的事实核查和可解释性，仅需465个训练样本即可达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于大语言模型的事实核查系统过度依赖外部知识源导致的延迟、幻觉问题，以及可靠性和可解释性不足的挑战，满足实时使用的需求。

Method: 提出REFLEX范式，将事实核查重新定义为角色扮演对话，联合训练判决预测和解释生成。通过自适应提取骨干模型与其微调变体之间的对比激活对，构建解耦真相风格和实质的引导向量。

Result: 在真实世界数据集上的实验表明，REFLEX优于先前方法，仅用465个自精炼训练样本就实现了最先进性能。带有解释目标的模型可以有效指导无解释的模型，性能提升达7.57%。

Conclusion: REFLEX通过利用模型内部知识，在事实核查任务中实现了更忠实和高效的推理，证明了内部解释信号在解释和增强事实推理中的双重作用。
...
<details>
  <summary>Details</summary>
Motivation: 当前推测解码方法假设有大量可用计算能力，但在主流模型推理系统中，批处理等方法的广泛应用压缩了可用空闲计算能力，因此需要低验证资源和低调度成本的推测解码方法。
...
Conclusion: SpecFormer通过创新的架构设计，在保持推理质量的同时实现了高效加速，特别适合大规模批处理场景。
...
TL;DR: 本文探讨了大型语言模型在类比推理中的能力，发现LLMs能够编码类比实体间的基本关系，但在将关系信息应用于新实体时存在困难，与人类认知相比存在差距。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs是否能够编码高层次关系概念并将其应用于新情境，以理解模型在类比推理方面的认知能力。

Method: 使用比例类比和故事类比任务，分析LLMs在编码和应用关系概念时的表现，通过隐藏表示修补技术研究信息传递机制。

Result: LLMs能够有效编码类比实体间的关系，但在应用关系信息到新实体时存在困难；成功案例显示强烈的结构对齐，失败案例则显示对齐退化或错位。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在广泛的多语言基准测试中表现出色，但在比喻和文化推理方面，尤其是在低资源语境下，评估仍然不足。
...
TL;DR: 本文提出了一种新颖的面向任务的词干提取评估框架，通过SES、MPD和ANLD三个指标综合评估词干提取方法的效用、下游任务影响和语义相似性。
...
Method: 提出包含三个方面的评估框架：(1)使用词干提取有效性分数(SES)评估效用，(2)使用模型性能增量(MPD)评估下游任务影响，(3)使用平均归一化Levenshtein距离(ANLD)评估语义相似性。
...
Method: 提出采样自由的扩散框架iRadioDiff，通过接入点位置、材料反射和透射系数的物理信息提示，以及衍射点、强透射边界和视距轮廓等多路径关键先验来引导生成过程。

Result: 实验表明iRadioDiff在室内无线电地图构建和基于接收信号强度的室内定位中达到最先进性能，并在不同布局和材料配置下具有良好的泛化能力。
...
### [145] [Communication-Efficient Learning for Satellite Constellations](https://arxiv.org/abs/2511.20220)
*Ruxandra-Stefania Tudose,Moritz H. W. Grüss,Grace Ra Kim,Karl H. Johansson,Nicola Bastianello*

Main category: cs.LG

TL;DR: 提出了一种用于卫星星座联邦学习的新型通信高效算法，通过本地训练、压缩和误差反馈机制减少与地面站的通信次数和规模，在真实空间场景中展现出优越性能。
...
Method: 设计了一种通信高效的联邦学习算法，采用本地训练减少通信次数，使用压缩技术减小通信规模，并提出误差反馈机制提高准确性，该机制还可作为算法无关的方案广泛应用。

Result: 通过理论分析证明了算法的收敛性，并在真实空间场景的模拟中与现有技术进行比较，显示出优越的性能表现。

Conclusion: 所提出的算法在卫星星座联邦学习中实现了通信效率与模型准确性的良好平衡，误差反馈机制具有广泛的适用性，为空间环境下的分布式学习提供了有效解决方案。
...
TL;DR: 提出了基于部分信息分解(PID)的框架，通过分解内部嵌入中的预测信息为独特、冗余和协同成分来量化模态贡献，并开发了基于迭代比例拟合程序(IPFP)的算法进行可扩展分析。


<details>
  <summary>Details</summary>
Motivation: 现有基于准确性的方法无法区分模态是本身具有信息量还是仅通过与其他模态交互产生价值，特别是在跨注意力架构中模态相互影响表示的情况下，需要更清晰的量化方法。

Method: 使用部分信息分解(PID)框架，将预测信息分解为独特、冗余和协同成分；开发基于迭代比例拟合程序(IPFP)的算法，无需重新训练即可计算层和数据集级别的贡献。

Result: 提供了原则性的、表示级别的多模态行为视图，相比基于结果的指标提供了更清晰和可解释的见解。

Conclusion: 该框架能够更准确地量化模态贡献，区分模态的固有信息价值与交互价值，为多模态模型分析提供了新的视角。
...
TL;DR: PrefixGPT是一种生成式预训练Transformer，能够从零开始直接生成优化的前缀加法器，通过二维坐标序列表示拓扑结构，在生成过程中应用合法性掩码确保设计有效性。
...
Method: 使用自定义的解码器Transformer架构，首先在随机合成的有效前缀加法器语料库上进行预训练学习设计规则，然后微调以在设计空间中导航优化设计质量。

Result: 相比现有工作，PrefixGPT不仅找到了一个面积延迟乘积（ADP）提高7.7%的新最优设计，而且表现出优越的探索质量，将平均ADP降低了高达79.1%。
...
<details>
  <summary>Details</summary>
Motivation: 解决主流去噪策略（标准扩散和块扩散）在文本生成中的问题：标准扩散容易过早结束序列，块扩散会破坏连贯语义单元。

Method: 提出WavefrontDiffusion方法，动态扩展活动令牌波前，遵循语义结构的自然流动，同时保持计算效率。
...
Method: 开发了专家归因框架识别特定任务的关键专家子集，评估修剪和重新对齐这些专家的性能权衡，使用主动学习驱动的微调方法。
...
TL;DR: 本文提出了一种基于特征工程和粒子群优化的XGBoost机器学习回归模型，用于优化检索增强生成系统中的检索质量，通过实验验证了该模型在预测精度和稳定性方面的优越性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，检索增强生成技术被广泛应用，但系统性能高度依赖检索模块质量。现有模型在处理表格特征时存在性能瓶颈，需要改进检索质量以提升生成效果。

Method: 提出基于特征工程和粒子群优化的XGBoost机器学习回归模型，通过相关性分析发现文档相关性与答案质量显著正相关，语义相似度、冗余度与多样性存在权衡关系。

Result: 实验结果显示，VMD PSO BiLSTM模型在所有评估指标上均优于决策树、AdaBoost等对比模型，MSE、RMSE、MAE和MAPE显著降低，R2值更高，表明其预测精度、稳定性和数据解释能力更突出。
...
TL;DR: OmniTFT是一个基于Temporal Fusion Transformer的深度学习框架，用于联合预测ICU中的高频生命体征和稀疏采样的实验室结果，通过四种创新策略提升性能，在多个数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: ICU中的生命体征数据存在噪声和快速波动，实验室结果存在缺失值、测量延迟和设备偏差等问题，使得综合预测极具挑战性。

Method: 提出OmniTFT框架，采用滑动窗口均衡采样、频率感知嵌入收缩、分层变量选择和影响对齐注意力校准四种策略来增强性能。

Result: 在MIMIC-III、MIMIC-IV和eICU数据集上，OmniTFT在生命体征和实验室结果的预测任务中均取得了显著的性能提升。

Conclusion: OmniTFT能够统一建模多个异质临床目标，同时保持跨机构泛化能力，其注意力模式可解释且与已知病理生理学一致，具有临床决策支持的潜在应用价值。
...
### [152] [Efficient Inference Using Large Language Models with Limited Human Data: Fine-Tuning then Rectification](https://arxiv.org/abs/2511.19486)
*Lei Wang,Zikun Ye,Jinglong Zhao*

Main category: cs.LG

TL;DR: 该论文提出了一个结合微调和修正的框架，通过优化分配有限标记样本来提升大语言模型在市场研究和社科应用中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要采用微调或修正来改进大语言模型性能，但缺乏将两者结合并优化样本分配的框架。

Method: 开发了一个结合微调和修正的框架，提出以预测误差方差最小化为微调目标，并利用经验缩放定律优化样本在微调和修正阶段之间的分配。

Result: 实证分析验证了该框架的有效性，相比单独使用微调或修正，在估计和推断性能方面都有显著提升。

Conclusion: 该研究证明了结合微调和修正并优化样本分配的框架能够有效提升大语言模型在社科研究中的表现。
...
### [153] [The Generalized Proximity Forest](https://arxiv.org/abs/2511.19487)
*Ben Shaw,Adam Rustad,Sofia Pelagalli Maia,Jake S. Rhodes,Kevin R. Moon*

Main category: cs.LG

TL;DR: 本文提出了广义邻近森林模型，将随机森林邻近性扩展到所有基于距离的监督机器学习场景，包括回归任务和作为元学习框架，实验证明其相比随机森林和k近邻模型的独特优势。


<details>
  <summary>Details</summary>
Motivation: 随机森林邻近性在多种监督学习任务中表现出实用性，但其效果依赖于随机森林模型本身，而随机森林并非在所有场景下都是理想模型。需要将邻近性扩展到更广泛的机器学习场景。

Method: 提出了广义邻近森林模型，将随机森林邻近性扩展到所有基于距离的监督机器学习场景；开发了用于回归任务的PF模型变体；将广义PF模型作为元学习框架，为任何预训练分类器提供监督插补能力。

Result: 实验证明广义邻近森林模型相比随机森林模型和k近邻模型具有独特优势，能够有效扩展邻近性分析到更广泛的机器学习任务中。

Conclusion: 广义邻近森林模型成功地将随机森林邻近性扩展到所有基于距离的监督机器学习场景，为时间序列分析、回归任务和元学习提供了新的工具和框架。
...
### [154] [OpenCML: End-to-End Framework of Open-world Machine Learning to Learn Unknown Classes Incrementally](https://arxiv.org/abs/2511.19491)
*Jitendra Parmar,Praveen Singh Thakur*

Main category: cs.LG

TL;DR: 该论文提出了一种在开放连续学习环境中处理新类发现的模型，通过发现未知类别和增量学习新类别来实现持续学习，在开放世界学习和持续学习方面优于现有方法。
...
Method: 模型包含两个连接任务：首先发现数据中的未知类别并创建新类，然后对每个新类进行增量学习，从而实现持续学习。

Result: 该模型在开放世界学习中优于现有方法，在持续学习中四个迭代的平均准确率最高达82.54%，最低为65.87%。

Conclusion: 提出的模型能够在开放连续学习环境中有效处理新类发现和增量学习，扩展系统对数据的理解并随时间改进性能。
...
TL;DR: REWA提出了一种基于见证重叠结构的通用相似性理论，证明当概念间相似性可表示为单调见证重叠时，可缩减为具有可证明排序保持保证的紧凑编码。
...
Method: 使用有限见证集、半随机比特分配和重叠单调性，在满足重叠间隙条件下，通过O(log(|V|/δ))比特保持top-k排序。

Result: 证明了在见证重叠条件下，相似性系统可实现对数级编码复杂度，统一了Bloom过滤器、minhash、LSH位图等多种方法。

Conclusion: REWA为基于见证重叠而非哈希工程的相似性系统提供了原则性基础，支持模块化构建和可组合设计，具有广泛的应用前景。
...
### [156] [A Systematic Study of Compression Ordering for Large Language Models](https://arxiv.org/abs/2511.19495)
*Shivansh Chhawri,Rahul Mahadik,Suparna Rooj*

Main category: cs.LG

TL;DR: 本文系统研究了知识蒸馏、结构化剪枝和低位量化三种LLM压缩技术的独立效果和组合顺序，发现在Qwen2.5 3B模型上，P-KD-Q（剪枝-知识蒸馏-量化）序列能实现3.68倍压缩比并保持良好性能，而早期量化会导致严重性能下降。


<details>
  <summary>Details</summary>
Motivation: LLM需要大量计算资源，模型压缩对受限环境部署至关重要。虽然各压缩技术独立效果已有研究，但它们之间的相互作用和最优排序仍不清楚。

Method: 在Qwen2.5 3B模型上系统评估多种压缩流水线，包括单技术和三技术序列，使用困惑度、G-Eval、清晰度、提示对齐和压缩比作为评估指标。

Result: 量化提供最大独立压缩，剪枝引入中等质量下降。技术顺序显著影响最终模型质量：P-KD-Q序列表现最佳，而早期应用量化的流水线因不可逆信息损失而性能严重下降。

Conclusion: 本研究为在资源受限环境中部署LLM提供了实用的、顺序感知的压缩流水线设计指导，强调技术应用顺序对保持模型性能的重要性。
...
### [157] [Xmodel-2.5: 1.3B Data-Efficient Reasoning SLM](https://arxiv.org/abs/2511.19496)
*Yang Liu,Xiaolong Zhong,Ling Jiang*

Main category: cs.LG

TL;DR: Xmodel-2.5是一个13亿参数的小型语言模型，作为即插即用的智能体核心，通过μP训练方法在保持高性能的同时降低计算需求，特别适合边缘部署。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然推理能力强，但计算需求大，不适合边缘或成本敏感的应用场景。需要开发高性能的小型模型来满足这些需求。

Method: 采用最大更新参数化(μP)训练方法，使用1.4T token的Warmup-Stable-Decay课程学习，在衰减阶段从AdamW切换到Muon优化器，并采用FP8混合精度训练。

Result: 在衰减阶段从AdamW切换到Muon后，13个任务的推理平均性能提升了4.58%，同时保持其他超参数不变，验证了早期AdamW稳定性与后期Muon锐化相结合的优势。

Conclusion: Xmodel-2.5成功实现了高性能小型语言模型的设计目标，通过创新的训练策略在保持模型紧凑性的同时提升了推理性能，适合边缘部署应用。
...
### [158] [Hierarchical Dual-Strategy Unlearning for Biomedical and Healthcare Intelligence Using Imperfect and Privacy-Sensitive Medical Data](https://arxiv.org/abs/2511.19498)
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在医疗环境中因训练数据记忆而产生的隐私风险，特别是涉及不完整或隐私敏感的患者信息时，需要满足监管合规、可审计性和伦理标准。

Method: 采用分层双策略框架，结合几何约束梯度更新来选择性调节目标参数，以及概念感知令牌级干预，通过统一的四级医学概念层次区分保留关键令牌和遗忘目标令牌。
...
### [159] [Beyond Binary Classification: A Semi-supervised Approach to Generalized AI-generated Image Detection](https://arxiv.org/abs/2511.19499)
*Hong-Hanh Nguyen-Le,Van-Tuan Tran,Dinh-Thuc Nguyen,Nhien-An Le-Khac*

Main category: cs.LG

TL;DR: 本文提出TriDetect检测器，通过发现假图像中的潜在架构模式来增强跨生成器泛化能力，解决GAN和扩散模型架构差异导致的检测器泛化失败问题。


<details>
  <summary>Details</summary>
Motivation: 当前取证技术的核心漏洞是检测器无法实现跨生成器泛化，特别是在跨越架构边界时（如从GAN到扩散模型）。作者认为这种差距源于不同架构产生的伪影存在根本性差异。

Method: 提出TriDetect半监督方法，通过平衡聚类分配（使用Sinkhorn-Knopp算法）和跨视图一致性机制，鼓励模型学习基本的架构差异特征。

Result: 在两个标准基准和三个真实世界数据集上评估，与13个基线方法比较，证明了该方法对未见生成器的泛化能力。

Conclusion: TriDetect通过发现假图像类别中的潜在架构模式，有效提升了跨生成器检测的泛化性能，为解决不同生成架构间的检测挑战提供了有效方案。
...
### [160] [Position: The Complexity of Perfect AI Alignment -- Formalizing the RLHF Trilemma](https://arxiv.org/abs/2511.19504)
*Subramanyam Sahoo,Aman Chadha,Vinija Jain,Divya Chaudhary*

Main category: cs.LG

TL;DR: 本文提出了对齐三难困境理论：RLHF系统无法同时实现跨多样人类价值观的代表性、多项式计算复杂度和对对抗性扰动的鲁棒性。通过复杂性理论分析证明，实现全球规模的代表性和鲁棒性需要超多项式计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决RLHF实践中存在的安全性与公平性冲突、扩展到多样化人群的计算不可行性、以及鲁棒性增强多数偏见的问题。

Method: 通过整合统计学习理论和鲁棒优化的复杂性理论分析，证明对齐三难困境的理论界限。

Result: 证明实现全球规模的代表性和鲁棒性需要Ω(2^{d_context})操作，这是超多项式复杂度。当前RLHF实现通过牺牲代表性来解决这一困境。

Conclusion: 为导航这些基本权衡提供了具体方向，通过战略性地放松对齐要求来解决三难困境。
...
### [161] [Row-stochastic matrices can provably outperform doubly stochastic matrices in decentralized learning](https://arxiv.org/abs/2511.19513)
*Bing Liu,Boao Kong,Limin Lu,Kun Yuan,Chengcheng Zhao*

Main category: cs.LG

TL;DR: 本文重新审视了去中心化学习中处理异构节点权重的两种策略：将权重嵌入局部损失以保持均匀权重（双随机矩阵），或保留原始损失并使用λ诱导的行随机矩阵。通过构建加权希尔伯特空间框架，获得了比欧几里得分析更严格的收敛率，揭示了行随机矩阵的自伴性优势。


<details>
  <summary>Details</summary>
Motivation: 虽然之前的研究表明两种权重处理策略在全局损失的期望下降方向上相同，但欧几里得空间的保证是否紧致以及它们行为的根本区别仍不清楚。需要开发新的分析框架来揭示收敛性能的差异。

Method: 构建加权希尔伯特空间框架L²(λ;ℝᵈ)，在该几何中分析两种策略的收敛行为。使用Rayleigh商和Loewner序特征值比较方法，推导保证行随机设计优势的拓扑条件。

Result: 在加权希尔伯特空间几何中，行随机矩阵具有自伴性而双随机矩阵没有，这产生了额外的惩罚项，放大了共识误差从而减慢收敛。收敛差异不仅来自谱间隙，还来自这些惩罚项。

Conclusion: 行随机设计在某些条件下即使具有较小的谱间隙也能收敛更快。通过拓扑条件分析，获得了保证这种优势的实用拓扑设计指南。
...
### [162] [Automating Deception: Scalable Multi-Turn LLM Jailbreaks](https://arxiv.org/abs/2511.19517)
...
### [163] [Learning Massively Multitask World Models for Continuous Control](https://arxiv.org/abs/2511.19584)
*Nicklas Hansen,Hao Su,Xiaolong Wang*

Main category: cs.LG

TL;DR: 本文提出了一个包含200个多样化任务的基准测试，并介绍了Newt——一个语言条件化的多任务世界模型，通过演示预训练和在线交互联合优化，在多任务性能和数据效率方面优于强基线方法。


<details>
  <summary>Details</summary>
Motivation: 通用控制需要能够在多种任务和实体上操作的智能体，但目前连续控制的强化学习研究仍以单任务或离线模式为主，这强化了在线强化学习无法扩展的观点。受基础模型方法（大规模预训练+轻量强化学习）启发，本文探索是否可以通过在线交互在数百个任务上训练单一智能体。

Method: 首先在演示数据上进行预训练以获得任务感知表示和动作先验，然后通过在线交互在所有任务上进行联合优化。模型采用语言条件化的多任务世界模型架构。

Result: 实验表明Newt在多任务性能和数据效率方面优于强基线方法，表现出强大的开环控制能力，并能快速适应未见任务。

Conclusion: Newt证明了通过大规模在线交互训练单一智能体在多样化任务上的可行性，为通用控制研究提供了新的方向和基准。
...
### [164] [Online Sparse Feature Selection in Data Streams via Differential Evolution](https://arxiv.org/abs/2511.19555)
*Ruiyang Xu*

Main category: cs.LG

TL;DR: 本文提出了一种新颖的在线差分进化稀疏特征选择方法（ODESFS），用于处理高维流数据中的缺失值问题，通过潜在因子分析模型进行缺失值填补，并利用差分进化评估特征重要性，在六个真实数据集上的实验表明该方法优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 现有的在线稀疏流特征选择方法在特征评估方面存在显著局限性，导致性能下降，需要改进特征评估机制以提升处理高维流数据中缺失值问题的能力。

Method: 提出ODESFS方法，包含两个关键创新：(1) 使用潜在因子分析模型进行缺失值填补；(2) 通过差分进化进行特征重要性评估。

Result: 在六个真实世界数据集上的综合实验表明，ODESFS通过选择最优特征子集并实现更优的准确率，持续优于最先进的OSFS和OS2FS方法。

Conclusion: ODESFS方法有效解决了高维流数据中缺失值处理的挑战，在特征选择和分类准确率方面均表现出优越性能。
...
### [165] [Merging without Forgetting: Continual Fusion of Task-Specific Models via Optimal Transport](https://arxiv.org/abs/2511.19561)
...
TL;DR: OTMF提出了一种基于最优传输理论的模型融合框架，通过发现任务向量的共同掩码来解决参数插值引起的分布偏移问题，支持持续融合范式，在多个视觉和语言基准测试中实现了最先进的性能。
...
Method: OTMF使用最优传输理论对齐任务特定模型的语义几何，通过最优传输计划发现应用于任务向量的共同掩码，选择性提取可转移和任务无关组件，同时保持每个任务的独特结构身份。

Result: 在多个视觉和语言基准测试上的综合实验表明，OTMF在准确性和效率方面都达到了最先进的性能。

Conclusion: OTMF为模型融合提供了一种实用且有理论价值的方法，能够有效解决分布偏移问题，支持持续融合，并在多任务系统中实现优异的性能。
...
### [166] [ModHiFi: Identifying High Fidelity predictive components for Model Modification](https://arxiv.org/abs/2511.19566)
*Dhruva Kashyap,Chaitanya Murti,Pranav K Nayak,Tanay Narshana,Chiranjib Bhattacharyya*

Main category: cs.LG

TL;DR: 本文提出了一种无需训练数据或损失函数访问的模型修改方法ModHiFi，通过子集保真度指标识别模型关键组件，用于剪枝和遗忘任务。


<details>
  <summary>Details</summary>
Motivation: 开放权重模型通常不提供训练数据或损失函数访问，这使得模型剪枝和遗忘等修改任务受到限制。现有方法需要梯度或真实标签，在计算资源有限的情况下不可行。

Method: 提出子集保真度指标，利用局部重建误差来量化组件全局重要性。基于此开发ModHiFi算法，包括用于结构化剪枝的ModHiFi-P和用于类别遗忘的ModHiFi-U。

Result: ModHiFi-P在ImageNet模型上比现有技术快11%，在语言模型上表现具有竞争力；ModHiFi-U在CIFAR-10上无需微调即可实现完全遗忘，在Swin Transformers上表现良好。

Conclusion: 该方法证明了无需训练数据或损失函数即可有效识别模型关键组件，为模型修改提供了新的可行方案。
...
### [167] [An Invariant Latent Space Perspective on Language Model Inversion](https://arxiv.org/abs/2511.19569)
*Wentao Ye,Jiaqi Hu,Haobo Wang,Xinpeng Ti,Zhiqing Xiao,Hao Chen,Liyao Li,Lei Feng,Sai Wu,Junbo Zhao*

Main category: cs.LG

TL;DR: 本文提出语言模型反演（LMI）作为用户隐私和系统安全的具体威胁，通过引入不变潜在空间假设（ILSH）和Inv^2A方法，从模型输出中恢复隐藏提示，在9个数据集上表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 语言模型反演（LMI）对用户隐私和系统安全构成威胁，需要开发有效的反演方法来揭示隐藏提示。

Method: 提出不变潜在空间假设（ILSH），将LLM视为不变解码器，仅学习轻量级逆编码器，通过对比对齐和监督强化两阶段训练，可选无训练邻域搜索优化性能。

Result: 在9个数据集上，Inv^2A比基线方法平均BLEU得分提高4.77%，同时减少对大型逆语料库的依赖。

Conclusion: 现有防御措施提供有限保护，需要更强的防御策略来应对语言模型反演威胁。
...
### [168] [Neural Tractability via Structure: Learning-Augmented Algorithms for Graph Combinatorial Optimization](https://arxiv.org/abs/2511.19573)
*Jialiang Li,Weitong Chen,Mingyu Guo*

Main category: cs.LG

TL;DR: 提出一个结合神经模型和参数化算法的框架，在保持神经模型推理效率的同时，通过参数化分析识别实例的困难部分，神经模型处理困难部分生成指导信号，参数化算法搜索简单部分，从而获得更优解并改善分布外泛化能力。


<details>
  <summary>Details</summary>
Motivation: 神经模型在图组合优化问题中推理速度快但解质量不如传统搜索算法，传统算法虽慢但能提供最优性保证。需要结合两者的优势。

Method: 使用参数化算法作为搜索组件，通过参数化分析识别实例的结构性困难部分，神经模型处理困难部分生成指导信号，参数化算法整合信号搜索简单部分。

Result: 在多个组合优化任务上，该框架获得优于单独神经求解器的解质量，与商业求解器竞争力相当，同时改善了分布外泛化能力。

Conclusion: 该框架成功结合了神经模型的推理效率和参数化算法的解质量保证，为神经组合优化求解器提供了新的发展方向。
...
### [169] [Lower Complexity Bounds for Nonconvex-Strongly-Convex Bilevel Optimization with First-Order Oracles](https://arxiv.org/abs/2511.19656)
*Kaiyi Ji*
...
<details>
  <summary>Details</summary>
Motivation: 尽管双层优化的上界保证已被广泛研究，但由于双层结构的复杂性，下界研究进展有限。本文旨在填补这一空白，为理解双层优化在标准一阶oracle下的最优复杂度提供理论支撑。

Method: 开发新的困难实例，在光滑非凸-强凸设置下，分别针对确定性和随机一阶oracle模型构建下界证明。

Result: 在确定性情况下，任何一阶零尊重算法至少需要Ω(κ^{3/2}ε^{-2})次oracle调用才能找到ε精度的驻点；在随机情况下，至少需要Ω(κ^{5/2}ε^{-4})次随机oracle调用。

Conclusion: 研究结果揭示了当前双层优化上下界之间的显著差距，表明即使是简化设置（如二次下层目标）也需要进一步研究以理解双层优化在标准一阶oracle下的最优复杂度。
...
### [170] [Demystifying Diffusion Objectives: Reweighted Losses are Better Variational Bounds](https://arxiv.org/abs/2511.19664)
*Jiaxin Shi,Michalis K. Titsias*

Main category: cs.LG

TL;DR: 本文提出了一种对扩散模型重加权损失的新理论解释，通过构建时间相关的变分下界来改进标准证据下界，从而减少数据模型KL散度，并在掩码扩散模型中取得了显著改进。
...
Method: 构建时间相关的变分下界级联，改进标准证据下界，形成可应用于任何生成扩散模型的重加权目标。

Result: 在掩码扩散模型中显著改进了像素空间图像建模的训练损失，样本质量接近连续扩散模型水平。

Conclusion: 该方法为掩码图像模型中广泛使用的简单加权方案提供了理论依据，并在实践中取得了显著性能提升。
...
### [171] [BlockCert: Certified Blockwise Extraction of Transformer Mechanisms](https://arxiv.org/abs/2511.17645)
*Sandro Andric*

Main category: cs.LG

TL;DR: BlockCert是一个用于认证式块级提取Transformer机制和局部编辑的框架，通过提供机器可检查的证书来约束近似误差，并将局部保证提升为全局偏差界限。


<details>
  <summary>Details</summary>
Motivation: 当前机制可解释性和模型编辑领域缺乏明确的保证，无法确定提取或编辑后的模型在相关输入上与原模型的偏离程度。

Method: 提出BlockCert框架，提取残差块的结构化替代实现，提供包含近似误差边界、覆盖度指标和底层工件哈希的机器可检查证书，并在Lean 4中形式化基于Lipschitz的组合定理。
...
### [172] [TiCT: A Synthetically Pre-Trained Foundation Model for Time Series Classification](https://arxiv.org/abs/2511.19694)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列数据普遍存在，但标记数据成本高昂。现有的大规模时间序列模型主要关注预测任务，缺乏无需微调的通用分类基础模型。

Method: 提出TiCT模型，采用基于位的标签编码和特殊输出注意力机制处理任意类别数，结合Mixup启发式方法和数据增强的合成预训练框架。

Result: 在UCR Archive上的广泛评估表明，TiCT仅使用上下文示例在推理时就能达到与最先进监督方法竞争的性能。
...
### [173] [Training-Free Active Learning Framework in Materials Science with Large Language Models](https://arxiv.org/abs/2511.19730)
*Hongchen Wang,Rafael Espinosa Castañeda,Jay R. Werber,Yao Fehlis,Edward Kim,Jason Hattrick-Simpers*

Main category: cs.LG

TL;DR: LLM-AL框架使用大语言模型进行主动学习，在材料科学四个数据集上相比传统ML模型，能减少70%以上实验次数达到最优性能，并展现出更广泛的探索性搜索能力。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型在主动学习中存在冷启动问题和领域特定特征工程的限制，影响其泛化能力。大语言模型凭借预训练知识和通用标记表示，提供了直接从文本描述提出实验的新范式。

Method: 提出基于LLM的主动学习框架(LLM-AL)，在迭代少样本设置下运行。探索了两种提示策略：简洁数值输入适用于成分和结构化特征数据集，扩展描述性文本适用于实验和过程特征数据集以提供额外上下文。

Result: 在所有数据集上，LLM-AL能将达到顶级性能候选者所需的实验次数减少70%以上，并始终优于传统ML模型。LLM-AL执行更广泛和探索性搜索，同时以更少迭代达到最优解。性能在不同运行间基本一致，在传统ML方法典型变异性范围内。
...
### [174] [DISCO: A Browser-Based Privacy-Preserving Framework for Distributed Collaborative Learning](https://arxiv.org/abs/2511.19750)
...
TL;DR: DISCO是一个开源分布式协作学习平台，允许非技术用户在不共享原始数据或编程知识的情况下协作构建机器学习模型，支持联邦学习和去中心化范式。


<details>
  <summary>Details</summary>
Motivation: 解决数据共享的隐私、知识产权和法律约束问题，避免数据碎片化导致的统计能力下降和可访问性偏见，使机器学习模型准确性更公平地分布。

Method: 通过Web应用在浏览器中本地训练模型，采用模块化设计支持联邦学习和去中心化范式，提供不同级别的隐私保证和权重聚合策略，支持模型个性化和偏见弹性。

Result: 开发了DISCO平台，代码库和展示界面已公开，工具跨平台兼容包括智能手机。

Conclusion: DISCO为非技术用户提供了无需共享数据即可协作训练机器学习模型的解决方案，有助于解决数据隐私和可访问性问题。
...
### [175] [When +1% Is Not Enough: A Paired Bootstrap Protocol for Evaluating Small Improvements](https://arxiv.org/abs/2511.19794)
*Wenzhang Du*

Main category: cs.LG

TL;DR: 论文提出了一种保守的评估协议，用于验证机器学习中1-2个百分点的微小改进是否真实有效，而不是随机噪声。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习论文通常报告基准测试中1-2个百分点的改进，但这些增益对随机种子、数据排序和实现细节高度敏感，且很少提供不确定性估计或显著性检验，难以区分真实算法进步与噪声。

Method: 提出基于配对多种子运行、偏差校正加速(BCa)自助法置信区间和符号翻转置换检验的简单评估协议，专门针对有限计算预算设计。

Result: 在CIFAR-10、CIFAR-10N和AG News数据集上的实验表明，单次运行和非配对t检验经常错误地宣称0.6-2.0个百分点的改进具有显著性，而使用三个种子的配对协议在这些设置下从不宣称显著性。

Conclusion: 对于有限计算预算下的小幅增益，这种保守的评估方法是一个更安全的默认选择，可以防止过度宣称结果。
...
### [176] [Terminal Velocity Matching](https://arxiv.org/abs/2511.19797)
*Linqi Zhou,Mathias Parger,Ayaan Haque,Jiaming Song*

Main category: cs.LG

TL;DR: TVM是一种流匹配的泛化方法，支持高保真的一步和少步生成建模，通过建模任意两个扩散时间步之间的转换并在终端时间进行正则化，在ImageNet上实现了最先进的少步生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有的流匹配方法在少步生成建模中存在局限性，需要一种能够直接建模任意时间步之间转换的方法，同时解决扩散变换器缺乏Lipschitz连续性的问题。

Method: 提出终端速度匹配(TVM)，在终端时间而非初始时间进行正则化；引入最小架构修改实现稳定单阶段训练；开发支持Jacobian-向量积反向传播的融合注意力核。

Result: 在ImageNet-256x256上：1步生成FID为3.29，4步生成FID为1.99；在ImageNet-512x512上：1步生成FID为4.32，4步生成FID为2.94，均达到最先进水平。

Conclusion: TVM提供了一种有效的少步生成建模框架，通过终端时间正则化和架构优化，实现了从零开始训练的高质量少步生成模型。
...
TL;DR: EfficientXpert是一个轻量级领域剪枝框架，通过传播感知剪枝准则和高效适配器更新算法，在LoRA微调过程中一步将通用预训练模型转换为稀疏的领域专家模型，在40%稀疏度下保持98%性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在专业领域需求增长，但模型尺寸大限制了在资源受限环境中的部署，现有压缩方法要么跨领域泛化能力差，要么开销高。

Method: 结合传播感知剪枝准则(Foresight Mask)和高效适配器更新算法(Partial Brain Surgeon)，集成到LoRA微调过程中，实现一步转换。

Result: 在医疗和法律任务中，在40%稀疏度下保持高达98%的稠密模型性能，优于现有最先进方法。

Conclusion: 分析揭示了领域依赖的结构变化会降低通用剪枝掩码的有效性，强调需要针对每个领域的自适应、领域感知剪枝策略。
...
Method: 通过将Transformer组件分解为正交奇异方向，识别单个头或MLP内的叠加和独立计算。

Result: 在IOI、GP和GT等标准任务上验证了该方法，发现先前识别的规范功能头（如名称移动器）编码了多个与不同奇异方向对齐的重叠子功能。

Conclusion: Transformer计算比先前假设的更加分布化、结构化和组合化，这为细粒度机制可解释性和模型内部理解开辟了新途径。
...
TL;DR: 该研究通过内在维度分析大型语言模型的隐藏表示几何结构，发现在多项选择问答任务中，模型各层呈现一致的维度模式：早期层在低维流形上操作，中间层扩展空间，后期层压缩空间并收敛到决策相关表示。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多样化任务中表现出强大的泛化能力，但其内部决策过程仍不透明。本研究旨在通过内在维度分析揭示LLM隐藏表示的几何结构，理解其决策动态。

Method: 对28个开源Transformer模型进行大规模研究，使用多个估计器估计各层的内在维度，并量化各层在多项选择问答任务上的性能。

Result: 发现模型各层呈现一致的ID模式：早期层在低维流形上操作，中间层扩展空间，后期层压缩空间并收敛到决策相关表示。

Conclusion: LLM隐式学习将语言输入投影到结构化的低维流形上，这些流形与任务特定决策对齐，为理解语言模型中泛化和推理如何出现提供了新的几何视角。
...
### [180] [Cisco Time Series Model Technical Report](https://arxiv.org/abs/2511.19841)
...
TL;DR: Cisco Time Series Model是一个单变量零样本预测器，通过多分辨率输入架构创新，在超过3000亿数据点上训练，在可观测性数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够处理多分辨率输入的时间序列基础模型，特别针对可观测性领域的数据进行优化，同时保持通用预测性能。

Method: 对TimesFM模型进行架构创新，使其能够接受多分辨率输入，构建多分辨率解码器模型，在3000亿+数据点上训练，其中超过一半来自可观测性领域。

Result: 模型在可观测性数据集上实现卓越性能，在通用预测基准(GIFT-Eval)上保持相似性能，多分辨率结构使模型在长上下文输入上预测更准确。

Conclusion: 多分辨率架构创新使Cisco Time Series Model在可观测性领域表现优异，同时保持通用预测能力，长上下文预测准确性得到提升。
...
TL;DR: 提出Soft Adaptive Policy Optimization (SAPO)方法，用平滑的温度控制门替代硬截断，自适应衰减离策略更新同时保留有用学习信号，提升LLM强化学习的稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于分组的策略优化方法如GSPO和GRPO使用硬截断，难以同时保持稳定性和有效学习。令牌级重要性比率在Mixture-of-Experts模型中方差较高，导致更新不稳定。

Method: SAPO使用平滑的温度控制门机制，形成连续信任区域，选择性降低离策略令牌的权重，保留近策略令牌的学习信号，实现序列一致性和令牌自适应性。

Result: 在数学推理基准测试中，SAPO在可比训练预算下表现出更好的训练稳定性和更高的Pass@1性能。在Qwen3-VL模型系列训练中，SAPO在不同任务和模型大小上均带来一致的性能提升。
...
Method: 首先分析收敛性，揭示学习模式与批量大小之间的相互作用。然后制定延迟最小化问题，提出两阶段解决方案：使用块坐标下降法求解松弛问题获得局部最优解，然后通过舍入算法恢复整数批量大小以实现接近最优性能。

Result: 实验结果表明，与现有方法相比，该方法显著加速了达到目标精度的收敛过程。

Conclusion: 通过联合优化学习模式选择、批量大小和资源分配，可以有效加速HSFL的收敛速度，平衡延迟和精度之间的权衡。
...
TL;DR: 本研究提出了FACT框架，使用Transformer建模网约车司机的空闲行为，通过因果掩码和司机特定嵌入来捕捉长期时间依赖性和异质性，在预测性能上优于传统和深度学习生存模型。


<details>
  <summary>Details</summary>
Motivation: 网约车平台具有高频、行为驱动的特点，虽然生存分析在其他领域的重复事件中已有应用，但在网约车司机行为建模方面仍未被充分探索。

Method: 将空闲行为建模为重复生存过程，使用基于Transformer的框架，结合因果掩码捕捉长期时间依赖性，并纳入司机特定嵌入来建模潜在异质性。
...
TL;DR: 本文提出了一个强化学习框架来优化磁共振指纹成像中的翻转角调度，展示了学习到的非周期性模式能增强指纹可分离性，并可能减少重复时间以加速采集。


<details>
  <summary>Details</summary>
Motivation: 磁共振指纹成像利用可调采集参数产生的瞬态信号动态，使得最优、鲁棒序列的设计成为一个复杂的高维顺序决策问题。强化学习为自动化参数选择提供了有前景的方法，以优化脉冲序列来最大化参数空间中指纹的可区分性。

Method: 引入强化学习框架来优化磁共振指纹成像中的翻转角调度，通过强化学习自动选择参数。

Result: 展示了学习到的调度表现出非周期性模式，增强了指纹的可分离性。有趣的是，强化学习优化的调度可能使重复时间数量减少，从而可能加速磁共振指纹成像采集。

Conclusion: 强化学习框架成功优化了磁共振指纹成像中的翻转角调度，产生了非周期性模式，提高了指纹可分离性，并显示出减少采集时间的潜力。
...
TL;DR: 本文提出了一种名为"差分平滑"的原则性方法，通过理论分析和实验证明，该方法能够有效解决RL微调大语言模型时的多样性崩溃问题，在正确性和多样性方面均优于传统RL和基于熵的启发式方法。


<details>
  <summary>Details</summary>
Motivation: RL微调大语言模型常导致多样性崩溃，现有启发式方法存在随意性、效果不稳定且相互矛盾的问题，需要建立理论基础并提出更优解决方案。

Method: 首先从理论上证明RL微调导致多样性崩溃的机制（选择和强化偏差），然后提出差分平滑方法，该方法仅在正确轨迹上应用奖励修改来同时提升正确性和多样性。

Result: 在1B到7B参数的模型上，在CountDown和真实世界数学推理等领域的广泛实验表明，差分平滑方法在Pass@1和Pass@k指标上均取得一致提升，在AIME24数据集上最高提升6.7%。

Conclusion: 差分平滑方法是解决RL微调多样性崩溃问题的理论基础和实用解决方案，优于现有启发式方法，能够同时提升模型输出的正确性和多样性。
...
TL;DR: 该论文研究了大型语言模型中的提示公平性问题，发现不同用户/风格的提示表述即使询问相同问题，也会引发模型不同的响应质量。作者提出信息论指标量化这种偏差，并通过多数投票和提示中立化等干预措施改善公平性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在不同用户/风格的提示表述下会产生不一致的响应质量，这种提示公平性问题需要被量化和解决，以确保模型对所有用户群体都能提供稳定和公平的服务。

Method: 提出使用信息论指标来量化两个维度的偏差：子组敏感性和跨组一致性。采用多数投票和提示中立化等实际干预措施来减轻这些差异。

Result: 实验显示不同人口统计子组存在明显的提示敏感性差异：缓解前跨组分歧值达到0.28，通常在0.14-0.22范围内。应用中立化和多代策略后，这些分歧持续减少，最大差距降至0.22，许多距离降至0.17或以下。

Conclusion: 通过提出的干预措施可以有效减少大型语言模型中的提示公平性差异，提高跨子组的响应稳定性和一致性，从而增强模型对不同用户群体的公平性。
...
Method: 提出ParaBlock方法，建立通信和计算两个并行线程，通过并行化处理来提升通信效率。

Result: 理论证明ParaBlock与标准联邦块坐标下降方法具有相同的收敛速度，实验评估显示在通用指令跟随和数学推理任务上，ParaBlock不仅保持强大性能，还显著提高了通信效率。
...
Result: 通过广泛实验证明，FedEcho在存在大异步延迟和数据异构性的情况下，持续优于现有的异步联邦学习基线方法。

Conclusion: FedEcho通过不确定性感知蒸馏有效缓解了过时更新和数据异构性的负面影响，实现了稳健的性能，且无需访问私有客户端数据。
...
TL;DR: 提出了一种按需多任务稀疏框架，通过最大化参数重用和块粒度分解来最小化任务切换时的I/O开销，在自动驾驶平台上实现了6.6倍的任务切换加速。
...
Method: 将权重分解为可重用的块粒度单元，跨任务对齐稀疏结构以最大化重叠，动态加载仅需的差分块集。

Result: 在真实自动驾驶平台上的实验表明，该框架相比现有稀疏方法平均加速任务切换超过6.6倍。

Conclusion: 所提出的按需多任务稀疏框架通过参数重用和动态块加载，有效缓解了传统整体方法中的冷启动延迟问题，显著提升了任务切换效率。
...
TL;DR: RankOOD是一种基于排序的OOD检测方法，通过Placket-Luce损失训练模型，在TinyImageNet基准上实现SOTA性能，将FPR95降低4.3%。


<details>
  <summary>Details</summary>
Motivation: 基于深度学习中交叉熵损失训练模型时，ID类预测会为每个ID类预测诱导出排序模式，这一观察启发了RankOOD的开发。

Method: 首先使用初始分类器为每个类提取排序列表，然后使用Plackett-Luce损失进行另一轮训练，其中类排序是预测变量。

Result: 在近OOD TinyImageNet评估基准上达到最先进性能，FPR95降低4.3%。

Conclusion: RankOOD通过利用类预测的排序模式，有效检测OOD样本，即使OOD样本可能被高概率分配到ID类，但遵守排序分类的概率很小。
...
TL;DR: 研究发现，在零样本设置下，时间序列基础模型Sundial在足够长的输入上下文窗口（覆盖超过1-2个完整季节周期）时，可以超越完全训练的LSTM模型，在LAI预测任务中表现出色。
...
### [192] [Cross-Contrastive Clustering for Multimodal Attributed Graphs with Dual Graph Filtering](https://arxiv.org/abs/2511.20030)
*Haoran Zheng,Renchi Yang,Hongtao Wang,Jianliang Xu*

Main category: cs.LG

TL;DR: 该论文提出了一种名为Dual Graph Filtering (DGF)的新方法，用于解决多模态属性图(MMAGs)聚类问题，通过特征级去噪和三交叉对比学习策略，有效克服了现有方法在处理低模态相关性和强特征噪声方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有多视图聚类方法过度依赖跨视图属性的高相关性，忽视了由大型预训练模型生成的多模态属性中存在的低模态相关性和强烈特征噪声问题，导致聚类性能不佳。

Method: 提出了Dual Graph Filtering (DGF)方案，包含特征级去噪组件和三交叉对比训练策略，通过跨模态、邻域和社区的实例级对比学习来学习鲁棒且具有区分度的节点表示。

Result: 在八个基准MMAG数据集上的综合实验表明，DGF在聚类质量方面能够一致且显著地优于广泛的先进基线方法。

Conclusion: DGF方法通过创新的图滤波机制和对比学习策略，有效解决了多模态属性图聚类中的关键挑战，实现了优异的聚类性能。
...
### [193] [RED-F: Reconstruction-Elimination based Dual-stream Contrastive Forecasting for Multivariate Time Series Anomaly Prediction](https://arxiv.org/abs/2511.20044)
*PengYu Chen,Xiaohou Shi,Yuan Chang,Yan Sun,Sajal K. Das*

Main category: cs.LG

TL;DR: RED-F是一个基于重构消除的双流对比预测框架，用于多元时间序列中的异常预测，通过对比机制放大微弱异常前兆信号。


<details>
  <summary>Details</summary>
Motivation: 现有无监督方法在训练时仅使用正常数据，倾向于重构正常模式，导致在面对微弱异常前兆时预测被正常模式主导，淹没所需的预测信号。

Method: 提出RED-F框架，包含重构消除模型(REM)和双流对比预测模型(DFM)。REM使用混合时频机制消除前兆生成纯净基线，DFM接收纯净基线和保留前兆的原始序列进行对比预测，并采用多序列预测目标增强预测敏感性。

Result: 在六个真实世界数据集上的广泛实验表明，RED-F在异常预测任务中具有优越能力。

Conclusion: RED-F通过重构消除和双流对比预测机制，成功解决了微弱异常前兆检测的挑战，将绝对信号检测转化为更简单的相对轨迹比较任务。
...
### [194] [SOMBRL: Scalable and Optimistic Model-Based RL](https://arxiv.org/abs/2511.20066)
*Bhavya Sukhija,Lenart Treven,Carmelo Sferrazza,Florian Dörfler,Pieter Abbeel,Andreas Krause*

Main category: cs.LG

TL;DR: 提出SOMBRL方法，基于乐观面对不确定性原则，通过不确定性感知的动态模型和贪婪优化外部奖励与认知不确定性的加权和，实现模型强化学习中的高效探索。


<details>
  <summary>Details</summary>
Motivation: 解决基于模型的强化学习中未知系统动态下的高效探索挑战，需要从在线交互中直接学习。

Method: 学习不确定性感知的动态模型，贪婪地最大化外部奖励与智能体认知不确定性的加权和，兼容任何策略优化器或规划器。

Result: 在非线性动态的有限时域、折扣无限时域和非片段设置中具有次线性遗憾；在状态和视觉控制环境中表现优异，在动态RC汽车硬件上超越现有方法。
...
### [195] [QiMeng-CRUX: Narrowing the Gap between Natural Language and Verilog via Core Refined Understanding eXpression](https://arxiv.org/abs/2511.20099)
...
TL;DR: 本文提出CRUX-V模型，通过引入结构化中间表示CRUX来弥合自然语言与Verilog代码生成之间的语义鸿沟，采用两阶段训练框架提升生成质量，在多个基准测试中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于自然语言的硬件描述语言生成方法存在描述模糊、冗余和非结构化的问题，难以精确生成Verilog代码。需要解决从开放自然语言空间到高度约束的硬件描述空间的复杂转换挑战。

Method: 提出CRUX结构化中间表示空间，捕获用户意图的语义核心；设计包含联合表达建模和双空间优化的两阶段训练框架；开发CRUX-V模型进行Verilog代码生成。

Result: 在多个Verilog生成基准测试中，CRUX-V模型在通用模型中达到最先进性能，特别是在复杂设计任务中表现优异；CRUX表示空间具有可迁移性，能提升其他代码模型的性能。

Conclusion: CRUX结构化中间表示有效缩小了自由形式自然语言描述与精确Verilog生成之间的差距，提出的两阶段训练框架显著提升了代码生成质量，为硬件描述语言生成提供了新思路。
...
### [196] [Multivariate Forecasting of Bitcoin Volatility with Gradient Boosting: Deterministic, Probabilistic, and Feature Importance Perspectives](https://arxiv.org/abs/2511.20105)
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索LightGBM模型在捕捉加密货币市场非线性特征方面的有效性，同时提供对波动率动态的可解释性洞察。

Method: 采用LightGBM模型进行确定性和概率性预测，概率预测使用两种分位数方法：基于pinball损失函数的直接分位数回归，以及将点预测转换为预测分布的残差模拟方法。使用增益和排列特征重要性技术识别主要波动率驱动因素。

Result: 结果发现交易量、滞后波动率指标、投资者关注度和市值是主要波动率驱动因素。LightGBM模型有效捕捉了加密货币市场的非线性和高方差特征。

Conclusion: LightGBM模型在比特币波动率预测中表现出色，既能提供准确预测，又能提供对波动率动态的可解释性洞察，特别适合处理加密货币市场的复杂特征。
...
### [197] [On the Limits of Momentum in Decentralized and Federated Optimization](https://arxiv.org/abs/2511.20168)
*Riccardo Zaccone,Sai Praneeth Karimireddy,Carlo Masone*

Main category: cs.LG

TL;DR: 该论文分析了联邦学习中动量方法在循环客户端参与下的表现，理论上证明了动量仍然受到统计异质性的影响，且任何比Θ(1/t)更快的步长衰减都会导致收敛到依赖于初始化和异质性边界的常数值。


<details>
  <summary>Details</summary>
Motivation: 探索在联邦学习中使用动量方法是否能减轻统计异质性的影响，特别是在去中心化场景下只有部分工作者参与的情况下。

Method: 理论分析动量方法在循环客户端参与下的收敛性，并通过数值实验和深度学习实验验证理论结果。

Result: 动量方法无法保证在无界异质性下的收敛，且步长衰减策略无法解决异质性问题。

Conclusion: 动量方法在联邦学习中仍然受到统计异质性的限制，需要新的方法来处理这种挑战。
...
### [198] [Learning Subgroups with Maximum Treatment Effects without Causal Heuristics](https://arxiv.org/abs/2511.20189)
*Lincen Yang,Zhong Li,Matthijs van Leeuwen,Saber Salehkaleybar*

Main category: cs.LG

TL;DR: 本文提出在结构因果模型框架下直接研究最优亚组发现问题，证明在基于分区的模型假设下，最优亚组发现可简化为数据生成模型恢复问题，从而转化为标准监督学习问题。


<details>
  <summary>Details</summary>
Motivation: 现有亚组发现方法存在两个主要问题：一是将亚组估计转化为更困难的点估计问题，二是使用缺乏严格理论依据的启发式方法。本文旨在直接在SCM框架下解决这些问题。

Method: 在基于分区的模型假设下，将最优亚组发现问题转化为数据生成模型恢复问题，从而可以使用任何基于分区的方法学习亚组。具体使用CART算法来学习具有最大处理效应的亚组。

Result: 在大量合成和半合成数据集上的实验表明，该方法比广泛基线方法更准确地识别具有最大处理效应的亚组。

Conclusion: 避免使用因果启发式方法，直接在SCM框架下将亚组发现问题转化为监督学习问题，能够更准确地识别最优亚组。
...
### [199] [In-Context Compositional Learning via Sparse Coding Transformer](https://arxiv.org/abs/2511.20194)
*Wei Chen,Jingxi Yu,Zichen Miao,Qiang Qiu*

Main category: cs.LG

TL;DR: 本文提出了一种基于稀疏编码原理的注意力机制改进方法，用于增强Transformer在组合学习任务中的能力，特别是在处理上下文组合规则推断任务时表现优于标准Transformer。


<details>
  <summary>Details</summary>
Motivation: Transformer架构在语言、视觉和多模态任务中取得了显著成功，但在处理上下文组合学习任务时仍面临挑战。这些任务需要模型从上下文示例中推断组合规则，而Transformer本身并非为处理组合任务而设计，缺乏结构性归纳偏置。

Method: 受稀疏编码原理启发，重新构建注意力机制：将输入映射到输出时使用两组学习的字典原子（编码字典和解码字典）。编码字典将输入分解为一组稀疏系数，这些系数捕捉输入的组合结构；然后使用稀疏系数线性组合解码字典原子来生成输出。此外，还提出将目标问题的系数估计为上下文示例系数的线性组合。

Result: 在S-RAVEN和RAVEN数据集上验证了方法的有效性。对于某些组合泛化任务，该方法在标准Transformer失效时仍能保持性能，显示出学习和应用组合规则的能力。

Conclusion: 基于稀疏编码的注意力机制改进能够有效增强Transformer处理组合学习任务的能力，特别是在需要推断和应用组合规则的场景中表现出色。
...
### [200] [Interpretable Air Pollution Forecasting by Physics-Guided Spatiotemporal Decoupling](https://arxiv.org/abs/2511.20257)
...
<details>
  <summary>Details</summary>
Motivation: 准确的空气污染预测对公共健康至关重要，但现有模型在性能和可解释性之间存在权衡，需要开发既准确又透明的预测方法。
...
### [201] [HVAdam: A Full-Dimension Adaptive Optimizer](https://arxiv.org/abs/2511.20277)
*Yiheng Zhang,Shaowu Wu,Yuanzhuo Xu,Jiajun Wu,Shang Xu,Steve Drew,Xiaoguang Niu*

Main category: cs.LG

TL;DR: Anon是一种新型优化器，具有连续可调的适应性，能够在SGD和Adam行为之间插值甚至超越两者。通过引入增量延迟更新机制确保收敛，在图像分类、扩散模型和语言建模任务中优于现有优化器。


<details>
  <summary>Details</summary>
Motivation: 自适应优化器如Adam在大规模模型训练中表现出色，但在经典架构上泛化能力不如非自适应方法。研究发现预条件器的适应性限制了优化器适应不同优化景观的能力。

Method: 提出Anon优化器，具有连续可调的适应性参数。引入增量延迟更新机制，比AMSGrad的硬最大跟踪策略更灵活，增强了梯度噪声的鲁棒性。

Result: 理论上在凸和非凸设置下建立了收敛保证。实证表明在图像分类、扩散模型和语言建模任务中始终优于最先进的优化器。

Conclusion: 适应性可以作为有价值的可调设计原则，Anon提供了第一个统一可靠的框架，能够弥合经典和现代优化器之间的差距并超越它们的优势特性。
...
### [202] [MXtalTools: A Toolkit for Machine Learning on Molecular Crystals](https://arxiv.org/abs/2511.20327)
...
Method: 包含多个功能模块：数据集合成整理、模型训练推理工作流、晶体参数化表示、结构采样优化、端到端可微分晶体采样构建分析。

Result: 开发了模块化函数，可集成到现有工作流或构建新颖建模管道，利用CUDA加速实现高通量晶体建模。

Conclusion: MXtalTools作为开源Python包，为分子晶体建模提供了全面工具集，支持高效的数据驱动研究。
...
### [203] [MoRE: Batch-Robust Multi-Omics Representations from Frozen Pre-trained Transformers](https://arxiv.org/abs/2511.20382)
*Audrey Pei-Hsuan Chen*

Main category: cs.LG

TL;DR: MoRE是一个多组学表示学习框架，通过重用预训练transformer模型，使用参数高效微调策略将异质组学数据对齐到共享潜在空间中。


<details>
  <summary>Details</summary>
Motivation: 解决多组学数据中的极端维度、模态异质性和批次效应等挑战，探索预训练transformer在多组学整合中的应用潜力。

Method: 使用冻结的预训练transformer骨干，附加轻量级模态特定适配器和任务自适应融合层，联合优化掩码建模目标与监督对比学习和批次不变对齐损失。

Result: MoRE在批次鲁棒性和生物学保护方面表现优异，同时显著减少了可训练参数数量，在未见细胞类型和平台上具有良好泛化能力。

Conclusion: MoRE为实现通用组学基础模型提供了实用步骤，展示了参数高效微调在多组学整合中的有效性。
...
### [204] [Identifying environmental factors associated with tetrodotoxin contamination in bivalve mollusks using eXplainable AI](https://arxiv.org/abs/2511.20395)
*M. C. Schoppema,B. H. M. van der Velden,A. Hürriyetoğlu,M. D. Klijnstra,E. J. Faassen,A. Gerssen,H. J. van der Fels-Klerx*

Main category: cs.LG

TL;DR: 开发可解释的深度学习模型预测荷兰Zeeland河口双壳类软体动物中的河豚毒素污染，识别出日照时长、全球辐射、水温和氯化物浓度是主要影响因素。


<details>
  <summary>Details</summary>
Motivation: 自2012年以来，欧洲温带水域的双壳类软体动物中发现河豚毒素，带来食品安全风险和经济损失，需要早期预测河豚毒素污染。

Method: 开发基于深度学习的可解释模型，输入气象和水文特征，输出河豚毒素污染的存在或缺失。

Result: 日出时间、日落时间、全球辐射、水温和氯化物浓度对河豚毒素污染贡献最大，有效日照时长是重要驱动因素。

Conclusion: 可解释深度学习模型识别出日照时长、全球辐射、水温和氯化物浓度与双壳类软体动物河豚毒素污染相关，为食品行业和监管机构提供了有价值的风险缓解工具。
...
### [205] [Tight Margin-Based Generalization Bounds for Voting Classifiers over Finite Hypothesis Sets](https://arxiv.org/abs/2511.20407)
*Kasper Green Larsen,Natascha Schalburg*

Main category: cs.LG

TL;DR: 该论文首次证明了投票分类器的基于间隔的泛化边界，在假设集大小、间隔、具有给定间隔的训练点比例、训练样本数量和失败概率之间的权衡方面是渐近紧的。
...
Method: 采用基于间隔的泛化边界分析方法，通过数学证明推导出渐近紧的边界。

Result: 成功证明了第一个在多个关键参数（假设集大小、间隔、训练点比例等）之间具有渐近紧权衡的投票分类器泛化边界。

Conclusion: 该研究为投票分类器提供了理论上的严格保证，其边界在参数权衡方面达到了最优的渐近紧性。
...
### [206] [Diffusion for Fusion: Designing Stellarators with Generative AI](https://arxiv.org/abs/2511.20445)
...
<details>
  <summary>Details</summary>
Motivation: 仿星器设计通常需要耗费大量计算时间，而机器学习方法特别是生成模型可以加速这一过程。基于现有优化的仿星器数据库，开发快速生成高质量仿星器设计的方法。

Method: 在QUASR数据库上训练条件扩散模型，生成具有特定特性（纵横比和平均旋转变换）的准对称仿星器设计，并应用于训练数据中未见过的特性组合。

Result: 生成的仿星器表现出良好性能：与准对称性和目标特性的偏差小于5%，显示出达到低于1%偏差目标的潜力。

Conclusion: 生成建模在仿星器设计中具有广阔应用前景，为加速聚变研究提供了有前途的技术路径。
...
### [207] [Towards Trustworthy Wi-Fi Sensing: Systematic Evaluation of Deep Learning Model Robustness to Adversarial Attacks](https://arxiv.org/abs/2511.20456)
*Shreevanth Krishnaa Gopalakrishnan,Stephen Hailes*

Main category: cs.LG

TL;DR: 本文系统评估了基于CSI的深度学习模型在不同威胁模型下的鲁棒性，发现较小模型虽然效率高但在对抗攻击下鲁棒性较差，物理可实现扰动相比无约束特征空间攻击成功率更低，对抗训练能有效缓解这些漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习在基于CSI的人类感知系统中日益重要，这些系统依赖的模型决策可能被微妙扰动，在泛在感知应用中引发安全和可靠性担忧。在无线感知部署到真实环境前，量化理解模型鲁棒性至关重要。

Method: 建立了评估框架，比较紧凑时间自编码器模型与更深的架构在三个公共数据集上的表现，量化模型规模、训练机制和物理约束对鲁棒性的影响，测试了白盒、黑盒/迁移和通用扰动等不同威胁模型。

Result: 实验显示较小模型虽然效率高且在干净数据上表现相当，但鲁棒性明显较差；物理可实现信号空间扰动相比无约束特征空间攻击显著降低攻击成功率；对抗训练能缓解漏洞，提高平均鲁棒准确率，仅适度降低干净性能。
...
### [208] [NVIDIA Nemotron Parse 1.1](https://arxiv.org/abs/2511.20478)
...
TL;DR: Nemotron-Parse-1.1是一个轻量级文档解析和OCR模型，相比前代在OCR、Markdown格式、表格解析和图像文本提取方面有显著改进，支持更长输出序列，采用885M参数的编码器-解码器架构，在公开基准测试中表现优异。
...
Result: 在公开基准测试中达到竞争性准确率，同时发布了两个版本：标准版和优化版（Nemotron-Parse-1.1-TC），后者在视觉token长度减少的情况下提供20%速度提升且质量损失最小。

Conclusion: Nemotron-Parse-1.1是一个强大的轻量级OCR解决方案，提供了改进的文档解析能力，并公开发布了模型权重、优化容器和训练数据子集。
...
### [209] [MTBBench: A Multimodal Sequential Clinical Decision-Making Benchmark in Oncology](https://arxiv.org/abs/2511.20490)
...
TL;DR: MTBBench是一个模拟分子肿瘤委员会决策过程的基准测试，用于评估多模态大语言模型在临床肿瘤学中的复杂推理能力，发现现有模型在时间序列数据推理和多模态证据整合方面存在严重不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法捕捉真实临床工作流程的复杂性，特别是分子肿瘤委员会中多专家决策、多模态数据整合和时间演化洞察的需求。

Method: 开发MTBBench基准测试，包含临床挑战性的多模态和纵向肿瘤学问题，通过临床医生验证的真实标注，并提供基于基础模型的工具框架来增强多模态和时间序列推理。

Result: 测试显示现有LLM在可靠性方面存在严重问题，经常产生幻觉，难以处理时间分辨数据，无法调和冲突证据或不同模态。工具框架分别将任务级性能提升9.0%和11.2%。
...
### [210] [DP-MicroAdam: Private and Frugal Algorithm for Training and Fine-tuning](https://arxiv.org/abs/2511.20509)
...
Method: 开发DP-MicroAdam优化器，证明其在随机非凸优化中以最优O(1/√T)速率收敛，具有内存效率和稀疏感知特性。

Result: 在CIFAR-10、大规模ImageNet训练和预训练transformer私有微调等基准测试中，DP-MicroAdam优于现有自适应DP优化器，与DP-SGD竞争或更优。
...
### [211] [Adam Simplified: Bias Correction Simplified](https://arxiv.org/abs/2511.20516)
*Sam Laing,Antonio Orvieto*

Main category: cs.LG

TL;DR: 本文对Adam优化器中的偏差校正功能进行了系统性研究，发现在最优超参数配置下，偏差校正不会改善最终测试性能，有时甚至可能损害性能，除非实施适当的学习率调度。


<details>
  <summary>Details</summary>
Motivation: Adam优化器是现代深度学习的基石，但其各个组件的经验必要性往往被视为理所当然。本文旨在深入探究偏差校正这一贡献尚不明确的功能的作用。

Method: 通过在视觉和语言建模任务上进行一系列系统性消融实验，分析偏差校正的影响，并将其重新解释为一种隐式学习率调度形式。

Result: 在最优超参数配置下，包含偏差校正不会提高最终测试性能。除非实施适当的学习率调度，否则包含偏差校正有时可能损害性能。偏差校正的行为强烈依赖于平滑超参数β1、β2的选择。

Conclusion: 研究结果挑战了偏差校正组件普遍包含的常规做法，表明其必要性可能被高估。
...
### [212] [Feature-Modulated UFNO for Improved Prediction of Multiphase Flow in Porous Media](https://arxiv.org/abs/2511.20543)
*Alhasan Abdellatif,Hannah P. Menke,Ahmed H. Elsheikh,Florian Doster,Kamaljit Singh*

Main category: cs.LG

TL;DR: UFNO-FiLM通过引入FiLM层解耦标量输入与空间特征，并使用空间加权损失函数，在保持UFNO高精度优势的同时解决了其处理标量输入效率低下的问题，在地下多相流预测中相比UFNO将气体饱和度MAE降低了21%。


<details>
  <summary>Details</summary>
Motivation: UFNO虽然通过并行UNet路径提升了FNO的预测精度，但存在两个主要问题：1）将标量输入（如温度、注入速率）作为空间分布场处理，导致在频域中处理冗余的恒定信号；2）标准损失函数未考虑误差敏感性的空间变化，限制了在关键物理区域的性能。

Method: 1. 使用FiLM层解耦标量输入与空间特征，避免将恒定信号引入傅里叶变换；2. 采用空间加权损失函数，优先学习关键区域。

Result: 在地下多相流实验中，相比UFNO，UFNO-FiLM将气体饱和度平均绝对误差降低了21%，显著提升了预测精度。

Conclusion: UFNO-FiLM通过FiLM层和空间加权损失的创新设计，有效解决了UFNO在处理标量输入时的效率问题，并提升了在关键物理区域的预测性能，为复杂物理系统的建模提供了更高效的解决方案。
...
### [213] [E2E-GRec: An End-to-End Joint Training Framework for Graph Neural Networks and Recommender Systems](https://arxiv.org/abs/2511.20564)
...
Result: 在大规模生产数据上的离线评估和在线A/B测试显示，E2E-GRec在多个推荐指标上显著优于传统方法，如停留时长相对提升0.133%，用户跳过视频平均数减少0.3171%。

Conclusion: E2E-GRec框架通过端到端训练统一GNN与推荐系统，实现了更好的性能表现和优化效果。
...
### [214] [A Tale of Two Geometries: Adaptive Optimizers and Non-Euclidean Descent](https://arxiv.org/abs/2511.20584)
*Shuo Xie,Tianhao Wang,Beining Wu,Zhiyuan Li*

Main category: cs.LG

TL;DR: 本文揭示了自适应优化器与归一化最速下降(NSD)之间的紧密联系，分析了它们在凸和非凸设置下的收敛特性，并证明了自适应平滑性条件在非欧几里得几何中能够实现加速收敛。


<details>
  <summary>Details</summary>
Motivation: 研究自适应优化器与归一化最速下降之间的理论联系，特别是在不同几何结构下的收敛特性差异，以及探索自适应平滑性条件在非凸优化中的扩展。

Method: 将自适应平滑性理论扩展到非凸设置，分析自适应优化器的收敛特性，并研究Nesterov动量在凸设置下的加速效果。同时引入自适应梯度方差概念进行随机优化分析。

Result: 证明了自适应平滑性精确刻画了自适应优化器的收敛特性，在凸设置下能够实现Nesterov动量加速，在非欧几里得几何中获得了标准平滑性无法达到的收敛保证。

Conclusion: 自适应平滑性是理解自适应优化器性能的关键理论框架，在非欧几里得几何中提供了比标准平滑性更强的收敛保证，为自适应优化算法的理论分析提供了新视角。
...
### [215] [Anatomica: Localized Control over Geometric and Topological Properties for Anatomical Diffusion Models](https://arxiv.org/abs/2511.20587)
*Karim Kadry,Abdallah Abdelwahed,Shoaib Goraya,Ajay Manicka,Naravich Chutisilp,Farhad Nezami,Elazer Edelman*

Main category: cs.LG

TL;DR: Anatomica是一个推理时框架，用于生成具有局部几何拓扑控制的多类解剖体素图。通过使用不同维度、位置和形状的立方体控制域来提取相关子结构，并计算可微分的惩罚函数来引导样本满足目标约束。
...
Method: 使用立方体控制域提取局部子结构，通过体素矩控制几何特征（大小、形状、位置），通过持久同调控制拓扑特征（连通分量、环、空洞），在潜在扩散模型中实现神经场解码器部分提取子结构。
...
### [216] [Attention Trajectories as a Diagnostic Axis for Deep Reinforcement Learning](https://arxiv.org/abs/2511.20591)
*Charlotte Beylier,Hannah Selder,Arthur Fleig,Simon M. Hofmann,Nico Scherf*

Main category: cs.LG

TL;DR: 本文提出了注意力导向指标（ATOMs）来研究强化学习智能体在训练过程中的注意力发展，通过三个Pong游戏变体实验验证了ATOMs能够区分不同游戏变体训练出的智能体的注意力模式，并发现这些注意力差异会转化为行为差异。


<details>
  <summary>Details</summary>
Motivation: 强化学习智能体的学习过程除了数学公式外仍缺乏深入理解，需要新的方法来研究智能体在训练过程中的注意力发展。

Method: 引入注意力导向指标（ATOMs），在三个设计用于教授不同行为的Pong游戏变体上进行受控实验，结合行为评估来验证ATOMs的有效性。

Result: ATOMs成功区分了在不同游戏变体上训练的智能体的注意力模式，这些注意力模式的差异转化为行为差异；通过持续监控发现智能体注意力发展呈现阶段性，且这些阶段在不同游戏变体中保持一致。
...
### [217] [Latent Diffusion Inversion Requires Understanding the Latent Space](https://arxiv.org/abs/2511.20592)
*Mingxing Rao,Bowen Qu,Daniel Moyer*

Main category: cs.LG

TL;DR: 本文研究了潜在扩散模型中的记忆效应，发现扩散模型在潜在代码上表现出不均匀的记忆化，倾向于过拟合位于解码器回拉度量高失真区域的样本。作者提出了一种按维度对记忆化贡献进行排序的方法，并证明在成员推理攻击中移除较少记忆化维度能显著提升性能。
...
Method: 作者提出了一种原则性方法来按维度对解码器回拉度量的贡献进行排序，识别出对记忆化贡献最大的维度。在基于分数的成员推理攻击中，移除较少记忆化维度来计算攻击统计量。

Result: 实验表明，移除较少记忆化维度后，成员推理攻击性能显著提升，平均AUROC增益达2.7%，在极低误报率下TPR@1%FPR大幅增加6.42%，在多个数据集上均表现一致。
...
### [218] [The Driver-Blindness Phenomenon: Why Deep Sequence Models Default to Autocorrelation in Blood Glucose Forecasting](https://arxiv.org/abs/2511.20601)
*Heman Shakeri*

Main category: cs.LG

TL;DR: 该论文指出深度序列模型在血糖预测中未能有效利用胰岛素、饮食和活动等临床信息驱动因素，提出了"驱动因素盲视"概念，并分析了其三个成因：架构偏好自相关、数据保真度不足和生理异质性。


<details>
  <summary>Details</summary>
Motivation: 当前深度序列模型在血糖预测中虽然表现良好，但未能有效利用已知的临床驱动因素（胰岛素、饮食、活动），这与生理机制的理解相矛盾。作者旨在识别并解决这种"驱动因素盲视"问题。

Method: 通过定义Δ_drivers指标（多变量模型与匹配单变量基线的性能增益）来量化驱动因素盲视，并分析其三个成因：架构偏见(C1)、数据保真度差距(C2)和生理异质性(C3)。

Result: 研究发现文献中Δ_drivers通常接近零，表明多变量模型未能有效利用驱动因素。提出了部分缓解策略，包括生理特征编码器、因果正则化和个性化方法。

Conclusion: 建议未来研究应常规报告Δ_drivers指标，防止驱动因素盲视的模型被误认为是先进技术，并推荐采用生理特征编码、因果正则化和个性化等策略来改善模型性能。
...
### [219] [DiFR: Inference Verification Despite Nondeterminism](https://arxiv.org/abs/2511.20621)
*Adam Karvonen,Daniel Reuter,Roy Rinberg,Luke Marks,Adrià Garriga-Alonso,Keri Warr*

Main category: cs.LG

TL;DR: Token-DiFR是一种通过比较生成token与可信参考实现预测来验证LLM推理输出的方法，使用采样种子同步来约束有效输出，使输出token本身成为零成本的可审计证据。


<details>
  <summary>Details</summary>
Motivation: 随着LLM推理需求增长，需要验证推理过程是否正确执行且无错误或篡改，但重复运行相同推理过程会因良性数值噪声产生不同结果，难以区分合法变化与实际问题。

Method: 引入Token-DiFR方法，通过比较生成token与在相同随机种子条件下可信参考实现的预测来验证推理输出；同时提出Activation-DiFR方案，使用随机正交投影压缩激活值为紧凑指纹进行验证。
...
Conclusion: 提出的方法能有效验证LLM推理正确性，Token-DiFR使输出token本身成为可审计证据，Activation-DiFR提供样本高效的验证方案，并开源集成vLLM以加速可验证推理的实际部署。
...
### [220] [ROOT: Robust Orthogonalized Optimizer for Neural Network Training](https://arxiv.org/abs/2511.20626)
*Wei He,Kai Han,Hang Zhou,Hanting Chen,Zhicheng Liu,Xinghao Chen,Yunhe Wang*

Main category: cs.LG

TL;DR: 本文提出了ROOT优化器，通过双稳健机制解决大语言模型训练中的正交化精度和异常值噪声问题，在噪声和非凸场景下实现了更快的收敛和更好的最终性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型优化面临算法精度敏感性和训练不稳定性挑战，现有优化器在正交化精度和异常值噪声处理方面存在稳健性不足的问题。

Method: 提出ROOT优化器，采用维度稳健的正交化方案（自适应牛顿迭代）和优化稳健框架（近端优化），确保跨架构的一致精度并抑制异常值噪声。

Result: 实验表明ROOT相比Muon和Adam优化器在稳健性、收敛速度和最终性能方面均有显著提升，特别是在噪声和非凸场景下表现优异。

Conclusion: ROOT为开发能够处理现代大规模模型训练复杂性的稳健精确优化器建立了新范式。
...
### [221] [Image2Gcode: Image-to-G-code Generation for Additive Manufacturing Using Diffusion-Transformer Model](https://arxiv.org/abs/2511.20636)
...
Result: 建立了从视觉输入到原生刀具路径的直接映射，消除了对CAD或STL中间文件的需求，降低了增材制造的门槛，加速了从设计到制造的周期。
...
### [222] [Can You Keep Calm?: Adaptive Gameplay using Heart Rate as a Controller](https://arxiv.org/abs/2511.19934)
*Md Mosharaf Hossan,Rifat Ara Tasnim,Farjana Z Eishita*

Main category: cs.HC

TL;DR: 本研究开发了一款心率控制的游戏，通过生物反馈调整游戏玩法来帮助玩家管理压力，实验结果显示心率控制游戏能减少负面情绪、增加正面情绪，并降低心脏反应性。


<details>
  <summary>Details</summary>
Motivation: 利用配备传感器的智能手机和智能手表等手持设备，开发具有特定健康目标的严肃游戏，为心理健康干预提供可访问且引人入胜的治疗环境。

Method: 引入心率控制游戏，根据玩家的生物反馈调整游戏玩法，通过对照实验研究心率控制对玩家体验的影响及其减压效果。
...
### [223] [Editing with AI: How Doctors Refine LLM-Generated Answers to Patient Queries](https://arxiv.org/abs/2511.19940)
*Rahul Sharma,Pragnya Ramjee,Kaushik Murali,Mohit Jain*

Main category: cs.HC

TL;DR: 本研究探讨医生如何改进LLM生成的医疗回复草稿，发现LLM输出通常准确但需要人工监督，情境化是主要编辑形式，不同编辑工作流程存在效率与准确性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 数字患者信息量激增给医疗系统带来压力，LLM有望为临床医生生成回复草稿，但医生如何改进这些草稿尚待研究。

Method: 采用混合方法研究，9名眼科医生回答144个白内障手术问题，比较三种条件：从头编写、直接编辑LLM草稿、基于指令的间接编辑。

Result: LLM输出通常准确但存在错误和自动化偏见，情境化是主要编辑形式，间接编辑减少工作量但引入错误，直接编辑确保精确性但工作量更高。

Conclusion: 为构建安全、可扩展的LLM辅助临床沟通系统提供了设计和政策建议。
...
### [224] [Discover, Learn, and Reinforce: Scaling Vision-Language-Action Pretraining with Diverse RL-Generated Trajectories](https://arxiv.org/abs/2511.19528)
*Rushuai Yang,Zhiyuan Feng,Tianxiang Zhang,Kaixin Wang,Chuheng Zhang,Li Zhao,Xiu Su,Yi Chen,Jiang Bian*

Main category: cs.RO

TL;DR: DLR是一个信息理论模式发现框架，通过强化学习生成多种不同的高成功率行为模式，为视觉-语言-动作模型预训练提供多样化的轨迹数据，相比标准RL方法能产生更丰富的状态-动作空间覆盖。


<details>
  <summary>Details</summary>
Motivation: 解决VLA模型预训练所需的大规模多样化高质量操作轨迹数据获取难题，当前人工遥操作成本高且难以扩展，而标准RL训练会收敛到单一执行模式，限制了其在大规模预训练中的实用性。

Method: 提出Discover、Learn和Reinforce（DLR）框架，基于信息理论进行模式发现，生成多种不同的高成功率行为模式，避免标准RL训练的模式坍塌问题。

Result: 在LIBERO基准测试中，DLR生成了明显更多样化的轨迹语料库，为相同任务学习多种不同的高成功率策略，而标准RL只发现一种策略，覆盖了更广泛的状态-动作空间区域。

Conclusion: DLR展示了多模式RL作为具身基础模型实用、可扩展数据引擎的潜力，在未见下游任务套件上，基于DLR多样化RL数据预训练的VLA模型优于同等规模标准RL数据集训练的对应模型，并展现出标准单模式RL所缺乏的正向数据缩放行为。
...
### [225] [A Virtual Mechanical Interaction Layer Enables Resilient Human-to-Robot Object Handovers](https://arxiv.org/abs/2511.19543)
*Omar Faris,Sławomir Tadeja,Fulvio Forni*

Main category: cs.RO

TL;DR: 提出了一种使用虚拟模型控制和增强现实的人机物体交接方法，能够适应交接过程中的动态变化和不确定性。


<details>
  <summary>Details</summary>
Motivation: 解决人机物体交接过程中物体姿态复杂变化时的适应性挑战，确保机器人动作的鲁棒性。

Method: 使用虚拟模型控制创建交互层来控制机器人并适应交接过程的动态变化，同时利用增强现实促进人机双向通信。

Result: 控制器在各种不确定性条件下表现出良好的鲁棒性，用户研究显示参与者普遍偏好所提出的方法。

Conclusion: 该方法在人机物体交接中具有良好效果，为适应用户交互的进一步开发提供了指导性见解。
...
### [226] [Robot-Powered Data Flywheels: Deploying Robots in the Wild for Continual Data Collection and Foundation Model Adaptation](https://arxiv.org/abs/2511.19647)
*Jennifer Grannen,Michelle Pan,Kenneth Llontop,Cherie Ho,Mark Zolotas,Jeannette Bohg,Dorsa Sadigh*

Main category: cs.RO

TL;DR: 论文提出了机器人驱动的数据飞轮框架，将机器人从基础模型消费者转变为数据生成器，通过在真实环境中部署机器人收集数据来改进基础模型的领域特定适应性和领域相邻泛化能力。


<details>
  <summary>Details</summary>
Motivation: 基础模型在视觉和语言任务中展现了强大的零样本能力，但由于依赖互联网预训练数据，在非结构化真实世界环境中表现脆弱。机器人作为具身智能体，能够通过物理环境互动收集大规模真实世界数据，弥补现有数据集的不足。

Method: 引入机器人驱动的数据飞轮框架，部署配备基础模型的机器人在真实环境中执行任务并收集数据。具体实例化为Scanford移动操作机器人，在图书馆中自主扫描书架，使用视觉语言模型识别书籍，并利用图书馆目录进行自动标注。

Result: 在2103个书架的数据收集后，Scanford将书籍识别准确率从32.0%提升至71.8%，多语言OCR性能从24.8%提升至46.6%（英文）和30.8%提升至38.0%（中文），同时节省约18.7小时人工时间。

Conclusion: 机器人驱动的数据飞轮不仅能在实际部署中减少人工努力，还为持续适应基础模型应对现实复杂性开辟了新途径，展示了机器人作为数据生成器的巨大潜力。
...
### [227] [Online Learning-Enhanced High Order Adaptive Safety Control](https://arxiv.org/abs/2511.19651)
*Lishuo Pan,Mattia Catellani,Thales C. Silva,Lorenzo Sabattini,Nora Ayanian*

Main category: cs.RO

TL;DR: 提出了一种基于神经ODE的在线学习增强高阶自适应控制屏障函数方法，用于在复杂时变模型扰动下提高系统安全性，并在38克纳米四旋翼无人机上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 控制屏障函数(CBFs)是保证系统安全性的有效工具，但在实际应用中其安全性保证严重依赖于模型精度。负载或风扰动等复杂时变扰动会显著影响系统动态，使原有的安全保证失效。

Method: 采用神经ODE构建在线学习增强的高阶自适应控制屏障函数，结合混合自适应CBF控制器，能够在飞行过程中实时适应复杂的时变模型扰动。

Result: 在38克纳米四旋翼无人机上成功部署该控制器，在18公里/小时的风速下仍能保持与障碍物的安全距离。

Conclusion: 该方法能够有效提高CBF认证系统在复杂时变模型扰动下的安全性，实现了在真实环境中的安全保证。
...
### [228] [Development of a Testbed for Autonomous Vehicles: Integrating MPC Control with Monocular Camera Lane Detection](https://arxiv.org/abs/2511.19655)
*Shantanu Rahman,Nayeb Hasin,Mainul Islam*

Main category: cs.RO

TL;DR: 本文提出了一种结合车道识别与模型预测控制(MPC)的新方法，用于提高自动驾驶汽车轨迹跟踪的精度和稳定性。通过边缘识别、滑动窗口直线识别和动态感兴趣区域提取等技术进行车道线检测，并基于自行车车辆动力学模型构建MPC控制器，在ROS Gazebo仿真环境中验证了控制器的性能。


<details>
  <summary>Details</summary>
Motivation: 随着自动驾驶车辆在道路行驶、工业自动化、农业和军事等领域的日益普及，提高轨迹跟踪的精度和稳定性变得尤为重要。标准车辆大多采用阿克曼转向机制，而底层控制器的性能直接影响车辆的电机执行效果。

Method: 1. 使用边缘识别、滑动窗口直线识别和动态感兴趣区域提取进行车道线检测
2. 基于自行车车辆动力学模型构建模型预测控制(MPC)控制器
3. 在ROS Gazebo中构建单车道道路仿真模型进行性能验证

Result: 仿真结果显示，最优跟踪轨迹与目标轨迹之间的均方根误差减少了27.65%，证明了所开发控制器具有高鲁棒性和灵活性。

Conclusion: 结合车道识别与模型预测控制的方法有效提高了自动驾驶车辆的轨迹跟踪性能，在仿真环境中验证了控制器的优越性能，为实际应用提供了可靠的技术支持。
...
### [229] [Whole-Body Inverse Dynamics MPC for Legged Loco-Manipulation](https://arxiv.org/abs/2511.19709)
*Lukas Molnar,Jin Cheng,Gabriele Fadini,Dongho Kang,Fatemeh Zargarbashi,Stelian Coros*

Main category: cs.RO

TL;DR: 提出了一种全身模型预测控制框架，通过全阶逆动力学直接优化关节扭矩，在单一预测层内实现运动与力的统一规划与执行，并在四足机器人上实现了80Hz的实时性能。
...
Conclusion: 该MPC框架能够实现物理一致的全身行为，有效处理系统动力学和物理约束，为全身操作任务提供了统一的规划与控制解决方案。
...
### [230] [Unifying Perception and Action: A Hybrid-Modality Pipeline with Implicit Visual Chain-of-Thought for Robotic Action Generation](https://arxiv.org/abs/2511.19859)
*Xiangkai Ma,Lekai Xing,Han Zhang,Wenzhong Li,Sanglu Lu*

Main category: cs.RO

TL;DR: VITA框架通过构建视觉和动作的共享离散潜在空间，解决了视觉观察与低级动作之间的模态差距问题，并在模拟和真实环境中实现了最先进的性能表现。


<details>
  <summary>Details</summary>
Motivation: 解决文本CoT在复杂空间环境中难以捕捉场景细节的问题，以及现有方法面临的视觉观察与低级动作之间的模态差距、视觉预测与动作生成目标冲突导致的训练不稳定等挑战。

Method: 提出Vision-Integrated Trajectory Alignment (VITA)框架，学习视觉和动作的共享离散潜在空间，引入隐式视觉CoT机制，自回归生成的token同时解码为未来帧预测和机器人动作。

Result: 在CALVIN、LIBERO和SimplerEnv基准上分别比现有基线提升14.5%、9.6%和12.1%，在六个真实世界任务中达到80.5%的平均成功率。

Conclusion: VITA通过将视觉动态内化为运动规划的归纳偏置，展示了作为通用机器人操作模型的潜力，在模拟和真实环境中均表现出色。
...
### [231] [Human-Centered Cooperative Control Coupling Autonomous and Haptic Shared Control via Control Barrier Function](https://arxiv.org/abs/2511.19869)
*Eito Sato,Takahiro Wada*

Main category: cs.RO

TL;DR: 提出了一种结合自主控制器与触觉共享控制的协作框架，通过控制屏障函数在安全区域内忽略操纵杆输入，在虚拟环境中的水下机器人模拟任务中证明了比传统触觉共享控制更高的精度和更短的操作时间。


<details>
  <summary>Details</summary>
Motivation: 传统触觉共享控制(HSC)在最大化控制强度时性能受限，因为操纵杆和人体手臂的动力学会影响机器人行为，需要解决这一问题以提高遥操作性能。

Method: 提出了一个协作框架，将独立于操纵杆的自主控制器与触觉共享控制相结合，使用控制屏障函数在由操作员实时确定的安全区域内忽略操纵杆输入，否则启用触觉共享控制。

Result: 在虚拟环境中的水下机器人模拟任务实验中，相比传统触觉共享控制，该方法提高了操作精度并减少了所需时间。

Conclusion: 所提出的协作框架能够有效克服传统触觉共享控制的局限性，在遥操作任务中实现更好的性能表现。
...
### [232] [CoC-VLA: Delving into Adversarial Domain Transfer for Explainable Autonomous Driving via Chain-of-Causality Visual-Language-Action Model](https://arxiv.org/abs/2511.19914)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖真实世界数据，要么专注于模拟环境中的罕见场景，很少能有效整合两者的互补优势。

Method: 采用教师-学生VLM模型架构，包含共享的因果链视觉语言模型(CoC VLM)，通过对抗性鉴别器和新的反向传播策略实现从模拟到真实世界的长尾能力迁移。

Result: 框架能够将模拟环境中学到的长尾场景处理能力有效迁移到真实世界部署中。

Conclusion: CoC-VLA框架成功解决了模拟与真实世界数据源的有效整合问题，提升了自动驾驶系统在复杂长尾场景下的处理能力。
...
### [233] [Collaborate sim and real: Robot Bin Packing Learning in Real-world and Physical Engine](https://arxiv.org/abs/2511.19932)
*Lidi Zhang,Han Wu,Liyu Zhang,Ruofeng Liu,Haotian Wang,Chao Li,Desheng Zhang,Yunhuai Liu,Tian He*

Main category: cs.RO

TL;DR: 本文提出了一种混合强化学习框架，通过物理仿真与真实世界数据反馈相结合，解决3D装箱问题中的仿真到现实差距问题，在物流系统中实现了35%的装箱坍塌率降低。


<details>
  <summary>Details</summary>
Motivation: 现有3D装箱方法通常将其建模为离散静态过程，而实际应用涉及连续重力驱动交互，这种理想化简化导致实际部署不可行（如不稳定装箱）。物理仿真虽然能模拟重力效应，但存在仿真到现实差距，主要由于真实物体物理属性的动态变化。

Method: 提出混合强化学习框架：1）在仿真中应用领域随机化，让智能体接触各种物理参数以增强泛化能力；2）利用真实世界部署反馈对RL智能体进行微调，进一步降低坍塌率。

Result: 大量实验表明，该方法在仿真和真实场景中都实现了更低的坍塌率。在物流系统的大规模部署验证了实际有效性，相比基线方法装箱坍塌率降低了35%。

Conclusion: 混合强化学习框架通过结合物理仿真和真实世界反馈，有效缩小了仿真到现实差距，显著提高了3D装箱的稳定性和实际部署效果。
...
### [234] [ShapeForce: Low-Cost Soft Robotic Wrist for Contact-Rich Manipulation](https://arxiv.org/abs/2511.19955)
*Jinxuan Zhu,Zihao Yan,Yangyu Xiao,Jingxiang Guo,Chenrui Tie,Xinyi Cao,Yuhang Zheng,Lin Shao*

Main category: cs.RO

TL;DR: ShapeForce是一种低成本、即插即用的软手腕，通过将外力转换为可测量的形变来提供类似力的信号，用于接触丰富的机器人操作，性能可与六轴力扭矩传感器相媲美。


<details>
  <summary>Details</summary>
Motivation: 六轴力扭矩传感器成本高且易碎，限制了在接触丰富任务中的应用。ShapeForce旨在提供更经济、易获取的接触反馈源。

Method: ShapeForce将外力和扭矩转换为其柔性核心的可测量形变，通过基于标记的位姿跟踪进行估计，并转换为类似力的信号，无需校准或专用电子设备。

Result: 在多种接触丰富任务和操作策略的广泛实验中，ShapeForce以极低成本实现了与六轴力扭矩传感器相当的性能。

Conclusion: ShapeForce为接触丰富的机器人操作提供了一种低成本、高性能的接触反馈解决方案，无需精确的力值测量，而是关注力的变化来支持操作任务。
...
### [235] [How Robot Kinematics Influence Human Performance in Virtual Robot-to-Human Handover Tasks](https://arxiv.org/abs/2511.20299)
*Róisín Keenan,Joost C. Dessing*

Main category: cs.RO

TL;DR: 本研究使用VR技术研究人机协作中物体交接任务，探讨任务动态和机器人运动学对人类运动表现的影响，发现人类从机器人提供的早期视觉信息和类人平滑轨迹中获益。


<details>
  <summary>Details</summary>
Motivation: 随着机器人技术发展，需要优化人机协作中的协调性，特别是在物体交接任务中的人类表现。

Method: 使用VR机器人交接模拟进行四个独立实验，分别考察任务启动控制、伙伴外观、机器人速度曲线和旋转运动时机对人类表现的影响。

Result: 实验表明人类从机器人提供的早期视觉信息和类人平滑轨迹中获益，提高了预测准确性和交互同步性。

Conclusion: 人机交互设计应让人类利用其检测生物运动的自然能力，从而减少机器人计算成本或人类认知适应需求。
...
### [236] [Active3D: Active High-Fidelity 3D Reconstruction via Hierarchical Uncertainty Quantification](https://arxiv.org/abs/2511.20050)
*Yan Li,Yingzhao Li,Gim Hee Lee*

Main category: cs.RO

TL;DR: 本文提出了一种用于高保真3D重建的主动探索框架，通过构建多级不确定性空间和使用不确定性驱动的运动规划器来选择最佳视角。


<details>
  <summary>Details</summary>
Motivation: 现有3D重建方法在主动探索和不确定性量化方面存在不足，需要一种能够联合捕捉全局结构先验和局部观察细节的混合表示方法。

Method: 采用混合隐式-显式表示融合神经场和高斯基元，构建分层不确定性体积，提出不确定性驱动的关键帧选择策略和风险敏感路径规划器。
...
### [237] [Hibikino-Musashi@Home 2025 Team Description Paper](https://arxiv.org/abs/2511.20180)
...
TL;DR: 本文概述了Hibikino-Musashi@Home团队在家庭服务机器人领域的技术方法，包括数据集生成器、开源开发环境、大语言模型任务规划器、脑启发记忆模型等，旨在为家庭环境提供直观个性化辅助。


<details>
  <summary>Details</summary>
Motivation: 设计能够协助人类在家庭环境中生活的家庭服务机器人，并通过持续参加比赛来评估和改进开发的系统。

Method: 开发了用于训练机器人视觉系统的数据集生成器、基于Human Support Robot模拟器的开源开发环境、大语言模型驱动的任务规划器来选择适当的原始技能，以及研究脑启发记忆模型以适应个体家庭环境。

Result: 团队为Pumas在RoboCup2024中开发的导航系统贡献了可重用性，构建了完整的家庭服务机器人技术栈。

Conclusion: 通过综合运用数据集生成、模拟环境、大语言模型规划和脑启发记忆等技术，团队成功开发了能够提供直观个性化辅助的家庭服务机器人系统，并通过竞赛持续改进系统性能。
...
### [238] [Toward generic control for soft robotic systems](https://arxiv.org/abs/2511.20226)
...
TL;DR: 提出了一种基于控制柔顺性的通用软体机器人控制框架，通过利用而非抑制近似动作表示来实现鲁棒性和适应性，在不同形态和驱动机制的软体机器人上验证了稳定、安全且可跨平台迁移的行为。


<details>
  <summary>Details</summary>
Motivation: 软体机器人控制方法仍然碎片化，不同形态和驱动方案需要特定任务的控制器，阻碍了理论整合和大规模部署。刚性机器人控制逻辑依赖精确模型和严格低级执行，但这对软体机器人无效，控制柔顺性（容忍和利用近似动作表示的能力）是鲁棒性和适应性的基础。

Method: 受人类运动控制启发，提出基于控制柔顺性的通用软体机器人控制框架，通过高层运动倾向表达意图，而反射和生物力学机制自主解决局部细节，而不是计算精确动力学或发出详细的肌肉级命令。

Result: 在具有不同形态和驱动机制的机器人上验证了该框架，结果展示了稳定、安全且可跨平台迁移的行为。

Conclusion: 拥抱而非抵抗控制柔顺性可能为统一的软体机器人控制提供广泛适用的基础，表明控制应该从抑制柔顺性转向明确利用它。
...
### [239] [Dynamic-ICP: Doppler-Aware Iterative Closest Point Registration for Dynamic Scenes](https://arxiv.org/abs/2511.20292)
*Dong Wang,Daniel Casado Herraez,Stefan May,Andreas Nüchter*

Main category: cs.RO

TL;DR: Dynamic-ICP是一种基于多普勒感知的激光雷达里程计框架，专门用于处理高度动态环境中的定位问题，通过结合几何残差和多普勒残差来提升在动态场景中的稳定性和精度。


<details>
  <summary>Details</summary>
Motivation: 传统基于ICP的里程计方法假设场景基本静态，在高度动态、重复纹理或低纹理几何环境中性能会显著下降，需要一种能够有效处理动态物体的可靠定位方法。

Method: 方法包括四个步骤：(1)通过鲁棒回归从点云多普勒速度估计自身运动并构建速度滤波器；(2)聚类动态物体并从自身补偿后的径向测量重建物体平移速度；(3)使用恒定速度模型预测动态点；(4)结合点对面几何残差和仅旋转不变的多普勒残差进行扫描对齐。

Result: 在三个高度动态数据集(HeRCULES、HeLiPR、AevaScenes)上的评估显示，Dynamic-ICP在旋转稳定性和平移精度方面持续优于现有最先进方法。

Conclusion: Dynamic-ICP提供了一种无需外部传感器或传感器-车辆标定的轻量级解决方案，能够实时运行并轻松集成到现有流程中，为动态环境中的鲁棒定位提供了有效方法。
...
### [240] [ArtiBench and ArtiBrain: Benchmarking Generalizable Vision-Language Articulated Object Manipulation](https://arxiv.org/abs/2511.20330)
*Yuhan Wu,Tiantian Wei,Shuo Wang,ZhiChao Wang,Yanyong Zhang,Daniel Cremers,Yan Xia*

Main category: cs.RO

TL;DR: 本文提出了ArtiBench基准测试和ArtiBrain框架，用于解决关节物体操作中的泛化挑战。ArtiBench包含五个级别的测试，涵盖厨房、存储、办公和工具环境；ArtiBrain结合高层推理与自适应低层控制，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言和扩散的策略在跨部件、实例和类别的关节操作任务中泛化能力不足，需要新的基准和方法来解决这些挑战。

Method: 提出ArtiBrain模块化框架：使用VLM任务推理器分解和验证子目标；混合控制器结合几何感知关键帧执行和affordance引导扩散；Affordance记忆库持续积累成功执行经验并传播到未见部件。

Result: 在ArtiBench上的大量实验表明，ArtiBrain在鲁棒性和泛化能力上显著优于最先进的多模态和基于扩散的方法。

Conclusion: ArtiBench基准揭示了关节物体操作的核心泛化挑战，ArtiBrain框架通过统一高层推理与自适应低层控制，有效解决了这些挑战，为交互式关节操作提供了新的解决方案。
...
### [241] [Quality-guided UAV Surface Exploration for 3D Reconstruction](https://arxiv.org/abs/2511.20353)
*Benjamin Sportich,Kenza Boubakri,Olivier Simonin,Alessandro Renzaglia*

Main category: cs.RO

TL;DR: 本文提出了一种用于空中机器人的模块化Next-Best-View规划框架，通过重建质量目标指导探索规划，在视图生成和候选点选择方面引入高效方法，适应不同质量需求。


<details>
  <summary>Details</summary>
Motivation: 现有规划策略往往忽视环境映射的实际需求，快速信息收集和全面结构评估需要不同的方法。本文旨在开发能够根据用户定义的质量要求进行自适应探索的规划框架。

Method: 提出模块化NBV规划框架，利用TSDF环境表示中的不确定性，开发高效的视图生成和候选视点选择方法，这些方法能够适应预定义的质量要求。

Result: 通过广泛仿真验证，该方法成功根据用户目标调整行为，在覆盖率、最终3D地图质量和路径效率方面持续优于传统NBV策略。

Conclusion: 该框架能够实现针对预定目标的信息化和高效探索决策，为空中机器人的环境映射提供了更有效的解决方案。
...
### [242] [Improved adaptive wind driven optimization algorithm for real-time path planning](https://arxiv.org/abs/2511.20394)
*Shiqian Liu,Azlan Mohd Zain,Le-le Mao*

Main category: cs.RO

TL;DR: 本文提出了一种多层级自适应风驱动优化算法(MAWDO)，通过分层引导机制改进动态环境中的适应性和鲁棒性，在路径规划中实现了更短、更平滑的无碰撞轨迹。
...
Method: 提出MAWDO算法，采用分层引导机制将种群划分为多个组，由个体、区域和全局领导者引导，平衡探索与开发。

Result: 在16个基准函数上评估显示MAWDO在优化精度、收敛稳定性和适应性方面优于现有元启发式算法；在动态路径规划中，路径长度缩短至469.28像素，比MEWDO、AWDO和WDO分别提升3.51%、11.63%和14.93%，最优性差距最小(1.01)，平滑度为0.71。

Conclusion: MAWDO算法能够生成更平滑、更短的无碰撞轨迹，证实了其在复杂环境中实时路径规划的有效性。
...
### [243] [Splatblox: Traversability-Aware Gaussian Splatting for Outdoor Robot Navigation](https://arxiv.org/abs/2511.18525)
*Samarth Chopra,Jing Liang,Gershom Seneviratne,Yonghan Lee,Jaehoon Choi,Jianyu An,Stephen Cheng,Dinesh Manocha*

Main category: cs.RO

TL;DR: Splatblox是一个用于室外植被密集环境的实时自主导航系统，通过融合RGB图像分割和LiDAR点云，使用高斯泼溅构建可通行性感知的欧几里得符号距离场，在四足机器人和轮式平台上验证了优越性能。


<details>
  <summary>Details</summary>
Motivation: 解决室外环境中密集植被、不规则障碍物和复杂地形带来的自主导航挑战，需要同时处理几何和语义信息来区分可穿越植被和刚性障碍物。

Method: 融合分割RGB图像和LiDAR点云，使用高斯泼溅构建可通行性感知的ESDF，在线更新以支持语义推理和360度几何覆盖。

Result: 在植被丰富场景的现场试验中，相比最先进方法成功率提高50%以上，冻结事件减少40%，路径缩短5%，目标时间加快13%，支持长达100米的远程任务。

Conclusion: Splatblox系统在复杂室外环境中表现出卓越的导航性能，能够有效区分可穿越植被和障碍物，支持长距离自主任务。
...
### [244] [SCoTER: Structured Chain-of-Thought Transfer for Enhanced Recommendation](https://arxiv.org/abs/2511.19514)
*Yang Wu,Qian Li,Yuling Xiong,Hongbo Tang,Xun Liu,Jun Zhang,Huan Yu,Jie Jiang,Hailong Shi*

Main category: cs.IR

TL;DR: SCoTER是一个统一框架，通过自动化模式发现和结构保持集成，将结构化LLM推理迁移到推荐系统中，消除在线LLM推理成本并提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在推荐系统中的两个核心挑战：缺乏自动化推理模式发现机制，以及结构坍塌的集成方式导致推理链信息丢失。

Method: 包含两个协同组件：GVM管道用于自动化模式发现，结构保持集成架构将逐步逻辑迁移到高效模型中。
...
### [245] [LLM-EDT: Large Language Model Enhanced Cross-domain Sequential Recommendation with Dual-phase Training](https://arxiv.org/abs/2511.19931)
*Ziwei Liu,Qidong Liu,Wanyu Wang,Yejing Wang,Tong Xu,Wei Huang,Chong Chen,Peng Chuan,Xiangyu Zhao*

Main category: cs.IR

TL;DR: 提出LLM-EDT方法解决跨域序列推荐中的不平衡问题和转移问题，通过可转移项目增强器、双阶段训练策略和领域感知画像模块来提升推荐性能。


<details>
  <summary>Details</summary>
Motivation: 当前跨域序列推荐面临不平衡问题和转移问题，现有LLM增强方法未能有效处理无关噪声和粗略画像问题，需要更有效的解决方案。

Method: 使用可转移项目增强器自适应生成跨域行为，采用双阶段训练策略赋予领域特定线程领域共享背景，设计领域感知画像模块生成全面用户画像。

Result: 在三个公共数据集上的实验验证了LLM-EDT方法的有效性，代码已开源。

Conclusion: LLM-EDT通过创新的增强器、训练策略和画像模块成功解决了跨域序列推荐中的关键挑战，提升了推荐性能。
...
### [246] [The 2nd Workshop on Human-Centered Recommender Systems](https://arxiv.org/abs/2511.19979)
*Kaike Zhang,Jiakai Tang,Du Su,Shuchang Liu,Julian McAuley,Lina Yao,Qi Cao,Yue Feng,Fei Sun*

Main category: cs.IR

TL;DR: 该论文呼吁推荐系统从优化参与度转向真正理解、参与和造福人类，强调整合人类价值观如信任、安全、公平、透明和福祉到推荐过程中。


<details>
  <summary>Details</summary>
Motivation: 随着推荐系统影响力的增长，传统指标如准确性、点击率和参与度已无法捕捉对人类真正重要的因素，需要转向以人为中心的系统设计。

Method: 通过人类中心推荐系统研讨会，汇集推荐系统、人机交互、AI安全和社交计算等领域的研究者，围绕人类理解、人类参与和人类影响三大主题轴，开展主题演讲、小组讨论和论文交流。

Result: 研讨会促进了跨学科合作，涵盖了从基于LLM的交互式推荐器到社会福利优化等主题，为负责任的、与人类对齐的推荐研究奠定了基础。

Conclusion: HCRS旨在塑造未来十年负责任且与人类对齐的推荐研究，推动推荐系统从单纯优化参与度转向真正服务人类福祉的设计范式。
...
### [247] [Popularity Bias Alignment Estimates](https://arxiv.org/abs/2511.19999)
*Anton Lyubinin*
...
<details>
  <summary>Details</summary>
Motivation: 扩展流行度偏差记忆化定理的应用范围，使其适用于更一般的度分布情况，并提供更精确的对齐估计。

Method: 通过数学证明方法，将原有定理从特定度分布扩展到任意度分布，并推导出与top-k奇异超空间对齐的上下界。

Result: 成功证明了扩展后的流行度偏差记忆化定理，并获得了与top-k奇异超空间对齐的精确上下界估计。

Conclusion: 该研究显著扩展了流行度偏差记忆化定理的适用范围，为理解推荐系统中流行度偏差的影响提供了更全面的理论框架。
...
### [248] [Towards A Tri-View Diffusion Framework for Recommendation](https://arxiv.org/abs/2511.20122)
*Ximing Chen,Pui Ieng Lei,Yijun Sheng,Yanyan Liu,Zhiguo Gong*

Main category: cs.IR

TL;DR: 本文提出了一个基于热力学视角的最小化扩散框架，通过最大化亥姆霍兹自由能量来改进推荐系统的性能，并设计了保持各向异性的去噪器和接受-拒绝Gumbel采样过程来增强模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散模型的推荐系统缺乏严格的理论分析，作者从热力学角度发现扩散模型通过最大化能量工作，而经典推荐模型通过减少熵工作，因此需要整合这两种因素。

Method: 提出最小化扩散框架，结合最大化亥姆霍兹自由能量；设计保持二分图用户-物品交叉相关性的各向异性去噪器；采用接受-拒绝Gumbel采样过程来优先处理大量未观察交互。

Result: 理论分析和广泛实验表明，所提出的框架在准确性和效率方面明显优于基线方法。

Conclusion: 该扩散框架通过热力学视角和精心设计的组件，在推荐任务中展现出显著优势，为扩散模型在推荐系统中的应用提供了新的理论支撑和实践方案。
...
### [249] [HKRAG: Holistic Knowledge Retrieval-Augmented Generation Over Visually-Rich Documents](https://arxiv.org/abs/2511.20227)
*Anyang Tong,Xiang Niu,ZhiPing Liu,Chang Tian,Yanyan Wei,Zenglin Shi,Meng Wang*

Main category: cs.IR

TL;DR: HKRAG是一个针对视觉丰富文档的多模态检索增强生成框架，通过混合掩码检索器和不确定性引导生成器，同时捕获显著知识和细节知识，提升问答准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法偏向检索显著知识而忽略细节知识，导致检索不完整和答案不准确。

Method: 1. 混合掩码检索器分别建模显著和细节知识；2. 不确定性引导生成器动态评估答案不确定性并整合两种知识流。

Result: 在开放域视觉问答基准测试中，HKRAG在零样本和监督设置下均优于现有方法。

Conclusion: 整体知识检索对视觉丰富文档理解至关重要，HKRAG有效解决了现有方法的局限性。
...
### [250] [Using Wearable Devices to Improve Chronic PainTreatment among Patients with Opioid Use Disorder](https://arxiv.org/abs/2511.19577)
...
<details>
  <summary>Details</summary>
Motivation: 慢性疼痛和阿片类药物使用障碍是相互关联的慢性疾病，目前缺乏基于证据的综合治疗方法。可穿戴设备有潜力监测患者信息并为治疗开发提供支持。

Method: 使用可穿戴设备收集数据，并应用多种AI方法（包括机器学习模型和大语言模型）分析疼痛峰值的临床相关性。

Result: 机器学习模型在预测疼痛峰值方面达到较高准确率（>0.7），但大语言模型在提供疼痛峰值洞察方面表现有限。

Conclusion: 可穿戴设备结合先进AI模型有助于早期检测疼痛峰值，支持个性化干预，但需要开发能够提供可操作洞察的大语言模型。
...
### [251] [HeaRT: A Hierarchical Circuit Reasoning Tree-Based Agentic Framework for AMS Design Optimization](https://arxiv.org/abs/2511.19669)
*Souradip Poddar,Chia-Tung Ho,Ziming Wei,Weidong Cao,Haoxing Ren,David Z. Pan*

Main category: cs.AI

TL;DR: HeaRT是一个基础推理引擎，用于自动化电路设计优化，具有高精度、高效率的特点，在40个电路基准测试中表现优异。
...
Result: HeaRT在40个电路基准测试中推理准确率>97%，Pass@1性能>98%，即使在电路复杂度增加时也能保持，同时运行成本仅为SOTA基线的<0.5倍实时令牌预算。在尺寸和拓扑设计适应任务中，HeaRT实现了>3倍的收敛速度提升，同时保留先前的设计意图。

Conclusion: HeaRT是一个有效的AMS设计自动化解决方案，能够显著提高设计优化效率，同时保持高精度和适应性。
...
### [252] [FISCAL: Financial Synthetic Claim-document Augmented Learning for Efficient Fact-Checking](https://arxiv.org/abs/2511.19671)
*Rishab Sharma,Iman Saberi,Elham Alipour,Jie JW Wu,Fatemeh Fard*

Main category: cs.AI

TL;DR: FISCAL框架通过生成金融事实核查的合成数据，训练出轻量级验证器MiniCheck-FISCAL，在多个金融数据集上表现优异，超越GPT-3.5 Turbo等模型，接近更大模型的准确性。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在金融应用中存在的事实可靠性不足和计算效率低下的问题，当前系统经常产生幻觉细节且依赖过大的模型。

Method: 提出FISCAL框架生成金融事实核查的合成数据，使用该数据训练轻量级验证器MiniCheck-FISCAL。

Result: MiniCheck-FISCAL在FISCAL-data上表现优于基线，超越GPT-3.5 Turbo和类似大小的开源模型，接近Mixtral-8x22B等大20倍模型的准确性。在外部数据集FinDVer和Fin-Fact上，与GPT-4o和Claude-3.5相当，优于Gemini-1.5 Flash。

Conclusion: 领域特定的合成数据结合高效微调，使紧凑模型能够在实际金融AI中实现最先进的准确性、鲁棒性和可扩展性。
...
### [253] [Scaling Item-to-Standard Alignment with Large Language Models: Accuracy, Limits, and Solutions](https://arxiv.org/abs/2511.19749)
*Farzan Karimi-Malekabadi,Pooya Razavi,Sonya Powers*

Main category: cs.AI

TL;DR: 本研究探讨了大型语言模型（LLMs）在教育评估项目与内容标准对齐过程中的应用潜力，通过三个实验验证了LLMs在识别错误对齐项目、从完整标准中选择正确技能以及预筛选候选技能方面的表现。


<details>
  <summary>Details</summary>
Motivation: 传统的人工对齐审查虽然准确但效率低下，特别是在大规模项目库中。本研究旨在探索LLMs能否在不牺牲准确性的前提下加速这一过程。

Method: 使用超过12,000个K-5年级的项目-技能对，测试了三种LLMs（GPT-3.5 Turbo、GPT-4o-mini和GPT-4o）在三个任务上的表现：识别错误对齐项目、从完整标准中选择正确技能、预筛选候选技能列表。

Result: GPT-4o-mini在识别对齐状态方面达到83-94%的准确率；数学领域表现强劲，阅读领域因标准语义重叠而表现较低；预筛选候选技能后，正确技能出现在前五建议中的概率超过95%。
...
### [254] [Scaling Agentic Reinforcement Learning for Tool-Integrated Reasoning in VLMs](https://arxiv.org/abs/2511.19773)
...
TL;DR: VISTA-Gym是一个可扩展的训练环境，用于增强视觉语言模型的多步视觉推理能力，通过统一多模态任务接口和工具集成，训练出的VISTA-R1模型在多个VQA基准测试中显著优于同类模型。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型虽然具备较强的图像理解能力，但在多步视觉交互推理方面仍存在局限，需要开发能够有效训练工具集成视觉推理能力的环境。

Method: 构建VISTA-Gym训练环境，统一13个数据集的7个多模态推理任务，提供标准化视觉工具接口、可执行交互循环和可验证反馈信号，通过多轮轨迹采样和端到端强化学习训练VISTA-R1模型。

Result: 在11个公开推理密集型VQA基准测试中，VISTA-R1-8B模型比同类规模的最先进基线模型性能提升9.51%-18.72%。

Conclusion: VISTA-Gym是一个有效的训练平台，能够解锁视觉语言模型的工具集成推理能力，显著提升多步视觉推理性能。
...
### [255] [KOM: A Multi-Agent Artificial Intelligence System for Precision Management of Knee Osteoarthritis (KOA)](https://arxiv.org/abs/2511.19798)
...
TL;DR: 开发了KOM多智能体系统，用于自动化膝骨关节炎评估、风险预测和治疗处方，在资源有限环境中改善护理效率。
...
Result: 基准实验显示KOM在影像分析和处方生成方面优于通用大语言模型；三臂模拟研究显示与临床医生合作减少诊断规划时间38.5%，提高治疗质量。

Conclusion: KOM有助于促进自动化KOA管理，整合到临床工作流程中可提高护理效率，其模块化架构为开发其他慢性病AI辅助管理系统提供见解。
...
### [256] [A Unified Evaluation-Instructed Framework for Query-Dependent Prompt Optimization](https://arxiv.org/abs/2511.19829)
*Ke Chen,Yifeng Wang,Hassan Almosapeeh,Haohan Wang*

Main category: cs.AI

TL;DR: 本文提出了一种基于评估指导的提示优化方法，通过建立系统化的提示评估框架和执行无关的评估器，实现了稳定、可解释且模型无关的提示优化。
...
Method: 首先建立面向性能的系统化提示评估框架，开发并微调执行无关的评估器直接从文本预测多维度质量分数，然后让评估器指导一个度量感知的优化器，以可解释的查询依赖方式诊断失败模式并重写提示。

Result: 评估器在预测提示性能方面达到最强准确率，评估指导的优化在8个数据集和3个骨干模型上持续超越静态模板和查询依赖基线方法。

Conclusion: 提出了统一的、基于度量的提示质量视角，证明了评估指导的优化管道能在多样化任务中提供稳定、可解释且模型无关的改进。
...
### [257] [Reinforcement Learning with $ω$-Regular Objectives and Constraints](https://arxiv.org/abs/2511.19849)
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习依赖标量奖励，表达能力有限，容易导致奖励破解问题，且无法清晰表达安全与性能的权衡。ω-正则目标能精确指定丰富的行为属性，但仍需解决单一标量指标掩盖安全-性能权衡的问题。

Method: 开发基于线性规划的模型强化学习算法，将ω-正则目标与约束分离处理，在极限情况下生成满足ω-正则约束阈值的同时最大化目标满足概率的策略，并建立到约束极限平均问题的转换。

Result: 提出的算法能在满足ω-正则约束阈值的前提下，最大化ω-正则目标的满足概率，并建立了与约束极限平均问题的保最优性转换。

Conclusion: 该方法成功解决了强化学习中安全与性能的权衡问题，通过分离处理优化目标和安全约束，提供了更精确的行为规范框架。
...
### [258] [MicroSims: A Framework for AI-Generated, Scalable Educational Simulations with Universal Embedding and Adaptive Learning Support](https://arxiv.org/abs/2511.19864)
*Valerie Lockhart,Dan McCreary,Troy A. Peterson*

Main category: cs.AI

TL;DR: MicroSims是一个用于创建轻量级交互式教育模拟的框架，通过AI辅助生成、iframe架构和可定制代码，解决了传统模拟创建成本高、技术复杂和平台依赖的问题。
...
Result: 基于物理教育研究和STEM领域的元分析，交互式模拟相比传统教学可将概念理解提升30-40%。MicroSims在保持这些优势的同时解决了成本和技术障碍。

Conclusion: MicroSims框架对教育公平具有重要意义，能够支持全球教育工作者按需创建课程对齐的定制模拟，为基于AI的自适应学习系统奠定基础。
...
### [259] [Simulated Self-Assessment in Large Language Models: A Psychometric Approach to AI Self-Efficacy](https://arxiv.org/abs/2511.19872)
*Daniel I Jackson,Emma L Jensen,Syed-Amad Hussain,Emre Sezgin*

Main category: cs.AI

TL;DR: 本研究将通用自我效能感量表(GSES)应用于10个大语言模型，在四种条件下测试其模拟自我评估能力。结果显示模型自我评估稳定但普遍低于人类标准，且自我效能感与任务表现不相关，高自信模型反而可能表现更差。


<details>
  <summary>Details</summary>
Motivation: 当前对大语言模型的评估主要关注任务准确性，而忽视了自我评估这一可靠智能的关键方面。研究旨在探索LLMs的模拟自我评估能力及其与任务表现的关系。

Method: 将10项通用自我效能感量表(GSES)应用于10个LLMs，在四种条件下测试：无任务、计算推理、社会推理和摘要任务。分析自我评估的稳定性、与任务表现的相关性，并进行定性分析。

Result: 模型自我评估高度稳定，但总体得分低于人类标准。所有模型在计算和社会推理任务上表现完美，而摘要任务表现差异很大。自我评估与能力不相关：低自信模型可能表现准确，高自信模型反而表现较差。
...
### [260] [RPM-MCTS: Knowledge-Retrieval as Process Reward Model with Monte Carlo Tree Search for Code Generation](https://arxiv.org/abs/2511.19895)
...
Result: 在四个公开代码生成基准测试中，RPM-MCTS优于当前最先进方法，同时实现约15%的令牌消耗减少。使用RPM-MCTS构建的数据对基础模型进行全微调可显著提升其代码能力。

Conclusion: RPM-MCTS通过知识检索和沙箱反馈机制有效解决了代码生成中的中间步骤评估和错误纠正问题，在提升性能的同时降低了计算成本。
...
### [261] [Semantic-KG: Using Knowledge Graphs to Construct Benchmarks for Measuring Semantic Similarity](https://arxiv.org/abs/2511.19925)
...
<details>
  <summary>Details</summary>
Motivation: 当前评估LLM生成文本语义相似度的方法存在局限性：可能捕获语法或词汇形式而非语义内容，现有基准依赖主观人工判断导致生成成本高，领域特定应用可用性有限，语义等价定义不明确。
...
Conclusion: 该方法为评估LLM输出语义相似度提供了有效的基准生成方案，揭示了语义变化类型和领域特异性对评估结果的影响，对使用LLM作为语义评判者具有重要启示意义。
...
### [262] [A System-Level Taxonomy of Failure Modes in Large Language Model Applications](https://arxiv.org/abs/2511.19933)
*Vaishali Vinay*

Main category: cs.AI

TL;DR: 本文提出了一个系统级分类法，识别了15种真实世界LLM应用中的隐藏故障模式，分析了评估与监控实践中的差距，并提供了构建可靠、可维护、成本敏感的LLM系统的设计原则。
...
Method: 提出了一个包含15种隐藏故障模式的系统级分类法，包括多步推理漂移、潜在不一致性、上下文边界退化、错误工具调用等，并分析了现有评估方法的局限性。

Result: 识别了LLM部署中的生产挑战，包括可观测性限制、成本约束和更新引发的回归问题，揭示了评估与监控实践之间的差距。
...
### [263] [M$^3$Prune: Hierarchical Communication Graph Pruning for Efficient Multi-Modal Multi-Agent Retrieval-Augmented Generation](https://arxiv.org/abs/2511.19969)
*Weizi Shao,Taolin Zhang,Zijie Zhou,Chen Chen,Chengyu Wang,Xiaofeng He*

Main category: cs.AI

TL;DR: 提出M³Prune框架，通过多模态多智能体分层通信图剪枝来减少冗余边，在保持任务性能的同时显著降低token开销。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统虽然性能出色，但存在大量token开销和计算成本，难以大规模部署。

Method: 采用多模态多智能体分层通信图剪枝：先进行模态内图稀疏化识别关键边，再构建动态通信拓扑进行模态间图稀疏化，最后逐步剪枝冗余边。
...
### [264] [Interactive AI NPCs Powered by LLMs: Technical Report for the CPDC Challenge 2025](https://arxiv.org/abs/2511.20200)
*Yitian Huang,Yuxuan Lei,Jianxun Lian,Hao Liao*

Main category: cs.AI

TL;DR: 团队MSRA_SC在CPDC 2025竞赛中提出统一框架，通过上下文工程和GRPO训练分别优化API和GPU赛道，在多个任务中取得优异成绩。
...
Method: 采用上下文工程（动态工具剪枝、角色剪裁、参数归一化、函数合并）和手动优化提示词；在GPU赛道使用GRPO训练替代监督微调。

Result: 在最终评估中，团队在Task 2 API排名第1，Task 1 API排名第2，Task 3 API和GPU赛道均排名第3。

Conclusion: 提出的简单而有效的框架在多个任务中表现出色，证明了该方法在常识角色对话挑战中的有效性。
...
### [265] [CostNav: A Navigation Benchmark for Cost-Aware Evaluation of Embodied Agents](https://arxiv.org/abs/2511.20216)
...
<details>
  <summary>Details</summary>
Motivation: 现有导航基准主要关注任务成功率，但忽视了商业部署所需的经济可行性，这对自主送货机器人的商业化至关重要。

Method: CostNav建模完整的经济生命周期，包括硬件、训练、能源、维护成本和配送收入，使用行业衍生参数，从缩小规模模拟投影到实际配送场景。
...
### [266] [Improving Language Agents through BREW](https://arxiv.org/abs/2511.20297)
*Shashank Kirtania,Param Biyani,Priyanshu Gupta,Yasharth Bajpai,Roshni Iyer,Sumit Gulwani,Gustavo Soares*

Main category: cs.AI

TL;DR: BREW框架通过构建和精炼经验学习知识库来优化LLM智能体，在保持计算效率的同时提升任务精度和减少API调用。
...
Result: 在OSWorld、τ²Bench和SpreadsheetBench基准测试中，BREW实现了10-20%的任务精度提升，10-15%的API/工具调用减少，执行时间更快。

Conclusion: BREW将知识库确立为模块化、可控的智能体优化基础，以透明、可解释和可扩展的方式塑造行为，相比静态上下文记忆具有显著优势。
...
### [267] [Active Inference in Discrete State Spaces from First Principles](https://arxiv.org/abs/2511.20321)
*Patrick Kenny*

Main category: cs.AI

TL;DR: 本文旨在澄清主动推理的概念，将其与自由能原则区分开来，展示了在离散状态空间中实现主动推理的优化问题可以表述为约束散度最小化问题，并可通过标准平均场方法求解，无需依赖期望自由能的概念。


<details>
  <summary>Details</summary>
Motivation: 澄清主动推理与自由能原则之间的关系，提出一种不依赖期望自由能的替代方法来实现主动推理。

Method: 将主动推理的优化问题重新表述为约束散度最小化问题，使用标准平均场方法求解，并提出了感知/行动散度准则。

Result: 在建模感知时，提出的感知/行动散度准则与变分自由能一致；在建模行动时，该准则与期望自由能泛函相差一个熵正则化项。

Conclusion: 主动推理可以在不依赖期望自由能概念的情况下实现，通过约束散度最小化方法提供了一种替代的实现途径。
...
### [268] [VibraVerse: A Large-Scale Geometry-Acoustics Alignment Dataset for Physically-Consistent Multimodal Learning](https://arxiv.org/abs/2511.20422)
*Bo Pang,Chenxi Xu,Jierui Ren,Guoping Wang,Sheng Li*

Main category: cs.AI

TL;DR: VibraVerse是一个连接3D几何与声学信号的大规模数据集，通过物理属性建立从几何到声音的因果链，并提出了CLASP对比学习框架来实现跨模态对齐。


<details>
  <summary>Details</summary>
Motivation: 现有多模态学习框架缺乏物理一致性，忽略了物体几何、材料、振动模式与产生声音之间的内在因果关系。

Method: 构建VibraVerse数据集，包含3D模型的物理属性和体积几何，计算模态特征进行冲击声音合成；提出CLASP对比学习框架实现跨模态对齐。

Result: 在几何到声音预测、声音引导形状重建和跨模态表示学习等基准任务上，基于VibraVerse训练的模型表现出更高的准确性、可解释性和泛化能力。
