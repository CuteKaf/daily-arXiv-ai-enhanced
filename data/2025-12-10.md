...
Motivation: 苏丹持续战争需要快速监测和分析冲突情况。深度学习技术和易获取的卫星遥感影像使得近实时监测成为可能，特别是监测武装冲突中的火灾破坏。
...
Conclusion: 4波段卫星影像结合深度学习模型可以近乎实时地有效监测武装冲突中的火灾破坏，为冲突监测提供了一种高效实用的技术方案。
...
TL;DR: LivingSwap是首个视频参考引导的人脸交换模型，通过关键帧条件化和视频参考引导实现高保真度和时间一致性的长视频人脸交换
...
Method: 提出LivingSwap模型，采用关键帧作为条件信号注入目标身份，结合关键帧条件化和视频参考引导进行时间拼接，确保稳定身份保持和高保真重建。为了解决参考引导训练数据稀缺问题，构建了Face2Face配对人脸交换数据集，并通过数据对反转确保可靠的真实监督

Result: 大量实验表明，该方法取得了最先进的结果，能够无缝地将目标身份与源视频的表情、光照和运动集成，同时显著减少了制作工作流程中的手动工作量
...
<details>
  <summary>Details</summary>
Motivation: 地图推理是解释地理关系的关键认知能力，在灾害响应和城市规划等任务中至关重要，但目前缺乏专门的评估基准。现有的地图视觉问答研究往往将地图视为图表的特例，但地图理解需要处理分层符号系统和跨地图的空间关系，这些在图表式评估中未能充分体现。

Method: FRIEDA基准从各种领域和地理区域的文档报告中收集真实地图图像，基于GIS文献分类，针对拓扑关系（边界、相等、相交、包含）、度量关系（距离）和方向关系（方位）三类空间关系设计问题。所有问题都需要多步推理，许多需要跨地图定位和推理。评估在两种设置下进行：直接设置（提供相关地图）和上下文设置（模型需先识别相关地图）。

Result: 评估了11个最先进的LVLM模型，即使在直接设置下，最强的Gemini-2.5-Pro和GPT-5-Think模型准确率分别仅为38.20%和37.20%，远低于人类表现的84.87%。这些结果揭示了在多步地图推理方面存在持续差距。

Conclusion: FRIEDA作为一个严谨的基准，能够推动LVLM空间智能的进步，当前模型在地图推理能力上仍有显著不足，需要专门针对地图理解的研究和改进。
...
TL;DR: 该论文提出了一种基于频域掩码的通用深度伪造检测方法，通过随机掩码和几何变换增强模型泛化能力，在保持高性能的同时支持模型剪枝，实现绿色AI目标。
...
Method: 提出频域掩码作为深度伪造检测器的训练策略，引入随机掩码和几何变换，特别关注频域掩码因其优越的泛化特性。该方法不依赖空间特征或大规模预训练模型。

Result: 频域掩码不仅提高了跨不同生成器的检测准确率，而且在显著模型剪枝下仍能保持性能。在GAN和扩散模型生成的图像数据集上实现了最先进的泛化性能，并在结构化剪枝下表现出一致的鲁棒性。

Conclusion: 频域掩码为可持续和可泛化的深度伪造检测提供了实用的解决方案，展示了频域掩码在通用深度伪造检测中的潜力。
...
TL;DR: M2A提出了一种简单的持续测试时自适应方法，通过随机掩码生成不同视图，结合一致性损失和熵最小化损失进行适应，在强噪声下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有的持续测试时自适应方法通常依赖复杂的掩码设计、校准的不确定性或稳定的注意力分数，增加了实现复杂度。本文探讨是否可以通过简单的随机掩码方案在强噪声条件下实现有效适应。

Method: 提出M2A方法：生成短序列的随机掩码视图（空间或频率域），使用两种损失函数：1）掩码一致性损失，对齐不同视图的预测；2）熵最小化损失，鼓励置信输出。研究了两种掩码家族：空间掩码（补丁vs像素）和频率掩码（全频vs低频vs高频）。

Result: 在CIFAR10C/CIFAR100C/ImageNetC（严重程度5）上，M2A（空间）达到8.3%/19.8%/39.2%的平均错误率，优于或匹配强基线方法，而M2A（频率）表现较差。消融实验显示简单随机掩码是有效且鲁棒的。

Conclusion: 简单的随机掩码方案结合一致性损失和熵最小化损失，足以驱动有效的测试时自适应，无需依赖不确定性或注意力信号。空间掩码优于频率掩码。
...
TL;DR: CVP框架受人类中央-周边视觉启发，通过目标亲和性令牌（中央视觉）和全局网格（周边视觉）的互补组件，显著提升了多模态模型在3D空间推理任务中的性能。
...
Method: 提出中央-周边视觉启发框架(CVP)，在大型多模态模型架构中引入两个互补组件：1)目标亲和性令牌（类似中央视觉），引导模型关注查询相关对象；2)全局网格（类似周边视觉），捕捉全局场景上下文和空间布局。

Result: CVP在多个3D场景理解基准测试中实现了最先进的性能表现。

Conclusion: 通过模拟人类中央-周边视觉机制，CVP框架能够实现结构化、上下文感知的复杂3D环境理解，显著提升了多模态模型的空间推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有Transformer方法虽然能捕捉全局上下文，但二次计算复杂度阻碍实时部署，需要解决真实世界非均匀雾霾条件下的去雾挑战

Method: 提出基于多状态感知范式的Fourier-RWKV框架：1)空间感知通过可变形四向令牌移位动态调整感受野；2)频域感知通过傅里叶混合块将RWKV的WKV注意力扩展到傅里叶域；3)语义关系感知通过语义桥接模块实现编码器-解码器特征对齐

Result: 在多个基准测试上取得最先进性能，显著降低计算开销，在恢复质量和实际效率之间建立良好平衡

Conclusion: Fourier-RWKV通过线性复杂度实现全面的雾霾退化建模，为真实世界非均匀雾霾条件下的图像去雾提供了高效解决方案
...
TL;DR: GeoLoom是一个用于几何图表生成的文本到图表框架，通过形式化语言和坐标求解实现高精度几何图生成。


<details>
  <summary>Details</summary>
Motivation: 高质量几何图表生成需要严格的空间精度，同时提供明确的约束条件来指导生成。几何问题求解的最新进展表明，使用形式化语言和符号求解器可以提高正确性和可解释性。

Method: GeoLoom包含两个核心组件：1）自动形式化模块，将自然语言翻译为专门设计的生成导向形式化语言GeoLinua；2）坐标求解器，使用高效的蒙特卡洛优化将形式约束映射到精确坐标。还引入了GeoNF数据集，对齐自然语言几何描述与形式化描述。

Result: 实证结果表明，GeoLoom在结构保真度方面显著优于最先进的基线方法，为可解释和可扩展的图表生成提供了原则性基础。

Conclusion: GeoLoom通过结合形式化语言和优化求解，为几何图表生成提供了一个结构准确、可解释的框架，在几何领域实现了高质量的文本到图表转换。
...
<details>
  <summary>Details</summary>
Motivation: 当前的对象识别仅提供类别标签，而深入的对象理解需要更全面的感知，包括组件、外观特征、类别关系等。现有数据通常是任务导向的，缺乏系统组织，不足以实现预期的对象类别理解。

Method: 提出了VisKnow框架，通过结合专家设计和大规模模型应用，从百科全书文档中提取文本知识三元组，整合对齐的文本和图像源知识，并提供对象和部件级别的区域标注。

Result: 构建了AnimalKB知识库，涵盖406个动物类别，包含22K文本知识三元组、420K图像及相应的区域标注。实验表明AnimalKB能增强零样本识别和细粒度视觉问答等任务，并为知识图谱补全和部件分割提供基准。

Conclusion: 自动构建视觉知识库具有推进视觉理解及其实际应用的潜力，VisKnow框架为系统组织多模态对象知识提供了有效方法。
...
TL;DR: 该论文探索了在3D目标检测领域中应用提示调优方法的有效性，研究了基于Waymo数据集训练的模型作为基础模型适应其他场景的能力，并提出了场景导向的提示池方法。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在NLP领域通过微调和提示调优展现强大泛化能力，该研究旨在探索这些方法在3D目标检测领域的适用性，研究基础模型在不同场景下的适应能力。

Method: 研究首先分析了提示标记和提示生成器的影响，然后提出了场景导向的提示池（SOP²）方法，探索提示池在3D目标检测中的应用潜力。

Result: 论文展示了提示池在3D目标检测中的有效性，验证了基于Waymo数据集训练的基础模型可以通过提示调优适应其他场景。

Conclusion: 研究证明了提示调优方法在3D目标检测领域的潜力，为未来研究者深入探索3D领域中提示技术的应用提供了启发。
...
TL;DR: MM-CoT是一个诊断性基准测试，专门用于评估多模态模型在思维链推理中的视觉基础和逻辑连贯性，通过要求模型选择同时满足视觉一致性和逻辑连贯性的事件链来测试其真实推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试强调生成能力但忽视验证能力，无法评估多模态模型的思维链推理是否真正基于视觉证据且逻辑连贯。需要填补这一空白，诊断模型推理的视觉基础和逻辑一致性。
...
TL;DR: 提出了一种基于法线引导的稀疏深度采样策略，通过PCA表面法线估计计算像素级深度可靠性，改进深度补全模型的训练数据生成，使其更接近真实传感器行为。
...
Method: 提出法线引导的稀疏深度采样策略：1）在RGB-D点云上使用PCA进行表面法线估计；2）基于法线计算每个像素的深度可靠性度量；3）根据可靠性分布进行稀疏深度采样；4）将该采样方法与Marigold-DC扩散式深度补全模型集成。

Result: 在NYU Depth v2数据集上的实验表明，几何感知的稀疏深度采样提高了深度补全的准确性，减少了边缘和不连续区域的伪影，并创造了更接近真实传感器行为的训练条件。

Conclusion: 通过引入几何感知的稀疏深度采样策略，能够生成更真实的训练数据，显著改善深度补全模型的性能，使其更好地适应实际工业机器人系统中的三维感知需求。
...
<details>
  <summary>Details</summary>
Motivation: 当前相机BEV感知面临性能与部署可行性之间的根本矛盾：先进性能依赖计算密集的视图变换和平台特定的定制内核，阻碍了车载部署。

Method: 采用"算法快速"和"设计可部署"两大原则：1) 通过确定性预排序策略将整体投影分解为标准索引-收集-重塑管道，使用基础算子实现，无需定制CUDA内核；2) 集成端到端深度感知融合机制，结合时间聚合和数据增强提升BEV表示几何保真度。

Result: 在nuScenes基准上达到新的SOTA性能0.359 NDS，同时在Tesla T4等车载硬件上实现超过134 FPS的实时性能，无需定制插件。

Conclusion: FastBEV++通过统一高性能与部署效率，为生产级自动驾驶系统提供了成熟且可扩展的设计方案，解决了BEV感知的性能-部署矛盾。
...
TL;DR: HTC-VLM提出了一种混合视觉语言模型框架，通过双通道分离语义和外观表示，实现580:1的高效压缩，在保持87.2%性能的同时解决传统方法的效率-保真度困境。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型面临计算效率与表示质量的困境：连续压缩会稀释高级语义信息（如物体身份），而离散量化会丢失细粒度细节（如纹理）。需要一种既能高效处理视觉补丁token，又能保持语义和细节完整性的方法。

Method: 提出HTC-VLM混合框架，采用双通道设计：1）连续通道通过ViT补丁保留细粒度细节；2）离散通道使用MGVQ量化生成符号锚点，投影为4个token。两者通过解耦注意力掩码和瓶颈融合成580个token的混合序列，最终压缩为单个voco token。

Result: 在7个基准测试（GQA、VQAv2、MMBench、MME、POPE、SEED-Bench、ScienceQA-Image）上平均性能保持率达到87.2%，优于领先连续基线（81.0%），实现580:1压缩比。注意力分析显示压缩token优先关注离散锚点，验证了其语义引导作用。

Conclusion: HTC-VLM通过极简混合设计解决了效率与保真度的困境，证明了分离语义和外观表示的有效性，为可扩展视觉语言模型的发展提供了新方向。
...
TL;DR: 提出了一种名为Residual-SwinCA-Net的新型深度混合分割框架，结合了残差CNN和Swin Transformer，用于乳腺癌超声图像分割，在BUSI数据集上取得了优异性能。
...
Method: 1. 结合残差CNN模块提取局部相关特征；2. 定制Swin Transformer块学习全局依赖；3. 使用拉普拉斯-高斯区域算子增强组织连续性和抑制噪声；4. 引入边界导向算子保持轮廓完整性；5. 采用阶段式收缩策略捕获尺度不变性；6. 提出多尺度通道注意力与压缩模块；7. 使用像素注意力模块自适应加权恶性病变像素。

Result: 在公开的BUSI数据集上，Residual-SwinCA-Net框架实现了99.29%的平均准确率、98.74%的IoU和0.9041的Dice系数，优于现有的CNN/ViT技术。

Conclusion: 提出的Residual-SwinCA-Net框架显著提升了乳腺癌超声图像分割性能，增强了BUSI病变诊断能力，有助于及时临床决策制定。
...
TL;DR: 该论文提出了一种基于稀疏查询的未来时间知识蒸馏方法（FTKD），用于将离线教师模型的未来帧知识转移到在线学生模型中，以提升自动驾驶中基于相机的时序3D目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的知识蒸馏方法主要关注空间特征蒸馏或时间关系蒸馏，但忽略了未来帧知识，导致在线模型难以有效学习未来信息。为了将离线模型（可使用未来帧）的丰富信息转移到在线模型（只能使用当前和过去帧），需要一种新的知识蒸馏方法。

Method: 提出FTKD方法，包含两个核心策略：1）未来感知特征重建策略，鼓励学生模型在不严格帧对齐的情况下捕获未来特征；2）未来引导的logit蒸馏，利用教师模型稳定的前景和背景上下文信息。该方法基于稀疏查询框架实现。

Result: 在nuScenes数据集上，将FTKD应用于两个高性能的3D目标检测基线模型，获得了最高1.3 mAP和1.3 NDS的提升，同时实现了最准确的车辆速度估计，且不增加推理成本。

Conclusion: FTKD通过有效转移未来帧知识，显著提升了在线3D目标检测模型的性能，特别是在准确性和速度估计方面，为自动驾驶中的实时检测提供了有效的知识蒸馏解决方案。
...
TL;DR: 本文提出了一种查询感知的枢纽原型学习方法，通过构建查询-支撑点二分图来生成与查询相关的原型，解决传统原型学习方法因忽略查询数据而导致的分布偏移问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于度量的原型学习方法仅从支撑集生成原型，不考虑与查询数据的相关性，导致原型偏差——原型过度拟合支撑集特性，无法泛化到查询分布，特别是在存在分布偏移时，这会降低分割性能。

Method: 提出查询感知枢纽原型学习方法，包含两个模块：1) 枢纽原型生成模块：构建查询-支撑点二分图，识别频繁连接的支撑枢纽，生成与查询相关的原型；2) 原型分布优化模块：使用纯度加权对比损失，将坏枢纽和边界模糊原型拉近到对应类别中心，优化原型表示。

Result: 在S3DIS和ScanNet数据集上的大量实验表明，QHP方法相比最先进方法取得了显著的性能提升，有效缩小了原型与查询集之间的语义差距。

Conclusion: 提出的查询感知枢纽原型学习方法通过显式建模支撑集与查询集之间的语义相关性，解决了原型偏差问题，提高了小样本3D点云语义分割的性能。
...
TL;DR: RLCNet：一种用于LiDAR、RADAR和摄像头多模态传感器同时在线校准的端到端可训练深度学习框架
...
Result: 在真实世界数据集上验证，RLCNet设计用于实际部署，在不同条件下表现出鲁棒性能。消融研究突显了架构选择的重要性，与现有方法的比较表明该方法具有更高的准确性和鲁棒性

Conclusion: RLCNet为多模态传感器在线校准提供了一种有效的深度学习解决方案，通过实时操作框架和鲁棒性设计，能够应对动态环境中的校准挑战
...
<details>
  <summary>Details</summary>
Motivation: 从第三人称视角视频转换为第一人称视角视频能够为沉浸式理解开辟新可能性，但由于相机姿态变化极大且视角重叠极少，这一任务极具挑战性。需要保持可见内容的同时合成未观察区域并保持几何一致性。

Method: EgoX框架通过轻量级LoRA适配利用大规模视频扩散模型的预训练时空知识，采用统一的调节策略结合第三人称和第一人称先验（通过宽度和通道级拼接），并引入几何引导的自注意力机制选择性地关注空间相关区域。
...
TL;DR: PAVAS是一种物理感知的视频到音频生成方法，通过物理驱动音频适配器将物理推理融入潜在扩散模型，能够基于物体质量和运动轨迹等物理参数合成更真实的声音。


<details>
  <summary>Details</summary>
Motivation: 现有视频到音频生成模型主要基于外观驱动，捕捉视觉-声学相关性，但忽略了塑造真实世界声音的物理因素。需要将物理推理融入音频生成过程，以产生更真实、物理合理的声音。

Method: 1. 使用物理参数估计器（PPE）提取物体级物理参数：通过视觉语言模型推断物体质量，通过基于分割的动态3D重建模块恢复运动轨迹计算速度；2. 通过物理驱动音频适配器（Phy-Adapter）将这些物理线索融入潜在扩散模型；3. 构建VGG-Impact基准测试集和音频-物理相关性系数（APCC）评估指标。

Result: PAVAS能够生成物理合理且感知连贯的音频，在定量和定性评估中均优于现有的视频到音频生成模型。通过APCC指标验证了物理属性与听觉属性之间的一致性。

Conclusion: 将物理推理融入视频到音频生成过程能够显著提升生成声音的真实性和物理合理性。PAVAS方法为物理感知的视听合成开辟了新方向，通过物体级物理参数估计和物理驱动适配器实现了更真实的音频生成。
...
TL;DR: OpenSubject是一个包含250万样本、435万图像的视频衍生大规模数据集，用于主体驱动的图像生成和编辑，通过四阶段流水线构建，显著提升了复杂场景下的生成和编辑性能。


<details>
  <summary>Details</summary>
Motivation: 当前主体驱动图像生成模型存在两个主要问题：1）容易偏离参考主体的身份特征；2）在包含多个主体的复杂场景中表现不佳。为了解决这些挑战，需要高质量、大规模的训练数据。

Method: 采用四阶段流水线构建数据集：1）视频筛选（分辨率与美学过滤）；2）跨帧主体挖掘与配对（基于VLM的类别共识、局部定位和多样性感知配对）；3）身份保持的参考图像合成（分割图引导的外绘和框引导的内绘，结合几何感知增强和不规则边界侵蚀）；4）验证与标注（VLM验证样本，失败样本重新合成，构建短长两种描述）。

Result: 构建了包含250万样本、435万图像的OpenSubject数据集，并建立了覆盖主体驱动生成和编辑的基准测试。实验表明，使用OpenSubject训练显著提升了生成和编辑性能，特别是在复杂场景中，在身份保真度、提示遵循度、编辑一致性和背景一致性等方面都有改善。

Conclusion: OpenSubject数据集通过精心设计的四阶段流水线构建，利用跨帧身份先验，有效解决了主体驱动图像生成中的身份偏离和复杂场景处理问题，为相关任务提供了高质量的训练数据和评估基准。
...
<details>
  <summary>Details</summary>
Motivation: 传统的程序化噪声函数（如Perlin噪声）虽然快速且无限扩展，但在真实感和大尺度连贯性方面存在根本限制。需要一种AI时代的新方法来结合扩散模型的高保真度与传统程序化噪声的实用特性

Method: 1. 提出InfiniteDiffusion算法实现无限生成；2. 使用分层堆叠的扩散模型耦合行星级上下文与局部细节；3. 采用紧凑的拉普拉斯编码稳定地球尺度动态范围的输出；4. 开源无限张量框架支持无界张量的恒定内存操作；5. 使用少步一致性蒸馏实现高效生成

Result: Terrain Diffusion成功将扩散模型确立为程序化世界生成的实用基础，能够合成整个行星的地形，具有连贯性、可控性且无限制

Conclusion: Terrain Diffusion是Perlin噪声的AI时代继任者，通过创新的无限生成算法和系统架构，实现了高保真度与程序化噪声实用特性的完美结合，为程序化世界生成提供了新的技术基础
...
TL;DR: GeoDM提出了一种几何感知的数据集蒸馏框架，通过在欧几里得、双曲和球面流形的笛卡尔积空间中操作，捕获数据的平坦、层次和循环结构，使用可学习的曲率和权重参数适应数据几何特性。


<details>
  <summary>Details</summary>
Motivation: 现有基于分布匹配的数据集蒸馏方法局限于欧几里得空间，只能捕捉线性结构而忽略了真实数据的固有几何特性（如曲率）。高维数据通常位于低维流形上，因此需要使蒸馏后的数据流形与原始数据流形对齐。
...
Conclusion: GeoDM通过在多几何流形空间中操作，成功捕获了数据的复杂几何结构，为数据集蒸馏提供了更有效的几何感知框架，显著提升了蒸馏数据的质量和模型性能。
...
TL;DR: 该论文介绍了3DTeethLand挑战赛，这是首个公开的3D牙齿标志点检测数据集，旨在通过深度学习技术解决牙齿几何结构复杂和个体差异大的挑战，推动临床牙科诊断和治疗的发展。


<details>
  <summary>Details</summary>
Motivation: 牙齿标志点检测在现代临床正畸学中至关重要，能够实现高级诊断、个性化治疗策略和有效的治疗进展监测。然而，由于牙齿几何结构复杂且个体间存在显著差异，开发先进技术特别是深度学习方法来精确可靠地检测3D牙齿标志点变得尤为必要。
...
Conclusion: 3DTeethLand挑战赛通过提供首个公开数据集，推动了3D牙齿标志点检测技术的发展，这对于临床牙科诊断、个性化治疗和治疗监测具有重要价值，有望促进该领域的研究进步和实际应用。
...
TL;DR: GeoDiffMM：一种基于扩散的拉格朗日视频运动放大框架，利用光流作为几何线索实现结构一致的运动放大，通过无噪声光流增强策略和扩散运动放大器，在保持场景语义和结构的同时抑制噪声。


<details>
  <summary>Details</summary>
Motivation: 现有欧拉方法通过解耦表示学习（如纹理、形状、频率方案）处理放大引起的噪声，但在运动位移非常小时难以将光子噪声与真实微运动分离，需要更有效的运动放大方法。

Method: 1. 无噪声光流增强策略：合成多样化的非刚性运动场作为监督，帮助模型学习更准确的几何感知光流；2. 扩散运动放大器：以光流作为几何先验和可学习放大因子控制幅度，选择性放大与场景语义和结构一致的运动分量；3. 基于光流的视频合成：将放大后的运动高保真地映射回图像域。

Result: 在真实和合成数据集上的大量实验表明，GeoDiffMM优于现有最先进方法，显著改善了运动放大效果。

Conclusion: 提出的GeoDiffMM框架通过结合光流几何线索和扩散模型，实现了结构一致的运动放大，有效分离光子噪声与真实微运动，在运动放大任务上取得了优越性能。
...
TL;DR: 该论文提出了一种用于彩色图像分类的低秩支持四元数矩阵机（LSQMM），将RGB通道作为纯四元数处理，通过四元数代数保留通道间的内在耦合关系，并引入四元数核范数正则化来促进低秩结构。


<details>
  <summary>Details</summary>
Motivation: 传统彩色图像分类方法通常将特征表示为实数域中的向量、矩阵或三阶张量，未能有效保留RGB通道间的内在耦合关系。受四元数数据建模在图像恢复和去噪任务中成功的启发，作者希望开发一种能更好处理彩色图像分类的方法。

Method: 提出低秩支持四元数矩阵机（LSQMM）：1）将RGB通道作为纯四元数处理，通过四元数代数保留通道间的内在耦合关系；2）引入四元数核范数正则化项（传统矩阵核范数在四元数域的自然扩展），促进由强相关颜色通道产生的低秩结构；3）设计基于交替方向乘子法（ADMM）的迭代算法来有效求解提出的四元数优化模型。

Result: 在多个彩色图像分类数据集上的实验结果表明，与使用支持向量机、支持矩阵机和支持张量机等几种最先进方法相比，提出的分类方法在分类精度、鲁棒性和计算效率方面都表现出优势。

Conclusion: LSQMM通过四元数表示有效保留了彩色图像通道间的耦合关系，结合低秩正则化提升了分类性能，为彩色图像分类提供了一种新颖且有效的方法。
...
TL;DR: 本文系统分析了Glaze和Nightshade等图像保护机制，通过可解释AI方法揭示了这些对抗性扰动在特征空间、空间域和频域的结构化特性，而非简单的语义漂移。


<details>
  <summary>Details</summary>
Motivation: 虽然Glaze和Nightshade等图像保护机制在经验上有效，但其内部结构、可检测性和表征行为仍缺乏深入理解。需要系统分析这些对抗性扰动的工作原理。

Method: 采用统一框架整合白盒特征空间检查与黑盒信号级探测，包括潜在空间聚类、特征通道激活分析、基于遮挡的空间敏感性映射和频域表征分析。

Result: 保护机制作为结构化、低熵扰动紧密耦合于底层图像内容；保护图像保留了内容驱动的特征组织，具有保护特异性子结构而非全局表征漂移；可检测性受扰动熵、空间部署和频率对齐相互作用影响；Glaze和Nightshade沿主导图像对齐频率轴重新分配能量而非引入扩散噪声。

Conclusion: 当代图像保护通过结构化特征级变形而非语义错位运作，解释了保护信号视觉上微妙但一致可检测的原因。研究推进了对抗性图像保护的可解释性，为未来生成AI系统的防御和检测策略设计提供信息。
...
### [28] [SDT-6D: Fully Sparse Depth-Transformer for Staged End-to-End 6D Pose Estimation in Industrial Multi-View Bin Picking](https://arxiv.org/abs/2512.08430)
*Nico Leuze,Maximilian Hoh,Samed Doğan,Nicolas R. -Peña,Alfred Schoettl*

Main category: cs.CV

TL;DR: 提出了一种基于深度信息的6D姿态估计方法，通过多视角深度图融合和稀疏TSDF表示，结合场景自适应注意力机制和密度感知稀疏Transformer，在密集堆叠的工业拣选场景中实现高精度姿态估计。


<details>
  <summary>Details</summary>
Motivation: 工业拣选环境中存在严重遮挡、反射和纹理缺失等问题，传统6D姿态估计方法难以准确恢复物体姿态。需要一种能够处理密集堆叠、高遮挡场景的深度感知方法。

Method: 1. 多视角深度图融合为精细3D点云或稀疏TSDF表示；2. 分层热图机制提供场景自适应注意力先验；3. 密度感知稀疏Transformer块动态处理遮挡和3D数据非均匀分布；4. 完全稀疏操作实现高分辨率体素表示；5. 基于每体素投票策略预测6D姿态。

Result: 在IPD和MV-YCB多视角数据集上验证，在重度堆叠的工业和家庭拣选场景中表现出竞争性性能，能够同时预测任意数量目标物体的姿态。

Conclusion: 该方法通过完全稀疏的高分辨率体素表示和场景自适应注意力机制，有效解决了工业拣选环境中的遮挡和纹理缺失问题，为密集堆叠场景的6D姿态估计提供了有效解决方案。
...
TL;DR: PointDico是一种新颖的3D表示学习方法，通过结合扩散模型和对比模型的优势，解决了现有方法在处理无序、密度不均的3D点云数据时的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督表示学习方法在3D数据上面临挑战，因为3D数据具有无序性和密度不均的特点。对比模型容易过拟合，而3D掩码自编码器难以处理无序点云。这促使研究者探索结合扩散模型和对比模型优势的新方法。
...
Conclusion: PointDico通过结合扩散模型和对比模型的优势，成功解决了3D表示学习中的关键挑战，为无序点云数据的自监督学习提供了有效的新方法。
...
TL;DR: 提出Bi^2MAC双模态双自适应掩码感知卷积，通过软硬掩码将特征分流处理，在降低计算成本的同时提升遥感图像融合性能


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法在处理遥感图像时难以适应区域异质性，现有自适应卷积方法计算成本高且对异质区域捕获能力有限

Method: 设计轻量级模块生成软掩码和硬掩码，软掩码初步调制输入特征，硬掩码将特征分流到两个处理分支：冗余特征进入紧凑分支进行低成本全局处理，异质特征进入专注分支进行细粒度建模

Result: 在多个基准数据集上达到SOTA性能，同时显著降低训练时间和参数数量，在自适应卷积模型中计算成本最低

Conclusion: Bi^2MAC能有效利用不同类型区域信息并智能分配计算资源，在保持高性能的同时大幅降低计算复杂度
...
### [31] [Disturbance-Free Surgical Video Generation from Multi-Camera Shadowless Lamps for Open Surgery](https://arxiv.org/abs/2512.08577)
*Yuna Kato,Shohei Mori,Hideo Saito,Yoshifumi Takatsume,Hiroki Kajita,Mariko Isogawa*

Main category: cs.CV

TL;DR: 提出了一种自动对齐多摄像头手术视频的方法，通过检测照明系统移动、重新对齐帧并选择遮挡最少的摄像头，生成固定视角的手术视频，提高观看舒适度和手术区域确认的便利性。


<details>
  <summary>Details</summary>
Motivation: 手术视频对教育和研究很重要，但拍摄无遮挡视频很困难，因为外科医生经常遮挡相机视野。现有方法使用多个摄像头但需要手动对齐，且每次调整照明时都需要重新对齐，工作量大。

Method: 提出全自动对齐方法：1) 识别照明系统移动的帧；2) 重新对齐这些帧；3) 选择遮挡最少的摄像头；4) 生成固定视角的连续视频。还实现了多种合成选项供用户选择。

Result: 外科医生用户研究表明，该方法生成的视频在确认手术区域的便利性和观看舒适度方面优于传统方法。视频质量也有改进。用户研究还评估了不同合成选项的偏好。
...
Conclusion: HybridSplat在保持反射渲染质量的同时，显著提升渲染速度和减少内存使用，成为复杂反射场景的新SOTA方法
...
### [33] [Accelerated Rotation-Invariant Convolution for UAV Image Segmentation](https://arxiv.org/abs/2512.08888)
*Manduhu Manduhu,Alexander Dow,Gerard Dooly,James Riordan*

Main category: cs.CV

TL;DR: 提出了一种GPU优化的旋转不变卷积框架，通过利用对称旋转滤波器间的结构化数据共享，显著减少了内存流量和计算冗余，在保持精度的同时大幅提升了训练速度和能效。
...
Result: 相比CUDNN，训练速度提升20-55%，能耗降低15-45%，同时保持与最先进旋转不变方法相当的精度。在八方向设置下，256×256输入上实现45%加速和41%节能，1024×1024输入上实现32%加速和23%节能。集成到U-Net分割模型中，相比非旋转感知基线精度提升达6%。

Conclusion: 该方法为现有旋转不变CNN框架提供了有效且高效的替代方案，在保持分割精度的同时显著提升了计算性能和能效。
...
### [34] [LiDAS: Lighting-driven Dynamic Active Sensing for Nighttime Perception](https://arxiv.org/abs/2512.08912)
*Simon de Moreau,Andrei Bursuc,Hafid El-Idrissi,Fabien Moutarde*

Main category: cs.CV

TL;DR: LiDAS是一个闭环主动照明系统，通过动态预测最优照明场来最大化下游感知性能，实现夜间场景的零样本泛化。
...
Method: 结合现成的视觉感知模型与高清头灯，动态预测最优照明场，减少空区域的照明以重新分配到物体区域，实现自适应照明控制。

Result: 在真实世界闭环驾驶场景中，相比标准近光灯，LiDAS实现了+18.7% mAP50和+5.0% mIoU的提升，同时减少40%能耗。

Conclusion: LiDAS将现成的头灯转变为主动视觉执行器，为鲁棒的夜间感知提供了经济有效的解决方案，无需重新训练即可增强模型鲁棒性。
...
TL;DR: TrackingWorld：一种新颖的单目视频密集3D跟踪方法，通过跟踪上采样器将稀疏2D轨迹提升为密集2D轨迹，并优化估计相机姿态和3D坐标，实现世界坐标系下的像素级3D跟踪


<details>
  <summary>Details</summary>
Motivation: 现有单目3D跟踪方法存在两个主要局限：1) 难以将相机运动与前景动态运动分离；2) 无法密集跟踪视频中新出现的动态物体。为解决这些问题，需要开发一种能够在世界坐标系中跟踪几乎所有像素的密集3D跟踪方法

Method: 提出TrackingWorld三阶段管道：1) 跟踪上采样器将任意稀疏2D轨迹高效提升为密集2D轨迹；2) 将上采样器应用于所有帧，通过消除重叠区域轨迹减少2D轨迹冗余，实现对新出现物体的跟踪；3) 基于优化的框架将密集2D轨迹反投影到世界坐标系中的3D轨迹，同时估计相机姿态和2D轨迹的3D坐标

Result: 在合成和真实世界数据集上的广泛评估表明，TrackingWorld系统在世界坐标系中实现了准确且密集的3D跟踪，能够有效分离相机运动与前景动态运动，并跟踪新出现的动态物体

Conclusion: TrackingWorld通过创新的跟踪上采样和优化框架，成功解决了现有单目3D跟踪方法的局限性，实现了世界坐标系下几乎所有像素的密集3D跟踪，为单目视频分析提供了更强大的工具
...
Result: SCU-CGAN在KID分数上比CycleGAN提升41.5%，生成的火灾图像质量更优。使用增强数据集后，YOLOv5 nano模型的mAP@0.5:0.95指标提升56.5%，显著提高了火灾检测准确率
...
TL;DR: 该论文发现多模态大语言模型中视觉和文本token的范数差异导致不对称更新动态，提出在视觉投影器后添加LayerNorm层进行范数对齐，显著提升多模态和纯文本任务性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型普遍采用Pre-Norm架构，导致视觉token的高范数与文本token的低范数之间存在严重不平衡，这种范数差异不是静态问题，而是引发"不对称更新动态"，使得高范数视觉token具有"表征惯性"，语义转换速度远慢于文本token，从而损害跨模态特征融合效果。

Method: 首先通过理论分析证明范数差异导致的不对称更新动态问题，然后在视觉投影器后插入一个精心初始化的LayerNorm层来强制范数对齐，该方法简单但有效，在LLaVA-1.5架构上进行实验验证。

Result: 实验验证表明范数差异和不对称更新速率是主流MLLMs中的普遍现象。在LLaVA-1.5架构上应用LayerNorm对齐方法后，不仅在广泛的多模态基准测试中取得显著性能提升，而且在纯文本评估（如MMLU）上也表现出改进，表明解决架构不平衡能产生更全面的模型能力。

Conclusion: 多模态大语言模型中的范数差异问题导致跨模态融合效率低下，通过简单的LayerNorm层进行范数对齐可以有效解决这一问题，提升模型在多模态和纯文本任务上的整体性能，为MLLM架构设计提供了重要洞见。
...
<details>
  <summary>Details</summary>
Motivation: 在挑战性光照条件下，捕获的图像常出现各种退化，影响视觉应用的性能。现有方法要么显著放大固有噪声，要么仅在特定光照条件下有效，需要一种能同时处理增强和噪声抑制的通用方法。

Method: 1. 使用梯度域加权引导滤波(GDWGIF)准确估计光照并提升图像质量；2. 应用Retinex模型将图像分解为光照层和反射层；3. 并行处理两个层：校正光照层优化照明条件，增强反射层提升图像质量；4. 通过多曝光融合和线性拉伸策略优化图像动态范围。
...
TL;DR: 该研究提出利用面部美化评估算法来检测面部修饰，在未知攻击修饰算法的情况下，单张图像检测达到1.1%的D-EER


<details>
  <summary>Details</summary>
Motivation: 面部修饰美化在社交媒体和广告中广泛使用，但当这些修饰后的图像作为生物特征样本时，会对人脸识别系统构成挑战。因此需要有效检测面部修饰

Method: 研究分析面部修饰对美容评估算法的影响，评估基于人工智能的特征提取方法，探索是否可以利用面部美容评估来提升修饰检测率

Result: 在未知攻击修饰算法的情况下，单张图像检测达到了1.1%的D-EER（等错误率）

Conclusion: 面部美容评估可以作为有效的特征来检测面部修饰，在未知修饰算法的情况下仍能实现高精度的检测性能
...
TL;DR: 该论文开发了一个优化的深度学习流水线，用于使用AutoFish数据集进行鱼类再识别，通过硬三元组挖掘和自定义图像变换显著提升了再识别性能。


<details>
  <summary>Details</summary>
Motivation: 随着电子监控系统的广泛采用，收集的视频数据量已超过人工审查的可行性范围，需要自动化解决方案来有效管理海洋资源。

Method: 使用AutoFish数据集（模拟电子监控系统的传送带场景，包含6种外观相似的鱼类），采用硬三元组挖掘和自定义图像变换流水线（包括数据集特定归一化），比较了Vision Transformer的Swin-T架构和CNN的ResNet-50。

Result: Swin-T架构在硬三元组挖掘和自定义变换下表现最佳，达到41.65% mAP@k和90.43% Rank-1准确率。分析显示主要挑战是区分同物种的相似个体（种内误差），其中视角不一致比部分遮挡影响更大。

Conclusion: 优化的深度学习流水线显著提升了鱼类再识别性能，为电子监控系统的自动化分析提供了有效解决方案，代码已开源供进一步研究使用。
...
TL;DR: SAM-Body4D：无需额外训练的视频时序一致性人体网格恢复框架，通过视频分割和遮挡感知模块提升在遮挡场景下的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有基于图像的人体网格恢复方法（如SAM 3D Body）在处理视频时采用逐帧推理，导致时序不一致且在遮挡情况下性能下降。需要一种无需重新训练的方法来利用视频中的人体连续性特征。

Method: 提出训练免费的SAM-Body4D框架：1）使用可提示视频分割模型生成身份一致的掩码片段；2）通过遮挡感知模块恢复缺失区域；3）用精炼后的掩码片段指导SAM 3D Body生成一致的全身体网格轨迹；4）采用基于填充的并行策略实现高效多人推理。

Result: 实验结果表明，SAM-Body4D在具有挑战性的野外视频中实现了更好的时序稳定性和鲁棒性，且无需任何重新训练。

Conclusion: SAM-Body4D通过利用视频中的人体连续性，解决了现有方法在时序一致性和遮挡鲁棒性方面的不足，为视频中的人体网格恢复提供了一种有效的训练免费解决方案。
...
TL;DR: OneClip-RAG：一种基于单次视频片段检索增强的高效长视频理解范式，通过查询引导的视频分块算法统一处理步骤，显著提升多模态大语言模型的长视频处理能力


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型由于内存开销过大，只能处理有限帧数的视频，无法有效理解长视频内容，需要一种既能保持知识完整性又能保证语义连贯性的高效解决方案

Method: 提出OneClip-RAG范式，包含查询引导的视频分块算法，将视频分块和跨模态检索统一在一个处理步骤中；创建SynLongVideo数据集并设计渐进式训练策略来提升指令跟随能力

Result: 在五个最新MLLMs上验证，在长视频基准测试中表现优异：将InternLV2 8B和Qwen2-VL 7B提升到GPT-4o在MLVU上的水平；效率显著提升，LLaVA-Video可在单张4090 GPU上2.2分钟内处理长达1小时的视频

Conclusion: OneClip-RAG是一种有效且高效的长视频理解解决方案，通过创新的检索增强范式解决了MLLMs处理长视频的内存限制问题，在性能和效率方面都取得了显著提升
...
TL;DR: LapFM是一个用于腹腔镜手术分割的基础模型，通过分层概念演化预训练范式，利用大规模无标注手术图像构建鲁棒的分割能力，显著优于现有方法。
...
Conclusion: LapFM是一个专门设计的手术基础模型，通过分层概念演化预训练从大规模无标注手术图像中学习鲁棒的分割能力，解决了现有方法在泛化性和语义一致性方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有快照多光谱成像系统虽然能提供比传统RGB传感器更丰富的频谱信息，但大多数现有方法将色彩校正流程分为多个独立阶段，往往在早期就丢弃了多光谱数据，无法充分利用多光谱信息的优势。

Method: 提出一个统一的学习框架，将整个色彩校正流程集成在单一模型中，联合利用高分辨率RGB传感器和辅助低分辨率多光谱传感器的数据。通过重构两种不同的最先进的图像到图像架构来展示框架的灵活性和通用性。

Result: 通过聚合和重新利用公开可用的光谱数据集，在多种RGB相机灵敏度下渲染构建专用数据集进行训练和评估。实验表明该方法显著提高了色彩准确性和稳定性，相比仅使用RGB和多光谱驱动的基线方法，误差减少了高达50%。

Conclusion: 该统一学习框架能够有效整合多光谱信息，实现端到端的色彩校正，显著优于现有的分离处理方法，为消费级和移动设备的多光谱成像应用提供了更优的解决方案。
...
TL;DR: 该论文提出了一种结合子模子集选择和梯度不确定性估计的框架，以解决现有基于子集选择的可解释性方法在分布外场景下性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 基于子集选择的方法在解释深度视觉模型时表现良好，但在分布外条件下其可靠性显著下降，产生冗余、不稳定且对不确定性敏感的解释。目前对这些方法在分布外场景下的行为了解不足。

Method: 提出一个结合子模子集选择和基于梯度的层间不确定性估计的框架。通过自适应权重扰动估计不确定性，并利用这些估计指导子模优化，确保多样化和信息丰富的子集选择。该方法无需额外训练或辅助模型。

Result: 实验评估表明，该框架不仅缓解了现有方法在分布外场景下的弱点，还在分布内设置中带来了改进。相比现有方法，能产生更稳健、多样且信息丰富的解释。

Conclusion: 研究揭示了当前基于子集选择方法的局限性，并展示了不确定性驱动的优化如何增强归因和对象级可解释性，为现实世界视觉应用中更透明和可信的AI铺平道路。
...
<details>
  <summary>Details</summary>
Motivation: 足球运动员跟踪面临频繁遮挡、相似外观和快速运动等挑战，需要一种轻量级且鲁棒的跟踪方法，特别是在拥挤场景中。

Method: 提出团队感知跟踪系统，使用SAM进行精确初始化，结合CSRT跟踪器和基于HSV直方图的球衣颜色外观模型进行重识别，提高遮挡恢复能力。

Result: 在足球视频序列上达到7.6-7.7 FPS，内存使用稳定(~1880 MB)，轻度遮挡下100%跟踪成功率，拥挤禁区场景(5+球员)90%成功率，重度遮挡恢复率50%，但长期离帧重获成功率仅8.66%。

Conclusion: SAM+CSRT组合在连续可见性场景表现稳定，但需要更强的重识别机制处理长期离帧情况，为资源受限的足球跟踪系统部署提供了实用指导。
...
TL;DR: ContextDrag是一种基于拖拽的图像编辑新范式，通过利用编辑模型（如FLUX-Kontext）的强大上下文建模能力，结合VAE编码的参考图像特征，实现了更连贯、保真度更高的图像编辑效果。
...
Method: 1. 引入Context-preserving Token Injection (CTI)，通过Latent-space Reverse Mapping (LRM)算法将无噪声的参考特征注入到正确目标位置；2. 采用Position-Consistent Attention (PCA)，对参考标记进行位置重编码并应用重叠感知掩码以消除无关参考特征的干扰。
...
TL;DR: 本文提出PCI框架，通过分析概念插入成功率来研究扩散模型中概念形成的动态过程，揭示不同概念在去噪轨迹中的锁定时机，为图像编辑提供优化时机指导。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注扩散模型的最终输出，但生成过程沿着轨迹展开，分析这一动态过程对于理解模型的可控性、可靠性和可预测性至关重要。本文旨在探究噪声何时转化为特定概念并在去噪轨迹中锁定。
...
Conclusion: PCI框架能够分析扩散模型中概念形成的动态过程，揭示概念锁定的关键时机。该方法无需访问模型内部或训练，为图像编辑提供了优化干预时机的指导，在语义准确性和内容保留之间实现了比基线更好的平衡。
...
TL;DR: 首个多相机阵列实时3D高斯泼溅重建框架，通过层次化相机初始化、轻量级多相机束调整、无冗余高斯采样和频率感知优化调度，在2分钟内重建数百米3D场景


<details>
  <summary>Details</summary>
Motivation: 现有单目3D高斯泼溅实时重建方法因视野有限无法实现完整3D覆盖，多相机阵列能从根本上解决此问题，但缺乏相应的实时重建框架

Method: 提出层次化相机初始化方案实现无需标定的相机间粗对齐，采用轻量级多相机束调整稳定轨迹，引入无冗余高斯采样策略减少高斯基元数量，使用频率感知优化调度减少优化迭代次数

Result: 仅使用原始多相机视频流，在2分钟内重建数百米3D场景，实现了前所未有的速度、鲁棒性和重建保真度，支持无漂移轨迹估计和高效在线重建

Conclusion: 该方法首次实现了多相机阵列的实时3D高斯泼溅重建，通过创新的相机对齐、优化调度和采样策略，在保持实时性能的同时显著提升了重建覆盖范围和效率
...
<details>
  <summary>Details</summary>
Motivation: 多模态大推理模型（MLRMs）通过分层链式推理从个人图像中推断精确地理位置，带来严重隐私风险。现有隐私保护技术主要针对感知模型设计，对MLRMs的多步推理过程无效。

Method: 提出ReasonBreak框架，基于概念感知扰动破坏MLRMs的分层推理。关键洞见是：有效破坏地理推理需要与概念层次对齐的扰动而非均匀噪声。该方法战略性地针对推理链中的关键概念依赖，生成使特定推理步骤无效的扰动，并级联影响后续推理阶段。同时贡献GeoPrivacy-6K数据集，包含6,341张超高分辨率图像和分层概念标注。

Result: 在7个最先进的MLRMs（包括GPT-o3、GPT-5、Gemini 2.5 Pro）上评估显示，ReasonBreak在区域级保护上提升14.4%（33.8% vs 19.4%），在街区级保护上几乎翻倍（33.5% vs 16.8%）。

Conclusion: 这项工作为对抗基于推理的隐私威胁建立了新范式，ReasonBreak通过概念感知扰动有效破坏MLRMs的分层推理过程，显著提升地理隐私保护效果。
...
TL;DR: NoisyCLIP：一种在去噪过程中早期检测文本-图像对齐的方法，通过测量噪声潜在空间中的语义对齐，实现实时对齐评估，减少50%计算成本的同时保持98%的CLIP对齐性能。


<details>
  <summary>Details</summary>
Motivation: 条件扩散模型虽然依赖语言-图像对齐方法来引导生成语义准确的输出，但错位和幻觉问题仍然常见。传统方法需要在完整生成后才能评估对齐性，计算成本高昂。本文假设文本/图像错位可以在去噪过程的早期被检测到，从而实现实时对齐评估。

Method: 提出NoisyCLIP方法，在反向扩散过程中使用双编码器测量噪声潜在空间中的语义对齐。这是首次探索和基准测试在图像生成过程中使用双编码器进行提示-潜在错位检测的方法。

Result: NoisyCLIP在定性定量评估中表现良好，在Best-of-N设置中将计算成本降低50%，同时达到98%的CLIP对齐性能。该方法能够在生成过程中进行实时对齐评估，在不牺牲语义保真度的前提下降低成本。
...
TL;DR: OCCDiff：一种在占用函数空间应用潜在扩散的方法，用于从不同分辨率的LiDAR点云中重建建筑物，通过函数自编码器架构生成可在任意位置评估的连续占用函数。


<details>
  <summary>Details</summary>
Motivation: 从LiDAR点云重建建筑物面临的主要挑战在于：在不同点密度和噪声干扰下准确捕捉建筑表面。需要一种灵活的方法来在不同分辨率下获取高质量的建筑3D轮廓。

Method: 提出OCCDiff方法，将潜在扩散过程与函数自编码器架构结合，生成可在任意位置评估的连续占用函数。包含点编码器提供条件特征给扩散学习，约束最终占用预测，并向潜在编码器插入多模态特征。采用多任务训练策略增强模型性能。

Result: 实验结果表明，该方法生成物理一致的样本，对目标分布具有高保真度，并对噪声数据表现出鲁棒性。

Conclusion: OCCDiff通过占用函数空间的潜在扩散，有效解决了LiDAR点云建筑物重建中的密度变化和噪声问题，能够生成高质量、连续的3D建筑轮廓。
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI grounding模型存在显著的坐标预测不稳定性问题，微小的视觉扰动（如裁剪几个像素）就会导致预测结果在正确和错误之间翻转，严重影响模型性能，特别是在高分辨率和小UI元素的样本上
...
TL;DR: 提出统一多模态油画生成编辑框架，通过空间对齐、语义增强和自监督风格迁移技术实现交互式油画创作


<details>
  <summary>Details</summary>
Motivation: 油画作为融合人类抽象思维与艺术表达的高级媒介，其复杂的笔触动态和风格化特征给数字生成和编辑带来巨大挑战。现有技术受限于训练数据分布，主要关注真实照片修改，缺乏针对油画风格的专业解决方案。

Method: 1. 训练阶段增强：采用空间对齐和语义增强条件策略，将掩码和草图映射为空间约束，从参考图像和文本编码上下文嵌入作为特征约束，实现对象级语义对齐。2. 自监督风格迁移：基于笔触渲染(SBR)构建自监督风格转换管道，模拟油画修复的修复动态，将真实图像转换为保留笔触纹理的风格化油画，构建大规模配对训练数据集。3. 推理阶段：使用AdaIN算子集成特征确保风格一致性。

Result: 大量实验证明，该交互式系统能够在保留油画艺术品质的同时实现细粒度编辑，在风格化油画生成和编辑方面达到了前所未有的想象力实现水平。

Conclusion: 提出的统一多模态框架成功解决了油画数字生成和编辑的挑战，通过创新的训练策略、数据增强技术和特征集成方法，实现了交互式油画创作，为艺术创作提供了新的技术可能性。
...
TL;DR: Photo3D是一个通过GPT-4o生成图像数据驱动的高质量3D生成框架，通过结构对齐的多视角合成和细节增强技术，解决现有3D生成器在真实感外观方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有3D原生生成器在几何结构方面取得了进展，但在实现真实感外观方面仍然不足。主要障碍在于缺乏多样化和高质量的真实世界3D资产，这些资产需要丰富的纹理细节，而由于场景尺度多样、物体非刚性运动以及3D扫描仪精度有限，获取这样的数据本质上是困难的。

Method: 1. 使用GPT-4o-Image模型生成图像数据作为驱动；2. 设计结构对齐的多视角合成流程，构建细节增强的多视角数据集与3D几何配对；3. 提出真实感细节增强方案，利用感知特征适应和语义结构匹配，在保持与3D原生几何结构一致性的同时增强外观真实感；4. 为不同的3D原生生成器提供专门的训练策略，支持几何-纹理耦合和解耦的生成范式。

Result: 实验表明，Photo3D能够很好地泛化到不同的3D原生生成范式，并实现了最先进的真实感3D生成性能。

Conclusion: Photo3D框架通过利用生成图像数据和创新的细节增强技术，有效解决了3D生成中真实感外观的挑战，为不同3D生成范式提供了通用的高质量解决方案。
...
TL;DR: Fast-ARDiff提出了一种统一的AR-扩散混合框架，通过熵感知的推测解码和动态调度策略，同时加速AR和扩散组件，实现4.3倍无损加速
...
2) 动态调度集成：将扩散解码整合到端到端框架中，使用动态调度器优先优化AR部分来指导后续扩散步骤
3) 联合蒸馏框架：结合轨迹匹配和分布匹配进行扩散部分优化，确保稳定训练和极少数步下的高质量合成
4) 推理优化：利用AR模块的浅层特征熵预过滤低熵草稿，避免冗余计算

Result: 在ImageNet 256×256上，TransDiff模型实现了4.3倍无损加速；在文本条件生成任务上，NextStep-1模型实现了3倍加速，达到最先进的加速效果

Conclusion: Fast-ARDiff通过统一优化AR和扩散组件，有效解决了AR-扩散混合范式的延迟瓶颈，实现了显著的加速效果，同时保持生成质量，为高效图像生成提供了新思路
...
TL;DR: 提出了一种无迭代的固定点估计器用于扩散反演，避免了传统固定点迭代方法的高计算成本和超参数选择问题


<details>
  <summary>Details</summary>
Motivation: 扩散反演旨在恢复与给定图像对应的初始噪声，以便通过去噪扩散过程重建原始图像。传统固定点迭代方法虽然能最小化重建误差，但存在计算成本高和超参数选择复杂的问题

Method: 首先从理想反演步骤推导出固定点的显式表达式，但其中包含未知的数据预测误差。然后引入误差近似，使用前一个反演步骤的可计算误差来近似当前步骤的未知误差，从而得到可计算的固定点近似表达式

Result: 在NOCAPS和MS-COCO两个文本-图像数据集上评估重建性能。与DDIM反演和其他基于固定点迭代的反演方法相比，该方法在不增加额外迭代或训练的情况下，在重建任务中取得了一致且优越的性能

Conclusion: 提出的无迭代固定点估计器是一种无偏、低方差的扩散反演方法，有效解决了传统固定点迭代方法的计算效率和超参数选择问题，在图像重建任务中表现出色
...
Method: 采用两阶段方法：1）通过无监督数据驱动分解方法发现fMRI活动中的可解释模式；2）通过识别最能激发这些模式的自然图像并生成描述其共享视觉意义的自然语言来解释每个模式。引入自动化管道测试多个候选解释，分配定量可靠性分数，为每个体素模式选择最一致的描述。

Result: 该框架揭示了数千个可解释模式，涵盖许多不同的视觉概念，包括以前未报告的细粒度表征。

Conclusion: 提出了一个可扩展的自动化框架，能够大规模发现和解释人类大脑皮层的视觉表征，克服了传统方法的局限性。
...
Method: 采用模块化神经ISP框架，将整个渲染过程分解为多个可控制的中间阶段，支持不同容量变体（0.5M-3.9M参数），并开发用户交互式照片编辑工具

Result: 在多个测试集上获得有竞争力的定性和定量结果，实现了高渲染精度，同时提升了可扩展性、可调试性、对未见相机的泛化能力以及匹配不同用户偏好风格的灵活性

Conclusion: 模块化神经ISP设计不仅实现了高质量图像渲染，还提供了更好的控制性和灵活性，支持丰富的用户交互编辑功能，为神经ISP系统设计提供了新的方向
...
TL;DR: 本文提出了一种用于连续测试时适应的新方法，通过自适应调整伪标签来反映每个图像内的置信度分布，并动态平衡受域偏移影响最大的类别的学习，从而在语义分割任务中实现更可靠的监督。


<details>
  <summary>Details</summary>
Motivation: 现有连续测试时适应方法通常依赖固定或批量级阈值，无法考虑不同类别和实例之间的难度差异。这在语义分割中尤其成问题，因为每个图像都需要密集的多类别预测。需要一种更精细的、类别和实例感知的适应方法。

Method: 提出了一种自适应调整伪标签的方法，能够反映每个图像内的置信度分布，并动态平衡学习向受域偏移影响最大的类别倾斜。该方法考虑了类别和实例级别的差异，提供更精细的适应策略。

Result: 在八个连续测试时适应和测试时适应场景（包括合成到真实和长期偏移）的广泛实验中，该方法始终优于最先进的技术，为演化条件下的语义分割设定了新标准。

Conclusion: 该方法通过自适应调整伪标签和动态平衡类别学习，解决了现有方法在连续测试时适应中的局限性，特别是在语义分割任务中，能够产生更可靠的监督并减轻错误累积，在多种场景下表现出优越性能。
...
TL;DR: 该研究应用深度学习改进和自动化花粉粒在光学和全息显微图像中的检测与分类，特别关注兽医细胞学应用。使用YOLOv8s进行目标检测，MobileNetV3L进行分类，在光学图像上取得良好性能，但在灰度全息图像上性能较差，通过数据集扩展和标注改进提升了全息图像的性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索深度学习在花粉粒检测和分类中的应用，特别是在光学和全息显微图像中，重点关注兽医细胞学用例。全息显微技术成本较低但性能较差，需要改进其深度学习应用效果。

Method: 采用YOLOv8s进行目标检测，MobileNetV3L进行分类任务。针对全息图像性能差的问题，使用自动标注和边界框区域扩大的数据集扩展技术来改进性能。

Result: 在光学图像上，检测达到91.3% mAP50，分类达到97%总体准确率。在全息图像上，初始检测性能仅2.49% mAP50，分类42%准确率。通过改进技术后，检测提升到13.3% mAP50，分类提升到54%准确率。

Conclusion: 研究表明，至少在图像分类任务中，可以将深度学习技术与经济实惠的无透镜数字全息显微设备结合使用，为低成本诊断工具开发提供了可能性。
...
TL;DR: CLIP模型中模板-样本相似性(TSS)引入偏差，导致模型依赖模板接近度而非真实类别对齐，降低分类准确性和鲁棒性。作者提出使用空提示来捕获无偏模板特征并抵消TSS偏差的两阶段框架。


<details>
  <summary>Details</summary>
Motivation: 研究发现CLIP模型中的模板-样本相似性(TSS)会引入偏差，使模型更关注文本模板与图像样本的相似性，而非图像与真实类别的对齐关系，这降低了少样本学习的准确性和鲁棒性。

Method: 提出两阶段框架：1) 预训练阶段使用空提示（不含类别信息的文本输入）揭示并减少CLIP编码器中的模板诱导偏差；2) 少样本微调阶段通过偏差校准损失强制图像与其类别的正确对齐，确保模型关注相关视觉线索。

Result: 在多个基准测试上的实验表明，该模板校正方法显著减少了由TSS引起的性能波动，获得了更高的分类准确性和更强的鲁棒性。

Conclusion: 通过使用空提示和偏差校准损失来解耦CLIP中的模板偏差，可以有效提高少样本学习的性能稳定性和准确性。
...
Method: 提出OpenMonoGS-SLAM框架，统一3D高斯溅射与开放集语义理解。利用视觉基础模型（MASt3R用于视觉几何，SAM和CLIP用于开放词汇语义），仅通过自监督学习目标实现单目相机跟踪和建图。设计了专门的内存机制来管理高维语义特征，构建高斯语义特征图。

Result: 实验结果表明，该方法在封闭集和开放集分割任务中达到或超越现有基线性能，且不依赖深度图或语义标注等辅助传感器。

Conclusion: OpenMonoGS-SLAM成功实现了无需深度输入或3D语义标注的单目SLAM与开放集语义理解的统一，为开放世界环境下的智能感知提供了有效解决方案。
...
TL;DR: 提出了一种通过分析视频流模糊模式来估计度量深度和密集轨迹的新方法，利用光学设计算法处理手持拍摄时的旋转动态模糊问题。


<details>
  <summary>Details</summary>
Motivation: 在缺乏机械稳定器的情况下，相机在拍摄过程中不可避免地会发生旋转动态，特别是在长时间曝光场景下会导致透视模糊。从光学角度看，这种模糊是深度位置相关的：不同空间位置的物体即使在相同成像设置下也会产生不同的模糊程度。

Method: 通过联合光学设计算法分析视频流的模糊模式来估计度量深度和密集轨迹。使用现成的视觉编码器和点跟踪器提取视频信息，通过窗口化嵌入和多窗口聚合估计深度图，并使用视觉语言模型对光学算法产生的稀疏轨迹进行稠密化。

Result: 在多个深度数据集上的评估表明，该方法在大深度范围内表现出强大的性能，同时保持良好的泛化能力。相对于手持拍摄环境中的真实轨迹，光学算法实现了更高的精度，稠密重建保持了较强的准确性。

Conclusion: 该方法通过分析视频流中的透视模糊模式，成功实现了度量深度估计和密集轨迹重建，为解决手持拍摄中的旋转动态模糊问题提供了一种有效的解决方案。
...
TL;DR: 本文提出了一种仅使用单目RGB图像和自然语言指令的无人机视觉语言导航统一框架，通过多任务学习和关键帧选择等技术，在单目RGB设置下取得了接近全景RGB-D方法的性能。
...
Method: 提出了一个统一的空中VLN框架，仅使用单目RGB观测和自然语言指令；将导航建模为下一个token预测问题，通过提示引导的多任务学习联合优化空间感知、轨迹推理和动作预测；提出了关键帧选择策略以减少视觉冗余，以及动作合并和标签重加权机制来缓解长尾监督不平衡并促进稳定的多任务协同训练。
...
Conclusion: 本文提出的仅使用单目RGB图像的空中视觉语言导航框架在减少系统复杂度的同时保持了良好的导航性能，为轻量级无人机的实际部署提供了可行的解决方案。
...
TL;DR: CoIG框架将图像生成重构为类似人类艺术创作的顺序语义过程，通过LLM将复杂提示分解为逐步指令，使生成过程可监控和干预
...
Result: 实验表明CoIG显著增强定量可监控性，同时在组合鲁棒性方面与基线模型竞争；提出两个新指标评估：CoIG可读性评估中间步骤清晰度，因果相关性量化每个步骤对最终图像的影响

Conclusion: CoIG框架为图像生成带来类似CoT在LLM中的优势，提高可监控性和性能，缓解实体崩溃问题，且模型无关，可与任何图像生成模型集成
...
Result: 在State Farm Distracted Driver Detection Dataset上，C-DIRA相比先前轻量级模型保持高准确率的同时显著减少FLOPs和降低延迟，在模糊和低光照等视觉退化条件下表现鲁棒，在未见域上性能稳定

Conclusion: C-DIRA框架在紧凑性、效率和泛化能力方面表现出色，有效解决了驾驶员行为识别中效率与准确性的平衡问题
...
<details>
  <summary>Details</summary>
Motivation: 去噪生成模型（如扩散模型、流匹配）在视觉合成中占主导地位，但存在训练成本高和表示学习效率低的问题。现有方法依赖外部预训练编码器进行判别表示对齐，这带来了额外开销和领域偏移问题。

Method: 提出Mname框架，采用内存银行机制维护跨训练迭代的动态更新负样本队列，将负样本数量与批大小解耦；使用低维投影头最小化内存和带宽开销；无需外部编码器，是即插即用的训练框架。

Result: 在ImageNet-256上，Mname在40万步内达到2.40的FID分数，创下新纪录，显著优于同类方法；实现了更快的收敛速度和更优的生成质量。

Conclusion: Mname框架通过内存银行机制有效解决了去噪生成模型对预训练编码器的依赖问题，实现了自包含、无推理额外开销的高效训练，显著提升了生成模型的训练效率和生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前行人重识别方法虽然准确率高但缺乏可解释性，无法明确模型依赖哪些高层语义属性。本文旨在系统性地量化行人属性对重识别的重要性。

Method: 提出MoSAIC-ReID框架，使用基于LoRA的专家模型，每个专家对应单一属性，配合路由机制进行可控的属性归因分析。采用广义线性模型、统计检验和特征重要性分析来量化属性贡献。

Result: 在Market-1501和DukeMTMC数据集上获得有竞争力的性能（假设测试时有属性标注）。通过量化分析发现服装颜色和内在特征贡献最大，而配件等不常见线索影响有限。

Conclusion: 该工作为可解释行人重识别提供了原则性框架，强调了在实践中整合显式语义知识的需求，为理解模型决策提供了系统方法。
...
TL;DR: 本文探索了将SAM 3模型应用于遥感开放词汇语义分割任务，无需训练。通过掩码融合策略结合语义分割头和实例头输出，并利用存在分数过滤不存在的类别，在遥感数据集上取得了良好效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的训练免费开放词汇语义分割方法在精确定位方面面临挑战，或需要复杂流水线组合多个模块，特别是在遥感场景中存在大量密集小目标的情况下。最近提出的SAM 3模型统一了分割和识别功能，为遥感开放词汇语义分割提供了新的可能性。

Method: 1. 实施掩码融合策略，结合SAM 3的语义分割头和Transformer解码器（实例头）的输出，充分利用两个头的优势改善土地覆盖分割；2. 利用存在头的存在分数过滤场景中不存在的类别，减少由于地理空间场景中词汇量大和补丁级处理导致的误报。

Result: 在广泛的遥感数据集上评估该方法，实验表明这种简单的适应策略取得了有希望的性能，展示了SAM 3在遥感开放词汇语义分割任务中的潜力。

Conclusion: SAM 3模型在无需训练的情况下，通过简单的适应策略就能在遥感开放词汇语义分割任务中表现出良好性能，为遥感场景下的开放词汇分割提供了新的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 肤色历史上一直是歧视的焦点，但医学影像机器学习中的公平性研究通常依赖粗糙的亚组分类，忽视了个体层面的差异。这种基于群体的方法可能会掩盖亚组内异常值面临的偏见。
...
Conclusion: 这些发现为在皮肤科AI系统中推进个体层面公平性建立了稳健的方法论，并强调了在医学影像分析中处理敏感连续属性的更广泛意义。
...
TL;DR: 提出了一种结合3D渲染和约束扩散模型的工作流程，用于自动生成高质量、逼真的蘑菇合成图像，无需专业图形学知识，在零样本设置下实现了最先进的分割性能。
...
Conclusion: 该方法虽然以双孢蘑菇为例进行演示，但提出的流程可以轻松适应其他蘑菇物种或其他农业领域，如水果和叶子检测。合成数据生成方法为农业计算机视觉应用提供了一种可扩展且高效的解决方案。
...
Method: 提出偏度指导的剪枝方法，基于输出分布的统计偏度选择性剪枝多模态Swin Transformer的多头自注意力层和多层感知机层，在水平联邦学习环境中验证

Result: 在紧凑型Swin Transformer上实验显示约36%的模型大小缩减，同时保持准确率不损失，验证了方法在联邦学习环境中的有效性
...
Method: 核心思想是直接使原始条件特征具有运动感知能力。首先用密集点轨迹表示物体运动，实现场景的细粒度控制；然后将轨迹投影到潜在空间，沿每条轨迹传播第一帧特征，生成对齐的时空特征图；最后将该特征图作为更新的潜在条件，无缝集成到现成的图像到视频模型中作为运动指导。

Result: 通过规模化训练，Wan-Move生成的5秒480p视频在运动可控性上可与Kling 1.5 Pro的商业Motion Brush相媲美（用户研究证实）。在MoveBench基准测试和公开数据集上的实验一致显示其优越的运动质量。

Conclusion: Wan-Move提供了一个简单可扩展的运动控制框架，无需辅助运动编码器，使基础模型微调易于扩展，在运动控制质量和可扩展性方面显著优于现有方法。
...
TL;DR: 提出自精炼扩散框架，利用可解释AI技术检测图像生成中的伪影和不真实区域，通过缺陷激活图指导扩散过程，显著提升图像质量。
...
Method: 提出自精炼扩散框架，使用基于可解释AI的缺陷高亮器生成缺陷激活图（FAMs），这些图能识别伪影和不真实区域。在正向过程中放大缺陷区域的噪声，在反向过程中重点关注这些区域，从而改善重建质量。

Result: 该方法在各种基于扩散的模型上实现了高达27.3%的Fréchet inception距离改进，在多样化数据集上表现一致强劲。在不同任务（图像生成、文本到图像生成、修复）中都显示出鲁棒的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 文档阴影去除对于提高数字化文档清晰度至关重要。阴影常常会模糊或扭曲文本边缘和线条等高频细节，因此保留这些细节是阴影去除过程中的关键挑战。

Method: 提出MatteViT框架：1）轻量级高频放大模块(HFAM)，分解并自适应放大高频分量；2）基于连续亮度的阴影蒙版，使用定制数据集和蒙版生成器提供精确空间引导。结合空间和频域信息进行阴影去除。

Result: 在RDD和Kligler等公开基准测试中达到最先进性能。在光学字符识别等下游任务中能更好地保留文本级细节，提高识别性能。

Conclusion: MatteViT为实际文档阴影去除提供了鲁棒实用的解决方案，通过高频细节保留策略有效恢复文档结构细节，在阴影去除和下游任务中都表现出优越性能。
...
TL;DR: 提出了一种无需训练的双曲适配器方法T-DHA，用于提升视觉语言模型在少样本图像识别和领域泛化任务中的性能，通过双曲空间建模层次化视觉-语言关系，结合负学习实现更准确鲁棒的分类。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在领域变化时性能下降，或需要大量计算资源进行微调。为了解决这些问题，需要开发一种无需训练的高效适配方法。

Method: 提出训练免费的双曲适配器T-DHA，在双曲空间（使用Poincaré球模型）而非传统欧氏空间中建模视觉-语言关系的层次树结构。双曲空间具有指数级体积增长特性，能更有效地嵌入层次化数据结构。结合负学习，使用更少的特征维度实现更准确鲁棒的分类。

Result: 在多个数据集上的实验结果表明，T-DHA方法在少样本图像识别和领域泛化任务中显著优于现有最先进方法。

Conclusion: T-DHA方法通过双曲空间建模视觉-语言层次关系，提供了一种无需训练的高效适配方案，显著提升了视觉语言模型在领域变化下的性能和泛化能力。
...
TL;DR: InfiniteVL是一种线性复杂度的视觉语言模型架构，结合滑动窗口注意力与Gated DeltaNet，在有限资源下实现竞争性多模态性能，显著超越现有线性复杂度VLM并匹配Transformer-based VLM性能。
...
Result: InfiniteVL不仅显著超越先前线性复杂度VLM，还匹配领先Transformer-based VLM性能，同时保持有效长期记忆。相比FlashAttention-2加速的类似大小Transformer-based VLM，推理速度提升超过3.6倍，保持恒定延迟和内存占用。在流式视频理解中保持24 FPS实时预填充速度。

Conclusion: InfiniteVL成功解决了窗口注意力和线性注意力的局限性，实现了线性复杂度下的竞争性多模态性能，在推理速度和内存效率方面具有显著优势，特别适合长序列和实时应用场景。
...
TL;DR: 该研究探讨生成式与非生成式视觉方法在组合泛化能力上的差异，发现生成式方法通过约束解码器并反转它能够实现组合泛化，而非生成式方法通常难以实现这种泛化能力。


<details>
  <summary>Details</summary>
Motivation: 人类视觉感知被认为需要生成式方法，但当前最成功的视觉模型都是非生成式的编码器模型。这引发了一个问题：生成是否真的是实现人类水平视觉感知的必要条件？研究旨在探索生成式和非生成式方法是否都能实现组合泛化这一人类感知的标志性特征。

Method: 在组合数据生成过程下，形式化定义了基于解码器（生成式）和基于编码器（非生成式）方法实现组合泛化所需的归纳偏置。理论上证明在编码器上强制执行这些归纳偏置通常不可行，而生成式方法可以通过约束解码器并反转它来直接实现。研究了两种反转方法：基于梯度的在线搜索和生成回放的离线方法。

Result: 理论分析表明，非生成式方法通常难以实现组合泛化，需要大规模预训练或额外监督来改善泛化能力。相比之下，生成式方法通过合适的解码器归纳偏置以及搜索和回放技术，能够在不需要额外数据的情况下显著提升组合泛化能力。在逼真图像数据集上的实证研究验证了这一理论发现。

Conclusion: 生成式方法在实现组合泛化方面具有理论优势，通过约束解码器并反转它能够自然地实现所需的归纳偏置，而非生成式方法在这方面存在根本性困难。这支持了生成式方法对于实现人类水平视觉感知的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型正在成为遥感领域的强大通用工具，但需要增强其在卫星图像中的视觉定位能力，特别是在复杂场景中精确定位物体的能力。

Method: 通过微调预训练的视觉语言模型，结合专门的定位模块和专用控制令牌，实现语言信息和空间信息的联合推理，形成结构化定位机制。

Result: 在多个遥感基准测试中取得了显著改进，视觉定位任务相对之前方法提升了24.8%，达到了最先进的性能水平。

Conclusion: 将结构化空间推理整合到视觉语言模型中能够显著提升卫星数据分析的可靠性，为实际应用铺平道路。
...
TL;DR: 提出VALOR框架，通过AI验证器实现无需标注的训练，提升视觉推理和物体定位能力，超越开源和专有模型


<details>
  <summary>Details</summary>
Motivation: 现有视觉推理方法存在局限性：语言思维链方法需要大规模监督数据，程序合成方法存在逻辑缺陷和定位错误。需要一种无需标注但能同时提升推理和定位能力的方法。

Method: 提出基于AI验证器的训练框架：1) LLM验证器通过强化学习精炼推理过程；2) VLM验证器通过自动硬负例挖掘增强视觉定位。结合语言模型的复杂推理能力和视觉专家的定位能力。

Result: 在多种空间推理任务上评估，方法提升了视觉推理能力，超越了开源和专有模型。改进的视觉定位模型进一步优于最近的纯文本视觉推理方法。

Conclusion: VALOR框架通过AI验证器实现了无需标注的训练，有效结合了语言推理模型和视觉专家模型的优势，显著提升了视觉推理和物体定位性能。
...
<details>
  <summary>Details</summary>
Motivation: 内容感知布局生成是图形设计自动化的关键任务，但现实应用多样性使得开发单一模型统一处理各种输入约束生成子任务（如元素类型、大小或关系约束）极具挑战。现有方法要么只处理部分任务，要么需要为不同条件使用单独模型参数，缺乏真正的统一解决方案。

Method: 提出UniLayDiff统一扩散Transformer，将布局约束作为独立模态，采用多模态扩散Transformer框架捕捉背景图像、布局元素和多样约束之间的复杂交互。通过LoRA微调整合关系约束，在预训练其他任务后进行微调。

Result: 大量实验表明，UniLayDiff在从无条件到各种条件生成任务中均达到最先进性能，并且是首个统一完整范围内容感知布局生成任务的模型。

Conclusion: UniLayDiff首次实现了使用单一端到端可训练模型统一处理各种内容感知布局生成任务，不仅实现了统一条件生成，还提升了整体布局质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于单张图像的3D生成方法存在局限性：对象中心化的训练方式难以泛化到复杂的大规模场景，无法保证场景结构和纹理的忠实性

Method: 采用三阶段迭代框架：1)空间先验初始化；2)视觉引导的3D场景网格生成；3)空间引导的新视角生成。通过在2D和3D域之间交替迭代，逐步改进结构和外观

Result: 在多样化场景上的实验表明，EvoScene相比现有基线方法，在几何稳定性、视角一致的纹理以及未见过区域的补全方面表现更优，能生成可直接用于实际应用的3D网格
...
<details>
  <summary>Details</summary>
Motivation: 从视频中理解和重建动态场景的复杂几何和运动是计算机视觉中的重大挑战，现有方法通常计算密集且复杂，需要更高效、统一的解决方案。

Method: 采用统一的Transformer架构，设计新颖的查询机制，避免密集的逐帧解码和多个任务特定解码器的复杂性，允许模型独立灵活地探测时空任意点的3D位置。

Result: D4RT在广泛的4D重建任务中超越了先前方法，实现了新的最先进性能，同时保持了轻量级和高可扩展性，训练和推理效率显著。

Conclusion: D4RT通过统一的Transformer架构和创新的查询机制，为动态场景的4D重建提供了一个高效、轻量且可扩展的解决方案，在多个任务上取得了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统新视角合成依赖显式3D归纳偏置和已知相机参数，而VGGT等视觉基础模型虽然能直接从无标定图像预测相机参数和3D表示，但其特征缺乏显式的多视角几何一致性。研究发现改进3D特征一致性对NVS和姿态估计都有益。

Method: 提出Selfi自改进3D重建流水线，通过特征对齐将VGGT骨干转换为高保真3D重建引擎。使用重投影一致性损失训练轻量级特征适配器，将VGGT输出蒸馏到新的几何对齐特征空间，该空间能捕捉3D空间邻近性。

Result: 在无标定图像的新视角合成和相机姿态估计任务中实现了最先进的性能，证明了特征对齐对下游3D推理的高度有益性。

Conclusion: 特征对齐是将VGGT等基础模型转换为高质量3D重建引擎的关键步骤，通过自改进流水线利用模型自身输出作为伪真值，显著提升了3D特征的一致性和下游任务性能。
...
Result: Astra实现了交互式、一致且通用的长时程视频预测，支持多种交互形式。在多个数据集上的实验表明，Astra在保真度、长程预测和动作对齐方面优于现有最先进的世界模型。

Conclusion: Astra是一个强大的交互式通用世界模型，能够为多样化真实世界任务生成高质量的未来预测，支持精确的动作控制，在多个关键指标上超越了现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有字幕对齐方法通常依赖于特定语言或数据集的端到端训练，这限制了其通用性。需要开发一个能够在多种语言和领域通用的字幕对齐框架。

Method: SEA方法包含三个步骤：1) 使用预训练模型将视频帧序列分割成单个手语单元；2) 将每个手语单元的视频片段嵌入到与文本共享的潜在空间中；3) 使用轻量级动态规划程序进行对齐，即使在长达一小时的视频中也能在CPU上快速完成。

Result: 在四个手语数据集上的实验表明，SEA方法实现了最先进的对齐性能，展示了其生成高质量平行数据以推进手语处理的潜力。

Conclusion: SEA提供了一个灵活、通用的字幕对齐框架，能够适应从小型词典到大型连续语料库的各种场景，其代码和模型已开源，有望促进手语处理领域的发展。
...
TL;DR: 该论文研究通用对抗后缀：短令牌序列（4-10个令牌），当附加到任何输入时，能广泛降低跨任务和模型的准确性。


<details>
  <summary>Details</summary>
Motivation: 语言模型作为零样本或少样本分类器时，通过评分标签词进行分类，但对对抗性提示仍然脆弱。先前工作通常优化任务或模型特定的触发器，使得结果难以比较且可转移性有限。

Method: 使用Gumbel-Softmax松弛学习可微分的"软"形式后缀，然后离散化用于推理。训练最大化标签区域的校准交叉熵，同时屏蔽黄金令牌以防止平凡泄漏，并使用熵正则化避免崩溃。

Result: 在情感分析、自然语言推理、释义检测、常识问答和物理推理等任务上，使用Qwen2-1.5B、Phi-1.5和TinyLlama-1.1B模型进行实验，证明了攻击效果的一致性和跨任务及模型族的可转移性。

Conclusion: 单个在一个模型上训练的后缀能有效转移到其他模型，持续降低准确性和校准置信度，展示了通用对抗后缀的有效性和可转移性。
...
TL;DR: ClinicalTrialsHub是一个整合ClinicalTrials.gov数据和PubMed文献的交互式临床试验搜索平台，通过大语言模型自动提取结构化信息，将可访问的临床试验数据增加了83.8%。


<details>
  <summary>Details</summary>
Motivation: 现有临床试验数据分散在ClinicalTrials.gov和PubMed文献中，缺乏统一的结构化访问方式，限制了患者、临床医生、研究人员和政策制定者获取完整试验信息的能力。
...
Conclusion: ClinicalTrialsHub通过整合和增强临床试验数据访问，为患者、临床医生、研究人员和政策制定者提供了更便捷的循证医学工具，推动了证据医学的发展。
...
TL;DR: 使用强化学习框架训练对抗性后缀，相比传统梯度搜索方法具有更好的跨任务和跨模型迁移能力


<details>
  <summary>Details</summary>
Motivation: 语言模型容易受到短对抗性后缀的攻击，传统方法（梯度搜索或基于规则的方法）通常脆弱且局限于单一任务或模型，需要更通用、可迁移的对抗攻击方法

Method: 采用强化学习框架，将对抗性后缀视为策略，使用近端策略优化（PPO）算法针对冻结模型作为奖励预言机进行训练；通过校准交叉熵塑造奖励，消除标签偏差并聚合不同表面形式以提高可迁移性

Result: 在五个不同的NLP基准数据集（涵盖情感分析、自然语言推理、释义、常识推理）上评估，使用三种不同语言模型（Qwen2-1.5B Instruct、TinyLlama-1.1B Chat、Phi-1.5）；结果显示RL训练的后缀能持续降低模型准确率，且比类似类型的先前对抗性触发器在跨任务和跨模型迁移方面更有效

Conclusion: 强化学习训练的对抗性后缀为评估语言模型鲁棒性提供了更通用、可迁移的攻击方法，相比传统方法具有显著优势
...
### [91] [Are generative AI text annotations systematically biased?](https://arxiv.org/abs/2512.08404)
*Sjoerd B. Stolwijk,Mark Boukes,Damian Trilling*

Main category: cs.CL

TL;DR: 该研究通过概念性复现手动标注，调查了GLLM标注中的偏见问题，发现GLLM在F1分数上表现尚可，但在流行度估计、下游结果和系统性偏见方面与手动标注存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 研究动机是调查生成式大语言模型（GLLM）在文本标注任务中存在的偏见问题。通过概念性复现Boukes（2024）的手动标注研究，探索GLLM标注与人工标注之间的差异及其对研究结果的影响。

Method: 研究方法包括使用多种GLLM模型（Llama3.1:8b、Llama3.3:70b、GPT4o、Qwen2.5:72b），结合五种不同的提示词，对五个概念（政治内容、互动性、理性、不文明程度和意识形态）进行标注。通过比较GLLM标注与手动标注的一致性来评估偏见程度。

Result: 研究结果显示：1）GLLM在F1分数方面表现尚可；2）但在流行度估计上与手动标注存在差异；3）导致下游结果产生实质性不同；4）存在系统性偏见，即不同GLLM模型之间的标注一致性高于它们与手动标注的一致性；5）F1分数的差异不能完全反映偏见程度。

Conclusion: 结论是GLLM标注虽然在某些指标上表现良好，但存在系统性偏见，与手动标注在流行度估计和下游结果上存在显著差异。仅依靠F1分数无法全面评估GLLM标注的偏见程度，研究者需要谨慎使用GLLM进行文本标注。
...
### [92] [What Triggers my Model? Contrastive Explanations Inform Gender Choices by Translation Models](https://arxiv.org/abs/2512.08440)
*Janiça Hackenbuchner,Arda Tezcan,Joke Daems*

Main category: cs.CL

TL;DR: 该研究通过对比解释和显著性归因分析，探索机器翻译模型中性别偏见的起源，而非仅仅测量偏见程度。研究使用性别模糊的自然源数据，分析源句中哪些上下文标记影响翻译模型选择特定性别屈折变化，并将模型归因与人类感知进行比较。


<details>
  <summary>Details</summary>
Motivation: 当前可解释性研究主要关注理解黑盒模型的决策，但在性别偏见这一重要问题上的研究有限。研究旨在从单纯测量偏见转向探索其起源，理解翻译模型中性别决策的触发因素。

Method: 使用性别模糊的自然源数据，采用对比解释和计算显著性归因的方法。首先解决缺乏评分阈值的问题，分析不同归因水平下源词对模型性别决策的影响。将显著的源词与人类对性别的感知进行比较，并进行语言学分析。

Result: 研究发现人类感知与模型归因之间存在显著重叠，表明模型在性别决策时与人类有相似的模式。通过语言学分析揭示了影响性别决策的关键源词特征。

Conclusion: 理解模型翻译决策中的性别因素至关重要，这些信息应与人类决策进行比较，并用于缓解性别偏见。研究展示了通过分析模型归因来理解性别偏见起源的价值。
...
TL;DR: 该研究提出一种软归纳偏置方法，通过明确定义推理视角来指导大语言模型的推理过程，用于韩语不当言论检测，相比标准监督学习提升了约3.89%的准确率。


<details>
  <summary>Details</summary>
Motivation: 在线游戏和社区中匿名性导致不当言论经常升级为言语虐待甚至犯罪行为，需要检测技术来构建更安全的交流环境。虽然韩语大语言模型和思维链推理受到关注，但在不当言论检测方面的应用研究仍然有限。
...
Conclusion: 提出的软归纳偏置方法通过明确定义推理视角，能够有效提升韩语大语言模型在不当言论检测任务中的性能，证明了该方法在构建更安全交流环境方面的有效性。
...
TL;DR: 提出了一种分层多智能体架构，在64×64网格上分布轻量级智能体，通过选择性oracle支持，使用空间课程学习逐步扩展操作区域，结合NLL置信度测量和Thompson Sampling课程管理器，在空间化汉诺塔基准上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型和多智能体系统在分解复杂任务方面表现出潜力，但在长时程推理任务和计算成本方面存在困难。需要一种能够有效处理长时程推理、降低计算成本并提高稳定性的方法。

Method: 1. 分层多智能体架构：在64×64网格上分布轻量级智能体，由选择性oracle支持；2. 空间课程学习：逐步扩展网格的操作区域，让智能体先掌握中心简单任务再处理边缘困难任务；3. 置信度测量：集成负对数似然(NLL)作为置信度指标；4. Thompson Sampling课程管理器：基于能力和NLL驱动的奖励信号自适应选择训练区域。

Result: 在空间化汉诺塔基准测试中，该方法表现出：1. 改进的稳定性；2. 减少的oracle使用；3. 通过分布式智能体协作实现更强的长程推理能力。

Conclusion: 提出的分层多智能体架构结合空间课程学习和置信度测量，能够有效处理长时程推理任务，降低计算成本，提高系统稳定性，为机器人操作和规划等类似任务提供了有前景的解决方案。
...
TL;DR: 这是一篇关于医疗领域NLP应用的教程论文，重点介绍医疗NLP的现状、挑战和未来方向，包含数据层、NLP评估层和患者层三个层次，并提供实践环节。


<details>
  <summary>Details</summary>
Motivation: 现有医疗NLP综述存在不足：要么忽略重要任务（如隐私保护的合成数据生成、可解释性临床NLP），要么遗漏重要方法（如检索增强生成、神经符号集成）。因此需要提供更全面的医疗NLP入门教程。

Method: 采用三层层次结构组织教程内容：1) 数据/资源层（标注指南、伦理审批、治理、合成数据）；2) NLP评估层（NER、关系抽取、情感分析等任务，强调可解释性）；3) 患者层（患者参与、健康素养、翻译简化、决策支持）。包含实践环节。

Result: 构建了一个系统性的医疗NLP教程框架，涵盖从数据准备到患者应用的完整流程，特别关注隐私保护、可解释性和患者参与等关键问题。

Conclusion: 该教程为医疗NLP领域提供了全面的入门指南，特别强调以患者和资源为中心的方法，填补了现有综述的空白，有助于推动医疗NLP的实际应用和发展。
...
<details>
  <summary>Details</summary>
Motivation: 为了支持基于大型语言模型（LLMs）的虚拟调查和标注任务，提高LLM研究的可重复性和可靠性，同时降低计算成本。

Method: 开发了QSTN开源框架，包含系统化的问卷响应生成功能，支持问卷呈现、提示扰动和响应生成方法的评估，并提供无代码用户界面。

Result: 通过超过4000万次调查响应的评估，发现问卷结构和响应生成方法对LLM生成响应与人类答案的一致性有显著影响，且能以较低计算成本实现。

Conclusion: QSTN框架有助于提高LLM研究的可重复性和可靠性，支持研究人员无需编码知识即可进行稳健的实验。
...
TL;DR: 首个巴斯克语自动作文评分数据集，包含3200篇C1水平作文，使用开源模型微调，在评分一致性和反馈质量上超越GPT-5等闭源系统
...
Method: 构建包含3200篇C1水平巴斯克语作文的公开数据集，每篇由专家标注包含正确性、丰富性、连贯性、衔接性和任务对齐的评分及详细反馈；微调RoBERTa-EusCrawl和Latxa 8B/70B等开源模型用于评分和解释生成；提出结合自动一致性指标和专家验证的新型反馈生成评估方法

Result: 编码器模型在自动作文评分上保持高可靠性；监督微调的Latxa模型在评分一致性和反馈质量上显著超越GPT-5和Claude Sonnet 4.5等最先进闭源系统；微调后的Latxa模型能生成标准对齐、教学意义明确的反馈，并能识别比专有模型更广泛的错误类型

Conclusion: 该资源和基准为巴斯克语等低资源语言的透明、可复现且教育基础扎实的NLP研究奠定了基础，展示了开源模型在特定语言任务上超越闭源系统的潜力
...
TL;DR: 提出一种针对低资源语言的后训练方法，通过on-policy训练策略，在不依赖目标语言指令调优数据的情况下，保持语言模型的流畅性，即使奖励模型本身不流畅。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化研究主要集中在英语和中文等高资源语言，低资源语言缺乏母语者编写的数据集和生成流畅合成数据的语言模型，需要开发不依赖目标语言指令调优数据的流畅偏好对齐方法。

Method: 采用on-policy训练方法，与两种常见方法对比：基于机器翻译数据的监督微调和多语言微调。以挪威博克马尔语为案例研究，通过母语者评估流畅性。

Result: 结果显示on-policy训练方法至关重要，在不依赖难以获取的数据的情况下优于其他替代方法。

Conclusion: 提出的on-policy后训练方法能够有效解决低资源语言偏好对齐中的流畅性问题，无需目标语言指令调优数据，为低资源语言模型对齐提供了可行方案。
...
TL;DR: 本文提出在联邦学习环境中评估大型语言模型与多样化人类偏好对齐的方法，引入自适应聚合策略来平衡对齐质量和公平性
...
Method: 建立综合评估框架，在联邦学习环境中评估不同聚合策略；每个组本地评估生成奖励信号，服务器聚合组级奖励而不访问原始数据；评估标准聚合技术（最小、最大、平均）并引入基于历史对齐性能动态调整偏好权重的自适应方案
...
TL;DR: 论文研究了逐步增加Transformer深度训练方法的机制原理，发现这种方法能更有效地利用模型深度，改变残差流结构，并形成可置换计算块，从而克服标准非增长模型的深度利用限制。


<details>
  <summary>Details</summary>
Motivation: 虽然MIDAS研究表明逐步增加Transformer深度能降低训练成本并提升推理性能，但迄今为止对这些增益的机制理解仍然缺失。本文旨在建立与"深度诅咒"现象的联系，深入理解深度增长如何影响模型计算结构。

Method: 使用深度分析技术，研究通过渐进中间堆叠的增长方式如何影响模型深度利用效率。分析了残差流结构的变化和可置换计算块的形成机制，并提出MIDAS的轻量级改进方案。

Result: 研究表明，深度增长能更有效地利用模型深度，改变残差流结构，促进可置换计算块的形成。提出的轻量级改进方案在下游推理基准测试中获得了进一步性能提升。

Conclusion: 逐步增加模型深度能形成独特的计算电路，克服标准非增长模型中观察到的深度利用限制，为理解深度增长训练方法的机制提供了重要见解。
...
TL;DR: RAGLens：基于稀疏自编码器从LLM内部激活中提取特征，构建轻量级RAG幻觉检测器，无需大规模标注数据或外部LLM查询，性能优于现有方法并提供可解释性。


<details>
  <summary>Details</summary>
Motivation: RAG虽然通过检索证据提高了LLM的事实性，但生成内容与源材料矛盾或超出范围的忠实性失败仍是关键挑战。现有幻觉检测方法要么需要大量标注数据进行大规模检测器训练，要么依赖外部LLM法官导致高推理成本，而基于LLM内部表示的方法准确率有限。

Method: 利用稀疏自编码器解耦LLM内部激活，识别RAG幻觉期间触发的特定特征。基于信息特征选择和加性特征建模的系统化流程，构建RAGLens轻量级幻觉检测器，使用LLM内部表示准确标记不忠实的RAG输出。

Result: RAGLens不仅实现了优于现有方法的检测性能，还为其决策提供可解释的理据，能够有效事后缓解不忠实的RAG输出。研究还揭示了幻觉相关信号在LLM中的分布新见解。

Conclusion: 通过机制可解释性方法，RAGLens成功构建了基于LLM内部表示的轻量级RAG幻觉检测器，在准确性和可解释性方面均优于现有方法，为RAG系统的忠实性监控提供了有效解决方案。
...
### [102] [ThreadWeaver: Adaptive Threading for Efficient Parallel Reasoning in Language Models](https://arxiv.org/abs/2512.07843)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM的顺序解码导致推理延迟高，现有并行推理方法要么局限于监督行为克隆，要么准确率显著下降，且需要定制化推理引擎，部署复杂。

Method: 1) 两阶段并行轨迹生成器产生大规模高质量并行标注的CoT数据；2) 基于trie的训练-推理协同设计，无需修改位置嵌入或KV缓存即可在现有自回归推理引擎上实现并行推理；3) 并行感知的强化学习框架，平衡准确率与并行效率。

Result: 在六个数学推理基准测试中，基于Qwen3-8B的ThreadWeaver达到与最先进顺序推理模型相当的准确率（平均71.9%，AIME24上79.9%），同时实现最高1.53倍的平均token延迟加速。

Conclusion: ThreadWeaver在准确率与效率之间建立了新的帕累托前沿，为LLM推理提供了高效并行解决方案，无需修改现有推理引擎即可部署。
...
### [103] [Space Alignment Matters: The Missing Piece for Inducing Neural Collapse in Long-Tailed Learning](https://arxiv.org/abs/2512.07844)
*Jinping Wang,Zhiqiang Gao,Zhiwu Xie*

Main category: cs.LG

TL;DR: 本文针对长尾分布中特征与分类器权重空间不对齐的问题，提出三种即插即用的对齐策略，显著提升了现有长尾方法的性能。


<details>
  <summary>Details</summary>
Motivation: 在长尾分布下，样本严重不平衡阻碍了神经坍缩现象的出现，导致泛化性能差。现有方法主要关注恢复ETF几何结构，但忽视了特征空间与分类器权重空间之间的严重不对齐问题。

Method: 通过最优错误指数分析理论量化不对齐的危害，提出三种明确的即插即用对齐策略，无需改变现有长尾方法的架构。

Result: 在CIFAR-10-LT、CIFAR-100-LT和ImageNet-LT数据集上的大量实验表明，所提策略能持续提升基线方法性能，并达到最先进的性能水平。

Conclusion: 特征与分类器权重空间的对齐对长尾学习至关重要，提出的三种即插即用策略能有效解决这一问题，显著提升模型在长尾分布下的泛化能力。
...
### [104] [RaX-Crash: A Resource Efficient and Explainable Small Model Pipeline with an Application to City Scale Injury Severity Prediction](https://arxiv.org/abs/2512.07848)
...
<details>
  <summary>Details</summary>
Motivation: 纽约市每年报告超过十万起机动车碰撞事故，造成严重的伤害和公共卫生负担，需要资源高效且可解释的预测模型来分析事故严重程度。

Method: 整合三个包含数千万条记录的链接表，构建统一特征模式的分区存储，训练基于树的集成模型（随机森林和XGBoost），并与使用文本摘要提示的小型语言模型进行比较。

Result: XGBoost和随机森林在时间保留测试集上分别达到0.7828和0.7794的准确率，明显优于小型语言模型（0.594和0.496）；类别不平衡分析显示简单类别加权可提高致命事故召回率，SHAP归因突出人类脆弱因素、时间和位置是预测严重程度的主要驱动因素。
...
### [105] [SABER: Small Actions, Big Errors - Safeguarding Mutating Steps in LLM Agents](https://arxiv.org/abs/2512.07850)
*Alejandro Cuadron,Pengfei Yu,Yang Liu,Arpit Gupta*

Main category: cs.LG

TL;DR: 论文研究发现LLM智能体在长时程工具使用任务中，突变动作的早期偏差会显著降低成功率（高达96%），而非突变动作偏差影响很小。作者提出了CM方法，通过突变门控验证、针对性反思和块级上下文清理来提升性能，并发布了τ-Bench Verified以解决基准测试中的天花板效应问题。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM智能体发展迅速，但在长时程工具使用任务上的表现仍然脆弱。为了理解这种脆弱性，研究者探讨了一个核心问题：所有动作对失败的影响是否相同？他们希望识别导致任务失败的关键因素，并开发相应的防护机制。

Method: 研究分析了τ-Bench（航空/零售）和SWE-Bench Verified上的执行轨迹，将轨迹分解为突变（环境改变）和非突变步骤，并形式化了"决定性偏差"概念。通过逻辑回归分析偏差影响，提出了CM方法：1）添加突变门控验证；2）在突变步骤前注入针对性反思；3）执行基于块的上下文清理。此外，还发布了τ-Bench Verified以解决基准测试中的天花板效应。

Result: 研究发现每个额外的突变动作偏差会显著降低成功率：在航空任务上降低92%，在零售任务上降低96%。而非突变动作偏差几乎没有影响。CM方法带来了显著性能提升：Qwen3-Thinking在航空任务上相对提升28%，零售任务提升11%，SWE-Bench Verified提升7%；Claude模型也有9%/7%的提升。

Conclusion: 研究结果表明，动作级别的分析、针对性防护机制和可靠的评估是构建鲁棒多轮智能体的先决条件。突变动作中的早期偏差是导致失败的关键因素，而CM方法能有效缓解这一问题。同时，基准测试的质量对准确评估模型性能至关重要。
...
### [106] [GPU Memory Prediction for Multimodal Model Training](https://arxiv.org/abs/2512.07853)
...
<details>
  <summary>Details</summary>
Motivation: 随着智能体AI系统中深度学习模型规模和复杂度的增加，GPU内存需求经常超过可用容量，导致内存溢出错误。现有研究仅关注单模态架构，无法推广到多模态模型，而多模态模型在智能体AI系统中是常见选择。

Method: 提出一个框架，通过将多模态模型分解为组成层，并应用因子化方法来估计每层的内存使用量，分析模型架构和训练行为来预测峰值GPU内存使用。

Result: 评估显示该框架实现了约8.7%的平均MAPE（平均绝对百分比误差）预测精度，具有较高的预测准确性。
...
### [107] [LAPA: Log-Domain Prediction-Driven Dynamic Sparsity Accelerator for Transformer Model](https://arxiv.org/abs/2512.07855)
*Huizheng Wang,Hongbin Wang,Shaojun Wei,Yang Hu,Shouyi Yin*

Main category: cs.LG

TL;DR: LAPA是一种基于对数域注意力预测的算法-架构协同设计，通过消除昂贵乘法运算和减少累积开销，实现跨阶段稀疏Transformer加速，能效比提升2.79-3.52倍。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在不同阶段的计算瓶颈会随输入序列长度变化而动态变化，需要跨阶段稀疏加速策略。现有稀疏Transformer方法多为单阶段设计，其稀疏预测机制在多阶段应用时会产生显著功耗开销。

Method: 提出LAPA对数域注意力预测算法-架构协同设计：1）设计非对称前导一计算方案消除昂贵乘法；2）提出混合精度多轮移位累积机制减少累积开销；3）设计数据特征依赖滤波器与MRSA协同工作；4）设计专用加速器将理论改进转化为实际硬件提升。

Result: LAPA相比现有最优方法Spatten、Sanger和FACT分别实现了3.52倍、3.24倍和2.79倍的能量效率提升。

Conclusion: LAPA通过算法-架构协同设计有效解决了Transformer模型跨阶段稀疏加速的功耗问题，显著提升了能量效率，为动态计算瓶颈的Transformer加速提供了有效解决方案。
...
### [108] [SA^2GFM: Enhancing Robust Graph Foundation Models with Structure-Aware Semantic Augmentation](https://arxiv.org/abs/2512.07857)
*Junhua Shi,Qingyun Sun,Haonan Yuan,Xingcheng Fu*

Main category: cs.LG

TL;DR: SA^2GFM是一个鲁棒的图基础模型框架，通过结构感知语义增强来改进领域自适应表示，在节点和图分类任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前图基础模型在各种任务中取得了显著进展，但其对领域噪声、结构扰动和对抗攻击的鲁棒性尚未充分探索。关键限制在于对层次结构语义的建模不足，而这对泛化至关重要。

Method: 提出SA^2GFM框架：1）通过将基于熵的编码树转换为结构感知文本提示来编码层次结构先验；2）使用自监督信息瓶颈机制通过结构引导压缩提取鲁棒可迁移表示；3）引入专家自适应路由机制解决跨领域适应的负迁移问题；4）提出微调模块通过联合社区内和社区间结构学习优化层次结构。

Result: 大量实验表明，SA^2GFM在节点和图分类任务中，在对抗随机噪声和对抗扰动方面，在有效性和鲁棒性上都优于9个最先进的基线方法。

Conclusion: SA^2GFM通过结构感知语义增强有效提升了图基础模型的鲁棒性和领域自适应能力，为图基础模型的稳健应用提供了新思路。
...
### [109] [FAIM: Frequency-Aware Interactive Mamba for Time Series Classification](https://arxiv.org/abs/2512.07858)
...
Method: 提出FAIM模型，包含自适应滤波块（利用傅里叶变换提取频域特征，采用可学习自适应阈值动态抑制噪声）和交互式Mamba块（促进多粒度信息交互），并加入自监督预训练机制增强鲁棒性。

Result: 在多个基准测试上的广泛实验表明，FAIM始终优于现有的最先进方法，在准确性和效率之间实现了优越的平衡，并在各种领域和高噪声场景中表现出色。

Conclusion: FAIM通过频率感知和交互式设计有效解决了时间序列分类中的计算效率、噪声敏感性和过拟合问题，为TSC任务提供了强大且高效的解决方案。
...
### [110] [SetAD: Semi-Supervised Anomaly Learning in Contextual Sets](https://arxiv.org/abs/2512.07863)
*Jianling Gao,Chongyang Tao,Xuelian Lin,Junfeng Liu,Shuai Ma*

Main category: cs.LG

TL;DR: SetAD将半监督异常检测重构为集合级任务，通过注意力集合编码器和分级学习目标，利用集合组合的监督信号来检测群体层面的异常。


<details>
  <summary>Details</summary>
Motivation: 现有半监督异常检测方法主要关注单个点或简单对，忽略了异常的上下文性质（异常是相对于群体的偏离），也未能充分利用集合组合产生的丰富监督信号，难以捕捉数据中的高阶交互作用。

Method: 提出SetAD框架，将半监督异常检测重构为集合级异常检测任务；采用基于注意力的集合编码器，通过分级学习目标训练模型量化整个集合的异常程度；提出上下文校准的异常评分机制，通过聚合点在多个不同上下文集合中相对于同伴行为的归一化偏差来评估异常分数。

Result: 在10个真实世界数据集上的广泛实验表明，SetAD显著优于最先进的模型；模型性能随着集合大小的增加而持续提升，为基于集合的异常检测公式提供了强有力的实证支持。

Conclusion: SetAD通过集合级视角重新定义异常检测，能够更好地建模定义异常的复杂群体级交互，利用集合组合的监督信号，在性能上显著优于现有方法，且具有随着集合规模增大而提升的扩展性优势。
...
### [111] [Pattern Recognition of Ozone-Depleting Substance Exports in Global Trade Data](https://arxiv.org/abs/2512.07864)
*Muhammad Sukri Bin Ramli*

Main category: cs.LG

TL;DR: 本文提出了一种基于无监督机器学习的框架，用于分析海关数据以监测环境条约（如蒙特利尔议定书）的执行情况，通过聚类、异常检测和启发式标记识别可疑贸易模式。


<details>
  <summary>Details</summary>
Motivation: 需要新的方法来监测环境条约（如蒙特利尔议定书）的执行情况，通过审查大规模、复杂的海关数据集来发现可疑贸易活动。

Method: 使用无监督机器学习框架，结合K-Means聚类发现贸易原型，隔离森林和IQR进行异常检测识别"大宗交易"和价格异常，启发式标记发现模糊描述等策略，最后综合为优先级评分。

Result: 应用于10万条贸易记录，成功识别出1,351个价格异常值和1,288个高优先级货物供海关审查。发现高优先级商品的价值重量比与普通商品不同且更高。通过SHAP验证模糊描述和高价值是最重要的风险预测因子。

Conclusion: 该工作提供了一个可重复的无监督学习流程，将原始贸易数据转化为监管机构可用的优先级情报，成功检测到2021年初"大宗交易"激增与真实世界监管影响（美国AIM法案）的相关性。
...
### [112] [Using Text-Based Life Trajectories from Swedish Register Data to Predict Residential Mobility with Pretrained Transformers](https://arxiv.org/abs/2512.07865)
...
<details>
  <summary>Details</summary>
Motivation: 解决数据分析中长期存在的两个挑战：分类变量的高基数性和随时间变化的编码方案不一致性，利用瑞典全面的人口登记数据进行纵向预测分析

Method: 将690万个体（2001-2013年）的登记数据转化为语义丰富的文本化生命轨迹，结合人口统计信息和年度居住、工作、教育、收入、家庭状况变化，预测后期（2013-2017年）居住流动性，比较LSTM、DistilBERT、BERT和Qwen等多种NLP架构

Result: 序列化和基于transformer的模型比基线模型更有效地捕捉时间和语义结构，文本化的登记数据保留了关于个体路径的有意义信息，支持复杂、可扩展的建模

Conclusion: 将语义丰富的登记数据与现代语言模型相结合，可以显著推进社会科学中的纵向分析，为开发和评估新的序列建模方法提供严格测试平台
...
### [113] [Advancing physiological time series reconstruction and imputation via mixture of receptive fields and experts fusion](https://arxiv.org/abs/2512.07873)
*Ci Zhang,Huayu Li,Changdi Yang,Jiangnan Xia,Yanzhi Wang,Xiaolong Ma,Jin Lu,Geng Yuan*

Main category: cs.LG

TL;DR: 提出基于专家混合（MoE）的噪声估计器，用于医学时间序列信号的扩散模型重建，通过RFAMoE模块自适应选择感受野，并通过Fusion MoE模块实现单次推理的多噪声信号并行融合，显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 医学时间序列信号具有多变量、高时间变异性、高噪声和易受伪影影响等独特特性，使得基于深度学习的方法在插补等任务上仍然具有挑战性。现有扩散模型方法需要多次推理平均来减少重建误差，但计算开销巨大。

Method: 提出基于分数扩散框架的MoE噪声估计器：1）RFAMoE模块让每个通道在扩散过程中自适应选择所需感受野；2）Fusion MoE模块并行生成K个噪声信号，通过路由机制融合，实现单次推理完成信号重建。

Result: 实验结果表明，该框架在不同任务和数据集上始终优于基于扩散模型的SOTA方法，不仅提高了性能，还消除了多次推理过程带来的巨大计算成本和延迟。

Conclusion: 提出的MoE-based噪声估计器框架有效解决了医学时间序列信号重建的挑战，通过自适应感受野选择和并行融合机制，在保持高性能的同时大幅降低计算开销，为医学时间序列分析提供了高效解决方案。
...
### [114] [Controllable risk scenario generation from human crash data for autonomous vehicle testing](https://arxiv.org/abs/2512.07874)
*Qiujing Lu,Xuanhan Wang,Runze Yuan,Wei Lu,Xinyi Gong,Shuo Feng*

Main category: cs.LG

TL;DR: CRAG框架统一建模自动驾驶车辆测试中的正常驾驶行为和罕见安全关键行为，通过结构化潜在空间分离正常与风险行为，结合优化机制实现从安全到风险状态的平滑过渡，提升场景多样性和AV鲁棒性评估效率。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆安全测试需要在日常驾驶和罕见安全关键条件下进行严格测试，但现有方法难以同时模拟真实交通中的正常行为和符合真实事故的风险行为，特别是如何有效利用有限的碰撞数据。

Method: CRAG构建结构化潜在空间，分离正常行为和风险相关行为，结合风险感知潜在表示和基于优化的模式转换机制，使智能体能够在长时间范围内从安全状态平滑、合理地过渡到风险状态。

Result: 实验表明CRAG相比现有基线提高了场景多样性，同时支持可控的风险场景生成，能够进行有针对性和高效的自动驾驶鲁棒性评估。

Conclusion: CRAG框架成功统一了正常驾驶行为和罕见安全关键行为的建模，通过结构化潜在空间和优化转换机制，有效解决了自动驾驶安全测试中场景多样性和风险行为可控生成的挑战。
...
### [115] [Softly Symbolifying Kolmogorov-Arnold Networks](https://arxiv.org/abs/2512.07875)
*James Bagrow,Josh Bongard*

Main category: cs.LG

TL;DR: S2KAN提出了一种将符号基元直接集成到KAN训练中的方法，通过可学习的门控机制稀疏化表示，在符号项足够时发现可解释形式，不足时优雅地退回到密集样条。


<details>
  <summary>Details</summary>
Motivation: 传统KANs虽然提供了可解释机器学习的路径，但训练后的激活函数往往缺乏符号保真度，学习到没有有意义的可解释形式对应的病态分解。需要一种能够自动发现符号形式的方法。

Method: 提出Softly Symbolified KANs (S2KAN)，将符号基元直接集成到训练中。每个激活函数从符号项和密集项的字典中抽取，通过可学习的门控稀疏化表示。这种稀疏化是可微分的，支持端到端优化，并受最小描述长度目标指导。

Result: 在符号基准测试、动力学系统预测和真实世界预测任务中，S2KAN展示了竞争性或更优的准确性，同时使用显著更小的模型。观察到即使没有正则化压力，也出现了自稀疏化的证据。

Conclusion: S2KAN成功地将符号表示集成到KAN框架中，能够在符号项足够时发现可解释形式，在不足时优雅地退回到密集样条，为可解释机器学习提供了有前景的方法。
...
### [116] [Balanced Accuracy: The Right Metric for Evaluating LLM Judges - Explained through Youden's J statistic](https://arxiv.org/abs/2512.08121)
*Stephane Collot,Colin Fraser,Justin Zhao,William F. Shen,Timon Willi,Ilias Leontiadis*

Main category: cs.LG

TL;DR: 论文提出在评估大语言模型时，使用Youden's J统计量或平衡准确率来选择最佳分类器（评估者），因为这些指标对类别不平衡不敏感，能提供更可靠的模型比较结果。


<details>
  <summary>Details</summary>
Motivation: 当前评估大语言模型时，通常使用准确率、精确率、F1等指标来选择分类器（如LLM作为评估者或人工标注者），但这些指标对类别不平衡和正类选择敏感，可能导致选择出的分类器扭曲模型行为的流行率估计，影响模型比较的可信度。

Method: 论文提出使用Youden's J统计量作为选择最佳分类器的理论依据，证明平衡准确率是J的线性变换等价形式。通过理论分析和实证模拟，展示了基于平衡准确率选择分类器的优势。

Result: 研究表明，Youden's J统计量和平衡准确率对类别不平衡不敏感，能更稳健地选择分类器，从而提供更准确的大语言模型行为流行率估计，改善模型比较的可信度。

Conclusion: 在评估大语言模型时，应使用Youden's J统计量或平衡准确率来选择分类器，这些指标能提供更可靠、更稳健的模型比较结果，避免传统指标因类别不平衡和正类选择带来的偏差。
...
### [117] [Graph Contrastive Learning via Spectral Graph Alignment](https://arxiv.org/abs/2512.07878)
*Manh Nguyen,Joshua Cape*

Main category: cs.LG

TL;DR: 提出SpecMatch-CL损失函数，通过最小化图嵌入的归一化拉普拉斯矩阵差异来对齐视图特定的图-图结构，在无监督、半监督和迁移学习任务上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有对比学习方法（如InfoNCE）在优化图嵌入的跨视图对齐时，缺乏对视图特定图-图结构全局结构的控制机制，需要一种能够更好地对齐图结构的方法。

Method: 提出SpecMatch-CL损失函数，通过最小化视图特定图-图的归一化拉普拉斯矩阵之间的差异来对齐图结构。理论分析表明，在特定假设下，归一化拉普拉斯矩阵的差异为理想完美对齐对比损失与当前损失之间的差异提供了上界。

Result: 在8个TU基准测试中，无监督学习和低标签率半监督学习方面建立了新的SOTA；在PPI-306K和ZINC 2M数据集的迁移学习任务中取得一致性能提升。

Conclusion: SpecMatch-CL通过对齐图结构的归一化拉普拉斯矩阵，有效改进了图对比学习，在多种学习场景下均表现出优越性能。
...
### [118] [Nonnegative Matrix Factorization through Cone Collapse](https://arxiv.org/abs/2512.07879)
*Manh Nguyen,Daniel Pimentel-Alarcón*

Main category: cs.LG

TL;DR: 论文提出Cone Collapse算法，从几何视角重新审视非负矩阵分解(NMF)，通过迭代收缩非负象限来恢复数据的最小生成锥，并在此基础上构建锥感知正交NMF模型(CC-NMF)，在聚类任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有NMF算法主要从优化角度出发，没有充分利用NMF诱导的锥几何特性：数据点位于凸锥中，其极端射线编码了基本方向或"主题"。本文从几何视角重新审视NMF，旨在开发能显式恢复数据锥的理论基础和实证性能更强的聚类方法。

Method: 提出Cone Collapse算法：从完整的非负象限开始，迭代收缩它朝向数据生成的最小锥。证明在温和的数据假设下，该算法在有限步内终止并恢复X^⊤的最小生成锥。在此基础上，通过将单正交NMF应用于恢复的极端射线，推导出锥感知正交NMF模型(CC-NMF)。

Result: 在16个基准基因表达、文本和图像数据集上，CC-NMF在聚类纯度方面一致匹配或优于强NMF基线方法，包括乘法更新、ANLS、投影NMF、ONMF和稀疏NMF。结果表明显式恢复数据锥可以产生理论基础扎实且实证性能强的基于NMF的聚类方法。

Conclusion: 从几何视角重新审视NMF，提出Cone Collapse算法和CC-NMF模型，显式利用数据的锥几何结构，在聚类任务中取得了理论和实证上的优势，为NMF-based聚类方法提供了新的方向。
...
### [119] [Semi-Supervised Contrastive Learning with Orthonormal Prototypes](https://arxiv.org/abs/2512.07880)
*Huanran Li,Manh Nguyen,Daniel Pimentel-Alarcón*

Main category: cs.LG

TL;DR: CLOP是一种新的半监督对比学习损失函数，通过促进类别嵌入形成正交线性子空间来防止维度坍缩，在图像分类和目标检测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 对比学习在深度学习中是强大的表示学习方法，但在半监督和自监督设置中面临维度坍缩的挑战，即嵌入收敛到低维空间。本文首先识别了导致标准对比损失收敛到坍缩解的关键学习率阈值。

Method: 基于对维度坍缩机制的理解，提出了CLOP（Contrastive Learning with Orthogonal Projections）损失函数，通过促进类别嵌入形成正交线性子空间来防止维度坍缩。
...
### [120] [GSPN-2: Efficient Parallel Sequence Modeling](https://arxiv.org/abs/2512.07884)
...
TL;DR: GSPN-2通过算法-系统联合重新设计，解决了原GSPN在GPU实现中的效率瓶颈，包括内核启动开销、内存传输和冗余计算，同时引入紧凑通道传播策略，在保持Transformer精度的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有GSPN虽然通过线扫描传播方案将计算复杂度降低到接近线性，但在GPU实现上仍存在效率瓶颈：重复启动GPU内核的开销、过多的全局内存数据传输、以及为每个通道维护独立传播权重导致的冗余计算。

Method: 1. 系统优化：将数千个微内核启动合并为单个2D内核，将每个warp固定到通道切片，将前一列的激活暂存到共享内存；2. 算法改进：引入紧凑通道传播策略，替代每通道矩阵，减少参数，并与Transformer注意力中的亲和力图自然对齐。

Result: GSPN-2在图像分类和文本到图像合成任务中表现出色，匹配Transformer级别的准确性，同时显著降低计算成本。实验证明其有效性，为视觉应用中的全局空间上下文建模建立了新的效率前沿。

Conclusion: GSPN-2通过结构化矩阵变换和GPU优化实现的独特组合，为视觉应用中建模全局空间上下文建立了新的效率标准，在保持准确性的同时大幅提升了计算效率。
...
### [121] [Towards symbolic regression for interpretable clinical decision scores](https://arxiv.org/abs/2512.07961)
*Guilherme Seidyo Imai Aldeia,Joseph D. Romano,Fabricio Olivetti de Franca,Daniel S. Herman,William G. La Cava*

Main category: cs.LG

TL;DR: Brush是一种符号回归算法，结合决策树分割和非线性常数优化，用于开发数据驱动的临床风险评分系统，在保持高预测性能的同时生成更简单的可解释模型。


<details>
  <summary>Details</summary>
Motivation: 医疗决策经常使用结合风险方程和规则的算法，但传统符号回归难以建模这种决策过程。然而，符号回归具有推导数据驱动、可解释模型的潜力，因此需要开发能够整合规则逻辑的符号回归方法。

Method: Brush算法结合了决策树式的分割算法和非线性常数优化，允许将基于规则的逻辑无缝集成到符号回归和分类模型中。

Result: Brush在SRBench上实现了帕累托最优性能，成功复现了两个广泛使用的临床评分系统，实现了高准确率和可解释模型。与决策树、随机森林和其他符号回归方法相比，Brush实现了相当或更优的预测性能，同时生成更简单的模型。

Conclusion: Brush算法能够有效开发数据驱动的临床风险评分系统，在保持预测性能的同时提高模型的可解释性，为医疗决策支持提供了有前景的工具。
...
### [122] [CIP-Net: Continual Interpretable Prototype-based Network](https://arxiv.org/abs/2512.07981)
*Federico Di Valerio,Michela Proietti,Alessio Ragno,Roberto Capobianco*

Main category: cs.LG

TL;DR: CIP-Net是一个无需存储历史样本的自解释原型模型，用于持续学习，在任务和类增量设置中实现了最先进的性能，同时显著降低了内存开销。


<details>
  <summary>Details</summary>
Motivation: 持续学习面临灾难性遗忘的挑战，现有可解释AI方法大多使用事后解释或需要为每个新任务分配额外内存，导致可扩展性有限。需要一种既保持可解释性又无需存储历史样本的实用解决方案。

Method: 提出CIP-Net（Continual Interpretable Prototype Network），这是一个无需示例的自解释原型模型。它避免存储过去的示例，保持简单的架构，同时提供有用的解释和强大的性能。

Result: CIP-Net在任务增量和类增量设置中，相比之前的无示例和自解释方法，实现了最先进的性能，同时显著降低了内存相关开销。

Conclusion: CIP-Net为持续学习提供了一个实用且可解释的解决方案，通过自解释原型模型在保持高性能的同时避免了内存负担，解决了可扩展性问题。
...
### [123] [Bridging the Clinical Expertise Gap: Development of a Web-Based Platform for Accessible Time Series Forecasting and Analysis](https://arxiv.org/abs/2512.07992)
...
Method: 开发了一个Web平台，支持用户上传数据、生成可视化图表、训练多种可定制化的预测模型，并集成大语言模型提供参数推荐和结果解释功能

Result: 创建了一个功能完整的平台，使非技术用户能够进行复杂的时间序列分析，支持多种预测模型和训练技术，并提供智能化的参数推荐和结果解释

Conclusion: 该平台成功降低了时间序列预测的技术门槛，旨在整合到学习型医疗系统中，实现临床数据管道的持续收集和推理，促进医疗研究的可及性
...
### [124] [Benchmarking Offline Multi-Objective Reinforcement Learning in Critical Care](https://arxiv.org/abs/2512.08012)
*Aryaman Bansal,Divya Sharma*

Main category: cs.LG

TL;DR: 本文在ICU重症监护场景中，比较了离线多目标强化学习算法与单目标基线方法，发现PEDA DT算法在灵活性和性能上表现最优，为个性化医疗决策提供了有前景的框架。


<details>
  <summary>Details</summary>
Motivation: 在ICU等重症监护环境中，临床医生面临平衡患者生存率和资源利用（如住院时间）的复杂挑战。单目标强化学习方法通常通过优化固定的标量化奖励函数来解决这一问题，但这种方法产生僵化的策略，无法适应不同的临床优先级。

Method: 本文在MIMIC-IV数据集上对三种离线多目标强化学习算法（Conditioned Conservative Pareto Q-Learning、Adaptive CPQL和修改版PEDA Decision Transformer）与三种标量化单目标基线方法（BC、CQL和DDQN）进行了基准测试。使用离线策略评估（OPE）指标进行性能比较。

Result: PEDA DT算法相比静态标量化基线方法展现出更优的灵活性。结果还扩展了先前关于单目标决策变换器在医疗领域的研究发现，确认序列建模架构在扩展到多目标条件生成时仍保持稳健和有效。
...
TL;DR: CLARITY是一个医学世界模型，通过在结构化潜在空间中直接预测疾病演化，整合时间间隔和患者特定数据，生成个体化治疗计划，并引入预测到决策的框架。
...
Result: 在MU-Glioma-Post数据集上，CLARITY在治疗规划方面表现出最先进的性能，比最近的MeWM方法高出12%，并显著超越所有其他医学特定的大型语言模型。

Conclusion: CLARITY通过引入一个医学世界模型，解决了现有方法在预测动态疾病演化方面的局限性，通过整合时间和临床背景，并建立预测到决策的框架，为肿瘤学临床决策提供了更有效的工具。
...
### [126] [LUNA: Linear Universal Neural Attention with Generalization Guarantees](https://arxiv.org/abs/2512.08061)
*Ashkan Shahbazi,Ping He,Ali Abbasi,Yikun Bai,Xinran Liu,Elaheh Akbari,Darian Salehi,Navid NaderiAlizadeh,Soheil Kolouri*

Main category: cs.LG

TL;DR: LUNA是一种可学习的核化线性注意力机制，通过参数化核特征映射实现线性计算复杂度，同时保持甚至超越二次注意力机制的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统softmax注意力机制存在O(n²)的二次计算成本，限制了其在长序列领域的应用。现有的线性注意力机制虽然将成本降低到O(n)，但依赖固定的随机特征映射，导致模型准确性和计算效率之间存在根本性权衡。

Method: LUNA的核心思想是学习核特征映射而非固定使用先验特征。通过参数化核函数，LUNA学习针对特定数据和任务的特征基，克服固定特征方法的表达能力限制。该方法实现了一个可学习的特征映射，该映射诱导出正定核并支持流式形式，从而在序列长度上实现线性的时间和内存扩展。

Result: 在Long Range Arena (LRA)基准测试中，LUNA在计算对等条件下（相同参数量、训练步数和近似FLOPs）实现了高效Transformer中的最先进平均准确率。此外，在BERT和ViT-B/16检查点中替换softmax并进行简短微调后，LUNA能够恢复大部分原始性能，显著优于固定线性化方法。

Conclusion: LUNA成功消除了线性注意力机制中准确性与计算效率之间的权衡，通过可学习的核特征映射实现了线性计算成本，同时达到甚至超越了二次注意力机制的准确性，为长序列处理提供了有效的解决方案。
...
### [127] [Deep Kernel Aalen-Johansen Estimator: An Interpretable and Flexible Neural Net Framework for Competing Risks](https://arxiv.org/abs/2512.08063)
*Xiaobin Shen,George H. Chen*

Main category: cs.LG

TL;DR: 提出Deep Kernel Aalen-Johansen (DKAJ)模型，将经典Aalen-Johansen估计器推广到深度学习框架，通过自动学习的核函数衡量数据点相似性，在保持竞争风险预测性能的同时提供可解释性可视化。


<details>
  <summary>Details</summary>
Motivation: 竞争风险分析中需要既准确又可解释的模型。经典Aalen-Johansen估计器虽然可解释但缺乏灵活性，而现有深度学习方法虽然灵活但缺乏可解释性。需要一种能结合两者优点的模型。

Method: 提出DKAJ模型，将每个数据点表示为多个簇的加权组合。通过自动学习的核函数计算数据点间的相似度权重。当数据点仅对一个簇有非零权重时，其预测CIFs对应经典Aalen-Johansen估计器在该簇上的结果。

Result: 在四个标准竞争风险数据集上测试，DKAJ与最先进基线方法性能相当，同时能够提供可视化辅助模型解释，实现了准确性与可解释性的平衡。

Conclusion: DKAJ成功将经典Aalen-Johansen估计器推广到深度学习框架，在保持预测准确性的同时提供了模型可解释性，为竞争风险分析提供了新的工具。
...
### [128] [CAMO: Causality-Guided Adversarial Multimodal Domain Generalization for Crisis Classification](https://arxiv.org/abs/2512.08071)
*Pingchuan Ma,Chengshuai Zhao,Bohan Jiang,Saketh Vishnubhatla,Ujun Jeong,Alimohammad Beigi,Adrienne Raglin,Huan Liu*

Main category: cs.LG

TL;DR: 提出因果引导的多模态领域泛化框架，通过对抗解耦和统一表示学习，提升社交媒体危机分类在未见灾难类型上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有社交媒体危机分类方法主要依赖深度学习融合文本和视觉线索，在领域内设置下取得数值上合理的结果，但在未见危机类型上泛化能力差。主要问题：1. 未能解耦虚假特征和因果特征，导致领域偏移下性能下降；2. 未能对齐异构模态表示，阻碍单模态领域泛化技术直接应用于多模态设置。

Method: 提出因果引导的多模态领域泛化框架，结合对抗解耦与统一表示学习。对抗目标鼓励模型解耦并专注于领域不变的因果特征，实现基于稳定因果机制的可泛化分类。统一表示将不同模态特征对齐到共享潜在空间，使单模态领域泛化策略能无缝扩展到多模态学习。

Result: 在不同数据集上的实验表明，该方法在未见灾难场景中取得了最佳性能。

Conclusion: 通过因果引导的多模态领域泛化框架，有效解决了社交媒体危机分类在未见灾难类型上的泛化挑战，结合对抗解耦和统一表示学习，实现了更稳健和可泛化的分类性能。
...
TL;DR: MAC提出了一种基于模型的离线强化学习方法，通过动作块模型减少长期预测误差，结合拒绝采样防止模型利用，在复杂长时程任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决基于模型的强化学习在离线设置下处理复杂长时程任务时的挑战，特别是长期价值自举中的偏差与模型误差累积之间的权衡问题。

Method: 1. 动作块模型：预测动作序列（动作块）后的未来状态，减少复合误差；2. 拒绝采样：从表达性行为动作块策略中采样，防止模型利用和分布外动作问题。

Result: 在包含高达1亿个转移的大规模数据集上的实验表明，MAC在离线基于模型的强化学习算法中表现最佳，尤其在具有挑战性的长时程任务上。

Conclusion: MAC通过动作块模型和拒绝采样的组合，为基于模型的强化学习提供了可扩展的解决方案，能够有效处理离线设置下的复杂长时程任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有后门检测方法依赖目标类表现出极端异常检测统计量，但在两类情况下会失效：1)某些非目标类本身就容易与其他类区分，自然获得极端统计量；2)后门特征相对内在特征很微弱时。关键观察是：后门目标类的检测统计量来自后门触发器和内在特征，而非目标类仅来自内在特征。

Method: 提出类子空间正交化(CSO)方法，通过抑制内在特征来优化检测统计量。具体使用少量干净样本，在优化检测统计量的同时，与类的内在特征进行正交化处理。这减少了非目标类的可达到统计量，而目标类中后门触发器的贡献仍然保留。

Result: CSO方法在具有挑战性的混合标签攻击和自适应攻击中表现出良好的检测性能，相比现有方法对隐蔽后门攻击更加敏感。

Conclusion: 通过抑制内在特征来增强后门检测统计量的方法有效提高了检测敏感性，特别是在处理易区分类别和隐蔽后门攻击时，CSO方法展现出优越性能。
...
TL;DR: 本文提出了一种新颖的生物威胁基准生成框架，专门用于评估AI模型（特别是大语言模型）在生物安全风险方面的潜在危害，重点关注细菌生物威胁。
...
Result: 提出了细菌生物威胁模式作为BBG框架的首个组成部分，为后续将查询转化为模型提示以及实施模型评估奠定了基础。该框架旨在提供可重复使用的评估结构。

Conclusion: BBG框架（包括细菌生物威胁模式）旨在为评估大语言模型在细菌生物风险方面提供稳健、可重复使用的评估结构，全面涵盖技术和操作要求，并考虑广泛的生物对手能力范围。
...
### [132] [Robust Agents in Open-Ended Worlds](https://arxiv.org/abs/2512.08139)
*Mikayel Samvelyan*

Main category: cs.LG

TL;DR: 该论文提出了一系列方法来增强AI智能体在不同环境中的鲁棒性，包括使用MiniHack框架创建多样化环境、Maestro方法生成对抗性课程、质量多样性方法识别多智能体系统中的漏洞，以及进化搜索增强LLM对抗对抗性提示的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随着AI在各种应用中的普及，需要能够成功导航和适应不断变化的开放世界的智能体。关键挑战是确保这些AI智能体不仅能在训练期间观察到的熟悉环境中表现出色，还能有效地泛化到以前未见过的多样化场景。

Method: 1. 引入MiniHack框架，基于NetHack游戏通过程序化内容生成创建多样化环境；2. 提出Maestro方法，为两人零和游戏生成对抗性课程以增强RL智能体的鲁棒性；3. 使用质量多样性方法在足球游戏领域系统识别预训练RL策略的漏洞；4. 应用进化搜索生成多样化有效输入，以增强LLM对抗对抗性提示的鲁棒性。

Result: 该研究开发了一系列工具和方法来增强AI智能体在不同领域的鲁棒性：MiniHack为RL智能体提供了泛化测试的沙盒环境；Maestro方法能够渐进增强智能体在对抗性游戏中的鲁棒性；质量多样性方法成功识别了复杂多智能体系统中的策略漏洞；进化搜索方法有效增强了LLM对抗对抗性提示的能力。

Conclusion: 这项工作为AI鲁棒性的未来发展铺平了道路，使智能体不仅能够适应不断变化的世界，还能在面对不可预见的挑战和交互时茁壮成长。通过结合开放式方法和多智能体学习，该研究为训练和评估能够在新颖环境、分布外输入以及与其他智能体交互中有效泛化的鲁棒AI智能体提供了系统框架。
...
### [133] [PolyLingua: Margin-based Inter-class Transformer for Robust Cross-domain Language Detection](https://arxiv.org/abs/2512.08143)
*Ali Lotfi Rezaabad,Bikram Khanal,Shashwat Chaurasia,Lu Zeng,Dezhi Hong,Hossein Beshashati,Thomas Butler,Megan Ganji*

Main category: cs.LG

TL;DR: PolyLingua是一个轻量级Transformer模型，用于领域内语言检测和细粒度语言分类，在计算和延迟受限的环境中超越大型模型性能。


<details>
  <summary>Details</summary>
Motivation: 语言识别是多语言系统的关键第一步，现有工具在特定场景（如音乐请求）中表现不佳，开源工具准确率低，而大型语言模型成本过高，不适合低延迟或低资源环境。

Method: 采用轻量级Transformer架构，结合两级对比学习框架：实例级分离和类级对齐，使用自适应边距，为相近语言生成紧凑且分离良好的嵌入表示。

Result: 在Amazon Massive数据集上达到99.25% F1分数，在Song数据集上达到98.15% F1分数，超越Sonnet 3.5，同时参数量减少10倍。

Conclusion: PolyLingua在保持高准确率的同时显著降低了计算成本和延迟，特别适合计算资源和延迟受限的环境，解决了现有语言识别工具在特定场景下的不足。
...
### [134] [TreeGRPO: Tree-Advantage GRPO for Online RL Post-Training of Diffusion Models](https://arxiv.org/abs/2512.08153)
...
Result: 在扩散模型和基于流的模型上的实验表明，TreeGRPO实现了2.4倍的训练加速，并在效率-奖励权衡空间中建立了更优的帕累托前沿。该方法在多个基准测试和奖励模型上始终优于GRPO基线。

Conclusion: TreeGRPO为基于强化学习的视觉生成模型对齐提供了一条可扩展且有效的途径，通过树结构方法显著提高了训练效率，解决了传统RL后训练的计算瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 随着高质量公开数据接近枯竭，移动设备上的微调可以利用私有用户数据同时保护隐私。现有方法主要是模拟或依赖IoT设备和PC，普通手机领域尚未充分探索，缺乏开源框架支持。

Method: 提出了MobileFineTuner统一开源框架，支持全参数微调和参数高效微调。针对手机内存和能耗限制，引入了参数分片、梯度累积和能量感知计算调度等系统级优化。

Result: 在真实手机上成功微调了GPT-2、Gemma 3和Qwen 2.5模型。大量实验和消融研究验证了所提优化的有效性，证明MobileFineTuner是未来设备端LLM训练的可行基础。

Conclusion: MobileFineTuner填补了移动设备上大语言模型微调的开源框架空白，为利用私有数据同时保护隐私提供了实用解决方案，为未来设备端LLM训练研究奠定了基础。
...
TL;DR: 该论文挑战了传统AdamW中权重衰减与学习率γ成比例的假设，提出权重衰减应与γ²成比例，基于稳态时权重范数稳定的理论推导，并验证了这种设置能改善训练动态和模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统AdamW优化器中，解耦权重衰减（decoupled weight decay）一直被设置为与学习率γ成比例，但这一假设从未被质疑。最近有研究者基于稳态正交性论证提出权重衰减应与γ²成比例，本文旨在深入探讨这一争议并建立更合理的理论框架。

Method: 作者首先分析了消除更新垂直分量对权重范数贡献的影响，发现对训练动态影响不大。然后基于"稳态时更新与权重无关"的简单假设，推导出解耦权重衰减应与γ²成比例才能保持权重范数稳定。基于同一假设，推导并经验验证了Scion优化器中minibatch的总更新贡献（TUC）更好地由动量相关的有效学习率表征。

Result: 理论推导和经验验证表明：1）解耦权重衰减∝γ²能保持权重和梯度范数稳定；2）这种设置能更好地控制训练动态；3）改进的训练动态最终提升了模型性能；4）Scion优化器中TUC的最佳值具有可迁移性。

Conclusion: 传统AdamW中权重衰减与学习率γ成比例的假设存在缺陷，应改为与γ²成比例。这种修正基于稳态时权重范数稳定的理论推导，能提供更稳定的训练动态，改善模型性能，并为优化器设计提供更坚实的理论基础。
...
### [137] [SPROCKET: Extending ROCKET to Distance-Based Time-Series Transformations With Prototypes](https://arxiv.org/abs/2512.08246)
*Nicholas Harner*

Main category: cs.LG

TL;DR: SPROCKET是一种基于原型的时间序列分类特征工程方法，通过原型选择实现随机卷积核变换，在多数UCR和UEA数据集上表现优异，其集成模型MR-HY-SP超越了现有最佳卷积集成方法。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列分类算法主要依赖特征工程策略，其中ROCKET通过随机核特征取得了优异性能。研究者希望探索基于原型的特征工程策略，以进一步提升时间序列分类的准确性和鲁棒性。

Method: SPROCKET（Selected Prototype Random Convolutional Kernel Transform）采用基于原型的特征工程策略，通过原型选择实现随机卷积核变换。该方法可以与现有卷积算法结合，形成MR-HY-SP（MultiROCKET-HYDRA-SPROCKET）集成模型。

Result: 在大多数UCR和UEA时间序列分类基准数据集上，SPROCKET取得了与现有卷积算法相当的性能。MR-HY-SP集成模型的平均准确率排名超过了之前最佳的卷积集成方法HYDRA-MR。

Conclusion: 实验结果表明，基于原型的特征变换能够同时提升时间序列分类的准确性和鲁棒性，为时间序列分类提供了新的有效特征工程策略。
...
### [138] [Geometric-Stochastic Multimodal Deep Learning for Predictive Modeling of SUDEP and Stroke Vulnerability](https://arxiv.org/abs/2512.08257)
*Preksha Girish,Rachana Mysore,Mahanthesha U,Shrey Kumar,Misbah Fatimah Annigeri,Tanish Jain*

Main category: cs.LG

TL;DR: 提出了一种统一的几何-随机多模态深度学习框架，整合EEG、ECG、呼吸、SpO2、EMG和fMRI信号，用于建模癫痫猝死（SUDEP）和中风易感性。


<details>
  <summary>Details</summary>
Motivation: 癫痫猝死和急性缺血性中风是危及生命的疾病，涉及皮层、脑干和自主神经系统之间的复杂相互作用。现有方法缺乏统一的数学模型来整合多模态神经生理信号，无法提供可解释的生物标志物。

Method: 结合黎曼流形嵌入、李群不变特征表示、分数随机动力学、哈密顿能量流建模和跨模态注意力机制。使用分数流行病扩散在结构脑图上建模中风传播。

Result: 在MULTI-CLARID数据集上的实验显示预测准确性提高，并从流形曲率、分数记忆指数、注意力熵和扩散中心性中提取了可解释的生物标志物。
...
TL;DR: JARF（Jacobian-Aligned Random Forests）提出了一种简单有效的监督预条件处理方法，通过计算随机森林预测的梯度，构建全局线性预条件器来旋转特征空间，使轴对齐决策树能够处理旋转或交互依赖的决策边界问题。


<details>
  <summary>Details</summary>
Motivation: 轴对齐决策树虽然快速稳定，但在处理具有旋转或特征交互依赖的决策边界时效果不佳，因为其只能基于单个特征的阈值进行分割。倾斜森林虽然能解决这个问题，但计算成本高且实现复杂。

Method: 首先拟合轴对齐森林来估计类别概率或回归输出，计算这些预测相对于每个特征的有限差分梯度，将它们聚合成期望雅可比外积（EGOP的推广），并将其用作所有输入的单一全局线性预条件器。这个监督预条件器对特征空间应用单一全局旋转，然后将转换后的数据交给标准轴对齐森林处理。

Result: 在表格分类和回归基准测试中，这种预条件处理方法持续改进了轴对齐森林的性能，通常匹配甚至超过倾斜基线方法，同时提高了训练时间。实验结果表明监督预条件处理能够恢复倾斜森林的大部分准确性，同时保持轴对齐树的简单性和鲁棒性。

Conclusion: JARF通过监督预条件处理方法，在保持轴对齐决策树简单性和鲁棒性的同时，能够有效处理旋转或交互依赖的决策边界问题，为表格数据建模提供了一种高效实用的解决方案。
...
TL;DR: 提出一种名为MAN的联邦学习正则化技术，通过最小化激活范数来约束优化问题的平坦性，从而提高联邦学习模型的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中的聚合过程可能导致全局模型收敛到"尖锐最小值"，这会损害模型的泛化能力。因此需要改进联邦学习框架，使模型收敛到更平坦的最小值。

Method: 提出平坦性约束的联邦学习优化问题，通过约束训练损失Hessian矩阵的最大特征值来实现。进一步提出MAN正则化技术，在客户端侧最小化每层激活范数，理论上证明这能降低客户端损失函数的层间Hessian最大特征值。

Result: 将提出的平坦性约束优化应用于现有联邦学习技术，获得了显著改进，建立了新的最先进水平。

Conclusion: MAN正则化技术能有效提高联邦学习模型的泛化性能，通过约束平坦性使模型收敛到更优的最小值。
...
<details>
  <summary>Details</summary>
Motivation: 多标签数据集中标签通常不互斥且频率差异大，传统采样方法难以同时保证稀有标签的充分代表性、控制采样偏差，并考虑标签间的依赖关系。

Method: 使用多元伯努利分布作为多标签问题的底层分布，基于观测标签频率估计分布参数，计算每个标签组合的权重，通过加权采样获得具有目标分布特性的样本，同时考虑标签依赖关系。

Result: 将方法应用于Web of Science的64个生物医学主题类别标记的研究文章样本，成功保持了类别频率顺序，减少了最常见和最不常见类别之间的频率差异，并考虑了类别依赖关系，生成了更平衡的子样本。

Conclusion: 提出的采样算法能够有效处理多标签数据中的不平衡问题，通过考虑标签依赖关系生成更平衡的样本，增强了少数类别的代表性，为多标签数据分析提供了更好的基础。
...
TL;DR: 研究人员开发了细菌生物威胁基准（B3）数据集，用于评估大型语言模型在生物安全方面的风险，并通过试点研究验证了该基准的有效性。


<details>
  <summary>Details</summary>
Motivation: 前沿人工智能模型（特别是大型语言模型）可能被用于生物恐怖主义或获取生物武器，这引发了政策、学术和公众的广泛担忧。模型开发者和政策制定者需要量化并减轻这种风险，因此需要开发能够评估特定模型生物安全风险的基准测试。

Method: 本文是生物威胁基准生成（BBG）框架系列论文的第三篇，描述了B3数据集的试点实施。试点包括：1）在样本前沿AI模型上运行基准测试；2）对模型响应进行人工评估；3）从多个维度对结果进行应用风险分析。

Result: 试点研究表明，B3数据集提供了一种可行且细致的方法，能够快速评估LLM的生物安全风险，识别风险的主要来源，并为优先缓解领域提供指导。

Conclusion: B3数据集为评估大型语言模型的生物安全风险提供了一个有效的基准测试工具，有助于识别风险来源并指导风险缓解措施的优先顺序。
...
TL;DR: 提出了一种结合fMRI数据和DICOM元数据的多模态transformer框架，用于解码大脑状态，提高准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习和深度学习方法在处理fMRI数据时未能充分利用DICOM元数据提供的丰富上下文信息，限制了大脑状态解码的性能。
...
Conclusion: 该框架有效利用了DICOM元数据的上下文信息，但面临元数据变异性和计算需求等限制，未来需要优化可扩展性和泛化能力。
...
TL;DR: 提出一种针对离线强化学习的全局预算分配攻击策略，通过TD误差敏感性分配扰动预算，相比传统均匀扰动方法更高效且隐蔽


<details>
  <summary>Details</summary>
Motivation: 现有离线RL数据投毒攻击通常采用局部均匀扰动，对所有样本不加区分，这既低效（浪费扰动预算在低影响力样本上）又缺乏隐蔽性（导致显著统计偏差）

Method: 提出全局预算分配攻击策略，基于理论洞察：样本对价值函数收敛的影响与其TD误差成正比。将攻击建模为全局资源分配问题，推导出闭式解，在全局L2约束下按TD误差敏感性比例分配扰动幅度
...
### [145] [Long-Sequence LSTM Modeling for NBA Game Outcome Prediction Using a Novel Multi-Season Dataset](https://arxiv.org/abs/2512.08591)
*Charles Rios,Longzhen Han,Almas Baimagambetov,Nikolaos Polatidis*

Main category: cs.LG

TL;DR: 该研究构建了一个覆盖2004-2024年NBA赛季的纵向数据集，并开发了一个LSTM深度学习框架，通过长达9840场比赛（相当于8个完整赛季）的序列长度来预测NBA比赛结果，相比传统机器学习方法取得了最佳性能。
...
Method: 构建了覆盖2004-05至2024-25赛季的纵向NBA数据集，开发了基于LSTM的深度学习框架，使用长达9840场比赛（8个完整赛季）的序列长度来捕捉球队动态演变和赛季间依赖关系，并与逻辑回归、随机森林、MLP和CNN等传统方法进行比较。

Result: LSTM模型在所有评估指标上表现最佳：准确率72.35%，精确率73.15%，AUC-ROC为76.13%，证明了长序列时间建模在篮球结果预测中的重要性。

Conclusion: 长序列时间建模对于篮球比赛结果预测至关重要，新构建的多赛季数据集为开发稳健、可泛化的NBA预测系统提供了重要价值，LSTM架构在捕捉长期性能趋势方面表现出色。
...
### [146] [DS FedProxGrad: Asymptotic Stationarity Without Noise Floor in Fair Federated Learning](https://arxiv.org/abs/2512.08671)
*Huzaifa Arif*

Main category: cs.LG

TL;DR: 改进的FedProxGrad算法（DS FedProxGrad）在非凸复合优化问题中实现了渐近收敛到平稳点，消除了原算法对噪声底限的依赖。


<details>
  <summary>Details</summary>
Motivation: 原FedProxGrad算法在解决群体公平联邦学习的非凸复合优化问题时，只能收敛到噪声主导的平稳邻域，且收敛性依赖于方差引起的噪声底限。本文旨在改进这一局限性。

Method: 提出DS FedProxGrad（衰减步长FedProxGrad）分析框架，采用Robbins-Monro步长调度策略，允许局部近端解存在一定的不精确性，并包含显式的公平正则化。

Result: 在满足局部不精确性衰减的温和条件下，证明了算法渐近收敛到平稳点：liminf E[‖∇F(x^r)‖²] = 0，收敛速率不再依赖于方差引起的噪声底限。

Conclusion: DS FedProxGrad框架显著改进了原FedProxGrad的收敛性分析，实现了渐近平稳性，为群体公平联邦学习提供了更强大的理论保证。
...
### [147] [An Additive Manufacturing Part Qualification Framework: Transferring Knowledge of Stress-strain Behaviors from Additively Manufactured Polymers to Metals](https://arxiv.org/abs/2512.08699)
*Chenglong Duan,Dazhong Wu*

Main category: cs.LG

TL;DR: 开发基于动态时间规整和迁移学习的框架，用于增材制造零件认证，通过将低成本聚合物的应力-应变行为知识迁移到金属材料


<details>
  <summary>Details</summary>
Motivation: 增材制造零件认证对确保关键应用中的零件一致性至关重要，需要准确预测复杂应力-应变行为。传统方法成本高且效率低，需要开发更有效的预测方法。

Method: 提出DTW-TL框架：使用动态时间规整选择与目标金属最相关的聚合物数据集作为源域，采用LSTM模型进行知识迁移。使用四种聚合物和三种金属材料验证框架有效性。

Result: DTW-TL框架能准确识别聚合物与金属的最佳匹配，选择单一聚合物作为源域。在三种金属目标域上，模型获得12.41%的最低平均绝对百分比误差和0.96的最高决定系数，优于无迁移学习的LSTM模型和基于四种聚合物的迁移学习模型。

Conclusion: DTW-TL框架为增材制造零件认证提供了一种有效的知识迁移方法，能够显著提高金属材料应力-应变行为的预测精度，降低认证成本。
...
### [148] [Exposing Hidden Biases in Text-to-Image Models via Automated Prompt Search](https://arxiv.org/abs/2512.08724)
*Manos Plitsis,Giorgos Bouritsas,Vassilis Katsouros,Yannis Panagakis*

Main category: cs.LG

TL;DR: BGPS框架自动生成能最大化图像偏见的提示词，揭示TTI扩散模型中未记录的微妙偏见，可作为偏见评估工具


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工或LLM构建的提示数据集进行偏见缓解和评估，成本高且可能遗漏未预见的偏见触发提示，需要自动发现偏见的方法

Method: 提出Bias-Guided Prompt Search (BGPS)框架：1) LLM生成属性中性提示；2) 基于TTI内部表示的属性分类器引导LLM解码过程，放大目标图像属性

Result: 在Stable Diffusion 1.5和先进去偏见模型上发现大量微妙且未记录的偏见，严重降低公平性指标，发现的提示词可解释且可由普通用户输入
...
### [149] [Neural Ordinary Differential Equations for Simulating Metabolic Pathway Dynamics from Time-Series Multiomics Data](https://arxiv.org/abs/2512.08732)
...
<details>
  <summary>Details</summary>
Motivation: 人类健康寿命和生物工程的进步严重依赖于预测复杂生物系统的行为。虽然高通量多组学数据日益丰富，但将这些数据转化为可操作的预测模型仍然是一个瓶颈。需要高容量、数据驱动的模拟系统来直接从观测数据推断潜在相互作用，以支持个性化医疗和合成生物学中的下游干预效果预测。

Method: 引入神经常微分方程(NODEs)作为动态框架，用于学习蛋白质组与代谢组之间的复杂相互作用。将该框架应用于工程大肠杆菌菌株的时间序列数据，对代谢途径的连续动态进行建模。

Result: NODE架构在捕捉系统动态方面表现出优于传统机器学习管道的性能。在柠檬烯和异戊烯醇途径数据集上，均方根误差相比基线改善了90%以上（柠檬烯最高94.38%，异戊烯醇最高97.65%）。此外，NODE模型的推理时间加速了1000倍。

Conclusion: NODE模型被确立为可扩展、高保真的工具，适用于下一代代谢工程和生物发现，能够有效预测复杂生物系统的动态行为。
...
### [150] [Learning and Editing Universal Graph Prompt Tuning via Reinforcement Learning](https://arxiv.org/abs/2512.08763)
...
Method: 提出LEAP模型：1）在所有节点构建基本通用图提示以保持理论基础；2）使用actor-critic强化学习选择节点并编辑提示，实现更理想的提示效果。

Result: 在图级和节点级任务的各种预训练策略下，无论是全样本还是少样本场景，LEAP都一致优于微调和其他基于提示的方法。

Conclusion: 在所有节点添加提示是保持通用图提示调优理论基础的必需条件，LEAP通过结合基础提示和强化学习编辑，实现了理论基础保持与性能优化的平衡。
...
### [151] [Can TabPFN Compete with GNNs for Node Classification via Graph Tabularization?](https://arxiv.org/abs/2512.08798)
...
<details>
  <summary>Details</summary>
Motivation: 基于TabPFN在表格数据和时间序列上的成功，研究是否可以将图节点分类问题重新构建为表格学习问题，探索无需图特定训练或语言模型依赖的图学习方法。

Method: 提出TabPFN-GN方法，将图数据转换为表格特征：提取节点属性、结构特征、位置编码，以及可选的平滑邻域特征，然后使用TabPFN进行直接节点分类。

Result: 在12个基准数据集上的实验表明，TabPFN-GN在同质图上与GNNs性能相当，在异质图上始终优于GNNs，证明了特征工程可以弥合表格和图领域之间的差距。

Conclusion: 通过合理的特征工程，可以将图节点分类问题转化为表格学习问题，为任务特定的GNN训练和LLM依赖的图基础模型提供了实用的替代方案。
...
### [152] [Identifying counterfactual probabilities using bivariate distributions and uplift modeling](https://arxiv.org/abs/2512.08805)
*Théo Verhelst,Gianluca Bontempi*

Main category: cs.LG

TL;DR: 提出一种基于提升模型预测得分的反事实估计方法，使用二元Beta分布拟合提升分数，获得反事实结果的后验分布。


<details>
  <summary>Details</summary>
Motivation: 提升建模只能估计干预的因果效应（处理组与对照组的差异），而反事实识别旨在恢复潜在结果的联合分布（如"如果给客户提供营销优惠，他们是否仍会流失？"）。反事实分布提供比提升更丰富的信息，但更难估计。然而这两种方法是协同的：可以利用提升模型进行反事实估计。

Method: 提出一种反事实估计器，将二元Beta分布拟合到预测的提升分数上，从而获得反事实结果的后验分布。该方法除了提升建模所需的因果假设外，不需要额外的因果假设。
...
### [153] [Forecasting Fails: Unveiling Evasion Attacks in Weather Prediction Models](https://arxiv.org/abs/2512.08832)
*Huzaifa Arif,Pin-Yu Chen,Alex Gittens,James Diffenderfer,Bhavya Kailkhura*

Main category: cs.LG

TL;DR: WAAPO框架生成针对AI天气预报模型的对抗性扰动，通过通道稀疏性、空间局部性和平滑性约束确保扰动有效且隐蔽，揭示了AI天气预报系统的安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着AI模型在天气预报中的依赖增加，需要评估其对对抗性扰动的脆弱性，以识别安全风险并推动防护措施。

Method: 提出Weather Adaptive Adversarial Perturbation Optimization (WAAPO)框架，通过通道稀疏性、空间局部性和平滑性约束生成物理真实且难以检测的对抗性扰动，使用ERA5数据集和FourCastNet模型进行实验验证。

Result: WAAPO能够生成与预定目标紧密对齐的对抗性轨迹，即使是在约束条件下；实验显示初始条件的微小扰动可导致预测天气模式的显著偏差，揭示了AI驱动预报模型的关键脆弱性。

Conclusion: AI天气预报模型存在严重安全漏洞，需要建立强大的防护措施来防止对抗性攻击在业务预报系统中的利用。
...
### [154] [Reinforcement Learning From State and Temporal Differences](https://arxiv.org/abs/2512.08855)
*Lex Weaver,Jonathan Baxter*

Main category: cs.LG

TL;DR: STD(λ)是一种改进的TD(λ)算法，专注于状态相对值的排序而非绝对值，解决了TD(λ)在某些情况下从最优策略收敛到次优策略的问题。


<details>
  <summary>Details</summary>
Motivation: TD(λ)在函数逼近中虽然能最小化状态值的平方误差，但对于策略而言，状态值的相对排序比绝对值更重要。研究发现TD(λ)在某些情况下会从最优策略收敛到次优策略，因此需要改进算法来更好地处理状态相对值。

Method: 提出了STD(λ)算法，这是一种改进的TD(λ)形式，专门针对二元决策问题训练函数逼近器以关注状态相对值。算法包括理论分析，在二状态系统中证明了策略单调改进的性质，并与Bertsekas的差分训练方法进行了比较。

Result: STD(λ)成功解决了二状态系统中TD(λ)的收敛问题，并在著名的acrobot问题的变体上取得了成功演示。理论分析表明STD(λ)在二状态系统中能保证策略的单调改进。

Conclusion: STD(λ)通过关注状态值的相对排序而非绝对值，解决了TD(λ)在某些强化学习问题中的局限性，为函数逼近中的策略改进提供了更有效的方法。
...
### [155] [Refining Diffusion Models for Motion Synthesis with an Acceleration Loss to Generate Realistic IMU Data](https://arxiv.org/abs/2512.08859)
*Lars Ole Häusler,Lena Uhlenberg,Göran Köber,Diyora Salimova,Oliver Amft*

Main category: cs.LG

TL;DR: 提出了一种文本到IMU运动合成框架，通过使用加速度二阶损失微调预训练扩散模型，提高IMU数据生成的真实性


<details>
  <summary>Details</summary>
Motivation: 现有的文本到运动生成模型在生成IMU数据时缺乏对加速度模式的专门优化，导致生成的IMU数据与真实记录存在差异，影响下游任务如人类活动识别的性能

Method: 提出加速度二阶损失函数L_acc，强制生成运动在离散二阶时间差分上的一致性，将L_acc集成到现有扩散模型的训练目标中，通过微调获得IMU特定的运动先验，结合表面建模和虚拟传感器模拟框架生成IMU数据

Result: L_acc损失相对原始模型降低了12.7%，高动态活动（跑步、跳跃）改进更显著，合成IMU数据在低维嵌入中更接近真实IMU记录分布，仅使用精炼合成IMU数据训练的人类活动识别分类性能比原始扩散模型提高8.7%，比最佳对比模型提高7.6%
...
### [156] [Fed-SE: Federated Self-Evolution for Privacy-Constrained Multi-Environment LLM Agents](https://arxiv.org/abs/2512.08870)
*Xiang Chen,Yuling Shi,Qizhen Lan,Yuchao Qiu,Xiaodong Gu*

Main category: cs.LG

TL;DR: Fed-SE：一种用于LLM智能体的联邦自进化框架，通过本地进化-全局聚合范式解决异构任务和稀疏奖励下的梯度冲突问题，在隐私受限环境中实现跨环境知识迁移


<details>
  <summary>Details</summary>
Motivation: LLM智能体在复杂交互任务中广泛应用，但隐私约束限制了集中式优化和跨动态环境的协同进化。虽然联邦学习在静态数据集上有效，但在开放端自进化智能体中的应用仍待探索。直接应用标准联邦学习面临挑战：异构任务和稀疏的轨迹级奖励会导致严重梯度冲突，破坏全局优化过程。

Method: Fed-SE采用本地进化-全局聚合范式。本地层面，智能体在过滤后的高回报轨迹上使用参数高效微调实现稳定梯度更新。全局层面，Fed-SE在低秩子空间内聚合更新，解耦环境特定动态，有效减少客户端间的负迁移。

Result: 在五个异构环境中的实验表明，Fed-SE相比联邦基线平均任务成功率提升约18%，验证了其在隐私受限部署中实现鲁棒跨环境知识迁移的有效性。

Conclusion: Fed-SE成功解决了LLM智能体联邦自进化中的梯度冲突问题，通过本地稳定更新和全局低秩聚合实现了有效的跨环境知识迁移，为隐私约束下的智能体协同进化提供了可行方案。
...
### [157] [When Tables Leak: Attacking String Memorization in LLM-Based Tabular Data Generation](https://arxiv.org/abs/2512.08875)
*Joshua Ward,Bochao Gu,Chi-Hua Wang,Guang Cheng*

Main category: cs.LG

TL;DR: 研究发现LLM生成表格数据时存在隐私泄露风险，数字序列容易被记忆并重现，提出LevAtt攻击方法和防御策略


<details>
  <summary>Details</summary>
Motivation: LLM在生成高质量表格合成数据方面表现出色，但现有方法存在隐私泄露风险，需要系统分析这种风险并开发有效防御措施

Method: 提出LevAtt黑盒成员推理攻击方法，仅利用生成的合成数据，针对数字序列进行攻击；同时提出两种防御方法，包括新颖的采样策略在生成过程中策略性地扰动数字

Result: 攻击方法在多种模型和数据集上暴露了显著的隐私泄露，在某些情况下甚至成为最先进模型的完美成员分类器；防御方法能够在最小化保真度和效用损失的情况下抵御这些攻击

Conclusion: LLM基于合成数据生成存在独特的隐私漏洞，需要有效防御措施；提出的防御策略能够有效保护隐私同时保持数据质量
...
### [158] [DAO-GP Drift Aware Online Non-Linear Regression Gaussian-Process](https://arxiv.org/abs/2512.08879)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界数据集常呈现随时间变化的数据分布（概念漂移），忽略这一现象会显著降低模型预测准确性。传统在线高斯过程模型存在多个关键限制：缺乏漂移感知能力、依赖固定超参数、易受数据窥探影响、缺乏原则性衰减机制以及内存效率低下。
...
Conclusion: DAO-GP被确立为一种漂移弹性的解决方案，适用于在线非线性回归任务，能够有效应对概念漂移带来的挑战。
...
### [159] [Unsupervised Learning of Density Estimates with Topological Optimization](https://arxiv.org/abs/2512.08895)
*Suina Tanweer,Firas A. Khasawneh*

Main category: cs.LG

TL;DR: 提出一种基于拓扑数据分析的带宽选择方法，使用拓扑损失函数实现无监督学习，相比传统方法在不同维度上表现更优


<details>
  <summary>Details</summary>
Motivation: 核密度估计在机器学习、贝叶斯推断等领域广泛应用，但带宽选择是关键超参数，需要平衡偏差-方差权衡。传统方法需要人工调参，而拓扑数据分析可以量化高维数据的拓扑特征，为解决带宽选择问题提供了新思路。

Method: 提出基于拓扑的损失函数进行无监督学习，自动选择最优带宽。利用拓扑数据分析方法量化密度估计的拓扑特征（如连通分量、环、空洞等），通过优化拓扑损失函数实现带宽的自动化选择。

Result: 该方法在不同维度上进行了基准测试，与传统技术相比表现出潜力。拓扑方法能够有效处理高维数据，在可视化困难的情况下仍能选择出合适的带宽。

Conclusion: 基于拓扑的损失函数为核密度估计的带宽选择提供了一种有效的无监督学习方法，能够自动优化带宽参数，在不同维度数据上展现出优于传统方法的潜力。
...
Method: 发布包含10K聚合物和5种性质（热导率、回转半径、密度、自由体积分数、玻璃化转变温度）的基准数据集，组织多任务聚合物性质预测竞赛，参与者在数据量小、标签不平衡、模拟源异质等现实约束下开发模型，采用特征增强、迁移学习、自监督预训练、针对性集成策略等技术。

Result: 竞赛揭示了数据准备、分布偏移和跨组模拟一致性等方面的重要经验教训，为未来大规模聚合物数据集的最佳实践提供了指导。生成的模型、分析和发布数据为聚合物科学中的分子AI建立了新基础。

Conclusion: Open Polymer Challenge通过发布首个社区开发的聚合物信息学基准，创建了加速可持续和节能材料开发的新基础，同时发布了测试数据集和数据生成管道。
...
### [161] [Joint Activity Design Heuristics for Enhancing Human-Machine Collaboration](https://arxiv.org/abs/2512.08036)
*Mohammadreza Jalaeian,Dane A. Morey,Michael F. Rayo*

Main category: cs.HC

TL;DR: 论文提出14条启发式设计原则，用于支持人机联合活动的技术设计与评估，重点关注团队中的五个宏观认知功能。


<details>
  <summary>Details</summary>
Motivation: 当前技术设计主要关注可用性，但缺乏对联合活动中人机协调互依赖性的专门支持。需要从单纯设计可用技术转向设计有效的"团队伙伴"。

Method: 综合显示设计、人因工程、认知系统工程、认知心理学和计算机科学等领域的相关文献，提炼出14条启发式设计原则。

Result: 提出了一个包含14条启发式的框架，用于指导支持人机联合活动的技术设计、开发和评估，重点关注事件检测、意义建构、适应性、视角转换和协调这五个宏观认知功能。

Conclusion: 支持联合活动与确保技术可用性同等重要，提出的启发式原则为设计有效的人机团队协作系统提供了实用指导框架。
...
### [162] [A Comparative Study of EMG- and IMU-based Gesture Recognition at the Wrist and Forearm](https://arxiv.org/abs/2512.07997)
*Soroush Baghernezhad,Elaheh Mohammadreza,Vinicius Prado da Fonseca,Ting Zou,Xianta Jiang*

Main category: cs.HC

TL;DR: 该研究探索使用惯性测量单元（IMU）信号作为手部静态手势识别的唯一输入传感器，发现IMU能够捕捉肌腱诱导的微运动，为手势识别提供足够信息。


<details>
  <summary>Details</summary>
Motivation: 虽然表面肌电图（sEMG）在手势识别中被广泛研究，但较少探索的惯性测量单元（IMU）可以提供关于细微肌肉运动的补充信息，这对于手势识别具有价值。研究旨在探索使用不同肌肉群的IMU信号来捕捉用户意图的潜力。

Method: 研究使用IMU信号作为输入传感器，比较不同肌肉群，并检查各个肌肉群上的模式识别质量。特别关注肌腱诱导的微运动在静态手势识别中的作用。

Result: 结果表明，IMU信号包含足够的信息，可以作为静态手势识别的唯一输入传感器。研究发现肌腱诱导的微运动是静态手势识别的主要贡献因素。

Conclusion: 利用肌肉微运动信息可以增强假肢对截肢者的可用性，并为机器人、远程操作、手语解释等领域的手势识别提供新的可能性。
...
### [163] ["Your Privacy is Your Responsibility": Understanding How Users Collectively Navigate the Complexity of Privacy on Quora](https://arxiv.org/abs/2512.08025)
*Varun Shiri,Maggie Xiong,Jin L. C. Guo,Jinghui Cheng*

Main category: cs.HC

TL;DR: 该研究通过分析Quora上用户讨论的隐私话题，探索如何通过自下而上的集体方法赋能用户隐私保护，而非传统的自上而下监管或技术设计方法。


<details>
  <summary>Details</summary>
Motivation: 当前技术环境中，用户在隐私保护方面处于弱势地位。以往隐私保护努力主要关注自上而下的监管和技术设计方法，忽视了通过自下而上、集体方法赋能用户的机会。

Method: 通过分析Quora平台上用户讨论的隐私相关话题，识别用户提出的广泛且相互关联的隐私主题，包括隐私风险与危险、保护策略、组织实践以及现有法律法规。

Result: 研究发现用户讨论涉及广泛的隐私话题，并揭示了影响用户隐私态度的个体、技术、组织和社会因素之间的相互作用。识别了用户多样化的隐私相关行为和偏好。

Conclusion: 研究结果为设计基于社区的工具提供了启示，这些工具能够更好地支持用户在隐私导航方面的集体努力，整合用户多样化的隐私行为和偏好，简化信息访问和共享，并连接设计师、开发者与用户社区。
...
### [164] [Mediating Personal Relationships with Robotic Pets for Fostering Human-Human Interaction of Older Adults](https://arxiv.org/abs/2512.08426)
*Delong Du,Sara Gilda Amirhajlou,Akwasi Gyabaah,Richard Paluch,Claudia Müller*

Main category: cs.HC

TL;DR: 该研究探讨如何设计机器人宠物来促进老年人的人际关系，而非仅关注人机关系


<details>
  <summary>Details</summary>
Motivation: 良好人际关系对幸福生活至关重要，现有研究多关注老年人与机器人宠物的直接关系，而忽视了机器人宠物如何促进人与人之间的关系

Method: 采用人种学研究方法，通过对六位老年人进行半结构化访谈，并进行主题分析

Result: 研究发现机器人宠物可以设计成远程机器人，帮助老年人远程连接他人，为促进人际关系提供新途径

Conclusion: 机器人宠物作为远程连接工具的设计思路，有助于推动未来促进心理健康的机器人系统发展
...
### [165] [Time and Money Matters for Sustainability: Insights on User Preferences on Renewable Energy for Electric Vehicle Charging Stations](https://arxiv.org/abs/2512.08437)
*Delong Du,Apostolos Vavouris,Omid Veisi,Lu Jin,Gunnar Stevens,Lina Stankovic,Vladimir Stankovic,Alexander Boden*

Main category: cs.HC

TL;DR: 研究探索将动态可再生能源可用性整合到充电站导航中，影响电动汽车用户选择可再生能源充电站的行为模式。


<details>
  <summary>Details</summary>
Motivation: 电动汽车使用可再生能源充电可减少环境影响，但可再生能源的波动性影响公共充电站的可持续性。不同充电站的能源来源差异很大，需要研究如何通过导航系统引导用户选择可再生能源充电站。

Method: 采用组内设计调查50名汽车用户，并对来自农村、郊区和城市地区的10名电动汽车用户进行半结构化访谈，研究不同驾驶场景下用户的选择偏好。

Result: 结果显示，选择充电站时，驾驶员通常根据驾驶场景优先考虑节省时间或节省金钱。当可再生能源充电站符合用户的主要优先级（无论是省钱还是省时）时，电动汽车用户倾向于选择这些充电站。

Conclusion: 研究为导航推荐系统的前端图形用户界面和后端排名算法开发提供了终端用户洞察，有助于整合动态可再生能源可用性，促进电动汽车的可持续使用。
...
### [166] [Exploring the Grassroots Understanding and Practices of Collective Memory Co-Contribution in a University Community](https://arxiv.org/abs/2512.08787)
*Zeyu Huang,Xinyi Cao,Yue Deng,Junze Li,Kangyu Yuan,Xiaojuan Ma*

Main category: cs.HC

TL;DR: 该研究从成员层面探讨集体记忆的形成过程，通过两个移动系统（定位叙事探索系统和传统在线论坛）进行为期两周的实地研究，分析集体记忆的贡献实践和互动模式。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要从整体社会学视角分析成熟的集体记忆，较少关注成员层面的集体记忆概念化以及共同贡献实践的具体形式。本研究旨在填补这一空白，从成员视角探索集体记忆的解释性和互动性模式。

Method: 采用混合方法实地研究，设计两个移动系统作为观察锚点：1）基于现有文献设计框架的定位叙事探索系统；2）代表当前实践的传统在线论坛。在校园环境中进行为期两周的研究（n=38）。

Result: 研究发现核心争议在于：是回顾性思考当下作为未来历史，还是记录当下作为未来历史。这影响了叙事焦点、对集体记忆构成的期望以及参与者从群体中寻求灵感的方式。研究进一步提取了能更好包容集体记忆多样化概念化并连接不同社区成员的设计考虑。

Conclusion: 研究为社区驱动的用户生成内容平台提供了设计见解，特别是关于如何设计专门的定位叙事体验来支持集体记忆的形成。通过反思设计过程，为未来相关系统开发提供了额外洞察。
...
### [167] ["Nothing about us without us": Perspectives of Global Deaf and Hard-of-hearing Community Members on Sign Language Technologies](https://arxiv.org/abs/2512.08839)
*Katherine Atwell,Saki Imai,Danielle Bragg,Malihe Alikhani*

Main category: cs.HC

TL;DR: 全球多语言调查显示，聋人和重听人士对手语技术既看到其支持访问和独立的潜力，又担忧文化侵蚀、翻译不准确以及听力主导的研究流程等问题。


<details>
  <summary>Details</summary>
Motivation: 手语技术发展迅速，但聋人和重听人士的视角在技术开发中被边缘化，特别是在西方以外和全球南方地区。本研究旨在通过全球调查了解不同国家、手语和文化背景下社区对手语技术的看法。

Method: 采用全球多语言调查方法，收集来自不同国家、使用不同手语、处于不同文化背景下的聋人和重听人士的观点。

Result: 参与者认识到手语技术有助于提升访问性和独立性，但普遍担忧文化侵蚀、翻译不准确以及听力主导的研究流程。手语熟练度、政策接触度和聋人身份认同等因素影响了对技术的看法。各地区参与者都强调聋人主导设计的重要性。

Conclusion: 研究提供了跨大洲、社区知情的手语技术分析，并为研究人员、技术专家和政策制定者提出了可行的建议，强调聋人社区参与技术决策的重要性。
...
### [168] [Impact of Data-Oriented and Object-Oriented Design on Performance and Cache Utilization with Artificial Intelligence Algorithms in Multi-Threaded CPUs](https://arxiv.org/abs/2512.07841)
...
TL;DR: 该研究对比了数据导向设计(DOD)与传统面向对象设计(OOD)在多线程环境下的性能表现，通过A*搜索算法的四种实现版本进行测试，发现DOD在多线程场景下具有显著性能优势，特别是在数据密集型操作中。


<details>
  <summary>Details</summary>
Motivation: 多核CPU与主内存之间日益增长的性能差距需要硬件感知的软件设计范式，因此需要评估不同设计范式在缓存利用和多线程效率方面的表现。

Method: 开发并比较了A*搜索算法的四个版本：单线程OOD、单线程DOD、多线程OOD和多线程DOD，基于执行时间、内存使用和CPU缓存未命中等指标进行评估。

Result: 在多线程测试中，DOD实现表现出显著性能优势，执行时间更快，系统调用和缓存未命中更少。对于A*这样的细粒度任务，单线程版本在两个范式中都显著优于多线程版本。
...
### [169] [SkipKV: Selective Skipping of KV Generation and Storage for Efficient Inference with Large Reasoning Models](https://arxiv.org/abs/2512.07993)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在推理过程中由于链式思维推理过程冗长，导致KV缓存线性增长，造成内存和吞吐量瓶颈。现有KV缓存淘汰方法在多批次设置中因不稳定的令牌级评分和填充令牌导致的KV预算减少而无法保持准确性，且往往生成更长的序列。

Method: SkipKV是一种无需训练的KV压缩方法，采用粗粒度的句子级序列移除策略。它引入句子评分指标来识别和移除高度相似的句子，同时保持语义连贯性。为抑制冗余生成，SkipKV动态调整引导向量来更新推理过程中的隐藏激活状态，强制模型生成简洁响应。

Result: 在多个推理基准测试中，SkipKV在相似压缩预算下相比替代方法保持高达26.7%的准确率提升。与最先进方法相比，SkipKV生成长度减少高达1.6倍，吞吐量提高高达1.7倍。

Conclusion: SkipKV通过句子级KV缓存压缩有效解决了大型推理模型在链式思维推理中的KV缓存开销问题，在保持准确性的同时显著提高了推理效率，为高效部署提供了有效解决方案。
...
### [170] [Toward an AI Reasoning-Enabled System for Patient-Clinical Trial Matching](https://arxiv.org/abs/2512.08026)
...
<details>
  <summary>Details</summary>
Motivation: 临床试验患者筛选目前是手动、耗时且资源密集的过程，需要自动化解决方案来提高效率并减轻协调员负担。

Method: 使用开源、支持推理的大型语言模型，超越二元分类，生成结构化资格评估和可解释推理链，支持人机协同审查，将资格表示为动态状态而非固定判定。

Result: 开发了一个安全、可扩展的概念验证系统，能够整合异构EHR数据，提供结构化资格评估，识别匹配患者，并为未来资格提供可操作建议。

Conclusion: 该系统旨在减轻协调员负担，智能扩展每个患者考虑的试验范围，并保证所有AI生成输出的全面可审计性，代表了临床试验匹配的创新方法。
...
### [171] [Large Language Models for Education and Research: An Empirical and User Survey-based Analysis](https://arxiv.org/abs/2512.08057)
*Md Mostafizer Rahman,Ariful Islam Shiplu,Md Faizul Ibne Amin,Yutaka Watanobe,Lu Peng*

Main category: cs.AI

TL;DR: 本研究对ChatGPT和DeepSeek两大LLM在教育研究领域进行综合评估，包括技术分析、实证实验和用户调查，比较了它们在文本生成、编程和专业问题解决方面的表现。


<details>
  <summary>Details</summary>
Motivation: 预训练大语言模型在教育研究领域展现出巨大潜力，ChatGPT和DeepSeek作为当前最先进的LLM在数学、科学、医学、文学和编程方面表现优异，但缺乏系统性的综合评估来比较它们的性能差异和实际应用价值。

Method: 采用三部分评估方法：1）背景技术分析；2）实证实验（在文本生成、编程和专门问题解决方面进行基准测试）；3）真实用户调查（涵盖学生、教育工作者和研究人员）。

Result: 实验结果显示：ChatGPT在通用语言理解和文本生成方面表现更优，而DeepSeek凭借其效率导向的设计在编程任务中表现更佳；两种模型都能提供医学准确的诊断输出并有效解决复杂数学问题；用户调查揭示了这些模型的实际益处和局限性。

Conclusion: ChatGPT和DeepSeek在教育研究领域各有优势，ChatGPT擅长语言理解，DeepSeek在编程方面更高效，两种模型都能有效支持专业任务；用户调查为理解这些模型在教育研究中的实际作用提供了重要见解。
...
### [172] [Scalable Back-End for an AI-Based Diabetes Prediction Application](https://arxiv.org/abs/2512.08147)
*Henry Anand Septian Radityo,Bernardus Willson,Reynard Tanadi,Latifa Dwiyanti,Saiful Akbar*

Main category: cs.AI

TL;DR: 本文开发了一个用于移动糖尿病预测应用的可扩展后端系统，采用水平扩展、数据库分片和异步消息队列架构，成功满足83%功能的性能目标，能处理1万并发用户。
...
Method: 采用水平扩展、数据库分片和通过消息队列（RabbitMQ）进行异步通信的架构设计，目标是故障率低于5%，平均延迟低于1000毫秒。

Result: 83%的系统功能（24个中的20个）达到了性能目标，用户档案管理、活动跟踪和读取密集型预测操作等关键功能表现良好，系统能处理1万并发用户，异步通信显著降低了计算密集型预测请求的错误率。

Conclusion: 该可扩展后端系统成功验证了其处理大规模用户的能力，异步通信机制在确保系统可靠性和防止数据丢失方面发挥了关键作用，为移动糖尿病预测应用提供了有效的技术基础。
...
### [173] [Beyond Traditional Diagnostics: Transforming Patient-Side Information into Predictive Insights with Knowledge Graphs and Prototypes](https://arxiv.org/abs/2512.08261)
*Yibowen Zhao,Yinan Zhang,Zhixiang Su,Lizhen Cui,Chunyan Miao*

Main category: cs.AI

TL;DR: KPI框架通过整合医学知识图谱、构建疾病原型和使用对比学习，解决了基于患者信息的疾病预测中的不平衡分布和可解释性问题，同时利用大语言模型生成个性化医学解释。


<details>
  <summary>Details</summary>
Motivation: 基于患者信息（人口统计和自述症状）的疾病预测研究面临两个关键挑战：疾病分布不平衡导致预测偏差，以及现有方法缺乏可解释性导致预测不可靠。

Method: 提出KPI框架：1）整合结构化可信医学知识构建统一疾病知识图谱；2）构建临床有意义的疾病原型；3）使用对比学习提高预测准确性，特别是对长尾疾病；4）利用大语言模型生成患者特异性、医学相关的解释。

Result: 在真实世界数据集上的实验表明，KPI在预测准确性上优于现有最先进方法，并能提供临床有效的解释，这些解释与患者叙述高度一致。

Conclusion: KPI框架通过整合医学知识和增强可解释性，提高了基于患者信息的疾病预测的准确性和可靠性，具有患者为中心医疗服务的实际应用价值。
...
### [174] [Reasoning Models Ace the CFA Exams](https://arxiv.org/abs/2512.08270)
*Jaisal Patel,Yunzhe Chen,Kaiwen He,Keyi Wang,David Li,Kairong Xiao,Xiao-Yang Liu*

Main category: cs.AI

TL;DR: 最新推理模型在CFA各级考试中表现优异，多数模型通过全部三级，其中Gemini 3.0 Pro在Level I创下97.6%的最高分记录
...
Method: 使用980道模拟CFA题目（包括Level I三级、Level II两级、Level III三级），采用与先前研究相同的通过标准，评估最先进的推理模型表现

Result: 多数模型通过了所有三级考试，按总体表现排序：Gemini 3.0 Pro、Gemini 2.5 Pro、GPT-5、Grok 4、Claude Opus 4.1、DeepSeek-V3.1。Gemini 3.0 Pro在Level I创下97.6%记录，GPT-5在Level II以94.3%领先，Gemini 2.5 Pro在Level III选择题获86.4%，Gemini 3.0 Pro在构建回答题获92.0%

Conclusion: 最新推理模型在CFA考试中表现出色，显著优于先前的大型语言模型，证明了推理能力在专业考试中的有效性
...
Result: See-Control通过低自由度机械臂实现智能手机的直接物理交互操作，提供了平台无关的解决方案，弥合了数字智能体与物理世界之间的差距。

Conclusion: 该框架为实现家庭机器人在真实环境中执行依赖智能手机的任务迈出了具体一步，为未来研究提供了有价值的资源。
...
TL;DR: 本文提出了一种强化策略注入机制(rSIM)，通过小型规划器引导大语言模型的思维链，将普通LLM转变为推理语言模型(RLM)，显著提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通过强化学习后训练可以进化为推理语言模型，其标志性特征是能够在思维链中表现出"顿悟"时刻，如自我反思和深度思考等策略。受此启发，本文旨在开发一种机制，使任何LLM都能通过策略注入成为RLM。

Method: 提出强化策略注入机制(rSIM)，采用领导者-追随者框架：小型规划器作为领导者智能体，LLM作为追随者智能体。两者通过多智能体强化学习联合训练，基于简单的基于规则的奖励机制，规划器自适应地向LLM的思维链注入推理策略。

Result: 实验结果显示，rSIM使Qwen2.5-0.5B模型转变为RLM，其性能显著超越Qwen2.5-14B模型。规划器具有通用性：只需训练一次即可作为插件应用于现有LLM，大幅提升其推理能力。此外，规划器支持跨任务的持续学习，其规划能力可逐步提升并泛化到更广泛的问题。

Conclusion: rSIM机制成功实现了将普通LLM转变为推理语言模型的目标，通过小型规划器的策略注入显著提升了模型的推理能力，且该规划器具有通用性、可插拔性和持续学习能力，为LLM的推理能力提升提供了一种有效的新方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统加州承载比测试方法耗时、昂贵且不适用于大规模或多样化的土壤剖面，需要更快速、精确的预测方法。人工智能和机器学习的发展为岩土工程提供了数据驱动的解决方案。

Method: 收集土耳其不同地理气候区域的382个土壤样本，包含与承载能力相关的物理化学特性。测试12种机器学习算法，包括决策树、随机森林、梯度提升、XGBoost、KNN、支持向量回归、多层感知器等，进行训练、验证和评估。

Result: 随机森林回归器表现最佳，训练集R²为0.95，验证集为0.76，测试集为0.83，显示出强大的非线性映射能力，是预测岩土任务的可靠工具。

Conclusion: 该研究证明了智能数据驱动模型在岩土工程中的有效性，为传统方法提供了有效替代方案，促进了基础设施分析和设计的数字化转型。
...
TL;DR: 本研究提出了一种基于自动机器学习（AutoML）的方法来预测土壤最优含水率（OMC）和最大干密度（MDD），以替代传统实验室试验和有限精度的经验回归模型。


<details>
  <summary>Details</summary>
Motivation: 传统确定土壤压实参数的方法存在劳动强度大、适用范围有限的问题，而现有机器学习模型在处理异质土壤数据时预测精度和泛化能力不足，需要更有效的自动化预测方法。

Method: 采用自动机器学习（AutoML）方法，自动进行算法选择和超参数优化，通过大量实验确定最佳算法，最终发现极端梯度提升（XGBoost）算法表现最优。

Result: XGBoost算法在独立数据集上取得了优异性能：MDD预测的R平方值为80.4%，OMC预测的R平方值为89.1%，证明了AutoML方法在不同土壤类型中预测压实参数的有效性。

Conclusion: AutoML方法能够有效预测土壤压实参数，异质数据集对提升机器学习模型的泛化能力和性能至关重要，该研究有助于提高建筑工程的效率和可靠性。
...
### [179] [DeepFeature: Iterative Context-aware Feature Generation for Wearable Biosignals](https://arxiv.org/abs/2512.08379)
...
Conclusion: DeepFeature是首个LLM赋能的上下文感知特征生成框架，通过整合专家知识、迭代精炼和稳健代码转换，显著提升了可穿戴生物信号特征提取的性能
...
### [180] [Autonomous Issue Resolver: Towards Zero-Touch Code Maintenance](https://arxiv.org/abs/2512.08492)
*Aliaksei Kaliutau*

Main category: cs.AI

TL;DR: 提出从代码属性图转向数据转换图的新范式，通过多智能体框架实现仓库级自动程序修复，解决了现有RAG系统中的"语义陷阱"问题


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在函数级代码生成方面取得进展，但仓库级自动程序修复仍面临挑战。现有方法采用控制中心范式，迫使智能体处理复杂的目录结构和无关的控制逻辑，效率低下。

Method: 提出数据转换图范式，将数据状态作为节点、函数作为边，通过数据谱系而非控制流追踪逻辑缺陷。引入多智能体框架协调数据完整性导航与控制流逻辑，并实现自主问题解决器系统，利用神经符号推理进行可扩展的逻辑修复。

Result: 在多个软件工程基准测试中取得良好结果，在SWE-Verified基准测试上达到87.1%的解决率，直接解决了当前AI代码助手工具的核心限制。

Conclusion: 通过从控制流到数据流的范式转变，解决了仓库级程序修复的挑战，为日益依赖软件的世界提供了更强大的基础，实现了零接触代码维护的自改进系统。
...
<details>
  <summary>Details</summary>
Motivation: 机器人在人类环境中需要伦理意识，但现有自动化规划工具缺乏伦理支持。手动指定伦理规则劳动密集且高度情境特定，需要更实用的解决方案。

Method: 提出Principles2Plan原型系统，通过人类与LLM协作：领域专家提供规划领域、问题细节和高级伦理原则；系统生成可操作的伦理规则；用户审查、优先排序并供给规划器生成伦理知情计划。

Result: 开发了首个支持用户在经典规划情境中生成基于原则的伦理规则的系统，展示了人机协作在伦理自动化规划中的潜力。

Conclusion: Principles2Plan展示了人类与LLM协作使伦理自动化规划更加实用可行，为机器人伦理决策提供了新的交互式方法。
...
TL;DR: 本文提出CogMCTS框架，将LLM的认知引导机制与MCTS紧密结合，通过多轮认知反馈、双轨节点扩展和策略变异，实现高效的自动启发式设计，在稳定性、效率和解决方案质量上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的进化方法依赖种群策略易陷入局部最优，而LLM与MCTS结合的方法在多轮认知整合和搜索多样性方面存在局限，需要克服这些限制以实现更高效的自动启发式设计。

Method: 提出认知引导的MCTS框架(CogMCTS)：1) 多轮认知反馈整合历史经验、节点信息和负面结果；2) 双轨节点扩展结合精英启发式管理平衡探索与利用；3) 策略变异修改启发式形式和参数增强多样性。

Result: 实验结果表明，CogMCTS在稳定性、效率和解决方案质量方面优于现有的基于LLM的自动启发式设计方法。

Conclusion: CogMCTS框架通过紧密集成LLM认知引导与MCTS，有效解决了现有方法在探索-利用权衡、搜索多样性和多轮认知整合方面的局限性，实现了更高效的自动启发式优化。
...
<details>
  <summary>Details</summary>
Motivation: 蛋白质二级结构（α螺旋、β折叠、无规卷曲）预测对于理解蛋白质功能至关重要。传统方法在处理变长序列和捕获长程残基相互作用方面存在局限，需要更有效的序列建模方法。

Method: 采用基于Transformer的模型，应用注意力机制处理蛋白质序列数据。使用滑动窗口数据增强技术在CB513数据集上扩展训练样本。模型能够处理变长序列，有效捕获局部和长程残基相互作用。

Result: Transformer模型在蛋白质二级结构预测任务中表现出强大的泛化能力，能够有效处理变长序列，并成功捕获氨基酸序列中的局部和长程相互作用模式。

Conclusion: 基于Transformer的模型为蛋白质二级结构预测提供了有效的解决方案，其注意力机制能够更好地建模氨基酸序列中的复杂相互作用，为蛋白质功能研究提供了有力工具。
...
<details>
  <summary>Details</summary>
Motivation: 胃肠道肿瘤学的多模态临床推理需要整合内镜图像、放射学数据和生化标志物。尽管多模态大语言模型展现出潜力，但在处理复杂异质医疗记录时经常面临上下文稀释和幻觉问题。

Method: 提出分层多智能体框架，模拟人类多学科团队的协作工作流程，以解决现有MLLM在处理复杂医疗数据时的局限性。

Result: 系统获得4.60/5.00的专家综合评分，相比单一基线模型有显著提升。基于智能体的架构在推理逻辑和医学准确性方面改善最为明显。
...
### [185] [Towards Foundation Models with Native Multi-Agent Intelligence](https://arxiv.org/abs/2512.08743)
*Shuyue Hu,Haoyang Yan,Yiqun Zhang,Yang Chen,Dongzhan Zhou,Lei Bai*

Main category: cs.AI

TL;DR: 论文认为基础模型作为AI智能体的"大脑"，需要从单智能体能力转向原生多智能体智能，因为实证研究表明单智能体性能强并不自动转化为多智能体智能。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型主要发展单智能体能力（如GUI交互、工具使用），但多智能体协作是AI发展的下一个前沿。作者发现单智能体性能强并不自动转化为多智能体智能，因此需要专门研究如何赋予基础模型原生多智能体智能。

Method: 通过41个大语言模型的广泛实证研究，验证单智能体性能与多智能体智能的关系。识别了基础模型在多智能体环境中的四个核心能力：理解、规划、高效通信和适应。

Result: 实证研究表明，强大的单智能体性能并不会自动产生稳健的多智能体智能，这挑战了多智能体能力会自然涌现的假设。
...
### [186] [A Practical Guide for Designing, Developing, and Deploying Production-Grade Agentic AI Workflows](https://arxiv.org/abs/2512.08769)
...
TL;DR: 本文提供了设计和部署生产级智能体AI工作流的端到端实用指南，介绍了结构化工程生命周期和九大核心最佳实践，并通过多模态新闻分析案例进行演示。
...
Method: 提出结构化工程生命周期，包括工作流分解、多智能体设计模式、模型上下文协议(MCP)和工具集成、确定性编排、负责任AI考虑因素以及环境感知部署策略。介绍九大核心最佳实践。

Result: 通过多模态新闻分析和媒体生成工作流的综合案例研究，展示了这些原则的实际应用，为构建稳健、可扩展和生产就绪的智能体AI工作流提供了基础参考。

Conclusion: 本文结合架构指导、操作模式和实践实施见解，为构建稳健、可扩展和生产就绪的智能体AI工作流提供了基础参考框架。
...
### [187] [CARLoS: Retrieval via Concise Assessment Representation of LoRAs at Scale](https://arxiv.org/abs/2512.08826)
...
<details>
  <summary>Details</summary>
Motivation: 生成式组件（如LoRA）的快速扩散形成了庞大但非结构化的生态系统，现有发现方法依赖不可靠的用户描述或有偏见的流行度指标，影响了可用性。

Method: 分析650多个LoRA，在多种提示和种子下进行图像生成，使用CLIP嵌入及其与基础模型生成的差异，定义三部分表征：方向（定义语义偏移）、强度（量化效果显著性）、一致性（量化效果稳定性）。

Result: 开发了高效的检索框架，能够语义匹配文本查询到相关LoRA，同时过滤过强或不稳定的LoRA，在自动和人工评估中优于文本基线。表征还支持将强度和一致性与版权中的实质性和意志性法律概念联系起来。

Conclusion: CARLoS是一个实用的系统，不仅专注于检索，其表征框架对LoRA分析具有更广泛的相关性，特别是在版权分析方面。
...
### [188] [Interpolation in Knowledge Representation](https://arxiv.org/abs/2512.08833)
...
### [189] [EcomBench: Towards Holistic Evaluation of Foundation Agents in E-commerce](https://arxiv.org/abs/2512.08868)
...
TL;DR: EcomBench是一个针对电子商务领域的基准测试，用于评估智能体在真实电商环境中的实际能力，包含真实用户需求、多任务类别和三个难度级别。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多关注学术环境或人工设计场景，忽视了真实应用中的挑战。电子商务领域具有大量多样化用户交互、动态市场条件和真实决策任务，需要更贴近实际的评估工具。

Method: 从全球领先电商生态系统中提取真实用户需求，通过人工专家精心策划和标注，确保清晰度、准确性和领域相关性。构建包含多任务类别和三个难度级别的基准测试。

Result: 开发了EcomBench基准测试，覆盖电商场景中的多个任务类别，定义了三个难度级别，评估智能体的深度信息检索、多步推理和跨源知识整合等关键能力。

Conclusion: EcomBench通过在真实电商环境中进行基准测试，为评估智能体在现代电商中的实际能力提供了一个严格且动态的测试平台。
...
### [190] [Same Content, Different Answers: Cross-Modal Inconsistency in MLLMs](https://arxiv.org/abs/2512.08923)
...
Method: 创建了REST和REST+基准测试，包含相同语义信息在三种模态（图像、文本、混合）中的样本。评估了15个多模态大语言模型，分析了文本识别（OCR）问题、视觉特征（文本颜色、分辨率、字体）和视觉标记数量对模型性能的影响。

Result: 研究发现：1）最先进的多模态大语言模型在不同模态中无法保持一致的推理能力；2）跨模态不一致程度在不同模型间差异很大；3）将文本渲染为图像或将图像渲染为文本都无法解决不一致性问题；4）即使OCR正确，视觉特征（文本颜色和分辨率）和视觉标记数量仍影响模型性能；5）一致性得分与文本和图像之间的模态差距相关。

Conclusion: 多模态大语言模型存在显著的跨模态不一致性问题，这种不一致性与模态差距有机制性关联。需要开发能够真正统一视觉和语言表示的多模态模型。
...
### [191] [Cascaded Tightly-Coupled Observer Design for Single-Range-Aided Inertial Navigation](https://arxiv.org/abs/2512.06198)
*Oussama Sifour,Soulaimane Berkane,Abdelhamid Tayebi*

Main category: cs.RO

TL;DR: 提出一种仅使用IMU、体坐标系矢量测量和固定锚点距离测量的单测距辅助导航观测器，通过级联架构重建刚体完整状态


<details>
  <summary>Details</summary>
Motivation: 为自主导航系统开发一种轻量级、有效的状态估计方法，减少对复杂传感器系统的依赖，仅使用IMU、磁力计等简单传感器和单测距信息实现完整状态重建

Method: 首先构建扩展线性时变系统估计体坐标系位置、速度和重力方向，然后结合体坐标系矢量测量重建完整姿态，形成级联观测器架构

Result: 在一致可观性条件下建立了级联设计的几乎全局渐近稳定性，仿真研究表明该方法能在三维轨迹上准确估计位置、速度和姿态

Conclusion: 单测距辅助是一种轻量级且有效的自主导航模态，能够仅使用简单传感器实现刚体完整状态重建，具有实际应用价值
...
### [192] [Sparse Variable Projection in Robotic Perception: Exploiting Separable Structure for Efficient Nonlinear Optimization](https://arxiv.org/abs/2512.07969)
*Alan Papalia,Nikolas Sanderson,Haoyu Han,Heng Yang,Hanumant Singh,Michael Everett*

Main category: cs.RO

TL;DR: 提出一种针对具有规范对称性的机器人感知问题的变量投影方法，通过矩阵无关的舒尔补算子实现高效求解，相比现有方法加速2-35倍。


<details>
  <summary>Details</summary>
Motivation: 机器人感知中的大规模非线性最小二乘问题通常具有可分离性结构（部分变量线性出现），但现有的变量投影方法难以处理常见的规范对称性（如全局平移旋转不变性），限制了其在机器人感知中的应用。

Method: 设计了一种针对规范对称性问题的变量投影方案，联合利用可分离性和稀疏性，通过一次性预处理构建矩阵无关的舒尔补算子，支持高效计算成本、梯度和Hessian-向量积，并与标准迭代NLS求解器集成。

Result: 在SLAM、SNL和SfM的合成和真实基准测试中，相比最先进方法实现了2-35倍的运行时间加速，同时保持精度。

Conclusion: 提出的变量投影方法有效解决了机器人感知中具有规范对称性的可分离问题，显著提升了求解效率，并提供了开源实现和实验数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于生成模型的离线决策方法常常产生动力学不可行的轨迹，需要一种能够同时保证任务对齐和动力学一致性的框架。

Method: 提出模型预测扩散器(MPDiffuser)，包含三个模块：规划器生成多样化任务对齐轨迹，动力学模型确保系统动力学一致性，排序器选择符合任务目标的行为。采用交替扩散采样方案，在采样过程中交替进行规划和动力学更新以逐步优化轨迹。

Result: 在无约束(D4RL)和有约束(DSRL)离线决策基准测试中均优于现有方法，组合设计提高了样本效率，能够利用低质量数据进行动力学学习并适应新动力学。初步研究扩展到视觉控制任务，并在真实四足机器人上验证了实用性。

Conclusion: MPDiffuser通过组合模型设计有效解决了离线决策中轨迹可行性的问题，在多个基准测试中表现优异，并展示了向高维感官输入扩展和实际机器人应用的潜力。
...
### [194] [VLD: Visual Language Goal Distance for Reinforcement Learning Navigation](https://arxiv.org/abs/2512.07976)
*Lazar Milikic,Manthan Patel,Jonas Frey*

Main category: cs.RO

TL;DR: VLD框架将感知学习与策略学习解耦，先在大规模视频数据上训练自监督的距离预测器，再在仿真中用强化学习训练策略，实现可扩展的多模态导航


<details>
  <summary>Details</summary>
Motivation: 现有端到端导航方法存在仿真到现实差距问题，且带动作标签的训练数据有限。需要一种既能利用大规模视觉数据又能学习鲁棒导航行为的可扩展方案

Method: 提出视觉语言距离学习框架：1）在大规模互联网视频数据上训练自监督的距离到目标预测器，支持图像和文本目标；2）在仿真中用强化学习训练策略，使用几何距离信号并注入噪声模拟预测器不确定性；3）部署时策略使用VLD预测，继承大规模视觉训练的语义信息

Result: VLD在仿真中实现竞争性导航性能，支持灵活的目标模态（图像和文本），优于ViNT和VIP等时序距离方法，通过序数一致性评估距离函数

Conclusion: 解耦设计为可靠的多模态导航策略提供了可扩展的替代路径，结合了大规模视觉训练的语义理解和仿真中学习的鲁棒底层导航行为
...
### [195] [DIJIT: A Robotic Head for an Active Observer](https://arxiv.org/abs/2512.07998)
*Mostafa Kamali Tabrizi,Mingshi Chi,Bir Bikram Dey,Yu Qing Yuan,Markus D. Solbach,Yiqian Liu,Michael Jenkin,John K. Tsotsos*

Main category: cs.RO

TL;DR: DIJIT是一个专为移动智能体设计的双目机器人头部，具有9个机械自由度和4个光学自由度，能够模拟人类眼-头-颈运动，用于主动视觉研究和人类与机器视觉对比研究


<details>
  <summary>Details</summary>
Motivation: 设计一个能够模拟人类眼-头-颈运动的机器人头部，用于研究主动视觉、人类视觉机制以及人类与计算机视觉方法的差异

Method: 设计了具有9个机械自由度和4个光学自由度的双目机器人头部，机械性能参数接近人类水平，支持会聚立体视觉所需的运动范围（会聚、共轭、旋转），并开发了新的扫视相机运动方法

Result: DIJIT能够实现接近人类运动性能的机械运动，特别是新开发的扫视相机运动方法在准确性上接近人类扫视运动

Conclusion: DIJIT是一个功能全面的机器人头部平台，能够有效支持主动视觉研究和人类-机器视觉对比研究，为探索眼-头运动在视觉任务中的作用提供了实验工具
...
### [196] [Optimized Area Coverage in Disaster Response Utilizing Autonomous UAV Swarm Formations](https://arxiv.org/abs/2512.08028)
...
TL;DR: 该论文提出了一种用于灾害场景的无人机集群系统，通过分布式传感器延长飞行时间，采用ESDF地图进行避障，结合TSP变体优化区域覆盖，优先扫描高价值兴趣点。


<details>
  <summary>Details</summary>
Motivation: 在野火等灾害场景中，第一响应者需要高效的环境感知能力。传统单无人机系统存在飞行时间有限、数据可用性不足、碰撞风险高等问题，需要开发更可靠的集群系统来支持救援行动。

Method: 1) 采用分布式传感器部署在多架无人机上，延长整体飞行时间；2) 使用局部欧几里得符号距离场(ESDF)地图进行自主导航和障碍物避让；3) 结合旅行商问题(TSP)变体优化区域覆盖路径；4) 基于环境行为和关键基础设施预分配值来优先扫描兴趣点(POIs)。

Result: 通过不同集群规模的仿真验证，系统能够在确保无人机与障碍物之间碰撞避免的同时，最大化区域覆盖效率。系统有效减少了因碰撞导致的任务失败风险。

Conclusion: 提出的无人机集群系统能够为灾害响应提供可靠的环境感知支持，通过分布式传感、自主避障和优化覆盖策略，显著提升了救援行动的效率和安全性。
...
### [197] [An Introduction to Deep Reinforcement and Imitation Learning](https://arxiv.org/abs/2512.08052)
*Pedro Santana*

Main category: cs.RO

TL;DR: 本文介绍了深度强化学习和深度模仿学习在具身智能体中的应用，采用深度优先的方法讲解基础算法，涵盖从马尔可夫决策过程到PPO和GAIL等核心方法。


<details>
  <summary>Details</summary>
Motivation: 具身智能体（如机器人和虚拟角色）需要解决复杂的顺序决策问题，手动设计控制器困难，因此需要学习型方法。深度强化学习和深度模仿学习成为有前景的替代方案，本文旨在提供对这些方法的深入理解。

Method: 采用深度优先的文献分析方法，聚焦于少量基础算法和技术。包含所有必要的数学和机器学习概念，涵盖DRL的马尔可夫决策过程、REINFORCE、PPO，以及DIL的行为克隆、DAgger和GAIL。

Result: 本文提供了自包含的学习材料，系统介绍了DRL和DIL在具身智能体中的应用框架，为读者提供了从基础概念到前沿算法的完整知识体系。

Conclusion: 该文档为具身智能体的控制器设计提供了深度学习方法的全面介绍，强调深度理解而非广泛覆盖，是学习DRL和DIL基础知识的有效资源。
...
### [198] [Chat with UAV -- Human-UAV Interaction Based on Large Language Models](https://arxiv.org/abs/2512.08145)
*Haoran Wang,Zhuohang Chen,Guang Li,Bo Ma,Chuanghuang Li*

Main category: cs.RO

TL;DR: 本文提出了一种基于大语言模型的双智能体人机交互框架，通过任务规划智能体和执行智能体的分离设计，解决传统HUI框架在复杂场景中混合任务规划与执行困难的问题，提升了交互流畅度和任务执行灵活性。


<details>
  <summary>Details</summary>
Motivation: 无人机交互系统正从工程师驱动转向用户驱动，但用户与无人机之间缺乏共同语言，难以实现个性化交互。现有基于LLM的HUI框架在复杂场景中混合任务规划与执行存在困难，适应性不足。

Method: 提出双智能体HUI框架，构建两个独立的LLM智能体（任务规划智能体和执行智能体），应用不同的提示工程技术分别处理任务的理解、规划和执行。

Result: 构建了覆盖四个典型无人机应用场景的任务数据库，使用三个独立指标量化框架性能。用户研究实验结果表明，该框架在设定任务场景中提高了HUI的流畅度和任务执行灵活性，有效满足用户个性化需求。

Conclusion: 双智能体HUI框架通过分离任务规划与执行，利用LLM的自然语言理解能力，实现了更流畅、更灵活的个性化人机交互，为无人机在农业、航拍、物流、环境监测等领域的应用提供了更好的交互解决方案。
...
### [199] [RAVES-Calib: Robust, Accurate and Versatile Extrinsic Self Calibration Using Optimal Geometric Features](https://arxiv.org/abs/2512.08170)
*Haoxin Zhang,Shuaixin Li,Xiaozhou Zhu,Hongbo Chen,Wen Yao*

Main category: cs.RO

TL;DR: 提出一个无需标定板、兼容多种LiDAR和相机传感器的标定工具包，仅需单对激光点和相机图像即可实现鲁棒的外参标定。


<details>
  <summary>Details</summary>
Motivation: 现有的LiDAR-相机标定方法通常需要标定板或初始变换参数，限制了在无目标环境中的适用性。需要一种更灵活、鲁棒的标定方法，能够适应大范围的外参偏差。

Method: 使用Gluestick流水线建立2D-3D点和线特征对应关系，获得鲁棒的自动初始猜测；通过定量分析特征分布对标定结果的影响，自适应加权每个特征的代价，过滤劣质特征的不良影响。

Result: 在室内外多种LiDAR-相机传感器上的大量实验验证表明，该方法相比现有技术具有更优的鲁棒性和精度。

Conclusion: 开发了一个用户友好的LiDAR-相机标定工具包，无需标定板且兼容多种传感器，通过特征分布分析和自适应加权实现了高精度外参标定，代码已开源。
...
### [200] [Ground Slow, Move Fast: A Dual-System Foundation Model for Generalizable Vision-and-Language Navigation](https://arxiv.org/abs/2512.08186)
...
TL;DR: DualVLN提出首个双系统视觉语言导航基础模型，通过分离高层推理与低层执行来解决传统端到端方法的碎片化动作、高延迟和动态避障问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航方法通常采用端到端管道，直接将视觉语言输入映射为短期离散动作，导致动作碎片化、延迟高，且难以应对动态障碍物避障等现实挑战。

Method: 提出双系统架构：System 2（基于VLM的全局规划器）通过图像基础推理预测中期路径点目标；System 1（轻量级多模态条件扩散Transformer策略）利用System 2提供的显式像素目标和潜在特征生成平滑准确的轨迹。
...
### [201] [Embodied Tree of Thoughts: Deliberate Manipulation Planning with Embodied World Model](https://arxiv.org/abs/2512.08188)
*Wenjiang Xu,Cindy Wang,Rui Fang,Mingkang Zhang,Lusong Li,Jing Xu,Jiayuan Gu,Zecui Zeng,Rui Chen*

Main category: cs.RO

TL;DR: EToT提出了一种基于物理模拟器的Real2Sim2Real规划框架，通过先验分支和反思分支的树搜索机制，解决机器人操作规划中的物理一致性和长期约束问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于视频生成的世界模型缺乏严格的物理基础，容易产生幻觉，并且在长期物理约束下难以保持一致性，这限制了它们在机器人操作规划中的应用。

Method: 提出Embodied Tree of Thoughts (EToT)框架，使用基于物理的交互式数字孪生作为具身世界模型。通过两种协同机制进行树搜索：1) 先验分支：基于语义和空间分析生成多样化候选执行路径；2) 反思分支：利用视觉语言模型诊断模拟器中的执行失败，并通过纠正动作迭代优化规划树。

Result: 在短期和长期操作任务套件上验证了EToT，该框架在预测物理动态和适应潜在失败方面始终优于基线方法。

Conclusion: 通过将高级推理基于物理模拟器，EToT确保生成的计划符合刚体动力学和碰撞约束，为机器人操作规划提供了物理基础扎实的解决方案。
...
### [202] [SensHRPS: Sensing Comfortable Human-Robot Proxemics and Personal Space With Eye-Tracking](https://arxiv.org/abs/2512.08518)
*Nadezhda Kushina,Ko Watanabe,Aarthi Kannan,Ashita Ashok,Andreas Dengel,Karsten Berns*

Main category: cs.RO

TL;DR: 研究使用眼动追踪和主观报告评估人与机器人互动中的舒适度，发现决策树模型在预测舒适度方面表现最佳，表明人机互动与人际互动的舒适度阈值存在差异。


<details>
  <summary>Details</summary>
Motivation: 社交机器人需要适应人类的近体空间规范以确保用户舒适度和参与度。虽然先前研究表明眼动追踪特征能可靠估计人际互动中的舒适度，但这些特征在人机互动中的适用性尚未探索。
...
Conclusion: 人机互动中的生理舒适度阈值与人际互动动态存在差异，可以使用可解释的逻辑模型有效建模。
...
### [203] [High-Performance Dual-Arm Task and Motion Planning for Tabletop Rearrangement](https://arxiv.org/abs/2512.08206)
...
<details>
  <summary>Details</summary>
Motivation: 解决双机械臂在近距离协同工作时，面对起始和目标配置高度纠缠的物体重排问题。传统方法难以处理这种复杂的依赖关系和同步运动规划挑战。

Method: SDAR包含两个核心组件：SDAR-T（依赖驱动的任务规划器）和SDAR-M（同步双臂运动规划器）。SDAR-T通过分解全局物体依赖图生成优化的双臂任务计划；SDAR-M采用分层运动规划策略，利用GPU SIMD工具筛选最佳同步运动计划。

Result: 在复杂、非单调、长视野的桌面重排任务中实现100%成功率，解决方案质量远超先前最先进方法。在UR-5e双臂机器人上的实验证实了SDAR能够直接可靠地迁移到实际硬件。

Conclusion: SDAR框架通过紧密集成的任务和运动规划，有效解决了双机械臂协同重排的挑战，在复杂场景中表现出卓越的性能和实际部署能力。
...
### [204] [Semantic-Metric Bayesian Risk Fields: Learning Robot Safety from Human Videos with a VLM Prior](https://arxiv.org/abs/2512.08233)
*Timothy Chen,Marcus Dominguez-Kuhne,Aiden Swann,Xu Liu,Mac Schwager*

Main category: cs.RO

TL;DR: 该研究提出了一种从人类安全演示视频中提取隐式风险模型的贝叶斯框架，通过语义条件和空间变化的参数化方法学习人类对齐的风险评估，用于机器人规划任务。


<details>
  <summary>Details</summary>
Motivation: 人类对安全的理解不是二元信号，而是连续、上下文相关且空间依赖的风险概念。虽然风险是主观的，但人类会形成理性的心理模型来指导动态环境中的行动选择。当前自主系统缺乏这种人类式的风险内部化能力。

Method: 提出贝叶斯风险框架：先验由预训练的视觉语言模型提供，似然函数通过学习ViT将预训练特征映射到像素对齐的风险值。系统输入RGB图像和查询对象字符串，输出像素密集的风险图像，可用于机器人规划或轨迹优化。

Result: 框架能够生成与人类偏好对齐的上下文风险评估，可泛化到新对象和场景，并支持快速适应额外观察或常识规则。下游应用表明可作为视觉运动规划的价值学习器或与传统轨迹优化算法结合使用。

Conclusion: 该框架是使自主系统内部化人类式风险的重要一步，具有扩展到更大训练数据集的潜力，为机器人提供更人性化的风险评估和决策能力。
...
### [205] [Zero-Splat TeleAssist: A Zero-Shot Pose Estimation Framework for Semantic Teleoperation](https://arxiv.org/abs/2512.08271)
*Srijan Dokania,Dharini Raghavan*

Main category: cs.RO

TL;DR: Zero-Splat TeleAssist是一个零样本传感器融合系统，将普通监控摄像头视频流转换为共享的6自由度世界模型，用于多边远程操作，无需标记或深度传感器。


<details>
  <summary>Details</summary>
Motivation: 解决传统远程操作系统中需要专用传感器、标记物或复杂校准的问题，利用现有监控基础设施实现低成本、易部署的多机器人远程协作。

Method: 整合视觉语言分割、单目深度估计、加权PCA姿态提取和3D高斯泼溅技术，从普通监控视频流中重建6自由度世界模型。

Result: 系统能够为每个操作员提供多个机器人的实时全局位置和方向信息，在交互为中心的远程操作设置中实现有效协作。

Conclusion: Zero-Splat TeleAssist展示了利用现有监控基础设施实现低成本、零样本多机器人远程操作的可行性，为机器人协作提供了新的解决方案。
...
### [206] [Robust Finetuning of Vision-Language-Action Robot Policies via Parameter Merging](https://arxiv.org/abs/2512.08333)
*Yajat Yadav,Zhiyuan Zhou,Andrew Wagenmaker,Karl Pertsch,Sergey Levine*

Main category: cs.RO

TL;DR: 提出一种简单有效的模型权重插值方法，在微调新任务时保留通用策略的泛化能力，实现单一策略同时掌握新技能和原有通用能力。


<details>
  <summary>Details</summary>
Motivation: 通用机器人策略在训练数据未覆盖的新任务上表现不佳，微调时容易过拟合到特定演示，既失去原有通用能力，又无法在新任务上良好泛化。

Method: 采用模型权重插值策略，将微调后的模型权重与预训练模型权重进行插值，生成单一模型继承两者的优势。

Result: 模型合并方法在模拟和真实世界实验中均表现出色，在新任务的分布外变体上优于预训练和微调模型，并能实现持续技能学习而不牺牲原有能力。

Conclusion: 简单的模型权重插值方法能有效保留通用策略的泛化能力，使单一策略既能学习新任务又能保持原有通用能力，支持持续学习。
...
### [207] [Learning Robot Manipulation from Audio World Models](https://arxiv.org/abs/2512.08405)
*Fan Zhang,Michael Gienger*

Main category: cs.RO

TL;DR: 提出一种生成式潜在流匹配模型来预测未来音频观测，使机器人策略能够基于长期音频后果进行推理，在需要感知真实世界音频或音乐信号的操作任务中表现优于无未来预测的方法。


<details>
  <summary>Details</summary>
Motivation: 许多机器人学习任务需要多模态推理，例如灌水任务中仅凭视觉信息可能模糊或不完整，需要基于音频的时间演变进行推理，考虑其物理特性和音高模式。

Method: 提出生成式潜在流匹配模型来预测未来音频观测，将该模型集成到机器人策略中，使系统能够推理长期后果。

Result: 在两个需要感知真实世界音频或音乐信号的操作任务中，该系统表现出优于无未来预测方法的性能，成功完成需要多模态输入和准确预测未来音频状态的任务。
...
### [208] [A Multi-Agent LLM Framework for Design Space Exploration in Autonomous Driving Systems](https://arxiv.org/abs/2512.08476)
*Po-An Shih,Shao-Hua Wang,Yung-Che Li,Chia-Heng Tu,Chih-Han Chang*

Main category: cs.RO

TL;DR: 提出基于多智能体大语言模型的自动驾驶系统设计空间探索框架，通过多模态推理和3D仿真工具自动分析执行输出，在机器人出租车案例中相比遗传算法基线找到更多帕累托最优解


<details>
  <summary>Details</summary>
Motivation: 传统设计空间探索方法难以处理多模态执行输出和复杂性能权衡，且需要人工干预评估正确性。自动驾驶系统设计需要在多样化环境条件下高效探索硬件/软件配置空间

Method: 开发基于多智能体大语言模型的DSE框架，集成多模态推理、3D仿真和性能分析工具。利用专门的LLM智能体处理用户输入解释、设计点生成、执行编排以及视觉和文本执行输出的分析

Result: 在SAE Level 4自动驾驶应用（机器人出租车）案例研究中，相比遗传算法基线，在相同探索预算下识别出更多帕累托最优、成本效益更高的解决方案，并减少了导航时间

Conclusion: 该框架展示了基于LLM的方法在设计空间探索中的效率，为自动驾驶系统的设计自动化铺平了道路
...
### [209] [Prospect Theory in Physical Human-Robot Interaction: A Pilot Study of Probability Perception](https://arxiv.org/abs/2512.08481)
*Yixiang Lin,Tiancheng Yang,Jonathan Eden,Ying Tan*

Main category: cs.RO

TL;DR: 该研究通过物理耦合目标到达任务，探索人类在不确定环境下的决策行为，发现存在两种不同的行为模式群体，强调需要更可解释的行为模型来指导自适应机器人控制器的设计。
...
Result: 发现两种明显的行为集群："权衡"组根据干扰可能性调整物理响应，"总是补偿"组则表现出强烈的风险规避，不考虑概率。这表明人类在pHRI中的决策高度个性化，且对概率的感知可能与真实值不同。

Conclusion: 研究强调需要更可解释的行为模型（如累积前景理论）来准确捕捉这些行为，为未来自适应机器人控制器的设计提供依据，以更好地适应个体差异和不确定性感知。
...
### [210] [vEDGAR - Can CARLA Do HiL?](https://arxiv.org/abs/2512.08541)
*Nils Gehrke,David Brecht,Dominik Kulmer,Dheer Patel,Frank Diermeyer*

Main category: cs.RO

TL;DR: 该研究开发了一个基于CARLA的硬件在环仿真框架vEDGAR，用于测试自动驾驶软件在专用硬件上的实时性能，填补了CARLA在完整传感器和执行器堆栈实时仿真方面的空白。


<details>
  <summary>Details</summary>
Motivation: CARLA等开源仿真器在自动驾驶算法训练和评估中广泛应用，但缺乏对完整传感器和执行器堆栈的实时硬件在环测试能力，这限制了其在完整开发流程中的一致性评估作用。

Method: 首先推导需求并指定仿真架构，然后实现vEDGAR软件框架，该框架基于修改的CARLA分支，支持自动驾驶软件在专用硬件上的实时硬件在环测试。

Result: 开发了开源的vEDGAR框架和修改的CARLA分支，评估了CARLA在自动驾驶车辆硬件在环测试中的适用性，为整个开发流程提供了一致的评估工具。

Conclusion: vEDGAR框架成功扩展了CARLA的硬件在环测试能力，使其能够作为自动驾驶功能整个开发过程中的一致性评估工具，填补了现有仿真工具的空白。
...
### [211] [Bridging Scale Discrepancies in Robotic Control via Language-Based Action Representations](https://arxiv.org/abs/2512.08548)
*Yuchi Zhang,Churui Sun,Shiqi Liang,Diyuan Liu,Chao Ji,Wei-Nan Zhang,Ting Liu*

Main category: cs.RO

TL;DR: 提出一种基于语义的语言化动作表示方法，通过忽略数值尺度、强调方向性来缓解机器人动作数据中的分布偏移问题，提高预训练模型的泛化能力和可迁移性。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的端到端机器人操作架构面临严重分布偏移问题，主要源于不同机器人平台和任务间动作命令的数值变化巨大，阻碍了预训练知识的有效迁移。

Method: 提出语义基础的语言化动作表示，将动作归一化用于高效预训练。该运动表示忽略数值尺度效应，强调方向性，从而缓解分布偏移，缩小动作标记与标准词汇标记之间的特征距离。

Result: 在两个基准测试上的多任务实验表明，该方法显著提高了机器人操作任务的泛化性能和可迁移性。

Conclusion: 通过语义基础的语言化动作表示，能够有效缓解机器人动作数据中的分布偏移问题，为跨平台、跨任务的机器人操作预训练提供了更通用的表示方法。
...
### [212] [RVC-NMPC: Nonlinear Model Predictive Control with Reciprocal Velocity Constraints for Mutual Collision Avoidance in Agile UAV Flight](https://arxiv.org/abs/2512.08574)
*Vit Kratky,Robert Penicka,Parakh M. Gupta,Ondrej Prochazka,Martin Saska*

Main category: cs.RO

TL;DR: 本文提出了一种基于非线性模型预测控制(NMPC)和时间依赖互惠速度约束(RVCs)的无人机互避碰撞方法，无需过多通信，能在100Hz频率下运行，在10架无人机、25m/s速度的挑战性场景中实现无碰撞导航。


<details>
  <summary>Details</summary>
Motivation: 现有无人机互避碰撞方法通常需要大量通信，而本文旨在开发一种仅依赖可观测信息、无需过多通信的高效避碰方法，适用于敏捷无人机飞行。

Method: 采用非线性模型预测控制(NMPC)结合时间依赖互惠速度约束(RVCs)，通过计算高效的RVCs算法，将这些约束直接集成到控制器级别的NMPC问题中，整个流程能在100Hz频率下运行。

Result: 在模拟10架无人机、速度达25m/s的真实场景中，以及在加速度达30m/s²的真实实验中，所有试验均实现无碰撞导航。与现有技术相比，在挑战性场景中飞行时间减少31%。

Conclusion: 提出的基于NMPC和RVCs的方法能够实现高效、无需过多通信的无人机互避碰撞，在高速敏捷飞行场景中表现出色，显著优于现有方法。
...
### [213] [Mind to Hand: Purposeful Robotic Control via Embodied Reasoning](https://arxiv.org/abs/2512.08580)
*Peijun Tang,Shangjin Xie,Binyan Sun,Baifu Huang,Kuncheng Luo,Haotian Yang,Weiqi Jin,Jianan Wang*

Main category: cs.RO

TL;DR: Lumo-1是一个统一的视觉-语言-动作模型，将机器人推理与动作相结合，通过三阶段预训练流程和强化学习，在具身推理和机器人控制任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 人类基于上下文和意图行动，推理在其中起核心作用。虽然互联网规模的数据使AI系统具备了广泛的推理能力，但将这些能力与物理动作相结合仍然是一个重大挑战。

Method: 采用三阶段预训练流程：1) 在精选的视觉-语言数据上继续预训练，增强具身推理能力；2) 在跨具身机器人数据和视觉-语言数据上进行联合训练；3) 在Astribot S1双手机器人收集的轨迹上进行带推理过程的动作训练。最后集成强化学习来优化推理-动作一致性。

Result: Lumo-1在具身视觉-语言推理方面取得显著性能提升，在真实世界评估中超越强基线，在广泛的机器人任务中表现出色，特别是在长时程任务和需要策略、概念和空间推理的人类自然指令响应方面。

Conclusion: Lumo-1成功地将机器人推理与动作统一起来，通过渐进式训练方法实现了强大的泛化能力，为通用机器人控制提供了关键的具身推理组件。
...
### [214] [Multi-Task Bayesian Optimization for Tuning Decentralized Trajectory Generation in Multi-UAV Systems](https://arxiv.org/abs/2512.08630)
*Marta Manzoni,Alessandro Nazzari,Roberto Rubinacci,Marco Lovera*

Main category: cs.RO

TL;DR: 该研究使用多任务贝叶斯优化来调优多无人机系统中的分散式轨迹生成算法，将每个任务视为特定无人机交互数量的轨迹生成场景，通过多任务高斯过程建模任务间关系，比较了优化所有任务平均任务时间和单独优化每个任务两种策略。


<details>
  <summary>Details</summary>
Motivation: 在多无人机系统中，分散式轨迹生成算法的参数调优是一个复杂问题，特别是当无人机数量增加时，优化成本会急剧上升。需要一种能够有效利用不同场景（不同无人机交互数量）之间共享信息的方法来提高优化效率。

Method: 采用多任务贝叶斯优化框架，使用多任务高斯过程来建模不同轨迹生成场景（任务）之间的关系，这些场景由特定的无人机交互数量定义。比较了两种优化策略：1）优化所有任务的平均任务时间；2）单独优化每个任务。

Result: 通过全面的仿真实验发现：单任务优化随着群体规模增长能够获得逐渐缩短的任务时间，但需要比平均任务方法显著更多的优化时间。

Conclusion: 多任务贝叶斯优化为多无人机系统的轨迹生成算法调优提供了一种有效方法，能够在优化效率和性能之间进行权衡。平均任务优化策略在时间效率上具有优势，而单任务优化在性能上可能更优但成本更高。
...
### [215] [A Sensor-Aware Phenomenological Framework for Lidar Degradation Simulation and SLAM Robustness Evaluation](https://arxiv.org/abs/2512.08653)
*Doumegna Mawuto Koudjo Felix,Xianjia Yu,Zhuo Zou,Tomi Westerlund*

Main category: cs.RO

TL;DR: 本文提出了一个传感器感知的现象学框架，用于在真实点云上直接模拟可解释的激光雷达退化，实现对SLAM系统的可控、可重复压力测试。


<details>
  <summary>Details</summary>
Motivation: 激光雷达SLAM系统对遮挡、噪声和视场退化等恶劣条件高度敏感，但现有的鲁棒性评估方法要么缺乏物理基础，要么无法捕捉传感器特定行为。

Method: 开发了一个传感器感知的现象学框架，直接在真实点云上模拟可解释的激光雷达退化，包括结构化丢弃、视场减少、高斯噪声、遮挡掩蔽、稀疏化和运动失真。框架具有自主主题和传感器检测、模块化配置（四个严重级别）和实时性能（每帧小于20毫秒）。

Result: 在三种激光雷达架构和五种最先进的SLAM系统上进行实验验证，揭示了由传感器设计和环境上下文塑造的独特鲁棒性模式。开源实现为在物理意义明确的退化场景下基准测试激光雷达SLAM提供了实用基础。

Conclusion: 该框架为激光雷达SLAM系统在物理意义明确的退化场景下提供了可控、可重复的压力测试方法，填补了现有评估方法的空白，有助于理解传感器设计和环境因素对SLAM鲁棒性的影响。
...
### [216] [Sim2Swim: Zero-Shot Velocity Control for Agile AUV Maneuvering in 3 Minutes](https://arxiv.org/abs/2512.08656)
*Lauritz Rismark Fosso,Herman Biørn Amundsen,Marios Xanthidis,Sveinung Johan Ohrem*

Main category: cs.RO

TL;DR: 提出Sim2Swim方法，首个基于零样本sim2real深度强化学习的AUV速度控制器，仅需3分钟训练即可实现6自由度敏捷机动和路径跟踪。


<details>
  <summary>Details</summary>
Motivation: 全向自主水下航行器具备硬件能力进行平移和旋转自由度的敏捷机动，但由于复杂的水静力学和水动力学、参数不确定性以及载荷变化导致的动态特性频繁变化，控制具有挑战性。现有方法需要针对特定平台配置精心调校控制器，且在不同载荷和水动力条件下需要重新调校，导致6自由度敏捷机动在实践中很少使用。

Method: 提出Sim2Swim方法，受最先进的DRL位置控制启发，利用领域随机化和大规模并行训练，无需后处理或调校即可为不同特性的AUV收敛到可现场部署的控制策略。这是首个基于零样本sim2real深度强化学习的速度控制器。

Result: 该方法在池试验中进行了广泛验证，展示了在各种配置下对高度敏捷运动的鲁棒控制。训练时间仅需3分钟，即可实现路径跟踪和6自由度敏捷机动。

Conclusion: Sim2Swim方法通过零样本sim2real深度强化学习，解决了AUV控制中的参数不确定性和载荷变化问题，实现了快速训练和现场部署的6自由度敏捷机动控制，为实际应用提供了可行的解决方案。
...
### [217] [Ergodic Trajectory Planning with Dynamic Sensor Footprints](https://arxiv.org/abs/2512.08661)
*Ziyue Zheng,Yongce Liu,Hesheng Wang,Zhongqiang Ren*

Main category: cs.RO

TL;DR: 本文提出了一种考虑动态变化传感器足迹的遍历规划方法，用于信息收集任务，相比传统方法能同时优化轨迹和传感器足迹，获得更好的遍历性。


<details>
  <summary>Details</summary>
Motivation: 现有的遍历规划方法通常假设点传感器或固定形状分辨率的传感器足迹，而实际中传感器足迹会随时间动态变化（如无人机摄像头随高度、姿态变化）。这种简化限制了信息收集任务的性能。

Method: 提出了一种考虑动态传感器足迹的新度量标准，分析了理论局部最优条件，并开发了数值轨迹优化算法，能够同时优化机器人轨迹和传感器足迹。

Result: 实验结果表明，所提方法能同时优化轨迹和传感器足迹，相比传统方法获得高达一个数量级更好的遍历性。在多无人机系统中成功应用于三维空间物体的遍历覆盖。

Conclusion: 本文提出的动态传感器足迹遍历规划方法克服了传统方法的局限性，为实际机器人系统中的信息收集任务提供了更有效的解决方案。
...
### [218] [Non Normalized Shared-Constraint Dynamic Games for Human-Robot Collaboration with Asymmetric Responsibility](https://arxiv.org/abs/2512.08688)
*Mark Pustilnik,Francesco Borrelli*

Main category: cs.RO

TL;DR: 提出了一种用于共享工作空间中人与机器人协同导航的动态博弈框架，引入非归一化均衡结构来处理共享安全约束


<details>
  <summary>Details</summary>
Motivation: 在存在障碍物的共享工作空间中，需要解决人与机器人协同导航的问题，既要满足共同的安全约束（如避障和间距保持），又要完成共同任务。传统方法难以处理不同智能体对安全约束的不同贡献程度。

Method: 提出动态博弈框架，引入非归一化均衡结构来处理共享安全约束，允许人和机器人以不同努力程度来满足安全要求。将该均衡结构嵌入到滚动时域最优控制方案中。

Result: 该方法能够处理共享工作空间中人与机器人的协同导航问题，使两个智能体能够以不同的贡献程度共同满足安全约束，同时完成共同任务。

Conclusion: 提出的非归一化均衡结构为共享约束下的协同导航提供了有效的解决方案，允许智能体根据自身能力以不同方式满足安全要求，提高了协同导航的灵活性和实用性。
...
### [219] [A Multi-Robot Platform for Robotic Triage Combining Onboard Sensing and Foundation Models](https://arxiv.org/abs/2512.08754)
...
TL;DR: 开发了一个用于大规模伤亡事件远程初级分诊的异构机器人系统，采用无人机和地面机器人协同工作，定位受害者、评估伤情并优先安排医疗援助


<details>
  <summary>Details</summary>
Motivation: 在大规模伤亡事件中，第一响应者面临生命危险，需要一种能够远程进行初级分诊的系统，以保护救援人员安全并最大化挽救生命

Method: 采用协调的空地机器人团队：无人机负责识别受害者并提供俯视图，地面机器人配备专业传感器测量生命体征并检测定位物理伤害，系统实现完整的分诊流程

Result: 作为DARPA分诊挑战赛的一部分开发，展示了多机器人系统如何增强人类在灾难响应场景中的能力，以最大化挽救生命

Conclusion: 异构机器人系统能够有效支持大规模伤亡事件的远程初级分诊，保护第一响应者安全，提高救援效率
...
### [220] [IPPO Learns the Game, Not the Team: A Study on Generalization in Heterogeneous Agent Teams](https://arxiv.org/abs/2512.08877)
*Ryan LeRoy,Jack Kolb*

Main category: cs.RO

TL;DR: 研究比较了自学习PPO智能体与旋转策略训练方法在异构多智能体环境中的泛化能力，发现标准自学习基线IPPO在未见过的队友策略上表现与多样化训练的RPT相当。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究在异构多智能体强化学习中，自学习PPO智能体是否真正学习了基于游戏本质的通用协调策略，还是仅仅过拟合了训练伙伴的行为模式。

Method: 使用异构多智能体挑战环境，引入旋转策略训练方法，在训练过程中轮换不同学习算法的异构队友策略，让智能体接触更广泛的伙伴策略。

Result: 当与未参与训练的DDQN队友策略合作时，RPT方法的表现与标准自学习基线IPPO相当，表明IPPO基线虽然训练时没有接触队友多样性，但仍能很好地泛化到新的队友算法。

Conclusion: 在异构多智能体设置中，简单的IPPO基线可能已经具备了足够的泛化能力，能够应对未见过的队友策略，这与多样化训练方法的设计目标相当。
...
### [221] [MixLM: High-Throughput and Effective LLM Ranking via Text-Embedding Mix-Interaction](https://arxiv.org/abs/2512.07846)
...
TL;DR: MixLM是一种基于大语言模型的排序框架，通过将物品描述编码为少量嵌入令牌来大幅减少输入上下文长度，从而显著提升系统吞吐量，同时保持语义排序性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在语义理解方面表现出色，但在工业级延迟和吞吐要求下计算开销过高。特别是交叉编码器排序系统需要处理长上下文预填充工作负载，因为模型需要同时处理用户、查询和物品信息。

Method: 提出MixLM框架，采用混合交互方式：将物品描述编码为少量嵌入令牌并存储在近线缓存中，在线推理时使用这些编码后的物品描述，从而将物品长度从数千个文本令牌减少到几个嵌入令牌。

Result: 在LinkedIn真实搜索应用中部署，在相同延迟预算下吞吐量提升10.0倍，同时保持相关性指标。效率提升使LLM驱动的搜索能够全流量部署，在线A/B测试中每日活跃用户显著增加0.47%。

Conclusion: MixLM通过创新的混合交互设计，有效解决了LLM排序系统在工业部署中的效率瓶颈，实现了语义性能与系统效率的良好平衡，为大规模LLM搜索应用提供了可行的解决方案。
...
### [222] [A Comparative Study of Retrieval Methods in Azure AI Search](https://arxiv.org/abs/2512.08078)
...
<details>
  <summary>Details</summary>
Motivation: 律师们希望超越传统的关键词和语义搜索，利用大语言模型通过自然语言提问来更高效地获取文档审查中的关键信息。电子取证中的早期案件评估需要快速理解数据、确定关键事实和风险，因此需要有效的检索策略。

Method: 研究在微软Azure的检索增强生成框架下，比较了Azure AI Search的五种检索方法：关键词检索、语义检索、向量检索、混合检索和混合语义检索。评估了每种方法在AI生成回答中的准确性、相关性和一致性。

Result: 研究展示了不同检索方法在准确性、相关性和一致性方面的表现差异，为法律从业者提供了选择RAG配置的实证依据。

Conclusion: 法律从业者可以利用本研究的结果来优化未来RAG配置的选择，从而提高电子取证早期案件评估的效率和效果。
...
### [223] [Leveraging Machine Learning and Large Language Models for Automated Image Clustering and Description in Legal Discovery](https://arxiv.org/abs/2512.08079)
*Qiang Mao,Fusheng Wei,Robert Neary,Charles Wang,Han Qin,Jianping Zhang,Nathaniel Huber-Fliflet*

Main category: cs.IR

TL;DR: 本文系统研究了自动聚类描述生成方法，通过整合图像聚类、图像描述和LLM技术，评估了采样策略、提示技术和生成方法三个关键维度，为法律发现等领域的图像集合自动化组织提供了实用指导。


<details>
  <summary>Details</summary>
Motivation: 数字图像创建和保留的快速增长给法律发现、数字存档和内容管理带来了巨大挑战。企业和法律团队需要在严格的时间压力下组织、分析和从大型图像集合中提取有意义的见解，这使得人工审查不切实际且成本高昂，因此迫切需要能够高效组织和描述大规模图像数据集的自动化方法。

Method: 采用K-means聚类将图像分组为20个视觉连贯的聚类，使用Azure AI Vision API生成基础描述。评估三个关键维度：1) 图像采样策略（随机、基于质心、分层、混合和基于密度采样与使用所有聚类图像对比）；2) 提示技术（标准提示与思维链提示对比）；3) 描述生成方法（基于LLM的生成与传统TF-IDF和基于模板的方法对比）。使用语义相似性和覆盖度指标评估描述质量。

Result: 结果显示，每个聚类使用20张图像的战略性采样在显著降低计算成本的同时，与穷尽包含方法表现相当，只有分层采样显示出适度的性能下降。基于LLM的方法始终优于TF-IDF基线，且对于此任务，标准提示优于思维链提示。

Conclusion: 这些发现为部署可扩展、准确的聚类描述系统提供了实用指导，支持法律发现和其他需要自动化组织大型图像集合的领域的高容量工作流程。战略性采样与基于LLM的标准提示相结合的方法能够有效平衡计算成本与描述质量。
...
### [224] [Exploiting the Randomness of Large Language Models (LLM) in Text Classification Tasks: Locating Privileged Documents in Legal Matters](https://arxiv.org/abs/2512.08083)
*Keith Huffman,Jianping Zhang,Nathaniel Huber-Fliflet,Fusheng Wei,Peter Gronvall*

Main category: cs.IR

TL;DR: 本文通过实证研究探讨了随机性在基于LLM的法律特权文件检测分类中的作用，发现LLM能有效识别特权文件，随机性控制参数对分类性能影响有限，而利用随机性的方法能显著提升准确性并增强企业信心。


<details>
  <summary>Details</summary>
Motivation: 在法律事务中，文本分类模型常用于筛选大型数据集以查找符合特定标准（如法律特权通信和律师指导文件）的文档。虽然大语言模型在此领域表现出色，但随机性在LLM分类中的作用尚未得到充分研究，特别是在涉及法律特权文件检测这一敏感领域。

Method: 本文采用实证研究方法，从四个维度探讨随机性在LLM分类中的作用：(1) LLM识别法律特权文件的有效性；(2) 随机性控制参数对分类输出的影响；(3) 随机性对整体分类性能的影响；(4) 利用随机性提升准确性的方法论。研究聚焦于律师-客户特权文件检测这一具体应用场景。

Result: 实验结果显示：1) LLM能有效识别特权文件；2) 随机性控制参数对分类性能影响有限；3) 提出的利用随机性的方法论能显著提升准确性。该方法还能增强企业对LLM输出的信心，特别是在制裁合规流程中，减少输出变异性有助于建立内部和监管机构对LLM决策的信任。

Conclusion: 研究表明LLM在法律特权文件检测中具有实用价值，随机性控制参数的影响有限，而系统性地利用随机性可以提升分类准确性。更重要的是，这种方法论能增强企业在合规流程中对LLM输出的信心，随着组织越来越多地依赖LLM增强合规工作流，减少输出变异性对于建立信任至关重要。
...
### [225] [Ontology-Based Knowledge Graph Framework for Industrial Standard Documents via Hierarchical and Propositional Structuring](https://arxiv.org/abs/2512.08398)
*Jiin Park,Hyuna Jeon,Yoonseo Lee,Jisu Hong,Misuk Kim*

Main category: cs.IR

TL;DR: 本文提出了一种基于本体的知识图谱构建方法，专门针对工业标准文档中的复杂结构（表格、条件规则、数值计算等），通过层次语义结构分解和LLM三元组提取，实现了对领域特定语义的有效表示。


<details>
  <summary>Details</summary>
Motivation: 工业标准文档包含大量技术信息和复杂规则，以高度结构化的格式呈现（表格、适用范围、约束、例外、数值计算等），这使得知识图谱构建特别具有挑战性。传统方法难以有效反映这些文档的层次和逻辑结构。

Method: 1) 将文档组织为层次语义结构；2) 将句子和表格分解为源自条件和数值规则的原子命题；3) 通过基于LLM的三元组提取将其整合到本体-知识图谱中；4) 实现了一个本体感知的KG-RAG框架进行对比评估。

Result: 实验结果表明，与现有的KG-RAG方法相比，该方法在所有问答类型上都取得了显著的性能提升。验证了即使在条件、约束和适用范围交织的工业文档中，可靠且可扩展的知识表示也是可行的。
...
### [226] [VI-MMRec: Similarity-Aware Training Cost-free Virtual User-Item Interactions for Multimodal Recommendation](https://arxiv.org/abs/2512.08702)
...
Method: 提出VI-MMRec框架，通过两种策略构建虚拟用户-物品交互：1) Overlay策略独立聚合模态特定相似性以保留模态特定用户偏好；2) Synergistic策略整体融合跨模态相似性以捕捉互补用户偏好。采用统计信息权重分配机制，根据数据集特定模态相关性自适应分配权重。

Result: 在六个真实世界数据集上使用七个最先进的多模态推荐模型进行综合实验，验证了VI-MMRec的有效性。

Conclusion: VI-MMRec作为一个即插即用框架，无需修改核心架构即可无缝集成到现有模型中，增强其性能，且训练过程无额外开销，便于实际部署。
