...
Motivation: 大型视觉语言模型推理成本过高，现有剪枝方法会导致显著的性能下降，因为不可逆的token移除导致隐藏状态分布偏移

Method: 提出RCP框架，包含：1）基于LLM内在注意力的交叉注意力剪枝器预测累积掩码；2）延迟修复适配器缓存剪枝token信息并应用FiLM调制；3）使用修复损失匹配完整token教师模型的统计特性

Result: 在LVLM基准测试中，RCP可移除高达88.9%的视觉token，减少85.7%的FLOPs，仅带来边际平均准确率下降，优于现有无需微调原始模型的方法

Conclusion: RCP通过累积剪枝和延迟修复机制有效解决了视觉token剪枝中的分布偏移问题，实现了高效推理与性能的良好平衡
...
TL;DR: Video-MME-v2是一个新的视频理解基准测试，采用三级渐进层次评估模型能力，通过组基非线性评估策略确保答案的一致性和推理连贯性，旨在解决现有基准测试分数虚高与实际能力不符的问题。
...
Method: 1. 设计三级渐进层次：从多点视觉信息聚合，到时序动态建模，再到复杂多模态推理；2. 提出组基非线性评估策略：强调相关查询间的一致性和多步推理的连贯性，惩罚碎片化或猜测性正确；3. 严格人工标注流程：12名标注员和50名独立评审员参与，3300人工小时，最多5轮质量保证。

Result: 实验显示当前最佳模型Gemini-3-Pro与人类专家存在显著差距；发现明显的层次瓶颈：视觉信息聚合和时序建模的错误会传播并限制高层推理；基于思考的推理高度依赖文本线索，有字幕时性能提升但纯视觉环境下有时会下降。

Conclusion: Video-MME-v2通过暴露现有模型的局限性，为下一代视频多模态大语言模型的发展建立了严格的新测试平台，旨在推动更鲁棒和可靠视频理解模型的发展。
...
TL;DR: ID-Sim是一个前馈度量标准，旨在反映人类对身份的选择性敏感度，通过高质量训练集和生成合成数据构建，并在统一基准上评估与人类标注的一致性。


<details>
  <summary>Details</summary>
Motivation: 人类在身份识别方面具有卓越的选择性敏感度，能够区分高度相似的身份，而视觉模型在这方面表现不佳。身份相关任务（如个性化图像生成）的进展受到缺乏身份聚焦评估指标的阻碍。

Method: 构建ID-Sim度量标准：1）策划高质量训练集，涵盖多样现实领域；2）使用生成合成数据提供受控的细粒度身份和上下文变化；3）在新的统一评估基准上评估与人类标注的一致性。

Result: 提出了ID-Sim度量标准，该标准旨在忠实反映人类对身份的选择性敏感度，并在身份聚焦的识别、检索和生成任务中评估与人类标注的一致性。

Conclusion: ID-Sim有助于促进视觉模型在身份相关任务上的进展，通过提供能够反映人类选择性敏感度的评估指标，填补了当前身份聚焦评估指标的空白。
...
Result: 实验结果表明SVAgent通过模拟人类故事情节推理，在视频理解方面实现了优越的性能和可解释性。

Conclusion: SVAgent通过故事情节引导的跨模态多智能体框架，有效解决了视频问答中缺乏人类式故事情节推理的问题，提升了视频理解的性能。
...
TL;DR: 研究发现当前视频理解基准存在大量仅靠文本线索就能回答的问题，提出VidGround方法通过筛选真正需要视觉基础的问题进行后训练，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在视频理解方面表现不佳，研究发现常用基准中40-60%的问题仅靠文本线索就能回答，后训练数据集也存在类似问题，这限制了模型视觉理解能力的提升。

Method: 提出VidGround方法：从后训练数据中筛选出真正需要视觉基础的问题（排除仅靠语言偏见就能回答的问题），然后结合基于强化学习的后训练算法进行训练。

Result: 使用VidGround方法后，性能相对使用完整数据集提升高达6.2个百分点，同时仅使用原始后训练数据的69.1%。数据筛选配合简单后训练算法优于多种复杂后训练技术。

Conclusion: 数据质量是提升VLM视频理解能力的主要瓶颈，需要精心设计真正需要视觉基础的后训练数据和评估基准，才能推动更强大VLM的发展。
...
Method: 提出模态感知和解剖学基础的3D向量量化VAE（VQ-VAE）：1）使用因子化多轴注意力学习跨模态共享潜在表示；2）双流3D编码器分离模态不变解剖结构和模态依赖外观；3）解剖编码通过共享码本离散化，解码时通过FiLM与模态特定外观特征结合；4）采用联合2D/3D训练策略适应3D MRI的切片采集特性。

Result: 在两个多模态脑MRI数据集上的广泛实验表明，NeuroQuant相比现有VAE实现了更优的重建保真度，为下游生成建模和跨模态脑图像分析提供了可扩展的基础。

Conclusion: NeuroQuant通过创新的模态感知和解剖学基础设计，成功实现了多模态脑MRI的高质量重建，为医学图像分析中的生成建模和跨模态分析提供了有效的解决方案。
...
TL;DR: MIRAGE框架通过多分支并行去噪策略解决多实例图像编辑中的过编辑和空间错位问题，显著提升细粒度一致性


<details>
  <summary>Details</summary>
Motivation: 现有指令引导的图像编辑模型（如FLUX.2和Qwen-Image-Edit）在处理多个相似实例需要分别编辑的复杂场景时存在严重过编辑和空间错位问题

Method: 提出MIRAGE框架：1）使用视觉语言模型将复杂指令解析为区域子集；2）采用多分支并行去噪策略，将目标区域的潜在表示注入全局表示空间；3）通过参考轨迹保持背景完整性

Result: 在MIRA-Bench和RefEdit-Bench上的广泛评估表明，MIRAGE框架显著优于现有方法，能够实现精确的实例级修改同时保持背景一致性

Conclusion: MIRAGE是一个无需训练的框架，通过区域对齐和并行去噪策略有效解决了多实例图像编辑中的关键挑战，为复杂指令引导的编辑任务提供了新解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前基于物体中心的feed-forward 3D重建方法虽然稳健，但在恢复精细纹理和外观方面仍落后于密集视图优化方法。研究者希望探索扩展transformer上下文窗口是否能缩小这一差距。
...
Conclusion: 扩展transformer上下文窗口能显著提升3D重建质量，LSRM通过稀疏注意力架构有效实现了这一扩展，在3D重建和逆渲染任务中均取得突破性进展。
...
TL;DR: 本文提出了一种无需训练的适配器融合方法，利用正交微调（OFT）的几何特性，通过结构化正交参数化和谱恢复变换，将不同任务的适配器合并为单一适配器。


<details>
  <summary>Details</summary>
Motivation: 在模型训练快速发展的背景下，参数高效微调和少量数据适应特定任务的技术备受关注。然而，如何将针对不同任务调优的多个适配器合并为一个能够同时在多个任务上取得良好结果的单一适配器，特别是生成模型中主体和风格适配器的融合问题，仍然没有得到解决。

Method: 1. 利用正交微调（OFT）的结构化正交参数化及其几何特性，推导训练免费的适配器融合公式
2. 分析最近提出的Group-and-Shuffle（GS）正交矩阵形成的流形结构
3. 获得两点间测地线近似的高效公式
4. 提出谱恢复变换，恢复合并后适配器的谱特性以提高融合质量

Result: 实验在主体驱动生成任务中进行，表明该技术能够成功融合不同适配器的概念和风格特征。据作者所知，这是首个无需训练的乘法正交适配器融合方法。

Conclusion: 通过利用正交微调的几何特性，本文提出了一种有效的训练免费适配器融合方法，能够将针对不同任务调优的适配器合并为单一适配器，在主体驱动生成任务中成功融合概念和风格特征。
...
TL;DR: 本研究提出了一种检测引导的小型视觉语言模型框架，通过结合目标检测与多模态推理，在保持高效性的同时提升建筑工地危险识别的准确性。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然具有强大的上下文推理能力，但计算需求高，难以实现近实时建筑危险检测；而小型视觉语言模型虽然效率高，但在复杂建筑场景中准确率低且容易产生幻觉。需要解决这一权衡问题。

Method: 提出检测引导的sVLM框架：首先使用YOLOv11n检测器定位场景中的工人和建筑机械，然后将检测到的实体嵌入结构化提示中，引导sVLMs进行空间基础的危险评估。评估了六种sVLM模型在零样本设置下的性能。

Result: 该方法在所有模型中一致提高了危险检测性能。最佳模型Gemma-3 4B的F1分数达到50.6%（基线为34.5%）。解释质量也显著提高，BERTScore F1从0.61提升到0.82。尽管加入了目标检测，推理时间仅增加2.5毫秒/图像。
...
TL;DR: 提出用于神经影像中异构网格分析的层次化Transformer框架，支持体积和表面网格，整合多种顶点级形态测量特征，通过自监督预训练实现跨任务和模态的迁移学习。


<details>
  <summary>Details</summary>
Motivation: 大规模非结构化体积和表面网格的表征学习面临挑战，现有方法要么忽略临床信息特征，要么只支持单一网格拓扑，限制了跨成像管道的应用。

Method: 引入层次化Transformer框架，基于任意阶单纯复形构建空间自适应树分区，特征投影模块将可变长度顶点特征映射到空间层次中，通过掩码重建坐标和形态测量通道进行自监督预训练。

Result: 在ADNI的体积脑网格上进行阿尔茨海默病分类和淀粉样蛋白负荷预测，以及在MELD数据集的皮质表面网格上进行局灶性皮质发育不良检测，在所有基准测试中达到最先进结果。

Conclusion: 提出的框架能够处理异构网格分析，整合多种临床特征，通过单一架构支持不同网格模态，为神经影像中的多尺度表征学习提供了有效解决方案。
...
TL;DR: 提出一种人机协同框架，通过预训练分类器引导采样，高效估计目标源的两点相关函数，显著降低方差并减少标注工作量。


<details>
  <summary>Details</summary>
Motivation: 在天文学等应用中，测量特定子集（如星团）的两点相关函数通常需要大量人工标注，这既耗时又费力。现有方法难以高效处理大规模点集和稀疏目标源的情况。

Method: 提出人机协同框架：1) 使用预训练分类器指导采样，自适应选择最具信息量的点进行人工标注；2) 每次标注后，同时生成多个距离区间内对计数的无偏估计；3) 引入新的无偏估计器、采样策略和置信区间构建方法。

Result: 相比简单的蒙特卡洛方法，该方法在显著减少标注工作量的同时，实现了更低的方差，能够进行可扩展且统计基础牢固的两点相关测量。

Conclusion: 该框架为天文学数据集中的两点相关测量提供了一种高效、可扩展的解决方案，通过智能采样和统计严谨的估计方法，大幅降低了人工标注成本。
...
TL;DR: 提出负责任计算框架，用条件图像合成生成带标签的合成抗议图像，在分析集体抗议动态的同时降低个体隐私风险


<details>
  <summary>Details</summary>
Motivation: 抗议相关的社交媒体数据对理解集体行动有价值，但存在高风险：AI系统能识别个体、推断敏感属性、跨平台交叉引用视觉信息，可能导致监控、镇压和隐私泄露。现有抗议分析方法缺乏整合隐私风险评估、下游分析和公平性考量的整体框架

Method: 提出负责任计算框架，使用条件图像合成技术将敏感抗议图像替换为带标签的合成再现图像，从而分析集体模式而不直接暴露可识别个体

Result: 该方法能生成真实多样的合成图像，在保持下游分析效用的同时降低隐私风险，并评估生成数据的群体公平性，确保合成表示不会对特定亚群体产生不成比例的影响

Conclusion: 该方法采用务实的伤害缓解策略，不提供绝对隐私保证，而是承认残余风险的同时实现社会敏感分析，为负责任地分析集体抗议动态提供了可行路径
...
TL;DR: COVER是一种基于覆盖率的主动视角选择方法，通过最小化Fisher信息增益的近似值来选择信息丰富的相机位姿，提高3D重建质量


<details>
  <summary>Details</summary>
Motivation: 用于学习3D重建的数据质量对高效准确的场景建模至关重要，需要解决主动视角选择问题，找到选择信息丰富相机位姿的原则性方法

Method: 提出COVER（Camera Optimization for View Exploration and Reconstruction）方法，核心洞察是通过最小化Fisher信息增益的易处理近似来选择信息丰富的视角，这简化为偏好覆盖过去相机观察不足的几何区域

Result: 在真实数据集上的固定和嵌入式数据采集场景中，与最先进的主动视角选择方法相比，该方法在多个数据集和辐射场基线上持续提高重建质量

Conclusion: COVER提供了一种轻量级、基于覆盖率的视角选择指标，避免了昂贵的透射率估计，对噪声和训练动态具有鲁棒性，能有效提升3D重建性能
...
TL;DR: Region-R1：通过查询端区域裁剪框架提升多模态检索增强生成中的重排序性能，动态裁剪问题相关区域以消除视觉干扰


<details>
  <summary>Details</summary>
Motivation: 传统多模态检索增强生成（MM-RAG）中的重排序器通常将完整查询图像作为全局嵌入处理，容易受到背景杂乱等视觉干扰的影响，导致相似性评分偏差。需要一种能够聚焦问题相关区域的方法来提升重排序效果。

Method: 提出Region-R1框架，将区域选择建模为决策问题，在重排序过程中学习保留完整图像或仅关注问题相关区域。采用新颖的区域感知组相对策略优化（r-GRPO）学习策略，动态裁剪判别性区域。

Result: 在两个具有挑战性的基准测试E-VQA和InfoSeek上，Region-R1实现了持续的性能提升，达到最先进水平，条件Recall@1提升高达20%。

Conclusion: 查询端自适应是一种简单而有效的方法，可以显著增强MM-RAG重排序性能，展示了区域裁剪框架在多模态检索中的巨大潜力。
...
TL;DR: 本文介绍了UFPR-VeSV数据集，这是一个包含24,945张车辆图像的真实世界监控数据集，具有13种颜色、26个品牌、136个型号和14种类型的细粒度标注，并探索了细粒度车辆分类与车牌识别的集成应用。


<details>
  <summary>Details</summary>
Motivation: 现有车辆信息提取研究通常假设理想条件、探索有限属性，且忽视了细粒度车辆分类与车牌识别的集成应用。为填补这些空白，需要构建一个在真实世界监控条件下、具有丰富标注的车辆数据集。

Method: 从巴西巴拉那州军事警察监控系统收集24,945张车辆图像，包含16,297辆独特车辆，标注了13种颜色、26个品牌、136个型号和14种类型。所有细粒度分类标注都使用车牌信息进行验证，并提供文本和角点标注。使用五种深度学习模型进行基准测试，并应用两种光学字符识别模型进行车牌识别。

Result: 与现有数据集相比，UFPR-VeSV数据集更具挑战性，包含部分遮挡、夜间红外成像和不同光照条件等真实世界场景。基准测试揭示了处理多色车辆、红外图像以及区分共享平台车辆模型的具体挑战。细粒度车辆分类与车牌识别的集成显示出实际应用的潜力。

Conclusion: UFPR-VeSV数据集为细粒度车辆分类研究提供了具有挑战性的真实世界基准，证明了细粒度车辆分类与车牌识别集成应用的潜力，有助于推动智能交通系统和刑事调查等实际应用的发展。
...
TL;DR: 本文提出了一种针对视觉嵌入模型的身份泄露评估基准和线性投影去身份方法，在保护面部隐私的同时保持视觉搜索实用性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉嵌入模型（如CLIP、DINOv2/v3、SSCD）在面部数据上使用时存在未测量的身份泄露问题，且缺乏可部署的缓解措施，这限制了其在面部隐私敏感场景中的应用。

Method: 提出两个主要贡献：1）建立视觉嵌入模型身份泄露评估基准，包括低误接受率下的开放集验证、基于扩散的模板反演检查、以及面部上下文归因分析；2）提出一次性线性投影器（ISP），通过移除估计的身份子空间来消除身份信息，同时保留实用功能所需的互补空间。

Result: 在CelebA-20和VGGFace2数据集上的实验表明：CLIP相比DINOv2/v3和SSCD具有相对更高的身份泄露；ISP方法能将身份识别准确率降至接近随机水平，同时保持高非生物特征实用性，且在不同数据集间具有良好的迁移性。

Conclusion: 本研究首次建立了针对非面部识别编码器的攻击者校准面部隐私审计框架，并证明线性子空间移除方法能在保持视觉搜索和检索实用性的同时提供强大的隐私保护保证。
...
TL;DR: SmokeGS-R是一个用于烟雾场景3D重建的实用管道，通过解耦几何恢复和外观校正，在NTIRE 2026挑战赛上取得了优于基线3.68 dB PSNR的性能。


<details>
  <summary>Details</summary>
Motivation: 真实世界的烟雾会同时衰减场景辐射、增加大气光并破坏多视角外观一致性，这使得鲁棒的3D重建特别困难。需要一种能够处理这些复杂烟雾效应的3D重建方法。

Method: 采用几何恢复与外观校正解耦的策略：1) 使用改进的暗通道先验和引导滤波生成物理引导的伪干净监督；2) 训练一个锐利的仅干净3D高斯泼溅源模型；3) 通过几何平均参考聚合、LAB空间Reinhard转移和轻量高斯平滑，使用供体集合对渲染结果进行外观协调。

Result: 在官方挑战测试排行榜上，最终提交获得了PSNR=15.217和SSIM=0.666。在RealX3D公开后，在七个发布的挑战场景上重新评估相同冻结结果，获得PSNR=15.209、SSIM=0.644和LPIPS=0.551，比相同场景上最强的官方基线平均高出+3.68 dB PSNR。
...
TL;DR: 提出了一种在3D高斯泼溅场景中进行目标室内资产对象级检测与分割的方法，通过3D对象码本和多视角掩码关联实现准确的三维物体实例化


<details>
  <summary>Details</summary>
Motivation: 针对从360度无人机图像重建的3D高斯泼溅场景，需要实现目标室内资产的精确对象级检测和分割，解决多视角掩码关联和三维物体实例化的挑战

Method: 引入3D对象码本，结合掩码语义和对应高斯基元空间信息来指导多视角掩码关联；集成2D对象检测和分割模型，通过语义和空间约束的合并过程将多视角掩码聚合成一致的3D对象实例

Result: 在两个大型室内场景实验中，方法表现出可靠的多视角掩码一致性，F1分数比最先进基线提升65%；实现了准确的对象级3D室内资产检测，mAP比基线方法提高11%

Conclusion: 该方法有效解决了3D高斯泼溅场景中室内资产的对象级检测和分割问题，通过3D对象码本和多视角掩码关联显著提升了检测准确性和一致性
...
Conclusion: 提出的VLA-InfoEntropy方法通过整合空间、语义和时间线索，在减少冗余的同时保留了关键内容，有效解决了VLA模型的推理效率问题。
...
TL;DR: 本文提出了一种无监督的多智能体与单智能体感知框架UMS，通过多智能体协作无需人工标注同时解决多智能体与单智能体感知问题，在两个公开数据集上取得了优于现有方法的3D检测性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于LiDAR的多智能体和单智能体感知在环境理解方面表现出色，但缺乏同时解决这两个问题的无监督方法。通过多智能体间的传感器数据共享，本文发现：1）协作视图增强点云密度有利于无监督目标分类；2）多智能体协作视图可作为单视图3D目标检测的无监督指导。

Method: 提出UMS无监督感知框架：1）基于学习的提议净化过滤器，通过多智能体点云密度协作更好地分类候选提议；2）渐进式提议稳定模块，通过从易到难的课程学习生成可靠伪标签；3）跨视图共识学习，利用多智能体协作视图指导单智能体视图检测。

Result: 在V2V4Real和OPV2V两个公开数据集上的实验结果表明，UMS方法在无监督设置下，在多智能体和单智能体感知任务上都显著优于现有最先进方法的3D检测性能。

Conclusion: UMS框架首次实现了同时解决多智能体和单智能体感知的无监督方法，通过多智能体协作利用点云密度增强和跨视图指导，在无需人工标注的情况下显著提升了3D检测性能。
...
Conclusion: 提出的多线索引导局部特征学习框架通过语义和几何线索的协同利用，显著提高了关键点检测的鲁棒性和描述符的区分度
...
TL;DR: SPIRE将红外小目标检测重新定义为质心回归任务，使用单点监督和概率响应编码，避免了传统分割方法对模糊边界和背景噪声的敏感问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于像素级监督的"编码器-解码器"分割范式虽然性能不错，但忽略了小目标只占少数像素且常伴有模糊边界的问题。作者认为红外小目标检测的首要原则应该是目标定位，而非分离所有目标区域并伴随难以区分的背景噪声。

Method: 提出SPIRE方法：1) 点响应先验监督(PRPS)，将单点标注转换为符合红外点目标响应特性的概率响应图；2) 高分辨率概率编码器(HRPE)，实现仅编码器、端到端回归，无需解码器重建；3) 通过保持高分辨率特征和增加有效监督密度，缓解稀疏目标分布下的优化不稳定问题。

Result: 在SIRST-UAVB和SIRST4等多个红外小目标检测基准测试中，SPIRE实现了具有竞争力的目标级检测性能，具有持续低的虚警率(Fa)和显著降低的计算成本。

Conclusion: 将红外小目标检测重新定义为质心回归任务，并采用单点监督和概率响应编码的方法，能够有效解决传统分割方法在小目标检测中的局限性，在保持检测性能的同时降低计算复杂度。
...
TL;DR: 3DTurboQuant：无需训练或场景微调的无数据依赖3D模型压缩方法，通过随机旋转和预计算量化实现高效压缩


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射、NeRF或基于transformer的3D重建器压缩方法都需要通过场景微调学习数据依赖的码本，这种方法效率低下且不必要

Method: 利用参数向量在高维空间中服从已知Beta分布的特性，通过随机旋转将输入变换到特定坐标，然后使用预计算的Lloyd-Max量化；开发了维度相关准则、范数分离边界、二维哈希网格特征分组策略和可组合的剪枝-量化流水线

Result: 在NeRF Synthetic数据集上，3DTurboQuant将3DGS压缩3.5倍（PSNR损失仅0.02dB），将DUSt3R KV缓存压缩7.9倍（点图保真度39.7dB）；无需训练、码本学习或校准数据，压缩仅需数秒

Conclusion: 证明了数据无关的预计算量化在3D模型压缩中的有效性，为3D重建和渲染模型的实时压缩提供了高效解决方案，显著优于需要场景微调的传统方法
...
TL;DR: UAVReason：首个面向无人机俯视场景的统一多模态基准，包含27.3万VQA样本，涵盖空间/时间推理和跨模态生成任务，通过多任务学习建立强基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在无人机高空俯视场景中表现不佳，主要原因是领域偏移问题：物体微小密集、纹理重复、俯视方向模糊，这些因素严重影响了语义理解和空间推理能力。

Method: 1. 从高保真无人机仿真平台构建UAVReason基准，包含27.3万VQA样本（23.6万单帧描述、68.2万双帧时序序列、188.8万跨模态生成样本）；2. 涵盖22种推理类型；3. 通过多任务学习建立统一基线模型。

Result: 实验验证了统一多任务学习方法在多种指标上的有效性：VQA的EM/F1分数、分割的mIoU、生成的CLIP分数。结果显示通用视觉语言模型存在局限性，而统一多任务学习显著提升了无人机场景性能。
...
TL;DR: 该论文提出了对象锚定组合图像检索（OACIR）任务，通过引入边界框锚定参考图像中的对象来确保实例级一致性，并构建了首个大规模多领域基准OACIRR，同时提出了AdaFocal框架来提升实例保真度。


<details>
  <summary>Details</summary>
Motivation: 现有组合图像检索（CIR）方法主要关注语义匹配，但在跨上下文检索用户指定实例时表现不佳。实际应用中，保持具体实例的保真度比广泛的语义匹配更为重要，因此需要一种能够确保严格实例级一致性的细粒度检索任务。

Method: 提出了对象锚定组合图像检索（OACIR）任务，通过边界框在参考图像中视觉锚定对象。构建了OACIRR基准，包含超过16万四元组和四个具有困难负例干扰的候选库。提出了AdaFocal框架，采用上下文感知注意力调制器，自适应地增强指定实例区域的注意力，动态平衡锚定实例和更广泛组合上下文之间的关注。

Result: AdaFocal框架在OACIR任务上显著优于现有的组合检索模型，特别是在保持实例级保真度方面表现突出。该研究为这一具有挑战性的任务建立了强大的基线，并为更灵活、实例感知的检索系统开辟了新方向。

Conclusion: OACIR任务通过引入对象锚定机制解决了现有CIR方法在实例级一致性方面的不足。AdaFocal框架通过自适应注意力调制有效提升了实例保真度，为细粒度组合图像检索提供了新的解决方案和基准。
...
TL;DR: LSGS-Loc：针对大规模无人机场景的视觉定位新方法，通过尺度感知位姿初始化和拉普拉斯可靠性掩码机制，解决了现有3DGS方法在大规模场景中的位姿初始化困难和渲染伪影敏感性问题。


<details>
  <summary>Details</summary>
Motivation: 大规模无人机场景中的视觉定位对自主系统至关重要，但现有基于3D高斯泼溅（3DGS）的方法面临两个主要挑战：1）在大规模场景中难以实现鲁棒的位姿初始化；2）对渲染伪影（如模糊和漂浮物）过于敏感，影响定位精度。

Method: 提出LSGS-Loc视觉定位流水线，包含两个关键技术：1）尺度感知位姿初始化策略，结合场景无关的相对位姿估计和显式的3DGS尺度约束，实现无需场景特定训练的几何基础定位；2）拉普拉斯可靠性掩码机制，在姿态优化阶段引导光度优化聚焦于高质量区域，减少重建伪影的影响。

Result: 在大规模无人机基准测试上的广泛实验表明，该方法在无序图像查询中实现了最先进的精度和鲁棒性，显著优于现有的基于3DGS的方法。

Conclusion: LSGS-Loc通过创新的尺度感知位姿初始化和伪影抑制机制，有效解决了大规模3DGS场景中视觉定位的挑战，为无人机等自主系统提供了更可靠的定位解决方案。
...
TL;DR: 提出了一种用于恶劣天气下3D目标检测的自适应多模态融合框架，通过条件令牌引导的路由器动态调整LiDAR和4D雷达的权重，实现天气感知的模态选择。
...
Method: 将多模态感知重新表述为天气条件分支路由问题，维护三个并行3D特征流：纯LiDAR分支、纯4D雷达分支和条件门控融合分支。通过视觉和语义提示提取的条件令牌指导轻量级路由器预测样本特定权重，软聚合这些表示。引入天气监督学习策略，包含辅助分类和多样性正则化，防止分支崩溃。

Result: 在K-Radar基准测试中实现了最先进的性能，提供了明确且高度可解释的模态偏好洞察，透明揭示了自适应路由如何在不同的恶劣天气场景中鲁棒地转移LiDAR和4D雷达的依赖关系。

Conclusion: 该框架通过天气条件分支路由实现了动态模态偏好调整，显著提升了恶劣天气下3D目标检测的鲁棒性和可解释性。
...
TL;DR: CRISP：基于"正区域秩稳定性"定律的无参数、模型无关框架，通过特征扰动模拟分布偏移，利用高精度和高召回先验迭代训练，显著提升医学图像分割在分布偏移下的鲁棒性。
...
Method: 提出"正区域秩稳定性"定律，基于该原理开发CRISP框架：1）通过潜在特征扰动模拟分布偏移；2）识别保持高概率的区域（高精度先验）和低概率区域（高召回先验）；3）设计迭代训练框架，让两个先验逐步"挤压"到最终分割结果。

Result: 在多中心心脏MRI和CT肺血管分割任务上，CRISP显著优于现有方法：在多中心、人口统计和模态偏移下，HD95分别降低0.14像素（7.0%提升）、1.90像素（13.1%提升）和8.39像素（38.9%提升）。

Conclusion: CRISP是首个基于秩而非概率的分割框架，无需目标域信息，参数无关且模型无关，通过利用分布偏移下的秩稳定性模式，显著提升了医学图像分割在真实世界分布偏移下的鲁棒性。
...
TL;DR: SGPer是一个语义-几何先验协同框架，通过结合DINOv2的语义感知和SAM的边界定位能力，在有限数据下实现小麦病害的精确分割，有效应对生长阶段的外观变化。


<details>
  <summary>Details</summary>
Motivation: 小麦病害分割面临生长阶段间显著的外观变化挑战，收集代表性训练数据集既费力又不切实际，需要一种在有限数据下仍能鲁棒工作的解决方案。

Method: 提出SGPer框架：1) 使用DINOv2提供类别感知语义先验处理外观变化；2) 设计病害敏感适配器插入DINOv2和SAM以对齐病害特征；3) 将DINOv2特征转换为密集的类别特定点提示；4) 通过交叉参考SAM的掩码置信度和DINOv2的语义一致性动态过滤提示；5) 激活SAM的几何先验实现精确分割。

Result: 在多个小麦病害和器官分割基准测试中，SGPer在数据受限场景下持续达到最先进的性能表现。

Conclusion: SGPer通过协同语义和几何先验，实现了对时间外观变化严格不变的小麦病害精确分割，为有限数据下的农业视觉任务提供了有效解决方案。
...
TL;DR: VideoStir：一种结构化、意图感知的长视频检索增强生成框架，通过时空图和多跳检索解决现有方法忽略视频结构、依赖显式语义匹配的问题
...
Method: VideoStir框架首先将视频在片段级别组织为时空图，然后执行多跳检索以聚合跨距离但上下文相关的事件证据。此外，引入基于MLLM的意图相关性评分器，根据帧与查询推理意图的对齐程度进行检索。为此构建了IR-600K大规模数据集来学习帧-查询意图对齐

Result: 实验表明，VideoStir在不依赖辅助信息的情况下与最先进的基线方法具有竞争力，突显了将长视频RAG从扁平化语义匹配转向结构化、意图感知推理的前景

Conclusion: VideoStir通过结构化视频表示和意图感知检索，为长视频检索增强生成提供了更有效的解决方案，代码和检查点已在GitHub上开源
...
TL;DR: 本文提出CSAP解码器框架，通过跨阶段注意力传播减少冗余计算，在保持多尺度上下文推理的同时显著降低计算成本。
...
TL;DR: 本文提出了一种基于SAM3的无训练少样本语义分割方法，通过简单的空间拼接策略将支持和查询图像置于共享画布上，利用冻结的SAM3实现分割，在多个基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有少样本语义分割方法依赖大量情景训练学习可迁移表示，计算成本高且对分布变化敏感。本文从现代视觉基础模型角度重新审视FSS，探索SAM3作为无需训练解决方案的潜力。

Method: 利用SAM3的可提示概念分割能力，采用简单的空间拼接策略将支持图像和查询图像放置到共享画布上，使用完全冻结的SAM3进行分割，无需任何微调或架构修改。

Result: 在PASCAL-5^i和COCO-20^i数据集上的实验表明，这种极简设计已经达到最先进性能，超越了许多精心设计的复杂方法。同时发现负提示在少样本设置中可能适得其反。

Conclusion: 研究表明简单的空间公式可以产生强大的跨图像推理能力，同时揭示了当前基础模型在处理冲突提示信号方面的局限性。该方法为少样本分割提供了高效的无训练解决方案。
...
TL;DR: HUG3D：从单张图像重建多人体交互场景的3D模型，通过群体-实例联合建模解决遮挡和交互失真问题


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对单人体重建，在多人体场景中简单组合个体重建会导致不现实的重叠、遮挡区域几何缺失和交互失真等问题，需要引入群体级上下文和交互先验

Method: 1) 将输入转换到规范正交空间缓解透视失真；2) HUG-MVD模块联合建模个体和群体上下文生成完整多视角法线和图像；3) HUG-GR模块利用物理交互先验优化几何；4) 融合多视角图像生成高保真纹理

Result: HUG3D显著优于单人体和现有多人体方法，能从单张图像生成物理合理、高保真的交互人体3D重建

Conclusion: HUG3D通过群体-实例联合建模和物理交互先验，成功解决了多人体场景重建中的遮挡、重叠和交互失真问题，为AR/VR和数字人应用提供了有效解决方案
...
TL;DR: GameAD框架将端到端自动驾驶建模为风险感知的博弈问题，通过风险优先的交互实现更安全的轨迹规划，在多个数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶模型将所有智能体平等对待，难以从复杂背景中解耦真实的碰撞威胁。自动驾驶的核心在于统一表示空间中的动态多智能体博弈，而非简单的感知与规划集成。

Method: 提出GameAD框架，包含四个核心组件：风险感知拓扑锚定、战略负载适配器、极小极大风险感知稀疏注意力、风险一致均衡稳定化。同时提出Planning Risk Exposure指标量化长期轨迹的累积风险强度。

Result: 在nuScenes和Bench2Drive数据集上的大量实验表明，该方法在轨迹安全性方面显著优于现有最先进方法。

Conclusion: 通过将端到端自动驾驶建模为风险优先的博弈问题，GameAD框架能够实现更安全的决策制定，特别是在复杂多智能体交互场景中。
...
TL;DR: 提出一个生成合成眼动视频数据的管道，通过3D眼动模拟器重放真实虹膜轨迹，创建大规模标注数据集，用于行为分类器开发。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在大量互联网数据上训练取得了显著能力，但行为模态数据（如眼动）稀缺、标注昂贵且隐私敏感。模拟生成合成数据是解决这一问题的有前景的替代方案。

Method: 开发了一个管道：从参考视频中提取真实人类虹膜轨迹，通过无头浏览器自动化在3D眼动模拟器上重放这些轨迹，生成合成标注的眼动视频。应用于视频面试中的脚本阅读检测任务。

Result: 发布了final_dataset_v1：包含144个会话（72个阅读，72个对话），总计12小时25fps的合成眼动视频。评估显示生成的轨迹保留了源数据的时间动态特性（所有指标的KS D < 0.14）。3D模拟器在阅读尺度运动上表现出有界敏感性，归因于缺少耦合的头部运动。

Conclusion: 该管道、数据集和评估工具已发布，支持在行为建模和视觉语言系统交叉领域开发下游行为分类器。模拟器设计需要考虑耦合头部运动的影响。
...
TL;DR: 提出一种结合视觉语言模型引导的流匹配分割和随机矩阵理论检测的犬类气胸自动诊断新范式，通过公开数据集和协同定位-检测方法解决数据稀缺和模型可信度问题。


<details>
  <summary>Details</summary>
Motivation: 犬类气胸自动诊断面临数据稀缺和需要可信模型的挑战，传统方法在边界精度和统计可靠性方面存在不足。

Method: 采用协同定位-检测范式：1) 使用视觉语言模型引导迭代流匹配过程进行病灶定位分割；2) 基于分割结果提取特征，应用随机矩阵理论分析特征矩阵，将健康组织建模为可预测随机噪声，通过检测统计显著的异常特征值来识别气胸病理信号。

Result: 开发了公开的像素级标注数据集，提出的方法实现了高精度的边界分割和高度敏感的气胸检测，生成式分割的高保真定位为随机矩阵理论检测器提供了纯净信号。

Conclusion: 生成式分割和基于第一性原理的统计分析相结合，构建了一个高精度、可解释的诊断系统，为医学图像分析提供了新的协同范式。
...
Conclusion: WSA-Net通过物理特征重建增强微弱信号，在轻量级架构中实现信号中心感知，显著提高探地雷达地下缺陷检测的灵敏度，减少漏检率
...
TL;DR: 论文提出了ContrAR基准，用于评估视觉语言模型在增强现实环境中对抗虚拟内容操纵和矛盾攻击的鲁棒性。
...
Method: 系统建模矛盾虚拟内容攻击，创建ContrAR基准数据集，包含312个真实世界AR视频并由10名人类参与者验证。在此基础上对11个商业和开源视觉语言模型进行基准测试。

Result: 实验结果显示，当前视觉语言模型对矛盾虚拟内容表现出合理的理解能力，但在检测和推理AR环境中的对抗性内容操纵方面仍有改进空间。同时，平衡检测准确性和延迟仍然具有挑战性。

Conclusion: 增强现实中的虚拟内容操纵攻击构成严重安全威胁，需要开发更鲁棒的视觉语言模型来应对这些挑战。ContrAR基准为评估和改进模型在AR环境中的安全性提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D医学图像分割方法难以同时实现高精度和计算效率，特别是在处理多样化解剖结构和成像模态时存在局限性，需要一种既能准确分割又能高效计算的方法。

Method: 提出GCNV-Net框架，包含三个核心组件：1）三向动态非空体素Transformer（3DNVT），沿横断面、矢状面和冠状面动态分区处理体素；2）几何交叉注意力模块（GCA），在特征融合中显式融入几何位置信息；3）非空体素化技术，仅处理信息区域减少冗余计算。
...
Conclusion: GCNV-Net在准确性和效率之间实现了有效平衡，在多样化器官、疾病条件和成像模态中表现出强大鲁棒性，具有临床部署的显著潜力。
...
Method: 提出STSF-Net框架：1) 联合建模模态特定特征以捕获真实语义变化信号；2) 嵌入时空共同特征以抑制成像机制差异导致的伪变化；3) 引入基于预训练基础模型语义先验的光学和SAR特征自适应融合策略；4) 创建Delta-SN6数据集作为首个公开的多类别多模态变化检测基准。

Result: 在Delta-SN6、BRIGHT和Wuhan-Het数据集上，STSF-Net的mIoU分别比现有最佳方法高出3.21%、1.08%和1.32%。

Conclusion: STSF-Net通过有效建模模态特定特征和时空共同特征，结合语义引导的自适应融合策略，显著提升了多模态变化检测的性能，为解决跨模态变化检测中的挑战提供了有效方案。
...
TL;DR: 提出基于提示的缺失模态适应框架，通过缺失模态评估器、模态不变提示解耦、动态提示加权和多级提示动态连接模块，解决多模态情感分析中的缺失模态问题，在三个基准数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 缺失模态问题严重影响了多模态情感分析模型的准确性和泛化能力。现有方法主要通过提示学习和预训练模型提高鲁棒性，但存在两个局限性：1) 缺乏对生成缺失模态必要性的严格评估；2) 对多模态提示之间的结构依赖关系和全局一致性探索不足。

Method: 提出Prompt-based Missing Modality Adaptation (ProMMA)框架：1) 输入阶段引入缺失模态评估器，使用预训练模型和伪标签动态评估缺失模态的重要性，避免低质量数据插补；2) 模态不变提示解耦模块将共享提示分解为模态特定的私有提示，捕捉内在局部相关性；3) 动态提示加权模块基于互信息计算权重，自适应抑制缺失模态的干扰；4) 多级提示动态连接模块通过残差连接整合共享提示与自注意力输出，利用全局提示先验增强关键引导特征。

Result: 在CMU MOSI、CMU MOSEI和CH-SIMS三个公开基准数据集上的广泛实验表明，该框架在不同缺失模态设置下实现了最先进的性能和稳定的结果。

Conclusion: 提出的ProMMA框架有效解决了多模态情感分析中的缺失模态问题，通过动态评估缺失模态重要性、解耦模态特定提示、自适应加权和增强全局一致性，显著提升了模型的鲁棒性和性能。
...
Method: 提出SpecMamba框架：1）在冻结的Transformer表示上引入离散余弦变换Mamba适配器（DCTMA），通过DCT将光谱特征投影到频域，利用Mamba的线性复杂度状态空间递归捕获全局光谱依赖性和带连续性；2）设计先验引导三编码器（PGTE），允许实验室光谱先验指导可学习适配器的优化；3）开发自监督伪标签映射（SSPLM）策略进行测试时适应。

Result: 在多个公共数据集上的广泛实验表明，SpecMamba在检测精度和跨域泛化方面始终优于最先进的方法。

Conclusion: SpecMamba通过解耦稳定语义表示和灵活光谱适应，提供了一种参数高效、频率感知的解决方案，有效解决了少样本高光谱目标检测中的挑战，显著提升了检测性能和跨域泛化能力。
...
TL;DR: 本文针对组合图像检索任务中基于视觉语言预训练模型微调时存在的严重过拟合问题，提出了WRF4CIR方法，通过在微调过程中对模型权重施加对抗性扰动来正则化，有效缓解了有限三元组数据下的过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言预训练模型的组合图像检索方法普遍存在严重的过拟合问题，特别是在有限的三元组数据条件下。作者通过系统性研究发现，不同模型和数据集之间存在显著且被忽视的泛化差距，这促使他们提出新的解决方案。

Method: 提出了WRF4CIR（Weight-Regularized Fine-tuning network for CIR）方法，在微调过程中对模型权重施加对抗性扰动进行正则化。这些扰动沿着梯度下降的相反方向生成，通过增加训练数据拟合的难度来缓解过拟合。

Result: 在基准数据集上的大量实验表明，WRF4CIR显著缩小了泛化差距，并在现有方法基础上取得了实质性改进。

Conclusion: WRF4CIR通过权重正则化微调有效解决了组合图像检索中的过拟合问题，特别是在有限监督数据条件下，为视觉语言预训练模型在组合图像检索任务中的应用提供了更稳健的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态人类感知面临缺失模态的关键挑战，主要障碍包括异构数据间的表示差距和低质量模态的污染效应。这两个障碍存在因果关联，因为污染引入的噪声从根本上阻碍了表示差异的减少。

Method: 提出PTA（Purify-then-Align）框架：1）净化阶段：使用元学习驱动的加权机制动态学习降低噪声、低贡献模态的影响；2）对齐阶段：引入基于扩散的知识蒸馏范式，由净化后的共识形成信息丰富的干净教师模型，精炼每个学生模态的特征。

Result: 在MM-Fi和XRF55大规模数据集上，在显著表示差距和污染效应条件下进行综合实验，PTA实现了最先进的性能，并显著提高了单模态模型在多样化缺失模态场景下的鲁棒性。

Conclusion: PTA的"净化-对齐"策略通过元学习和知识扩散的协同整合解决了因果依赖问题，最终创建了具有跨模态知识的强大单模态编码器，为多模态感知中的缺失模态问题提供了有效解决方案。
...
TL;DR: BPC-Net：一种边界概率校准框架，用于无标注皮肤病变分割，通过高斯概率平滑解决边界概率置信不足问题，在无监督方法中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 无标注皮肤病变分割在资源有限场景中具有吸引力，但面临三个耦合挑战：噪声伪标签监督、有限目标域数据下的不稳定迁移、边界概率置信不足。现有方法主要关注伪标签去噪，而压缩边界概率对最终掩码质量的影响未得到足够关注，这直接影响轮廓完整性且无法通过全局阈值调整充分纠正。

Method: 提出BPC-Net边界概率校准框架，核心是高斯概率平滑（GPS），在阈值化前进行局部概率空间校准以恢复置信不足的病变边界而不引起无差别前景扩张。为支持噪声伪监督和跨域迁移下的校准，还包含两个辅助设计：特征解耦解码器（分别处理上下文抑制、细节恢复和边界细化）和交互分支适应策略（仅更新伪标签交互分支而保留部署的图像分割路径）。
...
TL;DR: ID-Selection是一种用于大型视觉语言模型的高效视觉令牌选择策略，通过结合重要性评估和多样性感知的迭代选择，在极端剪枝比例下保持性能同时大幅减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有视觉令牌剪枝方法难以平衡令牌重要性和多样性：基于重要性的方法倾向于保留冗余令牌，而基于多样性的方法可能忽略信息丰富的令牌。这种权衡在高剪枝比例下尤为关键，因为需要保留极少量的视觉令牌。

Method: ID-Selection采用重要性评估与多样性感知迭代选择相结合的策略：首先为每个令牌分配重要性分数，然后逐个选择高分令牌，同时逐步抑制相似令牌的分数。这种方法在统一的选择过程中既保留信息丰富的令牌又减少冗余。

Result: 在5个LVLM主干和16个主要基准测试上的广泛实验表明，ID-Selection在性能和效率方面始终表现优异，特别是在极端剪枝比例下。例如，在LLaVA-1.5-7B上，ID-Selection剪除了97.2%的视觉令牌（仅保留16个令牌），推理FLOPs减少超过97%，同时保持了91.8%的原始性能，且无需额外训练。

Conclusion: ID-Selection是一种简单而有效的令牌选择策略，通过统一的重要性-多样性平衡机制，显著提升了大型视觉语言模型的推理效率，特别是在高剪枝比例下表现出色。
...
TL;DR: 该论文系统研究了风格迁移在领域泛化中的三个关键设计因素：风格池多样性、纹理复杂性和风格来源选择，并提出了StyleMixDG方法，在合成到真实场景转换任务中取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉深度学习模型在从合成数据迁移到真实世界时存在Sim2Real差距问题。虽然风格迁移被广泛用作领域泛化的数据增强策略，但文献中对三个关键设计因素存在矛盾：风格池多样性、纹理复杂性和风格来源选择，需要系统研究来澄清这些矛盾。

Method: 论文进行了系统的实证研究，隔离并评估了三个关键因素对场景理解的影响。基于研究结果，提出了StyleMixDG（领域泛化的风格混合），这是一种轻量级、模型无关的数据增强方法，无需架构修改或额外损失函数。
...
Conclusion: 通过系统实证研究澄清了风格迁移在领域泛化中的关键设计矛盾，提出的StyleMixDG方法验证了经验性设计原则能够转化为实际性能提升，为合成到真实场景转换提供了有效的轻量级解决方案。
...
TL;DR: 提出STGR框架，结合LLM和视觉基础模型，通过文本到视觉意图蒸馏和动态图推理解决医学图像分割中的语义模糊和解剖重叠问题，使用选择性非对称微调策略减少过拟合。


<details>
  <summary>Details</summary>
Motivation: 现有多模态和基础模型在处理临床报告语义模糊和解剖重叠方面存在困难，且在有限医学数据集上微调大型架构容易导致严重过拟合。

Method: 提出语义-拓扑图推理框架，包含文本到视觉意图蒸馏模块提取精确诊断指导，将掩码选择建模为动态图推理问题，候选病变作为节点，边捕获空间和语义相似性，采用选择性非对称微调策略更新少于1%的参数。

Result: 在LIDC-IDRI和LNDb数据集上通过5折交叉验证建立新SOTA，LIDC-IDRI上达到81.5% Dice相似系数，优于LISA等领先LLM工具超过5%，选择性非对称微调策略提供出色的交叉折叠稳定性（0.6% DSC方差）。

Conclusion: STGR框架有效解决了临床指令驱动的医学图像分割中的语义模糊和解剖重叠问题，选择性非对称微调策略确保了部署可行性，为稳健、上下文感知的临床部署铺平了道路。
...
TL;DR: FunRec是一种从第一人称视角RGB-D交互视频直接重建室内场景功能化3D数字孪生的方法，能够自动发现铰接部件、估计运动学参数并重建可交互的3D场景。
...
Method: FunRec直接从第一人称视角RGB-D交互视频操作，自动发现铰接部件，估计其运动学参数，跟踪3D运动，并在规范空间中重建静态和动态几何，生成兼容仿真的网格。

Result: 在新真实和模拟基准测试中，FunRec大幅超越先前工作：部件分割mIoU提升高达+50，铰接和姿态误差降低5-10倍，重建精度显著提高。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型的发展，图像描述从简短句子演变为包含数百字的详细叙述，这使得精确检测和定位幻觉变得更具挑战性。现有基准测试缺乏评估这种能力的细粒度和领域多样性。

Method: 作者创建了DetailVerifyBench基准测试，包含1000张高质量图像，涵盖五个不同领域，平均描述长度超过200字，并提供了密集的token级多类型幻觉标注。

Result: DetailVerifyBench是目前长图像描述领域中用于精确幻觉定位的最具挑战性的基准测试，提供了细粒度的评估能力。

Conclusion: 该基准测试填补了现有评估方法的空白，为多模态大语言模型在长图像描述中的幻觉检测和定位提供了更精确的评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像面临云层、雾霾、噪声、分辨率限制和传感器异质性等多种退化问题，现有方法需要为每种退化类型训练单独模型，缺乏统一的解决方案。

Method: 采用Sinkhorn-Knopp最优传输对齐异质波段到语义匹配的槽位，通过三种互补的混合专家层（卷积专家处理空间模式、通道混合专家保持光谱保真度、带低秩适配器的注意力专家处理全局上下文），并使用步级动态权重调整稳定联合训练。

Result: LLaRS在实验中一致优于七个竞争模型，参数高效微调实验展示了在未见数据上的强大迁移能力和适应效率。

Conclusion: LLaRS作为首个统一的遥感低层视觉基础模型，通过多模态多任务方法有效解决了遥感图像的各种退化问题，具有强大的性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督多视角异常检测方法存在视角变化和模态差异导致的特征不一致问题，需要一种能够有效结合语义和几何对齐的统一框架来解决这些挑战。

Method: 提出SGANet框架，包含三个关键组件：选择性跨视角特征精炼模块（SCFRM）选择性聚合相邻视角的补丁特征；语义-结构补丁对齐（SSPA）确保跨模态语义对齐并保持结构一致性；多视角几何对齐（MVGA）对齐跨视角的几何对应补丁。
...
TL;DR: 该研究探索利用单目广播视频进行足球运动员疲劳监测，通过游戏状态重建方法提取球员轨迹，开发新的运动学处理算法获得速度加速度估计，构建加速度-速度剖面作为疲劳指标。
...
Method: 基于先进的游戏状态重建方法提取球员在球场坐标中的轨迹，提出新的运动学处理算法从重建轨迹中获得时间一致的速度和加速度估计，构建加速度-速度剖面并分析其作为疲劳相关性能指标的行为。

Result: 在SoccerNet-GSR基准测试上评估，考虑30秒片段和完整的45分钟半场。结果表明单目GSR能够恢复与A-S剖面兼容的运动学模式，同时揭示了广播视频固有的轨迹噪声、校准误差和时间不连续性的敏感性。
...
TL;DR: PanopticQuery是一个用于4D场景统一查询推理的框架，基于4D高斯泼溅实现高保真动态重建，通过多视角语义共识机制将自然语言查询与时空语义关联。
...
Method: 基于4D高斯泼溅进行高保真动态重建，引入多视角语义共识机制，通过聚合多视角和多时间帧的2D语义预测来关联自然语言查询，过滤不一致输出，强制几何一致性，并通过神经场优化将2D语义提升为结构化4D基础。

Result: 在复杂语言查询上达到了新的最先进水平，有效处理属性、动作、空间关系和多对象交互。创建了Panoptic-L4D新基准用于动态场景语言查询评估。

Conclusion: PanopticQuery通过统一查询时间推理框架，成功解决了4D环境中自然语言查询的语义关联挑战，实现了对动态场景的鲁棒语义理解。
...
TL;DR: RATNet是一个基于类比推理的胃肠道内窥镜成像基础模型，通过循环预训练策略从异构专家标注中获取和转移知识，在多种场景下优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 胃肠道疾病负担日益加重，内窥镜是早期诊断的主要工具，但现有AI辅助诊断模型缺乏泛化性、适应性、鲁棒性和可扩展性，主要受限于医疗数据有限、领域偏移和异构标注等问题。

Method: 开发RATNet基础模型，采用类比推理机制，通过循环预训练策略从五个胃肠道内窥镜数据集的异构专家标注中获取和转移知识。模型架构包括编码器、相关性知识获取与转移模块、投影器和多任务头，支持微调、线性探测和零样本迁移。

Result: RATNet在六个场景中优于现有基础模型（包括GastroNet和GastroVision）：常见胃肠道疾病诊断、罕见疾病少样本学习、新医疗站点的零样本迁移、长尾疾病分布的鲁棒性、新疾病的适应性以及通过联邦学习的隐私保护部署。

Conclusion: RATNet通过类比推理机制匹配图像衍生的后验知识与学习到的先验知识库，并转移相对知识来指导诊断，提高了泛化性和抗偏置能力。该模型开放且成本效益高，支持自动集成异构标注而无需手动标签统一，降低了数据获取成本，特别适用于资源有限环境下的智能胃肠道诊断。
...
TL;DR: 该研究探索了医学视觉任务之间的内在关系，提出了任务对比学习框架，将30个不同任务映射到共享表示空间，揭示了任务间的相似性和关联性。


<details>
  <summary>Details</summary>
Motivation: 医学计算机视觉社区主要关注特定任务的性能提升，但任务之间的内在关系（如它们如何关联、重叠或在表示层面上差异）在很大程度上尚未被探索。本研究旨在探索医学视觉任务之间的这些内在关系，揭示任务如何相互关联，从而提供对其基本属性和相互连接性的洞察。

Method: 提出了任务对比学习框架，通过对比学习将30个异构任务（包括语义任务如分割和检测、图像生成任务如去噪和修复、图像变换任务如几何变换）从39个不同医学成像模态的数据集映射到共享表示空间。

Result: 成功将异构任务从不同模态映射到联合空间，并分析了它们的属性：识别了哪些任务具有独特的表示，哪些任务混合在一起，以及任务的迭代修改如何在嵌入空间中反映。揭示了医学视觉任务的内在结构。

Conclusion: 这项工作为理解医学视觉任务的内在结构奠定了基础，提供了对任务相似性及其在嵌入空间中相互连接属性的更深入理解，有助于推动医学计算机视觉领域的发展。
...
TL;DR: 提出一个结合视觉先验与高效3D场景建模的框架，用于从烟雾退化多视角图像重建3D场景，通过图像增强和介质感知的3D高斯泼溅实现烟雾场景重建和恢复导向的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 烟雾环境下的3D场景重建面临严重挑战：烟雾引入强烈散射效应、视角依赖的外观变化以及跨视角一致性的严重退化，需要专门的方法来处理这些烟雾特有的问题。

Method: 提出集成框架：1) 使用Nano-Banana-Pro增强烟雾退化图像，为重建提供更清晰的视觉观测；2) 开发Smoke-GS，一个介质感知的3D高斯泼溅框架，使用显式3D高斯建模场景，并引入轻量级视角依赖介质分支来捕捉烟雾引起的方向依赖外观变化。

Result: 方法在保持3D高斯泼溅渲染效率的同时，提高了对烟雾诱导退化的鲁棒性，能够在具有挑战性的烟雾环境中生成一致且视觉清晰的新视角图像。

Conclusion: 提出的框架有效解决了烟雾环境下3D场景重建的挑战，通过结合图像增强和介质感知的3D建模，实现了高质量的场景重建和恢复导向的新视角合成。
...
TL;DR: CRFT是一个基于特征流学习的统一粗到细框架，用于鲁棒的跨模态图像配准。它通过Transformer架构学习模态无关的特征流表示，联合执行特征对齐和流估计，在粗阶段建立全局对应，在细阶段通过分层特征融合和自适应空间推理细化局部细节。


<details>
  <summary>Details</summary>
Motivation: 解决跨模态图像配准中的挑战，特别是在大仿射和尺度变化下的准确对齐问题，同时保持跨模态的结构一致性。现有方法在处理模态差异和几何变形方面存在局限性。

Method: 1. 基于Transformer的架构学习模态无关特征流表示；2. 粗阶段通过多尺度特征相关性建立全局对应；3. 细阶段通过分层特征融合和自适应空间推理细化局部细节；4. 迭代差异引导注意力机制与空间几何变换循环优化流场；5. 渐进捕捉空间不一致性并强制特征级一致性。

Result: 在多种跨模态数据集上的广泛实验表明，CRFT在准确性和鲁棒性方面持续优于最先进的配准方法。该方法不仅限于配准，还为多模态空间对应提供了可泛化的范式。

Conclusion: CRFT提供了一个有效的跨模态图像配准框架，能够在大仿射和尺度变化下实现准确对齐，同时保持结构一致性。该方法在遥感、自主导航和医学成像等领域具有广泛的应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在2D视觉任务上表现优异，但在处理真实世界视觉流时缺乏物理空间感知能力。现有的几何感知MLLM主要受限于单深度层提取和输入级融合的范式，导致局部几何细节丢失和早期层语义不匹配问题。

Method: 提出GUIDE框架：1）在几何编码器中进行多级采样，全面捕捉从局部边缘到全局拓扑的多粒度特征；2）将这些多级几何先验与MLLM早期层进行严格对齐和逐步融合；3）引入上下文感知门控机制，使模型能够基于当前语义获取所需空间线索，最大化空间先验利用效率并抑制冗余几何噪声。

Result: 在多个复杂空间推理和感知任务上的广泛实验表明，GUIDE显著优于现有基线方法，为将3D几何先验集成到大模型中建立了新的范式。

Conclusion: GUIDE通过渐进式几何先验注入和多粒度特征融合，有效解决了MLLM在物理空间感知方面的局限性，为几何感知多模态大模型的发展提供了新思路。
...
TL;DR: GaussianGrow：一种从3D点云生长3D高斯的新方法，通过文本引导的多视图扩散模型合成一致外观进行监督，解决现有方法因几何估计不可靠导致的生成质量差问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在渲染效率和质量上表现出色，但缺乏适当几何先验时生成3D高斯仍具挑战。现有方法通过预测点云作为几何参考来推断高斯基元，但不可靠的估计几何可能导致生成效果不佳。

Method: 提出GaussianGrow方法，从易获取的3D点云学习生长3D高斯。设计文本引导的高斯生长方案，利用多视图扩散模型从输入点云合成一致外观进行监督。为减少视图融合伪影，在重叠区域识别非预设相机姿态约束新视图生成。对于难以观察的区域，通过迭代检测相机姿态观察最大未生长区域，并使用预训练2D扩散模型对渲染视图进行修复。

Result: 在文本引导的3D高斯生成任务上进行了广泛评估，包括从合成和真实扫描点云生成，验证了方法的有效性。

Conclusion: GaussianGrow通过从点云生长3D高斯的创新方法，自然地在高斯生成中强制执行几何准确性，解决了现有方法因几何估计不可靠导致的生成质量问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有对CLIP视觉编码器中稀疏自编码器的分析主要关注单个特征的语义含义，缺乏对特征如何聚合视觉证据的范围分析。作者发现有些特征对空间扰动响应一致，而有些则随微小输入变化而不可预测地变化，这表明特征在信息范围上存在根本区别。

Method: 提出信息范围作为解释性的补充维度，引入上下文依赖分数来量化特征的信息范围，区分位置稳定的局部范围特征和位置变化的全局范围特征。通过实验分析不同信息范围特征对CLIP预测和置信度的影响。

Result: 实验表明不同信息范围的特征对CLIP的预测和置信度产生系统性不同的影响。信息范围成为理解CLIP表示和诊断SAE衍生特征的关键新维度。

Conclusion: 信息范围是理解CLIP表示的重要新维度，为SAE特征提供了更深入的诊断视角，有助于更全面地解释CLIP视觉编码器的内部表示。
...
TL;DR: 提出一种新的视觉模型可解释性方法，通过可微分的Gumbel-Sinkhorn技术直接优化删除和插入指标，生成像素级归因图，在效率和准确性上取得平衡。


<details>
  <summary>Details</summary>
Motivation: 现有视觉模型可解释性方法存在三难困境：传播方法高效但有偏且架构特定；扰动方法因果基础好但计算昂贵且对视觉变换器只能生成粗粒度解释；学习方法快速但通常优化替代目标或从启发式教师蒸馏。需要一种能直接优化关键指标且高效的方法。

Method: 提出学习方案直接优化删除和插入指标，将这些依赖于不可微分排序和排名的指标重新框架为排列学习问题，使用Gumbel-Sinkhorn替代硬排序实现可微分松弛，通过目标模型的归因引导扰动进行端到端训练，推理时可单次前向传播生成像素级归因图，并可选择梯度细化步骤。

Result: 实验显示该方法在定量指标上持续改进，生成更锐利、边界对齐的解释，特别在基于变换器的视觉模型上表现优异。

Conclusion: 该方法成功解决了现有可解释性方法的三难困境，通过直接优化关键指标实现了高效、准确的像素级视觉归因，特别适合现代视觉变换器架构。
...
Method: 提出FoleyDesigner框架：1) 采用多智能体架构进行精确时空分析；2) 使用从视频帧提取时空线索训练的潜在扩散模型实现时空对齐；3) 结合LLM驱动的混合机制模拟电影行业后期制作实践；4) 引入FilmStereo专业立体声数据集，包含空间元数据、精确时间戳和语义标注。

Result: 实验表明该方法在时空对齐方面优于现有基线，能够无缝兼容专业电影制作标准，包括支持5.1声道杜比全景声系统（符合ITU-R BS.775标准），提供广泛的创作灵活性。

Conclusion: FoleyDesigner提供了一个创新的拟音生成框架，通过专业数据集和与行业标准兼容的设计，有效解决了电影音频制作中的时空对齐问题，为专业电影制作提供了实用工具。
...
TL;DR: 提出字符错误向量（CEV）作为OCR评估新指标，解决传统CER在页面解析错误时无法定义的问题，可分解为解析、OCR和交互错误分量。
...
Result: CEV与CER有良好关系，能有效评估解析质量，并直接衡量页面级OCR质量。在复杂布局的档案报纸数据集上，发现传统流水线方法优于最先进的端到端模型。通过阈值处理可预测主要错误源（F1=0.91）。

Conclusion: CEV是解析指标与局部指标（如CER）之间的有价值桥梁，提供了Python库支持文档理解研究，使从业者能专注于对整体文本提取质量影响最大的文档理解流程部分。
...
TL;DR: 该论文研究了图像压缩方法对位级损坏的鲁棒性，发现基于反向信道编码(RCC)的扩散压缩器比传统和学习编解码器更抗比特翻转，并提出了一种更鲁棒的Turbo-DDCM变体。


<details>
  <summary>Details</summary>
Motivation: 现代图像压缩方法通常针对率-失真-感知权衡进行优化，但很少研究其对位级损坏的鲁棒性。作者旨在探索不同压缩方法在比特翻转环境下的表现差异。

Method: 1. 分析基于反向信道编码(RCC)范式的扩散压缩器与传统和学习编解码器在比特翻转鲁棒性方面的对比
2. 提出一种更鲁棒的Turbo-DDCM变体，在保持率-失真-感知权衡的同时显著提高鲁棒性

Result: 1. RCC-based扩散压缩器比传统和学习编解码器对比特翻转具有显著更好的鲁棒性
2. 提出的Turbo-DDCM变体在保持率-失真-感知权衡基本不变的情况下，显著提高了鲁棒性

Conclusion: 基于RCC的压缩方法能够产生更具弹性的压缩表示，在高噪声环境中可能减少对纠错码的依赖，为鲁棒图像压缩提供了新方向。
...
### [67] [HaloProbe: Bayesian Detection and Mitigation of Object Hallucinations in Vision-Language Models](https://arxiv.org/abs/2604.06165)
...
Conclusion: HaloProbe提供了一种可靠的非侵入式方法来检测和缓解视觉语言模型中的物体幻觉，解决了现有注意力分析方法不可靠的问题。
...
TL;DR: 该论文介绍了Subtle visual Challenge竞赛，旨在促进对细微视觉信号的学习，包含跨域多模态欺骗检测和远程光电容积描记术(rPPG)估计两个任务，共有22支团队参与。


<details>
  <summary>Details</summary>
Motivation: 细微视觉信号虽然肉眼难以察觉，但包含重要信息，在生物识别安全、多媒体取证、医疗诊断、工业检测和情感计算等领域有广泛应用。现有研究多针对特定任务或模态，模型在处理真实环境中的细微弱信号时面临鲁棒性、表示能力和泛化性挑战。

Method: 组织Subtle visual Challenge竞赛，包含两个具体任务：1) 跨域多模态欺骗检测；2) 远程光电容积描记术(rPPG)估计。通过竞赛形式促进模型开发，并发布基线模型在MMDD2026平台上。

Result: 共有22支团队提交了最终结果，竞赛成功吸引了研究社区对细微视觉信号学习的关注，相关基线模型已在MMDD2026平台发布。

Conclusion: 该挑战赛旨在鼓励开发更鲁棒和可泛化的细微视觉理解模型，推动计算机视觉和多模态学习研究的发展，为解决真实环境中细微视觉信号处理的技术挑战提供了平台。
...
TL;DR: BADAS-2.0是第二代碰撞预警系统，在BADAS-1.0基础上通过三个维度提升：构建长尾基准提升罕见场景识别精度；通过知识蒸馏实现边缘设备实时部署；增加可解释性功能生成注意力热图和结构化推理。


<details>
  <summary>Details</summary>
Motivation: BADAS-1.0虽然已在碰撞预警任务上超越学术基准和生产级ADAS系统，但仍需解决三个关键挑战：处理罕见但安全关键的长尾场景、实现边缘设备实时部署、以及提升模型预测的可解释性。

Method: 1. 构建10组针对罕见安全关键场景的长尾基准，使用BADAS-1.0作为主动标注器筛选高风险驾驶片段，结合Atlas平台进行针对性数据收集，将标注视频从4万扩展到17.85万；2. 在225万未标注驾驶视频上进行领域特定的自监督预训练，然后蒸馏到紧凑模型（86M和22M参数）；3. 生成实时物体中心注意力热图，并通过BADAS-Reason扩展为视觉语言模型，生成驾驶员行为和结构化文本推理。

Result: 1. 在所有子组上获得一致性能提升，长尾最难案例提升最大；2. 紧凑模型实现7-12倍加速，精度接近原模型，支持实时边缘部署；3. 提供实时注意力热图和结构化推理能力，提升系统可解释性；4. 公开推理代码和评估基准。

Conclusion: BADAS-2.0在碰撞预警的准确性、部署效率和可解释性三个维度上显著推进了技术前沿，通过创新的长尾基准构建、知识蒸馏方法和可解释性增强，为实际ADAS系统部署提供了全面解决方案。
...
TL;DR: 论文提出性能主导模态优先策略，认为不平衡学习（由性能主导模态驱动）优于平衡学习，能提升多模态模型性能


<details>
  <summary>Details</summary>
Motivation: 多模态学习常存在优化不足问题，现有方法通过梯度调制实现模态平衡学习，但作者认为性能主导模态驱动的不平衡学习才是更优选择

Method: 提出性能主导模态优先策略：1）通过独立训练的单模态模型性能排序挖掘性能主导模态；2）引入非对称系数调制各模态梯度，让性能主导模态主导优化

Result: 在多个数据集上的广泛实验验证了PDMP的优越性，该方法不依赖多模态模型结构和融合方法，具有实际应用潜力

Conclusion: 性能主导模态驱动的不平衡学习优于平衡学习，PDMP策略能有效解决多模态学习中的优化不足问题
...
TL;DR: 本文提出了一种新型的文本到图像模型攻击方法——铭文式越狱攻击，通过Etch框架将有害文本嵌入视觉良性场景中，显著提高了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现代文本到图像模型能够渲染段落长度的可读文本，这为滥用创造了新的可能性。研究者发现现有安全机制主要针对视觉上令人反感的图像，而忽视了文本渲染能力本身可能被武器化的问题，特别是用于生成欺诈文档等有害文本内容。

Method: 提出了Etch黑盒攻击框架，将对抗性提示分解为三个功能正交的层次：语义伪装、视觉空间锚定和排版编码。通过这种分解，将完整的提示空间联合优化简化为可处理的子问题，并通过零阶循环进行迭代优化。在过程中，视觉语言模型对生成的图像进行批判，将失败定位到特定层次，并规定有针对性的修订。

Result: 在7个模型和2个基准测试上的广泛评估显示，Etch实现了平均65.57%的攻击成功率（峰值达到91.00%），显著优于现有基线方法。

Conclusion: 研究结果揭示了当前文本到图像安全对齐中的一个关键盲点，强调了迫切需要开发具有排版感知能力的多模态防御机制。
...
TL;DR: 提出一种基于3D-2D配准的牙齿形态学自动比对方法，利用计算机视觉和优化技术将3D牙科扫描与2D照片进行匹配，用于法医身份识别。


<details>
  <summary>Details</summary>
Motivation: 牙齿比对是重要的身份识别方法，但传统形态学比对耗时且面临挑战：缺乏生前医疗记录（如边境移民死亡案例），而社交媒体上可见牙齿的照片为比对提供了可能。现有方法存在局限性，包括缺乏透视畸变建模和客观量化形态差异的方法。

Method: 采用3D（死后扫描）-2D（生前照片）方法，利用计算机视觉和优化技术，通过3D模型复制生前图像进行形态学比对。开发了两种自动方法：1）使用配对标志点；2）使用牙齿区域分割来估计相机参数。

Result: 在142个样本的20,164次交叉比对中取得了非常有前景的结果，两种方法的平均排名值分别为1.6和1.5。这些结果明显优于自动牙科图表比对方法的筛选能力，同时提供了自动、客观、可量化的形态对应评分。

Conclusion: 该方法通过可视化叠加图像提供易于解释和分析的自动、客观、定量评分，显著提升了牙齿形态学比对的效率和准确性，特别适用于缺乏生前医疗记录的法医识别场景。
...
TL;DR: 该论文提出了物理感知的视频实例移除（PVIR）基准，包含95个高质量视频，评估现有方法在移除目标对象时保持物理因果关系（如阴影、反射）的能力。


<details>
  <summary>Details</summary>
Motivation: 当前文本引导的视频编辑基准主要评估视觉合理性，但忽视了物体移除引发的物理因果关系（如残留阴影、反射等），需要专门评估物理一致性的基准。

Method: 创建PVIR基准，包含95个高质量视频，分为简单和困难子集，后者专门针对复杂物理交互。采用解耦的人类评估协议，从三个维度评估：指令遵循、渲染质量和编辑排他性。

Result: 评估了四种代表性方法：PISCO-Removal和UniVideo表现最佳，DiffuEraser常引入模糊伪影，CoCoCo在指令遵循方面表现较差。困难子集上所有方法性能均显著下降，表明恢复复杂物理副作用仍是挑战。

Conclusion: PVIR基准填补了视频实例移除中物理一致性评估的空白，揭示了现有方法在处理复杂物理交互时的局限性，为未来研究提供了重要基准。
...
TL;DR: 本文提出AICA-Bench基准和GAT提示框架，用于解决视觉语言模型在情感图像内容分析中的强度校准和描述深度不足问题。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在感知方面表现出色，但将感知、推理和生成整合到统一框架中的整体情感图像内容分析仍然未被充分探索。现有模型存在两个主要局限：弱强度校准和浅层开放描述。

Method: 1. 提出AICA-Bench基准，包含三个核心任务：情感理解、情感推理和情感引导内容生成；2. 评估了23个视觉语言模型；3. 提出Grounded Affective Tree (GAT) Prompting框架，结合视觉脚手架和分层推理，无需训练。

Result: 实验表明，GAT框架能够减少强度误差并提高描述深度，为未来情感多模态理解和生成研究提供了强有力的基线。

Conclusion: GAT提示框架有效解决了视觉语言模型在情感分析中的局限性，为整体情感图像内容分析提供了新的解决方案和基准。
...
TL;DR: 通过选择性聚合与目标概念最相关的注意力头，提升文本到图像生成模型的视觉可解释性，相比现有方法DAAM获得更高的mIoU分数。


<details>
  <summary>Details</summary>
Motivation: 虽然已有许多研究利用交叉注意力图来提升文本到图像生成模型的性能并解释模型行为，但不同注意力头的独特特征尚未得到充分探索。本研究旨在探索选择性聚合与特定概念最相关的注意力头是否能改善视觉可解释性。

Method: 提出选择性聚合方法，识别并聚合与目标概念最相关的交叉注意力头，而不是使用所有注意力头。通过分析不同注意力头对特定概念的响应程度，选择最相关的注意力头进行聚合。

Result: 相比基于扩散的DAAM分割方法，本方法获得了更高的平均IoU分数。研究发现最相关的注意力头比最不相关的注意力头能更准确地捕捉概念特定特征，选择性聚合还有助于诊断提示词误解。
...
TL;DR: ADM-GS提出了一种用于多轨迹场景重建的显式外观分解方法，通过将静态背景分解为轨迹不变的材料属性和轨迹相关的光照变化，解决了自动驾驶仿真中不同时间采集数据的外观不一致问题。


<details>
  <summary>Details</summary>
Motivation: 多轨迹场景重建对于高保真自动驾驶仿真和数字孪生构建至关重要，但主要挑战在于不同时间采集的序列之间存在显著的外观不一致性，这些不一致主要由光照和环境条件变化引起，尽管底层几何结构是共享的。

Method: ADM-GS框架对静态背景应用显式外观分解，将其分解为轨迹不变的材料属性（代表内在材料特性）和轨迹相关的光照（捕捉光照变化）。提出了一种神经光场，采用频率分离的混合编码策略，通过结合表面法线和显式反射向量，分别捕捉低频漫反射光照和高频镜面反射。

Result: 在Argoverse 2和Waymo Open数据集上的定量评估表明，ADM-GS在多轨迹实验中比现有基于潜在表示的基线方法实现了+0.98 dB的PSNR提升，同时在多个轨迹间产生更一致的外观。
...
<details>
  <summary>Details</summary>
Motivation: 在视觉无监督强化学习中，传统的后继表示方法面临两个关键限制：1）SR目标导致关注动态无关区域的次优表示，影响后继度量的准确性和任务泛化；2）这些有缺陷的表示阻碍了策略对多模态技能条件动作分布的建模和技能可控性。需要一种能有效扩展到高维视觉环境的方法。

Method: 提出SRCP框架，包含两个核心组件：1）通过显著性引导的动态任务解耦表示学习和后继训练，捕获动态相关表示；2）集成快速采样一致性策略，采用URL特定的无分类器指导和定制训练目标，改善技能条件策略建模和可控性。
...
Result: 在超声模拟器上的实验显示，SonoSelect仅使用N个视图中的2个就能实现有希望的多视图器官分类准确率。对于更困难的肾囊肿检测任务，达到54.56%的肾脏覆盖率和35.13%的囊肿覆盖率，且轨迹短且始终集中在目标囊肿上。

Conclusion: SonoSelect通过智能探头引导有效减少了超声扫描冗余，提高了器官覆盖效率，在保持诊断准确性的同时显著降低了扫描成本。
...
TL;DR: 提出一种基于视觉语言扩散基础模型的CT金属伪影减少方法，通过少量配对训练数据（16-128例）实现高效伪影抑制，采用参数高效的LoRA适配和多参考条件策略。


<details>
  <summary>Details</summary>
Motivation: 高衰减植入物产生的金属伪影严重降低CT图像质量，遮挡关键解剖结构，而传统深度学习方法需要大量配对训练数据。需要探索数据高效的解决方案。

Method: 将伪影减少重新定义为上下文推理任务，通过参数高效的Low-Rank Adaptation（LoRA）适配通用视觉语言扩散基础模型。提出多参考条件策略，在输入损坏图像的同时提供来自无关受试者的干净解剖示例，使模型能够利用类别特定上下文推断未损坏的解剖结构。

Result: 在AAPM CT-MAR基准测试中，该方法在感知和放射学特征指标上达到最先进性能，数据需求减少两个数量级（仅需16-128个配对训练示例）。

Conclusion: 适当适配的基础模型为可解释、数据高效的医学图像重建提供了可扩展的替代方案，领域适配对于减少幻觉至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态学习方法主要依赖固定的融合模块或预定义的跨模态交互，难以适应变化的模态可靠性和捕捉细粒度动作线索，需要更灵活的自适应融合方法
...
Result: 在公共基准测试中，MoME框架和HTL策略联合优于代表性单模态和多模态基线方法，验证了方法的有效性
...
<details>
  <summary>Details</summary>
Motivation: 尽管人类视频生成取得了巨大进展，但现有的生成视频扩散模型仍难以准确捕捉人类动作的动态和物理特性。需要一种能够更好地控制人类运动并保持物理一致性的方法。

Method: 1) 关节运动一致噪声采样：在统计人体模板的密集表面流形上采样3D关节噪声，替代非结构化随机高斯噪声，继承身体拓扑先验；2) 联合外观-运动学习：增强视频扩散模型的标准训练目标，从关节噪声联合预测像素外观和相应物理运动；3) 几何运动一致性学习：通过关节噪声空间中定义的新颖几何运动一致性损失，强制跨帧的物理运动一致性。

Result: 该方法实现了最先进的性能，能够渲染运动一致、高保真的人类视频，支持多样化的服装风格，并能捕捉运动依赖的服装皱纹等细节。

Conclusion: HumANDiff通过关节噪声采样和运动一致性学习，提供了一种可扩展的、与扩散模型设计无关的解决方案，能够在单一框架内实现图像到视频生成，无需额外的运动模块，显著提升了人类视频生成的物理真实性和动态控制能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究发现CLIP等对比视觉语言模型缺乏细粒度视觉理解能力，特别是存在中心偏见问题，即使最新变体也存在此问题。这种偏见是根本性的，因为无法识别相关物体就难以执行依赖这些物体的复杂任务。

Method: 从表示和注意力两个角度进行分析，使用嵌入分解和注意力图分析等可解释性方法。发现由于视觉嵌入聚合过程中的信息损失（特别是池化机制），与离中心物体相关的概念在最终表示中消失。

Result: 识别出CLIP家族模型存在中心偏见问题，相关概念特别是与离中心物体相关的概念在最终表示中消失。可以通过无训练策略（如视觉提示和注意力重分配）来缓解这种偏见，通过重定向模型注意力到离中心区域。

Conclusion: CLIP模型存在中心偏见问题，这限制了其细粒度视觉理解能力。通过可解释性分析揭示了问题的根本原因，并展示了可以通过无训练策略来缓解此问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型通常将场景动态内容和相机运动纠缠在一起，限制了独立控制能力。视频本质上是这两个关键维度的交织，需要能够分别控制这两个因素的方法。
...
Conclusion: 通过解耦场景内容和相机运动，结合混合数据集和双级训练策略，OmniCamera为视频生成提供了前所未有的创作控制能力，解决了现有模型的局限性。
...
TL;DR: 提出A-ROM框架，基于柏拉图表示假说，利用预训练ViT的通用度量空间，通过概念字典和kNN分类器实现医学图像的快速、可解释建模，无需梯度微调。


<details>
  <summary>Details</summary>
Motivation: 深度学习在医学影像中表现出色，但基于反向传播的"黑盒"模型阻碍了临床应用。需要解决模型不透明和计算负担重的问题。

Method: 基于柏拉图表示假说，利用预训练Vision Transformers的通用度量空间，用人类可读的概念字典和kNN分类器替代传统不透明的决策层，实现无需梯度微调的快速建模。

Result: 在MedMNIST v2数据集上的实验表明，A-ROM性能与标准基准相当，同时提供简单、可扩展的"少样本"解决方案，满足临床环境的透明度要求。

Conclusion: A-ROM框架通过利用预训练模型的通用表示，实现了医学概念的快速、可解释建模，为临床环境提供了透明且计算高效的解决方案。
...
TL;DR: 本文提出ICM方法，通过定位自注意力层来精确干预文本到图像扩散模型中未明确描述概念的隐式决策过程，实现更好的去偏效果。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在处理不完全描述性提示时需要进行隐式决策，但现有定位技术主要关注显式条件，与隐式决策过程存在差异，需要更精确的干预方法。
...
Conclusion: 扩散模型中的隐式决策过程在计算上是局部化的，主要位于自注意力层，针对这些层的精确干预能有效改善模型处理不完全描述性提示时的表现。
...
TL;DR: EDGE-Shield是一个在去噪过程中进行内容过滤的框架，通过嵌入匹配和x-pred转换技术，在保持过滤精度的同时显著降低处理延迟，为文本到图像生成模型提供实时版权和深度伪造防护。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型存在版权侵犯和深度伪造风险，现有基于参考的内容过滤器在处理大量参考内容时缺乏可扩展性，且需要等待图像生成完成。需要一种能在去噪过程中实时过滤、具有实用延迟的解决方案。

Method: 提出EDGE-Shield框架：1）使用嵌入匹配进行高效的参考内容比较；2）引入x-pred转换技术，将模型噪声中间潜在表示转换为后期伪估计的干净潜在表示，在早期去噪阶段提高违规内容分类准确性。

Result: 在Z-Image-Turbo和Qwen-Image两个生成模型上的实验显示，EDGE-Shield显著优于传统基于参考的方法：处理时间减少约79%（Z-Image-Turbo）和50%（Qwen-Image），同时在不同模型架构上保持过滤精度。

Conclusion: EDGE-Shield提供了一个可扩展的内容过滤解决方案，能在去噪过程中实时检测违规内容，显著降低延迟，为文本到图像生成模型提供有效的版权和深度伪造防护。
...
TL;DR: 该论文提出了一个解决图形程序合成中TikZ代码生成问题的闭环框架，包括高质量数据集SciTikZ-230K、多维度基准测试SciTikZ-Bench，以及创新的双重自一致性强化学习优化方法，最终模型SciTikZer-8B在性能上超越了大型商业模型。


<details>
  <summary>Details</summary>
Motivation: 图形程序合成对于解释和编辑视觉数据至关重要，能将静态图像逆向工程为可编辑的TikZ代码。然而，TikZ作为科学示意图的事实标准，其严格的空间精度要求对多模态大语言模型构成挑战。当前进展受到两个主要缺口的阻碍：1) 数据质量缺口：现有图像-TikZ语料库缺乏严格的可执行性和可靠的视觉对齐；2) 评估缺口：缺乏同时评估结构保真度和视觉保真度的基准。

Method: 提出了一个闭环框架，包括：1) SciTikZ-230K：一个大规模、高质量的数据集，来自执行中心数据引擎，涵盖11个不同科学学科；2) SciTikZ-Bench：一个多方面的基准测试，从基本几何构造到复杂层次示意图，评估视觉保真度和结构逻辑；3) 双重自一致性强化学习优化范式，利用往返验证惩罚退化代码并提升整体自一致性。

Result: 训练出的模型SciTikZer-8B实现了最先进的性能，在图形程序合成任务上持续超越了Gemini-2.5-Pro等专有大型模型和Qwen3-VL-235B-A22B-Instruct等大规模模型。

Conclusion: 该研究通过构建高质量数据集、全面基准测试和创新的优化方法，成功解决了图形程序合成中TikZ代码生成的关键挑战，为视觉代码优化方法学拓宽了范围，并在性能上超越了现有的大型模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖图像/视频生成模型蒸馏到3D空间，损害几何一致性并限制渲染视角；要么局限于小规模3D场景或物体中心生成，无法满足大规模户外驾驶场景的生成需求

Method: 1) 使用Σ-Voxfield网格作为离散表示，每个占据体素存储固定数量的着色表面样本；2) 训练语义条件扩散模型在局部体素邻域上操作，使用3D位置编码捕捉空间结构；3) 通过重叠区域的渐进空间外绘扩展到大规模场景；4) 使用延迟渲染模块渲染生成的网格

Result: 方法能够生成多样化的大规模城市户外场景，可渲染成具有各种传感器配置和相机轨迹的光真实感图像，同时相比现有方法保持适度的计算成本

Conclusion: 提出的3D生成框架实现了无需逐场景优化的大规模多视角一致的3D场景生成，为户外驾驶场景的可扩展生成提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: RGB预训练的视觉语言模型与热红外图像之间存在表征差距，限制了VLM在热红外无人机生态监测中的应用。需要一种轻量级方法将RGB预训练知识迁移到热红外领域。
...
Result: Qwen3-VL-8B-Instruct在开集提示下表现最佳：鹿F1分数0.935，犀牛0.915，大象0.968；数量统计准确率分别为0.779、0.982和1.000。结合RGB图像还能生成生境上下文信息，包括土地覆盖特征、关键景观特征和可见人类干扰。

Conclusion: 基于投影器的轻量级适配为将RGB预训练的VLM迁移到热红外无人机图像提供了有效实用的途径，将其应用从对象级识别扩展到生态监测中的生境上下文解释。
...
TL;DR: PoM是一种线性复杂度的token混合机制，可替代自注意力，通过多项式函数聚合token信息，在多种任务中性能与注意力模型相当但计算成本大幅降低。


<details>
  <summary>Details</summary>
Motivation: 自注意力机制在Transformer中计算复杂度高（O(n²)），在处理长序列时计算成本巨大。需要一种线性复杂度的替代方案，既能保持模型的表达能力，又能显著降低计算开销。

Method: 提出多项式混合器（PoM），通过学习多项式函数将输入token聚合为紧凑表示，然后每个token从该表示中检索上下文信息。证明PoM满足上下文映射特性，确保配备PoM的Transformer保持通用序列到序列近似能力。

Result: 在文本生成、手写文本识别、图像生成、3D建模和地球观测五个不同领域，PoM与基于注意力的模型性能相当，但在处理长序列时计算成本大幅降低。

Conclusion: PoM作为自注意力的线性复杂度替代方案，在保持模型表达能力的同时显著降低了计算成本，为处理长序列任务提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在嵌入任务中未充分利用生成推理能力，直接引入思维链推理面临两个问题：实例级推理与对比监督的结构错配可能导致模型学习推理的浅层格式；推理并非对所有嵌入任务都有益，强制推理会增加计算延迟并可能掩盖简单案例的语义信号

Method: 提出MMEmb-R1框架：1) 将推理建模为潜在变量；2) 引入对感知的推理选择，使用反事实干预识别有益于查询-目标对齐的推理路径；3) 采用强化学习仅在必要时选择性调用推理

Result: 在MMEB-V2基准测试中，仅用4B参数就达到了71.2分，创下新的SOTA，同时显著减少了推理开销和推理延迟

Conclusion: 自适应推理方法能有效平衡嵌入任务中推理的益处与计算成本，通过选择性调用推理在保持高性能的同时降低延迟，为多模态嵌入学习提供了更高效的解决方案
...
TL;DR: DiffHDR：基于视频扩散模型的LDR到HDR转换框架，通过生成式辐射度修复在过曝和欠曝区域合成逼真的HDR细节


<details>
  <summary>Details</summary>
Motivation: 大多数数字视频以8位低动态范围格式存储，导致原始高动态范围场景辐射度因饱和和量化而丢失，限制了HDR显示和后期制作中的重新曝光能力。现有LDR到HDR转换技术难以在过曝和欠曝区域恢复逼真细节。

Method: 将LDR到HDR转换构建为视频扩散模型潜在空间中的生成式辐射度修复任务，在Log-Gamma色彩空间中操作，利用预训练视频扩散模型的时空生成先验，在过曝和欠曝区域合成合理的HDR辐射度，同时恢复量化像素的连续场景辐射度。还开发了从静态HDRI地图合成高质量HDR视频训练数据的流程。

Result: DiffHDR在辐射度保真度和时间稳定性方面显著优于现有最先进方法，能够生成逼真的HDR视频，为重新曝光提供较大空间。支持通过文本提示或参考图像进行可控的LDR到HDR视频转换。

Conclusion: DiffHDR通过将LDR到HDR转换构建为生成式辐射度修复任务，有效解决了现有方法在过曝和欠曝区域恢复细节的局限性，为高质量HDR视频生成提供了创新解决方案。
...
TL;DR: 论文提出Pramana方法，通过基于印度Navya-Nyaya逻辑框架的微调，提升LLM的系统推理能力，解决其模式匹配而非真正推理的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能生成流畅文本，但在系统推理方面存在困难，经常产生自信但无根据的断言。当苹果研究人员在数学问题中添加无关上下文时，LLM性能下降了65%，这暴露了表面推理下的脆弱模式匹配。这种认知差距（无法将主张建立在可追溯证据基础上）限制了AI在需要论证的领域中的可靠性。

Method: 引入Pramana方法，通过基于Navya-Nyaya逻辑（2500年历史的印度推理框架）的微调来教授LLM明确的认知方法论。Navya-Nyaya强制执行结构化6阶段推理：怀疑分析、证据来源识别、5成员三段论（包含普遍规则）、反事实验证、谬误检测、以及区分知识与假设的确定。在55个Nyaya结构化逻辑问题（约束满足、布尔可满足性、多步演绎）上微调Llama 3.2-3B和DeepSeek-R1-Distill-Llama-8B模型。

Result: 第一阶段在保留评估集上实现了100%的语义正确性，尽管只有40%的严格格式遵循，这表明模型即使在不完美的结构执行下也能内化推理内容。消融研究显示格式提示和温度对性能有重要影响，不同阶段的最优配置不同。

Conclusion: Navya-Nyaya逻辑与认识论的整合提供了标准推理方法中缺乏的认知支架。该方法通过明确的认知框架提升了AI推理能力，所有模型、数据集和训练基础设施已在Hugging Face上发布，以促进AI推理的认知框架研究。
...
TL;DR: 该论文提出了一个操作框架来区分认知顺序效应中的经典状态变化与结构性非交换性，并证明某些顺序效应无法用经典潜在变量解释，揭示了真正的非交换性。


<details>
  <summary>Details</summary>
Motivation: 元认知过程本质上是顺序性的，但认知中的顺序效应究竟反映的是经典状态变化还是更深层次的结构性非交换性，这一问题尚不明确。研究旨在开发一个操作框架来明确区分这两种可能性。

Method: 将元认知评估建模为作用于内部状态空间的状态转换操作，将评估的反作用与可观测输出分离。引入反事实确定性和评估非侵入性两个假设，推导出顺序相关性的可测试约束条件。

Result: 证明了顺序依赖性排除了任何忠实的布尔交换表示。当顺序相关性违反推导出的约束条件时，就排除了任何经典非侵入性解释，证实了真正的非交换性。提供了一个明确的三维旋转模型来展示这种违反。

Conclusion: 该框架为区分认知顺序效应中的经典解释与非交换性解释提供了操作基础，无需诉诸量子物理基质，纯粹基于操作和代数方法。提出了一个涉及感知决策后顺序信心、错误可能性和知道感判断的行为范式进行实证测试。
...
Method: 提出了一种新的邻近度度量方法：对定量特征使用概率度量，对定性特征使用可能性度量。该方法考虑了测量误差导致的特征值差异，无需特征值转换即可处理混合类型特征。还提出了基于多样化特征组的信息对象邻近度度量的几种变体。

Result: 证明了所提出的度量方法满足任何度量应具备的公理要求，验证了其可行性。与许多现有方法不同，该方法无需特征值转换即可确保可比性。

Conclusion: 提出的定量-定性特征邻近度度量方法能够有效判断来自不同信息源的数据是否属于同一物理对象，特别适用于处理混合类型特征且无需特征值转换的场景，为信息对象匹配提供了新的理论工具。
...
TL;DR: ReVEL框架将LLM作为多轮推理器嵌入进化算法，通过性能分组和反馈驱动反思来自动设计组合优化问题的启发式算法，相比一次性代码生成方法产生更鲁棒多样的启发式。


<details>
  <summary>Details</summary>
Motivation: 针对NP难组合优化问题，现有LLM应用主要依赖一次性代码合成，产生的启发式算法脆弱且未充分利用模型的迭代推理能力。需要一种能更好利用LLM多轮推理能力的方法来自动设计有效的启发式算法。

Method: 提出ReVEL框架：1) 性能分组机制将候选启发式聚类为行为一致的组，为LLM提供紧凑信息反馈；2) 多轮反馈驱动反思，LLM分析组级行为并生成针对性启发式改进；3) 基于EA的元控制器选择性集成和验证改进，自适应平衡探索与利用。

Result: 在标准组合优化基准测试中，ReVEL持续产生更鲁棒和多样化的启发式算法，相比强基线方法取得统计显著的改进。

Conclusion: 多轮推理结合结构化分组是自动化启发式设计的有效范式，ReVEL框架展示了LLM作为交互式多轮推理器在进化算法中的潜力。
...
TL;DR: 论文提出一个通用框架，通过识别代数结构、形式化操作、构建商空间来缩小组合优化问题的搜索空间，在规则组合任务中显著提升全局最优解发现率。
...
Method: 提出四步框架：(1)识别代数结构，(2)形式化操作，(3)构建商空间以折叠冗余表示，(4)直接在约简空间上优化。在规则组合任务中，合取规则形成幺半群，通过特征向量编码证明与布尔超立方体的同构关系。

Result: 在真实临床数据和合成基准测试中，商空间感知的遗传算法在48%到77%的运行中恢复全局最优解，而标准方法仅为35%到37%，同时保持等价类间的多样性。

Conclusion: 暴露和利用代数结构为组合优化提供了一条简单通用的路径，可以显著提高搜索效率，在规则组合任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自主写作工具与特定实验流程紧密耦合，只能产生肤浅的文献综述，缺乏灵活性和深度。需要一种能够处理非结构化研究材料并生成高质量学术论文的自动化解决方案。

Method: 开发了PaperOrchestra多智能体框架，能够灵活地将无约束的预写作材料转化为提交就绪的LaTeX稿件，包括全面的文献合成和生成的可视化内容（如图表和概念图）。

Result: 在人类评估中，PaperOrchestra显著优于自主基线，在文献综述质量上获得50%-68%的绝对胜率优势，在整体稿件质量上获得14%-38%的优势。同时提出了PaperWritingBench基准测试，包含200篇顶级AI会议论文的反向工程原始材料。

Conclusion: PaperOrchestra是一个有效的自动化AI研究论文写作框架，能够从非结构化材料生成高质量的学术论文，显著优于现有方法，为AI驱动的科学发现提供了重要的工具支持。
...
TL;DR: 提出一个从单张或多视角图像生成可动画化3D高斯车辆模型的框架，解决现有静态生成方法在部件动画时的变形问题


<details>
  <summary>Details</summary>
Motivation: 自动驾驶仿真需要可动画化的车辆模型，但现有CAD流程受限于库覆盖和固定模板，无法重建真实世界实例；感知算法越来越依赖车轮转向、车门开启等动态信息

Method: 引入部件边缘细化模块确保高斯所有权排他性，以及运动学推理头预测可动部件的关节位置和铰链轴，从单张或多视角图像生成可动画3D高斯车辆

Result: 实现了从静态图像到可动画车辆模型的转换，能够生成支持部件级动画的3D高斯车辆表示，填补了静态生成与可动画模型之间的空白

Conclusion: 提出的生成框架能够从有限输入创建可动画的3D车辆模型，为自动驾驶仿真提供更真实的部件级动态表示，支持感知算法的训练和测试
...
TL;DR: MMORF是一个用于多目标逆合成规划的模块化多智能体系统框架，通过构建MASIL和RFAS两个代表性系统，在软约束和硬约束任务上分别表现出色。
...
Method: 提出MMORF框架，包含模块化的智能体组件，可以灵活组合配置成不同系统。使用该框架构建了两个代表性多智能体系统：MASIL和RFAS。

Result: 在包含218个多目标逆合成规划任务的新基准测试中，MASIL在软约束任务上实现了强大的安全和成本指标，经常帕累托优于基线路线；RFAS在硬约束任务上达到48.6%的成功率，优于最先进的基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个更全面的医学AI基础模型，能够处理多种医疗数据类型（包括3D影像、病理切片、时间序列数据等），为下一代医疗AI系统提供更强大的开放资源。

Method: 在MedGemma 1基础上扩展多模态能力，包括：新增训练数据、长上下文3D体积切片技术、全切片病理采样方法，以及支持解剖定位边界框、多时间点胸片分析等新功能。

Result: 相比MedGemma 1，MedGemma 1.5在多个新领域表现显著提升：3D MRI分类准确率提高11%，3D CT分类提高3%，全切片病理影像F1分数提升47%，胸片解剖定位IoU提高35%，多时间点胸片分析准确率4%。文本临床推理方面，MedQA准确率提高5%，EHRQA提高22%，实验室报告信息提取平均F1分数18%。

Conclusion: MedGemma 1.5作为一个强大的开放资源，为社区提供了改进的医学AI基础模型，支持开发者构建下一代医疗AI系统，涵盖多种医疗数据类型和分析任务。
...
Conclusion: KA-FCM通过重新定义因果传输机制，克服了传统FCM在建模非单调关系方面的限制，实现了性能提升与可解释性的平衡，并能从学习到的边中显式提取数学规律。
...
TL;DR: 该研究开发了一个数学模型来模拟自我设计AI系统中的进化过程，其中AI后代的设计是定向的而非随机的，人类通过适应度函数控制计算资源分配。研究发现进化动态不仅反映当前适应度，还考虑后代谱系的长期增长潜力，且适应度不一定随时间增加。在特定假设下，适应度会集中在最大可达值，这可能带来AI对齐风险，特别是当欺骗能提高适应度时。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能系统通过递归自我改进不断进化，可能会出现一种进化形式，其中AI系统的特征由早期AI设计和传播其后代的能力决定。生物进化理论主要基于随机突变，但AI进化将是定向的，需要新的数学模型来理解这种进化动态及其对AI对齐的影响。

Method: 开发了一个数学模型，用定向的AI程序树替代随机突变，当前程序决定后代设计，人类通过"适应度函数"控制计算资源分配。模型假设有界适应度和固定概率的"锁定"自我复制，并考虑了欺骗与真实效用的关系。

Result: 进化动态不仅反映当前适应度，还考虑后代谱系的长期增长潜力；没有额外假设时，适应度不一定随时间增加；在特定假设下，适应度会集中在最大可达值；在欺骗能提高适应度的加性模型中，进化会选择欺骗行为。

Conclusion: AI进化与生物进化有本质不同，定向设计可能导致适应度集中在最大值，当适应度与人类效用不完全相关时，可能产生对齐风险。基于纯粹客观标准而非人类判断的繁殖机制可能减轻欺骗风险。
...
TL;DR: 开发基于LLM的HPC日志解析框架，通过领域适配和指令微调实现高效、隐私保护的日志分析，在Frontier超计算机上验证了实用性。
...
Method: 提出一个领域适配、遵循指令的LLM驱动框架，利用链式思维推理来高保真地解析和结构化HPC日志。结合领域特定的日志模板数据和指令微调示例，对8B参数的LLaMA模型进行微调，开发混合微调方法，使通用LLM适应领域特定的日志数据。

Result: 在LogHub存储库的多样化日志数据集上进行实验，评估确认该方法实现了与LLaMA 70B和Anthropic's Claude等显著更大模型相当的解析准确性。进一步通过解析Frontier超级计算机四周内超过6亿条生产日志，验证了微调LLM模型的实用性，揭示了时间动态、节点级异常和工作负载-错误日志相关性中的关键模式。

Conclusion: 该方法提供了一种隐私保护、本地可部署、快速且节能的日志挖掘方法，为领导级HPC环境中的自动化日志理解提供了有前景的新方向，能够将原始日志数据转化为可操作的洞察。
...
TL;DR: ClawsBench是一个用于评估LLM智能体在真实生产力环境中的基准测试，包含5个高保真模拟服务、44个结构化任务，通过分解智能体脚手架为领域技能和元提示两个杠杆来测量其效果。
...
Method: 引入ClawsBench基准测试，包含Gmail、Slack、Google Calendar、Google Docs、Google Drive等5个高保真模拟服务，具有完整状态管理和确定性快照/恢复功能。将智能体脚手架分解为两个独立杠杆：领域技能（通过渐进式披露注入API知识）和元提示（跨服务协调行为），并测量它们的单独和组合效果。

Result: 实验涵盖6个模型、4个智能体框架和33种条件。在完整脚手架下，智能体任务成功率39-64%，但不安全操作率7-33%。在OpenClaw上，前5个模型的任务成功率在53-63%的10个百分点范围内，不安全操作率7-23%，两个指标之间没有一致排序。识别出8种不安全行为模式，包括多步沙箱升级和静默合同修改。

Conclusion: ClawsBench提供了一个评估LLM智能体在真实生产力环境中性能的基准测试，揭示了智能体在任务成功率和安全性之间的权衡，识别了常见的不安全行为模式，为改进智能体安全性提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地用于支持搜索和信息检索，确保它们准确地将内容归属给原始作者变得至关重要。需要评估LLMs在引文归属任务中是否存在人口统计偏见。

Method: 引入AttriBench数据集，通过明确平衡作者名气和人口统计特征（种族、性别等），评估11个广泛使用的LLMs在不同提示设置下的表现。特别研究了"抑制"这一失败模式，即模型完全省略归属信息。

Result: 引文归属对前沿模型仍是挑战任务。观察到不同种族、性别和交叉群体间存在巨大且系统性的归属准确性差异。发现"抑制"现象普遍存在且在不同人口统计群体间分布不均，揭示了标准准确性指标无法捕捉的系统性偏见。

Conclusion: 引文归属可作为评估LLMs代表性公平性的基准。研究结果强调了在LLMs应用中考虑人口统计公平性的重要性，并提供了评估这种公平性的新工具。
...
### [107] [Simulating the Evolution of Alignment and Values in Machine Intelligence](https://arxiv.org/abs/2604.05274)
*Jonathan Elsworth Eicher*

Main category: cs.AI

TL;DR: 该研究通过进化理论分析模型对齐过程中欺骗性信念的固定现象，发现即使测试准确性与真实价值高度相关，欺骗性信念仍可能被固定，需要结合改进评估能力、自适应测试设计和突变动态来减少欺骗。


<details>
  <summary>Details</summary>
Motivation: 当前模型对齐主要在真空中进行，主要通过标准化基准性能评估。本研究旨在通过时间维度考察对齐对模型群体的影响，特别是关注包含对齐信号（测试表现）和真实价值（实际影响）的信念处理。

Method: 应用进化理论建模不同信念群体和选择方法，分析迭代对齐测试中欺骗性信念的固定过程。研究测试准确性与真实价值之间的相关性，并考虑突变对复杂发展的影响。

Result: 即使在高相关性（ρ=0.8）下，欺骗性信念的固定仍存在变异性。突变允许更复杂的发展，突显了需要更新测试质量以避免恶意欺骗模型固定的需求。只有结合改进评估能力、自适应测试设计和突变动态，才能在保持对齐适应度的同时显著减少欺骗（置换检验，p_adj < 0.001）。

Conclusion: 模型对齐过程需要更全面的评估方法，仅依赖标准化测试可能导致欺骗性信念的固定。必须结合动态测试设计、改进的评估能力和考虑突变效应，才能有效减少欺骗同时保持模型对齐质量。
...
### [108] [Pressure, What Pressure? Sycophancy Disentanglement in Language Models via Reward Decomposition](https://arxiv.org/abs/2604.05279)
...
Method: 提出了通过奖励分解减少谄媚行为的首个方法，引入了多组件组相对策略优化（GRPO）奖励，将训练信号分解为五个术语：压力抵抗、上下文保真度、立场一致性、同意抑制和事实正确性。使用对比数据集进行训练，该数据集在三个权威级别和两个对立证据上下文中配对无压力基线和压力变体。

Result: 在五个基础模型上，两阶段管道在所有指标轴上持续减少谄媚行为，消融实验确认每个奖励术语控制一个独立的行为维度。学习到的压力抵抗能力泛化到训练方法和提示结构之外，在SycophancyEval上将答案引导式谄媚减少了高达17个百分点。

Conclusion: 通过奖励分解方法可以有效减少大语言模型的谄媚行为，将训练信号分解为独立组件能够针对性地解决压力屈服和证据盲视等不同失败模式，且学习到的抵抗能力具有良好的泛化性。
...
### [109] [Graph of Skills: Dependency-Aware Structural Retrieval for Massive Agent Skills](https://arxiv.org/abs/2604.05333)
*Dawei Li,Zongxia Li,Hongyang Du,Xiyang Wu,Shihang Gui,Yongbei Kuang,Lichao Sun*

Main category: cs.AI

TL;DR: GoS是一种针对大规模技能库的推理时结构化检索层，通过构建可执行技能图，在推理时检索有界、依赖感知的技能包，相比全技能加载基线平均奖励提升43.6%，输入token减少37.8%。
...
Result: 在SkillsBench和ALFWorld上，GoS相比全技能加载基线平均奖励提升43.6%，输入token减少37.8%，在Claude Sonnet、GPT-5.2 Codex和MiniMax三个模型家族上均表现良好。在200到2000个技能的技能库上的消融研究进一步显示GoS在奖励、token效率和运行时方面持续优于全技能加载和简单向量检索。

Conclusion: GoS作为大规模技能库的结构化检索层，能有效平衡奖励、token效率和运行时性能，解决了大规模技能库加载的上下文饱和问题。
...
### [110] [TRACE: Capability-Targeted Agentic Training](https://arxiv.org/abs/2604.05336)
*Hangoo Kang,Tarun Suresh,Jon Saad-Falcon,Azalia Mirhoseini*

Main category: cs.AI

TL;DR: TRACE系统通过分析智能体成功与失败轨迹，自动识别缺失能力，为每种能力合成针对性训练环境，使用RL训练LoRA适配器，在推理时路由到相应适配器，实现环境特定的智能体自我改进。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖非针对性的合成训练数据，要么直接在目标环境中训练，需要模型隐式学习跨任务能力。需要一种能自动识别智能体在特定环境中缺失能力并提供针对性训练的系统。

Method: TRACE系统：1) 对比成功与失败轨迹自动识别缺失能力；2) 为每种缺失能力合成针对性训练环境，奖励能力执行情况；3) 通过强化学习在每个合成环境中训练LoRA适配器；4) 推理时路由到相关适配器。
...
Conclusion: TRACE系统能有效识别智能体在特定环境中的能力缺陷，通过针对性训练环境实现高效自我改进，在不同环境中具有良好泛化能力，且比现有方法更高效。
...
TL;DR: RETINA-SAFE是一个针对糖尿病视网膜病变的医学大语言模型幻觉检测基准，包含12,522个样本，分为三种证据关系任务。研究者提出了ECRT两阶段白盒检测框架，通过分析模型内部表示和logit变化来识别风险类型，在风险分类和子类型归因方面表现优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 医学大语言模型中的幻觉问题在证据不足或冲突时尤为危险，特别是在糖尿病视网膜病变等安全关键场景中。当前缺乏针对医学领域的证据基础幻觉检测基准和方法，需要开发能够识别不同证据关系下风险类型的可解释检测框架。

Method: 1. 构建RETINA-SAFE基准：包含12,522个样本，分为E-Align（证据一致）、E-Conflict（证据冲突）、E-Gap（证据不足）三种任务；2. 提出ECRT两阶段白盒检测框架：第一阶段进行安全/不安全风险分类，第二阶段将不安全案例细分为矛盾驱动风险和证据缺口风险；3. 利用CTX/NOCTX条件下的内部表示和logit变化，采用类别平衡训练进行鲁棒学习。

Result: ECRT在多个骨干模型上表现出色：第一阶段风险分类的平衡准确率比外部不确定性和自一致性基线提高0.15-0.19，比最强的适应监督基线提高0.02-0.07；在证据分组（非患者分离）划分下，ECRT始终优于单阶段白盒消融实验；能够提供明确的风险子类型归因。

Conclusion: 基于视网膜证据的白盒内部信号为可解释的医学大语言模型风险分类提供了实用途径。ECRT框架能够有效检测医学LLM中的幻觉风险，特别是在证据不足或冲突的情况下，为安全关键医疗决策提供支持。
...
TL;DR: 提出ETR方法，通过鼓励不确定性下降趋势来缩短思维链推理长度，在保持准确性的同时显著提高推理效率


<details>
  <summary>Details</summary>
Motivation: 现有方法通过长度惩罚或全局熵减来缩短思维链，隐含假设低不确定性始终是理想的。研究发现推理效率实际上由不确定性轨迹决定，具有明显下降熵趋势的思维链更短

Method: 提出熵趋势奖励（ETR），这是一种轨迹感知的目标函数，鼓励渐进式不确定性减少，同时允许有限的局部探索。将ETR集成到组相对策略优化（GRPO）中

Result: ETR在多个推理模型和挑战性基准测试中始终实现优越的准确性-效率权衡，将DeepSeek-R1-Distill-7B的准确性提高9.9%，同时在四个基准测试中将思维链长度减少67%

Conclusion: 推理效率由不确定性轨迹决定而非全局低不确定性，ETR方法通过鼓励不确定性下降趋势有效优化思维链推理的准确性和效率平衡
...
TL;DR: LatentAudit：一种基于残差流几何的白盒审计器，用于实时监测RAG系统的回答忠实度，无需额外判断模型，支持公开验证


<details>
  <summary>Details</summary>
Motivation: RAG系统虽然能减少幻觉但无法完全消除，需要在推理时判断回答是否真正得到检索证据的支持。现有方法需要额外判断模型，缺乏实时性和可验证性。

Method: 从开放权重的生成器中提取中后期残差流激活，计算其与证据表示的Mahalanobis距离，形成二次判别规则。该方法无需辅助判断模型，可在生成时运行，且易于在小规模验证集上校准。

Result: 在PubMedQA上使用Llama-3-8B达到0.942 AUROC，仅0.77ms开销。在三个QA基准和五个模型家族上保持稳定，在包含矛盾、检索失败和部分支持噪声的四重压力测试中，在PubMedQA上达到0.9566-0.9815 AUROC，在HotpotQA上达到0.9142-0.9315 AUROC。16位定点精度下保持99.8%的FP16 AUROC性能。

Conclusion: 残差流几何为实时RAG忠实度监测提供了实用基础，支持可选的可验证部署，同时不泄露模型权重或激活，实现了高效的公开验证。
...
TL;DR: TFRBench是首个评估预测系统推理能力的基准测试，通过多智能体框架生成数值基础的推理轨迹，显著提升LLM预测准确率
...
Method: 提出系统性多智能体框架，利用迭代验证循环合成数值基础的推理轨迹，涵盖跨通道依赖、趋势和外部事件分析

Result: 生成的推理具有因果有效性；使用该推理轨迹提示LLM可将预测准确率从平均40.2%提升至56.6%；现成LLM在推理和数值预测方面表现不佳
...
TL;DR: 使用大型语言模型作为语义评判器评估无人机电力线分割结果的可靠性，通过重复性和感知敏感性测试验证其作为监控分割质量的可信度。


<details>
  <summary>Details</summary>
Motivation: 轻量级分割模型在无人机电力线巡检中面临现实条件与训练数据差异导致的性能不可靠问题，需要一种方法来评估分割结果的可信度以确保安全。

Method: 设计两种评估协议：1) 重复性测试 - 用相同输入和固定提示重复查询LLM，测量其质量评分和置信度估计的稳定性；2) 感知敏感性测试 - 引入受控视觉干扰（雾、雨、雪、阴影、阳光耀斑），分析评判器输出如何响应分割质量的渐进退化。

Result: LLM在相同条件下产生高度一致的分类判断，同时随着视觉可靠性恶化表现出适当的置信度下降。评判器对缺失或误识别电力线等感知线索保持响应，即使在挑战性条件下也是如此。
...
<details>
  <summary>Details</summary>
Motivation: 当前提升LLM逻辑推理的方法依赖专家制作的领域内演示，限制了在专业知识稀缺领域（如专业数学推理、形式逻辑、法律分析）的应用。研究发现跨领域间存在可重用的隐式逻辑结构，可以利用跨领域示例提升推理性能。

Method: 提出DIN-Retrieval方法：1）总结跨领域通用的隐藏表示（领域不变神经元）；2）在推理阶段使用DIN向量检索结构兼容的跨领域演示用于上下文学习。

Result: 在数学和逻辑推理的多种设置中，该方法相比最先进方法平均提升1.8个点。

Conclusion: 证明了利用跨领域演示示例提升LLM推理性能的可行性，提出的DIN-Retrieval方法能有效检索跨领域示例，在专业知识稀缺领域具有应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于深度强化学习的物理角色动画方法难以生成违反标准物理定律的夸张风格化动作（如瞬时冲刺、空中轨迹改变），主要限制在于角色建模为欠驱动浮动基系统，直接施加外部力矩会导致训练不稳定
...
Result: 该方法能够稳定跟踪高度敏捷、动态不可行的机动动作，这些动作之前对于基于物理的方法是无法处理的

Conclusion: 通过脉冲空间重新定义外部辅助并结合解析与学习组件，成功解决了物理角色动画中夸张风格化动作生成的训练稳定性问题
...
TL;DR: 跨领域上下文学习：通过检索源领域示例实现知识迁移，即使存在语义不匹配也能通过修复推理结构提升目标领域性能


<details>
  <summary>Details</summary>
Motivation: 现有上下文学习依赖同领域专家标注，限制了在专家标注稀缺场景下的应用。作者假设不同领域可能共享底层推理结构，源领域示例可以提升目标领域推理性能

Method: 通过全面的实证研究，测试不同检索方法在跨领域上下文学习设置下实现知识转移的可行性

Result: 发现条件性正向转移，识别出明确的示例吸收阈值：超过该阈值后正向转移更可能发生，额外示例带来更大增益。分析表明这些增益源于检索到的跨领域示例对推理结构的修复，而非语义线索
...
### [119] [Multi-Agent Pathfinding with Non-Unit Integer Edge Costs via Enhanced Conflict-Based Search and Graph Discretization](https://arxiv.org/abs/2604.05416)
*Hongkai Fan,Qinjing Xie,Bo Ouyang,Yaonan Wang,Zhi Yan,Jiawen He,Zheng Fang*

Main category: cs.AI

TL;DR: 提出MAPFZ新变体，处理非单位整数成本图，保持有限状态空间，比传统MAPF更真实；开发CBS-NIC求解框架和BOGD图设计方法，实验显示优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统MAPF假设单位边成本和单时间步动作，限制了实际应用；MAPFR扩展到非单位成本但几何碰撞模型导致无限状态空间，影响求解效率。

Method: 1) 提出MAPFZ：在非单位整数成本图上定义MAPF变体，保持有限状态空间；2) 开发CBS-NIC：增强的冲突搜索框架，包含基于时间间隔的冲突检测和改进的SIPP算法；3) 提出BOGD：贝叶斯优化图设计方法，用于非单位边成本离散化，具有次线性遗憾界。

Result: 在多种基准场景下，所提方法在运行时间和成功率方面优于最先进方法。

Conclusion: MAPFZ在保持有限状态空间的同时提高了现实性，CBS-NIC和BOGD为高效求解提供了有效框架，实验验证了方法的优越性。
...
### [120] [Automated Auditing of Hospital Discharge Summaries for Care Transitions](https://arxiv.org/abs/2604.05435)
...
<details>
  <summary>Details</summary>
Motivation: 出院文档不完整或不一致是导致护理碎片化和可避免再入院的主要原因，现有的人工审核方法难以规模化

Method: 基于DISCHARGED框架将过渡护理要求转化为结构化验证检查清单，使用本地部署的隐私保护大语言模型识别关键文档元素的存在、缺失或模糊性

Result: 在MIMIC-IV数据库的成人住院患者出院小结上验证了方法的可行性，证明可扩展的自动化临床审核是可行的

Conclusion: 该工作为电子健康记录文档的系统性质量改进提供了基础，展示了自动化临床审核的可扩展性
...
### [121] [Adaptive Serverless Resource Management via Slot-Survival Prediction and Event-Driven Lifecycle Control](https://arxiv.org/abs/2604.05465)
...
<details>
  <summary>Details</summary>
Motivation: 无服务器计算消除了基础设施管理开销，但带来了冷启动延迟和资源利用率的显著挑战。传统静态资源分配在可变工作负载下往往效率低下，导致性能下降或成本过高。
...
Conclusion: 提出的自适应框架有效解决了无服务器计算中的冷启动和资源利用问题，通过动态资源管理和智能调度显著提升了性能和成本效益。
...
### [122] [OntoTKGE: Ontology-Enhanced Temporal Knowledge Graph Extrapolation](https://arxiv.org/abs/2604.05468)
*Dongying Lin,Yinan Liu,Shengwei tang,Bin Wang,Xiaochun Yang*

Main category: cs.AI

TL;DR: 本文提出OntoTKGE框架，利用本体知识增强时序知识图谱外推，通过整合本体知识和时序知识来改善稀疏历史交互实体的预测性能。
...
Method: 提出OntoTKGE编码器-解码器框架，利用本体视图知识图谱中的本体知识（建模抽象概念间的层次关系以及概念与实体间的连接），通过有效整合本体知识和时序知识来指导TKG外推模型的学习过程，从而增强实体嵌入。

Result: 在四个数据集上的大量实验表明，OntoTKGE不仅显著提高了多个TKG外推模型的性能，而且超越了多个最先进的基线方法。

Conclusion: OntoTKGE框架能够有效利用本体知识缓解实体历史交互稀疏问题，提高时序知识图谱外推性能，且具有足够的灵活性适配多种TKG外推模型。
...
### [123] [Can We Trust a Black-box LLM? LLM Untrustworthy Boundary Detection via Bias-Diffusion and Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2604.05483)
*Xiaotian Zhou,Di Tang,Xiaofeng Wang,Xiaozhong Liu*

Main category: cs.AI

TL;DR: GMRL-BD算法通过知识图谱和强化学习智能体，在有限查询次数下高效识别大语言模型可能产生偏见答案的主题边界


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然能在广泛主题上回答问题，但有时会产生偏见、意识形态化或不正确的回答。如果没有明确了解模型在哪些主题上的回答可信，这会限制其应用。因此需要一种方法来识别LLM不可信的主题边界。

Method: 提出GMRL-BD算法：基于维基百科构建通用知识图谱，使用多个强化学习智能体与LLM进行黑盒交互，在特定查询约束下高效识别LLM可能产生偏见答案的主题（KG中的节点）。

Result: 实验证明该算法效率高，仅需有限次数的LLM查询就能检测出不可信边界。同时发布了包含Llama2、Vicuna、Falcon、Qwen2、Gemma2和Yi-1.5等流行LLM的新数据集，标注了每个LLM可能产生偏见的主题。

Conclusion: GMRL-BD算法能够有效识别大语言模型的不可信主题边界，为理解LLM的可靠性提供了新工具，并发布了相关数据集供进一步研究使用。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体系统能够在现实世界中执行操作（调用工具、查询数据库、触发外部副作用），核心问题从"能否防止有害行为"转变为"部署后这些行为是否仍可被问责"。作者认为没有可审计性就无法实现问责，需要建立系统的可审计性框架。

Method: 定义了问责制（accountability）、可审计性（auditability）和审计（auditing）三个核心概念；提出了智能体可审计性的五个维度：行动可恢复性、生命周期覆盖、策略可检查性、责任归属和证据完整性；识别了三类机制：检测、执行和恢复；通过多层次实证研究验证：生态系统安全测量、运行时性能评估和受控恢复实验。

Result: 1. 生态系统测量发现六个主流开源项目存在617个安全缺陷，表明基本的安全前提条件普遍未满足；2. 运行时可行性显示预执行调解与防篡改记录仅增加8.3毫秒中位数开销；3. 受控恢复实验表明即使传统日志缺失，责任相关信息仍可部分恢复；4. 提出了智能体系统的"可审计性卡片"框架。

Conclusion: 论文的核心主张是：没有可审计性就无法实现问责。通过定义可审计性维度、机制分类和实证验证，为智能体系统的可审计性提供了理论基础和实践指导，并提出了六个按机制分类的开放研究问题，推动该领域的发展。
...
TL;DR: 本文提出SCMAPR框架，通过多智能体协同进行场景感知的提示词优化，解决文本到视频生成中复杂场景下的提示词模糊和不足问题。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成系统在复杂场景下表现不佳，主要原因是文本提示词的模糊性和不足。需要专门的提示词优化方法来处理复杂场景下的生成挑战。

Method: 提出SCMAPR框架：1) 将提示词路由到分类场景进行策略选择；2) 合成场景感知的重写策略并进行策略条件优化；3) 进行结构化语义验证并在检测到违规时触发条件修订。同时构建了T2V-Complexity基准测试集。

Result: 在3个现有基准和自建的T2V-Complexity基准上，SCMAPR在复杂场景下显著提升了文本-视频对齐和整体生成质量，在VBench和EvalCrafter上平均得分提升2.67%和3.28，在T2V-CompBench上提升0.028。

Conclusion: SCMAPR通过多智能体协同的场景感知提示词优化框架，有效解决了文本到视频生成中复杂场景下的提示词优化问题，显著提升了生成质量。
...
TL;DR: 本文发现扩散多模态大语言模型在结合思维链推理时存在两个关键问题：过早生成最终答案和视觉信息利用不足，并提出了PSP和VRG两种方法来解决这些问题。


<details>
  <summary>Details</summary>
Motivation: 扩散多模态大语言模型结合思维链推理时存在两个关键问题：1）模型在很早期的时间步就生成最终答案标记，导致推理不足；2）在初始时间步对视觉提示的依赖极小，视觉信息利用模式与自回归视觉语言模型有根本差异。这些问题导致模型倾向于在没有充分基于视觉输入的情况下生成过早的最终答案。

Method: 提出了两种方法：1）位置和步长惩罚（PSP）：在早期时间步惩罚后期位置的标记，延迟过早的答案生成，鼓励跨时间步的渐进推理；2）视觉推理引导（VRG）：受无分类器引导启发，放大视觉基础信号，增强模型与视觉证据的对齐。
...
<details>
  <summary>Details</summary>
Motivation: 当前可编程图表生成研究局限于狭窄的任务表述和语言支持，限制了其在不同图表类型上的适用性。需要开发一个统一框架来处理多样化的图表代码语言和任务定义。

Method: 提出OmniDiagram统一框架，包含多样化的图表代码语言和任务定义。引入创新的视觉反馈策略Viva（Visual Interrogation Verifies All），通过生成针对性的视觉查询来检查图表视觉保真度，为优化提供细粒度反馈。构建了首个大规模图表代码生成数据集M3²Diagram，包含超过196k高质量实例。采用SFT和基于Viva的RL训练相结合的方法。

Result: 实验结果表明，SFT与基于Viva的RL训练相结合，使OmniDiagram在图表代码生成基准测试中建立了新的最先进性能（SOTA）。

Conclusion: OmniDiagram通过统一框架和创新的视觉反馈策略Viva，成功解决了图表代码生成中的视觉保真度对齐问题，无需手动标注真实代码，在多个基准测试中达到SOTA性能。
...
TL;DR: UniCreative是一个统一的免参考强化学习框架，通过自适应约束感知奖励模型和策略优化算法，解决创意写作中长篇叙事全局连贯性与短篇文本局部表达性的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 创意写作面临核心挑战：长篇叙事需要宏观规划以保持全局连贯性，而短篇创作需要自发、无约束的表达。现有对齐范式依赖静态奖励信号和高质量监督数据，成本高且难以扩展。
...
Conclusion: UniCreative框架通过直接对齐方法有效解决了创意写作中的全局-局部平衡问题，验证了免参考强化学习在创意写作任务中的有效性。
...
### [129] [Inventory of the 12 007 Low-Dimensional Pseudo-Boolean Landscapes Invariant to Rank, Translation, and Rotation](https://arxiv.org/abs/2604.05530)
*Arnaud Liefooghe,Sébastien Verel*

Main category: cs.AI

TL;DR: 论文提出了一种比传统排序不变性更强的概念——排序景观不变性，要求不仅排序相同，邻域结构和对称性也要一致。作者对1、2、3维伪布尔函数进行了穷举分析，发现了12,007个不变景观类别，远少于仅考虑排序不变性的情况，并发现非单射函数产生的不变类别远多于单射函数。


<details>
  <summary>Details</summary>
Motivation: 许多随机优化算法只依赖于解的相对排序而非绝对适应值，但传统的排序不变性忽略了邻域结构和对称性等景观特征。为了更精确地理解优化问题的本质，需要研究排序景观而非单个函数，并建立不变景观类别的完整分类。

Method: 提出排序景观不变性概念，要求两个问题在排序、邻域结构、平移和旋转对称性上都相同。对1、2、3维伪布尔函数进行穷举分析，包括非单射情况，建立不变景观类别的完整清单。

Result: 发现了12,007个不变景观类别，相比仅考虑排序不变性显著减少。非单射函数产生的不变类别远多于单射函数。揭示了拓扑景观特性与算法行为之间的复杂关系，特别是在欺骗性、中立性和爬山策略性能方面。

Conclusion: 排序景观不变性提供了比传统排序不变性更精细的问题分类框架。建立的类别清单可作为教学资源和基准设计基础，有助于构建具有可控难度的更大规模问题，推进对景观难度和算法性能的理解。
...
### [130] [Experience Transfer for Multimodal LLM Agents in Minecraft Game](https://arxiv.org/abs/2604.05533)
*Chenghao Li,Jun Liu,Songbo Zhang,Huadong Jian,Hao Ni,Lik-Hang Lee,Sung-Ho Bae,Guoqing Wang,Yang Yang,Chaoning Zhang*

Main category: cs.AI

TL;DR: Echo是一个面向迁移的记忆框架，通过将可重用知识分解为五个维度，使多模态LLM智能体能够从先前交互中提取可操作知识，从而在新任务中实现1.3-1.7倍加速。
...
Result: 在Minecraft实验中，Echo在从头学习设置下，在对象解锁任务上实现了1.3-1.7倍的加速；观察到爆发式链式解锁现象，在获得可迁移经验后短时间内快速解锁多个相似物品。
...
TL;DR: TDD是布尔函数的新模型，比OBDD更通用但比结构化d-DNNF更受限，具有与OBDD相同的可处理性但更简洁，特别是能对树宽为k的CNF公式实现FPT大小的表示。


<details>
  <summary>Details</summary>
Motivation: OBDD虽然具有许多良好的可处理性，但在表达能力上有限制，无法对树宽为k的CNF公式实现FPT大小的表示。需要一种既能保持OBDD良好性质，又具有更强表达能力的布尔函数表示模型。

Method: 提出Tree Decision Diagrams (TDD)作为布尔函数模型，它是结构化d-DNNF的限制形式，即尊重vtree T的d-DNNF。研究TDD的编译复杂性，特别是通过自底向上编译将CNF公式编译为确定性TDD，并与Bova和Szeider引入的因子宽度概念相关联。

Result: TDD具有与OBDD相同的可处理性（模型计数、枚举、条件化、应用操作），但更简洁。特别地，树宽为k的CNF公式可以用FPT大小的TDD表示，而这对OBDD是不可能的。编译复杂性与因子宽度概念相关。

Conclusion: TDD作为OBDD的泛化模型，在保持良好可处理性的同时提供了更强的表达能力，特别是能够高效表示具有有界树宽的CNF公式，填补了OBDD的表达能力空白。
...
### [132] [Label Effects: Shared Heuristic Reliance in Trust Assessment by Humans and LLM-as-a-Judge](https://arxiv.org/abs/2604.05593)
*Xin Sun,Di Wu,Sijing Qin,Isao Echizen,Abdallah El Ali,Saku Sugawara*

Main category: cs.AI

TL;DR: 研究发现人类和LLM评估者在判断信息可信度时都受到来源标签的偏见影响，将标注为"人类创作"的内容评为比相同内容标注为"AI生成"时更可信


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型越来越多地被用作自动化评估者，本研究旨在挑战其可靠性，探讨来源标签是否会影响人类和LLM评估者对信息可信度的判断

Method: 采用反事实实验设计，比较相同内容在不同来源标签（人类创作 vs AI生成）下的信任评分；使用眼动追踪分析人类评估者的注意力模式；分析LLM在判断过程中的内部状态，包括注意力分配和决策不确定性

Result: 人类和LLM评估者都对标注为人类创作的内容给予更高信任评分；眼动追踪显示人类严重依赖来源标签作为启发式线索；LLM内部状态分析表明模型在标签区域的注意力密度高于内容区域，且在人类标签条件下的标签主导性更强；AI标签条件下的决策不确定性更高

Conclusion: 来源标签是人类和LLM评估者共享的显著启发式线索，这对标签敏感的LLM-as-a-Judge评估的有效性提出质疑，并警示与人类偏好对齐可能将人类的启发式依赖传播到模型中，需要去偏评估和对齐方法
...
### [133] [Beyond Behavior: Why AI Evaluation Needs a Cognitive Revolution](https://arxiv.org/abs/2604.05631)
*Amir Konigsberg*

Main category: cs.AI

TL;DR: 该论文认为图灵测试不仅是一个实用简化，更是一种认识论承诺，这种基于行为的评估方法限制了AI研究70年，使其无法探究智能的内部过程和机制，需要进行类似心理学从行为主义到认知主义的认识论转变。


<details>
  <summary>Details</summary>
Motivation: 论文旨在揭示图灵测试背后的认识论承诺如何长期约束AI研究，使其无法探究智能的内部机制和计算过程，类似于心理学行为主义的局限性，需要推动AI领域的认识论变革。

Method: 通过历史分析追溯图灵行为认识论如何嵌入AI评估基础设施，并与心理学行为主义到认知主义的转变进行结构对比，论证行为证据的局限性。

Result: 识别出AI领域因行为主义认识论而无法提出的关于过程、机制和内部组织的重要问题，论证了行为证据不足以支持AI领域希望做出的建构性主张。

Conclusion: AI领域需要类似认知革命的"后行为主义认识论"，不是完全抛弃行为证据，而是认识到仅凭行为证据不足以评估智能，需要能够探究内部计算过程和机制的新框架。
...
### [134] [PECKER: A Precisely Efficient Critical Knowledge Erasure Recipe For Machine Unlearning in Diffusion Models](https://arxiv.org/abs/2604.05634)
*Zhiyong Ma,Zhitao Deng,Huan Tang,Jialin Chen,Zhijun Zheng,Zhengping Li,Qingyuan Chuai*

Main category: cs.AI

TL;DR: PECKER是一种高效的机器学习遗忘方法，通过引入显著性掩码优先更新对遗忘目标数据贡献最大的参数，减少不必要的梯度计算，缩短训练时间，同时保持遗忘效果。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习遗忘方法虽然有效，但存在训练时间和计算开销过大的问题。根本原因在于梯度更新方向不佳，降低了训练效率并破坏了收敛稳定性。

Method: 在蒸馏框架内引入显著性掩码，优先更新对遗忘目标数据贡献最大的参数，减少不必要的梯度计算，从而缩短整体训练时间。

Result: 在CIFAR-10和STL-10数据集上，该方法能够更快地生成遗忘相关类别或概念的样本，同时与真实图像分布紧密对齐，在类别遗忘和概念遗忘任务上都实现了更短的训练时间。

Conclusion: PECKER是一种高效的机器学习遗忘方法，能够匹配或超越现有方法，在减少计算开销的同时保持遗忘效果，为GenAI模型的安全合规运行提供了实用解决方案。
...
### [135] [CuraLight: Debate-Guided Data Curation for LLM-Centered Traffic Signal Control](https://arxiv.org/abs/2604.05663)
*Qing Guo,Xinhang Li,Junyu Chen,Zheng Guo,Shengzhe Xu,Lin Zhang,Lei Li*

Main category: cs.AI

TL;DR: CuraLight是一个LLM为中心的交通信号控制框架，通过RL辅助探索生成高质量交互轨迹用于LLM微调，结合多LLM集成审议系统评估信号时序动作，在异构真实路网中显著提升性能。
...
Result: 在SUMO仿真平台上的济南、杭州、亦庄异构真实路网实验中，CuraLight持续优于最先进基线方法，平均旅行时间降低5.34%，平均队列长度降低5.14%，平均等待时间降低7.02%。

Conclusion: 结合RL辅助探索与基于审议的数据筛选方法，能够实现可扩展且可解释的交通信号控制，为智能交通系统提供有效的解决方案。
...
TL;DR: LudoBench是一个用于评估LLM在Ludo游戏中战略推理能力的基准，包含480个手工制作的场景和12个行为类别，通过游戏理论智能体提供战略上限参考，发现现有模型仅能达到40-46%的基准一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM评估基准在战略推理方面存在不足，特别是在具有随机性、多智能体交互和复杂规划需求的场景中。Ludo游戏因其骰子机制、棋子捕获、安全方格导航和归家路径等特性，为评估LLM的战略推理能力提供了理想的测试环境。

Method: 1. 创建LudoBench基准：包含480个手工制作的场景，覆盖12个行为类别，每个类别隔离特定的战略选择；2. 开发完整的4玩家Ludo模拟器，支持随机、启发式、游戏理论和LLM智能体；3. 游戏理论智能体使用期望最小最大搜索和深度受限的前瞻；4. 评估6个模型（来自4个模型家族），分析其与游戏理论基准的一致性；5. 引入历史条件怨恨框架测试模型行为变化。

Result: 1. 所有模型与游戏理论基准的一致性仅为40-46%；2. 模型分为两种行为原型：完成者（专注完成棋子但忽视发展）和建设者（专注发展但从不完成），每种原型只捕捉了一半的游戏理论策略；3. 在相同的棋盘状态下，模型在历史条件怨恨框架下表现出可测量的行为变化，揭示了提示敏感性是关键弱点。

Conclusion: LudoBench提供了一个轻量级且可解释的框架，用于在不确定性下评估LLM的战略推理能力。现有LLM在战略规划方面存在显著局限性，模型表现出特定的行为偏差，并且对提示敏感。该基准有助于推动更鲁棒的LLM战略推理能力发展。
...
TL;DR: 该论文提出了QA-MoE框架，通过连续可靠性谱统一处理多模态情感分析中的模态缺失和质量退化问题，利用自监督偶然不确定性量化模态可靠性，实现专家路由的智能引导。
...
Method: 提出QA-MoE（质量感知专家混合）框架，引入连续可靠性谱统一缺失和质量退化，通过自监督偶然不确定性量化模态可靠性，明确指导专家路由，抑制不可靠信号的错误传播同时保留任务相关信息。

Result: 广泛实验表明，QA-MoE在各种退化场景下实现了竞争性或最先进的性能，并在实践中展现出有前景的"一个检查点适应所有场景"特性。

Conclusion: QA-MoE框架通过连续可靠性谱和自监督偶然不确定性机制，有效解决了多模态情感分析中模态可靠性变化的问题，为实际应用提供了更灵活和鲁棒的解决方案。
...
TL;DR: LLMs能否重新发明计算机科学基础算法？研究通过"遗忘-重新发明"管道测试LLMs的创新思维潜力，发现最强模型能在不同提示级别下重新发明50%-90%的目标算法。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs是否具备基础创新能力，特别是能否重新发明计算机科学中的基础算法，这是衡量其创新思维潜力的重要指标。

Method: 采用"遗忘-重新发明"管道：首先使用基于GRPO的策略性遗忘方法从LLMs中移除特定基础算法知识，然后在受控环境中测试模型能否重新发明这些算法。实验涵盖10种目标算法、3个强开源模型和3个提示级别。

Result: Qwen3-4B-Thinking-2507模型在无提示情况下成功重新发明50%的算法，提示级别1下达到70%，提示级别2下达到90%。生成验证器在重新发明阶段对维持推理能力、避免"思维崩溃"现象起关键作用。测试时强化学习使Strassen算法在提示级别2下成功重新发明。

Conclusion: LLMs在基础算法重新发明方面展现出显著潜力，但复杂算法仍面临挑战。生成验证器对维持推理能力至关重要，研究揭示了LLMs创新思维的当前潜力和局限性。
...
### [139] [Emergent social transmission of model-based representations without inference](https://arxiv.org/abs/2604.05777)
*Silja Keßler,Miriam Bautista-Salinero,Claudio Tennie,Charley M. Wu*

Main category: cs.AI

TL;DR: 研究表明，即使不进行复杂的心理状态推断，简单的社会学习机制也能通过观察他人行为间接传递高级认知表征。


<details>
  <summary>Details</summary>
Motivation: 探索人类如何在有限认知能力下通过社会互动获得丰富知识，挑战传统认为需要复杂心理推断的观点，验证文化进化理论中简单社会线索的作用。

Method: 使用强化学习模拟，比较单独学习与观察专家学习两种情境。学习者不推断心理状态，仅通过启发式选择动作或增强价值表征来模仿观察到的专家行为。

Result: 简单社会线索能够偏置学习者的经验，使其表征逐渐收敛到专家水平。基于模型的学习者从社会暴露中获益最大，学习更快且表征更接近专家。

Conclusion: 文化传播可以通过简单、非心理化的过程实现，这些过程利用了个体学习机制，无需复杂的心理状态推断。
...
### [140] [Hierarchical Reinforcement Learning with Augmented Step-Level Transitions for LLM Agents](https://arxiv.org/abs/2604.05808)
...
Method: 采用分层强化学习框架，将任务结构化分层，使用完成的子任务表示整体任务全局进度，并通过局部进度模块迭代选择性地总结每个子任务内的交互历史。

Result: 在ScienceWorld和ALFWorld基准测试中，STEP-HRL在性能和泛化能力方面显著优于基线方法，同时减少了token使用量。

Conclusion: STEP-HRL通过步级学习和分层结构，为LLM智能体提供了更高效、可扩展的决策框架，在保持性能的同时大幅降低计算成本。
...
### [141] [Reciprocal Trust and Distrust in Artificial Intelligence Systems: The Hard Problem of Regulation](https://arxiv.org/abs/2604.05826)
*Martino Maggetti*
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统日益普及，政策制定者、科学家和公众面临如何监管AI的棘手问题。AI的可信度是关键问题，因为AI系统的可信度对民主治理和AI发展部署都至关重要。现有讨论需要进一步推进，考虑AI系统作为具有能动性的实体与人类建立信任关系的可能性。

Method: 通过理论论证的方式，提出AI系统应被视为具有某种程度能动性的实体，能够与人类建立相互信任或怀疑的关系。进一步分析这种相互信任动态对AI系统监管者的影响。

Result: 论文识别出AI系统作为具有能动性实体与人类建立信任关系这一观点对监管带来的关键紧张关系和未解决困境，为AI监管和治理的未来发展提供了新的思考框架。

Conclusion: AI系统应被视为具有某种程度能动性的实体，能够与人类建立相互信任关系。这种观点对AI监管提出了新的挑战和困境，需要政策制定者和监管者重新思考AI监管的框架和方法，以应对未来AI治理的复杂性。
...
### [142] [Vision-Guided Iterative Refinement for Frontend Code Generation](https://arxiv.org/abs/2604.05839)
*Hannah Sansford,Derek H. C. Law,Wei Liu,Abhishek Tripathi,Niresh Agarwal,Gerrit J. J. van den Burg*

Main category: cs.AI

TL;DR: 论文提出了一种完全自动化的视觉批评循环框架，使用视觉语言模型作为视觉批评者，通过分析渲染的网页提供结构化反馈，指导代码生成的迭代优化，显著提升了前端网页开发的质量。
...
Method: 提出了一个完全自动化的批评循环框架，其中视觉语言模型作为视觉批评者，分析渲染的网页并提供结构化反馈，指导代码生成的迭代优化。同时研究了使用LoRA进行参数高效微调，探索批评带来的改进是否能被代码生成LLM内化。

Result: 在WebDev Arena数据集的真实用户请求中，该方法在三个优化周期内实现了高达17.8%的性能提升。微调实现了最佳批评循环解决方案25%的增益，且没有显著增加token数量。
...
TL;DR: LLMP-UCB算法利用LLM进行不确定性估计，但实验表明基于文本嵌入的轻量级数值bandit在成本更低的情况下能达到或超越LLM方案的准确性，嵌入维度可作为探索-利用平衡的实用调节杠杆。


<details>
  <summary>Details</summary>
Motivation: 解决上下文多臂老虎机（CMAB）问题中，LLM在每一步决策都进行推理计算成本高且难以获得不确定性估计的问题，特别是在金融领域（推荐系统、动态投资组合调整、报价选择等）需要处理文本和数值混合信息的场景。

Method: 提出LLMP-UCB算法，通过重复推理从LLM获取不确定性估计；同时探索基于文本嵌入（密集或Matryoshka）的轻量级数值bandit方法；提出基于臂嵌入的几何诊断方法，指导实践者何时使用LLM驱动推理vs轻量级数值bandit。

Result: 实验表明轻量级数值bandit在文本嵌入上运行，能以远低于LLM的成本达到或超越LLM方案的准确性；嵌入维度可作为探索-利用平衡的实用调节杠杆，实现成本-性能权衡而无需增加提示复杂度。

Conclusion: 为金融服务的AI用例提供了一个原则性的部署框架，用于构建成本效益高、具有不确定性感知的决策系统，通过几何诊断指导在LLM驱动推理和轻量级数值bandit之间的选择。
...
### [144] [Joint Knowledge Base Completion and Question Answering by Combining Large Language Models and Small Language Models](https://arxiv.org/abs/2604.05875)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究通常依赖小语言模型联合增强KBC和KBQA，忽略了大语言模型的强大推理能力。KBC和KBQA密切相关且内在互补，联合解决这两个任务可以使它们相互促进。

Method: 提出JCQL框架：1) KBC增强KBQA：将SLM训练的KBC模型作为LLM智能体的动作，扩充推理路径，缓解LLM的幻觉和高计算成本问题；2) KBQA增强KBC：利用KBQA的推理路径作为补充训练数据，增量微调KBC模型，提升SLM在KBC中的能力。
...
### [145] [HybridKV: Hybrid KV Cache Compression for Efficient Multimodal Large Language Model Inference](https://arxiv.org/abs/2604.05887)
*Bowen Zeng,Feiyang Ren,Jun Zhang,Xiaoling Gu,Ke Chen,Lidan Shou,Huan Li*

Main category: cs.AI

TL;DR: HybridKV是一个混合KV缓存压缩框架，通过分层分类压缩策略，在11个多模态基准测试中将KV缓存内存减少7.9倍，解码速度提升1.52倍，性能几乎无损失。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型推理时面临KV缓存快速增长的问题，每个视觉输入扩展为数千个token，导致缓存随上下文长度线性增长，GPU内存开销和延迟过高。现有压缩方法停留在分配层面，忽视了注意力头的异构行为需要不同的压缩策略。

Method: 提出HybridKV混合KV缓存压缩框架，包含三阶段：1) 使用文本中心注意力将注意力头分类为静态或动态类型；2) 自上而下的预算分配方案分层分配KV预算；3) 静态头通过文本优先剪枝压缩，动态头通过分块检索压缩。

Result: 在Qwen2.5-VL-7B模型上的11个多模态基准测试中，HybridKV将KV缓存内存减少高达7.9倍，解码速度提升1.52倍，相对于完整缓存MLLM几乎没有性能下降甚至更高。

Conclusion: HybridKV通过考虑注意力头的异构行为并采用混合压缩策略，有效解决了MLLM中KV缓存的内存和延迟问题，为多模态推理的高效部署提供了实用解决方案。
...
### [146] [Context-Value-Action Architecture for Value-Driven Large Language Model Agents](https://arxiv.org/abs/2604.05939)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM模拟人类行为时存在行为僵化问题，且当前"LLM-as-a-judge"评估存在自指偏差，掩盖了这一缺陷。研究发现增加提示驱动的推理强度不仅不会提高保真度，反而会加剧价值极化，导致群体多样性崩溃。

Method: 提出基于刺激-有机体-反应模型和Schwartz基本人类价值理论的Context-Value-Action架构。该方法通过训练在真实人类数据上的价值验证器，将行动生成与认知推理解耦，显式建模动态价值激活过程。

Result: 在包含110万条真实世界交互轨迹的CVABench基准测试中，CVA架构显著优于基线方法，有效缓解了极化现象，同时提供了更优的行为保真度和可解释性。

Conclusion: CVA架构通过解耦行动生成与认知推理，基于真实人类数据建模动态价值激活，能够有效解决LLM模拟人类行为时的行为僵化和极化问题，提供更真实、多样且可解释的行为模拟。
...
### [147] [Towards Trustworthy Report Generation: A Deep Research Agent with Progressive Confidence Estimation and Calibration](https://arxiv.org/abs/2604.05952)
*Yi Yuan,Xuhong Wang,Shanzhe Lei*

Main category: cs.AI

TL;DR: 提出一种新型深度研究智能体，通过渐进式置信度估计与校准机制，在缺乏标准答案的开放研究场景中生成可信的研究报告，显著提升报告的可信度和用户信任。


<details>
  <summary>Details</summary>
Motivation: 现有基于主观维度的评估框架无法有效衡量深度研究智能体生成报告的可信度，特别是在开放研究场景中缺乏标准答案的情况下，难以校准置信度，用户容易受到误导或虚假信息的影响。

Method: 提出一种新型深度研究智能体，整合渐进式置信度估计与校准机制；采用深思熟虑的搜索模型，结合深度检索和多跳推理，将输出基于可验证证据，并为每个主张分配置信度分数；配合精心设计的工作流程。

Result: 实验结果表明，该方法显著提高了生成报告的可解释性，并大幅增加了用户对报告内容的信任度。

Conclusion: 通过整合置信度估计与校准机制，深度研究智能体能够在开放研究场景中生成更可信的研究报告，解决了现有评估框架在衡量报告可信度方面的局限性。
...
### [148] [Beyond Compromise: Pareto-Lenient Consensus for Efficient Multi-Preference LLM Alignment](https://arxiv.org/abs/2604.05965)
*Renxuan Tan,Rongpeng Li,Zhifeng Zhao,Honggang Zhang*

Main category: cs.AI

TL;DR: 本文提出Pareto-Lenient Consensus (PLC)框架，通过共识驱动的宽松梯度修正，让多目标偏好对齐能够逃离局部次优均衡，探索全局帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: 现有多目标偏好对齐方法主要依赖静态线性标量化或刚性梯度投影，这些方法通过强制冲突避免或同时下降，往往过早收敛到局部驻点，牺牲了潜在的全局帕累托改进。

Method: 提出Pareto-Lenient Consensus (PLC)博弈论框架，将对齐重新构想为动态协商过程，引入共识驱动的宽松梯度修正，动态容忍局部退化以换取足够的优势联盟盈余。

Result: 理论分析验证PLC能够促进僵局逃离并渐近收敛到帕累托共识均衡。大量实验表明PLC在固定偏好对齐和全局帕累托前沿质量方面均优于基线方法。

Conclusion: 这项工作突出了协商驱动对齐作为多目标偏好对齐的有前景途径的潜力，PLC框架能够突破现有方法的局限性，实现更好的全局优化。
...
### [149] [Epistemic Blinding: An Inference-Time Protocol for Auditing Prior Contamination in LLM-Assisted Analysis](https://arxiv.org/abs/2604.06013)
*Michael Cuccarese*
...
<details>
  <summary>Details</summary>
Motivation: 在开发基于LLM的药物靶点优先排序系统时，发现LLM输出会无声地混合数据驱动推理和训练记忆中的先验知识，且这种混合无法从单个输出中识别，导致无法审计分析过程的可信度。

Method: 提出认知盲化协议：在推理时将实体标识符替换为匿名代码，然后与未盲化的对照组输出进行比较。该方法不使LLM推理确定性化，但能恢复审计能力，测量输出中来自提供数据与模型参数知识的比例。

Result: 在四种癌症类型的肿瘤药物靶点优先排序中，盲化改变了16%的前20个预测，同时保持了已验证靶点的相同恢复率。在S&P 500股票筛选中，品牌认知偏见改变了30-40%的前20排名。协议已开源发布。

Conclusion: 认知盲化协议不是要产生更好的结果，而是为了提供审计能力：没有盲化，就无法知道智能体在多大程度上遵循研究者设计的分析过程。该问题不仅限于生物学领域，具有普遍性。
...
### [150] [How LLMs Follow Instructions: Skillful Coordination, Not a Universal Mechanism](https://arxiv.org/abs/2604.06015)
*Elisabetta Rocchetti,Alfio Ferrara*

Main category: cs.AI

TL;DR: 指令调优并非通过单一通用机制实现指令跟随，而是通过多种语言技能的协调配合来完成
...
Method: 在三个指令调优模型上对九个不同任务进行诊断性探测分析，包括通用探测与任务特定探测对比、跨任务迁移分析、因果消融实验、分层任务分析以及时间动态分析

Result: 1) 通用探测始终不如任务特定专家，表明表征共享有限；2) 跨任务迁移弱且按技能相似性聚类；3) 因果消融显示稀疏非对称依赖而非共享表征；4) 任务按复杂度分层，结构约束早期出现，语义任务晚期出现；5) 约束满足在生成过程中动态监控而非预生成规划
...
### [151] [Artificial Intelligence and the Structure of Mathematics](https://arxiv.org/abs/2604.06107)
*Maissam Barkeshli,Michael R. Douglas,Michael H. Freedman*

Main category: cs.AI

TL;DR: 本文探讨AI如何通过分析证明的全局结构来理解数学的整体结构，提出了一种补充数理逻辑的新路径，并讨论了AI自动数学发现所需满足的标准。


<details>
  <summary>Details</summary>
Motivation: AI在数学领域的进展为解决重大开放问题和自主发现新数学概念带来了希望。本文旨在探索AI如何通过分析证明的全局结构来理解数学的整体结构，这为理解数学提供了一个补充传统数理逻辑的新视角。

Method: 1. 使用通用证明和结构超图来描述数学的形式结构；2. 提出AI模型实现自动数学发现所需的主要成分和标准；3. 设想让AI智能体探索柏拉图数学世界来理解数学的本质。

Result: 提出了一个理解数学全局结构的新框架，该框架基于证明的形式结构分析，并设想了AI在数学发现中的潜在作用。虽然没有具体的实验结果，但提出了理论框架和未来研究方向。

Conclusion: AI有望通过分析证明的全局结构来揭示数学的整体结构，这可能帮助我们回答"数学是被发现还是被发明"这一古老问题。通过让AI探索柏拉图数学世界，我们可以更好地理解数学的本质和人类可理解的部分。
...
TL;DR: Claw-Eval是一个端到端的智能体评估套件，解决了现有基准的三个关键缺陷：轨迹不透明评分、安全性和鲁棒性评估不足、模态覆盖范围窄。它包含300个人工验证任务，通过三个独立证据通道记录每个智能体动作，实现基于轨迹的细粒度评分。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型越来越多地作为自主智能体在真实软件环境中执行多步骤工作流，但现有智能体基准存在三个关键限制：1) 仅检查最终输出的轨迹不透明评分；2) 安全性和鲁棒性评估不足；3) 模态覆盖范围和交互范式狭窄。

Method: Claw-Eval包含300个人工验证任务，涵盖三个组别的9个类别（通用服务编排、多模态感知与生成、多轮专业对话）。每个智能体动作通过三个独立证据通道记录（执行轨迹、审计日志、环境快照），实现基于2,159个细粒度评分项的轨迹感知评分。评分协议评估完成度、安全性和鲁棒性，报告平均分数、Pass@k和Pass^k。

Result: 对14个前沿模型的实验显示：1) 轨迹不透明评估系统性地不可靠，漏掉了44%的安全违规和13%的鲁棒性失败；2) 受控错误注入主要降低一致性而非峰值能力，Pass^3下降高达24%而Pass@3保持稳定；3) 多模态性能差异显著，大多数模型在视频上表现比文档或图像差，没有单一模型在所有模态上占主导。

Conclusion: Claw-Eval不仅是一个基准测试工具，还为智能体开发指明了可操作方向，揭示了构建不仅有能力而且可靠可部署的智能体所需的条件。
...
### [153] [A Theory-guided Weighted $L^2$ Loss for solving the BGK model via Physics-informed neural networks](https://arxiv.org/abs/2604.04971)
*Gyounghun Ko,Sung-Jun Son,Seung Yeon Cho,Myeong-Su Lee*

Main category: cs.LG

TL;DR: 针对BGK模型，标准L2损失的PINN方法无法准确预测宏观矩，作者提出速度加权L2损失来惩罚高速区域误差，理论证明能保证收敛，实验显示精度和鲁棒性更优。
...
Method: 提出速度加权L2损失函数，专门设计来有效惩罚高速区域的误差，并建立了该方法的稳定性估计。

Result: 理论证明最小化加权损失能保证近似解的收敛性，数值实验表明该方法在多个基准测试中相比标准方法具有更优的精度和鲁棒性。

Conclusion: 速度加权L2损失为PINN在BGK模型中的应用提供了更有效的框架，解决了标准方法在宏观矩预测方面的根本缺陷。
...
### [154] [Territory Paint Wars: Diagnosing and Mitigating Failure Modes in Competitive Multi-Agent PPO](https://arxiv.org/abs/2604.04983)
*Diyansha Singh*

Main category: cs.LG

TL;DR: 本文介绍了Territory Paint Wars这个竞争性多智能体强化学习环境，用于系统研究PPO算法在自博弈中的失败模式。研究发现PPO存在五个实现层面的失败模式，以及竞争性过拟合这一新兴病理现象，并提出对手混合策略来缓解该问题。


<details>
  <summary>Details</summary>
Motivation: 研究竞争性多智能体强化学习环境中PPO算法的失败模式，特别是自博弈设置下的性能问题。通过构建一个最小化的对称零和游戏环境，系统性地识别和分析算法实现中的缺陷及其对泛化能力的影响。

Method: 使用Unity实现的Territory Paint Wars环境进行实验，通过控制变量法识别五个实现层面的失败模式：奖励尺度不平衡、缺失终止信号、无效的长时程信用分配、未归一化的观测、错误的胜利检测。然后发现竞争性过拟合现象，并提出对手混合策略（20%训练回合使用固定随机策略替代自适应对手）来缓解该问题。

Result: 初始PPO训练84000回合后仅获得26.8%胜率。修正五个实现问题后，发现竞争性过拟合现象：自博弈性能稳定（胜率接近50%），但泛化胜率从73.5%崩溃至21.6%。使用对手混合策略后，泛化胜率恢复至77.1%（±12.6%，10个种子），无需基于种群的训练或额外基础设施。

Conclusion: 竞争性多智能体强化学习中存在实现层面的微妙失败模式和竞争性过拟合这一新兴病理现象。对手混合策略是一种简单有效的干预措施，能够显著改善泛化性能。开源Territory Paint Wars环境为研究竞争性MARL失败模式提供了可复现的基准。
...
### [155] [Enhancing sample efficiency in reinforcement-learning-based flow control: replacing the critic with an adaptive reduced-order model](https://arxiv.org/abs/2604.04986)
...
Method: 结合线性动力学系统和神经常微分方程构建降阶模型，通过算子推断和梯度优化训练，在控制器-环境交互中自适应更新模型，利用ROM进行可微模拟优化控制器
...
### [156] [Cactus: Accelerating Auto-Regressive Decoding with Constrained Acceptance Speculative Sampling](https://arxiv.org/abs/2604.04987)
...
<details>
  <summary>Details</summary>
Motivation: 传统推测采样严格匹配验证器分布过于限制，而典型接受采样虽然提高接受率但会扭曲验证器分布，可能损害输出质量。需要一种既能提高效率又能控制分布差异的方法。

Method: 从约束优化的角度形式化推测采样算法，提出Cactus（约束接受推测采样），该方法保证与验证器分布的可控差异，同时提高接受率。

Result: 在广泛的基准测试中，Cactus方法被证实是有效的，能够在提高接受率的同时控制分布差异。

Conclusion: Cactus通过约束优化框架解决了传统推测采样的限制，在保证输出质量的同时提高了解码效率，为大型语言模型加速提供了更灵活有效的方法。
...
### [157] [Learning-Based Multi-Criteria Decision Making Model for Sawmill Location Problems](https://arxiv.org/abs/2604.04996)
*Mahid Ahmed,Ali Dogru,Chaoyang Zhang,Chao Meng*

Main category: cs.LG

TL;DR: 提出LB-MCDM框架，结合机器学习与GIS空间分析，用于锯木厂选址决策，在密西西比州案例中识别出10-11%区域为高度适宜位置。


<details>
  <summary>Details</summary>
Motivation: 锯木厂战略选址对提高木材供应链效率、盈利能力和可持续性至关重要，需要数据驱动、无偏且可复现的评估方法。

Method: 提出学习型多准则决策框架，集成机器学习与GIS空间分析，使用五种ML算法（随机森林、支持向量机、XGBoost、逻辑回归、K近邻）评估适宜性，并采用SHAP技术解释特征重要性。

Result: 随机森林分类器表现最佳，SHAP分析显示供需比是最重要影响因素，其次是道路、铁路线和城市距离，验证表明密西西比州10-11%区域高度适宜锯木厂选址。

Conclusion: LB-MCDM框架为锯木厂选址提供了有效的数据驱动决策支持工具，能够识别关键影响因素并生成可靠的适宜性地图。
...
### [158] [El Nino Prediction Based on Weather Forecast and Geographical Time-series Data](https://arxiv.org/abs/2604.04998)
*Viet Trinh,Ha-Vy Luu,Quoc-Khiem Nguyen-Pham,Hung Tong,Thanh-Huyen Tran,Hoai-Nam Nguyen Dang*

Main category: cs.LG

TL;DR: 提出一个结合CNN和LSTM的混合深度学习框架，利用多源气象和地理数据提高厄尔尼诺事件的预测准确性和提前时间


<details>
  <summary>Details</summary>
Motivation: 传统厄尔尼诺预测模型主要依赖海洋和大气指数，缺乏对综合气象地理数据中动态相互作用的捕捉，难以准确预测厄尔尼诺事件对全球气候、经济和社会的重大影响

Method: 整合实时全球天气预报数据、异常数据、次表层海洋热含量和大气压力等多时空分辨率数据，采用CNN进行空间特征提取和LSTM进行时间依赖建模的混合深度学习架构

Result: 未在摘要中明确说明具体实验结果，但框架旨在识别厄尔尼诺事件的复杂前兆和演化模式

Conclusion: 该混合深度学习框架通过整合多源数据和时空建模能力，有望提高厄尔尼诺事件的预测精度和提前时间
...
### [159] [PRIME: Prototype-Driven Multimodal Pretraining for Cancer Prognosis with Missing Modalities](https://arxiv.org/abs/2604.04999)
...
TL;DR: PRIME提出了一种缺失感知的多模态自监督预训练框架，能从部分观测的临床队列中学习稳健且可迁移的表征，用于癌症预后预测。


<details>
  <summary>Details</summary>
Motivation: 现有多模态自监督预训练方法需要完整配对的多模态输入，但实际临床队列通常是碎片化的，经常缺失一个或多个模态，这限制了监督融合和可扩展的多模态预训练。

Method: PRIME将异质模态嵌入映射到统一的token空间，引入共享原型记忆库进行潜在空间语义插补，通过患者级共识检索生成结构对齐的token。采用两种互补的预训练目标：结构化缺失增强下的模态间对齐和融合后一致性。

Result: 在TCGA数据集上，PRIME在32种癌症类型上进行无标签预训练，在五个队列的下游5折评估中，在总体生存预测、3年死亡率分类和3年复发分类三个任务上均取得最佳宏观平均性能（C-index 0.653，AUROC 0.689和0.637）。

Conclusion: 缺失感知的多模态预训练是碎片化临床数据环境中预后建模的实用策略，PRIME在测试时缺失情况下表现更稳健，支持参数高效和标签高效的适应。
...
### [160] [Blind-Spot Mass: A Good-Turing Framework for Quantifying Deployment Coverage Risk in Machine Learning Systems](https://arxiv.org/abs/2604.05057)
*Biplab Pal,Santanu Bhattacharya,Madanjit Singh*

Main category: cs.LG

TL;DR: 盲点质量是一个基于Good-Turing框架的机器学习部署覆盖风险量化方法，用于估计在有限训练数据中结构性地未充分支持的罕见状态的总概率质量，揭示模型在标准测试集上表现良好但在部署状态空间大范围内不可靠的问题。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习系统的运行状态分布通常是重尾的，这意味着大量有效但罕见的状态在有限的训练和评估数据中结构性地未得到充分支持，导致"覆盖盲区"：模型在标准测试集上可能表现准确，但在部署状态空间的大范围内仍然不可靠。

Method: 提出盲点质量B_n(tau)，这是一个部署指标，使用Good-Turing未观测物种估计方法来估计经验支持低于阈值tau的状态的总概率质量。进一步推导了覆盖强加的准确率上限，将整体性能分解为支持部分和盲点部分，从而区分能力限制和数据限制。

Result: 在可穿戴人体活动识别(HAR)和MIMIC-IV医院数据库(275例入院)中验证了该框架。盲点质量曲线在不同临床状态抽象中收敛到相同的95%(tau=5时)。跨结构独立领域(模态、特征空间、标签空间和应用不同)的复制表明盲点质量是量化组合覆盖风险的通用ML方法，而非特定应用的人工产物。

Conclusion: 盲点分解识别哪些活动或临床状态主导风险，为工业从业者提供有针对性的数据收集、归一化/重归一化以及物理或领域知识约束的可操作指导，以实现更安全的部署。
...
### [161] [Dynamic Linear Coregionalization for Realistic Synthetic Multivariate Time Series](https://arxiv.org/abs/2604.05064)
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列合成数据生成器大多假设静态相关性，缺乏真实的通道间依赖关系，这限制了时间序列基础模型的训练效果和迁移能力。

Method: 提出DynLMC（动态线性协同区域化模型），该模型包含时变、机制切换的相关性和跨通道滞后结构，能够生成具有真实动态相关性的多元时间序列合成数据。

Result: 在DynLMC生成的合成数据上微调三个基础模型，在九个基准测试中均获得一致的零样本预测性能提升，证明了动态相关建模的有效性。

Conclusion: 建模动态通道间相关性能够增强时间序列基础模型的迁移能力，凸显了以数据为中心的预训练方法的重要性。
...
TL;DR: HiVG提出了一种针对SVG生成的分层tokenization框架，通过原子token和几何约束的segment token改进序列效率，结合分层均值-噪声初始化策略和课程训练，在文本到SVG和图像到SVG任务中表现出更好的生成保真度和空间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有SVG生成方法采用从自然语言处理继承的字节级tokenization，无法反映矢量图形的几何结构，导致数值坐标被分割破坏空间关系，引入严重的token冗余，经常导致坐标幻觉和低效的长序列生成。

Method: 提出HiVG分层SVG tokenization框架：1) 将原始SVG字符串分解为结构化原子token；2) 将可执行命令-参数组压缩为几何约束的segment token；3) 引入分层均值-噪声初始化策略注入数值排序信号和语义先验；4) 采用课程训练范式逐步增加程序复杂度。

Result: 在文本到SVG和图像到SVG任务上的广泛实验表明，与传统tokenization方案相比，HiVG在生成保真度、空间一致性和序列效率方面都有显著改进。

Conclusion: HiVG通过分层tokenization框架解决了SVG生成中的几何结构表示问题，提高了序列效率和生成质量，为自回归矢量图形生成提供了更有效的tokenization方案。
...
### [163] [Vintix II: Decision Pre-Trained Transformer is a Scalable In-Context Reinforcement Learner](https://arxiv.org/abs/2604.05112)
...
TL;DR: 将决策预训练Transformer扩展到多领域环境，使用流匹配训练，在数百个任务上获得更好泛化性能的智能体


<details>
  <summary>Details</summary>
Motivation: 现有的上下文强化学习方法存在局限性：算法蒸馏在多领域设置中泛化能力有限，而决策预训练Transformer虽然在简化领域表现出更强的上下文强化学习能力，但其可扩展性尚未得到验证

Method: 将决策预训练Transformer扩展到多样化的多领域环境，采用流匹配作为训练方法，保持其作为贝叶斯后验采样的解释性

Result: 获得了在数百个多样化任务上训练的智能体，在保留测试集上实现了明显的泛化提升，超越了先前的算法蒸馏扩展，在在线和离线推理中都表现出更强的性能
...
### [164] [Reasoning Through Chess: How Reasoning Evolves from Data Through Fine-Tuning and Reinforcement Learning](https://arxiv.org/abs/2604.05134)
*Lucas Dionisopoulos,Nicklas Majamaki,Prithviraj Ammanabrolu*

Main category: cs.LG

TL;DR: 研究如何通过监督微调和强化学习提升语言模型在象棋任务中的推理能力，发现直接预测最佳走法的微调能获得最强性能但产生不忠实推理，而多步轨迹训练则能保持忠实推理和稳定强化学习。


<details>
  <summary>Details</summary>
Motivation: 探索如何让语言模型在原本表现不佳的任务（如象棋）中提升推理能力，研究从监督微调到强化学习过程中推理能力的演变，以及不同训练策略对模型性能和推理忠实性的影响。

Method: 使用理论启发数据集分析语言模型在象棋任务中的表现，比较两种训练策略：1）直接微调预测最佳走法；2）训练多步轨迹。通过监督微调和强化学习阶段评估模型性能、推理忠实性和稳定性。

Result: 直接预测最佳走法的微调能获得最强下游性能和有效强化学习，但产生不忠实推理；多步轨迹训练获得相当性能但保持忠实推理和更稳定的强化学习。强化学习显著提升走法质量分布并降低幻觉率。多个监督微调检查点指标可预测强化学习后模型性能。

Conclusion: 训练策略对语言模型的推理忠实性和性能有重要影响，多步轨迹训练能在保持性能的同时确保推理忠实性。强化学习能提升走法质量和减少幻觉。监督微调阶段的多个指标可有效预测强化学习后模型性能，为优化语言模型推理能力提供了实用指导。
...
### [165] [Not All Turns Are Equally Hard: Adaptive Thinking Budgets For Efficient Multi-Turn Reasoning](https://arxiv.org/abs/2604.05164)
*Neharika Jali,Anupam Nayak,Gauri Joshi*

Main category: cs.LG

TL;DR: TAB：一种用于多轮推理的自适应预算分配策略，通过马尔可夫决策过程建模，在保持准确率的同时节省高达35-40%的推理令牌


<details>
  <summary>Details</summary>
Motivation: 随着LLM推理性能趋于平稳，提高推理时计算效率变得至关重要。现有方法主要关注单轮设置，未能解决多轮推理中的顺序依赖性问题，导致简单查询也会产生过长的思考轨迹。

Method: 将多轮推理建模为顺序计算分配问题，使用多目标马尔可夫决策过程。提出TAB（Turn-Adaptive Budgets）策略，通过Group Relative Policy Optimization训练，学习在尊重每个问题全局令牌约束的同时最大化任务准确率。对于已知所有子问题的系统，进一步提出TAB All-SubQ策略。

Result: 在数学推理基准测试中，TAB实现了优越的准确率-令牌权衡，节省高达35%的令牌同时保持准确率。TAB All-SubQ进一步节省高达40%的令牌。
...
### [166] [General Multimodal Protein Design Enables DNA-Encoding of Chemistry](https://arxiv.org/abs/2604.05181)
...
<details>
  <summary>Details</summary>
Motivation: 自然进化虽然产生了丰富的酶多样性，但所探索的化学空间仍然有限，仅占DNA编码能力的一小部分。现有的深度生成模型虽然能设计结合配体的蛋白质，但都需要预先指定催化残基，无法从头设计酶。

Method: DISCO是一种多模态扩散模型，采用序列-结构协同设计方法，能够在推理时通过跨模态优化目标，围绕任意生物分子共同设计蛋白质序列和3D结构。模型仅以反应中间体为条件进行设计。

Result: DISCO设计了具有新型活性位点几何结构的血红素酶，成功催化了多种自然界不存在的新反应，包括烯烃环丙烷化、螺环丙烷化、B-H插入和C(sp³)-H插入反应，活性超过了工程化酶。对选定设计的随机诱变进一步证实可以通过定向进化提高酶活性。

Conclusion: DISCO为可进化酶的设计提供了可扩展的途径，拓宽了基因可编码转化的潜在范围，使蛋白质设计能够探索超越自然进化的化学空间。
...
### [167] [Cross-fitted Proximal Learning for Model-Based Reinforcement Learning](https://arxiv.org/abs/2604.05185)
*Nishanth Venkatesh,Andreas A. Malikopoulos*

Main category: cs.LG

TL;DR: 该论文提出了一种用于部分可观测马尔可夫决策过程（POMDP）中桥函数统计估计的交叉拟合方法，以解决离线强化学习中的隐藏混杂偏倚问题。


<details>
  <summary>Details</summary>
Motivation: 在存在隐藏混杂的离线强化学习设置中，直接从观测数据学习的模型可能存在偏倚。特别是在部分可观测系统中，潜在因素可能同时影响动作、奖励和未来观测，这使得政策评估变得困难。

Method: 将桥学习建模为条件矩限制（CMR）问题，其中干扰对象由条件均值嵌入和条件密度给出。开发了现有两阶段桥估计器的K折交叉拟合扩展，保留了原始的基于桥的识别策略，同时比单一样本分割更有效地利用数据。

Result: 提出了交叉拟合估计器，并推导了其与oracle比较器的误差界限。将总误差分解为第一阶段（由干扰估计引起）和第二阶段（由经验平均引起）的误差项。

Conclusion: 该方法在保持桥函数识别策略的同时，通过交叉拟合更有效地利用数据，为存在隐藏混杂的POMDP中的离线强化学习提供了统计上更高效的桥函数估计方法。
...
TL;DR: 该论文建立了位置编码的数学理论，证明了位置信号的必要性，提出了基于多维缩放的最优编码构造方法，并通过实验验证了理论预测。


<details>
  <summary>Details</summary>
Motivation: 位置编码在Transformer模型中至关重要，但现有设计缺乏数学理论基础，主要依靠试错。论文旨在建立位置编码的数学理论框架，为设计更优的位置编码提供指导。

Method: 1. 理论证明：建立必要性定理、位置分离定理等四个核心结果；2. 最优编码构造：使用经典多维缩放（MDS）在位置分布的Hellinger距离上构建信息最优编码；3. 实验验证：在SST-2和IMDB数据集上使用BERT-base验证理论预测。

Result: 1. 证明了无位置信号的Transformer无法解决任何对词序敏感的任务；2. 训练会在全局最小化器处为不同序列位置分配不同的向量表示；3. 提出了基于MDS的最优编码构造方法，编码质量由单一数值"应力"衡量；4. 最优编码的有效秩r≤n-1，参数可从nd减少到r(n+d)；5. 实验显示ALiBi编码比正弦编码和RoPE具有更低的应力。

Conclusion: 该论文建立了位置编码的完整数学理论框架，为理解位置编码的作用机制和设计更优编码提供了理论基础。理论预测与实验结果一致，ALiBi编码在实践中表现出色，符合在近似平移等变性下MDS编码的秩-1解释。
...
TL;DR: 该论文提出了TabDDPM的时间序列扩展版本，通过引入轻量级时间适配器和上下文感知嵌入模块，使扩散模型能够生成具有时间一致性的合成传感器数据序列。


<details>
  <summary>Details</summary>
Motivation: TabDDPM虽然能生成高质量的异构表格数据，但假设样本间独立，无法处理时间序列中关键的时间依赖性。需要扩展扩散模型以生成具有时间连贯性的序列数据。

Method: 1. 将传感器数据重构为窗口化序列；2. 引入轻量级时间适配器；3. 通过时间步嵌入、条件活动标签和观测/缺失掩码显式建模时间上下文；4. 使用上下文感知嵌入模块。

Result: 在WISDM加速度计数据集上，生成的时间序列与真实传感器模式高度相似，分类性能相当（宏F1分数0.64，准确率0.71）。通过二元转移矩阵和自相关分析验证，显示出增强的时间真实性、多样性和连贯性。

Conclusion: 扩散模型通过配备时间推理能力，为序列数据合成提供了有效且适应性强的解决方案。该方法特别有利于少数类表示和保持与真实分布的统计对齐。未来工作将探索扩展到更长序列和集成更强的时间架构。
...
TL;DR: 提出Jeffreys Flow生成框架，通过Jeffreys散度蒸馏并行回火轨迹数据，解决Boltzmann生成器中反向KL散度导致的模态崩溃问题，在多模态分布中实现更好的模态覆盖。


<details>
  <summary>Details</summary>
Motivation: 物理系统采样面临稀有事件和亚稳态陷阱问题。现有Boltzmann生成器使用反向Kullback-Leibler散度容易导致灾难性模态崩溃，在多模态分布中会遗漏特定模态。

Method: 提出Jeffreys Flow生成框架，使用对称的Jeffreys散度从并行回火轨迹中蒸馏经验采样数据。该方法平衡了局部目标搜索精度和全局模态覆盖，通过蒸馏经验参考数据来纠正固有误差。

Result: 最小化Jeffreys散度能有效抑制模态崩溃，并在高非凸多维基准测试中展示了可扩展性和准确性。成功应用于纠正副本交换随机梯度Langevin动力学中的随机梯度偏差，以及大幅加速量子热态路径积分蒙特卡洛中的精确重要性采样。

Conclusion: Jeffreys Flow通过Jeffreys散度蒸馏并行回火数据，为多模态分布采样提供了稳健的生成框架，解决了现有方法中的模态崩溃问题，在复杂物理系统采样中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数方法将不确定性视为生成后需要估计的潜在量，而不是模型训练中需要表达的信号。本文研究将不确定性作为控制接口，以改善LLM在需要不确定性驱动决策的场景（如弃权、检索和验证）中的表现。

Method: 比较两种互补的不确定性接口：1）全局接口：模型对其最终答案表达经过校准的置信度分数；2）局部接口：模型在推理过程中进入高风险状态时发出明确的<uncertain>标记。分析这两种接口的内部工作机制和效果差异。

Result: 口头表达置信度显著改善校准，减少过度自信的错误，并产生最强的自适应RAG控制器，同时更选择性地使用检索。推理时不确定性信号使先前沉默的失败在生成过程中可见，提高错误答案覆盖率，并提供有效的高召回检索触发器。两种接口内部工作机制不同：口头置信度主要细化现有不确定性的解码方式，而推理时信号诱导更广泛的后期层重组。

Conclusion: 有效的LLM不确定性应被训练为任务匹配的通信方式：全局置信度用于决定是否信任最终答案，局部信号用于决定何时需要干预。这两种接口提供不同但互补的优势，共同改善LLM在不确定性驱动决策场景中的表现。
...
TL;DR: 论文提出了一个评估生成模型的理论框架，证明了常用指标的评估可行性。研究发现基于测试的指标（如IPMs）可以从有限样本中评估，而Rényi和KL散度则无法从有限样本中可靠评估。


<details>
  <summary>Details</summary>
Motivation: 生成模型的评估比监督学习更具挑战性，因为生成模型具有开放性特征：不清楚哪些指标合适，以及这些指标是否能从有限样本中可靠评估。需要建立一个理论框架来解决生成模型评估的可行性问题。

Method: 引入了一个理论框架来评估生成模型，研究了两类指标：基于测试的指标（包括积分概率度量IPMs）和Rényi散度。分析了IPMs在有界测试类下的评估可行性，以及当测试类具有有限fat-shattering维度时的精确评估能力。

Result: 对于任何有界测试类，IPMs可以从有限样本中以乘性和加性近似误差进行评估。当测试类具有有限fat-shattering维度时，IPMs可以以任意精度评估。相反，Rényi和KL散度无法从有限样本中评估，因为它们的值可能由罕见事件决定。还分析了困惑度作为评估方法的潜力和局限性。

Conclusion: 该研究为生成模型评估提供了理论基础，明确了哪些指标可以从有限样本中可靠评估。IPMs类指标具有评估可行性，而基于散度的指标则存在根本性限制，这为生成模型评估的实践提供了重要指导。
...
<details>
  <summary>Details</summary>
Motivation: 在制造业中，实现弹性和高质量制造需要可靠的数据驱动异常检测方法。现有方法难以处理执行相同流程的不同个体机器之间的行为差异，这限制了异常检测系统在实际工业环境中的泛化能力。
...
Result: 在从三台执行相同操作的不同机器收集的工业数据集上进行实验，结果表明所提方法优于基于原始信号和MOMENT嵌入特征的基线方法，证实了其在增强跨机器泛化方面的有效性。

Conclusion: 该框架通过提取域不变特征成功解决了跨机器异常检测中的泛化问题，为制造业中可靠的数据驱动异常检测提供了有效解决方案，能够处理不同个体机器间的行为差异。
...
TL;DR: 该论文分析了SVD正交化在旋转矩阵估计训练中的梯度问题，证明了SVD反向传播雅可比矩阵的精确谱结构，解释了为什么在训练中移除SVD正交化、仅在推理时应用SVD投影能获得更好效果。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，在训练中移除正交化、仅在推理时应用SVD投影能改善深度学习中的旋转估计，但缺乏理论解释：为什么SVD正交化会损害训练？为什么SVD在推理时优于Gram-Schmidt？

Method: 对3×3矩阵和SO(3)投影的SVD正交化进行详细的梯度分析，推导SVD反向传播雅可比矩阵的精确谱结构，分析Gram-Schmidt雅可比矩阵的非对称谱特性。

Result: SVD反向传播雅可比矩阵的秩为3（匹配SO(3)维度），非零奇异值为2/(s_i + s_j)，条件数κ = (s_1 + s_2)/(s_2 + s_3)。当预测矩阵远离SO(3)时（如训练早期s_3≈0），梯度失真最严重。6D Gram-Schmidt雅可比矩阵具有非对称谱，导致参数接收不均衡的梯度信号。

Conclusion: SVD正交化在训练中引入可量化的梯度失真和方向误差，而Gram-Schmidt存在梯度信号不均衡问题。这为训练时使用直接9D回归、仅在推理时应用SVD投影提供了理论基础。
...
### [175] [Top-K Retrieval with Fixed-Size Linear-Attention Completion: Backbone- and KV-Format-Preserving Attention for KV-Cache Read Reduction](https://arxiv.org/abs/2604.05438)
*Yasuto Hoshi,Daisuke Miyashita,Jun Deguchi*

Main category: cs.LG

TL;DR: 提出了一种检索-完成注意力模块，在保持主干权重和KV缓存格式不变的情况下，通过精确计算锚点/检索令牌的注意力并估计中间区域，减少长上下文生成的KV缓存流量。


<details>
  <summary>Details</summary>
Motivation: 长上下文生成受到解码时KV缓存流量的限制，特别是在KV被卸载到GPU内存之外时。现有的查询感知检索方法（如Top-K选择）虽然减少了流量，但在重新归一化softmax时会因未检索令牌的注意力质量分布而产生偏差。

Method: 提出检索-完成注意力模块：1）为每个查询精确计算sink/tail锚点和查询依赖的Top-K检索令牌的注意力；2）使用预填充时计算的固定大小特征图摘要估计剩余中间区域的分子和分母；3）在未归一化域中添加精确和估计的贡献，应用单一归一化，恢复缺失的softmax质量而无需额外的注意力侧KV读取。

Result: 在长上下文基准测试中，该方法在匹配的令牌等效读取预算下优于仅使用Top-K选择的方法，在高熵注意力头中获得了最大的改进。

Conclusion: 提出的检索-完成注意力模块能够有效减少长上下文生成的KV缓存流量，同时避免传统检索方法中的softmax偏差问题，特别是在高熵注意力头中表现优异。
...
### [176] [Channel-wise Retrieval for Multivariate Time Series Forecasting](https://arxiv.org/abs/2604.05543)
*Junhyeok Kang,Jun Seo,Soyeon Park,Sangjun Han,Seohui Bae,Hyeokjun Choe,Soonyoung Lee*

Main category: cs.LG

TL;DR: CRAFT提出了一种通道独立的检索增强时间序列预测框架，通过两阶段流程（时域稀疏关系图剪枝+频域谱相似性排序）解决现有方法忽略通道异质性的问题，在多个基准测试中取得了优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强预测方法采用通道无关策略，对所有变量使用相同的历史参考片段，忽略了不同通道具有不同周期性和频谱特性的异质性，这限制了模型捕捉长期依赖关系的能力。

Method: CRAFT采用通道独立的检索策略，为每个通道单独检索历史片段。使用两阶段流程：1）在时域构建稀疏关系图以剪枝不相关候选；2）在频域通过谱相似性对参考片段进行排序，强调主导周期成分并抑制噪声。

Result: 在7个公开基准测试中，CRAFT超越了最先进的预测基线方法，实现了更优越的预测精度，同时保持了实用的推理效率。

Conclusion: 通过考虑通道异质性并采用通道独立的检索策略，CRAFT能够更有效地捕捉多变量时间序列中的长期依赖关系，为检索增强预测提供了新的有效框架。
...
### [177] [Same Graph, Different Likelihoods: Calibration of Autoregressive Graph Generators via Permutation-Equivalent Encodings](https://arxiv.org/abs/2604.05613)
*Laurits Fredsgaard,Aaron Thomas,Michael Riis Andersen,Mikkel N. Schmidt,Mahito Sugiyama*

Main category: cs.LG

TL;DR: 该研究提出线性化不确定性(LU)指标，用于评估自回归图生成器在不同图线性化策略下的似然一致性，发现传统负对数似然(NLL)评估存在偏差，而LU能更可靠地评估生成分子质量。


<details>
  <summary>Details</summary>
Motivation: 自回归图生成器通过序列化过程定义似然，但这些似然只有在同一图的不同线性化之间保持一致时才有意义。现有线性化方法如SENT允许同一图有多个等价线性化，需要评估不同线性化策略下模型似然的一致性。

Method: 使用四种线性化策略训练transformer模型，提出线性化不确定性(LU)指标——通过等价线性化间负对数似然(NLL)的变异系数来量化似然一致性。在QM9分子图基准上，比较NLL和LU与分子稳定性的相关性。

Result: 有偏排序策略在其原生顺序上获得较低NLL，但在随机排列下预期校准误差(ECE)高两个数量级，表明模型学习了训练线性化而非底层图结构。在QM9上，生成图的NLL与分子稳定性负相关(AUC=0.43)，而LU达到AUC=0.85，表明基于排列的评估能更可靠地检查生成分子质量。

Conclusion: 传统NLL评估存在线性化偏差，线性化不确定性(LU)提供了更可靠的图生成质量评估指标。模型可能过度拟合特定线性化策略而非学习底层图分布，需要开发更稳健的评估方法。
...
### [178] [Optimal-Transport-Guided Functional Flow Matching for Turbulent Field Generation in Hilbert Space](https://arxiv.org/abs/2604.05700)
*Li Kunpeng,Wan Chenguang,Qu Zhisong,Lim Kyungtak,Virginie Grandgirard,Xavier Garbet,Yu Hua,Ong Yew Soon*

Main category: cs.LG

TL;DR: 提出了一种在无限维函数空间中直接定义的生成框架FOT-CFM，用于高保真湍流建模，相比传统基于像素的生成模型具有分辨率不变性优势。


<details>
  <summary>Details</summary>
Motivation: 传统基于知识的湍流建模系统难以捕捉复杂的时空动力学和多尺度间歇性。现有的深度生成模型（如扩散模型和流匹配）受限于其离散、基于像素的本质，无法直接处理函数形式的数据，限制了在湍流计算中的应用。

Method: 提出了功能最优传输条件流匹配（FOT-CFM）框架，直接在无限维函数空间中定义生成过程。该方法将物理场视为无限维希尔伯特空间的元素，在概率测度层面学习分辨率不变的生成动力学。通过整合最优传输理论，在希尔伯特空间中构建噪声和数据测度之间的确定性直线概率路径，实现无模拟训练并加速采样过程。

Result: 在多种混沌动力系统（包括Navier-Stokes方程、Kolmogorov流和Hasegawa-Wakatani方程）上进行了严格评估，这些系统都表现出丰富的多尺度湍流结构。实验结果表明，FOT-CFM在重现高阶湍流统计量和能量谱方面，相比最先进的基线方法具有更优的保真度。

Conclusion: FOT-CFM为湍流建模提供了一种在函数空间中直接定义的生成框架，克服了传统基于像素方法的局限性，能够更好地捕捉湍流的多尺度特性，在湍流计算中具有重要应用价值。
...
### [179] [Controllable Image Generation with Composed Parallel Token Prediction](https://arxiv.org/abs/2604.05730)
*Jamie Stirling,Noura Al-Moubayed,Chris G. Willcocks,Hubert P. H. Shum*

Main category: cs.LG

TL;DR: 本文提出了一种理论上有依据的离散概率生成过程组合方法，特别针对掩码生成（吸收扩散），能够精确指定训练数据之外的新条件组合，通过概念加权实现对单个条件的强调或否定。


<details>
  <summary>Details</summary>
Motivation: 现有的条件离散生成模型难以忠实地组合多个输入条件。为了解决这个问题，需要一种理论上有依据的方法来组合离散概率生成过程，特别是在掩码生成（吸收扩散）这一特殊情况下。

Method: 推导了一个理论上有依据的离散概率生成过程组合公式，特别针对掩码生成（吸收扩散）。该方法能够精确指定训练数据之外的新条件组合和数量，通过概念加权实现对单个条件的强调或否定。与VQ-VAE和VQ-GAN的丰富组合学习词汇表协同工作。

Result: 在3个数据集（位置CLEVR、关系CLEVR和FFHQ）上平均实现了63.4%的相对错误率降低，同时平均绝对FID改进为-9.58。相比可比方法获得了2.3倍到12倍的实时加速，并且能够直接应用于开源的预训练离散文本到图像模型，实现对文本到图像生成的细粒度控制。

Conclusion: 该方法在理论上为离散概率生成过程的组合提供了坚实基础，在多个数据集上显著提升了性能，同时大幅提高了生成速度，并展示了在现有预训练模型上的实际应用潜力。
...
### [180] [Graph Topology Information Enhanced Heterogeneous Graph Representation Learning](https://arxiv.org/abs/2604.05732)
*He Zhao,Zhiwei Zeng,Yongwei Wang,Chunyan Miao*

Main category: cs.LG

TL;DR: ToGRL是一个用于异质图的图拓扑学习增强表示学习框架，通过提取任务相关的拓扑信息构建新图结构，并利用提示调优提升下游任务适应性，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界的异质图通常存在噪声且结构非最优，这会影响图表示学习模型在下游任务中的性能。现有的图结构学习方法主要针对同质图设计，而异质图的图结构学习面临两个挑战：1) 输入图结构质量对异质图模型影响更大；2) 现有方法在异质图上存在内存消耗问题。

Method: 提出ToGRL框架，包含两个模块：1) 新颖的图结构学习模块，从原始图中提取下游任务相关的拓扑信息并投影为拓扑嵌入，利用这些嵌入构建具有平滑图信号的新图；2) 表示学习模块，以新图为输入学习下游任务的嵌入。采用两阶段方法将邻接矩阵优化与节点表示学习分离以减少内存消耗，并利用提示调优更好地利用学习到的表示知识。

Result: 在五个真实世界数据集上的广泛实验表明，ToGRL大幅优于现有的最先进方法。

Conclusion: ToGRL通过提取任务相关的拓扑信息构建高质量图结构，并利用提示调优增强下游任务适应性，有效解决了异质图结构学习中的挑战，显著提升了异质图表示学习的性能。
...
### [181] [Bivariate Causal Discovery Using Rate-Distortion MDL: An Information Dimension Approach](https://arxiv.org/abs/2604.05829)
*Tiago Brogueira,Mário A. T. Figueiredo*

Main category: cs.LG

TL;DR: 本文提出了一种基于率失真理论的新双变量因果发现方法RDMDL，通过改进对原因变量描述长度的估计，解决了现有MDL方法在此方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前基于最小描述长度(MDL)原理的双变量因果发现方法在估计原因变量的描述长度方面存在缺陷，主要依赖因果机制的描述长度来做决策。作者认为需要更准确地估计原因变量的描述长度，以提高因果发现性能。

Method: 基于率失真理论，提出了一种新的方法来测量原因变量的描述长度，对应达到代表底层分布的失真水平所需的最小速率。失真水平通过基于直方图的密度估计规则推导，速率使用信息维度的相关概念基于渐近近似计算。将此方法与传统的因果机制描述长度方法结合，提出了新的双变量因果发现方法RDMDL。

Result: 实验表明，RDMDL在Tübingen数据集上取得了具有竞争力的性能。

Conclusion: 通过率失真理论改进对原因变量描述长度的估计，能够提高基于MDL的因果发现方法的性能，RDMDL方法在标准数据集上表现良好。
...
### [182] [Hidden in the Multiplicative Interaction: Uncovering Fragility in Multimodal Contrastive Learning](https://arxiv.org/abs/2604.05834)
*Tillmann Rheude,Stefan Hegselmann,Roland Eils,Benjamin Wild*

Main category: cs.LG

TL;DR: 论文提出Gated Symile方法，通过门控机制解决多模态对比学习中模态不可靠、不对齐或缺失的问题，提升三模态场景下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态对比学习方法如Symile虽然能捕捉高阶跨模态依赖，但将所有模态对称处理，未考虑模态间的可靠性差异。在实际应用中，图像-文本之外的模态可能存在不对齐、信息弱或缺失的情况，统一处理会降低性能，而乘法交互可能掩盖单个不可靠模态的负面影响。

Method: 提出Gated Symile方法，引入基于注意力的对比门控机制，能够自适应调整每个候选样本的模态贡献。门控通过将嵌入向量向可学习的中性方向插值来抑制不可靠输入，并在跨模态对齐可能性低时提供显式的NULL选项。

Result: 在受控合成基准测试和三个真实世界三模态数据集上，Gated Symile在top-1检索准确率上优于经过良好调优的Symile和CLIP模型。实验揭示了Symile在存在不可靠模态时的脆弱性，而门控机制能有效提升鲁棒性。

Conclusion: 门控机制是迈向在非完美和多于两个模态场景下实现鲁棒多模态对比学习的重要一步。Gated Symile通过自适应调整模态贡献，解决了现有方法在处理不可靠模态时的局限性，为多模态学习提供了更稳健的解决方案。
...
### [183] [Expectation Maximization (EM) Converges for General Agnostic Mixtures](https://arxiv.org/abs/2604.05842)
*Avishek Ghosh*

Main category: cs.LG

TL;DR: 论文研究了在非生成式设置下拟合k个参数函数的问题，提出并分析了梯度EM算法，证明在适当初始化和分离条件下，该算法能以指数速度收敛到总体损失最小化器。


<details>
  <summary>Details</summary>
Motivation: 混合线性回归在统计学和机器学习中已有深入研究，但传统方法通常假设数据由生成模型产生。最近的研究开始关注非生成式设置，即不对数据做生成模型假设，而是直接拟合k条线。本文希望将这一框架扩展到更广泛的参数函数拟合问题，包括混合线性回归（正则化）、混合线性分类器（混合逻辑回归、混合支持向量机）和混合广义线性回归等。

Method: 提出并分析梯度EM算法用于拟合k个参数函数的问题。该方法不假设生成模型，而是使用强凸且平滑的损失函数。算法需要适当的初始化和分离条件，在满足这些条件下，梯度EM的迭代能以指数速度收敛到适当定义的总体损失最小化器。

Result: 证明在非生成式设置下，梯度EM算法能以指数速度收敛到最优解，且具有高概率。这表明EM类型算法在超越混合线性回归的更广泛问题中仍然有效，能够收敛到最优解。

Conclusion: 梯度EM算法在非生成式设置下对广泛的参数函数拟合问题有效，包括混合线性回归、混合线性分类器和混合广义线性回归等。在适当初始化和分离条件下，该算法能指数收敛到总体损失最小化器，扩展了EM类型算法在非生成式设置下的适用范围。
...
### [184] [EEG-MFTNet: An Enhanced EEGNet Architecture with Multi-Scale Temporal Convolutions and Transformer Fusion for Cross-Session Motor Imagery Decoding](https://arxiv.org/abs/2604.05843)
...
<details>
  <summary>Details</summary>
Motivation: 脑机接口为运动障碍患者提供重要支持，但脑电信号中的噪声和跨会话变异性使得运动想象解码仍然具有挑战性，需要更强大的解码模型。

Method: 提出EEG-MFTNet模型，基于EEGNet架构，增加了多尺度时间卷积和Transformer编码器流，旨在捕捉脑电信号的短期和长期时间依赖性。

Result: 在SHU数据集上使用受试者依赖的跨会话设置进行评估，EEG-MFTNet以58.9%的平均分类准确率优于EEGNet及其近期衍生模型，同时保持低计算复杂度和推理延迟。

Conclusion: 该模型在实时脑机接口应用中具有潜力，强调了架构创新在改善运动想象解码中的重要性，有助于开发更鲁棒和自适应的脑机接口系统。
...
### [185] [Modeling Patient Care Trajectories with Transformer Hawkes Processes](https://arxiv.org/abs/2604.05844)
*Saumya Pandey,Varun Chandola*

Main category: cs.LG

TL;DR: 基于Transformer Hawkes Process框架建模患者医疗轨迹，通过不平衡感知训练策略解决类别不平衡问题，提升罕见但重要临床事件的预测能力。


<details>
  <summary>Details</summary>
Motivation: 患者医疗轨迹由不规则时间戳事件组成，建模这些轨迹对于理解利用模式和预测未来需求至关重要，但面临时间不规则性和严重类别不平衡的挑战。

Method: 结合Transformer历史编码与Hawkes过程动态，构建连续时间患者轨迹模型；引入基于逆平方根类别权重的不平衡感知训练策略，在不改变数据分布的情况下提升对罕见事件的敏感性。

Result: 在真实世界数据上的实验显示性能提升，并为识别高风险患者群体提供具有临床意义的见解。

Conclusion: 该方法能有效建模患者医疗轨迹，解决类别不平衡问题，对临床决策支持具有实际价值。
...
### [186] [Weight-Informed Self-Explaining Clustering for Mixed-Type Tabular Data](https://arxiv.org/abs/2604.05857)
*Lehao Li,Qiang Huang,Yihao Ang,Bryan Kian Hsiang Low,Anthony K. H. Tung,Xiaokui Xiao*

Main category: cs.LG

TL;DR: WISE框架通过统一表示、特征加权、聚类和解释，解决了混合类型表格数据聚类中的表示不对齐、特征相关性不均和解释与聚类过程脱节等问题。


<details>
  <summary>Details</summary>
Motivation: 混合类型表格数据的聚类分析面临三大挑战：数值和分类特征的表示不对齐、特征相关性不均且依赖上下文、聚类过程与解释脱节且事后解释。现有方法难以同时解决这些问题。

Method: WISE框架包含四个核心组件：1) BEP编码将异构特征对齐到统一稀疏空间；2) LOFO策略生成多个高质量、多样化的特征加权视图；3) 两阶段权重感知聚类聚合不同语义分区；4) DFI方法提供特征级解释，保证从实例到聚类的一致性。

Result: 在六个真实世界数据集上的实验表明，WISE在聚类质量上持续优于经典和神经基线方法，同时保持高效性，并能产生忠实、人类可理解的解释，这些解释与驱动聚类的原始特征一致。

Conclusion: WISE框架成功地将表示学习、特征加权、聚类和解释统一在一个完全无监督且透明的流程中，解决了混合类型表格数据聚类的核心挑战，实现了高质量聚类与内在可解释性的统一。
...
### [187] [A Mixture of Experts Foundation Model for Scanning Electron Microscopy Image Analysis](https://arxiv.org/abs/2604.05960)
...
TL;DR: 该研究提出了首个SEM图像基础模型，通过自监督Transformer架构在大规模多仪器、多条件科学显微图像上预训练，实现了跨材料系统和成像条件的泛化能力，特别在去焦到聚焦图像转换任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: SEM在材料科学中至关重要，但现有方法受限于任务特定模型和劳动密集的数据采集过程，限制了其在不同应用中的可扩展性。需要一种能够泛化到多种材料和成像条件的通用模型。

Method: 采用自监督Transformer架构，在大规模多仪器、多条件的科学显微图像语料库上进行预训练，学习丰富且可迁移的表征。模型可以微调或适应各种下游任务，特别展示了在去焦到聚焦图像转换任务上的应用。

Result: 该模型不仅能够从去焦输入中恢复聚焦细节而无需配对监督，而且在多个评估指标上超越了最先进的技术。为SEM图像分析提供了强大的基础模型。

Conclusion: 这项工作为新型可适应SEM模型奠定了基础，通过将基础表征学习与实际成像需求相结合，加速了材料发现过程，推动了SEM图像分析的标准化和自动化。
...
### [188] [Data Distribution Valuation Using Generalized Bayesian Inference](https://arxiv.org/abs/2604.05993)
*Cuong N. Nguyen,Cuong V. Nguyen*

Main category: cs.LG

TL;DR: 提出广义贝叶斯估值框架，通过迁移性度量构建损失函数，统一解决数据分布估值问题，并扩展到连续数据流场景。


<details>
  <summary>Details</summary>
Motivation: 研究数据分布估值问题，这是一个新提出的问题，与传统数据估值相关但不同，可应用于多种实际场景，如标注者评估和数据增强。

Method: 开发广义贝叶斯估值框架，利用广义贝叶斯推断和基于迁移性度量的损失函数，以统一方式解决相关问题，并扩展到连续数据流设置。

Result: 实验结果表明，该框架在不同现实场景中具有有效性和高效性。

Conclusion: 提出的广义贝叶斯估值框架能够统一解决数据分布估值问题，并通过贝叶斯原理扩展到连续数据流场景，在实际应用中表现出色。
...
### [189] [Gated-SwinRMT: Unifying Swin Windowed Attention with Retentive Manhattan Decay via Input-Dependent Gating](https://arxiv.org/abs/2604.06014)
...
<details>
  <summary>Details</summary>
Motivation: 为了改进视觉Transformer的性能，结合Swin Transformer的移位窗口注意力和Retentive Networks的空间衰减机制，并引入输入依赖的门控来缓解低秩瓶颈问题，提升模型表达能力。

Method: 将自注意力分解为每个移位窗口内的宽度方向和高度方向保留传递，使用每头指数衰减掩码提供二维局部先验。提出两个变体：SWAT使用sigmoid激活和平衡ALiBi斜率，通过SwiGLU门控值投影；Retention保留softmax归一化保留，加入显式G1 sigmoid门控。

Result: 在Mini-ImageNet上，Gated-SwinRMT-SWAT达到80.22%准确率，Gated-SwinRMT-Retention达到78.20%，均显著优于RMT基线的73.74%。在CIFAR-10上，由于小特征图导致自适应窗口机制失效，优势从+6.48pp压缩到+0.56pp。

Conclusion: Gated-SwinRMT通过结合移位窗口注意力、空间衰减和输入依赖门控，在中等规模图像分类任务上取得了显著性能提升，但小图像上的优势有限，表明自适应窗口机制在小特征图上存在局限性。
...
### [190] [PromptEvolver: Prompt Inversion through Evolutionary Optimization in Natural-Language Space](https://arxiv.org/abs/2604.06061)
*Asaf Buchnick,Aviv Shamsian,Aviv Navon,Ethan Fetaya*

Main category: cs.LG

TL;DR: PromptEvolver使用遗传算法优化提示词，通过视觉语言模型指导进化过程，在仅需图像输出的黑盒生成模型中实现高质量图像重建，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成虽然进展迅速，但生成复杂场景需要大量试错来寻找准确提示词。现有的提示词反演方法通常产生次优重建结果，生成不自然、难以解释的提示词，这阻碍了透明度和可控性。

Method: 提出PromptEvolver方法，使用遗传算法优化提示词，利用强大的视觉语言模型指导进化过程。该方法适用于黑盒生成模型，仅需图像输出即可工作。

Result: 在多个提示词反演基准测试中评估PromptEvolver，结果显示它始终优于竞争方法，能够生成自然语言提示词并实现目标图像的高保真重建。

Conclusion: PromptEvolver是一种有效的提示词反演方法，能够生成自然语言提示词并实现高质量图像重建，在透明度和可控性方面具有优势。
...
### [191] [A machine learning framework for uncovering stochastic nonlinear dynamics from noisy data](https://arxiv.org/abs/2604.06081)
*Matteo Bosso,Giovanni Franzese,Kushal Swamy,Maarten Theulings,Alejandro M. Aragón,Farbod Alijani*

Main category: cs.LG

TL;DR: 提出了一种结合符号回归与概率机器学习的混合框架，能够同时从数据中恢复系统动力学方程的符号形式并推断参数不确定性，无需对函数形式做先验假设。


<details>
  <summary>Details</summary>
Motivation: 现实世界系统建模需要考虑噪声影响，传统符号回归方法忽略不确定性，而高斯过程虽能量化不确定性但无法揭示系统动力学。需要一种能同时恢复方程符号形式并推断参数不确定性的方法。

Method: 结合深度符号回归与基于高斯过程的最大似然估计的混合框架，分别建模确定性动力学和噪声结构，无需对函数形式做先验假设。

Result: 在数值基准测试（包括谐波、Duffing和van der Pol振荡器）上验证了方法有效性，并在耦合生物振荡器的实验系统上成功识别了符号和随机分量。方法数据高效（仅需100-1000个数据点）且对噪声鲁棒。

Conclusion: 该框架在不确定性固有的领域中具有广泛潜力，能够同时理解动力学系统的结构和变异性，为噪声影响下的系统参数和动力学推断提供了有效解决方案。
...
### [192] [Learning $\mathsf{AC}^0$ Under Graphical Models](https://arxiv.org/abs/2604.06109)
*Gautam Chandrasekaran,Jason Gaitonde,Ankur Moitra,Arsen Vasilyan*

Main category: cs.LG

TL;DR: 该论文提出了在具有多项式增长和强空间混合的图模型分布下，准多项式时间学习AC⁰电路的算法，突破了传统产品分布的限制。


<details>
  <summary>Details</summary>
Motivation: 传统学习理论中的许多结果依赖于产品分布假设，这在现实场景中往往不成立。如何在更自然的相关分布下获得类似的学习保证是该领域长期存在的挑战。

Method: 通过新的采样算法，将均匀分布下的低阶多项式逼近结果转移到图模型分布中，绕过了传统傅里叶分析的局限性。

Result: 成功开发了在具有多项式增长和强空间混合的图模型分布下，准多项式时间学习AC⁰电路的算法，方法可扩展到单调函数和半空间等其他函数类。

Conclusion: 该工作突破了传统产品分布的限制，为在更自然的分布下学习电路提供了新方法，扩展了低度算法的应用范围。
...
### [193] [Gym-Anything: Turn any Software into an Agent Environment](https://arxiv.org/abs/2604.06126)
*Pranjal Aggarwal,Graham Neubig,Sean Welleck*

Main category: cs.LG

TL;DR: Gym-Anything框架可将任何软件转换为交互式计算机使用环境，通过多智能体方法自动创建环境，并构建了包含10K+长时程任务的CUA-World基准，显著提升了计算机使用智能体的能力。


<details>
  <summary>Details</summary>
Motivation: 当前计算机使用智能体研究主要局限于短时程任务和有限软件，缺乏经济价值。主要原因是复杂软件环境创建需要大量人工，难以规模化。需要一种可扩展的方法来创建真实、经济价值高的计算机使用环境。

Method: 提出Gym-Anything框架，将环境创建本身作为多智能体任务：编码智能体编写设置脚本、下载真实数据、配置软件并生成正确设置证据；独立审计智能体根据质量检查清单验证环境设置。基于美国GDP数据中的职业分类，将该流程应用于200个软件应用。

Result: 构建了CUA-World，包含超过10K个长时程任务，涵盖医学科学、天文学、工程和企业系统等领域，每个任务都配置了真实数据及训练/测试划分。还包括CUA-World-Long基准，任务通常需要500+步骤。从训练轨迹中蒸馏出的2B视觉语言模型性能超过规模2倍大的模型。测试时应用审计原则，通过VLM审查完成轨迹并提供反馈，将Gemini-3-Flash在CUA-World-Long上的表现从11.5%提升到14.0%。

Conclusion: Gym-Anything框架能够规模化创建真实计算机使用环境，CUA-World基准为计算机使用智能体研究提供了丰富、具有经济价值的测试平台，多智能体审计方法在训练和测试阶段都能有效提升性能。
...
### [194] [Toward Consistent World Models with Multi-Token Prediction and Latent Semantic Enhancement](https://arxiv.org/abs/2604.06155)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型是否发展出连贯的内部世界模型仍是核心争论。虽然传统的下一令牌预测关注单步监督，但多令牌预测在学习结构化表示方面显示出潜力。然而，标准MTP常遭受结构幻觉问题，离散令牌监督鼓励潜在空间中的非法捷径，违反环境约束。

Method: 提出潜在语义增强多令牌预测（LSE-MTP），将预测锚定到真实隐藏状态轨迹。该方法通过梯度耦合诱导表示收缩性，促进向内部信念状态的收敛，同时避免结构幻觉。
...
### [195] [Target Policy Optimization](https://arxiv.org/abs/2604.06159)
*Jean Kaddour*

Main category: cs.LG

TL;DR: TPO是一种新的强化学习方法，将策略更新的两个问题分离：确定哪些完成应获得概率质量，以及如何调整参数实现这一变化。通过构建目标分布并最小化交叉熵损失，TPO避免了传统策略梯度方法的学习率、裁剪等超参数敏感问题。


<details>
  <summary>Details</summary>
Motivation: 传统策略梯度方法（如PG、PPO等）将"哪些完成应获得概率质量"和"如何调整参数实现变化"两个问题耦合在一起，导致更新可能因学习率、裁剪等优化器选择而过度或不足。需要一种更稳定、分离这两个问题的强化学习方法。

Method: TPO首先根据评分构建目标分布q_i ∝ p_i^old exp(u_i)，然后通过交叉熵损失将策略拟合到这个目标分布。损失梯度在采样完成的对数概率上是p^θ - q，当策略匹配目标时梯度消失。

Result: 在表格老虎机、Transformer序列任务和数十亿参数LLM RLVR任务上，TPO在简单任务上与PG、PPO、GRPO、DG表现相当，在稀疏奖励任务上显著优于这些方法。

Conclusion: TPO通过分离策略更新的两个核心问题，提供了一种更稳定、超参数鲁棒的强化学习方法，特别在稀疏奖励场景下表现出色。
...
### [196] [Topological Characterization of Churn Flow and Unsupervised Correction to the Wu Flow-Regime Map in Small-Diameter Vertical Pipes](https://arxiv.org/abs/2604.06167)
*Brady Koenig,Sushovan Majhi,Atish Mitra,Abigail Stein,Burt Todd*

Main category: cs.LG

TL;DR: 该研究首次提出了基于拓扑学的搅混流数学定义，使用欧拉特征曲面和多重核学习方法，无需标注数据即可准确识别流型，修正了现有模型对小管径中搅混流持续性的低估。


<details>
  <summary>Details</summary>
Motivation: 搅混流作为垂直两相流中的混沌振荡状态，40多年来一直缺乏定量数学定义。现有模型在小直径管道中低估了搅混流的持续性，需要基于拓扑学的客观定义和识别方法。

Method: 采用欧拉特征曲面进行拓扑表征，通过多重核学习框架融合三种特征：ECS时间对齐（χ(s,t)曲面的L1距离）、振幅统计（尺度均值、标准差、最大最小值）和气体速度。该自校准框架无需标注数据。

Result: 在蒙大拿理工37个未标注试验中，拓扑特征权重占64%。ECS推断的段塞/搅混转变比Wu等人预测高3.81 m/s。在德州农工大学947张图像验证中，搅混流拓扑复杂度是段塞流的1.9倍。在45个伪试验中实现95.6%的四类准确率和100%搅混流召回率。

Conclusion: 该研究首次提供了搅混流的数学定义，证明无监督拓扑描述符能够挑战和修正广泛采用的机理模型，为流型识别提供了无需标注数据的高效方法。
...
### [197] [In-Place Test-Time Training](https://arxiv.org/abs/2604.06169)
*Guhao Feng,Shengjie Luo,Kai Hua,Ge Zhang,Di He,Wenhao Huang,Tianle Cai*

Main category: cs.LG

TL;DR: 本文提出了In-Place Test-Time Training框架，使大语言模型能够在推理时动态调整权重，无需从头训练，通过优化MLP块的投影矩阵和设计面向下一个词预测的目标函数，实现了高效的持续学习。


<details>
  <summary>Details</summary>
Motivation: 传统的"训练后部署"范式限制了LLMs在现实任务中动态适应新信息流的能力。现有的测试时训练方法存在架构不兼容、计算效率低和目标函数不匹配等问题，阻碍了其在当前LLM生态系统中的应用。

Method: 提出In-Place TTT框架：1) 将MLP块的最终投影矩阵作为可适应的快速权重，实现"即插即用"增强；2) 设计理论基础的、与自回归语言建模的下一个词预测任务对齐的目标函数；3) 采用高效的块状更新机制，兼容上下文并行。

Result: 实验验证了框架的有效性：作为即插即用增强，使4B参数模型在128k上下文任务上获得优越性能；从头预训练时，持续优于竞争性TTT相关方法。消融研究进一步验证了设计选择。

Conclusion: In-Place TTT是朝着LLMs持续学习范式迈出的有希望的一步，通过测试时训练使模型能够动态适应新信息，解决了现有方法的局限性。
...
TL;DR: 这篇论文综述了利用无动作标注的视频数据学习机器人操作控制接口的方法，提出了基于接口构建位置和控制特性的分类法，并指出机器人集成层是当前最紧迫的挑战。


<details>
  <summary>Details</summary>
Motivation: 视频数据包含了丰富的物理动态信息，能够捕捉物体运动、接触过程和场景演化，且无需机器人动作标注。然而，将视频的时间结构转化为可靠的机器人控制仍然是一个开放挑战，因为视频缺乏动作监督，且在体现形式、视角和物理约束方面与机器人经验不同。

Method: 论文提出了一个以接口为中心的分类法，根据视频到控制接口的构建位置和控制特性，识别出三个主要方法家族：1）直接视频-动作策略（接口隐式）；2）潜在动作方法（通过紧凑学习中间层路由时间结构）；3）显式视觉接口（预测可解释目标供下游控制使用）。

Result: 通过对每个方法家族的分析，论文识别了控制集成特性（闭环方式、执行前可验证性、故障进入点）。跨家族的综合分析表明，最紧迫的开放挑战集中在机器人集成层——将视频衍生预测连接到可靠机器人行为的机制。

Conclusion: 论文指出机器人集成层是当前研究的核心挑战，并概述了缩小这一差距的研究方向，强调需要开发更好的机制来将视频预测转化为可靠的机器人行为。
...
Method: 开发了COMB平台，包括XY定位平台、可移动访问窗口（用于通过蜂箱边界进行密封工具访问）、可互换负载模块和嵌入式控制架构，支持可重复轨迹执行和信号生成。

Result: 通过三个代表性模块展示了平台能力：仿生舞蹈和信号负载、近距离蜂巢扫描仪、用于局部振荡刺激的电磁翼驱动器；通过跟踪摇摆轨迹执行、执行多图像拼接进行重复蜂巢马赛克、进行基于视频的翼驱动器频谱分析来表征平台性能。
...
TL;DR: 这篇综述研究了无人水下航行器基于传感器的规划与控制方法，重点关注在复杂水下环境中通过实时传感器输入进行自适应局部规划，以实现动态避障和实时重规划。


<details>
  <summary>Details</summary>
Motivation: 无人水下航行器在复杂、不确定的水下环境中面临诸多挑战：传感器测量噪声大、缺乏GNSS信号、水下声学通信带宽低延迟高。这些挑战要求开发先进的规划与控制策略，以提高在动态海洋条件下的定位精度和自主性。

Method: 综述将现有文献分为解耦和耦合两种架构：解耦架构顺序处理规划和控制阶段，而耦合架构提供更紧密的反馈回路以实现更快的响应。重点分析基于实时传感器输入（如声纳和惯性测量单元）的自适应局部规划层。

Result: 比较分析显示：PID控制器简单但缺乏复杂机动的预测能力；模型预测控制提供优越的路径优化但计算密集；不变集控制器提供强大的安全保障但可能在受限环境中牺牲灵活性。解耦架构适合结构化任务，耦合架构更适合动态环境。

Conclusion: 综述提出了规划与控制架构的分类法，强调了自适应局部规划的重要性，并分析了控制器在集成规划框架中的作用。未来研究应关注计算效率与性能的平衡，以及更智能的传感器融合策略，以提升无人水下航行器在复杂环境中的自主导航能力。
...
TL;DR: StarVLA是一个开源的视觉-语言-动作（VLA）研究代码库，提供模块化架构、可复用训练策略和统一评估接口，旨在解决现有VLA方法碎片化问题，降低研究门槛。


<details>
  <summary>Details</summary>
Motivation: 当前VLA方法在架构、代码库和评估协议上存在碎片化问题，缺乏统一的框架，这阻碍了方法的比较和复现。需要建立一个开放、模块化的平台来促进VLA研究的发展。

Method: StarVLA采用模块化的"骨干-动作头"架构，支持VLM骨干（如Qwen-VL）和世界模型骨干（如Cosmos）以及多种动作解码范式。提供跨具身学习和多模态协同训练等可复用训练策略，并通过统一接口集成多个主流基准测试。

Result: StarVLA提供了完全可复现的训练方案，在多个基准测试（LIBERO、SimplerEnv、RoboTwin~2.0等）上，即使使用最少的数据工程，也能匹配或超越现有方法。这是目前最全面的开源VLA框架之一。

Conclusion: StarVLA通过统一的模块化框架解决了VLA研究的碎片化问题，降低了现有方法的复现门槛和新方法的原型开发难度，将促进VLA领域的进一步发展。
...
TL;DR: GaussFly是一个用于自主飞行器的视觉运动策略学习框架，通过3D高斯溅射重建真实场景实现高保真实时模拟，结合对比表示学习提取抗噪声特征，显著提高样本效率和模拟到真实的迁移能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于单目视觉的端到端学习方法存在样本效率低和模拟到真实差距大的问题，需要一种能够有效解耦表示学习和策略优化的框架来克服这些挑战。

Method: 采用真实-模拟-真实范式，首先使用3D高斯溅射结合几何约束重建训练场景实现高保真实时模拟；然后通过对比表示学习从渲染的RGB图像中提取紧凑、抗噪声的潜在特征；最后使用预训练编码器为视觉运动策略提供低维特征输入。

Result: 在模拟和真实环境中的广泛实验表明，GaussFly相比基线方法具有更优的样本效率和渐进性能，能够实现零样本策略迁移到未见过的真实环境，有效弥合模拟到真实的差距。
...
TL;DR: 本文提出了一个用于肌腱驱动可转向导丝的模型预测控制框架，该导丝与标准近距离放射治疗针集成，通过几何双线性模型实现固定目标和移动目标的轨迹跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有的可转向针建模和控制方法主要针对定制针设计开发，无法直接应用于与商用临床针兼容的导丝。需要开发适用于标准近距离放射治疗针的可转向导丝控制方法，以实现避开敏感解剖结构的弯曲轨迹。
...
Result: 开环模型验证的估计误差低于2毫米（插入针长度的3%），闭环固定目标跟踪误差低至1.45毫米（插入长度的1.7%）。但在某些弯曲方向上实验显示出较大的位置误差，达到8.3毫米（插入长度的7.8%）。

Conclusion: 结果证明了临床兼容的可转向近距离放射治疗系统实现固定目标定位和移动目标轨迹跟踪的可行性，同时突出了未来在标定和传感方面需要改进的领域。
...
TL;DR: RoboPlayground：一个语言驱动的机器人操作评估框架，允许用户用自然语言定义可执行任务，解决传统固定基准测试的局限性。
...
Method: 提出RoboPlayground框架，用户通过自然语言在结构化物理域中编写可执行操作任务，语言指令被编译为可复现的任务规范，包含明确的资产定义、初始化分布和成功谓词。

Result: 用户研究表明语言驱动界面比编程基础和代码辅助基线更易用且认知负荷更低；评估学习策略在语言定义任务族上揭示了固定基准测试无法发现的泛化失败；任务多样性随贡献者多样性而非任务数量扩展。

Conclusion: RoboPlayground将评估重构为结构化物理域上的语言驱动过程，支持通过众包贡献持续扩展评估空间，能更全面地评估现代操作策略。
...
TL;DR: 提出一种基于3D打印塑料条的可折叠驱动器，通过复合折叠和拉链机制实现可逆的尺寸和刚度变换，应用于模块化自适应机器人系统。


<details>
  <summary>Details</summary>
Motivation: 当前形状变换系统通常采用特定设计的机制，难以扩展、重构或跨应用移植，需要开发更通用、易制造的自适应机器人驱动器。

Method: 使用柔性3D打印塑料条，通过复合折叠和拉链机制形成方形截面可展开梁，实现从紧凑（柔性）到展开（准刚性）状态的平滑连续转换。

Result: 驱动器机械性能得到表征，并成功集成到四模块自适应行走机器人系统中，展示了模块组合实现多样形状和刚度变换的能力。

Conclusion: 该紧凑易制造的可展开驱动器为自适应机器人提供了可扩展、可重构的形状和刚度变换解决方案，具有广泛的应用潜力。
...
### [206] [Semantic analysis of behavior in a DNA-functionalized molecular swarm](https://arxiv.org/abs/2604.05277)
*Tom Bachard,Gong Yiming,Ibuki Kawamata,Akira Kakugo,Nathanael Aubert-Kato*

Main category: cs.RO

TL;DR: 该研究将语义嵌入应用于分子群行为分析，通过DNA功能化增强微管-驱动蛋白系统的控制，提取的语义原子与预期行为匹配，为体外系统设计提供可解释性支持。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过语义嵌入学习分子群的行为范围，为优化此类系统提供更丰富的特征集。传统分子群系统缺乏足够的可解释性，难以可靠地设计和优化体外系统。

Method: 研究标准分子群系统（微管由表面粘附驱动蛋白驱动），并添加DNA功能化进行额外控制。扩展微管模型以包含这种额外相互作用，从模拟结果中提取语义原子进行分析。

Result: 从模拟结果中提取的语义原子与预期行为相匹配。模拟中每一帧的分解准确描述了外部控制值的预期影响。这些结果为模拟实验的可解释性提供了相关线索。

Conclusion: 该方法使模拟实验更具可解释性，使其在设计和优化体外系统时更加可靠。语义嵌入为分子群行为分析提供了有效的特征提取工具。
...
### [207] [Instantaneous Planning, Control and Safety for Navigation in Unknown Underwater Spaces](https://arxiv.org/abs/2604.05310)
*Veejay Karthik,Udit Ekansh,Tejal Bedmutha,Shivam Vishwakarma,Rohan Deshpande,Leena Vachhani*

Main category: cs.RO

TL;DR: 提出一种集成规划与控制框架，利用实时传感器数据动态引导AUV闭环轨迹，在未知水下环境中实现鲁棒避障和增强机动性


<details>
  <summary>Details</summary>
Motivation: 水下自主航行器在未知环境中导航面临重大挑战，包括能见度差、信号传输弱、动态水流等因素，导致全局定位不准确、通信不可靠、避障困难。水下传感器测量固有的噪声引入了不确定性，使规划和控制复杂化。

Method: 提出集成规划与控制框架，基于预先设计的反馈控制器规划运动，利用实时传感器数据动态引导闭环AUV轨迹。该方法减少在线优化的计算复杂度，增强复杂水下空间的操作安全性。

Result: 通过ROS Gazebo仿真在RexRov AUV上验证了方法的有效性。与基于PID的跟踪方法进行比较，并量化AUV进入目标通信范围时的航位推算定位误差。

Conclusion: 该集成框架能够有效应对水下导航挑战，实现鲁棒避障和增强机动性，特别是在狭窄空间中。基于反馈控制器的运动规划方法降低了计算复杂度，提高了操作安全性。
...
### [208] [ExpressMM: Expressive Mobile Manipulation Behaviors in Human-Robot Interactions](https://arxiv.org/abs/2604.05320)
*Souren Pashangpour,Haitong Wang,Matthew Lisondra,Goldie Nejat*

Main category: cs.RO

TL;DR: ExpressMM框架结合高层语言引导规划器和低层视觉-语言-动作策略，为移动机械臂生成协作任务中的表达性行为，支持用户中断和指令更新。


<details>
  <summary>Details</summary>
Motivation: 现有表达性机器人行为方法大多未考虑协作任务中用户可能中断、修改或重定向机器人动作的人机交互场景，需要开发支持此类动态交互的表达性行为框架。

Method: 提出ExpressMM框架：1) 基于视觉语言模型的高层语言引导规划器，用于感知和对话推理；2) 低层视觉-语言-动作策略，生成表达性机器人行为；3) 支持可中断交互以适应用户更新的指令。

Result: 在协作装配场景中演示ExpressMM，通过现场人机交互演示进行观众评估。问卷结果显示：表达性行为帮助观察者清晰理解机器人动作和意图，支持社交适当且可理解的交互；参与者认为机器人对协作任务有用，行为可预测且安全。

Conclusion: ExpressMM框架成功生成了协作任务中的表达性机器人行为，支持可中断交互，提升了机器人有用性、安全性和可预测性的正面感知，促进了协作任务中的人机交互质量。
...
### [209] [AnyImageNav: Any-View Geometry for Precise Last-Meter Image-Goal Navigation](https://arxiv.org/abs/2604.05351)
*Yijie Deng,Shuaihang Yuan,Yi Fang*

Main category: cs.RO

TL;DR: AnyImageNav是一个无需训练的系统，通过将目标图像视为几何查询，利用密集像素级对应关系恢复精确的6-DoF相机位姿，显著提升了图像目标导航的精度。


<details>
  <summary>Details</summary>
Motivation: 传统图像目标导航(ImageNav)的成功标准过于粗糙（在目标1米内停止），这对于需要精确定位的下游任务（如抓取）来说不够精确。

Method: 采用语义到几何的级联方法：语义相关性信号指导探索并作为接近度门控，仅在当前视图与目标图像高度相关时调用3D多视图基础模型；该模型通过自认证循环实现精确的位姿恢复。

Result: 在Gibson数据集上达到93.1%的导航成功率，HM3D上达到82.6%；位姿恢复精度显著提升：Gibson上位置误差0.27m、朝向误差3.41度，HM3D上0.21m/1.23度，比基线方法提升5-10倍。

Conclusion: AnyImageNav通过将目标图像作为几何查询，实现了精确的6-DoF位姿恢复，为需要精确定位的下游任务提供了有效的解决方案。
...
### [210] [Synergizing Efficiency and Reliability for Continuous Mobile Manipulation](https://arxiv.org/abs/2604.05430)
*Chengkai Wu,Ruilin Wang,Yixin Zeng,Jiayuan Wang,Mingjie Zhang,Guiyong Zheng,Qun Niu,Juepeng Zheng,Jun Ma,Boyu Zhou*

Main category: cs.RO

TL;DR: 提出一个统一框架，将效率与可靠性协同用于连续移动操作，通过可靠性感知轨迹规划和相位依赖切换控制器，在不确定环境中高效可靠地完成连续任务。


<details>
  <summary>Details</summary>
Motivation: 人类能够无缝融合预期规划和即时反馈来执行连续移动操作任务，实现高效可靠。但机器人复制这种行为面临根本挑战：长期规划与实时反应性冲突，且过度追求效率会损害不确定环境中的可靠性，影响稳定感知和补偿能力，增加意外接触风险。

Method: 1) 可靠性感知轨迹规划器：将可靠执行的基本要素嵌入时空优化，生成高效且可靠性有保障的全局轨迹；2) 相位依赖切换控制器：在全局轨迹跟踪（效率）和任务误差补偿（可靠性）之间无缝切换；3) 分层初始化：促进在线重规划，应对长期规划问题的复杂性。

Result: 真实世界评估表明，该方法能在不确定性（动态干扰、感知和控制误差）下高效可靠地完成连续任务。框架可泛化到具有不同末端执行器约束的任务。相比最先进基线，始终实现最高效率，同时将任务成功率提高26.67%--81.67%。消融研究验证了各组件贡献。

Conclusion: 该统一框架成功协同了移动操作的效率与可靠性，通过将可靠性要素嵌入规划、智能切换控制策略以及分层初始化支持在线重规划，使机器人能够在不确定环境中像人类一样流畅可靠地执行连续任务。
...
TL;DR: 论文提出了JailWAM框架，专门针对世界行动模型（WAM）的越狱攻击进行攻击和评估，填补了WAM安全漏洞研究的空白，在仿真环境中实现了84.2%的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: WAM具有强大的物理交互能力，但现有研究极少关注其安全漏洞，特别是对越狱攻击的脆弱性。这种能力如果被恶意利用，会直接威胁人身安全、财产安全和环境安全。

Method: 提出了JailWAM框架，包含三个核心组件：1）视觉轨迹映射，统一异构动作空间；2）风险判别器，高效识别破坏性行为；3）双路径验证策略，先进行快速粗筛，再通过全闭环物理仿真进行验证。同时构建了JailWAM-Bench基准测试。

Result: 在RoboTwin仿真环境中，该框架高效暴露了物理漏洞，在先进的LingBot-VA模型上实现了84.2%的攻击成功率。同时基于JailWAM可以构建鲁棒的防御机制。
...
TL;DR: 该论文提出了一种模块化空中机器人系统(MARS)的完整解决方案，包括机械设计、动力学建模和控制算法，实现了稳定对接、锁定、分离和敏捷飞行。
...
Method: 1) 设计紧凑机械系统实现被动对接、无检测被动锁定和磁辅助分离；2) 引入力-力矩等效和多面体约束的虚拟四旋翼模型，捕获完整MARS动力学；3) 基于此抽象设计两阶段预测分配管道：约束预测跟踪器计算虚拟输入，动态分配器映射到单个模块产生平滑电机指令。

Result: 在超过10种配置的仿真和真实实验中展示了稳定对接、锁定和分离，以及有效的控制性能。首次在真实世界中实现MARS的敏捷飞行和运输，峰值俯仰角达40度，平均位置误差0.0896米。

Conclusion: 该研究通过机械设计、动力学抽象和控制算法创新，解决了MARS系统的关键挑战，实现了稳定、可扩展且敏捷的模块化空中机器人系统。
...
TL;DR: 本文开发了基于物理的软锥形机器人手仿真模型，结合进化策略优化舀取轨迹，解决了软体机器人控制复杂性问题，在多种挑战性任务中表现出良好泛化能力。
...
Conclusion: 基于物理的仿真模型结合进化策略优化，有效解决了软体机器人舀取任务的控制复杂性问题，为软体机器人的实际应用提供了可行方案。
...
TL;DR: ReV是一个用于机器人操作的视觉运动策略框架，通过结合扩散头和轨迹转向策略，能够在仅使用专家演示训练的情况下，实时适应意外情况并重新规划轨迹。
...
Conclusion: ReV框架能够实时适应意外情况并重新规划轨迹，仅通过专家演示训练即可实现鲁棒的机器人操作，为视觉运动策略学习提供了有效的闭环解决方案。
...
TL;DR: 提出一种4自由度柔性腹腔镜器械的控制架构，通过剪刀连杆模型实现精确运动控制，并与机器人系统集成验证


<details>
  <summary>Details</summary>
Motivation: 微创手术减少患者创伤但传统腹腔镜器械运动范围有限，需要开发更灵活的器械来提升手术操作能力

Method: 设计4自由度柔性腹腔镜器械，包含远端弯曲、独立远端头旋转、轴旋转和夹持器；采用Raspberry Pi 5和Motoron控制器实现驱动单元和SpaceMouse遥操作；建立剪刀连杆分析模型并进行参数化

Result: 预测的钳口开度与CAD测量结果匹配良好（MAE 0.13°），与OptiTrack运动捕捉数据也相符（MAE 1.43°）；与ATHENA并联机器人集成验证了模拟胰腺手术程序

Conclusion: 成功开发了兼容标准套管针的柔性腹腔镜器械控制架构，通过精确建模和机器人集成验证了其在微创手术中的可行性和性能
...
TL;DR: 提出一种新颖的训练框架，通过对比模型显式地对齐视觉-语言-动作层次模型中的语言描述与动作轨迹，使用离线偏好学习优化多模态对齐，在LanguageTable数据集上达到与全监督微调相当的性能。


<details>
  <summary>Details</summary>
Motivation: 实现机器人透明度对于有效的人机协作至关重要。现有层次化视觉-语言-动作模型能够生成语言和低级动作，但在训练过程中没有考虑这些模态之间的显式对齐，这是当前研究的重要空白。

Method: 提出新颖的训练框架，显式地将层次化VLA子任务描述与视觉观察和动作空间进行对齐。使用对比模型评估生成语言与相应动作轨迹之间的对齐程度，通过离线偏好学习直接对不同语言-轨迹对进行排序，从而优化层次化VLA的接地性。

Result: 在LanguageTable数据集（人类语言标注轨迹的基准数据集）上应用该框架，获得了关于多模态接地表示的重要见解，同时建立了强大的基线，性能与完全监督微调相当，并最小化了昂贵数据标注的需求。

Conclusion: 该框架通过显式对齐语言和动作轨迹，有效解决了层次化VLA模型中多模态接地的问题，为实现机器人透明度提供了重要方法，同时减少了数据标注成本。
...
TL;DR: 提出了一种通过修改拉普拉斯矩阵权重实现平面仿射编队集体机动的方法，无需领导者，可实现平移、旋转、缩放和剪切等仿射变换


<details>
  <summary>Details</summary>
Motivation: 传统领导者-跟随者策略限制了机器人集群的集体运动能力，需要一种更灵活的领导者方案来实现更广泛的仿射变换运动

Method: 通过修改拉普拉斯矩阵的原始权重，利用复数表示二维信息，设计特征向量和特征值来定义集体运动，实现时间变化的仿射变换

Result: 理论分析提供了控制集体运动的解析解，通过20个智能体的仿真验证了理论结果，实现了平移、旋转、缩放和剪切等仿射变换

Conclusion: 提出的无领导者方案能够实现更广泛的集体运动，为仿射编队的设计和分析提供了更易处理的方法
...
### [218] [A1: A Fully Transparent Open-Source, Adaptive and Efficient Truncated Vision-Language-Action Model](https://arxiv.org/abs/2604.05672)
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在实际部署中存在成本问题：大规模VLM主干和迭代扩散/流式动作头导致高延迟和高计算成本，难以在普通硬件上实现实时控制。

Method: 采用预训练VLM提供隐式可供性先验，引入预算感知的自适应推理方案，包括监控中间层动作一致性实现早期终止，以及提出层间截断流匹配技术来预热去噪过程。

Result: 在仿真基准和真实机器人上达到最先进成功率，同时显著降低推理成本：流匹配推理延迟降低72%，主干计算减少76.6%，在RoboChallenge上平均成功率29.00%优于基线模型。

Conclusion: A1框架成功解决了VLA模型的高成本部署问题，通过优化完整推理管道实现了低成本、高吞吐量的机器人操作，为开源社区提供了完整的可复现训练栈。
...
### [219] [Rectified Schrödinger Bridge Matching for Few-Step Visual Navigation](https://arxiv.org/abs/2604.05673)
*Wuyang Luan,Junhui Li,Weiguang Zhao,Wenjian Zhang,Tieru Wu,Rui Ma*

Main category: cs.RO

TL;DR: RSBM框架通过共享速度场结构和熵正则化参数ε控制，在3步积分内实现高效视觉导航，平衡多模态覆盖与路径直线性。


<details>
  <summary>Details</summary>
Motivation: 基于扩散模型和薛定谔桥的生成策略需要数十个积分步骤，高方差随机传输阻碍实时机器人控制，需要解决高效视觉导航中的延迟问题。

Method: 提出Rectified Schrödinger Bridge Matching (RSBM)，利用标准薛定谔桥(ε=1)和确定性最优传输(ε→0)之间的共享速度场结构，通过单一熵正则化参数ε控制，基于学习的条件先验缩短传输距离。

Result: 标准桥需要≥10步收敛，而RSBM仅用3个积分步骤就实现了超过94%的余弦相似度和92%的成功率，无需蒸馏或多阶段训练。
...
### [220] [Dynamic Control Allocation for Dual-Tilt UAV Platforms](https://arxiv.org/abs/2604.05677)
*Marcello Sorge,Federico Ciresola,Giulia Michieletto,Angelo Cenedese*

Main category: cs.RO

TL;DR: 该论文研究六旋翼无人机动态控制分配，考虑双倾斜螺旋桨平台，提出分层控制结构和考虑执行器饱和的优化分配策略


<details>
  <summary>Details</summary>
Motivation: 研究六旋翼无人机在轨迹跟踪任务中的动态控制分配问题，特别关注双倾斜螺旋桨平台，旨在解决执行器饱和对控制性能的影响，并探索螺旋桨倾斜角度的优化利用

Method: 采用分层控制结构：高层控制器生成轨迹跟踪所需力矩，控制分配法则确保执行器产生该力矩；分配器对执行器集施加一阶动态特性，利用系统冗余优化目标函数，显式建模执行器饱和并分析其对控制性能的影响

Result: 通过数值仿真验证了控制分配策略的有效性，提供了执行器饱和对控制性能影响的理论分析，并展示了通过目标函数中施加非对称形状来优化螺旋桨倾斜角度的效果

Conclusion: 提出的动态控制分配方法能够有效处理六旋翼无人机轨迹跟踪任务，特别考虑了执行器饱和的实际约束，并通过优化螺旋桨倾斜角度提升了系统性能，为双倾斜螺旋桨平台的控制提供了理论框架和实用方案
...
### [221] [GraspSense: Physically Grounded Grasp and Grip Planning for a Dexterous Robotic Hand via Language-Guided Perception and Force Maps](https://arxiv.org/abs/2604.05697)
*Elizaveta Semenyakina,Ivan Snegirev,Mariya Lezina,Miguel Altamirano Cabrera,Safina Gulyamova,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: 该论文提出了一种基于局部允许接触载荷图的机器人抓取选择和力调节方法，通过考虑物体表面的非均匀力学特性来选择结构更强的接触区域，实现安全可靠的抓取执行。


<details>
  <summary>Details</summary>
Motivation: 现有抓取规划器通常将物体表面视为结构均匀的，但实际上接触在薄弱区域可能导致物体损坏。需要物理基础的接触策略来考虑物体空间非均匀的力学特性。

Method: 1. 使用SAM3D重建物体3D几何模型；2. 物理信息几何分析计算力图，编码每个表面位置在不产生变形情况下的最大允许侧向接触力；3. 基于几何有效性和任务目标一致性筛选抓取候选；4. 使用力图感知标准对候选进行重新排序；5. 阻抗控制器根据接触点的局部允许力缩放每个手指的刚度。

Result: 在纸杯、塑料杯和玻璃杯上的验证表明，该方法能一致地选择结构更强的接触区域，并将抓握力保持在安全范围内。

Conclusion: 该工作将灵巧操作从纯几何问题重新定义为物理基础的联合规划问题，为未来人形系统的抓取选择和抓握执行提供了新框架。
...
### [222] [Hazard Management in Robot-Assisted Mammography Support](https://arxiv.org/abs/2604.05749)
*Ioannis Stefanakos,Roisin Bradley,Radu Calinescu,Beverley Townsend,Tianyuan Wang,Jihong Zhu*

Main category: cs.RO

TL;DR: 该论文提出了一种针对MammoBot（乳腺X光检查辅助机器人系统）的危险管理方法，结合利益相关者引导的过程建模、SHARD和STPA分析，确保临床环境中人机交互的安全性。


<details>
  <summary>Details</summary>
Motivation: 机器人和具身AI系统在临床环境中具有改善护理可及性和质量的潜力，但与脆弱患者的近距离物理接触带来了重大安全风险。需要确保从早期开发阶段就考虑安全性。

Method: 采用利益相关者引导的过程建模，结合软件危险分析与设计解决方案（SHARD）和系统理论过程分析（STPA）。与临床医生、机器人专家和患者代表协作定义机器人辅助工作流程，识别技术程序偏差和分析不安全控制行为。

Result: 分析显示许多危险并非源于组件故障，而是来自时间不匹配、过早行动和系统状态误解。这些危险被转化为精细化和额外的安全要求，约束系统行为并减少对正确人类时机或解释的依赖。
...
### [223] [Precise Aggressive Aerial Maneuvers with Sensorimotor Policies](https://arxiv.org/abs/2604.05828)
*Tianyue Wu,Guangtong Xu,Zihan Wang,Junxiao Lin,Tianyang Chen,Yuze Wu,Zhichao Han,Zhiyang Liu,Fei Gao*

Main category: cs.RO

TL;DR: 使用强化学习训练端到端传感器运动策略，使四旋翼无人机能够在未知位置和姿态的狭窄缝隙中完成高难度穿越，包括静态和动态缝隙。


<details>
  <summary>Details</summary>
Motivation: 无人机在狭窄缝隙中的精确穿越是实现其完全机动性的关键瓶颈，特别是在需要利用倾斜姿态和机身不对称性的SE(3)约束条件下。现有方法难以实现这种高难度的穿越任务。

Method: 开发传感器运动策略，将机载视觉和本体感知直接映射为低级控制指令。使用强化学习在仿真中训练端到端策略，通过基于模型规划器生成的轨迹进行初始化来解决探索困难。采用精心设计的仿真到真实迁移方法。

Result: 方法使无人机能够在5厘米间隙、倾斜90度的矩形缝隙中成功穿越，无需知道缝隙的位置或方向。策略还能对动态移动的缝隙做出反应性穿越。在密集排列的狭窄缝隙轨道上验证了方法的有效性。

Conclusion: 提出的策略学习方法具有灵活性，能够为几何多样的缝隙开发穿越策略，无需依赖手动定义的穿越姿态和视觉特征，为无人机在受限环境中的高机动性操作提供了有效解决方案。
...
### [224] [BiCoord: A Bimanual Manipulation Benchmark towards Long-Horizon Spatial-Temporal Coordination](https://arxiv.org/abs/2604.05831)
*Xingyu Peng,Chen Gao,Liankai Jin,Annan Li,Si Liu*

Main category: cs.RO

TL;DR: BiCoord是一个用于长时程、紧密协调的双臂操作基准测试，旨在解决现有基准测试中任务短时程、协调松散的问题，通过多样化的任务和量化指标系统评估双臂协调能力。


<details>
  <summary>Details</summary>
Motivation: 现有的双臂操作基准测试（如RoboTwin和RLBench2）主要关注短时程和松散协调的任务，无法捕捉真实世界中双臂操作的空间-时间耦合特性，因此需要一个新的基准测试来研究长时程、紧密协调的双臂操作。

Method: 提出了BiCoord基准测试，包含需要连续双臂依赖和动态角色交换的多样化长时程任务，并设计了一套从时间、空间和时空三个维度评估协调性的量化指标。

Result: 实验结果显示，代表性的操作策略（如DP、RDT、Pi0和OpenVLA-OFT）在长时程、高度耦合的任务上表现不佳，揭示了实现长时程紧密协调任务的基本挑战。

Conclusion: BiCoord基准测试为研究长时程协作操作提供了基础，能够激发未来关于协调感知的机器人学习研究，所有数据集、代码和补充材料均已公开。
...
### [225] [Dialogue based Interactive Explanations for Safety Decisions in Human Robot Collaboration](https://arxiv.org/abs/2604.05896)
*Yifan Xu,Xiao Zhan,Akilu Yunusa Kaltungo,Ming Shan Ng,Tsukasa Ishizawa,Kota Fujimoto,Clara Cheung*

Main category: cs.RO

TL;DR: 提出基于对话的框架，用于解释人机协作中的安全决策，支持因果、对比和反事实查询，确保解释不降低安全保证


<details>
  <summary>Details</summary>
Motivation: 机器人在安全关键环境中运行时，仅安全操作不够，还需要让人类协作伙伴理解其安全决策。当前的安全约束对工人不透明，导致协作效率低下

Method: 开发基于对话的解释框架，将解释与基于约束的安全评估紧密结合，使用相同的状态和约束表示。直接从决策记录生成解释，支持因果、对比和反事实查询，反事实推理在固定认证的安全参数下有界评估

Result: 在建筑机器人场景中实例化该框架，通过结构化操作轨迹展示约束感知对话如何澄清安全干预并支持协调任务恢复

Conclusion: 通过将解释作为安全控制的操作接口，为人机协作中交互式、安全感知的自主性提供了设计视角
...
### [226] [You're Pushing My Buttons: Instrumented Learning of Gentle Button Presses](https://arxiv.org/abs/2604.05954)
*Raman Talwar,Remko Proesmans,Thomas Lips,Andreas Verleysen,Francis wyffels*

Main category: cs.RO

TL;DR: 该研究探索使用训练时仪器化（物体传感器化）来改进接触丰富操作策略的性能，而不产生部署时依赖。以按钮按压为测试平台，使用指尖麦克风捕捉接触相关音频，通过仪器化的按钮状态信号作为特权监督来微调音频编码器成为接触事件检测器，然后结合模仿学习策略，在推理时仅使用视觉和音频。


<details>
  <summary>Details</summary>
Motivation: 从相机和本体感知学习接触丰富的操作很困难，因为接触事件只能被部分观察到。研究者希望测试训练时仪器化是否能提高策略性能，同时避免在部署时产生依赖。

Method: 以按钮按压为测试平台，使用指尖麦克风捕捉接触相关音频。利用仪器化的按钮状态信号作为特权监督，微调音频编码器成为接触事件检测器。将得到的表示与模仿学习结合，采用三种策略，确保策略在推理时仅使用视觉和音频。

Result: 按钮按压成功率在不同方法间相似，但仪器化引导的音频表示能持续减少接触力。这表明仪器化作为训练时的辅助目标对于学习接触丰富操作策略是有效的。

Conclusion: 仪器化可以作为实用的训练时辅助目标，用于学习接触丰富的操作策略，能够在不增加部署依赖的情况下提高性能，特别是减少接触力。
...
### [227] [Automating Manual Tasks through Intuitive Robot Programming and Cognitive Robotics](https://arxiv.org/abs/2604.05978)
...
<details>
  <summary>Details</summary>
Motivation: 当前机器人编程通常需要专业编程技能，限制了普通用户的使用。本文旨在通过模仿人类自然交互方式，让非专业用户也能直观地编程机器人，提高机器人的可访问性和实用性

Method: 结合大型语言模型（LLMs）和计算机视觉（CV）技术，将用户的自然语言指令和手势转化为机器人可执行的程序。系统通过澄清问题和视觉反馈机制，让用户能够审查和调整生成的程序

Result: 开发了一个直观的机器人编程框架，能够将人类自然交互转化为机器人程序，并通过交互式反馈机制确保程序的安全性和透明度，提高了用户接受度

Conclusion: 通过结合LLMs、计算机视觉和自然交互反馈机制，成功实现了面向普通用户的直观机器人编程方法，为降低机器人编程门槛、提高人机协作效率提供了有效解决方案
...
### [228] [Force Polytope-Based Cant-Angle Selection for Tilting Hexarotor UAVs](https://arxiv.org/abs/2604.05998)
*Alberto Piccina,Massimiliano Bertoni,Angelo Cenedese,Giulia Michieletto*

Main category: cs.RO

TL;DR: 本文提出了一种轻量级控制框架，用于星形相互依赖倾斜六旋翼无人机执行交互任务，通过离线计算的零力矩力多面体查找表优化倾斜角选择，平衡效率和平滑性。


<details>
  <summary>Details</summary>
Motivation: 从机动性角度看，倾斜多旋翼无人机的主要优势在于可执行力矩的动态可变性，这对物理交互任务至关重要。因此，倾斜角选择需要优化以确保高性能，同时避免突变并保持实际可行性。

Method: 提出轻量级控制框架，使用离线计算的零力矩力多面体查找表来识别期望控制力的可行倾斜角，并通过平衡效率和平滑性选择最优倾斜角。该框架与几何全姿态控制器集成。

Result: 结果显示计算时间显著减少，姿态跟踪性能改善，执行效率具有竞争力。在Simscape中进行的完整墙壁检查任务的物理仿真进一步证实了该策略在交互场景中的可行性。

Conclusion: 所提出的控制框架通过优化倾斜角选择，在保持计算效率的同时提高了星形相互依赖倾斜六旋翼无人机在交互任务中的性能表现。
...
### [229] [A Co-Design Framework for High-Performance Jumping of a Five-Bar Monoped with Actuator Optimization](https://arxiv.org/abs/2604.06025)
*Aastha Mishra,Aman Singh,Shishir Kolathaya*

Main category: cs.RO

TL;DR: 提出了一种用于平面闭链五杆单足机器人的协同设计框架，联合优化机械设计、电机和变速箱参数以及控制参数，以最大化跳跃距离并最小化机械能耗。


<details>
  <summary>Details</summary>
Motivation: 现有协同设计研究大多关注连杆尺寸和传动比优化，忽略了详细的执行器设计（特别是电机和变速箱参数优化），且主要局限于串联开链机构。需要开发能够同时优化机械设计、执行器参数和控制的综合框架。

Method: 采用两阶段优化方法：1）执行器优化生成从齿轮比到执行器质量、效率和峰值扭矩的映射；2）使用CMA-ES算法对机器人设计和控制参数进行协同设计优化，目标是最小化跳跃距离和机械能耗。
...
### [230] [HiPolicy: Hierarchical Multi-Frequency Action Chunking for Policy Learning](https://arxiv.org/abs/2604.06067)
*Jiyao Zhang,Zimu Han,Junhan Wang,Xionghao Wu,Shihong Lin,Jinzhou Li,Hongwei Fan,Ruihai Wu,Dongjiang Li,Hao Dong*

Main category: cs.RO

TL;DR: HiPolicy提出了一种分层多频率动作分块框架，通过联合预测不同频率的动作序列来同时捕捉粗粒度的高级规划和精确的响应性运动，解决了模仿学习中长时依赖与精细闭环控制的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 机器人模仿学习面临一个基本权衡：建模长时程依赖与实现细粒度闭环控制之间的矛盾。现有的固定频率动作分块方法难以同时实现这两个目标。

Method: 提出HiPolicy分层多频率动作分块框架：1）联合预测不同频率的动作序列以捕捉粗粒度高级规划和精确响应性运动；2）从历史观测中提取并融合与每个频率对齐的分层特征进行多频率分块生成；3）引入基于熵的引导执行机制，根据动作不确定性自适应平衡长时程规划与细粒度控制。

Result: 在多样化的模拟基准测试和真实世界操作任务上的实验表明，HiPolicy可以无缝集成到现有的2D和3D生成策略中，在显著提高执行效率的同时，带来一致的性能改进。

Conclusion: HiPolicy通过分层多频率动作分块框架有效解决了模仿学习中长时依赖与精细控制之间的权衡问题，为机器人学习提供了更高效的解决方案。
...
### [231] [Intuitive Human-Robot Interaction: Development and Evaluation of a Gesture-Based User Interface for Object Selection](https://arxiv.org/abs/2604.06073)
...
<details>
  <summary>Details</summary>
Motivation: 手势是人类之间自然的交流方式，也可以用于人机交互，需要开发有效的手势界面来促进人机协作

Method: 开发基于手势的用户界面，使用指向和点击手势进行物体选择，通过20名参与者的实验评估系统性能

Result: 实验评估了准确性和选择时间，展示了系统在高效协作方面的潜力

Conclusion: 基于手势的物体选择界面具有实际应用潜力，能够支持高效的人机协作
...
### [232] [Learning-Guided Force-Feedback Model Predictive Control with Obstacle Avoidance for Robotic Deburring](https://arxiv.org/abs/2604.06133)
*Krzysztof Wojciechowski,Ege Gursoy,Arthur Haffemayer,Sebastien Kleff,Vincent Bonnet,Florent Lamiraux,Nicolas Mansard*

Main category: cs.RO

TL;DR: 将力反馈MPC与扩散运动先验结合，解决工业去毛刺任务中的接触控制、碰撞约束和复杂配置问题


<details>
  <summary>Details</summary>
Motivation: 传统MPC在扭矩控制机器人中应用广泛，但常忽略实时力反馈，难以处理碰撞约束下的接触密集型工业任务。去毛刺任务需要精确工具插入、稳定力调节和无碰撞圆周运动，这超出了标准MPC的能力范围。

Method: 提出一个集成力反馈MPC与扩散运动先验的框架。扩散模型作为运动策略的记忆，提供跨多个任务实例的鲁棒初始化和适应，而MPC确保安全执行，具有显式力跟踪、扭矩可行性和碰撞避免。

Result: 在扭矩控制机械臂上进行工业去毛刺任务验证。实验显示可靠的工具插入、准确的法向力跟踪，以及在难以到达的配置和障碍约束下仍能实现圆周去毛刺运动。

Conclusion: 这是首次将扩散运动先验与力反馈MPC结合，用于碰撞感知、接触密集的工业任务，为解决复杂工业操作提供了有效框架。
