...
TL;DR: 该论文提出了一种新的步态识别视觉模态SKETCH，通过边缘检测提取高频结构线索，并结合解析表示构建了多模态框架SketchGait，在多个数据集上取得了优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别研究主要基于轮廓和解析表示，但轮廓表示稀疏且缺乏内部结构细节，解析表示依赖上游人体解析器导致性能不稳定。需要从结构角度重新思考步态表示，探索新的视觉模态。

Method: 提出SKETCH作为新的步态识别视觉模态，通过边缘检测器从RGB图像中无标签地提取高频结构线索（如肢体关节和自遮挡轮廓）。进一步提出SKETCHGAIT框架，包含两个独立流进行模态特定学习，以及轻量级早期融合分支来捕捉结构互补性。

Result: 在SUSTech1K数据集上达到92.9%的Rank-1准确率，在CCPG数据集上达到93.1%的平均Rank-1准确率，验证了所提模态和框架的有效性。

Conclusion: SKETCH作为一种新的视觉模态，提供了密集的部件级结构信息而无须显式语义标签，与标签引导的解析表示在语义上解耦且结构上互补。多模态框架SketchGait能够有效利用这种互补性，提升步态识别性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有CAD生成方法受限于带有显式设计历史或BRep标签的小型标注数据集，而数百万未标注的3D网格未被利用，限制了可扩展CAD生成的进展。

Method: 将每个BRep表示为参数化补丁集合（如Bézier曲面），使用可微分细分方法生成网格，通过点级监督进行训练，无需CAD特定标注，并引入CADCap-1M数据集（包含100万+ GPT-5生成的描述）。

Result: 在ABC和Objaverse基准测试中，DreamCAD在文本、图像和点模态上都达到了最先进的性能，提高了几何保真度，超过75%的用户偏好。

Conclusion: DreamCAD通过点级监督直接生成可编辑BReps，无需CAD特定标注，解决了现有方法的局限性，并提供了最大的CAD标注数据集CADCap-1M，推动了文本到CAD研究的发展。
...
TL;DR: 提出ABRA方法，通过对抗性批量表示增强解决细胞图像分析中的生物批次效应问题，在RxRx1基准测试中达到最先进水平


<details>
  <summary>Details</summary>
Motivation: 高通量筛选产生大量细胞图像，但实验执行中的技术变异会导致生物批次效应，引起协变量偏移并降低深度学习模型在未见数据上的泛化能力

Method: 将生物批次缓解视为领域泛化问题，提出ABRA方法：通过参数化特征统计量为结构化不确定性来建模批次统计波动，使用最小-最大优化框架在表示空间中合成最坏情况生物批次扰动，采用严格角度几何边界保持细粒度类别可区分性，引入协同分布对齐目标防止表示崩溃

Result: 在RxRx1和RxRx1-WILDS基准测试上的广泛评估表明，ABRA在siRNA扰动分类任务上建立了新的最先进水平

Conclusion: ABRA通过对抗性批量表示增强有效缓解生物批次效应，提高了深度学习模型在细胞图像分析中的泛化能力
...
TL;DR: 提出PFS（后融合稳定器）模块，用于提升BEV融合检测器在域偏移和传感器故障下的鲁棒性，无需修改原有架构或重新训练。
...
Method: 设计轻量级PFS模块，在现有检测器的中间BEV表示上操作，生成精炼的特征图供原始检测头使用。通过稳定域偏移下的特征统计、抑制传感器退化影响的空间区域、通过残差校正自适应恢复弱化线索。

Result: 在nuScenes基准测试中，PFS在多种故障模式下达到最先进结果：相机丢失鲁棒性提升+1.2%，低光性能提升+4.4% mAP，同时仅需3.3M参数，保持轻量级。

Conclusion: PFS作为近恒等变换设计，在保持性能的同时显著提升BEV融合检测器在相机和LiDAR损坏下的鲁棒性，易于集成到已部署系统中。
...
TL;DR: CBM-Suite是一个解决概念瓶颈模型四大挑战的框架：提出概念相关性预评估指标、解决线性化问题、缩小准确率差距、系统分析视觉骨干和VLM的影响


<details>
  <summary>Details</summary>
Motivation: 概念瓶颈模型存在四个基本限制：缺乏预评估概念相关性的指标、线性化问题导致模型绕过概念瓶颈、与不透明模型相比存在准确率差距、缺乏对不同视觉骨干和视觉语言模型影响的系统研究

Method: 1. 提出基于熵的指标量化概念集的内在适用性；2. 在概念激活和分类器之间插入非线性层解决线性化问题；3. 使用线性教师探针引导的蒸馏损失缩小准确率差距；4. 系统分析不同视觉编码器、视觉语言模型和概念集对CBM准确率和可解释性的影响

Result: 广泛评估表明，CBM-Suite能产生更准确的模型，并为改进基于概念的可解释性提供见解

Conclusion: CBM-Suite系统地解决了概念瓶颈模型的关键挑战，通过新的指标、架构改进和系统分析，提高了模型的准确性和可解释性
...
<details>
  <summary>Details</summary>
Motivation: 传统变分自编码器（VAE）的重建FID（rFID）与潜在扩散模型的生成FID（gFID）相关性很差，这限制了使用rFID来预测扩散模型生成质量的能力。

Method: 提出插值FID（iFID）：对数据集中的每个样本，在潜在空间中检索其最近邻，插值它们的潜在表示，然后解码插值后的潜在向量，计算解码样本与原始数据集之间的FID。

Result: iFID与扩散gFID显示出强相关性（Pearson线性相关和Spearman秩相关约0.85），是首个与扩散生成质量强相关的指标。同时澄清了rFID与扩散细化阶段质量相关，而iFID与扩散导航阶段质量相关。

Conclusion: iFID是一种简单有效的评估指标，能够准确预测潜在扩散模型的生成质量，为模型评估提供了新的工具，并解释了重建指标与生成质量负相关的原因。
...
TL;DR: 本文提出隐式错误计数(IEC)方法，用于解决缺乏理想参考答案的任务中强化学习的奖励信号问题，在虚拟试衣任务上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于规则的奖励方法(RaR)依赖于理想参考答案来生成评估标准，但许多现实任务存在多个有效输出且缺乏单一理想答案。本文旨在填补这一空白，为无参考设置提供有效的奖励信号。

Method: 提出隐式错误计数(IEC)方法：不检查响应在规则中做对了什么，而是枚举出错了什么，应用严重性加权评分跨任务相关维度，并将其转换为校准的每方面奖励。包含两个关键设计：隐式分数发射和组校准。

Result: 在虚拟试衣任务上，IEC在MDressBench基准上全面优于RaR（CEC: 5.31 vs. 5.60）。在VITON-HD和DressCode数据集上，IEC在8个感知指标中的6个上匹配或超越了6个基线方法。级联错误计数(CEC)评估指标与人类偏好高度相关（60% top-1 vs. 30% others）。

Conclusion: 当理想答案不可用时，计数错误比构建规则提供更强的信号。IEC方法为无参考设置下的强化学习提供了有效的奖励机制。
...
TL;DR: SemVID是一种无需训练的视频时序定位剪枝框架，通过保留查询关键证据和跨帧连接性，仅用12.5%视觉token保持95.4% mIoU，实现5.8倍预填充加速


<details>
  <summary>Details</summary>
Motivation: 视频时序定位(VTG)需要在长视频中定位查询相关片段，但现有视频语言模型计算成本过高。虽然已有训练无关的视觉token剪枝方法用于视频问答，但直接应用于VTG会导致性能严重下降，因为VTG依赖边界敏感证据和跨帧推理链

Method: 提出SemVID训练无关剪枝框架，基于两个VTG特定原则：证据保留(ER)和连接强度(CS)。首先通过平衡查询相关性和帧间变化分配每帧token预算，然后选择三种token：对象token用于多样化查询关键证据，运动token捕获有意义的过渡并作为跨帧中继，少量上下文token保持场景连续性

Result: 在VTG基准测试中，SemVID实现了优秀的准确率-效率权衡，仅用12.5%视觉token保持95.4% mIoU，提供高达5.8倍预填充加速，在相同预算下始终优于先前方法

Conclusion: SemVID通过识别VTG特定剪枝原则并构建具有互补语义角色的紧凑token子集，有效解决了视频时序定位中的计算效率问题，为视频语言模型的高效应用提供了实用解决方案
...
TL;DR: OWL是一种基于视觉运动线索的3D感知函数，通过两种基本视觉运动线索直接计算值，无需先验知识即可实现3D场景重建和相机航向估计。
...
Method: 基于两种视觉运动线索：1) 注视点附近点的感知局部视觉逼近，2) 刚体相对于注视点的感知旋转。使用简约的像素级并行计算，直接从原始视觉运动图像序列中获取OWL值，无需环境、物体或相机运动的先验知识。

Result: 仿真表明OWL实现了3D物体随时间变化的几何恒常性，仅凭视觉运动线索就能实现缩放3D场景重建。提供了一种实时的相对运动3D点表示方法。

Conclusion: OWL为机器感知提供了新视角，可能扩展到自然感知领域，有助于更好地理解行为心理学和神经功能，为实时决策和交互开辟新可能性。
...
TL;DR: MultiHaystack是首个评估大规模跨模态检索与推理的基准，包含46,000+多模态检索候选项和747个可验证问题，揭示当前多模态大语言模型在异构多模态检索上的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准大多局限于小规模、单模态检索场景，过度简化了搜索空间，夸大了端到端可靠性，无法评估从大规模异构多模态语料库中检索相关证据的关键现实需求。

Method: 构建MultiHaystack基准，包含超过46,000个跨文档、图像和视频的多模态检索候选项，以及747个开放但可验证的问题，每个问题都基于检索池中唯一的已验证证据项，需要跨模态证据定位和细粒度推理。

Result: 模型在提供对应证据时表现良好，但从完整语料库中检索证据时性能大幅下降。最强的检索器E5-V仅达到40.8%的Recall@1，而最先进的MLLMs如GPT-5在提供对应证据时推理准确率为80.86%，但在top-5检索下降至51.4%。

Conclusion: 异构多模态检索仍然是MLLMs的主要瓶颈，MultiHaystack作为一个有价值的测试平台，揭示了小规模评估所掩盖的潜在限制，并促进多模态系统中检索为中心的发展。
...
TL;DR: 本文提出Audiovisual Geolocation框架，通过视听融合解决地理定位问题，创建了AVG基准数据集，采用三阶段方法显著超越单模态基线。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在图像定位方面虽有进展，但精确的全球地理定位仍面临挑战，主要因为视觉景观的固有模糊性和听觉线索的未充分利用。

Method: 提出三阶段框架：1)感知阶段使用混合自回归稀疏自编码器分解噪声音频为"声学原子"；2)多模态推理阶段通过GRPO微调的MLLM融合声学原子与视觉特征；3)精确预测阶段在S^2流形上使用黎曼流匹配。

Result: 实验表明该框架显著优于单模态基线，证明可解释的声音感知提供了关键的正交信号，结合多模态推理可实现高精度全球定位。

Conclusion: 通过视听融合的可解释感知和推理，能够有效解决地理定位中的模糊性问题，为精确全球定位提供了新方法。
...
TL;DR: 提出自动将ImageNet训练集转换为多标签数据集的流程，无需人工标注，通过自监督视觉Transformer进行无监督物体发现，生成的多标签标注能提升模型性能和迁移能力。
...
Method: 使用自监督视觉Transformer进行无监督物体发现，选择与原始标签对齐的区域训练轻量级分类器，然后将其应用于所有区域以生成整个数据集的一致多标签标注。

Result: 生成的多标签标注在定性评估中与人类判断高度一致。相比传统单标签方案，使用多标签监督训练的模型在不同架构上获得更好的域内准确率（ReaL上提升达+2.0 top-1，ImageNet-V2上+1.5），并在下游任务上表现出更强的迁移能力（COCO上提升达+4.2 mAP，VOC上+2.3 mAP）。

Conclusion: 准确的多标签标注对于提升分类性能和表征学习至关重要。提出的自动化流程成功将ImageNet训练集转换为高质量多标签数据集，无需人工标注，为更丰富的视觉学习提供了有效解决方案。
...
TL;DR: 提出基于CLIP的多模态框架，自动从手术视频生成结构化时间线和叙述，减少外科医生手动标注工作
...
Result: 该方法能够自动从手术视频中创建手术时间线和叙述，利用预训练多模态表示桥接视觉手势和文本叙述

Conclusion: 提出的基于CLIP的多模态框架能够有效减少外科医生手动视频审查和标注的需求，为手术时间线创建提供自动化解决方案
...
TL;DR: Icarus是一个全天气天空模型，能够学习全动态范围物理捕捉户外图像，实现用户可控制的太阳和云层定位，提供前所未有的光照准确性和真实感。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习天空模型在高分辨率下无法准确建模14EV+类别不平衡的太阳区域，导致视觉质量差、光照传输偏差、阴影和色调失真。需要更准确、真实的全天气天空模型来替代物理捕捉和参数化天空模型。

Method: 提出Icarus模型，能够学习全动态范围物理捕捉户外图像的曝光范围，支持条件生成环境贴图，允许用户直观定位太阳和云层位置，并扩展当前最先进技术以实现用户控制的大气形态纹理。

Result: Icarus可与全动态范围物理捕捉户外图像或参数化天空模型互换，在基于图像的光照中提供前所未有的准确性、真实感、光照方向性（阴影）和色调。

Conclusion: Icarus是一个创新的全天气天空模型，解决了现有深度学习天空模型在太阳区域建模方面的局限性，为视觉艺术、虚拟现实和科学工程应用提供了高质量的天空环境贴图生成方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有区域布局控制方法存在两大局限：基于训练的方法会继承数据偏差并降低图像质量；现有技术难以处理遮挡顺序，限制了实际应用。作者观察到空间布局和遮挡关系在去噪早期阶段就已确定，因此通过重新排列早期潜在结构来修改最终输出。
...
Conclusion: LayerBind是一种无需训练、即插即用的解决方案，通过分层建模和绑定机制解决了文本到图像生成中的区域布局和遮挡控制问题，为创意应用提供了灵活强大的工具。
...
<details>
  <summary>Details</summary>
Motivation: 稠密图像检索虽然准确，但可解释性有限、归因困难，且在大规模场景下计算成本高。需要一种既能保持高召回率又能降低计算复杂度、同时具备可解释性的检索方法。

Method: 使用稀疏自编码器（SAE）从Vision Transformer的patch特征中提取稀疏视觉词激活，然后应用Okapi BM25评分。利用BM25的逆文档频率（IDF）加权来抑制常见低信息词，强调稀有判别性词。通过稀疏倒排索引操作实现高效检索。

Result: 在七个基准测试中，BM25-V的Recall@200≥0.993，二阶段管道（先BM25-V检索200个候选，再稠密重排）平均准确率仅比纯稠密方法低0.2%。SAE在ImageNet-1K上训练一次即可零样本迁移到七个细粒度基准，检索决策可归因于特定视觉词及其IDF贡献。

Conclusion: BM25-V提供了一种高效、可解释的图像检索方法，作为一阶段检索器能显著降低计算成本，同时保持接近稠密检索的准确率，且具备良好的可迁移性和可归因性。
...
TL;DR: EventGeM：基于事件相机的视觉地点识别方法，通过全局-局部特征融合和深度估计实现实时高精度定位


<details>
  <summary>Details</summary>
Motivation: 事件相机因其稀疏激活和高时间分辨率在机器人视觉任务中日益流行，但在视觉地点识别中需要更准确和实时的定位方法

Method: 使用预训练ViT-S/16提取全局特征进行初始匹配，然后用MaxViT检测局部关键点进行2D单应性重排序，最后用视觉基础模型进行深度估计的结构相似性比较

Result: 在多个基准数据集和光照条件下达到最先进的定位性能，能够在各种计算架构上实时运行，并在真实机器人平台上成功部署

Conclusion: EventGeM通过全局-局部特征融合和深度估计重排序，为事件相机提供了高效准确的视觉地点识别解决方案，适合实时机器人导航应用
...
TL;DR: 提出LIPAR框架，通过检测和跳过重复的潜在补丁来减少视频生成的计算延迟，同时引入注意力恢复机制保持生成质量，将视频编辑吞吐量提升1.45倍。
...
Method: 提出Latent Inter-frame Pruning with Attention Recovery (LIPAR)框架：1) 检测并跳过重复计算的潜在补丁；2) 引入注意力恢复机制，近似被修剪token的注意力值，消除简单修剪方法产生的视觉伪影。

Result: 方法将视频编辑吞吐量提升1.45倍，在NVIDIA A6000上平均达到12.2 FPS（基线为8.4 FPS）。该方法不损害生成质量，无需额外训练即可与模型无缝集成。

Conclusion: LIPAR框架有效弥合了传统压缩算法与现代生成流水线之间的差距，通过利用时间冗余显著提升了视频生成效率。
...
TL;DR: Cog2Gen3D是一个3D认知引导的扩散框架，通过语义和几何表示的双流图融合来增强3D生成的空间几何约束，解决现有方法在绝对几何尺度一致性上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在2D图像生成上成功，但在3D生成中缺乏空间几何约束。现有方法通常使用几何特征作为条件，但只能建模相对关系，容易出现绝对几何尺度不一致的问题。作者认为语义信息和绝对几何能够赋能3D认知，从而实现物理世界的可控3D生成。

Method: 提出Cog2Gen3D框架，包含三个关键设计：1) 认知特征嵌入：将不同模态编码为语义和几何表示，并进一步提取逻辑表示；2) 3D潜在认知图：将不同表示结构化为双流语义-几何图，通过基于共通的交叉注意力融合得到3D认知图；3) 认知引导的潜在扩散：利用融合的3D认知图作为条件来引导潜在扩散过程进行3D高斯生成。

Result: 在基于Marble World Labs构建的验证子集上进行大量实验，结果表明Cog2Gen3D在语义保真度和几何合理性方面显著优于现有方法。

Conclusion: Cog2Gen3D通过统一的框架，利用3D认知图确保3D生成的物理合理性和结构合理性，解决了3D生成中的空间几何约束问题。
...
TL;DR: 本文提出了TumorChain框架，这是一个用于临床肿瘤分析的多模态推理系统，结合3D CT扫描和临床文本理解，通过链式思维推理提高诊断准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 准确的肿瘤分析对临床放射学和精准肿瘤学至关重要，需要从影像发现到临床印象再到病理结论的逐步推理。链式思维推理能提高可追溯性并减少诊断错误，但目前缺乏大规模基准和系统化方法。

Method: 构建了TumorCoT数据集（150万条带CoT标注的VQA指令），提出了TumorChain多模态交错推理框架，结合3D影像编码器、临床文本理解和器官级视觉语言对齐，通过跨模态对齐和迭代交错因果推理进行自我精炼。

Result: 在病变检测、印象生成和病理分类方面相比强基线有持续改进，在DeepTumorVQA基准上表现出强泛化能力，减少了幻觉风险并提高了可追溯性。

Conclusion: 多模态推理在临床实践中具有可靠且可解释的肿瘤分析潜力，TumorChain框架通过链式思维推理提高了诊断准确性和临床实用性。
...
TL;DR: PatchCue是一种基于图像块的视觉提示范式，通过将图像分割为块级表示来增强视觉语言模型的推理能力，相比像素级边界框和点级提示更有效且更符合人类感知习惯。
...
Method: 提出PatchCue：1）将图像分割为块，在块级别表示视觉线索；2）采用两阶段训练：冷启动监督微调输出块级线索，然后通过过程监督的线索奖励进行强化学习，引导中间视觉推理步骤。

Result: 在多个视觉语言模型和多样化基准测试（包括通用视觉问答、复杂推理和文档理解）上的实验表明，PatchCue持续提升整体模型性能。块级线索优于像素级边界框和点级线索。

Conclusion: PatchCue提供了一种更有效且认知对齐的视觉推理范式，通过块级视觉线索更好地与人类感知习惯和现代视觉语言模型的块标记化输入对齐，显著增强了视觉推理能力。
...
TL;DR: 本文系统评估了合成新颖视角在视频地点识别(VPR)中的应用，发现小规模合成视角添加能改善识别效果，而大规模添加时，视角变化幅度不如添加数量和数据集图像类型重要。


<details>
  <summary>Details</summary>
Motivation: 合成新颖视角生成在机器人导航中具有潜在应用价值：地面机器人场景可生成空中视角引导无人机；空中视角可生成地面视角帮助无人机识别地面机器人见过的地点。本研究旨在系统评估合成新颖视角在VPR中的效果。

Method: 使用5个公开VPR图像数据库和7种典型图像相似性方法，系统评估合成新颖视角在VPR中的表现。研究分析不同规模视角添加对识别效果的影响。

Result: 对于小规模合成视角添加，新颖视角能改善VPR识别统计效果。对于大规模添加，研究发现视角变化幅度的重要性不如添加的视角数量和数据集中的图像类型重要。

Conclusion: 合成新颖视角在VPR中具有应用价值，特别是在小规模添加时能改善识别效果。研究为机器人导航中视角合成技术的应用提供了实证依据。
...
TL;DR: CylinderSplat：首个前馈式全景3D高斯溅射框架，通过圆柱形Triplane表示和双分支架构，在单视图和多视图全景新视角合成中实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D高斯溅射方法在处理全景图像时面临挑战：多视图成本体积在稀疏视图场景中难以解决遮挡问题；标准笛卡尔Triplane表示无法有效捕捉360°场景的固有几何结构，导致失真和混叠

Method: 提出圆柱形Triplane表示，与全景数据和曼哈顿世界假设下的真实世界结构更匹配；采用双分支架构：像素分支重建观测良好的区域，体积分支利用圆柱形Triplane补全遮挡或稀疏观测区域；框架灵活支持从单视图到多视图的可变输入

Result: 在单视图和多视图全景新视角合成任务中均取得最先进结果，在重建质量和几何精度方面均优于先前方法

Conclusion: CylinderSplat通过创新的圆柱形Triplane表示和双分支架构，成功解决了全景3D高斯溅射中的几何表示和遮挡处理问题，为实时全景新视角合成提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖隐式有向距离场和事后布局优化，难以直接生成连贯且可直接用于下游应用的3D场景网格。需要一种能够从单张图像直接生成高质量、轻量级场景网格的统一方法。
...
Conclusion: PixARMesh提供了一种从单张图像直接生成完整3D室内场景网格的有效方法，通过统一的自回归模型实现了高质量的几何重建和布局预测，为下游应用提供了可直接使用的网格资产。
...
TL;DR: 本文提出InnoAds-Composer，一个单阶段框架，用于电子商务产品海报生成，通过重要性分析和路由机制实现主题、文字和风格的三条件高效控制，解决了多阶段管道中的保真度、准确性和一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的产品海报生成方法通常采用多阶段管道，难以同时控制主题、文字和风格，存在主题保真度差、文字不准确、风格不一致等问题，需要更高效的解决方案。

Method: 提出单阶段框架InnoAds-Composer，使用三条件控制令牌（主题、字形、风格）；通过层和时间步的重要性分析，将每个条件路由到最敏感的位置，缩短活动令牌序列；设计文本特征增强模块（TFEM）整合字形图像和字形裁剪特征，提升中文文字渲染准确性；构建首个包含主题、文字和风格条件的高质量电子商务产品海报数据集。

Result: 大量实验表明，InnoAds-Composer在显著优于现有产品海报生成方法的同时，没有明显增加推理延迟，在主题保真度、文字准确性和风格一致性方面均有显著提升。

Conclusion: InnoAds-Composer通过创新的三条件控制机制和高效的路由策略，成功解决了电子商务产品海报生成中的关键问题，为同时控制主题、文字和风格提供了有效的单阶段解决方案。
...
TL;DR: 本文提出三种技术来改善概念瓶颈模型(CBMs)的公平性：top-k概念过滤减少信息泄露、移除偏见概念、对抗性去偏见，在公平性与性能权衡方面优于先前工作。


<details>
  <summary>Details</summary>
Motivation: 概念瓶颈模型理论上应通过可解释概念支持公平性，但实际上存在概念信息泄露问题，且在ImSitu等数据集上仅能边际减少性别偏见，需要更有效的公平性提升方法。

Method: 提出三种偏见缓解技术：1) 使用top-k概念过滤器减少信息泄露；2) 移除偏见概念；3) 对抗性去偏见。这些方法共同改善CBM的公平性。

Result: 提出的去偏见CBM在公平性与性能权衡方面优于先前工作，为公平且可解释的图像分类提供了重要进展。

Conclusion: 通过三种偏见缓解技术的组合，显著改善了概念瓶颈模型的公平性，为实现公平且可解释的图像分类系统迈出了重要一步。
...
TL;DR: 该论文提出了CORE-Seg框架，通过推理驱动的复杂病灶分割方法，结合语义引导提示适配器和渐进式训练策略，在ComLesion-14K基准上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割正从传统的视觉模式匹配转向认知推理分析。现有的多模态大语言模型具有广泛常识但缺乏复杂病灶的专业视觉推理能力，而传统分割模型擅长像素级分割但缺乏逻辑可解释性。

Method: 提出了CORE-Seg端到端框架，通过语义引导提示适配器整合推理与分割。设计了从SFT到GRPO的渐进式训练策略，配备自适应双粒度奖励机制以缓解奖励稀疏问题。

Result: 在ComLesion-14K基准上实现了37.06%的平均Dice分数（比第二好的基线高14.89%），并将失败率降低到18.42%。

Conclusion: 该研究通过推理驱动的分割方法有效解决了复杂病灶分割问题，为医学图像分析提供了新的认知推理范式。
...
Method: 提出CollabOD轻量级协作检测框架，集成三种策略：1）结构细节保留；2）跨路径特征对齐；3）定位感知的轻量化设计。从图像处理、通道结构和轻量化设计三个角度优化传统无人机感知模型架构

Result: 该框架增强了表示稳定性，同时保持高效推理。统一的细节感知检测头进一步提高了回归鲁棒性，且不引入额外的部署开销

Conclusion: CollabOD框架有效解决了无人机图像中小目标检测的关键问题，通过协作设计在保持轻量化的同时提升了检测性能
...
TL;DR: 论文提出Art3D框架，将2D转3D的目标从几何精确重建转向艺术化视差合成，以模仿专业3D电影的艺术效果


<details>
  <summary>Details</summary>
Motivation: 现有2D转3D方法虽然几何准确但艺术性不足，无法复制专业3D电影的沉浸感和情感共鸣体验。传统几何重建范式将艺术意图（如零平面偏移、局部深度塑造）误认为是数据噪声或模糊性

Method: 提出Art3D框架，采用双路径架构解耦全局深度参数（宏观意图）和局部艺术效果（视觉笔触），通过间接监督从专业3D电影数据中学习。同时提出初步的评估方法来量化电影对齐度

Result: 实验表明该方法在复制关键局部出屏效果和与电影3D内容的全局深度风格对齐方面显示出潜力，为艺术驱动的转换工具奠定了基础

Conclusion: 论文主张从物理精确的视差估计转向艺术一致的视差合成的新范式，Art3D框架展示了这一方向的可能性，为未来艺术驱动的3D转换工具开发提供了基础
...
TL;DR: BlackMirror是一个用于黑盒环境下检测文本到图像模型后门攻击的框架，通过分析生成图像中部分语义模式的稳定性来识别后门行为。
...
Result: 综合实验表明，BlackMirror能够准确检测各种攻击，在广泛的后门攻击类型中实现了精确检测。

Conclusion: BlackMirror是一个有效的黑盒后门检测框架，能够应对视觉多样化的后门攻击，可作为模型即服务应用中的即插即用模块部署。
...
TL;DR: Pano3DComposer是一个高效的feed-forward框架，用于从全景图像生成3D场景，通过解耦对象生成和布局估计，使用Object-World Transformation Predictor将图像到3D模型生成的局部坐标对象转换到世界坐标，并引入Coarse-to-Fine对齐机制处理未见域图像。


<details>
  <summary>Details</summary>
Motivation: 当前组合式图像到3D场景生成方法存在两个主要问题：1）依赖耗时迭代布局优化或不灵活的联合对象-布局生成；2）大多基于有限视场透视图像，难以创建完整的360度环境。需要一种高效的前馈框架来解决这些限制。

Method: 提出Pano3DComposer框架，包含：1）Object-World Transformation Predictor模块，将现成图像到3D模型生成的3D对象从局部坐标转换到世界坐标；2）基于VGGT架构改进的Alignment-VGGT，使用目标对象裁剪、多视角对象渲染和相机参数预测变换；3）使用伪几何监督训练解决生成对象与真实对象形状差异；4）Coarse-to-Fine对齐机制，通过场景渲染反馈迭代优化几何一致性。

Result: 在合成和真实世界数据集上，该方法在图像/文本到3D任务中实现了优越的几何精度。在RTX 4090 GPU上，可以在约20秒内生成高保真3D场景。

Conclusion: Pano3DComposer通过解耦对象生成和布局估计，结合Object-World Transformation Predictor和Coarse-to-Fine对齐机制，实现了高效的全景图像到3D场景生成，解决了现有方法的效率、灵活性和全景覆盖问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统VAE存在重建-生成差距问题，解码路径不够灵活，且需要独立的编码器和解码器，参数效率较低。需要一种能同时提升重建质量、生成能力和计算效率的替代方案。

Method: 1. 采用Rectified Flow思想，在流时间步上应用解码器实现多步解码，解码路径直且可校正；2. 通过时间反转实现双向推理，解码器同时作为编码器使用；3. 生成式解码方法允许沿路径校正潜在变量。

Result: RAC在重建和生成质量上均超越SOTA VAE方法，同时计算成本降低约70%，参数数量减少近41%。

Conclusion: RAC通过Rectified Flow框架成功替代传统VAE，实现了更优的重建-生成性能平衡，显著降低了计算和参数成本，为自编码器设计提供了新思路。
...
TL;DR: S2I编码将骨架序列转换为图像格式，使预训练视觉模型能用于自监督骨架表示学习，解决骨架数据格式差异和数据集稀缺问题。
...
Method: 提出Skeleton-to-Image Encoding (S2I)表示方法，通过基于身体部位语义划分和排列关节，将骨架序列转换为图像数据，并调整到标准图像尺寸。

Result: 在NTU-60、NTU-120和PKU-MMD数据集上的实验表明，该方法在自监督骨架表示学习中具有有效性和泛化能力，包括在跨格式评估的挑战性场景下。

Conclusion: S2I编码首次实现了使用强大的视觉预训练模型进行自监督骨架表示学习，将丰富的视觉领域知识有效转移到骨架分析中，为异构骨架数据提供了统一的图像格式。
...
<details>
  <summary>Details</summary>
Motivation: 实现零碰撞智能车辆系统需要理解驾驶员的风险感知，这是一个复杂的认知过程，涉及驾驶员对外部刺激的自愿反应以及周围道路使用者对自车的注意力。目前缺乏专门用于驾驶员风险感知研究的大规模数据集。

Method: 1. 构建RAID数据集：包含4,691个标注视频片段，涵盖驾驶员意图操作、道路拓扑、风险情境、驾驶员反应和行人注意力等标签。2. 提出弱监督风险对象识别框架：通过建模驾驶员意图操作与反应之间的关系来识别潜在风险源。3. 分析行人注意力在风险评估中的作用。

Result: 提出的方法在RAID数据集上比现有最先进方法性能提升20.6%，在HDDS数据集上提升23.1%。实验评估证明了该方法的有效性。

Conclusion: RAID数据集为驾驶员风险感知研究提供了有价值的资源，提出的弱监督风险对象识别框架能够有效识别潜在风险源，行人注意力分析有助于更准确的风险评估。
...
TL;DR: ProCap是一个新颖的变更描述框架，将静态图像对比转化为动态过程建模，通过两阶段设计学习变更过程的时间动态，在三个数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有变更描述方法基于静态图像对，忽略了变更过程的丰富时间动态，而理解"如何发生"与理解"发生了什么"同样重要。

Method: 两阶段框架：第一阶段训练过程编码器，通过自动生成中间帧获取关键帧，通过字幕条件掩码重建任务学习潜在动态；第二阶段将编码器集成到编码器-解码器模型中，引入可学习的过程查询来推断潜在过程表示，端到端训练。

Result: 在三个数据集上的实验证明了ProCap的有效性，代码和预训练模型已开源。

Conclusion: ProCap通过将变更建模从静态图像比较重新定义为动态过程建模，能够更好地理解和描述变更过程的时间动态。
...
Result: 在PoseTrack2017基准测试中，相比单帧基线ViTPose实现了+2.3 mAP的提升，超越了现有最先进的视频方法，并在实际应用中获得了更高的运行时帧率。

Conclusion: TAR-ViTPose通过有效聚合和恢复时序信息，显著提升了视频2D人体姿态估计的性能，同时保持了计算效率，为实时应用提供了可行方案。
...
TL;DR: 该报告探讨了巴基斯坦制造的手术器械及其缺陷检测，通过与行业领导者合作，利用深度学习架构（YOLOv8、ResNet-152、EfficientNet-b4）开发自动化缺陷检测方法，提升制造质量标准。


<details>
  <summary>Details</summary>
Motivation: 在现代医疗动态环境中，手术器械的质量标准对临床成功至关重要。即使是微小缺陷也可能导致致命后果，因此制造精度至关重要。报告旨在通过自动化缺陷检测提升手术器械质量，保障患者安全并减少制造商经济损失。

Method: 1. 与巴基斯坦Sialkot手术器械集群的行业领导者Daddy D Pro和Dr. Frigz International合作；2. 开发包含4,414张高分辨率图像的新数据集；3. 集成深度学习架构包括YOLOv8、ResNet-152和EfficientNet-b4；4. 采用自动化光学检测（AOI）工具进行质量保证；5. 识别和纠正关键缺陷如裂纹、锈蚀和结构不规则。

Result: 1. 建立了专门针对巴基斯坦制造手术器械的缺陷检测数据集；2. 开发了基于深度学习的自动化缺陷检测方法；3. 为制造商、医疗专业人员和监管机构提供了质量保证资源；4. 提升了手术器械制造标准，确保更可靠的医疗环境。

Conclusion: 通过结合行业专业知识和尖端技术，该报告为手术器械质量保证提供了有效解决方案。自动化缺陷检测方法不仅有助于防止制造商经济损失，更重要的是保障了患者生命安全，为医疗环境可靠性做出了贡献。
...
Conclusion: 该方法能够直接从多视角图像生成仿真就绪的三角形表面模型，为机器人和仿真应用提供了高效的3D重建解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在处理罕见情况或复杂道路结构时仍有局限，当前道路基础设施主要为人类驾驶员设计，安全改进通常只在事故发生后进行，这种被动方法对需要主动风险缓解的自动驾驶系统构成重大挑战

Method: 提出OD-RASE框架：1)基于道路交通系统专业知识形式化本体论；2)使用大规模视觉语言模型生成基础设施改进建议；3)通过本体论驱动数据过滤提高可靠性；4)在事故前道路图像上自动标注改进建议构建新数据集；5)引入基线方法结合LVLM和扩散模型生成改进建议和改进后道路环境图像

Result: 实验表明本体论驱动数据过滤能够高度准确地预测导致事故的道路结构和相应的改进计划
...
<details>
  <summary>Details</summary>
Motivation: 现有不规则采样时间序列预测方法仅使用历史观测值，无法学习上下文语义和细粒度时间模式，需要更全面的多模态方法。

Method: 提出两阶段编码机制：1)跨模态视觉-文本编码自动生成信息丰富的图像和文本数据；2)ISTS编码提取历史观测的时序特征；3)自适应查询特征提取器压缩MLLM令牌；4)带模态感知门控的多模态对齐模块。

Result: 在真实数据上的大量实验证明了所提解决方案的有效性。

Conclusion: MM-ISTS通过融合时间、视觉和文本模态，能够捕捉复杂时间模式和全面上下文理解，提升不规则采样时间序列预测性能。
...
### [42] [Demystifying KAN for Vision Tasks: The RepKAN Approach](https://arxiv.org/abs/2603.06002)
*Minjong Cheon*

Main category: cs.CV

TL;DR: RepKAN是一种新型遥感图像分类架构，结合CNN的结构效率和KAN的非线性表示能力，通过双路径设计实现可解释的物理推理，在EuroSAT和NWPU-RESISC45数据集上表现优于现有模型。
...
Result: 在EuroSAT和NWPU-RESISC45数据集上的实验结果表明，RepKAN不仅提供了明确的物理可解释推理，而且在性能上超越了最先进的模型，展示了其作为可解释视觉基础模型骨干的潜力。

Conclusion: RepKAN在遥感图像分类中实现了性能与可解释性的双重突破，有望成为未来可解释视觉基础模型的核心架构，为地球观测应用提供更可信和实用的解决方案。
...
TL;DR: 本文提出了一种基于自适应径向投影的文档图像倾斜估计方法，并创建了DISE-2021数据集进行评估，结果表明该方法优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 文档倾斜估计是文档处理系统中的关键任务，其性能直接影响后续处理步骤。随着数字化时代的兴起，虽然已有大量研究关注这一挑战性问题，但仍需更有效的解决方案。

Method: 提出了一种新颖的倾斜估计方法：在2D离散傅里叶幅度谱上应用自适应径向投影，提取文档图像的主导倾斜角度。

Result: 创建了高质量的倾斜估计数据集DISE-2021用于评估不同估计器的性能。实验结果表明，所提方法具有鲁棒性和可靠性，在所有对比方法中表现最优。

Conclusion: 该方法为傅里叶基方法提供了多方面的改进，源代码已公开，为文档倾斜估计任务提供了有效的解决方案。
...
### [44] [Probing Visual Concepts in Lightweight Vision-Language Models for Automated Driving](https://arxiv.org/abs/2603.06054)
*Nikos Theodoridis,Reenu Mohandas,Ganesh Sistu,Anthony Scanlan,Ciarán Eising,Tim Brophy*

Main category: cs.CV

TL;DR: 研究通过分析视觉语言模型在自动驾驶场景中的中间激活，发现简单视觉概念（如物体存在）能线性编码，而空间概念（如物体朝向）只能隐式编码，并识别出感知失败和认知失败两种故障模式。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在自动驾驶应用中越来越普遍，但它们经常在自动驾驶相关的简单视觉问题上失败，且失败原因尚不明确。研究旨在通过分析模型中间激活来理解信息流动瓶颈，从而改进模型性能。

Method: 创建仅针对特定视觉概念不同的反事实图像集，然后使用四种最先进的视觉语言模型的激活训练线性探针来区分这些图像。通过分析概念在线性可分性上的表现，识别信息编码模式。

Result: 1. 物体存在等概念能在线性层面明确编码；2. 物体朝向等空间概念只能通过视觉编码器保留的空间结构隐式编码；3. 即使概念线性编码，模型仍可能回答错误，识别出感知失败和认知失败两种故障模式；4. 物体距离增加会快速降低相应视觉概念的线性可分性。

Conclusion: 研究揭示了视觉语言模型在自动驾驶相关简单视觉任务中的失败原因，区分了感知失败和认知失败两种故障模式，为改进模型在自动驾驶场景中的性能提供了理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 生成式真实世界图像超分辨率存在语义和结构幻觉问题，即输出看起来清晰但与低分辨率输入证据不忠实。基于偏好的强化学习虽然适合，但面临三个主要挑战：缺乏退化鲁棒的LR参考忠实度信号、rollout组优化中的优势崩溃问题，以及真实退化数据覆盖不足限制了rollout多样性和偏好信号质量。
...
Conclusion: LucidNFT通过解决生成式真实世界超分辨率中的关键挑战，包括LR锚定忠实度评估、rollout组优化瓶颈和训练数据限制，为基于流的真实世界超分辨率提供了一个有效的多奖励强化学习框架。
...
### [46] [TempoSyncDiff: Distilled Temporally-Consistent Diffusion for Low-Latency Audio-Driven Talking Head Generation](https://arxiv.org/abs/2603.06057)
...
Method: 采用教师-学生蒸馏框架，教师模型使用标准噪声预测目标训练，指导学生轻量级去噪器以更少推理步骤运行；结合身份锚定和时间正则化减少身份漂移和闪烁，使用音素基础音频条件控制唇部运动。

Result: 在LRS3数据集上实验显示，蒸馏扩散模型能保留强教师模型的重建特性，同时实现显著降低的推理延迟；提供了CPU和边缘计算测量，验证了边缘部署可行性。

Conclusion: 蒸馏扩散模型可以在保持教师模型性能的同时大幅降低推理延迟，这是向受限计算环境下实用扩散基说话头生成迈出的初步步骤。
...
### [47] [Text-Driven Emotionally Continuous Talking Face Generation](https://arxiv.org/abs/2603.06071)
*Hao Yang,Yanyan Zhao,Tian Zheng,Hongbo Zhang,Bichen Wang,Di Wu,Xing Fu,Xuda Zhi,Yongbo Huang,Hao He*

Main category: cs.CV

TL;DR: 本文提出情感连续说话人脸生成任务(EC-TFG)及相应模型TIE-TFG，能够根据文本和动态情感描述生成具有连续情感变化的高质量说话人脸视频


<details>
  <summary>Details</summary>
Motivation: 现有说话人脸生成方法虽然能生成自然的面部运动，但通常只能表达固定目标情感，缺乏人类在传达信息时那种连续变化的自然表达能力

Method: 提出TIE-TFG模型，采用时间密集型情感波动建模技术，能够根据输入文本生成对应的情感变化序列，驱动合成视频中连续的面部表情变化

Result: 广泛评估表明该方法能够产生平滑的情感过渡，并在不同情感状态下保持高质量的视觉效果和运动真实性

Conclusion: EC-TFG任务和TIE-TFG模型成功解决了说话人脸生成中情感连续变化的问题，为生成更自然、富有表现力的数字人脸提供了有效方案
...
TL;DR: OVGGT：一种训练免费的流式3D重建框架，通过自选择缓存和动态锚点保护技术，在固定计算和内存预算下处理任意长度视频，实现SOTA几何精度


<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型使用全注意力机制导致二次计算成本，只能处理短序列；因果注意力变体如StreamVGGT虽然支持单次流式处理，但KV缓存会持续增长，在数百帧内耗尽GPU内存，无法实现真正长期部署的流式推理
...
Conclusion: OVGGT通过训练免费的方式解决了流式3D重建中的内存和计算瓶颈，实现了真正可扩展的长序列视频处理，为实际部署提供了可行方案
...
Method: 采用两阶段策略：1）目标分割裁剪区域；2）使用CLIP生成图像-文本嵌入或CNN/MLP提取特征，通过SVD构建共享表示空间，最后通过嵌入相似度匹配进行识别。

Result: 在COCO、Pascal VOC和ADE20K数据集上，无训练的CLIP编码（无SVD）获得最高平均AP，超越当前SOTA方法，同时验证了CNN/MLP编码的潜力。

Conclusion: 提出的OVOR框架简化了开放词汇目标识别流程，无需复杂训练和标注，在多个基准上表现出色，为实际应用提供了高效解决方案。
...
TL;DR: Place-it-R1是一个端到端视频对象插入框架，利用多模态大语言模型进行环境感知推理，通过Think-then-Place范式实现物理一致的视频编辑。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑技术主要关注视觉保真度而非物理因果关系，导致插入的对象与环境物理不一致。需要开发能够理解物理场景并进行环境感知推理的视频对象插入方法。

Method: 采用Think-then-Place范式：1) MLLM进行物理场景理解和交互推理，生成环境感知的思维链令牌和有效插入区域；2) 引入MLLM引导的空间直接偏好优化，通过反馈评分提升视觉自然度；3) 推理时MLLM迭代触发精炼循环，形成闭环优化；4) 提供两种用户可选模式：灵活性模式（允许环境修改）和标准模式（保持场景完整性）。

Result: 大量实验表明，Place-it-R1相比最先进解决方案和商业模型，实现了物理一致性的视频对象插入，在物理合理性和视觉质量方面表现优异。

Conclusion: 该框架成功将MLLM的认知推理能力与视频扩散模型的生成能力相结合，通过环境感知推理实现了物理一致的视频对象插入，为用户提供了在合理性和保真度之间的明确控制。
...
TL;DR: 本文提出CR-QAT框架，通过课程量化感知训练和关系知识蒸馏，解决开放词汇目标检测模型在极端低比特量化下的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测（OVOD）模型通常规模庞大，难以部署到资源受限设备。量化虽然能压缩模型，但极低比特（如4位）量化会严重破坏细粒度视觉-语言对齐和区域间关系结构。
...
Conclusion: CR-QAT框架有效解决了OVOD模型在极端量化下的性能退化问题，通过课程量化和关系知识蒸馏的结合，在保持模型压缩的同时显著提升了检测性能。
...
TL;DR: VLM-RobustBench评估视觉语言模型在现实世界图像失真下的鲁棒性，发现当前VLM在语义理解上强大但在空间变换上脆弱，特别是重采样和几何失真对性能影响最大。
...
Method: 提出VLM-RobustBench基准测试，涵盖49种增强类型（噪声、模糊、天气、数字和几何扰动），在分级严重程度（低/中/高）和二元变换下评估，共133个损坏设置。评估了四个VLM家族（Qwen、InternVL、Molmo、Gemma）在两个基准上：MMBench（视觉基础）和MMMU-Pro（推理导向）。

Result: 视觉严重程度是性能下降的弱预测指标：低严重度的空间扰动通常比视觉上严重的光度失真更降低性能。低严重度的glass_blur使MMBench准确率平均下降约8pp，而最大下降来自重采样和几何失真（如upsample、elastic_transform），可达34pp。

Conclusion: 当前VLM语义理解强大但空间变换脆弱，需要定义新的鲁棒性评估协议和训练机制，强调重采样和几何不变性。
...
### [53] [Breaking Smooth-Motion Assumptions: A UAV Benchmark for Multi-Object Tracking in Complex and Adverse Conditions](https://arxiv.org/abs/2603.05970)
*Jingtao Ye,Kexin Zhang,Xunchi Ma,Yuehan Li,Guangming Zhu,Peiyi Shen,Linhua Jiang,Xiangdong Zhang,Liang Zhang*

Main category: cs.CV

TL;DR: DynUAV是一个新的无人机视角多目标跟踪基准测试，专门针对无人机快速移动和敏捷机动带来的复杂观测挑战，包含42个视频序列和超过170万个边界框标注。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角多目标跟踪基准测试缺乏真实场景中的复杂性，主要包含可预测的相机动态和线性运动模式，无法反映无人机快速移动和敏捷机动带来的显著观测挑战。

Method: 作者创建了DynUAV基准测试，包含42个视频序列，标注了超过170万个边界框，覆盖车辆、行人以及挖掘机、推土机、起重机等专业工业类别。该基准测试以强烈的自我运动和由此产生的复杂表观轨迹为特征。

Result: 与现有基准测试相比，DynUAV引入了由自我运动带来的重大挑战，包括剧烈的尺度变化、视角变化以及运动模糊。对最先进跟踪器的综合评估显示它们在处理这种动态条件下的检测和关联交织挑战方面存在局限性。

Conclusion: DynUAV作为一个严格的基准测试，能够推动真实世界无人机视角多目标跟踪的进展，所有资源将公开提供。
...
TL;DR: 本文提出RF-Sampling，一种专为流匹配模型设计的无训练推理增强框架，特别针对CFG蒸馏变体如FLUX，通过理论推导证明其隐式执行文本-图像对齐分数的梯度上升，提升生成质量和提示对齐。


<details>
  <summary>Details</summary>
Motivation: 当前推理时增强策略主要适用于传统扩散模型，在流模型上表现不佳，特别是CFG蒸馏变体如FLUX。需要为流模型设计专门的推理增强框架来提升生成质量和文本提示对齐。

Method: 提出反射流采样(RF-Sampling)，基于理论推导的无需训练框架，利用文本表示的线性组合并结合流反演，使模型能够探索与输入提示更一致的噪声空间。

Result: 在多个基准测试中，RF-Sampling一致提升了生成质量和提示对齐，并且是首个在FLUX上展现一定程度测试时缩放能力的推理增强方法。

Conclusion: RF-Sampling为流模型提供了有效的推理增强框架，填补了传统增强方法在流模型上的空白，通过理论推导和实验验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 字形和文字系统相似度学习面临根本挑战：人造字母表中的单个字素可以可靠标记，但不同文字系统之间的历史关系不确定且存在争议。需要一种方法既能学习清晰边界，又能发现潜在的历史相似性。

Method: 两阶段框架：第一阶段在有标记的人造字母表上使用对比损失训练编码器，建立具有鲁棒判别特征的教师模型；第二阶段通过师生蒸馏扩展到历史文字系统，学生在教师指导下学习无监督表示，但可以自由发现跨文字的潜在相似性。

Result: 在多样化的文字系统上进行实验，展示了有效的少样本字形识别和有意义的文字聚类，无需真实进化关系作为监督信号。

Conclusion: 该框架通过结合有监督对比学习和无监督发现，既能区分不同文字系统的硬边界，又能反映潜在历史影响的软相似性，为解决字形相似度学习中的认识论约束提供了有效方法。
...
### [56] [Cut to the Chase: Training-free Multimodal Summarization via Chain-of-Events](https://arxiv.org/abs/2603.06213)
*Xiaoxing You,Qiang Huang,Lingyu Li,Xiaojun Chang,Jun Yu*

Main category: cs.CV

TL;DR: CoE是一个无需训练的多模态摘要框架，通过事件链和分层事件图实现结构化推理，在8个数据集上超越现有视频CoT基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态摘要方法存在三个主要问题：依赖领域特定监督、隐式融合导致跨模态关联弱、平坦时间建模缺乏事件转换。需要解决这些问题以提升摘要质量。

Method: 提出CoE框架，基于分层事件图进行结构化推理，通过事件链指导跨模态定位、建模事件演化和因果转换，并使用轻量级风格适应进行领域对齐。

Result: 在8个多样化数据集上的实验表明，CoE一致优于最先进的视频CoT基线方法，平均提升ROUGE +3.04、CIDEr +9.51、BERTScore +1.88。

Conclusion: CoE框架通过显式的事件层次结构解决了现有多模态摘要方法的局限性，展现了强大的鲁棒性、可解释性和跨领域泛化能力。
...
TL;DR: RePer-360是一个针对360度全景图像深度估计的轻量级自适应框架，通过保持预训练透视先验知识，仅需1%训练数据即可显著提升全景深度估计性能


<details>
  <summary>Details</summary>
Motivation: 现有深度基础模型在透视图像上表现良好，但在360度全景图像上泛化能力差，因为透视和全景域之间存在显著的几何差异。完全微调这些模型通常需要大量全景数据，成本高昂。

Method: 提出RePer-360框架：1）轻量级几何对齐引导模块，从ERP和CP两种互补投影中提取调制信号；2）自条件AdaLN-Zero机制，生成像素级缩放因子以减少特征分布差距；3）立方体贴图域一致性损失，提高训练稳定性和跨投影对齐

Result: RePer-360超越标准微调方法，仅使用1%训练数据。在同域训练设置下，RMSE提升约20%。该框架有效保持了预训练的透视先验知识，同时适应全景域

Conclusion: 通过从互补投影融合转向在保持预训练透视先验下的全景域自适应，RePer-360实现了高效的全景深度估计，显著减少了数据需求并提升了性能
...
TL;DR: TaPD是一个用于可变历史长度轨迹预测的统一即插即用框架，通过渐进式知识蒸馏和时间回填机制，在短观测条件下显著提升预测性能。
...
Method: 提出TaPD框架，包含两个协作模块：1) 观测自适应预测器(OAF)，基于渐进式知识蒸馏(PKD)，通过层次特征回归将长时程"教师"的运动模式知识转移到短时程"学生"；2) 时间回填模块(TBM)，在极短历史条件下显式重建缺失的历史片段。采用余弦退火蒸馏权重方案平衡预测监督和特征对齐，并使用解耦的预训练-重建-微调协议。

Result: 在Argoverse 1和Argoverse 2数据集上的广泛实验表明，TaPD在所有观测长度上都一致优于强基线方法，在极短输入条件下提升尤其显著，并能以即插即用方式改进其他预测器（如HiVT）。

Conclusion: TaPD为可变历史长度的轨迹预测提供了一个有效的统一解决方案，通过渐进式知识蒸馏和时间回填机制，显著提升了在短观测条件下的预测性能，具有良好的实用性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 当前高质量视觉特效制作需要专业知识和高成本生产流程，现有AIGC系统面临特效特定数据稀缺、超自然或风格化效果建模困难等挑战，且通常需要逐特效微调，限制了其扩展性和对新特效的泛化能力。

Method: EffectMaker采用统一推理-生成框架：1）多模态大语言模型解释高级特效语义并推理如何适应目标主体；2）扩散变换器通过上下文学习从参考视频中捕捉细粒度视觉线索；3）构建语义-视觉双路径引导机制；4）创建EffectData数据集（包含3k个VFX类别的130k个视频）提升泛化能力。

Result: 实验表明EffectMaker在视觉质量和特效一致性方面优于现有最先进基线方法，提供了一个可扩展且灵活的自定义VFX生成范式。

Conclusion: EffectMaker通过统一的推理-生成框架解决了现有AIGC系统在视觉特效生成中的关键限制，实现了无需逐特效微调的准确、可控、特效一致的合成，为定制化VFX生成提供了可扩展的解决方案。
...
TL;DR: MOSIV：首个针对视频中多物体系统识别的框架，通过可微分模拟器直接优化连续物体材料参数，显著提升多物体交互场景的识别精度和仿真保真度。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对单物体场景或使用固定材料原型的离散分类，无法有效处理视频中多物体系统识别这一挑战性问题。

Method: 提出MOSIV框架，使用可微分模拟器直接优化连续、每物体的材料参数，通过从视频中提取的几何目标进行指导，并创建了包含接触丰富的多物体交互的合成基准数据集。

Result: 在新建的基准测试中，MOSIV相比适应后的基线方法，显著提高了接地精度和长时程仿真保真度，成为该新任务的强基线。

Conclusion: 物体级细粒度监督和几何对齐的目标对于复杂多物体场景中的稳定优化至关重要，MOSIV为多物体系统识别提供了有效的解决方案。
...
TL;DR: HiPP-Prune：一个层次化偏好条件结构化剪枝框架，用于视觉语言模型的高效部署，通过多目标优化平衡任务效用、幻觉鲁棒性和压缩率
...
Method: 提出分层偏好条件结构化剪枝框架，将剪枝视为多目标下的条件资源分配；通过单一策略调用输出全局剪枝蓝图，分解为总体稀疏度预算和分层分配；策略状态集成视觉敏感性信号；使用计划级组相对策略优化在多目标回报下优化剪枝计划

Result: 在LLaVA模型上使用POPE和ScienceQA的实验表明，HiPP-Prune能发现多样化的非支配剪枝计划，在匹配稀疏度预算下提供可控的鲁棒性-效用权衡

Conclusion: HiPP-Prune框架通过层次化偏好条件剪枝，有效解决了视觉语言模型剪枝中的多目标平衡问题，实现了可控的鲁棒性-效用权衡
...
### [62] [Devil is in Narrow Policy: Unleashing Exploration in Driving VLA Models](https://arxiv.org/abs/2603.06049)
...
TL;DR: Curious-VLA提出两阶段框架解决VLA模型探索不足问题：IL阶段通过FTE生成多样化轨迹，RL阶段使用ADAS采样和SDR奖励提升驾驶质量敏感性，在Navsim基准上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶VLA模型存在"窄策略限制"问题：模仿学习阶段探索不足，导致后续强化学习阶段因反馈多样性不足而提前饱和，限制了模型性能潜力。

Method: 1. IL阶段：引入可行轨迹扩展策略生成多个物理有效轨迹，使用步级归一化轨迹表示适配多样化数据；2. RL阶段：提出自适应多样性感知采样优先选择高多样性样本，引入跨度驾驶奖励配合聚焦风格加权来放大奖励值跨度，提升对驾驶质量的敏感性。

Result: 在Navsim基准测试中，Curious-VLA取得SOTA结果：PDMS 90.3，EPDMS 85.4，Best-of-N PDMS达到94.8，有效释放了VLA模型的探索潜力。

Conclusion: Curious-VLA通过两阶段设计缓解了探索-利用困境，解决了VLA模型的窄策略限制问题，显著提升了自动驾驶模型的性能表现。
...
TL;DR: StruVis是一个通过结构化视觉表示增强文本到图像生成推理的新框架，避免了传统方法的计算成本高或视觉信息缺失问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成的推理框架存在两种主要问题：纯文本推理缺乏视觉上下文，容易忽略关键空间和视觉元素；文本-图像交错推理虽然增强了视觉基础，但计算成本高昂且受限于生成器的表示能力。

Method: 提出StruVis框架，使用基于文本的结构化视觉表示作为中间推理状态，让多模态大语言模型在纯文本推理过程中有效"感知"视觉结构，实现结构化视觉引导的推理。

Result: 实验表明StruVis在推理型文本到图像生成基准上取得显著性能提升，在T2I-ReasonBench上获得4.61%增益，在WISE上获得4%增益。

Conclusion: StruVis作为一个与生成器无关的推理框架，能够无缝集成到多种文本到图像生成器中，高效提升其在推理型文本到图像生成中的性能。
...
### [64] [Occlusion-Aware SORT: Observing Occlusion for Robust Multi-Object Tracking](https://arxiv.org/abs/2603.06034)
...
Method: 提出OA-SORT框架，包含三个核心组件：1)遮挡感知模块(OAM)分析物体遮挡状态并引入高斯地图减少背景影响；2)遮挡感知偏移(OAO)利用遮挡状态缓解成本混淆；3)偏置感知动量(BAM)抑制估计不稳定性。

Result: 在DanceTrack测试集上，OA-SORT达到63.1% HOTA和64.2% IDF1。将遮挡感知框架集成到其他四个跟踪器中，平均提升HOTA 2.08%和IDF1 3.05%，证明了框架的可重用性。

Conclusion: 遮挡处理对多目标跟踪至关重要，OA-SORT框架通过创新的遮挡感知机制有效解决了位置成本混淆问题，且具有即插即用、无需训练的优点，能够显著提升多种跟踪器的性能。
...
### [65] [VG3S: Visual Geometry Grounded Gaussian Splatting for Semantic Occupancy Prediction](https://arxiv.org/abs/2603.06210)
*Xiaoyang Yan,Muleilan Pei,Shaojie Shen*

Main category: cs.CV

TL;DR: VG3S提出了一种基于视觉基础模型几何先验的3D高斯泼溅占用预测框架，通过分层几何特征适配器将VFM的3D几何能力注入到占用预测中，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯泼溅的占用预测方法严重依赖准确的几何线索，但在纯视觉中心范式中这些线索往往不足。视觉基础模型具有强大的几何理解能力，但如何将其有效注入到占用预测中是一个挑战。

Method: 提出了VG3S框架，包含一个即插即用的分层几何特征适配器。该适配器通过特征聚合、任务特定对齐和多尺度重构，将冻结VFM的通用token转换为适合占用预测的几何特征表示。

Result: 在nuScenes占用基准测试中，VG3S相比基线在IoU上提升了12.6%，在mIoU上提升了7.5%。框架能够无缝泛化到不同的VFM，持续提升占用预测精度。

Conclusion: VG3S证明了将预训练的几何基础视觉模型的先验知识整合到占用预测中的巨大价值，为自动驾驶场景理解提供了更强大的几何感知能力。
...
<details>
  <summary>Details</summary>
Motivation: 受生物视觉启发的高维超列特征向量在图像分割中应用有限，主要原因是其计算复杂度随训练集大小线性增长，这限制了其在实际应用中的可行性。

Method: 采用基于VGG16的超列特征，应用分层子采样技术降低计算复杂度，并研究堆叠和投票集成学习方法在稀疏超列上的性能表现。

Result: 在脑肿瘤数据集上，当N≤20的极端小样本情况下，简单的逻辑回归分类器表现最佳。在10%分层子采样率下，N=20时获得最佳平均Dice分数0.66，相比标准多尺度UNet基线有24.53%的显著提升（p值=3.07e-11）。

Conclusion: 分层子采样和集成学习能有效解决超列的计算复杂度问题，在小样本图像分割任务中，简单的逻辑回归分类器在极端低样本情况下表现最优，显著优于标准基线方法。
...
### [67] [DEX-AR: A Dynamic Explainability Method for Autoregressive Vision-Language Models](https://arxiv.org/abs/2603.06302)
*Walid Bousselham,Angie Boggust,Hendrik Strobelt,Hilde Kuehne*

Main category: cs.CV

TL;DR: DEX-AR是一种针对自回归视觉语言模型的新型可解释性方法，通过动态头过滤和序列级过滤机制，生成逐token和序列级的2D热力图来解释模型的决策过程。
...
Method: DEX-AR通过计算逐token生成过程中注意力图的层梯度来生成解释。该方法包含两个关键创新：1）动态头过滤机制，识别专注于视觉信息的注意力头；2）序列级过滤方法，聚合逐token解释同时区分视觉基础和纯语言token。

Result: 在ImageNet、VQAv2和PascalVOC数据集上的评估显示，DEX-AR在使用新型归一化困惑度度量的基于扰动的指标以及基于分割的指标上都取得了持续改进。

Conclusion: DEX-AR为自回归视觉语言模型提供了一种有效的可解释性方法，能够处理复杂的跨模态交互和逐token生成过程，在多个基准测试中表现出优越性能。
...
TL;DR: 该论文提出了一种数据驱动的方法来解决文本到图像模型中字体排版控制的问题，通过构建大规模字体数据集FontUse和训练模型，使模型能够更好地理解和生成符合用户需求的字体样式。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在生成高质量图像方面表现出色，但在控制字体排版方面存在明显不足——模型经常忽略或弱化用户请求的字体样式特征。这限制了模型在需要精确字体控制的实际应用场景中的实用性。

Method: 采用数据为中心的方法，构建了一个专门针对字体排版的大规模数据集FontUse（约7万张图像）。使用分割模型和多模态大语言模型自动生成标注，包括用户友好的提示词、文本区域位置和OCR识别的字符串。提示词明确结合字体样式（如衬线体、手写体、优雅字体）和使用场景（如婚礼请柬、咖啡店菜单）。基于这些标注对现有生成模型进行微调，无需修改架构。

Result: 实验表明，使用该管道训练的模型在多样化的提示词和布局下，生成的文本渲染与请求的属性一致性显著优于竞争基线。还引入了基于Long-CLIP的评估指标来测量生成字体与请求属性之间的对齐度。

Conclusion: 通过数据驱动的方法和专门的字体标注管道，成功提升了文本到图像模型在字体排版控制方面的能力，使模型能够更一致地解释样式和使用场景条件，为字体敏感的文本生成应用提供了实用解决方案。
...
### [69] [NOVA: Next-step Open-Vocabulary Autoregression for 3D Multi-Object Tracking in Autonomous Driving](https://arxiv.org/abs/2603.06254)
*Kai Luo,Xu Wang,Rui Fan,Kailun Yang*

Main category: cs.CV

TL;DR: NOVA提出了一种基于大语言模型自回归生成的新范式，将3D多目标跟踪从传统的距离匹配转向生成式时空语义建模，显著提升了开放词汇场景下的跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D多目标跟踪方法受限于闭集假设和"语义盲"启发式规则，无法有效泛化到未知目标类别。开放世界感知需要能够处理未知目标的通用跟踪方法。

Method: 将3D轨迹重新定义为结构化时空语义序列，利用大语言模型的自回归能力，将跟踪任务转化为基于原则的下一步序列补全过程，通过语言空间层次结构解决语义歧义并保持身份一致性。

Result: 在nuScenes、V2X-Seq-SPD和KITTI数据集上表现出优越性能。在nuScenes数据集上，NOVA在Novel类别上达到22.41%的AMOTA，相比基线绝对提升20.21%，仅使用0.5B参数的自回归模型。

Conclusion: NOVA通过生成式时空语义建模范式，成功将3D跟踪从传统距离匹配转向语义感知的序列生成，显著提升了开放词汇场景下的跟踪能力，为开放世界感知提供了新思路。
...
TL;DR: 提出GvU框架，通过自监督强化学习利用统一多模态模型的理解能力来提升其生成质量，缩小理解与生成之间的能力差距。
...
Method: 提出GvU框架：1）基于token级别的内在文本-图像对齐奖励机制，让模型同时作为教师和学生，使用理解分支评估自身输出来指导生成；2）设计自监督强化学习框架，让模型通过基于理解的内在奖励信号迭代提升生成质量，无需外部监督。
...
TL;DR: K-MaT是一个基于提示学习的框架，通过知识锚定和流形传输，实现生物医学视觉语言模型从高端影像模态（如CT）到低端模态（如X光）的零样本跨模态迁移，无需低端训练图像。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模生物医学视觉语言模型在适应高端影像模态（如CT）后，往往无法有效迁移到前线的低端模态（如X光），容易陷入模态特定的捷径学习。需要一种无需低端训练图像就能实现跨模态知识迁移的方法。

Method: 提出K-MaT框架：1）分解提示并锚定到临床文本描述；2）使用融合Gromov-Wasserstein最优传输将低端提示流形与视觉基础的高端空间对齐；3）通过提示学习实现决策结构迁移。

Result: 在四个跨模态基准测试（皮肤镜、乳腺X光到超声、CT到胸部X光）中取得最先进结果：平均准确率调和均值提升至44.1%（BiomedCoOp为42.0%），宏F1分数达36.2%。在具有挑战性的乳腺成像任务中，有效缓解了标准方法（如CoOp）的灾难性遗忘问题。

Conclusion: 通过最优传输对齐提示流形，为医学视觉语言模型的零样本跨模态部署提供了一条高效路径，能够在不使用低端训练图像的情况下实现稳健的跨模态性能。
...
### [72] [Can we Trust Unreliable Voxels? Exploring 3D Semantic Occupancy Prediction under Label Noise](https://arxiv.org/abs/2603.06279)
...
<details>
  <summary>Details</summary>
Motivation: 3D语义占据预测是机器人感知的核心，但真实世界体素标注存在结构伪影和动态拖尾效应等噪声。现有2D标签噪声学习策略在稀疏3D体素空间中会灾难性崩溃，这暴露了现有范式的关键脆弱性。

Method: 提出了DPR-Occ框架，通过双源部分标签推理构建可靠监督。该方法协同利用时间模型记忆和表示级结构亲和性，动态扩展和修剪候选标签集，以保留真实语义同时抑制噪声传播。

Result: 在SemanticKITTI上的实验表明，DPR-Occ在极端噪声下能防止几何和语义崩溃。即使在90%标签噪声下，相比现有标签噪声学习方法，该方法仍能获得显著性能提升（最高2.57% mIoU和13.91% IoU）。

Conclusion: 通过桥接标签噪声学习和3D感知，OccNL和DPR-Occ为动态环境中安全关键的机器人感知提供了可靠基础。
...
TL;DR: GenHOI是一种轻量级增强方法，通过时间平衡和空间选择的方式向预训练视频生成模型注入参考物体信息，显著提升手-物体交互视频合成的物理合理性和物体一致性。


<details>
  <summary>Details</summary>
Motivation: 当前手-物体交互(HOI)视频合成面临两大挑战：现有HOI重演方法缺乏对复杂真实场景的泛化能力，而通用视频编辑模型在HOI特定问题上（如物体外观不一致）表现不佳。需要一种能兼顾泛化能力和HOI特定需求的方法。

Method: 提出GenHOI框架，包含两个核心技术：1) Head-Sliding RoPE：通过为参考token分配头部特定的时间偏移，平衡参考信息在时间维度上的影响，缓解3D RoPE的时间衰减问题；2) 两级空间注意力门：在空间上集中物体条件注意力于HOI区域，并自适应调节注意力强度，保持背景真实性的同时增强交互保真度。

Result: 在未见过的真实场景中进行广泛定性和定量评估，GenHOI在HOI重演和通用视频编辑任务上均显著优于现有最先进方法，特别是在物体一致性和交互物理合理性方面表现突出。

Conclusion: GenHOI通过轻量级的时间平衡和空间选择性增强，有效解决了HOI视频合成中的关键挑战，为预训练视频生成模型提供了强大的HOI能力扩展，在复杂真实场景中展现出卓越的泛化性能。
...
TL;DR: 提出一种用于漫反射直接飞行时间LiDAR的空间校准方法，通过扫描反光片并减去背景，恢复每个像素在RGB图像平面中的响应映射，实现LiDAR与RGB的跨模态对齐和融合。


<details>
  <summary>Details</summary>
Motivation: 漫反射直接飞行时间LiDAR通过宽视场聚集光子返回形成深度直方图，违反了标准LiDAR-RGB校准的单射线假设，需要新的空间校准方法来建立LiDAR像素与RGB图像之间的对应关系。

Method: 使用扫描的反光片配合背景减法，恢复每个LiDAR像素在共置RGB图像平面中的足迹（有效支持区域）和相对空间灵敏度，获得明确的LiDAR到RGB对应关系。

Result: 在ams OSRAM TMF8828设备上演示了该方法，能够为每个漫反射LiDAR像素提供在RGB图像平面中的响应映射，实现跨模态对齐和融合。

Conclusion: 该方法提供了一种简单的空间校准程序，能够有效解决漫反射LiDAR与RGB相机之间的校准问题，为跨模态感知应用提供了实用的解决方案。
...
TL;DR: BEVLM框架将空间一致的鸟瞰图表示与LLMs连接，通过BEV特征作为统一输入提升自动驾驶场景中的推理能力，同时将LLMs的语义知识蒸馏到BEV表示中改善端到端驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将多视角多帧图像独立输入LLMs导致计算冗余和空间一致性不足，而传统BEV表示虽提供空间结构但缺乏语义丰富性，需要结合两者的优势。

Method: 提出BEVLM框架，将空间一致且语义蒸馏的BEV表示与LLMs连接，使用BEV特征作为统一输入，同时将LLMs的语义知识蒸馏到BEV表示中。

Result: BEVLM使LLMs在跨视角驾驶场景中的推理准确性提升46%，通过将LLMs语义知识蒸馏到BEV表示，端到端驾驶性能在安全关键场景中提升29%。

Conclusion: BEVLM成功结合了BEV的空间一致性和LLMs的语义理解能力，显著提升了自动驾驶系统的推理准确性和端到端驾驶性能。
...
Method: 将提示敏感性重新表述为组一致性问题，将语义相关的提示组织成共享相同真实掩码的提示组，引入提示组感知训练框架，包括：(1) 质量引导的组正则化，利用分割损失作为隐式排序信号；(2) 采用停止梯度策略的logit级一致性约束来对齐组内预测

Result: 在多数据集细胞核基准测试中，在文本提示下获得一致增益，显著降低了不同提示质量水平下的性能方差；在六个零样本跨数据集任务中，平均Dice系数提高2.16个百分点

Conclusion: 该方法无需修改架构且保持推理不变，显著提高了计算病理学中视觉语言分割的鲁棒性和泛化能力
...
TL;DR: PSIVG框架通过在视频扩散过程中集成物理模拟器，解决AI生成视频中物体运动违反物理定律的问题，生成物理一致的运动轨迹。
...
Method: 提出PSIVG框架：1) 从预训练扩散模型生成的模板视频重建4D场景和前景物体网格；2) 在物理模拟器中初始化并生成物理一致的运动轨迹；3) 用模拟轨迹指导视频生成器产生时空物理一致的运动；4) 提出测试时纹理一致性优化技术，基于模拟器的像素对应关系调整文本和特征嵌入。
...
TL;DR: CLoPA是一种持续适应策略，通过微调nnInteractive模型的小部分参数来提升医学图像分割性能，无需改变推理流程，在8个医学分割任务中快速达到专家水平。


<details>
  <summary>Details</summary>
Motivation: 现有零样本分割模型（如nnInteractive）在不同医学成像任务中无法稳定达到专家级性能，而标注活动会产生持续增长的任务特定标注数据，因此需要在线适应策略来补充零样本推理。

Method: 提出CLoPA持续适应策略，在标注缓存上微调nnInteractive模型的小部分参数，通过轻量级训练调度触发，无需新增参数或改变推理流程，完全在现有标注工作流中运行。

Result: 在8个医学分割十项全能任务中，CLoPA快速将性能提升至专家水平，即使nnInteractive先前失败的任务也能成功，多数增益在单次训练后实现；不同参数组的调优效果取决于任务特性和数据规模。

Conclusion: CLoPA通过持续适应有效提升交互式分割性能，对于复杂几何目标（如肝血管），实例归一化和低级特征调优会饱和，表明在最具挑战性场景中需要更深层的特征表示对齐。
...
### [79] [DeepSight: Bridging Depth Maps and Language with a Depth-Driven Multimodal Model](https://arxiv.org/abs/2603.06090)
...
Method: 1. 构建新颖的深度图像-文本对数据集和深度指令数据集：使用GLPN模型从视觉图像生成深度图，利用GPT-4生成对应的深度指令，并通过LLaVA验证；2. 改进CLIP中的ViT编码器：融入局部物体信息，以更有效地捕捉深度图的细微连续变化；3. 开发全面的深度问答基准：基于现有深度图像数据集，严格评估典型深度图场景下的理解能力。

Result: 实验结果表明，DeepSight显著增强了深度感知能力和下游任务性能，在多模态三维理解方面取得了实质性进展。

Conclusion: DeepSight作为首个专门用于深度理解的多模态大语言模型，通过创新的数据集构建和模型架构改进，成功解决了现有MLLM在深度信息理解方面的不足，为三维场景理解提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦域泛化行人重识别方法依赖全局特征表示和简单参数平均，存在两个局限：1) 仅使用全局特征难以捕捉细粒度域不变局部细节；2) 统一参数平均忽略客户端在鲁棒特征提取能力上的差异，稀释高质量客户端的贡献

Method: 提出FedARKS框架，包含两个机制：RK（鲁棒知识）机制和KS（知识选择）机制。RK机制专注于提取细粒度、域不变的局部特征；KS机制根据客户端特征提取能力差异进行选择性知识集成

Result: 论文声称FedARKS能够提升模型在未见域上的泛化能力，同时保护客户端数据隐私，但具体实验结果未在摘要中提供

Conclusion: FedARKS通过结合鲁棒知识提取和选择性知识集成，有效解决了联邦域泛化行人重识别中的关键挑战，为保护隐私的同时提升跨域泛化性能提供了新思路
...
TL;DR: 基于45,139张超声图像训练的人工智能系统，能够以超过93%的敏感性和95%的特异性诊断胎儿口面裂，性能媲美资深放射科医生，并能作为医疗副驾驶提升初级医生的诊断能力。
...
Method: 开发了一个人工智能系统，基于来自22家医院的9,215名胎儿的45,139张超声图像进行训练。该系统作为医疗副驾驶辅助放射科医生诊断，并通过包含24名放射科医生和学员的试点研究评估其对专业知识发展的影响。

Result: AI系统诊断胎儿口面裂的敏感性和特异性分别超过93%和95%，与资深放射科医生相当，并显著优于初级放射科医生。作为医疗副驾驶使用时，能将初级放射科医生的敏感性提高6%以上。试点研究表明，该模型能够改善罕见疾病的专业知识发展。

Conclusion: 这种双重用途方法为改善诊断准确性和专家培训提供了可扩展的解决方案，特别是在经验丰富的放射科医生稀缺的环境中，有助于解决口面裂产前检测的挑战。
...
TL;DR: 提出RMD框架，通过跨分辨率分布匹配蒸馏解决扩散蒸馏中步数减少饱和的问题，实现高保真少步多分辨率级联推理
...
Method: 使用对数信噪比曲线划分各分辨率时间步区间，引入logSNR映射补偿分辨率偏移；沿分辨率轨迹进行分布匹配以减少低分辨率生成分布与教师高分辨率分布的差距；在上采样中加入预测噪声重注入机制稳定训练

Result: RMD在保持高保真生成的同时显著加速推理，在SDXL上实现33.4倍加速，在Wan2.1-14B上实现25.6倍加速

Conclusion: RMD框架有效解决了跨分辨率分布差异问题，实现了高质量、高效率的多分辨率级联推理，为扩散模型加速提供了新思路
...
TL;DR: SUREON是一个从手术教学视频中提取专家推理信息构建的大规模视频问答数据集，包含20.68万个问答对，覆盖12个手术推理问题类别，用于训练具备手术推理能力的人工智能模型。


<details>
  <summary>Details</summary>
Motivation: 当前手术AI缺乏专家级的手术推理能力（如理解手术意图、风险评估、决策依据等），因为大规模标注这种推理数据非常困难。然而手术教学视频中已经包含了专家讲解的意图、原理和预期，这些正是手术AI所缺少的推理信息。

Method: 从手术学术视频中系统性地提取训练信号，定义了12个问题类别（包括安全评估、决策原理和预测等），使用多智能体管道大规模提取和结构化监督数据。构建了包含13.47万个视频片段、170种手术类型、20.68万个问答对的数据集，并创建了354个专家验证的基准测试。

Result: 开发了SureonVLM（通过监督微调适应的视觉语言模型）和SureonVLM-R1（使用组相对策略优化训练的推理模型）。两个模型都能回答复杂的手术问题，在SUREON基准测试中准确率超过84%，同时在标准手术感知任务上也优于更大的通用领域模型。

Conclusion: 通过从手术教学视频中提取专家推理信息，可以有效地训练具备手术推理能力的AI模型。SureonVLM-R1展现出明确的推理行为，如从视觉上下文中推断手术意图，证明了这种方法能够赋予AI类似专家的手术理解能力。
...
TL;DR: 研究视觉语言模型在结构化色彩扭曲下的系统感知弱点，发现即使场景对人类仍可识别，模型也会产生自信但无意义的预测


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在基准测试中表现良好，但存在系统性的感知弱点：像素值的结构化大幅变化会导致模型产生自信但无意义的预测，即使底层场景对人类来说仍然容易识别

Method: 使用空间色彩混合方法，这是一种程序化的色彩扭曲家族，在自然图像上叠加结构化模式（包括RGB和Ostwald色彩系统）。引入包含八种变体的框架，在四个数据集上评估来自三个模型家族的九个视觉语言模型

Result: 随着扭曲程度增加，所有模型和数据集上的准确率急剧下降，扩大语言模型规模并不能可靠地缓解这种失败。在61名参与者的人类研究中，人类在相同扭曲下显著优于视觉语言模型。简单的人类启发式预处理步骤可以恢复部分性能

Conclusion: 视觉语言模型存在系统性的感知弱点，需要感知感知的预处理和工具使用作为提高模型鲁棒性的实用策略
...
### [85] [3D CBCT Artefact Removal Using Perpendicular Score-Based Diffusion Models](https://arxiv.org/abs/2603.06300)
...
Method: 提出基于垂直分数扩散模型的3D牙科种植体修复方法，在投影域训练两个不同平面的2D分数扩散模型，通过在采样方案中结合这两个模型来建模投影序列的3D分布。
...
TL;DR: FreeOcc是一个无需训练的3D语义和全景占据预测框架，利用预训练的基础模型从多视角图像中恢复语义和几何信息，在未见过的环境中实现3D场景理解。


<details>
  <summary>Details</summary>
Motivation: 当前基于相机的3D占据预测方法通常需要昂贵的密集3D监督数据，或者需要在目标域数据上训练模型，这限制了在未见环境中的部署。研究者希望开发一种无需训练的方法来解决这个问题。

Method: FreeOcc使用预训练的基础模型：1）使用可提示的基础分割模型提取每视角的全景先验；2）使用重建基础模型重建度量3D点；3）通过深度和置信度感知的过滤将可靠标签提升到3D；4）随时间融合并通过确定性细化堆栈进行体素化；5）通过拟合和合并当前视角的3D边界框候选来恢复实例。

Result: 在Occ3D-nuScenes数据集上，FreeOcc在无需训练的情况下达到16.9 mIoU和16.5 RayIoU，与最先进的弱监督方法相当。作为伪标签生成管道训练下游模型时，达到21.1 RayIoU，超过之前的弱监督基线。在全景占据预测方面，分别达到3.1 RayPQ（无需训练）和3.9 RayPQ（弱监督）。

Conclusion: FreeOcc证明了基于基础模型的感知是实现无需训练3D场景理解的实用途径，为3D语义和全景占据预测建立了新的基线。
...
TL;DR: 提出了一种用于乳腺超声图像分割的半监督框架，通过免训练的伪标签生成和标签精炼，在仅有2.5%标注数据下达到与全监督模型相当的性能


<details>
  <summary>Details</summary>
Motivation: 现有半监督学习方法在标注数据极度有限时，伪标签不稳定导致监督不准确和性能下降。视觉语言模型为伪标签生成提供了新机会，但在乳腺超声图像上效果有限，因为领域特定提示难以迁移

Method: 1) 利用简单外观描述实现跨域结构迁移，让视觉语言模型生成结构一致的伪标签；2) 用伪标签预热静态教师模型捕获病灶全局结构先验；3) 结合指数移动平均教师，引入不确定性熵加权融合和自适应不确定性引导的反向对比学习来改善边界判别

Result: 在四个乳腺超声数据集上的实验表明，该方法在仅使用2.5%标注数据时，性能与全监督模型相当，显著优于现有半监督学习方法
...
TL;DR: JOPP-3D是一个开放词汇语义分割框架，通过联合利用全景图像和点云数据，实现语言驱动的场景理解，在开放和封闭词汇的2D和3D语义分割任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 跨视觉模态（如3D点云和全景图像）的语义分割面临挑战，主要原因是标注数据稀缺以及固定标签模型的适应性有限。需要一种能够利用自然语言查询进行跨模态场景理解的方法。

Method: 将RGB-D全景图像转换为对应的切向透视图像和3D点云，利用这两种模态提取和对齐基础视觉语言特征，通过自然语言查询生成两种输入模态的语义掩码。

Result: 在Stanford-2D-3D-s和ToF-360数据集上的实验表明，JOPP-3D能够生成跨全景和3D领域的一致且有语义意义的分割结果，在开放和封闭词汇的2D和3D语义分割任务上相比现有技术有显著提升。

Conclusion: JOPP-3D框架通过联合利用全景和点云数据，成功实现了语言驱动的跨模态场景理解，为解决标注数据稀缺和模型适应性有限的问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型已成为3D医学图像生成的有力工具，但标准训练目标与临床相关性之间存在差距。需要开发能够同时优化局部纹理细节和全局结构一致性的方法。

Method: 首先在MRI体积数据上预训练3D扩散模型建立生成先验，然后使用近端策略优化（PPO）进行微调，通过结合2D切片评估和3D体积分析的新型奖励系统指导模型优化。

Result: 在BraTS 2019和OASIS-1数据集上的验证表明，RL反馈有效引导生成过程向更高质量分布发展。Fréchet Inception Distance（FID）显著改善，合成数据在下游肿瘤和疾病分类任务中表现出比非优化基线更好的实用性。

Conclusion: 提出的强化学习增强方法能够有效提升3D医学扩散模型的生成质量，使合成数据在临床相关任务中更具实用价值。
...
TL;DR: 本文提出两种技术（自动聚合和逐像素重缩放）来解决训练免费扩散分割器在利用生成能力时的局限性，提升语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有的训练免费扩散分割方法通常依赖于扩散模型的交叉注意力图，理论上更强的生成能力应该带来更好的分割效果。但作者观察到现有方法往往无法有效利用更强大的扩散模型，主要存在两个问题：1）交叉注意力分布在多头和多层中，个体注意力图与统一全局表示之间存在差异；2）即使有全局图，由于不同文本标记之间的分数不平衡，也不能直接转化为准确的分割语义相关性。

Method: 提出了两种技术：1）自动聚合（auto aggregation）：解决多头多层注意力图与全局表示之间的差异；2）逐像素重缩放（per-pixel rescaling）：解决不同文本标记分数不平衡问题，使训练免费分割能更好地利用生成能力。

Result: 在标准语义分割基准上评估了该方法，并进一步将其集成到生成技术中，展示了改进的性能和广泛的适用性。

Conclusion: 通过提出的自动聚合和逐像素重缩放技术，训练免费扩散分割能够更好地利用生成模型的强大能力，在语义分割任务上取得更好的性能。
...
TL;DR: 该论文提出了一种基于骨架的时间动作分割的点监督框架，只需为每个动作片段标注单帧，显著减少标注成本，在多个数据集上达到与全监督方法竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 全监督方法需要昂贵的逐帧标注且对模糊动作边界敏感，点监督方法可以大幅减少标注成本，但现有方法在骨架数据上的应用有限。

Method: 使用预训练统一模型编码多模态骨架数据（关节、骨骼、运动信息），提出原型相似性方法并与能量函数和约束K-Medoids聚类结合生成伪标签，通过多模态伪标签集成指导模型训练。

Result: 在PKU-MMD（X-Sub和X-View）、MCFS-22和MCFS-130数据集上建立了新基准，实验表明该方法达到竞争性性能，甚至超越一些全监督方法，同时显著减少标注工作量。

Conclusion: 提出的点监督框架为骨架动作分割提供了有效的解决方案，在减少标注成本的同时保持高性能，为实际应用提供了实用方法。
...
TL;DR: EntON提出了一种基于特征熵优化的邻域密度化策略，用于3D高斯溅射，通过交替优化框架在几何精度和渲染质量之间取得更好平衡。


<details>
  <summary>Details</summary>
Motivation: 标准3DGS方法产生的高斯中心与底层物体几何对齐不佳，而专注于表面的重建方法又经常牺牲光度精度。需要一种既能保持几何准确性又能保证渲染质量的方法。

Method: 提出特征熵优化的邻域密度化策略EntON，通过计算每个高斯中心k近邻协方差矩阵特征值的特征熵来量化局部结构有序度。采用交替优化框架：标准梯度密度化与特征熵感知密度化交替进行，在低特征熵（有序、平坦）区域优先密度化以捕捉几何细节，在高特征熵（无序、球形）区域进行剪枝。

Result: 在DTU和TUM2TWIN数据集上的实验表明，该方法将几何精度提升高达33%，渲染质量提升高达7%，同时将高斯数量减少高达50%，训练时间减少高达23%。
...
TL;DR: WAFL提出了一种基于词锚定的时间伪造定位新范式，将任务从时间回归和连续定位转变为离散词级二元分类，显著提升了定位性能并降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有时间伪造定位方法存在特征粒度不对齐和计算成本高的问题，需要一种更高效、更准确的定位方法。

Method: 1) 识别最小有意义的伪造单元——词令牌，并与语音的自然语言边界对齐；2) 引入法医特征重对齐模块，将预训练语义空间映射到可区分的法医流形；3) 设计基于伪影的非对称损失，动态抑制真实梯度并优先处理细微的法医伪影。

Result: WAFL在数据集内和跨数据集设置下均显著优于现有方法，同时需要更少的可学习参数并具有更高的计算效率。

Conclusion: WAFL通过将时间伪造定位任务重新定义为词级二元分类，解决了现有方法的局限性，为高效准确的时间伪造检测提供了新范式。
...
TL;DR: HCF-RES是一个用于3D指称表达式分割的多模态框架，通过分层视觉语义分解和渐进多级融合，在稀疏点云基础上引入2D视觉语义信息，提升了细粒度描述下的物体定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D指称表达式分割方法仅依赖稀疏点云，缺乏丰富的视觉语义信息，难以处理匹配多个目标或零目标的细粒度自然语言描述。

Method: 提出HCF-RES框架：1) 分层视觉语义分解：利用SAM实例掩码指导CLIP编码，获取像素级和实例级双重粒度特征，保持2D到3D投影时的物体边界；2) 渐进多级融合：通过模态内协作、跨模态自适应加权和语言引导细化，整合2D语义和3D几何特征。

Result: 在ScanRefer和Multi3DRefer两个基准测试上取得了最先进的结果。

Conclusion: HCF-RES通过有效整合2D视觉语义和3D几何信息，显著提升了3D指称表达式分割的性能，特别是在处理复杂自然语言描述时表现出色。
...
TL;DR: StrSR提出了一种用于扩散变换器（DiT）图像超分辨率的一步对抗蒸馏框架，通过谱和轨迹正则化解决轨迹不匹配和网格状伪影问题
...
Conclusion: StrSR通过谱和轨迹正则化成功解决了DiT一步蒸馏中的关键挑战，为真实图像超分辨率提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 生成式零样本学习面临两个内在挑战：1）类级属性无法捕捉实例特定的视觉外观，导致类-实例差距；2）语义和视觉特征分布之间的显著不匹配，表现为类间相关性，导致语义-视觉领域差距。

Method: 提出ADiVA方法，包含两个模块：1）属性分布建模（ADM）模块，学习每个类的可转移属性分布并为未见类采样实例级属性；2）视觉引导对齐（VGA）模块，细化语义表示以更好地反映视觉结构。

Result: 在三个广泛使用的基准数据集上，ADiVA显著优于最先进方法（如在AWA2和SUN数据集上分别获得4.7%和6.1%的提升）。该方法可作为插件增强现有生成式ZSL方法。
...
Method: 提出FlowMotion框架，直接利用基于流的T2V模型的预测输出，提取潜在预测中的运动表示（流指导），并引入速度正则化策略稳定优化和确保运动平滑演化。

Result: FlowMotion在时间和资源效率上表现优异，与最先进方法相比具有竞争力，通过纯模型预测操作实现高效运动迁移。

Conclusion: FlowMotion通过直接利用流预测输出，实现了高效灵活的视频运动迁移，为无训练运动迁移方法提供了新的有效途径。
...
TL;DR: 提出了一种基于小波增强的专家混合提示学习方法，用于零样本异常检测，通过变分自编码器建模全局语义表示，结合小波分解提取多频特征，提升对复杂异常模式的检测能力。
...
Conclusion: 该方法通过增强语义表示和多频特征提取，显著提升了零样本异常检测的性能，能够更好地适应多样化的异常模式。
...
<details>
  <summary>Details</summary>
Motivation: 当前点云语义分割方法严重依赖人工标注，而无监督点云学习方法仍处于早期阶段，缺乏标注信息和预训练带来了巨大挑战，需要开发有效的无监督策略。

Method: 提出P-SLCR方法：1）一致性结构学习：通过选择高质量特征，在一致性点和一致性原型库之间建立结构特征学习；2）语义关系一致性推理：分别构建一致性和模糊性原型库之间的原型相互关系矩阵，通过该矩阵对两个原型库施加约束以保持语义一致性。

Result: 在S3DIS、SemanticKITTI和Scannet数据集上进行了广泛评估，相比无监督方法取得了最佳性能。在S3DIS数据集的Area-5上达到了47.1%的mIoU，比经典全监督方法PointNet高出2.5%。

Conclusion: 提出的P-SLCR方法通过原型库驱动的一致性学习和推理，有效解决了无监督点云语义分割的挑战，在多个数据集上超越了现有无监督方法，甚至在某些情况下超过了全监督方法。
...
<details>
  <summary>Details</summary>
Motivation: 扩散世界模型虽然具有统一世界模拟的潜力，但迭代去噪过程计算成本过高，不适合交互使用和长时域推演。现有特征缓存方法虽然能加速推理，但为单模态扩散设计的策略在应对世界模型时面临两个特定障碍：多模态耦合和空间变化导致的令牌异质性，以及非均匀时间动态性（少数困难令牌驱动误差增长）
...
Conclusion: WorldCache是针对扩散世界模型量身定制的缓存框架，有效解决了令牌异质性和非均匀时间动态性问题，显著提升了推理效率而不牺牲推演质量，为资源受限场景下的世界模型应用提供了实用解决方案
...
TL;DR: LATO是一种新颖的拓扑保持潜在表示方法，通过顶点位移场和稀疏体素VAE实现可扩展的、基于流匹配的显式3D网格生成。


<details>
  <summary>Details</summary>
Motivation: 现有基于等值面/三角形的扩散模型和自回归生成方法在生成复杂几何形状和良好拓扑结构的网格时存在效率问题，需要一种能够保持拓扑结构的高效网格生成方法。

Method: LATO将网格表示为基于表面的顶点位移场，使用稀疏体素VAE压缩为结构化、拓扑感知的体素潜在表示。通过两阶段流匹配过程：首先生成结构体素，然后细化体素级拓扑特征，最后通过连接头直接预测顶点对之间的边连接性。

Result: 与现有方法相比，LATO能够生成具有复杂几何形状和良好拓扑结构的网格，同时推理效率更高，无需等值面提取或启发式网格化。

Conclusion: LATO提供了一种新颖的拓扑保持潜在表示方法，实现了高效、高质量的显式3D网格生成，为3D内容创建提供了有前景的解决方案。
...
TL;DR: OralGPT-Plus是一个用于全景牙科X光片分析的智能视觉语言模型，通过迭代对称感知诊断推理提升临床可靠性，在MMOral-X基准测试中表现优异。
...
Method: 1. 构建DentalProbe数据集（5000张图像），包含专家标注的诊断轨迹，为局部检查和对比侧比较提供结构化监督；2. 开发重新检查驱动的强化学习框架，通过基于评分标准的奖励和条件诊断驱动奖励，鼓励临床意义的重新检查并稳定长时程推理；3. 创建MMOral-X基准测试，包含300个开放式问题和区域级标注。

Result: OralGPT-Plus在MMOral-X和现有全景基准测试中持续优于强基线模型，表明交互式和对称感知推理的有效性。

Conclusion: 该工作突出了智能建模在牙科影像中的价值，为未来临床对齐的全景X光片分析研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 获取密集像素级标注成本高昂，稀疏标注虽然效率更高但仍存在性能差距，需要利用3D场景重建作为辅助监督信号来弥补这一不足

Method: 采用双学生-教师架构，利用最先进的前馈式3D重建从2D视频恢复3D几何结构，在2D图像和重建的3D点云之间强制执行语义一致性，利用几何监督传播稀疏标注
...
<details>
  <summary>Details</summary>
Motivation: 准确的冠层高度信息对于量化森林碳储量、监测恢复与退化、评估栖息地结构至关重要，但全球范围内高精度的机载激光扫描数据分布不均

Method: 使用基于DINOv3构建的深度估计模型，从高分辨率光学卫星影像中提取冠层高度信息，通过大规模地理多样化的训练数据、自动化数据整理与配准，以及针对冠层高度分布定制的损失函数和数据采样策略

Result: CHMv2相比现有产品显著提高了精度，减少了高森林的偏差，更好地保留了冠层边缘和间隙等精细尺度结构，在主要森林生物群落中表现出稳定的性能

Conclusion: CHMv2提供了一个全球范围内高精度的冠层高度图，通过结合光学卫星影像和深度学习技术，解决了全球机载激光扫描数据分布不均的问题，为森林监测和生态研究提供了重要工具
...
TL;DR: REACT++是基于REACT架构改进的实时场景图生成模型，通过高效特征提取和原型空间中的主体-客体交叉注意力，在保持物体检测性能的同时提升关系预测准确率并降低延迟，比REACT快20%，关系预测准确率平均提升10%。


<details>
  <summary>Details</summary>
Motivation: 当前场景图生成方法往往只专注于关系预测准确性、物体检测准确性或降低延迟中的单一目标，缺乏同时平衡这三个目标的解决方案。为了支持实时应用（如具身智能体推理），需要在性能与推理速度之间取得更好的权衡。

Method: 基于REACT架构，采用高效特征提取和原型空间中的主体-客体交叉注意力机制，平衡延迟与表征能力。

Result: REACT++在现有SGG模型中达到最高推理速度，在保持物体检测性能的同时提升了关系预测准确性。相比REACT，速度提升20%，关系预测准确率平均提升10%。

Conclusion: REACT++通过平衡延迟、关系预测准确性和物体检测性能，为实时场景图生成任务提供了新的最先进解决方案，支持下游实时应用需求。
...
TL;DR: 提出人机协同拼图框架，结合自动算法与人工交互，解决大规模考古碎片重建难题，显著优于纯自动和纯人工方法


<details>
  <summary>Details</summary>
Motivation: 真实考古文物碎片重建面临侵蚀、缺失、不规则形状和大规模模糊性等挑战，传统拼图算法在数千碎片规模下表现不佳，需要新方法应对文化遗产重建的复杂性

Method: 提出人机协同拼图框架，集成自动松弛标记求解器与交互式人工指导，用户可迭代锁定已验证的放置、纠正错误，引导系统实现语义和几何一致的装配；引入两种互补交互策略：迭代锚定和连续交互细化

Result: 在多个RePAIR数据集上的实验表明，该混合方法在准确性和效率上显著优于完全自动化和纯人工基线，为大规模专家协同文物重建提供了实用解决方案

Conclusion: 人机协同框架有效解决了大规模考古碎片重建的挑战，结合自动算法的效率与人类专家的语义理解能力，为文化遗产保护提供了可行的技术路径
...
TL;DR: DiffInf：基于自影响引导扩散的框架，用于减轻面部属性学习中的标注不一致问题，通过生成式修正而非丢弃样本来提升分类性能


<details>
  <summary>Details</summary>
Motivation: 面部属性分类依赖大规模标注数据集，但许多属性（如年龄、表情）本质上是模糊和连续的，却被离散化为分类标签。标注不一致源于主观性和视觉混淆因素（姿态、光照、表情、人口统计变异），导致图像与分配标签不匹配，这些监督误差损害表示学习并降低下游预测性能

Method: 1. 训练基线分类器并计算样本级自影响分数，使用一阶近似识别对优化过程产生不成比例影响的训练实例；2. 应用基于潜在扩散自编码器的目标生成修正，在保持身份和真实性的同时，更好地对齐视觉内容与分配标签；3. 训练轻量级高影响成员预测器作为替代影响正则器，实现可微分的修正指导；4. 用编辑样本替换原始样本，生成大小不变的影响精炼数据集

Result: 在多类面部属性分类任务中，DiffInf相比标准噪声标签训练、鲁棒优化基线和基于影响的过滤方法，持续改善了泛化性能。结果表明，在图像层面修复有影响的标注不一致，能够在不牺牲分布覆盖的情况下增强下游面部属性分类

Conclusion: 通过生成式修正而非丢弃有影响的样本，DiffInf框架能够有效缓解面部属性学习中的标注不一致问题，提升分类性能，同时保持数据集的分布覆盖
...
TL;DR: 研究通过合成飞镖形状的感知冲突实验，发现BEiT视觉Transformer在局部几何证据与全局组织先验的竞争中偏好凸性完成，并定位了实现这一Gestalt先验的具体功能单元。


<details>
  <summary>Details</summary>
Motivation: 研究旨在定位经典Gestalt先验（凸性）在BEiT内部组件中的实现位置，理解视觉Transformer如何解决图形-背景组织中的感知歧义问题。

Method: 使用基于合成飞镖形状的受控感知冲突实验，系统性地掩蔽既可凹性完成也可凸性完成的区域；通过logit归因将内部激活投影到离散视觉码本空间，分析Transformer子结构中的功能单元。

Result: BEiT在感知竞争中可靠地偏好凸性完成；图形-背景组织在早期和中间层保持歧义，在后期层突然解决；识别出头L0H9作为早期种子引入弱凸性偏置，下调该注意力头可使感知冲突跨越连续决策边界。

Conclusion: Gestalt凸性先验在BEiT中由特定功能单元实现，特别是注意力头L0H9作为早期种子，通过调节单个注意力头可以改变感知决策，揭示了Transformer内部感知组织的机制。
...
TL;DR: WanderDream是首个用于心理探索模拟的大规模数据集，包含全景视频和问答对，让模型能够在不进行实际探索的情况下进行空间推理。
...
Conclusion: WanderDream数据集为心理探索模拟提供了首个大规模基准，使模型能够在不进行实际探索的情况下进行空间推理，在受限场景中具有重要应用价值。
...
TL;DR: CaTok是一种1D因果图像分词器，通过MeanFlow解码器和时间间隔选择策略，解决了视觉领域因果表示学习的问题，实现了高效的单步生成和高保真多步采样。


<details>
  <summary>Details</summary>
Motivation: 当前视觉分词器要么将2D图像块展平为非因果序列，要么采用启发式排序方式，与"下一个token预测"模式不匹配。扩散自编码器也存在缺乏因果性或引入不平衡的问题，需要开发真正的因果视觉表示方法。

Method: 提出CaTok：1D因果图像分词器，结合MeanFlow解码器。通过时间间隔选择token并将其绑定到MeanFlow目标，学习因果1D表示。还提出REPA-A正则化方法，将编码器特征与视觉基础模型对齐以稳定训练。

Result: 在ImageNet重建任务上达到SOTA：0.75 FID、22.53 PSNR和0.674 SSIM，且训练轮次更少。自回归模型性能与领先方法相当。

Conclusion: CaTok成功解决了视觉领域的因果表示学习问题，支持快速单步生成和高保真多步采样，同时能捕捉跨token间隔的多样化视觉概念，为视觉自回归建模提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型虽然灵活，但难以通过提示或简单推理适应来控制，无法满足具有严格产品要求的用例。需要构建专门支持图像编辑和增强任务的系统。

Method: 首先在多样化多模态数据集上训练基础扩散模型，然后针对具体下游任务在任务特定数据集上快速微调，生成专门化模型。包括数据集策划、训练和推理的最佳实践。

Result: 在线A/B实验显示，增强图像分别获得18.0%和12.5%的参与度提升。人工评估表明模型在这些任务上优于第三方模型。系统还可推广到多图像场景合成和图像到视频生成等任务。

Conclusion: Pinterest Canvas通过基础模型训练加任务特定微调的方法，有效解决了图像生成模型在严格产品要求下的控制问题，能够支持多种图像编辑和增强用例，并显著提升用户参与度。
...
TL;DR: 该研究系统分析了去噪生成模型的训练目标，重点关注损失权重和输出参数化（噪声、干净图像、速度等），通过数值实验探讨这些选择与数据流形内在维度、模型架构和数据规模的相互作用。


<details>
  <summary>Details</summary>
Motivation: 当前去噪生成模型存在多种训练目标选择（损失权重、输出参数化等），但缺乏对这些设计选择如何影响模型性能的系统理解。研究旨在理清训练流匹配模型时各种因素的重要性，为实践提供指导。

Method: 通过系统数值研究，在合成数据集（具有可控几何结构）和图像数据上实验，分析不同训练目标（噪声、干净图像、速度等参数化）与数据内在维度、模型架构、数据集大小的相互作用，使用PSNR（去噪精度）和FID（生成质量）进行量化评估。

Result: 研究提供了关于训练目标选择如何影响模型性能的实证见解，揭示了损失权重和输出参数化与数据几何特性、模型容量等因素的复杂相互作用关系，但没有提出单一"最佳"方法。

Conclusion: 该研究不是提出新方法，而是理清了训练流匹配模型时各种设计选择的重要性，为实践者提供了基于数据特性、模型架构和可用数据规模选择合适训练目标的实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有放射学基础模型（RFMs）依赖暴力扩展方法，直接移植自然图像处理技术，导致模型在临床实践中脆弱且昂贵。需要开发资源高效的方法来克服这些问题。

Method: 提出GreenRFM框架，采用MUST（More distilled, Ubiquitous, Semantic-enforcing, and Task-aligning）监督设计原则，最大化利用监督信号而非简单堆叠训练数据量。提供两种配置：高性能模型（24GB GPU/24小时）和轻量模型（6GB VRAM/4小时）。

Result: 在超过20万张来自4个机构、2种模态的图像上进行实验，GreenRFM在胸部和腹部CT数据集上均超越基线模型，在内部肌肉骨骼MRI图像上也表现出跨模态迁移能力，计算需求降低数个数量级。

Conclusion: GreenRFM挑战了"规模即一切"的教条，通过资源高效的预训练框架使临床医生即使在笔记本电脑上也能公平地开发最先进的放射学基础模型。
...
TL;DR: Match4Annotate是一个轻量级框架，用于视频内和视频间的点和掩码标注传播，通过SIREN隐式神经表示和DINOv3特征，在临床超声数据集上实现最先进的跨视频传播性能。


<details>
  <summary>Details</summary>
Motivation: 在医学影像等专业领域，逐帧视频标注成本高昂且耗时。现有标注传播方法存在局限性：视频跟踪器和分割模型需要逐视频初始化且无法跨视频泛化；传统对应方法在低纹理场景中表现不佳；密集特征匹配和单次分割方法缺乏时空平滑性且不支持统一的点和掩码标注。

Method: 提出Match4Annotate框架：1）在测试时使用SIREN隐式神经表示拟合DINOv3特征，生成连续高分辨率时空特征场；2）学习帧对之间的平滑隐式变形场来指导对应匹配；3）支持视频内和视频间的点和掩码标注传播。

Result: 在三个具有挑战性的临床超声数据集上评估：Match4Annotate在跨视频传播方面达到最先进水平，优于特征匹配和单次分割基线方法；在视频内传播方面与专用跟踪器保持竞争力。

Conclusion: 轻量级、测试时优化的特征匹配流程有潜力为可扩展的标注工作流程提供高效且易于访问的解决方案，特别是在医学影像等专业领域。
...
TL;DR: Self-Flow：一种自监督流匹配范式，通过双时间步调度机制在生成框架内集成表征学习，无需外部模型即可实现多模态高质量生成


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部模型存在多个问题：需要单独训练、目标不一致、缩放行为不可预测。这种依赖源于当前模型的训练目标（去噪任务）缺乏学习语义表征的激励

Method: 提出Self-Flow自监督流匹配范式，核心机制是双时间步调度：在不同token上应用异构噪声水平，创建信息不对称，迫使模型从损坏输入中推断缺失信息，从而同时学习强表征和生成能力

Result: 方法在图像、视频和音频生成上表现优异，遵循预期的缩放定律，支持多模态训练，无需外部监督即可获得强语义表征

Conclusion: Self-Flow通过将表征学习整合到生成框架中，解决了现有方法对外部模型的依赖问题，实现了自监督的强语义表征学习与高质量生成能力的统一
...
Method: 1) 将所有任务的输入输出标准化为离散标记序列，建立共享表示；2) 设计多尺度时间感知模块捕捉不同时间粒度的音频-视觉事件；3) 设计基于跨模态引导的空间感知模块建模空间音频-视觉关联；4) 使用任务特定的文本提示增强模型适应性和任务感知能力。

Result: 在多个基准数据集（AVE、LLP、MUSIC-AVQA、VGG-SS和AVS）上的广泛实验表明，AV-Unified在时间、空间和时空任务上均表现出有效性。

Conclusion: AV-Unified通过统一框架实现了跨多种音频-视觉场景理解任务的联合学习，标准化了任务表示并设计了有效的时空感知机制，为全面理解复杂音频-视觉场景提供了新方法。
...
TL;DR: 该论文提出了一种在扩散模型中处理语言否定的统一框架，通过将否定建模为扩散动态中语义引导的结构化可行性约束，无需重新训练模型参数。
...
Result: 实验表明该方法实现了稳健的否定遵从性，同时保持了视觉保真度和结构连贯性。该方法能够处理对象缺失、分级非反转语义、多否定组合和范围敏感消歧等多种否定现象，并扩展到时间演化的视频轨迹生成。

Conclusion: 该研究建立了基于扩散的生成模型中语言否定的第一个统一公式，超越了表示级评估，为处理复杂的语言否定现象提供了理论基础和实践框架。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统依赖多源多模态数据，但数据质量因环境和传感器限制而异。现有研究多关注算法设计而忽视数据质量分析，特别是冗余这一基础但未充分探索的数据质量问题。

Method: 使用nuScenes和Argoverse 2数据集，对多源摄像头数据和多模态图像-LiDAR数据中的冗余进行建模和测量，评估移除冗余标注对YOLOv8目标检测任务的影响。

Result: 选择性移除共享视野摄像头中的冗余多源图像标注能提升检测性能：在nuScenes的三个代表性重叠区域，mAP50从0.66提升到0.70、0.64到0.67、0.53到0.55；AV2中移除4.1-8.6%标注后mAP50保持在0.64基线附近。多模态分析显示图像和LiDAR数据间存在显著冗余。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLM发展过度依赖模型规模扩展，不利于在移动设备和边缘设备上部署。传统方法使用对比预训练（如CLIP/SigLIP）初始化的视觉编码器存在目标不匹配问题：对比学习优化的判别性特征抑制了密集描述和复杂推理所需的细粒度视觉线索。

Method: 提出Penguin-VL，其视觉编码器从纯文本LLM初始化，而非传统的对比预训练方法。这种方法能更好地保留细粒度的空间和时间线索，提高视觉保真度和数据效率。

Result: 在各种图像和视频基准测试中，Penguin-VL在数学推理方面与领先VLMs（如Qwen3-VL）性能相当，在文档理解、视觉知识和多视角视频理解等任务中表现更优。这些优势通过轻量级架构实现，表明改进视觉表示而非模型缩放是性能提升的主要驱动力。

Conclusion: Penguin-Encoder作为传统对比预训练编码器的优越替代方案，能保留密集感知和复杂推理所需的关键细粒度线索，为计算高效的VLMs提供了强大的即插即用替代方案，在资源受限环境中实现高性能。
...
TL;DR: 该研究发现多模态大语言模型（MLLM）分类性能评估存在严重偏差，主要源于评估协议缺陷和标注质量问题。通过修正标签和评估方法，MLLM与监督模型的性能差距显著缩小，表明先前报道的MLLM表现不佳主要是评估问题而非模型缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前关于MLLM分类性能的研究存在矛盾结论，作者认为这些矛盾源于评估协议的问题（要么夸大要么低估性能）。需要系统分析评估协议中的关键缺陷，并量化被忽视的设计选择对准确性的影响。

Method: 1. 识别并修复常见评估协议中的关键问题：模型输出超出类别列表被丢弃、弱多选题干扰项夸大结果、开放世界设置中输出映射不佳
2. 量化被忽视的设计选择影响：批量大小、图像顺序、文本编码器选择
3. 使用ReGT（ImageNet-1k的625类多标签重标注）进行评估
4. 进行控制案例研究，评估MLLM辅助人工标注的效果

Result: 1. 修正标签后MLLM性能提升高达+10.8%，显著缩小与监督模型的感知差距
2. 评估协议缺陷和标注噪声是MLLM表现不佳的主要原因，而非模型本身缺陷
3. 较少依赖监督训练信号的模型对标注质量最敏感
4. 在控制案例研究中，标注者在约50%的困难案例中确认或整合了MLLM的预测，显示其在数据集整理中的潜力

Conclusion: MLLM在分类任务中的表现不佳主要是评估协议缺陷和标注质量问题的结果，而非模型能力不足。通过修正评估方法和标注质量，MLLM性能显著提升。MLLM还可有效辅助人工标注，在大规模数据集整理中具有应用潜力。
...
### [121] [VDCook:DIY video data cook your MLLMs](https://arxiv.org/abs/2603.05539)
*Chengwei Wu*

Main category: cs.LG

TL;DR: VDCook是一个自演进的视频数据操作系统，通过自然语言查询和参数配置，自动执行查询优化、视频检索和可控合成，生成带有完整溯源和元数据的领域数据包。


<details>
  <summary>Details</summary>
Motivation: 传统静态数据集构建困难且无法持续更新，研究者需要更灵活、可扩展的视频数据构建平台来降低专业视频训练数据集的构建门槛。

Method: 系统基于自然语言查询和可调参数（规模、检索-合成比例、质量阈值），自动进行查询优化，并行运行真实视频检索和可控合成模块，生成带有完整溯源和元数据的领域数据包。

Result: VDCook能够生成包含多维元数据标注（场景分割、运动评分、OCR比例、自动字幕等）的数据包，支持持续更新和领域扩展，将数据集转变为动态演进的开放生态系统。

Conclusion: 该平台通过基础设施级解决方案显著降低了构建专业视频训练数据集的门槛，同时支持社区贡献和治理驱动的数据扩展范式，为视频数据研究提供了新的工具。
...
### [122] [IntSeqBERT: Learning Arithmetic Structure in OEIS via Modulo-Spectrum Embeddings](https://arxiv.org/abs/2603.05556)
*Kazuhisa Nakasho*
...
<details>
  <summary>Details</summary>
Motivation: OEIS整数序列包含从个位数到天文数字的数值范围，传统token化模型难以处理超出词汇表的数值，也无法利用周期性算术结构

Method: 提出IntSeqBERT双流Transformer编码器：1）连续对数尺度幅度嵌入；2）100个模数（2-101）的正弦/余弦模数嵌入；通过FiLM融合；三个预测头联合训练：幅度回归、符号分类和100个模数预测

Result: 大型模型（9150万参数）在测试集上达到95.85%幅度准确率和50.38%平均模数准确率，比token化Transformer基线分别提升8.9和4.5个百分点；CRT求解器使下一项预测提升7.4倍（Top-1：19.09% vs 2.59%）

Conclusion: 模数流对性能提升至关重要，模数谱分析显示复合模数通过CRT聚合能更高效地捕捉OEIS算术结构，为整数序列建模提供了新范式
...
### [123] [Autocorrelation effects in a stochastic-process model for decision making via time series](https://arxiv.org/abs/2603.05559)
*Tomoki Yamagami,Mikio Hasegawa,Takatomo Mihana,Ryoichi Horisaki,Atsushi Uchida*

Main category: cs.LG

TL;DR: 该研究分析了基于时间序列的决策模型，发现时间序列的自相关特性对解决双臂赌博机问题的性能有重要影响，且最优自相关方向取决于环境奖励丰富程度。


<details>
  <summary>Details</summary>
Motivation: 先前实验表明基于半导体激光器混沌动力学的决策系统中，时间序列的自相关特性显著影响决策准确性，但缺乏最小化数学模型来解释这种自相关优势。

Method: 采用基于时间序列决策的随机过程模型，结合拔河原理解决双臂赌博机问题，模型中阈值和二元马尔可夫信号共同演化，通过数值模拟分析自相关效应。

Result: 数值结果显示环境依赖结构：在奖励丰富的环境中负自相关最优，在奖励贫乏的环境中正自相关最优；当获胜概率之和等于1时，性能与自相关无关。

Conclusion: 该研究阐明了时间序列自相关在决策中的数学机制，为改进无线通信和机器人强化学习应用的决策方案铺平了道路。
...
### [124] [Towards Efficient and Stable Ocean State Forecasting: A Continuous-Time Koopman Approach](https://arxiv.org/abs/2603.05560)
*Rares Grozavescu,Pengyu Zhang,Mark Girolami,Etienne Meunier*

Main category: cs.LG

TL;DR: CT-KAE作为轻量级替代模型用于长期海洋状态预测，通过将非线性动力学投影到线性ODE控制的潜在空间，实现结构化、可解释的时间演化，相比自回归Transformer基线表现出更好的长期稳定性。


<details>
  <summary>Details</summary>
Motivation: 开发高效稳定的混合物理-机器学习气候模型，解决传统数值求解器计算成本高的问题，同时避免自回归模型在长期预测中的误差放大和能量漂移问题。
...
Conclusion: 连续时间Koopman替代模型为高效稳定的混合物理-机器学习气候模型提供了一个有前景的基础架构，尽管小尺度湍流结构部分耗散，但大尺度统计特性保持良好。
...
### [125] [When AI Levels the Playing Field: Skill Homogenization, Asset Concentration, and Two Regimes of Inequality](https://arxiv.org/abs/2603.05565)
*Xupeng Chen,Shuchen Meng*

Main category: cs.LG

TL;DR: 生成式AI在任务层面缩小技能差异，但通过向集中互补资产转移经济价值，可能加剧总体不平等，形成个体平等与总体不平等的悖论。


<details>
  <summary>Details</summary>
Motivation: 研究生成式AI如何通过压缩任务内技能差异同时向集中互补资产转移价值，揭示个体层面平等与总体层面不平等之间的张力，为理解AI对劳动力市场不平等的影响提供理论框架。

Method: 建立包含内生教育、雇主筛选和异质性企业的任务基础模型，通过模拟矩方法匹配六个经验目标进行情景分析，使用BLS OEWS数据（2019-2023）进行职业层面回归。

Result: 模型识别出两个制度边界取决于AI技术结构（专有vs商品）和劳动力市场制度；校准参数显示总体不平等方向由特定参数决定，而AI技术结构独立跨越边界；职业层面数据无法检验任务层面预测。

Conclusion: 主要贡献是揭示机制而非确定不平等方向；预测可通过任务内、职业内面板数据检验，但目前此类大规模数据尚不存在。
...
### [126] [Aligning the True Semantics: Constrained Decoupling and Distribution Sampling for Cross-Modal Alignment](https://arxiv.org/abs/2603.05566)
*Xiang Ma,Lexin Fang,Litian Xu,Caiming Zhang*

Main category: cs.LG

TL;DR: 提出CDDS算法，通过约束解耦和分布采样实现跨模态对齐，解决传统方法忽略嵌入中非语义信息的问题，在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 跨模态对齐需要实现视觉和语言之间的语义一致性。传统方法追求嵌入一致性来实现语义一致性，但忽略了嵌入中的非语义信息。直观方法是将嵌入解耦为语义和模态组件，仅对齐语义组件，但这面临两个挑战：1) 没有区分语义和模态信息的标准；2) 模态差距可能导致语义对齐偏差或信息丢失。

Method: 提出CDDS算法：1) 引入双路径UNet自适应解耦嵌入，应用多重约束确保有效分离；2) 提出分布采样方法弥合模态差距，确保对齐过程的合理性。

Result: 在多个基准测试和模型骨干上的广泛实验表明CDDS的优越性，比最先进方法高出6.6%到14.2%。

Conclusion: CDDS算法通过约束解耦和分布采样有效解决了跨模态对齐中的语义信息分离和模态差距问题，显著提升了跨模态对齐性能。
...
### [127] [Why Depth Matters in Parallelizable Sequence Models: A Lie Algebraic View](https://arxiv.org/abs/2603.05573)
*Gyuryang Heo,Timothy Ngotiaoco,Kazuki Irie,Samuel J. Gershman,Bernardo Sabatini*

Main category: cs.LG

TL;DR: 论文从李代数控制角度分析序列模型的表达能力边界，建立了模型深度与李代数扩展塔的对应关系，证明了深度增加时近似误差呈指数级下降。


<details>
  <summary>Details</summary>
Motivation: 当前可扩展的序列模型（如Transformer变体和结构化状态空间模型）通常在表达能力与序列级并行性之间进行权衡。本文旨在从李代数控制的角度，研究模型在其表达能力范围之外操作时的误差边界和缩放规律。

Method: 采用李代数控制理论视角，建立序列模型深度与李代数扩展塔之间的对应关系。通过理论分析推导近似误差边界，并在符号词和连续值状态跟踪问题上进行实验验证。

Result: 理论分析表明，随着模型深度增加，近似误差呈指数级下降，这与这些模型的强大实证性能一致。实验验证了理论预测的正确性。

Conclusion: 从李代数控制角度为序列模型提供了理论框架，揭示了模型深度与表达能力之间的数学关系，为理解现代序列模型的性能提供了理论基础。
...
### [128] [A Novel Hybrid Heuristic-Reinforcement Learning Optimization Approach for a Class of Railcar Shunting Problems](https://arxiv.org/abs/2603.05579)
*Ruonan Zhao,Joseph Geunes*

Main category: cs.LG

TL;DR: 论文提出了一种混合启发式-强化学习框架，用于解决铁路货场中具有双侧分类轨道访问的双机车编组问题，通过问题分解和状态-动作空间缩减来提高求解效率。


<details>
  <summary>Details</summary>
Motivation: 铁路货场编组是货运场站的核心规划任务，传统单侧轨道采用LIFO堆栈结构，而双侧轨道采用FIFO队列结构。现有方法难以有效解决具有双侧访问和双机车的复杂编组问题，需要新的求解框架。

Method: 将双侧访问问题分解为两个单侧访问子问题，每个子问题对应一侧的机车。提出混合启发式-强化学习框架，结合铁路专用启发式方法和Q-learning强化学习算法，通过状态-动作空间缩减和引导探索来优化求解过程。

Result: 数值实验表明，HHRL算法在单侧访问单机车问题和双侧访问双机车问题中均表现出良好的效率和求解质量，验证了该框架的有效性。

Conclusion: 混合启发式-强化学习框架能够有效解决铁路货场中复杂的双侧轨道访问编组问题，通过问题分解和智能算法结合，为实际铁路运营提供了实用的优化解决方案。
...
### [129] [On the Value of Tokeniser Pretraining in Physics Foundation Models](https://arxiv.org/abs/2603.05598)
*Hadi Sotoudeh,Payel Mukhopadhyay,Ruben Ohana,Michael McCabe,Neil D. Lawrence,Shirley Ho,Miles Cranmer*

Main category: cs.LG

TL;DR: 研究tokenizer预训练对物理仿真模型精度和效率的影响，发现同领域预训练能显著提升性能，并提出可调节时空压缩方法。


<details>
  <summary>Details</summary>
Motivation: 现代高分辨率物理模拟产生海量数据，训练基础模型学习其动力学有助于建模复杂多物理现象。现有物理基础模型通常同时学习两个任务：提取高分辨率时空数据的紧凑表示和捕获物理动力学，但这种同时学习可能影响两个任务的效果。

Method: 提出在训练动力学模型之前，使用自编码目标预训练tokenizer；引入灵活的时空压缩操作，扩展因果卷积以支持运行时可调节的压缩比，适应不同下游任务。

Result: 同领域预训练（相同物理系统）带来最大改进，10,500训练步后VRMSE降低64%；其他领域预训练提供中等增益。这是首次系统研究物理基础模型的tokenizer预训练。

Conclusion: tokenizer预训练能显著提升物理仿真器的计算效率，预训练数据的选择策略至关重要。提出的灵活压缩方法支持高效适应多样化下游任务。
...
### [130] [Identifying Adversary Characteristics from an Observed Attack](https://arxiv.org/abs/2603.05625)
*Soyon Choi,Scott Alfeld,Meiyi Ma*

Main category: cs.LG

TL;DR: 本文提出了一种防御机器学习系统对抗攻击的新框架，重点不是防御攻击本身，而是识别攻击者的特征，从而更有效地进行防御。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在自动化决策系统中容易受到数据操纵攻击。现有的防御机制要么直接影响ML模型（如对抗正则化），要么在更广泛的系统中发挥作用（如异常检测）。本文提出了一种不同的防御思路：关注攻击者本身而非攻击行为，通过识别攻击者特征来增强防御能力。

Method: 提出了一个领域无关的框架来识别最可能的攻击者。该框架首先证明了在没有额外知识的情况下，攻击者是不可识别的（多个潜在攻击者可能执行相同的攻击）。为了解决这一挑战，框架通过分析观察到的攻击来推断攻击者特征，帮助防御者采取针对性措施。

Result: 框架在多种学习器上进行了具体实例化，展示了其适用性。通过识别攻击者特征，防御者可以：1）进行外源性缓解（通过改变决策系统或限制攻击者能力来应对漏洞）；2）在实施直接影响学习过程的防御方法时，了解特定攻击者可以提高性能。

Conclusion: 关注攻击者而非攻击本身是一种有效的防御策略。提出的框架能够识别攻击者特征，为防御者提供更精准的防御手段，既可以通过系统层面的调整进行外源性缓解，也可以优化基于学习的防御方法。
...
### [131] [The Value of Graph-based Encoding in NBA Salary Prediction](https://arxiv.org/abs/2603.05671)
*Junhao Su,David Grimsman,Christopher Archibald*

Main category: cs.LG

TL;DR: 通过构建知识图谱并嵌入向量空间来改进NBA球员薪资预测，特别是针对老将和高薪球员


<details>
  <summary>Details</summary>
Motivation: 传统基于表格数据的监督学习方法在预测NBA球员薪资时存在局限性，特别是对于老将和高薪球员，因为这些球员的薪资不仅取决于场上表现，还受到多种场外因素的影响。

Method: 构建包含场上和场外数据的知识图谱，将图谱嵌入向量空间，然后将这些向量特征整合到传统的表格数据中，使用监督学习进行薪资预测，并比较了多种图嵌入算法。

Result: 研究表明，将知识图谱嵌入向量空间并整合到表格数据中，能够显著提升NBA球员薪资预测的准确性，特别是对于传统方法难以准确预测的老将和高薪球员群体。

Conclusion: 知识图谱嵌入方法是NBA薪资预测的关键技术，能够更好地捕捉影响球员薪资的复杂因素，弥补了传统表格数据方法的不足。
...
### [132] [Improved Scaling Laws via Weak-to-Strong Generalization in Random Feature Ridge Regression](https://arxiv.org/abs/2603.05691)
*Diyuan Wu,Lehan Chen,Theodor Misiakiewicz,Marco Mondelli*

Main category: cs.LG

TL;DR: 该论文研究了机器学习中弱到强泛化现象，发现通过弱教师模型标注数据训练强学生模型可以显著提升性能，甚至改变测试误差的缩放规律。


<details>
  <summary>Details</summary>
Motivation: 研究机器学习中常见的两阶段训练过程：使用学习模型标注数据，然后用这些数据训练更强大的模型。特别关注弱到强泛化现象，即强学生模型在弱教师模型提供的非完美标签上训练后，性能反而超过弱教师模型。

Method: 采用随机特征岭回归（RFRR）框架，推导出学生模型在教师模型提供的标签上训练时的测试误差的确定性等价表达式。通过这个数学工具分析不同参数区域下的性能表现。

Result: 研究发现学生模型的性能提升潜力很大，能够改变测试误差的缩放规律。在偏置主导和方差主导的设置中都能实现改进。最引人注目的是，即使教师模型的测试误差不随样本量衰减，学生模型仍能达到极小极大最优速率。

Conclusion: 弱到强泛化现象具有重要的理论和实践意义，通过两阶段训练过程，学生模型可以超越教师模型的性能极限，甚至达到理论最优性能，这为机器学习中的模型训练策略提供了新的理论支持。
...
### [133] [Revisiting the (Sub)Optimality of Best-of-N for Inference-Time Alignment](https://arxiv.org/abs/2603.05739)
...
<details>
  <summary>Details</summary>
Motivation: 尽管Best-of-N采样在实践中被广泛使用，但近期理论研究表明它在统计上不是最优的且容易受到奖励攻击。本文旨在重新评估BoN在实际条件下的表现，特别是考虑胜率这一更符合实践的评价指标。

Method: 在更贴近实际条件的假设下分析BoN方法，考虑胜率而非期望奖励作为评价指标。提出了一个简单的BoN变体来消除奖励攻击问题，同时保持最优统计性能。

Result: 在参考模型和奖励模型质量满足最小条件的情况下，适当调优的BoN在实现高胜率方面既是计算最优也是统计最优的。提出的变体能够消除奖励攻击问题，而先前的方法在考虑胜率时被证明是次优的。

Conclusion: BoN的实际成功部分可以通过其在胜率方面的最优性来解释。选择合适的分析目标对于评估推理时对齐方法至关重要，本文提出的BoN变体能够消除奖励攻击问题同时保持最优性能。
...
### [134] [MIRACL: A Diverse Meta-Reinforcement Learning for Multi-Objective Multi-Echelon Combinatorial Supply Chain Optimisation](https://arxiv.org/abs/2603.05760)
...
<details>
  <summary>Details</summary>
Motivation: 多目标强化学习在多梯队组合供应链优化中有效，但在动态环境中部署面临任务特定重训练和计算成本高的问题，需要能够少样本泛化的解决方案。

Method: 提出MIRACL框架：1) 将任务分解为结构化子问题进行高效策略适应；2) 使用帕累托适应策略跨任务元学习全局策略，促进元训练和微调中的多样性。

Result: 在简单到中等任务中，MIRACL优于传统多目标强化学习方法，实现了高达10%的超体积提升和5%的期望效用改善。

Conclusion: MIRACL展示了在多目标问题中实现鲁棒、高效适应的潜力，虽然是针对供应链领域验证的，但理论上与领域无关，适用于更广泛的动态多目标决策问题。
...
### [135] [Score-Guided Proximal Projection: A Unified Geometric Framework for Rectified Flow Editing](https://arxiv.org/abs/2603.05761)
*Vansh Bansal,James G Scott*

Main category: cs.LG

TL;DR: SGPP框架通过近端投影统一确定性优化与随机采样，解决Rectified Flow模型的控制问题，在保持输入保真度与生成真实性之间提供连续权衡。


<details>
  <summary>Details</summary>
Motivation: Rectified Flow模型虽然生成质量优秀，但在精确控制任务（如语义编辑、盲图像恢复）方面存在挑战。现有方法要么受限于"几何锁定"问题，要么计算成本高且不稳定。

Method: 提出Score-Guided Proximal Projection (SGPP)框架，将恢复任务重新表述为近端优化问题，定义平衡输入保真度与预训练评分场真实性的能量景观。该方法具有正常收缩特性，能将分布外输入映射到数据流形上。

Result: SGPP能有效将分布外输入映射到数据流形，达到流形约束下的后验模式。该方法推广了现有编辑方法，RF反转是其极限情况。通过调整近端方差，SGPP实现"软引导"，在严格身份保持与生成自由度之间提供连续权衡。

Conclusion: SGPP提供了一个统一框架，弥合了确定性优化与随机采样之间的差距，解决了Rectified Flow模型的控制问题，为精确生成任务提供了灵活的训练免费解决方案。
...
### [136] [Bridging Domains through Subspace-Aware Model Merging](https://arxiv.org/abs/2603.05768)
...
<details>
  <summary>Details</summary>
Motivation: 当前模型合并研究主要关注同分布或多任务场景，但领域泛化在模型合并中尚未充分探索。研究旨在探究在不同领域微调的模型合并后对未见领域的泛化能力

Method: 提出SCORE方法：通过计算所有模型主要奇异向量连接的主成分来找到共享正交基，然后将每个任务矩阵投影到共享基中，修剪非对角分量以移除冲突的奇异方向

Result: SCORE在领域泛化设置中平均优于现有模型合并方法，在多种架构和模型规模上表现一致，证明了其有效性和可扩展性

Conclusion: 模型合并中的领域泛化面临子空间冲突挑战，SCORE方法通过解决这些冲突显著提升了合并模型在未见领域的泛化能力
...
### [137] [First-Order Softmax Weighted Switching Gradient Method for Distributed Stochastic Minimax Optimization with Stochastic Constraints](https://arxiv.org/abs/2603.05774)
*Zhankun Luo,Antesh Upadhyay,Sang Bin Moon,Abolfazl Hashemi*

Main category: cs.LG

TL;DR: 提出一种用于联邦学习的Softmax-Weighted Switching Gradient方法，解决带随机约束的分布式随机极小极大优化问题，在完全参与和部分参与情况下都提供了理论保证。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中的分布式随机极小极大优化问题，特别是带有随机约束的情况。传统的主-对偶或基于惩罚的方法存在超参数敏感性和收敛振荡问题，需要更稳定的优化方法。

Method: 提出新颖的Softmax-Weighted Switching Gradient方法，这是一种单循环仅主变量切换机制。通过软最大加权和切换梯度策略，优化最坏情况客户端性能，避免传统方法的超参数敏感性问题。

Result: 在完全客户端参与下，算法达到标准的$\mathcal{O}(ε^{-4})$预言机复杂度，满足最优性间隙和可行性容忍度的统一界限$ε$。在部分参与情况下，通过量化客户端采样噪声建立理论分析。放松目标函数有界性假设后，建立了更严格的软最大超参数下界。

Conclusion: 单循环仅主变量切换机制为优化最坏情况客户端性能提供了稳定替代方案，有效避免了传统主-对偶或基于惩罚方法的超参数敏感性和收敛振荡问题。在Neyman-Pearson分类和公平分类任务上验证了算法的有效性。
...
### [138] [Sparse Crosscoders for diffing MoEs and Dense models](https://arxiv.org/abs/2603.05805)
*Marmik Chaudhari,Nishkal Hundia,Idhant Gulati*

Main category: cs.LG

TL;DR: MoE模型通过稀疏专家路由实现参数高效扩展，但其内部表示相比密集模型仍不明确。研究使用crosscoders（稀疏自编码器变体）系统比较MoE和密集模型的内部表示，发现MoE学习更少的独特特征，具有更专门化的表示结构。
...
Method: 使用crosscoders（稀疏自编码器变体）联合建模多个激活空间，训练5层密集模型和MoE模型（具有相等的活跃参数），在10亿个标记的数据集（代码、科学文本、英语故事）上进行训练。采用BatchTopK crosscoders并明确指定共享特征，实现约87%的分数方差解释。

Result: MoE模型学习的独特特征数量显著少于密集模型。MoE特定特征比共享特征具有更高的激活密度，而密集模型特定特征则显示较低的激活密度。分析表明，MoE发展出更专门化、更集中的表示，而密集模型则在更广泛、更通用的特征间分布信息。

Conclusion: MoE模型通过稀疏专家路由形成了更专门化的特征表示结构，学习更少的独特特征但具有更高的激活密度，而密集模型则采用更广泛、更通用的特征分布策略。这揭示了两种架构在表示学习上的根本差异。
...
### [139] [MoE Lens -- An Expert Is All You Need](https://arxiv.org/abs/2603.05806)
*Marmik Chaudhari,Idhant Gulati,Nishkal Hundia,Pranav Karra,Shivam Raval*

Main category: cs.LG

TL;DR: MoE模型通过稀疏专家激活实现参数高效扩展，但优化推理和内存成本仍具挑战。研究发现DeepSeekMoE模型主要依赖少数专业化专家，单个顶级专家的输出接近完整集成预测，为推理优化和专家剪枝提供了机会。


<details>
  <summary>Details</summary>
Motivation: MoE模型通过稀疏专家激活实现参数高效扩展，但对其专业化行为的理解有限，导致优化推理和内存成本面临挑战。需要系统分析专家专业化模式以发现优化机会。

Method: 采用两种互补方法：1) 领域特定路由模式分析；2) 早期解码框架追踪专家对输出表示的贡献。对DeepSeekMoE模型进行系统分析，包括令牌路由分布、隐藏状态相似性等定量验证。

Result: 研究发现：1) 尽管有64个路由专家（每层激活6个），模型主要依赖少数专业化专家；2) 顶级专家的输出与完整集成预测高度接近；3) 极少专家处理超过50%的路由决策；4) 单专家与集成专家的隐藏状态相似度极高（某些层余弦相似度达0.95）；5) 在所有三个领域使用单个专家时困惑度仅增加5%。
...
### [140] [Self-Auditing Parameter-Efficient Fine-Tuning for Few-Shot 3D Medical Image Segmentation](https://arxiv.org/abs/2603.05822)
*Son Thai Ly,Hien V. Nguyen*

Main category: cs.LG

TL;DR: SEA-PEFT是一种自动化参数高效微调方法，通过搜索-审计-分配循环自动配置适配器，在少量样本的3D医学图像分割任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 临床实践中，将基础模型适应到新医疗站点面临挑战：领域偏移、标注稀缺，且许多临床团队缺乏AI工程师来调整适配器设计和训练方案，导致适应周期长达数周到数月。现有PEFT方法要么需要手动配置适配器，要么自动化搜索在少量样本的3D设置中计算不可行。

Method: SEA-PEFT将适配器配置视为在线分配问题，在微调过程中通过搜索-审计-分配循环解决：训练活动适配器，通过暂时关闭每个适配器来估计其Dice效用，然后使用贪心背包分配器在参数预算下重新选择活动集。采用指数移动平均和四分位距平滑以及有限状态排名控制器来稳定循环，提高高噪声少量样本场景的可靠性。

Result: 在TotalSegmentator和FLARE'22数据集上，SEA-PEFT在1/5/10-shot设置中，相比最强的固定拓扑PEFT基线，平均Dice提升了2.4-2.8个点，同时训练参数少于1%。

Conclusion: SEA-PEFT通过自动化适配器配置过程，解决了临床实践中基础模型适应的关键挑战，在少量样本的3D医学图像分割任务中实现了显著性能提升，同时保持参数效率。
...
### [141] [Test-Time Adaptation via Many-Shot Prompting: Benefits, Limits, and Pitfalls](https://arxiv.org/abs/2603.05829)
*Shubhangi Upasani,Chen Wu,Jay Rainton,Bo Li,Changran Hu,Qizheng Zhang,Urmish Thakker*

Main category: cs.LG

TL;DR: 该研究实证分析了大型语言模型在测试时通过多示例提示进行适应的效果、局限性和可靠性，比较了不同更新策略在不同任务类型上的表现。


<details>
  <summary>Details</summary>
Motivation: 虽然多示例提示能够通过注入大量上下文学习示例来改进LLMs在推理时的性能，但这种更新机制的可靠性和局限性尚未得到充分理解，特别是对于开源模型。

Method: 通过实证研究分析多示例提示在不同任务和模型架构上的表现，考察更新幅度、示例顺序和选择策略的影响，并研究动态ICL和强化ICL作为替代的测试时更新策略。

Result: 多示例提示在结构化任务中效果显著（演示提供高信息增益），但对选择策略高度敏感，在开放生成任务中效果有限。不同更新策略在不同任务类型上表现差异明显。

Conclusion: 研究揭示了基于提示的测试时适应的实际局限性，明确了输入空间更新在何种情况下有益或有害，为实际应用提供了指导。
...
### [142] [Reference-guided Policy Optimization for Molecular Optimization via LLM Reasoning](https://arxiv.org/abs/2603.05900)
...
<details>
  <summary>Details</summary>
Motivation: 在基于指令的分子优化任务中，每个数据点通常只提供一个优化的参考分子，没有逐步优化的轨迹数据。传统的监督微调（SFT）和带可验证奖励的强化学习（RLVR）在这种设置下表现不佳：SFT会导致推理能力崩溃，而RLVR由于模型缺乏有效探索而提供稀疏反馈，限制了优化效果。

Method: 提出了Reference-guided Policy Optimization (RePO)方法，该方法不需要轨迹数据。在每次更新时，RePO从模型中采样候选分子及其中间推理轨迹，使用可验证奖励（衡量在相似性约束下的属性满足程度）以强化学习方式训练模型。同时，它通过将策略的中间推理轨迹作为上下文，仅以监督方式训练答案部分来应用参考指导。RL项促进探索，而指导项通过将输出锚定到参考分子来缓解奖励稀疏性并稳定训练。

Result: 在分子优化基准测试中，RePO始终优于SFT和RLVR基线（如GRPO），在优化指标（成功率×相似性）上实现了改进，更好地平衡了竞争目标，并且对未见过的指令风格具有更好的泛化能力。

Conclusion: RePO通过结合强化学习的探索能力和参考指导的稳定性，有效解决了分子优化中缺乏轨迹数据的问题，为指令驱动的分子设计提供了一种有效的优化框架。
...
### [143] [Stock Market Prediction Using Node Transformer Architecture Integrated with BERT Sentiment Analysis](https://arxiv.org/abs/2603.05917)
*Mohammad Al Ridhawi,Mahtab Haj Ali,Hussein Al Osman*

Main category: cs.LG

TL;DR: 本文提出了一种结合节点Transformer架构和BERT情感分析的集成框架用于股价预测，将股市建模为图结构，在20只标普500股票上实现了0.80%的MAPE，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 股市预测面临噪声、非平稳性和行为动态等挑战，传统方法难以捕捉金融市场中的复杂模式和跨截面依赖关系。

Method: 提出集成框架：1) 将股市表示为图结构（股票为节点，边捕捉行业关联、价格相关性和供应链连接）；2) 使用微调BERT模型从社交媒体提取情感信息；3) 通过注意力机制融合情感与量化市场特征；4) 节点Transformer处理历史市场数据，捕捉时间演化和跨截面依赖。

Result: 在1982年1月至2025年3月的20只标普500股票上：1) 集成模型MAPE为0.80%（ARIMA 1.20%，LSTM 1.00%）；2) 情感分析使预测误差降低10%（财报季降低25%）；3) 图建模贡献额外15%改进；4) 方向准确率达65%；5) 高波动期MAPE保持在1.5%以下（基准模型超过2%）。

Conclusion: 集成节点Transformer和BERT情感分析的框架显著提升了股价预测精度，有效捕捉了市场复杂性和跨资产依赖关系，为金融预测提供了新方法。
...
### [144] [Design Experiments to Compare Multi-armed Bandit Algorithms](https://arxiv.org/abs/2603.05919)
...
Method: 提出Artificial Replay（AR）实验设计：首先运行一个策略并记录其轨迹；当执行第二个策略时，如果选择第一个策略已经采取过的动作，则重用记录中的奖励，否则查询真实环境。开发新的分析框架证明该设计的三个关键特性。

Result: AR估计器具有无偏性；只需要T+o(T)次用户交互（而不是2T），当两个策略都有亚线性遗憾时几乎减半实验成本；其方差增长是亚线性的，而朴素设计的估计器方差是线性增长的。在UCB、Thompson Sampling和ε-greedy策略上的数值实验证实了这些理论优势。

Conclusion: Artificial Replay为在线平台比较多臂老虎机算法提供了一种高效、低成本的实验设计方法，显著减少实验成本并加速部署决策，同时保持统计可靠性。
...
### [145] [Omni-Masked Gradient Descent: Memory-Efficient Optimization via Mask Traversal with Improved Convergence](https://arxiv.org/abs/2603.05960)
*Hui Yang,Tao Ren,Jinyang Jiang,Wan Tian,Yijie Peng*

Main category: cs.LG

TL;DR: OMGD是一种基于掩码遍历的内存高效优化方法，在非凸设置下实现了改进的迭代复杂度$\tilde{\mathcal{O}}(ε^{-3})$，比标准方法更好，并能无缝集成到主流优化器中。


<details>
  <summary>Details</summary>
Motivation: 在GPU内存瓶颈下扩展大语言模型的全参数训练时，现有内存高效优化方法要么缺乏清晰的收敛保证，要么在非凸设置下只能达到标准的${\mathcal{O}}(ε^{-4})$迭代复杂度，需要更好的方法。

Method: 提出了Omni-Masked Gradient Descent (OMGD)，这是一种基于掩码遍历的内存高效训练优化方法。该方法轻量级、即插即用，可以无缝集成到大多数主流优化器中。

Result: 理论分析表明OMGD在非凸设置下实现了$\tilde{\mathcal{O}}(ε^{-3})$的改进迭代复杂度，用于找到$ε$-近似驻点。实证结果显示，在微调和预训练任务中，OMGD相比竞争基线都取得了持续改进。

Conclusion: OMGD是一种有效的内存高效优化方法，具有理论保证和实际性能优势，能够在大语言模型训练中解决GPU内存瓶颈问题。
...
### [146] [Preventing Learning Stagnation in PPO by Scaling to 1 Million Parallel Environments](https://arxiv.org/abs/2603.06009)
*Michael Beukman,Khimya Khetarpal,Zeyu Zheng,Will Dabney,Jakob Foerster,Michael Dennis,Clare Lyle*

Main category: cs.LG

TL;DR: PPO算法在深度在线强化学习中常出现性能停滞问题，研究发现这是由于样本估计损失逐渐偏离真实目标导致的。通过将PPO外循环建模为随机优化，发现可以通过减小步长或增加采样数量来解决停滞问题。实验表明增加并行环境数量是有效方法，并提出了相应的超参数缩放方案。


<details>
  <summary>Details</summary>
Motivation: PPO作为广泛采用的深度在线强化学习算法，在训练中常出现性能停滞在次优水平的问题。传统认为这是由探索、容量或优化挑战导致的，但本文研究发现，在某些情况下，性能停滞是因为基于样本的损失估计在训练过程中逐渐偏离真实目标。

Method: 将PPO的外循环（从并行环境采样）建模为随机优化过程。步长由对先前策略的正则化强度控制，梯度噪声由策略更新步骤之间收集的样本数量决定。通过分析超参数选择如何影响步长和更新噪声，提出增加并行环境数量来减少这两个因素。同时设计了超参数协同缩放方案。

Result: 验证了模型预测：当外循环步长相对于噪声过大时，性能会停滞在次优水平。增加并行环境数量能有效减少步长和噪声，从而避免性能停滞。在复杂开放领域中将PPO扩展到超过100万个并行环境，实现了单调性能提升，达到一万亿次转移。

Conclusion: PPO性能停滞问题可以通过将外循环视为随机优化过程来理解。通过适当增加并行环境数量并协同调整超参数，可以有效解决性能停滞问题，实现持续的性能提升。这为大规模强化学习训练提供了实用的指导方案。
...
### [147] [Agnostic learning in (almost) optimal time via Gaussian surface area](https://arxiv.org/abs/2603.06027)
*Lucas Pesenti,Lucas Slot,Manuel Wiedmer*

Main category: cs.LG

TL;DR: 本文改进了Klivans等人(2008)关于高斯分布下概念类学习复杂度的分析，将所需多项式次数从O(Γ²/ε⁴)降低到Õ(Γ²/ε²)，在统计查询模型中实现了多项式阈值函数近似学习的最优边界。


<details>
  <summary>Details</summary>
Motivation: 高斯分布下概念类的难学习复杂度与其低阶多项式的L₁近似能力密切相关。Klivans等人(2008)证明了对于高斯表面积至多为Γ的概念类，需要O(Γ²/ε⁴)次多项式才能达到ε近似，这为多种概念类的学习复杂度提供了已知最佳边界。本文旨在改进这一分析。

Method: 本文改进了Klivans等人的分析，证明仅需Õ(Γ²/ε²)次多项式即可达到ε近似。证明方法借鉴了Feldman等人(2020)在布尔超立方体上L₁近似构造的直接类比。

Result: 将所需多项式次数从O(Γ²/ε⁴)显著降低到Õ(Γ²/ε²)，结合Diakonikolas等人(2021)的下界结果，在统计查询模型中实现了多项式阈值函数难学习复杂度的（近似）最优边界。

Conclusion: 本文改进了高斯分布下概念类难学习复杂度的分析，将多项式近似所需次数降低了平方因子，为多项式阈值函数在统计查询模型中的学习复杂度提供了接近最优的边界。
...
### [148] [Improved high-dimensional estimation with Langevin dynamics and stochastic weight averaging](https://arxiv.org/abs/2603.06028)
*Stanley Wei,Alex Damian,Jason D. Lee*

Main category: cs.LG

TL;DR: 论文研究了Langevin动力学通过迭代平均而非最终迭代，能够在n≳d^{k*/2}样本量下恢复隐藏方向，这一结果与显式平滑方法相当，且适用于张量PCA和单指标模型。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明，在线SGD需要n≳d^{max(1,k*-1)}样本才能恢复隐藏方向θ*，而显式平滑方法可将样本需求降至n≳d^{max(1,k*/2)}。本文旨在探索是否能在不显式平滑的情况下达到相同效果。

Method: 采用Langevin动力学（噪声注入）结合迭代平均的方法。关键思想是噪声注入和迭代平均的组合能够模拟景观平滑的效果，从而降低样本复杂度要求。

Result: 证明Langevin动力学通过迭代平均（而非最终迭代）能够在n≳d^{k*/2}样本量下成功恢复隐藏方向θ*，这一结果与显式平滑方法相当，且适用于张量PCA和单指标模型。

Conclusion: 噪声注入与迭代平均的组合能够有效模拟景观平滑效果，使Langevin动力学在不显式平滑的情况下达到与平滑方法相当的样本效率。同时推测小批量SGD也能在不添加额外噪声的情况下达到相同效果。
...
### [149] [Dynamic Momentum Recalibration in Online Gradient Learning](https://arxiv.org/abs/2603.06120)
...
Method: 提出SGDF（带滤波器的SGD）优化器，基于最优线性滤波原理。通过计算在线、时变增益来动态优化梯度估计，最小化均方误差，在噪声抑制和信号保留间实现最优权衡。
...
### [150] [DQE: A Semantic-Aware Evaluation Metric for Time Series Anomaly Detection](https://arxiv.org/abs/2603.06131)
*Yuewei Li,Dalin Zhang,Huan Li,Xinyi Gong,Hongjun Chu,Zhaohui Song*

Main category: cs.LG

TL;DR: 该论文针对时间序列异常检测的评估问题，提出了一种基于检测语义的新型评估指标，通过分区策略和全阈值谱聚合来解决现有指标的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列异常检测评估指标存在多个局限性：偏向点级覆盖、对近似检测不敏感或不一致、对误报惩罚不足、以及因阈值选择导致的不一致性。这些限制可能导致不可靠或反直觉的结果，阻碍客观进展。

Method: 1. 提出基于检测语义的分区策略，将每个异常的局部时间区域分解为三个功能不同的子区域；2. 基于此分区评估跨事件的整体检测行为，并为每个子区域设计更细粒度的评分机制；3. 通过全阈值谱聚合检测质量，消除评估不一致性。

Result: 在合成和真实世界数据上的大量实验表明，该指标提供了稳定、可区分且可解释的评估，与十种广泛使用的指标相比实现了鲁棒的评估效果。

Conclusion: 该研究从检测语义的角度重新审视时间序列异常检测评估，提出的新指标能够更全面、可靠地评估检测性能，解决了现有评估方法的局限性，为领域提供了更客观的评估框架。
...
### [151] [Partial Policy Gradients for RL in LLMs](https://arxiv.org/abs/2603.06138)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习需要在未知环境中顺序学习行动策略，但现有方法在策略结构建模方面存在不足。作者希望提出一种自然的方法来建模策略梯度中的策略结构，通过优化未来奖励的子集来获得更简单、更可靠的策略。

Method: 提出在策略梯度中优化未来奖励子集的方法：较小的子集代表更简单的策略，其经验梯度估计更准确。该方法能够建模和比较不同的策略类别，包括完全规划、贪婪策略、K步前瞻和分段策略。

Result: 在多个角色对齐对话问题上进行了实证评估，发现不同策略在不同问题上表现出色，反映了它们的不同特性，并突显了所研究策略类别的重要性。

Conclusion: 通过优化未来奖励子集来建模策略结构是有效的，不同策略类别在不同问题中各有优势，强调了考虑策略结构在强化学习中的重要性。
...
### [152] [Topological descriptors of foot clearance gait dynamics improve differential diagnosis of Parkinsonism](https://arxiv.org/abs/2603.06212)
*Jhonathan Barrios,Wolfram Erlhagen,Miguel F. Gago,Estela Bicho,Flora Ferreira*

Main category: cs.LG

TL;DR: 使用拓扑数据分析（TDA）分析足部间隙时间序列，结合机器学习改进帕金森综合征的鉴别诊断


<details>
  <summary>Details</summary>
Motivation: 帕金森综合征的鉴别诊断具有临床挑战性，传统步态分析方法常忽略足部间隙模式中的非线性结构特征，需要更精确的鉴别工具

Method: 使用拓扑数据分析（TDA）处理足部间隙时间序列，生成Betti曲线、持久性景观和轮廓等拓扑特征，结合随机森林分类器进行鉴别诊断

Result: Betti曲线描述符表现最佳，在药物开启状态下，特发性帕金森病与血管性帕金森病的鉴别准确率达83%，AUC=0.89；拓扑特征对左旋多巴相关的步态变化敏感

Conclusion: 拓扑数据分析与机器学习结合可显著改善临床步态分析，有助于帕金森综合征的鉴别诊断，为临床治疗规划提供支持
...
### [153] [FedSCS-XGB -- Federated Server-centric surrogate XGBoost for continual health monitoring](https://arxiv.org/abs/2603.06224)
...
<details>
  <summary>Details</summary>
Motivation: 脊髓损伤患者面临压疮和血压不稳定等风险，需要连续监测进行早期检测和干预。可穿戴传感器结合本地数据处理能实现这一目标，但需要解决数据分散和隐私保护问题。

Method: 提出受PAX启发的分布式XGBoost协议，明确保留标准XGBoost的关键结构和优化特性，包括基于直方图的分割构建和树集成动态。协议在理论分析基础上进行实证评估。

Result: 理论分析表明在适当数据条件和超参数选择下，分布式协议能收敛到与集中式XGBoost训练等效的解。实证结果显示性能差距小于1%，同时保留XGBoost的结构优势。

Conclusion: 提出的分布式XGBoost协议在可穿戴传感器的人类活动识别中能匹配集中式性能，同时适应远程监测场景的数据异质性和碎片化特点，为医疗监测提供有效的分布式解决方案。
...
### [154] [DC-Merge: Improving Model Merging with Directional Consistency](https://arxiv.org/abs/2603.06242)
...
<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法在保留多个任务知识时面临两个主要问题：1) 任务向量中能量分布不平衡，少数奇异值主导总能量，导致语义重要但较弱的组件在融合时被忽略；2) 参数空间中任务向量的几何不一致性，直接融合会扭曲其底层方向几何。

Method: DC-Merge包含三个步骤：首先通过平滑奇异值平衡每个任务向量的能量分布；然后将这些能量平衡的向量投影到共享正交子空间，以最小重构误差对齐其方向几何；最后在共享正交子空间中聚合对齐的向量，并投影回原始参数空间。

Result: 在视觉和视觉语言基准测试上的大量实验表明，DC-Merge在完全微调和LoRA设置下都一致实现了最先进的性能。
...
### [155] [Gradient Flow Polarizes Softmax Outputs towards Low-Entropy Solutions](https://arxiv.org/abs/2603.06248)
...
Method: 分析value-softmax模型（定义为L(Vσ(a))）的梯度流动态，其中V是可学习的值矩阵，a是注意力向量。研究在各种目标函数（包括逻辑损失和平方损失）下的优化行为。

Result: 梯度流在这种结构上会固有地驱动优化朝着低熵输出的解决方案发展。这种极化效应在各种目标函数中都具有普遍性，为注意力汇聚和大规模激活等经验现象提供了形式化机制。

Conclusion: 该分析揭示了transformer训练动态的基本特性，为理解softmax模型的经验成功提供了理论依据，并解释了实际观察到的现象如注意力汇聚。
...
### [156] [Learning to Solve Orienteering Problem with Time Windows and Variable Profits](https://arxiv.org/abs/2603.06260)
*Songqun Gao,Zanxi Ruan,Patrick Floor,Marco Roveri,Luigi Palopoli,Daniele Fontanelli*

Main category: cs.LG

TL;DR: DeCoST是一种基于学习的两阶段解耦离散-连续优化方法，用于解决带时间窗和可变利润的定向问题，通过分离离散和连续变量并实现高效协调，显著提升了求解质量和计算效率。


<details>
  <summary>Details</summary>
Motivation: 当前方法无法有效解决同时包含离散和连续变量的定向问题变体，而带时间窗和可变利润的定向问题在许多实际应用中很常见，需要开发高效的求解器。

Method: 提出两阶段解耦优化框架：第一阶段使用并行解码结构预测路径和初始服务时间分配；第二阶段通过线性规划优化服务时间，并提供长期结构估计学习。严格证明了第二阶段解的全局最优性。

Result: 在OPTWVP实例上的实验表明，DeCoST在求解质量和计算效率上均优于最先进的构造性求解器和元启发式算法，在少于500个节点的实例上实现了高达6.6倍的推理加速，且与多种构造性求解器兼容。

Conclusion: DeCoST框架有效解决了OPTWVP问题中离散和连续变量的协调问题，提供了高效且可学习的优化方法，在实际应用中具有显著优势。
...
### [157] [Stem: Rethinking Causal Information Flow in Sparse Attention](https://arxiv.org/abs/2603.06274)
*Lin Niu,Xin Luo,Linchuan Xie,Yifu Sun,Guanghua Yu,Jianchen Zhu,S Kevin Zhou*

Main category: cs.LG

TL;DR: Stem提出了一种基于信息流的稀疏注意力模块，通过位置衰减策略和输出感知度量来优化因果注意力计算，减少LLM长上下文预填充的计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 自注意力机制的二次计算复杂度是LLM扩展到长上下文的主要瓶颈，特别是在预填充阶段。现有的稀疏方法通常对所有token位置应用统一的top-k选择，忽略了因果架构中token信息的累积依赖性。

Method: 提出Stem模块，采用两种策略：1) Token Position-Decay策略：在每层应用位置相关的top-k选择，保留初始token以支持递归依赖；2) Output-Aware Metric：基于近似输出幅度优先选择高影响力token。

Result: 广泛评估表明，Stem在减少计算量和预填充延迟的同时，实现了更优的准确性。

Conclusion: Stem是一种即插即用的稀疏模块，通过信息流对齐的方式有效解决了因果注意力计算复杂度过高的问题，为LLM长上下文扩展提供了有效解决方案。
...
### [158] [From Entropy to Calibrated Uncertainty: Training Language Models to Reason About Uncertainty](https://arxiv.org/abs/2603.06317)
*Azza Jenane,Nassim Walha,Lukas Kuhn,Florian Buettner*

Main category: cs.LG

TL;DR: 本文提出了一种三阶段训练流程，通过后训练让大语言模型能够高效地推断其响应的校准不确定性估计，相比传统后处理方法更高效且具有更好的校准性。
...
Method: 三阶段训练流程：1) 在训练数据上计算基于嵌入空间分布变化的细粒度熵不确定性分数；2) 通过Platt缩放校准这些分数，产生可靠且人类可解释的不确定性信号；3) 通过强化学习后训练目标LLM，使其策略与这些校准信号对齐。

Result: 实验表明，使用该流程训练的模型比基线方法具有更好的校准性，并且能够泛化到未见任务而无需进一步处理，表明模型学习了稳健的不确定性推理行为。

Conclusion: 该方法提供了一种可解释且计算高效的测试时不确定性估计方法，相比传统的后处理方法具有显著优势。
...
### [159] [Tiny, Hardware-Independent, Compression-based Classification](https://arxiv.org/abs/2603.06359)
*Charles Meyers,Aaron MacSween,Erik Elmroth,Tommy Löfstedt*

Main category: cs.LG

TL;DR: 本文提出改进归一化压缩距离(NCD)方法，使其适用于客户端机器学习，解决了隐私保护与模型性能的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 随着用户隐私意识增强，客户端数据存储和处理成为趋势，但现有机器学习方法需要大量标注数据且计算成本高，不适合客户端设备。需要开发既保护隐私又能在有限计算资源下有效运行的机器学习方法。

Method: 1) 证明归一化压缩距离(NCD)不是真正的度量；2) 将其扩展到核方法中以处理复杂数据；3) 改进使用该距离度量的模型训练时间；4) 开发适用于客户端设备的小样本学习方法。

Result: NCD方法在分类任务中表现与其他度量和核方法相当甚至更好，计算成本仅略高，且不受缺乏正式度量属性的影响。该方法能在极少样本上训练出高精度模型，适合在客户端设备上运行。

Conclusion: 归一化压缩距离方法为客户端机器学习提供了有效解决方案，在保护用户隐私的同时，实现了在有限计算资源和数据条件下的高性能模型训练，平衡了隐私保护与机器学习性能的需求。
...
### [160] [CLAIRE: Compressed Latent Autoencoder for Industrial Representation and Evaluation -- A Deep Learning Framework for Smart Manufacturing](https://arxiv.org/abs/2603.06361)
*Mohammadhossein Ghahramani,Mengchu Zhou*

Main category: cs.LG

TL;DR: CLAIRE是一个用于高维工业环境故障检测的混合端到端学习框架，结合无监督深度表示学习和有监督分类，通过深度自编码器提取紧凑潜在特征，再用分类器进行故障预测，显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 高维工业环境中传感器数据复杂、噪声多、冗余度高，准确故障检测面临重大挑战，需要更有效的智能质量控制方法。
...
Result: 在高维数据集上的实验结果表明，CLAIRE显著优于直接在原始特征上训练的传统分类器。框架具有模块化和可解释性，可适应其他复杂高维数据领域。

Conclusion: CLAIRE展示了将可解释AI与特征感知正则化相结合进行鲁棒故障检测的潜力，其模块化和可解释性使其在医疗、金融、环境监测等其他复杂高维数据领域具有广泛应用前景。
...
### [161] [Toward Generative Quantum Utility via Correlation-Complexity Map](https://arxiv.org/abs/2603.06440)
*Chen-Yu Liu,Leonardo Placidi,Eric Brunner,Enrico Rinaldi*

Main category: cs.LG

TL;DR: 提出相关性-复杂度图作为诊断工具，通过量子相关性指标和经典相关性复杂度指标判断真实数据分布是否与IQP型量子生成模型结构对齐，并应用于湍流数据建模。


<details>
  <summary>Details</summary>
Motivation: 开发实用诊断工具来确定真实世界数据分布何时与IQP型量子生成模型结构对齐，以推进量子生成模型的实用性和应用领域。

Method: 提出相关性-复杂度图，包含两个互补指标：量子相关性指标（基于数据集相关阶功率谱）和经典相关性复杂度指标（基于最优Chow-Liu树近似未捕获的相关性比例）。使用可逆浮点到比特串表示和潜在参数适应方案，通过紧凑IQP电路学习低维潜在轨迹。

Result: 识别湍流数据为IQP兼容且经典复杂（高QCLI/高CCI）。在比较评估中，IQP方法使用更少的训练快照和小型潜在块，实现了与RBM和DCGAN等经典模型竞争的分布对齐性能。

Conclusion: 相关性-复杂度图可作为实用指标定位IQP对齐领域，支持使用量子相关性指标和经典相关性复杂度指标推进生成量子实用性，为量子生成模型在复杂数据建模中的应用提供指导。
...
### [162] [NOBLE: Accelerating Transformers with Nonlinear Low-Rank Branches](https://arxiv.org/abs/2603.06492)
*Ethan Smith*
...
<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法（如LoRA）主要针对微调阶段，缺乏专门为从头预训练设计的架构增强方法。NOBLE旨在通过添加非线性低秩分支来提升预训练效率，同时保持较低的计算开销。

Method: NOBLE在Transformer线性层中添加永久性非线性低秩分支，计算形式为σ(xWdown)Wup，其中σ为可学习的非线性激活函数。研究发现CosNet（两层余弦非线性，在瓶颈空间具有可学习频率、相位和线性投影）效果最佳。

Result: NOBLE以最小开销实现显著改进：达到基线评估损失的速度提升最高达1.47倍（训练步骤减少32%），仅增加4%参数和7%单步时间开销，净墙钟时间加速最高达1.22倍。在LLM、BERT、VQGAN和ViT上均显示训练效率提升。

Conclusion: NOBLE是一种有效的预训练架构增强方法，能显著提升训练效率。但需注意Mixup/CutMix等随机增强会干扰其在Imagenet分类中的效果，这可能是因为正则化技术鼓励平滑拟合，而NOBLE更专注于目标函数的尖锐特征。
...
### [163] [COLD-Steer: Steering Large Language Models via In-Context One-step Learning Dynamics](https://arxiv.org/abs/2603.06495)
*Kartik Sharma,Rakshit S. Trivedi*

Main category: cs.LG

TL;DR: COLD-Steer是一个无需训练、基于梯度下降近似的方法，可在推理时通过少量样本有效控制大语言模型行为，相比现有方法样本效率提升50倍。


<details>
  <summary>Details</summary>
Motivation: 现有激活引导方法面临基本权衡：样本效率高的方法无法充分捕捉标注示例中的引导信号，而能更好提取信号的方法需要数百到数千个示例。需要一种既能高效利用样本又能有效提取引导信号的解决方案。

Method: 提出COLD-Steer框架，通过近似梯度下降对上下文示例的影响来引导LLM激活，无需实际参数更新。包含两种互补方法：(1)单位核近似方法，直接使用激活梯度更新激活，并在示例间归一化；(2)有限差分近似，无论示例数量多少都只需两次前向传播。

Result: 在各种引导任务和基准测试中，COLD-Steer达到高达95%的引导效果，同时比最佳基线方法少用50倍的样本。在多元对齐任务上的实验验证了该方法能够适应不同观点而无需大量演示数据。

Conclusion: COLD-Steer框架为自适应、上下文感知的模型控制开辟了新可能性，能够通过原理性地近似学习动态而非专门的训练程序，灵活应对不同的损失驱动的人类偏好。
...
### [164] [When One Modality Rules Them All: Backdoor Modality Collapse in Multimodal Diffusion Models](https://arxiv.org/abs/2603.06508)
*Qitong Wang,Haoran Dai,Haotian Zhang,Christopher Rasmussen,Binghui Wang*

Main category: cs.LG

TL;DR: 研究发现多模态扩散模型中的后门攻击存在"模态崩溃"现象，即攻击主要依赖少数模态而非多模态协同作用，挑战了多模态攻击具有互补效应的假设。


<details>
  <summary>Details</summary>
Motivation: 随着扩散模型在视觉内容生成领域的广泛应用，研究其安全漏洞（如后门攻击）变得至关重要。传统观点认为多模态（如文本和图像）同时攻击会产生互补效应，增强整体后门效果，但本文质疑这一假设。

Method: 引入两种新指标：触发模态归因（TMA）和交叉触发交互（CTI），通过多模态条件扩散模型中的多样化训练配置进行广泛实验，量化后门模态崩溃现象。

Result: 实验发现后门行为呈现"赢家通吃"动态：1）攻击往往崩溃为子模态主导；2）跨模态交互可忽略甚至为负值，与协同漏洞的直觉相矛盾。高攻击成功率常掩盖对子模态的根本依赖。

Conclusion: 揭示了当前评估中的关键盲点，为机制分析和未来防御开发建立了原则性基础。多模态后门攻击并非总是协同作用，而是可能退化到依赖少数模态。
...
### [165] [Hierarchical Industrial Demand Forecasting with Temporal and Uncertainty Explanations](https://arxiv.org/abs/2603.06555)
*Harshavardhan Kamarthi,Shangqing Xu,Xinjie Tong,Xingyu Zhou,James Peters,Joseph Czyzyk,B. Aditya Prakash*

Main category: cs.LG

TL;DR: 提出了一种用于大规模分层概率时间序列预测的新型可解释性方法，能够解释工业供应链场景中的预测结果，显著提高可解释性准确性。


<details>
  <summary>Details</summary>
Motivation: 虽然机器学习模型在分层时间序列预测任务上取得了显著的准确性和可扩展性，但其预测的可解释性仍然在很大程度上未被探索，特别是在实际应用场景中。需要弥合这一差距，为工业供应链等实际应用提供有价值的解释性见解。

Method: 提出了一种新颖的可解释性方法，专门针对大规模分层概率时间序列预测设计。该方法适应通用的可解释性技术，同时解决了与分层结构和不确定性相关的挑战。通过生成基于真实场景的半合成数据集进行评估，并在大型化工公司的实际案例中验证。

Result: 实验表明，该方法成功解释了最先进的工业预测方法，具有显著更高的可解释性准确性。多个真实案例研究证明了该方法在识别重要模式和解释方面的有效性，帮助利益相关者更好地理解预测结果。

Conclusion: 该方法能够识别预测需求背后的关键驱动因素，支持更明智的决策和战略规划。通过提高预测的可解释性，有助于建立用户信任和信心，最终促进分层预测模型在实际中的更好采用和利用。
...
### [166] [A recipe for scalable attention-based MLIPs: unlocking long-range accuracy with all-to-all node attention](https://arxiv.org/abs/2603.06567)
*Eric Qu,Brandon M. Wood,Aditi S. Krishnapriyan,Zachary W. Ulissi*

Main category: cs.LG

TL;DR: AllScAIP是一个基于注意力机制、能量守恒的机器学习原子间势能模型，可扩展到1亿训练样本，通过全节点注意力组件数据驱动地解决长程相互作用问题。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习原子间势能模型在处理生物分子和电解质等大系统时，难以准确捕捉长程相互作用，通常依赖显式的物理基础项或组件。

Method: 提出AllScAIP模型，采用基于注意力的能量守恒架构，包含全节点注意力组件，数据驱动地处理长程相互作用，可扩展到大规模训练数据。

Result: 在分子系统上达到最先进的能量/力精度，在物理评估(OMol25)中表现优异，在材料(OMat24)和催化剂(OC20)上具有竞争力，并能进行稳定长时间尺度的分子动力学模拟，准确预测密度和汽化热等实验观测值。

Conclusion: 研究表明：在小数据/小模型情况下，归纳偏置能提高样本效率；但当数据和模型规模扩大时，这些优势减弱甚至逆转，而全节点注意力对捕捉长程相互作用至关重要。AllScAIP为大规模系统提供了一种有效的解决方案。
...
### [167] [ProFocus: Proactive Perception and Focused Reasoning in Vision-and-Language Navigation](https://arxiv.org/abs/2603.05530)
*Wei Xue,Mingcheng Li,Xuecheng Wu,Jingqun Tang,Dingkang Yang,Lihua Zhang*

Main category: cs.RO

TL;DR: ProFocus是一个免训练的渐进式框架，通过LLM和VLM协作实现主动感知和聚焦推理，在VLN任务中达到零样本方法的最先进性能。
...
Conclusion: ProFocus通过主动感知和聚焦推理的统一框架，解决了VLN中感知效率低下和推理不聚焦的问题，为视觉语言导航提供了有效的解决方案。
...
### [168] [Digital-Twin Losses for Lane-Compliant Trajectory Prediction at Urban Intersections](https://arxiv.org/abs/2603.05546)
*Kuo-Yi Chao,Erik Leo Haß,Melina Gegg,Jiajie Zhang,Ralph Raßhofer,Alois Christian Knoll*

Main category: cs.RO

TL;DR: 提出基于数字孪生的V2X轨迹预测管道，结合车辆和基础设施的协同感知，通过双损失训练（MSE损失+孪生损失）实现安全合规的多智能体运动预测。


<details>
  <summary>Details</summary>
Motivation: 在V2X支持的复杂城市环境中，准确且安全的轨迹预测是智能交通系统的关键技术。需要解决多智能体交互中的交通规则遵守、碰撞避免和场景一致性等问题。

Method: 创建数字孪生驱动的V2X轨迹预测管道，结合车辆和基础设施的协同感知。使用Bi-LSTM生成器，采用双损失训练目标：标准MSE损失确保点精度，新颖的孪生损失编码基础设施约束、碰撞避免、预测模式多样性和数字孪生衍生的规则先验。

Result: 在真实V2X数据上训练评估，除了标准轨迹指标（ADE、FDE），引入了ITS相关的安全指标（基础设施和规则违规率）。实验结果表明，所提训练方案显著减少了关键违规，同时保持了相当的预测精度和实时性能。

Conclusion: 数字孪生驱动的多损失学习在V2X智能交通系统中具有重要潜力，能够实现安全合规的轨迹预测，平衡预测精度与安全约束。
...
### [169] [TEGA: A Tactile-Enhanced Grasping Assistant for Assistive Robotics via Sensor Fusion and Closed-Loop Haptic Feedback](https://arxiv.org/abs/2603.05552)
*Hengxu You,Tianyu Zhou,Fang Xu,Kaleb Smith,Eric Jing Du*

Main category: cs.RO

TL;DR: TEGA是一个闭环辅助遥操作系统，融合肌电意图推断和视觉触觉感知，通过可穿戴触觉背心提供实时振动反馈，实现直观的抓握力调节。


<details>
  <summary>Details</summary>
Motivation: 当前遥操作系统主要关注手指位置控制，但忽视了抓握力调节这一关键任务。这对于处理不同硬度、纹理和形状的物体至关重要，尤其对于上肢残疾用户来说，他们缺乏自然触觉反馈，难以推断合适的抓握力水平。

Method: 提出了TEGA系统，这是一个闭环辅助遥操作框架，融合了基于肌电的意图到力的推断和视觉触觉感知，通过可穿戴触觉背心将感知信息映射为实时振动触觉反馈，使用户能够在操作过程中直观地按比例调整抓握力。
...
### [170] [Task Parameter Extrapolation via Learning Inverse Tasks from Forward Demonstrations](https://arxiv.org/abs/2603.05576)
*Serdar Bahar,Fatih Dogangun,Matteo Saveriano,Yukie Nagai,Emre Ugur*

Main category: cs.RO

TL;DR: 提出一种新的联合学习方法，通过构建正向和逆向任务的共同表示，利用新配置下的正向演示来执行相应的逆向任务，实现准确高效的知识迁移。


<details>
  <summary>Details</summary>
Motivation: 机器人学习中的技能泛化仍然是一个关键挑战。模仿学习方法虽然数据效率高，但局限于训练区域，在输入数据超出该区域时容易失败。迁移学习方法虽然对环境或任务变化具有鲁棒性，但数据需求大且在零样本泛化中缺乏准确性。

Method: 将问题置于任务逆学习框架中，提出一种新颖的联合学习方法。该方法构建正向和逆向任务的共同表示，利用来自新配置的辅助正向演示来成功执行相应的逆向任务，无需任何直接监督。

Result: 通过消融研究和在模拟及真实环境中的实验展示了框架的泛化能力，这些实验需要复杂的操作技能和多样化的物体与工具。该方法在性能上优于基于扩散的替代方法。

Conclusion: 提出的联合学习方法能够实现准确高效的知识迁移，解决了机器人学习中技能泛化的关键挑战，特别是在处理复杂操作任务时表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 软体和软刚体混合机器人本质上是欠驱动的，且受限于执行器限制，使得具有稳定性保证的任务空间控制具有挑战性。现有的非线性控制策略通常假设完全驱动且无执行器限制，不适用于实际软体机器人

Method: 提出一个通用控制框架，将快速指数稳定控制Lyapunov函数作为凸不等式约束强制执行，同时满足欠驱动全系统动力学和执行器边界约束

Result: 在多个平台上验证了该方法，包括两连杆肌腱驱动"手指"、修剪螺旋面机械臂和高欠驱动螺旋机器人。与文献中的基准方法相比，该方法在输入限制下表现出更好的任务空间精度和一致的Lyapunov收敛性

Conclusion: 该方法为欠驱动软体机器人提供了在输入限制下具有稳定性保证的任务空间控制解决方案，在定点控制和轨迹跟踪方面均表现出优越性能
...
TL;DR: 该研究创建了医疗人机交互中的机器人故障数据集（RFM-HRI），分析了用户对四种机器人故障类型的多模态响应，发现故障会显著降低情感效价和感知控制，并观察到情绪随重复故障的变化模式。


<details>
  <summary>Details</summary>
Motivation: 在真实环境中部署的机器人不可避免地会出现交互故障，特别是在医疗场景中，这些故障会直接影响任务表现和用户信任。然而，用户如何通过言语和非言语行为响应这些故障，在HRI研究中尚未得到充分探索。

Method: 通过Wizard-of-Oz研究，在实验室和医院环境中对41名参与者进行实验，系统诱导四种故障类型（语音、时序、理解和搜索故障）。收集了214个交互样本，包括面部动作单元、头部姿态、语音转录和交互后自我报告等多模态数据。

Result: 故障显著降低了情感效价和感知控制；故障与困惑、恼怒和沮丧强烈相关，而成功交互则与惊喜、宽慰和任务完成信心相关；情绪响应随重复故障演变，困惑减少而沮丧增加。

Conclusion: 该工作贡献了：1）公开可用的多模态数据集RFM-HRI；2）对不同故障类型用户响应和偏好恢复策略的分析；3）支持系统比较恢复策略的医疗推车检索场景。研究结果为具身HRI中的故障检测和恢复方法提供了基础。
...
TL;DR: SCOUT是一种基于3D场景图的开放世界交互式物体搜索方法，通过离线程序化蒸馏从大语言模型中提取结构化关系知识，实现高效语义推理，在保持计算效率的同时达到LLM级别的性能。


<details>
  <summary>Details</summary>
Motivation: 家庭环境中的开放世界交互式物体搜索需要理解物体与周围环境的语义关系以高效引导探索。现有方法要么依赖视觉语言嵌入相似性（不能可靠捕捉任务相关的关系语义），要么使用大语言模型（太慢且成本高，不适合实时部署）。

Method: SCOUT方法直接在3D场景图上进行搜索，使用关系探索启发式（如房间-物体包含关系和物体-物体共现关系）为房间、边界和物体分配效用分数。通过离线程序化蒸馏框架从LLMs中提取结构化关系知识到轻量级模型中，实现机器人上的实时推理。同时提出了SymSearch符号基准来评估交互式物体搜索任务中的语义推理能力。

Result: 在符号和仿真环境中的广泛评估表明，SCOUT优于基于嵌入相似性的方法，并匹配LLM级别的性能，同时保持计算效率。真实世界实验展示了向物理环境的有效迁移，能够在实际感知和导航约束下实现开放世界交互式物体搜索。

Conclusion: SCOUT通过结合场景图表示和从LLMs蒸馏的关系知识，实现了高效、可扩展的开放世界交互式物体搜索，在保持语义推理能力的同时满足了实时部署的计算要求。
...
TL;DR: CGP是一种接触基础策略，通过预测机器人状态和触觉反馈来精细化控制接触丰富的灵巧操作，使用学习到的接触一致性映射将预测转换为控制器可执行目标，支持多种触觉传感器。


<details>
  <summary>Details</summary>
Motivation: 解决灵巧操作中需要精细接触控制的问题，传统方法难以处理多点接触的复杂交互，需要一种能够有效利用触觉反馈进行精细化操作的方法。

Method: 通过预测实际机器人状态和触觉反馈来建立多接触点基础，使用学习到的接触一致性映射将预测转换为控制器可执行目标，支持密集触觉阵列和视觉触觉传感器，通过遥操作收集仿真和物理机器人演示数据。

Result: 在多个灵巧操作任务中评估了CGP，展示了其能够实现精细化的接触控制，支持多种触觉传感器配置。

Conclusion: CGP为接触丰富的灵巧操作提供了一种有效的解决方案，能够通过触觉反馈实现精细化控制，在仿真和物理机器人上都表现出良好性能。
...
TL;DR: NC-tALC数据集：首个高精度轨迹数据集，专门用于研究过渡期自动驾驶车辆在强制换道场景中的交互行为，包含152次试验，为评估tAV决策和交互动态提供实证基础。


<details>
  <summary>Details</summary>
Motivation: 随着过渡期自动驾驶车辆（tAVs）与人类驾驶车辆（HDVs）共享道路，在复杂换道等操作中可能出现新的交互模式，影响交通稳定性和安全性。然而，目前缺乏能够捕捉tAV换道行为的高分辨率轨迹数据集。

Method: 研究开发了NC-tALC数据集，包含两个受控实验系列：1）tAV换道实验：tAV在配备自适应巡航控制（ACC）的目标车辆存在下执行换道；2）tAV响应实验：两辆tAV作为跟随者，响应另一辆tAV的切入操作。数据集包含152次试验，采样频率20Hz，采用厘米级RTK-GPS精度。

Result: 成功创建了包含72次换道试验和80次响应试验的高保真轨迹数据集，为分析tAV换道执行和跟随者响应动态提供了详细的实证数据。

Conclusion: NC-tALC数据集为评估过渡期自动驾驶车辆在强制换道场景中的决策制定和交互动态提供了严谨的实证基础，填补了该领域数据稀缺的空白。
...
TL;DR: 提出环境感知路径生成框架，首次将四种路径规划算法应用于在线结构设计，评估其在挑战性障碍环境中的表现，为机器人增材制造识别最有前景的路径规划器。
...
Method: 提出环境感知路径生成框架，集成两种基于搜索和两种基于采样的路径规划算法，在周期性/随机障碍排列下评估封闭/开放结构，开发结构和计算性能指标。

Result: 评估了四种路径规划器在挑战性环境中的可行性，确定了结构和计算性能指标的有效性，为机器人增材制造识别出在密集障碍环境下最有前景的路径规划器。

Conclusion: 首次提出环境感知路径生成框架，系统评估了路径规划算法在挑战性障碍环境中的表现，为机器人增材制造应用提供了关键的性能指标和最有前景的路径规划器选择。
...
TL;DR: 提出隐私保护的医疗翻译机器人视觉语言框架，通过本地部署开源模型检测特定言语行为并生成相应机器人手势，在用户研究中表现优于基线。


<details>
  <summary>Details</summary>
Motivation: 医疗环境中跨语言沟通至关重要，非语言线索和手势是关键要素。需要解决语言障碍下的有效沟通问题，同时保护患者隐私。

Method: 基于本地部署的开源模型构建隐私保护框架，使用少样本提示的大语言模型进行意图检测，引入临床对话数据集（标注言语行为并配对手势片段）。

Result: 识别模块达到0.90准确率、0.93加权精确率和0.91加权F1分数。在用户研究中，该方法在人性化方面优于语音-手势生成基线，同时保持相当的适当性。

Conclusion: 该隐私保护框架显著提高了计算效率，在医疗翻译机器人应用中实现了有效的言语行为检测和手势生成，改善了跨语言医疗沟通。
...
### [178] [EmboAlign: Aligning Video Generation with Compositional Constraints for Zero-Shot Manipulation](https://arxiv.org/abs/2603.05757)
*Gehao Zhang,Zhenyang Ni,Payal Mohapatra,Han Liu,Ruohan Zhang,Qi Zhu*

Main category: cs.RO

TL;DR: 本文提出了一种无需训练数据的框架，通过视觉语言模型提取任务约束来指导视频生成模型，从而提升零样本机器人操作的物理合理性和成功率。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型虽然能生成连贯的视频，但常产生物理上不合理的动作，且通过几何重定向转换为机器人动作时会引入累积误差，这限制了其在机器人操作中的应用。

Method: 提出一个名为\method{}的无数据框架，使用视觉语言模型自动提取任务特定的组合约束，然后通过两个阶段应用这些约束：1）约束引导的rollout选择，从一批视频生成模型输出中筛选最物理合理的候选；2）基于约束的轨迹优化，以选中的rollout为初始化，在相同约束下优化机器人轨迹以纠正重定向误差。

Result: 在六个需要精确、约束敏感执行的真实机器人操作任务上评估，相比最强基线，整体成功率提高了43.3个百分点，且无需任何任务特定的训练数据。

Conclusion: 通过结合视频生成模型的动态生成能力和视觉语言模型的结构化空间推理能力，提出的框架能够有效提升零样本机器人操作的物理合理性和执行成功率。
...
### [179] [Task-Level Decisions to Gait Level Control: A Hierarchical Policy Approach for Quadruped Navigation](https://arxiv.org/abs/2603.05783)
*Sijia Li,Haoyu Wang,Shenghai Yuan,Yizhuo Yang,Thien-Minh Nguyen*

Main category: cs.RO

TL;DR: 提出分层策略架构TDGC，通过高层决策和低层步态控制解决四足机器人导航中的尺度不匹配问题，支持部署时调优和故障诊断


<details>
  <summary>Details</summary>
Motivation: 现实世界四足导航存在高层导航决策与低层步态执行之间的尺度不匹配问题，以及环境变化导致的分布外不稳定性，这些挑战了仿真到现实的迁移，并可能导致策略缺乏自适应接口而摔倒

Method: 提出任务级决策到步态控制(TDGC)分层架构：低层策略通过强化学习训练实现步态条件化运动，将任务需求映射到可控行为参数集；高层策略基于稀疏语义或几何地形线索做出任务中心决策，并转换为低层目标

Result: 实验显示在混合地形和分布外测试中具有更高的任务成功率，架构提供明确的部署时调优、故障诊断和策略优化接口

Conclusion: TDGC架构通过分层设计和明确接口解决了四足导航的尺度不匹配问题，相比端到端方法提供了更好的可追溯决策流程和适应性
...
### [180] [CDF-Glove: A Cable-Driven Force Feedback Glove for Dexterous Teleoperation](https://arxiv.org/abs/2603.05804)
*Huayue Liang,Ruochong Li,Yaodong Yang,Long Zeng,Yuanpei Chen,Xueqian Wang*

Main category: cs.RO

TL;DR: CDF-Glove：低成本、轻量化的线驱动力反馈手套，用于灵巧操作的遥操作演示收集，相比无反馈遥操作提升4倍任务成功率，成本仅230美元。
...
Method: 开发了CDF-Glove，一种轻量化、低成本的线驱动力反馈手套。实时状态支持20个手指自由度（16个直接传感，4个被动耦合），建立了运动学模型和控制栈，并在多种机器人手上验证。

Result: CDF-Glove远端关节重复精度达0.4度，力反馈延迟约200毫秒，相比无反馈遥操作提升4倍任务成功率。收集了两个双手遥操作数据集，基于此训练的Diffusion Policy相比动觉示教平均成功率提升55%，平均完成时间减少15.2秒（相对减少47.2%）。

Conclusion: CDF-Glove为灵巧操作的模仿学习提供了高质量、低成本的遥操作解决方案，显著提升了演示质量和学习性能，且开源了代码和设计。
...
### [181] [Hierarchical Latent Action Model](https://arxiv.org/abs/2603.05815)
...
<details>
  <summary>Details</summary>
Motivation: 现有潜在动作模型(LAMs)通常只关注短期的帧转换，捕捉低级运动而忽略了长期的时间结构。然而，无动作视频通常包含时间上扩展的高级技能，需要能够发现这些长期依赖关系的模型。

Method: HiLAM采用分层架构，使用预训练的LAM作为低级提取器，将包含视频底层动态模式的潜在动作序列聚合成高级潜在技能，从而捕获长时域的时间依赖关系。

Result: 实验表明，HiLAM在基准测试中表现优于基线模型，并展现出鲁棒的动态技能发现能力。

Conclusion: HiLAM通过分层建模长期时间信息，能够有效发现无动作视频中的高级潜在技能，为从机器人控制到交互式世界模型等应用提供了更好的学习框架。
...
### [182] [OpenHEART: Opening Heterogeneous Articulated Objects with a Legged Manipulator](https://arxiv.org/abs/2603.05830)
*Seonghyeon Lim,Hyeonwoo Lee,Seunghyun Lee,I Made Aswin Nahrendra,Hyun Myung*

Main category: cs.RO

TL;DR: 提出OpenHEART框架，通过SAFE特征提取和ArtIEst关节信息估计，使腿式机器人能够高效、鲁棒地操作各种异质关节物体（如门、抽屉、柜子）。


<details>
  <summary>Details</summary>
Motivation: 腿式机械手具有高机动性和多功能操作能力，但操作异质关节物体（如门、抽屉、柜子）仍然具有挑战性，因为物体关节类型多样且腿式机器人动力学复杂。现有基于强化学习的方法通常依赖高维感官输入，导致样本效率低下。

Method: 提出OpenHEART框架：1) SAFE（基于采样的抽象特征提取）将手柄和面板几何编码为紧凑的低维表示，提高跨域泛化能力；2) ArtIEst（关节信息估计器）自适应混合本体感觉与外感觉，估计每个物体的开启方向和运动范围。

Result: 该框架在仿真和真实机器人系统中成功部署，能够操作各种异质关节物体。项目网站提供了演示视频。

Conclusion: OpenHEART框架为腿式机器人操作异质关节物体提供了鲁棒且样本高效的解决方案，通过紧凑的特征表示和自适应感知融合克服了现有方法的局限性。
...
### [183] [Terrain characterization and locomotion adaptation in a small-scale lizard-inspired robot](https://arxiv.org/abs/2603.05837)
*Duncan Andrews,Landon Zimmerman,Evan Martin,Joe DiGennaro,Baxi Chong*

Main category: cs.RO

TL;DR: SILA Bot：一种受蜥蜴启发的小型智能自适应机器人，通过关节扭矩等本体感觉信号感知颗粒介质深度，并采用线性反馈控制器调节身体运动模式，显著提升了在未知深度复杂地形中的运动性能。


<details>
  <summary>Details</summary>
Motivation: 小型机器人由于尺寸减小导致与地形的交互方式发生根本变化，缺乏对复杂自然地形中应获取何种感知信息以及如何响应的系统理解，限制了其在真实环境中的部署应用。

Method: 开发了受蜥蜴启发的SILA Bot机器人，使用不同深度的颗粒介质作为受控地形范式，通过K最近邻分类器利用关节扭矩等本体感觉信号估计颗粒深度，并设计了线性反馈控制器来调节身体相位。

Result: 最佳身体运动模式（从平坦地面的驻波弯曲到深颗粒介质的行波波动）可作为颗粒深度的线性函数参数化；关节扭矩信号能以95%的准确率估计颗粒深度；线性反馈控制器显著提升了在未知深度地形中的运动性能。

Conclusion: 该研究为小型运动中的感知与控制建立了原则性框架，实现了有效的自适应地形运动，同时保持了较低的计算复杂度，推动了小型机器人在真实环境中的应用。
...
### [184] [Expert Knowledge-driven Reinforcement Learning for Autonomous Racing via Trajectory Guidance and Dynamics Constraints](https://arxiv.org/abs/2603.05842)
*Bo Leng,Weiqi Zhang,Zhuoren Li,Lu Xiong,Guizhe Jin,Ran Yu,Chen Lv*

Main category: cs.RO

TL;DR: 提出TraD-RL方法，通过轨迹引导和动力学约束解决自动驾驶赛车环境中强化学习训练不稳定和不安全的问题，实现赛车性能与安全的协同优化。


<details>
  <summary>Details</summary>
Motivation: 强化学习在自动驾驶领域有潜力，但在高动态、强非线性的赛车环境中存在训练不稳定和输出不安全动作的缺陷，需要改进。

Method: 1) 利用专家赛车线构建增强状态表示和奖励塑形；2) 通过控制屏障函数嵌入车辆动力学先验形成安全操作包络；3) 采用多阶段课程学习策略，从专家引导过渡到自主探索。

Result: 在Tempelhof机场街道赛道的高保真仿真环境中评估，TraD-RL有效提高了自动驾驶赛车的单圈速度和驾驶稳定性，实现了赛车性能与安全的协同优化。

Conclusion: TraD-RL方法通过整合领域知识和安全约束，成功解决了自动驾驶赛车中强化学习的训练稳定性和安全性问题，使学习策略能够超越专家水平性能。
...
### [185] [AnyCamVLA: Zero-Shot Camera Adaptation for Viewpoint Robust Vision-Language-Action Models](https://arxiv.org/abs/2603.05868)
*Hyeongjun Heo,Seungyeon Woo,Sang Min Kim,Junho Kim,Junho Lee,Yonghyeon Lee,Young Min Kim*

Main category: cs.RO

TL;DR: 提出零样本相机适应框架，无需额外演示数据、策略微调或架构修改，通过实时虚拟调整测试时相机观测以匹配训练配置，提升视觉语言动作模型在视角变化下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言动作模型在特定环境部署时需要微调，且对相机视角变化高度敏感，这在非结构化环境中经常发生，需要提升模型在视角变化下的鲁棒性。

Method: 使用前馈式新视角合成模型实时虚拟调整测试时相机观测，使其匹配训练相机配置，处理外参和内参变化，作为即插即用方法应用于任何基于RGB的策略。

Result: 在LIBERO基准测试中一致优于使用数据增强进行策略微调或添加3D感知特征的基线方法，在真实机器人操作场景中持续增强视角鲁棒性，包括外参、内参变化和手持相机移动场景。

Conclusion: 提出的零样本相机适应框架有效提升了视觉语言动作模型在相机视角变化下的鲁棒性，无需额外数据或模型修改，在仿真和真实环境中均表现出优越性能。
...
### [186] [Improved hopping control on slopes for small robots using spring mass modeling](https://arxiv.org/abs/2603.05902)
*Heston Roberts,Pronoy Sarker,Sm Ashikul Islam,Min Gyu Kim*

Main category: cs.RO

TL;DR: 论文分析了斜坡上跳跃机器人失衡问题，提出了基于斜坡调整身体触地角度和起飞前施加小扭矩的简单控制方法，有效抵消斜坡引起的旋转，提高着陆稳定性。


<details>
  <summary>Details</summary>
Motivation: 跳跃机器人在斜坡上经常失去平衡，因为倾斜地面在着陆时会产生不期望的旋转。现有方法通常需要复杂的传感和计算，不适合低成本机器人平台。

Method: 使用简单的弹簧质量模型分析斜坡引起的冲击效应，提出两种简单修正：1) 根据斜坡调整身体触地角度；2) 在起飞前施加小的校正扭矩。这些方法不需要复杂传感或计算。

Result: 仿真结果表明，即使很小的斜坡感知调整也能显著提高着陆稳定性，使机器人能够在陡峭斜坡上平稳着陆并保持稳定跳跃。

Conclusion: 通过结合分析模型和最小控制动作，该方法为在崎岖地形上实现可靠跳跃提供了实用途径，适合未来需要在自然环境中导航的自主野外机器人。
...
### [187] [Iterative Convex Optimization with Control Barrier Functions for Obstacle Avoidance among Polytopes](https://arxiv.org/abs/2603.05916)
...
<details>
  <summary>Details</summary>
Motivation: 现有避障方法存在局限性：基于光滑几何近似的方法会扭曲真实几何形状并限制可行集；而基于精确多面体距离的非线性MPC方法会产生非凸优化问题，限制实时性能

Method: 通过精确最近点计算推导凸多面体之间的支撑超平面，构建线性离散时间控制屏障函数约束，提出迭代凸MPC-DCBF框架，通过局部线性化系统动力学和机器人几何确保每次迭代的有限时域优化保持凸性

Result: 该框架显著降低了计算复杂度，能够在毫秒级求解时间内实现安全关键控制和轨迹规划，支持多机器人和三维环境，数值实验展示了在杂乱迷宫场景中的无碰撞导航

Conclusion: 提出的迭代凸MPC-DCBF框架为多面体机器人在多面体障碍物环境中的实时避障控制提供了一种高效解决方案，平衡了几何精确性和计算效率
...
### [188] [PROBE: Probabilistic Occupancy BEV Encoding with Analytical Translation Robustness for 3D Place Recognition](https://arxiv.org/abs/2603.05965)
...
<details>
  <summary>Details</summary>
Motivation: 现有的LiDAR地点识别方法通常依赖于离散点云扰动或需要大量训练数据，缺乏传感器无关的通用性。PROBE旨在开发一种无需学习、传感器独立的方法，能够跨不同LiDAR传感器进行泛化，无需针对每个数据集进行调优。

Method: PROBE将每个BEV单元的占用建模为伯努利随机变量，通过极坐标雅可比矩阵对连续笛卡尔平移进行解析边缘化，产生距离自适应的角度不确定性σ_θ=σ_t/r。主要参数σ_t表示以米为单位的预期平移不确定性，这是一个传感器无关的物理量。成对相似性结合了伯努利-KL Jaccard距离、指数不确定性门控和基于FFT的高度余弦相似度进行旋转对齐。

Result: 在涵盖四种不同LiDAR类型的四个数据集上评估，PROBE在多会话评估中实现了手工描述符中最高的准确率，在单会话性能方面与手工和基于监督的基线方法具有竞争力。

Conclusion: PROBE作为一种无需学习的LiDAR地点识别描述符，通过概率建模和解析边缘化方法，实现了传感器无关的跨传感器泛化能力，在多会话场景中表现出色，为实际应用提供了实用且高效的解决方案。
...
### [189] [Proprioceptive Shape Estimation of Tensegrity Manipulators Using Energy Minimisation](https://arxiv.org/abs/2603.05976)
*Tufail Ahmad Bhat,Shuhei Ikemoto*

Main category: cs.RO

TL;DR: 该论文提出了一种仅使用每个支柱相对于重力的倾角信息来估计连续弯曲张拉整体机械臂形状的方法，实验证明在五层机械臂上可实现总长度2.1%的精度。


<details>
  <summary>Details</summary>
Motivation: 张拉整体机械臂的形状估计对其控制至关重要。虽然外感受传感器方法直接但成本高且环境受限，而本体感受方法则没有这些限制。目前缺乏大规模张拉整体结构作为机械臂的验证实例，需要一种简单有效的形状估计方法。

Method: 提出仅使用每个支柱相对于重力的倾角信息来估计整个张拉整体机械臂形状的方法。通过在每个支柱上安装惯性测量单元（IMU）获取倾角数据，这是一种内在的传感方式，无需外部传感器。

Result: 在五层张拉整体机械臂（20个支柱，总长1160mm）上的实验表明：该方法可以从任意初始条件在静态条件下估计形状，精度达到机械臂总长度的2.1%，并在外部干扰下保持稳定的形状估计。

Conclusion: 仅使用支柱倾角信息即可实现张拉整体机械臂的准确形状估计，提供了一种成本效益高、环境适应性强的解决方案，为大规模张拉整体机械臂的实际应用奠定了基础。
...
### [190] [HarvestFlex: Strawberry Harvesting via Vision-Language-Action Policy Adaptation in the Wild](https://arxiv.org/abs/2603.05982)
...
Method: 基于HarvestFlex平台构建端到端闭环系统，使用三视角RGB传感（两个固定场景视角加腕戴视角），避免深度云和显式几何标定，收集3.71小时VR遥操作演示数据，对pi_0、pi_0.5和WALL-OSS模型进行全微调和LoRA微调

Result: 在统一50次真实温室试验协议下，pi_0.5全微调模型达到74.0%成功率，32.6秒/采摘，4.1%损伤率；异步推理-控制解耦进一步提升了同步部署的性能

Conclusion: 研究表明在少于四小时真实数据下可实现非平凡的闭环采摘，但仍受限于近距离可观测性损失和接触动力学不匹配问题
...
### [191] [MagRobot:An Open Simulator for Magnetically Navigated Robots](https://arxiv.org/abs/2603.05992)
*Heng Wang,Haoyu Song,Jiatao Zheng,Yuxiang Han,Kunli Wang*

Main category: cs.RO

TL;DR: 开发了首个通用开源仿真平台，用于磁导航机器人的研究、设计和性能评估，支持刚性/软体机器人的磁驱动和磁跟踪任务仿真。


<details>
  <summary>Details</summary>
Motivation: 磁导航系统在微创医学中具有重要应用，但当前磁导航机器人设计依赖实验原型，耗时耗成本，且缺乏统一的实验环境来比较不同系统的硬件和算法。

Method: 提出首个通用开源仿真平台，具有直观的图形用户界面，支持用户高效设计、可视化和分析磁导航系统，可模拟刚性/软体机器人的磁驱动和磁跟踪任务，并支持第三方解剖模型加载和硬件/算法定制。

Result: 通过连续体机器人和胶囊机器人的体模和离体实验验证了仿真器的保真度，实现了支气管镜、血管内介入和胃肠道内镜三个应用案例，证明可通过仿真器灵活设计和优化磁导航系统的配置和算法以获得更好性能。

Conclusion: 该仿真平台为磁导航机器人的研究、设计和性能评估提供了首个通用开源解决方案，能够显著减少实验原型依赖，促进不同系统间的比较和优化。
...
### [192] [Moving Through Clutter: Scaling Data Collection and Benchmarking for 3D Scene-Aware Humanoid Locomotion via Virtual Reality](https://arxiv.org/abs/2603.05993)
...
<details>
  <summary>Details</summary>
Motivation: 现有类人机器人运动研究主要在开放平坦环境中展示动态行为，而真实世界环境（家庭、办公室等）是杂乱、三维且几何受限的，需要场景感知的全身协调和平衡控制，但目前缺乏系统耦合全身人类运动与场景几何的数据集。

Method: 开发了基于虚拟现实的数据收集和评估框架，程序化生成可控杂乱程度的场景，通过沉浸式VR导航捕获全身人类运动，并自动重定向到类人机器人模型。

Result: 创建了包含145个多样化3D杂乱场景中348条轨迹的数据集，并引入了量化环境杂乱程度和运动性能（稳定性和碰撞安全性）的基准测试。

Conclusion: MTC框架为研究类人机器人在几何约束下的适应性以及开发场景感知的规划和控制方法提供了基础，填补了杂乱3D环境中类人机器人运动研究的空白。
...
### [193] [TADPO: Reinforcement Learning Goes Off-road](https://arxiv.org/abs/2603.05995)
*Zhouchonghao Wu,Raymond Song,Vedant Mundheda,Luis E. Navarro-Serment,Christof Schoenborn,Jeff Schneider*

Main category: cs.RO

TL;DR: TADPO：一种基于PPO的新型策略梯度方法，通过教师引导和学生探索实现端到端视觉强化学习，用于高速越野自动驾驶，并成功实现零样本仿真到现实的迁移。
...
Method: 提出TADPO（一种新颖的策略梯度公式），扩展了PPO方法，利用离策略轨迹进行教师引导，同时使用在策略轨迹进行学生探索。基于此开发了基于视觉的端到端RL系统，用于高速越野驾驶。

Result: 在仿真中展示了性能，更重要的是，在真实全尺寸越野车辆上实现了零样本仿真到现实的迁移。这是首次在完整越野平台上部署基于RL的策略。

Conclusion: TADPO方法成功解决了越野自动驾驶中的长时程规划和自适应控制问题，实现了从仿真到真实世界的有效迁移，为越野自动驾驶提供了可行的强化学习解决方案。
...
### [194] [Restoring Linguistic Grounding in VLA Models via Train-Free Attention Recalibration](https://arxiv.org/abs/2603.06001)
...
<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人操作任务中表现出色，但其在分布外指令下的可靠性尚未充分研究。研究发现VLA模型存在语言盲视问题，即模型优先考虑视觉先验而忽略指令语义，导致在矛盾指令下仍执行错误动作。

Method: 1. 提出ICBench诊断基准，基于LIBERO数据集构建，注入受控的分布外矛盾指令；2. 提出IGAR（指令引导注意力重校准）方法，无需训练或架构修改，在推理时重新平衡注意力分布以恢复语言指令的影响。

Result: 在三个代表性VLA架构（Pi0、Pi0.5、OpenVLA OFT）上的评估显示，这些模型经常在逻辑不可能的指令下仍执行任务，表现出强烈的视觉偏差。IGAR在30个LIBERO任务中显著减少了矛盾指令下的错误执行，同时保持了基线任务性能。在真实Franka机械臂上的验证也表明IGAR能有效防止不一致指令触发的错误操作。

Conclusion: VLA模型存在语言盲视问题，需要关注其语言-动作耦合的可靠性。IGAR作为一种无需训练的推理时机制，能有效缓解这一问题，提高VLA模型在矛盾指令下的鲁棒性，为实际机器人部署提供了重要改进。
...
### [195] [RODEO: RObotic DEcentralized Organization](https://arxiv.org/abs/2603.06058)
*Milan Groshev,Eduardo Castelló Ferrer*

Main category: cs.RO

TL;DR: RODEO是一个基于区块链的机器人框架，通过DAO实现机器人服务的信任与问责机制，使机器人能够自主执行任务、获得代币报酬并投资服务扩展运营时间。
...
Method: 提出RODEO框架，包含：1) ROS-ETH桥接DAO与机器人；2) 组织集成模板；3) 可审计的机器人行动证明验证机制。实验中使用移动机器人作为垃圾收集器，通过智能垃圾桶分类处理垃圾，提交成功操作证明并获得DAO代币报酬，然后重新投资购买电池充电服务。

Result: 三天实验数据显示：机器人收入翻倍，通过再投资延长了运营时间；证明验证时间约1分钟确保可验证的任务执行；累积收入成功资助了未来88小时自主运营。机器人通过代币经济实现了自主协调任务和支付。

Conclusion: RODEO展示了机器人与组织如何通过可审计的执行证明和链上结算协调任务和支付，为机器人自主性和组织集成提供了基于区块链的信任与问责解决方案。
...
### [196] [Multimodal Behavior Tree Generation: A Small Vision-Language Model for Robot Task Planning](https://arxiv.org/abs/2603.06084)
*Cristiano Battistini,Riccardo Andrea Izzo,Gianluca Bardaro,Matteo Matteucci*

Main category: cs.RO

TL;DR: 使用视觉语言模型生成机器人行为树，通过多阶段生成管道构建数据集，微调小型开源模型，在家庭任务中达到87%成功率


<details>
  <summary>Details</summary>
Motivation: 结合语言模型的任务规划能力和视觉语言模型的视觉理解能力，解决现有数据集缺乏视觉观察与可执行行为树关联的问题

Method: 1) 从现有机器人数据集构建视觉-行为树数据集；2) 使用大模型作为教师在多阶段生成管道中创建数据；3) 通过参数高效微调方法微调500M到4B参数的视觉语言模型；4) 生成与BehaviorTree.CPP兼容的行为树

Result: 微调后的4B参数视觉语言模型接近最先进闭源模型性能，在家庭任务中达到87%成功率，同时仅需少量计算资源

Conclusion: 小型开源视觉语言模型经过适当微调后，能够有效生成机器人行为树，在计算效率和性能之间取得良好平衡
...
### [197] [Sticky-Glance: Robust Intent Recognition for Human Robot Collaboration via Single-Glance](https://arxiv.org/abs/2603.06121)
*Yuzhi Lai,Shenghai Yuan,Peizheng Li,Andreas Zell*

Main category: cs.RO

TL;DR: 提出对象中心凝视接地框架，通过粘性凝视算法稳定意图识别，在动态多对象环境中实现高精度凝视交互


<details>
  <summary>Details</summary>
Motivation: 对于运动能力严重受限的残障人士，凝视是重要的交流方式，但在多对象环境中，由于凝视噪声、微眼跳、视角变化和动态对象等因素，稳健的凝视意图识别面临挑战

Method: 提出对象中心凝视接地框架，采用粘性凝视算法联合建模几何距离和方向趋势来稳定意图，即使只有3个凝视样本的短暂注视也能将意图锚定到对象上，并引入连续共享控制和多模态交互范式

Result: 动态目标跟踪率达到0.94，静态目标选择准确率达到0.98，任务持续时间减少近10%，相比代表性基线方法在鲁棒性、效率和减少工作负荷方面均有提升

Conclusion: 提出的对象中心凝视接地框架通过粘性凝视算法有效解决了多对象环境中凝视意图识别的挑战，实现了高精度、高效率的人机交互，为残障人士提供了更可靠的辅助交互手段
...
### [198] [A Hazard-Informed Data Pipeline for Robotics Physical Safety](https://arxiv.org/abs/2603.06130)
...
<details>
  <summary>Details</summary>
Motivation: 需要将传统安全工程与数字孪生、合成数据生成和机器学习模型训练相结合，建立系统化的机器人物理安全保障框架

Method: 基于显式资产声明、系统化漏洞枚举和危险驱动的合成数据生成，通过形式化的危险本体论实现安全边界学习

Result: 建立了连接传统安全工程、数字孪生仿真、合成数据生成和机器学习模型训练的统一框架

Conclusion: 该框架成功将经典风险工程与现代机器学习流程相结合，为机器人物理安全提供了系统化的解决方案
...
### [199] [Dual-Agent Multiple-Model Reinforcement Learning for Event-Triggered Human-Robot Co-Adaptation in Decoupled Task Spaces](https://arxiv.org/abs/2603.06163)
*Yaqi Li,Zhengqi Han,Huifang Liu,Steven W. Su*

Main category: cs.RO

TL;DR: 提出了一种基于事件触发的双智能体多模型强化学习共享控制康复策略，用于6自由度上肢康复机器人，通过解耦空间轴和自适应参数调整来优化人机协作


<details>
  <summary>Details</summary>
Motivation: 传统固定频率控制在复杂到达任务中常因逆运动学执行时间变化导致轨迹振荡，需要一种能安全高效优化人机协同适应的控制策略

Method: 1) 事件驱动策略：当末端执行器进入以目标航点为中心的准入球体时才触发控制动作；2) DAMMRL框架：人类智能体选择准入球体半径反映速度-精度权衡，机器人智能体动态调整3D笛卡尔步长以适应用户认知状态

Result: 该方法有效抑制了航点抖动，平衡了空间精度与时间效率，在物体获取任务中显著提高了成功率，并在半虚拟设置（物理压力传感器连接MuJoCo仿真）中得到验证

Conclusion: 事件触发的DAMMRL方法为康复机器人提供了一种有效的共享控制策略，通过解耦空间轴和自适应参数调整实现了更安全高效的人机协同适应
...
### [200] [DreamToNav: Generalizable Navigation for Robots via Generative Video Planning](https://arxiv.org/abs/2603.06190)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于固定路径点的导航方式不够直观，需要用户提供精确的路径规划。本文旨在通过自然语言交互让机器人能够理解模糊的人类指令，并生成物理一致的导航行为，减少任务特定的工程需求

Method: 使用Qwen 2.5-VL-7B-Instruct将模糊的用户指令精炼为精确的视觉描述，然后用NVIDIA Cosmos 2.5视频基础模型合成机器人执行任务的物理一致视频序列，最后从合成视频中通过视觉姿态估计、机器人检测和轨迹恢复提取有效的运动学路径

Result: 在轮式移动机器人和四足机器人的室内导航任务中，DreamToNav实现了76.7%的成功率，最终目标误差通常在0.05-0.10米之间，轨迹跟踪误差低于0.15米

Conclusion: 从生成视频预测中提取的轨迹可以在不同运动平台的物理机器人上可靠执行，视频生成可以作为规划引擎，使机器人能够在执行前"可视化"复杂行为，为避障和目标导向导航提供统一框架
...
### [201] [Towards Robotic Lake Maintenance: Integrating SONAR and Satellite Data to Assist Human Operators](https://arxiv.org/abs/2603.06266)
*Ahmed H. Elsayed,Christoph Manss,Tarek A. El-Mihoub,Andrej Lejman,Frederic Stahl*

Main category: cs.RO

TL;DR: 本文提出了一种两阶段方法，结合卫星遥感和无人水面船声纳技术，用于人工水体中水下植被的检测和针对性收割管理。


<details>
  <summary>Details</summary>
Motivation: 人工水体需要持续监测和维护以管理其生态系统，其中水下植被生长迅速，需要定期收割以保持生态平衡。传统方法劳动强度大，需要更高效、针对性的管理方案。

Method: 采用两阶段方法：第一阶段使用卫星遥感数据，通过水生植物和藻类指数初步检测水下植被区域；第二阶段使用配备多波束声纳的无人水面船进行高分辨率水深测量，精确定位和量化水生植被。

Result: 初步结果表明，卫星影像与水下声学传感的集成在人工湖泊植被管理中具有可行性，实现了人机协作，缩小了搜索范围，减少了人工操作者的工作量。

Conclusion: 该方法为人工水体中水下植被的针对性收割提供了有效的技术方案，通过卫星引导无人船任务和详细声纳地图支持，使收割过程更加高效且劳动强度更低。
...
### [202] [SuperSuit: An Isomorphic Bimodal Interface for Scalable Mobile Manipulation](https://arxiv.org/abs/2603.06280)
...
<details>
  <summary>Details</summary>
Motivation: 移动机械臂需要连续的SE(2)运动学和精确操作的协调，现有遥操作和可穿戴接口存在局限性，高质量长时程演示数据获取成为瓶颈。

Method: 提出SuperSuit双模态数据采集框架，支持遥操作和主动演示两种模式。将自然人类步态映射到连续平面基座速度，使用严格同构的可穿戴臂，在策略训练中采用平移不变的delta关节表示来减轻校准偏移和结构柔顺性。

Result: 主动模式演示吞吐量比遥操作基线提高2.6倍；在固定数据集大小下，用主动演示替代遥操作数据可获得可比的策略性能；随着主动数据量增加，性能呈现单调提升。

Conclusion: 跨采集模式的一致运动学表示能够实现长时程移动操作的可扩展数据采集，为移动机械臂的高质量演示数据获取提供了有效解决方案。
...
### [203] [Open-Source Based and ETSI Compliant Cooperative, Connected, and Automated Mini-Cars](https://arxiv.org/abs/2603.06343)
...
TL;DR: 本文提出使用1:10比例缩小的协同、自主、互联迷你车作为从仿真到实际实验的中间步骤，以降低自动驾驶技术开发成本


<details>
  <summary>Details</summary>
Motivation: 汽车行业正从人类控制向完全自动化、互联和协同的车辆发展，但新算法和协议的现场测试成本高昂，从仿真到实际实验需要巨大投资，可能成为技术发展的障碍

Method: 开发1:10比例缩小的迷你车平台，配备Jetson Orin板运行ROS2系统实现自主操作，使用Raspberry Pi板搭载开源OScar软件实现ETSI C-ITS标准兼容的协同通信

Result: 通过实现Day-1交叉口碰撞预警应用案例，验证了该平台的可行性和潜力，展示了其在协同智能交通系统开发中的实用价值

Conclusion: 提出的迷你车平台为自动驾驶技术开发提供了成本效益高的中间测试步骤，有助于降低从仿真到实际实验的过渡成本，促进协同智能交通系统的发展
...
### [204] [Safe Consensus of Cooperative Manipulation with Hierarchical Event-Triggered Control Barrier Functions](https://arxiv.org/abs/2603.06356)
...
<details>
  <summary>Details</summary>
Motivation: 多机械臂协同运输重型或大型负载需要在变化环境中实现协调的队形跟踪，同时强制执行严格的安全约束，而通信和实时计算资源有限。

Method: 开发基于共识的协议实现任务空间平移和旋转一致性，提出三层分层事件触发安全架构与CBFs集成，采用风险感知领导者选择和平滑切换策略降低在线计算。

Result: 通过两个Franka机械臂的真实硬件实验和综合仿真验证，在严格安全约束下实现更高精度的协同，相比基线方法显著降低计算成本和通信频率。

Conclusion: 提出的分布式控制框架能够有效实现多机械臂协同运输的安全共识协调，在保证安全性的同时大幅降低计算和通信开销。
...
### [205] [Data Analogies Enable Efficient Cross-Embodiment Transfer](https://arxiv.org/abs/2603.06450)
*Jonathan Yang,Chelsea Finn,Dorsa Sadigh*

Main category: cs.RO

TL;DR: 该研究探讨了如何组织异构机器人演示数据以优化跨机器人设置的性能迁移，发现不同形态变化需要不同的数据策略：感知变化（如视角）受益于广泛多样性，而形态变化则更需要数据类比（配对的跨实施例演示）。


<details>
  <summary>Details</summary>
Motivation: 通用机器人策略在多种机器人、场景和视角的演示数据上训练，但如何有效组织和扩展这种异构数据以真正提升特定目标设置中的性能仍不清楚。研究旨在确定哪种形式的演示数据最能促进跨机器人设置的迁移。

Method: 通过控制实验，系统改变末端执行器形态、机器人平台外观和相机视角，比较单纯增加演示数量与以不同方式系统扩展多样性的效果。在模拟环境中测试不同数据组织策略，并将模拟结果应用于真实世界改进。

Result: 模拟实验显示：感知变化（如视角）从广泛多样性中获益最大，而形态变化从非结构化多样性中获益较少，最大增益来自数据类比（即跨不同实施例对齐场景、任务和/或轨迹的配对演示）。基于模拟结果，仅通过改变数据组成，在真实世界中将跨实施例迁移成功率平均提高了22.5%。

Conclusion: 跨机器人设置的迁移效果取决于数据组织方式而非单纯的数据规模。针对不同类型的转移（感知vs形态）需要采用不同的数据策略：感知变化需要广泛多样性，形态变化则需要结构化的数据类比。这种数据组成优化能显著提升真实世界的跨实施例迁移性能。
...
### [206] [History-Conditioned Spatio-Temporal Visual Token Pruning for Efficient Vision-Language Navigation](https://arxiv.org/abs/2603.06480)
*Qitong Wang,Yijun Liang,Ming Li,Tianyi Zhou,Christopher Rasmussen*

Main category: cs.RO

TL;DR: 提出无需训练的空时视觉token剪枝框架，用于VLA模型的视觉语言导航，在保持导航精度的同时显著提升推理效率


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在视觉语言导航中表现良好但计算成本高，延迟大，限制了实时部署。需要在不重新训练或修改预训练模型的情况下减少冗余计算，实现高效的长时推理

Method: 提出训练free的空时视觉token剪枝框架：1) 对当前视图应用空间token选择；2) 对历史记忆进行空时压缩；3) 基于注意力机制的token重要性评估和查询引导的空时过滤，保留导航相关信息

Result: 在标准VLN基准测试中显著优于现有剪枝策略，在极端剪枝场景下仍能保持优越的导航精度，同时保持高度竞争力的推理效率。在Unitree Go2四足机器人上的实际部署验证了可靠的低延迟指令跟随导航

Conclusion: 该工作有助于弥合大规模多模态建模与高效实时机器人导航部署之间的差距，实现了即插即用的集成到现有VLA系统中
...
### [207] [Control Barrier Corridors: From Safety Functions to Safe Sets](https://arxiv.org/abs/2603.06494)
*Ömür Arslan,Nikolay Atanasov*

Main category: cs.RO

TL;DR: 该论文提出了一种新的"控制屏障走廊"概念，将控制屏障函数和安全运动走廊两种安全方法统一起来，用于自主机器人在未知环境中的安全路径跟随。


<details>
  <summary>Details</summary>
Motivation: 自主机器人在非结构化复杂环境中安全运行是一个关键需求。控制屏障函数和安全运动走廊是两种广泛使用但技术不同的安全方法，分别属于功能性和几何性方法。论文旨在统一这两种方法，为机器人提供更有效的安全保障。

Method: 提出控制屏障走廊的新概念，将控制屏障函数转换为局部安全目标区域，用于反馈控制系统中的参考目标选择。通过完全驱动系统、运动学独轮车和线性输出调节系统的示例，展示了在控制收敛率与屏障衰减率匹配时，单个状态安全性可以在控制屏障走廊上局部扩展。

Result: 控制屏障走廊能够在系统运动过程中持续变化的屏障走廊上实现安全可达的持久目标选择。该方法在自主探索未知环境中的可验证安全和持久路径跟随方面得到了验证。

Conclusion: 控制屏障走廊统一了功能性和几何性安全方法，揭示了安全性和反应性之间的权衡关系，为自主机器人在复杂环境中的安全运动规划和控制提供了新的理论框架。
...
### [208] [CFEAR-Teach-and-Repeat: Fast and Accurate Radar-only Localization](https://arxiv.org/abs/2603.06501)
*Maximilian Hilger,Daniel Adolfsson,Ralf Becker,Henrik Andreasson,Achim J. Lilienthal*

Main category: cs.RO

TL;DR: CFEAR-TR是一个基于单旋转雷达的teach-and-repeat定位系统，专门为恶劣天气条件下的自主导航设计，通过联合对齐实时扫描与存储地图及滑动窗口关键帧，实现高精度鲁棒定位。


<details>
  <summary>Details</summary>
Motivation: 在恶劣天气条件下，光学传感器可能失效，因此需要可靠的地图定位方法。雷达传感器在恶劣天气下表现良好，但现有雷达定位方法在精度和鲁棒性方面仍有提升空间。

Method: 使用单旋转雷达，通过多普勒补偿测量计算稀疏定向表面点表示雷达扫描。定位时联合对齐实时扫描与存储的地图扫描以及滑动窗口中的实时关键帧。地图存储在姿态图中，在定位时遍历该图。

Result: 在Boreas数据集的测试序列上，CFEAR-TR实现了0.117米和0.096°的定位精度，相比先前最优方法提升高达63%，运行频率达29Hz，大幅缩小了与激光雷达定位的差距。

Conclusion: CFEAR-TR提供了一种轻量级、易于部署且鲁棒的雷达定位解决方案，在恶劣天气条件下实现了接近激光雷达水平的定位精度，特别是在航向估计方面表现优异。
...
### [209] [Underactuated multimodal jumping robot for extraterrestrial exploration](https://arxiv.org/abs/2603.06525)
*Neil R. Wagner,Justin K. Yim*

Main category: cs.RO

TL;DR: 提出一种仅使用三个执行器的欠驱动单足机器人，通过两个反应轮控制空间姿态，实现滚动、定向跳跃、着陆和自扶正等多模式运动，适用于低重力环境探索。


<details>
  <summary>Details</summary>
Motivation: 设计用于低重力天体探索的多模式运动机器人，通过简单机械结构实现多种运动能力，以适应不同地形和环境条件。

Method: 使用仅两个反应轮控制空间姿态，配备两种控制器：地面平衡控制器用于瞄准跳跃方向，空中重定向控制器用于瞄准着陆腿姿态。系统总共只有三个执行器。

Result: 成功演示了滚动、定向跳跃和着陆、自扶正等功能，系统尺寸为0.33米，重量1.25公斤，能够在不同地形和环境条件下切换运动模式。

Conclusion: 通过简单的执行器配置和控制器切换，实现了欠驱动单足机器人的多模式运动能力，为低重力环境探索提供了紧凑高效的解决方案。
...
### [210] [Unified Learning of Temporal Task Structure and Action Timing for Bimanual Robot Manipulation](https://arxiv.org/abs/2603.06538)
*Christian Dreher,Patrick Dormanns,Andre Meixner,Tamim Asfour*

Main category: cs.RO

TL;DR: 提出一种从人类演示中学习符号和次符号时间任务约束的方法，用于生成双手机器人操作的时间参数化计划


<details>
  <summary>Details</summary>
Motivation: 双手机器人操作需要同时考虑高层符号时间关系（如动作先后顺序）和低层具体时间参数（如动作开始时间和持续时间）。现有方法将这两个层面孤立处理，导致高层任务规划与低层运动同步之间存在鸿沟。

Method: 1) 使用基于多元高斯混合模型的3维时间表示来捕捉动作间的次符号时间关系；2) 基于DPLL算法的方法寻找并排序所有无矛盾的Allen关系分配，表示任务的不同模式；3) 基于优化的规划系统结合符号和次符号时间约束，生成时间参数化的执行计划。

Result: 在多个数据集上评估表明，该方法生成的时间参数化计划比最具代表性的演示基线更接近人类演示。

Conclusion: 该方法成功填补了高层符号时间推理与低层具体时间参数之间的鸿沟，为双手机器人操作提供了统一的时间约束学习和规划框架。
...
### [211] [Uncertainty-Aware Adaptive Dynamics For Underwater Vehicle-Manipulator Robots](https://arxiv.org/abs/2603.06548)
*Edward Morgan,Nenyi K Dadson,Corina Barbalata*

Main category: cs.RO

TL;DR: 提出了一种用于水下车辆-机械臂系统的不确定性感知自适应动力学模型框架，该框架保持线性参数化，嵌入凸物理一致性约束，并通过移动水平估计实现在线参数估计和不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 水下车辆-机械臂系统受水动力效应影响，参数随时间变化，需要准确且自适应的动态模型。现有方法在保持物理一致性、处理时变参数和量化不确定性方面存在不足。

Method: 提出不确定性感知自适应动力学模型框架：1）保持车辆和机械臂集中参数的线性形式；2）嵌入凸物理一致性约束（惯性、阻尼、摩擦、流体静力学）；3）使用移动水平估计堆叠回归器，强制执行物理一致性并量化参数演化不确定性。

Result: 在BlueROV2 Heavy和4自由度机械臂上的实验显示：机械臂拟合R²=0.88-0.98，斜率接近1；车辆纵荡、垂荡和横摇在强耦合和噪声下良好重现；中值求解时间约0.023秒/更新；与固定参数模型相比，各自由度MAE和RMSE一致降低；参数物理合理，置信区间覆盖率接近100%。
...
Method: 1. 在可微分布局优化过程中集成显式可达性约束；2. 引入智能体感知推理，支持多种实体类型；3. 提出局部细化阶段，选择性重新优化问题对象放置

Result: RoboLayout保持了LayoutVLM的语义对齐和物理合理性，同时增强了面向智能体的室内场景生成适用性，在不同场景配置中表现出色

Conclusion: RoboLayout通过智能体感知推理和优化稳定性改进，成功生成了既语义连贯又适合具身智能体交互的室内场景布局
...
### [213] [Reasoning Models Struggle to Control their Chains of Thought](https://arxiv.org/abs/2603.05706)
*Chen Yueh-Han,Robert McCarthy,Bruce W. Lee,He He,Ian Kivlichan,Bowen Baker,Micah Carroll,Tomek Korbak*

Main category: cs.AI

TL;DR: 该研究评估了语言模型对思维链（CoT）的控制能力，发现当前模型对思维链的控制能力远低于对最终输出的控制能力，这为思维链监控提供了乐观信号。


<details>
  <summary>Details</summary>
Motivation: 思维链监控是检测现代推理模型不当行为和理解其动机的重要工具，但如果模型能够控制其在思维链中的表达内容，可能会破坏思维链的可监控性。研究旨在衡量这种不良能力——思维链控制能力。

Method: 研究者引入了CoT-Control评估套件，包含要求模型在遵守思维链指令的同时解决问题的任务（例如，在不使用"染色体"一词的情况下推理遗传学问题）。评估了不同规模模型、不同RL训练程度、不同计算资源下的控制能力，并测试了激励、监控告知和对抗性提示优化等场景。

Result: 研究发现：1）推理模型的思维链控制能力显著低于输出控制能力（Claude Sonnet 4.5的CoT控制成功率仅2.7%，而输出控制为61.9%）；2）模型越大控制能力越高，但RL训练越多、测试时计算越多、问题越难，控制能力越低；3）即使给予激励逃避监控，模型控制能力仍然有限；4）告知被监控时控制能力略有提高；5）对抗性提示优化无法显著提高控制能力。

Conclusion: 研究结果谨慎乐观地表明，目前思维链控制能力不太可能成为思维链监控的失效模式。但低控制能力的机制尚不清楚，鉴于其对维持思维链可监控性的重要性，建议前沿实验室在未来模型中跟踪思维链控制能力。
...
### [214] [Evolving Medical Imaging Agents via Experience-driven Self-skill Discovery](https://arxiv.org/abs/2603.05860)
*Lin Fan,Pengyu Dai,Zhipeng Deng,Haolin Wang,Xun Gong,Yefeng Zheng,Yafei Ou*

Main category: cs.AI

TL;DR: MACRO是一个自演化的医学智能体，能够从已验证的执行轨迹中自主发现有效的多步骤工具序列，将其合成为可复用的复合工具，实现从静态工具组合到经验驱动工具发现的转变，提高多步骤编排的准确性和跨领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 临床图像解释本质上是多步骤和工具中心的，现有基于LLM的智能体系统将工具集和调用策略视为静态部署后不变，这种设计在面对现实世界的领域转移、跨任务和不断演变的诊断需求时变得脆弱，预定义的工具链经常退化并需要昂贵的手动重新设计。

Method: MACRO采用经验增强的自演化医学智能体框架：1）从已验证的执行轨迹中自主识别重复出现的有效多步骤工具序列；2）将这些序列合成为可复用的复合工具；3）将这些复合工具注册为新的高级原语，持续扩展行为库；4）使用轻量级图像特征记忆将工具选择基于视觉-临床上下文；5）采用GRPO式训练循环强化可靠调用发现的复合工具。

Result: 在多样化的医学影像数据集和任务上的广泛实验表明，自主复合工具发现相比强基线方法和最新的智能体方法，持续提高了多步骤编排的准确性和跨领域泛化能力，弥合了脆弱的静态工具使用与自适应、上下文感知的临床AI辅助之间的差距。

Conclusion: MACRO通过从静态工具组合转向经验驱动的工具发现，实现了自演化的医学智能体，能够自主发现和合成有效的多步骤工具序列作为可复用的复合工具，在减少监督的情况下实现闭环自我改进，为临床AI辅助提供了更适应性强、上下文感知的解决方案。
...
### [215] [The World Won't Stay Still: Programmable Evolution for Agent Benchmarks](https://arxiv.org/abs/2603.05910)
...
Result: 成功将单个环境演化为200个环境和3000个任务沙箱，并据此对代表性智能体进行了基准测试，验证了框架的有效性。

Conclusion: ProEvolve提供了一种可编程的环境演化框架，能够生成动态变化的环境，为评估智能体对现实世界动态变化的适应能力提供了更好的测试平台。
...
TL;DR: 本文提出了一种新的聚合语义家族，用于处理具有内在权重的定量双极论证框架，通过将攻击者和支持者分开聚合的三阶段计算，增强了语义的可解释性和参数化能力。


<details>
  <summary>Details</summary>
Motivation: 在定量双极论证框架中，现有的渐进语义通常对称处理攻击者和支持者，但在实际应用中，攻击者和支持者往往扮演不对称的角色。需要一种更灵活、更可解释的语义计算方法来更好地建模这种不对称性。

Method: 提出聚合语义家族，采用三阶段计算：1) 单独计算攻击者的全局权重；2) 单独计算支持者的全局权重；3) 将这两个值与论证的内在权重进行聚合。这种方法保持双极性，比现有文献中的方法更进一步。

Result: 聚合语义提供了更参数化的计算框架，能够处理攻击者和支持者不对称的情况。通过500种聚合语义的测试和比较，展示了该方法在行为范围上的多样性，同时保持了渐进语义的经典原则。

Conclusion: 聚合语义通过将计算分解为三个独立且可解释的步骤，实现了更可理解的渐进语义，增强了双极论证框架的建模能力和实际应用价值。
...
TL;DR: CliqueFlowmer：一种基于离线模型优化的材料发现新方法，结合团簇优化与生成模型，超越传统生成方法


<details>
  <summary>Details</summary>
Motivation: 传统基于最大似然训练的生成模型在材料发现中难以有效探索有前景的材料空间区域，需要一种能直接优化目标属性的替代方法

Method: 提出CliqueFlowmer模型，将团簇基模型优化（MBO）的最新进展融入Transformer和流生成中，实现目标材料属性的直接优化

Result: CliqueFlowmer在优化能力上表现优异，其生成的材料性能显著优于传统生成基线方法

Conclusion: CliqueFlowmer为计算材料发现提供了一种有效的离线模型优化方法，已开源代码以支持专业材料优化问题和跨学科研究
...
### [218] [Artificial Intelligence for Climate Adaptation: Reinforcement Learning for Climate Change-Resilient Transport](https://arxiv.org/abs/2603.06278)
...
<details>
  <summary>Details</summary>
Motivation: 气候变化加剧降雨和洪水，对城市交通系统造成更大破坏。制定有效适应策略面临长期基础设施投资、深度气候不确定性以及洪水-基础设施-交通复杂交互等多重挑战

Method: 提出基于强化学习的决策支持框架，作为综合评估模型，整合降雨预测、洪水建模、交通模拟，量化基础设施和交通的直接间接影响，学习平衡投资成本与避免影响的适应策略

Result: 在哥本哈根内城2024-2100年的案例研究中，框架优于传统优化方法，发现协调的空间和时间适应路径，学习影响减少与适应投资之间的权衡，产生更具韧性的策略

Conclusion: 强化学习作为灵活决策支持工具，在气候不确定性下进行适应性基础设施规划具有巨大潜力
...
### [219] [SAHOO: Safeguarded Alignment for High-Order Optimization Objectives in Recursive Self-Improvement](https://arxiv.org/abs/2603.06333)
*Subramanyam Sahoo,Aman Chadha,Vinija Jain,Divya Chaudhary*

Main category: cs.AI

TL;DR: SAHOO框架通过多信号检测器、约束保持检查和回归风险量化来监控和控制递归自我改进中的对齐漂移，在代码生成、数学推理和真实性任务中实现显著质量提升同时保持安全约束。
...
Method: 提出SAHOO框架，包含三个核心组件：(1)目标漂移指数(GDI)，结合语义、词汇、结构和分布度量的多信号检测器；(2)约束保持检查，强制执行安全关键不变量；(3)回归风险量化，标记可能撤销先前改进的改进周期。

Result: 在189个代码生成、数学推理和真实性任务中，SAHOO实现显著质量提升（代码任务18.3%，推理16.8%），同时在两个领域保持约束，在真实性任务中保持低违规率。阈值在18个任务的验证集上校准。

Conclusion: SAHOO使递归自我改进过程中的对齐保持变得可测量、可部署且可系统验证，揭示了能力-对齐前沿，显示早期改进周期高效但后期对齐成本上升，以及领域特定张力。
...
### [220] [Talk Freely, Execute Strictly: Schema-Gated Agentic AI for Flexible and Reproducible Scientific Workflows](https://arxiv.org/abs/2603.06394)
...
TL;DR: 该论文提出了一种名为"模式门控编排"的方法来解决LLM在科学工作流中确定性与灵活性之间的权衡问题，通过模式验证确保工作流的确定性执行，同时保持对话的灵活性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型可以将研究人员的自然语言目标转化为可执行计算，但科学工作流需要确定性、可追溯性和治理保障，而LLM自主决定执行内容难以满足这些要求。研究发现工业研发中存在两个相互冲突的需求：确定性的约束执行与不僵化的对话灵活性。

Method: 提出模式门控编排作为解决方案，将模式作为组合工作流级别的强制执行边界，确保任何操作（包括跨步骤依赖）都必须通过机器可检查的规范验证才能执行。将需求操作化为执行确定性（ED）和对话灵活性（CF），使用多模型协议对20个系统进行评分。

Result: 研究发现现有系统在确定性和灵活性之间存在权衡，形成了经验帕累托前沿，没有系统能同时实现高灵活性和高确定性。多模型LLM评分显示出高度一致性（ED的α=0.80，CF的α=0.98），表明多模型LLM评分可以作为人类专家评估的可重用替代方案。

Conclusion: 模式门控架构通过分离对话权限和执行权限，有望解耦确定性与灵活性之间的权衡。提出了三个操作原则：执行前澄清、约束的计划-行动编排、以及工具到工作流级别的门控，以指导实际应用。
...
### [221] [Boosting deep Reinforcement Learning using pretraining with Logical Options](https://arxiv.org/abs/2603.06565)
*Zihan Ye,Phil Chau,Raban Emunds,Jannis Blüml,Cedric Derstroff,Quentin Delfosse,Oleg Arenz,Kristian Kersting*

Main category: cs.AI

TL;DR: H^2RL：一种混合分层强化学习方法，通过逻辑选项预训练策略引导智能体避免短期奖励循环，实现长期目标导向行为


<details>
  <summary>Details</summary>
Motivation: 深度强化学习智能体经常存在对齐问题，过度利用早期奖励信号；纯符号方法难以扩展到连续环境；需要结合人类获取新技能的能力，将符号结构注入神经强化学习智能体

Method: 提出混合分层强化学习（H^2RL）框架，采用两阶段方法：1）逻辑选项预训练策略引导学习策略远离短期奖励循环；2）通过标准环境交互精炼最终策略，保持深度策略的表达能力

Result: 该方法在长期决策制定方面持续改进，智能体性能优于强大的神经、符号和神经符号基线方法

Conclusion: H^2RL成功将符号结构注入神经强化学习智能体，解决了对齐问题，改善了长期决策能力，同时保持了深度策略的表达能力
