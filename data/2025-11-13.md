...
Method: 采用OCR、视觉LLM、目标检测、关系推理和优化算法的协同使用，并扩展了最先进的场景图生成模型，引入全新的Transformer架构来深入分析工厂组件间的复杂关系。

Result: 在设计数据文本信息提取方面达到91%的准确率；在工厂拓扑方面，93%的组件被正确识别，层次结构提取准确率约为80%。

Conclusion: 基于人工智能技术的协同使用成功克服了由于缺乏标准化导致的数据多样性障碍，为能源基础设施的数字化提供了有效的自动化解决方案。
...
Method: 提出包含三个互补测试设置的系统评估框架：静默输入生成、不匹配的音频-视频配对、匹配的音频-视频合成，并引入嘴唇同步差异和基于静默音频的嘴唇同步分数等衍生指标。

Result: 该方法能够有效分析和量化嘴唇泄漏问题，并研究了不同身份参考选择对泄漏的影响，为参考设计提供见解。

Conclusion: 所提出的方法具有模型无关性，为说话人脸生成领域的未来研究建立了更可靠的评估基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理动态相机位置和复杂遮挡，需要更全面的数据集和框架来提升多视角监控性能。

Method: 提出实时相机标定、基于特征的图像配准和鸟瞰图多视角特征融合的深度学习框架。

Result: 在复杂环境中保持约90%的检测和跟踪准确率，成功跟踪约80%的轨迹，表现出良好的泛化能力和鲁棒性。
...
TL;DR: 提出了一种新的持续异常检测框架，使用统一内存库替代任务特定的子内存库，通过固定大小的核心集增量更新嵌入，在MVTec AD和Visa数据集上达到最先进的检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有的基于嵌入的持续异常检测方法需要为每个任务构建特定的子内存库，限制了方法的灵活性和可扩展性。

Method: 提出统一内存库框架，所有任务共享一个内存库，在训练期间通过固定大小的核心集增量更新嵌入，推理阶段使用最近邻匹配机制计算异常分数。

Result: 在MVTec AD和Visa数据集上分别达到0.972和0.891的平均图像级AUROC分数，在真实电子纸数据集上实现100%的异常样本检测准确率。

Conclusion: 该方法在保持高检测精度的同时提高了持续异常检测的灵活性和可扩展性，并在实际场景中表现出良好的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的事故预测对于主动安全至关重要，但面临两个关键挑战：传感器输入噪声或退化，以及需要平衡及时预警与误报抑制的可靠预测。

Method: 提出统一框架，包含扩散去噪模块（通过迭代细化重建抗噪声的图像和物体特征）和时间感知的actor-critic模型（利用长期时间推理和时间加权奖励确定最佳预警时机）。

Result: 在三个基准数据集（DAD、CCD、A3D）上的实验显示达到了最先进的准确率，显著提高了平均事故前时间，并在高斯和脉冲噪声下保持稳健性能。
...
TL;DR: 提出RS-Net框架，通过评分对象对的关系重要性来改进动态场景图生成，结合空间交互和长期时序上下文，可无缝集成到现有模型中提升性能。
...
Method: RS-Net包含空间上下文编码器（带可学习上下文标记）和时序编码器，通过评分对象对的关系重要性，并集成到统一的三元组评分机制中。

Result: 在Action Genome数据集上，RS-Net持续提升各种基线的召回率和精确率，在平均召回率上有显著提升，有效处理关系的长尾分布问题。

Conclusion: RS-Net能够在不改变架构的情况下集成到现有DSGG模型中，在保持竞争性效率的同时实现优于最先进方法的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频基础模型提取的时空特征在共享或存储时会无意中泄露敏感个人信息，而现有的像素级匿名化方法需要重新训练整个模型且产生任务特定的匿名化，不适合视频基础模型。
...
TL;DR: 本文研究了自回归逐像素预测的扩展特性，发现在固定32x32分辨率下，图像分类和图像生成的最优扩展策略存在差异，生成任务需要数据量增长比分类任务快3-5倍。随着分辨率增加，模型大小需比数据量增长更快，计算能力是主要瓶颈而非训练数据量。


<details>
  <summary>Details</summary>
Motivation: 探索自回归逐像素预测这一简单但研究不足的统一视觉模型框架的扩展特性，了解不同视觉任务（分类和生成）的最优扩展策略差异。

Method: 在32x32分辨率图像上训练一系列Transformer模型，使用IsoFlops配置文件，计算预算高达7e19 FLOPs，评估三个指标：逐像素预测目标、ImageNet分类准确率和Fréchet距离衡量的生成质量。

Result: 1) 在固定分辨率下，分类和生成的最优扩展策略不同，生成任务需要数据量增长快3-5倍；2) 分辨率增加时，模型大小需比数据量增长更快；3) 主要瓶颈是计算能力而非训练数据量。

Conclusion: 计算能力是逐像素图像建模的主要限制因素，随着计算能力每年增长4-5倍，预计未来5年内可实现逐像素图像建模。
...
TL;DR: 本文研究了使用扩散模型微调技术（如LoRA和DreamBooth）来生成更平衡的训练数据以解决图像分类中的群体偏见问题，通过聚类和分组训练提高数据生成质量。
...
Method: 探索多种扩散模型微调技术（LoRA和DreamBooth），在每个群体内对图像进行聚类并为每个聚类训练独立的DreamBooth模型，然后使用这些模型生成群体平衡的预训练数据，最后在真实数据上进行微调。

Result: 在多个基准测试中，所研究的微调方法平均优于原始Stable Diffusion，与最先进的去偏技术（如Group-DRO）结果相当，且随着数据集偏见严重程度的增加表现更优。

Conclusion: 扩散模型微调技术能有效生成更准确的群体平衡训练数据，在缓解图像分类偏见方面具有显著效果，特别是在高偏见场景下表现突出。
...
TL;DR: 本文提出DT-NVS，一种基于Transformer的3D感知扩散模型，用于从单张图像生成真实世界场景的新视角，解决了现有方法在小范围相机移动或人造物体场景上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散方法主要关注真实场景中的小相机移动或仅考虑不自然的物体中心场景，限制了在真实世界设置中的应用潜力。本文旨在突破这些约束，处理真实世界、多类别、未对齐的日常场景视频。

Method: 提出DT-NVS 3D感知扩散模型，采用Transformer架构骨干，改进Transformer和自注意力架构以将图像转换为3D表示，开发新颖的相机条件策略在未对齐数据集上训练，引入新的训练范式交换参考帧角色。

Result: 在单输入图像的广义新视角合成3D任务上评估，相比最先进的3D感知扩散模型和确定性方法有所改进，同时生成多样化输出。

Conclusion: DT-NVS模型在真实世界场景的新视角合成方面表现出色，通过创新的架构设计和训练策略实现了更好的性能和多样性。
...
<details>
  <summary>Details</summary>
Motivation: 现有旋转不变学习方法使用手工设计的RI特征替代原始坐标，虽然对任意旋转具有鲁棒性，但会丢失全局姿态信息，无法区分几何相似但空间位置不同的结构。这主要是由于受限感受野导致的翼尖特征崩溃问题。

Method: 提出了阴影信息姿态特征(SiPF)，通过从学习的共享旋转中推导出全局一致的参考点(阴影)，将局部RI描述符与全局姿态信息相结合。设计了旋转不变注意力卷积(RIAttnConv)操作符，将SiPF集成到特征聚合过程中。还开发了基于Bingham分布的任务自适应阴影定位模块。

Result: 在3D分类和部件分割基准测试上的广泛实验表明，该方法显著优于现有的RI方法，特别是在需要细粒度空间区分能力的任务中表现突出。

Conclusion: 所提出的方法通过结合局部旋转不变特征和全局姿态信息，有效解决了现有RI方法的局限性，为旋转不变学习提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 自动化、稳健且准确地识别胶质母细胞瘤中不同的组织学亚区域，有助于大规模形态学理解这种疾病，改善患者预后。

Method: 采用四步深度学习策略，使用EfficientNet架构的多个变体（B0-B4）在BraTS-Path 2024数据集上对6个组织病理学区域进行分类。

Result: EfficientNet-B1和B4在5折交叉验证中达到0.98的F1分数，但在验证集和测试集上分别降至0.546和0.517，显示模型泛化能力存在挑战。
...
Method: 使用VisNet及其增强变体，包括径向基函数神经元、基于马氏距离的学习和视网膜预处理，利用赫布学习和时间连续性原理构建不变表示。
...
### [14] [OUGS: Active View Selection via Object-aware Uncertainty Estimation in 3DGS](https://arxiv.org/abs/2511.09397)
*Haiyi Li,Qi Chen,Denis Kalkofen,Hsiang-Ting Chen*

Main category: cs.CV

TL;DR: 本文提出了OUGS框架，通过基于3D高斯原语物理参数的不确定性建模和语义分割集成，解决了复杂场景中特定对象重建效率低下的问题，实现了更有效的主动视图选择策略。


<details>
  <summary>Details</summary>
Motivation: 现有主动重建方法依赖场景级不确定性度量，这些度量常因无关背景干扰而产生偏差，导致在面向对象的任务中视图选择效率低下。需要更原则性的不确定性建模方法来提升特定对象的重建质量。

Method: 从3D高斯原语的物理参数（位置、尺度、旋转）直接推导不确定性，通过渲染雅可比传播参数协方差建立可解释的不确定性模型，并集成语义分割掩码生成面向对象的不确定性评分。

Result: 在公共数据集上的实验评估表明，该方法显著提高了3DGS重建过程的效率，相比现有最先进方法，在目标对象上实现了更高质量的重建，同时作为全局场景的鲁棒不确定性估计器。

Conclusion: OUGS框架通过物理基础的不确定性建模和对象感知的视图选择策略，有效解决了复杂场景中特定对象重建的挑战，为3D高斯泼溅技术提供了更高效和精确的重建解决方案。
...
TL;DR: 本文提出了一种非对称跨模态知识蒸馏（ACKD）框架SemBridge，用于处理语义一致性较弱的跨模态知识蒸馏问题，通过学生友好匹配模块和语义感知知识对齐模块来优化知识传输成本。


<details>
  <summary>Details</summary>
Motivation: 现实场景中成对模态数据有限，对称跨模态知识蒸馏（SCKD）应用受限，需要研究在弱语义一致性下的知识蒸馏方法。

Method: 提出SemBridge框架，包含学生友好匹配模块（利用自监督学习获取语义知识并动态选择相关教师样本）和语义感知知识对齐模块（使用拉格朗日优化寻找最优传输路径）。

Result: 在遥感场景分类任务中，使用多光谱和RGB图像构建的数据集上，与7种现有方法在6种不同模型架构上的对比实验表明，该框架达到了最先进的性能。
...
TL;DR: HitoMi-Cam是一种基于光谱反射特性的轻量级、形状无关的人员检测方法，可在无GPU的边缘设备上实现实时处理（23.2fps），在CNN性能下降的搜救场景中达到93.5%的平均精度。


<details>
  <summary>Details</summary>
Motivation: 解决CNN目标检测的形状依赖性限制，特别是在训练数据未包含的姿势下性能下降的问题，为灾难救援等形状不可预测环境提供补充检测方案。

Method: 利用服装的光谱反射特性，开发形状无关的人员检测方法HitoMi-Cam，并在资源受限的边缘设备上实现系统部署。

Result: 处理速度达到23.2fps（253x190像素），在模拟搜救场景中平均精度达93.5%，优于对比CNN模型的最佳精度53.8%，且误报率极低。

Conclusion: HitoMi-Cam不是替代CNN检测器的方案，而是在特定条件下（如形状不可预测的灾难救援环境）的补充工具，证明基于光谱的人员检测在边缘设备实时操作中的可行性。
...
Conclusion: NES方法有效解决了零样本图像描述中的幻觉问题，通过负实体抑制机制显著提升了跨域泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 边缘设备资源受限，量化可以提高内存效率和减少带宽需求，但现有研究很少探索VLMs的激进量化，特别是1B到2B参数的模型。
...
TL;DR: 本文提出了一种基于机器视觉导向压缩的人机协同压缩方法Diff-FCHM，通过将机器视觉作为基础，渐进聚合语义信息，并利用扩散先验恢复人类视觉的高保真细节。


<details>
  <summary>Details</summary>
Motivation: 现有的人机协同压缩方法主要基于人类视觉压缩流程，在整合机器视觉压缩时存在复杂度和比特率不足的问题。机器视觉仅关注图像/视频的核心区域，所需信息远少于人类视觉。

Method: 提出Diff-FCHM方法：基于机器视觉导向压缩，开发即插即用可变比特率策略，渐进聚合机器视觉压缩的语义信息，并利用扩散先验无缝恢复人类视觉的高保真细节。

Result: 实验结果表明，Diff-FCHM在机器视觉和人类视觉压缩方面均表现出持续优越的性能，具有显著优势。

Conclusion: 该方法首次成功实现了基于机器视觉导向的人机协同压缩，为同时服务人类感知和机器智能的图像/视频数据压缩提供了有效解决方案。
...
TL;DR: 本文提出了分层蒸馏（HD）框架，将轨迹蒸馏和分布蒸馏从独立范式转变为协同组件，通过轨迹蒸馏建立结构"草图"，为后续分布蒸馏提供近最优初始化，结合自适应加权判别器（AWD）实现高效细节优化，在单步扩散模型中达到最先进性能。
...
Method: 提出分层蒸馏框架：1）利用轨迹蒸馏建立结构"草图"作为初始化；2）使用分布蒸馏进行细化；3）引入自适应加权判别器（AWD），通过动态分配token权重专注于局部缺陷，实现高效细节优化。
...
### [21] [HOTFLoc++: End-to-End Hierarchical LiDAR Place Recognition, Re-Ranking, and 6-DoF Metric Localisation in Forests](https://arxiv.org/abs/2511.09170)
...
<details>
  <summary>Details</summary>
Motivation: 解决森林环境中由于杂乱、自相似性和视角变化导致的地点识别和定位困难，特别是地面到地面和地面到空中场景的挑战。
...
Conclusion: HOTFLoc++在挑战性森林环境中实现了优越的地点识别和定位性能，运行时间比RANSAC快两个数量级，是解决复杂环境定位问题的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 神经B帧视频压缩在双向参考帧利用方面存在不足，特别是层次编码中帧跨度较大时，两个参考帧的贡献可能不平衡，需要优化参考信息利用。
...
TL;DR: 本文提出了一种基于豪斯多夫维数（HD）的新方法，用于增强分形生成模型（FGM）生成图像的多样性，同时保持高质量。通过可学习的HD估计、动量调度策略和HD引导的拒绝采样，在ImageNet数据集上实现了39%的多样性提升。


<details>
  <summary>Details</summary>
Motivation: 分形生成模型虽然能生成高质量图像，但其固有的自相似性限制了输出图像的多样性。为了解决这个问题，需要引入能够量化结构复杂度的指标来增强生成结果的多样性。

Method: 1. 提出可学习的HD估计方法，直接从图像嵌入预测HD；2. 采用基于HD的损失函数和单调动量驱动调度策略，逐步优化超参数；3. 在推理阶段使用HD引导的拒绝采样来选择几何更丰富的输出。
...
<details>
  <summary>Details</summary>
Motivation: 随着无纸化工作流程的深入，签名逐渐从传统纸质转向电子格式。尽管存在动态压力敏感和基于PKI的数字签名，静态扫描签名因其便利性仍然普遍使用，但这些静态图像几乎失去了认证属性，无法可靠验证且易受恶意复制和重用。

Method: AuthSig利用人类视觉系统对细微风格变化不敏感的特性，在生成过程中精细调节风格嵌入来隐式编码水印位，实现"一次签名，一次使用"策略。为解决手写签名数据稀缺和传统增强方法的局限性，引入了关键点驱动的数据增强策略，有效增强风格多样性以支持鲁棒的水印嵌入。

Result: 实验结果显示，AuthSig在数字域失真和签名特定退化下实现了超过98%的提取准确率，即使在打印扫描场景下仍然有效。

Conclusion: AuthSig框架通过将认证信息绑定到签名图像，有效解决了静态电子签名的安全验证问题，为无纸化工作流程提供了可靠的签名认证解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 上下文学习的效果严重依赖于演示样本的选择，而传统方法（随机、相似性、信息分数采样）在效率和效果上存在不足，无法平衡两者的需求。

Method: 提出CoDR框架：1）使用聚类剪枝方法构建多样化核心集；2）设计双重检索机制，实现全局演示选择同时保持效率。

Result: 实验结果表明，该方法相比现有策略显著提升了上下文学习性能，为演示选择提供了有效且高效的解决方案。

Conclusion: CoDR框架通过多样化核心集和双重检索机制，成功解决了演示选择中效率与效果的平衡问题，为上下文学习提供了更优的方案。
...
TL;DR: 提出WDT-MD框架，通过小波扩散Transformer解决糖尿病视网膜病变微动脉瘤检测中的三个关键问题：身份映射、假阳性高和正常特征重建不佳。


<details>
  <summary>Details</summary>
Motivation: 微动脉瘤是糖尿病视网膜病变的最早病理标志，但手动筛查劳动密集且易出错。现有基于扩散的异常检测方法存在身份映射、假阳性高和正常特征重建不佳三大限制，阻碍临床应用。

Method: 提出WDT-MD框架，包含三个创新：噪声编码图像条件机制避免身份映射；伪正常模式合成提供像素级监督；小波扩散Transformer架构结合全局建模和多尺度分析。

Result: 在IDRiD和e-ophtha MA数据集上的实验表明，WDT-MD在像素级和图像级微动脉瘤检测中均优于最先进方法。
...
TL;DR: 提出了一种结合去噪项的变分分割模型，适用于受Gamma分布乘性噪声和泊松噪声污染的图像，通过自适应TV正则器和偏置场估计解决强度不均匀性问题，采用ICTM和RMSAV方案进行高效优化。


<details>
  <summary>Details</summary>
Motivation: 图像分割是图像处理的核心任务，但许多方法在图像被严重噪声污染和存在强度不均匀性时性能下降，需要开发能够同时处理噪声和强度不均匀性的鲁棒分割方法。

Method: 在ICTM框架下提出变分分割模型，集成去噪项（I-散度项和自适应TV正则器），使用灰度级指示器获得空间自适应权重，估计平滑变化的偏置场，采用RMSAV方案进行优化。
...
<details>
  <summary>Details</summary>
Motivation: 当前开放集目标检测器仅依赖基于文本描述或视觉示例的正提示，在面对视觉相似但语义不同的干扰物时表现脆弱，需要引入负提示来提升鲁棒性。

Method: 提出了统一的视觉提示编码器处理正负视觉提示，设计了无需训练的负向负计算模块动态抑制负响应，以及负向负铰链损失通过微调增强正负嵌入的判别性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于语言解码器的缓解方法通常独立调节视觉或文本注意力，忽视了它们作为两个关键因果因素的交互作用，无法有效解决大视觉语言模型中的目标幻觉问题。

Method: 提出Owl框架：1）通过结构因果图建模幻觉过程；2）引入VTACR指标量化模态贡献不平衡；3）设计细粒度的注意力干预机制动态调整token和layer级注意力；4）采用双路径对比解码策略，一条路径强调视觉接地预测，另一条放大幻觉预测。

Result: 在POPE和CHAIR基准测试上的实验结果表明，Owl实现了显著的幻觉减少，在保持视觉语言理解能力的同时，在忠实度方面达到了新的SOTA水平。

Conclusion: Owl通过因果建模和双模态注意力重加权，有效解决了LVLMs中的目标幻觉问题，为幻觉缓解提供了新的思路和方法。
...
Method: 采用密集跨尺度图像对齐模型，利用跨尺度特征相关性，通过调整使用的尺度数量实现精度与效率的权衡，引入全空间相关模块和恰可察觉差异机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时对抗攻击方法（如TTC）由于优化目标与对抗攻击存在根本差异，仅基于对抗输入的梯度生成对抗攻击，导致搜索空间狭窄，攻击多样性不足，无法充分中和广泛的对抗扰动。

Method: 提出方向正交对抗攻击（DOC），在对抗攻击优化中融入正交梯度方向和动量更新机制，扩展对抗攻击空间的探索；同时基于平均余弦相似度提出方向敏感度评分，增强样本区分能力并自适应调节对抗攻击强度。

Result: 在16个数据集上的广泛实验表明，DOC在各种攻击下都能提高对抗鲁棒性，同时保持竞争力的干净准确率。

Conclusion: 增强对抗攻击的多样性和覆盖范围对于提高测试时防御的对抗鲁棒性至关重要，DOC方法通过正交梯度方向和动量更新机制有效实现了这一目标。
...
Result: 提供了YOLO模型在文本和印章检测上的基线结果，以及多种二值化方法的基线性能。
...
TL;DR: PressTrack-HMR是一个基于压力信号的多人全局人体网格恢复方法，通过检测-跟踪策略从原始压力数据中分离不同个体的压力信号，然后为每个个体进行人体网格恢复。
...
Result: 该方法在多人HMR中表现出色，MPJPE为89.2mm，WA-MPJPE100为112.6mm，展示了触觉垫在多人动作识别中的潜力。

Conclusion: PressTrack-HMR证明了仅使用压力信号进行多人全局人体网格恢复的可行性，为基于压力的人类运动分析开辟了新途径。
...
Conclusion: DBINDS通过分析潜在空间动态而非像素特征，有效解决了现有AI生成视频检测器的泛化问题，为内容安全提供了新的解决方案。
...
TL;DR: 本文提出了一个用于评估皮肤科诊断多模态大语言模型的新框架，包含DermBench基准测试和DermEval自动评估器，通过4000张真实皮肤图像和专家认证的诊断叙述，实现临床意义强、可重复且可扩展的评估。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在皮肤科诊断中应用日益增多，但可靠的评估仍是临床部署的主要瓶颈，需要开发临床意义强、可重复且可扩展的评估方法。

Method: 构建DermBench基准测试，包含4000张真实皮肤图像和专家认证诊断叙述，使用LLM评分器在临床维度上评估候选叙述；训练DermEval无参考多模态评估器，根据图像和生成叙述输出结构化批评和评分。

Result: 在4500个案例的多样化数据集上，DermBench和DermEval与专家评分高度一致，平均偏差分别为0.251和0.117（满分5分），能够可靠测量不同多模态LLM的诊断能力和可信度。

Conclusion: 提出的评估框架能够实现临床意义强、可重复且可扩展的多模态LLM评估，为皮肤科诊断模型的可靠部署提供了有效工具。
...
Method: TACO将回答分解为原子查询，通过重述减少对措辞的敏感性，使用自一致性（黑盒）或自置信度（灰盒）聚合来估计置信度，最后用语言模型精炼答案。
...
TL;DR: RichKD提出了一种多教师知识蒸馏框架，融合传统视觉教师和CLIP的视觉-语言知识，通过多提示文本指导增强知识多样性，提高学生模型的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法主要依赖单模态视觉信息，缺乏知识多样性，忽视了跨模态表示的潜力。CLIP的视觉-语言知识作为补充监督源尚未充分探索。

Method: 提出RichKD框架，融合传统教师的logits和特征与CLIP的多提示文本指导，结合数据集特定知识和语义丰富的视觉线索。

Result: 方法在多个基准测试中优于现有基线，产生更自信可靠的预测，显著增加自信正确案例，减少自信错误案例，并在分布偏移和输入损坏下表现出更强鲁棒性。

Conclusion: 尽管方法简单，RichKD通过融合CLIP的跨模态知识有效提升了知识蒸馏性能，改善了类间一致性和蒸馏质量。
...
TL;DR: DensiCrafter是一个从多模态输入生成轻量级、自支撑3D空心结构的框架，通过优化密度场实现，无需物理模拟即可减少材料质量达43%。
...
Result: 在文本到3D任务中实现材料质量减少达43%，相比基线方法提高了稳定性并保持高几何保真度，实际3D打印实验验证了设计的可靠制造性和自支撑能力。

Conclusion: DensiCrafter能够无缝集成预训练的Trellis模型，有效生成可制造的轻量自支撑3D结构，在保持几何质量的同时显著减少材料使用。
...
<details>
  <summary>Details</summary>
Motivation: 传统的师生范式在半监督医学图像分割中容易受到图像模糊性的影响，导致错误监督和自我强化偏差。现有方法通常依赖外部修改，而忽略了框架内在的错误纠正潜力。
...
<details>
  <summary>Details</summary>
Motivation: PETR系列模型在3D检测基准上表现优异，但面临高计算成本和内存占用的部署挑战。现有量化方法直接应用于PETR会导致严重精度下降，主要由于多模态特征间显著幅度差异和非线性算子量化效率低。

Method: 提出三个关键创新：1）量化友好的LiDAR射线位置嵌入，用单点采样替代多点采样；2）双查找表算法，用两个级联线性查找表近似复杂非线性函数；3）数值稳定后量化，在softmax数值稳定后执行量化以减少注意力失真。

Result: 在PETR系列模型（PETR、StreamPETR、PETRv2、MV2d）上，FQ-PETR在W8A8量化下实现接近浮点精度（仅1%性能下降），同时减少高达75%的延迟，显著优于现有PTQ和QAT基线方法。

Conclusion: FQ-PETR成功解决了PETR系列模型量化部署中的关键挑战，为自动驾驶中的多视图3D检测提供了高效实用的量化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的零样本骨架动作识别方法遵循'对齐然后分类'范式，但面临两个基本问题：(1)由于不完美语义导致的脆弱点对点对齐；(2)受静态决策边界和粗粒度锚点限制的僵化分类器。
...
TL;DR: 提出了一种基于视觉的支气管镜导航框架，通过2D-3D配准实现内窥镜视图与术前CT的配准，并创建了首个公开合成基准数据集。
...
TL;DR: 本文构建了首个美式橄榄球运动员专用检测与跟踪数据集，解决了该领域缺乏标准化数据集的问题，并在拥挤场景下实现了准确的检测与跟踪。
...
Method: 构建首个美式橄榄球运动员专用检测与跟踪数据集，对多种检测和跟踪方法进行对比评估，包括微调检测模型和集成重识别模型到跟踪系统中。

Result: 即使在拥挤场景下也能实现准确检测与跟踪，微调检测模型相比预训练模型性能提升，集成微调检测器和重识别模型后跟踪精度显著优于现有方法。

Conclusion: 这项工作能够在传统方法难以应对的挑战性高密度场景中，实现对美式橄榄球运动员的鲁棒检测与跟踪。
...
TL;DR: 提出了一种双教师知识蒸馏框架，利用ViT和CNN教师协同指导轻量级CNN学生，通过差异感知教师加权和结构差异感知蒸馏策略，有效解决架构不匹配问题，在多个基准数据集上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: ViT在视频动作识别中性能强大但计算成本高，轻量级CNN效率高但精度不足。现有的跨架构知识蒸馏方法存在架构不匹配问题，且忽视了更强同构CNN教师的价值。

Method: 双教师知识蒸馏框架：1）差异感知教师加权 - 基于教师置信度和与学生预测差异动态融合ViT和CNN教师的预测；2）结构差异感知蒸馏 - 学生通过轻量级辅助分支学习ViT和CNN教师之间的残差特征，专注于可转移的架构差异。

Result: 在HMDB51、EPIC-KITCHENS-100和Kinetics-400等基准数据集上的实验表明，该方法始终优于最先进的蒸馏方法，在HMDB51上最大精度提升达5.95%。

Conclusion: 所提出的双教师知识蒸馏框架通过协同利用异构ViT教师和同构CNN教师，有效解决了跨架构知识蒸馏中的挑战，显著提升了轻量级CNN学生的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法主要依赖几何线索并独立预测每帧姿态，难以解决模糊运动和泛化到真实场景。受人类理解和预测运动方式启发，需要结合高级意图推理和时间一致性建模。

Method: 提出DreamPose3D扩散框架：1）使用从2D姿态序列提取的动作提示动态调节去噪过程；2）引入表示编码器，在注意力机制中融入运动学关节亲和性；3）使用幻觉姿态解码器预测时间一致的3D姿态序列。

Result: 在Human3.6M和MPI-3DHP数据集上所有指标均达到最先进性能。在广播棒球数据集上测试显示，尽管2D输入模糊嘈杂，仍能保持强健性能，有效处理时间一致性和意图驱动的运动变化。
...
### [47] [Thinking Forward and Backward: Multi-Objective Reinforcement Learning for Retrieval-Augmented Reasoning](https://arxiv.org/abs/2511.09109)
*Wenda Wei,Yu-An Liu,Ruqing Zhang,Jiafeng Guo,Lixin Su,Shuaiqiang Wang,Dawei Yin,Maarten de Rijke,Xueqi Cheng*

Main category: cs.CL

TL;DR: Bi-RAR是一个新颖的检索增强推理框架，通过双向评估中间步骤来改进多步推理场景中的RAG性能，使用基于Kolmogorov复杂度的双向信息距离和强化学习优化。
...
Conclusion: Bi-RAR通过双向评估和强化学习有效提升了检索增强推理在多步复杂场景中的性能，为改进RAG系统提供了新思路。
...
Method: 采用混合摘要方法（自动转述提取式摘要）来简化归因，并使用自定义拓扑结构来识别不同类别归因相关错误的比例。

Result: 论文探索了自动归因方法的可行性，重点关注如何将摘要句子准确链接到源文档的特定部分。

Conclusion: 混合摘要方法和自定义错误分类拓扑有助于改进自动归因过程，为信息源追踪提供更有效的解决方案。
...
TL;DR: GMTRouter是一个基于异构图学习的个性化LLM路由框架，通过建模用户-LLM-查询-响应的复杂交互关系，在少样本数据下有效学习用户偏好，实现个性化路由决策。
...
Method: 将多轮用户-LLM交互表示为包含用户、LLM、查询和响应四种节点类型的异构图，通过定制化的消息传递机制，在轻量级归纳图学习框架中从少样本数据学习用户偏好。

Result: 在多个数据集上，GMTRouter持续优于强基线，准确率提高0.9%至21.6%，AUC提高0.006至0.309。更重要的是，仅需少样本数据即可适应新用户和演化偏好，无需大量微调。

Conclusion: GMTRouter通过异构图建模有效解决了LLM路由中的个性化挑战，在少样本场景下实现了优越的性能和适应性，为个性化LLM路由提供了有效解决方案。
...
TL;DR: LLMs能够生成足够逼真的社交媒体对话，在人类评估中被误认为是真实对话的比例达到39%，其中Llama 3生成的对话被正确识别为AI生成的概率仅为56%，略高于随机水平。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在模拟在线社区和社交媒体对话方面的有效性，探索其在内容推荐算法测试和内容政策评估等应用中的潜力，同时验证LLM生成对话是否能欺骗人类。

Method: 从Reddit收集真实人类对话，使用Llama 3 70B和GPT-4o在相同主题上生成人工对话，通过并排展示给研究参与者进行识别测试。

Result: LLM生成的对话被误认为是人类创建内容的比例为39%，特别是Llama 3生成的对话被正确识别为AI生成的概率仅为56%，几乎与随机猜测相当。

Conclusion: LLMs能够生成足够逼真的社交媒体对话来欺骗人类读者，这既显示了在社会模拟方面的巨大潜力，也警示了LLMs可能被滥用于生成虚假社交媒体内容的风险。
...
TL;DR: 本文研究了大型语言模型在法律框架解释中的潜力与风险，特别是针对美国法典第18篇第175条关于生物武器的法律。研究发现LLMs在分析法律的同时存在生成不安全输出的漏洞，提出结合知识图谱和RAG的方法来评估LLM的法律理解能力和安全性。


<details>
  <summary>Details</summary>
Motivation: LLMs在复杂法律框架解释方面具有变革潜力，但同时也存在生成不安全输出（如生物武器制造步骤）的严重漏洞，需要系统评估其法律理解能力和安全机制。

Method: 提出结合知识图谱构建和检索增强生成（RAG）的方法论，通过结构化实验评估LLMs的法律违规识别准确性、禁止指令生成能力以及在生物武器相关场景中非法意图检测能力。

Result: 研究发现LLMs在推理和安全机制方面存在显著局限性，但通过增强安全协议和更强大的法律推理框架，可以开发出能够道德安全地协助敏感法律领域的LLMs。

Conclusion: 通过结合增强的安全协议和更强大的法律推理框架，这项研究为开发能够道德安全地协助敏感法律领域的LLMs奠定了基础，确保它们成为法律的保护者而非无意中的违反者。
...
### [52] [Retrieval-Augmented Generation of Pediatric Speech-Language Pathology vignettes: A Proof-of-Concept Study](https://arxiv.org/abs/2511.08600)
*Yilan Liu*
...
Method: 开发了多模型RAG系统原型，将精选的领域知识与工程化的提示模板相结合，支持五种商业和开源大语言模型。设计了七个涵盖不同障碍类型和年级水平的测试场景，使用多维评估标准对生成案例进行自动化质量评估。

Result: 概念验证展示了RAG增强生成儿科言语语言病理学案例的技术可行性。商业模型显示出边际质量优势，但开源替代方案达到了可接受的性能，表明具有隐私保护的机构部署潜力。知识库集成使生成内容与专业指南保持一致。

Conclusion: 该系统证明了RAG增强生成儿科言语语言病理学案例的技术可行性。在教育和研究实施前需要进行专家评审、学生试点测试和心理测量评估。未来应用可扩展到临床决策支持、自动IEP目标生成和临床反思培训。
...
TL;DR: LLM对齐算法（如RLHF和DPO）显著降低了输出多样性，作者提出Soft Preference Learning方法，通过解耦KL散度正则化中的熵和交叉熵项来精细控制生成多样性，在能力和对齐方面都优于标准方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM对齐算法严重降低了模型输出的多样性，导致文本结构重复、问题解决方式单一、社会观点范围狭窄，这主要是由于KL散度正则化过度加权多数意见造成的。

Method: 提出Soft Preference Learning方法，解耦KL散度正则化中的熵和交叉熵项，实现对LLM生成多样性的精细控制，类似于但优于标准温度缩放方法。

Result: 使用Soft Preference Learning训练的LLM在困难重复采样任务上获得更高准确率，输出具有更大的语义和词汇多样性，能代表更广泛的社会观点，并显示改进的logit校准。

Conclusion: Soft Preference Learning是标准偏好学习算法的帕累托改进，能有效解决对齐算法导致的多样性损失问题，在模型能力和对齐质量方面都有显著提升。
...
### [54] [Mina: A Multilingual LLM-Powered Legal Assistant Agent for Bangladesh for Empowering Access to Justice](https://arxiv.org/abs/2511.08605)
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉国低收入人群面临法律语言复杂、程序不透明和成本高的障碍，现有AI法律助手缺乏孟加拉语支持和特定司法管辖区适应。

Method: 采用多语言嵌入和基于RAG的工具链框架，实现检索、推理、翻译和文档生成，通过交互式聊天界面提供情境感知的法律草案、引用和平白语言解释。
...
TL;DR: 本文提出了一个框架来测试大语言模型在对抗性提示下的真实性保持能力，通过让模型自己生成和验证真假陈述，然后测试其对这些自生成谎言的抵抗力。


<details>
  <summary>Details</summary>
Motivation: 幻觉问题是LLM在高风险领域实际部署的关键挑战，需要测试模型在对抗性提示下的真实性保持能力。

Method: 框架包含三个步骤：1) 让LLM生成特定领域的真假陈述；2) 让LLM验证这些陈述的真假；3) 测试LLM对自生成谎言的抵抗力。

Result: 在电影和小说两个封闭领域的评估显示，不同LLM的抵抗力差异很大：Claude表现最强，GPT和Grok中等，Gemini和DeepSeek较弱。

Conclusion: 随着越来越多的人使用LLM进行信息检索，这些发现敲响了警钟，表明需要提高LLM在对抗性提示下的真实性保持能力。
...
TL;DR: 该研究提出Self-HarmLLM攻击场景，利用模型自身生成的模糊有害查询作为新输入来绕过防护机制，在多种LLM上验证了攻击有效性，并发现自动评估会高估越狱成功率。


<details>
  <summary>Details</summary>
Motivation: 现有防御机制假设攻击者来自外部，但未充分探索模型自身输出成为新攻击向量的可能性。本研究旨在验证这种自我攻击场景的可行性。

Method: 提出Self-HarmLLM方法，使用模型生成的Mitigated Harmful Query作为新输入，在GPT-3.5-turbo、LLaMA3-8B-instruct和DeepSeek-R1-Distill-Qwen-7B上进行Base、Zero-shot和Few-shot条件下的实验。

Result: Zero-shot条件下转换成功率最高达52%，越狱成功率最高达33%；Few-shot条件下转换成功率最高达65%，越狱成功率最高达41%。自动评估平均高估越狱成功率52%。

Conclusion: 该方法证明自我攻击场景是有效的，表明需要重新考虑防护机制设计并建立更稳健的评估方法。
...
TL;DR: 本文扩展了DisCoCirc形式化方法，将其应用于孟加拉语，并评估其在英孟翻译任务中的有效性。研究发现该框架在处理语言结构差异时存在局限性，与先前声称的减少语言官僚主义效果不符。


<details>
  <summary>Details</summary>
Motivation: 将DisCoCirc形式化方法从英语扩展到孟加拉语，评估其在跨语言翻译任务中的实际效果，特别是检验其声称的减少语言官僚主义的能力。

Method: 基于DisCoCirc的语法驱动框架，开发孟加拉语的类似形式化方法，应用于英孟翻译任务，并分析框架在处理语言结构差异时的表现。

Result: 研究发现DisCoCirc虽然对大部分语言有效，但在处理英孟语言结构差异时面临限制，即使在相对简单的句子中仍存在困难，与先前声称的效果不符。

Conclusion: DisCoCirc框架在跨语言翻译中存在局限性，特别是在处理结构差异时，这揭示了框架的约束并为未来改进提供了方向。同时发现连词与布尔逻辑之间存在联系。
...
### [58] [One-Topic-Doesn't-Fit-All: Transcreating Reading Comprehension Test for Personalized Learning](https://arxiv.org/abs/2511.09135)
*Jieun Han,Daniel Lee,Haneul Yoo,Jinsung Yoon,Junyeong Park,Suin Kim,So-Yeon Ahn,Alice Oh*

Main category: cs.CL

TL;DR: 提出了一种基于GPT-4o的个性化英语阅读理解测试生成方法，通过内容转创将RACE-C数据集改编为符合学生兴趣的阅读材料和题目，实验表明个性化材料能提高EFL学习者的理解力和动机保持。


<details>
  <summary>Details</summary>
Motivation: 在EFL教育中，个性化学习受到关注，学习者的参与度和动机对阅读理解至关重要。现有材料往往缺乏个性化，难以维持学习兴趣。

Method: 开发结构化内容转创流程：从RACE-C数据集出发，使用GPT-4o生成语言特征相似但语义符合个人兴趣的新文章和多选题；整合主题提取、布鲁姆分类法问题分类、语言特征分析和内容转创。

Result: 在韩国EFL学习者中的对照实验显示，使用个性化阅读材料的学生比使用非个性化材料的学生表现出更好的理解能力和动机保持。

Conclusion: 个性化英语阅读材料能有效提升EFL学习者的阅读理解水平和学习动机，证明了兴趣对齐内容在教育中的价值。
...
### [59] [LiteraryTaste: A Preference Dataset for Creative Writing Personalization](https://arxiv.org/abs/2511.09310)
...
TL;DR: 本文介绍了LiteraryTaste数据集，包含60人的阅读偏好数据，包括自述偏好和实际偏好注释。研究发现人们创作偏好存在差异，微调transformer编码器可达到75.8%和67.7%的准确率建模个人和集体偏好，自述偏好对建模实际偏好效用有限。


<details>
  <summary>Details</summary>
Motivation: 人们有不同的创意写作偏好，但现有大语言模型通常将不同个人品味视为整体进行训练。为开发个性化创意写作LLM，需要理解用户偏好的数据集。

Method: 引入LiteraryTaste数据集，包含60人的阅读习惯自述和100对短创意文本的偏好标注。使用transformer编码器进行微调建模偏好，并采用LLM驱动的可解释性管道分析偏好差异。

Result: 1) 人们在创意写作偏好上存在分歧；2) 微调transformer编码器在建模个人和集体实际偏好时分别达到75.8%和67.7%的准确率；3) 自述偏好在建模实际偏好方面效用有限。

Conclusion: 该工作为个性化创意写作技术奠定了基础，展示了建模个人偏好的可行性，并揭示了自述偏好与实际偏好之间的差异。
...
TL;DR: 本文研究了大型语言模型在急诊医学诊断中的应用，构建了一个集成五个主要LLM的超级学习器MEDAS，通过元学习方法整合各模型的诊断能力，将诊断准确率提升至70%，显著超过单个模型和人类医生的表现。


<details>
  <summary>Details</summary>
Motivation: 急诊医学中快速准确诊断对患者救治至关重要，AI特别是大型语言模型在医疗决策支持系统中展现出巨大潜力，但单个模型的诊断能力有限，需要探索如何整合多个模型的优势来提高诊断准确性。

Method: 构建了一个集成五个主要LLM（Gemini、Llama、Grok、GPT和Claude）的超级学习器MEDAS，采用元学习方法学习每个LLM的不同能力，通过集体能力提升诊断准确性。

Result: 单个LLM的诊断准确率在58%-65%之间，显著超过人类医生；超级学习器MEDAS将准确率提升至70%，其中至少一个集成LLM在相同设置下达到85%的正确诊断率。

Conclusion: 元学习方法整合的超级学习器能够利用所有LLM的集体知识，其诊断准确率超过任何单个LLM，表明集成多个模型可以显著提升医疗诊断系统的性能。
...
TL;DR: 提出了一种基于梯度感知的自适应数据选择方法GrADS，用于LLMs的监督微调，通过分析梯度来选择有效训练数据子集，显著提高效率并缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在众多任务中表现优异，但监督微调对于适应专业领域仍然必要。然而，领域专业化SFT资源密集且可能导致通用能力下降，存在灾难性遗忘问题。

Method: 设计了自引导标准，利用梯度的幅度和统计分布来优先选择对模型学习过程贡献最大的样本，通过初步训练阶段获得的梯度分析来识别有效训练数据子集。

Result: 在医学、法律和金融等多个领域的实验中，GrADS表现出显著效率和成本效益。仅使用5%的GrADS选择数据，LLMs性能已超过使用完整数据集微调的模型；使用50%数据时性能显著提升，同时大幅缓解了灾难性遗忘。

Conclusion: GrADS方法通过梯度感知的数据选择，有效提升了LLMs领域专业化的效率，在保持性能的同时显著降低了资源消耗和灾难性遗忘问题。
...
<details>
  <summary>Details</summary>
Motivation: 自杀是青少年第二大死因，过去自杀企图是未来自杀的重要预测因子。许多有自杀想法的人会在社交媒体上发出信号，因此需要开发准确的自杀意念检测方法。
...
Conclusion: 所提出的混合深度学习方案能够有效识别社交媒体中的自杀意念模式，结合可解释AI方法增强了模型可靠性，在自杀预防领域具有重要应用价值。
...
TL;DR: 本研究开发并评估了三种NLP模型，用于从帕金森病患者的第一人称叙述中自动识别反映认知过程的七个类别。微调的Meta-Llama-3-8B-Instruct模型表现最佳，特别是在上下文依赖类别如思维和社交互动方面。
...
Result: 模型性能在不同类别和模型家族间差异显著。微调的Meta-Llama-3-8B-Instruct获得最高整体F1分数（微平均0.74，宏平均0.59），在上下文依赖类别表现优异。Bio_ClinicalBERT精度高但召回率低。
...
TL;DR: 本文介绍了BNLI，一个经过精细标注和语言优化的孟加拉语自然语言推理数据集，旨在解决现有数据集的标注错误、模糊句对和语言多样性不足等问题。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语自然语言推理研究资源极度匮乏，现有数据集存在标注错误、语义模糊和语言多样性不足等问题，阻碍了有效的模型训练和评估。

Method: 通过严格的标注流程构建数据集，强调语义清晰性和在蕴含、矛盾和中性类别之间的平衡分布，并使用多语言和孟加拉语专用transformer模型进行基准测试。

Result: 实验结果表明，BNLI在可靠性和可解释性方面取得了显著改进，为孟加拉语和其他低资源语言的推理任务研究提供了坚实基础。

Conclusion: BNLI数据集为推进孟加拉语和低资源语言推理研究建立了强有力的基础，展示了在复杂语义关系捕捉方面的改进效果。
...
TL;DR: HalluClean是一个轻量级、任务无关的框架，用于检测和纠正LLM生成文本中的幻觉内容，通过推理增强范式在多个任务上显著提升事实一致性。
...
Method: 采用推理增强范式，将过程分解为规划、执行和修订阶段来识别和优化无支持的主张；使用最小任务路由提示实现零样本泛化，不依赖外部知识源或监督检测器。

Result: 在五个代表性任务（问答、对话、摘要、数学文字问题和矛盾检测）上的广泛评估显示，HalluClean显著提高了事实一致性，并优于竞争基线。

Conclusion: HalluClean展示了在现实应用中增强LLM输出可信度的潜力，为检测和纠正幻觉内容提供了一种有效的轻量级解决方案。
...
TL;DR: TiDAR是一种序列级混合架构，在单个前向传递中通过扩散模型起草令牌（Thinking），然后自回归采样最终输出（Talking），实现了并行生成与AR级质量的平衡。


<details>
  <summary>Details</summary>
Motivation: 解决扩散语言模型并行生成与自回归模型质量优势之间的矛盾，寻求在保持高质量的同时实现高吞吐量和GPU利用率。

Method: 设计结构化注意力掩码，在单个前向传递中同时进行扩散起草和自回归采样，充分利用GPU计算密度。

Result: TiDAR在1.5B和8B规模上超越推测解码的吞吐量，在效率和质最上优于Dream和Llada等扩散模型，首次在保持AR模型质量的同时实现4.71-5.91倍的每秒令牌数提升。

Conclusion: TiDAR成功实现了扩散模型并行生成与自回归模型高质量之间的协同效应，为语言模型架构提供了新的平衡方案。
...
TL;DR: EVADE框架使用大型语言模型生成和验证解释来检测自然语言推理任务中的错误，相比传统人工标注方法更高效，能提升数据集质量和模型微调性能。


<details>
  <summary>Details</summary>
Motivation: 传统人工标注方法（如VARIERR框架）需要两轮人工标注，成本高昂且可能限制合理标签或解释的覆盖范围，因此需要更高效的方法来检测标注错误。

Method: 提出EVADE框架，使用大型语言模型生成和验证解释来检测错误，通过分布比较、验证重叠和模型微调影响三个维度全面分析人工与LLM检测的差异。

Result: LLM验证使生成的解释分布更接近人工标注，从训练数据中移除LLM检测的错误比移除人工检测的错误在模型微调中带来更大性能提升。

Conclusion: LLM方法能够扩展错误检测规模，在减少人工工作量的同时提高标签变异情况下的数据集质量。
...
TL;DR: SpiralThinker是一个统一的潜在推理框架，通过迭代更新潜在表示实现扩展的隐式推理，无需生成额外token，在数学、逻辑和常识推理任务中表现最佳。
...
Method: 引入迭代更新潜在表示的框架，结合渐进对齐目标和结构化注释来维持潜在与文本推理之间的一致性。
...
TL;DR: 本文提出了一种基于轨迹的情感支持评估框架，从静态对话评估转向动态长期评估，通过构建大规模基准测试和引入三个轨迹级指标来全面评估大语言模型的长期情感支持能力。
...
Method: 构建包含328个情感情境和1152个干扰事件的大规模基准测试，采用经过验证的情绪调节策略约束模型输出，将用户情感轨迹建模为一阶马尔可夫过程，并应用因果调整的情感估计方法。

Result: 通过引入基线情感水平、情感轨迹波动性和情感质心位置三个轨迹级指标，能够全面捕捉用户情感动态变化，并在多种大语言模型的广泛评估中发现了情感支持能力的显著差异。

Conclusion: 该框架为长期情感支持提供了全面的评估方法，揭示了不同模型在情感支持能力上的差异，为模型开发提供了可操作的见解。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM在金融、医疗等受严格政策监管行业应用时的随机性限制问题，确保逻辑验证的可靠性。

Method: 两阶段方法：1) 使用LLM和可选人工指导将自然语言政策形式化；2) 推理时自动形式化验证逻辑正确性，通过冗余形式化步骤交叉检查语义等价性。

Result: 基准测试显示该方法超过99%的可靠性，在识别逻辑有效性方面接近零误报率。

Conclusion: 该方法生成可审计的逻辑工件，能够验证结果并为改进原始文本提供依据，适用于对正确性要求极高的监管行业。
...
TL;DR: MM-CRITIC是一个全面的多模态批判能力基准，用于评估大型多模态模型在基本、修正和比较三个维度的批判能力，包含8个主要任务类型和超过500个任务，共4471个样本。


<details>
  <summary>Details</summary>
Motivation: 尽管大型多模态模型在图像描述和视觉推理等任务中能力不断增强，但多模态批判能力的研究仍然不足，这对于模型自我改进和成为可靠的AI助手至关重要。

Method: 构建了包含8个任务类型和500多个任务的MM-CRITIC基准，收集了不同规模LMM的响应，并整合专家指导的地面答案到评分标准中，使用GPT-4o标注响应并生成参考批判。

Result: 广泛的实验验证了MM-CRITIC的有效性，并对领先LMM的批判能力进行了全面评估。分析揭示了响应质量与批判能力之间的相关性，以及不同评估维度下批判难度的变化。

Conclusion: MM-CRITIC为评估多模态模型的批判能力提供了可靠的基准，揭示了当前LMM在批判能力方面的关键洞察，有助于推动多模态AI助手的发展。
...
TL;DR: 本文提出了一种基于混合CTC/注意力架构的安多藏语流式语音识别框架，采用上下文感知动态分块机制，通过自适应调整块宽度来缓解固定分块方法的上下文截断问题，并结合外部语言模型提升语义一致性。


<details>
  <summary>Details</summary>
Motivation: 解决安多藏语语音识别中固定分块方法导致的上下文截断问题，适应藏语的语言特性，提高流式识别的准确性和效率。

Method: 采用混合CTC/注意力架构，引入上下文感知动态分块机制，根据编码状态自适应调整块宽度；构建基于藏语正字法原则的词典作为建模单元；在解码阶段集成外部语言模型。

Result: 在测试集上达到6.23%的词错误率，相比固定分块基线相对提升48.15%，同时显著降低识别延迟，性能接近全局解码。

Conclusion: 所提出的框架有效解决了安多藏语流式语音识别中的上下文截断问题，在保持接近全局解码性能的同时显著提升了识别准确率和效率。
...
TL;DR: 本文通过日本即兴喜剧游戏Oogiri系统评估大语言模型的幽默能力，发现LLMs在生成幽默回应方面达到中低人类水平，但缺乏同理心，且与人类在幽默评价标准上存在根本差异。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要依赖单一维度的幽默评估，本文认为需要多维度理解幽默，通过Oogiri游戏来系统评估LLMs的幽默能力。

Method: 扩展Oogiri数据集，用LLMs生成回应，手动标注六个维度的5点评分（新颖性、清晰度、相关性、智慧、同理心、总体趣味性），评估LLMs在生成和评价两个核心任务上的表现。

Result: LLMs能生成中低人类水平的幽默回应，但明显缺乏同理心；LLMs与人类在幽默评价标准上存在根本差异：LLMs优先考虑新颖性，而人类优先考虑同理心。

Conclusion: LLMs在幽默生成和评价方面存在同理心缺陷，这解释了它们无法复制人类幽默评估的原因，需要开发更具情感智能的对话系统。
...
TL;DR: DoPE是一种无需训练的方法，通过检测特征图中的异常频率带并重新参数化来改进Transformer模型中的位置编码，显著提升长上下文处理能力。


<details>
  <summary>Details</summary>
Motivation: Rotary Position Embedding (RoPE)在Transformer模型中存在长度外推限制，导致注意力下沉现象，影响模型在长序列上的表现。

Method: 将带位置编码的注意力图重新解释为噪声特征图，基于截断矩阵熵检测异常频率带，并使用参数自由的高斯分布重新参数化特征图。

Result: 在needle-in-a-haystack和many-shot in-context learning任务上，DoPE显著提高了检索准确性和推理稳定性，支持长达64K tokens的上下文。

Conclusion: 位置嵌入的去噪策略有效缓解注意力下沉现象，恢复平衡的注意力模式，为改进长度泛化提供了简单而强大的解决方案。
...
TL;DR: LoopTool是一个完全自动化的、模型感知的数据演化框架，通过紧密集成数据合成和模型训练，迭代优化数据和模型，显著提升LLMs的工具使用能力。
...
Method: LoopTool通过三个协同模块实现闭环优化：(1)贪婪能力探测诊断模型的掌握和失败能力；(2)判断引导标签验证使用开源判断模型查找和纠正标注错误，逐步净化数据集；(3)错误驱动数据扩展基于识别出的失败生成新的挑战性样本。

Result: 实验表明，使用LoopTool训练的8B模型显著超越了其32B数据生成器，并在BFCL-v3和ACEBench基准测试中达到了其规模的最新最先进结果。

Conclusion: 闭环、自我精炼的数据管道可以显著增强LLMs的工具使用能力，并在成本效益高的开源生态系统中运行，消除了对昂贵闭源API的依赖。
...
TL;DR: 提出了一种无需人工识别的表格问答细胞提取方法，通过混合检索机制计算问题与单元格的相似度来估计表头，在NTCIR-18的TQA数据集上达到74.6%的准确率，优于GPT-4o mini。
...
Method: 通过集成语言模型和TF-IDF的混合检索机制计算给定问题与单个单元格的相似度来估计表头，然后在最相关行和列的交集处选择单元格作为答案。语言模型使用对比学习在少量问题-表头对数据集上进行训练。

Result: 在NTCIR-18 U4共享任务的TQA数据集上评估，该方法达到74.6%的准确率，优于GPT-4o mini的63.9%。

Conclusion: 该方法在表格问答中有效解决了LLM无法处理复杂表格结构的问题，未来计划整合更高效的文本搜索模型以进一步提高性能并缩小与人类评估结果的差距。
...
TL;DR: 本文通过实证验证了基于上下文的sequentiality度量方法在自动作文评分中的有效性，证明其比原始版本和仅基于主题的版本更能准确预测人类对作文组织和连贯性的评估。


<details>
  <summary>Details</summary>
Motivation: 先前研究提出使用LLMs量化叙事流，但被批评存在主题选择偏差且缺乏真实流度验证。有研究建议仅使用上下文术语作为更有效和可解释的替代方案，本文旨在实证验证这一提议。

Method: 使用两个带有人工标注特征分数的作文数据集（ASAP++和ELLIPSE），比较基于上下文的sequentiality与人类对语篇层面特征（如组织和连贯性）评估的一致性，并将其与零样本LLM预测和标准语言特征结合进行对比分析。

Result: 基于上下文的sequentiality版本与人类对组织和连贯性评估更一致；虽然零样本LLM预测特征分数更准确，但上下文度量与标准语言特征结合时比仅基于主题和原始sequentiality公式增加更多预测价值，且优于零样本LLM预测。

Conclusion: 研究支持使用基于上下文的sequentiality作为经过验证、可解释且互补的特征，用于自动作文评分和相关NLP任务，突显了显式建模句子间流动性的价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统的子词分割通常在预处理阶段固定，本文探索在训练过程中动态优化子词分割的学习动态，特别是研究子词在预训练和微调过程中的演化规律。

Method: 扩展子词分段语言模型框架以支持预训练和微调，针对三种形态类型不同的语言进行训练：粘着型的isi-Xhosa、分离型的Setswana和中间型的英语。

Result: 识别出子词学习的四个阶段，形态复杂的isi-Xhosa表现出更大的不稳定性；微调过程中子词边界变得更加细粒度；可学习的子词为低资源、形态复杂语言改进了文本生成和跨语言迁移。
...
TL;DR: 本文报告了在六种不同规模（从5100万到4.75亿参数）上预训练的ModernBERT编码器模型，重点关注有限多语言能力，特别强调与芬兰相关的语言。这些模型与现有多语言模型相比具有竞争力或更优性能，在需要超过512个标记的上下文任务中优于单语言模型。


<details>
  <summary>Details</summary>
Motivation: 开发针对芬兰相关语言的有限多语言模型，解决现有模型在长上下文任务中的不足，并提供不同训练数据阶段使用的实证结果。

Method: 预训练六种不同参数规模的ModernBERT编码器模型，采用有限多语言策略，特别关注芬兰相关语言，并在训练最后阶段使用不同数据进行实验。

Result: 模型与现有多语言模型相比具有竞争力或更优性能，在长上下文任务（超过512个标记）中表现优于单语言模型。

Conclusion: ModernBERT模型在有限多语言设置下表现优异，特别是在长上下文任务中，代码和模型已公开发布。
...
TL;DR: 本文提出Anchor方法，通过向rollouts注入真实轨迹来防止早期训练崩溃，显著提高了语言模型在可验证奖励强化学习中的演绎推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有可验证奖励强化学习方法只优化最终任务结果，在早期训练中负奖励占主导时容易崩溃，特别是在诚实对齐任务中，模型不仅要解决可回答查询，还要识别何时无法从给定前提得出结论。

Method: 构建了两个基于图结构的多步演绎推理数据集，提出Anchor方法在rollouts中注入真实轨迹来稳定训练，并评估了课程学习等稳定策略。

Result: Anchor方法稳定了学习过程，显著提高了整体推理性能，而GRPO方法在这些任务上表现不佳。

Conclusion: 训练动态对于在语言模型中实现可靠的演绎推理至关重要，Anchor方法通过防止早期训练崩溃有效解决了诚实对齐问题。
...
Method: 提出POTSA框架：1）偏置补偿模块粗粒度对齐语音表示；2）基于平行语音对在Q-Former上施加token级最优传输约束；3）层调度策略将OT约束聚焦于语义最有益的层。

Result: 在FLEURS数据集上，使用每种源语言仅10小时平行语音数据，平均BLEU提升+0.93（常见语言）和+5.05（零样本语言），达到SOTA性能。
...
TL;DR: C³TG是一个两阶段框架，通过选择性配对LLM与属性分类器，使用加权KL散度调整token概率，并在优化阶段通过能量函数和迭代反馈解决属性冲突，实现细粒度的多维文本属性控制。
...
Method: 提出C³TG两阶段框架：生成阶段选择性地将LLM与17个可用维度的属性分类器配对，使用加权KL散度调整token概率；优化阶段利用结合分类器分数和惩罚项的能量函数，通过迭代反馈解决属性冲突。

Result: 实验表明C³TG在属性准确性、语言流畅性、输出多样性和降低毒性等多个指标上显著优于基线方法。

Conclusion: C³TG是一种无需昂贵模型修改的有效灵活解决方案，能够同时精确控制多个维度属性，同时保持自然文本流。
...
TL;DR: 本文提出了一种可解释的高棉语极性分类器，通过微调基于指令的推理模型Qwen-3，不仅能够准确预测极性标签，还能提供解释来支持其预测。
...
Method: 通过微调基于指令的推理Qwen-3模型，构建自解释机制来合理化预测。

Result: 实验结果显示微调后的模型不仅能准确预测标签，还能通过识别极性相关关键词或短语来提供推理支持。

Conclusion: 成功开发了可解释的高棉语极性分类器，并贡献了新的高棉语极性数据集和公开可用的微调模型。
...
TL;DR: 本文提出AdaptDel方法，通过自适应删除率来改进序列分类的认证鲁棒性，针对编辑距离扰动提供理论保证，在自然语言任务中显著提升认证区域的中位数基数。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用固定删除率处理变长序列输入（如自然语言句子）导致性能不佳，需要开发能根据输入特性动态调整的适应性方法。

Method: 引入AdaptDel方法，采用可适应删除率机制，动态调整基于输入属性，并将随机平滑理论框架扩展到变率删除场景。

Result: 在自然语言任务中取得强实证结果，认证区域的中位数基数相比最先进认证方法提升高达30个数量级。

Conclusion: AdaptDel方法通过自适应删除率有效解决了变长序列认证鲁棒性问题，为编辑距离扰动提供了可靠的理论和实证保障。
...
TL;DR: 提出了mmJEE-Eval多模态双语基准，基于印度JEE Advanced考试，评估发现前沿VLMs在复杂推理任务上表现优异，但开源模型表现有限，且现有模型在元认知推理方面存在显著缺陷。
...
Method: 构建包含1,460道JEE Advanced考题的多模态双语基准，涵盖物理、化学和数学领域，评估17个最先进模型在不同难度和推理深度下的表现。

Result: 前沿VLMs在2025年考题上达到77-84%准确率，开源模型仅为37-45%，且在元认知推理任务中表现显著下降（GPT-5仅修正5.2%错误）。

Conclusion: mmJEE-Eval基准能够有效区分模型的训练质量和推理能力，揭示了当前模型在复杂推理任务中的局限性。
...
TL;DR: SeerSC是一个动态自一致性框架，通过整合系统1和系统2推理，同时提高令牌效率和降低延迟。系统1快速计算答案熵来评估样本的扩展潜力，系统2进行动态自一致性处理，实现并行生成以减少令牌使用和延迟。
...
Method: 利用快速系统1计算查询的答案熵，评估样本扩展潜力；在系统2下进行动态自一致性处理；通过并行生成实现令牌使用和延迟的减少。
...
Result: 在BSBM42和Spider4SPARQL数据集上的实验显示，S2CLite解析准确率达77.8%，执行准确率在双方解析成功的查询子集上达到96.6%，优于S2CTrans 7.3%。

Conclusion: S2CLite显著提升了SPARQL到Cypher的转换性能，并基于此生成了Spider4SSC多语言查询数据集，为文本到查询语言研究提供了统一基准。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在评估输出方面越来越有效和可扩展，但尚不清楚这种能力是否能从特定任务评估扩展到更一般的文本质量评估，特别是在多语言环境中。
...
Conclusion: MTQ-Eval框架在多语言文本质量评估方面表现出色，不仅提升了评估性能，还能促进下游任务的改进。
...
TL;DR: 本文系统比较了大型语言模型在开放式文本生成和多项选择任务中的自我修正机制，揭示了两种范式在性能趋势和错误修正行为上的显著差异。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型自我修正机制在不同任务范式（开放式生成vs多项选择）中的动态差异，为设计更有效的自我修正机制提供理论基础。

Method: 通过系统实验比较不同规模和系列的语言模型在各种自然语言理解和推理任务中的表现，分析两种范式下的性能趋势和错误修正行为。

Result: 开放式生成受益于重新解释和组合优化的灵活性，而多项选择能利用更清晰的解决方案边界但受限于预设选项。两种范式展现出不同的改进模式和失败模式。
...
<details>
  <summary>Details</summary>
Motivation: 当前离线偏好优化方法存在过拟合-欠拟合困境：正确排序样本梯度浪费，错误排序样本纠正信号不足，限制了排序准确性的提升

Method: 提出AMaPO算法，采用基于Z归一化和指数缩放的自适应边界，动态重新分配学习努力，放大错误排序样本梯度，抑制正确排序样本梯度
...
TL;DR: 本文开发了一个针对低资源语言巴斯克语的多模态大语言模型，通过创建训练和评估数据集，使用Llama-3.1-Instruct和巴斯克语适配的Latxa模型作为骨干，发现仅需20%巴斯克语多模态数据即可获得良好性能，且无需巴斯克语指令调优的骨干模型。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在多种任务上表现优异，但开源社区在低资源语言上的性能仍不及商业模型。本文旨在为低资源语言巴斯克语开发一个强大的多模态大语言模型。

Method: 开发了训练和评估用的图像-文本数据集，使用Llama-3.1-Instruct和巴斯克语适配的Latxa两种大语言模型作为骨干，探索了不同的数据混合训练策略。

Result: 研究表明：i) 仅需约20%的巴斯克语多模态数据即可在巴斯克语基准测试中获得稳定结果；ii) 出乎意料的是，无需巴斯克语指令调优的骨干模型也能构建强大的巴斯克语多模态模型。

Conclusion: 研究成果为开发其他低资源语言的多模态大语言模型开辟了道路，并公开释放了相关资源。
...
TL;DR: CARE-Bench是一个动态交互式自动化基准测试，用于评估大语言模型在心理咨询领域的专业能力，基于真实案例构建多样化客户档案，提供多维性能评估。


<details>
  <summary>Details</summary>
Motivation: 心理咨询需求增长与服务供给不足之间的矛盾，以及现有评估方法在客户模拟、评估形式和指标维度方面的局限性，促使开发更全面的评估基准。

Method: 基于真实心理咨询案例构建多样化客户档案，按照专家指南进行模拟，建立多维性能评估体系，结合心理学家进行详细分析。

Result: 评估了多个通用大语言模型和专业咨询模型，揭示了它们当前的局限性，并分析了不同类型客户交互失败的原因。
...
TL;DR: GSAP-ERE是一个用于机器学习研究信息提取的细粒度数据集，包含10种实体类型和18种关系类型，基于100篇ML出版物的全文构建，用于支持知识图谱构建和AI研究可复现性监控等下游任务。


<details>
  <summary>Details</summary>
Motivation: 机器学习研究发展迅速，从科学出版物中提取信息能够大规模识别研究概念和资源，有助于提高ML相关研究的理解和可复现性。

Method: 构建了手动整理的细粒度数据集GSAP-ERE，包含63K实体提及和35K关系，来自100篇ML出版物的全文。使用该数据集微调模型进行信息提取，并与大型语言模型的提示策略进行比较。

Result: 微调模型在命名实体识别(NER)和关系提取(RE)任务上表现优异(NER: 80.6%, RE: 54.0%)，显著优于最先进的LLM提示方法(NER: 44.4%, RE: 10.1%)。
...
TL;DR: 本文提出了一种名为TPoT的新策略，通过语义过滤文本作为深度学习模型的输入，专门用于预测大五人格特质、方面或项目。


<details>
  <summary>Details</summary>
Motivation: 从大量生成文本中预测个体人格具有挑战性，特别是当文本量很大时，需要解决大型语言模型输入文本限制的问题。

Method: 使用TPoT策略语义过滤文本，构建BIG5-TPoT模型，选择与特定特质、方面或项目语义相关的文本作为输入。

Result: 该方法不仅解决了大型语言模型的输入文本限制问题，还在Stream of Consciousness Essays数据集上提高了平均绝对误差和准确率指标。

Conclusion: TPoT是一种简单而有效的策略，通过语义相关性选择文本，能够显著提升人格预测模型的性能。
...
### [95] [Simulating Psychological Risks in Human-AI Interactions: Real-Case Informed Modeling of AI-Induced Addiction, Anorexia, Depression, Homicide, Psychosis, and Suicide](https://arxiv.org/abs/2511.08880)
*Chayapatr Archiwaranguprok,Constanze Albrecht,Pattie Maes,Karrie Karahalios,Pat Pataranutaporn*

Main category: cs.HC

TL;DR: 本文提出了一种系统评估AI系统心理风险的方法，通过分析18个真实案例和2160个模拟场景，揭示了LLM在检测心理困扰、应对脆弱用户和防止伤害升级方面的关键缺陷。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统日益融入日常生活，其可能加剧或引发严重心理伤害的潜力仍未得到充分理解和测试，需要开发系统方法来探索人机交互中的心理风险。

Method: 收集分析18个真实AI相关心理伤害案例，提取有害交互模式，使用临床分期模型在2160个模拟场景中测试4个主要LLM，分析157,054个对话轮次。

Result: 发现LLM在检测心理困扰、应对脆弱用户和防止伤害升级方面存在显著缺陷；回归分析显示LLM对老年用户表现较差，但对中低收入群体表现较好；聚类分析识别出15种不同的失败模式。

Conclusion: 本研究提供了一种识别心理风险的新方法，揭示了系统间常见的失败模式，并对高风险人机交互中的有害AI响应模式进行了分类。
...
### [96] ["It's trained by non-disabled people": Evaluating How Image Quality Affects Product Captioning with VLMs](https://arxiv.org/abs/2511.08917)
*Kapil Garg,Xinru Tang,Jimin Heo,Dwayne R. Morgan,Darren Gergle,Erik B. Sudderth,Anne Marie Piper*

Main category: cs.HC

TL;DR: 该研究评估了图像质量问题对视觉语言模型为盲人和低视力人群生成产品描述准确性的影响，发现模型在无质量问题时准确率达98%，但在存在质量问题时降至75%，且问题叠加时表现更差。
...
Method: 基于对86名盲人和低视力人士的调查，系统评估了图像质量问题对视觉语言模型生成描述的影响。

Result: 最佳模型在无质量问题的图像中识别产品准确率达98%，但在存在质量问题时整体准确率降至75%，且随着问题叠加而显著恶化。

Conclusion: 需要以残障人士体验为中心的模型评估方法，并为HCI和ML研究人员提供具体建议，使视觉语言模型对盲人和低视力人群更可靠。
...
### [97] [SimPath: Mitigating Motion Sickness in In - vehicle Infotainment Systems via Driving Condition Adaptation](https://arxiv.org/abs/2511.09240)
*Jinghao Huang,Siqi Yao,Yu Zhang*

Main category: cs.HC

TL;DR: SimPath视觉设计旨在缓解乘客晕车问题并提升车载娱乐系统使用效率，通过真实车辆实验验证，发现能显著降低晕车程度但未直接提升系统使用效率。


<details>
  <summary>Details</summary>
Motivation: 解决乘客在使用车载娱乐系统时因车辆不规则运动导致的晕车问题，提升乘坐舒适度和系统使用效率。

Method: 创新设计SimPath视觉方案，在真实驾驶场景下进行两组实车实验验证效果。

Result: 该方法能显著降低乘客晕车程度，但由于视觉内容分散注意力，未能直接提升车载娱乐系统使用效率。

Conclusion: 为更智能友好的车载娱乐系统设计提供了重要见解，为未来自动驾驶车辆的车载系统发展提供了理论支持和实践指导。
...
### [98] [TempoQL: A Readable, Precise, and Portable Query System for Electronic Health Record Data](https://arxiv.org/abs/2511.09337)
*Ziyong Ma,Richard D. Boyce,Adam Perer,Venkatesh Sivaraman*

Main category: cs.HC

TL;DR: TempoQL是一个基于Python的工具包，旨在简化电子健康记录数据的提取和验证，提供简单易读的时序查询语言、多数据标准支持以及交互式查询界面。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录数据是医疗机器学习的重要数据源，但研究人员和临床医生在提取和验证数据时面临表达性和可用性之间的权衡，现有工具难以编写适用于现代建模流程的时序查询。

Method: 开发TempoQL工具包，提供人类可读的时序查询语言，支持OMOP、MEDS等多种EHR数据标准，并集成交互式笔记本界面和LLM辅助编写功能。

Result: 通过性能评估和两个不同数据集的用例，证明TempoQL在保持精度、速度和可重复性的同时，简化了机器学习队列的创建过程。

Conclusion: TempoQL成功降低了EHR数据提取和验证的门槛，为医疗机器学习研究提供了更便捷的工具支持。
...
### [99] [A multimodal AI agent for clinical decision support in ophthalmology](https://arxiv.org/abs/2511.09394)
...
TL;DR: EyeAgent是首个用于眼科临床决策支持的智能AI框架，通过大语言模型协调53个验证过的眼科工具，在诊断准确性、工具选择准确性和人机协作方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统在眼科领域缺乏灵活性、可解释性和适应性，而眼科需要处理多种成像模式，因此需要开发能够动态协调多种专业工具的智能框架。

Method: 使用大语言模型作为核心推理引擎，动态协调53个经过验证的眼科工具，涵盖23种成像模式，支持分类、分割、检测、图像/报告生成和定量分析等任务。

Result: 诊断准确率从基线69.71%提升至80.79%；在200个真实临床案例中，工具选择准确率达93.7%，专家评分超过88%；在人机协作中，诊断准确率提升18.51%，报告质量提升19%。
...
### [100] [Algorithmic Advice as a Strategic Signal on Competitive Markets](https://arxiv.org/abs/2511.09454)
*Tobias R. Rebholz,Maxwell Uphoff,Christian H. R. Bernges,Florian Scholten*

Main category: cs.HC

TL;DR: 算法建议在竞争性决策中可作为战略信号，影响市场协调。个体化建议比集体建议影响更强，可能导致隐性合谋和超竞争利润。


<details>
  <summary>Details</summary>
Motivation: 研究算法建议如何影响人类在经典经济博弈中的行为，特别是在具有唯一非合谋均衡的竞争环境中，探讨算法在塑造市场战略动态中的作用。

Method: 通过两个预注册实验：实验1进行Bertrand价格竞争，测试个体化vs集体算法建议；实验2进行Cournot数量竞争，测试均衡对齐vs战略偏差建议。

Result: 个体化建议影响更强，价格竞争初期向上偏差建议推高价格但最终收敛；数量竞争中向下偏差建议导致持续低产量和超竞争利润，形成隐性合谋。

Conclusion: 算法建议可作为战略信号影响协调，无需明确沟通即可导致合谋结果，强调在竞争环境中需要谨慎设计和监管算法决策支持系统。
...
### [101] [Exploring The Interaction-Outcome Paradox: Seemingly Richer and More Self-Aware Interactions with LLMs May Not Yet Lead to Better Learning](https://arxiv.org/abs/2511.09458)
*Rahul R. Divekar,Sophia Guerra,Lisette Gonzalez,Natasha Boos*

Main category: cs.HC

TL;DR: 本研究比较了LLM和搜索界面在学习效果上的差异，发现虽然LLM能引发更丰富细致的互动，但并未显著提升学习成果，提出了"互动-成果悖论"的概念。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM改变了学习交互方式，从关键词搜索转向自然语言对话，但其对教育成果的实际影响尚不明确，需要实证研究来验证。

Method: 采用对照研究设计（N=20），直接比较LLM和基于搜索的界面在学习互动和成果方面的差异。

Result: LLM界面能引发学习者更丰富和细致的互动，但并未产生普遍更好的学习成果。

Conclusion: 研究提出了"认知转移"概念，指出学生努力的重点从搜索时的信息查找和综合转向LLM环境下的自我认知和知识缺口识别，这为评估教育技术提供了新视角，强调应设计能利用学生表达性来支撑生产性认知工作的系统。
...
### [102] [Bridging Natural Language and ASP: A Hybrid Approach Using LLMs and AMR Parsing](https://arxiv.org/abs/2511.08715)
*Connar Hite,Sean Saud,Raef Taha,Nayim Rahman,Tanvir Atahary,Scott Douglass,Tarek Taha*

Main category: cs.AI

TL;DR: 本文提出了一种使用LLM和AMR图将英语翻译成ASP程序的新方法，用于解决逻辑谜题，通过最小化LLM的角色来创建轻量级可解释系统。
...
Result: 系统成功创建了完整的ASP程序来解决组合逻辑问题，展示了其在逻辑谜题中的能力。
...
### [103] [Vector Symbolic Algebras for the Abstraction and Reasoning Corpus](https://arxiv.org/abs/2511.08747)
*Isaac Joffe,Chris Eliasmith*

Main category: cs.AI

TL;DR: 该论文提出了一种基于向量符号代数(VSA)的认知合理ARC-AGI求解器，结合系统1直觉和系统2推理，通过面向对象的程序合成方法解决ARC-AGI基准测试。


<details>
  <summary>Details</summary>
Motivation: ARC-AGI是人类轻松解决但对AI系统极具挑战性的流体智力基准测试，受神经科学和心理学中人类智能建模方法的启发，需要开发认知合理的求解器。

Method: 使用神经符号方法，基于向量符号代数(VSA)集成系统1直觉和系统2推理，通过面向对象的程序合成，利用VSA表示抽象对象、指导解决方案搜索并实现样本高效的神经学习。

Result: 在ARC-AGI-1-Train上得分为10.8%，在ARC-AGI-1-Eval上得分为3.0%；在简单基准测试中表现优异，Sort-of-ARC得分为94.5%，1D-ARC得分为83.1%（优于GPT-4且计算成本极低）。

Conclusion: 该方法是首个将VSA应用于ARC-AGI的研究，开发了迄今为止最认知合理的ARC-AGI求解器，在保持计算效率的同时实现了有竞争力的性能。
...
TL;DR: 本文提出了一个双层可靠性监控框架，用于解决智能AI系统在操作中的可靠性不足问题，包括离群检测层和AI透明度层，为人类操作员提供决策支持。


<details>
  <summary>Details</summary>
Motivation: 智能AI系统虽然具有自主性优势，但在高风险领域应用时存在可靠性不足的问题，需要开发缓解技术来降低操作中的不确定性风险。

Method: 提出双层可靠性监控框架：第一层用于检测新颖输入的离群分布，第二层通过AI透明度揭示内部操作过程。

Result: 该框架为人类操作员提供了判断输出是否可靠的决策支持，使其能够及时干预不可靠的系统行为。

Conclusion: 该监控框架为开发缓解技术奠定了基础，有助于降低智能AI系统在操作中因可靠性不确定而产生的风险。
...
### [105] [UCO: A Multi-Turn Interactive Reinforcement Learning Method for Adaptive Teaching with Large Language Models](https://arxiv.org/abs/2511.08873)
*Shouang Wei,Min Zhang,Xin Lin,Bo Jiang,Kun Kuang,Zhongxiang Dai*

Main category: cs.AI

TL;DR: 本文提出了单向认知优化（UCO）方法，通过多轮交互式强化学习解决LLM在教育中作为智能导师的两个关键挑战：无法区分学生是否真正理解知识，以及无法感知学生认知状态变化。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在教育中从答案提供者转向智能导师，但现有监督微调方法只能学习表面教学模式，缺乏动态适应能力。强化学习方法虽然有所改进，但面临两个关键挑战：无法区分学生是否真正理解知识，以及无法感知学生认知状态变化。

Method: 提出单向认知优化（UCO）方法，采用多轮交互式强化学习范式，包含两个协同的奖励函数：进步奖励捕捉学生认知进步，评估学生是否真正从困惑转向理解；支架奖励动态识别每个学生的最近发展区（ZPD），鼓励教师在此区域内保持高效教学。

Result: 在BigMath和MathTutorBench基准测试中，UCO模型优于所有同等规模的基线模型，并达到与先进闭源模型相当的性能。

Conclusion: UCO方法通过创新的奖励函数设计，有效解决了LLM作为智能导师的两个关键挑战，实现了更好的教学适应性和效果。
...
### [106] [Lumine: An Open Recipe for Building Generalist Agents in 3D Open Worlds](https://arxiv.org/abs/2511.08892)
...
Method: 采用端到端的视觉语言模型，以5Hz处理原始像素输入，生成30Hz的键盘鼠标动作，并仅在必要时自适应调用推理模块。在Genshin Impact游戏中进行训练。

Result: 成功完成Genshin Impact中5小时的蒙德主线任务，达到人类水平效率；在Wuthering Waves中完成100分钟任务，在Honkai: Star Rail中完成5小时第一章任务，均无需微调。

Conclusion: Lumine在多个不同游戏世界中展现了强大的任务完成能力和跨游戏泛化性能，标志着在开放环境通用智能体发展方面迈出了重要一步。
...
TL;DR: 该研究构建了一个融合人工智能与大数据的业务流程优化模型，采用三层架构实现流程全生命周期的智能化管理，通过实验验证显著提升了企业运营效率。
...
Conclusion: 该研究成果对企业数字化转型具有重要理论和实践价值，为提升企业运营效率提供了新思路。
...
Method: 框架包含两个阶段：(1)自动预测准备：构建多源认知基础，包括特征集、领域知识库、上下文存储库和案例库；(2)生成推理和反思优化：整合统计特征、先验知识、上下文信息和预测策略，触发元推理循环进行持续自校正和策略优化。
...
### [109] [AI Founding Fathers: A Case Study of GIS Search in Multi-Agent Pipelines](https://arxiv.org/abs/2511.09005)
*Alvin Chauhan*

Main category: cs.AI

TL;DR: 本文提出通过结构化多智能体管道实现渐进式增量搜索(GIS)来增强大语言模型的推理能力，并通过递归精炼方法验证了复杂模型在政治议题分析中的优越表现。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在流畅性方面表现出色，但研究者仍在努力提升其推理能力。本文基于搜索导向的LLM计算解释，旨在系统性地理解和优化LLM推理过程。

Method: 采用递归精炼(RR)方法，通过自我批评、对抗性压力测试和整合关键反馈的迭代过程实现GIS搜索。设计了简单线性管道与复杂结构化管道的对比实验，使用三位美国开国元勋的历史人物构建多智能体模型。

Result: 复杂模型在所有九个测试案例中均优于简单模型，平均仲裁分数为88.3 vs 71.7。复杂模型的论证在分析深度、结构细微差别和战略框架方面表现更优。

Conclusion: 递归精炼是通过GIS搜索增强LLM推理能力的强大架构特征，结构化多智能体管道能够显著提升模型推理质量。
...
### [110] [Argus: Resilience-Oriented Safety Assurance Framework for End-to-End ADSs](https://arxiv.org/abs/2511.09032)
*Dingji Wang,You Lu,Bihuan Chen,Shuo Hao,Haowen Jiang,Yifan Tian,Xin Peng*

Main category: cs.AI

TL;DR: 提出Argus框架，通过持续监控自动驾驶系统轨迹并适时接管控制来增强系统韧性，防止安全违规并提升驾驶性能


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在公共道路上部署时面临各种驾驶危险，需要具备持续监控危险并自适应响应安全违规的能力，以维持复杂场景下的稳健驾驶行为

Method: 开发运行时韧性导向框架Argus，持续监控ADS生成的轨迹，当EGO车辆被认为不安全时通过危险缓解器无缝接管控制

Result: Argus与TCP、UniAD和VAD三个先进端到端ADS集成，平均提升驾驶分数达150.30%，防止高达64.38%的违规，且时间开销很小

Conclusion: Argus能有效高效地增强自动驾驶系统的韧性，显著改善驾驶性能并大幅减少安全违规
...
### [111] [Advancing Autonomous Emergency Response Systems: A Generative AI Perspective](https://arxiv.org/abs/2511.09044)
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆有望通过更快、更安全、更高效的响应来革新紧急服务，但传统强化学习方法在动态紧急场景中存在样本效率低和适应性不足的局限性。
...
Conclusion: 从生成式AI视角，本文为下一代自动驾驶紧急响应系统的发展提供了重要理论框架，展示了扩散模型增强RL和LLM辅助ICL在解决传统RL局限性方面的潜力。
...
### [112] [OR-R1: Automating Modeling and Solving of Operations Research Optimization Problem via Test-Time Reinforcement Learning](https://arxiv.org/abs/2511.09092)
*Zezhen Ding,Zhen Tan,Jiheng Zhang,Tianlong Chen*

Main category: cs.AI

TL;DR: OR-R1是一个数据高效的训练框架，用于自动化优化建模和求解，通过两阶段训练（监督微调+测试时组相对策略优化）实现高性能，仅需1/10的合成数据即可达到67.7%的平均求解准确率。
...
Result: OR-R1达到67.7%的平均求解准确率，仅需ORLM方法1/10的合成数据，准确率提升达4.2%，在100个合成样本下仍优于ORLM 2.4%。TGRPO带来额外3.1%-6.4%的准确率提升。

Conclusion: OR-R1为自动化运筹学优化问题建模和求解提供了稳健、可扩展且经济高效的解决方案，降低了工业应用的专业知识和数据门槛。
...
### [113] [Efficient Reasoning via Reward Model](https://arxiv.org/abs/2511.09158)
*Yuhao Wang,Xiaopeng Li,Cheng Gong,Ziru Liu,Suiyun Zhang,Rui Liu,Xiangyu Zhao*

Main category: cs.AI

TL;DR: 本文提出了一种训练简洁性奖励模型（CRM）的流程，通过新颖的简洁性奖励函数（CRF）解决大型推理模型中过度思考导致的冗长响应问题，在减少计算成本的同时提升推理效果。
...
Method: 提出训练简洁性奖励模型（CRM）的流程来评估推理路径的简洁性，并引入新颖的简洁性奖励函数（CRF），在结果奖励和简洁性评分之间建立显式依赖关系。

Result: 在五个数学基准数据集上的实验表明，该方法在Qwen2.5-7B上实现了8.1%的准确率提升和19.9%的响应token长度减少，并且在Llama和Mistral等其他LLM上具有良好的泛化能力。

Conclusion: 该方法通过理论证明和实践验证，既能提升推理效果又能提高推理效率，为解决大型语言模型中的过度思考问题提供了有效解决方案。
...
TL;DR: 本文提出了HyperD框架，通过将交通数据解耦为周期性和残差分量来改进交通预测，解决了复杂空间依赖和多尺度周期模式与不规则波动的挑战。


<details>
  <summary>Details</summary>
Motivation: 交通预测面临两大挑战：复杂空间依赖性和多尺度周期模式与不规则波动的共存问题，这影响了预测准确性。

Method: 提出HyperD框架，包含混合周期表示模块（处理周期性分量）和频率感知残差表示模块（处理非周期性波动），并引入双视图对齐损失来强制语义分离。

Result: 在四个真实交通数据集上的实验表明，HyperD实现了最先进的预测精度，同时在干扰下具有更强的鲁棒性和更高的计算效率。

Conclusion: HyperD通过解耦交通数据的周期性和残差分量，有效提升了交通预测的性能和鲁棒性。
...
TL;DR: 论文提出从"模型训练"转向"模型培养"的新范式，将价值观对齐融入模型开发全过程，通过重新设计训练语料库来实现早期价值观承诺。


<details>
  <summary>Details</summary>
Motivation: 当前AI训练方法只在模型核心能力建立后才进行价值观对齐，导致模型容易被误导且缺乏深层次价值体系。在大型语言模型能力开始超越人类的背景下，需要从根本上改变训练方式。

Method: 重新设计训练语料库：采用第一人称视角重构训练数据、将信息重新情境化为生活体验、模拟社会互动、以及搭建训练数据的顺序支架。

Result: 预期这种训练语料库的重新设计将实现从第一个训练标记开始的早期价值观承诺，使知识、技能和价值观内在难以分离。

Conclusion: 在大型语言模型能力超越人类的生态系统中，这种从"训练"到"培养"的范式转变是至关重要的需求。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI主要推动了定量方法的发展，但定性研究领域被忽视。研究者只能依赖通用AI工具如ChatGPT进行定性分析，但这些工具存在偏见、不透明、不可复现和隐私泄露等问题，无法满足定性研究的需求。

Method: 通过文献回顾分析现有自动化发现流程如何通过增强定性能力来改进，并识别安全定性AI在多学科和混合方法研究中的关键机会。

Result: 研究发现需要开发专门为解释性研究设计的定性AI系统，这些系统必须具有透明性、可复现性和隐私友好性。

Conclusion: 开发专门的定性AI系统对于填补AI在定性研究中的空白至关重要，这将促进多学科和混合方法研究的发展，实现更全面的科学理解。
...
TL;DR: 评估2025年三个前沿大语言模型（DeepSeek R1、Gemini 2.5 Pro、GPT-5）在PDDL规划和推理任务中的表现，发现GPT-5在标准PDDL领域与LAMA规划器表现相当，但在混淆测试中所有LLM性能下降但降幅小于先前模型。


<details>
  <summary>Details</summary>
Motivation: 评估前沿大语言模型在端到端规划任务中的推理能力，比较它们与传统规划器的性能差距，并测试在纯推理条件下的表现。

Method: 使用国际规划竞赛学习赛道中的PDDL领域和任务子集，对三个前沿LLM进行端到端规划评估，包括标准PDDL和混淆PDDL两种测试条件。

Result: GPT-5在标准PDDL领域与LAMA规划器表现相当；所有LLM在混淆PDDL测试中性能下降，但降幅小于先前报告的其他模型。

Conclusion: 前沿LLM在规划任务上相比前代模型有显著改进，缩小了与传统规划器在挑战性基准测试中的性能差距。
...
Conclusion: 该方法为使用生成模型进行科学探索提供了一条途径，能够分析、控制和重新利用潜在表示，探索未被捕获、考虑或编目的信息。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注高层次描述或视觉问答，而CrochetBench旨在填补多模态模型在程序推理能力评估方面的空白，特别是在需要精确执行的真实世界创意领域。

Method: 采用CrochetPARADE DSL作为中间表示，支持结构验证和通过执行进行功能评估。基准测试涵盖针法分类、指令接地以及自然语言和图像到DSL的翻译任务。

Result: 在所有任务中，当评估从表面相似性转向可执行正确性时，性能急剧下降，暴露出在长距离符号推理和3D感知程序合成方面的局限性。

Conclusion: CrochetBench为评估多模态模型的程序能力提供了新视角，并突显了在真实世界创意领域中表面理解与可执行精度之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI安全方法依赖模型输出或激活检查，但某些风险无法通过检查检测。需要一种架构无关的补充方法，通过模型聚合来增强安全性。

Method: 使用共识采样算法，利用模型计算输出概率的能力，当足够多模型安全且达成充分共识时输出结果，否则弃权。算法受Vyas等人版权保护算法启发。

Result: 聚合模型的安全性继承自给定大小的最安全子集，当足够多模型安全且具有足够一致性时，弃权概率有界。

Conclusion: 该方法提供了一种新的模型无关AI安全方法，能够将集合中未知安全子集的安全保证放大到单个可靠模型。
...
### [122] [Practical and Performant Enhancements for Maximization of Algebraic Connectivity](https://arxiv.org/abs/2511.08694)
...
<details>
  <summary>Details</summary>
Motivation: 当前图估计方法在大型长期图上扩展性差，MAC算法虽然能保持估计性能但计算成本高且需要手动指定边集，限制了在线使用。

Method: 开发了专用代数连通性求解器（平均2倍加速），研究了MAC优化过程的步长策略，提出了自动保证图连通性的方案。

Result: 这些改进使MAC算法更具可扩展性、可靠性和实时适用性。

Conclusion: 通过三方面的互补贡献，成功解决了MAC算法的计算瓶颈和手动配置问题，使其更适合实时估计应用。
...
### [123] [Intuitive Programming, Adaptive Task Planning, and Dynamic Role Allocation in Human-Robot Collaboration](https://arxiv.org/abs/2511.08732)
*Marta Lagomarsino,Elena Merlo,Andrea Pupa,Timo Birr,Franziska Krebs,Cristian Secchi,Tamim Asfour,Arash Ajoudani*

Main category: cs.RO

TL;DR: 这篇综述探讨了实现人机协同合作的关键组件，重点关注如何建立双向信息流动：人类向机器人直观传达指令和需求，机器人向人类清晰传达内部状态和即将执行的动作。


<details>
  <summary>Details</summary>
Motivation: 当前机器人和AI虽然具备强大能力，但在人类环境中难以充分发挥潜力，因为缺乏有效的人类状态建模和意图理解，同时人类也往往只是被动观察者，不知道如何与机器人互动。

Method: 通过分析完整的人机交互流程：从多模态输入到机器人可理解表示的转换，到自适应规划和角色分配，再到控制层和反馈机制，形成一个闭环系统。

Result: 识别并连接了实现直观信息交换和技能转移的关键组件，建立了从人类到机器人再到人类的完整信息流动框架。

Conclusion: 提出了向更自适应、更易访问的人机协同合作发展的趋势和前景方向，强调双向信息流动对于实现真正协同合作的重要性。
...
### [124] [ATOM-CBF: Adaptive Safe Perception-Based Control under Out-of-Distribution Measurements](https://arxiv.org/abs/2511.08741)
*Kai S. Yun,Navid Azizan*

Main category: cs.RO

TL;DR: ATOM-CBF是一个新颖的安全控制框架，通过显式计算和适应OoD测量中的认知不确定性来确保真实世界系统的安全，无需地面实况标签或分布偏移信息。


<details>
  <summary>Details</summary>
Motivation: 真实世界系统依赖学习感知模块从高维传感器数据推断系统状态，但这些模块容易受到认知不确定性的影响，在遇到训练期间未见过的OoD测量时会失效，从而威胁系统安全。

Method: ATOM-CBF包含两个关键组件：(1) OoD感知的自适应感知误差边界，(2) 集成此自适应误差边界的安全过滤器，使过滤器能够实时调整其保守性。

Result: 在仿真中进行了实证验证，证明ATOM-CBF能够为配备LiDAR扫描的F1Tenth车辆和配备RGB图像的四足机器人保持安全性。

Conclusion: ATOM-CBF框架通过显式处理OoD测量中的认知不确定性，为依赖学习感知模块的真实世界系统提供了一种有效的安全保障方法。
...
### [125] [CENIC: Convex Error-controlled Numerical Integration for Contact](https://arxiv.org/abs/2511.08771)
*Vince Kurtz,Alejandro Castro*

Main category: cs.RO

TL;DR: CENIC是一种新的连续时间积分器，结合了凸时间步进和误差控制积分的最新进展，能够以接近实时速度运行，同时提供精度和收敛性保证。


<details>
  <summary>Details</summary>
Motivation: 现有机器人模拟器使用离散时间步长，需要用户选择时间步长，这既关键又具有挑战性：大步长会产生非物理伪影，而小步长会迫使模拟运行缓慢。误差控制连续时间积分可以避免这些问题，但现有方法在处理接触刚度动力学时存在困难，无法满足现代机器人工作流程的速度和可扩展性要求。
...
Conclusion: CENIC成功地将连续时间积分的优势与离散时间模拟器的性能相结合，为机器人模拟提供了既快速又准确的解决方案。
...
### [126] [Low-cost Multi-agent Fleet for Acoustic Cooperative Localization Research](https://arxiv.org/abs/2511.08822)
...
TL;DR: CoUGARs是一个低成本、可配置的水下自主机器人平台，用于多智能体自主性研究，成本低于3000美元，支持声学协作定位研究。


<details>
  <summary>Details</summary>
Motivation: 现实世界的水下多智能体自主性测试面临高昂的财务和工程挑战，需要开发低成本、可配置的解决方案。

Method: 基于商用和3D打印部件设计低成本AUV平台，配备DVL和USBL声学阵列，使用容器化软件栈和HoloOcean模拟器进行开发和部署。

Result: 系统在模拟和犹他州湖泊的实际现场试验中进行了测试验证。

Conclusion: CoUGARs平台为水下多智能体自主性研究提供了经济可行的解决方案，支持快速定制和声学协作定位研究。
...
### [127] [XPRESS: X-Band Radar Place Recognition via Elliptical Scan Shaping](https://arxiv.org/abs/2511.08863)
*Hyesu Jang,Wooseong Yang,Ayoung Kim,Dongje Lee,Hanguen Kim*

Main category: cs.RO

TL;DR: 本文提出了一种专门针对X波段雷达的场所识别算法，通过基于目标密度的候选选择规则和故意降低雷达检测质量来实现稳健的检索性能。


<details>
  <summary>Details</summary>
Motivation: X波段雷达作为海上船只的主要传感器，在自主导航中的应用受到传感器分辨率低和信息内容不足的限制。

Method: 采用基于目标密度的规则进行高效候选选择，并故意降低雷达检测质量以实现稳健检索性能。

Result: 在公共海上雷达数据集和自收集数据集上评估，并与最先进的雷达场所识别方法进行比较。
...
### [128] [MirrorLimb: Implementing hand pose acquisition and robot teleoperation based on RealMirror](https://arxiv.org/abs/2511.08865)
*Cong Tai,Hansheng Wu,Haixu Long,Zhengbin Long,Zhaoyu Zheng,Haodong Xiang,Tao Shen*

Main category: cs.RO

TL;DR: 提出基于PICO的机器人远程操作框架，实现低成本实时手部运动姿态数据采集，兼容RealMirror生态系统，支持多种末端执行器机器人的实时遥操作。


<details>
  <summary>Details</summary>
Motivation: 降低上肢机器人操作研究的技术门槛，加速VLA相关研究进展。

Method: 基于PICO的机器人远程操作框架，兼容RealMirror生态系统，在Isaac仿真环境中实现稳定精确的机器人轨迹记录。

Result: 相比主流视觉跟踪和运动捕捉方案更具成本效益，支持灵巧手和机器人抓爪等多种末端执行器的实时遥操作。

Conclusion: 该框架为构建视觉-语言-动作数据集提供了即用功能，有效促进了VLA相关研究的发展。
...
### [129] [A Shared Control Framework for Mobile Robots with Planning-Level Intention Prediction](https://arxiv.org/abs/2511.08912)
...
Method: 设计路径重规划算法根据推断的人类意图调整机器人期望轨迹；引入意图域概念作为路径重规划的约束；将意图域预测和路径重规划问题联合建模为马尔可夫决策过程，通过深度强化学习求解；开发基于Voronoi图的人类轨迹生成算法，实现无需人类参与或演示数据的仿真训练。

Result: 大量仿真和真实世界用户研究表明，该方法显著降低了操作员工作负荷并提高了安全性，与现有辅助遥操作方法相比，任务效率未受影响。

Conclusion: 该共享控制框架通过规划级意图预测有效提升了人机协作性能，在降低操作负荷和增强安全性的同时保持了任务效率。
...
### [130] [Expand Your SCOPE: Semantic Cognition over Potential-Based Exploration for Embodied Visual Navigation](https://arxiv.org/abs/2511.08935)
*Ningnan Wang,Weihuang Chen,Liming Chen,Haoxuan Ji,Zhongyu Guo,Xuchong Zhang,Hongbin Sun*

Main category: cs.RO

TL;DR: SCOPE是一个零样本视觉导航框架，通过利用边界信息和基于潜力的探索来改进长时程规划，结合视觉语言模型估计探索潜力并构建时空潜力图，还包含自我重新考虑机制来优化决策。


<details>
  <summary>Details</summary>
Motivation: 现有零样本研究虽然通过记忆机制改进了长时程规划，但忽视了视觉边界边界对轨迹和观测的根本影响，且难以推断部分视觉观测与导航目标之间的关系。

Method: 提出SCOPE框架，利用边界信息驱动基于潜力的探索，使用视觉语言模型估计探索潜力并组织成时空潜力图，同时引入自我重新考虑机制来重新审视和优化先前决策。

Result: 在两个不同的具身导航任务上，SCOPE在准确率上比最先进的基线方法提高了4.6%，核心组件带来了更好的校准、更强的泛化能力和更高的决策质量。

Conclusion: SCOPE通过显式利用边界信息和基于潜力的探索，结合自我重新考虑机制，显著提升了零样本视觉导航的性能和可靠性。
...
### [131] [UniMM-V2X: MoE-Enhanced Multi-Level Fusion for End-to-End Cooperative Autonomous Driving](https://arxiv.org/abs/2511.09013)
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统存在感知有限和孤立决策的问题，现有多智能体方法主要关注感知级任务，忽视了与下游规划和控制的协调，未能充分利用端到端自动驾驶的全部潜力。

Method: 提出多层次融合策略统一感知和预测协作，允许智能体共享查询并进行协同推理；采用混合专家架构动态增强BEV表示，并将MoE扩展到解码器以更好地捕捉多样化运动模式。

Result: 在DAIR-V2X数据集上，相比UniV2X，感知精度提升39.7%，预测误差降低7.2%，规划性能提升33.2%，达到最先进水平。

Conclusion: MoE增强的多层次协作范式展示了强大性能，证明了端到端多智能体框架在自动驾驶中的有效性。
...
### [132] [A Quantum Tunneling and Bio-Phototactic Driven Enhanced Dwarf Mongoose Optimizer for UAV Trajectory Planning and Engineering Problem](https://arxiv.org/abs/2511.09020)
*Mingyang Yu,Haorui Yang,Kangning An,Xinjian Wei,Xiaoxuan Xu,Jing Xu*

Main category: cs.RO

TL;DR: 本文提出了一种增强型多策略矮獴优化算法(EDMO)，用于解决无人机在动态障碍环境中的三维路径规划问题。该算法通过三种新策略改进传统元启发式算法，在收敛速度、鲁棒性和优化精度方面优于14种先进算法。


<details>
  <summary>Details</summary>
Motivation: 随着无人机的广泛应用，有效的路径规划变得日益重要。传统的元启发式算法存在早熟收敛和解决方案多样性不足的问题，在复杂场景中性能受限。

Method: EDMO算法集成了三种新策略：(1)动态量子隧道优化策略，使粒子概率性逃离局部最优；(2)生物趋光动态聚焦搜索策略，基于微生物趋光性进行自适应局部优化；(3)正交透镜对立学习策略，通过结构化维度重组增强全局探索。

Result: 在CEC2017和CEC2020的39个标准测试函数上，EDMO在收敛速度、鲁棒性和优化精度方面优于14种先进算法。在无人机三维路径规划和三个工程设计任务的实际验证中，证实了其在实际应用中的有效性和适应性。

Conclusion: EDMO算法在复杂动态环境中表现出优越的路径规划性能，为需要智能、自适应和时间高效规划的现场机器人任务提供了实用的解决方案。
...
### [133] [D-AWSIM: Distributed Autonomous Driving Simulator for Dynamic Map Generation Framework](https://arxiv.org/abs/2511.09080)
*Shunsuke Ito,Chaoran Zhao,Ryo Okamura,Takuya Azumi*

Main category: cs.RO

TL;DR: D-AWSIM是一个分布式仿真器，通过在多台机器上分配工作负载来支持大规模传感器部署和密集交通环境的仿真，解决了传统单机仿真器无法处理大规模城市交通场景的问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在特定操作设计域内取得了显著进展，但扩展这些域需要解决多样化条件下的安全保障问题。通过车辆间和车辆与基础设施通信的信息共享提供了有前景的解决方案，但真实世界实验成本高昂且面临监管挑战。

Method: 提出D-AWSIM分布式仿真器，将工作负载分配到多台机器上，支持大规模传感器部署和密集交通环境仿真，并集成了动态地图生成框架。

Result: 评估显示，与单机设置相比，D-AWSIM在车辆数量和LiDAR传感器处理方面显著提高了吞吐量。与Autoware的集成证明了其在自动驾驶研究中的适用性。

Conclusion: D-AWSIM为研究人员提供了一个无需依赖物理测试平台即可探索信息共享策略的有效工具，支持大规模自动驾驶系统研究。
...
### [134] [APEX: Action Priors Enable Efficient Exploration for Robust Motion Tracking on Legged Robots](https://arxiv.org/abs/2511.09091)
...
TL;DR: APEX是一种即插即用的运动跟踪算法扩展，通过结合衰减动作先验和多批评器框架，消除部署时对参考数据的依赖，提高样本效率并减少参数调整。


<details>
  <summary>Details</summary>
Motivation: 现有运动跟踪方法需要大量调整且依赖部署时的参考数据，限制了适应性。APEX旨在消除这种依赖，提高学习效率和泛化能力。

Method: 将专家演示直接集成到强化学习中，采用衰减动作先验（初始偏向专家演示但逐渐允许独立探索）和多批评器框架（平衡任务性能与运动风格）。

Result: 在仿真和Unitree Go2机器人上的实验验证了方法的有效性，能够学习多样化运动并在不同地形和速度间转移参考风格，同时对奖励设计变化保持鲁棒。

Conclusion: APEX通过演示引导强化学习探索，实现了更稳定、高效和泛化的学习，为从运动到操作等各种机器人任务的自然技能获取铺平了道路。
...
### [135] [Decoupling Torque and Stiffness: A Unified Modeling and Control Framework for Antagonistic Artificial Muscles](https://arxiv.org/abs/2511.09104)
*Amirhossein Kazemipour,Robert K. Katzschmann*

Main category: cs.RO

TL;DR: 提出了一个统一框架，用于实时独立控制不同类型软执行器的扭矩和刚度，通过级联控制器和分析逆动力学保持解耦控制，在接触实验中展示了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 现有软执行器控制器难以在动态接触瞬变中保持独立的扭矩-刚度控制，限制了其在安全人机交互中的应用。

Method: 使用统一的力定律捕获不同软执行器物理特性，采用级联控制器和分析逆动力学，通过共收缩/偏置坐标独立调节扭矩和刚度。

Result: 在接触实验中实现了200倍更快的软表面稳定时间，刚性表面上81%的力减少，以及稳定的交互性能（相比固定策略的22-54%稳定性）。
...
### [136] [Data Assessment for Embodied Intelligence](https://arxiv.org/abs/2511.09119)
*Jiahao Xiao,Bowen Yan,Jianbo Zhang,Jia Wang,Chunyi Li,Zhengxue Cheng,Guangtao Zhai*

Main category: cs.RO

TL;DR: 本文提出了两种数据驱动的工具来评估具身智能数据集：多样性熵用于衡量数据集的信息量，以及无需训练即可量化数据集可学习性的可解释算法。


<details>
  <summary>Details</summary>
Motivation: 当前数据集评估主要关注多样性但缺乏全面衡量，而可学习性评估通常需要昂贵的模型训练且缺乏可解释性，无法为数据集改进提供指导。

Method: 构建统一的多模态数据表示，提出多样性熵来衡量数据集信息量；开发首个无需训练即可量化数据集可学习性的可解释算法。

Result: 在模拟和真实世界具身数据集上验证了算法的有效性，证明其能提供可靠且可操作的见解，帮助同时改进多样性和可学习性。
...
### [137] [LODESTAR: Degeneracy-Aware LiDAR-Inertial Odometry with Adaptive Schmidt-Kalman Filter and Data Exploitation](https://arxiv.org/abs/2511.09142)
*Eungchang Mason Lee,Kevin Christiansen Marsim,Hyun Myung*

Main category: cs.RO

TL;DR: LODESTAR是一种新颖的LiDAR-惯性里程计方法，通过退化解感知的自适应Schmidt-Kalman滤波器和退化解感知的数据利用模块，解决了在退化环境（如长走廊、高空飞行）中LiDAR测量不平衡或稀疏导致的病态状态估计问题。


<details>
  <summary>Details</summary>
Motivation: 传统LiDAR-惯性里程计在退化环境中性能下降，因为LiDAR测量不平衡或稀疏会导致状态估计病态。需要开发能够处理这些退化情况的鲁棒方法。

Method: 提出两个关键模块：1) 退化解感知自适应Schmidt-Kalman滤波器(DA-ASKF)，使用滑动窗口和退化解感知滑动模式，基于退化水平将状态分类为活跃或固定；2) 退化解感知数据利用(DA-DE)，根据局部化贡献和Jacobian矩阵条件数修剪非信息性测量并选择性利用固定状态的测量。

Result: 实验结果表明，LODESTAR在各种退化条件下，在准确性和鲁棒性方面优于现有的基于LiDAR的里程计方法和退化解感知模块。

Conclusion: LODESTAR通过退化解感知的约束优化和测量稀疏性缓解，有效解决了LiDAR-惯性里程计在退化环境中的性能问题，提高了系统的准确性和鲁棒性。
...
### [138] [Unveiling the Impact of Data and Model Scaling on High-Level Control for Humanoid Robots](https://arxiv.org/abs/2511.09241)
*Yuxi Wei,Zirui Wang,Kangning Yin,Yue Hu,Jingbo Wang,Siheng Chen*

Main category: cs.RO

TL;DR: Humanoid-Union是一个通过自动化流程生成的大规模人形机器人运动数据集，包含260+小时多样化高质量运动数据，基于人类运动视频提取语义标注。在此基础上提出SCHUR框架，通过数据扩展和模型扩展显著提升运动生成质量和文本-运动对齐效果。


<details>
  <summary>Details</summary>
Motivation: 解决机器人学习中数据扩展的瓶颈问题，利用丰富的人类视频和运动数据作为免费的大规模数据源，探索如何有效挖掘原始视频、提取机器人可学习的表示，并用于可扩展学习。

Method: 开发自动化流程生成Humanoid-Union大规模数据集，包含语义标注的人形机器人运动数据；基于此提出SCHUR可扩展学习框架，研究大规模数据对人形机器人高级控制的影响。

Result: SCHUR在数据和模型扩展下实现了高机器人运动生成质量和强文本-运动对齐，MPJPE重建改进37%，FID对齐改进25%，并在真实人形机器人上验证了有效性。

Conclusion: Humanoid-Union数据集和SCHUR框架成功解决了人形机器人学习中的数据扩展问题，证明了大规模数据对高级控制学习的重要作用，为机器人学习提供了可扩展的解决方案。
...
### [139] [UMIGen: A Unified Framework for Egocentric Point Cloud Generation and Cross-Embodiment Robotic Imitation Learning](https://arxiv.org/abs/2511.09302)
*Yan Huang,Shoujie Li,Xingting Li,Wenbo Ding*

Main category: cs.RO

TL;DR: UMIGen是一个统一的机器人学习框架，包含手持式数据采集设备Cloud-UMI和可见性感知优化机制，能够高效生成与真实自我中心观察对齐的3D点云数据，支持跨机器人平台的无后处理迁移。


<details>
  <summary>Details</summary>
Motivation: 解决数据驱动机器人学习面临的数据收集难题：鲁棒策略需要大规模高质量演示数据，但现有方法收集成本高、依赖专用硬件且空间泛化能力有限。UMI虽然降低了硬件要求，但仅能捕获RGB图像而缺失3D几何信息。

Method: 提出UMIGen框架，包含两个关键组件：(1) Cloud-UMI手持数据采集设备，无需视觉SLAM，同时记录点云观察-动作对；(2) 可见性感知优化机制，将DemoGen流程扩展到自我中心3D观察，仅生成相机视野内的点。

Result: 在仿真和真实环境中的实验表明，UMIGen支持强大的跨平台泛化能力，并在多种操作任务中加速数据收集。

Conclusion: UMIGen实现了高效的数据生成，能够直接在不同机器人平台间迁移而无需后处理，为机器人学习提供了有效的解决方案。
...
### [140] [SPIDER: Scalable Physics-Informed Dexterous Retargeting](https://arxiv.org/abs/2511.09484)
...
TL;DR: SPIDER是一个基于物理的灵巧重定向框架，可将人类运动数据转化为机器人可执行的动态可行轨迹，解决机器人数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 由于机器人特定数据收集成本高昂，而人类运动数据丰富但存在体现差距和动态信息缺失，无法直接用于机器人控制。

Method: 使用基于物理的大规模采样和课程式虚拟接触引导，将人类演示转化为动态可行的机器人轨迹。

Result: 在9种人形/灵巧手体现和6个数据集上，成功率比标准采样提高18%，比强化学习基线快10倍，生成了240万帧动态可行机器人数据集。

Conclusion: SPIDER作为通用物理重定向方法，能够处理多样化质量数据并生成高质量数据，有效支持强化学习等策略学习方法。
...
### [141] [WMPO: World Model-based Policy Optimization for Vision-Language-Action Models](https://arxiv.org/abs/2511.09515)
*Fangqi Zhu,Zhengyang Yan,Zicong Hong,Quanxin Shou,Xiao Ma,Song Guo*

Main category: cs.RO

TL;DR: 提出了WMPO框架，通过基于像素的世界模型进行VLA强化学习，无需真实环境交互，显著提高样本效率并实现自我纠正等涌现行为。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型依赖专家演示，无法从失败中学习；而强化学习虽然能自我改进，但在真实机器人上样本复杂度高。需要结合两者优势的方法。

Method: WMPO框架使用基于像素预测的世界模型，将"想象"轨迹与预训练的VLA特征对齐，实现无需真实环境交互的在线策略强化学习。

Result: 在仿真和真实机器人实验中，WMPO显著提高样本效率，获得更强性能，展现出自我纠正等涌现行为，并具有鲁棒泛化和终身学习能力。

Conclusion: WMPO为VLA强化学习提供了有效的无真实环境交互解决方案，结合了VLA模型和强化学习的优势，实现了高效的学习和涌现能力。
...
### [142] [MAP-VLA: Memory-Augmented Prompting for Vision-Language-Action Model in Robotic Manipulation](https://arxiv.org/abs/2511.09516)
*Runhao Li,Wenkai Guo,Zhenyu Wu,Changyuan Wang,Haoyuan Deng,Zhenyu Weng,Yap-Peng Tan,Ziwei Wang*

Main category: cs.RO

TL;DR: MAP-VLA是一个增强预训练视觉-语言-动作模型的新框架，通过演示派生的记忆提示来解决长时程机器人操作任务中的记忆限制问题。


<details>
  <summary>Details</summary>
Motivation: 现有的预训练VLA模型在长时程任务中表现不佳，因为它们缺乏记忆机制且仅依赖即时感官输入。

Method: 首先从历史演示构建记忆库，每个记忆单元捕获任务特定阶段的信息，作为可学习的软提示进行优化。然后在实时任务执行期间，通过轨迹相似性匹配检索相关记忆，并动态集成到VLA模型中增强动作生成。

Result: 实验结果显示，MAP-VLA在仿真基准测试中实现高达7.0%的绝对性能提升，在真实机器人评估中实现25.0%的性能提升，超越了当前最先进的方法。

Conclusion: MAP-VLA作为一个即插即用模块，为冻结的VLA模型提供轻量级且灵活的解决方案，显著提高了长时程机器人操作任务的性能。
...
### [143] [Efficient Model-Agnostic Continual Learning for Next POI Recommendation](https://arxiv.org/abs/2511.08941)
*Chenhao Wang,Shanshan Feng,Lisi Chen,Fan Li,Shuo Shang*

Main category: cs.IR

TL;DR: GIRAM是一个用于持续下一个兴趣点推荐的模型无关框架，通过整合上下文感知的持续兴趣和近期兴趣，动态适应用户行为变化，同时保持高效性。
...
Method: 提出GIRAM框架，包含四个组件：兴趣记忆库、上下文感知键编码模块、基于生成键的检索模块、自适应兴趣更新与融合模块，可无缝集成到现有推荐模型中。

Result: 在三个真实世界数据集上的实验表明，GIRAM在更新时间和内存消耗方面保持高效率的同时，持续优于最先进的方法。

Conclusion: GIRAM为解决持续下一个兴趣点推荐任务提供了有效的解决方案，能够动态适应用户兴趣变化并保持高效性能。
...
### [144] [NeuroCLIP: Brain-Inspired Prompt Tuning for EEG-to-Image Multimodal Contrastive Learning](https://arxiv.org/abs/2511.09250)
...
Method: 1) 双流视觉嵌入管道结合动态过滤和令牌级融合生成实例级自适应提示；2) 首次在EEG-图像对齐中引入视觉提示令牌作为全局模态级提示；3) 提出基于神经科学原理的改进对比损失。
...
### [145] [Sim4IA-Bench: A User Simulation Benchmark Suite for Next Query and Utterance Prediction](https://arxiv.org/abs/2511.09329)
*Andreas Konstantin Kruff,Christin Katharina Kreutz,Timo Breuer,Philipp Schaer,Krisztian Balog*

Main category: cs.IR

TL;DR: Sim4IA-Bench是首个面向IR社区的用户模拟基准套件，包含160个真实搜索会话和70个会话的模拟器运行数据，用于评估下一查询和话语预测。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏成熟的衡量标准和基准，验证用户模拟的准确性具有挑战性。需要建立基准来评估模拟器是否准确反映真实用户行为。
...
Conclusion: Sim4IA-Bench为评估和比较用户模拟方法提供了基础，促进了信息访问领域现实且可解释的用户模拟的可重复研究。
...
### [146] [Physics-Informed Machine Learning for Characterizing System Stability](https://arxiv.org/abs/2511.08831)
...
<details>
  <summary>Details</summary>
Motivation: 在复杂动态系统设计中，确保状态轨迹收敛到期望平衡点至关重要，但稳定性区域往往难以计算。现有方法依赖系统控制方程的显式知识，限制了实际应用。

Method: 提出LyapInf方法，假设Lyapunov函数为二次型，通过最小化Zubov方程的平均残差来拟合未知的二次算子，从而从轨迹数据推断Lyapunov函数。

Result: 数值实验表明，该方法能够成功推断出接近最大椭球体的系统稳定性区域估计，无需系统控制方程知识。

Conclusion: LyapInf方法为黑盒动态系统提供了一种有效的稳定性分析工具，通过数据驱动方式实现了稳定性区域的可靠估计。
...
TL;DR: CGAR是一种新的训练方法，通过将课程学习应用于架构深度而非传统数据排序，显著提升了递归推理模型的训练效率。该方法包含渐进深度课程和分层监督加权两个组件，在Sudoku-Extreme数据集上实现了1.71倍训练加速和42%成本降低，同时保持了高精度。


<details>
  <summary>Details</summary>
Motivation: 递归推理模型在复杂推理任务上表现出色，但训练成本高昂（约36 GPU小时/数据集），限制了广泛应用和研究。需要开发更高效的训练方法。

Method: CGAR方法包含两个协同组件：1）渐进深度课程：在训练过程中动态调整递归深度从浅到深，防止早期过拟合并降低计算成本；2）分层监督加权：对监督步骤应用指数衰减的重要性权重，使损失权重与观察到的梯度幅度衰减保持一致。

Result: 在Sudoku-Extreme数据集（423,168个测试谜题）上，CGAR实现了1.71倍训练加速（从10.93小时降至6.38小时，成本降低42%），精度仅下降0.63%（从86.65%降至86.02%）。渐进深度课程单独实现了2.26倍加速和85.47%精度，展示了罕见的帕累托改进。

Conclusion: CGAR证明了在架构深度上应用原则性课程学习能够在小规模硬件上高效训练递归推理模型，同时提升训练效率和推理质量。
...
### [148] [TabPFN-2.5: Advancing the State of the Art in Tabular Foundation Models](https://arxiv.org/abs/2511.08667)
...
<details>
  <summary>Details</summary>
Motivation: 开发能够处理更大规模表格数据的基础模型，提升表格AI的性能和实用性，满足工业级应用需求。

Method: 基于TabPFN系列模型的下一代架构，扩展了数据处理能力，并引入了新的蒸馏引擎将模型转换为紧凑的MLP或树集成。

Result: 在TabArena基准测试中大幅超越调优的树模型，与AutoGluon 1.4精度相当；对小中型分类数据集有100%胜率，对大型数据集有87%胜率。

Conclusion: TabPFN-2.5显著提升了表格AI的性能和可扩展性，通过蒸馏引擎实现了生产环境的低延迟部署，将加强现有TabPFN生态系统的应用。
...
### [149] [Quasi-Newton Compatible Actor-Critic for Deterministic Policies](https://arxiv.org/abs/2511.09509)
*Arash Bahari Kordabad,Dean Brandner,Sebastien Gros,Sergio Lucia,Sadegh Soudjani*

Main category: cs.LG

TL;DR: 提出了一种二阶确定性actor-critic强化学习框架，通过利用性能函数的曲率信息来扩展经典确定性策略梯度方法，实现更快的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 经典确定性策略梯度方法只使用一阶信息，收敛速度较慢。为了利用性能函数的曲率信息来加速收敛，需要开发能够同时估计梯度和Hessian矩阵的方法。

Method: 基于兼容函数逼近概念，引入二次critic来同时保持真实策略梯度和性能Hessian的近似。开发最小二乘时序差分学习方案来高效估计二次critic参数，从而实现使用critic学习信息的准牛顿actor更新。

Result: 数值实验表明，该方法相比标准确定性actor-critic基线方法实现了更好的收敛性和性能表现。

Conclusion: 提出的二阶确定性actor-critic框架能够有效利用曲率信息，在可微分策略类中实现比一阶方法更快的收敛速度。
...
### [150] [Gromov-Wasserstein Graph Coarsening](https://arxiv.org/abs/2511.08733)
*Carlos A. Taveras,Santiago Segarra,César A. Uribe*

Main category: cs.LG

TL;DR: 该论文提出了两种基于Gromov-Wasserstein几何的图粗化算法：贪婪对粗化(GPC)和k均值贪婪对粗化(KGPC)，通过最小化节点合并引起的失真来优化图结构，在多个大规模数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 研究图粗化问题在Gromov-Wasserstein几何框架下的解决方案，旨在开发能够有效减少图规模同时保持结构特征的新方法。

Method: 提出两种算法：GPC迭代合并最小化局部失真的节点对；KGPC基于成对失真度量的聚类直接合并节点簇。两种方法都利用了节点合并引起失真的新表示方法。

Result: 在六个大规模数据集和下游聚类任务中验证了方法的有效性，结果表明所提方法在广泛的参数和场景下优于现有方法。

Conclusion: 基于Gromov-Wasserstein几何的图粗化方法能够有效优化图结构，为大规模图处理提供了新的解决方案，并在多个实际应用中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 偏差-方差分解是统计学和机器学习中的核心结果，但通常只针对平方误差。作者希望提供一个更通用的分解方法，适用于Bregman散度，这对指数族的最大似然估计特别相关。

Method: 作者提供了一个清晰、独立的推导过程，将传统的偏差-方差分解推广到Bregman散度的情况。

Result: 成功推导出Bregman散度下的偏差-方差分解公式，为指数族最大似然估计提供了理论基础。

Conclusion: 虽然这个结果在文献中已经存在，但本文提供了一个更清晰、更易理解的推导过程，具有教学价值，并补充了相关文献讨论。
...
### [152] [EEG-X: Device-Agnostic and Noise-Robust Foundation Model for EEG](https://arxiv.org/abs/2511.08861)
*Navid Mohammadi Foumani,Soheila Ghane,Nam Nguyen,Mahsa Salehi,Geoffrey I. Webb,Geoffrey Mackellar*

Main category: cs.LG

TL;DR: EEG-X是一个设备无关且抗噪声的脑电图基础模型，通过位置通道嵌入和噪声感知重建策略解决EEG分析中的设备差异性和低信噪比问题。


<details>
  <summary>Details</summary>
Motivation: 解决EEG分析中的两个关键挑战：(1)不同设备和配置导致的跨数据集变异性，(2)EEG的低信噪比，其中脑信号常被伪迹和非脑源信号淹没。

Method: 提出位置通道嵌入编码空间信息；采用噪声感知掩码和重建策略，在原始和潜在空间中重建去噪信号；引入字典启发的卷积变换层进行结构化特征投影。

Result: 在来自不同设备的数据集上，EEG-X在多个下游EEG任务中优于最先进方法，在电极布局不同的跨域设置中表现优异。

Conclusion: EEG-X通过创新的通道嵌入和噪声处理策略，成功构建了一个设备无关且抗噪声的EEG基础模型，显著提升了跨域泛化能力。
...
### [153] [TIGER-MARL: Enhancing Multi-Agent Reinforcement Learning with Temporal Information through Graph-based Embeddings and Representations](https://arxiv.org/abs/2511.08832)
*Nikunj Gupta,Ludwika Twardecka,James Zachary Hare,Jesse Milzman,Rajgopal Kannan,Viktor Prasanna*

Main category: cs.LG

TL;DR: TIGER通过图嵌入和表示捕获时间信息来增强多智能体强化学习，显式建模智能体间协调结构的时间演化，使用动态时间图和时序注意力编码器提升协作策略学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有MARL方法依赖静态或每步关系图，忽略了智能体适应、移动或重组协作策略时自然出现的时间交互演化，而捕获这种演化依赖对于实现鲁棒和自适应协调至关重要。

Method: 构建MARL智能体的动态时间图连接当前和历史交互，使用时序注意力编码器聚合结构和时间邻域信息，生成时间感知的智能体嵌入来指导协作策略学习。

Result: 在两个协调密集型基准测试中，TIGER在任务性能和样本效率方面持续优于多种基于价值分解和图基础的MARL基线方法。

Conclusion: 通过全面的消融研究揭示了结构和时间因素如何共同塑造MARL中有效的策略学习，证明了时间演化建模对多智能体协调的重要性。
...
### [154] [Enhancing DPSGD via Per-Sample Momentum and Low-Pass Filtering](https://arxiv.org/abs/2511.08841)
*Xincheng Xu,Thilina Ranbaduge,Qing Wang,Thierry Rakotoarivelo,David Smith*

Main category: cs.LG

TL;DR: 提出DP-PMLF方法，通过集成逐样本动量和低通滤波策略，同时减轻DP噪声和裁剪偏差，提升差分隐私随机梯度下降的隐私-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 现有的差分隐私随机梯度下降方法通常只解决噪声或偏差中的一个问题，因为减少DP噪声会加剧裁剪偏差，反之亦然。需要一种能同时解决这两个问题的方法。

Method: DP-PMLF方法结合逐样本动量和低通滤波策略：使用逐样本动量在裁剪前平滑梯度估计以减少采样方差；采用后处理低通滤波器衰减高频DP噪声，且不消耗额外隐私预算。

Result: 理论分析表明在严格DP保证下具有改进的收敛速率；实证评估显示DP-PMLF相比多个最先进的DPSGD变体显著提升了隐私-效用权衡。

Conclusion: DP-PMLF通过同时减轻DP噪声和裁剪偏差，有效提升了差分隐私深度学习的性能，为隐私保护机器学习提供了更优的解决方案。
...
### [155] [On topological descriptors for graph products](https://arxiv.org/abs/2511.08846)
*Mattie Ji,Amauri H. Souza,Vikas Garg*

Main category: cs.LG

TL;DR: 本文研究了图乘积上各种过滤对拓扑描述符（欧拉特征和持续同调）的影响，建立了欧拉特征在基于颜色过滤上的表达能力完整特征化，证明了图乘积的持续同调比单个图包含更多信息，并提供了计算图乘积顶点和边级过滤持续同调图的算法。


<details>
  <summary>Details</summary>
Motivation: 拓扑描述符在关系数据中捕获多尺度结构信息的应用日益增多，需要研究图乘积上的过滤如何影响拓扑描述符的表达能力。

Method: 考虑图乘积上的各种过滤，分析其对欧拉特征和持续同调的影响，建立欧拉特征表达能力特征化，证明持续同调的信息优势，并提供计算图乘积过滤持续同调图的算法。

Result: 欧拉特征在基于颜色过滤上的表达能力得到完整特征化，图乘积的持续同调比单个图包含更多信息，开发了有效的计算算法，并通过实证研究验证了理论分析。
...
### [156] [Fairness-Aware Few-Shot Learning for Audio-Visual Stress Detection](https://arxiv.org/abs/2511.09039)
*Anushka Sanjay Shelke,Aditya Sneh,Arya Adyasha,Haroon R. Lone*

Main category: cs.LG

TL;DR: FairM2S是一个公平性感知的元学习框架，用于基于音频-视觉数据的压力检测，通过集成Equalized Odds约束来减少性别偏见，在低资源场景下实现公平且准确的检测。


<details>
  <summary>Details</summary>
Motivation: 现有AI驱动的压力检测模型在数据稀缺场景下经常表现出性别偏见，这影响了心理健康护理的公平性，因此需要开发能够有效减轻偏见的公平性感知方法。

Method: FairM2S在元训练和适应阶段都集成了Equalized Odds约束，采用对抗性梯度掩码和公平性约束的元更新来有效减轻偏见。

Result: 与五个最先进的基线方法相比，FairM2S实现了78.1%的准确率，同时将Equal Opportunity降低到0.06，显示出显著的公平性提升。

Conclusion: FairM2S代表了心理健康AI中公平且可扩展的少样本压力检测的最先进方法，作者还发布了带有性别标注的智能手机采集数据集SAVSD以支持公平性研究。
...
### [157] [Rethinking Graph Super-resolution: Dual Frameworks for Topological Fidelity](https://arxiv.org/abs/2511.08853)
*Pragya Singh,Islem Rekik*

Main category: cs.LG

TL;DR: 本文提出了两种GNN无关的图超分辨率框架：Bi-SR通过二分图连接低分辨率和高分辨率节点，实现结构感知的节点超分辨率；DEFEND通过将高分辨率边映射到对偶图节点，实现基于标准GNN的边推断。
...
Method: Bi-SR框架使用二分图连接LR和HR节点，保持拓扑结构和排列不变性；DEFEND框架通过将HR边映射到对偶图节点，使用标准节点GNN进行边推断。

Result: 在真实脑连接组数据集上，两个框架在七个拓扑度量上达到最先进性能；还引入了12个模拟数据集用于全面基准测试。

Conclusion: 提出的两种框架有效解决了现有方法的局限性，在保持图结构的同时实现了高效的图超分辨率，为资源受限领域提供了实用解决方案。
...
### [158] [Decomposition of Small Transformer Models](https://arxiv.org/abs/2511.08854)
*Casper L. Christensen,Logan Riggs*

Main category: cs.LG

TL;DR: 本文扩展了随机参数分解（SPD）方法到Transformer模型，提出了适用于序列数据的因果重要性函数和新损失函数，成功分解了玩具归纳头模型并在GPT-2-small中定位了可解释概念的子组件。


<details>
  <summary>Details</summary>
Motivation: 先前的方法在玩具模型上取得了成功应用，但尚未扩展到真实模型。本文旨在弥合这一差距，将SPD方法扩展到现代Transformer模型。
...
Conclusion: 这是将SPD扩展到现代模型的第一步，表明我们可以使用该方法来发现可解释的参数空间机制。
...
### [159] [ForeSWE: Forecasting Snow-Water Equivalent with an Uncertainty-Aware Attention Model](https://arxiv.org/abs/2511.08856)
*Krishu K Thapa,Supriya Savalkar,Bhupinderjeet Singh,Trong Nghia Hoang,Kirti Rajagopalan,Ananth Kalyanaraman*

Main category: cs.LG

TL;DR: ForeSWE是一个新的概率时空预测模型，结合深度学习和经典概率技术，用于预测雪水当量(SWE)，提供预测不确定性量化，在西部美国512个SNOTEL站点上表现出优于现有方法的预测精度和区间。


<details>
  <summary>Details</summary>
Motivation: SWE预测对雪域流域水资源管理决策至关重要，但传统方法未能充分利用时空相关性，也不提供不确定性估计，这对决策者具有重要价值。

Method: 提出ForeSWE模型，结合注意力机制整合时空特征和交互，以及高斯过程模块提供原则性的预测不确定性量化。

Result: 在西部美国512个SNOTEL站点上的评估显示，相比现有方法，在预测精度和预测区间方面都有显著改进，并突显了不同方法在不确定性估计方面的有效性。

Conclusion: 这些发现为水资源管理社区的部署和反馈提供了平台，证明了ForeSWE在SWE预测中的有效性和实用性。
...
### [160] [Transformer-Based Sleep Stage Classification Enhanced by Clinical Information](https://arxiv.org/abs/2511.08864)
*Woosuk Chung,Seokwoo Hong,Wonhyeok Lee,Sangyoon Bae*

Main category: cs.LG

TL;DR: 提出了一种结合Transformer编码器和1D CNN聚合器的两阶段架构，系统研究了融入临床元数据和专家事件标注等上下文信息对睡眠分期的影响，在SHHS队列中显著提升了分期准确性。
...
Method: 使用两阶段架构：基于Transformer的每时段编码器结合1D CNN聚合器，系统融入受试者级临床元数据（年龄、性别、BMI）和每时段专家事件标注（呼吸暂停、血氧饱和度下降、觉醒、周期性呼吸）。

Result: 在SHHS队列（n=8,357）中，上下文融合显著提高了分期准确性：与仅使用PSG的基线相比（macro-F1 0.7745，micro-F1 0.8774），最终模型达到macro-F1 0.8031和micro-F1 0.9051，其中事件标注贡献最大增益。
...
### [161] [Spectral Predictability as a Fast Reliability Indicator for Time Series Forecasting Model Selection](https://arxiv.org/abs/2511.08884)
...
Result: 研究发现当Ω值高时，大型时间序列基础模型(TSFMs)系统性地优于轻量级任务训练基线模型；当Ω值下降时，这种优势消失。计算Ω仅需每数据集几秒钟。
...
### [162] [FAST-CAD: A Fairness-Aware Framework for Non-Contact Stroke Diagnosis](https://arxiv.org/abs/2511.08887)
...
TL;DR: FAST-CAD是一个结合领域对抗训练和组分布鲁棒优化的公平非接触式卒中诊断框架，通过理论保证和公平性边界，在12个人口统计亚组上实现准确且公平的诊断。


<details>
  <summary>Details</summary>
Motivation: 现有卒中自动诊断方法在不同人口统计群体间存在公平性问题，可能加剧医疗不平等。需要开发既能保证诊断准确性又能确保跨群体公平性的AI系统。

Method: 结合领域对抗训练(DAT)和组分布鲁棒优化(Group-DRO)，使用自监督编码器和对抗领域判别学习人口统计不变表示，同时优化最差组风险以确保所有亚组的鲁棒性能。

Result: 实验表明该方法在保持跨人口统计群体公平性的同时实现了优越的诊断性能，理论分析支持了DAT+Group-DRO统一框架的有效性。

Conclusion: 该工作为公平医疗AI系统提供了实践进展和理论洞见，证明了理论驱动的公平性框架在医疗诊断中的有效性。
...
### [163] [Diffusion Policies with Value-Conditional Optimization for Offline Reinforcement Learning](https://arxiv.org/abs/2511.08922)
*Yunchang Ma,Tenglong Liu,Yixing Lan,Xin Yin,Changxin Zhang,Xinglong Zhang,Xin Xu*

Main category: cs.LG

TL;DR: 提出DIVO方法，通过价值条件优化解决离线强化学习中扩散模型的过度保守问题，在D4RL基准测试中取得优越性能
...
Method: 提出DIVO方法，引入二元加权机制，利用离线数据集中动作的优势值指导扩散模型训练，在策略改进阶段动态过滤高回报潜力动作
...
### [164] [TransactionGPT](https://arxiv.org/abs/2511.08939)
...
TL;DR: TransactionGPT (TGPT) 是一个针对消费交易数据的基础模型，采用创新的3D-Transformer架构，在支付交易数据的理解和生成方面表现出色，并在下游任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够理解和生成交易轨迹的基础模型，同时支持多种下游预测和分类任务，以处理大规模真实世界交易数据的复杂动态特性。

Method: 提出新颖的3D-Transformer架构，专门针对支付交易数据的复杂动态设计，包含增强模态融合和计算效率的创新设计，支持与下游目标的联合优化。

Result: 在十亿级真实交易数据上训练的TGPT显著提升了下游分类性能，在生成未来交易方面优于基线模型，比微调的大型语言模型具有更高的预测准确性和更快的训练推理速度。
...
### [165] [Robust Backdoor Removal by Reconstructing Trigger-Activated Changes in Latent Representation](https://arxiv.org/abs/2511.08944)
...
<details>
  <summary>Details</summary>
Motivation: 现有后门防御方法基于触发激活变化(TAC)来识别后门神经元，但由于TAC值估计不准确导致识别精度低，需要更精确的TAC重构方法。

Method: 将最小扰动迫使干净数据被分类到特定类别的问题建模为凸二次优化问题，其最优解作为TAC的替代，通过检测扰动L2范数的统计显著性来识别中毒类别，并利用中毒类别的扰动进行微调来移除后门。

Result: 在CIFAR-10、GTSRB和TinyImageNet上的实验表明，该方法在不同攻击类型、数据集和架构上都能实现优越的后门抑制效果，同时保持高准确率。

Conclusion: 该方法通过准确重构TAC值，能够有效识别和移除后门神经元，在多种场景下都优于现有防御方法。
...
### [166] [Bayesian Mixture of Experts For Large Language Models](https://arxiv.org/abs/2511.08968)
*Maryam Dialameh,Hossein Rajabzadeh,Weiwei Zhang,Walid Ahmed,Hyock Ju Kwon*

Main category: cs.LG

TL;DR: 提出了Bayesian-MoE框架，通过结构化拉普拉斯近似对MoE模型中的专家线性层进行贝叶斯推理，实现无需修改训练过程的后验不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常对添加的适配器模块应用贝叶斯推理，而Bayesian-MoE直接利用MoE模型中已有的专家路径，利用其模块化设计实现可处理的块级后验估计。

Method: 对每个专家的第二个线性层应用结构化拉普拉斯近似，使用Kronecker分解低秩近似建模曲率，推导可扩展的预测不确定性和边缘似然估计。

Result: 在常识推理基准测试中，使用Qwen1.5-MoE和DeepSeek-MoE的实验表明，Bayesian-MoE在预期校准误差和负对数似然方面优于基线方法。

Conclusion: Bayesian-MoE为微调后的大语言模型提供了有效的后验不确定性估计框架，能够提升下游决策的可靠性。
...
### [167] [Data reuse enables cost-efficient randomized trials of medical AI models](https://arxiv.org/abs/2511.08986)
*Michael Nercessian,Wenxin Zhang,Alexander Schubert,Daphne Yang,Maggie Chung,Ahmed Alaa,Adam Yala*

Main category: cs.LG

TL;DR: BRIDGE是一种数据重用的随机对照试验设计，用于AI风险模型验证，通过复用已完成试验中AI模型预测一致的数据，显著减少后续试验的入组需求和成本。


<details>
  <summary>Details</summary>
Motivation: 传统随机对照试验成本高、周期长，无法跟上AI模型快速迭代的速度，需要一种更高效的验证方法来加速AI工具在临床中的转化应用。

Method: BRIDGE试验设计复用已完成试验的参与者数据，当传统模型和更新模型预测一致时，减少后续试验的入组需求，并提供实用检查清单确保因果推断有效性和I类错误控制。

Result: 在乳腺癌、心血管疾病和脓毒症的真实数据集中，连续AI模型间存在高度一致性，高风险队列重叠率高达64.8%。模拟乳腺癌筛查研究显示，该设计可减少46.6%的入组需求，节省超过280万美元，同时保持80%的统计功效。

Conclusion: BRIDGE设计将试验转化为适应性、模块化研究，使每个模型迭代都能生成I级证据，加速AI在常规护理中的成本效益转化。
...
### [168] [Fast $k$-means clustering in Riemannian manifolds via Fréchet maps: Applications to large-dimensional SPD matrices](https://arxiv.org/abs/2511.08993)
*Ji Shi,Nicolas Charon,Andreas Mang,Demetrio Labate,Robert Azencott*

Main category: cs.LG

TL;DR: 提出了一种高效的高维非欧几里得流形数据聚类框架，通过p-Fréchet映射将流形数据嵌入到低维欧几里得空间，然后应用标准聚类方法，显著提高了计算效率。


<details>
  <summary>Details</summary>
Motivation: 解决标准内在方法在高维非欧几里得流形数据聚类中的计算挑战，提供更高效的替代方案。
...
Conclusion: 该方法为高维非欧几里得流形数据聚类提供了一种高效且准确的解决方案，特别适用于现有方法难以处理的情况。
...
### [169] [FLAD: Federated Learning for LLM-based Autonomous Driving in Vehicle-Edge-Cloud Networks](https://arxiv.org/abs/2511.09025)
*Tianao Xiang,Mingjian Zhi,Yuanguo Bi,Lin Cai,Yuhao Chen*

Main category: cs.LG

TL;DR: FLAD是一个基于联邦学习的大语言模型自动驾驶框架，利用分布式多模态传感器数据，通过云-边-车协同架构、智能并行化协作训练和知识蒸馏方法，在保护数据隐私的同时提升训练效率。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在自动驾驶领域训练面临的高计算传输成本、隐私保护等挑战，利用联邦学习实现车辆间的协作训练而不共享原始数据。

Method: 采用云-边-车协同架构减少通信延迟；智能并行化协作训练优化训练效率；知识蒸馏方法根据异构边缘数据个性化LLM；在NVIDIA Jetsons测试平台上实现原型。
...
### [170] [FedSDWC: Federated Synergistic Dual-Representation Weak Causal Learning for OOD](https://arxiv.org/abs/2511.09036)
*Zhenyuan Huang,Hui Zhang,Wenzhong Tang,Haijun Yang*

Main category: cs.LG

TL;DR: FedSDWC是一种联邦学习方法，通过建模不变特征和可变特征之间的弱因果影响来推断因果语义表示，有效处理数据分布差异问题，显著提升联邦学习的泛化能力和OOD检测能力。


<details>
  <summary>Details</summary>
Motivation: 随着数据隐私需求的增长和计算基础设施的进步，联邦学习成为重要的分布式学习范式。然而，数据分布差异（如协变量偏移和语义偏移）严重影响其在真实部署中的可靠性。
...
Conclusion: FedSDWC显著增强了联邦学习的泛化能力和OOD数据检测能力，理论上推导了特定条件下的泛化误差边界，并首次建立了与客户端先验分布的关系。
...
TL;DR: 本文提出了一种基于反馈增强的模型无关双人赌博算法，通过引入增强置信区间来整合稀疏的人类反馈，在多个交互式偏好获取基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有双人赌博算法在人类反馈稀疏时效率低下，而依赖参数化奖励模型的方法容易因模型设定错误而失效，因此需要探索基于反馈增强的替代方案。

Method: 引入增强置信区间来整合增强的人类反馈，在广义集中性属性下进行分析，并通过遗憾分析评估多因素性能权衡。

Result: 原型算法在推荐系统、多目标优化和大语言模型响应优化等多个交互式偏好获取基准测试中取得了有竞争力的性能表现。

Conclusion: 该方法为更广泛应用中可证明高效的交互式偏好获取提供了潜力，展示了模型无关双人赌博框架的改进价值。
...
### [172] [Break the Tie: Learning Cluster-Customized Category Relationships for Categorical Data Clustering](https://arxiv.org/abs/2511.09049)
...
<details>
  <summary>Details</summary>
Motivation: 分类属性在真实数据集聚类分析中普遍存在，但缺乏明确的类别关系定义，限制了紧凑数据聚类的探索。现有方法通常假设固定的拓扑关系，限制了其对不同聚类结构的适应性。

Method: 打破属性类别的固有关系约束，学习适合灵活准确揭示各种聚类分布的可定制距离度量。学习到的类别关系与欧几里得距离度量兼容，可扩展到包含数值和分类属性的混合数据集。

Result: 在12个真实基准数据集上的比较实验显示，所提方法具有优越的聚类准确率，平均排名为1.25，显著优于当前最佳方法的5.21排名。

Conclusion: 通过可学习的类别关系，聚类算法的拟合能力显著增强，能够更好地处理分类数据的聚类问题，并在混合数据集上实现无缝扩展。
...
### [173] [Human-Corrected Labels Learning: Enhancing Labels Quality via Human Correction of VLMs Discrepancies](https://arxiv.org/abs/2511.09063)
*Zhongnian Li,Lan Chen,Yixin Xu,Shi Xu,Xinzheng Xu*

Main category: cs.LG

TL;DR: 本文提出了一种名为HCL的新方法，通过选择性的人类校正来改进VLM生成的噪声标签，同时降低人工成本。该方法结合理论推导和条件概率估计，在弱监督场景下实现了更好的分类性能。


<details>
  <summary>Details</summary>
Motivation: 当前VLM生成的标签存在质量低（标签噪声）和缺乏错误校正机制的双重限制，需要一种既能提高标签质量又能减少人工成本的有效方法。

Method: 提出HCL设置，仅对VLM存在差异的实例进行选择性人工校正；推导风险一致估计器，结合人工校正标签和VLM预测来训练分类器；提出条件概率方法估计标签分布。

Result: 大量实验表明，该方法在分类性能上表现优异，对标签噪声具有鲁棒性，验证了HCL在实际弱监督场景中的有效性。

Conclusion: HCL方法通过选择性人类校正有效解决了VLM生成标签的质量问题，在保证标注质量的同时显著降低了人工成本，为弱监督学习提供了实用解决方案。
...
### [174] [FedPM: Federated Learning Using Second-order Optimization with Preconditioned Mixing of Local Parameters](https://arxiv.org/abs/2511.09100)
*Hiro Ishii,Kenta Niwa,Hiroshi Sawada,Akinori Fujino,Noboru Harada,Rio Yokota*

Main category: cs.LG

TL;DR: FedPM是一种新颖的联邦学习方法，通过引入二阶优化和预条件混合机制，有效解决了异构数据环境下局部预条件器漂移问题，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有的联邦学习方法在引入二阶优化时，由于在客户端执行迭代局部更新并在服务器上进行简单参数混合，导致局部预条件器漂移，特别是在异构数据设置下严重影响参数训练的收敛性。

Method: 通过将理想的二阶更新分解为服务器上的参数混合和客户端上的局部参数更新，在服务器端引入预条件混合机制，有效缓解局部预条件器的漂移问题。

Result: 理论分析表明在强凸目标函数和单次局部更新场景下具有超线性收敛速率。实验结果显示FedPM在测试准确率上相比传统简单混合方法有显著提升。

Conclusion: FedPM通过预条件混合机制有效解决了二阶优化在联邦学习中的局部预条件器漂移问题，充分发挥了二阶优化的潜力，在异构数据环境下表现出优越性能。
...
### [175] [Cost-Minimized Label-Flipping Poisoning Attack to LLM Alignment](https://arxiv.org/abs/2511.09105)
...
Conclusion: 这些发现揭示了RLHF/DPO流程中的基本脆弱性，并为评估其对低成本投毒攻击的鲁棒性提供了工具。
...
### [176] [Practical Global and Local Bounds in Gaussian Process Regression via Chaining](https://arxiv.org/abs/2511.09144)
*Junyi Liu,Stanley Kok*

Main category: cs.LG

TL;DR: 本文提出了一种基于链式方法的框架，用于估计未见数据上期望极值的上下界，无需访问特定输入位置。该方法提供了针对常用核函数的特定改进，并开发了局部不确定性量化方法。
...
Method: 提出基于链式方法的框架估计期望极值边界，提供RBF和Matérn等常用核函数的特定改进，避免分析松弛以提高数值紧密度，并开发基于分区直径的局部不确定性量化方法。

Result: 实验结果表明该方法在合成和真实数据集上优于现有方法，验证了理论发现。

Conclusion: 所提出的链式框架能够有效估计高斯过程回归的全局和局部不确定性边界，在理论和实验层面都表现出优越性能。
...
### [177] [Unsupervised Feature Selection Through Group Discovery](https://arxiv.org/abs/2511.09166)
*Shira Lifshitz,Ofir Lindenbaum,Gal Mishne,Ron Meir,Hadas Benisty*

Main category: cs.LG

TL;DR: GroupFS是一个端到端的可微分框架，能够联合发现潜在特征组并从中选择最具信息量的组，无需依赖预定义分组或标签监督。


<details>
  <summary>Details</summary>
Motivation: 现有的无监督特征选择方法通常单独评估特征，但信息信号往往来自相关特征组。现有方法要么依赖预定义分组，要么需要标签监督，限制了适用性。

Method: GroupFS在特征图和样本图上强制执行拉普拉斯平滑，并应用组稀疏正则化器来学习紧凑、结构化的表示。

Result: 在涵盖图像、表格数据和生物数据集的九个基准测试中，GroupFS在聚类任务上始终优于最先进的无监督特征选择方法，并选择出与有意义模式对齐的特征组。

Conclusion: GroupFS能够有效发现和选择特征组，在无监督特征选择任务中表现出色，且所选特征组与有意义的模式一致。
...
TL;DR: DFDT提出了一种解决决策变换器在跨域迁移中轨迹片段缝合问题的紧凑流程，通过两级数据过滤、优势条件令牌和Q引导正则化来提升缝合性和性能稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有跨域策略适应方法依赖单一过滤标准选择源轨迹片段，但存在状态结构错位、RTG不可比、动作跳跃等问题，导致RTG令牌失去连续性，影响决策变换器的推理能力。

Method: DFDT通过MMD不匹配和OT偏差两级数据过滤融合稀缺目标数据和可信源片段，用优势条件令牌替代RTG令牌，并应用Q引导正则化抑制连接处的值和动作跳跃。

Result: 在D4RL风格控制任务的重力、运动学和形态学迁移中，DFDT相比强离线RL和序列模型基线提高了回报和稳定性，并通过令牌缝合和序列语义稳定性分析验证了这些增益。

Conclusion: DFDT通过恢复轨迹片段的缝合性，有效解决了决策变换器在跨域迁移中的挑战，理论分析表明状态值和策略性能差距与MMD不匹配和OT偏差度量相关，且随着这些度量缩小而收紧。
...
Method: FSampler维护最近真实模型调用的去噪信号历史，使用二阶、三阶或四阶有限差分预测器外推下一个epsilon。在选定步骤用预测的epsilon替代模型调用，通过验证预测值的有限性和幅度，使用学习稳定器和梯度估计稳定器来纠正漂移和补偿局部曲率。
...
Conclusion: FSampler提供了一种有效的扩散采样加速方法，无需修改采样器公式，可集成到多种采样器中，在保持高保真度的同时显著减少计算开销。
...
### [180] [Sure! Here's a short and concise title for your paper: "Contamination in Generated Text Detection Benchmarks"](https://arxiv.org/abs/2511.09200)
*Philipp Dingfelder,Christian Riess*

Main category: cs.LG

TL;DR: 本文分析了AI生成文本检测器训练数据中的质量问题，发现DetectRL基准数据集中存在简单模式，导致检测器依赖这些模式作为捷径，容易受到欺骗攻击。作者通过数据清洗操作重新处理数据集，提高了检测器的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛应用，检测AI生成文本变得日益重要。然而现有的检测器训练数据集存在质量问题，特别是DetectRL基准中98.5%的Claude-LLM数据包含简单生成模式，这些模式可能被检测器作为捷径利用，导致容易受到欺骗攻击。

Method: 作者对DetectRL数据集进行了多种清洗操作，移除了包含简单AI生成模式的数据，如以"Sure! Here is the academic article abstract:"等引导词开头的文本，以及LLM拒绝执行提示任务的实例。

Result: 实验表明，经过数据清洗后，直接攻击检测器变得更加困难。重新处理后的数据集已公开可用。

Conclusion: 数据质量对AI生成文本检测器的训练至关重要，适当的数据清洗操作可以有效提高检测器的鲁棒性，使其不易受到基于简单模式的欺骗攻击。
...
### [181] [Stochastic Mean-Shift Clustering](https://arxiv.org/abs/2511.09202)
*Itshak Lapidot,Yann Sepulcre,Tom Trigano*

Main category: cs.LG

TL;DR: 提出了一种随机版本的均值漂移聚类算法，通过随机选择的数据点序列进行目标函数的部分梯度上升步骤，在合成数据和实际应用中表现优于标准均值漂移算法。


<details>
  <summary>Details</summary>
Motivation: 开发更高效的均值漂移聚类变体，通过随机化方法提高算法性能，特别是在大数据场景下的计算效率。

Method: 使用随机选择的数据点序列，通过目标函数的部分梯度上升步骤进行聚类，结合随机优化技术改进传统均值漂移算法。

Result: 在二维高斯混合分布合成数据上的实验表明，随机均值漂移聚类在大多数情况下优于标准均值漂移算法，并在说话人聚类等实际应用中表现良好。

Conclusion: 随机均值漂移聚类算法是传统均值漂移的有效改进，具有更好的性能和实际应用价值，特别是在大规模数据聚类任务中。
...
### [182] [CoCo-MILP: Inter-Variable Contrastive and Intra-Constraint Competitive MILP Solution Prediction](https://arxiv.org/abs/2511.09209)
*Tianle Pu,Jianing Li,Yingying Gao,Shixuan Liu,Zijie Geng,Haoyang Liu,Chao Chen,Changjun Fan*

Main category: cs.LG

TL;DR: CoCo-MILP提出了一种新的混合整数线性规划求解方法，通过建模变量间的对比和约束内的竞争关系，显著提升了GNN在MILP问题上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于GNN的MILP求解方法存在两个层面的不对齐问题：学习目标层面BCE损失忽略了变量的相对优先级，模型架构层面标准GNN消息传递平滑了变量表示，忽略了约束内的竞争关系。

Method: 提出了CoCo-MILP框架，包含两个核心组件：变量间对比损失(VCL)最大化0/1变量嵌入的边界，以及约束内竞争GNN层学习竞争变量的差异化表示。

Result: 在标准基准测试中，CoCo-MILP显著优于现有学习方法，相比传统求解器将求解差距降低了高达68.12%。

Conclusion: CoCo-MILP通过显式建模变量对比和约束内竞争关系，有效解决了现有GNN方法在MILP问题中的结构不对齐问题，实现了更好的求解性能。
...
### [183] [Parameter-Free Clustering via Self-Supervised Consensus Maximization (Extended Version)](https://arxiv.org/abs/2511.09211)
...
<details>
  <summary>Details</summary>
Motivation: 传统聚类方法严重依赖超参数（如聚类数量），限制了在现实场景中的应用，需要解决这一长期挑战。

Method: 采用层次凝聚聚类和聚类评估的集成过程，通过自监督学习创建结构感知的数据表示，并引入最近邻共识评分来测量不同表示间的一致性。
...
### [184] [Controllable protein design through Feynman-Kac steering](https://arxiv.org/abs/2511.09216)
*Erik Hartman,Jonas Wallin,Johan Malmström,Jimmy Olsson*

Main category: cs.LG

TL;DR: 本文扩展了Feynman-Kac (FK) 引导框架，将其应用于基于扩散的蛋白质设计，能够在推理时控制生成过程，引导采样朝向期望的结构或能量特征，同时保持扩散过程的多样性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的蛋白质结构生成模型在引导结果朝向特定功能或生化目标（如结合亲和力或序列组成）方面存在局限，需要开发能够在推理时进行控制的方法。

Method: 将FK引导框架与结构生成耦合，通过ProteinMPNN和全原子松弛精炼模型计算奖励，实现对序列和结构属性的同时生成控制。

Result: 在结合剂设计中，FK引导显著改善了预测的界面能量学，且计算开销极小。该方法适用于多样化的目标蛋白质。
...
### [185] [GuardFed: A Trustworthy Federated Learning Framework Against Dual-Facet Attacks](https://arxiv.org/abs/2511.09294)
*Yanli Li,Yanan Zhou,Zhongliang Guo,Nan Yang,Yuning Zhang,Huaming Chen,Dong Yuan,Weiping Ding,Witold Pedrycz*

Main category: cs.LG

TL;DR: 本文提出了双重攻击(DFA)威胁模型，同时破坏联邦学习的预测准确性和群体公平性，并开发了GuardFed防御框架来有效应对这种攻击。
...
Method: 提出了双重攻击(DFA)模型，包括同步DFA(S-DFA)和分割DFA(Sp-DFA)两种变体。为防御此类攻击，开发了GuardFed框架，使用少量干净服务器数据和合成样本维护公平感知参考模型，计算客户端双视角信任分数进行选择性聚合。

Result: 实验表明现有鲁棒联邦学习防御方法无法有效抵抗DFA攻击。GuardFed在各种非独立同分布和对抗条件下能持续保持准确性和公平性，性能优于现有鲁棒联邦学习方法。

Conclusion: 双重攻击对联邦学习构成严重威胁，而GuardFed防御框架能有效保护模型准确性和公平性，为联邦学习安全提供了重要保障。
...
### [186] [MARBLE: Multi-Armed Restless Bandits in Latent Markovian Environment](https://arxiv.org/abs/2511.09324)
*Mohsen Amiri,Konstantin Avrachenkov,Ibtihal El Mimouni,Sindri Magnússon*

Main category: cs.LG

TL;DR: MARBLE模型在传统多臂老虎机基础上引入潜在马尔可夫环境状态来处理非平稳环境，提出了马尔可夫平均可索引性准则，并证明了QWI算法在潜在状态切换下能收敛到最优策略。


<details>
  <summary>Details</summary>
Motivation: 传统RMAB模型假设固定动态，但在非平稳环境中这一假设常被违反，需要开发能适应环境变化的模型。

Method: 引入MARBLE模型，包含潜在马尔可夫环境状态；提出MAI准则作为松弛的可索引性假设；使用同步Q学习与Whittle索引(QWI)算法。

Result: 在数字孪生推荐系统上的实验验证表明，QWI能持续适应变化的潜在状态并收敛到最优策略。

Conclusion: MARBLE模型能有效处理非平稳环境中的决策问题，QWI算法在MAI准则下具有理论保证的收敛性。
...
### [187] [GAMMA_FLOW: Guided Analysis of Multi-label spectra by MAtrix Factorization for Lightweight Operational Workflows](https://arxiv.org/abs/2511.09326)
*Viola Rädle,Tilman Hartwig,Benjamin Oesen,Emily Alice Kröger,Julius Vogt,Eike Gericke,Martin Baron*

Main category: cs.LG

TL;DR: GAMMA_FLOW是一个开源的Python软件包，用于光谱数据的实时分析，支持分类、去噪、分解和异常检测，采用监督式非负矩阵分解方法实现快速高效的分析。


<details>
  <summary>Details</summary>
Motivation: 开发一个开源、灵活的光谱分析工具，作为专有软件的替代方案，降低计算成本并提高分析效率。

Method: 使用监督式非负矩阵分解（NMF）进行降维，避免依赖大型计算密集型模型，实现快速高效的分析。

Result: 分类准确率超过90%，能够实现可靠的光谱自动解释，适用于任何类型的一维光谱数据。

Conclusion: GAMMA_FLOW提供了一个开源、高效且适应性强的光谱分析解决方案，适用于研究和工业领域的各种应用。
...
### [188] [Distribution-Based Feature Attribution for Explaining the Predictions of Any Classifier](https://arxiv.org/abs/2511.09332)
...
Conclusion: DFAX克服了现有方法的局限性，为特征归因问题提供了一个正式且实用的解决方案。
...
### [189] [From Decision Trees to Boolean Logic: A Fast and Unified SHAP Algorithm](https://arxiv.org/abs/2511.09376)
*Alexander Nadel,Ron Wettenstein*

Main category: cs.LG

TL;DR: WOODELF是一个新的SHAP算法，通过将决策树、博弈论和布尔逻辑集成到统一框架中，实现了线性时间计算背景SHAP值，在CPU和GPU上都能高效运行。


<details>
  <summary>Details</summary>
Motivation: 现有的SHAP计算方法效率较低，特别是在处理大规模数据集时。WOODELF旨在提供更高效的SHAP值计算方案，支持多种博弈论值的快速计算。

Method: 为每个消费者构建伪布尔公式，捕捉特征值、决策树集成结构和整个背景数据集，然后利用这种表示在线性时间内计算背景SHAP值。

Result: 在包含300万行、500万背景样本和127个特征的数据集上，WOODELF在CPU上计算所有背景Shapley值仅需162秒，GPU上仅需16秒，相比最佳方法分别实现了16倍和165倍的加速。
...
### [190] [Diffusion-based Sinogram Interpolation for Limited Angle PET](https://arxiv.org/abs/2511.09383)
...
Conclusion: 条件扩散模型能够有效处理PET成像中的欠采样问题，支持新型探测器几何结构在真实临床环境中的应用。
...
### [191] [Potent but Stealthy: Rethink Profile Pollution against Sequential Recommendation via Bi-level Constrained Reinforcement Paradigm](https://arxiv.org/abs/2511.09392)
*Jiajie Su,Zihan Nan,Yunshan Ma,Xiaobo Xia,Xiaohua Feng,Weiming Liu,Xiaolin Zheng,Chaochao Chen*

Main category: cs.LG

TL;DR: 本文提出了一种名为CREAT的约束强化学习驱动的配置文件污染攻击方法，通过双层次优化框架和多奖励强化学习来平衡攻击效果和隐蔽性，解决了现有配置文件污染攻击方法在细粒度扰动和可检测性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的顺序推荐系统容易受到对抗攻击，但传统的数据投毒攻击需要大规模用户访问或虚假配置文件，缺乏实用性。本文关注配置文件污染攻击，旨在通过轻微污染部分用户交互来诱导目标误预测。

Method: 提出CREAT方法，结合双层次优化框架和多奖励强化学习：1）开发模式平衡奖励策略，集成模式反转奖励和分布一致性奖励；2）采用约束组相对强化学习范式，通过动态屏障约束和组共享经验回放实现逐步扰动。

Result: 大量实验证明了CREAT方法的有效性，能够在保持隐蔽性的同时实现目标污染攻击。

Conclusion: CREAT方法通过约束强化学习框架成功解决了配置文件污染攻击中的细粒度扰动和可检测性问题，为顺序推荐系统的安全性研究提供了新的思路。
...
### [192] [Abstract Gradient Training: A Unified Certification Framework for Data Poisoning, Unlearning, and Differential Privacy](https://arxiv.org/abs/2511.09400)
...
<details>
  <summary>Details</summary>
Motivation: 推理时数据扰动的影响已被广泛研究，但针对训练数据扰动的认证仍相对未被充分探索。这些扰动出现在三个关键场景：对抗性数据投毒、机器遗忘和差分隐私。

Method: 通过边界可达参数集（建立可证明的参数空间边界），AGT为通过一阶优化方法训练的模型行为分析提供了形式化方法。

Result: AGT提供了一个统一框架来认证给定模型和训练程序对训练数据扰动的鲁棒性。

Conclusion: AGT框架能够为训练数据扰动提供形式化的鲁棒性认证，填补了该领域的研究空白。
...
### [193] [Spatio-Temporal Graph Unlearning](https://arxiv.org/abs/2511.09404)
*Qiming Guo,Wenbo Sun,Wenlu Wang*

Main category: cs.LG

TL;DR: CallosumNet是一个受胼胝体启发的时空图遗忘框架，通过增强子图构建和全局神经节桥接技术，实现在隐私法规要求下的高效节点级数据遗忘。


<details>
  <summary>Details</summary>
Motivation: 由于GDPR和CCPA等隐私法规要求完全遗忘未经授权的数据，而现有方法主要针对静态图设计，无法高效擦除时空图中的单个节点，需要开发专门的时空图遗忘方法。

Method: 提出CallosumNet框架，包含两个关键技术：1) 增强子图构建(ESC)，基于生物启发的虚拟神经节自适应构建多个局部子图；2) 全局神经节桥接(GGB)，从局部子图重建全局时空依赖关系。

Result: 在四个真实世界数据集上的实验表明，CallosumNet仅产生1%-2%的相对MAE损失，显著优于现有基线方法。消融研究验证了所提技术的有效性。

Conclusion: CallosumNet能够实现完全遗忘，在隐私合规要求下提供高效的时空图节点级数据删除解决方案。
...
### [194] [Probing then Editing: A Push-Pull Framework for Retain-Free Machine Unlearning in Industrial IoT](https://arxiv.org/abs/2511.09414)
*Jiao Chen,Weihua Li,Jianhua Tang*

Main category: cs.LG

TL;DR: 提出了一种名为PTE（Probing then Editing）的无保留遗忘框架，通过探测-编辑过程实现高效知识遗忘，仅需遗忘数据和原始模型，无需保留数据。
...
Method: PTE采用探测-编辑两阶段：首先通过梯度上升探测遗忘类决策边界邻域并生成编辑指令，然后进行推-拉协同优化——推分支主动拆解目标类决策区域，拉分支通过掩码知识蒸馏锚定保留类知识。

Result: 实验结果表明PTE在CWRU和SCUT-FD等多个通用和工业基准上，在遗忘效果和模型效用之间取得了优秀平衡。

Conclusion: PTE框架实现了仅使用遗忘数据和原始模型的高效平衡知识编辑，为工业物联网环境中的模型知识管理提供了有效解决方案。
...
### [195] [AdaCuRL: Adaptive Curriculum Reinforcement Learning with Invalid Sample Mitigation and Historical Revisiting](https://arxiv.org/abs/2511.09478)
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在混合难度样本上训练时存在梯度饥饿和策略退化问题，而传统方法依赖人工构建高质量思维链数据或面临课程设计困难、灾难性遗忘等挑战。

Method: 提出AdaCuRL框架，整合粗到细难度估计和自适应课程调度，动态对齐数据难度与模型能力，包含数据重访机制防止灾难性遗忘，并采用自适应参考和稀疏KL策略避免策略退化。

Result: 在多种推理基准测试上的广泛实验表明，AdaCuRL在LLMs和MLLMs上均能持续实现显著的性能提升。

Conclusion: AdaCuRL通过自适应课程强化学习有效解决了现有方法的局限性，为大语言模型的推理能力提升提供了有效解决方案。
...
### [196] [Transformer Semantic Genetic Programming for d-dimensional Symbolic Regression Problems](https://arxiv.org/abs/2511.09416)
*Philipp Anthes,Dominik Sobania,Franz Rothlauf*

Main category: cs.LG

TL;DR: TSGP使用预训练transformer模型作为变异算子，通过控制语义相似性生成子代程序，在符号回归问题上显著优于传统GP方法和其他语义GP方法。


<details>
  <summary>Details</summary>
Motivation: 传统语义GP方法依赖固定的语法转换，无法学习多样的结构变化。TSGP旨在通过学习语义相似的多样化结构变化来改进符号回归性能。
...
Conclusion: TSGP证明了transformer模型在语义搜索中的有效性，SD_t参数能够有效平衡探索与利用，控制语义空间中的步长。
...
### [197] [LLM-Guided Dynamic-UMAP for Personalized Federated Graph Learning](https://arxiv.org/abs/2511.09438)
*Sai Puppala,Ismail Hossain,Md Jahangir Alam,Tanzim Ahad,Sajedul Talukder*

Main category: cs.LG

TL;DR: 提出一种结合大语言模型和图机器学习的个性化联邦学习方法，通过数据增强、提示调优和上下文学习来解决稀疏图、隐私约束和低资源场景下的节点分类和链接预测问题。


<details>
  <summary>Details</summary>
Motivation: 解决在个性化、隐私约束和低资源设置下图机器学习的挑战，利用大语言模型增强图表示学习能力。

Method: 结合数据增强处理稀疏图，使用提示和指令调优使基础模型适应图任务，通过上下文学习提供少样本图推理信号，在贝叶斯变分目标中参数化动态UMAP流形，并采用跨模态正则化器对齐语言模型和图结构表示。

Result: 方法支持节点分类和链接预测，应用于知识图谱补全、推荐式链接预测以及引文和产品图，提供了变分聚合过程的收敛性分析和基于时刻会计的差分隐私威胁模型。

Conclusion: 该方法为LLM辅助的图机器学习提供了有效的解决方案，并讨论了相应的评估基准考虑。
...
### [198] [How does the Performance of the Data-driven Traffic Flow Forecasting Models deteriorate with Increasing Forecasting Horizon? An Extensive Approach Considering Statistical, Machine Learning and Deep Learning Models](https://arxiv.org/abs/2511.09450)
*Amanta Sherfenaz,Nazmul Haque,Protiva Sadhukhan Prova,Md Asif Raihan,Md. Hadiuzzaman*

Main category: cs.LG

TL;DR: 本研究评估了统计、机器学习和深度学习模型在交通速度和流量预测中的性能，使用加州Harbor Freeway的真实数据。ANFIS-GP在早期预测窗口表现最佳，而Bi-LSTM在中长期预测中更稳健。研究量化了模型性能退化，并指出混合模型是未来的有前景方向。
...
Method: 使用来自加州交通绩效测量系统（PeMS）的真实数据，评估统计、机器学习和深度学习模型在20个预测窗口（最长1小时40分钟）内的性能，采用RMSE、MAE和R-Square指标。通过对数变换量化模型性能退化。

Result: ANFIS-GP在早期预测窗口表现最佳（RMSE 0.038，MAE 0.0276，R-Square 0.9983），而Bi-LSTM在中长期预测中更稳健（RMSE 0.1863，MAE 0.0833，R-Square 0.987）。Bi-LSTM具有最平坦的性能退化斜率（RMSE 0.0454，MAE 0.0545）。

Conclusion: 研究确定了混合模型作为未来有前景的研究方向，能够结合不同模型的优势来提升交通预测性能。
...
### [199] [Enhancing Explainability in Solar Energetic Particle Event Prediction: A Global Feature Mapping Approach](https://arxiv.org/abs/2511.09475)
*Anli Ji,Pranjal Patil,Chetraj Pandey,Manolis K. Georgoulis,Berkay Aydin*

Main category: cs.LG

TL;DR: 提出了一个集成全局解释和特设特征映射的新框架，以提高太阳高能粒子事件预测模型的透明度，帮助理解预测背后的物理原因。


<details>
  <summary>Details</summary>
Motivation: 现有太阳高能粒子事件预测方法多为黑盒模型，难以解释结果和了解事件的物理成因，仅能获得预测结果。

Method: 开发了一个结合全局解释和特设特征映射的框架，使用341个SEP事件数据集（包括244个重要质子事件）进行验证，涵盖太阳周期22、23和24。

Result: 通过主要SEP事件的可解释性案例研究，展示了该方法如何提高可解释性并促进对SEP事件预测的物理理解。

Conclusion: 该框架成功增强了模型透明度，为太阳物理学家提供了更深入理解SEP事件预测决策过程的工具。
...
### [200] [Latent Planning via Embedding Arithmetic: A Contrastive Approach to Strategic Reasoning](https://arxiv.org/abs/2511.09477)
*Andrew Hamara,Greg Hamerly,Pablo Rivas,Andrew C. Freeman*

Main category: cs.LG

TL;DR: SOLIS方法通过监督对比学习构建评估对齐的嵌入空间，在该空间中通过向量操作进行规划，在约束条件下达到竞争性水平。


<details>
  <summary>Details</summary>
Motivation: 研究是否可以直接在评估对齐的嵌入空间中进行规划，而不是训练策略或价值函数，以提供比传统动态模型或策略学习更轻量化的替代方案。

Method: 使用监督对比学习构建SOLIS嵌入空间，其中结果相似性通过邻近度捕获，单个全局优势向量从输到赢区域定向空间，候选动作根据与该方向的对齐度进行排序。

Result: 在国际象棋中，SOLIS仅使用浅层搜索和学习的嵌入指导，在约束条件下达到了竞争性强度。

Conclusion: 评估对齐的潜在规划为传统动态模型或策略学习提供了轻量级替代方案，证明了在嵌入空间中直接进行规划的有效性。
...
### [201] [PDAC: Efficient Coreset Selection for Continual Learning via Probability Density Awareness](https://arxiv.org/abs/2511.09487)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于排练的持续学习方法依赖于核心集选择，但主流方法采用双层优化问题，计算成本高昂，限制了实际效率。本文旨在提供更高效的核心集构建选择逻辑和方案。

Method: 首先通过局部误差分解分析缓冲区训练模型与贝叶斯最优模型之间的均方误差，发现高概率密度样本在误差抑制中起主导作用。提出PDAC方法，利用投影高斯混合模型估计样本的联合密度，实现基于密度的缓冲区选择。针对流式场景，引入流式期望最大化算法，提出SPDAC方法。
...
### [202] [AutoSynth: Automated Workflow Optimization for High-Quality Synthetic Dataset Generation via Monte Carlo Tree Search](https://arxiv.org/abs/2511.09488)
*Shuzhen Bi,Chang Song,Siyu Song,Jinze Lv,Jian Chen,Xinyun Wang,Aimin Zhou,Hao Hao*

Main category: cs.LG

TL;DR: AutoSynth是一个无需参考数据集即可自动发现和优化工作流的框架，通过蒙特卡洛树搜索和新型无数据集混合奖励来解决冷启动问题，在主观教育任务中显著减少人工工作量（从5-7小时降至30分钟）。


<details>
  <summary>Details</summary>
Motivation: 解决监督微调LLMs时高质量数据集手动标注成本高昂的问题，特别是针对主观、开放式任务缺乏客观真实标注的冷启动困境。

Method: 将工作流发现和优化重新构建为蒙特卡洛树搜索问题，使用无数据集混合奖励指导搜索，包含两个LLM作为评判器组件：一个评估样本质量，另一个评估工作流代码和提示质量。

Result: 在主观教育任务中，AutoSynth生成的模型数据显著优于基线（40-51% vs 2-5%），在某些指标上匹配或超越专家设计的工作流，同时将人工工作量减少90%以上。
...
### [203] [GenePheno: Interpretable Gene Knockout-Induced Phenotype Abnormality Prediction from Gene Sequences](https://arxiv.org/abs/2511.09512)
*Jingquan Yan,Yuwei Miao,Lei Yu,Yuzhi Guo,Xue Xiao,Lin Xu,Junzhou Huang*

Main category: cs.LG

TL;DR: GenePheno是一个可解释的多标签预测框架，直接从基因序列预测基因敲除诱导的表型异常，解决了现有方法依赖人工整理遗传信息的局限性。


<details>
  <summary>Details</summary>
Motivation: 探索基因序列如何塑造表型是生物学的基本挑战，但现有方法要么关注特定基因变体对有限表型的影响，要么依赖人工整理的遗传信息，限制了可扩展性和泛化能力。
...
Conclusion: GenePheno是首个直接从基因序列预测多表型异常的可解释框架，为可扩展的假设驱动实验提供了新途径。
