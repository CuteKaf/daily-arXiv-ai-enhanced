...
Method: 提出ABBSPO框架，包含两个核心组件：1) 自适应边界框缩放(ABBS)，适当缩放GT水平边界框以优化每个预测RBox的尺寸；2) 对称先验角度(SPA)损失，利用航空目标的固有对称性进行自监督学习，解决学习崩溃问题。

Result: 大量实验结果表明，ABBSPO实现了最先进的性能，优于现有方法。

Conclusion: ABBSPO框架通过自适应尺度优化和对称性利用，有效解决了弱监督定向目标检测中的关键问题，为HBox监督的OOD提供了更准确的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散去噪模型在离散领域无法超越标准自回归模型，只达到相当水平。研究者希望结合两者的优势：扩散模型的双向和精炼能力，以及自回归模型强大的语言结构能力。

Method: 提出Show, Suggest and Tell (SST)范式，采用扩散模型为自回归生成提供建议，而不是替代自回归模型。具体来说，扩散模型提供候选建议，自回归模型基于这些建议生成最终描述。

Result: 在COCO数据集上达到125.1 CIDEr-D的SOTA结果，无需强化学习，比自回归和扩散模型的SOTA结果分别高出1.5和2.5分。实验显示建议质量与描述质量呈正相关。

Conclusion: 通过结合扩散模型的建议能力和自回归模型的语言结构，SST展示了这一研究方向的潜力，为当前未被充分探索但前景广阔的研究方向提供了验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法通常针对特定预测方向和输入变量训练条件分布模型，需要为不同任务分别训练模型。MetaVoxel旨在通过建模联合分布来统一这些任务，减少模型复杂性并提高临床适用性。

Method: MetaVoxel采用生成式联合扩散建模框架，学习一个跨越所有变量（包括影像数据和临床元数据）的单一扩散过程。该方法使用超过10,000个T1加权MRI扫描和来自九个数据集的临床元数据进行训练。

Result: 单个MetaVoxel模型能够执行图像生成、年龄估计和性别预测任务，性能与专门针对这些任务训练的基线模型相当。实验还展示了其在灵活推理方面的能力。

Conclusion: 联合多模态扩散建模为统一医学AI模型和扩大临床适用性提供了有前景的方向，MetaVoxel框架展示了通过单一模型处理多种医学任务的潜力。
...
Method: 提出独立密度估计(IDE)方法，学习句子中单个单词与图像对应特征之间的连接。构建两种模型：第一种使用完全解耦的视觉表示作为输入，第二种利用变分自编码器从原始图像中获取部分解耦特征。还提出基于熵的组合推理方法来组合句子中每个单词的预测

Result: 与现有模型相比，提出的模型在各种数据集上对未见过的组合表现出更优越的泛化能力

Conclusion: IDE方法有效解决了视觉语言模型的组合泛化问题，通过建立单词与图像特征的直接连接实现了更好的组合泛化性能
...
TL;DR: TraceFlow是一个用于动态镜面场景高保真渲染的新框架，通过残差材料增强的2D高斯泼溅表示和动态环境高斯方法，结合混合渲染管线实现精确的反射方向估计和物理准确的反射建模。
...
Method: 1. 提出残差材料增强的2D高斯泼溅表示，建模动态几何和材料属性以计算精确反射光线；2. 引入动态环境高斯和混合渲染管线，将渲染分解为漫反射和镜面反射组件，通过光栅化和光线追踪实现物理基础的镜面合成；3. 设计从粗到精的训练策略，提高优化稳定性并促进物理有意义的分解。

Result: 在动态场景基准测试上的广泛实验表明，TraceFlow在定量和定性上都优于先前方法，在复杂动态环境中产生更锐利、更真实的镜面反射。

Conclusion: TraceFlow通过创新的表示方法和混合渲染管线，成功解决了动态镜面场景渲染的关键挑战，实现了高质量的物理准确反射效果，为动态场景渲染提供了新的解决方案。
...
TL;DR: 提出分层实例跟踪新任务，建立首个相关基准数据集，包含2765个实体、552个视频、40个类别，评估显示任务具有挑战性


<details>
  <summary>Details</summary>
Motivation: 现有跟踪任务主要关注单个对象或实例，缺乏对对象及其部件之间层次关系的跟踪。需要开发能够同时跟踪预定义类别对象和部件并保持其层次关系的新任务

Method: 提出分层实例跟踪新任务，建立首个支持该任务的基准数据集，包含2765个独特实体、552个视频、40个类别（涵盖对象和部件）。评估了针对该任务定制的四种模型的七个变体

Result: 评估结果显示新数据集具有挑战性，现有模型在同时跟踪对象和部件并保持层次关系方面面临困难

Conclusion: 分层实例跟踪是一个新颖且有挑战性的任务，提出的数据集为未来研究提供了基准，有助于推动对复杂视觉场景中层次关系的理解
...
TL;DR: LCDrive提出了一种用于自动驾驶的潜在语言思维链推理模型，用动作对齐的潜在空间替代自然语言进行推理，通过动作提议token和世界模型token的交替表示来统一推理和决策。
...
Method: 提出Latent-CoT-Drive模型，使用动作对齐的潜在空间进行推理：1) 动作提议token使用与输出动作相同的词汇表；2) 世界模型token基于学习的潜在世界模型表示动作的未来结果。通过真实场景未来轨迹监督冷启动，然后通过闭环强化学习进行后训练。

Result: 在大规模端到端驾驶基准测试中，LCDrive相比非推理和文本推理基线实现了更快的推理速度、更好的轨迹质量，以及从交互式强化学习中获得的更大改进。

Conclusion: 潜在语言思维链推理比自然语言推理更高效，通过动作对齐的潜在空间统一推理和决策能够显著提升自动驾驶模型的性能和安全性。
...
TL;DR: 该论文提出SWIFT方法，通过简单的分类器初始化和温度调优技术，解决视觉语言模型在半监督少样本学习中置信度不足的问题，显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有半监督少样本学习方法忽视了开源视觉语言模型及其预训练数据的潜力，而相关领域已成功利用这些资源。作者发现直接应用现有SSL方法微调VLM效果不佳，原因是VLM产生过于"平坦"的softmax概率分布，导致未标记数据利用率低和监督信号弱

Method: 提出SWIFT方法：1）分类器初始化：为VLM添加随机初始化的分类器头；2）温度调优：调整softmax温度参数以增加伪标签置信度；3）分阶段微调：在有限标记数据、丰富未标记数据和从VLM预训练集中检索的任务相关但嘈杂数据上进行分阶段微调

Result: 在五个SSFSL基准测试上的广泛实验表明，SWIFT比最近的FSL和SSL方法高出约5个准确率点，甚至能与使用真实标签微调VLM的监督学习方法相媲美

Conclusion: 通过简单的分类器初始化和温度调优技术，可以有效解决VLM在半监督少样本学习中的置信度问题，使现有SSL方法能够有效利用VLM和开源数据资源，实现接近监督学习的性能
...
TL;DR: RobustSora是一个评估AIGC视频检测中数字水印鲁棒性的基准测试，通过构建包含真实和生成视频的四种类型数据集，揭示了检测器对水印的部分依赖性。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成视频技术快速发展，现有AIGC视频检测基准忽视了关键因素：许多先进生成模型在输出中嵌入了数字水印，检测器可能部分依赖这些水印模式进行判断，需要评估这种依赖性对检测性能的影响。

Method: 构建包含6,500个视频的数据集，分为四类：真实-干净视频、真实-伪造水印视频、生成-带水印视频、生成-去水印视频。提出两个评估任务：任务I测试对去水印AI视频的检测性能，任务II评估对带有伪造水印的真实视频的误报率。使用十种模型进行实验，涵盖专用AIGC检测器、Transformer架构和MLLM方法。

Result: 实验显示水印操作导致检测性能变化2-8个百分点。基于Transformer的模型表现出一致的中等依赖性（6-8pp），而MLLM模型显示出多样化的模式（2-8pp）。这些发现表明检测器部分依赖水印进行判断。

Conclusion: AIGC视频检测器确实部分依赖数字水印模式，需要开发水印感知的训练策略来提高检测鲁棒性。RobustSora为推进鲁棒的AIGC检测研究提供了重要工具。
...
TL;DR: THE-Pose提出了一种新的类别级6D姿态估计框架，通过表面嵌入和混合图融合利用拓扑先验，克服了现有3D图卷积方法仅关注局部几何的局限性，在REAL275数据集上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 类别级物体姿态估计需要全局上下文和局部结构来应对类内变化。然而，现有的3D图卷积方法只关注局部几何和深度信息，对复杂物体和视觉模糊性较为脆弱。为了解决这一问题，需要引入更鲁棒的特征表示方法。

Method: THE-Pose框架通过表面嵌入提取一致且不变的拓扑特征，然后通过混合图融合模块自适应地整合拓扑特征与点云特征，有效桥接2D图像上下文和3D几何结构。

Result: 在REAL275数据集上的实验表明，THE-Pose相比3D-GC基线方法（HS-Pose）提升了35.8%，在所有关键指标上超越了先前的最优方法7.2%，对未见或复杂物体即使在严重遮挡下也能保持稳定。

Conclusion: 通过引入拓扑先验和混合图融合，THE-Pose有效克服了现有3D图卷积方法的局限性，为类别级6D姿态估计提供了更鲁棒的解决方案，显著提升了性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 超声心动图序列中心腔的准确分割对于心脏功能的定量分析至关重要，但成像噪声、伪影以及心脏的形变和运动给分割算法带来挑战。现有方法在捕捉长距离时空依赖性和保持计算效率之间存在权衡问题。
...
Conclusion: GDKVM通过创新的架构设计有效解决了超声心动图视频分割中的挑战，在精度、鲁棒性和计算效率方面取得了平衡，为临床诊断和治疗提供了可靠的工具。
...
TL;DR: LLM-NCD：一种多模态新类发现框架，通过融合视觉-文本语义和原型引导聚类，在已知类知识基础上发现未知类，相比现有方法在未知类准确率上提升达25.3%，且对长尾分布具有独特鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉特征的新类发现方法存在特征区分性不足和数据长尾分布等局限性，需要更有效的多模态方法来提升新类发现性能。

Method: 提出LLM-NCD多模态框架，通过联合优化已知类图像和文本特征来建模聚类中心和语义原型，采用双阶段发现机制：通过语义亲和度阈值动态分离已知/新类样本，并结合自适应聚类。

Result: 在CIFAR-100数据集上，相比现有方法，该方法在未知类准确率上提升达25.3%，且首次在新类发现文献中展现出对长尾分布的独特鲁棒性。

Conclusion: LLM-NCD通过融合视觉-文本语义和原型引导聚类，有效解决了现有新类发现方法的局限性，显著提升了新类发现性能，特别是在处理长尾分布数据方面表现出色。
...
TL;DR: 该论文提出了一种名为SAAD的对抗蒸馏方法，通过根据对抗样本的可迁移性重新加权训练样本，解决了强教师模型不一定能产生更强鲁棒性学生模型的问题。


<details>
  <summary>Details</summary>
Motivation: 现有对抗蒸馏方法往往忽视使用最先进的鲁棒教师模型，且发现更强的教师模型不一定能产生更鲁棒的学生模型（鲁棒饱和现象）。传统解释归因于容量差距，但作者认为这种解释不完整，需要更深入理解对抗鲁棒性转移机制。

Method: 提出样本自适应对抗蒸馏（SAAD），核心思想是识别对抗可迁移性（学生制作的对抗样本对教师仍然有效的比例）作为鲁棒性转移的关键因素。SAAD通过测量对抗可迁移性来重新加权训练样本，而不增加额外计算成本。

Result: 在CIFAR-10、CIFAR-100和Tiny-ImageNet数据集上的实验表明，SAAD在AutoAttack鲁棒性方面持续优于先前方法，证明了该方法在对抗蒸馏中的有效性。

Conclusion: 对抗可迁移性是成功转移鲁棒性的关键因素，SAAD方法通过自适应重新加权训练样本，有效解决了鲁棒饱和问题，为对抗蒸馏提供了新的解决方案。
...
TL;DR: MotionEdit是一个专注于运动中心图像编辑的新数据集和基准，MotionNFT是一个通过运动对齐奖励提升现有模型运动编辑能力的微调框架。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑数据集主要关注静态外观变化，缺乏高质量的运动编辑数据。运动中心图像编辑（修改主体动作和交互同时保持身份、结构和物理合理性）在科学上具有挑战性，在视频合成和动画等应用中具有实际意义。

Method: 1) 构建MotionEdit数据集：从连续视频中提取和验证高质量的运动变换图像对；2) 创建MotionEdit-Bench基准：使用生成式、判别式和偏好性指标评估模型性能；3) 提出MotionNFT框架：通过计算输入图像与模型编辑图像之间的运动流与真实运动的对齐奖励，引导模型实现准确运动变换。

Result: 基准测试显示现有最先进的基于扩散的编辑模型在运动编辑任务上表现不佳。MotionNFT框架在FLUX.1 Kontext和Qwen-Image-Edit模型上的实验表明，该框架能持续提升编辑质量和运动保真度，且不损害一般编辑能力。
...
TL;DR: ShotDirector框架通过参数级相机控制和分层编辑模式感知提示，实现电影风格的可控镜头转场生成，解决了现有方法忽视电影编辑模式和叙事表达的问题。


<details>
  <summary>Details</summary>
Motivation: 当前多镜头视频生成方法主要关注镜头间的低级视觉一致性，忽视了镜头转场设计和电影语言对连贯叙事表达的作用，导致镜头变化仅仅是顺序切换而缺乏有意的电影编辑模式。

Method: 提出ShotDirector框架：1）采用相机控制模块，整合6自由度位姿和内在参数设置，实现精确相机信息注入；2）使用镜头感知掩码机制，引入专业编辑模式感知的分层提示，实现对镜头内容的细粒度控制。

Result: 构建了ShotWeaver40K数据集捕捉电影风格编辑模式的先验知识，开发了一套可控多镜头视频生成的评估指标。大量实验证明了该框架的有效性。

Conclusion: ShotDirector通过结合参数级条件和高级语义指导，实现了电影风格的可控镜头转场，为多镜头视频生成提供了更专业的叙事表达和编辑控制能力。
...
TL;DR: Efficient-VLN通过渐进式记忆机制和可学习递归记忆减少长序列处理开销，引入动态混合策略平衡探索效率，在保持SOTA性能的同时大幅降低训练成本
...
Method: 提出Efficient-VLN：1) 渐进式记忆机制动态分配更多token给近期观察；2) 可学习递归记忆利用可学习token的键值缓存作为记忆状态；3) 动态混合策略平衡探索效率权衡

Result: 在R2R-CE上达到64.2% SR，RxR-CE上达到67.0% SR的SOTA性能，仅消耗282 H800 GPU小时，相比现有方法大幅降低训练开销

Conclusion: Efficient-VLN通过高效记忆机制和动态策略有效解决了VLN训练中的计算负担和探索效率问题，在保持高性能的同时显著降低了训练成本
...
TL;DR: 提出基于原型驱动的弱监督病理图像语义分割框架，通过视觉-语言对齐和多尺度金字塔模块提升区域发现能力，在BCSS-WSSS基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决病理图像弱监督语义分割中的三个主要挑战：类间同质性、类内异质性以及CAM监督的区域收缩效应，旨在降低标注成本的同时提升分割性能。

Method: 1. 采用CoOp风格的可学习提示调优生成文本原型；2. 结合可学习图像原型构建双模态原型库；3. 引入多尺度金字塔模块缓解ViT表示的过度平滑问题；4. 利用文本语义和视觉原型学习的互补性。

Result: 在BCSS-WSSS基准测试中超越了现有最先进方法，分析显示文本描述多样性、上下文长度以及文本与图像原型的互补行为对性能有显著提升。

Conclusion: 联合利用文本语义和视觉原型学习是数字病理学中弱监督语义分割的有效方法，双模态原型库和多尺度增强能够显著改善区域发现和定位质量。
...
Conclusion: Point2Pose通过生成式方法有效解决了3D人体姿态估计的挑战，同时提出的MVPose3D数据集为多模态研究提供了重要资源
...
TL;DR: EchoingPixels是一个音频-视觉大语言模型(LLM)的令牌压缩框架，通过跨模态语义筛(CS2)模块实现联合音频-视觉令牌池的智能压缩，仅需5-20%原始令牌即可达到基线性能，同时实现2-3倍加速和内存减少。


<details>
  <summary>Details</summary>
Motivation: 音频-视觉大语言模型面临大量音频和视频令牌带来的计算开销问题。现有的单模态令牌压缩方法无法利用音频-视觉跨模态协同效应，且静态的模态预算分配无法适应音频和视频不同的动态信息密度。如何对联合音频-视觉流进行令牌压缩仍是一个未解决的瓶颈。

Method: 提出EchoingPixels框架，核心是跨模态语义筛(CS2)模块，它通过早期音频-视觉交互，共同关注联合多模态流，从整个音频-视觉令牌池中减少令牌，而不是为每个模态使用固定预算。这种单池方法允许自适应分配令牌预算并动态识别重要令牌。同时设计同步增强RoPE(Sync-RoPE)来保持稀疏选择令牌的关键时间关系。

Result: 广泛实验表明，EchoingPixels仅使用5-20%的原始令牌就能达到强基线模型的性能，同时实现2-3倍的加速和内存减少。

Conclusion: EchoingPixels通过跨模态语义筛和同步增强RoPE的创新设计，有效解决了音频-视觉大语言模型的令牌压缩问题，实现了高效的多模态令牌减少，为音频-视觉LLM的实际应用提供了可行的解决方案。
...
Method: 提出StainNet模型，基于视觉Transformer架构，采用自蒸馏自监督学习方法，在HISTAI数据库的20,231张公开特殊染色全切片图像中裁剪的140万张图像块上进行训练。

Result: 在内部肝脏恶性肿瘤分类任务和两个公开ROI级数据集上验证了模型能力，通过少样本学习、检索评估以及与更大病理基础模型的比较，展示了StainNet的优势。
...
<details>
  <summary>Details</summary>
Motivation: 丝状结构（如微管和肌动蛋白丝）在生物系统中扮演重要角色，但其分割面临巨大挑战：密集分布和几何特性使得手动标注极其耗时费力，导致高质量像素级标注数据集难以获取。

Method: 基于Pix2Pix架构的条件生成框架，从二值掩码生成逼真的显微镜图像中的丝状结构；提出丝状结构感知的结构损失函数，以改善生成合成图像时的结构相似性。

Result: 实验证明该方法有效，且优于未使用合成数据训练的现有模型。

Conclusion: 提出的条件生成框架能够有效生成逼真的丝状结构图像，为解决丝状结构分割任务中的数据稀缺问题提供了可行方案。
...
Method: 提出HD-CLIP，将低质量图像分解为语义嵌入和有序退化嵌入，通过分类器自由投影引导集成到扩散模型中，作为即插即用模块提升超分辨率性能。

Result: HD-CLIP能够无缝集成到各种超分辨率框架中无需训练，显著提升细节保真度和感知真实性，在多样真实世界数据集上表现优异。
...
TL;DR: 提出了CoSPlan基准来评估视觉语言模型在视觉顺序规划任务中的表现，特别是在存在错误步骤的情况下。开发了SGI方法提升模型性能，在多个领域验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型在复杂推理方面表现出色，但在视觉顺序规划（即执行多步动作达成目标）方面研究不足。实际规划任务常包含非最优（错误）步骤，这对模型检测和纠正错误的能力提出了挑战。

Method: 提出了CoSPlan基准，在迷宫导航、积木重排、图像重建和物体重组4个领域评估视觉语言模型。开发了Scene Graph Incremental updates (SGI)方法，通过在初始状态和目标状态之间引入中间推理步骤，帮助模型进行序列推理。

Result: 即使使用最先进的推理技术（如思维链和场景图），现有视觉语言模型在CoSPlan上表现不佳，难以利用上下文线索达成目标。SGI方法带来了平均5.2%的性能提升，不仅增强了纠正顺序规划的可靠性，还能推广到传统规划任务如Plan-Bench和VQA。

Conclusion: 视觉语言模型在视觉顺序规划任务中仍有很大改进空间，特别是在处理错误步骤方面。SGI方法通过引入中间推理步骤有效提升了模型性能，为视觉顺序规划任务提供了新的解决方案。
...
TL;DR: 本文提出了OmniZoo大规模动物运动数据集和统一的自回归运动生成框架，能够为任意骨骼拓扑生成文本驱动的运动。
...
Method: 1) 引入OmniZoo数据集，涵盖140个物种和32,979个序列，包含多模态标注；2) 提出广义自回归运动生成框架，包含拓扑感知骨骼嵌入模块，将任意骨骼的几何和结构属性编码到共享标记空间，与文本语义无缝融合。

Result: 方法能够为给定文本提示和目标骨骼生成时间一致、物理合理且语义对齐的运动，并支持跨物种运动风格迁移。

Conclusion: OmniZoo数据集和提出的统一框架解决了当前运动生成方法的核心限制，实现了对任意骨骼拓扑的文本驱动运动生成，具有广泛的适用性。
...
TL;DR: TranSamba是一个用于弱监督体积医学分割的混合Transformer-Mamba架构，通过跨平面Mamba块实现3D上下文建模，在保持线性时间复杂度的同时提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督医学分割方法多依赖2D编码器，忽略了数据的体积特性，需要一种能有效捕捉3D上下文同时保持计算效率的方法。

Method: 提出TranSamba混合架构，在标准Vision Transformer骨干上添加跨平面Mamba块，利用状态空间模型的线性复杂度实现相邻切片间的高效信息交换，增强Transformer块内的自注意力机制。

Result: 在三个数据集上的实验表明，TranSamba建立了新的最先进性能，在不同模态和病理上一致优于现有方法，同时实现线性时间复杂度和恒定的批处理内存使用。

Conclusion: TranSamba通过结合Transformer和Mamba的优势，为弱监督体积医学分割提供了有效的3D上下文建模方案，在性能和效率之间取得了良好平衡。
...
TL;DR: mmCounter：一种利用毫米波雷达通过提取呼吸和微动作等超低频信号来准确计数静态密集人群的新方法


<details>
  <summary>Details</summary>
Motivation: 现有毫米波雷达在检测静态密集人群时存在局限性，主要依赖运动检测且空间分辨率不足，无法准确计数静止人员

Method: 采用多阶段信号处理流程，提取超低频（<1 Hz）信号（主要来自呼吸和微尺度身体运动），利用新颖的信号处理技术从背景噪声和静态物体中区分这些细微信号，并将信号源映射到个体人员

Result: 在熟悉环境中平均F1分数达87%，平均绝对误差0.6；在未测试环境中平均F1分数60%，平均绝对误差1.1；可在3平方米空间内计数最多7人（无并排间距，仅1米前后距离）

Conclusion: mmCounter通过提取超低频生理信号成功解决了毫米波雷达计数静态密集人群的难题，相比现有呼吸率估计方法（假设人数已知）有显著创新
...
TL;DR: 该论文提出了一个视频工具包和时空推理框架（STAR），通过增强多模态大语言模型（MLLMs）的时空推理能力来改进视频问答任务。
...
Result: STAR框架使用轻量级工具增强了GPT-4o，在VideoMME上实现了8.2%的性能提升，在LongVideoBench上实现了4.6%的提升。
...
TL;DR: Visual Funnel是一个无需训练的两步方法，通过上下文锚定和熵缩放组合来解决MLLMs中的"上下文盲视"问题，显著提升细粒度视觉感知能力。
...
Result: Visual Funnel显著优于简单的单裁剪和非结构化多裁剪基线。实验验证了单纯增加非结构化裁剪只能提供有限甚至有害的收益，而层次化组合结构是解决上下文盲视的关键。

Conclusion: 上下文盲视源于输入缺乏"结构多样性"而非信息"数量"不足。Visual Funnel通过层次化上下文组合有效解决了这一问题，为MLLMs在精度要求高的任务中的应用提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的长视频时刻检索方法面临两个主要问题：监督学习方法扩展性差且泛化能力有限，而零样本方法在搜索阶段面临候选爆炸问题，在精炼阶段需要高成本的视觉语言模型进行验证。需要一种既高效又无需训练的方法来解决这些挑战。

Method: 提出了P2S（Point-to-Span）框架，包含两个关键创新：1）自适应跨度生成器，通过动态调整候选片段长度来防止搜索阶段的候选爆炸；2）查询分解技术，将复杂查询分解为简单子查询，无需依赖高成本的视觉语言模型验证就能精炼候选结果。

Result: P2S是首个能够在小时级视频中进行时序定位的零样本框架，在MAD数据集上比监督学习方法高出3.7%（R5@0.1指标），显著超越了现有的监督学习状态方法。

Conclusion: P2S通过创新的自适应跨度生成和查询分解技术，成功解决了长视频时刻检索中的搜索效率低下和精炼成本高昂的问题，为无需训练的长视频理解提供了有效的解决方案。
...
TL;DR: 本文提出了首个用于4D毫米波雷达与激光雷达联合场景流估计的方法RaLiFlow，构建了首个雷达-激光雷达场景流数据集，通过动态感知双向跨模态融合模块和专门设计的损失函数实现了有效的多模态融合。


<details>
  <summary>Details</summary>
Motivation: 现有多模态融合方法主要关注图像与激光雷达点云的结合，而4D毫米波雷达与激光雷达的融合尚未探索。雷达相比激光雷达更便宜、在各种天气条件下更鲁棒，并能检测点级速度，是激光雷达的有价值补充。但目前缺乏专门用于场景流估计的雷达-激光雷达数据集，且雷达数据存在噪声、低分辨率和稀疏性等挑战。

Method: 1. 基于公开的真实世界自动驾驶数据集构建了首个雷达-激光雷达场景流数据集；2. 提出有效的雷达去噪和场景流标签生成预处理策略，为雷达点提供更可靠的流真值；3. 提出RaLiFlow框架，包含动态感知双向跨模态融合模块，将雷达的动态线索集成到局部交叉注意力机制中，实现跨模态上下文信息传播；4. 设计专门的损失函数，减轻不可靠雷达数据在训练中的负面影响，增强两种模态场景流预测的实例级一致性。

Result: 在重新构建的场景流数据集上进行的大量实验表明，该方法显著优于现有的基于激光雷达和基于雷达的单模态方法。

Conclusion: 本文首次探索了4D毫米波雷达与激光雷达的融合用于场景流估计，通过构建首个数据集和提出有效的融合框架，证明了雷达作为激光雷达补充的价值，为自动驾驶感知系统提供了更鲁棒、成本效益更高的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注推理任务，忽视了细粒度识别这一对实际应用场景至关重要的能力，因此需要专门的基准来评估大型视觉语言模型在这方面的性能。

Method: 提出FROW基准测试，包含马赛克数据（组合多个简短回答）和开放世界数据（基于真实世界问答生成），并从数据构建和训练过程两个角度提出优化策略。

Result: 马赛克数据提升类别识别准确率1%，开放世界数据提升FROW基准准确率10%-20%、内容准确率6%-12%，在预训练阶段加入细粒度数据可提升类别识别准确率最高达10%。

Conclusion: FROW基准填补了现有评估体系的空白，提出的优化策略能有效提升大型视觉语言模型在细粒度识别任务上的性能，为实际应用提供更好的支持。
...
<details>
  <summary>Details</summary>
Motivation: 激光雷达点云数据常受各种干扰产生大量噪声点，影响后续目标检测和识别精度。现有去噪方法难以同时实现高去噪精度、强边缘保持和实时性能，要么牺牲计算效率追求精度，要么牺牲边界细节提高速度。

Method: 1. 使用八叉树对全局点云进行空间分割实现并行加速；2. 在每个叶节点内，采用自适应体素占用统计和k近邻密度估计快速去除明显孤立和低密度噪声点；3. 构建结合密度权重和自适应距离权重的引力评分函数精细区分噪声点和目标点。

Result: 在斯坦福3D扫描库、加拿大恶劣驾驶条件数据集和实验室自采FMCW激光雷达点云上的实验表明，相比现有方法，该方法在各种噪声条件下在F1、PSNR和Chamfer距离指标上均取得一致改进，同时减少了单帧处理时间。

Conclusion: 该方法在多种噪声场景下验证了其高精度、鲁棒性和实时性能，能够同时实现高去噪精度、强边缘保持和实时处理能力，解决了现有方法的局限性。
...
TL;DR: MultiHateLoc：首个弱监督多模态仇恨言论定位框架，使用视频级标签实现细粒度帧级预测，在HateMM和MultiHateClip数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: TikTok和YouTube等平台视频内容快速增长加剧了多模态仇恨言论传播，现有研究主要关注视频级分类，忽略了实际应用中至关重要的时序定位任务。在弱监督（仅有视频级标签）下，静态融合或基于分类的架构难以捕捉跨模态和时序动态。

Method: 提出MultiHateLoc框架：1）模态感知时序编码器建模异质序列模式，包括定制化文本预处理模块增强特征；2）动态跨模态融合自适应强调每个时刻最信息丰富的模态，以及跨模态对比对齐策略增强多模态特征一致性；3）模态感知MIL目标在视频级监督下识别判别性片段。

Result: 在HateMM和MultiHateClip数据集上的实验表明，该方法在定位任务中实现了最先进的性能。尽管仅依赖粗粒度标签，MultiHateLoc能产生细粒度、可解释的帧级预测。

Conclusion: MultiHateLoc是首个针对弱监督多模态仇恨言论定位的框架，有效解决了现有方法在捕捉跨模态和时序动态方面的不足，为视频平台内容审核提供了实用解决方案。
...
TL;DR: TransLocNet：一种通过跨模态注意力融合LiDAR几何与航拍语义的空中-地面定位框架，在CARLA和KITTI数据集上实现亚米级、亚度级精度


<details>
  <summary>Details</summary>
Motivation: 地面LiDAR与航拍图像之间存在巨大的视点和模态差异，导致空中-地面定位困难，需要解决跨模态对齐问题

Method: 将LiDAR扫描投影为鸟瞰图表示，通过双向注意力与航拍特征对齐，使用似然图解码器输出位置和方向的空间概率分布，并采用对比学习模块增强跨模态嵌入空间对齐

Result: 在CARLA和KITTI数据集上超越现有最佳方法，定位误差降低高达63%，达到亚米级、亚度级精度，在合成和真实场景中均表现出鲁棒性和泛化能力

Conclusion: TransLocNet通过有效融合LiDAR几何与航拍语义上下文，实现了鲁棒且可泛化的空中-地面定位，为解决跨模态定位问题提供了有效解决方案
...
TL;DR: 该论文提出NCTTA方法，通过特征-分类器对齐和混合目标来解决测试时适应中的性能下降问题，在ImageNet-C上比Tent方法提升14.52%


<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法缺乏对域偏移下性能下降根本原因的理论洞察，需要基于神经崩溃现象开发更有效的适应方法

Method: 提出NCTTA方法：1) 扩展神经崩溃到样本级别，发现样本对齐崩溃现象；2) 使用混合目标（几何邻近性和预测置信度）来减轻不可靠伪标签的影响；3) 通过特征-分类器对齐来重新对齐特征嵌入

Result: NCTTA在ImageNet-C上比Tent方法提升14.52%，在各种域偏移场景中表现出色，有效增强了模型对分布变化的鲁棒性

Conclusion: 样本对齐崩溃是域偏移下性能下降的根本原因，NCTTA通过特征-分类器对齐和混合目标策略有效解决了这一问题，为测试时适应提供了新的理论框架和实践方法
...
<details>
  <summary>Details</summary>
Motivation: 现有学习型视频压缩框架存在时间对齐不准确与误差传播之间的两难困境：分离变换框架在帧间压缩中表现出色但会导致明显误差传播，而统一变换框架虽消除误差传播但在共享潜在域中的运动估计/补偿表现较差。

Method: 1. 双域渐进时间对齐：结合粗粒度像素域对齐（基于单参考帧光流）和细粒度潜在域对齐（基于多参考帧的流引导可变形Transformer实现长期运动细化）；2. 质量条件专家混合模块：根据目标质量和内容动态分配不同专家调整每像素量化步长，实现连续比特率适配。

Result: 实验结果表明，该方法在保持竞争性率失真性能的同时，成功消除了误差传播问题，实现了质量一致且无误差传播的视频流传输。

Conclusion: 提出的统一变换框架通过双域渐进时间对齐和质量条件专家混合模块，有效解决了学习型视频压缩中的关键挑战，在消除误差传播的同时保持了优异的压缩性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏用于研究面部血液脉动的3D数据集，这限制了远程光电容积描记成像方法的改进和验证，以及多视角方法在减轻光照影响方面的研究。

Method: 收集了15名受试者的23个视角的30Hz RGB视频，结合脉搏参考测量，使用单目运动结构技术生成3D面部扫描，并处理成与FLAME 3D头部模型纹理空间兼容的3D脉动图。

Result: 创建了包含原始视频、脉搏参考测量、3D扫描和处理后3D脉动图的完整数据集，脉动图包含信噪比、局部脉搏幅度、相位信息等数据，并提供了对光照条件、图一致性和生理特征捕获能力的全面评估。

Conclusion: Pulse3DFace填补了3D血液脉动数据集的空白，为开发动态面部血液脉动模型、验证远程脉搏估计方法以及研究多视角光照补偿方法提供了重要资源。
...
TL;DR: TaP是一种简单有效的小样本语义分割方法，通过低秩适应微调编码器来提升对新类别的适应能力，在多个基准测试中显著提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 小样本语义分割中，编码器对新类别的特征提取能力有限是主要瓶颈，现有研究主要关注解码器改进，而编码器的适应性问题未得到充分解决。

Method: 提出TaP方法，利用低秩适应在支持集上微调编码器，计算开销小，能快速适应新类别同时减轻灾难性遗忘，方法模型无关且可无缝集成到现有FSS流程中。

Result: 在COCO 20^i、Pascal 5^i以及跨域数据集DeepGlobe、ISIC、Chest X-ray等多个基准测试中，TaP在不同模型和shot设置下均能持续提升分割性能，尤其在复杂多类别场景中表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉感知方面仍有限制，而人类通过动态扫描和聚焦显著区域的"眨眼式"过程能高效感知复杂场景。研究发现MLLM在不同层会自然关注不同视觉区域，且选择性分配更多计算给显著token可以增强视觉感知

Method: 提出Blink动态视觉token分辨率框架，包含两个模块：显著性引导扫描和动态token分辨率。基于注意力图估计每层视觉token的显著性，通过即插即用的token超分辨率模块扩展重要token，在下一层当这些token失去焦点时丢弃扩展token

Result: 大量实验验证了Blink的有效性，证明其在增强视觉感知和多模态理解方面的效果

Conclusion: Blink框架通过模拟人类视觉处理策略，在MLLM中实现了动态视觉token分辨率，平衡了广泛探索和细粒度聚焦，从而自适应且高效地提升了视觉感知能力
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型使用自回归Transformer架构，需要对输入图像进行token化，这限制了模型在2D图像空间中准确定位对象的能力。需要解决如何改进序列语言token以更好地在2D空间中对对象进行定位的问题。

Method: 提出GETok方法，将专门的可学习token词汇表集成到MLLMs中。首先使用网格token将图像平面划分为结构化空间锚点，然后利用偏移token实现精确和迭代的定位预测细化。通过直接将空间关系嵌入到token中，在不修改自回归架构的情况下提升2D空间推理能力。

Result: 大量实验表明，GETok在监督微调和强化学习设置下的各种指代任务中，都优于最先进的方法，取得了卓越的性能表现。

Conclusion: GETok通过将空间表示直接嵌入到token中，显著提升了多模态大语言模型在原生2D空间推理中的能力，为对象定位任务提供了有效的解决方案。
...
TL;DR: 本文提出了一种基于对称U-Net架构的All-in-one图像修复方法，通过精心设计的特征提取和跨尺度传播，有效编码退化信息，无需复杂架构或退化提示策略，实现了更简单、更强、计算成本更低的效果。


<details>
  <summary>Details</summary>
Motivation: 现有All-in-one图像修复方法过度依赖复杂架构（如专家混合、扩散模型）和精细的退化提示策略，导致计算成本高且实现复杂。作者发现精心设计的特征提取本身就能编码退化信息，对称U-Net架构足以有效利用这些线索。

Method: 提出对称U-Net架构（SymUNet），通过对齐编码器-解码器特征尺度并实现简化的跨尺度传播，保留内在退化信号。进一步提出语义增强变体SE-SymUNet，通过简单交叉注意力从冻结的CLIP特征中直接注入语义信息，显式增强退化先验。

Result: 在多个基准数据集上验证了方法的优越性。SymUNet在减少计算成本的同时，在基准数据集上取得了比现有方法更好的结果。SE-SymUNet进一步提升了性能，为All-in-one图像修复建立了更简单、更强的基线。

Conclusion: 精心设计的特征提取和对称U-Net架构足以有效处理多种退化类型，无需复杂架构。SymUNet和SE-SymUNet为All-in-one图像修复的未来发展提供了更简单、更强的基线框架。
...
Result: 在WXSOD数据集上进行了广泛实验，使用不同训练数据规模（100%、50%、30%）、三种编码器和七种解码器配置，结果表明提出的框架（特别是NIFM增强的特定编码器）相比普通编码器在复杂天气条件下提高了分割精度

Conclusion: 提出的针对多天气条件的SOD框架有效提升了在复杂天气噪声下的显著目标检测性能，特定编码器保持与主流SOD解码器的兼容性
...
TL;DR: 该研究提出了一种无需训练的遥感图像语义检索方法TRSLLaVA，通过将跨模态检索重新定义为文本到文本匹配问题，利用VLM生成的丰富文本描述进行检索，在多个基准测试中取得了与有监督方法竞争的性能。
...
Method: 1) 引入RSRT数据集作为新基准，每张图像包含多个结构化描述；2) 提出完全无需训练的TRSLLaVA方法，将跨模态检索重新定义为文本到文本匹配问题；3) 利用丰富文本描述作为查询，在统一的文本嵌入空间中对VLM生成的描述进行检索。

Result: 在RSITMD和RSICD基准测试中，该方法表现出色：在RSITMD上平均召回率达到42.62%，是标准零样本CLIP基线（23.86%）的近两倍，甚至超越了多个顶级有监督模型。

Conclusion: 通过结构化文本实现高质量语义表示，为遥感图像检索提供了一个强大且经济高效的范式，验证了无需训练的方法在跨模态检索中的有效性。
...
Method: TCAM采用运动中心框架，通过运动场注意力机制自主观察视频、识别多个运动活动，并将自然语言描述空间定位到对应的运动轨迹。核心洞察是将运动模式与对比视觉-语言表示对齐，通过全局视频-文本对齐和细粒度空间对应的统一训练，实现多运动表达的无查询发现。
...
TL;DR: Lang2Motion是一个通过将运动流形与联合嵌入空间对齐来实现语言引导点轨迹生成的框架，能够为任意物体生成显式轨迹，在文本到轨迹检索和运动准确性方面显著优于基于视频的方法。


<details>
  <summary>Details</summary>
Motivation: 先前的研究主要集中在人体运动或视频合成上，而本文旨在为任意物体生成显式的点轨迹，利用从真实世界视频中提取的运动信息，通过语言引导实现更精确的运动控制。
...
Conclusion: Lang2Motion通过CLIP对齐的轨迹表示实现了有效的跨运动领域迁移，支持风格转移、语义插值和潜在空间编辑，为语言引导的运动生成提供了新的解决方案。
...
TL;DR: DOCR-Inspector：一个基于视觉语言模型的文档解析质量评估系统，通过细粒度错误检测和28种错误类型分类，提供全面的文档解析质量分析。


<details>
  <summary>Details</summary>
Motivation: 当前文档解析任务中，标准基准测试存在数据集特定偏差，导致模型排名不一致且与真实世界性能相关性有限。现有评估指标通常只提供整体分数，掩盖了输出中的具体错误模式，难以可靠全面地评估文档解析质量。

Method: 提出DOCR-Inspector框架，将文档解析评估形式化为细粒度错误检测和分析。利用VLM-as-a-Judge方法，分析文档图像及其解析输出，识别所有错误并归类到28种预定义类型。构建DOCRcase-200K训练数据集，并提出Chain-of-Checklist推理范式支持层次化的解析质量评估。

Result: 在DOCRcaseBench基准测试（882个真实世界文档解析案例）上，DOCR-Inspector-7B模型超越了Gemini 2.5 Pro等商业模型以及领先的开源模型。实验表明其质量评估能为解析结果优化提供有价值的指导。

Conclusion: DOCR-Inspector不仅是一个实用的评估工具，还能推动文档解析系统的大规模进步，解决了真实场景中文档解析质量可靠评估的关键挑战。
...
Method: K-Track采用混合策略：稀疏深度学习关键帧更新结合轻量级卡尔曼滤波进行中间帧预测，使用贝叶斯不确定性传播保持时间一致性。该框架与具体跟踪器无关，可应用于多种先进点跟踪器。

Result: 在NVIDIA Jetson Nano和RTX Titan等边缘平台上实现5-10倍加速，同时保持超过85%的原始跟踪器精度，实现了实时性能。
...
TL;DR: TriDF是一个用于可解释DeepFake检测的综合基准，包含图像、视频和音频三种模态的16种深度伪造类型，评估感知、检测和幻觉三个关键方面，揭示了这些方面之间的相互依赖关系。


<details>
  <summary>Details</summary>
Motivation: 生成建模技术的进步使得伪造个人真实肖像变得容易，这对安全、通信和公众信任构成严重风险。检测这类人员驱动的操纵需要系统不仅能区分伪造内容与真实媒体，还要提供清晰可靠的推理依据。

Method: TriDF基准包含来自先进合成模型的高质量伪造内容，涵盖16种DeepFake类型。它评估三个关键方面：感知（使用人工标注证据识别细粒度操纵伪影）、检测（评估不同伪造家族和生成器的分类性能）和幻觉（量化模型生成解释的可靠性）。

Result: 在多模态大语言模型上的实验表明，准确的感知对于可靠检测至关重要，但幻觉会严重干扰决策过程，揭示了这三个方面之间的相互依赖关系。
...
TL;DR: NaviHydra是一个可控的导航引导端到端模型，通过从现有规则模拟器中蒸馏得到，能够根据高级导航指令生成符合意图的轨迹，在NAVSIM基准测试中达到最先进水平。
...
Method: 提出NaviHydra框架，接受高级导航指令作为控制信号，生成符合指定意图的轨迹。采用基于鸟瞰图的轨迹收集方法增强轨迹特征提取，并引入新的导航合规性指标评估对预期路线的遵守程度。

Result: 在NAVSIM基准测试中显著优于基线模型，达到最先进的结果，证明了其在推进自动驾驶方面的有效性。

Conclusion: NaviHydra通过结合规则模拟器的蒸馏和端到端学习，实现了对导航指令的更好控制，提高了自动驾驶系统的可控性和导航安全性。
...
TL;DR: SpaceDrive是一个空间感知的视觉语言模型驾驶框架，通过将空间信息作为显式位置编码而非文本数字标记来处理，提升了3D空间关系理解能力，在自动驾驶任务中取得了优异表现。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型（VLMs）的端到端自动驾驶方法虽然具有强大的视觉理解和推理能力，但在理解细粒度的3D空间关系方面存在不足，而这是与物理世界交互系统的基本要求。

Method: 提出SpaceDrive框架，将空间信息作为显式位置编码处理，而非文本数字标记。使用通用位置编码器处理来自多视角深度估计、历史自我状态和文本提示的3D坐标，将这些3D位置编码叠加到对应的2D视觉标记上，并作为任务无关的坐标表示，替代数字标记作为VLM的输入和输出。

Result: 在nuScenes数据集上实现了最先进的开放环性能，在Bench2Drive闭环基准测试中获得了78.02的驾驶分数，在现有VLM方法中排名第二。

Conclusion: SpaceDrive通过将空间信息作为显式位置编码处理，显著提升了VLM在自动驾驶任务中的空间推理能力，能够更好地索引视觉语义并进行轨迹坐标回归，从而提高了规划准确性。
...
TL;DR: 本文提出IRMoGen（交错推理运动生成）新范式，通过文本-运动对话将运动生成与评估、精炼紧密耦合，显著提升文本-运动对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有运动感知大语言模型通常将运动理解和生成任务分开处理，限制了任务间交互反馈带来的相互增益。研究发现运动评估和精炼任务可作为理解与生成之间的关键桥梁，促进双向知识流动。

Method: 提出IRG-MotionLLM模型，首次实现运动生成、评估和精炼的无缝交错。采用新颖的三阶段训练方案逐步开发模型能力，并构建自动化数据引擎从现有文本-运动数据集中合成交错推理标注。

Result: 实验表明：(1)评估和精炼任务显著改善文本-运动对齐；(2)交错生成、评估和精炼步骤在各训练阶段均带来一致性能提升；(3)IRG-MotionLLM明显优于基线模型，在标准文本到运动生成基准上达到先进性能。

Conclusion: 通过将运动生成与评估、精炼任务紧密耦合的交错推理范式，能够有效促进理解与生成之间的双向知识流动，显著提升运动生成质量和对齐效果。
...
TL;DR: 开发了一个端到端管道，从冠脉CTA自动提取几何结构并生成模拟数据，同时提出扩散回归模型直接预测冠脉血压，无需CFD计算，在模拟数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统CFD模拟冠脉血流虽然能提供有价值的血流动力学标志物，但计算成本高、耗时且难以集成到大规模临床工作流中，限制了AI模型的训练数据和基于生理学的CAD评估的广泛应用。

Method: 开发了端到端管道，自动化从冠脉CTA提取几何结构，简化模拟数据生成；引入基于扩散的回归模型，直接从CTA特征预测冠脉血压，绕过CFD计算。

Result: 在模拟冠脉血流动力学数据集上评估，模型达到R2为64.42%，均方根误差0.0974，归一化RMSE为0.154，优于多个基线方法，实现了最先进的性能。

Conclusion: 该工作为快速、无创的血压预测提供了一个可扩展且易于访问的框架，支持CAD诊断，减少了传统CFD工作流的手动负担，同时产生一致的训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 研究REPA方法中目标表示的关键因素：是全局语义信息还是空间结构对生成性能更重要。传统观点认为更强的全局语义性能会带来更好的生成效果，但本文对此提出质疑。

Method: 首先对27种不同视觉编码器进行大规模实证分析，然后提出iREPA方法：1）用卷积层替换标准MLP投影层；2）引入空间归一化层来增强空间信息传递。

Result: 实证分析发现空间结构而非全局语义性能驱动生成性能。iREPA方法（仅需<4行代码）能一致提升REPA的收敛速度，适用于多种视觉编码器、模型规模和训练变体。

Conclusion: 空间结构是目标表示中影响生成性能的关键因素，iREPA的简单改进有效提升了表示对齐方法的训练效率，为生成模型训练机制提供了新见解。
...
TL;DR: 提出GLAT+IRM模型，通过迭代优化补丁选择和图拉普拉斯注意力机制提升前列腺癌全玻片图像分级性能


<details>
  <summary>Details</summary>
Motivation: 前列腺癌全玻片图像分级面临三大挑战：图像尺度大、组织结构异质性高、诊断相关区域选择困难。现有方法常采用随机或静态补丁选择，导致包含冗余或非信息区域，降低性能。

Method: 提出图拉普拉斯注意力Transformer（GLAT）结合迭代优化模块（IRM）。IRM利用预训练ResNet50提取局部特征，基础模型进行重要性评分，迭代优化补丁选择。GLAT构建补丁图，通过图拉普拉斯约束确保空间一致性，可学习滤波机制增强判别性组织学结构。凸聚合机制动态调整补丁重要性生成WSI级表示。
...
TL;DR: 该论文提出了一种名为SEPL的自集成后学习方法，用于解决领域泛化中遇到标签噪声时出现的伪特征放大问题，通过特征探测训练和预测集成推理来增强现有方法的鲁棒性。
...
Method: 提出自集成后学习方法（SEPL），包含两个部分：特征探测训练和预测集成推理。该方法利用模型架构中的中间特征表示，训练多个探测分类器以充分利用预训练模型的能力，同时通过集成这些不同分类头的输出获得最终预测。考虑到标签噪声的存在，采用半监督算法训练探测分类器。由于不同探测分类器关注图像的不同区域，使用众包推理方法集成它们的预测。

Result: 广泛的实验评估表明，所提出的方法不仅增强了现有方法的鲁棒性，而且在现实应用中展现出显著的潜力和高度的灵活性。

Conclusion: SEPL方法通过特征多样化和集成学习有效解决了领域泛化中标签噪声导致的伪特征放大问题，为现有方法在噪声环境下的应用提供了有效的增强方案。
...
TL;DR: SWiT-4D是一个基于滑动窗口Transformer的4D网格生成方法，能够从单目视频中生成高质量的动画3D资产，仅需少量4D监督数据即可实现高保真几何和稳定时间一致性。


<details>
  <summary>Details</summary>
Motivation: 当前4D内容生成面临两个主要挑战：1）将单目视频转换为高质量动画3D资产（具有显式4D网格）仍然困难；2）缺乏大规模自然捕获的4D网格数据集，限制了纯数据驱动的视频到4D模型的训练能力。同时，基于丰富数据集的图像到3D生成技术提供了强大的先验模型。

Method: SWiT-4D采用滑动窗口Transformer架构，实现无损、参数自由的时序4D网格生成。该方法无缝集成任何基于Diffusion Transformer的图像到3D生成器，在视频帧间添加时空建模，同时保留原始单图像前向过程，支持任意长度视频的4D网格重建。针对静态相机单目视频，还引入了基于优化的轨迹模块来恢复全局平移。

Result: SWiT-4D表现出强大的数据效率：仅需单个短视频（<10秒）进行微调，就能实现高保真几何和稳定时间一致性。在域内zoo-test集和具有挑战性的域外基准测试（C4D、Objaverse和野外视频）上的综合实验表明，SWiT-4D在时间平滑性方面始终优于现有基线方法。

Conclusion: SWiT-4D通过有效利用图像到3D生成器的先验知识，在极少量4D监督下实现了高质量的视频到4D网格转换，具有实际部署可行性，为4D内容生成提供了新的解决方案。
...
TL;DR: 论文提出微观空间智能(MiSI)概念，并创建包含16.3万问答对和58.7万图像的MiSI-Bench基准，评估视觉语言模型在微观空间推理能力，发现当前模型表现远低于人类水平，但微调后7B模型在空间转换任务中超越人类。
...
Method: 提出MiSI-Bench基准框架，基于约4000个分子结构生成超过16.3万个问答对和58.7万张图像，涵盖9个互补任务，从基础空间转换到复杂关系识别。

Result: 当前最先进的视觉语言模型在该基准上表现显著低于人类水平。但经过微调的7B模型展现出巨大潜力，在空间转换任务中甚至超越人类，而在氢键识别等科学基础任务上表现不佳，表明需要整合明确的领域知识。

Conclusion: 微观空间智能是科学发现的关键能力，当前模型在该领域仍有局限，需要整合领域知识才能向科学通用人工智能迈进。数据集已公开提供。
...
TL;DR: MoCapAnything是一个类别无关的运动捕捉框架，能够从单目视频和任意绑定3D资产中重建驱动特定资产的旋转动画，实现跨物种的运动重定向。


<details>
  <summary>Details</summary>
Motivation: 现有运动捕捉流程大多针对特定物种或模板，缺乏通用性。作者提出类别无关运动捕捉（CAMoCap）概念，旨在解决从单目视频和任意3D资产中重建驱动特定资产动画的通用性问题。

Method: 采用参考引导的分解框架：首先预测3D关节轨迹，然后通过约束感知逆运动学恢复资产特定旋转。系统包含三个可学习模块：参考提示编码器、视频特征提取器、统一运动解码器，以及轻量级IK阶段。

Result: 在领域内基准测试和野外视频实验中，MoCapAnything能够生成高质量的骨骼动画，并在异构绑定间实现有意义的跨物种重定向，为任意资产提供可扩展的提示驱动3D运动捕捉。

Conclusion: MoCapAnything框架实现了类别无关的运动捕捉，能够从单目视频和任意3D资产中重建驱动特定资产的动画，解决了现有流程的物种和模板限制问题，为内容创作提供了更通用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 表格提取是视觉文档理解的关键挑战，传统方法先检测表格再识别结构。当前缺乏标注数据阻碍了视觉语言模型等新方法的发展。

Method: 创建了大规模数据集PubTables-v2，支持多页表格结构识别等任务；基于此开发了Page-Object Table Transformer (POTATR)，将Table Transformer扩展为图像到图的页面级表格提取模型。

Result: PubTables-v2是首个大规模多页表格结构识别基准；使用该数据集评估了领域专用视觉语言模型；POTATR实现了全面的页面级表格提取。

Conclusion: PubTables-v2数据集解决了表格提取领域的数据缺乏问题，支持多种挑战性任务；POTATR模型展示了在页面级表格提取方面的进展；数据和模型将公开发布。
...
TL;DR: DuetSVG是一个统一的多模态模型，通过联合生成图像token和SVG token，并利用测试时缩放策略提升SVG生成质量，解决了现有VLM方法在复杂语义和视觉一致性方面的不足。
...
Result: 大量实验表明，该方法在广泛的应用场景中优于现有方法，能够生成视觉忠实、语义对齐且语法干净的SVG。

Conclusion: DuetSVG通过联合多模态生成和测试时视觉指导，有效提升了SVG生成的质量和一致性，为复杂语义场景下的矢量图形生成提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有说话头生成方法存在速度与质量矛盾：扩散方法真实但速度慢，高斯泼溅方法实时但不稳定。需要解决实时性与时间一致性的平衡问题

Method: 使用3D Morphable Models映射高斯泼溅生成个性化头像，引入基于transformer的音频到模型参数预测，从单目视频和独立音频输入生成实时说话头视频

Result: 方法实现了实时说话头视频生成，在定量和定性评估中表现出竞争力

Conclusion: 通过结合3D Morphable Models和高斯泼溅，配合transformer音频参数预测，成功解决了实时说话头生成中的时间一致性问题
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型采用自回归token生成方式，存在计算效率低和表面语言变异性干扰的问题。VL-JEPA旨在通过预测连续嵌入来关注任务相关语义，同时抽象掉表面语言变异性。

Method: 基于联合嵌入预测架构，在抽象表示空间中学习，预测目标文本的连续嵌入而非生成token。推理时仅在需要时调用轻量级文本解码器将预测嵌入转换为文本。

Result: 在相同视觉编码器和训练数据下，VL-JEPA比标准token空间VLM训练性能更强且可训练参数减少50%。选择性解码减少2.85倍解码操作。在8个视频分类和8个视频检索数据集上平均性能超越CLIP、SigLIP2和Perception Encoder。在4个VQA数据集上性能与经典VLM相当，仅使用16亿参数。

Conclusion: VL-JEPA通过联合嵌入预测架构在视觉语言任务中实现了高效且强大的性能，支持多种应用场景而无需架构修改，为视觉语言建模提供了新的有效范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有指代视频分割数据集通常关注静态属性丰富的语言表达，可能仅凭单帧就能识别目标物体，忽视了视频和语言中运动信息的重要性。为了探索利用运动表达和运动推理线索进行像素级视频理解的可行性，需要专门的数据集。

Method: 构建了MeViS数据集，包含33,072个人工标注的文本和音频运动表达，覆盖2,006个复杂场景视频中的8,171个物体。在MeViS支持的4个任务上评估了15种现有方法，并提出了LMPM++方法用于RVOS/AVOS/RMOT任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶系统在处理多摄像头数据时存在计算瓶颈，通常依赖显式的3D归纳偏置（如BEV、占用网格等），这些方法计算量大且可能限制模型的表达能力。需要一种更高效、数据驱动的场景编码方法。

Method: Flex采用几何无关的方法，使用少量可学习的场景token联合编码所有摄像头和时间步的图像token。这种方法不依赖显式的3D先验知识，而是直接从数据中学习紧凑的场景表示，为下游基于大语言模型的策略模型提供高度压缩的视觉输入。

Result: 在2万小时驾驶数据的大规模专有数据集上评估，Flex相比最先进方法实现了2.2倍的推理吞吐量提升，同时驾驶性能大幅提高。此外，这些紧凑的场景token在没有显式监督的情况下展现出场景分解的新兴能力。

Conclusion: Flex挑战了3D先验知识必要的普遍假设，证明数据驱动的联合编码策略为未来自动驾驶系统提供了更可扩展、高效和有效的路径。几何无关的方法在保持高性能的同时显著提升了计算效率。
...
Method: 提出ClusIR框架，包含两个关键组件：1) 概率聚类引导路由机制(PCGRM)，将退化识别与专家激活解耦，实现判别性退化感知和稳定专家路由；2) 退化感知频率调制模块(DAFMM)，利用聚类引导先验进行自适应频率分解和针对性调制。

Result: 在多个基准测试上的广泛实验验证了ClusIR在各种退化场景下都能达到有竞争力的性能，聚类引导的协同作用有效桥接了语义线索与频率域调制。

Conclusion: ClusIR通过显式建模退化语义和跨域传播聚类感知线索，实现了对多种退化类型的自适应图像修复，取得了显著的修复效果。
...
Conclusion: E-RayZer建立了3D感知视觉预训练的新范式，通过显式几何和细粒度课程学习，实现了真正3D感知表示的学习，为3D视觉基础模型的发展提供了新方向。
...
TL;DR: Group Diffusion提出了一种新的扩散模型推理方法，通过让多个样本在推理过程中协同生成，共享注意力机制，显著提升了生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型在推理时独立生成图像，本文探索了样本间协同生成的可能性，认为跨样本的注意力机制可以提升生成质量。

Method: 提出Group Diffusion方法，解锁注意力机制使其能够在图像间共享，而不仅限于单个图像内的补丁。通过让图像在推理时联合去噪，学习图像内和图像间的对应关系。

Result: 观察到明显的缩放效应——更大的组规模产生更强的跨样本注意力和更好的生成质量。在ImageNet-256x256上实现了高达32.2%的FID改进，且提出的定性测量方法与FID密切相关。
...
Method: 1）策划语义链接的图像对，标注正负属性，明确教导编码器保留或抑制什么；2）采用双目标训练范式，平衡生成保真度和对比解耦。

Result: 生成的嵌入在开放词汇属性检索、个性化和组合生成方面表现有效，在多个基准测试中达到最先进的性能。

Conclusion: Omni-Attribute通过联合设计数据和模型，成功解决了视觉概念个性化中的属性解耦问题，为开放词汇属性表示学习提供了有效解决方案。
...
TL;DR: StereoWalker提出了一种增强机器人导航基础模型的方法，通过引入立体视觉输入和显式中层视觉模块（深度估计、密集像素跟踪）来解决单目视觉的深度尺度模糊问题，显著提升了导航性能和数据效率。
...
Method: StereoWalker通过两个关键创新增强NFMs：1）使用立体视觉输入解决深度尺度模糊问题；2）引入显式中层视觉模块，包括深度估计和密集像素跟踪，提供可靠的几何和运动结构信息。同时构建了一个大型立体导航数据集，包含从互联网立体视频中自动标注的动作信息。

Result: 实验表明，中层视觉使StereoWalker仅使用1.5%的训练数据就能达到当前最佳性能，使用完整数据时则超越现有最佳方法。立体视觉相比单目输入能获得更高的导航性能。

Conclusion: 研究表明，在机器人导航基础模型中忽略中层视觉先验是低效的。通过引入立体视觉和显式中层视觉模块，可以显著提升导航性能和数据效率，为未来机器人导航研究提供了新的方向。
...
TL;DR: WorldLens是一个评估生成世界模型的全谱基准，涵盖生成、重建、动作跟随、下游任务和人类偏好五个维度，旨在统一评估生成世界的几何一致性、物理合理性和功能可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前生成世界模型虽然能合成逼真的4D驾驶环境，但往往在物理或行为上存在缺陷，且缺乏统一的评估方法来衡量生成世界是否保持几何结构、遵守物理规律或支持可靠控制。

Method: 提出WorldLens基准，涵盖五个评估维度：生成、重建、动作跟随、下游任务和人类偏好；构建WorldLens-26K大规模人工标注视频数据集；开发WorldLens-Agent评估模型，从人工标注中蒸馏学习以实现可扩展、可解释的评分。

Result: 现有世界模型在五个维度上表现不均衡：纹理强的模型常违反物理规律，而几何稳定的模型缺乏行为保真度。WorldLens生态系统为标准评估提供了统一框架。
...
TL;DR: 本文分析基于token补全的大语言模型推理机制，认为其本质是随机模式匹配而非真正的溯因推理，只是通过训练数据模仿人类推理结构，具有表面溯因性但缺乏真实理解、验证和语义基础。


<details>
  <summary>Details</summary>
Motivation: 探讨当前基于token补全的大语言模型的推理本质，揭示其表面看似进行溯因推理但实际上只是基于训练数据的模式匹配，澄清对LLM推理能力的误解。

Method: 通过分析LLM的随机性和token补全机制，对比人类溯因推理，使用具体示例展示LLM如何产生看似合理但缺乏真实理解、验证和语义基础的输出。

Result: LLM能够生成看似合理的想法、模仿常识推理、提供解释性答案，但这些输出缺乏真实性、语义基础、验证和理解，并非真正的溯因推理，只是训练数据中推理结构的模仿。

Conclusion: LLM具有随机基础但表面溯因的双重特性，可用于辅助人类思维和想法生成，但其输出需要批判性评估，因为它们无法识别真理或验证解释。文章最后回应了五个反对观点并指出了分析的局限性。
...
TL;DR: 研究探索使用小型语言模型（SLM）进行自动问题生成，作为大型语言模型在学习分析研究中的补充，提出了一种结合文本生成和概率推理能力的SLM问题生成管道。


<details>
  <summary>Details</summary>
Motivation: 当前学习分析研究主要依赖大型语言模型进行问题生成，但小型语言模型具有计算效率高、部署成本低等优势，研究旨在探索SLM作为LLM补充的潜力，开发能够生成高质量问题的SLM方法。

Method: 提出"生成-验证"策略的SLM问题生成管道：首先进行扩展性生成产生大量候选问题，然后通过基于概率推理的选择性验证进行精炼。该方法利用SLM的文本生成和概率推理能力。

Result: 通过两项评估研究（7名人类专家评估和大型语言模型评估）显示，大多数评估者（人类或LLM）认为生成的问题具有清晰答案，且与预期学习目标基本一致。SLM在精心设计的管道指导下能有效生成高质量问题。

Conclusion: 小型语言模型在适当设计的管道指导下能够有效生成高质量问题，可作为大型语言模型在学习分析研究中的补充方案，为资源受限环境提供了可行的替代方案。
...
TL;DR: DeepNews框架通过模拟专业财经记者的认知过程，解决了长文本生成中的"不可能三角"问题，显著降低了幻觉率并提升了内容质量。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在垂直领域长文本生成中存在"不可能三角"困境：难以同时实现低幻觉率、深度逻辑连贯性和个性化表达。研究发现这一瓶颈源于现有生成范式陷入了"统计平滑陷阱"，忽视了专家级写作所需的高熵信息获取和结构化认知过程。

Method: 提出DeepNews框架，模拟资深财经记者的隐式认知过程，包含三个核心模块：1)基于信息觅食理论的双粒度检索机制，强制执行10:1的饱和信息输入比；2)基于领域专家知识库（叙事模式）和原子块的模式引导战略规划；3)对抗性约束提示技术，包括节奏打破和逻辑迷雾等策略，打破模型生成文本的概率平滑性。

Result: 实验发现深度财经报道中存在明显的"知识悬崖"：当检索上下文低于15,000字符时，内容真实性崩溃；而超过30,000字符的高冗余输入能将无幻觉率稳定在85%以上。在与中国顶级科技媒体的生态效度盲测中，基于上一代模型构建的DeepNews系统获得了25%的投稿接受率，显著优于最先进模型的零样本生成（0%接受率）。

Conclusion: DeepNews框架通过显式建模专业写作的认知过程，有效解决了长文本生成中的"不可能三角"问题，为垂直领域的高质量内容生成提供了新的解决方案。
...
TL;DR: 提出两阶段提示框架，从短评论文本推断显式和隐式用户画像，用于个性化回复生成，无需模型微调
...
Method: 两阶段提示框架：1) 从短评论文本推断显式（用户明确偏好）和隐式（人口统计或风格线索）用户画像；2) 将推断的画像属性融入回复生成提示中；通过调整解码温度鼓励多样且忠实生成

Result: 在韩国外卖应用真实数据集上评估，显示该方法在精确度、多样性和语义一致性方面均有提升，增强了自动回复的相关性和个性化

Conclusion: 画像增强提示能有效提升自动回复的相关性和个性化，无需模型微调，在用户信息有限场景下具有实用价值
...
TL;DR: 研究探索将英语训练的视觉-语言模型适应到其他语言的方法，发现数据集翻译是主要瓶颈，建议未来应关注原生语言数据收集和改进翻译策略。
...
Method: 探索并比较了三种方法：基于翻译的流程、LoRA微调，以及将视觉适应与语言适应分离的两阶段微调策略。使用翻译后的多模态基准测试和母语专家手动评估来评估这些方法。

Result: 数据集翻译是多语言VLM性能的主要瓶颈，数据质量限制了训练和评估的有效性。

Conclusion: 未来工作应聚焦于原生语言数据集的收集和改进翻译策略，以提升多语言视觉-语言模型的性能。
...
TL;DR: 本文提出滑动窗口注意力适应（SWAA）方法，通过五种策略的组合，使基于全注意力预训练的大语言模型能够有效适应滑动窗口注意力，在保持长上下文性能的同时降低推理成本。


<details>
  <summary>Details</summary>
Motivation: Transformer大语言模型的自注意力机制具有二次方复杂度，长上下文推理成本高。滑动窗口注意力（SWA）能降低到线性复杂度，但直接将全注意力预训练的模型切换到SWA会导致严重的性能下降，需要解决训练-推理不匹配问题。

Method: 提出SWAA方法，包含五种适应策略的组合：1）仅在预填充阶段使用SWA；2）保留"sink"令牌；3）交错FA/SWA层；4）思维链（CoT）；5）微调。通过不同策略的组合配置来优化性能-效率权衡。

Result: 实验表明SWA适应是可行但非平凡的：单一方法不足，但特定的协同组合能有效恢复原始长上下文性能。分析了不同SWAA配置的性能-效率权衡，为不同场景提供了推荐方案。

Conclusion: 通过SWAA方法，全注意力预训练的LLM可以成功适应滑动窗口注意力，在保持长上下文性能的同时显著降低推理成本，为实际部署提供了实用的解决方案。
...
TL;DR: CoopRAG是一个新颖的检索增强生成框架，通过检索器和LLM的协作工作，以及检索器模型早期和后期层的协作，提高多跳和简单问答任务的准确性和减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型倾向于生成事实不准确的输出，现有的检索增强生成方法在简单和多跳问答任务中仍然容易产生错误检索和幻觉问题，需要更有效的解决方案。

Method: 提出CoopRAG框架，包含四个步骤：1)将问题分解为子问题和推理链并掩码不确定位置；2)基于增强的问题、子问题和推理链检索相关文档；3)通过对比检索器不同层来重新排序文档；4)通过LLM填充掩码位置重建推理链。

Result: CoopRAG在三个多跳问答数据集和一个简单问答数据集上，在检索和问答性能方面都一致优于最先进的问答方法。

Conclusion: CoopRAG通过检索器和LLM的协作机制，以及检索器模型内部层的协作，有效解决了现有RAG方法的局限性，显著提高了问答任务的准确性和可靠性。
...
TL;DR: T-pro 2.0是一个开源的俄语大语言模型，支持混合推理和高效推理，包含推理追踪生成、西里尔密集分词器和优化的推理管道


<details>
  <summary>Details</summary>
Motivation: 为俄语社区提供可复现、可扩展的研究资源，支持俄语推理研究和高效实用的LLM应用开发

Method: 使用西里尔密集分词器，采用适配的EAGLE推测解码管道降低延迟，支持直接回答和推理追踪生成

Result: 发布了模型权重、T-Wix 500k指令语料库、T-Math推理基准和EAGLE权重，提供公开Web演示展示推理加速效果

Conclusion: T-pro 2.0作为可访问的开放系统，可用于构建和评估高效实用的俄语LLM应用
...
TL;DR: SRAP框架通过两阶段方法检测并恢复对抗性抄袭文本：第一阶段使用SciBERT进行统计异常检测，第二阶段使用FAISS和SBERT进行语义重建，能够将"扭曲短语"恢复为原始术语。


<details>
  <summary>Details</summary>
Motivation: 科学文献的完整性正受到对抗性文本生成技术的威胁，特别是使用自动改写工具掩盖抄袭行为。现有检测方法依赖静态黑名单或通用领域语言模型，对新型混淆检测效果差且无法确定抄袭来源。

Method: 提出SRAP框架：1) 使用SciBERT基于伪困惑度进行统计异常检测；2) 使用FAISS进行密集向量检索和SBERT进行句子级对齐，实现基于来源的语义重建。

Result: 在对抗性科学文本平行语料上的实验显示，零基线方法完全失败（0.00%恢复准确率），而SRAP达到23.67%恢复准确率，显著优于基线方法。静态决策边界在术语密集的科学文本中表现更稳健。

Conclusion: SRAP不仅能检测对抗性抄袭，还能通过数学方法恢复原始术语，实现法证分析，将混淆表达链接回最可能的源文档，为科学文献完整性提供新工具。
...
TL;DR: 该研究通过将知识图谱（KGs）与大型语言模型（LLMs）集成，使用KG-BERT增强模型的事实一致性和推理能力，在知识密集型任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（如Claude、Mistral IA和GPT-4）在自然语言处理方面表现出色，但缺乏结构化知识，导致事实不一致问题。需要增强LLM的事实可靠性和推理能力。

Method: 通过知识图谱（KGs）集成，使用KG-BERT方法将结构化知识融入大型语言模型，增强模型的grounding和推理能力。

Result: 实验表明，在知识密集型任务（如问答和实体链接）上取得了显著提升，改善了事实可靠性。

Conclusion: 这种方法提高了大型语言模型的事实可靠性，并使其具备更强的上下文感知能力，为下一代LLMs的发展提供了有效途径。
...
TL;DR: 大型语言模型（LLMs）仅通过表层形式训练后，能够区分语法正确和不正确的句子变体，表明它们对句法结构敏感，而不仅仅是线性顺序。


<details>
  <summary>Details</summary>
Motivation: 传统生成语法中，语法性系统对比（如主助倒装和寄生空位许可）被视为内部层级语法的证据。本研究旨在测试仅基于表层形式训练的大型语言模型是否能够复现这些对比，从而暗示其具有底层结构表征。

Method: 聚焦两个经典结构：主助倒装（测试主语边界识别）和寄生空位许可（测试抽象依赖结构）。使用GPT-4和LLaMA-3等模型，通过提示引出可接受性评分进行评估。

Result: LLMs能够可靠地区分两种结构中语法正确和不正确的变体，表明它们对结构敏感，而不仅仅是线性顺序。结构概括（区别于认知知识）从表层形式的预测训练中涌现，表明在没有显式编码的情况下具有对句法的功能敏感性。

Conclusion: 仅通过表层形式训练的大型语言模型能够表现出对句法结构的敏感性，支持它们具有底层结构表征的假设，表明句法知识可以从预测性语言建模中涌现。
...
TL;DR: 本文研究发现，仅依赖上下文学习（ICL）不足以进行可靠的因果推理，解码器模型对分布变化敏感，而经过微调的编码器和编码器-解码器模型在因果推理任务中表现更稳健，特别是在小规模模型上。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究不同架构的大语言模型在因果推理任务中的表现差异，特别是上下文学习（ICL）在需要多跳组合和严格合取控制的因果推理中的适用性，以及不同模型架构对分布变化的鲁棒性。

Method: 方法包括比较微调后的编码器、编码器-解码器和仅解码器架构模型，在零样本和少样本上下文学习设置下，在自然语言和非自然语言场景中进行因果推理测试。

Result: 研究发现：1）仅依赖ICL不足以进行可靠的因果推理，模型容易过度关注不相关的输入特征；2）仅解码器模型对分布变化特别脆弱；3）微调后的编码器和编码器-解码器模型在测试中表现更稳健，包括非自然语言场景；4）仅解码器架构需要达到很大规模才能匹配或超越前两种架构的性能。

Conclusion: 结论是对于成本效益高、短期稳健的因果推理任务，经过针对性微调的编码器或编码器-解码器架构是更优选择，特别是在模型规模较小的情况下。
...
TL;DR: 论文提出了RoleRMBench，这是首个针对角色扮演对话的奖励建模系统基准，并开发了RoleRM模型，通过连续隐式偏好方法显著提升了角色扮演对话的奖励建模性能。
...
Method: 1. 构建RoleRMBench基准，覆盖七个细粒度能力维度；2. 提出RoleRM模型，采用连续隐式偏好方法，将主观评估重构为多重结构化策略下的连续一致成对监督。

Result: RoleRMBench评估显示通用奖励模型与人类判断存在巨大差距，尤其在叙事和风格维度；RoleRM模型在平均性能上超越开源和闭源奖励模型超过24%，在叙事连贯性和风格保真度方面取得显著提升。

Conclusion: 连续偏好表示和标注一致性对于主观对齐至关重要，为以人为中心的对话系统的主观对齐奠定了基础，强调了在角色扮演等主观领域需要专门的奖励建模方法。
...
TL;DR: AgriGPT-Omni是一个农业全模态框架，整合语音、视觉和文本，通过数据合成、三阶段训练和综合基准测试，解决了农业AI中多语言语音数据缺乏、统一架构不足和评估标准缺失的问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在农业应用中面临三大挑战：缺乏多语言语音数据、缺乏统一的多模态架构、缺乏全面的评估基准。这些问题限制了农业AI的发展，特别是在资源匮乏地区。

Method: 1. 构建可扩展的数据合成与收集管道，将农业文本和图像转换为训练数据，创建了迄今最大的农业语音数据集（49.2万合成+1400真实样本，覆盖6种语言）。
2. 采用三阶段训练范式：文本知识注入、渐进式多模态对齐、基于GRPO的强化学习，训练首个农业全模态模型。
3. 提出AgriBench-Omni-2K，首个农业三模态基准测试，涵盖多样化语音-视觉-文本任务和多语言切片。

Result: 实验表明，AgriGPT-Omni在多语言多模态推理以及真实世界语音理解方面显著优于通用基线模型。该框架在农业特定任务上表现出色。

Conclusion: AgriGPT-Omni通过统一框架解决了农业AI的关键瓶颈，促进了可重复研究、包容性农业智能和资源匮乏地区的可持续AI发展。所有模型、数据、基准和代码将公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型主要基于英语等主导语言训练，对低资源语言的表征往往反映源语言材料的文化和语言偏见。研究以塞尔维亚语为案例，旨在揭示影响低资源语言技术发展的结构、历史和社会技术因素。

Method: 采用半结构化访谈方法，采访了十位学者和实践者，包括语言学家、数字人文研究者和AI开发者，分析塞尔维亚语面临的挑战及其根源。

Result: 研究发现塞尔维亚语面临历史文本遗产破坏、当代问题导致的功能优先工程方法等挑战，具体表现为浅层音译、依赖英语训练模型、数据偏见和缺乏文化特异性的数据集管理。

Conclusion: 研究提出Data Care框架，基于CARE原则（集体利益、控制权、责任和伦理），将偏见缓解从事后技术修复转变为语料库设计、标注和治理的组成部分，为构建包容、可持续和基于文化的语言技术提供可复制模型。
...
TL;DR: 本文提出一个全面的数据偏见检测与缓解管道，包含四个组件，用于处理表示偏见和显性刻板印象两种数据偏见类型，并通过性别、宗教和年龄案例进行双重评估。


<details>
  <summary>Details</summary>
Motivation: 训练大语言模型的文本数据存在多方面的偏见表现，包括有害语言和倾斜的人口统计分布。虽然欧盟AI法案等法规要求识别和缓解针对受保护群体的偏见，但缺乏实际指导和操作化方法。

Method: 提出包含四个组件的偏见检测与缓解管道：1) 基于质量标准的LLM生成词表检测相关群体标签；2) 使用人口统计表示分数量化表示偏见；3) 基于社会语言学信息的过滤检测和缓解刻板印象；4) 通过语法和上下文感知的反事实数据增强补偿表示偏见。

Result: 在数据去偏见方面，成功减少了文本数据集中的表示偏见和显性刻板印象。但在模型偏见减少方面，发现在去偏见数据集上微调的LLM在偏见基准测试中并未一致表现出改进性能，暴露了当前评估方法的重大缺陷。

Conclusion: 虽然提出的管道能有效减少数据偏见，但数据去偏见不一定能转化为模型偏见的减少，这凸显了当前评估方法的局限性，并强调需要有针对性的数据操作来解决已显现的模型偏见。
...
TL;DR: 提出OPV验证器，通过总结长思维链的结果来验证推理过程，结合主动学习和专家标注，在多个基准测试中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于结果的验证器无法检查长思维链中的不可靠中间步骤，而基于过程的验证器由于高质量标注成本高昂，难以可靠检测复杂长思维链中的错误。

Method: 提出基于结果的过程验证器（OPV），通过总结长思维链的结果来验证推理过程；采用迭代主动学习框架，通过专家标注不确定案例，使用拒绝微调（RFT）和RLVR训练新OPV。
...
Conclusion: OPV验证器实现了准确高效的验证，支持大规模标注，在多个任务中表现出优越性能和广泛适用性。
...
<details>
  <summary>Details</summary>
Motivation: 紧急语音识别系统在非标准英语变体（特别是加勒比口音）上存在系统性性能下降，导致加勒比人口在紧急服务中存在关键差距，需要开发能够处理口音变化的应急AI系统。

Method: 三层架构：1) 加勒比口音调优的自动语音识别；2) 通过大语言模型进行本地实体提取；3) 生物声学压力检测。系统将低ASR置信度视为有价值的队列优先级信号，特别是与升高的声音压力标记结合时。

Result: 建立了一个口音弹性的紧急AI框架，确保加勒比声音能够公平获得国家分诊协议。系统提供三个互补信号：转录置信度、结构化临床实体和声音压力指标，但加勒比紧急呼叫的实证验证仍需未来工作。

Conclusion: TRIDENT通过将低ASR置信度重新定义为有价值的优先级信号，结合心理语言学研究中压力诱导的语码转换理论，为灾难场景下的离线操作提供了一个框架，确保加勒比口音英语使用者能够获得公平的紧急分诊服务。
...
TL;DR: 提出OPV（基于结果的流程验证器），通过验证长推理链中总结结果的推理过程，实现准确高效的验证和大规模标注，在多个基准测试中达到SOTA性能。
...
Result: 在OPV-Bench上达到F1分数83.1，优于Qwen3-Max-Preview的76.3；能有效检测合成数据集中的假阳性；与策略模型协作时显著提升性能，如将DeepSeek-R1-Distill-Qwen-32B在AIME2025上的准确率从55.2%提升到73.3%。
...
TL;DR: 研究探讨通过模型缩放作为高效适应新目标语言的策略，发现扩大英语基础模型比标准持续预训练更有效且资源高效，同时能减少灾难性遗忘，并探索语言特定模型的合并方法。


<details>
  <summary>Details</summary>
Motivation: 当前大规模多语言模型在中等和低资源语言上表现仍不如语言特定适应模型，尤其是在较小模型规模下。需要寻找更有效的策略来适应新目标语言。

Method: 通过全面的缩放消融实验，使用近似FLOP匹配的模型，测试放大英语基础模型是否比标准持续预训练更有效。同时探索语言特定模型的合并方法构建模块化多语言系统。

Result: 一旦接触足够的目标语言数据，更大的放大模型可以匹配或超越在更多数据上持续预训练的小模型，显示出缩放对数据效率的益处。缩放还能保持基础模型在英语上的能力，减少灾难性遗忘。合并方法虽然不如联合多语言训练有效，但放大模型的合并表现优于小模型。

Conclusion: 模型缩放是适应新目标语言的有效策略，能提高数据效率和减少灾难性遗忘。语言特定模型的合并有潜力构建模块化多语言系统，但需要专门针对语言级别整合的合并方法改进。
...
TL;DR: LLMs在印度临床应用中面临罗马化文本的性能下降问题，导致母婴健康分诊准确率显著降低
...
Method: 在母婴健康分诊这一关键领域，使用包含五种印度语言和尼泊尔语的真实用户生成查询数据集，对领先的LLMs进行基准测试，比较罗马化文本和原生文字的性能差异

Result: 罗马化消息的性能持续下降，F1分数比原生文字低5-12个百分点；在合作机构中，这种差距可能导致近200万次分诊错误；性能差距并非临床推理失败，LLMs通常能正确推断罗马化查询的语义意图，但最终分类输出在罗马化输入的正字法噪声面前仍然脆弱

Conclusion: LLM健康系统存在关键安全盲点：看似理解罗马化输入的模型可能仍无法可靠地对其采取行动，需要解决罗马化文本处理能力以确保临床应用的可靠性
...
TL;DR: FACTS Leaderboard是一个综合评估语言模型生成事实准确文本能力的在线评测套件，包含四个子评测板，通过自动化评分模型评估模型在多种场景下的真实性表现。


<details>
  <summary>Details</summary>
Motivation: 需要全面评估语言模型在不同场景下生成事实准确文本的能力，提供更全面、平衡的事实性评估标准，以促进模型在真实性方面的改进。

Method: 创建包含四个子评测板的综合评测套件：1) FACTS多模态：评估图像问题回答的事实性；2) FACTS参数化：评估模型内部参数中的世界知识；3) FACTS搜索：评估信息检索场景中的事实性；4) FACTS Grounding (v2)：评估长文本回答是否基于提供的文档。使用自动化评分模型对模型响应进行评分，最终得分为四个组成部分的平均值。

Result: 建立了FACTS Leaderboard评测套件，包含公开和私有数据集，提供在线访问平台（https://www.kaggle.com/benchmarks/google/facts），能够全面评估语言模型的事实性表现。

Conclusion: FACTS Leaderboard提供了一个全面、平衡的语言模型事实性评估框架，通过四个互补的子评测板从不同角度评估模型的事实准确性，并将持续维护更新以保持评测的有效性。
...
### [95] [Unsupervised Acquisition of Discrete Grammatical Categories](https://arxiv.org/abs/2503.18702)
*David Ph. Shakouri,Crit Cremers,Niels O. Schiller*

Main category: cs.CL

TL;DR: 本文介绍了一个用于语言习得实验的计算实验室环境，通过多智能体系统（成人语言模型和女儿语言模型）演示了如何从语言示例中习得抽象语法知识。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索语言习得的计算机制，特别是如何仅通过语言示例（而非内部知识）来获得抽象语法知识，模拟儿童语言习得过程。

Method: 采用多智能体系统：成人语言模型生成语言示例，女儿语言模型仅能访问这些示例。通过层次凝聚聚类分析对成人模型生成的连续话语进行统计分析，识别语法模式并转化为离散语法规则。

Result: 实验表明系统能够习得非平凡的语法知识，获得的语法类别类似于语言学家为自然语言提出的语法范畴。参数配置在训练数据上确定后，在测试集实验中也成功获得了非平凡语法类别。

Conclusion: 该计算实验室环境能够有效地模拟语言习得过程，仅通过语言示例即可获得抽象语法知识，为语言习得研究提供了有效的计算工具。
...
### [96] [STARS: Semantic Tokens with Augmented Representations for Recommendation at Scale](https://arxiv.org/abs/2512.10149)
*Han Chen,Steven Zhu,Yingrui Li*

Main category: cs.IR

TL;DR: STARS是一个为大规模、低延迟电商推荐设计的Transformer序列推荐框架，通过双记忆用户嵌入、语义物品标记、上下文感知评分和延迟敏感的两阶段检索管道，在保持毫秒级响应时间的同时显著提升推荐效果。


<details>
  <summary>Details</summary>
Motivation: 现实电商推荐系统需要在严格的毫秒级延迟约束下提供相关推荐，同时面临冷启动产品、快速变化的用户意图以及季节性、节假日、促销等动态上下文挑战。

Method: STARS结合多项创新：1) 分离长期偏好和短期会话意图的双记忆用户嵌入；2) 融合预训练文本嵌入、可学习增量值和LLM派生属性标签的语义物品标记；3) 学习日历和事件偏移的上下文感知评分；4) 离线嵌入生成和在线最大内积搜索的两阶段检索管道。

Result: 在离线评估中，STARS相比现有LambdaMART系统提升Hit@5超过75%。在600万次访问的大规模A/B测试中，总订单数+0.8%，首页加购+2.0%，用户访问次数+0.5%，均具有统计显著性。
...
### [97] [The Best of the Two Worlds: Harmonizing Semantic and Hash IDs for Sequential Recommendation](https://arxiv.org/abs/2512.10388)
*Ziwei Liu,Yejing Wang,Qidong Liu,Zijian Zhang,Chong Chen,Wei Huang,Xiangyu Zhao*

Main category: cs.IR

TL;DR: 本文提出H2Rec框架，通过协调语义ID和哈希ID来解决序列推荐系统中头部和尾部物品的性能权衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统序列推荐系统使用哈希ID构建物品嵌入，能有效学习协同信息但面临长尾问题。现有结合辅助信息的方法存在噪声协同共享或语义同质化问题。语义ID虽有代码共享和多粒度语义建模能力，但存在协同压倒现象，导致头部和尾部物品性能权衡困境。

Method: 提出H2Rec框架，采用双分支建模架构：一个分支捕获语义ID的多粒度语义，另一个分支保留哈希ID的独特协同身份。引入双级对齐策略桥接两种表示，促进知识转移和支持稳健偏好建模。

Result: 在三个真实世界数据集上的实验表明，H2Rec能有效平衡头部和尾部物品的推荐质量，并超越现有基线方法。

Conclusion: H2Rec通过协调语义ID和哈希ID，解决了序列推荐系统中头部和尾部物品的性能权衡问题，实现了更好的推荐平衡和性能提升。
...
### [98] [Rethinking Popularity Bias in Collaborative Filtering via Analytical Vector Decomposition](https://arxiv.org/abs/2512.10688)
*Lingfeng Liu,Yixin Song,Dazhong Shen,Bing Yin,Hao Li,Yanyong Zhang,Chao Wang*

Main category: cs.IR

TL;DR: 该论文揭示了协同过滤中流行度偏差是BPR优化的固有几何伪影，并提出DDC框架通过方向性分解和修正来纠正嵌入几何，显著提升推荐质量和公平性。


<details>
  <summary>Details</summary>
Motivation: 流行度偏差严重削弱了协同过滤模型的个性化能力，导致过度推荐热门项目而忽视用户对小众内容的真实偏好。现有方法将其视为外部混杂因素，但本文发现流行度偏差实际上是BPR优化中的固有几何伪影。

Method: 通过严格的数学分析证明BPR系统性地将项目嵌入组织在主导的"流行度方向"上，提出方向性分解与修正（DDC）框架。DDC通过非对称方向性更新来纠正嵌入几何：引导正交互沿个性化偏好方向，同时引导负交互远离全局流行度方向，从几何源头解耦偏好与流行度。

Result: 在多个基于BPR的架构上进行广泛实验，DDC显著优于最先进的去偏方法，将训练损失降低到经过大量调优基线的不到5%，同时实现了更优的推荐质量和公平性。

Conclusion: 流行度偏差是BPR优化的固有几何伪影而非外部混杂因素，DDC框架通过方向性分解和修正有效解决了这一问题，为协同过滤中的去偏提供了新的理论视角和实践方法。
...
### [99] [BAMBO: Construct Ability and Efficiency LLM Pareto Set via Bayesian Adaptive Multi-objective Block-wise Optimization](https://arxiv.org/abs/2512.09972)
*Kesheng Chen,Wenjian Luo,Zhenqian Zhu,Yamin Hu,Yiya Xi*

Main category: cs.LG

TL;DR: BAMBO是一个贝叶斯自适应多目标块级优化框架，用于自动构建LLM的帕累托前沿，解决现有模型合并方法在能力-效率权衡中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型合并技术无法有效构建帕累托前沿：粗粒度的模型级方法只能产生稀疏的次优解集，而细粒度的层级方法则面临"维度灾难"，导致搜索空间计算不可行。

Method: 提出BAMBO框架，引入混合最优块划分策略，将其表述为一维聚类问题，采用动态规划方法平衡块内同质性和块间信息分布，显著降低维度而不牺牲关键粒度。整个过程在由q-期望超体积改进获取函数驱动的进化循环中自动化完成。

Result: 实验表明，BAMBO能够发现比基线方法更优越、更全面的帕累托前沿，使模型选择能够根据不同的操作约束进行敏捷调整。

Conclusion: BAMBO成功解决了现有LLM合并技术的局限性，通过自适应块级优化实现了高效、全面的帕累托前沿构建，为不同约束条件下的模型选择提供了灵活解决方案。
...
### [100] [Latent Action World Models for Control with Unlabeled Trajectories](https://arxiv.org/abs/2512.10016)
*Marvin Alles,Xingyuan Zhang,Patrick van der Smagt,Philip Becker-Ehmck*

Main category: cs.LG

TL;DR: 提出了一种潜在动作世界模型，能够同时利用动作标注和无动作数据，通过共享潜在动作表示减少对动作标签的依赖，在DeepMind Control Suite上仅需十分之一的动作标注样本就能达到强性能。


<details>
  <summary>Details</summary>
Motivation: 人类学习结合直接交互和无动作经验（如视频），但标准世界模型依赖动作条件轨迹，当动作标签稀缺时效果受限。需要一种能同时利用动作标注和无动作数据的方法。

Method: 引入潜在动作世界模型家族，学习共享潜在动作表示，将观测控制信号与被动观测推断的动作对齐。使用单个动力学模型在大规模无标签轨迹上训练，仅需少量动作标注样本。通过离线强化学习学习潜在动作策略。

Result: 在DeepMind Control Suite上，该方法仅需约十分之一的动作标注样本就能达到强性能，显著优于纯动作条件基线。证明了潜在动作能够有效结合被动和交互数据进行训练。

Conclusion: 潜在动作世界模型能够有效利用异构数据，减少对动作标签的依赖，使世界模型学习更高效。该方法连接了离线强化学习和无动作训练这两个传统分离的领域。
...
### [101] [Cluster-Dags as Powerful Background Knowledge For Causal Discovery](https://arxiv.org/abs/2512.10032)
*Jan Marco Ruiz de Vargas,Kirtan Padh,Niki Kilbertus*

Main category: cs.LG

TL;DR: 该研究提出利用Cluster-DAGs作为先验知识框架来预热因果发现，并开发了Cluster-PC和Cluster-FCI两种改进的约束型算法，在模拟数据上表现优于无先验知识的基线方法。


<details>
  <summary>Details</summary>
Motivation: 因果发现面临高维数据和复杂依赖关系的挑战，而现有方法在处理这些问题时存在局限。引入系统先验知识可以辅助因果发现，但现有基于分层背景知识的方法灵活性不足。

Method: 提出Cluster-DAGs作为先验知识框架，并基于此开发了两种改进的约束型算法：Cluster-PC用于完全观测设置，Cluster-FCI用于部分观测设置。这些算法利用聚类DAG结构来预热因果发现过程。

Result: 在模拟数据上的实证评估表明，Cluster-PC和Cluster-FCI算法在各自设置下都显著优于没有先验知识的基线方法，证明了Cluster-DAGs框架的有效性。
...
### [102] [CHyLL: Learning Continuous Neural Representations of Hybrid Systems](https://arxiv.org/abs/2512.10117)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在离散模式下学习动力学，但面临模式切换和流不连续性的组合问题，难以准确学习具有连续和离散时间动态的混合系统流。

Method: CHyLL通过重置映射在守卫表面粘合状态空间，将状态空间重构为分段光滑商流形，基于微分拓扑的嵌入定理，在高维空间中同时学习无奇点的神经嵌入和其中的连续流。

Result: CHyLL能够准确预测混合系统的流，具有优越的准确性，并能识别混合系统的拓扑不变量，成功应用于随机最优控制问题。

Conclusion: CHyLL提供了一种无需轨迹分割或模式切换的连续混合系统学习方法，通过将状态空间重构为商流形，实现了对混合系统流的准确学习和拓扑分析。
...
### [103] [Robust Gradient Descent via Heavy-Ball Momentum with Predictive Extrapolation](https://arxiv.org/abs/2512.10033)
*Sarwan Ali*
...
<details>
  <summary>Details</summary>
Motivation: 传统加速梯度方法（如NAG）在病态或非凸问题上容易发散，需要一种既能保持加速效果又能在复杂地形上保持稳定的优化方法
...
Conclusion: HB-SGE为病态和非凸问题提供了鲁棒的优化方案，虽然NAG在良态问题上更快，但HB-SGE在复杂地形上表现更稳定，且仅需O(d)内存开销和与标准动量相同的超参数
...
### [104] [Intelligently Weighting Multiple Reference Models for Direct Preference Optimization of LLMs](https://arxiv.org/abs/2512.10040)
*Skyler Wu,Aymen Echarghaoui*

Main category: cs.LG

TL;DR: 该论文提出了四种新的参考权重设置策略来改进多参考偏好优化（MRPO），但实验发现单参考DPO通常优于所有多参考方法，质疑了多参考方法的实际价值。


<details>
  <summary>Details</summary>
Motivation: 当前多参考偏好优化（MRPO）方法中参考权重的设置方式缺乏统计依据且随意，导致性能不稳定。需要更系统、统计上可靠的方法来设置参考模型的权重，以充分利用多个参考模型的集体优势。

Method: 提出了四种新的权重设置策略：1）两种离线方法利用保留验证信号；2）一种在线方法使用滑动窗口估计器减少过拟合；3）一种在线方法将参考权重设置视为K臂老虎机问题，采用Thompson Sampling。使用Qwen2.5-0.5B作为策略模型，7个来自不同家族的参考模型（0.5B-14B）进行实验。

Result: 所有四种新策略在UltraFeedback和SafeRLHF数据集上的偏好准确率都优于当前的MRPO权重方法。但更令人深思的是，使用7个参考模型中任意6个的单参考DPO方法，在性能上持续优于所有测试的多参考方法。

Conclusion: 虽然提出的新权重策略改进了MRPO的性能，但单参考DPO的优越表现对多参考方法的实际应用价值提出了质疑。这表明在实践中，精心选择的单个参考模型可能比复杂的多参考方法更有效。
...
### [105] [SEMDICE: Off-policy State Entropy Maximization via Stationary Distribution Correction Estimation](https://arxiv.org/abs/2512.10042)
*Jongmin Lee,Meiqi Sun,Pieter Abbeel*

Main category: cs.LG

TL;DR: SEMDICE是一种用于强化学习无监督预训练的新算法，通过最大化状态熵来学习先验策略，能够从任意离策略数据集中计算单一、平稳的马尔可夫状态熵最大化策略。


<details>
  <summary>Details</summary>
Motivation: 在强化学习的无监督预训练中，智能体需要在不依赖任务特定奖励函数的情况下学习下游任务的先验策略。状态熵最大化（SEM）是一种重要方法，但现有方法在从离策略数据集中有效计算SEM策略方面存在局限。

Method: 提出SEMDICE算法，这是一种原则性的离策略算法，直接在平稳分布空间中优化策略。该算法能够从任意离策略数据集中计算单一、平稳的马尔可夫状态熵最大化策略。

Result: 实验结果表明，SEMDICE在最大化状态熵方面优于基线算法，同时在基于SEM的无监督RL预训练方法中实现了最佳的下游任务适应效率。

Conclusion: SEMDICE为强化学习的无监督预训练提供了一种有效的离策略算法，能够从任意数据集中学习状态熵最大化的先验策略，显著提升了下游任务的适应效率。
...
### [106] [Local LLM Ensembles for Zero-shot Portuguese Named Entity Recognition](https://arxiv.org/abs/2512.10043)
*João Lucas Luz Lima Sarcinelli,Diego Furtado Silva*

Main category: cs.LG

TL;DR: 提出一种用于葡萄牙语零样本命名实体识别的三阶段集成方法，通过组合多个本地运行的小型LLM，在资源有限的情况下超越单个模型性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在NLP任务中表现出色，但在命名实体识别（NER）方面表现不佳，特别是对于葡萄牙语等低资源语言。虽然开源模型支持本地部署，但没有单一模型在所有任务中占优，这促使了集成方法的研究。现有LLM集成主要关注文本生成或分类，NER领域研究不足。

Method: 提出一个新颖的三步集成流程：1）使用多个能力相近的本地运行LLM进行零样本NER；2）通过启发式方法选择最优模型组合；3）仅需少量标注数据来优化集成效果。

Result: 在五个葡萄牙语NER数据集中，该方法在四个数据集上超越了单个LLM的性能。此外，在不同源数据集上获得的集成模型在跨数据集配置中通常优于单个LLM，可能消除对当前任务标注数据的需求。

Conclusion: 该工作通过有效组合多个小型LLM而无需微调，推进了可扩展、低资源和零样本NER的发展。代码已开源，为资源有限的语言处理提供了实用解决方案。
...
### [107] [Detailed balance in large language model-driven agents](https://arxiv.org/abs/2512.10047)
*Zhuo-Yang Song,Qing-Hong Cao,Ming-xing Luo,Hua Xing Zhu*

Main category: cs.LG

TL;DR: 该论文首次发现了LLM生成动力学中的宏观物理定律，表明LLM生成可能不是通过学习规则集和策略，而是通过隐式学习一类潜在函数来实现，这些函数可能超越不同的LLM架构和提示模板。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM驱动的智能体在解决复杂问题方面取得了经验性成功，但缺乏理解其宏观动力学的理论框架。当前研究主要停留在工程实践层面，需要建立可预测、可量化的科学基础。

Method: 基于最小作用量原理，通过实验测量LLM生成状态之间的转移概率，统计发现LLM生成转换中存在详细平衡，表明LLM可能隐式学习了一类潜在函数。

Result: 首次发现了不依赖于具体模型细节的LLM生成动力学宏观物理定律，表明LLM生成可能通过隐式学习潜在函数实现，这些函数可能超越不同的LLM架构和提示模板。

Conclusion: 这是建立复杂AI系统宏观动力学理论的尝试，旨在将AI智能体研究从工程实践集合提升为基于可预测、可量化有效测量的科学。
...
### [108] [DB2-TransF: All You Need Is Learnable Daubechies Wavelets for Time Series Forecasting](https://arxiv.org/abs/2512.10051)
...
<details>
  <summary>Details</summary>
Motivation: 传统Transformer模型在时间序列预测中虽然能有效捕捉长程依赖关系，但其二次计算复杂度限制了在大规模高维场景下的可扩展性和适应性，需要更高效的架构。

Method: 提出DB2-TransF架构，用可学习的Daubechies小波系数层替代Transformer中的自注意力机制，该小波模块能高效捕捉多尺度局部和全局模式，增强多时间序列间的相关性建模。

Result: 在13个标准预测基准测试中，DB2-TransF实现了与传统Transformer相当或更优的预测精度，同时显著减少了内存使用量，证明了其作为可扩展和资源高效框架的有效性。

Conclusion: DB2-TransF通过小波系数层替代自注意力机制，为时间序列预测提供了一个计算高效且可扩展的解决方案，在保持预测性能的同时大幅降低了资源消耗。
...
### [109] [Mitigating Exposure Bias in Risk-Aware Time Series Forecasting with Soft Tokens](https://arxiv.org/abs/2512.10056)
...
<details>
  <summary>Details</summary>
Motivation: 自回归预测在糖尿病和血流动力学管理中至关重要，但标准模型使用教师强制训练时存在暴露偏差，导致闭环使用中的多步预测不稳定，需要更可靠的预测方法。

Method: 提出软令牌轨迹预测（SoTra），传播连续概率分布（软令牌）来缓解暴露偏差，学习校准的、不确定性感知的轨迹，并加入风险感知解码模块最小化临床风险。

Result: 在血糖预测中，SoTra将基于区域的平均风险降低18%；在血压预测中，将有效临床风险降低约15%，显著优于传统方法。

Conclusion: SoTra方法在安全关键的预测控制中表现出色，通过缓解暴露偏差和学习不确定性感知轨迹，显著降低了临床风险，适用于糖尿病和血流动力学管理。
...
### [110] [MedXAI: A Retrieval-Augmented and Self-Verifying Framework for Knowledge-Guided Medical Image Analysis](https://arxiv.org/abs/2512.10098)
*Midhat Urooj,Ayan Banerjee,Farhat Shaikh,Kuntal Thakur,Sandeep Gupta*

Main category: cs.LG

TL;DR: MedXAI是一个可解释的医学影像分类框架，通过整合深度学习模型与临床专家知识，提升泛化能力、减少罕见类别偏见，并提供临床可理解的解释。


<details>
  <summary>Details</summary>
Motivation: 医学AI面临领域偏移、罕见类别偏见和缺乏透明度等挑战，现有深度学习模型在真实世界分布变化下表现不佳，且缺乏安全关键临床环境所需的可解释性。

Method: 提出MedXAI框架，将深度视觉模型与临床专家知识相结合，通过定位相关诊断特征而非依赖技术性后处理方法（如显著性图、LIME）来提供人类可理解的解释。

Result: 在十个多中心数据集上评估，显示跨域泛化能力提升3%，罕见类别F1分数提升10%，显著优于强深度学习基线。消融实验证实符号组件作为有效的临床先验和正则化器。
...
### [111] [Partitioning the Sample Space for a More Precise Shannon Entropy Estimation](https://arxiv.org/abs/2512.10133)
*Gabriel F. A. Bastos,Jugurta Montalvão*

Main category: cs.LG

TL;DR: 提出一种离散熵估计器，通过结合缺失质量和未观测结果数量的估计来补偿负偏差，在小数据集情况下优于经典方法


<details>
  <summary>Details</summary>
Motivation: 从小数据集中可靠地估计香农熵是一个关键问题，特别是在样本数量可能少于可能结果数量的情况下

Method: 使用可分解性特性，结合缺失质量和未观测结果数量的估计来补偿负偏差

Result: 在欠采样情况下优于一些经典估计器，与一些成熟的先进估计器性能相当

Conclusion: 提出的方法在小数据集情况下能有效估计离散熵，解决了传统方法的负偏差问题
...
### [112] [Sequence-to-Image Transformation for Sequence Classification Using Rips Complex Construction and Chaos Game Representation](https://arxiv.org/abs/2512.10141)
*Sarwan Ali,Taslim Murad,Imdadullah Khan*

Main category: cs.LG

TL;DR: 该论文提出了一种新颖的拓扑方法，将分子序列通过混沌游戏表示转化为图像，并结合代数拓扑中的Rips复形构造，有效解决了传统特征工程和深度学习在分子序列分类中的问题。


<details>
  <summary>Details</summary>
Motivation: 传统分子序列分类方法存在稀疏性和计算复杂度问题，而深度学习模型在表格化生物数据上表现不佳，需要一种能有效捕捉分子序列结构特征的新方法。

Method: 结合混沌游戏表示和Rips复形构造：1) 将序列元素映射到2D坐标；2) 计算成对距离；3) 构建Rips复形以捕捉局部结构和全局拓扑特征。

Result: 在抗癌肽数据集上表现出色，在乳腺癌和肺癌数据集上分别达到86.8%和94.5%的准确率，优于基于向量、序列语言模型和现有图像方法。

Conclusion: 拓扑表示保留了关键序列信息，同时使基于视觉的深度学习架构能有效应用于分子序列分析，为分子序列分类提供了新的有效方法。
...
### [113] [Murmur2Vec: A Hashing Based Solution For Embedding Generation Of COVID-19 Spike Sequences](https://arxiv.org/abs/2512.10147)
...
### [114] [CIEGAD: Cluster-Conditioned Interpolative and Extrapolative Framework for Geometry-Aware and Domain-Aligned Data Augmentation](https://arxiv.org/abs/2512.10178)
*Keito Inoshita,Xiaokang Zhou,Akira Kawai,Katsutoshi Yada*

Main category: cs.LG

TL;DR: CIEGAD是一个用于数据增强的框架，通过聚类条件、插值和外推合成来补充真实数据分布中的语义未覆盖区域，特别适用于长尾和多类分类任务。


<details>
  <summary>Details</summary>
Motivation: 在实际深度学习部署中，数据稀缺和标签分布不平衡导致真实数据分布中存在语义未覆盖区域，这阻碍模型训练，导致类边界附近错误分类和边缘区域不稳定行为。虽然大语言模型在数据增强方面有潜力，但尚未建立能够同时实现生成方向控制、领域对齐和质量控制的集成框架。

Method: CIEGAD框架包含：1) 通过聚类条件构建领域配置文件；2) 使用结合类别频率和几何指标的分层频率-几何分配进行生成分配；3) 通过插值和外推合成共存精细控制生成方向；4) 通过几何约束过滤结合LLM-as-a-Judge机制进行质量控制。
...
### [115] [MiniF2F-Dafny: LLM-Guided Mathematical Theorem Proving via Auto-Active Verification](https://arxiv.org/abs/2512.10187)
...
<details>
  <summary>Details</summary>
Motivation: miniF2F基准此前仅存在于交互式定理证明器中，需要将其扩展到自动定理证明器Dafny，以探索自动化证明和LLM辅助的潜力

Method: 将miniF2F基准翻译到Dafny，评估自动化验证成功率，测试12个现成LLM在提供证明提示方面的表现，采用迭代错误修正策略

Result: Dafny自动化验证了测试集40.6%和验证集44.7%的问题（空证明），最佳LLM模型通过迭代错误修正达到55.7%的pass@4成功率

Conclusion: 展示了有效的分工协作：LLM提供高层指导，自动化处理底层细节，为数学定理自动证明提供了新思路
...
TL;DR: 提出了一种新的联邦域泛化方法，通过潜在空间反转技术增强客户端隐私保护，并采用重要权重聚合策略处理非独立同分布数据，在减少通信开销的同时实现更好的泛化性能。
...
Method: 1. 潜在空间反转技术：在本地客户端训练中强制跨本地模型的域不变性，增强客户端隐私保护；2. 重要权重聚合策略：在模型聚合时优先考虑对本地模型预测有显著影响的参数，保留本地适配能力。

Result: 实验表明该方法在减少通信开销的同时，取得了优于现有最先进方法的性能表现。

Conclusion: 提出的联邦域泛化方法通过潜在空间反转和重要权重聚合，在保护客户端隐私的同时有效处理非独立同分布数据，实现了更好的泛化性能和更低的通信成本。
...
TL;DR: 本文提出了自适应信息路由（AIR）框架，这是一种用于多模态时间序列预测的新方法，通过文本信息动态指导时间序列模型，并引入文本精炼管道将原始文本转换为适合预测的形式。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列预测主要依赖历史数据，但在实际场景中，仅凭时间序列数据往往信息有限，难以实现准确预测。多模态方法虽然结合了文本等额外数据，但现有方法将文本数据与时间序列数据同等对待，未能充分利用文本信息来指导时间序列模型的动态行为。

Method: 提出了自适应信息路由（AIR）框架，该框架利用文本信息动态控制多元时间序列信息的组合方式和程度，而不是将文本数据视为可互换的辅助特征。同时开发了文本精炼管道，使用大型语言模型将原始文本数据转换为适合多模态预测的形式，并建立了相应的基准测试平台。

Result: 在原油价格和汇率等真实市场数据上的实验结果表明，AIR能够有效利用文本输入调节时间序列模型的行为，在各种时间序列预测任务中显著提高了预测准确性。

Conclusion: AIR框架通过文本信息动态指导时间序列模型，提供了一种更有效的多模态时间序列预测方法，相比传统方法能更好地利用文本信息来提升预测性能。
...
TL;DR: 本文提出了一种双正则化异质高斯过程框架（R²-HGP），通过可训练的先验概率映射模型对齐异质输入域，结合物理知识正则化和稀疏惩罚，解决多源迁移学习中异质输入、物理信息利用和负迁移问题。


<details>
  <summary>Details</summary>
Motivation: 多输出高斯过程模型在多源迁移学习中面临三大挑战：1）源域和目标域输入空间异质，难以直接知识迁移；2）忽略先验知识和物理信息，导致不稳定映射；3）不当的信息共享容易引发负迁移。传统模型无法统一解决这些问题。

Method: 提出R²-HGP框架：1）设计可训练的先验概率映射模型对齐异质输入域，将对齐结果作为潜变量；2）构建多源迁移高斯过程模型，整合到条件变分自编码器框架中；3）加入物理知识作为正则项，确保对齐结果符合已知物理规律；4）对迁移系数施加稀疏惩罚，自适应选择最有信息量的源输出并抑制负迁移。

Result: 通过大量仿真和真实工程案例验证，R²-HGP在多种评估指标上均优于现有最先进基准方法，证明了其有效性。

Conclusion: R²-HGP框架成功解决了多源迁移学习中的异质输入对齐、物理知识整合和负迁移抑制问题，为复杂工程应用提供了更可靠的不确定性量化解决方案。
...
### [119] [An Interpretable AI Tool for SAVR vs TAVR in Low to Intermediate Risk Patients with Severe Aortic Stenosis](https://arxiv.org/abs/2512.10308)
*Vasiliki Stoumpou,Maciej Tysarowski,Talhat Azemi,Jawad Haider,Howard L. Haronian,Robert C. Hagberg,Dimitris Bertsimas*

Main category: cs.LG

TL;DR: 本文提出了一种可解释的处方框架，通过预后匹配、反事实结果建模和最优策略树，为主动脉瓣狭窄患者提供个体化的TAVR与SAVR治疗建议，以最小化5年死亡率。


<details>
  <summary>Details</summary>
Motivation: 目前临床实践中，对于低至中度风险的严重主动脉瓣狭窄患者，选择外科主动脉瓣置换术（SAVR）还是经导管主动脉瓣置换术（TAVR）存在较大变异性，主要受患者异质性和机构偏好影响。现有模型主要预测术后风险，缺乏能够直接优化长期结果的可解释、个体化治疗推荐。

Method: 研究引入了一个可解释的处方框架，整合了预后匹配、反事实结果建模和最优策略树（OPT）。使用Hartford医院和St. Vincent's医院的数据，通过预后匹配和样本加权模拟随机化，估计患者在SAVR和TAVR两种治疗下的反事实死亡率。策略模型基于这些反事实预测进行训练，将患者划分为临床上一致的亚组，并推荐估计风险较低的治疗方案。

Result: 如果应用OPT推荐的治疗方案，反事实评估显示，相对于实际临床实践中的治疗方案，5年死亡率在Hartford医院估计降低20.3%，在St. Vincent's医院降低13.8%。这表明该模型在不同机构的未见数据上具有良好的泛化能力。学习到的决策边界与现实世界结果和临床观察一致。

Conclusion: 该可解释的处方框架首次为TAVR与SAVR选择提供了透明、数据驱动的推荐，在内部和外部队列中都能改善估计的长期结果，同时保持临床合理性，为结构性心脏病精准医学提供了更系统、循证的方法。
...
### [120] [A Privacy-Preserving Cloud Architecture for Distributed Machine Learning at Scale](https://arxiv.org/abs/2512.10341)
...
Method: 设计了一个云原生隐私保护架构，整合了联邦学习（避免集中敏感数据）、差分隐私（提供形式化隐私保证）、零知识合规证明（可验证政策执行）和基于强化学习的自适应治理。在混合Kubernetes集群上部署了完整原型。

Result: 原型系统降低了成员推理攻击风险，保持了一致的正式隐私预算执行，在差分隐私下维持了稳定的模型性能。跨多机构工作负载的实验评估显示，该架构以最小开销保持效用，并提供持续的风险感知治理。

Conclusion: 该框架为大规模部署可信赖且合规的分布式机器学习系统建立了实用基础，实现了隐私保护、合规验证和自适应治理的集成解决方案。
...
### [121] [Dynamics of Agentic Loops in Large Language Models: A Geometric Theory of Trajectories](https://arxiv.org/abs/2512.10350)
*Nicolas Tacheny*

Main category: cs.LG

TL;DR: 论文提出几何框架分析智能体循环在语义嵌入空间中的动态行为，区分语言变换的工件空间和几何测量的嵌入空间，通过等距校准消除余弦相似度偏差，识别收缩与发散两种基本机制。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的智能体系统通过递归反馈循环运行，但人们对这些智能体循环的几何行为（收敛、发散或更复杂动态）了解甚少，需要建立理论框架来分析迭代变换的动态特性。

Method: 提出几何框架分析智能体轨迹在语义嵌入空间中的行为，将迭代变换视为离散动力系统；区分工件空间（语言变换发生）和嵌入空间（几何测量）；引入等距校准消除余弦相似度的各向异性偏差，使相似度与人类语义判断对齐；通过控制实验研究单一智能体循环。

Result: 识别两种基本机制：收缩重写循环收敛于稳定吸引子且离散度递减；探索性总结与否定循环产生无界发散且无聚类形成；这些机制显示出收缩与扩张的定性不同几何特征；提示设计直接控制智能体循环的动态机制。

Conclusion: 智能体循环的几何行为可通过系统框架分析，等距校准使几何测量更可靠，识别的基本机制为理解智能体动态提供理论基础，提示设计能系统控制迭代变换的收敛、发散和轨迹结构。
...
### [122] [GPG: Generalized Policy Gradient Theorem for Transformer-based Policies](https://arxiv.org/abs/2512.10365)
*Hangyu Mao,Guangting Dong,Zhicheng Dou*

Main category: cs.LG

TL;DR: 提出了专门为Transformer策略设计的广义策略梯度定理，证明标准策略梯度定理和GRPO都是其特例，并探索了在大型语言模型训练中的实际应用。


<details>
  <summary>Details</summary>
Motivation: 针对Transformer架构的策略优化问题，现有策略梯度方法可能不是最优的，需要专门为Transformer设计的策略梯度理论框架。

Method: 提出了广义策略梯度定理，专门针对Transformer策略设计，将标准策略梯度定理和GRPO作为其特例包含在内。

Result: 建立了统一的Transformer策略优化理论框架，为大型语言模型训练提供了新的策略优化视角。

Conclusion: 广义策略梯度定理为Transformer策略优化提供了理论基础，在大型语言模型训练中具有实际应用价值。
...
### [123] [Fitting magnetization data using continued fraction of straight lines](https://arxiv.org/abs/2512.10390)
*Vijay Prakash S*
...
<details>
  <summary>Details</summary>
Motivation: 铁磁物质磁化强度随外磁场增强而增加，这是由于微观磁畴逐渐与外场对齐。这种磁畴对齐形成了磁化的非线性物理基础，需要建立合适的数学模型来描述这种非线性行为。

Method: 采用直线连分数来逼近磁化非线性函数，这种代数表达式可以通过非线性回归来估计参数，从而拟合磁化曲线。

Result: 直线连分数方法能够有效逼近铁磁物质的磁化非线性函数，可用于解释磁畴在生长和收缩过程中的行为变化。

Conclusion: 直线连分数为铁磁物质磁化非线性行为提供了有效的数学描述工具，能够通过参数估计来量化磁畴对齐过程，为理解磁化机制提供了新的分析框架。
...
TL;DR: 该论文提出基于人类元认知科学的新框架，使用meta-d'指标评估AI模型的元认知敏感性，并通过基于bandit的仲裁器进行测试时模型选择，提高联合推理准确性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型虽然能表达预测置信度，但常常存在校准不良的问题，即表达的置信度不能反映真实能力。受人类认知科学启发，需要评估AI模型是否真正"知道它们知道什么"，并利用这种元认知能力改进模型选择。

Method: 1. 引入meta-d'作为心理学基础的元认知敏感性度量，评估模型置信度预测自身准确性的可靠性；2. 使用动态敏感性分数作为上下文，构建基于bandit的仲裁器，在测试时学习信任哪个专家模型；3. 在多个数据集和深度学习模型组合（包括CNN和VLM）上进行实验验证。

Result: 实验表明，这种元认知方法相比组成模型提高了联合推理准确性。该框架为AI模型提供了新颖的行为描述，将集成选择重新定义为评估短期信号（置信度预测分数）和中期特征（元认知敏感性）的问题。

Conclusion: 该工作从人类认知科学角度提出了评估和利用AI元认知的新框架，通过元认知敏感性度量和动态模型选择机制，有效改善了深度学习模型的校准问题和推理性能。
...
### [125] [Adaptive Replay Buffer for Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2512.10510)
*Chihyeon Song,Jaewoo Lee,Jinkyoo Park*

Main category: cs.LG

TL;DR: ARB是一种自适应回放缓冲区，通过"on-policyness"指标动态调整离线与在线数据的采样权重，解决O2O RL中固定数据混合比率的困境。


<details>
  <summary>Details</summary>
Motivation: 离线到在线强化学习面临平衡固定离线数据集和新收集在线经验的困境，现有方法使用固定数据混合比率难以兼顾早期学习稳定性和渐进性能。

Method: 提出自适应回放缓冲区(ARB)，基于轻量级"on-policyness"指标动态优先化数据采样，评估轨迹与当前策略行为的对齐程度，为每个转换分配比例采样权重。

Result: 在D4RL基准测试上的广泛实验表明，ARB能持续缓解早期性能下降，显著提升各种O2O RL算法的最终性能。

Conclusion: ARB证明了自适应、行为感知的回放缓冲区设计的重要性，能够有效利用离线数据实现初始稳定性，同时渐进聚焦于最相关、高回报的在线经验。
...
### [126] [Disentangled and Distilled Encoder for Out-of-Distribution Reasoning with Rademacher Guarantees](https://arxiv.org/abs/2512.10522)
*Zahra Rahiminasab,Michael Yuhas,Arvind Easwaran*

Main category: cs.LG

TL;DR: 提出DDE框架，通过蒸馏压缩变分自编码器的解耦潜在空间，减少OOD推理器大小，同时保持解耦特性


<details>
  <summary>Details</summary>
Motivation: 解耦潜在空间可用于多标签OOD样本推理，但现有模型在资源受限设备上部署困难，需要压缩模型同时保持解耦特性

Method: 提出解耦蒸馏编码器框架，将师生蒸馏形式化为约束优化问题，使用解耦约束保持解耦特性，基于Rademacher复杂度建立理论保证

Result: 在NVIDIA设备上部署压缩模型进行实证评估，证明方法能有效压缩模型同时保持解耦特性

Conclusion: DDE框架成功实现了模型压缩与解耦特性保持的平衡，为资源受限设备上的OOD推理提供了可行方案
...
### [127] [Mode-Seeking for Inverse Problems with Diffusion Models](https://arxiv.org/abs/2512.10524)
*Sai Bharath Chandra Gutha,Ricardo Vinuesa,Hossein Azizpour*

Main category: cs.LG

TL;DR: 提出VML损失函数，通过最小化扩散后验与测量后验的KL散度，引导生成样本趋向最大后验估计，无需任务特定训练即可解决逆问题


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练无条件扩散模型的后验采样和MAP估计方法存在建模近似和计算成本高的问题，需要更有效的方法来解决逆问题

Method: 提出变分模式寻求损失(VML)，在反向扩散的每一步最小化扩散后验与测量后验的KL散度，引导样本趋向MAP估计；对于线性逆问题，VML可解析推导无需近似

Result: 提出的VML-MAP算法在多种图像恢复任务和数据集上，相比现有方法在性能和计算时间上均表现出优势
...
### [128] [Unlocking the Address Book: Dissecting the Sparse Semantic Structure of LLM Key-Value Caches via Sparse Autoencoders](https://arxiv.org/abs/2512.10547)
...
<details>
  <summary>Details</summary>
Motivation: KV缓存是长上下文大语言模型中的主要内存瓶颈，但通常被视为不透明的数值张量。需要一种既能减少内存占用又能保持注意力机制几何结构的方法，同时提供对KV缓存的解释性理解。

Method: 提出STA-Attention框架，使用Top-K稀疏自编码器（SAEs）将KV缓存分解为可解释的语义原子。与标准L1正则化SAE不同，Top-K方法消除收缩偏差，保持注意力所需的精确点积几何。基于发现的键值不对称性（Key作为稀疏路由器，Value携带密集内容），引入双预算策略选择性保留信息量最大的语义成分。

Result: 在Yi-6B、Mistral-7B、Qwen2.5-32B等模型上的实验表明，语义重建保持了与原始模型相当的困惑度和零样本性能，有效弥合了机制可解释性和忠实注意力建模之间的差距。

Conclusion: STA-Attention框架通过将KV缓存分解为语义原子，不仅解决了内存瓶颈问题，还提供了对注意力机制的可解释性理解，同时保持模型性能，为长上下文LLM的高效部署提供了新思路。
...
### [129] [Is the Information Bottleneck Robust Enough? Towards Label-Noise Resistant Information Bottleneck Learning](https://arxiv.org/abs/2512.10573)
...
<details>
  <summary>Details</summary>
Motivation: 传统信息瓶颈方法严重依赖准确标签，在现实场景中容易受到标签噪声影响，导致性能下降和过拟合。需要开发能够在标签噪声下保持鲁棒性的表示学习方法。

Method: 提出LaT-IB方法：1）引入"最小充分干净"准则作为互信息正则化器；2）采用噪声感知潜在解耦，将潜在表示分解为与干净标签空间和噪声空间对齐的组件；3）设计三阶段训练框架：预热、知识注入和鲁棒训练。

Result: 理论分析推导了预测、压缩和解耦各组件互信息边界，证明优化目标能鼓励对输入噪声不变的表示并分离干净和噪声标签信息。实验表明LaT-IB在标签噪声下实现了优越的鲁棒性和效率。

Conclusion: LaT-IB通过创新的噪声感知信息瓶颈框架，有效解决了传统IB方法对标签噪声的脆弱性，显著增强了在现实标签噪声场景下的鲁棒性和适用性。
...
### [130] [Multi-Objective Reward and Preference Optimization: Theory and Algorithms](https://arxiv.org/abs/2512.10601)
*Akhil Agnihotri*

Main category: cs.LG

TL;DR: 该论文在约束强化学习领域提出了一系列理论框架和算法，涵盖平均成本CMDP、有限时域CMDP、人类偏好学习以及大语言模型对齐等多个方面，为安全和对齐的决策制定提供了理论和实践工具。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在处理安全约束、人类偏好和模型对齐方面存在不足，需要开发能够同时满足性能要求和约束条件的算法框架，特别是在安全关键环境和大型语言模型对齐等实际应用中。

Method: 1. ACPO算法：结合敏感性分析和信任域更新处理平均成本CMDP
2. e-COP算法：基于episodic策略差异引理处理有限时域CMDP
3. warmPref-PS：集成异构评分者离线偏好数据的线性赌博机后验采样策略
4. PSPL算法：从成对轨迹比较中联合采样奖励模型和转移动态
5. MOPO算法：多目标约束优化方法，用于大规模语言模型对齐

Result: 1. ACPO在平均成本CMDP中实现了最先进的实证性能并具有理论保证
2. e-COP在安全关键环境中提供了可证明的性能、简单性和可扩展性
3. warmPref-PS显著减少了遗憾并提高了RLHF的数据收集效率
4. PSPL提供了贝叶斯简单遗憾保证并能够稳健地识别最优策略
5. MOPO可扩展到数十亿参数的语言模型并在各种对齐设置中保持稳健性

Conclusion: 该论文统一了约束强化学习在平均成本、episodic和偏好驱动范式下的方法，为安全和对齐的决策制定提供了理论进展和实用工具，在控制、偏好学习和大型语言模型对齐等多个领域都有重要贡献。
...
### [131] [Supporting Migration Policies with Forecasts: Illegal Border Crossings in Europe through a Mixed Approach](https://arxiv.org/abs/2512.10633)
*C. Bosco,U. Minora,D. de Rigo,J. Pingsdorf,R. Cortinovis*

Main category: cs.LG

TL;DR: 提出一种混合方法预测欧洲非法边境越境，整合机器学习与专家定性洞察，旨在改进预测能力并支持欧盟移民政策


<details>
  <summary>Details</summary>
Motivation: 应对移民模式突然变化和传统数据限制的挑战，满足《欧盟移民与庇护公约》中概述的预测需求，支持庇护与移民管理法规

Method: 混合方法整合机器学习技术与移民专家的定性洞察，引入人类评估协变量，针对欧洲五条主要移民路线进行一年期预测

Result: 方法经过已知数据测试验证，证明其在移民相关政策背景下的适用性和可靠性

Conclusion: 通过结合数据驱动建模与专家判断，为欧盟移民治理提供了一个新颖的操作工具，支持政策相关预测、早期预警系统和成员国间团结机制
...
### [132] [Token Sample Complexity of Attention](https://arxiv.org/abs/2512.10656)
*Léa Bohbot,Cyril Letrouit,Gabriel Peyré,François-Xavier Vialard*

Main category: cs.LG

TL;DR: 本文研究了大型语言模型中注意力机制在极长序列下的收敛行为，提出了"token-sample复杂度"概念，分析了注意力映射在不同条件下的收敛速率。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型上下文窗口的不断扩大，需要研究注意力机制在极长序列长度下的行为特征，特别是注意力如何收敛到其无限token极限。

Method: 引入token-sample复杂度概念，在两个层面估计有限n的收敛界：注意力映射的点态一致收敛，以及变换后token分布矩的收敛。针对紧支撑分布和亚高斯分布，分析不同条件下的收敛速率。

Result: 对于紧支撑分布，注意力映射在半径为R的球上以C(R)/√n的速率一致收敛，其中C(R)随R指数增长。对于变换分布矩的收敛，速率为C'(R)/n^β，其中β<1/2，C'(R)多项式依赖于分布支撑大小。在注意力参数趋于无穷时，获得对数收敛速率。

Conclusion: 研究为理解长序列下注意力机制的行为提供了理论框架，实验验证了理论预测，对大型语言模型的设计和分析具有指导意义。
...
### [133] [DCFO Additional Material](https://arxiv.org/abs/2512.10659)
...
<details>
  <summary>Details</summary>
Motivation: 异常检测需要解释性来理解异常原因、验证显著性、识别偏差。现有反事实解释方法大多忽视异常检测的特殊挑战，且未针对LOF等经典算法。LOF作为最流行的无监督异常检测方法之一，缺乏可解释性。

Method: 提出Density-based Counterfactuals for Outliers (DCFO)方法，专门为LOF生成反事实解释。通过将数据空间划分为LOF行为平滑的区域，实现高效的基于梯度的优化。

Result: 在50个OpenML数据集上进行广泛实验验证，DCFO在生成反事实的接近度和有效性方面始终优于基准竞争对手。
...
### [134] [Learning by Analogy: A Causal Framework for Composition Generalization](https://arxiv.org/abs/2512.10669)
...
<details>
  <summary>Details</summary>
Motivation: 组合泛化能力使模型能够超越有限经验，但实现这一关键能力的数据结构和原理仍不清楚。论文认为组合泛化需要将高级概念分解为可跨相似上下文重组的低级基本概念，类似于人类通过类比理解新概念的方式。

Method: 使用因果模块化和最小变化原则形式化组合泛化过程，引入分层数据生成过程来编码不同层次概念及其交互机制。理论上证明该方法支持复杂概念关系的组合泛化，并证明潜在分层结构可从文本-图像对等观测数据中可识别恢复。

Result: 理论框架支持超越简单加性效应的复杂概念关系组合泛化，证明了潜在分层结构的可识别性。基于理论见解在基准数据集上取得了显著改进。

Conclusion: 通过因果模块化和分层数据生成过程的形式化框架，为组合泛化提供了理论基础，证明复杂概念关系的组合泛化是可能的，且潜在结构可从数据中恢复，为实际应用提供了理论指导。
...
### [135] [HybridVFL: Disentangled Feature Learning for Edge-Enabled Vertical Federated Multimodal Classification](https://arxiv.org/abs/2512.10701)
*Mostafa Anoosha,Zeinab Dehghani,Kuniko Paxton,Koorosh Aslansefat,Dhavalkumar Thakker*

Main category: cs.LG

TL;DR: HybridVFL是一个针对边缘AI场景的新型垂直联邦学习框架，通过客户端特征解耦和服务器端跨模态Transformer融合，在保护隐私的同时显著提升多模态数据分类性能


<details>
  <summary>Details</summary>
Motivation: 传统垂直联邦学习在边缘AI场景（如移动健康诊断）中存在性能限制，主要原因是特征融合方式过于简单，无法充分利用多模态数据的优势

Method: 提出HybridVFL框架：1）客户端进行特征解耦，提取有意义的特征表示；2）服务器端使用跨模态Transformer进行上下文感知的特征融合，实现更智能的多模态信息整合

Result: 在多模态皮肤病变数据集HAM10000上的系统评估表明，HybridVFL显著优于标准的联邦学习基线方法，验证了先进融合机制在隐私保护系统中的重要性

Conclusion: 通过结合客户端特征解耦和服务器端跨模态Transformer融合，HybridVFL为资源受限的边缘设备提供了一个既保护隐私又具有高性能的多模态学习解决方案
...
### [136] [Beyond the Black Box: Identifiable Interpretation and Control in Generative Models via Causal Minimality](https://arxiv.org/abs/2512.10720)
...
<details>
  <summary>Details</summary>
Motivation: 深度生成模型虽然革命性地改变了图像和文本生成领域，但通常作为不透明的黑箱运行，阻碍了人类理解、控制和对其对齐。现有方法如稀疏自编码器（SAEs）虽然经验上成功，但缺乏理论保证，可能导致主观见解。主要目标是建立可解释生成模型的原则性基础。

Method: 引入基于因果最小化原则的新理论框架——分层选择模型，其中高层概念由低层变量的约束组合产生，更好地捕捉数据生成的复杂依赖关系。在理论上推导的最小化条件（表现为稀疏性或压缩约束）下，学习到的表示可以等价于数据生成过程的真实潜在变量。

Result: 将因果最小化约束应用于领先的生成模型（扩散视觉模型和自回归语言模型），能够提取其固有的分层概念图，提供对其内部知识组织的新见解。这些基于因果的概念可作为杠杆实现细粒度的模型控制。

Conclusion: 因果最小化原则能够赋予生成模型的潜在表示清晰的因果解释和鲁棒的组件级可识别控制，为构建透明、可靠的系统铺平道路。
...
### [137] [Generalized Spherical Neural Operators: Green's Function Formulation](https://arxiv.org/abs/2512.10723)
*Hao Tang,Hao Chen,Chao Li*

Main category: cs.LG

TL;DR: 提出基于可设计球面格林函数及其谐波展开的通用算子设计框架，建立球面学习的算子理论基础，开发GSNO算子和GSHNet层次架构，在多个应用领域超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 神经算子虽能有效求解参数偏微分方程，但扩展到球面域仍具挑战性，需要保持内在几何特性同时避免破坏旋转一致性的失真。现有球面算子依赖旋转等变性但缺乏处理现实世界复杂性的灵活性。

Method: 提出基于可设计球面格林函数及其谐波展开的通用算子设计框架，建立球面学习的算子理论基础。基于此提出绝对和相对位置依赖的格林函数，平衡等变性和不变性。开发GSNO算子和新颖的谱学习方法，适应各向异性、约束丰富的系统。构建GSHNet层次架构，结合多尺度谱建模与球面上-下采样。

Result: 在扩散MRI、浅水动力学和全球天气预报等任务上评估，GSNO和GSHNet一致优于最先进方法。GSNO成为球面算子学习的原理性和通用框架，连接严谨理论与现实复杂性。

Conclusion: 提出的基于可设计球面格林函数的框架为球面算子学习建立了坚实的理论基础，GSNO算子和GSHNet架构在保持谱效率的同时能够适应现实世界的复杂性，在多个应用领域展现出优越性能。
...
### [138] [Interpretable and Steerable Concept Bottleneck Sparse Autoencoders](https://arxiv.org/abs/2512.10805)
*Akshay Kulkarni,Tsui-Wei Weng,Vivek Narayanaswamy,Shusen Liu,Wesam A. Sakla,Kowshik Thopalli*

Main category: cs.LG

TL;DR: 本文提出概念瓶颈稀疏自编码器(CB-SAE)，通过剪枝低效神经元并添加用户定义概念瓶颈，显著提升LVLM中稀疏自编码器的可解释性和可操控性。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器(SAE)在大型语言模型和视觉语言模型的机制可解释性、概念发现和模型操控方面具有潜力，但现有方法存在两个主要问题：1）大多数SAE神经元可解释性或可操控性较低，下游应用效果差；2）由于无监督特性，用户期望的概念往往不在学习到的字典中，限制了实际应用价值。

Method: 提出概念瓶颈稀疏自编码器(CB-SAE)框架，包含两个核心组件：1）剪枝低效神经元；2）在潜在空间中添加轻量级概念瓶颈，该瓶颈与用户定义的概念集对齐。该方法通过新的计算成本较低的可解释性和可操控性度量进行系统分析。

Result: CB-SAE在视觉语言模型和图像生成任务中，将可解释性提升32.1%，可操控性提升14.5%。该方法有效解决了传统SAE中神经元效用低和用户概念缺失的问题。

Conclusion: CB-SAE通过后处理框架显著提升了稀疏自编码器的实用价值，使其在机制可解释性、概念发现和模型操控方面更具实际应用潜力，为大型模型的解释和操控提供了有效解决方案。
...
### [139] [Learning Controllable and Diverse Player Behaviors in Multi-Agent Environments](https://arxiv.org/abs/2512.10835)
...
Method: 将玩家行为定义为N维连续空间，从包含真实人类风格子集的区域均匀采样目标行为向量；训练时每个智能体接收当前和目标行为向量作为输入，奖励基于两者距离的归一化减少；使用PPO-based多智能体策略

Result: 在自定义多玩家Unity游戏中，相比仅追求胜利的基线方法，该框架产生显著更大的行为多样性，并能可靠匹配不同目标的行为向量；单个策略可重现新的或未见过的游戏风格而无需重新训练
...
### [140] [Generative Modeling from Black-box Corruptions via Self-Consistent Stochastic Interpolants](https://arxiv.org/abs/2512.10857)
*Chirag Modi,Jiequn Han,Eric Vanden-Eijnden,Joan Bruna*

Main category: cs.LG

TL;DR: 提出自洽随机插值法(SCSI)，通过迭代更新污染数据与干净数据之间的传输映射，仅需访问污染数据集和污染通道的黑盒，即可构建干净数据的生成模型。


<details>
  <summary>Details</summary>
Motivation: 在许多科学和工程领域，我们只能获得通过噪声、病态通道污染的测量数据，而无法获得干净数据。需要一种能够在分布层面解决逆问题的方法，为原始数据构建生成模型。

Method: 基于随机插值法，迭代更新污染数据与干净数据样本之间的传输映射。该方法仅需访问污染数据集以及对污染通道的黑盒访问，在适当条件下收敛到自洽的传输映射，从而有效反转污染通道。

Result: SCSI方法在计算效率上优于变分替代方案，具有高度灵活性，能够处理任意非线性前向模型，并享有理论保证。在自然图像处理和科学重建的逆问题上表现出优越性能。

Conclusion: 自洽随机插值法为从污染数据构建干净数据生成模型提供了一种高效、灵活且理论可靠的新方法，特别适用于科学和工程领域中只能获得污染测量数据的场景。
...
### [141] [Scaling Behavior of Discrete Diffusion Language Models](https://arxiv.org/abs/2512.10858)
*Dimitri von Rütte,Janis Fluri,Omead Pooladzandi,Bernhard Schölkopf,Thomas Hofmann,Antonio Orvieto*

Main category: cs.LG

TL;DR: 该研究探索了离散扩散语言模型（DLMs）的缩放规律，发现不同噪声类型（掩码扩散vs均匀扩散）对缩放行为有显著影响，均匀扩散在数据受限场景下表现更优。
...
Method: 通过平滑插值掩码扩散和均匀扩散来研究不同噪声类型下DLMs的缩放行为，重点关注批量大小和学习率等关键超参数。实验分析了不同噪声类型在计算约束和数据约束下的表现差异。

Result: 研究发现DLMs的缩放行为强烈依赖于噪声类型，与ALMs有显著不同。所有噪声类型在计算约束缩放下收敛到相似的损失值，但均匀扩散在计算效率训练中需要更多参数和更少数据，在数据受限场景下表现更优。研究将均匀扩散模型扩展到100亿参数，训练了10^22 FLOPs，验证了预测的缩放行为。

Conclusion: 均匀扩散模型在数据受限场景下具有优势，是离散扩散语言模型的有前景候选方案。该研究为DLMs的缩放规律提供了深入理解，并成功构建了迄今为止最大的公开均匀扩散模型。
...
### [142] [CC-GRMAS: A Multi-Agent Graph Neural System for Spatiotemporal Landslide Risk Assessment in High Mountain Asia](https://arxiv.org/abs/2510.20875)
*Mihir Panchal,Ying-Jung Chen,Surya Parkash*

Main category: cs.LG

TL;DR: CC-GRMAS框架利用卫星观测和环境信号提升滑坡预测准确性，通过预测、规划、执行三个智能体协同实现实时态势感知、响应规划和干预
...
Method: 引入CC-GRMAS框架，利用系列卫星观测和环境信号，构建三个相互关联的智能体（预测、规划、执行）协同工作，结合当地环境因素和多智能体协调操作

Result: 该方法提供了可扩展的主动解决方案，增强了滑坡预测准确性，实现实时态势感知、响应规划和干预能力

Conclusion: CC-GRMAS框架为脆弱山区地形提供了气候韧性灾害准备的可扩展主动解决方案，通过多智能体协调和本地环境因素整合提升灾害应对能力
...
### [143] [Openpi Comet: Competition Solution For 2025 BEHAVIOR Challenge](https://arxiv.org/abs/2512.10071)
...
Method: 基于π₀.₅框架，系统研究训练技术和数据效果，通过精心设计的消融实验展示预训练和后训练阶段的扩展能力。

Result: 在2025 BEHAVIOR挑战赛中获得非常接近的第二名，并显著优于其他提交方案，证明了所提出方法的有效性。

Conclusion: 总结了实用经验和设计建议，为更广泛的具身AI社区在将强大的基础模型适应复杂具身场景时提供可操作的见解。
...
TL;DR: 提出一种新的功能冗余逆运动学算法，用于六轴机械臂的五轴任务，通过任务空间分解、阻尼最小二乘法和Halley方法实现快速鲁棒解，减少关节运动


<details>
  <summary>Details</summary>
Motivation: 六轴机械臂在焊接和增材制造等应用中存在功能冗余（对称工具轴使其成为五轴任务），利用这种冗余对于实现所需工作空间和灵巧性至关重要，但现有逆运动学算法在离线规划中未充分利用

Method: 提出新颖算法解决功能冗余逆运动学问题，采用任务空间分解方法、阻尼最小二乘法和Halley方法，实现快速鲁棒解并减少关节运动

Result: 算法能快速求解最小化关节运动的运动规划，扩展复杂工具路径的可行操作空间，在工业ABB机械臂和冷喷涂枪上验证了计算工具路径的有效性

Conclusion: 功能冗余逆运动学算法能够快速解决运动规划问题，最小化关节运动，扩展复杂工具路径的可行操作空间，为工业自动化应用提供了有效的解决方案
...
TL;DR: 提出两种惯性磁SLAM系统（松散耦合与紧密耦合），使用低成本IMU、磁力计阵列和气压计，在低能见度条件下实现定位和建图，典型误差为每100米数米级别。
...
Method: 提出松散耦合和紧密耦合两种惯性磁SLAM系统，均使用低成本传感器（IMU、磁力计阵列、气压计）。松散耦合系统将局部和全局磁场模型分别用于两个状态空间模型，而紧密耦合系统将它们集成到一个状态空间模型中。

Result: 紧密耦合IM-SLAM系统在大多数场景下比松散耦合系统定位误差更低，典型误差为每100米数米级别。证明了使用低成本传感器开发完整3D IM-SLAM系统的可行性。

Conclusion: 该工作展示了使用低成本传感器开发完整3D惯性磁SLAM系统的可行性，在矿井/火灾救援等应急响应场景中具有应用潜力，特别是在低能见度条件下优于视觉系统。
...
TL;DR: 提出了一种结合上下文奖励机器的强化学习框架，用于任务导向抓取，通过将复杂任务分解为子任务并引入阶段特定上下文，显著提高了学习效率和成功率。


<details>
  <summary>Details</summary>
Motivation: 任务导向抓取通常涉及复杂的决策过程，传统方法在处理多样化物体、功能属性和抓取拓扑时面临状态-动作空间过大、学习效率低的问题。需要一种能够有效分解任务复杂度、提供明确探索指导的方法。

Method: 提出上下文奖励机器框架，将抓取任务分解为可管理的子任务。每个子任务关联阶段特定上下文，包括奖励函数、动作空间和状态抽象函数。引入转移奖励来鼓励或惩罚阶段间转移，引导模型朝向理想的阶段序列。该方法与近端策略优化算法集成。

Result: 在1000个模拟抓取任务中达到95%的成功率，涵盖多样化物体、功能属性和抓取拓扑。在学习和成功率方面均优于最先进方法。在真实机器人上，60个抓取任务中达到83.3%的成功率，涵盖六种功能属性。

Conclusion: 上下文奖励机器框架通过任务分解和阶段特定上下文显著提高了任务导向抓取的准确性、数据效率和学习效率。该方法在模拟和真实环境中都表现出优越性能，展示了推进任务导向抓取技术的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有不确定性量化方法通常需要对误差分布或大小做出强假设，或依赖未校准的不确定性估计，这些方法缺乏频率概率保证，不足以实现安全控制。同时，当前保形预测方法将机器人视为欧几里得点，但许多系统具有非欧几里得配置空间。

Method: 提出CLAPS算法，利用李群理论严格分析配置误差，将欧几里得空间的理论保证扩展到SE(2)等非欧几里得空间。通过考虑配置空间的结构，设计对称感知的非符合性评分，构建更紧凑的预测区域。

Result: 在模拟JetBot和真实MBot上的实验表明，通过考虑配置空间结构，对称感知的非符合性评分能够产生更体积高效的预测区域，比现有方法更好地表示底层不确定性。

Conclusion: CLAPS提供了一种在非欧几里得配置空间中构建具有概率保证的预测集的方法，无需对真实系统动力学、不确定性来源或近似模型质量做出强假设，适用于安全控制应用。
...
TL;DR: 本文提出了一种使用低能量激光束进行杂草清除的自主除草机器人，该机器人采用六轮设计和新型双四连杆悬挂系统提高稳定性，通过三维线性驱动机制引导激光瞄准检测到的杂草。
...
Method: 设计了一种六轮自主除草机器人，采用新型双四连杆悬挂系统提高稳定性。机器人配备三维线性驱动机制，能够精确引导低能量激光束瞄准检测到的杂草。系统包括杂草检测、激光定位和自主导航等功能模块。

Result: 现场测试显示机器人能够有效导航农田地形，克服高达15厘米的障碍物。在42.5厘米/秒的最优速度下，杂草检测率达到86.2%，每米操作时间为87秒。激光驱动机制保持最小平均位置误差1.54毫米，命中率达到97%。

Conclusion: 该自主激光除草机器人结合了速度、准确性和效率，展示了在精准农业实践中显著提升杂草管理能力的潜力，为可持续农业提供了一种有前景的技术解决方案。
...
### [149] [Design and Validation of an Under-actuated Robotic Finger with Synchronous Tendon Routing](https://arxiv.org/abs/2512.10349)
*Quan Yuan,Zhenting Du,Daqian Cao,Weibang Bai*

Main category: cs.RO

TL;DR: 提出一种欠驱动腱驱动机器人手指（UTRF），采用同步腱绳布线实现关节机械耦合，单驱动器驱动所有关节，在保持刚度和顺应性的同时减少多指手所需驱动器数量。


<details>
  <summary>Details</summary>
Motivation: 腱驱动欠驱动机器人手指在灵巧操作方面具有优势，但如何在紧凑结构中同时实现高负载能力和自适应顺应性仍然具有挑战性。需要减少多指手所需驱动器数量，同时保持结构刚度和顺应性。

Method: 设计同步腱绳布线方案，机械耦合所有关节并固定角速度比，使整个手指可由单个驱动器驱动。建立包含腱弹性的运动学和静力学模型来预测结构刚度。制作单指原型并进行静态负载测试。

Result: 单指原型测试显示平均偏转预测误差为1.0mm（总指长的0.322%），在3kg指尖负载下测得刚度为1.2×10^3 N/m。集成到五指机器人手（UTRF-RoboHand）中，在不同场景下展示了有效的物体操作能力。

Conclusion: 提出的同步腱绳布线方案实现了可预测的刚度和可靠的抓取性能，同时最小化了驱动器数量，验证了该设计在保持紧凑结构和良好性能方面的有效性。
...
### [150] [RoboNeuron: A Modular Framework Linking Foundation Models and ROS for Embodied AI](https://arxiv.org/abs/2512.10394)
*Weifan Guan,Huasen Xi,Chenxiao Zhang,Aosheng Li,Qinghao Hu,Jian Cheng*

Main category: cs.RO

TL;DR: RoboNeuron是一个面向具身智能的通用部署框架，通过深度整合大语言模型、视觉语言动作模型与ROS系统，解决现有系统跨场景适应性差、模块耦合度高、推理加速碎片化等问题。


<details>
  <summary>Details</summary>
Motivation: 当前具身AI系统面临三大工程障碍：跨场景适应性差、模块间耦合度过高、推理加速碎片化。这些问题严重限制了具身智能系统的实际部署和扩展能力。

Method: 1. 深度整合LLM/VLA模型的认知能力与ROS实时执行框架；2. 使用模型上下文协议作为语义桥梁，让LLM动态编排底层机器人工具；3. 建立高度模块化架构，通过ROS统一通信接口严格解耦感知、推理和控制；4. 引入自动化工具将ROS消息转换为可调用的MCP函数。

Result: RoboNeuron显著提升了跨场景适应性和组件灵活性，同时建立了系统化的水平性能基准测试平台，为可扩展的真实世界具身应用奠定了坚实基础。

Conclusion: RoboNeuron是首个深度整合LLM/VLA认知能力与ROS实时执行框架的具身智能部署框架，通过MCP语义桥梁和模块化架构解决了现有系统的关键工程障碍，为具身智能的规模化应用提供了系统化解决方案。
...
### [151] [Symphony: A Heuristic Normalized Calibrated Advantage Actor and Critic Algorithm in application for Humanoid Robots](https://arxiv.org/abs/2512.10477)
*Timur Ishuov,Michele Folgheraiter,Madi Nurmanov,Goncalo Gordo,Richárd Farkas,József Dombi*

Main category: cs.RO

TL;DR: 提出Symphony算法，通过"襁褓"正则化限制动作强度、使用确定性策略和有限参数噪声，实现人形机器人从零开始的安全高效训练。


<details>
  <summary>Details</summary>
Motivation: 人类学习需要时间，但机器人训练不能等待数百万步。现有方法中高斯噪声对电机和齿轮箱有害，需要更安全高效的训练方法。

Method: 1. "襁褓"正则化：限制动作强度但不直接影响动作；2. 确定性策略和有限参数噪声；3. Fading Replay Buffer：结合近期和长期记忆；4. Temporal Advantage：单次更新Actor和Critic。

Result: 训练过程对环境和机器人机制更安全，实现了样本效率、样本邻近性和动作安全性，能够从零开始训练人形机器人。

Conclusion: Symphony算法通过创新的正则化、噪声控制和记忆管理方法，解决了机器人训练中的安全性和效率问题。
...
### [152] [Seamless Outdoor-Indoor Pedestrian Positioning System with GNSS/UWB/IMU Fusion: A Comparison of EKF, FGO, and PF](https://arxiv.org/abs/2512.10480)
*Jiaqiang Zhang,Xianjia Yu,Sier Ha,Paola Torrico Moron,Sahar Salimpour,Farhad Kerama,Haizhou Zhang,Tomi Westerlund*

Main category: cs.RO

TL;DR: 本文提出了一个统一的GNSS/UWB/IMU融合框架，用于实现无缝的行人定位，并比较了三种概率后端方法：误差状态扩展卡尔曼滤波、滑动窗口因子图优化和粒子滤波。
...
Method: 系统使用胸部安装的IMU-based PDR作为运动骨干，在室外集成GNSS绝对更新，在室内集成UWB绝对更新。为了增强过渡鲁棒性并减轻城市GNSS退化，引入了基于OpenStreetMap建筑足迹的轻量级地图可行性约束，将大多数建筑内部视为不可导航区域，同时允许在指定的UWB仪器建筑内运动。

Result: 评估了三种场景：室内（UWB+PDR）、室外（GNSS+PDR）和无缝室外-室内（GNSS+UWB+PDR）。结果表明，在作者的实现中，误差状态扩展卡尔曼滤波（ESKF）提供了最一致的整体性能。

Conclusion: 本文提出的统一融合框架能够实现无缝的行人定位，通过比较三种概率后端方法，发现ESKF在作者的实现中表现最为稳定可靠。
...
### [153] [Contact SLAM: An Active Tactile Exploration Policy Based on Physical Reasoning Utilized in Robotic Fine Blind Manipulation Tasks](https://arxiv.org/abs/2512.10481)
*Gaozhao Wang,Xing Liu,Zhenduo Ye,Zhengxiong Liu,Panfeng Huang*

Main category: cs.RO

TL;DR: 提出Contact SLAM方法，仅使用触觉传感和场景先验知识实现视觉遮挡下的接触式操作，通过主动探索策略提高效率


<details>
  <summary>Details</summary>
Motivation: 在视觉被遮挡的场景中，机器人无法通过视觉反馈获取实时场景状态信息，这被称为"盲操作"。接触丰富的操作对机器人执行困难，需要准确的环境感知。

Method: 提出一种新颖的物理驱动接触认知方法"Contact SLAM"，仅使用触觉传感和场景先验知识来估计环境状态并实现操作。同时设计了主动探索策略，逐步减少操作场景中的不确定性，最大化探索效率。

Result: 实验结果表明，所提方法在多个接触丰富的任务中表现出有效性和准确性，包括困难且精细的插座装配任务和方块推动任务。

Conclusion: Contact SLAM方法能够有效解决视觉遮挡下的盲操作问题，通过触觉传感和主动探索策略实现了准确的接触式操作。
...
### [154] [LEO-RobotAgent: A General-purpose Robotic Agent for Language-driven Embodied Operator](https://arxiv.org/abs/2512.10605)
*Lihuang Chen,Xiangyu Luo,Jun Meng*

Main category: cs.RO

TL;DR: LEO-RobotAgent是一个通用的语言驱动智能体框架，能让大语言模型操作不同类型的机器人完成跨场景的复杂任务，具有模块化工具集和人机交互机制。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多关注大模型在单任务场景和单一机器人类型中的应用，算法结构复杂且缺乏通用性，需要一种能增强人机双向意图理解、降低交互门槛的通用框架。

Method: 设计了一个结构简洁的框架，让大模型能在其中独立思考、规划和行动；提供了模块化、易注册的工具集，使大模型能灵活调用各种工具；集成了人机交互机制，实现人机协作。

Result: 该框架能轻松适配主流机器人平台（无人机、机械臂、轮式机器人），并能高效执行不同复杂度的任务，验证了框架的通用性和有效性。

Conclusion: LEO-RobotAgent是一个具有强泛化性、鲁棒性和效率的通用语言驱动智能体框架，能显著提升人机交互能力，降低使用门槛。
...
### [155] [Evaluating Gemini Robotics Policies in a Veo World Simulator](https://arxiv.org/abs/2512.10675)
...
TL;DR: 视频生成模型可用于机器人策略的全方位评估，包括正常性能、分布外泛化、物理和语义安全性测试，通过编辑真实场景生成多样化仿真环境。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型在机器人领域的应用主要局限于分布内评估，缺乏对策略在分布外场景、安全约束等全方位评估的能力，需要更全面的评估系统。

Method: 基于前沿视频基础模型Veo构建生成式评估系统，优化支持机器人动作条件化和多视角一致性，集成生成式图像编辑和多视角补全技术，合成真实场景的多种变体。

Result: 系统保持视频模型的基础能力，能准确模拟包含新交互对象、新视觉背景和新干扰对象的场景，可预测不同策略在正常和分布外条件下的相对性能，识别泛化轴对性能的影响，并进行红队测试暴露安全违规行为。

Conclusion: 视频模型可用于机器人策略的全方位评估，包括性能评估、泛化能力测试和安全性验证，通过1600+真实世界评估验证了系统在多种任务中的有效性。
...
### [156] [How to Brake? Ethical Emergency Braking with Deep Reinforcement Learning](https://arxiv.org/abs/2512.10698)
*Jianbo Wang,Galina Sidorenko,Johan Thunberg*

Main category: cs.RO

TL;DR: 该研究提出了一种结合深度强化学习与解析方法的混合方法，用于多车跟驰场景中的紧急制动决策，旨在提高整体安全性而非单车安全。


<details>
  <summary>Details</summary>
Motivation: 联网自动驾驶车辆虽然能提升驾驶安全，但传统的保守控制策略会降低灵活性并影响整体性能。需要在多车跟驰紧急制动场景中，通过车辆间通信实现整体伤害最小化而非仅保护单车。

Method: 提出混合方法：结合深度强化学习与基于解析表达式的最优恒定减速度选择方法。DRL用于学习紧急制动策略，而解析方法提供可靠性保障。

Result: 混合方法相比单独使用DRL提高了可靠性，同时在整体伤害减少和碰撞避免方面表现出更优性能。

Conclusion: 通过结合DRL与解析方法，可以在多车跟驰紧急制动场景中实现更可靠且性能更优的安全决策，平衡了安全性与灵活性需求。
...
### [157] [On the Stabilization of Rigid Formations on Regular Curves](https://arxiv.org/abs/2512.10700)
...
Method: 1. 使用随机多起点牛顿类算法寻找曲线上的内接正多边形；2. 设计连续反馈控制律，保证曲线扫描和收敛到期望编队顶点；3. 确保智能体间避碰

Result: 方法在不同类型曲线和不同刚性编队上通过数值模拟验证有效，代码已开源

Conclusion: 提出的方法能够成功实现多智能体在平面曲线上形成稳定等边多边形编队，具有实际应用价值
...
### [158] [Iterative Compositional Data Generation for Robot Control](https://arxiv.org/abs/2512.10891)
...
<details>
  <summary>Details</summary>
Motivation: 机器人操作数据收集成本高昂，难以覆盖多物体、多机器人、多环境的组合任务空间。现有生成模型无法利用机器人领域的组合结构，难以泛化到未见任务组合

Method: 提出语义组合扩散Transformer，将状态转移分解为机器人、物体、障碍物和目标特定组件，通过注意力机制学习组件间交互。训练后能零样本生成高质量转移数据，并引入迭代自改进过程，通过离线强化学习验证合成数据并纳入后续训练

Result: 方法显著优于单体和硬编码组合基线，在零样本性能上有大幅提升，最终解决了几乎所有保留任务，并在学习表示中展现出有意义的组合结构

Conclusion: 语义组合扩散Transformer能够有效利用机器人领域的组合结构，通过合成数据生成和迭代自改进，实现未见任务组合的零样本泛化，为解决组合性机器人学习问题提供了有效方案
...
### [159] [Curriculum-Based Reinforcement Learning for Autonomous UAV Navigation in Unknown Curved Tubular Conduit](https://arxiv.org/abs/2512.10934)
*Zamirddine Mari,Jérôme Pasquet,Julien Seinturier*

Main category: cs.RO

TL;DR: 本文提出了一种基于强化学习的无人机自主导航方法，用于在未知三维管道环境中飞行，仅依赖局部LiDAR观测和条件视觉检测，无需管道几何先验知识。


<details>
  <summary>Details</summary>
Motivation: 在受限的管状环境中，无人机自主导航面临重大挑战，包括管道几何约束、墙壁接近以及感知限制。现有方法通常需要管道的几何模型，这在未知环境中难以获得。

Method: 采用强化学习（PPO算法）结合渐进式课程学习策略，逐步让智能体适应越来越弯曲的管道几何。使用LiDAR局部观测和条件视觉检测管道中心，并开发了基于直接可见性、方向记忆和LiDAR对称性线索的转向协商机制来处理部分可观测性。

Result: PPO策略获得了鲁棒且可泛化的行为，在缺乏几何模型的情况下，其性能始终优于需要明确中心线信息的确定性控制器（Pure Pursuit算法）。在高保真3D环境中的验证进一步证实了学习行为向连续物理动力学的可迁移性。

Conclusion: 该方法为未知管状环境中的自主导航提供了完整框架，为工业、地下或医疗应用开辟了前景，在这些应用中，通过狭窄且感知能力弱的管道前进是核心挑战。
...
### [160] [ImplicitRDP: An End-to-End Visual-Force Diffusion Policy with Structural Slow-Fast Learning](https://arxiv.org/abs/2512.10946)
*Wendi Chen,Han Xue,Yi Wang,Fangyuan Zhou,Jun Lv,Yang Jin,Shirun Tang,Chuan Wen,Cewu Lu*

Main category: cs.RO

TL;DR: ImplicitRDP：一种统一的端到端视觉-力扩散策略，通过结构慢-快学习机制整合视觉规划和反应式力控制，在接触丰富的操作任务中显著优于纯视觉和分层基线方法。


<details>
  <summary>Details</summary>
Motivation: 人类水平的接触丰富操作依赖于视觉和力感知两种模态的协同作用：视觉提供空间丰富但时间缓慢的全局上下文，而力感知捕捉快速、高频的局部接触动态。由于这两种信号在频率和信息特性上的根本差异，有效整合它们具有挑战性。

Method: 提出ImplicitRDP统一端到端视觉-力扩散策略，采用结构慢-快学习机制，利用因果注意力同时处理异步的视觉和力令牌，使策略能够在力频率下进行闭环调整，同时保持动作块的时间一致性。此外，提出基于虚拟目标的表示正则化，将力反馈映射到与动作相同的空间，提供比原始力预测更强的物理基础学习信号。

Result: 在接触丰富的任务上进行广泛实验，ImplicitRDP显著优于纯视觉和分层基线方法，实现了更高的反应性和成功率，同时简化了训练流程。

Conclusion: ImplicitRDP成功整合了视觉规划和反应式力控制，通过结构慢-快学习和表示正则化解决了多模态整合中的频率差异和模态塌陷问题，为接触丰富的操作任务提供了高效统一的解决方案。
...
### [161] [ExaCraft: Dynamic Learning Context Adaptation for Personalized Educational Examples](https://arxiv.org/abs/2512.09931)
*Akaash Chatterjee,Suman Kundu*

Main category: cs.AI

TL;DR: ExaCraft是一个AI驱动的个性化学习示例生成系统，通过分析学习者的动态上下文（包括个人资料、行为模式和技能发展）来创建文化相关且适应个体需求的教学示例。


<details>
  <summary>Details</summary>
Motivation: 现有教育AI工具缺乏生成个性化示例的能力，无法适应学习者不断变化的理解水平、学习困难和技能发展，导致学习效果受限。

Method: 基于Google Gemini AI和Python Flask API构建的Chrome扩展系统，结合用户定义的个人资料（位置、教育、职业、复杂度偏好）和实时行为分析，动态调整示例生成以适应五个关键学习上下文方面：困难指标、掌握模式、主题进展历史、会话边界和学习进展信号。

Result: 系统能够生成从基础概念到高级技术实现的渐进式示例，响应主题重复、重新生成请求和主题进展模式，在不同使用场景中展示适应性。

Conclusion: ExaCraft通过动态适应学习者的上下文变化，提供个性化、文化相关的示例，解决了现有教育AI工具在个性化示例生成方面的不足，提升了学习效果。
...
### [162] [Linear socio-demographic representations emerge in Large Language Models from indirect cues](https://arxiv.org/abs/2512.10065)
*Paul Bouchaud,Pedro Ramaciotti*

Main category: cs.AI

TL;DR: 研究发现LLMs通过姓名和职业等间接线索编码用户的社会人口属性，形成线性可解释的几何表示，这些隐性表征会影响下游行为如职业推荐，即使通过偏见测试的模型仍可能隐含偏见。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究LLMs如何通过姓名、职业等间接线索推断人类对话伙伴的社会人口属性（如性别、种族），以及这些隐性表征如何影响模型行为，这对大规模应用中的公平性有重要影响。

Method: 方法包括：1）在四个开源Transformer模型（Magistral 24B, Qwen3 14B, GPT-OSS 20B, OLMo2-1B）的残差流中探测社会人口属性表征；2）使用显性人口属性披露提示；3）通过姓名激活与人口普查对齐的性别和种族表征；4）通过职业触发与现实世界劳动力统计数据相关的表征；5）分析这些线性表征如何影响下游行为如职业推荐。

Result: 结果显示：1）LLMs在激活空间中发展出用户社会人口属性的线性表示，刻板印象相关属性沿可解释的几何方向编码；2）相同的探测方法可以从隐性线索预测人口属性；3）姓名激活与人口普查对齐的性别和种族表征；4）职业触发与现实世界劳动力统计数据相关的表征；5）这些隐性人口表征主动塑造下游行为，如职业推荐；6）即使通过偏见基准测试的模型仍可能隐含并利用隐性偏见。

Conclusion: 结论是LLMs通过间接线索形成社会人口属性的隐性线性表征，这些表征可解释且影响模型行为，即使通过偏见测试的模型仍可能隐含偏见，这对大规模应用中的公平性有重要启示。
...
### [163] [Agile Deliberation: Concept Deliberation for Subjective Visual Classification](https://arxiv.org/abs/2512.10821)
...
TL;DR: 提出Agile Deliberation框架，通过概念界定和迭代两个阶段，支持用户在模糊概念下与系统协作训练视觉分类器，相比基线方法提升分类性能并降低用户认知负担。


<details>
  <summary>Details</summary>
Motivation: 现有的人机协同方法假设用户对概念有清晰稳定的理解，但现实中用户往往从模糊概念开始，需要通过"概念审议"过程逐步细化。内容审核专家的实践表明，概念审议是实际工作中的关键环节。
...
Conclusion: Agile Deliberation有效支持主观和演化概念的训练，通过结构化审议过程帮助用户从模糊概念开始，逐步明确意图并训练出符合需求的分类器，在人机协同视觉概念学习中具有实用价值。
...
### [164] [Exploring Health Misinformation Detection with Multi-Agent Debate](https://arxiv.org/abs/2512.09935)
*Chih-Han Chen,Chen-Han Tsai,Yu-Shao Peng*

Main category: cs.AI

TL;DR: 提出一个两阶段健康信息检测框架：先预测一致性分数，再通过多智能体辩论处理分歧证据


<details>
  <summary>Details</summary>
Motivation: 随着健康相关虚假信息在线传播日益严重，需要结合高质量证据检索和严格推理过程进行有效验证

Method: 两阶段框架：第一阶段使用大语言模型独立评估检索文章并计算聚合一致性分数；当分数低于阈值时进入第二阶段，多个智能体进行结构化辩论以综合冲突证据

Result: 实验结果表明，与基线方法相比，该两阶段方法取得了更优的性能
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在科学文献信息提取中存在局限性：处理长文档困难、多模态内容整合问题、跨多篇文献的细粒度信息标准化困难，以及数据模式快速变化时系统难以重新架构。

Method: 提出SciEx框架，采用模块化和可组合设计，解耦PDF解析、多模态检索、信息提取和聚合等关键组件，支持新模型、提示策略和推理机制的灵活集成。

Result: 在三个科学主题的数据集上评估SciEx提取细粒度信息的准确性和一致性，提供了关于当前LLM基流水线优势和局限性的实用见解。

Conclusion: SciEx框架通过模块化设计有效解决了科学文献信息提取中的关键挑战，为LLM在科学信息提取领域的应用提供了可扩展和灵活的解决方案。
...
### [166] [SimWorld-Robotics: Synthesizing Photorealistic and Dynamic Urban Environments for Multimodal Robot Navigation and Collaboration](https://arxiv.org/abs/2512.10046)
...
<details>
  <summary>Details</summary>
Motivation: 当前基础模型在机器人领域的研究主要集中在室内家庭场景，缺乏对大规模城市环境中机器人能力的全面评估。需要创建一个逼真的城市仿真平台来测试机器人在复杂城市环境中的关键能力。

Method: 基于虚幻引擎5构建SWR仿真平台，通过程序化生成无限逼真的城市场景，包含动态元素如行人和交通系统。平台支持多机器人控制和通信，并在此基础上构建了两个基准测试：多模态指令跟随任务和多智能体搜索任务。

Result: 实验结果表明，当前最先进的模型（包括视觉语言模型）在处理这些任务时表现不佳，缺乏在复杂城市环境中所需的稳健感知、推理和规划能力。

Conclusion: SWR平台填补了城市环境机器人仿真评估的空白，提出的两个基准测试能够全面评估机器人在真实城市场景中的关键能力，为开发更强大的通用机器人系统提供了重要测试平台。
...
### [167] [Parallel Decoder Transformer: Model-Internal Parallel Decoding with Speculative Invariance via Note Conditioning](https://arxiv.org/abs/2512.10054)
*Logan Robbins*

Main category: cs.AI

TL;DR: PDT是一种参数高效的并行解码架构，通过注入轻量级适配器让冻结的预训练模型实现并行解码，解决了传统方法中的一致性漂移问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的自回归解码本质上是顺序的，导致延迟随输出长度线性增长。现有的"分解-填充"方法虽然尝试并行化生成，但缺乏跨流通信导致一致性漂移问题。

Method: 提出并行解码变换器(PDT)，通过注入轻量级的推测性笔记调节(SNC)适配器，让并行解码流通过共享的动态潜在空间进行同步。将协调问题形式化为推测性共识问题，子流将语义"笔记"广播到全局总线，由学习到的验证头门控。

Result: 在冻结的200亿参数骨干模型上使用5万步课程验证，PDT实现了有效的自校正，覆盖预测精度达到77.8%，无需修改主干权重即可恢复近似串行语义。

Conclusion: PDT为结构化并行生成提供了一种可扩展、高效的替代方案，无需对完整模型进行微调，通过参数高效的方式解决了并行解码中的协调问题。
...
### [168] [Interpretable Embeddings with Sparse Autoencoders: A Data Analysis Toolkit](https://arxiv.org/abs/2512.10092)
*Nick Jiang,Xiaoqing Sun,Lisa Dunlap,Lewis Smith,Neel Nanda*

Main category: cs.AI

TL;DR: 稀疏自编码器（SAE）嵌入作为文本分析工具，比LLM更经济可靠，比密集嵌入更可控，能发现数据集语义差异和概念相关性
...
Result: SAE嵌入比LLM成本低2-8倍且更可靠，比密集嵌入更可控；能发现数据集语义差异、概念相关性，并在聚类和检索任务中表现更好

Conclusion: SAE是强大的非结构化数据分析工具，强调通过数据解释模型的重要性，为模型行为分析和偏见检测提供了新方法
...
### [169] [Robust AI Security and Alignment: A Sisyphean Endeavor?](https://arxiv.org/abs/2512.10100)
*Apostol Vassilev*
...
<details>
  <summary>Details</summary>
Motivation: 认识到AI技术负责任采用的重要性，需要了解AI安全和对齐鲁棒性的基本理论限制，并为这些挑战做好准备。

Method: 通过将哥德尔不完备性定理扩展到人工智能领域，建立信息论框架来分析AI安全和对齐的鲁棒性限制。

Result: 证明了AI系统在安全和对齐方面存在固有的信息论限制，这些限制源于哥德尔不完备性定理在AI领域的扩展。

Conclusion: AI系统存在认知推理的基本限制，了解这些限制对于负责任地采用AI技术至关重要，论文提供了应对这些挑战的实用方法。
...
### [170] [AgriRegion: Region-Aware Retrieval for High-Fidelity Agricultural Advice](https://arxiv.org/abs/2512.10114)
...
<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在农业领域经常出现上下文幻觉问题，提供的建议可能在一个地区科学合理但在另一个地区造成灾难性后果，这是由于土壤、气候和地方法规的差异造成的。需要专门针对农业领域的高保真、区域感知的咨询系统。
...
Conclusion: AgriRegion框架通过结合地理空间约束和区域优先检索，有效解决了农业领域LLM的上下文幻觉问题，为提供准确、可靠的区域特定农业建议提供了有效解决方案。
...
### [171] [The 2025 Foundation Model Transparency Index](https://arxiv.org/abs/2512.10169)
*Alexander Wan,Kevin Klyman,Sayash Kapoor,Nestor Maslej,Shayne Longpre,Betty Xiong,Percy Liang,Rishi Bommasani*

Main category: cs.AI

TL;DR: 2025年基础模型透明度指数显示，主要AI公司透明度显著下降，平均分从58降至40，数据获取、使用和监控方面最不透明，IBM表现最佳，xAI和Midjourney最差。
...
Method: 采用年度基础模型透明度指数评估方法，引入数据获取、使用数据和监控等新指标，首次评估阿里巴巴、DeepSeek和xAI等公司，对主要基础模型开发者进行量化评分。

Result: 透明度整体恶化：平均分从2024年的58分降至2025年的40分；公司在训练数据、训练计算以及部署后使用和影响方面最不透明；IBM得分95表现最佳，xAI和Midjourney仅得14分最差；前沿模型论坛成员处于中间位置。

Conclusion: 基础模型开发公司透明度普遍下降，需要更积极的政策干预来弥补关键信息缺口，特别是在数据实践和模型影响方面，政策制定者应加强透明度要求。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全离线强化学习方法通常只强制执行软期望成本约束，无法保证前向不变性；而控制屏障函数（CBFs）虽然提供严格安全保证，但通常依赖专家设计的屏障函数或完整的系统动力学知识。需要一种既能从离线数据学习安全屏障，又不需要在线交互或手工设计的方法。

Method: 提出V-OCBF框架：1）从离线演示中学习神经CBF，不假设访问动力学模型；2）推导递归有限差分屏障更新，实现模型无关的屏障学习；3）采用期望分位数目标，避免在分布外动作上查询屏障，并将更新限制在数据集支持的动作集内；4）使用学习到的屏障与二次规划（QP）公式合成实时安全控制。

Result: 在多个案例研究中，V-OCBF相比基线方法显著减少了安全违规次数，同时保持了强大的任务性能，展示了其在无需在线交互或手工设计屏障的情况下合成安全关键控制器的可扩展性。

Conclusion: V-OCBF提供了一种有效的框架，能够从离线演示中学习控制屏障函数，实现严格的安全保证，无需系统动力学模型或专家设计的屏障函数，为离线合成安全关键控制器提供了可扩展的解决方案。
...
### [173] [An exploration for higher efficiency in multi objective optimisation with reinforcement learning](https://arxiv.org/abs/2512.10208)
*Mehmet Emin Aydin*

Main category: cs.AI

TL;DR: 本文提出了一种基于多目标强化学习的广义方法，用于优化搜索过程中算子序列的选择问题，旨在提高多目标优化算法的效率。


<details>
  <summary>Details</summary>
Motivation: 优化和搜索过程中的效率问题一直是影响算法性能的关键挑战。虽然使用算子池而非单一算子来处理邻域内的移动操作具有潜力，但寻找最优或接近最优的算子序列仍需深入研究。现有研究主要集中在单目标优化，而多目标优化在这方面尚未得到充分探索。

Method: 提出了一种基于多目标强化学习的广义方法，该方法包含已完成和待完成的多个阶段，旨在通过强化学习技术来选择和优化算子序列，以提升多目标优化算法的性能。

Result: 论文概述了提出的广义方法，其中某些阶段已完成，某些阶段仍在进行中。该方法旨在展示使用多目标强化学习在优化效率方面的潜力。

Conclusion: 基于多目标强化学习的广义方法为解决多目标优化中算子序列选择问题提供了有前景的解决方案，有望显著提高优化算法的效率和性能。
...
### [174] [ID-PaS : Identity-Aware Predict-and-Search for General Mixed-Integer Linear Programs](https://arxiv.org/abs/2512.10211)
*Junyang Cai,El Mehdi Er Raqabi,Pascal Van Hentenryck,Bistra Dilkina*

Main category: cs.AI

TL;DR: 将预测-搜索框架扩展到参数化混合整数规划问题，提出身份感知学习框架ID-PaS，显著提升大规模实际问题的求解性能


<details>
  <summary>Details</summary>
Motivation: 现有预测-搜索方法局限于二元问题，且忽略了实际应用中常见的固定变量问题，需要扩展框架以处理更复杂的参数化混合整数规划问题

Method: 提出ID-PaS（身份感知学习框架），扩展预测-搜索框架到参数化MIPs，使机器学习模型能更有效地处理异构变量

Result: 在多个实际大规模问题上，ID-PaS相比最先进的Gurobi求解器和传统PaS方法都取得了更优越的性能

Conclusion: ID-PaS框架成功将预测-搜索方法扩展到参数化混合整数规划问题，通过身份感知学习有效处理异构变量，在实际应用中展现出显著优势
...
### [175] [Reverse Thinking Enhances Missing Information Detection in Large Language Models](https://arxiv.org/abs/2512.10273)
*Yuxin Liu,Chaojie Gu,Yihang Zhang,Bin Qian,Shibo He*

Main category: cs.AI

TL;DR: 论文提出了一种基于逆向思维的框架，用于提升大语言模型在缺失信息检测任务中的表现，相比传统前向推理方法显著提高了准确性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理缺失信息问题时存在不足，经常出现回答不完整、事实错误和幻觉等问题。虽然前向推理方法在结构化问题解决中表现良好，但无法系统性地识别和恢复被省略的信息。

Method: 提出了一种新颖的逆向思维框架，引导大语言模型通过逆向思考来识别必要条件和定位缺失元素。该方法将具有挑战性的缺失信息识别任务转化为更易管理的逆向推理问题。

Result: 实验结果表明，逆向思维方法相比传统前向推理方法取得了显著的性能提升，为大语言模型的逻辑完整性和推理鲁棒性提供了有前景的方向。

Conclusion: 逆向思维方法能够有效增强大语言模型在缺失信息检测任务中的表现，为解决模型在信息不完整情况下的推理问题提供了新的解决方案。
...
### [176] [Neuronal Attention Circuit (NAC) for Representation Learning](https://arxiv.org/abs/2512.10282)
*Waleed Razzaq,Izis Kankaraway,Yun-Bo Zhao*

Main category: cs.AI

TL;DR: 本文提出Neuronal Attention Circuit (NAC)，一种生物启发的连续时间注意力机制，通过线性一阶ODE和稀疏门控结构实现高效的自适应动态，在多个领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统注意力机制虽然改进了RNN的表示学习，但其离散特性限制了连续时间建模能力。需要一种生物合理且能有效处理连续时间数据的注意力机制。

Method: NAC将注意力对数计算重新表述为线性一阶ODE的解，使用源自秀丽隐杆线虫神经元电路策略的稀疏门控结构，包含稀疏感觉门、内容-目标门和可学习时间常数门，支持三种计算模式，并采用稀疏Top-K配对连接方案提高内存效率。

Result: NAC在多个领域（不规则时间序列分类、自动驾驶车道保持、工业预测）的实验中，在准确性上匹配或优于基线方法，在运行时间和内存效率上处于中等水平，同时提供了状态稳定性、有界近似误差和通用逼近等理论保证。

Conclusion: NAC是一种有效的生物启发连续时间注意力机制，通过ODE框架和稀疏门控结构实现了高效的自适应动态，为连续时间建模提供了新的解决方案。
...
### [177] [Planning, Living and Judging: A Multi-agent LLM-based Framework for Cyclical Urban Planning](https://arxiv.org/abs/2412.20505)
*Hang Ni,Yuzhi Wang,Hao Liu*

Main category: cs.AI

TL;DR: 本文提出了一种名为CUP的循环城市规划新范式，利用大语言模型在多智能体框架中实现城市计划的持续生成、评估和优化。


<details>
  <summary>Details</summary>
Motivation: 城市化背景下的城市更新面临重大挑战，需要适应性的方法来应对不断变化的需求。传统城市规划方法往往缺乏动态响应和持续优化的能力。

Method: 提出基于大语言模型的多智能体框架CUP，包含三个核心组件：1)规划组件：LLM智能体基于上下文数据生成和优化城市计划；2)生活组件：智能体模拟居民行为和互动，建模城市环境中的生活；3)评估组件：评估计划效果并提供迭代反馈。这三个组件形成闭环的循环过程。

Result: 在真实世界数据集上的实验证明了该框架作为连续自适应规划过程的有效性。

Conclusion: CUP框架提供了一种动态响应式的城市规划方法，能够通过持续的生成-评估-优化循环来适应城市发展的变化需求。
...
### [178] [Investigating The Functional Roles of Attention Heads in Vision Language Models: Evidence for Reasoning Modules](https://arxiv.org/abs/2512.10300)
*Yanbei Jiang,Xueqi Ma,Shu Liu,Sarah Monazam Erfani,Tongliang Liu,James Bailey,Jey Han Lau,Krista A. Ehinger*

Main category: cs.AI

TL;DR: 本文提出了一个解释视觉语言模型内部机制的新框架，通过注意力头功能分析揭示多模态推理的认知组织


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在多模态基准测试中表现出色，但其内部机制仍然是一个黑箱，需要系统性的解释性分析来理解其多模态推理过程

Method: 提出CogVision数据集，将复杂多模态问题分解为逐步子问题，模拟人类推理的思维链范式；采用基于探测的方法识别专门处理特定接收或认知功能的注意力头，并将其表征为功能头

Result: 分析发现功能头普遍稀疏，在不同功能间数量和分布各异，并介导交互和层次组织；干预实验表明功能头在多模态推理中起关键作用：移除功能头导致性能下降，强调功能头则提高准确性

Conclusion: 这些发现为视觉语言模型的认知组织提供了新见解，并为设计具有更符合人类感知和推理能力的模型指明了有前景的方向
...
### [179] [Trustworthy Orchestration Artificial Intelligence by the Ten Criteria with Control-Plane Governance](https://arxiv.org/abs/2512.10304)
...
Method: 提出了可信编排AI的十大标准框架，采用控制面板架构，整合人类输入、语义连贯性、审计和溯源完整性。该框架为整个AI组件、消费者和人类参与者提供治理保护伞，借鉴国际标准和澳大利亚国家AI保障框架。

Result: 通过工程化方法将可信度系统地纳入AI系统，确保执行层保持可验证、透明、可重现并受有意义的人类控制。

Conclusion: 可信编排AI框架为AI生态系统提供了全面的保障机制，通过架构设计将治理嵌入执行层，解决了技术能力与制度问责之间的差距问题。
...
### [180] [EpiPlanAgent: Agentic Automated Epidemic Response Planning](https://arxiv.org/abs/2512.10313)
*Kangkun Mao,Fang Xu,Jinru Ding,Yidong Jiang,Yujun Yao,Yirong Chen,Junming Liu,Xiaoqin Wu,Qian Wu,Xiaoyan Huang,Jie Xu*

Main category: cs.AI

TL;DR: EpiPlanAgent是一个基于大语言模型的智能体系统，能够自动生成和验证数字应急响应计划，显著提高计划完整性、指南一致性并大幅缩短开发时间。


<details>
  <summary>Details</summary>
Motivation: 传统的流行病应急响应规划依赖劳动密集型的人工方法，效率低下且难以保证质量。本研究旨在开发一个自动化系统来改进这一过程。

Method: 设计了EpiPlanAgent系统，采用基于智能体的多智能体框架，集成了任务分解、知识基础和模拟模块。公共卫生专业人员使用真实世界疫情场景在受控环境中测试系统。

Result: EpiPlanAgent显著提高了计划的完整性和指南一致性，同时大幅缩短了开发时间。专家评估确认AI生成内容与人工撰写内容高度一致，用户反馈显示系统具有高感知效用。
...
### [181] [User-Feedback-Driven Continual Adaptation for Vision-and-Language Navigation](https://arxiv.org/abs/2512.10322)
*Yongqiang Yu,Xuhui Li,Hazza Mahmood,Jinxing Zhou,Haodong Hong,Longtao Jiang,Zhiqiang Xu,Qi Wu,Xiaojun Chang*

Main category: cs.AI

TL;DR: 本文提出了一种用户反馈驱动的视觉语言导航适应框架，通过整合人类交互来增强GSA-VLN系统的持续学习能力，显著提升了导航成功率和路径效率。


<details>
  <summary>Details</summary>
Motivation: 当前GSA-VLN框架仅依赖无监督的环境暴露适应，忽略了用户反馈这一自然且有价值的监督信号。为了缩小静态基准测试与现实部署之间的差距，需要将用户反馈系统性地整合到持续学习中。

Method: 1. 将用户反馈（导航指令和纠正信号）转换为高质量、环境对齐的训练数据；2. 引入记忆库热启动机制，重用先前获取的环境知识；3. 在GSA-R2R基准上进行实验验证。

Result: 在GSA-R2R基准测试中，该方法持续超越GR-DUET等强基线，提高了导航成功率和路径效率。记忆库热启动机制稳定了早期导航并减少了更新后的性能下降。在持续和混合适应设置下均表现出鲁棒性和通用性。

Conclusion: 用户反馈驱动的适应框架有效增强了GSA-VLN系统的持续学习能力，通过整合人类交互和监督信号，实现了更高效、更稳定的现实世界部署。
...
### [182] [On the Collapse of Generative Paths: A Criterion and Correction for Diffusion Steering](https://arxiv.org/abs/2512.10339)
...
<details>
  <summary>Details</summary>
Motivation: 现有的概率密度比方法在组合异构模型时存在边际路径崩溃问题，导致中间密度不可归一化，这在分子设计等需要组合多个专家模型的场景中尤为突出

Method: 提出ACE方法：1) 推导路径存在性判据，仅从噪声调度和指数即可预测崩溃；2) 引入自适应路径校正，将Feynman-Kac引导扩展到时变指数，保证有效概率路径
...
### [183] [REMISVFU: Vertical Federated Unlearning via Representation Misdirection for Intermediate Output Feature](https://arxiv.org/abs/2512.10348)
*Wenhan Wu,Zhili He,Huanghuang Liang,Yili Gong,Jiawei Jiang,Chuang Hu,Dazhao Cheng*

Main category: cs.AI

TL;DR: REMISVFU是一个用于纵向联邦学习的表示误导框架，通过将遗忘方的编码器输出坍缩到单位球面上的随机锚点来实现快速客户端级遗忘，同时通过梯度正交投影保持剩余方的模型效用。


<details>
  <summary>Details</summary>
Motivation: GDPR等数据保护法规赋予联邦系统参与者被遗忘权。现有遗忘技术主要针对横向联邦学习（HFL），而纵向联邦学习（VFL）的特征分区架构使得HFL导向的遗忘方法无效，需要专门针对VFL的遗忘解决方案。

Method: 提出REMISVFU框架：当收到删除请求时，遗忘方将其编码器输出坍缩到单位球面上的随机锚点，切断其特征与全局模型的统计联系。服务器联合优化保留损失和遗忘损失，通过正交投影对齐梯度以消除破坏性干扰。

Result: 在公共基准测试中，REMISVFU将后门攻击成功率抑制到自然类别先验水平，仅牺牲约2.5个百分点的干净准确率，优于现有最先进基线方法。

Conclusion: REMISVFU为纵向联邦学习提供了一种有效的即插即用表示误导框架，实现了快速客户端级遗忘，在保护隐私的同时保持了模型效用，满足了数据保护法规的要求。
...
### [184] [LLM-Empowered Representation Learning for Emerging Item Recommendation](https://arxiv.org/abs/2512.10370)
*Ziying Zhang,Quanming Yao,Yaqing Wang*

Main category: cs.AI

TL;DR: EmerFlow是一个基于大语言模型的表示学习框架，专门用于推荐新兴项目，通过LLM推理增强原始特征，与现有推荐模型嵌入空间对齐，并利用元学习整合新交互，从而从有限交互中学习表达性嵌入。


<details>
  <summary>Details</summary>
Motivation: 现有推荐方法通常忽视新兴项目的动态积累过程，简单假设它们只有少量甚至没有历史交互。这种假设过度简化了问题，因为好的模型需要在保持新兴项目独特性的同时，利用它们与成熟项目的共享模式。

Method: 提出EmerFlow框架：1) 通过LLM推理增强新兴项目的原始特征；2) 将这些表示与现有推荐模型的嵌入空间对齐；3) 通过元学习整合新交互来优化嵌入。该框架能够从有限交互中学习表达性嵌入。

Result: 在包括电影和医药等多个领域的广泛实验中，EmerFlow始终优于现有方法，证明了其在推荐新兴项目方面的有效性。

Conclusion: EmerFlow通过结合LLM推理、嵌入对齐和元学习，成功解决了新兴项目推荐中的动态表示学习问题，能够从有限交互中学习到表达性强的嵌入表示。
...
### [185] [When Reject Turns into Accept: Quantifying the Vulnerability of LLM-Based Scientific Reviewers to Indirect Prompt Injection](https://arxiv.org/abs/2512.10449)
...
Method: 研究开发了WAVS（加权对抗性漏洞评分）评估指标，收集了200篇科学论文数据集，针对评审任务调整了15种领域特定的攻击策略，在包括GPT-5、Claude Haiku和DeepSeek在内的13个语言模型上进行了评估

Result: 结果显示，像"Maximum Mark Magyk"这样的混淆策略能够成功操纵评分，即使在大型模型中也能达到令人担忧的决策翻转率，证明了LLM评审系统存在严重的安全漏洞

Conclusion: LLM评审系统对对抗性PDF操纵具有显著脆弱性，需要加强安全防护措施，研究团队将发布完整数据集和注入框架以促进该领域的进一步研究
...
### [186] [Boosting RL-Based Visual Reasoning with Selective Adversarial Entropy Intervention](https://arxiv.org/abs/2512.10414)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于RL的微调方法主要在策略优化阶段对特定token进行熵干预，忽略了RL采样阶段的熵干预可以提升响应多样性，从而改善GRPO性能。
...
### [187] [Remember Me, Refine Me: A Dynamic Procedural Memory Framework for Experience-Driven Agent Evolution](https://arxiv.org/abs/2512.10696)
*Zouying Cao,Jiaji Deng,Li Yu,Weikang Zhou,Zhaoyang Liu,Bolin Ding,Hai Zhao*

Main category: cs.AI

TL;DR: ReMe是一个用于LLM智能体记忆演化的框架，通过多层面提炼、上下文自适应重用和基于效用的精炼机制，实现从静态存储到动态推理的转变，显著提升智能体性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体记忆框架主要采用"被动积累"范式，将记忆视为静态的只增不减档案，无法有效支持动态推理和知识演化，需要更智能的记忆管理机制。

Method: 提出ReMe框架，包含三个核心机制：1) 多层面提炼 - 识别成功模式、分析失败触发因素、生成比较性见解；2) 上下文自适应重用 - 通过场景感知索引将历史见解适配到新情境；3) 基于效用的精炼 - 自主添加有效记忆并修剪过时记忆，保持紧凑高质量的经验池。

Result: 在BFCL-V3和AppWorld基准测试中达到最先进水平，观察到显著的内存缩放效应：配备ReMe的Qwen3-8B超越无记忆的更大模型Qwen3-14B，表明自演化记忆为终身学习提供了计算高效的途径。

Conclusion: ReMe框架成功将静态记忆存储转变为动态推理系统，通过经验驱动的智能体演化机制显著提升LLM智能体性能，为终身学习提供了有效的计算高效解决方案。
...
### [188] [Representation of the structure of graphs by sequences of instructions](https://arxiv.org/abs/2512.10429)
...
<details>
  <summary>Details</summary>
Motivation: 当前图表示方法（基于邻接矩阵）不适合深度学习语言模型处理，而这些模型在文本处理方面表现出色。需要一种能利用这些强大模型处理图数据的新表示方法。

Method: 提出一种将图的邻接矩阵转换为简单指令字符串的新方法。这些指令逐步构建邻接矩阵，转换过程可逆（图↔字符串）。该表示保持图的局部结构模式，且具有紧凑性。

Result: 进行了初步计算实验，结果显示该方法效果良好。该表示方法有望提升深度学习模型处理图数据的能力。

Conclusion: 提出的指令字符串表示方法为图处理提供了新的途径，能够利用深度学习语言模型的强大能力，有望推动图数据处理技术的发展。
...
### [189] [Replace, Don't Expand: Mitigating Context Dilution in Multi-Hop RAG via Fixed-Budget Evidence Assembly](https://arxiv.org/abs/2512.10787)
*Moshe Lahmy,Roi Yozevitch*

Main category: cs.AI

TL;DR: SEAL-RAG提出了一种"替换而非扩展"策略，通过搜索-提取-评估-循环的控制器，在固定检索深度下主动替换干扰信息，解决多跳查询中的上下文稀释问题。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在处理多跳查询时，当初始检索遗漏桥接事实时，通常采用添加更多上下文或修剪现有列表的方法。但简单扩展上下文窗口会导致"上下文稀释"问题，即干扰信息挤占相关信息空间。

Method: SEAL-RAG采用训练免费的控制器，执行搜索→提取→评估→循环的周期：进行实时实体锚定提取构建"缺口规范"，触发有针对性的微查询，使用"实体优先排序"主动将干扰信息替换为填补缺口的证据。

Result: 在HotpotQA上，SEAL将答案正确率比Self-RAG提高了3-13个百分点，证据精度提高了12-18个百分点。在2WikiMultiHopQA上，比Adaptive-k的准确率提高了8.0个百分点，证据精度保持在96%，而CRAG仅为22%。

Conclusion: SEAL-RAG通过固定k替换策略，在保持可预测成本的同时，确保top-k槽位针对精度而非广度进行优化，显著提升了多跳查询的性能。
...
### [190] [Targeted Data Protection for Diffusion Model by Matching Training Trajectory](https://arxiv.org/abs/2512.10433)
*Hojun Lee,Mijin Koo,Yeji Song,Nojun Kwak*

Main category: cs.AI

TL;DR: TAFAP是一种通过轨迹对齐实现有效定向数据保护的新方法，能够控制整个训练轨迹，将模型输出重定向到用户指定的目标概念，同时保持高质量图像生成。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的个性化微调技术日益普及，但引发了未经授权数据使用和隐私侵犯的严重担忧。现有保护方法仅限于被动降低图像质量，无法实现稳定控制。虽然定向数据保护（TDP）提供了向用户指定目标概念主动重定向的有前景范式，但现有TDP方法因快照匹配方法无法考虑完整学习动态而控制性差。

Method: TAFAP（通过对抗性扰动微调的轨迹对齐）是首个通过控制整个训练轨迹成功实现有效TDP的方法。与快照方法不同，TAFAP采用受数据集蒸馏启发的轨迹匹配，在整个微调过程中强制执行持久、可验证的转换。该方法能够同时控制身份和视觉模式。

Result: 通过大量实验验证，TAFAP首次在扩散模型中成功实现了定向转换，同时控制身份和视觉模式。TAFAP显著优于现有TDP尝试，实现了向目标概念的稳健重定向，同时保持高图像质量。

Conclusion: 这项工作实现了可验证的安全保障，并为控制和追踪扩散模型输出的改变提供了新框架，解决了现有保护方法的局限性。
...
### [191] [Zero-shot 3D Map Generation with LLM Agents: A Dual-Agent Architecture for Procedural Content Generation](https://arxiv.org/abs/2512.10501)
...
<details>
  <summary>Details</summary>
Motivation: PCG需要精确配置不透明的技术参数，现有LLM难以弥合抽象用户指令与严格参数规范之间的语义鸿沟，需要更有效的零样本配置方法。

Method: 采用Actor-Critic双智能体架构：Actor负责生成参数配置，Critic评估并反馈，形成迭代工作流，系统自主推理工具参数并逐步优化配置以符合人类设计偏好。

Result: 在3D地图生成任务上验证，建立了PCG指令跟随新基准，实验表明该方法优于单智能体基线，能从自然语言描述生成多样且结构有效的环境。

Conclusion: 现成LLM可有效重新用作通用PCG工具智能体，通过将负担从模型训练转向架构推理，为无需任务特定微调而掌握复杂软件提供了可扩展框架。
...
### [192] [NormCode: A Semi-Formal Language for Context-Isolated AI Planning](https://arxiv.org/abs/2512.10563)
*Xin Guan*

Main category: cs.AI

TL;DR: NormCode是一种半正式语言，用于构建推理计划，通过严格分离语义操作和句法操作，消除多步LLM工作流中的上下文污染问题，实现可审计的AI工作流。


<details>
  <summary>Details</summary>
Motivation: 多步LLM工作流存在上下文污染问题：随着信息在步骤间累积，模型会产生幻觉、混淆中间输出并丢失任务约束。需要一种方法来消除步骤间的交叉污染，确保工作流的可靠性和可审计性。

Method: NormCode是一种半正式语言，采用结构化分解方法，每个步骤在数据隔离环境中运行，只接收显式传递的输入。它严格分离语义操作（LLM驱动的推理，非确定性）和句法操作（确定性数据重构），支持三种同构格式：.ncds用于人工编写、.ncd用于机器执行、.ncn用于人工验证。
...
### [193] [Phythesis: Physics-Guided Evolutionary Scene Synthesis for Energy-Efficient Data Center Design via LLMs](https://arxiv.org/abs/2512.10611)
*Minghao LI,Ruihang Wang,Rui Tan,Yonggang Wen*

Main category: cs.AI

TL;DR: Phythesis是一个结合大语言模型和物理引导进化优化的框架，用于自动生成仿真就绪的数据中心场景，实现能源高效设计。


<details>
  <summary>Details</summary>
Motivation: 传统数据中心设计方法依赖人工经验和专用仿真工具，难以应对日益增长的系统复杂性。现有生成式AI方法虽然能设计室内布局，但未考虑底层物理原理，无法满足数据中心对量化运营目标和严格物理约束的要求。
...
Conclusion: Phythesis成功地将大语言模型与物理引导优化相结合，为数据中心设计提供了一种自动化、仿真就续的场景合成方法，显著提高了设计质量和能源效率。
...
### [194] [Refinement Contrastive Learning of Cell-Gene Associations for Unsupervised Cell Type Identification](https://arxiv.org/abs/2512.10640)
...
<details>
  <summary>Details</summary>
Motivation: 现有的细胞聚类方法大多只关注细胞内在结构，而忽略了细胞-基因关联的关键作用，这限制了它们区分密切相关的细胞类型的能力。

Method: 提出了Refinement Contrastive Learning框架(scRCL)，包含两个对比分布对齐组件来揭示可靠的细胞内在结构，以及一个整合基因相关性结构学习的精炼模块来增强细胞嵌入表示。

Result: 在多个单细胞RNA-seq和空间转录组学基准数据集上的实验表明，该方法在细胞类型识别准确性上持续优于最先进的基线方法。
...
### [195] [CAPTAIN: Semantic Feature Injection for Memorization Mitigation in Text-to-Image Diffusion Models](https://arxiv.org/abs/2512.10655)
*Tong Zhang,Carlos Hinojosa,Bernard Ghanem*

Main category: cs.AI

TL;DR: CAPTAIN是一个无需训练的方法，通过在去噪过程中直接修改潜在特征来减少扩散模型的记忆效应，同时保持与提示的对齐和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型可能会无意中复制训练样本，引发隐私和版权问题。现有的推理时缓解方法通常操纵无分类器指导或扰动提示嵌入，但往往难以在减少记忆的同时保持与提示的对齐。

Method: CAPTAIN采用三步法：1）基于频率的噪声初始化，减少去噪早期复制记忆模式的倾向；2）识别特征注入的最佳去噪时间步并定位记忆区域；3）将非记忆参考图像的语义对齐特征注入到定位的潜在区域中。

Result: 实验表明，与基于CFG的基线方法相比，CAPTAIN显著减少了记忆效应，同时保持了与预期提示的强对齐。

Conclusion: CAPTAIN提供了一个有效的训练免费框架，能够在减少扩散模型记忆的同时保持提示保真度和视觉质量，解决了隐私和版权问题。
...
### [196] [On the Dynamics of Multi-Agent LLM Communities Driven by Value Diversity](https://arxiv.org/abs/2512.10665)
*Muhua Huang,Qinlin Zhao,Xiaoyuan Yi,Xing Xie*

Main category: cs.AI

TL;DR: 研究探索价值多样性如何影响AI多智能体社区的集体行为，发现适度的价值多样性可以增强价值稳定性、促进涌现行为并激发创造性原则，但极端异质性会导致不稳定。
...
Method: 使用基于施瓦茨基本人类价值理论的自然主义价值引出方法，构建了多智能体模拟，让具有不同价值多样性的社区参与开放式互动和宪法制定过程。

Result: 结果显示：价值多样性增强了价值稳定性，促进了涌现行为，并带来了更多由智能体自身开发、无需外部指导的创造性原则。但这些效应也呈现边际递减趋势：极端异质性会引发不稳定性。

Conclusion: 本研究将价值多样性定位为未来AI能力的新维度，连接了AI能力与社会学中的制度涌现研究，为理解AI社区的集体行为提供了重要洞见。
...
TL;DR: 该研究挑战了传统大语言模型安全评估只关注普遍风险的局限性，提出需要考虑用户特定情境的个体化安全评估方法，特别是在金融和健康等高风险咨询领域。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全评估主要关注普遍风险（如危险能力），但数百万用户将LLM用于金融、健康等高风险个人咨询，这些危害是情境依赖而非普遍的。虽然OECD等框架认识到需要评估个体风险，但用户福利安全评估仍然不足。

Method: 采用探索性研究方法，评估GPT-5、Claude Sonnet 4和Gemini 2.5 Pro在金融和健康领域的建议。首先比较有无用户情境信息的评估者评分差异，然后测试用户实际会披露的情境信息是否足够改善评估效果。

Result: 1. 评估者必须访问丰富的用户情境信息：相同LLM回答，无情境评估者评分显著高于有情境评估者，高脆弱性用户安全评分从5/7（安全）降至3/7（有些不安全）。2. 即使用户提供他们愿意披露的情境信息，评估效果也无显著改善。3. 有效评估需要评估者针对多样化用户档案评估回答。

Conclusion: 有效的用户福利安全评估需要评估者基于多样化用户档案评估回答，仅靠用户实际披露的情境信息不足，特别是对脆弱人群。本研究提供了情境感知评估的方法论起点，证明评估个体福利需要不同于现有普遍风险框架的方法。
...
TL;DR: 该研究探索了强化学习（RL）和显式推理（thinking）在胸部X光视觉语言模型中的应用，发现RL能提升报告生成和视觉定位性能，而显式推理则无明显帮助。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型主要依赖监督微调（SFT），但这种方法只优化下一个标记预测而不评估答案质量。强化学习能结合任务特定反馈，且在数学和编程任务中结合显式推理已显示出显著优势，因此研究者希望在胸部X光VLM中探索RL和thinking的效果。

Method: 基于Qwen3-VL构建RadVLM，首先进行大规模CXR数据的SFT，然后通过冷启动SFT阶段赋予模型基本推理能力。接着应用基于临床任务特定奖励的Group Relative Policy Optimization（GRPO）进行报告生成和视觉定位，并在有/无thinking的领域特定和通用领域Qwen3-VL变体上进行匹配的RL实验。

Result: 研究发现，虽然强SFT对基础性能至关重要，但RL在两个任务上都能提供额外增益，而显式推理并未进一步改善结果。在统一评估流程下，RL优化的RadVLM模型超越了基线对手，在报告生成和定位任务上都达到了最先进的性能。

Conclusion: 临床对齐的强化学习是医学视觉语言模型监督微调的有力补充，能显著提升模型性能，而显式推理在胸部X光解释任务中效果不明显。
...
TL;DR: CA-GPT在OCT引导的PCI规划和评估中，相比通用AI模型ChatGPT-5和初级医生，表现出显著更优的决策一致性，特别是在复杂场景下。


<details>
  <summary>Details</summary>
Motivation: 虽然血管内成像（特别是OCT）改善了PCI结果，但其解读依赖于操作者经验。通用AI模型虽有潜力但缺乏领域特异性可靠性，需要开发专门针对OCT引导PCI的AI系统。

Method: 单中心分析96例接受OCT引导PCI的患者，比较CA-GPT、ChatGPT-5和初级医生生成的程序决策与专家记录的一致性，使用10个预设指标评估术前和术后阶段。

Result: 术前规划：CA-GPT中位一致性评分(5[3.75-5])显著高于ChatGPT-5(3[2-4])和初级医生(4[3-4])。CA-GPT在所有术前指标上均优于ChatGPT-5，在支架直径(90.3% vs 72.2%)和长度选择(80.6% vs 52.8%)上优于初级医生。术后评估：CA-GPT保持优异一致性(5[4.75-5])，显著高于ChatGPT-5(4[4-5])和初级医生(5[4-5])。亚组分析证实CA-GPT在复杂场景中的稳健优势。

Conclusion: 基于CA-GPT的AI-OCT系统在PCI规划和评估阶段相比通用大语言模型和初级医生实现了更优的决策一致性，为血管内成像解读提供了标准化可靠方法，显著增强操作者专业能力并优化OCT引导PCI。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中，个体、设备、环境和时间的变化会导致相同活动数据的分布偏移。现有研究只在特定分布偏移场景（如跨设备或跨位置）应用或调整OOD算法，缺乏对这些算法有效性的全面洞察。

Method: 提出HAROOD基准测试，定义4种OOD场景（跨人、跨位置、跨数据集、跨时间），构建包含6个数据集、16种比较方法（基于CNN和Transformer架构）的测试平台，采用两种模型选择协议。

Result: 通过大量实验发现：没有单一方法在所有场景中持续优于其他方法，这突显了该领域仍有巨大的改进空间。基准测试代码模块化且易于扩展。

Conclusion: HAROOD为基于OOD的HAR研究提供了全面基准，揭示了当前方法的局限性，并提供了易于扩展的代码库以促进该领域未来研究。
...
### [201] [Offscript: Automated Auditing of Instruction Adherence in LLMs](https://arxiv.org/abs/2512.10172)
*Nicholas Clark,Ryan Bai,Tanu Mitra*

Main category: cs.HC

TL;DR: Offscript是一个自动化审计工具，用于检测LLM是否遵循用户自定义指令，在Reddit来源的指令测试中发现了86.4%的潜在违规行为。


<details>
  <summary>Details</summary>
Motivation: 随着LLM和生成式搜索系统被不同用户群体用于信息检索，用户可以通过自定义指令调整LLM行为，但缺乏有效机制来评估这些指令是否被正确遵循。

Method: 开发了Offscript自动化审计工具，通过分析Reddit上收集的自定义指令，系统性地检测LLM在对话中可能存在的指令遵循失败情况。

Result: 在试点研究中，Offscript检测到86.4%的对话存在潜在指令偏离，其中22.2%经过人工审查确认为实质性违规。这表明自动化审计是评估行为指令遵循性的可行方法。

Conclusion: 自动化审计工具如Offscript可以有效识别LLM的指令遵循失败，为评估LLM行为合规性提供了可行的技术解决方案。
...
### [202] [HyFinBall: a Hybrid User Interface for Coordinated 2D+3D Visualization in Semi-Immersive VR](https://arxiv.org/abs/2512.10196)
*Isaac Cho,Zachary Wartell*

Main category: cs.HC

TL;DR: 本文提出了一种名为HyFinBall的双模态混合用户界面，支持四种交互模式，用于协调2D和3D视图的可视化应用，通过自动切换输入设备模式来优化跨维度任务的交互体验。


<details>
  <summary>Details</summary>
Motivation: 现有的3D可视化应用通常提供协调的2D和3D视图，但传统3D输入设备在3D任务上表现更好，而在2D任务上表现不佳。需要一种能够无缝切换2D和3D交互模式的界面来支持跨维度任务。

Method: 开发了HyFinBall双模态混合用户界面，支持四种交互模式：双6自由度输入设备模式、双平面约束3自由度输入设备模式、双指多点触控模式以及3D手部和手指手势。使用buttonballs输入设备，可在3D和2D设备模式之间无缝切换，同时支持自由手部输入和设备使用的切换。

Result: 通过两个正式的用户研究评估HyFinBall在各种维度任务（包括3D、2D和跨维度任务）中的表现。实验结果表明，HyFinBall界面在需要3D和2D交互的跨维度任务中具有显著优势。

Conclusion: HyFinBall混合界面通过自动切换输入设备模式和交互技术，有效支持了协调2D和3D视图的可视化应用，特别在跨维度任务中表现出优越性能。
...
### [203] [InFerActive: Towards Scalable Human Evaluation of Large Language Models through Interactive Inference](https://arxiv.org/abs/2512.10234)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM评估范式需要审查大量单个响应，存在严重的可扩展性挑战。LLM输出可以更有效地表示为树结构，但传统树可视化无法扩展到现代采样方法生成的指数级大树

Method: 开发InFerActive交互式推理系统，支持基于概率的过滤和评估功能，通过自适应可视化技术弥合计算token与人类可读文本之间的语义鸿沟

Result: 技术评估和用户研究(N=12)表明，InFerActive显著提高了评估效率，并能够更全面地评估模型行为。专家案例研究证明了其实际适用性和改变LLM评估工作流程的潜力

Conclusion: InFerActive通过树形结构表示和交互式探索，解决了LLM人工评估的可扩展性问题，为LLM评估工作流程提供了创新解决方案
...
### [204] [Reject or Not?: A Benchmark for Voice Assistant Query Rejection in Smart Home Scenario and an Improved Method Based on LLMs](https://arxiv.org/abs/2512.10257)
*Huichao Men,Yizhen Hu,Yingyang He,Yu Gao,Xiaofeng Mou,Yi Xu*

Main category: cs.HC

TL;DR: 该论文针对智能家居语音助手的查询拒绝能力不足问题，提出了首个中文开源基准和评估套件，以及基于大语言模型的个性化查询拒绝方法，显著提升了家庭特定表达和复杂多轮场景的拒绝准确率。


<details>
  <summary>Details</summary>
Motivation: 当前智能家居语音助手在查询拒绝能力方面存在局限，无法有效处理家庭场景中的多样化对话类型，需要系统性的数据基础、评估标准和技术框架来提升可靠性。

Method: 1) 构建首个面向家庭场景的多模态查询拒绝数据集（11,913个文本-语音对，覆盖12种对话类型）；2) 提出三层协作架构：基于Qwen-2.5-3B适配器的语义边界建模、动态家庭级历史对话模块、家庭特定RAG知识库。

Result: 提出的方法在构建的数据集上显著优于零样本和微调的一般大语言模型，在家庭特定表达和复杂多轮场景的拒绝准确率方面有显著提升。

Conclusion: 该工作为智能家居语音交互的可靠性研究提供了可复现的数据基础、评估标准和可扩展的技术框架，有效提升了语音助手的查询拒绝能力。
