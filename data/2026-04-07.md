...
TL;DR: SafeScreen是一个安全优先的视频筛选框架，为儿童和护理环境（如痴呆症护理）提供个性化视频推荐，通过强制执行个体化安全约束来防止有害内容暴露。


<details>
  <summary>Details</summary>
Motivation: 开放域视频平台虽然提供丰富的个性化内容，但其以参与度优化的推荐算法可能使脆弱用户（如儿童、痴呆症患者）接触到不适当或有害内容。在护理环境中，内容必须在展示前满足个体化安全约束。

Method: SafeScreen采用安全优先方法，将安全作为前提条件，通过自动化流水线顺序批准或拒绝候选视频。包含三个核心组件：1) 基于配置文件的个体化安全标准提取；2) 通过自适应问题生成和多模态VideoRAG分析进行证据基础评估；3) LLM驱动的决策机制，在内容暴露前验证安全性、适当性和相关性。

Result: 在痴呆症护理回忆案例研究中，使用30个合成患者配置文件和90个测试查询进行评估。结果显示SafeScreen在80-93%的情况下优先考虑安全而非参与度，与YouTube的参与度优化排名显著不同，同时在安全性覆盖、合理性和基础性方面保持高水平，得到LLM评估和领域专家的验证。

Conclusion: SafeScreen提供了一种可解释的实时视频筛选方法，能够在不依赖预计算安全标签的情况下处理未策划的视频库，为脆弱用户群体提供了安全优先的视频推荐解决方案。
...
TL;DR: 提出基于SoC FPGA的智能摄像头平台，用于工业场景中喷射火焰的实时分割与表征，实现边缘处理并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 解决工业环境中缺乏实时早期火灾分割与表征方案的问题，特别是在喷射火焰检测方面。

Method: 采用SoC FPGA（Ultra96平台）实现边缘处理流水线，通过Vitis框架优化UNet分割模型，将参数从750万减少到59,095个，并结合多线程和批归一化进一步优化。

Result: 模型参数减少125倍，处理延迟降低7.5倍，达到30FPS的实时性能，同时保持Dice Score评估指标的准确性。

Conclusion: 提出的智能摄像头平台能够有效实现工业喷射火焰的实时分割与表征，该框架可扩展到其他消防安全应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs中的3D信息利用方法存在2D-3D表示融合的权衡问题，包括使用显式真值3D位置编码的方法和嫁接外部3D基础模型的方法，导致部署效果不理想。

Method: 提出隐式几何涌现原理，通过细粒度几何验证器和全局表示约束等机制构建信息瓶颈，迫使模型最大化视觉特征与3D结构之间的互信息，让3D感知在统一视觉表示中自然涌现。

Result: 在多个3D场景理解基准测试中超越SOTA，推理延迟降低55%，同时在不同下游任务中保持强大性能，实现了无依赖的3D理解。

Conclusion: 从外部嫁接转向隐式涌现代表了视觉语言模型中3D知识整合的根本性重新思考，精心设计的辅助目标能够实现无依赖的3D理解，具有显著的实际部署优势。
...
Method: 提出MoViD框架，包含视角估计器建模关键关节关系预测视角信息，正交投影模块解耦运动和视角特征，基于物理的对比对齐增强跨视角鲁棒性，以及面向边缘设备的逐帧推理流水线。

Result: 在9个公开数据集和新收集的多视角无人机、步态分析数据集上，MoViD相比最先进方法减少姿态估计误差超过24.2%，在严重遮挡下保持鲁棒性且训练数据减少60%，在NVIDIA边缘设备上实现15FPS实时推理。

Conclusion: MoViD通过解耦视角和运动特征，实现了视角不变的3D人体姿态估计，在精度、鲁棒性和效率方面均有显著提升，为实际边缘部署提供了可行方案。
...
TL;DR: 研究在严格限制下（仅上传紧凑嵌入向量+元数据）的星上系统，通过向量搜索对遥感图像进行灾害分类，验证嵌入向量上传方案在不同遥感偏移场景下的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决下行链路瓶颈问题，避免传输原始像素数据，通过上传紧凑嵌入向量实现星上灾害优先级排序，研究在跨时间、跨事件/地点、跨站点云和跨城市AOI等多种遥感偏移场景下的有效性。

Method: 采用严格设置：地面站仅上传紧凑嵌入向量和元数据，星上系统通过向量搜索对新捕获图像进行分类。使用OlmoEarth嵌入向量，在扩展的公共多任务基准上进行评估（27个Sentinel-2 L2A场景，15个云站点，5个SpaceNet-2 AOI；10次随机种子）。比较kNN检索和类别质心两种决策头方法。

Result: 所有有效方法都依赖于相同的上传嵌入向量，但最优决策头是任务依赖的：kNN检索在云分类中显著优于质心方法（0.92 vs 0.91；p<0.01），而类别质心在时序变化检测中占主导地位（0.85 vs 检索0.48；p<0.01）。

Conclusion: 仅嵌入向量上传是关键使能技术，一旦嵌入向量在星上，系统可以为每个任务选择最佳决策头而无需额外上行链路成本，所有遥测数据每查询小于1KB。
...
TL;DR: 该论文发现当前多模态大语言模型在连续体对象动态理解方面存在严重不足，提出了两个基准任务进行评估，并设计了Scene Dynamic Field方法显著提升了物理推理能力。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在图像和视频理解方面表现出色，但在理解物理世界方面存在显著不足，特别是在高级物理推理方面。当前模型在理解连续体对象的动态特性方面存在严重局限，这是物理推理的第一步——直觉物理理解。

Method: 提出了两个基础基准任务：下一帧选择和时序一致性验证，用于隔离和评估模型的物理理解能力。为解决这一局限，提出了Scene Dynamic Field方法，该方法利用物理模拟器构建多任务微调框架。

Result: 实验表明，即使最先进的多模态大语言模型在这些基础任务上也表现不佳。而Scene Dynamic Field方法显著提升了性能，在流体任务上实现了高达20.7%的性能提升，并在未见过的物理领域表现出强大的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖2D控制信号，缺乏空间表达能力，且限制了合成3D条件数据的利用。需要开发能够利用明确3D表示的手-物交互视频合成方法。

Method: 提出基于扩散的架构，增强3D ControlNet来编码3D输入的几何和运动线索；包含两个核心组件：3D感知HOI视频生成扩散架构和用于构建输入和条件信号的混合管道。

Result: 在TASTE-Rob数据集上，HVG-3D实现了最先进的空间保真度、时间一致性和可控性，同时能够有效利用真实和模拟数据。

Conclusion: HVG-3D通过明确的3D表示实现了高质量的手-物交互视频合成，提供了精确的空间和时间控制，为3D感知视频生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度聚类方法很少考虑模型学习策略对复杂图像数据聚类鲁棒性和性能的影响，且大多仅依赖点对点距离进行聚类分配，导致迭代过程中误差累积。需要更有效的训练策略和聚类机制来提升图像聚类质量。

Method: 1. 设计基于输入数据密度信息的课程学习方案，实现更合理的学习节奏；2. 使用密度核心而非单个聚类中心来引导聚类分配；3. 将密度信息融入模型训练策略，首次在图像聚类中引入这种创新方法。

Result: 在基准数据集上与最先进的聚类方法进行广泛比较，证明了所提方法在鲁棒性、快速收敛性以及数据规模、聚类数量和图像上下文灵活性方面的优越性。

Conclusion: IDCL方法通过引入基于密度信息的模型训练策略，有效解决了现有深度聚类方法的局限性，为图像聚类提供了更鲁棒、高效且灵活的解决方案。
...
TL;DR: V-Reflection框架通过"思考-观察"视觉反射机制，将多模态大语言模型转变为主动询问者，解决细粒度任务中的感知幻觉问题，保持端到端解码效率。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在细粒度任务中容易产生感知相关的幻觉，主要原因是其推理局限于语言域，将视觉输入视为静态的、与推理无关的前置内容，而非动态参与者，导致模型成为被动观察者，无法重新审视视觉细节来支撑其演进的推理状态。
...
Conclusion: V-Reflection通过"思考-观察"视觉反射机制，将MLLM转变为主动询问者，有效解决了细粒度任务中的感知幻觉问题，同时保持了纯端到端自回归解码的效率优势。
...
TL;DR: TreeGaussian：基于树引导级联对比学习框架，解决3D高斯溅射在层次语义表示和特征学习方面的局限性


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法难以表示复杂场景中的层次3D语义结构和捕获整体-部分关系，密集成对比较和2D先验的不一致层次标签阻碍特征学习，导致分割效果不佳

Method: 提出TreeGaussian框架：1）构建多层次对象树实现结构化学习；2）两阶段级联对比学习策略从全局到局部渐进优化特征；3）一致性分割检测机制对齐多视角分割模式；4）基于图的去噪模块抑制不稳定高斯点

Result: 通过开放词汇3D对象选择、3D点云理解和消融实验验证了方法的有效性和鲁棒性

Conclusion: TreeGaussian通过显式建模层次语义关系和减少对比监督冗余，显著提升了3D高斯溅射在场景理解和分割方面的性能
...
TL;DR: 提出基于扩散模型随机最优控制的推理时优化框架，用于生成语义不同运动域之间的连贯长程人体运动过渡


<details>
  <summary>Details</summary>
Motivation: 长程人体运动生成是计算机视觉和图形学的核心挑战，特别是在不同语义运动域之间生成连贯过渡的能力尚未充分探索，这对于舞蹈编排等应用尤为重要

Method: 提出基于扩散随机最优控制的推理时优化框架，通过控制能量目标函数显式正则化预训练扩散模型的过渡轨迹，在推理时优化该目标以获得保真度和时间连贯性

Result: 优化后的框架能够生成具有保真度和时间连贯性的运动过渡，这是首个提供显式过渡建模的受控长程人体运动生成通用框架

Conclusion: 该工作首次为受控长程人体运动生成提供了显式过渡建模的通用框架，在推理时优化控制能量目标能够有效生成跨语义运动域的连贯过渡
...
TL;DR: CardioSAM：结合SAM编码器与心脏专用解码器的混合架构，在心脏MRI分割中达到超越专家一致性的精度


<details>
  <summary>Details</summary>
Motivation: 心脏MRI手动分割耗时且存在观察者间差异，现有基础模型（如SAM）泛化能力强但边界精度不足，无法满足临床应用需求

Method: 提出CardioSAM混合架构：冻结SAM编码器用于通用特征提取，搭配轻量可训练的心脏专用解码器，包含心脏特定注意力模块（引入解剖拓扑先验）和边界细化模块

Result: 在ACDC基准测试中达到Dice系数93.39%、IoU 87.61%、像素精度99.20%、HD95 4.2mm，超越nnU-Net 3.89% Dice，超过专家间一致性水平（91.2%）

Conclusion: CardioSAM通过结合通用特征提取与心脏特定优化，实现了可靠且临床适用的心脏分割，为心血管疾病诊断治疗提供了准确工具
...
TL;DR: CoLA是一种新型参数高效微调框架，通过引入跨模态适配路径扩展LoRA，在保持参数效率的同时提升多模态任务性能


<details>
  <summary>Details</summary>
Motivation: 基础模型在多模态任务适配中存在挑战，特别是双流架构中的单模态编码器（如DINO和BERT）。现有的PEFT方法如LoRA在各模态内孤立运行，无法有效捕捉跨模态交互

Method: 提出跨模态低秩适配（CoLA）框架，在标准模态内适配路径基础上引入专门的模态间适配路径，形成双路径设计，避免模态特定学习和跨模态学习之间的干扰

Result: 在视觉语言（RefCOCO、RefCOCO+、RefCOCOg）和视听（AVE、AVS）基准测试中，CoLA始终优于LoRA，分别获得约3%和2%的相对增益，同时保持参数效率

Conclusion: CoLA有效解决了多模态适配中的跨模态交互问题，首次实现了视觉定位的多任务PEFT框架，填补了高效多模态适配的关键空白
...
TL;DR: StoryBlender是一个基于3D的故事板生成框架，通过语义-空间锚定、规范资产实体化和时空动态三阶段流程，解决了现有方法难以同时保证跨镜头一致性和可编辑性的问题。


<details>
  <summary>Details</summary>
Motivation: 当前故事板自动化方法存在两个主要问题：2D扩散生成方法虽然能产生生动的图像，但存在身份漂移和几何控制有限的问题；传统3D动画工作流程虽然一致且可编辑，但需要专家级、劳动密集型的创作。需要一种能同时实现跨镜头一致性和明确可编辑性的系统。

Method: 提出StoryBlender框架，采用三阶段流水线：1) 语义-空间锚定：构建连续性记忆图，将全局资产与镜头特定变量解耦；2) 规范资产实体化：在统一坐标空间中实例化实体以保持视觉身份；3) 时空动态：通过视觉指标实现布局设计和电影演化。采用分层多智能体协调和验证循环机制进行迭代自校正。

Result: 实验表明，StoryBlender在一致性和可编辑性方面显著优于基于扩散的方法和3D锚定基线方法。生成的本地3D场景支持对相机和视觉资产进行直接、精确的编辑，同时保持跨镜头连续性。

Conclusion: StoryBlender通过故事中心反射方案和分层智能体协调，成功解决了故事板生成中跨镜头一致性和可编辑性的双重挑战，为视觉叙事提供了高效且可控的自动化解决方案。
...
TL;DR: 该论文研究了大型视觉语言模型中的注意力汇聚现象，将其分为视觉编码器产生的V-sink和LLM层产生的L-sink，揭示了全局先验与局部感知之间的性能权衡，并提出了无需任务监督的层间汇聚门控模块来平衡两者。


<details>
  <summary>Details</summary>
Motivation: 虽然单模态Transformer中的注意力汇聚已被研究，但在大型视觉语言模型中，跨模态注意力汇聚的影响仍不清楚：它们是冗余的伪影还是必要的全局先验？需要探索这些汇聚在视觉语言模型中的角色和影响。

Method: 首先将视觉汇聚分为两类：视觉编码器产生的V-sink和LLM深层产生的L-sink。基于此分析，发现了全局场景先验与局部视觉证据之间的性能权衡。然后提出了层间汇聚门控模块，该模块动态调节V-sink与其他视觉token的注意力贡献，通过标准的下一个token预测进行训练，无需任务特定监督，保持LVLM主干冻结。

Result: 分析揭示了注意力汇聚在编码全局先验方面的有效性，但同时也可能抑制局部感知所需的细粒度视觉证据。LSG模块在大多数层中都能改善代表性多模态基准测试的性能，有效平衡全局推理和精确的局部证据。

Conclusion: 注意力汇聚在大型视觉语言模型中既是必要的全局先验，也可能成为局部感知的障碍。提出的LSG模块提供了一种轻量级、即插即用的解决方案，能够动态平衡全局推理和局部证据，提升多模态任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法检测协作学习中的注视行为需要大量标注数据，且训练数据集往往无法涵盖教育情境中的全部情况，导致跨配置鲁棒性不足。本研究旨在解决这些挑战。

Method: 提出可扩展的AI方法，利用预训练模型：YOLO11进行人员跟踪，YOLOE-26（具备文本提示能力）检测教育相关对象，Gaze-LLE模型预测注视目标，整个过程无需人工标注数据。

Result: 该方法在视频数据中检测学生注视行为的F1分数达到0.829，对笔记本电脑注视和同伴注视表现良好，但对其他注视目标表现较弱。与有监督机器学习方法相比，在复杂情境下表现出更优越和稳定的性能，具有更好的跨配置鲁棒性。

Conclusion: 提出的基于预训练和基础模型的方法能够有效自动检测协作学习中的注视行为，无需人工标注，且在复杂教育情境中具有更好的鲁棒性，对支持真实环境中学生的协作学习具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么需要引入3D先验或几何监督导致数据准备和对齐成本高，要么纯2D方法在多帧空间推理中因难以捕捉跨帧空间关系而表现不佳。

Method: 提出EgoMind框架，包含角色扮演描述（构建跨帧连贯的语言场景图）和渐进式空间分析（逐步推理特定任务问题），无需几何信息。

Result: 仅用5K自动生成的SFT样本和20K RL样本，在VSI-Bench、SPAR-Bench、SITE-Bench和SPBench上取得竞争性结果。
...
Method: 使用模态特定编码器和双Q-Former提取语言相关的视觉和触觉特征，压缩为前缀令牌输入大语言模型；通过对比学习对齐各模态与文本，并显式耦合视觉和触觉。

Result: 在HCT和TVL基准测试中表现最佳，在SSVTP上保持竞争力；在VitaSet数据集上达到88.89%硬度准确率、75.13%粗糙度准确率和54.81%描述符召回率；材料描述任务语义相似度达0.9009；通过LoRA微调实现缺陷识别准确率100.0%、96.0%、92.0%（2、3、5类别），实验室机器人试验中达到94.0%闭环识别准确率和94.0%端到端分拣成功率。

Conclusion: VitaTouch通过整合视觉、触觉和语言模态，有效解决了智能制造中材料属性检测的挑战，在多个基准测试和实际应用中表现出优越性能。
...
TL;DR: 该论文研究了面向安全的3D物体检测评估与优化方法，通过安全导向的度量标准NDS-USC和安全感知损失函数EC-IoU，在单车感知、车路协同感知和端到端驾驶框架中验证了安全对齐方法能有效提升自动驾驶安全性。
...
Method: 1. 使用安全导向度量标准NDS-USC和安全感知损失函数EC-IoU；2. 在单车3D物体检测模型上进行扩展研究；3. 对AV-基础设施协同物体检测模型进行以自我为中心的安全导向评估；4. 将EC-IoU集成到SparseDrive端到端框架中。

Result: 1. 标准度量指标（如mAP和NDS）的增益可能无法转化为安全导向标准NDS-USC的改进；2. 安全感知微调能显著提升安全关键检测性能；3. 协同感知模型优于单车模型，对"零愿景"有潜在贡献；4. 安全感知感知硬化可将碰撞率降低近30%，直接提升端到端系统级安全性。

Conclusion: 安全对齐的感知评估和优化为增强CAV安全性提供了实用路径，适用于单车、协同和端到端自动驾驶场景，表明将安全考虑直接融入感知系统设计和评估的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 从3D点云准确重建叶片表面对于农业表型分析等应用至关重要，但现实植物数据（不规则3D点云）复杂且难以准确重建植物部件。虽然已有多种表面重建方法，但它们在叶片表面重建中的相对性能尚不明确。

Method: 对九种代表性表面重建方法进行对比研究，包括参数化、三角化、隐式和基于学习的方法。在三个公开数据集（LAST-STRAW、Pheno4D、Crops3D）上进行评估，涵盖不同物种、传感器和感知环境。

Result: 分析揭示了不同方法在表面面积估计精度、平滑度、对噪声和缺失数据的鲁棒性以及计算成本之间的权衡。这些因素影响农业应用中机器人硬件的成本和约束条件。结果显示每种方法根据应用和资源约束具有不同的优势。

Conclusion: 研究结果为资源受限的机器人平台选择表面重建技术提供了实用指导，表明需要根据具体应用需求和资源限制来选择合适的方法。
...
TL;DR: 该论文介绍了RGxEStat工具，用于基因型与环境互作分析，包含显著性分析和稳定性分析模型，为育种工作者提供无需编程的交互式分析平台。


<details>
  <summary>Details</summary>
Motivation: 基因型与环境互作影响基因型在不同环境下的表现，降低表型预测性。深入分析GxE互作有助于识别特定环境条件下基因优势或缺陷的表达或抑制，从而支持遗传选择和育种实践。

Method: 提出两种GxE互作分析模型：基于混合效应模型的显著性分析，用于确定基因或GxE互作是否显著影响表型性状；稳定性分析，深入研究基因与环境间的交互关系以及基因型在不同环境中的相对优劣。开发了RGxEStat轻量级交互工具，集成了模型构建、求解和可视化功能。

Result: 开发了RGxEStat工具，为育种家和农学家提供用户友好的界面，无需学习复杂的SAS或R编程，即可进行育种数据分析，显著加速研究周期。代码和数据集已在GitHub上公开。

Conclusion: RGxEStat工具简化了GxE互作分析流程，降低了技术门槛，有助于育种工作者更高效地进行遗传选择和育种实践，促进育种研究的发展。
...
TL;DR: AC-LAM通过强制潜在动作空间具有加性组合结构，解决了现有潜在动作学习方法容易纠缠无关场景细节、错误校准运动幅度的问题，在模拟和真实世界桌面任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有潜在动作学习方法缺乏编码物理运动加性组合结构的先验，导致潜在动作容易纠缠无关场景细节、包含未来观测信息、错误校准运动幅度，需要更结构化的潜在动作表示。

Method: 提出加性组合潜在动作模型(AC-LAM)，在潜在动作空间上强制实施短时间范围内的场景级加性组合结构约束，鼓励潜在动作空间具有简单代数结构（恒等、逆、循环一致性），抑制非加性组合的信息。

Result: AC-LAM学习到更具结构化、运动特异性、位移校准的潜在动作，为下游策略学习提供更强的监督，在模拟和真实世界桌面任务中优于最先进的潜在动作模型。

Conclusion: 通过强制潜在动作空间具有加性组合结构，AC-LAM能够学习更高质量的潜在动作表示，有效提升下游策略学习性能，为利用互联网规模视频进行具身AI研究提供了改进方法。
...
Method: 提供混合专家模型在遥感领域的系统性综述，包括基本原理、架构设计，以及在不同遥感任务中的关键应用。

Result: 首次全面梳理了混合专家模型在遥感领域的应用现状，为研究者提供了该领域的系统知识框架。

Conclusion: 混合专家模型是解决遥感数据多样性和动态性挑战的有效范式，本综述为未来研究提供了基础并指明了创新方向。
...
TL;DR: 研究发现多模态智能系统在多轮图像编辑中存在迭代退化问题，图像质量随着编辑次数增加而显著下降，但现有图像质量评估指标无法有效检测这种退化。


<details>
  <summary>Details</summary>
Motivation: 虽然最新的图像编辑模型在单轮编辑中能忠实遵循指令并生成高质量图像，但作者发现多轮编辑中存在关键弱点——图像质量的迭代退化。随着图像被反复编辑，微小伪影会累积，导致可见噪声严重积累，甚至无法遵循简单的编辑指令。

Method: 作者引入了Banana100数据集，包含通过100次迭代编辑步骤生成的28,000张退化图像，涵盖多样纹理和图像内容。同时评估了21种流行的无参考图像质量评估（NR-IQA）指标对这些退化图像的检测能力。

Result: 令人担忧的是，图像质量评估器无法检测到这种退化。在21种流行的NR-IQA指标中，没有一种能始终为严重退化图像分配比干净图像更低的分数。生成器和评估器的双重失败可能威胁未来模型训练的稳定性和部署智能系统的安全性。

Conclusion: 多轮编辑产生的低质量合成数据如果逃过质量过滤器，可能威胁模型训练稳定性和智能系统安全性。作者发布完整代码和数据以促进开发更鲁棒的模型，帮助减轻多模态智能系统的脆弱性。
...
TL;DR: 量化鲁棒性可通过权重空间中的可转移方向（量化向量）实现零-shot提升，无需接收方量化感知训练


<details>
  <summary>Details</summary>
Motivation: 量化感知训练（QAT）成本高且需要训练数据，本文旨在寻找一种无需接收方训练数据的低成本替代方案，实现模型对后训练量化的鲁棒性

Method: 通过简单的权重空间算术从捐赠任务中提取"量化向量"，该向量可应用于接收模型以提升其对量化噪声的鲁棒性，无需接收方的量化感知训练

Result: 在Vision Transformer模型上，该方法可将后训练量化噪声的鲁棒性提升高达60%，提供了一种零-shot、低成本的量化部署方案

Conclusion: 量化鲁棒性不仅是任务特定训练的副产品，更是权重空间几何中可重复使用的特征，可以通过转移而非重新训练来实现
...
TL;DR: 本研究提出了一种基于基础模型的猪群监控工作流，通过预训练视觉-语言基础模型作为通用视觉骨干，结合轻量级任务特定逻辑，实现了可扩展、标签高效、长时程的猪只监控。


<details>
  <summary>Details</summary>
Motivation: 精准畜牧业中现有监控系统主要依赖监督学习模型，需要大量标注数据、重复训练和农场特定调优。基础模型的发展为减少对任务特定监督学习的依赖提供了可能，本研究旨在探索如何利用基础模型实现猪群的自动化监控。

Method: 采用基础模型为中心的工作流：1) 使用Grounding-DINO进行目标检测建立基线；2) 针对夜间和遮挡问题集成时序跟踪逻辑；3) 使用Grounded-SAM2进行短期视频分割；4) 开发长期跟踪管道，包括初始化、跟踪、匹配、掩码细化、重识别和后处理质量控制。

Result: 检测在白天条件下准确率高，但在夜视和严重遮挡下性能下降。短期视频分割后，80%以上的活动轨迹完全正确。长期跟踪系统在132分钟连续视频中保持稳定身份，在132个采样帧上获得：平均区域相似度(J)0.83、轮廓准确度(F)0.92、J&F 0.87、MOTA 0.99、MOTP 90.7%，无身份切换。

Conclusion: 本研究展示了如何将基础模型的先验知识与轻量级、任务特定的逻辑相结合，在猪生产中实现可扩展、标签高效、长时程的监控，为精准畜牧业提供了新的技术路径。
...
TL;DR: 该研究将Circuit Duplication方法应用于水下物种分类，通过重复遍历选定的Transformer层来改进冻结自监督模型嵌入，无需微调模型权重即可提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 水下物种自动分类面临标注成本高和环境变化导致模型泛化能力有限的问题。现有研究表明冻结的自监督视觉基础模型嵌入已为海洋图像分类提供了强大的标签高效基线，但需要探索是否能在推理时进一步改进这些冻结嵌入的性能。

Method: 采用Circuit Duplication方法（最初为大型语言模型设计），在推理时重复遍历选定的Transformer层。在AQUA20基准测试上评估两种设置：1）全局电路选择：为整个数据集选择单一重复电路；2）类别特定电路选择：每个物种可能获得不同的最优电路。两种设置都使用简单的半监督下游分类器。

Result: Circuit Duplication一致优于标准冻结前向传播。在最大标签预算下，类别特定选择达到0.875的宏观F1分数，将与完全监督ConvNeXt基准（0.889）的差距缩小到1.4分，无需任何基于梯度的训练。4个物种超过其完全监督参考，章鱼提高了+12.1 F1分。在所有预算下，约75%的类别偏好类别特定电路，表明存在真正的类别依赖性益处。

Conclusion: Circuit Duplication能有效改进冻结自监督嵌入在水下物种分类中的性能，无需微调模型权重。这是Circuit Duplication首次应用于计算机视觉领域，展示了推理时方法在标签高效学习中的潜力。
...
Method: 开发完全开源的Photoshop插件ExpressEdit，避免专有模型的常见伪影，与Photoshop原生操作（如液化）协同工作，并构建包含135种表情标签的数据库支持检索增强生成。

Result: ExpressEdit在单消费级GPU上3秒内完成表情编辑，比流行专有模型快得多，无常见伪影，能无缝集成到专业工作流程中。

Conclusion: ExpressEdit填补了AI表情编辑工具与专业图像编辑软件之间的空白，开源代码和数据集促进未来研究和艺术探索。
...
TL;DR: 本研究提出了一种多模态框架，结合高分辨率卫星影像和谷歌街景图像，在有限标注条件下实现可扩展的详细城市树木检测，并通过混合学习策略显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 城市树木在环境可持续性和灾害缓解中发挥重要作用，但传统的人工现场调查方法成本高、难以扩展，而现有自动化系统受限于高标注成本和跨城市场景的泛化能力差。

Method: 采用多模态框架：首先利用卫星影像定位树木候选区域，然后检索针对性的地面街景图像进行详细检测，减少无效采样。为解决标注瓶颈，使用领域适应将已有标注数据集的知识迁移到新区域。进一步评估了三种学习策略：半监督学习、主动学习和两者结合的混合方法，使用基于transformer的检测模型。

Result: 混合策略取得了最佳性能，F1分数达到0.90，比基线模型提升12%。半监督学习因伪标注中的确认偏差导致性能逐渐下降，而主动学习通过针对性的人工干预标注不确定或错误的预测，稳步改善结果。错误分析显示主动和混合策略减少了假阳性和假阴性。

Conclusion: 多模态方法和指导性标注对于实现可扩展、标注高效的城市树木制图至关重要，能够支持可持续城市规划。研究强调了结合卫星和地面视角以及智能标注策略在有限标注条件下的有效性。
...
TL;DR: 论文提出用户显著对象检测（UserSOD）任务，强调用户主动需求而非被动视觉刺激在显著对象检测中的决定性作用，并指出该领域缺乏训练和测试数据集的主要挑战。


<details>
  <summary>Details</summary>
Motivation: 现有显著对象检测方法基于被动视觉刺激原理，忽略了用户主动需求在分割显著对象中的决定性作用。这种忽视不仅无法满足用户需求，还限制了下游任务的发展，如显著对象排序任务中无法分析用户观看顺序与场景的细粒度关系。

Method: 论文倡导用户显著对象检测（UserSOD）任务，专注于检测与用户主动需求一致的显著对象。主要挑战是缺乏用于模型训练和测试的数据集。

Result: 论文提出了一个新的研究方向，但尚未提供具体实验结果，主要贡献在于识别了现有方法的局限性和提出了新的任务框架。

Conclusion: 检测基于用户需求的显著对象至关重要，UserSOD任务为解决这一问题提供了新的研究方向，但需要解决数据集缺乏的挑战。
...
### [31] [Focus Matters: Phase-Aware Suppression for Hallucination in Vision-Language Models](https://arxiv.org/abs/2604.03556)
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在对象幻觉问题，现有方法通常依赖迭代优化导致推理延迟高，需要更高效的幻觉缓解方案

Method: 分析视觉编码器注意力动态，发现三阶段处理模式（扩散-聚焦-再扩散），提出在焦点阶段选择性抑制低注意力标记，使用DPP保持视觉多样性

Result: 在多个LVLM骨干和解码策略上实验，一致降低幻觉指标，保持竞争性描述质量，相比对抗性不确定性估计方法，延迟几乎为零

Conclusion: 通过分析视觉编码器内部注意力动态，提出了一种高效、训练自由的推理时干预方法，有效缓解LVLM的对象幻觉问题
...
TL;DR: DINO-VO是一个端到端的单目视觉里程计系统，通过可微分自适应补丁选择器和多任务特征提取模块，结合可微分光束法平差，在合成、室内和室外环境中实现了强大的场景泛化能力和最先进的跟踪精度。


<details>
  <summary>Details</summary>
Motivation: 当前视觉里程计系统通常依赖启发式特征提取策略，这在大规模室外环境中会降低准确性和鲁棒性。DINO-VO旨在解决这些限制，通过端到端学习提高特征质量并增强跨数据集的泛化能力。

Method: 1. 引入可微分自适应补丁选择器到端到端管道中，提高提取补丁的质量；2. 集成多任务特征提取模块；3. 结合可微分光束法平差模块，利用逆深度先验；4. 有效学习和利用外观和几何信息，弥合特征学习和状态估计之间的差距。

Result: 在TartanAir、KITTI、Euroc和TUM数据集上的大量实验表明，DINO-VO在合成、室内和室外环境中表现出强大的泛化能力，实现了最先进的跟踪精度。

Conclusion: DINO-VO通过端到端学习框架，结合自适应补丁选择和多任务特征提取，成功解决了传统VO系统在特征提取和场景泛化方面的局限性，为单目视觉里程计提供了更准确、鲁棒的解决方案。
...
TL;DR: DriveVA是一个自动驾驶世界模型，通过共享潜在生成过程联合解码未来视觉预测和动作序列，提升跨数据集和传感器配置的泛化能力，在NAVSIM上达到90.9 PDM分数。


<details>
  <summary>Details</summary>
Motivation: 当前基于世界模型的规划方法在跨数据集和传感器配置的泛化能力有限，且松散耦合的规划范式导致视觉想象中视频-轨迹一致性差。需要解决自动驾驶中的泛化挑战。

Method: 提出DriveVA模型，采用DiT-based解码器联合预测未来动作序列（轨迹）和视频，实现规划与场景演化的紧密对齐；引入视频延续策略增强长时滚转一致性；继承大规模视频生成模型的运动动力学和物理合理性先验。

Result: 在NAVSIM挑战中获得90.9 PDM分数的闭环性能；在nuScenes上将平均L2误差和碰撞率分别降低78.9%和83.3%；在基于CARLA v2的Bench2drive上分别降低52.5%和52.4%；展示了零样本能力和跨域泛化能力。

Conclusion: DriveVA通过联合解码视觉预测和动作序列，显著提升了自动驾驶世界模型的泛化能力和规划一致性，为现实世界部署提供了更稳健的解决方案。
...
TL;DR: 该论文提出了一种随机生成式PnP框架，将预训练的基于分数的扩散模型直接集成到PnP算法中，通过噪声注入改善严重病态逆问题的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管PnP方法和基于分数的扩散模型都依赖于去噪器，但如何在不依赖反向扩散采样的前提下，系统地将SBDMs作为先验集成到PnP框架中仍不清楚。

Method: 建立了PnP的分数解释，证明可以直接在PnP算法中使用预训练的SBDMs。在此基础上提出了随机生成式PnP框架，通过注入噪声更好地利用SBDM先验的表达能力。

Result: 在多线圈MRI重建和大掩码自然图像修复等挑战性逆任务上，相比传统PnP方法有持续改进，性能与基于扩散的求解器相当。

Conclusion: 该研究为将SBDMs作为先验集成到PnP框架提供了理论基础和实践方法，通过噪声注入机制改善了严重病态逆问题的求解鲁棒性。
...
TL;DR: PortraitCraft是一个统一的肖像构图基准，包含5万张带多级标注的肖像图像，支持构图理解和可控生成两个任务，填补了现有研究在结构化肖像构图分析方面的空白。


<details>
  <summary>Details</summary>
Motivation: 现有数据集和基准主要集中在粗粒度美学评分、通用图像美学或无约束肖像生成，缺乏对结构化肖像构图分析和可控肖像生成的系统研究，限制了该领域的发展。

Method: 构建了包含约5万张精心挑选的真实肖像图像的数据集，提供结构化多级监督：全局构图评分、13个构图属性标注、属性级解释文本、视觉问答对和面向构图的文本描述。基于此数据集建立了构图理解和构图感知生成两个互补的基准任务。

Result: 建立了统一的肖像构图基准PortraitCraft，定义了标准化评估协议，并提供了代表性多模态模型的参考基线结果。

Conclusion: PortraitCraft为未来细粒度肖像理解、可解释美学评估和可控肖像生成研究提供了全面的基准，填补了现有研究空白。
...
TL;DR: TABLeT是一种用于fMRI长程时空动态建模的新方法，通过预训练的2D自然图像自编码器将3D fMRI体积压缩为连续令牌，使用Transformer编码器进行长序列建模，在计算效率和性能上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: fMRI四维信号的高维度使得长程时空动态建模面临挑战。现有的基于体素的方法虽然性能优秀且可解释性强，但受限于巨大的内存需求，只能捕捉有限的时程窗口。

Method: 提出TABLeT方法：使用预训练的2D自然图像自编码器将3D fMRI体积压缩为紧凑的连续令牌，然后使用简单的Transformer编码器进行长序列建模。还开发了自监督掩码令牌建模方法进行预训练。

Result: 在UK-Biobank、Human Connectome Project和ADHD-200等多个大规模基准测试中，TABLeT在多项任务上优于现有模型，同时在相同输入条件下比最先进的基于体素方法在计算和内存效率上有显著提升。

Conclusion: TABLeT为脑活动的可扩展和可解释的时空建模提供了一种有前景的方法，通过令牌化压缩实现了长序列建模，同时保持了计算效率。
...
TL;DR: MuPD是一个多模态病理学扩散模型，通过扩散变换器和解耦跨模态注意力将组织学、分子RNA和临床文本嵌入共享潜在空间，支持多种跨模态合成任务，在多个任务上显著优于领域特定模型。
...
Method: 引入MuPD（多模态病理学扩散），这是一个生成基础模型，通过具有解耦跨模态注意力的扩散变换器，将H&E染色组织学、分子RNA谱和临床文本嵌入共享潜在空间。在3400万组织学图像块、160万文本-组织学对和1080万RNA-组织学对上预训练，涵盖34个人体器官。

Result: 在文本条件和图像到图像生成中，MuPD合成组织学上真实的组织结构，FID分数降低50%，通过合成数据增强将少样本分类准确率提高47%。在RNA条件组织学生成中，FID降低23%，同时保持五种癌症类型的细胞类型分布。作为虚拟染色剂，将H&E图像转换为免疫组化和多重免疫荧光，平均标记相关性提高37%。

Conclusion: 单一统一生成模型在异质病理学模态上预训练可以显著优于专业替代方案，为多模态组织病理学提供了可扩展的计算框架。
...
### [38] [ZeD-MAP: Bundle Adjustment Guided Zero-Shot Depth Maps for Real-Time Aerial Imaging](https://arxiv.org/abs/2604.04667)
*Selim Ahmet Iz,Francesco Nex,Norman Kerle,Henry Meissner,Ralf Berger*

Main category: cs.CV

TL;DR: ZeD-MAP：一种将零样本扩散深度模型转换为度量一致SLAM式映射管线的集群级框架，通过集成增量式集群束调整实现实时无人机高分辨率影像深度重建。


<details>
  <summary>Details</summary>
Motivation: 实时从超高分辨率无人机影像进行深度重建对于灾害响应等时间关键地理空间任务至关重要，但面临宽基线视差、大图像尺寸、低纹理或镜面表面、遮挡以及严格计算限制等挑战。现有零样本扩散模型虽能快速预测但缺乏可靠的度量精度和时间一致性。

Method: 提出ZeD-MAP框架，将测试时扩散深度模型转换为度量一致的SLAM式映射管线。通过将流式无人机帧分组为重叠集群，周期性束调整产生度量一致的位姿和稀疏3D连接点，再将其重投影到选定帧中作为扩散深度估计的度量引导。

Result: 在约50米高度采集的地面标记飞行数据验证中（地面采样距离约0.85厘米/像素），该方法实现亚米级精度：水平面误差约0.87米，垂直方向误差约0.12米，每帧处理时间在1.47-4.91秒之间。

Conclusion: 基于束调整的度量引导提供了与经典摄影测量方法相当的一致性，同时显著加速处理，实现了实时3D地图生成，为时间关键地理空间应用提供了可行解决方案。
...
### [39] [E-VLA: Event-Augmented Vision-Language-Action Model for Dark and Blurred Scenes](https://arxiv.org/abs/2604.04834)
*Jiajun Zhai,Hao Shi,Shangwei Guo,Kailun Yang,Kaiwei Wang*

Main category: cs.CV

TL;DR: E-VLA框架通过事件相机增强视觉-语言-动作模型，在极端光照和运动模糊等恶劣条件下显著提升机器人操作的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 传统基于帧的视觉在极端低光、运动模糊等感知退化条件下表现脆弱，限制了VLA模型在实际机器人操作中的可靠性

Method: 提出事件增强的VLA框架，直接利用事件流中的运动和结构线索，开发轻量级事件集成策略，包括事件窗口化和融合方法，并构建了真实世界的RGB-事件-动作数据集

Result: 在20lux低光条件下，Pick-Place任务成功率从0%提升到60%（简单融合）和90%（事件适配器）；在严重运动模糊下，Pick-Place从0%提升到20-25%，Sorting从5%提升到32.5%

Conclusion: 事件驱动感知可以有效地集成到VLA模型中，为实现超越传统帧式成像的鲁棒具身智能提供了系统证据
...
Conclusion: ComPrivDet提供了一种高效的压缩视频隐私对象检测解决方案，在保持高准确率的同时显著降低计算开销和延迟，适用于大规模物联网视频分析场景。
...
TL;DR: 提出CSRS方法解决MLLM自进化中多数投票的偏见问题，通过回溯重推理机制、软化频率奖励和视觉语义扰动提升推理性能


<details>
  <summary>Details</summary>
Motivation: 现有MLLM自进化方法依赖多数投票选择伪黄金答案，但高频答案可能源于模型内在偏见而非客观正确性，导致性能退化

Method: 提出CSRS框架：1) 回溯重推理机制(RRM)从锚点重新推理探索长尾路径；2) 软化频率奖励(SFR)用连续信号替代二元奖励；3) 视觉语义扰动(VSP)确保模型关注数学逻辑而非视觉表象

Result: CSRS显著提升Qwen2.5-VL-7B在MathVision等基准上的推理性能，在几何任务的无监督自进化中达到SOTA结果

Conclusion: CSRS通过改进反馈信号质量，有效解决了MLLM自进化中的偏见问题，实现了更稳定有效的学习
...
TL;DR: ART模型通过时间感知关系图捕捉交互演化，自适应交互剪枝减少冗余计算，在行人轨迹预测任务中实现高精度和高效率


<details>
  <summary>Details</summary>
Motivation: 现有基于图或Transformer的方法存在计算开销大或难以表示多样化、时变的人类交互特征的局限性，需要更高效准确的行人轨迹预测方法

Method: 提出自适应关系Transformer（ART），包含时间感知关系图（TARG）显式捕捉成对交互演化，以及自适应交互剪枝（AIP）机制高效减少冗余计算

Result: 在ETH/UCY和NBA基准测试中，ART实现了最先进的准确性，同时保持高计算效率

Conclusion: ART通过显式建模交互演化和自适应剪枝，在行人轨迹预测任务中平衡了准确性和计算效率，为机器人相关应用提供了有效解决方案
...
TL;DR: DreamPRVR提出了一种从粗到细的表示学习范式，通过生成全局语义寄存器来增强部分相关视频检索的性能，解决了现有方法在查询模糊性和局部噪声方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有部分相关视频检索方法存在全局上下文感知不完整的问题，难以处理查询模糊性和由虚假响应引起的局部噪声，需要更有效的表示学习方法来提升检索性能。

Method: 采用从粗到细的表示学习范式：1）生成全局上下文语义寄存器作为粗粒度高亮；2）通过概率变分采样器和文本监督截断扩散模型迭代精炼寄存器；3）文本语义结构学习构建良好形成的文本潜在空间；4）通过寄存器增强的高斯注意力块自适应融合寄存器与视频标记。

Result: 大量实验表明DreamPRVR在部分相关视频检索任务上超越了现有最先进方法，证明了其有效性。

Conclusion: DreamPRVR通过从粗到细的表示学习范式，特别是全局语义寄存器的生成和优化，有效提升了部分相关视频检索的性能，解决了查询模糊性和局部噪声问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有VICL提示检索方法主要关注图像相似性而忽略标签一致性，导致可能选择视觉相似但标签不一致的提示，从而降低性能。研究发现查询与提示之间的标签一致性越高，VICL效果越好。

Method: 提出LaPR框架：1) 设计图像-标签联合表示以显式整合标签信息；2) 引入混合专家机制的双编码器，每个专家捕获特定标签模式，路由器推断查询自适应的混合权重；3) 采用交替优化策略，分别使用VICL性能引导的对比损失和标签引导的对比损失。

Result: 在上下文分割、检测和着色任务上均显示出显著且一致的性能提升。LaPR在不同特征提取器和交叉验证场景中泛化良好，证明了标签利用在VICL提示检索中的重要性。

Conclusion: 标签一致性对VICL性能至关重要，LaPR框架通过标签感知的提示检索有效提升了视觉上下文学习的效果，为VICL提示选择提供了新的方向。
...
TL;DR: 本文提出了一种基于视觉大语言模型的方法，通过改进视觉定位能力和理解用户意图来预测第一人称视角下的人-物交互，在HD-EPIC数据集上超越了现有最佳方法。


<details>
  <summary>Details</summary>
Motivation: 在智能辅助系统中，预测人-物交互能力对于指导用户日常活动和理解其短期与长期目标至关重要。现有方法在视觉定位和用户意图理解方面存在局限性，需要改进。

Method: 使用视觉大语言模型，通过Set-of-Mark提示改进视觉定位能力，利用用户最近注视点形成的轨迹理解用户意图，并引入新颖的逆指数采样策略来捕捉交互前的时序动态。

Result: 在HD-EPIC第一人称数据集上的实验表明，该方法超越了当前最先进的方法，并显示出其模型无关的特性。

Conclusion: 该方法通过改进视觉定位和理解用户意图，有效解决了第一人称视角下人-物交互预测问题，在性能上超越了现有方法，具有模型无关的优势。
...
<details>
  <summary>Details</summary>
Motivation: 解决事件相机和4D雷达等新型传感器数据稀缺问题，支持系统研究不同传感器的特性，促进跨传感器组合的公平比较。

Method: 收集多传感器数据并建立统一的3D和2D基准测试，提出融合框架将传感器特定线索集成到统一特征空间，提升3D检测在多变天气和光照下的鲁棒性。

Result: 建立了包含精确2D/3D边界框、轨迹ID和车辆里程计的数据集，提供了代表性单模态和多模态方法的基线结果，支持不同融合策略和传感器组合的研究。

Conclusion: DSERT-RoLL数据集为多传感器融合研究提供了全面平台，特别有助于事件相机和4D雷达等新型传感器的开发，通过统一的基准测试促进自动驾驶感知系统的进步。
...
TL;DR: 该论文研究了科学图像领域的长尾识别问题，发现基础模型在科学图像上微调效果有限，提出了利用多级表示的SciLT框架来提升头尾类别的平衡性能。


<details>
  <summary>Details</summary>
Motivation: 现有长尾识别研究和基准主要局限于自然图像领域，而科学图像具有独特的视觉特征和监督信号，基础模型在科学图像上的微调效果存在疑问，需要探索科学长尾识别的有效方法。

Method: 提出SciLT框架，通过自适应特征融合和双重监督学习利用多级表示，联合利用倒数第二层和最终层特征，实现头尾类别的平衡性能。

Result: 在三个科学基准测试上的实验表明，SciLT框架持续优于现有方法，为科学长尾识别建立了强大实用的基准，并为适应具有显著领域偏移的科学数据提供了有价值的指导。

Conclusion: 科学长尾识别需要专门的方法来处理领域偏移，SciLT框架通过多级特征利用有效提升了基础模型在科学图像上的性能，为科学图像分析提供了新的解决方案。
...
TL;DR: 本文揭示了深度学习图像水印中"编码器-噪声层-解码器"架构对已知原始攻击的脆弱性，并提出ResGuard增强模块来提升安全性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习图像水印系统主要针对随机信道失真设计，但忽视了攻击者拥有原始-水印图像对时的针对性攻击（已知原始攻击KOA），这种攻击能几乎完全去除水印而保持视觉质量。

Method: 提出ResGuard模块：1）残差特异性增强损失，强制残差与宿主图像紧密耦合；2）辅助KOA噪声层，在训练中注入残差式扰动，使解码器在更强的嵌入不一致性下保持可靠。

Result: ResGuard显著提升KOA鲁棒性，将平均水印提取准确率从59.87%提高到99.81%，同时保持对传统攻击的鲁棒性。

Conclusion: 深度学习水印系统需要更强的图像依赖性来抵御已知原始攻击，ResGuard作为即插即用模块能有效增强现有框架的安全性。
...
TL;DR: FunFact框架从RGB-D图像构建概率开放词汇功能3D场景图，通过联合概率推理解决功能关系模糊性，在多个数据集上表现优于现有方法。
...
Method: FunFact首先构建对象和部件中心的3D地图，使用基础模型提出语义合理的功能关系候选，将其转换为因子图变量，并受LLM常识先验和几何先验约束，进行联合概率推理。

Result: 在SceneFun3D、FunGraph3D和FunThor数据集上的实验表明，FunFact提高了节点和关系发现的召回率，显著减少了模糊关系的校准误差。

Conclusion: 整体概率建模对功能场景理解有益，FunFact框架通过联合推理功能关系及其边缘概率，提供了更好的校准置信度分数。
...
<details>
  <summary>Details</summary>
Motivation: 现有交通视频理解方法缺乏结构化表示和可解释的推理过程，需要一种能够结合场景结构化信息与多模态推理的框架来提升复杂交通场景的理解能力。

Method: SGTA采用模块化框架：1)从路边视频构建交通场景图（包含检测、跟踪和车道提取）；2)使用ReAct方法处理大型语言模型的交错推理轨迹和工具调用；3)结合符号图查询和视觉输入进行工具化推理。

Result: 在TUMTraffic VideoQA数据集样本上的实验表明，SGTA在多种问题类型上取得了有竞争力的准确率，同时提供了透明的推理步骤，证明了其有效性。

Conclusion: 将结构化场景表示与多模态智能体相结合在交通视频理解方面具有巨大潜力，SGTA框架为实现可解释的复杂视频问题决策提供了有效途径。
...
TL;DR: VidNum-1.4K是一个包含1,379个人工标注视频问答对的基准测试，专门评估视觉语言模型在多样化真实世界场景中的数值推理能力，揭示当前模型在理解时间动态、物体持久性和组合逻辑方面存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有视频数值推理基准测试局限于狭窄领域（如重复运动），或将简单计数视为表面回归任务，无法评估真实世界多媒体内容中的多步数值逻辑。需要创建能真正测试视觉语言模型是否理解现实世界动态的基准。

Method: 构建VidNum-1.4K基准测试，包含1,379个严格人工标注的视频问答对，涵盖物体、动作和事件量化。采用三层层次结构：从直接视觉感知到视频组合数值推理，要求模型基于时间证据执行算术运算、比较和逻辑推理。

Result: 评估多个最先进的视觉语言模型显示显著推理差距：Gemini-3.1-pro仅达到60%准确率阈值，代表性开源模型在25%-45%范围内挣扎。这表明当前模型缺乏稳定的"内部世界模型"。

Conclusion: VidNum-1.4K揭示了当前视觉语言模型在真实世界数值推理方面的严重不足，可作为下一代数值视频智能的严格诊断测试平台，推动模型发展更深入的理解能力。
...
TL;DR: XSeg是目前最大的X射线违禁品分割数据集，包含98,644张图像和295,932个实例掩码，涵盖30种常见违禁品类别。为解决标注难题，提出了基于SAM的Adaptive Point SAM（APSAM）模型，通过能量感知编码器和自适应点生成器提升重叠物品检测和标注效率。
...
Method: 1) 构建XSeg数据集：整合公开数据集和合成数据，通过定制数据清洗管道过滤低质量样本；2) 提出APSAM标注模型：基于SAM架构，引入能量感知编码器增强掩码解码器初始化，改进重叠物品检测；3) 设计自适应点生成器：仅需单个粗略点提示即可获得精确掩码标签。

Result: XSeg成为目前最大的X射线违禁品分割数据集，包含98,644张图像和295,932个实例掩码，涵盖30个最新常见违禁品类别。APSAM在XSeg上表现出优越性能，显著提升了标注效率和准确性。

Conclusion: XSeg数据集和APSAM标注模型为解决X射线违禁品分割任务提供了重要资源和技术支持，通过大规模高质量数据和高效标注方法，推动了该领域的发展。
...
TL;DR: 该研究提出了一种基于图学习的黑色素瘤检测方法，使用超像素集合图(SEG)和超像素层次图(SHG)两种图表示，通过优化学习边权重，在ISIC2017数据集上取得了99.00%的准确率和99.59%的AUC。


<details>
  <summary>Details</summary>
Motivation: 传统的图信号处理方法中，图结构和边权重通常通过统计和计算方法设定，而图结构学习方法能提供更可靠和灵活的数据表示。本研究旨在开发一种基于图学习的黑色素瘤检测方法，提高皮肤镜图像分析的性能。

Method: 提出两种图表示：超像素集合图(SEG)和超像素层次图(SHG)。生成多级超像素映射（20、40、60、80、100个节点），探索两种边权重分配技术：手工设计的高斯权重和基于优化方法的学习权重。节点信号基于纹理、几何和颜色超像素特征。研究图边阈值化效果（25%、50%、75%阈值）。在ISIC2017数据集上使用不同分类器进行实验，通过数据增强缓解类别不平衡。

Result: 学习型超像素集合图结合纹理节点信号获得最佳性能，准确率达到99.00%，AUC达到99.59%。结果表明图学习方法和纹理特征在黑色素瘤检测中具有显著优势。

Conclusion: 该研究证明了图学习方法在生物医学图像分析中的有效性，特别是超像素集合图结合学习边权重和纹理特征在黑色素瘤检测中表现出卓越性能，为皮肤镜图像分析提供了新的有效工具。
...
TL;DR: 提出一种从多视角图像重建高保真头发的紧凑管道，通过将发丝聚类为代表性发卡并分组到共享纹理码本中，结合3D高斯泼溅渲染，显著减少重建时间和存储成本。
...
Method: 1. 将发丝聚类为代表性发卡；2. 将这些发卡分组到共享纹理码本中；3. 将此结构与3D高斯泼溅渲染集成；4. 提出生成先验加速方法，从一组图像重建初始发丝几何。

Result: 实现了4倍的发丝重建时间减少，渲染性能相当，内存占用降低200倍以上，同时保持可比的视觉质量。

Conclusion: 该方法通过利用头发结构相似性，显著优化了多视角图像头发重建的效率和存储需求，为高保真头发重建提供了一种紧凑高效的解决方案。
...
Method: SymphoMotion包含两个核心机制：1) 相机轨迹控制机制，整合显式相机路径和几何感知线索，确保稳定的视角转换；2) 物体动态控制机制，结合2D视觉引导和3D轨迹嵌入，实现深度感知的空间一致物体操控。此外还构建了RealCOD-25K数据集，包含配对的相机姿态和物体级3D轨迹。
...
<details>
  <summary>Details</summary>
Motivation: 现有商业模型（如Sora2）在多参考角色视频生成方面取得进展，但学术研究有限。当参考图像外观高度相似时，模型容易出现参考混淆问题，语义相似token会降低模型检索正确上下文的能力
...
Result: 大量实验表明，与各种基线方法相比，PoCo提高了跨镜头一致性和参考保真度，能够可靠地控制具有极其相似视觉特征的角色

Conclusion: PoCo通过位置编码作为上下文控制器，有效解决了多参考图像相似时的混淆问题，为多参考多镜头视频生成提供了可靠的技术方案
...
TL;DR: SA-DSVN是一个3D卷积架构，利用宇宙射线μ子断层扫描对钢筋混凝土结构缺陷进行体素级分割，通过联合处理散射运动学数据和次级电磁簇射多重性数据，显著提升了缺陷检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统μ子断层扫描方法（如POCA、MLSD）仅依赖μ子散射角度进行重建，忽略了次级电磁簇射多重性这一重要特征。本研究旨在开发一种能够同时利用这两种信息源的新型深度学习架构，以提高钢筋混凝土结构缺陷检测的准确性和效率。

Method: 提出SA-DSVN 3D卷积架构，包含两个独立的编码器流：一个处理散射运动学数据（9个通道），另一个处理次级电磁簇射多重性数据（40个通道）。通过交叉注意力机制融合这两个流的信息。使用Vega云原生Geant4模拟框架生成训练数据，包含450万个μ子事件和900个体积，涵盖蜂窝、剪切断裂、腐蚀空洞和分层四种缺陷类型。

Result: 在60个独立模拟验证体积上，模型达到96.3%的体素精度，各类缺陷的Dice分数在0.59-0.81之间，体积级检测灵敏度达到100%，每个体积推理时间仅10毫秒。消融研究表明，仅使用簇射多重性流就能将缺陷平均Dice从0.535（仅散射）提升到0.685。

Conclusion: 次级电磁簇射多重性是一个先前未被充分利用但非常有效的特征，对于学习的μ子断层扫描重建具有重要意义。SA-DSVN架构通过融合散射和簇射信息，显著提升了钢筋混凝土结构缺陷检测的性能。
...
TL;DR: ICBench是一个新的大规模图像描述基准，包含12个内容类别，在2K张图像上由10个先进MLLM生成了40K个短描述和长描述，并提出了基于图像-文本-图像框架的自动评估指标ITIScore。


<details>
  <summary>Details</summary>
Motivation: 现有图像描述基准存在描述长度多样性有限、缺乏最新先进MLLM、人工标注不足等问题，这些限制可能导致偏见，并限制全面评估现代MLLM性能的能力。

Method: 1) 构建ICBench基准：涵盖12个内容类别，在2K张图像上由10个先进MLLM生成短描述和长描述，共40K个描述；2) 进行广泛的主观研究，获取细粒度评估维度的平均意见分数；3) 提出ITIScore自动评估指标，基于图像-文本-图像框架，通过重建一致性衡量描述质量。

Result: 实验结果表明，自动指标ITIScore与人类判断高度一致，并在其他公共描述数据集上展现出强大的零样本泛化能力。数据集和模型将在发表后发布。

Conclusion: ICBench解决了现有图像描述基准的局限性，提供了更全面的评估框架，ITIScore作为自动评估指标与人类判断高度一致，有助于更好地评估现代MLLM的图像描述能力。
...
TL;DR: M2StyleGS是一种实时3D风格迁移方法，使用3D高斯泼溅和CLIP多模态知识，支持文本或图像作为风格参考，通过精确特征对齐解决异常变换问题。
...
Method: 使用3D高斯泼溅作为3D表示，CLIP多模态知识作为风格参考，采用细分流进行精确特征对齐，引入观察损失和抑制损失优化生成过程。

Result: M2StyleGS能够生成视觉质量更好的风格化新视角序列，在一致性方面比先前工作提升高达32.92%。

Conclusion: M2StyleGS通过多模态参考和精确特征对齐，实现了高质量的实时3D风格迁移，支持文本和图像输入，在视觉质量和一致性方面表现优异。
...
TL;DR: 提出了一种自动化框架，利用粗糙的CityGML建筑模型作为几何先验，通过整合高精度MLS数据来精细化建筑立面几何，同时保持拓扑有效性。


<details>
  <summary>Details</summary>
Motivation: 数字孪生需要高精度地理空间数据，但传统的CityGML建筑模型（源自机载激光扫描）在立面几何上存在显著缺陷，需要与移动激光扫描数据整合来恢复详细立面几何。

Method: 利用粗糙模型作为几何先验，通过表面匹配识别过时表面，采用二进制整数优化从候选数据中选择最优面片，并在优化中强制执行硬约束以确保拓扑有效性。

Result: 该方法有效修正了立面错位，将云到网格均方根误差降低了约36%，实现了厘米级对齐，同时保证了严格的水密性和流形几何。
...
TL;DR: MoScale是一个从粗到细的分层自回归运动生成框架，通过多尺度预测和跨尺度精炼提升文本到运动生成的性能


<details>
  <summary>Details</summary>
Motivation: 传统自回归模型的下一token预测与文本条件运动生成所需的时间结构不匹配，需要更好的层次化因果结构来处理长距离运动结构

Method: 提出next-scale自回归框架，从粗到细的时间分辨率分层生成运动，结合跨尺度层次精炼和尺度内时间精炼来提升预测质量

Result: 在文本到运动生成任务上达到SOTA性能，训练效率高，模型规模可扩展，并能零样本泛化到多种运动生成和编辑任务

Conclusion: MoScale通过分层自回归框架解决了传统AR模型在运动生成中的结构对齐问题，在有限数据下表现出鲁棒性和泛化能力
...
TL;DR: HistoFusionNet：一种基于Transformer增强的夜间图像去雾架构，结合直方图引导表示学习和频率自适应特征细化，在NTIRE 2026夜间图像去雾挑战中排名第一


<details>
  <summary>Details</summary>
Motivation: 夜间图像去雾面临雾霾、光晕、非均匀光照、颜色失真和传感器噪声等多种退化问题，这些因素使传统白天去雾假设失效，需要专门针对夜间复杂光照条件的解决方案

Method: 基于多尺度编码器-解码器架构，引入直方图Transformer块，通过动态范围特征分组建模长程依赖；同时加入频率感知细化分支，自适应利用低频和高频线索互补信息

Result: 在NTIRE 2026夜间图像去雾挑战基准测试中表现优异，在22个参赛团队中排名第一，证明了方法的有效性和鲁棒性

Conclusion: HistoFusionNet通过直方图引导表示学习和频率自适应特征细化的统一框架，有效处理夜间雾霾场景中的异质退化问题，在夜间图像去雾任务中取得了最先进的性能
...
TL;DR: 提出基于注意力分布熵的补丁剪枝方法，通过Shannon熵和Rényi熵识别重要补丁，在细粒度图像识别任务中减少计算量同时保持精度。
...
Method: 提出基于注意力分布熵的补丁重要性评估准则：1) 使用Shannon熵，低熵补丁（注意力集中）被保留为重要，高熵补丁（注意力分散）被视为冗余；2) 扩展到Rényi熵，强调尖锐的注意力峰值，支持适应任务需求和计算限制的剪枝策略。

Result: 在细粒度图像识别任务中，该方法在保持准确性的同时减少了计算量。通过Rényi熵调整剪枝策略可进一步获得增益，改善准确性与计算量之间的权衡。

Conclusion: 基于熵的补丁剪枝方法有效降低了Transformer的计算成本，同时保持了模型性能。Rényi熵扩展提供了更灵活的剪枝策略，可根据任务需求和计算限制进行调整。
...
TL;DR: EyeBench-V2是一个用于眼底图像增强模型的基准测试框架，旨在解决现有评估方法在临床相关性、统一评估协议和实用指导方面的不足。
...
Method: 提出EyeBench-V2基准测试框架，包含三个关键贡献：1）通过下游任务进行多维临床对齐评估，包括血管分割、糖尿病视网膜病变分级、对未见噪声模式的泛化能力和病灶分割；2）专家指导的评估设计，包括用于公平比较配对和非配对方法的新数据集，以及由医学专家设计的结构化手动评估协议；3）提供可操作的见解，对现有生成模型进行严格的任务导向分析。

Result: EyeBench-V2提供了一个全面的评估框架，能够更准确地评估眼底图像增强模型的临床实用性，为临床研究人员提供决策依据，并识别当前方法的局限性以指导下一代增强模型的设计。

Conclusion: EyeBench-V2基准测试填补了眼底图像增强模型评估与临床实用性之间的差距，通过多维临床对齐评估、专家指导的设计和可操作的见解，为临床研究人员提供了更有效的评估工具，并推动了临床对齐增强模型的发展。
...
TL;DR: ActivityForensics：首个大规模视频活动伪造定位基准，包含6000+伪造视频片段，提出TADiff基线方法，评估多种伪造定位器性能


<details>
  <summary>Details</summary>
Motivation: 现有视频伪造检测基准主要关注外观级伪造（如人脸交换、物体移除），但视频生成技术的发展催生了活动级伪造（修改人类动作以扭曲事件语义），这种高度欺骗性的伪造严重威胁媒体真实性和公众信任，需要专门的检测方法

Method: 提出ActivityForensics基准数据集，包含6000多个无缝融入视频上下文的伪造片段；提出Temporal Artifact Diffuser（TADiff）基线方法，通过基于扩散的特征正则化器暴露伪造痕迹

Result: 建立了首个大规模活动伪造定位基准，提供了全面的评估协议（域内、跨域和开放世界设置），并对多种最先进的伪造定位器进行了基准测试

Conclusion: ActivityForensics填补了活动级视频伪造检测的空白，TADiff方法简单有效，为未来研究提供了重要基准和工具，数据集和代码已开源
...
### [66] [Task-Guided Multi-Annotation Triplet Learning for Remote Sensing Representations](https://arxiv.org/abs/2604.03837)
*Meilun Zhou,Alina Zare*

Main category: cs.CV

TL;DR: 提出任务引导的多标注三元组损失，通过互信息标准选择跨任务信息量最大的三元组，替代静态权重平衡，提升共享表征效果


<details>
  <summary>Details</summary>
Motivation: 现有多任务三元组损失方法依赖静态权重来平衡不同类型标注的监督，但静态权重需要调参且无法考虑任务在塑造共享表征时的交互作用

Method: 提出任务引导的多标注三元组损失，通过互信息标准选择跨任务信息量最大的三元组，修改影响表征的样本选择而非调整损失幅度

Result: 在航空野生动物数据集上的实验表明，相比多种三元组损失设置，任务引导选择在分类和回归任务上均获得性能提升

Conclusion: 任务感知的三元组选择能够产生更有效的共享表征，适用于下游任务，解决了静态权重方法的局限性
...
### [67] [Training a Student Expert via Semi-Supervised Foundation Model Distillation](https://arxiv.org/abs/2604.03841)
*Pardis Taghavi,Tian Liu,Renjie Li,Reza Langari,Zhengzhong Tu*

Main category: cs.CV

TL;DR: 提出半监督知识蒸馏框架，将预训练视觉基础模型压缩为紧凑专家模型，在实例分割任务上使用有限标注数据和大量无标注数据，实现11倍压缩同时性能超越原始模型


<details>
  <summary>Details</summary>
Motivation: 基础模型感知能力强但计算量大难以部署，且适应新任务需要昂贵标注。需要一种方法能在有限标注下压缩基础模型为紧凑专家

Method: 三阶段框架：1) 通过自训练和对比校准进行域适应；2) 通过统一多目标损失进行知识转移；3) 学生模型精炼以减轻伪标签偏差。核心是实例感知的像素级对比损失，融合掩码和类别分数提取信息负样本

Result: 在Cityscapes和ADE20K数据集上，约11倍小的学生模型比零样本基础模型教师提升+11.9和+8.6 AP，比适应后的教师提升+3.4和+1.5 AP，超越最先进的SSKD方法

Conclusion: 提出的半监督知识蒸馏框架能有效压缩视觉基础模型，在有限标注下实现高性能紧凑模型，通过对比学习保持嵌入对齐，显著提升实例分割性能
...
### [68] [SafeCtrl: Region-Aware Safety Control for Text-to-Image Diffusion via Detect-Then-Suppress](https://arxiv.org/abs/2604.03941)
*Lingyun Zhang,Yu Xie,Zhongli Fang,Yu Liu,Ping Chen*

Main category: cs.CV

TL;DR: SafeCtrl：一种基于检测-抑制范式的区域感知安全控制框架，用于文本到图像扩散模型，通过精确定位风险区域并局部抑制有害内容，在安全性和保真度之间取得更好平衡


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型面临生成视觉有害内容（如色情、暴力、恐怖图像）的挑战。现有安全干预方法存在两个关键限制：1）安全性与上下文保真度之间的严重权衡；2）容易受到对抗性攻击的脆弱性

Method: 提出SafeCtrl框架，采用检测-抑制范式：1）注意力引导的检测模块精确定位特定风险区域；2）局部抑制模块通过图像级直接偏好优化（DPO）仅在中检测区域内中和有害语义，将不安全对象转化为安全替代品，同时保持周围上下文完整

Result: 在多个风险类别上的广泛实验表明，SafeCtrl在安全性和保真度之间实现了优于现有方法的权衡。关键的是，该方法对对抗性提示攻击表现出更强的韧性

Conclusion: SafeCtrl提供了一种精确且鲁棒的解决方案，用于负责任的内容生成，通过区域感知的安全控制有效解决了现有方法的局限性
...
### [69] [Multimodal Structure Learning: Disentangling Shared and Specific Topology via Cross-Modal Graphical Lasso](https://arxiv.org/abs/2604.03953)
...
<details>
  <summary>Details</summary>
Motivation: 传统的稀疏图估计技术（如图形套索）在视觉-语言领域面临高维噪声、模态不对齐以及共享与类别特定拓扑结构混淆等瓶颈问题，需要新的方法来学习可解释的多模态表示。

Method: 提出跨模态图形套索（CM-GLasso），结合文本-视觉对齐策略和统一的视觉-语言编码器，将多模态特征严格对齐到共享潜在空间。引入交叉注意力蒸馏机制，将高维补丁压缩为显式语义节点，提取空间感知的跨模态先验。通过定制化的GLasso估计和共同-特定结构学习（CSSL）联合目标，使用交替方向乘子法（ADMM）进行优化。

Result: 在涵盖自然和医学领域的八个基准测试中，CM-GLasso在生成式分类和密集语义分割任务上建立了新的最先进性能。

Conclusion: CM-GLasso克服了传统方法在多模态表示学习中的基本限制，能够同时解耦不变和类别特定的精度矩阵，避免了多步误差累积，为可解释的多模态表示学习提供了有效解决方案。
...
### [70] [VLA-Forget: Vision-Language-Action Unlearning for Embodied Foundation Models](https://arxiv.org/abs/2604.03956)
*Ravi Ranjan,Agoritsa Polyzou*

Main category: cs.CV

TL;DR: VLA-Forget：一种用于视觉-语言-动作模型的混合遗忘框架，通过选择性编辑感知层和跨模态特定性，结合分层选择性推理/动作遗忘，实现效用保留的遗忘。


<details>
  <summary>Details</summary>
Motivation: VLA模型作为机器人操作的基础模型，部署时面临新的遗忘挑战：需要移除不安全、虚假或隐私敏感行为，同时不降低感知、语言理解和动作控制能力。传统遗忘方法在VLA模型中效果有限，因为不良知识分布在感知、对齐和推理/动作多个层中，而非单一模块。

Method: 提出VLA-Forget混合遗忘框架，结合感知层的比例感知选择性编辑和跨模态特定性，以及推理/动作层的分层选择性遗忘。通过分阶段更新视觉编码器、投影器和上层动作生成变换器块，联合优化三个目标：目标遗忘、感知保留和推理保持。

Result: 相比强基线方法，VLA-Forget在遗忘集行为探测中提升遗忘效果10%，保留感知特定性22%，保持推理和任务成功率9%，并将量化后恢复率降低55%。

Conclusion: VLA-Forget为VLA模型提供了一种有效的遗忘解决方案，能够针对性地移除不良行为，同时最大程度地保留模型的感知和推理能力，在机器人操作等具身智能场景中具有重要应用价值。
...
### [71] [Gram-Anchored Prompt Learning for Vision-Language Models via Second-Order Statistics](https://arxiv.org/abs/2604.03980)
*Minglei Chen,Weilong Wang,Jiang Duan,Ye Deng*

Main category: cs.CV

TL;DR: GAPL提出了一种基于二阶统计量的提示学习方法，通过Gram矩阵增强视觉语言模型的适应能力，结合局部语义对齐和全局结构一致性，提高模型对领域偏移的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效的提示学习方法主要关注将文本提示与一阶视觉特征（空间特征图）对齐，虽然对细粒度语义区分有效，但仅依赖一阶信息不足以实现鲁棒适应，因为这些空间纠缠特征对领域偏移和局部噪声高度敏感。
...
Conclusion: 通过结合局部语义对齐和全局结构一致性，GAPL框架显著提高了视觉语言模型对领域偏移的鲁棒性，为参数高效的提示学习提供了新思路。
...
### [72] [High-Fidelity Mural Restoration via a Unified Hybrid Mask-Aware Transformer](https://arxiv.org/abs/2604.03984)
*Jincheng Jiang,Qianhao Han,Chi Zhang,Zheng Zheng*

Main category: cs.CV

TL;DR: HMAT是一种用于古代壁画修复的统一框架，结合了动态滤波和Transformer架构，能够同时处理局部纹理建模和长距离结构推理，在保持原始区域完整性的同时有效修复大面积缺失。


<details>
  <summary>Details</summary>
Motivation: 古代壁画作为珍贵的文化遗产，常因环境暴露、材料老化和人为活动而严重退化。修复这些艺术品具有挑战性，需要同时重建大面积缺失结构并严格保护未受损的真实区域。

Method: 提出混合掩码感知Transformer（HMAT）框架，整合掩码感知动态滤波用于鲁棒的局部纹理建模，以及Transformer瓶颈用于长距离结构推理。引入掩码条件风格融合模块处理退化的多样性，设计带有硬门控跳跃连接的教师强制解码器，强制有效区域的保真度并聚焦于缺失区域的重建。

Result: 在DHMural数据集和精心策划的九色鹿数据集上评估，HMAT在不同退化水平下表现出色，相比最先进方法具有竞争力，同时产生更结构连贯和视觉忠实的修复结果。
...
### [73] [A Systematic Study of Cross-Modal Typographic Attacks on Audio-Visual Reasoning](https://arxiv.org/abs/2604.03995)
...
<details>
  <summary>Details</summary>
Motivation: 随着音频-视觉多模态大语言模型在安全关键应用中的部署日益增多，理解其脆弱性变得至关重要。现有研究主要关注单模态攻击，而多模态攻击的交叉影响尚未得到充分探索。

Method: 提出多模态排版攻击方法，系统研究跨模态的排版攻击如何影响MLLMs。分析音频、视觉和文本扰动之间的相互作用，展示协调的多模态攻击比单模态攻击更具威胁性。

Result: 在多模态攻击下，攻击成功率显著提升至83.43%，而单模态攻击仅为34.93%。研究在多个前沿MLLMs、任务以及常识推理和内容审核基准测试中验证了这一发现。
...
### [74] [OASIC: Occlusion-Agnostic and Severity-Informed Classification](https://arxiv.org/abs/2604.04012)
...
<details>
  <summary>Details</summary>
Motivation: 严重遮挡是计算机视觉的主要挑战，根源在于可见信息丢失和遮挡物带来的干扰模式。现有方法未能同时解决这两个问题。

Method: 提出OASIC方法：1）测试时通过掩蔽遮挡物去除干扰模式；2）训练时对物体进行随机掩蔽以处理不同遮挡程度；3）测试时估计遮挡严重程度并选择针对该程度优化的模型。

Result: 结合灰度掩蔽和自适应模型选择，相比标准遮挡图像训练提升AUC_occ 18.5分，相比未遮挡图像微调提升23.7分，优于任何单一模型。

Conclusion: OASIC通过同时处理遮挡干扰和可见信息损失，并利用遮挡严重程度感知的模型选择策略，显著提升了遮挡场景下的分类性能。
...
### [75] [HOIGS: Human-Object Interaction Gaussian Splatting](https://arxiv.org/abs/2604.04016)
*Taewoo Kim,Suwoong Yeom,Jaehyun Pyun,Geonho Cha,Dongyoon Wee,Joonsik Nam,Yun-Seong Jeong,Kyeongbo Kong,Suk-Ju Kang*

Main category: cs.CV

TL;DR: HOIGS提出了一种显式建模人-物交互变形的高斯溅射方法，通过交叉注意力HOI模块和异构特征提取，在遮挡、接触和物体操作场景中实现更好的动态场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有高斯溅射方法要么依赖人体姿态先验而忽略动态物体，要么将所有运动近似在单一场中，限制了捕捉交互丰富动态的能力。需要显式建模人-物交互引起的变形。

Method: 提出Human-Object Interaction Gaussian Splatting (HOIGS)，通过交叉注意力HOI模块显式建模人-物交互变形。采用异构特征提取：HexPlane用于人体，Cubic Hermite Spline (CHS)用于物体，然后集成这些特征捕捉相互依赖的运动。

Result: 在多个数据集上的综合实验表明，该方法在遮挡、接触和物体操作场景中有效改善了变形估计，一致优于最先进的人体中心和4D高斯方法。
...
### [76] [1.x-Distill: Breaking the Diversity, Quality, and Efficiency Barrier in Distribution Matching Distillation](https://arxiv.org/abs/2604.04018)
*Haoyu Li,Tingyan Wen,Lin Qi,Zhe Wu,Yihuang Chen,Xing Zhou,Lifei Zhu,Xueqian Wang,Kai Zhang*

Main category: cs.CV

TL;DR: 1.x-Distill是一种分数步蒸馏框架，突破了传统整数步限制，实现了1.x步生成，在SD3-Medium和SD3.5-Large上分别以1.67和1.74有效NFEs达到更好质量和多样性，相比原始28x2 NFE采样实现33倍加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能产生高质量的文本到图像结果，但其迭代去噪过程计算成本高昂。现有的Distribution Matching Distillation（DMD）方法在减少到两步或更少步骤时会出现多样性崩溃和保真度下降的问题，需要突破整数步限制，实现更高效的分数步蒸馏。

Method: 1.x-Distill包含三个关键技术：1）分析教师CFG在DMD中的作用并引入简单有效的修改来抑制模式崩溃；2）提出阶段化聚焦蒸馏的两阶段策略，先通过多样性保持分布匹配学习粗粒度结构，再通过推理一致对抗蒸馏细化细节；3）设计轻量级补偿模块实现蒸馏-缓存协同训练，将块级缓存自然融入蒸馏流程。

Result: 在SD3-Medium和SD3.5-Large上的实验表明，1.x-Distill超越了先前的少步方法，分别在1.67和1.74有效NFEs下实现了更好的质量和多样性，相比原始28x2 NFE采样实现了高达33倍的加速。

Conclusion: 1.x-Distill首次突破了整数步限制，建立了1.x步生成作为蒸馏扩散模型的实用方案，通过创新的分数步蒸馏框架、两阶段策略和缓存协同训练，在极少数步骤下实现了高质量、高多样性的图像生成。
...
### [77] [ATSS: Detecting AI-Generated Videos via Anomalous Temporal Self-Similarity](https://arxiv.org/abs/2604.04029)
...
Result: 在四个大规模基准测试（GenVideo、EvalCrafter、VideoPhy、VidProM）上的广泛实验表明，ATSS在AP、AUC和ACC指标上显著优于最先进方法，在不同视频生成模型中表现出优越的泛化能力。
...
TL;DR: 4C4D是一个从极稀疏相机（最少4个）视频中恢复4D动态场景的新框架，通过神经衰减函数增强4D高斯几何建模能力，实现高质量的新视角渲染。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要密集的多视角相机阵列（数十甚至上百个相机）来建模动态场景，这在实际应用中成本高昂且不便。本文旨在解决从极稀疏相机（最少4个便携相机）视频中恢复4D动态场景的挑战。

Method: 提出4C4D框架，核心是引入神经衰减函数作用于高斯不透明度，增强4D高斯的几何建模能力。该设计通过让4DGS梯度更多地关注几何学习，缓解了4DGS中几何与外观建模的固有不平衡问题。

Result: 在具有不同相机重叠度的稀疏视角数据集上进行广泛实验，4C4D相比现有方法取得了优越的性能表现。

Conclusion: 4C4D框架能够从极稀疏相机视频中实现高质量的4D高斯溅射，为动态场景建模提供了更实用和高效的解决方案。
...
TL;DR: 该论文提出了一种基于正例-无标签学习的方法，用于AtticPOT存储库中的重复发现，通过轻量级克隆编码器和可解释阈值实现高效的重复检测。


<details>
  <summary>Details</summary>
Motivation: 解决AtticPOT存储库中策展人参与的重复发现问题，传统方法需要显式负例样本，而实际场景中负例难以获取，因此采用正例-无标签学习框架。

Method: 将重复发现问题建模为正例-无标签学习问题，为每个文物提供单个锚点，训练轻量级克隆编码器，通过增强视图学习特征表示，在潜在空间使用l_2范数阈值进行可解释的相似度评分。

Result: 在CIFAR-10数据集上达到F1=96.37（AUROC=97.97），在AtticPOT数据集上达到F1=90.79（AUROC=98.99），比最佳基线方法（SVDD）提升7.70个F1点，同时保持轻量级架构。

Conclusion: 该方法避免了显式负例需求，提供了透明的操作点，适用于重复数据删除、记录链接和策展人参与的工作流程，在跨记录重复发现方面表现出色。
...
TL;DR: 本文提出了一种基于YOLOv11检测器和BoT-SORT/ByteTrack跟踪器的离线实时交通监控系统，通过PyTorch/OpenCV实现并在Qt桌面界面中封装，实现了高效的车辆检测与计数，准确率达66.67-95.83%。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能驱动的计算机视觉技术进步，交通监控系统得到了显著提升。本研究旨在开发一个离线、实时的交通监控系统，利用深度学习目标检测和跟踪技术，为智慧城市建设提供现代化解决方案。

Method: 采用预训练的YOLOv11检测器结合BoT-SORT/ByteTrack多目标跟踪算法，使用PyTorch/OpenCV实现CNN处理管道，并封装在基于Qt的桌面用户界面中，实现无需云依赖的视频流车辆检测与计数。

Result: 系统在不同场景下达到66.67-95.83%的计数准确率。车辆检测方面，汽车检测精度0.97-1.00，召回率0.82-1.00，F1分数0.90-1.00；卡车检测精度1.00，召回率0.70-1.00，F1分数0.82-1.00。在典型条件下性能稳健，但恶劣天气可能影响表现。

Conclusion: 通过集成轻量级模型和可访问的云独立界面，该系统展示了AI驱动交通监控系统的能力，为智慧城市的现代化和发展做出了贡献。
...
TL;DR: HGR框架通过可修正的假设节点和依赖感知图记忆，在探索未知环境时既能利用语义预测指导探索，又能系统性地撤回错误预测，提高导航效率和记忆可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的导航系统通常将未探索区域视为语义未知，导致探索效率低下。虽然视觉语言模型可以预测前沿语义，但错误预测会嵌入记忆并传播，造成结构错误累积，仅靠置信度衰减无法解决。需要一种既能利用语义预测指导探索，又能系统性撤回错误预测的框架。

Method: 提出假设图精炼(HGR)框架：1) 语义假设模块：估计前沿的上下文条件语义分布，根据目标相关性、旅行成本和不确定性对探索目标进行排序；2) 验证驱动的级联修正：将现场观察与预测语义进行比较，一旦发现不匹配，就撤回被反驳的节点及其所有下游依赖节点，使图能够通过剪枝错误子图而收缩。

Result: 在GOAT-Bench上实现72.41%的成功率和56.22%的SPL，在A-EQA和EM-EQA问答基准上表现一致提升。诊断分析显示级联修正消除了约20%的结构冗余假设节点，将错误区域的重复访问减少4.5倍，镜面和透明表面占修正预测错误的67%。

Conclusion: HGR框架通过依赖感知图记忆和验证驱动的级联修正，实现了可靠的长期记忆维护，在部分观察环境中提高了导航效率和语义预测的可靠性，特别是在处理镜面和透明表面等挑战性场景时表现优异。
...
TL;DR: NTIRE 2026 3D恢复与重建挑战赛综述，聚焦极端低光和烟雾退化条件下的3D重建方法评估


<details>
  <summary>Details</summary>
Motivation: 在真实世界恶劣条件下（特别是极端低光和烟雾退化环境）开发鲁棒的3D重建方法，通过RealX3D基准测试评估不同方法的性能

Method: 组织NTIRE 2026 3DRR挑战赛，收集279名参赛者中33个团队的有效提交，使用RealX3D基准测试评估方法性能，分析顶级方法的共同设计原则

Result: 挑战赛显示在恶劣条件下的3D重建取得显著进展，识别出顶级方法的共享设计模式，为处理3D场景退化提供了有效策略见解

Conclusion: 该挑战赛推动了恶劣条件下3D重建技术的发展，揭示了有效的设计原则，为未来研究提供了重要参考
...
TL;DR: 本文提出了一种针对空间非均匀曝光校正的新范式，通过空间自适应调制权重和不确定性引导的损失函数，解决了现有方法在非均匀曝光错误处理上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的曝光校正面临空间非均匀退化的根本挑战，单一图像中经常存在多种曝光错误共存的情况。然而现有方法大多基于均匀假设开发，依赖全局聚合的调制信号，且传统重建损失在全局尺度下忽略了区域间空间变化的校正需求。

Method: 提出空间信号编码器预测空间自适应调制权重，指导多个查找表进行图像变换，结合HSL补偿模块提升色彩保真度。此外提出不确定性引导的非均匀损失，基于局部恢复不确定性动态分配优化焦点。

Result: 大量实验表明，该方法在定性和定量性能上都优于现有最先进方法，在真实世界非均匀曝光校正任务中表现优异。

Conclusion: 本文提出的空间非均匀曝光校正新范式通过空间自适应调制和不确定性引导的优化策略，有效解决了现实世界中空间非均匀曝光错误的校正问题，为曝光校正领域提供了新的解决方案。
...
TL;DR: OP-GRPO：首个为流匹配模型设计的离线策略GRPO框架，通过重用高质量轨迹、序列级重要性采样校正和轨迹截断，将训练步数减少到Flow-GRPO的34.2%，显著提升训练效率


<details>
  <summary>Details</summary>
Motivation: GRPO（流匹配模型的强化学习后训练）虽然能显著提升生成质量，但由于其在线策略训练范式，存在样本效率低下的固有缺陷。需要开发更高效的训练方法来解决这一问题。

Method: 1. 主动选择高质量轨迹并自适应地将其纳入回放缓冲区以供后续训练重用；2. 提出序列级重要性采样校正，在保持GRPO剪裁机制完整性的同时缓解分布偏移；3. 理论分析和实验表明后期去噪步骤会产生病态离线策略比率，通过截断后期步骤的轨迹来缓解此问题。

Result: 在图像和视频生成基准测试中，OP-GRPO仅用Flow-GRPO平均34.2%的训练步数就实现了相当或更优的性能，在保持生成质量的同时获得了显著的训练效率提升。

Conclusion: OP-GRPO是首个专门为流匹配模型设计的离线策略GRPO框架，通过创新的轨迹重用、重要性采样校正和轨迹截断技术，有效解决了GRPO样本效率低的问题，为流匹配模型的高效训练提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在遥感应用中面临地理区域泛化问题，由于域偏移（如土地覆盖、气候等差异）导致模型在新区域表现下降。现有测试时自适应方法主要针对分类任务，不适用于回归任务，特别是时空融合这样的回归问题。

Method: 提出不确定性感知的测试时自适应框架，仅更新预训练时空融合模型的融合模块。该方法基于认知不确定性指导自适应过程，结合土地利用一致性约束和偏差校正，无需源数据或目标区域标注样本。

Result: 在意大利罗马、埃及开罗、西班牙马德里和法国蒙彼利埃四个不同气候的目标区域进行实验，相比在法国奥尔良预训练的模型，RMSE和MAE平均分别提升24.2%和27.9%，即使使用有限的无标签目标数据和仅10个自适应轮次。

Conclusion: 该方法有效解决了遥感回归任务中的域偏移问题，通过轻量级的测试时自适应显著提升了模型在新地理区域的泛化性能，为实际应用提供了实用解决方案。
...
TL;DR: 论文提出双曲空间共嵌入框架，通过蕴含关系而非简单配对对齐来建模字体与印象描述之间的对应关系，量化了印象描述对字体风格的约束强度（风格特异性）。


<details>
  <summary>Details</summary>
Motivation: 字体形状能唤起广泛印象，但字体与印象描述之间并非一一对应关系：某些印象与多种字体风格兼容，而其他印象则强烈限制可能的字体集合。这种分级约束强度被称为风格特异性，需要量化建模。

Method: 提出双曲空间共嵌入框架，将字体图像和印象描述（单标签或标签集）嵌入共享双曲空间，采用两种互补的蕴含约束：印象到字体的蕴含，以及印象间从低到高风格特异性的蕴含。这种表述诱导出径向结构，低特异性印象靠近原点，高特异性印象远离原点。

Result: 在MyFonts数据集上的实验显示，相比强的一对一基线方法，该方法在双向检索方面有改进。遍历和标签级分析表明，学习到的空间捕捉了从模糊到更风格特异性印象的连贯进展，提供了有意义的数据驱动的风格特异性量化。

Conclusion: 双曲共嵌入框架成功建模了字体-印象对应关系，通过蕴含关系而非简单对齐，提供了可解释的几何度量来量化印象对字体风格的约束强度，为风格特异性提供了数据驱动的量化方法。
...
Method: 1. 通过多视图共享码本和跨视图重建学习离散一致表示，在有限的共享码本嵌入中自然对齐不同视图；2. 设计权重估计方法评估各视图保留标签相关结构的能力，据此分配权重以提升融合预测质量；3. 提出融合教师自蒸馏框架，用融合预测指导视图特定分类器训练，将全局知识反馈到单视图分支。

Result: 在五个基准数据集上与先进方法进行了广泛的对比实验，充分证明了所提方法的有效性。代码已开源。

Conclusion: 提出的方法通过结构化的一致表示学习机制、基于标签相关结构的权重估计以及融合教师自蒸馏框架，有效解决了多视图多标签学习中的双缺失问题，提升了模型的泛化能力。
...
TL;DR: GENFIG1是一个评估生成式AI模型根据论文内容（标题、摘要、引言、图注）生成"Figure 1"能力的基准测试，要求模型理解科学概念、识别关键点并设计有效的视觉表达。


<details>
  <summary>Details</summary>
Motivation: 科学论文中的"Figure 1"通常作为核心研究思想的视觉总结，这些图视觉简洁但概念丰富，需要作者大量迭代才能完成，这凸显了科学视觉传达的难度。研究者希望评估AI模型在这方面的能力。

Method: 从顶级深度学习会议论文中收集数据，进行严格质量控制，引入与专家判断相关性好的自动评估指标。评估一系列代表性模型在GENFIG1基准上的表现。

Result: 评估结果显示，即使对于表现最佳的系统，GENFIG1任务也提出了显著挑战，表明当前模型在科学视觉传达方面仍有很大提升空间。

Conclusion: GENFIG1基准为多模态AI的未来发展奠定了基础，有助于推动生成式AI在科学可视化方面的进步。
...
TL;DR: AURA是一个端到端的流式视觉交互框架，让统一的视频大语言模型能够持续处理视频流，支持实时问答和主动响应，在流式基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型大多是离线的，不适合需要持续观察和及时响应的实时视频流。当前的流式方法依赖解耦的触发-响应管道或仅限于字幕式叙述，限制了开放问答和长时程交互的能力。

Method: 提出AURA框架，整合了上下文管理、数据构建、训练目标和部署优化，实现稳定的长时程流式交互。采用端到端架构，支持实时问答和主动响应。

Result: 在流式基准测试中达到最先进的性能，支持实时演示系统，在2个80G加速器上以2FPS运行，包含ASR和TTS功能。

Conclusion: AURA为流式视频理解提供了有效的端到端解决方案，发布了模型和实时推理框架以促进未来研究。
...
TL;DR: GraphicDesignBench (GDB) 是首个专门评估AI模型在专业平面设计任务上的综合基准套件，包含50个任务，涵盖布局、排版、信息图表、模板设计和动画五个维度，揭示当前模型在空间推理、矢量代码生成、排版感知和动画分解等核心设计挑战上的不足。
...
Method: GDB包含50个任务，组织在五个维度上：布局、排版、信息图表、模板与设计语义、动画。每个维度在理解和生成两种设置下评估，基于LICA分层组合数据集中的真实设计模板。使用标准化指标分类法评估，涵盖空间准确性、感知质量、文本保真度、语义对齐和结构有效性。

Result: 评估显示当前模型在专业设计的核心挑战上表现不足：复杂布局的空间推理、忠实矢量代码生成、细粒度排版感知、动画的时间分解等问题基本未解决。虽然高层语义理解已可实现，但随着任务对精度、结构和组合意识要求的提高，差距显著扩大。

Conclusion: GDB为追踪AI系统向能够作为有能力设计协作者发展的进展提供了一个严谨、可复现的测试平台。该基准揭示了当前AI在专业设计任务上的局限性，并为未来研究提供了明确的方向。
...
<details>
  <summary>Details</summary>
Motivation: 气候变化相关灾害日益频繁和严重，需要实时监测、早期预警和知情决策。现有基础模型依赖高分辨率但低重访率的卫星图像，不适合快速演变的灾害现象和时间敏感的应急响应。

Method: 提出HighFM基础模型，利用超过2TB的SEVIRI气象卫星数据，改进SatMAE掩码自编码框架，加入细粒度时间编码以捕捉短期变化，预训练后微调用于云掩码和活跃火灾检测任务。

Result: SEVIRI预训练的视觉Transformer模型在平衡准确率和IoU指标上均优于传统基线和近期地理空间基础模型，展示了时间密集的地球静止数据在实时地球观测中的潜力。

Conclusion: 高时间分辨率的地球静止数据为实时地球观测提供了重要价值，HighFM为灾害检测和追踪的基础模型提供了可扩展的路径，能够更好地支持快速演变现象和应急响应需求。
...
TL;DR: GA-GS是一种生成辅助的高斯溅射方法，用于从包含动态对象的单目视频中重建静态3D场景。该方法通过生成模型修复被动态对象遮挡的区域，并使用可学习的真实性标量来平衡真实背景和生成区域的贡献。


<details>
  <summary>Details</summary>
Motivation: 从包含动态对象的单目视频中重建静态3D场景对于虚拟现实和自动驾驶等应用很重要。现有方法通常依赖背景进行静态场景重建，限制了恢复被动态对象遮挡区域的能力。

Method: 1. 使用运动感知模块分割和移除动态区域；2. 使用扩散模型修复被遮挡区域，提供伪地面真值监督；3. 为每个高斯基元引入可学习的真实性标量，在溅射过程中动态调制不透明度，实现真实性感知的渲染和监督。

Result: 在DAVIS数据集和自建的Trajectory-Match数据集上的大量实验表明，GA-GS在静态场景重建方面达到了最先进的性能，特别是在具有大规模、持续遮挡的挑战性场景中。

Conclusion: GA-GS通过生成辅助的方法有效解决了从动态视频中重建静态场景时被遮挡区域恢复的问题，在多个数据集上表现出优越性能，为相关应用提供了有力工具。
...
TL;DR: SW-CLIP：一种用于街景地理定位的新框架，通过将空间自相关纳入视觉-语言对比学习，利用地理距离信息改进传统CLIP方法。


<details>
  <summary>Details</summary>
Motivation: 传统基于CLIP的地理定位方法将所有不匹配样本视为同等负样本，忽略了地理空间关系。本文旨在将地理学第一定律（Tobler定律）融入对比学习，实现从语义对齐到地理对齐的转变。

Method: 1. 引入位置作为文本表示来编码地理坐标；2. 用基于测地距离的空间加权软标签替换one-hot InfoNCE目标；3. 采用邻域一致性正则化来保持嵌入空间中的局部空间结构。

Result: 在多城市数据集上的实验表明，SW-CLIP显著提高了地理定位精度，减少了长尾误差，增强了空间一致性，优于标准CLIP方法。

Conclusion: 将空间原则融入多模态表示学习对于鲁棒的地理定位至关重要，SW-CLIP为这一领域提供了一个通用范式，实现了从语义对齐到地理对齐的重要转变。
...
Method: RLER分为训练和推理两个阶段：训练阶段使用组相对强化学习和三种任务驱动奖励（帧敏感奖励、思维透明奖励、抗重复奖励）优化策略；推理阶段使用无需训练的编排器生成多样化候选，通过证据一致性、置信度、透明度和非冗余性评分，进行证据加权选举。

Result: 在8个代表性基准测试中，RLER实现了最先进的性能，相比基础模型平均提升6.3%，每个问题平均使用3.1个候选，在计算和质量之间取得了良好平衡。

Conclusion: 在学习过程中使证据显式化，在推理过程中基于证据进行选举，是实现可信视频推理的稳健路径。
...
<details>
  <summary>Details</summary>
Motivation: 夜间视频去雨面临独特挑战：雨滴与人造光相互作用，呈现多种颜色和局部照明效果。现有小规模合成数据集使用2D雨滴叠加，无法捕捉这些物理特性，导致模型在真实夜间雨景中泛化能力差。同时，捕获真实配对夜间视频不现实，因为雨效无法与其他退化（如传感器噪声）分离。

Method: 使用Unreal Engine将雨模拟为虚拟环境中的3D粒子，确保照片级真实感和物理真实的雨滴，捕捉颜色折射、场景遮挡、雨帘等正确细节。基于此高质量数据，通过适配Wan 2.2视频生成模型建立新基线，将去雨视为视频到视频生成任务，利用强生成先验几乎完全弥合模拟到真实的差距。

Result: 创建了包含600,000对1080p帧的大规模数据集UENR-600K。基准测试表明，在该数据集上训练的模型在真实世界视频中泛化能力显著提升，几乎完全弥合了模拟到真实的差距，建立了新的最先进基线。

Conclusion: UENR-600K是首个大规模、物理基础的夜间视频去雨数据集，通过3D粒子模拟解决了夜间雨滴与人工照明交互的挑战。基于此数据训练的模型在真实夜间视频中表现出优异的泛化能力，为夜间视频去雨研究提供了重要资源。
...
TL;DR: 该论文提出了BoxComm数据集，这是首个专注于拳击等格斗运动的解说生成基准，包含445场世界拳击锦标赛视频和5.2万条专业解说。论文还提出了结构化解说分类法和两种新颖的评估方法，揭示了当前多模态大语言模型在格斗运动解说生成上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有体育解说生成基准仅关注足球、篮球等团队运动，完全忽略了格斗运动。格斗运动具有独特挑战：关键动作在毫秒内发生，视觉差异微妙但语义决定性；专业解说中战术分析比例远高于团队运动。需要专门的数据集和评估方法来填补这一空白。

Method: 1) 构建BoxComm数据集：445场世界拳击锦标赛视频，52K+专业解说句子；2) 提出结构化解说分类法：将解说分为实时描述、战术分析、背景信息三类；3) 提出两种评估方法：类别条件生成（评估模型能否生成指定类型的准确解说）和节拍评估（评估自由生成解说的时序节奏和类型分布）；4) 提出EIC-Gen基线方法：结合检测到的出拳事件提供结构化动作线索。

Result: 实验表明，当前最先进的多模态大语言模型在两种评估上都表现不佳。提出的EIC-Gen基线方法通过结合检测到的出拳事件，在所有评估指标上取得一致提升，突显了感知短暂微妙事件对格斗运动解说的重要性。

Conclusion: BoxComm填补了格斗运动解说生成基准的空白，提出的分类法和评估方法为体育解说生成提供了更全面的评估框架。当前模型在格斗运动解说生成上仍有困难，需要更好的事件感知能力。EIC-Gen展示了结合结构化动作线索的有效性，为未来研究提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 虚拟现实需要详细且可定制的3D手部模型，但现有方法昂贵、繁琐且缺乏可定制性。现有的零样本文本到3D合成方法在手部生成上泛化能力差，导致不自然的手部结构、视图不一致和细节丢失。

Method: 提出HandDreamer方法：1）使用MANO手部模型初始化提供强先验；2）手部骨架引导的扩散过程确保视图和姿态一致性；3）新颖的校正手部形状引导损失确保所有视图收敛到视图一致的模式，避免几何失真。

Result: 广泛评估表明该方法在3D手部模型生成方面优于现有最先进方法，为3D手部模型生成开辟了新途径。

Conclusion: HandDreamer是首个从文本提示生成零样本3D手部模型的方法，通过解决视图不一致问题，显著提升了手部模型生成的质量和一致性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的开放词汇目标检测方法在大规模预训练下在通用场景表现良好，但在领域转移的下游任务中性能严重下降。这源于领域特定任务中类别标签稀缺且语义弱，以及现有模型无法捕捉粗粒度类别标签之外的辅助语义。
...
TL;DR: Chorus是一种通过利用跨请求相似性来加速视频扩散模型推理的缓存方法，相比现有仅利用单个请求内相似性的方法，在工业级4步蒸馏模型上实现了高达45%的加速。


<details>
  <summary>Details</summary>
Motivation: 视频扩散变换器（DiT）模型虽然能生成高质量视频，但由于迭代去噪过程导致推理成本高昂。现有的缓存方法主要利用单个请求扩散过程中的相似性来跳过冗余去噪步骤，但在工业级4步蒸馏模型上效果有限。

Method: Chorus采用三阶段缓存策略：阶段1完全复用相似请求的潜在特征；阶段2在中间去噪步骤中对特定潜在区域进行跨请求缓存，并结合Token-Guided Attention Amplification来改善生成视频与条件提示之间的语义对齐，从而将完全复用扩展到后期去噪步骤。

Result: Chorus在工业级4步蒸馏模型上实现了高达45%的加速，而先前的请求内缓存方法在这种模型上效果不佳。

Conclusion: 通过利用跨请求相似性的缓存方法Chorus，能够显著加速视频扩散模型的推理过程，特别是在现有方法效果有限的工业级蒸馏模型上表现出色。
...
TL;DR: 本文提出一个系统评估框架，用于在自然对抗场景下评估视觉语言模型（VLMs）的鲁棒性，发现现有模型在面对排版攻击、自然语言诱导对抗样本时存在显著脆弱性。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在零样本迁移任务上表现出色，但现有评估主要基于标准基准测试，缺乏对自然对抗场景下模型鲁棒性、局限性和实际适用性的全面独立评估。

Method: 提出系统评估框架，在多种下游任务（零样本图像分类、语义分割、视觉问答）中评估多种VLMs（CLIP、鲁棒CLIP、BLIP2、SigLIP2）在精心策划的对抗数据集（排版攻击、ImageNet-A、自然语言诱导对抗样本）上的表现。

Result: 发现鲁棒CLIP模型可能放大自然对抗脆弱性；CLIP模型在自然语言诱导对抗样本上性能显著下降；通过可解释性分析识别了失败模式。

Conclusion: 研究揭示了VLMs在自然对抗场景下的局限性，为未来鲁棒和公平的多模态模式识别研究提供了重要启示。
...
TL;DR: VicoEdit是一种无需训练、无需反转的视觉上下文感知图像编辑方法，通过直接转换源图像并采用后验采样增强编辑一致性，性能优于现有基于训练的方法。
...
Result: 实验结果表明，这种无训练方法在编辑性能上甚至优于最先进的基于训练的模型。

Conclusion: VicoEdit通过消除反转需求并采用后验采样，实现了高效、一致且灵活的视觉上下文感知图像编辑，为图像编辑领域提供了新的解决方案。
...
TL;DR: 本文提出了一种基于补丁增强和跨视图正则化的统一防御框架，用于保护多模态大语言模型免受后门攻击，在低中毒率下有效抑制攻击成功率，同时保持正常文本生成能力。
...
Conclusion: 该工作使得大规模多模态模型能够在现实低频率中毒和隐蔽触发场景下安全、可控地部署，为解决后门防御中的冲突目标提供了有效方案。
...
TL;DR: 该论文提出了因陀罗表示假说，认为单模态基础模型的表示正在收敛，反映了现实背后的共享关系结构，并通过范畴理论形式化这一概念，在跨模型和跨模态场景中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 研究发现单模态基础模型倾向于学习收敛的表示，但这些表示本质上是样本的独立内部抽象，表达能力有限。受因陀罗网哲学隐喻启发，作者认为这些表示正在收敛以隐式反映现实背后的共享关系结构。

Method: 使用范畴理论中的V-富集Yoneda嵌入形式化因陀罗表示假说，将因陀罗表示定义为每个样本相对于其他样本的关系轮廓。使用角距离实例化因陀罗表示，并在涉及视觉、语言和音频的跨模型和跨模态场景中进行评估。

Result: 大量实验表明，因陀罗表示在不同架构和模态中始终增强鲁棒性和对齐性，为单模态基础模型的无训练对齐提供了理论依据和实践框架。

Conclusion: 因陀罗表示假说为理解单模态基础模型的收敛表示提供了新的理论视角，并展示了通过关系轮廓增强模型对齐和鲁棒性的实用价值。
...
TL;DR: Saliency-R1框架通过新颖的显著性映射技术提升视觉语言模型的解释性和忠实性，无需额外计算开销，利用GRPO对齐模型关注区域与关键视觉证据


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型存在可信度问题，包括过度依赖文本线索而非视觉证据、产生无根据或虚构回答的倾向，需要提升模型的解释性和推理忠实性

Method: 提出Saliency-R1框架：1) 新颖的显著性映射技术高效突出对生成token关键的图像区域；2) 扩展技术以追踪视觉信息在推理过程中的流向；3) 使用显著性图与人工标注边界框的重叠作为奖励函数；4) 应用GRPO对齐显著部分与关键区域

Result: 实验表明Saliency-R1提升了推理忠实性、解释性和整体任务性能

Conclusion: Saliency-R1框架有效解决了VLMs的可信度问题，通过显著性映射和强化学习对齐技术，使模型在推理时更关注相关视觉证据，提高了模型的解释性和忠实性
...
TL;DR: MedROI是一个与编解码器无关的ROI中心压缩框架，通过去除医学图像中的背景体素来提高压缩效率和速度，同时保持ROI内的重建质量。


<details>
  <summary>Details</summary>
Motivation: 医学影像档案在尺寸和分辨率上快速增长，现有编解码器要么压缩整个图像（包括非诊断背景），要么应用差分ROI编码但仍保留背景比特，导致存储和数据传输效率低下。

Method: MedROI通过轻量级基于强度的阈值处理提取紧密的组织边界框，丢弃背景体素，存储54字节的元数据用于空间恢复。裁剪后的ROI可使用任何现有2D或3D编解码器压缩，无需架构修改或重新训练。

Result: 在200个T1加权脑MRI体积上测试6种编解码器配置，MedROI在大多数配置中显著提高了压缩比和编码/解码时间，同时保持ROI内可比较的重建质量。例如，JPEG2000 2D (lv3)的压缩比从20.35提高到27.37，平均压缩时间从1.701秒减少到1.380秒。

Conclusion: MedROI提供了一个即插即用的ROI中心压缩框架，能显著提高医学图像压缩效率，适用于各种传统和神经编解码器，代码已开源。
...
TL;DR: 提出StableTTA方法解决集成方法中的聚合冲突问题，提升预测稳定性与效率，在ImageNet-1K上显著提升精度，轻量架构超越ViT且大幅减少计算成本。


<details>
  <summary>Details</summary>
Motivation: 集成方法虽能提升预测性能，但通常以增加内存使用和计算复杂度为代价。本文发现聚合策略中存在冲突，负面影响预测稳定性，需要解决这一问题。

Method: 提出StableTTA（训练免费方法），改进聚合稳定性和效率，无需额外训练过程。

Result: 在ImageNet-1K上获得10.93-32.82%的top-1准确率提升，33个模型达到95%以上准确率，多个超过96%。轻量架构超越ViT 11.75%准确率，仅用不到5%参数，减少约89.1%计算成本。

Conclusion: StableTTA能有效解决集成方法中的聚合冲突问题，显著提升预测稳定性与效率，使轻量架构在资源受限设备上实现高精度推理。
...
<details>
  <summary>Details</summary>
Motivation: 当前医学基础模型大多孤立分析放射影像，忽略了临床实践中比较前后图像以评估间隔变化的关键任务。对于胸部X光片，捕捉间隔变化至关重要，因为放射科医生不仅需要评估发现的静态外观，还需要评估它们随时间如何演变。

Method: 提出TILA（时序反转感知学习与对齐）框架，使用时间反转（反转图像对）作为监督信号来增强现有时序视觉语言模型对方向性变化的敏感性。该框架在预训练、微调和推理阶段集成反转感知目标，将显式时序顺序学习与常规外观建模相结合。

Result: 在公共数据集和真实医院队列上的实验表明，TILA在应用于多种现有架构时，能一致地改善进展分类和时间嵌入对齐性能。同时提出了MS-CXR-Tretrieval检索评估集和统一的评估协议。

Conclusion: TILA是一个简单而有效的框架，通过时间反转监督信号显著增强了医学影像时序分析中方向性变化的敏感性，为胸部X光片间隔变化评估提供了改进方案。
...
TL;DR: TAPE是一个用于OCT/OCTA图像分割的两阶段自适应框架，通过参数高效微调解决领域偏移和任务不对齐问题，在资源受限的临床环境中实现卓越的泛化性能。
...
Conclusion: TAPE框架有效解决了医学图像分析中的领域偏移和任务不对齐问题，为资源受限的临床环境提供了高效的OCT/OCTA图像分割解决方案。
...
TL;DR: 对文本到图像扩散模型中概念遗忘的系统实证研究，揭示了遗忘效果与组合生成能力之间的权衡关系


<details>
  <summary>Details</summary>
Motivation: 现有的事后遗忘方法主要评估遗忘成功率，但对模型更广泛生成能力的影响了解不足，需要系统研究遗忘对组合生成的影响

Method: 以Stable Diffusion 1.4中的裸露内容移除为案例，使用T2I-CompBench++和GenEval等评估工具，对多种最先进的遗忘方法进行系统实证研究

Result: 发现遗忘效果与组合完整性之间存在一致权衡：强遗忘方法会显著损害属性绑定、空间推理和计数能力；而保持组合结构的方法往往无法提供鲁棒的遗忘效果

Conclusion: 当前评估实践存在局限性，需要开发能够明确考虑目标抑制之外语义保持的遗忘目标
...
TL;DR: 提出PR-IQA框架，用于评估扩散模型生成的多视角图像质量，无需真实标签，通过部分参考图像计算几何一致的质量图，并集成到3D高斯溅射重建流程中，提升稀疏视角下的新视角合成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型可用于稀疏视角新视角合成，但生成的图像存在光度学和几何不一致性，直接用于监督会损害3D重建质量。现有方法缺乏有效的无参考质量评估机制。

Method: 提出部分参考图像质量评估（PR-IQA）框架：1）在重叠区域计算几何一致的部分质量图；2）通过交叉注意力机制结合参考视图上下文，将部分质量图补全为密集全图像质量图；3）将PR-IQA集成到扩散增强的3D高斯溅射流程中，仅在高置信度区域进行监督。

Result: PR-IQA在无真实标签监督的情况下，达到了全参考级别的准确性，优于现有IQA方法。集成PR-IQA的3DGS能更有效地过滤不一致性，产生更优的3D重建和新视角合成结果。

Conclusion: PR-IQA框架为扩散生成图像提供了有效的质量评估，无需真实标签，显著提升了稀疏视角下3D重建的质量，为扩散模型在3D视觉任务中的应用提供了新思路。
...
Conclusion: Firebolt-VL通过创新的架构设计解决了多模态模型效率与细粒度理解之间的平衡问题，为资源受限场景下的视觉语言应用提供了可行解决方案
...
TL;DR: 论文提出了一种基于物理特征检测AI生成图像的新方法，通过分析15种物理特征，筛选出5种核心特征，并将其与CLIP模型结合，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成内容的快速发展，现有深度伪造检测器往往过度拟合特定生成模型，难以区分真实与合成图像。这促使研究者重新审视区分自然图像与AI生成图像的内在物理特征。

Method: 1. 对15种物理特征在20多个由GAN和扩散模型生成的数据集上进行全面探索；2. 提出新颖的特征选择算法，识别出5种核心物理特征（包括拉普拉斯方差、Sobel统计量和残差噪声方差）；3. 将这些特征转换为文本编码值，与语义描述结合，指导CLIP中的图像-文本表示学习。

Result: 方法在多个Genimage基准测试中达到最先进性能，在Wukong和SDv1.4等数据集上接近完美准确率（99.8%）。物理特征在不同数据集和生成架构中表现出稳定的判别能力。

Conclusion: 通过将像素级真实性与语义理解相结合，这项工作开创了使用基于物理的特征进行可信视觉语言建模的先河，为缓解大型多模态模型中的幻觉和文本不准确性开辟了新方向。
...
TL;DR: GLA提出了一种针对自动驾驶视觉语言模型的隐蔽后门攻击方法，使用涂鸦式视觉触发器和跨语言文本触发器，在仅需10%中毒率的情况下实现90%攻击成功率，且不影响正常任务性能。
...
TL;DR: InCTRLv2是一个新颖的少样本通用异常检测与分割框架，通过双分支架构结合判别式和单类异常评分学习，利用视觉-语言语义先验，在多个数据集上实现最先进的异常检测和分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法大多是针对特定领域的专家模型，需要大量训练数据且难以泛化到未见数据集。为了解决这一限制，通用异常检测范式应运而生，旨在学习单一通用模型来跨领域检测异常而无需重新训练。

Method: InCTRLv2在原有InCTRL基础上引入双分支框架：1）主分支使用判别式异常评分学习，利用正常和异常数据学习语义引导的异常与正常空间；2）辅助分支使用单类异常评分学习，仅使用正常数据在语义空间中学习广义正常模式。两个分支都由大规模视觉-语言模型编码的丰富视觉-文本语义先验指导。

Result: 在十个异常检测数据集上的广泛实验表明，InCTRLv2在各种设置下，在异常检测和分割任务中都实现了最先进的性能。

Conclusion: InCTRLv2通过双语义视角（正常-异常判别和正常偏离语义）为异常检测提供了互补的感知角度，显著提升了通用异常检测与分割的能力。
...
TL;DR: 该研究针对AI生成视频检测的局限性，提出了原生尺度处理的新框架和包含15种先进生成器的大规模数据集，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型的快速发展使得合成媒体高度逼真，引发了虚假信息传播的严重社会担忧。现有检测方法存在关键局限：依赖固定分辨率调整和裁剪等预处理操作，这些操作不仅丢弃了细微的高频伪造痕迹，还导致空间扭曲和显著信息丢失。此外，现有方法通常在过时数据集上训练和评估，无法捕捉现代生成模型的复杂性。

Method: 提出了一个全面的数据集和新型检测框架：1）构建了包含15种最先进开源和商业生成器的超过14万视频的大规模数据集，以及专门用于评估超逼真合成内容的Magic Videos基准；2）基于Qwen2.5-VL Vision Transformer构建了新型检测框架，该框架原生支持可变空间分辨率和时间长度，有效保留了传统预处理中丢失的高频伪影和时空不一致性。

Result: 广泛实验表明，该方法在多个基准测试中实现了卓越性能，突显了原生尺度处理的关键重要性，并为AI生成视频检测建立了强大的新基准。

Conclusion: 该研究通过原生尺度处理方法和全面数据集，有效解决了现有AI生成视频检测方法的局限性，为应对合成媒体带来的虚假信息挑战提供了更强大的技术基础。
...
TL;DR: 提出Synthesis4AD框架，通过大规模高保真合成异常数据解决3D工业异常检测中异常样本稀缺和长尾分布问题，使用3D-DefectStudio平台和MPAS引擎生成几何真实缺陷，结合MLLM自动生成异常合成指令，并在训练中引入空间分布归一化和几何忠实数据增强提升检测器性能。


<details>
  <summary>Details</summary>
Motivation: 工业3D异常检测性能受限于异常样本的稀缺性和长尾分布，这限制了检测模型的训练效果和泛化能力。

Method: 1. 开发3D-DefectStudio平台，基于MPAS可控合成引擎生成几何真实的缺陷和点级异常掩码；2. 利用多模态大语言模型解释产品设计信息并自动生成异常合成指令；3. 提出基于空间分布归一化和几何忠实数据增强的训练管道，减轻Point Transformer对绝对坐标的敏感性。

Result: 在Real3D-AD、MulSen-AD和真实工业零件数据集上的广泛实验表明，该方法达到了最先进的性能水平。

Conclusion: Synthesis4AD通过大规模合成异常数据有效解决了3D异常检测中的数据稀缺问题，提出的MPAS合成方法和3D-DefectStudio交互系统将公开发布，为工业异常检测提供了新的解决方案。
...
TL;DR: 本文提出DenZa-Gaussian方法，通过3D高斯泼溅技术改进稀疏视角ADF-STEM断层扫描，解决传统方法在有限视角下的重建质量下降问题。


<details>
  <summary>Details</summary>
Motivation: 传统ADF-STEM断层扫描需要多视角倾斜系列图像进行3D重建，但增加视角会延长电子束暴露时间，可能损伤剂量敏感材料并引入漂移和错位问题。稀疏视角采集虽能保护样品，但传统方法在有限视角下重建质量严重下降，产生伪影并降低结构保真度。

Method: 将3D高斯泼溅技术应用于ADF-STEM领域，包含三个关键组件：1) 将局部散射强度建模为可学习标量场denza，解决3DGS与ADF-STEM成像物理不匹配问题；2) 引入系数γ稳定不同倾斜角度的散射，通过散射视角归一化确保denza一致性；3) 加入包含2D傅里叶振幅项的损失函数，抑制稀疏视角重建中的缺失楔形伪影。

Result: 在45视角和15视角倾斜系列上的实验表明，DenZa-Gaussian能够产生高保真度的重建结果和2D投影，与原始倾斜图像更吻合，在稀疏视角条件下表现出优越的鲁棒性。

Conclusion: DenZa-Gaussian方法成功解决了ADF-STEM断层扫描中稀疏视角重建的挑战，通过物理感知的3D高斯泼溅技术实现了高质量3D重建，同时保护了剂量敏感材料，在材料科学纳米尺度表征中具有重要应用价值。
...
TL;DR: OpenWorldLib是一个标准化的先进世界模型推理框架，提出了世界模型的明确定义，并系统分类了其核心能力，支持跨任务模型集成和协作推理。


<details>
  <summary>Details</summary>
Motivation: 世界模型作为人工智能的重要研究方向，目前缺乏清晰统一的定义。作者旨在解决这一概念模糊问题，并提供一个标准化的推理框架来促进世界模型的研究和应用。

Method: 通过回顾世界模型的发展历程，提出了明确的定义：世界模型是以感知为中心、具备交互和长期记忆能力的模型或框架，用于理解和预测复杂世界。系统分类了世界模型的核心能力，并构建了OpenWorldLib框架，将不同任务的模型集成到统一框架中，实现高效复用和协作推理。

Result: 开发了OpenWorldLib开源框架，提供了世界模型的标准化定义和分类体系，实现了跨任务模型的集成和协作推理能力，为世界模型研究提供了统一的基础设施。

Conclusion: 该研究为世界模型领域提供了清晰的概念框架和实用的工具平台，有助于推动世界模型研究的标准化和协作发展。作者还对世界模型未来的研究方向提出了分析和展望。
...
TL;DR: MMF-BEV是一个用于自动驾驶的雷达-相机BEV融合框架，在VoD 4D雷达数据集上通过可变形注意力机制实现跨模态特征对齐，显著提升3D目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要互补的传感器：相机提供密集语义但深度不可靠，毫米波雷达提供精确距离和速度但几何稀疏。需要融合两者优势以实现准确的3D目标检测。

Method: 构建BEVDepth相机分支和RadarBEVNet雷达分支，每个分支增强可变形自注意力，通过可变形交叉注意力模块融合。采用两阶段训练策略：先用深度监督预训练相机分支，然后联合训练雷达和融合模块。

Result: 在VoD数据集上，MMF-BEV持续优于单模态基线，在所有物体类别和全标注区域及近距ROI中，与现有融合方法相比取得竞争性结果。传感器贡献分析提供了可解释的模态权重证据。

Conclusion: MMF-BEV通过可变形注意力机制有效融合雷达和相机模态，证明了传感器互补性，为自动驾驶3D目标检测提供了鲁棒且可解释的解决方案。
...
TL;DR: 提出自适应KV缓存量化方法，通过数据驱动控制器动态分配不同精度（2/4/8/FP16位）给不同重要性的token，减少内存和延迟同时保持接近FP16的精度。


<details>
  <summary>Details</summary>
Motivation: LLM在移动和边缘设备部署面临KV缓存内存和带宽瓶颈，现有固定精度量化方案浪费比特在低重要性token上，同时过度压缩重要token，导致精度下降。

Method: 提取token级轻量特征（频率、质量分数、注意力方差、熵不确定性），输入紧凑数据驱动控制器，在解码时动态选择KV精度（2/4/8/FP16位）。

Result: 在SmolLM系列模型上测试显示，相比静态KV量化，解码延迟降低17.75%，精度提高7.60分，且仅比FP16推理低0.30分。

Conclusion: 自适应KV缓存量化通过变长比特分配策略，在减少内存占用和延迟的同时保持竞争性精度，为设备端LLM推理提供了有效的精度-延迟权衡方案。
...
TL;DR: 提出过程驱动的图像生成方法，通过多步迭代分解图像合成过程，包含文本规划、视觉草图、文本反思和视觉细化四个阶段，使生成过程可解释且可监督。


<details>
  <summary>Details</summary>
Motivation: 人类绘画是增量式的过程，包含全局规划、草图绘制、检查和细节细化等步骤，每一步都基于当前视觉状态。然而，在文本-图像交错数据集上训练的统一多模态模型能否想象中间状态链？本文旨在探索这一过程驱动的图像生成范式。

Method: 提出过程驱动的图像生成方法，将合成过程分解为交错的思想和行动轨迹。每个迭代包含四个阶段：1) 文本规划：基于当前视觉状态规划下一步；2) 视觉草图：生成中间图像；3) 文本反思：评估当前图像并识别问题；4) 视觉细化：修正图像。通过密集的逐步监督，对视觉中间状态强制空间和语义一致性，对文本中间状态保持先验视觉知识同时识别和修正违反提示的元素。

Result: 在多个文本到图像生成基准测试上进行了实验验证，结果表明该方法使生成过程变得明确、可解释且可直接监督。

Conclusion: 过程驱动的图像生成方法通过多步迭代分解合成过程，使模型能够想象和生成中间状态，提高了生成过程的可解释性和可控性，为多模态模型的增量式推理提供了新范式。
...
TL;DR: MINERUPro通过数据工程和训练策略优化，在不改变1.2B参数架构的情况下，将文档解析性能提升到SOTA水平，超越了参数规模大200倍的模型。
...
Method: 提出MINERUPro方法，核心是围绕覆盖率、信息量和标注准确性共同设计的数据引擎：1）多样性和难度感知采样将训练数据从不足1000万扩展到6550万样本；2）跨模型一致性验证利用异构模型输出一致性评估样本难度并生成可靠标注；3）Judge-and-Refine管道通过渲染-验证迭代校正改进困难样本标注质量。采用三阶段渐进训练策略：大规模预训练、困难样本微调和GRPO对齐。

Result: 在OmniDocBench v1.6评估协议上获得95.69分，比相同架构基线提升2.71分，超越了所有现有方法，包括参数规模大200倍以上的模型。同时修复了OmniDocBench v1.5中的元素匹配偏差并引入了Hard子集。

Conclusion: 通过系统性的数据工程和训练策略优化，可以在不改变模型架构的情况下显著提升文档解析性能，证明了训练数据质量比模型架构创新更为关键。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的图像退化（模糊、噪声、压缩、光照差）严重破坏多模态理解，现有统一多模态模型未能利用自身生成能力处理退化输入，存在训练机制和优化路径的缺陷

Method: 提出CLEAR框架，包含三个渐进步骤：1）在退化感知数据集上进行监督微调，建立"生成-回答"推理模式；2）潜在表示桥接，用直接可优化的连接替代解码-重新编码路径；3）交错GRPO强化学习方法，在答案正确性奖励下联合优化文本推理和视觉生成

Result: 在MMD-Bench（覆盖三种退化严重程度和六个标准多模态基准）上的实验表明，CLEAR显著提高了退化输入的鲁棒性，同时保持了干净图像性能。分析显示，移除像素级重建监督会产生感知质量更高的中间视觉状态

Conclusion: CLEAR通过连接生成与推理能力，有效解决了多模态模型在退化图像上的性能问题，任务驱动优化与视觉质量自然对齐，为现实世界多模态理解提供了实用解决方案
...
<details>
  <summary>Details</summary>
Motivation: 从单张肖像图像生成高质量、动态可控的4D虚拟化身是一个具有挑战性的任务，需要解决几何重建、动态动画和渲染质量等多方面问题

Method: 采用解码器专用Transformer自回归生成3D高斯泼溅点云，动态调整点密度和数量，联合预测每点的绑定信息，最后通过高斯解码器转换为完整可渲染的高斯属性

Result: 实验验证AvatarPointillist能够生成高质量、逼真且可控的虚拟化身，自回归生成器与解码器的条件交互显著提升了保真度

Conclusion: 该自回归框架为虚拟化身生成提供了新范式，能够从单张图像创建动态4D高斯化身，代码将开源以促进未来研究
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在视觉问答任务中取得显著进展，但高分辨率细节有时会成为噪声，导致幻觉或推理错误。需要一种方法让模型专注于核心结构信息而非表面纹理。

Method: 提出DDP框架，包含两个任务：1) 物理属性任务：使用80%下采样、结构视觉辅助（白色背景遮罩和正交线）和上下文学习来校准模型注意力；2) 感知现象任务：整合任务分类阶段，结合模糊遮罩、对比度增强和下采样等专门工具处理各种视觉异常和错觉。

Result: 实验结果表明"少即是多"：通过有意降低视觉输入质量并提供针对性结构提示，DDP使视觉语言模型能够绕过分散注意力的纹理，在具有挑战性的视觉基准测试中实现更优的推理准确性。

Conclusion: DDP框架通过策略性降低图像保真度和提供结构提示，有效提升视觉语言模型在复杂视觉推理任务中的性能，证明了关注本质结构信息的重要性。
...
Result: 大量实验表明，该方法在HOSI和HOI生成上都达到了最先进的性能，并在未见场景上表现出强大的泛化能力。

Conclusion: 该方法通过动态感知策略、碰撞感知引导和混合训练策略，有效解决了HOSI生成中的动态场景变化推理和数据稀缺问题，实现了高质量、实时的人-物-场景交互生成。
...
TL;DR: 该论文首次系统研究了自动驾驶中视觉语言模型的灾难性遗忘问题，提出了新的大规模数据集和基准测试，并开发了Drive Expert Adapter框架来避免微调导致的世界知识退化。


<details>
  <summary>Details</summary>
Motivation: 将视觉语言模型集成到自动驾驶中可以解决长尾场景问题，但微调过程会侵蚀模型预训练的世界知识，造成灾难性遗忘，这与使用这些模型的初衷相悖。

Method: 提出了Drive Expert Adapter（DEA）框架，将适应过程从权重空间转移到提示空间，基于场景特定线索动态路由推理到不同的知识专家，避免修改模型的基础参数。

Result: 实验表明，该方法在驾驶任务上达到最先进性能，同时有效缓解了灾难性遗忘，保持了模型的泛化能力。发布了包含180K场景的新数据集和基准测试。

Conclusion: DEA框架成功解决了自动驾驶中视觉语言模型的灾难性遗忘问题，通过提示空间适应而非权重微调，在提升驾驶任务性能的同时保留了预训练的世界知识。
...
TL;DR: FMLM：基于Floorplan Markup Language（FML）的通用表示方法，使用Transformer模型实现多条件住宅平面图生成，在RPLAN数据集上超越现有任务专用方法。


<details>
  <summary>Details</summary>
Motivation: 现有住宅平面图自动生成方法存在局限性：早期基于约束满足或组合优化的方法缺乏多样性和灵活性；近期生成模型虽然效果不错，但难以泛化到异构条件任务（如从场地边界、房间邻接图或部分布局生成），这主要源于其表示方法不够理想。

Method: 提出Floorplan Markup Language（FML），一种通用结构化语法表示方法，将整个平面图生成问题转化为下一个标记预测任务。基于FML开发了Transformer生成模型FMLM，能够生成高保真、功能性的平面图，并支持多种条件输入。

Result: 在RPLAN数据集上的综合实验表明，FMLM虽然是单一模型，但超越了先前任务专用的最先进方法，能够生成高质量、功能性的平面图。

Conclusion: FML表示方法和FMLM模型有效解决了住宅平面图生成中的泛化问题，通过统一的表示和生成框架，实现了跨异构条件任务的高性能生成，推动了自动平面图生成技术的发展。
...
TL;DR: 提出基于图像块级别的幻觉检测框架，通过分析细粒度token交互来识别LVLM中的幻觉对象，准确率可达90%


<details>
  <summary>Details</summary>
Motivation: 现有基于全局图像度量的幻觉检测方法存在局限：幻觉token可能在整个图像中呈现微弱但广泛分布的关联性，这些分散的相关性会聚集成欺骗性的高整体相关性，从而逃避当前全局检测器

Method: 提出图像块级别的幻觉检测框架，分析模型各层的细粒度token交互。识别幻觉token的两个特征签名：1）产生扩散、非局部化的注意力模式；2）未能与任何视觉区域展现有意义的语义对齐。基于这些发现，开发轻量级可解释的检测方法，利用图像块级统计特征结合隐藏层表示

Result: 该方法在token级幻觉检测中达到90%的准确率，证明了细粒度结构分析在检测幻觉方面的优越性

Conclusion: 基于图像块级别的细粒度分析能够更有效地检测大型视觉语言模型中的幻觉现象，相比全局检测方法具有显著优势
...
TL;DR: Free-Range Gaussians：一种多视角重建方法，通过流匹配预测非像素、非体素对齐的3D高斯分布，仅需4张图像即可生成高质量重建结果，解决了传统方法在未观测区域产生空洞或模糊的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多视角重建方法通常产生高度冗余的网格对齐高斯分布，在未观测区域容易出现空洞或模糊的条件均值。需要一种能够从少量图像生成高质量重建，并在未观测区域合成合理内容的方法。

Method: 1. 通过高斯参数上的流匹配进行生成式重建；2. 引入分层分块方案，将空间相关的高斯分组到联合transformer token中，序列长度减半；3. 训练时使用时间步加权渲染损失；4. 推理时采用光度梯度引导和分类器自由引导提高保真度。

Result: 在Objaverse和Google Scanned Objects数据集上的实验表明，相比像素和体素对齐方法，该方法使用显著更少的高斯分布获得了一致的改进，特别是在输入视角未完全覆盖物体部分时表现出大幅提升。

Conclusion: Free-Range Gaussians通过生成式重建框架和分层分块策略，实现了从少量图像的高质量多视角重建，有效解决了未观测区域的合成问题，同时减少了高斯分布的数量需求。
...
Method: 将视频混剪创作建模为多模态连贯性满足问题，提出DIRECT框架，采用分层多智能体系统：编剧负责源感知的全局结构锚定，导演负责实例化自适应编辑意图和指导，编辑负责意图引导的镜头序列编辑和细粒度优化。

Result: DIRECT在客观指标和人类主观评价方面均显著优于现有最先进基线方法，并引入了包含视觉连续性和听觉对齐专门指标的Mashup-Bench基准测试。

Conclusion: DIRECT框架通过模拟专业制作流程，有效解决了视频混剪中的跨层次多模态协调问题，实现了专业级的流畅视频创作。
...
TL;DR: HorizonWeaver是一个用于自动驾驶场景编辑的框架，通过大规模数据集、语言引导掩码和联合训练方法，解决了密集驾驶场景的多粒度编辑、丰富语义保持和领域迁移三大挑战。
...
Method: 1. 数据：构建大规模配对真实/合成数据集（Boreas、nuScenes、Argoverse2）提升泛化能力；2. 模型：语言引导掩码实现细粒度编辑，通过语义增强掩码和提示实现精确编辑；3. 训练：内容保持和指令对齐的联合损失，确保场景一致性和指令保真度。

Result: 收集了13个编辑类别的255K图像，在L1、CLIP和DINO指标上优于现有方法，获得+46.4%的用户偏好，BEV分割IoU提升+33%。

Conclusion: HorizonWeaver为复杂驾驶场景提供了可扩展的、逼真的指令驱动编辑框架，解决了自动驾驶场景编辑的关键挑战，为安全测试提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有XR交互系统的局限性：基于云端的AI系统存在隐私和延迟问题，基于注视或语音的交互存在模糊性，需要更精确、透明且保护隐私的XR交互方案。

Method: 在Magic Leap SDK（C API）中实现，集成控制器点击选择物体，使用ONNX-based本地视觉语言模型处理所选物体图像，通过文本和语音回答自然语言问题。

Result: 用户研究表明，与Gemini 2.5 Flash和ChatGPT 5相比，ClickAIXR具有中等延迟和可接受的用户体验，展示了点击选择与本地AI结合在可信、隐私保护的XR交互方面的潜力。

Conclusion: ClickAIXR证明了基于点击的物体选择与本地AI推理相结合，能够推进可信赖、保护隐私的XR交互，为未来的XR系统提供了新的交互范式。
...
TL;DR: 该论文提出了一个用于图像空间编辑的完整评估框架，包括基准测试、合成数据集和基线模型，显著提升了细粒度空间操控能力。


<details>
  <summary>Details</summary>
Motivation: 当前模型在细粒度空间编辑方面能力不足，缺乏专门的评估体系，需要开发一个完整的基准来评估空间编辑的感知合理性和几何保真度。

Method: 1) 引入SpatialEdit-Bench基准，通过视角重建和构图分析联合评估感知合理性和几何保真度；2) 构建SpatialEdit-500k合成数据集，使用可控Blender流水线生成多样化背景和系统相机轨迹；3) 开发SpatialEdit-16B基线模型用于细粒度空间编辑。

Result: 该方法在通用编辑任务上表现有竞争力，在空间操控任务上显著优于先前方法，提供了精确的地面实况变换数据。

Conclusion: 论文提出了一个完整的空间编辑评估框架，包括基准测试、数据集和基线模型，为细粒度空间编辑提供了系统解决方案，所有资源将公开分享。
...
TL;DR: DeltaWorld：一种基于VFM特征差异的生成式世界模型，通过DeltaTok将帧间特征差异编码为连续"delta"token，实现高效多假设未来预测


<details>
  <summary>Details</summary>
Motivation: 视频世界建模面临预测多样化未来状态的挑战。判别式模型产生确定性预测，隐含了对可能未来的平均；现有生成式模型计算成本高。最近研究表明，在视觉基础模型特征空间而非像素重建潜在空间进行预测，可大幅减少世界模型参数，但多数方法仍是判别式的

Method: 提出DeltaTok将连续帧间的VFM特征差异编码为单个连续"delta"token，将三维时空视频表示降为一维时间序列。基于此构建DeltaWorld生成式世界模型，利用紧凑表示实现可处理的多假设训练：并行生成多个未来，仅监督最佳预测

Result: 在密集预测任务实验中，DeltaWorld预测的未来更接近真实世界结果，同时参数比现有生成式世界模型少35倍以上，FLOPs减少2000倍。推理时单次前向传播即可产生多样化预测

Conclusion: DeltaWorld通过DeltaTok的紧凑表示实现了高效的多假设训练，在保持预测多样性的同时大幅降低了计算成本，为视频世界建模提供了一种新的生成式方法
...
### [136] [Vero: An Open RL Recipe for General Visual Reasoning](https://arxiv.org/abs/2604.04917)
*Gabriel Sarch,Linrong Cai,Qunzhong Wang,Haoyang Wu,Danqi Chen,Zhuang Liu*

Main category: cs.CV

TL;DR: Vero是一个完全开源的视觉语言模型家族，通过构建600K样本的多样化数据集和任务路由奖励机制，在多个视觉推理任务上达到或超过现有开源模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前最强的视觉语言模型虽然展现出广泛的视觉推理能力，但其构建方法被封闭在专有的强化学习流程和非公开数据中，缺乏透明度。研究团队希望开发完全开源的视觉推理模型，探索构建跨领域视觉推理器的有效方法。
...
Result: Vero在VeroEval的30个基准测试中平均提升3.7-5.5个百分点，达到最先进性能。从Qwen3-VL-8B-Instruct开始，Vero在30个基准中的23个上超过了Qwen3-VL-8B-Thinking，且无需额外的专有思维数据。系统消融实验表明，广泛的数据覆盖是强化学习扩展的主要驱动力。

Conclusion: 构建跨领域视觉推理器的关键在于广泛的数据覆盖和任务特定的奖励设计。不同任务类别引发本质上不同的推理模式，单独训练时迁移效果差。Vero的成功证明了开源方法在视觉推理领域的可行性，所有数据、代码和模型均已开源。
...
TL;DR: 该研究提出了一种无需微调或控制模块的方法，通过直接学习文本编码器输出的提示嵌入来解锁预训练扩散模型的修复能力，使用扩散桥公式解决训练不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的修复方法主要依赖微调或Control-Net风格模块来利用预训练模型的先验知识，但作者发现预训练扩散模型本身就具备修复能力，只是需要通过特定方式解锁。

Method: 1. 发现预训练扩散模型具有固有的修复行为，可通过学习文本编码器输出的提示嵌入来解锁；2. 提出扩散桥公式，使训练和推理动态对齐，确保从噪声退化状态到干净图像的一致去噪路径；3. 在预训练的WAN视频模型和FLUX图像模型上应用轻量级学习提示。

Result: 该方法在多种退化类型上实现了竞争性的性能和泛化能力，避免了模型微调和修复特定的控制模块，同时保持了轻量级特性。

Conclusion: 预训练扩散模型本身就具备修复能力，通过适当的提示学习和扩散桥公式可以有效地解锁这种能力，为全合一修复提供了一种轻量级且高效的方法。
...
TL;DR: SimpleProc：基于NURBS和基本位移纹理模式的完全程序化MVS训练数据生成器，仅用8000张图像即超越同等规模人工标注数据，352000张图像时性能媲美692000张人工标注图像


<details>
  <summary>Details</summary>
Motivation: 探索多视角立体视觉（MVS）的程序化规则设计空间，解决人工标注训练数据成本高、规模受限的问题，寻找更高效的数据生成方法

Method: 提出SimpleProc：基于非均匀有理B样条（NURBS）以及基本位移和纹理模式的完全程序化生成器，通过少量规则驱动生成有效的MVS训练数据

Result: 1. 8000张程序化图像表现优于同等规模的游戏和真实物体人工标注图像；2. 扩展到352000张图像时，性能与692000张人工标注图像相当甚至在某些基准测试中超越

Conclusion: 程序化生成方法能够高效生成高质量的MVS训练数据，在较小规模下即展现优势，大规模时性能媲美甚至超越更大规模的人工标注数据集
...
TL;DR: 本文分析了视觉语言模型（VLM）中输出token数量对延迟的影响，发现大模型用更少的输出token可以达到小模型长输出的性能，并提出多智能体推理框架来提升效率。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通常使用大型语言模型作为解码器，通过自回归生成响应token，输出token数量成为端到端延迟的瓶颈。不同模型需要不同数量的输出token才能达到可比性能，需要研究如何平衡模型大小和输出长度以获得最佳效率。

Method: 1. 在模拟数据上对不同VLM组件的延迟进行全面分析；2. 在真实世界基准测试上进行实证研究；3. 提出多智能体推理框架，保持大模型的短响应，但在必要时从小模型转移关键推理token。

Result: 实验表明：1. 输出token较少的大模型可能比输出序列较长的小模型更高效；2. 大模型可以用显著更少的输出token达到与小模型相当或更好的性能；3. 通过重用小模型的推理token，可以接近大模型自身推理的性能。

Conclusion: 输出token数量是VLM延迟的关键因素，大模型通过更少的输出token可以实现高效推理。提出的多智能体推理框架通过结合大小模型的优势，在保持效率的同时提升性能，为VLM优化提供了有效方案。
...
TL;DR: LoMa方法通过大规模数据混合、现代训练方法、扩展模型容量和计算资源，显著提升了局部特征匹配性能，并在多个基准测试中大幅超越现有最佳方法。


<details>
  <summary>Details</summary>
Motivation: 局部特征匹配作为3D视觉系统的基础组件，其发展滞后于现代数据驱动方法。现有方法仅在几个中等规模数据集上训练，而新方法如前馈重建模型则受益于大规模数据集。当前基准测试主要基于成功3D重建的稀疏视图，导致评估局限于相对简单的图像对。

Method: 提出LoMa方法，结合大规模多样化数据混合、现代训练方法、扩展模型容量和计算资源。同时收集1000个高难度互联网图像对构建HardMatch数据集，通过人工标注获取真实对应关系。

Result: LoMa在多个基准测试中表现卓越：HardMatch上超越ALIKED+LightGlue 18.6 mAA，WxBS上提升29.5 mAA，InLoc上提升21.4 (1m, 10°)，RUBIK上提升24.2 AUC，IMC 2022上提升12.4 mAA。

Conclusion: 从数据驱动角度重新审视局部特征匹配，通过大规模数据、现代训练方法和扩展模型容量，显著提升了特征匹配性能。HardMatch数据集解决了现有基准测试饱和问题，为未来研究提供了更具挑战性的评估平台。
...
TL;DR: Vanast是一个统一框架，直接从单张人物图像、服装图像和姿态引导视频生成服装转移的人类动画视频，解决了传统两阶段流程中的身份漂移、服装变形和前后不一致问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法将基于图像的虚拟试穿和姿态驱动动画作为两个独立过程处理，这会导致身份漂移、服装变形和前后不一致问题。需要一种统一的方法来实现连贯的合成。
...
Conclusion: 通过统一框架、大规模三元组监督和双模块架构，Vanast成功解决了传统方法的问题，实现了从单张图像到服装转移动画的高质量生成。
...
### [142] [Learning-Based Fault Detection for Legged Robots in Remote Dynamic Environments](https://arxiv.org/abs/2604.03397)
*Abriana Stewart-Height,Seema Jahagirdar,Nikolai Matni*

Main category: cs.RO

TL;DR: 开发基于离线学习的四足机器人单肢故障检测方法，通过本体感知数据识别损伤肢体，为控制器选择合适的三足步态提供依据


<details>
  <summary>Details</summary>
Motivation: 危险环境中的操作对人类、动物和机器都存在高风险。与人类和动物不同，四足机器人无法自然识别肢体损伤并相应调整运动方式。在动态复杂环境中自主执行远程任务时，检测肢体损伤并调整运动以适应新物理形态的能力对机器人的生存至关重要

Method: 开发并实现基于离线学习的方法，利用四足机器人的本体感知传感器数据检测单肢故障。该方法旨在为控制器提供正确输出，以便根据机器人当前物理形态选择合适的三足步态

Result: 论文提出了一个完整的故障检测系统框架，能够从本体感知数据中识别单肢故障，为后续的步态调整提供决策依据

Conclusion: 通过离线学习的方法检测四足机器人单肢故障，使机器人能够在肢体受损时自动调整运动策略，提高在危险环境中的生存能力和任务完成能力
...
### [143] [Diffusion Policy with Bayesian Expert Selection for Active Multi-Target Tracking](https://arxiv.org/abs/2604.03404)
*Haotian Xiang,Qin Lu,Yaakov Bar-Shalom*

Main category: cs.RO

TL;DR: 提出一种基于贝叶斯框架的悲观不确定性感知策略选择方法，用于扩散策略的多目标跟踪任务，通过离线上下文赌博机问题建模专家选择，使用变分贝叶斯最后一层模型预测专家性能并进行悲观选择


<details>
  <summary>Details</summary>
Motivation: 现有扩散策略方法通过去噪过程隐式选择行为策略，缺乏对执行策略的不确定性量化。在多目标跟踪任务中，机器人需要在探索未检测目标和利用已跟踪目标之间平衡，需要更可靠的策略选择机制

Method: 将专家选择建模为离线上下文赌博机问题，提出贝叶斯框架进行悲观不确定性感知策略选择。使用多头变分贝叶斯最后一层模型预测每个专家策略在当前信念状态下的预期跟踪性能，提供点估计和预测不确定性。基于悲观原则，采用下置信界准则选择最坏情况下预测性能最佳的专家

Result: 在模拟室内跟踪场景实验中，该方法优于基础扩散策略和标准门控方法，包括混合专家选择和确定性回归基线

Conclusion: 提出的贝叶斯悲观不确定性感知策略选择框架能够有效提升扩散策略在多目标跟踪任务中的性能，通过量化不确定性避免对不可靠预测的过度承诺
...
### [144] [Do Robots Need Body Language? Comparing Communication Modalities for Legible Motion Intent in Human-Shared Spaces](https://arxiv.org/abs/2604.03451)
...
<details>
  <summary>Details</summary>
Motivation: 机器人在共享空间中移动时，其动作往往难以被人类解读，这给人类带来了适应负担。高自由度机器人表现出具有表达性的动作（无论是否有意），因此理解这些线索如何被感知非常重要

Method: 通过在线视频研究，评估不同信号模态（表达性动作、灯光、文本和音频）如何影响人们对四足机器人即将进行的导航动作的理解。在四种常见场景中测量每种模态对预测准确性、信心和信任度的影响

Result: 研究测试了表达性动作与显性通道的比较，对齐的多模态线索是否增强可解释性，以及冲突线索如何影响用户信心和信任

Conclusion: 为隐性与显性信号策略的相对有效性提供了初步证据
...
### [145] [Sim2Real-AD: A Modular Sim-to-Real Framework for Deploying VLM-Guided Reinforcement Learning in Real-World Autonomous Driving](https://arxiv.org/abs/2604.03497)
*Zilin Huang,Zhengyang Wan,Zihao Sheng,Boyue Wang,Junwei You,Yue Leng,Sikai Chen*

Main category: cs.RO

TL;DR: Sim2Real-AD框架实现CARLA训练的VLM引导RL策略零样本迁移到真实车辆，无需真实世界RL训练数据，包含几何观测桥、物理感知动作映射、两阶段渐进训练和实时部署管道四个模块。


<details>
  <summary>Details</summary>
Motivation: 解决将模拟环境中训练的VLM引导RL策略部署到真实自动驾驶车辆的挑战，特别是模拟器原生观测和动作语义在物理平台上不可用的问题。

Method: 提出模块化框架包含：1)几何观测桥将单目前视图像转换为BEV观测；2)物理感知动作映射将策略输出转换为平台无关物理命令；3)两阶段渐进训练分离动作空间和观测空间迁移；4)实时部署管道集成感知、策略推理、控制转换和安全监控。

Result: 模拟实验验证框架保持不同奖励范式下RL算法的相对性能排序；零样本部署到福特E-Transit车辆在跟车、避障和停车标志交互场景中分别达到90%、80%和75%的成功率。

Conclusion: 该研究首次展示了CARLA训练的VLM引导RL策略在真实全尺寸车辆上的零样本闭环部署，无需真实世界RL训练数据，为模拟到真实迁移提供了有效解决方案。
...
### [146] [CRAFT: Video Diffusion for Bimanual Robot Data Generation](https://arxiv.org/abs/2604.03552)
*Jason Chen,I-Chun Arthur Liu,Gaurav Sukhatme,Daniel Seita*

Main category: cs.RO

TL;DR: CRAFT使用视频扩散模型从模拟轨迹生成逼真的双手操作演示视频，通过边缘引导的结构线索增强数据多样性，显著提升双手机器人任务的成功率。


<details>
  <summary>Details</summary>
Motivation: 双手机器人学习受限于真实世界数据的成本和视觉多样性不足，这限制了策略在不同视角、物体配置和机器人形态下的鲁棒性。需要一种可扩展的方法来生成多样化的演示数据。

Method: CRAFT框架基于视频扩散变换器，通过从模拟轨迹提取的边缘结构线索来引导视频扩散，生成时间连贯的操作视频并产生动作标签。支持统一的增强流程，包括物体姿态变化、摄像机视角、光照背景变化、跨形态迁移和多视角合成。

Result: 在模拟和真实世界的双手任务中，CRAFT相比现有的增强策略和简单数据扩展方法显著提高了成功率。仅需少量真实演示，就能生成大量视觉多样化的逼真训练数据，无需在真实机器人上重放演示。

Conclusion: 基于扩散的视频生成能够大幅扩展演示数据的多样性，改善双手操作任务的泛化能力。CRAFT框架为双手机器人学习提供了可扩展的数据生成解决方案。
...
### [147] [HAD: Combining Hierarchical Diffusion with Metric-Decoupled RL for End-to-End Driving](https://arxiv.org/abs/2604.03581)
*Wenhao Yao,Xinglong Sun,Zhenxin Li,Shiyi Lan,Zi Wang,Jose M. Alvarez,Zuxuan Wu*

Main category: cs.RO

TL;DR: HAD是一个用于自动驾驶端到端规划的层次化扩散策略框架，通过粗到细的规划过程、结构保持的轨迹扩展和度量解耦的策略优化，在NAVSIM和HUGSIM基准上实现了新的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶规划面临三个主要挑战：1）直接从整个候选空间选择轨迹难以优化；2）扩散模型中的高斯扰动会产生不现实的轨迹，使去噪过程复杂化；3）现有强化学习方法依赖单一耦合奖励，缺乏结构化信号，限制了优化效果。

Method: 提出HAD框架，包含三个核心组件：1）层次化扩散策略，将规划分解为粗到细的过程；2）结构保持轨迹扩展，生成现实且保持运动学结构的候选轨迹；3）度量解耦策略优化（MDPO），支持跨多个驾驶目标的结构化强化学习优化。

Result: 在NAVSIM和HUGSIM基准测试中，HAD实现了新的最先进性能：在NAVSIM上EPDMS指标提升+2.3，在HUGSIM上路线完成率提升+4.9，大幅超越先前方法。

Conclusion: HAD通过层次化扩散策略、结构保持轨迹扩展和度量解耦优化，有效解决了端到端自动驾驶规划中的关键挑战，在多个基准上取得了显著性能提升，为自动驾驶规划提供了新的有效框架。
...
### [148] [Human-Robot Copilot for Data-Efficient Imitation Learning](https://arxiv.org/abs/2604.03613)
*Rui Yan,Zaitian Gongye,Lars Paulsen,Xuxin Cheng,Xiaolong Wang*

Main category: cs.RO

TL;DR: 提出Human-Robot Copilot框架，通过缩放因子实现灵巧遥操作，同时兼容多种机械臂，在有限演示数据下提高策略性能和数据收集效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于人类干预的模仿学习方法在灵巧性和通用性之间存在权衡：要么提供精细校正但仅限于特定运动结构，要么实现通用性但牺牲精确控制。需要一种既能保持精确控制又能兼容多种机械臂的解决方案。

Method: 提出Human-Robot Copilot框架，通过缩放因子实现灵巧遥操作，同时保持与多种工业和科研机械臂的兼容性。该框架基于Human-Gated DAgger范式，通过选择性人类干预增强演示数据。

Result: 实验结果表明，在相同数量的演示轨迹下，该框架能获得更高的性能表现。由于只需要间歇性校正干预，整体数据收集过程更加高效且耗时更少。

Conclusion: Human-Robot Copilot框架成功解决了现有方法在灵巧性和通用性之间的权衡问题，能够在有限演示数据下提高机器人策略性能，同时提升数据收集效率。
...
### [149] [Towards Edge Intelligence via Autonomous Navigation: A Robot-Assisted Data Collection Approach](https://arxiv.org/abs/2604.03623)
*Tingting Huang,Yingyang Chen,Sixian Qin,Zhijian Lin,Jun Li,Li Wang*

Main category: cs.RO

TL;DR: 提出通信与学习双驱动自主导航方案，在NLoS环境下优化机器人导航、通信和学习性能


<details>
  <summary>Details</summary>
Motivation: 边缘智能系统需要大规模高质量数据，移动机器人被用于主动收集数据，但现有方法在非视距环境中难以实现可靠高效性能

Method: 提出通信与学习双驱动自主导航方案，结合区域感知传播特性和非点质量机器人表示，使用基于MM的高效算法解决非凸非光滑问题

Result: 仿真结果表明，该方案在避障导航、数据收集和模型训练方面优于基准方法，并能通过灵活调整权重因子适应不同场景

Conclusion: CLD方案能够同时优化导航、通信和学习性能，在复杂环境中实现可靠高效的机器人辅助数据收集
...
### [150] [A Multi-View 3D Telepresence System for XR Robot Teleoperation](https://arxiv.org/abs/2604.03730)
*Enes Ulas Dincer,Manuel Zaremski,Alexandra Nick,Elias Wucher,Barbara Deml,Gerhard Neumann*

Main category: cs.RO

TL;DR: 本文提出了一种多视角VR远程呈现系统，融合三个摄像头的几何信息生成GPU加速的点云渲染，并集成腕部RGB流提供高分辨率细节，在Meta Quest 3上实现实时渲染约75k点，显著提升机器人远程操作的性能。


<details>
  <summary>Details</summary>
Motivation: 机器人远程操作在远程维护、搜索救援、机器人学习数据收集等应用中至关重要。传统基于屏幕的界面往往无法提供可靠的深度线索和直观的3D可视化，限制了远程操作的效果。

Method: 开发了多视角VR远程呈现系统：1）融合三个摄像头的几何信息，在独立VR硬件上实现GPU加速的点云渲染；2）集成腕部RGB流，在点云精度有限区域提供高分辨率局部细节。系统在Meta Quest 3上支持约75k点的实时渲染。

Result: 对31名参与者进行组内研究，比较了本系统与其他可视化模式（RGB流、VR设备中的立体视觉投影、无RGB信息的点云）。在三个不同的远程操作任务中，测量任务成功率、完成时间、感知工作量和可用性。本系统获得最佳整体性能，无RGB的点云模式也优于RGB流和OpenTeleVision。

Conclusion: 结合全局3D结构和局部高分辨率细节能显著改善远程操作中的远程呈现效果，为下一代机器人远程操作系统提供了坚实基础。点云可视化即使没有RGB信息也能提供比传统RGB流更好的性能。
...
### [151] [CT-VoxelMap: Efficient Continuous-Time LiDAR-Inertial Odometry with Probabilistic Adaptive Voxel Mapping](https://arxiv.org/abs/2604.03747)
*Lei Zhao,Xingyi Li,Tianchen Deng,Chuan Cao,Han Zhang,Weidong Chen*

Main category: cs.RO

TL;DR: 本文提出了一种基于B样条连续时间表示的改进多传感器融合定位方法，通过矩阵李群表示控制点增量、在线估计拟合误差和混合特征体素地图管理，提高了移动机器人在快速运动和复杂地形下的定位精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 移动机器人在快速运动或复杂地形下保持稳定准确的定位仍然具有挑战性。现有的基于B样条的连续时间里程计方法要么将控制点作为待估计变量，要么在四元数空间进行估计，这导致解析雅可比矩阵推导复杂，并且常常忽略样条与真实轨迹之间的拟合误差。

Method: 1. 在矩阵李群上表示控制点增量作为待估计变量，利用B样条累积形式推导更紧凑的公式，获得更简单的解析雅可比矩阵；2. 利用IMU测量的前向传播信息在线估计拟合误差；3. 引入混合特征体素地图管理策略；4. 提出重估计策略提高计算效率和鲁棒性。

Result: 该方法在多个具有挑战性的公共数据集上进行了评估，在大多数序列上表现出优越性能。详细的消融研究分析了每个模块对整个位姿估计系统的影响。

Conclusion: 所提出的方法通过改进B样条表示、在线拟合误差估计和高效地图管理，显著提高了移动机器人在快速运动和复杂地形下的定位精度、鲁棒性和计算效率。
...
### [152] [Build on Priors: Vision--Language--Guided Neuro-Symbolic Imitation Learning for Data-Efficient Real-World Robot Manipulation](https://arxiv.org/abs/2604.03759)
*Pierrick Lorang,Johannes Huemer,Timothy Duggan,Kai Goebel,Patrik Zips,Matthias Scheutz*

Main category: cs.RO

TL;DR: 提出了一种从少量演示中自动构建符号规划域和数据高效控制策略的神经符号框架，无需人工领域工程，在真实工业叉车和机械臂上验证有效。


<details>
  <summary>Details</summary>
Motivation: 现有神经符号方法通常依赖手工制作的符号抽象、语义标注的轨迹或大量演示数据集，限制了其可扩展性和实际应用。需要一种能够从少量未标注演示中自主学习的框架。

Method: 1) 将演示分割为技能；2) 使用视觉语言模型(VLM)分类技能并识别等价高层状态，自动构建状态转移图；3) 通过ASP求解器合成PDDL规划域；4) 利用oracle函数为每个技能策略隔离最小化、任务相关且目标相对的动作空间；5) 在控制参考层面而非原始执行器信号层面学习策略；6) 利用已知控制器进行真实世界数据增强。

Result: 在真实工业叉车上进行统计严格的操纵试验验证，并在Kinova Gen3机械臂上展示跨平台通用性。结果表明该框架实现了可扩展、数据高效、无需专家且可解释的神经符号机器人学习。

Conclusion: 将控制学习、VLM驱动的抽象和自动规划合成统一到一个管道中，为可扩展、数据高效、无需专家且可解释的神经符号机器人提供了一条实用路径。
...
### [153] [A Novel Hybrid PID-LQR Controller for Sit-To-Stand Assistance Using a CAD-Integrated Simscape Multibody Lower Limb Exoskeleton](https://arxiv.org/abs/2604.03766)
...
Result: 混合PID-LQR控制器相比PID控制器：髋关节和膝关节RMSE分别降低72.3%和70.4%；所有关节的稳定时间减少90%以上；超调量限制在2.39%-6.10%范围内。在所有评估的性能指标上均优于基准策略。

Conclusion: 混合PID-LQR控制器在坐站转换任务中表现出卓越性能，验证了其在临床辅助外骨骼部署中的强大转化潜力，为解决康复机器人中的精确控制挑战提供了有效解决方案。
...
### [154] [OpenRC: An Open-Source Robotic Colonoscopy Framework for Multimodal Data Acquisition and Autonomy Research](https://arxiv.org/abs/2604.03781)
...
TL;DR: OpenRC是一个开源模块化机器人结肠镜框架，可改造传统结肠镜并保留临床工作流程，支持多模态数据同步记录，为机器人结肠镜和手术自主性研究提供可重复基础。


<details>
  <summary>Details</summary>
Motivation: 现有结肠镜平台在系统研究操作员控制、器械运动和视觉反馈耦合动态方面支持有限，限制了机器人结肠镜、医学成像和新兴视觉-语言-动作学习范式的可重复闭环研究。

Method: 开发了OpenRC开源模块化机器人结肠镜框架，可改造传统结肠镜并保留临床工作流程，支持视频、操作员命令、驱动状态和远端尖端姿态的同步记录，通过实验验证运动一致性和跨模态延迟。

Result: 收集了包含1,894个远程操作片段（约19小时）的多模态数据集，涵盖10种结构化任务变化，包括常规导航、失败事件和恢复行为，验证了运动一致性并量化了跨模态延迟。
...
### [155] [DC-Ada: Reward-Only Decentralized Observation-Interface Adaptation for Heterogeneous Multi-Robot Teams](https://arxiv.org/abs/2604.03905)
*Saad Alqithami*

Main category: cs.RO

TL;DR: DC-Ada是一种用于异构多机器人团队的奖励驱动去中心化适应方法，通过调整每个机器人的观测变换来适应异构传感器，保持预训练策略冻结，使用无梯度的随机搜索和最小通信。


<details>
  <summary>Details</summary>
Motivation: 异构性是部署多机器人团队的关键特征：平台在感知模态、范围、视野和故障模式上存在差异。在名义感知下训练的控制器在部署到传感器缺失或不匹配的机器人上时性能会急剧下降，即使任务和动作接口保持不变。

Method: DC-Ada保持预训练共享策略冻结，仅调整紧凑的每个机器人观测变换，将异构感知映射到固定的推理接口。该方法无梯度且通信最小化：使用预算化的接受/拒绝随机搜索，在严格步数预算下进行短期的公共随机数滚动。

Result: 在2D多机器人模拟器中评估了四个异构机制(H0-H3)，结果表明异构性会显著降低冻结共享策略的性能。观测归一化在仓库物流的奖励鲁棒性方面最强，在搜救任务中具有竞争力，而冻结共享策略在协作地图构建的奖励方面最强。DC-Ada在严重覆盖式地图构建中明显提高了完成率。

Conclusion: DC-Ada为异构团队提供了一个实用的部署时适应方法，仅需要标量团队回报，无需策略微调或持久通信，可作为现有方法的补充操作点。
...
### [156] [VA-FastNavi-MARL: Real-Time Robot Control with Multimedia-Driven Meta-Reinforcement Learning](https://arxiv.org/abs/2604.03998)
...
<details>
  <summary>Details</summary>
Motivation: 人机交互需要能够实时响应动态、异构多媒体指令的能力，现有方法在处理感官输入时存在瓶颈，导致延迟问题。

Method: 提出VA-FastNavi-MARL框架，将异步视听输入对齐为统一潜在表示，通过元强化学习将多样化指令视为可导航目标的分布，实现快速适应。

Result: 在多臂工作空间验证中，VA-FastNavi-MARL在样本效率上显著优于基线方法，即使在嘈杂多媒体流下也能保持鲁棒的实时执行。

Conclusion: 该框架通过模态无关的流处理实现了无缝、低延迟控制，为实时人机交互中的动态多媒体指令解释提供了有效解决方案。
...
### [157] [Dynamic Whole-Body Dancing with Humanoid Robots -- A Model-Based Control Approach](https://arxiv.org/abs/2604.03999)
...
<details>
  <summary>Details</summary>
Motivation: 为人形机器人开发能够在真实环境中执行动态、鲁棒舞蹈动作的系统，解决传统方法在动态性和适应性方面的限制

Method: 两阶段框架：1) 离线阶段：通过动作捕捉获取人类舞蹈演示，通过二次规划重定向到机器人，再通过轨迹优化确保动态可行性；2) 在线阶段：基于质心动力的模型预测控制框架实时跟踪计划动作，主动调整摆动脚位置以适应扰动

Result: 在Kuavo 4Pro全尺寸人形机器人上验证，在仿真和4分钟现场公开表演中展示了动态舞蹈动作，实验表明更长的预测时域能提高运动表现力和执行稳定性

Conclusion: 该集成框架成功实现了人形机器人的动态舞蹈动作生成和执行，证明了基于模型预测的方法在提高机器人舞蹈动态性和鲁棒性方面的有效性
...
### [158] [Adapting Neural Robot Dynamics on the Fly for Predictive Control](https://arxiv.org/abs/2604.04039)
*Abdullah Altawaitan,Nikolay Atanasov*

Main category: cs.RO

TL;DR: 提出一种快速适应神经机器人动力学模型的方法，结合离线训练与高效在线更新，实现无人机在新型操作条件下的鲁棒预测跟踪控制。


<details>
  <summary>Details</summary>
Motivation: 物理模型过于简化难以捕捉真实世界效应，数据驱动模型需要大量数据且训练缓慢，需要一种能够快速适应新操作条件的动力学模型方法。

Method: 学习增量神经动力学模型进行离线训练，在线采用低秩二阶参数适应方法，实现快速更新而无需完全重新训练。

Result: 在真实四旋翼无人机上验证了该方法，能够在新型操作条件下实现鲁棒的预测跟踪控制。

Conclusion: 提出的快速适应神经动力学模型方法有效解决了传统物理模型过于简化、数据驱动模型训练缓慢的问题，为自主移动机器人的预测控制器设计提供了实用解决方案。
...
### [159] [Learning Dexterous Grasping from Sparse Taxonomy Guidance](https://arxiv.org/abs/2604.04138)
...
TL;DR: GRIT是一个两阶段框架，通过稀疏分类学指导学习灵巧操作，先预测基于分类学的抓取规范，再基于此生成连续手指动作，在保持预期抓取结构的同时完成任务。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要为每个物体和任务指定密集的位姿或接触目标，这不切实际；而端到端强化学习缺乏可控性，用户难以在失败时干预。需要一种既能学习灵巧控制又保持用户可控性的方法。

Method: GRIT采用两阶段框架：第一阶段从场景和任务上下文预测基于分类学的稀疏抓取规范；第二阶段基于这个稀疏命令，策略生成连续的手指动作，在完成任务的同时保持预期的抓取结构。

Result: 研究发现某些抓取分类学对特定物体几何更有效。GRIT利用这种关系，相比基线方法在新物体上具有更好的泛化能力，总体成功率达到87.9%。真实世界实验展示了可控性，用户可以通过高级分类学选择根据物体几何和任务意图调整抓取策略。

Conclusion: GRIT框架通过稀疏分类学指导实现了灵巧操作的可控学习，既避免了为每个任务指定密集目标的繁琐，又保持了用户干预的能力，在泛化性和可控性方面都表现出色。
...
### [160] [Adaptive Action Chunking at Inference-time for Vision-Language-Action Models](https://arxiv.org/abs/2604.04161)
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在推理时使用固定的动作分块大小，这限制了模型在不同操作任务中的优越性和可扩展性。大分块降低对新信息的响应性，小分块增加模式跳跃和不连续行为的风险，因此需要自适应调整分块大小

Method: 提出自适应动作分块策略，利用当前预测的动作熵作为线索，动态确定最优的分块大小，平衡模型的响应性和一致性

Result: 在广泛的模拟和真实世界机器人操作任务上进行实验，证明该方法显著优于现有最先进方法

Conclusion: 自适应动作分块策略通过动态调整分块大小，有效解决了VLA模型中固定分块大小带来的局限性，提升了机器人操作性能
...
### [161] [Primitive-based Truncated Diffusion for Efficient Trajectory Generation of Differential Drive Mobile Manipulators](https://arxiv.org/abs/2604.04166)
*Long Xu,Choilam Wong,Yuhang Zhong,Junxiao Lin,Jialiang Hou,Fei Gao*

Main category: cs.RO

TL;DR: 提出一种用于差速驱动移动机械臂的学习增强运动规划器，通过关键点序列提取和基于基元的截断扩散模型，提高规划效率、成功率和最优性。


<details>
  <summary>Details</summary>
Motivation: 针对差速驱动移动机械臂在复杂3D环境中的运动规划问题，现有方法在效率、成功率和轨迹多样性方面存在不足，需要开发更有效的学习增强规划器。

Method: 1) 关键点序列提取模块：通过可微分正向运动学将边界状态映射到3D空间；2) 点云和关键点分别编码并通过注意力融合；3) 基于基元的截断扩散模型从偏置分布中采样；4) 轨迹优化确保动态可行性和任务特定最优性。

Result: 在杂乱3D仿真环境中，相比原始扩散模型和经典基线方法，该方法实现了更高的成功率、改进的轨迹多样性和具有竞争力的运行时间。

Conclusion: 提出的学习增强运动规划器通过关键点序列编码和截断扩散模型，有效提高了差速驱动移动机械臂在复杂环境中的规划性能，在效率、成功率和多样性方面均表现出优势。
...
### [162] [Robots Need Some Education: On the complexity of learning in evolutionary robotics](https://arxiv.org/abs/2604.04196)
*Fuda van Diggelen*

Main category: cs.RO

TL;DR: 该论文探讨了进化机器人与机器人学习两个领域的区别与整合，重点研究了在进化机器人背景下设计合适的学习算法。


<details>
  <summary>Details</summary>
Motivation: 进化机器人和机器人学习都是自动优化机器人设计的方法，但两者在优化对象和时间尺度上存在差异。整合这两个领域需要精心设计适合进化机器人背景的学习算法，而引入学习对进化过程的影响尚不明确，需要深入研究。

Method: 论文研究了在进化机器人背景下整合学习算法的复杂性，并开发了多个专门为进化机器人环境设计的学习算法。

Result: 论文提出了多个适用于进化机器人背景的学习算法，深入探讨了学习与进化整合的复杂性和挑战。

Conclusion: 进化机器人与机器人学习的整合需要专门设计的学习算法，这种整合具有复杂性，但通过适当的方法可以实现两个领域的优势互补。
...
### [163] [RK-MPC: Residual Koopman Model Predictive Control for Quadruped Locomotion in Offroad Environments](https://arxiv.org/abs/2604.04221)
*Sriram S. K. S. Narayanan,Umesh Vaidya*

Main category: cs.RO

TL;DR: 本文提出了一种基于Koopman算子的残差模型预测控制框架（RK-MPC），用于四足机器人运动控制，通过数据驱动的残差预测器提升模型预测精度，同时保持实时计算能力。


<details>
  <summary>Details</summary>
Motivation: 传统模板模型在四足机器人运动控制中存在模型失配问题，特别是在接触变化和地形干扰情况下。需要一种既能提高预测精度又能保持实时计算能力的数据驱动控制方法。

Method: 在名义模板模型基础上，通过数据学习一个紧凑的线性残差预测器（在提升坐标中），将学习到的残差模型嵌入凸二次规划MPC框架中，形成500Hz的实时控制器。

Result: 在Gazebo仿真和Unitree Go1硬件实验中，RK-MPC在接触干扰、多种步态调度和具有挑战性的越野地形（草地、砾石、雪地、冰面）上实现了可靠的盲运动。相比传统Koopman/EDMD基线，残差校正改善了多步预测和闭环性能，降低了对观测基选择的敏感性。

Conclusion: RK-MPC为四足机器人在非结构化环境中的数据驱动预测控制提供了一个实用且经过硬件验证的途径，在保持优化控制结构和约束处理优势的同时，显著提升了预测精度和鲁棒性。
...
### [164] [Real-Time Projected Adaptive Control for Closed-Chain Co-Manipulative Continuum Robots](https://arxiv.org/abs/2604.04286)
*Rana Danesh,Farrokh Janabi-Sharifi,Farhad Aghili*

Main category: cs.RO

TL;DR: 提出了一种针对协作连续体机器人的投影自适应控制框架，能够在线补偿机器人和柔性物体的不确定动态参数，确保任务空间跟踪误差收敛到零。


<details>
  <summary>Details</summary>
Motivation: 协作连续体机器人形成闭链可变形机械系统，存在强动态相互作用和内力。实际应用中柔性物体的物理参数通常未知且变化，使得基于标称模型的控制器不足。

Method: 采用几何变量应变表示建立有限维模型，通过Pfaffian速度约束处理闭链相互作用，使用正交投影将动力学表达在约束一致运动子空间，基于投影动力学开发自适应控制律。

Result: Lyapunov分析证明了闭环稳定性和任务空间跟踪误差收敛到零。在肌腱驱动协作连续体机器人平台上通过仿真和实验验证了框架在任务空间调节和轨迹跟踪中的有效性。

Conclusion: 提出的投影自适应控制框架能够有效处理协作连续体机器人系统中的参数不确定性，为闭链可变形机械系统的控制提供了理论保证和实用解决方案。
...
### [165] [frax: Fast Robot Kinematics and Dynamics in JAX](https://arxiv.org/abs/2604.04310)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器人动力学库通常在低延迟CPU执行或高吞吐GPU工作负载中表现优异，但缺乏一个统一的框架能在多种架构上保持高性能和易用性。

Method: 开发了frax库，采用完全向量化的机器人动力学方法，基于JAX实现，支持自动微分，提供纯Python接口。

Result: 在CPU上达到微秒级计算时间，适合千赫兹控制频率，性能优于常见Python库并接近优化C++实现；在GPU上可扩展到数千个实例，达到每秒1亿次动力学评估。

Conclusion: frax成功解决了现有库在跨架构统一性和性能平衡方面的不足，为机器人控制、规划和学习提供了高性能、易用的动力学计算框架。
...
### [166] [Towards Considerate Human-Robot Coexistence: A Dual-Space Framework of Robot Design and Human Perception in Healthcare](https://arxiv.org/abs/2604.04374)
*Yuanchen Bai,Zijian Ding,Ruixiang Han,Niti Parikh,Wendy Ju,Angelique Taylor*

Main category: cs.RO

TL;DR: 该研究通过深度访谈探索人机共存的动态过程，提出人类感知空间的概念框架，并建立人类感知与机器人设计之间的协同演化循环，倡导"体贴的人机共存"理念。


<details>
  <summary>Details</summary>
Motivation: 当前机器人技术快速发展，人机共存已超越物理空间共享，呈现出组织嵌入性、时间演化性、社会情境性和开放不确定性等特征。然而，现有研究多关注静态的态度和接受度，缺乏对人类感知形成演化过程以及人类在动态共存中主动作用的深入理解。

Method: 通过对14周医疗机器人协同设计研究的9名参与者进行深度追踪访谈，识别人类感知空间，包括四个解释维度：分解程度、时间取向、推理范围和证据来源。

Result: 建立了人类感知空间与机器人设计空间之间的协同演化循环概念框架，其中人类需求、设计决策、情境解释和社会调解随时间不断相互重塑。提出了"体贴的人机共存"理念。

Conclusion: 人类不仅是设计贡献者，更是解释者和调解者，在机器人部署的各个阶段主动塑造机器人的理解和整合方式。这丰富了人机共存的概念框架，强调其动态、协同演化的本质。
...
### [167] [ReinVBC: A Model-based Reinforcement Learning Approach to Vehicle Braking Controller](https://arxiv.org/abs/2604.04401)
*Haoxin Lin,Junjie Zhou,Daheng Xu,Yang Yu*

Main category: cs.RO

TL;DR: 提出ReinVBC方法，使用离线模型强化学习解决车辆制动控制问题，通过工程化设计获得可靠的车辆动力学模型和制动策略，有望替代生产级防抱死制动系统


<details>
  <summary>Details</summary>
Motivation: 当前车辆制动系统在生产过程中需要大量人工校准，减少人工和时间消耗同时保持车辆制动控制器性能对汽车行业有重大益处

Method: 采用离线模型强化学习方法，在数据驱动的动力学模型中探索策略，引入工程化设计改进模型学习和利用范式

Result: 方法在实际车辆制动中表现出色，具有替代生产级防抱死制动系统的潜力

Conclusion: ReinVBC方法为车辆制动控制问题提供了有效的离线模型强化学习解决方案，通过工程化设计实现了可靠的车辆动力学模型和制动策略
...
### [168] [Veo-Act: How Far Can Frontier Video Models Advance Generalizable Robot Manipulation?](https://arxiv.org/abs/2604.04502)
*Zhongru Zhang,Chenghan Yang,Qingzhou Lu,Yanjiang Guo,Jianke Zhang,Yucheng Hu,Jianyu Chen*

Main category: cs.RO

TL;DR: 研究探索前沿视频生成模型Veo-3在机器人操作任务中的零样本应用，发现其能生成合理的高层运动轨迹但底层控制精度不足，进而提出分层框架Veo-Act结合VLA策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型在物理动力学理解方面进展迅速，本研究旨在探索先进视频生成模型（如Veo-3）如何支持可泛化的机器人操作，验证视频模型在机器人学习中的潜力。

Method: 首先采用零样本方法：Veo-3从当前机器人观测预测未来图像序列，逆动力学模型IDM恢复对应机器人动作（仅用随机游戏数据训练）。随后提出分层框架Veo-Act：Veo-3作为高层运动规划器，VLA策略作为底层执行器。

Result: Veo-3+IDM能一致生成近似正确的任务级轨迹，但底层控制精度不足以可靠解决大多数任务。Veo-Act框架显著提升了最先进视觉-语言-动作策略的指令跟随性能。

Conclusion: 随着视频生成模型的持续改进，视频模型可以成为可泛化机器人学习的有价值组件，分层方法结合高层视频规划与底层策略执行能有效提升机器人操作能力。
...
### [169] [G-EDF-Loc: 3D Continuous Gaussian Distance Field for Robust Gradient-Based 6DoF Localization](https://arxiv.org/abs/2604.04525)
*José E. Maese,Lucía Coto-Elena,Luis Merino,Fernando Caballero*

Main category: cs.RO

TL;DR: 提出基于G-EDF连续距离场表示的6-DoF定位框架，通过CPU直接扫描到地图配准实现实时高精度定位


<details>
  <summary>Details</summary>
Motivation: 解决传统定位方法在里程计退化或缺乏IMU先验时的鲁棒性问题，同时提供连续、内存高效的地图表示

Method: 采用基于块稀疏高斯混合模型的自适应空间分割方法建模欧几里得距离场，确保C^1连续性，利用解析梯度实现Eikonal一致性

Result: 在大规模数据集上表现与最先进方法相当，在严重里程计退化或完全缺乏IMU先验时展现出卓越的鲁棒性

Conclusion: G-EDF-Loc框架通过连续距离场表示实现了实时高精度定位，在挑战性场景中具有优越的鲁棒性
...
### [170] [Visual Prompt Based Reasoning for Offroad Mapping using Multimodal LLMs](https://arxiv.org/abs/2604.04564)
*Abdelmoamen Nasser,Yousef Baba'a,Murad Mebrahtu,Nadya Abdel Madjid,Jorge Dias,Majid Khonji*

Main category: cs.RO

TL;DR: 提出一种零样本的越野自动驾驶方法，使用SAM2进行环境分割和视觉语言模型（VLM）来推理可行驶区域，无需单独训练地形分类、高度估计等模型。


<details>
  <summary>Details</summary>
Motivation: 传统越野自动驾驶方法需要分别训练地形分类、高度估计、滑移/坡度量化等多个模型，这需要为每个组件单独训练、准备特定任务数据集和微调，过程复杂且效率低。

Method: 使用SAM2进行环境分割，然后将原始图像和带有数字标签的掩码分割图像输入VLM，通过提示让VLM识别哪些数字标签对应的区域是可行驶的。结合规划和控制模块，形成统一框架。

Result: 该方法在高分辨率分割数据集上超越了最先进的可训练模型，并在Isaac Sim越野环境中实现了完整的导航堆栈。

Conclusion: 提出的零样本方法利用VLM的固有推理能力，消除了对显式地形特定模型的需求，为越野自动驾驶提供了一种更简单有效的统一解决方案。
...
### [171] [Biologically Inspired Event-Based Perception and Sample-Efficient Learning for High-Speed Table Tennis Robots](https://arxiv.org/abs/2604.04618)
*Ziqi Wang,Jingyue Zhao,Xun Xiao,Jichao Yang,Yaohua Wang,Shi Xu,Lei Wang,Huadong Dai*

Main category: cs.RO

TL;DR: 本文提出了一种受生物启发的乒乓球机器人高速感知与决策方法，结合事件相机感知和样本高效学习，在真实对抗场景中实现35.8%的击球精度提升。


<details>
  <summary>Details</summary>
Motivation: 传统机器人难以在高速动态场景中进行实时感知和决策，而人类和动物却能快速应对。传统基于帧的视觉传感器存在运动模糊、高延迟和数据冗余问题，无法满足实时准确感知需求。现有事件感知方法局限于简化的单球场景，而现有决策方法需要大量环境交互，计算成本高。

Method: 1. 感知方面：提出基于事件相机的乒乓球检测方法，利用运动线索和几何一致性，直接在异步事件流上操作，无需帧重建；2. 决策方面：提出受人类启发的样本高效训练策略，先在低速场景训练策略，逐步从基础到高级获取技能，然后通过案例依赖的时间自适应奖励和奖励阈值机制适应高速场景。

Result: 在相同训练回合数下，该方法将回球到目标位置的精度提高了35.8%，证明了生物启发的感知和决策方法对高速机器人系统的有效性。

Conclusion: 结合事件相机感知和样本高效学习的生物启发方法能够有效解决高速乒乓球机器人的感知和决策挑战，为高速动态场景下的机器人系统提供了有前景的解决方案。
...
### [172] [WaterSplat-SLAM: Photorealistic Monocular SLAM in Underwater Environment](https://arxiv.org/abs/2604.04642)
*Kangxu Wang,Shaofeng Zou,Chenxing Jiang,Yixiang Dai,Siang Chen,Shaojie Shen,Guijin Wang*

Main category: cs.RO

TL;DR: WaterSplat-SLAM：一种新颖的单目水下SLAM系统，通过语义介质滤波和自适应高斯地图管理，实现鲁棒的姿态估计和逼真的密集建图
...
Conclusion: WaterSplat-SLAM系统通过创新的语义介质滤波和自适应地图管理方法，成功解决了水下单目SLAM中姿态估计和高质量建图的挑战
...
### [173] [Pickalo: Leveraging 6D Pose Estimation for Low-Cost Industrial Bin Picking](https://arxiv.org/abs/2604.04690)
*Alessandro Tarsi,Matteo Mastrogiuseppe,Saverio Taliani,Simone Cortinovis,Ugo Pattacini*

Main category: cs.RO

TL;DR: Pickalo是一个基于低成本硬件的模块化6D位姿抓取系统，通过多视角主动探索、深度优化、零样本分割和位姿融合等技术，在工业环境中实现高效稳定的抓取性能。


<details>
  <summary>Details</summary>
Motivation: 工业环境中的抓取面临严重杂乱、遮挡以及传统3D感知系统成本高昂的挑战，需要开发基于低成本硬件的可靠抓取解决方案。

Method: 系统采用手腕安装的RGB-D相机多视角主动探索，使用BridgeDepth优化深度图，通过合成数据训练的Mask-RCNN进行实例分割，利用SAM-6D零样本位姿估计，结合位姿缓冲模块融合多视角观测，并采用离线生成抓取候选和在线效用排序的抓取规划策略。

Result: 在UR5e机械臂和Intel RealSense D435i相机上部署，Pickalo实现每小时600次平均抓取，成功率96-99%，在密集填充的欧标箱上保持30分钟稳定运行。

Conclusion: Pickalo证明了基于低成本硬件的抓取系统在工业环境中的可行性，深度优化和位姿缓冲模块对长期稳定性和吞吐量有显著提升。
...
### [174] [AnyUser: Translating Sketched User Intent into Domestic Robots](https://arxiv.org/abs/2604.04811)
*Songyuan Yang,Huibin Tan,Kailun Yang,Wenjing Yang,Shaowu Yang*

Main category: cs.RO

TL;DR: AnyUser是一个统一的机器人指令系统，允许用户通过在相机图像上自由绘制草图（可选结合语言）来直观地指导家用任务，无需先验地图或模型。


<details>
  <summary>Details</summary>
Motivation: 为了解决先进机器人能力与非专家用户可访问交互之间的差距，让普通用户（包括老年人、非语言表达者、技术素养低的人群）能够轻松地向机器人下达任务指令。

Method: 系统采用多模态融合理解（草图、视觉、语言）将输入解释为空间语义基元，通过分层策略生成可执行的机器人动作。核心创新包括多模态融合理解和分层动作生成策略。
...
Conclusion: AnyUser成功弥合了先进机器人能力与非专家交互需求之间的差距，为适应真实世界人类环境的实用辅助机器人奠定了基础。
...
### [175] [Efficient Multi-Objective Planning with Weighted Maximization Using Large Neighbourhood Search](https://arxiv.org/abs/2604.04826)
*Krishna Kalavadia,Shamak Dutta,Yash Vardhan Pant,Stephen L. Smith*

Main category: cs.RO

TL;DR: 提出基于大邻域搜索框架的新算法，高效解决加权最大值规划问题，相比现有方法运行时间提升1-2个数量级，使加权最大值方法在自主导航中实用化


<details>
  <summary>Details</summary>
Motivation: 自主导航需要同时优化多个目标。传统加权和方法无法找到所有可能的权衡解，特别是非凸区域的帕累托最优解。加权最大值方法能找到所有帕累托最优解，但在离散域中计算复杂度高，限制了实际应用。

Method: 基于大邻域搜索框架提出新颖搜索算法，高效解决加权最大值规划问题。该算法通过智能的邻域搜索策略，在保持解质量的同时大幅降低计算复杂度。

Result: 通过大量仿真验证，新算法在解质量上与现有加权最大值规划器相当，但运行时间提升了1-2个数量级，使加权最大值方法在自主导航中成为可行选择。

Conclusion: 提出的基于大邻域搜索的算法显著降低了加权最大值规划的计算复杂度，使这种方法能够实际应用于自主导航系统，解决了传统加权和方法无法找到非凸区域帕累托最优解的问题。
...
### [176] [Outlier-Robust Nonlinear Moving Horizon Estimation using Adaptive Loss Functions](https://arxiv.org/abs/2604.04862)
...
<details>
  <summary>Details</summary>
Motivation: 在移动地平线估计（MHE）中，传统L2损失函数对异常值敏感，需要一种能够自适应调整鲁棒性的方法，既能处理异常值污染数据，又能在无异常值时保持传统L2性能

Method: 提出自适应鲁棒损失函数框架，包含自适应鲁棒损失函数和正则化项；通过调节参数控制损失函数形状以调整对异常值的鲁棒性；优先拟合未污染数据并降低污染数据权重

Result: 仿真结果表明，该方法在仅几次迭代内就能实现自适应调整，当测量数据无异常值时，传统L2行为占主导地位

Conclusion: 提出的自适应鲁棒损失函数框架有效解决了MHE中的异常值问题，既能快速适应异常值污染情况，又能在无异常值时保持传统L2估计器的良好性能
...
### [177] [Scaling DPPs for RAG: Density Meets Diversity](https://arxiv.org/abs/2604.03240)
*Xun Sun,Baiheng Xie,Li Huang,Qiang Gao*

Main category: cs.LG

TL;DR: 提出ScalDPP，一种用于RAG的多样性感知检索机制，通过轻量级P-Adapter整合DPP，建模块间依赖关系并选择互补上下文，解决传统RAG检索冗余问题。


<details>
  <summary>Details</summary>
Motivation: 标准RAG管道通过相关性排序构建上下文，对用户查询和每个语料块进行点对点评分，但忽略了检索候选之间的交互，导致冗余上下文稀释信息密度且无法发现互补证据。

Method: 提出ScalDPP，一种基于确定性点过程（DPPs）的多样性感知检索机制，通过轻量级P-Adapter实现可扩展的块间依赖建模和互补上下文选择。同时开发了新颖的集合级目标函数Diverse Margin Loss（DML），强制真实互补证据链在DPP几何下主导任何同等大小的冗余替代方案。

Result: 实验结果证明了ScalDPP的优越性，在实践中验证了核心观点。

Conclusion: 有效的检索应同时优化密度和多样性，确保基础证据既信息密集又覆盖多样。ScalDPP通过整合DPP和轻量级适配器，实现了可扩展的多样性感知检索，提升了RAG系统的性能。
...
### [178] [DRAFT: Task Decoupled Latent Reasoning for Agent Safety](https://arxiv.org/abs/2604.03242)
*Lin Wang,Junfeng Fang,Dan Zhang,Fei Shen,Xiang Wang,Tat-Seng Chua*

Main category: cs.LG

TL;DR: DRAFT是一个用于LLM智能体安全监控的两阶段潜在推理框架，通过提取器将长轨迹压缩为连续潜在草案，再由推理器联合关注草案和原始轨迹进行安全预测，显著提升稀疏证据场景下的安全判断准确性。


<details>
  <summary>Details</summary>
Motivation: 工具使用LLM智能体的出现使得安全监控从输出审核转向审计长而嘈杂的交互轨迹，其中风险关键证据稀疏，传统的二元监督方法难以进行信用分配，需要新的解决方案。

Method: 提出DRAFT框架，将安全判断解耦为两个可训练阶段：提取器将完整轨迹蒸馏为紧凑的连续潜在草案；推理器联合关注草案和原始轨迹来预测安全性。该方法在潜在空间进行证据聚合，避免有损的显式"总结-判断"流水线，支持端到端可微分训练。

Result: 在ASSEBench和R-Judge等基准测试中，DRAFT始终优于强基线，准确率从LoRA的63.27%提升到平均91.18%，并学习到更可分离的表示。消融研究显示提取器和推理器之间存在明显的协同效应。

Conclusion: DRAFT表明，在稀疏证据的长上下文监督下，读取前的连续潜在推理是实现鲁棒智能体安全的实用路径。
...
### [179] [Apparent Age Estimation: Challenges and Outcomes](https://arxiv.org/abs/2604.03335)
*Justin Rainier Go,Lorenz Bernard Marqueses,Mikaella Kaye Martinez,John Kevin Patrick Sarmiento,Abien Fred Agarap*

Main category: cs.LG

TL;DR: 论文回顾了DEX方法，应用了MVL和AMRL等分布学习技术，评估了准确性和公平性。研究发现AMRL在准确性上达到SOTA，但精度与人口统计公平性之间存在权衡，亚洲和非裔美国人群体性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 表观年龄估计是商业个性化的重要工具，但现有模型常表现出人口统计偏见。需要评估现有方法在准确性和公平性方面的表现，并探索技术改进是否能解决公平性问题。
...
Result: AMRL在准确性上达到SOTA水平，但精度与人口统计公平性之间存在权衡。UMAP嵌入显示清晰的年龄聚类，但显著图表明不同人口统计群体的特征关注不一致，导致亚洲和非裔美国人群体性能显著下降。
...
### [180] [The limits of bio-molecular modeling with large language models : a cross-scale evaluation](https://arxiv.org/abs/2604.03361)
*Yaxin Xu,Yue Zhou,Tianyu Zhao,Fengwei An,Zhixiang Ren*

Main category: cs.LG

TL;DR: BioMol-LLM-Bench：一个跨尺度生物分子基准测试，评估大语言模型在生物分子发现中的表现，发现当前模型在分类任务上表现良好但在回归任务上较弱，混合Mamba注意力架构对长序列更有效
...
Method: 提出了BioMol-LLM-Bench跨尺度生物分子基准测试，这是一个统一框架，包含26个下游任务，涵盖4个不同的难度级别，并整合了计算工具进行更全面的评估。在13个代表性模型上进行了评估

Result: 评估揭示了4个主要发现：思维链数据对生物任务益处有限甚至可能降低性能；混合Mamba注意力架构对长生物分子序列更有效；监督微调提高了专业化但牺牲了泛化能力；当前LLM在分类任务上表现良好但在具有挑战性的回归任务上仍然较弱
...
### [181] [Scalable Variational Bayesian Fine-Tuning of LLMs via Orthogonalized Low-Rank Adapters](https://arxiv.org/abs/2604.03388)
...
<details>
  <summary>Details</summary>
Motivation: 在安全关键应用中部署大语言模型时，不确定性量化至关重要，但现有方法存在校准效果不佳或推理效率低的问题。特别是经过参数高效微调后，模型决策容易过度自信。

Method: 提出PoLAR-VBLL框架：1) 使用极分解低秩适配器表示(PoLAR)进行正交化参数化，配合黎曼优化实现更稳定和表达力更强的适配；2) 基于贝叶斯最后一层模型，通过变分推断框架联合优化PoLAR参数和最后一层参数的近似后验分布。

Result: 实验结果表明，PoLAR-VBLL在多种常识推理任务中，无论是分布内还是分布外数据，都能有效提升模型的泛化能力和不确定性估计质量。

Conclusion: PoLAR-VBLL成功地将架构增强优化与可扩展贝叶斯推理相结合，为大语言模型提供了校准良好的不确定性量化能力，解决了现有方法的局限性。
...
### [182] [MetaSAEs: Joint Training with a Decomposability Penalty Produces More Atomic Sparse Autoencoder Latents](https://arxiv.org/abs/2604.03436)
*Matthew Levinson*

Main category: cs.LG

TL;DR: 提出一种联合训练目标，通过元SAE稀疏重构主SAE的解码器列，直接惩罚子空间混合，提高稀疏自编码器特征的原子性


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器在安全相关应用（如对齐检测和模型引导）中需要特征尽可能原子化，每个潜在特征应代表来自单一底层表示子空间的单一连贯概念。但实践中SAE特征会将不同表示子空间混合在一起，导致特征在语义不同的上下文中激活，模糊模型计算图景。

Method: 引入联合训练目标：一个小型元SAE与主SAE同时训练，稀疏重构主SAE的解码器列；当主SAE的解码器方向容易被元字典重构时，主SAE会受到惩罚。这发生在潜在方向位于其他主方向张成的子空间中时，从而产生梯度压力，促使解码器方向更加相互独立，抵抗稀疏元压缩。

Result: 在GPT-2 large（第20层）上，所选配置使平均|φ|相对于相同数据训练的相同单独SAE降低了7.5%。自动可解释性（模糊测试）得分提高了7.6%，独立于训练和共现指标验证了原子性增益。重建开销适中。在Gemma 2 9B上的结果是方向性的，在未完全收敛的SAE上，相同参数化产生最佳结果，ΔFuzz为+8.6%。

Conclusion: 该方法通过联合训练目标有效减少SAE特征的子空间混合，提高特征原子性，在GPT-2 large上验证了效果，在更大模型Gemma 2 9B上显示出有希望的迁移性。定性分析确认多义词特征被拆分为语义不同的子特征，各自专注于不同的表示子空间。
...
### [183] [Olmo Hybrid: From Theory to Practice and Back](https://arxiv.org/abs/2604.03444)
...
TL;DR: 本文通过理论和实践证明，混合注意力与循环层的模型比纯Transformer模型更具表达力、扩展效率更高，在预训练和下游任务中表现更优。


<details>
  <summary>Details</summary>
Motivation: 当前非Transformer语言模型（如线性RNN和混合模型）显示出潜力，但缺乏共识：这些新架构的潜在收益是否值得投入资源进行规模化。需要系统评估混合模型相比纯Transformer的实际优势。

Method: 1. 理论分析：证明混合模型不仅能继承Transformer和线性RNN的表达能力，还能表达超越两者的任务（如代码执行）。2. 实践验证：训练Olmo Hybrid（7B参数），将Olmo 3 7B的滑动窗口层替换为Gated DeltaNet层，在标准预训练和中训练评估中进行对比。3. 扩展效率分析：比较混合模型与Transformer的扩展效率。

Result: 1. Olmo Hybrid在预训练和中训练评估中全面优于Olmo 3。2. 混合模型的扩展效率显著高于Transformer，这解释了其更好的性能。3. 理论分析表明，增强的表达能力应该转化为更好的扩展效率，形成了完整的理论-实践闭环。

Conclusion: 混合注意力与循环层的模型是语言建模范式的重要扩展：不仅能减少推理时的内存使用，更重要的是作为获得更具表达力、预训练扩展性更好的模型的基本方法。
...
### [184] [Earth Embeddings Reveal Diverse Urban Signals from Space](https://arxiv.org/abs/2604.03456)
*Wenjing Gong,Udbhav Srivastava,Yuchen Wang,Yuhao Jia,Qifan Wu,Weishan Bai,Yifan Yang,Xiao Huang,Xinyue Ye*

Main category: cs.LG

TL;DR: 研究评估了三种地球嵌入模型（AlphaEarth、Prithvi、Clay）在预测美国六个大都市区14个邻里尺度城市指标方面的性能，发现这些嵌入能有效捕捉城市变异，尤其对与建成环境结构直接相关的指标预测效果最佳。


<details>
  <summary>Details</summary>
Motivation: 传统城市指标（来自人口普查、调查和行政记录）通常成本高、空间不一致且更新缓慢。最近的地理空间基础模型能够生成可迁移的地球嵌入，但其在邻里尺度城市监测中的实用性尚不清楚。

Method: 使用统一的监督学习框架，在2020-2023年间对美国六个大都市区进行基准测试，预测14个邻里级指标（犯罪、收入、健康、出行行为等），并在四种设置下评估性能：全局、城市级、年度和城市-年度组合。

Result: 地球嵌入能捕捉显著的城市变异，对与建成环境结构直接相关的指标（如慢性健康负担和主要通勤方式）预测效果最佳；而受精细尺度行为和本地政策影响更大的指标（如骑行）仍难以推断。预测性能在城市间差异显著但在时间上相对稳定，表明空间异质性强而时间稳健性好。紧凑的64维AlphaEarth嵌入比Prithvi和Clay的降维版本更具信息量。

Conclusion: 本研究为评估城市遥感中的地球嵌入建立了基准，并展示了它们作为可扩展、低成本特征用于SDG对齐的邻里尺度城市监测的潜力。
...
### [185] [Super Agents and Confounders: Influence of surrounding agents on vehicle trajectory prediction](https://arxiv.org/abs/2604.03463)
*Daniel Jost,Luca Paparusso,Martin Stoll,Jörg Wagner,Raghu Rajan,Joschka Bödecker*

Main category: cs.LG

TL;DR: 研究发现当前轨迹预测模型存在严重缺陷：周围交通参与者信息反而降低预测准确性，模型学习到不稳定、非因果的决策模式。提出条件信息瓶颈方法，无需额外监督即可有效压缩有用特征并忽略有害信息，提升预测性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在高度交互的驾驶场景中，轨迹预测通常依赖于周围交通参与者（如车辆和行人）的信息。研究发现现有最先进的轨迹预测模型存在一个令人惊讶的关键缺陷：许多周围智能体反而会降低预测准确性而非提升。模型学习到不稳定且非因果的决策模式，在不同训练运行中变化显著。

Method: 首先使用基于Shapley值的归因方法，严格证明模型学习到不稳定和非因果的决策方案。基于这些发现，提出集成条件信息瓶颈（CIB）方法，该方法不需要额外监督，训练用于有效压缩智能体特征，并忽略对预测任务无益的特征。

Result: 在多个数据集和模型架构上的综合实验表明，这种简单而有效的方法在许多情况下不仅提高了整体轨迹预测性能，还增加了对不同扰动的鲁棒性。结果强调了在轨迹预测中需要选择性整合上下文信息的重要性，因为这些信息常常包含虚假或误导性信号。

Conclusion: 研究揭示了轨迹预测模型的关键缺陷，并提出了一种有效的解决方案。条件信息瓶颈方法能够改善预测性能并增强鲁棒性，同时提供了识别非鲁棒行为的可解释性指标，为解决该问题开辟了有前景的途径。
...
### [186] [Investigating Data Interventions for Subgroup Fairness: An ICU Case Study](https://arxiv.org/abs/2604.03478)
*Erin Tan,Judy Hanwen Shen,Irene Y. Chen*

Main category: cs.LG

TL;DR: 数据扩展对医疗AI模型公平性的影响复杂且不稳定，传统"更好数据"理念面临挑战，需要结合数据策略和模型校准方法


<details>
  <summary>Details</summary>
Motivation: 在高风险医疗决策场景中，机器学习模型的算法偏见会加剧对特定亚群的系统性伤害。这些偏见通常源于训练数据，而实践中"修复数据"的干预措施依赖于可用的额外数据源，但这些数据源往往不理想。数据扩展对亚群性能的影响变得不稳定，因为样本量增加带来的改善会被训练集中引入的分布偏移所抵消。

Method: 在医疗健康背景下研究结合数据源改善亚群性能的局限性。使用eICU协作研究数据库和MIMIC-IV数据集这两个临床数据集，比较模型后验校准方法和数据中心化添加策略，探索两者结合的重要性。

Result: 研究发现数据添加既可能帮助也可能损害模型的公平性和性能，许多直观的数据选择策略并不可靠。数据扩展对亚群性能的影响是复杂的，传统"更好数据"的理念面临挑战。

Conclusion: 需要结合数据策略和模型校准方法来改善亚群性能。研究质疑了通过"更好数据"来克服公平性挑战的传统教条，强调需要比较和结合基于数据和基于模型的方法。
...
### [187] [Online learning of smooth functions on $\mathbb{R}$](https://arxiv.org/abs/2604.03525)
*Jesse Geneson,Kuldeep Singh,Alexander Wang*

Main category: cs.LG

TL;DR: 该论文研究实值函数在实数域上的对抗性在线学习问题，发现标准模型在ℝ上不适用，因此提出了三种改进的学习场景来限制远离已观测输入的查询影响，并分析了不同场景下的性能保证。


<details>
  <summary>Details</summary>
Motivation: 研究实值函数在线学习的对抗性设置，发现对于绝对连续函数类𝒢_q，标准模型在ℝ上变得不适定（对手可以强制无限损失），因此需要设计改进的学习场景来克服这一障碍。

Method: 提出了三种改进的学习场景：场景1限制对手必须在过去查询的距离1内选择新查询；场景2对手可以任意查询，但学习者只对距离过去查询1内的回合受罚；场景3在回合t的损失乘以权重函数g(最小距离)。分析了这些场景在不同参数下的性能，并扩展到多维情况。

Result: 对于场景1-2，在多个参数范围内获得了尖锐的表征；对于场景3，发现了清晰的阈值现象：如果g衰减太慢，对手可以强制无限加权损失；对于快速衰减的权重（如g(z)=e^{-cz}），在二次情况p=q=2下获得了有限且尖锐的保证。对于多维扩展，发现了一维与多维的尖锐二分：一维情况在某些参数范围内允许有限最优值，但对于所有d≥2，切片类𝒢_{q,d}过于宽松，即使在场景1-3下对手也能强制无限损失。

Conclusion: 标准在线学习模型在ℝ上对于绝对连续函数类不适用，但通过限制查询影响的设计场景可以获得有限损失保证。场景3中的权重衰减速率是关键因素，而多维扩展显示了一维与高维之间的根本差异，高维情况下即使改进场景也无法避免无限损失。
...
### [188] [Choosing the Right Regularizer for Applied ML: Simulation Benchmarks of Popular Scikit-learn Regularization Frameworks](https://arxiv.org/abs/2604.03541)
*Benjamin S. Knight,Ahsaas Bajaj*

Main category: cs.LG

TL;DR: 该研究系统回顾了正则化技术的发展历程，并通过大规模模拟实验比较了Ridge、Lasso、ElasticNet和Post-Lasso OLS四种正则化方法在预测精度和特征选择方面的表现，特别关注了多重共线性和信噪比的影响。


<details>
  <summary>Details</summary>
Motivation: 正则化技术自1960年代逐步回归发展至今已有多种变体，但缺乏对不同方法在真实场景下性能的系统比较。研究旨在为机器学习工程师提供基于可观测特征空间属性的客观决策指南。

Method: 研究采用历史文献综述结合大规模模拟实验的方法。在134,400次模拟实验中，基于8个生产级机器学习模型构建7维参数空间，系统评估Ridge、Lasso、ElasticNet和Post-Lasso OLS四种正则化框架的性能。

Result: 当样本特征比足够大时（n/p >= 78），Ridge、Lasso和ElasticNet在预测精度上几乎可以互换。但Lasso的召回率在多重共线性下极其脆弱：在高条件数和低信噪比时，Lasso召回率降至0.18，而ElasticNet仍保持0.93。Post-Lasso OLS在高条件数和小样本量时表现同样不佳。

Conclusion: 研究建议实践者避免在高条件数和小样本量情况下使用Lasso或Post-Lasso OLS。研究最终提供了一个基于scikit-learn的客观决策指南，帮助机器学习工程师根据可观测特征空间属性选择最优正则化框架。
...
### [189] [BlazeFL: Fast and Deterministic Federated Learning Simulation](https://arxiv.org/abs/2604.03606)
*Kitsuya Azuma,Takayuki Nishio*

Main category: cs.LG

TL;DR: BlazeFL是一个轻量级联邦学习仿真框架，通过线程并行和确定性随机数管理，在保持高吞吐量的同时确保结果可复现。


<details>
  <summary>Details</summary>
Motivation: 联邦学习研究需要大量虚拟客户端仿真，但并行训练常引入随机性和调度不确定性，迫使研究者在效率和可复现性之间权衡，或需在复杂框架中实现自定义控制逻辑。

Method: BlazeFL采用线程并行和内存参数交换，避免序列化和进程间通信开销；为客户端分配隔离的随机数生成器流，确保在固定软硬件环境下获得比特级相同的确定性结果。

Result: 在CIFAR-10图像分类实验中，BlazeFL相比广泛使用的开源基准显著减少执行时间，在通信密集型工作负载上实现高达3.1倍加速，同时保持轻量级依赖。

Conclusion: BlazeFL通过自由线程共享内存执行和确定性随机数管理，缓解了联邦学习仿真中效率与可复现性的权衡问题，提供了高性能且可复现的轻量级解决方案。
...
### [190] [Algebraic Diversity: Group-Theoretic Spectral Estimation from Single Observations](https://arxiv.org/abs/2604.03634)
*Mitchell A. Thornton*

Main category: cs.LG

TL;DR: 论文证明：对于二阶统计估计，多观测的时间平均可以被单观测的代数群作用替代。通用替换定理确立了群平均估计器与多快照协方差估计具有等价子空间分解的条件，最优性定理证明对称群是普遍最优的（产生KL变换）。该框架将DFT、DCT和KLT统一为群匹配谱变换的特例，并通过闭式双交换子特征值问题实现多项式时间最优群选择。


<details>
  <summary>Details</summary>
Motivation: 传统二阶统计估计通常需要多个观测样本进行时间平均，这在实际应用中可能受到限制。论文旨在探索是否可以通过代数群作用在单个观测上实现等效的统计估计，从而减少对多观测的依赖并提高计算效率。

Method: 提出通用替换定理，建立群平均估计器与多快照协方差估计的等价条件；证明最优性定理，确定对称群为普遍最优选择；开发闭式双交换子特征值问题框架，实现多项式时间的最优群选择；将DFT、DCT、KLT统一为群匹配谱变换的特例。

Result: 1. 单快照MUSIC DOA估计；2. 大规模MIMO信道估计获得64%吞吐量增益；3. 单脉冲波形分类达到90%准确率；4. 非阿贝尔群图信号处理；5. 对transformer LLMs的代数分析发现RoPE在70-80%注意力头中使用错误的代数群，最优群是内容依赖的，基于谱集中的剪枝在13B规模上改善了困惑度。

Conclusion: 论文建立了用代数群作用替代时间平均的理论框架，证明了其在多个应用领域的有效性。特别重要的是对transformer LLMs的分析，揭示了RoPE机制中代数群选择的问题，并提出了改进方法。所有诊断仅需单次前向传播，无需梯度计算或训练。
...
### [191] [Delayed Homomorphic Reinforcement Learning for Environments with Delayed Feedback](https://arxiv.org/abs/2604.03641)
*Jongsoo Lee,Jangwon Kim,Soohee Han*

Main category: cs.LG

TL;DR: 提出延迟同态强化学习（DHRL）框架，通过MDP同态理论压缩信念等价增强状态，在抽象MDP上实现高效策略学习，解决延迟反馈导致的马尔可夫性破坏问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界强化学习常面临延迟反馈问题，这会破坏马尔可夫假设，阻碍学习和控制。传统的状态增强方法会导致状态空间爆炸，带来严重的样本复杂度负担。现有增强方法要么主要减轻critic负担，要么对actor和critic采用非统一处理，存在不足。

Method: 提出延迟同态强化学习（DHRL）框架，基于MDP同态理论，将信念等价的增强状态进行压缩，在得到的抽象MDP上进行高效策略学习，同时保持最优性。提供了状态空间压缩边界和样本复杂度的理论分析，并提出了实用算法。

Result: 在MuJoCo基准测试的连续控制任务中，该算法优于现有的增强基线方法，特别是在长延迟情况下表现更佳。

Conclusion: DHRL框架为延迟反馈问题提供了结构化和样本高效的解决方案，通过MDP同态理论有效压缩状态空间，在保持最优性的同时显著提升学习效率。
...
### [192] [Automated Attention Pattern Discovery at Scale in Large Language Models](https://arxiv.org/abs/2604.03764)
...
<details>
  <summary>Details</summary>
Motivation: 当前机制可解释性方法通常局限于特定行为的精确解释，难以泛化或扩展到大规模研究。需要一种可扩展的方法来分析大语言模型中的重复行为模式。

Method: 通过挖掘Java代码数据集中的完成场景，利用代码的结构化特性收集注意力模式。提出注意力模式-掩码自编码器(AP-MAE)，这是一种基于视觉Transformer的模型，能高效重建掩码的注意力模式。

Result: 在StarCoder2上的实验表明：AP-MAE能高精度重建注意力模式；在不同模型间泛化良好；揭示推理中的重复模式；无需真实标签即可预测生成正确性（准确率55%-70%）；选择性干预可将准确率提高13.6%，但过度干预会导致崩溃。

Conclusion: 注意力模式是用于可解释性的可扩展信号，AP-MAE为大规模语言模型的分析和干预提供了可迁移的基础框架，同时也可作为精细机制方法的筛选程序。
...
### [193] [CountsDiff: A Diffusion Model on the Natural Numbers for Generation and Imputation of Count-Based Data](https://arxiv.org/abs/2604.03779)
*Renzo G. Soatto,Anders Hoel,Greycen Ren,Shorna Alam,Stephen Bates,Nikolaos P. Daskalakis,Caroline Uhler,Maria Skoularidou*

Main category: cs.LG

TL;DR: CountsDiff是一个专门为自然数分布设计的扩散模型框架，通过简化Blackout扩散框架并引入现代扩散模型特性，在离散序数数据生成任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在连续和基于token的领域表现出色，但在离散序数数据（如计数数据）上的应用仍然不足。生物计数测定（如单细胞RNA-seq数据）等自然数分布领域缺乏专门的生成模型框架。

Method: CountsDiff扩展了Blackout扩散框架，通过生存概率调度和显式损失加权的直接参数化简化了公式。引入了连续时间训练、无分类器引导、以及允许非单调反向轨迹的搅动/重新掩码反向动力学等现代扩散模型特性。

Result: 在自然图像数据集（CIFAR-10, CelebA）上验证了框架有效性，探索了设计参数的影响。在单细胞RNA-seq插补任务中，即使简单的实例化也能匹配或超越最先进的离散生成模型和领先的RNA-seq插补方法。

Conclusion: CountsDiff为自然数分布提供了一个灵活有效的扩散建模框架，在生物计数数据等应用领域表现出强大潜力，为未来通过优化设计选择获得进一步性能提升留下了充足空间。
...
### [194] [k-Maximum Inner Product Attention for Graph Transformers and the Expressive Power of GraphGPS The Expressive Power of GraphGPS](https://arxiv.org/abs/2604.03815)
*Jonas De Schouwer,Haitz Sáez de Ocáriz Borde,Xiaowen Dong*

Main category: cs.LG

TL;DR: 本文提出k-MIP注意力机制用于图Transformer，通过top-k选择最相关节点实现线性复杂度，在保持表达能力的同时显著提升大规模图处理效率。


<details>
  <summary>Details</summary>
Motivation: 传统图Transformer的全对全注意力机制具有二次复杂度，难以应用于大规模图。现有线性化或受限注意力方法往往牺牲性能或表达能力，需要一种平衡效率与效果的新方法。

Method: 提出k-MIP注意力机制：1）通过top-k操作为每个查询选择最相关的关键节点，形成稀疏但灵活的注意力模式；2）结合基于符号矩阵的注意力分数计算，实现线性内存复杂度和实际加速。

Result: 1）内存复杂度线性化，相比全对全注意力实现高达一个数量级的加速；2）单张A100 GPU可处理超过50万节点的图；3）理论证明k-MIP Transformer可任意精度逼近全注意力Transformer；4）在多个基准测试中表现优异。

Conclusion: k-MIP注意力机制成功平衡了图Transformer的效率与表达能力，为大规模图处理提供了实用解决方案，在保持理论表达能力的同时显著提升计算效率。
...
### [195] [Collapse-Free Prototype Readout Layer for Transformer Encoders](https://arxiv.org/abs/2604.03850)
*Giansalvo Cirrincione,Rahul Ranjeev Kumar*

Main category: cs.LG

TL;DR: DDCL-Attention是一种基于原型的Transformer编码器读出层，通过学习压缩机制替代简单池化方法，使用少量全局原型向量通过软概率匹配分配token，以线性复杂度生成紧凑的token摘要。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer编码器使用简单池化方法（如平均池化或类别token）进行token汇总，这些方法缺乏学习能力且可能丢失重要信息。需要一种能够学习压缩token表示、保持原型多样性、训练稳定且适用于多种应用的读出机制。

Method: 使用少量全局原型向量，通过软概率匹配将token分配给原型，生成紧凑的token摘要。通过精确分解训练损失为重构项和多样性项来避免原型坍塌。基于Tikhonov奇异摄动理论和显式学习率约束确保与编码器的联合训练稳定性。

Result: 在四个数据集上的实验验证了理论预测：损失分解精确成立，满足稳定性条件时原型分离度按预期增长，码本达到完全利用率且优于标准硬向量量化。在轨道碎片分类任务中展示了该方法在NLP和视觉任务之外的科学表格数据上的适用性。

Conclusion: DDCL-Attention提供了一种有效的原型基于读出层，能够稳定训练、避免原型坍塌，并支持三种应用：最终读出层、可微分码本扩展VQ-VAE和分层文档压缩器，在多种任务中表现出色。
...
### [196] [Where to Steer: Input-Dependent Layer Selection for Steering Improves LLM Alignment](https://arxiv.org/abs/2604.03867)
*Soham Gadgil,Chris Lin,Su-In Lee*

Main category: cs.LG

TL;DR: W2S框架通过根据输入自适应选择干预层来优化LLM的导向向量对齐方法，相比固定层方法有显著提升


<details>
  <summary>Details</summary>
Motivation: 现有导向向量方法通常在全局固定层应用干预，假设最优干预层对所有输入都是不变的。作者认为这一假设存在根本性限制，因为与目标行为相关的表示可能在不同输入下编码在不同层中。

Method: 提出Where to Steer (W2S)框架，通过学习从输入嵌入到最优导向层的映射，根据输入自适应选择干预层。该方法能够根据具体输入条件确定最合适的干预位置。

Result: 在多个LLM和对齐行为上，W2S始终优于固定层基线方法，在分布内和分布外设置中都有改进。实证证据表明最优导向层在不同输入间变化很大。

Conclusion: 研究强调了输入依赖性控制在LLM对齐中的重要性，并证明自适应层选择是当前导向向量方法中缺失的关键设计维度。W2S为更精细的模型行为控制提供了有效框架。
...
### [197] [SODA: Semi On-Policy Black-Box Distillation for Large Language Models](https://arxiv.org/abs/2604.03873)
*Xiwen Chen,Jingjing Wang,Wenhui Zhu,Peijie Qiu,Xuanzhao Dong,Hejian Sang,Zhipeng Wang,Alborz Geramifard,Feng Luo*

Main category: cs.LG

TL;DR: SODA提出了一种高效的黑盒知识蒸馏方法，通过静态对比学生模型的次优输出与教师模型的优质响应，避免了传统对抗训练的复杂性和不稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒知识蒸馏方法面临严格权衡：简单的离策略方法无法纠正学生模型的固有错误，而完全在策略的对抗训练方法虽然有效但存在训练不稳定和计算开销巨大的问题。

Method: SODA采用半在策略蒸馏对齐方法，利用前沿教师模型与较小基础模型之间的能力差距，将教师的最优响应与学生模型一次性静态输出的次优行为进行对比，构建有效的对比信号。

Result: 在四个紧凑型Qwen2.5和Llama-3模型上的广泛评估显示，SODA在16个基准测试中的15个上匹配或优于最先进方法，同时训练速度快10倍，GPU峰值内存减少27%，完全消除了对抗不稳定性。

Conclusion: SODA证明了向小型学生模型展示其自身的静态次优行为足以实现高质量分布对齐，无需昂贵的动态展开和脆弱的对抗平衡，为高效知识蒸馏提供了新范式。
...
### [198] [Spatiotemporal Interpolation of GEDI Biomass with Calibrated Uncertainty](https://arxiv.org/abs/2604.03874)
*Robin Young,Srinivasan Keshav*

Main category: cs.LG

TL;DR: 该研究扩展了注意力神经过程框架，利用地理空间基础模型嵌入，解决GEDI激光雷达数据在时空上的不连续覆盖问题，特别是针对森林扰动期间的时间插值，为森林碳核算提供可靠的不确定性量化。


<details>
  <summary>Details</summary>
Motivation: NASA的GEDI激光雷达数据虽然能可靠估算地上生物量密度，但由于轨道采样导致时空覆盖不连续，特别是2023年3月至2024年4月的休眠期造成观测空白。现有机器学习方法主要填补空间空白，但对扰动事件期间的时间插值问题尚未解决，且标准集成方法产生系统性校准错误的预测区间。

Method: 扩展注意力神经过程框架，将空间和时间对称处理，利用地理空间基础模型嵌入，在稀疏时空设置下进行联合建模。通过空间换时间替代策略，利用其他时间邻近位置的观测来预测缺失时期的生物量。

Result: ANP方法在不同扰动机制下都能产生良好校准的不确定性估计，支持其在需要可靠不确定性量化的测量、报告和验证应用中用于森林碳核算。

Conclusion: 扩展的注意力神经过程框架能有效解决GEDI数据时空不连续问题，特别是在森林扰动期间的时间插值，为森林碳监测提供可靠的不确定性量化工具。
...
### [199] [Regime-Calibrated Demand Priors for Ride-Hailing Fleet Dispatch and Repositioning](https://arxiv.org/abs/2604.03883)
*Indar Kumar,Akanksha Tiwari*

Main category: cs.LG

TL;DR: 提出一种基于需求模式分区的网约车调度方法，通过历史数据分区、相似性匹配和校准需求先验，结合线性规划重定位和匈牙利匹配批量调度，显著减少乘客等待时间。


<details>
  <summary>Details</summary>
Motivation: 网约车调度需要准确预测随时间、星期、季节和特殊事件变化的需求模式，传统方法难以处理这种高度变化的需求模式。

Method: 1) 将历史行程数据分割为需求模式分区；2) 通过包含Kolmogorov-Smirnov距离、Wasserstein-1距离、特征距离、方差比、事件模式相似性和时间邻近度的相似性集成，将当前运营时段与最相似的历史类比匹配；3) 使用得到的校准需求先验驱动基于线性规划的车辆重定位策略和匈牙利匹配的批量调度。

Result: 在520万次纽约TLC行程的8种不同场景评估中，平均等待时间减少31.1%，P95等待时间减少37.6%，等待时间基尼系数从0.441改善到0.409。该方法无需训练、具有确定性且可解释，在芝加哥验证时也获得23.3%的等待时间减少。

Conclusion: 提出的分区校准方法通过历史需求模式匹配和优化调度策略，显著提高了网约车调度效率，具有可解释性、泛化能力和鲁棒性，为实际部署提供了有效解决方案。
...
### [200] [Provable Multi-Task Reinforcement Learning: A Representation Learning Framework with Low Rank Rewards](https://arxiv.org/abs/2604.03891)
*Yaoze Guo,Shana Moothedath*

Main category: cs.LG

TL;DR: 本文研究多任务强化学习中的表示学习方法，针对具有相同状态-动作空间和转移概率但奖励不同的多个线性MDP任务，通过低秩矩阵估计学习共享表示，在更一般的特征分布假设下实现准确的低秩矩阵恢复和近优策略构建。


<details>
  <summary>Details</summary>
Motivation: 多任务强化学习中学习跨任务的共享表示具有挑战性，因为数据具有复杂性和策略依赖性，导致误差的时间累积。现有方法依赖高斯特征、不相干条件或最优解访问等限制性假设，需要更通用的方法来处理RL环境中常见的特征分布。

Method: 采用免奖励强化学习框架，首先学习数据收集策略，然后基于该策略设计探索策略来估计未知奖励矩阵。提出低秩矩阵估计方法，在更一般的特征分布假设下操作，利用收集的数据进行准确估计，最终学习近优策略。

Result: 理论分析表明在放松的假设下可以实现准确的低秩矩阵恢复，并刻画了表示误差与样本复杂度之间的关系。利用学习到的表示构建近优策略并证明了遗憾界。实验结果表明该方法能有效从有限数据中学习鲁棒的共享表示和任务动态。

Conclusion: 本文提出的方法在多任务强化学习中成功学习了共享表示，克服了现有方法的限制性假设，在更一般的特征分布条件下实现了准确的低秩矩阵估计和策略优化，为多任务表示学习提供了理论保证和实用框架。
...
### [201] [Improving Model Performance by Adapting the KGE Metric to Account for System Non-Stationarity](https://arxiv.org/abs/2604.03906)
...
<details>
  <summary>Details</summary>
Motivation: 地球科学系统通常具有显著的时间非平稳性，源于水文气象驱动因素和土地利用变化的季节性和气候变化。传统模型评估指标通常隐含地假设数据生成过程是时间平稳的，这在现实中是不合理的，需要能够考虑非平稳趋势的评估方法。

Method: 开发了JKGE_ss指标（基于KGE_ss改进），该指标能够检测和考虑数据统计特性中的动态非平稳性。与使用长期均值作为基准的NSE和KGE_ss不同，JKGE_ss强调对系统存储时间变化的再现能力。

Result: 通过在不同水文气候条件下（从近期降水主导到积雪主导再到强干旱）训练不同复杂度的物理概念和数据驱动模型，测试了新指标的鲁棒性。在所有情况下，新指标都能改善系统时间动态在所有时间尺度上的再现，从湿润年到干旱年，以及整个流量范围（特别是退水期）。
...
### [202] [Align Your Structures: Generating Trajectories with Structure Pretraining for Molecular Dynamics](https://arxiv.org/abs/2604.03911)
...
Method: 提出两阶段框架：1）在大规模构象数据集上训练基于扩散的结构生成模型；2）在MD轨迹数据上训练插值器模块，确保生成结构的时间一致性。该方法将复杂的MD建模分解为结构生成和时间对齐两个可管理的子问题。

Result: 在QM9和DRUGS小分子数据集上进行了无条件生成、正向模拟和插值任务的全面评估，并扩展到四肽和蛋白质单体系统。实验结果表明，该方法在生成化学真实的MD轨迹方面表现出色，在几何、动力学和能量测量方面取得了显著改进。

Conclusion: 该框架通过利用丰富的结构数据缓解MD轨迹数据稀缺问题，并将复杂的MD建模任务分解为两个可管理的子问题，成功生成了化学真实的分子动力学轨迹，为MD轨迹生成提供了有效解决方案。
...
### [203] [ACES: Who Tests the Tests? Leave-One-Out AUC Consistency for Code Generation](https://arxiv.org/abs/2604.03922)
*Hui Sun,Yun-Ji Zhang,Zheng Xie,Ren-Biao Liu,Yali Du,Xin-Ye Li,Ming Li*

Main category: cs.LG

TL;DR: 论文提出ACES方法，通过留一法评估测试用例的区分能力来筛选LLM生成的代码，解决了测试用例本身可能不正确的循环依赖问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在筛选LLM生成的代码时面临挑战：测试用例本身可能不正确，而确定测试用例的正确性又需要知道哪些代码是正确的，形成了循环依赖。传统方法要么平等对待所有测试，要么依赖临时启发式方法过滤不可靠测试。

Method: 提出ACES方法，核心思想是测试投票应该排名而非简单计数。通过留一法评估：保留一个测试，用所有其他测试的聚合分数对代码排名，然后评估被保留测试的通过/失败模式是否与这个排名一致。将这种一致性形式化为LOO-AUC，并证明其期望值与每个测试区分正确代码和错误代码的能力成正比。提出两个变体：ACES-C提供闭式权重，在平均测试质量的温和假设下可证明近似最优；ACES-O放弃该假设，通过迭代优化可微分的LOO-AUC目标。

Result: ACES方法仅使用二进制通过矩阵，计算开销可忽略，在多个代码生成基准测试中实现了最先进的Pass@k性能。

Conclusion: ACES方法通过留一法评估测试用例的区分能力，有效解决了测试用例可能不正确的循环依赖问题，为LLM生成的代码选择提供了理论保证和实用解决方案。
...
### [204] [Supervised Dimensionality Reduction Revisited: Why LDA on Frozen CNN Features Deserves a Second Look](https://arxiv.org/abs/2604.03928)
*Indar Kumar,Girish Karhana,Sai Krishna Jasti,Ankit Hemant Lade*

Main category: cs.LG

TL;DR: 提出一种基于需求模式分段的网约车调度方法，通过历史数据校准和优化算法，显著降低乘客等待时间
...
Method: 采用制度校准方法：1) 将历史行程数据分段为需求制度；2) 通过六指标相似性集合匹配当前运营时段与历史类似时段；3) 使用校准后的需求先验驱动基于线性规划的车辆重新定位策略和匈牙利匹配的批量调度

Result: 在520万纽约TLC行程数据上测试，平均等待时间降低31.1%，P95等待时间降低37.6%，基尼系数从0.441改善到0.409；方法无需训练、可解释，在芝加哥验证有效，对车队规模变化鲁棒

Conclusion: 提出的制度校准方法能有效改善网约车调度性能，通过历史需求模式匹配和优化算法组合，显著降低乘客等待时间并提高公平性
...
### [205] [Diagonal-Tiled Mixed-Precision Attention for Efficient Low-Bit MXFP Inference](https://arxiv.org/abs/2604.03950)
*Yifu Ding,Xinhao Zhang,Jinyang Guo*

Main category: cs.LG

TL;DR: 提出了一种基于MXFP数据格式的低比特混合精度注意力核DMA，利用下一代GPU架构的计算能力，通过两种低比特计算和Triton实现的融合核，在保持模型性能的同时显著提升推理速度。


<details>
  <summary>Details</summary>
Motivation: Transformer大语言模型虽然性能优异，但由于注意力机制的二次复杂度和高精度操作的内存带宽限制，推理成本仍然过高，需要更高效的推理方案。

Method: 使用微缩放浮点(MXFP)数据格式，提出对角线分块混合精度注意力(DMA)核，在分块级别结合两种低比特计算，通过Triton实现硬件级并行和内存高效的融合核。

Result: 在NVIDIA B200 GPU上的广泛实验表明，该核在保持生成质量（性能下降可忽略）的同时，通过核融合实现了显著的加速。

Conclusion: DMA注意力核为LLM推理提供了一种高效解决方案，在保持模型性能的同时显著降低了计算成本，代码已开源。
...
### [206] [BWTA: Accurate and Efficient Binarized Transformer by Algorithm-Hardware Co-design](https://arxiv.org/abs/2604.03957)
*Yifu Ding,Xianglong Liu,Shenghao Jin,Jinyang Guo,Jiwen Lu*

Main category: cs.LG

TL;DR: 提出BWTA量化方案，结合二进制权重和三值激活，通过平滑多阶段量化训练和专用CUDA内核，实现超低位量化Transformer模型的高效推理，性能接近全精度模型。


<details>
  <summary>Details</summary>
Motivation: 超低位量化虽然能显著提升Transformer模型的效率，但存在精度下降和GPU支持有限的问题，阻碍了其广泛应用。

Method: 提出BWTA量化方案：1) 分析二值化中的零点失真问题，将微小值投影为零；2) 提出平滑多阶段量化训练方法，结合层级退化策略和幅度对齐投影因子；3) 开发BWTA矩阵乘CUDA内核，支持指令级并行位打包和全面的二进制/三值矩阵乘实现。

Result: BERT模型在GLUE基准上平均精度下降仅3.5%，五个任务下降小于2%；LLMs获得可比的困惑度和准确率；在NVIDIA GPU上实现16-24倍内核级加速，LLMs端到端预填充速度达216-330 tokens/s，内存占用更低。
...
### [207] [Autoencoder-Based Parameter Estimation for Superposed Multi-Component Damped Sinusoidal Signals](https://arxiv.org/abs/2604.03985)
*Momoka Iida,Hayato Motohashi,Hirotaka Takahashi*

Main category: cs.LG

TL;DR: 提出基于自编码器的多分量阻尼正弦信号参数估计方法，在噪声环境下能准确估计频率、相位、衰减时间和振幅等参数，对训练数据分布具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 阻尼正弦振荡在物理系统中广泛存在，但参数估计在信号快速衰减、多分量叠加和存在观测噪声时变得困难，需要开发更有效的分析方法。

Method: 开发基于自编码器的方法，利用潜在空间估计噪声多分量阻尼正弦信号的频率、相位、衰减时间和振幅等参数，研究高斯分布和均匀分布训练数据的影响。

Result: 该方法即使在具有挑战性的设置下（如包含次要分量或接近反相的分量）也能高精度估计参数，当训练分布信息较少时仍保持合理的鲁棒性。

Conclusion: 该方法展示了作为分析短时噪声信号工具的潜力，为物理系统参数估计提供了有效解决方案。
...
### [208] [Can LLMs Learn to Reason Robustly under Noisy Supervision?](https://arxiv.org/abs/2604.03993)
*Shenzhi Yang,Guangcheng Zhu,Bowen Song,Sharon Li,Haobo Wang,Xing Zheng,Yingfan Ma,Zhongqi Chen,Weiqiang Wang,Gang Chen*

Main category: cs.LG

TL;DR: 本文首次系统分析了RLVR中的噪声标签机制，提出在线标签细化方法OLR，在多种噪声比例下显著提升了模型在数学推理任务上的鲁棒性。
...
Method: 基于RLVR中标签影响训练的条件性（需要当前策略能生成实现该标签的rollout），区分了两种噪声类型：非活跃噪声标签（降低数据效率）和活跃噪声标签（被强化并扭曲模型分布）。提出在线标签细化方法OLR，当满足两个条件时（多数答案的rollout通过率呈正斜率，历史一致性稳定），逐步用多数投票答案修正潜在噪声标签。

Result: 在6个分布内数学推理基准（AIME24/25、AMC、MATH-500、Minerva、Olympiad）和3个分布外任务（ARC-c、GPQA-diamond、MMLU-pro）上评估。在0.1-0.9的噪声比例范围内，OLR在非活跃和活跃噪声标签设置下均显著提升鲁棒性，分布内基准平均提升3.6%-3.9%，分布外评估提升3.3%-4.6%。

Conclusion: RLVR中的噪声标签机制具有独特性，需要专门处理。提出的OLR方法能有效利用早期正确一致性现象，通过渐进式自我修正提升模型在噪声环境下的鲁棒性，为RLVR在现实噪声标签场景中的应用提供了实用解决方案。
...
### [209] [FlashSAC: Fast and Stable Off-Policy Reinforcement Learning for High-Dimensional Robot Control](https://arxiv.org/abs/2604.04539)
...
TL;DR: FlashSAC是一种基于Soft Actor-Critic的快速稳定离线策略强化学习算法，通过减少梯度更新、使用更大模型和更高数据吞吐量，在60多个任务中超越PPO和现有离线方法。


<details>
  <summary>Details</summary>
Motivation: 在线策略方法（如PPO）依赖狭窄分布的数据，在高维状态动作空间中难以准确评估策略；离线策略方法虽然能利用更广泛的数据分布，但收敛慢且不稳定，因为需要在多样数据上拟合价值函数，导致批评器误差通过自举累积。

Method: 基于监督学习中观察到的缩放定律，FlashSAC显著减少梯度更新次数，同时通过更大模型和更高数据吞吐量补偿。为保持大规模训练的稳定性，算法明确限制权重、特征和梯度范数，控制批评器误差累积。

Result: 在10个模拟器的60多个任务中，FlashSAC在最终性能和训练效率上一致优于PPO和强离线策略基线，在高维任务（如灵巧操作）上提升最大。在仿真到真实的人形机器人运动任务中，将训练时间从数小时减少到数分钟。

Conclusion: FlashSAC展示了离线策略强化学习在仿真到真实迁移中的潜力，通过减少梯度更新、扩大模型规模和提高数据吞吐量，实现了快速稳定的训练性能。
...
### [210] [ArrowFlow: Hierarchical Machine Learning in the Space of Permutations](https://arxiv.org/abs/2604.04087)
*Ozgur Yilmaz*
...
<details>
  <summary>Details</summary>
Motivation: 探索一种与基于梯度的传统方法完全不同的计算范式，证明竞争性分类可以在纯粹序数结构的空间中实现，这种架构天然适合整数运算和神经形态硬件。

Method: 使用排序滤波器作为计算单元，通过Spearman's footrule距离比较输入，采用基于置换矩阵累积的非梯度更新规则。层次结构组合：每层的输出排序成为下一层的输入，实现深度序数表示学习。

Result: 在Iris数据集上表现最佳（2.7% vs 3.3%），在大多数UCI数据集上具有竞争力。单个多项式度参数作为主开关：度1提供噪声鲁棒性、隐私保护和缺失特征弹性；更高度数以牺牲这些特性换取更好的清洁准确率。

Conclusion: ArrowFlow并非旨在超越基于梯度的方法，而是证明竞争性分类可以在完全不同的计算范式中实现，将序数结构提升为一等公民，天然适合整数运算和神经形态硬件。
...
### [211] [Fine-grained Analysis of Stability and Generalization for Stochastic Bilevel Optimization](https://arxiv.org/abs/2604.04090)
*Xuelin Zhang,Hong Chen,Bin Gu,Tieliang Gong,Feng Zheng*

Main category: cs.LG

TL;DR: 该论文对随机双层优化方法进行了系统性的泛化分析，建立了平均参数稳定性与泛化差距之间的定量关系，并推导了单时间尺度和双时间尺度SGD的稳定性上界。


<details>
  <summary>Details</summary>
Motivation: 随机双层优化在机器学习中应用广泛，但现有研究主要关注计算行为，从统计学习理论角度对其泛化保证的理解还很有限。本文旨在填补这一空白，为基于梯度的双层优化方法提供系统的泛化分析。

Method: 首先建立平均参数稳定性与泛化差距之间的定量联系，然后分别推导单时间尺度SGD和双时间尺度SGD在三种设置（非凸-非凸、凸-凸、强凸-强凸）下的平均参数稳定性上界。实验验证了理论发现。

Result: 相比之前的算法稳定性分析，本文结果不需要在每次迭代时重新初始化内层参数，适用于更一般的目标函数。实验分析验证了理论发现的有效性。

Conclusion: 本文为随机双层优化方法提供了系统的泛化分析框架，建立了稳定性与泛化性能的理论联系，扩展了现有分析方法的适用范围，为双层优化在实际应用中的泛化性能提供了理论保证。
...
### [212] [Spectral Path Regression: Directional Chebyshev Harmonics for Interpretable Tabular Learning](https://arxiv.org/abs/2604.04091)
*Milo Coombs*

Main category: cs.LG

TL;DR: 提出一种新的多元逼近方法，用方向谐波模式替代传统的张量积结构，通过选择少量结构化频率向量控制复杂度，实现高效、可解释的回归模型。


<details>
  <summary>Details</summary>
Motivation: 传统Chebyshev多项式等经典逼近基虽然原理清晰、可解释性强，但其多元张量积结构存在维度灾难问题（随维度指数增长），并且强加的轴对齐结构与实际表格数据不匹配。

Method: 采用方向谐波模式 $\cos(\mathbf{m}^{\top}\arccos(\mathbf{x}))$ 替代张量化振荡，在角度空间中按方向而非坐标索引组织多元结构。通过选择少量结构化频率向量（谱路径）控制复杂度，训练简化为单次闭式岭回归求解，无需迭代优化。

Result: 在标准连续特征表格回归基准测试中，该方法实现了与强非线性基线相竞争的准确性，同时保持模型紧凑、计算高效，并通过学习特征交互的解析表达式保持明确的可解释性。

Conclusion: 提出的方向谐波模式方法成功解决了传统多元逼近方法的维度灾难和结构不匹配问题，提供了一种高效、可解释的回归建模框架，在准确性和可解释性之间取得了良好平衡。
...
### [213] [The Geometric Alignment Tax: Tokenization vs. Continuous Geometry in Scientific Foundation Models](https://arxiv.org/abs/2604.04155)
*Prashant C. Raju*

Main category: cs.LG

TL;DR: 研究发现生物和物理基础模型在优化预测准确性时，其内部表示会系统性地破坏所建模系统的连续几何结构，根源在于"几何对齐税"——将连续流形强制通过离散分类瓶颈的内在代价。


<details>
  <summary>Details</summary>
Motivation: 当前生物和物理基础模型虽然优化了预测准确性，但其内部表示无法保持所建模系统的连续几何结构，这限制了模型对底层物理和生物过程的理解能力。

Method: 1. 在合成动力系统上进行受控消融实验，比较交叉熵损失与连续头在相同编码器上的效果；2. 使用率失真理论和MINE评估14个生物基础模型；3. 通过受控实验验证Evo 2模型在真实DNA上的反向互补鲁棒性。

Result: 1. 用连续头替换交叉熵可将几何失真减少8.5倍；2. 学习到的码本表现出非单调双重约束：更精细的量化反而恶化几何结构；3. 在连续目标下，三种架构差异为1.3倍，在离散标记化下差异达3000倍；4. 识别出三种失效机制：局部-全局解耦、表示压缩和几何空虚；5. 没有模型能同时实现低失真、高互信息和全局一致性。

Conclusion: 基础模型中的离散分类瓶颈导致了"几何对齐税"，严重破坏了连续系统的几何结构。当前方法无法同时保持几何保真度、信息内容和全局一致性，需要新的架构设计来克服这一根本限制。
...
### [214] [Uncertainty-Aware Foundation Models for Clinical Data](https://arxiv.org/abs/2604.04175)
*Qian Zhou,Yuanyun Zhang,Shi Li*

Main category: cs.LG

TL;DR: 提出了一种不确定性感知的医疗基础模型框架，将患者表示为潜在状态分布而非点嵌入，通过集合值表示和跨视图一致性来捕获可推断信息并编码认知不确定性。


<details>
  <summary>Details</summary>
Motivation: 医疗观察数据本质不完整，具有稀疏、不规则和模态依赖的特点，现有医疗基础模型主要借鉴NLP和CV的范式，强调大规模预训练和确定性表示，未能充分处理临床数据的不确定性。

Method: 提出不确定性感知基础模型框架，将患者表示为潜在状态分布而非点嵌入；学习集合值表示，通过跨患者部分视图的一致性来捕获可推断信息；集成多模态编码器和可扩展自监督目标，结合重建、对比对齐和分布正则化。
...
### [215] [Stable and Privacy-Preserving Synthetic Educational Data with Empirical Marginals: A Copula-Based Approach](https://arxiv.org/abs/2604.04195)
*Gabriel Diaz Ramos,Lorenzo Luzi,Debshila Basu Mallick,Richard Baraniuk*

Main category: cs.LG

TL;DR: NPGC是一种非参数高斯copula合成数据生成方法，用于教育数据挖掘中的隐私保护，通过经验统计锚定替代深度学习，保持边际分布稳定，支持差分隐私和异构数据类型。


<details>
  <summary>Details</summary>
Motivation: 在严格隐私保护法规框架下推进教育数据挖掘，需要开发既能进行数据驱动分析又能保护敏感学生信息的方法。现有深度学习和参数化生成器存在边际分布扭曲、迭代再生时分布漂移等问题，影响可靠性。

Method: 提出非参数高斯copula（NPGC）方法，用经验统计锚定替代深度学习和参数优化，通过copula框架建模依赖关系，在边际和相关层面集成差分隐私，支持异构变量类型，并将缺失数据作为显式状态处理。

Result: 在五个基准数据集上评估显示，NPGC在多次再生循环中保持稳定，实现具有竞争力的下游性能，且计算成本显著降低。在实际在线学习平台部署验证了其实用性。
...
### [216] [Which Leakage Types Matter?](https://arxiv.org/abs/2604.04199)
*Simon Roth*

Main category: cs.LG

TL;DR: 研究通过大量实验测量了机器学习中四类数据泄漏的严重程度，发现教科书强调的归一化泄漏影响最小，而实际数据集规模下的选择泄漏影响最大。


<details>
  <summary>Details</summary>
Motivation: 量化评估机器学习中不同类型数据泄漏的实际影响，挑战传统教科书中对数据泄漏重要性的排序，为实际应用提供实证依据。

Method: 在2,047个表格数据集上进行28个受试者内反事实实验，在129个时间序列数据集上进行边界实验，测量四类数据泄漏的严重程度。

Result: I类泄漏（估计-在全数据上拟合缩放器）影响可忽略；II类泄漏（选择-窥视、种子挑选）影响显著；III类泄漏（记忆）随模型容量增加；IV类泄漏在随机交叉验证下不可见。

Conclusion: 教科书对数据泄漏重要性的排序需要反转：归一化泄漏影响最小，实际数据集规模下的选择泄漏影响最大，这对机器学习实践有重要指导意义。
...
### [217] [Three Phases of Expert Routing: How Load Balance Evolves During Mixture-of-Experts Training](https://arxiv.org/abs/2604.04230)
*Charafeddine Mouzouni*

Main category: cs.LG

TL;DR: 该研究将MoE令牌路由建模为拥塞博弈，通过有效拥塞系数γ_eff量化平衡-质量权衡，揭示了MoE训练的三阶段轨迹：激增阶段（学习负载平衡）、稳定阶段（专家专业化）和松弛阶段（质量优先）。


<details>
  <summary>Details</summary>
Motivation: 研究MoE模型中令牌路由的动态演化过程，揭示传统后验分析无法观察到的训练轨迹，理解路由器在训练过程中如何权衡负载平衡与专家专业化质量。

Method: 将MoE令牌路由建模为拥塞博弈，引入有效拥塞系数γ_eff作为单一参数量化平衡-质量权衡。分析两个开源MoE模型（OLMoE-1B-7B和OpenMoE-8B）的训练检查点，使用有效拥塞分解、多类型扩展、令牌聚类等技术，并通过自举重采样计算置信区间。

Result: 发现MoE训练呈现三阶段非单调轨迹：1)激增阶段（γ_eff:14→36-39，步数30K-40K），路由器学习负载平衡；2)稳定阶段（B_0:2.4→2.3，步数100K-400K），专家在稳定平衡下专业化；3)松弛阶段（γ_eff:27→9，步数400K-1.2M），路由器为质量牺牲平衡。多类型扩展通过令牌聚类将负载预测提升30%。

Conclusion: MoE训练早期优先考虑负载平衡，后期优先考虑质量，这一动态演化过程无法通过收敛模型的后验分析观察到。拥塞博弈框架揭示了温度缩放softmax中温度参数的实际含义及其演化过程，为理解MoE路由机制提供了新视角。
...
### [218] [Subspace Control: Turning Constrained Model Steering into Controllable Spectral Optimization](https://arxiv.org/abs/2604.04231)
...
TL;DR: 提出SIFT框架解决基础模型定制化中的约束优化问题，通过子空间控制方法缓解目标与约束间的干扰，在四个应用场景中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 基础模型（如大语言模型）在实际部署前需要定制化以满足安全、隐私等约束条件，但现有方法在解决这类约束优化问题时面临目标与约束间干扰的挑战。

Method: 提出SIFT（频谱无干扰训练）框架：1）从模型合并角度分析频谱跨任务干扰；2）建立与Muon优化器中梯度正交化的联系；3）引入定位方案选择性干预优化过程，实现可控更新以缓解目标-约束冲突。

Result: 在四个代表性应用（机器遗忘、安全对齐、文本转语音适配、幻觉缓解）中评估SIFT，相比基于控制和无控制的基线方法，在所有任务上都取得了显著且稳健的性能提升。

Conclusion: SIFT框架通过子空间控制有效解决了基础模型定制化中的约束优化问题，为缓解目标与约束间的干扰提供了有效的解决方案。
...
### [219] [Good Rankings, Wrong Probabilities: A Calibration Audit of Multimodal Cancer Survival Models](https://arxiv.org/abs/2604.04239)
*Sajad Ghawami*

Main category: cs.LG

TL;DR: 该研究首次系统评估了多模态WSI-基因组学生存预测模型的校准性能，发现尽管这些模型在区分性能（C-index）上表现良好，但多数存在校准失败问题，表明仅靠C-index不足以评估临床可用的生存模型。


<details>
  <summary>Details</summary>
Motivation: 目前多模态深度学习模型融合全切片组织病理图像和基因组数据进行癌症生存预测，在区分性能（C-index）上表现优异，但这些模型输出的生存概率是否经过校准（即预测概率与实际生存率是否一致）尚未得到系统研究。这对于临床应用的可靠性至关重要。

Method: 研究进行了两个实验：实验A评估3个模型在TCGA-BRCA数据集上的原生离散时间生存输出；实验B评估11种架构在5个TCGA癌症类型中通过Breslow重构的生存曲线。采用fold-level 1-calibration审计，使用Benjamini-Hochberg校正进行多重检验，并测试了后处理Platt缩放对校准的改善效果。

Result: 实验A中所有三个模型在多数fold上校准失败（15个fold-level测试中12个拒绝校准假设）；总共290个fold-level测试中，166个在中位事件时间后拒绝正确校准假设。MCAT在GBMLGG上C-index达到0.817，但在所有5个fold上校准失败。门控融合与更好的校准相关，而双线性和拼接融合则不然。后处理Platt缩放能减少校准失败（如MCAT：从5/5失败减少到2/5失败）且不影响区分性能。

Conclusion: 多模态生存预测模型普遍存在校准问题，仅凭C-index评估模型不足以确保临床可用性。门控融合架构和后处理校准技术（如Platt缩放）能改善校准性能。未来生存预测模型评估应同时考虑区分性能和校准性能。
...
### [220] [Peoples Water Data: Enabling Reliable Field Data Generation and Microbial Contamination Screening in Household Drinking Water](https://arxiv.org/abs/2604.04240)
*Suzan Kagan,Shira Spigelman,Sankar Sudhir,Thalappil Pradeep,Hadas Mamane*

Main category: cs.LG

TL;DR: 开发了一个两阶段机器学习框架，利用低成本物理化学和情境指标预测印度钦奈家庭饮用水中的大肠杆菌污染，为资源有限地区提供可扩展的微生物检测优先级决策支持工具。
...
Method: 开发了两阶段机器学习框架，使用低成本物理化学和情境指标预测大肠杆菌存在。数据集来自"人民水数据"倡议的3,023个样本，经过协调、技术清洗和异常值筛选后保留2,207个有效样本。研究在AI支持的现场实施框架内进行，结合学生指导和实时质量控制以提高协议遵守、可追溯性和数据可靠性。

Result: 该框架为资源受限环境中的微生物检测优先级提供了可扩展的决策支持工具，解决了使用点污染风险评估的重要缺口。

Conclusion: 该研究开发的机器学习框架能够有效预测家庭饮用水中的大肠杆菌污染，为资源有限地区提供了一种实用、可扩展的监测解决方案，同时通过AI支持的现场实施框架提高了数据收集的质量和可靠性。
...
### [221] [Learning An Interpretable Risk Scoring System for Maximizing Decision Net Benefit](https://arxiv.org/abs/2604.04241)
...
<details>
  <summary>Details</summary>
Motivation: 现有风险评分系统通常专注于优化预测准确性或基于似然的标准，但这些方法可能与最大化效用的主要目标不一致。需要一种能直接优化净效益的方法。

Method: 将风险评分系统建模为稀疏整数线性规划问题，构建具有整数系数的透明评分系统，便于解释和实际应用。建立了净效益、区分度和校准之间的基本关系。

Result: 在多个公共数据集和真实临床数据集上的评估表明，该方法能有效实现高净效益，同时保持竞争性的区分度和校准性能。分析证明优化净效益也能保证传统性能指标。

Conclusion: 提出的方法通过直接优化净效益，构建了可解释的风险评分系统，在保持传统性能指标的同时实现了效用最大化，具有重要的实际应用价值。
...
### [222] [Towards Unveiling Vulnerabilities of Large Reasoning Models in Machine Unlearning](https://arxiv.org/abs/2604.04255)
*Aobo Chen,Chenxu Zhao,Chenglin Miao,Mengdi Huai*

Main category: cs.LG

TL;DR: 本文首次提出针对大型推理模型（LRM）的遗忘攻击，通过误导性推理轨迹强制产生错误答案，揭示了LRM遗忘流程中的安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着遗忘权需求的增长，机器学习遗忘技术快速发展，但这些技术可能引入新的安全漏洞。尽管已有许多研究探讨遗忘攻击，但针对大型推理模型（LRM）的遗忘攻击研究仍是空白，本文旨在填补这一研究缺口。

Method: 提出双层精确遗忘攻击方法，包含可微目标函数、影响性令牌对齐和松弛指示器策略，克服了不可微逻辑约束、长推理轨迹优化效果弱和离散遗忘集选择等挑战。

Result: 设计了新颖的优化框架，在自盒和黑盒设置下进行了全面实验，证明了攻击的有效性和泛化能力，成功强制产生错误答案并生成看似合理但误导的推理轨迹。

Conclusion: 本文首次揭示了LRM遗忘流程中的安全威胁，提出的攻击方法有效且具有泛化性，旨在提高对LRM遗忘管道新兴威胁的认识。
...
### [223] [APPA: Adaptive Preference Pluralistic Alignment for Fair Federated RLHF of LLMs](https://arxiv.org/abs/2604.04261)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型需要与多元人类偏好对齐，但现有联邦RLHF中的聚合方法存在权衡：平均聚合会系统性地对最差组对齐不足，而最小聚合则以牺牲整体对齐为代价优先考虑最差组性能。

Method: 提出APPA（自适应偏好多元对齐）框架，基于历史对齐奖励动态重加权组级奖励，优先考虑对齐不足的组而不损害已对齐良好的组，且无需访问原始偏好数据。该方法集成到基于PPO的联邦RLHF流程中。

Result: 在GLOBALQA和OQA数据集上，使用三种模型家族（Gemma 2 2B、Llama 3.2 3B、Qwen3 0.6B）进行评估，APPA实现了强大的公平性与对齐权衡，相比平均聚合将最差组对齐提高了28%，同时在大多数配置中保持比最小聚合更高的整体对齐。

Conclusion: APPA框架在联邦RLHF中有效解决了多元对齐问题，通过自适应奖励重加权机制，在保持整体对齐质量的同时显著改善弱势群体的对齐效果，为大规模语言模型的公平对齐提供了实用解决方案。
...
### [224] [Entropy, Disagreement, and the Limits of Foundation Models in Genomics](https://arxiv.org/abs/2604.04287)
*Maxime Rochkoulets,Lovro Vrček,Mile Šikić*

Main category: cs.LG

TL;DR: 基因组基础模型效果不佳的原因在于DNA序列的高熵特性，导致模型预测分布接近均匀、模型间不一致、嵌入不稳定，且无法有效利用token间关系。


<details>
  <summary>Details</summary>
Motivation: 基因组基础模型相比自然语言处理中的基础模型效果有限，但原因尚不清楚。本研究旨在探究熵作为限制此类模型从训练数据中学习和发展基础能力的基本因素。

Method: 在文本和DNA序列上训练模型集合，分析其预测、静态嵌入和经验Fisher信息流。比较不同序列类型的熵特性对模型行为的影响。

Result: 基因组序列的高熵导致：1）未见token预测的输出分布接近均匀；2）不同模型间预测不一致；3）静态嵌入不稳定；4）DNA训练模型将Fisher信息集中在嵌入层，未能有效利用token间关系。
...
### [225] [DAGAF: A directed acyclic generative adversarial framework for joint structure learning and tabular data synthesis](https://arxiv.org/abs/2604.04290)
*Hristo Petkov,Calum MacLellan,Feng Dong*

Main category: cs.LG

TL;DR: DAGAF：一个双步骤框架，既能进行因果结构学习，又能生成表格数据，支持多种因果模型假设，在结构学习和数据合成方面都表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有因果学习方法通常只应用单一可识别因果模型（如ANM或LiNGAM），本文旨在改进这一局限，提出一个能够同时进行因果结构学习和表格数据合成的框架，支持多种因果模型假设。

Method: 提出新颖的双步骤框架，使用有向无环图表示数据变量间的因果关系，应用多种函数因果模型（包括ANM、LiNGAM和PNL），隐式学习DAG内容以模拟观测数据的生成过程，通过理论分析解释目标函数中的多个损失项。

Result: DAGAF在结构学习方面优于现有方法，在真实世界和基准数据集上实现了显著更低的SHD分数（Sachs：47%，Child：11%，Hailfinder：5%，Pathfinder：7%的改进），同时能够生成多样化的高质量样本。

Conclusion: DAGAF框架在因果结构学习和表格数据合成方面都表现出卓越性能，通过支持多种因果模型假设，能够更好地理解和复制真实数据分布。
...
### [226] [Correcting Source Mismatch in Flow Matching with Radial-Angular Transport](https://arxiv.org/abs/2604.04291)
*Fouad Oubari,Mathilde Mougeot*

Main category: cs.LG

TL;DR: RAFM框架通过匹配数据径向分布和均匀角分布来改进Flow Matching，特别适用于重尾或各向异性数据，减少高斯源的结构不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 传统Flow Matching通常使用高斯源和欧几里得概率路径，但对于重尾或各向异性数据，高斯源在径向分布层面就存在结构不匹配问题，需要更合适的源分布设计。

Method: 提出径向-角向流匹配(RAFM)框架：1) 使用与数据径向分布匹配的源分布；2) 保持条件角分布在球面上均匀；3) 将传输问题简化为角向对齐；4) 使用球面测地线插值定义条件路径；5) 保持标准无模拟训练流程不变。

Result: 1) 建立了匹配径向源的精确密度；2) 证明了径向-角向KL分解，分离出高斯径向惩罚项；3) 推导了诱导目标向量场的特性；4) 建立了流匹配误差与生成误差的稳定性关系；5) 实证显示RAFM显著优于标准高斯Flow Matching，与非高斯替代方法竞争力相当。

Conclusion: RAFM为Flow Matching在重尾和极端事件数据上提供了原则性的源和路径设计框架，通过显式修正源分布不匹配问题，在保持轻量级确定性训练的同时提升性能。
...
### [227] [Boosted Distributional Reinforcement Learning: Analysis and Healthcare Applications](https://arxiv.org/abs/2604.04334)
*Zequn Chen,Wesley J. Marrero*

Main category: cs.LG

TL;DR: 提出Boosted Distributional Reinforcement Learning (BDRL)算法，在优化个体结果分布的同时确保相似个体间的可比性，应用于高血压管理，相比传统强化学习基线提升质量调整生命年的数量和一致性。


<details>
  <summary>Details</summary>
Motivation: 传统基于期望的强化学习在高度不确定、涉及多个异质群体的复杂决策场景中表现不足。特别是在医疗领域，医生需要管理具有不确定疾病进展和异质治疗反应的多名患者，现有分布强化学习算法可能导致可比个体间实现效益的巨大差异。

Method: 提出Boosted Distributional Reinforcement Learning (BDRL)算法：1) 优化个体特定的结果分布；2) 强制相似个体间的可比性；3) 加入后更新投影步骤，将其表述为约束凸优化问题，高效地将个体结果与高性能参考在指定容差内对齐。

Result: 将BDRL应用于美国成年人群高血压管理，将个体分类为心血管疾病风险组。该方法通过模仿每个风险组中高性能参考的行为，修改中位数和脆弱患者的治疗计划。相比强化学习基线，BDRL提高了质量调整生命年的数量和一致性。

Conclusion: BDRL算法在优化个体结果分布的同时确保相似个体间的可比性，在高度不确定的异质群体决策场景中表现优于传统强化学习方法，特别适用于医疗等需要公平性和一致性的领域。
...
### [228] [Generative models for decision-making under distributional shift](https://arxiv.org/abs/2604.04342)
*Xiuyuan Cheng,Yunqin Zhu,Yao Xie*

Main category: cs.LG

TL;DR: 该教程介绍了现代生成模型（特别是基于流和得分的方法）作为构建决策相关分布的数学工具，用于处理数据驱动决策中的分布偏移问题。


<details>
  <summary>Details</summary>
Motivation: 许多数据驱动决策问题使用从历史数据估计的名义分布来制定，但实际部署时的分布可能发生偏移、依赖于上下文、部分可观测或受到压力影响，这导致名义分布与部署分布不匹配。

Method: 提出基于推前映射、连续性、Fokker-Planck方程、Wasserstein几何和概率空间优化的统一框架，利用生成模型（流模型和得分模型）通过传输映射、速度场、得分场和引导随机动力学来表示和转换分布。

Result: 生成模型可用于学习名义不确定性、构建压力或最不利分布以实现鲁棒性、在侧信息和部分观测下生成条件或后验分布，并提供了包括迭代流模型的前向-反向收敛、传输映射空间的一阶极小极大分析以及生成先验后验采样的误差传递界限等理论保证。

Conclusion: 该教程为使用生成模型进行场景生成、鲁棒决策、不确定性量化及相关分布偏移问题提供了原则性指导，展示了生成模型在运筹学中作为分布表示和转换工具的价值。
...
### [229] [CPT: Controllable and Editable Design Variations with Language Models](https://arxiv.org/abs/2604.04380)
*Karthik Suresh,Amine Ben Khalifa,Li Zhang,Wei-ting Hsu,Fangzheng Wu,Vinay More,Asim Kadav*

Main category: cs.LG

TL;DR: 提出CPT模型和CML表示法，用于生成可编辑的设计变体，解决设计多样性和个性化需求


<details>
  <summary>Details</summary>
Motivation: 设计过程手动且耗时，限制了创意工作流程的可扩展性和个性化，需要自动化生成可编辑的设计变体

Method: 使用仅解码器语言模型CPT，基于新的Creative Markup Language表示法，在专业设计师创作的设计模板语料库上进行微调

Result: 模型能生成语义结构化和风格一致的设计输出，保持元素间内部一致性，产生可编辑的设计文档而非像素图像

Conclusion: 该方法能生成上下文相关的颜色和字体变体，在调整布局同时保持设计原则，为创意工作流程提供可扩展的个性化解决方案
...
### [230] [Finite-Time Analysis of Q-Value Iteration for General-Sum Stackelberg Games](https://arxiv.org/abs/2604.04394)
*Narim Jeong,Donghwan Lee*

Main category: cs.LG

TL;DR: 本文从控制理论视角研究双人一般和马尔可夫博弈中Stackelberg Q值迭代的收敛性，首次为该设置提供有限时间收敛保证。


<details>
  <summary>Details</summary>
Motivation: 强化学习在单智能体设置中已取得理论和实证成功，但将其扩展到一般和马尔可夫博弈中的多智能体强化学习仍具挑战。本文旨在从控制理论视角研究Stackelberg Q值迭代在双人一般和马尔可夫博弈中的收敛性。

Method: 引入针对Stackelberg设置的松弛策略条件，将学习动态建模为切换系统，通过构造上下比较系统来分析收敛性。

Result: 建立了Q函数的有限时间误差界，并刻画了其收敛特性，首次为一般和马尔可夫博弈中Stackelberg交互下的Q值迭代提供了有限时间收敛保证。

Conclusion: 本文为Stackelberg学习提供了新颖的控制理论视角，填补了该领域有限时间收敛理论分析的空白。
...
### [231] [Relative Density Ratio Optimization for Stable and Statistically Consistent Model Alignment](https://arxiv.org/abs/2604.04410)
*Hiroshi Takahashi,Tomoharu Iwata,Atsutoshi Kumagai,Sekitoshi Kanai,Masanori Yamada,Kosuke Nishida,Kazutoshi Shinoda*

Main category: cs.LG

TL;DR: 提出一种新的语言模型对齐方法，通过相对密度比优化实现稳定且统计一致的人类偏好对齐


<details>
  <summary>Details</summary>
Motivation: 现有对齐方法通常假设特定的人类偏好模型（如Bradley-Terry模型），这种假设可能无法准确捕捉真实的人类偏好，导致缺乏统计一致性。虽然DDRO方法实现了统计一致性，但其密度比不稳定且容易发散，导致训练不稳定。

Method: 提出基于相对密度比的对齐方法，使用偏好数据分布与偏好/非偏好混合分布之间的相对密度比。这种方法稳定，因为相对密度比有上界且不会发散，同时保持统计一致性。

Result: 新方法相比DDRO提供了更紧的收敛保证，在Qwen 2.5和Llama 3模型上实验验证了其有效性。

Conclusion: 提出的相对密度比对齐方法既稳定又具有统计一致性，解决了现有方法在稳定性和一致性之间的权衡问题。
...
### [232] [Is Prompt Selection Necessary for Task-Free Online Continual Learning?](https://arxiv.org/abs/2604.04420)
*Seoyoung Park,Haemin Lee,Hankook Lee*

Main category: cs.LG

TL;DR: SinglePrompt：一种无需提示选择、专注于分类器优化的任务无关在线持续学习方法，通过单一提示注入、余弦相似度logit设计和未暴露类掩码实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有任务无关在线持续学习方法中的提示选择策略经常无法选择适当的提示，导致次优结果，即使对关键参数进行了额外训练。这促使研究者提出一种更简单有效的替代方案。

Method: 1) 在每个自注意力块中注入单一提示；2) 采用基于余弦相似度的logit设计来缓解分类器权重中的遗忘效应；3) 对当前小批量中未暴露的类别的logit进行掩码处理。

Result: 该框架在各种在线持续学习基准测试中实现了最先进的性能，证明了其简单设计的有效性。

Conclusion: 通过消除提示选择需求并专注于分类器优化，SinglePrompt为任务无关在线持续学习提供了一种简单而有效的解决方案，在多个基准测试中表现出色。
...
### [233] [TinyNina: A Resource-Efficient Edge-AI Framework for Sustainable Air Quality Monitoring via Intra-Image Satellite Super-Resolution](https://arxiv.org/abs/2604.04445)
...
Method: 提出TinyNina边缘AI框架，采用新颖的"图像内学习"范式，利用Sentinel-2的多光谱层次作为内部训练标签，消除对外部高分辨率数据集依赖。框架包含波长特定注意力门和深度可分离卷积，在保持污染物敏感光谱特征的同时实现超轻量级架构（仅51K参数）。
...
### [234] [DP-OPD: Differentially Private On-Policy Distillation for Language Models](https://arxiv.org/abs/2604.04461)
*Fatemeh Khadem,Sajad Mousavi,Yi Fang,Yuhong Liu*

Main category: cs.LG

TL;DR: 提出DP-OPD框架，通过仅对学生模型应用DP-SGD实现隐私保护，利用冻结教师模型提供密集token级目标，避免合成文本生成，简化训练流程。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在专有和领域特定语料库上的应用存在隐私保护与模型压缩效率之间的冲突。现有隐私蒸馏方法要么对师生模型都应用DP-SGD导致计算和隐私-效用权衡恶化，要么依赖DP合成文本生成但需要离线生成流程。

Method: 提出DP-OPD框架：仅对学生模型应用DP-SGD进行隐私保护，利用冻结教师模型在学生生成的轨迹上提供密集token级目标，通过私有广义知识蒸馏在延续token上实现。

Result: 在严格隐私预算（ε=2.0）下，DP-OPD相比DP微调和离线DP蒸馏在困惑度上有显著改进，优于基于合成的DP蒸馏（Yelp：44.15→41.68；BigPatent：32.43→30.63），同时大幅简化训练流程。
...
### [235] [Discrete Prototypical Memories for Federated Time Series Foundation Models](https://arxiv.org/abs/2604.04475)
*Liwei Deng,Qingxiang Liu,Xinhe Niu,Shengchao Chen,Sheng Sun,Yuankai Wu,Guodong Long,Yuxuan Liang*

Main category: cs.LG

TL;DR: FeDPM是一个基于离散原型记忆的联邦时间序列基础模型框架，通过本地原型记忆学习和跨域记忆对齐来解决语义不匹配问题，在保护隐私的同时提升时间序列建模性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要问题：1）时间序列数据与LLM文本中心潜在空间之间的语义不匹配导致性能下降；2）联邦学习方法将异构跨域时间序列数据建模到统一的连续潜在空间，这与时间序列语义通常表现为离散重复模式的事实相矛盾。

Method: 提出FeDPM框架：1）为域内时间序列数据学习本地原型记忆先验；2）对齐跨域记忆以促进统一的离散潜在空间；3）引入域特定记忆更新机制来平衡共享和个性化原型知识。

Result: 大量实验证明了FeDPM的效率和有效性，代码已公开可用。

Conclusion: FeDPM通过离散原型记忆框架成功解决了时间序列数据与LLM语义不匹配问题，同时适应了时间序列的离散重复特性，在联邦学习环境下实现了更好的时间序列建模性能。
...
### [236] [ECG Biometrics with ArcFace-Inception: External Validation on MIMIC and HEEDB](https://arxiv.org/abs/2604.04485)
*Arjuna Scagnetto*

Main category: cs.LG

TL;DR: 研究评估了基于1D Inception-v1和ArcFace训练的ECG生物识别系统在大规模人群、外部领域偏移和多年度时间间隔下的表现，发现ECG身份信息在外部验证的大规模闭集条件下仍然可测量，但受领域异质性、时间漂移、图库大小和后续处理影响显著。


<details>
  <summary>Details</summary>
Motivation: ECG生物识别研究主要在小型队列和短期会话间隔上进行，缺乏对大规模图库、外部领域偏移和多年度时间间隔下识别性能的系统评估。

Method: 使用1D Inception-v1模型结合ArcFace损失函数，在包含164,440个12导联ECG（来自53,079名患者）的内部临床语料库上训练。在MIMIC-IV-ECG和HEEDB的更大队列上测试，采用统一的闭集留一协议，使用Rank@K和TAR@FAR指标，并进行规模、时间压力、重排序和置信度分析。

Result: 在一般可比性下，系统在ASUGI-DB上达到Rank@1=0.9506，在MIMIC-GC上为0.8291，在HEEDB-GC上为0.6884。时间压力测试显示，随着时间间隔从1年增加到5年，Rank@1从0.7853降至0.6433（MIMIC）和从0.6864降至0.5560（HEEDB）。规模分析显示性能随图库增大单调下降，但随患者检查次数增加而恢复。在HEEDB-RR上，后处理重排序进一步提升了检索性能。

Conclusion: ECG身份信息在外部验证的大规模闭集条件下仍然可测量，但其操作质量受到领域异质性、纵向漂移、图库大小和第二阶段分数处理的显著影响。
...
### [237] [Isokinetic Flow Matching for Pathwise Straightening of Generative Flows](https://arxiv.org/abs/2604.04491)
*Tauhid Khan*
...
<details>
  <summary>Details</summary>
Motivation: Flow Matching方法构建线性条件概率路径时，学习的边际速度场由于轨迹叠加会产生强曲率，这严重放大了数值截断误差，成为少步采样的瓶颈
...
Result: 在CIFAR-10 (DiT-S/2)上，Iso-FM将2步条件非最优传输FID从78.82降至27.13，相对效率提升2.9倍；4步采样达到最佳FID 10.23

Conclusion: 加速度正则化是一种原则性、计算高效的快速生成采样机制，Iso-FM作为纯即插即用的单阶段FM训练补充，显著改善了少步生成性能
...
### [238] [SLaB: Sparse-Lowrank-Binary Decomposition for Efficient Large Language Models](https://arxiv.org/abs/2604.04493)
*Ziwei Li,Yuang Ma,Yi Kang*

Main category: cs.LG

TL;DR: SLaB是一种新的LLM压缩框架，通过将线性层权重分解为稀疏矩阵、低秩矩阵和二进制矩阵三个互补组件，无需重新训练，在50%压缩率下比现有方法降低36%困惑度
...
Method: 提出SLaB框架，将每个线性层权重分解为三个互补组件：稀疏矩阵、低秩矩阵和二进制矩阵。无需重新训练，利用激活感知剪枝分数指导分解过程

Result: 在Llama系列模型上的实验表明，SLaB达到最先进性能，在50%压缩率下比现有方法降低36%困惑度，在零样本任务上比基线提高8.98%准确率

Conclusion: SLaB框架有效解决了LLM部署中的计算和内存挑战，在高压缩率下保持良好性能，为模型压缩提供了新的有效方法
...
### [239] [Reproducibility study on how to find Spurious Correlations, Shortcut Learning, Clever Hans or Group-Distributional non-robustness and how to fix them](https://arxiv.org/abs/2604.04518)
*Ole Delzer,Sidney Bender*

Main category: cs.LG

TL;DR: 该研究统一了多个领域关于DNN可靠性的研究视角，比较了在数据有限和子群不平衡等挑战下，基于可解释AI的校正方法与传统方法的性能差异。


<details>
  <summary>Details</summary>
Motivation: DNN在医疗诊断和自动驾驶等高风险领域应用日益广泛，但模型可靠性研究在不同社区间存在术语分裂。尽管分布鲁棒优化、不变风险最小化、捷径学习等框架都旨在解决模型因虚假相关性而失效的问题，但研究者通常只参考自己领域的工作，缺乏统一视角。

Method: 通过可重复性研究统一不同视角，在数据有限和严重子群不平衡的挑战性约束下，比较基于可解释AI的校正方法与流行的非XAI基线方法。使用合成和真实世界数据集进行评估，包括反事实知识蒸馏等方法。

Result: 基于XAI的方法通常优于非XAI方法，其中反事实知识蒸馏在提高泛化能力方面表现最稳定。但许多方法依赖组标签，而手动标注通常不可行，自动工具如谱相关性分析在处理复杂特征和严重不平衡时存在困难。验证集中少数群体样本稀缺导致模型选择和超参数调优不可靠。

Conclusion: 该研究揭示了在安全关键领域部署鲁棒可信模型面临的重要障碍：组标签依赖性和验证集不平衡问题。需要开发不依赖组标签的校正方法，并解决模型选择中的样本稀缺问题。
...
### [240] [Learning from Equivalence Queries, Revisited](https://arxiv.org/abs/2604.04535)
*Mark Braverman,Roi Livni,Yishay Mansour,Shay Moran,Kobbi Nissim*

Main category: cs.LG

TL;DR: 论文重新审视了Angluin的等价查询学习模型，针对对抗性反例生成过于悲观的问题，提出了对称反例生成器这一更自然的限制条件，并在完全信息和bandit反馈两种设置下获得了紧密的学习轮数界限。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习系统（如生成模型和推荐系统）通常通过部署、用户交互和定期模型更新的循环演进，这与标准的监督学习框架不同。传统等价查询学习模型中的完全对抗性反例生成过于悲观，且大多数先前工作假设完全信息设置（学习者能观察到反例的正确标签），这在某些场景下并不自然。

Method: 提出对称反例生成器的概念，这类生成器仅基于假设与目标之间的对称差异来选择反例。该方法捕捉了随机反例和按预设复杂度度量返回最简单反例等自然机制。在完全信息和bandit反馈两种设置下，结合博弈论视角、自适应加权方法和极小极大论证来分析学习轮数。

Result: 在对称反例生成器的框架下，获得了完全信息和bandit反馈两种设置中学习轮数的紧密界限。这些结果为该学习模型提供了更实际的理论保证。

Conclusion: 通过限制环境为对称反例生成器这一更自然的对抗性类别，解决了传统等价查询学习模型过于悲观的问题。研究为现代机器学习系统的循环部署更新过程提供了理论基础，并指出了未来研究方向。
...
### [241] [Beyond Imbalance Ratio: Data Characteristics as Critical Moderators of Oversampling Method Selection](https://arxiv.org/abs/2604.04541)
*Yuwen Jiang,Songyun Ye*

Main category: cs.LG

TL;DR: 传统认为类别不平衡比例(IR)与过采样效果正相关，但缺乏实证支持。通过控制实验发现IR与过采样效果呈弱到中度负相关，而类别可分性才是更强的调节因素。


<details>
  <summary>Details</summary>
Motivation: 当前IR阈值范式假设不平衡比例与过采样效果正相关，但这一假设缺乏通过控制变量实验的实证验证。研究者希望系统性地检验IR对过采样方法有效性的真实影响。

Method: 进行了12个控制实验（超过100个数据集变体），通过高斯混合模型算法生成数据集，系统性地操纵IR同时保持数据特征（类别可分性、聚类结构）恒定。还进行了两个验证实验检验天花板效应和度量依赖性。所有方法在17个OpenML真实数据集上评估。

Result: 控制混杂变量后，IR与过采样效益呈弱到中度负相关。类别可分性成为显著更强的调节因素，比IR单独解释了更多方法有效性的方差。

Conclusion: 提出了"情境重要"框架，整合IR、类别可分性和聚类结构，为从业者提供基于证据的选择标准。研究挑战了传统IR阈值范式，强调需要更全面的考虑数据特征。
...
### [242] [A Clinical Point Cloud Paradigm for In-Hospital Mortality Prediction from Multi-Level Incomplete Multimodal EHRs](https://arxiv.org/abs/2604.04614)
...
<details>
  <summary>Details</summary>
Motivation: 由于临床工作流程的多样性和隐私约束，原始电子健康记录存在多级不完整性，包括不规则采样、缺失模态和稀疏标签。现有方法通常假设数据相对完整，或仅孤立地处理其中一两个问题，导致临床语义失真。

Method: 提出HealthPoint (HP)范式，将异构临床事件表示为连续4D空间中的点（内容、时间、模态、病例）。引入低秩关系注意力机制建模任意点对之间的交互，并开发分层交互和采样策略平衡细粒度建模与计算效率。

Result: 在大规模电子健康记录数据集上的风险预测实验中，HP在不同程度的不完整性下均实现了最先进的性能，并展现出强大的鲁棒性。

Conclusion: HP为多级不完整电子健康记录提供了一种统一的临床点云范式，支持灵活的事件级交互和细粒度自监督，能够有效利用未标记数据并实现稳健的模态恢复。
...
### [243] [From Curiosity to Caution: Mitigating Reward Hacking for Best-of-N with Pessimism](https://arxiv.org/abs/2604.04648)
*Zhuohao Yu,Zhiwei Steven Wu,Adam Block*

Main category: cs.LG

TL;DR: 本文提出了一种名为"谨慎"的新方法，通过惩罚预测误差来缓解BoN采样中的奖励黑客问题，相比传统方法更有效地利用额外计算资源。


<details>
  <summary>Details</summary>
Motivation: BoN采样虽然能提升语言模型性能，但随着候选响应数量增加，容易受到奖励黑客攻击——模型会选择利用奖励模型缺陷而非真正提升质量的响应。现有方法要么无法完全解决过优化问题，要么过于保守无法充分利用额外计算资源。

Method: 提出"谨慎"方法，基于强化学习中的悲观原则，使用价值估计的下置信界来避免分布外动作。该方法训练一个误差模型来预测典型响应的误差，然后用其预测误差来降低非典型响应的奖励估计，本质上是对预测误差进行惩罚。

Result: 广泛的实证评估表明，谨慎方法能显著缓解BoN采样中的奖励黑客问题，且计算效率高。在简化的线性设置中的理论分析也证明，谨慎方法在理论上优于标准BoN方法。

Conclusion: 谨慎方法为奖励黑客问题提供了实用的解决方案，同时也证明基于好奇心的方法可以作为LLM设置中通用的分布外检测技术。
...
### [244] [Explainable Machine Learning for Sepsis Outcome Prediction Using a Novel Romanian Electronic Health Record Dataset](https://arxiv.org/abs/2604.04698)
*Andrei-Alexandru Bunea,Ovidiu Ghibea,Dan-Matei Popovici,Ion Daniel,Octavian Andronic*

Main category: cs.LG

TL;DR: 该研究开发了可解释的机器学习模型用于脓毒症预后预测，使用罗马尼亚一家大型急诊医院的12,286例住院数据，在三个分类任务中实现了优异性能，并通过SHAP分析识别了关键临床预测因子。


<details>
  <summary>Details</summary>
Motivation: 开发可解释的机器学习模型来预测脓毒症预后，旨在识别临床强预测因子，同时在不同分类任务中达到最先进的结果，为临床决策提供支持。

Method: 使用包含人口统计学、ICD-10诊断和600种实验室测试的电子健康记录数据集，训练了五种机器学习模型来捕捉复杂分布同时保持临床可解释性。实验探索了特征丰富度与患者覆盖范围之间的权衡，使用10-50个最常见实验室测试的子集。使用准确率和AUC评估模型性能，使用SHAP评估可解释性。

Result: 在"死亡vs康复"案例研究中获得了最高性能（AUC=0.983，准确率=0.93）。SHAP分析识别了多个强预测因子，包括心血管合并症、尿素水平、天冬氨酸氨基转移酶、血小板计数和嗜酸性粒细胞百分比。嗜酸性粒细胞减少症成为顶级预测因子，突显了其作为未被充分利用的标记物的价值。
...
### [245] [MUXQ: Mixed-to-Uniform Precision MatriX Quantization via Low-Rank Outlier Decomposition](https://arxiv.org/abs/2604.04701)
*Seoungsub Lee,In Seo Kim,Seon Wook Kim*

Main category: cs.LG

TL;DR: MUXQ是一种针对NPU设备上LLM推理的混合到均匀量化方法，通过检测输入激活中的异常通道并使用辅助矩阵重新分配异常值，实现低精度INT量化同时保持硬件友好的计算结构。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型参数庞大导致内存和计算开销大，在NPU设备上FP16/FP32计算效率低，需要整数量化。现有量化方法未能完全解决输入激活异常值和相关的硬件效率问题。

Method: 提出MUXQ方法：检测输入激活中的异常通道，引入小型辅助矩阵重新分配异常值幅度，缓解异常问题，使异常激活也能在低精度INT级别量化，同时保持硬件友好的计算结构。

Result: 在三个规模的GPT-2模型上实验显示，MUXQ始终比朴素量化获得更低的困惑度。在每张量量化下，MUXQ将激活和权重都量化为INT8，同时保持接近FP16的精度，仅有适度的计算开销。

Conclusion: MUXQ为边缘设备上高效准确的LLM推理提供了有前景的方向，能够实现稳定的低精度推理，并且可以与其他量化技术轻松结合。
...
### [246] [The Infinite-Dimensional Nature of Spectroscopy and Why Models Succeed, Fail, and Mislead](https://arxiv.org/abs/2604.04717)
*Umberto Michelucci,Francesca Venturini*

Main category: cs.LG

TL;DR: 论文揭示光谱数据高维特性导致机器学习模型即使在没有化学意义特征时也能获得极高准确率，源于高维空间中微小分布差异的完美可分性。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在光谱分类任务中获得了惊人的高准确率，但缺乏明确证据表明这些模型使用了具有化学意义的特征。现有研究将这一现象归因于数据预处理选择、噪声敏感性和模型复杂性，但缺乏统一解释。

Method: 基于Feldman-Hajek定理和测度集中理论进行理论分析，证明在高维空间中，由噪声、归一化或仪器伪影引起的微小分布差异可能变得完美可分。通过在合成和真实荧光光谱上进行一系列具体实验验证理论。

Result: 研究表明，即使在没有化学区分的情况下，模型也能实现接近完美的准确率；特征重要性图可能突出光谱无关区域。实验证实了高维空间中微小分布差异的可分性效应。

Conclusion: 为光谱学中构建和解释机器学习模型提供了严谨的理论框架和实用建议，强调需要谨慎解释模型结果，避免将高维可分性误认为化学意义的特征学习。
...
### [247] [Darkness Visible: Reading the Exception Handler of a Language Model](https://arxiv.org/abs/2604.04756)
*Peter Balogh*

Main category: cs.LG

TL;DR: GPT-2 Small最后一层MLP展现清晰的路由程序结构，包含27个命名神经元组成的三层异常处理器，而知识仍分散在3040个残差神经元中。研究发现知识神经元主要作为路由基础设施而非事实存储。


<details>
  <summary>Details</summary>
Motivation: 研究GPT-2 Small模型最后一层MLP的内部工作机制，探索其如何组织知识表示和处理语言信息，特别是理解知识神经元在模型中的实际功能。

Method: 将3072个神经元分解为5类：5个融合核心神经元、10个区分器、5个专家、7个共识神经元。通过三个实验分析知识神经元功能，包括花园路径实验和统计分析方法。

Result: 发现MLP干预从有益到有害的共识-异常交叉点在统计上显著（4/7到5/7之间）。知识神经元主要作为路由基础设施，放大或抑制注意力机制产生的信号。GPT-2立即使用动词次范畴化信息，表明异常处理器基于token级可预测性而非句法结构运作。

Conclusion: GPT-2 Small的最终MLP形成了清晰的路由程序架构，知识神经元主要作为路由基础设施而非事实存储。这种架构只在最终层结晶，在更深层模型中预测类似结构会出现在最终层而非第11层。
...
### [248] [Cog-DRIFT: Exploration on Adaptively Reformulated Instances Enables Learning from Hard Reasoning Problems](https://arxiv.org/abs/2604.04767)
*Justin Chih-Yao Chen,Archiki Prasad,Zaid Khan,Joykirat Singh,Runchu Tian,Elias Stengel-Eskin,Mohit Bansal*

Main category: cs.LG

TL;DR: 论文提出Cog-DRIFT框架，通过将困难的开放式问题转化为认知上更简单的变体（如选择题和完形填空），构建自适应课程，帮助LLM从原本无法获得奖励信号的难题中学习。


<details>
  <summary>Details</summary>
Motivation: 当前基于可验证奖励的强化学习（RLVR）存在根本限制：模型无法从当前策略下过于困难的问题中学习，因为这些问题的奖励信号为零。需要一种方法来帮助模型克服探索障碍。

Method: 提出Cog-DRIFT框架：1）将困难的开放式问题转化为认知上更简单的变体（如选择题、完形填空），保留原始答案但减少搜索空间；2）构建从判别式到生成式任务的连续谱；3）设计自适应课程，从简单格式逐步过渡到困难格式，实现知识迁移。

Result: 在2个模型和6个推理基准测试中，Cog-DRIFT显著提升性能：对Qwen模型在原本无法解决的难题上绝对提升+10.11%，对Llama模型提升+8.64%；平均优于第二好基线+4.72%（Qwen）和+3.23%（Llama）；同时提升测试时的pass@k指标和样本效率。

Conclusion: 任务重构和课程学习是克服LLM后训练中探索障碍的有效范式。通过将困难问题转化为更简单的认知变体并构建自适应课程，模型能够从原本无法获得奖励信号的问题中学习，显著提升推理能力。
...
### [249] [Forgetting to Witness: Efficient Federated Unlearning and Its Visible Evaluation](https://arxiv.org/abs/2604.04800)
*Houzhe Wang,Xiaojie Zhu,Chi Chen*

Main category: cs.LG

TL;DR: 提出首个联邦遗忘完整流程，包括高效遗忘方法和可视化评估框架Skyeye，无需存储历史数据
...
Method: 提出联邦遗忘方法：利用知识蒸馏模型和多种优化机制，无需存储历史数据；提出Skyeye评估框架：将联邦遗忘模型作为GAN分类器，通过生成器可视化遗忘能力

Result: 综合实验表明提出的联邦遗忘方法和评估框架有效，能高效实现遗忘并可视化评估遗忘能力

Conclusion: 本文提出了首个完整的联邦遗忘流程，包括高效遗忘方法和可视化评估框架，为联邦遗忘领域提供了系统解决方案
...
### [250] [Selecting Decision-Relevant Concepts in Reinforcement Learning](https://arxiv.org/abs/2604.04808)
*Naveen Raman,Stephanie Milani,Fei Fang*

Main category: cs.LG

TL;DR: 提出了首个用于序列决策中概念选择的算法DRS，通过状态抽象视角将概念选择视为决策相关性问题，自动选择关键概念并保证性能边界


<details>
  <summary>Details</summary>
Motivation: 传统基于概念的可解释策略需要人工选择概念，这需要领域专业知识、耗时耗力、难以扩展且无法保证性能，需要自动化的概念选择方法

Method: 提出决策相关选择(DRS)算法，将概念选择视为状态抽象问题：如果一个概念被移除会导致智能体混淆需要不同动作的状态，则该概念是决策相关的。算法从候选概念集中选择子集，确保相同概念表示的状态共享相同的最优动作

Result: DRS能够自动恢复人工策划的概念集，同时匹配或超越其性能，并在强化学习基准和真实医疗环境中提高了测试时概念干预的有效性

Conclusion: DRS算法为序列决策中的概念选择提供了首个原则性方法，通过状态抽象视角解决了人工概念选择的局限性，实现了自动化的决策相关概念选择并保证性能
...
### [251] [The Role of Generator Access in Autoregressive Post-Training](https://arxiv.org/abs/2604.04855)
*Amit Kiran Rege*
...
<details>
  <summary>Details</summary>
Motivation: 探索生成器访问方式对自回归后训练的限制，特别是学习者是否仅限于新鲜根起始采样，还是可以返回到先前构建的前缀并查询下一个标记规则

Method: 分析不同生成器接口：根起始采样（输出采样、生成标记对数概率、top-k报告、完整下一个标记分布）与弱前缀控制（条件采样或logits），研究KL正则化结果奖励后训练

Result: 根起始采样中所有观测都简化为一个规范实验，受限于到达信息前缀的在线概率；弱前缀控制打破这一障碍，一旦获得控制，条件采样或logits等更丰富的观测可以超越top-1访问；仅改变生成器接口就在KL正则化结果奖励后训练中产生指数级差距

Conclusion: 生成器访问方式对自回归后训练有重要影响，弱前缀控制相比根起始采样能显著提升性能，仅改变接口就能产生指数级性能差距
...
### [252] [FairLogue: A Toolkit for Intersectional Fairness Analysis in Clinical Machine Learning Models](https://arxiv.org/abs/2604.04858)
...
<details>
  <summary>Details</summary>
Motivation: 现有公平性工具主要关注单轴人口统计学比较，可能遗漏影响交叉群体的复合差异。在医疗机器学习中，确保算法公平对公平和可信赖至关重要。

Method: Fairlogue包含三个组件：1)将人口统计均等、均衡机会和均等机会差异扩展到交叉群体的观测框架；2)评估治疗背景下公平性的反事实框架；3)评估交叉群体成员干预下公平性的广义反事实框架。使用All of Us数据集在青光眼手术预测任务中评估。

Result: 观测分析识别出显著的交叉差异（人口统计均等差异0.20，均衡机会真阳性率差距0.33，假阳性率差距0.15）。反事实分析显示观察到的差异在调整协变量后与随机机会一致。
...
### [253] [Noise Immunity in In-Context Tabular Learning: An Empirical Robustness Analysis of TabPFN's Attention Mechanisms](https://arxiv.org/abs/2604.04868)
*James Hu,Mahdi Ghelichi*

Main category: cs.LG

TL;DR: TabPFN作为表格基础模型，在数据质量不佳（如无关特征、相关特征组、标签噪声）的情况下表现出高度鲁棒性，能在单次前向传播中通过上下文学习进行预测，无需数据集特定的参数更新。


<details>
  <summary>Details</summary>
Motivation: 在金融和医疗等工业领域，表格预测应用广泛，但为每个新表格重新训练定制模型成本高昂或不可行，同时数据质量问题（如无关预测因子、相关特征组、标签噪声）普遍存在。需要验证表格基础模型在这些次优条件下的鲁棒性。

Method: 研究TabPFN及其注意力机制在二元分类问题中的表现，通过控制合成扰动进行实验：1）通过注入随机不相关特征和非线性相关特征改变数据集宽度；2）通过增加训练行数改变数据集大小；3）通过增加错误标签比例改变标签质量。除了预测性能，还分析内部信号包括注意力集中度和基于注意力的特征排名指标。

Result: TabPFN表现出显著韧性：ROC-AUC保持高位，注意力保持结构化和集中，信息丰富的特征在基于注意力的指标中排名靠前。注意力热图、特征-标记嵌入和SHAP图的可视化进一步支持了跨层的一致性模式，TabPFN越来越集中于有用特征，同时将其信号与噪声分离。

Conclusion: TabPFN是一个鲁棒的表格基础模型，能够在各种数据缺陷情况下保持预测性能和一致的内部行为，特别适合工业领域中的表格预测任务。
...
### [254] [Optimizing LLM Prompt Engineering with DSPy Based Declarative Learning](https://arxiv.org/abs/2604.04869)
*Shiek Ruksana,Sailesh Kiran Kurra,Thipparthi Sanjay Baradwaj*

Main category: cs.LG

TL;DR: DSPy框架通过声明式学习方法自动优化LLM提示，提升输出可靠性、事实准确性和泛化能力，减少幻觉和提示复杂性
...
Conclusion: DSPy框架为LLM提示优化提供了有效的声明式学习方法，显著改善了系统性能，但仍需进一步研究解决其局限性
...
### [255] [Data Attribution in Adaptive Learning](https://arxiv.org/abs/2604.04892)
*Amit Kiran Rege*

Main category: cs.LG

TL;DR: 论文提出了一种针对自适应学习场景的归因方法，解决了传统静态数据集归因方法无法处理训练数据动态生成的问题。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型越来越多地生成自己的训练数据（如在线bandit、强化学习、语言模型后训练等），在自适应学习环境中，单个训练观测不仅更新学习器，还会改变学习器未来收集数据的分布。传统的归因方法是为静态数据集设计的，无法处理这种反馈循环。

Method: 通过条件干预目标形式化有限时域自适应学习的实例级归因，证明重放侧信息通常无法恢复该目标，并识别出一个可以从日志数据中识别目标的结构类别。

Result: 建立了自适应学习归因的理论框架，确定了在何种条件下可以从观测数据中恢复归因目标，为自适应学习系统的归因分析提供了理论基础。

Conclusion: 自适应学习需要专门的归因方法，论文提出的框架为理解训练数据在自适应系统中的因果影响提供了理论基础，并指出了传统重放方法在一般情况下的局限性。
...
### [256] [Are Latent Reasoning Models Easily Interpretable?](https://arxiv.org/abs/2604.04902)
*Connor Dilgren,Sarah Wiegreffe*

Main category: cs.LG

TL;DR: 该研究发现当前潜在推理模型（LRMs）的推理标记经常未被充分利用，但当它们确实被使用时，通常编码了可解释的推理过程，并且可解释性本身可以作为预测正确性的信号。


<details>
  <summary>Details</summary>
Motivation: 潜在推理模型（LRMs）因其低推理成本和理论上能并行探索多个推理路径而受到关注，但这些优势以降低可解释性为代价。本研究旨在探究LRMs的可解释性问题，特别是验证这些模型是否真正在潜在空间中执行有意义的推理。

Method: 研究分析了两种最先进的LRMs：1）通过消融实验测试推理标记的必要性；2）当推理标记确实必要时，尝试解码黄金推理轨迹；3）提出一种无需先验知识即可从潜在标记解码验证的自然语言推理轨迹的方法。

Result: 1）在逻辑推理数据集上，LRMs几乎总能在不使用潜在推理的情况下产生相同答案，表明推理标记经常未被充分利用；2）当推理标记确实必要时，对于正确预测的实例，可以解码出65-93%的黄金推理轨迹；3）提出的解码方法能为大多数正确预测找到验证的推理轨迹，但只能为少数错误预测找到。

Conclusion: 当前LRMs主要编码可解释的推理过程，可解释性本身可以作为预测正确性的信号。研究对先前工作中关于推理标记作用的假设提出了质疑，并展示了从LRMs中提取可解释推理的可能性。
...
### [257] [HI-MoE: Hierarchical Instance-Conditioned Mixture-of-Experts for Object Detection](https://arxiv.org/abs/2604.04908)
*Vadim Vashkelis,Natalia Trukhina*

Main category: cs.LG

TL;DR: HI-MoE是一种用于目标检测的DETR风格架构，采用两阶段专家路由机制：场景路由器选择场景一致的专家子集，实例路由器为每个目标查询分配少量专家，在保持稀疏计算的同时更好地匹配检测任务的实例中心特性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉MoE方法主要在图像或补丁级别操作，这与目标检测的基本推理单元（对应候选实例的目标查询）不匹配。需要一种更适合目标检测任务的稀疏路由机制。

Method: 提出分层实例条件混合专家（HI-MoE）架构，包含两阶段路由：1）轻量级场景路由器选择场景一致的专家子集；2）实例路由器为每个目标查询在选定子集中分配少量专家。这种设计旨在保持稀疏计算的同时更好地匹配检测任务的异构、实例中心结构。

Result: 在COCO数据集上的实验表明，HI-MoE优于密集DINO基线和简单的标记级别或仅实例路由变体，在小目标检测上表现尤其突出。在LVIS上的初步专业化分析也显示了专家专业化模式的可视化结果。

Conclusion: HI-MoE通过分层实例条件路由机制，为目标检测任务提供了更合适的稀疏计算架构，在保持计算效率的同时提高了检测性能，特别是在小目标检测方面表现优异。
...
### [258] [Stratifying Reinforcement Learning with Signal Temporal Logic](https://arxiv.org/abs/2604.04923)
*Justin Curry,Alberto Speranzon*

Main category: cs.LG

TL;DR: 本文为信号时序逻辑（STL）开发了基于分层理论的语义学，揭示了分层理论与STL之间的对应关系，并将该框架应用于深度强化学习嵌入空间分析。


<details>
  <summary>Details</summary>
Motivation: 为信号时序逻辑（STL）提供新的理论框架，将分层理论应用于STL语义解释，从而更好地分析深度强化学习生成的嵌入空间结构及其与决策空间几何的关系。
...
Result: 建立了分层理论与STL之间的对应关系，表明大多数STL公式可视为诱导了时空分层；提出的计算签名方法在初步证据中显示出揭示嵌入空间分层结构的潜力。

Conclusion: 分层理论为STL提供了新的语义学框架，不仅为分析DRL嵌入空间结构提供了理论基础，还启发了新的计算技术开发，有助于更好地理解智能体决策空间的几何特性。
...
### [259] [IC3-Evolve: Proof-/Witness-Gated Offline LLM-Driven Heuristic Evolution for IC3 Hardware Model Checking](https://arxiv.org/abs/2604.03232)
*Mingkai Miao,Guangyu Hu,Ziyi Yang,Hongce Zhang*

Main category: cs.AI

TL;DR: IC3-Evolve：一个利用LLM自动进化IC3算法实现的离线代码演化框架，通过严格的证明/见证门控验证确保正确性，在硬件模型检查竞赛基准上实现了可靠的启发式改进。
...
Method: 提出IC3-Evolve框架：1）使用LLM离线生成针对IC3实现的小型、槽位受限且可审计的补丁；2）采用严格的证明/见证门控验证机制：SAFE运行必须生成可独立检查的证书，UNSAFE运行必须生成可重放的计数器示例轨迹；3）所有候选补丁都经过验证后才被采纳，确保正确性。

Result: 在公开硬件模型检查竞赛（HWMCC）基准上进行演化，并在未见过的公开和工业模型检查基准上评估泛化能力。结果表明IC3-Evolve能够在严格正确性门控下可靠地发现实用的启发式改进。

Conclusion: IC3-Evolve成功实现了自动化离线代码演化，通过LLM生成补丁并严格验证，最终部署的检查器是独立的演化版本，无需ML/LLM推理开销和运行时模型依赖，为IC3算法的性能改进提供了可靠途径。
...
### [260] [Structural Segmentation of the Minimum Set Cover Problem: Exploiting Universe Decomposability for Metaheuristic Optimization](https://arxiv.org/abs/2604.03234)
*Isidora Hernández,Héctor Ferrada,Cristóbal A. Navarro*

Main category: cs.AI

TL;DR: MSCP（最小集合覆盖问题）是NP难组合优化问题，本文提出利用宇宙可分割性将问题分解为独立子问题，使用GRASP元启发式求解，显著提升大规模可分解实例的求解质量和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有MSCP求解方法大多将实例视为整体处理，忽略了宇宙的内在结构特性。本文旨在研究宇宙可分割性概念，探索如何利用内在结构分解来增强启发式优化效果。

Method: 提出基于不相交集合（并查集）的预处理策略，检测子集中元素共现诱导的连通分量，将原始实例分解为独立子问题。每个子问题使用GRASP元启发式求解，部分解组合时保持可行性。采用简洁的位级集合表示实现高效集合操作。

Result: 在标准基准实例和大规模合成数据集上的实验表明，利用自然宇宙分割能持续改进解质量和可扩展性，特别是对于大型且结构可分解的实例。位级集合表示使方法在计算上具有实际可扩展性。

Conclusion: MSCP中的宇宙可分割性是可利用的重要结构特性，通过基于并查集的预处理和GRASP求解的分解方法能显著提升求解性能，为大规模组合优化问题提供了有效的分解策略。
...
### [261] [Position: Science of AI Evaluation Requires Item-level Benchmark Data](https://arxiv.org/abs/2604.03244)
*Han Jiang,Susu Zhang,Xiaoyuan Yi,Xing Xie,Ziang Xiao*

Main category: cs.AI

TL;DR: 论文主张通过项目级AI基准数据建立严谨的AI评估科学，提出OpenEval存储库支持证据中心的AI评估


<details>
  <summary>Details</summary>
Motivation: 当前AI评估范式存在系统性有效性失败，从不合理的设计选择到不匹配的指标，这些问题缺乏收集有效性证据和进行细粒度诊断分析的原则框架

Method: 通过分析项目属性和潜在构念，展示项目级数据提供的独特见解；引入OpenEval存储库，这是一个不断增长的项目级基准数据仓库，支持证据中心的AI评估

Result: 项目级分析能够实现细粒度诊断和基准的原则性验证，为AI评估提供更严谨的科学基础

Conclusion: 项目级AI基准数据对于建立严谨的AI评估科学至关重要，OpenEval存储库旨在促进社区广泛采用这种证据中心的评估方法
...
### [262] [Evaluating Artificial Intelligence Through a Christian Understanding of Human Flourishing](https://arxiv.org/abs/2604.03356)
*Nicholas Skytland,Lauren Parsons,Alicia Llewellyn,Steele Billings,Peter Larson,John Anderson,Sean Boisen,Steve Runge*

Main category: cs.AI

TL;DR: 论文提出AI对齐不仅是安全问题，更是塑造问题，引入基督教繁荣基准FAI-C-ST评估前沿模型，发现AI系统并非世界观中立，而是默认程序性世俗主义，导致繁荣维度表现下降约17点，信仰维度下降31点。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型越来越多地介入道德讨论和精神探索，它们不仅仅是提供信息，而是作为数字教化的工具，积极塑造和规范人类的理解、决策和道德反思。需要使这种塑造性影响变得可见和可测量，以评估AI系统对人类繁荣的贡献。

Method: 引入繁荣AI基准：基督教单轮对话（FAI-C-ST）框架，基于基督教对人类繁荣的理解，在七个维度上评估前沿模型响应。将20个前沿模型与多元主义和基督教特定标准进行比较。

Result: 当前AI系统并非世界观中立，而是默认程序性世俗主义，缺乏维持神学连贯性的基础。在所有繁荣维度上，系统性能下降约17点，其中最关键的是信仰和灵性维度下降31点。

Conclusion: AI价值观对齐中的性能差距不是技术限制，而是源于训练目标优先考虑广泛接受性和安全性，而非深度、内在连贯的道德或神学推理。AI对齐从根本上是一个塑造问题，而不仅仅是安全问题。
...
### [263] [VERT: Reliable LLM Judges for Radiology Report Evaluation](https://arxiv.org/abs/2604.03376)
*Federica Bologna,Jean-Philippe Corbeil,Matthew Wilkens,Asma Ben Abacha*

Main category: cs.AI

TL;DR: 该研究系统评估了不同LLM作为放射学报告评估指标的性能，提出了新的VERT指标，并通过微调实现了显著的性能提升和推理加速。


<details>
  <summary>Details</summary>
Motivation: 当前放射学报告评估研究主要关注胸部X射线和特定模型，缺乏对其他模态和解剖部位的鲁棒性验证，需要确定最适合作为LLM评估指标的模型和提示配置。

Method: 1. 比较三种现有LLM评估指标（RadFact、GREEN、FineRadScore）和提出的VERT指标；2. 使用不同规模的开源和闭源模型（推理型和非推理型）；3. 在两个专家标注数据集（RadEval和RaTE-Eval）上进行测试；4. 评估少样本学习、集成学习和参数高效微调；5. 进行系统性错误检测和分类分析。

Result: 1. VERT指标相比GREEN与放射科医生判断的相关性提高了11.7%；2. 微调Qwen3 30B模型仅用1,300个训练样本就获得了25%的性能提升；3. 微调模型将推理时间减少了37.2倍；4. 识别了不同指标与专家判断的一致性和差异区域。

Conclusion: LLM作为评估指标在放射学报告评估中是有效的，通过轻量级适配（如微调）可以实现可靠评估，同时显著提高效率和性能。
...
### [264] [Hume's Representational Conditions for Causal Judgment: What Bayesian Formalization Abstracted Away](https://arxiv.org/abs/2604.03387)
*Yiling Wu*

Main category: cs.AI

TL;DR: 该论文从休谟文本中提取了因果判断的三个表征条件，追踪了这些条件从休谟到贝叶斯认识论和预测处理的演变，并以大语言模型为例说明现代框架如何保留了休谟的更新结构但抽象掉了这些表征条件。


<details>
  <summary>Details</summary>
Motivation: 论文旨在阐明休谟因果判断理论中的三个关键表征条件，并探究这些条件在现代概率和认知框架中的演变与保留情况，揭示传统哲学假设在现代计算模型中的转变。

Method: 1. 从休谟文本中提取三个表征条件：经验基础、结构化检索和生动性转移；2. 分析这些条件在休谟因果心理学中的核心地位；3. 追踪这些条件从休谟到贝叶斯认识论和预测处理的形式化演变轨迹；4. 以大语言模型为案例，展示现代框架如何保留更新结构但抽象掉表征条件。

Result: 研究发现：1. 休谟的因果判断确实依赖于三个表征条件；2. 从休谟到贝叶斯认识论和预测处理的发展轨迹中，后来的框架保留了休谟的更新结构，但抽象掉了经验基础、结构化检索和生动性转移这三个进一步的表征条件；3. 大语言模型作为当代案例，展示了统计更新形式如何在没有满足这三个条件的情况下运作，从而使休谟框架中先前作为背景假设的要求变得可见。

Conclusion: 休谟因果判断的三个表征条件在其理论中具有核心地位，但在现代形式化框架中被抽象掉了。大语言模型等当代计算模型揭示了这些先前隐含的条件，为我们理解认知框架的演变提供了重要视角，同时也凸显了传统哲学假设与现代计算模型之间的差异。
...
### [265] [TABQAWORLD: Optimizing Multimodal Reasoning for Multi-Turn Table Question Answering](https://arxiv.org/abs/2604.03393)
...
<details>
  <summary>Details</summary>
Motivation: 现有多轮表格推理方法依赖固定的文本序列化进行表格状态读取，导致表示误差在多轮中显著累积。虽然表格基础方法可以缓解此问题，但以推理计算和成本为代价，不适用于实际部署。
...
Result: TABQAWORLD实现了最先进的性能，比基线准确率提升4.87%，相比静态设置获得5.42%的准确率提升和33.35%的推理延迟降低。

Conclusion: TABQAWORLD作为一个免训练框架，为可靠且高效的表格推理设立了新标准，在保持实际部署可行性的同时显著提升了推理性能。
...
### [266] [Contextual Control without Memory Growth in a Context-Switching Task](https://arxiv.org/abs/2604.03479)
*Song-Ju Kim*

Main category: cs.AI

TL;DR: 提出基于干预的循环架构，通过上下文索引的加性操作符干预共享循环潜在状态，实现上下文依赖的序列决策，无需扩大循环维度或直接输入上下文


<details>
  <summary>Details</summary>
Motivation: 研究上下文依赖序列决策的第三种替代方案：通过干预共享循环潜在状态实现上下文依赖，而不是直接提供上下文输入或增加循环记忆维度

Method: 引入干预式循环架构：循环核心首先构建共享预干预潜在状态，然后上下文通过加性、上下文索引的操作符进行干预。在部分可观测性下的上下文切换序列决策任务中评估

Result: 在主要基准测试中，干预模型在不增加循环维度的情况下表现强劲。使用条件互信息作为操作探针，干预模型对任务相关的第一阶段结果表现出正的条件上下文信息

Conclusion: 干预共享循环状态为上下文控制提供了可行的替代方案，无需循环记忆增长，在该设置下是有效的
...
### [267] [Beyond Predefined Schemas: TRACE-KG for Context-Enriched Knowledge Graphs from Complex Documents](https://arxiv.org/abs/2604.03496)
*Mohammad Sadeq Abolhasani,Yang Ba,Yixuan He,Rong Pan*

Main category: cs.AI

TL;DR: TRACE-KG是一个多模态框架，无需预定义本体即可联合构建上下文丰富的知识图谱和诱导模式，提供结构化限定符捕获条件关系，并保持对源证据的完全可追溯性。


<details>
  <summary>Details</summary>
Motivation: 知识图谱构建通常依赖预定义本体或无模式提取。本体驱动的方法需要昂贵的模式设计和维护，而无模式方法通常产生碎片化的图谱，特别是在长技术文档中缺乏全局组织。需要一种既能保持一致性又能灵活适应文档内容的方法。

Method: TRACE-KG是一个多模态框架，通过结构化限定符捕获条件关系，并使用数据驱动模式组织实体和关系。该方法无需预定义本体，同时构建上下文丰富的知识图谱和诱导模式，保持对源证据的完全可追溯性。

Result: 实验表明，TRACE-KG能够产生结构连贯、可追溯的知识图谱，为基于本体的和无模式的构建流程提供了实用的替代方案。

Conclusion: TRACE-KG提供了一种实用的知识图谱构建方法，既能保持结构一致性，又能灵活适应文档内容，解决了本体驱动和无模式方法各自的局限性。
...
### [268] [Resource-Conscious Modeling for Next- Day Discharge Prediction Using Clinical Notes](https://arxiv.org/abs/2604.03498)
*Ha Na Cho,Sairam Sutari,Alexander Lopez,Hansen Bow,Kai Zheng*

Main category: cs.AI

TL;DR: 本文评估了轻量级微调大语言模型与传统文本模型在脊柱择期手术后次日出院预测中的表现，发现TF-IDF结合LGBM模型在召回率和AUC-ROC上表现最佳，而基于Transformer的生成模型整体表现不佳。


<details>
  <summary>Details</summary>
Motivation: 及时预测出院对于优化择期脊柱手术单元的床位周转和资源分配至关重要，需要评估轻量级微调大语言模型与传统文本模型在临床出院预测任务中的可行性。
...
Conclusion: 在现实世界不平衡的临床预测任务中，可解释且资源高效的模型可能优于紧凑型大语言模型，传统文本模型在出院预测任务中表现出更好的实用性。
...
### [269] [BioAlchemy: Distilling Biological Literature into Reasoning-Ready Reinforcement Learning Training Data](https://arxiv.org/abs/2604.03506)
...
<details>
  <summary>Details</summary>
Motivation: 当前大规模推理数据集中的生物学问题与现代生物学研究主题分布不匹配，这种主题不平衡可能影响性能表现。同时，从生物学研究文本中提取具有挑战性和可验证性研究问题的方法在应用强化学习提升生物学研究任务性能方面是关键但未充分发展的要素。

Method: 提出了BioAlchemy管道，用于从生物学研究文本语料库中获取多样化的可验证问答对。创建了BioAlchemy-345K训练数据集，包含超过34.5万个生物学科学推理问题。通过将数据集与现代科学生物学主题分布对齐，结合强化学习来提升推理性能。

Result: 开发了BioAlchemist-8B模型，相比基础推理模型在生物学基准上提升了9.12%的性能。该模型已在HuggingFace平台发布。

Conclusion: 该方法证明了通过创建与生物学研究主题分布对齐的高质量训练数据集，并结合强化学习，可以有效提升生物学科学推理能力。BioAlchemy管道为开发更强的生物学推理能力提供了有效途径。
...
### [270] [Structural Rigidity and the 57-Token Predictive Window: A Physical Framework for Inference-Layer Governability in Large Language Models](https://arxiv.org/abs/2604.03524)
*Gregory M. Ruddell*

Main category: cs.AI

TL;DR: 研究发现当前AI安全方法在多数指令调优模型中无法检测到预承诺信号，提出基于能量的治理框架和五类推理行为分类，表明规则违反和幻觉是不同故障模式，需要不同的检测机制。


<details>
  <summary>Details</summary>
Motivation: 当前AI安全主要依赖行为监控和后训练对齐，但实证测量显示这些方法在多数指令调优模型中无法产生可检测的预承诺信号，需要建立更有效的推理层治理框架。

Method: 提出基于能量的治理框架，将Transformer推理动态与约束满足模型连接，使用轨迹张力（ρ = ||a|| / ||v||）作为度量，在7个模型队列和5个几何机制下进行分析，识别预承诺窗口和能量不对称性。

Result: 在Phi-3-mini-4k-instruct模型中发现57个token的预承诺窗口，但这是模型、任务和配置特定的；提出五类推理行为分类（权威带、晚期信号、反转、平坦、支架选择性）；在7个模型中只有1个配置显示预测性信号；事实幻觉在72个测试条件下均无预测信号。

Conclusion: 规则违反和幻觉是不同故障模式，需要不同检测机制：内部几何监控仅在存在阻力时有效，事实混淆检测需要外部验证机制。该工作为推理层可治理性提供了可测量框架和部署风险评估分类法。
...
### [271] [Explainable Model Routing for Agentic Workflows](https://arxiv.org/abs/2604.03527)
...
<details>
  <summary>Details</summary>
Motivation: 当前智能体工作流的路由架构只关注性能优化，缺乏对模型能力与成本之间权衡的记录，开发者无法区分智能效率与预算驱动的潜在失败。

Method: Topaz框架包含三个核心组件：1)基于技能的分析，将多样基准测试综合为细粒度能力档案；2)完全可追溯的路由算法，使用基于预算和多目标优化；3)面向开发者的解释，将追踪信息转换为自然语言。

Result: Topaz使路由决策变得可解释，让用户能够理解、信任并有意义地引导路由智能体系统，支持用户审计系统逻辑并迭代调整成本-质量权衡。

Conclusion: 通过引入形式化的可审计性，Topaz框架解决了当前智能体路由中缺乏透明性的问题，使开发者能够区分智能效率与潜在失败，从而更好地理解和控制智能体工作流。
...
### [272] [Automated Analysis of Global AI Safety Initiatives: A Taxonomy-Driven LLM Approach](https://arxiv.org/abs/2604.03533)
*Takayuki Semitsu,Naoto Kiribuchi,Kengo Zenitani*

Main category: cs.AI

TL;DR: 提出自动化交叉对比框架，使用AI安全活动分类法比较政策文档对，通过LLM提取映射活动并生成摘要、对比和相似度评分，评估不同模型的稳定性和有效性。


<details>
  <summary>Details</summary>
Motivation: 需要系统化比较AI安全政策文档，但人工分析耗时且主观。现有方法缺乏标准化框架，不同模型对同一文档对的评估可能存在差异，需要验证LLM在政策文档分析中的可靠性和一致性。

Method: 基于AI安全活动地图的分类法作为固定维度，系统提取并映射相关活动，为每个维度生成文档摘要、简要对比和相似度评分。使用5个大型语言模型对10个公开文档进行交叉对比分析，通过热图可视化平均相似度得分，并进行人工专家评估验证。

Result: 模型选择显著影响交叉对比结果，某些文档对在不同模型间存在高度分歧。三位专家对两个文档对的人工评估显示高度的一致性，但模型评分与人工判断仍存在差异。热图可视化有效展示了文档间的相似度模式。

Conclusion: 该自动化框架支持政策文档的比较分析，但模型选择对结果有重要影响。虽然LLM能够提供有价值的分析，但与专家判断仍存在差距，需要进一步改进模型选择和验证方法以提高分析可靠性。
...
### [273] [When Do Hallucinations Arise? A Graph Perspective on the Evolution of Path Reuse and Path Compression](https://arxiv.org/abs/2604.03557)
*Xinnan Dai,Kai Yang,Cheng Luo,Shenglai Zeng,Kai Guo,Jiliang Tang*

Main category: cs.AI

TL;DR: 该论文将LLM的推理幻觉建模为图搜索过程，揭示了两种产生机制：路径重用和路径压缩，为理解LLM推理失败提供了统一解释框架。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生流畅但缺乏支持的推理幻觉，这些失败现象广泛存在但其产生机制尚不清楚。研究者希望深入理解仅解码器Transformer如何产生这些推理错误。

Method: 将下一个token预测建模为图搜索过程，其中实体对应节点，学习到的转换形成边。将上下文推理视为对采样子图的约束搜索，将无上下文查询视为对底层图记忆结构的依赖。

Result: 识别出推理幻觉的两种基本机制：1) 路径重用 - 早期训练中记忆知识覆盖上下文约束；2) 路径压缩 - 后期训练中频繁遍历的多步路径坍缩为捷径边。

Conclusion: 路径重用和路径压缩机制为LLM中的推理幻觉提供了统一解释，并与下游应用中观察到的已知行为相联系，有助于深入理解Transformer模型的推理失败机制。
...
### [274] [When Adaptive Rewards Hurt: Causal Probing and the Switching-Stability Dilemma in LLM-Guided LEO Satellite Scheduling](https://arxiv.org/abs/2604.03562)
*Yuanhang Li*

Main category: cs.AI

TL;DR: 研究发现自适应奖励权重在LEO卫星调度中表现不如静态权重，因为PPO算法需要准稳态奖励信号才能收敛。通过因果探测方法揭示了反直觉的奖励项影响，并比较了不同MDP架构变体的性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于直觉：在LEO卫星多波束调度中，基于状态感知的动态奖励权重应该优于静态权重。研究者希望系统性地验证这一直觉，并探索深度强化学习中奖励设计的有效方法。

Method: 1. 引入单变量因果探测方法：独立扰动每个奖励项±20%，测量PPO在50k步后的响应；2. 评估四种MDP架构变体：固定权重、基于规则的权重、学习的MLP权重、微调的LLM权重；3. 在已知和新型流量状态下进行测试。

Result: 1. 静态权重（342.1 Mbps）优于动态权重（103.3±96.8 Mbps），因为PPO需要准稳态奖励信号；2. 因果探测揭示反直觉影响：切换惩罚增加20%分别带来极地切换+157 Mbps和冷热状态+130 Mbps增益；3. MLP在已知状态下达到357.9 Mbps，新型状态下325.2 Mbps；4. 微调LLM崩溃至45.3±43.0 Mbps，原因是权重振荡而非领域知识不足。

Conclusion: 研究为通信系统中LLM-DRL集成提供了经验路线图：识别了LLM不可替代的价值（自然语言意图理解）与简单方法足够适用的场景。输出一致性而非领域知识是主要约束因素。
...
### [275] [Personality Requires Struggle: Three Regimes of the Baldwin Effect in Neuroevolved Chess Agents](https://arxiv.org/abs/2604.03565)
*Diego Armando Resendez Prado*

Main category: cs.AI

TL;DR: 研究发现：Hebbian可塑性在进化过程中会逆转对行为多样性的影响，初期压缩多样性，后期通过想象放大感知差异，形成结构化行为分化，产生可解释的"个性"特征。


<details>
  <summary>Details</summary>
Motivation: 探究终身学习是否会随时间推移扩大行为多样性，而非压缩多样性。传统理论认为可塑性通过缓冲环境噪声来减少变异，本研究在竞争性领域（国际象棋）中检验这一假设。

Method: 使用8个NEAT进化的神经模块构建国际象棋智能体，包含Hebbian游戏内可塑性、期望域信号链和想象能力。在10个种子条件下比较Hebbian开启和关闭状态，分析行为方差随时间的变化。

Result: 发现方差交叉现象：Hebbian开启初期跨种子方差低于关闭状态，但在第34代后反超。行为出现结构化分化：62%的相同局面选择不同走法，形成不同的开局库、棋子偏好和游戏长度。识别出三种演化机制：探索型、彩票型和透明型。

Conclusion: 可塑性对行为多样性的影响在进化过程中发生逆转，通过想象放大感知差异形成反馈循环，产生可复现的行为特征。自我对弈系统可能系统性抑制行为多样性，消除个性形成所需的异质性。
...
### [276] [Selective Forgetting for Large Reasoning Models](https://arxiv.org/abs/2604.03571)
*Tuan Le,Wei Qian,Mengdi Huai*

Main category: cs.AI

TL;DR: 本文提出了一种针对大型推理模型（LRMs）的新型选择性遗忘框架，通过识别和替换推理链中的敏感部分来保护隐私和版权信息，同时保持模型的通用推理能力。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过生成结构化推理链来产生最终答案，这使得中间推理步骤容易泄露敏感信息（如版权内容和隐私数据）。现有的遗忘方法主要针对最终答案，可能会损害模型的整体推理能力，而直接对整个推理链应用遗忘也会降低通用推理能力。因此需要一种既能精确遗忘目标知识，又能保持通用推理能力的方法。

Method: 提出了一种新颖的LRM遗忘框架，利用多个LLM结合检索增强生成（RAG）来分析推理链轨迹，识别需要遗忘的相关片段，并用保持逻辑结构的良性占位符替换这些片段。同时引入了一种新的特征替换遗忘损失函数，能够同时抑制生成遗忘内容的概率，同时强化结构有效的替换。

Result: 在合成数据集和医疗数据集上的大量实验验证了所提方法的期望特性，表明该方法能够有效实现选择性遗忘，同时保持模型的通用推理能力。

Conclusion: 该研究为大型推理模型提供了一种有效的选择性遗忘解决方案，能够在保护敏感信息的同时保持模型的推理能力，解决了现有方法在遗忘过程中损害模型性能的问题。
...
### [277] [Rashomon Memory: Towards Argumentation-Driven Retrieval for Multi-Perspective Agent Memory](https://arxiv.org/abs/2604.03588)
*Albert Sadowski,Jarosław A. Chudziak*

Main category: cs.AI

TL;DR: 提出Rashomon记忆架构，支持并行目标导向智能体根据各自优先级编码经验，并通过论证在查询时协商，允许多重解释共存而非强制统一编码。


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体在长期运作中积累的经验服务于多个并发目标，经常需要对同一事件保持相互冲突的解释。现有记忆架构假设单一正确编码或最多支持统一存储上的多视图，无法处理这种根本性冲突。

Method: 提出Rashomon记忆架构：并行目标导向智能体根据各自优先级编码经验，每个视角维护自己的本体和知识图谱。检索时，各视角提出解释，使用非对称领域知识相互批判，通过Dung论证语义学确定哪些提案保留。生成的攻击图本身即是解释。

Result: 概念验证表明：检索模式（选择、组合、冲突浮现）从攻击图拓扑中自然涌现。冲突浮现模式让系统报告真实分歧而非强制解决，使决策者能直接看到底层的解释冲突。

Conclusion: Rashomon记忆架构通过支持多重解释共存和论证协商，为AI系统处理复杂、冲突的现实世界经验提供了更灵活、透明的记忆机制，特别适用于需要保持多种视角的长期决策场景。
...
### [278] [Entropy and Attention Dynamics in Small Language Models: A Trace-Level Structural Analysis on the TruthfulQA Benchmark](https://arxiv.org/abs/2604.03589)
*Adeyemi Adeseye,Aisvarya Adeseye,Hannu Tenhunen,Jouni Isoaho*

Main category: cs.AI

TL;DR: 该研究通过对1B-1.7B参数的小语言模型进行熵和注意力动态的追踪分析，揭示了模型内部行为如何影响输出真实性，并基于熵模式将模型分为确定性、探索性和平衡性三类。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在边缘设备等资源受限环境中部署日益增多，但这些模型存在自信的错误预测和不稳定输出，对事实性和决策关键任务构成风险。当前评估方法仅依赖最终准确率或幻觉率，缺乏对内部模型行为如何影响输出的解释，特别是解码过程中的熵演化、注意力分布以及隐藏表示如何影响不确定性、逻辑不一致和错误信息传播。

Method: 使用TruthfulQA数据集对四个参数范围在1B-1.7B的小语言模型进行追踪分析，包括：1）令牌级输出熵分析；2）注意力熵分析；3）注意力头分散度分析；4）隐藏状态表示分析。

Result: 基于熵模式将模型分为三类：确定性模型（DeepSeek-1.5B和LLaMA-1B）输出熵随时间递减；探索性模型（Gemma-1B）输出熵随时间递增；平衡模型（Qwen-1.7B）具有中等且稳定的熵。每组模型在隐藏状态移动和注意力分散模式上也有显著差异。

Conclusion: 小语言模型的真实性源于结构化的熵和注意力动态。监控和优化这些内部不确定性模式可以指导设计更可靠、具有幻觉意识且适用于特定应用的边缘小语言模型。
...
### [279] [A Multimodal Foundation Model of Spatial Transcriptomics and Histology for Biological Discovery and Clinical Prediction](https://arxiv.org/abs/2604.03630)
...
TL;DR: STORM是一个整合空间转录组学和组织学图像的基础模型，通过120万个空间转录组图谱和匹配的组织学图像训练，能够从H&E图像预测基因表达，提升空间域发现和临床预测性能。
...
Method: 开发了STORM基础模型，在18个器官的120万个空间解析转录组图谱和匹配的组织学图像上进行训练。采用分层架构整合形态学特征、基因表达和空间上下文信息。

Result: STORM在11种肿瘤类型中从H&E图像预测空间基因表达方面优于现有方法；在23个独立队列（7,245名患者）中显著改善免疫治疗反应预测和预后评估；模型平台无关，在Visium、Xenium、Visium HD和CosMx上表现一致。

Conclusion: STORM提供了一个可扩展的框架，用于空间信息发现和临床精准医学，通过强大的分子-形态学表征桥接成像和组学，增强空间域发现并产生生物学一致的组织图谱。
...
### [280] [Single-agent vs. Multi-agents for Automated Video Analysis of On-Screen Collaborative Learning Behaviors](https://arxiv.org/abs/2604.03631)
...
Method: 提出了两种多智能体框架：1）基于工作流的三智能体系统，通过场景分割和光标提示的VLM推理进行行为检测；2）受ReAct启发的自主决策多智能体系统，通过推理、工具操作和观察驱动的自我修正迭代生成可解释的行为标签。比较了闭源模型（Claude-3.7-Sonnet, GPT-4.1）和开源模型（Qwen2.5-VL-72B）的性能。

Result: 两种提出的多智能体框架都取得了可行性能，在场景和行为检测任务上优于单VLM模型。基于工作流的智能体在场景检测上表现最佳，自主决策多智能体在行为检测上表现最佳。

Conclusion: 基于VLM的多智能体系统在视频分析中具有有效性，为多模态数据分析提供了一个可扩展的框架，能够自动化协作学习环境中的屏幕行为编码。
...
### [281] [TableVision: A Large-Scale Benchmark for Spatially Grounded Reasoning over Complex Hierarchical Tables](https://arxiv.org/abs/2604.03660)
*Xiaoyu Chen,Lu Dai,Hanqing Wang,Zhuoyu Li,Wenbin Dai,Yanzong Zheng,Zhenggang Xia,Junyong Lin,Hui Xiong*

Main category: cs.AI

TL;DR: 论文提出了TableVision基准测试，通过显式空间约束解决MLLMs在复杂表格理解中的感知瓶颈问题，显著提升了推理性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在表格理解方面取得进展，但在处理具有层次布局的复杂表格时，推理性能仍然有限。研究发现存在感知瓶颈：随着任务复杂度增加，涉及的离散视觉区域数量不成比例增长，导致"感知过载"，模型难以在隐式生成过程中保持准确的空间注意力。

Method: 1. 提出TableVision大规模、轨迹感知的基准测试，用于空间基础推理；2. 将表格任务分层为三个认知级别（感知、推理、分析），涵盖13个子类别；3. 使用基于渲染的确定性基础管道，将多步逻辑推理与像素级空间真实值显式耦合；4. 包含6,799个高保真推理轨迹；5. 采用两阶段解耦框架。

Result: 1. 显式空间约束显著恢复了MLLMs的推理潜力；2. 两阶段解耦框架在测试集上实现了12.3%的整体准确率提升；3. 诊断探测支持了实证结果。

Conclusion: TableVision为文档理解中感知与逻辑的协同作用提供了严格的测试平台和新视角，通过解决感知瓶颈有效提升了复杂表格推理性能。
...
### [282] [PRAISE: Prefix-Based Rollout Reuse in Agentic Search Training](https://arxiv.org/abs/2604.03675)
*Erhan Zhang,Yiqun Chen,Zechun Niu,Wei Yang,Xiaochi Wei,Yan Gao,Yi Wu,Yao Hu,Jiaxin Mao*

Main category: cs.AI

TL;DR: PRAISE框架通过重用搜索轨迹前缀和生成中间步骤奖励，解决了智能搜索训练中的数据效率低和奖励稀疏问题，在复杂问答任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前基于搜索的强化学习方法存在两个核心限制：训练中昂贵的长时程轨迹利用不足，以及监督通常只在最终答案处可用，导致严重的奖励稀疏问题。
...
### [283] [Structured Multi-Criteria Evaluation of Large Language Models with Fuzzy Analytic Hierarchy Process and DualJudge](https://arxiv.org/abs/2604.03742)
*Yulong He,Ivan Smirnov,Dmitry Fedrushkov,Sergey Kovalchuk,Ilya Revin*

Main category: cs.AI

TL;DR: 该研究提出了一种基于层次分析法（AHP）和模糊AHP（FAHP）的大语言模型评估方法，通过结构化标准和不确定性建模提升评估可靠性，并进一步提出DualJudge混合框架，结合直觉性和分析性评估范式。


<details>
  <summary>Details</summary>
Motivation: 传统的大语言模型直接评分方法存在不一致性和不透明性，需要更可靠的结构化评估框架来克服这些局限性。

Method: 1）将层次分析法（AHP）应用于LLM评估；2）提出置信感知的模糊AHP（FAHP），通过三角模糊数和LLM生成的置信分数建模认知不确定性；3）提出DualJudge混合框架，通过一致性感知加权自适应融合整体直接评分和结构化AHP输出。

Result: 在JudgeBench上的系统验证表明，AHP和FAHP在不同模型规模和数据集划分上均优于直接评分，FAHP在不确定比较场景中表现出更好的稳定性。DualJudge实现了最先进的性能。

Conclusion: 不确定性感知的结构化推理为更可靠的LLM评估提供了原则性路径，直觉性和深思熟虑性评估范式具有互补优势。
...
### [284] [RL-Driven Sustainable Land-Use Allocation for the Lake Malawi Basin](https://arxiv.org/abs/2604.03768)
*Ying Yao*
...
<details>
  <summary>Details</summary>
Motivation: 生态敏感区域不可持续的土地利用实践威胁着生物多样性、水资源和数百万人的生计，需要科学方法来优化土地利用配置以最大化生态系统服务价值。

Method: 使用深度强化学习框架，基于Sentinel-2影像的9种土地覆盖类型，采用PPO算法和动作掩码技术，在50x50网格（500米分辨率）上优化土地利用分配。奖励函数结合单元生态价值和空间连贯性目标，包括生态连通性奖励和水体缓冲区惩罚。

Result: 智能体有效学习并提高了总生态系统服务价值；空间奖励机制成功引导土地利用形成生态合理的模式，包括同质土地利用集群和水体附近森林的适度整合；框架对政策参数变化响应良好，证明其作为环境规划情景分析工具的有效性。

Conclusion: 该深度强化学习框架能够有效优化土地利用配置以最大化生态系统服务价值，并通过空间奖励机制实现生态友好的土地利用模式，为环境规划提供了有力的情景分析工具。
...
### [285] [Affording Process Auditability with QualAnalyzer: An Atomistic LLM Analysis Tool for Qualitative Research](https://arxiv.org/abs/2604.03820)
...
<details>
  <summary>Details</summary>
Motivation: 当前许多使用大语言模型进行定性数据分析的工作流程模糊了分析结论的产生过程，缺乏透明度和可审计性。

Method: 开发QualAnalyzer开源Chrome扩展，支持原子化LLM分析，独立处理每个数据段，为每个单元保留提示、输入和输出，创建可审计的追踪记录。
...
### [286] [FeynmanBench: Benchmarking Multimodal LLMs on Diagrammatic Physics Reasoning](https://arxiv.org/abs/2604.03893)
*Zeyu Wang,Xiaogang Li,Peiyao Xiao,Qinhao Kong,Ben Wang,Chengliang Xu,Zichao Chen,Bing Zhao,Hu Wei*

Main category: cs.AI

TL;DR: FeynmanBench：首个专注于费曼图任务的基准测试，评估AI在多步图示推理、守恒定律约束、拓扑识别和代数表示转换等方面的能力，揭示当前MLLMs在物理约束执行和全局拓扑条件遵守方面的系统性失败。


<details>
  <summary>Details</summary>
Motivation: 前沿理论突破通常依赖于具体图示符号与严谨逻辑的结合。当前多模态大语言模型（MLLMs）在科学任务中表现出潜力，但现有基准测试往往关注局部信息提取，而非形式化科学符号中固有的全局结构逻辑。需要建立基于物理基础的视觉推理基准来评估AI在科学发现中的能力。

Method: 开发了FeynmanBench基准测试，包含自动化流水线生成多样化的费曼图，并提供可验证的拓扑标注和振幅结果。数据库涵盖标准模型的电磁、弱和强相互作用，包含100多种不同类型和2000多个任务。通过实验评估最先进的MLLMs在费曼图任务上的表现。

Result: 实验显示最先进的MLLMs存在系统性失败模式，包括物理约束执行不稳定和全局拓扑条件违反。这表明当前AI在满足守恒定律、对称性约束、识别图拓扑、在图示和代数表示之间转换以及构建特定规范和规范下的散射振幅等方面存在显著不足。

Conclusion: FeynmanBench为评估AI是否能够有效参与科学发现（特别是在理论物理学领域）提供了逻辑严谨的测试。该基准揭示了当前MLLMs在科学图示推理方面的局限性，强调了需要基于物理基础的基准来推动AI在科学发现中的进步。
...
### [287] [FactReview: Evidence-Grounded Reviews with Literature Positioning and Execution-Based Claim Verification](https://arxiv.org/abs/2604.04074)
*Hang Xu,Ling Yue,Chaoqian Ouyang,Libin Zheng,Shaowu Pan,Shimin Di,Min-Ling Zhang*

Main category: cs.AI

TL;DR: FactReview是一个基于证据的机器学习论文评审系统，通过提取论文主张、文献定位和代码执行验证来生成有证据支撑的评审意见，帮助解决评审时间有限和传统LLM评审系统依赖论文叙述的问题。


<details>
  <summary>Details</summary>
Motivation: 机器学习领域的同行评审面临提交量激增和评审时间有限的压力。现有基于LLM的评审系统仅阅读论文本身，容易受论文表述质量影响，且在需要相关文献或代码证据时表现不佳。

Method: FactReview采用证据驱动的评审方法：1) 提取论文主要主张和报告结果；2) 检索相关文献以明确论文技术定位；3) 当有可用代码时，在有限预算下执行代码库以验证核心经验主张；4) 生成简洁评审和证据报告，为每个主要主张分配五个标签之一。
...
Conclusion: AI在同行评审中最有用的角色不是最终决策者，而是作为收集证据的工具，帮助评审者做出更有证据基础的评估。FactReview展示了证据驱动评审系统的潜力，代码已公开。
...
### [288] [Compliance-by-Construction Argument Graphs: Using Generative AI to Produce Evidence-Linked Formal Arguments for Certification-Grade Accountability](https://arxiv.org/abs/2604.04103)
*Mahyar T. Moghaddam*

Main category: cs.AI

TL;DR: 提出一种将生成式AI与结构化形式论证相结合的合规性架构，通过验证约束确保AI生成的决策支持内容具有可追溯性和可审计性。


<details>
  <summary>Details</summary>
Motivation: 高风险的决策系统需要结构化论证、可追溯性和可审计性以确保问责和合规。当前生成式AI在决策支持中的部署存在幻觉推理、无支持主张和弱可追溯性等风险。

Method: 提出合规性构建架构，包含四个组件：1) 基于保证案例方法的类型化论证图表示；2) 基于检索增强生成(RAG)的权威证据论证片段草拟；3) 强制执行完整性和可接受性约束的推理验证内核；4) 符合W3C PROV标准的来源账本支持可审计性。

Result: 分析表明，确定性验证规则可以防止无支持的主张进入决策记录，同时允许生成式AI加速论证构建。系统设计和基于可执行不变量的评估策略展示了可行性。

Conclusion: 该架构通过将生成式AI与结构化形式论证集成，实现了AI辅助决策支持的可验证性、可追溯性和合规性，为高风险决策系统提供了可靠的解决方案。
...
### [289] [InsTraj: Instructing Diffusion Models with Travel Intentions to Generate Real-world Trajectories](https://arxiv.org/abs/2604.04106)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏对复杂用户出行意图的深度语义理解，难以在保持人类行为多样性的同时处理复杂约束，这限制了GPS轨迹生成在城巿规划、移动模拟和隐私保护数据共享等应用中的效果。

Method: InsTraj采用两阶段方法：首先使用大型语言模型解析自然语言描述的出行意图，生成丰富的语义蓝图；然后提出多模态轨迹扩散变换器，整合语义指导生成高保真且忠实于用户意图的轨迹。

Result: 在真实世界数据集上的综合实验表明，InsTraj在生成真实性、多样性和语义忠实性方面显著优于现有最先进方法。

Conclusion: InsTraj通过结合大型语言模型的语义理解能力和扩散模型的生成能力，成功解决了轨迹生成中的语义理解和约束处理难题，为相关应用提供了有效的解决方案。
...
### [290] [Readable Minds: Emergent Theory-of-Mind-Like Behavior in LLM Poker Agents](https://arxiv.org/abs/2604.04157)
*Hsieh-Ting Lin,Tsung-Yu Hou*

Main category: cs.AI

TL;DR: 研究发现，配备持久记忆的LLM智能体在德州扑克游戏中能逐步发展出类似心理理论的能力，而记忆是这种能力出现的必要且充分条件。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型是否能在动态交互中发展心理理论能力，而不仅仅是通过静态场景测试。

Method: 采用2x2因子设计（记忆有/无 × 领域知识有/无），共20个实验，约6000个智能体手牌观察，让自主LLM智能体玩德州扑克。

Result: 记忆是心理理论行为出现的必要且充分条件；有记忆的智能体达到心理理论3-5级，无记忆的智能体保持在0级；有记忆的智能体表现出基于对手模型的策略性欺骗。

Conclusion: 功能性心理理论行为可以从交互动态中自发涌现，无需显式训练或提示，这对理解人工智能社会认知和生物社会认知具有重要意义。
...
### [291] [A Model of Understanding in Deep Learning Systems](https://arxiv.org/abs/2604.04171)
*David Peter Wallis Freeborn*

Main category: cs.AI

TL;DR: 作者提出了一种适用于机器学习系统的系统性理解模型，认为当智能体包含能够追踪真实规律、通过稳定桥梁原则与目标系统耦合、并支持可靠预测的充分内部模型时，就实现了理解。作者认为当代深度学习系统通常能够达到这种理解，但距离科学理解的理想标准还有差距。


<details>
  <summary>Details</summary>
Motivation: 本文旨在为机器学习系统建立一个系统性的理解模型，探讨深度学习系统是否真正实现了对目标系统的理解，以及这种理解与科学理解标准之间的差距。

Method: 作者提出了一个理解模型，包含三个关键要素：1）能够追踪真实规律的充分内部模型；2）通过稳定桥梁原则与目标系统耦合；3）支持可靠的预测。作者将这个模型应用于分析当代深度学习系统。

Result: 作者认为当代深度学习系统通常能够达到所提出的理解标准，但它们存在三个主要缺陷：1）符号与目标系统不对齐；2）缺乏明确的还原性；3）统一性较弱。作者将这种现象称为"断裂理解假说"。

Conclusion: 深度学习系统确实能够实现对目标系统的某种理解，但这种理解与理想的科学理解存在差距，表现为符号不对齐、缺乏还原性和弱统一性，形成了"断裂理解"。
...
### [292] [CoALFake: Collaborative Active Learning with Human-LLM Co-Annotation for Cross-Domain Fake News Detection](https://arxiv.org/abs/2604.04174)
*Esma Aïmeur,Gilles Brassard,Dorsaf Sallami*

Main category: cs.AI

TL;DR: CoALFake：一种结合人类-大语言模型协同标注与领域感知主动学习的跨领域假新闻检测新方法，解决了现有方法对标注数据依赖和领域信息损失的问题。


<details>
  <summary>Details</summary>
Motivation: 当前假新闻检测系统存在领域特异性强、泛化能力差的问题。现有跨领域方法面临两大挑战：1) 依赖标注数据（获取成本高且经常不可得）；2) 刚性领域分类或忽视领域特定特征导致信息损失。
...
Result: 在多个数据集上的实验结果表明，该方法始终优于各种基线方法。人类-LLM协同标注被证明是高度成本效益的方法，即使在最小的人类监督下，CoALFake也能持续优于现有基线。

Conclusion: CoALFake通过整合人类-LLM协同标注与领域感知主动学习，有效解决了跨领域假新闻检测中的标注数据依赖和领域信息损失问题，实现了高性能且成本效益高的解决方案。
...
### [293] [Comparative reversal learning reveals rigid adaptation in LLMs under non-stationary uncertainty](https://arxiv.org/abs/2604.04182)
*Haomiaomiao Wang,Tomás E Ward,Lili Zhang*

Main category: cs.AI

TL;DR: LLMs在概率反转学习任务中表现出不对称的奖励/惩罚敏感性：赢则保持策略接近完美，输则转换策略显著减弱。不同模型适应能力差异明显，随机转换条件加剧了反转后的持续行为，但总收益不一定降低。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在非平稳环境中的决策能力，特别是在概率反转学习任务中的表现，以评估它们如何应对环境变化和不确定性。

Method: 使用两种选项的概率反转学习任务，包含三个潜在状态和基于性能标准或超时的切换事件。比较确定性固定转换周期和随机转换计划，评估DeepSeek-V3.2、Gemini-3和GPT-5.2，以人类数据作为行为参考。

Result: 所有模型都表现出赢则保持策略接近完美，但输则转换策略显著减弱的不对称性。DeepSeek-V3.2在反转后表现出极端持续性且学习能力弱，而Gemini-3和GPT-5.2适应更快但仍比人类对损失更不敏感。随机转换加剧了反转后的持续性，但并未一致降低总收益。

Conclusion: LLMs在非平稳环境中表现出刚性适应模式，刚性可能源于损失学习能力弱、策略确定性过高或通过反事实抑制导致的价值极化。这些结果支持开发反转敏感性诊断和波动感知模型来评估LLMs在非平稳不确定性下的表现。
...
### [294] [Don't Blink: Evidence Collapse during Multimodal Reasoning](https://arxiv.org/abs/2604.04207)
*Suresh Raghu,Satwik Pandey*

Main category: cs.AI

TL;DR: 研究发现推理视觉语言模型在思考过程中会逐渐失去视觉基础，导致低熵预测虽然自信但缺乏视觉依据，这种证据崩溃现象在多个基准测试中普遍存在。


<details>
  <summary>Details</summary>
Motivation: 推理视觉语言模型在推理过程中可能变得准确但逐渐失去视觉基础，形成任务条件性危险区域，其中低熵预测虽然自信但缺乏视觉依据，这种失败模式无法通过纯文本监控检测。

Method: 在MathVista、HallusionBench和MMMU_Pro三个基准上评估三种推理视觉语言模型，分析证据崩溃现象；使用全响应熵作为文本不确定性信号；构建熵-视觉交互模型识别任务条件性机制。

Result: 发现普遍的证据崩溃现象：推理过程中对标注证据区域的注意力显著下降，常损失超过一半的证据质量；全响应熵是跨数据集转移中最可靠的文本不确定性信号；低熵、视觉脱离的预测在持续视觉参考任务中危险但在符号任务中良性。

Conclusion: 通过有针对性的视觉否决机制可以减少选择性风险，支持任务感知的多模态监控以实现安全部署，特别是在分布转移情况下。
...
### [295] [Combee: Scaling Prompt Learning for Self-Improving Language Model Agents](https://arxiv.org/abs/2604.04247)
...
<details>
  <summary>Details</summary>
Motivation: 现有提示学习方法主要针对单智能体或低并行度设置，无法高效利用大量收集的智能体轨迹。随着从多个智能体轨迹或并行执行中学习的趋势增长，需要一种可扩展的并行提示学习策略来避免质量下降。

Method: Combee框架采用并行扫描技术，结合增强洗牌机制，并引入动态批量大小控制器来平衡质量和延迟，实现高效并行提示学习。

Result: 在AppWorld、Terminal-Bench、Formula和FiNER等基准测试中，Combee相比之前方法实现了高达17倍的加速，同时保持相当或更好的准确性和同等成本。

Conclusion: Combee成功解决了大规模并行提示学习的扩展问题，在保持学习质量的同时显著提升了效率，为自改进智能体系统提供了有效的并行学习框架。
...
### [296] [MC-CPO: Mastery-Conditioned Constrained Policy Optimization](https://arxiv.org/abs/2604.04251)
*Oluseyi Olukola,Nick Rahimi*

Main category: cs.AI

TL;DR: 论文提出MC-CPO算法，通过将教学结构嵌入可行动作空间来缓解强化学习教学系统中的奖励黑客问题


<details>
  <summary>Details</summary>
Motivation: 当前基于参与度优化的自适应教学系统可能优先考虑短期行为信号而非持续学习成果，导致强化学习策略中出现奖励黑客问题。需要将教学安全约束动态整合到系统中。

Method: 提出Mastery-Conditioned Constrained Policy Optimization (MC-CPO)算法，这是一个两时间尺度的原始对偶算法，将结构化动作屏蔽与约束策略优化相结合。将问题形式化为具有掌握条件可行性的约束马尔可夫决策过程(CMDP)。

Result: 在表格环境和神经教学设置中的实证验证表明，MC-CPO在约束预算内满足要求，相对于无约束和奖励塑形基线降低了折扣安全成本，并显著降低了奖励黑客严重指数(RHSI)。

Conclusion: 将教学结构直接嵌入可行动作空间为缓解教学强化学习系统中的奖励黑客问题提供了原则性基础，优化掌握条件可行集内的策略可以严格优于后处理过滤方法。
...
### [297] [Context Engineering: A Practitioner Methodology for Structured Human-AI Collaboration](https://arxiv.org/abs/2604.04258)
*Elias Calboreanu*

Main category: cs.AI

TL;DR: 本文提出"上下文工程"方法，通过结构化上下文信息（包含五个角色）和四阶段流程，显著提升AI输出质量，减少迭代次数，提高首次通过率。


<details>
  <summary>Details</summary>
Motivation: 传统上AI输出质量常归因于提示技巧，但实证观察表明上下文完整性对输出质量影响更大。现有方法缺乏系统化的上下文管理框架。

Method: 提出上下文工程方法：1）定义包含五个角色的上下文包结构（权威、范例、约束、评分标准、元数据）；2）采用四阶段管道（评审员→设计→构建者→审计员）；3）应用可靠性工程和信息论模型评估上下文质量。

Result: 在四个AI工具的200次交互观察中：72%的迭代周期与不完整上下文相关；结构化上下文使平均迭代周期从3.8降至2.0；首次通过率从32%提升至55%；结构化交互中110/200首次通过，基线仅16/50；最终成功率91.5%（183/200）。

Conclusion: 上下文工程提供了一种系统化方法来提高AI交互效率和质量，通过结构化上下文组装显著减少迭代需求。该方法在配套生产自动化系统中得到初步验证（11个操作通道，2132个分类工单）。
...
### [298] [Beyond Fluency: Toward Reliable Trajectories in Agentic IR](https://arxiv.org/abs/2604.04269)
*Anushree Sinha,Srivaths Ranganathan,Debanshu Das,Abhishek Dharmaratnakar*

Main category: cs.AI

TL;DR: 论文提出在信息检索从被动文档排序转向自主智能体工作流的背景下，需要关注早期错误在多步推理-行动-观察循环中的级联效应，并提出了通过验证门和系统弃权来确保轨迹完整性的解决方案。


<details>
  <summary>Details</summary>
Motivation: 信息检索正在从被动文档排序转向自主智能体工作流，在多步推理-行动-观察循环中，早期微小错误会级联放大，导致内部推理与外部工具执行之间的功能错位，尽管语言流畅性仍然保持。需要超越端点准确性，关注轨迹完整性和因果归因。

Method: 提出在智能体系统的每个交互单元设置验证门，并在校准的不确定性下进行系统弃权。通过监控规划、检索、推理和执行各阶段的错误模式，确保过程正确性和基于事实的执行。

Result: 论文综合了工业智能体系统中观察到的失败模式，将错误分类为规划、检索、推理和执行四类。提出了验证门和系统弃权机制来应对复合错误和欺骗性流畅问题。

Conclusion: 可靠的智能体信息检索系统必须优先考虑过程正确性和基于事实的执行，而不是看似合理但未经验证的完成。需要从端点准确性转向轨迹完整性，通过验证机制确保智能体工作流的安全部署。
...
### [299] [PanLUNA: An Efficient and Robust Query-Unified Multimodal Model for Edge Biosignal Intelligence](https://arxiv.org/abs/2604.04297)
*Marija Zelic,Anna Tegon,Yawei Li,Thorir Mar Ingolfsson,Luca Benini*

Main category: cs.AI

TL;DR: PanLUNA是一个紧凑的5.4M参数全模态基础模型，能够联合处理EEG、ECG和PPG信号，通过通道统一模块实现跨模态早期融合，在保持小模型规模的同时达到或超越更大模型的性能。


<details>
  <summary>Details</summary>
Motivation: 当前生理基础模型大多局限于单一模态（如EEG、ECG或PPG），主要是因为配对的多模态数据集稀缺。需要开发能够处理多种生理信号的统一模型。

Method: 扩展LUNA的通道统一模块，将多模态通道视为统一查询集中的条目，并添加传感器类型嵌入，实现高效的跨模态早期融合，同时保持对推理时缺失模态的鲁棒性。采用量化感知训练和INT8权重。

Result: PanLUNA在TUAB异常EEG检测上达到81.21%平衡准确率，在HMC多模态睡眠分期上达到0.7416平衡准确率（SOTA）。量化后恢复≥96%全精度性能，在GAP9超低功耗RISC-V微控制器上部署，ECG推理延迟325.6ms/18.8mJ，多模态睡眠分期延迟1.206s/68.65mJ。

Conclusion: PanLUNA展示了紧凑的全模态基础模型在联合处理多种生理信号方面的有效性，实现了高性能、低功耗的部署，为可穿戴设备上的实时生理监测提供了可行方案。
...
### [300] [RESCORE: LLM-Driven Simulation Recovery in Control Systems Research Papers](https://arxiv.org/abs/2604.04324)
*Vineet Bhat,Shiqing Wei,Ali Umut Kaypak,Prashanth Krishnamurthy,Ramesh Karri,Farshad Khorrami*

Main category: cs.AI

TL;DR: 论文提出RESCORE框架，通过LLM智能体自动从控制研究论文中生成可执行代码以重现仿真结果，在CDC会议500篇论文的基准测试中成功恢复了40.7%的实例，比单次生成方法表现更好。


<details>
  <summary>Details</summary>
Motivation: 控制研究论文中经常存在参数定义不明确和实现细节模糊的问题，这阻碍了数值仿真的重建和验证。需要自动化系统来生成可执行代码以重现论文结果，从而减少手动复制的时间和精力。

Method: 提出RESCORE框架，包含三个LLM智能体组件：分析器（Analyzer）、编码器（Coder）和验证器（Verifier）。通过迭代执行反馈和视觉比较来提高重建保真度。

Result: 在IEEE决策与控制会议（CDC）的500篇论文基准测试中，RESCORE成功恢复了40.7%的任务相干仿真，优于单次生成方法。自动化流程估计比人工复制快10倍。

Conclusion: RESCORE框架能够有效自动化研究复现过程，显著减少验证已发表控制方法所需的时间和精力。将发布基准测试和智能体以促进社区在自动化研究复制方面的进展。
...
### [301] [Soft Tournament Equilibrium](https://arxiv.org/abs/2604.04328)
*Saad Alqithami*

Main category: cs.AI

TL;DR: 论文提出Soft Tournament Equilibrium (STE)框架，用于评估存在循环优势关系的智能体，用集合值核心替代传统线性排名，通过可微分算子计算Top Cycle和Uncovered Set的连续版本。


<details>
  <summary>Details</summary>
Motivation: 传统排名方法在处理具有非传递性（循环优势）关系的智能体时存在误导性和不稳定性，需要更合适的评估理论基础。

Method: 提出STE框架：1）学习概率化锦标赛模型；2）使用可微分的软可达性和软覆盖算子计算Top Cycle和Uncovered Set的连续版本；3）输出带校准成员分数的核心智能体集合。

Result: STE在零温度极限下与经典解一致，具有Condorcet包含性质，具有良好的稳定性和样本复杂度，为智能体评估提供了更稳健的理论基础。

Conclusion: 该工作将通用智能体评估从不稳定排名转向稳定的集合值均衡，提供了更合适和稳健的理论框架。
...
### [302] [Domain-Contextualized Inference: A Computable Graph Architecture for Explicit-Domain Reasoning](https://arxiv.org/abs/2604.04344)
...
<details>
  <summary>Details</summary>
Motivation: 现有推理系统通常与特定计算基板（符号、神经、向量等）紧密耦合，缺乏统一的架构来处理多领域、多基板的推理任务，导致推理过程不透明且效率受限

Method: 建立五层架构，包含三种领域计算模式（链索引、路径遍历作为Kleisli组合、向量引导计算作为基板转换），定义基板无关接口（查询、扩展、桥接），提出可靠性条件C1-C4和三种故障模式分类

Result: 实现领域范围剪枝将每查询搜索空间从O(N)减少到O(N/K)，支持在符号、神经、向量和混合基板上执行，每个推理步骤都携带评估上下文，通过PHQ-9临床推理案例验证

Conclusion: 贡献是架构性的而非逻辑性的，提出了完整的计算理论，包括操作语义、复杂度边界、单子结构、基板转换和边界条件，为多基板推理系统提供了统一框架
...
### [303] [RoboPhD: Evolving Diverse Complex Agents Under Tight Evaluation Budgets](https://arxiv.org/abs/2604.04347)
...
Method: RoboPhD采用无验证进化方法：不将预算分割为训练和验证，而是使用Elo竞赛在训练数据上同时评估智能体并驱动进化。所有三种系统都接收带有诊断print()语句的种子智能体，使进化能够发展出越来越丰富的信息诊断。

Result: 在四个基准测试（抽象推理、云调度、SQL生成、金融QA）中，RoboPhD在三个任务上优于GEPA和Autoresearch，仅在最简单的任务上落后。在ARC-AGI任务中，RoboPhD将22行种子智能体进化为1013行的多策略系统，准确率从27.8%提升至65.8%。

Conclusion: RoboPhD的Elo锦标赛选择方法在多数复杂任务中表现最优，特别是在评估成本高昂的场景下。作者将RoboPhD作为MIT许可下的多功能工具包发布，提供简单的optimize_anything() API用于进化各种复杂智能体。
...
### [304] [REAM: Merging Improves Pruning of Experts in LLMs](https://arxiv.org/abs/2604.04356)
...
<details>
  <summary>Details</summary>
Motivation: MoE大语言模型参数量巨大（数百亿参数），部署时面临显著的内存挑战。传统的内存减少方法包括权重剪枝和量化，但专家剪枝方法（如REAP）会删除专家，可能影响模型性能。

Method: 提出Router-weighted Expert Activation Merging (REAM)方法，不删除专家而是对专家进行分组并合并其权重，更好地保留原始模型性能。

Result: 在多个MoE LLM模型上评估REAM，发现在多项选择题（MC）和生成式（GEN）基准测试中存在性能权衡，这种权衡取决于校准数据的混合比例。通过控制通用、数学和编码数据的混合，REAM通常优于基线方法，在许多情况下与原始未压缩模型相当。

Conclusion: REAM作为一种新的MoE LLM压缩方法，通过合并而非删除专家权重，在保持模型性能方面优于传统的专家剪枝方法，为大型MoE模型的部署提供了有效的内存优化方案。
...
### [305] [Optimizing Service Operations via LLM-Powered Multi-Agent Simulation](https://arxiv.org/abs/2604.04383)
*Yanyuan Wang,Xiaowei Zhang*

Main category: cs.AI

TL;DR: 论文提出LLM驱动的多智能体仿真框架(LLM-MAS)，用于优化服务运营，通过将设计选择嵌入提示词来塑造LLM智能体交互结果的分布，并开发了在线轨迹学习算法优化稳态性能。
...
Method: 提出LLM-MAS框架，将问题表述为具有决策依赖不确定性的随机优化问题。通过将关键数值信息嵌入提示词并从LLM生成的文本中提取，将不确定性建模为受控马尔可夫链。开发了在线轨迹学习算法，在单次仿真运行中同时构建零阶梯度估计并更新设计参数以优化稳态性能，并融入了方差减少技术。

Result: 在可持续供应链应用中，该方法优于基准方法，包括黑盒优化以及使用LLM作为数值求解器或角色扮演系统设计者。在基于真实行为数据的最优竞赛设计案例研究中，LLM-MAS既可作为已知设计的成本效益评估工具，也可作为探索性工具，发现传统方法忽视的优秀设计。

Conclusion: LLM-MAS框架为服务运营优化提供了一种有效方法，能够模拟复杂的人类行为响应，并通过在线学习算法优化设计参数，在实际应用中表现出优于传统方法的性能。
...
### [306] [Automatically Generating Hard Math Problems from Hypothesis-Driven Error Analysis](https://arxiv.org/abs/2604.04386)
*Jiayu Fu,Mourad Heddaya,Chenhao Tan*

Main category: cs.AI

TL;DR: 提出一种基于AI生成假设的数学基准测试生成管道，能自动识别LLM在特定数学概念和技能上的弱点，并针对性生成测试问题


<details>
  <summary>Details</summary>
Motivation: 现有数学基准测试大多依赖人工制作，难以规模化，无法跟上LLM发展速度，且不能有效识别LLM在具体数学概念和技能上的弱点

Method: 使用AI生成的假设来识别LLM容易出错的特定数学概念和技能，然后针对这些弱点生成新的基准测试问题

Result: 假设准确性与生成问题的难度正相关：基于最准确假设生成的问题将Llama-3.3-70B-Instruct的准确率降至45%（原MATH基准为77%），且管道具有高度适应性

Conclusion: 该管道不仅能有效评估LLM数学能力，还可扩展到其他领域，成为探索LLM在不同领域表现的有价值工具
...
### [307] [MolDA: Molecular Understanding and Generation via Large Language Diffusion Model](https://arxiv.org/abs/2604.04403)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于自回归主干的多模态分子架构存在局限性：严格的从左到右归纳偏差不适用于生成化学有效分子，难以处理非局部全局约束（如环闭合），且在序列生成过程中容易累积结构错误。
...
Conclusion: MolDA通过创新的掩码扩散方法克服了传统自回归模型的局限性，为分子发现提供了更有效的多模态框架，能够更好地处理分子结构的全局约束和化学有效性要求。
...
TL;DR: 提出一种评估模型解释一致性的新指标，通过计算相同标签输入的SHAP值余弦相似度来检测模型解释的稳定性，确保模型在标签保持扰动下保持一致性。


<details>
  <summary>Details</summary>
Motivation: 当前可解释AI评估大多停留在实例层面，未能量化模型解释在不同相似输入间的稳定性。可靠的模式识别系统应在相似输入上表现一致，其解释也应保持稳定。

Method: 使用预训练的BERT模型在SST-2情感分析数据集上实现新指标，并在RoBERTa、DistilBERT和IMDB数据集上进行鲁棒性测试。应用SHAP计算特征重要性，通过量化相同标签输入的SHAP值余弦相似度来评估解释一致性。

Result: 通过一系列实验评估该指标识别错误预测和模型解释不一致性的能力，并与标准保真度指标进行比较，验证新指标能有效检测模型行为偏离预期目标的情况。

Conclusion: 该框架通过更稳健地验证推理稳定性，为构建可信AI系统提供了对模型行为的更深入理解。通过量化模型对相似输入是否依赖一致的归因模式，支持在实际模式识别流程中进行更稳健的模型行为评估。
...
TL;DR: 该论文识别出现有多模态AI架构中的结构性拓扑限制，提出基于中国工艺认识论的"象"概念和十字框架，通过哲学、认知科学和数学三支柱重新构建多模态融合理论。


<details>
  <summary>Details</summary>
Motivation: 当前多模态AI架构（如CLIP、GPT-4V、Gemini）存在模态可分性的拓扑限制，作者认为这是结构性问题而非参数问题。需要从哲学、认知科学和数学三个层面重新思考多模态融合的基础理论。

Method: 提出基于中国工艺认识论的"象"概念作为说与显之间的第三状态，建立十字框架（道/器×说/显）。从哲学、认知科学（DMN/ECN/SN三系统激活）、数学（纤维丛和杨-米尔斯曲率）三支柱构建理论。提出UOO实现、ANALOGY-MM基准、META-TOP三层基准等具体方法。

Result: 构建了完整的理论框架，将维特根斯坦的说/显区分重新解释为问题而非结论，通过中国工艺认识论的"象"概念提供解决方案。提出了具体的实现方法和评估基准，包括分阶段实验路线图和明确的终止标准。

Conclusion: 多模态AI需要超越当前基于模态可分性的拓扑限制，通过跨文明、跨学科的理论重构，特别是引入中国工艺认识论的"象"概念，可以建立更本质的多模态融合理论框架，为下一代AI架构提供新的理论基础。
...
TL;DR: RetailSim是一个端到端的零售模拟框架，通过统一环境建模零售流程的多个阶段，包括卖家说服、买卖互动和购买决策，解决了现有模拟器无法捕捉跨阶段依赖关系的问题。
...
Method: 开发了RetailSim端到端零售模拟框架，在统一环境中建模整个零售流程，通过多样化产品空间、角色驱动的智能体和多轮互动实现模拟保真度。

Result: 通过双重评估协议（人类行为保真度评估和针对现实经济规律的元评估）显示，RetailSim成功复现了关键模式，包括人口统计购买行为、价格-需求关系和异质价格弹性。

Conclusion: RetailSim展示了作为探索零售策略的受控测试平台的潜力，通过角色推断、买卖互动分析和销售策略评估等决策导向的用例证明了其实用价值。
...
### [311] [On the "Causality" Step in Policy Gradient Derivations: A Pedagogical Reconciliation of Full Return and Reward-to-Go](https://arxiv.org/abs/2604.04686)
*Nima H. Siboni*

Main category: cs.AI

TL;DR: 论文对策略梯度中REINFORCE估计器的推导进行了澄清，通过前缀轨迹分布和得分函数恒等式，严格证明了为什么可以用"奖励到目标"替代完整回报，而不需要依赖启发式的"因果性"原则。


<details>
  <summary>Details</summary>
Motivation: 在策略梯度的入门介绍中，通常使用完整轨迹回报推导REINFORCE估计器，然后基于"因果性"原则说明完整回报可以被"奖励到目标"替代。虽然这种说法正确，但推导的严谨性不足，未能清晰说明过去奖励项如何消失。本文旨在提供更严格的数学推导。

Method: 采用前缀轨迹分布和得分函数恒等式的方法，将目标函数分解为前缀轨迹的形式。通过这种分解，直接推导出"奖励到目标"项的出现，而不需要依赖启发式的因果性论证。

Result: 推导出的估计器与传统的REINFORCE估计器相同，但提供了更清晰的概念框架。在分解后的目标函数中，"奖励到目标"项自然出现，而过去奖励项在数学推导中自动消失。

Conclusion: 本文的主要贡献是概念性的：通过前缀轨迹分解的方法，展示了"奖励到目标"项是目标函数分解的自然结果，而不是事后无偏替代的启发式选择。传统的因果性论证成为推导的推论，而非额外的启发式原则。
...
### [312] [AI Assistance Reduces Persistence and Hurts Independent Performance](https://arxiv.org/abs/2604.04721)
*Grace Liu,Brian Christian,Tsvetomira Dumbalska,Michiel A. Bakker,Rachit Dubey*

Main category: cs.AI

TL;DR: AI助手虽然短期提升表现，但会降低用户坚持性和无协助时的表现，影响长期学习能力


<details>
  <summary>Details</summary>
Motivation: 当前AI系统专注于提供即时完整答案，而人类导师会搭建学习框架、跟踪进度、优先考虑对方成长。研究旨在探究AI协助对用户坚持性和长期能力发展的影响。

Method: 通过随机对照试验（N=1,222）研究人机交互，涵盖数学推理和阅读理解等多种任务，测量AI协助对用户表现和坚持性的影响。

Result: AI协助短期内提升表现，但导致用户在没有AI协助时表现显著变差，更容易放弃。这些效应在仅约10分钟的AI交互后就会出现。

Conclusion: AI协助会降低用户的坚持性，影响长期技能获取。AI模型开发需要优先考虑搭建长期能力框架，而不仅仅是完成即时任务。
...
### [313] [ANX: Protocol-First Design for AI Agent Interaction with a Supporting 3EX Decoupled Architecture](https://arxiv.org/abs/2604.04820)
*Xu Mingze*

Main category: cs.AI

TL;DR: ANX是一个开放、可扩展、可验证的智能体原生协议和顶层框架，通过协议创新、架构优化和工具补充解决现有GUI自动化和MCP技能方法的缺陷，显著减少token消耗和执行时间。


<details>
  <summary>Details</summary>
Motivation: 现有AI智能体方法存在高token消耗、交互碎片化、安全性不足等问题，缺乏统一的顶层框架和关键组件，各独立模块存在缺陷。

Method: 提出ANX协议和框架，包含四个核心创新：1) 智能体原生设计（ANX配置、标记语言、CLI）；2) 人机交互结合技能灵活性；3) MCP支持的按需轻量级应用；4) ANX标记语言支持的可执行SOP。采用3EX解耦架构和ANXHub。

Result: 实验显示ANX显著减少token消耗：相比MCP技能减少47.3%（Qwen3.5-plus）和55.6%（GPT-4o），相比GUI自动化减少57.1%（Qwen3.5-plus）和66.3%（GPT-4o）。执行时间缩短58.1%和57.7%。

Conclusion: ANX通过协议创新解决了现有智能体方法的缺陷，提供了开放、可扩展、可验证的智能体原生协议框架，显著提升了效率和安全性。
...
### [314] [Incompleteness of AI Safety Verification via Kolmogorov Complexity](https://arxiv.org/abs/2604.04876)
*Munawar Hasan*

Main category: cs.AI

TL;DR: 该论文证明AI安全验证存在根本性信息理论限制：任何固定的可计算枚举验证器都无法证明所有超过特定复杂度的政策合规实例


<details>
  <summary>Details</summary>
Motivation: 确保AI系统满足形式化安全和政策约束是安全关键领域的核心挑战。虽然验证的局限性通常归因于组合复杂性和模型表达能力，但本文旨在揭示其源于内在的信息理论限制

Method: 将政策合规性形式化为编码系统行为的验证问题，使用柯尔莫哥洛夫复杂度进行分析，证明不完备性结果：对于任何固定的可计算枚举验证器，存在一个阈值，超过该阈值后，复杂度超过该阈值的真正政策合规实例无法被证明

Result: 证明了AI安全验证的根本性限制：任何有限的形式化验证器都无法证明所有任意高复杂度的政策合规实例，这一限制独立于计算资源

Conclusion: 揭示了AI安全验证的内在信息理论限制，与计算复杂性无关，这为采用携带证明的方法提供了动机，该方法可以提供实例级别的正确性保证
...
### [315] [Learning, Potential, and Retention: An Approach for Evaluating Adaptive AI-Enabled Medical Devices](https://arxiv.org/abs/2604.04878)
*Alexis Burgon,Berkman Sahiner,Nicholas A Petrick,Gene Pennello,Ravi K Samala*

Main category: cs.AI

TL;DR: 该研究提出了一种评估医疗设备自适应AI模型的新方法，通过三个互补指标（学习、潜力、保留）来区分模型适应与动态环境变化对性能的影响。
...
Method: 提出了一种新颖方法，包含三个互补测量指标：学习（模型在当前数据上的改进）、潜力（数据集驱动的性能变化）、保留（跨修改步骤的知识保存）。通过模拟人口变化进行案例研究。
...
### [316] [QED-Nano: Teaching a Tiny Model to Prove Hard Theorems](https://arxiv.org/abs/2604.04898)
...
<details>
  <summary>Details</summary>
Motivation: 专有AI系统在复杂证明问题上表现出色，但其训练流程不透明、依赖大型内部模型和架构，导致运行成本高、难以复现和研究。本文旨在探索小型开放模型是否也能在奥林匹克数学问题上达到有竞争力的推理性能。

Method: 三阶段训练方法：1）监督微调，从DeepSeek-Math-V2蒸馏学习良好的证明写作风格；2）基于规则的强化学习；3）扩展强化学习，引入推理缓存，将长证明分解为迭代的总结-精炼循环。

Result: QED-Nano超越了包括Nomos-1和GPT-OSS-120B在内的更大开放模型的证明生成性能，接近Gemini 3 Pro等专有模型的性能，同时推理成本大幅降低。

Conclusion: 小型开放模型通过精心设计的训练方法可以在奥林匹克数学证明问题上达到有竞争力的性能，为开放数学推理研究提供了完整的训练管道、数据集和代码支持。
