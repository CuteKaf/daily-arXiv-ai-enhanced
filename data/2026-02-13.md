...
Motivation: 茶叶是孟加拉国的重要经济作物，但茶叶叶片易受各种病害感染，导致产量下降和品质降低。传统人工检测方法耗时且容易出错，因此需要开发自动化的深度学习模型来更高效准确地检测茶叶病害。

Method: 研究使用TeaLeafBD数据集（包含5,278张高分辨率图像，分为7类：6种病害和1种健康叶片）。采用数据预处理、数据分割、对抗训练、数据增强、模型训练和评估的完整流程。使用DenseNet201和EfficientNetB3两种深度学习架构进行分类，并通过对抗训练增强模型鲁棒性。采用Grad-CAM可视化技术分析模型预测的可解释性。

Result: 实验结果显示，EfficientNetB3取得了最高的分类准确率93%，而DenseNet201达到91%。对抗训练使模型在面对噪声或干扰输入时仍能有效工作。Grad-CAM可视化成功识别了图像中对模型预测最具影响力的区域。
...
TL;DR: Active-Zero框架通过主动探索视觉环境，让视觉语言模型自主构建学习轨迹，相比传统被动交互方法显著提升了性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的自对弈方法依赖于与静态图像集的被动交互，导致对初始数据集的强依赖和低效学习。模型无法主动寻找适合其当前能力的视觉数据，浪费计算资源在过于简单或过于困难的任务上

Method: 提出Active-Zero框架，包含三个协同进化的智能体：1）Searcher从开放世界存储库中检索适合模型能力前沿的图像；2）Questioner合成校准的推理任务；3）Solver通过准确性奖励进行优化。形成闭环的自脚手架自动课程学习系统
...
TL;DR: ReTracing是一个多智能体具身表演艺术项目，采用考古学方法研究人工智能如何塑造、约束和产生身体运动。通过提取科幻小说中的人机交互描述，使用LLM生成"做什么"和"不做什么"的提示，再用扩散模型转换为人类表演者和四足机器人的动作指导，最终通过运动追踪和3D重建形成数字运动档案。


<details>
  <summary>Details</summary>
Motivation: 该项目旨在探索人工智能如何影响和塑造身体运动，揭示生成系统如何通过编排的动作编码社会文化偏见。通过结合AI、人类和机器人的沉浸式互动，探讨在AI也能移动、思考和留下痕迹的时代，成为人类意味着什么这一关键问题。

Method: 1. 从科幻小说中提取描述人机交互的句子；2. 使用大语言模型为每个摘录生成"做什么"和"不做什么"的配对提示；3. 使用基于扩散的文本到视频模型将这些提示转换为人类表演者的编舞指导和四足机器人的运动指令；4. 在镜面地板上执行动作，通过多摄像头运动追踪捕捉；5. 重建为3D点云和运动轨迹，形成数字运动档案。

Result: 创建了一个数字运动痕迹档案，记录了AI生成的动作指导下人类和机器人的互动表演。通过这种多智能体具身表演艺术形式，揭示了生成系统如何通过编排的动作编码社会文化偏见，并为探讨AI时代的人类身份问题提供了新的研究方法。

Conclusion: ReTracing作为一种新颖的研究方法，通过AI、人类和机器人的沉浸式互动，揭示了生成系统如何通过编排的动作编码社会文化偏见。该项目不仅探索了人工智能对身体运动的影响，还提出了在AI也能移动、思考和留下痕迹的时代，成为人类意味着什么这一深刻问题，为理解人机关系提供了新的视角。
...
Method: 开发REVEAL诊断基准，包含五项压力测试：时间期望偏差、语言捷径依赖、视频奉承、相机运动敏感性、时空遮挡鲁棒性，并提供自动生成诊断示例的数据管道

Result: 测试显示主流视频语言模型存在严重问题：将反向场景描述为正向、回答问题忽略视频内容、同意虚假声明、处理基本相机运动困难、在简单时空遮挡下无法聚合时间信息，而人类在这些任务上表现优异
...
<details>
  <summary>Details</summary>
Motivation: 当前基于摄影测量的3D模型生成方法存在多种设计选择，但不同方法之间的差异主要依赖定性评估，缺乏可重复、可量化的实验框架来系统比较各种方法的性能

Method: 开发了一个新流程：从高质量3D模型生成合成图像，并通过程序化生成相机姿态，创建可重复的实验环境，将虚拟相机参数和物体真实信息与重建估计值进行量化比较

Result: 该流程能够支持多种可重复、可量化的实验，可以系统比较不同摄影测量方法在相机参数估计和物体重建精度方面的性能差异

Conclusion: 提出的合成图像生成流程为评估数字孪生生成方法提供了可靠的量化实验框架，能够客观比较不同摄影测量方法的性能优劣
...
TL;DR: 提出一种将学习深度先验集成到VINS-Mono优化后端的新框架，通过仿射不变深度一致性和成对序数约束，在边缘设备计算限制下实现鲁棒的度量尺度恢复，显著提升低纹理环境下的VIO性能。


<details>
  <summary>Details</summary>
Motivation: 传统单目VIO系统在低纹理环境中表现不佳，因为稀疏视觉特征不足以进行准确的姿态估计。虽然密集单目深度估计可以作为补充信息源，但基于ViT的复杂基础模型计算需求大，难以在边缘设备上实时部署。

Method: 提出将学习深度先验直接集成到VINS-Mono优化后端的新框架，强制实施仿射不变深度一致性和成对序数约束，通过基于方差的门控机制显式过滤不稳定伪影，严格遵循边缘设备的计算限制。

Result: 在TartanGround和M3ED数据集上的大量实验表明，该方法在挑战性场景中防止发散，并带来显著的精度提升，绝对轨迹误差（ATE）降低高达28.3%。

Conclusion: 该方法成功地将学习深度先验集成到VIO系统中，在边缘设备计算限制下实现了鲁棒的度量尺度恢复，显著提高了低纹理环境下的VIO性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有实时超分辨率方法在处理压缩视频内容时存在困难，常用数据集无法准确反映流媒体特性，限制了基准测试的相关性

Method: 1) 从YouTube收集StreamSR数据集，涵盖多种视频类型和分辨率；2) 提出EfRLFN模型，集成高效通道注意力和双曲正切激活函数；3) 设计复合损失函数改善训练收敛；4) 对11种最先进模型进行基准测试

Result: 1) 创建了StreamSR数据集；2) EfRLFN在视觉质量和运行性能上均有提升；3) 在其他模型上使用该数据集微调后，在各种标准基准测试中均获得显著性能提升

Conclusion: StreamSR数据集填补了流媒体超分辨率领域的空白，EfRLFN模型在效率和视觉质量方面表现出色，数据集和模型代码已开源
...
TL;DR: ArtContext：一个从开放获取艺术史文章和Wikidata知识中提取信息，为艺术品添加上下文注释的流水线系统，使用定制化的PaintingCLIP模型提升性能


<details>
  <summary>Details</summary>
Motivation: 艺术史文章通常讨论艺术品的整体和具体部分（如布局、图像学、物质文化），但在查看艺术品时，很难识别不同文章对该作品的讨论内容

Method: 1) 构建新颖的语料库收集流水线；2) 使用低秩适应（LoRA）技术定制CLIP模型，使其适应艺术领域；3) 通过弱监督学习训练PaintingCLIP模型

Result: 新模型PaintingCLIP在性能上超越了原始CLIP模型，能够为给定艺术品提供更好的上下文信息

Conclusion: 提出的ArtContext流水线具有通用性，可以轻松应用于众多人文学科领域
...
Method: 提出Latent Forcing方法，通过联合处理潜在表示和像素，使用分别调优的噪声计划来排序去噪轨迹。潜在表示作为中间计算的"草稿纸"，在高频像素特征生成之前发挥作用。特别研究了条件信号顺序的重要性。

Result: 在ImageNet数据集上，Latent Forcing在基于扩散变换器的像素生成任务中，在作者的计算规模下达到了新的最先进水平。同时分析了条件信号顺序对模型性能的影响。

Conclusion: Latent Forcing成功地将潜在扩散的效率优势与在原始图像上操作的端到端建模相结合，解决了传统潜在扩散模型的局限性，为高质量图像生成提供了更有效的方法。
...
TL;DR: 该研究提出使用高分辨率CTA数据训练神经隐式函数，以改善短轴CMRI的各向异性限制，实现任意分辨率的心脏形状重建，特别是右心室和心肌的精确建模。


<details>
  <summary>Details</summary>
Motivation: 短轴心血管磁共振成像的各向异性特性限制了心脏形状分析，需要解决分辨率不足和形状重建不准确的问题。

Method: 利用近各向同性的高分辨率CTA数据训练单个神经隐式函数，联合表示任意分辨率的CMRI心脏形状，特别针对右心室和心肌（同时建模左心室内膜和心外膜表面）进行重建。

Result: 通过从重建形状中提取4腔切面进行评估，右心室和心肌的Dice相似系数分别为0.91±0.07和0.75±0.13，Hausdorff距离分别为6.21±3.97mm和7.53±5.13mm，显示出准确、平滑且解剖学上合理的形状重建能力。

Conclusion: 该方法能够重建准确、平滑且解剖学上合理的心脏形状，支持心脏形状分析的改进，为解决CMRI各向异性限制提供了有效解决方案。
...
TL;DR: Ctrl&Shift是一个端到端的扩散框架，通过两阶段分解（物体移除和参考引导修复）实现几何一致的物体操控，无需显式3D重建，在背景保持、几何一致性和用户可控性方面达到SOTA。
...
Conclusion: Ctrl&Shift通过创新的两阶段扩散框架，首次实现了无需显式3D重建的几何一致物体操控，在保持场景真实性的同时提供了精确的用户控制能力。
...
TL;DR: ARFC框架通过单一模型支持任意压缩比，无需为不同压缩比训练专门模型，在跨模态检索等任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 现有特征压缩方法通常需要为特定压缩比训练专门模型，缺乏灵活性，当需要新压缩比时必须重新训练，这限制了方法的通用性和实际应用

Method: 提出ARFC框架，核心是自回归模型ARC通过下一令牌预测进行压缩，压缩比可通过生成令牌数量控制；引入MoS模块利用多个压缩结果减少不确定性，ERGC模块在训练中保持语义和结构关系

Result: 在跨模态检索、图像分类和图像检索等多个数据集上的实验表明，该方法在各种压缩比下均优于现有方法，某些情况下甚至超过原始未压缩特征

Conclusion: ARFC框架提供了一种灵活高效的特征压缩解决方案，在资源受限的实际场景中具有有效性和通用性
...
<details>
  <summary>Details</summary>
Motivation: X射线血管造影是心血管疾病诊断的金标准，但当前深度学习方法受限于标注数据稀缺。虽然大规模自监督学习有潜力，但在该领域尚未充分探索，主要缺乏有效的SSL框架和大规模数据集。

Method: 提出VasoMIM框架，包含两个关键设计：1）解剖引导的掩码策略，策略性地掩码包含血管的补丁以学习稳健的血管语义；2）解剖一致性损失，保持原始图像和重建图像中血管结构的一致性，增强学习表示的判别性。

Result: 在四个下游任务、六个数据集上验证，VasoMIM展现出卓越的迁移能力，相比现有方法实现了最先进的性能。

Conclusion: VasoMIM作为基础模型在推进X射线血管造影分析任务方面具有显著潜力，相关代码和数据集将开源。
...
<details>
  <summary>Details</summary>
Motivation: PET成像存在辐射暴露问题，降低剂量或扫描时间会降低图像质量。利用MR图像辅助恢复标准剂量PET是可行方案，但面临多模态融合结构纹理不一致以及OOD数据不匹配的挑战

Method: 1. 设计多模态特征融合模块学习优化融合特征，充分利用MR图像但不引入额外细节；2. 以融合特征为条件，基于扩散模型迭代生成高质量SPET图像；3. 采用两阶段监督辅助学习策略，结合模拟分布内数据的通用先验和体内OOD数据的特定先验
...
TL;DR: 提出基于生成压缩框架的感知去噪方法，通过熵编码潜在表示强制低复杂度结构，使用生成解码器恢复真实纹理，在保持竞争性失真性能的同时实现一致的感知改进。


<details>
  <summary>Details</summary>
Motivation: 传统失真驱动去噪方法在强噪声和分布偏移下往往产生过度平滑的重建结果，无法同时保持结构细节和感知真实性，需要新的方法来平衡失真与感知质量。

Method: 提出生成压缩框架：1) 基于条件Wasserstein GAN的压缩去噪器，明确控制率-失真-感知权衡；2) 基于条件扩散的重建策略，通过压缩潜在表示指导迭代去噪；3) 建立加性高斯噪声下压缩最大似然去噪器的非渐近保证。

Result: 在合成和真实噪声基准测试中，该方法在保持竞争性失真性能的同时，实现了感知质量的持续改进。

Conclusion: 生成压缩框架为感知去噪提供了有效解决方案，通过熵编码潜在表示强制结构简单性，结合生成模型恢复真实纹理，在失真与感知之间取得了良好平衡。
...
Conclusion: LUVE通过潜在级联架构和双频专家设计，有效解决了超高清视频生成的关键挑战，为高质量UHR视频生成提供了创新解决方案。
...
Method: 提出FlowAdapt框架：1）Wasserstein贪婪采样策略，通过有界覆盖半径选择性过滤冗余样本；2）渐进知识传递模块，通过可学习路径将压缩的早期表示逐步注入后期阶段

Result: 在三个基准测试上，FlowAdapt仅使用1%可训练参数即达到最先进性能，有效弥合域差距，具有优异的样本效率和泛化能力

Conclusion: FlowAdapt通过最优传输理论解决了多智能体域适应中的关键问题，为V2X协同感知提供了高效、稳定的参数适应方案
...
TL;DR: PLOT-CT是一种新型低剂量CT重建框架，通过在预对数域应用Voronoi分解将数据解耦为不同成分，在分离的潜在空间中学习判别特征，显著提升重建精度，在1e4光子水平下比传统方法PSNR提升2.36dB。
...
Method: 提出PLOT-CT框架，在预对数正弦图上应用Voronoi分解，将数据解耦为不同的基础成分，这些成分被嵌入到分离的潜在空间中。这种显式分解增强了模型学习判别特征的能力，直接通过减轻噪声和保留预对数域固有信息来提高重建精度。

Result: 在预对数域1e4入射光子水平下，PLOT-CT比传统方法获得2.36dB的PSNR提升，实现了最先进的性能。

Conclusion: PLOT-CT通过在预对数域应用Voronoi分解，有效解决了低剂量CT重建中的噪声和数据保真度问题，显著提升了重建精度，为低剂量CT重建提供了新的有效方法。
...
TL;DR: PLESS是一种通用的伪标签增强策略，通过层次化图像分割和空间一致性优化，提升医学图像分割中涂鸦标注的伪标签质量。
...
Method: 提出PLESS策略，基于层次化图像分割将图像划分为空间一致的区域层次结构，在语义一致的区域内传播涂鸦信息以细化伪标签。该框架与模型无关，可轻松集成到现有伪标签方法中。
...
Method: 提出GR-Diffusion框架：1) 使用离散高斯表示从投影数据生成参考3D PET图像；2) 在扩散过程中采用分层引导机制，细粒度引导优化局部细节，粗粒度引导使用多尺度差异图校正偏差

Result: 在UDPET和临床数据集上，GR-Diffusion在不同剂量水平下均优于现有方法，显著提升3D全身PET图像质量并保留生理细节

Conclusion: GR-Diffusion成功整合几何先验和生成模型，为低剂量PET重建提供了有效解决方案，克服了传统方法的局限性
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶端到端系统依赖多模态大语言模型进行人车交互，但需要处理大量视觉token，计算资源消耗大。现有token缩减方法往往导致性能下降，需要一种既能减少计算成本又能保持性能的解决方案。

Method: 提出SToRM框架：1) 轻量级重要性预测器通过短期滑动窗口估计token重要性分数；2) 监督训练方法使用辅助路径从全token LLM传递获得伪监督信号；3) 锚点-上下文合并模块将token分为锚点和上下文token，将上下文token合并到相关锚点以减少冗余。

Result: 在LangAuto基准测试中，SToRM在相同缩减token预算下优于最先进的端到端驾驶MLLM，保持全token性能的同时将计算成本降低高达30倍。

Conclusion: SToRM是首个用于多模态大语言模型的监督式token缩减框架，实现了高效端到端驾驶，在显著降低计算成本的同时保持了与全token方法相当的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成方法在创建情感丰富内容时存在局限：无法捕捉细微的情感语义，缺乏沉浸式体验所需的细粒度控制。需要一种能够理解情感语义并提供精细控制的方法来降低创建情感丰富VR内容的门槛。

Method: 提出EmoSpace框架，通过视觉-语言对齐学习动态可解释的情感原型；采用具有丰富可学习原型的分层情感表示，原型在训练过程中演化；开发可控生成流程，包括多原型指导、时间混合和注意力重加权。

Result: 在定性和定量评估中，EmoSpace都优于现有方法；用户研究全面调查了VR环境与桌面设置对情感感知的影响差异；支持情感图像外延、风格化生成和VR环境情感全景生成等多种应用。

Conclusion: EmoSpace通过细粒度情感控制促进了沉浸式视觉内容生成，支持治疗、教育、叙事、艺术创作和文化保护等应用，为创建情感丰富的VR内容提供了有效解决方案。
...
TL;DR: Clutt3R-Seg：一种用于杂乱场景中语言引导抓取的零样本3D实例分割方法，通过分层实例树处理遮挡和噪声掩码，在稀疏视角下表现优异
...
Method: 提出分层实例树方法，利用噪声掩码作为信息线索，通过跨视图分组和条件替换抑制过分割和欠分割，生成视角一致的掩码和鲁棒的3D实例，并引入一致性感知更新处理多阶段任务中的场景变化

Result: 在合成和真实数据集上评估，在杂乱和稀疏视角场景中始终优于现有基线方法，在最具挑战性的重度杂乱序列中AP@25达到61.66，比基线高2.2倍以上，仅用4个输入视角就超越使用8个视角的MaskClustering方法2倍以上

Conclusion: Clutt3R-Seg通过分层实例树方法有效解决了杂乱环境中的3D实例分割问题，为语言引导的机器人操作提供了可靠感知，在真实机器人上验证有效
...
TL;DR: 本文提出了颈戴式视角的视线估计新任务，收集了首个相关数据集，评估了GLC模型并提出了两种改进方法：视线越界分类和多视角协同学习。


<details>
  <summary>Details</summary>
Motivation: 现有的自我中心视线估计研究主要关注头戴式摄像头，而其他视角（如颈戴式）尚未得到充分探索。颈戴式摄像头提供了不同的视角，可能在实际应用中更有优势。

Method: 1) 收集了首个颈戴式视线估计数据集（8名参与者，约4小时日常活动视频）；2) 评估了基于Transformer的GLC模型；3) 提出了两种扩展方法：视线越界分类辅助任务和多视角协同学习方法（结合头戴式和颈戴式视角，使用几何感知辅助损失）。

Result: 实验结果显示：视线越界分类辅助任务相比标准微调能提升性能；但多视角协同学习方法未能带来性能增益。作者进一步分析了这些结果并讨论了颈戴式视线估计的启示。
...
<details>
  <summary>Details</summary>
Motivation: 开发轻量且计算高效的次日野火蔓延预测工具，适用于资源受限环境中的实时应用，需要平衡准确性和效率

Method: 提出TD-FusionUNet模型，包含可训练的Hadamard变换和离散余弦变换层，应用二维变换在正交化潜在空间捕获频率分量；采用随机边缘裁剪和高斯混合模型等预处理技术增强稀疏火前掩码表示
...
TL;DR: RI-Mamba是首个用于点云的旋转不变状态空间模型，通过全局和局部参考框架分离姿态与几何，使用希尔伯特排序构建保持旋转不变性的标记序列，实现了在任意方向下对200多个物体类别的先进检索性能。


<details>
  <summary>Details</summary>
Motivation: 随着3D资产数量的快速增长，文本到形状检索变得至关重要。然而，现有方法需要规范姿态且支持的物体类别有限，限制了在现实世界中的应用，因为现实中的物体可能属于不同类别且以随机方向出现。

Method: RI-Mamba通过定义全局和局部参考框架来分离姿态与几何，使用希尔伯特排序构建具有几何意义的标记序列，同时保持旋转不变性。引入方向嵌入计算策略并通过特征线性调制重新整合，有效恢复空间上下文并增强模型表达能力。采用跨模态对比学习与自动三元组生成进行训练。

Result: 在OmniObject3D基准测试中，RI-Mamba在200多个物体类别和任意方向下实现了最先进的性能，展示了卓越的表征能力和鲁棒性。

Conclusion: RI-Mamba是首个旋转不变的状态空间模型，能够有效处理任意方向的点云，在文本到形状检索任务中表现出色，为现实世界3D资产检索提供了实用解决方案。
...
TL;DR: 开发了一种语义条件化的潜在扩散模型，用于生成具有解剖循环和C臂位置控制的动脉期脑DSA图像，在临床评估中表现出良好的真实性和应用潜力。


<details>
  <summary>Details</summary>
Motivation: 数字减影血管造影（DSA）在脑血管疾病诊断和治疗中至关重要，但其侵入性和高成本限制了大规模数据收集和公共数据共享，因此需要开发能够生成高质量合成DSA图像的方法。

Method: 开发了语义条件化的潜在扩散模型（LDM），使用包含解剖循环（前循环vs后循环）和标准C臂位置信息的文本嵌入进行训练。收集了99,349帧单中心DSA数据集，训练条件LDM生成合成DSA图像。

Result: 生成的图像在临床专家评估中获得3.1-3.3的Likert评分，具有高评分者间信度（ICC(2,k)=0.80-0.87）。Fréchet inception距离中位数为15.27，表明与真实DSA图像分布相似度高。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D场景生成方法虽然使用大语言模型，但缺乏领域特定推理、验证机制和模块化设计，导致控制性差、可扩展性不足。本文旨在解决这些限制，特别是针对农业合成模拟环境的生成需求。

Method: 开发了模块化多LLM管道，整合3D资产检索、领域知识注入和Unreal渲染引擎API代码生成。采用混合策略结合few-shot提示、检索增强生成、微调和验证等LLM优化技术，实现结构化数据处理、中间验证和灵活扩展。

Result: 系统通过结构化提示和语义准确性指标进行评估。用户研究显示生成场景在真实感和熟悉度方面与真实世界图像相当，专家比较表明相比手动场景设计节省了大量时间，验证了多LLM管道在自动化领域特定3D场景生成中的有效性。

Conclusion: 多LLM管道在自动化领域特定3D场景生成方面具有改进的可靠性和精确性。未来工作将扩展资产层次结构、纳入实时生成，并将管道适配到农业以外的其他模拟领域。
...
TL;DR: GSO-SLAM：一种基于高斯场景表示的实时单目密集SLAM系统，通过双向耦合视觉里程计和高斯溅射，在EM框架下联合优化，实现实时高精度场景重建和跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有SLAM方法存在两个问题：1）使用统一场景表示耦合跟踪和建图导致计算成本高；2）松散集成跟踪框架引入冗余。需要一种既能实时运行又能保持高精度的方法。

Method: 1）在期望最大化框架下双向耦合视觉里程计和高斯溅射，联合优化VO生成的半稠密深度估计和高斯场景表示；2）提出高斯溅射初始化方法，利用VO提供的图像信息、关键帧位姿和像素关联生成接近最终高斯场景的近似，避免启发式方法。

Result: 实验验证表明，GSO-SLAM不仅能够实时运行，而且在重建场景的几何/光度保真度和跟踪精度方面达到了最先进水平。

Conclusion: GSO-SLAM通过双向耦合VO和GS的EM框架联合优化，实现了实时高性能的单目密集SLAM，避免了现有方法的计算冗余和启发式初始化问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于预训练视觉语言模型构建的视觉语言动作模型存在样本效率低和泛化能力有限的问题。研究发现这些限制与预训练视觉表示密切相关，现有视觉表示在环境理解和策略先验方面提供的信息不足。

Method: 提出JEPA-VLA方法，自适应地将预测性嵌入（特别是V-JEPA 2）整合到现有VLA模型中。预测性视频嵌入能够灵活丢弃不可预测的环境因素，编码任务相关的时序动态，从而弥补现有视觉表示的不足。

Result: 在多个基准测试（LIBERO、LIBERO-plus、RoboTwin2.0）和真实机器人任务中，JEPA-VLA都取得了显著的性能提升。

Conclusion: 预测性视频嵌入能够有效增强VLA模型的视觉表示能力，提高样本效率和泛化性能，为机器人操作任务提供了更有效的视觉表示解决方案。
...
TL;DR: 该论文提出了一种新的视觉提示范式STVG-R1，将时空视频定位中的坐标预测转化为实例级识别问题，避免了跨模态坐标对齐的困难，并通过强化学习框架优化时空精度和一致性。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在密集预测任务（如时空视频定位）中，文本描述与视觉坐标之间的错位会导致幻觉问题。现有方法通常专注于增强视觉-文本对齐或添加辅助解码器，但这会引入额外的可训练模块，导致显著的标注成本和计算开销。

Method: 提出一种新的视觉提示范式，将每帧坐标预测重新表述为紧凑的实例级识别问题，为每个对象分配唯一且时间一致的ID，并将这些ID作为视觉提示嵌入视频中。同时引入STVG-R1——首个用于时空视频定位的强化学习框架，使用任务驱动的奖励联合优化时间精度、空间一致性和结构格式正则化。

Result: 在六个基准测试上的广泛实验证明了该方法的有效性。STVG-R1在HCSTVG-v2基准测试上比基线Qwen2.5-VL-7B在m_IoU上显著提升了20.9%，建立了新的SOTA。令人惊讶的是，STVG-R1在多对象参考视频对象分割任务上也表现出强大的零样本泛化能力，在MeViS上达到了47.3% J&F的SOTA。

Conclusion: 该研究提出的视觉提示范式有效避免了跨模态坐标对齐的难题，通过实例级识别和强化学习框架显著提升了时空视频定位的性能，并在多个任务上展现了优越的泛化能力。
...
TL;DR: DiffPlace是一个用于地点可控多视角图像生成的框架，通过引入地点ID控制器，在保持背景建筑一致性的同时灵活修改前景物体和天气条件，提升自动驾驶场景中的地点识别性能。
...
Method: 提出DiffPlace框架，引入地点ID控制器，使用线性投影、感知器变换器和对比学习将地点ID嵌入映射到固定的CLIP空间，使模型能够合成背景建筑一致的图像，同时灵活修改前景物体和天气条件。

Result: 广泛的实验（包括定量比较和增强训练评估）表明，DiffPlace在生成质量和视觉地点识别训练支持方面都优于现有方法。
...
TL;DR: 本文通过大规模实验研究展示了如何针对性地调整通用视觉语言模型(VLMs)以显著提升电商产品理解性能，同时保持广泛的跨模态能力，并提出了一个全面的评估套件。


<details>
  <summary>Details</summary>
Motivation: 电商产品理解需要强大的多模态理解能力（文本、图像、结构化属性）。通用视觉语言模型(VLMs)虽然能实现通用化的多模态潜在建模，但缺乏针对电商数据特性（属性中心、多图像、噪声）的适配策略，且不能牺牲通用性能。

Method: 通过大规模实验研究探索通用VLM的针对性适配方法；提出一个全面的评估套件，涵盖深度产品理解、严格指令遵循和动态属性提取。

Result: 研究表明，针对性地调整通用VLMs可以显著提升电商性能，同时保持广泛的多模态能力。

Conclusion: 本文为电商领域的VLM适配提供了有效策略和评估框架，平衡了领域专业性和通用能力。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在物体幻觉问题，即模型生成描述中包含了图像中不存在的物体。现有的视觉对比解码方法需要改进，以更有效地抑制这种幻觉现象。

Method: 利用自监督视觉Transformer中的对象中心注意力机制，移除最显著的视觉证据来构建辅助视图。这种方法可以破坏不受支持的标记并产生更强的对比信号。该方法与提示无关、与模型无关，可以无缝集成到现有的VCD流程中，计算开销很小（只需一次可缓存的向前传播）。
...
Method: 提出Light4D框架，包含两个核心技术：1) 解耦流引导：一种时间感知策略，在潜在空间中有效注入光照控制同时保持几何完整性；2) 时间一致性注意力：在IC-Light架构中开发时间一致性注意力机制，并结合确定性正则化消除外观闪烁。

Result: 大量实验表明，该方法在时间一致性和光照保真度方面具有竞争力，能够稳健处理-90°到90°的相机旋转。

Conclusion: Light4D是一个无需训练的4D视频重光照框架，能够有效解决4D重光照中的时间一致性和极端视角变化问题，为4D内容创作提供了新工具。
...
TL;DR: 提出轻量级RGB-D融合框架，通过深度先验增强EfficientViT-SAM，仅用11.2k样本训练即可超越原模型性能


<details>
  <summary>Details</summary>
Motivation: SAM模型需要大规模数据集（1100万图像）且仅依赖RGB输入，现有高效变体仍依赖大规模训练。希望通过深度信息提供几何先验来减少数据需求
...
Conclusion: 深度信息可以作为有效的几何先验，显著减少分割模型对大规模训练数据的依赖，实现更高效的分割性能
...
TL;DR: 该研究提出了一种基于3D分形的合成数据生成方法，用于动作识别模型的预训练，通过Targeted Smart Filtering技术解决了传统分形生成速度慢和多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 合成数据集在深度学习中被视为替代人工标注真实数据的有效方案。公式驱动监督学习（FDSL）能够通过公式驱动方法（如分形或轮廓）提供无限量的完美标注数据，且避免了人工劳动、隐私和伦理问题。然而，传统3D分形生成方法速度慢且产生退化的分形，限制了其在动作识别预训练中的应用。

Method: 研究使用3D迭代函数系统（IFS）生成3D分形，并通过时间变换形成视频序列，作为动作识别下游任务的预训练数据集。针对传统方法速度慢和多样性不足的问题，提出了Targeted Smart Filtering方法，该方法在保持分形美学质量的同时，显著提高了生成速度和分形多样性。

Result: 提出的Targeted Smart Filtering方法实现了约100倍的采样速度提升，并且在动作识别下游任务中，相比其他3D分形过滤方法取得了更优的性能表现。

Conclusion: 该研究表明，通过改进的3D分形生成方法，可以有效利用合成数据进行动作识别模型的预训练。Targeted Smart Filtering技术成功解决了传统分形生成的速度和多样性问题，为基于公式驱动监督学习的预训练提供了更高效的解决方案。
...
TL;DR: WorldTree提出统一时空分解框架，通过时间分割树和空间祖先链实现层次化运动表示，在单目动态重建中取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 当前动态重建方法在单目输入应用中存在挑战，现有工作缺乏统一的时空分解框架，要么采用整体时间优化，要么采用耦合的层次空间组合，限制了运动表示的效率和效果

Method: 提出WorldTree统一框架：1) 时间分割树(TPT)基于继承的分割树结构实现从粗到细的层次化时间分解优化；2) 空间祖先链(SAC)递归查询祖先层次结构，提供互补的空间动态，同时在祖先节点间专门化运动表示

Result: 在不同数据集上的实验表明，该方法在NVIDIA-LS数据集上LPIPS指标提升8.26%，在DyCheck数据集上mLPIPS指标提升9.09%，优于现有最佳方法

Conclusion: WorldTree通过统一的时空分解框架有效解决了单目动态重建中的挑战，实现了层次化的运动表示和优化，在多个基准上取得了显著性能提升
...
TL;DR: DynaHOI-Gym是一个用于动态手-物交互评估的在线闭环平台，包含参数化运动生成器和基于rollout的评估指标，并发布了包含1000万帧的大规模基准数据集DynaHOI-10M。
...
Method: 开发了DynaHOI-Gym统一在线闭环平台，包含参数化运动生成器和基于rollout的评估指标；提出了简单的观察-行动基线方法（ObAct），通过时空注意力整合短期观察与当前帧来预测动作。

Result: 发布了包含1000万帧、18万条手部捕捉轨迹的大规模基准数据集DynaHOI-10M，目标运动分为8个主要类别和22个细粒度子类别；ObAct基线方法在位置成功率上实现了8.1%的提升。

Conclusion: DynaHOI-Gym平台和DynaHOI-10M基准填补了动态手-物交互评估的空白，为未来研究提供了重要的测试平台和数据集资源。
...
TL;DR: 提出基于隐式神经表示和去噪扩散模型的心肌瘢痕LGE图像及分割掩码合成框架，通过数据增强提升分割性能
...
Method: 结合隐式神经表示和去噪扩散模型：首先训练INR捕获LGE数据及相应心肌和纤维化掩码的连续空间表示，压缩为紧凑潜在嵌入，然后扩散模型在该潜在空间生成新表示，解码为合成LGE图像及解剖一致的分割掩码
...
TL;DR: 该研究评估了视觉语言模型在法语PDF转Markdown任务上的表现，创建了法语文档基准，发现专有模型在手写体和表单处理上更稳健，开源模型在标准印刷布局上仍有竞争力。


<details>
  <summary>Details</summary>
Motivation: 文档解析是RAG流程的关键步骤，但现有基准多关注英语或中文，且过度惩罚格式差异，缺乏对法语等语言和复杂布局的专门评估。

Method: 从6万份文档中通过模型分歧采样选取困难页面，创建法语基准；使用单元测试式检查评估文本存在性、阅读顺序和表格约束；采用类别特定归一化减少格式差异影响。

Result: 评估15个模型显示，最强专有模型在手写体和表单处理上稳健性显著更高，多个开源模型在标准印刷布局上仍具竞争力。

Conclusion: 需要专门的法语文档解析基准，专有模型在复杂场景表现更优，但开源模型在标准任务上仍有价值，格式归一化对公平评估很重要。
...
TL;DR: 本文提出了一种基于贝叶斯深度学习的概率优化框架，通过新的置信度-不确定性边界损失函数和双温度缩放策略，解决医学影像AI模型中的校准问题，确保不确定性估计与预测准确性对齐。


<details>
  <summary>Details</summary>
Motivation: 在基于医学影像的关键决策支持系统中，AI辅助决策的可靠性与预测准确性同等重要。尽管深度学习模型在准确性方面表现出色，但经常存在校准不足的问题，表现为对错误预测的过度自信。为了促进临床接受度，模型需要以与预测正确性相关的方式量化不确定性，使临床医生能够识别不可靠的输出进行进一步审查。

Method: 提出了一个通用的概率优化框架，基于贝叶斯深度学习。具体包括：1）引入新的置信度-不确定性边界损失函数（CUB-Loss），对高置信度错误和低置信度正确预测施加惩罚，明确强制预测正确性与不确定性估计的对齐；2）设计了双温度缩放策略（DTS）进行后校准，进一步细化后验分布以提高直观可解释性。

Result: 在三个不同的医学影像任务上验证了该框架：肺炎自动筛查、糖尿病视网膜病变检测和皮肤病变识别。实证结果表明，该方法在不同模态下实现了一致的校准改进，在数据稀缺场景下保持稳健性能，并且在严重不平衡的数据集上仍然有效。

Conclusion: 该框架展示了在真实临床部署中的潜力，通过确保模型不确定性估计与预测准确性对齐，提高了医学影像AI决策支持系统的可靠性，有助于临床医生识别需要进一步审查的不可靠预测。
...
Method: 提出空间思维链(SCoT)框架：1) 使用交错文本-坐标指令格式训练扩散模型增强布局感知；2) 利用先进MLLMs作为规划器生成全面布局计划，将其空间规划能力直接转移到生成过程

Result: 在图像生成基准测试中达到最先进性能，在复杂推理任务上显著优于基线方法，在图像编辑场景中也表现出强大效果

Conclusion: SCoT框架有效弥合了MLLMs的推理能力与扩散模型的生成能力，提供了一种即插即用的解决方案，解决了现有方法的局限性
...
TL;DR: 本研究探索使用生成式AI方法改进新生儿复苏视频中的活动识别，通过本地视觉语言模型结合大语言模型，相比传统监督学习方法显著提升了识别准确率。


<details>
  <summary>Details</summary>
Motivation: 新生儿复苏的准确记录对质量改进和临床指南遵循至关重要，但实践中仍未被充分利用。虽然之前使用3D-CNN和Vision Transformer的方法在检测关键活动方面显示出潜力，但在识别这种细粒度活动方面仍面临挑战。

Method: 本研究探索使用生成式AI方法，特别是本地视觉语言模型结合大语言模型，与监督学习的TimeSFormer基线进行比较。使用了包含13.26小时新生儿复苏视频的模拟数据集，评估了多种零样本VLM策略和带有分类头的微调VLM，包括低秩适应方法。

Result: 结果表明，小型本地VLM存在幻觉问题，但当使用LoRA微调后，F1分数达到0.91，显著超过了TimeSformer的0.70结果。

Conclusion: 生成式AI方法特别是经过适当微调的视觉语言模型，在新生儿复苏视频活动识别方面具有显著优势，能够有效提升识别准确率，为临床质量改进提供了更好的技术支持。
...
TL;DR: ReNoV：一种利用外部表征引导的扩散式新视角合成框架，通过表征投影模块注入外部表征，提升几何一致性
...
Result: 在标准基准测试中优于先前的扩散式新视角方法，在重建保真度和修复质量方面均有显著提升，能够从稀疏、无姿态的图像集合中进行鲁棒合成

Conclusion: 利用外部表征的对应特性可以有效增强扩散式新视角合成的几何一致性，ReNoV框架在多个方面表现出优越性能
...
TL;DR: 本文提出了一种基于数字微镜器件(DMD)的高动态范围成像系统，通过空间调制和自适应曝光控制，在焊接电弧监测等高光环境下实现127dB动态范围，显著提升数字图像相关法的测量精度。


<details>
  <summary>Details</summary>
Motivation: 在焊接电弧监测和抛光金属表面分析等极端光照条件下，传统CCD/CMOS传感器动态范围不足70dB，容易因强光饱和导致细节丢失，严重影响数字图像相关法的测量精度。

Method: 开发了基于数字微镜器件的HDR成像系统，包含两个协同子系统：DMD光学调制单元和自适应计算成像流程，实现自主区域分割和自适应曝光控制。

Result: 系统达到127dB可测量动态范围，有效消除高光饱和伪影，实验显示应变误差降低78%，DIC定位精度显著提升，在极端强度变化下表现可靠。

Conclusion: DMD基系统提供高保真自适应HDR成像，克服传统传感器关键限制，在传统方法不足的高光环境下具有强大的光学计量和应力分析潜力。
...
### [47] [AssetFormer: Modular 3D Assets Generation with Autoregressive Transformer](https://arxiv.org/abs/2602.12100)
...
<details>
  <summary>Details</summary>
Motivation: 数字行业对高质量、多样化的模块化3D资产需求日益增长，特别是在用户生成内容领域。现有方法难以生成由基本单元组成且符合设计约束的资产。

Method: 采用自回归Transformer架构，借鉴语言模型的模块序列化和解码技术，使用从在线平台收集的真实模块化资产数据进行训练。

Result: 初步结果表明AssetFormer能有效提升资产生成质量，为专业开发和用户生成内容场景简化资产创建流程。

Conclusion: AssetFormer提供了一个可扩展到多种模块化3D资产类型的灵活框架，为3D内容生成领域做出贡献。
...
### [48] [PosterOmni: Generalized Artistic Poster Creation via Task Distillation and Unified Reward Feedback](https://arxiv.org/abs/2602.12127)
*Sixiang Chen,Jianyu Lai,Jialin Gao,Hengyu Shi,Zhongying Liu,Tian Ye,Junfeng Luo,Xiaoming Wei,Lei Zhu*

Main category: cs.CV

TL;DR: PosterOmni是一个统一的图像到海报生成框架，通过数据蒸馏奖励管道整合局部编辑和全局创建两种模式，显著提升参考一致性、构图质量和美学和谐度。


<details>
  <summary>Details</summary>
Motivation: 图像到海报生成是一个高需求任务，需要同时处理局部调整和高层次设计理解。现有方法需要同时生成文本、布局、风格和视觉元素，同时保持语义保真度和美学一致性。这个过程涉及两个相互关联的领域：需要保留具体视觉实体的局部编辑，以及依赖抽象设计概念理解的全局创建。

Method: PosterOmni通过高效的数据蒸馏奖励管道整合局部编辑和全局创建：1）构建覆盖六种任务类型的多场景图像到海报数据集；2）在局部和全局专家之间进行知识蒸馏用于监督微调；3）应用统一的PosterOmni奖励反馈来联合对齐所有任务的视觉实体保留和美学偏好。同时建立了PosterOmni-Bench统一评估基准。

Result: 大量实验表明，PosterOmni显著增强了参考一致性、全局构图质量和美学和谐度，超越了所有开源基线，甚至超过了几个专有系统。

Conclusion: PosterOmni是一个通用的艺术海报创建框架，通过整合局部编辑和全局创建两种模式，解锁了基础编辑模型在多任务图像到海报生成中的潜力，为这一复杂任务提供了有效的解决方案。
...
### [49] [FAIL: Flow Matching Adversarial Imitation Learning for Image Generation](https://arxiv.org/abs/2602.12155)
*Yeyao Ma,Chen Li,Xiaosong Zhang,Han Hu,Weidi Xie*

Main category: cs.CV

TL;DR: FAIL提出了一种基于流匹配的对抗模仿学习框架，用于对齐生成模型的输出分布，无需显式奖励或成对比较，在图像和视频生成任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统监督微调无法纠正未见状态下的策略漂移，而偏好优化方法需要昂贵的偏好对或奖励建模。需要一种无需显式奖励或成对比较就能最小化策略与专家分布差异的方法。

Method: 提出Flow Matching Adversarial Imitation Learning (FAIL)框架，通过对抗训练最小化策略与专家分布的差异。开发两种算法：FAIL-PD利用可微分ODE求解器获得低方差路径梯度；FAIL-PG为离散或计算受限场景提供黑盒替代方案。

Result: 仅使用13,000个Nano Banana pro演示微调FLUX模型，在提示跟随和美学基准测试中达到竞争性性能。框架能有效泛化到离散图像和视频生成任务，并作为鲁棒正则化器缓解基于奖励的优化中的奖励黑客问题。
...
### [50] [TexSpot: 3D Texture Enhancement with Spatially-uniform Point Latent Representation](https://arxiv.org/abs/2602.12157)
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的多视图扩散管道存在视图不一致问题，现有表示方法要么依赖UV贴图（展开时会产生扭曲），要么使用点基方法（纹理保真度受几何密度限制），难以生成高质量3D纹理。

Method: 提出TexSpot框架，核心是Texlet表示：结合点基3D纹理的几何表达能力和UV表示的紧凑性。每个Texlet潜在向量通过2D编码器编码局部纹理块，再通过3D编码器聚合全局形状上下文。使用级联的3D到2D解码器重建高质量纹理块，训练基于Texlet的扩散变换器来增强多视图扩散方法生成的纹理。
...
### [51] [DreamID-Omni: Unified Framework for Controllable Human-Centric Audio-Video Generation](https://arxiv.org/abs/2602.12160)
*Xu Guo,Fulong Ye,Qichao Sun,Liyang Chen,Bingchuan Li,Pengze Zhang,Jiawei Liu,Songtao Zhao,Qian He,Xiangwang Hou*

Main category: cs.CV

TL;DR: DreamID-Omni是一个统一的可控人声视频生成框架，通过对称条件扩散Transformer和双级解耦策略解决多人场景中的身份-音色绑定失败问题，在多任务训练下实现全面的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将参考音频视频生成(R2AV)、视频编辑(RV2AV)和音频驱动视频动画(RA2V)等人声任务视为孤立目标，且难以在单一框架中实现对多人物身份和音色的精确解耦控制。

Method: 提出对称条件扩散Transformer，通过对称条件注入方案整合异构条件信号；采用双级解耦策略：信号级的同步RoPE确保注意力空间绑定，语义级的结构化字幕建立属性-主体映射；设计多任务渐进训练方案，利用弱约束生成先验正则化强约束任务。

Result: 在视频、音频和视听一致性方面实现全面的最先进性能，甚至超越领先的专有商业模型。

Conclusion: DreamID-Omni为可控人声音频视频生成提供了一个统一框架，解决了身份-音色绑定失败和说话者混淆问题，在学术研究和商业级应用之间架起了桥梁。
...
### [52] [DeepGen 1.0: A Lightweight Unified Multimodal Model for Advancing Image Generation and Editing](https://arxiv.org/abs/2602.12205)
...
TL;DR: DeepGen 1.0是一个轻量级5B参数的多模态统一模型，在图像生成和编辑任务上超越了更大参数规模的模型，通过创新的SCB架构和三阶段训练策略实现高效性能。
...
Result: 仅使用约5000万样本训练，DeepGen 1.0在多个基准测试中表现领先：在WISE上超越80B的HunyuanImage 28%，在UniREditBench上超越27B的Qwen-Image-Edit 37%。
...
### [53] [Best of Both Worlds: Multimodal Reasoning and Generation via Unified Discrete Flow Matching](https://arxiv.org/abs/2602.12221)
...
TL;DR: UniDFlow是一个统一的多模态离散流匹配框架，通过任务特定的低秩适配器解耦理解和生成任务，避免目标干扰和表示纠缠，同时使用基于参考的多模态偏好对齐优化相同条件下的相对结果，无需大规模重新训练即可提升忠实度和可控性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态系统在理解和生成任务之间存在目标干扰和表示纠缠的问题，需要统一的框架来处理多模态的理解、生成和编辑任务，同时保持高性能和良好的泛化能力。

Method: 提出UniDFlow框架：1) 通过任务特定的低秩适配器解耦理解和生成任务；2) 使用基于参考的多模态偏好对齐优化相对结果；3) 在相同条件下进行优化，避免大规模重新训练。

Result: 在八个基准测试中达到最先进的性能，展现出强大的零样本泛化能力，能够处理修复、上下文图像生成、基于参考的编辑和组合生成等任务，尽管没有进行明确的任务特定训练。

Conclusion: UniDFlow提供了一个有效的统一框架，通过解耦任务和偏好对齐，实现了多模态理解、生成和编辑的高性能，同时具备良好的泛化能力和可控性。
...
### [54] [MonarchRT: Efficient Attention for Real-Time Video Generation](https://arxiv.org/abs/2602.12271)
*Krish Agarwal,Zhuoming Chen,Cheng Luo,Yongqi Chen,Haizhong Zheng,Xun Huang,Atri Rudra,Beidi Chen*

Main category: cs.CV

TL;DR: Monarch-RT：一种用于实时视频生成的结构化注意力参数化方法，通过Monarch矩阵分解注意力，在保持计算效率的同时实现高表达能力，首次在单个RTX 5090上实现16 FPS的真实实时视频生成。


<details>
  <summary>Details</summary>
Motivation: 实时视频生成中的扩散变换器受限于3D自注意力的二次计算成本，特别是在少步和自回归的实时场景中。现有稀疏注意力近似方法在双向多步扩散中表现良好，但在实时场景中失效，因为视频注意力不是可靠稀疏的，而是结合了周期性结构、稀疏语义对应和密集混合。
...
Result: Monarch-RT在现有稀疏基线方法上表现出高效能，应用于最先进的Self-Forcing模型时达到95%的注意力稀疏度且无质量损失。优化实现优于FlashAttention-2/3/4内核，在Nvidia GPU上提供1.4-11.8倍的内核加速，首次在单个RTX 5090上实现16 FPS的真实实时视频生成。
...
### [55] [UniT: Unified Multimodal Chain-of-Thought Test-time Scaling](https://arxiv.org/abs/2602.12279)
...
<details>
  <summary>Details</summary>
Motivation: 当前统一模型虽然能处理多模态理解和生成，但缺乏迭代精炼能力。许多复杂多模态任务需要分解指令、验证中间结果并进行迭代修正，而测试时扩展范式在多模态领域的应用仍面临挑战。

Method: UniT框架结合智能数据合成、统一模型训练和灵活测试时推理，通过多模态思维链实现验证、子目标分解和内容记忆等认知行为。采用顺序思维链推理而非并行采样。

Result: 1) 在短推理轨迹上训练的统一模型能泛化到更长的测试推理链；2) 顺序思维链推理比并行采样更具可扩展性和计算效率；3) 生成和编辑轨迹训练改善了分布外视觉推理能力。
...
### [56] [Stroke of Surprise: Progressive Semantic Illusions in Vector Sketching](https://arxiv.org/abs/2602.12280)
*Huai-Hsun Cheng,Siang-Ling Zhang,Yu-Lun Liu*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的渐进式语义幻觉任务，通过矢量笔划的逐步添加实现单一草图从一种语义（如鸭子）到另一种语义（如绵羊）的戏剧性转变，并开发了Stroke of Surprise生成框架来解决这一双重约束优化问题。


<details>
  <summary>Details</summary>
Motivation: 传统视觉幻觉主要依赖空间操作如多视图一致性，本文旨在将视觉字谜从空间维度扩展到时间维度，通过矢量笔划的渐进添加实现语义转换，创造新的视觉幻觉体验。

Method: 提出Stroke of Surprise生成框架，采用序列感知联合优化方法，核心是双分支分数蒸馏采样机制，动态调整前缀笔划以发现对两个目标都有效的"共同结构子空间"，并引入新颖的叠加损失来确保空间互补性和结构整合而非遮挡。

Result: 大量实验表明，该方法在可识别性和幻觉强度方面显著优于现有基线方法，成功地将视觉字谜从空间维度扩展到时间维度。

Conclusion: 本文提出的渐进式语义幻觉任务和Stroke of Surprise框架为视觉幻觉研究开辟了新方向，通过时间维度的笔划序列实现了语义转换，为创造动态视觉体验提供了新方法。
...
### [57] [Automated Optimization Modeling via a Localizable Error-Driven Perspective](https://arxiv.org/abs/2602.11164)
*Weiting Liu,Han Wu,Yufei Kuang,Xiongwei Han,Tao Zhong,Jianfeng Feng,Wenlian Lu*

Main category: cs.LG

TL;DR: MIND框架通过错误驱动的本地化视角改进LLMs的自动化优化建模，解决了现有方法中错误特定问题稀疏和困难问题奖励稀疏的局限性，在六个基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有自动化优化建模方法存在两个根本局限性：(L1) 错误特定问题的稀疏性，以及(L2) 困难问题的稀疏奖励。这些限制导致LLMs在领域特定后训练中表现不佳。

Method: 提出MIND框架，基于优化建模中错误传播的独特本地化模式（错误可能局限于特定语义片段），构建聚焦的高密度训练语料库，并提出动态监督微调策略优化(DFPO)通过本地化精炼处理困难问题。

Result: 在六个基准测试上的实验表明，MIND框架在所有最先进的自动化优化建模方法中表现一致优于现有方法。

Conclusion: MIND框架通过错误驱动的本地化视角，有效解决了自动化优化建模中的数据稀缺和困难问题处理挑战，显著提升了LLMs在该领域的性能。
...
### [58] [KBVQ-MoE: KLT-guided SVD with Bias-Corrected Vector Quantization for MoE Large Language Models](https://arxiv.org/abs/2602.11184)
...
<details>
  <summary>Details</summary>
Motivation: MoE模型虽然通过稀疏专家激活提高了计算效率，但其巨大的参数量和内存需求给资源受限环境部署带来挑战。传统向量量化方法直接应用于MoE会导致性能显著下降，主要因为专家间冗余表示浪费码本容量，以及专家聚合会放大累积输出偏置。

Method: 提出KBVQ-MoE框架，包含两个关键技术：1）输入驱动的冗余消除——使用KLT引导的SVD提取主导权重分量并在专家间共享；2）偏置校正的输出稳定——仅对专家特定（非冗余）表示进行向量量化，并通过通道级仿射补偿校正量化输出。

Result: 在各种MoE LLM上的实验表明，KBVQ-MoE比现有量化方法更好地保持精度。例如，Qwen1.5-MoE-A2.7B的3比特量化平均精度达到67.99，几乎与FP16基线的68.07相同。

Conclusion: KBVQ-MoE有效解决了MoE模型超低比特量化中的冗余表示和输出偏置问题，为边缘设备等资源受限平台的高效部署提供了有前景的解决方案。
...
### [59] [Spectra: Rethinking Optimizers for LLMs Under Spectral Anisotropy](https://arxiv.org/abs/2602.11185)
...
TL;DR: 论文提出Spectra优化器，通过识别并抑制LLM训练中占主导的低秩梯度子空间，解决AdamW因梯度各向异性导致的尾部学习被抑制问题，显著提升训练效率。


<details>
  <summary>Details</summary>
Motivation: LLM训练中的梯度信号具有高度各向异性：循环语言结构将能量集中在少数主导谱方向（尖峰），而上下文特定信息位于长尾中。AdamW等优化器对尖峰子空间的过度关注抑制了尾部学习，限制了模型性能提升。

Method: 提出Spectra优化器：1）通过缓存、热启动的幂迭代跟踪尖峰子空间；2）应用低秩谱整形抑制主导子空间而不放大噪声敏感的谱尾部；3）以可忽略的开销和大幅减少的优化器状态内存实现。

Result: 在LLaMA3 8B模型上训练500亿token：1）比AdamW快30%达到相同目标损失；2）端到端开销减少0.7%；3）优化器状态内存减少49.25%；4）下游任务平均准确率提升1.62%。相比Muon优化器，处理速度快5.1倍，最终损失更低，准确率提升0.66%。

Conclusion: 梯度谱的尖峰-尾部分离是LLM训练中的关键瓶颈，Spectra通过针对性的谱整形有效解决了这一问题，在训练效率、内存使用和模型性能方面均取得显著改进。
...
### [60] [GAC-KAN: An Ultra-Lightweight GNSS Interference Classifier for GenAI-Powered Consumer Edge Devices](https://arxiv.org/abs/2602.11186)
*Zhihan Zeng,Kaihe Wang,Zhongpei Zhang,Yue Xiu*

Main category: cs.LG

TL;DR: 提出GAC-KAN框架解决GenAI时代边缘设备GNSS信号保护面临的数据稀缺和计算资源紧张双重挑战，通过物理仿真生成干扰数据集，设计轻量级MS-GAC主干和KAN决策头，实现98.0%准确率且仅需0.13M参数。


<details>
  <summary>Details</summary>
Motivation: GenAI在消费电子设备中的集成带来了巨大计算负担，导致边缘硬件资源紧张，难以支持GNSS信号保护等基本安全任务。同时，真实干扰数据稀缺阻碍了鲁棒分类器的训练。

Method: 1) 采用物理引导仿真方法合成大规模高保真干扰数据集；2) 设计MS-GAC主干结合非对称卷积块和Ghost模块提取频谱-时间特征；3) 用Kolmogorov-Arnold Network替代传统MLP决策头，使用可学习样条激活函数。

Result: GAC-KAN达到98.0%的整体准确率，优于现有基准方法。模型仅包含0.13M参数，比Vision Transformer基准少约660倍，具有极致的轻量化特性。

Conclusion: GAC-KAN框架成功解决了GenAI时代边缘设备GNSS保护的数据稀缺和资源约束问题，可作为"始终在线"的安全伴侣，在不与主要GenAI任务竞争计算资源的情况下确保GNSS可靠性。
...
### [61] [TDPNavigator-Placer: Thermal- and Wirelength-Aware Chiplet Placement in 2.5D Systems Through Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2602.11187)
*Yubo Hou,Furen Zhuang,Partha Pratim Kundu,Sezin Ata Kircali,Jie Wang,Mihai Dragos Rotaru,Dutta Rahul,Ashish James*

Main category: cs.LG

TL;DR: 提出TDPNavigator-Placer，一种基于热设计功率的多智能体强化学习框架，用于2.5D集成电路中的芯片粒布局优化，有效平衡线长和热管理这两个冲突目标。


<details>
  <summary>Details</summary>
Motivation: 随着2.5D集成电路规模扩大和异构性增强，现有布局方法通常只关注最小化线长或将多目标优化转化为加权单目标，无法有效处理线长减少和热管理这两个本质冲突的设计需求。

Method: 提出TDPNavigator-Placer框架，采用多智能体强化学习方法，将冲突目标分配给专门的智能体，每个智能体在统一的布局范式下使用不同的奖励机制和环境约束，基于芯片粒的热设计功率动态优化布局。

Result: 实验结果表明，TDPNavigator-Placer相比现有方法能提供显著改进的帕累托前沿，在线长和热性能之间实现更平衡的权衡。

Conclusion: 该多智能体强化学习框架能够有效处理2.5D集成电路布局中的冲突设计目标，为大规模异构芯片粒组装提供了实用的解决方案。
...
### [62] [MELINOE: Fine-Tuning Enables Memory-Efficient Inference for Mixture-of-Experts Models](https://arxiv.org/abs/2602.11192)
*Arian Raje,Anupam Nayak,Gauri Joshi*

Main category: cs.LG

TL;DR: MELINOE通过微调MoE模型使其更倾向于激活更少的专家，并将这些偏好专家缓存在GPU内存中，减少专家更换和CPU-GPU传输开销，显著提升推理效率。


<details>
  <summary>Details</summary>
Motivation: MoE模型虽然能减少每个token激活的参数数量，但其庞大的总参数需要在GPU内存中加载，限制了在资源受限环境中的应用。现有方法通过将专家卸载到CPU内存并在激活时传输到GPU，但面临显著的I/O延迟问题。

Method: 提出MELINOE方法，通过微调MoE模型使其更强烈地偏好在每个序列中激活更少的专家。将这些偏好专家缓存在GPU内存中，减少专家更换频率和CPU-GPU传输开销。

Result: MELINOE相比高效基线提升了1.2-3倍的吞吐量，相比传输密集型基线提升了高达14.7倍，同时在下游任务中保持甚至提升了模型性能。

Conclusion: MELINOE是一种可靠的方法，能够显著提高MoE模型的推理效率，同时保持模型性能，解决了MoE模型在资源受限环境中的内存瓶颈问题。
...
### [63] [Predicting the post-wildfire mudflow onset using machine learning models on multi-parameter experimental data](https://arxiv.org/abs/2602.11194)
*Mahta Movasat,Ingrid Tomac*

Main category: cs.LG

TL;DR: 该研究应用多种机器学习算法预测和分类野火后泥石流的发生，发现细沙在低强度长时间降雨下最易侵蚀，降雨前10分钟对泥石流形成最为关键。


<details>
  <summary>Details</summary>
Motivation: 野火后土壤疏水性增强导致泥石流灾害加剧，这些泥石流在强度、持续时间和破坏性上与自然泥流不同。需要理解泥石流发生的时机和条件，特别是降雨强度、坡度、水分入渗值和颗粒大小等关键参数的耦合效应。

Method: 应用多种机器学习算法：多元线性回归（MLR）、逻辑回归（LR）、支持向量分类器（SVC）、K均值聚类和主成分分析（PCA），通过实验室实验模拟野外条件，使用降雨装置在不同土壤的倾斜水槽中进行测试。

Result: MLR能有效预测总径流量，但对侵蚀的预测准确性较低，特别是粗沙。LR和SVC在分类破坏结果方面表现良好，得到聚类和降维分析的支持。敏感性分析显示细沙极易侵蚀，特别是在低强度长时间降雨条件下。降雨前10分钟的高强度降雨对径流和破坏最为关键。
...
### [64] [AM-FM: A Foundation Model for Ambient Intelligence Through WiFi](https://arxiv.org/abs/2602.11200)
*Guozhen Zhu,Yuqian Hu,Sakila Jayaweera,Weihang Gao,Wei-Hsiang Wang,Jiaxuan Zhang,Beibei Wang,Chenshu Wu,K. J. Ray Liu*

Main category: cs.LG

TL;DR: AM-FM是首个基于WiFi的通用环境智能基础模型，通过对比学习、掩码重建和物理信息目标在920万无标签CSI样本上预训练，在9个下游任务中表现出强大的跨任务性能和数据效率。
...
Method: AM-FM是首个用于环境智能和WiFi传感的基础模型，在439天内从20种商业设备收集的920万无标签CSI样本上进行预训练，使用对比学习、掩码重建和针对无线信号定制的物理信息目标来学习通用表示。

Result: 在涵盖9个下游任务的公共基准测试中，AM-FM显示出强大的跨任务性能和数据效率提升，证明了基础模型可以利用现有无线基础设施实现可扩展的环境智能。

Conclusion: AM-FM作为首个WiFi环境智能基础模型，通过大规模无标签数据预训练实现了强大的跨任务泛化能力，为利用现有无线基础设施实现可扩展环境智能提供了新途径。
...
### [65] [Zero-Sacrifice Persistent-Robustness Adversarial Defense for Pre-Trained Encoders](https://arxiv.org/abs/2602.11204)
*Zhuxin Lei,Ziyuan Yang,Yi Zhang*

Main category: cs.LG

TL;DR: 提出ZePAD方法，通过双分支结构实现对抗下游无关对抗样本的持久鲁棒性，同时保持良性性能


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习预训练编码器存在下游无关对抗样本的脆弱性，现有防御方法依赖任务特定的对抗微调，导致泛化性差、灾难性遗忘和良性性能下降

Method: 提出ZePAD双分支结构：1) MPAE-Branch使用两个对抗微调编码器增强对抗抵抗；2) BMP-Branch在本地数据上训练确保对抗鲁棒性不损害良性性能。无需训练对抗样本识别任务即可直接检测DAEs

Result: 在11种SSL方法和6个数据集上验证有效性，某些情况下良性性能提升29.20%，对抗鲁棒性提升73.86%，实现零牺牲特性

Conclusion: ZePAD通过单次对抗微调即可防御多种下游任务的DAEs，实现持久鲁棒性，同时保持良性性能，解决了现有方法的局限性
...
### [66] [Adaptive Physics Transformer with Fused Global-Local Attention for Subsurface Energy Systems](https://arxiv.org/abs/2602.11208)
...
<details>
  <summary>Details</summary>
Motivation: 地下系统（能源资源、CO2封存等）的物理模拟计算成本极高，主要挑战包括地质异质性、高分辨率需求和不同时间尺度的物理过程耦合。

Method: 提出自适应物理变换器（APT），结合图编码器提取局部异质特征和全局注意力机制处理长程物理影响，支持自适应网格细化模拟。

Result: APT在规则和不规则网格的地下任务中超越现有架构，具有强大的超分辨率能力，首次实现直接从自适应网格细化模拟中学习，支持跨数据集学习。

Conclusion: APT作为大规模地下基础模型开发的稳健可扩展骨干，为解决计算密集型地下物理模拟提供了有效方案。
...
### [67] [Towards Compressive and Scalable Recurrent Memory](https://arxiv.org/abs/2602.11212)
*Yunchong Song,Jushi Kai,Liming Lu,Kaixi Qiu,Zhouhan Lin*

Main category: cs.LG

TL;DR: Elastic Memory是一种基于HiPPO框架的新型记忆架构，通过在线压缩历史序列为固定大小记忆状态，使用多项式采样机制进行检索，在长上下文任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: Transformer在处理长上下文时面临二次方注意力瓶颈，现有引入循环记忆的方法在理论原则和实际可扩展性之间存在根本性权衡，需要更好的解决方案。

Method: 基于HiPPO框架的Elastic Memory架构，将历史序列视为连续信号的采样，应用最优在线压缩编码为固定大小记忆状态，采用多项式采样机制从压缩状态重建历史摘要。
...
### [68] [Charting Empirical Laws for LLM Fine-Tuning in Scientific Multi-Discipline Learning](https://arxiv.org/abs/2602.11215)
*Lintao Wang,Zhuqiang Lu,Yilin Zhu,Kun Hu,Zhenfei Yin,Shixiang Tang,Zhiyong Wang,Wanli Ouyang,Xinzhu Ma*

Main category: cs.LG

TL;DR: 本文首次系统研究多学科LLM微调，通过构建五学科语料库分析不同微调方法的学习模式，提炼出四条经验法则，为多学科微调提供实践指导。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在单一科学领域通过微调取得了良好性能，但在多学科背景下的学习动态仍不清楚，而跨领域知识协同有望提升泛化能力和更广泛的应用性。

Method: 构建五学科语料库，系统分析全微调、LoRA、LoRA-MoE和LoRA组合等微调方法的学习模式，研究多学科学习动态。

Result: 研究发现多学科学习比单学科训练变异性更大，并提炼出四条经验法则：平衡后多样性、合并后对齐、优化后扩展、共享后专业化，这些法则形成了多学科微调的实践配方。

Conclusion: 这些经验法则为原则性的多学科微调提供了实用配方，并为开发可泛化的科学LLMs提供了可操作的指导。
...
### [69] [Protein Language Model Embeddings Improve Generalization of Implicit Transfer Operators](https://arxiv.org/abs/2602.11216)
...
<details>
  <summary>Details</summary>
Motivation: 传统分子动力学计算成本高，而现有生成式分子动力学方法在跨分子系统转移性方面有限。本研究旨在通过整合辅助信息源来提升可转移隐式转移算子的数据效率和泛化能力。

Method: 提出PLaTITO方法，将蛋白质语言模型嵌入等辅助信息整合到可转移隐式转移算子中。研究了粗粒度模型与Boltzmann Emulators的比较，并评估了结构嵌入、温度和大语言模型衍生嵌入等额外条件信号对模型性能的影响。

Result: 粗粒度TITO模型比Boltzmann Emulators数据效率显著更高；整合蛋白质语言模型嵌入进一步改善了分布外泛化能力；PLaTITO在蛋白质系统平衡采样基准上达到最先进性能，包括快速折叠蛋白质。
...
### [70] [The Magic Correlations: Understanding Knowledge Transfer from Pretraining to Supervised Fine-Tuning](https://arxiv.org/abs/2602.11217)
*Simin Fan,Dimitris Paparas,Natasha Noy,Binbin Xiong,Noveen Sachdeva,Berivan Isik*

Main category: cs.LG

TL;DR: 研究预训练到监督微调阶段语言模型能力的转移机制，通过四个核心问题分析准确率、置信度排名、基准测试预测能力、模型规模影响以及校准质量


<details>
  <summary>Details</summary>
Motivation: 理解语言模型能力从预训练到监督微调的转移机制对于高效模型开发和数据管理至关重要，需要探究预训练阶段建立的排名和关系在微调后是否保持

Method: 使用一套相关性分析协议，应用于不同数据混合和模型规模下的准确率和置信度指标，通过实验设计回答四个核心研究问题

Result: 转移可靠性在不同能力类别、基准测试和模型规模间差异显著，准确率和置信度表现出不同的、有时甚至相反的扩展动态

Conclusion: 研究揭示了预训练决策与下游结果之间复杂的相互作用，为基准测试选择、数据管理和高效模型开发提供了实用指导
...
### [71] [Credal Concept Bottleneck Models: Structural Separation of Epistemic and Aleatoric Uncertainty](https://arxiv.org/abs/2602.11219)
...
Method: 采用可信集表示不确定性，将认知不确定性对应集合大小，偶然不确定性对应集合内元素的噪声；实现变分可信概念瓶颈模型，使用两个独立的头部、不同的训练目标和分离的梯度路径

Result: 在多标注者基准测试中，相比标准方法，将认知不确定性和偶然不确定性的相关性降低了一个数量级以上；同时改进了认知不确定性与预测误差、偶然不确定性与真实模糊度的对齐
...
### [72] [Learning Glioblastoma Tumor Heterogeneity Using Brain Inspired Topological Neural Networks](https://arxiv.org/abs/2602.11234)
*Ankita Paul,Wenyi Wang*

Main category: cs.LG

TL;DR: TopoGBM：一种基于拓扑正则化的深度学习框架，通过保留胶质母细胞瘤的复杂拓扑结构来捕获肿瘤异质性，提高跨机构MRI数据的预后预测鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 胶质母细胞瘤（GBM）的极端空间和结构异质性，以及不同机构间MRI采集协议的不一致性，导致传统深度学习方法在预后预测中泛化能力差，容易受到扫描仪特定伪影的影响。

Method: 提出TopoGBM框架，使用3D卷积自编码器，并通过拓扑正则化来保留肿瘤流形的复杂非欧几里得不变量，在压缩潜在空间中捕获异质性保留、扫描仪鲁棒的表征。

Result: 在多个异质性队列（UPENN、UCSF、RHUH）和TCGA外部验证中，TopoGBM表现优异（C-index测试0.67，验证0.58），优于在域偏移下性能下降的基线方法。重建残差高度定位于病理异质性区域，肿瘤限制和健康组织误差显著较低。

Conclusion: 引入拓扑先验能够学习形态保真的嵌入表征，既能捕获肿瘤异质性，又能保持跨机构鲁棒性，为无监督学习方法提供了临床可靠性。
...
### [73] [AI-Driven Clinical Decision Support System for Enhanced Diabetes Diagnosis and Management](https://arxiv.org/abs/2602.11237)
*Mujeeb Ur Rehman,Imran Rehan,Sohail Khalid*

Main category: cs.LG

TL;DR: 该研究开发了一种用于2型糖尿病诊断的人工智能临床决策支持系统，通过混合方法结合专家知识和机器学习技术，在测试数据集和临床试点研究中显示出高准确率。


<details>
  <summary>Details</summary>
Motivation: 2型糖尿病的识别对初级保健医生具有挑战性，需要开发人工智能临床决策支持系统来辅助医疗专业人员准确诊断糖尿病，特别是在糖尿病专家不易获得的情况下。

Method: 采用混合方法整合专家驱动见解和机器学习技术，开发AI-CDSS系统。使用1298名患者的数据集（训练集650人，测试集648人），算法利用BMI、空腹血糖和糖化血红蛋白等关键特征进行预测。同时进行包含105名患者的临床试点研究，评估系统与非内分泌专科医生的诊断准确性比较。

Result: AI-CDSS显示出高准确度：糖尿病预测准确率99.8%，糖尿病前期99.3%，高危人群识别99.2%，无糖尿病预测98.8%。测试数据集显示内分泌专家与AI-CDSS的一致性达98.8%。试点研究中，AI-CDSS与糖尿病专家的符合率达98.5%，远超非内分泌专科医生的85%符合率。
...
### [74] [Evaluating Memory Structure in LLM Agents](https://arxiv.org/abs/2602.11243)
*Alina Shutova,Alexandra Olenina,Ivan Vinogradov,Anton Sinitsin*

Main category: cs.LG

TL;DR: StructMemEval基准测试评估LLM智能体组织长期记忆结构的能力，而不仅仅是事实回忆
...
Result: 实验显示简单的检索增强LLM在这些任务上表现不佳，而记忆智能体如果被提示如何组织记忆，可以可靠地解决这些任务。但现代LLM在未被提示时并不总能识别记忆结构。

Conclusion: 这凸显了LLM训练和记忆框架未来改进的重要方向，需要提升智能体自主识别和组织记忆结构的能力。
...
### [75] [How Many Features Can a Language Model Store Under the Linear Representation Hypothesis?](https://arxiv.org/abs/2602.11246)
*Nikhil Garg,Jon Kleinberg,Kenny Peng*

Main category: cs.LG

TL;DR: 论文提出了线性表示假设的数学框架，将其分为线性表示和线性可访问性两个部分，研究了在保证线性解码的前提下，需要多少神经元才能表示m个特征，建立了线性压缩感知的上下界。


<details>
  <summary>Details</summary>
Motivation: 研究线性表示假设的数学基础，探讨语言模型中间层是否以线性方式存储特征，以及这种线性表示和线性解码需要多少神经元资源。

Method: 建立线性压缩感知的数学框架，使用随机矩阵构造证明上界，结合秩界和Turán定理证明下界，并扩展到带激活函数和偏置的解码器。

Result: 证明了线性压缩感知的上下界：下界为d = Ω_ε(k²/log k · log(m/k))，上界为d = O_ε(k² log m)，显示了线性可访问性比单纯线性表示要求更强，但神经元仍能指数级存储特征。

Conclusion: 线性可访问性比线性表示要求更高，但神经元仍能在线性解码约束下指数级存储特征，为"叠加假设"提供了理论支持，并揭示了特征表示几何的约束条件。
...
### [76] [HiFloat4 Format for Language Model Inference](https://arxiv.org/abs/2602.11287)
...
TL;DR: HiFloat4 (HiF4)是一种专为深度学习设计的块浮点数据格式，将64个4位元素打包，平均每个值4.5位，通过三级缩放层次结构提高表示空间利用率，支持高效定点矩阵乘法。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习推理需要高效的数据格式来减少内存占用和计算开销，同时保持模型精度。现有4位量化格式在动态范围表示和硬件效率方面存在限制。

Method: 提出HiF4格式：每个单元包含64个4位元素和32位共享缩放元数据。元数据采用三级缩放层次结构，捕获组间和组内动态范围。大组大小（64元素）支持高度定点化的矩阵乘法实现。

Result: 在LLaMA、Qwen、Mistral、DeepSeek-V3.1和LongCat等多个语言模型上的推理实验表明，HiF4在多个模型和下游任务上比最先进的NVFP4格式获得更高的平均准确率。

Conclusion: HiF4格式通过创新的三级缩放层次和大组设计，在保持高精度的同时显著降低硬件面积和功耗，为深度学习推理提供了高效的数据表示解决方案。
...
### [77] [Efficient Analysis of the Distilled Neural Tangent Kernel](https://arxiv.org/abs/2602.11320)
...
Result: 1) 数据集蒸馏可将所需雅可比计算减少20-100倍；2) 每类NTK矩阵具有低有效秩且该特性在压缩过程中得以保留；3) DNTK将NTK计算复杂度降低高达5个数量级，同时保持核结构和预测性能。

Conclusion: 通过NTK调优的数据集蒸馏可以有效地压缩神经正切空间，结合投影方法显著降低NTK计算复杂度，为大规模NTK应用提供了实用的解决方案。
...
### [78] [Predictive Associative Memory: Retrieval Beyond Similarity Through Temporal Co-occurrence](https://arxiv.org/abs/2602.11322)
*Jason Dury*

Main category: cs.LG

TL;DR: 论文提出预测性联想记忆（PAM）架构，通过时间共现学习关联结构，替代传统的基于相似性的记忆检索方法


<details>
  <summary>Details</summary>
Motivation: 当前神经系统中基于相似性检索的记忆方法（相似记忆即有用记忆）无法捕捉生物记忆通过时间共现形成关联的基本特性

Method: 提出预测性联想记忆（PAM）架构，使用JEPA风格预测器在连续经验流中学习时间共现，引入向内JEPA预测可联想到达的过去状态，作为标准向外JEPA的补充

Result: 在合成基准测试中：预测器前1检索是真实时间关联的概率为97%；跨边界Recall@20达到0.421（余弦相似度为0）；区分经验共现与从未共现状态的AUC为0.916（余弦相似度为0.789）；即使在嵌入相似度无信息的跨房间对中，AUC仍达0.849（余弦相似度仅0.503，接近随机）

Conclusion: PAM通过时间共现结构而非嵌入几何实现有效的联想回忆，时间洗牌控制实验证实信号来自真实的时间共现结构，所有结果在不同种子和查询选择中保持稳定
...
### [79] [Structured Hybrid Mechanistic Models for Robust Estimation of Time-Dependent Intervention Outcomes](https://arxiv.org/abs/2602.11350)
*Tomer Meir,Ori Linial,Danny Eytan,Uri Shalit*

Main category: cs.LG

TL;DR: 提出了一种结合机理模型与数据驱动方法的混合方法，用于估计动态系统中的干预效果，特别针对药代动力学中的丙泊酚给药优化问题。


<details>
  <summary>Details</summary>
Motivation: 在医学干预（如麻醉给药）中，需要准确估计干预效果以优化结果。纯数据驱动模型在分布外（OOD）情况下表现不佳，而纯机理模型可能过于简化。需要结合两者的优势来构建更稳健的干预优化模型。

Method: 提出混合机理-数据驱动方法：将动态系统的转移算子分解为参数化（机理）和非参数化（数据驱动）组件，并区分干预相关和无关的动态。对于机理参数未知的情况，采用两阶段程序：先在模拟数据上预训练编码器，再从观测数据中学习修正。

Result: 在周期性摆锤和丙泊酚推注两种不完全机理知识场景下，混合方法在分布外（OOD）情况下优于纯数据驱动和纯机理方法。

Conclusion: 混合机理-数据驱动模型在复杂现实动态系统中具有实现稳健干预优化的潜力，特别适用于医学干预等需要准确估计干预效果的场景。
...
### [80] [Provably Efficient Algorithms for S- and Non-Rectangular Robust MDPs with General Parameterization](https://arxiv.org/abs/2602.11387)
*Anirudh Satheesh,Ziyi Chen,Furong Huang,Heng Huang*

Main category: cs.LG

TL;DR: 本文研究了具有一般策略参数化的鲁棒马尔可夫决策过程，提出了新的梯度估计算法和优化算法，显著改进了样本复杂度，并在平均奖励设置中首次提供了理论保证。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒MDP研究主要局限于表格策略，要么缺乏样本复杂度保证，要么计算成本高昂。本文旨在解决具有一般策略参数化的RMDP问题，特别是在s-矩形和非矩形不确定性集下的挑战。

Method: 1) 将平均奖励RMDP转换为熵正则化的折扣鲁棒MDP，恢复强对偶性；2) 证明一般策略参数化的Lipschitz和Lipschitz-光滑性质；3) 引入多级蒙特卡洛梯度估计器，样本复杂度为$\tilde{\mathcal{O}}(ε^{-2})$；4) 设计投影梯度下降算法（s-矩形不确定性）和Frank-Wolfe算法（非矩形不确定性）。

Result: 1) 多级蒙特卡洛梯度估计器比先前工作改进了$\mathcal{O}(ε^{-2})$因子；2) s-矩形不确定性算法复杂度为$\mathcal{O}(ε^{-5})$；3) 非矩形不确定性算法复杂度为$\mathcal{O}(ε^{-4})$（折扣）和$\mathcal{O}(ε^{-10.5})$（平均奖励）；4) 首次为超越(s,a)-矩形的一般策略参数化RMDP提供样本复杂度保证。

Conclusion: 本文在鲁棒MDP领域取得了重要进展，首次为一般策略参数化提供了理论保证，显著改进了样本复杂度，并为平均奖励设置建立了首个此类保证，为实际应用中的鲁棒强化学习提供了更高效的算法基础。
...
### [81] [Sparse Semantic Dimension as a Generalization Certificate for LLMs](https://arxiv.org/abs/2602.11388)
*Dibyanayan Bandyopadhyay,Asif Ekbal*

Main category: cs.LG

TL;DR: 该论文提出了一种新的理论框架来解释大语言模型（LLM）的泛化能力，认为其泛化能力源于内部表征的稀疏性和低维流形结构，而非参数数量。通过引入稀疏语义维度（SSD）作为复杂度度量，论文展示了该框架能够提供非空泛的泛化保证，并发现了"特征锐度"缩放定律，表明更大模型学习到更可压缩的语义结构。


<details>
  <summary>Details</summary>
Motivation: 传统统计学习理论预测大语言模型应该过拟合，因为其参数数量远超训练数据量，但实际上它们表现出强大的泛化能力。作者旨在解释这种理论与实践的矛盾，并提出新的理论框架来理解LLM的泛化机制。

Method: 提出稀疏语义维度（SSD）作为复杂度度量，通过训练稀疏自编码器（SAE）来提取LLM各层的激活特征词汇表。将LLM和SAE视为冻结的预言机，利用该框架将模型的泛化能力归因于字典的稀疏性而非总参数数量。在GPT-2 Small和Gemma-2B上进行实证验证。

Result: 1. SSD框架能够在实际样本量下提供非空泛的泛化保证证书；2. 发现了"特征锐度"缩放定律：尽管Gemma-2B比GPT-2大一个数量级，但识别其活动流形所需的校准样本显著更少；3. 该框架可作为可靠的安全监控器：分布外输入会触发可测量的"特征爆炸"，通过学习的特征违反有效信号认知不确定性。

Conclusion: 大语言模型的泛化能力源于其内部表征的几何结构——高维参数空间中的激活状态位于低维稀疏流形上。稀疏语义维度框架不仅解释了LLM的泛化机制，还提供了实用的安全监控工具，并揭示了更大模型学习到更可压缩、更独特的语义结构。
...
### [82] [General and Efficient Steering of Unconditional Diffusion](https://arxiv.org/abs/2602.11395)
*Qingsong Wang,Mikhail Belkin,Yusu Wang*

Main category: cs.LG

TL;DR: 提出一种无需梯度指导的无条件扩散模型引导方法，通过噪声对齐和可迁移概念向量实现高效可控生成


<details>
  <summary>Details</summary>
Motivation: 传统条件扩散模型需要重新训练或每步计算梯度，计算开销大。本文旨在开发一种无需推理时梯度指导的高效引导方法

Method: 基于两个观察：1) 噪声对齐：在早期高度噪声阶段使用轻量级离线计算指导信号；2) 可迁移概念向量：在激活空间学习的概念方向可跨时间步和样本迁移。使用递归特征机(RFM)高效识别概念方向

Result: 在CIFAR-10、ImageNet和CelebA数据集上实验表明，相比基于梯度的方法，该方法在精度/质量上有提升，同时实现显著推理加速
...
### [83] [Can We Really Learn One Representation to Optimize All Rewards?](https://arxiv.org/abs/2602.11399)
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习转向利用大模型作为下游任务的先验，社区一直在讨论强化学习中先验的正确形式。前向-后向表示学习试图通过学习对尚未确定的奖励函数的策略先验来实现最优控制，但其训练目标和学习行为仍然神秘。本文旨在澄清这种表示何时存在、其目标优化什么以及实践中如何收敛。

Method: 通过分析与秩匹配、拟合Q评估和收缩映射的联系，提出了一种简化的无监督预训练方法——单步前向-后向表示学习。该方法不追求实现最优控制，而是执行单步策略改进。

Result: 在10个基于状态和图像的连续控制领域中进行实验，单步FB收敛到误差小10^5倍，零样本性能平均提升24%。

Conclusion: 单步前向-后向表示学习通过简化的训练目标，在收敛速度和零样本性能方面显著优于原始FB方法，为强化学习的无监督预训练提供了更有效的解决方案。
...
### [84] [CADET: Context-Conditioned Ads CTR Prediction With a Decoder-Only Transformer](https://arxiv.org/abs/2602.11410)
...
TL;DR: CADET是一个基于解码器Transformer的广告点击率预测模型，通过上下文条件解码、自门控注意力、时间感知位置编码等创新，解决了传统模型在上下文信号处理、离线在线一致性等方面的挑战，在LinkedIn广告平台上实现了11.04%的CTR提升。


<details>
  <summary>Details</summary>
Motivation: 虽然深度学习推荐模型在广告点击率预测领域长期占据主导地位，但生成式推荐器在内容推荐中显示出潜力。然而，将这些基于Transformer的架构应用于广告CTR预测仍面临独特挑战，包括处理评分后上下文信号、保持离线在线一致性以及扩展到工业级工作负载。

Method: 提出了CADET模型，包含五个关键创新：1）上下文条件解码架构，通过多塔预测头显式建模广告位置等评分后信号；2）自门控注意力机制，在表示和交互层面自适应调节信息流；3）基于时间戳的RoPE变体，捕捉从秒到月的时间尺度关系；4）会话掩码策略，防止模型学习不可用会话事件的依赖；5）生产工程优化，包括张量打包、序列分块和自定义Flash Attention内核。

Result: 在线A/B测试显示，CADET相比生产基线模型（DCNv2和序列编码器的混合集成）实现了11.04%的CTR提升。该系统已成功部署在LinkedIn广告平台上，为主流量提供主页信息流赞助更新的服务。

Conclusion: CADET通过创新的解码器Transformer架构成功解决了广告CTR预测中的关键挑战，实现了显著的性能提升，并已在实际生产环境中成功部署，证明了其在工业级广告系统中的实用性和有效性。
...
### [85] [Hierarchical Concept Embedding & Pursuit for Interpretable Image Classification](https://arxiv.org/abs/2602.11448)
*Nghia Nguyen,Tianjiao Ding,René Vidal*

Main category: cs.LG

TL;DR: HCEP框架通过层次化概念嵌入和稀疏编码，在图像分类中实现可解释性，同时考虑概念层次结构，相比传统稀疏编码方法在概念恢复和分类准确性上表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有可解释图像分类模型虽然能从图像中恢复人类可理解的概念，但忽略了概念的层次结构，导致预测正确但解释与概念层次不一致。需要一种能考虑概念层次结构的可解释模型。

Method: 提出层次化概念嵌入与追踪（HCEP）框架：1）在潜在空间中构建概念嵌入的层次结构；2）使用层次化稀疏编码恢复图像中的概念；3）假设图像的正确概念在层次中形成根路径，推导嵌入空间中识别这些概念的条件。

Result: 在真实数据集上的实验表明：1）HCEP在概念精确度和召回率上优于基线方法，同时保持竞争力的分类准确性；2）当样本数量有限时，HCEP在分类准确性和概念恢复方面表现更优；3）层次化稀疏编码能可靠恢复层次化概念嵌入，而传统稀疏编码则失败。

Conclusion: 将层次结构纳入稀疏编码能产生更可靠和可解释的图像分类模型，HCEP框架通过考虑概念层次结构提高了模型的可解释性和性能。
...
### [86] [Assessing Low Back Movement with Motion Tape Sensor Data Through Deep Learning](https://arxiv.org/abs/2602.11465)
*Jared Levy,Aarti Lalwani,Elijah Wyckoff,Kenneth J. Loh,Sara P. Gombatto,Rose Yu,Emilia Farcas*

Main category: cs.LG

TL;DR: 提出MT-AIM深度学习分类框架，通过条件生成模型生成合成数据和关节运动学特征增强，解决Motion Tape传感器数据量小、噪声大的问题，实现下背部运动的高精度分类。


<details>
  <summary>Details</summary>
Motivation: 下背部疼痛是普遍问题，需要监测患者运动以指导物理治疗。传统运动捕捉传感器昂贵且不适合自由生活环境，而新型织物传感器Motion Tape虽便携低成本，但数据集小且噪声大。
...
### [87] [External Division of Two Bregman Proximity Operators for Poisson Inverse Problems](https://arxiv.org/abs/2602.11482)
...
<details>
  <summary>Details</summary>
Motivation: 传统ℓ₁正则化在泊松噪声线性模型恢复稀疏向量时存在估计偏差，需要开发既能促进稀疏性又能减少偏差的新方法。

Method: 1) 引入通过两个Bregman邻近算子的外部除法定义的新算子，促进稀疏解同时减少ℓ₁正则化偏差；2) 将该算子以即插即用方式嵌入到NoLips算法中；3) 通过原始空间和对偶空间两种互补重构阐明算子的几何结构。

Result: 数值测试表明，相比传统的基于Kullback-Leibler散度的方法，所提方法表现出更稳定的收敛行为，在合成数据和图像恢复问题上取得了显著优越的性能。

Conclusion: 提出的外部除法Bregman邻近算子方法能有效解决泊松噪声线性模型中的稀疏向量恢复问题，减少估计偏差，提高恢复性能，为泊松逆问题提供了新的解决方案。
...
### [88] [Partial GFlowNet: Accelerating Convergence in Large State Spaces via Strategic Partitioning](https://arxiv.org/abs/2602.11498)
*Xuan Yu,Xu Wang,Rui Zhu,Yudong Zhang,Yang Wang*

Main category: cs.LG

TL;DR: 该论文提出了一种改进的GFlowNets方法，通过引入规划器将大状态空间划分为重叠的部分状态空间，限制智能体探索范围，从而提高在大状态空间中的收敛速度和生成质量。
...
Method: 引入规划器将整个状态空间划分为重叠的部分状态空间；智能体在有限大小的部分状态空间中高效探索；采用启发式策略在不同部分区域间切换，避免在已完全探索或低奖励区域浪费时间；通过迭代探索部分状态空间学习收敛到整个状态空间中的高奖励子区域。

Result: 在多个广泛使用的数据集上实验表明，该方法在大状态空间中比现有工作收敛更快；不仅生成具有更高奖励的候选者，还显著提高了候选者的多样性。

Conclusion: 通过限制智能体探索范围并采用智能的区域切换策略，提出的方法有效解决了GFlowNets在大状态空间中的收敛问题，实现了更快的收敛速度和更好的生成质量。
...
### [89] [A Generic Framework for Fair Consensus Clustering in Streams](https://arxiv.org/abs/2602.11500)
...
<details>
  <summary>Details</summary>
Motivation: 现有公平共识聚类方法需要存储所有输入聚类，这在处理大规模数据流时存储成本过高。因此需要研究流式模型下的解决方案，在内存有限的情况下处理连续到达的聚类输入。

Method: 提出了一个通用的算法框架，将最接近公平聚类与聚类拟合相结合。该框架适用于任何公平定义，只要能够高效计算近似最接近的公平聚类。算法在流式模型中仅存储对数数量的输入聚类。

Result: 设计了首个常数因子近似算法，在流式设置下仅需存储对数级别的输入。该框架不仅适用于流式场景，在离线重新审视时也能提供改进的近似保证。方法还可扩展到更一般的k-median共识聚类问题。

Conclusion: 本文开创了流式模型下公平共识聚类的研究，提出了高效的内存友好算法框架，该框架具有通用性、可扩展性，并为离线设置提供了改进的近似保证。
...
### [90] [Calibrating an Imperfect Auxiliary Predictor for Unobserved No-Purchase Choice](https://arxiv.org/abs/2602.11505)
*Jiangkai Xiong,Kalyan Talluri,Hanzhao Wang*

Main category: cs.LG

TL;DR: 论文提出校准方法，将不完美的外部预测器转化为统计有效的无购买估计，仅使用购买数据即可恢复无购买概率，无需收集新标签。


<details>
  <summary>Details</summary>
Motivation: 企业通常无法观察消费者的关键行为（如是否从竞争对手购买、选择不购买等），这种缺失的外部选择信息使得即使在简单的多项Logit模型中，市场规模和偏好估计也变得困难。现有方法通常依赖辅助市场份额、聚合或跨市场数据，但辅助预测器可能因在不同渠道、时期、人群训练或由外部机器学习系统产生而存在偏差或校准不当。

Method: 开发了两种校准方法：1）在Logit空间存在仿射错误校准的情况下，通过简单回归识别外部选择效用参数，一致恢复无购买概率；2）在较弱的近似单调条件下，提出基于排名的校准方法，推导有限样本误差界限，清晰分离辅助预测器质量与第一阶段效用学习误差。

Result: 数值实验显示在无购买估计和下游品类决策方面有改进。分析还将估计误差转化为下游品类优化的决策质量，量化校准准确性如何影响收入表现。误差界限明确显示了预测器对齐和效用学习误差的依赖关系。

Conclusion: 该方法能够将不完美的辅助预测器转化为统计有效的无购买估计，仅使用购买数据即可实现，无需收集新的无购买事件标签。同时讨论了结合多个辅助预测器的鲁棒聚合扩展。
...
TL;DR: 研究低比特量化对空间推理任务的影响，发现4比特是敏感过渡区，模块间比特分配比总比特数更重要


<details>
  <summary>Details</summary>
Motivation: 在有限精度预算下保持世界模型的可靠性，研究低比特规划行为主要由总比特数决定还是由比特在不同模块间的分配决定

Method: 使用DINO-WM在Wall规划任务上进行配对目标混合比特评估，测试均匀、混合、非对称和分层量化变体，在两个规划器预算下进行实验

Result: 观察到一致的三阶段模式：8比特和6比特接近FP16，3比特崩溃，4比特对分配敏感；在过渡区保持编码器精度能改善规划，非对称变体显示相同趋势；在严格22单元复制实验中，混合与均匀INT4的优劣取决于预算
...
### [92] [RooflineBench: A Benchmarking Framework for On-Device LLMs via Roofline Analysis](https://arxiv.org/abs/2602.11506)
*Zhen Bi,Xueshu Chen,Luoyang Sun,Yuhang Yao,Qing Shen,Jungang Lou,Cheng Deng*

Main category: cs.LG

TL;DR: 本文提出基于Roofline模型的系统框架，通过操作强度统一架构原语和硬件约束，引入相对推理潜力指标比较不同大语言模型在相同硬件上的效率差异，发现序列长度和模型深度对性能有显著影响，并识别出硬件异构性导致的效率陷阱。
...
Result: 跨多个计算层级的实证分析表明，性能和操作强度的变化受序列长度显著影响；发现模型深度增加时操作强度出现关键回归；识别出硬件异构性导致的效率陷阱；证明结构改进（如多头潜在注意力）可以有效解锁不同硬件基板上的潜在推理能力。

Conclusion: 这些见解为硬件-软件协同设计提供了可操作方向，使神经结构能够与设备智能的物理约束对齐，有助于实现更高效的边缘智能部署。
...
### [93] [Unifying Stable Optimization and Reference Regularization in RLHF](https://arxiv.org/abs/2602.11523)
*Li He,Qiang Qu,He Zhao,Stephen Wan,Dadong Wang,Lina Yao,Tongliang Liu*

Main category: cs.LG

TL;DR: 该论文提出了一种统一的正则化方法，通过显式平衡防止奖励黑客攻击和保持策略更新稳定性的目标，解决了RLHF中的奖励黑客和稳定优化两大核心挑战。


<details>
  <summary>Details</summary>
Motivation: RLHF在提升对齐能力方面取得了显著进展，但仍面临两个核心挑战：奖励黑客攻击和稳定优化问题。现有解决方案通过独立的正则化策略分别处理这些问题，但同时对π₀和π₇进行正则化所产生的隐式权衡尚未得到充分探索。

Method: 提出了一种统一的正则化方法，显式平衡防止奖励黑客攻击和保持策略更新稳定性的目标。该方法产生了一个带权重的监督微调损失，实现了更优的权衡，简化了实现复杂度。

Result: 在多个基准测试上的广泛实验验证表明，该方法在一致性和稳定性方面优于RLHF和在线偏好学习方法，实现了更好的对齐性能。

Conclusion: 通过统一的正则化框架显式平衡奖励黑客攻击预防和稳定优化目标，能够显著提升RLHF的对齐性能和稳定性，同时简化实现复杂度。
...
### [94] [Adaptive Milestone Reward for GUI Agents](https://arxiv.org/abs/2602.11524)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习在训练移动GUI智能体时面临长期信用分配问题，需要在奖励保真度和密度之间权衡：结果奖励保真度高但信号稀疏，过程奖励监督密集但易产生偏差和奖励黑客攻击。

Method: 提出自适应里程碑奖励(ADMIRE)机制：1) 通过从成功探索中动态提取里程碑来构建可验证的自适应奖励系统；2) 集成不对称信用分配策略，对成功轨迹去噪并为失败轨迹提供支架支持。

Result: 在AndroidWorld上，ADMIRE在不同基础模型上实现超过10%的绝对成功率提升；方法展现出强大的泛化能力，在多种RL算法和异构环境（如网页导航和具身任务）中均表现优异。

Conclusion: ADMIRE通过动态里程碑和不对称信用分配有效解决了强化学习中奖励保真度与密度之间的冲突，显著提升了移动GUI智能体在长期任务中的性能，并展现出良好的泛化能力。
...
### [95] [PASCAL: A Phase-Aware Scheduling Algorithm for Serving Reasoning-based Large Language Models](https://arxiv.org/abs/2602.11530)
*Eunyeong Cho,Jehyeon Bang,Ranggi Hwang,Minsoo Rhu*

Main category: cs.LG

TL;DR: PASCAL是一种针对推理型LLM的相位感知调度算法，通过区分推理和回答阶段，优先处理推理以减少首令牌延迟，同时在回答阶段使用受控抢占和令牌调步来保证用户体验质量。


<details>
  <summary>Details</summary>
Motivation: 基于推理的LLM（如使用思维链推理）带来了新的服务挑战，其扩展的推理阶段会延迟用户可见的输出并增加首令牌时间。现有的LLM服务框架无法区分推理和回答阶段，导致在GPU内存约束下性能下降。

Method: 提出PASCAL相位感知调度算法：1）优先处理推理阶段以减少TTFT；2）在回答阶段使用受控抢占和令牌调步来保持QoE；3）采用分层调度器结合实例级放置和实例内执行；4）在阶段边界启用动态迁移以平衡负载和减少干扰。

Result: 在DeepSeek-R1-Distill-Qwen-32B基准测试中，PASCAL将尾部TTFT降低了高达72%，同时保持了回答阶段的SLO达成率，证明了相位感知调度对于推理型LLM部署的重要性。

Conclusion: 相位感知调度对于推理型LLM部署至关重要，PASCAL通过区分推理和回答阶段并采用相应的优化策略，显著改善了服务性能，为未来推理型LLM的高效部署提供了有效解决方案。
...
### [96] [AltTS: A Dual-Path Framework with Alternating Optimization for Multivariate Time Series Forecasting](https://arxiv.org/abs/2602.11533)
*Zhihang Yuan,Zhiyuan Liu,Mahesh K. Marina*

Main category: cs.LG

TL;DR: ALTTS：一种双路径框架，通过交替优化将自回归建模与跨维度关系建模解耦，解决了多变量时间序列预测中的优化冲突问题
...
Method: 提出ALTTS双路径框架：AR路径使用线性预测器处理自回归动态，CR路径使用配备跨关系自注意力（CRSA）的Transformer处理跨维度关系。通过交替优化协调两个分支，隔离梯度噪声并减少跨块干扰。

Result: 在多个基准测试上的广泛实验表明，ALTTS始终优于先前方法，在长期预测方面改进最为显著。这表明精心设计的优化策略而非更复杂的架构是多变量时间序列预测进步的关键驱动力。

Conclusion: 通过将自回归和跨关系建模解耦并采用交替优化策略，ALTTS有效解决了多变量时间序列预测中的优化冲突问题，为长期预测提供了更稳定和准确的解决方案。
...
### [97] [Krause Synchronization Transformers](https://arxiv.org/abs/2602.11534)
*Jingkun Liu,Yisong Yue,Max Welling,Yue Song*

Main category: cs.LG

TL;DR: 本文提出Krause Attention机制，用有界置信度共识动力学替代传统的全局softmax注意力，通过距离基础的局部稀疏交互缓解注意力坍缩问题，并将计算复杂度从二次降至线性。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer的自注意力机制采用全局归一化的softmax权重，导致所有token在每个层都竞争影响力。这种跨深度的交互模式引发强烈的同步动态，倾向于收敛到主导模式，与表征坍缩和注意力下沉现象相关。

Method: 引入Krause Attention机制，受有界置信度共识动力学启发，用距离基础的局部化、选择性稀疏交互替代基于相似度的全局聚合。该方法将交互限制在局部邻域内，促进结构化局部同步而非全局混合。

Result: 实验在视觉（ViT在CIFAR/ImageNet）、自回归生成（MNIST/CIFAR-10）和大语言模型（Llama/Qwen）上展示了一致的性能提升，同时显著减少了计算量。有界置信度动态被证明是注意力机制的可扩展且有效的归纳偏置。

Conclusion: Krause Attention通过有界置信度共识动力学解决了传统注意力机制的全局同步问题，缓解了注意力坍缩现象，同时将计算复杂度从二次降至线性，为注意力机制提供了更高效且有效的替代方案。
...
### [98] [Native Reasoning Models: Training Language Models to Reason on Unverifiable Data](https://arxiv.org/abs/2602.11549)
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的大规模推理模型训练范式（SFT+RLVR）存在严重限制：依赖高质量人工标注推理数据、成本高昂、可能嵌入人类认知偏见，且强化学习阶段仅限于数学和编程等可客观评估的领域，无法应用于广泛的不可验证任务。

Method: NRT将推理过程视为隐变量，采用统一的训练目标，将推理建模为优化问题，内在奖励那些增加模型产生正确答案可能性的路径。该框架允许分析先前方法的内在失败模式（如策略崩溃），并系统设计更稳健的奖励聚合函数，创建自我强化的反馈循环。

Result: 在Llama和Mistral模型系列上的实证评估表明，NRT在无验证器方法中实现了最先进的性能，显著优于标准SFT基线和先前的无验证器RL方法。在复杂推理领域表现尤为突出，对策略崩溃具有高鲁棒性。

Conclusion: NRT提供了一种通用、可扩展的路径，用于构建更强大、更广泛适用的推理系统，克服了当前训练范式的关键限制。
...
### [99] [TS-Memory: Plug-and-Play Memory for Time Series Foundation Models](https://arxiv.org/abs/2602.11550)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列基础模型在零样本预测方面表现良好，但在分布偏移下适应下游领域仍具挑战。现有方法面临权衡：参数化适应会导致灾难性遗忘且需要昂贵的多领域维护，而非参数化检索虽然能改进预测但会因数据存储搜索带来高推理延迟。

Method: 提出参数化记忆蒸馏，实现为TS-Memory轻量级记忆适配器。分两阶段训练：1）构建离线的、防泄漏的kNN教师，从检索到的未来序列合成置信度感知的分位数目标；2）通过置信度门控监督将检索诱导的分布校正蒸馏到轻量级记忆适配器中。推理时，TS-Memory以恒定时间开销融合记忆和主干预测，实现检索自由部署。

Result: 在不同时间序列基础模型和基准测试上的实验表明，TS-Memory在点预测和概率预测方面均优于代表性适应方法，效率与冻结主干模型相当。

Conclusion: TS-Memory通过参数化记忆蒸馏有效解决了时间序列基础模型在分布偏移下的适应问题，实现了检索自由的高效部署，在保持效率的同时显著提升了预测性能。
...
TL;DR: 该研究分析了多类分类中随机最速下降法的隐式偏差，揭示了批次大小、动量和方差缩减如何影响最大间隔行为和收敛速率，阐明了随机优化与全批次行为对齐的条件。


<details>
  <summary>Details</summary>
Motivation: 许多广泛使用的优化方法（如SignSGD和Muon）可以被解释为不同范数诱导几何下的最速下降法。本研究旨在理解小批量随机最速下降法在多类分类中的隐式偏差，探索批次大小、动量和方差缩减如何塑造极限最大间隔行为和收敛速率。

Method: 研究采用统一的几何分析框架，在一般逐元素范数和Schatten-p范数下，分析小批量随机最速下降法的收敛行为。通过理论分析探索不同参数配置（批次大小、动量、方差缩减）对优化过程的影响。

Result: 1) 无动量时，收敛仅在大批次下发生，产生批次相关的间隔间隙但保持全批次收敛速率；2) 动量通过批次-动量权衡实现小批次收敛，但会减慢收敛速度；3) 方差缩减可恢复任何批次大小的精确全批次隐式偏差，但收敛速率较慢；4) 批次大小为1的无动量最速下降法收敛到完全不同的偏差，揭示了纯随机更新的关键局限性。
...
### [101] [Brain4FMs: A Benchmark of Foundation Models for Electrical Brain Signal](https://arxiv.org/abs/2602.11558)
*Fanqi Shen,Enhong Yang,Jiahe Li,Junru Hong,Xiaoran Pan,Zhizhang Yuan,Meng Li,Yang Yang*

Main category: cs.LG

TL;DR: 本文介绍了Brain4FMs平台，这是一个用于评估脑基础模型（BFMs）的开放评估平台，旨在解决该领域缺乏统一方法论理解和标准化评估框架的问题。


<details>
  <summary>Details</summary>
Motivation: 脑基础模型（BFMs）正在通过从神经信号中实现可扩展和可迁移的学习来改变神经科学，推动了临床诊断和前沿神经科学探索。然而，尽管BFMs快速发展，该领域缺乏对现有方法的统一理解和标准化评估框架。

Method: 1. 从模型角度，在自监督学习（SSL）分类法下组织BFMs；2. 从数据集角度，总结常见下游任务并整理代表性公共数据集；3. 构建Brain4FMs开放评估平台，集成15个代表性BFMs和18个公共数据集，提供即插即用接口。

Result: 开发了Brain4FMs平台，能够进行标准化比较和分析预训练数据、SSL策略和架构如何影响泛化能力和下游性能，为开发更准确和可迁移的BFMs提供指导。

Conclusion: Brain4FMs平台填补了脑基础模型领域缺乏统一评估框架的空白，通过标准化比较和分析，有助于指导开发更准确和可迁移的脑基础模型，推动神经科学和临床应用的进步。
...
### [102] [Gradient Compression May Hurt Generalization: A Remedy by Synthetic Data Guided Sharpness Aware Minimization](https://arxiv.org/abs/2602.11584)
...
<details>
  <summary>Details</summary>
Motivation: 梯度压缩在联邦学习中虽然提高了通信效率，但会导致损失景观变得尖锐，特别是在非IID数据分布下，这会阻碍模型的泛化能力。现有的锐度感知最小化（SAM）方法在联邦学习中由于数据异构性而难以准确估计全局扰动，特别是在结合模型更新压缩时效果受限。

Method: 提出FedSynSAM方法，利用全局模型轨迹构建合成数据，从而准确估计全局扰动。该方法在联邦学习框架中结合了梯度压缩和锐度感知最小化，通过合成数据来克服数据异构性带来的扰动估计不准确问题。

Result: 建立了算法的收敛性理论，并通过大量实验验证了FedSynSAM的有效性。该方法在保持通信效率的同时，改善了梯度压缩导致的损失景观尖锐化问题，提升了模型的泛化能力。

Conclusion: FedSynSAM成功解决了联邦学习中梯度压缩导致的损失景观尖锐化问题，通过利用全局模型轨迹构建合成数据来准确估计全局扰动，在保持通信效率的同时提升了模型的泛化性能。
...
### [103] [SkillRater: Untangling Capabilities in Multimodal Data](https://arxiv.org/abs/2602.11615)
*Naveen Sahi,Jeremy Dohmann,Armen Aghajanyan,Akshat Shrivastava*

Main category: cs.LG

TL;DR: SkillRater提出多维数据质量评估框架，将传统单一质量评分分解为针对不同模型能力的专门评分器，通过渐进式选择规则在不同训练阶段保留多样性和高质量样本
...
Method: 引入SkillRater框架：1) 将数据筛选分解为专门评分器，每个评分器对应一种能力，通过元学习在不相交的验证目标上训练；2) 采用渐进式选择规则：在每个训练阶段，样本只要被任一评分器评为高于随时间收紧的阈值就被保留，早期保持多样性，后期集中于高价值样本

Result: 在视觉语言模型上验证，将质量分解为视觉理解、OCR和STEM推理三个能力维度。在20亿参数规模下，相比未筛选基线，SkillRater在视觉理解上提升5.63%，OCR提升2.00%，STEM推理提升3.53%。学习到的评分器信号接近正交，证实分解捕获了真正独立的质量维度

Conclusion: 多维质量评估框架优于传统单一评分方法，能够更有效地支持多能力模型训练，评分器信号的正交性解释了为什么该方法优于未筛选训练和单一学习筛选
...
### [104] [How Well Do Large-Scale Chemical Language Models Transfer to Downstream Tasks?](https://arxiv.org/abs/2602.11618)
...
<details>
  <summary>Details</summary>
Motivation: 验证化学领域中普遍认为的增加训练资源（模型规模、数据集大小、训练计算）能同时改善预训练损失和下游任务性能的假设是否成立。

Method: 通过在不同训练资源规模下预训练化学语言模型，并在多样化的分子性质预测任务上测量迁移性能，同时使用基于Hessian或损失景观的替代指标进行评估。

Result: 预训练损失随训练资源增加而持续降低，但下游任务性能提升有限；替代指标也无法有效估计CLMs的下游性能；在某些条件下，尽管预训练指标持续改善，下游性能却会饱和甚至下降。
...
TL;DR: 本文重新审视了使用概率值（包括Shapley和Banzhaf值）来解释决策树局部预测值的特征排序问题，提出了TreeGrad算法来直接优化联合目标函数，并开发了TreeGrad-Ranker和TreeGrad-Shap等新方法。


<details>
  <summary>Details</summary>
Motivation: 传统的概率值方法在解释决策树局部预测值时存在可靠性问题，特别是在同时优化插入和删除指标方面表现不佳。作者发现这两个指标的协同优化与最大化特征子集预测值同时最小化其补集预测值的联合优化密切相关。

Method: 提出了TreeGrad算法，能够在O(L)时间内计算联合目标函数多线性扩展的梯度（包含加权Banzhaf值）。基于此开发了TreeGrad-Ranker（通过聚合梯度优化联合目标生成特征排序）和TreeGrad-Shap（计算Beta Shapley值的数值稳定算法）。还开发了TreeProb作为Linear TreeShap的泛化版本。

Result: 理论证明概率值在解决联合优化问题上通常不可靠。实验显示TreeGrad-Shap在计算Shapley值时的数值误差比Linear TreeShap小10^15倍。TreeGrad-Ranker在插入和删除指标上表现显著更好。

Conclusion: 通过直接优化联合目标函数而非依赖概率值，可以更可靠地生成决策树局部预测的特征解释。TreeGrad系列方法在数值稳定性和解释质量方面优于现有方法。
...
TL;DR: 该论文提出了一种双分支图提示学习方法GP2F，通过冻结分支保留预训练知识，适配分支进行任务特定调整，在跨域场景下优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 图提示学习从同域转向跨域场景更接近实际应用，但现有方法在域偏移下为何有效尚不明确。作者观察到代表性GPL方法与全微调和线性探测基线表现相当，需要深入理解提示机制。

Method: 提出GP2F双分支方法：1)冻结分支保留预训练知识；2)适配分支使用轻量适配器进行任务特定调整。通过对比损失和拓扑一致性损失在拓扑约束下进行自适应融合。

Result: 在跨域少样本节点和图分类任务上的大量实验表明，该方法优于现有方法。

Conclusion: 理论分析表明联合利用两个互补分支比单独使用任一分支能获得更小的估计误差，证明跨域GPL受益于预训练知识与任务特定适应的集成。GP2F通过显式实例化这两个极端并自适应融合，实现了更好的性能。
...
TL;DR: TIP提出了一种针对梯度反转攻击的联邦学习防御框架，通过模型可解释性和频域分析，选择性扰动关键卷积通道的高频成分，在保护隐私的同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然保护数据本地性，但梯度交换使其易受梯度反转攻击，现有防御方法如差分隐私对所有参数添加噪声，严重损害模型性能和收敛稳定性。

Method: TIP采用双目标策略：1) 使用Grad-CAM量化通道敏感性，动态识别编码主要语义特征的关键卷积通道；2) 通过离散傅里叶变换将选定核转换到频域，选择性地向高频谱注入校准扰动。

Result: 在基准数据集上的实验表明，TIP使重构图像在视觉上无法识别，同时保持全局模型精度与非私有基线相当，在隐私-效用权衡和可解释性方面显著优于现有DP防御方法。

Conclusion: TIP通过选择性扰动高频成分，有效破坏图像重构所需的细粒度细节，同时保留对模型精度至关重要的低频信息，实现了更好的隐私保护与模型性能平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：拓扑驱动方法无法充分利用丰富的语义信息；LLM基方法在合成OOD先验时存在可靠性-信息性不平衡问题，且依赖专门架构无法结合拓扑层面的有效见解。需要一种能同时利用语义和结构信息的方法。

Method: LG-Plug采用LLM引导的即插即用策略：1) 对齐拓扑和文本表示生成细粒度节点嵌入；2) 通过聚类迭代LLM提示生成共识驱动的OOD暴露；3) 利用轻量级集群码本和启发式采样降低LLM查询时间成本；4) 将OOD暴露作为正则化项分离ID和OOD节点。

Result: 该方法能够生成更可靠的OOD暴露，避免了现有方法中OOD先验的可靠性-信息性不平衡问题，同时能够与现有检测器无缝集成，利用了拓扑层面的有效见解。

Conclusion: LG-Plug提供了一种有效的文本属性图OOD检测解决方案，通过LLM引导的即插即用策略，同时利用了语义和结构信息，克服了现有方法的局限性，实现了更好的OOD检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 尽管UMAP被广泛使用且通过代数拓扑视角呈现，但其与经典谱方法的确切关系一直是非正式的。本研究旨在建立UMAP与谱聚类之间的理论联系。
...
Conclusion: 该研究将UMAP、对比学习和谱聚类统一在一个框架下，为关于UMAP行为的多个经验观察提供了理论基础。
...
TL;DR: 提出了一种完全一阶的在线双层优化算法，无需Hessian-向量乘积，通过拉格朗日函数重构问题，实现了O(1+V_T+H_{2,T})的遗憾界，并开发了改进的自适应内层迭代版本，达到O(√T+V_T)的遗憾。


<details>
  <summary>Details</summary>
Motivation: 现有在线双层优化算法主要基于超梯度下降，需要Hessian-向量乘积计算，计算成本高。作者希望开发完全一阶的算法来消除这一计算瓶颈。

Method: 将原始在线双层优化问题重构为带不等式约束的单层在线问题，构造拉格朗日函数序列，避免了隐式微分带来的Hessian-向量乘积需求。提出了完全一阶算法和带有自适应内层迭代方案的改进版本。

Result: 基本算法实现了O(1+V_T+H_{2,T})的遗憾界，改进的自适应版本实现了O(√T+V_T)的遗憾界，当V_T≥O(√T)时具有优势，且完全消除了对内层最优解漂移变化的依赖。

Conclusion: 成功开发了无需Hessian-向量乘积的完全一阶在线双层优化算法，显著降低了计算复杂度，同时提供了理论保证，为在线双层优化提供了更高效的计算框架。
...
TL;DR: 使用光学动作捕捉系统和机器学习分析跑步步态，识别膝关节损伤相关模式，CNN模型在PFPS和ITBS分类中表现最佳


<details>
  <summary>Details</summary>
Motivation: 跑步活动普遍但膝关节损伤率高，特别是髌股疼痛综合征（PFPS）和髂胫束综合征（ITBS）。识别与这些损伤相关的步态模式可以改善临床决策，这需要能够捕获和分析时间运动学数据的精确系统。

Method: 使用光学动作捕捉系统分析839个跑步机记录的公共数据集，专注于支撑相，使用关节和节段角度时间序列和离散点值。研究三个分类任务：健康vs损伤、健康vs PFPS、健康vs ITBS。测试不同特征空间（传统点度量、完整时间序列、混合表示）和多种模型（KNN、高斯过程、决策树、CNN、LSTM）。使用可解释性工具（Shapley值、显著性图、Grad-CAM）解释模型行为。

Result: 结合时间序列和点值显著改善检测效果。深度学习模型优于传统算法，CNN达到最高准确率：PFPS 77.9%、ITBS 73.8%、综合损伤类别71.43%。

Conclusion: 研究结果表明，动作捕捉系统与先进机器学习结合具有识别膝关节损伤相关跑步模式的潜力，为临床决策提供数据支持。
...
TL;DR: DRACO是一个用于评估深度研究系统性能的基准测试，包含来自10个领域、40个国家信息源的复杂研究任务，基于真实使用模式构建，从四个维度评估输出质量。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏能够全面评估深度研究系统在复杂、开放式研究任务上性能的基准测试，需要基于真实使用模式构建的评估框架来准确衡量研究系统的准确性、完整性和客观性。

Method: 从Perplexity Deep Research的真实使用请求中采样任务，经过过滤和增强处理，确保任务匿名、开放、复杂且可客观评估，涵盖10个领域和40个国家信息源，建立包含四个维度的评估标准。

Result: 创建了DRACO基准测试，包含复杂的深度研究任务，任务具有匿名性、开放性、复杂性、可客观评估性，并代表真实世界研究用例的广泛范围，基准测试已公开可用。

Conclusion: DRACO为评估深度研究系统提供了一个全面、基于真实使用模式的基准测试，能够从准确性、完整性、客观性和引用质量四个维度系统评估研究系统的性能。
...
TL;DR: ANML框架通过结合梯度信号和外部数据来源信息，为训练样本分配基于贡献者质量的权重，显著提升模型性能并实现可追溯性。


<details>
  <summary>Details</summary>
Motivation: 当前AI训练流程对所有样本一视同仁，无法区分高质量专家数据和未经验证数据之间的差异，导致模型性能受限且缺乏可追溯性。

Method: 提出ANML框架，基于四个质量因子（梯度一致性q、验证状态v、贡献者声誉r、时间相关性T）为训练样本分配权重，采用两阶段自适应门控机制确保性能不低于最佳基线。

Result: 在5个数据集上实现33-72%的错误率降低；质量加权训练数据效率高，20%高质量数据性能优于100%均匀加权数据47%；贡献者级溯源比样本级方法提升1.3-5.3倍。

Conclusion: ANML通过结合模型观察（梯度信号）和系统知识（数据来源信息），实现了质量加权的训练方法，显著提升模型性能、数据效率和可追溯性，并能抵御战略攻击。
...
TL;DR: SpiralFormer是一种多分辨率循环Transformer，通过在不同尺度上迭代执行循环计算，实现了比传统循环Transformer和非循环基线更好的参数和计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有循环Transformer虽然在计算深度和参数深度解耦方面有优势，但通常性能不如非循环基线，且都在固定全token分辨率下运行，忽略了在压缩潜在表示上计算带来的效率潜力。

Method: 提出SpiralFormer，采用多分辨率循环调度机制，在不同尺度上执行循环计算，使模型能够学习层次化依赖关系，并在不同尺度上实现迭代功能专门化。

Result: 在160M到1.4B不同模型规模下，SpiralFormer在参数和计算效率上都优于循环和非循环基线，证明了序列分辨率作为扩展递归架构的潜在维度。

Conclusion: 多分辨率循环机制使Transformer能够更有效地学习层次化表示，为递归架构的扩展提供了新的方向，即通过序列分辨率维度进行扩展。
...
TL;DR: TabSieve是一个选择-预测框架，通过先选择信息性行作为证据，再基于证据预测缺失目标，解决了表格预测中证据利用不一致和噪声干扰的问题。


<details>
  <summary>Details</summary>
Motivation: 现有表格模型通常进行实例级推理，基于LLM的提示方法往往脆弱，模型不能一致地利用相关行，且噪声上下文会降低性能。

Method: 提出TabSieve框架：1）先选择少量信息性行作为证据；2）基于所选证据预测缺失目标。构建TabSieve-SFT-40K数据集，使用强教师模型合成高质量推理轨迹。引入TAB-GRPO强化学习配方，通过单独奖励联合优化证据选择和预测正确性，并通过动态任务优势平衡稳定混合回归和分类训练。

Result: 在包含75个分类表和52个回归表的基准测试中，TabSieve在不同样本预算下持续提升性能，分类任务平均提升2.92%，回归任务平均提升4.45%，优于次优基线。分析表明TabSieve更关注所选证据，提高了对噪声上下文的鲁棒性。

Conclusion: TabSieve通过显式的证据选择和预测框架，使证据使用变得明确且可审计，有效解决了表格预测中证据利用不一致的问题，提高了模型性能和鲁棒性。
...
TL;DR: 提出势能门控方法，通过基于物理势能函数调节观测噪声协方差，在双阱随机动力系统中实现鲁棒状态估计，相比传统方法显著提升精度。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯滤波器在处理双阱随机动力系统时，对所有状态空间区域采用相同的观测噪声处理方式，无法有效处理势垒附近观测不可靠的问题。需要一种基于物理机制的鲁棒滤波方法，能够根据系统势能自动调节对观测的信任度。

Method: 提出势能门控方法，通过已知或假设的势能函数局部值来调制贝叶斯滤波器的观测噪声协方差：当状态接近势能最小值时信任观测，当接近分隔亚稳态阱的势垒时逐渐减少对观测的信任。该方法在扩展卡尔曼滤波、无迹卡尔曼滤波、集合卡尔曼滤波、自适应卡尔曼滤波和粒子滤波中实现，仅需两个额外超参数。

Result: 在Ginzburg-Landau双阱过程（含10%异常值污染）的合成基准测试中，相比标准扩展卡尔曼滤波，RMSE提升57-80%（所有结果统计显著，p<10^{-15}）。仅使用到最近阱距离的朴素拓扑基线方法达到57%提升，表明连续能量景观额外贡献约21个百分点。方法对参数误设具有鲁棒性：即使假设势参数偏离真实值50%，改进仍不低于47%。在外部强迫和自发Kramers型跃迁比较中，门控方法在噪声诱导跃迁下保持68%改进，而朴素基线降至30%。

Conclusion: 势能门控是一种有效的物理信息鲁棒滤波方法，能够显著提升双阱随机动力系统的状态估计精度，对参数误设具有鲁棒性，并在古气候数据（Dansgaard-Oeschger事件）应用中展示了实用价值，异常值比例解释了91%的滤波器改进方差。
...
TL;DR: DICE：基于扩散大语言模型的CUDA内核生成模型，通过构建高质量训练数据集CuKe和双阶段强化学习框架，在CUDA内核生成任务上超越自回归和扩散模型


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（dLLMs）因其并行令牌生成能力成为自回归LLMs的有力替代方案，特别适合需要整体结构规划和非顺序优化的代码生成任务。然而，为CUDA内核生成定制dLLMs面临挑战，包括任务高度专业化以及高质量训练数据的严重缺乏。

Method: 1. 构建CuKe数据集：优化的监督微调数据集，用于高性能CUDA内核生成；2. 提出BiC-RL框架：双阶段强化学习框架，包括CUDA内核填充阶段和端到端CUDA内核生成阶段；3. 开发DICE模型：基于扩散大语言模型的CUDA内核生成模型，包含1.7B、4B和8B三个参数规模。

Result: 在KernelBench上的广泛实验表明，DICE在CUDA内核生成任务上显著优于同等规模的自回归和扩散LLMs，建立了新的最先进水平。

Conclusion: 通过构建高质量训练数据集和创新的双阶段强化学习框架，DICE模型成功将扩散大语言模型应用于CUDA内核生成任务，展示了扩散模型在代码生成领域的潜力，特别是在需要并行生成和整体结构规划的场景中。
...
TL;DR: 本文提出了一种新的参数高效微调方法——Mode Specialization，通过冻结基础权重并学习每个神经元的阈值和增益来实现激活空间的适应，相比LoRA在可解释性和参数效率方面有优势。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法（如LoRA）虽然有效，但权重增量难以进行机制性解释，且无法直接揭示哪些内部计算被重用或绕过。受神经调节启发，本文探索将适应视为模式变化而非权重重写。

Method: 提出Mode Specialization方法：冻结基础权重，学习每个神经元的阈值和增益。训练时使用平滑门控决定神经元激活是否参与；推理时可硬化门控以获得显式条件计算和神经元级归因。

Result: 在MNIST（0°）和旋转MNIST（45°）任务上验证：相比冻结基线，该方法使用每层仅几百个可训练参数就能提高旋转准确率，并表现出部分激活稀疏性（少数单元强烈激活）。相比LoRA，在准确率上有所牺牲，但参数更少且机制更可解释。

Conclusion: Mode Specialization提供了一种激活空间的参数高效微调方法，具有更好的可解释性和参数效率，但表达能力受限于冻结基础模型是否包含目标模式所需特征。
...
TL;DR: UFO是一种新型架构，结合U-Net的多尺度特征提取、Transformer的全局建模和神经控制微分方程的连续时间动力学，用于不规则采样时间序列的概率预测，在准确性和推理速度上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 不规则采样时间序列的概率预测在医疗和金融等领域至关重要，但现有神经控制微分方程方法存在计算速度慢、顺序处理限制可扩展性、缺乏全局上下文访问等问题。

Method: 提出UFO架构，无缝集成U-Net的可并行多尺度特征提取、Transformer的强大全局建模能力和神经控制微分方程的连续时间动力学，构建完全因果、可并行的模型，实现全局感受野同时保持对局部时间动态的敏感性。

Result: 在五个标准基准测试（包括规则和不规则采样时间序列）上，UFO在预测准确性上始终优于10个最先进的神经基线方法，推理速度比传统神经控制微分方程快达15倍，在长序列和高维序列上表现一致强劲。

Conclusion: UFO通过整合U-Net、Transformer和神经控制微分方程的优势，成功解决了不规则采样时间序列概率预测中的计算效率和全局上下文建模问题，实现了准确性和速度的双重提升。
...
<details>
  <summary>Details</summary>
Motivation: 在二进制分类系统中，决策阈值将模型分数转化为行动。在多租户Score-as-a-Service环境中，模型重新训练会改变分数分布，使现有阈值失效，需要协调数百个客户端更新阈值，消耗大量人力并导致模型停滞。

Method: MUSE采用动态意图路由共享模型，结合两级分数转换，将模型输出映射到稳定的参考分布，从而解耦模型分数与客户端决策边界。

Result: MUSE在Feedzai大规模部署，每秒处理超过1000个事件，过去12个月处理超过550亿个事件，跨数十个租户，同时保持高可用性和低延迟保证。

Conclusion: MUSE将模型上线时间从数周缩短到几分钟，增强模型对变化攻击的韧性，节省数百万美元的欺诈损失和运营成本。
...
Method: 提出温度自适应元策略优化(TAMPO)框架，采用分层双循环：内循环使用元策略选择的温度采样轨迹更新LLM策略；外循环通过奖励最大化高优势轨迹似然来更新温度分布

Result: 在五个数学推理基准测试中，TAMPO优于使用固定或启发式温度的基线方法，证明了温度作为可学习元策略在LLM强化学习中的有效性

Conclusion: 温度可以作为有效的可学习元策略，实现LLM强化学习中的自适应探索，TAMPO框架能够在线适应而不需要额外rollout，直接将探索与策略改进对齐
...
TL;DR: SPECTRE是一种新的公平分类方法，无需访问人口统计信息，通过调整傅里叶特征映射的频谱并约束最坏情况分布与经验分布的偏差，解决了现有方法过度关注异常值或悲观场景的问题。


<details>
  <summary>Details</summary>
Motivation: 现有公平分类方法大多需要完整的群体信息，这在实践中很少能满足。而无群体信息的公平方法通常采用鲁棒优化技术，但其效果严重依赖于不确定性集的选择，往往过度关注异常值或过于悲观的场景，损害了整体性能和公平性。

Method: 提出SPECTRE方法，这是一种极小极大公平方法，通过调整简单傅里叶特征映射的频谱，并约束最坏情况分布与经验分布的偏差程度。该方法不需要访问人口统计群体信息。

Result: 在美国社区调查数据集（涉及20个州）上的广泛实验表明，SPECTRE在公平性保证方面提供了最高的平均值和最小的四分位距，甚至优于那些能够访问人口统计群体信息的最先进方法。

Conclusion: SPECTRE方法在无需群体信息的情况下实现了优越的公平性保证，同时提供了可计算的最坏情况误差界限的理论分析，并描述了导致这些极端性能的最坏情况分布特征。
...
TL;DR: APST框架通过重复采样相同提示来评估LLM在持续使用下的安全性和可靠性，发现传统基准测试可能掩盖了模型在重复推理中的实际故障率差异。


<details>
  <summary>Details</summary>
Motivation: 传统LLM安全评估主要关注广度任务测试，但实际部署中更关键的是重复推理下的操作故障风险。需要一种深度导向的评估框架来评估LLM在持续使用中的响应一致性和安全性。
...
Conclusion: 浅层的单样本评估可能掩盖持续使用下的可靠性差异。APST通过提供重复推理下的评估框架，补充现有基准，连接基准对齐和部署导向的风险评估。
...
TL;DR: 本文提出CIR框架，通过约束初始表征来提升离策略强化学习的样本效率，包含Tanh激活、跳跃连接和凸Q学习三个组件，在连续控制任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽然尝试提升离策略强化学习的样本效率，但忽视了直接约束输入数据初始表征的潜力，这可以直观地缓解分布偏移问题并稳定训练。
...
Conclusion: 通过约束初始表征的CIR框架有效提升了离策略强化学习的样本效率和训练稳定性，为强化学习算法设计提供了新的思路。
...
### [125] [From Path Signatures to Sequential Modeling: Incremental Signature Contributions for Offline RL](https://arxiv.org/abs/2602.11805)
...
<details>
  <summary>Details</summary>
Motivation: 标准路径签名将时间结构压缩为单一全局对象，限制了其在需要逐步反应的决策问题中的适用性。需要一种能保留签名表达能力同时显式捕捉时间演化的表示方法。

Method: 提出增量签名贡献(ISC)方法，将截断路径签名分解为张量代数空间中的时间有序序列元素，对应由最后路径增量引起的增量贡献。基于ISC构建ISC-Transformer模型，将ISC集成到标准Transformer架构中。

Result: 在HalfCheetah、Walker2d、Hopper和Maze2d等环境中评估，包括延迟奖励和降级数据集设置。结果表明ISC方法为时间敏感控制任务提供了理论上有基础且实际有效的路径处理替代方案。

Conclusion: ISC方法保留了签名的代数结构和表达能力，同时使其内部时间演化显式化，能够通过序列建模方法处理基于签名的表示。与完整签名相比，ISC对瞬时轨迹更新具有固有敏感性，这对敏感且需要稳定性的控制动力学至关重要。
...
### [126] [Deep Kernel Fusion for Transformers](https://arxiv.org/abs/2602.11808)
*Zixi Zhang,Zhiwen Mo,Yiren Zhao,Robert Mullins*

Main category: cs.LG

TL;DR: DeepFusionKernel：针对长上下文Agentic LLM推理的内存带宽瓶颈，通过深度融合内核减少HBM流量、提升缓存重用，在H100和A100上分别实现13.2%和9.7%的加速。
...
Method: 提出DeepFusionKernel深度融合内核，通过减少HBM（高带宽内存）流量和提升缓存重用来优化性能。与SGLang集成并配合内核调度器，确保在不同生成长度下的稳定加速。
...
### [127] [CAAL: Confidence-Aware Active Learning for Heteroscedastic Atmospheric Regression](https://arxiv.org/abs/2602.11825)
*Fei Jiang,Jiyang Xia,Junjie Yu,Mingfei Sun,Hugh Coe,David Topping,Dantong Liu,Zhenhui Jessie Li,Zhonghua Zheng*

Main category: cs.LG

TL;DR: 提出CAAL框架，通过解耦不确定性训练和置信感知采集函数，在异方差噪声设置下优化高成本大气颗粒物属性数据的主动学习样本选择。


<details>
  <summary>Details</summary>
Motivation: 大气颗粒物的毒性和吸湿性等关键属性对健康气候影响评估至关重要，但测量或模拟成本高昂，标记数据稀缺。常规观测数据与这些属性之间的映射存在异方差噪声，传统主动学习方法在有限预算下容易浪费资源于噪声主导区域。
...
Conclusion: CAAL框架通过有效区分认知不确定性和偶然不确定性，在异方差噪声环境下实现了更高效的样本选择，为大气科学等领域高成本标记数据的主动学习提供了稳健方法。
...
TL;DR: 本文提出了一种名为"On-Policy SFT"的框架，通过分布判别理论和两种技术（IDFT和Hinted Decoding）来弥合监督微调与强化学习在泛化性能上的差距。


<details>
  <summary>Details</summary>
Motivation: 监督微调虽然计算效率高，但其泛化能力通常不如强化学习。这种差距主要源于强化学习使用在线策略数据。作者希望在不牺牲效率的前提下，提升SFT的泛化性能。

Method: 1. 提出分布判别理论（DDT）来解释和量化数据与模型诱导分布之间的对齐程度；2. 引入两种互补技术：In-Distribution Finetuning（IDFT，损失层面的方法）和Hinted Decoding（数据层面的技术）。

Result: 实验表明，该框架在泛化性能上与DPO、SimPO等主流离线强化学习算法相当，同时保持了SFT流程的效率。

Conclusion: 该框架为强化学习不可行的领域提供了实用的替代方案，实现了监督微调效率与强化学习泛化性能的平衡。
...
TL;DR: 论文提出InvestESG多智能体模拟框架，分析气候风险下投资者与公司的互动，发现存在跨期社会困境，并应用Advantage Alignment算法引导智能体学习以实现更优的社会结果。


<details>
  <summary>Details</summary>
Motivation: 应对气候变化需要全球协调，但理性经济主体往往优先考虑短期收益而非集体福利，导致社会困境。需要研究如何在市场激励机制与长期可持续发展目标之间建立更好的协调机制。

Method: 1. 提出InvestESG多智能体模拟框架，捕捉气候风险下投资者与公司的动态互动；2. 形式化描述InvestESG中跨期社会困境的条件，推导个体激励与集体福利背离的理论阈值；3. 应用Advantage Alignment算法（一种在一般和博弈中有效的可扩展对手塑造算法）来影响智能体学习过程。

Result: Advantage Alignment算法通过偏向合作结果的学习动态，系统性地促进社会有益均衡。战略性地塑造经济主体的学习过程可以产生更好的结果，为政策机制设计提供参考。

Conclusion: 通过算法干预智能体学习过程，可以更好地协调市场激励机制与长期可持续发展目标，为解决气候变化中的社会困境提供新的政策思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有手语生成方法使用确定性潜在嵌入，容易导致潜在空间坍缩，无法保持发音器级别的表示。需要一种能够学习分布式潜在表示的方法来提升生成质量。

Method: 提出A²V-SLP框架：1）使用解耦变分自编码器（VAE）编码真实手语姿势序列，提取发音器特定的均值和方差向量；2）非自回归Transformer根据文本嵌入预测潜在均值和log方差；3）VAE解码器通过随机采样重建最终姿势序列；4）集成gloss注意力机制加强语言输入与动作对齐。

Result: 相比确定性潜在回归方法取得一致提升，在完全无gloss设置下实现最先进的回译性能，并改善了动作的真实性。

Conclusion: 通过分布式潜在建模避免了确定性潜在坍缩，保持了发音器级别的表示，结合gloss注意力机制增强了语言与动作的对齐，提升了手语生成质量。
...
TL;DR: LLMs通过高斯过程框架研究上下文学习，发现其学习曲线受函数生成核影响，能接近GP下界，且偏好较不光滑的核，可通过微调改变归纳偏好。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在推理时从少量演示中学习的能力（上下文学习），通过高斯过程框架量化其学习行为，并探索如何调整其归纳偏好以提高连续函数学习任务的样本效率。

Method: 建立受控实验，让模型观察从已知GP先验中采样的多元标量函数序列；使用GP回归学习器作为下界，1-NN规则作为上界；通过似然分析研究归纳偏好；探索强化学习和监督微调对改变归纳偏好的效果。

Result: LLMs的学习曲线受函数生成核强烈影响，随着演示数量增加接近GP下界；LLM预测在较不光滑的GP核下最可能；RL和监督微调都能有效将归纳偏好向训练数据方向调整。

Conclusion: 该框架量化了LLMs类似GP学习者的程度，并提供了调整其归纳偏好的工具，有助于改进连续函数学习任务的样本效率。
...
TL;DR: 提出一个通用的基于扩散模型的降尺度框架，能够将确定性低分辨率天气预报提升为概率性高分辨率预测，无需针对特定模型进行微调。


<details>
  <summary>Details</summary>
Motivation: 现有的天气预测系统存在分辨率限制，确定性预报缺乏不确定性表示，需要一种模型无关的方法来提升空间分辨率和概率性预测能力。

Method: 使用单一条件扩散模型，在配对数据（约25公里分辨率的粗分辨率输入和约5公里分辨率的高分辨率区域再分析目标）上进行训练，以零样本方式应用于异构上游天气模型的确定性预报。

Result: 在多种AI和数值天气预报系统中，降尺度预报的集合平均值持续优于各模型的原始确定性预报，概率性技能（CRPS）有显著提升，特别是在近地表变量上。

Conclusion: 基于扩散的降尺度方法为业务天气预报流程提供了可扩展、模型无关的概率性接口，能够增强空间分辨率和不确定性表示。
...
TL;DR: HyPO是一种改进的DPO方法，通过条件性地处理参考策略来解决DPO在悲观配对中的过早满足问题，保持参考策略稳定作用的同时增强学习信号。


<details>
  <summary>Details</summary>
Motivation: DPO依赖参考策略进行正则化，但在参考策略偏好被拒绝响应的悲观配对中，当策略边际仅超过参考边际时梯度就会过早衰减，即使策略仍然错误，这被称为"过早满足"问题。

Method: 提出HyPO方法，在参考策略乐观或中性时保持DPO行为，在参考策略悲观时将其视为中性，通过将Δθ-Δref替换为Δθ-max{0,Δref}来条件性地消除参考信号的偏差。

Result: HyPO在偏好对齐任务中改善了推理对齐指标，获得了更高的成对胜率，同时保持了DPO的目标形式和计算成本。

Conclusion: 直接偏好对齐可以通过条件性地消除参考信号的偏差来增强，而不是完全丢弃参考信号，HyPO提供了一种简单有效的改进方案。
...
TL;DR: 该论文在PAC框架下引入了条件平均值学习问题，目标是预测每个实例在其邻域内的平均标签，而非学习目标概念本身。这扩展了经典PAC学习，适用于可解释性、公平性和推荐系统等领域。


<details>
  <summary>Details</summary>
Motivation: 传统PAC学习关注学习目标概念本身，但许多实际应用（如可解释性、公平性、推荐系统）需要预测实例邻域的平均标签。作者旨在建立这一新学习问题的理论基础，填补经典PAC学习与实际应用需求之间的差距。

Method: 作者在PAC框架下形式化了条件平均值学习问题，定义了新的组合参数（依赖于概念类和邻域系统），并建立了与邻域图独立数的紧密联系。通过分析这些参数的联合有限性，提出了学习条件平均值的理论框架。

Result: 论文完全刻画了条件平均值可学习性的条件，并给出了样本复杂度界限（在对数因子内达到紧致）。关键发现是条件平均值可学习性取决于两个新颖组合参数的联合有限性，这些参数与邻域图的独立数密切相关。

Conclusion: 该研究将PAC学习扩展到条件平均值学习这一新问题，为可解释性、公平性和推荐系统等领域的机器学习任务提供了理论基础。提出的组合参数刻画和样本复杂度结果为这一新学习范式建立了坚实的理论框架。
...
TL;DR: 该论文提出了一种推理优化方法，通过神经架构搜索框架Puzzle对gpt-oss-120B进行优化，生成gpt-oss-puzzle-88B模型，在保持精度的同时显著提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 专注于推理的大语言模型通过生成长推理轨迹来提高答案质量，但额外的token会大幅增加服务成本，因此需要推理优化。

Method: 扩展并应用Puzzle神经架构搜索框架，结合异构MoE专家剪枝、选择性替换全上下文注意力为窗口注意力、FP8 KV缓存量化以及后训练强化学习来恢复精度。

Result: 在8×H100节点上，长上下文和短上下文设置中分别实现1.63倍和1.22倍的吞吐量加速；单H100 GPU上实现2.82倍吞吐量加速；请求级效率最高提升1.29倍；在各种基准测试中精度与原始模型相当或略有提升。

Conclusion: 后训练架构搜索可以显著降低推理成本而不牺牲质量，并提倡使用考虑推理token数量的请求级效率指标来评估推理优化效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列预测模型的对抗攻击方法忽略了时间序列数据固有的时间一致性，导致同一时间戳在不同重叠样本中的扰动值不一致，这种时间不一致的扰动使得对抗攻击在实际数据操作中不可行。
...
Conclusion: 提出的TUAPs和TGAM方法有效解决了时间序列对抗攻击中的时间不一致性问题，为时间序列预测模型提供了实用且高效的对抗攻击框架，在保持时间一致性的同时实现了优越的攻击性能。
...
TL;DR: 该论文探讨了AI决策系统中的预测多重性问题：存在多个整体准确率相似但个体预测不同的模型，这种任意性与欧盟AI法案要求不符。论文提出了评估和报告预测多重性的具体方法。


<details>
  <summary>Details</summary>
Motivation: 在构建AI决策支持系统时，经常遇到预测多重性现象：存在多个整体准确率相似但对个体案例预测不同的模型。当决策直接影响人类时，这种任意性令人不满。特别是欧盟AI法案要求高风险AI系统提供商不仅要报告数据集层面的性能，还要报告针对特定个体的性能，这种预测多重性与法案要求存在冲突。

Method: 1. 将预测多重性与欧盟AI法案的准确性条款进行法律分析；2. 提出个体冲突比率和δ-模糊度作为量化模型间个体预测分歧的工具；3. 基于计算洞察，推导易于实施的评估预测多重性的实践规则；4. 建议在AI法案框架下向部署者提供预测多重性信息。

Result: 论文论证了纳入预测多重性信息有助于提供商遵守欧盟AI法案的准确性条款，提出了具体的量化工具（个体冲突比率和δ-模糊度）来检测面临矛盾预测的个体，并提供了易于实施的评估规则。

Conclusion: 预测多重性信息应提供给AI系统的部署者，使他们能够判断系统对特定个体的输出是否足够可靠以满足其使用需求。这有助于实现欧盟AI法案对高风险AI系统的透明度和问责要求。
...
TL;DR: PMFL是一种性能增强的模型对比联邦学习框架，通过利用历史训练信息解决节点数据分布和参与频率异质性问题


<details>
  <summary>Details</summary>
Motivation: 联邦学习系统通常部署在异构场景中，节点在数据分布和参与频率上都存在差异，这会损害联邦学习的性能。需要解决这些异质性问题来提升联邦学习效果。

Method: 提出PMFL框架：在节点侧，通过将历史本地模型纳入节点优化目标，设计新颖的模型对比项来捕获稳定的对比点；在服务器侧，利用每个节点的累积参与计数自适应调整其聚合权重，并通过历史全局模型减少相邻轮次间的性能波动。

Result: 大量实验表明，PMFL在异构场景中相比现有联邦学习方法取得了更优的性能表现。

Conclusion: PMFL通过利用历史训练信息，有效解决了联邦学习中数据分布和参与频率异质性问题，提升了联邦学习在异构场景中的性能。
...
### [139] [RAM-Net: Expressive Linear Attention with Selectively Addressable Memory](https://arxiv.org/abs/2602.11958)
*Kaicheng Xiao,Haotian Li,Liran Dong,Guoliang Xing*

Main category: cs.LG

TL;DR: RAM-Net是一种新型注意力架构，通过将输入映射到高维稀疏向量作为显式地址，选择性访问大规模内存状态，在保持线性注意力计算效率的同时，显著提升了表达能力和检索保真度。


<details>
  <summary>Details</summary>
Motivation: 线性注意力架构虽然推理效率高，但将无限历史压缩到固定大小内存中会限制表达能力并导致信息丢失。需要一种既能保持线性注意力计算效率，又能提升表达能力的解决方案。

Method: 提出RAM-Net架构，核心是将输入映射到高维稀疏向量作为显式地址，允许模型选择性访问大规模内存状态。这种设计实现了指数级状态规模扩展而无需额外参数，稀疏性确保了计算效率，状态更新仅限于最小条目。

Result: 在细粒度长程检索任务中持续超越最先进基线，在标准语言建模和零样本常识推理基准测试中达到竞争性性能，验证了其以显著降低的计算开销捕捉复杂依赖关系的能力。

Conclusion: RAM-Net成功弥合了完全注意力的表示能力与线性模型内存效率之间的差距，通过显式地址和选择性内存访问机制，在保持计算效率的同时显著提升了模型表达能力。
...
### [140] [Manifold-Aware Temporal Domain Generalization for Large Language Models](https://arxiv.org/abs/2602.11965)
*Yiheng Yao,Zekun Cai,Xinyuan Song,Hiroki Hill Kobayashi,Xuan Song,Ryosuke Shibasaki,Liang Zhao*

Main category: cs.LG

TL;DR: 论文提出MaT-LoRA方法，通过低维流形约束时间演化，解决大语言模型在时间分布偏移下的高效适应问题。
...
Method: 提出几何重构方法，将时间域泛化问题重新表述在参数高效微调框架下。开发Manifold-aware Temporal LoRA (MaT-LoRA)，将时间更新约束在低秩适应子空间的共享低维流形上，并通过结构化时间核心建模其演化。

Result: 在合成和真实数据集（包括科学文档、新闻发布和评论评分）上的广泛实验表明，MaT-LoRA在保持表达力的同时显著降低了时间建模复杂度，实现了优越的时间泛化性能，并具有实际可扩展性。

Conclusion: 通过参数高效微调的低维流形约束方法，能够有效解决大语言模型在时间演化中的泛化问题，为实际部署提供了计算可行且性能优越的解决方案。
...
### [141] [Momentum LMS Theory beyond Stationarity: Stability, Tracking, and Regret](https://arxiv.org/abs/2602.11995)
*Yifei Jin,Xin Zheng,Lei Guo*

Main category: cs.LG

TL;DR: 论文研究了动量最小均方（MLMS）算法在非平稳数据流中的自适应识别能力，推导了其在时变随机线性系统中的跟踪性能和遗憾界，并通过实验验证了其快速适应和鲁棒跟踪能力。


<details>
  <summary>Details</summary>
Motivation: 大规模数据处理中，数据通常以顺序流形式到达，这些流由复杂系统生成，表现出分布漂移和时变系统参数。这种非平稳性挑战了理论分析，因为它违反了独立同分布样本的经典假设，需要能够实时更新而无需昂贵重新训练的算法。
...
Result: 在合成和真实世界数据流上的实验表明，MLMS实现了快速适应和鲁棒跟踪，与理论结果一致，特别是在非平稳设置中，突显了其在现代流处理和在线学习应用中的潜力。

Conclusion: MLMS算法在非平稳数据流处理中表现出色，具有快速适应和鲁棒跟踪能力，特别适用于现代流处理和在线学习应用场景。
...
### [142] [FedGRPO: Privately Optimizing Foundation Models with Group-Relative Rewards from Domain Client](https://arxiv.org/abs/2602.12014)
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦基础模型方法基于模型级或表示级知识转移，需要昂贵的本地训练，通信成本高且存在隐私风险。需要一种更高效、隐私保护更好的方法。

Method: 提出FedGRPO框架，包含两个模块：1）基于能力的专家选择，使用辅助数据构建轻量级置信图识别最适合每个问题的客户端；2）基于GRPO框架的"组相对"概念，将问题与解决方案打包成候选策略，分发给专家客户端子集，通过联邦组相对损失函数聚合标量奖励信号。

Result: 在多样化领域任务上的实证结果表明，FedGRPO相比传统联邦基础模型基线，实现了更优的下游准确性和通信效率。

Conclusion: FedGRPO通过交换奖励值而非数据或模型更新，减少了隐私风险和通信开销，同时支持异构设备的并行评估，为联邦基础模型提供了一种高效、隐私保护的解决方案。
...
TL;DR: 该论文提出了一种在倒易空间中生成晶体材料的管道，通过截断傅里叶变换表示晶体密度，而非直接建模原子坐标，解决了周期性边界条件、对称性和可变原子多重性等问题。


<details>
  <summary>Details</summary>
Motivation: 现有晶体材料生成模型难以处理周期性边界条件、晶体学对称性和物理约束，同时难以扩展到大型且结构多样的晶胞。特别是基于原子坐标的方法在处理可变原子多重性方面存在局限性。

Method: 提出倒易空间生成管道：1）通过截断傅里叶变换将晶体表示为物种分辨的晶胞密度；2）使用变换器变分自编码器处理复值傅里叶系数；3）在压缩潜空间中使用潜扩散模型进行生成。仅使用每个空间维度9个傅里叶基函数。

Result: 该方法能够重建每个化学物种最多包含108个原子的晶胞。在LeMaterial基准测试中评估了重建和潜扩散性能，并在小晶胞体系（≤16个原子/晶胞）中与基于坐标的基线方法进行了无条件生成比较。

Conclusion: 倒易空间表示方法为晶体材料生成提供了周期性原生、对称性友好且支持可变原子多重性的有效框架，能够处理大型复杂晶胞结构。
...
TL;DR: 使用强化学习结合运行时性能反馈和分阶段优化技术，提升大语言模型在HPC领域的代码生成能力


<details>
  <summary>Details</summary>
Motivation: 大语言模型在代码生成方面表现出色，但生成的代码运行时性能无法保证，且在高性能计算领域很少有研究使用运行时性能作为奖励来训练LLMs

Method: 提出在线强化学习方法：在超级计算机上执行LLM生成的代码，直接将测量的运行时性能(GFLOPS)作为奖励反馈；引入分阶段质量多样性算法，按问题逐步变化允许的优化技术；构建连接GPU训练集群和CPU基准测试集群的分布式系统；使用GRPO训练Qwen2.5 Coder 14B模型

Result: 通过两个实验证明，结合运行时性能反馈和分阶段优化的强化学习能够提升LLMs的HPC代码生成能力

Conclusion: 强化学习结合运行时性能反馈和分阶段优化技术是提升大语言模型在高性能计算领域代码生成能力的有效方法
...
<details>
  <summary>Details</summary>
Motivation: 足球事件数据收集目前主要依赖劳动密集型人工标注，而球追踪技术由于基础设施和运营成本高昂难以扩展，导致全面数据收集仅限于顶级赛事，限制了数据驱动分析在足球领域的广泛应用

Method: 将球员轨迹建模为全连接动态图，将事件检测问题转化为在每个时间步选择恰好一条对应当前持球状态的边。使用条件随机场确保连续边之间不可能转换的逻辑一致性，发射和转移分数由基于集合注意力的骨干架构动态计算，通过维特比解码获得最可能的边序列

Result: 实验表明PathCRF能够产生准确、逻辑一致的持球路径，实现可靠的下游分析，同时大幅减少手动事件标注的需求

Conclusion: PathCRF框架仅使用球员跟踪数据就能有效检测足球持球事件，为更广泛采用数据驱动分析提供了可行解决方案，减少了对手动标注的依赖
...
TL;DR: 提出Empirical GPs框架，通过从历史数据中经验估计均值和协方差函数来构建数据驱动的高斯过程先验，克服传统手工设计核函数的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统高斯过程的核函数通常需要专家手工设计，这限制了模型的适应性，对假设空间施加了强假设，且难以捕捉数据中丰富的协方差结构。

Method: 从历史观测数据中经验估计均值和协方差函数来构建高斯过程先验，将学习问题形式化为似然估计，并推导出具有闭式更新的期望最大化算法，能够处理跨数据集的不同观测位置。

Result: 理论证明该模型收敛到与真实数据生成过程最接近（KL散度意义上）的高斯过程，实验表明在学习曲线外推和时间序列预测基准测试中取得有竞争力的性能。

Conclusion: Empirical GPs提供了一个原则性的框架，能够构建灵活、数据驱动的高斯过程先验，克服传统手工设计核函数的局限性，在理论和实践中都表现出色。
...
TL;DR: 提出一种基于度量空间的多模态强化学习方法，通过结构化潜在表示学习状态间的距离与最小动作转换次数的相关性，无需显式概率建模即可实现鲁棒状态估计。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习方法在处理高维、多模态、噪声观测时依赖概率模型和显式噪声假设，限制了泛化能力。需要一种无需显式概率建模就能处理不确定性的方法。

Method: 提出基于度量空间的结构化潜在表示学习方法，学习状态间距离与最小动作转换次数的相关性。引入多模态潜在转换模型和基于逆距离加权的传感器融合机制，自适应集成多传感器模态而无需先验噪声分布知识。

Result: 在多模态强化学习任务上验证了方法的有效性，相比基线方法展现出对传感器噪声更强的鲁棒性和更优的状态估计性能。强化学习智能体通过学习的表示获得性能提升，无需显式噪声增强。
...
TL;DR: 该研究探讨了离线强化学习在Q*近似和部分覆盖下的理论边界，否定了Q*可实现性和贝尔曼完备性足以实现样本高效离线RL的假设，提出了一个基于决策估计系数的通用框架，并在多个方面改进了现有理论结果。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于一个开放性问题：在部分覆盖下，Q*可实现性和贝尔曼完备性是否足以实现样本高效的离线强化学习？这个问题对CQL等实用算法有重要意义，但缺乏充分的理论分析。

Method: 1. 建立信息论下界证明Q*可实现性和贝尔曼完备性不足以保证样本效率；2. 提出基于模型无关决策估计系数(DEC)的通用框架，刻画Q*函数类的内在复杂度；3. 开发新颖的二阶性能差异引理；4. 将决策估计分解与多种Q*估计过程结合。

Result: 1. 否定了开放性问题，证明Q*可实现性和贝尔曼完备性不足以保证样本效率；2. 为软Q学习在部分覆盖下获得首个ε^{-2}样本复杂度，改进了Uehara等人的ε^{-4}界限；3. 消除了Chen和Jiang对未知Q*值间隙时额外在线交互的需求；4. 首次刻画了无贝尔曼完备性的低贝尔曼秩MDPs的离线可学习性；5. 首次分析了CQL在非表格情况下的理论性能。

Conclusion: 该研究为离线强化学习在Q*近似和部分覆盖下的理论理解做出了重要贡献，提出了一个通用框架来刻画问题复杂度，并在多个关键问题上取得了突破性进展，为未来算法设计和理论分析提供了坚实基础。
...
### [149] [KAN-FIF: Spline-Parameterized Lightweight Physics-based Tropical Cyclone Estimation on Meteorological Satellite](https://arxiv.org/abs/2602.12117)
*Jiakang Shen,Qinghui Chen,Runtong Wang,Chenrui Xu,Jinglin Zhang,Cong Bai,Feng Zhang*

Main category: cs.LG

TL;DR: KAN-FIF框架通过结合MLP、CNN和KAN层，显著减少了热带气旋监测模型的参数数量和推理时间，同时保持高精度，适合边缘设备部署。
...
Method: 提出KAN-FIF框架，结合MLP和CNN层与样条参数化的KAN层，构建轻量级多模态架构，用于最大持续风速预测。

Result: 相比基线模型Phy-CoCo，KAN-FIF参数减少94.8%（0.99MB vs 19MB），推理速度提升68.7%（2.3ms vs 7.35ms），MAE降低32.5%，在风云四号卫星处理器上实现14.41ms推理延迟。

Conclusion: KAN-FIF框架为热带气旋监测提供了高效轻量的解决方案，在边缘设备AI应用中具有良好部署可行性。
...
### [150] [Meta-Sel: Efficient Demonstration Selection for In-Context Learning via Supervised Meta-Learning](https://arxiv.org/abs/2602.12123)
*Xubin Wang,Weijia Jia*

Main category: cs.LG

TL;DR: Meta-Sel是一种轻量级的监督元学习方法，用于意图分类中的演示选择，通过训练一个快速、可解释的评分函数来选择最相关的few-shot示例，无需模型微调或额外LLM调用。
...
Method: Meta-Sel通过从训练数据中采样(candidate, query)对构建元数据集，使用类别一致性作为监督信号，训练一个校准的逻辑回归模型。模型基于两个廉价元特征：TF-IDF余弦相似度和长度兼容性比率。

Result: 在四个意图数据集和五个开源LLM上的广泛实验中，Meta-Sel在12种方法中始终排名前列，特别适合较小模型，能够通过选择质量部分补偿有限的模型能力，同时保持有竞争力的选择时间开销。
...
### [151] [Learning beyond Teacher: Generalized On-Policy Distillation with Reward Extrapolation](https://arxiv.org/abs/2602.12125)
*Wenkai Yang,Weijie Liu,Ruobing Xie,Kai Yang,Saiyong Yang,Yankai Lin*

Main category: cs.LG

TL;DR: 本文提出广义在线蒸馏(G-OPD)框架，扩展标准在线蒸馏(OPD)，引入灵活参考模型和奖励缩放因子，在数学推理和代码生成任务中取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 在线蒸馏(OPD)在提升学生模型性能方面表现出色，但缺乏理论框架。本文旨在从理论上理解OPD，并提出更灵活的广义框架来进一步提升蒸馏效果。

Method: 首先理论证明OPD是密集KL约束强化学习的特例，然后提出G-OPD框架，引入参考模型选择和奖励缩放因子控制奖励项与KL正则化的相对权重。

Result: 实验发现：1) 奖励缩放因子大于1(ExOPD)持续优于标准OPD；2) 在强到弱蒸馏中，选择教师RL前的基础模型作为参考模型进行奖励校正能进一步提升性能。

Conclusion: G-OPD框架为在线蒸馏提供了新的理论视角和实用改进，特别是ExOPD和奖励校正策略能显著提升学生模型性能，甚至超越教师模型边界。
...
### [152] [Oscillators Are All You Need: Irregular Time Series Modelling via Damped Harmonic Oscillators with Closed-Form Solutions](https://arxiv.org/abs/2602.12139)
*Yashas Shende,Aritra Das,Reva Laxmi Chauhan,Arghya Pathak,Debayan Gupta*

Main category: cs.LG

TL;DR: ContiFormer结合NODE和Transformer处理不规则时间序列，但计算开销大。本文提出使用阻尼谐振子类比，获得闭式解，消除数值ODE求解器开销，保持表达力，在保持理论保证的同时实现数量级的速度提升。


<details>
  <summary>Details</summary>
Motivation: Transformer在处理不规则时间序列时存在局限，因为它假设均匀时间间隔。虽然ContiFormer结合NODE和Transformer能处理不规则时间序列，但继承了NODE使用数值求解器的计算瓶颈。需要找到既能处理不规则时间序列又计算高效的方法。

Method: 提出使用线性阻尼谐振子类比替代NODE，该类比具有已知闭式解。将键和值建模为阻尼驱动振荡器，将查询展开为正弦基函数到适当模式数。通过将注意力建模为共振现象，自然捕获查询-键耦合。闭式解消除了数值ODE求解器的计算开销。

Result: 证明了基于振荡器的参数化保持了连续时间注意力的通用逼近性质。在保持理论保证的同时，实现了数量级的速度提升，在不规则时间序列基准测试中达到最先进性能。

Conclusion: 通过阻尼谐振子类比获得闭式解，成功解决了ContiFormer的计算瓶颈问题，实现了高效处理不规则时间序列的Transformer架构，在理论和实际性能上都取得了显著进展。
...
### [153] [It's TIME: Towards the Next Generation of Time Series Forecasting Benchmarks](https://arxiv.org/abs/2602.12147)
...
TL;DR: TIME是一个面向时间序列基础模型的下一代任务中心化基准测试，包含50个新数据集和98个预测任务，专注于零样本评估，通过人类参与循环流程确保数据完整性，并引入模式级评估视角。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列基础模型基准测试存在四个主要局限：数据组成受限（主要依赖重复使用的传统数据源）、数据完整性不足（缺乏严格质量保证）、任务制定与现实场景脱节、以及分析视角僵化（难以获得可泛化的见解）。

Method: 1) 构建包含50个新数据集和98个预测任务的TIME基准测试；2) 采用人类参与循环的基准构建流程，结合大语言模型和人类专业知识确保数据完整性；3) 重新定义任务制定，使预测配置与现实操作需求和对变量可预测性对齐；4) 提出模式级评估视角，基于结构时间序列特征表征内在时间属性。

Result: 建立了多粒度排行榜，评估了12个代表性时间序列基础模型，排行榜可在Hugging Face上访问。该基准测试避免了数据泄漏，专注于严格的零样本评估。

Conclusion: TIME基准测试解决了现有基准的局限性，通过新鲜数据集、严格数据完整性控制、现实任务对齐和模式级分析视角，为时间序列基础模型提供了更全面、更真实的评估框架。
...
### [154] [SafeNeuron: Neuron-Level Safety Alignment for Large Language Models](https://arxiv.org/abs/2602.12158)
*Zhaoxin Wang,Jiaming Liang,Fengbin Zhu,Weixiang Zhao,Junfeng Fang,Jiayi Ji,Handing Wang,Tat-Seng Chua*

Main category: cs.LG

TL;DR: SafeNeuron：一种神经元级别的安全对齐框架，通过重新分布安全表征来提高LLM和多模态LLM的安全性鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型和多模态模型的安全对齐方法存在脆弱性，安全行为集中在少数参数中，容易被神经元级攻击绕过。大多数对齐方法在行为层面操作，对模型内部安全机制的控制有限。
...
Conclusion: SafeNeuron为模型对齐提供了一个可解释且鲁棒的视角，通过神经元级干预实现了更稳健的安全对齐。
...
### [155] [Amortized Molecular Optimization via Group Relative Policy Optimization](https://arxiv.org/abs/2602.12162)
...
TL;DR: GRXForm：基于预训练图Transformer的分子优化方法，通过GRPO降低起始结构异质性带来的方差，实现无需推理时查询的泛化优化


<details>
  <summary>Details</summary>
Motivation: 现有分子优化方法多为"实例优化器"，对每个输入结构都需要重新搜索，计算成本高。基于模型的方法理论上可通过学习可迁移策略实现摊销效率，但现有方法泛化能力不足，主要原因是不同起始结构的异质性难度导致的高方差问题。
...
Result: GRXForm能够泛化到分布外分子骨架，无需推理时的oracle调用或精炼，在多目标优化任务中取得了与领先实例优化器竞争的成绩。

Conclusion: GRXForm通过GRPO解决了分子优化中起始结构异质性带来的高方差问题，实现了可泛化的分子优化策略，为分子设计提供了更高效的方法。
...
### [156] [How Sampling Shapes LLM Alignment: From One-Shot Optima to Iterative Dynamics](https://arxiv.org/abs/2602.12180)
*Yurong Chen,Yu He,Michael I. Jordan,Fan Yao*

Main category: cs.LG

TL;DR: 本文研究了偏好对齐方法中的采样和参考策略选择对模型性能的影响，分析了Identity Preference Optimization框架，发现实例依赖采样能提供更强的排序保证，而偏斜的在线采样可能导致过度集中。研究还探讨了迭代对齐动态，揭示了在某些参数下会出现持续振荡或熵崩溃现象。


<details>
  <summary>Details</summary>
Motivation: 尽管基于成对比较和参考策略正则化的大语言模型对齐方法在实践中有效，但采样选择和参考策略选择的理论影响尚不明确。本文旨在深入理解这些因素如何影响偏好对齐的性能和稳定性。

Method: 采用Identity Preference Optimization（广泛使用的偏好对齐框架）作为分析基础，研究不同采样策略（实例依赖采样vs偏斜在线采样）的影响。分析迭代对齐动态，其中学习到的策略反馈到未来的采样和参考策略中，模拟模型生成偏好数据的常见实践。

Result: 研究发现：1）适当的实例依赖采样能提供更强的排序保证；2）偏斜的在线采样在结构化偏好下可能导致过度集中；3）迭代对齐动态在某些参数选择下会表现出持续振荡或熵崩溃；4）确定了保证稳定性的参数范围。这些理论见解也适用于Direct Preference Optimization等更广泛的偏好对齐方法。

Conclusion: 本文为理解偏好对齐方法中的采样和参考策略选择提供了理论框架，揭示了这些选择对模型性能、稳定性和收敛行为的重要影响。研究结果对实际应用中的参数选择和策略设计具有指导意义，有助于开发更稳定有效的对齐方法。
...
### [157] [WaveFormer: Wavelet Embedding Transformer for Biomedical Signals](https://arxiv.org/abs/2602.12189)
*Habib Irani,Bikram De,Vangelis Metsis*

Main category: cs.LG

TL;DR: WaveFormer：一种将小波分解集成到Transformer架构中的新方法，用于生物医学信号分类，通过多通道离散小波变换提取频率特征，并采用动态小波位置编码适应信号特定的时间结构。


<details>
  <summary>Details</summary>
Motivation: 生物医学信号分类面临长序列、复杂时间动态和多尺度频率模式的挑战，这些特征难以被标准Transformer架构有效捕捉。需要一种能够同时处理时域和频域信息的架构。

Method: 提出WaveFormer架构，在两个关键阶段集成小波分解：1）嵌入构建阶段使用多通道离散小波变换提取频率特征，创建包含时域和频域信息的token；2）位置编码阶段采用动态小波位置编码，通过单通道DWT分析使位置嵌入适应信号特定的时间结构。

Result: 在8个多样化数据集上评估WaveFormer，涵盖人类活动识别和脑信号分析，序列长度从50到3000个时间步，通道数从1到144。实验结果显示WaveFormer通过全面的频率感知处理实现了有竞争力的性能。

Conclusion: WaveFormer为将频域知识集成到基于Transformer的时间序列分类中提供了一个原则性框架，能够更好地处理生物医学信号的复杂特征。
...
### [158] [Learning to Forget Attention: Memory Consolidation for Adaptive Compute Reduction](https://arxiv.org/abs/2602.12204)
*Ibne Farabi Shihab,Sanjeda Akter,Anuj Sharma*

Main category: cs.LG

TL;DR: 论文提出C-RAM方法，通过记忆巩固机制动态减少注意力使用，实现88%注意力冗余的识别和37.8倍注意力计算减少
...
Method: 提出C-RAM（基于巩固的自适应记忆路由），受生物学启发的记忆巩固机制，逐步将情景检索提炼为参数化语义记忆，实现注意力使用的动态减少

Result: 在SRCD基准测试中达到100%检索准确率，仅需1.6%注意力计算（基线为68%）；注意力使用减少37.8倍；学习到的巩固动态与人类记忆转换曲线定量匹配（γ=0.43 vs γ_human≈0.4-0.5）

Conclusion: C-RAM通过动态记忆巩固机制有效减少注意力冗余，实现效率-质量权衡的显著改进，且与人类认知过程有定量相似性，为高效混合架构提供了新方向
...
### [159] [Categorical Flow Maps](https://arxiv.org/abs/2602.12233)
...
TL;DR: Categorical Flow Maps是一种用于加速分类数据少步生成的自蒸馏流匹配方法，通过定义向单纯形的流映射，实现概率质量向预测端点的传输，在图像、分子图和文本生成上达到SOTA少步结果。


<details>
  <summary>Details</summary>
Motivation: 基于最近流匹配的变分公式和扩散/流模型中加速推理的趋势，开发一种能够加速分类数据生成的少步生成方法，同时保持模型预测的自然约束。

Method: 提出Categorical Flow Maps方法，定义向单纯形的流映射，将概率质量传输到预测端点，实现连续轨迹而非离散轨迹。该方法可以使用现有蒸馏技术训练，并引入基于端点一致性的新目标。连续公式自动支持测试时推理，可以直接重用分类设置中的现有引导和重加权技术。

Result: 在图像、分子图和文本生成任务上实现了最先进的少步结果，即使在单步生成中也表现出强大的性能。

Conclusion: Categorical Flow Maps为分类数据的加速少步生成提供了一种有效的流匹配方法，通过连续轨迹和自蒸馏实现了高质量的快速生成，并兼容现有的引导技术。
...
TL;DR: Olmix是一个解决语言模型训练中数据混合比例确定问题的框架，通过实证研究确定最佳设计选择，并针对实际开发中数据集动态变化的情况，提出混合重用机制来高效更新混合比例。


<details>
  <summary>Details</summary>
Motivation: 现有数据混合方法在真实语言模型开发中存在不足：1）配置空间不明确，设计选择缺乏理论依据和共识，且忽视数据约束等实际问题；2）实际开发中数据集会动态变化（添加、删除、分区、修订），而现有方法假设固定域集。

Method: 1）对数据混合方法的配置空间进行全面实证研究，识别出导致强混合方法的设计选择；2）提出混合重用机制，在域集更新后重用现有比例，仅重新计算受更新影响的域的比例。

Result: 在模拟真实世界LM开发的五个域集更新序列中，混合重用机制与完全重新计算混合比例的性能相当，但计算量减少74%；相比无混合训练，在下游任务上性能提升11.6%。

Conclusion: Olmix框架通过实证研究明确了数据混合方法的设计选择，并针对实际开发中数据集动态变化的问题提出了高效的混合重用机制，显著减少了计算成本同时保持了性能优势。
...
### [161] [Intrinsic-Energy Joint Embedding Predictive Architectures Induce Quasimetric Spaces](https://arxiv.org/abs/2602.12245)
*Anthony Kobanda,Waris Radji*

Main category: cs.LG

TL;DR: 该论文连接了联合嵌入预测架构(JEPA)和拟度量强化学习(QRL)，通过引入内在能量概念，证明在特定条件下JEPA的能量函数可以转化为拟度量，从而为QRL提供理论基础。


<details>
  <summary>Details</summary>
Motivation: 论文旨在建立JEPA和QRL之间的理论联系。JEPA通过学习上下文嵌入预测目标嵌入来学习表示，而QRL通过非对称距离值研究目标条件控制。作者希望找到连接这两种方法的理论框架。

Method: 作者引入"内在能量"概念，定义为两个状态之间可容许轨迹上累积局部努力的下确界。在温和的闭包和可加性假设下，证明任何内在能量都是拟度量。进一步分析在目标到达控制中，最优成本函数具有这种内在形式。

Result: 证明在目标到达控制中，最优成本函数具有内在能量形式；反之，被训练来建模内在能量的JEPA位于QRL所针对的拟度量值类中。同时解释了为什么对称有限能量在结构上与单向可达性不匹配，从而在方向性重要时需要非对称(拟度量)能量。

Conclusion: 论文成功建立了JEPA和QRL之间的理论桥梁，通过内在能量概念将两种方法统一起来。这为理解表示学习和强化学习之间的联系提供了新的理论视角，特别是在处理非对称动态系统时。
...
### [162] [Explaining AI Without Code: A User Study on Explainable AI](https://arxiv.org/abs/2602.11159)
*Natalia Abarca,Andrés Carvallo,Claudia López Moncada,Felipe Bravo-Marquez*

Main category: cs.AI

TL;DR: 在DashAI无代码ML平台中集成PDP、PFI和KernelSHAP三种可解释性技术，通过用户研究发现该模块对新手用户友好且能提升信任度，但专家用户对解释的充分性和完整性要求更高。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习在医疗、金融等敏感领域的应用增加，自动化决策的透明度问题日益突出。虽然可解释AI（XAI）能够澄清模型预测过程，但大多数方法需要技术专业知识，限制了新手用户的使用价值。这一差距在无代码ML平台中尤为关键，这些平台旨在普及AI但很少包含可解释性功能。

Method: 在开源无代码ML平台DashAI中开发了一个以人为中心的XAI模块，集成了三种互补的可解释性技术：部分依赖图（PDP）、排列特征重要性（PFI）和KernelSHAP。通过用户研究（N=20，包括ML新手和专家）评估了该模块的可用性和解释效果。

Result: 1）所有可解释性任务的成功率均达到80%以上；2）新手用户在解释满意度量表（ESS）上认为解释有用、准确且可信（Cronbach's α=0.74），而专家对解释的充分性和完整性更为挑剔；3）在自动化信任量表（TiA）上，解释提高了感知可预测性和信心（α=0.60），新手比专家表现出更高的信任度。

Conclusion: 研究结果突显了无代码ML平台中XAI面临的核心挑战：需要使解释既对新手用户易于理解，又能为专家用户提供足够详细的信息。DashAI的XAI模块在平衡这一需求方面取得了积极进展。
...
### [163] [On Decision-Valued Maps and Representational Dependence](https://arxiv.org/abs/2602.11295)
*Gil Raitses*

Main category: cs.AI

TL;DR: 论文提出了决策值映射和DecisionDB基础设施，用于记录、重放和审计不同数据表示对计算结果的影响，确保决策的可重现性和可验证性。


<details>
  <summary>Details</summary>
Motivation: 相同数据的不同表示形式可能导致计算引擎产生不同的离散结果，有些表示能保持结果一致，而有些则会完全改变结果。需要一种系统化的方法来追踪和验证不同表示对决策结果的影响。

Method: 1. 形式化决策值映射概念，将表示族中的每个成员与其产生的离散结果关联；2. 开发DecisionDB基础设施，使用从内容和工件计算出的标识符，以只写形式存储和记录这些关系；3. 支持确定性重放，从存储的工件中精确恢复记录的决策标识符；4. 将表示空间划分为持久性区域和边界。

Result: 1. 实现了决策标识符的精确恢复，三个标识字段都能与其持久化值匹配；2. 建立了表示空间的划分框架，区分了保持结果的区域和改变结果的边界；3. 将决策重用转化为机械可检查的条件。

Conclusion: 该研究为处理数据表示对计算决策的影响提供了系统化框架，通过形式化的决策值映射和基础设施支持，确保了计算结果的可靠性和可审计性，为决策重用和验证建立了理论基础和实践工具。
...
### [164] [Voxtral Realtime](https://arxiv.org/abs/2602.11298)
...
<details>
  <summary>Details</summary>
Motivation: 现有的流式语音识别方法通常通过分块或滑动窗口来适配离线模型，存在延迟和性能折衷的问题。需要一种专门为流式处理设计的端到端模型，在保持低延迟的同时达到离线转录质量

Method: 基于延迟流建模框架，构建了新的因果音频编码器和Ada RMS-Norm用于改进延迟条件化。采用端到端流式训练，明确对齐音频和文本流。在13种语言的大规模数据集上进行预训练

Result: 在480ms延迟下，Voxtral Realtime的性能与最广泛部署的离线转录系统Whisper相当。模型权重以Apache 2.0许可发布

Conclusion: Voxtral Realtime证明了专门为流式处理设计的端到端模型可以在亚秒级延迟下达到离线转录质量，为实时语音识别提供了高质量解决方案
...
### [165] [The PBSAI Governance Ecosystem: A Multi-Agent AI Reference Architecture for Securing Enterprise AI Estates](https://arxiv.org/abs/2602.11301)
*John M. Willis*

Main category: cs.AI

TL;DR: PBSAI是一个面向企业AI资产安全的多智能体参考架构，通过12个领域分类和智能体家族来协调工具与策略，提供可实施的AI安全治理方案。


<details>
  <summary>Details</summary>
Motivation: 企业正在大规模部署LLM、RAG管道和工具使用智能体，这些系统构成了复杂的AI资产（AI estates），但现有治理框架（如NIST AI RMF）缺乏针对多智能体AI防御的可实施架构。

Method: 提出PBSAI治理生态系统，采用12个领域分类法，定义有界的智能体家族，通过共享上下文信封和结构化输出契约在工具与策略之间进行协调，并包含轻量级形式化模型来确保可追溯性和人工参与保证。

Result: PBSAI与NIST AI RMF功能对齐，展示了在企业SOC和超大规模防御环境中的应用，为开放生态系统开发和未来实证验证提供了结构化、以证据为中心的基础。

Conclusion: PBSAI为保护企业和超大规模AI资产提供了一个可实施的多智能体参考架构，填补了现有治理框架在实施层面的空白，有望成为AI安全治理的实践蓝图。
...
### [166] [Dissecting Subjectivity and the "Ground Truth" Illusion in Data Annotation](https://arxiv.org/abs/2602.11318)
*Sheza Munir,Benjamin Mah,Krisha Kalsi,Shivani Kapania,Julian Posada,Edith Law,Ding Wang,Syed Ishtiaque Ahmed*

Main category: cs.AI

TL;DR: 论文批判机器学习中"ground truth"范式的实证主义谬误，指出其将人类分歧视为技术噪声而非重要的社会技术信号，并通过系统性文献综述揭示数据标注实践中的"共识陷阱"问题。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习领域将"ground truth"视为绝对正确标签的范式存在根本性问题，这种实证主义方法忽视了人类分歧作为社会技术信号的重要性，导致文化偏见和系统性不平等被编码到模型中。

Method: 采用系统性文献综述方法，分析2020-2025年间在ACL、AIES、CHI、CSCW、EAAMO、FAccT、NeurIPS等七个顶级会议发表的文献。通过分层关键词筛选从30,897条记录中筛选出3,042条进行人工审查，最终纳入346篇论文进行定性综合和反思性主题分析。

Result: 研究发现：1）系统性位置可读性失败与人类作为验证者模型架构转变相结合，导致深度锚定偏见；2）地理霸权将西方规范强加为普遍基准；3）不稳定的数据工作者为规避经济惩罚而进行表演性对齐；4）统计模型将文化多元性误诊为随机误差的"噪声传感器"谬误。

Conclusion: 提出从寻找单一"正确答案"转向映射人类经验多样性的多元标注基础设施路线图，主张将分歧重新定位为构建文化胜任模型所需的高保真信号，而非需要消除的技术噪声。
...
TL;DR: BLPO：一种用于多模态LLM评估的双层提示优化框架，通过将图像转换为文本表示来解决上下文窗口限制问题，提升AI生成图像评估效果


<details>
  <summary>Details</summary>
Motivation: 大型语言模型作为AI生成内容的自动评估工具已被广泛采用，但将其评估结果与人类判断对齐仍然具有挑战性。虽然监督微调可以改善对齐效果，但成本高且不灵活。现有的自动提示优化方法主要针对纯文本评估，在多模态场景下研究不足，特别是评估AI生成图像时面临上下文窗口限制的问题。
...
Conclusion: BLPO框架成功解决了多模态LLM评估中的上下文窗口限制问题，通过双层优化策略有效提升了AI生成图像评估的质量和效率，为多模态自动评估提供了新的解决方案。
...
TL;DR: 论文提出AgentNoiseBench框架，用于系统评估智能体模型在噪声环境下的鲁棒性，通过分析真实世界噪声类型并开发自动化噪声注入管道，发现当前智能体模型对现实环境扰动敏感。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型智能体在基准测试中表现良好，但在真实世界部署中性能下降，主要原因是当前训练和评估范式基于理想化假设，忽略了真实世界交互中的随机性和噪声。

Method: 1) 深入分析真实场景中的偏差和不确定性，将环境噪声分为用户噪声和工具噪声两类；2) 开发自动化管道，在保持任务可解性的前提下向现有智能体基准注入可控噪声；3) 使用该管道对多种架构和参数规模的模型进行广泛评估。

Result: 评估结果显示，在不同噪声条件下，当前智能体模型表现存在一致的性能变化，突显了这些模型对现实环境扰动的敏感性。

Conclusion: AgentNoiseBench框架能够系统评估智能体模型的鲁棒性，揭示了当前模型在噪声环境中的脆弱性，为开发更鲁棒的智能体系统提供了重要工具和洞见。
...
Conclusion: BAO框架能有效训练复杂多轮场景中主动且用户对齐的LLM智能体，解决任务性能与用户参与度的权衡问题
...
### [170] [Causal-JEPA: Learning World Models through Object-Level Latent Interventions](https://arxiv.org/abs/2602.11389)
...
Method: 提出C-JEPA模型，将掩码联合嵌入预测从图像块扩展到对象中心表示，通过对象级掩码要求从其他对象推断目标对象状态，诱导潜在干预并防止捷径解

Result: 在视觉问答中实现约20%的反事实推理绝对提升；在智能体控制任务中，仅需基于补丁世界模型1%的潜在输入特征即可达到相当性能，规划效率大幅提升

Conclusion: 对象级掩码通过潜在干预诱导因果归纳偏置，C-JEPA为简单灵活的对象中心世界模型，能有效增强交互推理能力
...
### [171] [GHOST: Unmasking Phantom States in Mamba2 via Grouped Hidden-state Output-aware Selection & Truncation](https://arxiv.org/abs/2602.11408)
*Michael Menezes,Anastasios Kyrillidis*

Main category: cs.AI

TL;DR: GHOST是一种结构化剪枝框架，通过仅使用前向传播统计量来近似控制理论平衡截断，显著减少Mamba2模型的状态维度，降低推理开销


<details>
  <summary>Details</summary>
Motivation: Mamba2扩展的状态维度虽然增强了时序建模能力，但在自回归生成过程中会产生显著的推理开销，导致带宽饱和。传统的剪枝方法无法有效解决这一瓶颈问题
...
Conclusion: GHOST框架能够有效减少Mamba2模型的推理开销，在保持模型性能的同时显著降低计算成本，为高效的大规模语言模型推理提供了新方法
...
<details>
  <summary>Details</summary>
Motivation: 当前集中训练分散执行的多智能体强化学习方法在现实环境中面临可靠性问题，主要由于模拟到现实的差距、模型不匹配和系统噪声等环境不确定性因素。

Method: 提出了分布鲁棒的IGM原则(DrIGM)，定义了鲁棒个体动作价值，并基于此推导出与DrIGM兼容的鲁棒价值分解架构变体（如VDN/QMIX/QTRAN），这些方法训练鲁棒Q目标，保持可扩展性，并能无缝集成到现有代码库。

Result: 在高保真SustainGym模拟器和星际争霸游戏环境中，该方法在分布外性能方面持续提升，证明了其有效性。

Conclusion: DrIGM原则为多智能体强化学习系统提供了可证明的鲁棒性保证，能够有效应对现实环境中的不确定性挑战，且与现有方法兼容。
...
TL;DR: 论文提出Anchor-Token Reinforcement Learning (AT-RL)框架，通过选择性强化高连接性视觉锚点token来提升多模态大语言模型的推理能力，仅增加1.2%开销就能让32B模型超越72B基准。


<details>
  <summary>Details</summary>
Motivation: 尽管可验证奖励的强化学习(RLVR)显著提升了多模态大语言模型的推理能力，但视觉证据在推理过程中如何被整合仍然不清楚。需要理解跨模态注意力连接性如何影响多模态推理的质量。

Method: 通过分析跨模态注意力连接性，发现只有约15%的token表现出强烈的视觉-文本耦合。基于此提出AT-RL框架，使用基于图的注意力拓扑聚类来识别高连接性锚点token，并选择性强化这些视觉锚点。

Result: AT-RL仅引入1.2%的开销，但使32B模型在MathVista上达到80.2分，超越了72B-Instruct基线。在STEM、视频和通用任务上都观察到一致的性能提升。相反，仅训练低连接性token会导致严重性能下降。

Conclusion: 推理质量不是由token数量决定，而是由跨模态锚点的保真度决定。有效的多模态强化学习依赖于对视觉锚点的精确信用分配，AT-RL通过选择性强化高连接性锚点token实现了这一目标。
...
Method: 开发了AgentLeak基准测试，包含1000个跨医疗、金融、法律和企业领域的场景，配备32类攻击分类法和三层检测管道，测试了GPT-4o、GPT-4o-mini、Claude 3.5 Sonnet、Mistral Large和Llama 3.3 70B等模型，分析了4979条跟踪记录

Result: 多智能体配置降低了单通道输出泄露（C1：27.2% vs 单智能体43.2%），但未监控的内部通道使总系统暴露率达到68.9%。智能体间消息（C2）泄露率高达68.8%，而输出通道（C1）仅为27.2%，输出审计会遗漏41.7%的违规。Claude 3.5 Sonnet在外部（3.3%）和内部（28.1%）通道上都表现出最低泄露率

Conclusion: 智能体间通信是主要漏洞，需要开发包含内部通道隐私保护的协调框架，并在智能体间通信中强制执行隐私控制措施
...
Conclusion: 该框架通过将内部推理过程作为主要学习对象，实现了处理过程中的学习，使系统能够优化推理活动的组织、调度和演进，为开发能够在动态环境中持续适应的AI系统提供了新途径。
...
Conclusion: CausalAgent作为一种新颖的用户中心化人机协作范式，明确建模了分析工作流程，为因果推断提供了一种更易用、自动化的解决方案。
...
TL;DR: INTENT框架：面向预算约束的工具增强智能体，通过意图感知分层世界模型进行推理时规划，在严格预算限制下提高任务成功率


<details>
  <summary>Details</summary>
Motivation: 研究预算约束下的工具增强智能体问题，其中大型语言模型需要在严格的货币预算下通过调用外部工具解决多步任务。现有方法面临状态-动作空间巨大、结果方差高、探索成本昂贵等挑战，使得直接规划难以实现。

Method: 提出INTENT推理时规划框架，利用意图感知分层世界模型来预测未来工具使用、风险校准成本，并在线指导决策。该框架将问题形式化为上下文空间中的序列决策问题，考虑定价和随机工具执行。

Result: 在成本增强的StableToolBench上，INTENT在严格强制执行硬预算可行性的同时，显著提高了任务成功率，并且在工具价格变化和预算变化等动态市场变化下保持鲁棒性。

Conclusion: INTENT框架有效解决了预算约束下工具增强智能体的规划问题，通过推理时规划和意图感知模型实现了在严格预算限制下的高效任务执行，对动态市场环境具有适应性。
...
TL;DR: 本文综述了多智能体通信研究的演变历程，从强化学习到涌现语言再到大语言模型，通过"五个W"框架分析通信设计的关键要素和挑战。


<details>
  <summary>Details</summary>
Motivation: 在动态、部分可观测的环境中，通信是减少不确定性、实现协作的关键。然而，现有的多智能体通信方法存在任务特定性、可解释性差、泛化能力不足等问题，需要系统性的框架来连接不同研究范式。

Method: 采用"五个W"框架（谁与谁通信、通信内容、通信时机、通信目的）分析多智能体通信，并追溯三个主要研究范式的演变：多智能体强化学习（MARL）、涌现语言（EL）和大语言模型（LLMs）。

Result: MARL方法通过端到端学习优化通信协议但缺乏可解释性；EL方法试图发展结构化通信但面临基础、泛化和可扩展性挑战；LLM方法利用自然语言先验在开放环境中表现出色但仍有局限性。不同选择塑造了通信设计，存在各种权衡。

Conclusion: 未来需要结合学习、语言和控制能力的混合系统，以实现可扩展且可解释的多智能体协作。本文提炼了实用设计模式和开放挑战，为这一方向提供指导。
...
TL;DR: MAPLE是一个完整的模态感知后训练学习生态系统，通过模态感知策略优化、自适应加权和课程调度，解决了多模态RL训练中模态无关处理导致的梯度方差大、收敛慢和鲁棒性差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态语言模型将文本、音频和视频整合进行统一推理，但现有的RL后训练管道将所有输入信号视为同等相关，忽略了每个任务实际需要的模态。这种模态盲训练导致策略梯度方差增大、收敛速度减慢，并且在现实世界分布变化（信号可能缺失、添加或重新加权）时鲁棒性下降。

Method: MAPLE包含三个核心组件：1) MAPLE-bench：首个明确标注每个任务所需最小信号组合的基准；2) MAPO：模态感知策略优化框架，按模态需求对批次进行分层，以减少异构组优势带来的梯度方差；3) 自适应加权和课程调度，平衡并优先处理更难的信号组合。系统分析了损失聚合、裁剪、采样和课程设计，建立了MAPO的最优训练策略。

Result: MAPLE将单模态/多模态准确率差距缩小了30.24%，收敛速度提高了3.18倍，并在现实减少信号访问条件下保持所有模态组合的稳定性。自适应加权和课程聚焦学习进一步提升了跨信号组合的性能。
...
### [180] [scPilot: Large Language Model Reasoning Toward Automated Single-Cell Analysis and Discovery](https://arxiv.org/abs/2602.11609)
...
TL;DR: scPilot是首个实现组学原生推理的系统框架，让大语言模型能够直接检查单细胞RNA-seq数据并使用生物信息学工具进行自然语言对话，将单细胞分析转化为逐步推理问题。


<details>
  <summary>Details</summary>
Motivation: 传统单细胞分析通常需要专业生物信息学知识，缺乏透明度和可解释性。scPilot旨在让大语言模型直接基于原始组学数据进行推理，实现可审计、可解释且具有诊断信息的单细胞分析。

Method: scPilot将核心单细胞分析（细胞类型注释、发育轨迹重建、转录因子靶向）转化为逐步推理问题，让LLM必须解决、论证并在需要时用新证据修订。框架包含scBench评估套件，包含9个专家策划的数据集和评分器。

Result: 实验显示，迭代组学原生推理将细胞类型注释的平均准确率提高了11%，Gemini-2.5-Pro将轨迹图编辑距离减少了30%（相比单次提示）。模型生成透明推理轨迹，解释标记基因模糊性和调控逻辑。

Conclusion: scPilot通过将LLM基于原始组学数据，实现了可审计、可解释且具有诊断信息的单细胞分析，为生物医学研究提供了新的透明分析范式。
...
### [181] [When Agents Disagree With Themselves: Measuring Behavioral Consistency in LLM-Based Agents](https://arxiv.org/abs/2602.11619)
*Aman Mehta*
...
<details>
  <summary>Details</summary>
Motivation: 研究LLM智能体在相同任务重复执行时是否会产生一致的行为，以及这种行为不一致性如何影响任务成功率

Method: 在HotpotQA数据集上对三个模型（Llama 3.1 70B、GPT-4o和Claude Sonnet 4.5）进行了3,000次智能体运行实验，使用ReAct风格智能体，分析行为序列的多样性

Result: 智能体行为存在显著不一致性：平均每10次运行产生2.0-4.2个不同的动作序列；行为一致性预测失败率：一致性任务（≤2个唯一路径）准确率80-92%，高度不一致任务（≥6个唯一路径）准确率仅25-60%；69%的分歧发生在第二步（第一个搜索查询）
...
### [182] [Neuro-Symbolic Multitasking: A Unified Framework for Discovering Generalizable Solutions to PDE Families](https://arxiv.org/abs/2602.11630)
...
<details>
  <summary>Details</summary>
Motivation: 传统数值方法（如有限元法）需要独立求解PDE家族中的每个实例，计算成本巨大；而机器学习PDE求解器虽然计算速度快，但缺乏可解释性，只能提供数值近似，缺少对科学洞察至关重要的解析表达式

Method: 提出NMIPS（神经辅助多任务符号PDE求解器）框架，采用多因子优化同时发现PDE的解析解；设计仿射转移方法，在PDE家族间转移学习的数学结构，避免从零开始求解每个PDE

Result: 在多个案例实验中，相比现有基线方法实现了显著改进，精度提升高达约35.7%，同时提供可解释的解析解

Conclusion: NMIPS框架成功解决了PDE家族求解中的计算效率和可解释性问题，通过多任务学习和结构转移实现了高效且具有解析表达式的PDE求解
...
### [183] [Do MLLMs Really Understand Space? A Mathematical Reasoning Evaluation](https://arxiv.org/abs/2602.11635)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在感知任务上表现良好，但在数学空间推理能力（解析和操作二维、三维关系）方面存在明显不足，与人类95%以上的准确率相比，大多数领先模型甚至达不到60%。这种差距凸显了空间推理是当前模型的基本弱点。

Method: MathSpatial框架包含三个互补组件：1) MathSpatial-Bench：包含3个类别11个子类型的2000个问题的基准测试，旨在将推理难度与感知噪声分离；2) MathSpatial-Corpus：包含8000个已验证解决方案的额外训练数据集；3) MathSpatial-SRT：将推理建模为由三个原子操作（关联、约束、推断）组成的结构化轨迹。

Result: 实验表明，在MathSpatial上微调Qwen2.5-VL-7B模型能够达到有竞争力的准确率，同时减少25%的token使用。该框架首次提供了大规模资源，能够将感知与推理分离，实现对多模态大语言模型数学空间推理能力的精确测量和全面理解。

Conclusion: MathSpatial揭示了当前多模态大语言模型在数学空间推理方面的显著缺陷，并提供了一个统一的框架来评估和改进这一能力。该工作为理解模型的空间推理能力提供了重要工具，有助于推动多模态人工智能在复杂推理任务上的发展。
...
### [184] [Quark Medical Alignment: A Holistic Multi-Dimensional Alignment and Collaborative Optimization Paradigm](https://arxiv.org/abs/2602.11661)
...
TL;DR: 该论文提出了一种针对医学问答领域的强化学习对齐新范式，通过构建多维医学对齐矩阵和统一优化机制，解决了现有RLHF方法在医学领域存在的成本高、验证难、多目标冲突等问题。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习对齐方法在医学问答领域存在根本性不匹配：RLHF依赖昂贵的人工偏好标注且难以反映医学事实的绝对正确性；基于可验证奖励的方法缺乏有效自动验证器且难以处理复杂临床情境；医学对齐需要同时优化正确性、安全性和合规性，但多目标异质奖励信号容易产生尺度不匹配和优化冲突。
...
Conclusion: 该研究提出了一种稳健的医学对齐范式，通过系统化的多维度分解和智能优化机制，解决了医学领域强化学习对齐的特殊挑战，为高风险的垂直领域AI对齐提供了新思路。
...
### [185] [Benchmark Health Index: A Systematic Framework for Benchmarking the Benchmarks of LLMs](https://arxiv.org/abs/2602.11674)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，用于衡量进展的基准测试变得越来越不可靠。分数膨胀和选择性报告削弱了标准基准的权威性，导致社区不确定哪些评估结果值得信赖。

Method: 引入基准健康指数（BHI）框架，从三个正交互补维度审计评估集：1）能力区分度：衡量基准区分模型性能的能力；2）抗饱和性：估计基准在达到天花板效应前的剩余空间；3）影响力：通过采用广度和实践塑造能力量化基准在学术和工业生态系统中的影响。

Result: 从2025年91个代表性模型的技术报告中提取了106个验证基准，系统性地描述了评估现状。BHI是首个在宏观层面量化基准健康度的框架。

Conclusion: BHI为基准选择提供了原则性基础，并支持下一代评估协议的动态生命周期管理，是量化基准健康度的首个宏观框架。
...
### [186] [Right for the Wrong Reasons: Epistemic Regret Minimization for Causal Rung Collapse in LLMs](https://arxiv.org/abs/2602.11675)
*Edward Y. Chang*

Main category: cs.AI

TL;DR: 论文提出Rung Collapse理论，解释自回归训练无法区分关联P(Y|X)与干预P(Y|do(X))，导致模型"正确但理由错误"。作者提出Epistemic Regret Minimization（ERM）作为因果信念修正算子，通过三层架构解决因果推理错误，实验证明在1360个因果陷阱场景中，ERM能恢复53-59%的顽固错误。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统常通过捷径获得高性能，但这些捷径在分布变化时会失效。作者发现这种病理现象有精确的因果起源：自回归训练无法提供梯度信号来区分关联P(Y|X)与干预P(Y|do(X))，导致模型即使答案正确但推理过程错误，这种现象被称为Rung Collapse。

Method: 提出Epistemic Regret Minimization（ERM）作为因果信念修正目标，独立于任务成功惩罚因果推理错误。构建三层架构：1）物理基础定理证明满足执行器独立的动作实现有效的do操作；2）ERM作为满足AGM公设的因果信念修正算子；3）失败模式分类法对常见推理错误分类并注入领域无关防护。提供有限样本边界下的渐近恢复证明。

Result: 在6个前沿LLM的1360个因果陷阱场景实验中：1）Rung Collapse在推理增强模型中持续存在（GPT-5.2为3.7%）；2）可操控性呈现逆缩放现象，高级模型抵抗通用修正；3）针对性ERM反馈能恢复53-59%的顽固错误，而结果级反馈失败。

Conclusion: 论文揭示了自回归训练中Rung Collapse的根本问题，提出了理论严谨的ERM框架来解决因果推理错误。通过将因果理论与机器学习实践结合，为构建更鲁棒、可解释的AI系统提供了新方向，特别是在防止模型因错误推理而固守错误信念方面具有重要意义。
...
### [187] [Beyond Pixels: Vector-to-Graph Transformation for Reliable Schematic Auditing](https://arxiv.org/abs/2602.11678)
*Chengwei Ma,Zhen Tian,Zhou Zhou,Zhixian Xu,Xiaowei Zhu,Xia Hua,Si Shi,F. Richard Yu*

Main category: cs.AI

TL;DR: 论文提出V2G管道，将CAD图转换为属性图，解决MLLMs在工程示意图中的结构盲区问题，显著提升电气合规检查准确率。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉理解方面虽有进步，但在处理工程示意图时存在"结构盲区"问题。基于像素的范式无法捕捉拓扑结构和符号逻辑，丢弃了推理所需的显式向量定义关系，导致在工程领域应用受限。

Method: 提出Vector-to-Graph（V2G）管道，将CAD图转换为属性图。图中节点代表组件，边编码连接性，使结构依赖关系显式化且可机器审计。

Result: 在电气合规检查的诊断基准测试中，V2G在所有错误类别上都取得了大幅准确率提升，而领先的MLLMs表现接近随机水平。这突显了基于像素方法的系统性不足。
...
### [188] [ThinkRouter: Efficient Reasoning via Routing Thinking between Latent and Discrete Spaces](https://arxiv.org/abs/2602.11683)
*Xin Xu,Tong Yu,Xiang Chen,Haoliang Wang,Julian McAuley,Saayan Mitra*

Main category: cs.AI

TL;DR: ThinkRouter：一种推理时置信度感知路由机制，在模型置信度低时路由到离散token空间，置信度高时路由到潜在空间，以提高推理效率和准确性


<details>
  <summary>Details</summary>
Motivation: 研究发现，在潜在推理中，错误答案的思考轨迹包含更少的低置信度步骤，而多个低置信度思考替代方案的软嵌入可能引入和传播噪声，导致对不可靠推理轨迹的高置信度
...
### [189] [Beyond Parameter Arithmetic: Sparse Complementary Fusion for Distribution-Aware Model Merging](https://arxiv.org/abs/2602.11717)
*Weihong Lin,Lin Sun,Qilong Shi,Aomufei Yuan,Yuxuan Tian,Zhengyang Wang,Guangxiang Zhao,Xiangzheng Zhang,Tong Yang*

Main category: cs.AI

TL;DR: SCF-RKL是一种新颖的模型融合框架，通过稀疏、分布感知的更新来显式控制功能干扰，使用反向KL散度测量模型间的功能差异，有选择地整合互补参数，在保持稳定表示的同时集成新能力。


<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法主要依赖参数空间的启发式方法，常常引入严重的干扰，导致泛化能力下降和不稳定的生成行为（如重复和不连贯输出）。需要一种能够显式控制功能干扰的融合方法。

Method: 提出SCF-RKL框架，使用反向Kullback-Leibler散度测量模型间的功能差异，通过稀疏、分布感知的更新选择性整合互补参数，采用模式寻求、稀疏诱导的设计来保持稳定表示。

Result: 在24个基准测试（涵盖高级推理、一般推理和知识、指令遵循、安全性、视觉分类）上的广泛实验表明，SCF-RKL在各种模型规模和架构上始终优于现有模型融合方法，同时保持强大的泛化能力和生成稳定性。

Conclusion: SCF-RKL通过显式控制功能干扰的稀疏互补融合方法，有效解决了现有模型融合中的干扰问题，实现了更好的性能表现和生成稳定性，为模型融合提供了新的有效范式。
...
### [190] [Text2GQL-Bench: A Text to Graph Query Language Benchmark [Experiment, Analysis & Benchmark]](https://arxiv.org/abs/2602.11745)
...
TL;DR: 本文介绍了Text2GQL-Bench，一个统一的文本到图查询语言基准测试，包含178,184个问题-查询对，覆盖13个领域，并提出了包含语法有效性、相似度、语义对齐和执行准确率的综合评估方法。
...
Method: 提出Text2GQL-Bench基准，包含多图查询语言数据集和可扩展的构建框架，能够生成不同领域、问题抽象层次和图查询语言的数据集。同时引入综合评估方法，联合报告语法有效性、相似度、语义对齐和执行准确率。
...
Conclusion: Text2GQL-Bench为文本到图查询语言系统提供了全面的基准测试和评估框架，揭示了当前LLM在图查询语言生成方面的局限性，特别是ISO-GQL的生成挑战，同时展示了通过示例学习和微调可以显著提升性能。
...
### [191] [AIR: Improving Agent Safety through Incident Response](https://arxiv.org/abs/2602.11749)
*Zibo Xiao,Jun Sun,Junjie Chen*

Main category: cs.AI

TL;DR: AIR是首个针对LLM智能体系统的安全事件响应框架，通过语义检查检测事件、引导智能体执行遏制恢复操作、并合成防护规则防止未来类似事件，在三种代表性智能体上实现了超过90%的成功率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体的安全机制主要关注事前预防，对事件发生后的响应、遏制和恢复能力有限。随着LLM智能体在实际应用中的广泛部署，需要建立有效的事件响应机制来处理不可避免的安全事件。

Method: AIR框架定义了一个领域特定语言来管理LLM智能体系统中的事件响应生命周期，并将其集成到智能体执行循环中：1）通过基于当前环境状态和近期上下文的语义检查检测事件；2）引导智能体通过其工具执行遏制和恢复操作；3）在根除阶段合成防护规则以阻止未来类似事件。
...
<details>
  <summary>Details</summary>
Motivation: 物种分布模型产生的概率性预测需要转换为二值化存在-缺失地图用于生态推断和保护规划，但传统二值化方法通常是启发式的，会显著扭曲物种普遍性和群落组成的估计。

Method: 提出MaxExp决策驱动二值化框架，通过直接最大化选定的评估指标来选择最可能的物种组合，无需校准数据且适用于多种评分指标；同时引入Set Size Expectation方法，基于期望物种丰富度预测物种组合。

Result: 在三个涵盖不同分类群、物种数量和性能指标的案例研究中，MaxExp始终匹配或超越广泛使用的阈值化和校准方法，特别是在强类别不平衡和高稀有性情况下；SSE提供了简单但具有竞争力的替代方案。

Conclusion: MaxExp和SSE方法为多物种分布模型二值化提供了稳健、可重复的工具，能够改善物种普遍性和群落组成的估计。
...
TL;DR: RELATE是一个基于强化学习的端到端框架，统一了广告文本生成和目标对齐，通过将转化指标和合规约束作为多维奖励，直接在生成过程中优化广告价值。
...
Method: 提出RELATE框架，通过强化学习将生成和目标对齐统一在单个模型中。将性能指标和合规约束作为多维奖励直接整合到生成过程中，特别关注超越点击信号的转化导向指标。

Result: 在大规模工业数据集上的实验表明，RELATE始终优于基线方法。在实际广告平台上的在线部署在严格政策约束下显著提升了点击转化率，验证了框架的鲁棒性和实际有效性。

Conclusion: RELATE通过端到端的强化学习框架成功解决了广告文本生成与目标对齐的分离问题，能够生成在政策约束下提升转化性能的高质量广告文本。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型能够通过推理和工具使用解决复杂任务，但将这些解决方案准确转化为结构化工作流仍然具有挑战性。现有方法在执行过程中构建工作流时，常因两个过程的相互干扰而导致不准确。
...
Conclusion: Execute-Summarize框架通过解耦任务执行和工作流构建，显著提高了结构化工作流的准确性和可靠性，为解决LLM推理到结构化工作流的转换问题提供了有效方案。
...
Method: 采用分层LLM多智能体系统，由编排智能体协调解决方案智能体（严谨问题求解）、插图智能体（生成可执行可视化代码）和叙述智能体（学习者导向教学脚本）。通过语义批判、基于规则的约束和工具编译检查确保质量，构建结构化可执行视频脚本，使用模板驱动组装规则确定性地编译为同步视觉和叙述。

Result: 在大规模部署中，LAVES实现每天超过100万个视频的吞吐量，相比当前行业标准方法成本降低超过95%，同时保持高接受率。

Conclusion: LAVES通过将教育视频生成分解为专门智能体协调的多目标任务，结合显式质量门和迭代批判机制，实现了高质量教学视频的自动化生成，解决了现有方法的局限性。
...
TL;DR: 本文提出了一种名为Min-kNN Distance的简单黑盒检测方法，用于识别强化学习验证奖励（RLVR）训练过的模型中的训练数据污染问题，该方法通过分析生成文本的多样性来区分训练时见过的和未见过的提示。


<details>
  <summary>Details</summary>
Motivation: 强化学习验证奖励（RLVR）训练方法在现代推理模型训练中很常见，但由于训练数据未公开，存在基准污染问题。传统的基于似然度的检测方法对RLVR效果不佳，因为RLVR是基于自生成推理轨迹的奖励反馈进行微调的，而不是基于词元级概率优化。

Method: 作者发现RLVR训练会诱导出独特的行为特征：训练时见过的提示会导致更僵化和相似的生成结果，而未见的提示则保持更大的多样性。基于此观察，提出了Min-kNN Distance检测方法：对给定提示采样多个补全结果，计算k个最小最近邻编辑距离的平均值，量化这种多样性崩溃现象。该方法无需访问参考模型或词元概率。

Result: 在多个RLVR训练的推理模型上的实验表明，Min-kNN Distance能够可靠地区分RL训练时见过的示例和未见过的示例，并且在性能上优于现有的成员推理和RL污染检测基线方法。

Conclusion: Min-kNN Distance是一种简单有效的黑盒检测方法，能够有效识别RLVR训练模型中的训练数据污染问题，为解决基准污染问题提供了实用工具。
...
### [197] [Multi UAVs Preflight Planning in a Shared and Dynamic Airspace](https://arxiv.org/abs/2602.12055)
...
TL;DR: DTAPP-IICR：一种面向大规模无人机机群预飞行规划的交付时间感知优先级规划方法，通过增量迭代冲突解决处理动态空域中的时间性禁飞区、异构无人机和严格交付期限问题。


<details>
  <summary>Details</summary>
Motivation: 大规模无人机机群在动态共享空域中的预飞行规划面临时间性禁飞区、异构无人机配置和严格交付期限等挑战，现有多智能体路径规划方法缺乏实际无人机交通管理所需的可扩展性和灵活性。

Method: 提出DTAPP-IICR框架：1）基于任务紧急度生成初始优先级规划；2）使用SFIPP-ST（带软约束和时间约束的安全飞行间隔路径规划）计算往返轨迹，处理异构无人机、严格执行时间性禁飞区、将智能体间冲突建模为软约束；3）基于几何冲突图指导的迭代大邻域搜索解决剩余冲突；4）采用完整性保持的方向性剪枝技术加速3D搜索。

Result: 在包含时间性禁飞区的基准测试中，DTAPP-IICR在多达1000架无人机的机群上实现接近100%的成功率，剪枝技术带来高达50%的运行时间减少，优于批处理增强冲突搜索方法，在城市规模实际运营中成功扩展，而其他基于优先级的方法在中等部署规模下即失败。
...
### [198] [Hi-SAM: A Hierarchical Structure-Aware Multi-modal Framework for Large-Scale Recommendation](https://arxiv.org/abs/2602.11799)
*Pingjun Pan,Tingting Zhou,Peiyao Lu,Tingting Fei,Hongxiang Chen,Chuanjiang Luo*

Main category: cs.AI

TL;DR: Hi-SAM是一个分层结构感知的多模态推荐框架，通过解耦语义分词器和分层记忆锚点Transformer解决现有方法中的令牌化次优和架构数据不匹配问题，在冷启动场景表现优异，已在大型社交平台部署并获得显著效果提升。


<details>
  <summary>Details</summary>
Motivation: 多模态推荐面临两个主要挑战：1）现有语义ID方法（如RQ-VAE）缺乏跨模态共享语义和模态特定细节的解耦，导致冗余或崩溃；2）普通Transformer将语义ID视为扁平流，忽略了用户交互、物品和令牌的层次结构，扩展物品为多个令牌会放大长度和噪声，使注意力偏向局部细节而非整体语义。

Method: 提出Hi-SAM框架：1）解耦语义分词器（DST）：通过几何感知对齐统一模态，采用粗到细策略量化，共享码本提取共识，模态特定码本从残差中恢复细节，通过互信息最小化强制执行；2）分层记忆锚点Transformer（HMAT）：通过分层RoPE将位置编码拆分为物品间和物品内子空间以恢复层次结构，插入锚点令牌将物品压缩为紧凑记忆，保留当前物品细节，仅通过压缩摘要访问历史。

Result: 在真实世界数据集上实验显示，相比现有最先进基线方法有持续改进，尤其在冷启动场景表现突出。在服务数百万用户的大型社交平台部署后，Hi-SAM在核心在线指标上获得了6.55%的提升。

Conclusion: Hi-SAM通过解耦语义分词和分层Transformer架构，有效解决了多模态推荐中的令牌化次优和架构数据不匹配问题，在冷启动和实际部署场景中表现出色，为多模态推荐系统提供了有效的解决方案。
...
### [199] [PuYun-LDM: A Latent Diffusion Model for High-Resolution Ensemble Weather Forecasts](https://arxiv.org/abs/2602.11807)
*Lianjun Wu,Shengchen Zhu,Yuxuan Liu,Liuyu Kai,Xiaoduan Feng,Duomin Wang,Wenshuo Liu,Jingxuan Zhang,Kelvin Li,Bin Wang*

Main category: cs.AI

TL;DR: PuYun-LDM通过3D-MAE编码天气状态演化特征作为扩散模型条件，结合VA-MFM策略自适应调整各变量的频谱正则化，解决了LDMs在高分辨率集合天气预报中扩散能力受限的问题。


<details>
  <summary>Details</summary>
Motivation: 传统潜在扩散模型(LDMs)在高分辨率(≤0.25°)集合天气预报中存在扩散能力受限的问题。气象场缺乏任务无关的基础模型和明确的语义结构，使得基于VFM的正则化方法不适用。现有的基于频率的方法在同质性假设下对所有通道施加相同的频谱正则化，导致在多元气象数据的频谱异质性下正则化强度不均匀。

Method: 提出3D掩码自编码器(3D-MAE)编码天气状态演化特征作为扩散模型的额外条件，同时提出变量感知掩码频率建模(VA-MFM)策略，根据每个变量的频谱能量分布自适应选择阈值，共同构成PuYun-LDM框架。

Result: PuYun-LDM增强了潜在扩散能力，在短预报时效上表现优于ENS，在长预报时效上与ENS相当。在单个NVIDIA H200 GPU上5分钟内生成15天全球预报（6小时时间分辨率），并能高效并行生成集合预报。

Conclusion: PuYun-LDM通过结合3D-MAE和VA-MFM策略，有效解决了气象数据中LDMs的扩散能力受限问题，实现了高效高分辨率的集合天气预报，为气象AI模型提供了新的解决方案。
...
Result: 构建并发布ForeLen基准测试，包含长序列、思维链和RL数据。EGTP在ForeLen上达到最先进精度，MAE比最佳基线降低29.16%。将方法与长度感知调度器集成可获得显著的端到端吞吐量提升。

Conclusion: 该工作为高效LLM推理提供了新的技术和评估基准，通过轻量级框架有效解决了批处理推理中的计算浪费问题，在保持高精度的同时显著降低了预测开销。
...
Result: ProtoT在计算可扩展性上实现线性复杂度（vs SOTA自注意力变换器的二次复杂度），在模型和数据规模上扩展良好，在文本生成和下游任务（GLUE）上表现良好。原型提供了可解释的推理路径，并允许对其行为进行有针对性的编辑。
...
TL;DR: 本文提出了Talk2DM系统，通过自然语言查询增强动态地图系统，结合VRCsim仿真框架和VRC-QA数据集，使用链式提示机制整合人类规则与LLM常识知识。


<details>
  <summary>Details</summary>
Motivation: 现有动态地图系统缺乏自然语言支持的人机交互界面，限制了人类与动态地图系统的交互效率。虽然DM增强的自动驾驶系统已在日本实际部署，但缺乏自然语言查询功能。

Method: 1. 开发VRCsim仿真框架生成车路云协同感知数据流；2. 构建VRC-QA数据集专注于混合交通场景的空间查询和推理；3. 提出Talk2DM插件模块，采用新颖的链式提示机制，将人类定义规则与大型语言模型的常识知识渐进式整合。

Result: Talk2DM在不同LLM间无缝切换的同时保持高查询准确率（超过93%），平均响应时间仅2-5秒。虽然大模型准确率更高但效率下降明显，Qwen3:8B、Gemma3:27B和GPT-oss模型表现出色。

Conclusion: Talk2DM成功扩展了车路云动态地图系统的自然语言查询和常识推理能力，展示了强大的泛化能力和实际应用潜力，为动态地图系统提供了有效的人机交互解决方案。
...
TL;DR: HSAE（分层稀疏自编码器）通过联合学习多个SAE及其特征间的父子关系，从LLM中提取分层语义特征，比传统孤立特征提取方法更好地捕捉语言的内在层次结构。
...
Result: 在各种LLM和层上的广泛实验表明，HSAE始终能恢复语义上有意义的层次结构，得到定性案例研究和严格定量指标的支持。同时，HSAE在不同字典大小下保持了标准SAE的重建保真度和可解释性。

Conclusion: HSAE为发现和分析嵌入在LLM表示中的多尺度概念结构提供了一个强大、可扩展的工具，能够更好地理解和解释LLM的内部表示。
...
TL;DR: 论文提出选择性抽象框架，让大语言模型通过降低不确定内容的细节来平衡准确性和信息量，而不是简单的全有或全无的弃权机制。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在事实错误问题，影响用户信任和高风险应用。现有的不确定性估计机制采用二元"全有或全无"的弃权方式，在长文本生成中过于严格，会丢弃有价值的信息。

Method: 提出选择性抽象框架，通过原子级选择性抽象方法将回答分解为原子声明，用更高置信度但更抽象的表述替换不确定的原子声明。

Result: 在六个开源模型上，在FactScore和LongFact-Objects基准测试中，原子级选择性抽象方法显著优于现有基线，风险-覆盖率曲线下面积提升高达27.73%。

Conclusion: 通过降低细节特异性来提升准确性，同时保留大部分原始含义，为LLM可靠性问题提供了更灵活的解决方案。
...
Method: AlphaPROBE框架将alpha挖掘重构为有向无环图(DAG)的战略导航，将因子建模为节点，进化链接建模为边。框架包含两个核心组件：1)贝叶斯因子检索器，通过后验概率模型平衡利用和探索，识别高潜力种子；2)DAG感知因子生成器，利用因子的完整祖先轨迹，生成上下文感知的非冗余优化。

Result: 在三个主要中国股票市场数据集上对8个竞争基线的广泛实验表明，AlphaPROBE在预测准确性、收益稳定性和训练效率方面均获得显著提升。结果证实利用全局进化拓扑对于高效稳健的自动化alpha发现至关重要。

Conclusion: AlphaPROBE通过将因子池建模为动态互连生态系统，提供了一种新的alpha因子挖掘范式，解决了现有方法缺乏全局结构视角的问题，实现了更高效和多样化的因子发现。
...
TL;DR: MEME提出了一种逻辑导向的金融市场建模方法，将市场视为竞争性投资叙事（思维模式）的动态演化生态系统，通过多智能体提取投资论点和高斯混合建模来识别潜在共识，在三个中国股票池中超越了7个SOTA基线。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的量化金融方法主要采用资产中心范式（关注个股预测）或市场中心范式（关注投资组合分配），往往忽略了驱动市场变动的底层推理逻辑。需要一种能够捕捉市场动态演化逻辑的新视角。

Method: MEME采用逻辑导向视角，将金融市场建模为竞争性投资叙事（思维模式）的动态演化生态系统。使用多智能体提取模块将噪声数据转化为高质量投资论点，通过高斯混合建模在语义空间中识别潜在共识，并实现时间评估和对齐机制来追踪这些模式的演变周期和历史盈利能力。

Result: 在2023-2025年三个异质性中国股票池上的广泛实验表明，MEME在7个SOTA基线方法中表现一致更优。消融研究、敏感性分析、生命周期案例研究和成本分析验证了MEME识别和适应金融市场演化共识的能力。

Conclusion: MEME通过逻辑导向的金融市场建模方法，能够捕捉市场动态演化中的投资叙事共识，优先考虑持久市场智慧而非短暂异常，为投资组合构建提供稳健的推理指导，在量化金融领域展现出显著优势。
...
TL;DR: Gaia2是一个用于评估LLM智能体在真实异步环境中的基准测试，引入独立于智能体动作的环境演化场景，支持细粒度动作级评估和强化学习


<details>
  <summary>Details</summary>
Motivation: 现有评估基准多为静态或同步环境，缺乏对智能体在真实异步、动态环境中能力的评估，需要能够测试智能体在时间约束、噪声事件、模糊性处理和协作等方面的综合能力

Method: 基于开源Agents Research Environments平台构建消费者环境，每个场景配备写动作验证器，支持细粒度动作级评估和可验证奖励的强化学习

Result: 评估显示没有模型在所有能力上占优：GPT-5总体得分最高（42% pass@1）但在时间敏感任务上失败，Claude-4 Sonnet在准确性和速度间权衡成本，Kimi-K2在开源模型中领先（21% pass@1）
...
TL;DR: CSEval：利用语言模型评估医学图像生成中临床语义对齐的新框架，能识别其他指标忽略的语义不一致问题


<details>
  <summary>Details</summary>
Motivation: 现有医学图像生成评估方法主要关注图像真实性和多样性，但无法评估生成的图像是否准确反映临床语义（如解剖位置和病理特征），这在医疗应用中至关重要

Method: 提出临床语义评估器（CSEval），利用语言模型评估生成图像与其条件提示之间的临床语义对齐程度

Result: CSEval能够识别其他指标忽略的语义不一致问题，并且与专家判断具有相关性，为现有评估方法提供了可扩展且具有临床意义的补充

Conclusion: CSEval框架支持生成模型在医疗领域的安全采用，通过评估临床语义对齐来确保生成图像的临床可靠性
...
TL;DR: 该研究提出两种无需参数更新的优化方法（InjectCorrect和InjectRLOpt），通过注入模型自身推理行为模式来提升大语言模型的推理性能，在多种推理任务上分别获得最高5.34%和8.67%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要通过直觉性的提示调整来增强推理能力，缺乏对底层行为模式的系统性分析。作者旨在从行为模式角度探究模型推理行为如何塑造推理过程，并基于观察到的行为模式分布规律开发优化方法。
...
Conclusion: 模型在面对特定类型问题时表现出自适应的推理行为分布，结构化注入这些行为模式能显著影响推理过程和结果质量。提出的InjectCorrect和InjectRLOpt方法有效利用了模型自身的行为模式来增强推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在处理复杂任务时经常产生过长的思维链，包含大量重复自我提问和循环推理等不必要的反思步骤，导致高令牌消耗、计算开销增加和延迟上升，但并未提高准确性，特别是在较小模型中问题更严重。

Method: 提出自适应反射与长度协调惩罚（ARLCP）强化学习框架，包含两个关键创新：1）自适应反射惩罚，动态减少不必要的反思步骤同时保留必要推理；2）根据问题估计复杂度校准的长度惩罚。通过协调这两个惩罚机制，鼓励模型生成更简洁有效的推理路径。
...
Conclusion: ARLCP框架有效解决了大型推理模型中过度反思导致的效率问题，通过动态平衡推理效率与解决方案准确性，显著减少了计算开销同时提高了模型性能，为推理模型的优化提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统二次注意力计算成本高，线性注意力作为高效近似但使用低阶有理函数近似softmax，效果有限。需要一种既能保持线性注意力计算效率，又能更好近似标准softmax注意力的方法。

Method: 提出Hadamard线性注意力(HLA)，不同于传统线性注意力在查询和键上分别应用非线性，HLA在计算成对相似度后应用非线性变换，使用更高阶有理函数近似softmax，并推导出类似标准线性注意力的高效计算方案，无需耗时的张量重塑操作。

Result: HLA在保持线性注意力计算效率的同时，能更好地近似标准softmax注意力。在大型视频生成扩散变换器模型中的应用证明了其有效性，特别是在处理大量token的场景中。

Conclusion: Hadamard线性注意力提供了一种新颖的线性注意力变体，通过改进非线性应用位置和使用更高阶有理函数，在保持计算效率的同时更好地近似标准注意力机制，适用于大规模token应用场景。
...
TL;DR: 研究量化了Gemini Flash 2.5 Image和GPT Image 1.5两个商业图像生成器中的性别和肤色偏见，发现中性提示词会产生高度极化的默认输出，两个模型都表现出强烈的"默认白人"偏见，但在性别偏好上存在分歧。
...
Method: 使用四个语义中性提示词生成3,200张照片级真实图像，采用混合颜色归一化、面部标志点掩码和感知均匀肤色量化（使用Monk、PERLA和Fitzpatrick量表）的严格分析流程。

Result: 中性提示词产生了高度极化的默认输出：两个模型都表现出强烈的"默认白人"偏见（>96%输出），但在性别偏好上存在分歧——Gemini偏好女性呈现的主体，而GPT偏好肤色较浅的男性呈现主体。
...
Method: 引入价值对齐税（VAT）框架，通过基于施瓦茨价值理论的受控情境-行动数据集，收集对齐前后的规范性判断配对数据，分析不同模型、价值和策略下的对齐效应。

Result: 结果显示对齐通常会在价值间产生不均衡、结构化的协同变动。这些效应在传统的仅关注目标的评估中不可见，揭示了系统性和过程层面的对齐风险。

Conclusion: VAT框架为理解LLM中价值对齐的动态性提供了新视角，强调了考虑整个价值系统变化的重要性，而不仅仅是单一目标价值的改进。
...
<details>
  <summary>Details</summary>
Motivation: 随着大模型全面评估成本急剧增加，从有限观测中预测模型性能变得至关重要。现有统计方法难以处理模式变化、数据稀疏和缺乏解释性，而纯LLM方法仍不可靠。

Method: 提出STAR框架，结合数据驱动的统计期望与知识驱动的智能推理。使用专业检索器获取外部知识，将语义特征嵌入约束概率矩阵分解(CPMF)生成带不确定性的统计期望。基于期望违背理论(EVT)的推理模块通过族内分析、跨模型比较和可信度感知聚合来细化预测，产生可追溯解释的调整。

Result: 在广泛实验中，STAR在所有基线上均表现优异，在基于分数和基于排名的指标上都优于所有基线方法。在极端稀疏条件下（每个测试模型仅1-2个观测分数），比最强统计方法的总分提高了14.46%。

Conclusion: STAR框架成功解决了大模型性能预测中的模式变化、数据稀疏和解释性不足问题，通过统计与知识推理的结合实现了更准确、可解释的预测。
...
Method: 使用强化学习训练T5语言模型，将数据压缩为token序列，采用离策略强化学习算法优化序列长度

Result: 相比传统方法显著提高压缩比，保持语义完整性，无需外部语法或世界知识

Conclusion: 基于强化学习的token序列压缩方法为无损压缩提供了更高效实用的解决方案
...
TL;DR: 研究发现，尽管大语言模型在简单的心理理论任务上能近似人类判断，但在逻辑等价任务上失败，且其行为预测与心理状态推断之间一致性低，表明其社交能力并非源于领域通用或一致的心理理论。


<details>
  <summary>Details</summary>
Motivation: 当前研究主要评估大语言模型在心理理论基准测试上的表现，但这些评估并未测试心理理论所假设的实际表征——即心理状态与行为的因果模型。本研究旨在开发基于认知心理学定义的新评估框架，探究LLMs是否真正拥有心理理论能力。

Method: 采用基于认知心理学的心理理论定义，开发新的评估框架，测试LLMs是否拥有连贯、领域通用且一致的心理状态导致行为的模型。通过简单心理理论范式及其逻辑等价任务，比较模型在行为预测和心理状态推断上的一致性。

Result: 研究发现：1) LLMs在简单心理理论范式上能近似人类判断；2) 但在逻辑等价任务上失败；3) 其行为预测与相应心理状态推断之间一致性较低。这表明LLMs的社交熟练度并非源于领域通用或一致的心理理论。

Conclusion: 大语言模型虽然表现出一定的社交能力，但这种能力并非基于真正的心理理论机制。它们缺乏连贯、领域通用且一致的心理状态-行为因果模型，其成功更多是表面近似而非深层理解。
...
TL;DR: Sci-CoE：两阶段科学协同演化框架，通过从稀疏监督到无监督学习的转变，让模型作为求解器和验证器自我演化，提升科学推理能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在科学推理任务中仍然脆弱，主要原因是不可靠的解决方案评估和验证策略的多样性有限。现有协同演化范式在代码和数学领域表现良好，但在科学推理方面需要改进。

Method: 提出Sci-CoE两阶段框架：第一阶段使用少量标注数据为验证器建立基本正确性判断锚点；第二阶段引入几何奖励机制，综合考虑共识、可靠性和多样性，在无标签数据上进行大规模自我迭代。

Result: 在多个通用科学基准测试上的实验表明，Sci-CoE增强了复杂推理能力，表现出强大的可扩展性，有助于构建更鲁棒和多样化的评估系统。

Conclusion: Sci-CoE通过协同演化框架有效解决了科学推理中的评估不可靠和策略单一问题，为构建更强大的科学推理系统提供了新途径。
...
TL;DR: 提出基于教育原则的三阶段IOA框架，通过知识识别、组织和适配，实现LLM到小模型的高效知识蒸馏，在推理任务上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM知识蒸馏方法缺乏教学意识，将知识转移视为一次性数据合成和训练任务，而不是系统化的学习过程。需要借鉴教育原则来改进蒸馏效果。

Method: 提出三阶段IOA框架：1) 知识识别器识别学生模型的知识缺陷；2) 知识组织器通过渐进式课程组织知识传递；3) 知识适配器调整表示以匹配学生模型的认知能力。整合布鲁姆掌握学习原则和维果茨基最近发展区理论，创建动态蒸馏过程。

Result: 使用LLaMA-3.1/3.2和Qwen2.5作为学生模型的实验表明，IOA在DollyEval上保留了教师模型94.7%的性能，参数少于1/10。在复杂推理任务上表现突出：MATH提升19.2%，HumanEval提升22.3%，显著超越现有基线方法。

Conclusion: 基于教育原则的IOA框架为LLM知识蒸馏提供了系统化的教学意识方法，显著提升了小模型在复杂任务上的性能，为高效AI系统部署提供了新思路。
...
TL;DR: SAM3-LiteText：通过轻量级文本编码框架替换SAM3的大文本编码器，在保持分割性能的同时减少88%参数，降低内存开销


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言分割模型（如SAM3）使用为开放语言理解设计的大型通用文本编码器，但实际分割提示通常简短、结构化且语义受限，导致文本编码器容量过度配置，计算和内存开销大

Method: 对404,796个真实提示进行大规模解剖分析，发现严重冗余；提出SAM3-LiteText框架，用紧凑的MobileCLIP学生模型替换原SAM3文本编码器，通过知识蒸馏优化
...
TL;DR: 语音识别系统在标准基准测试中表现良好，但在真实世界高风险短语音转录（如美国街道名称）中失败率高达44%，对非英语母语者影响更大，通过合成数据微调可显著改善准确性。


<details>
  <summary>Details</summary>
Motivation: 尽管语音识别系统在标准基准测试中取得了低词错误率，但在真实世界部署中，对于高风险短语音（如街道名称）的转录经常失败。本研究旨在探索这一失败模式，特别是在美国街道名称转录这一高风险任务中，并量化其对不同语言背景使用者的影响差异。

Method: 评估了15个来自OpenAI、Deepgram、Google和Microsoft的语音识别模型，使用来自语言多样化的美国说话者的录音。引入了合成数据生成方法，利用开源文本转语音模型生成命名实体的多样化发音。使用少于1000个合成样本对模型进行微调。

Result: 在街道名称转录任务中，平均转录错误率达到44%。错误对所有说话者都有系统性影响，但对非英语母语者的路由距离错误是英语母语者的两倍。通过合成数据微调，非英语母语者的街道名称转录准确率相对于基础模型提高了近60%。

Conclusion: 研究揭示了语音系统基准性能与真实世界可靠性之间的关键差距，特别是在高风险短语音转录任务中。合成数据生成方法提供了一条简单、可扩展的路径来减少高风险转录错误，特别是对非英语母语者等边缘化群体。
...
TL;DR: KeplerAgent是一个基于大语言模型的智能框架，通过模拟科学家的多步推理过程（先推断物理属性，再限制方程空间）来发现符号方程，相比直接猜测的方法在准确性和抗噪性上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型方法直接从数据猜测方程，没有模拟科学家常用的多步推理过程（先推断物理属性如对称性，再用这些作为先验限制候选方程空间），这限制了方程发现的准确性和鲁棒性。

Method: KeplerAgent是一个智能框架，协调基于物理的工具来提取中间结构，并使用这些结果来配置符号回归引擎（如PySINDy和PySR），包括它们的函数库和结构约束，从而模拟科学家的推理过程。

Result: 在一系列物理方程基准测试中，KeplerAgent相比传统的大语言模型和传统基线方法，实现了显著更高的符号准确性和对噪声数据更强的鲁棒性。

Conclusion: 通过模拟科学家的多步推理过程（先推断物理属性再限制方程空间），KeplerAgent框架在符号方程发现任务上表现优异，为科学发现提供了更可靠的工具。
...
### [222] [Mitigating Error Accumulation in Continuous Navigation via Memory-Augmented Kalman Filtering](https://arxiv.org/abs/2602.11183)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航模型采用航位推测法，通过迭代更新位置来预测下一个航点，这种方式会导致随时间累积的位置误差，产生内部信念与客观坐标之间的错位（状态漂移），最终影响完整轨迹预测的准确性。

Method: 受经典控制理论启发，将序列预测建模为递归贝叶斯状态估计问题。设计NeuroKalman框架，将导航解耦为两个互补过程：基于运动动力学的先验预测和基于历史观测的似然校正。通过核密度估计将测量似然与基于注意力的检索机制数学关联，使系统能够在不进行梯度更新的情况下使用检索到的历史锚点修正潜在表示。

Result: 在TravelUAV基准测试上的综合实验表明，仅使用10%的训练数据进行微调，该方法明显优于强基线模型，并有效调节了漂移累积。

Conclusion: 通过将递归贝叶斯状态估计引入视觉语言导航，NeuroKalman框架成功解决了状态漂移问题，提高了无人机在复杂环境中连续导航的准确性和鲁棒性。
...
### [223] [H-WM: Robotic Task and Motion Planning Guided by Hierarchical World Model](https://arxiv.org/abs/2602.11291)
...
<details>
  <summary>Details</summary>
Motivation: 现有世界模型方法侧重于视频生成或自然语言预测，难以直接应用于机器人动作，且长时程规划中误差会累积。传统任务和运动规划使用符号逻辑世界模型，虽然可执行且鲁棒，但缺乏视觉感知的同步预测能力。

Method: 提出分层世界模型(H-WM)，采用双层框架：高层逻辑世界模型预测符号状态转换，低层视觉世界模型预测视觉状态转换。通过机器人数据集训练，该数据集对齐了机器人运动、符号状态、动作和视觉观测。

Result: 在视觉-语言-动作(VLA)控制策略实验中验证了方法的有效性和通用性。分层输出为长时程任务提供稳定一致的中间指导，减轻误差累积，实现跨扩展任务序列的鲁棒执行。

Conclusion: H-WM成功整合了符号推理的机器人可执行性和长时程鲁棒性与视觉观测的感知基础，为机器人规划提供了统一的双层世界模型框架。
...
### [224] [ExtremControl: Low-Latency Humanoid Teleoperation with Direct Extremity Control](https://arxiv.org/abs/2602.11321)
...
<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人遥操作系统依赖预处理的人-机器人运动重定向和位置控制，导致显著延迟（约200ms），限制了响应性，无法完成需要快速反馈和反应的任务

Method: 提出ExtremControl框架：1) 直接操作选定刚性链接（主要是人形机器人末端）的SE(3)位姿，避免全身重定向；2) 使用笛卡尔空间映射直接将人类运动转换为机器人链接目标；3) 在底层加入速度前馈控制以支持快速变化的控制接口下的高响应性行为

Result: 实现了端到端延迟低至50ms的人形机器人遥操作系统，支持光学动作捕捉和VR运动追踪，能够完成乒乓球平衡、杂耍和实时回击等高响应性任务，显著超越了先前工作的200ms延迟限制

Conclusion: ExtremControl框架通过简化控制流程和加入速度前馈，成功构建了低延迟的人形机器人遥操作系统，为实现多样化反应性和动态演示收集提供了有效解决方案
...
### [225] [MolmoSpaces: A Large-Scale Open Ecosystem for Robot Navigation and Manipulation](https://arxiv.org/abs/2602.11337)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界机器人部署需要应对日常场景的长尾问题，现有机器人基准测试无法覆盖真实环境中场景布局、物体几何和任务规范的无数变化，物理评估无法提供所需规模和多样性的基础设施。

Method: 创建MolmoSpaces生态系统，包含23万多个多样化室内环境（从手工制作的家庭场景到程序生成的多房间房屋），13万个丰富标注的物体资产（包括4.8万个可操作物体和4200万个稳定抓取），支持多种模拟器（MuJoCo、Isaac、ManiSkill），并设计MolmoSpaces-Bench基准测试套件包含8个任务。

Result: MolmoSpaces-Bench表现出强大的模拟到真实相关性（R=0.96，ρ=0.98），确认新的零样本策略在基准测试中优于早期版本，并识别出对提示措辞、初始关节位置和相机遮挡的关键敏感性。

Conclusion: MolmoSpaces通过开源资产和工具为机器人学习研究提供了可扩展的数据生成、策略训练和基准创建的基础，支持机器人策略的大规模评估和开发。
...
### [226] [Human Preference Modeling Using Visual Motion Prediction Improves Robot Skill Learning from Egocentric Human Video](https://arxiv.org/abs/2602.11393)
*Mrinal Verghese,Christopher G. Atkeson*

Main category: cs.RO

TL;DR: 提出一种从人类第一视角视频学习机器人行为的方法，通过建模人类偏好为奖励函数，并优化机器人行为来最大化该奖励。该方法通过学习预测跟踪点在连续图像间的运动，并将奖励定义为机器人行为中预测与观察物体运动的一致性。
...
Method: 方法包括：1) 学习预测跟踪点在连续图像间的运动来建模人类偏好；2) 将奖励函数定义为机器人行为中预测与观察物体运动在每个步骤的一致性；3) 使用改进的Soft Actor Critic算法，用10个机器人演示初始化，从该奖励估计价值函数；4) 优化策略以最大化该价值函数，整个过程在机器人上完成。

Result: 该方法能够在真实机器人上学习，在模拟和真实机器人上的多个任务中，使用该奖励模型学习的策略匹配或优于先前工作。

Conclusion: 通过建模人类偏好为预测运动一致性奖励，并直接在机器人上优化策略，该方法能够有效从人类第一视角视频学习机器人行为，克服了现有方法在跨本体转移和视频学习方面的限制。
...
### [227] [EasyMimic: A Low-Cost Framework for Robot Imitation Learning from Human Videos](https://arxiv.org/abs/2602.11464)
*Tao Zhang,Song Xia,Ye Wang,Qin Jin*

Main category: cs.RO

TL;DR: EasyMimic是一个低成本可复制的机器人模仿学习框架，通过RGB摄像头采集人类视频演示，提取3D手部轨迹并映射到低成本机器人控制空间，显著减少对昂贵机器人数据收集的依赖。


<details>
  <summary>Details</summary>
Motivation: 机器人模仿学习通常需要大量真实世界数据，收集成本高昂。对于面向家庭使用的低成本机器人来说，这尤其具有挑战性，因为这类机器人需要既用户友好又价格实惠。

Method: 1) 从RGB摄像头视频中提取3D手部轨迹；2) 通过动作对齐模块将轨迹映射到低成本机器人夹爪控制空间；3) 引入简单用户友好的手部视觉增强策略来弥合人-机器人领域差距；4) 使用协同训练方法，在处理的的人类数据和少量机器人数据上微调模型，实现快速适应新任务。
...
### [228] [Effective Task Planning with Missing Objects using Learning-Informed Object Search](https://arxiv.org/abs/2602.11468)
...
Method: 开发了基于模型的LIOS（Learning-Informed Object Search）动作框架，每个LIOS动作是一个旨在查找和检索单个对象的策略。高层规划将LIOS动作视为确定性动作，通过基于模型计算每个动作的预期成本，生成交织搜索和执行的任务计划

Result: 在模拟的ProcTHOR家庭环境和真实世界中，该方法在检索和餐食准备等任务上优于非学习和学习基线方法，能够有效处理不确定性同时保持与现有全知识求解器的兼容性

Conclusion: 该框架通过将学习型对象搜索整合到任务规划中，实现了在不确定环境下的有效、可靠且完整的任务规划，既考虑了不确定性又保持了与现有求解器的兼容性
...
### [229] [ReaDy-Go: Real-to-Sim Dynamic 3D Gaussian Splatting Simulation for Environment-Specific Visual Navigation with Moving Obstacles](https://arxiv.org/abs/2602.11575)
*Seungyeon Yoo,Youngseok Jang,Dabin Kim,Youngsoo Han,Seungwoo Jung,H. Jin Kim*

Main category: cs.RO

TL;DR: ReaDy-Go是一个新颖的真实到仿真模拟管道，通过结合静态3D高斯泼溅场景和动态人类障碍物，生成逼真的动态导航数据集，以训练对仿真到真实差距和移动障碍物都具有鲁棒性的导航策略。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉导航模型在真实动态环境中表现不佳，主要因为仿真到真实的差距以及难以针对特定部署环境（如家庭、餐厅、工厂）训练策略。虽然使用3D高斯泼溅的真实到仿真模拟可以缓解这一差距，但先前工作仅假设静态场景或不现实的动态障碍物，而动态环境中的安全导航至关重要。
...
Result: ReaDy-Go在仿真和真实世界实验中均优于基线方法，在目标环境中展示了改进的导航性能，即使在仿真到真实迁移后和存在移动障碍物的情况下。此外，在未见环境中的零样本仿真到真实部署显示了其泛化潜力。

Conclusion: ReaDy-Go通过生成逼真的动态场景，成功解决了视觉导航在动态环境中的挑战，提高了导航策略对仿真到真实差距和移动障碍物的鲁棒性，并展示了良好的泛化能力。
...
### [230] [ABot-N0: Technical Report on the VLA Foundation Model for Versatile Embodied Navigation](https://arxiv.org/abs/2602.11598)
...
TL;DR: ABot-N0是一个统一的视觉-语言-动作基础模型，通过分层"大脑-动作"架构实现了5个核心导航任务的"大统一"，在7个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决具身导航领域长期以来任务特定架构碎片化的问题，实现跨多个核心导航任务的统一模型。

Method: 采用分层"大脑-动作"架构：LLM基础的认知大脑进行语义推理，配合基于流匹配的动作专家生成精确连续轨迹；开发了ABot-N0数据引擎，收集了大规模训练数据。

Result: 在7个基准测试中达到新的最先进性能，显著优于专用模型；智能导航系统结合规划器和分层拓扑记忆，能在动态现实环境中执行鲁棒的长时程任务。

Conclusion: ABot-N0实现了具身导航任务的统一，通过大规模数据训练和分层架构设计，在多个任务上超越了专用模型，为现实世界智能导航系统奠定了基础。
...
### [231] [ViTaS: Visual Tactile Soft Fusion Contrastive Learning for Visuomotor Learning](https://arxiv.org/abs/2602.11643)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注视觉和触觉特征的对齐，集成机制通常是直接拼接，忽视了两种模态的固有互补性，对齐利用不足，难以有效处理遮挡场景，限制了实际部署潜力。
...
Conclusion: ViTaS是一个简单而有效的框架，通过软融合对比学习和CVAE模块更好地利用视觉和触觉信息的对齐与互补，在遮挡场景中表现出色，具有实际部署潜力。
...
### [232] [AC-MASAC: An Attentive Curriculum Learning Framework for Heterogeneous UAV Swarm Coordination](https://arxiv.org/abs/2602.11735)
...
<details>
  <summary>Details</summary>
Motivation: 异构无人机集群的协同路径规划对多智能体强化学习提出了重大挑战，特别是在处理非对称智能体间依赖关系、应对稀疏奖励风险以及训练过程中的灾难性遗忘问题方面。

Method: 提出注意力课程学习框架AC-MASAC：1）引入角色感知异构注意力机制来显式建模非对称依赖关系；2）设计结构化课程策略，整合分层知识迁移和阶段比例经验回放来解决稀疏奖励和灾难性遗忘问题。

Result: 在自定义多智能体仿真平台上验证，结果显示该方法在成功率、编队保持率和任务时间加权成功率等指标上相比其他先进方法具有显著优势。
...
### [233] [HAIC: Humanoid Agile Object Interaction Control via Dynamics-Aware World Model](https://arxiv.org/abs/2602.11758)
...
TL;DR: HAIC是一个统一框架，用于人形机器人与具有独立动力学特性的欠驱动物体进行鲁棒交互，无需外部状态估计，通过动力学预测器和动态占据地图实现复杂任务


<details>
  <summary>Details</summary>
Motivation: 现有的人-物交互方法主要关注完全驱动且与机器人刚性耦合的物体，忽略了具有独立动力学和非完整约束的欠驱动物体，这些物体带来的耦合力和遮挡问题给控制带来了挑战

Method: HAIC框架包含：1）动力学预测器，仅从本体感知历史估计高阶物体状态；2）将预测投影到静态几何先验形成空间接地的动态占据地图；3）非对称微调，世界模型持续适应学生策略的探索

Result: 在人形机器人实验中，HAIC在敏捷任务（滑板、推/拉不同负载的推车）中实现了高成功率，能主动补偿惯性扰动，并能掌握多物体长时域任务（如携带箱子穿越不同地形）

Conclusion: HAIC为处理各种物体动力学提供了统一的鲁棒交互框架，无需外部状态估计，通过本体感知预测和动态占据地图成功解决了欠驱动物体的控制挑战
...
### [234] [LAMP: Implicit Language Map for Robot Navigation](https://arxiv.org/abs/2602.11862)
*Sibaek Lee,Hyeonwoo Yu,Giseop Kim,Sunwook Choi*

Main category: cs.RO

TL;DR: LAMP提出了一种基于神经语言场的导航框架，通过隐式表示语言特征和稀疏图结合，实现高效的大规模环境导航和精细路径规划。


<details>
  <summary>Details</summary>
Motivation: 现有基于显式存储语言向量的导航方法在大规模环境中存在内存需求过高和分辨率有限的问题，无法支持精细规划。

Method: 1) 学习连续的语言驱动地图作为隐式神经场；2) 结合稀疏图进行粗路径规划；3) 在学习的场中进行梯度优化细化姿态；4) 采用贝叶斯框架建模嵌入不确定性；5) 使用图采样策略优先考虑空间覆盖和嵌入置信度。

Result: 在NVIDIA Isaac Sim和真实多层建筑中的实验表明，LAMP在内存效率和精细目标到达精度方面优于现有显式方法。

Conclusion: LAMP通过隐式语言地图表示和粗到细的规划管道，为大规模环境下的零样本导航提供了高效且精确的解决方案。
...
### [235] [Learning to Manipulate Anything: Revealing Data Scaling Laws in Bounding-Box Guided Policies](https://arxiv.org/abs/2602.11885)
*Yihao Wu,Jinming Ma,Junbo Tan,Yanzhao Yu,Shoujie Li,Mingliang Zhou,Diyun Xiang,Xueqian Wang*

Main category: cs.RO

TL;DR: 本文提出了一种结合边界框指令的语义操作扩散策略，通过手持分割设备和自动标注流程收集大规模演示数据，揭示了语义操作任务中数据规模与泛化性能之间的幂律关系。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的策略在语义操作任务中泛化能力有限，这成为现实世界机器人部署的关键障碍。仅依赖文本指令不足以在复杂动态环境中引导策略关注目标对象。

Method: 提出利用边界框指令直接指定目标对象，设计手持分割设备Label-UMI实现高效标注数据收集，并提出语义-运动解耦框架，集成目标检测和边界框引导的扩散策略。

Result: 在大规模数据集上的真实世界实验中验证了方法的有效性，揭示了泛化性能与边界框对象数量之间的幂律关系，总结出能在四个任务上对已见和未见对象达到85%成功率的有效数据收集策略。

Conclusion: 通过边界框指令和语义-运动解耦框架显著提升了语义操作的泛化能力，发现了数据规模与性能的幂律关系，为语义操作任务提供了有效的数据收集策略，所有数据集和代码将开源。
...
### [236] [General Humanoid Whole-Body Control via Pretraining and Fast Adaptation](https://arxiv.org/abs/2602.11929)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要任务特定训练或在新运动适应时性能下降，人形机器人控制面临运动分布多样性、快速适应困难和高动态场景下平衡保持等挑战

Method: 提出Parseval引导的残差策略适应，在正交性和KL约束下学习轻量级增量动作策略；引入质心感知控制，整合质心相关观测和目标以增强平衡

Result: 在仿真和真实世界部署中，FAST在鲁棒性、适应效率和泛化能力方面持续优于最先进的基线方法

Conclusion: FAST框架通过高效的策略适应机制和增强的平衡控制，为人形机器人提供了通用、快速适应且稳定的全身控制解决方案
...
### [237] [Accelerating Robotic Reinforcement Learning with Agent Guidance](https://arxiv.org/abs/2602.11978)
*Haojun Chen,Zili Zou,Chengdong Ma,Yaoxiang Pu,Haotong Zhang,Yuanpei Chen,Yaodong Yang*

Main category: cs.RO

TL;DR: AGPS用多模态智能体替代人类监督，通过语义世界模型提供内在价值先验，使用可执行工具提供精确指导，实现自动化机器人策略搜索，显著提高样本效率。


<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人操作技能学习中样本效率低下，现有的人机协同方法存在1:1监督比例限制、操作员疲劳和人类技能不一致等问题，难以规模化扩展。

Method: 提出Agent-guided Policy Search框架，用多模态智能体替代人类监督，将智能体视为语义世界模型，通过可执行工具提供纠正路径点和空间约束来引导物理探索。

Result: 在从精密插入到可变形物体操作的两个任务上验证，AGPS在人机协同方法的基础上显著提高了样本效率，实现了监督流程的自动化。

Conclusion: AGPS框架通过自动化监督流程，为无人工干预和可扩展的机器人学习开辟了道路，解决了人机协同方法的可扩展性瓶颈。
...
### [238] [Decentralized Multi-Robot Obstacle Detection and Tracking in a Maritime Scenario](https://arxiv.org/abs/2602.12012)
*Muhammad Farhan Ahmed,Vincent Frémont*

Main category: cs.RO

TL;DR: 本文提出了一种分散式多机器人框架，用于使用多个无人机与自主水面船只协作检测和跟踪海上漂浮容器。


<details>
  <summary>Details</summary>
Motivation: 自主空中-水面机器人团队在海上监测方面具有前景，但需要在水面反射环境下实现可靠的感知，并在有限通信条件下实现可扩展的协调。

Method: 每个无人机使用YOLOv8和立体视差进行视觉检测，然后通过基于不确定性的数据关联使用每对象EKF跟踪目标。通过协方差交集交换和融合紧凑的跟踪摘要，确保在未知相关性下的一致性。信息驱动的分配模块通过权衡预期不确定性减少与旅行成本和安全性来分配目标和选择无人机悬停视点。

Result: 海上场景的仿真结果表明，该方法在保持适度通信需求的同时，提高了覆盖范围、定位精度和跟踪一致性。

Conclusion: 该分散式框架能够有效协调多机器人系统进行海上漂浮容器检测和跟踪，解决了水面反射环境和有限通信条件下的挑战。
...
### [239] [Adaptive-Horizon Conflict-Based Search for Closed-Loop Multi-Agent Path Finding](https://arxiv.org/abs/2602.12024)
*Jiarui Li,Federico Pecora,Runyu Zhang,Gioele Zardini*

Main category: cs.RO

TL;DR: ACCBS是一种基于CBS的闭环多智能体路径规划算法，通过动态调整规划时域和重用约束树，实现了快速高质量解和渐进最优性，平衡了理论最优性和实际鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体路径规划方法要么是开环规划器（生成固定轨迹，难以处理扰动），要么是缺乏可靠性能保证的闭环启发式方法，限制了在安全关键部署中的应用。

Method: 基于有限时域CBS变体，采用受MPC迭代深化启发的时域变化机制，动态调整规划时域，重用单一约束树实现不同时域间的无缝转换。

Result: ACCBS能够快速生成高质量可行解，随着计算预算增加具有渐进最优性，表现出随时行为，有效结合了对扰动的灵活性和强性能保证。

Conclusion: ACCBS成功弥合了大规模机器人部署中理论最优性和实际鲁棒性之间的差距，为自动化仓库和物流中的大型机器人车队提供了有效的协调解决方案。
...
### [240] [When would Vision-Proprioception Policies Fail in Robotic Manipulation?](https://arxiv.org/abs/2602.12032)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究对视觉-本体感知策略的泛化能力存在不一致的观察结果，需要深入探究视觉和本体感知在机器人操作任务中的协作机制，特别是在复杂任务中如何提升策略性能。

Method: 提出梯度调整与相位引导（GAP）算法：利用本体感知捕捉机器人状态并估计轨迹中每个时间步属于运动转换阶段的概率；在策略学习过程中，基于估计概率精细调整本体感知梯度的大小，促进视觉模态的学习。

Result: GAP算法在模拟和真实环境中均适用，支持单臂和双臂设置，兼容传统模型和视觉-语言-动作模型，能够产生鲁棒且可泛化的视觉-本体感知策略。

Conclusion: 通过动态调整本体感知的优化过程，GAP算法能够促进视觉-本体感知策略中多模态的有效协作，为机器人操作中视觉-本体感知策略的开发提供了有价值的见解。
...
### [241] [Safety Beyond the Training Data: Robust Out-of-Distribution MPC via Conformalized System Level Synthesis](https://arxiv.org/abs/2602.12047)
*Anutam Srinivasan,Antoine Leeman,Glen Chou*

Main category: cs.RO

TL;DR: 提出了一种结合保形预测和系统级综合的鲁棒分布外规划控制框架，通过加权CP学习状态控制相关协方差模型，推导高置信度模型误差边界，并集成到SLS鲁棒非线性MPC中，通过体积优化前向可达集进行约束收紧。


<details>
  <summary>Details</summary>
Motivation: 解决在超出训练数据分布时使用学习动力学模型的安全性和鲁棒性挑战，确保在分布漂移情况下的可靠控制。

Method: 使用加权保形预测学习状态控制相关协方差模型，推导高置信度模型误差边界；将边界集成到基于系统级综合的鲁棒非线性模型预测控制中，通过体积优化前向可达集在预测时域内进行约束收紧。

Result: 提供了分布漂移下的覆盖率和鲁棒性理论保证，分析了数据密度和轨迹管大小对预测覆盖率的影响；在非线性系统（包括4D汽车和12D四旋翼）上实证验证，相比固定边界和非鲁棒基线，在数据分布外显著提升了安全性和鲁棒性。

Conclusion: 该框架有效解决了学习动力学模型在分布外场景下的安全鲁棒控制问题，通过保形预测和系统级综合的结合，为实际应用中的不确定性管理提供了理论保证和实用方法。
...
### [242] [HoloBrain-0 Technical Report](https://arxiv.org/abs/2602.12062)
...
TL;DR: HoloBrain-0是一个全面的视觉-语言-动作框架，通过结合机器人先验知识提升3D空间推理，采用预训练后训练范式在仿真和真实世界任务中取得SOTA结果，并开源完整生态系统。


<details>
  <summary>Details</summary>
Motivation: 弥合基础模型研究与可靠真实世界机器人部署之间的差距，通过显式结合机器人先验知识来增强3D空间推理能力，支持多样化的机器人具身化。

Method: 提出新颖的VLA架构，显式结合机器人具身先验（多视角相机参数和运动学描述URDF）；采用可扩展的"预训练后训练"范式；开发完整的HoloBrain生态系统，包括预训练VLA基础、后训练检查点以及RoboOrchard全栈基础设施。

Result: 在RoboTwin 2.0、LIBERO、GenieSim等仿真基准测试中取得最先进结果；在具有挑战性的长时程真实世界操作任务中表现强劲；高效的0.2B参数变体能与更大的基线模型竞争，支持低延迟设备端部署。

Conclusion: HoloBrain-0为高性能机器人操作提供了完整、可复现的路径，通过开源整个生态系统（包括预训练模型、后训练检查点和基础设施）加速研究和实际应用。
...
### [243] [VLAW: Iterative Co-Improvement of Vision-Language-Action Policy and World Model](https://arxiv.org/abs/2602.12063)
*Yanjiang Guo,Tony Lee,Lucy Xiaoyang Shi,Jianyu Chen,Percy Liang,Chelsea Finn*

Main category: cs.RO

TL;DR: 提出一种通过迭代在线交互提升视觉-语言-动作模型性能的方法，利用真实世界数据改进世界模型，再用改进后的世界模型生成合成数据来增强VLA模型训练
...
Method: 提出简单的迭代改进算法：使用真实世界策略执行数据改进世界模型的保真度，然后用改进后的世界模型生成补充合成数据，再用这些合成数据改进VLA模型，形成良性循环

Result: 在真实机器人实验中，该方法显著提升了最先进VLA模型在多个下游任务上的性能：相比基础策略获得39.2%的绝对成功率提升，相比仅使用生成合成数据训练获得11.6%的额外提升

Conclusion: 通过迭代改进世界模型并利用其生成合成数据的方法，可以有效提升VLA模型的性能和可靠性，为解决真实世界数据收集成本高的问题提供了可行方案
...
### [244] [Affordance-Graphed Task Worlds: Self-Evolving Task Generation for Scalable Embodied Learning](https://arxiv.org/abs/2602.12065)
*Xiang Liu,Sen Cui,Guocai Yao,Zhong Cao,Jingheng Ma,Min Zhang,Changshui Zhang*

Main category: cs.RO

TL;DR: AGT-World：基于真实世界观察自主构建交互式仿真环境和机器人任务策略的统一框架，通过结构化图表示任务空间，结合自进化机制实现可扩展的机器人学习


<details>
  <summary>Details</summary>
Motivation: 直接在现实世界中训练机器人策略成本高昂且难以扩展。现有生成仿真方法难以生成逻辑一致的长时程任务，且由于开环执行而难以应对动态物理不确定性

Method: 提出AGT-World框架：1）将任务空间形式化为结构化图，实现复杂目标的精确层次分解；2）引入结合视觉语言模型推理和几何验证的混合反馈自进化机制，自主优化策略

Result: 实验表明该方法在成功率和泛化能力上显著优于现有方法，实现了提案、执行和修正的自改进循环，支持可扩展的机器人学习

Conclusion: AGT-World通过结构化任务表示和自进化机制，有效解决了机器人策略训练中的可扩展性和逻辑一致性问题，为大规模机器人学习提供了新途径
...
### [245] [RF-Modulated Adaptive Communication Improves Multi-Agent Robotic Exploration](https://arxiv.org/abs/2602.12074)
*Lorin Achey,Breanne Crockett,Christoffer Heckman,Bradley Hayes*

Main category: cs.RO

TL;DR: ART算法通过动态调整传输位置，基于信号强度和数据负载大小，使异构机器人团队在通信受限环境中高效共享信息，减少不必要的折返


<details>
  <summary>Details</summary>
Motivation: 在通信受限环境中，多机器人探索面临可靠协调和高效通信的挑战，需要一种能动态适应信号条件和数据负载的通信感知规划算法

Method: 提出自适应射频传输(ART)算法，根据信号强度和数据负载大小动态调整传输位置；进一步开发ART-SST扩展，强制执行信号强度阈值以确保高保真数据传输

Result: 在三个洞穴式环境中进行480多次模拟，ART算法始终优于现有策略（包括完全会合和最小信号启发式方法），相比基线方法减少58%的行驶距离和52%的探索时间
...
### [246] [Pack it in: Packing into Partially Filled Containers Through Contact](https://arxiv.org/abs/2602.12095)
...
<details>
  <summary>Details</summary>
Motivation: 仓库自动化对提高生产力和减少人员危险环境暴露至关重要。现有装箱研究主要关注空容器装箱且采用无碰撞策略，但实际中容器常已部分填充物品且布局不理想，需要处理已有物品的接触交互

Method: 使用基于接触的多物体轨迹优化器在模型预测控制器中实现，集成物理感知的感知系统（即使在遮挡时也能估计物体姿态），以及提出物理可行放置位置的方法

Result: 该方法能够通过有目的的接触交互创造空间，成功放置新物品，解决了部分填充容器的装箱问题

Conclusion: 接触感知的装箱方法能够有效处理实际仓库环境中部分填充容器的装箱任务，通过利用接触交互而非避免接触来创造放置空间
...
### [247] [Multi Graph Search for High-Dimensional Robot Motion Planning](https://arxiv.org/abs/2602.12096)
*Itamar Mishani,Maxim Likhachev*

Main category: cs.RO

TL;DR: MGS是一种基于搜索的运动规划算法，将经典单向和双向搜索推广到多图设置，通过维护多个隐式图并增量扩展，专注于高潜力区域探索，实现高效的高维机器人运动规划。


<details>
  <summary>Details</summary>
Motivation: 高维机器人系统（如机械臂和移动机械臂）的高效运动规划对实时操作和可靠部署至关重要。现有规划算法虽然提高了对高维状态空间的扩展性，但往往以生成不可预测、不一致的运动或需要过多计算资源和内存为代价。

Method: 提出多图搜索（MGS）算法，将经典单向和双向搜索推广到多图设置。MGS在状态空间上维护并增量扩展多个隐式图，专注于高潜力区域探索，同时允许初始断开的子图在搜索过程中通过可行转换进行合并。

Result: 证明了MGS具有完备性和有界次优性，并在一系列操作和移动操作任务上实证展示了其有效性。提供了演示、基准测试和代码。

Conclusion: MGS是一种高效的高维机器人运动规划算法，通过多图搜索策略在保持算法完备性和有界次优性的同时，实现了更好的计算效率和运动质量。
...
### [248] [3DGSNav: Enhancing Vision-Language Model Reasoning for Object Navigation via Active 3D Gaussian Splatting](https://arxiv.org/abs/2602.12159)
*Wancai Zheng,Hao Chen,Xianlong Lu,Linlin Ou,Xinyi Yu*

Main category: cs.RO

TL;DR: 3DGSNav：一种新颖的零样本目标导航框架，通过3D高斯泼溅作为视觉语言模型的持久记忆来增强空间推理，实现高效可靠的目标定位
...
Method: 1）使用3D高斯泼溅作为VLMs的持久记忆，通过主动感知增量构建环境3DGS表示；2）设计结构化视觉提示并结合思维链提示增强VLM推理；3）实时目标检测器筛选潜在目标，VLM驱动的主动视点切换进行目标重新验证

Result: 在多个基准测试和四足机器人的真实世界实验中，该方法展现出稳健且具有竞争力的性能，优于现有最先进方法

Conclusion: 3DGSNav通过将3DGS作为VLMs的持久记忆，有效增强了空间推理能力，实现了高效可靠的零样本目标导航，为具身智能提供了新的解决方案
...
### [249] [Sub--Riemannian boundary value problems for Optimal Geometric Locomotion](https://arxiv.org/abs/2602.12199)
*Oliver Gross,Florine Hartwig,Martin Rumpf,Peter Schröder*

Main category: cs.RO

TL;DR: 提出几何模型计算细长运动体（如蛇、精子）的最优形状变化运动，将运动建模为亚黎曼测地线问题，同时考虑环境位移能耗和身体变形能耗


<details>
  <summary>Details</summary>
Motivation: 研究细长运动体（如蛇在沙地上滑行）的最优运动机制，这些运动完全由身体形状序列决定。现有模型通常只考虑位移能耗，忽略了身体变形（弯曲、拉伸）所需的代谢或驱动能耗，无法全面评估运动效率

Method: 提出几何模型，基于拉格朗日最小耗散原理建立边界值问题，其解由亚黎曼测地线给出。模型同时考虑环境位移能耗和身体变形能耗。开发了时间和空间一致离散化方法，可数值计算三种边界条件下的亚黎曼测地线：固定初始和目标身体、限制为循环运动、仅规定位移和方向

Result: 计算得到的最优变形步态在质量上与蛇和精子等生物观察到的运动轨迹相匹配，也与低维系统（如Purcell游泳器）的已知最优结果一致。由于几何约束较少，模型能对广义Purcell游泳器等运动机制提供新见解。代码已公开

Conclusion: 提出的几何模型能有效计算细长运动体的最优形状变化运动，同时考虑位移和变形能耗，为理解生物和机器人运动机制提供了新框架。模型比先前框架更灵活，能揭示新的运动机制
...
### [250] [LDA-1B: Scaling Latent Dynamics Action Model via Universal Embodied Data Ingestion](https://arxiv.org/abs/2602.12215)
...
TL;DR: LDA-1B是一个机器人基础模型，通过统一世界模型框架联合学习动力学、策略和视觉预测，利用结构化DINO潜在空间和扩散变换器处理异构数据，在30k小时数据集上训练，在接触密集、灵巧和长时域任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有机器人基础模型主要依赖大规模行为克隆，丢弃了异构体现数据中可迁移的动力学知识。统一世界模型有潜力利用这种多样数据，但现有实现因数据使用粗粒度且数据集碎片化而难以扩展到基础模型级别。

Method: 提出LDA-1B模型，通过联合学习动力学、策略和视觉预测来统一处理体现数据，为不同质量数据分配不同角色。使用结构化DINO潜在空间进行预测以避免冗余像素空间建模，采用多模态扩散变换器处理异步视觉和动作流，支持10亿参数规模训练。

Result: 在仿真和真实世界实验中，LDA-1B在接触密集任务上比先前方法提升21%，灵巧任务提升48%，长时域任务提升23%。特别地，能够高效微调，利用30%通常有害的低质量轨迹获得10%性能提升。

Conclusion: LDA-1B通过统一世界模型框架成功扩展到基础模型规模，有效利用了异构体现数据中的动力学知识，在多种机器人任务上取得显著性能提升，并展示了数据高效微调的能力。
...
### [251] [Any House Any Task: Scalable Long-Horizon Planning for Abstract Human Tasks](https://arxiv.org/abs/2602.12244)
*Zhihong Liu,Yang Li,Rengming Huang,Cewu Lu,Panpan Cai*

Main category: cs.RO

TL;DR: AHAT是一个用于大规模家庭环境的机器人任务规划系统，通过LLM将任务指令和场景图映射为PDDL子目标，再通过符号推理生成最优长时程规划，使用TGPO算法增强意图分解能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的任务规划方法在环境规模扩大、规划长度增加、指令模糊性和约束复杂性增加时性能会迅速下降，需要解决开放世界语言条件任务规划的可扩展性问题。

Method: AHAT使用训练过的LLM将任务指令和文本场景图映射到PDDL中定义的接地子目标，然后通过显式符号推理解决这些子目标以生成可行且最优的长时程规划。引入TGPO强化学习算法，将中间推理轨迹的外部校正集成到GRPO中，增强模型分解复杂模糊意图的能力。

Result: 实验表明，AHAT在性能上显著优于最先进的提示、规划和学习方法，特别是在人类风格的家庭任务中表现突出，这些任务通常指令简短但需要复杂的执行计划。

Conclusion: AHAT通过结合LLM的语义理解和符号推理的精确性，有效解决了大规模家庭环境中长时程任务规划的可扩展性问题，TGPO算法进一步提升了处理模糊复杂意图的能力。
...
### [252] [Scaling Verification Can Be More Effective than Scaling Policy Learning for Vision-Language-Action Alignment](https://arxiv.org/abs/2602.12281)
*Jacky Kwok,Xilun Zhang,Mengdi Xu,Yuejiang Liu,Azalia Mirhoseini,Chelsea Finn,Marco Pavone*

Main category: cs.RO

TL;DR: 本文提出CoVer框架，通过测试时验证来缩小VLA模型中的"意图-行动差距"，利用重述指令和生成动作的联合扩展提高样本多样性，在多个基准测试中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉-语言-行动模型在理解自然语言指令方面取得了显著进展，但生成的行动仍可能与给定指令存在偏差。本文旨在通过测试时验证来缩小这种"意图-行动差距"。

Method: 提出CoVer对比验证器架构，引入"启动时计算"和分层验证推理流程。在部署时，预计算多样化的重述指令，为每个指令重复生成行动候选，然后使用验证器选择最优的高层提示和低层行动块。

Result: 在SIMPLER基准测试中，验证方法相比策略预训练带来22%的分布内增益和13%的分布外增益，在真实世界实验中进一步改善45%。在PolaRiS基准测试中，CoVer实现14%的任务进展增益和9%的成功率提升。

Conclusion: 测试时验证是缩小VLA模型中意图-行动差距的有效方法，通过联合扩展重述指令和生成动作的数量，结合对比验证器架构，能够显著提高模型在指令跟随任务中的性能。
