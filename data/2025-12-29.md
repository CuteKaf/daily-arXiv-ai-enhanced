...
Motivation: 现有视频评估框架（如VideoScore-2）主要关注视觉和语义保真度，但未能捕捉具体视听属性如何驱动真实观众参与度。需要超越表面质量指标，实现人类对齐的多模态推理评估
...
Conclusion: 通过将评估基于多模态特征重要性和以人为中心的参与度信号，该方法推动了稳健且可解释的视频理解评估框架的发展
...
TL;DR: SDSNet是一个可扩展的深度子空间聚类框架，通过地标点近似、联合优化和因子化表示实现O(n)复杂度，显著降低了传统方法的O(n³)计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统子空间聚类方法面临可扩展性限制，因为构建完整n×n亲和度矩阵和进行谱分解需要O(n³)计算成本。即使深度学习方法改进了特征提取，但仍通过穷举的成对相似性计算保持这一计算瓶颈。

Method: SDSNet采用三种关键技术：(1)基于地标点的近似方法，避免构建完整亲和度矩阵；(2)联合优化自编码器重建与自表达目标；(3)直接在因子化表示上进行谱聚类。该框架将卷积自编码器与子空间保持约束相结合。

Result: 实验结果表明，SDSNet在保持与最先进方法相当的聚类质量的同时，显著提高了计算效率。

Conclusion: SDSNet通过创新的可扩展架构解决了子空间聚类的计算瓶颈问题，实现了线性复杂度，为大规模数据处理提供了实用的解决方案。
...
TL;DR: 提出CCAD方法，结合重建和表示学习优势，通过压缩全局特征作为重建条件，在异常检测中实现更好的性能和更快的收敛。


<details>
  <summary>Details</summary>
Motivation: 异常检测在工业中很重要，特别是异常数据有限时。现有方法存在局限：基于无监督表示的方法在领域偏移下难以提取鲁棒特征，而基于重建的方法训练效率低且因约束不足导致性能下降。

Method: 提出CCAD方法，将全局特征作为重建模型的新模态条件，并设计自适应压缩机制来提升泛化能力和训练效率。

Result: 大量实验表明CCAD在AUC指标上持续优于现有最优方法，同时实现更快收敛。还贡献了重新组织和标注的DAGM 2007数据集。

Conclusion: CCAD成功结合了两种异常检测范式的优势，通过压缩全局特征条件化重建，在性能和效率上都取得了显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏大规模公开的多标注者皮肤病变分割数据集，这对于研究多标注者医学图像分割问题至关重要。皮肤镜成像允许人类专家和AI系统观察常规临床照片无法辨别的形态结构，但现有数据集有限。

Method: 从ISIC档案中收集皮肤镜图像，构建包含17,684个分割掩码的数据集，涵盖14,967张图像。数据集包含标注者的技能水平和分割工具等元数据，并提供数据分析和共识分割掩码。

Result: 创建了ISIC MultiAnnot++数据集，包含14,967张皮肤镜图像和17,684个分割掩码，其中2,394张图像有2-5个分割标注，成为最大的公开皮肤病变分割数据集。数据集包含标注者元数据，支持标注者特定偏好建模和元数据分析研究。
...
TL;DR: GMFF是一个两阶段生成式多焦点图像融合框架，首阶段使用StackMFF V4进行确定性融合，第二阶段通过IFControlNet进行生成式修复，以重建缺失焦平面内容并消除边缘伪影。


<details>
  <summary>Details</summary>
Motivation: 现有多焦点图像融合算法通常假设场景中每个空间位置至少在一张输入图像中是清晰的，且在实际复杂场景中常因焦点估计不确定或硬选择操作而产生边缘伪影。需要解决这些限制。

Method: 提出GMFF两阶段框架：1) 确定性融合阶段使用StackMFF V4结合可用焦平面信息生成初始融合图像；2) 生成式修复阶段通过IFControlNet利用潜在扩散模型的生成能力重建缺失焦平面内容、恢复细节并消除边缘伪影。两阶段独立开发但级联工作。

Result: 大量实验表明GMFF实现了最先进的融合性能，在涉及复杂多焦点内容的场景中展现出显著的实际应用潜力。

Conclusion: GMFF通过结合确定性融合和生成式修复，有效解决了现有多焦点图像融合方法的局限性，在复杂场景中表现出色，具有实际应用价值。
...
TL;DR: 该论文提出了首个视频生成中的社会推理基准测试，基于发展心理学和社会心理学的经典范式，评估了当前文本到视频生成模型在社会行为理解方面的系统性缺陷。
...
Method: 研究者开发了一个完全无需训练的基于智能体的流程：1) 提炼每个实验的推理机制；2) 合成多样化的视频就绪场景；3) 通过基于线索的批判来确保概念中立性和难度控制；4) 使用高容量视觉语言模型(VLM)作为评判者，在五个可解释的社会推理维度上评估生成的视频。该基准将30个经典社会认知范式组织成七个核心维度。

Result: 对七个最先进的视频生成系统进行的大规模研究显示，现代模型在表面合理性方面表现出色，但在意图识别、信念推理、联合注意力和亲社会推理方面系统性地失败，存在显著的性能差距。

Conclusion: 该研究揭示了当前视频生成模型在社会推理能力方面的根本性局限，提出了首个系统性的评估框架，为未来开发具有社会智能的视频生成模型提供了重要的基准和方向。
...
<details>
  <summary>Details</summary>
Motivation: 胸部X光多模态分析通常需要微调大型视觉语言模型，计算成本高昂。本研究旨在探索参数高效训练策略，在保持性能的同时大幅降低计算需求。

Method: 使用Indiana University胸部X光数据集（3,851个图像-报告对），采用多种参数高效训练策略：冻结编码器、BitFit、LoRA和适配器。为避免数据泄漏，从文本输入中删除了病理学术语但保留临床上下文。在固定参数预算（2.37M参数，占总参数的2.51%）下进行比较，并在CheXpert数据集上进行外部验证。

Result: 所有参数高效训练变体在AUROC上达到0.892-0.908，优于全微调的0.770（使用94.3M可训练参数，减少40倍）。外部验证显示所有PET方法在<9%可训练参数下达到>0.69 AUROC，适配器表现最佳（0.7214 AUROC）。预算匹配比较显示纯视觉模型（0.653 AUROC）优于预算匹配的多模态模型（0.641 AUROC），表明改进主要来自参数分配而非跨模态协同。PET方法存在校准退化问题（ECE: 0.29-0.34）。

Conclusion: 冻结编码器策略在显著降低计算成本的同时提供优越的判别性能，但校准校正是临床部署的必要步骤。改进主要源于参数分配优化而非跨模态协同效应。
...
TL;DR: 该研究针对AI生成图像检测器在真实部署中的鲁棒性问题，提出固定阈值评估协议，揭示了不同架构在照片和艺术内容检测中的性能差异，为实际应用提供指导。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测方法在评估鲁棒性时存在误导性，通常针对每种后处理条件重新调整阈值，人为夸大鲁棒性估计，掩盖了实际部署中的性能下降问题。

Method: 引入固定阈值评估协议，在干净验证数据上选择一次决策阈值后，在所有后处理变换中保持固定；使用轻量级CNN-ViT混合架构，结合门控融合和可选频率增强；在三个操作点（低误报率、ROC最优、最佳F1）进行系统退化测试。

Result: 研究发现：频率增强CNN在原始照片上表现出色（93.33%），但在压缩下崩溃（61.49%）；ViT通过语义模式识别降解最小（92.86%到88.36%）；所有架构在艺术内容上的AUROC比照片高15%；混合方法在跨域性能上取得平衡。

Conclusion: 固定阈值评估消除了重新调整阈值带来的性能膨胀，揭示了真实的鲁棒性差距，为实际部署提供指导：干净照片验证首选CNN，压缩内容使用ViT，艺术/图形筛选采用混合方法。
...
Method: 构建MuS-Polar3D基准数据集，包含42个物体在7种定量控制散射条件和5个视角下的偏振图像，提供高精度3D模型、法线图和前景掩码。从成像链角度提出两阶段流水线：先散射抑制，后3D重建。

Result: 在复杂散射条件下使用多种基线方法进行广泛评估，证明了基准数据集的有效性，最佳平均角度误差达到15.49度。这是首个公开可用的定量浊度水下偏振3D成像基准数据集。

Conclusion: MuS-Polar3D是首个公开的定量浊度水下偏振3D成像基准数据集，支持在可控散射条件下进行准确重建和公平算法评估，推动了该领域的研究发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统GRPO在图像生成后期会产生同质化输出，缺乏创造力和视觉多样性，限制了应用场景。这源于两个问题：1）传统GRPO依赖单样本质量作为奖励信号，导致模型收敛于少数高奖励生成模式，忽视分布级多样性；2）传统正则化忽视了早期去噪阶段对保持多样性的主导作用，导致正则化预算错配。

Method: 提出双管齐下的方法：1）奖励层面：基于语义分组的分布级创意奖励，通过谱聚类构建同标题生成样本的分布级表示，根据组大小自适应分配探索性奖励以鼓励发现新的视觉模式；2）生成层面：结构感知正则化，在早期阶段施加更强的约束以保持多样性，同时不影响奖励优化效率。

Result: 实验表明，在匹配质量分数下，该方法实现了13%-18%的语义多样性提升，为基于GRPO的图像生成建立了新的质量-多样性帕累托前沿。

Conclusion: 通过从奖励建模和生成动力学两个角度重新审视多样性退化问题，提出的方法有效解决了GRPO图像生成中的同质化问题，在保持图像质量的同时显著提升了视觉多样性。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通过大规模图像文本预训练学习强大的多模态表示，但将其适配到层次分类任务中尚未充分探索。标准方法将标签视为扁平类别，需要完全微调，成本高昂且在不同分类层级产生不一致的预测
...
<details>
  <summary>Details</summary>
Motivation: 自注意力机制在视觉Transformer中至关重要，但其二次复杂度在高分辨率场景下带来沉重计算负担，限制了实际应用。现有方法通过引入手工设计的局部性或稀疏性模式来缓解此问题，但这不可避免地会损害模型能力。

Method: 提出循环注意力机制，发现视觉Transformer中的自注意力矩阵常近似于具有循环块的块循环矩阵(BCCB)。利用这种结构化矩阵特性，将注意力图显式建模为其最近的BCCB矩阵，并提出高效计算算法进行快速计算。该方法与标准自注意力非常相似，仅使用BCCB矩阵。

Result: 该方法实现了O(NlogN)计算复杂度，同时很大程度上保持了标准自注意力的能力。在多种视觉任务上的广泛实验证明了该方法的有效性。

Conclusion: 循环注意力机制作为一种有前景的替代方案，为视觉Transformer架构提供了高效的自注意力计算范式，在保持模型能力的同时显著降低计算复杂度。
...
TL;DR: EraseLoRA：一种无需数据集的物体移除框架，通过背景感知推理和测试时适应，避免传统注意力操作导致的目标重现问题，实现更精确的背景重建
...
Conclusion: EraseLoRA通过背景感知推理和测试时适应，避免了传统注意力操作的问题，实现了更可靠的物体移除和背景重建，代码将在发表后提供
...
TL;DR: 该论文提出了两个新的图像编辑任务：上下文感知的对象插入和赞助商产品标识增强，并构建了相应的数据集来支持这些任务。


<details>
  <summary>Details</summary>
Motivation: 当前智能图像编辑虽然依赖计算机视觉、多模态推理和生成模型，但现有方法很少确保插入的对象在上下文中是合适的。特别是在广告和数字媒体领域，需要更智能的上下文感知编辑能力。

Method: 引入了两个新任务：1) 上下文感知对象插入，需要预测合适的对象类别、生成对象并在场景中合理放置；2) 赞助商产品标识增强，涉及检测产品并插入正确的品牌标识，即使物品未标记或标记错误。为此构建了两个包含类别标注、放置区域和赞助商产品标签的新数据集。

Result: 论文提出了两个新的图像编辑任务框架，并创建了相应的数据集来支持这些任务的实现。这些任务旨在解决现有图像编辑方法在上下文适合作和品牌一致性方面的不足。

Conclusion: 通过引入上下文感知的对象插入和赞助商产品标识增强任务，以及构建相应的数据集，该研究为广告和数字媒体领域的智能图像编辑提供了新的方向，强调了上下文适合作和品牌一致性的重要性。
...
TL;DR: 该研究分析了AIGC图像检测领域存在的可复现性和泛化性问题，通过复现7篇关键论文和代表性方法，发现文献细节缺失和模型过拟合特定生成器是主要原因。


<details>
  <summary>Details</summary>
Motivation: AIGC图像检测技术虽然发展迅速，但在实际应用中面临两个核心问题：可复现性差和泛化能力不足。本研究旨在通过系统分析现有文献，找出这些问题的根本原因。

Method: 1. 回顾7篇AIGC检测关键论文；2. 构建轻量级测试数据集；3. 复现代表性检测方法；4. 分析复现过程中的技术细节缺失和模型过拟合问题。

Result: 实验表明：严格遵循原文核心流程可复现基本性能，但当预处理破坏关键特征或跨生成器测试时，检测性能急剧下降。这证实了细节缺失和过拟合特定生成器特征的问题。

Conclusion: 研究为改进AIGC检测技术的可复现性提供了实证依据，建议研究者更全面地披露实验细节，并验证所提方法的泛化能力，以推动该领域的实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（VLM）的小分支由于窗口大小限制，在语言-图像对齐方面表现不佳。研究发现知识蒸馏可以弥补RoPE在窗口大小方面的不足，因此需要开发专门针对长程注意力机制迁移的蒸馏方法

Method: 提出LAid方法，包含两个互补组件：1）渐进式距离加权注意力匹配，在训练中动态强调更长的位置差异；2）可学习RoPE响应增益调制，选择性放大需要的位置敏感性

Result: LAid蒸馏的模型相比基线小模型，有效上下文窗口最多扩大3.2倍，同时在标准VL基准测试中保持或提升性能。频谱分析表明LAid成功保留了传统方法无法迁移的关键低频注意力组件

Conclusion: LAid不仅为构建更高效的长上下文VLM提供了实用技术，还从理论上揭示了位置理解在蒸馏过程中如何产生和迁移
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶面临长尾数据稀缺问题，现有3D仿真方法难以同时实现逼真渲染和交互式交通编辑，特别是在大角度新视角合成和资产操作时存在几何或光照伪影

Method: 提出对称自回归在线恢复范式，构建成对对称视图通过真值引导的双视图公式恢复细节，使用自回归策略生成一致侧视图；利用训练免费协调机制，将车辆插入视为上下文感知修复以确保光照和阴影一致性

Result: 大量实验表明SymDrive在新视角增强和逼真3D车辆插入方面均达到最先进性能

Conclusion: SymDrive是一个统一的基于扩散的框架，能够实现联合高质量渲染和场景编辑，有效解决自动驾驶仿真中的渲染质量和编辑控制问题
...
TL;DR: 提出Pearl：一种基于监督学习的无LLM图像描述评估指标，适用于有参考和无参考设置，通过新颖的相似度表示学习机制，在多个数据集上超越现有无LLM指标。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型（LLM）的评估指标倾向于偏好自身生成的内容，存在中立性问题；而大多数无LLM指标虽然避免了这一问题，但性能表现不够理想。

Method: 提出Pearl监督学习指标，引入新颖机制学习图像-描述和描述-描述相似度的表示，并构建包含约33.3万个人类标注、来自2360名标注者、覆盖7.5万张图像的数据集。

Result: Pearl在Composite、Flickr8K-Expert、Flickr8K-CF、Nebula和FOIL数据集上，在有参考和无参考设置下均优于其他现有无LLM指标。

Conclusion: Pearl成功解决了LLM指标的中立性问题，同时提升了无LLM指标的性能，为图像描述评估提供了更可靠、更中立的解决方案。
...
TL;DR: GaussianEM是一种基于高斯伪原子框架的cryo-EM图像分析方法，能够同时建模复合物异质性和构象异质性，通过双编码器-单解码器架构将图像映射到高斯组件，直观描述构象变化。
...
Method: 提出GaussianEM，一种高斯伪原子框架，通过双编码器-单解码器架构将实验性cryo-EM图像映射到其个体高斯组件，通过高斯参数的变化表示结构变异性。

Result: 该方法提供了构象变化的直观可解释描述，保持了沿过渡轨迹的局部结构一致性，并自然弥合了基于密度的模型与相应原子模型之间的差距。在模拟和实验数据集上都证明了其有效性。

Conclusion: GaussianEM为分析包含连续运动和离散状态的cryo-EM数据集提供了有效的解决方案，能够同时处理复合物异质性和构象异质性，为蛋白质动力学研究提供了新的工具。
...
TL;DR: LCMP是首个长上下文MLLM个性化评估基准，用于评估MLLM在长对话中感知个性化概念变化并生成合适响应的能力。TAME作为基线框架，采用双记忆管理和RA2G范式，在LCMP上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM个性化方法主要关注简单的视觉识别和文本替换，缺乏对长上下文对话的支持。理想的个性化MLLM助手应该能够进行长上下文对话，并通过学习历史对话不断提升体验质量。

Method: 提出了LCMP评估基准，并引入TAME框架。TAME采用无训练、状态感知的双记忆管理机制，区分处理个性化概念的时序变化和持久变化。同时提出RA2G范式，通过检索-对齐增强生成，从多记忆知识中提取与当前问题匹配的信息。

Result: 在LCMP基准上的实验表明，TAME取得了最佳性能，在长上下文场景中展现出卓越且不断演进的交互体验。

Conclusion: LCMP填补了长上下文MLLM个性化评估的空白，TAME框架通过双记忆管理和RA2G范式有效提升了MLLM在长对话中的个性化能力，为复杂现实用户查询提供了更好的交互体验。
...
TL;DR: 本文提出了一种基于因果推理的少样本细粒度视觉分类方法CausalFSFG，通过样本级和特征级干预消除少样本条件带来的偏差分布和虚假相关性，在多个基准数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有少样本细粒度视觉分类方法通常通过增强判别性局部特征来提高分类精度，但忽视了少样本支持集作为混淆变量会引入偏差数据分布并误导判别特征提取的问题。这导致模型性能受到虚假相关性的影响。

Method: 提出CausalFSFG方法，基于结构因果模型分析问题，包含两个关键组件：1) 干预式多尺度编码器(IMSE)进行样本级干预；2) 干预式掩码特征重建(IMFR)进行特征级干预。两者共同揭示从输入到子类别的真实因果关系。

Result: 在CUB-200-2011、Stanford Dogs和Stanford Cars等广泛使用的公共数据集上进行了大量实验和深入分析，证明CausalFSFG方法取得了新的最先进性能。

Conclusion: 通过因果推理方法处理少样本细粒度视觉分类中的偏差数据分布问题，提出的CausalFSFG方法有效消除了虚假相关性，显著提升了分类性能，为少样本细粒度识别提供了新的解决方案。
...
TL;DR: TrackTeller是一个用于动态3D驾驶场景中基于时间语言的目标定位框架，通过多帧观测和语言条件解码来识别被描述的对象。


<details>
  <summary>Details</summary>
Motivation: 在动态3D驾驶场景中，许多指代表达式通过最近的运动或短期交互来描述目标，这些无法仅从静态外观或几何信息中解决，因此需要研究时间语言基础的3D定位问题。

Method: TrackTeller是一个时间多模态定位框架，集成了LiDAR-图像融合、语言条件解码和时间推理。它构建了与文本语义对齐的共享UniScene表示，生成语言感知的3D提议，并使用运动历史和短期动态来细化定位决策。

Result: 在NuPrompt基准测试中，TrackTeller持续提升了语言基础跟踪性能，在多目标跟踪准确率上实现了70%的相对改进，并将误报频率降低了3.15-3.4倍。
...
TL;DR: 论文提出在PPE框架中引入L1正则化约束，通过稀疏化潜在空间操作来减少属性编辑中的语义泄漏问题，实现更聚焦和可控的图像编辑。


<details>
  <summary>Details</summary>
Motivation: 文本驱动的图像编辑经常面临属性纠缠问题，修改目标属性（如添加刘海）会无意中改变其他语义属性（如身份或外观）。原始PPE框架的正则化策略存在局限性，潜在更新仍然密集且容易发生语义泄漏。

Method: 在PPE框架基础上引入稀疏性约束，使用L1正则化对潜在空间操作进行约束。分析框架的架构组件，包括基于BERT的属性预测和基于StyleGAN2的图像生成，在CelebA-HQ数据集上进行实验。

Result: 实验结果表明，提出的方法能够实现更聚焦和可控的编辑，有效减少非目标属性的意外变化，同时保持面部身份特征。

Conclusion: 通过引入L1正则化的稀疏性约束，改进了PPE框架，解决了属性编辑中的语义泄漏问题，实现了更解耦的图像编辑效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有天气建模方法将准确预测和机制解释分离，分别处理生成和理解任务，缺乏统一框架。需要同时实现精确预测和可解释的天气机制分析。

Method: 1. 采用雷达编码器处理天气生成任务；2. 使用共享自注意力机制进行统一处理；3. 构建因果推理链数据集，支持可解释输出和感知质量提升。

Result: 在天气生成和理解任务上都达到了最先进的性能表现，实验表明生成和理解任务在天气领域可以相互促进，验证了统一框架的可行性和价值。

Conclusion: Omni-Weather首次证明了天气生成与理解可以在单一架构中统一实现，生成和理解任务相互增强，为天气建模提供了新的范式。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆需要高效、安全、可靠的感知系统来检测和识别道路上的车辆、行人和交通标志。随着机器学习和深度学习算法的快速发展，需要评估新兴模型在实际应用场景中的性能表现。

Method: 使用自定义数据集对YOLO-NAS和YOLOv8两种深度学习模型进行对比实验，评估它们在检测任务中的性能。通过收集真实世界数据并训练模型，比较训练时间和检测准确率等关键指标。

Result: YOLOv8s模型相比YOLO-NAS模型节省了75%的训练时间。在目标检测准确率方面，YOLOv8s达到83%，优于YOLO-NAS的81%。

Conclusion: YOLOv8s在自动驾驶感知任务中表现出更好的性能，既节省训练时间又提高检测准确率。这项比较研究为相关研究社区提供了关于新兴深度学习模型在实际应用场景中性能的重要参考。
...
TL;DR: 该研究提出了一个孟加拉语手写文本生成方法，使用自收集的多样化数据集，能够从纯文本输入生成多样化的手写输出。


<details>
  <summary>Details</summary>
Motivation: 手写文本生成是一个新兴领域，但孟加拉语作为世界第五大语言，其手写文本生成研究相对缺乏。由于个体手写风格差异大，需要大规模多样化数据集，但这类数据集难以收集且不易获得。

Method: 开发并使用自收集的孟加拉语手写样本数据集，包含约500名不同年龄和性别个体的贡献。所有图像经过预处理以确保一致性和质量，然后提出孟加拉语手写单词生成方法。

Result: 该方法能够从输入的纯文本生成多样化的手写输出，展示了生成现实孟加拉语手写文本的能力。
...
TL;DR: UniPercept-Bench是一个用于评估多模态大语言模型在感知级图像理解能力的统一框架，涵盖美学、质量、结构和纹理四个关键领域，通过大规模数据集和强基线模型提升MLLMs的感知能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉理解任务（如视觉定位、分割和描述）方面取得了显著进展，但它们在感知级图像特征理解方面的能力仍然有限。需要建立一个系统性的框架来评估和提升MLLMs在感知级图像理解方面的能力。

Method: 提出了UniPercept-Bench统一框架，建立了层次化的定义系统，构建了大规模数据集来评估感知级图像理解。开发了强基线模型UniPercept，通过领域自适应预训练和任务对齐强化学习进行训练，支持视觉评分和视觉问答任务。

Result: UniPercept在感知级图像理解方面优于现有的多模态大语言模型，可以作为即插即用的奖励模型用于文本到图像生成，为推进感知级多模态图像理解提供了坚实基础。

Conclusion: 这项工作在MLLMs时代定义了感知级图像理解，通过引入全面的基准测试和强基线模型，为推进感知级多模态图像理解提供了坚实基础，并可作为文本到图像生成的奖励模型。
...
TL;DR: SlideChain是一个基于区块链的多模态语义提取可验证性框架，针对教育内容中的视觉-语言模型输出提供完整性保障
...
Method: 提出SlideChain框架，使用SlideChain Slides Dataset（包含1,117个医学影像讲座幻灯片），从四个最先进的VLMs中提取概念和关系三元组，构建结构化溯源记录，并将这些记录的加密哈希锚定在本地EVM兼容区块链上

Result: 通过首次系统分析多模态教育内容中的语义分歧、跨模型相似性和讲座级变异性，揭示了显著的跨模型差异，包括许多幻灯片上的低概念重叠和接近零的关系三元组一致性。同时评估了模拟部署条件下的gas使用、吞吐量和可扩展性，并展示了完美的篡改检测和跨独立提取运行的确定性可复现性

Conclusion: SlideChain为可信、可验证的多模态教育流程提供了实用且可扩展的步骤，支持AI辅助教学系统的长期可审计性、可复现性和完整性
...
<details>
  <summary>Details</summary>
Motivation: VGGT模型虽然在3D重建中表现优异，但其全局自注意力层在处理超过几百帧的长序列时会出现严重的崩溃现象：注意力矩阵迅速变为近似秩一，token几何退化到几乎一维子空间，重建误差超线性累积。需要从数学上解释这一现象。

Method: 将全局注意力迭代视为退化扩散过程，证明token特征流以O(1/L)的速率收敛到Dirac型测度，得到闭式平均场偏微分方程，该方程能精确预测实验观察到的秩分布。

Result: 理论定量匹配了注意力热图演化及相关工作中的一系列实验结果，解释了token合并策略（定期移除冗余token）通过降低有效扩散系数来延迟崩溃的机制，无需额外训练。
...
TL;DR: ShinyNeRF：一种能够处理各向同性和各向异性反射的新型NeRF框架，特别针对金属拉丝等各向异性高光表面，通过ASG分布和vMF分布混合学习实现高质量3D数字化。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF方法在处理各向异性高光表面（如金属拉丝）时存在困难，无法准确建模这类表面的反射特性，限制了文化遗产数字化中真实感3D重建的质量。

Method: 提出ShinyNeRF框架，通过联合估计表面法线、切线、高光集中度和各向异性幅度，将出射辐射度近似为各向同性von Mises-Fisher分布的编码混合，使用各向异性球面高斯分布建模反射特性。

Result: 实验结果表明，ShinyNeRF在各向异性高光反射数字化方面达到最先进性能，相比现有方法能提供更合理的物理解释和材质属性编辑能力。

Conclusion: ShinyNeRF成功解决了NeRF在处理各向异性高光表面时的局限性，为文化遗产数字化提供了更高质量的3D表示，同时具备更好的可解释性和编辑性。
...
TL;DR: InstructMoLE：一种基于指令引导的低秩专家混合框架，通过全局路由信号解决传统MoLE在扩散Transformer多条件微调中的任务干扰问题，显著提升图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法（如LoRA）在多条件任务中存在任务干扰问题，而MoLE架构虽然提供模块化解决方案，但其基于token级别的局部路由策略与用户指令的全局性质冲突，导致生成图像出现空间碎片化和语义漂移等伪影。

Method: 提出InstructMoLE框架，核心创新包括：1）指令引导路由（IGR），从用户完整指令中提取全局路由信号，统一应用于所有输入token，确保选择一致的专家组合；2）输出空间正交性损失，促进专家功能多样性，防止表示崩溃。

Result: 在具有挑战性的多条件生成基准测试中，InstructMoLE显著优于现有的LoRA适配器和MoLE变体，实现了更好的组合控制和用户意图保真度。

Conclusion: InstructMoLE为生成模型的指令驱动微调提供了一个鲁棒且可泛化的框架，通过全局路由机制有效解决了多条件任务中的语义一致性问题，为复杂图像生成任务提供了优越的控制能力。
...
Result: 在GenImage、WildFake、DiTFake、GPT-ImgEval和Chameleon数据集上评估：1）FUSE Stage 1在Chameleon基准测试中达到SOTA；2）GenImage数据集平均准确率91.36%；3）所有测试生成器平均准确率88.71%；4）平均精度均值94.96%；5）Stage 2训练进一步提升了大多数生成器的性能。

Conclusion: 结合频谱和语义特征的方法在AI生成图像的通用检测方面具有显著优势，能够在多样化的生成器上保持鲁棒性，特别是在处理高质量图像时优于现有方法。
...
TL;DR: ST-MoE：一种基于专家混合的时空解耦模型，通过双向时空Mamba专家灵活捕捉多人运动中的复杂时空依赖，显著降低计算成本并提升预测精度。
...
Method: 提出时空解耦专家混合模型（ST-MoE），包含四种不同类型的时空专家，专门捕捉不同的空间或时间依赖。为降低计算开销，引入双向时空Mamba作为专家，通过共享双向时间和空间Mamba的不同组合实现模型效率和参数经济性。
...
TL;DR: RAPTOR是一种实时高分辨率视频预测架构，通过创新的Efficient Video Attention模块和3阶段训练课程，在保持高质量的同时实现实时性能，解决了视频预测中的分辨率-质量-速度三难问题。


<details>
  <summary>Details</summary>
Motivation: 视频预测面临分辨率、感知质量和实时速度之间的三难困境，特别是在自主无人机等延迟关键应用中。现有方法（扩散模型、自回归模型或二次复杂度注意力）无法在边缘硬件上满足这些严格需求。

Method: 提出RAPTOR架构，采用单次前向设计避免迭代方法的误差累积和延迟。核心创新是Efficient Video Attention（EVA）模块，通过沿空间和时间轴交替操作，将时间复杂度降至O(S+T)，内存复杂度降至O(max(S,T))。配合3阶段训练课程，从粗结构逐步细化到清晰的时间相干细节。
...
TL;DR: AstraNav-World是一个端到端世界模型，通过统一的概率框架联合推理未来视觉状态和动作序列，将扩散视频生成器与视觉语言策略集成，实现视觉预测与动作规划的同步更新，在开放动态环境中提升具身导航性能。


<details>
  <summary>Details</summary>
Motivation: 在开放动态环境中进行具身导航需要准确预测世界如何演化以及动作如何随时间展开。传统"先预测后规划"的解耦方法容易产生累积误差，需要更紧密的视觉-动作耦合来保持决策基于物理一致、任务相关的未来状态。

Method: 提出AstraNav-World端到端世界模型，将扩散视频生成器与视觉语言策略集成在统一概率框架中。采用双向约束训练：生成动作条件多步视觉预测，以及基于预测视觉推导轨迹。这种同步更新机制使视觉预测可执行，决策基于物理一致未来。

Result: 在多样化具身导航基准测试中显示出改进的轨迹准确性和更高的成功率。消融实验证实紧密视觉-动作耦合和统一训练的必要性。在真实世界测试中展示了卓越的零样本能力，无需真实世界微调即可适应未见场景。

Conclusion: 通过将前瞻视觉和控制统一在单一生成模型中，AstraNav-World捕捉了可迁移的空间理解和规划相关导航动态，而非仅仅过拟合仿真特定数据分布，向可靠、可解释、通用的具身智能体迈进一步。
...
<details>
  <summary>Details</summary>
Motivation: 实时肖像动画在虚拟助手和实时化身等交互应用中至关重要，需要高视觉保真度、时间一致性、超低延迟和响应式控制。现有扩散模型质量高但非因果性阻碍流式部署，因果自回归视频生成方法存在误差累积、块边界运动不连续和长期一致性退化问题。
...
Result: Knot Forcing能够在消费级GPU上实现高保真度、时间一致性和交互式肖像动画，支持无限序列的实时生成，具有强大的视觉稳定性。

Conclusion: 提出的Knot Forcing框架通过创新的分块生成、时间节点和超前运行机制，成功解决了实时肖像动画中的误差累积、运动不连续和长期一致性等关键挑战，实现了高质量的流式生成性能。
...
TL;DR: SyncAnyone是一个两阶段学习框架，用于高质量AI视频配音，通过扩散视频变换器实现精准唇部同步，同时保持面部结构和背景一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于掩码训练的视频配音方法虽然能实现唇部同步，但破坏了时空上下文，导致动态面部运动表现不佳、面部结构不稳定和背景一致性差的问题。

Method: 提出两阶段框架：第一阶段训练基于扩散的视频变换器进行掩码唇部修复，生成音频驱动的精准唇部运动；第二阶段开发无掩码微调管道，通过合成伪配对训练样本来解决掩码引起的伪影问题。

Result: 实验表明该方法在视觉质量、时间连贯性和身份保持方面达到最先进水平，特别是在野外唇部同步场景中表现优异。

Conclusion: SyncAnyone通过创新的两阶段学习框架，同时实现了精确的运动建模和高视觉保真度，解决了现有掩码训练方法在时空上下文破坏方面的局限性。
...
TL;DR: BertsWin是一种结合BERT风格完整标记掩码和Swin Transformer窗口的混合架构，用于3D医学影像的自监督学习，相比标准ViT-MAE基线加速5.8倍语义收敛，训练轮次减少15倍
...
Method: 提出BertsWin混合架构：结合BERT风格完整标记掩码和Swin Transformer窗口，保持完整3D标记网格（包括掩码和可见标记）以保留空间拓扑；使用单层局部Swin窗口降低ViT二次复杂度；引入结构优先损失函数；评估颞下颌关节锥形束CT数据

Result: BertsWin架构相比标准ViT-MAE基线语义收敛加速5.8倍；结合GradientConductor优化器，训练轮次从660减少到44（15倍减少）；在保持理论FLOP与稀疏ViT基线相当的同时，实现显著计算资源节省

Conclusion: BertsWin通过保持完整三维空间拓扑结构，在3D医学影像自监督学习中实现了显著加速和计算效率提升，为密集体数据处理提供了高效解决方案
...
Method: 提出Scene-VLM框架，联合处理视觉和文本线索（帧、转录文本、可选元数据），采用因果依赖的序列预测，引入上下文聚焦窗口机制，从token级logits提取置信度分数，并通过最小监督生成自然语言推理

Result: 在标准场景分割基准上达到最先进性能，在MovieNet上相比先前领先方法提升+6 AP和+13.7 F1分数

Conclusion: Scene-VLM通过多模态推理、序列处理和可解释性机制，显著提升了视频场景分割的性能和实用性
...
TL;DR: 提出了一种新的脑MRI病变检测方法，通过解耦成像信息和解剖图像，结合边缘到图像恢复模块，显著提升了多模态多中心MRI的异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 当前无监督学习方法在脑MRI病变检测中存在两个主要限制：1）对多模态多中心MRI的泛化能力受限，因为依赖正常训练数据中的特定成像信息；2）性能受限，因为异常残差会从输入图像传播到重建的伪健康图像。
...
Result: 在9个公共数据集（来自多个中心的4,443名患者的MRI）上评估，该方法优于17种最先进方法，在AP和DSC指标上分别实现了+18.32%和+13.64%的绝对提升。

Conclusion: 该方法通过解耦成像信息和解剖图像，结合边缘恢复策略，有效解决了现有无监督脑MRI病变检测方法的泛化性和性能限制问题，在多模态多中心MRI数据上表现出卓越性能。
...
<details>
  <summary>Details</summary>
Motivation: 在高风险领域中，小型任务特定视觉模型虽然计算需求低且可解释性强，但其解释常显示模型与人类领域知识不一致，依赖虚假相关性，导致实际部署时行为脆弱
...
Conclusion: LVLM-VA方法为小型任务特定视觉模型与人类领域知识对齐提供了一种新颖高效的解决方案，能够提升模型在实际部署中的鲁棒性和可靠性
...
TL;DR: 本文介绍了mAIcetoma挑战赛，旨在通过AI技术解决足菌肿（一种被忽视的热带病）的诊断难题，特别是针对组织病理学图像中菌粒的分割和疾病类型分类。


<details>
  <summary>Details</summary>
Motivation: 足菌肿是一种被忽视的热带病，主要影响贫困农村社区，其诊断在资源匮乏地区尤为困难，因为缺乏专业病理学家。AI技术有望解决这一诊断挑战。

Method: 组织mAIcetoma挑战赛，邀请全球团队开发深度学习模型，使用标准化的足菌肿数据库（MyData）进行菌粒分割和疾病类型分类，并采用评估指标对模型性能进行评价。

Result: 挑战赛吸引了多个团队参与，最终五个团队完成了目标。所有模型都实现了高精度的菌粒分割，强调了菌粒检测在足菌肿诊断中的重要性。表现最佳的模型在疾病类型分类方面也显示出显著性能。

Conclusion: mAIcetoma挑战赛成功推动了AI在足菌肿诊断中的应用，展示了深度学习模型在菌粒分割和疾病分类方面的潜力，为资源匮乏地区的足菌肿诊断提供了有前景的技术解决方案。
...
TL;DR: LongFly是一个用于无人机视觉语言导航的时空上下文建模框架，通过历史感知的时空建模策略处理长视距导航中的复杂环境，提升语义对齐和路径规划准确性。


<details>
  <summary>Details</summary>
Motivation: 无人机在灾后搜救等任务中面临高信息密度、视角快速变化和动态结构等挑战，特别是在长视距导航中。现有的无人机视觉语言导航方法难以在复杂环境中建模长视距时空上下文，导致语义对齐不准确和路径规划不稳定。

Method: 提出LongFly框架，包含三个核心模块：1）基于槽的历史图像压缩模块，将多视角历史观测动态压缩为固定长度的上下文表示；2）时空轨迹编码模块，捕捉无人机轨迹的时间动态和空间结构；3）提示引导的多模态集成模块，将现有时空上下文与当前观测集成，支持基于时间的推理和稳健的航点预测。

Result: 实验结果表明，LongFly在成功率上优于最先进的无人机视觉语言导航基线7.89%，在路径长度加权成功率上优于6.33%，在已见和未见环境中均表现一致。

Conclusion: LongFly通过有效的时空上下文建模策略，显著提升了无人机在复杂长视距导航任务中的性能，为无人机视觉语言导航提供了更稳健的解决方案。
...
TL;DR: 该研究探讨了在已知退化模型下，使用扩散后验采样（DPS）进行单图像超分辨率（SISR）。通过结合无条件扩散先验和基于梯度的条件约束，实现了4倍超分辨率重建，并在不同引导尺度和噪声水平下评估了后验采样的性能。
...
Method: 采用似然引导的采样过程，结合无条件扩散先验和基于梯度的条件约束来强制测量一致性。通过后验采样（PS）条件化在不同引导尺度和噪声水平下进行评估，使用PSNR和SSIM作为保真度指标，并采用综合选择分数(PSNR/40)+SSIM进行优化。

Result: 实验表明，适度的引导可以改善重建质量，最佳配置在PS尺度0.95和噪声标准差σ=0.01时获得（分数1.45231）。定性结果显示，所选PS设置相比下采样输入能恢复更清晰的边缘和更连贯的面部细节，而其他条件化策略（如MCG和PS-annealed）表现出不同的纹理保真度权衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于熵的攻击方法假设所有token对生成不稳定性贡献相同，但实际上只有少数高熵token（约20%）主导输出轨迹。通过针对这些关键决策点进行攻击，可以用更小预算实现更有效的安全威胁

Method: 提出Entropy-bank Guided Adversarial attacks (EGA)方法，识别自回归生成过程中的高熵关键决策点，集中对抗扰动在这些位置，而不是均匀攻击所有解码步骤

Result: 在多个代表性VLM上，选择性攻击将35-49%的良性输出转换为有害输出；攻击具有可迁移性（17-26%有害率）；EGA方法达到93-95%的攻击成功率，同时实现高有害转换率

Conclusion: 自回归生成中的高熵关键决策点对VLM安全性至关重要，针对这些点的集中攻击能有效暴露当前VLM安全机制的新弱点，为VLM安全防御提供了重要见解
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶V2X场景中，遮挡、视角受限和通信延迟等问题严重影响3D时空理解的可靠性，需要多视角协作感知和多模态融合来提升感知系统的鲁棒性。
...
Conclusion: XET-V2X通过统一的多视角多模态融合框架，在V2X协作场景中实现了有效的跨模态交互和时空一致性，为自动驾驶在复杂环境下的可靠感知提供了解决方案。
...
TL;DR: ADT-Tree通过自适应调整草稿树的深度和宽度，解决了自回归图像模型中推测解码因图像区域预测难度不同而导致的接受率不一致问题，实现了3倍以上的加速效果。


<details>
  <summary>Details</summary>
Motivation: 自回归图像模型虽然能达到扩散模型的质量水平，但需要约2000步的序列推理，推理速度慢。现有的推测解码技术（草稿树）在语言模型上有效，但在视觉自回归模型上表现不佳，主要原因是图像不同区域的预测难度差异导致草稿树接受率不一致。

Method: 提出邻接自适应动态草稿树（ADT-Tree），通过利用相邻标记状态和先前的接受率，动态调整草稿树的深度和宽度。方法包括：1）通过水平邻接初始化；2）通过二分法自适应细化深度/宽度；3）在简单区域生成更深树，在复杂区域生成更宽树。

Result: 在MS-COCO 2017和PartiPrompts数据集上的实验表明，ADT-Tree分别实现了3.13倍和3.05倍的加速。此外，该方法能与LANTERN等松弛采样方法无缝集成，实现进一步加速。

Conclusion: ADT-Tree有效解决了视觉自回归模型中推测解码的瓶颈，通过自适应调整草稿树结构显著加速推理，同时保持图像生成质量，为高效视觉生成提供了新思路。
...
TL;DR: DIOR是一种无需训练的方法，利用大型视觉语言模型生成条件图像嵌入，通过提示模型用单个词描述图像来提取特征，在条件图像相似性任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 条件图像嵌入需要关注文本条件指定的特定图像方面（如颜色、风格），现有视觉基础模型（如CLIP）虽然提供丰富的图像表示，但并非专门设计用于聚焦特定条件。
...
TL;DR: EasyOmnimatte：首个统一的端到端视频omnimatte方法，通过双专家策略（效果专家和质量专家）在预训练视频修复扩散模型上微调，实现高质量前景层分解及相关效果提取，显著提升质量和效率。


<details>
  <summary>Details</summary>
Motivation: 现有视频omnimatte方法通常依赖缓慢的多阶段或推理时优化流程，未能充分利用强大的生成先验，导致分解效果不理想。研究发现，如果视频修复模型能够微调以去除前景相关效果，那么它必然能够感知这些效果，从而也能用于前景层分解任务。

Method: 提出EasyOmnimatte方法：1）在预训练视频修复扩散模型上微调双互补专家，保持原始权重不变；2）效果专家：仅在效果敏感的DiT块上应用LoRA，捕捉前景和相关效果的粗粒度结构；3）质量专家：完全LoRA微调，细化alpha遮罩；4）采样时，早期高噪声步骤使用效果专家，后期低噪声步骤使用质量专家，避免两次完整扩散过程。

Result: 实验表明EasyOmnimatte在视频omnimatte任务上达到新的最先进水平，支持多种下游任务，在质量和效率方面显著优于基线方法。消融研究验证了双专家策略的有效性。

Conclusion: EasyOmnimatte是首个统一的端到端视频omnimatte方法，通过创新的双专家策略解决了现有方法在效果捕捉和计算效率方面的限制，为视频前景层分解及相关效果提取提供了高效高质量的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统解码器自回归图像生成使用固定长度标记化方案，标记数量随分辨率呈二次增长，导致注意力的计算和内存需求大幅增加。需要一种更高效的图像生成方法。

Method: DPAR提出动态聚合图像标记为可变数量补丁的方法，利用轻量级无监督自回归模型的下一标记预测熵作为可靠标准，根据信息内容合并标记为更大补丁。该方法对标准解码器架构进行最小修改，确保与多模态生成框架兼容，并将更多计算资源分配给高信息图像区域。

Result: 在Imagenet 256和384生成分辨率上，DPAR分别减少标记数量1.81倍和2.06倍，训练成本FLOPs减少高达40%。方法表现出更快的收敛速度，FID相对基线模型提升高达27.1%。

Conclusion: DPAR通过动态补丁聚合机制有效解决了自回归图像生成中的计算效率问题，利用信息熵指导的标记合并策略在保持生成质量的同时显著降低计算成本，为高效图像生成提供了新思路。
...
TL;DR: AutoPP是一个自动化产品海报生成与优化系统，通过生成器和优化器两个模块，结合百万级数据集，实现无需人工干预的海报创作和点击率优化。


<details>
  <summary>Details</summary>
Motivation: 传统产品海报制作需要人工设计并基于在线表现进行手动优化，这一过程既耗时又耗费资源。为了解决这个问题，需要开发一个自动化系统来生成和优化产品海报。

Method: AutoPP包含生成器和优化器两个核心模块。生成器使用统一设计模块整合海报的三个关键元素（背景、文本和布局），然后通过元素渲染模块将这些元素编码为条件标记来生成海报。优化器则利用在线反馈提升点击率，通过系统性地替换元素收集细粒度CTR比较数据，并采用孤立直接偏好优化（IDPO）将CTR提升归因于孤立元素。

Result: AutoPP在离线和在线环境中都取得了最先进的结果。该系统基于AutoPP1M数据集，这是专门为产品海报生成和优化设计的最大数据集，包含100万张高质量海报和超过100万用户的反馈数据。

Conclusion: AutoPP提供了一个完全自动化的产品海报生成和优化解决方案，通过结合生成和优化模块以及大规模数据集，能够高效地创建吸引人的产品海报并持续提升其在线表现。
...
TL;DR: SLIM-Brain是一种新型的无图谱fMRI基础模型，通过两阶段自适应设计解决了数据效率和训练效率的双重瓶颈问题，在保持空间细节的同时显著降低了计算和内存需求。
...
Method: SLIM-Brain采用两阶段自适应设计：1）轻量级时间提取器捕获全局上下文并对数据窗口进行重要性排序；2）4D分层编码器（Hiera-JEPA）仅从选定的前k个窗口中学习细粒度体素级表示，同时删除约70%的掩码补丁。

Result: 在七个公共基准测试上的广泛实验表明，SLIM-Brain在多样化任务上建立了新的最先进性能，同时仅需4千个预训练会话和传统体素级方法约30%的GPU内存。

Conclusion: SLIM-Brain成功解决了fMRI基础模型的数据和训练效率瓶颈，通过创新的自适应设计实现了在保持空间细节的同时显著降低计算需求，为fMRI分析提供了高效且强大的基础模型解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有低光照增强研究主要针对自然图像和视频，而动漫场景图像的低光照质量退化问题尚未充分探索。为解决这一领域差距，需要处理数据稀缺和光照条件多样性带来的挑战。

Method: 首先构建非配对动漫场景数据集；提出数据相对论不确定性（DRU）框架，受相对论GAN启发，类比光的波粒二象性定义和量化光照不确定性，利用不确定性信息动态调整目标函数来重新校准模型学习。

Result: 通过训练多个版本的EnlightenGANs进行广泛实验，DRU框架在感知和美学质量上超越了现有最先进方法，这些方法无法从数据不确定性角度进行学习。

Conclusion: DRU框架为动漫场景图像低光照增强提供了有效解决方案，并可能为视觉和语言领域的数据中心学习开辟新范式。
...
Method: 采用激活-定位-编辑对抗性范式：1)构建包含基于视觉特征的正样本和反映先验偏差的负样本的激活数据集；2)分析响应对的隐藏状态差异识别幻觉易发参数簇；3)使用注入对抗性调优前缀的提示微调这些参数簇，迫使模型优先考虑视觉证据而非参数偏差。

Result: 在生成性和判别性VLM任务上的评估表明，ALEAHallu在缓解幻觉方面具有显著有效性。

Conclusion: 提出的对抗性参数编辑框架为缓解视觉语言模型幻觉问题提供了一种有效的可训练解决方案，通过强制模型依赖视觉证据而非语言先验来改善输出质量。
...
TL;DR: CrownGen是一个基于去噪扩散模型的生成框架，可自动化患者定制牙冠设计，显著减少设计时间并保持临床质量
...
TL;DR: 提出自动化方法寻找紧凑光谱指数用于植被分类，通过构建归一化差值的多项式组合，在保持光照不变性的同时实现特征选择，在Sentinel-2数据上验证效果良好。


<details>
  <summary>Details</summary>
Motivation: 传统植被分类需要人工设计光谱指数，过程繁琐且可能不是最优。需要一种自动化方法来发现紧凑、可解释且具有光照不变性的光谱指数。

Method: 1. 从所有光谱波段中生成所有成对归一化差值作为基础特征；2. 构建这些基础特征的多项式组合（最高到指定阶数）；3. 使用特征选择方法（ANOVA过滤、递归消除、L1正则化SVM）选择小型指数集；4. 在Sentinel-2数据上进行验证。

Result: 在加拿大萨斯喀彻温省的Kochia检测任务中：单个二阶指数（红边波段两个归一化差值的乘积）达到96.26%准确率；使用8个指数仅提高到97.70%。所选特征均为b4-b8波段的二阶乘积，表明判别信号来自光谱交互作用而非单个波段比值。

Conclusion: 该方法能够自动发现紧凑、可解释的光谱指数，保持光照不变性且易于部署（如Google Earth Engine）。开源实现ndindex可用于其他传感器和分类任务。
...
### [57] [Patch-Discontinuity Mining for Generalized Deepfake Detection](https://arxiv.org/abs/2512.22027)
...
Method: GenDF框架包含四个核心组件：1）将强大的大规模视觉模型迁移到深度伪造检测任务；2）深度伪造特定表示学习，捕捉真实和伪造面部图像之间的判别模式；3）特征空间重分布，缓解分布不匹配问题；4）分类不变特征增强策略，在不引入额外可训练参数的情况下增强泛化能力。

Result: 广泛的实验表明，GenDF在跨域和跨操作设置中实现了最先进的泛化性能，同时仅需要0.28M可训练参数，验证了所提出框架的有效性和效率。

Conclusion: GenDF提供了一个简单而有效的深度伪造检测框架，通过创新的表示学习、特征重分布和增强策略，在保持参数效率的同时显著提升了跨域泛化能力，为解决深度伪造检测中的泛化挑战提供了有前景的解决方案。
...
TL;DR: 针对提示驱动视频分割基础模型（VSFMs）的后门攻击框架BadVSFM，通过两阶段训练实现高攻击成功率，同时保持正常分割质量，揭示了当前VSFMs的安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着SAM2等提示驱动视频分割基础模型在自动驾驶、数字病理学等关键领域的应用增加，后门安全威胁日益受到关注。研究发现传统后门攻击方法对VSFMs几乎无效（攻击成功率低于5%），需要专门针对VSFM特性的攻击框架。

Method: 提出BadVSFM两阶段攻击框架：1）引导图像编码器，使触发帧映射到指定目标嵌入，而干净帧保持与参考编码器对齐；2）训练掩码解码器，使不同提示类型的触发帧-提示对产生共享目标掩码，同时干净输出接近参考解码器。

Result: 在两个数据集和五个VSFM模型上的实验表明，BadVSFM在多种触发器和提示类型下实现了强大且可控的后门效果，同时保持了干净分割质量。攻击成功率显著提升，而四种代表性防御方法基本无效。

Conclusion: BadVSFM揭示了当前提示驱动视频分割基础模型中未充分探索的安全漏洞，其两阶段设计能有效分离触发和干净表示，将注意力转移到触发区域，为VSFM安全研究提供了重要参考。
...
TL;DR: 提出了一种名为\method的新框架，能够从单张图像或文本提示生成逼真、交互式和连续的世界，支持键盘探索，解决了现有扩散模型在参数规模、推理步骤和历史上下文方面的限制。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型生成交互式可探索世界的方法存在参数规模过大、依赖冗长推理步骤、历史上下文快速增长等关键挑战，严重限制了实时性能且缺乏文本控制生成能力。

Method: 框架包含三个核心组件：1）集成统一上下文压缩与线性注意力的长视频生成框架；2）基于双向注意力蒸馏和增强文本嵌入方案的实时流式加速策略；3）用于生成世界事件的文本控制方法。

Result: 实现了从单张图像或文本提示生成逼真、交互式和连续世界的能力，支持键盘探索，解决了现有方法的实时性能限制并增加了文本控制功能。

Conclusion: \method框架通过创新的架构设计，成功解决了扩散模型在生成交互式世界时面临的参数规模、推理效率和历史上下文管理等问题，为实时、文本控制的虚拟世界生成提供了有效解决方案。
...
TL;DR: TDLP是一种基于检测的跟踪方法，通过轨迹与检测之间的链接预测进行逐帧关联，结合几何特征学习关联规则，在保持计算效率的同时超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前多目标跟踪存在两种主要范式：基于检测的跟踪方法计算高效但依赖手工设计的关联启发式规则，而端到端方法能从数据学习关联但计算复杂度高。需要一种既能从数据学习关联规则，又能保持模块化和计算效率的方法。

Method: 提出Track-Detection Link Prediction (TDLP)，这是一种基于检测的跟踪方法，通过轨迹与检测之间的链接预测进行逐帧关联。该方法主要设计用于几何特征（如边界框），同时可选地整合姿态和外观等额外线索。与基于启发式的方法不同，TDLP直接从数据学习关联规则，同时相比端到端跟踪器保持模块化和计算效率。

Result: 在多个基准测试上的广泛实验表明，TDLP在基于检测的跟踪和端到端方法中都一致超越了最先进的性能。详细分析显示，与基于度量学习的关联相比，链接预测在处理异构特征（如检测边界框）时更加有效。

Conclusion: TDLP通过链接预测方法实现了从数据学习关联规则的目标，在保持计算效率的同时超越了现有方法，特别是在处理异构特征时表现出色，为多目标跟踪提供了一种有效的新范式。
...
TL;DR: ProEdit提出了一种基于反转的视觉编辑方法，通过KV-mix和Latents-Shift技术解决现有方法过度依赖源图像信息导致编辑效果不佳的问题，实现了更好的属性编辑效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于反转的视觉编辑方法在采样过程中过度注入源图像信息，虽然保持了编辑一致性，但负面影响目标图像的编辑效果，导致无法有效改变主体属性（如姿态、数量、颜色等）。

Method: 提出ProEdit方法，从注意力机制和潜在空间两个角度改进：1）注意力方面引入KV-mix，混合源图像和目标图像在编辑区域的KV特征，减少源图像对编辑区域的影响同时保持背景一致性；2）潜在空间方面提出Latents-Shift，扰动源潜在空间的编辑区域，消除反转潜在对采样的影响。

Result: 在多个图像和视频编辑基准测试上实现了最先进的性能，能够有效改变主体属性（姿态、数量、颜色等）同时保持编辑一致性。

Conclusion: ProEdit通过创新的KV-mix和Latents-Shift技术解决了基于反转的视觉编辑中源图像信息过度依赖问题，实现了更好的编辑效果，且设计为即插即用，可无缝集成到现有方法中。
...
Method: 提出双向感知塑造(BiPS)：1) 应用KL一致性约束，在原始图像和保留证据的视图之间，鼓励对支持像素的完整覆盖；2) 应用KL分离约束，在原始图像和证据消除的视图之间，防止文本捷径并强制细粒度视觉依赖
...
### [63] [A Reinforcement Learning Approach to Synthetic Data Generation](https://arxiv.org/abs/2512.21395)
...
Result: 在MIMIC-IV数据集上与扩散模型表现相当，优于GANs；在更小的AI-READI数据集上优于扩散模型和GANs
...
### [64] [kooplearn: A Scikit-Learn Compatible Library of Algorithms for Evolution Operator Learning](https://arxiv.org/abs/2512.21409)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个统一的库来实现动态算子的各种估计方法，使研究人员能够通过谱方法分析动态系统，并集成到现有的机器学习工作流中。

Method: 实现线性、核和深度学习估计器来建模离散时间演化算子（Koopman/Transfer）和连续时间无穷小生成元，提供与scikit-learn兼容的API接口。

Result: 开发了kooplearn库，支持动态算子的学习、谱分析、降阶建模和预测，包含基准数据集，代码已在GitHub上开源。

Conclusion: kooplearn提供了一个功能完整的动态系统分析工具包，通过兼容scikit-learn的API促进了与现有机器学习生态系统的集成，有助于推动动态系统机器学习研究的发展。
...
### [65] [dUltra: Ultra-Fast Diffusion Language Models via Reinforcement Learning](https://arxiv.org/abs/2512.21446)
*Shirui Chen,Jiantao Jiao,Lillian J. Ratliff,Banghua Zhu*

Main category: cs.LG

TL;DR: dUltra是一个基于强化学习的框架，通过优化扩散语言模型的并行解码策略，提高采样效率，在数学推理和代码生成任务上超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的掩码扩散语言模型（MDLMs）虽然具有并行生成token的潜力，但实际采样速度有限，通常与自回归模型加推测解码方案相当。现有的基于蒸馏的加速方法存在策略偏移问题，且性能受限于基础模型的样本质量。

Method: 提出dUltra框架，基于组相对策略优化（GRPO）的在线强化学习方法，学习高效的并行解码策略。引入解掩码规划器头，预测每个token在独立伯努利分布下的解掩码概率，联合优化基础扩散LLM和解掩码顺序规划器，使用可验证奖励、蒸馏奖励和解掩码步骤数的组合奖励信号。

Result: 在数学推理和代码生成任务上，dUltra在准确性和效率的权衡上超越了最先进的启发式和蒸馏基线方法，朝着实现"扩散模型优势"（超越自回归模型）迈进。

Conclusion: dUltra通过强化学习框架优化并行解码策略，有效解决了现有MDLMs采样效率低的问题，在保持准确性的同时显著提高了采样速度，为扩散语言模型的实际应用提供了有前景的解决方案。
...
### [66] [RLLaVA: An RL-central Framework for Language and Vision Assistants](https://arxiv.org/abs/2512.21450)
*Lei Zhao,Zihao Ma,Boyu Lin,Yuhe Liu,Wenjun Wu,Lei Huang*

Main category: cs.LG

TL;DR: RLLaVA是一个用于语言视觉助手的强化学习框架，通过MDP公式化，将RL算法逻辑与模型架构和执行解耦，支持在单GPU上高效训练1B-7B模型


<details>
  <summary>Details</summary>
Motivation: 现有RL框架通常与特定模型架构和训练引擎紧密耦合，限制了研究者在多模态任务中快速实验不同RL算法和视觉语言模型的能力，需要更灵活、资源高效的框架

Method: 提出RLLaVA框架，基于马尔可夫决策过程(MDP)公式化，将RL算法逻辑与模型架构和分布式执行解耦，支持多种RL方法和视觉语言模型，保持对训练和推理引擎的不可知性

Result: RLLaVA能在单24GB GPU上端到端训练4B规模模型，在多模态和智能体任务上展示任务可扩展性，训练出的模型性能持续优于基础模型，与其他专门设计的RL框架竞争力相当

Conclusion: RLLaVA提供了一个灵活、高效的强化学习框架，使研究者能够以最小代码实现新RL算法，支持广泛的多模态模型训练，为语言视觉助手的研究提供了实用工具
...
### [67] [Statistical vs. Deep Learning Models for Estimating Substance Overdose Excess Mortality in the US](https://arxiv.org/abs/2512.21456)
*Sukanya Krishna,Marie-Laure Charpignon,Maimuna Majumder*

Main category: cs.LG

TL;DR: 本文系统比较了SARIMA与三种深度学习架构（LSTM、Seq2Seq、Transformer）在药物过量死亡率反事实估计中的表现，发现LSTM在预测精度和不确定性校准方面优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 美国2023年药物过量死亡超过8万例，COVID-19大流行通过医疗中断和行为改变加剧了现有趋势。准确估计超额死亡率（超出基于大流行前模式的预期死亡水平）对于理解大流行影响和制定干预策略至关重要。传统统计方法如SARIMA假设线性、平稳性和固定季节性，在结构性中断下可能不成立。

Method: 使用美国CDC国家数据（2015-2019年用于训练/验证，2020-2023年用于预测），系统比较SARIMA与三种深度学习架构（LSTM、Seq2Seq、Transformer）。研究采用可重复的管道，包含符合预测区间和60多次试验的收敛分析，并提供了可部署到15个州卫生部门的开源框架。
...
### [68] [MotionTeller: Multi-modal Integration of Wearable Time-Series with LLMs for Health and Behavioral Understanding](https://arxiv.org/abs/2512.21506)
...
<details>
  <summary>Details</summary>
Motivation: 随着可穿戴传感技术的普及，如何从原始生理信号（如加速度计收集的分钟级运动数据）生成自然语言摘要成为一个关键挑战。需要将可穿戴传感器数据转化为流畅、以人为中心的描述，为行为监测、临床审查和个性化健康干预开辟新途径。

Method: MotionTeller结合了预训练的活动记录编码器和轻量级投影模块，将行为嵌入映射到冻结的解码器LLM的令牌空间。使用从真实世界NHANES记录中提取的54383个（活动记录，文本）对数据集进行训练，仅对语言令牌使用交叉熵损失进行监督训练。

Result: MotionTeller实现了高语义保真度（BERTScore-F1 = 0.924）和词汇准确性（ROUGE-1 = 0.722），比基于提示的基线方法在ROUGE-1上高出7%。平均训练损失在第15个epoch收敛到0.38，表明优化稳定。定性分析确认模型能捕捉昼夜节律结构和行为转换，PCA图显示训练后嵌入空间的聚类对齐增强。
...
### [69] [Missing Pattern Tree based Decision Grouping and Ensemble for Deep Incomplete Multi-View Clustering](https://arxiv.org/abs/2512.21510)
*Wenyuan Yang,Jie Xu,Hongqing He,Jiangzhang Gan,Xiaofeng Zhu*

Main category: cs.LG

TL;DR: TreeEIC：基于缺失模式树的不完整多视图聚类框架，通过分组决策集、决策集成和知识蒸馏，充分利用可用多视图对，提升高度不一致缺失模式下的聚类性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界多视图数据通常存在高度不一致的缺失模式，现有不完整多视图聚类方法忽视了"配对利用不足"问题——不一致的缺失模式使得不完整但可用的多视图对无法被充分利用，限制了模型性能。

Method: 1) 定义缺失模式树模型，根据不同的缺失模式将数据分组到多个决策集中，在每个集合内进行多视图聚类；2) 提出多视图决策集成模块，聚合所有决策集的聚类结果，通过基于不确定性的权重抑制不可靠的聚类决策；3) 设计集成到个体的知识蒸馏模块，将集成知识转移到视图特定的聚类模型中，通过优化跨视图一致性和簇间区分性损失实现相互促进。

Result: 在多个基准数据集上的广泛实验表明，TreeEIC实现了最先进的不完整多视图聚类性能，在高度不一致的缺失模式下表现出卓越的鲁棒性。

Conclusion: TreeEIC通过充分利用可用多视图对、集成多个决策集的聚类结果以及知识蒸馏机制，有效解决了不完整多视图聚类中的配对利用不足问题，显著提升了在高度不一致缺失模式下的聚类性能。
...
### [70] [Global-Graph Guided and Local-Graph Weighted Contrastive Learning for Unified Clustering on Incomplete and Noise Multi-View Data](https://arxiv.org/abs/2512.21516)
*Hongqing He,Jie Xu,Wenyuan Yang,Yonghua Zhu,Guoqiu Wen,Xiaofeng Zhu*

Main category: cs.LG

TL;DR: 提出了一种统一的对比学习多视图聚类框架，通过全局-局部图引导的对比学习解决不完整和噪声多视图数据中的样本配对问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界多视图数据存在不完整性和噪声，导致样本配对稀少或错误配对，这严重影响了基于对比学习的多视图聚类的有效性。稀少配对问题阻碍了多视图互补信息的充分提取，而错误配对问题则导致对比学习在错误方向上优化模型。

Method: 1) 针对稀少配对问题，设计了全局图引导的对比学习，所有视图样本构建全局视图亲和力图，形成新的样本对以充分探索互补信息；2) 针对错误配对问题，提出局部图加权对比学习，利用局部邻居生成配对权重，自适应地加强或减弱配对对比学习。该方法无需插补，可集成到统一的全局-局部图引导对比学习框架中。

Result: 在不完整和噪声设置下的多视图数据上进行了大量实验，证明该方法相比现有最先进方法取得了优越的性能。
...
### [71] [First Provable Guarantees for Practical Private FL: Beyond Restrictive Assumptions](https://arxiv.org/abs/2512.21521)
*Egor Shulgin,Grigory Malinovsky,Sarit Khirirat,Peter Richtárik*

Main category: cs.LG

TL;DR: Fed-α-NormEC：首个在标准假设下提供可证明收敛性和差分隐私保证的联邦学习框架，支持本地更新、部分客户端参与等实际特征


<details>
  <summary>Details</summary>
Motivation: 当前联邦学习的差分隐私方法依赖不现实的假设（如梯度有界或同质性），阻碍实际应用；现有放宽假设的研究通常忽略实际FL特征，如多次本地更新和部分客户端参与

Method: 提出Fed-α-NormEC框架，整合本地更新（完整和增量梯度步骤）、独立的服务器和客户端步长，以及关键的部分客户端参与机制

Result: 在标准假设下提供可证明的收敛性和差分隐私保证，通过私有深度学习任务的实验验证了理论保证

Conclusion: Fed-α-NormEC是首个在支持实际FL特征的同时提供严格隐私保证的框架，解决了现有方法的局限性，具有实际部署价值
...
### [72] [Generative Actor Critic](https://arxiv.org/abs/2512.21527)
*Aoyang Qin,Deqian Kong,Wei Wang,Ying Nian Wu,Song-Chun Zhu,Sirui Xie*

Main category: cs.LG

TL;DR: GAC是一种新的强化学习框架，将策略评估重构为学习轨迹与回报的联合分布生成模型，将策略改进重构为对该模型的推理，显著提升了离线到在线学习的性能。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习算法主要关注估计或最大化期望回报，但在用在线经验精调离线预训练模型时面临挑战。需要一种能够更好地结合离线预训练和在线精调的方法。

Method: 提出生成式行动者-评论家(GAC)框架：1) 策略评估：学习轨迹τ和回报y的联合分布p(τ,y)的生成模型；2) 策略改进：对学习到的模型进行灵活的推理。具体实现基于连续潜在计划向量的隐变量模型，开发了两种推理策略：利用（优化潜在计划以最大化期望回报）和探索（基于动态调整的目标回报采样潜在计划）。

Result: 在Gym-MuJoCo和Maze2D基准测试中，GAC展现出强大的离线性能，并且在离线到在线改进方面显著优于最先进的方法，即使在缺乏逐步奖励的情况下也表现良好。

Conclusion: GAC通过将序列决策解耦为生成建模和推理，提供了一种新颖的强化学习框架，能够有效结合离线预训练和在线精调，在多种环境中展现出优越性能。
...
### [73] [AVP-Fusion: Adaptive Multi-Modal Fusion and Contrastive Learning for Two-Stage Antiviral Peptide Identification](https://arxiv.org/abs/2512.21544)
*Xinru Wen,Weizhong Lin,Xuan Xiao*

Main category: cs.LG

TL;DR: AVP-Fusion：一种新颖的两阶段深度学习框架，通过自适应特征融合和对比学习准确识别抗病毒肽，显著优于现有方法，并能进行病毒家族和特定病毒的亚类预测。
...
Method: 提出AVP-Fusion两阶段深度学习框架：1）使用10种不同描述符构建全景特征空间；2）引入自适应门控机制动态调节CNN提取的局部基序和BiLSTM捕获的全局依赖的权重；3）采用基于在线难例挖掘和BLOSUM62数据增强的对比学习策略；4）第二阶段利用迁移学习进行病毒家族和特定病毒的亚类预测。

Result: 在基准数据集Set 1上，AVP-Fusion达到0.9531的准确率和0.9064的MCC，显著优于现有最先进方法。模型还能在有限样本下精确预测六个病毒家族和八种特定病毒的亚类。

Conclusion: AVP-Fusion是一个强大且可解释的工具，可用于高通量抗病毒药物筛选，为抗病毒肽识别提供了有效的解决方案。
...
### [74] [Discovering Sparse Recovery Algorithms Using Neural Architecture Search](https://arxiv.org/abs/2512.21563)
*Patrick Yubeaton,Sarthak Gupta,M. Salman Asif,Chinmay Hegde*

Main category: cs.LG

TL;DR: 本文提出使用元学习工具（如神经架构搜索）自动发现信号处理中逆问题求解算法的新方法，以ISTA和FISTA算法为案例，成功从超过5万个变量的搜索空间中重新发现了这些算法的关键元素。


<details>
  <summary>Details</summary>
Motivation: 信号处理中逆问题求解算法的设计非常困难、依赖启发式方法且耗时，需要更自动化的算法发现方法。

Method: 开发了一个元学习框架，利用神经架构搜索（NAS）等工具，在包含超过50,000个变量的搜索空间中自动发现算法，以ISTA和FISTA算法为具体研究对象。

Result: 成功重新发现了ISTA和FISTA算法的多个关键元素，并展示了该框架可应用于不同数据分布和算法类型。

Conclusion: 元学习框架能够有效自动化信号处理算法的发现过程，为算法设计提供了新的自动化途径。
...
### [75] [Videos are Sample-Efficient Supervisions: Behavior Cloning from Videos via Latent Representations](https://arxiv.org/abs/2512.21586)
*Xin Liu,Haoran Li,Dongbin Zhao*

Main category: cs.LG

TL;DR: BCV-LR是一个从视频中进行无监督模仿学习的新框架，通过自监督任务提取动作相关的潜在特征，利用基于动力学的无监督目标预测潜在动作，然后在线对齐到真实动作空间进行策略克隆，实现了极高的样本效率。


<details>
  <summary>Details</summary>
Motivation: 人类能够通过少量试错从视频中高效提取知识和学习技能，但这对自主智能体来说是一个巨大挑战，因为视觉输入的复杂性、缺乏动作或奖励信号以及交互步骤的限制。

Method: 提出BCV-LR框架：1）通过自监督任务从高维视频输入中提取动作相关的潜在特征；2）利用基于动力学的无监督目标预测连续帧之间的潜在动作；3）在线微调预训练的潜在动作并与真实动作空间对齐进行策略行为克隆；4）克隆的策略反过来丰富智能体经验，形成迭代的策略改进循环。

Result: 在包括离散控制和连续控制在内的一系列具有挑战性的视觉任务上进行广泛实验，BCV-LR仅需少量交互就能实现有效（在某些任务上甚至达到专家级）的策略性能，在24/28个任务上超越了最先进的ILV基线和强化学习方法（提供环境奖励）的样本效率。

Conclusion: 这是首次证明视频可以在不需要任何其他专家监督的情况下支持极其样本高效的视觉策略学习，为从视频中进行无监督模仿学习提供了新的解决方案。
...
### [76] [A Data-Driven Multi-Objective Approach for Predicting Mechanical Performance, Flowability, and Porosity in Ultra-High-Performance Concrete (UHPC)](https://arxiv.org/abs/2512.21610)
...
Method: 采用两阶段流程：首先从21种机器学习算法中筛选出5个高性能模型，XGBoost表现最佳；然后通过去除多重共线性特征、孤立森林识别异常值、SHAP分析选择重要特征来清洗数据集，重新训练XGBoost模型；最后开发图形用户界面支持应用。

Result: XGBoost模型经过超参数调优和数据集清洗后，在所有输出指标上都实现了高预测精度，显著减少了实验测试需求，并开发了实用的图形用户界面。

Conclusion: 该框架显著提高了超高性能混凝土性能预测的准确性，为材料设计提供了高效的数据驱动工具，减少了实验成本和时间消耗。
...
### [77] [Mechanical Strength Prediction of Steel-Polypropylene Fiber-based High-Performance Concrete Using Hybrid Machine Learning Algorithms](https://arxiv.org/abs/2512.21638)
*Jagaran Chakma,Zhiguang Zhou,Badhan Chakma*

Main category: cs.LG

TL;DR: 本研究开发并评估了三种机器学习模型（ET-XGB、RF-LGBM、Transformer-XGB）来预测钢-聚丙烯纤维增强高性能混凝土的力学性能，ET-XGB模型整体表现最佳，SHAP分析揭示了关键影响因素。
...
Method: 使用三种模型家族：ET-XGB、RF-LGBM和Transformer-XGB，基于已发表实验研究的大数据集，预测压缩强度、弯曲强度和拉伸强度。采用k折交叉验证、超参数优化、SHAP分析和不确定性分析确保模型鲁棒性和可解释性。

Result: ET-XGB模型整体精度最高（CS R²=0.994，FS R²=0.944，TS R²=0.978），CS和TS不确定性最低（13-16%和30.4%）。RF-LGBM对FS预测最稳定可靠（R²=0.977，不确定性5-33%）。Transformer-XGB预测能力强但不确定性最高。SHAP分析显示纤维长径比、硅灰和钢纤维含量是最重要预测因子。

Conclusion: 机器学习模型能够提供准确、可解释且可泛化的HPC力学性能预测，为优化混凝土配合比设计和提高工程应用中的结构性能评估提供了有价值的工具。
...
### [78] [Variance-Aware Prior-Based Tree Policies for Monte Carlo Tree Search](https://arxiv.org/abs/2512.21648)
*Maximilian Weichart*

Main category: cs.LG

TL;DR: 本文提出了Inverse-RPO方法，能够从任何无先验的UCB算法中系统性地推导出基于先验的UCT树搜索策略，并应用于方差感知的UCB-V算法，获得了优于传统PUCT的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然AlphaZero中引入先验项的PUCT策略在实践中表现出色，但它是经验性推导而非基于理论原则。许多具有更强理论保证的UCB算法难以扩展到基于先验的UCT中。需要一种系统性的方法来从任何无先验UCB推导出基于先验的UCT策略。

Method: 基于将MCTS视为正则化策略优化问题的视角，提出了Inverse-RPO方法。该方法能够从任何无先验的UCB算法系统性地推导出基于先验的UCT策略。特别地，将方法应用于方差感知的UCB-V算法，得到了两种新的基于先验的树搜索策略。

Result: 实验表明，这些方差感知的基于先验UCT策略在多个基准测试中优于传统的PUCT策略，且不增加额外的计算成本。同时开发了mctx库的扩展，支持方差感知UCT，代码改动最小化。

Conclusion: Inverse-RPO提供了一种系统性的方法来从任何无先验UCB推导出基于先验的UCT策略，为基于先验的UCT研究提供了理论基础。方差感知的UCT策略在实践中表现出优越性能，有望推动强化学习领域的发展。
...
### [79] [Causal-HM: Restoring Physical Generative Logic in Multimodal Anomaly Detection via Hierarchical Modulation](https://arxiv.org/abs/2512.21650)
*Xiao Liu,Junchen Jin,Yanjie Zhao,Zhixuan Xing*

Main category: cs.LG

TL;DR: 提出Causal-HM框架，通过显式建模物理过程到结果的依赖关系，解决多模态异常检测中的因果盲点和异质性差距问题，在焊接质量检测中达到90.7%的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态异常检测方法存在因果盲点，将过程模态（视频、音频、传感器）和结果模态（焊后图像）视为平等特征源，忽略了物理生成逻辑；同时高维视觉数据和低维传感器信号之间的异质性差距导致关键过程上下文被淹没。

Method: 提出Causal-HM统一多模态异常检测框架：1）传感器引导的CHM调制机制，利用低维传感器信号作为上下文指导高维视听特征提取；2）因果层次架构，强制单向生成映射以识别违反物理一致性的异常。
...
### [80] [Rethinking Output Alignment For 1-bit Post-Training Quantization of Large Language Models](https://arxiv.org/abs/2512.21651)
*Dung Anh Hoang,Cuong Pham,Cuong Nguyen,Trung le,Jianfei Cai,Thanh-Toan Do*

Main category: cs.LG

TL;DR: 本文研究1位LLM量化中输出匹配方法失败的原因，并提出一种数据感知的PTQ方法，通过考虑激活误差累积来提升1位量化性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在资源受限设备上部署困难，需要压缩技术。虽然后训练量化（PTQ）因无需重训练而受欢迎，但1位量化（权重转为±1）仍面临显著性能下降的挑战。现有1位PTQ方法主要关注权重对齐而非输出对齐，而直接应用输出匹配方法在1位LLM中会导致性能下降，需要研究其失败原因和改进方案。

Method: 研究1位LLM量化中输出匹配方法失败的原因和条件，基于研究发现提出一种新颖的数据感知PTQ方法，该方法明确考虑激活误差累积，同时保持优化效率。

Result: 实验证明，所提出的解决方案在最小开销下持续优于现有的1位PTQ方法。

Conclusion: 通过分析1位LLM量化中输出匹配失败的原因，提出了一种有效的数据感知PTQ方法，能够显著提升1位量化的性能，为资源受限设备上的LLM部署提供了更好的解决方案。
...
### [81] [Dynamic Feedback Engines: Layer-Wise Control for Self-Regulating Continual Learning](https://arxiv.org/abs/2512.21743)
...
<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法通常对所有层统一处理，在稳定性和可塑性之间权衡，但不同层在分类任务时自然表现出不同的不确定性（熵）。高熵层容易欠拟合，低熵层容易过拟合，需要针对性地调节

Method: 提出熵感知持续学习方法，采用动态反馈机制根据每层的熵值进行调节：减少高熵层的熵以缓解欠拟合，增加过度自信层的熵以缓解过拟合。这种自适应调节鼓励模型收敛到更宽的局部最小值，提升泛化能力。方法具有通用性，可与回放和正则化方法无缝集成

Result: 在多个数据集上的实验表明，相比最先进的持续学习基线方法，该方法取得了显著的性能提升

Conclusion: 通过熵感知的动态层调节机制，有效解决了持续学习中不同层的不平衡问题，提高了模型的泛化性能，为持续学习提供了一种新的有效方法
...
### [82] [A Comedy of Estimators: On KL Regularization in RL Training of LLMs](https://arxiv.org/abs/2512.21852)
...
<details>
  <summary>Details</summary>
Motivation: 尽管KL正则化在LLM强化学习中广泛应用，但现有实践中KL估计器的配置方式缺乏系统研究，且存在目标函数与实现之间的不一致性，导致训练不稳定和性能问题。
...
Result: 在在线策略设置中：(1)梯度有偏的估计器配置会导致训练不稳定；(2)使用梯度无偏的估计器配置在域内和域外任务上都能获得更好性能。在离线策略设置中，KL正则化有助于稳定异步设置导致的训练不稳定。

Conclusion: KL估计器配置的选择对LLM强化学习训练至关重要，梯度无偏的配置能带来更稳定和更好的性能表现，为实践中的KL正则化实现提供了重要指导。
...
### [83] [Secure and Explainable Fraud Detection in Finance via Hierarchical Multi-source Dataset Distillation](https://arxiv.org/abs/2512.21866)
*Yiming Qian,Thorsten Neumann,Xueyining Huang,David Hardoon,Fei Gao,Yong Liu,Siow Mong Rick Goh*

Main category: cs.LG

TL;DR: 提出一种可解释、保护隐私的数据集蒸馏框架，用于协作式金融欺诈检测。将训练好的随机森林转换为透明的轴对齐规则区域，并在每个区域内均匀采样生成合成交易，创建紧凑、可审计的替代数据集。


<details>
  <summary>Details</summary>
Motivation: 解决金融欺诈检测中多机构协作时的数据隐私保护问题，同时需要保持模型性能、可解释性和可审计性，满足监管要求。

Method: 将训练好的随机森林转换为轴对齐规则区域（叶节点超矩形），在每个区域内均匀采样生成合成交易，创建替代数据集。支持通过规则统计进行全局解释，通过分配案例到生成区域提供局部解释和不确定性量化。

Result: 在IEEE-CIS欺诈数据集上，蒸馏数据集减少数据量85%-93%，保持竞争力精度和micro-F1，AUC略有下降。跨机构共享合成数据提升跨集群性能。真实与合成数据结构相似度超过93%，成员推理攻击准确率接近随机水平（约0.50）。

Conclusion: 树区域蒸馏框架实现了可信赖、可部署的欺诈分析，具有可解释的全局规则、带量化不确定性的案例解释，以及适合多机构设置和监管审计的强大隐私保护特性。
...
### [84] [GQ-VAE: A gated quantized VAE for learning variable length tokens](https://arxiv.org/abs/2512.21913)
*Theo Datta,Kayla Huang,Sham Kakade,David Brandfonbrener*

Main category: cs.LG

TL;DR: GQ-VAE是一种新型神经分词器，可作为现有分词器的即插即用替代方案，通过变长离散标记编码改善压缩和语言建模性能。


<details>
  <summary>Details</summary>
Motivation: 当前前沿模型大多使用确定性频率分词算法（如BPE），而神经分词器通常增加模型复杂性并需要大幅改变架构，难以大规模实施。

Method: 提出门控量化变分自编码器（GQ-VAE），可独立预训练作为现有分词器的替代方案，关键创新是学习编码变长离散标记。

Result: GQ-VAE相比标准VQ-VAE分词器改善了压缩和语言建模性能，接近BPE的压缩率和语言建模性能；在压缩率相同时，GQ-VAE能改善下游语言模型学习。

Conclusion: GQ-VAE为神经分词器提供了一种实用方案，可作为现有分词器的即插即用替代，并指出了未来研究的多个方向。
...
### [85] [Exploring the Heterogeneity of Tabular Data: A Diversity-aware Data Generator via LLMs](https://arxiv.org/abs/2512.21915)
*Yafeng Tang,Xiaoou Ding,Jianzhuo Du,Zishuo Yan,Zhuang Ma,Zheng Liang,Zekai Qian,Hongzhi Wang*

Main category: cs.LG

TL;DR: DATE是一个多样性感知的表格数据生成框架，通过数据分区、LLM决策树推理和MAB采样算法，在保持数据多样性的同时生成高质量表格数据，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的表格数据具有天然的异质性，包含多样化的分布，这使得为多样化数据生成找到一个普遍适用的好模型变得困难。现有生成模型难以处理这种分布多样性问题。

Method: DATE框架包含三个关键步骤：(1) 将原始异质数据有效分区为多个分布不同的子集，为上下文学习准备高质量示例；(2) 利用LLM通过决策树推理探索分区分布的多样性，为每个子集生成高质量标记数据；(3) 设计基于多臂老虎机的采样算法，平衡生成数据的多样性和质量。

Result: 在表格分类和回归基准测试中，DATE始终优于最先进的基于GAN和LLM的方法。平均而言，仅使用100个生成数据就能实现23.75%的错误率降低。DATE生成的数据还能提高DPO的准确性，并增强LLM在目标数据上的推理能力。

Conclusion: DATE通过有效处理表格数据的异质性分布，在保持多样性的同时生成高质量数据，为机器学习应用提供了大规模、高质量的数据生成解决方案，显著提升了现有方法的性能。
...
### [86] [Hybrid Combinatorial Multi-armed Bandits with Probabilistically Triggered Arms](https://arxiv.org/abs/2512.21925)
*Kongchang Zhou,Tingyu Zhang,Wei Chen,Fang Kong*

Main category: cs.LG

TL;DR: 提出混合CMAB-T框架，结合离线数据和在线交互，解决传统在线方法交互成本高、离线方法受限于数据质量和缺乏探索能力的问题。


<details>
  <summary>Details</summary>
Motivation: 传统组合多臂老虎机（CMAB-T）研究主要分为在线和离线两种范式：在线算法存在高交互成本和慢适应问题，而离线方法受限于数据集质量和缺乏探索能力。需要结合两者优势来解决这些互补的局限性。

Method: 提出混合CMAB-T框架，设计混合CUCB算法，利用离线数据指导探索和加速收敛，同时策略性地结合在线交互来弥补离线数据集覆盖不足或分布偏差的问题。

Result: 理论分析证明了算法的遗憾界，表明当有高质量离线数据时，混合CUCB显著优于纯在线方法；当数据有限或存在偏差时，能有效纠正纯离线方法的偏差。实证结果进一步证明了算法的持续优势。

Conclusion: 混合CMAB-T框架成功整合了离线数据和在线交互的优势，在理论上和实证上都表现出色，为解决组合多臂老虎机问题提供了更有效的方法。
...
### [87] [Direction Finding with Sparse Arrays Based on Variable Window Size Spatial Smoothing](https://arxiv.org/abs/2512.22024)
...
<details>
  <summary>Details</summary>
Motivation: 传统固定窗口协阵列MUSIC方法在处理稀疏阵列时存在性能限制，需要改进协阵列数据平滑技术以提升DOA估计精度

Method: 提出VWS-CA-MUSIC和VWS-CA-rMUSIC算法，采用可变窗口大小空间平滑框架，压缩平滑孔径，用未扰动的低秩附加项替换部分扰动的秩一外积

Result: 仿真显示在稀疏阵列几何下，相比固定窗口协阵列MUSIC方法，新方法在性能和计算复杂度方面均有显著提升

Conclusion: 可变窗口大小空间平滑框架能有效增强协阵列DOA估计性能，同时推导了保证可识别性的压缩参数边界条件
...
### [88] [LibContinual: A Comprehensive Library towards Realistic Continual Learning](https://arxiv.org/abs/2512.22029)
*Wenbin Li,Shangge Liu,Borui Kang,Yiyang Chen,KaXuan Lew,Yang Chen,Yinghuan Shi,Lei Wang,Yang Gao,Jiebo Luo*

Main category: cs.LG

TL;DR: LibContinual是一个用于持续学习的统一框架库，旨在解决该领域方法碎片化、评估不一致的问题，并通过严格评估揭示了现有方法在现实约束下的性能下降。


<details>
  <summary>Details</summary>
Motivation: 持续学习领域面临方法碎片化问题，缺乏统一框架导致公平比较和可复现研究困难。现有评估存在不现实的假设，如离线数据可访问、无限制内存资源和任务内语义同质性，这些假设高估了方法在真实场景中的适用性。

Method: 提出LibContinual库，采用高内聚、低耦合的模块化架构，集成了5个主要方法类别的19种代表性算法。基于该框架，系统识别并研究了三个主流评估中的隐含假设，并通过严格的在线CL设置、统一内存预算协议和类别随机化设置进行综合分析。

Result: 研究发现，当受到现实约束时，许多代表性CL方法的性能显著下降。严格的在线CL设置、统一内存预算和类别随机化设置揭示了现有方法在资源受限和语义异质性场景中的局限性。

Conclusion: 需要开发资源感知和语义鲁棒的CL策略，LibContinual为现实持续学习的未来研究提供了基础工具包，强调了在更真实约束下评估CL方法的重要性。
...
### [89] [From In Silico to In Vitro: Evaluating Molecule Generative Models for Hit Generation](https://arxiv.org/abs/2512.22031)
...
<details>
  <summary>Details</summary>
Motivation: 传统的高通量筛选耗时耗力，虚拟筛选方法仍然成本高昂。虽然深度学习生成模型能够生成新化合物，但用机器学习完全替代整个药物发现流程极具挑战性。因此，研究者探索是否可以用生成模型替代其中一个步骤——hit-like分子生成。

Method: 提出专门针对hit-like分子生成任务的评估框架，整合物理化学、结构和生物活性相关标准，构建多阶段过滤流程来定义hit-like化学空间。对两种自回归模型和一种扩散模型进行基准测试，使用标准指标和靶点特异性对接评分评估输出。

Result: 生成模型能够为多个靶点生成有效、多样且具有生物相关性的化合物。部分选定的GSK-3β命中分子被合成并在体外实验中确认具有活性。研究还识别了当前评估指标和可用训练数据的关键局限性。

Conclusion: 这是首次将hit-like分子生成作为独立任务的研究，实证表明生成模型可以直接支持药物发现流程的这一阶段，但当前评估指标和训练数据存在局限性需要改进。
...
### [90] [Scaling Adversarial Training via Data Selection](https://arxiv.org/abs/2512.22069)
*Youran Ye,Dejin Wang,Ajinkya Bhandare*

Main category: cs.LG

TL;DR: 提出选择性对抗训练，仅扰动每个小批量中的关键样本子集，通过两种选择标准（边界采样和梯度匹配采样）减少对抗计算50%，同时保持或超越完整PGD对抗训练的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: PGD对抗训练计算成本高，所有训练样本都经过相同的迭代内循环优化，尽管它们对鲁棒性的贡献不均衡。这种效率低下促使研究者开发更高效的对抗训练方法。

Method: 提出选择性对抗训练，仅扰动每个小批量中的关键样本子集。引入两种选择标准：1）基于边界的采样，优先处理决策边界附近的样本；2）梯度匹配采样，选择梯度与主导批量优化方向对齐的样本。对选定子集生成对抗样本，其余样本使用混合目标进行干净训练。

Result: 在MNIST和CIFAR-10上的实验表明，所提方法实现了与完整PGD对抗训练相当甚至更好的鲁棒性，同时将对抗计算减少高达50%。
...
### [91] [Unifying Learning Dynamics and Generalization in Transformers Scaling Law](https://arxiv.org/abs/2512.22088)
*Chiwun Yang*
...
<details>
  <summary>Details</summary>
Motivation: 尽管缩放定律在大型语言模型开发中经验性地得到验证，但其理论基础仍然薄弱。本研究旨在为Transformer语言模型的缩放行为提供严格的理论分析，超越以往的简化模型分析。

Method: 将基于Transformer的语言模型学习动态形式化为常微分方程系统，然后近似为核行为。对多层Transformer在序列到序列数据上的随机梯度下降训练进行严格分析，考虑任意数据分布，贴近真实条件。

Result: 建立了超额风险的理论上界，揭示了明显的相变现象：初始优化阶段超额风险随计算成本指数衰减；超过特定资源分配阈值后，系统进入统计阶段，泛化误差遵循幂律衰减Θ(C^{-1/6})。此外，理论还推导了模型大小、训练时间和数据集大小的独立缩放定律。

Conclusion: 该研究为Transformer语言模型的缩放定律提供了统一的理论框架，阐明了计算资源分配如何影响泛化性能，揭示了优化阶段和统计阶段之间的相变现象，为大型语言模型开发提供了理论指导。
...
### [92] [Explainable Multimodal Regression via Information Decomposition](https://arxiv.org/abs/2512.22102)
*Zhaozhao Ma,Shujian Yu*

Main category: cs.LG

TL;DR: 提出基于偏信息分解的多模态回归框架，通过高斯假设解析计算模态间的独特、冗余和协同信息，提升预测精度和可解释性
...
Method: 基于偏信息分解框架，通过高斯性假设和逆正态变换实现PID项的解析计算，并引入条件独立性正则化促进独特信息的分离

Result: 在六个真实数据集（包括大规模脑年龄预测）上优于现有方法，在预测精度和可解释性方面均有提升，并能指导模态选择

Conclusion: 提出的PID框架为多模态回归提供了可解释的量化工具，在保持预测性能的同时增强了模型透明度
...
TL;DR: 本研究开发了一个基于多模态大语言模型的多智能体系统，用于标准化HTP绘画测试的评估，解决了传统方法主观性强、评分标准不统一的问题。
...
Conclusion: 研究证实了多模态大模型作为投射评估标准化工具的潜力，提出的多智能体框架为数字心理健康服务提供了新范式。
...
TL;DR: 本文分析使用当前最先进的计算机围棋求解器（基于相关区域搜索和相关区域模式表技术）解决围棋死活题的行为，发现求解器能识别关键区域、发现罕见模式，甚至在某些问题上给出与标准答案不同的解法，但也存在对罕见模式价值误判和偏好直接活棋而非最大化地域的问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是分析当前最先进的计算机围棋求解器在解决围棋死活题时的行为表现，特别是使用相关区域搜索（RZS）和相关区域模式表技术时，求解器如何工作、发现了什么模式，以及与人类棋手解法的差异。

Method: 研究方法包括：1）使用基于相关区域搜索和相关区域模式表的计算机围棋求解器；2）测试7个来自赵治勋《死活辞典》的经典死活题；3）分析求解器识别的相关区域、发现的模式类型；4）对比求解器答案与书中标准答案的差异。

Result: 研究结果：1）求解器能为每个问题识别出关键的相关区域；2）发现一系列模式，包括一些罕见模式；3）在两个问题上找到了与书中标准答案不同的解法；4）发现求解器存在两个问题：对罕见模式的价值误判，以及倾向于直接活棋而非最大化地域（与人类棋手行为不同）。

Conclusion: 结论表明当前计算机围棋求解器在死活题求解方面表现出色，能识别关键区域和发现罕见模式，甚至在某些问题上提供新解法，但仍存在对罕见模式价值评估和策略选择方面的局限性，需要进一步改进以更接近人类棋手的思考方式。
...
TL;DR: 本文在回顾完整信息下三支决策的计算与概念两种形式的基础上，将其推广到更实用的不完备信息场景，提出了基于相似度度量和公式满足度度量的新方法。


<details>
  <summary>Details</summary>
Motivation: 现有三支决策方法主要处理完备信息，但在现实应用中不完备信息更为常见。需要将完备信息下的两种形式（计算形式和概念形式）推广到不完备信息场景，以增强方法的实用性。

Method: 1. 计算形式：提出对象相似度度量作为等价关系的推广，基于此讨论两种三支决策方法：使用α-相似类和对象可逼近性；2. 概念形式：提出公式满足度度量作为完备信息下满足关系的定量推广，基于此研究两种方法：使用α-意义集和公式置信度。

Result: 成功将三支决策的计算和概念两种形式推广到不完备信息场景，提出了基于相似度度量和满足度度量的新方法。其中对象可逼近性和概念形式中的两种方法指出了新的研究方向。

Conclusion: 本文为不完备信息下的三支决策提供了系统的理论框架，将完备信息下的两种形式成功推广，提出的新概念和方法为不完备信息处理开辟了有前景的研究方向。
...
TL;DR: LogicLens是一个统一的视觉-文本协同推理框架，用于文本中心伪造分析，通过跨线索感知思维链机制和GRPO优化，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前文本中心伪造分析方法存在三个主要问题：1）仅限于粗粒度的视觉分析，缺乏复杂推理能力；2）将检测、定位和解释视为离散子任务，忽视了它们之间的内在联系；3）缺乏高质量、认知对齐的标注数据集。

Method: 提出了LogicLens统一框架，包含：1）跨线索感知思维链机制，迭代交叉验证视觉线索与文本逻辑；2）加权多任务奖励函数用于GRPO优化；3）PR²管道（感知器、推理器、评审器）生成高质量标注；4）构建RealText数据集（5,397张图像，包含文本解释、像素级分割和真实性标签）。

Result: 在T-IC13的零样本评估中，LogicLens比专用框架高出41.4%，比GPT-4o高出23.4%的宏平均F1分数。在密集文本T-SROIE数据集上，在mF1、CSS和宏平均F1方面显著领先其他MLLM方法。

Conclusion: LogicLens通过统一的视觉-文本协同推理框架，有效解决了文本中心伪造分析的现有局限性，在检测、定位和解释任务上实现了整体性能提升，为伪造分析提供了新的解决方案。
...
TL;DR: Leash：一个通过自适应长度惩罚和奖励塑形来优化LLM推理效率的强化学习框架，能在保持任务性能的同时将平均推理长度减少60%。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖固定长度惩罚，但这种惩罚难以调整且无法适应LLM不断发展的推理能力，导致准确性和简洁性之间的权衡不理想。

Method: 将长度控制建模为约束优化问题，采用拉格朗日对偶方法动态调整惩罚系数：当生成超过目标长度时加强惩罚，当生成较短时放松惩罚。

Result: 在Deepseek-R1-Distill-Qwen-1.5B和Qwen3-4B-Thinking-2507上的实验表明，Leash能在保持竞争力的任务性能的同时，在数学推理、编码和指令遵循等多种任务中将平均推理长度减少60%。

Conclusion: Leash为开发可控且高效的LLM提供了一个实用有效的范式，能够在推理能力和计算预算之间取得良好平衡。
...
TL;DR: 基于LLaVA的医疗诊断框架，通过视觉语言对齐和逻辑正则化推理，提高多模态医疗AI的诊断准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型和视觉语言模型在医学领域的快速发展，简单地整合临床文本和医学影像并不能保证可靠的推理。现有的多模态模型经常产生幻觉或不一致的思维链，限制了临床信任。
...
Conclusion: 这些结果表明了朝着可信赖的多模态医疗AI迈出的有希望的一步。
...
TL;DR: OrchestRA是一个人类在环的多智能体平台，将生物学、化学和药理学整合为自主发现引擎，通过自主执行模拟和结果推理来驱动迭代优化，实现从随机搜索到可编程循证工程的药物发现转变。


<details>
  <summary>Details</summary>
Motivation: 当前治疗发现面临专业领域碎片化以及计算设计与生理验证之间执行差距的挑战。现有生成式AI模型通常作为被动助手而非自主执行者，无法有效整合多学科知识并实现闭环优化。

Method: OrchestRA采用人类在环的多智能体架构：1) Orchestrator协调整个流程；2) Biologist Agent基于包含1000万以上关联的大规模知识图谱进行深度推理，识别高置信度靶点；3) Chemist Agent自主检测结构口袋进行从头设计或药物重定位；4) Pharmacologist Agent通过基于生理的药代动力学(PBPK)模拟评估候选药物。系统建立动态反馈循环，药代动力学和毒性特征直接触发结构重新优化。

Result: 该平台将生物学、化学和药理学统一为自主发现引擎，实现了从被动辅助到主动执行的转变。通过整合自主执行与人类指导，OrchestRA能够将药物发现从随机搜索转变为可编程的循证工程学科。

Conclusion: OrchestRA通过人类在环的多智能体平台，弥合了计算设计与生理验证之间的执行差距，实现了治疗发现的民主化，将药物发现转变为基于证据的可编程工程学科。
...
TL;DR: 本文提出了一种针对LLM应用、边缘智能等资源分配问题的多臂赌博机变体，其中每个臂具有随机容量，容量单位与奖励函数关联，每次选择具有优先级权重。当多个选择竞争臂容量时，按优先级权重从大到小分配。论文证明了实例无关和实例相关的遗憾下界，并设计了算法来寻找最优分配策略。


<details>
  <summary>Details</summary>
Motivation: 针对LLM应用、边缘智能等场景中的资源分配问题，传统多臂赌博机模型无法处理具有随机容量和优先级权重的复杂资源分配机制。需要开发新的模型和算法来解决这种具有优先级资源共享机制的非线性组合效用函数优化问题。

Method: 提出MSB-PRS-OffOpt算法，在给定模型参数时以O(MK^3)计算复杂度定位最优分配策略。以此为基础，设计了近似上置信界（UCB）算法，通过优先级资源共享机制处理非线性组合效用函数优化和学习的技术挑战。

Result: 证明了实例无关遗憾下界Ω(α₁σ√(KMT))和实例相关遗憾下界Ω(α₁σ²(M/Δ)lnT)。设计的UCB算法实现了与相应下界匹配的遗憾上界（分别相差√(K ln KT)和α₁K²因子）。

Conclusion: 该研究为具有优先级资源共享机制的多臂赌博机问题提供了理论框架和算法解决方案，能够有效处理LLM应用和边缘智能中的复杂资源分配问题，填补了传统模型在处理此类非线性组合效用函数优化方面的空白。
...
TL;DR: 本文提出了一种基于多模型共识和推理层治理的负责任(RAI)和可解释(XAI)AI智能体架构，用于生产级智能体工作流，通过异构模型共识和集中推理层控制来提高系统的鲁棒性、透明度和可信度。


<details>
  <summary>Details</summary>
Motivation: 智能体AI系统虽然功能强大，但自主性增加带来了可解释性、问责制、鲁棒性和治理方面的关键挑战。现有实现通常强调功能和可扩展性，但缺乏理解决策原理或跨智能体交互执行责任的机制。

Method: 提出基于多模型共识和推理层治理的架构：异构LLM和VLM智能体从共享输入上下文独立生成候选输出，暴露不确定性、分歧和替代解释；专用推理智能体对这些输出进行结构化整合，强制执行安全和策略约束，减轻幻觉和偏见，生成可审计、有证据支持的决策。

Result: 在多个真实世界智能体AI工作流中评估该架构，证明共识驱动的推理提高了不同应用领域的鲁棒性、透明度和操作信任度。

Conclusion: 这项工作为设计既自主可扩展又天生负责任和可解释的智能体AI系统提供了实用指导，通过多模型共识和集中推理层控制实现了更好的治理和透明度。
...
<details>
  <summary>Details</summary>
Motivation: 生成式人工智能（GAI）的快速发展依赖于大规模开源数据集，但这些数据集通常采用不受限制且不透明的数据收集方式。现有文献多关注GAI模型的开发和应用，而忽略了数据集创建过程中的伦理和法律考量。此外，数据集在共享、编辑和复制过程中，其来源、合法性和安全性信息往往丢失。
...
Conclusion: CRS框架和相应的开源工具填补了GAI数据集伦理和法律考量的空白，通过数据溯源技术确保数据集透明度、问责制和安全性，促进负责任的数据集创建和使用。
...
TL;DR: 提出一种用于无人机水质监测的主动路径规划方法，通过动态预测光照阴影干扰、改进IFDS算法和MPC框架，在复杂光照环境下实现高成功率避障并提升观测数据质量。
...
Conclusion: 该研究为复杂光照环境下精确的无人机水质监测提供了一种有效的工程解决方案，能够在动态光照干扰条件下实现高成功率避障并提升观测数据质量。
...
TL;DR: PaceForecaster 通过结合副驾驶指令来预测环境地图和子目标，帮助机器人在杂乱环境中更果断地规划路径，同时保持安全，导航性能提升36%。


<details>
  <summary>Details</summary>
Motivation: 在杂乱环境中，运动规划器常常面临安全性和速度之间的权衡，由于遮挡和传感器范围有限导致的不确定性。本研究探讨副驾驶指令是否能帮助机器人更果断地规划路径同时保持安全。

Method: 提出 PaceForecaster 方法，将副驾驶指令整合到局部规划器中。该方法以机器人的局部传感器覆盖范围（Level-1）和提供的副驾驶指令作为输入，预测（i）从 Level-1 可见的所有区域的预测地图（Level-2），以及（ii）在 Level-2 内基于指令条件的子目标。该子目标为规划器提供明确指导，以目标导向的方式利用预测环境。

Result: 将 PaceForecaster 与 Log-MPPI 控制器集成，实验表明使用语言条件的预测和目标在多边形环境中比仅使用局部地图的基线提高了36%的导航性能。

Conclusion: 副驾驶指令能够有效帮助机器人在不确定环境中进行更果断的规划，PaceForecaster 方法通过预测地图和子目标显著提升了导航性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着无人机密度增加，城市空中交通（UAM）预计会出现类似地面交通的拥堵问题，但目前对UAM交通流的基本特性，特别是在真实世界运行条件下的特性，了解甚少。

Method: 提出一个构建UAM交通基本图的通用框架，包括：1）设计两种无人机防撞控制律；2）开发基于仿真的交通生成方法；3）应用Edie定义和交通流理论构建FD；4）在缩比实验平台上使用Crazyflie无人机进行物理实验验证；5）收集数据形成UAMTra2Flow数据集。

Result: 1）地面交通的经典FD结构同样适用于UAM系统；2）物理实验获得的FD曲线与仿真结果存在偏差，凸显了实验验证的重要性；3）将缩比实验结果扩展到真实运行条件，为未来UAM交通系统提供了实用见解。

Conclusion: 该研究首次使用真实世界物理测试数据推导出UAM基本图，证明了理论分析与实验验证相结合的重要性，为理解UAM交通流特性和未来系统设计提供了基础框架和数据集。
...
TL;DR: EVE框架通过零样本视觉语言模型验证器在推理时提升生成式策略性能，无需额外训练


<details>
  <summary>Details</summary>
Motivation: 当前基于生成架构的视觉运动策略在分布偏移下性能下降，缺乏恢复能力，而语言模型领域通过推理时计算扩展显著提升了推理能力。本文旨在探索生成式策略是否也能通过零样本VLM验证器在推理时获得性能提升。

Method: 提出EVE框架：将预训练的生成式策略与多个零样本VLM验证器封装，验证器提出动作改进建议，动作整合器融合验证器输出与基础策略预测，生成最终执行动作。

Result: 在多样化操作任务中，EVE框架持续提升任务成功率，无需额外策略训练。通过消融实验分析了验证器能力和动作整合策略的贡献。

Conclusion: EVE框架为构建可扩展、模块化的生成器-验证器系统提供了实用指南，展示了推理时计算扩展在具身控制中的有效性。
...
Method: 从火星和月球的高分辨率数字地形图像中提取数据，创建了两个大型平面基准数据集；在统一框架下设置了经典和基于学习的路径规划算法，并在提出的数据集和流行规划基准上进行了评估。

Result: 经典算法在挑战性地形（如月球南北极）上平均能达到100%的全局路径规划成功率，解释了NASA在实践中使用这些算法的原因；而基于学习的模型虽然在较简单环境中表现良好，但在行星领域仍难以泛化。

Conclusion: 该研究为行星路径规划研究提供了首个标准化基准，展示了经典算法在行星地形上的优势，同时揭示了学习模型在该领域的泛化挑战，为未来研究奠定了基础。
...
TL;DR: 扩展时空管框架处理动态环境中不确定障碍物的概率时空可达-规避-停留任务，提供实时管合成和概率安全保证


<details>
  <summary>Details</summary>
Motivation: 解决动态环境中存在不确定障碍物时的概率时空可达-规避-停留任务，需要实时控制同时提供形式化概率安全保证

Method: 扩展时空管框架，将管表示为状态空间中随时间变化的球体，基于不确定感知信息在线更新中心点和半径，推导闭式无近似控制律将系统轨迹限制在管内

Result: 开发出模型无关、无近似、无优化的控制器，提供概率规避和有限时间任务完成的正式保证，在移动机器人、无人机和7自由度机械臂上验证有效性和可扩展性

Conclusion: 提出的框架能有效处理动态不确定环境中的概率时空任务，提供形式化安全保证并实现实时高效执行
...
TL;DR: 提出了一种新型螺旋缠绕结构静电层卡滞（HWS-ELJ）可变刚度机制，用于机器人手指设计，通过静电吸附增强层间摩擦实现可调刚度，相比传统平面ELJ具有指数级刚度调节能力。


<details>
  <summary>Details</summary>
Motivation: 传统平面静电层卡滞（ELJ）在刚度调节范围和空间效率方面存在局限，需要一种更高效的可变刚度机制来提升机器人手指的性能和实用性。

Method: 采用螺旋缠绕结构的静电层卡滞机制，利用静电吸引力增强层间摩擦，抑制相对滑动，通过电压控制实现刚度调节，并构建了集成该机制的机器人手指原型。

Result: HWS-ELJ相比传统平面ELJ在相同电极接触面积下能实现显著更大的刚度增强，在同等刚度条件下减少所需空间，实验结果符合理论推导趋势，机器人手指原型成功展示了电压驱动的刚度调制。

Conclusion: HWS-ELJ是一种有效的可变刚度机制，通过螺旋缠绕结构显著提升了刚度调节性能，在机器人手指应用中展示了可行性，为可变刚度机器人设计提供了新思路。
...
TL;DR: 提出了一种轻量级框架，利用SAM 3D Body作为感知骨干，通过MHR表示和物理约束从单目视频恢复世界坐标系人体运动，并重定向到人形机器人


<details>
  <summary>Details</summary>
Motivation: 从单目视频恢复世界坐标系人体运动对于具身智能和机器人学很重要，但现有方法通常需要复杂的SLAM流程或重型时序模型，因此需要更轻量、工程化的解决方案

Method: 使用SAM 3D Body作为冻结感知骨干，采用Momentum HumanRig作为机器人友好的中间表示，通过锁定身份和骨骼尺度参数保证时间一致性，在低维MHR潜在空间进行滑动窗口优化平滑，结合可微分软足地接触模型恢复物理合理的全局根轨迹，最后通过运动学感知的两阶段逆运动学管道重定向到Unitree G1人形机器人

Result: 在真实单目视频上的结果显示，该方法具有稳定的世界轨迹和可靠的机器人重定向性能，表明结构化人体表示结合轻量级物理约束可以从单目输入获得机器人就绪的运动

Conclusion: 结构化人体表示结合轻量级物理约束能够有效地从单目视频恢复世界坐标系人体运动并成功重定向到人形机器人，为具身智能和机器人学提供了实用的工程解决方案
...
TL;DR: 本文提出了一种基于图像中心的记忆框架，通过视觉上下文压缩模块实现长期隐式记忆，结合Qwen2.5-VL导航策略，在终身具身导航任务中实现高效探索和快速目标到达。


<details>
  <summary>Details</summary>
Motivation: 终身具身导航需要智能体在任务间积累、保留和利用空间语义经验，但现有的对象中心记忆方法依赖检测和重建流程，限制了鲁棒性和可扩展性。

Method: 提出图像中心记忆框架，包含高效的视觉上下文压缩模块（基于ViT骨干和DINOv3特征）和Qwen2.5-VL导航策略，支持可配置的压缩率（如16倍压缩，每图像约30个token）。

Result: 在GOAT-Bench和HM3D-OVON基准测试中达到最先进的导航性能，提升陌生环境探索效率，缩短熟悉环境路径，中等压缩率在效率和准确性间取得最佳平衡。
...
TL;DR: 提出一种基于结构诱导探索的多机器人路径规划框架，将空间结构先验集成到蚁群优化中，通过结构约束搜索空间、负载感知目标函数和重叠抑制策略，实现高效且公平的任务分配。


<details>
  <summary>Details</summary>
Motivation: 多机器人路径规划具有组合复杂性，需要在全局效率和机器人间公平任务分配之间取得平衡。传统群体智能方法在小规模实例上有效，但在复杂环境中容易过早收敛且难以扩展。

Method: 提出结构诱导探索框架，将任务空间分布的结构先验集成到蚁群优化搜索过程中：1) 初始化时利用结构先验约束搜索空间；2) 设计强调结构有意义连接的启发式更新规则；3) 引入负载感知目标函数平衡总行程和个体工作量；4) 采用显式重叠抑制策略确保任务分配明确且平衡。

Result: 在多种基准场景（不同实例规模和机器人配置）上验证，结果显示在路径紧凑性、稳定性和工作量分配方面相比代表性元启发式基线方法有持续改进。

Conclusion: 该方法不仅提升了性能，还提供了一个可扩展且可解释的框架，可应用于物流、监控和搜救等需要可靠大规模协调的应用场景。
...
TL;DR: MAction-SocialNav是一个用于社交合规导航的高效视觉语言模型，通过处理动作模糊性来生成多种合理行动，使用元认知提示增强推理能力，并在多动作社交导航数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有社交导航方法通常假设单一正确动作，无法处理现实世界中的社交不确定性。社交规范常常模糊不清，同一场景下可能有多个同样可接受的动作选择，需要能够处理这种模糊性的方法。

Method: 提出MAction-SocialNav模型，采用元认知提示方法增强推理能力，构建包含789个样本的多动作社交导航数据集（含室内外环境、不同人群密度和双人标注），设计五个评估指标来衡量决策精度、安全性和多样性。
...
Conclusion: MAction-SocialNav通过处理动作模糊性，在社交推理性能上表现出色，同时保持高效率，展示了在现实世界人机导航中的潜力。
...
### [114] [MoonBot: Modular and On-Demand Reconfigurable Robot Toward Moon Base Construction](https://arxiv.org/abs/2512.21853)
...
TL;DR: MoonBot是一种模块化可重构机器人系统，专为月球探索设计，能在严格的质量限制下适应不同环境条件和任务需求，通过模拟月球基础设施建设的里程碑任务验证了概念可行性。


<details>
  <summary>Details</summary>
Motivation: 月球表面探索和开发具有重要战略意义，机器人对于探索未知地形、利用当地资源以及建设未来人类栖息地至关重要。需要开发能在严格质量限制下适应多变环境和任务需求的机器人系统。

Method: 设计了模块化可按需重构的机器人系统（MoonBot），详细介绍了其设计和开发过程。通过初步现场演示验证概念，执行模拟建立月球基础设施的里程碑任务，包括土木工程操作、基础设施组件运输部署以及与充气模块的辅助操作。

Result: 成功验证了MoonBot的概念可行性，系统能够执行模拟月球基础设施建设的各项任务。测试过程中总结了连接器设计等方面的经验教训，为未来月球任务中模块化机器人系统的发展提供了宝贵见解。

Conclusion: MoonBot作为一种模块化可重构机器人系统，在适应月球探索的严格约束条件方面表现出潜力。通过测试获得的经验教训，特别是连接器设计的改进，将推动未来月球任务中模块化机器人系统的进一步发展。
...
### [115] [Optimal Trajectory Planning for Orbital Robot Rendezvous and Docking](https://arxiv.org/abs/2512.21882)
*Kenta Iizuka,Akiyoshi Uchida,Kentaro Uno,Kazuya Yoshida*

Main category: cs.RO

TL;DR: 提出一种基于非线性优化的轨迹规划方法，用于空间碎片清除任务中接近翻滚目标的近距离交会，引入动态禁区球体确保安全接近，并开发了考虑实际约束的ON/OFF推力器控制策略。


<details>
  <summary>Details</summary>
Motivation: 空间碎片清除任务中，使用机械臂的服务卫星需要安全接近翻滚的碎片目标，这是一个关键挑战。需要解决如何在二维平面内将自由漂浮、旋转的碎片物体带入机械臂工作空间的问题。

Method: 提出基于非线性优化的轨迹规划方法，引入动态禁区球体根据接近条件自适应调整，允许更近且更安全地接近目标。同时开发了使用离散ON/OFF推力器的控制策略来复现优化轨迹，考虑了实际实现约束。

Result: 该方法能够规划出安全接近翻滚空间碎片的轨迹，通过动态禁区球体机制实现了更近距离的接近，同时控制策略能够有效复现优化轨迹，考虑了实际工程实现的需求。

Conclusion: 提出的轨迹规划和控制方法为空间碎片清除任务中的安全接近操作提供了有效解决方案，动态禁区球体概念和考虑实际约束的控制策略增强了系统的实用性和安全性。
...
### [116] [Online Inertia Parameter Estimation for Unknown Objects Grasped by a Manipulator Towards Space Applications](https://arxiv.org/abs/2512.21886)
*Akiyoshi Uchida,Antonine Richard,Kentaro Uno,Miguel Olivares-Mendez,Kazuya Yoshida*

Main category: cs.RO

TL;DR: 提出一种用于自由浮动基座机器人的在线惯性参数估计方法，通过动量守恒原理扩展现有方法，适用于空间机器人操作未知目标物体


<details>
  <summary>Details</summary>
Motivation: 在空间机器人操作中，了解被抓取物体的惯性参数对于动力学感知操作至关重要，特别是在自由浮动基座的情况下

Method: 扩展现有在线识别方法，通过结合动量守恒原理，使其适用于自由浮动基座机器人系统

Result: 通过数值仿真验证，估计参数与真实值比较，结果显示在多种场景下能够准确识别惯性参数

Conclusion: 该方法适用于在轨服务和其他空间任务，为空间机器人操作未知物体提供了有效的惯性参数估计解决方案
...
### [117] [Aerial World Model for Long-horizon Visual Generation and Navigation in 3D Space](https://arxiv.org/abs/2512.21887)
...
<details>
  <summary>Details</summary>
Motivation: 现有无人机导航策略通常只优化避障和轨迹平滑等低层目标，缺乏将高层语义信息融入规划的能力，需要弥合这一差距。

Method: 提出ANWM模型，基于4-DoF无人机轨迹训练，引入物理启发的未来帧投影模块，将过去帧投影到未来视点以提供粗略几何先验，减少长距离视觉生成的表示不确定性。

Result: ANWM在长距离视觉预测方面显著优于现有世界模型，并提高了无人机在大规模环境中的导航成功率。

Conclusion: ANWM通过结合语义理解和物理先验，为无人机导航提供了更智能的决策能力，实现了高层语义与低层导航目标的统一。
...
### [118] [Flexible Multitask Learning with Factorized Diffusion Policy](https://arxiv.org/abs/2512.21898)
*Chaoqi Liu,Haonan Chen,Sigmund H. Høeg,Shaoxiong Yao,Yunzhu Li,Kris Hauser,Yilun Du*

Main category: cs.RO

TL;DR: 提出一种模块化扩散策略框架，将复杂的机器人动作分布分解为多个专用扩散模型的组合，每个模型捕捉行为空间的不同子模式，从而更有效地拟合多任务学习中的复杂动作分布。


<details>
  <summary>Details</summary>
Motivation: 多任务学习中机器人动作分布具有高度多模态和多样性，现有单一模型往往难以有效拟合这些复杂分布，存在欠拟合问题且缺乏灵活适应能力。

Method: 引入模块化扩散策略框架，将复杂动作分布分解为多个专用扩散模型的组合，每个模型捕捉行为空间的不同子模式，通过模块化结构实现灵活的策略适应。

Result: 在仿真和真实世界机器人操作场景中，该方法在性能上持续优于现有的模块化和单一模型基线方法。

Conclusion: 模块化扩散策略框架能够有效分解复杂动作分布，提高策略拟合效果，同时通过模块化结构实现灵活适应新任务并减轻灾难性遗忘问题。
...
### [119] [StereoVLA: Enhancing Vision-Language-Action Models with Stereo Vision](https://arxiv.org/abs/2512.21970)
*Shengliang Deng,Mi Yan,Yixin Zheng,Jiayi Su,Wenhao Zhang,Xiaoguang Zhao,Heming Cui,Zhizheng Zhang,He Wang*

Main category: cs.RO

TL;DR: StereoVLA是一个利用立体视觉几何线索的视觉-语言-动作模型，通过几何-语义特征提取模块和交互区域深度估计任务，在立体视觉设置下显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 立体相机模拟人类双眼视觉，提供丰富的空间线索，这对精确的机器人操作至关重要。然而，在视觉-语言-动作模型中，立体视觉的应用仍然未被充分探索。

Method: 提出StereoVLA模型，包含：1）几何-语义特征提取模块，利用视觉基础模型提取和融合两种关键特征：从立体视图差异中提取几何特征用于空间感知，从单目视图中提取语义丰富特征用于指令跟随；2）辅助的交互区域深度估计任务，进一步增强空间感知并加速模型收敛。

Result: 大量实验表明，该方法在立体视觉设置下的多样化任务中大幅优于基线方法，并且对相机姿态变化表现出强大的鲁棒性。

Conclusion: StereoVLA成功地将立体视觉的几何线索整合到视觉-语言-动作模型中，显著提升了空间感知能力和任务性能，为机器人操作提供了更精确的视觉理解。
...
### [120] [Bab_Sak Robotic Intubation System (BRIS): A Learning-Enabled Control Framework for Safe Fiberoptic Endotracheal Intubation](https://arxiv.org/abs/2512.21983)
*Saksham Gupta,Sarthak Mishra,Arshad Ayub,Kamran Farooque,Spandan Roy,Babita Gupta*

Main category: cs.RO

TL;DR: 本文提出BRIS机器人插管系统，整合可操控支气管镜、独立气管导管推进机制和摄像头增强口器，通过学习型闭环控制和单目深度估计实现安全、直观的插管操作。


<details>
  <summary>Details</summary>
Motivation: 现有机器人插管系统主要关注气道导航，缺乏对气管导管推进的集成控制和相对于隆突的客观深度验证。气管插管失败或位置不当会导致严重并发症，需要更安全、更一致的临床兼容系统。

Method: BRIS系统整合四向可操控纤维支气管镜、独立气管导管推进机制和摄像头增强口器。采用学习型闭环控制框架，利用实时形状感知将操纵杆输入映射到支气管镜尖端在笛卡尔空间的运动。使用单目内窥镜深度估计对气道区域进行分类，提供可解释的解剖感知引导。

Result: 在高保真气道模型上进行验证，包括标准和困难气道配置，系统展示了可靠的导航和可控的导管放置性能。

Conclusion: BRIS系统是实现更安全、更一致、临床兼容的机器人气道管理的重要一步，通过集成控制和客观深度验证提高了插管操作的安全性和可靠性。
