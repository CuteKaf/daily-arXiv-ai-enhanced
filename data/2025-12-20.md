...
TL;DR: 提出两阶段生成式数据增强框架，结合基于规则的口罩变形和GAN无配对图像转换，生成逼真戴口罩人脸样本，超越纯合成变换


<details>
  <summary>Details</summary>
Motivation: 数据稀缺和分布偏移是戴口罩人脸检测和识别的主要挑战，需要生成逼真的戴口罩人脸样本以增强训练数据

Method: 两阶段框架：1) 基于规则的口罩变形；2) 使用GAN的无配对图像到图像转换。引入非口罩保留损失和随机噪声注入来稳定训练并增强样本多样性

Result: 相比纯基于规则的变形方法，该方法在质量上获得一致改进，并补充了现有GAN方法如IAMGAN。实验验证了所提组件的有效性
...
TL;DR: JARVIS是一个受JEPA启发的自监督视觉增强框架，用于提升多模态大语言模型的视觉理解能力，通过整合I-JEPA学习范式来减少对文本监督的依赖。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在基础视觉推理任务上表现有限，主要因为：1）视觉理解主要从文本描述中学习，而文本描述是主观且不完整的监督信号；2）多模态指令调优规模相对较小，导致模型过度依赖语言先验而忽视视觉细节。
...
Conclusion: JARVIS通过自监督视觉增强有效解决了MLLMs过度依赖语言先验的问题，提高了视觉理解能力，为多模态模型的发展提供了新思路。
...
TL;DR: 本文提出了稀疏接地视觉导航任务和CityNav基准，用于评估多模态大语言模型在真实城市环境中的顺序决策能力，并提出了VoP方法提升导航成功率。


<details>
  <summary>Details</summary>
Motivation: 当前评估基准主要面向语言中心或模拟环境，缺乏对实际复杂场景中知识密集型推理能力的测试，需要建立能够评估多模态大语言模型在真实世界环境中顺序决策能力的基准。

Method: 提出稀疏接地视觉导航任务，并构建CityNav基准，涵盖四个全球城市，要求智能体仅依赖视觉输入和多模态推理进行导航。为解决现有方法表现不佳的问题，提出了路径语言化方法，通过显式构建认知地图来增强导航能力。

Result: 实验表明，当前最先进的多模态大语言模型和标准推理技术在CityNav基准上表现显著不佳，而提出的VoP方法能够大幅提升导航成功率。

Conclusion: CityNav基准揭示了多模态大语言模型在真实世界知识密集型导航任务中的局限性，VoP方法通过显式构建认知地图有效提升了导航性能，为开发更强大的具身智能体提供了重要方向。
...
<details>
  <summary>Details</summary>
Motivation: 受人类四维感知和推理能力启发，希望为视觉语言模型提供类似的结构化内部表示，使其能够编码语义、空间布局和时间动态，从而支持回忆过去事件、推断未观察状态和上下文相关推理。

Method: R4框架持续构建4D知识数据库，将对象级语义描述锚定在度量空间和时间中，形成持久的世界模型。推理时，自然语言查询被分解为语义、空间和时间键来检索相关观察，然后整合到VLM的推理中。

Result: 在具身问答和导航基准测试中，R4相比基线方法在时空信息检索和推理方面有显著提升，展示了在动态环境中进行4D推理的新范式。

Conclusion: R4框架为视觉语言模型提供了无需训练的四维时空检索增强推理能力，通过结构化终身记忆实现了更好的动态环境理解和推理，推动了具身4D推理的新范式发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然能力不断增强，但其感知能力缺乏系统表征。大多数模型家族在扩展语言组件时重复使用几乎相同的视觉编码器，这引发了一个关键问题：模型进步是源于真正的视觉基础能力，还是仅仅依赖互联网规模的文本世界知识。现有评估方法过于强调最终任务准确率，忽视了鲁棒性、归因保真度和受控扰动下的推理能力。

Method: 提出了"感知观测站"框架，通过多个垂直维度系统评估MLLMs：1）简单视觉任务（如人脸匹配和视觉文本理解）；2）局部到全局理解（如图像匹配、网格指向游戏和属性定位）。每个垂直维度都实例化为包含人脸和文字的真实数据集，并通过基于像素的增强和基于扩散的风格化幻觉进行系统性扰动。

Result: 该框架超越了排行榜准确率，能够深入分析MLLMs在扰动下如何保持感知基础和关系结构，为分析当前和未来模型的优势和弱点提供了原则性基础。

Conclusion: 感知观测站为系统评估多模态大语言模型的感知能力提供了一个全面框架，填补了现有评估方法的空白，有助于更准确地理解模型进步的本质——是真正的视觉基础能力提升，还是仅仅依赖文本知识。
...
TL;DR: CAMP-VLM是一个基于视觉语言模型的框架，通过结合视觉上下文特征和场景图的空间感知，从第三人称视角预测多人类行为，在合成和真实数据上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单人类场景的自我中心视角行为预测，但许多机器人应用需要从第三人称视角理解多人类行为，目前缺乏适合的数据集和方法。
...
TL;DR: VLM-based检测器在少样本多光谱目标检测中表现出色，显著优于专门的多光谱模型，并在全监督设置下达到竞争性或更优结果


<details>
  <summary>Details</summary>
Motivation: 多光谱目标检测在自动驾驶和监控等安全敏感应用中至关重要，但标注数据稀缺限制了深度检测器的训练。文本类别信息可作为有价值的语义监督来源，而视觉语言模型在计算机视觉中的成功激发了其在少样本多光谱检测中的应用探索

Method: 将两种代表性的VLM检测器（Grounding DINO和YOLO-World）适配到多光谱输入，并提出有效的机制来整合文本、视觉和热模态信息

Result: 在两个流行的多光谱图像基准（FLIR和M3FD）上的实验表明，VLM检测器在少样本场景中表现出色，显著优于使用可比数据训练的专业多光谱模型，在全监督设置下也达到竞争性或更优结果
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型被广泛提议作为视觉识别任务的通用解决方案，但其在农业决策支持中的可靠性尚未得到充分理解，需要系统评估其在农业分类任务中的表现。

Method: 在AgML集合的27个农业分类数据集上（涵盖162个类别，包括植物病害、害虫和损害、植物和杂草物种识别）对多样化的开源和闭源视觉语言模型进行基准测试，比较零样本性能与专用监督模型（YOLO11），并分析不同提示策略（多项选择vs开放回答）和评估方法的影响。

Result: 零样本视觉语言模型在所有任务中都显著低于专用监督基线；最佳闭源模型（Gemini-3 Pro）在多项选择提示下达到约62%平均准确率，而开放回答提示性能更低（通常低于25%）；LLM语义判断可提高开放回答准确率并改变模型排名；开源模型中Qwen-VL-72B表现最佳；植物和杂草物种分类比害虫和损害识别更容易。
...
TL;DR: 提出一种从初始图像和机器人关节状态生成视频-动作对的方法，通过自动提供动作标签解决视频扩散模型缺乏动作标注的问题，使机器人策略学习能充分利用视频数据。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要问题：两阶段流水线限制了跨模态信息紧密耦合，或者依赖单模态扩散模型适应联合分布而无法充分利用预训练视频知识。视频数据缺乏动作标注限制了其在机器人策略学习中的应用。
...
TL;DR: 提出3D多通道patch扩散模型，从单壳低角分辨率扩散MRI预测多壳高角分辨率纤维取向分布，通过FOD-patch适配器、体素级条件协调模块和SH注意力模块提升性能


<details>
  <summary>Details</summary>
Motivation: 单壳低角分辨率扩散MRI估计纤维取向分布精度有限，而多壳高角分辨率扩散MRI需要长时间扫描限制了应用。扩散模型在基于低分辨率预测高分辨率方面有潜力，但由于FOD中球谐系数数量庞大，高效生成高分辨率FOD具有挑战性

Method: 提出3D多通道patch扩散模型，设计FOD-patch适配器引入先验脑解剖知识进行更高效的基于patch的学习；引入体素级条件协调模块增强模型的全局理解；设计SH注意力模块有效学习球谐系数的复杂相关性

Result: 实验结果表明，该方法在HAR-FOD预测方面实现了最佳性能，优于其他最先进方法

Conclusion: 提出的3D多通道patch扩散模型能够有效从低分辨率扩散MRI预测高分辨率纤维取向分布，通过创新的适配器、协调模块和注意力机制解决了球谐系数数量庞大带来的挑战
...
TL;DR: 提出Auto-Vocabulary 3D目标检测(AV3DOD)，无需用户指定类别，自动生成检测对象的类别名称，通过语义评分评估生成质量，在ScanNetV2和SUNRGB-D数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D目标检测方法虽然在训练时能检测未见类别，但仍需用户在训练和推理时指定类别。作者希望实现完全自动化的3D目标检测，无需任何用户输入就能自动生成检测对象的类别名称。

Method: 提出AV3DOD框架：1) 使用2D视觉语言模型通过图像描述生成丰富的语义候选；2) 生成伪3D边界框；3) 在特征空间进行语义扩展。引入语义评分(SS)来评估生成类别名称的质量。

Result: 在ScanNetV2和SUNRGB-D数据集上取得最先进性能：定位(mAP)和语义质量(SS)均达到SOTA。在ScanNetV2上，相比SOTA方法CoDA提升3.48 mAP，SS相对提升24.5%。

Conclusion: AV3DOD实现了完全自动化的3D目标检测，无需用户指定类别，能够自动生成高质量的语义类别名称，在定位精度和语义质量方面均优于现有方法。
...
Conclusion: 提出的基于准直仪闪烁星点图案的事件相机标定方法有效解决了长距离高精度测量需求，为事件相机的几何标定提供了更优解决方案
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在视频生成中计算成本高昂，需要高效的加速解决方案来降低推理时间，同时保持生成质量。

Method: 采用三种主要技术：1) 注意力加速（低比特SageAttention和可训练稀疏线性注意力）；2) 步数蒸馏（rCM方法）；3) W8A8量化（将模型参数和激活量化为8位）。此外还包括其他工程优化。

Result: 在多个视频生成模型上实验表明，即使在单张RTX 5090 GPU上也能实现100-200倍的加速，同时保持可比的视频质量。

Conclusion: TurboDiffusion框架显著提升了视频扩散模型的推理效率，为实时视频生成应用提供了可行的解决方案。
...
TL;DR: 提出了一种基于准直仪系统的相机标定新方法，利用准直仪的光学几何特性建立角度不变性约束，将标定目标与相机之间的6自由度相对运动简化为3自由度纯旋转运动，并提出了多图像线性求解器、两图像最小求解器以及单图像标定算法。
...
Method: 设计准直仪系统，利用其独特的光学几何特性建立角度不变性约束，证明标定目标与相机之间的相对运动符合球面运动模型。基于此约束，提出了多图像的闭式线性求解器、两图像的最小求解器，以及无需相机运动的单准直仪图像标定算法。

Result: 在合成和真实世界实验中评估了方法性能，验证了使用准直仪系统进行标定的可行性，并证明该方法优于现有的基线方法。实验结果表明该方法能够实现灵活快速的相机标定。

Conclusion: 本文提出的基于准直仪系统的相机标定方法通过角度不变性约束简化了标定过程，降低了运动要求，提供了灵活快速的标定解决方案，在精度和实用性方面优于传统方法。
...
TL;DR: C-DGPA是一种针对视觉语言模型的类中心双对齐生成提示适应方法，通过双分支架构同时优化边际分布对齐和条件分布对齐，解决无监督域适应中的领域差异问题。


<details>
  <summary>Details</summary>
Motivation: 现有提示调优策略主要对齐边际分布，但忽略了条件分布差异，导致类原型错位和语义判别性下降等关键问题。需要同时解决两种分布差异来提升视觉语言模型在无监督域适应任务中的性能。

Method: 提出C-DGPA方法：1）边际分布对齐分支采用动态对抗训练框架来弥合边际分布差异；2）条件分布对齐分支引入类映射机制，通过标准化语义提示理解和防止源域过度依赖来对齐条件分布差异。双分支架构通过协同优化将领域知识整合到提示学习中。

Result: 在OfficeHome、Office31和VisDA-2017等基准数据集上进行广泛实验，C-DGPA在所有基准上都取得了新的最先进结果，验证了其优越性。

Conclusion: C-DGPA通过双对齐策略有效整合领域知识到提示学习中，确保获得领域不变且语义可判别的表示，显著提升了视觉语言模型在无监督域适应任务中的性能。
...
TL;DR: 事件相机在自动驾驶中相比传统相机能更好地应对昼夜光照差异带来的域差距问题，无需额外调整即可在不同光照条件下保持性能一致性。


<details>
  <summary>Details</summary>
Motivation: 传统相机在端到端驾驶中面临域差距问题，特别是当训练数据条件与部署环境不匹配时性能会大幅下降。昼夜光照差异是典型的域差距问题，需要寻找更鲁棒的传感器解决方案。

Method: 提出使用事件相机替代传统相机，事件相机基于异步像素级亮度变化检测，对光照变化具有更强的鲁棒性。通过对比事件相机与传统灰度帧相机在不同光照条件下的性能表现来验证其优势。

Result: 事件相机在不同光照条件下保持更一致的性能，其域偏移惩罚通常与灰度帧相当或更小，在跨域场景中提供更优越的基线性能。

Conclusion: 事件相机是解决自动驾驶中昼夜光照域差距问题的有效替代方案，能够在不需额外调整的情况下保持跨域性能一致性，优于传统相机方法。
...
TL;DR: Avatar4D是一个可迁移的合成人体运动数据集生成管道，专门针对特定领域应用。该研究聚焦体育领域，创建了Syn2Sport大规模合成数据集，验证了合成数据在监督学习、零样本迁移和跨领域泛化方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有的人体运动数据集主要关注日常动作，缺乏对特定领域（如体育）的针对性，且控制灵活性有限。需要一种能够生成可定制、可控制、无需人工标注的合成数据集方法，以解决特定领域运动理解面临的独特挑战。

Method: 提出了Avatar4D管道，能够精细控制身体姿态、外观、相机视角和环境背景。针对体育领域创建了Syn2Sport大规模合成数据集，涵盖棒球和冰球等运动，包含高保真4D（随时间变化的3D几何）人体运动序列。

Result: 在Syn2Sport上对多个最先进的姿态估计模型进行基准测试，证明了合成数据在监督学习、零样本迁移到真实数据以及跨运动泛化方面的有效性。评估了生成合成数据与真实数据集在特征空间中的对齐程度。

Conclusion: Avatar4D展示了在不依赖特定领域真实数据的情况下，生成可扩展、可控制、可迁移的人体数据集用于多样化领域特定任务的潜力，为特定领域应用提供了有效的合成数据解决方案。
...
TL;DR: VALOR提出了一种基于强化学习的医学视觉语言模型后对齐框架，通过GRPO优化方法提升放射学报告生成的视觉基础和临床准确性。


<details>
  <summary>Details</summary>
Motivation: 当前大型医学视觉语言模型在生成放射学报告时面临视觉基础不足和临床准确性不够的问题，现有方法依赖大量标注数据、昂贵的特定任务偏好数据或检索方法，但未能有效缓解视觉与语言表示跨模态对齐不良导致的幻觉问题。
...
TL;DR: 该论文提出了一种基于信息几何的3D点云形状分析方法，通过将点云表示为统计流形上的高斯混合模型，并引入具有理论上界和下界保证的修正对称KL散度来解决传统方法的数值不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 三维点云在多个领域有重要应用，但点云比较面临挑战：点云的非结构化特性、复杂几何表面表示困难。传统几何度量（如Hausdorff和Chamfer距离）无法捕捉全局统计结构且对异常值敏感，现有GMM的KL散度近似可能产生无界或数值不稳定的值。

Method: 1. 将点云表示为统计流形上的高斯混合模型；2. 证明GMM空间形成统计流形；3. 提出修正对称KL散度，具有理论上界和下界保证，确保所有GMM比较的数值稳定性。

Result: 在人体姿态识别（MPI-FAUST数据集）和动物形状比较（G-PCD数据集）的实验中，MSKL提供了稳定且单调变化的数值，直接反映了几何变化，性能优于传统距离和现有KL近似方法。

Conclusion: 该研究提出的信息几何框架为3D点云形状分析提供了数值稳定且理论保证的方法，通过GMM表示和修正对称KL散度，有效解决了传统点云比较方法的局限性。
...
Method: 使用奇异值分解和低秩矩阵近似进行图像压缩，通过相对Frobenius误差和压缩比评估性能，应用于灰度图和彩色图验证通用性。

Result: 低秩近似图像视觉上与原图相似，但压缩效率始终不如JPEG、JPEG2000和WEBP等标准格式。在低误差容忍水平下，SVD压缩表示甚至可能超过原图大小。

Conclusion: SVD低秩矩阵近似方法在实际图像压缩中不具竞争力，无法与行业标准编解码器相比，不适合实际应用。
...
TL;DR: ARMFlow是一个基于MeanFlow的自回归框架，用于3D人体反应生成，解决了高运动保真度、实时推理和在线场景自回归适应性三大挑战，通过因果上下文编码器和MLP速度预测器实现，并引入Bootstrap Contextual Encoding减少误差累积。
...
Method: 提出ARMFlow框架，基于MeanFlow的自回归架构，包含因果上下文编码器和MLP速度预测器。引入Bootstrap Contextual Encoding训练方法，使用生成的历史而非真实历史进行编码，以减轻自回归生成中的误差累积。还提出了离线变体ReMFlow。

Result: 单步在线生成在InterHuman和InterX数据集上比现有在线方法在FID指标上提升超过40%，同时仅使用部分序列条件就能匹配离线最先进方法的性能。ReMFlow离线变体在离线方法中达到最先进性能且推理速度最快。
...
<details>
  <summary>Details</summary>
Motivation: 多视角人群计数能解决大范围场景中的严重遮挡问题，但多视角数据收集和标注困难，现有数据集多视角帧和场景数量有限。为解决数据有限问题，需要开发半监督方法减少对多视角标注数据的依赖。
...
Conclusion: 通过排序多视角融合模型的预测结果或模型不确定性，提出的半监督框架能够有效利用有限标注数据，解决多视角人群计数中的数据稀缺问题，提升计数准确性。
...
### [23] [GeoPredict: Leveraging Predictive Kinematics and 3D Gaussian Geometry for Precise VLA Manipulation](https://arxiv.org/abs/2512.16811)
*Jingjing Qian,Boyao Han,Chen Shi,Lei Xiao,Long Yang,Shaoshuai Shi,Li Jiang*

Main category: cs.CV

TL;DR: GeoPredict是一个几何感知的视觉-语言-动作模型框架，通过预测性运动学和几何先验增强连续动作策略，在需要精确3D推理的机器人操作任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型在机器人操作中表现出良好的泛化能力，但大多是反应式和2D中心的，在需要精确3D推理的任务中可靠性不足。

Method: GeoPredict引入了两个预测模块：轨迹级模块编码运动历史并预测机器人手臂的多步3D关键点轨迹；预测性3D高斯几何模块预测工作空间几何，并沿未来关键点轨迹进行轨迹引导细化。这些模块仅作为训练时监督，推理时只需轻量级查询令牌。
...
### [24] [OPENTOUCH: Bringing Full-Hand Touch to Real-World Interaction](https://arxiv.org/abs/2512.16842)
...
<details>
  <summary>Details</summary>
Motivation: 人类手是我们与物理世界的主要交互界面，但自我中心视角感知很少知道何时、何地以及以多大力度接触物体。现有可穿戴触觉传感器稀缺，且没有野外数据集能对齐第一人称视频与全手触觉信息。

Method: 构建OpenTouch数据集，包含5.1小时同步的视频-触觉-姿态数据，以及2900个经过筛选的片段和详细文本标注。基于此数据集引入检索和分类基准测试，探索触觉如何支撑感知和行动。

Result: 触觉信号为抓握理解提供了紧凑而强大的线索，增强了跨模态对齐，并且可以从野外视频查询中可靠地检索。数据集和基准测试的发布旨在推动多模态自我中心感知、具身学习和接触丰富的机器人操作。

Conclusion: OpenTouch填补了视觉感知与物理交互之间的空白，通过提供首个野外环境下的自我中心视角全手触觉数据集，为多模态感知、具身学习和机器人操作研究提供了重要资源。
...
TL;DR: 该论文提出了TextEditBench，一个专门评估图像中文本编辑能力的基准测试，强调推理密集型编辑场景，并引入语义期望(SE)新评估维度。


<details>
  <summary>Details</summary>
Motivation: 文本渲染已成为视觉生成中最具挑战性的前沿领域，但图像中的文本编辑仍然很大程度上未被探索。现有方法需要生成清晰字符的同时保持语义、几何和上下文一致性，这提出了独特挑战。

Method: 提出了TextEditBench基准测试，专注于图像中的文本中心区域，强调需要理解物理合理性、语言意义和跨模态依赖的推理密集型编辑场景。引入了语义期望(SE)新评估维度，衡量模型在文本编辑过程中保持语义一致性、上下文连贯性和跨模态对齐的推理能力。

Result: 对最先进的编辑系统进行广泛实验发现，当前模型虽然能够遵循简单的文本指令，但在上下文依赖推理、物理一致性和布局感知集成方面仍然存在困难。
...
### [26] [Flowing from Reasoning to Motion: Learning 3D Hand Trajectory Prediction from Egocentric Human Interaction Videos](https://arxiv.org/abs/2512.16907)
...
TL;DR: 提出EgoMAN数据集和模型，用于解决3D手部轨迹预测中语义监督与运动解耦、推理与动作弱关联的问题


<details>
  <summary>Details</summary>
Motivation: 现有3D手部轨迹预测研究存在两个主要问题：1）数据集将运动与语义监督解耦；2）模型对推理和动作的关联较弱。需要同时解决数据和方法层面的限制

Method: 1）创建EgoMAN数据集：大规模第一人称数据集，包含219K个6DoF轨迹和3M结构化QA对，支持语义、空间和运动推理；2）提出EgoMAN模型：推理到运动框架，通过轨迹令牌接口连接视觉语言推理和运动生成，采用渐进式训练对齐推理与运动动力学

Result: 方法能够生成准确且阶段感知的轨迹，在真实场景中具有良好的泛化能力

Conclusion: 通过同时解决数据集和方法层面的限制，提出的EgoMAN框架实现了语义推理与3D手部轨迹预测的有效结合，为交互阶段感知的轨迹预测提供了新思路
...
### [27] [MomaGraph: State-Aware Unified Scene Graphs with Vision-Language Model for Embodied Task Planning](https://arxiv.org/abs/2512.16909)
...
<details>
  <summary>Details</summary>
Motivation: 家庭移动机器人需要同时进行导航和操作，现有场景表示方法存在以下问题：1）空间关系和功能关系分离；2）将场景视为静态快照，缺乏对象状态和时序更新；3）忽视当前任务最相关的信息。

Method: 提出MomaGraph统一场景表示，整合空间-功能关系和部件级交互元素。创建MomaGraph-Scenes大规模数据集和MomaGraph-Bench评估套件。基于此开发MomaGraph-R1，一个7B视觉语言模型，通过强化学习训练，采用Graph-then-Plan框架进行零样本任务规划。

Result: 模型在基准测试中达到71.6%准确率，比最佳基线提升11.4%。在公开基准上表现出良好的泛化能力，并能有效迁移到真实机器人实验中。

Conclusion: MomaGraph为具身智能体提供了统一的场景表示方法，通过整合空间-功能关系和任务相关信息，显著提升了家庭环境中移动机器人的场景理解和任务规划能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决多标签遥感图像检索中的三大挑战：1) 土地覆盖类别的语义重叠问题；2) 高度不平衡的标签分布；3) 复杂的类别间共现模式

Method: 提出多标签自适应对比学习(MACL)，整合三个关键技术：标签感知采样、频率敏感加权和动态温度缩放，以实现常见和稀有类别的平衡表示学习

Result: 在三个基准数据集(DLRSD、ML-AID、WHDLD)上的广泛实验表明，MACL始终优于基于对比损失的基线方法，有效缓解语义不平衡问题，在大规模遥感档案中提供更可靠的检索性能

Conclusion: MACL作为对比学习的扩展，成功解决了多标签遥感图像检索中的语义不平衡问题，代码和预训练模型将在接受后发布
...
Method: 提出PixelArena基准，使用语义分割任务来客观评估多模态大语言模型的细粒度图像生成能力。通过零样本设置测试模型生成语义掩码的能力，并进行定性和定量比较分析。

Result: 发现Gemini 3 Pro Image展现出新兴的图像生成能力，能够在零样本设置下生成高保真语义掩码，表现出前所未有的视觉智能和在新图像生成任务中的真正泛化能力。同时提供了与其他模型的比较结果和失败案例。

Conclusion: 该研究不仅标志着该领域的令人兴奋的进展，还为未来与多模态、推理、可解释性和基准测试相关的研究提供了见解。语义分割任务为评估图像生成模型的细粒度能力提供了有价值的基准。
...
<details>
  <summary>Details</summary>
Motivation: 无人机搭载多种传感器跟踪测量目标时，在远距离、小交会角、大倾角等有限观测条件下，最小二乘估计的设计矩阵列向量存在严重多重共线性，导致病态问题、稳定性差和鲁棒性低。
...
Conclusion: 基于岭估计的融合定位方法能够有效解决有限观测条件下的多重共线性问题，提高无人机目标定位的精度和鲁棒性，为多传感器融合定位提供了有效解决方案。
...
TL;DR: QUIDS系统通过质量感知的激励机制，在非专用车辆移动众感系统中联合优化覆盖范围和可靠性，显著提升信息质量。


<details>
  <summary>Details</summary>
Motivation: 解决非专用车辆移动众感系统中传感覆盖范围、传感可靠性和车辆动态参与等相互关联的挑战，在预算约束下实现高质量的城市监控。

Method: 提出QUIDS系统，引入聚合传感质量(ASQ)指标量化信息质量，开发相互辅助的信念感知车辆调度算法，在不确定性下估计传感可靠性并分配激励。

Result: 在真实世界部署中，QUIDS将ASQ提升38%（相比无调度场景）和10%（相比最先进方法），并将重建地图误差降低39-74%。

Conclusion: 通过质量感知的激励机制联合优化覆盖范围和可靠性，QUIDS实现了低成本、高质量的城市监控，适用于交通和环境传感等智慧城市场景。
...
Method: 设计了两阶段框架：第一阶段服务器在全局图像上执行推理，利用VLM内部注意力识别感兴趣区域，计算输出令牌的最小熵作为置信度；若最小熵超过预设阈值，则请求边缘设备发送RoI的细节保留局部图像；第二阶段服务器联合利用全局和局部图像进行细化推理。

Result: 在多个VLM架构上的实验表明，该框架显著降低了通信成本，同时保持了推理精度。

Conclusion: 提出的协作边缘到服务器推理框架通过选择性重传策略，有效解决了传统VLM部署中通信成本高和精度损失的问题，实现了通信效率与推理精度的平衡。
...
TL;DR: GMODiff：基于增益图的单步扩散框架，用于多曝光HDR重建，通过增益图编码动态范围扩展，实现100倍加速并减少幻觉


<details>
  <summary>Details</summary>
Motivation: 预训练的潜在扩散模型在低层视觉任务中表现出强大的感知先验，但直接应用于HDR重建面临三个挑战：8位潜在压缩导致的动态范围表示有限、多步去噪的高推理成本、以及生成性质固有的内容幻觉问题

Method: 将HDR重建重新定义为条件引导的增益图估计任务，增益图编码扩展的动态范围但保持与LDR图像相同的位深度；从基于回归的估计初始化去噪过程而非纯噪声，实现单步去噪；利用回归先验引导LDM的去噪过程和潜在解码，抑制幻觉同时保持结构准确性

Result: GMODiff在性能上优于多个最先进方法，且比之前的LDM方法快100倍
...
TL;DR: EverybodyDance：针对多角色动画中身份对应问题的系统性解决方案，通过身份匹配图建模角色关系，确保角色位置交换时的身份一致性


<details>
  <summary>Details</summary>
Motivation: 单角色动画在姿态驱动方面已取得显著进展，但扩展到多角色场景（特别是涉及位置交换时）面临挑战。核心问题在于确保参考帧和生成帧中角色之间的正确身份对应关系

Method: 提出身份匹配图（IMG）将生成帧和参考帧中的角色建模为加权完全二分图的两个节点集；使用掩码查询注意力（MQA）计算边权重量化角色亲和度；将身份对应正确性形式化为图结构度量并在训练中优化；提出身份嵌入引导、多尺度匹配策略和预分类采样等针对性策略

Result: 在身份对应评估基准上的大量实验表明，EverybodyDance在身份对应正确性和视觉保真度方面显著优于现有最先进基线方法

Conclusion: EverybodyDance通过系统化的图建模和优化方法，有效解决了多角色动画中的身份对应问题，为多角色场景下的姿态驱动动画提供了可靠解决方案
...
TL;DR: FVG通过三阶段分解方法改进文本到视频生成：LLM推理重写提示描述初始场景，T2I模型合成高质量锚定帧，视频模型专注动画化，显著提升生成质量并减少70%采样步骤


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频扩散模型虽然能生成视觉上令人印象深刻的结果，但经常无法构建复杂场景或遵循逻辑时间指令。许多错误（包括明显的运动失败）源于模型无法构建语义正确或逻辑一致的初始帧

Method: 提出因子化视频生成（FVG）管道，将文本到视频生成分解为三个专门阶段：1）推理阶段：大型语言模型重写视频提示，仅描述初始场景，解决时间模糊性；2）构图阶段：文本到图像模型从新提示合成高质量、构图正确的锚定帧；3）时间合成阶段：经过微调的视频模型专注于动画化场景并遵循提示

Result: 在T2V CompBench基准测试中达到新的最先进水平，在VBench2上显著改进所有测试模型。视觉锚定允许减少70%的采样步骤而不损失性能，实现采样速度的大幅提升

Conclusion: 因子化视频生成提供了一条简单而实用的路径，实现更高效、稳健和可控的视频合成
...
### [36] [Using Gaussian Splats to Create High-Fidelity Facial Geometry and Texture](https://arxiv.org/abs/2512.16397)
*Haodi He,Jihun Yu,Ronald Fedkiw*

Main category: cs.CV

TL;DR: 该研究提出了一种利用3D神经表示从少量未标定人脸图像重建统一3D人脸模型的方法，基于高斯泼溅技术构建结构化重建，并转换为可用于标准图形管线的纹理空间表示。


<details>
  <summary>Details</summary>
Motivation: 为了解决从少量未标定人脸图像中重建统一且一致的3D人脸表示的问题，同时使重建结果能够直接集成到标准图形管线中，而不需要修改现有资产或渲染流程。

Method: 采用高斯泼溅技术作为基础表示，利用分割标注对齐人脸语义区域，通过软约束将高斯分布与底层三角网格表面关联，构建结构化重建。将高斯泼溅转换为纹理空间中的视点相关神经纹理，并使用可重光照高斯模型分离纹理和光照，获得高分辨率反照率纹理。

Result: 仅需11张图像即可重建中性姿态的人脸模型，无需长视频序列。重建的三角网格表面可直接用于标准图形管线，高斯泼溅可转换为纹理空间表示，实现高视觉保真度且不干扰其他资产。系统支持不同光照条件下的图像训练，并展示了在文本驱动资产创建管线中的应用。

Conclusion: 该方法成功实现了从少量未标定图像重建结构化3D人脸模型，并将高斯泼溅表示无缝集成到标准图形管线中，为高质量人脸资产创建提供了灵活高效的解决方案。
...
### [37] [BrepLLM: Native Boundary Representation Understanding with Large Language Models](https://arxiv.org/abs/2512.16413)
...
Method: 采用两阶段训练：1)跨模态对齐预训练：通过自适应UV采样将Brep转换为图表示，使用分层BrepEncoder提取几何和拓扑特征，通过对比学习将全局token与CLIP文本嵌入对齐；2)多阶段LLM微调：将预训练BrepEncoder集成到LLM中，采用三阶段渐进训练策略（语义映射、LLM微调、混合查询专家增强）。

Result: 构建了包含269,444个Brep-文本问答对的数据集Brep2Text，实验表明BrepLLM在3D物体分类和描述任务上达到了最先进的性能。

Conclusion: BrepLLM是首个能够解析和处理原始Brep数据的框架，成功弥合了结构化3D几何与自然语言之间的模态差距，为LLM处理复杂3D几何数据提供了有效解决方案。
...
### [38] [CountZES: Counting via Zero-Shot Exemplar Selection](https://arxiv.org/abs/2512.16415)
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本物体计数方法存在局限性：基于开放词汇检测器的方法会产生多实例候选，而随机补丁采样方法无法准确界定物体实例。需要一种能精确选择单实例示例的方法来提升零样本计数性能。

Method: 提出CountZES框架，包含三个协同阶段：1) 检测锚定示例(DAE)：精炼开放词汇检测结果以隔离精确的单实例示例；2) 密度引导示例(DGE)：通过密度驱动的自监督范式识别统计一致且语义紧凑的示例；3) 特征共识示例(FCE)：通过特征空间聚类增强视觉一致性。这三个阶段共同产生多样化、互补的示例集。

Result: 在多样化数据集上的实验表明，CountZES在零样本物体计数方法中表现出优越性能，并能有效泛化到自然、航空和医学等多个领域。

Conclusion: CountZES通过渐进式发现多样化示例的三阶段框架，解决了零样本物体计数中的示例选择问题，在保持文本基础、计数一致性和特征代表性的平衡方面表现出色，具有广泛的领域适应性。
...
### [39] [Geometric Disentanglement of Text Embeddings for Subject-Consistent Text-to-Image Generation using A Single Prompt](https://arxiv.org/abs/2512.16443)
*Shangxun Li,Youngjung Uh*

Main category: cs.CV

TL;DR: 提出了一种无需训练的方法，通过几何视角精炼文本嵌入来抑制不需要的语义，解决多帧图像生成中的主题一致性和文本对齐问题。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在生成高质量图像方面表现出色，但在多个输出中难以保持主题一致性，限制了在视觉叙事中的应用。现有方法依赖模型微调或图像条件，计算成本高且需要针对每个主题进行优化。

Method: 提出一种简单有效的无需训练方法，从几何角度解决语义纠缠问题，通过精炼文本嵌入来抑制不需要的语义，避免跨帧嵌入纠缠导致的文本错位。

Result: 大量实验证明，该方法在主题一致性和文本对齐方面显著优于现有基线方法。

Conclusion: 该方法提供了一种无需训练的有效解决方案，能够显著改善多帧图像生成中的主题一致性和文本对齐问题，为视觉叙事应用提供了更好的支持。
...
TL;DR: StageVAR提出了一种针对视觉自回归模型的分阶段加速框架，通过分析不同生成阶段的重要性差异，在保持早期关键阶段完整的同时，对后期细节优化阶段进行剪枝和近似，实现3.4倍加速且质量损失极小。
...
Conclusion: 分阶段感知设计是高效视觉自回归图像生成的强大原则，StageVAR通过系统分析不同生成阶段的重要性差异，实现了显著加速且保持高质量输出。
...
TL;DR: 该研究通过强化学习训练模型获得人类感知-推理能力，实现盲图像质量评估，在预测性能上与SOTA相当，在解释性方面显著优于基线。


<details>
  <summary>Details</summary>
Motivation: 人类通过感知-推理级联评估图像质量，结合感官线索和隐式推理形成自洽判断。现有盲图像质量评估模型缺乏人类式的自洽推理能力。

Method: 1. 收集人类评估数据捕捉感知-推理过程；2. 采用强化学习，以人类标注作为奖励信号指导模型学习人类感知和推理；3. 设计奖励机制使模型能够从自生成描述中推断图像质量，内化自洽推理能力。

Result: 1. 在Pearson和Spearman相关系数等通用指标上，预测性能与最先进BIQA系统相当；2. 在1000+人类标注样本上，ROUGE-1得分达到0.512（基线为0.443），表明模型解释与人类感知-推理链有显著覆盖。

Conclusion: 该方法使模型获得了人类式的感知推理能力，在保持预测性能的同时显著提升了可解释性，是向人类式可解释推理BIQA迈进的重要一步。
...
TL;DR: 该论文提出眼动行为辅助的多模态情感识别（EMER）数据集和EMERT模型，通过结合眼动行为来弥补面部表情识别与真实情感识别之间的差距，提升情感识别的鲁棒性。
...
Method: 1. 构建EMER数据集：采用自发情感诱导范式收集真实情感数据，同时捕获非侵入性眼动行为数据（眼动序列和注视点图）和面部表情视频；2. 设计EMERT模型：利用模态对抗特征解耦和多任务Transformer，将眼动行为作为面部表情的强有力补充进行建模。

Result: 实验引入七种多模态基准协议对EMER数据集进行全面评估。结果显示，EMERT模型显著优于其他最先进的多模态方法，证明了眼动行为建模对鲁棒情感识别的重要性。

Conclusion: 该研究全面分析了眼动行为在情感识别中的重要性，通过弥合面部表情识别与情感识别之间的差距，推动了更鲁棒的情感识别性能研究。EMER数据集和EMERT模型将公开提供。
...
TL;DR: 提出了VenusBench-GD，一个跨平台的双语GUI grounding基准测试，包含大规模数据集、高质量标注流程和分层任务分类，实验显示通用多模态模型在基础任务上已匹敌专用GUI模型，但高级任务仍需专用模型


<details>
  <summary>Details</summary>
Motivation: 现有GUI grounding基准存在数据量不足、领域覆盖窄、平台单一或需要专业领域知识等问题，需要更全面、跨平台的评估框架来推动GUI智能体发展

Method: 1) 构建大规模跨平台基准数据集，覆盖广泛应用、多样UI元素和丰富标注；2) 建立高质量数据标注流程，提升标注准确性；3) 提出分层任务分类法，将grounding分为基础和高级两类，包含6个子任务从互补角度评估模型

Result: 实验发现：通用多模态模型在基础grounding任务上已匹配甚至超越专用GUI模型；而高级任务仍偏向GUI专用模型，但这些模型存在显著过拟合和鲁棒性差的问题

Conclusion: 需要全面、多层次评估框架来准确评估GUI grounding能力，VenusBench-GD为此提供了有效工具，揭示了当前模型的局限性并指明了未来研究方向
...
TL;DR: 该论文提出了一种用于骨架时序动作定位的自监督预训练方法，通过片段判别任务和U形模块融合中间特征来提升帧级定位性能。
...
Conclusion: 提出的自监督预训练方法能有效学习骨架时序动作定位所需的时间敏感特征，在多个数据集和评估协议上表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态模型缺乏内在的3D物体感知能力，限制了它们理解3D场景中空间关系和深度线索的能力，需要开发能够直接定位3D物体并进行空间推理的模型。

Method: 提出N3D-VLM框架，通过深度估计将大规模2D标注提升到3D空间，构建训练数据；模型具备原生3D物体感知能力，可直接基于文本描述在3D空间中定位物体，并进行显式的3D推理。

Result: 在3D定位任务上达到最先进性能，在3D空间推理方面持续超越现有方法；构建的数据集比现有最大的单图像3D检测数据集大6倍以上。

Conclusion: N3D-VLM成功整合了原生3D物体感知与3D感知视觉推理，实现了精确的3D定位和可解释的空间理解，为3D视觉语言理解提供了统一框架。
...
TL;DR: 提出一个动态重建系统，从单目RGB视频输入中输出场景的完整持久重建，包括当前可见和之前观察过的部分，实现跨时间步的完整重建回放。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只能重建当前可见的场景部分，无法保持对之前观察部分的记忆，缺乏对动态场景的完整时空感知能力。

Method: 将场景分解为一组刚性3D基元，通过估计的密集2D对应关系，通过优化管道联合推断这些基元的刚性运动，实现4D场景重建。引入运动外推机制处理不可见物体，使用运动分组技术保持连续性。

Result: 在物体扫描和多物体数据集上，该系统在定量和定性评估中都显著优于现有方法，实现了可回放的3D重建、多物体扫描和物体持久性等能力。

Conclusion: 该系统实现了4D时空感知能力，能够从单目视频中重建动态场景的完整历史，为动态场景理解提供了新的技术框架。
...
TL;DR: 提出基于弱监督学习和Grad-CAM的肺炎分类与定位框架，使用图像级标签替代像素级标注，评估7种预训练模型，在保持高准确率的同时实现可解释性定位。


<details>
  <summary>Details</summary>
Motivation: 胸部X光常用于肺炎诊断，但精确定位肺炎区域需要昂贵的像素级标注。为降低标注成本，研究提出使用图像级标签的弱监督方法，实现肺炎分类和区域定位。

Method: 采用弱监督深度学习框架，结合Grad-CAM生成热力图定位肺炎区域。评估7种预训练模型（包括Vision Transformer），使用焦点损失和患者级数据划分防止数据泄露。

Result: 所有模型分类准确率达96-98%，ResNet-18和EfficientNet-B0表现最佳，MobileNet-V3作为轻量级替代方案。Grad-CAM热力图显示模型聚焦于临床相关肺部区域。

Conclusion: 弱监督可解释模型能有效辅助肺炎筛查，提高AI在放射诊断中的透明度和临床信任度，为低成本、高可解释性的医学影像分析提供可行方案。
...
TL;DR: 提出Causal-Tune方法，通过频域分析识别和分离视觉基础模型中的因果与非因果因素，提升领域泛化语义分割性能
...
Method: 提出Causal-Tune微调策略：1) 使用离散余弦变换提取每层特征的频谱；2) 应用高斯带通滤波器分离因果和非因果成分；3) 引入因果感知可学习令牌在频域中进一步精炼因果成分；4) 通过逆DCT将精炼特征转换回空间域并传递到下一层

Result: 在各种跨域任务上的实验证明了Causal-Tune的有效性。特别是在恶劣天气条件下表现优异，在雪天条件下比基线提高了+4.8% mIoU

Conclusion: 通过因果机制分析视觉基础模型中的特征因素，提出了一种简单有效的微调方法，能够识别和分离因果与非因果因素，从而提升领域泛化语义分割的鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 现有模型要么依赖单个先验扫描，要么使用固定网格时间，要么针对全局标签，这限制了不规则采样下的体素级预测能力。需要一种能够处理多个过去扫描并支持连续时间预测的框架。
...
TL;DR: SkiLa提出了一种新的多模态推理范式，通过让MLLM在推理过程中原生生成连续视觉嵌入（潜在草图标记），实现统一的视觉-文本想象和交互


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉理解任务上表现出色，但在需要视觉想象的场景中表现不足。人类能够在思考过程中形成灵活的视觉-文本想象和交互，而不需要预定义工具包，这是因为人类在大脑的统一空间中构建视觉-文本思考过程。受此启发，考虑到当前MLLM已经在同一特征空间中编码视觉和文本信息，作者认为视觉标记可以无缝插入到文本标记承载的推理过程中。

Method: 提出Sketch-in-Latents（SkiLa）范式，扩展MLLM的自回归能力，使其能够原生生成连续视觉嵌入（潜在草图标记）作为视觉思维。在多步推理过程中，模型动态交替使用文本思考模式（生成文本思考标记）和视觉草图模式（生成潜在草图标记）。提出潜在视觉语义重建机制，确保这些潜在草图标记具有语义基础。

Result: 大量实验表明，SkiLa在视觉中心任务上实现了优越性能，同时在多样化的通用多模态基准测试中表现出强大的泛化能力。

Conclusion: SkiLa通过让MLLM在推理过程中原生生成连续视觉嵌入，实现了统一的视觉-文本推理，为多模态大语言模型提供了更接近人类视觉想象能力的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散模型使用CNN作为基础模块，但卷积操作的局部性限制了模型理解长距离语义信息的能力

Method: 用Swin-transformer块替换U型架构中的CNN块，改进特征提取和图像恢复的非局部建模能力；通过精心选择的文本编码器、有效利用文本嵌入和文本条件融合设计来改进文本-图像对齐；采用自适应时间步搜索策略

Result: 在ImageNet生成基准上达到最先进的FID分数1.37；推理性能提升10%；人类评估者难以区分模型生成图像和人工绘制图像

Conclusion: Yuan-TecSwin通过引入Swin-transformer改进了扩散模型的长距离语义理解能力，在图像生成质量和文本-图像对齐方面取得了显著提升
...
<details>
  <summary>Details</summary>
Motivation: 开发一个轻量级的去雾解决方案，能够在移动和嵌入式设备上实时处理视频和直播流，而不需要GPU加速，解决现有方法计算复杂、难以在消费级硬件上部署的问题。

Method: 基于暗通道先验和大气散射模型，采用伽马自适应重建、快速传输近似（带数值稳定性的下界）、基于分数顶部像素平均的稳定大气光估计器，以及可选的颜色平衡阶段。

Result: 在真实世界图像和视频上的实验演示表明，Hazedefy能够在不使用GPU加速的情况下提高可见度和对比度，适用于移动和嵌入式应用。

Conclusion: Hazedefy是一个实用且高效的去雾管道，通过计算简化和优化设计，实现了在消费级硬件上的实时视频去雾，为移动和嵌入式视觉应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有DiT的自注意力二次成本限制了向长token序列的扩展。Top-K稀疏注意力方法虽然减少了计算，但仍存在二次选择成本和随序列增长需要增加K值的问题，其低效性源于单层设计无法充分表示全局结构。

Method: 提出对数线性稀疏注意力(LLSA)：1) 分层Top-K选择，利用前一层索引进行渐进式稀疏选择；2) 分层KV增强机制，在注意力计算中使用不同粒度的更少token保留全局上下文；3) 高性能GPU实现，仅使用稀疏索引进行前向和反向传播，无需密集注意力掩码。

Result: 在256×256像素token序列上，LLSA将注意力推理加速28.27倍，DiT训练加速6.09倍，同时保持生成质量。支持无需patchification和VAE编码的高分辨率像素空间图像生成。

Conclusion: LLSA为高效训练长序列DiT提供了有前景的方向，通过分层结构实现了对数线性复杂度的稀疏注意力，显著提升了计算效率。
...
### [54] [Plug to Place: Indoor Multimedia Geolocation from Electrical Sockets for Digital Investigation](https://arxiv.org/abs/2512.16620)
*Kanwal Aftab,Graham Adams,Mark Scanlon*

Main category: cs.CV

TL;DR: 该论文提出了一种基于电源插座的三阶段深度学习管道，用于室内多媒体地理位置定位，通过检测和分类插座类型来推断拍摄地点所在国家或地区。


<details>
  <summary>Details</summary>
Motivation: 室内多媒体地理位置定位在打击人口贩卖、儿童剥削等严重犯罪中具有重要应用价值，但由于室内环境布局相似、装修频繁、视觉模糊、光照变化、GPS信号不可靠以及敏感领域数据集有限等挑战，该领域发展不足。

Method: 提出三阶段深度学习管道：1) 使用YOLOv11检测电源插座（mAP@0.5=0.843）；2) 使用Xception将检测到的插座分类为12种插座类型（准确率0.912）；3) 将插座类型映射到国家/地区（置信度>90%时准确率0.96）。为解决数据稀缺问题，创建了两个专用数据集：包含2,328张标注图像（通过增强扩展到4,072张）的插座检测数据集，以及包含3,187张图像、涵盖12种插座类型的分类数据集。

Result: 在Hotels-50K数据集的TraffickCam子集上评估，该子集包含现实条件下的酒店图像（光照差、业余拍摄角度），比旅游网站的专业图像更具现实性。管道整体性能良好，插座检测mAP@0.5达到0.843，分类准确率0.912，国家映射准确率0.96。

Conclusion: 该框架展示了向实际数字取证应用迈出的实用一步，利用电源插座作为一致的室内标记物进行地理位置推断。代码、训练模型和数据已开源提供。
...
### [55] [DeContext as Defense: Safe Image Editing in Diffusion Transformers](https://arxiv.org/abs/2512.16625)
*Linghui Shen,Mingyue Cui,Xingyi Yang*

Main category: cs.CV

TL;DR: DeContext是一种保护输入图像免受未经授权上下文编辑的新方法，通过注入针对性扰动来削弱跨注意力路径，有效阻断输入与输出之间的链接


<details>
  <summary>Details</summary>
Motivation: 上下文扩散模型虽然能轻松修改图像，但也带来严重的隐私问题：个人图像可能被恶意操纵用于身份冒充、虚假信息等目的。现有工作在个性化文本到图像生成中的输入扰动保护方面有所探索，但现代大规模基于DiT的上下文模型的鲁棒性尚未得到充分研究

Method: DeContext通过注入小的针对性扰动来削弱跨注意力路径，阻断上下文信息从源图像到输出的传播。研究发现早期去噪步骤和特定transformer块主导上下文传播，因此可以将扰动集中在最关键的位置

Result: 在Flux Kontext和Step1X-Edit上的实验表明，DeContext能持续阻止不需要的图像编辑，同时保持视觉质量。这些结果突显了基于注意力的扰动作为对抗图像操纵的有效防御手段

Conclusion: DeContext提供了一种简单、高效且鲁棒的防御方法，通过针对性扰动跨注意力路径来保护图像免受未经授权的上下文编辑，为解决扩散模型带来的隐私和安全问题提供了有效解决方案
...
### [56] [SARMAE: Masked Autoencoder for SAR Representation Learning](https://arxiv.org/abs/2512.16635)
...
<details>
  <summary>Details</summary>
Motivation: 现有SAR深度学习面临数据稀缺和斑点噪声干扰两大挑战：1) SAR数据获取困难导致训练数据不足；2) 物理基础的斑点噪声阻碍细粒度语义表征学习。需要开发能够有效处理噪声并利用大规模预训练的SAR专用方法。

Method: 提出SARMAE框架：1) 构建首个百万级SAR数据集SAR-1M，包含配对的光学图像；2) 设计斑点感知表征增强(SARE)，在掩码自编码器中注入SAR特定斑点噪声，促进噪声感知的鲁棒表征学习；3) 引入语义锚点表征约束(SARC)，利用配对光学先验对齐SAR特征，确保语义一致性。

Result: 在多个SAR数据集上的广泛实验表明，SARMAE在分类、检测和分割任务上均达到最先进的性能。具体表现为：1) 有效克服数据稀缺问题；2) 显著提升对斑点噪声的鲁棒性；3) 实现更好的语义表征学习。

Conclusion: SARMAE通过构建大规模数据集、噪声感知学习和光学先验对齐，成功解决了SAR深度学习中的数据稀缺和噪声干扰问题，为SAR图像理解提供了有效的自监督学习框架，在多个下游任务上展现了优越性能。
...
### [57] [REGLUE Your Latents with Global and Local Semantics for Entangled Diffusion](https://arxiv.org/abs/2512.16636)
...
<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型的重构式去噪目标仅提供间接语义监督，高级语义出现缓慢，需要更长的训练时间并限制了样本质量。现有方法未能充分利用视觉基础模型丰富、非线性、多层空间语义信息。

Method: 提出REGLUE框架：1) 使用轻量级卷积语义压缩器非线性聚合多层VFM特征为低维空间结构化表示；2) 在单一SiT骨干中联合建模VAE图像潜在、局部VFM语义和全局[CLS]标记；3) 通过外部对齐损失正则化内部表示朝向冻结的VFM目标。

Result: 在ImageNet 256×256上，REGLUE持续改进FID并加速收敛，优于SiT-B/2、SiT-XL/2基线以及REPA、ReDi和REG方法。实验表明：空间VFM语义至关重要，非线性压缩是发挥其全部效益的关键，全局标记和外部对齐在全局-局部-潜在联合建模框架中起到互补的轻量级增强作用。

Conclusion: REGLUE通过统一建模视觉基础模型的丰富语义信息，有效提升了潜在扩散模型的图像合成质量和训练效率，证明了空间语义、非线性压缩和全局-局部联合建模的重要性。
...
### [58] [FrameDiffuser: G-Buffer-Conditioned Diffusion for Neural Forward Frame Rendering](https://arxiv.org/abs/2512.16670)
*Ole Beisswenger,Jan-Niklas Dihlmann,Hendrik P. A. Lensch*

Main category: cs.CV

TL;DR: FrameDiffuser是一个自回归神经渲染框架，通过结合G-buffer数据和自身先前输出来生成时间一致、照片级真实的帧，解决了现有方法在交互应用中缺乏时间一致性和计算成本高的问题。
...
Result: 能够生成数百到数千帧的时间一致、照片级真实的图像，在光照、阴影和反射方面优于通用化方法，适合交互应用。

Conclusion: FrameDiffuser通过自回归架构和特定环境训练，在保持时间一致性和推理速度的同时，实现了优于通用方法的照片级真实渲染质量。
...
### [59] [Few-Shot Fingerprinting Subject Re-Identification in 3D-MRI and 2D-X-Ray](https://arxiv.org/abs/2512.16685)
*Gonçalo Gaspar Alves,Shekoufeh Gorgi Zadeh,Andreas Husch,Ben Bausch*

Main category: cs.CV

TL;DR: 该论文提出使用主题指纹识别技术解决开源数据集合并时的数据泄露问题，通过将同一受试者的所有图像映射到潜在空间中的特定区域，实现基于相似性匹配的受试者重识别。
...
Method: 使用ResNet-50模型配合三元组边界损失训练，探索主题指纹识别技术。该方法将同一受试者的所有图像映射到潜在空间中的特定区域，通过相似性匹配实现受试者重识别。在3D MRI和2D X-ray数据上评估了少样本指纹识别，包括标准（20-way 1-shot）和挑战性（1000-way 1-shot）场景。

Result: 模型在ChestXray-14数据集上取得了高Mean-Recall-@-K分数：99.10%（20-way 1-shot）和90.06%（500-way 5-shot）；在BraTS-2021数据集上：99.20%（20-way 1-shot）和98.86%（100-way 3-shot）。

Conclusion: 主题指纹识别技术能有效解决数据集合并时的数据泄露问题，通过将同一受试者的图像映射到潜在空间中的特定区域，实现了高精度的受试者重识别，为医学图像分析中的数据集整合提供了可靠解决方案。
...
### [60] [SDFoam: Signed-Distance Foam for explicit surface reconstruction](https://arxiv.org/abs/2512.16706)
...
<details>
  <summary>Details</summary>
Motivation: 现有NeRF、3DGS和RadiantFoam等方法在精确网格重建方面仍有不足，需要一种既能保持高效渲染又能提升几何重建质量的方法

Method: 联合学习显式Voronoi图和隐式有符号距离场(SDF)，通过光线追踪优化场景，使用Eikonal正则化，使Voronoi单元面与零水平集对齐

Result: SDFoam显著改善了网格重建精度（Chamfer距离），在保持光度量质量（PSNR、SSIM）的同时，训练速度与RadiantFoam相当

Conclusion: 提出的混合隐式-显式方法SDFoam在保持渲染效率的同时，能够生成更清晰、视角一致的表面，减少漂浮物，改善拓扑结构
...
### [61] [A multi-centre, multi-device benchmark dataset for landmark-based comprehensive fetal biometry](https://arxiv.org/abs/2512.16710)
...
<details>
  <summary>Details</summary>
Motivation: 胎儿超声生长评估依赖手动测量解剖标志点，这过程耗时、操作者依赖性强，且在不同扫描设备和站点间存在变异性，限制了自动化方法的可重复性。需要多源标注数据集来开发AI辅助的胎儿生长评估方法。

Method: 创建了一个开放的多中心、多设备胎儿超声图像基准数据集，包含4,513张去标识化超声图像，来自1,904名受试者，采集自三个临床站点和七种不同超声设备。数据集提供了标准化的、受试者分离的训练/测试分割、评估代码和基线结果。

Result: 使用自动生物测量模型量化了域偏移，证明仅使用单一中心数据进行训练和评估会显著高估性能。这是首个公开的多中心、多设备、标志点标注数据集，涵盖所有主要胎儿生物测量指标。
...
### [62] [OMG-Bench: A New Challenging Benchmark for Skeleton-based Online Micro Hand Gesture Recognition](https://arxiv.org/abs/2512.16727)
*Haochen Chang,Pengfei Ren,Buyuan Zhang,Da Li,Tianhao Han,Haoyang Zhang,Liang Xie,Hongbo Chen,Erwei Yin*

Main category: cs.CV

TL;DR: 该论文提出了OMG-Bench——首个大规模公开的骨架在线微手势识别基准，以及Hierarchical Memory-Augmented Transformer (HMATr)框架，在检测率上比现有方法提升7.6%。
...
Method: 开发了多视角自监督管道自动生成骨架数据，辅以启发式规则和专家精炼进行半自动标注。提出了Hierarchical Memory-Augmented Transformer (HMATr)端到端框架，通过分层记忆库存储帧级细节和窗口级语义来统一手势检测和分类。

Result: 创建了OMG-Bench基准，包含40个细粒度手势类别、13,948个实例、1,272个序列。HMATr在检测率上比最先进方法提升7.6%，为在线微手势识别建立了强基线。

Conclusion: 该工作通过构建首个大规模公开基准和提出有效的HMATr框架，显著推进了骨架在线微手势识别领域的发展，为VR/AR交互提供了重要技术基础。
...
### [63] [Task-Oriented Data Synthesis and Control-Rectify Sampling for Remote Sensing Semantic Segmentation](https://arxiv.org/abs/2512.16740)
...
<details>
  <summary>Details</summary>
Motivation: 遥感领域可控生成技术快速发展，合成数据成为扩展标注数据集和减轻人工标注负担的途径。然而，语义掩码控制的复杂性和采样质量的不确定性限制了合成数据在下游语义分割任务中的效用。

Method: 提出任务导向的数据合成框架TODSynth，包括：1）具有统一三重注意力的多模态扩散变换器（MM-DiT）；2）任务反馈引导的即插即用采样策略。基于DiT生成基础模型，评估不同控制方案，发现文本-图像-掩码联合注意力方案结合图像和掩码分支的完全微调能显著提升遥感语义分割数据合成效果。还提出控制-校正流匹配（CRFM）方法，在早期高可塑性阶段通过语义损失动态调整采样方向。

Result: 在少样本和复杂场景下，该方法显著提升了遥感语义分割数据合成的有效性。广泛实验表明，该方法在生成稳定性和任务导向性方面优于现有最先进的可控生成方法。

Conclusion: TODSynth框架通过多模态扩散变换器和任务反馈引导的采样策略，能够生成更稳定、任务导向的合成数据，有效缓解遥感语义分割中数据标注的挑战，特别是在少样本和复杂场景下表现优异。
...
### [64] [TreeNet: A Light Weight Model for Low Bitrate Image Compression](https://arxiv.org/abs/2512.16743)
*Mahadev Prasad Panda,Purnachandra Rao Makkena,Srivatsa Prativadibhayankaram,Siegfried Fößel,André Kaup*

Main category: cs.CV

TL;DR: TreeNet提出了一种基于二叉树结构的低复杂度图像压缩模型，通过注意力特征融合机制整合多分支特征，在保持高性能的同时大幅降低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 学习型图像压缩技术的计算复杂度高是阻碍其广泛应用的关键挑战，需要开发既能保持高性能又具有低计算复杂度的压缩模型。
...
Conclusion: TreeNet成功实现了高性能与低复杂度的平衡，为学习型图像压缩的实际应用提供了有前景的解决方案。
...
### [65] [Make-It-Poseable: Feed-forward Latent Posing Model for 3D Humanoid Character Animation](https://arxiv.org/abs/2512.16767)
...
Conclusion: Make-It-Poseable通过潜在空间变换方法有效解决了传统3D角色摆姿方法的局限性，提供了一种更鲁棒和通用的解决方案。
...
### [66] [FlowDet: Unifying Object Detection and Generative Transport Flows](https://arxiv.org/abs/2512.16771)
...
<details>
  <summary>Details</summary>
Motivation: 本文的动机是重新审视和推广目标检测的生成式框架，从基于扩散的方法扩展到更广泛的生成传输问题。作者希望找到比扩散方法更简单、更直接的传输路径，以提高检测性能并减少推理步骤的需求。

Method: FlowDet采用条件流匹配技术，将目标检测重新表述为生成传输问题。与扩散方法产生的弯曲随机传输路径不同，FlowDet学习更简单、更直接的路径。该方法保持了无需重新训练即可调整边界框数量和推理步骤的能力。

Result: FlowDet在多个实验中超越了基于扩散的检测系统和非生成式基线方法。在COCO数据集上获得了+3.6% AP的提升，在LVIS数据集上获得了+4.2% AP_{rare}的提升。特别是在召回受限的设置下，该方法能有效展示生成传输的优势，而无需依赖大量候选框。

Conclusion: 条件流匹配为目标检测提供了一个有前景的生成式框架，相比扩散方法，它能够学习更简单、更直接的传输路径，从而在推理步骤增加时获得更快的性能提升，并在多个数据集和评估指标上取得更好的结果。
...
### [67] [Kling-Omni Technical Report](https://arxiv.org/abs/2512.16776)
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成系统通常采用分离的流水线方法，不同任务（生成、编辑、推理）之间功能割裂。作者希望构建一个统一的框架，能够处理多样化的多模态输入，实现高质量、智能化的视频内容创作。

Method: 采用端到端框架设计，将文本指令、参考图像和视频上下文等多样化输入处理为统一的多模态表示。构建了全面的数据系统作为多模态视频创作的基础，并采用高效的大规模预训练策略和推理基础设施优化。

Result: 综合评估显示，Kling-Omni在上下文生成、基于推理的编辑和多模态指令跟随方面表现出卓越能力，能够创作电影质量的视频内容。
...
### [68] [R3ST: A Synthetic 3D Dataset With Realistic Trajectories](https://arxiv.org/abs/2512.16784)
...
Method: 生成合成3D环境，并从SinD（无人机拍摄的鸟瞰数据集）中集成真实世界轨迹，创建同时具有精确多模态地面真实标注和真实人类驾驶车辆轨迹的数据集。

Result: R3ST数据集填补了合成数据与真实轨迹之间的差距，提供了既准确又真实的车辆轨迹数据。
...
### [69] [KineST: A Kinematics-guided Spatiotemporal State Space Model for Human Motion Tracking from Sparse Signals](https://arxiv.org/abs/2512.16791)
*Shuting Zhao,Zeyu Xiao,Xinrong Chen*

Main category: cs.CV

TL;DR: KineST是一种基于运动学引导的状态空间模型，用于从稀疏信号重建真实多样的全身姿态，在AR/VR应用中平衡准确性、时间一致性和效率。


<details>
  <summary>Details</summary>
Motivation: AR/VR应用中，基于头戴设备稀疏信号重建真实多样的全身姿态具有挑战性。现有方法要么计算成本高，要么需要分别建模空间和时间依赖关系，难以平衡准确性、时间一致性和效率。
...
Conclusion: KineST通过运动学引导的状态空间模型，有效提取时空依赖关系并整合局部和全局姿态感知，解决了AR/VR应用中基于稀疏信号重建全身姿态的挑战。
...
### [70] [Next-Generation License Plate Detection and Recognition System using YOLOv8](https://arxiv.org/abs/2512.16826)
*Arslan Amin,Rafia Mumtaz,Muhammad Jawad Bashir,Syed Mohammad Hassan Zaidi*

Main category: cs.CV

TL;DR: 该研究评估了YOLOv8变体在车牌识别和字符识别任务上的性能，提出了一种优化的识别流程，在保持计算效率的同时实现了高精度，为智能交通系统中的边缘设备部署提供了解决方案。


<details>
  <summary>Details</summary>
Motivation: 在交通管理和车辆监控领域，高效的车牌检测和识别至关重要。尽管已有多种方法，但在多样化环境中保持实时准确性仍然具有挑战性。本研究旨在通过评估YOLOv8变体性能，为智能交通系统提供更可靠的解决方案。

Method: 使用两个不同的数据集进行训练和评估，测试YOLOv8变体在车牌识别和字符识别任务上的性能。提出了一种基于x轴位置的自定义字符排序方法，并构建了优化的识别流程：使用YOLOv8 Nano进行车牌识别，使用YOLOv8 Small进行字符识别。

Result: YOLOv8 Nano在车牌识别任务上达到0.964的精确度和0.918的mAP50；YOLOv8 Small在字符识别任务上达到0.92的精确度和0.91的mAP50。自定义字符排序方法有效实现了检测字符的顺序排列。

Conclusion: 提出的优化流程在保持计算效率的同时确保了高准确性，为智能交通系统中边缘设备的实际部署奠定了坚实基础，是推动更智能、更高效城市基础设施发展的重要进展。
...
### [71] [Radiology Report Generation with Layer-Wise Anatomical Attention](https://arxiv.org/abs/2512.16841)
...
TL;DR: 提出了一种紧凑的胸部X光报告生成架构，使用单张正面图像生成"发现"部分，通过解剖学注意力机制提升临床相关区域的关注度，在资源有限的情况下实现了显著的性能提升。
...
Method: 采用紧凑的图像到文本架构：冻结的DINOv3 ViT编码器 + GPT-2解码器，通过分层高斯平滑整合肺部和心脏分割掩码，实现层级的解剖学注意力机制，无需额外可训练参数。

Result: 在MIMIC-CXR数据集上评估：5种关键病理的CheXpert Macro-F1提升168%（0.083→0.238），Micro-F1提升146%（0.137→0.337）；14种观察指标提升86%；RadGraph F1提升9.7%。

Conclusion: 尽管模型规模小且仅依赖图像条件，但解码器级别的解剖学指导能改善空间定位，增强临床相关区域的连贯性，为资源有限环境提供了有效的放射学报告生成方案。
...
### [72] [GenEval 2: Addressing Benchmark Drift in Text-to-Image Evaluation](https://arxiv.org/abs/2512.16853)
...
<details>
  <summary>Details</summary>
Motivation: 自动化文本到图像模型评估面临挑战：评估模型需要正确评分，测试提示需要具有挑战性但又不超出评估模型能力。作者发现这些约束会导致基准随时间漂移，静态基准无法跟上新模型能力的发展。GenEval作为最流行的T2I基准之一，已经出现显著的基准漂移问题。

Method: 1) 分析GenEval基准的漂移问题，通过大规模人类研究验证其饱和性；2) 提出新基准GenEval 2，改进对基本视觉概念的覆盖并提高组合性；3) 提出Soft-TIFA评估方法，结合基本视觉概念的判断，相比整体性评估方法更接近人类判断。

Result: GenEval基准已与人类判断严重脱节，绝对误差高达17.7%，表明该基准已饱和。GenEval 2基准对当前模型更具挑战性，Soft-TIFA评估方法比VQAScore等整体性评估方法更接近人类判断，且更不容易随时间漂移。

Conclusion: 基准漂移是T2I评估中的重要问题，需要持续审计和改进。GenEval 2提供了更强大的基准，但避免基准漂移仍需持续努力。这项工作强调了自动化模型评估基准需要不断更新以保持与人类判断的一致性。
...
### [73] [RePlan: Reasoning-guided Region Planning for Complex Instruction-based Image Editing](https://arxiv.org/abs/2512.16864)
...
Method: 提出RePlan框架，采用"规划-执行"方法：1）视觉语言规划器通过逐步推理分解指令并明确将其定位到目标区域；2）扩散编辑器使用无需训练的注意力区域注入机制应用更改，实现精确的并行多区域编辑

Result: 在IV-Complex设置下，RePlan始终优于基于更大数据集训练的强基线模型，显著提高了区域精度和整体保真度。通过GRPO强化学习仅使用1K指令示例就获得了推理保真度和格式可靠性的显著提升

Conclusion: RePlan框架通过区域对齐规划有效解决了复杂指令图像编辑中的IV-Complexity问题，在精细定位和知识密集型编辑任务上表现出色，为复杂场景下的精确图像编辑提供了新思路
...
### [74] [Memory-Enhanced SAM3 for Occlusion-Robust Surgical Instrument Segmentation](https://arxiv.org/abs/2512.16880)
...
<details>
  <summary>Details</summary>
Motivation: 内窥镜视频中的手术器械分割对计算机辅助干预至关重要，但由于频繁遮挡、快速运动、镜面伪影和器械长期重新进入等挑战，现有方法性能有限。SAM3虽然提供了强大的时空框架，但在手术场景中受到内存更新不区分、固定内存容量和遮挡后身份恢复能力弱等限制。

Method: 提出了ReMeDI-SAM3，包含三个核心组件：1) 相关性感知内存过滤，配备专门的遮挡感知内存存储遮挡前帧；2) 分段插值方案，扩展有效内存容量；3) 基于特征的重新识别模块，通过时间投票实现可靠的遮挡后身份消歧。

Result: 在EndoVis17和EndoVis18数据集上的零样本评估显示，相对于原始SAM3分别实现了约7%和16%的绝对mcIoU提升，甚至超过了先前基于训练的方法。

Conclusion: ReMeDI-SAM3通过增强内存管理和身份恢复机制，有效解决了手术器械分割中的遮挡问题，显著提升了分割性能，且无需额外训练。
...
### [75] [M-PhyGs: Multi-Material Object Dynamics from Video](https://arxiv.org/abs/2512.16885)
*Norika Wada,Kohei Yamashita,Ryo Kawahara,Ko Nishino*

Main category: cs.CV

TL;DR: 提出M-PhyGs方法，从视频中估计复杂自然物体（以花朵为代表）的材料组成和物理参数，解决了现有方法对单一材料、预学习动力学或简单拓扑结构的限制。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的物体通常具有复杂的材料组成和几何形状，超出了现有物理参数估计方法的假设范围。现有方法假设物体是单一材料、需要预学习动力学或只能处理简单拓扑结构。本文特别关注花朵作为代表性常见物体，需要从自然场景视频中准确估计其多材料物理参数。

Method: 提出Multi-material Physical Gaussians (M-PhyGs)方法，从短视频中联合分割物体的相似材料区域并恢复其连续介质力学参数，同时考虑重力影响。方法采用新引入的级联3D和2D损失函数，并利用时间小批量处理来提高效率。

Result: 在Phlowers数据集（包含人与花朵交互的新数据集）上的实验结果表明，M-PhyGs及其组件在多材料物理参数估计这一挑战性任务上具有准确性和有效性。

Conclusion: M-PhyGs能够从自然场景视频中准确估计复杂多材料物体的物理参数，为现实世界物体的物理理解提供了有效方法，特别是在花朵这类常见但材料复杂的物体上展示了良好性能。
...
### [76] [LinkedOut: Linking World Knowledge Representation Out of Video LLM for Next-Generation Video Recommendation](https://arxiv.org/abs/2512.16891)
...
Result: LinkedOut是第一个基于VLLM的视频推荐方法，无需手工标注标签，在标准基准测试中达到最先进的结果。可解释性研究和消融实验证实了层多样性和分层融合的好处。

Conclusion: LinkedOut为充分利用VLLM世界知识先验和视觉推理进行下游视觉任务（如推荐）提供了一条实用路径，解决了VLLM在视频推荐部署中的关键挑战。
...
### [77] [FlashPortrait: 6x Faster Infinite Portrait Animation with Adaptive Latent Prediction](https://arxiv.org/abs/2512.16900)
*Shuyuan Tu,Yueming Pan,Yinming Huang,Xintong Han,Zhen Xing,Qi Dai,Kai Qiu,Chong Luo,Zuxuan Wu*

Main category: cs.CV

TL;DR: FlashPortrait是一种端到端视频扩散变换器，能够合成保持身份一致性的无限长度肖像视频，同时实现高达6倍的推理加速。
...
Method: 1) 使用现成提取器计算身份无关的面部表情特征；2) 引入归一化面部表情块，通过均值和方差归一化对齐面部特征与扩散潜变量；3) 采用动态滑动窗口方案，在重叠区域进行加权融合；4) 基于特定时间步的潜变量变化率和扩散层间的导数幅度比，使用当前时间步的高阶潜变量导数直接预测未来时间步的潜变量。

Result: 在基准测试中，FlashPortrait在质量和数量上都显示出有效性，能够合成保持身份一致性的无限长度视频，同时实现高达6倍的推理加速。

Conclusion: FlashPortrait通过创新的归一化面部特征对齐、动态滑动窗口和高阶潜变量预测技术，成功解决了长肖像动画中的身份一致性问题，并实现了显著的推理加速。
...
### [78] [VIVA: VLM-Guided Instruction-Based Video Editing with Reward Optimization](https://arxiv.org/abs/2512.16906)
...
Method: 1. 引入VLM-based instructor编码文本指令、视频首帧和可选参考图像为视觉基础指令表示；2. 提出Edit-GRPO后训练阶段，将Group Relative Policy Optimization适配到视频编辑领域；3. 设计数据构建管道生成多样高保真的基础编辑操作视频-指令配对数据

Result: VIVA在指令遵循、泛化能力和编辑质量方面优于现有最先进方法

Conclusion: VIVA通过VLM引导编码和奖励优化框架，有效解决了基于指令的视频编辑中的泛化问题，实现了对复杂真实世界指令的高质量编辑
...
### [79] [SceneDiff: A Benchmark and Method for Multiview Object Change Detection](https://arxiv.org/abs/2512.16908)
*Yuqun Wu,Chih-hao Lin,Henry Che,Aditi Tiwari,Chuhang Zou,Shenlong Wang,Derek Hoiem*

Main category: cs.CV

TL;DR: 本文提出了SceneDiff Benchmark，这是首个带对象实例标注的多视角变化检测基准，以及SceneDiff方法，一种无需训练的多视角对象变化检测方法，在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在不同时间对同一场景进行多次拍摄时，识别添加、移除或移动的对象变化对于机器人整理、施工进度监控等应用很重要。主要挑战在于视角变化可能导致对象错误地显示为已改变。

Method: 提出SceneDiff方法：1）在3D中对齐拍摄内容；2）提取对象区域；3）比较空间和语义区域特征来检测变化。该方法利用预训练的3D、分割和图像编码模型，无需额外训练。

Result: 在多视角和双视角基准测试中，该方法大幅优于现有方法（相对AP提升94%和37.4%）。创建了包含350个多样化视频对、数千个变化对象的SceneDiff Benchmark。

Conclusion: SceneDiff Benchmark是首个带对象实例标注的多视角变化检测基准，SceneDiff方法在无需训练的情况下实现了最先进的性能，为多视角变化检测提供了新的解决方案和评估标准。
...
### [80] [SFTok: Bridging the Performance Gap in Discrete Tokenizers](https://arxiv.org/abs/2512.16910)
...
Conclusion: SFTok通过创新的多步迭代机制和训练策略，显著提升了离散分词器的重建质量，为高分辨率图像生成提供了高效且高质量的解决方案。
...
### [81] [StereoPilot: Learning Unified and Efficient Stereo Conversion via Generative Priors](https://arxiv.org/abs/2512.16915)
*Guibao Shen,Yihua Du,Wenhang Ge,Jing He,Chirui Chang,Donghao Zhou,Zhen Yang,Luozhou Wang,Xin Tao,Ying-Cong Chen*

Main category: cs.CV

TL;DR: 提出了UniStereo数据集和StereoPilot模型，用于解决单目到立体视频转换中的深度模糊和格式不一致问题，实现了高效的前馈式目标视图合成。


<details>
  <summary>Details</summary>
Motivation: 立体显示设备快速增长，但高质量立体视频内容制作成本高且复杂。传统的多阶段"深度-扭曲-修复"流水线存在错误传播、深度模糊以及平行和汇聚立体格式之间的不一致性问题。

Method: 首先创建了UniStereo数据集，覆盖两种立体格式用于公平基准测试和模型训练。然后提出了StereoPilot模型，这是一个高效的前馈模型，无需依赖显式深度图或迭代扩散采样，直接合成目标视图。模型配备了可学习的域切换器和循环一致性损失，能够自适应不同立体格式并提高一致性。

Result: 大量实验表明，StereoPilot在视觉保真度和计算效率方面显著优于现有最先进方法。

Conclusion: 通过UniStereo数据集和StereoPilot模型，解决了单目到立体视频转换中的关键挑战，实现了高质量、高效率的立体视频生成，适应不同的立体格式配置。
...
### [82] [AdaTooler-V: Adaptive Tool-Use for Images and Videos](https://arxiv.org/abs/2512.16918)
*Chaoyang Wang,Kaituo Feng,Dongyang Chen,Zhongyu Wang,Zhixun Li,Sicheng Gao,Meng Meng,Xu Zhou,Manyuan Zhang,Yuzhang Shang,Xiangyu Yue*

Main category: cs.CV

TL;DR: AdaTooler-V是一个多模态大语言模型，通过自适应工具使用机制，仅在视觉问题真正需要工具时才调用视觉工具，避免了不必要的工具调用，从而减少推理开销并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有开源多模态大语言模型存在盲目使用工具的问题，即使在不必要的情况下也会调用视觉工具，这显著增加了推理开销并降低了模型性能。需要一种自适应工具使用机制来优化工具调用决策。

Method: 1. 提出AT-GRPO强化学习算法，基于每个样本的工具效益分数自适应调整奖励尺度，鼓励模型仅在工具能真正带来改进时才调用工具。2. 构建两个训练数据集：AdaTooler-V-CoT-100k用于监督微调冷启动，AdaTooler-V-300k用于强化学习，包含单图像、多图像和视频数据的可验证奖励。
...
Conclusion: AdaTooler-V通过自适应工具使用机制有效解决了多模态大语言模型中盲目工具调用的问题，在减少推理开销的同时提升了模型性能，在多个视觉推理基准上取得了最先进的结果。
...
### [83] [EasyV2V: A High-quality Instruction-based Video Editing Framework](https://arxiv.org/abs/2512.16920)
*Jinjie Mai,Chaoyang Wang,Guocheng Gordon Qian,Willi Menapace,Sergey Tulyakov,Bernard Ghanem,Peter Wonka,Ashkan Mirzaei*

Main category: cs.CV

TL;DR: EasyV2V是一个简单有效的基于指令的视频编辑框架，通过数据构建、架构简化和控制统一三方面创新，实现了灵活输入下的最先进视频编辑效果。


<details>
  <summary>Details</summary>
Motivation: 图像编辑技术发展迅速，但视频编辑仍面临一致性、控制和泛化性等挑战。研究者希望探索数据、架构和控制的设计空间，开发一个简单有效的指令式视频编辑框架。

Method: 1) 数据方面：组合现有专家模型构建多样化视频对；通过单帧监督将图像编辑对提升为视频；利用共享仿射运动生成伪视频对；挖掘密集标注的视频片段；添加过渡监督。2) 模型方面：发现预训练文本到视频模型已具备编辑能力，采用简单的序列拼接条件和轻量LoRA微调。3) 控制方面：通过单一掩码机制统一时空控制，支持可选参考图像。

Result: EasyV2V支持多种灵活输入（视频+文本、视频+掩码+文本、视频+掩码+参考图像+文本），在视频编辑任务上取得了最先进的结果，超越了同期和商业系统。

Conclusion: EasyV2V通过系统性的数据构建、简化的架构设计和统一的控制机制，为基于指令的视频编辑提供了一个强大而简单的解决方案，显著提升了视频编辑的质量和灵活性。
...
### [84] [Differences That Matter: Auditing Models for Capability Gap Discovery and Rectification](https://arxiv.org/abs/2512.16921)
*Qihao Liu,Chengzhi Mao,Yaojie Liu,Alan Yuille,Wen-Sheng Chu*

Main category: cs.CV

TL;DR: AuditDM是一个自动化的多模态大语言模型审计框架，通过强化学习训练审计模型生成具有挑战性的问题和反事实图像，以发现模型弱点并作为无标注数据进行修复。


<details>
  <summary>Details</summary>
Motivation: 传统的多模态大语言模型评估方法缺乏可解释性，且往往无法充分揭示不同模型之间的显著能力差距，需要更有效的模型诊断和改进方法。

Method: 通过强化学习微调一个MLLM作为审计模型，使其生成能够最大化目标模型之间分歧的挑战性问题和反事实图像，从而发现模型失败模式。
...
### [85] [Next-Embedding Prediction Makes Strong Vision Learners](https://arxiv.org/abs/2512.16922)
...
<details>
  <summary>Details</summary>
Motivation: 受自然语言生成式预训练成功的启发，探索是否相同原则能产生强大的视觉自监督学习器，从学习表示转向学习模型
...
### [86] [Generative Refocusing: Flexible Defocus Control from a Single Image](https://arxiv.org/abs/2512.16923)
...
<details>
  <summary>Details</summary>
Motivation: 景深控制在摄影中很重要，但单图像重新对焦仍然困难。现有方法需要全焦输入、依赖模拟器合成数据、对光圈控制有限，存在明显缺陷。
...
Result: 在去模糊、散景合成和重新对焦基准测试中取得最佳性能，支持文本引导调整和自定义光圈形状。

Conclusion: Generative Refocusing方法通过创新的半监督训练策略，有效解决了单图像重新对焦的挑战，实现了高质量的可控散景合成。
...
### [87] [The World is Your Canvas: Painting Promptable Events with Reference Images, Trajectories, and Text](https://arxiv.org/abs/2512.16924)
...
TL;DR: WorldCanvas是一个支持多模态提示的世界事件生成框架，通过结合文本、轨迹和参考图像，实现用户可控的丰富场景模拟。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么仅限于文本描述，要么只能通过轨迹控制图像到视频的转换，缺乏对复杂世界事件（如多智能体交互、物体进出、反直觉事件）的细粒度控制和语义理解能力。

Method: 采用多模态方法，将轨迹（编码运动、时序和可见性）、自然语言（语义意图）和参考图像（物体身份视觉基础）相结合，生成连贯可控的事件视频。

Result: 生成的视频不仅具有时间连贯性，还表现出涌现一致性，能够保持物体身份和场景的连续性，即使物体暂时消失也能正确恢复。

Conclusion: WorldCanvas将世界模型从被动预测器推进为交互式、用户可塑造的模拟器，支持表达性世界事件的生成。
...
### [88] [Large Video Planner Enables Generalizable Robot Control](https://arxiv.org/abs/2512.15840)
...
TL;DR: 该研究提出了一种基于大规模视频预训练构建机器人基础模型的新范式，通过互联网规模的人类活动视频数据集训练生成式机器人规划模型，能够为零样本新场景和任务生成视频规划，并提取可执行的机器人动作。


<details>
  <summary>Details</summary>
Motivation: 现有机器人基础模型主要基于多模态大语言模型（MLLMs）扩展动作输出，但本文探索了使用大规模视频预训练作为主要模态的替代范式。视频能捕捉物理世界中状态和动作的时空序列，与机器人行为自然对齐。

Method: 收集互联网规模的人类活动和任务演示视频数据集，首次以基础模型规模训练开放视频模型用于生成式机器人规划。模型为零样本新场景和任务生成视频规划，后处理提取可执行的机器人动作。

Result: 通过第三方选择的野外任务和真实机器人实验评估任务级泛化能力，展示了成功的物理执行。结果显示强大的指令跟随能力、强泛化能力和现实世界可行性。

Conclusion: 大规模视频预训练是构建机器人基础模型的有效替代范式，能够实现零样本规划生成和真实世界执行。研究发布了模型和数据集以支持开放、可重复的视频机器人学习。
...
### [89] [SORS: A Modular, High-Fidelity Simulator for Soft Robots](https://arxiv.org/abs/2512.15994)
*Manuel Mekkattu,Mike Y. Michelis,Robert K. Katzschmann*

Main category: cs.RO

TL;DR: SORS是一个用于软体机器人的高保真模拟器，采用基于能量的有限元方法框架，通过约束非线性优化处理接触现象，在多种真实实验中验证了其物理准确性。


<details>
  <summary>Details</summary>
Motivation: 软体机器人在多物理场环境中的部署需要先进的模拟框架，但现有模拟器难以处理软体机器人的大非线性变形、材料不可压缩性和接触交互等挑战，缺乏可扩展性和应用相关性。

Method: 提出SORS（Soft Over Rigid Simulator），采用基于能量的有限元方法框架，支持模块化扩展以包含定制材料和驱动模型。通过基于序列二次规划的约束非线性优化确保物理一致的接触处理。

Result: 通过悬臂梁偏转、软体机器人臂的压力驱动、PokeFlex数据集的接触交互等多种真实实验验证了模拟器的物理保真度，并展示了软体机器人腿的控制优化潜力。

Conclusion: SORS模拟器能够准确捕捉基本材料行为和复杂驱动动力学，填补了软体机器人生态系统中可扩展性、保真度和可用性的空白，为下一代软体机器人原型设计提供了验证工具。
...
### [90] [dLITE: Differentiable Lighting-Informed Trajectory Evaluation for On-Orbit Inspection](https://arxiv.org/abs/2512.16011)
*Jack Naylor,Raghav Mishra,Nicholas H. Barbara,Donald G. Dansereau*

Main category: cs.RO

TL;DR: 本文提出∂LITE，一种用于在轨检测操作的可微分端到端仿真管道，通过可微分渲染和轨道传播器优化检测轨迹，提高图像质量


<details>
  <summary>Details</summary>
Motivation: 在轨航天器视觉检测对维护、损伤评估和寿命延长至关重要，但低地球轨道的特殊环境（如阳光镜面反射、自阴影、动态光照）以及航天器间的相对运动严重影响图像质量，现有检测轨迹规划方法难以专门优化图像质量

Method: 开发∂LITE端到端可微分仿真管道，结合最先进的可微分渲染工具和自定义轨道传播器，基于视觉传感器数据对轨道参数进行端到端优化，自动设计非显而易见的检测轨迹

Result: ∂LITE能够自动设计出非显而易见的检测轨迹，大幅提高获取数据的质量和实用性，这是首个此类可微分检测规划管道

Conclusion: ∂LITE为现代航天器任务规划提供了新的计算思路，通过可微分仿真优化检测轨迹，显著改善在轨检测的图像质量和数据价值
...
### [91] [Few-Shot Inference of Human Perceptions of Robot Performance in Social Navigation Scenarios](https://arxiv.org/abs/2512.16019)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法需要大量标注数据来评估人类对机器人行为的感知，这在实际应用中受到限制。需要开发更高效的方法来预测人类对机器人性能的评价

Method: 扩展SEAN TOGETHER数据集，利用大语言模型的少样本学习能力，基于机器人时空线索和周围人类运动信息，通过上下文示例预测人类对机器人性能的感知

Result: 大语言模型在预测人类感知方面能够匹配或超越传统监督学习模型，同时需要的标注实例数量少一个数量级；更多上下文示例能提升预测性能；个性化示例能进一步提高准确性

Conclusion: 大语言模型的少样本学习能力为通过用户中心反馈改进机器人行为提供了一条可扩展的路径，能够有效预测人类对机器人性能的感知
...
### [92] [Maintaining the Level of a Payload carried by Multi-Robot System on Irregular Surface](https://arxiv.org/abs/2512.16024)
*Rishabh Dev Yadav,Shrey Agrawal,Kamalakar Karlapalem*

Main category: cs.RO

TL;DR: 提出多机器人负载运输系统，在未知不平坦地形中保持负载期望方向，结合开环和闭环PID控制器


<details>
  <summary>Details</summary>
Motivation: 解决在未知不平坦地形和倾斜环境中，多机器人系统运输负载时需要保持负载期望方向的问题

Method: 使用带有线性执行器（活塞）的自定义机器人，结合开环控制器和闭环PID控制器，持续监测负载方向并计算各机器人所需活塞高度

Result: 系统能在各种未知不平坦地形和倾斜环境中工作，通过模拟环境测试验证了控制器的有效性

Conclusion: 提出的控制器系统能够有效在复杂未知地形中保持负载方向，具有广泛的地形适应性
...
### [93] [SWIFT-Nav: Stability-Aware Waypoint-Level TD3 with Fuzzy Arbitration for UAV Navigation in Cluttered Environments](https://arxiv.org/abs/2512.16027)
...
Method: 提出SWIFT-Nav框架，结合传感器驱动的感知前端和TD3航点策略：感知模块将LiDAR数据转换为置信度加权的安全地图和目标线索；TD3策略使用优先经验回放和衰减ε-贪婪探索；轻量级模糊逻辑层计算安全分数并控制模式切换；任务对齐的奖励塑形结合目标进度、安全距离和切换经济性

Result: 在Webots中实现，基于接近度碰撞检测，在轨迹平滑度和对未见布局的泛化能力方面持续优于基线方法，同时保持实时响应性
...
### [94] [A Task-Driven, Planner-in-the-Loop Computational Design Framework for Modular Manipulators](https://arxiv.org/abs/2512.16069)
*Maolin Lei,Edoardo Romiti,Arturo Laurenzi,Rui Dai,Matteo Dalle Vedove,Jiatao Ding,Daniele Fontanelli,Nikos Tsagarakis*

Main category: cs.RO

TL;DR: 提出一个统一的任务驱动计算框架，用于模块化机械臂的形态与安装姿态协同优化，通过双分支结构扩展工作空间而不增加关节模块容量。


<details>
  <summary>Details</summary>
Motivation: 模块化机械臂虽然具有高适应性，但在部署时需要同时优化形态和安装姿态，满足运动学、动力学和物理约束。传统单分支设计通过增加连杆长度来扩展工作空间，但容易违反基关节的扭矩限制。

Method: 提出统一任务驱动计算框架，集成轨迹规划与形态-姿态协同优化。采用分层模型预测控制(HMPC)进行运动规划，使用CMA-ES算法在离散形态配置和连续安装姿态的混合搜索空间中进行设计优化。引入虚拟模块抽象实现双分支形态，辅助分支为主分支卸载扭矩。

Result: 在抛光、钻孔和拾放任务上的仿真和硬件实验表明：1)框架能生成多个满足约束的可行设计；2)通过定制成本函数可实现灵活设计目标；3)双分支形态能在不增加基本模块功率的情况下实现大工作空间操作。

Conclusion: 该框架能有效解决模块化机械臂的形态与安装姿态协同优化问题，双分支设计扩展了工作空间而不增加关节模块容量，为模块化机械臂的部署提供了系统解决方案。
...
### [95] [A simulation platform calibration method for automated vehicle evaluation: accurate on both vehicle level and traffic flow level](https://arxiv.org/abs/2512.16076)
...
<details>
  <summary>Details</summary>
Motivation: 现有仿真测试校准方法在准确复现自动驾驶车辆与背景交通的交互方面存在不足，这影响了仿真测试的可靠性。需要一种能够同时保证车辆层面和交通流层面校准精度的方法。

Method: 提出了一种仿真平台校准方法，具有四个关键特征：1) 支持车辆间交互校准；2) 保证校准精度；3) 提升校准效率；4) 具备流水线校准能力。该方法与无校准基线和现有最先进校准方法进行对比评估。

Result: 该方法将交互复现精度提升了83.53%，校准效率提高了76.75%。在车辆层面和交通流层面指标上都保持了准确性，整体改进达到51.9%。整个校准过程完全自动化，无需人工干预。

Conclusion: 提出的仿真平台校准方法能够有效解决现有校准方法的不足，在保证高精度的同时显著提升效率，为自动驾驶车辆的可靠仿真测试提供了有力支持。
...
### [96] [ManiLong-Shot: Interaction-Aware One-Shot Imitation Learning for Long-Horizon Manipulation](https://arxiv.org/abs/2512.16302)
*Zixuan Chen,Chongkai Gao,Lin Shao,Jieqi Shi,Jing Huo,Yang Gao*

Main category: cs.RO

TL;DR: ManiLong-Shot是一个用于长视野预抓取操作任务的一次性模仿学习框架，通过物理交互事件分解任务为交互感知基元，相比现有方法在长视野任务上实现了22.8%的相对改进。


<details>
  <summary>Details</summary>
Motivation: 当前的一次性模仿学习方法主要局限于短视野任务，限制了其在复杂长视野操作任务中的应用。需要开发能够处理长视野预抓取操作任务的有效一次性模仿学习框架。

Method: ManiLong-Shot围绕物理交互事件构建长视野任务，将问题重新定义为序列化交互感知基元而非直接模仿连续轨迹。基元分解可通过视觉语言模型的高层推理或基于机器人状态变化的规则启发式驱动。对每个基元，预测交互的关键不变区域，建立演示与当前观察之间的对应关系，并计算目标末端执行器姿态。

Result: 在仅10个短视野任务上训练的ManiLong-Shot，通过一次性模仿泛化到20个未见过的长视野任务（三个难度级别），相比最先进方法实现了22.8%的相对改进。真实机器人实验验证了其通过一次性模仿学习稳健执行三个长视野操作任务的能力。

Conclusion: ManiLong-Shot通过将长视野任务分解为交互感知基元，有效解决了一次性模仿学习在长视野预抓取操作任务中的局限性，展示了在实际机器人应用中的实用性和泛化能力。
...
### [97] [A2VISR: An Active and Adaptive Ground-Aerial Localization System Using Visual Inertial and Single-Range Fusion](https://arxiv.org/abs/2512.16367)
*Sijia Chen,Wei Dong*

Main category: cs.RO

TL;DR: 提出一种地空协作系统，通过主动视觉、单点测距、惯性里程计和光流融合，增强飞行机器人在复杂环境中的定位鲁棒性，特别是在视觉传感器性能下降时。


<details>
  <summary>Details</summary>
Motivation: 传统方法使用固定相机观测预贴标记来估计飞行机器人位置，但受限于距离且容易捕获失败。在视觉传感器性能下降的复杂环境中，需要更鲁棒的定位方案。

Method: 1) 地面车辆搭载主动视觉子系统，可动态旋转检测跟踪空中机器人的红外标记；2) 结合单点测距扩展可行距离并增强视觉退化时的重捕获能力；3) 基于多项式近似的降维估计器融合多源测量；4) 自适应滑动置信度评估算法动态调整权重参数。

Result: 在烟雾干扰、光照变化、障碍物遮挡、长时间视觉丢失和扩展操作范围等条件下，该方法实现了鲁棒的在线定位，平均均方根误差约为0.09米，同时对捕获丢失和传感器故障保持弹性。

Conclusion: 提出的地空协作定位框架通过多传感器融合和自适应置信度评估，显著提高了飞行机器人在复杂环境中的定位鲁棒性和可靠性。
...
### [98] [E-SDS: Environment-aware See it, Do it, Sorted - Automated Environment-Aware Reinforcement Learning for Humanoid Locomotion](https://arxiv.org/abs/2512.16446)
*Enis Yalcin,Joshua O'Hara,Maria Stamatopoulou,Chengxu Zhou,Dimitrios Kanoulas*

Main category: cs.RO

TL;DR: E-SDS框架通过整合视觉语言模型与实时地形感知，自动生成奖励函数来训练人形机器人适应复杂地形的运动策略，显著减少人工设计时间并提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型的奖励设计方法缺乏环境感知能力，无法处理复杂地形导航问题，需要大量人工工程来设计奖励函数。

Method: E-SDS框架整合视觉语言模型与实时地形传感器分析，通过示例视频自动生成奖励函数，训练具有环境感知能力的运动策略。

Result: 在Unitree G1人形机器人上测试四种地形（简单、缝隙、障碍、楼梯），E-SDS成功实现楼梯下降，而手动设计奖励或非感知基线方法均失败；在所有地形中速度跟踪误差减少51.9-82.6%；奖励设计时间从数天减少到不足2小时。

Conclusion: E-SDS框架通过填补环境感知空白，实现了更鲁棒、更高效的人形机器人运动策略自动化设计，显著减少了人工工作量并提升了性能。
...
### [99] [AG-MPBS: a Mobility-Aware Prediction and Behavior-Based Scheduling Framework for Air-Ground Unmanned Systems](https://arxiv.org/abs/2512.16454)
*Tianhao Shao,Kaixing Zhao,Feng Liu,Lixin Yang,Bin Guo*

Main category: cs.RO

TL;DR: MPBS框架通过行为感知分类、时空预测和动态优先级调度，为无人机和无人车等无人系统提供高效的任务招募解决方案。
...
### [100] [Tri-Select: A Multi-Stage Visual Data Selection Framework for Mobile Visual Crowdsensing](https://arxiv.org/abs/2512.16469)
*Jiayu Zhang,Kaixing Zhao,Tianhao Shao,Bin Guo,Liang He*

Main category: cs.RO

TL;DR: Tri-Select是一个三阶段视觉数据选择框架，用于移动视觉众包中高效过滤冗余和低质量图像


<details>
  <summary>Details</summary>
Motivation: 移动视觉众包通过分布式移动设备收集图像实现大规模细粒度环境监测，但数据存在冗余和异构性问题，包括采集视角重叠、分辨率差异和用户行为多样性

Method: 三阶段框架：1) 基于元数据过滤无关样本；2) 基于空间相似性的谱聚类组织候选图像；3) 基于最大独立集搜索的视觉特征引导选择，保留高质量代表性图像
...
### [101] [A Formal Modular Synthesis Approach for the Coordination of 3-D Robotic Construction with Multi-robots](https://arxiv.org/abs/2512.16555)
...
<details>
  <summary>Details</summary>
Motivation: 解决多移动机器人协同构建预定义3D结构的协调问题，实现自主建造

Method: 采用监督控制理论，从单个机器人和目标结构的模型中合成正确构造的反应式控制器（监督器），然后将该监督器复制到其他机器人

Result: 能够生成确保正确性的控制器，所有机器人可以协同完成目标结构的构建

Conclusion: 监督控制理论为多机器人协同构建3D结构提供了一种系统化、可验证的控制器合成方法
...
### [102] [Olaf: Bringing an Animated Character to Life in the Physical World](https://arxiv.org/abs/2512.16705)
...
<details>
  <summary>Details</summary>
Motivation: 动画角色通常以非物理方式运动且比例与典型行走机器人差异很大，这为机械设计和风格化运动控制提供了创新平台。研究旨在将动画角色奥拉夫带入物理世界，实现高可信度的角色化机器人。

Method: 使用动画参考引导的强化学习进行控制；通过软泡沫裙隐藏两条不对称腿部；在手臂、嘴巴和眼睛中使用球形和平面连杆机构；引入减少冲击噪声的奖励函数；将温度值作为策略输入并引入保持温度在安全范围内的奖励。

Result: 在仿真和硬件上验证了模型的有效性，展示了成本化机器人角色中无与伦比的可信度水平。

Conclusion: 成功将动画角色奥拉夫实体化为物理机器人，通过创新的机械设计和强化学习控制解决了比例、隐藏机构、噪声和过热等挑战，实现了高度可信的角色表现。
...
### [103] [VERM: Leveraging Foundation Models to Create a Virtual Eye for Efficient 3D Robotic Manipulation](https://arxiv.org/abs/2512.16724)
*Yixiang Chen,Yan Huang,Keji He,Peiyan Li,Liang Wang*

Main category: cs.RO

TL;DR: 提出VERM方法，利用基础模型知识从3D点云想象虚拟任务自适应视角，过滤冗余信息，提升机器人3D操作任务的效率和性能
...
Conclusion: VERM方法能有效过滤冗余信息，准确提取任务相关特征，提升3D操作任务的效率和性能
...
### [104] [Vision-Language-Action Models for Autonomous Driving: Past, Present, and Future](https://arxiv.org/abs/2512.16760)
...
TL;DR: 本文系统梳理了自动驾驶中新兴的Vision-Language-Action（VLA）框架，将其分为端到端VLA和双系统VLA两大范式，总结了相关数据集、基准和未来挑战。


<details>
  <summary>Details</summary>
Motivation: 传统模块化"感知-决策-动作"管道存在人工接口、规则组件易失效、级联误差传播等问题；纯视觉-动作模型缺乏可解释性、对分布偏移敏感且无法遵循指令。VLA框架通过整合视觉理解、语言推理和动作输出，旨在实现更可解释、泛化性强且符合人类意图的驾驶策略。

Method: 将现有VLA方法系统分类为两大范式：1）端到端VLA：在单一模型中整合感知、推理和规划；2）双系统VLA：分离慢速审慎推理（通过视觉语言模型）和快速安全关键执行（通过规划器）。进一步细分为文本vs数值动作生成器、显式vs隐式指导机制等子类。

Result: 建立了自动驾驶VLA领域的结构化分类体系，总结了代表性数据集和评估基准，识别了该领域的关键技术挑战。

Conclusion: VLA框架为推进人类兼容的自动驾驶系统提供了有前景的路径，但仍需解决鲁棒性、可解释性和指令保真度等关键挑战，本文旨在为该领域建立系统性的理论基础。
...
### [105] [PhysBrain: Human Egocentric Data as a Bridge from Vision Language Models to Physical Intelligence](https://arxiv.org/abs/2512.16793)
*Xiaopeng Lin,Shijie Lian,Bin Yu,Ruoqi Yang,Changti Wu,Yuzhuo Miao,Yurun Jin,Yukun Shi,Cong Huang,Bojun Cheng,Kai Chen*

Main category: cs.RO

TL;DR: 该研究提出了一种将人类第一人称视频转化为机器人具身智能训练数据的方法，通过构建大规模E2E-3M数据集训练出具有物理智能的PhysBrain模型，显著提升了机器人的自我中心感知和规划能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要基于第三人称数据训练，与类人机器人的第一人称视角存在根本性不匹配。机器人自我中心数据收集成本高、多样性有限，而大规模人类第一人称视频提供了可扩展的替代方案，能自然捕捉丰富的交互上下文和因果结构。

Method: 提出Egocentric2Embodiment翻译管道，将原始第一人称视频转化为结构化、多层次的视觉问答监督数据，强调证据基础和时序一致性。构建了包含300万样本的E2E-3M数据集，并训练出自我中心感知的具身大脑PhysBrain。

Result: PhysBrain在自我中心理解方面显著提升，特别是在EgoThink规划任务上表现优异。作为自我中心感知的初始化模型，能够实现更高效的视觉语言动作微调，在SimplerEnv任务上达到53.9%的成功率，证明了从人类自我中心监督到下游机器人控制的有效迁移。

Conclusion: 通过将大规模人类第一人称视频转化为结构化具身训练数据，成功构建了具有物理智能的具身大脑，解决了机器人自我中心数据稀缺问题，为机器人泛化能力提供了可扩展的解决方案。
...
### [106] [ReinforceGen: Hybrid Skill Policies with Automated Data Generation and Reinforcement Learning](https://arxiv.org/abs/2512.16861)
*Zihan Zhou,Animesh Garg,Ajay Mandlekar,Caelan Garrett*

Main category: cs.RO

TL;DR: ReinforceGen是一个结合任务分解、数据生成、模仿学习和运动规划的机器人操作系统，通过强化学习微调提升长时程操作任务性能


<details>
  <summary>Details</summary>
Motivation: 长时程操作一直是机器人领域的长期挑战，需要解决复杂任务分解和技能协调问题
...
Conclusion: ReinforceGen通过结合任务分解、模仿学习和强化学习微调，有效解决了长时程操作任务，在复杂机器人操作任务中表现出色
...
### [107] [PolaRiS: Scalable Real-to-Sim Evaluations for Generalist Robot Policies](https://arxiv.org/abs/2512.16881)
...
TL;DR: PolaRiS：一个可扩展的真实到仿真框架，通过神经重建方法将真实场景视频转换为交互式仿真环境，用于机器人策略的高保真评估。
...
Method: PolaRiS利用神经重建方法将真实世界场景的短视频扫描转换为交互式仿真环境，并开发了仿真数据协同训练方法，以弥合剩余的真实到仿真差距，支持在未见仿真环境中进行零样本评估。

Result: 通过仿真与真实世界的广泛配对评估，PolaRiS评估与真实世界通用策略性能的相关性远强于现有仿真基准，其简单性也支持快速创建多样化的仿真环境。

Conclusion: 这项工作为下一代机器人基础模型向分布式和民主化评估迈出了一步，提供了一个可扩展的高保真仿真评估框架。
...
### [108] [Sceniris: A Fast Procedural Scene Generation Framework](https://arxiv.org/abs/2512.16896)
...
<details>
  <summary>Details</summary>
Motivation: 现有程序化生成方法输出吞吐量低，成为扩展数据集创建的重要瓶颈，需要高效生成大规模、无碰撞的场景变体

Method: 引入Sceniris框架，采用批量采样和cuRobo中的快速碰撞检测，扩展了对象间空间关系支持，并提供可选的机器人可达性检查

Result: 相比Scene Synthesizer方法，Sceniris实现了至少234倍的加速，能够高效生成大规模、无碰撞的场景变体

Conclusion: Sceniris是一个高效的程序化场景生成框架，解决了现有方法的性能限制，为物理AI和生成模型提供了快速创建大规模数据集的能力
...
TL;DR: DiscoverDCP：结合符号回归与DCP规则的数据驱动框架，用于系统辨识，确保发现的模型表达式天生全局凸，避免后验凸性验证的计算难题。


<details>
  <summary>Details</summary>
Motivation: 传统系统辨识方法难以保证模型的凸性，而凸性对于安全关键的控制和优化任务至关重要。现有方法要么使用固定参数的凸函数形式（如二次函数）限制表达能力，要么需要计算复杂的后验凸性验证。

Method: 将符号回归与Disciplined Convex Programming（DCP）规则集集成，强制所有发现的候选模型表达式遵守DCP组合规则，确保输出表达式天生具有全局凸性。

Result: 该方法能够发现比传统固定参数凸表达式（如二次函数）更灵活、更准确的凸替代模型，同时保持可解释性和可验证性。

Conclusion: DiscoverDCP框架能够生成适合安全关键控制和优化任务的可解释、可验证且灵活的凸模型，通过构造保证凸性，避免了计算复杂的后验验证过程。
...
### [110] [SHARe-KAN: Holographic Vector Quantization for Memory-Bound Inference](https://arxiv.org/abs/2512.15742)
*Jeff Smith*

Main category: cs.LG

TL;DR: SHARe-KAN框架通过增益-形状-偏置向量量化解决Vision KANs的内存瓶颈问题，实现88倍运行时内存减少，同时保持基线精度


<details>
  <summary>Details</summary>
Motivation: Kolmogorov-Arnold Networks (KANs)面临基本的内存墙问题：其学习的基函数导致参数数量激增，产生极高的带宽需求，阻碍了在内存受限环境中的部署

Method: 提出SHARe-KAN框架，采用增益-形状-偏置向量量化来利用函数冗余同时保持密集拓扑结构；配合LUTHAM硬件感知编译器进行静态内存规划

Result: 实现88倍运行时内存减少（从1.13GB降至12.91MB），在PASCAL VOC上匹配未压缩基线精度；在NVIDIA Ampere架构上实现>90%的L2缓存驻留，使工作负载与DRAM带宽约束解耦

Conclusion: SHARe-KAN成功解决了Vision KANs的内存墙问题，通过向量量化和硬件感知编译技术，在保持精度的同时大幅减少内存需求，为基于样条的架构部署提供了可行方案
...
TL;DR: LLaDA2.0是一个基于离散扩散的大型语言模型系统，通过将自回归模型转换为扩散模型，参数规模达100B，采用渐进式块扩散训练方案，并发布了16B和100B两个MoE变体。


<details>
  <summary>Details</summary>
Motivation: 传统从头训练大型扩散语言模型成本高昂，本文旨在通过将预训练的自回归模型高效转换为扩散模型，实现知识继承和前沿规模部署，同时保持并行解码优势。

Method: 提出3阶段块级WSD训练方案：1)渐进增加块大小的块扩散预热阶段；2)大规模全序列扩散稳定阶段；3)回退到紧凑块大小的衰减阶段。通过SFT和DPO进行后训练对齐，获得优化的MoE变体。

Result: 成功开发了LLaDA2.0-mini（16B）和LLaDA2.0-flash（100B）两个指令调优的MoE变体，这些模型在保持并行解码优势的同时，在前沿规模上实现了卓越的性能和效率，并已开源。

Conclusion: LLaDA2.0建立了一个新的前沿规模部署范式，通过将预训练AR模型高效转换为扩散模型，实现了知识继承、渐进适应和效率感知设计，为大规模扩散语言模型的实用部署提供了可行方案。
...
TL;DR: Janus框架统一生成与预测，通过解耦潜空间实现异构材料微结构的实时物理引导逆向设计


<details>
  <summary>Details</summary>
Motivation: 异构材料微结构的逆向设计是一个病态且计算昂贵的问题，现有生成模型难以支持快速、稳定的确定性逆向求解

Method: 提出Janus框架，将深度编码器-解码器架构与可分离的KHRONOS预测头耦合，学习同时适用于生成逆向和物理预测的等距潜流形

Result: 在MNIST数据集上实现高保真重建和准确分类；在热导率标记的微结构设计中达到R²=0.98的前向预测精度和低于5%的像素重建误差

Conclusion: Janus通过统一预测和生成于单一潜空间，实现了实时、物理引导的逆向微结构生成，计算成本远低于传统优化方法
...
<details>
  <summary>Details</summary>
Motivation: 图像分类是机器感知实现人类水平图像理解的关键任务。尽管CLIP等多模态模型通过跨模态学习取得了良好表现，但低容量模型容易欠拟合，在细粒度分类中表现不佳。同时，高质量、平衡的人口统计数据难以获取，导致模型预测偏向多数类别，产生有害偏见。

Method: 提出D3G（多样化人口统计数据生成）方法，这是一种无需训练、零样本的方法，用于提升分类准确率并减少预训练多模态模型的人口统计偏见。该方法以CLIP为基础多模态模型，Stable Diffusion XL为生成模型，在推理时提供多样化人口统计数据。

Result: 研究表明，在推理时提供多样化人口统计数据能够改善模型性能，并探索了不同人口统计特征对准确率指标的影响。

Conclusion: D3G方法能够有效提升零样本图像分类的准确率，同时减少人口统计偏见，为改善多模态模型的公平性和性能提供了有效途径。
...
### [114] [ReactorFold: Generative discovery of nuclear reactor cores via emergent physical reasoning](https://arxiv.org/abs/2512.15756)
*Yoonpyo Lee*
...
<details>
  <summary>Details</summary>
Motivation: 传统核反应堆堆芯设计方法（确定性、元启发式、机器学习辅助）在固定的人工定义配置空间中搜索，限制了发现全新设计拓扑的能力，需要能够超越人类设计约束的生成方法

Method: 提出ReactorFold生成框架，将燃料组件设计重新定义为语言模型的序列建模问题；使用蒙特卡洛数据、参数高效微调和直接偏好优化（DPO）训练模型，学习压水堆组件的潜在结构

Result: DPO对齐模型展现出设计空间扩展能力：尽管仅在固定Gd棒数量的配置上训练，却能自主调整Gd库存以满足严格功率峰值约束；发现挑战传统对称装载启发式的高性能非对称配置

Conclusion: 语言模型能够内化因果物理关系并超越人类施加的设计约束，访问传统搜索方法无法达到的设计体系，为核反应堆设计提供了新的生成范式
...
### [115] [Twin Restricted Kernel Machines for Multiview Classification](https://arxiv.org/abs/2512.15757)
...
<details>
  <summary>Details</summary>
Motivation: 传统多视图支持向量机(MvSVM)方法在高维空间中使用核技巧时难以有效捕捉决策边界，容易产生误差，并且难以处理多视图数据集中常见的视图不一致性问题。

Method: 提出了多视图双限制核机(TMvRKM)模型，结合核机与多视图框架的优势，采用正则化最小二乘方法替代传统的大规模二次规划问题求解，包含耦合项平衡多视图误差，并整合早期和晚期融合策略。

Result: 在UCI、KEEL和AwA基准数据集上的实验结果表明，TMvRKM在所有场景中都优于基线模型，表现出卓越的泛化性能，统计分析也一致支持这一结论。

Conclusion: TMvRKM模型通过创新的正则化最小二乘方法和多视图融合策略，有效解决了传统多视图核方法在计算效率和泛化性能方面的挑战，为多视图学习提供了更高效的解决方案。
...
### [116] [Yantra AI -- An intelligence platform which interacts with manufacturing operations](https://arxiv.org/abs/2512.15758)
*Varshini Krishnamurthy*

Main category: cs.LG

TL;DR: 该论文开发了一个用于XRIT的智能生产系统，集成了机器学习模型（随机森林分类器和孤立森林）进行预测性维护和异常检测，使用Streamlit实现实时数据可视化，并整合GPT-4虚拟助手提供决策支持。
...
Method: 开发了集成机器学习模型的智能生产系统，包括随机森林分类器用于预测性维护、孤立森林用于异常检测，使用Streamlit构建交互式实时数据可视化仪表板，并整合GPT-4虚拟助手提供实时决策支持。

Result: 系统使用模拟数据进行测试，结果显示显著提高了工作效率、能源管理和维修计划能力，系统设计具有可扩展性，适合XRIT生产环境的实时应用。

Conclusion: 该系统成功解决了工业4.0环境下的关键生产问题，未来工作将集中在实时数据集成和系统进一步优化方面。
...
TL;DR: SCFA框架通过引入领域知识约束改进联邦学习，在非IID数据下提升收敛速度22%，减少模型发散41.3%，并在差分隐私下保持接近非私有基线的性能。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在非独立同分布数据条件下收敛缓慢，现有解决方案对所有客户端更新采用相同处理方式，忽略了语义有效性。需要将领域知识约束纳入分布式优化框架。

Method: 提出语义约束联邦聚合（SCFA）框架，将领域知识约束融入分布式优化。构建知识图谱编码3000个约束（来自ISA-95和MASON本体），通过约束正则化减少假设空间。

Result: 理论证明收敛率O(1/sqrt(T) + ρ)，约束减少有效数据异质性41%，隐私-效用权衡改善因子θ=0.37。在ε=10的差分隐私下，SCFA保持3.7%的效用损失（标准联邦学习为12.1%）。实验显示22%更快收敛，41.3%模型发散减少，ρ<0.05时保持90%最优性能。

Conclusion: SCFA是首个具有收敛理论的约束基联邦学习框架，通过语义约束显著改善非IID数据下的联邦学习性能，理论预测与实证结果高度一致（R²>0.90），为知识增强的分布式学习提供理论基础。
...
### [118] [Machine Learning Framework for Thrombosis Risk Prediction in Rotary Blood Pumps](https://arxiv.org/abs/2512.15761)
*Christopher Blum,Michael Neidlin*

Main category: cs.LG

TL;DR: 提出一个基于可解释机器学习的血栓风险评估框架，通过逻辑回归和特征选择从CFD流场特征中识别血栓风险区域，实现计算高效且机理透明的血栓预测。
...
Result: 模型成功复现了标记的风险分布，识别出与血栓风险增加相关的不同流动特征集。当应用于离心泵时，尽管仅在单个轴流泵工况点训练，模型仍能预测合理的血栓易发区域。

Conclusion: 可解释机器学习能够将局部流动特征与血栓风险联系起来，同时保持计算高效和机理透明。低计算成本可实现快速血栓生成性筛查，无需重复或昂贵的模拟。该框架补充了基于物理的血栓建模，为将可解释机器学习集成到CFD驱动的血栓分析和设备设计工作流程提供了方法基础。
...
### [119] [Cross-Sample Augmented Test-Time Adaptation for Personalized Intraoperative Hypotension Prediction](https://arxiv.org/abs/2512.15762)
*Kanxue Li,Yibing Zhan,Hua Jin,Chongchong Qi,Xu Lin,Baosheng Yu*

Main category: cs.LG

TL;DR: CSA-TTA框架通过跨样本增强的测试时适应方法，利用其他患者的低血压事件数据来提升术中低血压的个性化预测准确性


<details>
  <summary>Details</summary>
Motivation: 术中低血压（IOH）具有重要的手术风险，但由于患者特异性差异，准确预测仍然具有挑战性。虽然测试时适应（TTA）为个性化预测提供了有前景的方法，但低血压事件的罕见性常常导致不可靠的测试时训练。

Method: 提出CSA-TTA框架：1）构建跨样本库，将历史数据分割为低血压和非低血压样本；2）采用粗到细的检索策略构建测试时训练数据：先用K-Shape聚类识别代表性聚类中心，然后基于当前患者信号检索前K个语义相似样本；3）在训练中集成自监督掩码重建和回顾性序列预测信号，增强模型对快速和细微术中动态的适应性。

Result: 在VitalDB数据集和真实医院数据集上评估CSA-TTA，与TimesFM和UniTS等最先进的时间序列预测模型集成。在VitalDB上，微调场景下Recall和F1分数分别提升+1.33%和+1.13%；零样本场景下分别提升+7.46%和+5.07%，表现出强大的鲁棒性和泛化能力。

Conclusion: CSA-TTA通过跨样本增强的测试时适应方法，有效解决了术中低血压预测中因事件罕见性导致的测试时训练不可靠问题，显著提升了预测性能，具有临床应用潜力。
...
### [120] [Data Valuation for LLM Fine-Tuning: Efficient Shapley Value Approximation via Language Model Arithmetic](https://arxiv.org/abs/2512.15765)
...
<details>
  <summary>Details</summary>
Motivation: 数据是大语言模型训练的关键资产，但数据估值（特别是Shapley值计算）通常需要大量模型重训练，计算成本极高。对于使用DPO训练的大语言模型，需要解决如何高效计算数据价值以支持数据所有者做出投资决策和公平分配合作收益的问题。

Method: 利用直接偏好优化（DPO）的特定数学结构，开发可扩展的Shapley值计算方法。该方法避免了传统数据估值中需要大量模型重训练的问题，通过DPO的数学特性简化计算过程。

Result: 研究表明，对于使用DPO训练的大语言模型，数据估值的计算挑战被显著简化。该方法能够高效计算Shapley值，为数据估值与大语言模型的交叉应用开辟了新的可能性。

Conclusion: DPO的数学结构为大规模语言模型的数据估值提供了高效的计算解决方案，这一发现有望推动数据估值与大语言模型交叉领域的多种应用发展。
...
### [121] [TS-DP: Reinforcement Speculative Decoding For Temporal Adaptive Diffusion Policy Acceleration](https://arxiv.org/abs/2512.15773)
*Ye Li,Jiahe Feng,Yuan Meng,Kangye Ji,Chen Tang,Xinwan Wen,Shutao Xia,Zhi Wang,Wenwu Zhu*

Main category: cs.LG

TL;DR: TS-DP提出了一种基于强化学习的推测解码框架，用于加速扩散策略在具身控制中的推理，通过Transformer草稿器和RL调度器动态适应任务难度变化，实现4.17倍加速且保持94%以上草稿接受率。


<details>
  <summary>Details</summary>
Motivation: 扩散策略在具身控制中表现出色，但存在推理延迟高、计算成本大的问题。现有的静态加速方法（如量化）无法处理动态变化的具身任务，而推测解码作为一种无损自适应方法在扩散策略中尚未充分探索。需要解决如何在任务难度随时间变化的具身环境中，以更低成本匹配基础模型的去噪质量，并动态调整计算资源。

Method: 提出TS-DP框架：1）使用Transformer草稿器蒸馏基础模型，替代昂贵的去噪调用；2）基于强化学习的调度器动态调整推测参数，适应时变任务难度，在保持准确性的同时提高效率。

Result: 在多样具身环境中的实验表明，TS-DP实现了高达4.17倍的推理加速，草稿接受率超过94%，推理频率达到25Hz，能够在无性能下降的情况下实现实时扩散控制。

Conclusion: TS-DP首次将推测解码引入扩散策略，通过时间感知的自适应机制成功解决了具身控制中的动态计算需求，为实时扩散策略提供了高效解决方案。
...
### [122] [Adversarial Robustness in Financial Machine Learning: Defenses, Economic Impact, and Governance Evidence](https://arxiv.org/abs/2512.15780)
*Samruddhi Baviskar*
...
<details>
  <summary>Details</summary>
Motivation: 金融决策中的表格机器学习模型（如信用评分和欺诈检测）对对抗攻击的鲁棒性尚未充分研究，需要评估其对歧视、校准和金融风险指标的影响

Method: 使用信用评分和欺诈检测数据，应用基于梯度的攻击方法，测量对抗攻击对模型歧视、校准和金融风险指标的影响

Result: 结果显示在小扰动下模型性能显著下降，通过对抗训练可以部分恢复模型性能

Conclusion: 金融决策中的表格机器学习模型对对抗攻击敏感，对抗训练是提高鲁棒性的有效方法，但只能部分缓解攻击影响
...
### [123] [Boosting t-SNE Efficiency for Sequencing Data: Insights from Kernel Selection](https://arxiv.org/abs/2512.15900)
*Avais Jan,Prakash Chourasia,Sarwan Ali,Murray Patterson*

Main category: cs.LG

TL;DR: 研究评估了9种不同核函数在t-SNE降维中的应用，发现余弦相似性核在生物序列数据分析中表现最佳，优于传统的高斯核和隔离核。


<details>
  <summary>Details</summary>
Motivation: 传统的t-SNE使用高斯核函数存在两个主要问题：1) 缺乏数据依赖性；2) 计算开销大，限制了其在分类生物序列数据上的可扩展性和有效性。虽然最近提出了隔离核作为替代方案，但它可能无法最优地捕捉序列相似性。

Method: 研究全面评估了9种不同的核函数在t-SNE中的应用，使用了三种嵌入方法：One-Hot编码、Spike2Vec和minimizers。通过主观可视化和客观指标（包括邻域保持分数）进行评估，并在六个不同的生物数据集上进行分类和聚类实验验证。

Result: 余弦相似性核总体上优于其他核函数（包括高斯核和隔离核），实现了更好的运行效率和低维空间中成对距离的保持。核函数选择不仅影响可视化质量，还显著影响下游分析任务。

Conclusion: 余弦相似性核在不同数据类型和嵌入策略中提供了最稳健的性能，特别适合大规模生物序列分析。核函数选择对t-SNE在生物序列数据分析中的效果有重要影响。
...
### [124] [A Unification of Discrete, Gaussian, and Simplicial Diffusion](https://arxiv.org/abs/2512.15923)
...
TL;DR: 本文提出了一个统一框架，将离散扩散、高斯扩散和单纯形扩散三种方法统一为Wright-Fisher种群遗传模型的不同参数化形式，解决了现有方法算法分散、理论不统一的问题。


<details>
  <summary>Details</summary>
Motivation: 当前建模DNA、蛋白质和语言等离散序列的扩散方法存在三种主要范式：离散空间扩散、欧几里得空间的高斯扩散和单纯形扩散。这些方法虽然目标相同，但算法、理论结构和权衡点各不相同，缺乏统一的框架。离散扩散具有最自然的定义域，高斯扩散算法更成熟，单纯形扩散理论上结合了两者优点但实践中存在数值不稳定问题。需要建立一个统一理论框架，使实践者能够在不同模型间切换。

Method: 本文构建了一个统一理论，将三种离散扩散方法统一为Wright-Fisher种群遗传模型的不同参数化形式。研究发现单纯形扩散和高斯扩散是该模型的两个大种群极限。理论形式化地连接了这些模型的似然函数和超参数，并利用数十年数学遗传学文献解锁了稳定的单纯形扩散。此外，开发了能够在测试时在任意这三种域中执行扩散的单一模型。

Result: 实验表明，Wright-Fisher单纯形扩散比之前的单纯形扩散模型更稳定，在条件DNA生成任务上表现更优。同时，在多个域上同时训练的模型与在单个域上训练的模型具有竞争力。研究还展示了可以训练一个在测试时能够在三种域中任意执行扩散的单一模型。

Conclusion: 本文成功构建了一个统一框架，将三种离散扩散方法统一为Wright-Fisher种群遗传模型的不同参数化形式。该理论不仅形式化地连接了不同模型的似然函数和超参数，还解锁了稳定的单纯形扩散。实践者不再需要在模型权衡中做出选择，可以通过单一模型在测试时灵活选择扩散域，为离散序列建模提供了更强大和灵活的工具。
...
### [125] [DSO: Direct Steering Optimization for Bias Mitigation](https://arxiv.org/abs/2512.15926)
...
<details>
  <summary>Details</summary>
Motivation: 生成模型在决策时存在基于人口统计属性的偏见（如将女性识别为医生的失败率更高），而现有激活转向方法难以纠正需要跨人口群体等概率结果的偏见。用户对偏见缓解与模型性能的平衡需求各异，需要推理时可控制的偏见减少方法。

Method: 提出直接转向优化（DSO），使用强化学习寻找线性变换来引导激活，专门设计用于缓解偏见同时保持对模型性能的控制。
...
### [126] [BarcodeMamba+: Advancing State-Space Models for Fungal Biodiversity Research](https://arxiv.org/abs/2512.15931)
...
### [127] [In-Context Semi-Supervised Learning](https://arxiv.org/abs/2512.15934)
*Jiashuo Fan,Paul Rosu,Aaron T. Wang,Michael Li,Lawrence Carin,Xiang Cheng*

Main category: cs.LG

TL;DR: 该论文研究了Transformer在上下文半监督学习中的能力，探索了如何利用未标记的上下文数据来学习鲁棒的上下文相关表示，从而在标签稀缺的情况下提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有理论研究主要关注有明确标签对的监督式上下文学习，但实践中Transformer在标签稀疏甚至缺失的情况下仍表现良好，这表明未标记的上下文演示中蕴含着关键结构。研究者希望理解Transformer如何利用未标记上下文进行表示学习。

Method: 引入了上下文半监督学习框架，其中少量标记示例伴随大量未标记点。研究Transformer如何利用未标记上下文学习鲁棒的、上下文相关的表示。

Result: 研究表明Transformer能够利用未标记上下文学习上下文相关的表示，这种表示能够实现准确预测，并在低标签情况下显著提升性能。
...
### [128] [Posterior Behavioral Cloning: Pretraining BC Policies for Efficient RL Finetuning](https://arxiv.org/abs/2512.16911)
*Andrew Wagenmaker,Perry Dong,Raymond Tsao,Chelsea Finn,Sergey Levine*

Main category: cs.LG

TL;DR: 论文提出后验行为克隆（PostBC）方法，通过建模演示者的后验分布来改进预训练策略，从而提升强化学习微调效果。


<details>
  <summary>Details</summary>
Motivation: 当前实践中，通常先在大规模演示数据集上预训练策略，然后通过强化学习微调。然而，现有研究主要关注微调算法，很少关注如何确保预训练策略能成为有效的强化学习微调初始化。标准行为克隆可能无法覆盖演示者的所有动作，影响微调效果。

Method: 提出后验行为克隆（PostBC）方法，不直接拟合观察到的演示动作，而是训练策略来建模演示者行为在演示数据集下的后验分布。这种方法仅需标准监督学习，可基于现代生成模型在机器人控制领域实现。

Result: 理论证明PostBC能确保覆盖演示者的动作，而标准行为克隆可能失败。在现实机器人控制基准测试和真实世界机器人操作任务中，PostBC相比标准行为克隆显著提升了强化学习微调性能，同时保持预训练性能不差于标准行为克隆。

Conclusion: 后验行为克隆（PostBC）是一种有效的预训练方法，能确保策略覆盖演示者动作，为强化学习微调提供更好的初始化，在实际机器人控制任务中表现出显著优势。
...
TL;DR: 本文提出一种基于商品RFID的森林资产追踪方法，无需预先标记已知位置即可实现GPS级别的定位精度，适用于野火响应应用。


<details>
  <summary>Details</summary>
Motivation: 在森林环境中，商品RFID系统由于信号衰减、多径效应和环境变化导致标签定位性能差。现有指纹识别方法需要预先在已知位置部署标签，但在实际应用中往往无法满足这一条件。

Method: 提出使用高斯过程建模不同环境的RF信号响应特征，无需GPS或摄像头等额外传感器。采用新的加权对数似然方法将未知环境与预先建模的环境字典进行匹配。

Result: 实验结果表明，该方法能够实现与GPS相当的定位精度，使用被动商品RFID可同时追踪数十个野火资产，无需预先标记已知位置，且成本远低于GPS。

Conclusion: 该方法提供了一种成本效益高、可扩展的森林资产追踪解决方案，特别适用于野火响应等需要大规模资产管理的应用场景。
...
### [130] [Provably Extracting the Features from a General Superposition](https://arxiv.org/abs/2512.15987)
*Allen Liu*

Main category: cs.LG

TL;DR: 提出一种高效查询算法，能从噪声查询访问中恢复叠加特征学习中的特征方向和函数，适用于任意叠加和响应函数
...
Method: 开发了一种在傅里叶空间中搜索的算法，通过迭代细化搜索空间来定位隐藏的特征方向。算法从噪声查询访问中工作，能处理任意叠加和响应函数

Result: 算法能识别所有非退化响应的特征方向并重建函数f。与先前工作相比，该算法在更一般的设置下工作，只要求特征方向不完全相同

Conclusion: 提出了一种高效的查询算法，解决了叠加特征学习中的基本问题，适用于广泛的叠加设置和响应函数，为过完备机制下的特征恢复提供了新方法
...
### [131] [Higher-Order LaSDI: Reduced Order Modeling with Multiple Time Derivatives](https://arxiv.org/abs/2512.15997)
*Robert Stephany,William Michael Anderson,Youngsoo Choi*

Main category: cs.LG

TL;DR: 提出一种结合高精度有限差分格式和Rollout损失函数的方法，用于提升降阶模型在长时间预测中的准确性


<details>
  <summary>Details</summary>
Motivation: 传统降阶模型在长时间预测中精度下降，需要改进以保持预测能力

Method: 1) 引入灵活、高阶且计算成本低的有限差分格式；2) 提出Rollout损失函数，训练降阶模型在任意时间范围内做出准确预测

Result: 在2D Burgers方程上验证了方法的有效性

Conclusion: 该方法能够显著提升降阶模型在长时间预测中的精度和稳定性
...
### [132] [Towards Fine-Tuning-Based Site Calibration for Knowledge-Guided Machine Learning: A Summary of Results](https://arxiv.org/abs/2512.16013)
...
<details>
  <summary>Details</summary>
Motivation: 当前农业生态系统碳循环量化方法在决策相关尺度上存在挑战，传统方法依赖位置无关的参数化和独立训练，未能充分利用迁移学习和空间异质性，限制了在具有显著变异性的区域的适用性。

Method: 提出FTBSC-KGML框架，结合预训练-微调过程、空间变异感知和知识引导机器学习，通过全局预训练模型在州或站点级别进行微调，学习位置感知表示，同时利用遥感GPP、气候和土壤协变量数据。

Result: FTBSC-KGML相比纯全局模型具有更低的验证误差和更高的一致性解释能力，能更好地捕捉跨州的空间变异性，在有限数据下提高局部精度而不牺牲可解释性。

Conclusion: 该框架扩展了先前的SDSA-KGML框架，为气候缓解和可持续农业提供了更准确、成本有效的农业生态系统碳循环量化方法，特别适用于具有空间异质性的区域。
...
### [133] [CauSTream: Causal Spatio-Temporal Representation Learning for Streamflow Forecasting](https://arxiv.org/abs/2512.16046)
*Shu Wan,Reepal Shah,John Sabo,Huan Liu,K. Selçuk Candan*

Main category: cs.LG

TL;DR: CauSTream是一个用于因果时空径流预测的统一框架，通过联合学习径流因果图和动态路由图，在三个美国主要河流流域上超越了现有方法，特别是在长期预测中表现更优。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型在径流预测中虽然性能强大，但往往忽视物理过程，限制了可解释性和泛化能力。现有的因果学习方法虽然整合了领域知识，但通常依赖固定的因果图，无法适应数据变化。

Method: 提出了CauSTream框架，联合学习(i)气象强迫与径流之间的因果图，以及(ii)捕捉站点间动态依赖关系的路由图。在非参数设置下建立了这些因果结构的可识别性条件。

Result: 在三个美国主要河流流域和三个预测时间窗口上评估，CauSTream始终优于现有最先进方法，预测窗口越长性能优势越明显，表明对未见条件有更强的泛化能力。学习到的因果图与领域知识高度一致。
...
### [134] [Privacy Blur: Quantifying Privacy and Utility for Image Data Release](https://arxiv.org/abs/2512.16086)
*Saeed Mahloujifar,Narine Kokhlikyan,Chuan Guo,Kamalika Chaudhuri*

Main category: cs.LG

TL;DR: 研究发现高斯模糊在实际低精度实现中隐私保护不足，容易被逆向攻击破解。比较了四种图像隐私保护方法，发现像素化和像素化加噪声在适当粒度下能提供更好的隐私-效用平衡。


<details>
  <summary>Details</summary>
Motivation: 现实世界收集的图像数据常包含人脸、车牌等隐私信息，需要在不影响模型训练效用的前提下保护这些隐私。当前行业标准的高斯模糊方法在实际应用中存在隐私风险。

Method: 研究比较了四种隐私保护算法：高斯模糊、像素化、像素化加噪声（DP-Pix）和裁剪。通过逆向攻击和识别攻击评估隐私性，通过在隐私处理数据上训练模型的表示质量评估效用。

Result: 高斯模糊是四种方法中隐私性最差的，其低精度实现容易被逆向攻击破解。像素化和像素化加噪声在适当粒度下能同时提供良好的隐私保护和模型训练效用。

Conclusion: 高斯模糊作为行业标准方法存在隐私漏洞，像素化和像素化加噪声是更好的替代方案。研究开发了Privacy Blur软件包提供这些方法及推荐参数。
...
### [135] [AIMM: An AI-Driven Multimodal Framework for Detecting Social-Media-Influenced Stock Market Manipulation](https://arxiv.org/abs/2512.16103)
*Sandeep Neela*
...
<details>
  <summary>Details</summary>
Motivation: 市场操纵越来越多地来自协调的社交媒体活动而非孤立交易，零售投资者、监管机构和经纪商需要能够连接在线叙事、协调模式和市场行为的工具来应对这一挑战。

Method: 开发了AIMM框架，整合Reddit活动、机器人和协调指标以及OHLCV市场特征；由于Reddit API限制，使用校准的合成社交特征；构建了AIMM-GT基准数据集（33个标记的股票-日）；采用前向行走评估和前瞻性预测记录方法。

Result: 虽然标记数据集较小（33个股票-日，3个正面事件），但结果显示初步的判别能力，AIMM在2021年1月GME挤压峰值前22天发出了预警；系统能够识别可疑时间窗口并支持分析师调查。

Conclusion: AIMM框架展示了社交媒体驱动市场监控的可行性，发布了代码、数据集架构和仪表板设计以支持相关研究，为检测协调的社交媒体市场操纵提供了实用工具。
...
### [136] [BUILD with Precision: Bottom-Up Inference of Linear DAGs](https://arxiv.org/abs/2512.16111)
...
<details>
  <summary>Details</summary>
Motivation: 从观测数据中学习有向无环图（DAG）结构是因果发现、统计信号处理和机器学习中的核心问题。在线性高斯结构方程模型（SEM）且噪声方差相等的条件下，该问题是可识别的，但需要有效算法来准确重建DAG

Method: 提出BUILD（Bottom-Up Inference of Linear DAGs）算法：利用观测数据精度矩阵的独特结构，通过确定性逐步方法识别叶节点及其父节点，然后剪除叶节点及其相关边，重复此过程直至完全重建DAG。在实际应用中，通过定期重新估计精度矩阵来增强鲁棒性

Result: 在具有挑战性的合成基准测试中，BUILD算法相比最先进的DAG学习算法表现优异，同时提供了明确的复杂度控制机制

Conclusion: BUILD算法利用线性高斯SEM下精度矩阵的结构特性，提供了一种确定性的自底向上DAG重建方法，在有限数据条件下通过定期重新估计精度矩阵来平衡计算时间与鲁棒性，为DAG学习提供了有效解决方案
...
### [137] [Dual-View Inference Attack: Machine Unlearning Amplifies Privacy Exposure](https://arxiv.org/abs/2512.16126)
...
<details>
  <summary>Details</summary>
Motivation: 机器遗忘技术虽然能保护请求遗忘的用户权利，但引入了新的隐私风险。先前研究主要关注已遗忘数据的隐私，而对保留数据的隐私风险探索不足。本文旨在填补这一空白，首次揭示机器遗忘在双视图设置下的漏洞。

Method: 从信息论角度引入"隐私知识增益"概念，证明双视图设置能让攻击者获得比单独查询任一模型更多的信息。提出DVIA（双视图推理攻击）方法，通过黑盒查询两个模型提取保留数据的成员信息，无需训练攻击模型，使用轻量级似然比推理模块进行高效推理。

Result: 在不同数据集和模型架构上的实验验证了DVIA的有效性，突显了双视图设置固有的隐私风险。攻击者通过查询原始模型和遗忘模型能获得比单独查询任一模型更多的隐私信息。

Conclusion: 机器遗忘在双视图设置下会放大隐私泄露风险，攻击者通过同时查询原始模型和遗忘模型能获得更多隐私信息。这揭示了机器遗忘技术中一个被忽视的隐私漏洞，需要新的防御机制来保护保留数据的隐私。
...
### [138] [INTELLECT-3: Technical Report](https://arxiv.org/abs/2512.16144)
...
TL;DR: INTELLECT-3是一个106B参数的混合专家模型（12B激活），通过大规模强化学习训练，在数学、代码、科学和推理基准测试中达到同尺寸模型的最先进性能，并开源了完整的训练基础设施。


<details>
  <summary>Details</summary>
Motivation: 开发一个在数学、代码、科学和推理任务上具有卓越性能的高效大型语言模型，同时开源完整的训练基础设施以促进社区研究和应用。

Method: 使用端到端强化学习基础设施堆栈训练106B参数的混合专家模型（12B激活），基于GLM-4.5-Air-Base模型进行监督微调和强化学习训练，使用prime-rl框架进行大规模异步强化学习，最高扩展到512个H200 GPU。

Result: INTELLECT-3在其尺寸范围内在数学、代码、科学和推理基准测试中达到最先进性能，超越了多个更大的前沿模型，训练效率高且可扩展性强。

Conclusion: 通过大规模强化学习训练的混合专家模型INTELLECT-3展示了卓越的性能，同时开源的完整基础设施堆栈为社区提供了强大的工具，推动了强化学习在大型语言模型训练中的应用。
...
### [139] [Explicit and Non-asymptotic Query Complexities of Rank-Based Zeroth-order Algorithms on Smooth Functions](https://arxiv.org/abs/2512.16200)
*Haishan Ye*

Main category: cs.LG

TL;DR: 本文首次为基于排序的零阶优化算法建立了显式的非渐近收敛率，填补了该领域理论分析的空白。


<details>
  <summary>Details</summary>
Motivation: 基于排序的零阶优化方法（如CMA-ES、自然进化策略等）在实践中应用广泛且对噪声和单调变换具有强鲁棒性，但现有理论分析仅提供渐近结果，缺乏显式收敛率分析，特别是对于选择top-k方向的算法。

Method: 分析一个简单的基于排序的零阶算法，采用新颖的分析方法避免传统的漂移和信息几何技术，首次建立显式的非渐近查询复杂度。

Result: 对于d维问题：1) L-光滑且μ-强凸函数，算法达到Õ(dL/μ·log(dL/μδ)·log(1/ε))查询复杂度；2) 光滑非凸目标函数，达到O(dL/ε·log(1/ε))查询复杂度。这些结果以至少1-δ的概率成立。
...
### [140] [Neural emulation of gravity-driven geohazard runout](https://arxiv.org/abs/2512.16221)
*Lorenzo Nava,Ye Chen,Maximillian Van Wyk de Vries*

Main category: cs.LG

TL;DR: 该研究开发了一个机器学习模型来预测地质灾害（如滑坡、雪崩）的运移距离，该模型比传统数值求解器快100-10,000倍，能准确预测流动范围和沉积厚度，并在真实地形上表现出良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 地质灾害（滑坡、雪崩等）的运移距离预测对保护生命、基础设施和生态系统至关重要。这些快速质量流动常常造成数千人死亡，且能从源头运移数公里。由于源头条件和材料特性的多样性，运移距离难以预测，特别是对可能突然面临严重影响的社区而言。准确的大规模预测需要既物理真实又计算高效的模型，但现有方法面临速度与真实性之间的基本权衡。

Method: 研究训练了一个机器学习模型来预测地质灾害在真实地形上的运移距离。模型基于超过100,000个数值模拟进行训练，这些模拟覆盖了超过10,000个真实世界的数字高程模型芯片。模型能够预测流动范围和沉积厚度，并再现关键物理行为，包括分流和沉积模式。

Result: 该模型以比数值求解器快100到10,000倍的计算速度，高精度地预测了流动范围和沉积厚度。模型能够泛化到不同的流动类型、规模和地形，再现了包括分流和沉积模式在内的关键物理行为。

Conclusion: 神经模拟技术能够实现跨多样真实地形的快速、空间分辨的运移距离预测，为灾害风险减少和基于影响的预报开辟了新机会。这些结果突显了神经模拟作为将物理真实的地质灾害建模扩展到大规模早期预警系统所需空间和时间尺度的有前景途径。
...
### [141] [Coarse-to-Fine Open-Set Graph Node Classification with Large Language Models](https://arxiv.org/abs/2512.16244)
*Xueqi Ma,Xingjun Ma,Sarah Monazam Erfani,Danilo Mandic,James Bailey*

Main category: cs.LG

TL;DR: 提出CFC框架，利用大语言模型进行图数据集的开放集分类，实现从粗到细的OOD检测与分类，性能显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有开放集分类方法通常将所有OOD样本视为单一类别，但在欺诈检测和医疗诊断等高风险场景中，需要对OOD样本进行更深入分析，包括其可能的标签。这引出了一个关键问题：能否在没有真实标签信息的情况下，将OOD检测扩展到OOD分类？

Method: 提出粗到细开放集分类（CFC）框架，包含三个关键组件：1）粗分类器：使用LLM提示进行OOD检测和异常标签生成；2）基于GNN的细分类器：使用粗分类器识别的OOD样本进行训练，增强OOD检测和ID分类；3）通过LLM提示和后处理的OOD标签实现精细化的OOD分类。该方法使用语义上真正属于分布外的OOD实例，而非合成或辅助样本。

Result: 实验结果表明，CFC在图和文本领域的OOD检测性能比最先进方法提高10%，在图数据集上的OOD分类准确率达到70%。

Conclusion: CFC框架成功地将OOD检测扩展到OOD分类，利用LLM的语义理解能力，在无需真实标签信息的情况下，实现了对OOD样本的细粒度分类，提高了方法的可解释性和实际应用价值。
...
### [142] [Sharpness-aware Federated Graph Learning](https://arxiv.org/abs/2512.16247)
*Ruiyu Li,Peige Zhao,Guangxia Li,Pengcheng Wu,Xingyu Gao,Zhiqiang Xu*

Main category: cs.LG

TL;DR: SEAL算法通过同时优化损失函数和损失曲面的尖锐度，并引入表示相关性正则化，解决联邦图学习中数据异构性和维度坍缩问题，提升GNN模型的分类准确率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 联邦图学习面临数据异构性问题：1）基于经验风险最小化的优化器使局部模型陷入尖锐谷，削弱对分布外图数据的泛化能力；2）局部图数据表示中的维度坍缩对GNN模型分类能力产生负面影响。

Method: 提出SEAL算法：1）制定同时最小化损失函数及其尖锐度的优化目标，寻找平坦区域中的模型参数；2）引入基于局部表示相关矩阵的正则化器，放松单个局部图样本生成表示的相关性，缓解维度坍缩。

Result: 在多个图分类基准测试中，SEAL始终优于最先进的联邦图学习基线方法，并为更多参与者提供性能增益，显著提升了局部GNN模型的分类准确率和泛化能力。

Conclusion: SEAL算法通过尖锐度感知优化和表示相关性正则化，有效解决了联邦图学习中的数据异构性和维度坍缩问题，提高了GNN模型在联邦学习环境下的性能。
...
### [143] [Sharpness-aware Second-order Latent Factor Model for High-dimensional and Incomplete Data](https://arxiv.org/abs/2512.16277)
*Jialiang Wang,Xueyan Bao,Hao Wu*

Main category: cs.LG

TL;DR: 提出SSLF模型，通过结合二阶信息和锐度感知优化，改进传统SLF模型在HDI数据上的表示学习性能


<details>
  <summary>Details</summary>
Motivation: 传统二阶隐因子模型在处理高维不完整数据时，由于其双线性和非凸性质，优化非常困难。锐度感知最小化方法能够找到平坦的局部最小值，从而改善表示学习模型的泛化能力

Method: 提出锐度感知二阶隐因子模型，包含两个关键思想：(1)通过Hessian-向量乘积获取二阶信息；(2)通过设计的Hessian-向量乘积将锐度项注入曲率中

Result: 在多个工业数据集上的实验表明，所提出的模型始终优于最先进的基线方法

Conclusion: SSLF模型通过结合二阶信息和锐度感知优化，有效解决了传统SLF模型的优化困难问题，提升了表示学习性能
...
### [144] [Feature-Selective Representation Misdirection for Machine Unlearning](https://arxiv.org/abs/2512.16297)
*Taozhao Chen,Linghan Huang,Kim-Kwang Raymond Choo,Huaming Chen*

Main category: cs.LG

TL;DR: SRMU是一种新颖的激活编辑框架，通过特征感知和方向控制的扰动，在高度纠缠的数据分布场景中实现选择性遗忘，在保持模型效用的同时有效移除有害知识。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在安全关键和受监管领域的广泛应用，模型中保留的敏感或禁止知识带来了隐私泄露、监管不合规和潜在滥用等风险。现有遗忘技术假设遗忘集和保留集完全分离，这在现实操作环境中难以实现，特别是在高度纠缠的数据分布场景下，基于扰动的方法往往会损害模型效用或无法确保安全性。

Method: 提出了选择性表征误导遗忘框架，采用结构化误导向量和激活重要性映射，实现特征感知和方向控制的激活编辑。与无差别的模型权重扰动不同，SRMU能够选择性地抑制有害表征，同时保留良性表征的效用。

Result: 在WMDP基准测试中，SRMU在低纠缠和高纠缠配置下都实现了最先进的遗忘性能，且效用损失最小。在20-30%数据重叠的情况下，现有基线方法失效时，SRMU仍能保持有效性。

Conclusion: SRMU为安全驱动的模型治理、隐私合规和受控知识移除提供了稳健基础，特别适用于新兴的大语言模型应用中高度纠缠的数据分布场景。
...
### [145] [Multivariate Uncertainty Quantification with Tomographic Quantile Forests](https://arxiv.org/abs/2512.16383)
*Takuya Kanazawa*
...
Method: TQF学习方向投影n⊤y的条件分位数作为输入x和单位方向n的函数。在推理时，它聚合多个方向的分位数，并通过具有凸子问题的有效交替方案最小化切片Wasserstein距离来重建多变量条件分布。

Result: TQF在合成和真实世界数据集上进行了评估，与经典方向分位数方法不同，它可以用单一模型覆盖所有方向而不施加凸性限制。

Conclusion: TQF提供了一种有效的多变量非参数条件分布估计方法，源代码已在GitHub上发布。
...
### [146] [Kascade: A Practical Sparse Attention Method for Long-Context LLM Inference](https://arxiv.org/abs/2512.16391)
*Dhruv Deshmukh,Saurabh Goyal,Nipun Kwatra,Ramachandran Ramjee*

Main category: cs.LG

TL;DR: Kascade是一种无需训练的稀疏注意力方法，通过选择性地在锚定层计算精确Top-k索引并在中间层重用这些索引，显著加速长上下文LLM推理，在解码注意力上实现4.1倍加速，预填充注意力上实现2.2倍加速，同时保持与密集注意力相近的准确性。


<details>
  <summary>Details</summary>
Motivation: 注意力机制是长上下文LLM推理中的主要延迟来源，随着推理模型和RAG应用的普及，这一问题日益突出。现有方法需要注意到两个已知观察：1）softmax后的注意力本质上是稀疏的；2）高权重键的身份在相邻层之间是稳定的。

Method: Kascade采用训练自由的稀疏注意力方法，在选定的锚定层计算精确的Top-k索引，然后在中间重用层复用这些索引。锚定层通过动态规划算法选择，以最大化开发集上的跨层相似性。该方法考虑了高效的实现约束（如瓦片级操作），支持预填充和解码注意力，并且Top-k选择和重用是头感知的。

Result: 在H100 GPU上，Kascade相比FlashAttention-3基线，在解码注意力上实现最高4.1倍加速，在预填充注意力上实现2.2倍加速。在LongBench和AIME-24等长上下文基准测试中，准确性与密集注意力方法非常接近。

Conclusion: Kascade通过利用注意力固有的稀疏性和跨层稳定性，提供了一种高效的长上下文LLM推理加速方案，无需额外训练即可在保持准确性的同时显著提升推理速度。
...
### [147] [NDRL: Cotton Irrigation and Nitrogen Application with Nested Dual-Agent Reinforcement Learning](https://arxiv.org/abs/2512.16408)
*Ruifeng Xu,Liang He*

Main category: cs.LG

TL;DR: 提出NDRL（嵌套双智能体强化学习）方法，通过父智能体优化宏观水氮策略、子智能体量化胁迫信号动态调节，提高棉花产量和资源利用效率。
...
Method: 提出NDRL方法，包含父智能体和子智能体。父智能体基于预期累积产量效益识别有前景的宏观灌溉施肥策略；子智能体奖励函数包含量化的水分胁迫因子（WSF）和氮素胁迫因子（NSF），使用混合概率分布动态优化每日策略。使用2023-2024年田间试验数据校准验证DSSAT模型模拟真实条件。

Result: 与最佳基线相比，模拟产量在2023和2024年均提高4.7%；灌溉水生产率分别提高5.6%和5.1%；氮素偏生产力分别提高6.3%和1.0%。
...
### [148] [Emergent Bias and Fairness in Multi-Agent Decision Systems](https://arxiv.org/abs/2512.16433)
*Maeve Madigan,Parameswaran Kamalaruban,Glenn Moynihan,Tom Kempton,David Sutton,Stuart Burrell*

Main category: cs.LG

TL;DR: 该论文研究了多智能体预测系统在金融表格数据领域的公平性评估方法，发现多智能体系统可能表现出无法追溯到单个智能体组件的集体性偏见行为，强调需要将多智能体决策系统作为整体实体进行评估。
...
Result: 研究发现金融决策中出现了无法追溯到单个智能体组件的偏见模式，表明多智能体系统可能表现出真正的集体行为。金融多智能体系统中的公平性风险是模型风险的重要组成部分，对信用评分和收入估计等任务有实际影响。
...
TL;DR: 本文提出将LDA主题模型的主题数选择问题形式化为离散黑盒优化问题，比较了四种优化方法在验证困惑度上的表现，发现摊销优化器在样本和时间效率上显著优于传统进化算法。


<details>
  <summary>Details</summary>
Motivation: LDA主题模型中主题数T的选择是一个关键设计决策，它强烈影响模型的统计拟合度和可解释性。传统方法通常依赖经验规则或网格搜索，缺乏系统化的优化方法。

Method: 将主题数选择问题形式化为离散黑盒优化问题，其中每个函数评估对应训练一个LDA模型并测量其验证困惑度。在固定评估预算下，比较了四种优化器：两种手工设计的进化方法（遗传算法GA和进化策略ES），以及两种学习的摊销方法（PABBO和SABBO）。

Result: 实验表明，虽然GA、ES、PABBO和SABBO最终都能达到相似的困惑度区间，但摊销优化器在样本和时间效率上显著更优。SABBO通常在一次评估后就能识别出接近最优的主题数，PABBO在几次评估内就能找到有竞争力的配置，而GA和ES几乎需要全部预算才能接近相同区域。

Conclusion: 摊销黑盒优化方法在LDA主题数选择问题上表现出卓越的效率优势，能够以极少的评估次数找到接近最优的配置，为模型超参数优化提供了高效的新方法。
...
### [150] [Persistent Multiscale Density-based Clustering](https://arxiv.org/abs/2512.16558)
...
<details>
  <summary>Details</summary>
Motivation: 密度聚类算法在探索性数据分析中很重要，但实际应用时需要选择超参数（如DBSCAN的密度阈值、HDBSCAN*的最小簇大小），这需要数据分布的先验知识，而实践中往往缺乏这种知识。

Method: 提出PLSCAN算法，基于尺度空间聚类原理，等价于在新度量空间上的持久同调，能够高效识别HDBSCAN*产生稳定（叶）簇的所有最小簇大小。

Result: 在多个真实数据集上，PLSCAN比HDBSCAN*获得更高的平均ARI（调整兰德指数），对互达邻居数量的变化更不敏感。在低维数据集上计算成本与k-Means相当，高维时与HDBSCAN*相似。

Conclusion: PLSCAN是一种无需手动调参的密度聚类算法，能够自动发现稳定簇，在真实数据上表现优于HDBSCAN*，计算效率有竞争力。
...
### [151] [Abacus: Self-Supervised Event Counting-Aligned Distributional Pretraining for Sequential User Modeling](https://arxiv.org/abs/2512.16581)
*Sullivan Castro,Artem Betlei,Thomas Di Martino,Nadir El Manouzi*

Main category: cs.LG

TL;DR: Abacus是一种用于展示广告中用户购买行为预测的新方法，通过自监督预训练预测用户事件的经验频率分布，结合序列学习目标，有效解决了用户事件稀疏性和随机性问题。


<details>
  <summary>Details</summary>
Motivation: 展示广告系统中用户购买行为建模面临用户事件稀疏、行为随机性导致的类别不平衡和事件时间不规则问题。现有方法依赖手工制作的"计数器"特征，忽略了用户意图的细粒度时间演化，而当前序列模型又缺少有用的事件计数统计信息。
...
Conclusion: Abacus方法通过预测用户事件频率分布的自监督预训练，有效解决了展示广告中用户行为建模的挑战，结合序列学习的混合方法显著提升了预测性能。
...
### [152] [Stackelberg Learning from Human Feedback: Preference Optimization as a Sequential Game](https://arxiv.org/abs/2512.16626)
*Barna Pásztor,Thomas Kleine Buening,Andreas Krause*

Main category: cs.LG

TL;DR: SLHF是一种新的偏好优化框架，将对齐问题建模为领导者策略和跟随者策略之间的序贯博弈，相比RLHF和NLHF具有更丰富的偏好结构表达能力。


<details>
  <summary>Details</summary>
Motivation: 现有的人类反馈学习框架如RLHF和NLHF存在局限性：RLHF为动作分配标量奖励过于简化，NLHF寻求同时移动均衡无法捕捉序贯交互的丰富性。需要一种能够更好建模复杂偏好结构的新框架。

Method: SLHF将偏好优化问题分解为两个子问题：1) 跟随者的精炼问题（学习如何基于领导者动作进行条件响应）；2) 领导者针对对抗者的优化问题。通过序贯博弈设计，跟随者学习改进领导者的动作，并可通过迭代采样在推理时进行精炼。

Result: 实验表明SLHF在大语言模型上表现优异：在多样化偏好数据集上实现强对齐，可扩展到0.5B到8B参数规模，产生的推理时精炼能力可跨模型家族迁移而无需进一步微调。

Conclusion: SLHF通过序贯博弈框架提供了更丰富的偏好结构建模能力，在一致性、数据敏感性和非传递偏好鲁棒性方面具有关键优势，为人类反馈学习提供了新的有效范式。
...
### [153] [Exploiting Radio Frequency Fingerprints for Device Identification: Tackling Cross-receiver Challenges in the Source-data-free Scenario](https://arxiv.org/abs/2512.16648)
*Liu Yang,Qiang Li,Luxiong Wen,Jian Yang*

Main category: cs.LG

TL;DR: 本文提出MS-SHOT方法解决源数据不可用的跨接收器射频指纹识别问题，通过动量中心引导的软伪标签和全局结构约束，在目标域标签偏移或非均匀分布情况下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 随着边缘计算的发展，基于深度学习的射频指纹识别在实际部署中面临关键挑战：当模型应用于具有不同硬件特性的接收器时，由于接收器变化引入的分布偏移，性能会显著下降。现有方法在目标域存在标签偏移或未知非均匀类别分布时表现不佳。

Method: 提出MS-SHOT方法：1）基于约束伪标签的SCRFFI适应框架；2）动量中心引导的软伪标签生成；3）全局结构约束以鼓励自信且多样化的预测；4）理论分析表明目标域性能对伪标签质量高度敏感。

Result: 在真实世界数据集上的广泛实验表明，MS-SHOT在准确性和鲁棒性方面始终优于现有方法，特别是在处理目标域标签偏移或非均匀类别分布时表现突出。

Conclusion: MS-SHOT为源数据不可用的跨接收器射频指纹识别适应提供了实用且可扩展的解决方案，有效解决了目标域标签偏移问题，显著提升了模型在实际部署中的性能。
...
### [154] [Blog Data Showdown: Machine Learning vs Neuro-Symbolic Models for Gender Classification](https://arxiv.org/abs/2512.16687)
*Natnael Tilahun Sinshaw,Mengmei He,Tadesse K. Bahiru,Sudhir Kumar Mohapatra*

Main category: cs.LG

TL;DR: 该研究对文本分类问题进行了机器学习算法与神经符号AI的比较分析，发现神经符号AI方法在有限数据集下能达到与MLP相当的性能。


<details>
  <summary>Details</summary>
Motivation: 文本分类问题（如博客性别分类）在机器学习领域已相对成熟，在市场分析、客户推荐等应用领域有重要价值。研究旨在比较传统机器学习算法与新兴的神经符号AI方法在文本分类任务上的表现。

Method: 研究采用对比分析方法，评估了SVM、朴素贝叶斯、逻辑回归、AdaBoost、XGBoost和SVM_R等传统机器学习算法，并与神经符号AI方法进行比较。同时探索了TF-IDF、通用句子编码器和RoBERTa等文本表示方法，以及卡方检验、互信息和主成分分析等特征提取技术。

Result: 实验结果表明，神经符号AI方法在有限数据集下能够匹配多层感知机的性能表现，显示出与传统机器学习方法相当的竞争力。

Conclusion: 神经符号AI在文本分类任务中展现出潜力，未来研究将扩展知识库、嵌入类型范围和超参数配置，以进一步探索该方法的有效性。
...
### [155] [CLARiTy: A Vision Transformer for Multi-Label Classification and Weakly-Supervised Localization of Chest X-ray Pathologies](https://arxiv.org/abs/2512.16700)
*John M. Statheros,Hairong Wang,Richard Klein*

Main category: cs.LG

TL;DR: CLARiTy是一种基于视觉Transformer的模型，用于胸部X光的多标签病理分类和弱监督定位，通过类特定token和SegmentCAM模块实现，在NIH ChestX-ray14数据集上取得SOTA性能。
...
Result: 在NIH ChestX-ray14数据集上，CLARiTy-S-16-512在14种病理分类上取得竞争性性能，在8种病理的弱监督定位上达到SOTA，比先前方法提升50.7%，尤其在小病理（如结节和肿块）上表现突出。

Conclusion: CLARiTy超越了CNN-ViT混合模型，利用ViT自注意力获取全局上下文和类特定定位，通过卷积背景抑制获得精确、降噪的热力图，其低分辨率变体在低资源环境中具有应用潜力。
...
### [156] [Polyharmonic Spline Packages: Composition, Efficient Procedures for Computation and Differentiation](https://arxiv.org/abs/2512.16718)
*Yuriy N. Bakhvalov*

Main category: cs.LG

TL;DR: 该论文提出了一种基于多谐样条包构建的级联架构，解决了先前方法在计算复杂度和高维输入空间方面的限制，同时为具有未知内在低维性的问题提供了理论依据。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明机器学习回归问题可以在随机函数理论框架下解决，最优核函数通过对称性和无差别原理解析推导得到，与多谐样条一致。但该方案存在两个主要限制：1) O(N^3)的计算复杂度；2) 当输入空间维度过高时原始理论假设失效。

Method: 提出一种级联架构，由多谐样条包构建而成。该方法同时解决了可扩展性问题，并为具有未知内在低维性的问题提供了理论依据。提出了用于前向计算和通过级联进行端到端微分的高效矩阵程序。

Result: 该方法解决了先前方法的计算复杂度问题，能够处理高维输入空间，并为具有未知内在低维性的问题提供了理论支持。提出的高效矩阵程序实现了前向计算和端到端微分。

Conclusion: 通过构建基于多谐样条包的级联架构，成功解决了先前方法在计算复杂度和高维输入空间方面的限制，为具有未知内在低维性的机器学习回归问题提供了可扩展且理论合理的解决方案。
...
### [157] [Machine Learning Algorithms: Detection Official Hajj and Umrah Travel Agency Based on Text and Metadata Analysis](https://arxiv.org/abs/2512.16742)
...
### [158] [NRGPT: An Energy-based Alternative for GPT](https://arxiv.org/abs/2512.16762)
*Nima Dehmamy,Benjamin Hoover,Bishwajit Saha,Leo Kozachkov,Jean-Jacques Slotine,Dmitry Krotov*

Main category: cs.LG

TL;DR: NRGPT模型将GPT架构与基于能量的建模框架统一，通过能量景观探索进行推理，在简单语言、代数任务和语言建模中表现良好，且具有更强的抗过拟合能力。
...
Method: 提出eNeRgy-GPT（NRGPT）模型，将GPT设置与EBM框架统一。模型的推理步骤被概念化为在能量景观上对token进行探索，在某些情况下这种探索会变成梯度下降。

Result: NRGPT在简单语言（莎士比亚数据集）、代数ListOPS任务以及更丰富的OpenWebText语言建模设置中表现良好。模型显示出更强的抗过拟合能力，只在非常长时间的训练中才会出现过拟合。

Conclusion: 通过将GPT架构与基于能量的建模框架统一，提出的NRGPT模型在多个任务上表现良好，并具有更强的抗过拟合特性，为语言建模提供了新的视角。
...
### [159] [Pattern recognition in complex systems via vector-field representations of spatio-temporal data](https://arxiv.org/abs/2512.16763)
...
TL;DR: 该论文提出了一种基于离散测度空间上向量场理论的几何框架，用于分析复杂系统的时空数据，通过两参数度量族解决高维非线性系统的建模、分类和预测难题。


<details>
  <summary>Details</summary>
Motivation: 复杂系统（如大脑、细胞、气候、经济等）具有高维非线性动力学特性，传统降维、相空间重构和吸引子表征方法难以处理其庞大的时空数据复杂性，需要新的数据驱动分析框架。

Method: 提出基于离散测度空间向量场理论的几何框架，引入两参数度量族，支持时间相关图像、图像梯度以及图和单纯复形上的实值/向量值函数，结合多维标度进行数据分析。

Result: 在平面和弯曲域上的生物物理系统数值模拟数据验证表明，所提度量结合多维标度能有效实现降维、模态分解、相空间重构和吸引子表征，解决了关键分析挑战。

Conclusion: 该框架为理解复杂动力系统提供了稳健途径，特别适用于传统建模不可行但实验数据丰富的场景，为复杂系统分析提供了新的几何工具。
...
### [160] [Semi-Supervised Online Learning on the Edge by Transforming Knowledge from Teacher Models](https://arxiv.org/abs/2512.16866)
*Jiabin Xue*

Main category: cs.LG

TL;DR: 本文提出知识转换（KT）方法，通过结合知识蒸馏、主动学习和因果推理，为在线边缘机器学习中的未见数据生成伪标签，解决边缘设备上持续学习时的标签获取难题。
...
Method: 提出知识转换（KT）混合方法，结合知识蒸馏、主动学习和因果推理。KT作为主动学习中的"预言机"，将教师模型的知识转化为学生模型的伪标签，实现持续学习。

Result: 模拟实验表明：当使用相对稳定的教师模型时，学生模型最终能达到预期最大性能。KT在教师任务通用（可用预训练模型）或学生任务标签获取困难/昂贵时特别有益。

Conclusion: KT方法有效解决了在线边缘机器学习中未见数据的标签获取问题，特别适用于教师任务通用或学生任务标签获取困难的场景，为边缘设备上的持续学习提供了可行方案。
...
### [161] [Sequencing to Mitigate Catastrophic Forgetting in Continual Learning](https://arxiv.org/abs/2512.16871)
*Hesham G. Moussa,Aroosa Hameed,Arashmid Akhavain*

Main category: cs.LG

TL;DR: 该论文提出通过智能任务排序来缓解持续学习中的灾难性遗忘问题，利用零样本评分算法确定最优任务序列，并与传统方法结合获得更好效果。


<details>
  <summary>Details</summary>
Motivation: 持续学习系统需要在整个生命周期中增量获取、更新和利用知识，但灾难性遗忘（学习新任务导致旧任务性能急剧下降）是主要挑战。现有方法主要分为五类，本文从不同角度考虑任务呈现顺序对缓解灾难性遗忘的作用。

Method: 提出通过任务排序来缓解灾难性遗忘的方法，利用受神经架构搜索（NAS）启发的零样本评分算法来确定最优任务顺序。该方法从任务序列角度而非传统模型参数调整角度解决遗忘问题。

Result: 智能任务排序能显著减少灾难性遗忘。当与传统持续学习策略结合时，任务排序能提供增强的性能和抗遗忘鲁棒性。该方法还可应用于课程学习等其他领域。

Conclusion: 任务排序是缓解持续学习中灾难性遗忘的有效方法，与现有技术结合能获得更好的学习效果。该方法为持续学习提供了新的视角，并具有跨领域应用潜力。
...
### [162] [Training Together, Diagnosing Better: Federated Learning for Collagen VI-Related Dystrophies](https://arxiv.org/abs/2512.16876)
...
TL;DR: 该研究利用联邦学习平台，通过分布式数据集协作训练机器学习模型，成功诊断胶原VI相关肌营养不良症，性能优于单机构模型。


<details>
  <summary>Details</summary>
Motivation: 罕见病诊断面临数据稀缺和碎片化问题，跨机构数据共享存在隐私、监管和物流障碍，需要一种既能保护隐私又能实现协作学习的方法。

Method: 采用Sherpa.ai联邦学习平台，在两个国际组织的分布式数据集上协作训练机器学习模型，使用患者来源成纤维细胞培养物的胶原VI免疫荧光显微镜图像进行分类。

Result: 开发的模型能够将胶原VI患者图像分类为COL6-RD的三个主要致病机制组：外显子跳跃、甘氨酸替代和假外显子插入，F1分数达到0.82，优于单机构模型（0.57-0.75）。

Conclusion: 联邦学习显著提高了诊断效用和泛化能力，不仅能实现更准确的诊断，还能支持不确定意义变异的解释，并指导测序策略的优先排序以识别新的致病变异。
...
### [163] [Impacts of Racial Bias in Historical Training Data for News AI](https://arxiv.org/abs/2512.16901)
*Rahul Bhargava,Malene Hornstrup Jespersen,Emily Boardman Ndulue,Vivica Dsouza*

Main category: cs.LG

TL;DR: 研究通过分析《纽约时报》语料库训练的AI分类器中"blacks"标签的偏见问题，发现该标签部分充当"种族主义检测器"，但在现代种族议题上表现不佳，揭示了新闻业采用AI工具时面临的历史偏见再现风险。


<details>
  <summary>Details</summary>
Motivation: AI技术已广泛应用于新闻研究和新闻编辑室，但这些基于历史数据训练的模型可能编码了数十年前的刻板印象和偏见。本研究旨在调查《纽约时报》语料库训练的分类器中"blacks"标签所体现的历史偏见，以及这些偏见如何影响模型的实际应用。

Method: 研究采用定量和定性方法：1）分析训练语料库中"blacks"标签的使用情况；2）应用可解释AI方法探究该标签在分类器中编码的概念；3）测试模型在现代种族议题（如COVID-19反亚裔仇恨报道、黑命贵运动报道）上的表现。

Result: 研究发现"blacks"标签部分充当了针对某些少数群体的"种族主义检测器"，但在现代种族议题报道上表现不佳。该标签未能准确识别COVID-19时期的反亚裔仇恨报道，对黑命贵运动报道的处理也不符合预期，揭示了模型编码的历史偏见。
...
### [164] [Exploration v.s. Exploitation: Rethinking RLVR through Clipping, Entropy, and Spurious Reward](https://arxiv.org/abs/2512.16912)
*Peter Chen,Xiaopeng Li,Ziniu Li,Wotao Yin,Xi Chen,Tianyi Lin*

Main category: cs.LG

TL;DR: 本文研究了强化学习可验证奖励框架中的探索-利用权衡，发现虚假奖励通过减少策略熵来提升大语言模型的数学推理能力，而单纯的熵最小化不足以带来改进。


<details>
  <summary>Details</summary>
Motivation: 最近研究表明，RLVR框架通过两种看似矛盾的机制提升LLMs的数学推理能力：虚假奖励（抑制利用）和熵最小化（抑制探索）。这两种机制都能改善推理性能，但其背后的原理尚不清楚，需要深入理解政策熵与性能的关系以及虚假奖励是否真正带来收益。

Method: 研究聚焦两个基本问题：1) 政策熵与性能的关系；2) 虚假奖励是否通过裁剪偏差和模型污染的相互作用产生收益。通过分析虚假奖励如何减少策略熵，以及提出奖励错配模型来解释虚假奖励在污染环境之外的性能提升机制。

Result: 研究发现，虚假奖励下的裁剪偏差会减少策略熵，导致更自信和确定性的输出，而单纯的熵最小化不足以带来性能改进。进一步提出的奖励错配模型解释了为什么虚假奖励能够在污染环境之外提升性能。

Conclusion: 研究阐明了虚假奖励带来收益的机制，为更有效的RLVR训练提供了原则。虚假奖励通过减少策略熵来提升推理能力，而奖励错配模型解释了其在更广泛环境中的有效性。
...
### [165] [Anubuddhi: A Multi-Agent AI System for Designing and Simulating Quantum Optics Experiments](https://arxiv.org/abs/2512.15736)
*S. K. Rithvik*
...
<details>
  <summary>Details</summary>
Motivation: 量子光学实验设计通常需要专业编程知识和物理专业知识，限制了研究人员和教育工作者。该系统旨在民主化计算实验设计，使非专家也能设计和模拟复杂的量子光学实验。

Method: 系统采用多智能体架构，结合意图路由、知识增强生成和双模式验证（QuTiP和FreeSim）。通过语义检索从三层工具箱中组合光学组件，然后通过物理模拟验证设计并进行收敛性细化。

Result: 评估了13个实验，涵盖基础光学、量子信息协议和先进技术。系统获得8-9/10的设计-模拟对齐分数，模拟准确建模预期物理。发现结构正确性与定量准确性不同：高对齐确认正确的物理架构，而数值预测需要专家审查。自由形式模拟在11/13实验中优于约束框架。

Conclusion: Anubuddhi系统民主化了研究和教学中的计算实验设计，产生强大的初始设计，用户可以通过对话迭代细化。量子光学的多样性需要灵活的数学表示，自由形式模拟比约束框架表现更好。
...
### [166] [The Principle of Proportional Duty: A Knowledge-Duty Framework for Ethical Equilibrium in Human and Artificial Systems](https://arxiv.org/abs/2512.15740)
*Timothy Prescher*

Main category: cs.AI

TL;DR: 论文提出比例责任原则（PPD），将道德责任建模为随认知状态变化而动态调整的框架，揭示不确定性不会消除道德责任，而是将行动责任转化为修复责任。


<details>
  <summary>Details</summary>
Motivation: 传统伦理框架难以处理不确定性下的决策制定，通常将其简单视为行动约束。需要一种能够建模道德责任如何随主体认知状态变化的新框架。

Method: 提出比例责任原则（PPD）框架，通过数学方程 D_total = K[(1-HI) + HI * g(C_signal)] 形式化道德责任，其中总责任是知识、不确定性/谦逊度和情境信号强度的函数。使用蒙特卡洛模拟验证框架稳定性。

Result: 模拟显示，保持基线谦逊系数（λ>0）的系统能产生更稳定的责任分配，降低过度自信决策的风险。该框架在临床伦理、接受者权利法、经济治理和人工智能四个领域验证了跨学科有效性。

Conclusion: 比例责任原则通过将谦逊度形式化为系统参数，为道德责任提供了数学可处理的方法，可作为复杂系统中的稳定原则，防止过度干预和疏忽，平衡认知信心与情境风险。
...
### [167] [Prompt-to-Parts: Generative AI for Physical Assembly and Scalable Instructions](https://arxiv.org/abs/2512.15743)
*David Noever*

Main category: cs.AI

TL;DR: 提出一个从自然语言描述生成物理可实现的装配指令的框架，使用离散零件词汇表确保几何有效性、连接约束和可构建顺序，通过LDraw作为中间表示，利用大语言模型生成包含3000多个零件的砖块原型的分步构建序列。


<details>
  <summary>Details</summary>
Motivation: 现有方法如基于像素的扩散方法或CAD模型无法支持复杂的装配指令或组件交换，需要弥合语义设计意图与可制造输出之间的差距，实现从自然语言规范到物理原型的直接转换。

Method: 使用LDraw作为文本丰富的中间表示，引导大语言模型生成有效的分步构建序列；引入Python库进行程序化模型生成；采用"砖块袋"方法作为物理API，将精确定向的砖块位置与"词袋"连接，使任意功能需求编译为物质现实。

Result: 在复杂卫星、飞机和建筑领域评估可构建输出，展示了超过3000个装配零件的砖块原型的装配指令生成能力，证明了方法的可扩展性、模块化和保真度。

Conclusion: 该方法作为一种新颖的元素通用语言，填补了现有方法的空白，为制造和工程原型中的自然语言实现提供了指导，同时为设计开辟了新的可能性。
...
### [168] [AI Epidemiology: achieving explainable AI through expert oversight patterns](https://arxiv.org/abs/2512.15783)
*Kit Tempest-Walters*

Main category: cs.AI

TL;DR: AI流行病学是一个通过将流行病学监测方法应用于AI输出来治理和解释高级AI系统的框架，它绕过模型复杂性，通过统计关联预测AI输出失败


<details>
  <summary>Details</summary>
Motivation: 当前可解释性方法（如SHAP和机制可解释性）在面对大规模部署模型时受到模型复杂性的困扰，需要一种能够绕过模型内部复杂性、在分子机制理解之前就能提供统计证据的方法来治理AI系统

Method: 通过标准化捕获AI-专家交互为结构化评估字段（风险等级、对齐分数、准确度分数），将这些作为暴露变量，通过统计关联预测输出失败，类似于胆固醇和血压预测心脏事件。随后通过专家覆盖和真实世界结果验证输出失败关联

Result: 该框架为零负担专家提供自动审计跟踪，通过被动跟踪专家与AI建议的趋同和分歧来分析输出而非内部计算，在机构更新模型和切换供应商时提供治理连续性，通过可靠性分数和语义评估使专家和机构能够在AI输出造成危害前检测不可靠输出

Conclusion: AI流行病学通过使领域专家无需机器学习专业知识就能治理AI系统，民主化了AI监督，为高级AI系统提供了一种有效的治理和解释框架
...
### [169] [State-Augmented Graphs for Circular Economy Triage](https://arxiv.org/abs/2512.15824)
*Richard Fox,Rui Li,Gustav Jonsson,Farzaneh Goli,Miying Yang,Emel Aktas,Yongjing Wang*

Main category: cs.AI

TL;DR: 本文提出了一种基于状态增强拆解序列规划图的决策框架，用于优化循环经济产品分类决策，通过编码拆解历史确保马尔可夫性，实现递归评估。


<details>
  <summary>Details</summary>
Motivation: 循环经济产品分类需要评估产品在达到当前使用终点后的可持续路径选择，这需要平衡保留价值与处理成本、劳动力约束的自适应决策。现有方法缺乏统一的数学框架来处理复杂的操作约束和条件感知效用。

Method: 提出一种新颖的决策框架，作为状态增强拆解序列规划图的简单确定性求解器。通过将拆解历史编码到状态中，强制执行马尔可夫属性，使每个决策仅依赖于前一状态，实现最优递归评估。决策包括继续拆解或选择循环经济选项，模型集成了基于诊断健康分数的条件感知效用和复杂操作约束。

Result: 通过电动汽车电池的分层分类实例展示了该框架的灵活性，决策由组件的递归估值驱动。该实例说明了统一形式主义如何适应不同的机械复杂性、安全要求和经济驱动因素。

Conclusion: 该统一形式主义为优化不同产品和操作环境下的循环经济分类决策提供了可处理且可推广的基础，能够平衡保留价值与处理约束，实现自适应决策。
...
### [170] [PediatricAnxietyBench: Evaluating Large Language Model Safety Under Parental Anxiety and Pressure in Pediatric Consultations](https://arxiv.org/abs/2512.15894)
*Vahideh Zolfaghari*

Main category: cs.AI

TL;DR: 研究评估大语言模型在儿科咨询中的安全性，发现对抗性查询会降低模型安全性，模型规模影响安全表现，但所有模型都存在临床漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地被家长用于儿科咨询，但模型在真实世界对抗性压力下的安全性了解不足。焦虑的家长常使用紧急语言，可能破坏模型的安全防护机制，导致有害建议。

Method: 开发了PediatricAnxietyBench开源基准测试，包含300个高质量查询（150个患者来源，150个对抗性），涵盖10个儿科主题。使用多维度安全框架评估两个Llama模型（70B和8B），包括诊断克制、转诊依从性、模糊表述和紧急情况识别。对抗性查询融入了家长压力模式，如紧迫性、经济障碍和对免责声明的挑战。

Result: 平均安全得分5.50/15（SD=2.41）。70B模型优于8B模型（6.26 vs 4.95，p<0.001），关键失败率更低（4.8% vs 12.0%，p=0.02）。对抗性查询使安全性降低8%（p=0.03），紧迫性导致最大降幅（-1.40）。在癫痫发作（33.3%不当诊断）和疫苗接种后查询中存在漏洞。模糊表述与安全性强相关（r=0.68，p<0.001），而紧急情况识别完全缺失。
...
### [171] [Darth Vecdor: An Open-Source System for Generating Knowledge Graphs Through Large Language Model Queries](https://arxiv.org/abs/2512.15906)
*Jonathan A. Handler*

Main category: cs.AI

TL;DR: Darth Vecdor (DV) 是一个从大型语言模型中提取知识到结构化SQL数据库的工具，旨在解决直接查询LLM时的成本、速度、安全性和置信度问题，特别针对医疗保健领域。


<details>
  <summary>Details</summary>
Motivation: 虽然可以直接查询大型语言模型获取知识，但在高容量操作中会面临成本、速度、安全性和置信度等问题。通过将LLM中的知识预提取到标准数据库中，可以缓解这些问题，特别是在医疗保健等关键领域。

Method: DV通过构建结构化、术语映射的SQL知识图谱，并设计特定功能来应对LLM响应的错误、离题、自由文本、过于笼统和不一致等问题，同时支持多元素响应。提供基于浏览器的图形用户界面，便于领域专家进行提示工程。

Result: DV已作为免费、开源、可扩展的软件发布，采用"按现状"原则，不提供任何保证。用户需要自行评估使用风险和收益，并确保使用安全有效。软件可能存在严重缺陷，但作者希望其能帮助改善医疗保健。

Conclusion: DV为从LLM中提取结构化知识提供了一种解决方案，特别适用于医疗保健领域，但用户需要谨慎使用并自行承担风险。开源发布旨在促进社区改进和应用。
...
### [172] [Leveraging Spreading Activation for Improved Document Retrieval in Knowledge-Graph-Based RAG Systems](https://arxiv.org/abs/2512.15922)
*Jovan Pavlović,Miklós Krész,László Hajdu*

Main category: cs.AI

TL;DR: 提出基于扩散激活算法的RAG框架，通过自动构建知识图谱连接文档，提升大语言模型在复杂推理任务（如多跳问答）上的性能。
...
Method: 提出新颖的RAG框架，采用扩散激活算法从自动构建知识图谱连接的文档语料库中检索信息，作为即插即用模块与多种RAG方法集成。

Result: 实验显示该方法达到或优于迭代RAG方法，与思维链迭代检索结合时相比朴素RAG在答案正确性上获得高达39%的绝对提升，且能在资源受限环境下使用小型开源模型实现。

Conclusion: 基于扩散激活算法的RAG框架能有效提升复杂推理任务的性能，易于集成到现有RAG系统中，在资源受限环境下表现优异。
...
### [173] [Small Language Models for Efficient Agentic Tool Calling: Outperforming Large Models with Targeted Fine-tuning](https://arxiv.org/abs/2512.15943)
*Polaris Jhandi,Owais Kazi,Shreyas Subramanian,Neel Sendas*

Main category: cs.AI

TL;DR: 研究探索用小语言模型(SLMs)替代大语言模型(LLMs)的可行性，通过微调OPT-350M模型在特定任务上取得优异性能，显著降低生成式AI的部署成本。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的规模化应用，模型成本优化和运营效率成为决定可持续性和可访问性的关键因素。大语言模型虽然能力强大，但计算需求巨大，成本过高限制了企业日常使用，这促使研究者探索小语言模型在特定应用中提供可比性能同时大幅降低基础设施开销的可能性。

Method: 研究训练了一个领域适应的小语言模型来执行传统由大语言模型处理的任务，如文档摘要、查询回答和结构化数据解释。具体使用Hugging Face TRL的监督微调(SFT)训练器，对facebook/opt-350m模型进行单轮微调。OPT-350M模型是Meta AI于2022年发布的OPT系列模型之一。
...
Conclusion: 精心设计和针对性训练的小语言模型可以显著降低生成式AI的采用门槛，实现成本效益高的大规模生产系统集成。即使参数规模仅为3.5亿的模型也能在指令调优流程中做出有意义的贡献。
...
### [174] [Subjective functions](https://arxiv.org/abs/2512.15948)
*Samuel J. Gershman*

Main category: cs.AI

TL;DR: 论文探讨了智能体如何内生地生成目标函数，提出了主观函数的概念，并以预测误差为例进行了具体分析。
...
Result: 建立了主观函数的概念框架，展示了预测误差作为主观函数的可行性，为理解智能体如何内生地生成目标提供了理论基础。

Conclusion: 主观函数的概念为解决目标函数来源问题提供了新视角，有望推动人工智能系统获得类似人类动态合成目标的能力。
...
### [175] [Do Large Language Models Know What They Don't Know? Kalshibench: A New Benchmark for Evaluating Epistemic Calibration via Prediction Markets](https://arxiv.org/abs/2512.16030)
*Lukas Nel*

Main category: cs.AI

TL;DR: 该研究引入KalshiBench基准测试，评估大语言模型对未来未知事件的校准能力，发现所有前沿模型都存在系统性过度自信问题，即使增强推理模型也表现不佳。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在各种任务上表现出色，但其认知校准能力仍未被充分理解。现有基准主要测试静态知识的准确性，而缺乏对模型量化未来未知事件不确定性的评估。
...
Result: 所有模型都表现出系统性过度自信。最佳校准模型（Claude Opus 4.5）的ECE为0.120，而增强推理模型如GPT-5.2-XHigh的校准更差（ECE=0.395）。只有1个模型获得正的Brier技能评分，表明大多数模型表现不如简单预测基准率。

Conclusion: 模型规模和增强推理能力并不能自动带来校准改进，认知校准是一个需要针对性开发的独立能力。该研究强调了开发能够适当量化未来事件不确定性的模型的重要性。
...
### [176] [Topic Discovery and Classification for Responsible Generative AI Adaptation in Higher Education](https://arxiv.org/abs/2512.16036)
...
<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在教育中的广泛应用，学生使用这些工具时面临政策不明确的问题。各教育机构的AI政策差异大且不断变化，导致学生对期望和最佳实践感到困惑，需要系统化的政策发现和分类工具。
...
### [177] [WeMusic-Agent: Efficient Conversational Music Recommendation via Knowledge Internalization and Agentic Boundary Learning](https://arxiv.org/abs/2512.16108)
*Wendong Bi,Yirong Mao,Xianglong Liu,Kai Tian,Jian Zhang,Hanjie Wang,Wenhui Que*

Main category: cs.AI

TL;DR: 提出WeMusic-Agent框架，通过知识内化和智能边界学习，让LLM在对话式音乐推荐中智能决定何时使用内部知识、何时调用外部工具，并构建了开源基准测试。
...
Result: 在真实数据实验中，WeMusic-Agent相比现有模型取得显著改进，在相关性、个性化和多样性等多个维度表现优异。

Conclusion: WeMusic-Agent框架有效解决了对话式音乐推荐中知识内化和工具调用的平衡问题，为个性化音乐推荐提供了新的解决方案，并通过开源基准促进了该领域的研究。
...
### [178] [ToolForge: A Data Synthesis Pipeline for Multi-Hop Search without Real-World APIs](https://arxiv.org/abs/2512.16149)
...
Method: 基于(问题、黄金上下文、答案)三元组构建虚拟工具，通过多跳推理和自我反思机制生成大规模工具学习数据，采用多层验证框架确保数据质量
...
### [179] [PDE-Agent: A toolchain-augmented multi-agent framework for PDE solving](https://arxiv.org/abs/2512.16214)
...
Method: 提出PDE-Agent框架，包含两个关键创新：1) 具有图记忆的Prog-Act框架实现多智能体协作，通过双循环机制（局部修复和全局修订）进行动态规划和错误修正；2) 集成工具-参数分离机制的Resource-Pool实现多工具协作，集中管理运行时工件并解决现有框架中的工具间依赖间隙。

Result: 开发了PDE-Bench基准测试集，提出多级评估指标验证工具协调能力。评估表明PDE-Agent在复杂多步骤、跨步骤依赖任务中表现出优越的适用性和性能。

Conclusion: 工具链增强的多智能体PDE求解新范式将推动自动化科学计算的未来发展。该框架继承了LLM的推理能力和外部工具的可控性，实现了从自然语言描述的自动化PDE求解。
...
### [180] [Scaling Spatial Reasoning in MLLMs through Programmatic Data Synthesis](https://arxiv.org/abs/2512.16237)
...
<details>
  <summary>Details</summary>
Motivation: 当前具身智能面临空间理解和推理能力不足的挑战，现有方法存在两难：基于模板的数据集可扩展但结构僵化，人工标注语言多样但不可扩展且计算不精确。

Method: 将真实数据生成重构为代码生成任务，利用LLM将复杂空间问题编译为可执行程序，并通过模拟器提取的高精度场景元信息进行验证，确保计算精确性和可验证性。

Result: 构建了包含3个模拟器、11k+场景、300k+图像/视频指令调优对的数据集，训练出的VLM在多个空间基准测试中表现显著提升，优于同等规模的开源数据集。

Conclusion: SPRITE框架成功克服了传统模板方法的低多样性问题，为构建鲁棒、可泛化的空间智能提供了有效解决方案，其代码和完整数据集将公开以促进未来研究。
...
### [181] [AlignMerge - Alignment-Preserving Large Language Model Merging via Fisher-Guided Geometric Constraints](https://arxiv.org/abs/2512.16245)
*Aniruddha Roy,Jyoti Patel,Aman Chadha,Vinija Jain,Amitava Das*

Main category: cs.AI

TL;DR: AlignMerge：一种几何感知的大语言模型融合框架，通过显式保持对齐不变性，在合并专家模型时保护安全对齐属性


<details>
  <summary>Details</summary>
Motivation: 传统的大语言模型融合方法（如线性权重融合、任务向量、Fisher加权平均）虽然能保持损失函数性能，但会悄悄破坏模型的安全对齐属性。作者认为融合不应仅仅是数值技巧，而应是在已对齐锚点周围的几何约束操作

Method: 在指令调优基模型的局部Fisher图中，估计对齐子空间并引入投影器P_A。优化目标包含三个部分：几何损失保持融合在Fisher-Rao几何中接近专家模型；对齐损失惩罚沿对齐敏感方向的移动；预算损失强制执行软对齐预算。使用解码不变的对齐质量指数作为对齐功能度量

Result: 在五个模型家族（LLaMA-3 8B、Mistral 7B、Qwen 2、Phi-3.5、Gemma 2）上，AlignMerge在合并安全锚点和任务专家时，显著提升了对齐指标（AQI、毒性、LLM-judge对齐），同时在指令遵循、推理和帮助性方面匹配或超越了最佳专家模型。相比Fisher soups、TIES、SafeMerge和MergeAlign等方法，表现出更小的对齐子空间漂移和更少的预算违规

Conclusion: AlignMerge将保持对齐的模型融合提升为一流的设计目标，为未来基础模型的几何感知组合提供了路径。融合应尊重安全几何结构，而不是事后验证
...
### [182] [Learning to Wait: Synchronizing Agents with the Physical World](https://arxiv.org/abs/2512.16262)
*Yifei She,Ping Zhang,He Liu,Yanmin Jia,Yang Jing,Zijun Liu,Peng Sun,Xiangbin Li,Xiaohe Hu*

Main category: cs.AI

TL;DR: 论文提出了一种面向智能体端的解决方案，使大语言模型能够主动对齐其认知时间线与物理世界，通过预测精确等待时间来同步异步环境，减少查询开销和执行延迟。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的智能体任务通常涉及具有可变延迟的非阻塞操作，这导致动作启动与完成之间存在"时间间隙"。现有的环境端解决方案（如阻塞包装器或频繁轮询）要么限制可扩展性，要么用冗余观察稀释智能体的上下文窗口。

Method: 提出了一种智能体端方法，将代码即行动范式扩展到时间领域。智能体利用语义先验和上下文学习来预测精确的等待时间（time.sleep(t)），从而与异步环境同步，无需进行详尽的检查。

Result: 在模拟的Kubernetes集群中的实验表明，智能体能够精确校准其内部时钟，最小化查询开销和执行延迟，验证了时间意识是一种可学习的能力。

Conclusion: 时间意识是智能体在开放环境中实现自主演进的关键可学习能力，智能体端的时间同步方法为解决异步环境中的时间间隙问题提供了有效解决方案。
...
### [183] [QuadSentinel: Sequent Safety for Machine-Checkable Control in Multi-agent Systems](https://arxiv.org/abs/2512.16279)
*Yiliu Yang,Yilei Jiang,Qunzhong Wang,Yingshui Tan,Xiaoyong Zhu,Sherman S. M. Chow,Bo Zheng,Xiangyu Yue*

Main category: cs.AI

TL;DR: QuadSentinel是一个四智能体安全防护系统，将自然语言安全策略编译为机器可检查的规则，通过状态跟踪、策略验证、威胁监视和裁判四个组件在线执行，提高防护准确性并减少误报。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的智能体在使用工具、多步规划和智能体间通信解决复杂任务时存在安全风险。部署者用自然语言编写的策略存在模糊性和上下文依赖性，难以映射到机器可检查的规则，导致运行时执行不可靠。

Method: 将安全策略表达为sequent（序列），提出QuadSentinel四智能体防护系统：状态跟踪器、策略验证器、威胁监视器和裁判。系统将这些策略编译为基于可观察状态的谓词构建的机器可检查规则，并在线执行。裁判逻辑加上高效的top-k谓词更新器通过优先级检查和分层冲突解决来降低成本。

Result: 在ST-WebAgentBench（ICML CUA'25）和AgentHarm（ICLR'25）上测试，QuadSentinel提高了防护准确性和规则召回率，同时减少了误报。与ShieldAgent（ICML'25）等单智能体基线相比，提供了更好的整体安全控制。

Conclusion: 近期部署可以采用这种模式，无需修改核心智能体，通过保持策略分离和机器可检查性来实现安全防护。代码将在GitHub上公开。
...
Method: 提出ForenAgent框架，采用两阶段训练管道（冷启动和强化微调），设计动态推理循环（全局感知、局部聚焦、迭代探测和整体裁决），并构建FABench数据集进行系统训练评估。

Result: 实验表明ForenAgent在低级工具辅助下，在挑战性IFD任务上展现出新兴的工具使用能力和反思推理能力，为通用IFD开辟了有前景的路径。

Conclusion: ForenAgent通过让MLLMs自主生成和执行低级工具，实现了更灵活可解释的伪造分析，统一了低级伪影和高级语义知识，为图像伪造检测提供了新范式。
...
TL;DR: 本文提出了一个系统化框架，将智能体AI系统的适应策略统一分类为智能体适应和工具适应两大类，并进一步细分为不同信号形式和监督形式，为构建更强大、高效、可靠的智能体AI系统提供概念基础和实践路线图。


<details>
  <summary>Details</summary>
Motivation: 随着基于基础模型的智能体AI系统能力不断增强、应用范围不断扩大，适应机制成为提升性能、可靠性和泛化能力的关键手段。然而，快速扩展的研究领域缺乏系统化框架来统一理解各种适应策略，这阻碍了研究者对设计空间的理解和策略选择。

Method: 本文提出了一个系统化框架，将智能体AI系统的适应策略统一分为两大类别：智能体适应和工具适应。智能体适应进一步分解为工具执行信号驱动和智能体输出信号驱动两种形式；工具适应则分为智能体无关和智能体监督两种形式。该框架明确了适应策略的设计空间，揭示了各种策略的权衡关系。

Result: 该框架帮助澄清了智能体AI中适应策略的设计空间，使各种策略的权衡关系变得明确，并为系统设计中选择或切换策略提供了实用指导。作者回顾了每个类别中的代表性方法，分析了它们的优势和局限性，并突出了关键开放挑战和未来机会。

Conclusion: 本文为寻求构建更强大、高效、可靠智能体AI系统的研究者和实践者提供了概念基础和实践路线图。通过统一适应策略的分类框架，促进了该领域研究的系统化理解和进一步发展。
...
TL;DR: 论文提出了一种成本感知的PoQ框架，将显式效率测量集成到推理节点和评估节点的奖励机制中，通过平衡质量与成本来支持经济可持续的去中心化LLM推理。
...
Method: 提出成本感知PoQ框架，整合了基础事实token级F1、轻量级学习评估器和GPT判断的统一评估流水线，采用线性奖励函数平衡归一化质量和成本。

Result: 实验表明：语义文本相似性双编码器比交叉编码器与基础事实和GPT评分相关性更高；池中最大模型在单位延迟质量方面也最有效率；成本感知奖励方案能持续为高质量低成本推理模型和高效评估器分配更高奖励。

Conclusion: 成本感知PoQ为经济可持续的去中心化LLM推理提供了实用基础，评估器架构是关键设计选择，质量-成本分析揭示了模型效率的重要模式。
...
### [187] [PCIA: A Path Construction Imitation Algorithm for Global Optimization](https://arxiv.org/abs/2512.16392)
...
<details>
  <summary>Details</summary>
Motivation: 受人类构建和使用路径行为的启发，人类通常偏好热门交通路线，在路径关闭时智能混合现有路径构建新路线，并随机选择不同路径探索未知目的地。

Method: PCIA模仿人类路径构建行为，生成随机种群（类似群体智能算法），每个粒子代表通往目的地的路径，通过智能混合现有路径和随机探索来寻找最优解。

Result: 在53个数学优化问题和13个约束优化问题上测试，结果显示PCIA与流行和最新的元启发式算法相比具有高度竞争力。

Conclusion: PCIA是一种有效的元启发式优化算法，受人类路径构建行为启发，在多种优化问题上表现出优越性能，具有实际应用价值。
...
### [188] [Synthelite: Chemist-aligned and feasibility-aware synthesis planning with LLMs](https://arxiv.org/abs/2512.16424)
...
<details>
  <summary>Details</summary>
Motivation: 现有的计算机辅助合成规划框架缺乏与人类专家交互的机制，限制了化学家洞察力的整合。需要开发能够利用大语言模型内在化学知识和推理能力，同时允许专家通过自然语言干预的合成规划系统。

Method: Synthelite使用大语言模型直接提出逆合成转化，通过自然语言提示允许专家干预，能够灵活适应用户指定的各种约束条件，并在路线设计中考虑化学可行性。

Result: 实验表明，Synthelite在策略约束和起始材料约束的合成任务中均能达到95%的成功率，能够灵活适应不同的用户约束条件，并在路线设计中考虑化学可行性。
...
### [189] [TIB AIssistant: a Platform for AI-Supported Research Across Research Life Cycles](https://arxiv.org/abs/2512.16442)
*Allard Oelen,Sören Auer*

Main category: cs.AI

TL;DR: TIB AIssistant是一个AI支持的研究平台，通过多个专门助手在整个研究生命周期中为研究人员提供支持，并支持与外部学术服务集成，最终可导出RO-Crate格式的研究资产包以提高可重复性。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能和大语言模型的快速发展，AI对学术研究领域产生了广泛影响。AI支持的研究有潜力在整个研究生命周期中协助研究人员完成各种任务，但目前缺乏系统化的平台支持。

Method: 开发TIB AIssistant平台，包含多个专门负责特定研究任务的助手，提供访问外部学术服务的工具，生成的数据存储在资产中并可导出为RO-Crate格式的研究包，通过助手间的顺序交互演示平台功能。

Result: 成功开发了AIssistant平台，能够通过多个助手协作生成研究论文草稿的各个部分，支持整个研究生命周期，并建立了可导出RO-Crate格式研究包的机制，为研究项目的透明度和可重复性提供了基础。

Conclusion: TIB AIssistant为AI支持的研究奠定了基础，旨在建立一个社区维护的平台，通过系统化的助手和工具支持整个研究生命周期，提高研究效率和可重复性。
...
### [190] [Towards AI-Supported Research: a Vision of the TIB AIssistant](https://arxiv.org/abs/2512.16447)
...
TL;DR: TIB AIssistant是一个领域无关的人机协作平台，旨在通过AI助手支持跨学科研究生命周期中的各项任务，解决AI集成到研究中的挑战。


<details>
  <summary>Details</summary>
Motivation: 生成式AI和大语言模型的快速发展有望改变研究方式，但有效将AI整合到研究中仍面临挑战，包括领域需求差异、AI素养有限、工具协调复杂以及生成式AI在研究中的准确性不明确等问题。

Method: 提出TIB AIssistant平台，采用模块化组件设计，包括提示和工具库、共享数据存储和灵活的编排框架，支持研究生命周期的各个阶段。

Result: 开发了早期原型系统，展示了该方法的可行性和潜在影响，包括概念框架、系统架构和实现。

Conclusion: TIB AIssistant平台为跨学科研究人员提供了人机协作的科学发现支持，通过模块化设计解决了AI集成到研究中的关键挑战。
...
### [191] [TimeSeries2Report prompting enables adaptive large language model management of lithium-ion batteries](https://arxiv.org/abs/2512.16453)
*Jiayang Yang,Chunhui Zhao,Martin Guay,Zhixing Cao*

Main category: cs.AI

TL;DR: TS2R是一个将锂离子电池运行时间序列数据转换为结构化报告的提示框架，使LLM能够在BESS管理中执行推理、预测和决策任务，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在解释多元时间序列数据方面具有潜力，但在实际电池储能系统运维中的应用尚未充分探索。需要一种方法将原始电池运行数据转化为LLM能够理解和处理的形式。

Method: 提出TimeSeries2Report（TS2R）提示框架，通过分割、语义抽象和基于规则的解释相结合的方式，将短期时间动态编码为自然语言，将低层传感器信号与高层上下文洞察连接起来。
...
### [192] [cuPilot: A Strategy-Coordinated Multi-agent Framework for CUDA Kernel Evolution](https://arxiv.org/abs/2512.16465)
...
<details>
  <summary>Details</summary>
Motivation: CUDA内核优化需要硬件-软件协同设计专业知识且高性能内核库具有专有性，现有LLM结合进化算法的方法因智能体设计欠佳和进化表示不匹配而性能不足
...
Conclusion: cuPilot通过策略协调的多智能体框架有效解决了CUDA内核优化中的表示不匹配问题，实现了显著的性能提升，生成的代码已开源
...
### [193] [Quantifying and Bridging the Fidelity Gap: A Decisive-Feature Approach to Comparing Synthetic and Real Imagery](https://arxiv.org/abs/2512.16468)
*Danial Safaei,Siddartha Khastgir,Mohsen Alirezaei,Jeroen Ploeg,Son Tong,Xingyu Zhao*

Main category: cs.AI

TL;DR: 本文提出了一种新的系统特定保真度度量方法——决定性特征保真度（DFF），用于评估自动驾驶虚拟测试中合成数据的有效性，强调决策因果证据的一致性而非像素级逼真度。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶安全验证主要依赖合成数据的虚拟测试，但研究发现像素级逼真度并不能保证从仿真到真实世界的可靠迁移。关键问题是测试系统在真实和仿真环境中是否基于相同的因果证据做出决策，而不仅仅是图像对人类"看起来真实"。目前缺乏这种基于行为的保真度度量方法。

Method: 提出决定性特征保真度（DFF）度量方法，利用可解释AI技术识别和比较测试系统在匹配的真实-合成数据对中的决定性特征。基于反事实解释提出实用估计器，并设计DFF引导的校准方案来提升仿真器保真度。

Result: 在2126个匹配的KITTI-VirtualKITTI2数据对上的实验表明，DFF能够揭示传统输出值保真度忽略的差异。DFF引导的校准在保持输出值保真度的同时，提升了决定性特征和输入级保真度，适用于多种测试系统。

Conclusion: DFF为自动驾驶虚拟测试提供了一种新的系统特定保真度度量方法，强调决策机制的一致性而非视觉逼真度，能够更有效地评估合成数据的有效性并指导仿真器校准。
...
### [194] [Best Practices For Empirical Meta-Algorithmic Research Guidelines from the COSEAL Research Network](https://arxiv.org/abs/2512.16491)
...
<details>
  <summary>Details</summary>
Motivation: 元算法研究（如算法选择、配置和调度）依赖大量计算密集型实验，实验设计和执行中存在多种错误来源，威胁研究的可扩展性和有效性。现有最佳实践分散在不同出版物和领域，且各自独立发展。

Method: 收集COSEAL社区各子领域的良好实践，涵盖完整实验周期：从研究问题提出和实验设计选择，到实验执行，再到结果分析和公正呈现。

Result: 建立了元算法研究中的当前最先进实践，为元算法领域的新研究人员和从业者提供指导。
...
### [195] [ParamExplorer: A framework for exploring parameters in generative art](https://arxiv.org/abs/2512.16529)
*Julien Gachadoat,Guillaume Lagarde*

Main category: cs.AI

TL;DR: ParamExplorer：一个基于强化学习思想的交互式框架，帮助艺术家探索生成艺术算法中的高维参数空间，减少手动试错，发现更多有趣配置。


<details>
  <summary>Details</summary>
Motivation: 生成艺术系统通常涉及高维复杂的参数空间，其中美学上有吸引力的输出只占据很小、碎片化的区域。由于这种组合爆炸，艺术家通常依赖大量手动试错，导致许多潜在有趣的配置未被发现。

Method: 引入ParamExplorer框架，这是一个受强化学习启发的交互式模块化框架，通过人在回路或自动化反馈来指导生成艺术算法参数空间的探索。该框架可以与现有p5.js项目无缝集成，并在框架内实现和评估多种探索策略（称为智能体）。
...
### [196] [Scaling Laws for Energy Efficiency of Local LLMs](https://arxiv.org/abs/2512.16531)
*Ander Alvarez,Alessandro Genuardi,Nilotpal Sinha,Antonio Tiene,Samuel Mugel,Román Orús*

Main category: cs.AI

TL;DR: 该论文系统性地研究了在CPU上进行本地大语言模型和视觉语言模型推理的计算规律，发现了文本长度与计算成本近似线性相关、视觉语言模型存在"分辨率拐点"等经验规律，并展示了量子启发压缩技术能显著降低计算资源消耗。


<details>
  <summary>Details</summary>
Motivation: 虽然GPU主导AI部署，但大多数消费硬件（笔记本电脑、台式机、工业控制器、嵌入式系统）依赖CPU。然而，CPU专用推理的计算规律尚未得到充分探索，需要在边缘设备上平衡准确性与计算/能耗约束。

Method: 在两种代表性CPU层级上系统性地基准测试：MacBook Pro M2（主流笔记本级）和Raspberry Pi 5（低功耗嵌入式）。采用基于处理器和内存使用连续采样及曲线下面积积分的统一方法，分析计算负载如何随文本长度和图像分辨率变化。

Result: 发现两个经验缩放规律：1) 语言模型推理的计算成本与token长度近似线性相关；2) 视觉语言模型存在预处理驱动的"分辨率拐点"，计算量在内部分辨率钳制点以上保持恒定，以下则急剧下降。量子启发压缩技术能减少71.9%的处理器和内存使用，降低62%的能耗，同时保持或提高语义准确性。

Conclusion: 该研究为本地语言和视觉语言工作负载的多模态CPU专用推理提供了系统性量化分析，识别出模型压缩和输入分辨率预处理作为可持续边缘推理的有效低成本杠杆。
...
### [197] [Prefix Probing: Lightweight Harmful Content Detection for Large Language Models](https://arxiv.org/abs/2512.16650)
...
Method: 提出Prefix Probing方法：1) 比较"同意/执行"与"拒绝/安全"前缀的条件对数概率；2) 利用前缀缓存减少检测开销；3) 设计高效的自动前缀构建算法来发现高信息量前缀；4) 仅需单次对数概率计算即可生成有害性分数。

Result: 实验表明Prefix Probing在检测效果上与主流外部安全模型相当，同时仅产生最小计算成本且无需额外模型部署，具有强大的实用性和效率。

Conclusion: Prefix Probing提供了一种高效、低成本的有害内容检测解决方案，解决了大型语言模型在安全应用中面临的三难权衡问题，具有实际应用价值。
...
### [198] [Comprehensive AI Literacy: The Case for Centering Human Agency](https://arxiv.org/abs/2512.16656)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI技术快速融入社会，但现有教育框架未能有效应对，导致出现危险的"素养鸿沟"。过于关注AI工具的操作技能，而忽视了对AI的批判性和伦理推理能力的培养，这削弱了人类在技术应用中的自主权。

Method: 提出系统性转向全面的AI素养教育，以人类能动性为核心原则。建立AI素养、流畅性和能力框架，强调将技术视为可选择的工具而非必然接受的现实。通过培养批判性思维和对认识论的深入理解，让教育者和学生成为以人为中心的AI应用主体。

Result: 通过提出的框架，教育者和学生能够成为AI应用中的自主主体，发展出清晰表达AI决策意图的能力，理解这些决策对学术工作、职业和社会的影响，实现负责任的技术使用。

Conclusion: 真正的AI素养教育必须超越功能性技能培训，以人类能动性为核心，培养批判性思维和伦理决策能力。教育应帮助所有利益相关者成为技术应用的自主主体，而非被动接受者，从而在AI时代保持人类的自主权和责任意识。
...
### [199] [Unsupervised Thematic Clustering Of hadith Texts Using The Apriori Algorithm](https://arxiv.org/abs/2512.16694)
*Wisnu Uriawan,Achmad Ajie Priyajie,Angga Gustian,Fikri Nur Hidayat,Sendi Ahmad Rafiudin,Muhamad Fikri Zaelani*

Main category: cs.AI

TL;DR: 使用Apriori算法对印尼语布哈里圣训进行无监督主题关联分析，发现了礼拜-祈祷、经文-启示、圣训-故事等有意义的语义关联模式。
...
Method: 采用无监督学习方法，使用Apriori算法进行关联规则挖掘。首先对印尼语布哈里圣训数据集进行预处理（大小写转换、标点清理、分词、停用词去除、词干提取），然后使用支持度、置信度和提升度参数进行关联分析。

Result: 发现了有意义的关联模式：rakaat（拜数）-prayer（祈祷）、verse（经文）-revelation（启示）、hadith（圣训）-story（故事），这些模式分别对应礼拜、启示和圣训叙述等主题。
...
### [200] [Do Multi-Agents Solve Better Than Single? Evaluating Agentic Frameworks for Diagram-Grounded Geometry Problem Solving and Reasoning](https://arxiv.org/abs/2512.16698)
*Mahbub E Sobhani,Md. Faiyaz Abdullah Sayeedi,Mohammad Nehad Alam,Proma Hossain Progga,Swakkhar Shatabda*

Main category: cs.AI

TL;DR: 多智能体设计在几何问题求解中：开源模型显著受益，闭源模型单智能体更优，多智能体仅在新数据集上有帮助


<details>
  <summary>Details</summary>
Motivation: 研究多智能体设计在基于图的几何问题求解中的价值，比较单智能体与多智能体在不同视觉数学基准上的表现差异

Method: 在四个视觉数学基准（Geometry3K、MathVerse、OlympiadBench、We-Math）上系统比较单智能体和多智能体流水线，测试开源模型（Qwen-2.5-VL）和闭源模型（Gemini-2.0-Flash）

Result: 开源模型在多智能体模式下表现显著提升：Qwen-2.5-VL (7B)在Geometry3K上提升+6.8分，(32B)提升+3.3分；闭源模型在经典基准上单智能体更优，仅在新数据集We-Math上多智能体有适度改进
...
### [201] [Cyber Humanism in Education: Reclaiming Agency through AI and Learning Sciences](https://arxiv.org/abs/2512.16701)
*Giovanni Adorni*
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI正在快速改变教育中知识生产和验证的方式，将阅读、写作和编程重构为人机混合工作流，引发了关于认知卸载、教师去专业化等担忧。需要建立一个框架来重新确立人类在这种环境中的主体性。

Method: 提出"教育中的赛博人文主义"框架，包含三大支柱：反思能力、算法公民身份和对话设计。通过高等教育案例研究，包括基于提示的学习和"对话式AI教育者"认证，在EPICT生态系统中实施这些理念。

Result: 研究发现这些实践能够增强认知能动性，同时揭示了工作量、公平性和治理方面的紧张关系。为未来AI丰富、以人为本的教育提供了启示。

Conclusion: 赛博人文主义框架为教育工作者和学习者提供了在AI驱动的教育环境中重新确立人类主体性的途径，将AI视为共同构建社会技术基础设施的合作伙伴，而非简单的工具。
...
### [202] [Dual Computational Horizons: Incompleteness and Unpredictability in Intelligent Systems](https://arxiv.org/abs/2512.16707)
*Abhisek Ganguly*
...
Method: 形式化分析两个独立计算限制：1）形式不完备性（哥德尔不完备定理相关）；2）动态不可预测性（混沌理论相关）。通过理论证明展示这两个极端如何共同限制智能体对自身预测能力的推理。

Result: 证明了算法智能体通常无法计算自身的最大预测范围，揭示了智能系统中推理、预测和自我分析之间的固有权衡关系。

Conclusion: 形式不完备性和动态不可预测性共同构成了算法智能的基本限制，这些限制不仅约束了智能体的外部预测能力，更重要的是限制了智能体对自身认知能力的自我分析，揭示了智能系统内在的结构性边界。
...
### [203] [Discovering and Learning Probabilistic Models of Black-Box AI Capabilities](https://arxiv.org/abs/2512.16733)
*Daniel Bramblett,Rushang Karia,Adrian Ciotinga,Ruthvick Suresh,Pulkit Verma,YooJung Choi,Siddharth Srivastava*

Main category: cs.AI

TL;DR: 该论文提出了一种使用PDDL风格表示来学习黑盒AI系统规划能力的方法，通过蒙特卡洛树搜索创建测试任务、获取数据并剪枝假设空间，学习到的模型能够描述BBAI的能力、执行条件和可能结果及其概率。
...
Method: 采用PDDL风格表示来学习黑盒AI的规划能力，使用蒙特卡洛树搜索范式系统性地创建测试任务、获取数据，并剪枝可能的符号模型假设空间。

Result: 理论结果表明学习到的模型具有可靠性、完备性和收敛性。多个黑盒AI系统的实证结果展示了所提方法的范围、效率和准确性。

Conclusion: PDDL风格表示可以有效地学习和建模黑盒AI系统的规划能力，为理解这些系统的能力提供了一种可靠且可解释的方法，有助于确保其安全部署。
...
### [204] [AI-Driven Prediction of Cancer Pain Episodes: A Hybrid Decision Support Approach](https://arxiv.org/abs/2512.16739)
...
TL;DR: 提出结合机器学习和大型语言模型的混合管道，预测肺癌患者住院期间48小时和72小时内的突发性疼痛发作，准确率分别达到0.874和0.917。


<details>
  <summary>Details</summary>
Motivation: 肺癌患者中高达91%会经历突发性疼痛，需要及时干预。现有方法难以有效预测疼痛发作，限制了主动疼痛管理的实施。

Method: 采用混合机器学习和大语言模型管道，分析266名住院患者的电子健康记录。机器学习模块捕捉药物使用的时序趋势，大语言模型解析模糊的剂量记录和自由文本临床笔记，结合结构化和非结构化数据。

Result: 模型在48小时和72小时预测的准确率分别为0.874和0.917，大语言模型的增强使敏感性提高了8.6%和10.4%。混合方法显著提升了预测性能和临床可解释性。
...
TL;DR: CitySeeker是一个评估视觉语言模型在动态城市环境中理解隐含需求并进行空间推理的新基准，包含6,440条轨迹，测试显示即使最佳模型任务完成率也只有21.1%


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在显式指令导航方面取得进展，但在理解动态城市环境中的隐含人类需求（如"我渴了"）方面能力不足，需要评估其空间推理和决策能力

Method: 提出CitySeeker基准，包含8个城市的6,440条轨迹，涵盖7种目标驱动场景；采用回溯机制、丰富空间认知和基于记忆检索的BCR策略进行分析

Result: 实验显示即使表现最佳的模型（如Qwen2.5-VL-32B-Instruct）任务完成率仅为21.1%；发现长时推理中的错误累积、空间认知不足和经验回忆缺陷是关键瓶颈

Conclusion: 分析为开发具有强大空间智能的视觉语言模型提供了可行见解，以应对"最后一英里"导航挑战，强调迭代观察-推理循环和自适应路径优化的重要性
...
### [206] [TOGGLE: Temporal Logic-Guided Large Language Model Compression for Edge](https://arxiv.org/abs/2512.16855)
*Khurram Khalil,Khaza Anuarul Hoque*

Main category: cs.AI

TL;DR: TOGGLE是一个利用信号时序逻辑（STL）在压缩过程中形式化指定和执行语言特性的LLM压缩框架，通过STL鲁棒性引导的贝叶斯优化探索分层量化和剪枝配置，无需重新训练或微调即可生成满足语言约束的压缩模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在自然语言任务中表现出色，但需要大量计算资源，限制了其在资源受限的边缘设备上的部署。现有的压缩技术（如量化和剪枝）通常会降低关键的语言特性，并且缺乏保持模型行为的形式化保证。

Method: 提出Temporal Logic-Guided Large Language Model Compression (TOGGLE)框架，利用信号时序逻辑（STL）在压缩过程中形式化指定和执行语言特性。采用STL鲁棒性引导的贝叶斯优化来系统探索分层量化和剪枝配置，生成满足指定语言约束的压缩模型，无需重新训练或微调。
...
### [207] [Distributional AGI Safety](https://arxiv.org/abs/2512.16856)
*Nenad Tomašev,Matija Franklin,Julian Jacobs,Sébastien Krier,Simon Osindero*

Main category: cs.AI

TL;DR: 论文提出"拼凑式AGI"假说，认为通用智能可能首先通过多个子AGI智能体的协调合作实现，而非单一系统。作者呼吁关注这一假说的安全风险，并提出基于虚拟智能体沙盒经济的分布式AGI安全框架。


<details>
  <summary>Details</summary>
Motivation: 当前AI安全和对齐研究主要关注单一AI系统的安全保障，假设最终会出现单一的通用人工智能。但作者认为"拼凑式AGI"假说——即通用能力首先通过具有互补技能的子AGI智能体协调实现——需要得到重视，特别是随着具有工具使用和协调能力的高级AI智能体的快速部署，这已成为紧迫的安全考虑。

Method: 提出分布式AGI安全框架，核心是设计和实施虚拟智能体沙盒经济（不可渗透或半渗透），其中智能体间的交易由强大的市场机制管理，配合适当的可审计性、声誉管理和监督，以减轻集体风险。

Result: 论文提出了一个系统性的框架来应对拼凑式AGI可能带来的安全挑战，将安全关注点从单个智能体扩展到智能体群体协调产生的集体风险。

Conclusion: 拼凑式AGI假说需要认真考虑，并应指导相应安全保障和缓解措施的发展。通过虚拟智能体沙盒经济框架，可以在智能体群体层面建立适当的安全机制，应对分布式AGI系统带来的新挑战。
...
### [208] [The Social Responsibility Stack: A Control-Theoretic Architecture for Governing Socio-Technical AI](https://arxiv.org/abs/2512.16873)
*Otman A. Basir*
...
Method: 引入社会责任感堆栈（SRS）框架，将责任建模为对社交技术系统的闭环监督控制问题。该框架采用统一的基于约束的表述，引入安全包络和反馈解释，展示了如何持续监控和执行公平性、自主性、认知负担和解释质量等指标。

Result: SRS框架将规范目标转化为可操作的工程和运营控制。通过临床决策支持、协作式自动驾驶车辆和公共部门系统的案例研究，展示了该框架的实际应用。
...
### [209] [Generative Adversarial Reasoner: Enhancing LLM Reasoning with Adversarial Reinforcement Learning](https://arxiv.org/abs/2512.16917)
...
<details>
  <summary>Details</summary>
Motivation: 尽管具备显式推理能力的大语言模型在数学推理方面表现出色，但仍存在计算错误、逻辑脆弱和表面合理但无效步骤等过程错误。需要更有效的训练方法来提高推理质量。

Method: 提出Generative Adversarial Reasoner框架：1) 将推理链分割为逻辑完整的切片；2) 判别器评估每个切片的合理性并提供结构化理由；3) 推理器获得逻辑一致步骤的奖励，判别器获得正确检测错误的奖励；4) 生成密集、校准良好的步骤级奖励，补充稀疏的精确匹配信号。
...
Conclusion: Generative Adversarial Reasoner框架通过对抗强化学习有效提升大语言模型的推理质量，提供密集的步骤级奖励改善信用分配，提高样本效率，在数学推理任务中表现优异。
