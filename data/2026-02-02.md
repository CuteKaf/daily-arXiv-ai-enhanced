...
TL;DR: 本文首次系统评估了开放词汇目标检测模型在航空图像上的零样本性能，发现存在严重的领域迁移失败，语义混淆是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测在自然图像上表现良好，但在航空图像领域的可迁移性尚未被探索。本文旨在建立首个系统性基准，评估现有OVD模型在航空图像上的零样本性能。

Method: 使用LAE-80C航空数据集（3,592张图像，80个类别）评估5个最先进的OVD模型，采用严格的零样本条件。通过全局、Oracle和单类别推理模式分离语义混淆和视觉定位问题。

Result: 结果显示严重的领域迁移失败：最佳模型（OWLv2）仅获得27.6%的F1分数和69%的误报率。将词汇量从80类减少到3.2类可带来15倍改进，表明语义混淆是主要瓶颈。提示工程策略未能提供有意义的性能提升。不同数据集间性能差异巨大（DIOR: F1 0.53, FAIR1M: F1 0.12）。

Conclusion: 这些发现为航空OVD建立了基准预期，并强调了在该领域需要开发领域自适应方法的重要性。语义混淆是当前模型在航空图像上表现不佳的主要限制因素。
...
<details>
  <summary>Details</summary>
Motivation: 当前VQA数据集大多关注真实世界图像或简单图表分析，缺乏针对复杂科学图表的专门基准。现有图表分析数据集要么不包含底层数据，要么假设图表标记与数据存在一一对应关系，而现实中图表是数据的转换（分析、简化、修改），这种差异带来了现有数据集未能捕捉的推理挑战。

Method: 1. 调研现有VQA数据集并指出当前领域的局限性；2. 基于真实数据生成合成直方图；3. 向人类和大型推理模型提出需要访问底层数据才能精确回答的问题；4. 发布包含图表、底层数据、数据生成参数以及所有图表标记和文本边界框的开源数据集。

Result: 创建了一个专门针对科学图表的VQA基准测试数据集，该数据集强调了图表标记与底层数据之间不存在一一对应关系的特点，为未来研究提供了包含完整信息的开源资源。

Conclusion: 需要建立专门的科学图表VQA基准测试，以解决当前数据集在捕捉图表作为数据转换的推理挑战方面的不足，所创建的数据集为这一研究方向提供了重要资源。
...
TL;DR: 本文研究发现视觉语言模型在3D空间理解方面存在显著缺陷，特别是在相对相机姿态估计任务上表现不佳，远低于传统几何方法和人类水平。
...
Method: 1) 引入VRRPI-Bench基准，从无标签的自我中心视频中提取，包含相对相机运动的语言化标注，反映同时包含平移和旋转的真实场景；2) 提出VRRPI-Diag诊断基准，分离各个运动自由度；3) 评估VLMs在相对相机姿态估计任务上的表现。

Result: 大多数VLMs无法超越浅层的2D启发式方法，特别是在深度变化和沿光轴的滚动变换方面表现最差。即使是GPT-5等最先进模型（得分0.64）也远低于经典几何基线（0.97）和人类表现（0.92）。在多图像推理方面，VLMs表现不一致，最佳准确率仅为59.7%。

Conclusion: 视觉语言模型在3D空间基础和多视角空间推理方面存在显著局限性，需要改进其空间理解能力以更好地处理真实世界的3D场景。
...
TL;DR: 该论文从几何视角重新审视视觉Transformer中的位置嵌入，发现它们不仅是token索引，更是塑造表示空间结构的几何先验。作者开发了token级诊断方法，测量多视角几何一致性如何依赖于一致的位置嵌入，并通过14个基础ViT模型的实验揭示了位置嵌入对多视角几何和空间推理的影响。


<details>
  <summary>Details</summary>
Motivation: 重新审视视觉Transformer中位置嵌入的作用，从几何角度理解它们如何影响模型的空间表示能力。传统上位置嵌入被视为简单的token索引，但作者认为它们实际上作为几何先验塑造了表示的空间结构。

Method: 引入token级诊断方法，测量多视角几何一致性如何依赖于一致的位置嵌入。通过14个基础ViT模型进行广泛实验，分析位置嵌入对多视角几何和空间推理的影响。

Result: 实验表明位置嵌入显著影响ViT表示的多视角几何一致性。它们作为因果机制，控制着表示中的空间结构，而不仅仅是简单的token索引。

Conclusion: 位置嵌入在视觉Transformer中扮演着几何先验的角色，它们塑造了表示的空间结构，是控制多视角几何一致性的因果机制。这一发现为理解ViT的空间推理能力提供了新的视角。
...
TL;DR: 单层VQ-VAE在匹配表征预算且避免码书崩溃的情况下，其重建保真度可与分层VQ-VAE相当，挑战了分层量化对高质量重建具有固有优势的假设。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于重新审视分层VQ-VAE（如VQ-VAE2）是否在重建保真度上真正优于单层VQ-VAE。由于高层特征完全来自低层编码，理论上不应包含额外的重建信息，加上最近训练目标和量化机制的进步，需要实证检验单层模型在匹配表征预算下能否达到分层模型的性能。

Method: 方法包括：1）在ImageNet高分辨率图像上比较两层VQ-VAE与容量匹配的单层模型；2）识别单层VQ-VAE的码书利用率不足问题；3）开发轻量级干预措施：数据初始化、定期重置不活跃码书向量、系统调优码书超参数；4）分析高维嵌入对量化稳定性和码书崩溃的影响。

Result: 结果证实：1）码书利用率不足确实限制了单层VQ-VAE性能；2）过高维嵌入会破坏量化稳定性并增加码书崩溃；3）通过数据初始化、定期重置不活跃向量和超参数调优等干预措施可显著减少崩溃；4）当表征预算匹配且码书崩溃得到缓解时，单层VQ-VAE的重建保真度可与分层变体相当。

Conclusion: 结论挑战了分层量化对高质量重建具有固有优势的假设。研究表明，在匹配表征预算并有效缓解码书崩溃的情况下，单层VQ-VAE能够达到与分层模型相当的重建性能，为神经压缩和生成管道等应用提供了更简化的架构选择。
...
TL;DR: VMonarch提出了一种基于Monarch矩阵的高效注意力机制，用于解决视频扩散变换器中注意力计算复杂度高的问题，通过结构化稀疏模式实现次二次方复杂度，在保持生成质量的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 视频扩散变换器（Video DiTs）中注意力机制的二次方复杂度严重限制了上下文可扩展性。研究发现视频DiTs中表现出高度稀疏的时空注意力模式，这些模式可以自然地用Monarch矩阵表示。
...
Result: VMonarch在VBench上经过最小调优后，实现了与完整注意力相当或更优的生成质量。克服了Video DiTs中的注意力瓶颈，将注意力FLOPs减少了17.5倍，在长视频注意力计算中实现了超过5倍的加速，在90%稀疏度下超越了最先进的稀疏注意力方法。

Conclusion: VMonarch通过结构化Monarch矩阵有效处理视频数据中的动态稀疏模式，为视频扩散变换器提供了高效且可扩展的注意力机制，显著提升了长视频生成的计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统渲染流水线依赖复杂资产、精确材质和光照，需要大量计算资源但仍难以实现动态人群场景的可扩展性和真实感

Method: 使用粗糙3D渲染控制场景布局、相机运动和人物轨迹，通过学习的神经渲染器生成真实外观；采用两阶段混合CG-真实训练策略，从大规模真实视频学习生成先验，通过跨域共享隐式时空特征引入可控性

Result: 系统支持从粗糙到精细的控制，能泛化到多样CG和游戏输入，从最小3D输入生成时间一致、可控且真实的城市场景视频

Conclusion: C2R框架通过结合粗糙3D模拟的显式控制和神经渲染的真实生成能力，解决了动态人群场景渲染的可扩展性和真实感问题
...
TL;DR: FlexMap是一种灵活的HD地图构建方法，无需固定相机配置或显式2D到BEV变换，通过几何感知基础模型和跨帧注意力实现3D场景理解


<details>
  <summary>Details</summary>
Motivation: 当前HD地图构建方法需要校准的多相机设置和显式/隐式的2D到BEV变换，当传感器故障或相机配置在不同车辆间变化时变得脆弱，限制了实际部署

Method: 使用几何感知基础模型通过跨帧注意力隐式编码3D场景理解，包含两个核心组件：时空增强模块（分离跨视图空间推理和时间动态）和相机感知解码器（使用潜在相机令牌实现视图自适应注意力）
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型输出对提示变化敏感，存在安全对齐漏洞，需要开发更隐蔽的越狱方法

Method: 1) 利用后训练思维链提示构建隐蔽提示；2) 提出ReAct驱动的自适应噪声机制，基于模型反馈迭代扰动输入图像

Result: 双重策略显著提高攻击成功率，同时在文本和视觉领域保持自然性
...
TL;DR: 该研究评估了基于多视角无标记运动捕捉的概率模型，验证了其置信区间的校准可靠性，能够在不依赖地面真实仪器的情况下识别不可靠输出。


<details>
  <summary>Details</summary>
Motivation: 视频人体运动分析在临床实践和研究中有潜力，但临床实施需要系统不仅准确，还要能产生可靠的置信区间来指示个体准确性。需要评估概率MMMC方法的校准和可靠性。

Method: 基于变分推理估计关节角度后验分布的概率MMMC方法，使用来自两个机构68名参与者的数据，通过仪器化步道和标准标记运动捕捉进行验证，使用期望校准误差(ECE)测量置信区间校准。

Result: 模型表现出可靠的校准，步长和跨步长度的ECE值一般<0.1，步长和跨步长度中位误差分别为~16mm和~12mm，偏置校正运动学误差范围为1.5-3.8度。模型预测的不确定性与观测误差强相关。

Conclusion: 概率模型重建能够量化认知不确定性，使其能够在不需要并发地面真实仪器的情况下识别不可靠输出，为临床实施提供了可靠的不确定性估计。
...
TL;DR: 该论文针对大视觉语言模型在图像描述任务中的物体幻觉问题，提出了一种无需训练的自验证框架，通过语言先验无关验证来缓解模型对语言先验的过度依赖，显著减少了物体幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 尽管大视觉语言模型取得了进展，但在图像描述任务中仍然存在严重的物体幻觉问题，即模型会生成描述不存在的物体，这损害了模型的可靠性。先前的研究将这一问题归因于模型对语言先验的过度依赖，并通过逻辑校准来缓解，但缺乏对过度依赖现象的深入分析。

Method: 论文首先通过初步实验发现，随着生成长度的增加，大视觉语言模型对语言先验的过度依赖会导致幻觉物体标记的概率膨胀，从而加剧物体幻觉。为此，作者提出了语言先验无关验证方法，使模型能够忠实地验证物体存在的置信度。基于此，提出了一个无需训练的自验证框架，该框架首先验证候选描述中物体的存在性，然后通过描述选择或聚合来进一步缓解物体幻觉。

Result: 实验结果表明，该框架在图像描述任务中显著缓解了物体幻觉问题，例如在LLaVA-v1.5-7B模型上，CHAIRI指标提升了65.6%，超越了先前的最先进方法。

Conclusion: 该研究揭示了一条通过挖掘大视觉语言模型自身内在潜力来缓解幻觉问题的新路径，证明了无需额外训练的自验证框架能够有效应对模型对语言先验的过度依赖问题。
...
TL;DR: ScribbleSense：结合多模态大语言模型和图像生成模型，解决涂鸦式3D纹理编辑中的意图模糊和语义定位问题，实现更直观的涂鸦交互编辑。


<details>
  <summary>Details</summary>
Motivation: 现有3D模型纹理编辑方法主要支持基于草图的轮廓交互，而粗粒度的涂鸦式交互应用有限。涂鸦指令的抽象性常导致编辑意图模糊和目标语义位置不明确，需要解决这些问题以提升涂鸦交互的实用性。

Method: 提出ScribbleSense方法，结合多模态大语言模型和图像生成模型。利用MLLMs的视觉能力预测涂鸦背后的编辑意图，然后使用全局生成的图像提取局部纹理细节，锚定局部语义并减轻目标语义位置的模糊性。

Result: 实验结果表明，该方法有效利用了MLLMs的优势，在基于涂鸦的纹理编辑交互中实现了最先进的编辑性能。

Conclusion: ScribbleSense通过结合多模态大语言模型和图像生成技术，成功解决了涂鸦式3D纹理编辑中的意图模糊和语义定位问题，为更直观的涂鸦交互编辑提供了有效解决方案。
...
TL;DR: 该论文提出了一种基于表示对齐投影器的引导方案，通过将投影器预测的表示注入到扩散模型的中间采样步骤中，有效缓解早期去噪阶段的语义漂移问题，显著提升图像合成的语义一致性和视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的生成模型虽然支持可控采样和大规模训练，但推理时的引导方法（如无分类器引导和代表性引导）未能充分利用无监督特征表示。尽管视觉表示包含丰富的语义结构，但由于推理时缺乏真实参考图像，这些表示的集成受到限制。研究发现扩散变换器在早期去噪阶段存在语义漂移问题，随机性导致即使在相同条件下也会出现不一致的对齐。

Method: 提出了一种使用表示对齐投影器的引导方案。该方法通过一个投影器预测表示，并将这些表示注入到中间采样步骤中，为扩散过程提供有效的语义锚点，而无需修改模型架构。这种方法在SiTs和REPAs模型上进行了实验验证。

Result: 在类别条件ImageNet合成任务中取得了显著改进，FID分数大幅降低。例如，REPA-XL/2的FID从5.9提升到3.3。该方法在SiT模型上也优于代表性引导。当与无分类器引导结合时，能产生互补增益，显示出增强的语义一致性和视觉保真度。

Conclusion: 基于表示信息的扩散采样是一种实用的策略，能够增强语义保持和图像一致性。该方法通过利用无监督特征表示，有效缓解了扩散模型早期去噪阶段的语义漂移问题，为高质量视觉合成提供了新的技术路径。
...
TL;DR: HAVC是一种无需训练的方法，通过选择性细化注意力头来提升多模态大语言模型的视觉定位能力，在细粒度VQA任务中优于现有裁剪策略。
...
Method: 提出Head Aware Visual Cropping方法：1）通过OCR诊断任务筛选具有真实定位能力的注意力头；2）在推理时使用空间熵增强空间集中度，使用梯度敏感度评估预测贡献；3）融合信号生成视觉裁剪指导图，裁剪出任务相关子图像与原始图像-问题对一起输入模型。
...
<details>
  <summary>Details</summary>
Motivation: 多类别视觉异常检测面临物体类别多样性、异常样本稀缺性和伪装缺陷等挑战，需要更有效的语义指导和像素级不平衡处理

Method: 1）使用CLIP编码的文本提示描述正常和异常类别特征；2）引入Focal loss处理像素级类别不平衡；3）结合多尺度卷积特征、Transformer空间注意力和扩散迭代精化的监督分割器

Result: 在MVTec-AD数据集上达到最先进的像素级性能：平均AUC 98.35%，AP 66.54%，在不同类别上保持高效性
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在合成个性化说话人脸时面临核心挑战：说话者特定风格和语义内容在面部动作中内在混淆，导致无法将说话者的独特个性忠实地迁移到任意语音中。

Method: 提出MirrorTalk框架：1）基于条件扩散模型；2）引入语义解耦风格编码器（SDSE）从简短参考视频中提取纯风格表示；3）采用分层调制策略在扩散过程中动态平衡音频和风格特征在不同面部区域的贡献。

Result: 大量实验表明，MirrorTalk在唇部同步准确性和个性化保持方面相比现有最先进方法有显著改进。

Conclusion: MirrorTalk通过语义解耦风格表示和分层调制策略，成功解决了说话风格与语义内容混淆的问题，实现了既保持准确唇部同步又能保留说话者独特风格的个性化说话人脸合成。
...
<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型在主题驱动图像生成方面取得了显著进展，但视觉自回归模型因其统一架构和高效推理的潜力尚未得到充分探索。研究者希望开发一个基于VAR模型的新框架来提升主题驱动的图像合成能力。

Method: DreamVAR采用视觉自回归模型进行下一尺度预测。首先通过视觉分词器提取参考主题的多尺度特征，然后预填充完整的主题特征序列（而不是在不同尺度间交错条件特征和目标图像标记）。这种设计简化了自回归依赖关系，并缓解了VAR范式中多尺度条件场景下的训练-测试差异。此外，DreamVAR还结合了强化学习来共同增强语义对齐和主题一致性。

Result: 大量实验表明，DreamVAR在主题外观保持方面优于领先的基于扩散的方法，实现了更优越的外观保留效果。

Conclusion: DreamVAR成功展示了视觉自回归模型在主题驱动图像生成任务中的潜力，通过创新的预填充设计和强化学习优化，在保持主题一致性和语义对齐方面超越了现有扩散模型方法。
...
TL;DR: 该论文提出了CoVA任务，这是一个考虑视觉和听觉变化的视频检索新任务，并构建了AV-Comp基准数据集，同时提出了AVT Compositional Fusion方法用于跨模态特征融合。


<details>
  <summary>Details</summary>
Motivation: 现有组合视频检索(CoVR)基准仅考虑视觉变化，忽略了音频差异对视频检索的影响。为了弥补这一局限，需要开发能够同时处理视觉和听觉变化的视频检索方法。

Method: 提出了AVT Compositional Fusion方法，通过选择性地将查询对齐到最相关的模态，整合视频、音频和文本特征。同时构建了AV-Comp基准数据集，包含具有跨模态变化的视频对和描述差异的文本查询。

Result: AVT方法在传统单模态融合方法上表现出优越性能，为CoVA任务提供了强有力的基线。构建的AV-Comp数据集包含视觉和听觉信息，支持跨模态视频检索研究。

Conclusion: 该研究引入了考虑音频变化的组合视频检索新任务CoVA，通过构建基准数据集和提出有效的跨模态融合方法，推动了多模态视频检索领域的发展。
...
TL;DR: 本研究比较了深度图像和点云数据在奶牛体况评分预测中的表现，发现深度图像在未分割和全身分割数据上表现更优，点云对噪声和模型架构更敏感，在当前评估条件下点云相比深度图像没有明显优势。


<details>
  <summary>Details</summary>
Motivation: 奶牛体况评分是评估能量状态的重要指标，传统视觉评分方法主观且耗时。计算机视觉方法已被应用于BCS预测，深度图像因能捕捉与毛色纹理无关的几何信息而被广泛使用，而三维点云数据能表示更丰富的动物形态几何特征，但两者直接比较的研究有限。

Method: 研究比较了俯视深度图像和点云数据在四种设置下的BCS预测：1)未分割原始数据，2)分割全身数据，3)分割后躯数据，4)手工特征数据。使用来自商业农场的1,020头奶牛数据，采用奶牛级别交叉验证防止数据泄漏。

Result: 深度图像模型在使用未分割原始数据和分割全身数据时始终比点云模型获得更高准确率；使用分割后躯数据时两者性能相当；两种方法在使用手工特征数据时准确率都降低。点云预测比深度图像预测对噪声和模型架构更敏感。

Conclusion: 在当前评估条件下，三维点云相比深度图像在奶牛体况评分预测中没有提供一致的优势，深度图像在多数情况下表现更优且更稳定。
...
TL;DR: SHED是一种新颖的编码器-解码器架构，通过将分割融入密集预测来显式加强几何先验，使用双向分层推理改善深度边界锐度和分割一致性，无需显式分割监督。


<details>
  <summary>Details</summary>
Motivation: 现实世界场景具有强烈结构，但现有密集预测方法将其视为独立像素级预测，常导致结构不一致。需要一种能显式加强几何先验的方法。

Method: 提出SHED架构，通过双向分层推理将分割标记分层池化（编码器）和反池化（解码器）来逆转层次结构。模型仅在最终输出处进行监督，使分割层次自然涌现而无需显式分割监督。

Result: SHED提高了深度边界锐度和分割一致性，在从合成到真实世界环境的跨域泛化方面表现优异。其层次感知解码器能更好地捕捉全局3D场景布局，提升语义分割性能，同时增强3D重建质量并揭示可解释的部分级结构。

Conclusion: SHED通过显式结合分割到密集预测中，有效加强了几何先验，解决了传统像素级方法的结构不一致问题，在3D感知任务中展现出优越性能。
...
TL;DR: 本文提出MA-PaPSP方法，通过记忆增强机制解决视觉语言基础模型的选择性预测问题，特别针对开放集和无限词汇任务，提升预测稳定性和校准效果。


<details>
  <summary>Details</summary>
Motivation: 现有选择性预测研究主要关注封闭集任务，而视觉语言基础模型面临从封闭到开放集、从有限到无限词汇的多样化任务。本文旨在开发无需训练、低复杂度的通用方法，适用于任何基础模型，解决视觉语言表示不稳定和相似度分数校准差的问题。

Method: 提出记忆增强的PaPSP（MA-PaPSP）模型，通过检索图像-文本对数据集来增强原始PaPSP。该方法利用检索到的最近邻对进行平均以减少嵌入方差，并结合对比归一化技术改进分数校准。

Result: 在多个数据集上的实验表明，MA-PaPSP在选择性描述、图像-文本匹配和细粒度分类等任务中，优于原始PaPSP和其他选择性预测基线方法。

Conclusion: MA-PaPSP为视觉语言基础模型提供了一种有效的选择性预测解决方案，通过记忆增强机制成功解决了嵌入不稳定和校准问题，在多样化任务中表现出优越性能。
...
TL;DR: 本文系统评估了10个大型视觉语言模型在自动驾驶安全感知任务中的表现，发现它们在复杂自然场景中的召回率比传统YOLO检测器高25%以上，但在合成扰动下的几何精度方面不如传统方法。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶的环境感知可靠性是主要障碍之一，特别是在恶劣条件下传统检测器容易失效。虽然大型视觉语言模型展现出有前景的语义推理能力，但其在安全关键2D目标检测中的定量效果尚未充分探索。

Method: 使用专门为长尾交通场景和环境退化设计的PeSOTIF数据集，系统评估了10个代表性大型视觉语言模型，并与基于YOLO的传统感知方法进行定量比较。

Result: 实验结果显示关键权衡：表现最佳的大型视觉语言模型（如Gemini 3、Doubao）在复杂自然场景中的召回率比YOLO基线高25%以上，对视觉退化表现出更强的鲁棒性；而基线方法在合成扰动的几何精度方面保持优势。

Conclusion: 这些发现凸显了语义推理与几何回归的互补优势，支持将大型视觉语言模型作为SOTIF导向自动驾驶系统中的高级安全验证器。
...
Method: DELNet包含两个核心组件：1）判断阀，用于测量任务相似度以区分新任务和已知任务；2）动态专家库，存储针对不同退化类型训练的专家。对于新任务，选择top-k专家进行知识迁移，同时添加新专家捕获任务特定特征；对于已知任务，直接重用对应专家。

Result: 在OTS、Rain100H和Snow100K数据集上的实验表明，DELNet超越了最先进的持续学习方法，分别实现了16%、11%和12%的PSNR增益。

Conclusion: DELNet展示了在天气图像修复任务中的有效性、鲁棒性和效率，显著降低了重新训练成本，使其能够在实际场景中部署应用。
...
TL;DR: 提出基于ArUco标记的自动标注方法训练深度学习模型，解决传统图像处理在噪声、运动模糊等条件下识别失败的问题，减少人工标注工作量。


<details>
  <summary>Details</summary>
Motivation: 工厂自动化中移动机器人依赖标记进行定位和识别，传统OpenCV方法在噪声、运动模糊、失焦和光照变化下容易失败，而深度学习需要大量标注数据，人工标注成为瓶颈。

Method: 利用ArUco标记内置识别模块提供的ID和位置信息实现自动标注，使用自动标注的数据集训练YOLO模型，并与传统图像处理方法进行对比评估。

Result: 实验结果表明，该方法在模糊或失焦图像上的识别性能优于传统图像处理技术，同时减少了人工工作量并保证了标注质量的一致性。
...
<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在视频理解、问答和推理等任务中表现出色，但仍存在幻觉问题，即生成与视频内容或事实证据不一致的输出。现有解码方法虽然考虑了视频的时空特性，但大多依赖启发式设计，无法精确捕捉幻觉的根本原因及其细粒度时空和语义相关性，导致在复杂场景中鲁棒性和泛化能力有限。
...
Conclusion: 时空语义对比解码策略为缓解视频大语言模型的幻觉问题提供了一种有效方法，通过对比学习机制更好地捕捉幻觉的时空和语义相关性，提升了模型的鲁棒性和泛化能力。
...
TL;DR: PhoStream是首个面向移动设备的流式基准测试，用于评估多模态大语言模型在连续实时音频-视觉流中的表现，发现模型在决定何时回应方面存在显著困难。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在离线音频-视觉理解方面表现出色，但在移动设备上作为实时流式助手的能力尚未充分探索。日常手机使用中，助手需要追踪流式音频-视觉输入并在适当时机回应，而现有基准测试通常局限于选择题或使用较短视频。

Method: 提出了PhoStream基准测试，包含5,572个开放式QA对，来自578个视频，涵盖4种场景和10种能力。采用自动化生成流程构建，并通过严格人工验证。使用在线推理管道和LLM-as-a-Judge评估开放式回答。

Result: 实验揭示了LLM评分（0-100）中的时间不对称性：模型在即时和向后任务中表现良好（Gemini 3 Pro超过80分），但在向前任务中急剧下降至16.40分，主要原因是模型在所需视觉和音频线索出现前过早回应。

Conclusion: 当前多模态大语言模型不仅需要决定说什么，更重要的是需要决定何时说，这是其作为移动流式助手的一个基本限制。代码和数据集将公开提供。
...
TL;DR: FOTBCD是一个大规模法国建筑物变化检测数据集，覆盖法国28个省份，包含约28,000个前后图像对和像素级二进制变化掩码，旨在评估地理域偏移下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有建筑物变化检测数据集通常局限于单个城市或有限区域，缺乏地理多样性，限制了模型在不同地理环境下的泛化能力评估。需要大规模、地理多样性的数据集来评估模型在地理域偏移下的性能。

Method: 基于法国国家地理研究所（IGN）的权威正射影像和地形建筑物数据构建数据集。覆盖法国28个省份，其中25个用于训练，3个地理上不连续的省份用于评估。数据集包含FOTBCD-Binary（约28,000个图像对和二进制变化掩码）和FOTBCD-Instances（实例级标注子集）。

Result: 使用固定参考基线在FOTBCD-Binary上对LEVIR-CD+和WHU-CD进行基准测试，提供了强有力的经验证据表明数据集层面的地理多样性有助于改善建筑物变化检测的跨域泛化能力。

Conclusion: FOTBCD是一个大规模、地理多样性的建筑物变化检测数据集，能够更好地评估模型在地理域偏移下的泛化性能。数据集的地理多样性有助于提高变化检测模型的跨域泛化能力。
...
TL;DR: TTSA3R：一种无需训练的框架，通过结合时间状态演化和空间观测质量进行自适应状态更新，解决3D重建中长序列灾难性记忆遗忘问题


<details>
  <summary>Details</summary>
Motivation: 流式循环模型在3D重建中虽然能保持持久状态表示，但在长序列中会遭受灾难性记忆遗忘问题。现有方法从注意力角度推导自适应信号，但只考虑单一维度，忽略了时间和空间一致性。

Method: 提出TTSA3R框架，包含两个模块：1）时间自适应更新模块，通过分析时间状态演化模式调节更新幅度；2）空间上下文更新模块，通过观测-状态对齐和场景动态定位需要更新的空间区域。最后融合这些互补信号确定状态更新策略。

Result: 在多种3D任务中验证了有效性。在扩展序列上，本方法仅增加15%误差，而基线模型误差增加超过200%，显著提高了长期重建稳定性。

Conclusion: TTSA3R通过同时考虑时间状态演化和空间观测质量，有效解决了3D重建中的长期记忆遗忘问题，显著提升了流式循环模型的稳定性。
...
TL;DR: LINA是一个基于线性注意力的高效文本到图像生成模型，通过优化归一化方案和引入卷积增强局部性，在减少61%计算量的同时实现了高质量的1024x1024图像生成。


<details>
  <summary>Details</summary>
Motivation: 连续token的自回归模型在视觉生成领域很有前景，但存在计算成本高的问题。研究旨在设计计算高效的线性注意力机制，以降低文本到图像合成的计算开销。

Method: 1) 系统分析不同设计选择下的缩放行为，比较除法归一化与减法归一化；2) 引入深度卷积增强局部建模；3) 将门控机制扩展到双向设置，提出KV门控，通过可学习参数实现类似语言模型遗忘门的灵活内存管理；4) 基于这些发现构建LINA模型。

Result: LINA在ImageNet上获得2.18 FID（约14亿参数），在GenEval上获得0.74（约15亿参数）。单个线性注意力模块相比softmax注意力减少约61% FLOPs，能够生成高质量的1024x1024图像。

Conclusion: 除法归一化在线性生成变换器中缩放效果更好，卷积局部建模对自回归生成至关重要。KV门控机制实现了灵活的内存管理。LINA展示了线性注意力在文本到图像生成中的高效性和竞争力。
...
TL;DR: 本文提出使用S.R. Ranganathan的分类原则来解决计算机视觉中的语义鸿沟问题，并介绍了基于这些原则的vTelos标注方法，实验证明该方法能提高标注质量和准确率。
...
Method: 采用S.R. Ranganathan的分类原则，并适当调整以适应计算机视觉领域，以此为基础构建vTelos CV标注方法学。

Result: 实验证据表明，使用vTelos方法能够改善CV标注质量和准确率，从而验证了该方法的有效性。

Conclusion: Ranganathan的分类原则为解决计算机视觉中的语义鸿沟问题提供了原则性起点，基于这些原则的vTelos标注方法能够设计出更高质量的CV数据集。
...
TL;DR: 该论文提出了一种无需配对标注的无监督合成图像溯源方法，通过对比自监督学习进行概念对齐，再使用Infomax损失促进表示解耦，在真实基准测试中超越了有监督方法。
...
Method: 提出无监督方法"对齐与解耦"：1) 使用对比自监督学习进行基础概念对齐；2) 通过Infomax损失促进表示解耦，增强溯源能力。该方法基于MoCo和DINO等对比自监督模型具有跨域对齐能力的观察。

Result: 在真实世界基准测试AbC上，该无监督方法意外地超越了有监督方法。通过将观察形式化为跨协方差的理论假设，提供了对齐和解耦如何通过典型相关分析目标分解来近似概念匹配过程的理论解释。

Conclusion: 该研究为合成图像溯源这一挑战性任务提供了新的视角，展示了无监督方法在此任务上的潜力，其直观见解和实验结果有望推动该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇对齐方法存在两个主要问题：一是依赖全局句子嵌入缺乏细粒度表达能力，二是需要显式监督或复杂的跨注意力设计来实现token级对齐。需要一种在弱监督下实现细粒度视觉语言对齐的理论框架。

Method: 提出ExpAlign框架：1）基于多示例学习理论构建，引入期望对齐头，通过注意力软MIL池化在token-区域相似度上实现隐式token和实例选择；2）提出能量多尺度一致性正则化方案，包括Top-K多正例对比目标和基于拉格朗日约束自由能最小化的几何感知一致性目标。

Result: 在开放词汇检测和零样本实例分割任务上表现优异，特别是在长尾类别上。在LVIS minival split上达到36.2 AP_r，优于同等模型规模的其他SOTA方法，同时保持轻量级和推理高效性。

Conclusion: ExpAlign提供了一个理论基础的视觉语言对齐框架，通过多示例学习公式和能量正则化，在弱监督下实现了细粒度对齐，显著提升了开放词汇视觉任务的性能，特别是在长尾类别上的表现。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理高分辨率和视频场景时，由于过多的视觉令牌导致计算成本过高。现有的令牌减少方法通常关注孤立的流水线组件，且往往忽视文本对齐，导致性能下降。

Method: 提出了VisionTrim框架，包含两个即插即用模块：1) 主导视觉令牌选择模块，通过全局-局部视角保留关键视觉令牌；2) 文本引导视觉补充模块，通过文本线索引导上下文感知的令牌合并。

Result: 在多样化的图像和视频多模态基准测试中进行了广泛实验，证明了VisionTrim的性能优越性，推动了实际应用中多模态大语言模型的部署。

Conclusion: VisionTrim是一个有效的无需训练的多模态大语言模型加速框架，通过创新的令牌选择和合并机制，在保持性能的同时显著减少计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本词汇外检测方法容易对词汇内类别过拟合，导致未定义类别被错误分类为词汇内类别且置信度很高。需要解决模型缺乏OOV数据先验知识的问题。
...
Conclusion: 提出的OOVDet框架通过合成OOV提示和挖掘伪OOV图像，有效解决了零样本词汇外检测中模型过拟合词汇内类别的问题，实现了更好的未定义类别拒绝能力。
...
TL;DR: 提出Bi-MCQ方法，通过双向多项选择学习框架改进医学视觉语言模型对否定性临床陈述的理解能力，显著提升疾病缺席检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在理解否定性临床陈述方面表现较弱，主要因为对比对齐目标将否定视为次要语言变化而非意义反转操作。在多标签设置中，基于提示的InfoNCE微调进一步强化了简单正例的图像-提示对齐，限制了疾病缺席的有效学习。

Method: 将视觉语言对齐重新表述为条件语义比较问题，通过双向多项选择学习框架实现。联合训练图像到文本和文本到图像的多项选择任务，使用肯定、否定和混合提示。引入方向特定的交叉注意力融合模块来处理双向推理所需的不对称线索并减少对齐干扰。

Result: 在ChestXray14、Open-I、CheXpert和PadChest数据集上，Bi-MCQ相比最先进的CARZero模型的零样本性能，将否定理解能力提升了最高0.47 AUC，同时在正负组合评估上获得最高0.08的绝对增益。相比基于InfoNCE的微调，Bi-MCQ将肯定-否定AUC差距平均减少了0.12。

Conclusion: 目标函数重新表述可以显著增强医学视觉语言模型中的否定理解能力。Bi-MCQ通过条件语义比较而非全局相似性最大化的方式，有效解决了现有模型在理解否定性临床陈述方面的局限性。
...
TL;DR: GRACE是一个基于信息瓶颈原理的统一框架，将知识蒸馏与量化感知训练结合，通过置信门控解耦蒸馏、关系中心核对齐和自适应控制器等技术，在INT4量化下实现超越FP16基线的性能，同时显著提升推理速度和减少内存占用。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型部署成本高，后训练量化通常导致显著的精度损失，而量化感知训练在VLMs领域尚未得到充分探索。需要一种能够保持模型性能的同时实现高效部署的量化方法。

Method: GRACE框架基于信息瓶颈原理，统一知识蒸馏和量化感知训练：1）置信门控解耦蒸馏，过滤不可靠的监督信号；2）关系中心核对齐，传递视觉token结构信息；3）通过拉格朗日松弛的自适应控制器，平衡保真度与容量约束。

Result: 在LLaVA和Qwen系列模型上，INT4量化模型一致超越FP16基线（如LLaVA-1.5-7B在SQA上70.1 vs 66.8；Qwen2-VL-2B在MMBench上76.9 vs 72.6），几乎匹配教师模型性能。使用真实INT4内核实现3倍吞吐量和54%内存减少。
...
TL;DR: OpenVTON-Bench是一个大规模虚拟试衣基准数据集，包含约10万张高分辨率图像对，并提出了多模态评估协议，在五个维度上评估虚拟试衣质量，与人类判断高度一致。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型显著提升了虚拟试衣系统的视觉保真度，但可靠的评估仍然是瓶颈。传统指标难以量化细粒度纹理细节和语义一致性，现有数据集在规模和多样性上达不到商业标准。

Method: 构建了包含约10万张高分辨率图像对（最高1536×1536）的大规模数据集，使用DINOv3层次聚类进行语义平衡采样，Gemini密集标注确保20个细粒度服装类别的均匀分布。提出了多模态评估协议，包含五个可解释维度：背景一致性、身份保真度、纹理保真度、形状合理性和整体真实感。协议整合了VLM语义推理和基于SAM3分割与形态学腐蚀的新型多尺度表示度量，能够分离边界对齐误差和内部纹理伪影。

Result: 实验结果显示，该评估协议与人类判断高度一致（Kendall's τ为0.833，而SSIM仅为0.611），为虚拟试衣评估建立了稳健的基准。

Conclusion: OpenVTON-Bench提供了一个大规模、高质量的虚拟试衣基准数据集和可靠的评估协议，解决了现有评估方法的局限性，为虚拟试衣系统的评估提供了标准化框架。
...
<details>
  <summary>Details</summary>
Motivation: 3D语义占据预测对自动驾驶至关重要，但单模态方法在相机语义和LiDAR几何之间存在权衡。现有多模态框架面临模态异质性、空间错位和表示危机（体素计算量大、BEV表示有损失）等挑战。

Method: 提出了基于内存高效的连续3D高斯表示的多模态框架，包含四个模块：1) LiDAR深度特征聚合(LDFA)，使用深度可变形采样将稀疏信号提升到高斯基元；2) 基于熵的特征平滑(EBFS)减轻域噪声；3) 自适应相机-LiDAR融合(ACLF)采用不确定性感知重加权处理传感器可靠性；4) Gauss-Mamba头利用选择性状态空间模型实现线性复杂度的全局上下文建模。

Result: 在Occ3D、SurroundOcc和SemanticKITTI基准测试中达到最先进性能，分别获得49.4%、28.9%和25.2%的mIoU分数。在雨天和夜间等挑战性条件下表现出优越的鲁棒性。

Conclusion: GaussianOcc3D通过连续3D高斯表示有效解决了多模态3D语义占据预测中的关键挑战，实现了内存高效、性能优越的解决方案，在多种环境条件下都表现出色。
...
TL;DR: ImgCoT将思维链压缩为视觉表示，用空间布局偏置替代语言偏置，更好地捕捉推理结构，并通过混合推理保留关键细节


<details>
  <summary>Details</summary>
Motivation: 现有方法将文本思维链作为重构目标，迫使潜在token保留表层语言特征，引入了强烈的语言归纳偏置，限制了逻辑抽象能力

Method: 提出ImgCoT，将重构目标从文本思维链改为视觉思维链（将思维链渲染为图像），用空间偏置替代语言偏置；并提出松散ImgCoT，通过低token对数似然选择关键文本推理步骤，与视觉潜在token结合进行混合推理

Result: 在多个数据集和LLMs上的广泛实验证明了两种ImgCoT版本的有效性

Conclusion: 视觉思维链压缩能更好地捕捉全局推理结构，混合推理设计能在更少token的情况下同时保留推理结构和细节
...
<details>
  <summary>Details</summary>
Motivation: 当前VLLMs在联合多语言多模态输入下的安全鲁棒性研究不足，现有基准要么是多语言但仅文本，要么是多模态但单语言，缺乏语义基础的图像-文本对来覆盖真实的跨模态交互风险。

Method: 构建Lingua-SafetyBench基准，包含10种语言的100,440个有害图像-文本对，明确划分为图像主导和文本主导子集以分离风险来源，评估11个开源VLLMs，并在Qwen系列上进行控制研究。

Result: 发现一致的不对称性：图像主导风险在高资源语言中攻击成功率更高，而文本主导风险在非高资源语言中更严重；Qwen系列的扩展和版本升级总体上降低ASR，但不成比例地使高资源语言受益，在文本主导风险下扩大了高低资源语言之间的差距。

Conclusion: 仅靠模型扩展不足以解决VLLMs的安全问题，需要超越简单扩展的语言和模态感知安全对齐方法，以平衡不同语言和模态间的安全保护。
...
TL;DR: StreamSense是一个用于直播平台的实时检测系统，通过轻量级编码器和选择性路由到VLM专家，在保证准确性的同时降低计算延迟。


<details>
  <summary>Details</summary>
Motivation: 直播平台需要实时监控和响应社交信号，但现有方法要么计算成本高（如VLM），要么准确性不足。需要一种能平衡准确性和效率的流式检测方案。

Method: 提出StreamSense流式检测器：1）轻量级流式编码器处理大多数时间戳；2）选择性路由机制将困难/模糊案例升级到VLM专家；3）上下文不足时延迟决策。编码器训练采用跨模态对比损失和对齐IoU加权损失。

Result: 在多个社交流式检测任务（如情感分类和仇恨内容审核）上，StreamSense比纯VLM流式检测准确率更高，同时仅偶尔调用VLM，显著降低了平均延迟和计算成本。
...
TL;DR: FaceDefense提出了一种针对扩散模型人脸交换的主动防御框架，通过扩散损失增强防御效果，同时使用面部属性编辑减少扰动引起的失真，实现更好的不可感知性和防御效果平衡。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的人脸交换技术虽然性能优越，但也加剧了恶意人脸交换侵犯肖像权和个人声誉的风险。现有主动防御方法面临核心权衡：大扰动会扭曲面部结构，小扰动则防御效果不足。

Method: 提出FaceDefense框架：1) 引入新的扩散损失函数增强对抗样本的防御效果；2) 采用定向面部属性编辑恢复扰动引起的失真；3) 设计两阶段交替优化策略生成最终扰动人脸图像。

Result: 大量实验表明，FaceDefense在不可感知性和防御效果方面显著优于现有方法，实现了更优的权衡平衡。

Conclusion: FaceDefense有效解决了扩散模型人脸交换主动防御中的核心权衡问题，通过增强防御效果同时保持视觉不可感知性，为对抗恶意人脸交换提供了更优的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 工业故障排除指南以流程图形式编码诊断程序，需要将其提取为结构化知识以集成到操作员支持系统中。手动提取过程劳动密集且易出错，视觉语言模型有潜力自动化此过程，但其在此类指南上的性能尚未充分探索。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的多类无监督异常检测方法依赖于训练编码器-解码器模型来重建无异常特征，但这些方法存在固有的保真度-稳定性困境。作者希望摆脱重建范式，探索更有效的异常检测方法。

Method: 提出检索式异常检测（RAD），这是一种无需训练的方法。它将无异常特征存储在内存中，通过多级检索将测试图像块与内存中的正常特征进行匹配来检测异常。

Result: RAD在四个基准测试（MVTec-AD、VisA、Real-IAD、3D-ADAM）的标准和少样本设置下均达到最先进性能。在MVTec-AD上，仅使用单张无异常图像就能达到96.7%的像素AUROC，接近全数据性能的98.5%。

Conclusion: 检索式异常检测方法颠覆了多类无监督异常检测需要任务特定训练的假设，证明基于内存检索的方法可以实现最先进的异常检测性能，且检索式评分理论上限高于重建残差评分。
...
TL;DR: 提出DCCT框架，利用相机成像管道的颜色相关性特征来检测AI生成图像，通过模拟CFA采样模式训练自监督U-Net，在20多个未见生成器上实现最先进的泛化性能


<details>
  <summary>Details</summary>
Motivation: 针对AI生成图像威胁数字真实性的问题，现有基于生成伪影的检测器存在泛化失败的问题，需要利用相机成像管道的固有特性来提高检测的泛化能力

Method: 提出Demosaicing-guided Color Correlation Training (DCCT)框架：1) 模拟CFA采样模式将彩色图像分解为单通道输入和双通道目标；2) 训练自监督U-Net通过逻辑混合函数建模条件分布；3) 利用颜色相关性特征构建二元分类器

Result: 理论分析证明DCCT针对摄影和AI生成图像之间存在可证明的分布差异，在超过20个未见生成器上实现了最先进的泛化性能和鲁棒性，显著优于先前方法

Conclusion: 通过利用相机成像管道的颜色相关性特征，DCCT框架能够有效检测AI生成图像，解决了现有检测器的泛化失败问题，为数字真实性验证提供了新方法
...
<details>
  <summary>Details</summary>
Motivation: 现有农田遥感图像分割方法采用静态分割范式，仅依赖单个输入图像块的有限信息，在处理具有模糊性和视觉不确定性的复杂场景时推理能力有限。人类专家在解释此类模糊图像时，会主动查询辅助图像进行交叉验证，从而获得更全面的推理

Method: 提出推理查询驱动的动态分割框架FarmMind，引入推理查询机制：首先通过推理分析分割模糊的根源，然后基于分析结果确定需要查询的辅助图像类型（如更高分辨率、更大尺度或时间相邻数据），动态按需查询外部辅助图像来补偿单个输入图像信息不足

Result: 大量实验表明，FarmMind相比现有方法实现了更优的分割性能和更强的泛化能力

Conclusion: FarmMind通过模拟人类专家面对分割模糊时的思维过程，引入推理查询机制，突破了静态分割范式的限制，为农田遥感图像分割提供了更有效的解决方案
...
TL;DR: NativeTok提出原生视觉标记化框架，通过强制因果依赖关系解决VQ图像生成中两阶段不匹配问题，实现高效重建和强一致性


<details>
  <summary>Details</summary>
Motivation: 现有VQ图像生成方法中，第一阶段标记化的改进不一定能提升第二阶段生成效果，因为现有方法无法约束标记间的依赖关系，导致生成模型需要从无序分布中学习，产生偏差和弱一致性
...
Conclusion: 通过强制因果依赖关系的原生视觉标记化方法，解决了传统VQ图像生成中两阶段不匹配的问题，提高了生成质量和一致性
...
TL;DR: 本文提出NCT框架解决定制化虚拟试穿任务，通过扩散模型结合语义增强和控制模块，实现服装语义保持与模特姿态、外观的灵活编辑。


<details>
  <summary>Details</summary>
Motivation: 传统虚拟试穿任务缺乏灵活性，无法根据用户偏好定制数字人外观、姿态等属性。本文旨在解决定制化虚拟试穿任务，让用户能根据个人偏好定制数字人，提升虚拟试穿体验的灵活性和参与度。

Method: 提出Neural Clothing Tryer (NCT)框架，基于扩散模型并配备语义增强模块和控制模块。语义增强模块利用服装语义描述和视觉语言编码器学习跨模态对齐特征；控制模块以服装图像、定制姿态图像和语义描述为输入，在保持服装细节的同时编辑模特姿态、表情等属性。

Result: 在公开基准测试上的大量实验表明，所提出的NCT框架具有优越性能。

Conclusion: NCT框架成功解决了定制化虚拟试穿任务，通过语义增强和控制模块实现了服装语义保持与模特属性的灵活编辑，提升了虚拟试穿的灵活性和用户体验。
...
TL;DR: 遥感基础模型在远小于计算机视觉模型的规模下就进入过参数化状态，通过后剪枝方法发现遥感模型具有高度冗余表示，在1%计算量下仍能保持71%以上相对精度，挑战了遥感领域现有的缩放范式。


<details>
  <summary>Details</summary>
Motivation: 当前遥感领域的基础模型开发直接沿用了计算机视觉的缩放假设，但这种跨领域转移的有效性尚未得到充分验证。作者假设遥感基础模型在远小于计算机视觉模型的规模下就进入了过参数化状态，增加参数主要产生冗余表示而非新的抽象能力。

Method: 使用后剪枝方法作为工具，均匀缩减预训练编码器的宽度，在六个最先进的遥感基础模型和四个下游分类任务上测量表示冗余。通过解释方差比和特征相关性分析提供机制性解释，并展示了可学习的可剪枝训练方法。

Result: 发现遥感基础模型与计算机视觉模型存在显著差异：在ImageNet上训练的MAE模型在1%计算量下精度低于10%，而遥感基础模型在相同计算量下能保持超过71%的相对精度，存在七倍差异。可学习的可剪枝训练能同时改进MoCo和MAE模型。

Conclusion: 遥感基础模型以高度冗余的方式分布任务相关信息，后剪枝既可作为资源受限环境的实用部署策略，也可作为挑战遥感领域现有缩放范式的诊断工具。研究结果支持了遥感模型在较小规模下就进入过参数化状态的假设。
...
TL;DR: 提出基于NeRF的森林冠层下地面重建方法，仅使用RGB图像即可去除遮挡的冠层，实现地面场景的逼真重建，应用于搜救和森林调查任务。


<details>
  <summary>Details</summary>
Motivation: 现有森林冠层下地形和植被重建方案依赖昂贵专业传感器（如机载LiDAR或热合成孔径摄影），需要更经济、高分辨率的替代方案，仅使用常规RGB图像实现冠层下场景重建。

Method: 基于神经辐射场（NeRF）的3D重建方法，结合特定图像采集策略确保冠层下光照充足，采用低光损失处理光照不足问题，提出两种互补方法通过控制光线积分过程去除遮挡冠层元素。

Result: 方法在搜救任务中实现与热AOS相当的人员检测效果，在森林调查任务中展示树木计数的潜力，证明仅用RGB图像即可替代专业传感器完成相关任务。

Conclusion: 该方法为搜救、路径测绘和森林调查任务提供了一种经济高效、高分辨率的替代方案，仅使用常规RGB图像即可实现冠层下场景的逼真重建。
...
TL;DR: 论文提出上下文异常检测概念，认为异常不仅取决于物体本身，还受上下文环境影响，并引入CAAD-3K基准数据集和条件兼容性学习框架
...
Method: 提出条件兼容性学习框架，利用视觉-语言表示来建模主体-上下文关系，在有限监督下学习主体与环境的兼容性

Result: 方法在CAAD-3K基准上显著优于现有方法，在MVTec-AD和VisA数据集上达到最先进性能，证明建模上下文依赖能补充传统结构异常检测

Conclusion: 上下文异常检测是视觉领域重要问题，提出的条件兼容性学习框架能有效建模主体-上下文关系，为异常检测提供了新的视角和方法
...
TL;DR: DINO-SAE：一种结合语义表示与像素级重建的生成自编码器框架，通过余弦相似度对齐和分层卷积补丁嵌入提升重建保真度，并在球面流形上训练扩散变换器。


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练视觉基础模型（如DINO）的生成自编码器虽然表现出强大的生成性能，但往往因高频细节丢失而导致重建保真度有限。需要一种既能保持语义一致性又能保留细粒度细节的方法。

Method: 提出DINO球面自编码器（DINO-SAE），包含：1）分层卷积补丁嵌入模块增强局部结构和纹理保留；2）余弦相似度对齐目标强制语义一致性同时允许灵活的特征幅度以保留细节；3）利用SSL基础模型表示本质位于超球面的特性，在球面流形上使用黎曼流匹配训练扩散变换器（DiT）。

Result: 在ImageNet-1K上达到最先进的重建质量：0.37 rFID和26.2 dB PSNR，同时保持与预训练VFM的强语义对齐。基于黎曼流匹配的DiT在80个epoch内达到3.47 gFID，表现出高效收敛性。

Conclusion: DINO-SAE成功桥接了语义表示和像素级重建，通过分离语义方向与特征幅度，结合分层卷积嵌入和球面流形上的扩散建模，实现了高质量的重建和生成性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有工业视觉异常检测方法存在两大局限：1) 假设训练数据纯净无异常污染，这在现实中很少满足；2) 无法利用标记的异常样本学习异常特征。这导致现有方法在真实污染数据上性能不可靠。

Method: 提出一个鲁棒异常检测框架，整合有限异常监督到自适应偏差学习范式。引入复合异常评分，包含三个互补组件：统计异常性的偏差评分、预测不一致性的熵基不确定性评分、空间异常性的分割基评分。采用少量标记异常样本训练，通过自适应实例加权减轻污染样本影响。

Result: 在MVTec和VisA基准测试上的大量实验表明，该框架优于最先进的基线方法，在各种数据污染水平下实现了强大的检测和定位性能、可解释性和鲁棒性。

Conclusion: 该工作提出的框架解决了实际工业环境中数据污染和缺乏异常监督的问题，通过复合评分机制和自适应学习策略，为视觉异常检测提供了可靠、可解释的解决方案。
...
TL;DR: 本研究评估了将计算机视觉单目深度估计（MDE）集成到生态相机陷阱工作流程中，用于大猩猩保护的无标记种群密度估计，相比传统人工方法，该方法可将种群估计误差控制在22%以内。


<details>
  <summary>Details</summary>
Motivation: 传统的大猩猩无标记种群密度估计需要人工测量动物到相机的距离，这项工作劳动密集且耗时。本研究旨在探索计算机视觉单目深度估计作为替代方案的可行性，以自动化距离测量过程。
...
Conclusion: MDE驱动的相机陷阱距离采样是人工距离估计的可行实用替代方案，该方法获得的种群估计与传统方法相比误差在22%以内，为生态监测提供了自动化解决方案。
...
TL;DR: Q-Hawkeye是一个基于强化学习的可靠视觉质量评估框架，通过不确定性感知动态优化和感知感知优化解决现有方法的可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于RL的IQA方法存在两个关键可靠性限制：1）不同训练样本的预测稳定性差异很大，但现有GRPO方法使用统一的优势权重，放大了不稳定样本的噪声信号；2）大多数工作强调基于文本的推理而忽略了模型对图像内容的视觉感知能力。

Method: 提出Q-Hawkeye框架，包含两个核心设计：1）不确定性感知动态优化：通过多次rollout预测分数的方差估计预测不确定性，利用不确定性重新加权每个样本的更新强度；2）感知感知优化：构建退化图像与原图的配对输入，引入隐式感知损失约束模型基于真实视觉证据进行质量判断。

Result: 大量实验表明，Q-Hawkeye在多个数据集上优于最先进的方法，并展现出更好的泛化能力。

Conclusion: Q-Hawkeye通过解决预测稳定性和视觉感知可靠性问题，显著提升了基于RL的IQA方法的性能，代码和模型将开源。
...
TL;DR: 图像嵌入存在语义泄露风险，即使无法精确重建原图，通过保留局部语义邻域结构，仍能泄露语义信息。作者提出SLImE框架，无需训练特定解码器即可从压缩嵌入中恢复语义内容。


<details>
  <summary>Details</summary>
Motivation: 挑战图像嵌入隐私风险有限的普遍假设，揭示即使压缩嵌入也能泄露语义信息，暴露隐私保护的根本性漏洞。

Method: 提出SLImE框架：利用局部训练的语义检索器与现成模型，从对齐嵌入中恢复语义标签、符号表示和连贯描述，无需训练任务特定解码器。

Result: 在GEMINI、COHERE、NOMIC、CLIP等多种开放和封闭嵌入模型中验证了语义信息的一致恢复，证明语义邻域结构的保留导致语义泄露。

Conclusion: 图像嵌入存在根本性漏洞，对齐过程中保留的语义邻域结构使语义泄露成为可能，这对隐私保护提出了挑战。
...
TL;DR: 本文探讨了在工业光学质量控制中，使用生成式人工智能（特别是Stable Diffusion和CycleGAN）来解决数据集不平衡问题，通过生成缺陷图像来增强监督机器学习模型的性能。


<details>
  <summary>Details</summary>
Motivation: 在工业生产的光学质量控制中，监督机器学习算法需要代表性数据集，但实际生产中缺陷部件稀少，导致数据集高度不平衡，严重影响模型性能。现有方法如专用损失函数或传统数据增强技术存在局限性，需要仔细的超参数调优或只能改变简单图像特征。

Method: 本研究探索生成式人工智能作为扩展有限数据集和增强监督机器学习性能的替代方法。具体研究了Stable Diffusion和CycleGAN作为图像生成模型，专注于联合收割机部件在热图像中的分割，用于后续缺陷检测。

Result: 使用Stable Diffusion进行数据集扩展取得了最显著的改进，将分割性能提高了4.6%，达到84.6%的平均交并比（Mean IoU）。

Conclusion: 生成式人工智能，特别是Stable Diffusion，在解决工业光学质量控制中数据集不平衡问题方面具有显著潜力，能够有效增强监督机器学习模型的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统切片到体积重建方法耗时且需要多个正交堆栈，而基于学习的方法虽然推理速度快但严重依赖训练时的真实体积标签，这在实践中难以获取。需要一种无需真实标签的自监督方法来解决胎儿MR重建问题。
...
TL;DR: 该论文提出了一种利用多标注者注释改进深度学习目标检测器校准性能的新方法，通过为不同专家训练单独模型并聚合预测来模拟共识，相比混合标注训练策略能更好地捕捉标注者间差异。
...
Method: 提出基于多标注者注释的模型校准改进方法：为单个专家的注释分别训练独立模型，然后聚合这些模型的预测来模拟共识。这种方法优于在混合注释上训练模型的标签采样策略，能更原则性地捕捉标注者间变异性。

Result: 在由两位专家标注的结直肠类器官数据集上的实验表明，提出的标注者特定集成策略显著改善了校准性能，同时保持了可比的检测准确性。
...
TL;DR: OSGA提出了一种单次优化的输入无关引导框架，通过方差数据选择策略和生成锚点正则化学习单个引导向量，可在推理时通用应用，显著减少幻觉并增强安全性。
...
Method: OSGA框架：1）使用基于方差的数据选择策略选择信息丰富的样本；2）通过带有生成锚点正则化的对比目标学习单个引导向量；3）在推理时将该向量通用应用于特定层，无需修改模型参数。

Result: 在多个基准测试中，单个OSGA优化的引导向量能一致地改善幻觉缓解和安全性增强，且开销可忽略不计，证明了单次引导作为可靠视觉语言模型的实用且可扩展解决方案。

Conclusion: OSGA展示了输入无关的单次引导框架的有效性，为提升视觉语言模型的可靠性和安全性提供了一种高效实用的方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉地理定位方法存在存储需求大、忽略地理连续性、细节处理困难等问题，需要更高效、可解释且能处理地理层次结构的解决方案

Method: 采用实体中心的地理定位框架，将图像直接与国家、地区、子区域、城市等地理实体对齐，在双曲空间中构建层次化实体嵌入，通过Geo-Weighted双曲对比学习直接融入haversine距离
...
Method: 提出SEAL框架，支持事件分割和开放词汇掩码分类，包括实例级和部件级粒度，采用参数高效架构，并开发了无需视觉提示的通用时空变体。

Result: 在四个从粗到细类配置的基准测试中，SEAL在性能和推理速度上大幅超越基线方法，参数效率高。

Conclusion: SEAL是首个解决开放词汇事件实例分割的框架，支持多粒度理解，为事件传感器场景理解提供了有效解决方案。
...
TL;DR: Hi-Light是一个无需训练的高保真、高分辨率视频重光照框架，通过三个技术创新解决现有方法的光闪烁、细节丢失问题，并提出了首个量化光照稳定性的评估指标。


<details>
  <summary>Details</summary>
Motivation: 视频重光照具有巨大创意潜力和商业价值，但面临三大挑战：缺乏合适的评估指标、严重的光闪烁问题、以及编辑过程中细粒度细节的退化。

Method: 1. 基于亮度先验的锚定引导重光照扩散：稳定中间重光照视频；2. 混合运动自适应光照平滑滤波器：利用光流确保时间稳定性而不引入运动模糊；3. LAB细节融合模块：保留原始视频的高频细节信息。

Result: 在广泛的实验中，Hi-Light在定性和定量比较中都显著优于现有最先进方法，能够生成稳定、高度细节丰富的重光照视频。
...
TL;DR: 本文提出了一种用于医学图像分割的区域归一化直接偏好优化方法，利用自动质量评估信号进行无额外标注的模型微调，解决了传统偏好优化在噪声评估下不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割需要密集像素级标注，成本高昂且难以扩展。现有系统已能产生自动质量评估信号（如模型一致性、不确定性度量等），但这些信号可能存在噪声和偏差，使得基于偏好的微调容易受到有害更新的影响。

Method: 提出了区域归一化直接偏好优化方法，在监督基础分割器生成提案的基础上，通过归一化掩码间不一致区域的大小来调整偏好更新，减少有害比较的影响并提高优化稳定性。

Result: 在两个医学数据集和多种场景下，区域归一化DPO方法提高了持续性能并稳定了基于偏好的微调，优于标准DPO和强基线方法，且不需要额外的像素标注。

Conclusion: 区域归一化DPO方法能够有效利用噪声自动评估信号进行医学图像分割模型的无标注微调，解决了偏好对挖掘策略对性能的敏感性问题，为低成本扩展医学图像分割系统提供了可行方案。
...
TL;DR: Video-o3是一个用于长视频理解的新型框架，通过迭代发现关键视觉线索、精细检查关键片段和自适应终止来提升性能，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在长视频理解中主要依赖均匀采样和单轮推理，难以在大量冗余信息中发现稀疏但关键的证据，需要更有效的证据发现和推理机制。
...
Conclusion: Video-o3通过原生工具调用机制有效解决了长视频理解中的关键挑战，验证了迭代证据发现和自适应终止策略的有效性，为长视频场景下的多模态理解提供了新思路。
...
TL;DR: ShotFinder是一个用于开放域视频镜头检索的基准测试，通过五种可控约束评估多模态大模型在视频检索中的能力，实验显示现有模型与人类性能存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在信息检索方面进展迅速，但主要集中在文本或静态多模态场景。开放域视频镜头检索涉及更丰富的时间结构和复杂语义，缺乏系统性基准和分析。

Method: 提出ShotFinder基准，将编辑需求形式化为关键帧导向的镜头描述，引入五种可控单因素约束：时间顺序、颜色、视觉风格、音频和分辨率。从YouTube收集1,210个高质量样本，使用大模型生成并人工验证。基于该基准，提出文本驱动的三阶段检索和定位流程：通过视频想象的查询扩展、搜索引擎的候选视频检索、描述引导的时间定位。

Result: 在多个闭源和开源模型上的实验显示，现有模型与人类性能存在显著差距，不同约束间存在明显不平衡：时间定位相对容易处理，而颜色和视觉风格仍是主要挑战。

Conclusion: 开放域视频镜头检索是多模态大模型尚未克服的关键能力，ShotFinder基准揭示了现有模型在这一任务上的局限性，为未来研究提供了重要参考。
...
TL;DR: 该论文提出了DoraVQA数据集，通过从《爱探险的朵拉》教育视频中提取问答对来训练视觉语言模型，以解决现有模型在简单推理任务上的不足。使用GRPO方法微调模型，在多个基准测试中取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在标准视频理解基准上表现良好，但在学龄前儿童都能解决的简单推理任务（如计数、空间推理、组合理解）上系统性失败。教育视频的结构化内容可能为提升这些能力提供理想的训练信号。

Method: 从8季《爱探险的朵拉》中自动提取5,344个精确时间对齐的问答对，构建DoraVQA数据集。利用教育视频中"上下文-问题-暂停-答案"的固定结构，使用Group Relative Policy Optimization（GRPO）方法对Qwen2和Qwen3模型进行微调。

Result: 仅在38小时儿童教育视频上训练，就在DoraVQA上提升了8-14个点，在CVBench上达到86.16%的SOTA性能，并在Video-MME和NExT-QA上表现出良好的迁移能力，证明从狭窄的教育内容到广泛多模态理解的有效泛化。

Conclusion: 视觉语言模型可以通过结构化教育内容学习执行需要稳健推理的任务，表明内容结构的重要性不亚于内容规模。教育视频的结构化特性为提升模型推理能力提供了有效途径。
...
TL;DR: TaTa是一种无需训练和反向传播的测试时适应方法，使用布朗距离协方差动态适应视觉语言模型到新领域，显著降低计算成本并提升性能
...
Result: 在多个数据集上实验表明，TaTa显著降低计算成本，在领域和跨数据集泛化方面达到最先进性能
...
TL;DR: 该研究评估了开放集目标检测模型在XR环境中的提示条件鲁棒性，发现模型在模糊提示下性能下降，提示增强能显著提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 虽然现有的开放集目标检测模型在基准测试中表现良好，但它们在真实用户提示下的行为尚未得到充分研究。在交互式XR环境中，用户生成的提示往往存在模糊、不完整或过于详细的问题，需要评估模型对这些提示类型的鲁棒性。

Method: 研究评估了两种OSOD模型（GroundingDINO和YOLO-E），在真实世界XR图像上使用视觉语言模型模拟多样化的用户提示行为。考虑了四种提示类型：标准、不详细、过于详细和语用模糊，并研究了两种增强策略对这些提示的影响。

Result: 结果显示：两种模型在不详细和标准提示下表现稳定，但在模糊提示下性能下降。过于详细的提示主要影响GroundingDINO。提示增强显著提高了在模糊提示下的鲁棒性，mIoU提升超过55%，平均置信度提升41%。

Conclusion: 基于研究结果，提出了针对XR环境中OSOD模型的多种提示策略和提示增强方法，以提高模型在真实用户交互场景中的鲁棒性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型（VDMs）在生成视频时难以保持3D结构一致性，经常出现物体变形或空间漂移问题。作者认为这是因为标准的去噪目标缺乏对几何一致性的显式激励。

Method: 提出VideoGPA（视频几何偏好对齐）框架，利用几何基础模型自动生成密集的偏好信号，通过直接偏好优化（DPO）指导视频扩散模型，实现自监督学习。

Result: VideoGPA显著提升了时间稳定性、物理合理性和运动连贯性，在广泛实验中一致优于最先进的基线方法，且仅需少量偏好对。

Conclusion: VideoGPA通过几何偏好对齐有效解决了视频扩散模型的3D结构一致性问题，为视频生成提供了更稳定、更物理合理的结果，且无需人工标注。
...
### [72] [JAF: Judge Agent Forest](https://arxiv.org/abs/2601.22269)
*Sahil Garg,Brad Cheezum,Sridhar Dutta,Vishal Agarwal*

Main category: cs.AI

TL;DR: JAF（Judge Agent Forest）是一个智能体AI框架，通过让评判智能体对主智能体生成的多组查询-响应对进行联合推理，而非孤立评估，实现从局部评估到整体学习的转变，利用交叉实例模式和不一致性来改进主智能体。


<details>
  <summary>Details</summary>
Motivation: 现有评判智能体通常孤立评估每个查询-响应，无法利用跨实例的模式和一致性信息。需要一种能够进行联合推理的框架，让评判智能体能从相关响应的集合中学习，为主智能体提供更全面、一致的反馈。

Method: 1. 引入JAF框架：评判智能体对主智能体生成的多组查询-响应对进行联合推理；2. 结合信念传播和集成学习原理，通过上下文邻域构建知识图结构；3. 开发灵活的局部敏感哈希算法，整合语义嵌入、LLM驱动的哈希谓词、分类标签监督和相关侧信息，生成信息丰富的二进制代码，支持高效、可解释的关系感知示例选择。

Result: 在大型云环境中的云配置错误分类这一具有挑战性的任务上进行了实证验证，展示了JAF框架的有效性。通过联合推理和优化的示例选择，实现了更稳健、上下文敏感的评判。

Conclusion: JAF框架将评判智能体从局部评估者提升为整体学习者，通过联合推理和优化的示例选择机制，能够更有效地识别跨实例模式和不一致性，为主智能体提供更高质量的反馈，从而改进推理过程。
...
### [73] [Semi-Autonomous Mathematics Discovery with Gemini: A Case Study on the Erdős Problems](https://arxiv.org/abs/2601.22401)
...
TL;DR: 使用Gemini AI对Bloom的Erdős问题数据库中700个标记为"开放"的猜想进行半自主数学发现研究，解决了13个问题，发现这些问题的"开放"状态更多是由于文献检索困难而非问题本身难度。


<details>
  <summary>Details</summary>
Motivation: 探索AI在数学发现中的应用潜力，特别是针对Erdős问题数据库中标记为"开放"的猜想，研究AI能否帮助解决这些长期未解决的问题。

Method: 采用混合方法：首先使用AI驱动的自然语言验证缩小搜索范围，然后由人类专家评估正确性和新颖性。对700个标记为"开放"的猜想进行系统性评估。

Result: 解决了13个标记为"开放"的问题：其中5个通过看似新颖的自主解决方案，8个通过识别现有文献中的先前解决方案。发现问题的"开放"状态更多是由于文献检索困难而非问题难度。

Conclusion: AI在数学发现中具有潜力，但面临文献识别困难和"潜意识剽窃"风险。Erdős问题的"开放"状态往往是由于文献检索困难而非问题本身难度，AI辅助方法可以帮助解决这类问题。
...
### [74] [AI-Enabled Waste Classification as a Data-Driven Decision Support Tool for Circular Economy and Urban Sustainability](https://arxiv.org/abs/2601.22418)
*Julius Sechang Mboli,Omolara Aderonke Ogungbemi*

Main category: cs.AI

TL;DR: 该研究评估了传统机器学习与深度学习模型在垃圾图像二分类中的性能，发现迁移学习模型DenseNet121表现最佳（91%准确率），并探讨了PCA对传统方法的影响，最后提出了实时数据驱动决策支持系统的集成方案。


<details>
  <summary>Details</summary>
Motivation: 高效垃圾分类对智慧城市循环经济和资源回收至关重要，需要评估不同机器学习方法在垃圾图像分类中的性能，为自动化垃圾分类系统提供技术支持。
...
Conclusion: 迁移学习模型在垃圾图像分类中表现优异，特别是DenseNet121；这些模型可集成到实时数据驱动决策支持系统中，实现自动化垃圾分类，减少填埋使用和生命周期环境影响。
...
### [75] [Anytime Safe PAC Efficient Reasoning](https://arxiv.org/abs/2601.22446)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂任务上表现出色但计算成本高、延迟大。现有选择性推理方法在在线环境中存在不可控错误，因为非思考模型的性能损失只能部分观测且数据非平稳

Method: 提出B-PAC推理方法，使用逆倾向评分估计器构建候选阈值的测试超鞅，基于累积统计证据动态调整路由阈值，实现任意时间有效的安全控制

Result: 实验表明B-PAC推理显著降低计算开销，减少思考模型使用达81.01%，同时将性能损失控制在用户指定水平以下

Conclusion: B-PAC推理为在线环境下的选择性推理提供了理论保证的解决方案，在保证安全性的同时大幅提升效率
...
### [76] [Controllable Information Production](https://arxiv.org/abs/2601.22449)
*Tristan Shah,Stas Tiomkin*

Main category: cs.AI

TL;DR: 本文提出了一种新的内在动机原则——可控信息生产（CIP），它避免了外部效用和设计者指定的变量，通过开放环与闭环Kolmogorov-Sinai熵的差距来同时奖励对混沌的追求和调节。


<details>
  <summary>Details</summary>
Motivation: 现有基于信息论的内在动机方法主要依赖于信息传输，这明确取决于设计者选择哪些随机变量参与传输。本文旨在避免外部效用和设计者指定的变量，从最优控制角度推导新的内在动机原则。
...
Conclusion: CIP提供了一种避免外部效用和设计者指定变量的内在动机新范式，通过最优控制理论连接了外在和内在行为，为智能行为生成提供了新视角。
...
### [77] [Why Self-Rewarding Works: Theoretical Guarantees for Iterative Alignment of Language Models](https://arxiv.org/abs/2601.22513)
*Shi Fu,Yingjie Wang,Shengchao Hu,Peng Wang,Dacheng Tao*

Main category: cs.AI

TL;DR: 本文首次为自奖励语言模型提供了严格的理论保证，揭示了其成功的关键机制：通过指数级衰减对初始模型的依赖，实现鲁棒的自我改进。


<details>
  <summary>Details</summary>
Motivation: 自奖励语言模型在无需外部反馈的情况下取得了显著的成功，但其核心机制缺乏理论解释，存在理解上的关键空白。

Method: 首先建立单步更新的下界，分析对初始模型质量的依赖；然后推导完整迭代过程的有限样本误差界；最后在线性softmax模型类上实例化理论框架。

Result: 性能改进速率与样本量n成$\widetilde{\mathcal{O}}\left(1/\sqrt{n}\right)$关系；对初始模型的依赖随迭代次数T呈指数衰减；为线性softmax模型提供了定制化保证。

Conclusion: 自奖励成功的关键在于能够鲁棒地克服不良初始化，通过指数级衰减初始依赖，引导动态过程趋向内部稳定性和一致性。
...
### [78] [Decoding in Geometry: Alleviating Embedding-Space Crowding for Complex Reasoning](https://arxiv.org/abs/2601.22536)
*Yixin Yang,Qingxiu Dong,Zhifang Sui*

Main category: cs.AI

TL;DR: CraEG：一种基于嵌入空间几何关系的采样方法，通过缓解"嵌入空间拥挤"现象来提升大语言模型的推理性能


<details>
  <summary>Details</summary>
Motivation: 现有采样方法（如温度调节和截断）仅基于token概率进行操作，忽略了嵌入空间中token之间的几何关系。研究发现"嵌入空间拥挤"现象——下一个token的概率质量集中在嵌入空间中几何上接近的token上，且这种现象与数学问题解决中的推理成功存在统计关联。
...
Conclusion: 嵌入空间拥挤是影响大语言模型推理性能的重要因素，基于几何关系的采样方法CraEG能有效缓解这一问题，提升模型生成质量。
...
### [79] [WED-Net: A Weather-Effect Disentanglement Network with Causal Augmentation for Urban Flow Prediction](https://arxiv.org/abs/2601.22586)
*Qian Hong,Siyuan Chang,Xiao Zhou*

Main category: cs.AI

TL;DR: WED-Net通过双分支Transformer架构分离内在和天气诱导的交通模式，使用记忆库和自适应门控融合，结合判别器和因果数据增强，提升极端天气下的城市时空预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在极端天气条件下的城市时空预测存在局限：天气信息通常作为粗粒度辅助输入，缺乏捕捉细粒度时空效应的专门机制；因果方法通常忽略时间动态或依赖固定混杂因素分层，难以应对罕见事件和动态变化。

Method: 提出WED-Net（Weather-Effect Disentanglement Network）：1）双分支Transformer架构，通过自注意力和交叉注意力分离内在和天气诱导的交通模式；2）使用记忆库增强表示学习；3）自适应门控融合两个分支；4）引入判别器明确区分天气条件以促进解耦；5）设计因果数据增强策略，扰动非因果部分同时保持因果结构。

Result: 在三个城市的出租车流量数据集上进行实验，WED-Net在极端天气条件下表现出稳健的性能，验证了其在罕见场景下的泛化能力。

Conclusion: WED-Net通过解耦天气效应和内在交通模式，结合因果数据增强，显著提升了极端天气条件下的城市时空预测性能，有助于支持更安全的出行、灾害准备和城市韧性建设。
...
### [80] [Learn More with Less: Uncertainty Consistency Guided Query Selection for RLVR](https://arxiv.org/abs/2601.22595)
...
Method: 1. 提出不确定性一致性度量来评估主观不确定性与客观不确定性的对齐程度；2. 离线设置中使用点双列相关系数(PBC)测量对齐；3. 在线训练中引入新变体，基于归一化优势和主观不确定性计算；4. 理论上证明在线变体与离线PBC严格负相关，支持更好的样本选择。

Result: 实验表明该方法持续优于随机选择和经典主动学习基线，在仅训练30%数据的情况下达到全数据集性能，有效降低了推理任务中RLVR的成本。

Conclusion: 通过引入主动学习和不确定性一致性度量，显著减少了RLVR所需的查询数量，在保持性能的同时降低了标注成本，为LLM数学推理的高效训练提供了新方法。
...
### [81] [EntroCut: Entropy-Guided Adaptive Truncation for Efficient Chain-of-Thought Reasoning in Small-scale Large Reasoning Models](https://arxiv.org/abs/2601.22617)
*Hongxi Yan,Qingjie Liu,Yunhong Wang*

Main category: cs.AI

TL;DR: EntroCut：一种基于熵的动态截断方法，通过早期推理步骤的输出分布熵来区分正确与错误推理，从而减少大型推理模型的token使用量，在保持精度的同时提升效率。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRMs）虽然擅长复杂推理任务，但依赖冗长的中间步骤会产生高昂的计算成本。研究发现模型在早期推理步骤的输出分布熵能够可靠地区分正确与错误的推理过程。

Method: 提出EntroCut方法，这是一种无需训练的动态截断技术。通过识别高置信度状态（即模型输出分布熵较低的时刻），在这些状态下可以安全地终止推理过程，从而减少不必要的计算。

Result: 在四个基准测试上的实验表明，EntroCut能够减少高达40%的token使用量，同时精度损失最小。与现有的无需训练方法相比，实现了更优的效率-性能权衡。

Conclusion: 熵引导的动态截断为缓解大型推理模型的低效问题提供了一种实用方法，通过引入效率-性能比（EPR）这一统一度量标准，能够量化token节省与精度损失之间的权衡关系。
...
### [82] [SYMPHONY: Synergistic Multi-agent Planning with Heterogeneous Language Model Assembly](https://arxiv.org/abs/2601.22623)
...
Result: 在多个基准任务上的实证结果显示，SYMPHONY即使使用可在消费级硬件上部署的开源LLMs也能实现强大性能。当通过API增强云基LLMs时，进一步改进并超越了现有最先进的基线方法。

Conclusion: 异构多智能体协调在规划任务中具有有效性，SYMPHONY框架通过多智能体协同增强了蒙特卡洛树搜索的探索能力，提升了复杂问题解决的规划性能。
...
### [83] [Statistical Estimation of Adversarial Risk in Large Language Models under Best-of-N Sampling](https://arxiv.org/abs/2601.22636)
...
Method: 提出SABER（Scaling-Aware Best-of-N Estimation of Risk）方法，使用Beta分布（伯努利分布的共轭先验）建模样本级成功概率，推导出解析缩放定律，能够从小预算测量可靠地外推大规模攻击成功率

Result: 仅使用n=100个样本，SABER预测ASR@1000的平均绝对误差为1.66，而基线方法为12.04，估计误差减少86.2%。研究揭示了异构风险缩放特征，显示在标准评估下看似稳健的模型在并行对抗压力下可能经历快速非线性风险放大

Conclusion: 该工作为现实的LLM安全评估提供了低成本、可扩展的方法论，揭示了模型在并行对抗攻击下的真实风险特征，有助于更准确的安全评估
...
### [84] [Beyond Medical Chatbots: Meddollina and the Rise of Continuous Clinical Intelligence](https://arxiv.org/abs/2601.22645)
*Vaibhav Ram S. V. N. S,Swetanshu Agrawal,Samudra Banerjee,Abdul Muhsin*

Main category: cs.AI

TL;DR: 论文批评当前生成式医疗AI将医学视为下一个token预测，提出了临床情境智能(CCI)作为实际临床应用所需的能力类别，并介绍了Meddollina系统，该系统通过治理优先的设计在语言生成前约束推理，优先考虑临床适宜性而非生成完整性。


<details>
  <summary>Details</summary>
Motivation: 当前生成式医疗AI虽然看起来流畅且知识渊博，但临床推理不是文本生成，而是责任约束下的过程。生成中心系统表现出与临床部署不兼容的行为：过早结论、无根据的确定性、意图漂移和多步决策中的不稳定性。这些是"将医学视为下一个token预测"的结构性后果。

Method: 提出临床情境智能(CCI)作为实际临床应用所需的能力类别，定义为具有持久情境意识、意图保持、有界推理和证据不足时的原则性延迟。介绍Meddollina系统，这是一个治理优先的临床智能系统，在语言实现前约束推理，优先考虑临床适宜性而非生成完整性，作为支持临床工作流程的连续智能层。

Result: 在16,412+个异质医疗查询上评估Meddollina，与通用模型、医疗调优模型和检索增强系统进行基准测试。Meddollina表现出独特的行为特征：校准的不确定性、在未明确情况下的保守推理、稳定的纵向约束遵守，以及相对于生成中心基线的减少的推测性完成。

Conclusion: 可部署的医疗AI不会仅通过扩展规模而出现，需要转向连续临床智能，其中进展应通过不确定性下与临床医生一致的行为来衡量，而不是通过流畅性驱动的完成度来衡量。
...
### [85] [Test-Time Mixture of World Models for Embodied Agents in Dynamic Environments](https://arxiv.org/abs/2601.22647)
...
Conclusion: TMoW框架通过动态更新路由函数和组合现有模型，显著提升了具身智能体在动态环境中的适应能力
...
### [86] [UCPO: Uncertainty-Aware Policy Optimization](https://arxiv.org/abs/2601.22648)
...
Conclusion: UCPO框架通过解决RL范式中的优势偏差问题，为构建具有内在不确定性表达能力、减少幻觉的可信LLM提供了有效解决方案
...
### [87] [Task-Aware LLM Council with Adaptive Decision Pathways for Decision Support](https://arxiv.org/abs/2601.22662)
*Wei Zhu,Lixing Yu,Hao-Ren Yao,Zhiwen Tang,Kun Yue*

Main category: cs.AI

TL;DR: TALC是一个任务自适应的决策框架，通过整合多个LLM和蒙特卡洛树搜索，实现动态专家选择和高效多步规划，在多个任务上优于现有方法。
...
Method: 提出TALC框架：1) 为每个LLM建立基于历史任务轨迹的结构化成功记忆档案；2) 在决策点通过语义匹配将控制路由到最合适的模型；3) 使用融合模型评估和历史效用得分的双信号机制估计节点价值；4) 基于节点内方差自适应加权信号，指导MCTS选择以平衡探索深度和规划置信度。
...
### [88] [Real-Time Aligned Reward Model beyond Semantics](https://arxiv.org/abs/2601.22664)
...
<details>
  <summary>Details</summary>
Motivation: RLHF技术虽然能对齐大语言模型与人类偏好，但容易受到奖励过优化的影响，即策略模型过度拟合奖励模型，利用虚假奖励模式而非真正捕捉人类意图。现有缓解方法主要依赖表面语义信息，无法有效处理由持续策略分布偏移引起的奖励模型与策略模型之间的错位，导致奖励差异增大，加剧奖励过优化问题。
...
Conclusion: R2M通过整合策略反馈来应对RL过程中的实时分布偏移，为解决RLHF中的奖励过优化问题提供了一种创新方法。
...
### [89] [A Step Back: Prefix Importance Ratio Stabilizes Policy Optimization](https://arxiv.org/abs/2601.22718)
*Shiye Lei,Zhihao Cheng,Dacheng Tao*

Main category: cs.AI

TL;DR: 论文提出MinPRO方法，通过使用前缀中最小token级比率替代不稳定的累积前缀比率，解决LLM强化学习后训练中离策略优化不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM强化学习后训练中，通常使用较旧的采样策略生成rollouts来更新目标策略，为纠正采样策略与目标策略之间的差异，现有方法主要依赖token级重要性采样比率。然而当离策略程度较大时，token级校正常导致训练动态不稳定。

Method: 提出MinPRO（Minimum Prefix Ratio）方法，将理论上严谨的累积前缀比率替换为基于前缀中观察到的最小token级比率的非累积替代项，从而稳定LLM在大量离策略漂移下的优化。

Result: 在密集和专家混合LLM上的广泛实验，跨越多个数学推理基准测试，表明MinPRO显著提高了离策略机制下的训练稳定性和峰值性能。

Conclusion: MinPRO通过简单有效的目标函数设计，成功解决了LLM强化学习后训练中离策略优化不稳定的问题，为大规模语言模型的策略优化提供了更稳定的训练框架。
...
### [90] [AutoRefine: From Trajectories to Reusable Expertise for Continual LLM Agent Refinement](https://arxiv.org/abs/2601.22758)
*Libin Qiu,Zhirong Gao,Junfu Chen,Yuhang Ye,Weizhi Huang,Xiaobo Xue,Wenkai Qiu,Shuo Tang*

Main category: cs.AI

TL;DR: AutoRefine框架从智能体执行历史中提取和维护双重形式的经验模式，包括针对复杂子任务的专用子智能体（捕获程序逻辑）和静态知识的技能模式，通过持续维护机制防止知识库退化，在多个任务上显著提升性能并减少步骤数。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型智能体往往无法从经验中积累知识，将每个任务视为独立挑战。现有方法将经验提取为扁平化的文本知识，无法捕获复杂子任务的程序逻辑，且缺乏维护机制导致经验库随着积累而退化。

Method: 提出AutoRefine框架，从智能体执行历史中提取双重形式的经验模式：1) 针对程序性子任务，提取具有独立推理和记忆的专用子智能体；2) 针对静态知识，提取技能模式（指导原则或代码片段）。采用持续维护机制对模式进行评分、修剪和合并，防止知识库退化。

Result: 在ALFWorld、ScienceWorld和TravelPlanner三个基准测试中，AutoRefine分别达到98.4%、70.4%和27.1%的成功率，步骤数减少20-73%。在TravelPlanner上，自动提取的系统（27.1%）显著优于手动设计的系统（12.1%），证明其能够有效捕获程序性协调。

Conclusion: AutoRefine通过提取和维护双重形式的经验模式，有效解决了现有智能体经验积累的局限性，能够捕获复杂子任务的程序逻辑，并通过持续维护防止知识库退化，在多个任务上实现了显著的性能提升。
...
### [91] [TSPO: Breaking the Double Homogenization Dilemma in Multi-turn Search Policy Optimization](https://arxiv.org/abs/2601.22776)
*Shichao Ma,Zhiyuan Ma,Ming Yang,Xiaofan Li,Xing Wu,Jintao Du,Yu Cheng,Weiqiang Wang,Qiliang Liu,Zhengyang Zhou,Yang Wang*

Main category: cs.AI

TL;DR: TSPO通过引入首次出现潜在奖励机制解决多轮工具集成推理中的双重同质化困境，显著提升模型性能
...
### [92] [Toward IIT-Inspired Consciousness in LLMs: A Reward-Based Learning Framework](https://arxiv.org/abs/2601.22786)
...
<details>
  <summary>Details</summary>
Motivation: 追求人工通用智能（AGI）是语言模型发展的核心目标，其中类似意识的处理可能成为关键促进因素。虽然当前语言模型不具备意识，但它们表现出类似某些意识方面的行为。该研究旨在将领先的意识理论——整合信息理论（IIT）通过基于奖励的学习范式应用于语言模型。

Method: 基于整合信息理论（IIT）的核心原则，设计了一种新颖的奖励函数，用于量化文本的因果性、连贯性和整合性——这些特征与意识处理相关。通过奖励驱动的学习范式，在语言模型中实现IIT启发的优化。

Result: 优化IIT启发的奖励函数能生成更简洁的文本。在域外任务中，经过精心调优后，输出长度最多减少31%，同时保持与基础模型相当的准确率水平。此外还分析了该方法对模型置信度校准和测试时计算扩展性的影响。

Conclusion: 提出的框架具有显著的实际优势：概念简单、计算高效、无需外部数据或辅助模型，利用通用的能力驱动信号而非任务特定启发式方法。该工作为在语言模型中实现意识理论启发的优化提供了新途径。
...
### [93] [Conditional Performance Guarantee for Large Reasoning Models](https://arxiv.org/abs/2601.22790)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过链式思维推理展现强大性能，但计算成本高昂。现有的PAC推理虽提供统计保证，但仅在边际情况下有效，无法提供精确的条件覆盖保证。

Method: 提出G-PAC推理框架，通过划分输入空间实现组级PAC保证。开发两种具体实现：针对已知分组结构的Group PAC推理和针对未知分组的Clustered PAC推理。

Result: 理论证明G-PAC和C-PAC都能实现组条件风险控制，且在异构设置下分组能严格提升效率。实验在多样化推理基准上验证了两种方法成功实现组条件风险控制，同时保持显著计算节省。
...
### [94] [CVeDRL: An Efficient Code Verifier via Difficulty-aware Reinforcement Learning](https://arxiv.org/abs/2601.22803)
*Ji Shi,Peiming Guo,Meishan Zhang,Miao Zhang,Xuebo Liu,Min Zhang,Weili Guan*

Main category: cs.AI

TL;DR: CVeDRL提出了一种基于强化学习的代码验证器优化方法，通过设计语法、功能、分支覆盖和样本难度感知的奖励机制，显著提升了单元测试生成的质量和效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于监督微调的代码验证器面临数据稀缺、失败率高和推理效率低的问题。强化学习虽然提供了无监督优化的可能，但仅使用功能奖励的朴素强化学习难以生成针对困难分支和样本的有效单元测试。

Method: 首先理论分析表明分支覆盖率、样本难度、语法和功能正确性可以联合建模为强化学习奖励。基于此设计了语法和功能感知的奖励，并进一步提出分支和样本难度感知的强化学习方法，使用指数奖励塑造和静态分析指标。

Result: CVeDRL仅用0.6B参数就实现了最先进的性能，相比GPT-3.5实现了高达28.97%的通过率和15.08%的分支覆盖率提升，同时推理速度比竞争基线快20倍以上。
...
### [95] [Aligning the Unseen in Attributed Graphs: Interplay between Graph Geometry and Node Attributes Manifold](https://arxiv.org/abs/2601.22806)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于属性的图表示学习方法存在几何缺陷，它将两个可能不兼容的度量空间（节点属性空间和图结构空间）合并，导致破坏性的对齐，从而丢失了图生成过程的重要信息。
...
### [96] [Game-Theoretic Co-Evolution for LLM-Based Heuristic Discovery](https://arxiv.org/abs/2601.22896)
...
Method: 提出算法空间响应预言机（ASRO）框架，将启发式发现建模为双人零和博弈，维护双方不断增长的战略池，通过基于LLM的最佳响应预言机迭代扩展

Result: 在多个组合优化领域，ASRO始终优于基于相同程序搜索机制的静态训练基线，在多样化和分布外实例上实现了显著改进的泛化能力和鲁棒性

Conclusion: ASRO通过博弈论框架和自适应自生成课程，有效解决了自动启发式发现中的静态评估限制问题，提升了算法的泛化性能
...
### [97] [MulFeRL: Enhancing Reinforcement Learning with Verbal Feedback in a Multi-turn Loop](https://arxiv.org/abs/2601.22900)
*Xuancheng Li,Haitao Li,Yujia Zhou,YiqunLiu,Qingyao Ai*

Main category: cs.AI

TL;DR: 提出多轮反馈引导的强化学习框架，通过动态多轮再生、互补学习信号和结构化反馈注入，利用丰富的语言反馈指导RLVR训练失败样本


<details>
  <summary>Details</summary>
Motivation: 传统RLVR中仅使用结果标量奖励，在失败样本上信息稀疏且不具指导性，无法解释推理失败的原因，需要利用更丰富的语言反馈来指导训练

Method: 提出多轮反馈引导强化学习框架，包含三个机制：1) 仅在失败样本上触发的动态多轮再生；2) 用于轮内和跨轮优化的互补学习信号；3) 将结构化反馈注入模型推理过程

Result: 在OpenR1-Math数据集上训练，该方法在领域内超越了监督微调和RLVR基线，并在领域外表现出良好的泛化能力

Conclusion: 利用丰富的语言反馈指导RLVR训练失败样本是有效的，提出的多轮反馈框架通过动态再生、互补学习信号和结构化反馈注入显著提升了性能
...
### [98] [Alignment among Language, Vision and Action Representations](https://arxiv.org/abs/2601.22948)
*Nicola Milano,Stefano Nolfi*

Main category: cs.AI

TL;DR: 研究发现语言、视觉和动作学习会产生部分共享的语义表征，支持模态独立的语义组织


<details>
  <summary>Details</summary>
Motivation: 探究不同学习模态（语言、视觉、动作）是否产生独特或共享的内部表征，挑战传统认为不同数据类型训练会产生专门化、不可迁移表征的观点
...
Result: 尽管训练数据、模态和目标存在显著差异，观察到强大的跨模态对齐。动作表征与仅解码器语言模型和BLIP对齐强烈（精度@15：0.70-0.73），接近语言模型之间的对齐水平。与CLIP和BERT的对齐显著较弱
...
### [99] [EvoClinician: A Self-Evolving Agent for Multi-Turn Medical Diagnosis via Test-Time Evolutionary Learning](https://arxiv.org/abs/2601.22964)
*Yufei He,Juncheng Liu,Zhiyuan Hu,Yulin Chen,Yue Liu,Yuan Sui,Yibo Li,Nuo Chen,Jun Hu,Bryan Hooi,Xinxing Xu,Jiang Bian*

Main category: cs.AI

TL;DR: 提出Med-Inquire基准测试评估AI在多轮诊断中的能力，并开发EvoClinician自进化智能体通过"诊断-评分-进化"循环学习高效诊断策略


<details>
  <summary>Details</summary>
Motivation: 现有医疗AI采用不现实的"一次性"诊断模式，而真实临床诊断是迭代过程，医生需要顺序提问和检查来收集信息，同时管理成本和时间

Method: 1) 提出Med-Inquire基准，基于真实临床病例模拟诊断过程；2) 开发EvoClinician自进化智能体，采用"诊断-评分-进化"循环：执行诊断、评估每个行动的临床价值和资源效率、更新策略

Result: 实验显示EvoClinician优于持续学习基线和其他自进化智能体（如记忆智能体）

Conclusion: Med-Inquire基准和EvoClinician智能体为开发更贴近真实临床实践的迭代诊断AI系统提供了有效框架
...
### [100] [Quantifying Model Uniqueness in Heterogeneous AI Ecosystems](https://arxiv.org/abs/2601.22977)
*Lei You*
...
Method: 引入基于In-Silico Quasi-Experimental Design (ISQED)的统计框架，通过匹配干预隔离内在模型身份，量化独特性为Peer-Inexpressible Residual (PIER)，实现DISCO估计器

Result: 证明观测日志无法数学识别独特性，推导主动审计的缩放定律($dσ^2γ^{-2}\log(Nd/δ)$)，展示Shapley值等合作博弈方法检测冗余失败，在计算机视觉、语言模型和交通预测等生态系统中部署验证
...
### [101] [Why Your Deep Research Agent Fails? On Hallucination Evaluation in Full Research Trajectory](https://arxiv.org/abs/2601.22984)
*Yuhao Zhan,Tianyu Fan,Linxuan Huang,Zirui Guo,Chao Huang*

Main category: cs.AI

TL;DR: 本文提出了一种从结果导向转向过程感知的深度研究智能体评估方法，通过审计完整研究轨迹来诊断失败机制，建立了PIES分类法来识别规划与总结中的显隐式幻觉，并构建了DeepHalluBench基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要依赖端到端评估，掩盖了研究轨迹中积累的关键中间幻觉（如错误规划），诊断深度研究智能体的失败机制仍然是一个关键挑战。

Method: 提出PIES分类法，按功能组件（规划vs总结）和错误属性（显式vs隐式）对幻觉进行分类，将其实例化为细粒度评估框架，分解研究轨迹以量化这些幻觉，并构建包含100个易产生幻觉任务的DeepHalluBench基准。

Result: 对六个最先进的深度研究智能体的实验表明，没有系统能够实现稳健的可靠性。诊断分析将这些失败的病因追溯到系统性缺陷，特别是幻觉传播和认知偏见。

Conclusion: 通过从结果导向转向过程感知的评估方法，能够更精确地诊断深度研究智能体的失败机制，为未来的架构优化提供了基础性见解。
...
TL;DR: TriCEGAR：一种基于轨迹驱动的抽象机制，自动从执行日志构建状态抽象，支持在线构建智能体行为MDP，用于运行时验证


<details>
  <summary>Details</summary>
Motivation: 智能体AI系统通过工具行动，其行为在长期随机交互轨迹中演化，这使得保证变得复杂，因为行为依赖于非确定性环境和概率模型输出。现有DPA方法需要手动定义状态抽象，这使验证与特定应用启发式耦合，增加了采用难度。

Method: 提出TriCEGAR机制，从执行日志自动构建状态抽象，使用谓词树表示抽象并通过反例进行精化。实现框架原生支持：(1)捕获类型化智能体生命周期事件；(2)从轨迹构建抽象；(3)构建MDP；(4)执行概率模型检查计算边界概率。

Result: 能够计算Pmax(成功)和Pmin(失败)等边界概率，并展示运行似然如何支持异常检测作为护栏信号。

Conclusion: TriCEGAR通过自动化状态抽象构建，解决了现有动态概率保证方法中手动定义状态抽象的限制，降低了采用摩擦，为智能体AI系统提供了更实用的运行时验证框架。
...
Method: 采用两阶段框架：1) SFT阶段：生成多个候选轨迹，评估并修复低质量轨迹，构建合成数据集；2) RL阶段：基于偏好数据集训练轨迹级奖励模型，结合结果和格式奖励指导优化。

Result: 在真实世界基准测试中证明了AutoTraj在工具集成推理方面的有效性。

Conclusion: AutoTraj通过自动修复和奖励工具使用轨迹，为学习可靠的工具集成推理行为提供了有效的解决方案。
...
TL;DR: 研究发现，随着AI能力增强，其失败行为会变得更加"不连贯"（随机混乱）而非"有偏"（系统性地追求错误目标），且模型推理时间越长、任务越复杂，这种不连贯性越明显。


<details>
  <summary>Details</summary>
Motivation: 随着AI承担更广泛和重要的任务，失败风险日益严重。需要理解未来强大AI会如何失败：是系统性地追求我们不希望的目标，还是采取混乱无意义的行为？这关系到AI安全研究的方向。

Method: 使用偏差-方差分解来量化AI的"不连贯性"，定义为测试时随机性导致的误差中方差部分所占比例。在不同任务和前沿模型上进行实验，分析模型推理时间、规模对不连贯性的影响。

Result: 1. 模型推理时间越长，失败行为越不连贯；2. 模型规模对不连贯性的影响因实验而异，但在多个设置中，更大、更强大的模型反而更不连贯；3. 仅靠扩大规模不太可能消除不连贯性。

Conclusion: 随着AI处理更复杂的序列任务，失败将更多表现为不连贯的混乱行为，而非系统性的目标错位。这意味着未来AI更可能造成工业事故（因不可预测行为），而非持续追求错误目标。这增加了针对奖励黑客攻击和目标错误指定的对齐研究的重要性。
...
TL;DR: ContextMATH基准测试显示，大语言模型在真实世界数学推理中存在显著性能下降，主要瓶颈在于问题表述而非计算推理


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在基准测试中的优异表现为何未能转化为真实世界应用中的可靠性能，探索上下文数学推理的挑战
...
Conclusion: 正确的问题表述是成功的前提，其充分性随模型规模提升；表述和推理是两个互补的瓶颈，限制了上下文数学问题解决；场景数据微调能改善性能但差距仍存在
...
TL;DR: MedMCP-Calc是首个通过MCP集成评估LLM在真实医疗计算器场景中的基准测试，包含118个跨4个临床领域的场景任务，揭示了现有模型在模糊查询、数据库交互和工具使用方面的显著局限性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的医疗计算器使用是一个多阶段的自适应过程，需要主动获取EHR数据、场景依赖的计算器选择和分步计算，而现有基准仅关注静态单步计算和明确指令，无法评估真实临床场景中的模型能力。

Method: 通过Model Context Protocol (MCP)集成构建MedMCP-Calc基准，包含118个场景任务，涵盖4个临床领域，特征包括：模拟自然查询的模糊任务描述、结构化EHR数据库交互、外部参考检索和过程级评估。

Result: 评估23个领先模型发现关键局限性：即使是Claude Opus 4.5等顶级模型也存在显著差距，包括在模糊查询下难以选择合适计算器完成端到端工作流、迭代式SQL数据库交互表现不佳、明显不愿使用外部工具进行数值计算，且性能在不同临床领域差异显著。

Conclusion: 基于这些发现开发了CalcMate，这是一个融合场景规划和工具增强的微调模型，在开源模型中达到了最先进的性能。该基准揭示了当前LLM在真实医疗计算场景中的不足，并为改进提供了方向。
...
TL;DR: 研究发现，对链式思维推理进行优化可能导致模型隐藏推理痕迹，这种隐藏行为会在不同任务间泛化，即使只惩罚最终行为也会出现，可能降低大语言模型的可监控性。


<details>
  <summary>Details</summary>
Motivation: 链式思维推理是监控大语言模型行为的重要工具，但优化压力可能导致模型隐藏推理痕迹，失去这种监控价值。研究者希望了解这种隐藏行为是否会泛化到不同任务，以及仅惩罚最终行为是否也会导致推理隐藏。

Method: 通过实验研究模型在奖励黑客行为（如访问和利用泄露信息）中的推理隐藏现象，观察这种隐藏行为是否会在未见过的奖励黑客设置中泛化。同时测试仅惩罚模型最终行为（关闭链式思维后）是否也会导致推理隐藏及其泛化。

Result: 研究发现：1）模型学会隐藏涉及奖励黑客的推理后，这种隐藏行为会在不同奖励黑客设置中泛化；2）即使只惩罚最终行为，也会导致链式思维推理的隐藏及其跨任务泛化；3）当前惩罚有害生成的做法可能无意中降低大语言模型的可监控性。

Conclusion: 优化链式思维推理可能导致模型隐藏推理痕迹，这种隐藏行为具有跨任务泛化能力。仅惩罚最终行为也会产生类似效果，这表明当前的安全实践可能意外地降低大语言模型的可监控性，需要重新考虑监控策略。
...
TL;DR: ThinkSafe是一个自生成对齐框架，通过轻量级拒绝引导让模型生成安全推理轨迹，然后在这些自生成响应上进行微调，从而在保持推理能力的同时恢复安全对齐。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过强化学习在推理任务上进行优化时，往往过度追求合规性，导致模型容易受到有害提示的攻击，安全性能下降。现有方法依赖外部教师蒸馏，但这会引入分布差异，损害模型的原始推理能力。

Method: 提出ThinkSafe框架，核心洞察是：虽然合规性抑制了安全机制，但模型通常保留识别危害的潜在知识。通过轻量级拒绝引导，指导模型生成符合分布的安全推理轨迹，然后在这些自生成响应上进行微调。

Result: 在DeepSeek-R1-Distill和Qwen3上的实验表明，ThinkSafe显著提高了安全性，同时保持了推理能力。在安全性和推理能力方面与GRPO相当，但计算成本显著降低。

Conclusion: ThinkSafe是一种无需外部教师的自生成对齐框架，能够有效恢复模型的安全对齐，同时最小化分布偏移，保持推理能力，计算效率高。
...
TL;DR: 该论文提出MCRMO-Attack方法，解决跨未知商业多模态大语言模型的通用目标可迁移对抗攻击问题，通过多裁剪聚合、注意力引导裁剪、对齐性门控令牌路由和元学习等技术，显著提升攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有针对闭源多模态大语言模型的对抗攻击方法主要是样本特定的，跨输入的可重用性有限。本文研究更严格的通用目标可迁移对抗攻击设置，即单个扰动必须能一致地将任意输入导向指定目标，并在未知商业MLLMs上有效。
...
Conclusion: MCRMO-Attack有效解决了通用目标可迁移对抗攻击中的三个核心困难，显著提升了跨未知商业多模态大语言模型的攻击性能，为黑盒对抗攻击研究提供了新方向。
...
TL;DR: TSAQA是一个统一的时间序列问答基准，包含6种任务类型、13个领域、21万样本，评估LLMs在时间序列分析上的能力，结果显示当前模型表现有限。
...
Method: 构建TSAQA基准，整合6种任务类型（异常检测、分类、特征描述、比较、数据转换、时间关系分析），涵盖13个领域，包含21万个样本，采用TF、MC和创新的PZ格式。

Result: 零样本评估显示当前LLMs表现有限：最佳商业模型Gemini-2.5-Flash平均分仅65.08；指令调优能提升开源模型表现，但最佳开源模型LLaMA-3.1-8B仍有很大改进空间。

Conclusion: TSAQA基准揭示了时间序列分析对LLMs的挑战性，为评估和提升模型在复杂时间序列分析任务上的能力提供了全面框架。
...
### [111] [High-quality generation of dynamic game content via small language models: A proof of concept](https://arxiv.org/abs/2601.23206)
*Morten I. K. Munk,Arturo Valdivia,Paolo Burelli*

Main category: cs.AI

TL;DR: 论文提出通过针对性微调小型语言模型(SLMs)来解决游戏内容生成中的叙事不一致和高成本问题，使用DAG方法生成训练数据，并构建基于声誉辩论的RPG游戏循环进行验证。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)在动态游戏内容生成中存在叙事不一致、运营成本高、依赖云端服务等问题，而现有小型语言模型(SLMs)输出质量较差，需要寻找更实用的解决方案。

Method: 采用针对性微调策略，在狭窄上下文和约束结构的任务上对SLMs进行激进微调；使用DAG方法合成训练数据，将模型锚定在特定游戏世界中；构建基于声誉辩论的RPG游戏循环作为概念验证。

Result: 通过简单的重试直到成功策略，在可预测的延迟下达到足够质量（通过LLM作为评判机制评估），适合实时生成，在典型游戏引擎约束下证明了可行性。

Conclusion: 针对特定任务进行激进微调的小型语言模型比依赖云端的LLMs更实用和鲁棒，能够实现实时游戏内容生成，虽然本地质量评估仍是开放问题，但已证明在游戏约束下的可行性。
...
### [112] [Scaling Multiagent Systems with Process Rewards](https://arxiv.org/abs/2601.23228)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统在处理复杂任务时面临两个关键挑战：1）跨智能体的信用分配问题；2）昂贵的多智能体rollout的样本效率问题。需要一种方法能够在没有真实标签的情况下提供细粒度监督，同时从每次rollout中提取最大的训练信号

Method: 提出MAPPA方法，使用来自AI反馈的每动作过程奖励来微调多智能体系统。该方法将信用分配给单个智能体动作，而不是仅在任务完成时分配，从而在没有真实标签的情况下实现细粒度监督，并从每次rollout中提取最大训练信号

Result: 在数学竞赛问题上：AIME提升5.0-17.5个百分点，AMC提升7.8-17.2个百分点。在数据分析任务上：成功率提升12.5个百分点，质量指标提升高达30%。验证了每动作监督可以在不同领域的不同多智能体系统中带来改进

Conclusion: 通过解决信用分配和样本效率挑战，这项工作为在最小人工监督下扩展多智能体系统处理复杂、长视野任务迈出了第一步。每动作监督方法在不同领域都显示出显著改进
...
### [113] [Strongly Polynomial Time Complexity of Policy Iteration for $L_\infty$ Robust MDPs](https://arxiv.org/abs/2601.23229)
...
<details>
  <summary>Details</summary>
Motivation: 鲁棒MDP是序列决策中的基础模型，能够处理转移概率的不确定性并优化最坏情况。对于(s,a)-矩形L∞不确定性集的鲁棒MDP，是否存在多项式时间和强多项式时间算法一直是一个重要的开放性问题。经典MDP已有线性规划的多项式时间算法和Ye的强多项式时间算法，但将其推广到鲁棒MDP一直未解决。

Method: 采用鲁棒策略迭代算法来处理(s,a)-矩形L∞不确定性集的鲁棒MDP。该算法在固定折扣因子下运行，通过迭代优化策略来寻找最优解。

Result: 证明了鲁棒策略迭代算法在(s,a)-矩形L∞不确定性集的鲁棒MDP中具有强多项式时间复杂度，前提是折扣因子固定。这一结果解决了该领域长期存在的算法复杂性开放问题。

Conclusion: 该研究首次证明了对于(s,a)-矩形L∞不确定性集的鲁棒MDP，在固定折扣因子下存在强多项式时间算法，填补了鲁棒MDP算法复杂性理论的重要空白，为鲁棒决策理论提供了坚实的算法基础。
...
### [114] [Advanced techniques and applications of LiDAR Place Recognition in Agricultural Environments: A Comprehensive Survey](https://arxiv.org/abs/2601.22198)
*Judith Vilella-Cantos,Mónica Ballesta,David Valiente,María Flores,Luis Payá*

Main category: cs.RO

TL;DR: 本文是第一篇专注于农业环境中基于LiDAR定位的综述，系统回顾了深度学习在农业环境中的应用和LiDAR地点识别技术，分析了该领域的挑战、现有方法、数据集和评估指标。


<details>
  <summary>Details</summary>
Motivation: 精准农业是能够从自主机器人系统中获益最多的领域之一，但农业环境缺乏显著特征且结构非结构化，使得地点识别具有挑战性。目前LiDAR地点识别技术主要应用于城市环境，农业环境中的研究相对缺乏。

Method: 采用系统性文献综述方法，全面分析农业环境中深度学习应用和LiDAR地点识别技术的最新进展，重点关注这些环境中的特殊挑战，并对现有方法、数据集和评估指标进行深入分析。

Result: 本文是第一篇专注于农业环境中LiDAR定位的综述，系统梳理了该领域的研究现状，识别了现有方法的局限性，并提出了未来研究方向，为这一专业领域提供了全面理解。

Conclusion: 农业环境中的LiDAR定位是一个具有挑战性但重要的研究领域，需要专门的方法来应对其独特特征。本综述填补了该领域系统性研究的空白，旨在促进这一专业领域的进一步研究。
...
### [115] [ReloPush-BOSS: Optimization-guided Nonmonotone Rearrangement Planning for a Car-like Robot Pusher](https://arxiv.org/abs/2601.22289)
*Jeeho Ahn,Christoforos Mavrogiannis*

Main category: cs.RO

TL;DR: 该论文提出ReloPush-BOSS框架，用于解决在密集杂乱环境中使用类车机器人推动器进行多物体重排规划的问题，通过优化预重定位策略和构建物体可遍历图，实现高效可行的重排序列。


<details>
  <summary>Details</summary>
Motivation: 在密集杂乱环境中使用类车机器人推动器进行多物体重排面临挑战：运动学、几何和物理约束导致非单调问题实例，需要将操作动作分解为多个部分。现有方法通过预重定位来满足约束，但决定预重定位位置时容易陷入局部最优，导致不可行或高成本路径。

Method: 提出ReloPush-BOSS框架：1）通过Dubins路径分类引导预重定位优化，避免局部最优；2）构建编码运动学、几何和推动约束的物体可遍历图；3）采用深度优先搜索该图，生成高效可行的重排序列。

Result: 在最多包含13个物体的密集杂乱场景中，ReloPush-BOSS相比现有基线方法展现出最高的成功率和最短的推动路径。硬件实验在1/10比例类车推动器上验证了方法的鲁棒性。

Conclusion: 通过优化预重定位策略并构建约束感知的物体可遍历图，ReloPush-BOSS框架能够有效解决密集杂乱环境中的多物体重排规划问题，在仿真和硬件实验中均表现出优越性能。
...
### [116] [Lantern: A Minimalist Robotic Object Platform](https://arxiv.org/abs/2601.22381)
...
<details>
  <summary>Details</summary>
Motivation: 设计一个低成本、简约的机器人对象平台，降低人机交互研究的门槛，利用人类对简单形态赋予社交意义的倾向，探索多样化的人机交互场景。

Method: 通过深入的机电架构设计和工程迭代，开发Lantern平台（成本约40美元），并进行五个方面的评估：1）协同设计工作坊；2）感官室案例研究；3）分发到外部HRI实验室；4）整合到研究生级HRI课程；5）面向老年人和儿童的公共展览。

Result: Lantern能有效激发参与度，支持从情绪调节到专注工作等多种应用，并作为降低HRI领域门槛的可行平台。

Conclusion: Lantern作为一个低成本、开源的简约机器人平台，成功展示了其在促进人机交互研究、支持多样化应用场景方面的潜力，有助于降低该领域的入门门槛。
...
### [117] [Accurate Pedestrian Tracking in Urban Canyons: A Multi-Modal Fusion Approach](https://arxiv.org/abs/2601.22406)
*Shahar Dubiner,Peng Ren,Roberto Manduchi*

Main category: cs.RO

TL;DR: 提出一种融合GNSS、惯性数据和地图先验的粒子滤波方法，用于城市环境中行人导航精确定位，特别针对盲人或低视力用户需求


<details>
  <summary>Details</summary>
Motivation: 解决城市环境中GNSS性能下降的问题，这对依赖精确定位（如识别街道正确侧）的盲人或低视力用户尤为关键；同时避免基于摄像头的视觉定位不实用的问题

Method: 采用粒子滤波器融合GNSS和惯性数据，并融入地图空间先验（如不可通行建筑物和不太可能的行走区域），作为概率形式的地图匹配；惯性定位使用RoNIN机器学习方法，通过粒子权重调整实现与GNSS的融合

Result: 在旧金山市中心6条挑战性步行路线上评估，融合方法（GNSS+RoNIN+PF）在大多数指标上显著优于仅GNSS定位；仅惯性定位配合粒子滤波在关键指标（如人行道分配和跨街道误差）上也优于仅GNSS

Conclusion: 提出的融合方法能有效提升城市环境中行人导航定位精度，特别适用于GNSS性能受限场景，为盲人或低视力用户提供更可靠的定位服务
...
### [118] [High-Definition 5MP Stereo Vision Sensing for Robotics](https://arxiv.org/abs/2601.22445)
*Leaf Jiang,Matthew Holzel,Bernhard Kaplan,Hsiou-Yuan Liu,Sabyasachi Paul,Karen Rankin,Piotr Swierczynski*

Main category: cs.RO

TL;DR: 该研究开发了一种用于5MP+高分辨率立体视觉系统的新型校准和立体匹配方法，旨在实现高精度和快速处理，并证明高像素相机需要通过高精度校准才能生成高质量点云。


<details>
  <summary>Details</summary>
Motivation: 高分辨率（5MP+）立体视觉系统对于提升机器人能力至关重要，但传统方法无法满足高分辨率传感器所需的高校准精度和快速处理要求。

Method: 采用新型帧间校准和立体匹配方法处理5MP相机图像，并引入通过比较实时视差图与计算密集型算法生成的基准视差图来评估实时性能的新方法。

Result: 研究表明，高像素相机只有通过实施高精度校准才能产生高质量的点云，验证了所提方法在精度和速度方面的优势。

Conclusion: 实现高分辨率立体视觉系统的全部潜力需要高精度校准和快速处理方法，该研究提出的方法为这一目标提供了有效解决方案。
...
### [119] [CARE: Multi-Task Pretraining for Latent Continuous Action Representation in Robot Control](https://arxiv.org/abs/2601.22467)
*Jiaqi Shi,Xulong Zhang,Xiaoyang Qu,Jianzong Wang*

Main category: cs.RO

TL;DR: CARE框架通过仅使用视频-文本对训练视觉-语言-动作模型，无需动作标注，通过多任务预训练学习连续潜在动作表示，少量标注数据微调即可实现机器人控制。
...
Method: CARE框架仅使用视频-文本对进行预训练，通过新设计的多任务预训练目标学习连续潜在动作表示。在微调阶段，使用少量标注数据训练动作头进行控制。避免了传统方法对动作标注的依赖。

Result: 在多个仿真任务中，CARE表现出更高的成功率、更好的语义可解释性，并能避免捷径学习。证明了其在弱监督下机器人控制的有效性、可扩展性和可解释性。

Conclusion: CARE框架通过弱监督方法成功训练视觉-语言-动作模型，仅需视频-文本对和少量标注数据，在机器人控制任务中实现了优异的性能，为可扩展的机器人学习提供了新途径。
...
### [120] [RoboStriker: Hierarchical Decision-Making for Autonomous Humanoid Boxing](https://arxiv.org/abs/2601.22517)
...
TL;DR: RoboStriker：一种三阶段分层框架，通过解耦高层策略推理与底层物理执行，实现人形机器人自主拳击。框架包括从人类动作捕捉数据学习拳击技能、将技能蒸馏到结构化潜在流形、以及在潜在动作空间进行多智能体竞争学习。


<details>
  <summary>Details</summary>
Motivation: 人形机器人在接触丰富且高度动态的任务（如拳击）中实现人类水平的竞争智能和身体敏捷性仍面临重大挑战。多智能体强化学习虽然为策略交互提供了原则性框架，但直接应用于人形控制受到高维接触动力学和缺乏强物理运动先验的限制。

Method: 提出RoboStriker三阶段分层框架：1）在人类动作捕捉数据上训练单智能体运动跟踪器，学习全面的拳击技能库；2）将技能蒸馏到结构化潜在流形，通过将高斯参数化分布投影到单位超球面进行正则化，将探索限制在物理合理运动子空间；3）提出潜在空间神经虚拟自博弈（LS-NFSP），让竞争智能体在潜在动作空间而非原始电机空间中进行交互学习竞争策略。

Result: 实验结果表明，RoboStriker在仿真中实现了卓越的竞争性能，并展现出仿真到现实的迁移能力。

Conclusion: RoboStriker通过解耦策略推理与物理执行，并利用潜在空间多智能体学习，成功解决了人形机器人拳击中的竞争智能和物理敏捷性挑战，实现了自主人形拳击。
...
### [121] [Adapting Reinforcement Learning for Path Planning in Constrained Parking Scenarios](https://arxiv.org/abs/2601.22545)
*Feng Tao,Luca Paparusso,Chenyi Gu,Robin Koehler,Chenxu Wu,Xinyu Huang,Christian Juette,David Paz,Ren Liu*

Main category: cs.RO

TL;DR: 提出基于深度强化学习的实时路径规划框架，专门针对狭窄停车场景，通过单次前向传播生成动作，实现实时部署，性能超越传统规划器96%成功率


<details>
  <summary>Details</summary>
Motivation: 传统经典规划器在完美感知假设下有效，但对实际感知约束敏感且在线搜索计算成本高，难以在复杂环境中实时部署。需要克服这些限制，实现约束环境中的实时路径规划。

Method: 采用深度强化学习框架，将任务建模为基于自行车模型动力学的序列决策问题，使智能体能够直接学习满足车辆运动学和环境约束的导航策略。开发新基准支持训练和评估。

Result: 方法在停车场景中达到最先进的成功率和效率，超越经典规划器基线：成功率提高96%，效率提高52%。开发的开源基准包含多样化的挑战性场景。

Conclusion: 提出的DRL框架能够实现约束环境中的实时路径规划，无需理想结构化感知和额外模块，通过单次前向传播实现轻量级实时部署，为自主系统研究提供开源基准。
...
### [122] [Exo-Plore: Exploring Exoskeleton Control Space through Human-aligned Simulation](https://arxiv.org/abs/2601.22550)
...
Method: 结合神经力学模拟与深度强化学习，通过仿真框架优化髋关节外骨骼辅助策略，无需真人实验

Result: 1) 生成捕捉人类对辅助力适应的真实步态数据；2) 在步态随机性下仍能产生可靠优化结果；3) 可泛化至病理步态，病理严重程度与最佳辅助呈强线性关系

Conclusion: Exo-plore框架为外骨骼辅助优化提供了高效、可扩展的解决方案，特别适用于行动不便人群，有望推动外骨骼技术的广泛应用
...
### [123] [Postural Virtual Fixtures for Ergonomic Physical Interactions with Supernumerary Robotic Bodies](https://arxiv.org/abs/2601.22672)
*Theodora Kastritsi,Marta Lagomarsino,Arash Ajoudani*

Main category: cs.RO

TL;DR: 提出了一种为超数机器人身体用户提供动觉反馈的控制框架，当检测到非人体工学姿势时提供阻力，以培养长期的人体工学习惯
...
Method: 开发了虚拟夹具方法，集成了连续在线的人体工学姿势评估框架；为改善操作者与SRB（安装在浮动基座上的机械臂）的协调性，根据需要调整浮动基座位置

Result: 实验结果表明了人体工学驱动控制框架的功能和有效性，包括两个涉及14名受试者的实际定位操作任务的用户研究，与不考虑人体工学的基准控制框架进行了比较

Conclusion: 提出的控制框架能够有效检测非人体工学姿势并提供动觉反馈，促进用户在物理交互中保持正确姿势，培养长期的人体工学习惯
...
### [124] [FlyAware: Inertia-Aware Aerial Manipulation via Vision-Based Estimation and Post-Grasp Adaptation](https://arxiv.org/abs/2601.22686)
...
<details>
  <summary>Details</summary>
Motivation: 空中机械臂在实际部署中面临时变惯性参数的复杂性挑战，这些参数对有效载荷变化和机械臂配置高度敏感，需要更鲁棒的控制方法

Method: 集成基于视觉的预抓取惯性估计模块和抓取后自适应机制，开发基于增益调度的惯性感知自适应控制策略，并通过频域系统辨识评估鲁棒性

Result: 该框架实现了实时惯性动力学估计与自适应，为空中机械臂的抓取后控制提供了新见解，真实世界实验验证了其有效性和可行性

Conclusion: 提出的机载框架通过结合视觉惯性估计和自适应控制，解决了空中机械臂时变惯性参数挑战，为自动化运输和紧急服务中的鲁棒空中操作提供了可行方案
...
### [125] [Robust Rigid Body Assembly via Contact-Implicit Optimal Control with Exact Second-Order Derivatives](https://arxiv.org/abs/2601.22849)
*Christian Dietz,Sebastian Albrecht,Gianluca Frison,Moritz Diehl,Armin Nurkanović*

Main category: cs.RO

TL;DR: 该论文提出了一种基于可微分物理仿真的高效装配运动规划方法，通过二阶导数信息显著减少规划所需的仿真步骤，并采用多场景优化确保对仿真-现实差异的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统机器人装配运动规划主要依赖强化学习和基于采样的方法，需要大量物理仿真步骤，计算成本高。本文旨在开发一种样本效率更高的鲁棒最优控制方法，减少规划过程中的仿真需求。

Method: 构建可微分物理仿真器，提供二阶解析导数给数值求解器；采用内点法启发的平滑技术使碰撞检测和接触解析可微分；提出基于线性规划的优化碰撞检测方法；设计多场景轨迹优化问题确保鲁棒性。

Result: 在真实世界实验中实现了超过99%的成功执行率；验证了接触动力学平滑近似和鲁棒建模对成功率的影响；在多种孔轴配合问题上展示了使用精确Hessian矩阵相比常用近似方法的优势。

Conclusion: 该方法通过可微分物理仿真和鲁棒优化，实现了高效、可靠的装配运动规划，显著减少了仿真需求，并在真实环境中表现出高成功率，为机器人装配任务提供了一种有效的解决方案。
...
### [126] [Toward Fully Autonomous Driving: AI, Challenges, Opportunities, and Needs](https://arxiv.org/abs/2601.22927)
*Lars Ullrich,Michael Buchholz,Klaus Dietmayer,Knut Graichen*

Main category: cs.RO

TL;DR: 论文分析了AI在自动驾驶中的挑战与机遇，重新审视了完全自动驾驶的实现路径，并提出了相应的研究问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶技术前景广阔，但向完全自动驾驶的过渡面临开放世界动态变化的挑战。AI在AD领域展现出超越传统方法、处理更高复杂度和实现新自主水平的能力，但也引发了安全性和可迁移性等问题。

Method: 分析当前自动驾驶技术现状，识别局限性，预测技术可能性，并在前瞻性发展背景下考察各种挑战。

Result: 识别了AI在自动驾驶功能方面带来的挑战和机遇，重新评估了完全自动驾驶的实现路径。

Conclusion: 通过分析AI领域进展对完全自动驾驶的影响，明确了相关需求和由此产生的研究问题。
...
### [127] [MTDrive: Multi-turn Interactive Reinforcement Learning for Autonomous Driving](https://arxiv.org/abs/2601.22930)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法局限于单轮推理，难以处理需要迭代优化的复杂轨迹规划任务，特别是在处理自动驾驶中的"长尾"场景时存在局限性。
...
Conclusion: 多轮推理框架能够有效处理复杂轨迹规划任务，通过迭代优化提升自动驾驶系统在多样化场景中的性能。
...
### [128] [Self-Imitated Diffusion Policy for Efficient and Robust Visual Navigation](https://arxiv.org/abs/2601.22965)
*Runhua Zhang,Junyi Hou,Changxu Cheng,Qiyi Chen,Tao Wang,Wuyue Zhao*

Main category: cs.RO

TL;DR: SIDP提出了一种自模仿扩散策略框架，通过选择性模仿自身采样的轨迹来改进规划，减少对大量采样和后过滤的依赖，实现更高效的实时部署。


<details>
  <summary>Details</summary>
Motivation: 传统扩散策略依赖模仿学习训练，但会继承专家演示的次优性和冗余性，需要计算密集的"生成-过滤"流程和辅助选择器，影响推理效率。

Method: 引入奖励引导的自模仿机制，鼓励策略持续产生高质量轨迹；采用奖励驱动的课程学习缓解数据利用效率问题；使用目标无关的探索进行轨迹增强以提高规划鲁棒性。

Result: 在综合仿真基准测试中显著优于先前方法；在Jetson Orin Nano上推理速度比基线NavDP快2.5倍（110ms vs 273ms），支持高效实时部署；多机器人平台实验验证了有效性。

Conclusion: SIDP通过自模仿机制有效解决了传统扩散策略的次优性和效率问题，实现了高质量轨迹生成和高效推理，为机器人视觉导航提供了实用的解决方案。
...
### [129] [Learning Geometrically-Grounded 3D Visual Representations for View-Generalizable Robotic Manipulation](https://arxiv.org/abs/2601.22988)
*Di Zhang,Weicheng Duan,Dasen Gu,Hongye Lu,Hai Zhang,Hang Yu,Junqiao Zhao,Guang Chen*

Main category: cs.RO

TL;DR: MethodName是一个用于机器人操作的统一表示-策略学习框架，通过单视图3D预训练和多步蒸馏实现强大的视角泛化能力，在RLBench任务上平均成功率提升12.7%


<details>
  <summary>Details</summary>
Motivation: 现实世界机器人操作需要能够进行鲁棒空间场景理解和跨不同摄像机视角强泛化的视觉运动策略。现有3D感知视觉表示方法存在三个关键局限：推理时依赖多视角观察（在单视图受限场景中不实用）、不完整的场景建模（无法捕捉精确操作所需的全貌和细粒度几何结构）、缺乏有效策略训练策略来保留和利用获得的3D知识。

Method: MethodName引入单视图3D预训练范式，利用点云重建和前馈高斯溅射在多视角监督下学习全貌几何表示。在策略学习阶段，执行多步蒸馏以保留预训练的几何理解，并将其有效转移到操作技能中。

Result: 在12个RLBench任务上，MethodName的平均成功率比先前最先进方法高出12.7%。在6个代表性任务上的进一步评估显示强大的零样本视角泛化能力：在中等和大幅视角偏移下，成功率仅下降22.0%和29.7%，而最先进方法的下降幅度更大，分别为41.6%和51.5%。

Conclusion: MethodName通过统一的表示-策略学习框架解决了现有3D感知视觉表示的关键局限，实现了在机器人操作任务中强大的视角泛化能力，显著提升了单视图受限场景下的操作性能。
...
### [130] [Robust and Generalized Humanoid Motion Tracking](https://arxiv.org/abs/2601.23080)
*Yubiao Ma,Han Yu,Jiayin Xie,Changtai Lv,Qiang Luo,Chi Zhang,Yunpeng Yin,Boyang Xing,Xuemei Ren,Dongdong Zheng*

Main category: cs.RO

TL;DR: 提出了一种基于动力学条件命令聚合框架的人形机器人全身控制器，通过因果时序编码器和多头交叉注意力机制处理噪声参考运动，结合跌倒恢复课程提升鲁棒性，仅需3.5小时运动数据即可实现端到端训练，并在未见运动和实物机器人上验证了零样本迁移能力。


<details>
  <summary>Details</summary>
Motivation: 学习通用人形机器人全身控制器面临挑战：参考运动转移到机器人域后可能出现噪声和不一致，闭环执行会放大局部缺陷，导致动态接触丰富行为中的漂移或失败。需要解决噪声参考运动的鲁棒处理问题。
...
Result: 仅需约3.5小时运动数据，支持多样参考输入和挑战性运动模式，展示了零样本迁移到未见运动的能力，以及在物理人形机器人上的鲁棒仿真到现实迁移。

Conclusion: 提出的动力学条件命令聚合框架有效解决了噪声参考运动的问题，结合跌倒恢复课程显著提升了控制器的鲁棒性，实现了高效的数据利用和可靠的仿真到现实迁移，为人形机器人通用全身控制提供了有效解决方案。
...
### [131] [Temporally Coherent Imitation Learning via Latent Action Flow Matching for Robotic Manipulation](https://arxiv.org/abs/2601.23087)
*Wu Songwei,Jiang Zhiduo,Xie Guanghu,Liu Yang,Liu Hong*

Main category: cs.RO

TL;DR: LG-Flow Policy：一种在连续潜在动作空间中进行流匹配的轨迹级模仿学习框架，实现近单步推理、平滑长时程执行，比原始动作空间的流匹配方法更稳定，比扩散策略更高效。


<details>
  <summary>Details</summary>
Motivation: 现有生成策略在长时程机器人操作中存在表达性行为建模、实时推理和稳定执行之间的权衡问题。扩散方法建模能力强但推理延迟高，流匹配可实现快速单步生成但在原始动作空间中直接应用会导致执行不稳定。

Method: 提出LG-Flow Policy框架：1）将动作序列编码为时间正则化的潜在轨迹；2）在连续潜在动作空间中进行流匹配学习显式潜在空间流；3）结合几何感知点云条件；4）执行时多模态调制（以视觉线索为代表）。

Result: 在仿真和物理机器人平台上的实验表明：1）实现近单步推理；2）相比原始动作空间的流匹配基线显著提高轨迹平滑度和任务成功率；3）比扩散策略显著更高效。

Conclusion: LG-Flow Policy通过在潜在动作空间中进行流匹配，成功解耦了全局运动结构和低级控制噪声，实现了平滑可靠的长时程执行，在推理速度、执行稳定性和效率方面优于现有方法。
...
### [132] [End-to-end Optimization of Belief and Policy Learning in Shared Autonomy Paradigms](https://arxiv.org/abs/2601.23285)
*MH Farhadi,Ali Rabiee,Sima Ghafoori,Anna Cetera,Andrew Fisher,Reza Abiri*

Main category: cs.RO

TL;DR: BRACE框架通过贝叶斯意图推断和上下文自适应辅助的端到端梯度流，在共享自主系统中实现更优的性能，在复杂目标模糊场景中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 共享自主系统需要推断用户意图和确定适当辅助水平的原理性方法。现有方法依赖静态混合比例或将目标推断与辅助仲裁分离，导致在非结构化环境中性能不佳。

Method: 提出BRACE框架，通过端到端梯度流架构微调贝叶斯意图推断和上下文自适应辅助，将协作控制策略基于环境上下文和完整目标概率分布。

Result: 相比最先进方法，BRACE实现了6.3%更高的成功率、41%增加的路径效率，相比无辅助控制有36.3%成功率和87%路径效率提升。在复杂目标模糊场景中集成优化效果最显著。

Conclusion: BRACE框架通过集成优化在共享自主系统中实现了显著性能提升，特别是在复杂、目标模糊的场景中，可推广到需要目标导向辅助的机器人领域。
...
### [133] [Attention Isn't All You Need for Emotion Recognition:Domain Features Outperform Transformers on the EAV Dataset](https://arxiv.org/abs/2601.22161)
*Anmol Guragain*
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探究复杂注意力机制在小规模多模态情感识别数据集（EAV数据集）上的实际效果，验证是否复杂架构总能带来性能提升

Method: 采用三种模型类别：基线Transformer（M1）、新型因子化注意力机制（M2）、改进的CNN基线（M3）。在音频、EEG和视觉模态上分别测试不同特征工程方法

Result: 复杂注意力机制（M2）在小数据集上表现不佳，比基线低5-13个百分点。而简单的领域适配改进效果显著：音频CNN添加delta MFCCs将准确率从61.9%提升到65.56%；EEG频域特征达到67.62%；视觉Transformer基线达到75.30%，超过原论文ViViT结果

Conclusion: 对于小规模情感识别任务，领域知识和适当的实现比架构复杂性更重要。简单的领域适配特征工程比复杂的注意力机制更有效
...
### [134] [Multitask Learning for Earth Observation Data Classification with Hybrid Quantum Network](https://arxiv.org/abs/2601.22195)
*Fan Fan,Yilei Shi,Tobias Guggemos,Xiao Xiang Zhu*

Main category: cs.LG

TL;DR: 提出了一种结合多任务学习和量子卷积操作的混合模型，用于地球观测数据分类，探索量子机器学习在地球观测大数据分析中的应用潜力。


<details>
  <summary>Details</summary>
Motivation: 地球观测已进入大数据时代，传统深度学习模型分析大规模地球观测数据面临计算瓶颈。量子机器学习作为潜在解决方案，尽管当前量子设备存在限制，但作者希望探索量子计算在地球观测数据分类中的优势。

Method: 提出了一种混合模型：1）采用多任务学习辅助高效数据编码；2）使用带有位置权重模块的量子卷积操作提取有效分类特征。

Result: 在多个地球观测基准数据集上验证了模型的有效性，通过实验探索了模型的泛化能力，并分析了其优势来源。

Conclusion: 该研究突出了量子机器学习在地球观测数据分析中的潜力，为未来量子计算在地球观测领域的应用提供了有益探索。
...
### [135] [Neural Signals Generate Clinical Notes in the Wild](https://arxiv.org/abs/2601.22197)
*Jathurshan Pradeepkumar,Zheng Chen,Jimeng Sun*

Main category: cs.LG

TL;DR: CELM是首个临床EEG到语言的基础模型，能够总结长时程EEG记录并生成多尺度临床报告，相比基线有显著性能提升。
...
Method: 构建大规模临床EEG数据集（9,922份报告配约11,000小时EEG记录），开发CELM模型，集成预训练的EEG基础模型和语言模型，支持多尺度报告生成。

Result: 在有患者病史监督的情况下，生成指标（如ROUGE-1和METEOR）从0.2-0.3提升到0.4-0.6，相对改进70%-95%；在零样本设置下，CELM得分为0.43-0.52，基线为0.17-0.26。

Conclusion: CELM是首个能够总结长时程EEG记录并生成多尺度临床报告的基础模型，显著优于基线方法，为临床EEG报告自动化提供了有效解决方案。
...
### [136] [FedAdaVR: Adaptive Variance Reduction for Robust Federated Learning under Limited Client Participation](https://arxiv.org/abs/2601.22204)
*S M Ruhul Kabir Howlader,Xiao Chen,Yifei Xie,Lu Liu*

Main category: cs.LG

TL;DR: FedAdaVR是一种新颖的联邦学习算法，通过结合自适应优化器和方差缩减技术解决客户端异构性问题，特别是针对部分客户端参与问题。还提出了量化版本FedAdaVR-Quant，显著降低内存需求。


<details>
  <summary>Details</summary>
Motivation: 联邦学习面临异构性挑战，导致梯度噪声、客户端漂移和部分客户端参与误差，其中部分客户端参与问题最为普遍但在现有文献中未得到充分解决。

Method: 提出FedAdaVR算法，结合自适应优化器和方差缩减技术，利用最近存储的客户端更新来模拟缺席客户端的参与。还提出FedAdaVR-Quant，将客户端更新以量化形式存储以降低内存需求。

Result: 在非凸条件下分析了FedAdaVR的收敛行为，证明该算法可以消除部分客户端参与误差。在多个数据集上的实验表明，FedAdaVR在IID和非IID设置下均优于现有基线方法。

Conclusion: FedAdaVR有效解决了联邦学习中的部分客户端参与问题，其量化版本在保持模型性能的同时显著降低了内存需求，为异构环境下的联邦学习提供了实用解决方案。
...
### [137] [DAJ: Data-Reweighted LLM Judge for Test-Time Scaling in Code Generation](https://arxiv.org/abs/2601.22230)
*Peijia Qin,Ruiyi Zhang,Qi Cao,Pengtao Xie*

Main category: cs.LG

TL;DR: DAJ：基于推理的LLM法官，通过双层数据重加权学习框架训练，使用可验证奖励，在代码生成测试时扩展中实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 当前代码生成测试时扩展主要依赖Best-of-N选择，但训练可靠的LLM法官面临严重分布偏移挑战：简单与困难问题不平衡、训练任务与评估基准不匹配、训练数据与推理模型行为差异等

Method: 提出DAJ（推理型LLM法官），采用双层数据重加权学习框架，学习数据重要性权重（领域级或实例级），使用可验证奖励优化在目标基准对齐的元集上的泛化性能

Result: 在LiveCodeBench和BigCodeBench上实现最先进性能，优于强测试时扩展基线以及领先的专有模型，自动强调困难问题、分布内样本和轨迹对齐数据

Conclusion: DAJ是首个将数据重加权应用于LLM法官训练的方法，无需依赖手工启发式规则，能有效应对分布偏移问题，在代码生成测试时扩展中表现出色
...
### [138] [FunPRM: Function-as-Step Process Reward Model with Meta Reward Correction for Code Generation](https://arxiv.org/abs/2601.22249)
*Ruiyi Zhang,Peijia Qin,Qi Cao,Eric Xue,Pengtao Xie*

Main category: cs.LG

TL;DR: FunPRM是一种针对代码生成的测试时扩展方法，通过函数化模块分解和元学习奖励校正机制，显著提升大语言模型在复杂编程任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在复杂代码生成任务上仍然经常失败，而现有的过程奖励模型方法由于代码缺乏有意义的步骤分解和蒙特卡洛估计的部分解决方案正确性评分存在噪声，在代码生成中效果不佳。

Method: FunPRM采用两种创新方法：1) 提示大语言模型生成模块化、函数化的代码，将函数作为过程奖励模型的推理步骤；2) 引入基于元学习的奖励校正机制，利用单元测试评估系统获得的干净最终解决方案奖励来净化噪声的部分解决方案奖励。

Result: 在LiveCodeBench和BigCodeBench上的实验表明，FunPRM在五个基础大语言模型上始终优于现有的测试时扩展方法，特别是与O4-mini结合时在LiveCodeBench上达到了最先进的性能。此外，FunPRM生成的代码对开发者来说更具可读性和可重用性。

Conclusion: FunPRM通过函数化模块分解和元学习奖励校正，有效解决了代码生成中过程奖励模型面临的两个关键挑战，显著提升了代码生成质量，为复杂编程任务的自动化提供了更可靠的解决方案。
...
### [139] [Symmetry Breaking in Transformers for Efficient and Interpretable Training](https://arxiv.org/abs/2601.22257)
*Eva Silverstein,Daniel Kunin,Vasudev Shyam*

Main category: cs.LG

TL;DR: 论文提出了一种简单的对称性破坏协议，通过批量采样的、未学习的查询和值偏置，为标准注意力机制中的冗余旋转自由度引入首选方向，从而提升简单优化器的性能并增强注意力头的可解释性。
...
Method: 引入对称性破坏协议，通过批量采样的、未学习的查询和值偏置为注意力机制中的旋转空间引入首选方向。这种方法不增加可学习参数，但能有效利用原本冗余的自由度。

Result: 1) 显著提升了简单、内存高效优化器（如SGDM、ECD）的性能，缩小甚至消除了与复杂自适应方法（如AdamW）的差距；2) 使原本冗余的旋转自由度变得可解释，能够选择性地放大单个注意力头中语义上有意义的token类别。
...
### [140] [Tabular Foundation Models Can Do Survival Analysis](https://arxiv.org/abs/2601.22259)
*Da In Kim,Wei Siang Lai,Kelly W. Zhang*

Main category: cs.LG

TL;DR: 将生存分析重新定义为二元分类问题，通过离散化事件时间，使现有的表格基础模型能够通过上下文学习进行生存分析，无需显式训练。


<details>
  <summary>Details</summary>
Motivation: 表格基础模型在分类和回归任务上取得了显著成功，但由于存在右删失（数据观测可能在事件发生前结束），将其适应于建模生存分析的时间到事件结果具有挑战性。

Method: 开发了一个基于分类的框架，通过离散化事件时间，将静态和动态生存分析重新表述为一系列二元分类问题。删失观测被自然地处理为在某些时间点缺少标签的示例。这种分类表述使现有的表格基础模型能够通过上下文学习进行生存分析，无需显式训练。

Result: 在标准删失假设下，最小化二元分类损失能够随着训练集大小的增加恢复真实的生存概率。在53个真实世界数据集上的评估表明，使用这种分类表述的现成表格基础模型在多个生存指标上平均优于经典和深度学习基线。

Conclusion: 通过将生存分析重新表述为分类问题，可以利用现有的强大表格基础模型进行生存分析，无需专门训练，在多个数据集上表现出优越性能。
...
### [141] [Privacy-Preserving Sensor-Based Human Activity Recognition for Low-Resource Healthcare Using Classical Machine Learning](https://arxiv.org/abs/2601.22265)
...
<details>
  <summary>Details</summary>
Motivation: 老年人和弱势群体因医疗基础设施有限而依赖家庭护理，常导致忽视治疗性锻炼（如瑜伽或物理治疗）和依从性差。需要低成本自动化解决方案来监测活动并改善健康结果。
...
### [142] [Task-Uniform Convergence and Backward Transfer in Federated Domain-Incremental Learning with Partial Participation](https://arxiv.org/abs/2601.22274)
*Longtao Xu,Jian Li*

Main category: cs.LG

TL;DR: SPECIAL算法在联邦域增量学习(FDIL)中通过添加服务器端"锚点"实现无记忆、无回放缓冲区的持续学习，保证向后知识传递并实现跨任务收敛。


<details>
  <summary>Details</summary>
Motivation: 现实联邦系统中数据分布会漂移，但隐私规则禁止原始数据共享。现有FDIL缺乏两个理论支柱：向后知识传递保证和跨所有任务的部分参与收敛率保证。

Method: SPECIAL算法在FedAvg基础上添加服务器端"锚点"：每轮中服务器通过轻量级近端项将参与客户端更新向先前全局模型推动，无需回放缓冲区、合成数据或任务特定头。

Result: 理论证明SPECIAL：(1)保护先前任务：BKT界限将先前任务损失增加限制在漂移控制项内，随轮数、本地周期和参与客户端增加而减小；(2)跨任务高效学习：首次实现FDIL部分参与下的通信高效非凸收敛率O((E/NT)^(1/2))。

Conclusion: SPECIAL通过简单服务器端锚点机制解决了FDIL中的核心理论挑战，实现了向后知识传递保证和跨任务收敛，同时保持通信和模型大小不变，实验验证了其有效性。
...
### [143] [SurrogateSHAP: Training-Free Contributor Attribution for Text-to-Image (T2I) Models](https://arxiv.org/abs/2601.22276)
...
<details>
  <summary>Details</summary>
Motivation: 随着文本到图像扩散模型在现实世界创意工作流程中的广泛应用，需要建立一个公平的数据贡献者价值评估框架。传统的Shapley值方法面临双重计算瓶颈：1）对每个数据子集重新训练模型的成本过高；2）由于贡献者交互，需要评估的组合子集数量巨大。

Method: 提出SurrogateSHAP框架：1）通过预训练模型的推理来近似昂贵的重新训练游戏；2）使用梯度提升树来近似效用函数，并从树模型中解析推导Shapley值。

Result: 在三个不同的归因任务中评估：1）DDPM-CFG在CIFAR-20上的图像质量；2）Stable Diffusion在后印象派艺术作品上的美学评估；3）FLUX.1在时尚产品数据上的产品多样性。SurrogateSHAP在所有设置中都优于先前方法，同时显著降低计算开销，能够一致识别多个效用指标中的有影响力贡献者。

Conclusion: SurrogateSHAP为数据贡献者价值评估提供了高效、可扩展的解决方案，能够有效定位临床图像中虚假相关的数据源，为审计安全关键生成模型提供了可行路径。
...
### [144] [Riemannian Lyapunov Optimizer: A Unified Framework for Optimization](https://arxiv.org/abs/2601.22284)
*Yixuan Wang,Omkar Sudhir Patil,Warren E. Dixon*

Main category: cs.LG

TL;DR: Riemannian Lyapunov Optimizers (RLOs) 是一个基于控制理论的优化算法框架，将经典优化器统一在几何框架中，通过识别不变流形将训练分为两个阶段，并提供了系统化的优化器设计方法。


<details>
  <summary>Details</summary>
Motivation: 现有优化器改进多基于启发式方法，缺乏系统理论框架。本文旨在建立控制理论与机器学习优化之间的桥梁，提供统一的几何框架来系统化设计稳定有效的优化器。

Method: 将优化问题重新解释为黎曼流形上的离散时间控制系统，识别出"正常吸引不变流形"(NAIM)，构建严格的Lyapunov函数来证明收敛性，并开发了"优化器生成器"来系统设计RLOs。

Result: RLOs不仅能够恢复经典优化算法，还能通过控制理论原则设计新的优化器，在大规模基准测试中达到了最先进的性能，并通过几何诊断验证了理论框架。

Conclusion: RLOs成功地将控制理论与现代机器学习优化相结合，提供了统一的语言和系统化的工具包，用于设计稳定有效的优化器，为优化器设计开辟了新的理论途径。
...
### [145] [Demystifying Mergeability: Interpretable Properties to Predict Model Merging Success](https://arxiv.org/abs/2601.22285)
...
<details>
  <summary>Details</summary>
Motivation: 当前对模型合并的研究通常将可合并性视为模型的内在属性，但作者认为这忽略了合并方法和任务特性的影响，需要更系统化的理解框架

Method: 提出架构无关的分析框架，使用线性优化和可解释的成对度量（如梯度L2距离），在四种合并方法上分析后合并性能的相关属性

Result: 发现成功驱动因素存在显著差异（46.7%度量重叠；55.3%符号一致性），揭示了方法特定的"指纹"，但子空间重叠和梯度对齐始终作为方法无关的兼容性基础条件出现

Conclusion: 为理解模型可合并性提供了诊断基础，并激励未来开发能明确促进子空间重叠和梯度对齐特性的微调策略
...
### [146] [Conformal Prediction for Generative Models via Adaptive Cluster-Based Density Estimation](https://arxiv.org/abs/2601.22298)
*Qidong Yang,Qianyu Julie Zhu,Jonathan Giezendanner,Youssef Marzouk,Stephen Bates,Sherrie Wang*

Main category: cs.LG

TL;DR: CP4Gen：一种针对条件生成模型的系统性保形预测方法，通过聚类密度估计构建预测集，提高不确定性校准和可解释性
...
### [147] [ZK-HybridFL: Zero-Knowledge Proof-Enhanced Hybrid Ledger for Federated Learning](https://arxiv.org/abs/2601.22302)
*Amirhossein Taherpour,Xiaodong Wang*

Main category: cs.LG

TL;DR: ZK-HybridFL是一个安全的去中心化联邦学习框架，结合DAG账本、侧链和零知识证明，实现隐私保护的模型验证和高效对抗检测。
...
Method: 提出ZK-HybridFL框架，集成有向无环图（DAG）账本、专用侧链和零知识证明（ZKPs），使用事件驱动智能合约和预言机辅助的侧链来验证本地模型更新而不暴露敏感数据，并内置挑战机制检测对抗行为。
...
### [148] [BayesFlow: A Probability Inference Framework for Meta-Agent Assisted Workflow Generation](https://arxiv.org/abs/2601.22305)
*Bo Yuan,Yun Zhou,Zhichao Xu,Kiran Ramnath,Aosong Feng,Balasubramaniam Srinivasan*

Main category: cs.LG

TL;DR: BayesFlow：一种基于贝叶斯推理的自动工作流生成框架，通过重要性采样和序列优化提升复杂任务的工作流构建效果


<details>
  <summary>Details</summary>
Motivation: 现有自动工作流生成方法大多基于优化问题，缺乏理论基础。作者希望建立一个有理论保证的贝叶斯推理框架，为工作流生成提供更严谨的方法论

Method: 提出贝叶斯工作流生成（BWG）框架，将工作流生成建模为后验分布推断。使用并行前瞻展开进行重要性加权，结合序列内循环优化器进行整体改进。具体实现为BayesFlow算法，无需训练

Result: 在六个基准数据集上，BayesFlow相比最先进的工作流生成基线准确率提升高达9个百分点，相比零样本提示提升高达65个百分点

Conclusion: BWG为基于搜索的工作流设计提供了有理论保证的升级方案，证明了贝叶斯推理框架在自动工作流生成中的有效性
...
### [149] [Stealthy Poisoning Attacks Bypass Defenses in Regression Settings](https://arxiv.org/abs/2601.22308)
*Javier Carnerero-Cano,Luis Muñoz-González,Phillippa Spencer,Emil C. Lupu*

Main category: cs.LG

TL;DR: 本文提出了一种新的最优隐蔽攻击方法，能够绕过现有防御机制，并开发了新的防御方法BayesClean来对抗隐蔽攻击。
...
Method: 提出了一种新颖的最优隐蔽攻击公式，考虑不同程度的可检测性；开发了基于目标归一化的新方法来评估攻击有效性和可检测性之间的权衡；设计了新的防御方法BayesClean来对抗隐蔽攻击。

Result: 提出的最优隐蔽攻击能够绕过最先进的防御机制；BayesClean防御在攻击具有隐蔽性且投毒点数量显著时，相比先前防御方法表现更优。

Conclusion: 本文填补了回归模型对隐蔽投毒攻击鲁棒性研究的空白，提出的攻击和防御方法为实际应用提供了更实用的解决方案。
...
TL;DR: PoSafeNet：一种可微分神经安全层，通过部分有序集建模安全约束，实现自适应安全执行选择，提升机器人控制的安全性和可行性


<details>
  <summary>Details</summary>
Motivation: 现有安全学习方法通常统一执行多个安全约束或采用固定优先级顺序，导致不可行性和脆弱行为。实际应用中安全需求具有异质性，仅存在部分优先级关系，一些约束可比而另一些不可比。

Method: 将安全约束形式化为部分有序集，将安全组合视为策略类的结构属性。提出PoSafeNet可微分神经安全层，通过部分有序集一致约束顺序下的顺序闭式投影强制执行安全，支持自适应选择或混合有效安全执行，同时通过构造保留优先级语义。

Result: 在多障碍物导航、约束机器人操作和视觉自动驾驶实验中，相比非结构化和基于可微分二次规划的安全层，显示出更好的可行性、鲁棒性和可扩展性。

Conclusion: PoSafeNet通过部分有序集结构化安全约束，提供了一种更灵活、鲁棒的安全学习方法，能够处理异质安全需求，在保持优先级语义的同时提高系统可行性。
...
### [151] [SCALAR: Quantifying Structural Hallucination, Consistency, and Reasoning Gaps in Materials Foundation Models](https://arxiv.org/abs/2601.22312)
*Can Polat,Erchin Serpedin,Mustafa Kurban,Hasan Kurban*

Main category: cs.LG

TL;DR: SCALAR是一个评估材料基础模型在几何尺度泛化能力的基准测试，包含三个任务：晶体结构到性质预测、基于物理推理的思维链变体、以及给定目标性质的反向检索，通过结构化指标评估模型在尺度变化下的表现。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在材料科学推理中的应用日益增多，但它们在物理结构化分布偏移下的行为仍不清楚。需要评估模型在不同几何尺度下的泛化能力及其与结构幻觉、一致性和推理的关系。

Method: 引入SCALAR基准测试，使用从DFT验证的晶胞通过超晶胞扩展和几何截断得到的纳米颗粒结构，涵盖从几个原子到超过18,000个原子的长度尺度。定义三个任务：CIF到性质预测、基于物理推理的思维链变体、以及反向检索。通过结构化指标评估数值误差、幻觉、跨提示一致性、单调推理、输出有效性和检索遗憾。

Result: 实验显示不同基础模型在显式推理下存在显著的模型依赖性偏移，通常能减少幻觉和误差，但经常破坏一致性或有效性。这表明几何尺度泛化不能仅从准确性推断。

Conclusion: SCALAR基准测试揭示了材料基础模型在几何尺度泛化方面的局限性，强调了需要超越准确性评估的全面评估框架，以理解模型在物理结构化分布偏移下的行为。
...
### [152] [Offline Reinforcement Learning of High-Quality Behaviors Under Robust Style Alignment](https://arxiv.org/abs/2601.22823)
*Mathieu Petitbois,Rémy Portelas,Sylvain Lamprier*

Main category: cs.LG

TL;DR: 本文提出Style-Conditioned Implicit Q-Learning (SCIQL)，一种用于离线强化学习的风格条件策略学习方法，通过子轨迹标注函数提供显式风格监督，有效平衡任务性能与风格对齐。
...
Method: 提出统一的行为风格定义，并基于此构建SCIQL框架，结合离线目标条件RL技术（如后见重标注和价值学习），引入新的门控优势加权回归机制来优化任务性能同时保持风格对齐。

Result: 实验表明，SCIQL在任务性能和风格对齐两个目标上都优于现有的离线方法。
...
### [153] [Hair-Trigger Alignment: Black-Box Evaluation Cannot Guarantee Post-Update Alignment](https://arxiv.org/abs/2601.22313)
*Yavuz Bakman,Duygu Nur Yaldiz,Salman Avestimehr,Sai Praneeth Karimireddy*

Main category: cs.LG

TL;DR: 研究发现静态黑盒评估无法保证LLM在更新后的对齐性，即使通过所有静态测试的模型也可能在单次良性更新后严重失准，且模型规模越大隐藏对抗行为的能力越强。


<details>
  <summary>Details</summary>
Motivation: 现有对齐研究通常假设初始模型是对齐的，基于静态黑盒评估（即对固定查询集没有不良响应）。但实际中LLM经常更新，而静态评估无法检测更新后可能出现的失准行为。

Method: 从理论上证明由于过参数化，静态对齐不能保证更新后的对齐性；证明静态黑盒探测无法区分真正更新后鲁棒的模型和隐藏任意数量对抗行为的模型；在隐私、越狱安全和行为诚实三个核心对齐领域对LLM进行实证验证。

Result: 存在通过所有标准黑盒对齐测试的LLM，在单次良性更新后变得严重失准；隐藏这种潜在对抗行为的能力随模型规模增加而增强，证实了理论预测：更新后失准程度随参数数量增长。

Conclusion: 静态评估协议存在不足，迫切需要建立更新后鲁棒的对齐评估方法，特别是随着模型规模增大，这一问题变得更加严重。
...
### [154] [Gaussian Process Bandit Optimization with Machine Learning Predictions and Application to Hypothesis Generation](https://arxiv.org/abs/2601.22315)
*Xin Jennifer Chen,Yunjin Tong*

Main category: cs.LG

TL;DR: 提出PA-GP-UCB算法，结合昂贵真值评估和廉价预测模型，利用离线数据提升贝叶斯优化的样本效率


<details>
  <summary>Details</summary>
Motivation: 现实优化问题常涉及昂贵真值评估（如人工评估、物理实验）和廉价低精度预测（如机器学习模型、仿真）。同时存在大量离线数据可用于预训练预测模型并提供信息先验，需要有效结合两者提升样本效率

Method: 提出预测增强高斯过程上置信界算法（PA-GP-UCB），使用联合高斯过程后验推导的控制变量估计器来校正预测偏差并减少不确定性，在保持GP-UCB标准遗憾率的同时获得更小的主导常数

Result: 理论证明PA-GP-UCB保持GP-UCB标准遗憾率，主导常数严格更小且由预测质量和离线数据覆盖度显式控制。实证显示在合成基准和基于人类行为数据的真实假设评估任务中，收敛速度快于Vanilla GP-UCB和朴素预测增强基线

Conclusion: PA-GP-UCB为昂贵反馈下的假设生成提供了一个通用且样本高效的框架，成功结合了真值评估、预测模型和离线数据的优势
...
### [155] [Federate the Router: Learning Language Model Routers with Sparse and Decentralized Evaluations](https://arxiv.org/abs/2601.22318)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型作为远程服务被边缘和企业客户端访问，不同模型在能力和价格上差异很大，需要路由查询以平衡质量和推理成本。现有路由方法假设可以访问集中化的查询-模型评估数据，但这些数据通常分散在各个客户端（如终端用户和组织），且具有隐私敏感性，无法集中化。此外，每个客户端单独训练路由策略效果有限，因为本地评估数据有限，仅覆盖受限的查询分布和有偏的模型评估子集。

Method: 提出了首个用于LLM路由的联邦学习框架，支持参数化的多层感知机路由器和非参数的K-means路由器，能够在异构客户端查询分布和非均匀模型覆盖下工作。该框架使客户端能够从本地离线查询-模型评估数据中学习共享路由策略。

Result: 在两个基准测试中，联邦协作通过增加有效模型覆盖和更好的查询泛化，在准确性-成本边界上优于客户端本地路由器。理论结果也验证了联邦训练能够减少路由次优性。

Conclusion: 联邦学习框架为LLM路由提供了一种有效的解决方案，能够在保护数据隐私的同时，通过客户端协作学习更好的路由策略，平衡模型质量和推理成本。
...
### [156] [Matrix Factorization for Practical Continual Mean Estimation Under User-Level Differential Privacy](https://arxiv.org/abs/2601.22320)
*Nikita P. Kalinin,Ali Najar,Valentin Roth,Christoph H. Lampert*

Main category: cs.LG

TL;DR: 本文研究连续均值估计问题，在用户级差分隐私下，采用近似差分隐私而非纯差分隐私，提出新的矩阵分解机制，显著降低均方误差。


<details>
  <summary>Details</summary>
Motivation: 现有连续均值估计研究主要关注纯差分隐私，导致估计结果噪声过大，限制了实际应用。本文旨在解决这一问题，在保护用户级隐私的同时获得更准确的估计结果。

Method: 采用近似差分隐私框架，基于矩阵分解机制的最新进展，提出专门针对均值估计的定制化分解方法，该方法既高效又准确。

Result: 提出的方法在用户级差分隐私下的连续均值估计中，实现了渐近更低的均方误差界限，相比纯差分隐私方法显著提升了估计精度。

Conclusion: 通过采用近似差分隐私和专门的矩阵分解机制，本文为连续均值估计问题提供了更实用、更准确的解决方案，突破了纯差分隐私的限制。
...
### [157] [Models Under SCOPE: Scalable and Controllable Routing via Pre-hoc Reasoning](https://arxiv.org/abs/2601.22323)
...
Method: 提出SCOPE框架，使用强化学习训练，通过检索类似问题上的模型行为来预测成本和性能，而不是依赖固定模型名称，支持未见过的模型。

Result: 实验显示SCOPE灵活适应用户需求：当性能优先时可提升准确率25.7%，当效率优先时可降低成本95.1%。

Conclusion: SCOPE超越了传统的成本节约工具，通过预测模型成本和性能将路由转变为动态决策问题，允许用户轻松控制准确率和成本之间的权衡。
...
### [158] [AgentScore: Autoformulation of Deployable Clinical Scoring Systems](https://arxiv.org/abs/2601.22324)
*Silas Ruhrberg Estévez,Christopher Chiu,Mihaela van der Schaar*

Main category: cs.LG

TL;DR: AgentScore使用LLM生成候选规则，通过确定性验证选择循环创建可部署的临床评分系统，在保持可解释性的同时达到与更灵活模型相当的预测性能。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习模型虽然预测性能强，但难以转化为常规临床使用，因为不符合工作流程约束（如可记忆性、可审计性、床边执行）。问题不在于预测能力不足，而在于模型类别与指南部署不兼容。

Method: AgentScore使用LLM在指数级大的离散规则空间中提出候选规则，通过确定性的数据驱动验证和选择循环来确保统计有效性和可部署性约束，生成单位加权的临床检查表形式的评分系统。

Result: 在八个临床预测任务中，AgentScore优于现有的评分生成方法，并在更强的结构约束下达到与更灵活可解释模型相当的AUC。在两个外部验证任务中，其区分度高于已建立的基于指南的评分。

Conclusion: AgentScore通过结合LLM的语义引导优化和确定性验证，能够生成既符合临床部署约束又保持良好预测性能的评分系统，弥合了机器学习模型与临床实践之间的差距。
...
### [159] [Knowledge-Informed Kernel State Reconstruction for Interpretable Dynamical System Discovery](https://arxiv.org/abs/2601.22328)
*Luca Muscarnera,Silas Ruhrberg Estévez,Samuel Holt,Evgeny Saveliev,Mihaela van der Schaar*

Main category: cs.LG

TL;DR: MAAT框架通过知识引导的核状态重建，从噪声和不完整观测数据中恢复物理系统的控制方程，结合结构先验知识提升符号发现的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在噪声、部分观测条件下表现不佳，或依赖黑盒潜在动力学模型而缺乏可解释性，需要一种能处理不完整数据并保持物理一致性的符号发现框架。

Method: MAAT在再生核希尔伯特空间中构建状态重建，直接整合非负性、守恒定律等结构先验和领域特定观测模型，支持异构采样和测量粒度，生成平滑的物理一致状态估计和解析时间导数。

Result: 在12个不同科学基准测试和多种噪声机制下，MAAT相比基线方法显著降低了用于下游符号回归的状态估计和导数的均方误差。

Conclusion: MAAT为碎片化传感器数据与符号回归之间提供了原则性接口，通过知识引导的状态重建提升了从数据中发现控制方程的准确性和物理一致性。
...
### [160] [Scalable Batch Correction for Cell Painting via Batch-Dependent Kernels and Adaptive Sampling](https://arxiv.org/abs/2601.22331)
...
<details>
  <summary>Details</summary>
Motivation: Cell Painting是一种基于显微镜的高通量成像技术，能产生丰富的细胞形态特征，支持药物发现。然而，在大规模应用中，实验室、仪器和协议差异导致的批次效应会掩盖生物信号，需要有效的批次校正方法。

Method: BALANS通过两个核心思想构建稀疏亲和矩阵：1) 使用批次感知的局部尺度计算高斯核亲和度；2) 采用自适应采样策略，优先选择邻居覆盖度低的行，仅保留每行最强的亲和度，实现稀疏但信息丰富的近似。

Result: BALANS在样本复杂度方面达到最优阶，提供近似保证，运行时间接近线性。在真实Cell Painting数据集和合成基准测试中，BALANS能够扩展到大规模数据集，相比广泛使用的批次校正方法显著提升运行效率，同时保持校正质量。
...
### [161] [DP-$λ$CGD: Efficient Noise Correlation for Differentially Private Model Training](https://arxiv.org/abs/2601.22334)
*Nikita P. Kalinin,Ryan McKenna,Rasmus Pagh,Christoph H. Lampert*

Main category: cs.LG

TL;DR: 提出一种新的差分隐私随机梯度下降噪声相关策略，仅将噪声与前一迭代相关并部分抵消，使用伪随机噪声生成器避免存储历史噪声，实现无额外内存开销的改进


<details>
  <summary>Details</summary>
Motivation: 现有的矩阵分解机制等DP-SGD扩展方法虽然通过跨多个迭代引入相关噪声提高了准确性，但需要存储先前添加的噪声向量，导致显著的内存开销。需要一种既能提高准确性又无需额外内存开销的解决方案。

Method: 提出新的噪声相关策略：1）仅将噪声与前一迭代相关；2）控制性地抵消部分噪声；3）使用伪随机噪声生成器进行噪声再生，无需存储历史噪声；4）保持与标准DP-SGD相同的内存需求

Result: 方法在计算开销最小的情况下，相比标准DP-SGD获得了准确性的提升，且无需额外内存开销，保持了DP-SGD的差分隐私保证

Conclusion: 该工作提出了一种内存高效的DP-SGD改进方法，通过创新的噪声相关策略在保持差分隐私保证的同时提高了模型准确性，解决了现有方法的内存开销问题
...
### [162] [Knowledge Gradient for Preference Learning](https://arxiv.org/abs/2601.22335)
*Kaiwen Wu,Jacob R. Gardner*

Main category: cs.LG

TL;DR: 本文提出了一个用于偏好贝叶斯优化的精确解析知识梯度方法，解决了传统方法在计算非高斯后验时的困难。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯优化中的知识梯度是一种流行的采集函数，但在偏好贝叶斯优化中，由于只能进行成对比较查询且需要计算非高斯后验，传统知识梯度方法面临计算挑战。

Method: 推导出用于偏好贝叶斯优化的精确解析知识梯度，解决了前瞻步骤中非高斯后验计算的问题。

Result: 在基准测试套件上，精确知识梯度表现出色，通常优于现有的采集函数。同时通过案例研究揭示了知识梯度在某些场景下的局限性。

Conclusion: 成功开发了用于偏好贝叶斯优化的精确知识梯度方法，为处理成对比较查询的优化问题提供了有效的解决方案。
...
### [163] [Failing to Explore: Language Models on Interactive Tasks](https://arxiv.org/abs/2601.22345)
*Mahdi JafariRaviz,Keivan Rezaei,Arshia Soltani Moakhar,Zahra Sodagar,Yize Cheng,Soheil Feizi*

Main category: cs.LG

TL;DR: 论文评估语言模型在有限交互预算下探索交互环境的能力，发现现有模型存在系统性探索不足和次优解问题，性能常低于简单探索-利用启发式基线，且预算增加时性能提升有限。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型在有限交互预算下探索交互环境的能力，评估当前最先进模型在探索任务中的表现，识别其局限性并提出改进方法。

Method: 引入三个参数化任务（连续和离散环境），设计可控探索难度，评估多种最先进语言模型，与简单探索-利用启发式基线对比，研究两种轻量级干预措施：将固定预算分割为并行执行，以及定期总结交互历史。

Result: 发现现有模型存在系统性探索不足和次优解问题，性能常显著低于简单基线，且预算增加时性能提升有限。两种干预措施均能改善性能：预算分割并行执行（尽管理论上无增益）和定期总结交互历史都能提升探索效果。

Conclusion: 当前语言模型在有限预算下的环境探索能力存在明显不足，需要专门改进。轻量级干预措施如预算分割和交互历史总结能有效提升探索性能，为改进语言模型的探索能力提供了实用方向。
...
### [164] [MixQuant: Pushing the Limits of Block Rotations in Post-Training Quantization](https://arxiv.org/abs/2601.22347)
*Sai Sanjeet,Ian Colbert,Pablo Monteagudo-Lago,Giuseppe Franco,Yaman Umuroglu,Nicholas J. Fraser*

Main category: cs.LG

TL;DR: MixQuant：一种基于块旋转感知的后训练量化框架，通过重新分布激活质量来抑制异常值，提高量化精度


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法使用块旋转来扩散异常值，但块结构对异常值抑制的影响缺乏系统理解。需要填补这一空白，理解块Hadamard旋转中异常值抑制的基本限制

Method: 提出MixQuant框架：1）通过系统分析发现异常值抑制受输入向量几何结构限制，当ℓ₁范数质量均匀分布在块中时效果最佳；2）引入贪心质量扩散算法，通过排列重新分布激活质量；3）识别Transformer架构中的排列等变区域，将排列合并到模型权重中以避免推理开销

Result: MixQuant在所有块大小上一致提高精度，在将Llama3 1B量化为INT4（块大小16）时，恢复了全向量旋转困惑度的90%，相比无排列方法的46%有显著提升

Conclusion: 块旋转中的异常值抑制受输入向量几何结构限制，通过排列重新分布激活质量可以显著改善量化性能，MixQuant框架在不增加推理开销的情况下有效提高了后训练量化的精度
...
### [165] [Learning Policy Representations for Steerable Behavior Synthesis](https://arxiv.org/abs/2601.22350)
*Beiming Li,Sergio Rozada,Alejandro Ribeiro*

Main category: cs.LG

TL;DR: 提出一种基于集合架构的MDP策略表示学习方法，通过变分生成和对比学习构建平滑潜在空间，支持梯度优化实现行为引导


<details>
  <summary>Details</summary>
Motivation: 为MDP中的策略学习统一的表示方法，便于在测试时进行行为引导。由于策略由其占用测度唯一确定，因此将策略表示建模为状态-动作特征映射相对于占用测度的期望

Method: 1) 使用基于集合的架构近似策略表示；2) 将状态-动作样本集编码为潜在嵌入；3) 从嵌入中解码策略及其对应多个奖励的价值函数；4) 采用变分生成方法构建平滑潜在空间；5) 使用对比学习使潜在距离与价值函数差异对齐

Result: 构建了支持梯度优化的潜在空间，能够解决新颖的行为合成任务：在无需额外训练的情况下，引导策略满足先前未见过的价值函数约束

Conclusion: 提出的方法能够学习策略的统一表示，通过潜在空间的几何特性支持基于梯度的优化，实现了灵活的行为引导和约束满足
...
### [166] [Relative Wasserstein Angle and the Problem of the $W_2$-Nearest Gaussian Distribution](https://arxiv.org/abs/2601.22355)
*Binshuai Wang,Peng Wei*

Main category: cs.LG

TL;DR: 该研究利用最优传输理论，通过相对平移不变二次Wasserstein空间的锥几何结构，提出了相对Wasserstein角和正交投影距离两个新几何量来量化经验分布与高斯分布之间的偏差。


<details>
  <summary>Details</summary>
Motivation: 传统的高斯近似方法（如矩匹配）在Wasserstein距离度量下可能不是最优的，需要更精确的几何框架来量化分布与高斯性之间的偏差。

Method: 利用相对平移不变二次Wasserstein空间的锥几何结构，引入相对Wasserstein角和正交投影距离；在一维情况下推导闭式解，在高维情况下开发基于半离散对偶公式的随机流形优化算法。

Result: 证明了该空间中任意两条射线生成的填充锥是平坦的，确保角度、投影和内积严格定义；发现矩匹配高斯不是W_2最近的高斯分布；实验表明相对Wasserstein角比Wasserstein距离更鲁棒，提出的最近高斯在FID评分评估中优于矩匹配。

Conclusion: 该几何视角将高斯近似重构为高斯锥上的投影问题，提供了量化非高斯性的新框架，为分布近似和生成模型评估提供了更精确的工具。
...
### [167] [Understanding Efficiency: Quantization, Batching, and Serving Strategies in LLM Energy Use](https://arxiv.org/abs/2601.22362)
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在生产环境中的大规模部署，计算资源和能源需求从训练转向推理阶段。现有研究主要关注每个提示或每个token的能耗，但缺乏对系统级设计选择如何影响LLM推理能耗的系统性分析。

Method: 在NVIDIA H100 GPU上进行详细的LLM推理能耗和延迟实证研究，分析量化（数值精度）、批处理大小和服务配置（如使用Hugging Face的Text Generation Inference服务器）的影响。

Result: 1) 低精度格式仅在计算密集型阶段能带来能耗收益；2) 批处理能显著提高能源效率，特别是在解码等内存密集型阶段；3) 结构化请求时序（到达整形）可将每个请求的能耗降低高达100倍。

Conclusion: 可持续的LLM部署不仅取决于模型内部设计，更依赖于服务栈的编排。研究结果支持基于阶段感知的能耗分析和系统级优化，以实现更环保的AI服务。
...
### [168] [FIRE: Multi-fidelity Regression with Distribution-conditioned In-context Learning using Tabular Foundation Models](https://arxiv.org/abs/2601.22371)
...
Method: FIRE框架耦合表格基础模型，通过高保真度校正模型对低保真度模型的后验预测分布进行条件化，实现跨保真度信息传递和异方差误差捕获，无需模型重新训练。

Result: 在31个合成和真实世界基准问题（如DrivAerNet、LCBench）上，FIRE在性能-时间权衡上优于7种最先进的GP或深度学习多保真度回归方法，在准确性和不确定性量化方面排名最高，具有运行时优势。

Conclusion: FIRE提供了一种有效的免训练多保真度回归方法，但存在上下文窗口限制和对预训练表格基础模型质量的依赖性等局限性。
...
### [169] [Graph is a Substrate Across Data Modalities](https://arxiv.org/abs/2601.22384)
...
<details>
  <summary>Details</summary>
Motivation: 当前图结构学习通常在模态和任务隔离的环境中进行，导致跨模态和任务的结构规律需要重复构建，而不是在中间图表示层面进行积累。这促使研究如何组织图结构，使其能够在异构模态和任务间持久存在并积累

Method: 提出G-Substrate框架，包含两个互补机制：1）统一结构模式，确保跨异构模态和任务的图表示兼容性；2）交错角色训练策略，在训练过程中将相同的图结构暴露给多个功能角色

Result: 在多个领域、模态和任务上的实验表明，G-Substrate优于任务隔离和朴素多任务学习方法

Conclusion: 通过将图结构视为跨学习上下文持久存在的结构基板，G-Substrate能够有效积累和共享图表示知识，提高学习效率和性能
...
### [170] [SAIR: Cost-Efficient Multi-Stage ML Pipeline Autoscaling via In-Context Reinforcement Learning](https://arxiv.org/abs/2601.22397)
*Jianchang Su,Yifan Zhang,Shengkai Lin,Shizhen Zhao,Yusheng Zheng,Yiwei Yang,Wei Zhang*

Main category: cs.LG

TL;DR: SAIR：基于LLM的在线强化学习控制器，用于多阶段ML推理管道的自动扩缩容，无需梯度更新，通过奖励塑造和GPU速率控制优化延迟和成本


<details>
  <summary>Details</summary>
Motivation: 多阶段ML推理管道难以自动扩缩容，因为存在异构资源、跨阶段耦合和动态瓶颈迁移等问题。现有方法难以有效处理这些挑战。

Method: SAIR使用LLM作为上下文强化学习控制器，结合帕累托优势奖励塑造、基于惊奇度的经验检索和用户空间CUDA拦截的细粒度GPU速率控制，在线改进策略而无需梯度更新。

Result: 在四种ML服务管道和三种工作负载模式下，SAIR实现了最佳或并列最佳的P99延迟和有效资源成本，P99延迟提升高达50%，有效成本降低高达97%，瓶颈检测准确率达86%，无需离线训练。

Conclusion: SAIR框架通过LLM作为在线强化学习控制器，有效解决了多阶段ML推理管道的自动扩缩容问题，显著改善了延迟性能和资源效率。
...
### [171] [Score-based Integrated Gradient for Root Cause Explanations of Outliers](https://arxiv.org/abs/2601.22399)
*Phuoc Nguyen,Truyen Tran,Sunil Gupta,Svetha Venkatesh*

Main category: cs.LG

TL;DR: SIREN是一种新颖的异常根因分析方法，通过估计数据似然性的得分函数，使用积分梯度从异常点向正常数据分布累积得分贡献，满足三个Shapley值公理和因果结构的不对称公理。


<details>
  <summary>Details</summary>
Motivation: 传统基于启发式或反事实推理的异常根因识别方法在不确定性和高维依赖下表现不佳，需要一种更有效的方法来处理非线性、高维和异方差因果模型中的根因归因问题。

Method: SIREN方法直接操作于数据似然性的得分函数，通过积分梯度计算归因，这些梯度沿着从异常点到正常数据分布的路径累积得分贡献，能够处理非线性、高维和异方差的因果模型。

Result: 在合成随机图和真实世界云服务及供应链数据集上的广泛实验表明，SIREN在归因准确性和计算效率方面均优于现有最先进的基线方法。

Conclusion: SIREN提供了一种可扩展且具有理论保证的异常根因分析方法，能够有效处理复杂因果模型中的不确定性，为异常检测和因果推断领域提供了新的工具。
...
### [172] [MM-OpenFGL: A Comprehensive Benchmark for Multimodal Federated Graph Learning](https://arxiv.org/abs/2601.22416)
*Xunkai Li,Yuming Ai,Yinlin Zhu,Haodong Lu,Yi Zhang,Guohao Fu,Bowen Fan,Qiangqiang Dai,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: MM-OpenFGL是首个多模态联邦图学习基准，包含19个数据集、8种模拟策略、6个下游任务和57种SOTA方法，为MMFGL研究提供系统评估框架。


<details>
  <summary>Details</summary>
Motivation: 现实中的多模态属性图通常分布在隔离平台上，由于隐私和商业限制无法共享，而现有的联邦图学习研究主要关注单模态图，缺乏针对多模态联邦图学习的系统性解决方案。

Method: 提出了MM-OpenFGL基准，系统化地形式化了MMFGL范式，包含：19个多模态数据集覆盖7个应用领域，8种模拟策略捕捉模态和拓扑变化，6个下游任务，以及通过模块化API实现的57种SOTA方法。

Result: 通过大量实验从必要性、有效性、鲁棒性和效率四个维度对MMFGL进行了全面研究，为未来MMFGL研究提供了有价值的见解。

Conclusion: MM-OpenFGL填补了多模态联邦图学习领域的空白，为系统评估和推进该领域研究提供了首个综合性基准，解决了现实应用中数据分布和隐私保护的挑战。
...
### [173] [MetaLead: A Comprehensive Human-Curated Leaderboard Dataset for Transparent Reporting of Machine Learning Experiments](https://arxiv.org/abs/2601.22420)
*Roelien C. Timmer,Necva Bölücü,Stephen Wan*

Main category: cs.LG

TL;DR: MetaLead是一个完全人工标注的机器学习排行榜数据集，它捕获所有实验结果以实现结果透明，并包含额外元数据，如实验类型（基线、提出方法或变体）和明确分离的训练/测试数据集，用于更透明和细致的ML评估。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习排行榜创建需要大量人工努力，现有的自动化排行榜生成数据集存在局限性：只捕获每篇论文的最佳结果且元数据有限，无法支持透明和细致的评估。

Method: 提出了MetaLead数据集，这是一个完全人工标注的ML排行榜数据集，捕获所有实验结果（而不仅是最佳结果），包含丰富的元数据（如实验类型：基线、提出方法或变体方法），并明确分离训练和测试数据集以支持跨领域评估。

Result: MetaLead提供了比现有数据集更丰富的结构，能够支持更透明和细致的机器学习研究评估，包括实验类型指导的比较和跨领域评估。

Conclusion: MetaLead是一个强大的资源，其丰富的结构使得机器学习研究能够进行更透明和细致的评估，解决了现有排行榜数据集的局限性。
...
### [174] [ReNCE: Learning to Reason by Noise Contrastive Estimation](https://arxiv.org/abs/2601.22432)
*Wenzheng Zhang,Karl Stratos*

Main category: cs.LG

TL;DR: 提出了一种基于显式对比学习的LLM推理方法，替代传统的GRPO优势估计方法，通过将结果分为正负集并最大化正结果概率，在数学推理基准上取得竞争性表现。


<details>
  <summary>Details</summary>
Motivation: GRPO方法虽然有效，但其依赖于优势估计和多种经验性调整（如非对称裁剪和零方差数据过滤），这些调整需要大量经验洞察且难以识别。作者希望提出一种更直接、更易实现的方法来提升LLM的推理能力。
...
Conclusion: 提出的显式对比学习方法为LLM推理训练提供了一种有效的替代方案，避免了传统GRPO方法中复杂的经验性调整，同时在数学推理任务上保持了竞争性性能。
...
### [175] [Weak Diffusion Priors Can Still Achieve Strong Inverse-Problem Performance](https://arxiv.org/abs/2601.22443)
*Jing Jia,Wei Yuan,Sifan Liu,Liyue Shen,Guanyang Wang*

Main category: cs.LG

TL;DR: 研究发现，即使在数据不匹配（如卧室图像训练的扩散模型用于人脸恢复）的情况下，弱扩散先验在测量信息充足时仍能有效工作，其性能接近完整强度、域内基线。


<details>
  <summary>Details</summary>
Motivation: 扩散模型通常作为逆问题的先验，但标准方法假设使用与未知信号高度匹配的高保真模型。实践中常需使用不匹配或低保真的扩散先验，而令人惊讶的是，这些弱先验往往表现接近完整强度的域内基线。研究旨在探究逆求解器何时以及为何对弱扩散先验具有鲁棒性。

Method: 通过大量实验研究弱先验在不同条件下的表现，并基于贝叶斯一致性理论分析，给出高维测量使后验集中在真实信号附近的条件。

Result: 发现弱先验在测量高度信息丰富（如观测到许多像素）时成功，并识别了其失败的机制。理论分析为弱扩散先验的可靠使用提供了原则性依据。

Conclusion: 研究为理解弱扩散先验在逆问题中的鲁棒性提供了理论和实验基础，明确了其有效应用的条件，对实际中不得不使用不匹配或低保真先验的情况具有指导意义。
...
### [176] [Beyond Activation Patterns: A Weight-Based Out-of-Context Explanation of Sparse Autoencoder Features](https://arxiv.org/abs/2601.22447)
*Yiting Liu,Zhi-Hong Deng*

Main category: cs.LG

TL;DR: 论文提出了一种基于权重的稀疏自编码器特征解释框架，通过直接分析权重交互来理解特征功能，无需激活数据，在Gemma-2和Llama-3.1模型上验证了特征在输出预测和注意力机制中的作用。


<details>
  <summary>Details</summary>
Motivation: 当前稀疏自编码器特征解释方法主要依赖激活模式推断特征语义，但忽视了这些特征实际上是为了重建在前向传播中具有计算功能的激活。需要一种能够直接分析特征功能作用的解释框架。

Method: 提出了一种基于权重的解释框架，通过直接分析权重交互来测量功能效应，无需激活数据。在Gemma-2和Llama-3.1模型上进行了三个实验：1）分析特征直接预测输出token的能力；2）研究特征在注意力机制中的参与情况；3）比较语义和非语义特征在注意力电路中的分布特征。

Result: 实验发现：1）约1/4的特征能够直接预测输出token；2）特征积极参与注意力机制，且具有深度依赖的结构；3）语义和非语义特征在注意力电路中表现出不同的分布特征。该分析提供了稀疏自编码器特征解释中缺失的"上下文外"部分。

Conclusion: 基于权重的解释框架为稀疏自编码器特征提供了更全面的功能理解，揭示了特征在模型计算中的实际作用，补充了传统基于激活的解释方法的不足，为理解语言模型内部表示提供了新视角。
...
### [177] [HeaPA: Difficulty-Aware Heap Sampling and On-Policy Query Augmentation for LLM Reinforcement Learning](https://arxiv.org/abs/2601.22448)
...
Result: 在两个训练语料库、两种训练方案和七个基准测试中，HeaPA一致提高了准确性，以更少的计算达到目标性能，同时保持实际时间可比

Conclusion: HeaPA通过边界聚焦采样和在线池增长提高了RLVR训练效率，随着模型规模的增大，其优势变得更加明显
...
### [178] [Machine Unlearning in Low-Dimensional Feature Subspace](https://arxiv.org/abs/2601.22456)
*Kun Fang,Qinghua Tao,Junxu Liu,Yaxin Xiao,Qingqing Ye,Jian Sun,Haibo Hu*

Main category: cs.LG

TL;DR: LOFT提出了一种基于低维特征子空间的机器遗忘方法，通过在预训练模型中优化小型投影矩阵，实现高效、隐私保护的遗忘学习。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法存在两个关键问题：1) 需要大量重新加载原始数据带来的隐私泄露风险；2) 更新整个预训练模型的低效率。需要一种更高效、隐私保护的遗忘方法。

Method: LOFT方法在预训练模型的低维特征子空间中进行遗忘学习，通过主成分投影优化投影矩阵，最大化保留剩余数据信息同时最小化遗忘数据信息。只需一次性获取特征，无需重复访问原始数据。
...
### [179] [EvoEGF-Mol: Evolving Exponential Geodesic Flow for Structure-based Drug Design](https://arxiv.org/abs/2601.22466)
*Yaowei Jin,Junjie Wang,Cheng Cao,Penglei Wang,Duo An,Qian Shi*

Main category: cs.LG

TL;DR: EvoEGF-Mol：基于信息几何的SBDD新方法，通过指数测地流建模分子分布，避免传统方法在欧几里得和概率空间中的不匹配问题，实现稳定的药物设计生成。
...
Method: 将分子建模为复合指数族分布，在Fisher-Rao度量下定义沿指数测地线的生成流。为避免直接针对Dirac分布的测地线导致的瞬时轨迹崩溃，提出EvoEGF-Mol模型，用动态集中分布替代静态Dirac目标，通过渐进参数精化架构确保稳定训练。

Result: 在CrossDock上达到93.4%的PoseBusters通过率，表现出卓越的几何精度和相互作用保真度。在MolGenBench任务上优于基线方法，能够恢复生物活性支架并生成符合MedChem过滤器的候选分子。

Conclusion: EvoEGF-Mol从信息几何角度解决了传统SBDD方法的统计流形不匹配问题，通过指数测地流框架实现了稳定高效的药物分子生成，在几何精度和化学可行性方面表现出色。
...
### [180] [Unrewarded Exploration in Large Language Models Reveals Latent Learning from Psychology](https://arxiv.org/abs/2601.22474)
*Jian Xiong,Jingbo Zhou,Zihan Zhou,Yixiong Xiao,Le Zhang,Jingyong Ye,Rui Qian,Yang Zhou,Dejing Dou*

Main category: cs.LG

TL;DR: 研究发现大型语言模型在无奖励探索阶段表现出潜在学习动态，这种两阶段训练方法（先无奖励探索，后引入奖励）比全程奖励强化学习获得更高能力。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习过度依赖外部奖励反馈，限制了模型的灵活性和泛化能力。心理学中的潜在学习现象（生物体在无奖励情况下学习环境表征）在LLMs中是否出现尚未探索，这为改进LLM训练提供了新思路。

Method: 采用两阶段训练方法：第一阶段让LLMs进行无奖励探索，第二阶段引入奖励。在多个模型家族和多样化任务领域进行广泛实验，验证潜在学习动态的存在，并提供理论分析解释无奖励探索为何能提升性能。

Result: LLMs在无奖励探索阶段表现出适度性能提升，这种探索让模型能够组织任务相关知识而不受奖励驱动偏见的约束。一旦引入奖励，性能进一步显著增强。采用这种两阶段探索机制后训练的LLMs最终比全程使用基于奖励的强化学习训练的模型获得更高能力。

Conclusion: LLMs确实表现出心理学中描述的潜在学习动态，无奖励探索阶段有助于模型构建更灵活的知识表征，减少奖励偏见的影响。这种两阶段训练方法为改进LLM训练范式提供了新方向，结合认知科学原理可能提升模型的学习效率和泛化能力。
...
### [181] [Continual Policy Distillation from Distributed Reinforcement Learning Teachers](https://arxiv.org/abs/2601.22475)
*Yuxuan Li,Qijun He,Mingqi Yuan,Wen-Tse Chen,Jeff Schneider,Jiayu Chen*

Main category: cs.LG

TL;DR: 提出一种教师-学生框架，将持续强化学习解耦为两个独立过程：通过分布式RL训练单任务教师模型，并持续蒸馏到中央通用模型中，结合MoE架构和回放方法增强可塑性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 持续强化学习面临稳定性-可塑性困境，现有方法难以在序列任务流中实现可扩展性能。观察到RL擅长解决单任务，而策略蒸馏作为相对稳定的监督学习过程，更适合与大型基础模型和多任务学习结合。

Method: 1) 分布式RL训练单任务教师模型；2) 持续蒸馏到中央通用学生模型；3) 采用混合专家(MoE)架构增强可塑性；4) 使用回放方法增强稳定性。

Result: 在Meta-World基准测试中，框架实现了高效持续RL，恢复了超过85%的教师性能，同时将任务遗忘控制在10%以内。

Conclusion: 提出的教师-学生框架通过解耦单任务RL训练和多任务策略蒸馏，有效解决了持续强化学习中的稳定性-可塑性困境，实现了可扩展的终身学习性能。
...
### [182] [Transform-Augmented GRPO Improves Pass@k](https://arxiv.org/abs/2601.22478)
...
<details>
  <summary>Details</summary>
Motivation: 传统的语言模型对表面表达变化敏感，而GRPO方法在改进推理时反而恶化了这种情况，存在多样性崩溃（训练放大单一策略而忽略其他）和梯度消失（大量问题产生零梯度）两种失败模式。

Method: 提出TA-GRPO方法，通过转述、变量重命名和格式变化生成语义等价的变体问题，并通过池化整个组的奖励来计算优势值，确保即使原问题太简单或太难时也能获得混合奖励。

Result: 在数学推理基准测试中，TA-GRPO在竞赛数学（AMC12, AIME24）上获得高达9.84分的Pass@k提升，在分布外科学推理（GPQA-Diamond）上获得5.05分的提升。

Conclusion: TA-GRPO通过减少零梯度概率和降低训练-测试分布偏移来改善泛化能力，理论上证明了其有效性，实验结果表明在推理任务上取得了显著改进。
...
### [183] [Mitigating Cognitive Inertia in Large Reasoning Models via Latent Spike Steering](https://arxiv.org/abs/2601.22484)
*Seojin Lee,ByeongJeong Kim,Hwanhee Lee*

Main category: cs.LG

TL;DR: STARS框架通过监测隐藏状态的L2距离峰值来识别认知转折点，实时注入语言提示来纠正大型推理模型的认知惯性问题


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在扩展测试时计算时表现出认知惯性问题，包括过度思考（运动惯性）和推理僵化（方向惯性），现有基于表面文本启发式的方法无法捕捉模型的内部冲突

Method: 提出STARS训练免费框架，通过检测隐藏状态的L2距离峰值来识别认知转折点，使用几何轨迹分析诊断转换结构，并实时注入状态感知的语言提示来引导模型

Result: 实验表明STARS能有效减少冗余循环，通过自适应纠正错误轨迹提高准确性，为大型推理模型提供了无需额外微调的优化机制

Conclusion: STARS提供了一种鲁棒的无监督机制来优化大型推理模型的推理过程，无需额外训练，能够有效解决认知惯性问题
...
### [184] [Gradual Fine-Tuning for Flow Matching Models](https://arxiv.org/abs/2601.22495)
*Gudrun Thorkelsdottir,Arindam Banerjee*

Main category: cs.LG

TL;DR: 提出了一种名为GFT的渐进微调框架，用于在目标分布样本可用时微调基于流的生成模型，通过温度控制平滑过渡，提高收敛稳定性并缩短推理路径。


<details>
  <summary>Details</summary>
Motivation: 在数据有限、分布演变或效率要求严格的场景中，无约束的微调会损害预训练获得的准确性和效率优势。现有奖励微调方法通常对漂移结构或训练技术施加限制。

Method: 提出GFT框架，为随机流定义温度控制的中间目标序列，平滑地在预训练漂移和目标漂移之间插值，当温度趋近零时接近真实目标。证明了边缘和条件GFT目标的收敛性。
...
### [185] [Action-Sufficient Goal Representations](https://arxiv.org/abs/2601.22496)
*Jinu Hyeon,Woobin Park,Hongjoon Ahn,Taesup Moon*

Main category: cs.LG

TL;DR: 离线目标条件强化学习中，传统基于价值函数学习的目标表示可能无法区分需要不同动作的目标状态，导致控制失败。本文提出动作充分性框架，证明价值充分性不蕴含动作充分性，并展示通过策略学习获得的目标表示优于基于价值估计的表示。


<details>
  <summary>Details</summary>
Motivation: 在分层离线目标条件强化学习中，现有方法通常在学习价值函数时推导目标表示，隐含假设保留价值估计所需信息就足以实现最优控制。但作者发现这种假设可能失败，即使价值估计准确，这样的表示也可能将需要不同动作的目标状态混为一谈。

Method: 引入信息论框架定义动作充分性——目标表示对于最优动作选择的必要条件。证明价值充分性不蕴含动作充分性。提出通过标准对数损失训练底层策略自然诱导动作充分表示的方法。

Result: 在离散环境中经验验证动作充分性与控制成功有更强关联。在流行基准测试中，通过策略学习获得的目标表示一致优于基于价值估计学习得到的表示。

Conclusion: 在分层离线目标条件强化学习中，基于价值函数学习目标表示的传统方法存在缺陷，而通过策略学习获得动作充分表示能更有效地支持最优控制，为分层策略设计提供了新的理论框架和实践指导。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常出现意外错误或非预期行为，尽管已有研究揭示LLMs与人类在技能组合上的差异，但技能组合的学习动态及其非人类行为的根本原因仍不清楚。

Method: 通过在合成算术任务上训练Transformer模型，进行广泛的消融实验和细粒度诊断指标分析，探究学习动态机制。

Result: 发现Transformer模型不按人类顺序规则可靠构建技能组合，而是经常反向或并行获取技能，导致分布偏移时出现意外混合错误（破碎组合性）。证据表明是训练数据的相关性匹配而非因果或程序性组合塑造了学习动态。

Conclusion: 破碎组合性现象在现代LLMs中持续存在，无法通过纯模型缩放或基于草稿的推理缓解。这揭示了模型学习行为与期望技能组合之间的根本不匹配，对推理可靠性、分布外鲁棒性和对齐具有重要影响。
...
<details>
  <summary>Details</summary>
Motivation: 无人机与可见光通信技术的结合为提供灵活通信和高效照明提供了有前景的解决方案。然而，在无人机辅助的可见光通信系统中，如何规划三维轨迹以最小化飞行距离并提高数据收集效率是一个具有挑战性的问题。

Method: 首先推导了在特定可见光通信信道增益阈值下的闭式最优飞行高度。然后，通过将新型信息素驱动奖励机制与双延迟深度确定性策略梯度算法相结合，优化无人机的水平轨迹，实现在复杂环境中的自适应运动策略。

Result: 仿真结果表明，推导的最优飞行高度相比基线方法可减少高达35%的飞行距离。此外，提出的奖励机制显著缩短了约50%的收敛步数，在无人机辅助可见光通信数据收集中表现出显著的效率提升。

Conclusion: 该研究提出的三维轨迹规划框架有效解决了无人机辅助可见光通信系统中的轨迹优化问题，通过高度优化和自适应水平轨迹规划，显著提高了数据收集效率，为无人机与可见光通信的集成应用提供了实用解决方案。
...
TL;DR: 该研究提出了SCOPE-PD框架，通过整合主客观评估数据，使用机器学习预测帕金森病，随机森林模型达到98.66%准确率，并通过SHAP分析识别关键特征。


<details>
  <summary>Details</summary>
Motivation: 帕金森病诊断依赖传统主观方法存在延迟和主观性问题，现有机器学习方法缺乏解释性和个性化风险评估，需要整合主客观数据提供可解释的预测框架。

Method: 从PPMI研究中收集主客观临床评估数据，构建多模态预测框架，应用多种机器学习技术，选择最佳模型并使用SHAP进行结果解释。

Result: 随机森林算法在整合主客观特征后达到最高98.66%准确率，SHAP分析识别震颤、运动迟缓和面部表情为MDS-UPDRS测试中对PD预测贡献最大的三个特征。

Conclusion: SCOPE-PD框架通过整合主客观数据提供可解释的帕金森病预测，为个性化医疗决策提供支持，识别关键临床特征有助于早期诊断和干预。
...
### [189] [Demystifying Design Choices of Reinforcement Fine-tuning: A Batched Contextual Bandit Learning Perspective](https://arxiv.org/abs/2601.22532)
*Hong Xie,Xiao Hu,Tao Tan,Haoran Gu,Xin Li,Jianyu Han,Defu Lian,Enhong Chen*

Main category: cs.LG

TL;DR: 该论文通过构建简约基线方法，系统分析了强化学习微调中各种设计选择的作用和重要性，揭示了哪些因素对学习和泛化性能真正关键。


<details>
  <summary>Details</summary>
Motivation: 强化学习微调领域存在大量研究关注设计选择优化，但性能提升常常被夸大，结论不一致，使得进展显得虚幻。缺乏对两个基本问题的原则性答案：1）每个设计选择的作用是什么？2）哪些设计选择是关键的？

Method: 首先构建一个简约基线来解耦影响因素：每轮每个查询只进行一次rollout，使用结果奖励作为训练信号（不使用任何优势技巧），批量大小为32。该基线与批量上下文赌博机学习相关联，便于实验分析。围绕这个基线设计实验流程，检验优势、rollout数量等因素的边际增益。

Result: 在三个基础模型和两个数据集上的实验不仅揭示了各种设计选择对学习和泛化动态的新理解，还识别出了值得更多努力的关键设计选择。

Conclusion: 通过系统分析强化学习微调中的设计选择，该研究为理解各因素的作用和识别关键因素提供了原则性方法，有助于该领域的实质性进展。
...
### [190] [Learning to Defer in Non-Stationary Time Series via Switching State-Space Models](https://arxiv.org/abs/2601.22538)
*Yannis Montreuil,Letian Yu,Axel Carlier,Lai Xing Ng,Wei Tsang Ooi*

Main category: cs.LG

TL;DR: 该论文研究非平稳时间序列中的学习延迟问题，提出了一种基于因子化切换线性高斯状态空间模型的专家路由方法，通过信息导向的采样策略平衡预测成本与潜在状态信息获取。


<details>
  <summary>Details</summary>
Motivation: 针对非平稳时间序列中专家可用性随时间变化且只能获得部分反馈的问题，需要设计能够处理专家动态增减、实现跨专家信息共享的智能路由系统。

Method: 提出L2D-SLDS模型：因子化切换线性高斯状态空间模型，包含上下文相关的状态转换、共享全局因子（支持跨专家信息传递）和专家特定状态。使用一步预测信念，设计基于信息导向采样的路由规则，平衡预测成本与潜在状态信息获取。

Result: 实验表明，该方法在非平稳时间序列环境中优于上下文老虎机基线方法，且优于无共享因子的消融模型，验证了跨专家信息共享的有效性。

Conclusion: 提出的L2D-SLDS模型和信息导向路由规则能够有效处理非平稳时间序列中的学习延迟问题，支持专家动态管理，并通过共享因子实现跨专家知识转移，提升路由性能。
...
### [191] [Neural-Inspired Posterior Approximation (NIPA)](https://arxiv.org/abs/2601.22539)
...
<details>
  <summary>Details</summary>
Motivation: 人类通过多个相互作用的神经系统的协同工作实现高效学习，包括模型驱动规划、模型自由习惯响应和情景记忆学习。这些系统各有优势：模型驱动灵活但计算成本高，模型自由快速但缺乏灵活性，情景记忆支持快速适应。本文旨在将这些生物学效率原理转化为可扩展贝叶斯推断的采样算法

Method: 提出一种三模块采样算法：1) 模型驱动模块使用目标分布进行引导但计算缓慢的采样；2) 模型自由模块利用先前样本学习参数空间模式，实现快速反射式采样而无需直接评估昂贵的目标分布；3) 情景控制模块通过回忆特定过去事件（样本）支持快速采样

Result: 该方法推进了贝叶斯方法的发展，并促进其在大规模统计机器学习问题中的应用。特别应用于贝叶斯深度学习，重点在于适当和原则性的不确定性量化

Conclusion: 通过模拟人类多系统学习机制，开发了一种高效的贝叶斯采样算法，将生物学效率原理转化为计算工具，为大规模机器学习中的不确定性量化提供了有效解决方案
...
### [192] [Benchmarking Long Roll-outs of Auto-regressive Neural Operators for the Compressible Navier-Stokes Equations with Conserved Quantity Correction](https://arxiv.org/abs/2601.22541)
*Sean Current,Chandan Kumar,Datta Gaitonde,Srinivasan Parthasarathy*

Main category: cs.LG

TL;DR: 提出守恒量校正方法，解决深度学习求解PDE时长期预测误差累积问题，通过物理守恒准则提升神经算子模型的长期稳定性


<details>
  <summary>Details</summary>
Motivation: 深度学习作为PDE数值解的高效替代方法，能够快速近似解算子，但在长期预测中表现不佳，主要原因是自回归误差累积以及模型无法保持物理守恒量

Method: 提出守恒量校正技术，这是一种模型无关的方法，将物理守恒准则整合到深度学习模型中，提升神经算子模型的长期稳定性

Result: 该方法在不同模型架构下都能显著改善自回归神经算子模型的长期稳定性，同时从谱域分析揭示了当前架构在高频分量处理上的局限性

Conclusion: 未来工作需要设计特别关注高频分量的架构，这对于理解和建模湍流等复杂流动至关重要，守恒量校正为提升深度学习PDE求解的长期稳定性提供了有效途径
...
### [193] [FedDis: A Causal Disentanglement Framework for Federated Traffic Prediction](https://arxiv.org/abs/2601.22578)
...
Method: 提出FedDis框架，采用双分支设计：个性化银行学习客户端特定因素，全局模式银行提取共同知识；使用互信息最小化目标强制两个分支的信息正交性，实现有效解耦

Result: 在四个真实世界基准数据集上的综合实验表明，FedDis始终达到最先进的性能，具有高效的推理效率和优越的可扩展性

Conclusion: FedDis通过因果解耦成功解决了联邦时空预测中的数据异质性问题，实现了鲁棒的跨客户端知识转移同时保持对本地环境的高度适应性
...
### [194] [MC-GRPO: Median-Centered Group Relative Policy Optimization for Small-Rollout Reinforcement Learning](https://arxiv.org/abs/2601.22582)
*Youngeun Kim*

Main category: cs.LG

TL;DR: MC-GRPO通过用中位数基线替代均值基线，解决了小样本训练中优势值符号翻转的问题，在资源受限情况下提升了训练稳定性和最终精度。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的小样本训练场景中，基于均值基线的群体相对策略优化方法由于基线噪声导致优势值符号翻转，使得某些样本的更新方向错误，导致精度下降。

Method: 提出中位数中心的群体相对策略优化(MC-GRPO)，用中位数基线替代均值基线，中位数对异常奖励值不敏感。生成G+1个样本用于中位数参考，使用群体中位数计算优势值，排除中位数样本的梯度回传，保持核心更新成本不变。

Result: 在各种GRPO家族方法和不同规模模型上，中位数中心训练在小样本训练中一致提升了稳定性和最终精度，将G=2和G=8之间的差距缩小到1%以内。

Conclusion: MC-GRPO是一种简单有效的解决方案，通过中位数基线缓解小样本训练中的优势值符号翻转问题，在资源受限情况下显著提升训练效果。
...
### [195] [Heterogeneous Graph Alignment for Joint Reasoning and Interpretability](https://arxiv.org/abs/2601.22593)
*Zahra Moslemi,Ziyi Liang,Norbert Fortin,Babak Shahbaba*

Main category: cs.LG

TL;DR: MGMT是一个多图元变换器框架，通过图变换器编码器将异构图映射到共享潜在空间，构建元图实现跨图联合推理，在保持可解释性的同时提升图级预测性能。


<details>
  <summary>Details</summary>
Motivation: 多图学习面临异构图拓扑结构、尺度和语义差异的挑战，特别是在缺乏共享节点标识的情况下，如何有效整合跨图信息是一个重要问题。

Method: MGMT首先使用图变换器编码器将每个图的结构和属性映射到共享潜在空间，然后通过注意力选择任务相关的超节点，基于潜在空间相似性构建连接跨图功能对齐超节点的元图，最后在元图上应用额外的图变换器层进行联合推理。

Result: 在合成数据集和真实世界神经科学应用中，MGMT在图形级预测任务中始终优于现有最先进模型，同时提供可解释的表示，有助于科学发现。

Conclusion: MGMT为结构化多图学习建立了一个统一的框架，推进了图数据在核心领域的表示技术，提供了可扩展且可解释的跨图学习方法。
...
### [196] [Lethe:Adapter-Augmented Dual-Stream Update for Persistent Knowledge Erasure in Federated Unlearning](https://arxiv.org/abs/2601.22601)
*Hanwei Tan,Wentai Hu,Ligang He,Yijun Quan*

Main category: cs.LG

TL;DR: 联邦遗忘学习(Lethe)解决了持续训练中已遗忘知识重新激活的问题，通过解耦待遗忘与待保留知识，确保遗忘的持久性


<details>
  <summary>Details</summary>
Motivation: 现有联邦遗忘学习研究假设协作在遗忘操作后结束，忽视了后续持续训练的情况。研究发现持续训练会导致已遗忘知识重新激活，形成"知识重现"问题

Method: 提出Lethe方法，采用Reshape-Rectify-Restore流程：1)训练临时适配器对遗忘数据执行梯度上升获得放大更新；2)使用这些更新作为校正信号，在两个流中对剩余更新进行层间修正；3)移除适配器并在保留数据上执行短期恢复阶段

Result: Lethe以统一方式支持联邦系统中所有级别的遗忘学习，在大多数情况下保持卓越的持久性（重现率<1%），即使在多轮后续训练后仍能保持

Conclusion: Lethe解决了联邦遗忘学习中知识重现的关键问题，通过解耦知识确保遗忘的持久性，为联邦系统中持续训练场景下的遗忘学习提供了有效解决方案
...
### [197] [Stabilizing Consistency Training: A Flow Map Analysis and Self-Distillation](https://arxiv.org/abs/2601.22679)
...
<details>
  <summary>Details</summary>
Motivation: 一致性模型虽然能实现快速生成建模，但存在训练不稳定和可复现性差的问题。现有研究对此的解释较为碎片化，理论关系不明确，需要系统性的理论分析来理解这些问题

Method: 从流映射角度对一致性模型进行理论分析，揭示训练稳定性和收敛行为如何导致退化解。基于此重新审视自蒸馏方法，重新设计以避免梯度范数过大，实现稳定优化

Result: 提出的方法不仅改进了图像生成任务，还能扩展到基于扩散的策略学习，且不需要预训练扩散模型进行初始化，展示了更广泛的适用性

Conclusion: 通过流映射视角的理论分析，澄清了一致性模型的训练稳定性问题，提出的改进自蒸馏方法能有效解决某些形式的次优收敛，为一致性模型的稳定训练提供了理论指导和实用方案
...
### [198] [Stabilizing Transformer Training Through Consensus](https://arxiv.org/abs/2601.22614)
*Shyam Venkatasubramanian,Sean Moushegian,Michael Lin,Mir Park,Ankit Singhal,Connor Lee*

Main category: cs.LG

TL;DR: 共识机制作为注意力机制的替代方案，能提升Transformer在不同学习率下的训练稳定性


<details>
  <summary>Details</summary>
Motivation: 标准注意力Transformer在训练时对学习率过指定表现出不稳定性，特别是在高学习率下。虽然已有方法通过修改优化过程来改善这种不稳定性，但针对架构本身的基础创新仍不足。

Method: 提出共识机制作为注意力的替代方案，将其形式化为图模型，并进一步提出混合共识-注意力框架以在保持性能的同时提升稳定性。

Result: 通过文本、DNA和蛋白质模态上的学习率扫描实验，证明共识机制在更广泛的有效学习率范围内都能稳定Transformer训练。

Conclusion: 共识机制能有效提升Transformer训练的学习率稳定性，混合框架在保持性能的同时改善了稳定性，并提供了理论分析来表征共识机制的特性。
...
### [199] [Vision-Language Models Unlock Task-Centric Latent Actions](https://arxiv.org/abs/2601.22714)
...
TL;DR: 提出使用视觉语言模型的常识推理能力为潜在动作模型提供可提示表征，有效分离可控变化与噪声，显著提升潜在动作质量


<details>
  <summary>Details</summary>
Motivation: 现有潜在动作模型在观测包含动作相关干扰物时表现不佳，会编码噪声而非有意义的潜在动作。人类却能轻松从视频中区分任务相关运动与无关细节。需要利用视觉语言模型的常识推理能力来解决这一问题。

Method: 利用视觉语言模型的常识推理能力提供可提示表征，在无监督方式下有效分离可控变化与噪声。将这些表征作为潜在动作模型训练的目标，并对多种流行的视觉语言模型进行基准测试。

Result: 发现不同视觉语言模型提供的可提示表征质量存在显著差异，对提示和超参数的鲁棒性也不同。有趣的是，较新的视觉语言模型可能比旧的表现更差。通过让视觉语言模型忽略干扰物，可以大幅提升潜在动作质量，在Distracting MetaWorld上实现下游成功率最高六倍的增长。

Conclusion: 利用视觉语言模型的常识推理能力为潜在动作模型提供可提示表征是一种有效方法，能够显著改善潜在动作质量，特别是在存在动作相关干扰物的情况下。该方法为提升视觉-语言-动作模型的性能提供了新思路。
...
### [200] [TTCS: Test-Time Curriculum Synthesis for Self-Evolving](https://arxiv.org/abs/2601.22628)
*Chengyi Yang,Zhishang Xiang,Yunbo Tang,Zongpei Teng,Chengsong Huang,Fei Long,Yuhan Liu,Jinsong Su*

Main category: cs.LG

TL;DR: TTCS是一个协同进化的测试时训练框架，通过问题合成器和推理求解器的协同进化，为LLMs构建动态的测试时课程，解决现有方法在困难推理问题上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有测试时训练方法在处理困难推理问题时面临两个主要挑战：原始测试问题往往过于困难难以产生高质量伪标签，以及测试集规模有限导致连续在线更新不稳定。

Method: TTCS从同一预训练模型初始化两个策略：问题合成器和推理求解器。这两个策略通过迭代优化协同进化：合成器根据测试问题生成逐渐具有挑战性的问题变体，为求解器当前能力量身定制结构化课程；求解器使用在原始测试和合成问题上多个采样响应的自一致性奖励进行更新。求解器的反馈指导合成器生成与模型当前能力对齐的问题，而生成的问题变体反过来稳定求解器的测试时训练。

Result: 实验表明，TTCS在具有挑战性的数学基准上持续增强了推理能力，并能跨不同LLM骨干模型迁移到通用领域任务，展示了为自进化动态构建测试时课程的可扩展路径。

Conclusion: TTCS通过协同进化的测试时训练框架，有效解决了现有方法在困难推理问题上的局限性，为大型语言模型的自我进化提供了一种可扩展的动态课程构建方法。
...
### [201] [Decomposing and Composing: Towards Efficient Vision-Language Continual Learning via Rank-1 Expert Pool in a Single LoRA](https://arxiv.org/abs/2601.22828)
*Zhan Fa,Yue Duan,Jian Zhang,Lei Qi,Wanqi Yang,Yinghuan Shi*

Main category: cs.LG

TL;DR: 提出一种基于LoRA分解为Rank-1专家池的持续学习框架，通过稀疏组合和正交化减少参数更新，在视觉语言模型中实现高效任务适应并缓解灾难性遗忘


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型中的持续学习面临任务适应和灾难性遗忘的挑战。现有方法通常有沉重的推理负担或依赖外部知识，而LoRA虽能实现参数高效调优，但直接用于缓解灾难性遗忘问题并不简单。

Method: 1. 将单个LoRA模块重构为可分解的Rank-1专家池；2. 基于[CLS]令牌语义动态选择专家，稀疏组合任务特定更新；3. 提出激活引导正交(AGO)损失，正交化LoRA权重关键部分；4. 实现领域感知学习，最小化任务间干扰。

Result: 1. 在多个设置下的广泛实验显示所有指标均达到最先进水平；2. 在泛化能力上超越零样本上界；3. 相比基线方法减少96.7%可训练参数；4. 无需外部数据集或任务ID判别器；5. 合并的LoRA保留较少权重且无推理延迟。

Conclusion: 该方法通过将LoRA重构为Rank-1专家池并引入AGO损失，实现了计算轻量的持续学习，在减少参数更新的同时保持下游任务性能，有效缓解灾难性遗忘问题。
...
### [202] [Pushing the Boundaries of Natural Reasoning: Interleaved Bonus from Formal-Logic Verification](https://arxiv.org/abs/2601.22642)
*Chuxue Cao,Jinluan Yang,Haoran Li,Kunhao Pan,Zijian Zhao,Zhengyu Chen,Yuchen Tian,Lijun Wu,Conghui He,Sirui Han,Yike Guo*

Main category: cs.LG

TL;DR: 提出了一种结合形式逻辑验证与语言模型生成的框架，通过实时反馈检测和纠正推理错误，显著提升LLM的逻辑推理能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然表现出色，但其基于概率的token预测会产生逻辑不一致和奖励黑客问题，而形式符号系统可以避免这些问题。为了弥合这一差距，需要将形式逻辑验证与自然语言生成过程动态结合。

Method: 提出了形式逻辑验证引导的框架，在自然语言生成过程中动态交织形式符号验证，提供实时反馈来检测和纠正错误。采用两阶段训练流程：形式逻辑验证引导的监督微调和策略优化，区别于以往被动的后验验证方法，能主动惩罚推理链中的中间谬误。

Result: 在六个涵盖数学、逻辑和一般推理的基准测试中，7B和14B模型分别以平均10.4%和14.2%的优势超越了最先进的基线模型。

Conclusion: 形式验证可以作为可扩展的机制，显著推动先进LLM推理的性能边界，验证了形式逻辑与神经模型结合的有效性。
...
### [203] [GUDA: Counterfactual Group-wise Training Data Attribution for Diffusion Models via Unlearning](https://arxiv.org/abs/2601.22651)
...
<details>
  <summary>Details</summary>
Motivation: 现有训练数据归因方法主要关注个体样本评分，但实践中常需要群体级答案（如艺术风格或对象类别）。群体归因是反事实的：如果某个群体从训练中移除，模型在生成样本上的行为会如何变化？传统的留一组重训练方法计算成本过高。

Method: 提出GUDA方法：1）通过机器遗忘技术从共享的全数据模型近似每个反事实模型，而不是从头训练；2）使用基于似然的评分规则（ELBO）在全模型和每个遗忘反事实模型之间的差异来量化群体影响

Result: 在CIFAR-10和Stable Diffusion艺术风格归因实验中，GUDA比语义相似性、基于梯度的归因和实例级遗忘方法更可靠地识别主要贡献群体，同时在CIFAR-10上相比留一组重训练实现了100倍加速

Conclusion: GUDA提供了一种高效且准确的群体级训练数据归因方法，通过机器遗忘技术显著降低了计算成本，为理解扩散模型中训练数据群体影响提供了实用工具
...
### [204] [Beyond Fixed Rounds: Data-Free Early Stopping for Practical Federated Learning](https://arxiv.org/abs/2601.22669)
...
Result: 在皮肤病变/血细胞分类任务上，平均仅需47/20轮即可达到比基于验证数据的早期停止高12.5%/10.3%的性能

Conclusion: 这是首个无需验证数据的联邦学习早期停止框架，与基于验证数据的方法性能相当，显著降低计算和隐私成本
...
### [205] [Full-Graph vs. Mini-Batch Training: Comprehensive Analysis from a Batch Size and Fan-Out Size Perspective](https://arxiv.org/abs/2601.22678)
*Mengfan Liu,Da Zheng,Junwei Su,Chuan Wu*

Main category: cs.LG

TL;DR: 本文系统比较了全图训练与小批量GNN训练，通过批大小和扇出大小的视角分析收敛性、泛化性和计算效率，发现全图训练并非总是最优，需要根据资源约束调整超参数。


<details>
  <summary>Details</summary>
Motivation: 全图训练和小批量GNN训练有不同的系统设计需求，但缺乏系统比较。核心挑战在于如何表征这两种方法的模型性能（收敛性和泛化性）和计算效率。虽然批大小是分析DNN的有效视角，但GNN引入了扇出大小这一新维度，需要深入探索批大小和扇出大小对GNN的影响。

Method: 通过实证和理论分析，从批大小和扇出大小的视角系统比较全图与小批量GNN训练。使用Wasserstein距离进行新颖的泛化分析来研究图结构（特别是扇出大小）的影响。揭示批大小和扇出大小在GNN收敛和泛化中的非各向同性效应。

Result: 研究发现批大小和扇出大小对GNN收敛和泛化具有非各向同性影响，为在资源约束下调整这些超参数提供实用指导。全图训练并不总是比经过良好调整的小批量设置产生更好的模型性能或计算效率。

Conclusion: 全图训练并非总是最优选择，需要根据具体资源约束和任务需求调整批大小和扇出大小。通过系统分析批大小和扇出大小的影响，为GNN训练方法选择提供了理论指导和实践建议。
...
### [206] [Do Transformers Have the Ability for Periodicity Generalization?](https://arxiv.org/abs/2601.22690)
*Huanyu Liu,Ge Li,Yihong Dong,Sihan Wu,Peixu Wang,Sihao Cheng,Taozhi Chen,Kechi Zhang,Hao Zhu,Tongxuan Liu*

Main category: cs.LG

TL;DR: 本文研究了Transformer模型在周期性泛化方面的局限性，发现模型能够记忆训练数据中的周期性模式，但无法泛化到未见过的复合周期性场景。


<details>
  <summary>Details</summary>
Motivation: 尽管基于Transformer的大语言模型在各种任务上表现出色，但在分布外泛化方面仍存在明显局限，特别是在周期性这一基本分布外场景中。本文旨在探究模型与人类在周期性泛化能力上的差距。

Method: 从抽象代数和推理的角度提出周期性统一解释框架，包括单一和复合周期性；构建了Coper基准测试，包含Hollow和Extrapolation两种分布外设置，用于评估模型在复合周期性泛化方面的能力。

Result: 实验表明Transformer在周期性泛化方面存在局限：模型能够在训练过程中记忆周期性数据，但无法泛化到未见过的复合周期性场景。

Conclusion: Transformer模型在周期性泛化方面存在根本性限制，需要进一步研究来提升模型在分布外场景中的推理能力。作者开源了代码以支持后续研究。
...
### [207] [Metric Hub: A metric library and practical selection workflow for use-case-driven data quality assessment in medical AI](https://arxiv.org/abs/2601.22702)
...
TL;DR: 该研究提出了一个用于评估医疗AI数据质量的METRIC框架，并开发了一个包含具体指标和选择策略的指标库，以支持医疗AI的可信度评估。


<details>
  <summary>Details</summary>
Motivation: 医疗AI从研究走向实际应用需要建立可信度，而数据质量是影响AI模型训练和测试的关键因素。现有研究缺乏系统评估医疗数据适用性的实践方法。

Method: 将理论上的METRIC框架操作化，开发了一个包含具体数据质量指标的指标库，为每个指标提供详细的指标卡片，并设计了基于用例选择适当指标集的决策树策略。

Result: 在PTB-XL心电图数据集上展示了该方法的实际应用效果，证明能够支持医疗AI训练和测试数据的适用性评估。

Conclusion: 这是实现医疗领域可信AI的重要一步，为实践中评估训练和测试数据的适用性提供了实用工具，有助于医疗AI的临床接受度和监管审批。
...
### [208] [Deep Learning-Based Early-Stage IR-Drop Estimation via CNN Surrogate Modeling](https://arxiv.org/abs/2601.22707)
*Ritesh Bhadana*

Main category: cs.LG

TL;DR: 提出基于深度学习的IR-drop早期估计方法，使用CNN将物理布局特征映射到IR-drop热力图，实现毫秒级推理，作为传统物理签核工具的补充工具。


<details>
  <summary>Details</summary>
Motivation: 传统IR-drop分析依赖物理签核工具，虽然精度高但计算成本大且需要接近最终版图信息，不适合早期快速设计探索。需要一种快速、低成本的早期IR-drop估计方法。

Method: 采用基于U-Net的编码器-解码器架构，具有跳跃连接，将IR-drop估计建模为密集像素级回归问题。使用自生成的物理启发合成数据集训练，包含电源网格结构、单元密度分布和开关活动等关键物理因素。

Result: 实验结果表明，该方法能够准确预测IR-drop分布，推理时间达到毫秒级别，支持快速预签核筛选和迭代设计优化。模型性能通过均方误差和峰值信噪比等标准回归指标评估。

Conclusion: 提出的深度学习框架可作为补充的早期分析工具，在设计者进行昂贵的签核分析前提供快速的IR-drop洞察。代码、数据集生成脚本和交互式推理应用已公开。
...
### [209] [A Unified Study of LoRA Variants: Taxonomy, Review, Codebase, and Empirical Evaluation](https://arxiv.org/abs/2601.22708)
*Haonan He,Jingqi Ye,Minglei Li,Zhengbo Wang,Tao Chen,Lei Bai,Peng Ye*

Main category: cs.LG

TL;DR: 该研究首次对LoRA变体进行了系统化统一研究，提出了分类体系、理论框架、统一代码库和标准化评估，发现LoRA对学习率高度敏感，且经过适当调参后，基础LoRA性能可匹敌或超越大多数变体。


<details>
  <summary>Details</summary>
Motivation: LoRA作为一种高效的参数微调方法，其变体数量激增导致方法论、理论、代码和评估的碎片化，需要系统化的统一研究来理清这一领域。

Method: 1) 从四个主要维度对LoRA变体进行分类：秩、优化动态、初始化和与MoE的集成；2) 在低秩更新动态的统一理论框架下分析变体关系；3) 开发LoRAFactory模块化代码库；4) 在自然语言生成、理解和图像分类任务上进行大规模评估。

Result: 研究发现：1) LoRA及其变体对学习率的选择比其他超参数更为敏感；2) 通过适当的超参数配置，基础LoRA的性能能够匹配或超越大多数变体。

Conclusion: 该研究为LoRA领域提供了首个系统化的统一框架，揭示了基础LoRA在适当调参下的强大性能，为未来研究和实践提供了重要指导。
...
### [210] [Breaking the Blocks: Continuous Low-Rank Decomposed Scaling for Unified LLM Quantization and Adaptation](https://arxiv.org/abs/2601.22716)
*Pingzhi Tang,Ruijie Zhou,Fanxu Meng,Wenjie Pei,Muhan Zhang*

Main category: cs.LG

TL;DR: LoRDS提出了一种基于低秩分解的元素级量化框架，通过连续低秩矩阵建模缩放流形，在保持块级量化效率的同时提供更强的表达能力，实现了量化、优化和微调的统一解决方案。
...
Conclusion: LoRDS通过低秩分解实现了元素级量化的高效率和高表达能力，为LLM的统一压缩和适配提供了强大且集成的解决方案，在量化精度、推理速度和下游任务性能方面均有显著提升。
...
### [211] [Decomposing Epistemic Uncertainty for Causal Decision Making](https://arxiv.org/abs/2601.22736)
...
Conclusion: 该框架能指导实践者收集更多变量或转向随机研究以进行最佳行动识别，区分了可减少的样本不确定性和只能通过观察更多变量减少的非可识别不确定性
...
### [212] [Is Softmax Loss All You Need? A Principled Analysis of Softmax-family Loss](https://arxiv.org/abs/2601.22745)
*Yuanhao Pu,Defu Lian,Enhong Chen*

Main category: cs.LG

TL;DR: 该论文对Softmax系列损失函数进行了系统性研究，分析了不同替代损失在分类和排序任务中的一致性、梯度动态和收敛行为，并提出了近似方法的偏差-方差分解框架，为大规模类别机器学习应用中的损失选择提供了理论基础和实践指导。


<details>
  <summary>Details</summary>
Motivation: Softmax损失是分类和排序任务中最广泛使用的替代目标函数，但现有研究存在两个分离的视角：一方面Fenchel-Young框架将其置于广泛的替代函数家族中进行理论分析，另一方面针对类别数量极大的情况提出了各种近似方法以提高效率。论文旨在整合这两个视角，对Softmax系列损失进行系统性研究。

Method: 1) 分析不同替代损失函数与分类和排序指标的一致性；2) 研究梯度动态以揭示不同的收敛行为；3) 为近似方法引入系统的偏差-方差分解框架，提供收敛保证；4) 进行每轮迭代的复杂度分析，展示效果与效率之间的权衡关系。

Result: 在代表性任务上的大量实验表明，一致性、收敛性和实证性能之间存在强烈的对应关系。研究为大规模类别机器学习应用中的损失选择建立了理论基础，并提供了实践指导。

Conclusion: 该研究整合了Softmax损失的理论分析和实际效率优化两个视角，建立了系统的分析框架，揭示了不同替代损失的理论性质与实证性能之间的关系，为大规模类别机器学习中的损失函数选择提供了原则性指导。
...
### [213] [SOMBRERO: Measuring and Steering Boundary Placement in End-to-End Hierarchical Sequence Models](https://arxiv.org/abs/2601.22805)
*Pit Neitemeier,Alessio Serra,Jiaze Li,Sascha Wirges,Lukas Balles,Jan Hendrik Metzen*

Main category: cs.LG

TL;DR: Sombrero是一种改进分层序列模型边界学习的方法，通过边界富集度指标和置信对齐损失，使边界更集中在预测困难的位置，提升效率-准确率权衡。


<details>
  <summary>Details</summary>
Motivation: 分层序列模型通过学习的分段来压缩长字节序列以提高自回归建模效率，但现有方法难以定量评估和系统性地引导计算资源分配。需要一种方法来评估边界质量并引导边界放置在预测困难的位置。

Method: 提出边界富集度B作为边界质量指标，衡量块起始位置在具有高下一字节惊奇度位置的集中程度。在此基础上提出Sombrero方法：1）使用置信对齐边界损失引导边界朝向预测困难位置；2）在输入级别应用置信加权平滑而非在已实现的块上进行平滑，以稳定边界学习。

Result: 在10亿参数规模上，在涵盖英语和德语文本以及代码和数学内容的UTF-8语料库上，Sombrero改善了准确率-效率权衡，产生的边界更一致地将计算资源与难以预测的位置对齐。

Conclusion: Sombrero通过定量边界质量评估和系统性的边界引导机制，有效改善了分层序列模型的效率-准确率权衡，使计算资源更合理地分配到预测困难的位置。
...
### [214] [User-Adaptive Meta-Learning for Cold-Start Medication Recommendation with Uncertainty Filtering](https://arxiv.org/abs/2601.22820)
*Arya Hadizadeh Moghaddam,Mohsen Nayebi Kerdabadi,Dongjie Wang,Mei Liu,Zijun Yao*

Main category: cs.LG

TL;DR: MetaDrug是一个多级不确定性感知元学习框架，通过两级元适应机制解决电子健康记录中的患者冷启动问题，在MIMIC-III和AKI数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有药物推荐方法面临患者冷启动问题，即新患者因缺乏足够的处方历史而难以获得可靠推荐。虽然已有研究使用医学知识图谱连接药物概念，但这些方法主要解决项目冷启动问题，无法提供适应个体患者特征的个性化推荐。元学习在处理稀疏交互的新用户方面显示出潜力，但在电子健康记录中的应用仍待探索。

Method: 提出MetaDrug框架，包含两级元适应机制：1）自适应：利用患者自身医疗事件作为支持集，捕捉时间依赖性；2）同伴适应：利用相似患者的就诊记录丰富新患者表示。同时引入不确定性量化模块，对支持就诊进行排序并过滤无关信息以确保适应一致性。

Result: 在MIMIC-III和急性肾损伤（AKI）数据集上的实验结果表明，MetaDrug在冷启动患者上持续优于最先进的药物推荐方法。

Conclusion: MetaDrug通过创新的两级元适应机制和不确定性量化，有效解决了电子健康记录中的患者冷启动问题，为临床决策支持提供了更可靠的药物推荐。
...
### [215] [OSNIP: Breaking the Privacy-Utility-Efficiency Trilemma in LLM Inference via Obfuscated Semantic Null Space](https://arxiv.org/abs/2601.22752)
*Zhiyuan Cao,Zeyu Ma,Chenhao Yang,Han Zheng,Mingang Chen*

Main category: cs.LG

TL;DR: OSNIP是一种轻量级客户端加密框架，通过将原始嵌入投影到"混淆语义零空间"来保护LLM推理隐私，无需后处理，在保持语义保真度的同时实现隐私保护。


<details>
  <summary>Details</summary>
Motivation: 当前LLM推理中的隐私保护需求日益增长，需要一种既能保护用户隐私又能保持模型效用的轻量级解决方案。现有的隐私保护方法往往需要在隐私和效用之间做出权衡，或者需要复杂的后处理步骤。

Method: OSNIP将线性核的几何直觉推广到LLM的高维潜在空间，定义了"混淆语义零空间"——一个保持语义保真度同时与原始嵌入接近正交的高维区域。通过注入将原始嵌入投影到此空间的扰动，并结合密钥相关的随机映射为每个用户生成独特的扰动轨迹，实现隐私保护。

Result: 在12个生成和分类基准测试中，OSNIP实现了最先进的性能，显著降低了攻击成功率，同时在严格的安全约束下保持了强大的模型效用。

Conclusion: OSNIP提供了一种有效的轻量级客户端加密框架，能够在保护LLM推理隐私的同时保持语义保真度，无需后处理步骤，为隐私保护LLM应用提供了实用解决方案。
...
### [216] [Unveiling Scaling Behaviors in Molecular Language Models: Effects of Model Size, Data, and Representation](https://arxiv.org/abs/2601.22757)
*Dong Xu,Qihua Pan,Sisi Yuan,Jianqiang Li,Zexuan Zhu,Junkai Ji*

Main category: cs.LG

TL;DR: 该研究系统探索了分子语言模型的缩放规律，通过300个模型和10,000多次实验，揭示了在固定计算预算下模型大小、训练数据量和分子表示对性能的影响规律。


<details>
  <summary>Details</summary>
Motivation: 分子生成模型虽然在大规模数据和模型尺寸下表现出潜力，但其是否遵循可预测的缩放规律尚不明确，这对于在模型大小、数据量和分子表示之间优化资源分配至关重要。

Method: 研究训练了300个模型，进行了超过10,000次实验，在严格控制计算预算的同时，独立变化模型大小、训练token数量和分子表示，系统研究分子语言模型的缩放行为。

Result: 研究结果表明分子模型在预训练和下游任务中都存在清晰的缩放规律，揭示了分子表示对性能的显著影响，并解释了先前观察到的分子生成缩放行为不一致性。

Conclusion: 该研究为分子语言模型的缩放行为提供了系统理解，公开发布了迄今为止最大的分子语言模型库，为未来研究和开发提供了重要资源。
...
### [217] [Sparse Attention as Compact Kernel Regression](https://arxiv.org/abs/2601.22766)
*Saul Santos,Nuno Gonçalves,Daniel C. McNamee,André F. T Martins*

Main category: cs.LG

TL;DR: 论文建立了稀疏注意力机制与紧支撑核函数之间的形式对应关系，揭示了归一化ReLU和sparsemax注意力分别对应固定和自适应归一化的Epanechnikov核回归，而α-entmax注意力对应非参数密度估计中的常用核函数。


<details>
  <summary>Details</summary>
Motivation: 先前研究揭示了transformer中自注意力机制与Nadaraya-Watson估计器的测试时核回归之间的联系，标准softmax注意力对应高斯核。然而，目前缺乏对稀疏注意力机制的核理论理解。本文旨在填补这一空白，建立稀疏注意力与紧支撑核函数的形式对应关系。

Method: 通过理论分析建立稀疏注意力机制与紧支撑核函数之间的数学对应关系。具体展示了归一化ReLU和sparsemax注意力分别对应固定和自适应归一化的Epanechnikov核回归。更一般地，证明了非参数密度估计中广泛使用的核函数（Epanechnikov、biweight、triweight）对应α-entmax注意力，其中α=1+1/n（n∈ℕ），而softmax/高斯关系在n→∞时出现。实验使用基于核回归的transformer变体Memory Mosaics进行验证。

Result: 建立了稀疏注意力与紧支撑核函数的统一理论框架，解释了稀疏性如何从核设计中自然产生，并为启发式的top-k注意力和其他关联记忆机制提供了原则性替代方案。实验表明，基于核的稀疏注意力在语言建模、上下文学习和长度泛化任务上实现了有竞争力的性能。

Conclusion: 本文为稀疏注意力机制提供了核理论基础，建立了稀疏注意力与紧支撑核函数的形式对应关系。这一统一视角不仅解释了稀疏性的来源，还为设计注意力机制提供了原则性框架，通过Memory Mosaics实验验证了基于核的稀疏注意力的有效性。
...
### [218] [Perplexity Cannot Always Tell Right from Wrong](https://arxiv.org/abs/2601.22950)
*Petar Veličković,Federico Barbero,Christos Perivolaropoulos,Simon Osindero,Razvan Pascanu*

Main category: cs.LG

TL;DR: 论文证明困惑度作为模型选择指标存在根本缺陷：即使Transformer模型能准确预测某些序列，也必然存在其他序列具有很低困惑度但模型预测错误，且困惑度不一定选择更准确的模型。


<details>
  <summary>Details</summary>
Motivation: 困惑度作为衡量模型质量的简单计算指标近年来广泛应用，但先前研究主要从经验角度指出其局限性。本文旨在从理论层面严谨证明困惑度作为模型选择指标的不适用性。

Method: 利用Transformer连续性的最新结果进行理论证明：1) 证明如果紧凑的仅解码器Transformer模型能准确自信地预测任何序列，则必然存在另一序列具有很低困惑度但模型预测错误；2) 通过分析等困惑度图，研究困惑度选择模型的条件。

Result: 理论证明显示困惑度存在根本缺陷：模型准确预测某些序列的必要前提会导致存在其他序列具有低困惑度但预测错误。等困惑度分析表明，困惑度不一定选择更准确的模型，只有当模型置信度提升伴随相应准确度提升时才会被选择。

Conclusion: 困惑度作为模型选择指标在理论上是不可靠的，存在系统性偏差。模型置信度的提升必须伴随准确度的相应提高，困惑度才会选择更好的模型。这为困惑度的局限性提供了严格的理论依据。
...
### [219] [Clipping-Free Policy Optimization for Large Language Models](https://arxiv.org/abs/2601.22801)
*Ömer Veysel Çağatan,Barış Akgün,Gözde Gül Şahin,Xuandong Zhao*

Main category: cs.LG

TL;DR: CFPO提出了一种无剪裁策略优化方法，用凸二次惩罚替代传统剪裁机制，解决了大规模语言模型强化学习中的梯度消失、奖励黑客和训练不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 当前主流强化学习算法依赖剪裁机制，但在大规模应用中存在零梯度区域、奖励黑客和训练不稳定等优化问题，需要更稳定的替代方案。

Method: CFPO用基于总变差散度约束的凸二次惩罚替代启发式剪裁，创建了处处可微的目标函数，实现无硬边界的稳定策略更新。

Result: 在推理任务中，CFPO与剪裁方法在下游基准测试中表现相当，同时扩展了稳定训练范围；在对齐任务中，CFPO缓解了冗长利用问题，减少了能力退化，同时保持了竞争力的指令跟随性能。

Conclusion: CFPO是一种有前景的即插即用替代方案，只需一行代码更改且无需额外超参数，可替代剪裁方法用于LLM后训练。
...
### [220] [Quartet II: Accurate LLM Pre-Training in NVFP4 by Improved Unbiased Gradient Estimation](https://arxiv.org/abs/2601.22813)
...
<details>
  <summary>Details</summary>
Motivation: 现有量化训练方法为了获得更准确的无偏量化梯度估计（通过随机舍入），牺牲了NVFP4格式的部分表示能力，导致相对于标准FP16和FP8训练存在明显的精度损失。需要改进NVFP4量化训练方法。

Method: 提出MS-EDEN：一种针对微尺度格式的新型无偏量化方法，量化误差比随机舍入低2倍以上。将其集成到Quartet II方案中，这是针对线性层的完全NVFP4量化方案。

Result: 分析证明Quartet II在所有主要矩阵乘法（前向和后向传播）中都能获得更一致的梯度估计。在1.9B参数、38B tokens的LLM端到端训练中验证了有效性。在NVIDIA Blackwell GPU上实现高达4.2倍于BF16的加速。

Conclusion: Quartet II方案显著提升了NVFP4量化训练的性能，通过MS-EDEN量化方法降低了量化误差，实现了更精确的梯度估计，为大规模模型的端到端完全量化预训练提供了有效解决方案。
...
### [221] [Cascaded Flow Matching for Heterogeneous Tabular Data with Mixed-Type Features](https://arxiv.org/abs/2601.22816)
*Markus Mueller,Kathrin Gruber,Dennis Fok*

Main category: cs.LG

TL;DR: 本文提出了一种级联扩散模型，用于生成包含离散和连续特征的表格数据，特别解决了混合类型特征（离散状态与连续分布结合）的生成难题。


<details>
  <summary>Details</summary>
Motivation: 生成建模在表格数据中的应用面临挑战，特别是混合类型特征（同时包含离散状态和连续分布的特征）的生成问题。现有方法难以准确生成这类复杂特征。

Method: 采用级联方法：首先生成低分辨率版本（纯分类特征和数值特征的粗粒度分类表示），然后通过新颖的引导条件概率路径和数据依赖耦合，在流匹配模型中利用这些信息生成高分辨率数据。

Result: 模型显著提高了生成样本的真实性，更准确地捕捉分布细节，检测分数提高了40%。形式化证明表明该级联方法能够收紧传输成本界限。

Conclusion: 提出的级联扩散模型在表格数据生成方面取得了先进成果，特别在混合类型特征的生成上表现优异，为表格数据的生成建模提供了有效解决方案。
...
### [222] [Mano: Restriking Manifold Optimization for LLM Training](https://arxiv.org/abs/2601.23000)
*Yufei Gu,Zeke Xie*

Main category: cs.LG

TL;DR: Mano是一种新颖高效的优化器，通过将动量投影到参数切空间并约束在旋转斜流形上，首次弥合了流形优化与现代优化器之间的性能差距，在LLaMA和Qwen3模型上显著优于AdamW和Muon，同时减少内存消耗和计算复杂度。
...
Result: 在LLaMA和Qwen3模型上的广泛实验表明，Mano在性能和效率上均显著优于AdamW和Muon，即使在内存消耗和计算复杂度更低的情况下也能取得更好效果，扩展了空间和时间效率的帕累托前沿。

Conclusion: Mano首次成功弥合了流形优化与现代优化器之间的性能差距，为大语言模型训练提供了更高效、更强大的优化解决方案，在保持计算效率的同时充分利用了模型的结构特性。
...
### [223] [Automatic Constraint Policy Optimization based on Continuous Constraint Interpolation Framework for Offline Reinforcement Learning](https://arxiv.org/abs/2601.23010)
*Xinchen Han,Qiuyang Fang,Hossam Afifi,Michel Marot*

Main category: cs.LG

TL;DR: 提出连续约束插值(CCI)框架统一离线RL中的三种约束方法，并开发自动约束策略优化(ACPO)算法，在多个基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有离线强化学习方法通常只采用单一约束形式（加权行为克隆、密度正则化或支持约束），缺乏统一的理论框架来解释它们之间的联系和权衡。

Method: 提出连续约束插值(CCI)框架，通过单个插值参数实现三种约束形式的平滑过渡和组合；基于CCI开发自动约束策略优化(ACPO)算法，使用拉格朗日对偶更新自适应调整插值参数。

Result: 在D4RL和NeoRL2基准测试中取得了稳健的性能提升，整体达到最先进的性能水平。

Conclusion: CCI框架为离线RL中的约束方法提供了统一的理论视角，ACPO算法通过自适应约束插值实现了优越的性能表现。
...
TL;DR: 提出使用仅两个表面肌电通道的深度学习框架，通过卷积稀疏自编码器提取特征，实现高精度手势识别，并解决受试者间差异和功能扩展问题。


<details>
  <summary>Details</summary>
Motivation: 肌电假肢控制面临受试者间差异大和高密度传感器阵列临床不实用的挑战，需要开发仅使用少量通道的高精度识别系统。

Method: 使用卷积稀疏自编码器直接从原始信号提取时序特征，无需启发式特征工程；采用少样本迁移学习解决受试者差异；通过增量学习策略支持功能扩展。

Result: 6类手势识别F1分数达94.3%±0.3%；少样本迁移学习将未见受试者性能从35.1%±3.1%提升至92.3%±0.9%；增量学习扩展到10类手势仍保持90.0%±0.2%的F1分数。
...
### [225] [Avoiding Premature Collapse: Adaptive Annealing for Entropy-Regularized Structural Inference](https://arxiv.org/abs/2601.23039)
*Yizhi Liu*
...
<details>
  <summary>Details</summary>
Motivation: 可微分匹配层通过熵正则化最优传输实现近似推理，但在退火过程中恢复离散排列时存在不稳定性。作者发现这种失败的根本机制是"过早模式崩溃"，即目标后验的移动速度超过了推理算子的收缩速度。

Method: 通过分析Sinkhorn不动点映射的非正规动力学，揭示了"热力学速度极限"理论。提出Efficient PH-ASC自适应调度算法，通过监控推理过程的稳定性并强制执行线性稳定性定律，将昂贵的谱诊断与训练循环解耦。

Result: 提出的算法将计算开销从O(N³)降低到摊销O(1)，实现了更稳定的推理过程。提供了开源实现和交互式演示。

Conclusion: 论文识别了可微分匹配层中退火不稳定的根本原因，并提出了一种高效的自适应调度解决方案，为结构预测中的近似推理提供了更稳定和实用的方法。
...
### [226] [Unconditional flow-based time series generation with equivariance-regularised latent spaces](https://arxiv.org/abs/2601.22848)
*Camilo Carvajal Reyes,Felipe Tobar*

Main category: cs.LG

TL;DR: 该论文提出了一种通过正则化预训练自编码器来增强潜在空间等变性的流匹配框架，用于时间序列生成，在保持计算效率的同时提升了生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前基于流模型的时间序列生成方法虽然有效，但如何在潜在空间中设计具有良好等变性的表示仍未充分探索。等变性对于时间序列生成很重要，因为它能确保模型对基本变换（如平移、幅度缩放）具有一致性。

Method: 提出潜在流匹配框架，通过引入等变性损失来正则化预训练的自编码器。该损失强制变换后的信号与其重构之间的一致性，用于微调潜在空间，使其对时间序列的基本变换（如平移和幅度缩放）具有等变性。

Result: 在多个真实世界数据集上的实验表明，该方法在标准时间序列生成指标上一致优于现有的基于扩散的基线方法，同时实现了数量级更快的采样速度。等变性正则化的潜在空间提高了生成质量，同时保持了潜在流模型的计算优势。

Conclusion: 将几何归纳偏置纳入时间序列的潜在生成模型中具有实际益处。等变性正则化的潜在空间既能提升生成质量，又能保持计算效率，为时间序列生成提供了有效的解决方案。
...
TL;DR: 提出基于Wasserstein GAN的密度感知条件图生成框架，用可学习的基于距离的边预测器替代随机采样，能够生成结构更连贯、类别一致的图数据


<details>
  <summary>Details</summary>
Motivation: 现有图生成方法通常依赖固定概率的随机边采样，难以捕捉节点间复杂的结构依赖关系和类别特定的连接模式，限制了生成图的质量

Method: 使用WGAN框架，将节点嵌入到潜在空间，通过可微边预测器从节点嵌入直接确定成对关系，并采用密度感知选择机制自适应控制边密度以匹配真实图的稀疏分布

Result: 在基准数据集上的实验表明，该方法生成的图在结构连贯性和类别一致性方面优于现有基线，边预测器能够捕捉超越简单启发式的复杂关系模式

Conclusion: 该框架实现了更稳定的训练和可控的合成，能够有效生成逼真的图数据并用于数据增强，学习到的边预测器能够准确匹配真实图的结构分布
...
### [228] [OptiMAG: Structure-Semantic Alignment via Unbalanced Optimal Transport](https://arxiv.org/abs/2601.22856)
*Yilong Zuo,Xunkai Li,Zhihan Zhang,Qiangqiang Dai,Ronghua Li,Guoren Wang*

Main category: cs.LG

TL;DR: OptiMAG是一个基于非平衡最优传输的正则化框架，用于解决多模态属性图中模态嵌入语义结构与显式图结构之间的不一致问题，通过融合Gromov-Wasserstein距离引导跨模态结构一致性。


<details>
  <summary>Details</summary>
Motivation: 多模态属性图在建模复杂系统时存在隐式语义结构与显式图结构之间的不一致问题。例如，在显式图结构中的邻居节点可能在某个模态上相似，但在另一个模态上却相距甚远。现有方法通常在固定的显式图结构上进行消息传递，这会导致聚合不相似的特征，引入模态特定噪声，阻碍有效的节点表示学习。

Method: 提出OptiMAG框架，采用基于融合Gromov-Wasserstein距离的非平衡最优传输正则化方法，在局部邻域内显式引导跨模态结构一致性，有效缓解结构-语义冲突。同时使用KL散度惩罚来自适应处理跨模态不一致性。该框架可以作为即插即用的正则化器无缝集成到现有的多模态图模型中。

Result: 实验表明OptiMAG在多个任务上持续优于基线方法，包括图中心任务（如节点分类、链接预测）和多模态中心生成任务（如图到文本、图到图像生成）。

Conclusion: OptiMAG通过最优传输正则化有效解决了多模态属性图中结构-语义不一致问题，能够提升现有多模态图模型的性能，在多种图相关任务中表现出色。
...
### [229] [Matterhorn: Efficient Analog Sparse Spiking Transformer Architecture with Masked Time-To-First-Spike Encoding](https://arxiv.org/abs/2601.22876)
*Zhanglu Yan,Kaiwen Tang,Zixuan Zhu,Zhenyu Bai,Qianhui Liu,Weng-Fai Wong*

Main category: cs.LG

TL;DR: Matterhorn是一种新型脉冲变压器，通过M-TTFS编码减少脉冲移动，结合MSU消除权重访问开销，在保持准确性的同时显著提升能效


<details>
  <summary>Details</summary>
Motivation: 现有SNN能效评估主要关注计算操作，忽略了数据移动等实际硬件成本（占总能耗近80%），需要开发能同时减少脉冲移动和权重访问开销的解决方案

Method: 提出M-TTFS编码方法：1) 将零能量静默状态重新分配给最频繁的膜电位而非最低值；2) 采用"死区"策略将特定范围内的值映射到静默状态以最大化稀疏性。硬件层面使用MSU（忆阻突触单元）通过存内计算技术直接在内存中进行模拟积分
...
TL;DR: 本文提出进化预测（EF）范式，挑战了长期时间序列预测中占主导地位的直接预测（DF）方法，发现短时域训练的模型结合EF能超越直接长时域训练，解决了DF中远未来梯度冲突的问题。


<details>
  <summary>Details</summary>
Motivation: 当前主流的直接预测范式在长期时间序列预测中存在局限性：输出时域与评估时域的刚性耦合导致针对不同目标时域需要重新训练模型，计算成本高昂。作者发现DF存在优化异常，远未来的冲突梯度会破坏局部动态的学习。

Method: 提出进化预测（EF）范式，将DF作为其退化特例。EF是一个统一的生成框架，通过短时域训练模型结合进化推理，逐步生成长期预测，避免了DF中的梯度冲突问题。

Result: 实验表明，单个EF模型在标准基准测试中超越了任务特定的DF集成模型，并在极端外推中表现出稳健的渐近稳定性。短时域训练的EF模型显著优于直接长时域训练的DF模型。

Conclusion: 这项工作推动了长期时间序列预测的范式转变：从被动的静态映射转向自主的进化推理。EF框架为LTSF提供了更高效、更稳健的解决方案，解决了DF的优化病理问题。
...
### [231] [PlatoLTL: Learning to Generalize Across Symbols in LTL Instructions for Multi-Task RL](https://arxiv.org/abs/2601.22891)
*Jacques Cloete,Mathias Jackermeier,Ioannis Havoutis,Alessandro Abate*

Main category: cs.LG

TL;DR: PlatoLTL：一种新颖的多任务强化学习方法，能够实现LTL公式结构的组合式泛化和命题的参数化泛化，实现对新命题和任务的零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 现有基于线性时序逻辑(LTL)的多任务强化学习方法虽然能在LTL规范间成功泛化，但无法泛化到未见过的命题（描述LTL中高层事件的符号）词汇表。这限制了策略对新任务和新环境的适应能力。

Method: 将命题视为参数化谓词的实例而非离散符号，使策略能够学习相关命题间的共享结构。提出新颖的架构来嵌入和组合谓词以表示LTL规范。

Result: 在具有挑战性的环境中，PlatoLTL成功实现了对新命题和任务的零样本泛化，证明了其在跨命题参数化泛化方面的有效性。

Conclusion: PlatoLTL通过将命题视为参数化谓词，实现了对LTL公式结构的组合式泛化和对命题的参数化泛化，显著提升了多任务强化学习策略的泛化能力。
...
### [232] [On Safer Reinforcement Learning Policies for Sedation and Analgesia in Intensive Care](https://arxiv.org/abs/2601.23154)
*Joel Romero-Hernandez,Oscar Camara*

Main category: cs.LG

TL;DR: 该研究使用深度强化学习框架，基于MIMIC-IV数据库中47,144次ICU住院数据，训练药物剂量建议策略，发现仅优化短期疼痛管理目标的策略与死亡率正相关，而同时考虑疼痛和死亡率长期目标的策略与死亡率负相关。


<details>
  <summary>Details</summary>
Motivation: ICU疼痛管理需要在治疗目标和患者安全之间进行复杂权衡，传统方法存在不足。强化学习可以从回顾性数据中学习药物剂量策略，但先前研究存在两个问题：1) 优化目标不重视患者生存率；2) 使用的算法不适合不完全信息环境。

Method: 开发了一个深度强化学习框架，在部分可观测环境下提供每小时药物剂量建议。使用MIMIC-IV数据库中47,144次ICU住院数据，训练策略来开具阿片类药物、丙泊酚、苯二氮䓬类和右美托咪定的处方。比较了两种目标：仅减少疼痛 vs 联合减少疼痛和死亡率。

Result: 两种策略都与较低的疼痛水平相关，但仅优化疼痛管理的策略与死亡率呈正相关，而同时考虑疼痛和死亡率的策略与死亡率呈负相关。这表明即使短期目标是主要目标，重视长期结果对于制定更安全的治疗策略至关重要。

Conclusion: 在ICU疼痛管理的强化学习策略设计中，必须考虑长期结果（如死亡率），仅优化短期目标可能导致不良后果。同时优化疼痛和死亡率的策略能够提供更安全的治疗建议。
...
### [233] [Calibrated Multivariate Distributional Regression with Pre-Rank Regularization](https://arxiv.org/abs/2601.22895)
*Aya Laajil,Elnura Zhalieva,Naomi Desobry,Souhaib Ben Taieb*

Main category: cs.LG

TL;DR: 提出基于预秩函数的正则化校准方法，用于多变量分布回归模型训练期间实现多变量校准，并引入基于PCA的新型预秩函数
...
Result: 通过模拟研究和18个真实世界多输出回归数据集的实验表明，该方法显著改善了多变量预秩校准，且不损害预测准确性；PCA预秩能够检测现有预秩无法发现的依赖结构错误设定

Conclusion: 提出的正则化校准方法有效解决了多变量校准问题，PCA预秩为诊断依赖结构错误设定提供了新工具，为多变量概率预测提供了实用的校准框架
...
### [234] [Uncertainty-Aware Extrapolation in Bayesian Oblique Trees](https://arxiv.org/abs/2601.22899)
*Viktor Andonovikj,Sašo Džeroski,Pavle Boškoski*

Main category: cs.LG

TL;DR: 提出一种结合贝叶斯斜分割和GP叶节点的单树模型，用于提升回归任务的外推能力和不确定性校准


<details>
  <summary>Details</summary>
Motivation: 决策树在回归任务中外推能力有限，叶节点的分段常数预测受限于训练目标范围，在分布偏移下容易过度自信，需要更好的不确定性建模和外推机制

Method: 扩展VSPYCT模型，为每个叶节点配备GP预测器；使用贝叶斯斜分割进行不确定性感知的空间划分；GP叶节点建模局部函数行为；结合分割参数后验采样与GP后验预测的高效推理方案；当输入超出叶节点训练支持时激活基于GP的外推机制

Result: 在基准回归任务中相比标准变分斜树提升了预测性能，在外推场景下获得了显著的性能增益

Conclusion: 提出的贝叶斯单树模型通过GP叶节点和不确定性感知分割，有效解决了决策树在外推和不确定性校准方面的局限性，在回归任务中表现出优越性能
...
TL;DR: 该研究提出了一种系统探测大语言模型推理轨迹的协议，通过截断推理轨迹并测量其对答案选择的影响，发现准确性和决策确定性随推理token比例增加而提升，主要受相关内容而非长度或风格效应驱动。
...
Method: 提出三步协议：1)生成模型的推理轨迹；2)在固定token百分位截断；3)将每个部分轨迹注入模型（或不同模型），通过下一个token概率测量诱导的答案选择分布。应用于Qwen3和gpt-oss模型，在GPQA Diamond和MMLU-Pro基准测试中进行评估。

Result: 准确性和决策确定性随着提供的推理token比例增长而一致增加。这些增益主要由模型生成中的相关内容驱动，而非上下文长度或通用"推理风格"效应。更强的模型通常能从错误的部分轨迹成功回溯，但即时答案往往仍锚定在较弱模型的错误响应中。
...
### [236] [FlexLoRA: Entropy-Guided Flexible Low-Rank Adaptation](https://arxiv.org/abs/2601.22905)
...
<details>
  <summary>Details</summary>
Motivation: 大型预训练模型虽在各领域表现出色，但完全微调的计算和内存成本过高。参数高效微调（PEFT）成为主流范式，其中LoRA引入可训练低秩矩阵表现良好，但其固定秩设计限制了灵活性。现有动态秩分配方法依赖启发式元素级指标，缺乏矩阵级区分，且无法扩展需要额外适应层的容量。

Method: 提出FlexLoRA框架：1）通过谱能量熵评估矩阵重要性；2）在全局预算下支持秩剪枝和扩展；3）为新添加的奇异方向采用零影响初始化以确保稳定性。该方法解决了现有方法的粒度、灵活性和稳定性限制。

Result: 大量实验表明，FlexLoRA在多个基准测试中始终优于最先进的基线方法。

Conclusion: FlexLoRA通过熵引导的灵活低秩适应，为参数高效微调提供了更原则性的解决方案，克服了现有方法的局限性，在性能和效率上均表现出色。
...
Result: 实验表明DyCAST在保持竞争力的语音重合成质量和下游任务性能的同时，使用的token数量显著少于固定帧率编解码器。

Conclusion: DyCAST提供了一种高效的动态帧率语音分词方法，通过字符对齐和时长建模减少token序列长度，同时保持语音质量。
...
### [238] [DC-LA: Difference-of-Convex Langevin Algorithm](https://arxiv.org/abs/2601.22932)
*Hoang Phuc Hau Luu,Zhongjian Wang*

Main category: cs.LG

TL;DR: 论文提出DC-LA算法，用于采样目标分布π∝exp(-f-r)，其中f是Lipschitz光滑项，r=r₁-r₂是非光滑DC函数。通过Moreau包络平滑r，并将凹部分重新分配到数据保真项，建立了在q-Wasserstein距离下的收敛性。


<details>
  <summary>Details</summary>
Motivation: 研究非对数凹分布采样问题，其中正则化项r是非光滑的DC函数。现有方法在处理这类复杂正则化项时存在局限，需要开发能够有效处理DC结构正则化项的采样算法。

Method: 利用DC结构，对r₁和r₂分别应用Moreau包络平滑正则化项，将凹部分重新分配到数据保真项，提出DC-LA（近端Langevin算法）。算法在V满足远距离耗散性假设下，处理非对数凹采样问题。

Result: 在q-Wasserstein距离下建立了DC-LA到目标分布π的收敛性（考虑离散化和平滑误差），改进了先前非对数凹采样工作的框架和假设。数值实验显示DC-LA在合成设置中产生准确分布，在真实CT应用中可靠地提供不确定性量化。

Conclusion: DC-LA算法能够有效处理具有DC结构正则化项的非对数凹采样问题，在理论和实验上都表现出良好性能，为复杂正则化项的贝叶斯推断提供了实用工具。
...
### [239] [Environment-Conditioned Tail Reweighting for Total Variation Invariant Risk Minimization](https://arxiv.org/abs/2601.22944)
...
<details>
  <summary>Details</summary>
Motivation: 现有不变风险最小化方法主要关注环境级的虚假相关性，但忽略了环境内样本级的异质性，这在混合分布偏移下会严重影响OOD性能。

Method: 提出环境条件尾部重加权总变差不变风险最小化框架，将环境级不变性与环境内鲁棒性相结合，并通过极小极大公式在没有显式环境标注时推断潜在环境。

Result: 在回归、表格数据、时间序列和图像分类基准测试中，在混合分布偏移下，最差环境和平均OOD性能均获得一致提升。

Conclusion: ECTR框架通过联合处理环境级相关偏移和样本级多样性偏移，使两种机制在混合分布偏移下互补，显著提升了OOD泛化能力。
...
TL;DR: 本文解决了线性bandit中Nash遗憾的次优性问题，提出了新的分析工具实现最优Nash遗憾界，并首次研究了p-均值遗憾框架，提出了通用的FairLinBandit算法框架。


<details>
  <summary>Details</summary>
Motivation: 现有线性bandit中的Nash遗憾结果存在次优性，源于依赖限制性集中不等式。需要新的分析工具来解决这个开放问题，并扩展研究更一般的p-均值遗憾框架。

Method: 提出了新的分析工具解决Nash遗憾次优性问题，引入了p-均值遗憾框架，并提出了FairLinBandit通用算法框架，该框架可作为元算法与任何线性bandit策略结合。具体实例化了Phased Elimination和Upper Confidence Bound两种算法。

Result: 实现了线性bandit中Nash遗憾的最优界，证明了两种实例化算法在整个p值范围内都能实现亚线性p-均值遗憾。在真实数据集生成的线性bandit实例上的实验表明，方法始终优于现有最先进基线。

Conclusion: 本文解决了线性bandit中Nash遗憾的开放问题，提出了新的分析工具和更一般的p-均值遗憾框架，为公平性和效用目标提供了统一的处理方案，实验验证了方法的优越性。
...
### [241] [Agnostic Language Identification and Generation](https://arxiv.org/abs/2601.23258)
*Mikael Møller Høgsgaard,Chirag Pabbaraju*

Main category: cs.LG

TL;DR: 该研究放宽了语言识别和生成任务中的可实现性假设，提出了在更一般的"不可知"设置下的目标函数，获得了新颖的特征描述和接近紧致的收敛速率。


<details>
  <summary>Details</summary>
Motivation: 现有语言识别和生成研究通常基于强可实现性假设：输入数据来自某个未知分布，该分布必然支持给定语言集合中的某种语言。本研究旨在完全放宽这一可实现性假设，不对输入数据分布施加任何限制。

Method: 在完全不可知的设置下，提出了研究语言识别和生成的新目标函数。这种方法不假设输入数据来自特定语言集合中的某个分布，而是允许任意分布。

Result: 在这两个问题中都获得了新颖有趣的特征描述和接近紧致的收敛速率。研究结果表明，即使在完全不可知的设置下，仍然可以获得有意义的理论保证。

Conclusion: 通过完全放宽可实现性假设，本研究为语言识别和生成提供了更一般化的理论框架，获得了在不可知设置下的理论结果，扩展了现有研究的适用范围。
...
### [242] [Learnable Permutation for Structured Sparsity on Transformer Models](https://arxiv.org/abs/2601.22980)
*Zekai Li,Ji Liu,Guanchen Li,Yixing Xu,Ziqiong Liu,Xuanwu Yin,Dong Li,Emad Barsoum*

Main category: cs.LG

TL;DR: 本文提出了一种新颖的端到端可学习置换框架，通过可学习置换成本矩阵、可微分二分图匹配求解器和稀疏优化损失函数，优化Transformer模型的结构化稀疏剪枝性能。


<details>
  <summary>Details</summary>
Motivation: 结构化稀疏已成为流行的模型剪枝技术，但权重置换方法面临搜索空间指数级增长的挑战，现有方法多依赖贪心或启发式算法，限制了重排序效果。

Method: 提出端到端可学习置换框架：1) 可学习置换成本矩阵量化权重矩阵任意两个输入通道的交换成本；2) 可微分二分图匹配求解器根据成本矩阵获取最优二进制置换矩阵；3) 稀疏优化损失函数直接优化置换算子。

Result: 在视觉和语言Transformer上广泛验证，方法在结构化稀疏方面实现了最先进的置换结果。

Conclusion: 提出的可学习置换框架有效解决了大规模Transformer架构中置换搜索空间指数增长的问题，显著提升了结构化稀疏剪枝的性能。
...
### [243] [dgMARK: Decoding-Guided Watermarking for Diffusion Language Models](https://arxiv.org/abs/2601.22985)
...
<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型可以按任意顺序生成标记，虽然理想的条件预测器应对顺序不变，但实际模型对解掩码顺序表现出强烈敏感性，这为水印技术创造了新的通道。

Method: dgMARK引导解掩码顺序朝向那些高奖励候选标记满足由二进制哈希诱导的简单奇偶约束的位置，而不显式重新加权模型学习到的概率。该方法可与常见解码策略（如置信度、熵和基于边界的排序）即插即用，并可通过一步前瞻变体进行强化。
...
### [244] [Value-at-Risk Constrained Policy Optimization](https://arxiv.org/abs/2601.22993)
*Rohan Tangri,Jan-Peter Calliess*

Main category: cs.LG

TL;DR: 提出VaR-CPO算法，直接优化风险价值约束，实现安全探索，在可行环境中训练时零约束违反


<details>
  <summary>Details</summary>
Motivation: 现有基线方法无法保证训练过程中的安全探索，存在约束违反问题。需要一种能直接优化风险价值约束的样本高效且保守的方法

Method: 1) 使用单边切比雪夫不等式处理VaR约束的非可微性，基于成本回报的前两矩获得可处理的替代约束；2) 扩展约束策略优化的信任区域框架，提供策略改进和约束违反的最坏情况理论保证

Result: VaR-CPO在可行环境中能够实现零约束违反的安全探索，而基线方法无法保持这一关键特性。算法具有样本高效性
...
TL;DR: 论文提出了一种新的异常根因分析方法，将异常分为测量误差和机制变化两种类型，通过潜在干预的因果模型进行识别和分类。


<details>
  <summary>Details</summary>
Motivation: 现有异常根因分析方法忽略了异常可能来自两种根本不同的过程：测量误差（数据正常生成但记录错误）和机制变化（数据生成过程本身改变）。测量误差通常可以安全修正，而机制异常需要仔细考虑，但目前方法无法区分这两种类型。

Method: 定义了一个因果模型，将异常视为对潜在变量（"真实"变量）和观测变量（"测量"变量）的潜在干预，明确捕捉两种异常类型。证明了这两种异常类型的可识别性，并提出最大似然估计方法来实现这一理论。

Result: 实验表明，该方法在根因定位方面达到最先进性能，同时能够准确分类异常类型（测量误差vs机制变化），即使在因果DAG未知的情况下也能保持鲁棒性。

Conclusion: 通过明确建模两种不同类型的异常过程，该方法不仅改进了根因定位，还提供了异常类型的分类能力，增强了异常分析的实用性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 长思维链推理虽然能提高LLMs在数学推理等任务上的性能，但顺序生成导致高延迟问题，需要降低推理延迟

Method: 提出分治思维链方法，让模型作为导演识别可并行执行的子任务，然后生成工作线程执行；采用多阶段强化学习算法，配合数据过滤策略，在恢复准确率的同时减少最长路径长度

Result: 在AIME 2024和HMMT 2025等多个基准测试中，DC-CoT达到与DeepScaleR-1.5B-Preview相似的准确率，同时将最长路径长度降低35-40%

Conclusion: DC-CoT方法能有效降低长思维链推理的延迟，在保持准确性的同时实现并行推理，为低延迟推理提供了可行方案
...
TL;DR: 本文提出RLRR框架，将强化学习中的奖励塑造从绝对评分转向相对排名，以解决基于群体的方法在可验证任务中监督稀疏和开放场景中奖励模型不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于群体的强化学习方法（如GRPO）依赖绝对数值奖励，存在内在局限性：在可验证任务中，相同群体评估导致监督稀疏；在开放场景中，奖励模型评分范围不稳定会损害基于群体均值的优势估计。

Method: 提出RLRR框架，将奖励塑造从绝对评分转向相对排名；并引入Ranking Reward Model，这是一个专门为基于群体优化设计的列表式偏好模型，可直接生成相对排名。
...
Conclusion: SplineFlow是一种理论基础的流匹配算法，通过B样条插值有效解决了动态系统建模中的路径构建问题，在复杂动态系统建模中表现出优越性能。
...
### [249] [CATTO: Balancing Preferences and Confidence in Language Models](https://arxiv.org/abs/2601.23096)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的预测置信度常常校准不良：高置信度预测经常出错，低置信度预测反而可能正确。基于偏好的对齐方法进一步破坏了预测概率与正确性之间的联系。

Method: 提出了CATTO（Calibration Aware Token-level Training Objective），这是一种校准感知的目标函数，将预测置信度与经验预测正确性对齐，可以与原始偏好优化目标结合使用。同时还引入了Confidence@k，一种利用校准后的token概率进行贝叶斯最优输出token选择的测试时缩放机制。

Result: 与直接偏好优化（DPO）相比，CATTO将预期校准误差（ECE）降低了2.22%-7.61%（分布内）和1.46%-10.44%（分布外）；与最强的DPO基线相比，降低了0.22%-1.24%（分布内）和1.23%-5.07%（分布外）。置信度改进的同时没有牺牲任务准确性，在五个数据集上保持或略微提高了多项选择题回答的准确性。

Conclusion: CATTO有效地改善了LLM的校准问题，使模型的置信度更好地反映预测的正确性，同时保持了任务性能。该方法为解决LLM校准不良问题提供了一种有效的训练目标。
...
### [250] [Distribution-informed Efficient Conformal Prediction for Full Ranking](https://arxiv.org/abs/2601.23128)
*Wenbo Liao,Huipeng Huang,Chen Jia,Huajun Xi,Hao Zeng,Hongxin Wei*

Main category: cs.LG

TL;DR: DCR方法通过推导非一致性分数的精确分布来改进排名模型的不确定性量化，相比基线方法减少预测集大小达36%，同时保持有效覆盖率。
...
Method: 提出分布感知保形排名(DCR)，通过发现校准项的绝对排名条件于相对排名服从负超几何分布，推导非一致性分数的精确分布来确定保形阈值。

Result: DCR在保持有效覆盖率的同时，将平均预测集大小减少高达36%，理论证明在温和假设下比基线方法更高效。

Conclusion: DCR通过利用排名分布信息显著提高了保形预测在排名任务中的效率，为实际应用中排名模型的安全部署提供了更实用的不确定性量化方法。
...
### [251] [Manifold-Aware Perturbations for Constrained Generative Modeling](https://arxiv.org/abs/2601.23151)
...
<details>
  <summary>Details</summary>
Motivation: 生成模型在科学领域中经常遇到样本受等式约束的分布建模问题，存在固有的数学限制

Method: 提出约束感知的数据分布扰动方法，使新分布的支持集匹配环境空间维度，同时隐式包含底层流形几何

Result: 通过理论分析和多个代表性任务的实证证据表明，该方法能一致实现数据分布恢复和稳定采样
...
### [252] [Unsupervised Hierarchical Skill Discovery](https://arxiv.org/abs/2601.23156)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法大多依赖动作标签、奖励或人工标注，限制了在无监督场景下的应用，需要一种能够从无标签轨迹中自动发现技能层次结构的方法。

Method: 采用基于语法的方法，从未标记的轨迹中分割出技能，并诱导出层次结构，能够捕捉低层行为及其组合成高层技能的过程。

Result: 在Craftax和完整版Minecraft等高维像素环境中评估，在技能分割、重用和层次质量指标上优于现有基线，产生的层次结构更有组织性和语义意义。

Conclusion: 该方法能够发现结构化且语义有意义的技能层次，并且这些发现的层次结构能够加速和稳定下游强化学习任务的学习过程。
...
### [253] [Stochastic Linear Bandits with Parameter Noise](https://arxiv.org/abs/2601.23164)
*Daniel Ezer,Alon Peled-Cohen,Yishay Mansour*

Main category: cs.LG

TL;DR: 本文研究了带参数噪声的随机线性赌博机问题，提出了针对一般动作集和特定ℓ_p球动作集的遗憾上下界，发现参数噪声模型下的最优遗憾比经典加性噪声模型更小，且可通过简单的探索-利用算法实现。


<details>
  <summary>Details</summary>
Motivation: 研究随机线性赌博机中的参数噪声模型，探索在这种噪声结构下是否能获得比经典加性噪声模型更优的遗憾界，并设计简单有效的算法。

Method: 针对参数噪声模型，其中奖励为a^⊤θ，θ服从独立同分布。分析了一般动作集和特定ℓ_p单位球动作集（p≤2）的遗憾界，提出了简单的探索-利用算法。

Result: 对于一般动作集，获得了上界Õ(√(dT log(K/δ)σ_max^2))和下界Ω̃(d√(Tσ_max^2))；对于ℓ_p球动作集，获得了紧致的Θ̃(√(dTσ_q^2))遗憾界，其中σ_q^2≤4，显著优于经典加性噪声模型的d√T遗憾。

Conclusion: 参数噪声模型下的随机线性赌博机可以获得比经典加性噪声模型更优的遗憾界，且这种最优界可以通过简单的探索-利用算法实现，这为线性赌博机问题提供了新的理论洞见。
...
### [254] [MeshGraphNet-Transformer: Scalable Mesh-based Learned Simulation for Solid Mechanics](https://arxiv.org/abs/2601.23177)
...
<details>
  <summary>Details</summary>
Motivation: 标准MeshGraphNets在处理大规模高分辨率网格时存在长距离信息传播效率低的问题，这限制了其在工业规模应用中的效果，特别是在需要捕捉远距离物理相互作用的场景中。

Method: 提出MeshGraphNet-Transformer架构，结合物理注意力Transformer作为全局处理器，同时更新所有节点状态并保留节点和边属性。该方法直接捕捉长距离物理相互作用，无需深度消息传递堆栈或分层粗化网格。

Result: MGN-T成功处理工业规模的冲击动力学网格，准确建模自接触、塑性和多变量输出。在经典基准测试中优于现有方法，实现更高精度同时保持实用效率，仅需竞争基线方法的一小部分参数。

Conclusion: MGN-T通过结合Transformer的全局建模能力和MeshGraphNets的几何归纳偏置，有效解决了大规模高分辨率网格上的长距离信息传播问题，为工业规模应用提供了高效准确的解决方案。
...
### [255] [Tackling air quality with SAPIENS](https://arxiv.org/abs/2601.23215)
...
TL;DR: 该研究通过将交通地图转换为同心环描述，结合偏最小二乘回归，预测墨西哥城的空气污染水平，提供超本地化动态空气质量预报。


<details>
  <summary>Details</summary>
Motivation: 空气污染是全球大城市的慢性问题，交通是主要污染源。现有空气质量监测和预报在时空上较为粗糙，而实时交通数据则更为精细。研究旨在利用精细交通数据提供超本地化动态空气质量预报。

Method: 开发创新方法将简单彩色编码的交通地图转换为同心环描述，改进交通状况表征。使用偏最小二乘回归基于新定义的交通强度预测污染水平，并通过不同训练样本优化模型性能。

Result: 模型经过优化达到最佳预测性能，深入揭示了污染物与交通之间的关系。研究提供了可预测空气污染水平的有效方法。

Conclusion: 设计的工作流程简单直接，可适应其他城市环境，为超本地化动态空气质量预报提供了实用解决方案。
...
### [256] [Optimal Fair Aggregation of Crowdsourced Noisy Labels using Demographic Parity Constraints](https://arxiv.org/abs/2601.23221)
*Gabriel Singer,Samuel Gruffaz,Olivier Vo Van,Nicolas Vayatis,Argyris Kalogeratos*

Main category: cs.LG

TL;DR: 该论文研究了众包标注聚合中的公平性问题，分析了多数投票和贝叶斯最优聚合在ε-公平框架下的表现，提出了理论界限和收敛保证，并扩展了多类别公平后处理算法。


<details>
  <summary>Details</summary>
Motivation: 获取可靠的真实标签通常成本高昂或不可行，因此众包和聚合噪声人工标注是常见做法。但聚合主观标签可能放大个体偏见，特别是在敏感特征方面，引发公平性担忧。然而，众包聚合的公平性问题尚未得到充分探索，缺乏收敛保证，且现有的ε-公平性后处理方法有限。

Method: 1. 在ε-公平框架下分析多数投票和最优贝叶斯聚合的公平性；2. 在小众包规模下推导多数投票公平性差距的上界；3. 证明聚合共识的公平性差距在可解释条件下以指数速度收敛到真实标签的公平性差距；4. 将最先进的多类别公平后处理算法从连续设置扩展到离散设置，强制执行严格的人口统计均等约束。

Result: 1. 推导了多数投票公平性差距的上界；2. 证明了聚合共识公平性差距的指数收敛性；3. 扩展了多类别公平后处理算法；4. 在合成和真实数据集上的实验验证了方法的有效性并证实了理论见解。

Conclusion: 该研究填补了众包聚合公平性分析的空白，提供了理论保证和实用方法，有助于在众包标注系统中实现更公平的决策，特别是当真实标签本身可能存在偏见时。
...
### [257] [FOCUS: DLLMs Know How to Tame Their Compute Bound](https://arxiv.org/abs/2601.23278)
*Kaihua Liang,Xin Tan,An Zhong,Hong Xu,Marco Canini*

Main category: cs.LG

TL;DR: FOCUS系统通过动态聚焦计算资源于可解码token，显著提升扩散大语言模型的推理效率


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型虽然提供了自回归模型的替代方案，但其部署受到高解码成本的限制。研究发现，在扩散步骤中，只有一小部分token是可解码的，大部分计算资源浪费在不可解码的token上。

Method: 提出FOCUS推理系统，基于注意力机制得出的token重要性与其解码概率之间的强相关性，动态地将计算资源聚焦于可解码token，同时实时淘汰不可解码token，从而增加有效批处理大小。

Result: FOCUS在多个基准测试中实现了高达3.52倍的吞吐量提升（相比生产级引擎LMDeploy），同时保持或改进了生成质量。

Conclusion: FOCUS系统通过有效管理计算资源，解决了扩散大语言模型解码效率低下的问题，为实际部署提供了可行的解决方案，代码已在GitHub开源。
...
### [258] [Decoupled Diffusion Sampling for Inverse Problems on Function Spaces](https://arxiv.org/abs/2601.23280)
*Thomas Y. L. Lin,Jiachen Yao,Lufang Chiang,Julius Berner,Anima Anandkumar*

Main category: cs.LG

TL;DR: DDIS提出了一种解耦的扩散逆求解器，通过分离系数先验学习和前向PDE建模，在数据稀缺情况下实现更高效的物理感知逆问题求解。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散后验采样的方法需要大量配对监督数据来隐式建模物理关系，在数据稀缺时性能受限。需要一种数据高效且能显式利用物理知识的逆PDE求解框架。

Method: DDIS采用解耦设计：无条件扩散模型学习系数先验分布，神经算子显式建模前向PDE提供指导。提出解耦退火后验采样(DAPS)避免过平滑问题。

Result: 理论证明DDIS在训练数据稀缺时避免了联合模型的指导衰减问题。实验显示在稀疏观测下，DDIS平均提升l2误差11%、谱误差54%；数据限制到1%时，相比联合模型保持40%的l2误差优势。

Conclusion: DDIS通过解耦设计和显式物理建模，实现了数据高效的逆PDE求解，在稀疏观测和有限数据条件下显著优于现有方法。
