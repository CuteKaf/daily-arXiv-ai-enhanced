...
TL;DR: 该研究评估了通用时间序列基础模型Chronos-2在交通预测任务中的零样本性能，发现其在多个数据集上无需微调即可达到或超越专门模型的效果。


<details>
  <summary>Details</summary>
Motivation: 当前交通预测方法通常需要针对特定数据集进行训练、架构设计和超参数调优，过程繁琐且缺乏通用性。研究旨在探索通用时间序列基础模型能否作为交通预测的有效工具。

Method: 使用最先进的Chronos-2模型，在10个真实世界交通数据集上进行零样本性能基准测试，涵盖高速公路交通量、城市交通速度、共享单车需求和电动汽车充电站数据，采用一致的评估协议。

Result: Chronos-2在大多数数据集上无需任务特定微调即可达到最先进或具有竞争力的准确度，经常超越经典统计基线和专门深度学习架构，特别是在较长预测时间范围内表现更优。其概率输出也能提供有用的不确定性量化。

Conclusion: 时间序列基础模型可以作为交通预测研究的关键基准，支持采用这类通用模型来简化交通预测流程并提高预测性能。
...
TL;DR: CHAT是一种基于RNN-T的新型流式语音模型，通过分块处理和跨注意力机制，在保持实时性的同时显著提升效率和精度。


<details>
  <summary>Details</summary>
Motivation: 解决传统RNN-T模型在处理语音时严格单调对齐的局限性，特别是在语音翻译任务中表现不佳的问题，同时希望提升模型的训练和推理效率。

Method: 提出分块注意力转换器（CHAT），将音频按固定大小的块进行处理，在每个块内使用跨注意力机制，这种混合方法保持了RNN-T的流式特性，同时引入了局部对齐建模的灵活性。

Result: CHAT显著降低了RNN-T需要处理的时间维度，带来显著效率提升：训练内存峰值降低达46.2%，训练速度提升1.36倍，推理速度提升1.69倍。在精度方面，语音识别相对WER降低达6.3%，语音翻译BLEU提升达18.0%。

Conclusion: CHAT模型为部署更强大的流式语音模型提供了实用解决方案，在不牺牲实时约束的情况下，显著提升了效率和性能，特别是在语音翻译任务中效果尤为突出。
...
### [3] [Who Guards the Guardians? The Challenges of Evaluating Identifiability of Learned Representations](https://arxiv.org/abs/2602.24278)
*Shruti Joshi,Théo Saulus,Wieland Brendel,Philippe Brouillard,Dhanya Sridhar,Patrik Reizinger*

Main category: cs.LG

TL;DR: 现有表示学习可识别性评估指标存在假设条件限制，当数据生成过程或编码器几何结构不满足特定条件时，这些指标会产生误判，作者提出了分类框架和评估套件来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 当前表示学习可识别性评估依赖于标准指标（如MCC、DCI、R²），这些指标假设能够反映理论保证的等价类恢复程度。然而，作者发现这种假设只在特定结构条件下成立，当数据生成过程或编码器几何结构不满足假设时，指标会产生系统性的误判。

Method: 作者提出了一个分类框架，将数据生成过程假设与编码器几何结构假设分开，用于描述现有指标的有效域。同时发布了评估套件，用于可重复的压力测试和比较。

Result: 研究发现，每个评估指标都隐式地编码了对数据生成过程和编码器的特定假设。当这些假设被违反时，指标会变得错误指定，产生假阳性和假阴性结果。这种失败既发生在经典可识别性机制内，也发生在最需要可识别性的后处理场景中。

Conclusion: 表示学习可识别性评估需要更严谨的方法，现有指标的有效性依赖于特定结构条件。作者提出的分类框架和评估套件为系统性地测试和比较可识别性评估指标提供了工具，有助于避免误判并提高评估的可靠性。
...
### [4] [Memory Caching: RNNs with Growing Memory](https://arxiv.org/abs/2602.24281)
*Ali Behrouz,Zeman Li,Yuan Deng,Peilin Zhong,Meisam Razaviyayn,Vahab Mirrokni*

Main category: cs.LG

TL;DR: 本文提出Memory Caching技术，通过缓存RNN隐藏状态检查点来增强循环模型的记忆能力，使其记忆容量随序列长度增长，在保持线性复杂度的同时接近Transformer的召回性能。


<details>
  <summary>Details</summary>
Motivation: Transformer虽然性能强大但具有二次复杂度，而现有循环模型虽然复杂度线性但记忆容量固定，在召回密集型任务中表现不佳。需要一种既能保持线性复杂度又能增强记忆能力的方法。

Method: 提出Memory Caching技术，缓存RNN隐藏状态的检查点，包括门控聚合和稀疏选择机制等四种变体，支持线性和深度记忆模块，使有效记忆容量随序列长度增长。

Result: 在语言建模和长上下文理解任务中，MC显著提升了循环模型的性能。在上下文召回任务中，MC变体表现出与Transformer竞争的性能，缩小了与Transformer的差距，优于现有最先进的循环模型。

Conclusion: Memory Caching是一种简单有效的技术，通过缓存记忆状态检查点，使循环模型在保持线性复杂度的同时获得随序列长度增长的记忆容量，在召回密集型任务中接近Transformer的性能。
...
### [5] [Taming Momentum: Rethinking Optimizer States Through Low-Rank Approximation](https://arxiv.org/abs/2602.24283)
*Zhengbo Wang,Jian Liang,Ran He,Zilei Wang,Tieniu Tan*

Main category: cs.LG

TL;DR: LoRA-Pre是一种新型低秩优化器，通过将动量矩阵分解为紧凑的低秩子空间，显著降低优化器内存开销，在预训练和微调任务中均优于现有方法。
...
Method: 将指数移动平均(EMA)重新定义为通过在线梯度流训练线性回归器，并在此基础上引入LoRA-Pre，将完整的动量矩阵分解为在线线性学习器中的紧凑低秩子空间。

Result: 在Llama架构家族从60M到1B参数的预训练中，LoRA-Pre在所有模型规模上均取得最高性能，仅使用基线方法1/8的秩就能达到相当或更好的结果。在微调场景中，相比标准LoRA，在Llama-3.1-8B上提升3.14分，在Llama-2-7B上提升6.17分。
...
TL;DR: CUDA Agent是一个基于大规模智能体强化学习的系统，通过数据合成、技能增强的开发环境和强化学习算法，显著提升了LLM在CUDA内核优化方面的性能，超越了现有编译器和专有模型。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在CUDA内核优化方面表现不佳，无法与torch.compile等编译器系统竞争。现有方法要么依赖无训练的精炼，要么在固定的多轮执行反馈循环中进行微调，但都无法从根本上提升模型的CUDA优化能力，导致性能提升有限。

Method: CUDA Agent包含三个核心组件：1）可扩展的数据合成流水线；2）技能增强的CUDA开发环境，具有自动化验证和性能分析功能，提供可靠的奖励信号；3）实现稳定训练的强化学习算法技术。

Result: 在KernelBench基准测试中，CUDA Agent在Level-1、Level-2和Level-3三个难度级别上分别比torch.compile快100%、100%和92%。在最难的Level-3设置上，比Claude Opus 4.5和Gemini 3 Pro等最强专有模型快约40%。

Conclusion: CUDA Agent通过大规模智能体强化学习系统成功开发了CUDA内核优化专业知识，实现了在CUDA内核生成任务上的最先进性能，显著超越了现有编译器和专有模型。
