...
TL;DR: Any4D是一个可扩展的多视角transformer，用于度量尺度、密集前馈4D重建，能够处理多种模态数据，在精度和计算效率上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常专注于2视角密集场景流或稀疏3D点跟踪，而4D重建方法主要基于单目RGB视频。Any4D旨在构建一个更灵活、更全面的4D重建框架，能够整合多种传感器数据。
...
TL;DR: 该论文首次系统研究强化学习在文本到3D自回归生成中的应用，提出了AR3D-R1模型，通过奖励设计、算法优化和层次化方法解决了3D生成的挑战。


<details>
  <summary>Details</summary>
Motivation: 虽然强化学习在2D图像生成中已证明有效，但在3D生成中应用仍未被充分探索。3D对象具有更高的空间复杂性，需要全局一致的几何结构和细粒度的局部纹理，这使得3D生成对奖励设计和RL算法特别敏感。

Method: 1) 系统研究奖励设计维度，发现与人类偏好对齐至关重要；2) 研究GRPO变体，强调token级优化的有效性；3) 引入MME-3DR基准测试；4) 提出Hi-GRPO层次化RL范式，通过专用奖励集成优化全局到局部的3D生成。

Result: 基于这些研究，开发了AR3D-R1模型，这是第一个RL增强的文本到3D模型，能够从粗略形状到纹理细化进行专家级生成。

Conclusion: 该研究为RL驱动的3D生成推理提供了重要见解，希望推动3D生成领域的发展。代码已开源。
...
TL;DR: MMSI-Video-Bench是一个全面评估多模态大语言模型视频空间智能的基准测试，包含1,106个问题，涵盖感知、规划、预测和跨视频推理四个层次，揭示了当前模型与人类在空间理解方面存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 空间理解对于MLLMs在物理环境中成为通用助手至关重要，但目前缺乏全面评估这一能力的基准测试。作者旨在填补这一空白，为视频空间智能提供系统的评估框架。

Method: 构建了一个完全人工标注的基准测试，采用四层框架（感知、规划、预测、跨视频推理），包含1,106个问题，基于25个数据集和内部视频的1,278个片段。每个问题由3DV专家精心设计和评审，确保精确无歧义。

Result: 评估了25个开源和专有MLLMs，发现显著的人机差距：许多模型表现接近随机水平，最佳推理模型落后人类近60%。空间微调模型在基准测试上泛化能力仍然不足，存在几何推理、运动基础、长时预测和跨视频对应等系统性失败。

Conclusion: MMSI-Video-Bench为推进视频空间智能建立了坚实的测试平台，揭示了当前MLLMs在空间理解方面的局限性，并发现典型帧采样策略、3D空间线索和思维链提示在推理密集型任务中效果有限。
...
<details>
  <summary>Details</summary>
Motivation: 早期儿童发展轨迹为视觉基础模型的高效预训练提供了自然目标，需要开发符合儿童认知发展规律的视觉语言模型。

Method: 1) 构建纵向、多方面的婴儿中心视听语料预训练集；2) 开发DevCV工具箱，将NIH婴儿工具箱的视觉相关测量转化为10个多模态任务基准；3) 使用紧凑模型从头开始预训练。

Result: 紧凑模型在DevCV工具箱上表现出色，在某些任务上超越了GPT-4o，证明了发展性预训练的有效性。

Conclusion: BabyVLM-V2为发展性合理的视觉基础模型预训练提供了原则性统一框架，有望加速相关研究。
...
TL;DR: OmniView是一个统一的4D一致性框架，通过分离空间、时间和视角条件，能够处理多种4D任务，包括新颖视角合成、文本/图像到视频生成等，在多个基准测试中表现优于任务专用模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法专注于特定的4D一致性任务子集（如新颖视角合成、文本到视频等），导致训练数据分散且方法碎片化。需要一种统一的框架来泛化处理广泛的4D任务。

Method: OmniView分别表示空间、时间和视角条件，使这些输入能够灵活组合。该方法能够从静态、动态和多视角输入合成新颖视角，在时间上向前向后外推轨迹，以及从文本或图像提示创建具有完整相机控制的视频。

Result: 在多个基准测试中与任务专用模型竞争，在LLFF多视角NVS数据集上图像质量评分提升33%，在Neural 3D Video动态NVS基准上提升60%，在RE-10K静态相机控制上提升20%，在文本条件视频生成中相机轨迹误差减少4倍。

Conclusion: OmniView展示了通用4D视频模型的可行性，通过单一模型实现强大的泛化能力，为统一的4D处理框架提供了有效解决方案。
...
TL;DR: Mull-Tokens是一种模态无关的潜在令牌，允许模型在文本和图像模态之间自由思考，以解决多模态推理问题，在空间推理基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界推理需要超越语言的多种模态（空间、时间、功能等），现有多模态模型依赖专业工具、图像生成或手工制作的数据，存在脆弱性和扩展性问题。

Method: 提出Mull-Tokens（模态无关潜在令牌），预训练用于在文本或图像模态中保存中间信息。首先使用交错文本-图像轨迹进行监督训练，然后仅使用最终答案进行无监督微调。

Result: 在四个具有挑战性的空间推理基准测试中（包括解谜和视角转换任务），Mull-Tokens优于仅使用文本推理或交错图像-文本推理的基线方法，平均提升3%，在推理密集型解谜任务上提升高达16%。

Conclusion: Mull-Tokens为多模态抽象推理提供了一个简单解决方案，有助于解决文本和视觉推理的落地挑战，让模型能够在多种模态中自由思考。
...
TL;DR: AlcheMinT是一个统一的主题驱动视频生成框架，通过引入显式时间戳条件，实现了对主题出现和消失的细粒度时间控制，解决了现有方法缺乏时间控制的问题。
...
Method: 提出AlcheMinT框架，引入新颖的位置编码机制来编码与主题身份相关的时间间隔，同时与预训练视频生成模型的位置嵌入无缝集成。还加入主题描述性文本标记来加强视觉身份与视频描述之间的绑定。

Result: 实验结果表明，AlcheMinT在视觉质量上与最先进的视频个性化方法相当，同时首次实现了视频内多主题生成的精确时间控制。

Conclusion: AlcheMinT通过引入显式时间戳条件，为多主题视频生成提供了精确的时间控制能力，在保持视觉质量的同时解决了现有方法的局限性。
...
TL;DR: SceneMaker是一个解耦的3D场景生成框架，通过分离去遮挡模型和3D物体生成，并改进姿态估计模型，解决了现有方法在严重遮挡和开放集场景下同时生成高质量几何和准确姿态的难题。
...
Method: 1. 将去遮挡模型与3D物体生成解耦，利用图像数据集和收集的去遮挡数据集增强模型对多样化开放集遮挡模式的处理能力。2. 提出统一姿态估计模型，整合全局和局部机制的自注意力和交叉注意力以提高准确性。3. 构建开放集3D场景数据集以扩展姿态估计模型的泛化能力。

Result: 综合实验表明，该解耦框架在室内和开放集场景上均表现出优越性能。

Conclusion: SceneMaker通过解耦去遮挡和物体生成、改进姿态估计模型以及构建开放集数据集，有效解决了严重遮挡和开放集场景下的3D场景生成问题，代码和数据集已开源。
...
TL;DR: 使用计算文本分析研究《霍比特人》对话的情感基调，发现对话保持积极平静的基调，随着故事发展逐渐增强能动性，反映了小说紧张与安慰交替的情感节奏。
...
Result: 对话整体保持积极（高效价）和冷静（低唤醒度）的基调，随着故事发展逐渐增强能动性（支配度）。情感轨迹图和词云可视化显示托尔金的语言在紧张与安慰之间循环。

Conclusion: 计算工具与文学解读相结合可以揭示文学作品中的微妙情感结构，展现了《霍比特人》中塑造叙事的稳定节奏和情感调节机制。
...
TL;DR: 评估多模态大语言模型在视频情感分析中的表现，发现在理想条件下表现良好，但在真实议会辩论场景中效果不佳


<details>
  <summary>Details</summary>
Motivation: 情感在政治沟通中至关重要，随着研究越来越多地利用视听材料分析情感表达，多模态生成AI显示出巨大潜力，但缺乏关于其在情感分析中有效性的证据

Method: 使用两个互补的人类标注视频数据集，评估当前多模态大语言模型在视频情感唤起分析中的表现

Result: 在理想条件下，mLLMs的情感唤起评分高度可靠且几乎没有人口统计学偏见；但在真实议会辩论录音中，其情感唤起评分未能达到预期效果，可能对下游统计推断产生负面影响

Conclusion: 强调需要对新兴生成AI方法在政治分析中进行持续、彻底的评估，并提供了一个可复制的评估框架
...
TL;DR: 本文提出基于PPO算法的深度强化学习方法，用于水下机器人BlueROV2的自主导航，在复杂障碍环境中优于传统DWA方法，并验证了从仿真到实物的迁移能力。


<details>
  <summary>Details</summary>
Motivation: 水下环境自主导航面临GPS缺失、能见度差、水下障碍物等挑战，需要开发能够在复杂环境中有效导航的解决方案。

Method: 采用基于PPO算法的深度强化学习方法，结合目标导向导航信息、虚拟占用网格和操作区域边界的射线投射作为观测空间，与DWA基准方法进行比较。

Result: PPO策略在高度杂乱环境中始终优于DWA方法，表现出更好的局部适应性和更少的碰撞，成功实现了从仿真到物理BlueROV2的迁移验证。

Conclusion: 深度强化学习在水下机器人自主导航中具有实际应用价值，PPO方法在复杂障碍环境中表现出色，仿真到实物的迁移能力得到验证。
...
TL;DR: 提出一种新算法，将评论家的块长度与策略的块长度解耦，通过优化策略对抗蒸馏评论家来保留多步价值传播优势，同时避免长动作块策略学习的困难。


<details>
  <summary>Details</summary>
Motivation: 时间差分方法通过自举机制高效学习状态和动作价值，但容易产生自举偏差。现有块化评论家方法估计短动作序列的价值，但提取策略时需要输出整个动作块，这在需要策略反应性的环境中可能次优，且随着块长度增长学习难度增加。

Method: 提出将评论家的块长度与策略的块长度解耦，允许策略在较短的动作块上操作。通过优化策略对抗蒸馏评论家来近似部分动作块扩展为完整块时可达到的最大价值，保留多步价值传播优势。

Result: 在具有挑战性的长时域离线目标条件任务上评估，该方法可靠地优于先前方法。

Conclusion: 该方法成功解决了块化评论家中策略提取的挑战，既保留了多步价值传播的优势，又避免了长动作块策略学习的困难，在复杂任务中表现出优越性能。
...
TL;DR: 该研究提出了一种无需额外训练的方法，使具备推理能力的LLM能够像人类一样异步地思考、倾听和生成输出，显著减少实时交互延迟。


<details>
  <summary>Details</summary>
Motivation: 现有LLM需要先完成思考才能给出答案，这种顺序交互方式不适合需要实时响应和适应额外信息的实际应用场景（如语音助手）。人类能够异步地倾听、思考和行动，而当前LLM缺乏这种能力。

Method: 利用旋转嵌入（rotary embeddings）的特性，使原本设计用于顺序交互的LLM能够同时进行思考、倾听和生成输出，无需额外训练。

Result: 在数学推理、常识推理和安全推理任务上评估，该方法能够在实时环境中生成准确的思考增强答案，将第一个非思考token的生成时间从几分钟减少到≤5秒，整体实时延迟降低了6-11倍。

Conclusion: 通过利用旋转嵌入的特性，可以使LLM实现异步思考、倾听和生成的能力，显著提升实时交互性能，为语音助手等需要实时响应的应用场景提供了可行的解决方案。
...
TL;DR: 该论文提出了Derf函数作为归一化层的替代方案，通过大规模搜索发现erf函数在多个领域超越传统归一化方法


<details>
  <summary>Details</summary>
Motivation: 虽然归一化层长期以来被视为深度学习架构的必备组件，但最近提出的Dynamic Tanh (DyT)表明存在替代方案。本研究旨在寻找能够超越DyT性能的函数设计，探索点函数的内在特性如何影响训练和性能。

Method: 首先研究点函数的内在特性对训练和性能的影响，然后基于这些发现进行大规模搜索以寻找更有效的函数设计。最终引入了Derf函数：Derf(x) = erf(αx + s)，其中erf(x)是重新缩放的高斯累积分布函数。
...
TL;DR: 线性模型（特别是DLinear）在仅使用历史室内温度数据的长期预测任务中，表现优于更复杂的Transformer架构


<details>
  <summary>Details</summary>
Motivation: 研究在仅使用历史温度数据的挑战性单变量设置下，不同模型在长期温度预测中的表现，比较线性模型与Transformer家族模型的性能差异

Method: 使用线性模型（Linear, NLinear, DLinear）和Transformer家族模型（Transformer, Informer, Autoformer），在标准化的训练、验证和测试分割下进行长期外生温度预测评估

Result: 线性基线模型（Linear, NLinear, DLinear）一致性地优于更复杂的Transformer架构，其中DLinear在所有分割中实现了最佳整体准确性

Conclusion: 精心设计的线性模型在仅使用外生变量的挑战性时间序列预测场景中仍然是强大的基线方法，复杂模型不一定优于简单模型
...
TL;DR: 提出GTL方法，无需微调预训练离散扩散模型即可实现跨域迁移学习，通过引导采样和高效采样器解决大规模词汇和长序列的计算问题
...
Method: 基于比率迁移学习提出GTL方法，通过引导采样从目标分布生成样本而不修改预训练去噪器。方法适用于离散时间和连续时间离散扩散，并提出高效采样器，通过规划选择位置和候选词来降低计算成本

Result: GTL在序列数据（包括合成马尔可夫链和语言建模）上进行了评估，证明了其有效性。高效采样器使大规模词汇和长序列的引导语言建模变得实用

Conclusion: GTL为离散扩散模型提供了一种无需微调的迁移学习方法，通过引导采样和高效采样器解决了计算效率问题，使跨域适应更加实用
...
TL;DR: 提出一种针对大语言模型的高效剪枝方法，通过行级等稀疏约束和Gram矩阵计算最优1-swap交换，显著降低剪枝误差


<details>
  <summary>Details</summary>
Motivation: 大语言模型剪枝面临计算挑战：完全重训练成本过高，传统全局幅度剪枝对Transformer架构效果不佳，现有方法依赖近似或启发式算法，无法获得最优解

Method: 1. 采用行级等稀疏约束解耦行间依赖；2. 利用校准数据的Gram矩阵高效计算最优1-swap交换；3. 提出简单高效的1-swap算法，可从任意剪枝掩码开始，在GPU上高效运行

Result: 相比Wanda方法减少层级剪枝误差达60%，在多种GPT架构上持续改善困惑度和零样本准确率

Conclusion: 通过行级约束和Gram矩阵优化，大幅提升大语言模型剪枝问题的可解性，实现高效且超参数自由的剪枝算法
...
TL;DR: DaSH方法通过分层建模数据集和组的效用，在资源约束下选择整个数据集来提升下游性能，相比现有方法准确率提升最高达26.2%


<details>
  <summary>Details</summary>
Motivation: 现实世界中数据通常以离散数据集形式组织，不同数据集在相关性、质量和效用上存在差异。现有方法主要选择单个样本且将所有数据视为同等相关，忽略了数据集及其来源之间的差异，因此需要一种能够选择整个数据集的方法来优化下游性能。

Method: 提出了DaSH（Dataset Selection via Hierarchies）方法，该方法在数据集和组（如集合、机构）两个层次上建模效用，通过分层建模实现从有限观察中的高效泛化，支持在资源约束下进行可扩展和自适应的数据集选择。

Result: 在两个公共基准测试（Digit-Five和DomainNet）上，DaSH相比最先进的数据选择基线方法在准确率上提升了最高达26.2%，同时需要显著更少的探索步骤。消融实验表明DaSH在低资源设置和相关数据集缺乏的情况下具有鲁棒性。

Conclusion: DaSH方法通过分层建模数据集和组的效用，为实际多源学习工作流中的可扩展和自适应数据集选择提供了有效的解决方案，能够在资源约束下显著提升下游性能。
...
TL;DR: BiFlow框架通过移除精确解析逆映射的需求，使用近似逆模型实现更灵活的架构和损失函数，在ImageNet上相比因果解码方法提升了生成质量并加速采样达两个数量级。


<details>
  <summary>Details</summary>
Motivation: 传统归一化流需要精确的解析逆映射，限制了架构灵活性。TARFlow等基于Transformer的自回归流方法虽然复兴了NF范式，但因果解码成为主要瓶颈。需要一种既能保持NF原理性又能突破因果解码限制的方法。

Method: 提出双向归一化流(BiFlow)框架，学习一个近似噪声到数据逆映射的逆向模型，而不是要求精确的解析逆。这使得可以使用更灵活的损失函数和架构设计。

Result: 在ImageNet上的实验表明，BiFlow相比因果解码对应方法，在提升生成质量的同时，采样速度加快了最多两个数量级。在基于NF的方法中达到最先进水平，在单评估方法中具有竞争力。

Conclusion: BiFlow通过移除精确解析逆映射的需求，为归一化流提供了更灵活的框架，有望进一步推动这一经典范式的发展。
...
TL;DR: LLMs可显著改善大型用户设施的提案评审流程，提供比传统人工评审更高效、一致且成本更低的替代方案，尤其在成对偏好排名和提案相似性分析方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统人工提案评审存在提案间相关性弱、评审者偏见和不一致等问题，而成对偏好方法虽然逻辑上更优，但工作量呈二次方增长，对人类评审者不切实际。需要寻找更高效、一致的评审方法。

Method: 使用大型语言模型（LLMs）进行提案评审，利用橡树岭国家实验室散裂中子源三个束线的精心策划提案和发表记录，采用成对偏好方法进行提案排名，并通过嵌入模型进行提案相似性定量评估。

Result: LLM排名与人工排名高度相关（Spearman ρ≈0.2-0.8，去除10%异常值后≥0.5）；在识别高发表潜力提案方面不亚于人类评审者；成本降低两个数量级以上；还能进行人类难以完成的提案相似性分析。

Conclusion: LLMs为大型用户设施的提案选择提供了可扩展、一致且经济高效的解决方案，不仅能替代传统人工评审，还能实现更高级的分析功能，为评审委员会提供关键信息。
...
TL;DR: 提出节点级剪枝框架用于电路发现，解决现有方法计算成本高、粒度粗的问题，通过多粒度可学习掩码实现单次优化，显著减少节点数量并降低内存占用
...
Result: 方法发现的电路节点数量少于先前方法，证明粗粒度方法认为重要的许多神经元实际上无关紧要，同时保持任务性能；内存占用显著降低5-10倍，无需在内存中保存中间激活

Conclusion: 提出的节点级剪枝框架解决了电路发现中的可扩展性和粒度限制问题，能够识别更精细的电路结构，同时大幅降低计算和内存需求
