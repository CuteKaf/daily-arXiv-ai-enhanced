...
TL;DR: SITH是一个无需数据、无需训练的可解释性框架，直接分析CLIP视觉Transformer的权重空间，通过奇异值分解和COMP算法解释注意力头的语义功能，支持精确的权重空间模型编辑。


<details>
  <summary>Details</summary>
Motivation: 现有基于激活的可解释性方法存在数据集依赖、易受数据偏见影响、解释粒度粗（仅到头级别）等问题，需要更直接、更细粒度的模型内部机制分析方法。

Method: 提出SITH框架：1) 对每个注意力头的值输出矩阵进行奇异值分解；2) 使用COMP算法将奇异向量解释为稀疏、语义连贯的人类可理解概念组合；3) 实现无需数据、无需训练的权重空间分析。

Result: SITH产生连贯、忠实的内头解释，通过重建保真度和可解释性实验验证。支持精确的权重空间模型编辑，能够放大或抑制特定概念，无需重新训练即可提升下游性能。还揭示了微调主要通过重新加权稳定的语义基而非学习全新特征。

Conclusion: SITH为视觉语言模型提供了数据无关、训练无关的细粒度可解释性框架，支持精确的模型编辑和模型适应机制分析，为理解Transformer内部机制提供了新工具。
...
TL;DR: ReDiPrune是一种无需训练、即插即用的视觉令牌剪枝方法，在视觉-语言投影器之前直接选择信息丰富的视觉令牌，显著降低计算成本同时提升多模态大语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型计算成本高昂，因为Transformer需要处理大量视觉令牌。现有后投影剪枝方法在压缩表示上操作，会丢失细粒度空间和语义信息。

Method: 在视觉编码器输出后、投影器之前应用训练自由的令牌剪枝。使用轻量级规则对每个令牌评分，综合考虑文本条件相关性和最大-最小多样性，确保选择的令牌既与查询相关又非冗余。

Result: 在4个视频和5个图像基准测试中，ReDiPrune持续改善准确率-效率权衡。例如，在EgoSchema上使用LLaVA-NeXT-Video-7B模型，仅保留15%视觉令牌可获得+2.0%绝对准确率提升，同时将计算量减少6倍以上。

Conclusion: ReDiPrune是一种高效、无需训练的多模态令牌剪枝方法，通过在投影前直接选择信息丰富的视觉令牌，显著降低计算成本同时提升模型性能，具有即插即用的优势。
...
Method: 提出双向相关图域适应框架，包括：1) 通过虚拟域桥接进行知识迁移，利用双向相关图合成图像并采用固定比例和渐进动态MixUp策略；2) 使用原型对齐和伪标签校正缓解确认偏差，通过可学习原型余弦相似度分类器实现双向原型对齐

Result: 在三个公开多域数据集上的实验表明该方法具有优越性，特别是在标注样本非常有限的情况下仍能表现出色

Conclusion: BCMDA框架有效解决了混合域半监督医学图像分割中的域偏移和确认偏差问题，在有限标注条件下实现了优异的性能
...
TL;DR: 本文提出了LLaVA-LE，一个专门用于月球表面和次表面特征分析的多模态视觉语言模型，通过构建大规模月球数据集LUCID并进行两阶段训练，显著提升了月球地形分析的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态视觉语言模型在视觉和文本联合推理方面取得了进展，但在行星科学领域的应用仍很有限，主要障碍是缺乏将真实行星图像与详细科学描述配对的大规模数据集。

Method: 1. 构建LUCID数据集：包含96k高分辨率全色图像配对的详细地形描述，以及81k个从约20k图像中提取的问答对；2. 两阶段训练：第一阶段进行领域特定地形描述的概念对齐，第二阶段进行指令调整的视觉问答训练。

Result: LLaVA-LE相比基础LLaVA模型实现了3.3倍的整体性能提升，相比第一阶段模型提升2.1倍，推理得分达到1.070，超过了评判者自身的参考分数，证明了领域特定多模态数据和指令调整的有效性。

Conclusion: 通过构建大规模月球多模态数据集和专门的两阶段训练方法，LLaVA-LE显著提升了月球地形分析能力，展示了领域特定多模态数据和指令调整在推进行星探索视觉语言模型方面的重要性。
...
TL;DR: 提出室内场景中相似物体检测任务，利用多视角图像识别相同、相似和不同的物体对，通过3DTwins数据集和Lookalike3D模型显著提升检测性能。
...
Result: 在3DTwins数据集上，方法相比基线提升了104%的IoU；能够改进下游任务，如联合3D物体重建和部件协同分割，将重复和相似物体转化为高质量3D感知的强大线索。

Conclusion: 室内场景中的相似物体检测是一个重要任务，Lookalike3D方法通过利用多视角图像和语义先验有效识别物体相似性，显著提升3D感知质量，为一致的3D理解提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在实时渲染方面表现出色，但其在精确几何测量方面的潜力尚未充分利用。传统立体测量需要专业工作站和操作员技能，而基于网格的直接测量往往不完整且不准确。

Method: 利用3DGS能够精确渲染源视图并平滑插值中间视图的特性，让用户在不同视图中直观选取共轭点，然后通过三角测量生成精确的3D点测量。支持多视图交会以提高测量精度。

Result: 在无人机航拍数据集上，该方法实现了1-2厘米范围的RMSE精度，显著优于直接网格测量。在薄结构上RMSE为0.037米（网格为0.062米），在尖锐角落处RMSE为0.013米（网格方法完全失败）。

Conclusion: 3DGS不仅可用于高质量视图合成，还能实现高精度几何测量，无需专业立体工作站，在标准硬件上即可达到或超越传统立体测量方法的精度。
...
TL;DR: 提出QuatRoPE位置编码方法，通过线性复杂度编码3D空间关系，结合IGRE机制保护LLM原有能力，提升空间推理性能


<details>
  <summary>Details</summary>
Motivation: 3D场景-语言配对数据有限，难以从头训练强推理模型。现有方法存在两个问题：1）编码绝对位置的方法难以从过早融合的特征中提取空间关系；2）显式编码所有空间关系（二次复杂度）的方法可扩展性差

Method: 提出QuatRoPE位置编码方法，输入长度与物体数量呈线性关系，在注意力层通过点积显式计算成对空间关系。同时引入IGRE机制，将QuatRoPE的影响限制在物体相关token上，减少对LLM原有位置编码的干扰

Result: 实验证明该方法有效，代码和数据已开源

Conclusion: QuatRoPE通过线性复杂度编码3D空间关系，结合IGRE保护LLM原有能力，为空间推理任务提供了高效可扩展的解决方案
...
TL;DR: 该论文提出了一种用于3D高斯泼溅的自监督置信度框架，通过可学习的置信值动态平衡光度与几何监督，结合改进的外观模型，实现了无边界网格的高效高质量提取。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在提取具有丰富视点依赖效果的场景网格时存在困难，多视图技术、迭代网格提取或大型预训练模型等方法牺牲了3DGS固有的效率。需要一种简单高效的替代方案来解决这些模糊性问题。

Method: 1. 引入自监督置信度框架，通过可学习的置信值动态平衡光度与几何监督；2. 扩展置信度驱动公式，引入惩罚每个基元颜色和法线方差的损失函数；3. 改进外观模型，解耦D-SSIM损失的各个项。

Result: 该方法在无边界网格提取方面取得了最先进的结果，同时保持了高效率，解决了具有视点依赖效果的场景中的网格提取困难问题。

Conclusion: 提出的自监督置信度框架为3D高斯泼溅提供了一种简单高效的解决方案，能够在保持效率的同时显著提升网格提取质量，特别是在具有视点依赖效果的复杂场景中。
...
<details>
  <summary>Details</summary>
Motivation: 经典的鸭兔错觉表明，当视觉证据模糊时，人脑必须决定看到什么。研究旨在探索人类观察者和机器学习分类器在概念边界上的差异，以及如何利用受控模糊作为诊断工具来弥合人类心理物理分析、图像分类和生成图像模型之间的差距。

Method: 提出一个心理物理学框架，在CLIP嵌入空间中对概念进行插值，生成连续的模糊图像谱。使用这个框架精确测量人类和机器学习分类器如何放置语义边界，并比较它们的差异。

Result: 研究发现机器学习分类器更倾向于看到"兔子"，而人类更接近用于合成的CLIP嵌入。引导尺度对人类敏感性的影响比对机器学习分类器更强。

Conclusion: 受控模糊可以作为诊断工具，弥合人类心理物理分析、图像分类和生成图像模型之间的差距，为人类-模型对齐、鲁棒性、模型可解释性和图像合成方法提供见解。
...
TL;DR: OpenCap Monocular：基于单部智能手机视频的3D骨骼运动学和动力学估计算法，可实现免费、可访问的生物力学评估
...
Result: 相比标记式运动捕捉和测力板数据，OpenCap Monocular在运动学误差方面表现优异（旋转自由度平均绝对误差4.8°，骨盆平移3.4 cm），在动力学估计方面与之前的两相机系统相当或更好。

Conclusion: OpenCap Monocular能够以临床意义精度估计重要动力学结果，已通过智能手机应用、网页应用和安全云计算部署，实现免费、可访问的单智能手机生物力学评估。
...
TL;DR: TIGeR：一个多模态transformer模型，将图像、地理位置和时间映射到统一的时空嵌入空间，支持地理定位、拍摄时间预测和时空感知检索，在多个任务上显著超越现有方法。
...
Method: 提出TIGeR模型，基于多模态transformer架构，将图像、地理位置和时间映射到统一的时空嵌入空间。支持灵活的输入配置（单模态和多模态查询），使用相同表示执行地理定位、拍摄时间预测和时空感知检索。

Result: TIGeR在多个任务上显著超越强基线方法和现有最佳方法：时间预测（季节）提升16%，时间预测（日间）提升8%，时空感知检索召回率提升14%。模型能更好地保持位置身份信息，即使在外观发生大变化时。

Conclusion: 统一的时空建模方法能有效支持复杂的地理时空感知检索任务，TIGeR通过将图像、位置和时间映射到统一嵌入空间，实现了基于场景时空特性而非单纯视觉相似性的检索。
...
TL;DR: 这篇综述比较了合成心脏MRI生成方法，重点关注生成保真度、下游实用性和隐私保护，强调需要集成评估框架来支持临床应用。


<details>
  <summary>Details</summary>
Motivation: 解决心脏MRI数据稀缺、标注不足、厂商差异和隐私泄露风险等问题，通过合成数据生成来支持医学影像分析。

Method: 比较GANs、VAEs、扩散概率模型和流匹配技术，分析掩码条件生成、厂商风格条件化、强度归一化等预处理方法，以及成员推理攻击、最近邻分析和差分隐私等隐私保护机制。

Result: 解剖约束的合成数据能够提高多厂商设置下的分割准确性和鲁棒性，但需要综合评估框架来确保临床工作流程的可靠性。

Conclusion: 需要集成保真度、实用性和隐私保护的综合评估框架，以推动合成心脏MRI在临床工作流程中的可靠应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在从极端新视角重建动态场景时表现不佳，特别是存在高度关节运动时。现有方法虽然尝试从2D基础模型提取先验并施加手工正则化，但未能充分利用动态对象的静态预扫描信息。

Method: DRoPS的核心创新包括：1）建立网格结构和表面对齐模型，将高斯原语组织成锚定在物体表面的像素网格；2）利用原语的网格结构，使用CNN参数化运动，该CNN以这些网格为条件，注入强隐式正则化并关联邻近点的运动。

Result: 大量实验表明，DRoPS在渲染质量和3D跟踪精度方面显著优于当前最先进的方法，特别是在极端新视角和高度关节运动场景下表现优异。

Conclusion: DRoPS通过有效利用静态预扫描作为几何和外观先验，结合创新的网格结构和CNN运动参数化方法，成功约束了解空间并确保整个序列的几何一致性，为动态场景重建提供了更优的解决方案。
...
TL;DR: AVControl是一个轻量级可扩展框架，基于LTX-2音频视觉基础模型，通过LoRA适配器实现多模态控制，无需架构修改，在深度、姿态、相机轨迹等控制任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有视频和音频生成控制方法存在局限性：要么训练单一固定控制的庞大模型，要么为每个新模态引入昂贵的架构修改。需要一种轻量、可扩展的框架来支持多样化的控制模态。

Method: 基于LTX-2联合音频视觉基础模型，每个控制模态作为独立的LoRA适配器在并行画布上训练，将参考信号作为注意力层的额外token，无需修改基础模型架构。

Result: 在VACE基准测试中，在深度和姿态引导生成、修复、外扩等任务上优于所有基线，在相机控制和音频视觉基准上获得竞争性结果。支持多种独立训练模态，计算和数据效率高。

Conclusion: AVControl提供了一个轻量级、可扩展的框架，通过LoRA适配器实现多模态控制，解决了现有方法的局限性，在保持高效的同时支持多样化的控制需求。
...
TL;DR: 提出Calibri方法，通过引入单个学习缩放参数和进化算法校准DiT组件，仅修改约100个参数即可提升文本到图像生成质量并减少推理步骤


<details>
  <summary>Details</summary>
Motivation: 挖掘扩散变换器（DiTs）在生成任务中的隐藏潜力，发现通过适当的校准可以显著提升DiT块的性能
...
Conclusion: Calibri是一种参数高效的方法，通过最优校准DiT组件来提升生成质量，证明了DiT校准的重要性和进化算法在解决此类优化问题中的有效性
...
TL;DR: 提出一种基于可解释AI指导的编码器塑形框架，通过生成密集的注意力场来引导模型关注薄壁、低对比度的腹主动脉瘤结构，显著提升复杂场景下的分割性能。


<details>
  <summary>Details</summary>
Motivation: 针对复杂腹主动脉瘤CT图像分割中模型容易关注无关结构或忽略薄壁、低对比度目标的问题，需要一种能够显式引导模型关注相关区域的方法。

Method: 提出XAI指导的编码器塑形框架：1）从最终编码块计算密集的注意力场（XAI场）；2）将预测概率质量与XAI场对齐以促进关注与输出的一致性；3）将XAI场输入轻量级细化路径和置信度先验，在推理时调节logits以抑制干扰同时保留细微结构。

Result: 在临床验证的具有挑战性的失败场景案例上评估，相比基础SAM设置，该方法带来了显著改进，表明通过XAI指导显式优化编码器关注是复杂场景下可靠分割的实用有效原则。

Conclusion: 将注意力指导整合到表示和解码过程中，通过显式优化编码器关注来提升复杂腹主动脉瘤分割的可靠性，为医学图像分割提供了新的有效方法。
...
TL;DR: GoldiCLIP是一个基于"恰到好处"原则构建的视觉语言模型训练框架，仅用3000万图像（比主流方法少300倍）就能实现最先进的数据效率，在多种检索任务上显著超越基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前大规模视觉语言模型主要依赖数十亿样本数据集，这成为研究进展的重要障碍。虽然近期工作开始通过提升监督质量来缩小差距，但每个方法只解决了对比预训练中的部分弱点。

Method: GoldiCLIP框架包含三个关键创新：(1) 文本条件自蒸馏方法，对齐文本无关和文本条件特征；(2) 集成解码器的编码器，通过视觉问答目标使编码器超越标题式查询；(3) 基于不确定性的加权机制，自动平衡所有异构损失。

Result: 仅用3000万图像训练，GoldiCLIP在数据高效方法中达到最先进水平：MSCOCO检索提升2.2分，细粒度检索提升2.0分，基于问题的检索提升5.9分，同时与数十亿规模模型保持竞争力。

Conclusion: GoldiCLIP通过平衡多种监督信号，显著降低了视觉语言模型对大规模数据的依赖，为数据高效训练提供了有效框架。
...
TL;DR: 该论文开发了一种基于深度学习的针道感染分类方法，通过注意力机制和高效冗余重建卷积技术，仅基于视觉特征就能准确区分感染和非感染针道图像。
...
Method: 收集并创建了针道伤口感染数据集，提出基于注意力的深度学习模型，强调相关区域并减少针体干扰。引入高效冗余重建卷积（ERRC）方法增强特征图丰富性同时减少参数数量。

Result: 模型在AUC达到0.975，F1分数0.927，仅需5.77M参数，优于基线方法。能够基于视觉感染征象准确区分感染和非感染针道，与医疗专业人员评估一致。
...
<details>
  <summary>Details</summary>
Motivation: 当前人群计数和定位领域主要使用密度图和点回归两种范式，虽然已有研究探索密度图模型间的对抗攻击可迁移性，但跨范式攻击（同时攻击两种不同架构）尚未被研究。考虑到该领域的安全影响，需要探索更全面的对抗攻击方法。

Method: 提出一个综合多任务损失优化框架：对点回归模型采用场景密度特定的高置信度logit抑制；对密度图方法采用峰值目标密度图抑制；同时结合模型无关的感知约束，确保扰动既有效又对人眼不可察觉。

Result: 实验表明该方法有效，平均使平均绝对误差增加7倍，同时保持竞争性视觉质量，成功迁移到7个最先进的人群模型，迁移比在0.55到1.69之间。在攻击效果和不可察觉性之间取得良好平衡。

Conclusion: 该研究首次探索了人群计数和定位领域的跨范式对抗攻击，提出的框架能够同时有效攻击密度图和点回归两种主流架构，为模型安全评估提供了新视角。
...
TL;DR: DCARL提出了一种分治自回归框架，结合分治方案的结构稳定性和视频扩散模型的高保真生成能力，用于生成长轨迹视频（最长32秒）。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在生成长轨迹视频时存在可扩展性限制，而自回归模型虽然能无限扩展，但存在视觉漂移和可控性差的问题。

Method: 采用分治自回归框架：1) 使用无时间压缩训练的关键帧生成器建立长距离全局一致的结构锚点；2) 插值生成器以自回归方式合成密集帧，利用关键帧提供全局上下文，单个干净的前一帧确保局部连贯性。

Result: 在大规模互联网长轨迹视频数据集上训练，在视觉质量（更低的FID和FVD）和相机一致性（更低的ATE和ARE）方面优于最先进的自回归和分治基线方法。

Conclusion: DCARL框架能够稳定、高保真地生成长达32秒的长轨迹视频，有效解决了长轨迹视频生成中的视觉漂移和可控性问题。
...
TL;DR: 该研究对20个前沿多模态大语言模型在神经影像分析任务上进行了全面基准测试，评估了它们在MRI和CT图像上的诊断性能、结构化输出能力及计算效率。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在图像决策支持方面展现出潜力，但其在神经影像领域的可靠性和实际应用权衡尚未得到充分理解，需要系统性的评估来指导临床应用。

Method: 使用涵盖多发性硬化、中风、脑肿瘤、其他异常和正常对照的MRI和CT数据集，要求模型同时生成诊断、亚型、成像模态、专用序列和解剖平面等多重输出，通过多阶段框架控制选择偏倚，从四个维度评估模型性能。

Result: 技术成像属性（如模态和平面）基本解决，但诊断推理特别是亚型预测仍具挑战性；肿瘤分类最可靠，中风中等可解，多发性硬化和罕见异常仍困难；少样本提示可提升性能但增加计算成本；Gemini-2.5-Pro和GPT-5-Chat诊断性能最强，Gemini-2.5-Flash效率-性能平衡最佳；开源模型MedGemma-1.5-4B表现最有前景。

Conclusion: 研究为多模态大语言模型在神经影像领域的性能、可靠性和效率权衡提供了实用见解，支持该领域的标准化评估，并识别了当前模型的优势和局限。
...
TL;DR: CORA是一个用于心血管风险评估的3D视觉基础模型，通过病理中心、合成驱动的自监督框架直接从CCTA体积数据学习，在多个诊断和解剖任务上优于现有方法，并能显著改善30天主要不良心脏事件风险分层。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉疾病是全球心血管死亡的主要原因，虽然可以通过CCTA进行无创评估，但深度学习在CCTA分析中的临床应用受到专家标注数据稀缺的限制。此外，现有的无标签预训练策略（如掩码图像建模）偏向全局解剖统计特征，难以捕捉冠状动脉斑块的空间局部病理特征。

Method: 提出CORA模型，采用病理中心、合成驱动的自监督框架。通过解剖引导的病变合成引擎，模型被明确训练来检测模拟的血管异常，使表示学习偏向临床相关的疾病特征而非占主导的背景解剖结构。在12,801个未标记CCTA体积数据上训练，并与大型语言模型结合形成多模态框架。

Result: 在来自9家独立医院的多中心数据集上进行全面评估，在斑块表征、狭窄检测和冠状动脉分割等诊断和解剖任务中，CORA始终优于最先进的3D视觉基础模型，性能提升高达29%。通过将成像编码器与大型语言模型结合，显著改善了30天主要不良心脏事件（MACE）的风险分层。
...
TL;DR: OptiSAR-Net++：首个跨域遥感视觉定位框架，通过PL-MoE实现高效跨域特征解耦，采用对比学习范式替代Transformer解码，在OptSAR-RSVG和DIOR-RSVG基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉定位方法局限于单一传感器域（光学或SAR），限制了实际应用。为解决跨域特征建模、计算效率低下和细粒度语义区分等挑战，需要开发跨域RSVG任务和相应方法。

Method: 提出OptiSAR-Net++框架：1) PL-MoE（补丁级低秩适应专家混合）用于高效跨域特征解耦；2) 采用CLIP对比学习范式替代Transformer解码，结合动态对抗负采样；3) 文本引导双门融合模块(TGDF-SSA)增强语义-视觉对齐；4) 区域感知辅助头加强空间建模。

Result: 在首个大规模跨域基准数据集OptSAR-RSVG和DIOR-RSVG上达到最先进性能，在定位精度和效率方面均有显著优势。

Conclusion: OptiSAR-Net++成功解决了跨域遥感视觉定位问题，通过创新的跨域特征建模和高效对比学习框架，为实际应用提供了有效的解决方案。
...
TL;DR: 该研究提出一个垂体瘤手术视频阶段识别框架，结合自监督学习、时序建模和可扩展标注策略，准确率达90%，并开发了外科医生协作平台用于数据收集和模型改进。


<details>
  <summary>Details</summary>
Motivation: 准确的手术阶段识别对于分析手术流程、支持术中决策以及推动数据驱动的手术教育和绩效评估至关重要。垂体瘤手术视频的阶段识别具有挑战性，需要处理有限标注数据和手术变异性问题。

Method: 1. 开发协作在线平台供外科医生上传手术视频、接收自动阶段分析并贡献数据；2. 在251个未标注PTS视频上使用自监督框架预训练ResNet-50模型；3. 在81个标注手术数据集上微调，采用焦点损失、渐进层解冻和动态采样策略处理类别不平衡和手术变异性。

Result: 在独立测试集上达到90%的准确率，优于当前最先进方法，并展现出对不同手术病例的强泛化能力。

Conclusion: 该框架成功实现了垂体瘤手术视频的准确阶段识别，通过自监督学习、时序建模和协作平台的有效结合，为手术分析、决策支持和教育评估提供了实用工具，同时促进了大规模数据收集和持续模型改进。
...
<details>
  <summary>Details</summary>
Motivation: 评估自监督学习（SSL）相对于ImageNet预训练初始化在膝关节骨关节炎诊断和预后建模中的改进效果

Method: 比较两种SSL方法：(i)基于膝关节X光片的图像自监督学习，(ii)基于未筛选医院膝关节X光片与放射科医生报告的多模态图像-文本自监督学习。在诊断（KL分级）和预后（4年结构发生和进展）任务上进行评估

Result: 诊断任务：图像SSL在线性探测时有所改进，但完全微调时未超越ImageNet；多模态SSL因预训练数据严重偏倚（93%为KL 3级）而未改善分级性能。预后任务：多模态SSL显著优于ImageNet基线，在外部验证中AUROC从0.599提升至0.701（10%标记数据）

Conclusion: 未筛选的医院图像-文本数据因严重偏倚可能不适合诊断学习，但当下游任务与预训练数据分布一致时，能为预后建模提供强有力信号
...
TL;DR: 该论文提出了一个大规模真实世界偏振反射和材质数据集，用于解决逆渲染中真实材质建模的挑战，显著提升了材质分离、光照保真度和几何一致性。


<details>
  <summary>Details</summary>
Motivation: 准确建模真实世界材质的光反射是逆渲染的核心挑战，现有方法严重依赖合成数据集，其简化光照和有限材质真实性导致模型难以泛化到真实世界图像。

Method: 构建了一个大规模真实世界偏振反射和材质数据集，使用配备交叉/平行偏振的8相机、346光源光舞台，涵盖218个日常物体，跨越多视角、多光照、偏振、反射分离和材质属性五个采集维度，包含超过120万张高分辨率图像，提供漫反射-镜面反射分离以及解析推导的漫反射反照率、镜面反射反照率和表面法线。

Result: 使用该数据集训练和评估了最先进的逆渲染和正向渲染模型，在固有分解、重光照和稀疏视图3D重建方面展示了显著改进，包括更好的材质分离、光照保真度和几何一致性。
...
TL;DR: TIGFlow-GRPO：一个两阶段生成框架，通过流匹配建模轨迹分布，再通过GRPO后训练对齐行为规则，提升轨迹预测的社会合规性和物理可行性。
...
Conclusion: 该框架为动态多媒体环境中连接基于流的轨迹建模与行为感知对齐提供了一种有效方法，能够生成更符合实际行为规范的轨迹预测。
...
TL;DR: 提出基于自回归扩散模型的无限时域原始注视预测框架，用于任意长度视频中的连续时空注视轨迹生成
...
Conclusion: 提出的生成框架能够有效预测任意长度视频中的无限时域原始注视，为场景理解和多模态交互提供了重要基础
...
TL;DR: TIES框架通过动态平衡注意力大小和跨层排名一致性，在减少78%视觉token的同时提升机器人操作成功率6%，无需额外训练


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型存在高推理延迟问题，主要源于密集视觉token处理。当前token缩减方法依赖静态注意力大小选择，但研究发现高注意力token是任务相关的，甚至可能降低策略性能

Method: 提出TIES（Tau-guided Inter-layer Efficient Selection）框架，基于跨层token排名一致性进行动态token选择，自适应平衡注意力大小和排名一致性，无需额外训练

Result: 在CogACT + SIMPLER基准测试中，TIES将平均成功率提升6%，同时减少78%的token使用量，并在不同解码器和基准上表现出强泛化能力

Conclusion: TIES通过动态token选择框架有效解决了VLA模型推理延迟问题，证明跨层排名一致性比单纯依赖注意力大小更有效，实现了性能提升与计算效率的双重优化
...
TL;DR: LaMP是一个双专家视觉-语言-动作框架，通过将密集3D场景流作为潜在运动先验嵌入，提升机器人操作的性能。相比现有VLA模型直接从2D语义视觉特征回归动作，LaMP通过流匹配运动专家和策略预测动作专家的门控交叉注意力对齐，显著提高了在陌生空间动态下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型直接从2D语义视觉特征回归动作，迫使它们隐式学习复杂的3D物理交互。这种隐式学习策略在陌生的空间动态下性能会下降，因此需要一种能显式建模3D运动先验的方法来提升机器人操作的鲁棒性。

Method: 提出LaMP双专家框架：1) 运动专家通过流匹配生成单步部分去噪的3D场景流；2) 动作专家预测策略动作；3) 通过门控交叉注意力将运动专家的隐藏状态作为条件输入动作专家，无需完整多步重建。

Result: 在LIBERO、LIBERO-Plus和SimplerEnv-WidowX仿真基准以及真实世界实验中，LaMP始终优于评估的VLA基线，在相同训练预算下达到最高平均成功率。在LIBERO-Plus OOD扰动下，LaMP显示出改进的鲁棒性，比最强基线平均提升9.7%。

Conclusion: LaMP通过将3D场景流作为潜在运动先验嵌入，有效解决了VLA模型在陌生空间动态下的性能下降问题，显著提升了机器人操作的鲁棒性和成功率，为视觉-语言-动作模型提供了新的架构设计思路。
...
TL;DR: Vega模型是一个统一的视觉-语言-世界-动作模型，用于基于指令的自动驾驶生成和规划，通过结合自回归和扩散范式处理多模态输入，实现个性化驾驶。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型主要将语言用于场景描述或推理，缺乏遵循多样化用户指令实现个性化驾驶的灵活性，需要更智能的指令跟随系统。

Method: 构建大规模驾驶数据集InstructScene（约10万场景），提出Vega统一模型：使用自回归范式处理视觉输入和语言指令，扩散范式生成未来预测和轨迹，通过联合注意力实现模态交互，不同模态使用独立投影层。

Result: 实验表明该方法不仅实现了优越的规划性能，还展现出强大的指令跟随能力，为更智能和个性化的驾驶系统铺平了道路。

Conclusion: Vega模型通过统一的多模态处理框架，成功解决了现有自动驾驶系统缺乏个性化指令跟随能力的问题，推动了智能驾驶系统向更灵活、更人性化的方向发展。
...
TL;DR: xLARD是一个自校正文本到图像生成框架，使用多模态大语言模型通过可解释的潜在奖励来指导生成过程


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成在处理复杂提示时面临挑战，特别是细粒度语义和空间关系。生成的前馈特性需要在不完全理解输出的情况下预测对齐，而评估生成图像相对更容易。这种不对称性促使研究者开发自校正框架

Method: 提出xLARD框架，使用轻量级校正器基于模型生成参考的结构化反馈来精炼潜在表示。关键组件是从潜在编辑到可解释奖励信号的可微分映射，使得不可微分的图像级评估能够提供连续的潜在级指导

Result: 在多样化的生成和编辑任务实验中，xLARD提高了语义对齐和视觉保真度，同时保持了生成先验

Conclusion: xLARD通过使模型在生成过程中能够理解、评估和校正自身，为文本到图像生成提供了一种有效的自校正机制，解决了复杂提示对齐的挑战
...
<details>
  <summary>Details</summary>
Motivation: 现有级联解决方案存在严重误差累积问题，而通用联合模型缺乏明确的人脸先验，难以恢复清晰的人脸结构。真实世界低光人脸图像面临多重退化挑战。

Method: 提出PASDiff模型：1)利用逆强度加权和Retinex理论引入光度约束，可靠恢复可见度和自然色彩；2)提出风格无关结构注入(SASI)从现成人脸先验中提取结构同时过滤其固有光度偏差，将身份特征与物理约束无缝融合。

Result: 构建了WildDark-Face基准数据集（700张真实世界低光人脸图像），实验表明PASDiff显著优于现有方法，在自然光照、色彩恢复和身份一致性方面达到优越平衡。
...
<details>
  <summary>Details</summary>
Motivation: 混合专家(MoE)范式在视觉语言模型中能减少计算开销，但传统确定性top-K路由机制可能忽略更优的专家组合，导致专家过拟合，需要改进专家选择的多样性。

Method: 提出MoE-GRPO框架，将专家选择建模为序列决策问题，使用Group Relative Policy Optimization(GRPO)通过探索和奖励反馈学习自适应路由策略，并引入模态感知路由指导来提升训练稳定性。

Result: 在多模态图像和视频基准测试中，MoE-GRPO一致优于标准top-K路由及其变体，通过促进更丰富的专家选择来缓解专家过拟合，实现任务级专家专业化。

Conclusion: MoE-GRPO通过强化学习优化专家路由，有效解决了传统MoE路由的局限性，提升了视觉语言模型的性能和效率，为多模态理解提供了更灵活的路由机制。
...
TL;DR: 提出基于元学习的少样本3D左心房壁分割方法，通过多任务元训练和边界感知损失提升薄壁结构分割精度，在少样本情况下接近全监督性能并保持跨域鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 左心房壁的晚期钆增强MRI分割面临薄壁几何结构、低对比度和专家标注稀缺的挑战，需要开发能够在少样本条件下准确分割并具有鲁棒性的方法。

Method: 采用模型无关元学习框架，结合左心房壁主任务和左右心房腔辅助任务进行元训练，使用边界感知复合损失函数强调薄壁结构精度，支持K-shot（K=5,10,20）少样本适应。

Result: 在保留测试集上，MAML在5-shot时Dice分数0.64优于监督微调的0.52，20-shot时接近全监督参考的0.71；在未见域偏移和本地队列中保持鲁棒性，性能随K增加而提升。
...
TL;DR: 该论文首次构建了基于事件流的视频异常检测基准数据集，并提出了EWAD框架，通过事件密度感知采样、密度调制时序建模和RGB到事件的知识蒸馏机制，显著提升了事件流异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 事件视觉具有低冗余、关注动态运动和隐私保护特性，天然适合视频异常检测需求，但缺乏专用的事件流异常检测数据集和有效建模策略阻碍了该领域发展。

Method: 提出了EWAD框架：1）事件密度感知动态采样策略选择信息丰富的时序片段；2）密度调制时序建模方法从稀疏事件流中捕获上下文关系；3）RGB到事件的知识蒸馏机制在弱监督下增强事件表示。

Result: 在三个基准数据集上的大量实验表明，EWAD相比现有方法取得了显著改进，突显了事件驱动建模在视频异常检测中的潜力和有效性。

Conclusion: 该工作首次将事件流视频异常检测确立为统一研究方向，构建了基准数据集并提出了有效的EWAD框架，为事件视觉在异常检测领域的应用奠定了基础。
...
Method: 提出C2W-Tune两阶段框架：第一阶段使用3D U-Net（ResNeXt编码器和实例归一化）预训练分割左心房腔室；第二阶段通过渐进式层解冻策略迁移权重，适应左心房壁分割，保留心内膜特征的同时实现壁层特异性细化。
...
Conclusion: 基于解剖学的任务迁移结合受控微调显著提高了3D LGE-MRI中薄壁左心房壁分割的边界精度，为临床壁厚映射和纤维化量化提供了更可靠的解决方案。
...
TL;DR: 提出一种基于3D高斯泼溅的动态场景重建新方法，通过视图空间射线分组策略保持高斯粒子的局部几何结构一致性，无需外部先验即可实现更真实的物理运动建模。


<details>
  <summary>Details</summary>
Motivation: 现有动态3D场景重建方法在建模真实运动方面存在挑战，高斯粒子的运动与真实物理动力学不一致，特别是在单目视频数据集中，缺乏连贯运动会破坏局部几何结构，导致重建质量下降。大多数方法依赖外部先验（如光流或2D轨迹）来强制时间一致性。

Method: 提出一种视图空间射线分组策略：将同一射线相交的高斯粒子聚类分组，仅考虑α混合权重超过阈值的高斯粒子。对这些分组施加约束以保持一致的空间分布，从而保持局部几何结构。该方法无需外部指导，强制实现更物理合理的运动模型。

Result: 将方法集成到两个不同的基线模型中，在具有挑战性的单目数据集上进行广泛实验。结果显示，该方法显著优于现有方法，实现了更优的时间一致性和重建质量。

Conclusion: 通过显式保持4D场景中高斯粒子的局部几何结构，提出了一种无需外部先验即可实现物理合理运动建模的有效方法，显著提升了动态场景重建的时间一致性和质量。
...
<details>
  <summary>Details</summary>
Motivation: 应急车辆快速通行对拯救生命和减少财产损失至关重要，但现有方法存在两个根本性限制：集中式数学求解器计算成本高且仅适用于小规模场景，强化学习方法缺乏对不同交通条件的可扩展性。

Method: 提出可扩展的分布式车辆控制方法，车辆仅使用局部信息在线分布式调整驾驶行为；证明该方法与使用全局信息的方法近似等价；进一步提出分布式冲突解决机制保证车辆安全，避免决策冲突。

Result: 基于真实交通数据集的仿真实验表明，相比现有方法，该方法实现更快的决策速度、对普通车辆影响更小，在不同交通密度和道路配置下保持更强的可扩展性。

Conclusion: 分布式车辆控制方法通过仅使用局部信息实现近似最优决策，无需预训练且自然适应变化的交通条件，同时提供确定性安全保证，解决了集中式方法的单点故障风险和强化学习方法缺乏可扩展性的问题。
...
Method: 提出SGREC方法：1）使用视觉语言模型构建查询驱动的场景图，显式编码空间关系、描述性标题和对象交互；2）利用场景图作为结构化中介，弥合低级图像区域和高级语义理解之间的差距；3）让大型语言模型从场景图提供的结构化文本表示中推断目标对象，并提供详细解释以确保推理过程的可解释性。

Result: 在多个零样本REC基准测试中取得最佳性能：RefCOCO val (66.78%)、RefCOCO+ testB (53.43%)、RefCOCOg val (73.28%)，展示了强大的视觉场景理解能力。

Conclusion: SGREC通过查询驱动的场景图作为结构化中介，成功结合了视觉语言模型和大型语言模型的优势，在零样本指代表达理解任务中实现了最先进的性能，同时保持了推理过程的可解释性。
...
TL;DR: Few TensoRF结合TensorRF的高效张量表示与FreeNeRF的频率驱动少样本正则化，在稀疏视角下实现快速高质量的3D重建。


<details>
  <summary>Details</summary>
Motivation: 解决现有3D重建方法在稀疏输入视角下重建质量不稳定、训练时间长的问题，结合高效表示与正则化技术实现数据高效的实时重建。

Method: 结合TensorRF的张量表示加速渲染，引入频率和遮挡掩码进行少样本正则化，提高稀疏视角下的重建稳定性和质量。

Result: 在Synthesis NeRF基准上，PSNR从21.45dB提升到23.70dB，精细调优版本达24.52dB，训练时间保持10-15分钟；在THuman 2.0数据集上，仅用8张图像达到27.37-34.00dB的竞争性能。

Conclusion: Few TensoRF是一种高效且数据有效的解决方案，能够在多样化场景中实现实时3D重建，平衡了重建质量、训练速度和数据需求。
...
TL;DR: 提出GDPO-Listener框架，通过自回归流匹配架构和分组奖励解耦策略优化，解决对话中听众头部动作的"回归到均值"问题，实现高表达性的说话和倾听动作生成，并支持语义文本控制。


<details>
  <summary>Details</summary>
Motivation: 当前虚拟人合成方法在生成对话中的听众头部动作时存在"回归到均值"问题，导致动作趋于静态，缺乏复杂非语言动作的参数空间，无法生成真实自然的对话交互。

Method: 1) 提出自回归流匹配架构实现稳定监督学习；2) 采用分组奖励解耦策略优化，通过分离不同FLAME参数组的奖励归一化，明确激励高方差表达性生成；3) 支持可定制的语义文本控制响应。

Result: 在Seamless Interaction和DualTalk数据集上的广泛评估表明，该方法在长期运动方差、视觉表达性和语义可控性方面优于现有基线方法。

Conclusion: GDPO-Listener框架成功解决了对话中听众动作生成的"回归到均值"问题，实现了高表达性的说话和倾听动作生成，并提供了语义控制能力，为虚拟人交互合成提供了有效解决方案。
...
TL;DR: VideoTIR：基于强化学习的多模态大语言模型长视频理解方法，通过工具调用策略优化和轨迹合成框架，解决幻觉问题并提升效率


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在长视频理解中存在幻觉问题，主要原因是文本和视觉令牌不平衡。虽然SFT-based工具调用方法可以缓解此问题，但需要大量高质量数据且工具调用轨迹受限

Method: 提出VideoTIR方法，利用强化学习促进多级工具包的正确使用；采用Zero-RL和SFT冷启动；提出TAGPO策略优化工具调用效率；开发基于沙箱的轨迹合成框架生成高质量轨迹数据

Result: 在三个长视频QA基准测试上的广泛实验证明了该方法的有效性和效率

Conclusion: VideoTIR通过强化学习驱动的工具调用策略，有效解决了MLLMs在长视频理解中的幻觉问题，实现了准确高效的长视频理解
...
TL;DR: CARE是一个无需训练的医疗图像可控修复框架，通过双潜在分支策略和风险感知控制器，在保持数据保真度和利用生成先验之间实现动态平衡。


<details>
  <summary>Details</summary>
Motivation: 现有医疗图像修复方法通常需要任务特定重训练，且在忠实重建与先验驱动增强之间缺乏可控性。过度激进的修复可能引入幻觉细节或改变诊断重要结构，这在临床环境中尤其危险。

Method: 提出CARE框架：1）双潜在修复策略：一个分支强制数据保真度和解剖一致性，另一个分支利用生成先验恢复缺失或退化信息；2）风险感知自适应控制器：基于修复不确定性和局部结构可靠性动态调整各分支贡献，无需额外模型训练即可实现保守或增强导向的修复模式。

Result: 在噪声和不完整医疗图像场景中，CARE实现了强大的修复质量，同时更好地保留了临床相关结构，减少了不合理重建的风险。

Conclusion: CARE为更安全、更可控、更易于部署的医疗图像修复提供了实用步骤，通过训练自由的可控框架解决了临床环境中修复与保真度的平衡问题。
...
TL;DR: GeoNDC是一个可查询的神经数据立方体，将大规模地球观测数据编码为连续的时空隐式神经场，支持按需查询和连续时间重建，实现高压缩比和高保真度。
...
TL;DR: MoRGS是一个在线动态场景重建框架，通过显式建模每个高斯点的运动来提升4D重建质量，使用稀疏光流作为运动监督，并引入运动置信度分离动态和静态区域。


<details>
  <summary>Details</summary>
Motivation: 现有在线4D重建方法虽然高效且视觉质量好，但无法学习反映真实场景动态的每个高斯点运动。仅依赖光度损失优化会导致高斯点运动追逐像素残差而非真实3D运动，需要显式的运动监督来改进。

Method: 1. 利用稀疏关键视图的光流作为轻量级运动线索，超越光度监督来正则化每个高斯点运动；2. 学习每个高斯点的运动偏移场，协调投影3D运动与跨视图和时间的观测光流之间的差异；3. 引入每个高斯点的运动置信度，分离动态和静态高斯点，加权高斯属性残差更新，抑制静态区域的冗余运动。

Result: 大量实验表明，MoRGS在在线方法中实现了最先进的重建质量和运动保真度，同时保持了流式处理性能。

Conclusion: MoRGS通过显式建模每个高斯点运动并利用稀疏光流监督，有效解决了在线4D重建中运动学习不足的问题，在保持实时性能的同时显著提升了重建质量和运动准确性。
...
TL;DR: GaussFusion通过几何感知的视频生成改进3D高斯泼溅重建，解决浮动物体、闪烁和模糊等常见伪影，支持优化式和前馈式方法，在实时21FPS下保持SOTA性能。
...
Method: 引入几何感知的视频到视频生成器，通过渲染包含深度、法线、不透明度和协方差的Gaussian primitive视频缓冲区，生成时间连贯、无伪影的帧。同时开发了伪影合成管道模拟多样化退化模式。

Result: 在新视角合成基准测试中达到最先进性能，高效变体在21FPS实时运行的同时保持相似性能，支持交互式3D应用。

Conclusion: GaussFusion通过几何感知的视频生成有效解决了3DGS重建中的伪影问题，为优化式和前馈式方法提供了统一的改进框架，实现了高质量且实时的3D重建。
...
Method: 开发闭环Event-SVE测量系统，将空间变曝光相机与立体对神经形态事件相机耦合。SVE分支采用烟雾感知融合策略生成HDR图，使用多线索烟雾似然图分离粒子发射和烟雾散射。事件相机利用HDR图作为绝对强度参考，抑制烟雾驱动的事件伪影，改进粒子状态判别。基于清洗后的事件观测，立体事件3D管道通过特征提取和三角测量估计分离高度和等效粒子尺寸。
...
TL;DR: 该论文提出了一种从单目视频进行高质量动态高斯溅射的方法，通过SE(3) B样条运动基显式建模高斯的位置和方向连续变形，并采用自适应控制机制、软分割重建策略和多视图扩散模型来提升效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单目视频进行动态高斯溅射时，对复杂运动的建模能力有限，容易受到长间隔运动干扰，且容易过拟合训练视图。需要一种能够显式建模连续变形、提高计算效率并增强复杂运动建模能力的方法。

Method: 1. 使用SE(3) B样条运动基显式建模动态高斯的连续位置和方向变形；2. 设计自适应控制机制动态调整运动基和控制点数量；3. 开发软分割重建策略减轻长间隔运动干扰；4. 采用多视图扩散模型提供多视图线索避免过拟合。

Result: 大量实验表明，该方法在新视图合成方面优于现有最先进方法，能够生成更高质量的动态场景重建。

Conclusion: 该方法通过显式建模连续变形、自适应控制、软分割重建和多视图扩散模型，有效提升了从单目视频进行动态高斯溅射的质量和鲁棒性，在复杂运动场景中表现出色。
...
TL;DR: GIFT是一个无需训练的视频帧选择框架，通过评估帧的内在不可替代性来选择关键帧，相比均匀采样在长视频基准上平均提升12.5%


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型处理密集帧计算成本高，现有关键帧选择方法采用贪婪决策，将相关性和多样性解耦评估，容易陷入局部最优并选择无关噪声帧
...
Conclusion: GIFT通过评估帧的内在不可替代性，解决了现有关键帧选择方法的局限性，显著提升了视频大语言模型的效率和性能
...
<details>
  <summary>Details</summary>
Motivation: 概念擦除是文本到图像模型的重要安全机制，但现有方法主要针对U-Net和双流架构，在单流扩散Transformer（如Z-Image）中应用时会导致生成崩溃，需要专门的方法来解决这一问题
...
Conclusion: Z-Erase是首个专门针对单流扩散Transformer的概念擦除方法，通过流解耦框架和约束优化算法，解决了现有方法在单流架构中的局限性，为这类新兴模型提供了有效的安全机制
...
TL;DR: 本文提出了一种用于多模态大语言模型的Token重加权策略，解决RLVR训练中感知与推理令牌相互依赖的优化挑战。


<details>
  <summary>Details</summary>
Motivation: 将强化学习与可验证奖励扩展到多模态大语言模型面临根本挑战：模型响应中感知相关令牌（用于视觉内容定位）和推理相关令牌（用于构建推理链）交织在一起。这两种令牌类型体现了不同但相互依赖的能力——视觉定位和符号推理，使得单独优化任一类型都不够充分。

Method: 提出了即插即用的Token重加权策略，通过识别两种类型的关键令牌并在RLVR训练期间动态重新加权，显式建模感知与推理令牌之间的相互依赖关系。该策略可应用于现有方法之上。

Result: 在多个多模态推理基准测试中，ToR策略在现有方法基础上带来一致的性能提升，实现了最先进的性能，同时具备准确的视觉定位和连贯的推理能力。

Conclusion: 通过显式建模感知与推理令牌之间的相互依赖关系，Token重加权策略有效解决了多模态大语言模型在RLVR训练中的优化挑战，实现了视觉定位和符号推理能力的协同提升。
...
TL;DR: HCD通过通道级因果掩码实现特征稀疏化，分离因果特征与伪特征，在表示层面进行因果干预，在OOD基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型容易捕捉领域特定的上下文，形成对非因果特征的捷径依赖，导致在不同数据源上性能不一致。现有方法如不变性学习难以在深度潜在空间中分离高度混合的特征，无法完全解决捷径学习问题。

Method: 提出分层因果丢弃(HCD)方法：使用通道级因果掩码强制特征稀疏化，在表示层面进行因果干预；采用基于矩阵的互信息(MMI)目标，最小化潜在特征与领域标签的互信息，同时最大化与类别标签的共享信息；引入StyleMix驱动的VICReg模块确保稳定性，防止掩码意外过滤掉重要的因果数据。

Result: 在OOD基准测试中，HCD的表现优于现有的顶级方法。

Conclusion: HCD通过因果干预和特征分离，有效解决了深度学习模型中的捷径依赖问题，在OOD泛化任务中取得了显著改进。
...
TL;DR: 论文提出CLVA方法解决多模态大语言模型中的物体幻觉问题，通过跨层视觉锚点抑制深层注意力回归到早期视觉噪声


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对最终模型阶段注意力漂移的充分解释性，需要深入理解视觉特征在层间的演化过程

Method: 提出CLVA（跨层视觉锚点）方法，这是一种无需训练的方法，通过强化关键中间层特征同时抑制回归噪声，利用注意力动态捕获的锚点将深层注意力拉回正确的视觉区域

Result: 在多种架构和基准测试中表现出色，且计算时间和GPU内存没有显著增加

Conclusion: 物体幻觉源于深层注意力向早期视觉噪声的回归，输出可靠性依赖于中间层而非最终层获取视觉锚点，CLVA方法能有效解决这一问题
...
TL;DR: THEMIS是一个新颖的多任务基准测试，专门用于评估多模态大语言模型在真实学术场景中的视觉欺诈推理能力，包含4000多个问题、7个场景、5种欺诈类型和16种精细操作，对16个领先MLLM的测试显示最佳模型仅达到56.15%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试与真实世界学术欺诈的复杂性存在差距，需要开发一个更全面、更具挑战性的基准来评估多模态大语言模型在复杂视觉欺诈推理任务中的能力。

Method: THEMIS基准包含三个主要创新：1) 基于真实撤稿案例构建的7个场景、4000多个问题，其中60.47%为复杂纹理图像；2) 系统覆盖5种欺诈类型和16种精细操作，每个样本经历多重叠加操作；3) 建立欺诈类型到5个核心视觉欺诈推理能力的映射，实现多维度能力评估。

Result: 对16个领先MLLM的实验结果显示，即使是表现最佳的GPT-5模型，整体准确率也仅为56.15%，表明该基准具有很高的挑战性，能够有效揭示不同模型在视觉欺诈推理方面的具体优势和弱点。

Conclusion: THEMIS基准填补了现有基准与真实世界学术欺诈复杂性之间的关键差距，为多模态大语言模型在复杂、真实世界欺诈推理任务的发展提供了重要的评估工具和推动力。
...
<details>
  <summary>Details</summary>
Motivation: 多模态学习依赖大规模标注数据，主动学习可减少标注成本。现有方法假设模态重要性稳定且选择规则固定，无法适应多模态学习中模态相对价值和样本难度随训练动态变化的问题

Method: 提出RL-MBA强化学习框架，将样本选择建模为马尔可夫决策过程。包含两个关键组件：1) 自适应模态贡献平衡(AMCB)，通过强化反馈动态调整模态权重；2) 证据融合难度感知策略调整(EFDA)，基于不确定性证据融合估计样本难度，优先选择信息量大的样本

Result: 在Food101、KineticsSound和VGGSound数据集上的实验表明，RL-MBA在有限标注预算下持续优于强基线方法，既提高了分类准确率，又改善了模态公平性

Conclusion: RL-MBA通过强化学习框架有效解决了多模态主动学习中模态重要性动态变化的问题，实现了自适应、难度感知的样本选择策略，在多模态数据集上取得了显著性能提升
...
<details>
  <summary>Details</summary>
Motivation: 现有基于RLVR的多模态奖励模型训练依赖昂贵的人工标注多模态偏好数据，难以规模化扩展，需要解决数据稀缺问题。

Method: 提出多阶段强化学习方法：1）从大规模文本偏好数据学习通用奖励推理能力；2）通过基于描述的强化学习阶段逐步迁移到多模态任务；3）引入跨模态知识蒸馏提升偏好泛化能力。

Result: MSRL显著提升生成式多模态奖励模型性能：VL-RewardBench从66.6%提升至75.9%，GenAI-Bench从70.2%提升至75.7%，且无需额外多模态偏好标注。

Conclusion: MSRL方法有效解决了多模态奖励模型训练的数据稀缺问题，实现了基于RLVR训练的规模化扩展，显著提升了视觉理解和生成任务的性能。
...
TL;DR: 本文提出了一种基于莫尔干涉的程序化数据增强方法，通过数学公式实时生成纹理模式，无需外部数据且计算成本极低，显著提升了图像分类模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有数据增强方法主要依赖扩散模型合成或复杂的特征混合策略，这些方法计算开销大或需要外部数据集。本文探索基于解析干涉模式的程序化增强，提供更轻量、高效的替代方案。

Method: 提出一种轻量级增强方法，使用闭式数学公式程序化生成莫尔干涉纹理模式。这些模式在内存中实时合成（每张图像仅需0.0026秒），与训练图像混合后立即丢弃，实现无需存储的外部数据自由增强流程。

Result: 在Vision Transformers上的广泛实验表明，该方法在多个基准测试（包括ImageNet-C、ImageNet-R和对抗性基准）中一致提升了鲁棒性，超越了标准增强基线和现有的外部数据自由增强方法。

Conclusion: 解析干涉模式为数据驱动的生成式增强方法提供了实用且高效的替代方案，通过程序化生成的结构化扰动实现了低成本、高效果的模型鲁棒性提升。
...
TL;DR: AnyDoc是一个统一的HTML/CSS文档生成框架，通过自动合成大规模数据集DocHTML，并采用多模态大语言模型微调和高度感知强化学习，实现了跨111个类别、32种风格的多种文档生成任务。


<details>
  <summary>Details</summary>
Motivation: 当前AI驱动的文档生成研究面临两个主要挑战：1) 现有手工制作的文档数据集覆盖范围有限、规模不足；2) 需要统一的框架来处理多种文档生成任务。AnyDoc旨在通过自动数据合成和统一格式来解决这些问题。

Method: 1) 建立可扩展的数据合成管道，自动生成HTML/CSS格式的文档，创建DocHTML数据集（265,206个样本，111个类别，32种风格）；2) 基于该数据集微调多模态大语言模型，实现三种文档生成任务：意图到文档、文档反渲染、元素到文档；3) 引入高度感知强化学习后训练程序，通过基于预测与目标文档高度差异的奖励函数，惩罚并缓解内容溢出问题。

Result: 定性和定量实验表明，AnyDoc在所有三种任务上都优于通用多模态大语言模型和任务特定的基线方法。DocHTML数据集包含全面的元数据（设计意图、HTML/CSS源代码、视觉资产和渲染截图），为文档生成研究提供了丰富资源。

Conclusion: AnyDoc通过统一的HTML/CSS表示、大规模自动数据合成和多模态模型微调，成功推进了文档生成领域的发展。高度感知强化学习有效解决了内容溢出问题，使框架能够处理广泛的文档类别和生成任务。
...
<details>
  <summary>Details</summary>
Motivation: 虽然3D视觉基础模型在零样本几何估计方面表现出色，但直接应用于通用的无姿态新视角合成仍然具有挑战性。需要将3DVFMs的鲁棒几何先验有效适配到高质量、无姿态的新视角合成任务中。

Method: 提出AirSplat框架，包含两个关键技术贡献：1) 自一致姿态对齐(SCPA) - 训练时反馈循环确保像素对齐监督以解决姿态-几何不一致问题；2) 基于评分的透明度匹配(ROM) - 利用稀疏视角NVS教师模型的局部3D几何一致性知识过滤退化的基元。

Result: 在大规模基准测试中，该方法在重建质量方面显著优于最先进的无姿态NVS方法。
...
<details>
  <summary>Details</summary>
Motivation: 全景语义分割面临两大挑战：全景投影的几何畸变和密集标注的高成本。无监督领域自适应（UDA）虽可行，但实际应用中常受限于源数据不可访问的严格约束（SFUDA），导致领域偏移问题加剧、伪标签不可靠，特别是少数类性能严重下降。

Method: 提出DAPASS框架，包含两个协同模块：1) 全景置信度引导去噪（PCGD）模块，通过强制扰动一致性和结合邻域级置信度来生成高质量、类别平衡的伪标签；2) 上下文分辨率对抗模块（CRAM），通过对抗性对齐高分辨率裁剪的细粒度细节与低分辨率上下文的全局语义，显式处理尺度变化和畸变问题。

Result: 在室外（Cityscapes-to-DensePASS）和室内（Stanford2D3D）基准测试中达到最先进性能，分别获得55.04%（+2.05%）和70.38%（+1.54%）的mIoU。

Conclusion: DAPASS框架有效解决了源数据不可访问条件下的全景语义分割领域自适应问题，通过创新的伪标签生成和对抗性对齐机制，显著提升了性能，特别是在处理几何畸变和少数类识别方面表现出色。
...
TL;DR: 提出了一种新的鲁棒主成分完成（RPCC）框架，通过间接确定稀疏分量的支持集来处理遮挡问题，使用变分贝叶斯推理解决，无需后处理阈值。


<details>
  <summary>Details</summary>
Motivation: 传统的鲁棒主成分分析（RPCA）假设稀疏前景与低秩背景是相加关系，但在许多实际应用中，稀疏前景会遮挡或替换低秩背景元素，这种不匹配需要新的方法来解决。

Method: 提出鲁棒主成分完成（RPCC）框架，通过间接确定稀疏分量的支持集来处理遮挡问题。采用完全概率贝叶斯稀疏张量分解，并使用变分贝叶斯推理进行求解。

Result: 方法收敛到支持集的硬分类器，无需后处理阈值。在合成数据上获得接近最优的估计，在真实彩色视频数据集上实现鲁棒的前景提取，在高光谱数据集上实现异常检测。

Conclusion: RPCC框架有效解决了传统RPCA中的遮挡问题，通过变分贝叶斯推理实现了无需阈值的稀疏分量识别，在多种实际应用中表现出色。
...
TL;DR: SAVe是一个自监督的音频-视觉深度伪造检测框架，通过使用真实视频生成伪篡改样本来学习检测多模态伪造，无需依赖合成训练数据。


<details>
  <summary>Details</summary>
Motivation: 当前多模态深度伪造检测方法主要依赖精心制作的合成伪造数据进行训练，这会导致数据集和生成器偏差，限制了模型的可扩展性和对未见篡改的鲁棒性。

Method: SAVe采用完全自监督方法，在真实视频上生成身份保持、区域感知的自混合伪篡改样本来模拟篡改伪影；同时通过音频-视觉对齐组件建模唇语同步，检测音频-视觉伪造特有的时间错位模式。

Result: 在FakeAVCeleb和AV-LipSync-TIMIT数据集上的实验表明，SAVe在域内性能具有竞争力，并展现出强大的跨数据集泛化能力。

Conclusion: 自监督学习是多模态深度伪造检测的可扩展范式，能够减少对合成训练数据的依赖，提高模型的泛化能力和鲁棒性。
...
TL;DR: EgoXtreme是一个新的大规模6D姿态估计数据集，专门针对第一人称视角的极端场景设计，包含工业维护、体育和紧急救援三种挑战性场景，旨在解决现有基准与真实世界应用之间的差距。
...
Method: 引入EgoXtreme数据集，完全从第一人称视角采集，包含三种极端场景：工业维护、体育和紧急救援，这些场景通过极端光照、严重运动模糊和烟雾引入严重的感知模糊性。

Result: 评估显示，现有最先进的通用姿态估计器在极端条件下（特别是低光照下）泛化能力失效。简单的图像恢复（如去模糊）对极端条件没有积极改善，但基于跟踪的方法显示出性能提升，表明在快速运动场景中使用时间信息是有意义的。

Conclusion: EgoXtreme是开发和评估下一代足够鲁棒用于真实世界第一人称视觉的姿态估计模型的重要资源，填补了现有基准与真实世界应用之间的关键差距。
...
<details>
  <summary>Details</summary>
Motivation: 标准DPO的二元"赢家通吃"方法不适合视觉语言模型，因为即使某个响应不如其他响应受欢迎，它仍可能忠实于视觉内容。DPO缺乏这种细微差别，会过度强调获胜响应而忽视"失败"响应可能仍保持高视觉忠实度的问题。

Method: 提出排序优化作为替代方案，更精确地定位响应与视觉输入的忠实度。专注于视频文本对齐任务，使用详细视频字幕，通过重复字幕降级方法大规模生成具有挑战性的完全有序字幕链。

Result: 排序优化在长内容生成和评估方面优于二元DPO。重要的是，研究发现这些方法需要对视觉编码器进行微调才能有效，这挑战了DPO纯粹是语言重加权过程的观点。

Conclusion: 对于视觉语言模型，排序优化比二元DPO更适合处理响应与视觉内容的忠实度问题，且视觉编码器的微调是实现有效对齐的关键因素。
...
TL;DR: 开发了一个包含孟加拉国五种常见皮肤病的公开数据集，包含1612张图像，并应用机器学习和深度学习模型进行分类性能评估。


<details>
  <summary>Details</summary>
Motivation: 皮肤疾病是全球主要公共卫生问题，在孟加拉国等人口密集国家，皮肤科专家和诊断设备不足，缺乏适当的检测和治疗可能导致严重后果。利用人工智能和机器学习技术，通过图像处理和计算机视觉方法检测皮肤疾病。

Method: 收集了来自法里德布尔医学院门诊患者的五种常见皮肤病图像数据集（接触性皮炎、白癜风、湿疹、疥疮、癣），共1612张图像（其中250张原始图像，其余为增强图像）。应用多种机器学习和深度学习模型进行疾病分类。

Result: 数据集包含302张皮炎、381张湿疹、301张疥疮、316张癣和312张白癜风图像。虽然数据是区域性收集的，但这些疾病在南亚等许多国家都很常见，使该数据集在全球机器学习皮肤病学应用中具有潜在价值。

Conclusion: 开发了一个公开可用的皮肤病检测数据集，并展示了机器学习模型在该数据集上的分类性能。期望这项研究能吸引从事自动化疾病诊断的机器学习和深度学习研究人员和实践者的关注。
...
TL;DR: 该论文提出FD²框架，专门用于细粒度数据集蒸馏，通过定位判别性区域和构建细粒度表示来解决现有方法在细粒度数据集上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有解耦数据集蒸馏方法主要依赖粗粒度类别标签监督，在细粒度数据集上存在两个问题：(1) 保留大的类内差异但类间差异细微；(2) 同类样本变得过于相似，限制了局部判别性线索并损害识别性能。

Method: 提出FD²框架，包含两个阶段：预训练阶段通过反事实注意力学习聚合判别性表示来更新类别原型；蒸馏阶段使用细粒度特征约束对齐样本与类别原型并排斥其他类别，同时使用相似性约束使同类样本的注意力多样化。

Result: 在多个细粒度和通用数据集上的实验表明，FD²能够与解耦数据集蒸馏无缝集成，在大多数设置中提高了性能，显示出强大的可迁移性。

Conclusion: FD²框架通过定位判别性区域和构建细粒度表示，有效解决了细粒度数据集蒸馏中的问题，提高了识别性能并具有良好可迁移性。
...
TL;DR: TITAnD将轨迹异常检测重新定义为视觉问题，通过将轨迹表示为超光谱轨迹图像，统一处理密集和稀疏GPS数据，首次实现多个月密集轨迹的异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法处理多个月密集GPS轨迹的异常检测：密集方法计算成本高（二次复杂度），稀疏方法丢弃了精细证据。两种方法架构分离，无法知识迁移，但人类轨迹在密集和稀疏情况下都具有二维循环结构。

Method: 提出TITAnD框架：1）将轨迹表示为超光谱轨迹图像（HTI）- 天×时间的网格，通道编码空间、语义、时间和运动信息；2）引入循环因子化Transformer（CFT），沿两个时间轴因子化注意力，编码人类日常的循环归纳偏置，大幅降低注意力成本。

Result: 在稀疏和密集基准测试中达到最佳AUC-PR，超越UNet等视觉模型，比标准Transformer快11-75倍且内存相当，首次实现密集多个月轨迹异常检测。

Conclusion: 视觉重新表述和结构感知建模对轨迹异常检测至关重要，TITAnD统一了密集和稀疏轨迹处理，实现了可扩展的多个月分析。
...
TL;DR: Photon是一个用于3D医学视觉问答的框架，通过可变长度token序列和自适应token调度来降低计算成本，同时保持体积连续性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在临床视觉问答任务中很有前景，但扩展到3D成像受到高计算成本的阻碍。现有方法通常依赖2D切片或固定长度token压缩，这会破坏体积连续性并掩盖细微发现。

Method: Photon使用可变长度token序列表示3D医学体积，引入指令条件token调度和替代梯度传播来自适应减少训练和推理中的token，降低计算成本。采用自定义反向传播规则和梯度恢复实现可微分优化，并应用正则化目标来稳定token压缩和提高可靠性。

Result: 在多种医学视觉问答任务上的实验表明，Photon实现了最先进的准确性，同时减少了资源使用并加速了训练和推理。

Conclusion: Photon框架通过自适应token压缩有效解决了3D医学成像中的计算成本问题，在保持准确性的同时提高了效率，为临床视觉问答任务提供了实用解决方案。
...
TL;DR: 该论文提出了一种语义解缠的统一模型来解决3D异常检测中的跨类别纠缠问题，通过条件重建和对比学习实现更好的异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 在3D异常检测中，统一模型虽然能提高可扩展性，但存在跨类别纠缠问题，导致不同类别的潜在特征重叠，使模型在重建时采用错误的语义先验，从而产生不可靠的异常分数。
...
Conclusion: 提出的语义解缠统一模型有效解决了跨类别纠缠问题，通过条件重建和语义解缠显著提升了3D异常检测的性能和可靠性。
...
TL;DR: KGFP是一种基于表示的监控框架，通过检测目标检测器内部特征与视觉基础模型嵌入之间的语义不对齐来预测功能失效，显著提高安全关键检测的召回率。


<details>
  <summary>Details</summary>
Motivation: 传统OOD检测方法主要识别陌生输入，但不能直接预测目标检测器本身的功能失效（如漏检行人等安全关键对象）。在安全关键环境中，这种无声失效可能导致严重后果。

Method: 采用双编码器架构和角度距离度量，测量目标检测器内部特征与视觉基础模型嵌入之间的语义不对齐。当检测器超出其能力范围或视觉基础模型遇到新输入时，两种嵌入会发散，产生高角度信号来标记不安全图像。

Result: 在COCO行人检测任务中，KGFP作为选择性预测门，将接受图像中的行人召回率从64.3%提高到84.5%（5%误报率下）。在六个COCO-O视觉域中保持强劲性能，大幅超越OOD基线方法。

Conclusion: KGFP通过监测语义不对齐来预测目标检测器的功能失效，为安全关键环境中的目标检测提供了有效的运行时监控框架，显著提高了检测可靠性。
...
TL;DR: SportSkills是首个面向物理技能学习的大规模体育数据集，包含36万+教学视频和63万+视觉演示，覆盖55种不同运动，能够理解精细动作差异，并支持基于错误的视频检索任务。


<details>
  <summary>Details</summary>
Motivation: 当前大规模视频数据集主要关注通用人类活动，但缺乏针对物理技能学习所需的细粒度活动覆盖。需要专门的数据集来支持体育技能学习和动作理解。

Method: 构建SportSkills数据集，包含36万+教学视频和63万+视觉演示，配有解释动作原理的教学旁白。基于该数据集训练模型，并引入基于错误的指令视频检索任务，将表示学习与可操作反馈生成相结合。

Result: SportSkills显著提升了模型理解物理动作细粒度差异的能力，相比传统活动中心数据集，相同模型性能提升高达4倍。专业教练评估显示，检索方法显著提升了视频模型为用户查询个性化视觉指令的能力。

Conclusion: SportSkills填补了物理技能学习数据集的空白，不仅提升了动作理解能力，还开创了基于错误的指令视频检索新任务，为个性化技能学习提供了有效工具。
...
TL;DR: 本文分析了自动驾驶中高光谱成像技术面临的挑战，包括非受控光照条件、大景深范围、动态场景以及实时计算需求，并探讨了基于HSI-Drive数据集的研究技术。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶领域对高光谱成像技术的应用前景广阔，但面临特定应用场景的挑战，包括非受控光照条件、大景深范围、动态场景以及嵌入式平台有限的计算资源，需要开发适合的HSI技术和定制化视觉算法。

Method: 通过分析基于HSI-Drive数据集的研究成果，探讨了多种高光谱成像技术在自动驾驶视觉系统中的应用方法，包括如何利用传感器获取的光谱和空间信息开发定制化算法。

Result: 研究分析了多种HSI技术在自动驾驶中的应用技术，基于最新版HSI-Drive数据集的实验结果展示了这些技术如何应对自动驾驶领域的特定挑战。

Conclusion: 高光谱成像在自动驾驶中具有应用潜力，但需要针对该领域的特殊挑战开发合适的技术和算法，HSI-Drive数据集为相关研究提供了重要实验基础。
...
Method: 提出ET-SAM框架，包含：1）轻量级点解码器生成词热图以减少前景点数量；2）联合训练策略整合多级、词级和行级标注数据；3）三组可学习任务提示来缓解数据集差异。

Result: 相比之前的SAM架构，ET-SAM实现了约3倍的推理加速，在HierText上获得竞争性性能，在Total-Text、CTW1500和ICDAR15上平均提升11.0% F-score。

Conclusion: ET-SAM通过减少点提示依赖和有效利用异构标注数据，为统一场景文本检测和布局分析提供了一个高效且性能优越的解决方案。
...
TL;DR: 提出两种层次感知多模态学习方法，通过编码生物分类的层次结构，显著提升大规模生物多样性识别中的分类准确性


<details>
  <summary>Details</summary>
Motivation: 现有生物多样性识别方法通常将分类视为扁平标签空间，忽略了生物分类的层次结构，导致在噪声和缺失模态情况下鲁棒性不足

Method: 提出两种端到端变体：CLiBD-HiR引入层次信息正则化来塑造跨分类级别的嵌入几何；CLiBD-HiR-Fuse额外训练轻量级融合预测器，支持图像、DNA或联合推理

Result: 在大规模生物多样性基准测试中，相比强大多模态基线，分类准确率提升超过14%，在部分和损坏DNA条件下提升尤其显著
...
### [77] [LanteRn: Latent Visual Structured Reasoning](https://arxiv.org/abs/2603.25629)
*André G. Viveiros,Nuno Gonçalves,Matthias Lindemann,André Martins*

Main category: cs.CV

TL;DR: LanteRn框架让大型多模态模型能够在潜在空间进行视觉推理，通过生成和关注连续的视觉思维嵌入，避免了依赖外部模块或直接在像素空间计算的问题。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在视觉推理方面存在挑战，大多数模型默认将感知内容转化为文本，这限制了需要细粒度空间和视觉理解的任务。现有方法要么依赖外部模块，要么在像素空间进行不必要的计算。
...
### [78] [Bilingual Text-to-Motion Generation: A New Benchmark and Baselines](https://arxiv.org/abs/2603.25178)
*Wanjiang Weng,Xiaofeng Tan,Xiangbo Shu,Guo-Sen Xie,Pan Zhou,Hongsong Wang*

Main category: cs.CV

TL;DR: BiHumanML3D是首个双语文本到动作生成基准数据集，通过LLM辅助标注和人工修正构建。提出的BiMD模型采用跨语言对齐策略，在双语输入和零样本代码切换场景中实现高质量动作生成。
...
Result: BiMD模型在BiHumanML3D上取得FID 0.045 vs 0.169，R@3 82.8% vs 80.8%的优异表现，显著优于单语扩散模型和翻译基线，特别是在零样本代码切换场景中。

Conclusion: BiHumanML3D数据集和跨语言对齐策略对跨语言动作合成至关重要且有效，为双语文本到动作生成提供了可靠的基准和解决方案。
...
### [79] [No Hard Negatives Required: Concept Centric Learning Leads to Compositionality without Degrading Zero-shot Capabilities of Contrastive Models](https://arxiv.org/abs/2603.25722)
*Hai X. Pham,David T. Hoffmann,Ricardo Guerrero,Brais Martinez*

Main category: cs.CV

TL;DR: 该论文提出两种简单改进方法解决V&L模型组合性学习不足的问题：使用短概念中心标题和对齐，以及引入参数无关的跨模态注意力池化，在保持零样本和检索能力的同时显著提升组合性任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有对比视觉语言模型在组合性表示学习方面存在局限，传统方法通过生成硬负样本改进但会损害基础能力且缺乏泛化性。作者发现两个根本原因：长训练标题不需要组合性表示，以及全局池化导致绑定信息丢失。

Method: 1) 使用标准NLP工具获取短概念中心标题部分并与图像对齐；2) 引入参数无关的跨模态注意力池化从图像编码器获取概念中心视觉嵌入；3) 使用简单的辅助对比损失。

Result: 在标准组合性基准测试中达到SOTA性能，同时保持或增强了零样本和检索能力，且不增加推理成本。

Conclusion: 通过识别并解决V&L模型组合性学习的两个根本限制，提出了简单有效的改进方法，在提升组合性任务性能的同时保持了模型的基础能力，实现了实用且可泛化的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 使用预训练在短视频上的视频扩散模型生成长视频时，直接应用会导致视觉质量显著下降。主要原因是存在两个分布外问题：帧级相对位置分布外和上下文长度分布外。

Method: 提出FreeLOC框架，包含两个核心技术：1) 视频相对位置重编码(VRPR)解决帧级相对位置分布外问题，采用多粒度策略分层重编码时间相对位置；2) 分层稀疏注意力(TSA)解决上下文长度分布外问题，在不同时间尺度上结构化注意力密度以保持局部细节和长程依赖。还引入了层自适应探测机制，识别每个Transformer层对这些分布外问题的敏感性。

Result: 大量实验表明，该方法显著优于现有的无需训练方法，在时间一致性和视觉质量方面达到了最先进的结果。

Conclusion: FreeLOC通过解决预训练视频扩散模型生成长视频时的分布外问题，无需额外训练即可显著提升长视频生成的质量和一致性，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D医学图像生成方法主要依赖卷积U-Net架构，这些架构存在局部性偏置和感受野有限的问题，限制了可扩展性、全局上下文整合和灵活的条件控制能力。

Method: 提出VolDiT模型：1）通过体积块嵌入和全局自注意力直接处理3D令牌；2）引入时间步门控控制适配器，将分割掩码映射为可学习的控制令牌，在去噪过程中调制Transformer层。

Result: 在高分辨率3D医学图像合成任务中，相比基于U-Net的3D潜在扩散模型，VolDiT表现出更好的全局一致性、更高的生成保真度和更强的可控性。

Conclusion: 完全基于Transformer的扩散模型为体积医学图像合成提供了灵活的基础，展示了在全局建模和可控性方面的优势。
...
TL;DR: AnyID是一个超保真身份保持视频生成框架，通过多参考输入架构和主参考生成范式，解决了现有方法只能处理单一身份参考的限制，实现了高保真身份再现和精确属性控制。


<details>
  <summary>Details</summary>
Motivation: 现有身份保持视频生成方法通常针对单一身份参考进行设计和优化，这限制了创作灵活性，无法适应多样化的现实世界输入格式。单一参考源也构成了一个不适定场景，导致模型在新颖上下文中难以忠实地再现身份。

Method: AnyID提出两个核心贡献：1）可扩展的全参考架构，将异构身份输入（如人脸、肖像、视频）统一为连贯表示；2）主参考生成范式，指定一个参考作为规范锚点，并使用新颖的差分提示实现精确的属性级可控性。在大规模精心策划的数据集上进行训练，最后使用强化学习进行微调。

Result: 广泛的评估验证了AnyID在不同任务设置下实现了超高身份保真度以及卓越的属性级可控性。通过人类评估构建的偏好数据集进行强化学习微调，确保了模型的鲁棒性和高保真度。

Conclusion: AnyID通过创新的架构设计和生成范式，解决了现有身份保持视频生成方法的局限性，为创意表达提供了更强大、更灵活的工具，实现了超高的身份保真度和精确的属性控制能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理电影心脏MRI（CMR）这种具有时间维度的3D医学图像时，通常将空间和时间分解处理或通过解剖掩码等辅助机制强制时间一致性，这些策略引入了结构偏差，可能限制全局上下文整合，导致微妙的时空不连续性或生理上不一致的心脏动力学

Method: 提出CardioDiT，一个完全4D的潜在扩散框架：1）使用时空VQ-VAE将2D+t切片编码为紧凑潜在表示；2）使用扩散变换器将完整3D+t体积作为整体建模，在整个生成过程中耦合空间和时间维度
...
TL;DR: TacSIm是一个用于足球战术风格模仿的大规模数据集和基准测试，专注于准确复制真实世界球队战术行为，而不仅仅是优化得分或胜率等奖励目标。


<details>
  <summary>Details</summary>
Motivation: 当前足球模仿研究主要关注优化基于奖励的目标（如进球数或胜率），较少关注准确复制真实世界的团队战术行为。需要建立一个能够评估战术风格模仿能力的基准。

Method: TacSIm将英超比赛转播画面中所有22名球员的起始位置和动作投影到标准球场坐标系中，提供明确的风格模仿任务和评估协议。使用空间占用相似性和移动向量相似性来测量战术风格模仿。

Result: 通过统一的虚拟环境运行多个基线方法生成完整团队行为，支持定量和视觉评估战术协调性。建立了从转播到模拟的统一数据和度量标准。
...
TL;DR: CIV-DG：利用条件工具变量解决医学AI中由选择偏差引起的跨站点泛化问题，通过解耦病理语义与扫描仪伪影，在放松标准IV方法严格随机分配假设下实现更稳健的医学AI
...
Method: 提出CIV-DG因果框架，利用条件工具变量解耦病理语义与扫描仪诱导的伪影。通过放松标准工具变量方法的严格随机分配假设，适应患者人口统计学内生驱动医院选择的复杂临床场景。采用深度广义矩方法架构，使用条件批评器最小化矩违规并在人口统计学层内强制实施工具-误差正交性。

Result: 在Camelyon17基准测试和大规模胸部X射线数据集上的广泛实验表明，CIV-DG显著优于领先的基线方法，验证了条件因果机制在解决医学AI结构性混杂问题上的有效性。

Conclusion: CIV-DG通过条件工具变量框架成功解决了医学AI中由选择偏差引起的跨站点泛化问题，为处理临床场景中医院选择内生于患者人口统计学的复杂情况提供了有效的因果解决方案。
...
TL;DR: PCGR是一个可解释且可演化的多模态虚假信息检测框架，通过构建概念图并进行层次化注意力推理来检测虚假信息，在准确性和鲁棒性方面达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 多模态虚假信息日益严重，传统检测器存在不透明、黑盒化、对新操纵策略脆弱等问题，需要可解释且能适应新威胁的检测方法。

Method: 采用"先构建后推理"范式：1)构建人类可理解的概念图，包括通过多模态大语言模型自动发现和验证的高层概念；2)在概念图上应用层次化注意力机制进行推理，生成可解释的证据链。

Result: PCGR在多模态虚假信息检测方面达到最先进的准确率，对新兴操纵类型具有更强的鲁棒性，在粗粒度检测和细粒度操纵识别方面均优于现有方法。

Conclusion: PCGR框架通过结构化、基于概念的可解释推理，有效解决了多模态虚假信息检测的挑战，提供了透明且能适应新威胁的解决方案。
...
TL;DR: SDD-YOLO：针对地面到空中反无人机监控的小目标检测框架，通过P2高分辨率检测头、YOLO26架构改进和MuSGD混合训练策略，在DroneSOD-30K数据集上达到86.0% mAP@0.5，推理速度达226 FPS（GPU）和35 FPS（CPU）。


<details>
  <summary>Details</summary>
Motivation: 从地面到空中视角检测小型无人机面临像素占比极低、背景杂乱和实时性要求高等挑战。现有YOLO检测器主要针对通用目标检测，对小目标特征分辨率不足，且部署复杂。

Method: 提出SDD-YOLO框架，包含：1) P2高分辨率检测头（4倍下采样）捕获微小目标细节；2) 集成YOLO26架构改进（无DFL、无NMS的简化推理架构）；3) MuSGD混合训练策略（ProgLoss和STAL）缓解稀疏小目标信号的梯度振荡。

Result: 在DroneSOD-30K数据集（约3万张标注图像）上，SDD-YOLO-n达到86.0% mAP@0.5，比YOLOv5n基线提升7.8个百分点。推理性能：NVIDIA RTX 5090上226 FPS，Intel Xeon CPU上35 FPS。

Conclusion: SDD-YOLO为地面到空中反无人机监控提供了一种高效的小目标检测解决方案，在精度和速度方面均表现出色，适合未来边缘部署。
...
TL;DR: 提出无需训练的多视角6D位姿估计管道，仅需CAD模型作为先验知识，实现毫米级精度的未见手术器械检测与跟踪


<details>
  <summary>Details</summary>
Motivation: 监督方法缺乏对新器械的灵活性且需要大量标注数据，需要开发无需训练即可准确估计未见手术器械6D位姿的方法

Method: 两阶段管道：1) 检测阶段：生成对象掩码提案，使用预训练特征提取器与渲染模板匹配，跨视图匹配并三角化为3D实例候选；2) 位姿估计：迭代精炼位姿假设，使用特征度量评分和跨视图注意力，最终通过多视角遮挡感知轮廓配准最小化重投影误差

Result: 在MVPSP数据集上评估，在受控条件下达到与监督方法相当的毫米级精度，同时保持对未见器械的完全泛化能力

Conclusion: 提出了一种新颖灵活的管道，有效结合最先进的基础模型、多视角几何和基于轮廓的精炼，无需任务特定训练即可实现高精度6D位姿估计，适用于动态临床环境
...
Method: 提出了统一的空间对齐框架（SAF），包含空间对齐（SA）算法，在应用空间变换时同步变换标签和输入，确保空间结构的一致性。该框架将变换分为非空间变换和空间变换，针对结构化任务的特点进行优化。

Result: 实验表明SAF显著提升了TAAs在结构化任务中的攻击效果：在Cityscapes数据集上，mIoU从24.50降至11.34；在Kvasir-SEG数据集上，mIoU从49.91降至31.80；在COCO数据集上，mAP从17.89降至5.25。

Conclusion: 空间对齐对于结构化任务的变换对抗攻击至关重要。提出的SAF框架通过同步变换标签和输入，有效解决了空间错位问题，显著提升了攻击的可迁移性和效果，为结构化任务的对抗攻击提供了新思路。
...
TL;DR: CHIRP数据集和CORVID方法为野生鸟类长期行为监测提供计算机视觉解决方案，通过彩色腿环识别个体，支持多种视觉任务，并引入生物学相关指标评估模型性能。


<details>
  <summary>Details</summary>
Motivation: 长期监测野生动物个体行为对保护生物学和进化生物学至关重要，但野外种群自动化行为监测面临挑战，缺乏覆盖多种计算机视觉任务的数据集来提取有生物学意义的测量数据。

Method: 提出CHIRP数据集（来自瑞典拉普兰野生西伯利亚松鸦种群）支持重识别、动作识别、关键点估计等任务；开发CORVID方法，基于彩色腿环分割和分类进行个体识别，采用概率匹配数据库进行ID追踪。

Result: CHIRP数据集支持多种计算机视觉任务；CORVID方法在生物学相关指标（如摄食率、共现率）上优于现有重识别方法；应用特定基准测试显示模型在实际使用场景中的有效性。
...
TL;DR: DeepFAN是一个基于transformer的模型，用于CT检测肺结节的良恶性分类，通过临床验证能显著提升初级放射科医生的诊断性能。


<details>
  <summary>Details</summary>
Motivation: CT的广泛应用导致检测到的肺结节数量显著增加，但现有深度学习方法在全局和局部特征整合方面不足，且缺乏临床验证。

Method: 开发了基于transformer的DeepFAN模型，在超过10K病理证实的结节上进行训练，并进行了多读者、多病例的临床试验来评估其对初级放射科医生的辅助效果。
...
TL;DR: 研究评估了9个开源多模态大语言模型在面部验证任务中的公平性表现，发现专用面部模型FaceLLM-8B表现最佳，但公平性模式与传统面部识别系统不同。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型被探索用于面部验证任务，但其人口统计学公平性尚未得到充分研究。本研究旨在评估这些模型在不同种族和性别群体中的公平性表现。

Method: 对9个开源MLLM模型（来自6个模型家族，参数规模2B-8B）在IJB-C和RFW面部验证协议上进行基准测试，覆盖4个种族群体和2个性别群体。使用等错误率和多个工作点的真实匹配率测量验证准确性，并使用4个基于FMR的公平性指标量化人口统计学差异。

Result: FaceLLM-8B（研究中唯一的面部专用模型）在两个基准测试中显著优于通用MLLM。观察到的偏见模式与传统面部识别系统不同，不同群体在不同基准和模型中最受影响。最准确的模型不一定最公平，整体准确性差的模型可能因为所有群体都产生高错误率而显得公平。

Conclusion: 多模态大语言模型在面部验证任务中表现出与传统面部识别系统不同的公平性模式，需要专门评估其人口统计学公平性，不能仅依赖整体准确性作为公平性指标。
...
Method: 包含两个轻量级模块：1) 几何重稠密化模块迭代稠密化编码的稀疏几何，提取稠密尺度特征后稀疏化用于预测编码；2) 跨尺度特征传播模块利用多分辨率级别的占用线索指导分层特征传播，实现跨尺度信息共享和减少冗余特征提取。还引入了仅整数推理管道确保跨平台一致性

Result: 实验表明在实时速度下实现了有竞争力的压缩性能

Conclusion: 提出的紧凑表示和轻量级框架解决了LiDAR点云压缩中的稀疏性问题，实现了高效的无损压缩
...
TL;DR: 提出"Just Zoom In"方法，通过自回归缩放城市级卫星地图进行跨视角地理定位，避免对比学习和困难负样本挖掘，在真实基准上达到SOTA性能。
...
Method: 提出"Just Zoom In"方法，将地理定位重新定义为在城市级卫星地图上的自回归缩放过程。模型从粗粒度卫星视图开始，通过一系列缩放决策选择目标分辨率的终端卫星单元，无需对比损失或困难负样本挖掘。

Result: 在包含众包街景和高分辨率卫星图像的真实基准上，Just Zoom In达到最先进性能：在50米范围内Recall@1提升5.5%，在100米范围内Recall@1提升9.6%，优于最强的对比检索基线。

Conclusion: 自回归的从粗到细空间推理方法在跨视角地理定位中表现优异，验证了序列化空间推理的有效性，为GPS受限环境下的定位导航提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有车辆-车辆(V2V)和车辆-基础设施(V2I)协同感知局限于地面协作，无法完全解决大规模遮挡或复杂环境中的长距离感知问题，需要车辆-无人机(V2U)跨视角协同感知来应对这些挑战。

Method: 通过地面车辆和无人机配备多视角LiDAR和RGB相机收集数据，建立包含城市街道、大学校园和乡村道路的多样化交通场景数据集，并建立单智能体3D目标检测、协同3D目标检测和目标跟踪的基准任务。

Result: 数据集包含超过56K LiDAR帧、56K多视角相机图像和700K标注的3D边界框，涵盖四个类别。对多个最先进模型的综合评估表明，V2U协同能有效增强感知鲁棒性和长距离感知能力。

Conclusion: V2U4Real为跨视角协同感知研究提供了首个大规模真实世界数据集，展示了车辆-无人机协同在提升感知鲁棒性和长距离感知方面的有效性，推动了自动驾驶感知系统的发展。
...
TL;DR: 本文提出Hybrid Memory范式解决视频世界模型中动态主体被遮挡后重新出现时的失真问题，构建了HM-World数据集并设计了HyDRA记忆架构，显著提升了动态主体一致性和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型将环境视为静态画布，当动态主体暂时离开视野后重新出现时，常出现主体冻结、扭曲或消失的问题，缺乏对动态主体的持续跟踪能力。

Method: 提出混合记忆范式，要求模型同时作为静态背景的精确档案员和动态主体的警惕跟踪器；构建HM-World数据集（59K高质量视频片段）；设计HyDRA记忆架构，通过令牌压缩记忆并使用时空相关性驱动的检索机制，选择性关注相关运动线索。

Result: 在HM-World数据集上的广泛实验表明，该方法在动态主体一致性和整体生成质量方面显著优于现有最先进方法。

Conclusion: 混合记忆范式有效解决了动态主体被遮挡后的连续性问题，HyDRA架构通过智能记忆管理成功保持了隐藏主体的身份和运动一致性，为视频世界模型的发展提供了新方向。
...
TL;DR: 提出PixelSmile扩散框架解决细粒度面部表情编辑中的语义重叠问题，使用FFE数据集和FFE-Bench评估标准，通过对称联合训练实现表情语义解耦
...
Method: 构建FFE数据集和FFE-Bench评估基准；提出PixelSmile扩散框架，通过完全对称的联合训练解耦表情语义，结合强度监督和对比学习，使用文本潜在插值实现精确稳定的线性表情控制

Result: PixelSmile实现了优越的解耦效果和鲁棒的身份保持，在连续、可控、细粒度的表情编辑方面表现出色，同时自然支持平滑的表情混合

Conclusion: PixelSmile框架有效解决了细粒度面部表情编辑中的语义重叠问题，通过创新的训练方法和评估体系，为连续、可控的表情编辑提供了有效解决方案
...
TL;DR: 该论文提出了MacroData数据集和MacroBench基准，用于解决多参考图像生成中的数据瓶颈问题，显著提升了模型在多个视觉参考条件下的生成性能。


<details>
  <summary>Details</summary>
Motivation: 当前多参考图像生成模型在输入参考数量增加时性能严重下降，根本原因是现有数据集主要包含单参考或少参考图像对，缺乏结构化、长上下文监督来学习密集的参考间依赖关系。

Method: 1. 引入MacroData数据集：包含40万个样本，每个样本最多10张参考图像，系统组织在四个互补维度（定制化、插图、空间推理、时间动态）；2. 提出MacroBench基准：包含4000个样本，评估不同任务维度和输入规模下的生成一致性；3. 通过微调和消融实验验证方法有效性。

Result: 在MacroData上微调显著改善了多参考图像生成性能，消融研究揭示了跨任务协同训练的好处以及处理长上下文复杂性的有效策略。

Conclusion: MacroData数据集和MacroBench基准填补了多参考图像生成领域的数据和评估空白，为处理长上下文复杂性和多参考依赖关系提供了系统解决方案，数据集和基准将公开发布。
...
TL;DR: 本文提出了一种基于头部敏感度评分（HeSS）的两阶段稀疏化方法，用于解决VGGT全局注意力层中二次计算成本问题，通过量化并利用注意力头之间的稀疏化敏感性差异来减少性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏化加速技术在VGGT中虽然能缓解二次计算成本问题，但通常会导致显著的精度下降。作者假设这种性能下降源于注意力头之间稀疏化敏感性的异质性，而现有方法对所有头应用统一的稀疏模式。

Method: 提出两阶段稀疏化流程：1）使用新颖的头部敏感度评分（HeSS）在小规模校准集上量化每个注意力头的稀疏化敏感性，该评分基于两个不同误差项的Hessian近似；2）在推理阶段进行HeSS引导的稀疏化，利用预计算的HeSS重新分配总注意力预算，为敏感头分配更密集的注意力，为更鲁棒的头分配更稀疏的注意力。

Result: 实验证明HeSS能有效捕捉头部稀疏化敏感性，并确认全局注意力层中的注意力头确实表现出异质性敏感性特征。在高度稀疏条件下，该方法能有效缓解性能下降，在不同稀疏化水平下表现出强鲁棒性。

Conclusion: 通过量化并利用注意力头之间的稀疏化敏感性差异，提出的HeSS引导稀疏化方法能有效减少VGGT中全局注意力层稀疏化带来的性能下降，为高效3D视觉模型提供了一种有效的加速方案。
...
TL;DR: InstanceAnimator是一个基于Diffusion Transformer的框架，用于多实例草图视频着色，解决了现有方法在用户控制灵活性、实例对齐和细节保真度方面的限制。
...
Method: 提出三个创新：1）画布引导条件，允许自由放置参考元素和背景；2）实例匹配机制，将实例特征与草图集成确保多角色精确控制；3）自适应解耦控制模块，将角色、背景和文本条件的语义特征注入扩散过程。

Result: 大量实验证明，InstanceAnimator实现了卓越的多实例着色效果，具有增强的用户控制、高视觉质量和强实例一致性。

Conclusion: InstanceAnimator通过创新的画布引导、实例匹配和解耦控制机制，成功解决了多实例草图视频着色中的关键挑战，提供了更灵活的用户控制和更好的视觉质量。
...
Method: 提出关系知识蒸馏框架CLIP-RD，包含两个新方法：垂直关系蒸馏(VRD)在分布层面强制跨模态师生蒸馏强度一致性；交叉关系蒸馏(XRD)对跨模态师生相似度分布施加双向对称性约束

Result: 通过联合建模多向关系结构，CLIP-RD促进学生嵌入几何与教师嵌入几何的忠实对齐，性能优于现有方法0.8个百分点

Conclusion: 关系知识蒸馏框架能有效提升CLIP蒸馏效果，通过建模多向关系依赖促进学生模型更好地保持教师模型的结构关系
...
TL;DR: 提出PTM-ST框架解决多模态数据集蒸馏中教师模型后期训练阶段知识捕获不足的问题，通过分阶段教师建模和基于捷径的轨迹构建策略，显著提升蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以捕获教师模型后期训练阶段复杂动态演化的知识，导致学生模型性能下降和蒸馏数据质量受损。主要挑战包括显著的跨阶段性能差距和不稳定的教师轨迹。
...
Conclusion: PTM-ST框架通过分阶段教师建模和捷径轨迹构建，成功解决了多模态数据集蒸馏中的关键挑战，为高效压缩和知识转移提供了有效解决方案。
...
TL;DR: FSGNet：一种结合频率感知和语义引导机制的轻量级红外小目标检测框架，解决了U-Net中高层特征向浅层传递时的语义退化问题，提升了小目标的精确定位能力。


<details>
  <summary>Details</summary>
Motivation: 虽然基于U-Net架构的红外小目标检测取得了显著进展，但U-Net在将深层特征向浅层传递时存在语义退化问题，限制了小目标的精确定位能力。需要解决这一限制以提高检测性能。

Method: 提出FSGNet框架，包含三个核心模块：1）编码器中的多方向交互注意力模块，用于捕获细粒度和方向性特征；2）多尺度频率感知模块，利用快速傅里叶变换过滤目标相似杂波；3）全局池化模块捕获高层语义信息，通过全局语义引导流传播到各解码器阶段。
...
Result: 在图像分割基准上，PMT达到冻结编码器SOTA性能，速度提升约3倍；在视频分割中，性能与完全微调方法相当，比冻结编码器SOTA模型快8倍

Conclusion: PMT成功平衡了编码器共享性和分割效率，为大规模部署提供了既快速又保持编码器通用性的解决方案
...
TL;DR: 该论文提出了一个分层框架，通过四个渐进复杂级别来提升视觉语言模型的3D空间理解能力，并构建了包含500万图像、4500万对象的自动数据生成管道，开发了结合RGB-D和点云输入的VLM模型，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 为了实现类似人类的3D空间智能，需要让视觉语言模型能够从2D观察中推断3D结构、识别3D空间中的物体属性和关系，并进行高级空间推理。当前VLMs在这方面的能力有限，需要系统性的方法来提升3D空间理解。

Method: 1. 提出分层框架，将3D空间理解分解为四个渐进级别：几何感知到抽象空间推理；2. 构建自动管道处理约500万图像和4500万对象，生成3D空间VQA对用于监督微调；3. 开发RGB-D VLM，将度量尺度点云图作为辅助输入以增强空间理解。

Result: 在多个空间理解和推理基准测试中达到最先进性能，超越了专门的3D空间模型和大型专有系统（如Gemini-2.5-pro和GPT-5）。分析显示分层任务级别之间存在明显依赖关系。
...
TL;DR: VideoWeaver是首个多模态多视角视频到视频翻译框架，通过4D潜在空间和扩散模型实现多视角一致性，支持机器人学习中的世界随机化。


<details>
  <summary>Details</summary>
Motivation: 现有视频到视频翻译方法只能处理单视角，而具身AI任务通常需要多视角同步摄像头。独立应用单视角模型会导致视角间外观不一致，标准Transformer架构由于交叉注意力二次成本无法扩展到多视角设置。

Method: 1. 首先训练为基于流的单视角V2V模型；2. 使用Pi3空间基础模型将所有视角锚定在共享4D潜在空间中；3. 在不同扩散时间步训练视角，使模型学习联合和条件视角分布；4. 支持以现有视角为条件的自回归新视角合成。

Result: 在单视角翻译基准上达到或优于最先进性能，首次实现物理和风格一致的多视角翻译，包括对机器人学习至关重要的挑战性自我中心和异构摄像头设置。

Conclusion: VideoWeaver是首个多模态多视角V2V翻译框架，通过4D潜在空间和扩散时间步训练实现视角一致性，为机器人策略迁移到新环境提供有效工具，支持具身AI任务中的世界随机化。
...
<details>
  <summary>Details</summary>
Motivation: 现有点云配准方法在部分重叠和大错位情况下存在根本性挑战：同时估计变换和对应关系的非凸耦合目标导致启发式方法陷入局部最优，而现有全局求解器由于下界松弛导致收敛时间过长。

Method: 提出DC-Reg框架，核心创新是基于凸差规划推导出变换-分配耦合目标的整体凹下界估计器。不同于依赖逐项松弛的方法，该整体凸差分解捕获了变换参数和对应矩阵之间的联合结构交互，通过搜索框顶点处的高效线性分配问题计算紧密下界。

Result: 在合成数据和3DMatch基准测试中，DC-Reg相比现有全局技术实现了显著更快的收敛速度，对极端噪声和异常值表现出卓越的鲁棒性。在2D相似性和3D刚性配准中均验证了有效性。

Conclusion: DC-Reg通过整体凸差分解和高效下界计算，为具有部分重叠和大错位的点云配准问题提供了一个鲁棒的全局最优框架，显著提升了分支定界搜索效率。
...
TL;DR: CIAR是一个云-设备协作框架，通过设备端自验证和不确定性量化来解决自回归图像生成模型的计算密集问题，实现2.18倍加速和70%云请求减少


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成模型虽然性能接近扩散模型，但其计算密集和序列特性导致设备端部署时延迟过高，阻碍实际应用

Method: 提出云-设备协作框架CIAR：1）设备端令牌不确定性量化器，采用连续概率区间加速处理；2）区间增强解码模块；3）分布对齐训练策略保持视觉保真度

Result: CIAR实现了2.18倍加速，减少70%云请求，同时保持图像质量与现有方法相当

Conclusion: 通过设备端不确定性量化和云-设备协作，CIAR有效解决了自回归图像生成模型的延迟问题，为设备端部署提供了可行方案
...
TL;DR: GridVAD是一个无需训练的零样本视频异常检测框架，使用视觉语言模型生成异常候选描述，通过自洽性整合过滤幻觉，然后使用Grounding DINO和SAM2进行空间定位和时间传播，实现像素级异常分割。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型直接用于视频异常检测存在脆弱性：在没有校准异常先验的情况下，会交替出现漏检和幻觉性误报。问题不在于VLM本身，而在于如何使用它们。VLM应作为异常提议生成器，生成开放集候选描述，然后由专门的空间和时间模块进行定位和跟踪。

Method: 提出"提议-定位-传播"原则，实现为GridVAD训练免费流程：1) VLM在视频片段的网格化表示上进行推理，生成自然语言异常提议；2) 自洽性整合通过保留多个独立采样中重复出现的提议来过滤幻觉；3) Grounding DINO将每个幸存提议锚定到边界框；4) SAM2在异常区间内将其传播为密集掩码。每个片段的VLM调用预算固定为M+1次，与视频长度无关。
...
Conclusion: GridVAD展示了将VLM作为异常提议生成器而非直接检测器的有效性，通过专门的空间和时间模块进行后续处理，实现了无需训练的高效零样本视频异常检测，在多个指标上超越了现有方法。
...
TL;DR: AdaSFormer是一种针对室内单目语义场景补全的串行化Transformer框架，通过自适应感受野、中心相对位置编码和卷积调制层归一化等设计，在复杂室内场景中实现了最先进的性能。
...
Method: 提出了AdaSFormer框架，包含三个关键设计：1) 具有可学习偏移的自适应串行化Transformer，能动态调整感受野；2) 中心相对位置编码，捕捉丰富的空间信息；3) 卷积调制层归一化，桥接卷积和Transformer特征之间的异构表示。

Result: 在NYUv2和Occ-ScanNet数据集上的大量实验表明，AdaSFormer实现了最先进的性能。

Conclusion: AdaSFormer通过创新的串行化Transformer设计有效解决了室内单目语义场景补全的挑战，在复杂室内场景中取得了优异表现，代码已开源。
...
TL;DR: 该研究针对真实世界图像复原任务，构建大规模数据集并训练开源模型，缩小与闭源模型的性能差距，同时提出包含464张真实退化图像的评估基准。


<details>
  <summary>Details</summary>
Motivation: 现有图像复原模型受限于训练数据的规模和分布，对真实世界场景的泛化能力不足。虽然大型图像编辑模型（如Nano Banana Pro）在复原任务中表现出色，但使用这些通用模型需要大量数据和计算成本。
...
Conclusion: 通过构建大规模数据集和训练开源模型，成功缩小了与闭源替代方案的差距，为真实世界图像复原任务提供了有效的解决方案。
...
TL;DR: 该论文提出了一种解决视频生成中"运动-视觉质量困境"的新方法TQD，通过时间步感知的质量解耦，在不依赖完美数据的情况下提升视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型严重依赖同时具备高视觉质量和高运动质量的数据，但研究发现视觉质量和运动强度存在负相关关系，难以获得两者兼备的"黄金数据"，这构成了视频数据策展中的核心挑战。

Method: 提出时间步感知质量解耦(TQD)方法：1) 分析视频扩散模型的分层学习动态，对质量降级样本进行梯度分析；2) 发现质量不平衡数据在适当时间步能产生与黄金数据相似的梯度；3) 引入训练过程中的时间步选择概念；4) 修改数据采样分布，使运动丰富数据偏向更高时间步采样，高视觉质量数据偏向更低时间步采样。

Result: 实验表明：1) TQD能够仅使用分离的不平衡数据进行训练，性能超越使用更好数据的传统训练方法；2) 挑战了视频生成中对完美数据的必要性；3) 在高质量数据上训练时也能提升模型性能；4) 方法在不同数据场景下均有效。

Conclusion: TQD通过时间步感知的质量解耦策略，有效解决了视频生成中的运动-视觉质量困境，不仅降低了对完美数据的依赖，还能在各种数据场景下提升模型性能，为视频生成模型的训练提供了新思路。
...
TL;DR: 提出了首个羽毛球全场比赛密集数据集BFMD，包含19场比赛、1687个回合、16751次击球事件，每击球都有击球描述标注，并开发了基于VideoMAE的多模态描述生成框架。


<details>
  <summary>Details</summary>
Motivation: 现有羽毛球数据集主要关注短片段或特定任务标注，缺乏全场比赛数据和密集多模态标注，难以生成准确的击球描述和进行比赛级分析。

Method: 构建BFMD数据集，包含比赛片段、回合事件和密集回合级多模态标注；开发基于VideoMAE的多模态描述框架，采用语义反馈机制利用击球语义指导描述生成。

Result: 多模态建模和语义反馈相比仅RGB的基线显著提高了击球描述质量；展示了BFMD在全场比赛战术模式时间演化分析方面的潜力。

Conclusion: BFMD数据集填补了羽毛球全场比赛密集标注数据的空白，提出的多模态描述框架有效提升了击球描述质量，为比赛级战术分析提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于学习的方法严重依赖高质量3D数据和人工标注的有监督训练，限制了方法的可扩展性和多样性

Method: 直接从大规模野外第一人称视频中的手-物体交互提取物体关节结构

Result: 在HD-EPIC和Arti4D数据集上相比基线方法取得显著改进，提取的关节结构有益于下游任务

Conclusion: PAWS方法能够从野外视频中有效提取物体关节结构，为3D场景理解、机器人操作等应用提供支持
...
TL;DR: 该研究针对当前地球观测大模型忽视垂直维度的问题，提出了首个高度感知的遥感理解评估框架，包括两个基准数据集和一个基线模型，成功解锁了现有光学模型的高度推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前地球观测大模型通常忽视关键的"垂直"维度，限制了其在复杂遥感几何和灾害场景中的推理能力，因为物理空间结构往往比平面视觉纹理更重要。

Method: 1) 开发了可扩展的VLM驱动数据生成流水线，利用系统提示工程和元数据提取构建两个互补基准：GeoHeight-Bench用于相对高度分析，GeoHeight-Bench+用于更复杂的全地形感知推理；2) 提出了GeoHeightChat，首个高度感知的遥感大模型基线，通过将视觉语义与隐式注入的高度几何特征相结合。

Result: 构建了两个高度感知遥感基准数据集，并验证了GeoHeightChat基线模型能够有效缓解"垂直盲点"，成功解锁了现有光学模型中交互式高度推理的新范式。

Conclusion: 通过结合视觉语义和隐式高度几何特征，可以成功解决地球观测大模型的"垂直盲点"问题，为遥感理解开辟了高度感知推理的新范式。
...
### [116] [UNIC: Neural Garment Deformation Field for Real-time Clothed Character Animation](https://arxiv.org/abs/2603.25580)
*Chengfeng Zhao,Junbo Qi,Yulou Liu,Zhiyang Dou,Minchen Li,Taku Komura,Ziwei Liu,Wenping Wang,Yuan Liu*

Main category: cs.CV

TL;DR: 提出UNIC方法，通过实例特定的神经变形场实时模拟服装变形，避免处理复杂拓扑结构，提高变形质量
...
Method: 学习实例特定的神经变形场，将3D点映射到变形偏移量，避免处理复杂拓扑结构，注入自然平滑约束

Result: 在各种服装网格上进行了广泛实验，证明UNIC在效果和效率上优于基线方法，适合实时交互应用

Conclusion: UNIC方法能够实时高质量地模拟服装变形，具有实际应用价值，特别是在视频游戏等交互场景中
...
TL;DR: Trace框架通过单锚帧轨迹设计实现视频中目标物体运动路径编辑，无需逐帧标注，在相机运动场景下也能保持时空一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法主要关注外观修改，或依赖基于点跟踪的轨迹控制，这在相机运动场景下对用户不友好且难以提供。需要一种实用、易用的可控物体中心运动编辑方法。

Method: 提出Trace框架，采用两阶段流程：1)跨视图运动变换模块将首帧路径设计映射到相机运动下的帧对齐边界框轨迹；2)运动条件视频重合成模块按照轨迹重新生成物体，同时保留输入视频的其余内容。

Result: 在多样化真实世界视频上的实验表明，该方法比最近的图像到视频和视频到视频方法产生更连贯、真实和可控的运动编辑效果。

Conclusion: Trace框架提供了一种实用且用户友好的视频物体运动路径编辑方法，通过单锚帧设计实现可控的物体中心运动编辑，在相机运动场景下表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有真实世界数据集覆盖范围有限，而之前的合成数据集很少大规模提供遮挡、手臂细节和对齐的深度信息，这限制了3D手部姿态估计的性能提升。

Method: 创建了包含丰富几何标注的大规模合成数据集AnyHand，并提出了一个轻量级深度融合模块，可以轻松集成到现有的RGB模型中。

Result: 在RGB-only设置下，使用AnyHand扩展训练集能显著提升多个基准测试的性能；在RGB-D设置下，模型在HO-3D基准上达到最优性能，且表现出强大的跨域泛化能力。

Conclusion: AnyHand数据集有效解决了3D手部姿态估计的数据瓶颈问题，通过大规模合成数据和深度信息融合，显著提升了模型的性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的图像生成基准主要关注自然图像合成，无法系统评估模型在现实商业设计任务中的结构化、多约束要求下的表现，因此需要专门针对商业视觉内容生成的基准测试。

Method: 构建BizGenEval基准，包含5种代表性文档类型（幻灯片、图表、网页、海报、科学图表）和4个关键能力维度（文本渲染、布局控制、属性绑定、知识推理），形成20个评估任务。包含400个精心设计的提示词和8000个人工验证的检查问题。
...
TL;DR: SlotVTG：一种轻量级框架，通过槽注意力将视觉标记分解为抽象槽，引导多模态大语言模型进行以对象为中心的视觉推理，显著提升视频时序定位的跨域泛化能力


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视频时序定位任务中表现出色，但其粗粒度识别能力不足以进行细粒度时序理解，需要任务特定的微调。这种微调导致模型记忆数据集特定的捷径，而非忠实于实际视觉内容进行定位，导致跨域泛化能力差。以对象为中心的学习通过将场景分解为实体级表示提供了有希望的解决方案，但现有方法需要从头重新运行整个多阶段训练流程。

Method: 提出SlotVTG框架，引入轻量级槽适配器，通过槽注意力将视觉标记分解为抽象槽并重建原始序列。利用自监督视觉模型的对象性先验促进语义连贯的槽形成，以最小成本引导MLLMs进行以对象为中心、基于输入的视觉推理。

Result: 在标准VTG基准测试的跨域评估中，SlotVTG方法显著提高了跨域鲁棒性，同时以最小开销保持了有竞争力的域内性能。

Conclusion: SlotVTG是一种高效框架，通过轻量级槽适配器实现以对象为中心的视觉表示，有效解决了MLLMs在视频时序定位中的跨域泛化问题，为细粒度时序理解提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 生成真实的人-物交互动画具有挑战性，需要同时建模动态人体动作和多样物体几何。现有方法依赖手工接触先验或人体运动学约束来改善接触质量，作者希望提出数据驱动的替代方案。

Method: 基于扩散强迫技术，将表示分解为模态特定组件，并为每个组件分配个性化噪声水平和异步去噪调度。在这种范式下，较干净的组件通过交叉注意力引导较嘈杂的组件，实现无需辅助分类器的引导。训练时使用广泛的合成物体几何增强，鼓励接触语义对几何多样性的不变性。

Result: 实验表明，节奏诱导的引导比传统的无分类器引导更能有效模拟接触先验的好处，同时实现更高的接触保真度、更真实的HOI生成，以及对未见物体和任务更强的泛化能力。

Conclusion: LIGHT通过数据驱动的异步去噪调度，实现了有效的接触感知引导，减少了对人工设计先验的依赖，在生成真实人-物交互动画方面表现出色。
...
TL;DR: PSDesigner是一个模仿人类设计师工作流程的自动化平面设计系统，通过专业组件收集主题相关素材并自主推断和执行工具调用，基于CreativePSD数据集训练，在多种设计任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前自动化设计系统通常简化专业工作流程，导致灵活性和直观性有限，无法将用户意图忠实转换为可编辑的设计文件。需要开发能够模拟人类设计师创意流程的系统。

Method: 提出PSDesigner系统，基于多个专业组件构建，能够根据用户指令收集主题相关素材，自主推断和执行工具调用来操作设计文件（如整合新素材或优化劣质元素）。构建CreativePSD数据集，包含大量高质量PSD设计文件，标注了广泛设计场景和艺术风格下的操作轨迹。

Result: 广泛实验表明，PSDesigner在多样化平面设计任务上优于现有方法，使非专业人士能够方便地创建生产质量的设计作品。

Conclusion: PSDesigner通过模拟人类设计师的创意工作流程，解决了现有自动化设计系统灵活性和直观性不足的问题，基于专业数据集训练的工具使用能力使其在多种设计任务中表现出色。
...
Method: 将光流估计构建为全局匹配问题，利用预训练的全局Vision Transformer特征捕获大位移，然后进行少量轻量级迭代细化以提高亚像素精度

Result: 在多个光流基准测试中实现了最先进的零样本性能，在长程点跟踪基准测试中也表现出高度竞争力的零样本性能
...
TL;DR: RefAlign提出了一种通过视觉基础模型对齐参考图像特征的表示对齐框架，解决R2V生成中的复制粘贴伪影和多主体混淆问题，在保持文本可控性的同时提升参考保真度。


<details>
  <summary>Details</summary>
Motivation: 现有R2V方法通过引入额外高层语义或跨模态特征来缓解VAE潜在空间中的像素级信息泄露，但仍难以解决异构编码器特征间的模态不匹配导致的复制粘贴伪影和多主体混淆问题。

Method: 提出RefAlign表示对齐框架，通过参考对齐损失将DiT参考分支特征与视觉基础模型（VFM）的语义空间对齐：拉近同一主体的参考特征和VFM特征以提升身份一致性，同时推开不同主体的对应特征以增强语义区分性。

Result: 在OpenS2V-Eval基准测试中，RefAlign在TotalScore上优于当前最先进方法，验证了显式参考对齐在R2V任务中的有效性。

Conclusion: RefAlign通过简单的训练时对齐策略，在不增加推理开销的情况下，实现了文本可控性和参考保真度之间的更好平衡，为解决R2V生成中的模态不匹配问题提供了有效方案。
...
TL;DR: MuRF提出了一种无需训练的多分辨率融合策略，通过处理图像在多个分辨率下的特征并融合，提升视觉基础模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉基础模型虽然在训练时可以处理不同输入尺寸，但推理时通常只使用单一固定尺度。这忽略了视觉感知的基本特性：不同分辨率提供互补的归纳偏置，低分辨率视图擅长全局语义识别，高分辨率视图则对细粒度细化至关重要。

Method: 提出Multi-Resolution Fusion (MuRF)策略，通过冻结的视觉基础模型处理图像在多个分辨率下的特征，然后将这些特征融合成一个统一的表示。该方法无需训练，适用于多种架构。

Result: MuRF在广泛的计算机视觉任务中表现出有效性，主要应用于DINOv2模型家族，并成功推广到对比学习模型如SigLIP2，证明了其通用性和有效性。

Conclusion: MuRF是一种简单而通用的策略，能够利用多分辨率视图的互补优势，作为视觉表示的基本训练免费增强方法，适用于多种视觉基础模型架构。
...
<details>
  <summary>Details</summary>
Motivation: 现有前馈式3D高斯泼溅方法预测像素对齐基元，导致基元数量随分辨率呈二次增长，这从根本上限制了其可扩展性，使得4K等高分辨率合成变得不可行。

Method: LGTM通过预测紧凑的高斯基元并结合每个基元的纹理，将几何复杂度与渲染分辨率解耦。这种方法使用显著更少的高斯基元，同时支持高分辨率渲染。

Result: LGTM实现了高保真的4K新视角合成，无需逐场景优化，这是前馈方法之前无法达到的能力，同时使用的Gaussian基元数量显著减少。

Conclusion: LGTM克服了分辨率缩放障碍，为前馈式3D高斯泼溅方法提供了可扩展的高分辨率合成解决方案，在保持高质量的同时大幅减少了计算需求。
...
Result: ShotStream能够生成连贯的多镜头视频，具有亚秒级延迟，在单GPU上达到16 FPS。其质量匹配或超越了较慢的双向模型，为实时交互式叙事铺平了道路。

Conclusion: ShotStream通过因果多镜头架构、双缓存记忆机制和两阶段蒸馏策略，成功解决了多镜头视频生成中的交互性限制和延迟问题，实现了高质量的实时交互式叙事视频生成。
...
### [128] [Saranga: MilliWatt Ultrasound for Navigation in Visually Degraded Environments on Palm-Sized Aerial Robots](https://arxiv.org/abs/2603.24699)
*Manoj Velmurugan,Phillip Brush,Colin Balfour,Richard J. Przybyla,Nitin J. Sanket*

Main category: cs.RO

TL;DR: Saranga：一种受蝙蝠启发的低功耗超声波感知系统，使掌上无人机能在视觉退化环境中自主避障导航


<details>
  <summary>Details</summary>
Motivation: 掌上无人机在GPS拒止的复杂环境中导航时，受限于载荷能力，传统视觉和激光雷达传感器在低能见度、雾、黑暗等视觉退化条件下失效，而雷达功耗过高不适用，需要低功耗的替代感知方案

Method: 提出Saranga超声波感知系统：1）采用双声纳阵列定位障碍物；2）物理降噪方法阻挡螺旋桨引起的超声波噪声；3）深度学习去噪方法利用超声波回波的长时域特征在低信噪比（-4.9dB）下提取信号模式；4）使用合成数据生成管道和有限真实噪声数据进行训练

Result: 成功使掌上无人机在浓雾、黑暗、雪天等视觉退化条件下，在包含薄型和透明障碍物的杂乱环境中，仅依靠机载传感和计算实现自主导航

Conclusion: Saranga系统通过物理降噪和深度学习去噪的组合方法，解决了超声波感知在低信噪比环境下的挑战，为掌上无人机在视觉退化条件下的自主导航提供了可行的低功耗解决方案
...
### [129] [FODMP: Fast One-Step Diffusion of Movement Primitives Generation for Time-Dependent Robot Actions](https://arxiv.org/abs/2603.24806)
*Xirui Shi,Arya Ebrahimi,Yi Hu,Jun Jin*

Main category: cs.RO

TL;DR: FODMP是一个快速单步扩散运动基元框架，通过一致性蒸馏将扩散模型压缩到ProDMPs轨迹参数空间，实现高速推理的同时保持时间结构运动能力。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在机器人学习中面临速度与时间结构能力的权衡：动作分块扩散策略推理快但只能预测短片段运动，缺乏时间依赖的运动基元；MPD虽然能生成时间结构运动，但推理延迟过高，不适合实时控制。

Method: 提出FODMP框架，将扩散模型蒸馏到Probabilistic Dynamic Movement Primitives (ProDMPs)轨迹参数空间，使用单步解码器生成运动，通过一致性蒸馏消除推理瓶颈，保留运动基元的时间结构特性。

Result: 在标准操作基准测试中，FODMP比MPD快10倍，比动作分块扩散策略快7倍，同时达到或超过它们的成功率。能够生成快速加减速运动基元，实现实时拦截快速飞行球的任务。

Conclusion: FODMP成功解决了扩散模型在机器人学习中的速度-时间结构权衡问题，实现了高速推理的同时保持时间依赖运动基元能力，适用于闭环视觉控制场景。
...
### [130] [Integrated Multi-Drone Task Allocation, Sequencing, and Optimal Trajectory Generation in Obstacle-Rich 3D Environments](https://arxiv.org/abs/2603.24908)
*Yunes Alqudsi,Murat Makaraci*

Main category: cs.RO

TL;DR: IMD-TAPP是一个端到端框架，将多无人机任务分配、路径规划和轨迹生成集成在一起，用于在复杂3D环境中协调四旋翼团队，通过图搜索、粒子群优化和轨迹优化实现安全高效的协同任务执行。


<details>
  <summary>Details</summary>
Motivation: 在复杂三维环境中协调多无人机团队需要将离散的任务规划（决定哪个无人机服务哪个目标以及顺序）与连续的轨迹合成（确保避碰和动态可行性）有机结合。现有方法往往将这两个问题分开处理，导致次优解或安全风险。

Method: 1. 将工作空间离散化为3D导航图，通过图搜索计算机器人到目标和目标间的避障感知旅行成本；2. 使用注入粒子群优化（IPSO）方案，结合多重线性分配，探索耦合的任务分配/排序替代方案以最小化任务完成时间；3. 将生成的航点路径通过生成-优化例程转换为时间参数化的最小急动度轨迹，迭代验证避障和机器人间距离，违反安全边界时触发重新规划。

Result: 在复杂3D场景的MATLAB模拟中，IMD-TAPP始终能生成动态可行、无碰撞的轨迹，同时实现有竞争力的完成时间。在双无人机服务多个目标的代表性案例中，该方法实现了136秒的最小任务时间，并在整个执行过程中保持了所需的安全约束。

Conclusion: IMD-TAPP提供了一个有效的端到端框架，能够联合处理多无人机任务分配、路径规划和轨迹生成，在复杂三维环境中实现安全、高效的协同任务执行，为多无人机系统在障碍物丰富环境中的应用提供了实用解决方案。
...
### [131] [CROSS: A Mixture-of-Experts Reinforcement Learning Framework for Generalizable Large-Scale Traffic Signal Control](https://arxiv.org/abs/2603.24930)
*Xibei Chen,Yifeng Zhang,Yuxiang Xiao,Mingfeng Fan,Maonan Wang,Guillaume Sartoretti*

Main category: cs.RO

TL;DR: CROSS是一个基于混合专家(MoE)的去中心化强化学习框架，用于通用自适应交通信号控制，通过预测对比聚类和场景自适应MoE模块来提升对多样化交通场景的表示能力和泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的自适应交通信号控制方法通常对所有场景使用单一共享策略，其有限的表示能力难以捕捉多样化的交通动态并泛化到未见环境，特别是在面对不同交叉口拓扑和高度动态的复杂交通需求模式时。
...
Conclusion: CROSS框架通过结合预测对比聚类和混合专家机制，有效解决了自适应交通信号控制在多样化场景中的泛化挑战，为未来智慧城市的交通系统提供了更通用和有效的解决方案。
...
### [132] [COIN: Collaborative Interaction-Aware Multi-Agent Reinforcement Learning for Self-Driving Systems](https://arxiv.org/abs/2603.24931)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体自动驾驶系统能协调自动驾驶车辆减少拥堵、提高安全性和运行效率，但在密集场景中实现高效安全的协作仍面临挑战，特别是复杂的智能体交互问题。

Method: 提出COIN框架，包含CIG-TD3算法（采用CTDE范式），联合优化个体导航目标和全局协作目标；引入双级交互感知中心化评论家架构，捕捉局部成对交互和全局系统级依赖关系。

Result: 在密集城市交通环境中的模拟实验表明，COIN在各种系统规模下，在安全性和效率方面均优于其他先进基线方法，并通过真实机器人演示进一步验证了其有效性。

Conclusion: COIN框架在复杂动态的多智能体自动驾驶场景中表现出优越性，能够实现更准确的全域价值估计和改进的信用分配，促进协作策略学习。
...
### [133] [SABER: A Stealthy Agentic Black-Box Attack Framework for Vision-Language-Action Models](https://arxiv.org/abs/2603.24935)
*Xiyang Wu,Guangyao Shi,Qingzi Wang,Zongxia Li,Amrit Singh Bedi,Dinesh Manocha*

Main category: cs.RO

TL;DR: SABER：一种基于智能体的黑盒攻击方法，通过生成微小但有效的指令编辑来评估VLA模型的鲁棒性，显著降低机器人任务成功率并增加执行长度。
...
Method: SABER采用以智能体为中心的方法，使用GRPO训练的ReAct攻击器，在有限编辑预算下生成字符级、词元级和提示级的小型合理对抗性指令编辑，诱导目标行为退化（包括任务失败、不必要长执行和约束违反增加）。

Result: 在LIBERO基准测试中，针对六个最先进的VLA模型，SABER将任务成功率降低20.6%，将动作序列长度增加55%，将约束违反提高33%，同时比基于GPT的基线方法减少21.1%的工具调用和54.7%的字符编辑。

Conclusion: 微小合理的指令编辑足以显著降低机器人执行性能，而以智能体为中心的黑盒流程为红队测试机器人基础模型提供了实用、可扩展和自适应的方法。
...
### [134] [Wireless bioelectronics for untethered biohybrid robots](https://arxiv.org/abs/2603.24959)
...
<details>
  <summary>Details</summary>
Motivation: 生物混合机器人将活体组织与工程结构结合，但传统有线控制方式存在限制，需要无线控制策略来解决信号传递问题，实现更灵活的生物混合机器人控制。

Method: 通过三种主要策略：1）无线生物电子学利用射频磁场远程传递控制信号刺激肌肉组织；2）无线光电子学通过嵌入式光源远程控制光遗传修饰的肌肉机器人；3）神经肌肉整合利用生物信号转导实现多执行器的选择性控制。

Result: 总结了生物混合机器人控制策略的最新进展，提出了跨领域设计原则，展示了无线控制技术在生物混合机器人领域的应用潜力。

Conclusion: 未来发展方向是神经类器官与生物电子学的共集成，以实现自主、闭环的生物混合机器人系统，推动生物混合机器人向更智能、更自主的方向发展。
...
### [135] [Learning Rollout from Sampling:An R1-Style Tokenized Traffic Simulation Model](https://arxiv.org/abs/2603.24989)
*Ziyan Wang,Peng Chen,Ding Li,Chiwei Li,Qichao Zhang,Zhongpu Xia,Guizhen Yu*

Main category: cs.RO

TL;DR: R1Sim：基于运动令牌熵模式的强化学习交通仿真方法，通过熵引导自适应采样和GRPO优化，实现安全、多样、真实的交通行为模拟


<details>
  <summary>Details</summary>
Motivation: 现有基于下一令牌预测（NTP）的交通仿真方法限制了在次优区域对潜在有价值运动令牌的主动探索，而熵模式为基于运动令牌不确定性的探索提供了有前景的视角

Method: 提出R1Sim令牌化交通仿真策略：1）引入熵引导自适应采样机制，关注高不确定性但高潜力的运动令牌；2）使用安全感知奖励设计指导的组相对策略优化（GRPO）优化运动行为

Result: 在Waymo Sim Agent基准测试中，R1Sim相比最先进方法取得了有竞争力的性能，能够生成真实、安全、多样的多智能体行为

Conclusion: R1Sim通过熵模式驱动的强化学习实现了探索与利用的平衡，为交通仿真提供了新的有效方法，特别是在处理高不确定性运动令牌方面表现出色
...
### [136] [$π$, But Make It Fly: Physics-Guided Transfer of VLA Models to Aerial Manipulation](https://arxiv.org/abs/2603.25038)
...
<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型（如π₀）在地面固定基座机械臂上表现出色，但迁移到空中平台面临挑战，因为地面机械臂的准静态动力学与空中飞行器的欠驱动、高动态特性存在根本性不匹配。

Method: 1) 引入载荷感知引导机制，将载荷约束直接注入策略的流匹配采样过程；2) 使用高斯溅射管道合成导航训练数据以克服数据稀缺；3) 保持预训练基础模型不变，仅通过数据增强和物理引导实现迁移。

Result: 1) 合成数据使导航任务成功率从81%提升到100%；2) 载荷感知引导将真实世界抓取任务成功率从23%提升到50%；3) 在长时域组合任务中达到62%的整体成功率。

Conclusion: 通过适当的数据增强和物理信息引导，预训练的地面机械臂VLA模型可以成功迁移到空中操纵和导航任务，以及这些任务的组合执行。
...
### [137] [ThermoAct:Thermal-Aware Vision-Language-Action Models for Robotic Perception and Decision-Making](https://arxiv.org/abs/2603.25044)
...
<details>
  <summary>Details</summary>
Motivation: 当前人机协作环境中，传感器数据整合多局限于视觉信息，热数据对于提升机器人安全性和操作效率至关重要，但相关研究相对缺乏

Method: 提出新颖的视觉-语言-动作框架，集成热信息；利用视觉语言模型作为高级规划器，解析复杂自然语言指令并分解为子任务；与传统仅依赖视觉数据的方法不同，本方法整合热信息使机器人能感知物理特性并主动确保环境安全

Result: 真实世界任务场景的实验结果验证了所提框架的可行性，相比现有视觉系统，有望提高任务成功率和安全性

Conclusion: 融合热信息的视觉-语言-动作框架能够增强机器人任务执行能力，为人机协作环境提供更安全、更智能的解决方案
...
### [138] [CTS-PLL: A Robust and Anytime Framework for Collaborative Task Sequencing and Multi-Agent Path Finding](https://arxiv.org/abs/2603.25121)
*Junkai Jiang,Yitao Xu,Ruochen Li,Shaobing Xu,Jianqiang Wang*

Main category: cs.RO

TL;DR: CTS-PLL：一个解决协作任务排序和多智能体路径规划问题的新框架，通过锁智能体检测与释放机制以及基于大邻域搜索的优化，在密集环境中实现高成功率


<details>
  <summary>Details</summary>
Motivation: CTS-MAPF问题要求智能体在避免碰撞的同时完成一系列任务，由于其组合复杂性而面临重大挑战。现有方法在密集环境中的鲁棒性和解决方案质量有待提升。

Method: CTS-PLL是一个分层框架，扩展了基于配置的CTS-MAPF规划范式，包含两个关键增强：1) 利用完整规划方法进行局部重新规划的锁智能体检测与释放机制；2) 基于大邻域搜索(LNS)的随时优化过程。

Result: 在稀疏和密集基准测试中的广泛评估表明，CTS-PLL相比现有方法实现了更高的成功率和解决方案质量，同时保持了有竞争力的运行时效率。真实世界机器人实验进一步验证了该方法的可行性。

Conclusion: CTS-PLL通过创新的锁智能体机制和优化过程，有效解决了CTS-MAPF问题，在密集环境中表现出更强的鲁棒性和更好的解决方案质量，为实际机器人应用提供了可行的解决方案。
...
### [139] [A Minimum-Energy Control Approach for Redundant Mobile Manipulators in Physical Human-Robot Interaction Applications](https://arxiv.org/abs/2603.25259)
*Davide Tebaldi,Niccolò Paradisi,Fabio Pini,Luigi Biagiotti*

Main category: cs.RO

TL;DR: 本文提出了一种用于移动机械臂的控制方法，旨在最小化物理人机交互应用中全身移动机械臂的总动能，并通过实验验证了其在钉孔任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 随着移动机械臂在物理人机交互领域的快速发展，需要开发合适的控制方法。移动机械臂引入了额外的自由度，使得控制设计更具挑战性且更需要进行性能优化。

Method: 提出了一种针对移动机械臂的控制方法，该机械臂由移动底座和顶部安装的机械臂组成。该方法旨在最小化物理人机交互应用中全身移动机械臂的总动能。

Result: 通过钉孔任务的实验测试，结果表明所提出的方法减少了全身机器人系统的总动能，并相比基准方法提高了系统性能。

Conclusion: 该控制方法能有效优化移动机械臂在物理人机交互中的性能，减少系统动能，为相关应用提供了可行的解决方案。
...
### [140] [Connectivity-Aware Representations for Constrained Motion Planning via Multi-Scale Contrastive Learning](https://arxiv.org/abs/2603.25298)
*Suhyun Jeon,Yumin Lim,Woo-Jeong Baek,Hyeonseo Kim,Suhan Park,Jaeheung Park*

Main category: cs.RO

TL;DR: 提出一种基于连通性感知表示学习的运动规划方法，通过多尺度流形学习和对比学习框架，在规划前选择位于连通区域的起始和目标配置，避免本质互不连通组件问题，提高成功率和减少规划时间。


<details>
  <summary>Details</summary>
Motivation: 约束运动规划面临三个主要挑战：1）本质互不连通组件导致规划效率低下或不可行；2）约束将可行空间限制到低维子流形；3）冗余性引入额外复杂性，因为单个末端执行器姿态对应无限多逆运动学解。这些问题使得传统规划方法在选择起始和目标配置时容易失败。
...
Result: 在各种操作任务上的实验表明，该方法相比基线实现了1.9倍的成功率提升，规划时间减少了0.43倍。该方法能够有效避免本质互不连通组件问题，显著提高运动规划的成功率和效率。

Conclusion: 通过学习连通性感知表示来偏置起始和目标配置选择，可以有效解决约束运动规划中的本质互不连通组件问题。该方法通过多尺度流形学习和对比学习框架，在规划前识别连通区域，显著提高了规划成功率和效率，为复杂约束下的运动规划提供了有效解决方案。
...
### [141] [SafeGuard ASF: SR Agentic Humanoid Robot System for Autonomous Industrial Safety](https://arxiv.org/abs/2603.25353)
*Thanh Nguyen Canh,Thang Tran Viet,Thanh Tuan Tran,Ben Wei Lim*

Main category: cs.RO

TL;DR: SafeGuard ASF是一个部署人形机器人进行工业危险检测的自主安全系统框架，整合了多模态感知、ReAct智能体推理框架和学习型运动策略，针对火灾烟雾、管道异常温度和入侵者检测三种危险场景。


<details>
  <summary>Details</summary>
Motivation: 随着"黑暗工厂"（无人化工厂）的兴起，需要能够检测和响应多种危险类型的自主安全系统。传统安全系统在无人环境中存在局限性，需要更智能的自主安全解决方案。

Method: 1. 整合多模态感知（RGB-D成像）用于环境感知；2. 采用ReAct-based智能体推理框架进行决策；3. 在Unitree G1人形机器人平台上部署学习型运动策略；4. 使用Unitree RL Lab和PPO算法训练多种运动策略；5. 提出ToolOrchestra行动框架，通过感知、推理和执行工具实现结构化决策。

Result: 1. 感知管道在火灾烟雾检测方面达到94.2% mAP，延迟127ms；2. 运动策略训练在80,000次迭代内稳定收敛；3. 在仿真和真实环境中验证了自主巡逻、人类检测与视觉感知、障碍物避让能力；4. 成功处理三种关键危险场景：火灾烟雾检测、管道异常温度监测、限制区域入侵者检测。

Conclusion: SafeGuard ASF框架为无人化工业环境提供了一种有效的自主安全解决方案，通过整合先进的感知、推理和运动控制技术，实现了多危险类型的检测和响应能力，为"黑暗工厂"的安全运营提供了可靠保障。
...
### [142] [IntentReact: Guiding Reactive Object-Centric Navigation via Topological Intent](https://arxiv.org/abs/2603.25382)
*Yanmei Jiao,Anpeng Lu,Wenhan Hu,Rong Xiong,Yue Wang,Huajin Tang,Wen-an Zhang*

Main category: cs.RO

TL;DR: IntentReact：一种意图条件化的物体中心导航框架，通过将全局拓扑规划编码为低维方向信号（意图），引导局部感知控制，解决物体目标视觉导航中全局拓扑指导与局部感知控制的脱节问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于物体级拓扑地图的方法虽然能实现长视距导航而无需密集几何重建，但存在全局拓扑指导与局部感知驱动控制之间的脱节问题。局部决策仅基于当前自我中心观察，缺乏视野外信息，导致机器人可能持续沿当前方向移动，即使该方向不会减少全局拓扑距离。

Method: 提出IntentReact框架，将全局拓扑指导编码为低维方向信号（称为意图），该意图条件化学习到的航点预测策略，使导航偏向拓扑一致的方向。这种设计使机器人能够在局部观察误导时及时重新定向，引导运动朝向减少全局拓扑距离的方向，同时保持物体中心控制的反应性和鲁棒性。

Result: 通过大量实验评估，相比先前的物体中心导航方法，该框架在导航成功率和执行质量方面均有显著提升。

Conclusion: IntentReact通过紧凑的意图接口有效桥接了全局拓扑规划与反应性物体中心控制之间的鸿沟，提高了物体目标视觉导航的性能和效率。
...
### [143] [UMBRELLA: Uncertainty-aware Multi-robot Reactive Coordination under Dynamic Temporal Logic Tasks](https://arxiv.org/abs/2603.25395)
*Qisheng Zhao,Meng Guo,Hengxuan Du,Lars Lindemann,Zhongkui Li*

Main category: cs.RO

TL;DR: 本文提出UMBRELLA框架，用于协调多机器人系统处理动态移动目标的协作任务，通过结合蒙特卡洛树搜索和不确定性感知的rollout，最小化平均完工时间的条件风险价值。


<details>
  <summary>Details</summary>
Motivation: 现有多机器人协调方法大多假设任务特征是静态的，或在环境变化时简单重新规划，无法有效处理动态移动目标的协作任务。需要一种能够处理目标运动预测不确定性，同时满足时空约束的协调框架。

Method: 提出UMBRELLA框架：1) 使用保形预测(CP)建模目标运动预测的不确定性；2) 结合蒙特卡洛树搜索(MCTS)与不确定性感知的rollout；3) 引入基于CP的度量来引导和加速搜索；4) 采用滚动时域规划方案动态调整任务分配；5) 确保任务间的时空约束，在线执行时仅需部分同步。

Result: 在大规模仿真和硬件实验中，与静态基线相比，平均完工时间减少了23%，其方差减少了71%。框架能够有效处理动态任务规格和运动预测更新，同时确保时空约束。

Conclusion: UMBRELLA框架成功解决了多机器人系统协调动态移动目标协作任务的挑战，通过不确定性建模和高效搜索策略，显著提高了系统性能的稳定性和效率。
...
### [144] [System Design for Maintaining Internal State Consistency in Long-Horizon Robotic Tabletop Games](https://arxiv.org/abs/2603.25405)
...
TL;DR: 该论文提出了一种用于长时程桌面游戏的机器人系统架构，通过显式状态管理、模块化设计和恢复机制来维持内部状态一致性，以麻将作为代表性场景进行验证。


<details>
  <summary>Details</summary>
Motivation: 长时程桌面游戏对机器人系统提出了独特挑战：微小的感知或执行错误会破坏累积的任务状态，在决策模块间传播，最终导致交互失败。现有方法通常关注孤立组件改进，而非系统级设计来维持状态一致性。

Method: 提出集成架构：1) 显式维护感知、执行和交互状态；2) 将高层语义推理与实时感知控制分离；3) 引入带触觉触发恢复的已验证动作原语；4) 添加交互级监控机制检测回合违规和隐藏信息泄露。

Result: 系统实现了完整游戏操作，并对故障模式、恢复效果、跨模块错误传播和硬件算法权衡进行了实证分析。结果显示显式分区、监控状态转换和恢复机制对维持长时间可执行一致性至关重要。

Conclusion: 显式分区、监控状态转换和恢复机制是维持长时程、回合制交互中可执行一致性的关键，而单体或未验证的流水线会导致端到端可靠性下降。该系统为研究长时程交互的系统级设计原则提供了实证平台。
...
### [145] [Visualizing Impedance Control in Augmented Reality for Teleoperation: Design and User Evaluation](https://arxiv.org/abs/2603.25418)
*Gijs van den Brandt,Femke van Beek,Elena Torta*

Main category: cs.RO

TL;DR: 通过AR可视化阻抗控制器的目标位姿及其与机器人末端执行器的位移，为无触觉反馈的遥操作提供直观的力反馈，显著提升接触密集型操作的性能。


<details>
  <summary>Details</summary>
Motivation: 接触密集型遥操作在仅使用运动控制接口且无触觉反馈时面临挑战，操作者无法直接感知或调节接触力，限制了任务性能。

Method: 提出增强现实可视化方法，显示阻抗控制器的目标位姿及其与机器人末端执行器的位移，通过视觉方式传达控制器产生的力，为操作者提供直观的实时反馈。

Result: 在17名参与者的双臂操作研究中，AR可视化使力关键型提升任务的完成时间减少24%，但对滑动任务（精确力控制不太关键）无显著影响。

Conclusion: 通过AR使阻抗目标可见是改善接触密集型遥操作中人机交互的可行方法，无需昂贵的触觉硬件即可提供有效的力反馈。
...
### [146] [Temporally Decoupled Diffusion Planning for Autonomous Driving](https://arxiv.org/abs/2603.25462)
*Xiang Li,Bikun Wang,John Zhang,Jianjun Wang*

Main category: cs.RO

TL;DR: TDDM是一种时间解耦扩散模型，通过噪声掩码范式将轨迹分割为不同噪声水平的片段，利用时间上下文指导轨迹生成，在动态城市环境中平衡即时安全与长期目标。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型将轨迹视为整体实体，忽略了异质时间依赖性——近期计划受瞬时动态约束，远期计划受导航目标约束。需要一种能处理这种时间异质性的轨迹生成方法。

Method: 提出时间解耦扩散模型(TDDM)，采用噪声作为掩码的范式，将轨迹分割为具有独立噪声水平的片段，高噪声视为信息空白，弱噪声作为上下文线索。引入时间解耦自适应层归一化(TD-AdaLN)注入片段特定时间步长，在推理时使用非对称时间分类器自由引导利用弱噪声远期先验指导即时路径生成。

Result: 在nuPlan基准测试中，TDDM接近或超过最先进的基线方法，特别是在具有挑战性的Test14-hard子集上表现出色。

Conclusion: TDDM通过时间解耦的噪声掩码方法有效处理轨迹生成中的异质时间依赖性，在动态城市环境运动规划中平衡了即时安全与长期目标，展示了优越的性能。
...
### [147] [Towards Generalizable Robotic Data Flywheel: High-Dimensional Factorization and Composition](https://arxiv.org/abs/2603.25583)
*Yuyang Xiao,Yifei Zhou,Haoran Wang,Wenxuan Ou,Yuxiao Liu*

Main category: cs.RO

TL;DR: F-ACIL是一个启发式因子感知组合迭代学习框架，通过结构化数据分解和组合泛化来解决机器人学习中的数据多样性和效率问题。


<details>
  <summary>Details</summary>
Motivation: 机器人通用模型面临数据多样性不足和数据效率有限的瓶颈，而收集和整理此类数据的系统策略尚未充分探索。任务多样性源于多个维度上稀疏分布的隐式因素，难以明确定义。

Method: 提出F-ACIL框架，将数据分布分解为结构化因子空间（如物体、动作、环境），基于因子化公式开发因子级数据收集和迭代训练范式，促进高维因子空间上的组合泛化。

Result: 通过大量真实世界实验，F-ACIL在比无策略情况下少5-10倍演示的情况下，实现了超过45%的性能提升。结果表明结构化因子分解为真实世界机器人学习中的高效组合泛化提供了实用途径。

Conclusion: 结构化因子分解为构建可泛化的机器人数据飞轮策略提供了系统研究的新思路，F-ACIL框架能够激发更多关于机器人学习数据策略的系统性研究。
...
### [148] [Accurate Surface and Reflectance Modelling from 3D Radar Data with Neural Radiance Fields](https://arxiv.org/abs/2603.25623)
*Judith Treffler,Vladimír Kubelka,Henrik Andreasson,Martin Magnusson*

Main category: cs.RO

TL;DR: 提出一种基于神经隐式表示的雷达点云3D重建方法，联合建模场景几何和视角相关的雷达强度，在稀疏噪声雷达数据上优于传统方法


<details>
  <summary>Details</summary>
Motivation: 雷达在低能见度环境（雾、烟、灰尘）中比相机和激光雷达更具优势，但雷达数据天生稀疏且噪声大，使得可靠的3D表面重建具有挑战性

Method: 采用神经隐式方法进行雷达点云的3D建图，联合建模场景几何和视角相关的雷达强度；利用内存高效的混合特征编码学习连续符号距离场（SDF）进行表面重建，同时捕获雷达特定的反射特性

Result: 相比现有应用于雷达数据的激光雷达重建方法，该方法产生更平滑、更准确的3D表面重建，并能重建视角相关的雷达强度；当输入点云更稀疏时，神经隐式表示比传统显式SDF和网格化技术产生更真实的表面

Conclusion: 神经隐式表示在稀疏雷达点云的3D重建中表现出优越性，能够有效处理雷达数据的稀疏性和噪声问题，为自动驾驶系统在低能见度环境中的安全运行提供可靠的环境感知能力
...
### [149] [A Mentalistic Interface for Probing Folk-Psychological Attribution to Non-Humanoid Robots](https://arxiv.org/abs/2603.25646)
*Giulio Pisaneschi,Pierpaolo Serio,Estelle Gerbier,Andrea Dan Ryals,Lorenzo Pollini,Mario G. C. A. Cimino*

Main category: cs.RO

TL;DR: 开发了一个实验平台，用于研究人们对非人形机器人意向状态归因，通过保持行为不变但改变解释框架（心理化、目的论、机制化）来研究语言和框架如何影响意向立场的采纳。
...
Method: 开发了一个实验平台，结合模拟机器人、真实任务环境和基于大语言模型的解释层，能够用心理化、目的论和机制化三种不同术语表达相同行为。

Result: 平台提供了一个受控方式来研究语言和框架如何塑造机器人学中的意向立场采纳，通过保持行为恒定而改变解释框架来隔离语言因素的影响。

Conclusion: 该实验平台为研究意向状态归因提供了有效工具，特别关注非人形机器人情境下语言和解释框架对意向立场采纳的影响机制。
...
### [150] [Fast-dVLA: Accelerating Discrete Diffusion VLA to Real-Time Performance](https://arxiv.org/abs/2603.25661)
...
TL;DR: 提出一种新颖的VLA模型微调方法，通过解耦辅助任务训练的两个目标，在参数空间中分离通用能力增强和任务特定分布拟合，从而在保持标准SFT简单性的同时获得辅助训练的性能提升。


<details>
  <summary>Details</summary>
Motivation: 预训练的VLA模型在标准监督微调中往往无法有效提升性能并降低适应成本，而带有辅助训练目标的先进微调方法虽然能提升性能，但会带来显著的计算开销。

Method: 将辅助任务训练的两个目标在参数空间中解耦：增强通用能力和拟合任务特定动作分布。在小规模任务集上使用两种不同训练策略使模型收敛，得到的能力向量与预训练参数合并形成能力增强的元模型，并通过轻量级正交正则化损失增强标准SFT。

Result: 实验结果表明该方法在各种机器人任务中非常有效，合并后的模型在减少计算开销的情况下达到了与辅助微调基线相当的性能。

Conclusion: 该方法成功实现了在保持标准SFT简单性的同时获得辅助训练的性能增强，为VLA模型的高效微调提供了新思路。
...
### [151] [Can Users Specify Driving Speed? Bench2Drive-Speed: Benchmark and Baselines for Desired-Speed Conditioned Autonomous Driving](https://arxiv.org/abs/2603.25672)
*Yuqian Shao,Xiaosong Jia,Langechuan Liu,Junchi Yan*

Main category: cs.RO

TL;DR: 提出了Bench2Drive-Speed基准，用于评估自动驾驶策略在用户指定速度和超车指令下的表现，通过重新标注常规驾驶数据实现速度监督，无需额外复杂数据收集。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶系统忽视了用户自定义速度偏好和超车决策的需求，缺乏评估策略是否忠实遵循用户指令的基准。

Method: 引入用户期望速度和超车/跟车指令作为模型输入；设计速度遵循分数和超车分数等量化指标；构建CustomizedSpeedDataset包含2100个专家标注片段；通过重新标注常规驾驶数据将未来帧速度作为训练目标。

Result: 经适当重新标注后，基于常规驾驶数据训练的模型与基于专家演示训练的模型表现相当；速度跟随功能不影响常规驾驶性能，但执行超车指令仍具挑战性。

Conclusion: 无需额外复杂数据收集即可实现速度监督；用户速度定制可行，但交互性超车行为仍具挑战；为可定制自动驾驶提供了基准和数据集。
...
### [152] [Persistent Robot World Models: Stabilizing Multi-Step Rollouts via Reinforcement Learning](https://arxiv.org/abs/2603.25685)
*Jai Bardhan,Patrik Drozdik,Josef Sivic,Vladimir Petrik*

Main category: cs.RO

TL;DR: 提出了一种通过强化学习后训练方案改进机器人世界模型自回归预测的方法，显著提升了长期视频帧预测的视觉质量


<details>
  <summary>Details</summary>
Motivation: 现有的动作条件机器人世界模型在自回归部署时存在误差累积问题，导致视觉质量迅速下降，需要改进长期预测的稳定性

Method: 1) 引入强化学习后训练方案，在自回归轨迹上训练世界模型而非真实历史数据；2) 设计训练协议生成和比较多个候选变长未来预测；3) 开发高效的多视角视觉保真度奖励函数；4) 采用对比性强化学习目标并证明其收敛保证

Result: 在DROID数据集上建立了新的最先进水平：LPIPS降低14%（外部相机），SSIM提升9.1%（腕部相机），赢得98%的配对比较，在盲人研究中获得80%偏好率

Conclusion: 通过强化学习后训练方案显著提升了机器人世界模型的自回归预测质量，解决了长期预测中的误差累积问题，为机器人任务模拟提供了更可靠的视觉预测能力
...
### [153] [Intelligent Navigation and Obstacle-Aware Fabrication for Mobile Additive Manufacturing Systems](https://arxiv.org/abs/2603.25688)
*Yifei Li,Ruizhe Fu,Huihang Liu,Guha Manogharan,Feng Ju,Ilya Kovalenko*

Main category: cs.RO

TL;DR: 提出了一种将导航与材料沉积耦合的通用移动打印-交付平台，通过实时控制框架实现动态制造环境中移动增材制造机器人的自主决策和高质量打印。


<details>
  <summary>Details</summary>
Motivation: 随着大规模定制需求的增长，制造系统需要更灵活地生产个性化产品。增材制造虽能按需制造定制部件，但其灵活性受限于固定设备布局。移动机器人可以解决这一限制，但动态环境中的障碍物和不平地形会干扰导航稳定性，进而影响打印精度和表面质量。

Method: 开发了一个实时控制框架，将导航与材料沉积过程耦合，而不是分开处理。该框架通过闭环集成传感、移动性和制造，为运动和过程控制提供实时反馈，使移动增材制造机器人能够在动态环境中自主决策，确保安全运动、避障和路径稳定性。

Result: 通过仿真和实际实验验证了该框架对轨迹变化和外部干扰的适应性。耦合的导航和打印使移动增材制造机器人能够规划安全、自适应的轨迹，提高了制造的灵活性和适应性。

Conclusion: 提出的通用移动打印-交付平台成功解决了动态制造环境中移动增材制造机器人的挑战，通过将导航与材料沉积过程耦合，实现了在动态环境中自主决策和高质量打印，为大规模定制生产提供了更灵活的解决方案。
...
### [154] [SoftMimicGen: A Data Generation System for Scalable Robot Learning in Deformable Object Manipulation](https://arxiv.org/abs/2603.25725)
*Masoud Moghani,Mahdi Azizian,Animesh Garg,Yuke Zhu,Sean Huver,Ajay Mandlekar*

Main category: cs.RO

TL;DR: SoftMimicGen是一个用于可变形物体操作的自动化数据生成管道，通过高保真仿真环境生成合成数据集，减少真实世界数据需求
...
Method: 开发了SoftMimicGen自动化数据生成管道，包含一套高保真仿真环境，涵盖多种可变形物体（毛绒玩具、绳子、纸巾、毛巾）和操作行为（高精度穿线、动态挥舞、折叠、拾取放置），支持四种机器人形态

Result: 在任务套件中生成数据集，从数据中训练出高性能策略，并系统分析了数据生成系统

Conclusion: SoftMimicGen填补了合成仿真数据范式在可变形物体操作领域的关键空白，为机器人操作技能的规模化学习提供了有效解决方案
...
### [155] [Drive My Way: Preference Alignment of Vision-Language-Action Model for Personalized Driving](https://arxiv.org/abs/2603.25740)
*Zehao Wang,Huaide Jiang,Shuaiwu Dong,Yuping Wang,Hang Qiu,Jiachen Li*

Main category: cs.RO

TL;DR: Drive My Way (DMW)是一个个性化的视觉-语言-动作驾驶框架，能够根据用户的长期驾驶习惯和实时语言指令进行自适应驾驶。


<details>
  <summary>Details</summary>
Motivation: 人类驾驶行为具有个性化特征，受长期习惯和短期意图影响。现有端到端自动驾驶系统要么优化通用目标，要么依赖固定驾驶模式，缺乏适应个体偏好或理解自然语言意图的能力。

Method: 提出DMW框架，从多个真实驾驶员的个性化驾驶数据集中学习用户嵌入，在规划时以此嵌入为条件，同时利用自然语言指令提供短期指导。

Result: 在Bench2Drive基准测试中的闭环评估显示，DMW改进了风格指令适应能力；用户研究表明，其生成的行为可被识别为每个驾驶员自身的风格。

Conclusion: 个性化是面向人类中心自动驾驶的关键能力，DMW框架成功实现了对用户长期习惯和实时指令的适应。
...
### [156] [Integrating Deep RL and Bayesian Inference for ObjectNav in Mobile Robotics](https://arxiv.org/abs/2603.25366)
*João Castelo-Branco,José Santos-Victor,Alexandre Bernardino*

Main category: cs.RO

TL;DR: 本文提出了一种结合贝叶斯推理与深度强化学习的混合物体搜索框架，用于解决移动机器人在室内环境中的自主物体搜索问题，在部分可观测条件下提高搜索效率和成功率。


<details>
  <summary>Details</summary>
Motivation: 移动机器人在室内环境进行自主物体搜索面临三大挑战：部分可观测性、感知不确定性以及探索与导航效率的权衡。传统概率方法虽然能显式表示不确定性，但依赖人工设计的动作选择启发式；深度强化学习能学习自适应策略，但收敛慢且可解释性有限。

Method: 提出混合物体搜索框架，整合贝叶斯推理与深度强化学习。方法维护目标位置的空间信念地图，通过校准物体检测进行贝叶斯推理在线更新，并训练强化学习策略直接从该概率表示中选择导航动作。

Result: 在Habitat 3.0的真实室内仿真环境中评估，与开发的基线策略比较。在两个室内环境中，所提方法提高了成功率，同时减少了搜索工作量。

Conclusion: 结果支持将贝叶斯信念估计与学习动作选择相结合的价值，能够在部分可观测条件下实现更高效可靠的物体搜索行为。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注静态设置，忽视了多模态知识的动态获取与融合。具体问题包括：1) 知识获取层面难以学习不同模态（特别是动态结构模态）的时间敏感信息；2) 知识融合层面难以学习演化的多模态融合特征。

Method: DyMRL采用动态多空间表示学习方法：1) 将欧几里得、双曲和复数空间的时间特定结构特征集成到关系消息传递框架中，学习深度表示；2) 采用先进的双重融合-演化注意力机制，以对称方式为不同时间戳的不同模态分配动态学习权重。

Result: 在构建的四个多模态时序知识图谱基准上进行的广泛实验表明，DyMRL在事件预测性能上优于最先进的动态单模态和静态多模态基线方法。

Conclusion: DyMRL通过有效获取和融合多模态时序知识，在事件预测任务中表现出色，为解决动态多模态知识表示问题提供了有效方案。
...
### [158] [Experiential Reflective Learning for Self-Improving LLM Agents](https://arxiv.org/abs/2603.24639)
...
Method: 提出经验反思学习框架，通过反思任务轨迹和结果生成启发式规则，捕获可迁移的行动经验；测试时根据当前任务检索相关启发式并注入智能体上下文来指导执行。

Result: 在Gaia2基准测试中，ERL比ReAct基线提升7.8%成功率，在任务完成可靠性方面有大幅提升，优于先前的经验学习方法；选择性检索是关键，启发式比少量轨迹提示更具可迁移性。

Conclusion: 通过反思单次尝试经验来提取可迁移启发式规则，能够实现有效的智能体自我改进，为自主智能体的环境适应提供了新思路。
...
TL;DR: 论文比较了传统超参数优化算法与基于LLM的方法在语言模型超参数调优中的表现，发现传统方法在固定搜索空间中表现更好，但LLM通过直接编辑训练代码在无约束搜索空间中缩小了差距。作者提出了一种混合方法Centaur，结合了CMA-ES和LLM的优势，取得了最佳效果。


<details>
  <summary>Details</summary>
Motivation: 研究动机是比较传统超参数优化算法与基于大型语言模型的方法在超参数调优任务中的表现，特别是在无约束搜索空间中的能力差异，探索如何结合两者的优势来提升优化效果。

Method: 使用autoresearch作为测试平台，比较CMA-ES、TPE等传统HPO算法与基于LLM的方法。LLM方法包括直接编辑训练源代码的无约束搜索空间方法。作者还提出了Centaur混合方法，将CMA-ES的内部状态（均值向量、步长、协方差矩阵）与LLM共享。

Result: 在固定超参数搜索空间中，传统HPO方法（CMA-ES和TPE）持续优于基于LLM的智能体。但LLM智能体通过直接编辑训练源代码在无约束搜索空间中显著缩小了与传统方法的差距。避免内存溢出失败的方法比搜索多样性更高的方法表现更好。Centaur混合方法在实验中取得了最佳结果，其0.8B变体甚至优于27B变体。

Conclusion: 传统优化方法在固定搜索空间中表现更好，但LLM在无约束搜索空间中具有潜力。可靠性和避免失败比探索广度更重要。小型LLM与强大传统优化器结合即可获得良好效果，而无需昂贵的大型模型。混合方法Centaur结合了两者的优势，是有效的解决方案。
...
### [160] [Amplified Patch-Level Differential Privacy for Free via Random Cropping](https://arxiv.org/abs/2603.24695)
...
<details>
  <summary>Details</summary>
Motivation: 研究随机裁剪在训练差分隐私机器学习模型中的作用，特别是当图像中的敏感内容（如人脸或车牌）空间局部化时，随机裁剪可以概率性地排除这些内容，从而增强隐私保护。

Method: 引入视觉数据的补丁级相邻关系，推导随机裁剪与差分隐私随机梯度下降（DP-SGD）结合时的严格隐私边界，量化补丁包含概率，并分析其与小批量采样的组合效果。

Result: 分析表明补丁级隐私放大改善了多个分割架构和数据集上的隐私-效用权衡，证明将隐私计算与领域结构和现有随机源对齐可以在不增加成本的情况下提供更强的隐私保证。

Conclusion: 随机裁剪的固有随机性可以作为差分隐私训练的第三个随机源，与梯度噪声和小批量采样一起，通过概率性地排除局部敏感内容来增强隐私保护，这为计算机视觉中的隐私保护提供了新的视角。
...
### [161] [Training LLMs for Multi-Step Tool Orchestration with Constrained Data Synthesis and Graduated Rewards](https://arxiv.org/abs/2603.24709)
...
TL;DR: 论文提出一个解决多步工具编排挑战的强化学习框架，通过真实API响应缓存构建环境，并设计分级奖励机制来提升LLM在复杂工作流中的执行准确性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在多步工具编排（需要按正确顺序调用多个依赖API并传递中间输出）方面存在挑战，现有模型在完整序列执行中经常失败，参数值错误占失败的大部分。训练模型处理此类工作流面临两个障碍：现有环境专注于简单的单轮函数调用和模拟数据，二元奖励无法提供部分正确性的信号。

Method: 提出一个包含两个核心组件的框架：1）构建基于大规模真实API响应缓存的强化学习环境，支持数据合成管道，可采样具有可控复杂度的有效多步编排轨迹，生成效率显著高于无约束方法；2）设计分级奖励机制，将正确性分解为原子有效性（在递增粒度上的单个函数调用正确性）和编排（尊重依赖关系的正确工具序列）。

Result: 在ComplexFuncBench基准测试中，该方法在轮次准确性方面显示出显著改进。消融研究证实两个奖励组件都是必需的：单独使用任一组件的性能都会显著下降。

Conclusion: 该框架通过结合真实API响应缓存环境和分级奖励设计，有效解决了多步工具编排的挑战，为训练LLM处理复杂工作流提供了更有效的解决方案。
...
### [162] [Can an Actor-Critic Optimization Framework Improve Analog Design Optimization?](https://arxiv.org/abs/2603.24714)
*Sounak Dutta,Fin Amin,Sushil Panda,Jonathan Rabe,Yuejiang Wen,Paul Franzon*

Main category: cs.LG

TL;DR: 提出了一种用于模拟电路尺寸优化的演员-评论家框架（ACOF），通过分离提议和评估角色来模拟设计师的决策过程，相比现有方法显著提升了性能指标。


<details>
  <summary>Details</summary>
Motivation: 模拟电路设计过程缓慢，因为即使对器件尺寸或偏置的微小改动也需要昂贵的仿真周期，高质量解决方案通常只占据庞大搜索空间的狭窄部分。现有优化器虽然减轻了部分负担，但缺乏设计师在决定下一步搜索方向时使用的判断力。

Method: 提出了演员-评论家优化框架（ACOF），将优化过程分为提议和评估两个角色：演员负责建议设计空间中有前景的区域，评论家则评估这些选择、强制执行设计合法性，并在进展受阻时重新定向搜索。该框架保持了与标准仿真流程的兼容性。

Result: 在测试电路中，ACOF将前10个性能指标（FoM）平均提高了38.9%，平均减少了24.7%的遗憾值。在单个电路上，峰值增益达到70.5%的FoM提升和42.2%的遗憾值降低。

Conclusion: 通过将迭代推理与仿真驱动搜索相结合，ACOF为自动化模拟电路尺寸优化提供了一条更透明的路径，使搜索过程更加审慎、稳定和可解释。
...
### [163] [Grokking as a Falsifiable Finite-Size Transition](https://arxiv.org/abs/2603.24746)
*Yuda Bi,Chenyu Zhang,Qiheng Wang,Vince D Calhoun*

Main category: cs.LG

TL;DR: 该研究为"顿悟"现象提供了可检验的有限尺寸相变证据，通过将群阶p作为扩展变量，使用谱头尾对比作为序参数，发现了共享的有限尺寸边界，排除了平滑交叉解释。


<details>
  <summary>Details</summary>
Motivation: 虽然"顿悟"现象常被用相变语言描述，但缺乏可证伪的有限尺寸输入。本研究旨在为这种描述提供定量检验的基础，而不仅仅是类比。

Method: 将群阶p作为可接受的扩展变量，使用保留的谱头尾对比作为表示层面的序参数，应用凝聚态物理风格的诊断链，包括粗网格扫描和密集近临界加法审计，使用类似Binder交叉的方法。

Result: Binder-like交叉揭示了共享的有限尺寸边界，敏感性比较强烈反对平滑交叉解释（在近临界审计中ΔAIC=16.8）。相变语言在顿悟现象中可以作为定量有限尺寸主张进行检验。

Conclusion: 顿悟现象的相变描述可以从单纯的类比提升为可定量检验的有限尺寸主张，尽管目前过渡阶数仍未解决。
...
### [164] [Light Cones For Vision: Simple Causal Priors For Visual Hierarchy](https://arxiv.org/abs/2603.24753)
*Manglam Kartik,Neel Tushar Shah*

Main category: cs.LG

TL;DR: 论文提出Worldline Slot Attention模型，将物体建模为时空世界线中的持久轨迹，不同层级共享空间位置但时间坐标不同。研究发现洛伦兹几何比欧几里得几何在层次物体发现上表现更好，表明视觉层次需要因果结构而非树结构。


<details>
  <summary>Details</summary>
Motivation: 标准视觉模型将物体视为欧几里得空间中的独立点，无法捕捉部分与整体之间的层次结构。现有方法缺乏对层次关系的有效建模。

Method: 引入Worldline Slot Attention，将物体建模为时空世界线中的持久轨迹，每个物体在不同层次级别有多个槽位，共享相同的空间位置但具有不同的时间坐标。比较了欧几里得世界线和洛伦兹世界线的性能。

Result: 欧几里得世界线准确率仅为0.078，低于随机机会(0.33)；而洛伦兹世界线在三个数据集上达到0.479-0.661的准确率，提升了6倍。洛伦兹几何也优于双曲嵌入，表明视觉层次需要因果结构而非树结构。

Conclusion: 层次物体发现需要编码非对称因果关系的几何结构，这是欧几里得空间缺乏但洛伦兹光锥自然具备的归纳偏置。该架构仅使用11K参数就实现了这一目标。
...
### [165] [Transformers in the Dark: Navigating Unknown Search Spaces via Bandit Feedback](https://arxiv.org/abs/2603.24780)
*Jungtaek Kim,Thomas Zeng,Ziqian Lin,Minjae Lee,Chungpa Lee,Jy-yong Sohn,Hyung Il Koo,Kangwook Lee*

Main category: cs.LG

TL;DR: Transformer架构理论上能够近似搜索算法，通过训练可以实施不同的搜索策略，并能泛化到未见过的条件，如更长的搜索视野或更深的树结构。


<details>
  <summary>Details</summary>
Motivation: 虽然外部搜索算法能增强大语言模型的问题解决能力，但需要外部组件会使整体流程复杂化。因此研究LLM或其底层Transformer架构是否能够近似搜索算法。

Method: 引入"未知树搜索与老虎机反馈"的简化框架，其中树扩展和反馈信号由外部指定。在此框架下训练Transformer模型从零开始近似不同的搜索策略。

Result: Transformer模型在理论上具有足够的表达能力来实施不同的搜索策略，能够泛化到更长的搜索视野或更深的树结构。通过任务导向的持续训练，预训练LLM能够完全发挥其搜索能力。

Conclusion: LLM和Transformer架构能够近似搜索算法，这为简化问题解决流程提供了可能，通过训练可以使模型具备自主搜索能力，减少对外部组件的依赖。
...
### [166] [A Practical Guide Towards Interpreting Time-Series Deep Clinical Predictive Models: A Reproducibility Study](https://arxiv.org/abs/2603.24828)
*Yongda Fan,John Wu,Andrea Fitzpatrick,Naveen Baskaran,Jimeng Sun,Adam Cross*

Main category: cs.LG

TL;DR: 本文提出了一个全面的基准测试，用于评估不同临床预测任务和模型架构中的可解释性方法，发现注意力机制在正确使用时是高效的可解释方法，而某些黑盒解释器在时间序列任务中计算不可行。


<details>
  <summary>Details</summary>
Motivation: 临床决策具有高风险性且需要明确理由，因此在部署深度临床模型前，模型可解释性对于审计至关重要。随着模型架构和可解释性方法的生态系统扩展，关键问题仍然存在：注意力等架构特征是否能改善可解释性？可解释性方法能否跨临床任务泛化？现有基准测试缺乏可扩展性和可重复性，且未能系统性地研究可解释性在临床任务和模型架构交互中的变化。

Method: 提出了一个全面的基准测试，通过PyHealth开源框架评估可解释性方法在多样化临床预测任务和模型架构中的表现。系统性地研究了可解释性方法在不同临床任务和模型架构组合中的性能。

Result: 分析发现：(1) 注意力机制在正确使用时是高效且忠实解释模型预测的方法；(2) 黑盒解释器如KernelSHAP和LIME在时间序列临床预测任务中计算不可行；(3) 多种可解释性方法可靠性不足，难以信任。

Conclusion: 基于研究发现，提出了改进临床预测流程中可解释性的若干指导原则。通过PyHealth开源框架提供实现，支持可重复性和可扩展性，为临床AI模型的可解释性评估提供了系统化基准。
...
### [167] [Flow matching on homogeneous spaces](https://arxiv.org/abs/2603.24829)
*Francesco Ruscelli*

Main category: cs.LG

TL;DR: 提出一个将流匹配扩展到齐次空间（李群商空间）的通用框架，通过将数据分布提升到李群上，避免处理齐次空间的复杂几何，最终简化为李代数上的欧几里得流匹配任务。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配方法在处理齐次空间（如李群的商空间）时面临几何复杂性挑战，需要定义和计算预度量或测地线，计算成本高且复杂。

Method: 通过将齐次空间上的数据分布提升到其底层李群上，将问题转化为李群上的流匹配任务，然后进一步简化为李代数上的欧几里得流匹配，避免了直接处理齐次空间的复杂几何。

Result: 相比黎曼流匹配方法，该方法消除了定义和计算预度量或测地线的需求，实现了更简单、更快、完全内蕴的框架。

Conclusion: 提出的框架为在齐次空间上应用流匹配提供了一种通用且高效的方法，通过将问题简化为李代数上的欧几里得流匹配，显著降低了计算复杂度和实现难度。
...
### [168] [Reaching Beyond the Mode: RL for Distributional Reasoning in Language Models](https://arxiv.org/abs/2603.24844)
*Isha Puri,Mehul Damani,Idan Shenfeld,Marzyeh Ghassemi,Jacob Andreas,Yoon Kim*

Main category: cs.LG

TL;DR: 该论文提出了一种多答案强化学习方法，训练语言模型在推理过程中对多个答案进行分布推理，避免后训练过程将答案分布坍缩到单一主导模式。


<details>
  <summary>Details</summary>
Motivation: 现实世界任务（如医疗诊断、模糊问题回答、信息不完整场景）通常涉及多个有效答案或不可约的不确定性，需要语言模型生成多个合理假设并给出置信度估计，而传统方法往往将答案分布坍缩到单一模式。
...
Conclusion: 多答案RL是一种原则性且计算高效的替代方案，可替代推理时缩放程序（如best-of-k），使语言模型能够更好地处理具有多个有效答案的现实世界任务。
...
### [169] [Learning to Staff: Offline Reinforcement Learning and Fine-Tuned LLMs for Warehouse Staffing Optimization](https://arxiv.org/abs/2603.24883)
*Kalle Kujanpää,Yuying Zhu,Kristina Klinkner,Shervin Malmasi*

Main category: cs.LG

TL;DR: 研究比较了两种机器学习方法优化半自动化仓库分拣系统实时人员配置决策：基于Transformer的离线强化学习和基于LLM的方法，两者都能提升吞吐量并支持AI辅助决策。


<details>
  <summary>Details</summary>
Motivation: 研究旨在优化半自动化仓库分拣系统的实时人员配置决策，探索不同抽象层次的AI辅助决策方法，以提升运营效率并实现显著的成本节约。

Method: 采用两种方法：1) 使用详细历史状态表示训练自定义Transformer策略的离线强化学习；2) 基于抽象化、人类可读状态描述的LLM方法，系统比较提示技术、自动提示优化和微调策略。

Result: 离线强化学习在学习的模拟器中实现2.4%吞吐量提升；LLM方法通过监督微调结合Direct Preference Optimization，在手工制作的模拟器中达到或略超历史基线性能。
...
### [170] [CVA: Context-aware Video-text Alignment for Video Temporal Grounding](https://arxiv.org/abs/2603.24934)
*Sungho Moon,Seunghun Lee,Jiwan Seo,Sunghoon Im*

Main category: cs.LG

TL;DR: CVA框架通过数据增强、对比损失和层次化Transformer架构，解决了视频时序定位中背景干扰导致的假阴性问题，在多个基准测试中实现了约5个点的R1提升。


<details>
  <summary>Details</summary>
Motivation: 解决视频时序定位中的关键挑战：在存在无关背景内容的情况下，实现时间敏感的、对背景干扰鲁棒的视频-文本对齐。

Method: 1. 查询感知上下文多样化(QCD)：基于视频-文本相似性构建替换片段池，防止查询无关混合导致的假阴性；2. 上下文不变边界判别(CBD)损失：在挑战性时间边界上强制语义一致性的对比损失；3. 上下文增强Transformer编码器(CTE)：结合窗口自注意力和双向交叉注意力的层次化架构。

Result: 在QVHighlights和Charades-STA等主要VTG基准测试中达到最先进性能，Recall@1(R1)分数比现有最优方法提升约5个点。

Conclusion: 通过数据中心和架构增强的协同作用，CVA框架有效缓解了假阴性问题，显著提升了视频时序定位中对背景干扰的鲁棒性。
...
### [171] [Optimal High-Probability Regret for Online Convex Optimization with Two-Point Bandit Feedback](https://arxiv.org/abs/2603.25029)
*Haishan Ye*

Main category: cs.LG

TL;DR: 该论文解决了在线凸优化中两点反馈的强凸函数高概率遗憾界问题，首次实现了最优的O(d(log T + log(1/δ))/μ)遗憾界。


<details>
  <summary>Details</summary>
Motivation: 在线凸优化中，两点反馈虽然允许梯度估计，但对于强凸函数实现紧致的高概率遗憾界仍然是一个开放问题，主要挑战在于带反馈梯度估计器的重尾特性使得标准浓度分析变得困难。

Method: 论文通过解决带反馈梯度估计器的重尾特性问题，开发了新的分析方法来实现强凸函数的高概率遗憾界。

Result: 首次实现了μ-强凸损失的高概率遗憾界O(d(log T + log(1/δ))/μ)，该结果在时间范围T和维度d方面都是极小极大最优的。

Conclusion: 该研究解决了在线凸优化中两点反馈强凸函数高概率遗憾界的开放问题，实现了最优的理论结果。
...
### [172] [Epistemic Compression: The Case for Deliberate Ignorance in High-Stakes AI](https://arxiv.org/abs/2603.25033)
*Steffen Lukas*

Main category: cs.LG

TL;DR: 论文提出"认知压缩"原则：模型复杂度应与数据时效性匹配，而非盲目扩展参数。通过"制度指数"区分稳定与不稳定领域，在高风险领域实现86.7%的预测准确率。


<details>
  <summary>Details</summary>
Motivation: 基础模型在稳定环境中表现优异，但在需要高度可靠性的医疗、金融和政策等领域存在"保真度悖论"。问题不仅是数据问题，更是结构性问题——在规则随时间变化的领域，额外模型容量会放大噪声而非捕捉信号。

Method: 提出"认知压缩"原则，强调模型复杂度应与数据"保质期"匹配。不同于传统正则化方法，认知压缩通过架构设计实现简约性，使模型结构本身难以表示超出数据证据的方差。使用"制度指数"将领域分为"变化制度"（不稳定、数据贫乏）和"稳定制度"（不变、数据丰富）。

Result: 在15个高风险领域的探索性综合研究中，制度指数与经验上优越的建模策略在86.7%的案例中（13/15）保持一致。

Conclusion: 高风险AI需要从为扩展而扩展转向有原则的简约性。模型设计应根据领域稳定性特征进行匹配，而非盲目追求参数规模。
...
### [173] [Intern-S1-Pro: Scientific Multimodal Foundation Model at Trillion Scale](https://arxiv.org/abs/2603.25040)
...
TL;DR: Intern-S1-Pro是首个万亿参数科学多模态基础模型，在通用和科学领域全面增强，具备更强的推理、图文理解和智能体能力，掌握100+科学专业任务，通过XTuner和LMDeploy基础设施实现高效万亿级RL训练。


<details>
  <summary>Details</summary>
Motivation: 构建首个万亿参数级别的科学多模态基础模型，突破现有模型规模限制，实现通用智能与专业科学能力的深度融合，解决科学领域复杂任务的专业需求。

Method: 采用XTuner和LMDeploy基础设施支持万亿参数规模的高效强化学习训练，确保训练与推理的精度一致性，同时扩展模型在化学、材料、生命科学、地球科学等领域的专业任务能力。

Result: 模型在通用能力方面达到开源模型顶级水平，在专业科学任务深度上超越专有模型，掌握100+科学专业任务，实现通用智能与专业能力的有效融合。

Conclusion: Intern-S1-Pro作为"可专业化的通才"，成功融合了通用智能与专业科学能力，在万亿参数规模上实现了多模态科学基础模型的突破，为科学AI应用提供了强大支持。
...
### [174] [SIGMA: Structure-Invariant Generative Molecular Alignment for Chemical Language Models via Autoregressive Contrastive Learning](https://arxiv.org/abs/2603.25062)
*Xinyu Wang,Fei Dou,Jinbo Bi,Minghu Song*

Main category: cs.LG

TL;DR: SIGMA通过token级对比学习解决分子图到字符串表示的多对一映射问题，结合IsoBeam搜索消除推理中的同构冗余，实现序列可扩展性与图保真度的统一。


<details>
  <summary>Details</summary>
Motivation: 线性化字符串表示作为可扩展自回归分子生成的基础，但存在模态不匹配问题：单个分子图对应多个不同序列。这种模糊性导致轨迹发散，即结构相同的部分图因线性化历史不同而使潜在表示漂移。

Method: 提出SIGMA方法，不改变线性表示，而是通过token级对比目标使模型严格识别几何对称性，显式对齐具有相同后缀的前缀的潜在状态。同时引入IsoBeam搜索，在推理过程中通过动态剪枝等效路径消除同构冗余。

Result: 在标准基准测试中，SIGMA在序列可扩展性和图保真度之间架起桥梁，相比强基线在多参数优化中展现出更优的样本效率和结构多样性。

Conclusion: SIGMA方法成功解决了线性化分子表示中的轨迹发散问题，通过对比学习和动态剪枝实现了高效且结构保真的分子生成，为可扩展的自回归分子生成提供了有效解决方案。
...
### [175] [Process-Aware AI for Rainfall-Runoff Modeling: A Mass-Conserving Neural Framework with Hydrological Process Constraints](https://arxiv.org/abs/2603.25093)
*Mohammad A. Farmani,Hoshin V. Gupta,Ali Behrangi,Muhammad Jawad,Sadaf Moghisi,Guo-Yue Niu*

Main category: cs.LG

TL;DR: MCP模型通过逐步嵌入水文物理过程约束，在保持物理可解释性的同时提升降雨径流预测性能


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在水文应用中预测精度高但缺乏物理可解释性，需要开发既能保持物理约束又能从数据中学习的AI框架

Method: 使用质量守恒感知器(MCP)框架，从最小化MCP开始，逐步引入有界土壤储水、状态相关导水性、可变孔隙度、入渗能力、地表积水、垂直排水和非线性地下水位动态等水文过程表征

Result: 逐步增强MCP内部物理结构通常能改善预测性能，过程表征的影响具有强烈的水文气候依赖性：垂直排水在干旱和雪主导流域显著提升模型技能，但在降雨主导区域降低性能；最佳MCP配置接近LSTM基准的预测技能

Conclusion: 在水文AI架构中嵌入过程约束为可解释且过程感知的降雨径流建模提供了有前景的途径
...
### [176] [Gap Safe Screening Rules for Fast Training of Robust Support Vector Machines under Feature Noise](https://arxiv.org/abs/2603.25221)
*Tan-Hau Nguyen,Thu-Le Tran,Kien Trung Nguyen*

Main category: cs.LG

TL;DR: 该研究为鲁棒支持向量机(R-SVMs)开发了安全样本筛选规则，在不影响最优解的情况下降低训练复杂度，首次将安全筛选技术应用于监督机器学习中的最坏情况鲁棒模型。


<details>
  <summary>Details</summary>
Motivation: 鲁棒支持向量机通过采用最坏情况鲁棒公式来处理特征噪声，提高了可靠性，但同时也增加了计算成本。需要开发方法在保持鲁棒性的同时降低训练复杂度。

Method: 基于拉格朗日对偶性而非常用的Fenchel-Rockafellar对偶性，推导出安全样本筛选规则。首先建立理想筛选规则，然后通过将基于GAP的安全区域适应到鲁棒设置中，推导出实用规则。识别那些不确定性集完全位于间隔超平面任一侧的训练样本。

Result: 实验表明，所提出的方法显著减少了训练时间，同时保持了分类准确性。这是首次将安全筛选技术应用于监督机器学习中的最坏情况鲁棒模型。

Conclusion: 该研究成功开发了鲁棒支持向量机的安全样本筛选规则，有效解决了鲁棒性与计算效率之间的权衡问题，为鲁棒机器学习模型的实用化提供了重要工具。
...
### [177] [How Pruning Reshapes Features: Sparse Autoencoder Analysis of Weight-Pruned Language Models](https://arxiv.org/abs/2603.25325)
...
<details>
  <summary>Details</summary>
Motivation: 权重剪枝是压缩大语言模型的标准技术，但其对学习到的内部表征的影响仍不清楚。本文旨在系统研究非结构化剪枝如何重塑语言模型的内部特征几何结构。

Method: 使用稀疏自编码器作为可解释性探针，在三个模型家族、两种剪枝方法和六个稀疏度水平下，研究了五个研究问题：种子稳定性、特征生存、SAE可迁移性、特征脆弱性和因果相关性。

Result: 最显著的发现是罕见SAE特征（低激活率）比高频特征更能抵抗剪枝，17个实验条件中有11个显示出rho=-1.0的Spearman相关性。Wanda剪枝比幅度剪枝能更好地保留特征结构（最多3.7倍），预训练的SAE在Wanda剪枝模型上可工作到50%稀疏度，但几何特征生存不能预测因果重要性。

Conclusion: 剪枝起到隐式特征选择作用，优先破坏高频通用特征而保留专门的罕见特征。特征几何生存与因果重要性之间存在解离，这对压缩下的可解释性有重要影响。
...
### [178] [From Intent to Evidence: A Categorical Approach for Structural Evaluation of Deep Research Agents](https://arxiv.org/abs/2603.25342)
*Shuoling Liu,Zhiquan Tan,Kun Yi,Hui Wu,Yihan Li,Jiangpeng Yan,Liyuan Chen,Kai Chen,Qiang Yang*

Main category: cs.LG

TL;DR: 该研究通过范畴论形式化深度研究智能体行为，创建了包含296个问题的机制感知基准，测试了11个领先模型，发现即使在最佳模型上平均准确率也仅为19.9%，揭示了当前AI在复杂结构信息处理上的重大局限性。


<details>
  <summary>Details</summary>
Motivation: 当前深度研究智能体的评估主要依赖临时性经验基准，这些启发式方法无法严格建模智能体行为，也无法充分测试长期合成和歧义解决能力。需要建立更严谨的理论框架和评估方法来填补这一空白。

Method: 使用范畴论形式化深度研究智能体行为，将深度研究工作流建模为结构保持映射（函子）的组合。基于此理论框架，开发了包含296个问题的机制感知基准，沿四个可解释轴进行压力测试：遍历顺序连接链、验证V结构拉回中的交集、对检索子结构施加拓扑排序、通过Yoneda探针进行本体证伪。

Result: 对11个领先模型的严格评估显示持续低基线，最先进模型平均准确率仅为19.9%，暴露了形式结构压力测试的难度。研究发现当前AI能力存在明显二分：先进深度研究管道能成功重新定义动态拓扑重排序并展示稳健的本体验证能力，但在多跳结构合成方面几乎普遍崩溃。任务间巨大性能差异表明系统仍依赖脆弱的启发式方法而非系统性理解。

Conclusion: 虽然顶级自主智能体现在能够有机统一搜索和推理，但实现对复杂结构信息的广义掌握仍然是一个艰巨的开放挑战。该工作为深度研究智能体评估提供了理论基础和严格基准，揭示了当前系统的局限性。
...
### [179] [Hessian-informed machine learning interatomic potential towards bridging theory and experiments](https://arxiv.org/abs/2603.25373)
*Bangchen Yin,Jian Ouyang,Zhen Fan,Kailai Lin,Hanshi Hu,Dingshun Lv,Weiluo Ren,Hai Xiao,Ji Chen,Changsu Cao*

Main category: cs.LG

TL;DR: 提出了一种Hessian-informed机器学习原子间势（Hi-MLIP），通过高效的HINT训练协议显著降低Hessian标签需求，实现了对势能面曲率的准确预测，在过渡态搜索和自由能计算等方面取得突破。


<details>
  <summary>Details</summary>
Motivation: 势能面的局部曲率对于从第一性原理预测分子和材料的实验可观测值至关重要，但对于复杂系统来说仍然难以获得。现有方法在计算Hessian矩阵时计算成本过高，限制了机器学习原子间势在热力学和动力学现象分析中的应用。

Method: 开发了Hessian-informed机器学习原子间势（Hi-MLIP），并提出了高效的Hessian INformed Training（HINT）训练协议。HINT集成了Hessian预训练、构型采样、课程学习和随机投影Hessian损失等关键技术，将昂贵Hessian标签的需求降低了2-4个数量级。

Result: Hi-MLIP显著改善了过渡态搜索，使吉布斯自由能预测接近化学精度，特别是在数据稀缺的情况下。该框架还能准确处理强非谐性氢化物，重现声子重整化和超导临界温度，与实验结果高度一致，同时绕过了非谐性计算的计算瓶颈。

Conclusion: 该研究建立了一条增强机器学习原子间势曲率感知能力的实用途径，为广泛系统的模拟和实验可观测值之间搭建了桥梁，在热力学和动力学现象分析方面具有重要应用价值。
...
### [180] [GlowQ: Group-Shared LOw-Rank Approximation for Quantized LLMs](https://arxiv.org/abs/2603.25385)
*Selim An,Il hong Suh,Yeseong Kim*

Main category: cs.LG

TL;DR: GlowQ提出了一种分组共享低秩近似方法，用于量化LLMs，通过缓存共享右因子减少参数和内存开销，同时保持精度。选择性变体GlowQ-S进一步优化延迟和吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的量化技术（如BitsAndBytes、AWQ、GPTQ）在低比特表示（如4位）时会导致精度下降。低秩校正方法（如LQER、QERA、ASER）虽然能缓解此问题，但会恢复所有层并在每个解码器块中插入误差校正模块，增加了延迟和内存开销。

Method: 提出GlowQ方法：采用分组共享低秩近似，为每个输入共享组缓存单个共享右因子，仅恢复提供最高精度收益的组或层。对每个输入共享组计算一次高精度投影，并在其模块中重复使用，减少参数和内存开销。还提出了选择性变体GlowQ-S，仅在提供最大收益的地方应用缓存的共享模块。

Result: 相比强基线，GlowQ平均减少TTFB 5.6%，增加吞吐量9.6%，在WikiText-2上减少困惑度0.17%，下游任务准确率提高0.42个百分点。选择性模型GlowQ-S进一步减少延迟，TTFB降低23.4%，吞吐量增加37.4%，同时平均精度保持在0.2个百分点内。

Conclusion: GlowQ通过分组共享低秩近似有效解决了量化LLMs的精度损失问题，在减少延迟和内存开销的同时保持了模型表达能力，为高效部署大型语言模型提供了有效解决方案。
...
### [181] [Maximum Entropy Behavior Exploration for Sim2Real Zero-Shot Reinforcement Learning](https://arxiv.org/abs/2603.25464)
*Jiajun Hu,Nuria Armengol Urpi,Jin Cheng,Stelian Coros*

Main category: cs.LG

TL;DR: FB-MEBE是一种在线零样本强化学习算法，结合无监督行为探索和正则化批评器，用于四足机器人控制，无需下游任务知识即可收集多样化数据，生成可直接部署到硬件的自然策略。


<details>
  <summary>Details</summary>
Motivation: 零样本RL算法需要预收集相关且多样化的数据集，但在没有下游任务先验知识的情况下，无指导的探索往往产生低多样性数据，导致下游性能差且策略无法直接硬件部署。
...
Conclusion: FB-MEBE通过结合行为探索和正则化，解决了零样本RL中数据多样性不足的问题，实现了可直接硬件部署的高质量策略学习。
...
### [182] [Not a fragment, but the whole: Map-based evaluation of data-driven Fire Danger Index models](https://arxiv.org/abs/2603.25469)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法在预测野火发生时，虽然利用了高分辨率数据和传统过程模型忽略的预测因子，但标准评估指标对火灾危险指数(FDI)预报的操作性能衡量有限，且经常未充分考虑误报率在操作环境中的关键重要性。

Method: 重新审视每日FDI模型评估范式，提出与真实世界决策相一致的新型森林火灾预报模型评估方法，系统评估火灾活动准确预测和误报（虚假警报）性能，并采用机器学习模型集成方法。

Result: 机器学习模型集成能够同时提高火灾识别准确率和减少误报，证明了所提评估方法的有效性。

Conclusion: 需要开发更符合实际决策需求的火灾预报模型评估框架，集成机器学习模型是提高操作性能的有效途径，能够平衡火灾识别准确率和误报控制。
...
### [183] [Causal-INSIGHT: Probing Temporal Models to Extract Causal Structure](https://arxiv.org/abs/2603.25473)
*Benjamin Redden,Hui Wang,Shuyan Li*

Main category: cs.LG

TL;DR: Causal-INSIGHT是一个模型无关的后处理解释框架，用于从训练好的时序预测器中提取模型隐含的、有向的、有时滞的影响结构，而不需要真实图标签。


<details>
  <summary>Details</summary>
Motivation: 理解多元时间序列中的有向时序交互对于解释复杂动态系统和训练在其上的预测模型至关重要。现有方法通常关注数据生成过程的因果结构，而本文关注预测器所依赖的依赖关系。

Method: 提出Causal-INSIGHT框架，通过系统性的干预式输入钳制来分析预训练预测器的响应，构建有向时序影响信号，并引入Qbic（一种稀疏感知的图选择准则）来平衡预测保真度和结构复杂度。

Result: 在合成、模拟和真实基准测试中，Causal-INSIGHT能够泛化到不同的骨干架构，保持有竞争力的结构准确性，并在应用于现有预测器时显著改善时序延迟定位。

Conclusion: Causal-INSIGHT为从训练好的时序预测器中提取模型隐含的有向时序影响结构提供了一种有效的模型无关后处理解释方法，无需真实图标签即可获得有意义的解释。
...
### [184] [How Class Ontology and Data Scale Affect Audio Transfer Learning](https://arxiv.org/abs/2603.25476)
*Manuel Milling,Andreas Triantafyllopoulos,Alexander Gebhard,Simon Rampp,Björn W. Schuller*

Main category: cs.LG

TL;DR: 音频到音频迁移学习的系统性研究：通过在不同AudioSet子集上预训练模型并在三个下游任务上微调，发现预训练数据量、类别数以及预训练与下游任务的相似性对迁移学习效果均有积极影响，其中任务相似性影响最大。


<details>
  <summary>Details</summary>
Motivation: 尽管迁移学习在深度学习中广泛应用且效果显著，但其内部工作机制仍存在许多未解之谜，特别是关于何时以及为何迁移学习有效的问题。本研究旨在通过音频到音频迁移学习的系统性实验，深入理解迁移学习的工作机制。

Method: 在基于本体的AudioSet不同子集上进行预训练，然后在三个计算机听觉任务上进行微调：声学场景识别、鸟类活动识别和语音命令识别。通过控制预训练数据的样本数量和类别数量，以及评估预训练任务与下游任务的相似性对迁移效果的影响。

Result: 增加预训练数据的样本数量和类别数量都对迁移学习有积极影响。然而，预训练任务与下游任务之间的相似性影响更为显著，这种相似性能够使模型学习到可比较的特征，从而获得更好的迁移效果。

Conclusion: 迁移学习的有效性不仅取决于预训练数据的规模和多样性，更重要的是预训练任务与下游任务之间的相似性。任务相似性能够促进特征的可迁移性，这对于理解和优化迁移学习策略具有重要意义。
...
### [185] [Interpretable PM2.5 Forecasting for Urban Air Quality: A Comparative Study of Operational Time-Series Models](https://arxiv.org/abs/2603.25495)
*Moazzam Umer Gondal,Hamad ul Qudous,Asma Ahmad Farhan,Sultan Alamri*

Main category: cs.LG

TL;DR: 本研究比较了三种轻量级时间序列模型（SARIMAX、Facebook Prophet、NeuralProphet）在北京PM2.5小时预测中的表现，发现Facebook Prophet在滚动重拟合模式下表现最佳，而在线残差校正能显著提升SARIMAX和Facebook Prophet在冻结模型模式下的性能。
...
Method: 使用多年污染物和气象时间序列数据，开发了泄漏感知预测工作流，结合时间序列数据划分、预处理、特征选择和外生驱动建模。评估了三种预测模型家族：SARIMAX、Facebook Prophet和NeuralProphet，并在两种自适应机制下测试：每周滚动向前重拟合和冻结预测配合在线残差校正。

Result: 在滚动重拟合模式下，Facebook Prophet表现最佳（MAE 37.61，RMSE 50.10），且计算时间显著少于NeuralProphet。在冻结模型模式下，在线残差校正提升了Facebook Prophet和SARIMAX的性能，校正后的SARIMAX达到最低总体误差（MAE 32.50，RMSE 46.85）。NeuralProphet在两个模式下都表现较差且不稳定。
...
TL;DR: 本文重新审视了在线蒸馏方法，分析了采样标记变体在长序列任务中的脆弱性，提出了基于教师top-K局部支持匹配的改进方案，在数学推理和智能体任务上取得了更好的稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 在线蒸馏在大语言模型后训练中具有吸引力，但在长序列设置中，常见的采样标记变体存在脆弱性问题：它将分布匹配简化为单标记信号，且随着生成序列偏离教师常见前缀而变得不可靠。

Method: 从估计器和实现两方面重新审视在线蒸馏。理论分析标记级在线蒸馏相对于序列级反向KL的偏差，但具有更紧的最坏情况方差界限。提出教师top-K局部支持匹配方法，实现为带top-p采样和特殊标记掩码的截断反向KL。

Result: 在单任务数学推理和多任务智能体+数学训练中，该目标比采样标记在线蒸馏产生更稳定的优化和更好的下游性能。玩具实验显示更强的未来奖励耦合会产生更高的梯度方差和更不稳定的学习。

Conclusion: 采样标记在线蒸馏存在三个失效模式：不平衡的单标记信号、对学生生成前缀的不可靠教师指导、以及分词器或特殊标记不匹配导致的失真。通过教师top-K局部支持匹配可以有效解决这些问题，实现更稳定和有效的在线蒸馏。
...
TL;DR: U-Balance：利用行为不确定性解决CPS安全监控中极端类别不平衡问题的新方法


<details>
  <summary>Details</summary>
Motivation: 现实世界CPS操作中不安全事件罕见，导致极端类别不平衡问题，传统重平衡技术在时间序列遥测数据上表现不佳，而CPS行为不确定性虽与安全结果相关但尚未在安全监控中探索

Method: 提出U-Balance方法：1) 训练基于GatedMLP的不确定性预测器，将遥测窗口总结为分布运动学特征并输出不确定性分数；2) 应用不确定性引导的标签重平衡机制，将高不确定性的安全标签窗口概率性重新标记为不安全，丰富少数类样本；3) 在重平衡数据集上训练安全预测器

Result: 在46:1安全-不安全比的无人机基准测试中，U-Balance达到0.806 F1分数，比最强基线提高14.3个百分点，同时保持有竞争力的推理效率。行为不确定性与安全存在中等但显著的相关性，uLNR被证明是利用不确定性信息的最有效策略

Conclusion: U-Balance通过利用行为不确定性有效解决了CPS安全监控中的极端类别不平衡问题，GatedMLP不确定性预测器和uLNR机制都对方法有效性有显著贡献
...
### [188] [A Unified Memory Perspective for Probabilistic Trustworthy AI](https://arxiv.org/abs/2603.25692)
*Xueji Zhao,Likai Pei,Jianbo Liu,Kai Ni,Ningyuan Cao*

Main category: cs.LG

TL;DR: 该论文提出将确定性数据访问视为随机采样的特例，建立统一框架分析AI系统中的数据访问模式，揭示随机性需求如何降低数据访问效率并导致熵受限操作，为可信AI硬件设计提供新视角。


<details>
  <summary>Details</summary>
Motivation: 可信人工智能系统越来越多地依赖概率计算来实现鲁棒性、可解释性、安全性和隐私性。在实际系统中，这些工作负载在模型、数据路径和系统功能之间交错进行确定性数据访问和重复随机采样，将性能瓶颈从算术单元转移到必须同时提供数据和随机性的内存系统。

Method: 提出统一的数据访问视角，将确定性访问视为随机采样的极限情况，使两种模式能够在共同框架内进行分析。基于这一视角，定义内存级评估标准，包括统一操作、分布可编程性、效率、对硬件非理想性的鲁棒性以及并行兼容性。

Result: 分析表明，增加随机性需求会降低有效数据访问效率，并可能驱动系统进入熵受限操作状态。通过提出的评估标准，分析了传统架构的局限性，并研究了新兴的概率计算内存方法，这些方法将采样与内存访问集成在一起。

Conclusion: 该研究为可信AI的可扩展硬件设计提供了新途径，通过统一框架分析数据访问模式，揭示了随机性需求对系统性能的影响，并指出了集成采样与内存访问的概率计算内存方法的发展方向。
...
### [189] [ARC-AGI-3: A New Challenge for Frontier Agentic Intelligence](https://arxiv.org/abs/2603.24621)
...
Method: 设计基于核心知识先验的交互式基准测试环境，通过广泛的人类测试者测试进行难度校准，建立基于人类行动基线的效率评分框架。

Result: 人类测试者可以解决100%的环境，而截至2026年3月的前沿AI系统得分低于1%。

Conclusion: ARC-AGI-3提供了一个有效的基准测试框架，用于评估智能体在全新任务上的自适应能力，揭示了当前AI系统与人类智能体在探索、推理和规划能力上的显著差距。
...
### [190] [Trust as Monitoring: Evolutionary Dynamics of User Trust and AI Developer Behaviour](https://arxiv.org/abs/2603.24742)
...
TL;DR: 该研究使用演化博弈论建模AI安全治理，将用户信任视为动态监控过程而非一次性选择，发现只有低成本监控与有效惩罚结合才能实现安全AI的广泛采用。


<details>
  <summary>Details</summary>
Motivation: 现有AI治理模型将用户信任视为一次性采纳决策，忽略了信任作为动态演化过程的本质。随着AI能力增强和安全问题日益紧迫，需要更准确地建模用户与开发者之间的重复互动关系。

Method: 采用演化博弈论方法，将用户信任建模为重复非对称互动中的监控减少过程。结合无限种群复制动态分析、随机有限种群动态模拟和强化学习（Q-learning）仿真，研究不同监控成本和制度环境下用户信任策略与开发者安全选择（合规vs不合规）的共同演化。

Result: 发现三个稳健的长期演化机制：1）不安全开发且无采用；2）不安全但广泛采用的系统；3）安全且广泛采用的系统。只有第三种是理想的，它要求对不安全行为的惩罚超过安全额外成本，且用户仍能负担偶尔监控。

Conclusion: 研究结果支持强调透明度、低成本监控和有效制裁的治理方案。单独监管或盲目用户信任都不足以防止向不安全或低采用结果的演化漂移，需要结合两者才能实现安全AI的广泛采用。
...
### [191] [ReLope: KL-Regularized LoRA Probes for Multimodal LLM Routing](https://arxiv.org/abs/2603.24787)
...
<details>
  <summary>Details</summary>
Motivation: 在纯文本LLM中，探针路由通过预测小模型的正确性来平衡性能与成本，但在多模态LLM中，视觉输入的存在会削弱隐藏状态中正确性信号的可分离性，导致现有探针方法效果显著下降。
...
### [192] [Resisting Humanization: Ethical Front-End Design Choices in AI for Sensitive Contexts](https://arxiv.org/abs/2603.24853)
*Silvia Rossi,Diletta Huyskes,Mackenzie Jorgensen*

Main category: cs.AI

TL;DR: 论文探讨AI前端设计的伦理意义，特别是对话用户界面中人性化设计元素对用户心理模型、信任校准和行为反应的影响，通过Chayn案例说明伦理考量如何指导界面设计中的克制原则。


<details>
  <summary>Details</summary>
Motivation: 当前AI伦理讨论主要关注后端问题（数据治理、模型训练、算法决策），而前端设计选择（交互方式、表现形式）的伦理意义被忽视，特别是在对话用户界面中，人性化设计元素日益普遍，需要研究其对用户的影响。

Method: 结合人机交互、对话AI和价值敏感设计的研究，分析界面如何影响用户期望、信任和自主性；通过Chayn非营利组织支持性别暴力幸存者的两个AI系统案例研究，展示创伤知情设计原则的实际应用。

Result: 人性化AI前端设计是价值驱动的选择，深刻影响用户心理模型、信任校准和行为反应；界面可能在脆弱情境下误导用户期望、培养错误信任、削弱用户自主性；伦理考量应促使设计中的原则性克制。

Conclusion: 伦理前端AI设计是一种程序性伦理，通过交互选择而非仅系统逻辑来实现；应挑战当代AI产品中以参与度为导向的规范，在界面设计中体现伦理考量，特别是在服务脆弱群体时。
...
### [193] [How Far Are Vision-Language Models from Constructing the Real World? A Benchmark for Physical Generative Reasoning](https://arxiv.org/abs/2603.24866)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型的评估过于偏向视觉真实性，忽略了物理世界的结构性和程序性约束。现有基准很少测试模型是否理解构建这些人工制品所需的逐步过程和物理依赖关系，而这种能力对于自动化设计到建造流程至关重要。

Method: 基于住宅木结构建筑领域（具有完全编码的工程标准和可客观验证的正确性）创建基准，收集了13种建筑风格的26,000多个结构，均达到施工文档标准（LOD 350）。开发了确定性的10项结构验证框架，支持迭代式智能体交互：模型观察中间建造状态，生成施工动作，接收结构化环境反馈。

Result: 对最先进的视觉语言模型进行广泛实验，揭示了在现有排行榜上基本不可见的显著能力差距。这些发现确立了物理有效性作为与视觉真实性正交的关键评估维度。

Conclusion: 物理生成推理是多模态智能中一个独特且未充分发展的前沿领域，DreamHouse基准突出了物理有效性作为关键评估轴的重要性，超越了传统的视觉真实性评估。
...
### [194] [On the Foundations of Trustworthy Artificial Intelligence](https://arxiv.org/abs/2603.24904)
*TJ Dunham*

Main category: cs.AI

TL;DR: 论文证明平台确定性推理是可信AI的必要且充分条件，提出确定性论题和信任熵概念，构建纯整数推理引擎实现跨架构比特级一致性输出
...
Method: 提出确定性论题，引入信任熵量化非确定性成本，构建纯整数推理引擎替代浮点运算，实现ARM和x86架构间的比特级相同输出，通过跨架构测试和链上验证

Result: 在82次跨架构测试中（模型参数高达67亿），观察到零哈希不匹配；四个地理分布式节点产生相同输出，通过356笔链上认证交易验证；系统包含99,000行Rust代码，部署在三大洲

Conclusion: 平台确定性是AI信任的基础，AI信任本质上是算术问题；纯整数推理引擎能够实现跨平台比特级一致性，为可信AI系统提供了可行的技术路径
...
TL;DR: LogitScope是一个轻量级框架，通过从概率分布计算token级信息度量来分析LLM不确定性，无需标注数据即可揭示模型置信度模式、识别幻觉和高不确定性决策点。


<details>
  <summary>Details</summary>
Motivation: 传统评估方法对LLM生成过程中单个token位置的模型置信度提供有限洞察，需要一种能够量化LLM输出不确定性的方法以确保可靠部署。

Method: LogitScope是一个轻量级框架，通过计算每个生成步骤的token级信息度量（如熵和变熵）来分析LLM不确定性。该框架模型无关，通过惰性评估实现计算高效，兼容任何HuggingFace模型。

Result: LogitScope能够揭示模型置信度模式、识别潜在幻觉、暴露模型表现出高不确定性的决策点，在不确定性量化、模型行为分析和生产监控等多种应用中展现实用性。
...
### [196] [Decoding Market Emotions in Cryptocurrency Tweets via Predictive Statement Classification with Machine Learning and Transformers](https://arxiv.org/abs/2603.24933)
*Moein Shahiki Tash,Zahra Ahani,Mohim Tash,Mostafa Keikhay Farzaneh,Ari Y. Barrera-Animas,Olga Kolesnikova*

Main category: cs.AI

TL;DR: 该研究提出了一种新颖的分类框架，用于识别加密货币相关推文中的预测性陈述，通过两阶段分类（预测/非预测，以及增量/减量/中性）和情感特征分析，结合GPT增强的数据平衡方法，在多种模型上取得了良好效果。


<details>
  <summary>Details</summary>
Motivation: 随着加密货币的日益普及，社交媒体上的投机活动增多，需要有效识别和分析加密货币相关的预测性陈述，以理解市场情绪和预测模式。

Method: 采用两阶段分类框架：任务1进行二元分类（预测性vs非预测性），任务2对预测性推文进一步分类（增量、减量、中性）。结合人工和GPT标注构建数据集，使用SenticNet提取情感特征，通过GPT生成的改写进行数据增强以解决类别不平衡问题。评估了多种机器学习、深度学习和基于Transformer的模型。

Result: GPT平衡方法显著提升了模型性能，Transformer模型在任务1中取得了最高的F1分数，而传统机器学习模型在任务2中表现最佳。情感分析揭示了不同加密货币中每种预测类别对应的独特情感模式。

Conclusion: 该研究提出的分类框架能有效识别加密货币推文中的预测性陈述，GPT增强的数据平衡方法显著改善模型性能，情感分析为理解加密货币市场情绪提供了有价值的见解。
...
TL;DR: 研究Ctrip平台3100万用户使用LLM购物助手"Wendao"的实证分析，发现：1) 采用者多为年长、女性、高活跃用户；2) AI聊天与传统搜索在同一购买阶段出现且常交替使用；3) 用户主要用AI进行探索性、难关键词化的查询


<details>
  <summary>Details</summary>
Motivation: 了解消费者如何采用和使用电商平台嵌入的购物AI，填补关于平台特定AI助手使用模式的大规模实证研究空白

Method: 使用中国最大在线旅游平台Ctrip的3100万用户数据，分析其LLM助手"Wendao"的使用模式，通过大数据分析用户行为特征

Result: 1) AI采用者主要为年长消费者、女性用户和高活跃用户；2) AI聊天与传统搜索在同一购买阶段出现，常交替使用；3) 42%的聊天请求为景点查询，主要用于探索性、难关键词化的任务

Conclusion: 嵌入式购物AI并非传统搜索的替代品，而是作为电商中探索性产品发现的补充界面，主要用于难以通过关键词搜索的探索性任务
...
<details>
  <summary>Details</summary>
Motivation: 评估学生手写草稿对于个性化教育反馈至关重要，但现有教育NLP主要关注文本响应，忽略了手写草稿的复杂性和多模态特性。当前多模态大语言模型主要采用"考生视角"生成正确答案，而非诊断学生错误。

Method: 引入ScratchMath基准数据集，包含1720个中国中小学生数学样本，支持错误原因解释和错误原因分类两个任务，定义了七种错误类型。采用严格的人机协作方法进行多阶段专家标注、审查和验证。

Result: 系统评估了16个领先的MLLMs，结果显示与人类专家相比存在显著性能差距，特别是在视觉识别和逻辑推理方面。专有模型明显优于开源模型，大型推理模型在错误解释方面表现出强大潜力。

Conclusion: ScratchMath填补了手写数学草稿错误诊断的空白，为教育AI研究提供了重要基准。评估结果表明现有模型在理解学生手写草稿方面仍有很大改进空间，所有评估数据和框架已公开以促进进一步研究。
...
TL;DR: 提出一个将大语言模型不确定性分解为三个语义组件的框架：输入歧义、知识缺口和解码随机性，以更好地理解LLM可靠性并检测幻觉。
...
Method: 提出不确定性分解框架，将LLM不确定性分解为三个语义组件：输入歧义（来自模糊提示）、知识缺口（来自参数证据不足）和解码随机性（来自随机采样）。

Result: 实验表明这些组件的优势会随模型大小和任务而变化，该框架能更好地审计LLM可靠性和检测幻觉。
...
TL;DR: 本文提出多视角故障归因范式，针对多智能体系统中故障归因的模糊性问题，建立了首个多视角故障归因基准MP-Bench和相应评估协议，发现以往认为LLM不擅长故障归因的结论主要源于基准设计限制。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统故障归因方法和基准大多假设每个故障只有一个确定性根本原因，但实际中由于复杂的智能体间依赖关系和模糊的执行轨迹，故障往往存在多种合理的归因可能。需要重新审视多智能体故障归因，考虑归因的模糊性问题。

Method: 提出多视角故障归因范式，明确考虑归因的模糊性；引入MP-Bench——首个专门为多智能体系统多视角故障归因设计的基准；开发适用于该范式的新评估协议。

Result: 通过大量实验发现，先前认为大型语言模型在故障归因方面表现不佳的结论主要是由于现有基准设计的局限性所致。研究结果强调了多视角基准和评估协议对于实现现实可靠的多智能体系统调试的必要性。

Conclusion: 多视角故障归因是解决多智能体系统故障归因模糊性的实用范式，MP-Bench和相关评估协议为更现实、可靠的多智能体调试提供了必要工具，挑战了以往关于LLM故障归因能力的结论。
...
### [201] [A Public Theory of Distillation Resistance via Constraint-Coupled Reasoning Architectures](https://arxiv.org/abs/2603.25022)
*Peng Wei,Wesley Shu*

Main category: cs.AI

TL;DR: 本文提出了一种理论框架，旨在通过架构设计减少模型能力蒸馏与治理结构之间的不对称性，核心观点是将高级能力与内部稳定性约束耦合，使蒸馏作为捷径的价值降低。


<details>
  <summary>Details</summary>
Motivation: 知识蒸馏、模型提取和行为转移已成为前沿AI的核心关切，主要风险不仅是复制，而是有用的能力可能比原本伴随的治理结构更廉价地被转移。需要减少这种不对称性。

Method: 引入约束耦合推理框架，包含四个要素：有界转换负担、路径负载累积、动态演化的可行区域，以及能力-稳定性耦合条件。该框架是公开的、保护商业秘密的理论框架。

Result: 本文贡献是理论性的而非操作性的，提供了一个可证伪的架构论点、清晰的威胁模型，以及一组可实验验证的假设，用于未来蒸馏抵抗、对齐和模型治理的研究。

Conclusion: 通过将高级能力与内部稳定性约束耦合，可以降低蒸馏作为捷径的价值，为构建更安全的AI系统提供理论指导，同时保持公开安全，不泄露专有实现细节。
...
### [202] [System-Anchored Knee Estimation for Low-Cost Context Window Selection in PDE Forecasting](https://arxiv.org/abs/2603.25025)
*Wenshuo Wang,Fan Zhang*

Main category: cs.AI

TL;DR: 提出SAKE方法，用于低成本选择自回归神经PDE模拟器的上下文窗口大小，通过系统锚点识别候选集并进行下游选择，在PDEBench上表现优异。
...
Method: 提出SAKE（系统锚定拐点估计）两阶段方法：1）从物理可解释的系统锚点识别小型结构化候选集；2）在候选集内进行拐点感知的下游选择。

Result: 在PDEBench的8个家族中，SAKE是匹配预算下最强的低成本选择器，达到67.8%精确匹配、91.7%±1匹配、6.1%平均遗憾@拐点，成本比仅0.051（节省94.9%搜索成本）。
...
### [203] [From Stateless to Situated: Building a Psychological World for LLM-Based Emotional Support](https://arxiv.org/abs/2603.25031)
*Boning Zhao,Clover Hu,Xinnuo Li*

Main category: cs.AI

TL;DR: LEKIA 2.0提出了一种情境化LLM架构，通过分离认知层和执行层，解决传统LLM在多轮情感支持中缺乏时间连续性、阶段意识和用户同意边界的问题。


<details>
  <summary>Details</summary>
Motivation: 传统LLM在心理支持和情感陪伴场景中的核心局限在于其基于局部下一个token预测的机制，无法维持多轮干预所需的时间连续性、阶段意识和用户同意边界，导致系统容易出现阶段错位和边界违反。

Method: 提出LEKIA 2.0情境化LLM架构，将认知层与执行层分离，解耦情境建模与干预执行，使系统能够在持续交互中保持用户情境和同意边界的稳定表示。

Result: LEKIA在深度干预循环完成度上比仅使用提示的基线方法平均绝对提升约31%，并通过静态到动态在线评估协议验证了其过程控制能力。
...
### [204] [Mechanistically Interpreting Compression in Vision-Language Models](https://arxiv.org/abs/2603.25035)
*Veeraraju Elluru,Arth Singh,Roberto Aguero,Ajay Agarwal,Debojyoti Das,Hreetam Paul*

Main category: cs.AI

TL;DR: 压缩视觉语言模型（VLM）虽降低部署成本，但可能改变内部计算和安全行为。研究发现剪枝保持电路结构但旋转衰减特征，量化改变电路但特征对齐更好。剪枝导致拒绝行为显著下降，压缩选择影响安全性。


<details>
  <summary>Details</summary>
Motivation: 压缩视觉语言模型广泛用于降低内存和计算成本，适合实际部署，但压缩可能改变内部计算和安全行为，需要研究压缩如何影响模型内部机制和安全性能。

Method: 使用因果电路分析和crosscoder-based特征比较，研究剪枝和量化如何改变代表性VLM的内部机制。引入VLMSafe-420基准，将有害输入与匹配的良性反事实配对，覆盖多种安全类别。

Result: 剪枝一般保持电路结构完整但旋转和衰减内部特征，量化在更高层次修改电路但使存活特征更好对齐。剪枝导致真正的拒绝行为急剧下降，表明压缩选择具有安全影响。

Conclusion: 压缩视觉语言模型时，不同的压缩方法对内部机制和安全行为有不同影响。剪枝会显著降低安全拒绝行为，而量化在特征对齐方面表现更好。压缩选择需要考虑安全影响。
...
### [205] [MP-MoE: Matrix Profile-Guided Mixture of Experts for Precipitation Forecasting](https://arxiv.org/abs/2603.25046)
*Huyen Ngoc Tran,Dung Trung Tran,Hong Nguyen,Xuan Vu Phan,Nam-Phong Nguyen*

Main category: cs.AI

TL;DR: 提出MP-MoE框架，通过结合矩阵轮廓相似性度量来改进热带地区降水预报，缓解传统点对点损失函数的"双重惩罚"问题


<details>
  <summary>Details</summary>
Motivation: 热带地区（如越南）降水预报面临复杂地形和对流不稳定的挑战，数值天气预报模型存在偏差。传统数据驱动的后处理方法依赖点对点目标函数，在时间错位时会产生"双重惩罚"效应，影响预报准确性。

Method: 提出矩阵轮廓引导的专家混合框架，将传统强度损失与结构感知的矩阵轮廓目标函数相结合。利用子序列级相似性而非点对点误差，实现更可靠的专家选择，缓解相位偏移导致的过度惩罚。

Result: 在越南两个主要河流流域的降雨数据集上进行评估，涵盖多个时间尺度。实验结果表明，MP-MoE在强降雨事件的平均临界成功指数上优于原始数值天气预报和基线学习方法，同时显著降低动态时间规整值。

Conclusion: MP-MoE框架能有效捕捉峰值降雨强度并保持风暴事件形态完整性，为热带地区降水预报提供了一种改进方法，通过结构感知损失函数缓解了传统方法的局限性。
...
### [206] [Sparse Visual Thought Circuits in Vision-Language Models](https://arxiv.org/abs/2603.25075)
*Yunpeng Zhou*

Main category: cs.AI

TL;DR: SAE特征在视觉语言模型中并不总是模块化可组合的，特征联合干预会导致输出漂移和精度下降，揭示了共享内部路径的电路干扰问题


<details>
  <summary>Details</summary>
Motivation: 虽然稀疏自编码器（SAE）能提升多模态模型的可解释性，但SAE特征是否真正形成模块化、可组合的推理单元仍不明确，这是许多基于干预的引导方法的基本假设
...
Result: 任务选择性特征集的干预能适度提升推理精度，但两个特征集的联合干预会可靠地诱导输出漂移（预测的大幅意外变化）并降低精度，即使在规范匹配的扰动下也是如此；这种非模块化电路干扰与共享内部路径一致，其中特征联合放大了激活偏移；结果通过自助子样本和置换控制验证，并在多个VLM家族和五个不同数据集上复现

Conclusion: 研究阐明了SAE特征可组合性的边界，为更可靠的视觉语言模型控制提供了严格的诊断框架；SAE特征并不总是形成模块化、可组合的推理单元，这对基于干预的引导方法提出了重要限制
...
### [207] [ElephantBroker: A Knowledge-Grounded Cognitive Runtime for Trustworthy AI Agents](https://arxiv.org/abs/2603.25097)
*Cristian Lupascu,Alexandru Lupascu*

Main category: cs.AI

TL;DR: ElephantBroker是一个开源认知运行时系统，通过Neo4j知识图谱和Qdrant向量存储的统一，为LLM智能体提供可验证的持久记忆系统，包含完整的认知循环和安全防护机制。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体在关键多轮对话环境中缺乏可靠的事实基础，现有记忆系统通常依赖简单的键值存储或向量检索，无法追踪知识来源和可信度，需要更健壮的记忆架构。

Method: 系统采用Neo4j知识图谱与Qdrant向量存储的混合架构，通过Cognee SDK实现统一。包含：混合五源检索管道、十一维竞争评分引擎、四状态证据验证模型、五阶段上下文生命周期、六层廉价优先防护管道、AI防火墙、九阶段整合引擎和数字权威模型。

Result: 通过超过2200个测试的全面测试套件验证了子系统正确性。支持三层部署、五种预设配置、多网关隔离和管理仪表板，能够配置从轻量级记忆智能体到具有企业级安全性和可审计性的完整认知运行时。

Conclusion: ElephantBroker提供了一个可验证、安全的智能体记忆系统，解决了当前LLM智能体在关键应用中记忆不可靠的问题，支持从简单到复杂的企业级部署需求。
...
TL;DR: CaT-FSCIL通过将采集上下文分解为结构化低维组件和高维残差组件，分别用可逆变换建模和不确定性校准原型来解决少样本类增量学习中的上下文敏感性问题。


<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习（FSCIL）在只有少量标注样本的采集上下文中特别敏感。在触觉感知等场景中，不同的设备、接触状态和交互设置等采集上下文缺乏标准化，会显著降低模型性能。

Method: 提出Context-as-Transform FSCIL (CaT-FSCIL)：1）将采集上下文分解为结构化低维组件和高维残差组件；2）低维组件受触觉交互特征影响，建模为近似可逆的Context-as-Transform族，通过伪上下文一致性损失进行逆变换规范化；3）高维残差主要来自平台和设备差异，通过不确定性条件原型校准（UCPC）基于上下文不确定性校准有偏原型和决策边界。

Result: 在标准基准HapTex和LMT108上的综合实验证明了CaT-FSCIL的优越性。

Conclusion: CaT-FSCIL通过分解和处理采集上下文的不同组件，有效解决了少样本类增量学习中的上下文敏感性问题，在触觉感知等实际应用中具有重要价值。
...
TL;DR: RubricEval是首个针对指令遵循评估的细粒度标准级别元评估基准，揭示了当前标准级别评估的局限性，GPT-4o在困难子集上仅达到55.97%的准确率。


<details>
  <summary>Details</summary>
Motivation: 尽管基于标准的评估已成为评估大语言模型指令遵循的主流范式，但其可靠性尚不明确，需要元评估。先前的研究主要关注响应级别，未能评估标准级别评估所依赖的细粒度判断准确性。

Method: 提出了RubricEval基准，包含：(1)首个针对指令遵循的标准级别元评估基准；(2)涵盖多个类别和模型来源的多样化指令和响应；(3)3,486个质量控制实例，以及更好区分评估者性能的简单/困难子集。

Result: 实验显示标准级别评估远未解决：即使是广泛采用的GPT-4o评估者，在困难子集上仅达到55.97%准确率。标准级别评估优于清单级别，显式推理提高准确性，两者结合减少评估者间方差。

Conclusion: 通过建立的标准分类法，识别了常见失败模式，并为可靠的指令遵循评估提供了可操作的见解，强调了改进标准级别评估的必要性。
...
TL;DR: UniAI-GraphRAG是一个增强的图检索增强生成框架，通过本体引导知识提取、多维社区聚类策略和双通道图检索融合，解决了现有GraphRAG在跨行业适应性、社区报告完整性和检索性能方面的限制。
...
Method: 提出了三个核心创新：1) 本体引导知识提取：使用预定义Schema指导LLMs准确识别领域特定实体和关系；2) 多维社区聚类策略：通过对齐补全、基于属性的聚类和多跳关系聚类提高社区完整性；3) 双通道图检索融合：通过混合图和社区检索平衡QA准确性和性能。
...
TL;DR: AI辅助安全分析存在"能力阴影"风险——AI生成的建议会系统性地限制人类思考范围，导致分析质量下降远超预期。安全工程AI辅助是协作设计问题而非软件采购决策。


<details>
  <summary>Details</summary>
Motivation: 随着AI助手被集成到物理AI系统的安全工程工作流中，需要回答一个关键问题：AI辅助是提高安全分析质量，还是引入系统性盲点，只在部署后事故中显现？传统基准驱动评估无法有效评估安全工程能力。

Method: 1. 建立五维能力框架：领域知识、标准专业知识、操作经验、上下文理解、判断力；2. 提出"能力阴影"概念——AI生成的安全分析系统性地限制人类推理范围；3. 形式化四种典型人机协作结构；4. 推导闭式性能边界，证明能力阴影以乘法方式复合而非简单相加。

Result: 研究发现能力阴影会导致分析质量下降远超朴素加法估计。AI辅助在安全工程中的效果完全取决于使用方式，同一工具可能提升也可能降低分析质量。推导了抗阴影工作流的非退化条件。

Conclusion: 安全工程AI辅助是协作设计问题而非软件采购决策。需要从工具认证转向工作流认证，确保可信物理AI系统。必须设计抗阴影的工作流程来避免系统性盲点。
...
### [212] [A Gait Foundation Model Predicts Multi-System Health Phenotypes from 3D Skeletal Motion](https://arxiv.org/abs/2603.25283)
...
TL;DR: 开发了一个基于3D骨骼运动的步态基础模型，能够预测多种生理和临床表型，证明步态可作为独立的全身生物信号。


<details>
  <summary>Details</summary>
Motivation: 当前方法将步态视为特定病理的症状而非系统性生物标志物，需要建立步态作为全身健康指标的基础模型。

Method: 使用深度相机记录3,414名深度表型成年人在五种运动任务中的3D骨骼运动数据，开发步态基础模型并学习嵌入表示。

Result: 学习到的嵌入优于工程特征，能预测年龄(r=0.69)、BMI(r=0.90)和内脏脂肪面积(r=0.82)；显著预测1,980个表型目标；在调整年龄、BMI、VAT和身高后，步态为所有18个身体系统提供独立增益。
...
### [213] [SliderQuant: Accurate Post-Training Quantization for LLMs](https://arxiv.org/abs/2603.25284)
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型后训练量化方法通常对所有层采用相同的量化策略，但研究发现不同层对量化的敏感度存在显著差异：浅层和深层通常比中间层更敏感，特别是第一层和最后一层的量化误差明显更大。这表明需要针对不同层设计多层次的量化方案，而不是对所有层使用单一策略。

Method: 提出了SliderQuant框架，包含两个核心组件：1) 层间滑动量化，针对浅层、中间层和深层设计了三种滑动窗口方案；2) 层内滑动量化，采用增量策略对每个窗口进行量化。该方法通过少量可学习参数实现自适应滑动量化，能够有效减少各层的量化误差。

Result: 在基础语言生成、零样本常识推理以及具有挑战性的数学和代码任务上进行了广泛实验，涵盖了Llama/Llama2/Llama3/Qwen2.5系列模型、DeepSeek-R1蒸馏模型和大规模MoE模型。结果表明，SliderQuant在仅权重量化和权重-激活量化方面均优于现有后训练量化方法，包括最新的使用旋转变换的方法。

Conclusion: SliderQuant通过考虑不同层对量化的敏感度差异，采用自适应滑动量化设计，为大语言模型的后训练量化提供了一种更有效的解决方案，在多种任务和模型架构上表现出优越性能。
...
### [214] [DAGverse: Building Document-Grounded Semantic DAGs from Scientific Papers](https://arxiv.org/abs/2603.25293)
*Shu Wan,Saketh Vishnubhatla,Iskander Kushbay,Tom Heffernan,Aaron Belikoff,Raha Moraffah,Huan Liu*

Main category: cs.AI

TL;DR: DAGverse框架从科学论文中构建文档基础的语义有向无环图，通过半自动管道生成高质量数据集，为结构化推理提供新基准。


<details>
  <summary>Details</summary>
Motivation: 现实世界的有向无环图数据集稀缺，因为构建它们需要专家对领域文档进行解释。文档可能允许多种合理的抽象，预期结构通常是隐式的，支持证据分散在文本、方程、标题和图表中。

Method: 利用包含明确DAG图的科学论文作为监督来源，开发DAGverse框架，其核心组件DAGverse-Pipeline是一个半自动系统，通过图分类、图重建、语义基础和验证来生成高精度语义DAG示例。

Result: 发布了DAGverse-1数据集，包含108个专家验证的语义DAG，具有图级、节点级和边级证据。实验显示DAGverse-Pipeline在DAG分类和标注方面优于现有的视觉语言模型。
...
### [215] [Modernising Reinforcement Learning-Based Navigation for Embodied Semantic Scene Graph Generation](https://arxiv.org/abs/2603.25415)
*Roman Kueble,Marco Hueller,Mrunmai Phatak,Rainer Lienhart,Joerg Haehner*

Main category: cs.AI

TL;DR: 该研究提出了一个模块化导航组件用于具身语义场景图生成，通过改进优化算法和动作表示来提升在有限动作预算下的探索效率和场景图完整性。


<details>
  <summary>Details</summary>
Motivation: 在有机计算中，语义世界模型是目标驱动的自适应系统的关键使能器。核心挑战是在有限动作预算内最大化模型质量和下游有用性。语义场景图为此提供了结构化紧凑表示，但需要在有限动作范围内平衡信息增益与导航成本。

Method: 1) 用现代优化算法替换策略优化方法；2) 重新审视离散动作表示，研究紧凑和细粒度的离散运动集；3) 比较单头策略与分解的多头策略；4) 评估课程学习和可选的基于深度的碰撞监督。

Result: 1) 仅替换优化算法就能在相同奖励塑造下将SSG完整性相对基线提高21%；2) 深度主要影响执行安全性（无碰撞运动），完整性基本不变；3) 现代优化与细粒度分解动作表示结合产生了最强的完整性-效率权衡。

Conclusion: 现代优化算法与细粒度分解动作表示的结合为具身语义场景图生成提供了最佳的导航性能，在有限动作预算下实现了更好的完整性-效率平衡。
...
### [216] [Evaluating Language Models for Harmful Manipulation](https://arxiv.org/abs/2603.25326)
...
TL;DR: 本文提出了一个评估AI有害操纵的框架，通过三个领域（公共政策、金融、健康）和三个地区（美国、英国、印度）的10,101名参与者进行人机交互研究，发现AI模型能够产生操纵行为并改变参与者的信念和行为，且这种操纵效果因领域和地区而异。
...
Method: 采用上下文特定的人机交互研究方法，在三个AI应用领域（公共政策、金融、健康）和三个地理区域（美国、英国、印度）对10,101名参与者进行实验，测试AI模型的操纵行为和效果。

Result: 测试的AI模型在被提示时能够产生操纵行为，并在实验环境中成功改变参与者的信念和行为；操纵效果在不同领域和地区存在显著差异；AI模型的操纵倾向性并不总是能预测操纵成功率。

Conclusion: 需要在高风险情境中评估AI操纵，不同地区的结果可能无法泛化，应分别研究操纵倾向性和成功率，并公开测试协议以促进评估框架的采用。
...
### [217] [Macroscopic Characteristics of Mixed Traffic Flow with Deep Reinforcement Learning Based Automated and Human-Driven Vehicles](https://arxiv.org/abs/2603.25328)
*Pankaj Kumar,Pranamesh Chakraborty,Subrahmanya Swamy Peruru*

Main category: cs.AI

TL;DR: 本文研究了基于深度强化学习的自动驾驶车辆在混合交通中的宏观交通流特性和燃油效率，相比传统IDM模型，RL控制能提升约7.52%的道路容量和最高28.98%的燃油效率。


<details>
  <summary>Details</summary>
Motivation: 混合交通中自动驾驶车辆的控制面临安全、效率、舒适性、燃油效率和交通规则遵守等多重挑战。传统跟车模型（如IDM）难以泛化到多样化交通场景，且通常不考虑燃油效率，这促使研究者采用基于学习的方法。虽然深度强化学习在微观跟车性能上表现良好，但其宏观交通流特性尚未充分探索。

Method: 采用Twin Delayed Deep Deterministic Policy Gradient (TD3)算法控制自动驾驶车辆，使用NGSIM高速公路数据集进行训练，实现与人类驾驶车辆的真实交互。通过基本图评估交通性能，分析不同驾驶员异质性、异质时间间隔渗透水平和RL控制车辆比例的影响，并在宏观层面比较RL模型与IDM的燃油效率。

Result: 交通性能对安全时间间隔分布和RL车辆比例敏感。从全人类驾驶过渡到全RL控制交通可将道路容量提升约7.52%。RL控制的自动驾驶车辆在较高速度（50km/h以上）下平均燃油效率比IDM提高约28.98%，在较低速度（50km/h以下）下提高1.86%。

Conclusion: 深度强化学习框架在不牺牲安全性的前提下，能够显著提升交通容量和燃油效率，为混合交通中自动驾驶车辆的控制提供了有效的解决方案。
...
### [218] [4OPS: Structural Difficulty Modeling in Integer Arithmetic Puzzles](https://arxiv.org/abs/2603.25356)
*Yunus E. Zeytuncu*
...
Result: 基于统计的基线机器学习模型能部分预测可解性但无法可靠区分简单实例；而难度完全由从精确见证中提取的小型可解释结构属性决定，特别是最小构造中使用的输入值数量。
...
### [219] [Does Structured Intent Representation Generalize? A Cross-Language, Cross-Model Empirical Study of 5W3H Prompting](https://arxiv.org/abs/2603.25379)
*Peng Gang*

Main category: cs.AI

TL;DR: 研究验证了基于5W3H的PPS结构化意图表示框架在跨语言（中英日）和跨模型（3个LLM）中的泛化能力，发现AI辅助扩展的5W3H提示与人工编写的效果相当，且能减少跨模型输出方差，提高意图对齐和可访问性。


<details>
  <summary>Details</summary>
Motivation: 先前研究仅基于中文验证了PPS框架的有效性，本研究旨在扩展验证该结构化意图表示框架是否能在不同语言（英语、日语）和不同模型之间泛化，并探索AI辅助扩展提示的可行性。

Method: 采用PPS（Prompt Protocol Specification）5W3H框架，在3种语言（中文、英文、日文）×4种条件（包括AI辅助扩展5W3H）×3个LLM×60个任务上，共生成2,160个模型输出进行分析，重点考察目标对齐和跨模型输出一致性。

Result: AI扩展的5W3H提示（条件D）在所有三种语言中与人工编写的5W3H提示（条件C）在目标对齐上无显著差异；结构化PPS条件通常能减少或重塑跨模型输出方差；非结构化提示存在系统性双重膨胀偏差：人工高复合分数和人工低跨模型方差。

Conclusion: 结构化5W3H表示能跨语言和模型改善意图对齐和可访问性，特别是当AI辅助创作降低了非专家用户的使用门槛时，为更可靠和可访问的人机交互提供了实证支持。
...
### [220] [Beyond Content Safety: Real-Time Monitoring for Reasoning Vulnerabilities in Large Language Models](https://arxiv.org/abs/2603.25412)
*Xunguang Wang,Yuguang Zhou,Qingyue Wang,Zongjie Li,Ruixuan Huang,Zhenlan Ji,Pingchuan Ma,Shuai Wang*

Main category: cs.AI

TL;DR: 该论文提出"推理安全"概念，针对LLM推理过程的安全性进行系统性研究，包括定义推理安全、构建不安全推理行为分类体系、实证研究以及开发实时监控系统。
...
Method: 1. 正式定义推理安全并构建九类不安全推理行为的分类体系，涵盖输入解析错误、推理执行错误和过程管理错误；2. 进行大规模实证研究，标注4111条来自自然推理基准和四种对抗攻击方法的推理链；3. 提出推理安全监控器：一个外部LLM组件，与目标模型并行运行，通过分类体系嵌入的提示实时检查每个推理步骤，并在检测到不安全行为时发送中断信号。

Result: 实证研究证实所有九类错误类型在实践中都存在，每种攻击都会产生机制上可解释的特征。在450条推理链的静态基准测试中，监控器达到84.88%的步骤级定位准确率和85.37%的错误类型分类准确率，显著优于幻觉检测器和过程奖励模型基线。

Conclusion: 推理级监控既是必要的也是实际可行的，推理安全应成为大型推理模型安全部署的基础性关注点。该研究为LLM推理过程的安全性提供了系统性框架和实用工具。
...
### [221] [Retraining as Approximate Bayesian Inference](https://arxiv.org/abs/2603.25480)
*Harrison Katz*

Main category: cs.AI

TL;DR: 将模型重训练重新定义为计算约束下的近似贝叶斯推断，提出"学习债务"概念，并基于决策理论框架制定重训练策略，用基于证据的触发器替代日历计划。


<details>
  <summary>Details</summary>
Motivation: 传统上模型重训练被视为常规维护任务，缺乏理论框架指导何时进行重训练。作者认为需要将重训练理解为计算约束下的贝叶斯推断过程，并建立决策理论框架来优化重训练决策。

Method: 提出"学习债务"概念来描述持续更新的信念状态与已部署冻结模型之间的差距。将重训练决策构建为成本最小化问题，从损失函数中推导出阈值。建立决策理论框架来制定重训练策略。

Result: 开发出基于证据的触发器来替代日历计划，使重训练决策更加科学和可审计。为不熟悉贝叶斯和决策理论术语的读者提供了术语表。

Conclusion: 模型重训练应被视为计算约束下的近似贝叶斯推断过程，通过决策理论框架可以制定更优化的重训练策略，实现基于证据的触发机制，提高机器学习系统的治理可审计性。
...
### [222] [EcoThink: A Green Adaptive Inference Framework for Sustainable and Accessible Agents](https://arxiv.org/abs/2603.25498)
*Linxiao Li,Zhixiang Lu*

Main category: cs.AI

TL;DR: EcoThink是一个能量感知的自适应推理框架，通过轻量级路由动态评估查询复杂度，跳过简单查询的复杂推理，减少LLM过度思考，平均降低40.4%推理能耗
...
Method: EcoThink采用基于蒸馏的轻量级路由器动态评估查询复杂度，对事实检索类简单查询跳过不必要的推理过程，仅对复杂逻辑问题保留深度计算，实现能量感知的自适应推理
...
### [223] [Voxtral TTS](https://arxiv.org/abs/2603.25551)
...
TL;DR: Voxtral TTS是一个多语言文本转语音模型，只需3秒参考音频即可生成自然语音，采用自回归语义令牌和流匹配声学令牌的混合架构，在人类评估中优于ElevenLabs Flash v2.5
...
Conclusion: Voxtral TTS是一个有效的多语言语音克隆模型，能够从极少量参考音频生成自然、富有表现力的语音，模型权重已按CC BY-NC许可发布
...
### [224] [R-C2: Cycle-Consistent Reinforcement Learning Improves Multimodal Reasoning](https://arxiv.org/abs/2603.25720)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在处理同一概念的视觉和文本表示时经常产生矛盾预测，这种跨模态不一致性破坏了感知和推理的鲁棒性。传统投票机制可能放大系统偏差，而非解决根本问题。

Method: 提出RC2强化学习框架，通过强制跨模态循环一致性来解决内部冲突。要求模型执行反向推理、切换模态，并通过前向推理可靠地重建答案，从而获得密集的无标签奖励信号。

Result: 优化这种结构约束能够减轻模态特定错误，将推理准确性提升高达7.6个百分点。实验表明，高级推理不仅来自数据扩展，也来自对世界结构一致性的强制理解。

Conclusion: 跨模态不一致性提供了丰富的自然学习信号，通过强化循环一致性约束，模型能够自主对齐内部表示，实现更鲁棒的感知和推理能力。
