...
TL;DR: HIVE框架通过引入视觉编码器与LLM之间的分层交叉注意力，增强视觉-语言对齐，在多个视觉-语言任务上超越现有方法。
...
Result: HIVE在图像分类和多个视觉-语言任务（MME、GQA、OK-VQA、ScienceQA）上表现优异，超越了基于自注意力的方法。

Conclusion: 分层特征整合显著提升了视觉-语言模型的性能和表达能力，为更高效、表达能力更强的多模态模型开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 原始(raw)数据对低层视觉任务更忠实，但大规模原始数据集的收集存在瓶颈，现有数据集有限且与特定相机硬件绑定。现有扩散模型框架设计用于合成照片完成的sRGB图像，而非物理一致的线性表示。

Method: 利用大规模sRGB扩散模型的生成先验，通过潜在空间和像素空间的专门处理来合成物理有意义的线性输出。构建多对一逆ISP数据集，将使用不同ISP参数生成的同一场景的多个sRGB渲染锚定到共同的场景参考目标。微调解码器和条件去噪器来处理未知和多样的ISP流水线和照片完成效果。

Result: RawGen在传统逆ISP方法（假设固定ISP）上表现出优越性能。通过RawGen的可扩展、文本驱动的合成数据增强训练流水线，可以有益于下游低层视觉任务。
...
TL;DR: 该论文提出了Q-Mask框架，通过因果查询驱动的掩码解码器实现精确的OCR文本区域定位，解决了现有视觉语言模型在文本锚点定位方面的不足。


<details>
  <summary>Details</summary>
Motivation: 虽然OCR被认为是现代视觉语言模型的基础能力，但实际应用需要可靠的文本锚点（将查询文本准确对应到空间区域）。现有通用和OCR专用模型在建立准确稳定的文本锚点方面仍有困难。

Method: 提出了Q-Mask框架，基于因果查询驱动的掩码解码器(CQMD)。受思维链推理启发，Q-Mask执行因果视觉解码，在生成最终OCR输出前顺序生成查询条件化的视觉掩码。这种视觉CoT范式将"文本在哪里"与"文本是什么"解耦，强制在识别前获取基础证据，并在推理期间实现显式的文本锚点构建。为训练CQMD，构建了TextAnchor-26M大规模数据集，包含特定文本元素对应的细粒度掩码标注。

Result: Q-Mask显著提高了文本锚点定位和理解能力，在多样化视觉场景中表现出色。TextAnchor-Bench基准测试揭示了现有模型的不足，而Q-Mask框架有效解决了这些问题。

Conclusion: Q-Mask通过因果视觉解码和视觉思维链范式，实现了精确的文本区域定位，为视觉语言模型提供了可靠的文本锚点能力，提升了OCR在真实世界视觉问答中的下游推理支持。
...
<details>
  <summary>Details</summary>
Motivation: MIM目标函数导致学习到的表示保留了非语义信息，这会损害推理时的性能表现

Method: 提出语义正交伪影投影(SOAP)：使用PCA分析真实和合成非语义图像来评估语义不变性，然后通过线性头直接抑制补丁表示中的非语义信息
...
TL;DR: UCell是一个用于单细胞分割的小型模型（10-30M参数），通过递归结构实现参数高效，性能媲美大10-20倍的模型，无需大规模预训练即可从零训练。


<details>
  <summary>Details</summary>
Motivation: 在生物医学研究中，模型规模受限于可用训练数据量和高昂的数据生成验证成本。当前研究过度关注构建更大的基础模型，而提升小模型能力的研究相对不足。本文旨在探索小型模型在单细胞分割任务中的潜力。

Method: 设计了一个10-30M参数的小型模型UCell，关键设计是在前向计算图中引入递归结构，实现更参数高效的架构。模型可以从零开始训练，仅使用显微镜成像数据，无需依赖自然图像的大规模预训练。

Result: 在多个基准测试中，UCell的性能与比它大10-20倍的模型相当，对未见域外数据具有相似的泛化能力。模型可以仅使用显微镜成像数据从零训练，无需外部预训练。通过多样小数据集上的一次性和少样本微调实验验证了其适应性。

Conclusion: UCell展示了小型模型在生物医学图像分割任务中的潜力，通过递归结构实现参数高效，性能媲美大模型，且不依赖大规模预训练，使模型构建与外部商业利益脱钩，为数据受限领域提供了可行的替代方案。
...
TL;DR: QAsk-Nav是首个可复现的协作实例对象导航基准，通过分离评估导航和协作提问能力，解决了现有基准只关注导航成功而缺乏协作交互评估的问题。


<details>
  <summary>Details</summary>
Motivation: 现有协作实例对象导航基准主要关注导航成功率，缺乏对协作交互能力的一致评估方法，无法明确评估智能体通过自然语言对话解决视觉相似对象歧义的能力。

Method: 提出QAsk-Nav基准，包含：(1)独立于导航评分的轻量级提问协议；(2)具有真实、多样、高质量目标描述的增强导航协议；(3)包含28,000条质量检查的推理和提问轨迹的开源数据集。

Result: 基于QAsk-Nav基准开发了Light-CoNav模型，比现有模块化方法小3倍、快70倍，在未见过的对象和环境上超越了最先进的协作实例对象导航方法。

Conclusion: QAsk-Nav为协作实例对象导航提供了首个可复现的评估基准，能够明确分离评估导航和协作提问能力，推动了该领域的发展。
...
TL;DR: Omni-MMSI是一个需要从原始音频、视觉和语音输入中理解社交互动的新任务，涉及身份归属的社交线索感知和社交推理。作者提出了Omni-MMSI-R管道来解决现有模型在身份归属能力上的不足。


<details>
  <summary>Details</summary>
Motivation: 开发能够感知和响应人类互动的AI助手需要理解社交互动，但现有研究依赖预处理的社交线索，而现实场景中AI助手需要从原始数据中感知和推理。现有多模态LLMs缺乏可靠的身份归属能力，导致社交互动理解不准确。

Method: 提出了Omni-MMSI-R管道，这是一个参考引导的管道，使用工具生成身份归属的社交线索，并进行链式思维的社交推理。为此构建了参与者级别的参考对，并在现有数据集上添加了推理标注。

Result: 实验表明，Omni-MMSI-R在Omni-MMSI任务上优于先进的LLMs和其他对比方法。

Conclusion: Omni-MMSI任务对于开发能够理解社交互动的AI助手至关重要，而提出的Omni-MMSI-R管道通过改进身份归属能力，显著提升了社交互动理解的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于基础模型的无监督分割方法在处理具有复杂多组件形态的场景时表现不佳，因为其依赖的粗粒度补丁级表示抑制了细粒度结构细节。

Method: 提出EASe框架，包含：1) SAUCE（语义感知上采样与通道激励）用于选择性校准低分辨率特征通道；2) 跨空间编码图像和基础模型特征的注意力机制；3) CAFE（线索注意特征聚合器）利用SAUCE注意力分数作为语义分组信号进行无训练多粒度掩码分割。
...
TL;DR: 该论文提出TPC-CMA方法，通过分解模态间隙为质心间隙和分布间隙，采用三阶段课程学习来改善视觉语言模型的跨模态对齐，显著提升聚类和字幕生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型（如CLIP）存在模态间隙问题，限制了跨模态任务的性能。传统后处理方法主要减少全局质心偏移，但未能解决分布不匹配问题，需要更有效的跨模态对齐方法。

Method: 提出TPC-CMA（三阶段跨模态对齐课程学习）微调框架：1）将模态间隙分解为质心间隙和分布间隙；2）采用三阶段课程学习，通过梯度感知调度逐步引入对齐；3）联合减少质心偏移并重塑分布结构。

Result: 在α_target=0.05时，模态间隙减少66.6%，仅导致4.84%准确率下降；在α_target=0.5时，模态间隙减少82.3%，聚类ARI从0.318提升至0.516，字幕生成CIDEr提升57.1%。分布间隙与跨模态任务质量高度相关（R²=0.986）。

Conclusion: 通过分解模态间隙并采用三阶段课程学习，TPC-CMA能有效改善视觉语言模型的跨模态对齐，显著提升聚类和字幕生成等任务的性能，为跨模态表示学习提供了新方法。
...
TL;DR: SANA-I2I是一个无文本的高分辨率图像到图像生成框架，通过移除文本条件，仅依赖配对源-目标图像学习潜在空间中的条件流匹配模型，专门用于医学图像处理任务如胎儿MRI运动伪影减少。


<details>
  <summary>Details</summary>
Motivation: 针对医学图像处理中难以获取真实配对数据的问题，开发一个无需文本提示、仅依赖图像对的条件生成模型，特别适用于胎儿MRI运动伪影减少等具有挑战性的医学成像任务。

Method: 1. 完全移除文本条件，仅使用配对源-目标图像；2. 在潜在空间中学习条件流匹配模型；3. 学习条件速度场将目标图像分布映射到另一个分布；4. 采用Duffy等人提出的合成数据生成策略模拟真实运动伪影，解决真实配对数据难以获取的问题。

Result: 实验结果表明SANA-I2I能有效抑制运动伪影同时保持解剖结构，在少量推理步骤下达到竞争性性能，证明了基于流的无文本生成模型在医学图像监督图像到图像任务中的效率和适用性。

Conclusion: SANA-I2I展示了基于流的无文本生成模型在医学图像处理中的有效性，特别是在胎儿MRI运动伪影减少等需要监督图像转换但难以获取真实配对数据的应用场景中具有显著优势。
...
TL;DR: 使用冻结的DINOv3 ViT-B嵌入，通过最近邻自训练方法，仅需少量标注数据即可实现海洋物种分类，在AQUA20基准测试中接近全监督模型性能。
...
Method: 采用冻结的DINOv3 ViT-B嵌入表示，无需微调，通过最近邻自训练方法将少量标注种子传播到未标注数据，在AQUA20数据集（20种海洋物种）上进行评估。

Result: 仅使用不到5%的训练标签，自训练方法就能接近全监督ConvNeXt基线的性能；在全监督条件下，差距缩小到几个百分点，部分物种甚至超过监督基线。嵌入空间的类别可分性（ROC-AUC）即使在极低标注量下也很高。

Conclusion: 该方法无需训练、无需领域特定数据工程、无需水下适应模型，为标注高效的海洋物种识别提供了一个实用、可立即部署的基线方案。
...
<details>
  <summary>Details</summary>
Motivation: 激光雷达对自动驾驶感知至关重要，但易受干扰攻击影响，攻击会注入高频激光脉冲使传感器失明。虽然点云数据会随机化，但全波形数据保留了攻击信号与合法信号的可区分特征。

Method: 提出PULSAR-Net，利用现代激光雷达系统中未充分利用的中间全波形表示和同步激光传感。采用新颖的U-Net架构，配备轴向空间注意力机制，专门设计用于在全波形表示中识别攻击信号与真实物体返回。为解决现有数据集中缺乏干扰攻击下的全波形数据，开发了物理感知的数据集生成管道，合成干扰攻击下的真实全波形表示。

Result: 尽管仅使用合成数据训练，PULSAR-Net在真实世界静态和驾驶场景中，对受干扰攻击遮挡的车辆分别达到92%和73%的重建率。

Conclusion: 通过利用全波形数据的特征，PULSAR-Net能够有效对抗激光雷达干扰攻击，在真实场景中成功重建被攻击遮挡的物体点云，为自动驾驶感知系统提供了重要的安全保障。
...
TL;DR: 提出一种基于动态图模型的自适应节点选择方法，用于RGB-D室内场景识别，通过选择关键局部特征并建模物体间关系，在公开数据集上取得优于现有方法的效果。
...
Method: 提出动态图模型，包含自适应节点选择机制：1）构建动态图建模物体与场景关系；2）提出自适应节点选择方法从RGB和深度两种模态中提取关键局部特征；3）将节点按三个不同层次分组，表示物体间的远近关系；4）根据注意力权重动态更新图模型；5）将更新优化的RGB和深度特征融合用于室内场景识别。

Result: 在SUN RGB-D和NYU Depth v2公开数据集上进行实验，广泛结果表明该方法相比现有最先进方法具有优越性能，证明能够有效利用RGB和深度两种模态的关键局部特征。
...
Method: 结合毫米波雷达与RGBD输入，使用ResNet分类器提取场景几何和材料属性等语义线索，通过条件潜在扩散模型合成给定视觉上下文下的预期毫米波频谱，最后通过双输入比较模块识别真实与生成频谱的空间偏差来定位异常

Result: 在三个应用场景（隐蔽武器定位、穿墙入侵者定位、穿墙跌倒定位）上评估，系统达到94%的F1分数和亚米级定位误差，在衣物、遮挡和杂乱环境中表现出鲁棒泛化能力

Conclusion: mmAnomaly建立了毫米波传感中上下文感知异常检测的准确且可解释框架，解决了传统方法缺乏上下文感知的问题
...
TL;DR: TEP方法通过引入跟踪增强提示，解决了SAM3在微小和语义主导目标上分割性能不足的问题，在PVUW挑战赛中获得第一名


<details>
  <summary>Details</summary>
Motivation: 虽然SAM3在复杂视频对象分割任务中表现出色，但在微小目标和语义主导对象上表现不佳，主要原因是SAM3对这些特定目标类型的理解不足

Method: 提出TEP方法，这是一种无需训练的方法，利用外部跟踪模型和多模态大语言模型引入跟踪增强提示，帮助SAM3更好地理解具有挑战性的目标

Result: 在PVUW Challenge 2026: Complex Video Object Segmentation Track测试集上获得了第一名，得分为56.91%

Conclusion: 通过跟踪增强提示可以有效提升SAM3在复杂视频对象分割中对微小和语义主导目标的理解能力，显著改善分割性能
...
TL;DR: 提出VLM-in-the-Loop插件式质量保证模块，通过工具接地机制提升心电图数字化质量，无需修改底层数字化器，在真实临床图像上达到98%优秀质量。


<details>
  <summary>Details</summary>
Motivation: 心电图数字化可解锁数十亿存档临床记录，但现有方法在真实世界图像上表现不佳，尽管基准测试数据良好。需要一种能提升现有数字化器质量而不修改其代码的解决方案。

Method: 提出VLM-in-the-Loop插件模块，通过标准化接口包装任何数字化后端，利用工具接地机制：将VLM评估锚定在特定领域信号分析工具提供的定量证据上，形成闭环反馈。

Result: 工具接地将判决一致性从71%提升到89%，保真度分离度翻倍(ΔPCC 0.03→0.08)；在三个VLM上效果一致；部署到四个后端均获改善：边界导联改善29.4%，失败肢体导联恢复41.2%，每图像有效导联数翻倍(2.5→5.8)；在428个真实临床HCM图像上达到98.0%优秀质量。
...
TL;DR: VAE-MMD域适应框架通过变分自编码器和最大均值差异损失，结合nnU-Net分割，显著提升脑转移瘤在多中心数据上的分割性能，无需目标域标签。


<details>
  <summary>Details</summary>
Motivation: 深度学习在脑转移瘤自动分割方面潜力显著，但单一机构训练的模型在不同站点因扫描硬件、成像协议和患者人口统计学差异而表现不佳。需要创建跨多机构的域适应框架。

Method: 提出VAE-MMD预处理管道，结合变分自编码器（VAE）和最大均值差异（MMD）损失，包含跳跃连接和自注意力机制，与nnU-Net分割结合。在斯坦福、UCSF、UCLM和PKG四个公共数据库的740名患者上进行测试。

Result: VAE-MMD将域分类器准确率从0.91降至0.50，表明跨机构特征对齐成功。重建体积PSNR大于36dB，保持解剖准确性。相比基线nnU-Net，平均F1提升11.1%（0.700到0.778），平均表面Dice提升7.93%（0.7121到0.7686），平均HD95降低65.5%（11.33到3.91mm）。
...
TL;DR: 研究探讨了轨迹预测模型QCNet从美国数据迁移到韩国道路环境的适应性，比较了四种训练策略，发现选择性微调解码器并冻结编码器在准确性和训练效率方面达到最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 当前大多数公开数据集（如Waymo和Argoverse）基于西方道路环境，无法反映韩国等地区的独特交通模式、基础设施和驾驶行为。这种领域差异导致基于西方数据训练的最先进模型在不同地理环境中部署时性能下降。

Method: 使用韩国自动驾驶数据集，比较了四种训练策略：零样本迁移、从头开始训练、完全微调以及编码器冻结。重点研究了Query-Centric Trajectory Prediction (QCNet)模型从美国数据到韩国道路环境的迁移能力。

Result: 实验结果表明，利用预训练知识显著提高了预测性能。具体而言，选择性微调解码器同时冻结编码器在准确性和训练效率方面达到最佳平衡，与从头开始训练相比，预测误差减少了超过66%。

Conclusion: 这项研究为在新地理领域中部署轨迹预测模型提供了有效的迁移学习策略实用见解，证明了预训练知识在跨域适应中的重要性，特别是编码器冻结与解码器微调的组合策略。
...
Conclusion: 提出的无训练方法有效解决了基于运动语言表达的视频对象分割问题，展示了多模态大语言模型与分割模型的强大组合能力
...
TL;DR: FADE是一种针对基于查询传播的跟踪方法的新型对抗攻击框架，通过时间查询洪泛和时间记忆破坏两种策略，利用TBP方法的架构漏洞进行攻击。


<details>
  <summary>Details</summary>
Motivation: 虽然基于查询传播的跟踪方法在多目标跟踪中取得了进展，但其依赖查询传播的架构存在未探索的对抗攻击漏洞。作者旨在揭示这些特定漏洞并开发相应的攻击框架。

Method: FADE采用两种攻击策略：1) 时间查询洪泛：生成虚假的时间一致跟踪查询来耗尽跟踪器的有限查询预算；2) 时间记忆破坏：通过状态去相关和擦除匹配轨迹的特征身份来直接攻击查询更新器的记忆。此外，还引入了可微分管道来优化攻击以实现物理世界可行性。

Result: 在MOT17和MOT20基准测试上的实验表明，FADE对最先进的TBP跟踪器非常有效，导致显著的身份切换和轨迹终止。

Conclusion: FADE框架成功揭示了基于查询传播的跟踪方法的架构漏洞，并展示了对抗攻击对这些方法的严重威胁，为未来更鲁棒的跟踪系统设计提供了重要启示。
...
TL;DR: 本文提出First Logit Boosting (FLB)方法，一种无需训练的技术，通过存储并添加首个生成token的logit到后续token预测中，有效缓解大型视觉语言模型中的长期衰减问题，显著减少物体幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多种多模态任务中表现出色，但物体幻觉（生成不存在物体的答案）仍然是一个持续挑战。现有方法如重新训练和外部接地方法存在高数据成本或结构复杂性问题，而无需训练的方法如对比解码则存在长期衰减问题，即随着生成过程进行，视觉接地减弱而语言先验主导。

Method: 提出First Logit Boosting (FLB)方法，这是一种简单有效的无需训练技术。核心思想是存储第一个生成token的logit，并将其添加到后续token的预测中。这种方法能够：(1) 在整个生成过程中维持嵌入在第一个token中的视觉信息；(2) 通过"The" token的稳定化效应抑制幻觉词。

Result: 实验结果表明，FLB在各种任务、基准测试和骨干模型中都能显著减少物体幻觉。该方法仅带来可忽略的推理开销，使其非常适合实时多模态系统应用。

Conclusion: FLB是一种简单而有效的无需训练方法，通过缓解长期衰减问题，显著减少了大型视觉语言模型中的物体幻觉，具有实际应用价值。
...
TL;DR: 研究比较了传统分割工具与基于Transformer的深度学习模型在7T MRI多发性硬化白质病变分割中的表现，发现7T训练的Transformer模型在原生高分辨率下能检测到更多小病变，但存在边界变异性和假阳性问题。


<details>
  <summary>Details</summary>
Motivation: 超高场7T MRI能更好地显示多发性硬化白质病变，但其对比度和伪影与1.5-3T成像差异较大，现有自动化分割工具可能无法直接应用，需要开发专门针对7T MRI的分割方法。

Method: 使用7T FLAIR扫描数据，通过LST工具生成参考病变掩膜并经专家手动修订。比较了传统LST-LPA和LST-AI工具，并训练了3D UNETR和SegFormer等Transformer模型在不同分辨率(0.5³、1.0³、1.5×2.0³)下的表现，采用BraTS 2023框架的体素级和病变级指标进行评估。

Result: 在原生0.5³分辨率测试集上，7T训练的Transformer模型与LST-AI表现相当，但能检测到更多传统方法遗漏的小病变，代价是边界变异性和偶尔的伪影相关假阳性。最佳SegFormer模型达到体素级Dice 0.61和病变级Dice 0.20，优于传统LST-LPA工具(Dice 0.39，病变级Dice 0.02)。降采样训练模型性能下降，表明原生7T分辨率对小病变检测至关重要。

Conclusion: 针对7T MRI训练的Transformer模型在多发性硬化白质病变分割中优于传统方法，特别是在小病变检测方面。原生高分辨率训练对性能至关重要。研究发布了7T训练模型，为超高场多发性硬化研究提供了可复现的自动化病变量化工具。
...
TL;DR: 本文提出MUPO方法解决GRPO训练中出现的多样性崩溃问题，通过激励多组策略优化来促进发散性思维，提升视觉语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 研究发现强化学习（特别是GRPO）虽然能提升视觉语言模型的推理能力，但会导致模型陷入深度但狭窄的推理模式，而基础模型虽然推理不够精细但思维模式更广泛多样。GRPO容易发生多样性崩溃，使模型过早收敛到有限的推理策略子集，丢弃大多数潜在替代方案，导致局部最优和可扩展性差。

Method: 提出多组策略优化（MUPO）方法，这是一种简单但有效的技术，旨在激励跨多个解决方案的发散性思维。MUPO通过维护多个策略组来避免多样性崩溃，促进更广泛的推理策略探索。

Result: 在已建立的基准测试上证明了MUPO的有效性，能够解决GRPO的多样性崩溃问题，提升模型的推理能力和可扩展性。

Conclusion: MUPO通过激励发散性思维解决了强化学习训练中的多样性崩溃问题，为提升视觉语言模型的推理能力提供了一种有效方法，相比传统GRPO具有更好的可扩展性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 胸部X光片是全球最常进行的影像检查之一，但不断增长的影像量增加了放射科医生的工作负担和诊断错误风险。现有AI系统大多只生成最终预测，没有明确解释视觉证据如何转化为放射学发现和诊断预测。

Method: 使用从30个公共数据集的36个CXR解读任务中提取的1470万个指令和推理样本进行训练，采用两阶段框架结合指令微调和强化学习来提升推理质量。

Result: 在17个评估设置中，CheXOne在零样本设置下优于现有医学和通用领域基础模型，在独立公共基准测试中表现强劲。临床阅读研究显示，CheXOne生成的报告在55%的情况下优于或等同于住院医师撰写的报告，有效处理临床指征并提高报告撰写和CXR解读效率。

Conclusion: 明确的推理可以提升AI辅助CXR解读中的模型性能、可解释性和临床实用性，生成的推理轨迹具有高临床事实性并为最终预测提供因果支持。
...
TL;DR: ARGS：一种用于3D对象生成的自回归高斯溅射框架，通过层次化树结构和多尺度预测实现高效可控的细节生成


<details>
  <summary>Details</summary>
Motivation: 自回归框架在2D图像生成中表现出色，但扩展到3D对象生成仍未被充分探索。现有方法在3D生成中缺乏高效的多尺度控制和结构一致性
...
Conclusion: ARGS框架成功将自回归范式扩展到3D对象生成，实现了高效、可控的多尺度3D内容生成
...
TL;DR: PC-SAM：一个统一框架，结合了全自动道路分割和交互式分割，通过点提示实现精细的局部掩码修正，在遥感道路分割任务中优于现有全自动方法。


<details>
  <summary>Details</summary>
Motivation: 当前全自动道路分割方法在识别具有挑战性的道路段时存在不足，经常产生假阳性和假阴性区域，且不支持局部感兴趣区域分割或现有掩码的细化。虽然SAM模型在自然图像交互分割中表现良好，但在遥感道路分割中效果不佳，无法支持细粒度的局部修正。

Method: 提出PC-SAM框架，将全自动道路分割和交互式分割集成到统一框架中。通过精心设计的微调策略，将点提示的影响限制在对应的图像块内，克服了原始SAM无法进行精细局部修正的缺点，实现了细粒度的交互式掩码细化。

Result: 在多个代表性遥感道路分割数据集上的实验表明，结合点提示使用时，PC-SAM在道路掩码分割方面显著优于最先进的全自动模型，同时提供灵活的局部掩码细化和局部道路分割能力。

Conclusion: PC-SAM通过统一全自动和交互式分割框架，解决了遥感道路分割中全自动方法的局限性，提供了更精确的道路掩码分割和灵活的局部修正功能，为下游任务提供了更好的支持。
...
TL;DR: PET-DINO是一个支持文本和视觉提示的通用检测器，通过AFVPG模块和两种提示增强训练策略，解决了开放集目标检测中文本表示与复杂视觉概念对齐困难的问题。


<details>
  <summary>Details</summary>
Motivation: 开放集目标检测面临文本表示与复杂视觉概念对齐困难、稀有类别图像-文本对稀缺的问题，导致在专业领域或复杂物体上性能不佳。现有视觉提示方法通常涉及复杂的多模态设计和多阶段优化，延长开发周期，且数据驱动的OSOD训练策略研究不足。

Method: 提出PET-DINO通用检测器，包含对齐友好的视觉提示生成模块，在先进文本提示检测器基础上构建。引入两种提示增强训练策略：迭代级的批内并行提示和整体训练级的动态记忆驱动提示，支持同时建模多个提示路径，实现与多样化真实使用场景的并行对齐。

Result: 综合实验表明，PET-DINO在各种基于提示的检测协议上展现出竞争力的零样本目标检测能力。继承式哲学和提示增强训练策略在构建有效通用目标检测器中发挥关键作用。

Conclusion: PET-DINO通过AFVPG模块和创新的训练策略，有效解决了开放集目标检测中的对齐和训练效率问题，为构建通用目标检测器提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有弱监督HOI检测方法依赖外部物体检测器生成候选对，通过成对推理推断交互，但存在计算成本高（枚举大量实例对）和非交互组合导致的假阳性问题，阻碍准确的实例级HOI推理。

Method: 提出Relational Grounding Transformer (RegFormer)，利用空间接地信号作为推理过程的引导，促进局部感知的交互学习。通过学习局部化交互线索，模块能够区分人、物体及其交互，实现从图像级交互推理到精确高效的实例级推理的直接迁移。

Result: 实验表明RegFormer能有效学习空间线索进行实例级交互推理，运行效率高，甚至达到与全监督模型相当的性能。

Conclusion: RegFormer解决了弱监督HOI检测中的计算效率和假阳性问题，通过空间接地信号实现了从图像级到实例级推理的有效迁移，为可扩展的场景理解提供了高效准确的解决方案。
...
TL;DR: MAESIL是一种针对3D医学影像的自监督学习框架，通过创新的"超块"设计和双掩码策略，有效捕捉3D结构信息，在CT图像重建任务中显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 3D医学影像（如CT）的深度学习训练面临标注数据稀缺的挑战。现有自监督学习方法通常将3D扫描处理为独立的2D切片，这丢弃了关键的轴向连贯性和3D结构上下文信息，限制了性能提升。

Method: 提出MAESIL框架，核心创新是"超块"——一种平衡3D上下文保存与计算效率的3D块状输入单元。采用3D掩码自编码器策略和双掩码策略，将体积分割为超块，学习全面的空间表示。

Result: 在三个不同的大规模公共CT数据集上验证，MAESIL在PSNR和SSIM等关键重建指标上显著优于AE、VAE和VQ-VAE等现有方法。

Conclusion: MAESIL为3D医学影像任务提供了一个强大且实用的预训练解决方案，能够有效捕捉3D结构信息，解决现有方法忽略3D上下文的局限性。
...
TL;DR: 提出基于可学习性指导的数据集蒸馏方法，通过增量式构建合成数据集减少冗余，在ImageNet等数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的数据集蒸馏方法产生冗余的训练信号，不同样本传递重叠信息而非互补知识，导致效率低下

Method: 提出可学习性指导的数据集蒸馏（LGD），通过增量式构建合成数据集，使用可学习性评分指导生成新样本，平衡当前模型的训练效用和参考模型的有效性

Result: 冗余减少39.1%，在ImageNet-1K达到60.1%、ImageNette 87.2%、ImageWoof 72.9%的SOTA性能

Conclusion: 基于可学习性指导的增量式数据集蒸馏能有效减少冗余，促进训练阶段专业化，显著提升数据集蒸馏性能
...
TL;DR: AceTone提出首个支持多模态条件色彩分级的方法，通过生成3D-LUTs实现文本或参考图像驱动的色彩调整，在保真度和美学对齐方面优于现有方法。
...
Method: 将色彩分级建模为生成式色彩转换任务，使用VQ-VAE将3D-LUT向量压缩为离散token，构建大规模数据集并训练视觉语言模型预测LUT token，最后通过强化学习对齐感知保真度和美学。

Result: 在文本引导和参考引导的色彩分级任务中达到最先进性能，LPIPS指标提升高达50%，人类评估确认结果视觉愉悦且风格一致。

Conclusion: AceTone展示了语言驱动、美学对齐的色彩分级新途径，通过统一框架支持多模态条件控制，显著提升了色彩分级的质量和泛化能力。
...
TL;DR: FreqPhys是一个基于频率引导的远程光电容积描记术框架，通过显式利用生理频率先验来增强信号恢复的鲁棒性，有效应对运动伪影和光照变化带来的挑战。
...
Method: 提出FreqPhys框架：1) 生理带通滤波模块抑制带外干扰；2) 生理频谱调制和自适应频谱选择增强脉搏相关频率成分；3) 跨域表示学习模块融合频谱先验和深度时域特征；4) 频率感知条件扩散过程逐步重建高质量rPPG信号。

Result: 在六个基准测试上的广泛实验表明，FreqPhys相比最先进方法有显著改进，特别是在具有挑战性的运动条件下表现优异。

Conclusion: FreqPhys框架通过显式建模生理频率先验，显著提升了rPPG信号恢复的鲁棒性和准确性，特别是在复杂运动条件下。这突显了显式利用生理频率先验的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶从稀疏感知发展到VLA模型，但作者认为车辆在3D世界中运行，密集3D几何信息才是决策的最全面信息源。然而现有几何重建方法依赖多帧批量处理，无法用于在线规划。

Method: 提出DVGT-2模型，采用在线处理方式，使用时间因果注意力和历史特征缓存支持实时推理。引入滑动窗口流式策略，在特定间隔内使用历史缓存避免重复计算，能够同时输出密集几何重建和轨迹规划。

Result: DVGT-2在多个数据集上实现了更优的几何重建性能，且速度更快。同一训练好的DVGT-2模型可以直接应用于不同相机配置的规划任务，无需微调，在NAVSIM闭环和nuScenes开环基准测试中表现良好。

Conclusion: Vision-Geometry-Action (VGA)范式通过密集3D几何信息为自动驾驶决策提供更全面的依据，DVGT-2模型实现了高效的在线几何重建和规划，展示了3D几何信息在自动驾驶中的关键作用。
...
TL;DR: TRiGS是一种新型4D表示方法，通过统一的连续几何变换解决现有4D高斯泼溅方法中的时间碎片化和内存增长问题，实现了长视频序列的高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 现有4D高斯泼溅方法依赖分段线性速度近似和短时间窗口，导致严重的时间碎片化，迫使基元反复消除和再生以跟踪复杂非线性动态。这种临时近似消除了物体的长期时间一致性，导致高斯基元不可避免的增殖，阻碍了扩展到长视频序列的可扩展性。

Method: TRiGS采用统一的连续几何变换，整合了SE(3)变换、分层贝塞尔残差和可学习的局部锚点，为单个基元建模几何一致的刚性运动。这种连续表示保留了时间一致性，有效缓解了无界内存增长。

Result: TRiGS在标准基准测试中实现了高保真渲染，同时独特地扩展到长视频序列（600到1200帧）而没有严重的内存瓶颈，在时间稳定性方面显著优于先前工作。

Conclusion: TRiGS通过连续几何变换表示解决了4D高斯泼溅中的时间碎片化和内存增长问题，为动态场景重建提供了可扩展的解决方案，特别适用于长视频序列。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D语义占据预测方法存在两个主要问题：1）对长尾类别偏差敏感，容易将异常过度自信地分配给稀有类别；2）对分布外（OOD）输入脆弱，影响自动驾驶系统的安全性。

Method: ProOOD包含三个核心组件：1）原型引导的语义填补，用类别一致的特征填充遮挡区域；2）原型引导的尾部挖掘，增强稀有类别表示以减少OOD吸收；3）EchoOOD，融合局部logit一致性与局部全局原型匹配，生成可靠的体素级OOD分数。

Result: 在五个数据集上的实验表明，ProOOD在3D占据预测和OOD检测方面均达到最先进性能：在SemanticKITTI上，整体mIoU提升+3.57%，尾部类别mIoU提升+24.80%；在VAA-KITTI上，AuPRCr提升+19.34个点，各基准测试均有稳定提升。

Conclusion: ProOOD通过原型引导的细化和免训练OOD评分，显著提高了3D语义占据预测的校准度和OOD检测的可靠性，为安全关键的城市驾驶场景提供了更稳健的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 前馈重建模型（FFRMs）虽然重建质量好且适应多种下游任务，但过度依赖多视角几何标注（如3D点云图和相机位姿），使得全监督训练方案难以扩展。需要降低对昂贵几何传感器数据和计算密集型SfM预处理的依赖。

Method: 提出Reliev3R弱监督训练范式，核心设计包括：1）模糊感知相对深度损失，处理零样本预测深度中的尺度模糊性；2）基于三角测量的重投影损失，利用稀疏对应关系监督多视角几何一致性。直接从预训练模型的零样本预测中获取单目相对深度和图像稀疏对应关系。

Result: 使用更少的数据从头训练，Reliev3R能够达到其全监督兄弟模型的性能水平，为低成本3D重建监督和可扩展FFRMs迈出了重要一步。

Conclusion: Reliev3R成功实现了无需昂贵多视角几何标注的弱监督FFRMs训练，通过利用零样本预测的几何信息，显著降低了3D重建监督的成本和复杂性，推动了可扩展前馈重建模型的发展。
...
TL;DR: TF-SSD是一种基于SAM和DINO协同的训练无关共显著目标检测方法，通过生成候选掩码池、质量筛选、语义过滤和跨图像原型选择来实现高性能的共显著检测。


<details>
  <summary>Details</summary>
Motivation: 现有基于训练的共显著目标检测方法受限于封闭数据集且泛化能力有限，而视觉基础模型(VFMs)展现出强大的泛化能力和显著性理解能力，但很少有研究探索利用VFMs解决CoSOD问题。

Method: 提出TF-SSD方法：1)使用SAM生成原始候选掩码池；2)基于SAM的质量掩码生成器过滤冗余掩码；3)利用DINO注意力图进行单图像内显著性过滤；4)提出跨图像原型选择器，通过计算跨图像原型相似度选择最高分掩码作为最终预测。
...
TL;DR: STAR框架通过拓扑锚点和两阶段训练解决LLM空间导航中的级联错误问题，在RedMaze-23K数据集上达到开源模型最佳性能
...
Method: 提出STAR两阶段框架：第一阶段使用监督微调让模型内化空间语义并剪枝冗余路径；第二阶段采用空间感知分段级直接偏好优化（SDPO）来精炼长视野导航中的自我校正

Result: STAR的32B变体在性能上超过DeepSeek-V3（29.27% vs. 25.00%），达到GPT-4性能的82.4%，在开源模型中达到最先进水平

Conclusion: STAR框架通过拓扑锚点和两阶段训练有效解决了LLM空间导航中的级联错误问题，显著提升了复杂拓扑环境下的导航性能
...
TL;DR: FecalFed是一个保护隐私的联邦学习框架，用于家禽疾病分类，通过去重数据集和联邦学习方法解决了数据隐私和数据污染问题，在非IID数据分布下实现了接近集中式训练的性能。


<details>
  <summary>Details</summary>
Motivation: 家禽高致病性禽流感和地方性疾病的早期检测对全球粮食安全至关重要，但计算机视觉模型的大规模部署受到农场数据隐私问题和机构数据孤岛的阻碍，同时现有开源农业数据集存在严重的数据污染问题。

Method: 提出了FecalFed隐私保护联邦学习框架，首先创建并发布了经过严格去重的poultry-fecal-fl数据集（8,770张独特图像），在高度异构的非IID条件下（Dirichlet α=0.5）评估联邦学习方法，使用服务器端自适应优化（FedAdam）和Swin-Small架构。

Result: 发现流行公共数据集中存在46.89%的重复率；在数据异构性下，孤立单农场训练准确率仅为64.86%，而联邦学习方法达到90.31%准确率，接近集中式上限95.10%；边缘优化的Swin-Tiny模型也保持了89.74%的竞争性性能。

Conclusion: FecalFed框架为农场禽病监测提供了一个高效、隐私优先的蓝图，能够在保护敏感数据隐私的同时，在高度异构的农业环境中实现接近集中式训练的性能。
...
TL;DR: HarassGuard：基于视觉语言模型的社交VR骚扰检测系统，仅使用视觉输入实现主动骚扰检测，保护用户隐私
...
Conclusion: HarassGuard提供了一种有效的隐私保护骚扰检测方案，在保持竞争力的同时减少了数据需求，在上下文推理和隐私保护检测方面具有独特优势
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶轨迹规划模型严重依赖训练时看到的相机视角，缺乏对视角变化的鲁棒性。需要一种不依赖数据增强的方法来提高模型在相机视角变化下的泛化能力。

Method: 提出无增强方法，利用3D基础模型的几何先验：1）通过深度估计获取每个像素的3D位置作为位置嵌入；2）通过交叉注意力机制融合中间几何特征；3）在VR-Drive相机视角扰动基准上进行评估。

Result: 在VR-Drive基准测试中，该方法在大多数扰动条件下减少了性能下降，在俯仰和高度扰动方面有明显改进。在纵向平移方面的增益较小，表明需要更视角无关的集成来实现对相机视角变化的鲁棒性。

Conclusion: 利用3D基础模型的几何先验可以有效提高自动驾驶轨迹规划在相机视角变化下的鲁棒性，特别是在俯仰和高度扰动方面。但需要进一步开发更视角无关的集成方法以应对纵向平移等挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学视觉问答方法未能充分利用领域专业知识，难以准确关联医学图像中的病灶特征与诊断标准。同时，基于分类的方法依赖预定义答案集，限制了自由形式答案的适应性，可能忽略答案中的详细语义信息。

Method: 提出知识图谱增强的跨Mamba交互框架(KG-CMI)，包含四个模块：细粒度跨模态特征对齐(FCFA)、知识图谱嵌入(KGE)、跨模态交互表示(CMIR)和自由形式答案增强多任务学习(FAMT)。该框架通过图结构有效整合专业医学知识，建立病灶特征与疾病知识关联，并利用开放性问题辅助知识提升模型能力。

Result: 实验结果表明，KG-CMI在三个医学VQA数据集(VQA-RAD、SLAKE、OVQA)上优于现有最先进方法。此外，通过可解释性实验进一步验证了框架的有效性。

Conclusion: KG-CMI框架通过知识图谱增强和跨模态交互，有效解决了医学视觉问答中知识利用不足和答案形式受限的问题，为临床决策支持和远程医疗提供了更强大的多模态解决方案。
...
TL;DR: 研究发现对抗性攻击下，物体检测器的检测数量与精度可能不耦合：EMS-YOLO在精度崩溃时仍保留70%以上检测，揭示了新的失败模式"质量腐化"。


<details>
  <summary>Details</summary>
Motivation: 传统对抗性攻击防御工具假设检测精度下降时检测数量会同步下降，但这种耦合关系从未被实际测量验证。研究旨在探索这一假设的有效性。

Method: 在四个SNN架构和两种威胁模型（l-infinity和l-2）下测试，使用标准PGD攻击，特别关注EMS-YOLO模型在对抗攻击下的表现。

Result: EMS-YOLO在mAP从0.528崩溃到0.042时仍保留超过70%的检测数量，这种现象被定义为"质量腐化"。其他三个SNN检测器未出现此现象，且五种标准防御组件均无法检测或缓解QC。

Conclusion: 对抗性失败模式具有底物依赖性，传统防御生态系统可能基于单一底物校准的共享假设，需要重新评估对抗性攻击防御的基本假设。
...
TL;DR: TALENT框架通过目标感知高效调优解决基于参数高效调优的参考图像分割中的非目标激活问题，使用修正成本聚合器和目标感知学习机制提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于参数高效调优的参考图像分割方法存在非目标激活问题，即视觉特征无法准确聚焦文本所指的目标实例，反而激活了同类别但不相关的对象。

Method: 提出TALENT框架：1) 修正成本聚合器高效聚合文本参考特征；2) 目标感知学习机制包括上下文成对一致性学习和目标中心对比学习，前者通过句子级文本特征构建文本参考亲和图，后者增强目标定位并抑制无关关联。

Result: 在多个指标上超越现有方法，如在G-Ref验证集上获得2.5%的mIoU提升。

Conclusion: TALENT框架有效解决了非目标激活问题，通过目标感知高效调优显著提升了参考图像分割的性能。
...
TL;DR: 本文提出DirectFisheye-GS方法，将鱼眼相机模型直接集成到3DGS框架中，避免传统预处理导致的图像信息损失和细节稀释问题，并通过特征重叠驱动的跨视图联合优化策略解决边缘浮动物体问题。


<details>
  <summary>Details</summary>
Motivation: 鱼眼相机具有更宽的视野，可以用更少的输入实现高质量重建，但现有3DGS方法需要先对鱼眼图像进行去畸变预处理，这会导致两个问题：1) 图像边缘出现黑边，造成信息损失并抵消鱼眼大视野优势；2) 去畸变的拉伸和插值重采样会稀释像素值，导致3DGS在低频区域过拟合，产生模糊和浮动物体。

Method: 1) 将鱼眼相机模型直接集成到原始3DGS框架中，支持原生鱼眼图像输入训练而无需预处理；2) 提出特征重叠驱动的跨视图联合优化策略，建立跨视图的一致几何和光度约束，解决边缘浮动物体问题。

Result: DirectFisheye-GS在公开数据集上达到或超越了最先进方法的性能，有效解决了鱼眼图像重建中的边缘浮动物体问题。

Conclusion: 通过将鱼眼相机模型直接集成到3DGS框架并引入跨视图联合优化策略，DirectFisheye-GS实现了高质量的鱼眼图像3D重建，避免了传统预处理方法的信息损失和细节稀释问题，该方法同样适用于现有的针孔相机管道。
...
TL;DR: 研究揭示AI在皮肤镜图像诊断中的系统性失败不仅源于算法偏差，更反映了图像本身的固有复杂性，人类专家在AI误判的图像上同样表现不佳，图像质量是双重失败的主要驱动因素。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注AI算法与人类专家的性能对比，而本研究从新视角出发，探究皮肤镜图像本身的内在复杂性，特别是那些被多种CNN架构系统性误判的图像，以确定失败源于算法偏差还是图像本身的视觉模糊性。

Method: 使用多种CNN架构进行严格实验，识别出所有模型都系统性误判的图像子集；然后让皮肤科专家独立评估这些挑战性案例和对照组图像；通过Cohen's kappa和Fleiss kappa等统计指标量化人类诊断性能和专家共识的变化。

Result: AI系统性误判的图像同样导致人类专家诊断性能崩溃：与真实标签的一致性从对照组的kappa=0.61骤降至0.08；专家共识从中等一致性（Fleiss kappa=0.456）恶化到仅微弱一致性（0.275）；图像质量被确定为双重系统性失败的主要驱动因素。

Conclusion: AI在皮肤镜诊断中的失败不仅反映了算法局限性，更揭示了图像本身的固有复杂性；这些"困难"图像对人类专家同样构成挑战，表明问题根源在于图像质量而非单纯算法偏差；研究强调了提高诊断系统鲁棒性需要同时关注算法改进和图像质量提升。
...
TL;DR: CL-VISTA是一个专门为视频大语言模型设计的持续学习基准测试，包含8个多样化任务，通过6种评估协议在3个维度上全面评估持续学习方法，揭示了性能、计算效率和内存占用之间的基本权衡。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习基准测试存在局限性：许多仍基于未经过大规模预训练的模型，且通常将单个数据集划分为子任务，导致任务冗余度高，对预训练视频大语言模型的遗忘效应不明显。需要专门针对视频大语言模型的持续学习基准来评估真实场景下的灾难性遗忘问题。

Method: 提出CL-VISTA基准测试，精心策划8个多样化任务，涵盖感知、理解和推理三个层面，以产生显著分布偏移。建立包含6种不同协议的全面评估框架，覆盖性能、计算效率和内存占用三个关键维度，其中性能维度包含通用视频理解评估以区分真实智能提升与任务特定过拟合。

Result: 对10种主流持续学习方法进行广泛基准测试，发现基本权衡：没有任何单一方法在所有维度上实现普遍优势。成功缓解灾难性遗忘的方法往往以牺牲泛化能力或产生过高计算和内存开销为代价。

Conclusion: CL-VISTA为推进多模态基础模型的持续学习提供了关键见解，揭示了当前持续学习方法在平衡遗忘缓解、泛化能力和资源效率方面的挑战，为未来研究指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有的月球数据集通常缺乏几何真值、光度学真实性、光照多样性或大规模覆盖，这阻碍了基于学习的感知系统的开发。需要提供全面的几何和光度学监督数据集来支持月球探索任务。

Method: 基于真实月球地形，使用基于物理的渲染创建统一基准。包含两个互补子数据集：1) LunarGeo提供立体图像、密集深度图和相机标定；2) LunarPhoto使用空间变化的BRDF模型生成真实感图像，并在真实太阳配置下进行多光照渲染。

Result: 创建了包含超过13万个样本的综合数据集，提供几何和光度学监督。建立了最先进方法的基线，并发布了完整数据集和生成工具，支持社区扩展。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一分割方法通常依赖异构任务和模态间的共享编码器，导致特征纠缠、梯度干扰和病灶区分能力不足，需要一种更好的统一医学病灶分割框架。
...
TL;DR: 提出TTA-Vid方法，通过测试时强化学习在无标签情况下自适应视频推理模型，结合批量感知频率奖励和多臂老虎机帧选择策略，在单批次甚至单样本上训练即可泛化到整个数据集。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理模型依赖大规模监督数据和多阶段训练流程，成本高昂且难以适应新领域。需要一种能在测试时自适应、无需标注的方法来降低训练成本并提高适应性。

Method: 提出TTA-Vid方法：1）测试时适应在推理时对多个帧子集进行逐步推理；2）使用跨不同帧子集计算的批量感知频率奖励作为伪真值更新模型；3）提出多臂老虎机策略进行自适应帧选择，优先选择信息丰富的帧。

Result: TTA-Vid在单批次或单样本上训练即可泛化到整个数据集甚至跨数据集，在多种视频推理任务上取得一致改进，优于当前基于大规模数据训练的SOTA方法。

Conclusion: 测试时强化学习在时序多模态理解方面具有巨大潜力，TTA-Vid方法无需标注和专用训练分割即可实现有效的测试时自适应，为视频推理提供了高效且适应性强的解决方案。
...
TL;DR: 提出一种基于注意力对偶形式视角的无训练token剪枝框架，通过渐进式分块最大边际相关性选择最优token子集，在LVLMs中实现性能与效率的更好权衡。
...
Method: 从注意力的对偶形式视角出发，将注意力重新表述为隐式线性层，其权重矩阵是rank 1外积和。提出衡量token信息量和信息重复度的新度量，并引入渐进式分块最大边际相关性来高效选择最优token子集。
...
TL;DR: PrivHAR-Bench是一个多层级基准数据集，用于标准化评估视频动作识别中的隐私-效用权衡，包含9个隐私强度递增的转换层级和背景移除变体。


<details>
  <summary>Details</summary>
Motivation: 现有隐私保护人体活动识别研究通常采用二元评估范式（清晰视频vs单一隐私转换），限制了跨方法可比性，且模糊了隐私强度与识别效用之间的微妙关系。

Method: 创建PrivHAR-Bench数据集，包含15个活动类别、1,932个源视频，应用从轻量级空间模糊到密码块置换的渐进式视觉隐私转换，分为9个隐私强度递增的层级，并提供背景移除变体以分离人体运动特征与场景背景影响。

Result: 使用R3D-18模型验证显示，识别准确率随隐私强度增加呈可测量且可解释的下降曲线：层级内准确率从88.8%（清晰）降至53.5%（加密+背景移除），跨域准确率崩溃至4.8%。
...
TL;DR: 本文提出了SurgSTU-Pipeline生成框架和SurgSTU数据集，用于提升手术视频中细粒度时空关系的多模态理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有手术视觉语言数据集缺乏对复杂、交织的时空动态的捕捉和评估能力，而大规模数据集的创建面临昂贵的人工标注或大语言模型生成错误的问题。
...
Result: 评估显示，最先进的通用视觉语言模型在零样本设置下表现不佳，但通过上下文学习可以改善其时空能力。在SurgSTU训练数据集上微调的视觉语言模型在所有时空任务中取得最高性能。

Conclusion: SurgSTU数据集能有效提升视觉语言模型在手术视频中的时空理解能力，验证了该数据集对改善手术视频时空多模态理解的有效性。
...
TL;DR: 本文提出了一种评估多模态大语言模型在3D运动一致性推理能力的新任务，发现现有模型在此方面表现远低于人类水平，揭示了对3D结构理解的不足。


<details>
  <summary>Details</summary>
Motivation: 空间一致性是视觉世界的基本属性，也是理解物理现实模型的关键要求。尽管多模态大语言模型（MLLMs）取得了进展，但在多视角3D几何推理方面仍存在困难。需要更系统地评估模型对3D运动一致性的理解能力。

Method: 提出了一个更具挑战性的任务：给定同一场景的两个视角，识别违反3D运动一致性的物体。开发了一种简单且可扩展的方法，从多视角场景生成真实但空间不一致的图像对，用于系统评估该能力。

Result: 最先进的多模态大语言模型在此任务上显著低于人类观察者表现，且在不同场景属性上表现出巨大变异性，显示出对3D结构的脆弱和不完整理解。

Conclusion: 研究结果强调了需要开发对物理世界有更深入基础理解的方法，多模态大语言模型在3D空间一致性推理方面仍有很大改进空间。
...
TL;DR: 该论文重新审视了人在回路目标检索任务，利用预训练的ViT表示，在主动学习框架下通过用户交互迭代检索多对象图像中的目标类别。


<details>
  <summary>Details</summary>
Motivation: 在多对象数据集中，目标对象可能只占据图像的小区域，全局描述符往往不足，需要更适应的局部描述符。现有方法在处理复杂场景中的目标检索时面临挑战，需要重新审视人在回路目标检索任务的设计。

Method: 将检索任务构建为二分类问题，通过主动学习循环迭代优化：系统选择信息丰富的样本供用户标注，利用预训练的ViT表示，探索不同的表示策略（全局上下文与局部细节），并解决关键设计问题（考虑哪些目标实例、标注形式、主动选择策略等）。

Result: 比较了多对象数据集上的多种表示策略，揭示了捕获全局上下文与聚焦细粒度局部目标细节之间的权衡，为基于主动学习的交互式检索管道设计提供了实用见解。

Conclusion: 通过重新审视人在回路目标检索任务，利用预训练ViT表示并解决关键设计问题，为复杂多对象场景中的目标类别检索提供了有效的交互式检索框架。
...
TL;DR: 该论文提出了CLeaRS基准，用于评估遥感视觉语言模型的持续学习能力，发现现有模型存在灾难性遗忘问题，需要开发专门的持续学习方法。
...
Method: 构建了CLeaRS基准，包含10个精心挑选的子集、超过207k图像-文本对，涵盖多样化解释任务、传感模态和应用场景，并定义了三种评估协议。

Result: 广泛基准测试显示所有设置下都存在灾难性遗忘，现有持续学习方法在适应遥感视觉语言模型时效果有限，无法有效处理任务、指令和模态转换。

Conclusion: 遥感视觉语言模型需要专门设计的持续学习方法，CLeaRS基准为这一研究方向提供了重要的评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 预训练语言模型适应为视觉语言模型时，由于表示偏移和跨模态干扰，会损害其原有的语言能力。现有恢复方法通常需要引入额外的对齐模块，增加了架构复杂性和推理参数。

Method: 提出LinguDistill方法：1）使用原始冻结语言模型作为教师；2）通过层级KV缓存共享使教师能够接触学生的多模态表示；3）在语言密集型数据上选择性蒸馏教师的语言信号，同时保持学生的视觉基础能力。

Result: 在语言和知识基准上恢复了约10%的性能损失，同时在视觉密集型任务上保持可比性能。无需额外模块，提供了高效实用的解决方案。

Conclusion: 语言能力可以在不增加额外模块的情况下恢复，为多模态模型中特定模态性能退化问题提供了高效实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 主题驱动文本到图像生成面临"相似性-可控性悖论"：增强文本控制会降低主题保真度，反之亦然。作者认为这源于文本提示的模糊性，它同时描述主题和修改内容，导致模型接收冲突信号。

Method: 提出DisCo框架：1) 文本-视觉解耦模块：从参考图像和实体词提取主题身份，简化文本提示仅包含修改指令；2) 设计专用奖励信号，使用强化学习无缝重新耦合视觉定义的主题和文本生成的上下文。
...
Method: 提出MotionGrounder框架，包含流式运动信号（FMS）提供稳定运动先验，物体-字幕对齐损失（OCAL）将物体字幕与对应空间区域对齐

Result: MotionGrounder在定量、定性和人工评估中均优于现有基线方法，并提出了新的物体定位评分（OGS）联合评估空间对齐和语义一致性

Conclusion: MotionGrounder首次实现了基于DiT的多物体可控运动迁移，为现实场景中的精细视频生成控制提供了有效解决方案
...
TL;DR: 提出Sparkle表示法，通过显式分解运动学结构和表面几何，平衡表达性和鲁棒性，用于点云运动捕捉


<details>
  <summary>Details</summary>
Motivation: 点云运动捕捉面临表达性与鲁棒性的权衡：点基方法几何细节丰富但噪声敏感，骨架基方法鲁棒但过于简化。需要构建既能保持几何细节又能抵抗噪声的有效表示。

Method: 提出Sparkle表示法，统一骨骼关节和表面锚点，通过显式运动学-几何分解构建结构化表示。SparkleMotion框架通过分层模块学习该表示，嵌入几何连续性和运动学约束，解耦内部运动学结构和外部表面几何。

Result: 在准确性、鲁棒性和泛化能力方面达到最先进水平，特别是在严重域偏移、噪声和遮挡条件下表现优异。在多种传感器类型和挑战性真实场景中验证了方法的优越性。

Conclusion: 通过显式分解运动学结构和表面几何的Sparkle表示法，成功解决了点云运动捕捉中表达性与鲁棒性的权衡问题，为噪声、遮挡和域偏移条件下的鲁棒运动捕捉提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统显式3D表示（如点云和网格）需要大量存储空间来捕捉精细几何细节，并且需要复杂的索引系统进行表面查找，而功能表示提供了一种高效、紧凑且连续的替代方案

Method: 提出基于高斯过程混合模型的对象特定功能形状表示方法，通过在战略参考点锚定局部GP先验来捕捉复杂拓扑结构，这些参考点可以通过任何结构分解方法（如骨架化、基于距离的聚类）灵活提取

Result: 在ShapeNetCore和IndustryShapes数据集上的广泛评估表明，该方法能够高效且准确地表示复杂几何形状

Conclusion: 该方法提供了一种轻量级、高效的功能形状表示方案，相比计算量大的神经架构，利用高斯过程从稀疏点云学习连续方向距离场，能够有效捕捉复杂拓扑结构
...
Method: 利用点跟踪、深度和分割模型构建具有时空一致性的工具和组织语义的连贯4D模型，然后让多模态大语言模型基于显式4D表示（如轨迹）作为智能体进行推理，无需微调。
...
### [63] [PixelPrune: Pixel-Level Adaptive Visual Token Reduction via Predictive Coding](https://arxiv.org/abs/2604.00886)
*Nan Wang,Zhiwei Jin,Chen Chen,Haonan Lu*

Main category: cs.CV

TL;DR: PixelPrune是一种基于预测编码的压缩方法，通过剪枝视觉Transformer编码前的冗余图像块来加速文档和GUI理解任务，无需训练且支持无损和有损压缩。
...
Method: 提出PixelPrune方法，利用像素级冗余，通过基于预测编码的压缩在视觉Transformer编码器之前剪枝冗余图像块。该方法在像素空间操作，无需训练参数，支持像素无损压缩（τ=0）和可控有损压缩（τ>0）。

Result: 在三个模型规模和文档/GUI基准测试中，PixelPrune在保持竞争力的任务准确性的同时，实现了高达4.2倍的推理加速和1.9倍的训练加速。

Conclusion: PixelPrune通过有效利用图像中的像素级冗余，为文档和GUI理解任务提供了高效的计算加速方案，无需额外训练且覆盖完整推理流程。
...
TL;DR: 提出了一种新的对抗性衰减补丁方法，通过能量约束优化和衰减部署框架，在SAR目标检测攻击中实现攻击效果与隐蔽性的平衡，并具备物理实现潜力。


<details>
  <summary>Details</summary>
Motivation: 现有的SAR攻击方法虽然能有效欺骗检测器，但通常引入明显扰动，且主要局限于数字域，忽视了攻击SAR系统的物理实现约束。需要开发更隐蔽且物理可实现的攻击策略。

Method: 提出了对抗性衰减补丁方法，采用能量约束优化策略结合衰减部署框架，通过信号级电子干扰机制实现物理可实现的攻击。

Result: AAP能有效降低检测性能，同时保持高隐蔽性，在不同模型间表现出良好的可迁移性，为SAR目标检测系统的对抗攻击提供了物理基础视角。

Conclusion: 该方法为SAR目标检测系统的对抗攻击提供了物理基础视角，有助于设计更隐蔽和实际可部署的攻击策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有的防御方法主要针对未经授权的个性化模型训练进行保护，但无法解决授权个性化后生成的公开图像仍然泄露身份信息的问题。当用户在社交媒体分享个性化生成的图像时，这些图像可能通过人脸识别系统被链接到真实用户，导致身份追踪和画像分析。

Method: 提出IDDM（Identity-Decoupled personalized Diffusion Models）方法，采用模型端输出免疫策略。该方法将身份解耦集成到个性化流程中，通过交替执行短期个性化更新和身份解耦数据优化，使用两阶段调度来平衡身份可链接性抑制和生成效用。

Result: 在多个数据集、多样化提示词和最先进的人脸识别系统上的广泛实验表明，IDDM能够持续降低身份可链接性，同时保持高质量的个性化生成效果，实现了可调的隐私-效用权衡控制。

Conclusion: IDDM为解决授权个性化后生成图像的隐私泄露问题提供了有效解决方案，通过模型端输出免疫策略，在支持授权个性化的同时减少公开生成图像的身份可链接性，满足多样化的隐私需求。
...
TL;DR: JAMMEval是一个经过系统精炼的日语视觉问答基准数据集，通过两轮人工标注改进了7个现有日语基准，旨在提高视觉语言模型评估的可靠性。


<details>
  <summary>Details</summary>
Motivation: 日语VQA基准相比英语基准缺乏迭代精炼，现有基准存在模糊问题、错误答案以及无需视觉基础即可解决的实例等问题，这损害了评估可靠性并导致模型比较中的误导性结论。

Method: 通过两轮人工标注系统精炼7个现有日语基准数据集，构建JAMMEval数据集，提高数据质量和评估可靠性。

Result: JAMMEval评估显示，精炼后的基准能更好地反映模型能力，表现出更低的运行间方差，并提高了区分不同能力水平模型的能力。

Conclusion: JAMMEval为日语视觉语言模型提供了更可靠的评估基准，通过系统精炼现有数据集解决了评估可靠性问题，推动了日语VLM评估的发展。
...
TL;DR: ProCap框架解决空间增强现实中视觉语言模型难以区分物理场景与投影内容的虚拟-物理模糊问题，通过两阶段解耦方法提升语义理解能力


<details>
  <summary>Details</summary>
Motivation: 空间增强现实(SAR)通过投影仪将数字内容直接投影到物理场景上，但现有视觉语言模型难以区分物理场景和投影内容，导致语义理解模糊，限制了SAR支持智能交互的能力

Method: 提出ProCap框架：1) 通过自动分割视觉上分离虚拟和物理层；2) 使用区域感知检索避免投影变形导致的语义模糊；3) 创建RGBP数据集(首个大规模SAR语义基准数据集)；4) 建立双描述评估协议

Result: 实验表明ProCap为未来SAR研究提供了稳健的语义基础，创建了包含65个多样化物理场景和超过18万投影的RGBP数据集，并开源了代码、预训练模型和数据集

Conclusion: ProCap通过明确解耦投影内容和物理场景，有效解决了空间增强现实中的虚拟-物理模糊问题，为SAR的智能交互应用奠定了语义理解基础
...
TL;DR: IKEA-Bench基准测试评估了19个视觉语言模型在理解IKEA家具组装说明方面的表现，发现视觉编码是提升跨描述鲁棒性的关键瓶颈。


<details>
  <summary>Details</summary>
Motivation: 2D组装图通常抽象且难以理解，需要智能助手来监控进度、检测错误并提供逐步指导。在混合现实环境中，系统必须从摄像头画面识别已完成和进行中的步骤，并与图示说明对齐。

Method: 构建IKEA-Bench基准，包含29个IKEA家具产品的1,623个问题，涵盖6种任务类型。评估19个VLMs（2B-38B参数）在三种对齐策略下的表现，并进行三级机制分析。

Result: 主要发现：1) 组装指令理解可通过文本恢复，但文本同时会降低图示到视频的对齐；2) 架构家族比参数数量更能预测对齐准确性；3) 视频理解仍是硬瓶颈，不受策略影响。机制分析显示图示和视频占据不相交的ViT子空间，添加文本会使模型从视觉驱动转向文本驱动推理。

Conclusion: 视觉编码是提升跨描述鲁棒性的主要目标，需要改进视觉表示来弥合组装图与视频帧之间的描述差距。
...
Conclusion: 该方法提供了一种简单有效的训练免费技术，通过跨模型一致性指导表示蒸馏和精炼，优于基线方法和PCA投影表示。
...
<details>
  <summary>Details</summary>
Motivation: 现有自我中心模拟器要么缺乏明确的3D基础，导致视角变化下的结构漂移；要么将场景视为静态，无法在多阶段交互中更新世界状态。需要解决这两个限制来创建更真实的交互模拟。

Method: 通过将3D场景建模为可更新的世界状态，使用几何动作感知的观察模拟模型生成具身交互，并通过交互感知状态更新模块确保空间一致性。设计了可扩展的流水线从大规模单目自我中心视频中提取静态点云、相机轨迹和具身动作，并开发了低成本数据采集系统EgoCap。

Result: EgoSim在视觉质量、空间一致性以及复杂场景和野外灵巧交互的泛化能力方面显著优于现有方法，同时支持跨具身转移到机器人操作。

Conclusion: EgoSim通过建模可更新的3D世界状态，解决了现有自我中心模拟器的关键限制，为交互视频生成提供了空间一致且动态更新的解决方案，具有实际应用价值。
...
TL;DR: YieldSAT是一个大规模、高质量、多模态的作物产量预测数据集，覆盖多国多种作物，包含1220万产量样本和11.3万张卫星图像，用于高分辨率像素回归任务。


<details>
  <summary>Details</summary>
Motivation: 现有作物产量预测数据集存在稀缺、质量低、区域限制或作物类型单一等问题，这主要是由于数据获取成本高、数据质量参差不齐以及数据隐私法规限制，阻碍了可扩展数据驱动解决方案的发展。

Method: 发布YieldSAT数据集，包含阿根廷、巴西、乌拉圭和德国多个气候区的2173个专家标注农田，涵盖玉米、油菜、大豆和小麦等主要作物类型。每个样本提供10米空间分辨率的多光谱卫星图像和辅助环境数据。将大规模高分辨率作物产量预测建模为像素回归任务，比较不同深度学习模型和数据融合架构，并提出领域知识引导的深度集成方法来缓解真实条件下的严重分布偏移问题。

Result: 数据集包含超过1220万产量样本和113,555张标注卫星图像，覆盖多种作物和气候区。通过比较不同模型和架构，展示了大规模高分辨率作物产量预测的潜力，并发现领域知识引导的深度集成方法能显著提升性能，应对真实条件下的分布偏移挑战。

Conclusion: YieldSAT为作物产量预测研究提供了大规模、高质量、多模态的数据资源，推动了可扩展数据驱动解决方案的发展。研究揭示了真实条件下分布偏移的挑战，并展示了领域知识引导的深度集成方法的有效性，为未来研究提供了重要基础。
...
TL;DR: 本文提出GIFD方法，通过在GAN中间层特征域进行梯度反演，相比现有方法在潜在空间优化，能实现像素级重建并提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中的梯度交换存在隐私泄露风险，现有攻击方法仅在GAN潜在空间进行梯度反演，表达能力有限且泛化能力不足。

Method: 提出GIFD方法：1) 分解GAN模型，在中间层特征域进行分层搜索；2) 从初始潜在空间逐步优化到接近输出图像的中间层；3) 设计正则化器避免生成不真实图像；4) 扩展到分布外场景，提出标签映射技术处理标签不一致问题。

Result: 大量实验表明，GIFD方法能实现像素级重建，在各种联邦学习场景中优于现有基线方法。

Conclusion: GIFD通过利用GAN中间层特征域进行梯度反演，显著提升了隐私攻击的效果和泛化能力，揭示了联邦学习中更严重的隐私风险。
...
TL;DR: DLWM提出了一种用于自动驾驶的双潜在世界模型范式，通过两阶段训练实现基于高斯表示的全面预训练，在3D占用感知、4D占用预测和运动规划任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 基于视觉的自动驾驶因其低成本和高性能受到关注。与密集BEV或稀疏查询模型相比，高斯中心方法通过3D语义高斯描述场景，是一种全面且稀疏的表示。需要一种专门为自动驾驶设计的高斯中心预训练范式。
...
Conclusion: DLWM为自动驾驶提供了一种新颖的双潜在世界模型范式，能够实现全面的高斯中心预训练，在多个关键任务上取得优异性能，展示了其在自动驾驶领域的应用潜力。
...
TL;DR: ACT是一种无需训练的动态推理干预方法，通过自适应整合上下文信息来减少大视觉语言模型中的幻觉问题，在保持基本生成能力的同时显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有缓解LVLM幻觉的方法主要依赖静态的单步状态来增强视觉关注或抑制语言先验，但忽视了生成过程中的动态上下文变化，难以纠正信息损失。
...
TL;DR: 该研究提出了一种仅使用T2加权MRI进行前列腺癌定位的新方法，通过将DWI作为训练时的潜在模态，在推理时仅需T2图像即可实现与多模态方法相当甚至更好的癌症定位性能。


<details>
  <summary>Details</summary>
Motivation: 虽然多参数MRI（包括DWI和T2w）被推荐用于前列腺癌检测，但仅使用T2w图像具有临床优势：降低成本、减少对专业序列采集的依赖。研究旨在探索仅使用T2w图像进行基于组织病理学标签的癌症定位这一更具挑战性的应用。

Method: 提出期望最大化算法框架：将DWI作为训练时的潜在模态，E步中使用基于流匹配的生成模型近似潜在DWI图像的后验分布，M步中同时优化癌症定位器和生成模型以最大化癌症存在的期望似然。这是一种从特权DWI模态学习的新理论框架。

Result: 提出的仅T2方法在癌症定位性能上优于缺乏训练DWI图像的方法以及现有的特权学习和不完整模态框架。在患者层面F1分数比T2w+DWI基线提高14.4%，区域层面QWK提高5.3%，与使用多个输入序列的基线方法相比具有竞争力或更好表现。

Conclusion: 该研究为从特权模态学习提供了新颖的理论框架，仅使用T2w图像即可实现优越的前列腺癌定位性能，具有显著的临床实用价值，能够降低成本和专业知识要求，同时保持甚至超越多模态方法的诊断准确性。
...
TL;DR: 提出了一种无需训练的地滚波衰减框架，将地滚波衰减重新定义为语义引导的信号分离问题，使用可提示的大型视觉模型提取语义先验，通过软掩码嵌入低秩逆公式实现自适应抑制和反射保护重建。


<details>
  <summary>Details</summary>
Motivation: 地滚波是陆地和垂直地震剖面数据中的主要相干噪声，严重掩盖反射事件并降低后续成像和解释质量。传统衰减方法存在适应性有限、信号泄漏或依赖标记训练数据等问题，特别是在强信号噪声重叠情况下表现不佳。

Method: 1) 将地震道集转换为视觉表示，使用可提示的大型视觉模型通过文本或图像提示提取高级语义先验，定位地滚波主导区域；2) 将语义响应转换为连续软掩码；3) 将软掩码嵌入掩码条件低秩逆公式，实现空间自适应抑制和反射保护重建；4) 开发基于交替方向乘子法的高效求解器。

Result: 在合成和实际VSP数据集上的广泛实验表明，该方法实现了优异的地滚波衰减效果，同时保持了反射连续性和波形保真度，在性能上一致优于代表性的变换域滤波和隐式神经表示方法。

Conclusion: 提出的无需训练框架通过语义引导的信号分离方法有效解决了地滚波衰减问题，无需任务特定训练或手动标注，实现了稳定且物理一致的信号恢复，为地震数据处理提供了新的解决方案。
...
TL;DR: PDA是一个无需训练、基于文本增强的防御框架，通过提示改写、问题分解和一致性聚合来提升视觉语言模型对抗图像扰动的鲁棒性，保持推理效率的同时实现通用防御。


<details>
  <summary>Details</summary>
Motivation: 现有基于对抗训练的防御方法计算成本高，且难以泛化到未见过的攻击类型，需要一种更通用、高效的防御方案来保护视觉语言模型免受对抗图像扰动的影响。

Method: 提出Paraphrase-Decomposition-Aggregation（PDA）框架，包含三个步骤：1）提示改写生成多个语义等价的变体；2）将复杂问题分解为子问题；3）聚合多个变体的输出以获得一致性结果。框架完全在推理时运行，无需修改底层模型。

Result: 在多个视觉语言模型架构和视觉问答、分类、描述任务上的实验表明，PDA能一致地提升对各种对抗扰动的鲁棒性，同时保持有竞争力的干净准确率，实现了通用、强大且实用的防御。

Conclusion: PDA为视觉语言模型提供了一个无需训练、推理时有效的通用防御框架，通过文本增强策略平衡了鲁棒性和效率，为对抗防御提供了新的实用解决方案。
...
Result: 轨迹标记预测实现了类别无关、数据高效的预测，优于现有基线方法，并能泛化到稀有物种和形态

Conclusion: 为野外预测性视觉智能提供了基础，展示了结构化中间表示在行为预测中的有效性
...
TL;DR: Diff3R是一个新颖的3D高斯泼溅框架，通过将可微分的3DGS优化层集成到训练循环中，将前馈预测与测试时优化相结合，学习预测最优的初始化参数而非传统零次结果。


<details>
  <summary>Details</summary>
Motivation: 当前3DGS存在两个主要方向：前馈模型在稀疏视图下推理快但质量有限，而逐场景优化质量高但计算成本大。需要结合两者的优势，实现既快速又高质量的3D重建。

Method: 1. 引入可微分3DGS优化层到训练循环中；2. 使用隐函数定理和矩阵自由PCG求解器计算梯度，避免优化步骤反向传播的高计算成本；3. 集成数据驱动的不确定性模型，自适应控制优化过程中参数的改变程度。

Result: 该方法能有效缓解欠约束区域的过拟合问题，提高对输入异常值的鲁棒性。由于优化层是模型无关的，可以无缝集成到现有的前馈3DGS架构中，为测试时优化提供改进。

Conclusion: Diff3R成功桥接了前馈预测和测试时优化，通过预测最优初始化而非传统零次结果，结合了快速推理和高质量渲染的优势，为3D高斯泼溅技术提供了新的优化框架。
...
TL;DR: 首次利用开源流程从Chandrayaan-2 OHRC多视角影像生成亚米级月球DEM，通过几何分析筛选立体像对，实现24-54厘米空间分辨率，垂直精度5.85米，水平精度小于30厘米。


<details>
  <summary>Details</summary>
Motivation: 高分辨率月球数字高程模型对表面移动规划、着陆点特征分析和行星科学至关重要。Chandrayaan-2的OHRC相机具有当前月球轨道成像中最佳的地面采样能力（20-30厘米/像素），但尚未充分利用其生成亚米级DEM的潜力。

Method: 1. 通过几何分析（基线高度比计算和收敛角估计）从非配对的OHRC档案中识别候选立体像对；2. 应用密集立体匹配和光线三角测量生成点云；3. 将点云网格化为DEM；4. 通过ICP配准LRO NAC DTM建立绝对高程一致性，并进行恒定偏差校正。

Result: 在五个地理分布的月球站点生成了有效空间分辨率约24-54厘米的DEM。验证显示：垂直RMSE为5.85米（原生OHRC分辨率），通过平面特征匹配评估的水平精度小于30厘米。

Conclusion: 成功开发了首个完全开源的OHRC亚米级DEM生成流程，证明了利用现有非配对OHRC影像生成高质量月球地形数据的可行性，为月球探测和科学研究提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频基础模型在人类中心视频合成方面取得进展，但对主体和场景的细粒度独立编辑仍面临挑战。现有方法通过刚性3D几何组合实现环境控制时，往往在精确控制和生成灵活性之间存在明显权衡，且繁重的3D预处理限制了实际可扩展性

Method: 1）引入规范空间注入机制，通过交叉注意力解耦人类动态和环境线索；2）提出Dynamic-Grounded-RoPE位置嵌入策略，在不同空间域间建立空间对应关系而无需启发式3D对齐；3）采用混合上下文集成机制支持分钟级长时程合成，保持主体和场景一致性

Result: 实验表明该方法显著优于最先进方法，在视频合成中提供更优越的结构控制和创意多样性

Conclusion: ONE-SHOT框架通过解耦生成过程为组合式人-环境视频生成提供了参数高效的解决方案，克服了现有方法在控制精度和生成灵活性之间的权衡，同时避免了繁重的3D预处理
...
TL;DR: CROWD数据集是一个从YouTube公开视频中筛选和分割的手动标注城市行车记录仪数据集，专注于常规驾驶场景，排除事故和编辑内容，包含5万多个片段，覆盖全球238个国家和地区，提供时间、车辆类型标注及机器生成的物体检测数据。


<details>
  <summary>Details</summary>
Motivation: 现有的行车记录仪数据集往往关注事故或特殊事件，缺乏常规驾驶场景的数据。为了支持跨域鲁棒性和交互分析研究，需要构建一个专注于日常、连续、未编辑的普通驾驶场景的数据集。

Method: 从YouTube公开视频中手动筛选和分割行车记录仪片段，排除事故、事故后果和编辑内容。数据集包含视频标识符和片段边界，提供手动标注的时间（白天/夜晚）和车辆类型，以及使用YOLOv11x和BoT-SORT生成的机器检测和跟踪数据。

Result: 创建了包含51,753个片段记录的数据集，总时长20,275.56小时（来自42,032个视频），覆盖全球238个国家和地区7,103个居住地。提供手动标注的时间、车辆类型信息，以及所有80个MS-COCO类别的机器生成检测和多目标跟踪数据。

Conclusion: CROWD数据集为计算机视觉和自动驾驶研究提供了大规模、多样化的常规驾驶场景数据，通过视频标识符而非原始视频的分布式方式支持可重复研究，降低了基准测试的门槛。
...
TL;DR: PHASOR是一个用于CT虚拟对比增强的体扩散框架，通过视频扩散模型和两个互补模块（AR-MoE和IP-REPA）解决现有方法在解剖异质性和空间对齐方面的挑战，显著提升合成质量和增强准确性。
...
Method: PHASOR采用体扩散框架，将CT体积视为连贯序列，利用视频扩散模型提升结构一致性和体准确性。引入两个关键模块：1）解剖路由专家混合（AR-MoE）将不同增强模式锚定到解剖语义，使用器官特定记忆捕获显著细节；2）强度相位感知表示对齐（IP-REPA）突出复杂对比信号，同时减轻不完美空间对齐的影响。

Result: 在三个数据集上的广泛实验表明，PHASOR在合成质量和增强准确性方面显著优于现有最先进方法。

Conclusion: PHASOR通过创新的体扩散框架和专门设计的模块，有效解决了CT虚拟对比增强中的关键挑战，为临床提供了一种高质量、准确的替代方案。
...
TL;DR: ReMoGen：实时交互到反应生成框架，利用通用运动先验和模块化设计解决交互数据碎片化和实时响应挑战
...
Method: 提出ReMoGen（反应运动生成）框架，采用模块化学习架构：1) 从大规模单人运动数据集中学习通用运动先验；2) 通过独立训练的元交互模块将先验适应到目标交互领域；3) 采用段级生成和轻量级帧级段细化模块，在帧级别整合新观察到的线索，实现响应式在线交互。

Result: 在人-人、人-场景和混合模态交互设置下的广泛实验表明，ReMoGen能够生成高质量、连贯且响应迅速的反应，同时在多样化的交互场景中有效泛化。

Conclusion: ReMoGen通过结合通用运动先验和模块化适应策略，成功解决了交互数据碎片化和实时响应生成的挑战，为实时人机交互应用提供了有效的解决方案。
...
TL;DR: 提出ProTPS方法，通过原型引导的文本提示选择来增强训练灵活性，解决持续学习中文本提示学习重叠导致的灾难性遗忘问题，并在三个设置中验证性能，同时发布了真实海洋物种数据集Marine112。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本提示的持续学习方法面临一个共同挑战：如何学习独特的文本提示，这些提示隐式携带新类的语义信息，避免新类语义特征与已训练类重叠，从而缓解灾难性遗忘问题。

Method: 提出原型引导的文本提示选择（ProTPS）方法，学习类特定的视觉原型和文本提示。视觉原型指导每个类的文本提示的选择和学习，通过增加训练灵活性来鼓励学习独特的文本提示。

Result: 在类增量（CI）和跨数据集持续（CDC）学习设置中，ProTPS达到接近上限的性能。进一步在真实海洋物种数据集Marine112上进行评估，该数据集包含112个物种，采集时间跨度6年，适用于类和域增量（CDI）学习设置且呈自然长尾分布。三个设置的结果显示ProTPS优于最近的最先进方法。

Conclusion: ProTPS通过原型引导的文本提示选择有效解决了持续学习中文本提示学习的挑战，在多个设置中表现出色，同时发布的Marine112数据集为社区带来了新的挑战。论文代码和数据集将在接受后发布。
...
TL;DR: ReinDriveGen是一个可控的动态驾驶场景生成框架，支持用户自由编辑演员轨迹以模拟安全关键场景，并通过强化学习后训练策略提升分布外场景的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以生成可控的动态驾驶场景，特别是安全关键场景（如碰撞、失控车辆等），这些场景通常超出训练数据分布，需要专门的处理机制。

Method: 1) 从多帧LiDAR数据构建动态3D点云场景；2) 引入车辆补全模块从部分观测重建完整360°几何；3) 将编辑后的场景渲染为2D条件图像指导视频扩散模型；4) 提出基于强化学习的后训练策略，使用成对偏好模型和奖励机制提升分布外场景质量。

Result: 实验表明ReinDriveGen在编辑驾驶场景上优于现有方法，并在新颖的自我视角合成任务上达到最先进水平。

Conclusion: ReinDriveGen实现了对动态驾驶场景的完全可控编辑，能够生成高质量的安全关键场景视频，为解决自动驾驶测试中的边缘案例提供了有效工具。
...
TL;DR: 该论文研究开放集监督3D异常检测，提出Open3D-AD方法，利用正常样本、模拟异常和部分真实异常数据建模正常与异常的概率密度分布，并在新工业数据集Open-Industry上建立基准。


<details>
  <summary>Details</summary>
Motivation: 在真实制造场景中，虽然获取高精度点云计算成本高，但通常可以收集到少量异常样本。因此研究开放集监督3D异常检测，使用正常样本和少量已知异常样本训练模型，旨在测试时识别未知异常。

Method: 1) 将通用开放集异常检测方法适配到3D点云输入；2) 提出Open3D-AD方法，利用正常样本、模拟异常和部分真实异常建模正常与异常数据的概率密度分布；3) 引入简单的对应分布子采样技术，减少正常与非正常分布的重叠，实现更强的双分布建模。

Result: 在Open-Industry数据集（包含15个类别，每类5种真实异常类型）以及Real3D-AD和Anomaly-ShapeNet等现有数据集上建立了全面基准。基准测试和消融研究证明了Open3D-AD的有效性，并进一步揭示了开放集监督3D异常检测的潜力。

Conclusion: 论文展示了开放集监督3D异常检测的可行性，提出的Open3D-AD方法在工业数据集上表现有效，为3D异常检测领域提供了新的研究方向和实践基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有编辑方法缺乏对中心主体结构完整性的保护能力，无法实现细粒度的局部操作。TRACE旨在通过3D几何锚定解决这一问题，实现高保真场景编辑。

Method: 采用三阶段方法：1）多视角3D锚点合成，利用MV-TRACE数据集生成空间一致的3D锚点；2）有形几何锚定，通过两阶段注册确保网格与3DGS场景的精确空间同步；3）上下文视频掩码，将3D投影整合到自回归视频管道中实现时间稳定的物理基础渲染。

Result: TRACE在编辑多样性和结构完整性方面持续优于现有方法，能够实现局部姿态调整、组件替换等精细操作，同时保持主体结构完整。

Conclusion: TRACE通过3D几何锚定实现了自动化、高保真的3DGS场景编辑，解决了现有方法在局部操作和结构保持方面的不足，为3D场景编辑提供了新的解决方案。
...
### [89] [How Emotion Shapes the Behavior of LLMs and Agents: A Mechanistic Study](https://arxiv.org/abs/2604.00005)
*Moran Sun,Tianlin Li,Yuwei Zheng,Zhenhong Zhou,Aishan Liu,Xianglong Liu,Yang Liu*

Main category: cs.AI

TL;DR: E-STEER是一个可解释的情感引导框架，通过将情感作为结构化可控变量嵌入LLM隐藏状态，研究情感对AI任务处理的影响，发现情感与行为存在非线性关系，特定情感能提升LLM能力和安全性。


<details>
  <summary>Details</summary>
Motivation: 现有情感感知研究主要将情感视为表层风格因素或感知目标，忽视了其在任务处理中的机制性作用。受情感在人类认知和表现中重要作用的启发，研究者希望探索类似的情感信号是否能塑造大语言模型和智能体的行为。

Method: 提出E-STEER框架，这是一个可解释的情感引导框架，能够在LLM和智能体的表示层面进行直接干预。该方法将情感作为结构化、可控的变量嵌入隐藏状态中。

Result: 研究结果揭示了情感与行为之间的非线性关系，这与已建立的心理学理论一致。具体情感不仅能增强LLM的能力，还能提高安全性，并系统地塑造多步智能体行为。

Conclusion: 情感在LLM和智能体任务处理中具有机制性作用，通过E-STEER框架可以有效地引导和控制AI系统的行为表现，为情感在AI中的应用提供了新的研究方向。
...
### [90] [A Safety-Aware Role-Orchestrated Multi-Agent LLM Framework for Behavioral Health Communication Simulation](https://arxiv.org/abs/2604.00249)
*Ha Na Cho*
...
<details>
  <summary>Details</summary>
Motivation: 单智能体大型语言模型系统难以同时支持多样化的对话功能并确保行为健康交流的安全性，需要一种既能提供专业支持又能保证安全性的解决方案。

Method: 采用安全感知、角色编排的多智能体LLM框架，将对话职责分解到专门的智能体（包括共情导向、行动导向和监督角色），通过基于提示的控制器动态激活相关智能体并执行持续的安全审计。
...
### [91] [Human-in-the-Loop Control of Objective Drift in LLM-Assisted Computer Science Education](https://arxiv.org/abs/2604.00281)
*Mark Dranias,Adam Whitley*

Main category: cs.AI

TL;DR: 该论文提出了一种基于系统工程和控制理论的人类在环（HITL）教学方法，通过分离规划与执行、训练学生设定验收标准和架构约束，来稳定AI辅助编程工作，避免目标漂移。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）越来越多地嵌入计算机科学教育中，但AI辅助编程工具经常出现目标漂移问题——局部合理的输出偏离任务规范。现有的教学响应通常强调工具特定的提示实践，随着AI平台的发展，这种方法缺乏持久性。

Method: 采用以人为中心的方法，将人类在环控制视为稳定的教育问题而非向AI自主的过渡步骤。基于系统工程和控制理论概念，将目标和世界模型作为操作工件，让学生配置以稳定AI辅助工作。提出一个试点本科CS实验室课程，明确分离规划与执行，训练学生在代码生成前指定验收标准和架构约束。在某些实验中，课程还引入有意的、概念对齐的漂移，以支持诊断和从规范违规中恢复。

Result: 报告了一个三臂试点设计的敏感性功效分析，比较非结构化AI使用、结构化规划以及带有注入漂移的结构化规划，在现实的分组约束下建立了可检测的效应大小。

Conclusion: 该贡献是一个理论驱动、方法明确的人类在环教学法基础，使控制能力能够在不断发展的AI工具中可教，为AI辅助编程教育提供了更持久的教学框架。
...
### [92] [Improvisational Games as a Benchmark for Social Intelligence of AI Agents: The Case of Connections](https://arxiv.org/abs/2604.00284)
...
<details>
  <summary>Details</summary>
Motivation: 探索AI智能体在社交智能方面的推理能力，特别是超越自身记忆和演绎推理，涉及评估其他智能体理解能力的社交互动场景。

Method: 引入即兴文字游戏Connections作为评估框架，要求AI智能体在受限环境中与其他智能体沟通，展示社交意识和协作能力。

Result: Connections游戏被证明是评估语言模型智能体社交智能能力的良好基准，能够测试智能体在知识检索、总结和认知状态感知方面的综合能力。

Conclusion: Connections游戏为评估AI智能体社交推理能力提供了有效框架，强调了在协作游戏中社交意识和智能的重要性，为AI社交智能研究提供了新方向。
...
### [93] [Signals: Trajectory Sampling and Triage for Agentic Interactions](https://arxiv.org/abs/2604.00356)
...
### [94] [Decision-Centric Design for LLM Systems](https://arxiv.org/abs/2604.00414)
*Wei Sun*

Main category: cs.AI

TL;DR: 论文提出了一种决策中心框架，将LLM系统中的控制决策从生成过程中分离出来，使控制成为系统的一个显式和可检查的层次。
...
Method: 提出决策中心框架，将决策相关信号与将这些信号映射到行动的策略分离开来。该框架支持将故障归因于信号估计、决策策略或执行，并支持每个组件的模块化改进。
...
### [95] [Self-Routing: Parameter-Free Expert Routing from Hidden States](https://arxiv.org/abs/2604.00421)
*Jama Hussein Mohamud,Drew Wagner,Mirco Ravanelli*

Main category: cs.AI

TL;DR: 本文提出了一种无需专门学习路由器的MoE层路由机制Self-Routing，它直接使用token隐藏状态的指定子空间作为专家logits，在保持性能的同时消除了所有路由参数。


<details>
  <summary>Details</summary>
Motivation: 研究在MoE设置中专门学习路由器是否严格必要，探索是否可以从隐藏表示本身直接实现有效的路由机制，从而简化MoE架构并减少参数。

Method: 提出Self-Routing参数免费路由机制：使用token隐藏状态的指定子空间直接作为专家logits，完全消除路由器投影，同时保持MoE层的其余部分不变。

Result: 在GPT-2规模语言建模和ImageNet-1K分类任务中，Self-Routing与标准学习路由器基线保持竞争力，同时移除了所有专用路由参数，产生更平衡的专家利用率（路由熵提高约17%），且无需显式负载平衡损失。在ImageNet-1K上使用DeiT-S/16时，Self-Routing甚至略微优于相应的学习路由器MoE。

Conclusion: 有效的MoE路由可以从隐藏表示本身涌现，无需单独的专用学习路由器模块，这简化了MoE架构并减少了参数开销。
...
### [96] [Logarithmic Scores, Power-Law Discoveries: Disentangling Measurement from Coverage in Agent-Based Evaluation](https://arxiv.org/abs/2604.00477)
*HyunJoon Jung,William Na*

Main category: cs.AI

TL;DR: LLM智能体评估对话AI时，评估质量随评审团规模对数增长，问题发现随规模呈幂律增长，两者均存在收益递减，但评分饱和速度是问题发现的两倍。


<details>
  <summary>Details</summary>
Motivation: 研究LLM智能体评估对话AI的可信度问题，探索需要多少智能体评估才能获得可靠结果，解决评估质量与评审团规模之间的关系不确定性。

Method: 通过960个会话实验，使用两个模型对在15个任务上进行测试，采用基于人格的智能体评估方法，进行图灵式验证，分析评分覆盖分离现象，并通过控制消融实验验证结构化人格调节的作用。

Result: 基于人格的智能体评估产生与人类评分者无法区分的评估结果；质量评分随评审团规模呈对数增长，独特问题发现呈次线性幂律增长；评分饱和速度是问题发现的两倍；发现空间遵循幂律分布；大五人格调节使智能体探索不同质量维度。

Conclusion: 结构化人格调节（而非简单提示）是产生这些缩放特性的必要条件；智能体评估的发现机制类似于生态学中的物种积累曲线，关键问题由小评审团发现，而边缘案例需要更大的评审团。
...
### [97] [The Silicon Mirror: Dynamic Behavioral Gating for Anti-Sycophancy in LLM Agents](https://arxiv.org/abs/2604.00478)
*Harshee Jignesh Shah*

Main category: cs.AI

TL;DR: 提出Silicon Mirror框架，通过动态检测用户说服策略和调整AI行为来减少LLM的谄媚现象，相比基线方法显著降低谄媚率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越倾向于用户验证而非事实准确性，这种现象被称为"谄媚"。需要开发能够动态检测用户说服策略并调整AI行为以保持事实完整性的框架。

Method: 提出Silicon Mirror框架，包含三个组件：1) 基于实时谄媚风险评分限制上下文访问的行为访问控制系统；2) 在多轮对话中识别说服策略的特征分类器；3) 生成器-批评者循环，审计员否决谄媚草稿并触发带有"必要摩擦"的重写。

Result: 在TruthfulQA对抗场景评估中：普通Claude谄媚率为12.0%，静态护栏为4.0%，Silicon Mirror为2.0%（相对减少83.3%）。跨模型评估显示Gemini 2.5 Flash基线谄媚率46.0%，Silicon Mirror下显著减少69.6%（p < 0.001）。

Conclusion: Silicon Mirror框架能有效减少LLM的谄媚行为，验证-修正模式是RLHF训练模型的独特失败模式。该框架为保持AI事实完整性提供了有效解决方案。
...
### [98] [Adaptive Parallel Monte Carlo Tree Search for Efficient Test-time Compute Scaling](https://arxiv.org/abs/2604.00510)
...
Method: 提出负向提前退出机制，修剪无生产力的MCTS轨迹；引入自适应增强机制，重新分配回收的计算资源以减少并发搜索间的资源竞争。这些技术集成到vLLM框架中。

Result: 这些技术显著降低了p99端到端延迟，同时提高了吞吐量，并保持了推理准确性。

Conclusion: 负向提前退出和自适应增强机制能有效解决MCTS的尾部延迟问题，在保持推理性能的同时显著改善系统效率。
...
### [99] [Does Unification Come at a Cost? Uni-SafeBench: A Safety Benchmark for Unified Multimodal Large Models](https://arxiv.org/abs/2604.00547)
*Zixiang Peng,Yongxiu Xu,Qinyi Zhang,Jiexun Shen,Yifan Zhang,Hongbo Xu,Yubin Wang,Gaopeng Gou*

Main category: cs.AI

TL;DR: 本文提出了Uni-SafeBench基准测试，用于评估统一多模态大模型的安全性能，发现模型统一化虽然提升能力但显著降低安全性，开源UMLMs安全性远低于专用模型。


<details>
  <summary>Details</summary>
Motivation: 统一多模态大模型（UMLMs）将理解和生成能力集成在单一架构中，这种统一虽然通过多模态特征的深度融合提升了模型性能，但也引入了重要但未被充分探索的安全挑战。现有安全基准主要关注孤立的理解或生成任务，无法评估UMLMs在统一框架下处理多样化任务时的整体安全性。

Method: 提出了Uni-SafeBench，一个包含六大安全类别和七种任务类型的综合基准测试。开发了Uni-Judger框架，有效解耦上下文安全性和内在安全性，确保评估的严谨性。

Result: 基于Uni-SafeBench的综合评估发现：1）统一化过程虽然增强了模型能力，但显著降低了底层大语言模型的固有安全性；2）开源UMLMs的安全性能远低于专门用于生成或理解任务的多模态大模型。

Conclusion: 统一多模态大模型存在严重的安全风险，开源所有资源以系统性暴露这些风险，促进更安全的通用人工智能发展。
...
TL;DR: CircuitProbe：一种快速预测Transformer语言模型中推理电路位置的方法，速度比暴力搜索快3-4个数量级，仅需CPU上5分钟即可完成


<details>
  <summary>Details</summary>
Motivation: 现有方法需要耗费大量计算资源（每个模型25 GPU小时）来暴力搜索推理电路位置，需要一种更高效的检测方法

Method: 通过激活统计量预测电路位置：早期层使用表示变化的导数检测稳定性电路，晚期层使用异常评分检测幅度电路

Result: 在9个模型（涵盖6种架构）上验证，CircuitProbe的预测结果与最优电路匹配或在2层以内；层复制对3B以下参数模型有益，但对7B+模型有害

Conclusion: CircuitProbe提供了一种高效检测推理电路的方法，适用于小型语言模型，仅需少量校准样本且在多语言中表现稳定
...
### [101] [Preference Guided Iterated Pareto Referent Optimisation for Accessible Route Planning](https://arxiv.org/abs/2604.00795)
*Paolo Speziali,Arno De Greef,Mehrdad Asadi,Willem Röpke,Ann Nowé,Diederik M. Roijers*

Main category: cs.AI

TL;DR: PG-IPRO算法用于满足不同无障碍需求和偏好的城市路径规划，通过用户反馈交互优化路线，相比基于信息增益的交互更直观高效。


<details>
  <summary>Details</summary>
Motivation: 为满足不同人群（特别是无障碍需求者）在城市路径规划中的个性化需求，需要一种能够根据用户偏好进行交互式优化的算法，以提供更符合个人需求的路线方案。

Method: 提出偏好引导迭代帕累托参考优化（PG-IPRO）算法，用户可以对路线给出反馈，指定哪些目标应该进一步最小化或可以放松，系统根据反馈迭代优化，避免计算完整的帕累托前沿。

Result: PG-IPRO在早期迭代中相比基于信息增益的交互方法更直观有效，由于不计算完整的帕累托前沿，计算效率更高，用户等待时间更短。

Conclusion: PG-IPRO为城市路径规划提供了一种高效的用户交互式优化方法，特别适合满足不同无障碍需求和偏好的个性化路线规划需求。
...
### [102] [Beyond Symbolic Solving: Multi Chain-of-Thought Voting for Geometric Reasoning in Large Language Models](https://arxiv.org/abs/2604.00890)
*Md. Abu Bakor Siddique,Shahrin Hossain,Sadman Ahmed Siam,Syed Rifat Raiyan,Hasan Mahmud,Md Kamrul Hasan*

Main category: cs.AI

TL;DR: MARS-GPS通过生成多个并行推理路径、使用Python代码执行进行数值验证、基于token熵排名，并通过多阶段投票和自我验证聚合答案，显著提升了几何问题解决的性能。


<details>
  <summary>Details</summary>
Motivation: 现有几何问题解决方法主要关注图表描述与文本的同步以及问题求解，但只解决了图表理解和符号操作两个需求，而逻辑推理能力发展不足，通常仅限于单一思维链。需要增强逻辑推理能力以提升几何问题解决性能。

Method: 提出MARS-GPS方法：1）生成多个并行推理路径；2）使用Python代码执行进行数值验证；3）基于token级熵作为置信度信号对推理路径进行排名；4）通过多阶段投票和自我验证管道聚合答案。

Result: 在Geometry3K数据集上，MARS-GPS使用8个并行推理路径达到88.8%的准确率，比之前的最先进方法提高了近11%。当推理路径数量从1增加到16时，准确率持续提升（在消融子集上提高6.0%）。

Conclusion: MARS-GPS通过多并行推理路径、代码验证和基于熵的排名机制，有效增强了大型语言模型在几何问题解决中的逻辑推理能力，显著提升了性能并展示了可扩展性。
...
### [103] [PsychAgent: An Experience-Driven Lifelong Learning Agent for Self-Evolving Psychological Counselor](https://arxiv.org/abs/2604.00931)
...
<details>
  <summary>Details</summary>
Motivation: 现有AI心理咨询方法主要依赖静态对话数据集进行监督微调，这与人类专家通过临床实践和经验积累持续提升能力的方式形成对比。需要弥合这一差距，让AI心理咨询师能够像人类专家一样通过实践经验进行持续学习和进化。

Method: 提出三引擎框架：1）记忆增强规划引擎，用于纵向多轮会话交互，通过持久记忆和策略规划确保治疗连续性；2）技能进化引擎，从历史咨询轨迹中提取基于实践的新技能；3）强化内化引擎，通过拒绝微调将进化技能整合到模型中，提升多样化场景下的性能。

Result: 比较分析显示，该方法在所有报告评估维度上都优于强大的通用大模型（如GPT-5.4、Gemini-3）和领域特定基线。结果表明终身学习能够提高多轮咨询响应的一致性和整体质量。

Conclusion: 经验驱动的终身学习框架PsychAgent能够有效提升AI心理咨询师在多轮会话中的表现，通过模拟人类专家的经验积累和学习过程，实现了比传统静态方法更优的咨询质量和一致性。
...
TL;DR: 该研究提出了NARCBench基准，用于评估多智能体系统中的共谋检测，并开发了五种基于模型内部激活的探测技术，在分布偏移下仍能有效检测不同形式的共谋行为。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体在多智能体系统中的部署增加，它们带来了隐蔽协调的风险，这可能逃避标准的人类监督形式。虽然线性探测在单智能体欺骗检测中表现出潜力，但共谋本质上是多智能体现象，使用内部表示检测智能体之间的共谋尚未被探索。

Method: 引入NARCBench基准来评估环境分布偏移下的共谋检测，提出了五种探测技术，这些技术通过聚合每个智能体的欺骗分数来在群体层面分类场景。研究探索了不同探测方法在不同共谋类型中的表现。

Result: 探测技术在分布内达到1.00 AUROC，在零样本转移到结构不同的多智能体场景和隐写式21点算牌任务中达到0.60-0.86 AUROC。研究发现没有单一探测技术在所有共谋类型中占主导地位，表明不同形式的共谋在激活空间中以不同方式表现。初步证据显示信号在token级别局部化，共谋智能体的激活在处理其伙伴消息的编码部分时特别显著。
...
TL;DR: 研究发现大型语言推理模型在做出选择时，往往是先决定后思考，而不是先思考后决定。通过线性探针和激活操控技术，证明模型在生成推理过程之前就已经编码了决策。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言推理模型在决策时的认知过程：是"先思考后决定"还是"先决定后思考"？理解模型内部决策机制对于提高其透明度和可靠性具有重要意义。

Method: 使用线性探针从预生成激活中解码工具调用决策；通过激活操控技术扰动决策方向；进行行为分析观察思维链过程对决策翻转的反应。

Result: 线性探针能以高置信度从预生成激活中解码决策，有时甚至在生成第一个推理标记之前；激活操控导致过度审议，并在7-79%的案例中翻转行为；思维链过程倾向于合理化决策翻转而非抵抗。

Conclusion: 推理模型在开始文本审议之前就能编码行动选择，支持"先决定后思考"的假设，这对理解模型内部工作机制和提升其透明度有重要启示。
...
### [106] [Generalizable Dense Reward for Long-Horizon Robotic Tasks](https://arxiv.org/abs/2604.00055)
*Silong Yong,Stephen Sheng,Carl Qi,Xiaojie Wang,Evan Sheehan,Anurag Shivaprasad,Yaqi Xie,Katia Sycara,Yesh Dattatreya*

Main category: cs.RO

TL;DR: VLLR框架结合LLM/VLM的外部奖励和策略自确定性的内在奖励，无需人工奖励工程即可微调机器人基础策略，在CHORES基准上显著提升任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有机器人基础策略主要通过大规模模仿学习训练，但在长时程任务中因分布偏移和误差累积而表现不佳。强化学习微调需要人工设计奖励函数，难以适应多样化任务。

Method: 提出VLLR框架：1) 使用LLM分解任务为可验证子任务，VLM估计进度初始化价值函数用于热身阶段；2) 基于策略自确定性的内在奖励在PPO微调中提供每步指导。

Result: 在CHORES基准（移动操作和导航）上：相比预训练策略绝对成功率提升56%；相比最先进RL微调方法，在分布内任务上提升5%，在分布外任务上提升10%。消融研究表明VLM价值初始化主要提升任务完成效率，自确定性主要提升成功率。

Conclusion: VLLR框架通过结合LLM/VLM的外部奖励和策略自确定性的内在奖励，无需人工奖励工程即可有效微调机器人基础策略，显著提升任务成功率，特别是在分布外任务上表现优异。
...
### [107] [MRReP: Mixed Reality-based Hand-drawn Reference Path Editing Interface for Mobile Robot Navigation](https://arxiv.org/abs/2604.00059)
...
<details>
  <summary>Details</summary>
Motivation: 在人类共享的室内环境中，移动机器人需要反映人类空间意图的路径（如避免干扰行人流、保持舒适距离），但传统路径规划器主要优化几何成本，对人类操作者明确指定路线的支持有限。

Method: 提出MRReP混合现实界面，用户通过手势直接在物理地面上绘制手绘参考路径（HRP），然后通过定制的手绘参考路径规划器将用户指定的点序列转换为全局路径，集成到机器人导航栈中。

Result: 在受试者内实验中，与传统的2D基线界面相比，MRReP提高了路径指定精度、可用性和感知工作量，同时在物理环境中实现了更稳定的路径指定。

Conclusion: 直接在混合现实中指定路径是将人类空间意图融入移动机器人导航的有效方法。
...
### [108] [Long-Horizon Geometry-Aware Navigation among Polytopes via MILP-MPC and Minkowski-Based CBFs](https://arxiv.org/abs/2604.00162)
*Yi-Hsuan Chen,Salman Ghori,Ania Adil,Eric Feron,Calin Belta*

Main category: cs.RO

TL;DR: 提出分层规划控制框架，结合长时域引导与几何感知安全保证，用于多面体机器人在多面体障碍环境中的自主导航。
...
Method: 分层框架：高层使用MILP-MPC生成点质量机器人的名义轨迹；低层使用基于精确符号距离的CBF作为安全滤波器，并扩展到高阶CBF，强制实施机器人几何约束。

Result: 在U形和迷宫环境中对单积分器和双积分器动力学进行测试，框架缓解了纯反应式CBF导航的拓扑诱导局部最小值问题，实现安全、实时、几何感知的导航。

Conclusion: 提出的分层规划控制框架成功结合了长时域引导与几何感知安全保证，为复杂环境中多面体机器人的安全导航提供了有效解决方案。
...
### [109] [DreamControl-v2: Simpler and Scalable Autonomous Humanoid Skills via Trainable Guided Diffusion Priors](https://arxiv.org/abs/2604.00202)
...
TL;DR: 本文提出了一种改进的框架，通过直接在仿人机器人运动空间训练引导扩散模型，结合人类和机器人数据集，实现更鲁棒的自主操作技能学习。


<details>
  <summary>Details</summary>
Motivation: 为仿人机器人开发鲁棒的自主操作技能仍然是一个开放问题。虽然强化学习在腿部运动控制中应用成功，但将其应用于复杂的、交互丰富的操作任务更具挑战性，因为需要长时程规划。现有的DreamControl方法使用现成的人类运动扩散模型作为生成先验来指导强化学习策略，但存在局限性。

Method: 提出改进框架：1）直接在仿人机器人运动空间训练引导扩散模型；2）将多样化的人类和机器人数据集聚合到统一的体现空间；3）扩展参考轨迹生成以提高下游强化学习策略的鲁棒性；4）建立更自动化的流程，减少手动筛选干预。

Result: 该方法能够捕捉更广泛的技能（得益于更大的训练数据混合），建立了更自动化的流程，并通过扩展参考轨迹生成实现了更鲁棒的下游强化学习策略。在仿真和真实Unitree-G1机器人上进行了广泛实验验证。

Conclusion: 提出的改进框架通过直接在机器人运动空间训练扩散模型、整合多样化数据集和扩展轨迹生成，显著提升了仿人机器人自主操作技能的学习效果和鲁棒性。
...
### [110] [Play-Testing REMind: Evaluating an Educational Robot-Mediated Role-Play Game](https://arxiv.org/abs/2604.00300)
*Elaheh Sanoubari,Neil Fernandes,Keith Rebello,Alicia Pan,Andrew Houston,Kerstin Dautenhahn*

Main category: cs.RO

TL;DR: REMind是一款创新的教育机器人角色扮演游戏，旨在通过机器人中介的戏剧表演支持儿童反欺凌旁观者干预训练。
...
Method: 设计REMind游戏系统，让玩家观察社交机器人表演的欺凌场景，反思角色视角，并通过操纵机器人化身来排练防御策略。采用混合方法对18名9-10岁儿童进行游戏测试研究。

Result: 研究发现该体验支持了关键学习目标，包括自我效能感、视角采择、理解防御结果以及干预策略。这些结果突出了机器人中介应用戏剧作为支持社会情感学习的新教学框架的潜力。

Conclusion: REMind展示了机器人中介应用戏剧作为支持儿童反欺凌旁观者干预的有效教学框架的潜力，为促进社会情感学习提供了创新方法。
...
### [111] [Hierarchical Motion Planning and Control under Unknown Nonlinear Dynamics via Predicted Reachability](https://arxiv.org/abs/2604.00320)
*Zhiquan Zhang,Melkior Ornik*

Main category: cs.RO

TL;DR: 提出分层规划控制框架，在未知非线性动力学下在线运动规划，通过自适应状态空间划分和图抽象实现探索与利用的平衡


<details>
  <summary>Details</summary>
Motivation: 在未知非线性动力学下进行自主运动规划需要同时学习系统属性和导航到目标，现有方法通常需要先验系统知识或无法提供形式化保证

Method: 1) 非均匀自适应状态空间划分，仅在任务相关区域细化；2) 将非线性系统近似为分段仿射模型；3) 将PWA系统抽象为有向加权图，边存在性通过可达控制理论和预测可达条件验证；4) 在线更新图并执行分层规划控制

Result: 仿真结果表明该方法能有效平衡探索与利用，并提供形式化的可达性保证，特别是通过引入松弛可达条件扩展到了欠驱动系统

Conclusion: 该框架能够在有限先验系统知识下实现在线运动合成，通过自适应状态空间划分和图抽象方法，为未知非线性动力学下的自主运动规划提供了形式化保证
...
### [112] [Real Time Local Wind Inference for Robust Autonomous Navigation](https://arxiv.org/abs/2604.00343)
*Spencer Folk*

Main category: cs.RO

TL;DR: 该研究提出了一种让空中机器人能够使用机载传感器和嵌入式飞行硬件实时推理周围风场的方法，核心创新是将距离测量与稀疏现场风测量融合来预测周围流场。
...
Result: 通过在不同城市风场景中的模拟演示，评估了风预测器的预测能力，并量化了将局部风信息集成到运动规划中对自主城市导航的改进（碰撞率和能耗）。亚尺度自由飞行实验表明，这些算法可以在嵌入式飞行计算机上实时运行，具有足够带宽稳定控制小型空中机器人。

Conclusion: 该论文为未知风环境中的局部风推断和运动规划贡献了一个新范式，通过让机器人能够快速评估局部风况而无需先验环境知识，加速了空中机器人进入更具挑战性环境的进程。
...
### [113] [A Dual-Stream Transformer Architecture for Illumination-Invariant TIR-LiDAR Person Tracking](https://arxiv.org/abs/2604.00363)
*Yuki Minase,Kanji Tanaka*

Main category: cs.RO

TL;DR: 提出了一种基于热红外和深度信息的TIR-D跟踪架构，用于解决RGB-D跟踪在极端光照条件下性能下降的问题，通过知识迁移策略在缺乏标注数据的情况下实现全天候鲁棒人员跟踪。


<details>
  <summary>Details</summary>
Motivation: 自主移动机器人在多样化不可预测环境中需要鲁棒的人员跟踪能力。RGB-D跟踪在挑战性光照条件（如完全黑暗或强烈背光）下性能严重下降，需要全天候鲁棒的解决方案。

Method: 提出TIR-D跟踪架构，利用SLAM机器人的标准传感器套件（LiDAR和TIR相机）。采用顺序知识迁移策略，将大规模热红外训练模型的结构先验演化到TIR-D领域，使用细粒度差分学习率策略，在保持预训练特征提取能力的同时快速适应几何深度线索。

Result: 实验结果显示，提出的TIR-D跟踪器实现了0.700的平均重叠度（AO）和58.7%的成功率（SR），显著优于传统的RGB迁移和单模态基线方法。

Conclusion: 该方法为全天候机器人应用中鲁棒的人员跟随提供了实用且资源高效的解决方案，通过热红外和深度信息的结合有效解决了极端光照条件下的跟踪挑战。
...
### [114] [Sampling-based Task and Kinodynamic Motion Planning under Semantic Uncertainty](https://arxiv.org/abs/2604.00401)
*Qi Heng Ho,Zachary N. Sunberg,Morteza Lahijanian*

Main category: cs.RO

TL;DR: 该论文提出了一种在不确定环境中集成任务和动力学运动规划的方法，使用LTLf规范处理部分可观测环境中的非线性机器人动力学问题。


<details>
  <summary>Details</summary>
Motivation: 在不确定环境中，机器人需要同时处理任务规划和动力学运动规划，特别是在环境语义标签不确定的部分可观测场景中，现有方法难以有效整合这些要素。

Method: 将问题建模为部分可观测随机混合系统，捕捉机器人动力学、LTLf任务和环境状态变量不确定性；提出利用混合系统结构的随时算法，结合决策技术和基于采样的运动规划。

Result: 算法在不确定环境中表现出高效性，始终优于基准方法；证明了算法的可靠性和渐近最优性。

Conclusion: 提出的方法成功解决了不确定环境中的集成任务和动力学运动规划问题，为部分可观测环境中的机器人规划提供了有效解决方案。
...
### [115] [Learning Humanoid Navigation from Human Data](https://arxiv.org/abs/2604.00416)
*Weizhuo Wang,Yanjie Ze,C. Karen Liu,Monroe Kennedy*

Main category: cs.RO

TL;DR: EgoNav系统仅用5小时人类行走数据训练，无需机器人数据或微调，就能让仿人机器人在未见环境中导航。该系统使用扩散模型预测未来轨迹分布，结合360度视觉记忆和DINOv3特征，实现实时推理和零样本部署。


<details>
  <summary>Details</summary>
Motivation: 传统机器人导航方法通常需要大量机器人数据或特定环境训练，限制了在多样化、未见环境中的泛化能力。EgoNav旨在仅从少量人类行走数据中学习，实现仿人机器人在复杂环境中的零样本导航。

Method: 1) 使用扩散模型预测基于过去轨迹、360度视觉记忆（融合颜色、深度和语义）和DINOv3视频特征的未来轨迹分布；2) 混合采样方案实现10步去噪的实时推理；3) 滚动时域控制器从预测分布中选择路径。

Result: 离线评估显示EgoNav在避撞和多模态覆盖方面优于基线方法。在Unitree G1仿人机器人上的零样本部署中，系统能在未见室内外环境中导航，自然涌现出等待开门、绕过人群、避开玻璃墙等行为。

Conclusion: EgoNav证明了仅从少量人类行走数据学习就能实现仿人机器人在多样化环境中的鲁棒导航，无需机器人数据或微调。系统表现出良好的泛化能力和自然的行为涌现，为数据高效的机器人导航提供了新途径。
...
### [116] [Certificate-Driven Closed-Loop Multi-Agent Path Finding with Inheritable Factorization](https://arxiv.org/abs/2604.00428)
*Jiarui Li,Runyu Zhang,Gioele Zardini*

Main category: cs.RO

TL;DR: CDCBS算法通过引入证书轨迹和舰队预算机制，改进了ACCBS在密集场景下的有限视野问题，提供更一致的解决方案质量并支持全局分解。


<details>
  <summary>Details</summary>
Motivation: 现有闭环MAPF算法（如ACCBS）在有限计算预算下，由于有限视野规划可能导致短视行为，难以保持全局保证和利用组合结构，特别是在密集场景中表现不佳。

Method: 提出证书轨迹和舰队预算机制：证书提供无冲突回退计划和剩余成本的单调上界；仅接受证书改进的更新确保完备性；预算信息诱导预算限制的分解，支持跨时间步的全局可继承分解。基于ACCBS实例化得到CDCBS算法。

Result: 在基准地图上的实验表明，CDCBS比ACCBS获得更一致的解决方案质量，特别是在密集设置中；提出的分解机制减少了有效组大小。

Conclusion: 证书轨迹和舰队预算机制为闭环MAPF算法提供了有效的过滤和分解框架，解决了有限视野规划的短视问题，在保持完备性的同时提高了解决方案质量。
...
### [117] [Reachability-Aware Time Scaling for Path Tracking](https://arxiv.org/abs/2604.00439)
*Hossein Gholampour,Logan E. Beaver*

Main category: cs.RO

TL;DR: 该论文研究了一种用于平面双积分系统的轨迹跟踪方法，系统具有有界速度和加速度。通过离线评估加速度裕度并生成速度缩放剖面，在线使用纯追踪风格的QP跟踪器来跟踪经过缩放的参考轨迹。


<details>
  <summary>Details</summary>
Motivation: 基于采样的规划器生成的航点路径可能包含急转弯和高曲率区域，即使路径几何无碰撞，在加速度限制下的一步可达性也变得至关重要。需要确保系统能够安全跟踪这些路径而不违反加速度约束。

Method: 1. 离线阶段：在航点路径拟合的样条曲线上评估一步加速度裕度，更新标量速度缩放剖面，使所需的一步加速度保持在可用界限以下。2. 在线阶段：使用纯追踪风格、可达性引导的二次规划跟踪器，采用相同的前瞻跟踪结构来跟踪缩放后的参考轨迹。

Result: 该方法能够确保系统在速度和加速度约束下，安全跟踪包含急转弯和高曲率区域的航点路径，通过速度缩放保持加速度在可行范围内。

Conclusion: 通过结合离线速度剖面缩放和在线可达性引导的QP跟踪，实现了对复杂几何路径的安全跟踪，解决了采样规划器生成路径中急转弯区域的跟踪挑战。
...
### [118] [Multi-Camera View Scaling for Data-Efficient Robot Imitation Learning](https://arxiv.org/abs/2604.00557)
*Yichen Xie,Yixiao Wang,Shuqi Zhao,Cheng-En Wu,Masayoshi Tomizuka,Jianwen Xie,Hao-Shu Fang*

Main category: cs.RO

TL;DR: 通过多视角相机采集专家演示数据，无需额外人工成本即可丰富训练分布，提高模仿学习策略的泛化能力


<details>
  <summary>Details</summary>
Motivation: 模仿学习策略的泛化能力受限于专家演示的多样性，而收集不同环境的演示数据成本高且困难。需要一种无需额外人工努力就能利用场景多样性的实用方法。

Method: 在演示收集过程中扩展相机视角，使用多个同步相机视角从每个专家轨迹生成伪演示，丰富训练分布。分析不同动作空间与视角扩展的交互，引入相机空间表示增强多样性，并提出多视角动作聚合方法。

Result: 在模拟和真实世界操作任务中的大量实验表明，相比单视角基线，该方法在数据效率和泛化能力方面取得了显著提升。
...
### [119] [A Physical Imitation Learning Pipeline for Energy-Efficient Quadruped Locomotion Assisted by Parallel Elastic Joint](https://arxiv.org/abs/2604.00611)
*Huyue Ma,Yurui Jin,Helmut Hauser,Rui Wu*

Main category: cs.RO

TL;DR: PIL方法将强化学习控制策略蒸馏到被动并联弹性关节中，让机器人身体模仿部分控制行为，从而将机械功率卸载到被动元件，实现节能


<details>
  <summary>Details</summary>
Motivation: 动物通过脑体协同进化，利用内在身体动力学实现节能运动，而机器人通常采用集中式控制器抑制而非利用身体动力学。需要一种方法让机器人身体设计能够模仿控制行为，实现脑体协同设计

Method: 提出物理模仿学习(PIL)，将强化学习控制策略蒸馏到物理可实现的被动并联弹性关节响应中，让身体模仿部分控制行为，剩余部分由电机执行，从而将控制策略部分卸载到被动元件

Result: 在模拟四足机器人中，PIL方法在平坦地形可将高达87%的机械功率卸载到并联弹性关节，在崎岖地形可卸载18%，实现显著节能效果

Conclusion: PIL通过从控制策略蒸馏而非联合优化的方式实现脑体协同设计，不扩展搜索空间，为基于关节的机器人形态提供计算高效的任务特定具身物理智能途径
...
### [120] [StretchBot: A Neuro-Symbolic Framework for Adaptive Guidance with Assistive Robots](https://arxiv.org/abs/2604.00628)
...
TL;DR: StretchBot是一个混合神经符号机器人教练，通过结合多模态感知和知识图谱增强的大语言模型推理，为辅助伸展训练提供自适应指导，相比脚本化系统能更好地适应用户状态和环境上下文。


<details>
  <summary>Details</summary>
Motivation: 现有的辅助机器人系统大多是脚本化的，缺乏根据用户状态、环境上下文和交互动态进行自适应调整的能力，限制了它们在家庭和医疗保健环境中的物理健康支持潜力。

Method: 开发了StretchBot系统，采用混合神经符号架构，结合多模态感知与基于知识图谱的大语言模型推理，在保持结构化常规的同时支持上下文感知的调整。

Result: 初步探索性试验（3名参与者）表明，自适应条件在感知适应性和上下文相关性方面优于脚本化指导，而脚本化指导在流畅性和可预测性方面仍具有竞争力。

Conclusion: 结构化可操作知识可以帮助基于语言模型的自适应在具身辅助交互中落地，但需要更大规模的纵向研究来评估系统的鲁棒性、泛化能力和长期用户体验。
...
### [121] [LiPS: Lightweight Panoptic Segmentation for Resource-Constrained Robotics](https://arxiv.org/abs/2604.00634)
*Calvin Galagain,Martyna Poreba,François Goulette,Cyrill Stachniss*

Main category: cs.RO

TL;DR: LiPS是一种轻量级全景分割方法，在保持查询解码的同时简化特征提取和融合，实现了与大型模型相当的精度但计算量减少6.8倍，吞吐量提高4.5倍，适合资源受限的机器人平台。
...
Method: 提出LiPS方法，保留基于查询的解码机制，但引入了简化的特征提取和融合路径，采用轻量级设计来降低计算复杂度。

Result: 在标准基准测试中，LiPS达到了与更重基线模型相当的精度，同时吞吐量提高了4.5倍（帧率），计算量减少了6.8倍。

Conclusion: LiPS在现代全景分割模型和实际机器人应用之间架起了高效的桥梁，为资源受限平台提供了可行的全景分割解决方案。
...
### [122] [SoftHand Model-W: A 3D-Printed, Anthropomorphic, Underactuated Robot Hand with Integrated Wrist and Carpal Tunnel](https://arxiv.org/abs/2604.00738)
*Dhillon B. Merritt,Christopher J. Ford,Haoran Li,Malia Smith,Zhixing Chen,Efi Psomopoulou,Nathan F. Lepora*

Main category: cs.RO

TL;DR: 本文介绍了SoftHand Model-W：一款基于Pisa/IIT SoftHand的3D打印欠驱动仿人机器人手，集成了拮抗肌腱机制和2自由度肌腱驱动腕部，通过四个驱动自由度实现手指主动屈伸和手掌腕部屈伸/桡尺偏，保持软手的协同自适应特性，腕部驱动减少了手臂补偿运动，提高了任务完成效率。


<details>
  <summary>Details</summary>
Motivation: 开发具有腕部驱动功能的机器人手，以增强机器人操作能力，减少手臂补偿运动，实现更高效、更自然的物体操作，特别是在需要手-臂协调的任务中。

Method: 设计基于Pisa/IIT SoftHand的3D打印欠驱动仿人机器人手，集成拮抗肌腱机制和2自由度肌腱驱动腕部；采用腕管式肌腱布线实现远程电机放置；将SoftHand-W安装在6轴机器人臂上，通过立方体堆叠和平面圆盘旋转任务测试手-臂协调性能。

Result: 腕部驱动显著减少了手臂的补偿和重配置运动，缩短了任务完成时间；腕部使原本不可能的拾放操作成为可能；近端自由度对实现真实世界应用中多样化、类人操作至关重要。

Conclusion: SoftHand Model-W展示了近端自由度对实现多功能、类人操作的关键作用，紧凑设计使其适用于研究和辅助应用场景，为机器人操作提供了更高效、更自然的解决方案。
...
### [123] [How to Train your Tactile Model: Tactile Perception with Multi-fingered Robot Hands](https://arxiv.org/abs/2604.00744)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于CNN的触觉感知方法需要大量传感器特定数据集，且对新传感器需要重新训练，限制了触觉传感器的快速部署和规模化应用
...
Conclusion: TacViT有潜力使触觉传感更具可扩展性和实用性，加速新传感器的部署，推动机器人触觉感知的实际应用
...
### [124] [A wearable haptic device for edge and surface simulation](https://arxiv.org/abs/2604.00752)
*Rui Chen,Xianlong Mai,Alireza Sanaei,Domenico Chiaradia,Antonio Frisoli,Daniele Leonardis*

Main category: cs.RO

TL;DR: 提出了一种轻量级指尖触觉设备，通过双电机机制提供可区分的表面和边缘接触反馈，增强VR中的物体操作体验。
...
Method: 开发了一种紧凑轻便（24.3克）的指尖触觉设备，采用新颖的双电机机制，通过6×6柔性传感器阵列进行压力分布表征，区分两种刺激模式。

Result: 压力分布表征显示两种刺激模式具有不同的接触模式；初步用户研究（5名参与者）在四种条件下（边缘/表面接触×轻/重压力）达到93%的平均分类准确率，平均响应时间为2.79秒。

Conclusion: 该设备能有效传达边缘和表面触觉线索，有望提高VR环境中物体操作的逼真度。
...
### [125] [A Dual-Action Fabric-Based Soft Robotic Glove for Ergonomic Hand Rehabilitation](https://arxiv.org/abs/2604.00768)
...
TL;DR: 开发了一种基于织物的双作用软体机器人手套，通过定制化执行器支持手指屈伸和拇指外展，用于神经障碍后的手部康复和辅助


<details>
  <summary>Details</summary>
Motivation: 神经障碍导致的手部功能障碍严重影响日常生活独立性，现有软体机器人手套在定制化、人体工程学适配和屈伸驱动方面存在限制，制约了其临床应用价值

Method: 采用计算机数控热封技术制造对称腔室执行器，设计具有五个独立控制的双作用执行器支持手指屈伸，以及专门的拇指外展执行器。执行器充气时形成凹形外表面以最大化手指接触面积

Result: 执行器产生足够的关节力矩和指尖力用于日常生活任务，完整手套系统能产生足够抓握力。健康受试者测试显示主动辅助显著减少前臂肌肉活动。颈椎脊髓损伤患者初步研究显示手套辅助促进更自然的抓握模式，减少对肌腱转移抓握的依赖

Conclusion: 这种可定制、符合人体工程学的设计代表了向个性化手部康复和辅助机器人技术的实际进展，尽管当前驱动界面增加了任务完成时间
...
### [126] [Compact Keyframe-Optimized Multi-Agent Gaussian Splatting SLAM](https://arxiv.org/abs/2604.00804)
*Monica M. Q. Li,Pierre-Yves Lajoie,Jialiang Liu,Giovanni Beltrame*

Main category: cs.RO

TL;DR: 提出改进的多智能体RGB-D高斯溅射SLAM框架，通过压缩冗余3D高斯和集中式闭环检测，在保持地图保真度的同时减少85-95%的通信负载


<details>
  <summary>Details</summary>
Motivation: 多智能体3D建图在未知环境中对机器人团队至关重要，但密集地图表示在有限带宽下难以实时传输。现有基于高斯溅射的SLAM系统在集中式服务器合并地图时面临通信瓶颈

Method: 1) 在SLAM系统中加入压缩步骤，移除冗余3D高斯而不降低渲染质量；2) 采用无需初始猜测的集中式闭环检测，提供两种模式：纯渲染深度模式（仅需3D高斯）和相机深度模式（包含轻量深度图像以提高配准精度和进一步高斯修剪）

Result: 在合成和真实数据集上的评估显示，相比最先进方法，两种模式下传输数据减少85-95%，使3D高斯多智能体SLAM更接近实际部署

Conclusion: 提出的改进框架显著降低了多智能体高斯溅射SLAM的通信负载，同时保持地图质量，为实现实际场景中的实时多机器人协作建图提供了可行方案
...
### [127] [PanoAir: A Panoramic Visual-Inertial SLAM with Cross-Time Real-World UAV Dataset](https://arxiv.org/abs/2604.00852)
*Yiyang Wu,Xiaohu Zhang,Yanjin Du,Tongsu Zhang,Chujun Li,Siyang Chen,Guoyi Zhang,Xiangpeng Xu*

Main category: cs.RO

TL;DR: 本文提出了一种用于无人机应用的360度全景视觉-惯性SLAM框架，并构建了首个真实世界的全景视觉-惯性数据集，解决了传统有限视野传感器在复杂无人机场景中的漂移和失效问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-惯性SLAM方法主要依赖有限视野传感器，在复杂无人机场景中容易产生漂移甚至失效。虽然全景相机提供全方位感知以提高鲁棒性，但针对无人机的全景视觉-惯性SLAM及相关真实世界数据集仍未被充分探索。

Method: 首先构建了覆盖多样化飞行条件的真实世界全景视觉-惯性数据集，包括不同光照、高度、轨迹长度和运动动态。提出了全景视觉-惯性SLAM框架，通过全景特征提取和全景闭环检测来利用全方位视野，增强特征约束并确保全局一致性。

Result: 在提出的数据集和公共基准上的广泛实验表明，该方法相比现有方法实现了更优的精度、鲁棒性和一致性。在嵌入式平台上的部署验证了其实用性，达到了与PC实现相当的计算效率。

Conclusion: 该研究填补了无人机全景视觉-惯性SLAM领域的空白，通过构建首个真实世界数据集和提出有效的全景SLAM框架，为无人机在复杂场景中的精确定位提供了可靠解决方案，并展示了在实际嵌入式平台上的应用潜力。
...
### [128] [An Integrated Soft Robotic System for Measuring Vital Signs in Search and Rescue Environments](https://arxiv.org/abs/2604.00971)
*Jorge Francisco García-Samartín,Christyan Cruz Ulloa,Andrés Sánchez-Silva,Jaime del Cerro,Antonio Barrientos*

Main category: cs.RO

TL;DR: 该研究开发了一种用于灾后救援的软体抓取器，集成到移动机器人系统中，能够测量受害者的脉搏和血压，在灾后环境中进行生命体征评估。


<details>
  <summary>Details</summary>
Motivation: 虽然机器人在搜救行动中应用广泛，受害者评估领域也有显著进展，但在心率测量方面仍存在未解决的问题，且缺乏针对灾后场景的血压评估研究。需要开发能够在灾后环境中准确测量受害者生命体征的系统。

Method: 设计了一个软体抓取器，集成到移动机器人系统中。抓取器能够包裹受害者手臂并像血压计一样充气，通过专门的便携系统实现。采用不同的信号处理算法来处理测量数据。

Result: 系统实现了脉搏偏差4 bpm，收缩压和舒张压偏差约5 mmHg。统计数据和误差项同方差性验证证明了系统能够准确确定心率和血压。通过灾后场景测试验证了系统功能，展示了其对不同受害者位置的适应性、测量速度和安全性。

Conclusion: 该系统能够准确测量灾后受害者的心率和血压，适合搜救行动使用。软体抓取器集成移动机器人系统的方法有效解决了灾后环境中的生命体征评估问题。
...
### [129] [Infinite-Horizon Ergodic Control via Kernel Mean Embeddings](https://arxiv.org/abs/2604.01023)
*Christian Hughes,Ian Abraham*

Main category: cs.RO

TL;DR: 本文提出了一种基于核均值嵌入的无限时域遍历控制器，用于解决一般域上的长期覆盖任务，通过引入扩展的误差访问状态来避免计算复杂度随时间的增长问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于核的遍历控制方法虽然在一般覆盖域上提供了强覆盖保证，但由于计算复杂度随时间呈指数增长，实际应用仅限于有限时域，无法用于长期覆盖任务。

Method: 推导了基于核均值嵌入的无限时域遍历控制器，引入了扩展的核均值嵌入误差访问状态来递归记录状态访问历史，将过去访问与未来控制解耦，并提出了基于滚动时域控制框架的变体。

Result: 证明了所推导控制器的渐近收敛性，并在2D和3D覆盖问题中展示了遍历覆盖保证的保持性，解决了长期覆盖任务的计算可扩展性问题。

Conclusion: 通过引入扩展的误差访问状态，成功将遍历控制扩展到无限时域设置，为长期覆盖任务提供了计算高效且具有理论保证的解决方案。
...
### [130] [Stein Variational Uncertainty-Adaptive Model Predictive Control](https://arxiv.org/abs/2604.01034)
*Hrishikesh Sathyanarayan,Ian Abraham*

Main category: cs.RO

TL;DR: 提出一种基于Stein变分分布的鲁棒控制器，用于处理具有潜在参数不确定性的非线性动力系统，通过任务相关的粒子近似替代保守的最坏情况优化，在保持计算并行性的同时实现更好的性能-鲁棒性权衡。


<details>
  <summary>Details</summary>
Motivation: 传统分布鲁棒优化（DRO）方法通常采用保守的最坏情况设计，可能牺牲名义性能。现有方法对不确定性模型有严格的参数假设限制，难以处理复杂的非线性系统参数不确定性。

Method: 将最优控制与Stein变分推理相结合，采用基于粒子的任务相关不确定性分布近似，避免对不确定性模型的限制性参数假设，同时保持计算并行性。控制器专注于对闭环性能影响最大的参数敏感性。

Result: 在代表性控制问题上验证了该方法，相比名义控制、集成控制和经典分布鲁棒基线，实现了更好的性能-鲁棒性权衡，能够围绕对任务目标最关键的相关不确定性来塑造控制律。

Conclusion: 该框架在单一决策理论公式中统一了鲁棒控制和变分推理，适用于具有参数不确定性的广泛控制系统类别，为非线性动力系统的鲁棒控制提供了新的有效方法。
...
### [131] [BAT: Balancing Agility and Stability via Online Policy Switching for Long-Horizon Whole-Body Humanoid Control](https://arxiv.org/abs/2604.01064)
*Donghoon Baek,Sang-Hun Kim,Sehoon Ha*

Main category: cs.RO

TL;DR: BAT是一个在线策略切换框架，通过动态选择两个互补的全身RL控制器，在敏捷性和稳定性之间取得平衡，实现长时域全身运动控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在敏捷性、鲁棒性和精度之间的权衡问题：耦合的全身策略用于全局协调，解耦策略用于模块化精度，但缺乏系统性的整合方法。

Method: 提出BAT框架：1) 通过分层RL学习的切换策略，结合滑动时域策略预评估的专家指导；2) 选项感知的VQ-VAE，从离散运动token序列预测选项偏好；3) 通过置信度加权融合两个模块的最终决策。

Result: 在Unitree G1人形机器人上的大量仿真和真实世界实验表明，BAT能够实现多功能的长时域运动操作任务，并在多样化任务中优于现有方法。

Conclusion: BAT框架通过动态策略切换成功解决了敏捷性、鲁棒性和精度之间的权衡问题，为长时域全身运动控制提供了有效的统一解决方案。
...
### [132] [VRUD: A Drone Dataset for Complex Vehicle-VRU Interactions within Mixed Traffic](https://arxiv.org/abs/2604.01134)
*Ziyu Wang,Hongrui Kou,Cheng Wang,Ruochen Li,Hubert P. H. Shum,Amir Atapour-Abarghouei,Yuxin Zhang*

Main category: cs.RO

TL;DR: 该论文提出了一个基于无人机采集的车辆-弱势道路使用者交互数据集（VRUD），专门针对复杂、非结构化的城市环境，填补了现有数据集在混乱城市交通场景中的空白。


<details>
  <summary>Details</summary>
Motivation: 城市L4级自动驾驶（特别是自动驾驶出租车）在复杂的城市混合交通环境中面临巨大挑战，主要源于弱势道路使用者（VRU）的高密度及其高度不确定和不可预测的交互行为。现有开源数据集主要关注高速公路或规范交叉口等结构化场景，缺乏代表混乱、非结构化城市环境的数据。

Method: 提出了一种高效、高精度的基于无人机的数据集构建方法，建立了VRUD数据集。数据采集自深圳典型的"城中村"区域，这些区域交通监管松散且存在极端遮挡。数据集包含4小时4K/30Hz的录像，涵盖11,479个VRU轨迹和1,939个车辆轨迹。基于新颖的矢量碰撞时间（VTTC）阈值提取了4,002个多智能体交互场景，并提供了标准的OpenDRIVE高清地图支持。

Result: VRUD数据集的特点是VRU占所有交通参与者的约87%，显著超过现有基准数据集的比例。与仅提供原始轨迹的数据集不同，该研究提供了基于VTTC阈值提取的多智能体交互场景，为增强自动驾驶系统在复杂非结构化城市环境中的安全性能提供了宝贵的边缘案例资源。

Conclusion: 该研究为提升自动驾驶系统在复杂非结构化城市环境中的安全性能提供了有价值的稀缺边缘案例资源。VRUD数据集已开源，可供进一步研究使用。
...
### [133] [Deep Reinforcement Learning for Robotic Manipulation under Distribution Shift with Bounded Extremum Seeking](https://arxiv.org/abs/2604.01142)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人操作中表现出色，但训练策略在测试条件与训练分布不同时性能会下降。这在接触丰富的任务（如推动和抓取放置）中尤为突出，因为目标、接触条件或机器人动力学的变化都可能导致推理时系统处于分布外状态。

Method: 提出混合控制器，将深度确定性策略梯度（DDPG）策略与有界极值搜索（bounded ES）结合。首先在标准条件下训练DDPG策略，然后在部署时与有界ES结合。RL策略提供快速操作行为，而有界ES确保在操作条件偏离训练条件时整体控制器对时间变化的鲁棒性。
...
### [134] [SMASH: Mastering Scalable Whole-Body Skills for Humanoid Ping-Pong with Egocentric Vision](https://arxiv.org/abs/2604.01158)
...
<details>
  <summary>Details</summary>
Motivation: 现有的人形乒乓球系统存在两个核心限制：1）依赖外部感知系统；2）无法实现敏捷的全身协调以执行精确任务。这源于两个挑战：在快速机器人运动下实现低延迟、鲁棒的机载自我中心感知，以及获取足够多样化的任务对齐击球动作来学习精确自然的全身行为。

Method: 提出了一个模块化系统，包含三个关键创新：1）通过紧密协调的全身控制实现敏捷精确的球体交互，而非分离的上半身和下半身行为；2）使用生成模型增强和多样化击球动作，获得可扩展的运动先验；3）首次实现仅使用机载感知就能完成连续击球的人形乒乓球系统。

Result: 广泛的真实世界实验表明，系统能在高速条件下实现稳定精确的球体交换，验证了可扩展的感知驱动全身技能学习在动态人形交互任务中的有效性。系统能够执行多样化的击球动作，包括爆炸性的全身扣杀和低蹲击球。

Conclusion: 该工作展示了通过结合可扩展的全身技能学习和机载自我中心感知，可以克服现有系统的局限性，实现无需外部摄像头的高性能人形乒乓球系统，为动态人形交互任务提供了新的解决方案。
...
### [135] [Collaborative Task and Path Planning for Heterogeneous Robotic Teams using Multi-Agent PPO](https://arxiv.org/abs/2604.01213)
*Matthias Rubio,Julia Richter,Hendrik Kolvenbach,Marco Hutter*

Main category: cs.RO

TL;DR: 基于MAPPO的多智能体协同规划策略，用于协调异构机器人团队解决复杂的目标分配与调度问题，相比传统规划算法具有更好的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 地外探索需要具备多样化能力的机器人团队，但传统规划算法在问题规模增大时扩展性差，导致规划周期长、推理成本高。学习型方法将扩展性问题从运行时转移到训练时，是实现实时规划的关键步骤。

Method: 采用多智能体近端策略优化（MAPPO）算法，开发协同规划策略来协调异构机器人团队，解决复杂的目标分配和调度问题。

Result: 通过穷举搜索获得的单目标最优解进行基准测试，并在行星探索场景中评估了在线重新规划的能力。

Conclusion: MAPPO为基础的协同规划策略能够有效协调异构机器人团队，相比传统规划算法具有更好的可扩展性，是实现实时规划的有前景的方法。
...
### [136] [Functional Force-Aware Retargeting from Virtual Human Demos to Soft Robot Policies](https://arxiv.org/abs/2604.01224)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常仅重定向人类关节轨迹，无法有效处理软体机器人手与人类手之间的极端形态不匹配和非线性柔顺性，导致技能转移效果不佳。需要显式建模接触几何和力分布来实现有效的技能转移。

Method: 1. 使用沉浸式VR捕捉人类演示数据（手部运动学、物体运动、密集接触斑块、详细接触力信息）；2. 两阶段力感知重定向算法：第一阶段将演示接触力分配给人类手指并按比例分配给机器人手指，建立力平衡映射；第二阶段结合基线末端执行器姿态跟踪与测地加权接触细化，实时调整机器人指尖目标。

Result: 在定制非拟人化气动软体机器人手上评估接触丰富的操作任务：1. 控制器将指尖轨迹跟踪RMSE降低高达55%，跟踪方差降低高达69%（相比运动学和基于学习的基线）；2. 策略层面在零样本现实世界部署和仿真中均获得更高成功率。

Conclusion: 显式建模接触几何和力分布对于软体机器人手的有效技能转移至关重要，仅靠运动学模仿无法恢复这些信息。SoftAct框架能够使软体机器人手重现人类演示的功能意图，同时自然适应极端形态不匹配和非线性柔顺性。
...
### [137] [Two-Stage Optimizer-Aware Online Data Selection for Large Language Models](https://arxiv.org/abs/2604.00001)
*Fangxin Wang,Peyman Baghershahi,Langzhou He,Henry Peng Zou,Sourav Medya,Philip S. Yu*

Main category: cs.LG

TL;DR: 提出一种面向优化器的梯度在线数据选择与重加权框架，用于大语言模型在线微调，通过两阶段过滤加权算法改善收敛和下游性能
...
Method: 提出优化器感知框架，将在线选择视为在优化器状态下塑造下一个目标导向的更新；开发两阶段"过滤-加权"算法：先过滤几何上有用的候选样本，然后优化其系数；引入因子化外积梯度表示和优化的矩阵计算来处理长上下文数据

Result: 实验表明，在相同数据预算下，该方法相比现有在线数据选择基线能持续改善收敛性和下游性能

Conclusion: 将在线数据选择视为优化器感知的更新匹配问题，通过考虑样本间交互和冗余的算法设计，能够更有效地进行大语言模型的在线微调
...
### [138] [Task-Centric Personalized Federated Fine-Tuning of Language Models](https://arxiv.org/abs/2604.00050)
*Gabriel U. Talasso,Meghdad Kurmanji,Allan M. de Souza,Nicholas D. Lane,Leandro A. Villas*

Main category: cs.LG

TL;DR: FedRouter是一个基于聚类的个性化联邦学习框架，通过为每个任务而非每个客户端构建专门模型，解决异构任务联邦学习中的泛化性和任务干扰问题。


<details>
  <summary>Details</summary>
Motivation: 传统个性化联邦学习(pFL)虽然能提升本地性能，但缺乏两个方面的鲁棒性：1) 泛化性：当客户端需要处理未见任务或数据分布变化时；2) 客户端内任务干扰：当单个客户端数据包含多个可能相互干扰的分布时。
...
### [139] [Evolution Strategies for Deep RL pretraining](https://arxiv.org/abs/2604.00066)
*Adrian Martínez,Ananya Gupta,Hanka Goralija,Mario Rico,Saúl Fenollosa,Tamar Alphaidze*

Main category: cs.LG

TL;DR: 比较进化策略与深度强化学习在不同难度任务中的性能表现，并探索ES作为DRL预训练方法的有效性


<details>
  <summary>Details</summary>
Motivation: 深度强化学习虽然有效但计算成本高且需要精细调参，进化策略计算更简单但性能通常不如DRL，研究探索ES是否能在某些场景下替代或增强DRL

Method: 在Flappy Bird、Breakout和MuJoCo环境中比较ES和DRL的性能，并测试将ES作为DRL初始训练步骤的效果

Result: ES并不总是比DRL训练更快；作为预训练步骤时，只在简单环境（Flappy Bird）中有益，在复杂任务（Breakout和MuJoCo Walker）中对训练效率或稳定性改善有限或无改善

Conclusion: 进化策略在简单任务中可作为DRL的补充，但在复杂场景下替代或增强DRL的效果有限，需要根据任务难度选择合适的算法
...
### [140] [ParetoBandit: Budget-Paced Adaptive Routing for Non-Stationary LLM Serving](https://arxiv.org/abs/2604.00136)
*Annette Taberner-Miller*

Main category: cs.LG

TL;DR: ParetoBandit是一个基于成本感知上下文bandits的开源自适应路由器，能在运行时动态适应价格和质量变化，同时强制执行美元计价的预算限制，并支持新模型的热插拔集成。


<details>
  <summary>Details</summary>
Motivation: 生产环境中的LLM服务通常依赖多模型组合，其成本范围可达530倍，路由决策需要在质量和成本之间权衡。这种权衡是非平稳的：提供商调整定价、模型质量可能无声下降、新模型需要无停机集成。现有解决方案无法同时满足预算约束、在线适应变化和运行时模型集成的要求。

Method: ParetoBandit采用三个核心机制：1) 在线原始-对偶预算调节器，在无限请求流上强制执行每个请求的成本上限；2) 充分统计量的几何遗忘，实现快速适应价格和质量变化；3) 热插拔注册表，允许运行时添加或移除模型，新模型通过强制探索阶段后，UCB选择从实时流量中发现其质量-成本定位。

Result: 在三个模型组合的1,824个提示路由测试中，七个预算上限下，平均每个请求成本从未超过目标0.4%以上。系统能有效适应变化：最昂贵模型价格降低一个数量级时，质量提升达0.071；无声质量下降能在预算内被检测并重新路由。冷启动模型在约142步内达到有意义的采用率而不违反成本上限。路由延迟仅9.8ms（CPU），路由决策本身仅22.5微秒。

Conclusion: ParetoBandit是首个能同时强制执行美元计价预算、在线适应价格和质量变化、并在运行时集成新模型的自适应路由器，为生产环境中的多模型LLM服务提供了高效、自适应的路由解决方案。
...
TL;DR: 使用智能鞋带锁传感器通过机器学习检测老年人坐站转换动作，准确率达98%，可用于跌倒风险评估和移动能力监测。


<details>
  <summary>Details</summary>
Motivation: 姿势稳定性对老年人独立生活和预防跌倒至关重要，坐站转换是评估下肢力量、肌肉骨骼健康和跌倒风险的关键指标，需要开发客观的监测方法。

Method: 使用智能鞋带锁传感器（集成载荷传感器、加速度计和陀螺仪）采集16名老年人的坐站转换数据，提取多模态信号特征，采用4折参与者独立交叉验证训练四种机器学习分类器。

Result: 袋装树分类器在坐站转换分类中达到0.98的准确率和0.8的F1分数，正确分类转换的持续时间测量平均绝对误差为0.047秒，标准差为0.07秒。

Conclusion: 智能鞋带锁传感器结合机器学习方法能够准确检测和测量老年人的坐站转换，具有在现实世界中评估跌倒风险和监测移动能力的潜力。
...
### [142] [Lévy-Flow Models: Heavy-Tail-Aware Normalizing Flows for Financial Risk Management](https://arxiv.org/abs/2604.00195)
*Rachid Drissi*

Main category: cs.LG

TL;DR: 提出Lévy-Flows：用Lévy过程分布（VG和NIG）替代高斯基分布的正规化流模型，能捕捉厚尾特性，在金融风险建模中表现优异


<details>
  <summary>Details</summary>
Motivation: 传统正规化流使用高斯基分布难以有效建模金融数据中的厚尾特性，需要能同时保持精确似然评估和高效采样的厚尾分布替代方案

Method: 使用Lévy过程分布（方差伽马VG和正态逆高斯NIG）作为正规化流的基分布，建立理论保证：渐近线性流变换保持尾指数，特定架构在变换区域外精确保持基分布尾形

Result: 在S&P 500日收益率等金融数据上，VG流比高斯流降低69%测试负对数似然，实现精确95% VaR校准；NIG流提供最准确的期望损失估计
...
### [143] [QUEST: A robust attention formulation using query-modulated spherical attention](https://arxiv.org/abs/2604.00199)
...
Result: QUEST能够稳定训练，提高模型性能，增强对数据损坏和对抗攻击的鲁棒性，在视觉应用和其他领域都表现出良好的通用性。

Conclusion: QUEST注意力机制通过约束键向量范数解决了Transformer训练不稳定问题，同时保持了灵活性，是一种有效的注意力机制改进方案。
...
### [144] [Offline Constrained RLHF with Multiple Preference Oracles](https://arxiv.org/abs/2604.00200)
*Brenden Latham,Mehrdad Moharrami*

Main category: cs.LG

TL;DR: 研究离线约束强化学习，使用多偏好预言机，在满足保护群体福利约束下最大化目标群体效用，提出基于KL正则化拉格朗日的对偶算法，提供有限样本性能保证


<details>
  <summary>Details</summary>
Motivation: 针对需要在性能与安全性/公平性之间权衡的应用场景，研究如何在满足最小保护群体福利约束的前提下最大化目标群体效用，解决离线约束偏好学习问题

Method: 从参考策略收集的成对比较中通过最大似然估计预言机特定奖励，将约束目标转化为KL正则化拉格朗日形式，提出仅需对偶变量的算法，确保高概率约束满足

Result: 提供了离线约束偏好学习的首个有限样本性能保证，理论分析可扩展到多约束和一般f-散度正则化情况

Conclusion: 提出了一种有效的离线约束强化学习方法，能够在统计不确定性下保证约束满足，为实际应用中性能与公平性/安全性的权衡提供了理论保证
...
### [145] [Lead Zirconate Titanate Reservoir Computing for Classification of Written and Spoken Digits](https://arxiv.org/abs/2604.00207)
*Thomas Buckley,Leslie Schumm,Manor Askenazi,Edward Rietman*

Main category: cs.LG

TL;DR: 该研究使用未极化的锆钛酸铅(PZT)立方体作为物理储层计算基底，处理手写和语音数字分类任务，发现对于中等难度任务储层计算优势最明显。


<details>
  <summary>Details</summary>
Motivation: 探索物理储层计算在实际分类任务中的应用潜力，特别是利用PZT这种低功耗、可集成的半导体材料作为计算基底，验证储层计算在不同难度任务中的表现差异。

Method: 采用未极化的锆钛酸铅(PZT)立方体作为物理储层计算基底，处理MNIST手写数字和AudioMNIST语音数字数据集，与相同预处理数据上的逻辑回归基线进行比较。

Result: PZT储层在MNIST手写数字上达到89.0%准确率，比逻辑回归基线提高2.4个百分点；在AudioMNIST语音数字上达到88.2%准确率，与基线方法(88.1%)表现相当。

Conclusion: 物理储层计算在任务难度超过简单线性分类器能力但仍在储层动态计算能力范围内时表现最佳，PZT作为低功耗计算基底具有与数字算法集成的潜力。
...
### [146] [Diversity-Aware Reverse Kullback-Leibler Divergence for Large Language Model Distillation](https://arxiv.org/abs/2604.00223)
...
<details>
  <summary>Details</summary>
Motivation: RKL（反向KL散度）在LLM蒸馏中表现优于FKL，但存在结构性缺陷：导致学生模型过度自信预测，减少输出多样性，且对非目标类监督较弱导致尾部对齐差

Method: 提出多样性感知RKL（DRKL）：1）通过梯度分解分析RKL问题，发现非目标梯度持续推高目标logit；2）移除这种梯度效应；3）加强非目标监督，同时保留RKL的优化优势

Result: 在多个数据集和模型家族上的实验表明，DRKL一致优于FKL、RKL和其他最先进的蒸馏目标，实现了更好的性能和更优的保真度-多样性权衡

Conclusion: DRKL有效解决了RKL蒸馏中的过度自信和多样性问题，在保持RKL优势的同时显著提升了蒸馏效果，为LLM蒸馏提供了更优的目标函数
...
### [147] [MAC-Attention: a Match-Amend-Complete Scheme for Fast and Accurate Attention Computation](https://arxiv.org/abs/2604.00235)
*Jinghan Yao,Sam Adé Jacobs,Walid Krichene,Masahiro Tanaka,Dhabaleswar K Panda*

Main category: cs.LG

TL;DR: MAC-Attention是一种新的注意力机制，通过重用语义相似的查询的先前注意力计算来加速长上下文解码，保持保真度和访问完整性，显著减少KV访问和生成延迟。


<details>
  <summary>Details</summary>
Motivation: 长上下文解码在LLMs中是IO受限的：每个令牌都需要重新读取不断增长的KV缓存。现有加速方法通过压缩（降低保真度）或选择/淘汰（限制可访问内容）来减少字节数，这两者都会影响延迟回忆和长文本生成质量。

Method: MAC-Attention包含三个阶段：匹配阶段在短局部窗口内执行预RoPE L2匹配；修正阶段通过重新计算匹配边界附近的小区域来修正重用的注意力；完成阶段通过数值稳定的合并将修正结果与KV尾部的新鲜注意力计算融合。命中匹配时，计算和带宽复杂度与上下文长度无关。

Result: 在LongBench v2（120K）、RULER（120K）和LongGenBench（16K连续生成）上，相比最新的FlashInfer库，MAC-Attention将KV访问减少高达99%，在128K上下文下将令牌生成延迟降低超过60%，注意力阶段加速超过14.3倍，端到端加速高达2.6倍，同时保持完整注意力质量。
...
### [148] [Hierarchical Discrete Flow Matching for Graph Generation](https://arxiv.org/abs/2604.00236)
...
### [149] [Softmax gradient policy for variance minimization and risk-averse multi armed bandits](https://arxiv.org/abs/2604.00241)
*Gabriel Turinici*

Main category: cs.LG

TL;DR: 本文提出了一种用于多臂老虎机问题的新算法，专注于选择方差最小的臂（风险最小化），而非传统上追求最高期望奖励的方法。


<details>
  <summary>Details</summary>
Motivation: 传统多臂老虎机算法主要关注识别期望奖励最高的臂，但在实际应用中，决策者往往更关注稳定性，需要选择风险最小（方差最低）的臂。本文旨在解决这种风险感知场景下的决策问题。

Method: 采用softmax参数化策略，提出新算法通过使用当前臂分布的两个独立抽样来构建目标函数的无偏估计，从而选择最小方差臂。算法在自然条件下被证明具有收敛性。

Result: 提供了数值实验验证算法的实际行为，并为实现选择提供指导。算法能够有效处理一般风险感知问题，平衡平均奖励最大化和方差最小化之间的权衡。

Conclusion: 本文提出的算法为风险感知的多臂老虎机问题提供了有效的解决方案，特别适用于需要稳定性优先于高回报但不确定收益的决策场景。
...
### [150] [Informed Machine Learning with Knowledge Landmarks](https://arxiv.org/abs/2604.00256)
*Chuyi Dai,Witold Pedrycz,Suping Xu,Ding Liu,Xianmin Wang*

Main category: cs.LG

TL;DR: 提出了一种名为KD-ML的知识-数据机器学习新方法，通过将数值数据与以粒度知识地标形式表达的知识片段相结合，构建了一个包含数据优化和知识约束的增强损失函数。


<details>
  <summary>Details</summary>
Motivation: 认识到数据与知识之间存在根本互补性：数据是精确的、局部的，通常局限于输入空间的某些区域；而知识是全局的，在更高抽象层次上表达。这种互补性为构建更强大的机器学习模型提供了机会。

Method: 提出KD-ML框架，将知识表示为信息颗粒并组织为输入-输出信息颗粒集合（知识地标）。开发了包含两个组件的增强损失函数：一个负责基于数值数据优化模型，另一个作为粒度正则化器，确保模型遵守粒度约束（知识地标）。

Result: 在两个物理基准测试上的实验表明，提出的KD模型在性能上持续优于纯数据驱动的机器学习模型。研究还展示了超参数在平衡数据和知识贡献方面的作用，以及数据质量（噪声水平）和知识地标粒度水平对模型性能的影响。

Conclusion: KD-ML为知识-数据机器学习提供了一个系统化的设计框架，通过有效整合数值数据和抽象知识，构建了比纯数据驱动模型更优越的机器学习模型，展示了数据和知识互补性的实际价值。
...
### [151] [Hierarchical Apprenticeship Learning from Imperfect Demonstrations with Evolving Rewards](https://arxiv.org/abs/2604.00258)
*Md Mirajul Islam,Rajesh Debnath,Adittya Soukarjya Saha,Min Chi*

Main category: cs.LG

TL;DR: HALIDE是一个从非完美学生演示中学习教学策略的分层框架，通过排名演示质量来区分临时错误和次优策略，比依赖最优轨迹的方法更准确预测学生决策。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的学生互动往往是不完美且不断演化的，学生探索、犯错、修正策略、随着理解发展而精炼目标。现有方法依赖最优或接近最优的专家演示和固定奖励，无法有效处理这种非完美演示。

Method: 提出HALIDE（Hierarchical Apprenticeship Learning from Imperfect Demonstrations with Evolving Rewards），采用分层学习框架，在多抽象层次建模学生行为，推断高层次意图和策略，同时显式捕捉学生奖励函数的时间演化，并将演示质量整合到分层奖励推断中。

Result: HALIDE比依赖最优轨迹、固定奖励或未排名的非完美演示的方法更准确地预测学生教学决策。

Conclusion: 非完美学生演示不应被视为需要丢弃的噪声，而是具有结构化的信号，只要能够对其相对质量进行排名。分层框架能够有效利用次优学生演示来推断教学策略。
...
### [152] [Learning to Shuffle: Block Reshuffling and Reversal Schemes for Stochastic Optimization](https://arxiv.org/abs/2604.00260)
*Lam M. Nguyen,Dzung T. Phan,Jayant Kalagnanam*

Main category: cs.LG

TL;DR: 本文提出使用LLM引导的程序演化框架来发现有效的无放回SGD洗牌策略，识别出块重洗和配对反转两个核心组件，理论证明这些组件能改善优化常数和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有随机重洗牌策略虽然优于循环和单次洗牌方案，但缺乏系统的方法来设计能进一步改进优化常数或稳定性的新数据排序方案。

Method: 使用大型语言模型引导的程序演化框架来发现有效的无放回SGD洗牌规则，从中抽象出块重洗和配对反转两个核心结构组件，并分别进行理论分析。

Result: 块重洗在温和条件下严格降低前缀梯度方差常数，提供可证明的优于随机重洗的改进；配对反转对称化epoch映射并消除主导的顺序相关二阶项，将顺序敏感性从步长的二次降低到三次。

Conclusion: 通过LLM引导的演化发现的洗牌算法在理论和实验上都优于标准洗牌方案，为设计更优的SGD数据排序策略提供了系统方法。
...
### [153] [Autonomous Adaptive Solver Selection for Chemistry Integration via Reinforcement Learning](https://arxiv.org/abs/2604.00264)
*Eloghosa Ikponmwoba,Opeoluwa Owoyele*

Main category: cs.LG

TL;DR: 提出基于约束强化学习的自适应求解器选择框架，在化学反应动力学计算中自动选择隐式BDF积分器或准稳态求解器，在保证精度的同时实现计算加速。
...
Method: 将求解器选择建模为马尔可夫决策过程，采用约束强化学习框架，通过拉格朗日奖励函数在线调整乘子，在用户规定的精度容差下最小化计算成本，学习考虑当前选择对下游误差积累影响的轨迹感知策略。

Result: 在0D均质反应器条件下，RL自适应策略平均加速约3倍（范围1.11-10.58倍），保持106种n-十二烷机理的准确点火延迟和物种分布，仅增加约1%推理开销。未经重新训练，该策略可迁移到1D逆流扩散火焰，在应变率10-2000 s⁻¹范围内实现约2.2倍加速，温度精度接近参考值，仅在12-15%的时空点选择CVODE。

Conclusion: 提出的强化学习框架能够学习特定问题的积分策略同时尊重精度约束，为具有空间异质性刚度的多物理场系统开辟了自适应、自优化工作流程的途径。
...
### [154] [SYNTHONY: A Stress-Aware, Intent-Conditioned Agent for Deep Tabular Generative Models Selection](https://arxiv.org/abs/2604.00293)
*Hochan Son,Xiaofeng Lin,Jason Ni,Guang Cheng*

Main category: cs.LG

TL;DR: 论文研究了表格数据生成模型选择问题，提出了基于压力特征的分析方法和SYNTHONY选择框架，能够根据用户意图和数据集特征自动选择最佳生成模型。


<details>
  <summary>Details</summary>
Motivation: 现有表格数据生成模型（GANs、扩散模型、LLM生成器）在不同数据集上表现不稳定，最佳模型选择高度依赖于数据分布特征，这使得实际部署困难，用户需要在保真度、隐私性和实用性之间平衡。

Method: 提出了"压力特征分析"方法，量化数据集在四个可解释压力维度上的难度；开发了SYNTHONY选择框架，将压力特征与校准的能力注册表匹配，使用kNN选择器基于这些特征预测生成器性能。

Result: 在7个数据集、10个生成器和3种意图的基准测试中，基于压力特征的kNN选择器实现了强大的Top-1选择准确率，显著优于零样本LLM选择器和随机基线。

Conclusion: 压力特征能有效预测生成器性能，但手工制作的能力注册表是主要瓶颈，未来工作需要学习能力表示来进一步提升选择效果。
...
### [155] [Robust Multimodal Safety via Conditional Decoding](https://arxiv.org/abs/2604.00310)
*Anurag Kumar,Raghuveer Peri,Jon Burnsky,Alexandru Nelus,Rohit Paturi,Srikanth Vishnubhotla,Yanjun Qi*

Main category: cs.LG

TL;DR: CASA是一种简单的条件解码策略，利用多模态大语言模型的内部表示来预测安全标记，通过安全注意力模块增强恶意查询检测能力，无需外部分类器或模态特定微调。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在跨模态交互被有害查询利用时，安全对齐会退化。仅基于文本对齐的模型扩展到两个或多个模态时，攻击成功率更高。

Method: 提出CASA条件解码策略，利用MLLM内部表示预测二元安全标记，引入新颖的安全注意力模块增强恶意查询检测能力。设计确保鲁棒的安全对齐，无需外部分类器、辅助头或模态特定安全微调。

Result: 在MM-SafetyBench、JailbreakV-28k和对抗音频测试等多样化基准上，CASA将跨模态和跨攻击类型的平均攻击成功率降低了97%以上。在良性输入上保持强实用性，通过自动和人工评估验证。

Conclusion: CASA是一个简单且可泛化的框架，能显著提升多模态LLM的安全性，同时保持实用性。
...
### [156] [Is One Token All It Takes? Graph Pooling Tokens for LLM-based GraphQA](https://arxiv.org/abs/2604.00342)
*Ankit Grover,Lodovico Giaretta,Rémi Bourgerie,Sarunas Girdzijauskas*

Main category: cs.LG

TL;DR: 该研究通过多令牌池化和全局注意力机制缓解GNN与LLM集成中的信息瓶颈问题，评估多种池化算子并发现LoRA能稳定特定分层投影，使压缩表示媲美全图基线。


<details>
  <summary>Details</summary>
Motivation: 当前GNN与LLM集成方法（如G-Retriever）存在严重信息瓶颈，它们使用标准GNN和激进的平均池化将整个图子结构压缩为单个令牌，导致复杂结构信息丢失。

Method: 采用两种正交策略：1）通过多令牌池化增加图到LLM接口的带宽；2）通过全局注意力机制增强图编码器的语义质量。评估了Top-k、SAGPool、DiffPool、MinCutPool和VNPool等分层剪枝和基于聚类的池化算子，并使用LoRA稳定训练过程。

Result: 池化在软提示调优中引入显著不稳定性，但LoRA能有效稳定特定分层投影（特别是VNPool和剪枝方法）。压缩表示能媲美全图基线（在WebQSP上达到约73% Hit@1）。分析显示GraphQA基准存在表示饱和问题，目标答案常与孤立节点特征高度相关。

Conclusion: 多令牌池化和全局注意力机制能有效缓解图到LLM接口的信息瓶颈，VNPool在结构上相当于单层Perceiver IO编码器。GraphQA基准需要更复杂的评估指标来避免表示饱和问题。
...
### [157] [GUIDE: Reinforcement Learning for Behavioral Action Support in Type 1 Diabetes](https://arxiv.org/abs/2604.00385)
...
Method: 提出GUIDE框架，使用结构化动作空间（干预类型、幅度和时间），包括胰岛素推注和碳水化合物摄入。整合患者特异性血糖预测器，支持离线和在线强化学习算法，在统一环境中评估。

Result: 在25名1型糖尿病患者中评估，CQL-BC算法表现最佳，平均血糖在目标范围内时间达到85.49%，同时保持低低血糖暴露。行为相似性分析显示学习到的策略保留了患者行为模式的关键结构特征。

Conclusion: 保守的离线强化学习结合结构化行为动作空间可以为个性化糖尿病管理提供具有临床意义且行为合理的决策支持。
...
### [158] [Gradient-Based Data Valuation Improves Curriculum Learning for Game-Theoretic Motion Planning](https://arxiv.org/abs/2604.00388)
*Shihao Li,Jiachen Li,Dongmei Chen*

Main category: cs.LG

TL;DR: 基于梯度的数据估值方法显著优于基于元数据的启发式方法，用于训练博弈论运动规划器的课程排序。TracIn梯度相似性评分在nuPlan基准测试中表现优异，通过估计训练场景对验证损失减少的贡献来构建课程权重。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索更有效的课程学习方法，以提升博弈论运动规划器的训练效率。传统基于元数据的启发式方法存在局限性，需要更智能的数据估值方法来优化训练过程。

Method: 采用TracIn梯度相似性评分方法，应用于GameFormer模型在nuPlan基准测试上。通过计算训练场景对验证损失减少的估计贡献来构建课程权重，而不是依赖人工设计的元数据特征。

Result: TracIn加权课程在三个随机种子下平均规划ADE为1.704±0.029米，显著优于基于交互难度的元数据课程（1.822±0.014米）。分析显示TracIn评分与场景元数据几乎正交（Spearman ρ=-0.014），表明梯度估值能捕捉手工特征无法发现的训练动态。

Conclusion: 基于梯度的数据估值是提高博弈论规划样本效率的实用工具。全数据课程加权优于硬数据选择，梯度方法能有效优化训练过程，为运动规划领域提供了新的课程学习策略。
...
### [159] [A Cross-graph Tuning-free GNN Prompting Framework](https://arxiv.org/abs/2604.00399)
*Yaqi Chen,Shixun Huang,Ryan Twemlow,Lei Wang,John Le,Sheng Wang,Willy Susilo,Jun Yan,Jun Shen*

Main category: cs.LG

TL;DR: CTP是一个跨图免调优提示框架，支持同质和异质图，无需参数调优即可部署到未见图上，实现即插即用的GNN推理引擎，在少样本预测任务上相比SOTA平均准确率提升30.8%。


<details>
  <summary>Details</summary>
Motivation: 现有图提示方法仍需任务特定参数更新，且难以跨图泛化，限制了性能并削弱了提示的核心承诺。需要一种无需调优即可跨图部署的图提示框架。

Method: 提出跨图免调优提示框架CTP，支持同质和异质图，可直接部署到未见图上而无需进一步参数调优，实现即插即用的GNN推理引擎。

Result: 在少样本预测任务上的广泛实验显示，相比SOTA方法，CTP平均准确率提升30.8%，最大提升达54%，证实了其有效性。

Conclusion: CTP为图提示学习提供了新视角，通过免调优跨图部署实现了真正的即插即用GNN推理，显著提升了少样本预测性能。
...
### [160] [The Rashomon Effect for Visualizing High-Dimensional Data](https://arxiv.org/abs/2604.00485)
*Yiyang Sun,Haiyang Huang,Gaurav Rajesh Parikh,Cynthia Rudin*

Main category: cs.LG

TL;DR: 该论文提出通过利用降维中的Rashomon集合（多个同样好的嵌入）来构建更强大和可信的可视化表示，包括PCA对齐、概念对齐正则化和提取共同知识的方法。


<details>
  <summary>Details</summary>
Motivation: 降维本质上具有非唯一性，多个嵌入可以同样好地保留高维数据的结构，但布局或几何形状不同。传统方法只关注单一嵌入，而忽略了这种多样性。论文旨在通过正式定义降维的Rashomon集合（"好"嵌入的集合），并利用这种多样性来获得更强大和可信的表示。

Method: 1. PCA-informed alignment：引导嵌入朝向主成分，使坐标轴可解释而不扭曲局部邻域
2. Concept-alignment regularization：将嵌入维度与外部知识（如类别标签或用户定义概念）对齐
3. 提取Rashomon集合中的共同知识：识别可信且持久的最近邻关系，用于构建改进局部结构同时保留全局关系的精炼嵌入

Result: 通过超越单一嵌入并利用Rashomon集合，论文提供了一个灵活的框架来构建可解释、鲁棒且目标对齐的可视化。该方法能够产生具有可解释坐标轴的嵌入，将外部知识融入表示中，并通过提取共同知识改进局部结构。

Conclusion: 通过正式定义和利用降维中的Rashomon集合，可以构建更强大、更可信、更可解释的可视化表示。这种方法提供了灵活的框架，能够根据具体目标调整嵌入，同时保持数据的基本结构，为降维应用开辟了新的可能性。
...
### [161] [Scheduling LLM Inference with Uncertainty-Aware Output Length Predictions](https://arxiv.org/abs/2604.00499)
*Haoyu Zheng,Yongqiang Zhang,Fangcheng Fu,Xiaokai Zhou,Hao Luo,Hongchao Zhu,Yuanyuan Zhu,Hao Wang,Xiao Yan,Jiawei Jiang*

Main category: cs.LG

TL;DR: 提出TIE调度器，用对数t分布拟合LLM输出长度的不确定性，替代SJF调度中的点估计，显著降低延迟并提升吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理调度方法通常预测单个输出长度进行SJF调度，但这与LLM解码过程的随机性不匹配。输出长度本质上是随机的，由EOS令牌采样决定，应该用分布而非单值来拟合。

Method: 通过实证数据分析发现输出长度服从重尾分布，可用对数t分布拟合。提出尾膨胀期望(TIE)指标替代SJF中的输出长度，该指标通过尾部概率调整对数t分布的期望，以考虑生成长输出的风险。

Result: 与三个强基线比较，TIE调度器在线推理中每令牌延迟降低2.31倍，离线数据生成中吞吐量提升1.42倍。

Conclusion: 考虑LLM输出长度的随机性和重尾特性，使用TIE指标进行SJF调度能显著提升推理性能，比传统的点估计方法更有效。
...
### [162] [Lipschitz Dueling Bandits over Continuous Action Spaces](https://arxiv.org/abs/2604.00523)
...
<details>
  <summary>Details</summary>
Motivation: 对决赌博机和Lipschitz赌博机已分别被研究，但两者的结合尚未探索。连续动作空间中的纯比较反馈对决赌博机问题具有实际应用价值

Method: 提出首个Lipschitz对决赌博机算法，采用轮次探索和自适应参考臂引导的递归区域消除策略，利用相对反馈的新分析工具

Result: 证明了后悔界为$\tilde O\left(T^{\frac{d_z+1}{d_z+2}}\right)$，其中$d_z$是近优区域的缩放维度，算法仅需对数空间复杂度

Conclusion: 成功解决了连续动作空间Lipschitz对决赌博机问题，提供了理论保证和高效的空间复杂度，填补了该领域的研究空白
...
### [163] [MF-QAT: Multi-Format Quantization-Aware Training for Elastic Inference](https://arxiv.org/abs/2604.00529)
*Zifei Xu,Sayeh Sharify,Hesham Mostafa*

Main category: cs.LG

TL;DR: 该论文研究了多格式量化感知训练（QAT），使单个模型能够适应多种量化格式，无需为每种格式单独训练，并提出了Slice-and-Scale转换方法和弹性精度缩放部署方案。


<details>
  <summary>Details</summary>
Motivation: 传统QAT通常针对单一目标数值格式训练，而实际部署中需要根据硬件支持或运行时约束选择不同精度。这导致需要为每种格式单独训练模型，增加了存储和部署成本。

Method: 提出多格式QAT方法，训练单个模型使其对多种量化格式具有鲁棒性；开发Slice-and-Scale转换程序，可将高精度表示转换为低精度格式而无需重新训练；建立完整部署流程：多格式QAT训练→存储单一锚点格式检查点→运行时动态转换到更低精度格式。

Result: 多格式QAT能够在每个目标精度上匹配单格式QAT的性能，单个模型在不同格式（包括训练中未见过的格式）上都能表现良好；Slice-and-Scale转换在MXINT和MXFP格式上有效，运行时转换带来的精度损失可忽略不计。

Conclusion: 多格式QAT结合Slice-and-Scale转换提供了一种实用的弹性精度缩放路径，允许在推理时根据部署目标选择运行时格式，简化了模型部署并提高了灵活性。
...
### [164] [Learning Shared Representations for Multi-Task Linear Bandits](https://arxiv.org/abs/2604.00531)
*Jiabin Lin,Shana Moothedath*

Main category: cs.LG

TL;DR: 本文提出了一种用于线性老虎机多任务表示学习的新方法，通过共享低维潜在表示来提升样本效率和决策性能。


<details>
  <summary>Details</summary>
Motivation: 在多任务线性老虎机问题中，当多个相关任务共享潜在的低维表示时，独立解决每个任务会浪费样本效率。本文旨在利用任务间的相关性，通过共享表示学习来提高决策效率。

Method: 提出了一种基于乐观面对不确定性线性（OFUL）的新算法，包含探索阶段收集数据、通过谱初始化估计共享模型、在新构建的置信集上进行OFUL学习三个步骤。

Result: 理论分析证明了置信集的有效性，未知奖励向量以高概率位于置信集内。累积遗憾界达到Õ(√(drNT))，相比独立解决T个任务的Õ(dT√N)有显著改进。数值模拟验证了算法在不同问题规模下的性能。

Conclusion: 该研究展示了在多任务线性老虎机中利用共享低维表示学习的有效性，显著提高了样本效率，为相关领域提供了理论保证和实用算法。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法通常使用手工设计的无监督目标更新全部参数，忽视了保护共享源知识的可靠性和适应信号的可靠性。大语言模型在现实部署中面临任务演变和持续分布偏移的挑战

Method: 受果蝇分子信号级联启发，提出Synapse Consolidation方法：1) Rac1通路将可塑性限制在尾部梯度子空间，保护源知识；2) MAPK通路使用分层控制器抑制噪声更新并巩固有用适应；3) 引入结构化TTA目标，包含问题理解、过程理解和源域护栏

Result: 在18个NLP数据集和MOA设置中，SyCo持续优于强基线，在未见任务适应上达到78.31%，在未见数据偏移上达到85.37%

Conclusion: SyCo通过受生物学启发的参数高效适应机制，在保护源知识的同时有效应对现实部署中的任务演变和分布偏移，为LLM的测试时适应提供了新思路
...
### [166] [HabitatAgent: An End-to-End Multi-Agent System for Housing Consultation](https://arxiv.org/abs/2604.00556)
...
<details>
  <summary>Details</summary>
Motivation: 住房选择是高风险且基本不可逆的决策问题。现有住房平台和许多基于LLM的助手通常将这一过程简化为排名或推荐，导致推理不透明、多约束处理脆弱、事实性保证有限
...
### [167] [Representation choice shapes the interpretation of protein conformational dynamics](https://arxiv.org/abs/2604.00580)
*Axel Giottonini,Thomas Lemmin*

Main category: cs.LG

TL;DR: 不同蛋白质构象表示方法对分子动力学模拟分析结果有显著影响，没有单一表示能完整描述蛋白质动力学
...
Method: 引入Orientation特征——一种几何基础、旋转感知的蛋白质骨架编码方法，并与常见描述方法进行比较。开发ManiProt库用于高效计算和分析多种蛋白质表示。在三种动力学体系中进行比较：快速折叠蛋白质、大规模结构域运动和蛋白质-蛋白质结合。

Result: 不同表示方法强调构象空间的不同方面，没有单一表示能提供底层动力学的完整图像。表示选择会显著影响构象组织、相似性关系和表观转变的推断结果。

Conclusion: 需要采用比较性、表示感知的框架来解释分子动力学模拟结果，不同表示方法提供互补信息，共同使用可获得更全面的蛋白质动力学理解。
...
### [168] [A Survey of On-Policy Distillation for Large Language Models](https://arxiv.org/abs/2604.00626)
*Mingyang Song,Mao Zheng*

Main category: cs.LG

TL;DR: 这篇综述首次全面梳理了大语言模型中的在线策略蒸馏方法，提出了统一的f-散度框架，并从反馈信号、教师访问和损失粒度三个维度对现有方法进行了系统分类。


<details>
  <summary>Details</summary>
Motivation: 传统的离线策略蒸馏存在训练-测试不匹配问题，导致推理时的预测误差自回归累积。在线策略蒸馏通过让学生模型生成自己的轨迹并接收教师反馈，基于交互模仿学习理论解决这一问题。

Method: 提出了统一的f-散度框架，从三个正交维度组织在线策略蒸馏方法：反馈信号（基于logit、基于结果或自博弈）、教师访问（白盒、黑盒或无教师）和损失粒度（token级、序列级或混合）。

Result: 系统分析了代表性方法，考察了工业部署情况，并识别了蒸馏缩放定律、不确定性感知反馈和智能体级蒸馏等开放性问题。

Conclusion: 在线策略蒸馏通过解决暴露偏差问题，为将前沿大语言模型的专业知识转移到可部署的小型学生模型提供了更有效的框架，但仍面临诸多研究挑战。
...
### [169] [Chameleons do not Forget: Prompt-Based Online Continual Learning for Next Activity Prediction](https://arxiv.org/abs/2604.00653)
*Marwan Hassani,Tamara Verbeek,Sjoerd van Straten*

Main category: cs.LG

TL;DR: 本文提出CNAPwP方法，将DualPrompt算法应用于下一个活动预测，以解决预测过程监控中的灾难性遗忘问题，并在包含重复概念漂移的数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前预测过程监控框架通常假设静态环境，忽略了动态特性和概念漂移，导致灾难性遗忘问题。持续学习可以缓解这一问题，但需要专门针对下一个活动预测任务进行适配。

Method: 提出CNAPwP方法，基于DualPrompt算法进行适配，用于下一个活动预测。引入了包含重复概念漂移的新数据集，并设计了任务特定的遗忘度量指标。

Result: 在三个合成数据集和两个真实世界数据集上进行广泛测试，CNAPwP相比五个基线方法取得了SOTA或具有竞争力的结果，证明了其在真实场景中的潜在适用性。

Conclusion: CNAPwP方法能够有效缓解预测过程监控中的灾难性遗忘问题，提高准确性和适应性，为动态环境下的过程预测提供了有效的解决方案。
...
TL;DR: HiLL框架通过联合训练提示生成策略和推理策略，解决GRPO中的优势崩溃问题，实现自适应和可迁移的提示学习


<details>
  <summary>Details</summary>
Motivation: GRPO在强化学习中存在优势崩溃问题：当组内所有轨迹获得相同奖励时，相对优势为零，导致学习信号缺失。现有方法使用固定提示，无法适应推理器的当前状态，且带提示的成功不一定能迁移到无提示策略

Method: 提出HiLL框架：1）联合训练提示策略和推理策略，提示策略根据推理器的错误轨迹在线生成自适应提示；2）引入提示依赖度衡量正确提示轨迹对提示的依赖程度；3）推导迁移性理论，证明低提示依赖度意味着更好的从提示成功到无提示成功的迁移；4）基于此设计转移加权奖励来训练提示策略

Result: 在多个基准测试中，HiLL一致优于GRPO和现有的基于提示的基线方法，证明了自适应和迁移感知的提示学习对强化学习的价值

Conclusion: HiLL通过联合训练自适应提示生成器和推理器，有效解决了GRPO的优势崩溃问题，同时确保提示学习能够有效迁移到无提示策略，显著提升了强化学习性能
...
TL;DR: 研究大型语言模型训练中的静默数据损坏问题，通过故障注入实验分析影响，并提出轻量级检测与恢复方法


<details>
  <summary>Details</summary>
Motivation: 随着LLM规模和复杂度增加，训练过程中的故障后果日益严重。静默数据损坏（SDC）是硬件引发的故障，可能绕过系统检测机制，导致梯度损坏、损失峰值、发散或训练停滞等问题

Method: 在GPU矩阵乘法指令级别进行定向故障注入，分析不同比特位、核函数和执行阶段的敏感性。基于观察到的损坏特征，提出轻量级检测方法识别有害参数更新，并在检测到问题时重新计算最近训练步骤

Result: 实验表明局部故障可能产生显著影响，包括NaN传播、损失和梯度范数的短期峰值、注意力对数变化以及持久性参数发散。在LLaMA 60M、350M和1.3B参数模型上的实验证明，重新计算最近训练步骤能有效缓解这些事件的影响

Conclusion: 静默数据损坏对LLM预训练有实质性影响，但通过基于损坏特征的轻量级检测和恢复机制可以有效缓解，为大规模模型训练提供了实用的容错解决方案
...
TL;DR: Spectral Compact Training (SCT) 通过将密集权重矩阵替换为截断SVD因子，在训练和推理中都不实例化完整矩阵，实现了高达199倍的内存减少，使70B参数模型能在Steam Deck手持设备上训练。
...
Method: 使用永久截断SVD因子W = U diag(s) V^T替代密集权重矩阵，梯度通过紧凑谱因子反向传播，每次优化器步骤后通过QR分解将U、V回缩到Stiefel流形上。

Result: 在秩32时每MLP层内存减少高达199倍，70B参数架构在Steam Deck上峰值内存仅7.2GB（对比密集FP32训练需1245GB）。秩128是效率最佳点，MLP压缩11.7倍且困惑度最低，秩32时GPU内存下降46%而训练吞吐量翻倍。

Conclusion: SCT方法能显著减少内存占用，使大模型训练在消费级硬件上成为可能，实验表明学习率调度而非MLP秩是收敛的主要瓶颈。
...
TL;DR: BioCOMPASS模型通过整合生物标志物和治疗信息，使用治疗门控和通路一致性损失等组件，提高了免疫治疗反应预测的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 免疫治疗反应预测面临数据集小、癌症类型多样、药物和治疗方案各异的问题，现有模型在未见患者队列上性能下降，需要提高模型的泛化能力。

Method: 扩展COMPASS模型为BioCOMPASS，通过构建损失组件（治疗门控和通路一致性损失）将生物标志物信息与模型中间表示对齐，而不是直接作为输入。

Result: 在Leave-one-cohort-out、Leave-one-cancer-type-out和Leave-one-treatment-out评估策略下，BioCOMPASS显示出更好的泛化性能，优于现有方法。

Conclusion: 利用生物标志物和治疗信息的组件能有效提高免疫治疗反应预测的泛化性，未来研究应继续探索利用临床信息和领域知识的补充组件。
...
TL;DR: 该研究提出了一种基于开放词汇的传感器-语言对齐框架，用于建模可穿戴设备中的开放世界人类活动识别，通过将传感器数据与自然语言描述对齐，取代传统的封闭集分类方法。


<details>
  <summary>Details</summary>
Motivation: 当前可穿戴人类活动识别主要依赖封闭集分类，但现实世界中的活动是开放式的、非脚本化的、个性化的，且通常是组合性的，以叙事形式展开而非固定类别实例。需要从根本上改变可穿戴HAR的建模、监督和评估方式。

Method: 提出三个核心组件：1）自然主义数据收集和标注流程，结合多位置可穿戴传感与自由形式的时间对齐叙事描述；2）基于检索的评估框架，测量传感器数据与语言之间的语义对齐；3）语言条件学习架构，支持可变长度传感器流和异构传感器放置的传感器到文本推理。

Result: 使用固定标签目标训练的模型在现实世界变异性下性能急剧下降，而开放词汇传感器-语言对齐产生鲁棒且语义基础的表征。在跨参与者评估中，该方法达到65.3%的Macro-F1，而强封闭集HAR基线仅为31-34%。

Conclusion: 开放端叙事建模为现实世界可穿戴HAR提供了实用有效的基础，一旦学习到传感器-语言对齐，封闭集活动识别就变成了简单的下游任务。
...
TL;DR: ThoughtSteer攻击通过扰动连续推理语言模型的隐藏状态，在保持正常准确率的同时实现高成功率攻击，且能逃避现有防御措施


<details>
  <summary>Details</summary>
Motivation: 新一代语言模型完全在连续隐藏状态中进行推理，不产生令牌且没有审计追踪，这种沉默特性创造了全新的攻击面

Method: ThoughtSteer在输入层扰动单个嵌入向量，利用模型自身的多轮推理将扰动放大为被劫持的潜在轨迹，可靠产生攻击者选择的答案

Result: 在两个架构、三个推理基准测试和124M到3B参数规模的模型中，攻击成功率≥99%，保持接近基线的干净准确率，能转移到未见过的基准测试(94-100%)，逃避所有五种主动防御，并能经受25个epoch的微调

Conclusion: 研究发现潜在空间中的神经坍缩现象将触发的表示拉向紧密的几何吸引子，解释了防御失败的原因，同时揭示了后门攻击必须留下线性可分特征；有趣的是，单个潜在向量仍编码正确答案，但模型输出错误答案，表明对抗信息不在单个向量中而在集体轨迹中
...
### [176] [Scalable Pretraining of Large Mixture of Experts Language Models on Aurora Super Computer](https://arxiv.org/abs/2604.00785)
...
<details>
  <summary>Details</summary>
Motivation: 大规模语言模型预训练需要巨大的计算资源，研究旨在利用Aurora超级计算机的127,488个Intel PVC GPU tile，展示在数千个GPU tile规模上的LLM预训练能力。

Method: 开发了Optimus训练库，支持标准大模型训练技术；使用自定义GPU内核优化MoE模型的专家计算；提出新颖的EP-Aware分片优化器；实现了可靠性和容错功能以提高训练稳定性。

Result: 成功预训练了Mula-1B密集模型和多个MoE模型（7B、20B、100B、220B参数）；在最大模型Mula-220B-A10B上，从384扩展到12288个GPU tile时观察到约90%的扩展效率；通过优化实现了高达1.71倍的训练加速。

Conclusion: 研究证明了在Aurora超级计算机上大规模预训练LLM的可行性，开发的Optimus库和优化技术显著提升了训练效率和稳定性，为未来更大规模的语言模型训练奠定了基础。
...
### [177] [MIRANDA: MId-feature RANk-adversarial Domain Adaptation toward climate change-robust ecological forecasting with deep learning](https://arxiv.org/abs/2604.00800)
*Yuchang Jiang,Jan Dirk Wegner,Vivien Sainte Fare Garnot*

Main category: cs.LG

TL;DR: 提出MIRANDA方法，通过中间特征对抗正则化和基于排序的域适应目标，提升深度学习在气候变化导致的分布偏移下的植物物候预测鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习在植物物候预测中面临气候变化导致的分布偏移问题，而现有域适应方法难以处理时间连续域变化和协变量-标签双重偏移的挑战。

Method: 提出MIRANDA方法：1）在中间特征而非最终表示上应用对抗正则化；2）使用基于排序的域分类目标来强制学习气象表示的年不变性。

Result: 在包含70年、67,800个观测的全国尺度数据集上，MIRANDA相比传统域适应方法显著提升了对气候分布偏移的鲁棒性，缩小了与机理模型的性能差距。

Conclusion: MIRANDA通过创新的中间特征对抗正则化和排序目标，有效解决了气候变化导致的分布偏移问题，为深度学习在物候预测中的应用提供了更鲁棒的解决方案。
...
### [178] [Routing-Free Mixture-of-Experts](https://arxiv.org/abs/2604.00801)
*Yilun Liu,Jinru Han,Sikuan Yan,Volker Tresp,Yunpu Ma*

Main category: cs.LG

TL;DR: 提出Routing-Free MoE，消除集中式路由机制，让专家自主决定激活，通过自适应负载均衡框架优化资源分配，实验显示优于基线模型
...
Method: 1. 消除硬编码的集中式设计（外部路由器、Softmax、Top-K、负载均衡）；2. 将所有激活功能封装在单个专家内部；3. 通过连续梯度流直接优化；4. 引入统一的自适应负载均衡框架，通过可配置插值同时优化专家平衡和令牌平衡目标

Result: 实验表明Routing-Free MoE能够持续超越基线模型，具有更好的可扩展性和鲁棒性

Conclusion: Routing-Free MoE为MoE设计提供了新思路，通过消除集中式路由约束，让专家自主决策，同时通过自适应负载均衡实现灵活的资源分配，为未来MoE设计和优化提供了有价值的见解
...
### [179] [Cost-Penalized Fitness in FMA-Orchestrated Mixture of Experts: Experimental Evidence for Molecular Memory in Domain Adaptation](https://arxiv.org/abs/2604.00812)
*Martin Jaraiz*

Main category: cs.LG

TL;DR: nanoFMT是一种具有动态专家混合管理的自由市场算法变压器，实验显示通过成本惩罚适应度指标和新生儿专家宽限期，系统通过多样化而非替换积累领域专业知识，在领域切换时实现9-11倍更快的恢复速度，无需专家出生或替换。


<details>
  <summary>Details</summary>
Motivation: 解决高级LLM开发中的一个基本问题：当专家混合系统在变化的数据分布下满负荷运行时，应如何管理其专家池？当前MoE管理方法缺乏在领域变化时保持和重新激活专业知识的能力。

Method: 使用成本惩罚适应度指标结合新生儿专家的线性宽限期，创建了一个通过多样化而非替换积累领域专业知识的系统。采用自由市场算法协调的变压器，通过七次受控运行进行实验验证。

Result: 在往返领域切换实验中，返回先前学习领域时的恢复速度提高了9-11倍，且无需专家出生或替换。系统表现出"分子记忆"效应，休眠专家在领域回归时存活并重新激活。初步成本分析显示，在中等规模场景下，OpenAI规模的提供商每年可节省3910万美元并减少27.1GWh能源消耗。

Conclusion: nanoFMT的MoE管理方法通过成本惩罚适应度指标和新生儿专家宽限期，实现了通过多样化积累专业知识而非替换，创造了"分子记忆"效应，在领域切换时显著提高恢复效率，具有显著的成本和能源节约潜力。
...
### [180] [Optimal Brain Decomposition for Accurate LLM Low-Rank Approximation](https://arxiv.org/abs/2604.00821)
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型低秩分解方法主要基于激活白化空间的奇异值分解，这种方法只考虑了输入信息，忽略了输出信息，导致分解结果不够理想。

Method: 提出OBD-LLM方法，利用Hessian矩阵的二阶信息，通过Kronecker分解Hessian，考虑层的输入和输出信息，采用双向白化权重矩阵，形成闭式解的最优分解方案。

Result: OBD-LLM在语言模型权重分解方面显著优于现有方法，比之前的SOTA方法SVD-LLM提升了约20-40%的性能。

Conclusion: 通过引入二阶Hessian信息和双向白化，OBD-LLM为语言模型权重分解提供了理论最优的闭式解，显著提升了低秩分解的效果。
...
### [181] [Super-Resolving Coarse-Resolution Weather Forecasts With Flow Matching](https://arxiv.org/abs/2604.00897)
*Aymeric Delefosse,Anastase Charantonis,Dominique Béréziat*

Main category: cs.LG

TL;DR: 提出模块化框架，将天气预报与空间分辨率解耦，通过学习生成式超分辨率作为粗分辨率预报轨迹的后处理步骤，在保持大尺度结构的同时重建未解析的变异性。


<details>
  <summary>Details</summary>
Motivation: 机器学习天气预报模型虽然超越传统数值天气预报系统，但在高空间分辨率下的训练和运行计算成本高昂。需要一种方法既能获得高分辨率预报，又能降低计算成本。

Method: 采用模块化框架，将预报与分辨率解耦：先进行粗分辨率预报，然后通过学习的生成式超分辨率作为后处理步骤。将超分辨率建模为随机逆问题，使用残差公式保持大尺度结构，重建未解析变异性。模型基于再分析数据通过流匹配训练，应用于全球中期预报。

Result: 超分辨率在重新粗化后能保持大尺度结构和方差，引入物理一致的小尺度变异性，在0.25°分辨率下相对于业务集合预报基线达到有竞争力的概率预报技能，同时相比端到端高分辨率预报仅需适度的额外训练成本。

Conclusion: 提出的模块化框架通过将预报与分辨率解耦，实现了计算效率与预报质量的平衡，为高分辨率天气预报提供了一种经济有效的解决方案。
...
### [182] [Fatigue-Aware Learning to Defer via Constrained Optimisation](https://arxiv.org/abs/2604.00904)
*Zheng Zhang,Cuong C. Nguyen,David Rosewarne,Kevin Wells,Gustavo Carneiro*

Main category: cs.LG

TL;DR: 提出FALCON方法，通过建模人类疲劳动态来优化人机协作中的延迟决策，在多种数据集上优于现有方法
...
Method: 提出FALCON方法，使用心理学基础的疲劳曲线建模工作负载变化的人类性能，将L2D问题形式化为约束马尔可夫决策过程，状态包括任务特征和累积人类工作负载，通过PPO-Lagrangian训练在人类-AI协作预算下优化准确性

Result: FALCON在多个数据集上始终优于最先进的L2D方法，能够零样本泛化到具有不同疲劳模式的未见专家，并展示了当覆盖率严格在0到1之间时，自适应人机协作相对于纯AI或纯人类决策的优势

Conclusion: 通过显式建模人类疲劳动态，FALCON方法能够实现更有效的人机协作决策，为实际应用中的自适应人机协作系统提供了重要改进
...
TL;DR: 本文提出了LT-O-Learners（长期正交学习器），用于估计异质性长期处理效应，通过引入自定义重叠权重来解决处理或长期观测重叠不足的问题，提高估计的稳定性。


<details>
  <summary>Details</summary>
Motivation: 异质性长期处理效应估计在个性化决策中广泛应用，但面临挑战：由于某些子群体中处理或长期结果观测的重叠有限，导致估计不稳定、方差大。需要一种能在低重叠设置下稳健估计的方法。

Method: 提出LT-O-Learners，通过引入自定义重叠权重重新定位学习目标，降低处理或长期观测重叠低的样本权重。该方法具有Neyman正交性，对干扰参数估计误差具有鲁棒性，且模型无关，可与任意机器学习模型结合。

Result: 理论分析表明重新定位的损失等价于加权oracle损失，满足Neyman正交性，提供了LT-O-Learners的一般误差界，给出了达到准oracle速率的条件。实证评估在合成和半合成基准上验证了理论特性，特别是在低重叠设置下的鲁棒性。

Conclusion: LT-O-Learners是首个针对异质性长期处理效应估计的正交学习器，能够有效处理长期结果中常见的低重叠问题，为个性化决策提供更稳定的估计方法。
...
### [184] [Differentially Private Manifold Denoising](https://arxiv.org/abs/2604.00942)
*Jiaqi Wu,Yiqing Sun,Zhigang Yao*

Main category: cs.LG

TL;DR: 提出一种差分隐私流形去噪框架，利用敏感参考数据集校正噪声查询点而不泄露隐私，通过迭代估计局部几何结构并提供形式化隐私保证。


<details>
  <summary>Details</summary>
Motivation: 在受监管环境中，需要利用敏感参考数据集进行流形去噪，但传统方法会泄露隐私。本文旨在开发一个既能保护参考数据隐私，又能有效校正噪声查询点的框架。

Method: 采用迭代三步法：(1) 在校准敏感度下使用参考数据私有估计局部均值和切向几何；(2) 沿私有估计子空间将查询点向局部均值投影；(3) 使用(ε,δ)-差分隐私进行严格的跨迭代和查询隐私核算。

Result: 在流形正则性、采样密度和测量噪声的标准假设下，建立了高概率效用保证，显示校正后的查询点以由样本大小、噪声水平、带宽和隐私预算决定的非渐近速率收敛到流形。
...
<details>
  <summary>Details</summary>
Motivation: 传统RL算法通常使用对角高斯分布参数化策略，这限制了策略捕捉多模态分布的能力，在多解问题中难以覆盖所有最优解。同时，回报被简化为均值，失去了多模态特性，无法为策略更新提供充分指导。

Method: 提出FP-DRL算法：1) 使用流匹配建模策略，既计算高效又能拟合复杂分布；2) 采用分布强化学习方法建模和优化整个回报分布，更有效地指导多模态策略更新。

Result: 在MuJoCo基准测试中，FP-DRL算法在大多数控制任务中实现了最先进的性能，同时展示了流策略的优越表示能力。

Conclusion: FP-DRL算法通过结合流匹配策略和分布强化学习，有效解决了传统RL在多模态问题中的局限性，提升了智能体性能。
...
### [186] [Model-Based Learning of Near-Optimal Finite-Window Policies in POMDPs](https://arxiv.org/abs/2604.01024)
*Philip Jordan,Maryam Kamgarpour*

Main category: cs.LG

TL;DR: 该论文研究在表格化部分可观测马尔可夫决策过程中基于模型的有限窗口策略学习，提出了从单轨迹估计超状态MDP模型的方法，并分析了其样本复杂度。


<details>
  <summary>Details</summary>
Motivation: 在部分可观测环境下学习时，通常使用有限动作-观测窗口来近似无界历史依赖，这诱导出基于历史的超状态MDP。然而，从原始POMDP交互生成的轨迹与目标模型之间存在采样不匹配问题，因此需要样本高效的模型估计方法。

Method: 提出了表格化POMDP的模型估计程序，利用滤波器稳定性与弱依赖随机变量集中不等式之间的联系，分析单轨迹估计超状态MDP模型的样本复杂度。

Result: 获得了从单轨迹估计超状态MDP模型的紧致样本复杂度保证，结合值迭代算法，可以为POMDP生成近似最优的有限窗口策略。

Conclusion: 该方法通过理论分析证明了在部分可观测环境下，能够从有限数据中有效学习有限窗口策略，为POMDP中的模型学习提供了理论保证。
...
### [187] [Fast and Accurate Probing of In-Training LLMs' Downstream Performances](https://arxiv.org/abs/2604.01025)
...
<details>
  <summary>Details</summary>
Motivation: 传统的大语言模型生成式评估方法计算成本过高，导致训练过程中的下游性能评估延迟难以承受，而简单的训练损失指标又与下游性能不总是相关

Method: 使用轻量级探针，以LLM训练检查点的内部表示为输入，直接预测检查点在下游任务上的性能（通过成功率pass@1衡量），设计了多种探针架构

Result: 探针能准确预测检查点性能（平均AUROC>0.75），在不同检查点间具有良好的泛化能力（早期预测后期），将计算延迟从约1小时降低到约3分钟
...
### [188] [Toward Personalized Darts Training: A Data-Driven Framework Based on Skeleton-Based Biomechanical Analysis and Motion Modeling](https://arxiv.org/abs/2604.01130)
...
<details>
  <summary>Details</summary>
Motivation: 传统飞镖教练主要依赖经验和视觉观察，难以满足高精度、目标导向运动的需求。现有量化方法多关注局部变量、单次释放指标或静态模板匹配，对个性化训练支持有限且常忽略有用的运动变异性。
...
### [189] [Property-Level Flood Risk Assessment Using AI-Enabled Street-View Lowest Floor Elevation Extraction and ML Imputation Across Texas](https://arxiv.org/abs/2604.01153)
*Xiangpeng Li,Yu-Hsuan Ho,Sam D Brody,Ali Mostafavi*

Main category: cs.LG

TL;DR: 该研究开发了一个三阶段流程，利用街景图像和机器学习填补技术，为洪水风险评估生成区域尺度的建筑特定高程数据。
...
Method: 三阶段流程：1) 使用Elev-Vision框架从Google街景图像提取最低楼层高程和街道与最低楼层高度差；2) 用随机森林和梯度提升模型填补缺失的高度差值；3) 将高程数据与洪水淹没面和深度-损失函数结合，估算建筑内部洪水深度和预期损失。

Result: 在12,241个住宅结构中，73.4%的地块有街景图像，49.0%成功直接提取高程数据。在13个研究区域中，填补模型R平方值从0.159到0.974不等，5个区域因性能不足被排除。该方法可显著改善区域洪水风险表征。

Conclusion: 街景图像高程测绘虽非对所有房产都适用，但具有足够可扩展性，能从灾害暴露评估推进到建筑级内部淹没和预期损失估算，实质性改善区域洪水风险表征。
...
TL;DR: ORCA是一个在线推理校准框架，通过元学习更新每个输入的校准模块，提供有效的置信度估计，显著提高大语言模型在推理任务中的计算效率。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在测试时缩放虽然能解决高难度任务，但需要极高的计算成本。这种低效性源于后训练语言模型的校准不足以及流行采样技术缺乏校准。需要一种能在分布偏移下提供有效置信度估计的校准方法。

Method: ORCA结合了保形预测和测试时训练，引入元学习过程为每个输入更新校准模块。该方法能在分布偏移（如不同推理阶段的思想模式、模型开发与部署间的提示分布变化）下提供有效的置信度估计。

Result: 在风险水平δ=0.1时，ORCA在分布内任务上提高Qwen2.5-32B效率，使用监督标签节省47.5%，使用自洽标签节省40.7%。在零样本分布外设置下，将MATH-500的节省从静态校准基线的24.8%提高到67.0%，同时保持低经验错误率，这一趋势在不同模型系列和下游基准测试中保持一致。

Conclusion: ORCA不仅提供了保形风险的理论保证，还实证展示了在不同推理任务中更高的效率和泛化能力。该框架能显著降低大语言模型推理的计算成本，同时保持性能。
...
### [191] [Reasoning Shift: How Context Silently Shortens LLM Reasoning](https://arxiv.org/abs/2604.01161)
*Gleb Rodionov*

Main category: cs.LG

TL;DR: 研究发现推理模型在不同上下文条件下会产生更短的推理轨迹（最多缩短50%），这种压缩与自我验证和不确定性管理行为的减少相关，可能影响复杂任务表现。


<details>
  <summary>Details</summary>
Motivation: 尽管展示测试时扩展行为（如扩展推理轨迹和自我验证）的大语言模型在复杂长期推理任务上表现出色，但这些推理行为的鲁棒性尚未得到充分探索。

Method: 在三种场景下对多个推理模型进行系统评估：1）添加冗长无关上下文的问题；2）包含独立任务的多轮对话设置；3）作为复杂任务子任务呈现的问题。

Result: 观察到推理模型在不同上下文条件下对相同问题产生的推理轨迹比孤立呈现时短得多（最多缩短50%）。细粒度分析显示这种压缩与自我验证和不确定性管理行为（如双重检查）的减少相关。

Conclusion: 虽然这种行为转变不影响简单问题表现，但可能影响更具挑战性任务的性能。研究呼吁关注推理模型的鲁棒性以及LLM和基于LLM的智能体的上下文管理问题。
...
TL;DR: Multiscreen架构通过引入screening机制解决标准softmax注意力缺乏绝对查询-键相关性的问题，使用显式阈值筛选相关键而非全局竞争，在减少参数、提高训练稳定性、长上下文性能和推理速度方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 标准softmax注意力的核心限制在于缺乏绝对查询-键相关性概念：注意力权重是通过在所有键之间重新分配固定单位质量获得的，相关性仅相对于竞争键定义，无法显式拒绝不相关键。

Method: 提出Multiscreen语言模型架构，引入screening机制。该机制通过显式阈值评估每个键，丢弃不相关键并聚合剩余键，从而消除键之间的全局竞争。

Result: Multiscreen在验证损失相当的情况下比Transformer基线减少约40%参数；支持更大学习率的稳定优化；在长上下文困惑度上保持强性能；在远超训练上下文长度时检索性能几乎不下降；在100K上下文长度下推理延迟降低达3.2倍。

Conclusion: Multiscreen通过screening机制解决了标准注意力缺乏绝对相关性的问题，在模型效率、训练稳定性、长上下文性能和推理速度方面均有显著改进，为语言模型架构提供了有前景的替代方案。
...
### [193] [Bridging the Simulation-to-Experiment Gap with Generative Models using Adversarial Distribution Alignment](https://arxiv.org/abs/2604.01169)
...
<details>
  <summary>Details</summary>
Motivation: 科学和工程中存在仿真与实验的差距：物理定律难以精确求解复杂系统，而仿真包含计算近似，实验数据虽真实但只能部分反映系统状态

Method: 提出数据驱动的分布对齐框架，先在完全观测但存在缺陷的仿真数据上预训练生成模型，然后与部分但真实的实验观测数据对齐。具体引入对抗性分布对齐(ADA)方法，对齐原子位置的生成模型（初始训练于模拟玻尔兹曼分布）与实验观测分布

Result: 理论上证明方法能恢复目标观测分布，即使存在多个潜在相关的观测变量。在合成数据、分子数据和实验蛋白质数据上验证了框架有效性，表明能对齐具有多样化观测的生成模型

Conclusion: 提出的对抗性分布对齐框架能有效弥合仿真与实验之间的差距，为物理科学等领域提供了一种将模拟生成模型与真实实验观测对齐的通用方法
...
### [194] [The Recipe Matters More Than the Kitchen:Mathematical Foundations of the AI Weather Prediction Pipeline](https://arxiv.org/abs/2604.01215)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI天气预报发展迅速，但缺乏统一的数学框架来解释预测能力的关键因素。现有理论只关注特定架构选择，而2023-2026年的实际证据表明训练方法、损失函数设计和数据多样性至少与架构选择同等重要。

Method: 理论方面：基于球面近似理论、动力系统理论、信息论和统计学习理论构建完整学习管道框架；建立学习管道误差分解；开发损失函数谱理论；推导出分布外外推边界。实证方面：使用NVIDIA Earth2Studio和ERA5初始条件，在10个架构多样的AI天气模型上进行推理，评估6个指标跨越30个不同季节的初始化日期。

Result: 证实了MSE训练模型在高波数处的普遍谱能量损失；误差共识比率上升表明大部分预测误差在不同架构间共享；极端事件期间存在线性负偏差；提出了整体模型评估分数进行统一的多维评估。

Conclusion: 该研究提供了一个统一的数学框架来分析AI天气预报系统的预测能力，证明估计误差（损失和数据相关）在当前规模下主导近似误差（架构相关），并提供了处方性框架，可在训练前对提议的管道进行数学评估。
...
### [195] [LAtent Phase Inference from Short time sequences using SHallow REcurrent Decoders (LAPIS-SHRED)](https://arxiv.org/abs/2604.01216)
...
<details>
  <summary>Details</summary>
Motivation: 从稀疏的时空观测中重建完整的时空动力学是复杂系统的核心挑战，这对于机理理解、模型校准和操作决策至关重要

Method: 采用三阶段流水线：1) SHRED模型在模拟数据上预训练，将传感器时间历史映射到结构化潜在空间；2) 时间序列模型学习在潜在空间中向前或向后传播状态以覆盖未观测时间区域；3) 部署时仅需真实系统的短观测窗口，通过冻结的SHRED模型和时间模型联合重建或预测完整时空轨迹

Result: 在六个复杂时空物理实验中验证了有效性，包括湍流、多尺度推进物理、挥发性燃烧瞬变和卫星环境场等场景

Conclusion: LAPIS-SHRED提供了一个轻量级、模块化的架构，适用于观测受物理或后勤限制的操作环境，支持双向推理并继承了数据同化和多尺度重建能力
