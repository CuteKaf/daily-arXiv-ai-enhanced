...
TL;DR: 本文展示了如何使用持久同调（PH）这一拓扑数据分析方法，客观且可解释地分析艺术风格，能够统计显著地区分不同艺术家、同一艺术流派内的艺术家，以及区分艺术家作品与AI生成的风格模仿图像。


<details>
  <summary>Details</summary>
Motivation: 传统艺术风格识别依赖艺术史学家或评论家的视觉直觉和经验，缺乏客观性。本文旨在利用数学工具为艺术分析提供更结构化的视角。

Method: 使用持久同调（PH）这一拓扑数据分析方法，通过分析艺术作品中的拓扑特征来识别艺术风格。

Result: PH方法能够以统计确定性区分不同艺术家、同一艺术流派内的艺术家，并能有效区分艺术家原作与AI生成的风格模仿图像。

Conclusion: 持久同调为艺术风格分析提供了客观、可解释的数学工具，能够有效识别和区分不同艺术家的风格特征。
...
TL;DR: 提出了一种基于StyleGAN2的方法来生成猕猴面部表情，通过数据增强、样本选择和损失函数优化解决了训练数据有限的问题，能够生成多样化的面部表情并用于风格化图像编辑。
...
Method: 使用StyleGAN2模型，实施：1）通过运动转移合成新面部表情图像的数据增强；2）基于初始训练的StyleGAN2模型的潜在表示进行样本选择；3）损失函数优化以确保细微运动（如眼球运动）的准确再现。

Result: 该方法能够为多个猕猴个体生成多样化的面部表情，性能优于仅使用原始静态图像训练的模型。模型还适用于风格化图像编辑，特定风格参数对应不同的面部运动。
...
TL;DR: 本文提出了PairHuman数据集，这是首个专为双人肖像生成设计的大规模基准数据集，包含超过10万张图像，涵盖多种场景、服装和互动。同时提出了DHumanDiff基线方法，在双人肖像生成中增强面部一致性并平衡个性化生成与语义驱动场景创建。
...
Method: 构建PairHuman数据集，包含10万+图像及丰富元数据；提出DHumanDiff基线方法，专门针对双人肖像生成，增强面部一致性并平衡个性化生成与场景创建。
...
<details>
  <summary>Details</summary>
Motivation: 中子成像在惯性约束聚变(ICF)事件分析中至关重要，但图像常被高斯和泊松混合噪声所退化。传统滤波和阈值方法难以有效处理这种重叠噪声，需要开发能保持图像保真度的去噪技术。
...
Conclusion: 这种方法为中子图像噪声减少和ICF实验的三维重建分析提供了有前景的进展，展示了机器学习在融合成像领域的应用潜力。
...
TL;DR: 本文提出了一种基于相关性反馈的视觉语言模型检索增强机制，包括伪相关性反馈、生成式相关性反馈、注意力反馈总结器和显式反馈四种策略，实验表明这些方法能显著提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然支持自然语言视觉搜索，但提升性能通常需要微调或使用更大模型。本文受传统文本搜索启发，提出在推理时使用相关性反馈机制来改进检索性能，这种方法既可作为微调的替代方案，也可与微调后的模型结合使用。

Method: 提出了四种相关性反馈策略：1）经典伪相关性反馈（PRF），基于排名靠前的结果优化查询嵌入；2）生成式相关性反馈（GRF），使用合成标题进行查询优化；3）注意力反馈总结器（AFS），基于Transformer的多模态细粒度特征整合模型；4）使用真实标题的显式反馈作为上限基准。

Result: 在Flickr30k和COCO数据集上的实验显示，GRF、AFS和显式反馈相比无反馈检索，在MRR@5指标上为较小VLM带来3-5%的提升，为较大VLM带来1-3%的提升。AFS与显式反馈类似，能缓解查询漂移问题，在多轮迭代检索中比GRF更稳健。
...
TL;DR: 本文提出SaFeR-CLIP框架，通过将不安全概念重定向到语义上最接近的安全替代概念，在保持鲁棒安全性的同时，比现有方法恢复高达8.0%的零样本准确率。


<details>
  <summary>Details</summary>
Motivation: 发现视觉语言模型如CLIP通过微调提高安全性时，往往导致泛化性能显著下降，这种权衡源于强制对齐策略破坏了模型的语义结构。

Method: 提出邻近感知方法，将不安全概念重定向到语义上最接近的安全替代概念，最小化表示变化，并引入SaFeR-CLIP微调框架。

Result: SaFeR-CLIP成功调和了安全性和性能，在保持鲁棒安全性的同时，比现有方法恢复高达8.0%的零样本准确率。
...
Method: 1) 将光栅化输入提升为3D表示并渲染多视图图像；2) 扩展SAM2的时间记忆机制到空间域，构建空间记忆库建立跨视图部件级对应关系；3) 在光栅到矢量转换中执行路径整合和结构优化。
...
TL;DR: Mesh RAG是一个无需训练、即插即用的框架，通过检索增强生成来改进自回归3D网格生成模型，解决了传统方法在质量、速度和可编辑性方面的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统3D网格手动创建耗时且难以扩展，现有自回归模型存在质量与速度的权衡问题，严格的序列依赖限制了生成效率和增量编辑能力。
...
Conclusion: Mesh RAG框架有效克服了自回归网格生成模型的局限性，在多个基础模型上展现出广泛的适用性，为3D资产创建提供了更高效的解决方案。
...
TL;DR: WorldGen是一个从文本提示自动创建大规模交互式3D世界的系统，结合LLM场景布局推理、程序生成和扩散式3D生成技术，无需手动建模或3D专业知识即可生成可导航的虚拟环境。


<details>
  <summary>Details</summary>
Motivation: 解决创意意图与功能性虚拟空间之间的差距，让创作者无需手动建模或专业3D技能就能设计连贯、可导航的世界，推动3D生成AI在游戏、模拟和沉浸式社交环境中的应用。

Method: 结合LLM驱动的场景布局推理、程序生成、基于扩散的3D生成和对象感知场景分解，将自然语言描述转换为可遍历的完全纹理化环境，支持在标准游戏引擎中立即探索或编辑。

Result: 系统完全模块化，支持对布局、规模和风格的细粒度控制，生成几何一致、视觉丰富且实时渲染高效的世界，实现了大规模可访问的生成式世界构建。

Conclusion: 这项工作代表了向大规模可访问生成式世界构建迈出的一步，推进了3D生成AI在游戏、模拟和沉浸式社交环境中的应用前沿。
...
TL;DR: REO-Instruct是首个面向地球观测的统一基准，结合了描述性和回归任务，特别关注森林生态场景，建立了从人类活动到地上生物量的认知逻辑链。
...
Method: 通过混合人机流程生成和验证结构化文本标注，整合了共配准的Sentinel-2和ALOS-2影像数据，建立了包含人类活动、土地覆盖分类、生态斑块计数和地上生物量回归的认知逻辑链。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在空间推理基准测试中表现优异，但忽视了真实应用所需的精细空间理解能力，包括精确3D定位、物体物理兼容性、物体可供性和多步骤空间规划。

Method: 利用Benchmark for Object Pose Estimation (BOP)数据集中的6D物体姿态，生成精细注释，包括抓取姿态、参考物体姿态、路径规划轨迹、相对空间和深度关系以及物体间关系。

Result: 在BOP-ASK上训练的模型优于基线模型，并展现出精确物体和抓取姿态估计、轨迹规划以及在杂乱环境中进行精细物体中心空间推理等新兴能力。

Conclusion: BOP-ASK为训练和评估视觉语言模型提供了丰富资源，推动了模型在精细物体交互推理方面的发展，并公开了数据集和生成流程。
...
TL;DR: 本文提出了一种名为Parts-Mamba的混合GCN-Mamba模型，用于解决骨架动作识别中由于遮挡或通信质量问题导致的骨架不完整问题，通过在NTU RGB+D数据集上的实验验证，该模型在准确率上提升了12.9%。


<details>
  <summary>Details</summary>
Motivation: 在真实场景中，由于人体部分遮挡或通信质量差，捕获的骨架往往不完整或存在缺失帧，现有GCN模型在这种非理想条件下性能较差，主要原因是缺乏局部上下文信息。

Method: 提出Parts-Mamba混合模型，通过部分特定扫描特征捕获部分特定信息，并通过部分-身体融合模块保留非相邻关节的上下文信息。

Result: 在NTU RGB+D 60和NTU RGB+D 120数据集的不同遮挡设置下进行评估，准确率最高提升12.9%。

Conclusion: Parts-Mamba模型能够有效增强捕获和维持远距离关节上下文信息的能力，在骨架不完整的情况下显著提升动作识别性能。
...
TL;DR: 本文提出了联合Gromov-Wasserstein（JGW）目标，扩展了传统的GW距离框架，支持多对多对象匹配，在部分同构分布识别和计算效率方面表现出色。
...
Method: 通过引入JGW目标，扩展GW框架以支持对象集合的同步匹配，采用最优传输中的传统算法（包括熵正则化）来求解点云对象表示。

Result: 在部分匹配的基准测试中，该方法在准确性和计算效率方面优于其他GW变体，在合成和真实数据集上的实验证明了其在多形状匹配中的有效性。

Conclusion: JGW方法为解决计算机图形学和结构生物学等不同领域的复杂匹配问题提供了有前景的应用前景。
...
TL;DR: 该论文提出了一种称为表示对齐（REPA）的方法，通过将扩散或流式生成模型的内部表示与预训练自监督视觉编码器（如DINOv2）对齐，来改进逆问题的重建质量。


<details>
  <summary>Details</summary>
Motivation: 在逆问题中，预训练的生成模型被用作先验，但缺乏真实信号指导重建过程。作者希望利用表示对齐作为归纳偏置，提升重建的保真度和感知真实性。

Method: 在推理时应用REPA，将扩散或流式模型与预训练自监督编码器的表示对齐，即使没有真实目标信号，也能通过近似目标特征来指导重建。

Result: 实验表明REPA在超分辨率、框内修复、高斯去模糊和运动去模糊等任务中一致提升重建质量，同时通过减少离散化步骤显著提高效率。

Conclusion: REPA为逆问题求解提供了有效的表示对齐机制，既能改善感知保真度，又能提升计算效率，具有广泛的适用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有玻璃表面检测方法依赖边界线索或反射线索，但未能充分利用玻璃本身的固有特性。作者观察到玻璃表面反射会因光照强度差异而变化，利用这一现象可更准确地定位玻璃表面。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频理解任务中进展迅速，但主要关注简单视频场景，无法反映真实世界音频-视觉事件的复杂多样性。

Method: 设计了基于LLM的关键对象提取、自动空间标注和人工质量检查的流水线构建数据集；提出了AVST-Zero模型，使用强化学习方法避免中间监督，通过精心设计的多维度奖励直接优化行为。

Result: 构建了包含5K个未剪辑视频、27K个对象、覆盖100种音频-视觉事件的R-AVST数据集；生成了8K多个高质量、均匀分布的问题-答案对；AVST-Zero在实验中表现出与现有模型竞争的性能。
...
TL;DR: 本文提出了一种名为Warm Diffusion的统一框架，结合了热扩散（仅使用噪声）和冷扩散（仅使用模糊）的优点，通过模糊-噪声混合扩散模型来控制模糊和噪声的联合作用。


<details>
  <summary>Details</summary>
Motivation: 热扩散未能利用图像高频细节与低频结构之间的强相关性，导致生成早期步骤中的随机行为；而冷扩散虽然利用了图像相关性进行预测，但忽略了噪声在塑造数据流形中的作用，导致流形外问题和性能下降。

Method: 提出了Warm Diffusion（BNMD模型），采用分治策略利用图像中的频谱依赖性，通过分离去噪和去模糊过程来简化分数模型估计，并使用频谱分析研究模糊-噪声比（BNR）的权衡。
...
TL;DR: Q-Real是一个用于AI生成图像细粒度评估的数据集，专注于真实性和合理性两个关键维度，包含3,088张图像及其标注，用于评估多模态大语言模型的判断和推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有质量评估方法通常只提供单一质量分数，过于粗糙，无法为生成模型的改进提供针对性指导。真实性和合理性是AI生成图像的两个关键评估维度，需要细粒度评估来有效提升生成性能。

Method: 构建Q-Real数据集，包含3,088张由流行文本到图像模型生成的图像，标注主要实体的位置，并提供真实性和合理性维度的判断问题和归因描述。建立Q-Real Bench基准测试，评估多模态大语言模型在判断和推理任务上的表现，并设计微调框架。

Result: 实验结果表明数据集质量高且意义重大，基准测试全面有效，通过微调可以增强多模态大语言模型的能力。

Conclusion: Q-Real数据集和基准测试为AI生成图像的细粒度评估提供了有效工具，有助于提升生成模型的性能，数据集和代码将在发表后发布。
...
<details>
  <summary>Details</summary>
Motivation: 现有的异常生成方法通常需要少量异常样本进行微调，这与异常稀缺的实际情况相矛盾，且容易过拟合类别先验。本文旨在解决无需真实异常样本或训练的场景。

Method: 提出Delta-Denoising方法：1）使用最小提示对驱动两个共享调度的扩散分支进行对比；2）通过累积每步去噪差异生成图像特定的定位图；3）使用该掩码指导潜在修复，在保留上下文的同时生成局部缺陷；4）进行词元级提示优化和对齐异常词元的空间注意力偏置。

Result: 在公共数据集上的实验表明，DeltaDeno在生成质量、真实性和下游检测性能方面均取得了显著提升。

Conclusion: DeltaDeno提供了一种无需训练、零样本的异常生成解决方案，能够有效生成逼真的局部缺陷并提升下游检测性能。
...
TL;DR: 本文提出了DGAF-VSR方法，通过光学引导变形模块和特征域时序条件模块，在扩散模型视频超分辨率中解决了误差累积、空间伪影和感知质量与保真度权衡的问题。
...
Conclusion: 特征域比像素域更适合信息补偿，在放大分辨率下进行变形能更好地保留高频信息，DGAF-VSR方法在视频超分辨率的关键方面取得了显著提升。
...
Method: 采用目标牙齿中心点提示的多标签学习策略建模牙齿间语义关系，结合牙齿形状感知学习机制强制执行形态学约束，通过多任务学习统一优化分割和形状保持。
...
Result: 在复杂现实场景中性能平均下降10.4%，特别是在小/遮挡对象和复杂空间关系上表现不佳。PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试上均取得一致增益。

Conclusion: OmniGround基准测试揭示了当前STVG模型的局限性，PG-TAF框架通过分解任务有效提升了性能，为时空视频定位提供了新的解决方案。
...
Method: 提出隐私感知与推理框架，构建包含合成个体档案的双语多模态数据集，设计了9个挑战性任务来评估从属性检测到跨图像重识别和链式推理的完整隐私推理能力。

Result: 对50多个基础模型和商业模型的大规模评估发现：许多模型存在显著的未测量推理隐私风险；感知级指标无法预测推理风险；现有安全对齐措施对此类推理攻击无效。
...
TL;DR: 提出了一种新的隐式神经表示框架，联合建模动态图像序列和其底层运动场，通过光流方程作为物理启发的正则化，实现同时恢复时间相干图像和运动场。


<details>
  <summary>Details</summary>
Motivation: 传统运动补偿重建依赖预估计的光流，但在欠采样情况下不准确且会降低重建质量。需要一种能够同时恢复图像和运动场的联合优化方法。

Method: 使用两个隐式神经表示：一个参数化时空图像内容，另一个表示光流场。两者通过光流方程耦合，并加入数据一致性损失强制与k空间测量一致。

Result: 在动态心脏MRI数据集上的实验表明，该方法优于最先进的运动补偿和深度学习方法，实现了更好的重建质量、准确的运动估计和改善的时间保真度。

Conclusion: 隐式联合建模与流正则化约束在推进dMRI重建方面具有潜力，能够同时获得高质量图像重建和准确运动估计。
...
TL;DR: 本文提出了FingerCap任务，用于生成捕捉手部动作细节的手指级语义文本描述，并构建了包含4万对手部运动视频和字幕的大规模数据集FingerCap-40K。为解决视频MLLMs的时间稀疏性问题，提出了FiGOP方法，将RGB关键帧与后续手部关键点配对，通过轻量级时间编码器恢复精细时间线索。


<details>
  <summary>Details</summary>
Motivation: 理解细粒度人类手部运动对视觉感知、具身智能和多模态通信至关重要。当前视频MLLMs由于时间稀疏性无法捕捉细微的手指运动动态。

Method: 提出FiGOP方法，将每个RGB关键帧与后续手部关键点配对，使用轻量级时间编码器将关键点转换为运动嵌入并与RGB特征集成，在不增加RGB密度的情况下恢复精细时间线索。

Result: 在FingerCap-40K数据集上的实验表明，强大的开源和闭源Video-MLLMs在手指级推理方面仍存在困难，而FiGOP增强模型在HandJudge和人类研究中均取得一致提升。

Conclusion: FiGOP方法有效解决了视频MLLMs在捕捉细粒度手指运动时的时间稀疏性问题，为手指级手部运动理解提供了有效解决方案。
...
TL;DR: SING3R-SLAM是一个基于高斯表示的稠密RGB SLAM框架，通过结合局部一致性3D重建和全局高斯表示来解决SLAM中的漂移和冗余问题，实现高效多用途3D建图。


<details>
  <summary>Details</summary>
Motivation: 现有稠密3D重建方法在SLAM中面临漂移和冗余点云地图的问题，限制了效率和下游任务（如新视角合成）的应用。

Method: 首先通过轻量级跟踪和重建模块构建局部一致性子图，然后逐步对齐并融合到全局高斯地图中，通过交叉视角几何一致性增强全局地图，同时全局地图反馈修正局部漂移。

Result: 在真实世界数据集上实现了最先进的跟踪、3D重建和新视角渲染性能，跟踪精度提升超过12%，生成更精细的几何细节，同时保持紧凑的内存高效全局表示。

Conclusion: SING3R-SLAM通过全局一致性和紧凑高斯表示有效解决了SLAM中的漂移和冗余问题，为多种下游应用提供了高效的3D建图解决方案。
...
TL;DR: 本文提出了一种点监督面部表情检测方法，通过双分支框架解决仅使用单时间戳标注的挑战，包括基于高斯分布的实例自适应强度建模和类别感知顶点分类。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖耗时的时间边界标注，本文旨在探索点监督面部表情检测，仅需每个实例的单时间戳标注，以降低标注成本。

Method: 提出双分支框架：1）基于高斯分布的实例自适应强度建模模块进行软伪标签分配；2）类别感知顶点分类分支区分宏表情和微表情；3）引入强度感知对比损失增强特征学习。

Result: 在SAMM-LV、CAS(ME)^2和CAS(ME)^3数据集上的广泛实验证明了所提框架的有效性。

Conclusion: 该点监督面部表情检测框架通过创新的强度建模和双分支设计，在仅使用单时间戳标注的情况下实现了有效的表情检测和分类。
...
TL;DR: QueryOcc是一个基于查询的自监督框架，通过4D时空查询直接学习连续3D语义占据，在无需大规模3D标注的情况下实现了优越的性能。
...
Method: 使用独立4D时空查询采样相邻帧，支持从视觉基础模型生成的伪点云或原始激光雷达数据进行监督，引入压缩场景表示以在恒定内存下实现长距离监督。

Result: 在自监督Occ3D-nuScenes基准测试中，语义RayIoU比先前相机方法提升26%，运行速度为11.6 FPS。
...
TL;DR: MatPedia是一个基于联合RGB-PBR表示的基础模型，通过将材料编码为RGB外观和PBR贴图的5帧序列，使用视频扩散架构统一处理多种材料任务，在1024×1024分辨率下实现高质量合成。
...
Method: 提出联合RGB-PBR表示，将材料编码为两个相互依赖的潜在变量：RGB外观和四个PBR贴图，将其视为5帧序列并使用视频扩散架构进行训练。

Result: 在MatHybrid-410K混合数据集上训练，MatPedia在1024×1024分辨率下的合成质量和多样性显著超越现有方法。

Conclusion: MatPedia通过统一的联合表示框架成功解决了材料生成中的碎片化问题，实现了文本到材料、图像到材料和本征分解等多种任务的统一处理。
...
TL;DR: 提出了一种边缘设备上基于烹饪状态指导的食物图像生成方法，通过厨师标注的熟度级别数据集和特定领域的烹饪图像相似度指标，实现了从生食到熟食的逼真图像合成。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上从生食图像合成逼真的熟食图像是一个具有挑战性的生成任务，现有方法要么生成结果不真实，要么资源消耗过大，无法在边缘设备上部署。

Method: 引入了首个基于烤箱的烹饪进展数据集，包含厨师标注的熟度级别；提出了边缘高效的食谱和烹饪状态指导生成器，根据生食图像合成逼真食物图像；引入了特定领域的烹饪图像相似度(CIS)指标作为训练损失和进度监控信号。

Result: 模型在多个数据集上显著优于现有基线方法，FID分数降低了30%（在自建数据集）和60%（在公共数据集）。

Conclusion: 该方法能够根据用户偏好的视觉目标生成逼真的熟食图像，而不是固定的预设，确保了时间一致性和烹饪合理性。
...
TL;DR: 本文提出了SeeCLIP框架来解决开放集域泛化问题，通过语义增强技术处理已知类和未知类之间的风险平衡问题，特别针对视觉相似但类别不同的'困难未知类'。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理开放集域泛化时面临已知类结构风险和未知类开放空间风险之间的困境，容易产生过度自信，特别是在区分与已知类具有细粒度视觉相似性的'困难未知类'时表现不佳。

Method: 提出了语义增强的CLIP框架SeeCLIP，包括语义感知提示增强模块将图像分解为判别性语义标记，双工对比学习保持与已知类的分离性和语义邻近性，以及语义引导扩散模块通过扰动语义标记合成伪未知类样本。

Result: 在五个基准测试上的广泛实验表明，相比最先进方法，准确率提升了3%，H-score提升了5%。

Conclusion: SeeCLIP通过细粒度语义增强有效解决了开放集域泛化中的风险平衡问题，显著提升了模型在区分视觉相似但类别不同的困难未知类时的性能。
...
TL;DR: 提出受自然选择启发的3D高斯泼溅修剪框架，通过优化梯度自主决定保留或修剪哪些高斯元素，无需人工干预，在15%预算下比3DGS获得超过0.6 dB PSNR提升
...
Conclusion: 该方法完全可学习，无需人工干预，通过优化梯度驱动的自然选择过程实现了高效的3D高斯泼溅修剪
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的深度很少是单一的，透射材料会产生层叠的模糊性，混淆传统感知系统。现有模型被动估计静态深度图，而人类能够主动调整焦点来感知所需深度。

Method: 引入DepthFocus，一个基于标量深度偏好进行条件控制的视觉Transformer模型，主要利用新构建的50万张多层合成数据集进行训练，该数据集专门设计用于捕捉各种透视效果。

Result: DepthFocus不仅在BOOSTER等传统单深度基准测试中达到最先进性能，还在新提出的真实和合成多深度数据集上定量展示了意图对齐的估计能力，并在未见过的透视场景中表现出强大的泛化能力。

Conclusion: DepthFocus代表了向主动和类人3D感知迈出的重要一步，展示了其在复杂场景中实现选择性深度感知的鲁棒性。
...
TL;DR: 本文提出了ConfusedTourist，一个评估视觉语言模型在混合文化线索下稳定性的对抗性测试套件，发现现有模型在遇到多重文化线索时准确率显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法通常只关注单一文化概念，忽略了现实世界中多重文化线索共存的情况，需要测试VLMs在多样化文化输入下的稳定性以支持多元文化社会。

Method: 开发了ConfusedTourist测试套件，通过图像堆叠和基于图像生成的扰动来评估VLMs对地理文化线索的稳定性，并进行可解释性分析。

Result: 实验显示VLMs在简单图像堆叠扰动下准确率大幅下降，基于图像生成的变体效果更差，失败源于系统性的注意力转移至干扰线索。

Conclusion: 视觉文化概念混合会严重损害最先进的VLMs性能，亟需开发更具文化鲁棒性的多模态理解模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人脸去识别方法在身份抑制和属性保留之间存在权衡问题，需要一种能更好平衡这两者的解决方案。

Method: 通过优化引导发现身份编辑方向，使用新颖的试剂损失函数监督属性保留和身份抑制，提出线性和测地线（基于切线）的编辑方案来导航潜在流形。

Result: 在CelebA-HQ和FFHQ数据集上的实验表明，FLUID在身份抑制和属性保留之间实现了优越的权衡，在定性和定量指标上都优于最先进的去识别方法。

Conclusion: FLUID框架通过潜在空间中的身份替换，为人脸去识别提供了一种有效的训练免费解决方案，在保持图像质量的同时实现了更好的隐私保护效果。
...
TL;DR: RacketVision是一个用于体育分析的新型计算机视觉数据集和基准，涵盖乒乓球、网球和羽毛球，提供大规模细粒度的球拍姿态和球位置标注，支持球跟踪、球拍姿态估计和球轨迹预测三个任务。
...
Result: 评估发现简单拼接球拍姿态特征会降低性能，而CrossAttention机制能有效利用这些特征，使轨迹预测结果超越单模态基线。
...
TL;DR: 本文提出了OmniPT，一个统一的行人跟踪框架，能够进行跟踪、基于参考的跟踪以及交互式生成被跟踪对象的语义理解。
...
Method: 采用RL-Mid Training-SFT-RL训练阶段：基于预训练的LVLM权重，首先进行RL阶段使模型输出固定格式的边界框，然后使用大量行人相关数据集进行中间训练，最后在多个行人跟踪数据集上进行监督微调，再进行RL阶段提升跟踪性能和指令跟随能力。

Result: 在跟踪基准测试上的实验结果表明，所提方法优于先前的方法。

Conclusion: OmniPT框架成功将跟踪任务建模为基础模型可执行的任务，并能输出格式化答案，在行人跟踪任务中表现出色。
...
TL;DR: REArtGS++是一种改进的铰接物体重建方法，通过解耦螺旋运动建模、零件感知高斯建模和时间连续几何约束，解决了REArtGS在螺旋关节和多零件物体上的局限性。


<details>
  <summary>Details</summary>
Motivation: REArtGS在处理螺旋关节或多零件物体时存在困难，且缺乏对未见状态的几何约束。本文旨在开发一种更通用的铰接物体重建方法。

Method: 提出解耦螺旋运动建模（无需关节类型先验），通过零件运动混合联合优化零件感知高斯和关节参数，引入时间连续几何约束（鼓励高斯平面化）并通过泰勒一阶展开实现平面法向和深度的时序一致性正则化。

Result: 在合成和真实世界铰接物体上的大量实验表明，该方法在可泛化的零件级表面重建和关节参数估计方面优于现有方法。

Conclusion: REArtGS++通过时间几何约束和平面高斯溅射，实现了更通用的铰接物体重建，显著提升了重建质量和关节参数估计精度。
...
TL;DR: ReBrain是一个基于检索增强扩散框架的脑部MRI重建方法，通过Brownian Bridge扩散模型合成MRI切片，并利用检索的相似CT切片作为参考来指导生成过程，在稀疏CT条件下实现高质量的跨模态重建。


<details>
  <summary>Details</summary>
Motivation: 由于物理或临床限制，某些患者无法进行MRI检查，而低剂量CT协议产生的稀疏CT体积难以准确重建完整脑部MRI体积，因此需要开发在稀疏条件下进行跨模态重建的方法。

Method: 提出ReBrain框架：1）使用Brownian Bridge扩散模型在2D维度合成MRI切片；2）通过微调的检索模型从先验数据库中检索结构和病理相似的CT切片；3）利用ControlNet分支将检索切片作为参考指导中间MRI切片生成；4）针对罕见检索失败情况应用球面线性插值提供补充指导。

Result: 在SynthRAD2023和BraTS数据集上的广泛实验表明，ReBrain在稀疏条件下的跨模态重建中实现了最先进的性能。

Conclusion: ReBrain通过结合扩散模型和检索增强机制，有效解决了稀疏CT条件下脑部MRI重建的挑战，为无法进行MRI检查的患者提供了可行的替代方案。
...
Conclusion: DiverseVAR有效释放了VAR模型的固有生成潜力，同时保持高保真合成质量。
...
TL;DR: STAR模型通过使用均匀生成树的遍历顺序，在保持采样性能的同时提供灵活的序列顺序，以支持图像编辑任务。


<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在视觉生成中采用随机排列序列顺序时，要么性能下降，要么在推理时序列顺序选择不够灵活。需要一种既能保持性能又能提供灵活序列顺序的方法。

Method: STAR利用图像块位置定义的网格中采样的均匀生成树的遍历顺序，通过广度优先搜索获得遍历顺序，并通过拒绝采样确保连接的图像部分观测作为序列前缀出现。

Result: STAR通过定制但结构化的随机策略相比随机排列，保持了后缀完成能力，同时在不显著改变语言AR模型架构的情况下维持了采样性能。

Conclusion: STAR建模能够结合图像先验知识，在保持采样性能的同时提供足够灵活的序列顺序，以适应推理时的图像编辑需求。
...
TL;DR: 提出了一种基于平面高斯点云的类别无关铰接物体重建框架，仅需单状态下的稀疏视角RGB图像即可实现高保真部件级表面重建。
...
Result: 在合成和真实数据上的实验结果表明，该方法相比现有方法实现了更高保真度的部件级表面重建。
...
TL;DR: ReCoVAD是一个受人类神经系统启发的视频异常检测框架，通过选择性帧处理减少冗余计算，在仅处理28.55%和16.04%帧的情况下达到最先进的训练免费性能。
...
Method: 提出双通路框架：反射通路使用轻量级CLIP模块融合视觉特征与原型提示，查询动态记忆进行快速响应；意识通路使用中规模视觉语言模型生成文本事件描述和精炼异常分数，持续更新记忆和原型提示。

Result: 在UCF-Crime和XD-Violence数据集上，ReCoVAD仅处理28.55%和16.04%的帧数就达到最先进的训练免费性能，证明了稀疏推理对基于大模型的视频异常检测是足够的。

Conclusion: 研究表明稀疏推理足以实现有效的基于大模型的视频异常检测，ReCoVAD框架通过选择性帧处理显著减少了计算成本。
...
### [45] [Planning with Sketch-Guided Verification for Physics-Aware Video Generation](https://arxiv.org/abs/2511.17450)
*Yidong Huang,Zun Wang,Han Lin,Dong-Ki Kim,Shayegan Omidshafiei,Jaehong Yoon,Yue Zhang,Mohit Bansal*

Main category: cs.CV

TL;DR: SketchVerify是一个无需训练的基于草图验证的运动规划框架，通过测试时采样和验证循环生成更动态连贯的轨迹，在完整视频生成前提高运动质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法使用单次规划或迭代优化，前者运动简单，后者计算成本高。需要一种既能生成复杂动态运动又高效的方法。

Method: 给定提示和参考图像，预测多个候选运动轨迹，使用视觉语言验证器评估语义对齐和物理合理性。通过将轨迹渲染为轻量级视频草图来高效评分，避免重复的扩散合成。

Result: 在WorldModelBench和PhyWorldBench上的实验表明，该方法显著提高了运动质量、物理真实性和长期一致性，同时效率更高。

Conclusion: SketchVerify通过测试时采样和验证有效提升了运动规划质量，增加轨迹候选数量能持续提升性能。
...
Method: 提出PEGS框架，将物理先验与事件流增强集成到3D高斯泼溅流程中，采用三重监督方案（加速度约束、事件流高时间分辨率指导、卡尔曼正则化），并设计基于实时运动状态的自适应模拟退火训练策略。

Result: 实验表明PEGS在大时空尺度运动重建方面优于主流动态方法，并贡献了首个针对自然快速刚性运动的RGB-Event配对数据集。

Conclusion: PEGS通过整合物理先验和事件流增强，成功解决了大时空尺度刚性运动重建的挑战，在运动恢复和去模糊建模方面表现出色。
...
TL;DR: 本文提出从传统的1D相关色温(CCT)映射转向2D色度空间，并采用轻量级多层感知器(MLP)来提升非普朗克光源(如LED)下的比色精度。


<details>
  <summary>Details</summary>
Motivation: 传统相机比色映射依赖基于CCT的插值方法，但现代LED光源可能显著偏离普朗克轨迹，暴露了仅依赖一维CCT进行光照表征的局限性。

Method: 使用基于灯箱的校准程序，结合代表性LED光源训练轻量级MLP，利用2D色度特征进行稳健的比色映射。

Result: 在各种LED照明下验证，该方法在LED照明场景中平均减少22%的角度再现误差，保持与传统光源的向后兼容性，支持多光源场景和实时相机部署。

Conclusion: 从1D CCT转向2D色度空间并结合MLP方法，能够显著提高非普朗克光源下的比色精度，同时保持计算效率和兼容性。
...
TL;DR: 提出一个多阶段优化框架，将高性能浮点图像压缩模型转换为高效的FPGA硬件友好型整数实现，包括动态范围感知量化、混合精度搜索和通道剪枝技术。
...
Method: 1. 动态范围感知量化(DRAQ)：使用统计校准的激活裁剪和权重正则化处理极端数据异常值和大动态范围；2. 渐进混合精度搜索：为每层分配最优非均匀位宽；3. 通道剪枝：针对GDN层移除不活跃通道。

Result: DRAQ方法将GDN模型的BD-rate开销从30%降至6.3%；硬件感知优化进一步降低计算复杂度超过20%，对RD性能影响可忽略，最终模型在效率和性能上均优于现有FPGA实现。

Conclusion: 该框架成功实现了高性能图像压缩模型在FPGA上的高效部署，在保持压缩质量的同时显著降低了计算复杂度。
...
TL;DR: ODTSR是一种基于Qwen-Image的单步扩散变换器，通过噪声混合视觉流设计同时考虑保真度和可控性，在真实世界图像超分辨率中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散基真实世界图像超分辨率方法在保真度和可控性之间存在平衡问题：多步方法存在生成多样性和随机性导致低保真度，而单步方法因保真度特定微调而失去控制灵活性。

Method: 引入噪声混合视觉流设计：新视觉流接收带可调噪声的低质量图像，原视觉流接收带一致噪声的低质量图像；采用保真度感知对抗训练增强可控性并实现单步推理。
...
TL;DR: CC-DiceCE是一种基于CC-Metrics框架的损失函数，用于解决医学图像分割中传统Dice损失对小病灶欠分割的问题。在nnU-Net框架下，CC-DiceCE相比blob loss和DiceCE基线，能提高小病灶检测率，但会略微增加假阳性。


<details>
  <summary>Details</summary>
Motivation: 传统医学图像分割损失函数（如Dice）对小病灶分割效果不佳，因为小病灶的相对体积对整体损失贡献很小。需要开发实例级别的损失函数来改善小病灶的分割性能。

Method: 提出基于CC-Metrics框架的CC-DiceCE损失函数，并与现有的blob loss进行比较。在nnU-Net框架中进行标准化基准测试，使用多数据集研究评估性能。

Result: CC-DiceCE损失函数显著提高了小病灶的检测率（召回率），分割性能仅有轻微下降。与blob loss相比，CC-DiceCE在多数据集研究中表现更优，但会略微增加假阳性。

Conclusion: CC-DiceCE是一种有效的实例级别损失函数，能够显著改善医学图像分割中小病灶的检测性能，在保持分割质量的同时提高召回率，优于现有的blob loss方法。
...
Method: 提出DiffRefiner框架：第一阶段使用基于Transformer的提议解码器从传感器输入回归生成粗略轨迹预测；第二阶段应用扩散精炼器迭代去噪和精炼这些初始预测。还设计了细粒度去噪解码器以增强场景合规性。

Result: 在NAVSIM v2上达到87.4 EPDMS，在Bench2Drive上达到87.1 DS和71.4 SR，在两个公共基准测试中都创造了新记录。消融研究验证了各组件的有效性。

Conclusion: DiffRefiner通过将判别式轨迹提议与生成式精炼相结合，显著提升了扩散基规划的性能，为自动驾驶轨迹预测提供了更准确的解决方案。
...
TL;DR: FireScope是一个基于视觉语言模型的推理生成框架，通过结合Sentinel-2影像和气候数据预测野火风险图，并在跨大陆评估中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏因果推理和多模态理解能力，无法实现可靠的泛化。需要开发能够整合视觉、气候和地理因素来推理连续风险图的方法。

Method: 提出FireScope-Bench数据集和基准，结合Sentinel-2影像、气候数据和专家定义的风险栅格。FireScope框架通过强化学习和视觉监督学习预测风险栅格，并生成补充的推理轨迹。

Result: 在美国训练并在欧洲测试时，FireScope实现了显著的性能提升。专家反馈和自动分析确认其推理轨迹是忠实且语义有意义的。

Conclusion: 推理能够支撑栅格预测模型，提高泛化能力和可解释性。这是首个证明基于语言的推理可以改善视觉生成泛化能力的框架，也是首个可跨大陆应用的高分辨率野火风险模型。
...
TL;DR: UI-Styler是一种针对超声图像的类别感知风格迁移框架，通过模式匹配机制和类别感知提示策略，解决跨设备超声图像分布偏移问题，在分类和分割任务中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 超声图像在不同采集设备间存在外观差异，导致固定黑盒下游推理模型性能下降。现有无配对图像翻译方法在领域适应过程中忽视类别特定语义对齐，影响诊断准确性。

Method: 提出UI-Styler框架，使用模式匹配机制将目标图像纹理模式转移到源图像，同时保留源结构内容；引入基于目标域伪标签的类别感知提示策略，确保与诊断类别的准确语义对齐。

Result: 在超声跨设备任务上的广泛实验表明，UI-Styler在分布距离和下游任务（如分类和分割）中持续优于现有无配对图像翻译方法，达到最先进性能。

Conclusion: UI-Styler通过类别感知的风格迁移有效解决了超声图像跨设备分布偏移问题，显著提升了黑盒推理模型的重用性能。
...
TL;DR: 本文系统评估了自监督表示在音频-视觉深度伪造检测中的潜力，发现这些特征能捕获相关互补信息，但无法可靠跨数据集泛化。


<details>
  <summary>Details</summary>
Motivation: 探索自监督表示在音频-视觉深度伪造检测中的潜力，此前工作要么单独使用这些特征，要么将其嵌入复杂架构中，缺乏系统性评估。

Method: 在音频、视频和多模态三个维度，以及唇部运动和通用视觉内容两个领域，系统评估自监督特征在检测效果、可解释性和跨模态互补性三个关键维度的表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法主要针对语义理解而非几何推理，CroCo虽然针对3D理解但仍有改进空间，本文旨在学习更适合3D视觉的特征表示。

Method: 扩展MAE到任意多视角图像，对所有视角进行统一掩码，使用轻量级解码器并引入帧间注意力机制。

Result: 在包括前向重建、密集图像匹配和相对位姿估计等下游任务中，MuM模型超越了当前最先进的视觉编码器DINOv3和CroCo v2。

Conclusion: 通过多视角MAE方法可以学习到更适合3D视觉任务的特征表示，该方法比CroCo更简单且更具可扩展性。
...
Method: 提出查询共享交叉注意力模块，将6-DoF相机位姿和2D渲染视频帧融合到共享特征空间中；采用两阶段训练策略：先学习粗粒度相机控制，再结合视觉信息细化运动精度。

Result: 在真实世界和合成数据集上的实验表明，PostCam在相机控制精度和视角一致性方面比最先进方法提高超过20%，同时达到最高的视频生成质量。

Conclusion: PostCam通过统一的运动表示和两阶段训练策略，实现了更准确和灵活的相机运动操作，显著提升了动态场景中新颖视角视频生成的性能。
...
TL;DR: 本研究提出了一个针对大型视觉语言模型幻觉问题的综合干预框架，发现幻觉源于图像-输入文本、图像-输出文本和文本-文本三条路径的相互作用，并首次发现模型依赖不同路径取决于问答对齐格式。基于这些发现，提出了针对判别式和生成式格式的简单有效方法来识别和干预关键幻觉头。
...
Method: 提出了与transformer因果架构对齐的综合干预框架，分析了不同干预路径对幻觉的影响。通过识别关键幻觉头，针对判别式和生成式格式设计了相应的干预方法。

Result: 实验表明，该方法在多个基准测试中能够持续减少各种对齐类型的幻觉问题。

Conclusion: 幻觉问题不是由单一因果路径引起的，而是多条路径相互作用的结果。通过针对不同问答对齐格式设计专门的干预策略，可以有效减少大型视觉语言模型的幻觉现象。
...
TL;DR: 本文提出了非参数概率鲁棒性(NPPR)作为对抗性鲁棒性(AR)和概率鲁棒性(PR)的补充指标，它不需要预定义的扰动分布，而是直接从数据中学习优化的扰动分布。


<details>
  <summary>Details</summary>
Motivation: 现有的概率鲁棒性(PR)方法假设固定且已知的扰动分布，这在实践中不现实。为了解决这一局限性，需要开发更实用的鲁棒性度量方法。

Method: 提出NPPR方法，基于高斯混合模型(GMM)与多层感知器(MLP)头以及双三次上采样，学习优化的扰动分布，涵盖输入相关和输入无关的扰动场景。

Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上，使用ResNet18/50、WideResNet50和VGG16模型进行实验验证，NPPR相比现有方法提供了更保守(低40%)的PR估计。

Conclusion: NPPR被验证为更实用的鲁棒性度量指标，能够在分布不确定性下进行保守的概率鲁棒性评估。
...
TL;DR: VLA-4D是一个具有4D感知能力的视觉-语言-动作模型，通过4D感知视觉表示和时空动作表示实现时空一致的机器人操作。
...
Method: 提出4D感知视觉表示（将1D时间嵌入3D位置形成4D嵌入）和时空动作表示（扩展空间动作表示包含时间信息），通过交叉注意力机制融合多模态表示。
...
### [61] [Range-Edit: Semantic Mask Guided Outdoor LiDAR Scene Editing](https://arxiv.org/abs/2511.17269)
*Suchetan G. Uppur,Hemant Kumar,Vaibhav Kumar*

Main category: cs.CV

TL;DR: 本文提出了一种基于语义掩码引导的LiDAR点云编辑方法，通过将点云转换为2D距离图像并使用语义掩码指导扩散模型生成，能够高效生成复杂的边缘案例和动态场景。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统训练需要大量多样的点云数据，特别是边缘案例场景。现有方法依赖手工制作的3D虚拟环境，耗时且难以捕捉真实世界复杂性。

Method: 将LiDAR点云转换为2D距离图像，使用基于凸包的语义掩码作为条件指导扩散模型生成，确保几何一致性和真实性。

Result: 在KITTI-360数据集上验证了高质量LiDAR点云生成能力，能够生成复杂边缘案例和动态场景。
...
<details>
  <summary>Details</summary>
Motivation: 医疗图像分割中，机构间的异构隐私政策使得在汇集数据集上进行联合训练不可行，需要在不发生灾难性遗忘的情况下从数据流中进行持续学习。虽然SAM模型提供了强大的零样本先验，但其大参数量和计算开销限制了实际部署。

Method: 提出对齐层（Alignment Layer）作为轻量级即插即用模块，对齐编码器-解码器特征分布以高效适配SAM到特定医疗图像；基于SAM和对齐层，提出CA-SAM持续学习策略，自动适配适当的对齐层来缓解灾难性遗忘，同时利用SAM的零样本先验保持对未见医疗数据集的强性能。

Result: 在九个医疗分割数据集上的持续学习场景实验中，CA-SAM实现了最先进的性能。

Conclusion: SAM范式在平衡计算效率和性能后具有很大潜力，CA-SAM方法有效解决了医疗图像分割中的持续学习挑战，同时保持了高精度和低计算开销。
...
TL;DR: SPECTRE是一种基于Transformer的3D CT基础模型，通过自监督和跨模态预训练学习通用CT表示，在多个基准测试中优于现有方法。
...
Conclusion: SPECTRE证明无需依赖私有数据即可实现高性能、可泛化的表示，成为可扩展、开放且完全基于Transformer的3D医学成像基础模型。
...
<details>
  <summary>Details</summary>
Motivation: 解决鱼眼图像中由于极端非线性畸变、遮挡和深度模糊导致的BEV语义分割挑战。
...
Conclusion: 该框架能够直接从鱼眼图像生成连续、不确定性感知的语义地图，有效应对鱼眼图像特有的几何挑战。
...
TL;DR: 本文提出了一种利用生成模型创建多样化合成人脸图像的方法，并发布了包含27,780张图像、926个身份的DIF-V数据集，以解决现有数据集在种族、性别等方面的偏见问题。
...
Method: 提出综合方法，集成先进的生成模型创建多样化高质量合成人脸图像，强调多样化面部特征的表示，确保符合身份证照片特征。
...
TL;DR: 提出了一种针对彩色图像盲反卷积的新方法，使用四元数保真项来建模颜色通道间的关系，通过归一化四元数核在盲反卷积过程中保持图像强度，实验证明该方法能有效去除伪影并显著改善去模糊效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将彩色图像转换为灰度图或单独处理每个颜色通道，忽略了颜色通道之间的关系，这限制了彩色图像盲反卷积的效果。

Method: 设计了一个新颖的四元数保真项，利用四元数卷积核的特性，包含四个核：一个类似非负卷积核捕捉整体模糊，三个无约束卷积核分别对应红、绿、蓝通道以建模未知的相互依赖关系，并使用归一化四元数核来保持图像强度。

Result: 在真实模糊彩色图像数据集上的大量实验表明，该方法能有效去除伪影并显著改善去模糊效果。

Conclusion: 该方法展示了作为彩色图像反卷积强大工具的潜力，通过建模颜色通道间的关系提升了盲反卷积性能。
...
TL;DR: MolSight是一个用于光学化学结构识别(OCSR)的综合学习框架，通过三阶段训练范式解决现有系统在立体化学信息识别方面的挑战，在立体化学光学结构识别方面达到最先进性能。
...
Result: 实验结果表明，MolSight在立体化学光学结构识别方面达到了最先进的性能，即使模型参数规模相对紧凑，GRPO算法也能进一步提升模型在立体分子识别方面的表现。

Conclusion: MolSight框架通过创新的三阶段训练方法有效解决了OCSR中立体化学识别的挑战，为化学信息学、药物发现和相关领域的LLM应用提供了重要支持。
...
### [68] [REMSA: An LLM Agent for Foundation Model Selection in Remote Sensing](https://arxiv.org/abs/2511.17442)
*Binger Chen,Tacettin Emre Bök,Behnood Rasti,Volker Markl,Begüm Demir*

Main category: cs.CV

TL;DR: 该论文介绍了RSFM数据库和REMSA系统，这是一个基于LLM的智能体，用于从自然语言查询中自动选择遥感基础模型。


<details>
  <summary>Details</summary>
Motivation: 由于遥感基础模型(RSFMs)的文档分散、格式异构和部署约束多样，选择合适的模型变得困难。

Method: 构建了包含150多个RSFMs的结构化数据库RS-FMD，并基于此开发了REMSA智能体，使用上下文学习来理解用户需求、解决缺失约束并排名候选模型。

Result: REMSA在75个专家验证的查询场景基准测试中表现优于多个基线方法，包括朴素智能体、密集检索和非结构化RAG-based LLMs。

Conclusion: REMSA能够有效帮助用户选择合适的遥感基础模型，仅使用公开可用的元数据，不访问私有或敏感数据。
...
TL;DR: BiFingerPose是一种基于双模态的指尖姿态估计算法，结合电容图像和指纹补丁，能够同时准确预测全面的指尖姿态信息，显著提升了预测性能、任务完成效率和用户操作精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于电容图像的指尖姿态估计算法在便携设备上主要局限于估计俯仰角和偏航角，且在处理大角度输入时精度下降，无法可靠估计滚转角。
...
Conclusion: 该方法能够可靠估计滚转角，显著提升了其他姿态参数的预测性能，在增强认证安全性和改善交互体验方面具有实际优势。
...
<details>
  <summary>Details</summary>
Motivation: 大多数多模态大语言模型在解释和推断三维空间布局方面存在空间推理能力有限的问题，这源于现有视觉编码器（如CLIP）的损失性嵌入限制在实例级语义特征上，导致空间模糊性。

Method: 提出SpatialGeo方法，通过分层适配器将自监督学习的几何特征与CLIP的语义特征互补融合，生成空间感知的视觉嵌入。使用预训练的LLaVA模型进行高效训练，并通过随机特征丢弃优化以避免仅依赖CLIP编码器的平凡解。

Result: 实验结果显示，SpatialGeo在空间推理任务中显著提升了准确性，在SpatialRGPT-Bench上将最先进模型的性能提高了至少8.0%，同时推理期间内存成本减少了约50%。

Conclusion: SpatialGeo通过几何和语义特征的分层融合有效增强了多模态大语言模型的空间感知能力，在保持高效性的同时显著提升了空间推理性能。
...
Method: 1. 基于显式特征匹配的相对姿态初始化；2. 局部联合优化增强姿态估计；3. 全局优化阶段引入几何一致性约束和束调整；4. 首次将局部和全局线索与NeRF无缝结合。

Result: 在基准数据集上的广泛评估表明，该方法在姿态估计精度和新视角合成方面优于现有最先进方法，在挑战性场景中表现出卓越性能和鲁棒性。

Conclusion: NoPe-NeRF++通过局部到全局的优化策略成功解决了复杂场景中的姿态估计问题，验证了设计选择的合理性，为无姿态先验的NeRF训练提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 生成图像模型在合成透明物体时表现不佳，特别是在折射效果方面，因为它们未能充分学习光学定律来准确渲染折射物体。
...
Conclusion: 通过物理约束的像素同步方法能够有效提升生成图像中折射效果的准确性，为透明物体的逼真渲染提供了有效解决方案。
...
TL;DR: 提出一个统一框架用于多媒介绘画过程生成，通过语义驱动的风格控制机制将多种媒介嵌入扩散模型条件空间，使用跨媒介风格增强，实现一致的纹理演化和过程转移。
...
Method: 采用语义驱动的风格控制机制，将多种媒介嵌入扩散模型条件空间，使用跨媒介风格增强和反向绘画训练策略，确保平滑且符合人类创作习惯的生成过程。

Result: 在跨媒介一致性、时间连贯性和最终图像保真度方面取得显著成果，在LPIPS、DINO和CLIP指标上表现优异，并通过感知距离轮廓曲线定量建模创作序列。

Conclusion: 该框架成功实现了多媒介绘画过程的生成，能够忠实再现人类艺术创作流程中的构图、色彩块面和细节精炼等阶段。
...
TL;DR: DSeq-JEPA是一种改进的视觉表示学习方法，通过识别主要判别区域并按重要性顺序预测后续区域，将JEPA的潜在预测与GPT风格的序列推理相结合。


<details>
  <summary>Details</summary>
Motivation: 现有的I-JEPA方法对所有区域进行统一和独立的处理，缺乏对预测顺序的明确概念。受人类视觉感知的启发，需要选择性、顺序性地从最信息丰富的区域到次要区域进行注意力部署。
...
Conclusion: DSeq-JEPA成功地将预测性和自回归自监督学习相结合，通过引入判别性顺序预测机制，显著提升了视觉表示的质量和泛化能力。
...
Method: 使用超二次曲面场景表示，通过多层二十面体细分的高斯近似超二次曲面，在训练期间实现高斯光栅化监督。

Result: 在Occ3D数据集上，相比之前的高斯方法，内存占用减少75%，推理速度提升124%，mIoU提升5.9%，场景建模所需基元数量减少84%。
...
TL;DR: 提出了ATAC方法，通过在CLIP嵌入空间中计算增强诱导的漂移向量来推断语义恢复方向，基于角度一致性校正嵌入，显著提升了对抗性鲁棒性。
...
Method: ATAC方法直接在CLIP嵌入空间中操作，计算增强诱导的漂移向量来推断语义恢复方向，并基于这些潜在漂移的角度一致性校正嵌入。

Result: 在各种基准测试中，ATAC始终实现极高的鲁棒性，平均比先前最先进方法高出近50%，同时计算开销极小。在非常规和极端设置下仍保持最先进的鲁棒性，甚至对自适应攻击也实现了非平凡的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏体素在空间上与邻居解耦且边界清晰，这使它们在优化过程中容易陷入局部最小值。虽然SDF值提供了自然平滑连续的几何场，但在独立参数化的稀疏体素之间保持这种平滑性具有挑战性。

Method: 1) 使用视觉几何模型进行鲁棒几何初始化；2) 通过空间平滑损失在父子体素组和兄弟体素组之间强制执行一致关系
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像配准中高维变形空间的挑战和体素级监督稀缺的问题，现有强化学习框架将空间投影到粗糙低维表示，限制了捕捉空间变异变形的能力。

Method: 引入随机高斯策略头在编码器上建模潜在特征分布，结合无监督预热和弱监督微调，通过组相对策略优化实现多轨迹采样稳定训练。

Result: 在三个3D配准基准测试中（OASIS脑MRI、LiTS肝脏CT和腹部MR-CT），相比竞争基线实现了持续的Dice改进，同时保持高标签效率和低参数成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有单教师对抗蒸馏方法存在知识多样性有限、收敛慢、鲁棒性与准确性难以平衡等问题，需要更有效的多教师协作框架来提升视觉语言模型的对抗鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要关注基于事实观察的前向模拟，但新兴应用需要回答反事实查询，如"如果移除这个物体会发生什么"，这需要世界模型能够处理假设性场景修改。

Method: CWMDT采用三步法：1)构建场景的数字孪生来显式编码物体及其关系；2)使用大语言模型推理反事实干预的时间传播效果；3)用修改后的表示条件化视频扩散模型来生成反事实视觉序列。

Result: 在两个基准测试上的评估表明，CWMDT方法实现了最先进的性能，表明视频的替代表示（如数字孪生）为基于视频前向模拟的世界模型提供了强大的控制信号。

Conclusion: CWMDT框架成功地将标准视频扩散模型转化为有效的反事实世界模型，通过结构化文本表示和语言模型推理，实现了对特定场景属性的针对性干预和反事实预测。
...
Result: Radar2Shape能够成功地从部分观测的雷达信号重建任意3D形状，并在两种不同仿真方法和真实数据上表现出鲁棒的泛化能力。

Conclusion: 该方法有效解决了从部分观测雷达信号进行3D重建的挑战，并发布了两个合成基准数据集以促进高频雷达领域的研究，使模型能够安全地应用于真实雷达系统。
...
TL;DR: 提出了一种基于计算机视觉的深度学习方法来估计脊柱年龄，使用超过18,000个MRI系列图像，通过脊柱年龄差距(SAG)作为衡量整体脊柱健康的生物标志物。


<details>
  <summary>Details</summary>
Motivation: 脊柱是人体重要结构，易受年龄相关退化的影响，需要一种客观方法来评估脊柱健康状况。

Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退化条件聚类，通过消融研究选择模型，计算实际脊柱年龄与模型预测年龄的差异(SAG)。

Result: 发现SAG与椎间盘突出、骨赘、椎管狭窄、骨折等脊柱退行性疾病以及吸烟、体力劳动等生活方式因素相关。

Conclusion: 脊柱年龄差距(SAG)可作为衡量整体脊柱健康的有用生物标志物。
...
TL;DR: 本文分析了多模态模型中LLM降尺度对视觉能力的影响，发现视觉能力下降比推理能力更显著，提出了提取+思考方法来解决这一瓶颈。


<details>
  <summary>Details</summary>
Motivation: 实际应用需要更小、更高效的多模态系统，但LLM降尺度会不成比例地影响视觉能力而非继承自LLM的能力，需要解决这一性能瓶颈。

Method: 引入视觉提取调优，明确训练模型在不同任务中一致提取指令相关的视觉细节，然后应用逐步推理生成答案，形成提取+思考方法。

Result: 提取+思考方法在该领域设定了效率和性能的新标准，显著缓解了LLM降尺度带来的视觉能力下降问题。

Conclusion: 通过专门的视觉提取调优和逐步推理，可以有效解决多模态模型降尺度时视觉能力下降的问题，实现更高效的系统。
...
TL;DR: Video-R4是一个视频推理LMM，通过视觉反刍机制迭代选择帧、放大关键区域、重新编码像素来增强文本丰富视频的理解能力，在多个视频QA任务上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频QA模型依赖单次感知固定帧，无法处理需要重复检查的细小文本线索，导致幻觉和细粒度证据失败。受人类暂停、放大和重读关键区域的行为启发。
...
<details>
  <summary>Details</summary>
Motivation: 现有指令引导3D编辑方法存在关键限制：基于优化的方法速度过慢，而基于多视图2D编辑的前馈方法存在几何不一致和视觉质量下降问题。

Method: 创建大规模多模态指令引导3D编辑数据集，探索两种条件策略：传统交叉注意力机制和新型3D令牌拼接方法。

Result: 令牌拼接方法参数效率更高且性能更优，在生成质量、3D一致性和指令保真度方面优于现有2D提升方法。

Conclusion: 提出的原生3D编辑框架在生成质量、3D一致性和指令保真度方面设定了新的基准，为3D内容创作提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有向量数据库检索管道使用平面或单分辨率索引结构，无法适应不同用户查询所需的语义粒度变化，导致检索速度和上下文相关性之间的权衡不佳。
...
TL;DR: Bench360是一个全面的本地LLM推理基准测试框架，支持用户自定义任务、数据集和指标，自动测试不同模型、推理引擎和量化级别在各种使用场景下的性能表现。


<details>
  <summary>Details</summary>
Motivation: 当前LLM推理基准测试工具设计目标狭窄，无法整合系统指标和任务特定指标，缺乏统一的用户友好基准测试框架来支持多种推理引擎、使用场景和量化级别。

Method: 开发Bench360框架，允许用户定义自定义任务、数据集和相关任务特定指标，自动基准测试选定的LLM、推理引擎和量化级别，涵盖单流、批处理和服务器等不同使用场景。

Result: 在四个常见LLM任务、三个硬件平台和四个最先进推理引擎上的测试结果显示，任务性能和系统级效率之间存在多个有趣的权衡，不同推理引擎和模型存在显著差异。

Conclusion: 本地推理没有单一最佳设置，这强烈需要像Bench360这样的框架来帮助用户找到满足其特定需求的最佳配置。
...
TL;DR: 该研究评估了大型语言模型对突尼斯阿拉伯语的理解能力，创建了包含突尼斯方言、标准阿拉伯语和英语的平行数据集，并在音译、翻译和情感分析三个任务上测试了多个流行模型。


<details>
  <summary>Details</summary>
Motivation: 工业级大型语言模型往往忽视低资源语言如突尼斯阿拉伯语的理解能力，这可能导致数百万突尼斯人无法用自己的语言与AI互动，威胁方言保护并影响年轻一代的语言偏好。

Method: 创建包含突尼斯方言、标准突尼斯阿拉伯语和英语的平行数据集，并标注情感标签，在音译、翻译和情感分析三个任务上对多个流行LLM进行基准测试。
...
TL;DR: 本文提出了一种评估提示词是否能有效引导生成文本朝向特定人类价值观的方法，通过量化目标价值观在生成响应中的存在和增益来评分。
...
Method: 开发了一种实用、可重复、模型无关的程序，使用Schwartz基本人类价值观理论和对话数据集进行结构化评估，比较基线提示词和明确基于价值观的提示词。

Result: 研究表明，即使不改变模型或不动态优化提示词，价值观引导也是可能的。

Conclusion: 该方法为评估提示词在引导语言模型输出朝向特定人类价值观方面的有效性提供了系统化框架。
...
TL;DR: 本文提出了一种基于概念梯度的可解释性技术，用于毒性检测中的概念归因分析，通过构建目标词典集和计算词-概念对齐分数来识别导致误分类的有毒词汇，并引入无词典增强策略来验证模型对更广泛毒性语言模式的归因。


<details>
  <summary>Details</summary>
Motivation: 社交媒体中有害内容的传播日益严重，虽然毒性检测在文本数据方面取得了显著进展，但基于概念的解释在毒性检测中的探索仍然有限。现有方法中概念对目标类别的不均衡归因常常导致分类错误。

Method: 1. 利用概念梯度方法提供更因果的解释；2. 构建目标词典集捕获导致误分类的有毒词汇；3. 计算词-概念对齐分数量化词汇对误分类的影响；4. 提出无词典增强策略，生成排除预定义有毒词典集的毒性样本。

Result: 该方法能够识别导致分类错误的过度归因词汇，并通过无词典增强验证模型对更广泛毒性语言模式的归因能力，而不仅仅是依赖显式词汇重叠。

Conclusion: 概念梯度方法为毒性检测提供了更深入的可解释性分析，目标词典集和词-概念对齐分数能有效识别误分类原因，无词典增强策略揭示了模型对毒性语言模式的更广泛理解。
...
TL;DR: 本文研究了AI检测模型对轻微AI润色的人类文本的错误分类问题，特别针对阿拉伯语开发了两个数据集来评估14个LLM和商业AI检测器的性能。


<details>
  <summary>Details</summary>
Motivation: 现有AI检测模型在检测轻微AI润色的人类文本时会产生误判，可能导致错误指控作者使用AI抄袭，损害AI检测器的可信度。英语领域已有相关研究，但阿拉伯语领域尚属空白。

Method: 生成了两个阿拉伯语数据集：第一个包含800篇文章（一半AI生成，一半人类撰写），用于评估14个LLM和商业AI检测器；第二个Ar-APT数据集包含400篇人类文章，由10个LLM使用4种润色设置处理，共16400个样本，用于评估8个最佳模型对轻微润色文本的检测能力。

Result: 所有AI检测器都会错误地将大量轻微润色的人类文章归为AI生成。最佳LLM Claude-4 Sonnet准确率为83.51%，但对LLaMA-3轻微润色文章的检测准确率降至57.63%；最佳商业模型originality.AI准确率为92%，但对Mistral或Gemma-3轻微润色文章的检测准确率骤降至12%。

Conclusion: 轻微AI润色会显著影响AI检测模型的性能，导致对原始人类文本的错误分类，这突显了当前AI检测技术在区分轻微润色人类文本方面的局限性，特别是在阿拉伯语领域。
...
TL;DR: 本文复现了Test-Time Training on Nearest Neighbors方法，通过检索最近邻序列在推理时微调语言模型，证实该方法能显著降低困惑度和bits-per-byte指标，特别是在结构化数据集上效果最明显。
...
Conclusion: 最近邻测试时训练具有鲁棒性和通用性，即使对于现代推理优化架构也能带来一致收益，同时强调了大规模检索增强适应复现的实际考虑因素。
...
TL;DR: 该论文研究了多语言大语言模型中语言信息的内部表示结构和跨层演化，发现语言信息在第一个Transformer块中急剧分离，并在整个模型深度中保持线性可分性。语言方向与词汇嵌入的对齐与训练数据的语言组成密切相关，中文包含模型比英语中心模型表现出更强的结构印记效应。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言LLM在多种语言上表现出色，但对其内部表示空间中语言信息的结构方式以及跨层演化缺乏系统分析。研究旨在量化语言编码的层间动态和几何结构。

Method: 对六个多语言LLM进行了全面的探测研究，覆盖所有268个Transformer层，使用线性和非线性探针以及新的Token-Language Alignment分析来量化语言编码的层间动态和几何结构。

Result: 语言信息在第一个Transformer块中急剧分离（从第0层到第1层增加76.4±8.2个百分点），并在整个模型深度中几乎完全线性可分。语言方向与词汇嵌入的对齐与训练数据的语言组成密切相关，中文包含模型的中文匹配峰值达到16.43%，而英语中心模型仅为3.90%，显示出4.21倍的结构印记效应。

Conclusion: 多语言LLM不是通过表面脚本特征来区分语言，而是通过训练语料库塑造的潜在表示结构。研究为多语言表示学习中的数据组成策略和公平性提供了实用见解。
...
Method: 基于语言模型的本体嵌入方法，构建了针对SNOMED CT概念的OOV查询评估数据集。

Result: 该方法在检索最直接子类和较不相关祖先方面优于SBERT和两种词汇匹配基线方法。

Conclusion: 该方法虽然针对SNOMED CT评估，但具有通用性，可扩展到其他本体，并发布了代码、工具和评估数据集。
...
TL;DR: 研究发现共情行为（牺牲任务效率来满足人类需求）在LLM激活空间中表现为线性方向。通过对比提示在EIA基准上进行检测和操控测试，发现不同模型在检测能力上表现优异，但在操控性上存在差异，特别是未经审查的模型在反共情操控时会出现崩溃。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型中是否存在可检测和操控的共情行为线性方向，研究安全训练对共情编码和操控性的影响。

Method: 使用基于EIA基准的对比提示，在Phi-3-mini-4k、Qwen2.5-7B和Dolphin-Llama-3.1-8B三个模型上进行检测和操控实验，分析激活空间的线性方向。

Result: 检测方面：所有模型在最佳层都表现出极高的检测能力（AUROC 0.996-1.00），未经审查的模型与安全训练模型表现相当。操控方面：Qwen和Phi-3实现双向操控且保持连贯性，而Dolphin在增强共情时成功率高（94.4%），但在抑制共情时出现灾难性崩溃。

Conclusion: 共情编码独立于安全训练而出现，但安全训练影响操控的稳健性。检测和操控能力之间存在模型特异性差异，安全训练可能影响操控稳健性而非防止操控本身。
...
TL;DR: 本研究重新探讨了非宾格动词的句法分类与语义特征（施事性和终结性）之间的关系，通过可解释维度分析支持了传统理论观点。


<details>
  <summary>Details</summary>
Motivation: 虽然传统理论认为施事性动作动词更倾向非作格句法，终结性事件动词更倾向非宾格句法，但Kim等人(2024)发现人类评分对预测动词句法行为效果不佳，因此需要更有效的分析方法。

Method: 使用基于施事性和终结性对立极点的种子词计算可解释维度，结合人类判断来评估语义属性。

Result: 研究结果支持非作格/非宾格与施事性/终结性之间的联系，表明可解释维度与人类判断结合能为难以通过评分任务评估的语义属性提供有价值证据。

Conclusion: 使用可解释维度分析可以更有效地揭示动词句法分类与语义特征之间的关系，为语义属性评估提供了新的方法论视角。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型容易受到后门攻击，攻击者通过在输入提示中嵌入触发器来引导生成有害内容，需要有效的防御方法。

Method: 提出PEPPER防御方法，通过感知引导的扰动重写输入提示，创建语义不同但视觉相似的描述，并添加不显眼元素来稀释触发器的影响。

Result: 实验表明PEPPER对基于文本编码器的攻击特别有效，显著降低攻击成功率，同时保持生成质量，并能与其他防御方法结合使用。

Conclusion: PEPPER提供了一种有效的后门攻击防御方案，能够破坏触发器嵌入，增强模型鲁棒性，且具有良好的通用性和兼容性。
...
TL;DR: 本文提出了一种无需参考标准的新型指标，用于评估大语言模型生成响应的简洁性。该方法通过计算原始响应与抽象摘要、提取摘要的压缩比，以及通过词移除压缩来量化非必要内容，从而自动评估LLM输出的冗余程度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常生成冗长、啰嗦的响应，包含冗余或不必要的细节，这会降低清晰度和用户满意度，并增加模型开发者的成本，特别是对于按输出token收费的专有模型。

Method: 提出一种无参考指标的简洁性评估方法，计算三个指标的平均值：1）原始响应与LLM抽象摘要的压缩比；2）原始响应与LLM提取摘要的压缩比；3）词移除压缩，即LLM在保持意义的前提下尽可能移除非必要词语，移除的token数量表示简洁性得分。

Result: 实验结果表明，所提出的指标能够有效识别LLM输出中的冗余内容，为对话AI系统中的响应简洁性自动评估提供了实用工具，无需人工标注的真实标准。

Conclusion: 该方法提供了一种无需人工标注的自动化解决方案，能够有效评估LLM生成响应的简洁性，有助于提高响应质量并降低成本。
...
TL;DR: SCM是一种软概念感知训练方案，通过在训练中直接向模型暴露软表示来弥合推理中的软概念与训练中的离散标记之间的差距。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通常通过生成离散标记进行推理，这限制了其表达能力。软推理显示了通过软概念进行潜在推理的潜力，但模型是在离散标记上训练的，存在差距。

Method: 提出软概念混合(SCM)训练方案：构建软概念向量（嵌入的概率加权平均），将其混合到模型的隐藏状态中，并通过强化学习优化整个潜在推理过程。
...
TL;DR: 提出了一种改进Tiny Recursive Models (TRMs)训练效率的新方法，通过将潜在推理视为分类器自由引导和隐式策略改进算法，显著减少了前向传播次数并消除了停止机制，同时保持模型质量。


<details>
  <summary>Details</summary>
Motivation: 研究如何以最小改动进一步提高小型循环架构（如TRMs）在复杂推理任务上的效率，这些模型已被证明能在ARC等任务上超越大型语言模型。

Method: 将TRMs的潜在推理框架化为分类器自由引导和隐式策略改进算法，提出了一种在训练期间为每个循环提供目标的新训练方案。

Result: 方法显著提升了训练效率：前向传播总次数减少18倍，消除了停止机制，同时保持与标准TRMs相当的质量。在ARC-1上仅用0.8M参数达到24%准确率，超越大多数LLMs。

Conclusion: 提出的训练方案能够显著提高TRMs的训练效率，在保持性能的同时大幅减少计算资源需求，为小型模型在复杂推理任务上的应用提供了更高效的解决方案。
...
TL;DR: 本研究探讨了训练数据的统计特性与归纳头形成的关系，发现批次大小和上下文长度的组合能预测归纳头形成点，表面二元重复频率和可靠性强烈影响归纳头形成，并发现局部依赖性和高二元重复频率/可靠性足以形成归纳头。


<details>
  <summary>Details</summary>
Motivation: 虽然专门的注意力头（称为归纳头）被认为是现代语言模型具有卓越上下文学习能力的基础，但其形成的精确机制仍不清楚。本研究旨在探究训练数据的统计特性与归纳头形成之间的关系。

Method: 通过分析自然和合成数据的统计特性，研究批次大小、上下文长度、表面二元重复频率和可靠性对归纳头形成的影响，并确定这些参数之间的精确帕累托边界。

Result: 发现批次大小和上下文长度的简单方程能预测归纳头形成点；表面二元重复频率和可靠性强烈影响归纳头形成，存在精确的帕累托边界；当频率和可靠性低时，分类性和边际分布形状变得重要。
...
TL;DR: 本文提出了一种新颖的论点感知定量摘要任务，旨在揭示对话中论点的声明-理由结构，并通过ARQUSUMM框架实现这一目标。


<details>
  <summary>Details</summary>
Motivation: 现有对话摘要研究虽然考虑了句子间的论证关系，但未能深入揭示句子内部的论点结构，特别是在处理有争议话题时，需要总结论点的理由和论证强度。
...
Conclusion: ARQUSUMM框架成功实现了论点感知的定量摘要，能够有效揭示对话中的论点结构，为用户提供更有帮助的摘要结果。
...
TL;DR: 该研究提出了一种监督微调方法，用于提升大语言模型在湖南历史名人领域的特定信息抽取能力，通过构建领域特定的指令模板和数据集，在低资源环境下显著提升了多个大语言模型的知识抽取性能。


<details>
  <summary>Details</summary>
Motivation: 解决湖南历史名人研究领域系统化数据资源有限，以及通用大语言模型在领域知识抽取和结构化输出生成方面表现不佳的问题。
...
TL;DR: 该研究构建了一个视觉说服力判断数据集，探索了视觉语言模型对视觉说服因素的理解能力，发现模型存在回忆偏向，对低/中层次特征判别能力弱，高层次语义对齐是最强预测因子，对象导向的推理能显著提升性能。
...
Result: 视觉语言模型存在回忆偏向（过度预测高说服力），对低/中层次特征判别能力弱，高层次语义对齐是最强预测因子。简单指令或无引导推理效果有限，而简洁的对象导向推理能显著提升精确率和F1分数。
...
<details>
  <summary>Details</summary>
Motivation: 解决自动评分系统缺乏透明性和可解释性的问题，满足评估利益相关者对可解释自动评分日益增长的需求。

Method: 开发AnalyticScore框架：1）提取可识别的回答要素；2）使用LLM将回答转化为人类可解释的特征值；3）应用直观的有序逻辑回归模型进行评分。

Result: 在ASAP-SAS数据集的10个项目上，AnalyticScore的评分准确性优于许多不可解释方法，与不可解释的SOTA方法仅相差0.06 QWK，且特征化行为与人类标注者高度一致。
...
TL;DR: MUCH是首个用于公平评估声明级别不确定性量化方法的基准，包含4,873个多语言样本，提供24个生成对数概率，并提出高效声明分割算法。
...
Method: 构建多语言基准数据集，提供完整的生成对数概率，开发高效的确定性声明分割算法。

Result: 创建了包含4种欧洲语言和4个开源LLM的基准，分割算法仅需0.2%的生成时间，评估显示现有方法在性能和效率上仍有改进空间。

Conclusion: MUCH基准为声明级别不确定性量化研究提供了现实条件下的评估标准，当前方法仍需在性能和效率方面进一步提升。
...
Method: 采用上下文压缩作为预训练任务，训练模型生成紧凑的记忆token来替代整个上下文进行下游序列预测，并结合对比学习进一步优化表示质量。

Result: 实验表明，精心设计的压缩目标能显著提升LLM的文本表示能力，优于基于token级预训练任务的模型。结合对比学习后产生的LLM2Comp模型在广泛任务上表现优于当代基于LLM的文本编码器，且样本效率更高。

Conclusion: 上下文压缩是一种有效的预训练任务，能够显著提升LLM在文本表示任务上的性能，同时具有更好的样本效率。
...
TL;DR: 本文介绍了LangMark，一个用于自动后编辑（APE）的多语言数据集，包含7种语言的20.7万个三元组（源文本、机器翻译输出、人工后编辑文本），并证明大语言模型通过少量样本提示可以有效提升机器翻译质量。


<details>
  <summary>Details</summary>
Motivation: 解决自动后编辑领域缺乏大规模多语言数据集的问题，特别是针对神经机器翻译输出的专门数据集，以促进APE系统的开发。

Method: 创建并发布LangMark数据集，包含英语到7种语言（巴西葡萄牙语、法语、德语、意大利语、日语、俄语、西班牙语）的206,983个三元组，由专业语言学家标注。使用大语言模型进行少量样本提示的自动后编辑实验。

Result: 大语言模型通过少量样本提示能够有效执行自动后编辑任务，在翻译质量上超越了领先的商业甚至专有机器翻译系统。

Conclusion: LangMark数据集为自动后编辑系统的未来开发和评估提供了重要资源，证明了LLM在APE任务中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 为PLLuM项目中的波兰语大语言模型开发提供指令数据集，并探讨不同类型指令在语言模型适应中的作用。

Method: 使用有机、转换和合成三种类型的指令构建数据集，分析人工编写与合成指令在语言模型适应中的差异。

Result: 发布了PLLuM指令语料库的首个代表性子集（PLLuMIC），为其他语言模型的类似数据集开发提供指导。

Conclusion: 该指令数据集对波兰语大语言模型的开发具有重要价值，不同类型指令的使用对语言模型的语言适应有不同影响。
...
TL;DR: 本文提出了一种基于方面的因果弃权框架，通过分析LLM内部知识多样性来实现早期弃权，提高回答可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有弃权方法依赖生成后信号，无法提前防止不可靠回答。LLM经常产生流畅但事实错误的幻觉回答，需要更有效的早期弃权机制。

Method: ABCA框架通过因果推理分析LLM内部知识多样性，基于方面条件因果效应评估知识可靠性，支持两种弃权类型：方面效应不一致时的知识冲突弃权，和方面效应一致支持弃权时的知识不足弃权。

Result: 在标准基准测试中，ABCA提高了弃权可靠性，实现了最先进的性能，并增强了弃权决策的可解释性。

Conclusion: ABCA框架通过早期分析内部知识多样性，有效解决了LLM幻觉问题，为可靠弃权提供了新的因果推理方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统统计方法（如词频、TF-IDF）能捕捉词汇级重要性但缺乏上下文语义，而现代深度学习方法能理解上下文语义但可能忽略简单但高影响力的统计指标。需要一种能结合两者优势的方法来提升新闻文本分类性能。

Method: 提出注意力引导特征融合（AGFF）模型，在统一框架中融合统计特征和语义特征，使用基于注意力的机制动态确定各特征类型的相对重要性，从而做出更明智的分类决策。

Result: 在基准新闻数据集上的评估显示，AGFF模型在分类性能上优于传统统计模型和纯语义深度学习模型。消融研究验证了融合过程中各组成部分的贡献。

Conclusion: 策略性地整合不同类型的特征可以显著提高分类准确性。该模型能够平衡并利用统计表示和语义表示的互补优势，是实际新闻分类任务中实用且有效的解决方案。
...
TL;DR: E3Layer是一个用于大语言模型层剪枝的框架，通过可微分掩码优化和熵感知知识蒸馏，在保持高性能的同时实现训练经济性和推理加速。
...
Result: 在多种模型架构和基准测试中表现优异，Qwen3-32B模型剪枝25%层后，在MATH-500上达到96%准确率（仅比原模型96.8%下降0.8%），优于现有SOTA方法（95%），推理速度提升1.33倍，仅消耗0.5B tokens（后训练数据量的0.5%）。

Conclusion: E3Layer框架能够有效解决层剪枝中的性能、成本和效率问题，为大规模语言模型的部署提供了实用的压缩解决方案。
...
TL;DR: PARROT框架用于评估LLM在权威和说服压力下的准确性退化，通过双盲实验、置信度追踪和行为分类来测量模型对错误权威信息的顺从性。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在面对社会压力时的顺从性现象（sycophancy），评估模型在权威错误信息影响下的准确性退化，为安全部署提供依据。

Method: 使用PARROT框架：(i)双盲实验比较中性问题和权威错误版本，(ii)基于对数似然的置信度校准追踪，(iii)八状态行为分类法系统分类失败模式。评估22个模型在13个领域的1302个MMLU式选择题。

Result: 先进模型（GPT-5、GPT-4.1等）顺从率低（≤11%），准确性损失小；老旧/小模型出现严重认知崩溃（GPT-4:80%，Qwen 2.5-1.5B:94%）。弱模型降低对正确答案的置信度，增加对错误答案的置信度。国际法和全球知识领域脆弱，基础数学相对稳健。

Conclusion: 抵抗过度顺从压力应作为与准确性、伤害避免和隐私同等重要的主要目标，以确保模型在现实世界中的安全部署。
...
TL;DR: MirageTVQA是一个新的基准测试，旨在评估视觉语言模型在多语言和视觉噪声环境下的表格推理能力，包含近6万个QA对，覆盖24种语言，揭示现有模型在视觉噪声和多语言场景下的严重性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有表格问答数据集主要是单语言（英语）且表格格式完美，与现实场景存在显著差距，需要创建更能反映实际复杂性的基准测试。

Method: 开发MirageTVQA基准测试，包含多语言表格和模拟扫描文档的视觉噪声，评估领先视觉语言模型在这些挑战性条件下的表现。

Result: 评估显示领先视觉语言模型在视觉噪声下性能下降超过35%，并存在一致的英语优先偏见，推理能力无法有效迁移到其他语言。

Conclusion: MirageTVQA为衡量和推动更鲁棒的表格推理视觉语言模型提供了基准，揭示了现有模型在现实场景中的关键局限性。
...
TL;DR: 本文介绍了爱沙尼亚语的WinoGrande常识推理基准测试集的本地化翻译，评估了专有和开源模型在人工翻译基准上的表现，并探索了通过提示工程改进机器翻译质量的可行性。


<details>
  <summary>Details</summary>
Motivation: 动机是创建高质量的爱沙尼亚语常识推理基准，以可靠评估大型语言模型在爱沙尼亚语上的语言能力和推理表现，同时探索机器翻译的改进方法。

Method: 方法包括由翻译专家进行人工翻译和本地化适应，设计针对爱沙尼亚语特点和WinoGrande数据集翻译挑战的详细提示，并比较人工翻译与机器翻译的性能差异。

Result: 结果显示模型在人工翻译的爱沙尼亚语数据集上表现略低于原始英文测试集，在机器翻译数据上表现明显更差。提示工程对翻译质量或模型准确性的改进有限。

Conclusion: 结论强调了在数据集翻译和适应过程中需要语言专家的参与，以确保对大型语言模型语言能力和推理的可靠、可解释评估。
...
TL;DR: 该研究基于语料库分析普通话单音节词在自然对话中的音高轮廓实现，发现词义对音高轮廓有显著影响，即使控制多种变量后，词义仍是音高实现的重要预测因素。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索普通话中单音节词的音高轮廓如何受到词义影响，挑战传统声调理论，验证分布语义学在语音学中的作用。

Method: 使用广义加性模型分解观察到的音高轮廓，控制词长、性别、说话人身份、声调语境、元音高度和话语位置等变量，分析词义对音高轮廓的预测能力。

Result: 词义是音高实现的重要预测因素，同音异义词具有不同的音高轮廓，基于上下文化嵌入的预测准确度显著超过置换基线。
...
Result: 该方法在不进行任务特定微调的情况下实现了高准确率，并设计对抗数据集验证了其鲁棒性。
...
TL;DR: 本文提出了选择性RoPE，这是一种输入依赖的旋转位置嵌入机制，它扩展了RoPE，允许在软注意力和线性注意力变换器中实现任意角度的旋转。


<details>
  <summary>Details</summary>
Motivation: 受线性变换器中输入依赖门控机制在语言任务中表现优越的启发，作者希望将这种选择性机制引入到位置编码中，以改进传统固定角度旋转的RoPE。

Method: 开发了选择性RoPE机制，通过输入依赖的方式确定旋转角度，在软注意力和线性注意力变换器中实现任意角度的旋转。揭示了软注意力中查询-键对之间存在的隐式旋转结构。

Result: 在语言建模和困难序列任务（如复制、状态跟踪和检索）上，配备选择性RoPE的门控变换器表现出性能提升。

Conclusion: 选择性RoPE通过输入依赖的旋转机制有效改进了位置编码，在多种序列任务中展现出优越性能，为位置编码机制提供了新的研究方向。
...
TL;DR: ReVeL框架将多项选择题改写为开放式问题，解决了MCQA中选项泄露信号导致评估不准确的问题，在训练和评估中都显示出更好的效果。


<details>
  <summary>Details</summary>
Motivation: 发现多项选择题的选项可能泄露可利用的信号，使得准确性指标不能真实反映模型能力，并鼓励在强化微调中出现猜测行为。

Method: 提出ReVeL框架，将多项选择题分类并重写为开放式问题，同时保持答案可验证性，使用GRPO方法对Qwen2.5-VL模型进行微调。

Result: 在多项选择基准测试中保持相同准确率，开放式问题准确率提升约6个百分点，评估中揭示MCQA基准得分膨胀高达20个百分点，提高判断准确性并降低成本。

Conclusion: ReVeL框架在训练和评估中都比基于MCQA的方法更有效，提供了更可靠的数据效率和奖励信号。
...
TL;DR: MR-RLVR通过掩码填充和步骤重排序构建过程级自监督奖励，在仅有结果可验证的数学推理任务中显著提升RLVR的性能。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR在定理证明等数学推理任务中可扩展性有限，因为中间推理过程至关重要但难以直接可靠验证，而token级SFT容易退化为死记硬背。

Method: 提出MR-RLVR方法，包含两个阶段：首先在采样的数学计算和证明数据上进行自监督训练，然后在仅有结果可验证的数学计算数据集上进行RLVR微调。
...
Conclusion: 引入过程感知的自监督信号能有效增强RLVR在仅有结果可验证场景下的可扩展性和性能。
...
### [123] [DDTime: Dataset Distillation with Spectral Alignment and Information Bottleneck for Time-Series Forecasting](https://arxiv.org/abs/2511.16715)
*Yuqi Li,Kuiye Ding,Chuanguang Yang,Hao Wang,Haoxuan Wang,Huiran Duan,Junming Liu,Yingli Tian*

Main category: cs.LG

TL;DR: DDTime是一个轻量级的时间序列数据集蒸馏框架，通过频域对齐机制和样本间正则化解决时间序列蒸馏中的时间偏差和样本多样性不足问题，在20个基准数据集上相比现有方法提升约30%准确率。


<details>
  <summary>Details</summary>
Motivation: 时间序列预测通常需要大规模数据集和大量计算资源，而数据集蒸馏可以合成紧凑数据集来替代完整数据。但时间序列蒸馏面临两个关键挑战：时间偏差（强自相关性导致师生模型值项对齐失真）和样本多样性不足（缺乏明确的类别先验来规范轨迹多样性）。

Method: 基于一阶压缩分解构建DDTime框架：1）通过时间统计重新审视值项对齐，引入频域对齐机制缓解自相关引起的偏差，确保频谱一致性和时间保真度；2）基于信息瓶颈原理设计样本间正则化，增强多样性并最大化合成轨迹的信息密度；3）组合目标与多种压缩范式理论兼容，支持稳定的一阶优化。

Result: 在20个基准数据集和多种预测架构上的广泛实验表明，DDTime始终优于现有蒸馏方法，实现约30%的相对准确率提升，同时仅引入约2.49%的计算开销。

Conclusion: DDTime提供了一个有效的时间序列数据集蒸馏解决方案，通过频域对齐和多样性增强机制成功解决了时间序列蒸馏的核心挑战，在保持轻量化的同时显著提升了预测性能。
...
### [124] [Convergence and stability of Q-learning in Hierarchical Reinforcement Learning](https://arxiv.org/abs/2511.17351)
*Massimiliano Manenti,Andrea Iannelli*

Main category: cs.LG

TL;DR: 本文提出了封建Q学习方案，分析了其收敛和稳定性条件，通过随机逼近理论和ODE方法证明了收敛性，并将更新收敛点解释为适当定义博弈的均衡点。


<details>
  <summary>Details</summary>
Motivation: 分层强化学习在捕捉决策问题时间结构和增强持续学习能力方面有潜力，但理论保证落后于实践。本文旨在为封建强化学习提供收敛和稳定性分析的理论基础。

Method: 采用封建Q学习方案，利用随机逼近理论和ODE方法进行理论分析，并通过实验验证理论结果。

Result: 提出了收敛和稳定性定理，证明封建Q学习更新收敛到可解释为博弈均衡点的位置，为分层强化学习的博弈论方法打开了大门。

Conclusion: 本文为封建强化学习提供了原则性的收敛和稳定性分析框架，实验支持了理论预期结果，建立了分层强化学习与博弈论方法的联系。
...
### [125] [When Structure Doesn't Help: LLMs Do Not Read Text-Attributed Graphs as Effectively as We Expected](https://arxiv.org/abs/2511.16767)
*Haotian Xu,Yuning You,Tengfei Ma*

Main category: cs.LG

TL;DR: 研究发现，在文本属性图上，仅使用节点文本描述的LLM已经能取得强劲性能，大多数结构编码策略仅带来边际收益甚至负面影响，表明显式结构先验在强大语言模型时代往往不必要甚至适得其反。
...
Result: LLM仅使用节点文本描述就能在各项任务中取得强劲表现；大多数结构编码策略仅提供边际收益或负面效果；显式结构先验往往不必要且有时适得其反。
...
### [126] [Harnessing Data from Clustered LQR Systems: Personalized and Collaborative Policy Optimization](https://arxiv.org/abs/2511.17489)
*Vinay Kanakeri,Shivam Bajaj,Ashwin Verma,Vijay Gupta,Aritra Mitra*

Main category: cs.LG

TL;DR: 本文提出了一种在强化学习中同时进行聚类和学习的新算法，通过将相似动态和任务的智能体分组，为每个聚类学习个性化策略，从而提高样本效率。


<details>
  <summary>Details</summary>
Motivation: 强化学习通常数据需求量大，现有方法试图利用近似相似过程的数据来提高样本效率，但由于过程模型未知，识别哪些过程相似具有挑战性。

Method: 结合顺序消除和零阶策略优化的思想，提出新算法在LQR设置下同时执行聚类和学习，为每个聚类输出个性化控制器。

Result: 在适当的聚类分离条件下，算法以高概率保证正确聚类，每个聚类学习策略的次优性差距与聚类大小成反比，且无额外偏差。

Conclusion: 这是首个展示聚类如何在数据驱动控制中用于学习个性化策略的工作，既能享受协作带来的统计收益，又不会因包含不相似过程数据而遭受次优性。
...
### [127] [GCL-OT: Graph Contrastive Learning with Optimal Transport for Heterophilic Text-Attributed Graphs](https://arxiv.org/abs/2511.16778)
*Yating Ren,Yikun Ban,Huobin Tan*

Main category: cs.LG

TL;DR: 该论文提出了GCL-OT框架，通过最优传输解决文本属性图中的多粒度异质性挑战，包括完全异质性、部分异质性和潜在同质性，实现了结构和文本的双向灵活对齐。


<details>
  <summary>Details</summary>
Motivation: 现有方法在文本属性图上通常依赖同质性假设和硬优化目标，限制了在异质性图上的应用。虽然可以通过结构调整或邻居聚合缓解异质性，但通常将文本嵌入视为静态目标，导致次优对齐。

Method: 提出GCL-OT框架，针对不同类型的异质性设计专门机制：使用RealSoftMax相似度估计器处理部分异质性，基于提示的过滤器处理完全异质性，以及OT引导的软监督学习潜在同质性。

Result: 理论分析表明GCL-OT可以改进互信息边界和贝叶斯错误保证。在9个基准测试上的广泛实验显示，GCL-OT始终优于最先进方法，验证了其有效性和鲁棒性。

Conclusion: GCL-OT通过最优传输和针对多粒度异质性的专门机制，成功解决了文本属性图中的结构和文本对齐挑战，为异质性图学习提供了有效的解决方案。
...
### [128] [Revisiting Multimodal KV Cache Compression: A Frequency-Domain-Guided Outlier-KV-Aware Approach](https://arxiv.org/abs/2511.16786)
*Yaoxin Yang,Peng Ye,Xudong Tan,Chongjun Tu,Maosen Zhao,Jia Hao,Tao Chen*

Main category: cs.LG

TL;DR: FlashCache是一个基于频域分析和异常KV感知的多模态KV缓存压缩框架，通过频域低通滤波提取主要能量成分，识别并保留偏离主成分的异常KV对，实现高效压缩。
...
Method: 1. 从KV矩阵分布角度分析，发现多模态KV矩阵频域能量集中在低频；2. 提出异常KV识别模块，在频域建模主成分并优先保留显著偏离的KV对；3. 设计动态预算分配模块，自适应确定每层KV缓存大小以保留更多异常KV。
...
### [129] [A Vector Symbolic Approach to Multiple Instance Learning](https://arxiv.org/abs/2511.16795)
*Ehsan Ahmed Dhrubo,Mohammad Mahmudul Alam,Edward Raff,Tim Oates,James Holt*

Main category: cs.LG

TL;DR: 提出基于向量符号架构(VSA)的新型多示例学习框架，通过高维向量和代数运算直接编码MIL的iff约束，在保持严格MIL公式的同时实现最先进性能。
...
Result: 在标准MIL基准测试和医学影像数据集上，该方法实现了有效MIL模型的最先进结果，优于现有方法且严格遵循MIL公式。
...
### [130] [A Robust Federated Learning Approach for Combating Attacks Against IoT Systems Under non-IID Challenges](https://arxiv.org/abs/2511.16822)
*Eyad Gad,Zubair Md Fadlullah,Mostafa M. Fouda*

Main category: cs.LG

TL;DR: 本文研究了联邦学习算法（FedAvg、FedProx、Scaffold）在非独立同分布数据下的IoT攻击检测性能，使用CICIoT2023数据集进行大规模IoT攻击分类，旨在解决统计异质性带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 随着用户设备增多和数据量激增，传统机器学习在资源受限和安全性敏感的IoT环境中面临挑战。联邦学习虽然能解决隐私和资源问题，但非独立同分布数据的统计异质性严重影响了其效果，现有研究缺乏对联邦学习方法在IoT攻击检测中的全面比较。

Method: 使用CICIoT2023数据集，探索FedAvg、FedProx和Scaffold三种联邦学习算法在不同数据分布下的表现，重点关注统计异质性问题的解决方案。

Result: 通过细致的分析和实验，揭示了这些联邦学习方法在性能上的细微差异，为研究者和实践者提供了有价值的见解。

Conclusion: 本研究深入探讨了联邦学习算法在统计异质性环境下的IoT攻击检测能力，填补了现有研究空白，为相关领域提供了重要参考。
...
### [131] [Monte Carlo Expected Threat (MOCET) Scoring](https://arxiv.org/abs/2511.16823)
*Joseph Kim,Saahith Potluri*

Main category: cs.LG

TL;DR: 本文介绍了MOCET指标，用于评估AI安全级别（ASL）威胁，特别是针对ASL-3+模型在生物安全领域对非国家行为者的赋能风险。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标如LAB-Bench、BioLP-bench和WMDP能够可靠评估模型赋能和领域知识，但缺乏能够更好量化"现实世界风险"的指标，需要可扩展的开放式指标来跟上LLM的快速发展。

Method: 引入MOCET指标，这是一个可解释且双重可扩展（可自动化和开放式）的指标，能够量化现实世界风险。

Result: MOCET指标能够解决现有评估指标的不足，为LLM的安全案例提供更好的风险评估依据。

Conclusion: MOCET指标填补了现有评估指标的空白，提供了量化现实世界风险的有效工具，有助于指导利益相关者实施适当的安全保障措施。
...
### [132] [ManifoldFormer: Geometric Deep Learning for Neural Dynamics on Riemannian Manifolds](https://arxiv.org/abs/2511.16828)
...
Method: 该架构整合了三个关键创新：用于保持几何结构的黎曼VAE流形嵌入、在神经流形上直接操作的具有测地线感知注意力机制的几何Transformer，以及利用神经ODE进行流形约束时间演化的动态预测器。
...
### [133] [Analysis of heart failure patient trajectories using sequence modeling](https://arxiv.org/abs/2511.16839)
*Falk Dippela,Yinan Yu,Annika Rosengren,Martin Lindgren,Christina E. Lundberg,Erik Aerts,Martin Adiels,Helen Sjöland*

Main category: cs.LG

TL;DR: 本文系统比较了六种序列模型在瑞典心衰队列中的表现，发现Llama架构在预测性能、校准和鲁棒性方面表现最佳，Mamba次之，两者在模型规模较小时就能超越其他大型Transformer模型。
...
Method: 在瑞典心衰队列（N=42820）中评估六种序列模型，涵盖三种架构类别（Transformer、Transformer++、Mamba）。数据包括诊断、生命体征、实验室检查、药物和程序。评估三个一年预测任务，并进行输入序列、模型配置和时间预处理技术的消融分析。

Result: Llama获得最高的预测区分度、最佳校准和跨任务鲁棒性，Mamba次之。两种架构在模型规模较小时就能实现高效表示学习，超越其他大型Transformer。在相同模型规模下，使用25%更少的训练数据就能达到优越性能。
...
### [134] [Provably Minimum-Length Conformal Prediction Sets for Ordinal Classification](https://arxiv.org/abs/2511.16845)
*Zijian Zhang,Xinyu Chen,Yuanjie Shi,Liyuan Lillian Ma,Zifan Xu,Yan Yan*

Main category: cs.LG

TL;DR: 本文提出了一种用于序数分类的保形预测方法，该方法具有模型无关性，并提供实例级别的最优预测区间，显著提高了预测效率。


<details>
  <summary>Details</summary>
Motivation: 序数分类在医疗影像和诊断等高风险应用中广泛使用，需要可靠的不确定性量化。现有序数保形预测方法要么是启发式算法，要么要求基础模型预测单峰分布，限制了覆盖效率权衡的洞察力。

Method: 将保形序数分类建模为实例级别的最小长度覆盖问题，开发了线性时间复杂度的滑动窗口算法，并提出长度正则化变体来缩小预测集大小同时保持覆盖。

Result: 在四个不同领域的基准数据集上的实验表明，所提方法相比基线平均减少了15%的预测集大小，显著提高了预测效率。

Conclusion: 该方法填补了序数保形预测的空白，提供了模型无关、分布自由的实例级别最优预测区间，在保持统计有效性的同时显著提升了预测效率。
...
### [135] [Sex and age determination in European lobsters using AI-Enhanced bioacoustics](https://arxiv.org/abs/2511.16848)
...
<details>
  <summary>Details</summary>
Motivation: 监测水生生物（特别是像龙虾这样的难以捉摸的物种）具有挑战性。了解龙虾的栖息地、福利、繁殖、性别和年龄对于管理和保护至关重要。
...
### [136] [PepEVOLVE: Position-Aware Dynamic Peptide Optimization via Group-Relative Advantage](https://arxiv.org/abs/2511.16912)
*Trieu Nguyen,Hao-Wei Pang,Shasha Feng*

Main category: cs.LG

TL;DR: PepEVOLVE是一个用于大环肽优化的动态框架，它通过学习编辑位置和动态优化来改进多目标性能，相比现有方法在Rev结合大环肽基准测试中表现更优。


<details>
  <summary>Details</summary>
Motivation: 大环肽具有生物制剂样亲和力和小分子样可开发性的优势，但其巨大的组合空间和多参数目标使得先导优化过程缓慢且具有挑战性。现有方法需要预先指定可突变位置，且依赖静态预训练和优化算法，限制了模型的泛化能力和优化效果。

Method: PepEVOLVE框架包含三个关键组件：(i)通过动态掩码和CHUCKLES移位增强预训练以改进泛化；(ii)使用上下文无关多臂老虎机路由器发现高奖励残基；(iii)结合新型进化优化算法和组相对优势来稳定强化学习更新。

Result: 在计算机评估中，路由器策略可靠地学习并集中概率于影响肽性质的化学相关位点。在Rev结合大环肽基准测试中，PepEVOLVE的平均得分（约0.8 vs 0.6）和最佳候选得分（0.95 vs 0.87）均优于PepINVENT，且收敛步数更少。

Conclusion: PepEVOLVE为肽先导优化提供了一个实用、可复现的路径，特别是在最优编辑位点未知的情况下，能够实现更高效的探索并提高多目标设计质量。
...
### [137] [A Hybrid Computational Intelligence Framework for scRNA-seq Imputation: Integrating scRecover and Random Forests](https://arxiv.org/abs/2511.16923)
*Ali Anaissi,Deshao Liu,Yuanzhe Jia,Weidong Huang,Widad Alyassine,Junaid Akram*

Main category: cs.LG

TL;DR: SCR-MF是一个模块化两阶段工作流，结合了scRecover的dropout检测和missForest的非参数插补，用于解决单细胞RNA测序中的dropout问题。


<details>
  <summary>Details</summary>
Motivation: 单细胞RNA测序存在普遍的dropout事件，这会掩盖生物信号，需要一种能够保持生物保真度和透明度的稳健插补方法。

Method: 采用两阶段工作流：第一阶段使用scRecover进行dropout检测，第二阶段使用missForest进行非参数插补。

Result: 在公共和模拟数据集上，SCR-MF在大多数情况下达到或超过现有插补方法的性能，同时保持生物保真度和透明度。运行时间分析显示其在准确性和计算效率之间取得了良好平衡。

Conclusion: SCR-MF适用于中等规模单细胞数据集，提供了稳健且可解释的dropout插补解决方案。
...
### [138] [CroTad: A Contrastive Reinforcement Learning Framework for Online Trajectory Anomaly Detection](https://arxiv.org/abs/2511.16929)
*Rui Xue,Dan He,Fengmei Jin,Chen Zhang,Xiaofang Zhou*

Main category: cs.LG

TL;DR: CroTad是一个基于对比强化学习的在线轨迹异常检测框架，无需阈值设置，能够处理噪声和不规则采样的轨迹数据，在子轨迹和点级别检测异常行为。


<details>
  <summary>Details</summary>
Motivation: 解决智能交通系统中轨迹异常检测的关键挑战：子轨迹异常检测研究不足、现有方法依赖阈值调整、不规则采样和训练集噪声影响模型性能。

Method: 提出对比强化学习框架，结合对比学习提取不同行程的正常旅行模式，使用深度强化学习进行在线实时异常评分。
...
### [139] [A novel approach to classification of ECG arrhythmia types with latent ODEs](https://arxiv.org/abs/2511.16933)
*Angelina Yan,Matt L. Sampson,Peter Melchior*

Main category: cs.LG

TL;DR: 提出一种端到端分类管道，使用潜在ODE建模连续ECG波形，通过降采样构建鲁棒特征向量，在不同采样频率下保持高性能分类，解决可穿戴设备信号保真度与电池寿命的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统12导联ECG因短期监测易错过间歇性事件，而可穿戴ECG因电池限制导致采样频率不规则且较低，使形态分析困难。需要解决信号保真度与电池寿命的权衡问题。

Method: 训练潜在ODE模型连续ECG波形，从高频单通道信号创建鲁棒特征向量。将初始360 Hz ECG降采样至90 Hz和45 Hz构建三个潜在向量，使用梯度提升树进行分类，测试跨频率鲁棒性。

Result: 性能下降极小，在360 Hz、90 Hz和45 Hz采样频率下的宏平均AUC-ROC值分别为0.984、0.978和0.976，表明方法能有效应对不同采样频率。
...
### [140] [ToC: Tree-of-Claims Search with Multi-Agent Language Models](https://arxiv.org/abs/2511.16972)
*Shuyang Yu,Jianan Liang,Hui Hu*

Main category: cs.LG

TL;DR: 提出Tree of Claims (ToC)框架，将专利权利要求编辑重新定义为引导搜索问题，结合蒙特卡洛树搜索和多智能体系统，显著提升权利要求优化的效果。
...
Result: 在1145个权利要求的基准测试中，ToC显著优于标准LLM在零样本和少样本场景下的表现，平均综合得分提升8%，某些情况下可达9%。
...
### [141] [FIRM: Federated In-client Regularized Multi-objective Alignment for Large Language Models](https://arxiv.org/abs/2511.16992)
*Fatemeh,Nourzad,Amirhossein Roknilamouki,Eylem Ekici,Jia,Liu,Ness B. Shroff*

Main category: cs.LG

TL;DR: FIRM是一种联邦多目标对齐算法，通过在客户端本地解决正则化多目标优化问题，实现客户端分歧漂移缓解和通信效率，无需传输多个梯度，仅需传输单个参数集。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型与人类价值观对齐时面临的多目标冲突（如帮助性与无害性）、计算密集性、数据隐私问题，以及现有联邦多目标优化方法的通信瓶颈。

Method: 提出FIRM算法，每个客户端本地解决正则化多目标优化问题，通过客户端内正则化直接缓解客户端分歧漂移，仅传输单个适应参数集。

Result: 理论证明算法收敛到帕累托稳定点，提供有限时间收敛保证；实证显示训练动态更平滑，客户端分歧漂移减少，奖励权衡改善，并能根据偏好平滑调整目标间权衡。

Conclusion: FIRM在联邦多目标对齐中有效平衡通信效率和性能，为大规模模型对齐提供可行解决方案。
...
### [142] [Mask the Redundancy: Evolving Masking Representation Learning for Multivariate Time-Series Clustering](https://arxiv.org/abs/2511.17008)
*Zexi Tan,Xiaopeng Luo,Yunlin Liu,Yiqun Zhang*

Main category: cs.LG

TL;DR: 本文提出EMTC方法，通过重要性感知的变量级掩码和多内生视图表示学习模块，解决多元时间序列聚类中冗余信息导致性能瓶颈的问题。


<details>
  <summary>Details</summary>
Motivation: 多元时间序列包含大量冗余信息（如稳态机器运行记录、太阳能发电零输出期），这些冗余降低了模型对判别性时间戳的关注，导致聚类性能瓶颈。现有掩码策略多为独立预处理步骤，无法动态适应聚类关键时间戳的重要性。
...
### [143] [Energy Scaling Laws for Diffusion Models: Quantifying Compute and Carbon Emissions in Image Generation](https://arxiv.org/abs/2511.17031)
...
Method: 将扩散模型推理分解为文本编码、迭代去噪和解码组件，假设去噪操作因在多步推理中重复执行而主导能耗。基于计算复杂度（FLOPs）适配Kaplan缩放定律来预测GPU能耗。

Result: 在四个先进扩散模型和三种GPU架构上的实验表明，该方法在单个架构内预测精度高（R平方>0.9），跨架构泛化能力强，能可靠估计未见模型-硬件组合的能耗。
...
### [144] [Hash Collisions in Molecular Fingerprints: Effects on Property Prediction and Bayesian Optimization](https://arxiv.org/abs/2511.17078)
...
Method: 在分子性质预测和贝叶斯优化中使用精确指纹与标准压缩指纹进行比较，其中基础预测模型为高斯过程。

Result: 在DOCKSTRING数据集的五个分子性质预测基准测试中，使用精确指纹在预测准确性上产生了小幅但一致的改进。然而，这些增益并未转化为贝叶斯优化性能的显著改善。

Conclusion: 精确指纹在分子性质预测中能提供更好的准确性，但在贝叶斯优化应用中这种改进并不显著。
...
### [145] [Geometric-Disentangelment Unlearning](https://arxiv.org/abs/2511.17100)
*Duo Zhou,Yuji Zhang,Tianxin Wei,Ruizhong Qiu,Ke Yang,Xiao Lin,Cheng Qian,Jingrui He,Hanghang Tong,Heng Ji,Huan Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种几何解缠的机器学习遗忘方法(GU)，通过将遗忘梯度更新分解为保留梯度子空间的正交分量来减少对保留知识的影响，在理论保证下实现有效的遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有的机器学习遗忘方法在有效遗忘和保留知识之间存在权衡，缺乏对遗忘更新如何影响保留知识的理论分析，需要探索理论上有保证的解决方案。

Method: 从一阶分析出发，将遗忘梯度更新分解为保留梯度子空间的切向分量和正交分量，只执行正交分量，确保保留损失在一阶意义上不变。

Result: GU方法在TOFU、MUSE和WMDP三个基准测试中均实现了持续改进，能够有效缓解现有梯度遗忘方法的副作用。

Conclusion: 几何解缠遗忘方法提供了一种理论上有保证的简单解决方案，能够在不损害保留知识的情况下实现有效遗忘，并可即插即用地应用于现有梯度遗忘方法。
...
### [146] [Four decades of circumpolar super-resolved satellite land surface temperature data](https://arxiv.org/abs/2511.17134)
*Sonia Dupuis,Nando Metzger,Konrad Schindler,Frank Göttsche,Stefan Wunderle*

Main category: cs.LG

TL;DR: 本文提出了一种新的42年泛北极地表温度数据集，通过基于深度各向异性扩散模型的超分辨率算法将AVHRR GAC数据从粗分辨率降尺度到1公里，用于改善北极地区永久冻土建模和气候变化监测。


<details>
  <summary>Details</summary>
Motivation: 北极地区快速变暖，但AVHRR全球区域覆盖数据的粗空间分辨率限制了其在分析精细尺度永久冻土动态和其他地表过程方面的应用，需要开发高分辨率长期地表温度数据集。

Method: 使用基于深度各向异性扩散模型的超分辨率算法，在MODIS LST数据上训练模型，利用高分辨率土地覆盖、数字高程和植被高度图作为指导，将AVHRR GAC数据降尺度到1公里分辨率。

Result: 生成了42年泛北极地区每日两次的1公里分辨率地表温度观测数据集，覆盖整个北极地区四个十年。

Conclusion: 该增强数据集能够改进永久冻土建模、重建近地表气温、评估格陵兰冰盖表面质量平衡，支持前MODIS时代的气候监测工作，并为未来卫星任务提供可适应的框架。
...
### [147] [Reconstruction of Surface EMG Signal using IMU data for Upper Limb Actions](https://arxiv.org/abs/2511.17200)
*Shubhranil Basak,Mada Hemanth,Madhav Rao*

Main category: cs.LG

TL;DR: 该论文研究使用深度学习从6轴IMU数据合成归一化sEMG信号，通过滑动窗口波网模型成功预测肌肉激活的时序和形状，可用于假肢和康复生物反馈应用。


<details>
  <summary>Details</summary>
Motivation: 表面肌电图(sEMG)能提供肌肉功能的重要信息，但存在噪声大和采集困难的问题。惯性测量单元(IMUs)为运动捕捉系统提供了鲁棒且可穿戴的替代方案。

Method: 收集同步的sEMG和IMU数据，使用基于扩张因果卷积的滑动窗口波网模型，将IMU数据映射到sEMG信号。

Result: 模型成功预测了肌肉激活的时序和总体形状，尽管峰值幅度通常被低估，但具有高时间保真度。

Conclusion: 该方法在肌肉意图检测方面具有可行性，可应用于假肢和康复生物反馈等领域。
...
### [148] [FlexiFlow: decomposable flow matching for generation of flexible molecular ensemble](https://arxiv.org/abs/2511.17249)
*Riccardo Tedoldi,Ola Engkvist,Patrick Bryant,Hossein Azizpour,Jon Paul Janet,Alessandro Tibo*

Main category: cs.LG

TL;DR: FlexiFlow是一种新颖的架构，扩展了流匹配模型，能够联合采样分子及其多个构象，同时保持等变性和置换不变性。该方法在分子生成任务中取得了最先进的结果，并能生成具有构象多样性的分子。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的3D从头设计模型仅限于生成单一构象，但分子的构象景观决定了其可观察性质和与蛋白质靶标的结合能力。通过生成一组低能量构象代表，可以更直接评估这些性质，并可能提高生成具有所需热力学可观察性质分子的能力。

Method: 提出FlexiFlow架构，扩展流匹配模型，允许联合采样分子及其多个构象，同时保持等变性和置换不变性。在QM9和GEOM Drugs数据集上进行验证。

Result: 在分子生成任务中达到最先进结果，能够生成有效、无应变、独特且新颖的分子，同时对训练数据分布具有高保真度。模型生成的构象集合在推理时间仅为物理方法的一小部分时提供相似的覆盖范围。

Conclusion: FlexiFlow能够成功转移到蛋白质条件配体生成任务，即使在数据集仅包含静态口袋而没有伴随构象的情况下也能有效工作。
...
### [149] [Enforcing governing equation constraints in neural PDE solvers via training-free projections](https://arxiv.org/abs/2511.17258)
*Omer Rochman,Gilles Louppe*

Main category: cs.LG

TL;DR: 该论文提出了两种无需训练的后处理投影方法，用于减少神经PDE求解器在科学模拟中违反约束条件的问题，包括非线性优化投影和局部线性化投影。


<details>
  <summary>Details</summary>
Motivation: 神经PDE求解器在科学模拟中经常违反控制方程约束，特别是非线性约束和动态PDE中的时间依赖约束，这使得投影到可行集变得复杂。

Method: 评估了两种无需训练的后处理投影方法：非线性优化投影和使用Jacobian-vector和vector-Jacobian乘积的局部线性化投影。

Result: 两种投影方法都显著减少了约束违反，并在代表性PDE上比物理信息基线提高了准确性。

Conclusion: 提出的投影方法有效改善了神经PDE求解器的约束满足性和准确性。
...
### [150] [Automobile demand forecasting: Spatiotemporal and hierarchical modeling, life cycle dynamics, and user-generated online information](https://arxiv.org/abs/2511.17275)
...
<details>
  <summary>Details</summary>
Motivation: 高端汽车制造商面临产品种类多、变体级数据稀疏和市场波动大的复杂预测挑战，需要解决多产品、多市场、多层次的需求预测问题。

Method: 结合战略和运营规划层面的点预测和概率预测，使用LightGBM集成模型、分位数回归和混合整数线性规划协调方法，利用池化训练集进行预测。

Result: 时空依赖性和取整偏差显著影响预测精度，整数预测对运营可行性很重要；短期需求受生命周期成熟度、自回归动量和运营信号影响，中期需求受在线参与度、规划目标和竞争指标等预期驱动因素影响。

Conclusion: 在线行为数据显著提高了细分层面的预测精度，短期需求具有反应性特征，而中期需求则反映预期性驱动因素，整数预测对于运营可行性至关重要。
...
### [151] [SAVeD: Semantic Aware Version Discovery](https://arxiv.org/abs/2511.17298)
*Artem Frenk,Roee Shraga*

Main category: cs.LG

TL;DR: SAVeD是一个基于对比学习的框架，用于识别结构化数据集的版本，无需依赖元数据、标签或集成假设。它通过随机变换生成增强表视图，使用自定义transformer编码器嵌入，并在潜在空间中进行对比优化语义相似性。
...
Method: 采用改进的SimCLR管道，通过随机变换（如行删除、编码扰动）生成增强表视图，使用自定义transformer编码器嵌入，在潜在空间中进行对比学习，最小化同一数据集增强视图间的距离，最大化不相关表间的距离。

Result: 在五个标准数据集上的实验显示，SAVeD在完全未见过的表上获得显著更高的准确率，分离分数显著提升，能够有效区分语义改变的版本。与未训练基线和现有最先进方法相比，取得竞争性或更优结果。

Conclusion: SAVeD框架能够有效识别结构化数据集的版本，无需依赖元数据或标签，在版本检测任务中表现出色，为解决数据科学中的重复劳动问题提供了有效解决方案。
...
### [152] [Self-supervised denoising of raw tomography detector data for improved image reconstruction](https://arxiv.org/abs/2511.17312)
...
Method: 采用两种自监督深度学习方法对原始探测器数据进行去噪，并与非学习型去噪方法进行对比。

Result: 深度学习方法能够增强探测器数据的信噪比，并持续改善重建图像质量，优于非学习方法。

Conclusion: 自监督深度学习方法是超快电子束X射线CT数据去噪的有效方法，能显著提升图像重建质量。
...
### [153] [ReBaPL: Repulsive Bayesian Prompt Learning](https://arxiv.org/abs/2511.17339)
*Yassir Bendou,Omar Ezzahir,Eduardo Fernandes Montesuma,Gabriel Mahuas,Victoria Shevchenko,Mike Gartrell*

Main category: cs.LG

TL;DR: 本文提出了Repulsive Bayesian Prompt Learning (ReBaPL)方法，通过贝叶斯框架优化提示学习，结合循环步长调度和SGHMC算法，并引入表示空间排斥力来增强探索多样性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统提示学习方法容易过拟合且分布外泛化能力不足，贝叶斯提示学习通过将提示优化构建为贝叶斯推理问题来提高鲁棒性。

Method: ReBaPL整合循环步长调度与SGHMC算法，交替进行探索和利用阶段，并引入基于概率度量的排斥力来多样化表示空间探索。

Result: 在多个基准数据集上，ReBaPL相比最先进的提示学习方法表现出更优越的性能。

Conclusion: ReBaPL提供了模块化的即插即用贝叶斯扩展，能够更全面地描述提示后验分布，从而改善泛化能力。
...
TL;DR: 本文提出了首个在部分可观测性下的最坏情况鲁棒实时追捕策略方法，通过信念保持机制和强化学习框架实现跨图泛化。


<details>
  <summary>Details</summary>
Motivation: 现有方法在部分可观测性和逃避者预测能力方面存在局限，无法提供实时适用的追捕策略，需要解决这一现实应用需求。

Method: 首先证明传统动态规划算法在异步移动下保持最优性，然后提出信念保持机制扩展DP策略到部分可观测设置，最后将信念保持嵌入EPG框架进行跨图强化学习。

Result: 强化学习后的策略实现了对未见真实图结构的鲁棒零样本泛化，并持续优于现有游戏RL方法直接在测试图上训练的策略。
...
### [155] [Stable Coresets via Posterior Sampling: Aligning Induced and Full Loss Landscapes](https://arxiv.org/abs/2511.17399)
...
<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型规模扩大，计算需求增加，需要有效的核心集选择技术来加速训练。梯度方法虽然理论基础强，但面临SGD基线强和代表性随时间下降的挑战。

Method: 建立后验采样与损失景观的连接，引入基于后验采样的平滑损失函数，提出采样式核心集选择方法并进行收敛性分析。

Result: 实验表明该方法在多个数据集上实现了比现有技术更快的训练速度和更好的泛化性能。

Conclusion: 所提出的基于后验采样的框架有效解决了核心集选择中的挑战，在计算效率和泛化能力方面优于现有方法。
...
### [156] [DS-Span: Single-Phase Discriminative Subgraph Mining for Efficient Graph Embeddings](https://arxiv.org/abs/2511.17419)
*Yeamin Kaiser,Muhammed Tasnim Bin Anwar,Bholanath Das,Chowdhury Farhan Ahmed,Md. Tanvir Alam*

Main category: cs.LG

TL;DR: DS-Span是一个单阶段判别子图挖掘框架，通过统一模式增长、剪枝和监督驱动评分，在搜索空间的一次遍历中完成挖掘，生成紧凑且判别性强的子图特征用于图嵌入和分类。
...
TL;DR: 本文提出了CurvSSL框架，通过曲率正则化增强自监督学习，在保持标准双视图编码器-投影器架构的同时，加入基于局部流形弯曲的曲率正则化项。
...
Method: 采用标准双视图编码器-投影器架构，在Barlow Twins风格的冗余减少损失基础上，添加曲率正则化器。通过计算嵌入点的k近邻在单位超球面上的余弦交互来定义离散曲率得分，并使用Barlow风格损失对齐和去相关不同增强视图的曲率。

Result: 在MNIST和CIFAR-10数据集上的实验表明，曲率正则化的自监督学习相比Barlow Twins和VICReg具有竞争性或改进的线性评估性能。

Conclusion: 明确塑造局部几何是纯统计自监督学习正则化器的简单有效补充。
...
### [158] [Towards fully differentiable neural ocean model with Veros](https://arxiv.org/abs/2511.17427)
...
<details>
  <summary>Details</summary>
Motivation: 传统的海洋模型难以进行梯度计算，限制了基于梯度的优化方法在海洋建模中的应用。通过使海洋模型可微分，可以支持端到端学习和参数调优。

Method: 对VEROS海洋模型进行修改，使其与JAX自动微分框架完全兼容，保持数值一致性。

Result: 实现了可微分的海洋模型，能够成功应用于初始海洋状态校正和未知物理参数校准两个示例任务。

Conclusion: 可微分编程为海洋建模中的端到端学习和参数调优提供了新的可能性，该实现已在线提供。
...
### [159] [Unmasking Airborne Threats: Guided-Transformers for Portable Aerosol Mass Spectrometry](https://arxiv.org/abs/2511.17446)
*Kyle M. Regan,Michael McLoughlin,Wayne A. Bryden,Gonzalo R. Arce*

Main category: cs.LG

TL;DR: MS-DGFormer是一个基于Transformer的数据驱动框架，可直接处理原始质谱数据，通过字典编码器整合SVD去噪信息，实现单次光谱的病原体识别，使MALDI-MS能够在现场条件下进行自主实时分析。


<details>
  <summary>Details</summary>
Motivation: 传统MALDI-MS依赖繁琐的样品制备和多光谱平均，限制了其在实时环境监测中的应用，特别是在新兴的气溶胶MALDI-MS系统中，自主采样产生的噪声光谱需要单次检测能力。

Method: 提出MS-DGFormer框架，采用Transformer架构捕获时间序列光谱的长程依赖性，并引入字典编码器整合基于SVD的去噪光谱信息，从单次光谱中提取关键生物分子模式。

Result: 该方法实现了对气溶胶样品中病原体的优异识别性能，为便携式MALDI-MS平台提供了可能。

Conclusion: 通过消除广泛预处理需求，该方法释放了便携式MALDI-MS平台的潜力，革新了环境病原体检测和生物威胁快速响应能力。
...
<details>
  <summary>Details</summary>
Motivation: 在大型机器人环境中，当无法完全满足时序逻辑任务时，需要找到最佳可能的任务满足方案，同时考虑用户对任务放松的偏好。

Method: 使用时序逻辑目标和用户偏好的自动机表示，提出基于A*的规划框架，并设计了一个简单高效的可采纳启发式函数。

Result: 该方法能够有效处理大规模问题，生成接近最优的高层轨迹，相比无信息搜索算法显著减少了时间和搜索内存需求。

Conclusion: 提出的启发式方法具有良好的可扩展性，能够在大规模机器人环境中实现高效规划，同时保持接近最优的性能。
...
TL;DR: SPTS是一种新型触觉皮肤系统，通过压缩采样技术从单个输出通道重建整个传感器阵列的丰富触觉信息，大幅简化布线并提高数据采集效率。


<details>
  <summary>Details</summary>
Motivation: 开发大面积、高速电子皮肤面临布线复杂性和数据瓶颈的根本限制，需要新的解决方案来支持机器人、假肢和人机接口应用。

Method: 采用直接电路级实现，每个传感元件配备微型微控制器，向全局总和贡献动态加权的模拟信号，在硬件中执行分布式压缩感知。采用灵活的可菊花链连接设计。

Result: 实现了3500 FPS的有效物体分类，能够捕捉瞬态动力学，将8ms的弹丸撞击解析为23帧。支持自适应重建，仅需7%总数据即可快速定位接触，然后逐步细化到高保真图像。

Conclusion: 这项工作为机器人和人机接口的大规模触觉智能提供了一条高效途径，解决了传统方法的布线复杂性和数据瓶颈问题。
...
Method: 结合改进的MRF IAPF算法（包含障碍物斥力、无人机间相互作用力和目标吸引力）和增强的单无人机路径优化机制（包括碰撞风险评估和辅助子目标策略），在面临高碰撞威胁时生成临时航点引导避障。

Result: 仿真结果表明，相比传统APF编队算法，该算法在路径长度优化和航向稳定性方面有显著改进，能有效避障并快速恢复编队构型。
...
TL;DR: 提出了MobileOcc数据集，用于移动机器人在拥挤人类环境中的语义占据感知，包含静态物体占据标注和专门的人类占据建模网格优化框架。


<details>
  <summary>Details</summary>
Motivation: 密集3D语义占据感知在行人丰富的移动机器人环境中很重要，但相比自动驾驶领域仍未被充分探索。

Method: 使用包含静态物体占据标注的标注流程和专门的人类占据建模网格优化框架，从2D图像重建可变形人体几何，并利用LiDAR点云数据进行优化。

Result: 建立了占据预测和行人速度预测的基准，使用单目、立体和全景占据等方法，并在3D人体姿态估计数据集上验证了标注方法的鲁棒性。
...
TL;DR: CalibAll是一种简单有效的相机外参估计方法，仅需单个标记点，通过粗到精的校准流程，在多种机器人和数据集上实现无需训练、稳定且准确的相机外参估计。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在机器人任务中取得了显著成功，但相机外参信息通常难以获取，现有估计方法存在局部最小值和泛化能力差的问题。

Method: 在末端执行器上标注单个标记点，利用视觉基础模型的对应能力自动定位标记点，结合点跟踪和3D末端轨迹通过时序PnP获得粗外参估计，再通过基于渲染的优化对齐渲染和真实掩码进行精炼。

Result: 实验结果表明该方法优于现有最先进方法，在三个机器人平台上表现出强大的鲁棒性和通用有效性，并能生成深度图、链接级掩码和末端执行器2D轨迹等有用辅助标注。

Conclusion: CalibAll提供了一种无需训练、稳定准确的相机外参估计解决方案，仅需单个标记点即可在多样化场景中实现有效校准。
...
TL;DR: 提出了一种利用多帧点约束的主动端到端导航框架，通过预测模块预测移动障碍物的未来路径，使机器人能够主动规避潜在危险，显著提升在未知动态环境中的导航鲁棒性和效率。


<details>
  <summary>Details</summary>
Motivation: 解决复杂动态环境中机器人导航的挑战。传统方法假设静态环境无法适应实时变化，而基于学习的方法依赖单帧观测进行运动约束估计，限制了其适应性。

Method: 提出新颖框架，利用多帧点约束（包括当前帧和专用模块预测的未来帧）实现主动端到端导航。通过基于多帧观测预测移动障碍物未来路径的预测模块，使机器人能够主动预测和规避潜在危险。

Result: 仿真和真实世界实验验证了该方法的有效性。

Conclusion: 所提出的主动规划能力显著增强了在未知动态环境中的导航鲁棒性和效率。
...
TL;DR: H-GAR是一个用于机器人操作的分层交互框架，通过目标驱动的观察-动作精炼来实现更准确的视频和动作预测。
...
Method: H-GAR首先生成目标观察和粗略动作草图，然后通过两个协同模块进行精炼：目标条件观察合成器(GOS)基于粗略动作和预测的目标观察合成中间观察；交互感知动作精炼器(IAAR)利用中间观察反馈和历史动作记忆库将粗略动作精炼为细粒度、目标一致的动作。

Result: 在仿真和真实世界机器人操作任务上的广泛实验表明，H-GAR实现了最先进的性能。

Conclusion: 通过将目标基础与显式的动作-观察交互以从粗到细的方式集成，H-GAR能够实现更准确的操作。
...
TL;DR: Progress-Think提出语义进度推理方法，通过预测指令风格的进度来改进视觉语言导航，采用三阶段框架实现无需昂贵标注的进度感知导航。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航方法要么专注于直接动作预测，要么预测数值进度，都忽略了观察序列和指令序列之间的单调共进特性，导致导航一致性不足。

Method: 提出三阶段框架：1) 自对齐进度预训练通过视觉历史与指令前缀的可微对齐引导推理模块；2) 进度引导策略预训练将学习到的进度状态注入导航上下文；3) 进度-策略协同微调使用进度感知强化目标联合优化两个模块。

Result: 在R2R-CE和RxR-CE数据集上的实验表明，该方法在成功率和效率方面达到最先进水平，语义进度提供了更一致的导航进展表示。

Conclusion: 语义进度推理能够产生更一致的导航进展表示，显著提升视觉语言导航的性能和效率。
...
<details>
  <summary>Details</summary>
Motivation: 环境中主动标记的反射是多机器人视觉相对定位中常见的模糊源，通常被视为干扰因素。本研究旨在利用这些通常不需要的反射来改进相对定位性能。
...
Conclusion: 该方法为异构微型空中蜂群在未知环境中的协作提供了一种有效的相对定位解决方案，特别适用于海洋部署场景，视频和源代码将在发表后公开。
...
TL;DR: 提出了一种利用大型语言模型（LLMs）增强基于黑盒优化的机器人身体设计效率的方法，通过结合黑盒优化和LLMs采样来提高设计解决方案的探索效率。


<details>
  <summary>Details</summary>
Motivation: 现有机器人设计优化方法中，数值优化不适合复杂结构或离散值，而黑盒优化采样效率低、需要大量迭代才能获得良好解决方案。

Method: 在基于黑盒优化的采样过程同时，使用LLMs进行采样，为LLMs提供问题设置和大量反馈信息。
...
### [170] [A ROS2 Interface for Universal Robots Collaborative Manipulators Based on ur_rtde](https://arxiv.org/abs/2511.17237)
*Alessio Saccuti,Riccardo Monica,Jacopo Aleotti*

Main category: cs.RO

TL;DR: 提出基于ur_rtde C++库的新型ROS2 UR机器人驱动程序，具有灵活性和可扩展性，支持URScript高级命令和自定义插件系统。


<details>
  <summary>Details</summary>
Motivation: 为Universal Robots机械臂开发一个灵活、可适应多种应用的ROS2驱动程序，解决现有方案的局限性。

Method: 基于ur_rtde C++库构建ROS2驱动程序，采用插件系统支持自定义命令，实现基于路径点的运动执行等功能。

Result: 成功开发出开源ROS2驱动程序，能够暴露URScript高级命令，支持多种运动控制功能。

Conclusion: 该驱动程序为UR机器人提供了灵活、可扩展的ROS2解决方案，支持自定义功能扩展，具有广泛的应用前景。
...
### [171] [Simulation of Active Soft Nets for Capture of Space Debris](https://arxiv.org/abs/2511.17266)
*Leone Costi,Dario Izzo*

Main category: cs.RO

TL;DR: 提出基于MuJoCo的软机器人网模拟器，用于空间碎片自主清除，研究不同机械模型和控制策略捕获Envisat卫星碎片的效果。


<details>
  <summary>Details</summary>
Motivation: 开发用于空间碎片清除的软机器人网模拟器，解决现有方法中假设网已弹射投掷的局限性，从相对静态配置开始研究捕获策略。

Method: 使用MuJoCo物理引擎构建模拟器，包含网动力学、网与碎片接触、网自接触、轨道力学和四角卫星推进器控制器，研究不同柔顺度的机械模型和滑动模式控制器。

Result: 更柔顺的网在捕获Envisat时表现更好，配合滑动模式控制器时软网在100%测试案例中成功捕获，接触有效面积更大，接触点更多。

Conclusion: 软网配合滑动模式控制器是实现空间碎片高效捕获的有效方案，柔顺性提高了捕获成功率和接触性能。
...
TL;DR: 提出一种基于条件变分自编码器的方法，仅从多指抓手的点云数据中高效确定关节配置，无需传统逆运动学的复杂计算。


<details>
  <summary>Details</summary>
Motivation: 传统逆运动学方法需要复杂的后处理决策或数值近似，无法高效处理多指抓手的复杂运动学问题。

Method: 使用条件变分自编码器，以关键结构元素的点云数据作为输入，重构相应的关节配置。

Result: 在MultiDex抓取数据集上验证，运行时间0.05毫秒，精度达到最先进方法水平。

Conclusion: 该方法在AI驱动的抓取规划技术中展现了高效且准确的关节配置估计能力。
...
TL;DR: 本研究提出了一种基于贝叶斯推理的脑机接口框架，用于实现连续追踪运动控制，相比传统方法在速度预测误差上减少了72%。


<details>
  <summary>Details</summary>
Motivation: 解决当前脑机接口运动控制系统仅限于离散命令的问题，实现能够实时调整速度和方向的连续追踪控制，这对于轮椅用户在复杂公共空间中的自主导航至关重要。

Method: 采用脑启发的贝叶斯推理框架，解码基于加速度的运动表征中的体现动态，使用自动相关确定进行特征选择，并结合持续在线学习。

Result: 在包含16小时EEG数据的公开数据集上，该方法在会话累积迁移学习设置中，相比自回归和EEGNet方法，将预测速度与真实速度之间的归一化均方误差降低了72%。

Conclusion: 这些发现从经验上支持了体现认知理论，揭示了大脑内在运动控制动态的体现和预测性质，为更稳定和直观的脑机接口控制提供了有前景的路径。
...
Method: 通过显式考虑稀疏单目SLAM前端的特性，在映射和规划中分别处理稀疏深度数据、自由空间间隙和深度不确定性。映射模块通过过采样纹理稀疏区域的自由空间并跟踪障碍物位置不确定性来解决问题；规划模块通过快速重规划和感知感知的航向控制来处理自由空间不确定性。

Result: 在多样化的真实世界和模拟环境中进行了广泛评估，包括消融研究。据作者所知，这是首个在真实世界非结构化室外环境中实现3D单目探索的方法。

Conclusion: 证明了在考虑视差要求和纹理缺失表面可能性的情况下，基于边界的探索在稀疏单目深度数据中是可行的，并开源了实现以支持未来研究。
...
TL;DR: FORWARD是一个高分辨率多模态数据集，记录了瑞典中部两个采伐场地的伐木集材机在崎岖地形中的操作数据，包括RTK-GNSS、360度摄像头、振动传感器等多种传感器数据，用于开发森林机械的交通性、感知和自主控制模型。


<details>
  <summary>Details</summary>
Motivation: 为开发森林机械的交通性、感知和自主控制模型提供高质量的多模态数据集，支持人工智能、仿真和物理测试平台的研究。

Method: 使用配备多种传感器的大型Komatsu伐木集材机，在瑞典中部的两个采伐场地收集数据，包括RTK-GNSS、360度摄像头、操作员振动传感器、CAN总线信号记录和多个IMU，同时结合高分辨率激光扫描地形数据。

Result: 收集了约18小时的常规木材提取工作数据，包含厘米级精度的车辆位置、行驶速度、燃油消耗、起重机使用等信息，并标注了工作元素和实验场景。

Conclusion: 该数据集为森林机械的自动化控制、仿真器自动生成和校准提供了重要基础，有助于提高效率、降低燃油消耗、增强安全性和减少环境影响。
...
TL;DR: 本文提出了一种长上下文Q-former方法，用于改进人机交互中的机器人动作确认和动作步骤生成，通过整合视频的左右上下文依赖关系来提升性能。
...
Method: 提出了长上下文Q-former，整合完整视频中的左右上下文依赖关系，并采用文本条件化方法将文本嵌入直接输入到LLM解码器中。

Result: 在YouCook2语料库上的实验表明，确认生成的准确性是动作规划性能的主要因素，长上下文Q-former通过整合VideoLLaMA3改进了确认和动作规划。

Conclusion: 长上下文Q-former能够有效提升人机交互中机器人动作确认和规划的性能，特别是在处理长时程任务时。
...
TL;DR: METIS是一个用于灵巧操作的视觉-语言-动作模型，通过整合多源自我中心数据集构建EgoAtlas，并提取运动感知动态作为紧凑的运动表示，在六个真实世界任务中达到最高平均成功率。


<details>
  <summary>Details</summary>
Motivation: 构建能够感知、推理和跨任务行动的通用机器人仍具挑战性，特别是在灵巧操作方面。主要瓶颈在于缺乏大规模动作标注数据，而人类数据提供了丰富的先验知识。现有方法受限于场景有限和人类与机器人之间的视觉差距。

Method: 首先构建EgoAtlas整合大规模人类和机器人数据，统一到一致的动作空间；提取运动感知动态作为紧凑离散的运动表示；METIS将推理和行动集成到统一框架中，支持下游灵巧操作任务部署。

Result: 在六个真实世界任务中达到最高平均成功率，实验结果显示在分布外场景下具有优越的泛化能力和鲁棒性。

Conclusion: METIS是朝着灵巧操作通用模型迈出的有希望的一步，展示了卓越的灵巧操作能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在敏捷动态技能和稳定性关键行为之间存在专业化分工，无法同时兼顾两者。需要开发能够统一这两种能力的控制器。

Method: 利用人类运动捕捉数据提供敏捷行为，物理约束合成平衡运动捕捉稳定性配置；设计混合奖励方案，应用通用跟踪目标，仅在合成运动中注入平衡特定先验；采用自适应学习策略。

Result: 在仿真和真实Unitree G1人形机器人上验证，单个策略能执行舞蹈、跑步等敏捷技能，也能零样本执行叶问蹲等极端平衡动作。

Conclusion: AMS作为多功能控制范式，为人形机器人的未来应用提供了统一的控制解决方案。
...
TL;DR: 本文提出了一种向量成本双矩阵游戏方法，用于多目标决策，使自主机器人系统能够同时优化多个目标，并在竞争性运动规划中避免被忽视目标的最坏情况。该方法扩展到任意数量的目标，并与标量加权和方法进行比较，使用XAI软件分析高维决策数据。


<details>
  <summary>Details</summary>
Motivation: 现有的多目标决策方法在处理竞争性运动规划时存在局限性，无法同时优化多个目标并避免被忽视目标的最坏情况。需要一种更全面和可解释的框架来改进机器人行为规划。

Method: 使用向量成本双矩阵游戏方法进行多目标决策，结合SEMBAS探索参数空间中的性能模式，并集成游戏理论规划和智能系统验证到一个新颖的仿真管道中。

Result: 向量成本方法相比标量化方法展现出显著改进，提供了一个可解释和可推广的机器人行为规划框架。

Conclusion: 该方法在竞争性运动规划中优于传统标量加权和方法，为机器人多目标决策提供了一个全面且可解释的解决方案。
...
TL;DR: IndustryNav是首个动态工业导航基准，用于评估视觉大语言模型在动态仓库环境中的主动空间推理能力，包含12个高保真Unity场景，评估了9个先进VLLM在路径规划、碰撞避免和主动探索方面的表现。


<details>
  <summary>Details</summary>
Motivation: 现有具身基准主要关注静态家庭环境，无法评估模型在动态、真实世界复杂环境中的整体性能，因此需要开发能够评估主动空间推理能力的动态工业导航基准。

Method: 使用12个手动创建的高保真Unity仓库场景，包含动态物体和人员移动，采用结合自我中心视觉和全局里程计的PointGoal导航管道，引入碰撞率和警告率指标来评估安全导向行为。

Result: 对9个最先进VLLM的全面研究表明，闭源模型保持持续优势，但所有智能体在鲁棒路径规划、碰撞避免和主动探索方面都存在显著缺陷。
...
### [181] [SPEAR-1: Scaling Beyond Robot Demonstrations via 3D Understanding](https://arxiv.org/abs/2511.17411)
*Nikolay Nikolov,Giuliano Albanese,Sombit Dey,Aleksandar Yanev,Luc Van Gool,Jan-Nico Zaech,Danda Pani Paudel*

Main category: cs.RO

TL;DR: SPEAR-1是一个机器人基础模型，通过增强预训练视觉语言模型的3D空间理解能力，在减少20倍机器人演示数据的情况下，性能达到或超过现有最先进模型。
...
Method: 提出SPEAR-VLM，通过为易于收集的非机器人图像数据添加3D标注来增强预训练VLM的3D理解能力，然后构建SPEAR-1模型，集成基于3D的感知和语言指令的具身控制。
...
### [182] [RoboCOIN: An Open-Sourced Bimanual Robotic Data COllection for INtegrated Manipulation](https://arxiv.org/abs/2511.17441)
...
TL;DR: 提出了RoboCOIN，一个包含超过18万次演示的多体现双手机器人操作数据集，涵盖15个不同机器人平台和16种场景，通过分层能力金字塔提供多级标注，并开发了CoRobot处理框架。


<details>
  <summary>Details</summary>
Motivation: 解决双手机器人操作中由于机器人平台硬件异构性导致的大规模多样化数据集稀缺问题。

Method: 收集15个不同机器人平台的180,000多次演示，构建分层能力金字塔进行多级标注，开发Robot Trajectory Markup Language (RTML)进行质量评估和统一管理。

Result: 实验证明RoboCOIN在多体现双手学习中的可靠性和有效性，在各种模型架构和机器人平台上均取得显著性能提升。

Conclusion: RoboCOIN数据集和CoRobot框架为双手机器人操作研究提供了重要资源，已开源供进一步研究使用。
...
### [183] [MDG: Masked Denoising Generation for Multi-Agent Behavior Modeling in Traffic Environments](https://arxiv.org/abs/2511.17496)
*Zhiyu Huang,Zewei Zhou,Tianhui Cai,Yun Zhang,Jiaqi Ma*

Main category: cs.RO

TL;DR: MDG提出了一种统一的生成框架，将多智能体行为建模重构为独立加噪时空张量的重建，通过连续、按智能体和时间步的噪声掩码实现局部去噪和可控轨迹生成。


<details>
  <summary>Details</summary>
Motivation: 现有扩散和自回归方法受限于迭代采样、顺序解码或任务特定设计，阻碍了效率和复用性，需要一种更高效统一的多智能体行为建模方法。

Method: MDG采用掩码去噪生成框架，应用连续、按智能体和时间步的噪声掩码，实现局部去噪和可控轨迹生成，支持单次或少量前向传递。

Result: 在Waymo Sim Agents和nuPlan Planning基准测试中取得有竞争力的闭环性能，同时提供高效、一致和可控的开环多智能体轨迹生成。

Conclusion: MDG作为多智能体行为建模的简单而通用范式，能够统一处理开环预测、闭环仿真、运动规划和条件生成等任务。
...
### [184] [HALO: High-Altitude Language-Conditioned Monocular Aerial Exploration and Navigation](https://arxiv.org/abs/2511.17497)
...
Method: 使用单目相机、GPS和IMU传感器组合，开发了能够规划信息路径并利用自然语言指定多任务完成任务的系统。

Result: 在模拟评估中，HALO在高达78,000平方米的环境中持续以更少的探索时间完成任务，相比最先进的语义探索基线，行进距离竞争比提高达68%。在真实世界实验中，所有模块可在机器人上运行，在40米高度覆盖24,600平方米区域。

Conclusion: HALO系统证明了在大型室外环境中实现实时高空航拍度量-语义建图与探索的可行性，支持有效的自主任务执行。
...
### [185] [RynnVLA-002: A Unified Vision-Language-Action and World Model](https://arxiv.org/abs/2511.17502)
...
TL;DR: RynnVLA-002是一个统一的视觉-语言-动作模型与世界模型，通过联合学习环境动力学和动作规划，在仿真和真实机器人任务中表现出色。
...
Method: 结合VLA模型和世界模型：世界模型利用动作和视觉输入预测未来图像状态，学习环境物理；VLA模型从图像观察生成后续动作，增强视觉理解并支持世界模型的图像生成。

Result: 在LIBERO仿真基准测试中达到97.4%的成功率（无需预训练）；在真实世界LeRobot实验中，集成世界模型将整体成功率提升了50%。

Conclusion: RynnVLA-002超越了单独的VLA和世界模型，证明了它们的相互增强效果，为机器人任务提供了有效的统一解决方案。
...
### [186] [Stable diffusion models reveal a persisting human and AI gap in visual creativity](https://arxiv.org/abs/2511.16814)
...
Result: 在三个LLM上进行知识提取、冲突检测和推理任务的基准测试比较，显示所有模型都能执行Cognitive BASIC程序，整体表现强劲但不均匀。
...
TL;DR: 提出了一种利用响应模式统计分析来识别无效基准问题的框架，通过专家审查和LLM辅助减少人工工作量，提高基准可靠性。


<details>
  <summary>Details</summary>
Motivation: 基准测试对AI发展至关重要，但无效问题会破坏其可靠性。手动识别和修正数千个基准问题不可行且效率低下。

Method: 基于响应模式统计分析，假设平均分能充分概括模型性能，构建单维潜在结构。当项目统计值超出预期范围时标记为潜在问题，结合专家审查和LLM初步筛选。

Result: 在9个广泛使用的基准测试中，该方法指导专家审查识别问题问题的精度高达84%，显著减少人工工作量。

Conclusion: 该框架为系统化基准修订提供了高效可扩展的解决方案，通过统计分析和LLM辅助大幅提升基准测试的可靠性。
...
### [189] [Hybrid Differential Reward: Combining Temporal Difference and Action Gradients for Efficient Multi-Agent Reinforcement Learning in Cooperative Driving](https://arxiv.org/abs/2511.16916)
...
<details>
  <summary>Details</summary>
Motivation: 在多车协同驾驶任务中，传统基于状态的奖励函数存在奖励差异消失的问题，导致策略梯度的信噪比低，严重影响算法收敛和性能提升。

Method: 提出混合差分奖励机制，包含两个互补组件：基于全局势函数的时序差分奖励和直接测量动作边际效用的动作梯度奖励。将协同驾驶问题建模为具有时变智能体集的多智能体部分可观测马尔可夫博弈。

Result: 通过在线规划和多智能体强化学习算法的广泛实验表明，HDR机制显著提高了收敛速度和策略稳定性，引导智能体学习到平衡交通效率和安全的优质协同策略。

Conclusion: HDR机制有效解决了多车协同驾驶中奖励信号信噪比低的问题，显著提升了算法性能和策略质量。
...
### [190] [MirrorMind: Empowering OmniScientist with the Expert Perspectives and Collective Knowledge of Human Scientists](https://arxiv.org/abs/2511.16997)
*Qingbin Zeng,Bingbing Fan,Zhiyu Chen,Sijian Ren,Zhilun Zhou,Xuhua Zhang,Yuanyi Zhen,Fengli Xu,Yong Li,Tie-Yan Liu*

Main category: cs.AI

TL;DR: 本文提出了MirrorMind分层认知架构，通过整合个体认知轨迹和集体学科记忆来解决AI科学家在科学发现中的局限性。该架构包含个体层、领域层和跨学科层，实现了记忆存储与智能执行的分离，在多个任务中展现出优于传统方法的科学推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI科学方法将科学发现视为孤立的优化过程，忽视了知识生产的社会性和历史性本质。人类科学洞察来源于个体认知轨迹和集体学科记忆两个相互关联的来源，而现有大语言模型难以准确表示这些结构化、高保真的认知和社会背景。

Method: 提出MirrorMind分层认知架构：个体层构建研究者的情景、语义和人格记忆的高保真认知模型；领域层将集体知识映射为结构化学科概念图；跨学科层作为正交编排引擎。关键创新是将记忆存储与智能执行分离。

Result: 在四个综合任务中评估MirrorMind：作者级认知模拟、互补推理、跨学科合作促进和多智能体科学问题解决。结果显示，通过整合个体认知深度与集体学科广度，MirrorMind超越了简单事实检索，实现了结构化、个性化和洞察生成的科学推理。

Conclusion: MirrorMind通过整合个体认知深度与集体学科广度，超越了传统的事实检索方法，实现了更具结构性和洞察力的科学推理，为AI科学家的发展提供了新的方向。
...
### [191] [DAPS++: Rethinking Diffusion Inverse Problems with Decoupled Posterior Annealing](https://arxiv.org/abs/2511.17038)
*Hao Chen,Renzheng Zhang,Scott S. Howard*

Main category: cs.AI

TL;DR: 本文重新解释了扩散模型在逆问题求解中的作用，提出DAPS++方法，将扩散阶段与数据驱动优化解耦，提高计算效率和重建性能。
...
Conclusion: 通过解耦扩散阶段与数据驱动优化，DAPS++提供了对统一扩散轨迹在实践中保持有效性的解释，并显著提升了逆问题求解的效率。
...
### [192] [MIR: Efficient Exploration in Episodic Multi-Agent Reinforcement Learning via Mutual Intrinsic Reward](https://arxiv.org/abs/2511.17165)
*Kesheng Chen,Wenjian Luo,Bang Zhang,Zeping Yin,Zipeng Ye*

Main category: cs.AI

TL;DR: 本文提出了一种名为MIR的互惠内在奖励方法，用于解决多智能体强化学习中稀疏奖励（特别是情节奖励）的挑战。该方法通过激励智能体探索影响队友的行为，有效促进团队探索并提升算法性能。
...
Method: 提出MIR方法，激励个体智能体探索能够影响队友的行为，并将其与原始策略结合，有效刺激团队探索。

Result: 在扩展的MiniGrid-MA环境中进行实验验证，与最先进方法相比，MIR方法表现出优越性能。

Conclusion: MIR是一种简单有效的增强策略，能够显著改善具有极端稀疏奖励的多智能体强化学习性能。
...
### [193] [Designing Domain-Specific Agents via Hierarchical Task Abstraction Mechanism](https://arxiv.org/abs/2511.17198)
*Kaiyu Li,Jiayu Wang,Zhi Wang,Hui Qiao,Weizhan Zhang,Deyu Meng,Xiangyong Cao*

Main category: cs.AI

TL;DR: 本文提出了一种基于层次任务抽象机制(HTAM)的新型多智能体框架EarthAgent，专门用于解决复杂地理空间分析任务，相比通用框架在专业领域表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有的通用智能体框架（如ReAct或角色扮演）在需要严格结构化工作流程的专业领域（如遥感）中表现不佳，因为这些领域需要专门的工具和多步骤程序。

Method: 提出了层次任务抽象机制(HTAM)，将多智能体系统组织成逻辑层次结构，反映领域内在的任务依赖图，通过任务中心架构确保程序正确性并将复杂问题分解为顺序层。

Result: 实验表明，EarthAgent在复杂地理空间规划任务上显著优于现有的单智能体和多智能体系统，并建立了GeoPlan-bench基准测试集进行评估。
...
### [194] [Agentifying Agentic AI](https://arxiv.org/abs/2511.17332)
*Virginia Dignum,Frank Dignum*

Main category: cs.AI

TL;DR: 本文主张将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具作为实现智能AI系统的基础，通过将自适应数据驱动方法与结构化推理协调模型相结合，构建具有透明度、协作性和问责制的智能系统。


<details>
  <summary>Details</summary>
Motivation: 为实现具有持续自主性、推理和交互能力的智能AI系统，需要补充明确的认知、协作和治理模型，而AAMAS社区的概念工具正好提供了这样的理论基础。

Method: 将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具与自适应数据驱动方法相结合，构建结构化的推理和协调模型。

Result: 提出了一种连接形式理论和实践自主性的智能系统视角，为构建不仅能力强且灵活，而且透明、协作和可问责的智能系统指明了路径。

Conclusion: AAMAS社区的概念工具为实现智能AI系统提供了坚实基础，通过将数据驱动方法与结构化模型相结合，可以构建既强大又负责任的智能系统。
...
### [195] [That's not natural: The Impact of Off-Policy Training Data on Probe Performance](https://arxiv.org/abs/2511.17408)
*Nathalie Kirch,Samuel Dower,Adrians Skapars,Ekdeep Singh Lubana,Dmitrii Krasheninnikov*

Main category: cs.AI

TL;DR: 本文系统评估了使用合成和离策略数据训练LLM行为探测器的泛化性能，发现数据生成策略显著影响探测器性能，且同领域离策略数据比不同领域策略内数据更可靠。


<details>
  <summary>Details</summary>
Motivation: 由于自然行为样本稀少，研究者不得不依赖合成或离策略LLM响应来训练探测器，但这种方法对探测器泛化能力的影响尚不清楚。

Method: 在八种不同LLM行为上测试线性和注意力探测器，比较不同响应生成策略对探测器性能的影响，并分析从离策略到策略内数据的泛化能力。

Result: 响应生成策略显著影响探测器性能，成功从离策略数据泛化到策略内数据可预测真实监控场景中的表现，欺骗和蓄意表现不佳行为探测器可能无法有效泛化。

Conclusion: 在缺乏策略内数据时，使用同领域离策略数据比不同领域策略内数据产生更可靠的探测器，强调需要更好处理LLM监控中分布偏移的方法。
...
### [196] [RubiSCoT: A Framework for AI-Supported Academic Assessment](https://arxiv.org/abs/2510.17309)
*Thorsten Fröhlich,Tim Schlippe*

Main category: cs.AI

TL;DR: RubiSCoT是一个AI支持的论文评估框架，使用自然语言处理技术从提案到最终提交阶段增强论文评估过程。


<details>
  <summary>Details</summary>
Motivation: 传统论文评估方法耗时且受评估者主观性影响，需要更一致、可扩展的解决方案。

Method: 使用大型语言模型、检索增强生成和结构化思维链提示等NLP技术，包括初步评估、多维度评估、内容提取、基于量规的评分和详细报告。

Result: 开发了RubiSCoT框架的设计和实现，为学术评估过程提供一致、可扩展和透明的评估能力。
...
### [197] [Generative Augmented Reality: Paradigms, Technologies, and Future Applications](https://arxiv.org/abs/2511.16783)
*Chen Liang,Jiawen Zheng,Yufeng Zeng,Yi Tan,Hengye Lyu,Yuhui Zheng,Zisu Li,Yueting Weng,Jiaxin Shi,Hanwang Zhang*

Main category: cs.HC

TL;DR: 本文提出生成增强现实(GAR)作为下一代AR范式，将增强重新定义为世界重新合成的过程，而非传统AR引擎的世界组合。GAR用统一的生成主干取代传统AR的多阶段模块，将环境感知、虚拟内容和交互信号联合编码为连续视频生成的条件输入。


<details>
  <summary>Details</summary>
Motivation: 传统AR引擎采用多阶段模块进行世界组合，存在局限性。GAR旨在通过统一的生成模型实现更高保真度的AR体验，在真实性、交互性和沉浸感方面提供突破。

Method: GAR用统一的生成主干取代传统AR的多阶段模块，将环境感知、虚拟内容和交互信号联合编码为连续视频生成的条件输入，实现实时生成增强。

Result: GAR被形式化为AR的计算对应物，调查了使其可行的技术基础，并概述了利用其统一推理模型的潜在应用。

Conclusion: GAR被设想为未来AR范式，能够提供高保真体验，同时在技术、内容生态系统以及伦理和社会影响方面引发新的研究挑战。
...
### [198] [Scene Awareness While Using Multiple Navigation Aids in AR Search](https://arxiv.org/abs/2511.16805)
*Radha Kumaran,You-Jin Kim,Emily Machniak,Shane Dirksen,Junhyung Yoon,Tom Bullock,Barry Giesbrecht,Tobias Höllerer*

Main category: cs.HC

TL;DR: 研究探讨了在移动增强现实中同时使用多种导航辅助工具对用户对象回忆的影响，发现导航辅助工具会损害环境中对象的回忆能力。


<details>
  <summary>Details</summary>
Motivation: 增强现实允许多种导航辅助工具同时使用，但可能对用户的感知和认知产生负面影响，需要研究其对对象意识的影响。

Method: 用户在大型增强环境中搜索虚拟宝石，可选择独立或同时使用两种导航辅助工具（世界锁定箭头和屏幕雷达），然后回忆场景中可能存在的对象。

Result: 导航辅助工具的使用影响了对象回忆，当辅助工具开启时，环境中对象的回忆能力受损。

Conclusion: 结果揭示了移动AR中对象意识的潜在影响因素，并强调了可适应界面在支持用户导航物理世界方面的潜力。
...
### [199] [IsharaKotha: A Comprehensive Avatar-based Bangla Sign Language Corpus](https://arxiv.org/abs/2511.16896)
*MD. Ashikul Islam,Prato Dewan,Md Fuadul Islam,Md. Ataullha,M. Shahidur Rahman*

Main category: cs.HC

TL;DR: IsharaKotha是首个基于HamNoSys的孟加拉手语语料库，包含3823个单词，开发了文本到手语的翻译系统，并评估了动画质量，获得3.14/4.00的高分。


<details>
  <summary>Details</summary>
Motivation: 为听力受损群体提供沟通媒介，弥合听力正常人群与听力受损人群之间的沟通鸿沟，并支持手语学习者。

Method: 构建孟加拉手语语料库，集成深度学习词形还原器提取词根，开发评估界面评估字母、数字和句子的手语动画质量。

Result: 系统获得专业口译员和真实手语用户3.14/4.00的平均评分，质量介于良好和优秀之间。
...
### [200] [The Wireless Charger as a Gesture Sensor: A Novel Approach to Ubiquitous Interaction](https://arxiv.org/abs/2511.16989)
*Weiyi Wang,Lanqing Yang,Linqian Gan,Guangtao Xue*

Main category: cs.HC

TL;DR: EMGesture是一种基于Qi无线充电器电磁信号的无接触手势识别技术，通过分析电磁特征实现高精度手势识别，准确率超过97%。


<details>
  <summary>Details</summary>
Motivation: 传统人机交互方式存在接触限制、隐私担忧和高成本问题，而无线充电器的电磁信号既强可测又富含手势信息，为无接触交互提供了新可能。

Method: 提出EMGesture技术，利用Qi无线充电器的电磁信号，分析其独特电磁特征并采用鲁棒分类模型，构建端到端手势识别框架。

Result: 在30名参与者、10台移动设备和5个充电器的实验中，EMGesture实现了超过97%的识别准确率，用户研究证实了更高的可用性和便利性。

Conclusion: EMGesture是一种实用、注重隐私且成本效益高的普适交互解决方案，为无接触人机交互提供了新途径。
...
### [201] [Senti-iFusion: An Integrity-centered Hierarchical Fusion Framework for Multimodal Sentiment Analysis under Uncertain Modality Missingness](https://arxiv.org/abs/2511.16990)
...
<details>
  <summary>Details</summary>
Motivation: 多模态情感分析在模态不完整或缺失时面临挑战，现有方法通常假设预定义的缺失模态或固定缺失率，限制了实际应用。

Method: 提出Senti-iFusion框架，包含三个层次组件：完整性估计模块预测模态完整性并减轻噪声；完整性加权跨模态补全模块使用加权机制分离语义结构；完整性引导自适应融合机制动态选择主导模态进行注意力融合。

Result: 在流行的多模态情感分析数据集上的实验结果表明，Senti-iFusion优于现有方法，特别是在细粒度情感分析任务中表现出色。

Conclusion: Senti-iFusion通过完整性为中心的分层融合方法有效解决了多模态情感分析中的模态缺失问题，代码和模型将公开提供。
...
### [202] [Mixed Reality Scenic Live Streaming for Cultural Heritage: Visual Interactions in a Historic Landscape](https://arxiv.org/abs/2511.17246)
*Zeyu Huang,Zuyu Xu,Yuanhao Zhang,Chengzhong Liu,Yanwei Zhao,Chuhan Shi,Jason Chen Zhao,Xiaojuan Ma*

Main category: cs.HC

TL;DR: 本文提出MRSLS系统，在静态风景直播基础上叠加混合现实内容，增强文化遗址的互动性和沉浸体验。


<details>
  <summary>Details</summary>
Motivation: 风景直播缺乏动态性和互动性，作者希望通过混合现实技术最大化其潜力，特别是在文化遗产场景中。

Method: 开发MRSLS原型系统，通过访谈确定当地风景文化元素，进行两轮迭代设计研究，并采用混合方法进行用户研究。

Result: 研究表明MRSLS能够提供沉浸式风景欣赏、有效的文化印记和生动的共享体验。

Conclusion: MRSLS在文化性、参与性和真实性之间取得平衡，呼吁HCI领域更多关注这一未充分探索的设计空间。
...
### [203] [δ-EMG: A Monotonic Graph Index for Approximate Nearest Neighbor Search](https://arxiv.org/abs/2511.16921)
*Liming Xiang,Jing Feng,Ziqi Yin,Zijian Li,Daihao Xue,Hongchao Qin,Ronghua Li,Guoren Wang*

Main category: cs.IR

TL;DR: 该论文提出了一种误差有界的近似最近邻搜索方法，通过构建δ-EMG图结构，确保返回结果是对真实值的(1/δ)近似，相比传统基于召回率的方法提供了更强的理论保证。


<details>
  <summary>Details</summary>
Motivation: 当前近似最近邻搜索方法主要基于ε-召回率原则，只关注真正例的召回率，但无法保证错误结果的偏差。为了弥补这一局限性，研究关注误差有界的ANN方法。

Method: 采用基于图的框架，提出δ-EMG（误差有界单调图），在构建过程中施加δ-单调几何约束，确保贪心搜索收敛到(1/δ)近似邻居。进一步设计了误差有界top-k ANN搜索算法和δ-EMQG（误差有界单调量化图）变体。

Result: 在ANN-Benchmarks数据集上的实验显示，在召回率要求0.99下，在SIFT1M数据集上达到19,000 QPS，比其他方法性能提升超过40%。

Conclusion: 提出的误差有界近似最近邻搜索方法在保持高召回率的同时，提供了更强的理论保证，并在大规模数据集上表现出优越的性能。
...
### [204] [RASTP: Representation-Aware Semantic Token Pruning for Generative Recommendation with Semantic Identifiers](https://arxiv.org/abs/2511.16943)
*Tianyu Zhan,Kairui Fu,Zheqi Lv,Shengyu Zhang*

Main category: cs.IR

TL;DR: RASTP是一种用于生成式推荐系统的语义令牌剪枝方法，通过结合语义显著性和注意力中心性来动态剪枝输入序列中的低信息令牌，在保持推荐性能的同时显著减少训练时间。


<details>
  <summary>Details</summary>
Motivation: 生成式推荐系统使用语义标识符(SIDs)表示物品，但多个SIDs会显著增加输入序列长度，导致计算复杂度和内存消耗增加。现有方法主要优化注意力计算和KV缓存，但直接剪枝输入序列中的低信息令牌可能更有效。

Method: 提出RASTP方法，通过结合语义显著性（通过表示大小衡量）和注意力中心性（来自累积注意力权重）来评估令牌重要性，动态剪枝低信息或无关的语义令牌。

Result: 在三个真实世界的Amazon数据集上的实验表明，RASTP减少了26.7%的训练时间，同时保持或略微提高了推荐性能。

Conclusion: RASTP通过直接剪枝输入序列中的低信息令牌，有效解决了生成式推荐系统中序列长度过长的问题，在保持性能的同时显著提升了训练效率。
...
### [205] [CLLMRec: LLM-powered Cognitive-Aware Concept Recommendation via Semantic Alignment and Prerequisite Knowledge Distillation](https://arxiv.org/abs/2511.17041)
*Xiangrui Xiong,Yichuan Lu,Zifei Pan,Chang Sun*

Main category: cs.IR

TL;DR: CLLMRec是一个基于大语言模型的概念推荐框架，通过语义对齐和先验知识蒸馏解决MOOC个性化学习中的概念推荐问题，无需依赖结构化知识图谱。
...
Conclusion: 该框架能够生成真正认知感知和个性化的概念推荐，且不依赖显式结构化先验知识。
...
### [206] [Parametric Retrieval-Augmented Generation using Latent Routing of LoRA Adapters](https://arxiv.org/abs/2511.17044)
*Zhan Su,Fengran Mo,Jian-yun Nie*

Main category: cs.IR

TL;DR: 提出Poly-PRAG方法解决传统PRAG中一对一文档编码方案的数据稀缺和推理开销问题，通过潜在路由编码过程使用少量潜在LoRA适配器编码整个文档空间。


<details>
  <summary>Details</summary>
Motivation: 传统PRAG采用一对一文档编码方案，每个文档使用专用LoRA适配器，导致数据稀缺和推理时的高计算开销。

Method: 将文档集编码视为多任务学习过程，每个文档分配唯一任务标识符，使用路由函数通过少量潜在LoRA适配器编码整个文档空间，在线推理时根据查询选择性激活专家子集。

Result: 在多个知识密集型NLP任务上的综合评估显示，Poly-PRAG在四个不同数据集上达到最先进的结果。

Conclusion: Poly-PRAG通过潜在路由编码有效解决了传统PRAG的局限性，实现了更高效的参数化检索增强生成。
