...
Motivation: 饮食相关的慢性疾病（如肥胖和糖尿病）的上升需要准确监测食物摄入量。虽然AI驱动的饮食评估近年来取得进展，但从单目图像恢复食物尺寸（份量）信息以准确估计"吃了多少"是一个紧迫挑战。现有的3D重建方法虽然实现了令人印象深刻的几何重建，但未能恢复重建对象的真实世界尺度，限制了其在精准营养领域的应用。
...
TL;DR: DiSa是一个新颖的显著性感知前景-背景解耦框架，通过显式结合显著性线索来解决开放词汇语义分割中CLIP模型的前景偏见和空间定位有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型（如CLIP）的开放词汇语义分割方法存在两个关键限制：1）前景偏见，倾向于忽略背景区域；2）有限的空间定位，导致对象边界模糊。这些限制源于VLMs在图像-文本对上预训练时偏向于显著、以对象为中心的区域。
...
Conclusion: DiSa通过显著性感知的前景-背景解耦和分层细化，有效解决了开放词汇语义分割中VLMs的前景偏见和空间定位有限的问题，取得了优异的性能。
...
TL;DR: Sparse CLIP：在CLIP训练中直接集成稀疏性，实现可解释性和性能的协同优化，挑战了传统认为可解释性需要牺牲准确性的观念。


<details>
  <summary>Details</summary>
Motivation: CLIP作为视觉-语言表示学习的基石，其密集不透明的潜在表示存在显著的可解释性挑战。传统观点认为可解释性和性能存在矛盾：训练中强制稀疏性会降低准确性。现有的后处理方法如稀疏自编码器（SAEs）往往导致下游性能下降和CLIP固有多模态能力的丧失。

Method: 提出一种简单有效的方法，将稀疏性直接集成到CLIP训练中，生成既可解释又高性能的表示。与SAEs相比，稀疏CLIP表示保留了强大的下游任务性能，实现了更优的可解释性，并保持了多模态能力。
...
Conclusion: 研究结果挑战了传统认为可解释性需要牺牲准确性的观念，证明可解释性和性能可以协同优化，为未来模型设计提供了有前景的原则。
...
TL;DR: 该研究聚焦于H&E染色图像中细胞核实例分割的数据集标准化与评估，而非开发新算法。通过文献综述收集公开数据集，统一格式后使用两种先进模型评估性能，提出统一测试集(NucFuse-test)和训练集(NucFuse-train)，为细胞核实例分割提供了新的基准。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多关注开发新的细胞核实例分割算法，并在有限数量的公开数据集上进行基准测试。然而，这些数据集往往格式不统一，缺乏系统性评估。本研究旨在填补这一空白，专注于数据集本身的分析、标准化和评估，为领域提供更可靠的基准。

Method: 1. 通过文献综述识别手动标注的公开H&E染色图像数据集；2. 将所有数据集标准化为统一的输入和标注格式；3. 使用两种最先进的分割模型（基于CNN和混合CNN-Vision Transformer架构）系统评估数据集性能；4. 提出统一测试集(NucFuse-test)用于公平的跨数据集评估；5. 提出统一训练集(NucFuse-train)通过合并多个数据集图像提升分割性能。

Result: 1. 对多个数据集进行了系统评估和性能排名；2. 生成了融合数据集(NucFuse-test和NucFuse-train)；3. 进行了外部验证；4. 将实现代码公开，为H&E染色组织学图像的细胞核实例分割模型训练、测试和评估提供了新的基准。

Conclusion: 本研究通过数据集标准化、系统评估和融合数据集创建，为细胞核实例分割领域提供了重要的基准资源。与专注于算法开发不同，这项工作强调了数据集质量、标准化和系统性评估的重要性，有助于推动该领域的公平比较和模型性能提升。
...
TL;DR: 本文提出了一种无需训练的结构锚点剪枝方法(SAP)，可在视觉文档检索中实现超过90%的索引向量压缩，同时保持检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型(如ColPali)在细粒度视觉文档检索中索引向量开销过大，而无训练剪枝方法在高度压缩场景(>80%)下表现不佳，甚至不如随机选择。先前研究认为视觉令牌重要性本质上是查询依赖的，质疑无训练剪枝的可行性。

Method: 提出结构锚点剪枝(SAP)，一种无需训练的剪枝方法，从中间层识别关键视觉补丁以实现高性能压缩。同时引入Oracle Score Retention(OSR)协议来评估分层信息如何影响压缩效率。

Result: 在ViDoRe基准测试中，SAP将索引向量减少了90%以上，同时保持了稳健的检索保真度，为视觉RAG提供了高度可扩展的解决方案。OSR分析显示语义结构锚点补丁持续存在于中间层，而传统剪枝方法关注最终层，那里的结构信号已经消散。

Conclusion: SAP证明了无需训练的高效视觉令牌剪枝是可行的，通过从中间层提取结构锚点补丁，实现了高度压缩下的稳健性能，为大规模视觉文档检索系统提供了实用解决方案。
...
TL;DR: 针对扩散式大模型LLaDA-V提出结构化token剪枝策略，在保持95%任务性能的同时减少65%计算成本


<details>
  <summary>Details</summary>
Motivation: 扩散式大模型LLaDA-V存在计算开销大的问题，其双向注意力机制和迭代去噪范式导致视觉token在所有层和去噪步骤中重复处理。研究发现LLaDA-V主要在中间到后期层聚合跨模态信息，存在语义对齐延迟

Method: 提出结构化token剪枝策略，在第一个去噪步骤的中间到后期层选择性移除部分视觉token以减少FLOPs。与FastV的浅层剪枝不同，该方法针对LLaDA-V的延迟注意力聚合特性，在第一个步骤剪枝可减少后续所有步骤的计算

Result: 在多个基准测试中，最佳配置减少65%计算成本的同时保持平均95%的任务性能

Conclusion: 这是首个研究扩散式大模型结构化token剪枝的工作，为高效LLaDA-V推理提供了经验基础，并突显了视觉感知剪枝在扩散式多模态模型中的潜力
...
<details>
  <summary>Details</summary>
Motivation: 解决扩散变换器（DiTs）在内容保持风格迁移中的挑战，即内容与风格特征在内部表示中的固有纠缠问题
...
Conclusion: TeleStyle是一个有效的轻量级图像和视频风格迁移模型，能够泛化到未见过的风格而不损害内容保真度
...
<details>
  <summary>Details</summary>
Motivation: 现有基于GRPO和流匹配模型的方法在文本到图像生成中取得了显著进展，但仍存在稀疏奖励问题：整个去噪轨迹的终端奖励被应用于所有中间步骤，导致全局反馈信号与中间去噪步骤的细粒度贡献不匹配。
...
Conclusion: DenseGRPO通过密集奖励解决了GRPO方法中的稀疏奖励问题，实现了更细粒度的人类偏好对齐，并通过奖励感知探索空间校准提高了训练效果。
...
TL;DR: 提出FPL方法，通过将类别原型特征投影到查询图像特征空间并重建特征图，将分类问题转化为特征投影问题，结合CLIP原始预测，在有限监督下高效适应下游任务。
...
Method: 提出特征投影学习（FPL）方法：开发投影模型将类别原型特征投影到查询图像特征空间并重建查询图像特征图，使用负平均平方重建误差作为类别分数，将分类问题转化为特征投影问题，最终输出结合投影模型预测和原始预训练CLIP。

Result: 综合实证评估证实FPL提供卓越的准确性，以显著优势超越当前最先进方法。

Conclusion: FPL是一种简单、高效且有效的方法，解决了现有方法在适应CLIP模型到下游任务时的局限性，通过特征投影学习实现了更好的性能。
...
TL;DR: 提出RED模型，一种显式监督训练框架，继承扩散模型的强大生成能力，同时避免分布估计，解决多模态图像融合中的细节损失问题


<details>
  <summary>Details</summary>
Motivation: 多模态图像融合需要将不同源图像的互补信息整合到统一表示中，但现有扩散模型在图像融合任务中存在细节损失问题，这是由于马尔可夫过程中的噪声误差累积导致的。同时，在扩散模型中引入显式监督训练会面临计算效率挑战

Method: 提出可逆高效扩散（RED）模型，这是一个显式监督训练框架，继承扩散模型的强大生成能力，同时避免分布估计，从而解决细节损失和计算效率问题

Result: RED模型能够保持细粒度细节和视觉保真度，避免了传统扩散模型在图像融合中的细节损失问题，同时提高了计算效率

Conclusion: RED模型为多模态图像融合提供了一种有效的解决方案，既保留了扩散模型的生成优势，又克服了其在融合任务中的细节损失和计算效率限制
...
<details>
  <summary>Details</summary>
Motivation: 现有基于注意力动态的方法在可靠区分幻觉和事实性输出方面存在局限，仅依赖前向传播注意力模式而忽略了梯度信号，需要更全面的诊断框架。

Method: 提出LVLMs-Saliency梯度感知诊断框架，融合注意力权重和输入梯度量化视觉基础强度；基于分析发现，提出Saliency-Guided Rejection Sampling (SGRS)动态过滤候选token，以及Local Coherence Reinforcement (LocoRE)模块加强当前token对前序token的注意力。

Result: 在多个LVLMs上的实验表明，该方法显著降低幻觉率，同时保持流畅性和任务性能，提供了增强模型可靠性的鲁棒且可解释的解决方案。

Conclusion: LVLMs-Saliency框架通过梯度感知分析揭示了幻觉产生的关键模式，提出的推理时干预机制能有效减少幻觉，为提升大型视觉语言模型的可靠性提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 传统域自适应方法通常需要访问源域数据、对抗训练或复杂的伪标签技术，这些方法计算成本高且实现复杂。本文旨在解决这些挑战，提出一种无需源域数据、更高效的自适应方法。

Method: 提出基于多视图增强和潜在空间一致性的无源域自适应方法：1）生成目标域数据的多个增强视图；2）在潜在空间中最小化这些视图特征表示之间的距离以学习域不变特征；3）引入ConvNeXt编码器；4）设计结合分类和一致性目标的损失函数。

Result: 在Office-31、Office-Home和Office-Caltech数据集上分别达到90.72%、84%和97.12%的平均分类准确率。相比现有方法，分别提升了+1.23%、+7.26%和+1.77%的平均准确率。

Conclusion: 该方法成功实现了无需源域数据的域自适应，通过多视图增强和潜在空间一致性有效学习域不变特征，在多个数据集上超越了现有方法的性能。
...
TL;DR: 论文提出了一个全面的视频生成质量评估协议，关注外观、运动和相机三个关键方面，定义了10种常见伪影类别，并构建了包含8万视频的大规模数据集GenVID，开发了密集视频伪影识别框架DVAR。
...
Method: 1) 提出关注外观、运动和相机三个关键方面的评估协议；2) 定义10种常见生成伪影类别；3) 构建包含8万视频的大规模数据集GenVID；4) 开发密集视频伪影识别框架DVAR，用于细粒度伪影检测和分类。

Result: 实验表明，该方法显著提高了伪影检测的准确性，并能有效过滤低质量内容。

Conclusion: 该研究为视频生成质量评估提供了一个全面框架，通过细粒度伪影检测和分类，解决了现有方法缺乏详细定位的问题，有助于提升生成视频的质量控制。
...
TL;DR: C-SAM框架通过将锐度感知学习从参数扰动转移到掩码扰动，在模型压缩的同时保持对抗鲁棒性，解决了SAM训练模型在剪枝后鲁棒性下降的问题。


<details>
  <summary>Details</summary>
Motivation: SAM能提升DNN对输入变化的鲁棒性，但在设备端部署的紧凑性要求下，其与模型压缩的交互关系尚未充分探索。简单剪枝SAM训练模型会破坏鲁棒性，而先剪枝后应用SAM又受限于早期剪枝模式的结构限制。

Method: 提出压缩感知锐度最小化(C-SAM)框架，将锐度感知学习从参数扰动转移到掩码扰动。通过在训练中显式扰动剪枝掩码，C-SAM促进相对于模型结构的更平坦损失景观，从而发现同时优化模型紧凑性和输入变化鲁棒性的剪枝模式。

Result: 在CelebA-HQ、Flowers-102和CIFAR-10-C数据集上，使用ResNet-18、GoogLeNet和MobileNet-V2进行广泛实验，C-SAM始终比强基线获得更高的认证鲁棒性，提升幅度达42%，同时保持与相应未剪枝模型相当的任务准确率。

Conclusion: C-SAM通过将锐度感知学习从参数空间扩展到结构空间，成功解决了模型压缩与鲁棒性之间的权衡问题，为设备端部署提供了同时具备紧凑性和鲁棒性的DNN模型。
...
TL;DR: 该研究提出了一种双域学习策略，通过整合无施源器（NA）和有施源器（WA）的CT数据，仅需10-30%的WA数据即可达到与纯WA数据训练相当的膀胱分割性能，有效应对协变量偏移问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中协变量偏移导致的性能下降是主要挑战。在CT引导的妇科近距离放疗膀胱分割任务中，无施源器（NA）的CT扫描数据丰富，而有施源器（WA）的数据稀缺且存在解剖变形和成像伪影，导致自动分割困难。研究旨在探索如何有效利用分布偏移的样本来支持有限目标域数据的学习。

Method: 提出双域学习策略，整合NA和WA CT数据。使用精心策划的混合数据集，系统实验在轴位、冠状位和矢状位平面进行，采用多种深度学习架构。研究探索了在主要NA训练集中引入不同比例的WA数据对分割性能的影响。

Result: 仅使用NA数据无法捕捉WA图像的解剖和伪影特征。但在NA训练集中引入少量（10-30%）WA数据即可显著提升性能，达到与纯WA数据训练模型相当的分割效果。获得Dice相似系数高达0.94，交并比高达0.92，表明有效的域适应和临床可靠性提升。

Conclusion: 该研究证明了整合解剖相似但分布偏移的数据集的价值，能够克服数据稀缺问题，提升近距离放疗治疗计划中基于深度学习的分割性能。双域学习策略为医学图像分割中的协变量偏移问题提供了有效解决方案。
...
TL;DR: 提出了一种基于物理结构的单图像质量估计框架，通过几何体积和材料密度推理来解决RGB图像中质量估计的模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 从视觉输入估计物体质量具有挑战性，因为质量同时取决于几何体积和材料相关的密度，这两者都无法直接从RGB外观中观察到。像素级别的质量预测是不适定问题，需要物理上有意义的表示来约束合理解的空间。

Method: 提出物理结构化框架：1) 通过单目深度估计恢复物体中心三维几何来获取体积信息；2) 使用视觉语言模型提取粗略材料语义来指导密度相关推理；3) 通过实例自适应门控机制融合几何、语义和外观表示；4) 通过两个独立的回归头预测体积相关和密度相关的物理引导潜在因子，仅使用质量监督。

Result: 在image2mass和ABO-500数据集上的实验表明，该方法始终优于最先进的方法。

Conclusion: 通过将视觉线索与物理因素对齐，提出的物理结构化框架能够有效解决单图像质量估计中的模糊性问题，在多个数据集上表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 模块化感知预测流水线存在信息流受限和误差放大的问题，而现有的基于查询的端到端模型在摄像头和LiDAR融合方面探索不足，通常采用启发式对齐和离散选择步骤，导致信息利用不充分和引入偏差。

Method: 提出查询门控可变形融合（QGDF）：1）通过掩码注意力跨摄像头和特征层级聚合图像证据；2）通过带学习偏移的全可微分BEV采样提取LiDAR上下文；3）应用查询条件门控机制自适应加权每个智能体的视觉和几何线索。整个架构在单一端到端模型中联合优化检测、跟踪和多假设轨迹预测。

Result: 在nuScenes数据集上，Li-ViP3D++提高了端到端行为质量和检测质量，达到更高的EPA（0.335）和mAP（0.502），同时显著减少误报（FP ratio 0.147），并且比之前的Li-ViP3D变体更快（139.82 ms vs. 145.91 ms）。

Conclusion: 查询空间中全可微分的摄像头-LiDAR融合可以在不牺牲部署性的情况下提高端到端感知预测的鲁棒性，表明这种融合方法能够更充分利用多模态信息。
...
Conclusion: SLDM模型能够在不完全配对图像条件下实现准确的CT图像增强，在降低碘对比剂剂量的同时保持诊断能力，为临床提供了一种有效的低剂量对比剂CT血管造影重建方法。
...
TL;DR: OSDEnhancer首次通过高效一步扩散过程实现真实世界时空视频超分辨率，结合线性预插值、时序精化-空间增强专家混合和双向可变形VAE解码器，在保持时序一致性的同时提升空间细节。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在视频超分辨率方面表现出色，但在时空视频超分辨率（STVSR）领域探索不足。现有STVSR方法通常在简化的退化假设下工作，难以应对真实世界中复杂的未知退化情况，同时需要兼顾重建保真度和时序一致性。

Method: 提出OSDEnhancer框架：1）使用线性预插值策略初始化基本时空结构；2）训练时序精化-空间增强专家混合（TR-SE MoE），让不同专家路径分别学习时序一致性和空间细节的专门表示；3）引入双向可变形VAE解码器进行循环时空聚合和传播，增强跨帧重建保真度。

Result: 实验表明该方法在真实世界场景中实现了最先进的性能，同时保持了优越的泛化能力。

Conclusion: OSDEnhancer是首个通过高效一步扩散过程实现真实世界时空视频超分辨率的方法，有效解决了复杂退化场景下的时空一致性重建问题。
...
TL;DR: CPiRi是一个通道排列不变的多变量时间序列预测框架，通过解耦时空架构和排列不变正则化策略，从数据中推断跨通道结构而非记忆固定顺序，实现结构漂移和分布漂移下的稳定部署。


<details>
  <summary>Details</summary>
Motivation: 现有多变量时间序列预测方法存在局限性：通道依赖模型学习跨通道特征但容易过拟合通道顺序，当通道增减或重排时难以适应；通道独立模型增加灵活性但忽略通道间依赖关系，限制性能。需要一种既能学习跨通道结构又对通道顺序不敏感的方法。

Method: CPiRi采用时空解耦架构：冻结的预训练时间编码器提取高质量时间特征，轻量级空间模块学习内容驱动的通道间关系。通过通道洗牌策略在训练中强制实施排列不变性，结合排列不变正则化训练策略，使模型从数据中推断跨通道结构而非记忆固定顺序。

Result: 在多个基准测试中取得最先进结果。当通道顺序被打乱时保持稳定，即使仅用一半通道训练也能对未见通道表现出强大的归纳泛化能力，同时在大规模数据集上保持实际效率。

Conclusion: CPiRi通过通道排列不变框架有效解决了多变量时间序列预测中通道顺序过拟合和跨通道依赖忽略的问题，实现了结构漂移和分布漂移下的稳定部署，具有实际应用价值。
...
Method: 1. 高斯可见性感知的多视角几何一致性约束：通过聚合跨视角共享高斯基元的可见性，实现更准确稳定的几何监督。
2. 渐进式四叉树校准的单目深度约束：从粗到细的空间尺度执行块状仿射校准，缓解深度先验的尺度模糊问题，同时保留细粒度表面细节。

Result: 在DTU和TNT数据集上的广泛实验表明，该方法在几何精度方面相比先前的高斯基方法和隐式表面重建方法都有持续改进。

Conclusion: 提出的两种约束方法有效解决了3D高斯泼溅表面重建中的关键问题，通过结合可见性感知的多视角几何一致性和渐进式校准的单目深度约束，实现了更准确的几何重建。
...
TL;DR: TopoOT：一种拓扑感知的最优传输框架，通过多过滤持久图与测试时适应相结合，实现稳健的异常分割，在2D和3D基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 深度拓扑数据分析能捕捉跨尺度的结构不变量（如连通性和环），适合异常分割。传统基于阈值的二值化方法在分布偏移下产生脆弱掩码，而TDA能将异常表征为全局结构破坏而非局部波动。

Method: 提出TopoOT框架，核心创新是最优传输链式对齐：在多阈值和过滤之间顺序对齐持久图，生成测地稳定性分数来识别跨尺度一致保留的特征。这些稳定性感知的伪标签通过OT一致性和对比目标在线训练轻量级头，确保域偏移下的稳健适应。

Result: 在标准2D和3D异常检测基准测试中，TopoOT达到最先进性能：2D数据集上平均F1提升高达+24.1%，3D异常分割基准上提升+10.2%。

Conclusion: TopoOT通过将拓扑持久性与最优传输相结合，提供了一种稳健的异常分割方法，在域偏移下表现出色，证明了拓扑结构表征在异常检测中的有效性。
...
TL;DR: SpatialGenEval是一个评估文本到图像模型空间智能的新基准，包含1230个信息密集的长提示，涵盖25个真实场景和10个空间子领域。评估发现高阶空间推理是主要瓶颈，同时构建的SpatialT2I数据集通过微调能显著提升模型的空间关系处理能力。
...
Method: 提出SpatialGenEval基准，包含1230个信息密集的长提示，涵盖25个真实场景，每个提示整合10个空间子领域和相应的10个多项选择题对。同时构建SpatialT2I数据集，包含15400个文本-图像对，通过重写提示确保图像一致性同时保持信息密度。
...
Conclusion: SpatialGenEval基准系统地评估了T2I模型的空间智能，揭示了当前模型的局限性，并通过数据中心的范式展示了实现T2I模型空间智能的有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 场景图虽然为场景理解提供了结构化抽象，但常常过拟合到虚假相关性，严重阻碍了分布外泛化能力。需要解决场景图对特定环境关系的过拟合问题。
...
Conclusion: CURVE通过因果启发的框架有效抑制了场景图中的环境特定关系，提升了分布外泛化能力，同时提供了可靠的不确定性估计来应对分布偏移。
...
TL;DR: 本文提出RAW-Flow框架，将RGB到RAW重建重新定义为确定性潜在传输问题，通过流匹配学习潜在空间向量场，结合跨尺度上下文引导和双域潜在自编码器，显著提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的方法将RGB-to-RAW重建视为直接回归任务，但由于逆ISP问题的病态性和量化RGB图像的信息损失，存在细节不一致和颜色偏差问题，需要新的解决方案。

Method: 提出RAW-Flow框架：1) 将RGB-to-RAW重建重新定义为确定性潜在传输问题，使用流匹配学习潜在空间向量场；2) 引入跨尺度上下文引导模块，将分层RGB特征注入流估计过程；3) 设计具有特征对齐约束的双域潜在自编码器，联合编码RGB和RAW输入。

Result: 大量实验表明，RAW-Flow在定量和视觉上都优于现有最先进方法，能够更准确地重建结构细节和颜色信息。

Conclusion: 通过生成视角重新定义RGB-to-RAW重建问题，提出的RAW-Flow框架有效解决了现有方法的局限性，为高质量RAW数据恢复提供了新的解决方案。
...
TL;DR: HINT是一个用于多人生成运动的自回归扩散框架，通过分层交互建模处理复杂交互，支持可变长度文本和不同人数，在InterHuman数据集上FID达到3.100，显著优于之前的5.154。


<details>
  <summary>Details</summary>
Motivation: 现有离线方法在生成固定长度运动、固定人数方面存在局限，无法处理长文本、可变文本和不同人数。自回归方法虽然能逐步预测未来运动，但需要更好地建模复杂交互和保持长期一致性。

Method: HINT采用自回归扩散框架，使用解耦的运动表示将局部运动语义与人际交互分离，在规范化潜在空间中操作。采用滑动窗口策略进行高效在线生成，聚合窗口内局部条件和跨窗口全局条件，同时建模细粒度交互和保持长期一致性。

Result: 在公开基准测试中，HINT与强离线模型性能相当，并超越自回归基线。在InterHuman数据集上，HINT的FID达到3.100，显著优于之前最佳成绩5.154。

Conclusion: HINT是首个用于多人生成运动的自回归扩散框架，通过分层交互建模有效处理复杂交互，支持可变文本和人数，在保持细粒度交互建模的同时确保长期一致性，在多个指标上达到最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有细粒度文本描述与局部图像块对齐方法存在冗余信息问题，导致文本-视觉对齐效果不佳

Method: 提出双向精炼方法：1) 视图精炼 - 通过高IoU比率去除冗余图像块；2) 描述精炼 - 通过高余弦相似度去除冗余文本描述

Result: 在6个基准数据集上，BiFTA为ViT-based和ResNet-based CLIP模型带来了卓越的零样本性能提升

Conclusion: 去除视觉-文本对齐中的冗余信息对于提升细粒度对齐效果至关重要，BiFTA方法验证了这一必要性
...
TL;DR: Quartet of Diffusions是一个结构感知的点云生成框架，通过四个协调的扩散模型分别建模全局形状潜在变量、对称性、语义部件及其空间组装，实现保证对称性、部件连贯放置和多样化高质量输出的点云生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么将形状生成视为整体过程，要么仅支持部件组合，缺乏对对称性和部件先验的完整集成。需要一种能够同时保证对称性、部件连贯放置并提供细粒度控制的点云生成框架。

Method: 提出Quartet of Diffusions框架，包含四个协调的扩散模型：1) 全局形状潜在变量模型，2) 对称性模型，3) 语义部件模型，4) 空间组装模型。通过解耦生成过程为可解释组件，使用中央全局潜在变量确保结构一致性。

Result: 实验表明该方法达到了最先进的性能，是第一个在生成过程中完全集成并强制执行对称性和部件先验的3D点云生成框架，能够保证对称性、部件连贯放置并生成多样化高质量输出。

Conclusion: Quartet of Diffusions通过结构化管道实现了保证对称性、部件连贯放置和多样化高质量输出的点云生成，支持对形状属性的细粒度控制，同时保持全局一致性，为结构感知的3D形状生成提供了新范式。
...
TL;DR: Youtu-Parsing是一个高效多功能的文档解析模型，采用ViT视觉编码器和Youtu-LLM-2B语言模型，通过高并行度解码策略（token并行和query并行）实现5-11倍加速，支持多种文档元素解析，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决传统文档解析模型在处理复杂文档结构时的效率瓶颈，特别是针对表格识别等高度结构化场景，需要开发一种能够同时兼顾高性能和高效率的文档解析方案，以支持大规模文档智能应用。

Method: 采用解耦且特征可复用的框架：1）使用动态分辨率视觉编码器的原生ViT提取共享文档特征；2）结合提示引导的Youtu-LLM-2B语言模型进行布局分析和区域提示解码；3）引入高并行度解码策略，包括token并行（每步生成64个候选token）和query并行（同时预测多个边界框内容）。
...
Conclusion: Youtu-Parsing通过创新的并行解码架构实现了文档解析的高效性和多功能性，特别适合高度结构化场景，具有显著的实验价值和实际应用潜力，为大规模文档智能应用提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着生成模型的快速发展，深度伪造检测面临新的挑战。现有方法主要将问题建模为分类或空间定位，缺乏可解释性和可靠性。需要开发能够提供可解释推理的检测方法。

Method: MARE设计了包含人类反馈强化学习（RLHF）的综合奖励函数，激励生成文本-空间对齐的推理内容。同时引入伪造解耦模块，从高级面部语义中捕获内在伪造痕迹，提升真实性检测能力。

Result: 定量和定性实验结果表明，MARE在准确性和可靠性方面达到了最先进的性能水平，能够生成高质量的推理内容。

Conclusion: MARE通过多模态对齐和强化学习机制，显著提升了视觉语言模型在深度伪造检测中的准确性和可解释性，为可解释的深度伪造检测提供了有效解决方案。
...
TL;DR: 本文提出Dummy Forcing方法，通过控制不同注意力头对历史帧的访问，减少多头自注意力中的上下文冗余，实现视频扩散模型的高效推理加速。


<details>
  <summary>Details</summary>
Motivation: 研究发现自回归视频扩散模型中的多头自注意力机制存在历史帧利用不足的问题：约25%的注意力头几乎只关注当前帧，丢弃它们的KV缓存只会导致轻微的性能下降。
...
Conclusion: Dummy Forcing通过有效控制注意力头对历史上下文的访问，显著提升了自回归视频扩散模型的推理效率，为实时视频生成提供了可行方案。
...
TL;DR: 研究探索了六种利用部分标注数据训练WMH和ISL联合分割模型的策略，发现伪标签方法效果最佳


<details>
  <summary>Details</summary>
Motivation: WMH和ISL是脑小血管病的影像特征，在FLAIR序列中视觉上容易混淆且常同时出现，开发准确分割和区分这两种特征的深度学习模型具有挑战性

Method: 研究了六种使用部分标注数据训练WMH和ISL联合分割模型的策略，结合了私有和公开数据集共2052个MRI体积，其中1341个有WMH标注，1152个有ISL标注

Result: 多种方法能有效利用部分标注数据提升模型性能，其中伪标签方法取得了最佳结果

Conclusion: 部分标注数据可以有效提升WMH和ISL分割模型的性能，伪标签是最有效的训练策略
...
TL;DR: 提出Latent Temporal Discrepancy (LTD)作为运动先验来指导损失加权，解决视频生成模型在动态场景中质量下降的问题，显著提升运动视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在静态场景表现良好，但在动态视频生成中质量下降，主要原因是噪声破坏时间一致性，且现有扩散模型对所有场景使用静态损失，限制了捕捉复杂动态的能力。

Method: 引入潜在时间差异(LTD)作为运动先验来指导损失加权。LTD在潜在空间中测量帧间变化，对差异较大的区域分配更大惩罚，对稳定区域保持常规优化，这种运动感知策略稳定训练并提升高频动态重建能力。

Result: 在通用基准VBench和运动专注的VMBench上均取得显著提升，分别超过强基线3.31%和3.58%，在运动质量方面实现显著改进。

Conclusion: LTD作为运动先验的损失加权策略有效解决了动态视频生成中的质量问题，通过运动感知的优化方法显著提升了模型对复杂动态的捕捉能力。
...
Method: 提出了CHEESE数据集（包含24K肖像收藏和573K样本，通过大型视觉语言模型流程构建）和SCheese框架（结合文本引导生成与分层身份和细节保持）。SCheese采用自适应特征融合机制保持身份一致性，并使用ConsistencyNet注入细粒度特征以保持细节一致性。

Result: 综合实验验证了CHEESE在推进PCG任务方面的有效性，SCheese实现了最先进的性能表现。

Conclusion: 本文成功定义了PCG新任务，并通过构建大规模数据集和提出创新框架，有效解决了多属性修改和细节保持的挑战，为肖像收藏生成提供了系统解决方案。
...
TL;DR: 该论文针对扩散式多模态大语言模型在缓存加速解码时出现的重复文本生成问题（重复诅咒），通过信息流分析揭示了其机制，并提出了一种即插即用的CoTA方法来缓解重复问题。


<details>
  <summary>Details</summary>
Motivation: 扩散式多模态大语言模型（dMLLMs）依赖缓存技术加速解码，但缓存机制常常导致不希望的重复文本生成，作者称之为"重复诅咒"。为了深入理解这一问题的底层机制，需要从信息流角度分析重复生成现象。

Method: 作者从信息流角度分析重复生成机制，发现三个关键发现：1）上下文token作为语义锚点聚合信息并指导最终预测；2）信息跨层传播时，上下文token的熵在深层收敛，反映模型预测确定性；3）重复通常与上下文token信息流中断及其熵在深层无法收敛相关。基于这些洞察，提出了CoTA方法：增强上下文token的注意力以保持内在信息流模式，同时在解码时对置信度分数引入惩罚项以避免由不确定上下文token驱动的输出。

Result: 通过大量实验，CoTA在缓解重复问题上表现出显著效果，并在通用任务上实现了持续的性能提升。该方法为即插即用方案，代码已开源。

Conclusion: 该研究从信息流角度深入分析了扩散式多模态大语言模型中的重复生成问题，揭示了其机制，并提出了一种有效的CoTA方法来缓解重复诅咒，同时保持模型性能。
...
TL;DR: IOTA框架将预训练模型视为黑盒与白盒的结合，通过对比错误预测与正确认知生成纠正知识，并将其转化为可解释提示来指导黑盒模块，实现更有效的下游任务适应。
...
Result: 在12个图像分类基准测试中，在少样本和易到难适应设置下，验证了纠正知识的有效性，并展示了该方法优于现有最先进方法。
...
<details>
  <summary>Details</summary>
Motivation: 该研究的动机是缩小开源与闭源技术之间的差距，为社区提供实用的世界模拟工具，以支持内容创作、游戏和机器人学习等应用领域。

Method: 基于视频生成技术构建世界模拟器，通过优化模型架构实现高保真度、长时记忆和低延迟交互。具体包括在多种环境类型中保持动态稳定性，实现分钟级时间跨度的上下文一致性，以及达到每秒16帧、延迟低于1秒的实时交互性能。

Result: 成功开发出LingBot-World世界模拟器，具备三个核心特征：1）在现实、科学、卡通等多种环境中保持高保真度和鲁棒动态；2）支持分钟级时间跨度并保持上下文一致性（长时记忆）；3）实现实时交互，每秒16帧输出时延迟低于1秒。代码和模型已开源。

Conclusion: LingBot-World作为顶级世界模型，通过开源方式缩小了技术差距，有望在内容创作、游戏开发和机器人学习等领域产生实际应用价值，赋能社区发展。
...
TL;DR: DeepSeek-OCR 2提出了一种新颖的视觉编码器DeepEncoder V2，能够根据图像语义动态重排视觉标记，模拟人类视觉感知的因果推理过程。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型以固定的光栅扫描顺序处理视觉标记，这与人类视觉感知的灵活语义驱动扫描模式相矛盾。人类视觉特别是对于复杂布局图像，表现出基于因果推理的顺序处理能力。

Method: 设计DeepEncoder V2编码器，赋予其因果推理能力，使其能够在基于LLM的内容解释之前智能地重排视觉标记。探索通过两个级联的1D因果推理结构实现2D图像理解的新范式。

Result: 提出了新的架构方法，代码和模型权重已在GitHub公开，为实现真正的2D推理提供了潜在可能。

Conclusion: 这项工作探索了通过因果推理结构动态重排视觉标记的新范式，为视觉语言模型架构设计提供了创新思路，有望实现更接近人类视觉感知的图像理解。
...
TL;DR: DiffVC-RT是首个实现实时扩散神经视频压缩的框架，通过高效架构设计、显隐式一致性建模和异步并行解码，在保持高质量的同时大幅降低计算复杂度和推理延迟。
...
Method: 1. 高效信息模型架构：通过策略性模块替换和剪枝降低计算复杂度；2. 显隐式一致性建模：引入零成本在线时间移位模块和混合隐式一致性约束；3. 异步并行解码流水线：采用混合半精度和批量维度时间移位设计。

Result: 在HEVC数据集上相比VTM-17.0实现80.1%的LPIPS比特率节省，在NVIDIA H800 GPU上对720p视频达到206/30 fps的实时编码/解码速度。

Conclusion: DiffVC-RT在保持高质量压缩的同时实现了实时性能，标志着基于扩散的视频压缩技术的重要里程碑。
...
TL;DR: 本文提出StructAlign方法，通过结构化跨模态对齐来解决持续文本-视频检索中的特征漂移问题，包括跨模态非合作漂移和模态内特征漂移。


<details>
  <summary>Details</summary>
Motivation: 持续文本-视频检索面临灾难性遗忘的挑战，主要原因是特征漂移问题：模态内特征漂移和跨模态非合作漂移导致的模态不对齐。

Method: 提出StructAlign方法：1) 引入单纯形等角紧框架作为统一几何先验；2) 设计跨模态ETF对齐损失，将文本和视频特征与类别级ETF原型对齐；3) 设计跨模态关系保持损失，利用互补模态保持跨模态相似性关系。

Result: 在基准数据集上的广泛实验表明，该方法在持续文本-视频检索任务中一致优于最先进的持续检索方法。

Conclusion: 通过同时解决跨模态非合作漂移和模态内特征漂移，StructAlign有效缓解了持续文本-视频检索中的灾难性遗忘问题。
...
TL;DR: 该论文研究了行人重识别（ReID）的三种训练范式（监督学习、自监督学习、语言对齐模型），评估了它们在跨域应用中的鲁棒性，并探讨了基础模型在提升泛化能力方面的作用。


<details>
  <summary>Details</summary>
Motivation: 行人重识别在计算机视觉中仍然是一个具有挑战性的问题。本研究旨在评估最先进的ReID模型在跨域应用中的鲁棒性，并研究基础模型如何通过更丰富、更具可迁移性的视觉表示来改善泛化能力。

Method: 比较了三种训练范式：监督学习、自监督学习和语言对齐模型。分析了11个模型在9个数据集上的表现，特别关注基础模型（如SigLIP2）在ReID任务中的性能。

Result: 监督模型在训练域内表现优异，但在跨域数据上表现崩溃。语言对齐模型尽管没有针对ReID任务进行专门训练，却在跨域场景中表现出令人惊讶的鲁棒性。

Conclusion: 语言对齐模型在跨域行人重识别任务中展现出强大的泛化能力，这为未来ReID研究提供了新的方向，即利用基础模型的丰富视觉表示来提升跨域性能。
...
TL;DR: ProSkill是首个用于程序性任务中动作级别技能评估的基准数据集，提供绝对技能评估标注和成对评估标注，通过瑞士锦标赛方案和ELO评分系统实现高效标注，现有SOTA算法在该数据集上表现不佳，突显了其挑战性和价值。
...
Method: 提出新颖且可扩展的标注协议，采用瑞士锦标赛方案进行高效的成对比较，然后通过ELO评分系统将这些成对评估聚合成一致、连续的全局分数，从而创建绝对技能评估排名。

Result: 创建了ProSkill基准数据集，用于评估现有最先进的技能评估算法（包括基于排名和成对范式的方法），结果显示当前SOTA算法在该数据集上表现不佳，证明了该数据集的挑战性和价值。

Conclusion: ProSkill填补了程序性视频技能评估领域大规模数据集的空白，其标注协议和数据集为技能评估研究提供了新的基准和挑战，现有算法的不足突显了该领域仍需进一步研究。
...
TL;DR: LEAF是一个标签高效的图像质量评估框架，通过从MLLM教师模型蒸馏感知质量先验到轻量级学生回归器，用最少的人工监督实现MOS尺度校准
...
Method: 提出LEAF框架：1）教师模型通过点级判断和配对偏好进行密集监督，并估计决策可靠性；2）学生模型通过联合蒸馏学习教师的质量感知模式；3）在小规模MOS子集上进行校准以对齐人类标注

Result: 在用户生成和AI生成的IQA基准测试中，该方法显著减少了对人类标注的需求，同时保持了强大的MOS对齐相关性，使得在有限标注预算下实现轻量级IQA成为可能

Conclusion: LEAF框架通过有效蒸馏MLLM教师的感知质量先验，解决了MLLM-based IQA中的MOS尺度校准瓶颈，实现了在有限标注预算下的高效图像质量评估
...
TL;DR: LEMON是一个基于STEM讲座视频的多模态评估基准，专注于长时程推理和跨模态整合，包含2,277个视频片段和4,181个高质量QA对，揭示了当前最先进MLLMs在时序推理和教学预测方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉、音频和语言任务上取得了显著进展，但在长形式、知识密集、时序结构化的教育内容上的表现尚未得到充分探索。需要建立一个专门的基准来评估模型在这类复杂内容上的能力。

Method: 构建了LEMON基准，包含2,277个STEM讲座视频片段，覆盖5个学科和29门课程，平均时长196.1秒。生成了4,181个高质量QA对（3,413个选择题和768个开放性问题），设计了6个主要任务和12个子任务，涵盖从感知到推理再到生成的完整认知谱系。

Result: 综合实验显示各任务间存在显著性能差距，即使是GPT-4o等最先进的MLLMs在时序推理和教学预测任务上也表现不佳。基准揭示了当前模型在长形式教学内容理解方面的局限性。
...
TL;DR: 该论文探讨了视觉编码与视觉标记技术之间的统一性，指出压缩效率与模型性能的正相关性，并预测了下一代视觉编解码器和标记技术的发展方向。


<details>
  <summary>Details</summary>
Motivation: 传统视觉编码技术已在多媒体领域取得巨大成功，而新兴的生成式多模态大模型的视觉标记技术也追求在表示学习中最大化语义信息保真度同时最小化计算成本。这两种技术具有相似的根本目标，但缺乏统一的理论框架。

Method: 首先全面综述视觉编码和视觉标记技术两大主导技术家族，然后从优化角度统一它们，讨论压缩效率与模型性能权衡的本质。基于提出的统一公式，综合双向见解并预测下一代技术发展。

Result: 实验表明面向任务的标记开发在多模态LLMs、AIGC和具身AI等实际任务中具有巨大潜力，揭示了未来可能像传统编解码器（如H.264/265）那样标准化通用标记技术的前景。

Conclusion: 压缩效率与智能模型性能密切相关，视觉编码和视觉标记技术可以在统一框架下相互借鉴，为下一代高效视觉表示技术发展指明方向，有望实现面向广泛智能任务的统一有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 文本到视频扩散模型虽然进展迅速，但其人口统计学偏见（特别是性别偏见）尚未得到充分探索。研究发现偏见主要来自预训练文本编码器，即使对于中性提示也会编码隐含的性别关联。

Method: 1. 分析T2V模型的人口统计学偏见，发现主要源于预训练文本编码器；2. 提出性别倾向评分量化偏见；3. 通过锚点球面测地变换中和提示嵌入，同时保持语义；4. 采用动态去噪计划，仅在早期身份形成步骤应用去偏以保持时间一致性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏对物体功能和相应人-场景接触的显式推理，导致生成不合理的或功能错误的交互。需要一种能够生成功能正确、物理合理的3D人-场景交互的方法，应用于具身AI、机器人和交互内容创建。

Method: FunHSI采用无需训练的功能驱动框架：1) 通过功能感知接触推理识别功能场景元素并重建其3D几何；2) 通过接触图建模高级交互；3) 利用视觉语言模型合成任务图像并估计3D身体和手部姿态；4) 通过分阶段优化细化3D身体配置以确保物理合理性和功能正确性。

Result: FunHSI不仅能够合成更合理的通用3D交互（如"坐在沙发上"），还支持细粒度的功能人-场景交互（如"提高房间温度"）。在多样化的室内外场景中，FunHSI始终生成功能正确且物理合理的人-场景交互。

Conclusion: FunHSI是一个有效的训练免费框架，通过显式的功能推理和接触建模，解决了生成功能正确、物理合理的3D人-场景交互的挑战，在具身AI、机器人和交互内容创建中具有应用价值。
...
TL;DR: 本文提出了一种融合图像和惯性测量的多模态道路表面分类框架，并引入包含真实、视觉和合成三个子集的ROAD数据集，在多个基准测试中取得显著性能提升。
...
Method: 提出多模态框架，使用轻量级双向交叉注意力模块融合图像和惯性测量，然后通过自适应门控层在域偏移下调整模态贡献。同时引入包含真实世界多模态记录、大规模视觉子集和合成子集的ROAD数据集。

Result: 在PVS基准测试上比先前最优方法提升1.4个百分点，在ROAD多模态子集上提升11.6个百分点，在少数类别上获得更高的F1分数，并在夜间、大雨和混合表面过渡等挑战性视觉条件下表现稳定。

Conclusion: 结合经济型摄像头和IMU传感器与多模态注意力机制，为道路表面理解提供了可扩展、鲁棒的基础，特别适用于环境变化大且成本限制高的地区。
...
TL;DR: FreeFix提出了一种无需微调的2D-3D交替优化方法，利用预训练图像扩散模型提升外推视图渲染质量，通过像素级置信掩码定位不确定区域进行针对性改进。


<details>
  <summary>Details</summary>
Motivation: 现有方法在泛化性和保真度之间存在权衡：基于微调扩散模型的方法能提高保真度但可能过拟合，而无微调方法保持泛化性但保真度较低。需要一种既能保持强泛化能力又能提升外推视图渲染质量的方法。

Method: 提出FreeFix框架：1) 2D-3D交替优化策略，利用预训练图像扩散模型进行一致性优化而无需视频扩散模型；2) 像素级置信掩码识别不确定区域进行针对性改进；3) 完全无需微调，保持预训练模型的泛化能力。

Result: 在多个数据集上的实验表明，FreeFix提高了多帧一致性，性能达到或超过基于微调的方法，同时保持了强大的泛化能力。

Conclusion: FreeFix通过创新的2D-3D交替优化和置信掩码机制，成功突破了泛化性与保真度之间的权衡，为无需微调的扩散模型辅助新视角合成提供了有效解决方案。
...
### [50] [NeuroAI and Beyond](https://arxiv.org/abs/2601.19955)
...
<details>
  <summary>Details</summary>
Motivation: 神经科学与人工智能在过去几年都取得了显著进展，但两者之间的连接仍然松散。本文旨在识别这两个领域当前和未来的协同机会，促进更紧密的交叉融合。

Method: 基于2025年8月举行的研讨会，聚焦于具身性、语言与通信、机器人学、人类与机器学习以及神经形态工程等子领域，收集多位领先研究人员的个人观点，并附上研究人员和学员的SWOT分析。

Result: 提出了NeuroAI概念——一种神经科学启发的人工智能，识别了多个交叉领域的进展和未来方向，包括具身智能、语言处理、机器人学习等，并通过SWOT分析评估了NeuroAI的益处和风险。

Conclusion: 倡导发展NeuroAI，认为这种神经科学启发的人工智能有潜力显著提高AI算法的范围和效率，同时改变我们对生物神经计算的理解方式，促进两个领域的共同进步。
...
### [51] [Teaching LLMs to Ask: Self-Querying Category-Theoretic Planning for Under-Specified Reasoning](https://arxiv.org/abs/2601.20014)
*Shuhui Qu*

Main category: cs.AI

TL;DR: SQ-BCP是一种用于部分可观测环境下推理时规划的方法，通过显式表示前提条件状态（满足/违反/未知），使用自查询和桥接假设解决未知信息，通过双向搜索和拉回验证器确保计划与目标兼容。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在部分可观测环境下的推理时规划经常失败：当任务关键前提条件未在查询时指定时，模型容易产生幻觉或违反硬约束的计划。

Method: 引入自我查询双向分类规划（SQ-BCP），显式表示前提条件状态（Sat/Viol/Unk），通过（i）针对性的自查询到预言机/用户或（ii）桥接假设（通过额外动作建立缺失条件）解决未知信息。采用双向搜索，使用基于拉回的验证器作为目标兼容性的分类证书，距离分数仅用于排序和剪枝。

Result: 在WikiHow和RecipeNLG任务中，当隐藏前提条件时，SQ-BCP将资源违规率降低到14.9%和5.8%（最佳基线为26.0%和15.7%），同时保持竞争力的参考质量。

Conclusion: SQ-BCP在部分可观测环境下显著提高了推理时规划的可靠性，通过显式处理未知前提条件和严格的验证机制，在保持计划质量的同时大幅减少了约束违反。
...
### [52] [Fuzzy Categorical Planning: Autonomous Goal Satisfaction with Graded Semantic Constraints](https://arxiv.org/abs/2601.20021)
*Shuhui Qu*

Main category: cs.AI

TL;DR: 提出模糊范畴论规划(FCP)，通过模糊逻辑处理自然语言规划中的模糊谓词，使用t-范数组合计划质量，同时保持可执行性的严格验证。
...
Method: FCP为每个动作（态射）标注[0,1]程度值，使用Lukasiewicz t-范数组合计划质量，通过拉回验证保持可执行性检查。使用LLM进行k样本中位数聚合从语言中获取分级适用性，支持基于剩余的后向需求中间相遇搜索。
...
Conclusion: FCP成功地将模糊逻辑与范畴论规划结合，有效处理自然语言规划中的模糊谓词，在保持可执行性验证的同时支持分级质量评估。
...
### [53] [Insight Agents: An LLM-Based Multi-Agent System for Data Insights](https://arxiv.org/abs/2601.20048)
*Jincheng Bai,Zhenyu Zhang,Jennifer Zhang,Zhihuai Zhu*

Main category: cs.AI

TL;DR: 论文提出Insight Agents (IA)，一个基于LLM的对话式多智能体数据洞察系统，旨在帮助电商卖家通过自动化信息检索获取个性化数据和业务洞察，降低决策成本并提高决策速度。


<details>
  <summary>Details</summary>
Motivation: 电商卖家面临两大挑战：1）难以发现和有效利用现有程序工具；2）难以理解和利用来自各种工具的丰富数据。因此需要开发一个能够提供个性化数据洞察的系统，作为卖家的"力量倍增器"，降低决策成本并提高决策速度。

Method: 采用基于计划-执行范式的端到端多智能体系统架构，包含分层结构：管理智能体和两个工作智能体（数据呈现和洞察生成）。管理智能体采用轻量级编码器-解码器模型进行OOD检测，结合BERT分类器进行智能体路由。工作智能体采用API数据模型进行战略规划，将查询分解为细粒度组件，并动态注入领域知识增强洞察生成。

Result: 系统已在美国亚马逊卖家平台上线，人工评估准确率达到90%，P90延迟低于15秒，实现了高准确率和低延迟的目标。

Conclusion: Insight Agents系统成功解决了电商卖家在工具使用和数据理解方面的挑战，通过多智能体架构和优化的ML解决方案，实现了高效、准确、低延迟的数据洞察服务，有效提升了卖家的业务决策效率。
...
TL;DR: 本文提出了一种多模态LLM智能体框架，用于解决传统遥感影像变化检测方法在城市新建公园发展监测中的局限性，通过语义理解和推理能力实现更灵活的多模态数据分析。


<details>
  <summary>Details</summary>
Motivation: 传统基于遥感影像的变化检测方法在城市新建公园发展监测中存在明显局限性，难以满足当前城市规划管理的需求，特别是在处理复杂多模态数据分析和多样化应用场景方面能力不足。

Method: 提出了一个多模态LLM智能体框架，设计了通用的横向和纵向数据对齐机制以确保多模态数据的一致性和有效追踪，同时构建了特定工具包来缓解LLM因缺乏领域知识而产生的幻觉问题。

Result: 与原始GPT-4o和其他智能体相比，该方法能够实现稳健的多模态信息融合与分析，为城市公园发展监测提供了可靠且可扩展的解决方案。

Conclusion: 该多模态LLM智能体框架充分利用了LLM的语义理解和推理能力，有效解决了城市公园发展监测中的挑战，为城市规划和管理提供了更智能的分析工具。
...
TL;DR: SEER框架通过内生循环训练，将统一多模态模型的理解能力转化为显式生成推理步骤，解决认知鸿沟问题


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型虽然具备强大的理解能力，但这种能力往往无法有效指导生成过程，存在认知鸿沟问题

Method: 提出内生重提示机制，通过SEER训练框架建立两阶段内生循环：1) RLVR通过课程学习激活模型潜在评估能力；2) RLMT利用内生奖励信号优化生成推理策略

Result: SEER在评估准确性、重提示效率和生成质量方面均优于最先进基线，且不牺牲通用多模态能力

Conclusion: 内生重提示机制成功将模型理解从被动编码过程转化为显式生成推理步骤，有效弥合了认知鸿沟
...
Conclusion: AMA框架通过多智能体协作的自适应记忆管理，有效解决了现有记忆系统的局限性，为LLM智能体提供了更高效、更一致的长时记忆支持。
...
TL;DR: PoT框架通过在线优化策略解决LLM长程推理问题，使用探索机制生成候选方案并通过GRPO更新LoRA适配器，显著提升推理性能


<details>
  <summary>Details</summary>
Motivation: 当前LLM在复杂长程推理中存在困难，现有方法仅将执行反馈作为外部信号用于过滤或重写轨迹，未能将其内化以改进底层推理策略。受波普尔"猜想与反驳"认识论启发，认为智能需要通过学习失败尝试实时演化模型策略

Method: 提出Policy of Thoughts (PoT)框架，将推理重新定义为实例内在线优化过程。首先通过高效探索机制生成多样化候选解决方案，然后使用Group Relative Policy Optimization (GRPO)基于执行反馈更新瞬态LoRA适配器

Result: PoT显著提升性能：4B模型在LiveCodeBench上达到49.71%准确率，超过GPT-4o和DeepSeek-V3，尽管模型规模小50倍以上

Conclusion: PoT通过闭环设计实现模型推理先验的动态、实例特定细化，为LLM复杂推理问题提供了有效的在线优化解决方案
...
TL;DR: CtrlCoT是一个双粒度思维链压缩框架，通过语义抽象和令牌级剪枝的协调，减少推理延迟和内存成本，同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: 思维链提示虽然能提升大语言模型的推理能力，但冗长的推理轨迹会导致高延迟和高内存成本，现有压缩方法要么过于保守（语义层面缩短），要么过于激进（令牌级剪枝），容易丢失关键推理线索并降低准确性。

Method: 提出CtrlCoT框架，包含三个核心组件：1）分层推理抽象：生成多粒度语义的思维链；2）逻辑保持蒸馏：训练逻辑感知剪枝器，保留关键推理线索（如数字和运算符）；3）分布对齐生成：对齐压缩轨迹与推理风格，避免碎片化。

Result: 在MATH-500数据集上使用Qwen2.5-7B-Instruct模型，CtrlCoT相比最强基线减少了30.7%的令牌使用量，同时准确率提升了7.6个百分点。

Conclusion: CtrlCoT通过协调语义抽象和令牌级剪枝，实现了更高效可靠的推理，在减少计算成本的同时保持了推理准确性。
...
### [59] [Normative Equivalence in human-AI Cooperation: Behaviour, Not Identity, Drives Cooperation in Mixed-Agent Groups](https://arxiv.org/abs/2601.20487)
*Nico Mutzner,Taha Yasseri,Heiko Rauhut*

Main category: cs.AI

TL;DR: 研究通过四玩家公共物品游戏实验发现，在人类-AI混合小组中，合作规范机制与纯人类小组相似，合作水平不受AI标签影响，支持"规范等价性"模式。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注人类与AI的双向互动，但对AI如何影响小群体中合作规范的形成与维持知之甚少。本研究旨在填补这一空白，探讨AI参与对群体合作规范的影响。

Method: 采用在线实验设计，使用重复四玩家公共物品游戏。每组包含三名人类参与者和一个机器人，机器人被标记为人类或AI，并遵循三种预设策略之一：无条件合作、条件合作或搭便车。共有236名参与者。

Result: 合作主要由互惠群体动态和行为惯性驱动，这些规范机制在不同条件下运作相同。合作水平在人类和AI标签间无显著差异。后续囚徒困境中规范持续性也无差异，参与者的规范感知相似。合作取决于群体行为而非伙伴身份。

Conclusion: 支持"规范等价性"模式，即维持合作的机制在人类-AI混合群体和纯人类群体中功能相似。合作规范足够灵活，可以扩展到人工智能体，模糊了人类与AI在集体决策中的界限。
...
### [60] [PathWise: Planning through World Model for Automated Heuristic Design via Self-Evolving LLMs](https://arxiv.org/abs/2601.20539)
...
Method: 提出PathWise多智能体推理框架：将启发式生成建模为在蕴含图上的序列决策过程；策略智能体规划进化动作；世界模型智能体基于动作生成启发式推演；批评智能体提供路由反射总结先前步骤的经验教训。

Result: 实验表明：PathWise能更快收敛到更好的启发式；在不同LLM骨干上具有良好泛化性；能够扩展到更大的问题规模。

Conclusion: PathWise将基于LLM的自动启发式设计从试错进化转向基于推理的状态感知规划，通过世界模型和状态记忆显著提升了启发式生成的质量和效率。
...
TL;DR: 本文研究了部分可观测环境下基于迭代条件风险价值（ICVaR）动态风险度量的风险敏感规划，开发了具有有限时间性能保证的ICVaR策略评估算法，并将三种在线规划算法扩展为优化ICVaR值函数而非期望回报。


<details>
  <summary>Details</summary>
Motivation: 在部分可观测环境中进行风险敏感规划的需求，传统基于期望回报的规划方法无法有效控制尾部风险，需要开发能够优化动态风险度量ICVaR的规划算法。

Method: 1. 开发了ICVaR策略评估算法，具有与动作空间基数无关的有限时间性能保证；2. 将三种在线规划算法（稀疏采样、PFT-DPW、POMCPOW）扩展为优化ICVaR值函数；3. 引入风险参数α控制风险厌恶程度；4. 为ICVaR稀疏采样建立了风险敏感目标下的有限时间性能保证，并设计了针对ICVaR的新型探索策略。

Result: 在基准POMDP领域上的实验表明，提出的ICVaR规划器相比风险中性对应方法实现了更低的尾部风险，验证了风险敏感规划的有效性。

Conclusion: 本文成功开发了基于ICVaR动态风险度量的风险敏感规划框架，为部分可观测环境下的风险控制提供了有效的算法解决方案，通过风险参数α实现了从风险中性到风险厌恶的平滑过渡。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI对齐研究缺乏实证测试方法，需要开发可复制的框架来在实际部署前压力测试对齐提案，同时探索AI系统是否能够参与复杂的对话推理过程。

Method: 采用基于和平研究的Viral Collaborative Wisdom方法，设计实验让Claude、Gemini和GPT-4o三个大语言模型扮演四个不同角色（提议者、回应者、监督者、翻译者），在六种条件下进行72轮对话，共576,822字符的结构化交流。

Result: AI系统能够有意义地参与和平研究概念讨论，从不同架构视角提出互补的反对意见，并产生初始框架中不存在的涌现见解（如"VCW作为过渡框架"）。不同模型关注点不同：Claude强调验证挑战，Gemini关注偏见和可扩展性，GPT-4o突出实施障碍。

Conclusion: 该框架为研究人员提供了在实际实施前压力测试对齐提案的可复制方法，初步证据表明AI具备VCW所提出的对话推理能力。但对话更多关注过程元素而非AI本质的基础主张，未来研究可探索人-AI混合协议和扩展对话研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习可验证奖励方法在算法和数据层面都缺乏对更具挑战性问题的关注，这限制了模型在数学推理中未充分开发能力的提升

Method: 提出MathForge框架，包含两个核心组件：难度感知组策略优化算法（DGPO）通过难度平衡组优势估计和难度感知问题级加权来纠正GRPO的隐式不平衡；多方面问题重构策略（MQR）通过多角度重构问题来增加难度同时保持原答案

Result: 在多种数学推理任务上，MathForge显著优于现有方法，代码和增强数据已开源

Conclusion: MathForge通过算法和数据层面的协同改进，有效提升了模型在数学推理中对难题的处理能力，形成了MQR扩展数据边界、DGPO有效学习增强数据的良性循环
...
### [64] [Investigating the Development of Task-Oriented Communication in Vision-Language Models](https://arxiv.org/abs/2601.20641)
*Boaz Carmeli,Orr Paradise,Shafi Goldwasser,Yonatan Belinkov,Ron Meir*

Main category: cs.AI

TL;DR: LLM智能体能在协作推理任务中发展出不同于自然语言的任务导向通信协议，这些协议具有高效性和隐蔽性特征，引发透明度和控制担忧。
...
### [65] [Implementing Metric Temporal Answer Set Programming](https://arxiv.org/abs/2601.20735)
*Arvid Becker,Pedro Cabalar,Martin Diéguez,Susana Hahn,Javier Romero,Torsten Schaub*

Main category: cs.AI

TL;DR: 提出了一种计算性度量答案集编程方法，用于表达定量时间约束，通过差异约束扩展解决时间粒度导致的规模化问题。


<details>
  <summary>Details</summary>
Motivation: 需要处理定量时间约束（如持续时间和截止时间）的度量答案集编程方法，但细粒度时间约束会显著加剧ASP的接地瓶颈，影响可扩展性。

Method: 利用ASP的差异约束扩展（一种简化的线性约束形式）来外部处理时间相关方面，将度量ASP与时间粒度解耦。

Result: 该方法有效地将度量ASP与时间精度分离，产生的解决方案不受时间精度影响，解决了细粒度时间约束下的可扩展性问题。

Conclusion: 通过差异约束扩展ASP，成功开发了一种能够处理定量时间约束且不受时间粒度影响的度量ASP计算方法，解决了可扩展性瓶颈。
...
TL;DR: 提出Gap-K%方法，利用LLM预训练中的优化动态，通过分析top-1预测与目标token之间的log概率差距来检测预训练数据，结合滑动窗口策略捕获局部相关性


<details>
  <summary>Details</summary>
Motivation: 大规模预训练语料库的不透明性引发了隐私和版权问题，现有基于token似然的方法忽略了与top-1预测的差异以及相邻token之间的局部相关性

Method: 基于LLM预训练优化动态，利用top-1预测token与目标token之间的log概率差距，采用滑动窗口策略捕获局部相关性并减轻token级波动

Result: 在WikiMIA和MIMIR基准测试中达到最先进性能，在各种模型大小和输入长度上一致优于先前基线方法

Conclusion: Gap-K%通过利用预训练优化动态中的梯度信号，有效解决了现有预训练数据检测方法的局限性，为隐私和版权保护提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 在去中心化联邦学习中，设备间的个体经验和交互程度差异导致数据和模型初始化异质性，造成本地模型参数变化，从而降低收敛速度

Method: 提出新颖的聚合方法，通过捕捉本地模型参数变化来执行鲁棒的邻居更新聚合。具体使用本地模型在其本地数据集上的二阶信息近似来生成共识权重，这些权重用于在聚合前缩放邻居更新

Result: 在计算机视觉任务上的广泛实验表明，该方法在降低通信成本的同时，表现出本地模型的强大泛化能力

Conclusion: 通过显式处理本地模型参数层面的变化证据可信度，提出的方法有效解决了去中心化联邦学习中的数据与模型异质性问题
...
TL;DR: Oculomix：一种用于视网膜图像分析的层次化采样混合增强方法，通过考虑患者和检查级别的临床先验来保留患者特定属性，相比传统图像级混合方法在心血管事件预测上提升3%的AUROC。


<details>
  <summary>Details</summary>
Motivation: 传统图像级混合数据增强方法（如CutMix和MixUp）在训练视网膜图像分析模型时，会扰动患者特定的医疗共病和临床因素，因为这些方法只考虑图像和标签，忽略了患者层面的临床属性。

Method: 提出Oculomix层次化采样策略，基于两个临床先验：1）同一患者同一时间点获取的图像共享相同属性（检查级别）；2）同一患者不同时间点获取的图像具有软时间趋势（患者级别）。该方法将混合空间约束在患者和检查级别，利用层次关系更好地保留患者特定特征。

Result: 在大型多民族人群（Alzeye）的五年前瞻性心血管事件预测任务中，Oculomix相比图像级CutMix和MixUp方法，AUROC指标提升高达3%，证明了该方法在眼组学中的必要性和价值。

Conclusion: Oculomix通过考虑患者和检查级别的层次关系，有效解决了传统混合增强方法在眼组学中扰动患者特定属性的问题，显著提升了心血管事件预测性能，为基于视网膜图像的疾病预测提供了更有效的训练策略。
...
TL;DR: 论文通过几何和统计物理视角研究Transformer语言模型中多步推理能力的涌现，发现隐藏状态轨迹在隐式黎曼流形上的流动存在相变现象，有效维度在临界深度处急剧下降，形成可重用的瞬态类对象结构。


<details>
  <summary>Details</summary>
Motivation: 研究深度Transformer语言模型中多步推理能力的涌现机制，从几何和统计物理的角度理解模型内部表示的形成和演化过程。

Method: 将隐藏状态轨迹视为隐式黎曼流形上的流动，分析激活的层间协方差谱，追踪其与随机矩阵本体的偏差。使用基于稀疏性/局部化的序参数，将前向传播形式化为离散粗粒度映射，并将稳定"概念盆地"的出现与此类重整化动力学的固定点联系起来。

Result: 在1.5B-30B规模的模型中观察到有效维度的急剧下降，表明存在相变现象：基于稀疏性的序参数在临界归一化深度γ_c≈0.42处出现不连续性。低熵区域表现为谱尾塌缩和表示空间中瞬态、可重用对象结构的形成（称为瞬态类对象）。

Conclusion: 深度Transformer语言模型中存在相变现象，导致有效维度降低和稳定概念盆地的形成，这为多步推理能力的涌现提供了统计物理和几何解释，理论条件将逻辑可分性与谱衰减联系起来，并通过多层探针在多个开源模型家族中得到验证。
...
<details>
  <summary>Details</summary>
Motivation: 探索在没有显式通信或多样性激励的情况下，学习者群体如何自发发展出协调且多样化的行为，解决群体智能中的专业化分工问题。

Method: 提出NichePopulation算法，结合竞争排斥和生态位亲和度追踪机制，通过竞争动态诱导学习者自发分区成为不同环境机制下的专家。

Result: 在六个实际领域验证，平均专业化指数达0.75，效应量Cohen's d > 20；多样化群体比同质基线性能提升26.5%；比MARL基线方法性能提升4.3倍且速度快4倍。

Conclusion: 竞争机制足以诱导涌现的专业化分工，学习者通过竞争动态自发分区成为不同环境机制下的专家，实现了生态位理论在实际应用中的验证。
...
TL;DR: 提出一种基于概率神经元的智能传感范式，通过概率决策实现数据采集的自主激活，在保持信息完整性的同时显著提升能效
...
Method: 受自主神经系统启发，采用由模拟特征提取电路驱动的概率神经元(p-neuron)，实现微秒级响应时间的概率决策

Result: 在主动地震勘测数据验证中实现无损概率数据采集，归一化均方误差0.41%，系统活动运行时间和生成样本数节省93%

Conclusion: 该概率传感范式突破了亚采样率响应时间限制，实现了实时智能自主数据采样激活，在保持数据质量的同时大幅提升能效
...
TL;DR: MeanCache是一个无需训练的高效Flow Matching推理缓存框架，通过平均速度视角和轨迹稳定性调度策略，在保持生成质量的同时实现显著加速。


<details>
  <summary>Details</summary>
Motivation: 现有缓存方法主要依赖瞬时速度信息（如特征缓存），在高加速比下容易导致严重的轨迹偏差和误差累积，需要更稳定的缓存策略来解决这些问题。

Method: MeanCache引入平均速度视角，利用缓存的Jacobian-向量乘积构建区间平均速度来缓解局部误差累积；开发轨迹稳定性调度策略，在预算约束下使用峰值抑制最短路径来确定调度方案。

Result: 在FLUX.1、Qwen-Image和HunyuanVideo上的实验表明，MeanCache分别实现了4.12倍、4.56倍和3.59倍的加速，同时在生成质量上始终优于最先进的缓存基线方法。
...
TL;DR: TCLA框架通过任务条件潜在对齐解决脑机接口中的跨会话非平稳性问题，在有限数据条件下显著提升神经解码性能。


<details>
  <summary>Details</summary>
Motivation: 植入电极记录的神经活动存在跨会话非平稳性，导致在一个会话上训练的脑机接口解码器难以泛化到后续会话，特别是在新会话数据有限时，重新训练或适应解码器变得尤为困难。

Method: 提出任务条件潜在对齐框架(TCLA)，基于自编码器架构：首先从有足够数据的源会话学习神经动力学的低维表示；对于数据有限的目标会话，以任务条件方式将目标潜在表示与源会话对齐，实现学习到的神经动力学的有效迁移。

Result: 在猕猴运动和眼动中心-外数据集上评估，与仅使用目标会话数据训练的基线方法相比，TCLA在数据集和解码设置中一致提升解码性能，在运动数据集的y坐标速度解码中决定系数增益高达0.386。
...
TL;DR: BayPrAnoMeta：基于贝叶斯原型MAML的少样本工业图像异常检测方法，使用概率正态模型替代确定性原型，通过贝叶斯后验预测似然进行内循环适应，在MVTec AD基准上显著优于现有方法。
...
Method: 提出BayPrAnoMeta，将原型替换为任务特定的概率正态模型，使用Normal-Inverse-Wishart先验对正常支持嵌入建模，产生Student-t预测分布。内循环适应通过贝叶斯后验预测似然进行，支持不确定性感知和重尾异常评分。进一步扩展到联邦元学习框架，包含监督对比正则化以处理异构工业客户端。

Result: 在MVTec AD基准测试中，相比MAML、Proto-MAML和基于PatchCore的方法，在少样本异常检测设置中取得了持续且显著的AUROC改进。

Conclusion: BayPrAnoMeta通过概率建模和贝叶斯适应机制，有效解决了极端少样本工业异常检测问题，提供了不确定性感知的鲁棒解决方案，并可扩展到联邦学习环境。
...
<details>
  <summary>Details</summary>
Motivation: 边缘设备上基于LLM的个性化虚拟助手面临效率瓶颈，特别是当用户档案数据快速增长时。虽然存内计算架构通过原位操作减少数据移动，但易受环境噪声影响，在动态多领域边缘场景中检索精度下降成为关键问题

Method: 提出任务导向的噪声弹性嵌入学习框架，采用噪声感知投影模型学习与存内计算硬件约束兼容的任务特定嵌入，在噪声条件下实现准确检索

Result: 在个性化基准测试上的广泛实验表明，该方法相对于强基线在任务特定噪声场景中表现出有效性和实用性

Conclusion: TONEL框架成功解决了边缘设备上RAG系统在噪声环境中的鲁棒性和领域适应性问题，为动态多领域边缘场景提供了可行的解决方案
...
TL;DR: RAMBO：一种基于狄利克雷过程混合高斯过程的贝叶斯优化方法，用于处理多区域优化问题，通过自动发现潜在区域并分别建模来提升性能。
...
Method: 提出RAMBO方法，使用狄利克雷过程混合高斯过程自动发现优化过程中的潜在区域，每个区域由具有局部优化超参数的独立高斯过程建模。推导了折叠吉布斯采样来解析边缘化潜在函数以实现高效推理，并引入自适应浓度参数调度进行从粗到细的区域发现。采集函数将不确定性分解为区域内和区域间两个部分。

Result: 在合成基准测试和实际应用（包括分子构象优化、药物发现的虚拟筛选、聚变反应堆设计）上的实验表明，RAMBO在多区域目标上相比最先进的基线方法取得了持续改进。

Conclusion: RAMBO通过自动发现和建模多区域问题中的不同区域，有效解决了标准贝叶斯优化在非均匀平滑搜索空间中的局限性，在多个实际应用中表现出优越性能。
...
TL;DR: 开发并验证了用于转移性去势抵抗性前列腺癌患者的180天死亡风险预测模型，使用纵向临床数据，GRU模型在外部验证中表现最佳


<details>
  <summary>Details</summary>
Motivation: 转移性去势抵抗性前列腺癌预后差且治疗反应异质性高，需要开发能够预测短期死亡风险的模型，以支持临床决策和主动护理规划

Method: 使用两个III期临床试验队列的纵向数据，比较了LSTM、GRU、Cox比例风险、随机生存森林和逻辑回归五种模型架构，仅使用可观察180天结局的访视数据

Result: GRU和RSF模型初始区分度最高（C指数均为87%），外部验证中GRU校准度最佳（斜率0.93，截距0.07），PR-AUC达0.87，BMI和收缩压是最重要的预测因子
...
TL;DR: 提出Domain Expansion框架解决多目标训练中的潜在表示崩溃问题，通过正交池化机制为每个目标分配相互正交的子空间，在ShapeNet、MPIIGaze和Rotated MNIST等基准上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 多目标训练中，不同目标的梯度冲突会损害共享表示，使其陷入对所有任务都次优的妥协状态，即潜在表示崩溃问题。

Method: 提出Domain Expansion框架，使用新颖的正交池化机制重构潜在空间，为每个目标分配相互正交的子空间，防止梯度冲突。

Result: 在ShapeNet、MPIIGaze和Rotated MNIST等多个基准上验证了方法的有效性，不仅防止了表示崩溃，还产生了可解释、可组合的潜在空间，概念可以直接操作。

Conclusion: Domain Expansion框架通过正交子空间结构有效解决了多目标训练中的潜在表示崩溃问题，创造了可解释、可操作的潜在表示。
...
TL;DR: 该技术报告介绍了量化感知蒸馏(QAD)方法及其最佳实践，用于恢复NVFP4量化大型语言模型和视觉语言模型的精度。QAD通过KL散度损失将全精度教师模型蒸馏到量化学生模型中，在多阶段后训练流程中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统量化感知训练(QAT)在多阶段后训练流程中面临工程复杂性和训练不稳定问题，而QAD方法能够有效解决这些挑战，在数据质量和覆盖范围有限的情况下也能恢复模型精度。

Method: 提出量化感知蒸馏(QAD)方法，使用KL散度损失将全精度教师模型的知识蒸馏到量化学生模型中。该方法特别适用于经过监督微调、强化学习和模型合并等多阶段后训练的模型。

Result: 在多个后训练模型上评估QAD，包括AceReason Nemotron、Nemotron 3 Nano、Nemotron Nano V2、Nemotron Nano V2 VL和Llama Nemotron Super v1，结果显示QAD能够一致地将模型精度恢复到接近BF16精度的水平。

Conclusion: QAD是一种有效且稳定的方法，能够恢复量化大型语言模型和视觉语言模型的精度，特别适用于复杂的多阶段后训练流程，且对数据质量和覆盖范围具有鲁棒性。
...
TL;DR: 提出Decision Importance Transformer(DIT)框架，用于上下文强化学习，通过基于Transformer的价值函数评估行为策略的优势函数，然后使用加权最大似然估计训练策略，有效处理次优离线数据。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在上下文学习方面表现出色，但将其应用于上下文强化学习(ICRL)时面临挑战。特别是当离线数据集包含从次优行为策略采样的轨迹时，标准的自回归训练（模仿学习）会导致次优性能。需要开发能够从次优历史数据中学习并提升性能的方法。

Method: 提出Decision Importance Transformer(DIT)框架：1) 首先训练基于Transformer的价值函数，用于估计收集次优轨迹的行为策略的优势函数；2) 然后训练基于Transformer的策略，使用加权最大似然估计损失，权重基于训练好的价值函数构建，以引导次优策略向最优策略转变。

Result: 在bandit和马尔可夫决策过程问题上进行了广泛实验，结果表明DIT实现了优越的性能，特别是在离线数据集包含次优历史数据的情况下表现突出。

Conclusion: DIT框架成功地将actor-critic算法以上下文方式实现，能够有效处理次优离线数据，在上下文强化学习中取得了显著性能提升，为解决从次优历史数据中学习的问题提供了有效方案。
...
TL;DR: 提出基于强化学习的通用Polar码序列设计框架，可扩展到2048码长，在5G支持的所有(N,K)配置中性能优于5G NR序列，在N=2048时比beta-expansion基线提升0.2dB


<details>
  <summary>Details</summary>
Motivation: 为6G应用推进Polar码设计，需要开发可扩展且适应不同信道条件和解码策略的通用序列设计框架

Method: 基于强化学习的通用序列设计框架，采用物理定律约束学习（基于Polar码的通用偏序特性）、利用决策的弱长期影响限制前瞻评估、联合多配置优化提高学习效率

Result: 框架可扩展到2048码长，在5G支持的所有(N,K)配置中性能优于5G NR序列，在N=2048时比beta-expansion基线提升0.2dB增益

Conclusion: 提出的强化学习框架为6G Polar码设计提供了可扩展、适应性强的解决方案，通过关键技术创新实现了大规模学习，适合标准化应用
...
TL;DR: 该论文将Facebook Prophet模型完全用PyMC重新实现，以支持更多贝叶斯推断方法，包括MCMC、MAP和变分推断，并详细分析了不同推断技术的实现和性能。


<details>
  <summary>Details</summary>
Motivation: 在使用Prophet进行贝叶斯推断时，发现其内置的推断方法有限（仅支持MAP和MCMC），且API设计不够灵活，无法实现自定义建模想法，因此需要更灵活的框架来扩展模型和评估不同推断方法。

Method: 使用PyMC框架完全重新实现Prophet模型，支持多种贝叶斯推断技术：完整的MCMC方法、MAP估计和变分推断技术，并在时间序列预测问题上进行详细分析和比较。

Result: 实现了基于PyMC的Prophet模型，能够扩展基础模型并评估比较多种贝叶斯推断方法，详细分析了采样方法、收敛诊断、预测指标以及计算效率，并识别了未来需要解决的问题。

Conclusion: PyMC重新实现的Prophet模型提供了更大的灵活性，支持更多贝叶斯推断方法，为时间序列预测提供了更强大的建模和分析能力，未来工作将解决已识别的问题。
...
TL;DR: 本文首次系统研究了扩散语言模型（DLMs）在成员推理攻击（MIA）下的隐私泄露漏洞，提出了SAMA攻击方法，相比自回归模型，DLMs因多掩码配置而面临更大风险。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（DLMs）作为自回归语言模型的有前景替代方案，使用双向掩码标记预测，但其在成员推理攻击下的隐私泄露脆弱性尚未得到充分探索。与自回归模型的单一固定预测模式不同，DLMs的多掩码配置指数级增加了攻击机会。

Method: 提出了SAMA（子集聚合成员攻击）方法，通过采样渐进密度的掩码子集，应用基于符号的统计量来处理稀疏信号挑战。采用逆加权聚合优先处理稀疏掩码的清洁信号，将稀疏记忆检测转化为鲁棒的投票机制。

Result: 在九个数据集上的实验表明，SAMA相比最佳基线实现了30%的相对AUC改进，在低误报率下可达8倍改进。这些发现揭示了DLMs中先前未知的显著漏洞。

Conclusion: 扩散语言模型存在显著的成员推理攻击脆弱性，需要开发针对性的隐私防御措施。SAMA攻击方法有效利用了DLMs的多掩码配置特性，显著提高了攻击成功率。
...
TL;DR: 该论文提出从谱表示视角建立表示学习的统一理论框架，揭示现有成功自监督学习算法的谱本质，为理解、分析和设计更高效的表示学习算法提供理论基础。


<details>
  <summary>Details</summary>
Motivation: 自监督学习在表示学习方面取得了显著进展，但缺乏清晰统一的理论理解。现有方法差异巨大，缺乏统一框架阻碍了表示学习的进一步发展，导致理论基础缺失、算法设计原则不明确，实践应用缺乏理论依据。迫切需要建立统一的理论框架。

Method: 从谱表示视角理论分析表示的充分性，揭示现有成功自监督学习算法的谱本质，建立表示学习的统一理论框架和分析方法。

Result: 建立了表示学习的统一理论框架，揭示了自监督学习算法的谱本质，为理解和分析现有算法提供了理论基础，并为开发更高效、易用的表示学习算法指明了方向。

Conclusion: 通过谱表示视角建立的统一理论框架为表示学习提供了理论基础，不仅有助于理解现有算法，还能指导开发更高效、易用的表示学习算法，推动表示学习在实际应用中的发展。
...
TL;DR: PASS框架通过将ML约束压缩为伪点并提供两种选择器（边界规则和信息几何规则），在保持约束满足的同时实现可扩展的高质量聚类


<details>
  <summary>Details</summary>
Motivation: 现有的成对约束聚类方法在处理数据可扩展性方面存在困难，特别是在量子或量子混合聚类等专业应用中，ML和CL约束增加了聚类问题的复杂性

Method: PASS框架将ML约束压缩为伪点，提供两种选择器：1)约束感知边界规则收集近边界点和所有检测到的CL违规；2)信息几何规则通过软分配后验的Fisher-Rao距离对点进行评分，然后在简单预算下选择最高信息子集

Result: 在多样化基准测试中，PASS以显著低于精确或基于惩罚方法的成本获得竞争性的SSE（平方误差和），并在先前方法失败的场景中保持有效性

Conclusion: PASS框架能够在保持成对约束满足的同时，实现可扩展的高质量聚类解决方案，特别适用于需要处理复杂约束的大规模数据场景
...
TL;DR: 该研究提出了一种更简单的方法来评估语言模型中的隐式规划能力，通过押韵诗歌生成和问答案例研究，发现即使小至10亿参数的模型也存在隐式规划机制。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明语言模型在训练过程中可能表现出隐式规划行为，但现有评估方法复杂。本研究旨在开发更简单、可扩展的技术来系统评估语言模型的隐式规划能力。

Method: 提出简单可扩展的评估方法，通过押韵诗歌生成和问答两个案例研究，使用向量引导技术操纵模型在生成中间标记时的行为，从而观察隐式规划现象。

Result: 研究发现隐式规划是普遍存在的机制，存在于比先前认为更小的模型中（从10亿参数开始）。通过向量引导可以操纵生成的押韵词或答案，影响中间标记的生成过程。
...
TL;DR: 该论文填补了稀疏支持向量机（SSVM）理论基础的空白，通过建立局部对偶理论，证明了SSVM是0/1损失SVM的对偶问题，并解释了为什么SSVM在实证研究中优于铰链损失SVM和斜坡损失SVM。


<details>
  <summary>Details</summary>
Motivation: 由于基数最小化在优化中的兴起，稀疏支持向量机（SSVM）最近受到广泛关注，并在实证中显示出优于凸SVM的优势。然而，通过向凸SVM的对偶问题添加基数函数（如ℓ0-范数）来推导SSVM的过程缺乏理论依据，本文旨在填补这一理论空白。

Method: 开发了SSVM的局部对偶理论，探索了SSVM与铰链损失SVM（hSVM）和斜坡损失SVM（rSVM）的关系。证明了SSVM是0/1损失SVM的确切对偶问题，并建立了线性表示定理。在特定条件下，证明了hSVM全局解序列收敛到0/1损失SVM的局部解，且0/1损失SVM的局部极小值是rSVM的局部极小值。

Result: 理论分析表明：1）SSVM是0/1损失SVM的对偶问题；2）线性表示定理对它们的局部解成立；3）SSVM的局部解为hSVM和rSVM的超参数选择提供了指导；4）解释了为什么SSVM在先前实证研究中优于hSVM和rSVM；5）在真实数据集上的数值测试验证了SSVM的潜在优势。

Conclusion: 该研究为SSVM提供了坚实的理论基础，填补了现有理论空白。通过局部对偶理论，不仅解释了SSVM相对于其他SVM变体的优越性，还为超参数选择提供了理论指导。数值实验进一步证实了基于本文提出的局部良好解的SSVM在实际应用中的优势。
...
### [88] [Loss Landscape Geometry and the Learning of Symmetries: Or, What Influence Functions Reveal About Robust Generalization](https://arxiv.org/abs/2601.20172)
...
TL;DR: 提出一种基于梯度影响的诊断方法，用于评估神经偏微分方程仿真器是否内化了物理对称性，通过测量对称轨道上参数更新的传播来检测学习动态是否耦合物理等价配置。


<details>
  <summary>Details</summary>
Motivation: 研究神经偏微分方程仿真器如何内化物理对称性，超越传统的前向传递等变性测试，直接评估学习动态是否将物理等价配置耦合起来。

Method: 引入基于影响的诊断方法，测量对称轨道上参数更新之间的传播，定义为沿群轨道评估的损失梯度度量加权重叠。该方法探测学习损失景观的局部几何结构。

Result: 应用于自回归流体流动仿真器，显示轨道梯度相干性提供了学习在对称变换上泛化的机制，并指示训练何时选择了对称兼容的盆地。

Conclusion: 开发了一种新技术，用于评估替代模型是否内化了已知解算子的对称性属性，为分析神经仿真器的对称性学习提供了新工具。
...
TL;DR: MAPLE是一种新的非线性降维方法，通过自监督学习和最大流形容量表示来增强UMAP的流形建模能力，特别适用于具有高簇内方差和弯曲流形结构的数据。


<details>
  <summary>Details</summary>
Motivation: UMAP在流形建模方面存在局限性，特别是对于具有高簇内方差和复杂弯曲流形结构的高维数据（如生物数据或图像数据），需要更有效的降维方法来改善聚类分离和子聚类分辨率。
...
Conclusion: MAPLE通过改进流形建模，为高维数据的降维提供了更有效的解决方案，特别适用于具有复杂流形结构的数据分析任务。
...
### [90] [Causal-Driven Feature Evaluation for Cross-Domain Image Classification](https://arxiv.org/abs/2601.20176)
*Chen Cheng,Ang Li*
...
<details>
  <summary>Details</summary>
Motivation: 现有OOD泛化方法大多追求域不变表征，隐含假设不变性意味着可靠性。然而，跨域不变的特征并不一定对预测具有因果有效性，这限制了真实场景下的分类性能。
...
### [91] [On the Computational Complexity of Performative Prediction](https://arxiv.org/abs/2601.20180)
*Ioannis Anagnostides,Rohan Chauhan,Ioannis Panageas,Tuomas Sandholm,Jingming Yan*

Main category: cs.LG

TL;DR: 本文揭示了在预测模型部署会改变数据分布的情况下，当影响强度ρ>1时，计算ε-执行稳定点是PPAD完全的，即使ρ=1+O(ε)也如此，这相当于一般和博弈中纳什均衡的计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 研究执行预测中，当模型部署对数据分布影响较强时（ρ>1），计算稳定点的计算复杂度问题。之前已知在弱影响下（ρ<1）简单重训练能线性收敛，但强影响下的复杂度一直未解决。

Method: 通过建立尖锐的相变理论，证明计算ε-执行稳定点是PPAD完全的，即使在二次损失函数和线性分布偏移的简单设置下也成立。技术贡献包括将PPAD-硬度结果扩展到一般凸域，并在战略分类的特殊情况下证明计算战略局部最优是PLS-硬的。

Result: 发现当ρ>1时，计算ε-执行稳定点是PPAD完全的，与一般和博弈中纳什均衡计算复杂度相同。即使在ρ=1+O(ε)的边界情况下，这种难解性仍然存在。在战略分类的特殊情况下，计算战略局部最优是PLS-硬的。

Conclusion: 本文揭示了执行预测中强影响机制下的计算复杂性壁垒，建立了与博弈论中纳什均衡计算难度的等价关系，为理解执行预测的计算复杂度提供了重要理论框架。
...
### [92] [Meta-Cognitive Reinforcement Learning with Self-Doubt and Recovery](https://arxiv.org/abs/2601.20193)
...
Method: 提出元认知强化学习框架，引入基于价值预测误差稳定性驱动的元信任变量，通过故障安全调节和渐进信任恢复来调节学习动态。

Result: 在具有奖励损坏的连续控制基准测试中，启用恢复的元认知控制相比强鲁棒基线实现了更高的平均回报，并显著减少了后期训练失败。

Conclusion: 元认知强化学习框架通过内部可靠性评估和调节机制，有效解决了传统鲁棒强化学习的局限性，提高了学习过程的稳定性和性能。
...
### [93] [DeRaDiff: Denoising Time Realignment of Diffusion Models](https://arxiv.org/abs/2601.20198)
*Ratnavibusena Don Shahain Manujith,Yang Zhang,Teoh Tze Tzun,Kenji Kawaguchi*

Main category: cs.LG

TL;DR: DeRaDiff提出一种无需重新训练即可在采样时调节正则化强度的扩散模型对齐方法，避免了传统方法需要多次训练调参的高计算成本。


<details>
  <summary>Details</summary>
Motivation: 扩散模型对齐人类偏好时，KL正则化强度的选择非常困难：强度过高导致对齐效果有限，强度过低导致"奖励黑客"问题。现有方法需要训练多个不同正则化强度的模型进行网格搜索，计算成本极高。

Method: DeRaDiff是一种去噪时间重对齐方法，在采样时通过几何混合对齐后验和参考后验来调节正则化强度。该方法基于单一可调参数λ，在常见调度器下具有闭式更新，无需额外训练或微调。

Result: 实验表明，DeRaDiff在不同文本图像对齐和图像质量指标上，能够有效近似不同正则化强度下从头训练的模型，为寻找最优正则化强度提供了高效方法。

Conclusion: DeRaDiff通过采样时调节正则化强度，显著降低了扩散模型对齐的计算成本，消除了昂贵的对齐网格搜索需求，实现了高效的正则化强度优化。
...
### [94] [Hyperparameter Transfer with Mixture-of-Expert Layers](https://arxiv.org/abs/2601.20205)
*Tianze Jiang,Blake Bordelon,Cengiz Pehlevan,Boris Hanin*

Main category: cs.LG

TL;DR: 提出了一种新的MoE层参数化方法，通过动态平均场理论分析，实现了从5100万到20亿参数模型间的超参数可靠迁移，降低了稀疏MoE模型的训练复杂度。


<details>
  <summary>Details</summary>
Motivation: 稀疏MoE层虽然能解耦总参数量和激活参数量，但增加了训练复杂度：需要调节路由器权重的新超参数，以及专家数量和大小等架构维度选择。为了降低超参数选择的成本和提高可靠性，需要一种有效的参数化方法。

Method: 提出了一种新的参数化方法，用于在缩放模型宽度、深度、专家数量和专家隐藏大小时，对带有MoE层的Transformer模型进行参数化。该方法基于新颖的动态平均场理论分析，能够在固定token预算下训练不同维度的模型时实现可靠的超参数迁移。

Result: 经验证明，该参数化方法能够在从5100万到超过20亿总参数的模型中实现可靠的超参数迁移。通过在小模型上短token范围内识别出的超参数，能够成功训练更大模型在更长token范围上，并获得良好的模型性能表现。

Conclusion: 提出的参数化方法显著降低了MoE模型超参数选择的成本和复杂性，实现了跨模型规模的可靠超参数迁移，为大规模MoE模型的训练提供了实用的解决方案。
...
### [95] [Spark: Strategic Policy-Aware Exploration via Dynamic Branching for Long-Horizon Agentic Learning](https://arxiv.org/abs/2601.20209)
*Jinyang Wu,Shuo Yang,Changpeng Yang,Yuhao Shen,Shuai Zhang,Zhengqi Wen,Jianhua Tao*

Main category: cs.LG

TL;DR: Spark框架通过关键状态动态分支实现资源高效探索，在关键决策点选择性分支以探索有前景的轨迹，显著减少训练样本需求并提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在训练大语言模型执行长时程任务时面临高质量轨迹稀缺的挑战，特别是在有限资源下。现有方法通常盲目扩大rollout规模并在中间步骤中不加区分地分配计算资源，导致大量计算预算浪费在平凡步骤上，且无法保证样本质量。

Method: 提出Spark（Strategic Policy-Aware exploration via Key-state dynamic branching）框架，在关键决策状态选择性分支进行资源高效探索。核心思想是在关键决策点激活自适应分支探索来探测有前景的轨迹，实现精确的资源分配，优先考虑采样质量而非盲目覆盖。该设计利用智能体内在的决策信号减少对人类先验的依赖，使智能体能够自主扩展探索。

Result: 在多样化任务（如具身规划）上的实验表明，Spark以显著更少的训练样本实现了更高的成功率，在未见场景中展现出强大的泛化能力。

Conclusion: Spark框架通过关键状态动态分支策略，实现了资源高效的强化学习探索，解决了长时程任务中高质量轨迹稀缺的问题，为智能体训练提供了更有效的探索机制。
...
### [96] [An Accounting Identity for Algorithmic Fairness](https://arxiv.org/abs/2601.20217)
*Hadi Elzayn,Jacob Goldin*

Main category: cs.LG

TL;DR: 本文推导了一个预测模型的会计恒等式，将准确性与常见公平性标准联系起来，表明准确性与公平性是互补关系而非权衡关系。


<details>
  <summary>Details</summary>
Motivation: 当前公平机器学习领域存在准确性与公平性之间的权衡讨论，作者旨在通过数学恒等式揭示两者之间的本质关系，澄清误解并为公平性干预提供理论指导。

Method: 推导了一个预测模型的会计恒等式，将模型的均方误差与群体间结果类别的患病率差异联系起来，定义了"总不公平预算"概念，并通过基准数据集实验验证理论。

Result: 恒等式表明：对于全局校准模型，组内校准误差和组间误差不平衡的加权和等于总不公平预算；在二元结果中，该预算等于模型的均方误差乘以群体间结果类别的患病率差异；准确性与公平性是互补关系，提高准确性会减少总不公平预算。

Conclusion: 准确性与公平性在二元预测任务中是互补而非权衡关系，提高准确性会减少总不公平预算；许多公平性干预主要是在不同公平性违规之间进行替代，当它们降低准确性时往往会扩大总不公平预算；结果可自然扩展到非二元结果预测任务。
...
TL;DR: 本文提出两个机器学习框架：快速参数化模型用于预测聚合曲线并优化EPEX SPOT日前市场中的储能策略，以及生成模型用于综合分析市场情景。


<details>
  <summary>Details</summary>
Motivation: 为了在EPEX SPOT日前市场中有效预测电力供需曲线并优化储能策略，需要开发既能快速运行又具有良好解释性的预测模型，同时还需要能够生成全面市场情景的分析工具。

Method: 1. 快速参数化模型：使用低维网格稳健表示预测小时级供需曲线，结合最小/最大交易量和切比雪夫多项式表示弹性段。2. 生成模型：学习给定天气和燃料变量下24小时订单级提交的联合分布，生成合成日情景的个体买卖订单，再聚合得到小时级供需曲线。

Result: 参数化模型实现了低误差和良好解释性，适合日常使用；生成模型能生成全面的市场情景。基于这些预测，优化了价格制定储能策略，量化了收入分布，并揭示了容量扩张时的价格压缩效应（峰值降低、非峰值水平升高、收益递减）。

Conclusion: 本文提出的两个框架为EPEX SPOT日前市场提供了实用的预测和优化工具：参数化模型适合日常运营，生成模型适合全面分析。储能策略优化显示了容量扩张时的收益递减规律，为市场参与者提供了重要决策支持。
...
TL;DR: ProFlow：一种零样本物理一致性采样框架，使用预训练生成先验从稀疏观测中推断物理场，严格满足PDE约束且无需重新训练


<details>
  <summary>Details</summary>
Motivation: 现有深度生成模型在处理物理反问题时难以强制执行硬物理约束，要么需要昂贵的重新训练，要么会破坏已学习的生成先验。需要一种采样机制能够协调严格的物理一致性、观测保真度和预训练先验的统计结构

Method: 提出ProFlow框架，采用严格的两步交替方案：1）终端优化步骤，通过近端最小化将流预测投影到物理一致集和观测一致集的交集；2）插值步骤，将精炼状态映射回生成轨迹以保持与学习流概率路径的一致性

Result: 在泊松方程、亥姆霍兹方程、达西方程和粘性伯格斯方程上的综合基准测试表明，ProFlow相比最先进的扩散和流基线方法，实现了更好的物理和观测一致性，以及更准确的分布统计

Conclusion: ProFlow为零样本物理一致性采样提供了一个有效的框架，能够在不重新训练的情况下，从稀疏观测中推断出严格满足PDE约束的物理场解
...
TL;DR: 提出CGP方法用于噪声Lipschitz函数黑箱优化，通过显式证书识别次优点，在边际条件下获得近最优样本复杂度，并开发了自适应、信任区域和混合变体。


<details>
  <summary>Details</summary>
Motivation: 现有自适应离散化方法隐含地避免次优区域，但缺乏显式的最优性证书或可测量的进展保证。需要一种能提供明确次优性证明的方法。

Method: 提出证书引导剪枝(CGP)方法，通过置信调整的Lipschitz包络维护潜在最优点的显式活动集A_t。开发三个扩展：CGP-Adaptive在线学习Lipschitz常数；CGP-TR通过信任区域和局部证书扩展到高维；CGP-Hybrid在检测到局部平滑时切换到高斯过程细化。

Result: 在边际条件下，证明A_t的体积以受控速率收缩，获得样本复杂度$\tildeO(\varepsilon^{-(2+α)})$。在12个基准测试(d∈[2,100])中，CGP变体匹配或超过强基线，同时通过证书体积提供原则性停止准则。

Conclusion: CGP为噪声Lipschitz函数优化提供了具有显式证书和原则性停止准则的有效方法，通过自适应、信任区域和混合策略实现了实际可扩展性。
...
### [100] [HE-SNR: Uncovering Latent Logic via Entropy for Guiding Mid-Training on SWE-BENCH](https://arxiv.org/abs/2601.20255)
*Yueyang Wang,Jiawei Fu,Baolong Bi,Xili Wang,Xiaoqing Liu*

Main category: cs.LG

TL;DR: 该论文针对大语言模型在软件工程任务中的性能评估问题，提出了一种基于熵压缩假设的新度量方法HE-SNR，以解决传统困惑度指标在长上下文场景下的局限性。


<details>
  <summary>Details</summary>
Motivation: SWE-bench已成为评估大语言模型在复杂软件工程任务上的主要基准，但模型能力的获取主要在中训练阶段，而当前缺乏有效指导中训练的度量指标。传统指标如困惑度受"长上下文税"影响，与下游软件工程性能相关性弱。

Method: 首先引入严格的数据过滤策略，然后提出熵压缩假设，将智能重新定义为将不确定性结构化为低阶熵压缩状态的能力。基于细粒度熵分析，提出了新度量指标HE-SNR（高熵信噪比）。

Result: 在工业级混合专家模型上验证了该方法，涵盖不同上下文窗口（32K/128K），结果显示该方法具有优越的鲁棒性和预测能力。

Conclusion: 该研究为优化大语言模型在复杂工程领域的潜在能力提供了理论基础和实用工具，解决了中训练阶段缺乏有效指导指标的关键问题。
...
### [101] [C2:Cross learning module enhanced decision transformer with Constraint-aware loss for auto-bidding](https://arxiv.org/abs/2601.20257)
...
<details>
  <summary>Details</summary>
Motivation: 决策变换器在生成式自动出价中表现出潜力，但存在两个关键限制：1）状态、动作和回报序列之间的跨相关性建模不足；2）对最优/次优行为不加区分的学习。

Method: 提出C2框架，包含两个核心创新：1）通过交叉注意力机制的交叉学习块来增强序列间相关性建模；2）包含预算和CPA约束的约束感知损失，用于选择性学习最优轨迹。

Result: 在AuctionNet数据集上的离线评估显示，C2在不同预算设置下均取得一致性能提升（比最先进的GAVE方法最高提升3.23%），消融研究验证了CLB和CL的互补协同作用。
...
<details>
  <summary>Details</summary>
Motivation: 随机微分方程参数估计通常依赖准确的时间戳观测序列，但当时间顺序信息被破坏、缺失或故意隐藏时，现有方法往往失效

Method: 利用前向和后向过程的不对称性，推导得分匹配准则推断观测对之间的正确时间顺序，通过排序过程恢复总顺序，然后使用最大似然估计从重建序列中估计SDE参数

Result: 在合成和真实数据集上进行广泛实验，证明该方法在时间顺序缺失情况下的有效性

Conclusion: 该方法扩展了参数估计到时间顺序缺失的场景，拓宽了在敏感领域的适用性
...
TL;DR: δ-Adapter是一种轻量级、架构无关的后处理方法，可在不重新训练或修改已部署主干模型的情况下提升时间序列预测的准确性和不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 随着预测模型在准确性方面接近收益递减，一个关键但未被充分探索的机会出现了：后处理的战略使用。本文旨在解决时间序列预测中的"最后一英里"问题，即在不重新训练或修改已部署主干模型的情况下提高准确性和不确定性估计。

Method: 提出δ-Adapter，一种轻量级、架构无关的方法。它在两个接口处学习微小的有界模块：输入微调（对协变量进行软编辑）和输出残差校正。该方法提供局部下降保证、O(δ)漂移界限和组合稳定性。同时，它可以通过学习输入上的稀疏、时间感知掩码来选择重要特征，从而作为特征选择器。此外，它还可以作为分布校准器来测量不确定性，引入了分位数校准器和符合性校正器来提供校准的个性化区间。

Result: 在多种主干模型和数据集上的实验表明，δ-Adapter以可忽略的计算成本和无需接口更改的方式提高了准确性和校准效果。

Conclusion: δ-Adapter为提升已部署时间序列预测模型的性能提供了一种有效且实用的后处理方法，填补了模型部署后的"最后一英里"改进空白。
...
<details>
  <summary>Details</summary>
Motivation: 解决计算廉价模型与复杂物理系统之间的模拟-现实差距问题，特别是在多尺度场景中，降阶模型通常只能捕捉主导动力学，需要重建高保真状态空间。

Method: 提出Cheap2Rich多尺度数据同化框架，结合快速低精度先验模型和可学习的可解释差异修正，从稀疏传感器历史数据重建高保真状态空间，特别应用于旋转爆震发动机系统。

Result: 成功从稀疏测量重建旋转爆震发动机的高保真状态，并分离出与喷射器驱动效应相关的物理意义明确的差异动力学，为复杂多尺度系统提供通用多保真度框架。

Conclusion: Cheap2Rich为复杂多尺度系统的数据同化和系统识别提供了通用框架，支持快速设计探索、实时监测控制，并提供可解释的差异动力学分析。
...
TL;DR: 该研究探讨了Transformer注意力层在LLMs中的记忆机制，提出关键词作为检索线索的假设，并识别了编码关键词的特定神经元，为可解释AI和机器学习遗忘提供新视角。


<details>
  <summary>Details</summary>
Motivation: 尽管XAI领域在LLMs中有所进展，但注意力层在Transformer模型中的具体作用仍未被充分探索。监管压力增加促使需要理解LLMs的透明度、问责制和隐私保护机制，特别是机器学习遗忘方面。

Method: 研究基于心理学和计算心理语言学的先前研究，将Transformer注意力与人类记忆中的线索检索联系起来。通过编码特异性原则指导，提出关键词作为检索线索的假设，并识别注意力层中编码关键词的特定神经元。

Result: 研究提供了关键词作为线索假设的收敛证据，成功分离出注意力层中编码上下文定义关键词的神经元，这些关键词可以从识别的神经元中提取，并应用于下游任务如机器学习遗忘。

Conclusion: 注意力层在LLMs中实现了类似人类记忆的检索机制，关键词作为检索线索，特定神经元编码这些关键词。这一发现为XAI提供了新见解，并有助于机器学习遗忘等应用。
...
TL;DR: 本文提出了一种基于同伴预测（peer prediction）的大语言模型评估和训练方法，该方法利用博弈论激励兼容性，无需真实标签即可奖励诚实和信息的回答，有效抵抗欺骗行为。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的评估和后训练依赖于监督信号，但对于困难任务往往缺乏强监督，特别是在评估前沿模型时。基于不完美监督的评估容易被模型利用，导致欺骗性结果。博弈论中的机制设计研究提供了在弱监督下激励诚实和信息回答的方法，但在LLM研究中尚未充分利用。

Method: 引入同伴预测方法用于模型评估和后训练。该方法基于相互可预测性构建度量标准，无需真实标签即可奖励诚实和信息的回答，惩罚欺骗性和无信息的回答。通过理论保证和实证验证（模型参数高达405B）证明方法的有效性。

Result: 使用同伴预测奖励训练的8B模型能够恢复因恶意微调而下降的大部分真实性，即使奖励由未微调的0.135B语言模型产生。在评估方面，发现同伴预测具有逆缩放特性：专家与参与者能力差距越大，抵抗欺骗的能力越强，能够在弱监督下可靠评估强模型。当面对比裁判大5-20倍的欺骗性模型时，LLM-as-a-Judge表现比随机猜测更差，而同伴预测在大差距情况下（包括超过100倍大小差异）表现良好。

Conclusion: 同伴预测方法为LLM评估和后训练提供了一种有效的弱监督解决方案，能够抵抗欺骗行为，特别是在评估强模型时表现出色。该方法利用博弈论激励兼容性原理，为缺乏强监督的困难任务评估提供了新途径。
...
### [107] [A Learning-based Framework for Spatial Impulse Response Compensation in 3D Photoacoustic Computed Tomography](https://arxiv.org/abs/2601.20291)
*Kaiyi Yang,Seonyeong Park,Gangwon Jeong,Hsuan-Kai Huang,Alexander A. Oraevsky,Umberto Villa,Mark A. Anastasio*

Main category: cs.LG

TL;DR: 提出一种学习型空间脉冲响应补偿框架，用于3D光声计算机断层扫描，通过数据域补偿提升图像分辨率同时保持计算效率


<details>
  <summary>Details</summary>
Motivation: 光声计算机断层扫描中，使用大面积超声换能器可提高检测灵敏度，但忽略换能器空间脉冲响应的解析重建方法会损害空间分辨率，而考虑空间脉冲响应的优化重建方法计算成本过高，特别是在3D应用中

Method: 提出学习型空间脉冲响应补偿框架，在数据域将受空间脉冲响应影响的测量数据映射到理想点状换能器应记录的补偿数据，使用U-Net和专门设计的物理启发模型Deconv-Net两种变体，并包含快速解析训练数据生成过程

Result: 虚拟成像研究中验证了分辨率改善和对噪声变化、物体复杂性及声速异质性的鲁棒性，应用于体内乳腺成像数据时，学习补偿模型揭示了被空间脉冲响应伪影掩盖的精细结构

Conclusion: 该框架首次在3D光声计算机断层扫描中实现了学习型空间脉冲响应补偿，能够在保持计算效率的同时提升图像质量
...
### [108] [Delayed Feedback Modeling for Post-Click Gross Merchandise Volume Prediction: Benchmark, Insights and Approaches](https://arxiv.org/abs/2601.20307)
...
<details>
  <summary>Details</summary>
Motivation: 在线广告排序模型的预测目标正从转化率等概率指标转向GMV等数值业务指标。与CVR预测中已充分研究的延迟反馈问题不同，GMV预测的延迟反馈建模尚未探索且面临更大挑战，因为GMV是连续目标，且单个点击可能产生多次购买累积形成标签。

Method: 研究首先建立了TRACE基准数据集，包含从每个用户点击产生的完整交易序列，支持在线流式延迟反馈建模。基于分析发现，提出了READER模型：这是一个基于复购感知的双分支预测器，通过路由器根据复购预测选择性地激活专家参数，并动态校准回归目标以缓解不完整标签导致的低估问题。

Result: 实验结果显示，READER在TRACE基准上优于基线方法，在准确性方面实现了2.19%的提升。TRACE基准数据集的分析揭示了两个关键发现：(1) GMV标签分布的快速演化需要在在线流式训练下建模延迟反馈；(2) 复购样本的标签分布与单次购买样本存在显著差异，需要分别建模。

Conclusion: 该研究为GMV预测的在线延迟反馈建模开辟了新方向，TRACE基准数据集和READER模型为这一有前景领域的研究和应用提供了基础。代码和数据集已开源，将促进该领域的未来发展。
...
### [109] [Window-Diffusion: Accelerating Diffusion Language Model Inference with Windowed Token Pruning and Caching](https://arxiv.org/abs/2601.20332)
*Fengrui Zuo,Zhiwei Ke,Yiming Liu,Wenqi Lou,Chao Wang,Xvehai Zhou*

Main category: cs.LG

TL;DR: 该论文提出了一种基于窗口的token剪枝和缓存方法，用于加速扩散语言模型的推理过程，通过局部计算窗口和token分类策略，在保持生成性能的同时实现了高达99倍的推理加速。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在推理时需要全序列注意力计算，导致大量冗余计算。现有块级扩散方法通常需要重新训练且更新顺序受限，难以直接应用于预训练模型。研究发现DLM推理中存在显著的结构局部性特征，这为优化提供了机会。

Method: 提出基于窗口的token剪枝和缓存方法：维护一个随去噪过程向右滑动的局部计算窗口，将未解码token分为三类：在线计算的活跃token、KV状态被缓存并定期刷新的缓冲区token、以及被剪枝的远场token。计算仅限于窗口内的活跃和缓冲区token。

Result: 在LLaDA和Dream模型上的实验表明，在匹配的计算预算下，该方法实现了高达99倍的推理加速，同时基本保持了生成性能。

Conclusion: 通过利用扩散语言模型推理中的结构局部性，提出的窗口化方法能够显著加速推理过程，为预训练DLM的高效部署提供了实用解决方案。
...
### [110] [TABED: Test-Time Adaptive Ensemble Drafting for Robust Speculative Decoding in LVLMs](https://arxiv.org/abs/2601.20357)
*Minjae Lee,Wonjun Kang,Byeongkeun Ahn,Christian Classen,Kevin Galim,Seunghyuk Oh,Minghao Yan,Hyung Il Koo,Kangwook Lee*

Main category: cs.LG

TL;DR: TABED：一种针对大型视觉语言模型的测试时自适应批量集成草稿方法，通过动态集成多个草稿来加速推理，相比自回归解码平均获得1.74倍加速


<details>
  <summary>Details</summary>
Motivation: 推测解码在加速LLM推理方面已被证明有效，但在大型视觉语言模型（LVLMs）中仍未被充分探索。现有方法在不同输入场景下表现不稳定，需要一种更稳健的加速方案

Method: 提出测试时自适应批量集成草稿（TABED），通过批量推理获得多个草稿，利用推测解码设置中可用的历史真实值偏差进行动态集成，保持训练免费且通过参数共享最小化集成成本

Result: 在11个数据集上测试，平均获得1.74倍于自回归解码的稳健墙时加速，相比单一草稿方法提升5%，具有即插即用兼容性，可集成高级验证和替代草稿方法

Conclusion: TABED为大型视觉语言模型提供了一种有效且高效的推测解码加速方案，通过动态集成策略解决了场景特定性能波动问题，同时保持低成本和易用性
...
### [111] [Can Continuous-Time Diffusion Models Generate and Solve Globally Constrained Discrete Problems? A Study on Sudoku](https://arxiv.org/abs/2601.20363)
*Mariia Drozdova*

Main category: cs.LG

TL;DR: 连续时间生成模型能否表示支撑集为极度稀疏、全局约束离散集合的分布？研究以数独网格为测试平台，发现随机采样优于确定性流，基于分数的采样器最可靠，DDPM式祖先采样有效性最高，模型还可用于引导生成作为概率数独求解器。


<details>
  <summary>Details</summary>
Motivation: 探究标准连续时间生成模型能否表示支撑集为极度稀疏、全局约束离散集合的分布，以数独网格作为受控测试平台来研究这一基本问题。

Method: 使用数独网格作为连续松弛空间的子集，训练流匹配和基于分数的模型，沿着高斯概率路径比较确定性ODE采样、随机SDE采样以及从相同连续时间训练导出的DDPM式离散化方法。

Result: 无条件生成时，随机采样显著优于确定性流；基于分数的采样器在连续时间方法中最可靠；DDPM式祖先采样总体有效性最高。模型还可通过重复采样固定线索下的完成情况，在满足约束时停止，作为概率数独求解器使用。

Conclusion: 经典扩散/流公式能够为非零概率质量分配到全局约束的组合结构，并可通过随机搜索用于约束满足问题，尽管在样本效率上不如经典求解器和离散几何感知的扩散方法。
...
### [112] [LLM-AutoDP: Automatic Data Processing via LLM Agents for Model Fine-tuning](https://arxiv.org/abs/2601.20375)
...
<details>
  <summary>Details</summary>
Motivation: 领域特定数据通常包含大量低质量样本，需要有效的数据处理。传统方法依赖人工迭代分析和试错调整，成本高且在高隐私领域（如医疗）存在隐私泄露风险。需要实现自动化数据处理而不暴露原始数据

Method: 提出LLM-AutoDP框架，利用LLM作为智能体自动生成和优化数据处理策略。通过生成多个候选策略，利用反馈信号和比较评估进行迭代优化。引入三种加速技术：分布保持采样（减少数据量同时保持分布完整性）、处理目标选择（使用二元分类器识别低质量样本进行重点处理）、缓存重用机制（减少冗余计算）

Result: 使用该框架处理的数据训练的模型，相比未处理数据训练的模型获得超过80%的胜率。与基于LLM智能体的AutoML基线相比，LLM-AutoDP获得约65%的胜率。加速技术将总搜索时间减少高达10倍

Conclusion: LLM-AutoDP框架能够有效自动化数据处理策略的生成和优化，在保护隐私的同时显著提升模型性能，并通过加速技术实现高效搜索
...
### [113] [FedRD: Reducing Divergences for Generalized Federated Learning via Heterogeneity-aware Parameter Guidance](https://arxiv.org/abs/2601.20397)
*Kaile Wang,Jiannong Cao,Yu Yang,Xiaoyin Li,Mingjin Zhang*

Main category: cs.LG

TL;DR: FedRD是一种针对异构联邦学习的新型算法，通过参数引导的全局泛化聚合和局部去偏分类来解决优化分歧和性能分歧问题，提升对未见客户端的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 异构联邦学习中，新加入的客户端需要大量调整和额外训练才能与现有系统对齐，使得联邦学习模型在异构数据下对未见客户端的泛化问题变得日益重要。论文指出了联邦域泛化中两个未解决的挑战性问题：优化分歧和性能分歧。

Method: 提出FedRD算法，采用参数引导的全局泛化聚合和局部去偏分类的协同方法，通过减少分歧来获得适用于参与客户端和未见客户端的最优全局模型。

Result: 在公共多域数据集上的大量实验表明，该方法在解决这一特定问题上相比竞争基线表现出显著的性能优势。

Conclusion: FedRD通过有效解决异构联邦学习中的优化分歧和性能分歧问题，为联邦域泛化提供了有效的解决方案，能够获得对参与客户端和未见客户端都具有良好泛化能力的全局模型。
...
<details>
  <summary>Details</summary>
Motivation: 可解释人工智能(XAI)对于增强机器学习模型透明度至关重要。反事实解释(CFs)能够展示输入特征变化如何改变模型决策，为用户提供可操作的补救措施。确保具有相似属性的个体和不同受保护群体的个体获得相似且可操作的补救选项，对于可信和公平的决策至关重要。

Method: 将公平反事实生成问题表述为优化任务，提出了一种新颖的模型无关、基于强化学习的方法。该方法同时考虑：1)个体公平性：确保相似个体获得相似CFs；2)群体公平性：确保不同受保护群体间获得公平的CFs；3)混合公平性：兼顾个体和群体层面的公平性。扩展了现有用于审计ML模型的公平性指标。

Result: 在三个基准数据集上评估了该方法，结果显示它有效地确保了个体和群体公平性，同时在邻近性和合理性方面保持了生成CFs的质量，并分别量化了不同层面公平性的成本。

Conclusion: 该工作开启了关于混合公平性及其在XAI和超越反事实解释领域的角色和影响的更广泛讨论，为解决公平反事实生成问题提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 时间序列预测面临复杂多时间尺度依赖关系的挑战，现有方法难以有效捕捉局部和全局模式。

Method: 包含四个关键组件：1)分层散射变换模块提取多尺度不变特征；2)尺度自适应特征增强模块动态调整特征重要性；3)多分辨率时间注意力机制学习不同时间跨度的依赖关系；4)趋势-季节-残差分解引导的结构感知损失函数。

Result: 在七个基准数据集上的实验表明，ScatterFusion优于其他常见方法，在各种预测时间跨度上显著降低了误差指标。
...
### [116] [Unsupervised Ensemble Learning Through Deep Energy-based Models](https://arxiv.org/abs/2601.20556)
*Ariel Maymon,Yanir Buznah,Uri Shaham*

Main category: cs.LG

TL;DR: 提出一种基于深度能量的无监督集成学习方法，仅使用个体学习器的预测就能构建准确的元学习器，无需标签数据或学习器特征，在条件独立假设下有理论保证。
...
Method: 提出基于深度能量的方法，仅利用个体学习器的预测来构建元学习器，能够捕捉学习器之间的复杂依赖结构。该方法不需要标签数据、学习器特征或问题特定信息。
...
TL;DR: 论文提出了一种理论驱动的概念提取框架ConCA，通过线性解混过程从LLM表示中恢复概念的后验对数，解决了现有稀疏自编码器缺乏理论基础的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前稀疏自编码器（SAEs）虽然在实际应用中取得进展，但缺乏理论基础，导致概念提取与人类可解释概念之间的对应关系不明确，这给方法设计和评估带来了困难。

Method: 提出概念成分分析（ConCA）框架，将LLM表示建模为概念后验对数的线性混合，通过无监督线性解混过程恢复每个概念的后验对数。特别提出了稀疏ConCA变体，利用稀疏性先验解决解混问题的病态性。

Result: 实现了12个稀疏ConCA变体，在多个LLM上展示了其提取有意义概念的能力，相比SAEs具有理论支持的优势。

Conclusion: ConCA为LLM概念提取提供了理论依据，通过将LLM表示与概念后验对数联系起来，为可解释性研究提供了更严谨的框架。
...
TL;DR: 提出了一种名为AsylADMM的新型gossip算法，用于去中心化的中位数和分位数估计，该算法设计用于异步更新，每个节点仅需两个变量，具有通信高效、内存占用低的特点。


<details>
  <summary>Details</summary>
Motivation: 资源受限的边缘设备需要通信高效、对数据损坏具有鲁棒性且内存占用低的去中心化学习算法。现有的gossip方法虽然通信高效，但实现鲁棒性仍然具有挑战性。现有的异步去中心化ADMM方法需要与节点度成比例的内存，这在内存有限的情况下不实用。

Method: 提出了AsylADMM算法，一种用于去中心化中位数和分位数估计的新型gossip算法。该算法主要设计用于异步更新，每个节点仅需要两个变量。通过分析同步变体来建立理论保证，并通过实验验证异步算法的快速收敛性。

Result: 算法能够实现基于分位数的修剪、几何中位数估计和基于深度的修剪。实验表明，基于分位数的修剪在性能上优于现有的基于秩的方法。此外，还通过马尔可夫链理论提供了基于秩的修剪的新理论分析。

Conclusion: AsylADMM算法为资源受限的边缘设备提供了一种通信高效、内存占用低且具有鲁棒性的去中心化中位数和分位数估计解决方案，在异步更新环境下表现良好，并扩展了多种鲁棒性应用。
...
### [119] [Nonlinear Dimensionality Reduction with Diffusion Maps in Practice](https://arxiv.org/abs/2601.20428)
...
<details>
  <summary>Details</summary>
Motivation: 扩散映射作为一种光谱降维技术，能够揭示高维数据中的非线性子流形，在生物学、工程学和社会科学等领域广泛应用。然而，数据预处理、参数设置和成分选择对最终流形结果有显著影响，但文献中对此缺乏全面讨论。

Method: 提供了扩散映射技术的实践导向综述，阐述了常见陷阱，并展示了一种最近引入的识别最相关成分的技术。

Result: 研究结果表明，第一个成分不一定是最相关的成分，这挑战了传统认知。

Conclusion: 扩散映射的应用需要仔细考虑数据预处理、参数设置和成分选择，新提出的成分识别方法有助于更准确地揭示数据的内在结构。
...
Method: 提出RARL框架，包含统一目标函数整合回归和排序学习，使用排名感知可验证奖励，并引入响应突变操作防止训练停滞

Result: 在三个不同基准测试上进行了广泛实验，验证了RARL的有效性

Conclusion: RARL通过强化学习框架成功解决了序数回归和排序中的序数依赖建模问题，实现了回归和排序任务的协同提升
...
TL;DR: 提出正则化GTD算法（R-GTD），解决传统GTD算法在特征交互矩阵奇异时的不稳定性问题，保证收敛性


<details>
  <summary>Details</summary>
Motivation: 传统梯度时序差分学习算法在离策略评估中广泛使用，但其收敛分析依赖于特征交互矩阵非奇异的限制性假设。实践中该矩阵可能奇异，导致算法不稳定或性能下降。

Method: 通过重新表述均方投影贝尔曼误差最小化问题，提出正则化优化目标，自然导出正则化GTD算法（R-GTD），即使在特征交互矩阵奇异时也能保证收敛到唯一解。

Result: 建立了理论收敛保证和显式误差界，并通过实证实验验证了所提方法的有效性。

Conclusion: R-GTD算法解决了传统GTD算法在特征交互矩阵奇异时的稳定性问题，提供了更强的理论保证和实际性能。
...
TL;DR: 提出Wasserstein-Fisher-Rao平均流匹配框架，用于单细胞生物学中的动态不平衡最优传输，实现无需轨迹模拟的快速一步生成
...
Method: 提出平均流框架，使用平均速度和质量增长场总结任意时间间隔内的传输和质量增长动态；基于此框架开发Wasserstein-Fisher-Rao平均流匹配方法
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成高质量图像，但存在记忆化问题，会无意中复制训练图像。现有基于分数差异范数的检测方法主要在各向同性对数概率分布假设下有效，但在低噪声的各向异性区域效果有限。

Method: 提出新的记忆化检测指标，结合各向同性范数和各向异性对齐（引导向量与无条件分数之间的角度对齐）。该方法只需在纯噪声输入上进行两次前向传播（条件和无条件），无需去噪步骤。

Result: 在Stable Diffusion v1.4和v2上的实验表明，该指标优于现有的免去噪检测方法，速度比之前最佳方法快至少5倍。基于该指标的缓解策略能有效调整记忆化提示词。

Conclusion: 通过分析各向异性区域中引导向量与无条件分数的角度对齐，开发了高效准确的记忆化检测指标，为扩散模型记忆化问题提供了实用的检测和缓解方案。
...
### [124] [Reinforcement Unlearning via Group Relative Policy Optimization](https://arxiv.org/abs/2601.20568)
*Efstratios Zaradoukas,Bardh Prenkaj,Gjergji Kasneci*

Main category: cs.LG

TL;DR: PURGE是一种基于群体相对策略优化的新型大语言模型遗忘方法，通过内在奖励信号惩罚被禁止概念的提及，实现可验证的遗忘，相比现有方法显著提升了效率、流畅性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在预训练过程中会无意记忆敏感或受版权保护的数据，这违反了GDPR和欧盟AI法案等法规要求。现有遗忘方法存在数据泄露、牺牲流畅性和鲁棒性、依赖昂贵外部奖励模型等问题，需要更可靠、高效、可扩展的遗忘技术。

Method: PURGE基于群体相对策略优化框架，将遗忘问题形式化为可验证任务。使用内在奖励信号惩罚任何被禁止概念的提及，实现安全一致的遗忘。该方法不依赖外部奖励模型，通过相对群体优化确保遗忘效果。

Result: PURGE相比最先进方法将每个目标的token使用量减少了46倍，流畅性提高了5.48%，对抗鲁棒性提高了12.02%。在真实世界知识遗忘基准测试中，实现了11%的遗忘效果，同时保留了98%的原始效用。

Conclusion: 将大语言模型遗忘问题形式化为可验证任务，能够实现更可靠、高效、可扩展的遗忘。PURGE展示了结合理论保证、改进安全性和实际部署效率的新研究方向，为合规性遗忘提供了有前景的解决方案。
...
### [125] [Continual GUI Agents](https://arxiv.org/abs/2601.20732)
...
Method: 提出GUI-Anchoring in Flux (GUI-AiF)强化微调框架，包含两种新颖的奖励机制：Anchoring Point Reward in Flux (APR-iF) 和 Anchoring Region Reward in Flux (ARR-iF)。这些奖励引导智能体与变化的交互点和区域对齐，避免过度适应静态基础线索。

Result: 大量实验表明GUI-AiF超越了最先进的基线方法。该工作建立了首个GUI智能体的持续学习框架。

Conclusion: 揭示了强化微调在持续GUI智能体中的未开发潜力，为解决GUI环境动态变化下的智能体性能退化问题提供了有效方案。
...
TL;DR: 提出一种基于上下文感知的运行时监控框架，用于AI控制集成系统，通过监控系统上下文并选择最适合当前条件的控制器，提高安全性和性能。
...
Result: 在两种模拟自动驾驶场景中验证了该框架，相比非上下文基线方法，在安全性和性能方面都取得了显著改进。

Conclusion: 提出的上下文感知监控框架能够识别和利用控制器在不同上下文中的专业优势，提供理论安全保证并改善控制器多样性的利用，为AI控制集成系统提供了更安全有效的解决方案。
...
### [127] [Adapting the Behavior of Reinforcement Learning Agents to Changing Action Spaces and Reward Functions](https://arxiv.org/abs/2601.20714)
...
Method: MORPHIN框架集成了概念漂移检测机制，动态调整学习和探索超参数，能够适应奖励函数变化和动作空间扩展，同时保留先前的策略知识以防止灾难性遗忘。

Result: 在Gridworld基准测试和交通信号控制模拟中，MORPHIN相比标准Q学习基线表现出更优的收敛速度和连续适应能力，学习效率提升高达1.7倍。

Conclusion: MORPHIN框架为强化学习在非平稳环境中的应用提供了有效的自适应解决方案，能够在环境变化时保持高效学习而不丢失先前知识。
...
TL;DR: Hestia是一个Hessian引导的可微分量化感知训练框架，用于极低比特大语言模型，通过温度控制的softmax松弛替代硬量化，结合Hessian迹指标进行细粒度温度退火，在1.58比特量化上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模扩大，内存瓶颈日益突出，需要极低比特量化。但现有量化感知训练方法过早使用硬量化和直通估计器，导致优化景观过早离散化，潜在权重与量化权重之间存在持续梯度不匹配，阻碍量化模型的有效优化。

Method: 提出Hestia框架：1) 用温度控制的softmax松弛替代刚性阶跃函数，在训练早期保持梯度流动，同时逐步硬化量化；2) 利用张量级Hessian迹指标作为轻量级曲率信号，驱动细粒度温度退火，实现跨模型的敏感感知离散化。

Result: 在Llama-3.2模型上的评估显示，Hestia持续优于现有三元量化感知训练基线，1B和3B模型的平均零样本性能分别提升5.39%和4.34%。结果表明Hessian引导的松弛有效恢复了表示能力，为1.58比特大语言模型建立了更鲁棒的训练路径。

Conclusion: Hestia通过Hessian引导的可微分量化框架解决了极低比特量化中的梯度不匹配问题，实现了更有效的量化模型优化，为内存受限环境下的大语言模型部署提供了有前景的解决方案。
...
TL;DR: 提出condPED-ANOVA框架，用于在条件搜索空间中估计超参数重要性，解决了传统方法无法处理条件超参数的问题
...
Result: 实验表明，现有HPI估计器的简单适应在条件设置下会产生误导性或不可解释的重要性估计，而condPED-ANOVA能持续提供反映底层条件结构的有意义重要性

Conclusion: condPED-ANOVA为条件搜索空间提供了原则性的超参数重要性估计框架，解决了传统方法的局限性
...
TL;DR: SDPO是一种利用文本反馈进行强化学习的新方法，通过自我蒸馏将反馈信息转化为密集学习信号，在代码、数学等可验证领域显著提升样本效率和最终准确率。


<details>
  <summary>Details</summary>
Motivation: 当前基于可验证奖励的强化学习方法仅使用标量结果奖励，存在严重的信用分配瓶颈。许多可验证环境实际上提供了丰富的文本反馈（如运行时错误、评估意见），这些反馈解释了失败原因，但现有方法未能充分利用这些信息。

Method: 提出自我蒸馏策略优化（SDPO），将标记化的反馈转化为密集学习信号，无需外部教师或显式奖励模型。SDPO将当前模型在反馈条件下的输出作为自我教师，将其反馈感知的下一个标记预测蒸馏回策略中，利用模型在上下文中回顾性识别自身错误的能力。

Result: 在科学推理、工具使用和LiveCodeBench v6的竞争性编程任务中，SDPO相比强基线方法提高了样本效率和最终准确率。即使在仅返回标量反馈的标准RLVR环境中，SDPO通过使用成功轨迹作为失败尝试的隐式反馈也优于基线。在测试时对单个问题应用SDPO，在困难二元奖励任务中加速了发现过程，以3倍更少的尝试次数实现了与最佳k采样或多轮对话相同的发现概率。

Conclusion: SDPO通过利用丰富的文本反馈解决了RLVR中的信用分配瓶颈，展示了自我蒸馏在强化学习中的有效性，为可验证领域的大语言模型训练提供了更高效的优化方法。
...
TL;DR: 该研究探索了神经常微分方程（NODEs）在复杂系统动力学建模中的外推能力，以及符号回归（SR）从噪声数据中恢复控制方程的能力，展示了NODEs数据增强与符号回归结合用于科学发现的新方法。


<details>
  <summary>Details</summary>
Motivation: 准确建模复杂系统动力学并发现其控制微分方程对于加速科学发现至关重要。研究旨在探索NODEs的外推能力和SR从噪声数据恢复方程的能力，为数据驱动的科学发现提供新方法。

Method: 使用两个阻尼振荡系统的噪声合成数据，首先测试NODEs对新边界条件的外推能力；其次评估SR从噪声真实数据恢复方程的能力；最后将NODE训练于10%的完整模拟数据，用SR从NODE生成数据中恢复方程。

Result: 1. NODEs能有效外推到新边界条件，前提是结果轨迹与训练数据具有动态相似性；2. SR能从噪声真实数据成功恢复方程，但性能取决于输入变量的正确选择；3. 使用仅训练于10%完整模拟数据的NODE生成数据，SR恢复了三个控制方程中的两个，并对第三个给出了良好近似。

Conclusion: 使用NODEs丰富有限数据并让符号回归推断物理定律，代表了科学发现的一种有前景的新方法。虽然最后一个发现指出了未来工作的方向，但整体结果表明这种结合方法具有实际应用潜力。
...
TL;DR: 本文提出了一种名为"失败前缀条件化"的新方法，用于解决强化学习与可验证奖励（RLVR）训练中因问题饱和而停滞的问题。该方法通过基于罕见错误推理轨迹的前缀重新分配探索，让模型接触失败倾向状态，从而在保持标记效率的同时提升性能。


<details>
  <summary>Details</summary>
Motivation: RLVR训练虽然显著提升了大型语言模型的推理能力，但随着问题变得饱和，训练往往会停滞。核心挑战在于信息性失败的可访问性差：学习信号存在但很少在标准rollout中遇到。需要一种方法来有效利用饱和问题中的学习机会。

Method: 提出了失败前缀条件化方法：不是从原始问题开始训练，而是基于从罕见错误推理轨迹中提取的前缀重新分配探索。这使模型暴露于失败倾向状态。还探索了迭代方法，在训练期间刷新失败前缀以突破性能瓶颈。

Result: 失败前缀条件化产生的性能提升与在中等难度问题上训练相当，同时保持标记效率。该方法减少了在误导性失败前缀下的性能退化，尽管在遵循正确早期推理方面有轻微权衡。迭代方法在性能平台期后能解锁额外收益。

Conclusion: 失败前缀条件化为在饱和问题上扩展RLVR训练提供了一条有效途径。该方法通过重新分配探索到失败倾向状态，使模型能够从罕见错误中学习，从而在训练停滞时继续提升性能。
...
### [133] [MuRAL-CPD: Active Learning for Multiresolution Change Point Detection](https://arxiv.org/abs/2601.20686)
...
<details>
  <summary>Details</summary>
Motivation: 传统变化点检测方法通常是完全无监督的，无法适应任务特定的变化定义，也不能利用用户知识。这导致模型检测到的变化点可能与用户实际关心的变化不一致。

Method: 提出MuRAL-CPD方法，结合小波多分辨率分解检测多个时间尺度上的变化，并集成主动学习机制，通过用户反馈迭代优化关键超参数，使模型与用户对变化的理解对齐。

Result: 在多个真实世界数据集上的实验表明，MuRAL-CPD相比最先进方法具有显著优势，特别是在只有少量监督可用的场景下表现突出。

Conclusion: MuRAL-CPD通过半监督主动学习方法成功解决了传统变化点检测方法缺乏适应性和无法利用用户知识的问题，提高了检测准确性和可解释性。
...
### [134] [Reward Models Inherit Value Biases from Pretraining](https://arxiv.org/abs/2601.20838)
...
TL;DR: 研究发现奖励模型（RMs）的行为显著受其基础大语言模型（LLMs）影响，即使使用相同的偏好数据和微调过程，Llama和Gemma基础模型分别表现出对"能动性"和"共融性"的稳定偏好，这表明基础模型选择本身就是一种价值选择。


<details>
  <summary>Details</summary>
Motivation: 奖励模型在将大语言模型与人类价值观对齐中至关重要，但相对于预训练和微调模型本身，其研究关注较少。奖励模型从基础LLMs初始化，继承了其表征，但这种影响的性质和程度尚未得到充分研究。

Method: 对10个领先的开源权重奖励模型进行综合研究，使用经过验证的心理语言学语料库，分析它们在人类价值观多个维度上的差异。通过"大二"心理轴（能动性与共融性）分析模型偏好，追踪到指令微调和预训练模型的logits差异，并推导出可用的隐式奖励分数。

Result: 研究发现奖励模型的行为显著受基础模型影响：Llama基础模型表现出对"能动性"的稳定偏好，而Gemma基础模型表现出对"共融性"的稳定偏好。这种差异即使在偏好数据和微调过程完全相同的情况下依然存在，且通过消融实验证明这种效应是可重复且持久的。

Conclusion: 尽管奖励模型旨在代表人类偏好，但其输出受到所基于的预训练LLMs的影响。这项工作强调了预训练阶段安全和对齐工作的重要性，并明确指出开源开发者选择基础模型既是性能考虑，也是价值观选择。
...
### [135] [Positive-Unlabeled Reinforcement Learning Distillation for On-Premise Small Models](https://arxiv.org/abs/2601.20687)
...
TL;DR: 提出了一种适用于本地部署小模型的PU强化学习蒸馏方法，无需人工偏好标注或奖励模型，通过教师模型的单次查询和本地自排序实现偏好优化


<details>
  <summary>Details</summary>
Motivation: 本地部署小模型日益普遍，但实际应用通常只进行监督微调而无法进行强化学习对齐，因为RL对齐需要昂贵的人工偏好标注或依赖大规模API使用的高质量奖励模型，这些都不适合本地部署环境

Method: 提出正例-未标注强化学习蒸馏方法：对每个提示，查询教师模型一次获得锚定响应，本地采样多个学生候选响应，进行锚定条件自排序以生成成对或列表偏好，通过直接偏好优化或组相对策略优化实现完全本地训练循环

Result: 理论分析表明该方法诱导的偏好信号具有顺序一致性并集中在近最优候选上，支持偏好优化的稳定性。实验证明在低成本设置下该方法能实现持续强劲的性能

Conclusion: 该方法填补了本地部署小模型在强化学习对齐方面的空白，无需人工偏好标注或奖励模型，通过教师模型的单次查询和本地自排序实现了有效的偏好优化
...
### [136] [$\mathbb{R}^{2k}$ is Theoretically Large Enough for Embedding-based Top-$k$ Retrieval](https://arxiv.org/abs/2601.20844)
*Zihao Wang,Hang Yin,Lihui Liu,Hanghang Tong,Yangqiu Song,Ginny Wong,Simon See*

Main category: cs.LG

TL;DR: 本文研究了将m个元素及其k元素子集嵌入向量空间所需的最小维度(MED)，推导了不同距离/相似度度量下的紧致边界，并通过数值模拟验证了对数依赖关系，表明嵌入检索的限制主要来自可学习性问题而非几何约束。


<details>
  <summary>Details</summary>
Motivation: 研究嵌入式检索系统的根本限制，区分几何约束与可学习性挑战，为未来算法设计提供理论指导。

Method: 理论推导最小可嵌入维度(MED)的紧致边界，涵盖ℓ₂距离、内积和余弦相似度等不同度量；数值模拟在更实际场景下验证结果，其中子集嵌入采用包含元素的质心表示。

Result: MED与元素数量m呈对数依赖关系，理论边界得到实证支持；模拟实验轻松实现对数依赖，表明几何约束并非主要瓶颈。

Conclusion: 嵌入式检索的限制主要源于可学习性挑战而非几何约束，这一发现为未来算法设计提供了重要指导方向。
...
### [137] [Optimal Transport Group Counterfactual Explanations](https://arxiv.org/abs/2601.20692)
...
<details>
  <summary>Details</summary>
Motivation: 现有群体反事实解释方法存在三个主要问题：(1) 只能针对固定群体优化，无法泛化到新成员；(2) 严格依赖强模型假设（如线性）以保证可解性；(3) 对反事实群体几何失真的控制能力差。需要一种能够泛化、减少参数、易于解释共同可行动方案的方法。
...
Conclusion: 提出的学习显式最优传输映射的方法解决了现有群体反事实解释方法的局限性，实现了更好的泛化能力、更少的参数需求、更好的几何结构保持，并为解释共同可行动方案提供了更优的框架。
...
### [138] [Is Pure Exploitation Sufficient in Exogenous MDPs with Linear Function Approximation?](https://arxiv.org/abs/2601.20694)
*Hao Liang,Jiayu Cheng,Sean R. Sinclair,Yali Du*

Main category: cs.LG

TL;DR: 本文证明了在外生MDPs中，纯利用算法无需探索即可获得有限样本遗憾界，颠覆了传统认为探索必要的观点。


<details>
  <summary>Details</summary>
Motivation: 外生MDPs（Exo-MDPs）捕捉了不确定性仅来自外生输入（如需求、到达、价格）的序列决策问题，这在库存控制、能源存储等运筹学应用中很常见。尽管经验证据表明贪婪的纯利用方法在这些设置中效果很好，但现有理论都依赖于显式探索或表格假设。本文旨在证明探索是不必要的。

Method: 提出了纯利用学习（PEL）算法。对于表格情况，PEL实现了$\widetilde{O}(H^2|Ξ|\sqrt{K})$的遗憾界。对于大型连续内生状态空间，引入了LSVI-PE，一种简单的线性近似方法。分析引入了两个新工具：反事实轨迹和Bellman闭特征传输，使贪婪策略无需乐观主义就能获得准确的价值估计。

Result: 在表格情况下，PEL实现了$\widetilde{O}(H^2|Ξ|\sqrt{K})$的遗憾界。对于连续状态空间，LSVI-PE的遗憾界在特征维度、外生状态空间和视野上是多项式的，且独立于内生状态和动作空间。实验表明PEL在合成和资源管理任务上始终优于基线方法。

Conclusion: 本文结果颠覆了传统认为探索必要的观点，证明了在外生MDPs中，纯利用就足够了。这为运筹学中的序列决策问题提供了新的理论保证和实践指导。
...
### [139] [Post-Training Fairness Control: A Single-Train Framework for Dynamic Fairness in Recommendation](https://arxiv.org/abs/2601.20848)
...
<details>
  <summary>Details</summary>
Motivation: 现有公平感知方法通常在训练时固定公平性要求，缺乏训练后的灵活性。现实场景中不同利益相关者可能有不同的公平性需求，重新训练成本过高。

Method: 提出Cofair框架，包含带有公平条件适配器模块的共享表示层，生成针对不同公平性水平的用户嵌入，以及用户级正则化项确保跨公平性水平的单调公平性改进。

Result: 在多个数据集和骨干模型上的实验表明，Cofair能够提供不同水平的动态公平性，在公平性-准确性曲线上达到或优于最先进的基线方法，无需为每个新公平性要求重新训练。

Conclusion: Cofair通过单次训练实现训练后的公平性控制，为推荐系统中的动态公平性需求提供了实用解决方案，理论分析和实验验证了其有效性。
...
### [140] [Exploring Transformer Placement in Variational Autoencoders for Tabular Data Generation](https://arxiv.org/abs/2601.20854)
*Aníbal Silva,Moisés Santos,André Restivo,Carlos Soares*

Main category: cs.LG

TL;DR: 该研究探讨了在VAE中集成Transformer架构对表格数据生成的影响，发现Transformer在潜在空间和解码器中的位置会影响生成质量与多样性的权衡，且Transformer连续块之间存在高度相似性。


<details>
  <summary>Details</summary>
Motivation: 表格数据对生成模型具有挑战性，传统的基于多层感知机的VAE难以建模特征间关系，而Transformer的注意力机制更适合捕捉复杂特征交互。研究旨在实证探索Transformer集成到VAE不同组件中的效果。

Method: 在OpenML CC18套件的57个数据集上进行实验，研究将Transformer集成到VAE不同组件（编码器、解码器、潜在空间）的影响，分析不同架构配置的效果。

Result: 1. Transformer在潜在空间和解码器中的位置会导致生成质量（保真度）与多样性之间的权衡；2. 在所有组件中，Transformer的连续块之间显示出高度相似性，特别是在解码器中，Transformer的输入输出关系近似线性。

Conclusion: Transformer可以改善VAE对表格数据的建模能力，但需要仔细考虑其在架构中的位置，因为这会影响到生成质量与多样性的平衡。Transformer块间的相似性表明可能存在架构简化的机会。
...
### [141] [Evolutionary Strategies lead to Catastrophic Forgetting in LLMs](https://arxiv.org/abs/2601.20861)
*Immanuel Abdi,Akshat Gupta,Micah Mok,Alexander Lu,Nicholas Lee,Gopala Anumanchipalli*

Main category: cs.LG

TL;DR: 进化策略(ES)作为无梯度替代方案在LLM训练中表现接近GRPO，但在持续学习中存在严重的遗忘问题，更新稀疏度低且范数大导致性能下降
...
Method: 对进化策略进行综合分析，评估其在增加更新步数时的遗忘曲线，与GRPO在数学和推理任务上比较计算预算，分析更新稀疏度和ℓ₂范数差异

Result: ES在数学和推理任务上能达到接近GRPO的性能，但伴随显著的先前能力遗忘；ES更新比GRPO更新稀疏度低且ℓ₂范数大几个数量级，解释了两种算法遗忘曲线的差异

Conclusion: 进化策略在持续学习中存在严重遗忘问题，限制了其在在线训练模型中的应用，需要未来研究缓解这些问题
...
TL;DR: 提出SA-PEF方法，结合步前校正和部分误差反馈，解决联邦学习中非IID数据下梯度压缩的早期收敛停滞问题，实现更快的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 在联邦学习中，带有误差反馈的梯度压缩可以减少通信开销，但在非IID数据下，残差误差衰减缓慢，导致早期训练轮次出现梯度失配和进展停滞问题。

Method: 提出步前部分误差反馈(SA-PEF)，结合步前校正和部分误差反馈。SA-PEF在步前系数α=0时恢复为EF，α=1时恢复为步前EF(SAEF)。方法针对非凸目标和δ-压缩器建立二阶矩边界和残差递归。

Result: 理论分析表明SA-PEF在异构数据和部分客户端参与下能收敛到平稳点，收敛率与标准非凸Fed-SGD保证匹配，达到O((η,η₀TR)⁻¹)收敛到方差/异构性下界。实验显示SA-PEF比EF更快达到目标精度。

Conclusion: SA-PEF通过步前控制的残差收缩ρᵣ解释了早期训练阶段的加速现象，通过选择接近理论最优的α值，平衡了SAEF的快速预热和EF的长期稳定性，在多种架构和数据集上表现优于EF。
...
TL;DR: 该论文提出了一种在无限维函数空间中基于分数扩散模型的条件化方法，用于贝叶斯逆问题的后验采样，通过Doob's h-变换的理论框架和监督引导训练实现高效稳定的后验采样。


<details>
  <summary>Details</summary>
Motivation: 分数扩散模型已被扩展到无限维函数空间，用于偏微分方程等逆问题。在贝叶斯逆问题中，目标是通过对先验分布进行条件化来从后验分布中采样。虽然扩散模型提供了函数空间中的表达能力强的先验，但如何条件化它们以从后验中采样的理论仍然开放。

Method: 假设先验位于Cameron-Martin空间或相对于高斯测度绝对连续，证明了可以使用Doob's h-变换的无限维扩展来条件化模型，条件分数分解为无条件分数和引导项。由于引导项难以处理，提出了监督引导训练的无模拟分数匹配目标，实现高效稳定的后验采样。

Result: 提出了第一个函数空间方法，用于微调训练好的扩散模型以准确从后验中采样，并在函数空间的贝叶斯逆问题上进行了数值示例验证。

Conclusion: 该工作为函数空间中扩散模型的条件化提供了理论框架和实用方法，解决了贝叶斯逆问题中后验采样的挑战，首次实现了在函数空间中微调扩散模型以准确采样后验分布。
...
TL;DR: 论文提出C^4方法解决离线强化学习中的分布偏移问题，通过分区缓冲采样和梯度修正惩罚来抑制TD交叉协方差的有害影响，在OOD区域表现更好。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习面临分布偏移的根本挑战，稀缺数据或由分布外(OOD)区域主导的数据集加剧了这一问题。标准平方误差目标会诱导有害的TD交叉协方差，在OOD区域放大这种效应，导致优化偏差和政策学习退化。

Method: 提出两种互补策略：1) 分区缓冲采样，将更新限制在局部回放分区中，减弱不规则协方差效应并对齐更新方向，形成易于集成到现有实现的C^4方案；2) 显式梯度修正惩罚，在每次更新中消除协方差诱导的偏差。证明缓冲分区保留了最大化目标的下界性质。

Result: 方法在极端OOD区域减轻了过度保守性而不改变政策约束离线强化学习的核心行为。实验显示更高的稳定性，相比先前方法回报提升达30%，特别是在小数据集和强调OOD区域的数据分割上表现突出。

Conclusion: C^4方法通过控制TD交叉协方差有效缓解了离线强化学习中的分布偏移问题，在OOD区域表现出更好的稳定性和性能，为小数据集场景提供了有效的解决方案。
...
TL;DR: 论文提出基于距离的黑盒模型复制框架，将硬标签监督转换为到教师模型决策边界的带符号距离，从而将复制问题转化为平滑回归问题，提高了复制效率和边界几何恢复能力。


<details>
  <summary>Details</summary>
Motivation: 实际部署的机器学习系统需要持续演进，但往往无法访问原始训练数据或模型内部结构。在黑盒设置下，仅通过输入-输出查询复制模型时，硬标签输出将复制问题转化为不连续的表面重建问题，严重限制了边界几何恢复的效率。

Method: 提出基于距离的蒸馏框架，用带符号距离替代硬标签监督；开发α控制的平滑和正则化方案，对诱导目标表面进行Hölder/Lipschitz控制；引入两种模型无关算法，在仅标签访问条件下估计带符号距离。

Result: 在合成问题和UCI基准测试中，相比硬标签基线，该方法在保真度和泛化准确性方面表现出一致的改进，同时能够输出距离作为黑盒副本的不确定性相关信号。

Conclusion: 基于距离的复制框架通过将硬标签监督转换为平滑回归问题，有效解决了黑盒模型复制中的边界几何恢复难题，为实际部署中的模型升级提供了更高效的机制。
...
### [146] [When More Data Doesn't Help: Limits of Adaptation in Multitask Learning](https://arxiv.org/abs/2601.20774)
*Steve Hanneke,Mingyue Xu*

Main category: cs.LG

TL;DR: 该论文超越现有的无免费午餐定理，证明了多任务学习中更强的不可适应性结果，表明即使每个任务有大量数据，多任务学习的困难也无法克服。


<details>
  <summary>Details</summary>
Motivation: 多任务学习框架在现代应用中取得了巨大成功，但arXiv:2006.15785的工作表明，在没有分布信息的情况下，仅基于样本聚合的算法无法保证最优风险。本文旨在超越该结果，深入理解多任务学习的统计极限。

Method: 通过建立更强的不可适应性结果来超越现有的无免费午餐定理，该结果适用于任意大的每个任务样本量，并讨论可能对未来研究有意义的优化适应性概念。

Result: 证明了多任务学习中存在更强的不可适应性，即即使每个任务有大量数据，多任务学习的困难也无法克服，这比之前仅针对有限样本量的结果更为严格。

Conclusion: 多任务学习的统计极限比之前认为的更为严格，即使有丰富的每个任务数据，也无法克服其固有困难。论文还提出了优化适应性的概念，为未来研究提供了方向。
...
TL;DR: 该论文首次分析了决策树作为二元分类器的主动学习标签复杂度，提出了在特定假设下实现多对数标签复杂度的理论框架，并设计了首个具有乘法误差保证的主动学习算法。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏对决策树主动学习标签复杂度的理论理解，特别是决策树分歧系数的分析以及实现多对数标签复杂度的条件。需要建立理论框架来理解在何种条件下决策树可以实现高效的主动学习。

Method: 1. 首次分析决策树的分歧系数（关键参数）；2. 提出两个自然假设：每个根到叶路径查询不同特征维度，输入数据具有规则的网格结构；3. 设计首个具有乘法误差保证的通用主动学习算法，产生(1+ε)-近似分类器；4. 结合这些结果设计针对决策树的主动学习算法。

Result: 1. 证明了在所述假设下，决策树可以实现多对数标签复杂度；2. 展示了放松假设会导致多项式标签复杂度；3. 设计的算法在数据集大小上仅使用多对数数量的标签查询；4. 建立了标签复杂度的下界，证明算法对误差容忍度ε的依赖接近最优。

Conclusion: 该研究为决策树的主动学习提供了首个完整的理论分析框架，明确了实现高效主动学习的条件，设计了接近最优的算法，填补了该领域的理论空白，为实际应用提供了理论基础。
...
### [148] [PatchFormer: A Patch-Based Time Series Foundation Model with Hierarchical Masked Reconstruction and Cross-Domain Transfer Learning for Zero-Shot Multi-Horizon Forecasting](https://arxiv.org/abs/2601.20845)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列预测在气候、能源、医疗和金融等领域具有广泛应用，但现有方法通常需要领域特定的特征工程和大量标注数据。需要开发一个能够跨领域通用、减少对标注数据依赖的时间序列基础模型。

Method: 1. 将时间序列分割成patch，学习多尺度时间表示；2. 使用动态掩码的分层掩码重建进行自监督预训练；3. 采用跨领域知识蒸馏；4. 使用轻量级适配器进行高效迁移学习。

Result: 在24个基准数据集（天气、能源、交通、金融、医疗）上实现了最先进的零样本多步预测性能，相对强基线平均平方误差降低27.3%，所需任务特定训练数据减少94%。模型在1000亿数据点上呈现近对数线性扩展，处理长度512序列比全序列Transformer快3.8倍。

Conclusion: PatchFormer是一个有效的时间序列基础模型，通过自监督预训练和轻量级适配器实现了跨领域的强大预测能力，显著减少了对标注数据的依赖，具有良好的可扩展性和计算效率。
...
### [149] [E2HiL: Entropy-Guided Sample Selection for Efficient Real-World Human-in-the-Loop Reinforcement Learning](https://arxiv.org/abs/2601.19969)
*Haoyuan Deng,Yuanjiang Xue,Haoyang Du,Boyang Zhou,Zhenyu Wu,Ziwei Wang*

Main category: cs.RO

TL;DR: 提出了一种名为E2HIL的样本高效人机交互强化学习框架，通过主动选择信息丰富的样本来减少人类干预需求，在真实世界操作任务中实现了更高的成功率和更低的人工成本。


<details>
  <summary>Details</summary>
Motivation: 现有的人机交互强化学习框架样本效率低下，需要大量人类干预才能收敛，导致高昂的劳动力成本。需要一种更高效的框架来减少人类干预需求。

Method: 提出E2HIL框架，通过稳定降低策略熵来改善探索与利用的权衡。首先构建不同样本对策略熵的影响函数，通过动作概率和策略软优势的协方差高效估计。然后选择具有适度影响函数值的样本，剔除导致熵急剧下降的捷径样本和影响可忽略的噪声样本。

Result: 在四个真实世界操作任务上的实验表明，E2HIL相比最先进的人机交互强化学习方法，成功率提高了42.1%，同时减少了10.1%的人类干预。

Conclusion: E2HIL框架通过主动选择信息丰富的样本，实现了更高效的样本利用和更少的人类干预，为真实世界人机交互强化学习提供了有效的解决方案。
...
### [150] [Just in time Informed Trees: Manipulability-Aware Asymptotically Optimized Motion Planning](https://arxiv.org/abs/2601.19972)
...
<details>
  <summary>Details</summary>
Motivation: 传统采样方法在高维机器人路径规划中难以高效识别复杂多障碍环境中的可行且最优路径，特别是对于机器人操纵器，运动奇点和自碰撞风险进一步影响运动效率和安全。

Method: 提出JIT*算法，包含两个核心模块：即时模块（含即时边动态优化连接性和即时样本调整瓶颈区域采样密度）和运动性能模块（通过动态切换平衡可操作性和轨迹成本）。

Result: JIT*在R^4到R^16维度上持续优于传统采样规划器，在单臂和双臂操作任务中表现出有效性，实验视频已发布。

Conclusion: JIT*算法通过动态优化连接性和采样密度，结合运动性能平衡，显著提升了高维复杂环境中机器人路径规划的效率和安全性。
...
### [151] [A Taylor Series Approach to Correct Localization Errors in Robotic Field Mapping using Gaussian Processes](https://arxiv.org/abs/2601.20149)
*Muzaffar Qureshi,Tochukwu Elijah Ogri,Kyle Volle,Rushikesh Kamalapurkar*

Main category: cs.RO

TL;DR: 提出一种基于二阶修正的高斯过程模型更新方法，用于处理移动机器人定位误差导致的测量位置不确定性，提高标量场映射的预测精度和计算效率。


<details>
  <summary>Details</summary>
Motivation: 现实世界中标量场映射应用通常依赖移动机器人收集测量数据，但机器人定位不完美会引入状态不确定性，导致测量位置估计与真实位置存在差异，这会降低高斯过程模型的均值和协方差估计精度。

Method: 利用核函数的可微性，开发了基于预计算雅可比矩阵和海森矩阵的二阶修正算法，当获得改进的位置估计时，能够实时修正高斯过程模型，而无需完全重新训练模型。

Result: 仿真结果表明，与完全重新训练模型相比，该方法在预测精度和计算效率方面都有显著提升。

Conclusion: 该方法有效解决了移动机器人定位不确定性对高斯过程模型性能的影响，提供了一种高效实时的模型更新机制，适用于需要处理测量位置误差的标量场映射应用。
...
### [152] [TRACER: Texture-Robust Affordance Chain-of-Thought for Deformable-Object Refinement](https://arxiv.org/abs/2601.20208)
*Wanjun Jia,Kang Li,Fan Yang,Mengfei Duan,Wenrui Chen,Yiming Jiang,Hui Zhang,Kailun Yang,Zhiyong Li,Yaonan Wang*

Main category: cs.RO

TL;DR: TRACER是一个纹理鲁棒的仿效链式思维框架，通过跨层次映射将高层语义推理与物理一致的功能区域细化相结合，解决可变形物体操作中边界溢出和功能区域碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 可变形物体操作的核心挑战在于高层语义指令与物理交互点的对齐，现有基于视觉的仿效预测方法常因边界溢出和功能区域碎片化而失效，特别是在复杂外观和纹理变化下。

Method: 提出TRACER框架：1) 树状仿效链式思维(TA-CoT)将高层任务意图分解为层次化子任务语义；2) 空间约束边界细化(SCBR)机制抑制预测溢出；3) 交互收敛细化流(ICRF)聚合被外观噪声破坏的离散像素。

Result: 在Fine-AGDDO15数据集和真实机器人平台上进行广泛实验，TRACER显著提高了仿效定位精度，增强了长时域任务的成功率，有效弥合了高层语义推理与低层物理执行之间的差距。

Conclusion: TRACER通过跨层次映射建立了从语义推理到物理一致功能区域细化的桥梁，显著提升了可变形物体操作中仿效预测的鲁棒性和准确性，为机器人操作提供了更可靠的解决方案。
...
### [153] [TouchGuide: Inference-Time Steering of Visuomotor Policies via Touch Guidance](https://arxiv.org/abs/2601.20239)
...
TL;DR: TouchGuide：一种新颖的跨策略视觉-触觉融合范式，通过两阶段推理过程引导预训练的扩散或流匹配视觉运动策略，利用触觉反馈改进接触丰富的精细操作任务。


<details>
  <summary>Details</summary>
Motivation: 当前机器人精细操作和接触丰富的任务仍然具有挑战性，主要原因是触觉反馈的利用不足。为了解决这个问题，需要开发能够有效融合视觉和触觉信息的策略。

Method: TouchGuide采用两阶段推理过程：第一阶段使用视觉输入生成粗略的视觉合理动作；第二阶段通过任务特定的接触物理模型（CPM）提供触觉指导，使用对比学习在有限专家演示上训练，提供触觉感知的可行性分数来引导采样过程。同时开发了TacUMI数据收集系统，利用刚性指尖获取直接触觉反馈。

Result: 在五个具有挑战性的接触丰富任务（如系鞋带和芯片交接）上的广泛实验表明，TouchGuide始终显著优于最先进的视觉-触觉策略。

Conclusion: TouchGuide通过创新的跨策略视觉-触觉融合范式，有效解决了机器人精细操作中触觉反馈利用不足的问题，在接触丰富的任务中表现出色，同时TacUMI系统为高质量触觉数据收集提供了经济有效的解决方案。
...
TL;DR: FAEA将前沿LLM智能体框架直接应用于机器人操作，无需任务演示或微调，在多个基准测试中达到接近VLA模型的性能，展示了通用智能体在需要深思熟虑任务规划的操作任务中的潜力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型需要任务特定演示和微调，且领域迁移能力差。研究是否可以将原本为软件工程开发的通用大型语言模型智能体框架作为机器人操作的控制范式替代方案。

Method: 引入FAEA，将LLM智能体框架（Claude Agent SDK）直接应用于机器人操作，无需修改。利用软件智能体调试代码的迭代推理能力，让机器人智能体推理操作策略。在LIBERO、ManiSkill3和MetaWorld基准上进行评估。

Result: 在拥有环境状态特权访问的情况下，FAEA在LIBERO、ManiSkill3和MetaWorld上分别达到84.9%、85.7%和96%的成功率。这接近每个任务使用少于100个演示训练的VLA模型性能，且无需演示或微调。加入一轮人类反馈优化后，LIBERO性能提升至88.2%。

Conclusion: 通用智能体足以处理一类以深思熟虑任务规划为主的机器人操作任务，为机器人系统利用前沿智能体基础设施和模型进步开辟了新路径。无演示能力具有实际价值，可用于自主探索新场景和生成训练数据增强。
...
TL;DR: RF-MatID是首个开源、大规模、宽频带、几何多样化的射频数据集，用于细粒度材料识别，包含16个细粒度类别、142k样本，覆盖4-43.5GHz频段，并建立了多设置、多协议的深度学习基准测试。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉的材料识别受光学传感器固有局限，而基于射频的方法能揭示材料内在特性但缺乏大规模公开数据集和学习方法的基准测试，阻碍了该领域发展。

Method: 创建RF-MatID数据集，包含16个细粒度材料类别（分为5个超类），覆盖4-43.5GHz宽频段，142k个频域和时域样本，系统引入入射角和距离等几何扰动。建立多设置、多协议基准，评估最先进深度学习模型，包括分布内性能和跨角度、跨距离的分布外鲁棒性。

Result: 提供了首个开源、大规模、宽频带、几何多样化的射频材料识别数据集，建立了包含5种频率分配协议的系统基准，支持频率和区域级分析，促进实际部署。
...
TL;DR: 本文提出了一个用于月球多机器人勘探任务的关键绩效指标（KPI）评估框架，旨在解决现有野外试验因平台和实验设置差异而难以比较的问题，强调效率、鲁棒性和精度的场景依赖性优先级。


<details>
  <summary>Details</summary>
Motivation: 月球资源勘探（如钛铁矿、稀土元素、水冰）需要可靠的机器人探索方法，但现有的野外模拟试验由于机器人平台和实验设置的差异，结果难以比较。这些任务通常使用特定场景的工程指标评估性能，未能建立野外性能与科学目标之间的明确联系。

Method: 从三个现实的多机器人月球场景中推导出结构化的KPI框架，这些场景反映了科学目标和操作约束。该框架强调效率、鲁棒性和精度的场景依赖性优先级，并专门设计用于野外部署的实际应用。通过多机器人野外测试验证了该框架。

Result: 在多机器人野外测试中验证了该框架，发现效率和鲁棒性相关的KPI实用且易于应用，而精度导向的KPI需要可靠的基准数据，这在户外模拟环境中并非总是可行。
...
TL;DR: 利用低成本压电麦克风和音频谱图Transformer，通过振动声学感知实现高精度全身触觉定位，静态条件下定位误差小于5mm，为机器人提供经济可扩展的触觉感知方案。
...
Method: 在机器人手上安装7个低成本压电麦克风，利用音频谱图Transformer解码物理交互产生的振动特征，通过振动声学感知实现触觉定位。

Result: 静态条件下定位误差小于5mm；不同材料特性影响显著：刚性材料（如金属）在脉冲响应定位中表现优异，纹理材料（如木材）在轨迹跟踪中提供更好的摩擦特征；系统对机器人自身运动具有鲁棒性。

Conclusion: 复杂物理接触动力学可以从简单的振动信号中有效解码，为机器人领域提供了经济可扩展的触觉感知途径。研究提供完整数据集、模型和实验设置作为开源资源。
...
Method: 提出MeCo框架，引入相似性测试方法检索高相关性的已解决任务，实现无需重新调用LLM的有效计划重用

Result: 实验结果显示，MeCo显著降低了规划成本并提高了成功率，优于现有最先进方法

Conclusion: MeCo通过相似性感知和缓存重用机制，有效解决了多机器人协作中的重复计算问题，提高了系统的适应性和效率
...
TL;DR: GPO是一种用于足式机器人强化学习训练的新框架，通过时变动作变换限制早期动作空间，促进有效数据收集，然后逐步扩展动作空间以增强探索。


<details>
  <summary>Details</summary>
Motivation: 足式机器人的强化学习训练面临高维连续动作、硬件限制和探索有限等挑战。现有方法在位置控制中表现良好，但在力矩控制中效果较差，因为力矩控制需要更有效的动作空间探索和梯度信号获取。

Method: 提出Growing Policy Optimization (GPO)框架，应用时变动作变换来限制早期有效动作空间，促进更有效的数据收集和策略学习，然后逐步扩展动作空间以增强探索。

Result: 在四足和六足机器人上评估GPO，包括将仿真训练策略零样本部署到硬件上。GPO训练的策略始终获得更好的性能。

Conclusion: GPO为学习足式运动提供了一个通用、环境无关的优化框架，通过时变动作变换解决了力矩控制中的探索难题。
...
TL;DR: 提出一种基于肌腱位移和张力估计关节位置的计算方法，用于无关节编码器的仿人机器人手，通过非线性优化和雅可比PI控制器实现闭环控制


<details>
  <summary>Details</summary>
Motivation: 肌腱驱动的仿人机器人手通常缺乏直接关节角度传感，因为集成关节编码器会影响机械紧凑性和灵巧性，需要一种替代的关节位置估计方法

Method: 1) 基于Denavit-Hartenberg约定建立高效的仿人手机械运动学模型；2) 使用简化的肌腱模型推导肌腱状态与关节位置的非线性方程组；3) 通过非线性优化方法求解关节角度；4) 采用雅可比比例积分控制器加前馈项进行闭环控制

Result: 在MuJoCo仿真环境中使用解剖学正确的生物机电手（每个长指5自由度，拇指6自由度）验证了所提估计和控制框架的有效性和局限性

Conclusion: 该方法能够在没有直接关节传感的情况下实现手势跟踪，为肌腱驱动仿人手机器人提供了一种可行的关节位置估计和控制解决方案
...
TL;DR: DMPO提出了一种单步生成策略框架，通过MeanFlow数学推导实现无需知识蒸馏的单步推理，结合分散正则化和RL微调，在保持性能的同时实现5-20倍推理加速，满足实时控制需求(>120Hz)。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散和流匹配的生成策略需要多步采样，这在时间关键场景中部署受限。实时机器人控制需要快速动作生成，因此需要开发能够实现真正单步生成的方法。

Method: DMPO包含三个关键组件：1) MeanFlow - 通过数学推导实现无需知识蒸馏的单步推理；2) 分散正则化 - 防止表示崩溃；3) RL微调 - 超越专家演示。采用轻量级模型架构，三个算法组件协同工作。

Result: 在RoboMimic操作和OpenAI Gym运动基准测试中，DMPO表现与多步基线相当或更优。实现5-20倍推理加速，满足实时控制要求(>120Hz)，在高性能GPU上可达数百赫兹。在Franka-Emika-Panda机器人上的物理部署验证了实际应用性。

Conclusion: DMPO通过统一的单步生成框架解决了实时机器人控制中的动作生成速度瓶颈，在保持性能的同时显著提升推理速度，为时间关键场景提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前自主机器人系统需要更高效和智能的任务执行能力，特别是在复杂任务规划中。知识图谱能够增强机器人的决策能力和任务表现，但需要系统化的方法在ROS 2环境中实现。

Method: 提出分步方法：1) 定义初始和目标条件；2) 结构化任务和子任务；3) 规划任务序列；4) 在知识图谱中表示任务相关数据；5) 使用高级语言设计任务。在Aerostack2框架中实现，并在Gazebo环境中进行模拟搜救任务验证。

Result: 通过模拟搜救任务（无人机自主定位目标）验证了方法的有效性。知识图谱的运用显著提升了决策能力和任务性能，证明了该方法在增强机器人系统智能性方面的价值。

Conclusion: 提出的知识图谱实现方法能够有效提升ROS 2系统中自主机器人任务的效率和智能性。该方法为复杂机器人任务提供了系统化的解决方案，并通过实际应用验证了其可行性。
...
TL;DR: 提出一种基于神经风格迁移的sim-to-real强化学习策略转移方法，通过变分自编码器学习特征表示并生成弱配对轨迹，在机器人切割未知材料任务中表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 强化学习在复杂不确定环境中的机器人控制应用受到仿真与物理系统领域差距的限制，且真实世界样本有限。需要一种能够利用未配对未标记真实世界数据的方法来缩小sim-to-real差距。

Method: 将神经风格迁移从图像处理重新解释到强化学习领域，使用变分自编码器联合学习自监督特征表示，进行风格迁移并生成弱配对的源-目标轨迹，提高合成轨迹的物理真实性。

Result: 在机器人切割未知材料案例研究中，相比基线方法（包括作者先前工作、CycleGAN和条件变分自编码器时间序列翻译），该方法实现了改进的任务完成时间和行为稳定性，仅需最少真实世界数据。框架对几何和材料变化具有鲁棒性。

Conclusion: 该方法展示了在真实世界奖励信息不可用的挑战性接触丰富任务中策略适应的可行性，为sim-to-real强化学习转移提供了一种有效的新方法。
